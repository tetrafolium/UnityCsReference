...
namespace Unity.SerializationLogic {
  ...
  public class TypeResolver {
    ...
    private readonly Dictionary<string, GenericInstanceHolder> _context =
        new Dictionary<string, GenericInstanceHolder>();
    ...
    public TypeResolver(IGenericInstance typeDefinitionContext,
                        IGenericInstance methodDefinitionContext) {
      _typeDefinitionContext = typeDefinitionContext;
      _methodDefinitionContext = methodDefinitionContext;
    }
    ...
    public void Add(GenericInstanceMethod genericInstanceMethod) {
      Add(ElementTypeFor(genericInstanceMethod).FullName,
          genericInstanceMethod);
    }

    private static MemberReference
    ElementTypeFor(TypeSpecification genericInstanceType) {
      return genericInstanceType.ElementType;
    }

    private static MemberReference
    ElementTypeFor(MethodSpecification genericInstanceMethod) {
      return genericInstanceMethod.ElementMethod;
    }

    public void Remove(GenericInstanceMethod genericInstanceMethod) {
      Remove(genericInstanceMethod.ElementMethod.FullName,
             genericInstanceMethod);
    }
    ...
    public TypeReference Resolve(TypeReference typeReference) {
      ...
      if (genericParameter != null) {
        ...
        if (genericParameter ==
            resolved) // Resolving failed, return what we have.
          return resolved;
      ...
      if (genericInstanceType != null) {
        var newGenericInstanceType =
            new GenericInstanceType(Resolve(genericInstanceType.ElementType));
      ...
      if (optModifierType != null)
        return new OptionalModifierType(Resolve(optModifierType.ModifierType),
                                        Resolve(optModifierType.ElementType));
    ...
    ResolveGenericParameter(GenericParameter genericParameter) {
      ...
      if (!_context.ContainsKey(key)) {
        ...
        if (_methodDefinitionContext != null)
          return _methodDefinitionContext
              .GenericArguments[genericParameter.Position];
    ...
    private void Add(string key, IGenericInstance value) {
      ...
      _context.Add(
          key, new GenericInstanceHolder{Count = 1, GenericInstance = value});
    }
  ...
  public static class UnitySerializationLogic {
    ...
    public static bool WillUnitySerialize(FieldDefinition fieldDefinition,
      ...
      // skip static, const and NotSerialized fields before even checking the
      // type
      if (fieldDefinition.IsStatic || IsConst(fieldDefinition) ||
          fieldDefinition.IsNotSerialized || fieldDefinition.IsInitOnly)
        return false;
      ...
      // as serialization weaver will fail to resolve that (due to it being in
      ...
      // Resolving types is more complex and slower than checking their names or
      // attributes, thus keep those checks below
      ...
      if (typeReference is ArrayType ||
          CecilUtils.IsGenericList(typeReference)) {
    ...
    public static bool
    ShouldFieldBePPtrRemapped(FieldDefinition fieldDefinition) {
    ...
    ShouldFieldBePPtrRemapped(FieldDefinition fieldDefinition,
      ...
      return CanTypeContainUnityEngineObjectReference(
          typeResolver.Resolve(fieldDefinition.FieldType));
    }

    private static bool
    CanTypeContainUnityEngineObjectReference(TypeReference typeReference) {
      ...
      if (IsSupportedCollection(typeReference))
        return CanTypeContainUnityEngineObjectReference(
            CecilUtils.ElementTypeOfCollection(typeReference));

      var definition = typeReference.Resolve();
      if (definition == null)
        return false;

      return HasFieldsThatCanContainUnityEngineObjectReferences(
          definition, new TypeResolver(typeReference as GenericInstanceType));
    }

    private static bool HasFieldsThatCanContainUnityEngineObjectReferences(
        TypeDefinition definition, TypeResolver typeResolver) {
      return AllFieldsFor(definition, typeResolver)
          .Where(kv =>
                     kv.Value.Resolve(kv.Key.FieldType).Resolve() != definition)
          .Any(kv => CanFieldContainUnityEngineObjectReference(
                   definition, kv.Key, kv.Value));
    }

    private static IEnumerable<KeyValuePair<FieldDefinition, TypeResolver>>
    AllFieldsFor(TypeDefinition definition, TypeResolver typeResolver) {
      ...
      foreach (var fieldDefinition in definition.Fields)
        yield return new KeyValuePair<FieldDefinition, TypeResolver>(
            fieldDefinition, typeResolver);
    }

    private static bool
    CanFieldContainUnityEngineObjectReference(TypeReference typeReference,
                                              FieldDefinition t,
                                              TypeResolver typeResolver) {
    ...
    public static bool HasSerializeFieldAttribute(FieldDefinition field) {
      // return
      // FieldAttributes(field).Any(UnityEngineTypePredicates.IsSerializeFieldAttribute);
    ...
    FieldAttributes(FieldDefinition field) {
    ...
    public static bool ShouldNotTryToResolve(TypeReference typeReference) {
      ...
      try { // This will throw an exception if typereference thinks it's
            // referencing a .dll,
        // but actually there's .winmd file in the current directory. RRW will
        // fix this at a later step, so we will not try to resolve this type.
        // This is OK, as any type defined in a winmd cannot be serialized.
    ...
    private static bool
    IsFieldTypeSerializable(TypeReference typeReference,
                            FieldDefinition fieldDefinition) {
      return IsTypeSerializable(typeReference) ||
    ...
    private static bool IsValueTypeSerializable(TypeReference typeReference) {
      ...
      return UnityEngineTypePredicates.IsSerializableUnityStruct(
                 typeReference) ||
    ...
    private static bool
    IsReferenceTypeSerializable(TypeReference typeReference) {
      if (typeReference.MetadataType == MetadataType.String)
        return IsSerializablePrimitive(typeReference);
    ...
    public static CustomAttribute
    GetFixedBufferAttribute(FieldDefinition fieldDefinition) {
      if (!fieldDefinition.HasCustomAttributes)
        return null;

      return fieldDefinition.CustomAttributes.SingleOrDefault(
    ...
    public static int GetFixedBufferLength(FieldDefinition fieldDefinition) {
      ...
      if (fixedBufferAttribute == null)
        throw new ArgumentException(
            string.Format("Field '{0}' is not a fixed buffer field.",
    ...
    public static int PrimitiveTypeSize(TypeReference type) {
      ...
      default:
        throw new ArgumentException(
            string.Format("Unsupported {0}", type.MetadataType));
      }
    }
    ...
    public static bool IsSupportedCollection(TypeReference typeReference) {
      ...
      return IsTypeSerializable(
          CecilUtils.ElementTypeOfCollection(typeReference));
    }

    private static bool ShouldHaveHadAllFieldsPublic(FieldDefinition field) {
      return UnityEngineTypePredicates.IsUnityEngineValueType(
          field.DeclaringType);
    }
    ...
    public static bool IsNonSerialized(TypeReference typeDeclaration) {
      ...
      if (typeDeclaration.FullName == UnityEngineTypePredicates.MonoBehaviour)
        return true;
      if (typeDeclaration.FullName ==
          UnityEngineTypePredicates.ScriptableObject)
        return true;
    ...
    public static bool
    ShouldImplementIDeserializable(TypeReference typeDeclaration) {
      ...
      try {
        if (UnityEngineTypePredicates.ShouldHaveHadSerializableAttribute(
                typeDeclaration))
          return true;

        var resolvedTypeDeclaration = typeDeclaration.CheckedResolve();
        if (resolvedTypeDeclaration.IsValueType) {
          return resolvedTypeDeclaration.IsSerializable &&
                 !resolvedTypeDeclaration.CustomAttributes.Any(
                     a => a.AttributeType.FullName.Contains(
          ...
          return (resolvedTypeDeclaration.IsSerializable &&
                  !resolvedTypeDeclaration.CustomAttributes.Any(
                      a => a.AttributeType.FullName.Contains(
                          "System.Runtime.CompilerServices.CompilerGenerated"))) ||
                 resolvedTypeDeclaration.IsSubclassOf(
                     UnityEngineTypePredicates.MonoBehaviour,
                     UnityEngineTypePredicates.ScriptableObject);
        }
      } catch (Exception) {
        return false;
