...
[Serializable] struct MeshExtents {
	public Vector2 min;
	public Vector2 max;
...
// Structure used for Word Wrapping which tracks the state of execution when
// the last space or carriage return character was encountered.
...
public static readonly Vector2 largePositiveVector2 =
	new Vector2(2147483647, 2147483647);
public static readonly Vector2 largeNegativeVector2 =
	new Vector2(-214748364, -214748364);
...
/// <returns></returns>
public static Color32 HexCharsToColor(char[] hexChars, int startIndex,
                                      int length) {
	if (length == 7) {
		byte r = (byte)(HexToInt(hexChars[startIndex + 1]) * 16 +
		                HexToInt(hexChars[startIndex + 2]));
		byte g = (byte)(HexToInt(hexChars[startIndex + 3]) * 16 +
		                HexToInt(hexChars[startIndex + 4]));
		byte b = (byte)(HexToInt(hexChars[startIndex + 5]) * 16 +
	...
	if (length == 9) {
		...
		byte b = (byte)(HexToInt(hexChars[startIndex + 5]) * 16 +
		                HexToInt(hexChars[startIndex + 6]));
		byte a = (byte)(HexToInt(hexChars[startIndex + 7]) * 16 +
...
/// <summary>
/// Extracts a float value from char[] assuming we know the position of the
/// start, end and decimal point.
/// </summary>
/// <param name="chars"></param>
/// <param name="startIndex"></param>
/// <param name="length"></param>
/// <returns></returns>
public static float ConvertToFloat(char[] chars, int startIndex,
                                   int length) {
	int lastIndex;
...
/// </summary>
/// <param name="chars"></param> The Char[] containing the numerical
/// sequence. <param name="startIndex"></param> The index of the start of
/// the numerical sequence. <param name="length"></param> The length of the
/// numerical sequence. <param name="lastIndex"></param> Index of the last
/// character in the validated sequence. <returns></returns>
public static float ConvertToFloat(char[] chars, int startIndex, int length,
	...
	// Set value multiplier checking the first character to determine if we
	// are using '+' or '-'
...
public static void
StringToCharArray(string sourceText, ref int[] charBuffer,
                  ref RichTextTagStack<int> styleStack,
	...
	for (int i = 0; i < sourceText.Length; i++) {
		...
		if (sourceText[i] == 60 && generationSettings.richText) {
			...
			if (IsTagName(ref sourceText, "<STYLE=", i)) {
				int srcOffset;
				if (ReplaceOpeningStyleTag(ref sourceText, i, out srcOffset,
				                           ref charBuffer, ref writeIndex,
				                           ref styleStack)) {
					i = srcOffset;
					continue;
				}
...
/// <summary>
/// Method to handle inline replacement of style tag by opening style
/// definition.
/// </summary>
/// <param name="sourceText"></param>
...
/// <param name="styleStack"></param>
/// <returns></returns>
static bool ReplaceOpeningStyleTag(ref int[] sourceText, int srcIndex,
                                   out int srcOffset, ref int[] charBuffer,
                                   ref int writeIndex,
                                   ref RichTextTagStack<int> styleStack) {
	// Validate <style> tag.
	int hashCode =
	...
	for (int i = 0; i < styleLength; i++) {
		...
		if (c == 60) {
			...
			if (IsTagName(ref openingTagArray, "<STYLE=", i)) {
				int offset;
				if (ReplaceOpeningStyleTag(ref openingTagArray, i, out offset,
				                           ref charBuffer, ref writeIndex,
				                           ref styleStack)) {
					i = offset;
					continue;
...
/// <param name="styleStack"></param>
/// <returns></returns>
static bool ReplaceOpeningStyleTag(ref string sourceText, int srcIndex,
                                   out int srcOffset, ref int[] charBuffer,
                                   ref int writeIndex,
                                   ref RichTextTagStack<int> styleStack) {
	// Validate <style> tag.
...
/// <summary>
/// Method to handle inline replacement of style tag by closing style
/// definition.
/// </summary>
/// <param name="charBuffer"></param>
/// <param name="writeIndex"></param>
/// <param name="styleStack"></param>
/// <returns></returns>
static void ReplaceClosingStyleTag(ref int[] charBuffer, ref int writeIndex,
                                   ref RichTextTagStack<int> styleStack) {
	...
	for (int i = 0; i < styleLength; i++) {
		...
		if (c == 60) {
			...
			if (IsTagName(ref closingTagArray, "<STYLE=", i)) {
				int offset;
				if (ReplaceOpeningStyleTag(ref closingTagArray, i, out offset,
				                           ref charBuffer, ref writeIndex,
				                           ref styleStack)) {
					i = offset;
					continue;
...
static int GetTagHashCode(ref int[] text, int index, out int closeIndex) {
	...
	for (int i = index; i < text.Length; i++) {
		...
		hashCode = (hashCode << 5) + hashCode ^
		           (int) TextUtilities.ToUpperASCIIFast((char) text[i]);
	}
...
static int GetTagHashCode(ref string text, int index, out int closeIndex) {
	...
	for (int i = index; i < text.Length; i++) {
		...
		hashCode = (hashCode << 5) + hashCode ^
		           (int) TextUtilities.ToUpperASCIIFast(text[i]);
	}
...
public static void
FillCharacterVertexBuffers(int i, TextGenerationSettings generationSettings,
                           TextInfo textInfo) {
	...
	if (generationSettings.inverseYAxis) {
		...
		textInfo.meshInfo[materialIndex].vertices[0 + indexX4] =
			position + axisOffset;
		position = textElementInfoArray[i].vertexTopLeft.position;
		position.y *= -1;
		textInfo.meshInfo[materialIndex].vertices[1 + indexX4] =
			position + axisOffset;
		position = textElementInfoArray[i].vertexTopRight.position;
		position.y *= -1;
		textInfo.meshInfo[materialIndex].vertices[2 + indexX4] =
			position + axisOffset;
		position = textElementInfoArray[i].vertexBottomRight.position;
		position.y *= -1;
		textInfo.meshInfo[materialIndex].vertices[3 + indexX4] =
			position + axisOffset;
	} else {
		textInfo.meshInfo[materialIndex].vertices[0 + indexX4] =
			textElementInfoArray[i].vertexBottomLeft.position;
		textInfo.meshInfo[materialIndex].vertices[1 + indexX4] =
			textElementInfoArray[i].vertexTopLeft.position;
		textInfo.meshInfo[materialIndex].vertices[2 + indexX4] =
			textElementInfoArray[i].vertexTopRight.position;
		textInfo.meshInfo[materialIndex].vertices[3 + indexX4] =
			textElementInfoArray[i].vertexBottomRight.position;
	}

	// Setup UVS0
	textInfo.meshInfo[materialIndex].uvs0[0 + indexX4] =
		textElementInfoArray[i].vertexBottomLeft.uv;
	textInfo.meshInfo[materialIndex].uvs0[1 + indexX4] =
		textElementInfoArray[i].vertexTopLeft.uv;
	textInfo.meshInfo[materialIndex].uvs0[2 + indexX4] =
		textElementInfoArray[i].vertexTopRight.uv;
	textInfo.meshInfo[materialIndex].uvs0[3 + indexX4] =
		textElementInfoArray[i].vertexBottomRight.uv;

	// Setup UVS2
	textInfo.meshInfo[materialIndex].uvs2[0 + indexX4] =
		textElementInfoArray[i].vertexBottomLeft.uv2;
	textInfo.meshInfo[materialIndex].uvs2[1 + indexX4] =
		textElementInfoArray[i].vertexTopLeft.uv2;
	textInfo.meshInfo[materialIndex].uvs2[2 + indexX4] =
		textElementInfoArray[i].vertexTopRight.uv2;
	textInfo.meshInfo[materialIndex].uvs2[3 + indexX4] =
		textElementInfoArray[i].vertexBottomRight.uv2;

	// setup Vertex Colors
	textInfo.meshInfo[materialIndex].colors32[0 + indexX4] =
		textElementInfoArray[i].vertexBottomLeft.color;
	textInfo.meshInfo[materialIndex].colors32[1 + indexX4] =
		textElementInfoArray[i].vertexTopLeft.color;
	textInfo.meshInfo[materialIndex].colors32[2 + indexX4] =
		textElementInfoArray[i].vertexTopRight.color;
	textInfo.meshInfo[materialIndex].colors32[3 + indexX4] =
		textElementInfoArray[i].vertexBottomRight.color;
...
/// <param name="textInfo"></param>
public static void
FillSpriteVertexBuffers(int i, TextGenerationSettings generationSettings,
                        TextInfo textInfo) {
	int materialIndex = textInfo.textElementInfo[i].materialReferenceIndex;
	int indexX4 = textInfo.meshInfo[materialIndex].vertexCount;
...
public static void AdjustLineOffset(int startIndex, int endIndex,
                                    float offset, TextInfo textInfo) {
	...
	for (int i = startIndex; i <= endIndex; i++) {
		...
		if (textInfo.textElementInfo[i].isVisible) {
			...
			textInfo.textElementInfo[i].vertexBottomRight.position -=
				vertexOffset;
		}
	}
}
...
LegacyStyleToNewStyle(UnityEngine.FontStyle fontStyle) {
	switch (fontStyle) {
...
LegacyAlignmentToNewAlignment(TextAnchor anchor) {
	switch (anchor) {
