...
namespace UnityEngine.TextCore {
  //[CreateAssetMenu(fileName = "TextSettings.asset", menuName = "TextCore/Text
  //Settings", order = 150)]
  [System.Serializable]
  class TextSettings : ScriptableObject {
    const string k_DefaultLeadingCharacters =
        "([｛〔〈《「『【〘〖〝‘“｟«$—…‥〳〴〵\\［（{£¥\"々〇〉》」＄｠￥￦ #";
    const string k_DefaultFollowingCharacters =
        ")]｝〕〉》」』】〙〗〟’”｠»ヽヾーァィゥェォッャュョヮヵヶぁぃぅぇぉっゃゅょゎゕゖㇰㇱㇲㇳㇴㇵㇶㇷㇸㇹㇺㇻㇼㇽㇾㇿ々〻‐゠–〜?!‼⁇⁈⁉・、%,.:;。！？］）：；＝}¢°\"†‡℃〆％，．";

    static TextSettings s_Instance;

    /// <summary>
    /// The character the will be used as a replacement for missing glyphs in a
    /// font asset.
    /// </summary>
    ...
    [SerializeField] int m_missingGlyphCharacter;
    ...
    [SerializeField] bool m_warningsDisabled = true;
    ...
    [SerializeField] FontAsset m_defaultFontAsset;
    ...
    [SerializeField] string m_defaultFontAssetPath;
    ...
    [SerializeField] List<FontAsset> m_fallbackFontAssets;

    /// <summary>
    /// Controls whether or not TextCore will create a matching material preset
    /// or use the default material of the fallback font asset.
    /// </summary>
    public static bool matchMaterialPreset {
      get { return instance.m_matchMaterialPreset; }
      set { instance.m_matchMaterialPreset = value; }
    }
    [SerializeField] bool m_matchMaterialPreset;
    ...
    [SerializeField] TextSpriteAsset m_defaultSpriteAsset;
    ...
    [SerializeField] string m_defaultSpriteAssetPath;

    /// <summary>
    /// The relative path to a Resources folder in the project that contains
    ...
    [SerializeField] string m_defaultColorGradientPresetsPath;
    ...
    [SerializeField] TextStyleSheet m_defaultStyleSheet;

    /// <summary>
    /// Text file that contains the leading characters used for line breaking
    /// for Asian languages.
    /// </summary>
    [SerializeField] UnityEngine.TextAsset m_leadingCharacters = null;

    /// <summary>
    /// Text file that contains the following characters used for line breaking
    /// for Asian languages.
    /// </summary>
    [SerializeField] UnityEngine.TextAsset m_followingCharacters = null;
    ...
    [SerializeField] LineBreakingTable m_linebreakingRules;
    ...
    public static TextSettings instance {
      get {
        if (s_Instance == null) {
          s_Instance = Resources.Load<TextSettings>("TextSettings")
              ?? CreateInstance<TextSettings>();
        }
    ...
    public static void LoadLinebreakingRules() {
      ...
      s_Instance.m_linebreakingRules.leadingCharacters =
          s_Instance.m_leadingCharacters !=
          null ? GetCharacters(s_Instance.m_leadingCharacters.text)
          : GetCharacters(k_DefaultLeadingCharacters);
      s_Instance.m_linebreakingRules.followingCharacters =
          s_Instance.m_followingCharacters !=
          null ? GetCharacters(s_Instance.m_followingCharacters.text)
          : GetCharacters(k_DefaultFollowingCharacters);
    }
