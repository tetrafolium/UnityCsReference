...
struct MaterialReference {
	...
	public MaterialReference(int index, FontAsset fontAsset,
	                         TextSpriteAsset spriteAsset, Material material,
	                         float padding) {
		this.index = index;
		...
		isDefaultMaterial =
			material.GetInstanceID() == fontAsset.material.GetInstanceID();
	...
	/// <summary>
	/// Function to check if a certain font asset is contained in the material
	/// reference array.
	/// </summary>
	/// <param name="materialReferences"></param>
	/// <param name="fontAsset"></param>
	/// <returns></returns>
	public static bool Contains(MaterialReference[] materialReferences,
	                            FontAsset fontAsset) {
		int id = fontAsset.GetInstanceID();

		for (int i = 0; i < materialReferences.Length &&
		     materialReferences[i].fontAsset != null;
		     i++) {
	...
	/// <summary>
	/// Function to add a new material reference and returning its index in the
	...
	/// <returns></returns>
	public static int
	AddMaterialReference(Material material, FontAsset fontAsset,
		...
		materialReferences[index].isDefaultMaterial =
			materialId == fontAsset.material.GetInstanceID();
	...
	/// <returns></returns>
	public static int
	AddMaterialReference(Material material, TextSpriteAsset spriteAsset,
	                     MaterialReference[] materialReferences,
	                     Dictionary<int, int> materialReferenceIndexLookup) {
		int materialId = material.GetInstanceID();
		int index;
