...
namespace UnityEngine.TextCore {
  ...
  class KerningPair {
    ...
    [SerializeField] uint m_FirstGlyph;
    ...
    [SerializeField] GlyphValueRecord m_FirstGlyphAdjustments;
    ...
    [SerializeField] uint m_SecondGlyph;
    ...
    [SerializeField] GlyphValueRecord m_SecondGlyphAdjustments;
    ...
    public KerningPair(uint firstGlyph, GlyphValueRecord firstGlyphAdjustments,
                       uint secondGlyph,
  ...
  class KerningTable {
    ...
    public KerningTable() { kerningPairs = new List<KerningPair>(); }

    /// <summary>
    /// Add Glyph pair adjustment record
    /// </summary>
    /// <param name="first">The first glyph</param>
    /// <param name="firstAdjustments">Adjustment record for the first
    /// glyph</param> <param name="second">The second glyph</param> <param
    /// name="secondAdjustments">Adjustment record for the second glyph</param>
    /// <returns></returns>
    public int
    AddGlyphPairAdjustmentRecord(uint first, GlyphValueRecord firstAdjustments,
                                 uint second,
                                 GlyphValueRecord secondAdjustments) {
      int index = kerningPairs.FindIndex(item => item.firstGlyph == first &&
                                                 item.secondGlyph == second);

      if (index == -1) {
        kerningPairs.Add(new KerningPair(first, firstAdjustments, second,
                                         secondAdjustments));
        return 0;
      }
    ...
    public void RemoveKerningPair(int left, int right) {
      int index = kerningPairs.FindIndex(item => item.firstGlyph == left &&
                                                 item.secondGlyph == right);

      if (index != -1)
        kerningPairs.RemoveAt(index);
    }

    public void RemoveKerningPair(int index) { kerningPairs.RemoveAt(index); }

    public void SortKerningPairs() {
      // Sort List of Kerning Info
      if (kerningPairs.Count > 0)
        kerningPairs = kerningPairs.OrderBy(s => s.firstGlyph)
                           .ThenBy(s => s.secondGlyph)
                           .ToList();
    }
  }
}
