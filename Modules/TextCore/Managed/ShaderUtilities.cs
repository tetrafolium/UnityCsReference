	...
	get {
		if (k_ShaderRef_MobileSDF == null)
			k_ShaderRef_MobileSDF =
				Shader.Find("Hidden/TextCore/Distance Field SSD");
...
public static void UpdateShaderRatios(Material mat) {
	...
	float weight = Mathf.Max(mat.GetFloat(ID_WeightNormal),
	                         mat.GetFloat(ID_WeightBold)) /
	               4.0f;

	float t = Mathf.Max(1, weight + faceDilate + outlineThickness +
	                    outlineSoftness);
	...
	if (mat.HasProperty(ID_GlowOffset)) {
		...
		float ratioB = isRatioEnabled ? Mathf.Max(0, scale - m_clamp - range) /
		               (scale * t) : 1;
	...
	if (mat.HasProperty(ID_UnderlayOffsetX)) {
		...
		t = Mathf.Max(1, Mathf.Max(Mathf.Abs(underlayOffsetX),
		                           Mathf.Abs(underlayOffsetY)) +
		              underlayDilate + underlaySoftness);

		float ratioC = isRatioEnabled ? Mathf.Max(0, scale - m_clamp - range) /
		               (scale * t) : 1;
...
public static bool IsMaskingEnabled(Material material) {
	...
	return material.shaderKeywords.Contains(Keyword_MASK_SOFT) ||
	       material.shaderKeywords.Contains(Keyword_MASK_HARD) ||
	       material.shaderKeywords.Contains(Keyword_MASK_TEX);
}

// Function to determine how much extra padding is required as a result of
// material properties like dilate, outline thickness, softness, glow,
// etc...
public static float GetPadding(Material material, bool enableExtraPadding,
	...
	// Iterate through each of the assigned materials to find the max values
	// to set the padding.
	...
	if (material.HasProperty(ID_GlowOffset) &&
	    shaderKeywords.Contains(Keyword_Glow)) // Generates GC
	{
	...
	uniformPadding =
		Mathf.Max(uniformPadding, faceDilate + glowOffset + glowOuter);

	// Underlay padding contribution
	if (material.HasProperty(ID_UnderlaySoftness) &&
	    shaderKeywords.Contains(Keyword_Underlay)) // Generates GC
	{
		...
		padding.x =
			Mathf.Max(padding.x, faceDilate + dilate + softness - offsetX);
		padding.y =
			Mathf.Max(padding.y, faceDilate + dilate + softness - offsetY);
		padding.z =
			Mathf.Max(padding.z, faceDilate + dilate + softness + offsetX);
		padding.w =
			Mathf.Max(padding.w, faceDilate + dilate + softness + offsetY);
	}
