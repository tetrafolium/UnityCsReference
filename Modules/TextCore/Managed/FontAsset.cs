...
namespace UnityEngine.TextCore {
  ...
  [Serializable] struct FontAssetCreationSettings {
    ...
    internal FontAssetCreationSettings(string fontFileGUID, int pointSize,
                                       int pointSizeSamplingMode, int padding,
                                       int packingMode, int atlasWidth,
                                       int atlasHeight,
                                       int characterSelectionMode,
  ...
  internal class FontAsset : ScriptableObject {
    ...
    /// Version 1.1.0 adds support for the new TextCore.FontEngine and Dynamic
    /// SDF system.
    /// </summary>
    ...
    [SerializeField] string m_Version = "1.1.0";
    ...
    [SerializeField] FaceInfo m_FaceInfo;
    ...
    /// TODO: This should be enclosed in #if UNITY_EDITOR once Katana tests
    /// issue related to serialized properties in the Editor is resolved.
    /// </summary>
    [SerializeField]
    internal Font m_SourceFontFile_EditorRef;

    /// <summary>
    /// Source font file when atlas population mode is set to dynamic. Null when
    /// the atlas population mode is set to static.
    /// </summary>
    ...
    /// When set to Dynamic, the sourceFontFile is set to reference the source
    /// font file. Otherwise it is set to null.
    /// </summary>
    ...
    [SerializeField] List<Glyph> m_GlyphTable = new List<Glyph>();

    /// <summary>
    /// Dictionary used to lookup glyphs contained in the font asset by their
    /// index.
    /// </summary>
    ...
    [SerializeField] List<Character> m_CharacterTable = new List<Character>();

    /// <summary>
    /// Dictionary used to lookup characters contained in the font asset by
    ...
    [SerializeField] Texture2D[] m_AtlasTextures;

    /// <summary>
    /// Index of the font atlas texture that still has available space to add
    /// new glyphs. Once an atlas texture is full, the index is increased to
    /// point to the next element in the m_AtlasTextures array.
    /// TODO:
    /// </summary>
    [SerializeField]
    ...
    /// <summary>
    /// The padding used between glyphs contained in the atlas texture(s) used
    /// by this font asset.
    /// </summary>
    ...
    [SerializeField] List<GlyphRect> m_UsedGlyphRects;
    ...
    public Material material {
      ...
      set {
        m_Material = value;
        m_MaterialHashCode =
            TextUtilities.GetHashCodeCaseInSensitive(m_Material.name);
      }
    }
    ...
    /// <summary>
    /// TODO: This is only used to display an empty kerning pair in the Editor.
    /// This should be implemented more cleanly.
    /// </summary>
    [SerializeField]
    ...
    /// <summary>
    /// Array containing font assets to be used as alternative typefaces for the
    /// various potential font weights of this font asset.
    /// </summary>
    ...
    public float regularStyleWeight {
      get { return m_RegularStyleWeight; }
      set { m_RegularStyleWeight = value; }
    }
    [SerializeField] float m_RegularStyleWeight = 0;

    /// <summary>
    /// The spacing between characters when using regular style.
    /// </summary>
    public float regularStyleSpacing {
      get { return m_RegularStyleSpacing; }
      set { m_RegularStyleSpacing = value; }
    }
    [SerializeField] float m_RegularStyleSpacing = 0;

    /// <summary>
    /// Defines the dilation of the text when using bold style.
    /// </summary>
    public float boldStyleWeight {
      get { return m_BoldStyleWeight; }
      set { m_BoldStyleWeight = value; }
    }
    [SerializeField] float m_BoldStyleWeight = 0.75f;

    /// <summary>
    /// The spacing between characters when using regular style.
    /// </summary>
    public float boldStyleSpacing {
      get { return m_BoldStyleSpacing; }
      set { m_BoldStyleSpacing = value; }
    }
    [SerializeField] float m_BoldStyleSpacing = 7f;

    /// <summary>
    /// Defines the slant of the text when using italic style.
    /// </summary>
    public byte italicStyleSlant {
      get { return m_ItalicStyleSlant; }
      set { m_ItalicStyleSlant = value; }
    }
    [SerializeField] byte m_ItalicStyleSlant = 35;

    /// <summary>
    /// The number of spaces that a tab represents.
    /// </summary>
    public byte tabMultiple {
      get { return m_TabMultiple; }
      set { m_TabMultiple = value; }
    }
    [SerializeField] byte m_TabMultiple = 10;
    ...
    /// <param name="atlasPadding">The padding / spread between individual
    /// glyphs in the font asset.</param> <param name="renderMode"></param>
    ...
    /// <returns></returns>
    public static FontAsset CreateFontAsset(
        Font font, int samplingPointSize, int atlasPadding,
      ...
      // string guid;
      // long localID;
      // UnityEditor.AssetDatabase.TryGetGUIDAndLocalFileIdentifier(font, out
      // guid, out localID); fontAsset.m_SourceFontFileGUID = guid;
      ...
      int packingModifier;
      if (((GlyphRasterModes) renderMode &
        ...
        tmp_material.SetFloat(ShaderUtilities.ID_GradientScale,
                              atlasPadding + packingModifier);

        tmp_material.SetFloat(ShaderUtilities.ID_WeightNormal,
                              fontAsset.regularStyleWeight);
        tmp_material.SetFloat(ShaderUtilities.ID_WeightBold,
                              fontAsset.boldStyleWeight);

        fontAsset.material = tmp_material;
      }

      fontAsset.freeGlyphRects = new List<GlyphRect>(){new GlyphRect(
          0, 0, atlasWidth - packingModifier, atlasHeight - packingModifier)};
    ...
    /// <summary>
    /// Read the various data tables of the font asset to populate its different
    /// dictionaries to allow for faster lookup of related font asset data.
    /// </summary>
    internal void InitializeDictionaryLookupTables() {
      // Create new instance of the glyph lookup dictionary or clear the
      // existing one.
      if (m_GlyphLookupDictionary == null)
      ...
      // Add the characters contained in the character table into the dictionary
      // for faster lookup.
      ...
      // Create new instance of the character lookup dictionary or clear the
      // existing one.
      if (m_CharacterLookupDictionary == null)
      ...
      // Read Kerning pairs and update Kerning pair dictionary for faster
      // lookup.
      ...
      if (glyphPairAdjustmentRecord != null) {
        for (int i = 0; i < glyphPairAdjustmentRecord.Count; i++) {
          ...
          KerningPairKey uniqueKey =
              new KerningPairKey(pair.firstGlyph, pair.secondGlyph);

          if (m_KerningLookupDictionary.ContainsKey((int) uniqueKey.key) ==
              false) {
            m_KerningLookupDictionary.Add((int) uniqueKey.key, pair);
          } else {
            if (!TextSettings.warningsDisabled)
              Debug.LogWarning("Kerning Key for [" + uniqueKey.ascii_Left +
    ...
    internal void ReadFontAssetDefinition() {
      ...
      if (m_CharacterLookupDictionary.ContainsKey(9) == false) {
        Glyph glyph = new Glyph(
            0, new GlyphMetrics(0, 0, 0, 0, m_FaceInfo.tabWidth * tabMultiple),
            GlyphRect.zero, 1.0f, 0);
      ...
      if (m_CharacterLookupDictionary.ContainsKey(10) == false) {
        Glyph glyph = new Glyph(0, new GlyphMetrics(10, 0, 0, 0, 0),
                                GlyphRect.zero, 1.0f, 0);
      ...
      if (m_CharacterLookupDictionary.ContainsKey(8203) == false) {
        Glyph glyph = new Glyph(0, new GlyphMetrics(0, 0, 0, 0, 0),
                                GlyphRect.zero, 1.0f, 0);
      ...
      // Set Cap Height
      if (m_FaceInfo.capLine == 0 &&
          m_CharacterLookupDictionary.ContainsKey(72))
        m_FaceInfo.capLine =
            m_CharacterLookupDictionary[72].glyph.metrics.horizontalBearingY;
      ...
      if (m_AtlasPadding == 0) {
        if (material.HasProperty(ShaderUtilities.ID_GradientScale))
          m_AtlasPadding =
              (int) material.GetFloat(ShaderUtilities.ID_GradientScale) - 1;
      }
      ...
      m_MaterialHashCode =
          TextUtilities.GetHashCodeCaseInSensitive(material.name);
    }
    ...
    /// <summary>
    /// Function to check if a character is contained in the font asset with the
    /// option to also check through fallback font assets.
    /// </summary>
    /// <param name="character"></param>
    ...
    internal bool HasCharacter(char character, bool searchFallbacks) {
      ...
      if (searchFallbacks) {
        ...
        if (fallbackFontAssetTable != null &&
            fallbackFontAssetTable.Count > 0) {
          for (int i = 0; i < fallbackFontAssetTable.Count &&
                          fallbackFontAssetTable[i] != null;
               i++) {
            if (fallbackFontAssetTable [i]
                    .HasCharacter_Internal(character, searchFallbacks))
              return true;
          }
        }
        ...
        if (TextSettings.fallbackFontAssets != null &&
          ...
          for (int i = 0; i < TextSettings.fallbackFontAssets.Count &&
                          TextSettings.fallbackFontAssets[i] != null;
               i++) {
            if (TextSettings.fallbackFontAssets[i]
                    .m_CharacterLookupDictionary == null)
              TextSettings
                  .fallbackFontAssets [i]
                  .ReadFontAssetDefinition();
        ...
        if (TextSettings.defaultFontAsset != null) {
          ...
          if (TextSettings.defaultFontAsset.m_CharacterLookupDictionary !=
                  null &&
              TextSettings.defaultFontAsset.HasCharacter_Internal(
                  character, searchFallbacks))
            return true;
        }
      }
    ...
    /// <summary>
    /// Function to check if a character is contained in a font asset with the
    /// option to also check through fallback font assets. This private
    /// implementation does not search the fallback font asset in the Text
    /// Settings file.
    /// </summary>
    /// <param name="character"></param>
    /// <param name="searchFallbacks"></param>
    ...
    /// <summary>
    /// Function to check if certain characters exists in the font asset.
    /// Function returns a list of missing characters.
    /// </summary>
    /// <returns></returns>
    ...
    /// <summary>
    /// Function to check if certain characters exists in the font asset.
    /// Function returns false if any characters are missing.
    /// </summary>
    ...
    /// <summary>
    /// Function which returns an array that contains all the characters from a
    /// font asset.
    /// </summary>
    /// <param name="fontAsset"></param>
    /// <returns></returns>
    ...
    // ================================================================================
    // Properties and functions related to character and glyph additions as well
    // as tacking glyphs that need to be added to various font asset atlas
    // textures.
    // ================================================================================
    ...
    /// <summary>
    /// List of glyphs that have been packed in the atlas texture and ready to
    /// be rendered.
    /// </summary>
    ...
    internal Character AddCharacter_Internal(uint unicode, Glyph glyph) {
      ...
      if (m_GlyphLookupDictionary.ContainsKey(glyphIndex) == false) {
        if (glyph.glyphRect.width == 0 || glyph.glyphRect.width == 0) {
          // Glyphs with zero width and / or height can be automatically added
          // to font asset.
          m_GlyphTable.Add(glyph);
        } else {
          // Try packing new glyph
          if (FontEngine.TryPackGlyphInAtlas(
                  glyph, m_AtlasPadding, GlyphPackingMode.ContactPointRule,
                  m_AtlasRenderMode, m_AtlasWidth, m_AtlasHeight,
                  m_FreeGlyphRects, m_UsedGlyphRects) == false) {
      ...
      // Debug.Log("Adding character [" + (char)unicode + "] with Unicode (" +
      // unicode + ") to [" + this.name + "] font asset.");
      ...
      // OPTIMIZATION: This could be handled when exiting Play mode if we added
      // any new characters to the asset. Could also add some update registry to
      // handle this.
      // SortGlyphTable();
    ...
    /// <param name="character">The character data if successfully added to the
    /// font asset. Null otherwise.</param> <returns>Returns true if the
    /// character has been added. False otherwise.</returns>
    internal bool TryAddCharacter(uint unicode, out Character character) {
      ...
      // Load font face.
      if (FontEngine.LoadFontFace(sourceFontFile, m_FaceInfo.pointSize) !=
      ...
      // Check if glyph is already contained in the font asset as the same glyph
      // might be referenced by multiple characters.
      if (m_GlyphLookupDictionary.ContainsKey(glyphIndex)) {
        ...
        // Makes the changes to the font asset persistent.
        // OPTIMIZATION: This could be handled when exiting Play mode if we
        // added any new characters to the asset. Could also add some update
        // registry to handle this.
        // SortGlyphTable();
        // UnityEditor.EditorUtility.SetDirty(this);
      ...
      if (m_AtlasTextures[m_AtlasTextureIndex].width == 0 ||
          m_AtlasTextures[m_AtlasTextureIndex].height == 0) {
        m_AtlasTextures [m_AtlasTextureIndex]
            .Resize(m_AtlasWidth, m_AtlasHeight);
        FontEngine.ResetAtlasTexture(m_AtlasTextures[m_AtlasTextureIndex]);
      }

      Glyph glyph;
      if (FontEngine.TryAddGlyphToTexture(
              glyphIndex, m_AtlasPadding, GlyphPackingMode.BestShortSideFit,
              m_FreeGlyphRects, m_UsedGlyphRects, m_AtlasRenderMode,
              m_AtlasTextures[m_AtlasTextureIndex], out glyph)) {
    ...
    internal void UpdateAtlasTexture() {
      ...
      // TODO: Packing and glyph render modes should be defined in the font
      // asset.
      // FontEngine.PackGlyphsInAtlas(m_GlyphsToPack, m_GlyphsPacked,
      // m_AtlasPadding, GlyphPackingMode.ContactPointRule,
      // GlyphRenderMode.SDFAA, m_AtlasWidth, m_AtlasHeight, m_FreeGlyphRects,
      // m_UsedGlyphRects); FontEngine.RenderGlyphsToTexture(m_GlyphsPacked,
      // m_AtlasPadding, GlyphRenderMode.SDFAA,
      // m_AtlasTextures[m_AtlasTextureIndex]);

      FontEngine.RenderGlyphsToTexture(m_GlyphsToRender, m_AtlasPadding,
                                       m_AtlasRenderMode,
                                       m_AtlasTextures[m_AtlasTextureIndex]);
      ...
      for (int i = 0; i < m_GlyphsToRender.Count /* m_GlyphsPacked.Count */;
           i++) {
      ...
      // Add any remaining glyphs into new atlas texture if multi texture
      // support if enabled.
      if (m_GlyphsToPack.Count > 0) {
        /*
        // Create new atlas texture
        Texture2D tex = new Texture2D(m_AtlasWidth, m_AtlasHeight,
        TextureFormat.Alpha8, false, true); tex.SetPixels32(new
        Color32[m_AtlasWidth * m_AtlasHeight]); tex.Apply();

        m_AtlasTextureIndex++;

        if (m_AtlasTextures.Length == m_AtlasTextureIndex)
            Array.Resize(ref m_AtlasTextures,
        Mathf.NextPowerOfTwo(m_AtlasTextureIndex + 1));
    ...
    /// <summary>
    /// Try adding the characters from the provided array of unicode values to
    /// the font asset.
    /// </summary>
    /// <param name="unicodes">Array that contains the unicode characters to be
    /// added.</param> <returns>Returns true if all the characters were
    /// successfully added to the font asset. Return false otherwise.</returns>
    public bool TryAddCharacters(uint[] unicodes) {
      ...
      for (int i = 0; i < unicodes.Length; i++) {
        ...
        // Check if glyph is already contained in the font asset as the same
        // glyph might be referenced by multiple characters.
        if (m_GlyphLookupDictionary.ContainsKey(glyphIndex)) {
          Character character =
              new Character(unicode, m_GlyphLookupDictionary[glyphIndex]);
          m_CharacterTable.Add(character);
          m_CharacterLookupDictionary.Add(unicode, character);
      ...
      Glyph[] glyphs;
      bool allCharactersAdded = FontEngine.TryAddGlyphsToTexture(
          m_GlyphIndexes, m_AtlasPadding, GlyphPackingMode.BestShortSideFit,
          m_FreeGlyphRects, m_UsedGlyphRects, m_AtlasRenderMode,
          m_AtlasTextures[m_AtlasTextureIndex], out glyphs);
    ...
    /// </summary>
    /// <param name="characters">String containing the characters to add to the
    /// font asset.</param> <returns>Returns true if all the characters were
    /// successfully added to the font asset. Return false otherwise.</returns>
    public bool TryAddCharacters(string characters) {
      // Make sure font asset is set to dynamic and that we have a valid list of
      // characters.
      if (string.IsNullOrEmpty(characters) ||
          m_AtlasPopulationMode == AtlasPopulationMode.Static) {
        if (m_AtlasPopulationMode == AtlasPopulationMode.Static)
          Debug.LogWarning(
              "Unable to add characters to font asset [" + this.name +
                  "] because its AtlasPopulationMode is set to Static.",
              this);
        else {
          Debug.LogWarning(
              "Unable to add characters to font asset [" + this.name +
      ...
      // Load font face.
      if (FontEngine.LoadFontFace(m_SourceFontFile, m_FaceInfo.pointSize) !=
          FontEngineError.Success)
        return false;
      ...
      // Clear list / dictionary used to track which glyph needs to be added to
      // atlas texture.
      m_GlyphIndexes.Clear();
      s_GlyphLookupMap.Clear();
      ...
      for (int i = 0; i < characterCount; i++) {
        ...
        if (s_GlyphLookupMap.ContainsKey(glyphIndex)) {
          // Exclude duplicates.
          if (s_GlyphLookupMap [glyphIndex]
                  .Contains(unicode))
            continue;
      ...
      if (m_GlyphIndexes == null || m_GlyphIndexes.Count == 0) {
        Debug.LogWarning(
            "No characters will be added to font asset [" + this.name +
            "] either because they are already present in the font asset or missing from the font file.");
        return true;
      }
    ...
    /// <summary>
    /// Clears font asset data including the glyph and character tables and
    /// textures. Function might be changed to Internal and only used in tests.
    /// </summary>
    internal void ClearFontAssetData() {
      ...
      if (m_FreeGlyphRects != null) {
        int packingModifier = ((GlyphRasterModes) m_AtlasRenderMode &
                               GlyphRasterModes.RASTER_MODE_BITMAP) ==
                              GlyphRasterModes.RASTER_MODE_BITMAP ? 0 : 1;
        m_FreeGlyphRects = new List<GlyphRect>(){
            new GlyphRect(0, 0, m_AtlasWidth - packingModifier,
      ...
      if (m_AtlasTextures != null) {
        for (int i = 0; i < m_AtlasTextures.Length; i++) {
          ...
          if (texture.width != m_AtlasWidth || texture.height != m_AtlasHeight)
            texture.Resize(m_AtlasWidth, m_AtlasHeight, TextureFormat.Alpha8,
                           false);
