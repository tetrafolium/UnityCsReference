...
/// <summary>
/// Function used to determine if the position intersects with the
/// RectTransform.
/// </summary>
/// <param name="rectTransform">A reference to the RectTranform of the text
/// object.</param> <param name="position">Position to check for
/// intersection.</param> <param name="camera">The scene camera which may be
/// assigned to a Canvas using ScreenSpace Camera or WorldSpace render mode.
/// Set to null is using ScreenSpace Overlay.</param> <returns></returns>
public static bool IsIntersectingRectTransform(RectTransform rectTransform,
                                               Vector3 position,
                                               Camera camera) {
	// Convert position into Worldspace coordinates
	ScreenPointToWorldPointInRectangle(rectTransform, position, camera,
	                                   out position);

	rectTransform.GetWorldCorners(s_RectWorldCorners);

	return PointIntersectRectangle(
		position, s_RectWorldCorners[0], s_RectWorldCorners[1],
		s_RectWorldCorners[2], s_RectWorldCorners[3]);
}
...
/// <param name="d"></param>
/// <returns></returns>
static bool PointIntersectRectangle(Vector3 m, Vector3 a, Vector3 b,
                                    Vector3 c, Vector3 d) {
	Vector3 ab = b - a;
	Vector3 am = m - a;
	Vector3 bc = c - b;
	...
	return 0 <= abamDot && abamDot <= Vector3.Dot(ab, ab) && 0 <= bcbmDot &&
...
/// <returns></returns>
public static bool
ScreenPointToWorldPointInRectangle(Transform transform, Vector2 screenPoint,
                                   Camera cam, out Vector3 worldPoint) {
	worldPoint = Vector3.zero;
	Ray ray = cam.ScreenPointToRay(screenPoint);
	float enter;
	if (!new Plane(transform.rotation * Vector3.back, transform.position)
	    .Raycast(ray, out enter))
		return false;
...
/// <returns></returns>
static bool IntersectLinePlane(LineSegment line, Vector3 point,
                               Vector3 normal,
	...
	if (Mathf.Abs(d) <
	    Mathf.Epsilon) // if line is parallel & co-planar to plane
	{
		return n == 0;
	}
...
/// <summary>
/// Function which returns a simple hashcode from a string converted to
/// lowercase.
/// </summary>
/// <returns></returns>
...
/// <summary>
/// Function to convert a properly formatted string which contains an hex
/// value to its decimal value.
/// </summary>
/// <param name="s"></param>
/// <returns></returns>
