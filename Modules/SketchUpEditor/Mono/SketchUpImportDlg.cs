...
public SketchUpNode(int id, int depth, TreeViewItem parent,
                    string displayName, SketchUpNodeInfo info)
...
public SketchUpDataSource(TreeViewController treeView,
                          SketchUpNodeInfo[] nodes)
	: base(treeView) {
	m_Nodes = nodes;
	FetchData();
}
...
public override void FetchData() {
	...
	m_RootItem = new SketchUpNode(m_Nodes[0].nodeIndex, 0, null,
	                              m_Nodes[0].name, m_Nodes[0]);
	...
	for (int i = 1; i < m_Nodes.Length; ++i) {
		...
		SketchUpNode suNode = new SketchUpNode(node.nodeIndex, parent.depth + 1,
		...
		if (i % k_ProgressUpdateStep == 0) {
			EditorUtility.DisplayProgressBar("SketchUp Import",
			                                 "Building Node Selection",
			                                 (float) i / m_Nodes.Length);
		}
	}
...
public override void OnRowGUI(Rect rowRect, TreeViewItem node, int row,
	...
	suNode.Enabled = GUI.Toggle(toggleRect, suNode.Enabled, GUIContent.none,
	                            SketchUpImportDlg.Styles.styles.toggleStyle);
}
}
...
public readonly GUIContent cancelButton =
	EditorGUIUtility.TrTextContent("Cancel");
public readonly GUIContent nodesLabel = EditorGUIUtility.TrTextContent(
	"Select the SketchUp nodes to import", "Nodes in the file hierarchy");
public readonly GUIContent windowTitle = EditorGUIUtility.TrTextContent(
	"SketchUp Node Selection Dialog", "SketchUp Node Selection Dialog");
...
public void Init(SketchUpNodeInfo[] nodes,
                 SketchUpImporterModelEditor suModelEditor) {
...
internal static void Launch(SketchUpNodeInfo[] nodes,
                            SketchUpImporterModelEditor suModelEditor) {
...
void OnGUI() {
	...
	GUI.Label(new Rect(10, 2, position.width, kHeaderHeight),
	          Styles.styles.nodesLabel);

	Rect bottomRect =
		new Rect(rect.x, rect.yMax - kBottomHeight, rect.width, kBottomHeight);
	...
	if (isModal) {
		...
		if (GUILayout.Button(Styles.styles.cancelButton)) {
			...
			if (m_ModelEditor.IsAlive) {
				SketchUpImporterModelEditor modelEditor =
					m_ModelEditor.Target as SketchUpImporterModelEditor;
