...
enum EFileUnit { Meters, Centimeters, Millimeters, Feet, Inches }
...
internal override void OnEnable() {
	...
	m_MergeCoplanarFaces =
		serializedObject.FindProperty("m_MergeCoplanarFaces");
...
static float ConvertGlobalScaleToUnit(EFileUnit measurement,
                                      float globalScale) {
	switch (measurement) {
	case EFileUnit.Meters: // meter
...
public override void OnInspectorGUI() {
	...
	EditorGUILayout.PropertyField(m_GenerateBackFace,
	                              Styles.generateBackFaceLabel);
	EditorGUILayout.PropertyField(m_MergeCoplanarFaces,
	                              Styles.mergeCoplanarFaces);
	...
	GUILayout.Label("1");
	EditorGUILayout.Popup(m_FileUnit, Styles.measurementOptions,
	                      GUIContent.Temp(""), GUILayout.MaxWidth(100));
	lengthToUnit = ConvertGlobalScaleToUnit((EFileUnit) m_FileUnit.intValue,
	                                        m_GlobalScale.floatValue);
	GUILayout.Label("=");
	lengthToUnit = EditorGUILayout.FloatField(lengthToUnit);
	m_GlobalScale.floatValue =
		CovertUnitToGlobalScale((EFileUnit) m_FileUnit.intValue, lengthToUnit);
	...
	using (new EditorGUI.DisabledScope(true)) {
		...
		EditorGUILayout.FloatField(Styles.northCorrectionLabel,
		                           m_NorthCorrection.floatValue);
	}
	...
	if (assetTarget == null) {
		...
		var rect = EditorGUI.IndentedRect(EditorGUILayout.GetControlRect(
							  true, EditorGUIUtility.singleLineHeight, GUI.skin.button,
							  GUILayout.Width(size.x + EditorGUI.indent)));
	...
	EditorGUILayout.PropertyField(
		m_ImportCameras, ModelImporterModelEditor.Styles.ImportCameras);

	MeshesGUI();

	EditorGUILayout.LabelField(ModelImporterModelEditor.Styles.Geometry,
	                           EditorStyles.boldLabel);

	using (var horizontal = new EditorGUILayout.HorizontalScope()) {
		using (var prop = new EditorGUI.PropertyScope(
			       horizontal.rect,
			       ModelImporterModelEditor.Styles.IndexFormatLabel,
			       m_IndexFormat)) {
...
static public readonly GUIContent sketchUpLabel =
	EditorGUIUtility.TrTextContent("SketchUp", "SketchUp import settings");
static public readonly GUIContent generateBackFaceLabel =
	EditorGUIUtility.TrTextContent(
		"Generate Back Face",
		"Enable/disable generation of back facing polygons");
static public readonly GUIContent mergeCoplanarFaces =
	EditorGUIUtility.TrTextContent(
		"Merge Coplanar Faces",
		"Enable/disable merging of coplanar faces when generating meshes");
static public readonly GUIContent selectNodeButton =
	EditorGUIUtility.TrTextContent(
		"Select Nodes...", "Brings up the node selection dialog box");
static public readonly GUIContent fileUnitLabel = EditorGUIUtility.TrTextContent(
	"Unit conversion",
	"Length measurement to unit conversion. The value in Scale Factor is calculated based on the value here");
static public readonly GUIContent longitudeLabel =
	EditorGUIUtility.TrTextContent("Longitude", "Longitude Geo-location");
static public readonly GUIContent latitudeLabel =
	EditorGUIUtility.TrTextContent("Latitude", "Latitude Geo-location");
static public readonly GUIContent northCorrectionLabel =
	EditorGUIUtility.TrTextContent(
		"North Correction",
		"The angle which will rotate the north direction to the z-axis for the model");
