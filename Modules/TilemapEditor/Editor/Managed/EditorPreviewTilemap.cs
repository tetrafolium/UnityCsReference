...
public override Sprite GetSprite(Vector3Int position) {
	...
	return tile ? m_Tilemap.GetEditorPreviewSprite(position)
	: m_Tilemap.GetSprite(position);
}

public override Color GetColor(Vector3Int position) {
	var tile = m_Tilemap.GetEditorPreviewTile(position);
	return tile ? m_Tilemap.GetEditorPreviewColor(position)
	: m_Tilemap.GetColor(position);
}

public override Matrix4x4 GetTransformMatrix(Vector3Int position) {
	var tile = m_Tilemap.GetEditorPreviewTile(position);
	return tile ? m_Tilemap.GetEditorPreviewTransformMatrix(position)
	: m_Tilemap.GetTransformMatrix(position);
}

public override TileFlags GetTileFlags(Vector3Int position) {
	var tile = m_Tilemap.GetEditorPreviewTile(position);
	return tile ? m_Tilemap.GetEditorPreviewTileFlags(position)
	: m_Tilemap.GetTileFlags(position);
}
...
private TileBase CreateInvalidTile() {
	...
	tile.sprite = sprite;
	// Try to get a pinkish look with a random color to differentiate between
	// other invalid tiles
	tile.color =
		UnityEngine.Random.ColorHSV(340f / 360f, 1f, 0.3f, 0.6f, 0.7f, 1.0f);
