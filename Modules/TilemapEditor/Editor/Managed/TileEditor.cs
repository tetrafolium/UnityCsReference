...
public static readonly GUIContent invalidMatrixLabel =
	EditorGUIUtility.TrTextContent(
		"Invalid Matrix",
		"No valid Position / Rotation / Scale components available for this matrix");
public static readonly GUIContent resetMatrixLabel =
	EditorGUIUtility.TrTextContent("Reset Matrix");
public static readonly GUIContent previewLabel =
	EditorGUIUtility.TrTextContent("Preview",
	                               "Preview of tile with attributes set");

public static readonly GUIContent spriteEditorLabel =
	EditorGUIUtility.TrTextContent("Sprite Editor");
public static readonly GUIContent offsetLabel =
	EditorGUIUtility.TrTextContent("Offset");
public static readonly GUIContent rotationLabel =
	EditorGUIUtility.TrTextContent("Rotation");
public static readonly GUIContent scaleLabel =
...
public override void OnInspectorGUI() {
	...
	using (new EditorGUI.DisabledGroupScope(m_Sprite.objectReferenceValue ==
	                                        null)) {
...
public static void DoTilePreview(Sprite sprite, Color color,
                                 Matrix4x4 transform) {
	if (sprite == null)
		return;
	...
	guiRect =
		EditorGUI.PrefixLabel(guiRect, new GUIContent(Styles.previewLabel));
	Rect previewRect =
		new Rect(guiRect.xMin, guiRect.yMin, k_PreviewWidth, k_PreviewHeight);
	Rect borderRect = new Rect(guiRect.xMin - 1, guiRect.yMin - 1,
	                           k_PreviewWidth + 2, k_PreviewHeight + 2);

	if (Event.current.type == EventType.Repaint)
		EditorStyles.textField.Draw(borderRect, false, false, false, false);

	Texture2D texture =
		SpriteUtility.RenderStaticPreview(sprite, color, 32, 32, transform);
...
public static Matrix4x4 TransformMatrixOnGUI(Matrix4x4 matrix) {
	...
	if (matrix.ValidTRS()) {
		...
		if (EditorGUI.EndChangeCheck() && scale.x != 0f && scale.y != 0f &&
		    scale.z != 0f) {
...
private static Vector3 Round(Vector3 value, int digits) {
	...
	return new Vector3(Mathf.Round(value.x * mult) / mult,
...
public override Texture2D RenderStaticPreview(string assetPath,
                                              ...
                                              int height) {
	return SpriteUtility.RenderStaticPreview(tile.sprite, tile.color, width,
	                                         height);
}
}
}
