...
public static readonly GUIContent animationFrameRateLabel =
	EditorGUIUtility.TrTextContent(
		"Animation Frame Rate",
		"Frame rate for playing animated tiles in the tilemap");
public static readonly GUIContent tilemapColorLabel =
	EditorGUIUtility.TrTextContent(
		"Color", "Color tinting all Sprites from tiles in the tilemap");
public static readonly GUIContent tileAnchorLabel =
	EditorGUIUtility.TrTextContent(
		"Tile Anchor",
		"Anchoring position for Sprites from tiles in the tilemap");
public static readonly GUIContent orientationLabel =
	EditorGUIUtility.TrTextContent("Orientation",
	                               "Orientation for tiles in the tilemap");
public static readonly string pointTopHexagonCreateUndo =
	L10n.Tr("Hexagonal Point Top Tilemap");
public static readonly string flatTopHexagonCreateUndo =
	L10n.Tr("Hexagonal Flat Top Tilemap");
public static readonly string isometricCreateUndo =
	L10n.Tr("Isometric Tilemap");
public static readonly string isometricZAsYCreateUndo =
	L10n.Tr("Isometric Z As Y Tilemap");
}

private void OnEnable() {
	m_AnimationFrameRate =
		serializedObject.FindProperty("m_AnimationFrameRate");
	...
	m_OrientationMatrix =
		serializedObject.FindProperty("m_TileOrientationMatrix");
}
...
public override void OnInspectorGUI() {
	...
	EditorGUILayout.PropertyField(m_AnimationFrameRate,
	                              Styles.animationFrameRateLabel);
	...
	GUI.enabled = (!m_Orientation.hasMultipleDifferentValues &&
	               Tilemap.Orientation.Custom == tilemap.orientation);
	if (targets.Length > 1)
		EditorGUILayout.PropertyField(m_OrientationMatrix, true);
	else {
		EditorGUI.BeginChangeCheck();
		var orientationMatrix =
			TileEditor.TransformMatrixOnGUI(tilemap.orientationMatrix);
...
private Bounds OnGetFrameBounds() {
	...
	Bounds bounds = new Bounds(
		tilemap.transform.TransformPoint(localBounds.center), Vector3.zero);
	for (int i = 0; i < 8; ++i) {
		...
		var worldPoint =
			tilemap.transform.TransformPoint(localBounds.center + extent);
...
internal static void CreateRectangularTilemap() {
	...
	var uniqueName =
		GameObjectUtility.GetUniqueNameForSibling(root.transform, "Tilemap");
	var tilemapGO = ObjectFactory.CreateGameObject(uniqueName, typeof(Tilemap),
...
internal static void CreateHexagonalPointTopTilemap() {
	CreateHexagonalTilemap(GridLayout.CellSwizzle.XYZ,
	                       Styles.pointTopHexagonCreateUndo);
}

[MenuItem("GameObject/2D Object/Hexagonal Flat Top Tilemap")]
internal static void CreateHexagonalFlatTopTilemap() {
	CreateHexagonalTilemap(GridLayout.CellSwizzle.YXZ,
	                       Styles.flatTopHexagonCreateUndo);
}

[MenuItem("GameObject/2D Object/Isometric Tilemap")]
internal static void CreateIsometricTilemap() {
	CreateIsometricTilemap(GridLayout.CellLayout.Isometric,
	                       Styles.isometricCreateUndo);
}

[MenuItem("GameObject/2D Object/Isometric Z As Y Tilemap")]
internal static void CreateIsometricZAsYTilemap() {
	CreateIsometricTilemap(GridLayout.CellLayout.IsometricZAsY,
	                       Styles.isometricZAsYCreateUndo);
}

private static void
CreateIsometricTilemap(GridLayout.CellLayout isometricLayout,
                       string undoMessage) {
	var root = FindOrCreateRootGrid();
	var uniqueName =
		GameObjectUtility.GetUniqueNameForSibling(root.transform, "Tilemap");
	...
	// Case 1071703: Do not reset cell size if adding a new Tilemap to an
	// existing Grid of the same layout
...
private static GameObject FindOrCreateRootGrid() {
	...
	// If neither the active object nor its parent has a grid, create a grid for
	// the tilemap
	if (gridGO == null) {
