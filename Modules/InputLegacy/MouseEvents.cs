...
internal class SendMouseEvents {
  ...
  static readonly HitInfo[] m_LastHit = {new HitInfo(), new HitInfo(),
                                         new HitInfo()};
  static readonly HitInfo[] m_MouseDownHit = {new HitInfo(), new HitInfo(),
                                              new HitInfo()};
  static readonly HitInfo[] m_CurrentHit = {new HitInfo(), new HitInfo(),
                                            new HitInfo()};
  ...
  static void DoSendMouseEvents(int skipRTCameras) {
    ...
    // If UnityGUI has the mouse over, we simply don't do any mouse hit
    // detection. That way, it will appear as if the mouse has missed
    // everything.
    if (!s_MouseUsed) {
      foreach (var camera in m_Cameras) {
        // we do not want to check cameras that are rendering to textures,
        // starting with 4.0
        if (camera == null ||
            skipRTCameras != 0 && camera.targetTexture != null)
          continue;
        ...
        if (eventPosition != Vector3.zero) {
          ...
          // Discard events that are not part of this display so the user does
          // not interact with multiple displays at once.
          if (eventDisplayIndex != displayIndex)
            continue;

          // Multiple display support only when not the main display. For
          // display 0 the reported resolution is always the desktop resolution
          // since it's part of the display API, so we use the standard non
          // multiple display method.
          ...
          // it is not supported the returned position will be all zeros so when
          // the returned index is 0 we will default to the mouse position to be
          // safe.
        ...
        // There is no need to continue if the camera shouldn't be sending out
        // events
        ...
        var distanceToClipPlane =
            Mathf.Approximately(0.0f, projectionDirection)
                ? Mathf.Infinity
                : Mathf.Abs((camera.farClipPlane - camera.nearClipPlane) /
                            projectionDirection);

        // Did we hit any 3D colliders?
        var hit3D = CameraRaycastHelper.RaycastTry(
            camera, screenProjectionRay, distanceToClipPlane,
            camera.cullingMask & camera.eventMask);
        if (hit3D != null) {
          m_CurrentHit[m_HitIndexPhysics3D].target = hit3D;
          m_CurrentHit[m_HitIndexPhysics3D].camera = camera;
        }
        // We did not hit anything with a raycast from this camera. But our
        // camera clears the screen and renders on top of whatever was below,
        // thus making things rendered before invisible. So clear any previous
        // hit we have found.
        ...
        var hit2D = CameraRaycastHelper.RaycastTry2D(
            camera, screenProjectionRay, distanceToClipPlane,
            camera.cullingMask & camera.eventMask);
        if (hit2D != null) {
