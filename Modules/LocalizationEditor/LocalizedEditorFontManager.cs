...
internal class LocalizedEditorFontManager {
  ...
  private class FontSetting {
    ...
    public FontSetting(string[] fontNames) { m_fontNames = fontNames; }
  ...
  private static void ReadFontSettings() {
    ...
    if (!System.IO.File.Exists(filepath)) {
      filepath = EditorApplication.applicationContentsPath +
                 "/Resources/fontsettings.txt";
    }
    if (!System.IO.File.Exists(filepath)) {
    ...
    foreach (var line0 in lines) {
      ...
      var lang = (SystemLanguage)
                     System.Enum.Parse(typeof(SystemLanguage), cols [0]
                                                                   .Trim());
      string[] pair = cols [1]
                          .Split('=');
      if (pair.Length != 2) {
      ...
      string baseFontName = pair [0]
                                .Trim();
      string[] fontNames = pair [1]
                               .Split(',');
      ...
      m_fontDictionaries [lang]
          .Add(baseFontName, new FontSetting(fontNames));
    }
  }
  ...
  public static void LocalizeEditorFonts() {
    ...
    /*
    We have the Lucida Grande ttf asset which was always the font we used in the
    editor. But on windows we call ReplaceFontForWindows to override the font
    used for that asset. It would probably make sense to rename the asset to
    something generic, such as "EditorFont" instead, though that might possibly
    break some things.
    */
    ReplaceFontForLocalization(
        dict, (Font) EditorGUIUtility.LoadRequired(EditorResources.fontsPath +
                                                   "Lucida Grande.ttf"));
    ReplaceFontForLocalization(
        dict, (Font) EditorGUIUtility.LoadRequired(EditorResources.fontsPath +
    ...
    ReplaceFontForLocalization(
        dict, (Font) EditorGUIUtility.LoadRequired(EditorResources.fontsPath +
                                                   "Lucida Grande Small.ttf"));
    ReplaceFontForLocalization(
        dict, (Font) EditorGUIUtility.LoadRequired(
                  EditorResources.fontsPath + "Lucida Grande Small Bold.ttf"));
    ReplaceFontForLocalization(
        dict, (Font) EditorGUIUtility.LoadRequired(EditorResources.fontsPath +
                                                   "Lucida Grande Big.ttf"));
  }
  ...
  static void ReplaceFontForLocalization(FontDictionary dict, Font font) {
    if (dict != null && dict.ContainsKey(font.name)) {
      ...
      // Avoid the font to be unloaded in
      // EditorUtility.UnloadUnusedAssetsIgnoreManagedReferences and lose the
