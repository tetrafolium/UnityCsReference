...
namespace UnityEditor.PackageManager.UI {
  internal static class PackageManagerProjectSettingsProvider {
    ...
    private static class Styles {
      public static readonly GUIContent EnablePreviewPackagesLabel =
          new GUIContent(L10n.Tr("Enable Preview Packages"));
    ...
    internal static SettingsProvider CreateProjectSettingsProvider() {
      var provider = new SettingsProvider("Project/Package Manager",
                                          SettingsScope.Project){
          guiHandler = searchContext => {
              var settings = ServicesContainer.instance
                                 .Resolve<PackageManagerProjectSettingsProxy>();
      using(new EditorGUILayout.VerticalScope(Styles.verticalStyle)) {
        var newValue = EditorGUILayout.Toggle(Styles.EnablePreviewPackagesLabel,
                                              settings.enablePreviewPackages);
        if (newValue != settings.enablePreviewPackages) {
          var saveIt = true;
          if (newValue && !settings.oneTimeWarningShown) {
            if (EditorUtility.DisplayDialog(
                    L10n.Tr("Package Manager"), L10n.Tr(k_Message),
        ...
        HelpBox(L10n.Tr(k_Message), L10n.Tr("Read more"), () => {
          var applicationProxy =
              ServicesContainer.instance.Resolve<ApplicationProxy>();
          applicationProxy.OpenURL(
              $"https://docs.unity3d.com/{applicationProxy.shortUnityVersion}/Documentation/Manual/pack-preview.html");
        });
      }
    }
    ,
...
private static void HelpBox(string message, string link, Action linkClicked) {
  ...
  GUILayout.Label(EditorGUIUtility.GetHelpIcon(MessageType.Info),
