...
namespace UnityEditor.PackageManager.UI {
  ...
  internal class PackageFiltering {
    public virtual event Action<PackageFilterTab>
        onFilterTabChanged = delegate{};
    ...
    internal static bool FilterByTab(IPackage package, PackageFilterTab tab,
                                     bool showDependencies, bool isLoggedIn) {
      switch (tab) {
      case PackageFilterTab.BuiltIn:
        return package.Is(PackageType.BuiltIn);
      case PackageFilterTab.All:
        return package.Is(PackageType.Installable) &&
               (package.isDiscoverable ||
                (package.versions.installed?.isDirectDependency ?? false));
    ...
    internal static bool FilterByText(IPackage package, IPackageVersion version,
                                      string text) {
      ...
      if (version.name.IndexOf(text,
                               StringComparison.CurrentCultureIgnoreCase) >= 0)
        return true;

      if (!string.IsNullOrEmpty(version.displayName) &&
          version.displayName.IndexOf(
              text, StringComparison.CurrentCultureIgnoreCase) >= 0)
        return true;
      ...
      if (version.version != null &&
          ((SemVersion) version.version)
                  .Prerelease.IndexOf(
                      prerelease, StringComparison.CurrentCultureIgnoreCase) >=
              0)
        return true;

      if (version.HasTag(PackageTag.Preview) &&
          PackageTag.Preview.ToString().IndexOf(
              text, StringComparison.CurrentCultureIgnoreCase) >= 0)
        return true;

      if (version.HasTag(PackageTag.Verified) &&
          PackageTag.Verified.ToString().IndexOf(
              text, StringComparison.CurrentCultureIgnoreCase) >= 0)
        return true;

      if (version.version?.StripTag().StartsWith(
              text, StringComparison.CurrentCultureIgnoreCase) == true)
        return true;
                ...
                word =>
                    ...
                    categories.Any(
                        category => category.StartsWith(
                            word, StringComparison.CurrentCultureIgnoreCase))))
          return true;
      }
    ...
    public virtual bool FilterByCurrentSearchText(IPackage package) {
      ...
      return string.IsNullOrEmpty(trimText) ||
             FilterByText(package, package.versions.primary, trimText);
    }
