...
namespace UnityEditor.PackageManager.UI {
  ...
  internal class PackageDatabase : ISerializationCallbackReceiver {
    // This instance reference is kept for compatibility reasons, as it is
    // internal visible to the Upm Develop package To be addressed further in
    // https://jira.unity3d.com/browse/PAX-1317
    internal static PackageDatabase instance =>
        ServicesContainer.instance.Resolve<PackageDatabase>();

    public virtual event Action<IPackage, IPackageVersion>
        onInstallSuccess = delegate{};
    ...
    public virtual event Action<
        IEnumerable<IPackage> /*added*/, IEnumerable<IPackage> /*removed*/,
        IEnumerable<IPackage> /*preUpdated*/,
        IEnumerable<IPackage> /*postUpdated*/> onPackagesChanged = delegate{};

    private readonly Dictionary<string, IPackage> m_Packages =
        new Dictionary<string, IPackage>();

    private readonly Dictionary<string, IEnumerable<Sample>> m_ParsedSamples =
        new Dictionary<string, IEnumerable<Sample>>();
    ...
    [SerializeField]
    private List<AssetStorePackage> m_SerializedAssetStorePackages =
        new List<AssetStorePackage>();

    [SerializeField]
    private List<PlaceholderPackage> m_SerializedPlaceholderPackages =
        new List<PlaceholderPackage>();
    ...
    public void ResolveDependencies(
        UnityConnectProxy unityConnect, AssetDatabaseProxy assetDatabase,
        AssetStoreUtils assetStoreUtils, AssetStoreClient assetStoreClient,
        AssetStoreDownloadManager assetStoreDownloadManager,
        UpmClient upmClient, IOProxy ioProxy) {
    ...
    public virtual IEnumerable<IPackage> allPackages {
      get { return m_Packages.Values; }
    }
    public virtual IEnumerable<IPackage> assetStorePackages {
      get { return m_Packages.Values.Where(p => p is AssetStorePackage); }
    }
    public virtual IEnumerable<IPackage> upmPackages {
      get { return m_Packages.Values.Where(p => p is UpmPackage); }
    }
    ...
    public virtual bool IsInstallInProgress(IPackageVersion version) {
      return m_UpmClient.IsAddInProgress(version.uniqueId) ||
             m_UpmClient.IsEmbedInProgress(version.uniqueId);
    }
    ...
    // In some situations, we only know an id (could be package unique id, or
    // version unique id) or just a name (package Name, or display name) but we
    // still might be able to find a package and a version that matches the
    // criteria
    public virtual void
    GetPackageAndVersionByIdOrName(string idOrName, out IPackage package,
                                   out IPackageVersion version) {
      // GetPackage by packageUniqueId itself is not an expensive operation, so
      // we want to try and see if the input string is a packageUniqueId first.
      ...
      // if we are able to break the string into two by looking at '@' sign,
      // it's possible that the input idOrDisplayName is a versionId
      var idOrDisplayNameSplit = idOrName?.Split(new[]{'@'}, 2);
      if (idOrDisplayNameSplit?.Length == 2) {
        var packageUniqueId = idOrDisplayNameSplit[0];
        GetPackageAndVersion(packageUniqueId, idOrName, out package,
                             out version);
        if (package != null)
          return;
      }

      // If none of those find-by-index options work, we'll just have to find it
      // the brute force way by matching the name & display name
      package = m_Packages.Values.FirstOrDefault(
          p => p.name == idOrName || p.displayName == idOrName);
      version = null;
    }

    public virtual void GetPackageAndVersion(string packageUniqueId,
                                             string versionUniqueId,
                                             out IPackage package,
                                             out IPackageVersion version) {
      package = GetPackage(packageUniqueId);
      version =
          package?.versions.FirstOrDefault(v => v.uniqueId == versionUniqueId);
    }

    public virtual IPackageVersion GetPackageVersion(string packageUniqueId,
                                                     string versionUniqueId) {
      IPackage package;
      IPackageVersion version;
      GetPackageAndVersion(packageUniqueId, versionUniqueId, out package,
                           out version);
      return version;
    }
    ...
    public virtual IPackageVersion GetPackageVersion(DependencyInfo info) {
      IPackage package;
      IPackageVersion version;
      GetUpmPackageAndVersion(info.name, info.version, out package,
                              out version);
      return version;
    }

    private void GetUpmPackageAndVersion(string name, string versionIdentifier,
                                         out IPackage package,
                                         out IPackageVersion version) {
      ...
      // the versionIdentifier could either be SemVersion or file, git or ssh
      // reference and the two cases are handled differently.
      if (!string.IsNullOrEmpty(versionIdentifier) &&
          char.IsDigit(versionIdentifier.First())) {
        SemVersion? parsedVersion;
        SemVersionParser.TryParse(versionIdentifier, out parsedVersion);
        version =
            package.versions.FirstOrDefault(v => v.version == parsedVersion);
      } else {
        var packageId =
            UpmPackageVersion.FormatPackageId(name, versionIdentifier);
        version = package.versions.FirstOrDefault(v => v.uniqueId == packageId);
      }
    }

    public virtual IEnumerable<IPackageVersion>
    GetReverseDependencies(IPackageVersion version) {
      if (version?.dependencies == null)
        return null;
      var installedRoots = allPackages.Select(p => p.versions.installed)
    ...
    private IEnumerable<Sample>
    GetSamplesFromPackageInfo(PackageInfo packageInfo) {
      ...
      try {
        var packageJson = Json.Deserialize(m_IOProxy.FileReadAllText(jsonPath))
                              as Dictionary<string, object>;
        var samples =
            packageJson.GetList<IDictionary<string, object>>("samples");
        return samples?.Select(sample => {
          ...
          var importPath = IOUtils.CombinePaths(
              Application.dataPath, "Samples",
              IOUtils.SanitizeFileName(packageInfo.displayName),
              packageInfo.version, IOUtils.SanitizeFileName(displayName));
          return new Sample(m_IOProxy, m_AssetDatabase, displayName,
                            description, resolvedSamplePath, importPath,
                            interactiveImport);
        });
      } catch (Exception) {
        return Enumerable.Empty<Sample>();
    ...
    public virtual IEnumerable<Sample> GetSamples(IPackageVersion version) {
      if (version?.packageInfo == null ||
          version.packageInfo.version != version.version?.ToString())
        return Enumerable.Empty<Sample>();
    ...
    public virtual void AddPackageError(IPackage package, UIError error) {
      ...
      onPackagesChanged?.Invoke(k_EmptyList, k_EmptyList,
                                new[]{packagePreUpdate}, new[]{package});
    }
    ...
    public virtual IEnumerable<IPackage> packagesInError => allPackages.Where(
        p => p.errors.Any());

    public virtual void SetPackageProgress(IPackage package,
    ...
    public virtual void Reload() {
      onPackagesChanged?.Invoke(Enumerable.Empty<IPackage>(), m_Packages.Values,
    ...
    private void OnDownloadProgress(IOperation operation) {
      ...
      SetPackageProgress(package,
                         operation.isInProgress ? PackageProgress.Downloading
                         : PackageProgress.None);
    }
    ...
    private void OnUserLoginStateChange(bool userInfoReady, bool loggedIn) {
      if (!loggedIn) {
        var assetStorePackages =
            m_Packages.Where(kp => kp.Value is AssetStorePackage)
                .Select(kp => kp.Value)
                .ToList();
        ...
        onPackagesChanged?.Invoke(k_EmptyList, assetStorePackages, k_EmptyList,
                                  k_EmptyList);
      }
    }
    ...
    private void OnPackagesChanged(IEnumerable<IPackage> packages) {
      ...
      foreach (var package in packages) {
        ...
        if (m_SpecialInstallations.Any() &&
            package.versions.installed != null &&
            oldPackage?.versions.installed == null)
          packagesInstalled.Add(package);
      }

      if (packagesAdded.Count + packagesRemoved.Count +
              packagesPostUpdate.Count >
          0)
        onPackagesChanged?.Invoke(packagesAdded, packagesRemoved,
                                  packagesPreUpdate, packagesPostUpdate);

      // special handling to make sure onInstallSuccess events are called
      // correctly when special unique id is used
      for (var i = m_SpecialInstallations.Count - 1; i >= 0; i--) {
        var match = packagesInstalled.FirstOrDefault(
            p => p.versions.installed.uniqueId.ToLower().Contains(
                m_SpecialInstallations [i]
                    .ToLower()));
        if (match != null) {
    ...
    private void OnUpmAddOperation(IOperation operation) {
      // if we don't know the package unique id before hand, we'll do some
      // special handling as we don't know what package will be installed until
      // the installation finishes (e.g, git packages)
      ...
      SetPackageProgress(GetPackage(operation.packageUniqueId),
                         PackageProgress.Installing);
      operation.onOperationSuccess += (op) => {
        IPackage package;
        IPackageVersion version;
        GetPackageAndVersion(operation.packageUniqueId,
                             operation.versionUniqueId, out package,
                             out version);
    ...
    private void OnUpmRemoveOperation(IOperation operation) {
      SetPackageProgress(GetPackage(operation.packageUniqueId),
                         PackageProgress.Removing);
      operation.onOperationSuccess += (op) =>
          onUninstallSuccess(GetPackage(operation.packageUniqueId));
      operation.onOperationError += OnUpmOperationError;
      operation.onOperationFinalized += OnUpmOperationFinalized;
    }
    ...
    private void OnUpmPackageVersionUpdated(string packageUniqueId,
                                            IPackageVersion version) {
    ...
    public virtual void Embed(IPackageVersion packageVersion) {
      if (packageVersion == null ||
          !packageVersion.HasTag(PackageTag.Embeddable))
        return;
    ...
    public virtual bool IsDownloadInProgress(IPackageVersion version) {
      return m_AssetStoreDownloadManager.GetDownloadOperation(
    ...
    public virtual void Download(IPackage package) {
      ...
      // When we start a new download, we want to clear past operation errors to
      // give it a fresh start. Eventually we want a better design on how to
      // show errors, to be further addressed in
      // https://jira.unity3d.com/browse/PAX-1332
      package.ClearErrors(e => e.errorCode ==
                               UIErrorCode.AssetStoreOperationError);
    }
