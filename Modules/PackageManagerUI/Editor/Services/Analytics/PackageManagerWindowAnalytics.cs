...
internal struct PackageManagerWindowAnalytics {
	...
	public static void Setup() {
		...
		EditorAnalytics.RegisterEventWithLimit(
			"packageManagerWindowUserAction", maxEventsPerHour,
			maxNumberOfElementInStruct, vendorKey);
	}
	...
	public static void SendEvent(string action, string packageId = null) {
		...
		if (!string.IsNullOrEmpty(packageId))
			packageId =
				Regex.Replace(packageId, "(?<package>[^@]+)@(?<protocol>[^:]+):.+",
				              "${package}@${protocol}");

		var packageFiltering =
			ServicesContainer.instance.Resolve<PackageFiltering>();
		var packageManagerPrefs =
			ServicesContainer.instance.Resolve<PackageManagerPrefs>();
		var settingsProxy = ServicesContainer.instance
		                    .Resolve<PackageManagerProjectSettingsProxy>();

		var parameters = new PackageManagerWindowAnalytics {
			action = action, package_id = packageId ?? string.Empty,
			search_text = packageFiltering.currentSearchText,
			filter_name = packageFiltering.currentFilterTab.ToString(),
			window_docked = EditorWindow.GetWindowDontShow<PackageManagerWindow>()
			                ?.docked ?? false,
		...
		EditorAnalytics.SendEventWithLimit("packageManagerWindowUserAction",
		                                   parameters);
	}
}
}
