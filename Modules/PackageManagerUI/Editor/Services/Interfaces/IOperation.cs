...
// `onOperationProgress` will only be triggered if `isProgressTrackable` is
// true
event Action<IOperation> onOperationProgress;

// the special unique id is used when neither package unique id or version
// unique id applies e.g. git url, tar ball path that does not contain any
...
// in the case of an online operation, it is the time when the operation
// starts in the case of an offline operation, it is set to the timestamp of
// the last online operation
long timestamp {
	get;
}
