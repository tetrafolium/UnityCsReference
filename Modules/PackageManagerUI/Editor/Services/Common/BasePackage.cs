...
namespace UnityEditor.PackageManager.UI {
  internal abstract class BasePackage : IPackage {
    ...
    public string displayName => versions.FirstOrDefault()
    ...
    // Stop lookup after first error encountered on a version to save time not
    // looking up redundant errors.
    ...
    public bool Is(PackageType type) { return (m_Type & type) != 0; }

    [SerializeField]
    protected long m_FirstPublishedDateTicks;
    public DateTime? firstPublishedDate =>
        m_FirstPublishedDateTicks == 0
            ? null
            : (DateTime?) new DateTime(m_FirstPublishedDateTicks,
                                       DateTimeKind.Utc);

    public virtual bool isDiscoverable => true;

    public virtual IEnumerable<PackageImage>
        images => Enumerable.Empty<PackageImage>();
    public virtual IEnumerable<PackageLink> links => Enumerable
                                                         .Empty<PackageLink>();
