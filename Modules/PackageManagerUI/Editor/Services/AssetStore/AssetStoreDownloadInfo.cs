...
namespace UnityEditor.PackageManager.UI {
  ...
  internal class AssetStoreDownloadInfo {
    ...
    public string[] destination => new string[]{publisherName.Replace(".", ""),
                                                categoryName.Replace(".", ""),
                                                packageName.Replace(".", "")};

    public static AssetStoreDownloadInfo
    ParseDownloadInfo(IDictionary<string, object> rawInfo) {
      if (rawInfo?.Any() != true)
        return null;

      try {
        var download =
            rawInfo.GetDictionary("result").GetDictionary("download");
