...
namespace UnityEditor.PackageManager.UI {
  ...
  internal class PurchasesQueryArgs : PageFilters {
    ...
    public override string ToString() {
      ...
      var stringBuilder =
          new StringBuilder($"?offset={startIndex}&limit={limit}", 512);
      if (!string.IsNullOrEmpty(searchText))
        ...
        if (labels?.Any() ?? false)
          stringBuilder.Append(
              $"&tagging={string.Join(",
              ", labels.Select(label => Uri.EscapeDataString(label)).ToArray())}");
        if (categories?.Any() ?? false)
          stringBuilder.Append(
              $"&categories={string.Join(",
              ", categories.Select(cat => Uri.EscapeDataString(cat)).ToArray())}");
  ...
  internal class AssetStorePurchases {
    ...
    public List<AssetStorePurchaseInfo> list =
        new List<AssetStorePurchaseInfo>();
    ...
    public void ParsePurchases(IDictionary<string, object> rawList) {
      ...
      var results = rawList.GetList<Dictionary<string, object>>("results")
          ?? Enumerable.Empty<Dictionary<string, object>>();
      foreach (var item in results)
        list.Add(AssetStorePurchaseInfo.ParsePurchaseInfo(item));

      var categories = rawList.GetList<Dictionary<string, object>>("category")
          ?? Enumerable.Empty<Dictionary<string, object>>();
      foreach (var item in categories)
        this.categories.Add(new Category{name = item.GetString("name"),
