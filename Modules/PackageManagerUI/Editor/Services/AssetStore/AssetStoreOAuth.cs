...
namespace UnityEditor.PackageManager.UI {
  ...
  internal class AssetStoreOAuth {
    ...
    public class AssetStoreToken {
      private const long k_BufferTime =
          15L; // We make sure we still have 15 seconds with the token

      public string accessToken;
      public string expiresIn {
        get { return m_ExpirationIn.ToString(); }
        set {
      ...
      public bool IsValid(long bufferTime = k_BufferTime) {
        return m_ExpirationIn > 0 && (EpochSeconds - m_ExpirationStart) <
                                         (m_ExpirationIn - bufferTime);
      }
    }
    ...
    public class UserInfo {
      ...
      public UserInfo(Dictionary<string, object> rawData, TokenInfo tokenInfo) {
        ...
        defaultOrganization = rawData.GetDictionary("extendedProperties")
            ?.GetString("UNITY_DEFAULT_ORGANIZATION");
    ...
    // if the OAuth singleton does go through serialization, all events are
    // destroyed and callbacks won't be triggered therefore we choose not to
    // serialize this filed so that we'll request auth code again
    [NonSerialized]
    ...
    // this onError callback is shared between all steps in this process, no
    // matter which step has the error, we'll call this event
    ...
    private string host {
      get {
        if (string.IsNullOrEmpty(m_Host))
          m_Host =
              m_UnityConnect.GetConfigurationURL(CloudConfigUrl.CloudIdentity);
        return m_Host;
      }
    }
    ...
    public virtual void FetchUserInfo(Action<UserInfo> doneCallback,
                                      Action<UIError> errorCallback) {
    ...
    private void GetAuthCode(Action<string> doneCallback) {
      ...
      // if the result is not cached already, we will register the callback to
      // be called later async this is needed because we only want to have one
      // request going at a time and don't want to lose any callback events
      ...
      try {
        m_UnityOAuth.GetAuthorizationCodeAsync(
            k_ServiceId, authCodeResponse => {
              m_AuthCode = "";
              m_AuthCodeRequested = false;
    ...
    private void GetAccessToken(Action<AccessToken> doneCallback) {
      GetAuthCode(authCode => {
            ...
            var secret = m_UnityConnect.GetConfigurationURL(
                CloudConfigUrl.CloudPackagesKey);
            ...
            m_AccessTokenRequest.header["Content-Type"] =
                "application/x-www-form-urlencoded";
            m_AccessTokenRequest.doneCallback = httpClient => {
              m_AccessTokenRequest = null;
              m_AccessToken = null;

              var response = AssetStoreUtils.ParseResponseAsDictionary(
                  httpClient, OnGetAccessTokenError);
              if (response != null) {
    ...
    private void GetTokenInfo(Action<TokenInfo> doneCallback) {
      GetAccessToken(accessToken => {
            ...
            m_TokenRequest = m_HttpClientFactory.GetASyncHTTPClient(
                $"{host}{k_TokenInfoUri}?access_token={accessToken.accessToken}");
            m_TokenRequest.doneCallback = httpClient => {
              m_TokenRequest = null;
              m_TokenInfo = null;

              var response = AssetStoreUtils.ParseResponseAsDictionary(
                  httpClient, OnOperationError);
              if (response != null) {
    ...
    private void GetUserInfo(Action<UserInfo> doneCallback) {
      GetTokenInfo(tokenInfo => {
        ...
        m_UserInfoRequest = m_HttpClientFactory.GetASyncHTTPClient(
            $"{host}{k_UserInfoUri}/{tokenInfo.sub}");
        m_UserInfoRequest.header["Authorization"] =
        ...
        m_UserInfoRequest.doneCallback = httpClient => {
          ...
          if (response != null) {
            ...
            if (userInfo.isValid) {
              ...
              // note that we only clear error callbacks on the when user info
              // is fetched as we need the error callback to be present for the
              // whole process.
              m_OnError = null;
            } else
    ...
    private void OnGetAccessTokenError(string errorMessage) {
      // when we have trouble getting access token, it's most likely because the
      // auth code is no longer valid. therefore we want to clear the auth code
      // in the case of error, such that new auth code will be fetched in the
      // next refresh
    ...
    private void OnOperationError(string errorMessage) {
      m_OnError?.Invoke(
          new UIError(UIErrorCode.AssetStoreAuthorizationError, errorMessage));
      m_OnError = null;
    }
  }
}
