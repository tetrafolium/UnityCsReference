...
namespace UnityEditor.PackageManager.UI {
  ...
  internal class AssetStoreLocalInfo {
    ...
    public static AssetStoreLocalInfo
    ParseLocalInfo(UnityEditor.PackageInfo localInfo) {
      if (string.IsNullOrEmpty(localInfo.jsonInfo))
        return null;

      try {
        var jsonInfo =
            Json.Deserialize(localInfo.jsonInfo) as Dictionary<string, object>;
        var id = jsonInfo?.GetString("id");
        if (string.IsNullOrEmpty(id))
          return null;

        return new AssetStoreLocalInfo{
            id = id, packagePath = localInfo.packagePath ?? string.Empty,
            ...
            supportedVersion =
                jsonInfo.GetString("unity_version") ?? string.Empty,
            publishNotes = jsonInfo.GetString("publishnotes") ?? string.Empty,
            updateInfoFetched = false, canUpdate = false };
      } catch (Exception) {
        return null;
      }
    }
    ...
    public Dictionary<string, string> ToDictionary() {
      return new Dictionary<string, string>{["local_path"] =
