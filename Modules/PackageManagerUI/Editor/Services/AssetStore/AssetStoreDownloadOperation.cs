...
internal static readonly string k_DownloadErrorMessage =
	"The download could not be completed. Please try again.";
internal static readonly string k_AbortErrorMessage =
	"The download could not be aborted. Please try again.";
...
public bool isInProgress => state == DownloadState.Connecting ||
state == DownloadState.Downloading ||
state == DownloadState.Decrypting;

public bool isInPause =>
state == DownloadState.Paused || state == DownloadState.Pausing;

public bool isProgressVisible =>
state == DownloadState.ResumeRequested || isInProgress || isInPause;

public bool isProgressTrackable => true;

public float progressPercentage => m_TotalBytes > 0
                                           ? m_DownloadedBytes /
(float) m_TotalBytes
                                           : 0.0f;
...
public AssetStoreDownloadOperation(AssetStoreUtils assetStoreUtils,
                                   AssetStoreRestAPI assetStoreRestAPI,
                                   string productId) {
...
public void OnDownloadProgress(string message, ulong bytes, ulong total) {
	...
	case "aborted":
		if (!isInPause) {
			...
			onOperationError?.Invoke(
				this, new UIError(UIErrorCode.AssetStoreOperationError,
				                  m_ErrorMessage));
			onOperationFinalized?.Invoke(this);
		} else {
...
private void OnErrorMessage(string errorMessage) {
	...
	onOperationError?.Invoke(
		this,
		new UIError(UIErrorCode.AssetStoreOperationError, m_ErrorMessage,
		            UIError.Attribute.IsDetailInConsole));
	onOperationFinalized?.Invoke(this);
}
...
public void Pause() {
	...
	if (state == DownloadState.Aborted || state == DownloadState.Completed ||
	    state == DownloadState.Error || state == DownloadState.Paused)
		return;

	m_State = DownloadState.Pausing;

	// Pause here is the same as aborting the download, but we don't delete
	// the file so we can resume from where we paused it from
	if (!m_AssetStoreUtils.AbortDownload(
		    $"{k_AssetStoreDownloadPrefix}{m_ProductId}",
		    downloadInfo.destination))
		Debug.LogError(
			$"{L10n.Tr("[Package Manager
                             Window] ")} {L10n.Tr(k_AbortErrorMessage)}");
}
...
public void Abort() {
	...
	// the actual download state change from `downloading` to `aborted`
	// happens in `OnDownloadProgress` callback
	if (!m_AssetStoreUtils.AbortDownload(
		    $"{k_AssetStoreDownloadPrefix}{m_ProductId}",
		    downloadInfo.destination))
		Debug.LogError(
...
public void Download(bool resume) {
	m_State = resume ? DownloadState.ResumeRequested
	  : DownloadState.DownloadRequested;
	...
	m_AssetStoreRestAPI.GetDownloadDetail(productId, downloadInfo => {
				...
				var json = m_AssetStoreUtils.CheckDownload(
					...
					downloadInfo.url, dest, downloadInfo.key);
				...
				try {
				        ...
				        var inProgress =
						current.ContainsKey("in_progress") &&
						(current["in_progress"] is bool? (bool) current["in_progress"]
						 : false);
				        if (inProgress) {
				                if (!isInPause)
							m_State = DownloadState.Downloading;
				        ...
				        if (current.ContainsKey("download") &&
				            current["download"] is IDictionary<string, object>) {
				                var download = (IDictionary<string, object>)current["download"];
				                var existingUrl = download.ContainsKey("url")
				  ? download["url"] as string
				  : string.Empty;
				                var existingKey = download.ContainsKey("key")
				  ? download["key"] as string
				  : string.Empty;
				                resumeOK = (existingUrl == downloadInfo.url &&
				                            existingKey == downloadInfo.key);
					}
				} catch (Exception e) {
				...
				json =
					$"{{\"download\":{{\"url\":\"{downloadInfo.url}\",\"key\":\"{downloadInfo.key}\"}}}}";
				m_AssetStoreUtils.Download(
					$"{k_AssetStoreDownloadPrefix}{downloadInfo.productId}",
					downloadInfo.url, dest, downloadInfo.key, json, resumeOK && resume);
