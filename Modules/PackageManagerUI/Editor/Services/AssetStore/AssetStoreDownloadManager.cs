...
namespace UnityEditor.PackageManager.UI {
  ...
  internal class AssetStoreDownloadManager : ISerializationCallbackReceiver {
    public virtual event Action<IOperation, UIError>
        onDownloadError = delegate{};
    ...
    private Dictionary<string, AssetStoreDownloadOperation>
        m_DownloadOperations =
            new Dictionary<string, AssetStoreDownloadOperation>();

    [SerializeField]
    private AssetStoreDownloadOperation[] m_SerializedDownloadOperations =
        new AssetStoreDownloadOperation[0];
    ...
    // The AssetStoreDownloadManager implementation requires the help of a
    // ScriptableObject to dispatch download progress event.
    private class DownloadDelegateHandler : ScriptableObject {
      public AssetStoreDownloadManager downloadManager {
        get;
        set;
      }

      public void OnDownloadProgress(string downloadId, string message,
                                     ulong bytes, ulong total) {
    ...
    private void RegisterDownloadDelegate() {
      if (m_DownloadDelegateHandler != null)
        return;
      m_DownloadDelegateHandler =
          ScriptableObject.CreateInstance<DownloadDelegateHandler>();
    ...
    public virtual void Download(string productId) {
      ...
      operation = new AssetStoreDownloadOperation(
          m_AssetStoreUtils, m_AssetStoreRestAPI, productId);
      SetupDownloadOperation(operation);
      operation.Download(false);
    }

    public virtual void ClearCache() { AbortAllDownloads(); }

    public virtual AssetStoreDownloadOperation
    GetDownloadOperation(string productId) {
      return string.IsNullOrEmpty(productId)
                 ? null
                 : m_DownloadOperations.Get(productId);
    }
    ...
    private void SetupDownloadOperation(AssetStoreDownloadOperation operation) {
      ...
      operation.onOperationError += (op, error) =>
          onDownloadError?.Invoke(op, error);
    ...
    // This function will be called by AssetStoreUtils after the download
    // delegate registration assetStoreUtils.RegisterDownloadDelegate
    public virtual void OnDownloadProgress(string downloadId, string message,
                                           ulong bytes, ulong total) {
      // The download unique id we receive from the A$ download callback could
      // be in the format of `123456` or `content__123456` (depending on where
      // the download starts). therefore we have a progress that makes sure
      // everything is in the format of `123456`.
      var productId = downloadId;
      if (downloadId.StartsWith(
              AssetStoreDownloadOperation.k_AssetStoreDownloadPrefix))
        productId = downloadId.Substring(
            AssetStoreDownloadOperation.k_AssetStoreDownloadPrefix.Length);

      // `GetDownloadOperation` could return null when the user starts the
      // download in the legacy `Asset Store` window in those cases, we create a
      // download operation to track it
      var operation = GetDownloadOperation(productId);
      if (operation == null) {
        operation = new AssetStoreDownloadOperation(
            m_AssetStoreUtils, m_AssetStoreRestAPI, productId);
