...
ParseResponseAsDictionary(IAsyncHTTPClient request,
                          Action<string> errorMessageCallback) {
	string errorMessage;
	if (request.IsSuccess() && request.responseCode == 200) {
		try {
			var response =
				Json.Deserialize(request.text) as Dictionary<string, object>;
			if (response != null)
				return response;
			...
			errorMessage =
				string.Format(L10n.Tr("Failed to parse JSON: {0}"), e.Message);
		}
	} else {
		if (request.responseCode == 0 && string.IsNullOrEmpty(request.text))
			errorMessage = L10n.Tr("Failed to parse response.");
		else {
			var text = request.text.Length <= 128
			 ? request.text
			 : request.text.Substring(0, 128) + "...";
			errorMessage = string.Format(
				L10n.Tr("Failed to parse response: Code {0} \"{1}\""),
				request.responseCode, text);
		}
	}
...
public virtual void Download(string id, string url, string[] destination,
                             string key, string jsonData, bool resumeOK) {
	UnityAssetStoreUtils.Download(id, url, destination, key, jsonData,
	                              resumeOK);
}

public virtual string CheckDownload(string id, string url,
...
public virtual string assetStoreUrl => m_UnityConnect.GetConfigurationURL(
