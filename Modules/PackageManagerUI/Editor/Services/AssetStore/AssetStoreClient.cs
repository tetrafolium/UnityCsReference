...
namespace UnityEditor.PackageManager.UI {
  ...
  internal class AssetStoreClient {
    ...
    public virtual event Action<AssetStorePurchases, bool>
        onProductListFetched = delegate{};
    ...
    public virtual void ListCategories(Action<List<string>> callback) {
      m_AssetStoreRestAPI.GetCategories(
          result => {
          ...
          error => {
            Debug.LogWarning(string.Format(
                L10n.Tr(
                    "[Package Manager Window] Error while fetching categories: {0}"),
                error.message));
    ...
    public virtual void ListLabels(Action<List<string>> callback) {
      m_AssetStoreRestAPI.GetTaggings(
          result => {
            var labels =
                new List<string>(result.GetList<string>("results").ToList());
            labels.Remove("#BIN");
            labels.Sort();
          ...
          error => {
            Debug.LogWarning(string.Format(
                L10n.Tr(
                    "[Package Manager Window] Error while fetching labels: {0}"),
                error.message));
            callback?.Invoke(new List<string>());
          });
    }
    ...
    public virtual void Fetch(long productId) {
      if (!m_UnityConnect.isUserLoggedIn) {
        onFetchDetailsError?.Invoke(
            new UIError(UIErrorCode.AssetStoreAuthorizationError,
      ...
      if (purchaseInfo != null) {
        ...
        // when the purchase info is not available for a package (either it's
        // not fetched yet or just not available altogether) we'll try to fetch
        // the purchase info first and then call the `FetchInternal`. In the
        // case where a package not purchased, `purchaseInfo` will still be
        // null, but the generated `AssetStorePackage` in the end will contain
        // an error.
        var fetchOperation =
            new AssetStoreListOperation(m_UnityConnect, m_AssetStoreRestAPI);
        var queryArgs =
            new PurchasesQueryArgs{productIds = new List<long>{productId}};
    ...
    private void FetchInternal(long productId,
                               AssetStorePurchaseInfo purchaseInfo) {
      RefreshLocalInfos();
      ...
      // create a placeholder before fetching data from the cloud for the first
      // time
      if (m_AssetStoreCache.GetProductInfo(id) == null)
        onPackagesChanged?.Invoke(new[]{
            new PlaceholderPackage(id, purchaseInfo?.displayName ?? string.Empty,
                                   PackageType.AssetStore, PackageTag.None,
                                   PackageProgress.Refreshing)});
    ...
    public virtual void ListPurchases(PurchasesQueryArgs queryArgs,
      ...
      m_ListOperation = m_ListOperation ?? new AssetStoreListOperation(
          m_UnityConnect, m_AssetStoreRestAPI);
      m_ListOperation.onOperationSuccess += op => {
        ...
        if (result.list.Count > 0) {
          ...
          foreach (var purchaseInfo in result.list) {
            ...
            var oldPurchaseInfo =
                m_AssetStoreCache.GetPurchaseInfo(productIdString);
            m_AssetStoreCache.SetPurchaseInfo(purchaseInfo);

            // create a placeholder before fetching data from the cloud for the
            // first time
            var productInfo = m_AssetStoreCache.GetProductInfo(productIdString);
            if (productInfo == null)
              updatedPackages.Add(new PlaceholderPackage(
                  productIdString, purchaseInfo.displayName,
                  PackageType.AssetStore, PackageTag.None,
                  PackageProgress.Refreshing));
            else if (oldPurchaseInfo != null) {
              // for now, `tags` is the only component in `purchase info` that
              // can be updated over time, so we only check for changes there
              ...
              if (!oldTags.SequenceEqual(newTags))
                updatedPackages.Add(new AssetStorePackage(
                    m_AssetStoreUtils, m_IOProxy, purchaseInfo, productInfo,
                    m_AssetStoreCache.GetLocalInfo(productInfo.id)));
            }
          }
      ...
      foreach (var id in productIds) {
        m_AssetStoreRestAPI.GetProductDetail(id, productDetail => {
          ...
          if (string.IsNullOrEmpty(error)) {
            var productInfo = AssetStoreProductInfo.ParseProductInfo(
                m_AssetStoreUtils, idString, productDetail);
            if (productInfo == null)
              package = new AssetStorePackage(
                  m_AssetStoreUtils, m_IOProxy, idString,
                  new UIError(UIErrorCode.AssetStoreClientError,
                              L10n.Tr("Error parsing product details.")));
            else {
              var oldProductInfo = m_AssetStoreCache.GetProductInfo(idString);
              if (oldProductInfo == null ||
                  oldProductInfo.versionId != productInfo.versionId ||
                  oldProductInfo.versionString != productInfo.versionString) {
                if (string.IsNullOrEmpty(productInfo.packageName))
                  package = new AssetStorePackage(
                      m_AssetStoreUtils, m_IOProxy,
                      m_AssetStoreCache.GetPurchaseInfo(idString), productInfo,
                      m_AssetStoreCache.GetLocalInfo(idString));
                else
                  m_UpmClient.FetchForProduct(idString,
            ...
            package = new PlaceholderPackage(
                idString, purchaseInfo?.displayName ?? string.Empty,
                PackageType.AssetStore, PackageTag.None, PackageProgress.None,
                uiError);
          }
    ...
    private void OnProductPackageChanged(string productId, IPackage package) {
      ...
      if (productInfo != null) {
        var assetStorePackage =
            new AssetStorePackage(m_AssetStoreUtils, m_IOProxy, purchaseInfo,
                                  productInfo, package as UpmPackage);
        onPackagesChanged?.Invoke(new[]{assetStorePackage});
      }
    }

    private void OnProductPackageVersionUpdated(string productId,
    ...
    private void OnProductPackageFetchError(string productId, UIError error) {
      ...
      var productInfo = m_AssetStoreCache.GetProductInfo(productId);
      if (productInfo != null) {
        var assetStorePackage = new AssetStorePackage(
            m_AssetStoreUtils, m_IOProxy, purchaseInfo, productInfo);
        var assetStorePackageVersion =
            assetStorePackage.versions.primary as AssetStorePackageVersion;
    ...
    public void OnEnable() {
      ...
      m_UpmClient.onProductPackageVersionUpdated +=
          OnProductPackageVersionUpdated;
    ...
    public virtual void ClearCache() { m_AssetStoreCache.ClearCache(); }
    ...
    public virtual void RefreshProductUpdateDetails(
        IEnumerable<AssetStoreLocalInfo> localInfos = null) {
      localInfos = localInfos ?? m_AssetStoreCache.localInfos.Where(
          info => !info.updateInfoFetched);
      if (!localInfos.Any())
        return;
      ...
      m_AssetStoreRestAPI.GetProductUpdateDetail(localInfos, updateDetails => {
        if (updateDetails.ContainsKey("errorMessage")) {
          Debug.Log(string.Format(
              L10n.Tr(
                  "[Package Manager Window] Error while getting product update details: {0}"),
              updateDetails["errorMessage"]));
          return;
        }

        var results =
            updateDetails.GetList<IDictionary<string, object>>("results");
        if (results == null)
          return;
    ...
    private void RefreshLocalInfos() {
      ...
      m_AssetStoreCache.SetLocalInfos(
          infos.Select(info => AssetStoreLocalInfo.ParseLocalInfo(info)));
    }

    private void
    OnLocalInfosChanged(IEnumerable<AssetStoreLocalInfo> addedOrUpdated,
                        IEnumerable<AssetStoreLocalInfo> removed) {
      var packagesChanged = new List<IPackage>();
      foreach (var info in addedOrUpdated ??
                   Enumerable.Empty<AssetStoreLocalInfo>()) {
        var productInfo = m_AssetStoreCache.GetProductInfo(info.id);
        if (productInfo == null)
          continue;
        packagesChanged.Add(new AssetStorePackage(
            m_AssetStoreUtils, m_IOProxy,
            m_AssetStoreCache.GetPurchaseInfo(info.id), productInfo, info));
      }
      foreach (var info in removed ?? Enumerable.Empty<AssetStoreLocalInfo>()) {
        ...
        if (productInfo == null)
          continue;
        packagesChanged.Add(
            new AssetStorePackage(m_AssetStoreUtils, m_IOProxy,
                                  m_AssetStoreCache.GetPurchaseInfo(info.id),
                                  productInfo, (AssetStoreLocalInfo) null));
      }
