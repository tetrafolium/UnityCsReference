...
namespace UnityEditor.PackageManager.UI {
  ...
  internal class AssetStoreProductInfo {
    ...
    private AssetStoreProductInfo(AssetStoreUtils assetStoreUtils,
                                  string productId,
                                  IDictionary<string, object> productDetail) {
      ResolveDependencies(assetStoreUtils);

      id = productId;
      description =
          CleanUpHtml(productDetail.GetString("description")) ?? string.Empty;
      ...
      category = productDetail.GetDictionary("category") ?.GetString("name")
          ?? string.Empty;
      publishNotes = productDetail.GetString("publishNotes") ?? string.Empty;
      firstPublishedDate = productDetail.GetDictionary("properties")
          ?.GetString("firstPublishedDate");
      ...
      supportedVersions =
          productDetail.GetList<string>("supportedUnityVersions") ?.ToList();
    ...
    public static AssetStoreProductInfo
    ParseProductInfo(AssetStoreUtils assetStoreUtils, string productId,
                     IDictionary<string, object> productDetail) {
      if (string.IsNullOrEmpty(productId) || productDetail == null ||
          !productDetail.Any())
        return null;
      return new AssetStoreProductInfo(assetStoreUtils, productId,
                                       productDetail);
    }
    ...
    private List<PackageImage>
    GetImagesFromProductDetails(IDictionary<string, object> productDetail) {
      int imageLimit = 3;
      int imagesLoaded = 0;
      ...
      if (!string.IsNullOrEmpty(mainImageThumbnailUrl)) {
        mainImageThumbnailUrl = mainImageThumbnailUrl.Replace(
        ...
        result.Add(new PackageImage{
            ...
            thumbnailUrl = "http:" + mainImageThumbnailUrl, url = imageUrl});
        ++imagesLoaded;
      }

      var images = productDetail.GetList<IDictionary<string, object>>("images")
      ...
      foreach (var image in images) {
        ...
        thumbnailUrl =
            thumbnailUrl.Replace("//d2ujflorbtfzji.cloudfront.net/",
                                 "//assetstorev1-prd-cdn.unity3d.com/");
        ...
        result.Add(new PackageImage{type = imageType,
    ...
    private List<PackageLink>
    GetLinksFromProductDetails(IDictionary<string, object> productDetail) {
      ...
      if (publisher != null) {
        ...
        if (!string.IsNullOrEmpty(supportUrl) &&
            Uri.IsWellFormedUriString(supportUrl, UriKind.RelativeOrAbsolute))
          result.Add(GetPackageLink("Publisher Support", supportUrl));
      }
      return result;
    }

    private PackageLink GetAssetStoreLinkFromProductDetails(
        IDictionary<string, object> productDetail) {
      var slug =
          productDetail.GetString("slug") ?? productDetail.GetString("id");
      var packagePath = $"/packages/p/{slug}";

      return GetPackageLink("View in the Asset Store", packagePath);
    }

    private List<PackageSizeInfo>
    GetSizeInfoFromProductDetails(IDictionary<string, object> productDetail) {
      var result = new List<PackageSizeInfo>();
      var uploads = productDetail.GetDictionary("uploads");
      if (uploads != null) {
        foreach (var key in uploads.Keys) {
          var simpleVersion = Regex.Replace(
              key, @"(?<major>\d+)\.(?<minor>\d+).(?<patch>\d+)[abfp].+",
              "${major}.${minor}.${patch}");

          SemVersion? version;
          bool isVersionParsed =
              SemVersionParser.TryParse(simpleVersion.Trim(), out version);
          ...
          if (isVersionParsed) {
            ...
            result.Add(new PackageSizeInfo{
                ...
                assetCount = string.IsNullOrEmpty(assetCount)
                                 ? 0
                                 : ulong.Parse(assetCount),
                downloadSize = string.IsNullOrEmpty(downloadSize)
                                   ? 0
                                   : ulong.Parse(downloadSize)});
          }
        }
      }
      return result;
    ...
    private PackageLink GetPackageLink(string name, string url) {
      if (!url.StartsWith("http:", StringComparison.InvariantCulture) &&
          !url.StartsWith("https:", StringComparison.InvariantCulture))
