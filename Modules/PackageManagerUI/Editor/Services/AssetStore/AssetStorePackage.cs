...
namespace UnityEditor.PackageManager.UI {
  ...
  internal class AssetStorePackage : BasePackage {
    ...
    public override IVersionList versions =>
        string.IsNullOrEmpty(name) ? m_VersionList as IVersionList
                                   : m_UpmVersionList as IVersionList;
    ...
    public override DateTime? purchasedTime =>
        m_PurchasedTimeTicks == 0
            ? (DateTime?) null
            : new DateTime(m_PurchasedTimeTicks, DateTimeKind.Utc);
    ...
    public AssetStorePackage(AssetStoreUtils assetStoreUtils, IOProxy ioProxy,
                             string productId, UIError error) {
    ...
    public AssetStorePackage(AssetStoreUtils assetStoreUtils, IOProxy ioProxy,
                             ...
                             AssetStoreProductInfo productInfo,
                             AssetStoreLocalInfo localInfo = null) {
      ResolveDependencies(assetStoreUtils, ioProxy);
      ...
      var firstPublishedDateString =
          productInfo?.firstPublishedDate ?? string.Empty;
      m_FirstPublishedDateTicks =
          !string.IsNullOrEmpty(firstPublishedDateString)
              ? DateTime.Parse(firstPublishedDateString).Ticks
              : 0;

      m_Labels = purchaseInfo?.tags;
      m_PurchasedTimeTicks =
          !string.IsNullOrEmpty(purchaseInfo?.purchasedTime)
              ? DateTime.Parse(purchaseInfo?.purchasedTime).Ticks
              : 0;
      ...
      if (string.IsNullOrEmpty(productInfo?.id) ||
          string.IsNullOrEmpty(productInfo?.versionId)) {
        AddError(new UIError(UIErrorCode.AssetStorePackageError,
                             L10n.Tr("Invalid product details.")));
      } else if (localInfo == null) {
        m_VersionList.AddVersion(new AssetStorePackageVersion(
            assetStoreUtils, ioProxy, productInfo));
      } else {
        m_VersionList.AddVersion(new AssetStorePackageVersion(
            assetStoreUtils, ioProxy, productInfo, localInfo));
        if (localInfo.canUpdate &&
            (localInfo.versionId != productInfo.versionId ||
             localInfo.versionString != productInfo.versionString))
          m_VersionList.AddVersion(new AssetStorePackageVersion(
              assetStoreUtils, ioProxy, productInfo));
      }
    }
    ...
    public AssetStorePackage(AssetStoreUtils assetStoreUtils, IOProxy ioProxy,
      ...
      m_UpmVersionList =
          package?.versions as UpmVersionList ?? new UpmVersionList();
      if (productInfo != null) {
      ...
      if (purchaseInfo == null) {
        var errorMessage = L10n.Tr(
            "Unable to get asset purchase details because you may not have purchased this package.");
        AddError(new UIError(UIErrorCode.AssetStorePackageError, errorMessage));
      }
      if (string.IsNullOrEmpty(productInfo?.id) ||
          string.IsNullOrEmpty(productInfo?.versionId))
        AddError(
            new UIError(UIErrorCode.AssetStorePackageError,
                        L10n.Tr("Unable to retrieve asset product details.")));
      else if (string.IsNullOrEmpty(package?.name))
        AddError(
            new UIError(UIErrorCode.AssetStorePackageError,
                        L10n.Tr("Unable to retrieve asset package info.")));
    }
