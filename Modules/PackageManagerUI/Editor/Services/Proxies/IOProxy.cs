...
namespace UnityEditor.PackageManager.UI {
  internal class IOProxy {
    // Need to re-create this method since Unity's FileUtil equivalent (with
    // overwrite) is internal only. From:
    // https://stackoverflow.com/questions/58744/copy-the-entire-contents-of-a-directory-in-c-sharp
    public virtual void DirectoryCopy(string sourcePath, string destinationPath,
    ...
    public virtual void RemovePathAndMeta(string path,
                                          bool removeEmptyParent = false) {
      ...
      if (removeEmptyParent) {
        ...
        if (parent.GetDirectories().Length == 0 &&
            parent.GetFiles().Length == 0)
    ...
    public virtual void FileCopy(string sourcePath, string destinationPath,
                                 bool overwrite) {
      File.Copy(sourcePath, destinationPath, overwrite);
    }

    public virtual void FileWriteAllBytes(string destinationPath,
                                          byte[] bytes) {
      File.WriteAllBytes(destinationPath, bytes);
    }

    public virtual void FileDelete(string filePath) { File.Delete(filePath); }
  }
}
