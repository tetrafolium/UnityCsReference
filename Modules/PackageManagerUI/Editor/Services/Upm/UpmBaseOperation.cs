...
public string packageUniqueId {
	get { return m_PackageUniqueId; }
}
public string versionUniqueId {
	get { return packageId; }
}

public virtual string specialUniqueId {
...
public long lastSuccessTimestamp {
	get { return m_LastSuccessTimestamp; }
}

[SerializeField]
protected bool m_OfflineMode = false;
public bool isOfflineMode {
	get { return m_OfflineMode; }
}
...
public abstract RefreshOptions refreshOptions {
	get;
}
}

internal abstract class UpmBaseOperation<T> : UpmBaseOperation where T
	: Request {
public override event Action<IOperation, UIError> onOperationError =
	delegate {};
...
public override bool isInProgress {
	get {
		return m_Request != null && m_Request.Id != 0 && !m_Request.IsCompleted;
	}
}
...
protected void Start() {
	if (isInProgress) {
		Debug.LogError(L10n.Tr(
				       "[Package Manager Window] Unable to start the operation again while it's in progress. " +
	...
	// Usually the timestamp for an offline operation is the last success
	// timestamp of its online equivalence (to indicate the freshness of the
	// data) But in the rare case where we start an offline operation before
	// an online one, we use the start timestamp of the editor instead of 0,
	// because we consider a `0` refresh timestamp as `not initialized`/`no
	// refreshes have been done`.
	else if (m_Timestamp == 0)
		m_Timestamp =
			DateTime.Now.Ticks - (long)(EditorApplication.timeSinceStartup *
		...
		if (m_Request.Status == StatusCode.Success)
			...
			Debug.LogError(
				$"{L10n.Tr("[Package Manager
                               Window] ")} {m_Request.Error.message}");
			OnError(new UIError((UIErrorCode) m_Request.Error.errorCode,
			                    m_Request.Error.message,
			                    UIError.Attribute.IsDetailInConsole));
		} else
			Debug.LogError(string.Format(
					       L10n.Tr(
						       "[Package Manager Window] Unsupported progress state {0}."),
					       m_Request.Status));
		OnFinalize();
	}
}
...
private void OnError(UIError error) {
	try {
		...
		Debug.LogError(string.Format(
				       L10n.Tr(
					       "[Package Manager Window] An error occurred while reporting an error in an operation: {0}"),
				       exception.Message));
	}
}
...
private void OnSuccess() {
	try {
		...
		Debug.LogError(string.Format(
				       L10n.Tr(
					       "[Package Manager Window] An error occurred while completing an operation: {0}"),
				       exception.Message));
	}
}
