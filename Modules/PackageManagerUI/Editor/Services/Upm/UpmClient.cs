...
namespace UnityEditor.PackageManager.UI {
  ...
  internal class UpmClient {
    ...
    public virtual event Action<IEnumerable<IPackage>>
        onPackagesChanged = delegate{};
    public virtual event Action<string, IPackage>
        onProductPackageChanged = delegate{};

    public virtual event Action<string, IPackageVersion>
        onPackageVersionUpdated = delegate{};
    public virtual event Action<string, IPackageVersion>
        onProductPackageVersionUpdated = delegate{};
    public virtual event Action<string, UIError>
        onProductPackageFetchError = delegate{};

    [SerializeField]
    private UpmSearchOperation m_SearchOperation;
    private UpmSearchOperation searchOperation =>
        m_SearchOperation ??(m_SearchOperation = new UpmSearchOperation());
    [SerializeField]
    private UpmSearchOperation m_SearchOfflineOperation;
    private UpmSearchOperation searchOfflineOperation =>
        m_SearchOfflineOperation ??(m_SearchOfflineOperation =
                                       new UpmSearchOperation());
    [SerializeField]
    private UpmListOperation m_ListOperation;
    private UpmListOperation listOperation =>
        m_ListOperation ??(m_ListOperation = new UpmListOperation());
    [SerializeField]
    private UpmListOperation m_ListOfflineOperation;
    private UpmListOperation listOfflineOperation => m_ListOfflineOperation ??(
        m_ListOfflineOperation = new UpmListOperation());

    [SerializeField]
    private UpmAddOperation m_AddOperation;
    private UpmAddOperation addOperation =>
        m_AddOperation ??(m_AddOperation = new UpmAddOperation());
    [SerializeField]
    private UpmRemoveOperation m_RemoveOperation;
    private UpmRemoveOperation removeOperation =>
        m_RemoveOperation ??(m_RemoveOperation = new UpmRemoveOperation());
    [SerializeField]
    private UpmEmbedOperation m_EmbedOperation;
    private UpmEmbedOperation embedOperation =>
        m_EmbedOperation ??(m_EmbedOperation = new UpmEmbedOperation());

    private readonly Dictionary<string, UpmBaseOperation>
        m_ExtraFetchOperations = new Dictionary<string, UpmBaseOperation>();
    ...
    public void
    ResolveDependencies(PackageManagerPrefs packageManagerPrefs,
                        UpmCache upmCache, IOProxy IOProxy,
    ...
    public virtual bool isAddRemoveOrEmbedInProgress {
      get {
        return addOperation.isInProgress || removeOperation.isInProgress ||
               embedOperation.isInProgress;
      }
    }

    public virtual bool IsEmbedInProgress(string packageName) {
      return embedOperation.isInProgress &&
             embedOperation.packageName == packageName;
    }

    public virtual bool IsRemoveInProgress(string packageName) {
      return removeOperation.isInProgress &&
             removeOperation.packageName == packageName;
    }
    ...
    private void SetupAddOperation() {
      ...
      addOperation.onOperationError += (op, error) => {
        var packageId = string.IsNullOrEmpty(addOperation.packageId)
                            ? addOperation.specialUniqueId
                            : addOperation.packageId;
        Debug.LogError(string.Format(
            L10n.Tr("[Package Manager Window] Error adding package: {0}."),
            packageId));
      };
    ...
    public virtual void AddByPath(string path) {
      ...
      if (path.StartsWith(projectPath)) {
        ...
        if (relativePathToProjectRoot.StartsWith(
                packageFolderPrefix,
                StringComparison.InvariantCultureIgnoreCase))
          path =
              relativePathToProjectRoot.Substring(packageFolderPrefix.Length);
        else
    ...
    public virtual void List(bool offlineMode = false) {
      ...
      if (operation.isInProgress)
        operation.Cancel();
      var errorMessage =
          offlineMode ? L10n.Tr("Error fetching package list offline.")
          : L10n.Tr("Error fetching package list.");
      if (offlineMode)
        operation.ListOffline(listOperation.lastSuccessTimestamp);
      else
        operation.List();
      operation.onProcessResult += request =>
          OnProcessListResult(request, offlineMode);
      operation.onOperationError += (op, error) => Debug.LogError(
          $"{L10n.Tr("[Package Manager Window] ")} {errorMessage}");
      onListOperation(operation);
    }

    private void OnProcessListResult(ListRequest request, bool offlineMode) {
      // skip operation when the result from the online operation is more
      // up-to-date.
      if (offlineMode &&
          listOfflineOperation.timestamp < listOperation.lastSuccessTimestamp)
        return;
    ...
    public virtual void EmbedByName(string packageName) {
      ...
      embedOperation.onOperationError += (op, error) =>
          Debug.LogError(string.Format(
              L10n.Tr("[Package Manager Window] Error embedding package: {0}."),
              embedOperation.packageName));
    ...
    private void SetupRemoveOperation() {
      ...
      removeOperation.onOperationError += (op, error) =>
          Debug.LogError(string.Format(
              L10n.Tr("[Package Manager Window] Error removing package: {0}."),
              removeOperation.packageName));
      onRemoveOperation(removeOperation);
    }

    private void OnProcessRemoveResult(RemoveRequest request) {
      var installedPackage =
          m_UpmCache.GetInstalledPackageInfo(request.PackageIdOrName);
    ...
    public virtual void SearchAll(bool offlineMode = false) {
      ...
      if (operation.isInProgress)
        operation.Cancel();
      var errorMessage =
          offlineMode ? L10n.Tr("Error searching for packages offline.")
          : L10n.Tr("Error searching for packages.");
      if (offlineMode)
        operation.SearchAllOffline(searchOperation.lastSuccessTimestamp);
      else
        operation.SearchAll();
      operation.onProcessResult += request =>
          OnProcessSearchAllResult(request, offlineMode);
      operation.onOperationError += (op, error) => Debug.LogError(
          $"{L10n.Tr("[Package Manager Window] ")} {errorMessage}");
      onSearchAllOperation(operation);
    }

    private void OnProcessSearchAllResult(SearchRequest request,
                                          bool offlineMode) {
      // skip operation when the result from the online operation is more
      // up-to-date.
      if (offlineMode && searchOfflineOperation.timestamp <
                             searchOperation.lastSuccessTimestamp)
        return;
    ...
    private void ExtraFetchInternal(string packageIdOrName,
      ...
      operation.onProcessResult += (requst) =>
          OnProcessExtraFetchResult(requst, productId);
      operation.onOperationError += (op, error) =>
          OnProcessExtraFetchError(error, productId);
      operation.onOperationFinalized += (op) =>
          OnExtraFetchFinalized(packageIdOrName);
    ...
    private void OnProcessExtraFetchResult(SearchRequest request,
      ...
      if (!string.IsNullOrEmpty(productId)) {
        ...
        // remove the created package that's created before asset store info was
        // fetched such that there won't be two entries of the same package
        if (oldInfo == null && m_UpmCache.IsPackageInstalled(packageInfo.name))
          onPackagesChanged(
              new[]{CreateUpmPackage(null, null, packageInfo.name)});

        m_UpmCache.SetProductPackageInfo(productId, packageInfo);
      } else {
        m_UpmCache.AddExtraPackageInfo(packageInfo);

        // only trigger the call when the package is not installed, as installed
        // version always have the most up-to-date package info
        var installedPackageInfo =
            m_UpmCache.GetInstalledPackageInfo(packageInfo.name);
        if (installedPackageInfo?.packageId != packageInfo.packageId) {
          productId = m_UpmCache.GetProductId(packageInfo.name);
          if (string.IsNullOrEmpty(productId))
            onPackageVersionUpdated?.Invoke(
                packageInfo.name, new UpmPackageVersion(packageInfo, false));
          else
            onProductPackageVersionUpdated?.Invoke(
                productId, new UpmPackageVersion(packageInfo, false));
        }
      }
    }

    private void OnProcessExtraFetchError(UIError error,
                                          string productId = null) {
      if (!string.IsNullOrEmpty(productId))
    ...
    private void OnPackageInfosUpdated(IEnumerable<PackageInfo> packageInfos) {
      ...
      foreach (var p in packageInfos) {
        ...
        if (string.IsNullOrEmpty(productId))
          upmPackages.Add(CreateUpmPackage(
              m_UpmCache.GetSearchPackageInfo(p.name), installedInfo, p.name));
        else
          productPackages.Add(CreateUpmPackage(
              m_UpmCache.GetProductPackageInfo(p.name), installedInfo, p.name));
      }
      ...
      foreach (var package in productPackages)
        onProductPackageChanged?.Invoke(m_UpmCache.GetProductId(package.name),
                                        package);
    }
    ...
    private void OnShowPreviewPackagesesChanged(bool showPreview) {
      ...
      foreach (var installedInfo in m_UpmCache.installedPackageInfos) {
        ...
        if (string.IsNullOrEmpty(productId)) {
          var package = CreateUpmPackage(
              m_UpmCache.GetSearchPackageInfo(installedInfo.name),
              installedInfo);
          if (HasHidablePreviewVersions(package))
            updatedUpmPackages.Add(package);
        } else {
          var package = CreateUpmPackage(
              m_UpmCache.GetProductPackageInfo(installedInfo.name),
              installedInfo);
          if (HasHidablePreviewVersions(package))
      ...
      foreach (var searchInfo in m_UpmCache.searchPackageInfos.Where(
                   p => !m_UpmCache.IsPackageInstalled(p.name))) {
        var package = CreateUpmPackage(searchInfo, null);
        if (HasHidablePreviewVersions(package))
          updatedUpmPackages.Add(package);
      }

      foreach (var productPackageInfo in m_UpmCache.productPackageInfos.Where(
                   p => !m_UpmCache.IsPackageInstalled(p.name))) {
      ...
      foreach (var package in updatedUpmPackages.Concat(
    ...
    private UpmPackage CreateUpmPackage(PackageInfo searchInfo,
      ...
      if (searchInfo == null)
        ...
        if (installedInfo != null)
          result.AddInstalledVersion(
              new UpmPackageVersion(installedInfo, true));
      }
      return result;
    }

    private void UpdateExtraPackageInfos(string packageName,
      ...
      var extraVersions = m_UpmCache.GetExtraPackageInfos(packageName);
        ...
        // if the primary version is not fully fetched, trigger an extra fetch
        // automatically right away to get results early since the primary
        // version's display name is used in the package list
        var primaryVersion = versions.primary;
        if (!primaryVersion.isFullyFetched)
          ExtraFetch(primaryVersion.uniqueId);
    }

    // check if this package have preview packages that's `hidable` (will be
    // filtered out if `show preview` is not selected). if the installed version
    // is preview, then we we always show other preview versions if no installed
    // version or installed version is not preview, we hide the preview versions
    // according to the `show previews` toggle
    private static bool HasHidablePreviewVersions(IPackage package) {
      var previewInstalled =
          (!package.versions.installed?.HasTag(PackageTag.Release)) ?? false;
      if (previewInstalled)
        return false;
    ...
    private static void RemovePreviewVersions(UpmPackage package) {
      package.UpdateVersions(
          package.versions.Where(v => v.HasTag(PackageTag.Release))
              .Cast<UpmPackageVersion>());
    }
    ...
    public void OnEnable() {
      m_SettingsProxy.onEnablePreviewPackagesChanged +=
          OnShowPreviewPackagesesChanged;
    ...
    public virtual void ClearProductCache() { m_UpmCache.ClearProductCache(); }

    public void Resolve() { Client.Resolve(); }
  }
}
