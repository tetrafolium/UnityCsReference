...
namespace UnityEditor.PackageManager.UI {
  ...
  internal class UpmPackageVersion : BasePackageVersion {
    ...
    UIError entitlementsError =>
        !entitlements.isAllowed &&
        isInstalled ? new UIError(
            UIErrorCode.UpmError,
            L10n.Tr("You do not have entitlements for this package."))
        : null;
    public override IEnumerable<UIError> errors =>
        m_PackageInfo.errors
            .Select(e => new UIError((UIErrorCode) e.errorCode, e.message))
            .Concat(entitlementsError !=
                        null ? new List<UIError>{entitlementsError}
                    : new List<UIError>());
    public override bool isDirectDependency =>
        isFullyFetched && m_PackageInfo.isDirectDependency;
    ...
    public string sourcePath {
      get {
        if (m_PackageInfo.source == PackageSource.Local ||
            m_PackageInfo.source == PackageSource.LocalTarball) {
          return m_PackageInfo.packageId.Substring(
              m_PackageInfo.packageId.IndexOf("@file:") + 6);
        } else
          return null;
      }
    }
    ...
    public bool installedFromPath =>
        HasTag(PackageTag.Local | PackageTag.InDevelopment | PackageTag.Git);

    public override bool isAvailableOnDisk =>
        m_IsFullyFetched && !string.IsNullOrEmpty(m_PackageInfo.resolvedPath);

    public string shortVersionId =>
        FormatPackageId(name, version?.ShortVersion());
    ...
    public UpmPackageVersion(PackageInfo packageInfo, bool isInstalled,
                             SemVersion? version, string displayName) {
      m_Version = version;
    ...
    internal void UpdatePackageInfo(PackageInfo newPackageInfo) {
      ...
      // For core packages, or packages that are bundled with Unity without
      // being published, use Unity's build date
      m_PublishedDateTicks = 0;
      if (HasTag(PackageTag.Bundled) && m_PackageInfo.datePublished == null)
        m_PublishedDateTicks = new DateTime(1970, 1, 1).Ticks +
                               InternalEditorUtility.GetUnityVersionDate() *
                                   TimeSpan.TicksPerSecond;

      m_Author = string.IsNullOrEmpty(m_PackageInfo.author.name) &&
                         m_PackageInfo.name.StartsWith(k_UnityPrefix)
                     ? "Unity Technologies Inc."
                     : m_PackageInfo.author.name;
    ...
    private void RefreshTags() {
      // in the case of git/local packages, we always assume that the
      // non-installed versions are from the registry
      var source = m_PackageInfo.source == PackageSource.BuiltIn ||
                   m_IsInstalled ? m_PackageInfo.source : PackageSource.Registry;
      switch (source) {
      ...
      if (isInstalled && isDirectDependency && !installedFromPath &&
          !HasTag(PackageTag.BuiltIn))
        m_Tag |= PackageTag.Embeddable;

      if (m_Version?.IsRelease() == true) {
        m_Tag |= PackageTag.Release;
        SemVersion? verified;
        bool isVerifiedParsed = SemVersionParser.TryParse(
            m_PackageInfo.versions.verified, out verified);
        ...
        if ((version?.Major == 0 &&
            ...
            PackageTag.Preview.ToString().Equals(
    ...
    public static string ExtractDisplayName(string packageName) {
      if (packageName.StartsWith(k_UnityPrefix)) {
        var displayName =
            packageName.Substring(k_UnityPrefix.Length).Replace("modules.", "");
