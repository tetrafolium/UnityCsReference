...
namespace UnityEditor.PackageManager.UI {
  ...
  internal class UpmCache : ISerializationCallbackReceiver {
    private Dictionary<string, PackageInfo> m_SearchPackageInfos =
        new Dictionary<string, PackageInfo>();
    private Dictionary<string, PackageInfo> m_InstalledPackageInfos =
        new Dictionary<string, PackageInfo>();
    private Dictionary<string, PackageInfo> m_ProductPackageInfos =
        new Dictionary<string, PackageInfo>();

    private Dictionary<string, Dictionary<string, PackageInfo>>
        m_ExtraPackageInfo =
            new Dictionary<string, Dictionary<string, PackageInfo>>();

    // the mapping between package name (key) to asset store product id (value)
    private Dictionary<string, string> m_ProductIdMap =
    ...
    public virtual IEnumerable<PackageInfo>
        searchPackageInfos => m_SearchPackageInfos.Values;
    public virtual IEnumerable<PackageInfo>
        installedPackageInfos => m_InstalledPackageInfos.Values;
    public virtual IEnumerable<PackageInfo>
        productPackageInfos => m_ProductPackageInfos.Values;
    ...
    private static List<PackageInfo>
    FindUpdatedPackageInfos(Dictionary<string, PackageInfo> oldInfos,
                            Dictionary<string, PackageInfo> newInfos) {
      return newInfos.Values
          .Where(p => !oldInfos.TryGetValue(p.name, out var info) ||
                      IsDifferent(info, p))
          .Concat(oldInfos.Values.Where(p => !newInfos.ContainsKey(p.name)))
          .ToList();
    }

    // For BuiltIn and Registry packages, we want to only compare a subset of
    // PackageInfo attributes, as most attributes never change if their
    // PackageId is the same. For other types of packages, always consider them
    // different
    private static bool IsDifferent(PackageInfo p1, PackageInfo p2) {
      if (p1.packageId != p2.packageId ||
          ...
          p1.version != p2.version || p1.source != p2.source ||
          p1.resolvedPath != p2.resolvedPath || p1.status != p2.status ||
          ...
          p1.versions.compatible.Length != p2.versions.compatible.Length ||
          !p1.versions.compatible.SequenceEqual(p2.versions.compatible) ||
          p1.versions.all.Length != p2.versions.all.Length ||
          !p1.versions.all.SequenceEqual(p2.versions.all) ||
          p1.errors.Length != p2.errors.Length ||
          !p1.errors.SequenceEqual(p2.errors) ||
          p1.dependencies.Length != p2.dependencies.Length ||
          !p1.dependencies.SequenceEqual(p2.dependencies) ||
          p1.resolvedDependencies.Length != p2.resolvedDependencies.Length ||
          !p1.resolvedDependencies.SequenceEqual(p2.resolvedDependencies))
        return true;

      if (p1.source == PackageSource.BuiltIn ||
          p1.source == PackageSource.Registry)
        return false;

      if (p1.source == PackageSource.Git)
        return p1.git.hash != p2.git?.hash ||
               p1.git.revision != p2.git?.revision;

      return true;
    }

    public void OnBeforeSerialize() {
      m_SerializedInstalledPackageInfos =
          m_InstalledPackageInfos.Values.ToArray();
      ...
      m_SerializedExtraPackageInfos =
          m_ExtraPackageInfo.Values.SelectMany(p => p.Values).ToArray();
    ...
    public void OnAfterDeserialize() {
      ...
      m_ProductPackageInfos =
          m_SerializedProductPackageInfos.ToDictionary(p => p.name, p => p);

      foreach (var p in m_SerializedExtraPackageInfos)
        AddExtraPackageInfo(p);

      for (var i = 0; i < m_SerializedProductIdMapKeys.Length; i++)
        m_ProductIdMap[m_SerializedProductIdMapKeys[i]] =
            m_SerializedProductIdMapValues[i];
    }
    ...
    public virtual Dictionary<string, PackageInfo> GetExtraPackageInfos(
        string packageName) => m_ExtraPackageInfo.Get(packageName);
    ...
    public virtual bool IsPackageInstalled(
        string packageName) => m_InstalledPackageInfos.ContainsKey(packageName);

    public virtual PackageInfo GetInstalledPackageInfo(
        string packageName) => m_InstalledPackageInfos.Get(packageName);
    ...
    public virtual void
    SetInstalledPackageInfos(IEnumerable<PackageInfo> packageInfos) {
      ...
      var updatedInfos =
          FindUpdatedPackageInfos(oldPackageInfos, newPackageInfos);

      if (updatedInfos.Any())
        onPackageInfosUpdated?.Invoke(updatedInfos);
    }

    public virtual PackageInfo GetSearchPackageInfo(
        string packageName) => m_SearchPackageInfos.Get(packageName);
    ...
    public virtual PackageInfo GetProductPackageInfo(
        string packageName) => m_ProductPackageInfos.Get(packageName);
    public virtual void SetProductPackageInfo(string productId,
                                              PackageInfo info) {
    ...
    GetProductId(string packageName) => m_ProductIdMap.Get(packageName);
    ...
    private static bool IsPreviewInstalled(PackageInfo packageInfo) {
      ...
      return packageInfo.isDirectDependency &&
             packageInfo.source == PackageSource.Registry &&
             SemVersionParser.TryParse(packageInfo.version,
                                       out packageInfoVersion) &&
             !((SemVersion) packageInfoVersion).IsRelease();
    }
