...
namespace UnityEditor.PackageManager.UI {
  ...
  internal sealed class ServicesContainer
      : ScriptableSingleton<ServicesContainer>,
    ...
    public ServicesContainer() {
      // In the constructor we only need to worry about creating a brand new
      // instance. In the case of assembly reload, a deserialize step will
      // automatically happen after the constructor to restore all the
      // serializable states/services and we don't need to worry about that
      ...
      // Since dictionaries doesn't survive through serialization, we always
      // re-create the default registration
    ...
    private void ResolveDependencies() {
      ...
      m_AssetStoreCache.ResolveDependencies(m_ApplicationProxy,
                                            m_AssetStoreUtils,
                                            m_HttpClientFactory, m_IOProxy);
      m_AssetStoreClient.ResolveDependencies(
          m_UnityConnectProxy, m_AssetStoreCache, m_AssetStoreUtils,
          m_AssetStoreRestAPI, m_UpmClient, m_IOProxy);
      m_AssetStoreOAuth.ResolveDependencies(
          m_UnityConnectProxy, m_UnityOAuthProxy, m_AssetStoreUtils,
          m_HttpClientFactory);
      m_AssetStoreUtils.ResolveDependencies(m_UnityConnectProxy);
      m_AssetStoreRestAPI.ResolveDependencies(
          m_UnityConnectProxy, m_AssetStoreOAuth, m_AssetStoreCache,
          m_HttpClientFactory);
      m_AssetStoreDownloadManager.ResolveDependencies(
          m_ApplicationProxy, m_HttpClientFactory, m_UnityConnectProxy,
          m_AssetStoreCache, m_AssetStoreUtils, m_AssetStoreRestAPI);

      m_UpmCache.ResolveDependencies(m_PackageManagerPrefs);
      m_UpmClient.ResolveDependencies(m_PackageManagerPrefs, m_UpmCache,
                                      m_IOProxy, m_SettingsProxy);

      m_PackageFiltering.ResolveDependencies(m_UnityConnectProxy,
                                             m_PackageManagerPrefs);

      m_PackageDatabase.ResolveDependencies(
          m_UnityConnectProxy, m_AssetDatabaseProxy, m_AssetStoreUtils,
          m_AssetStoreClient, m_AssetStoreDownloadManager, m_UpmClient,
          m_IOProxy);
      m_PageManager.ResolveDependencies(
          m_ApplicationProxy, m_SelectionProxy, m_UnityConnectProxy,
          m_PackageFiltering, m_PackageManagerPrefs, m_UpmClient,
          m_AssetStoreClient, m_PackageDatabase, m_SettingsProxy);
    ...
    void OnEnable() {
      ...
      // Some services has dependencies that requires some initialization in
      // `OnEnable`.
    ...
    public T Resolve<T>() where T : class {
      object result;
      return m_RegisteredObjects.TryGetValue(typeof(T), out result)
                 ? result as T
                 : null;
    }
    ...
    public void OnAfterDeserialize() { ResolveDependencies(); }
  }
}
