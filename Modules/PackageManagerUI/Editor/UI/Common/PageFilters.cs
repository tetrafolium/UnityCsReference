...
namespace UnityEditor.PackageManager.UI {
  ...
  internal class PageFilters : IEquatable<PageFilters> {
    ...
    public bool isFilterSet => !string.IsNullOrEmpty(searchText) ||
                               statuses.Any() || categories.Any() ||
                               labels.Any();
    public bool isOrderSet => !string.IsNullOrEmpty(orderBy) || isReverseOrder;

    public PageFilters Clone() { return (PageFilters) MemberwiseClone(); }

    public bool Equals(PageFilters other) {
      return other != null && searchText == other.searchText &&
             orderBy == other.orderBy &&
             isReverseOrder == other.isReverseOrder &&
             statuses.Count == other.statuses.Count &&
             statuses.SequenceEqual(other.statuses) &&
             categories.Count == other.categories.Count &&
    ...
    public override string ToString() {
      ...
      if (statuses.Any())
        stringBuilder.Append($"statuses=[{string.Join(",
                             ", statuses.ToArray())}],");
      if (categories.Any())
        stringBuilder.Append($"categories=[{string.Join(",
                             ", categories.ToArray())}],");
      if (labels.Any())
        stringBuilder.Append($"labels=[{string.Join(",
                             ", labels.ToArray())}],");
