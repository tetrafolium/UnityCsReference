  ...
  internal static class UIUtils {
    ...
    public static void SetElementDisplayNonEmpty(TextElement element) {
      ...
      element.style.visibility =
          nonEmpty ? Visibility.Visible : Visibility.Hidden;
    }
    ...
    public static VisualElement FindNextSibling(VisualElement element,
                                                bool reverseOrder) {
      if (element == null)
        return null;
    ...
    public static void ScrollIfNeeded(ScrollView container,
    ...
    GetParentsOfType<T>(VisualElement element) where T : VisualElement {
    ...
    public static T GetParentOfType<T>(VisualElement element) where T
        : VisualElement {
    ...
    public static void
    ShowTextTooltipOnSizeChange<T>(this T element, int deltaWidth = 0) where T
        : TextElement {
      InternalShowTextTooltipOnSizeChange(element, deltaWidth);
    }

    private static void InternalShowTextTooltipOnSizeChange(TextElement element,
                                                            int deltaWidth) {
      element.RegisterCallback<GeometryChangedEvent>(evt => {
        ...
        var size = target.MeasureTextSize(
            target.text, float.MaxValue, VisualElement.MeasureMode.AtMost,
            evt.newRect.height, VisualElement.MeasureMode.Undefined);
      ...
      element.RegisterValueChangedCallback(evt => {
        ...
        var size = target.MeasureTextSize(
            evt.newValue, float.MaxValue, VisualElement.MeasureMode.AtMost,
            target.contentRect.height, VisualElement.MeasureMode.Undefined);
    ...
    public static Action<Label> TextTooltipOnSizeChange =
        ActionShowTextTooltipOnSizeChange;
  }
}
