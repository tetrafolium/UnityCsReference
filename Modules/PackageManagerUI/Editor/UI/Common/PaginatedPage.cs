...
namespace UnityEditor.PackageManager.UI {
  ...
  internal class PaginatedPage : BasePage {
    [SerializeField]
    private PaginatedVisualStateList m_VisualStateList =
        new PaginatedVisualStateList();
    ...
    public PaginatedPage(PackageDatabase packageDatabase,
                         AssetStoreClient assetStoreClient,
                         PackageFiltering packageFiltering,
                         PackageManagerPrefs packageManagerPrefs,
                         PackageFilterTab tab, PageCapability capability)
        : base(packageDatabase, tab, capability) {
      ResolveDependencies(packageDatabase, assetStoreClient, packageFiltering,
                          packageManagerPrefs);
    }
    ...
    public override void UpdateFilters(PageFilters filters) {
      ...
      var queryArgs = BuildQueryFromFilter(
          0, m_PackageManagerPrefs.numItemsPerPage ?? PageManager
                 .k_DefaultPageSize);
    ...
    // in the case of paginated pages, we don't need to change the list itself
    // (it's predefined and trumps other custom filtering) hence we don't need
    // to trigger any list change event.
    public override void OnPackagesChanged(IEnumerable<IPackage> added,
    ...
    private void RefreshVisualStates() {
      ...
      foreach (var state in m_VisualStateList ??
        ...
        if (package != null) {
          var visible =
              tab == PackageFilterTab.AssetStore ? true
              : m_PackageFiltering.FilterByCurrentSearchText(package);
    ...
    private PurchasesQueryArgs BuildQueryFromFilter(int startIndex, int limit) {
      return new PurchasesQueryArgs{startIndex = startIndex,
                                    limit = limit,
    ...
    public override void LoadMore(int numberOfPackages) {
      ...
      var queryArgs =
          BuildQueryFromFilter((int) numCurrentItems, numberOfPackages);
      m_AssetStoreClient.ListPurchases(queryArgs, false);
    }

    public override void Load(IPackage package,
      ...
      SetSelected(package.uniqueId,
                  version?.uniqueId ?? package.versions.primary?.uniqueId);
    }
    ...
    public void OnProductFetched(long productId) {
      ...
      if (m_PackageFiltering.currentFilterTab == PackageFilterTab.AssetStore) {
        ...
        TriggerOnListUpdate(new[]{package}, Enumerable.Empty<IPackage>(),
                            false);
    ...
    public void OnProductListFetched(AssetStorePurchases purchases,
      ...
      if (purchases.startIndex > 0 && numTotalItems != purchases.total) {
        // if a new page has arrived but the total has changed or the searchText
        // has changed, do a re-fetch
        var queryArgs = BuildQueryFromFilter(
            (int) numCurrentItems, purchases.startIndex + purchases.list.Count);
        m_AssetStoreClient.ListPurchases(queryArgs);
        return;
      }

      var oldPackageIds =
          new HashSet<string>(m_VisualStateList.Select(v => v.packageUniqueId));
      var newPackageIds =
          purchases.productIds.Select(id => id.ToString()).ToList();
      if (purchases.startIndex == 0) {
        // override the result if the new list starts from index 0 (meaning it's
        // a refresh)
        ...
        // if the content is neither starting from zero or next page, we simply
        // discard it
        return;
      }
      ...
      // only try to rebuild the list immediately if we are already on the
      // `AssetStore` tab. if not we'll just wait for tab switch which will
      // trigger the rebuild as well
      if (m_PackageFiltering.currentFilterTab == PackageFilterTab.AssetStore) {
        ...
        var addedPackages =
            added?.Select(id => m_PackageDatabase.GetPackage(id));
        var removedPackages =
            removed?.Select(id => m_PackageDatabase.GetPackage(id));
