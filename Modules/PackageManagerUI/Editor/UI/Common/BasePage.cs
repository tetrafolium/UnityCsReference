...
public event Action<IEnumerable<VisualState> > onVisualStateChange =
	delegate {};
public event Action<IPage, IEnumerable<IPackage>, IEnumerable<IPackage>,
                    bool> onListUpdate = delegate {};
...
public abstract long numTotalItems {
	get;
}

public abstract long numCurrentItems {
	get;
}

public abstract IEnumerable<VisualState> visualStates {
	get;
}
...
protected BasePage(PackageDatabase packageDatabase, PackageFilterTab tab,
...
public abstract void OnPackagesChanged(IEnumerable<IPackage> added,
...
protected void TriggerOnListUpdate(IEnumerable<IPackage> addOrUpdateList,
                                   IEnumerable<IPackage> removeList,
                                   bool reorder) {
...
public void GetSelectedPackageAndVersion(out IPackage package,
                                         out IPackageVersion version) {
	var selected = GetVisualState(m_SelectedUniqueIds.FirstOrDefault());
	m_PackageDatabase.GetPackageAndVersion(selected?.packageUniqueId,
...
public void SetSelected(string packageUniqueId, string versionUniqueId) {
	...
	if (oldPackageUniqueId == packageUniqueId &&
	    oldSelection?.selectedVersionId == versionUniqueId)
		return;
	...
	if (!string.IsNullOrEmpty(packageUniqueId) &&
	    !string.IsNullOrEmpty(versionUniqueId)) {
	...
	TriggerOnVisualStateChange(new[] {GetVisualState(oldPackageUniqueId),
	                                  GetVisualState(packageUniqueId)}
	                           .Where(s => s != null));
}
