...
namespace UnityEditor.PackageManager.UI {
  [Serializable]
  internal class VisualStateList : ISerializationCallbackReceiver,
                                   IEnumerable<VisualState> {
    [SerializeField]
    protected List<VisualState> m_OrderedVisualStates = new List<VisualState>();
    public virtual IEnumerable<VisualState>
        orderedList => m_OrderedVisualStates;

    public virtual long numItems => m_OrderedVisualStates.Count;
    public virtual long numTotalItems => m_OrderedVisualStates.Count;

    // a reverse look up table such that we can find an visual state easily
    // through package unique id
    protected Dictionary<string, int> m_UniqueIdToIndexLookup =
        new Dictionary<string, int>();

    public void OnBeforeSerialize() {}

    public void OnAfterDeserialize() { SetupLookupTable(); }
    ...
    public virtual VisualState GetVisualState(string packageUniqueId) {
      ...
      if (!string.IsNullOrEmpty(packageUniqueId) &&
          m_UniqueIdToIndexLookup.TryGetValue(packageUniqueId, out index))
    ...
    public virtual bool Contains(string packageUniqueId) {
      return !string.IsNullOrEmpty(packageUniqueId) &&
             m_UniqueIdToIndexLookup.ContainsKey(packageUniqueId);
    }
