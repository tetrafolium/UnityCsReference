...
public SimplePage(PackageDatabase packageDatabase,
                  PackageFiltering packageFiltering, PackageFilterTab tab,
...
private void RebuildOrderedVisualStates() {
	...
	if (m_Filters.orderBy == "name")
		...
		orderedPackages = packages.OrderBy(
			p => p.versions.primary?.publishedDate ?? new DateTime(1, 1, 1));
	else // displayName
		orderedPackages =
			packages.OrderBy(p => p.versions.primary?.displayName ?? p.name);

	var orderedPackageIds =
		m_Filters?.isReverseOrder ?? false
...
public override void SetExpanded(string packageUniqueId, bool value) {
	if (m_VisualStateList.SetExpanded(packageUniqueId, value))
		TriggerOnVisualStateChange(
			new[] {m_VisualStateList.GetVisualState(packageUniqueId)});
}
...
public override void LoadMore(int numberOfPackages) {
	// do nothing, as for a single page we have a complete/known list and
