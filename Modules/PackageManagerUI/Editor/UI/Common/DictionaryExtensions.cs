...
public static T Get<T>(this IDictionary<string, object> dict,
...
public static T Get<T>(this IDictionary<string, object> dict, string key,
                       T fallbackValue = default(T)) where T : struct {
	object result;
	return dict.TryGetValue(key, out result) ? (T) result : fallbackValue;
}

public static T Get<T>(this IDictionary<string, T> dict, string key) where T
: class {
	T result;
...
GetDictionary(this IDictionary<string, object> dict, string key) {
	return Get<IDictionary<string, object> >(dict, key);
}

public static IEnumerable<T>
GetList<T>(this IDictionary<string, object> dict, string key) {
	return Get<IList<object> >(dict, key)?.OfType<T>();
}

public static string GetString(this IDictionary<string, object> dict,
                               string key) {
