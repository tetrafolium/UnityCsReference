...
namespace UnityEditor.PackageManager.UI {
  ...
  internal class PageManager : ISerializationCallbackReceiver {
    ...
    private static readonly RefreshOptions[] k_RefreshOptionsByTab = {
        RefreshOptions.UpmList |
            RefreshOptions.UpmSearch, // PackageFilterTab.All
        RefreshOptions.UpmList,       // PackageFilterTab.InProject
        RefreshOptions.UpmListOffline |
            RefreshOptions.UpmSearchOffline, // PackageFilterTab.BuiltIn
        RefreshOptions.Purchased             // PackageFilterTab.AssetStore
    };

    public virtual event Action<IPackageVersion>
        onSelectionChanged = delegate{};
    public virtual event Action<IEnumerable<VisualState>>
        onVisualStateChange = delegate{};
    public virtual event
        Action<IPage, IEnumerable<IPackage>, IEnumerable<IPackage>, bool>
            onListUpdate = delegate{};
    ...
    private Dictionary<RefreshOptions, long> m_RefreshTimestamps =
        new Dictionary<RefreshOptions, long>();
    private Dictionary<RefreshOptions, UIError> m_RefreshErrors =
        new Dictionary<RefreshOptions, UIError>();
    ...
    [SerializeField]
    private RefreshOptions[] m_SerializedRefreshTimestampsKeys =
        new RefreshOptions[0];

    [SerializeField]
    private long[] m_SerializedRefreshTimestampsValues = new long[0];

    [SerializeField]
    private RefreshOptions[] m_SerializedRefreshErrorsKeys =
        new RefreshOptions[0];

    [SerializeField]
    private UIError[] m_SerializedRefreshErrorsValues = new UIError[0];

    private Dictionary<PackageFilterTab, IPage> m_Pages =
        new Dictionary<PackageFilterTab, IPage>();
    ...
    [NonSerialized]
    private Dictionary<string, PackageSelectionObject>
        m_PackageSelectionObjects =
            new Dictionary<string, PackageSelectionObject>();
    ...
    public void ResolveDependencies(
        ApplicationProxy application, SelectionProxy selection,
        UnityConnectProxy unityConnect, PackageFiltering packageFiltering,
        PackageManagerPrefs packageManagerPrefs, UpmClient upmClient,
        AssetStoreClient assetStoreClient, PackageDatabase packageDatabase,
      ...
      foreach (var page in m_SerializedPaginatedPage)
        page.ResolveDependencies(packageDatabase, assetStoreClient,
                                 packageFiltering, packageManagerPrefs);
    }

    public void OnBeforeSerialize() {
      m_SerializedSimplePages = m_Pages.Values.Select(p => p as SimplePage)
                                    .Where(p => p != null)
                                    .ToArray();
      m_SerializedPaginatedPage = m_Pages.Values.Select(p => p as PaginatedPage)
                                      .Where(p => p != null)
                                      .ToArray();

      m_SerializedRefreshTimestampsKeys = m_RefreshTimestamps.Keys.ToArray();
      m_SerializedRefreshTimestampsValues =
          m_RefreshTimestamps.Values.ToArray();
      m_SerializedRefreshErrorsKeys = m_RefreshErrors.Keys.ToArray();
      m_SerializedRefreshErrorsValues = m_RefreshErrors.Values.ToArray();

      m_SerializedPackageSelectionInstanceIds =
          m_PackageSelectionObjects.Select(kp => kp.Value.GetInstanceID())
              .ToArray();
    }

    public void OnAfterDeserialize() {
      foreach (var page in m_SerializedSimplePages.Cast<IPage>().Concat(
                   m_SerializedPaginatedPage)) {
        m_Pages[page.tab] = page;
        RegisterPageEvents(page);
      }

      for (var i = 0; i < m_SerializedRefreshTimestampsKeys.Length; i++)
        m_RefreshTimestamps[m_SerializedRefreshTimestampsKeys[i]] =
            m_SerializedRefreshTimestampsValues[i];

      for (var i = 0; i < m_SerializedRefreshErrorsKeys.Length; i++)
        m_RefreshErrors[m_SerializedRefreshErrorsKeys[i]] =
            m_SerializedRefreshErrorsValues[i];
    }

    public virtual PackageSelectionObject
    GetPackageSelectionObject(IPackage package, IPackageVersion version = null,
      ...
      var packageSelectionObject =
          m_PackageSelectionObjects.Get(version.uniqueId);
      if (packageSelectionObject == null && createIfNotFound) {
        packageSelectionObject =
            ScriptableObject.CreateInstance<PackageSelectionObject>();
    ...
    private IPage GetPageFromTab(PackageFilterTab? tab = null) {
      ...
      IPage page;
      return m_Pages.TryGetValue(filterTab, out page) ? page
                                                      : CreatePageFromTab(tab);
    }

    private T GetPageFromTab<T>(PackageFilterTab? tab = null) where T : class
    , IPage { return GetPageFromTab(tab) as T; }

    private IPage CreatePageFromTab(PackageFilterTab? tab = null) {
      var filterTab = tab ?? m_PackageFiltering.currentFilterTab;
      IPage page;
      if (filterTab == PackageFilterTab.AssetStore) {
        page = new PaginatedPage(
            m_PackageDatabase, m_AssetStoreClient, m_PackageFiltering,
            m_PackageManagerPrefs, filterTab,
            new PageCapability{
                requireUserLoggedIn = true, requireNetwork = true,
                supportFilters = true,
                orderingValues = new[]{
                    new PageCapability.Ordering("Name", "name"),
                    new PageCapability.Ordering("Purchased date",
                                                "purchased_date"),
                    new PageCapability.Ordering("Update date", "update_date"),
                }});
      } else if (filterTab == PackageFilterTab.All) {
        page = new SimplePage(
            m_PackageDatabase, m_PackageFiltering, filterTab,
            new PageCapability{
                requireUserLoggedIn = false, requireNetwork = false,
                orderingValues =
                    new[]{new PageCapability.Ordering("Name", "displayName"),
                          new PageCapability.Ordering("Published date",
                                                      "publishedDate")}});
    ...
    private void
    TriggerOnVisualStateChange(IEnumerable<VisualState> visualStates) {
      onVisualStateChange?.Invoke(visualStates);
    }

    private void TriggerOnPageUpdate(IPage page,
                                     IEnumerable<IPackage> addedOrUpdated,
    ...
    public virtual IPage GetCurrentPage() { return GetPageFromTab(); }
    ...
    public virtual void
    GetSelectedPackageAndVersion(out IPackage package,
                                 out IPackageVersion version) {
      GetPageFromTab().GetSelectedPackageAndVersion(out package, out version);
    }

    public virtual void ClearSelection() { SetSelected(null); }

    public virtual void SetSelected(IPackage package,
                                    IPackageVersion version = null,
                                    bool forceSelectInInspector = false) {
      GetPageFromTab().SetSelected(package, version);
      SelectInInspector(package, version, forceSelectInInspector);
    }

    private void SelectInInspector(IPackage package, IPackageVersion version,
                                   bool forceSelectInInspector) {
      // There are two situations when we want to select a package in the
      // inspector 1) we explicitly/force to select it as a result of a manual
      // action (in this case, forceSelectInInspector should be set to true) 2)
      // currently another package is selected in inspector, hence we are sure
      // that we are not stealing selection from some other window
      var currentPackageSelection =
          m_Selection.activeObject as PackageSelectionObject;
      if (forceSelectInInspector || currentPackageSelection != null) {
        var packageSelectionObject =
            GetPackageSelectionObject(package, version, true);
    ...
    public virtual PackageFilterTab FindTab(IPackage package,
                                            IPackageVersion version = null) {
      ...
      if (version?.isInstalled == true || package?.versions?.installed != null)
        ...
        {
          Debug.Log(
              "You must check \"Enable Preview Packages\" in Project Settings > Package Manager in order to see this package.");
          return page.tab;
        }
    ...
    private void
    OnInstalledOrUninstalled(IPackage package,
                             IPackageVersion installedVersion = null) {
      if (package != null)
    ...
    private void OnFilterChanged(PackageFilterTab filterTab) {
      ...
      // When the filter tab is changed, on a page level, the selection hasn't
      // changed because selection is kept for each filter However, the active
      // selection is still changed if you look at package manager as a whole
    ...
    private void OnPackagesChanged(IEnumerable<IPackage> added,
      ...
      foreach (var package in removed) {
        ...
        if (packageSelectionObject != null) {
          m_PackageSelectionObjects.Remove(
              packageSelectionObject.versionUniqueId);
    ...
    private void OnProductListFetched(AssetStorePurchases productList,
                                      bool fetchDetailsCalled) {
      GetPageFromTab<PaginatedPage>(PackageFilterTab.AssetStore)
          .OnProductListFetched(productList, fetchDetailsCalled);
    }

    private void OnProductFetched(long productId) {
      GetPageFromTab<PaginatedPage>(PackageFilterTab.AssetStore)
          .OnProductFetched(productId);
    }
    ...
    private static RefreshOptions GetRefreshOptionsByTab(PackageFilterTab tab) {
      var index = (int) tab;
      return index >= k_RefreshOptionsByTab.Length ? RefreshOptions.None
          : k_RefreshOptionsByTab[index];
    }

    public virtual void Refresh(PackageFilterTab? tab = null,
                                int pageSize = k_DefaultPageSize) {
    ...
    public virtual void Refresh(RefreshOptions options,
      ...
      if ((options & RefreshOptions.Purchased) != 0) {
        var queryArgs = new PurchasesQueryArgs{startIndex = 0, limit = pageSize,
    ...
    public virtual void Fetch(string uniqueId) {
      ...
      if (m_UnityConnect.isUserLoggedIn &&
          long.TryParse(uniqueId, out productId)) {
    ...
    private void OnUserLoginStateChange(bool userInfoReady, bool loggedIn) {
      ...
      if (canRefresh && loggedIn)
        Refresh(RefreshOptions.Purchased,
                m_PackageManagerPrefs.numItemsPerPage ?? k_DefaultPageSize);
    }
    ...
    private void OnEditorSelectionChanged() {
      ...
      m_PackageDatabase.GetPackageAndVersion(
          packageSelectionObject.packageUniqueId,
          packageSelectionObject.versionUniqueId, out package, out version);
    ...
    public void OnEnable() {
      ...
      m_PackageManagerPrefs.onShowDependenciesChanged +=
          OnShowDependenciesChanged;
    ...
    private void InitializeSelectionObjects() {
      foreach (var id in m_SerializedPackageSelectionInstanceIds) {
        var packageSelectionObject =
            UnityEngine.Object.FindObjectFromInstanceID(id)
                as PackageSelectionObject;
        if (packageSelectionObject != null)
          m_PackageSelectionObjects[packageSelectionObject.name] =
              packageSelectionObject;
      }
    }
    ...
    public virtual bool IsRefreshInProgress(RefreshOptions option) {
      return m_RefreshOperationsInProgress.Any(
          operation =>(operation.refreshOptions & option) != 0);
    }
