...
namespace UnityEditor.PackageManager.UI {
  internal class PackageList : VisualElement {
    ...
    private PackageItem GetPackageItem(string packageUniqueId) {
      return string.IsNullOrEmpty(packageUniqueId)
                 ? null
                 : m_PackageItemsLookup.Get(packageUniqueId);
    }
    ...
    private ISelectableItem GetSelectedItem() {
      ...
      return packageItem.versionItems.FirstOrDefault(v => v.targetVersion ==
                                                          selectedVersion);
    }
    ...
    private void ShowPackages(bool updateScrollPosition) {
      ...
      var selectedVisualState =
          selectedVersion !=
          null ? page.GetVisualState(selectedVersion.packageUniqueId) : null;
      if (selectedVisualState?.visible != true) {
        var firstVisible = page.visualStates.FirstOrDefault(v => v.visible);
        if (firstVisible != null) {
          IPackage package;
          IPackageVersion version;
          m_PackageDatabase.GetPackageAndVersion(firstVisible.packageUniqueId,
                                                 firstVisible.selectedVersionId,
                                                 out package, out version);
    ...
    private void HidePackagesShowLogin() {
      ...
      // when the editor first starts, we detect the user as not logged in (even
      // though they are) because userInfo is not ready yet in this case, we
      // want to delay showing the login window until the userInfo is ready
      UIUtils.SetElementDisplay(loginContainer, m_UnityConnect.isUserInfoReady);

      m_PageManager.ClearSelection();
    }

    private void HidePackagesShowMessage(bool isRefreshInProgress,
                                         bool isInitialFetchingDone) {
      UIUtils.SetElementDisplay(scrollView, false);
      UIUtils.SetElementDisplay(emptyArea, true);
      ...
      if (isRefreshInProgress) {
        ...
        noPackagesLabel.text =
            string.Format(L10n.Tr("No results for \"{0}\""), searchText);
      }
    ...
    private void RefreshList(bool updateScrollPosition) {
      ...
      if (isListEmpty || !isInitialFetchingDone) {
        HidePackagesShowMessage(m_PageManager.IsRefreshInProgress(),
                                isInitialFetchingDone);
        return;
      }
    ...
    private void OnLoginClicked() { m_UnityConnect.ShowLogin(); }
    ...
    private void OnGeometryChange(GeometryChangedEvent evt) {
      ...
      if (!float.IsNaN(containerHeight))
        m_PackageManagerPrefs.numItemsPerPage =
            (int)(containerHeight / PackageItem.k_MainItemHeight);
    }
    ...
    private void OnRefreshOperationStart() { RefreshList(false); }

    private void OnRefreshOperationFinish() { RefreshList(false); }

    internal void OnFocus() { ScrollIfNeeded(); }
    ...
    private void OnListUpdate(IPage page, IEnumerable<IPackage> addedOrUpated,
                              IEnumerable<IPackage> removed, bool reorder) {
      ...
      if (reorder) {
        ...
        m_PackageItemsLookup = packageItems.ToDictionary(
            item => item.package.uniqueId, item => item);
      }
    ...
    internal bool SelectNext(bool reverseOrder) {
      ...
      // If the PackageItem is expanded, we want to start the search in the
      // version list of the PackageItem
      if (packageItem.visualState.expanded) {
        var versionItem = packageItem.versionItems.FirstOrDefault(
            v => v.targetVersion == selectedVersion);
        var nextVersionItem = UIUtils.FindNextSibling(versionItem, reverseOrder)
                                  as PackageVersionItem;
        if (nextVersionItem != null) {
          m_PageManager.SetSelected(nextVersionItem.package,
                                    nextVersionItem.targetVersion, true);
          return true;
        }
      }
      ...
      var nextPackageItem =
          UIUtils.FindNextSibling(packageItem, reverseOrder) as PackageItem;
      if (nextPackageItem == null)
        return false;

      if (nextPackageItem.visualState.expanded) {
        var versionItem = reverseOrder ? nextPackageItem.versionItems
                              .LastOrDefault()
            : nextPackageItem.versionItems.FirstOrDefault();
        if (versionItem != null) {
          m_PageManager.SetSelected(versionItem.package,
                                    versionItem.targetVersion, true);
          return true;
        }
      }
      m_PageManager.SetSelected(nextPackageItem.package,
                                nextPackageItem.targetVersion, true);
      return true;
    }
    ...
    private ScrollView scrollView {
      get { return cache.Get<ScrollView>("scrollView"); }
    }
    private VisualElement itemsList {
      get { return cache.Get<VisualElement>("itemsList"); }
    }
    private VisualElement emptyArea {
      get { return cache.Get<VisualElement>("emptyArea"); }
    }
    private Label noPackagesLabel {
      get { return cache.Get<Label>("noPackagesLabel"); }
    }
    private VisualElement loginContainer {
      get { return cache.Get<VisualElement>("loginContainer"); }
    }
    private Button loginButton {
      get { return cache.Get<Button>("loginButton"); }
    }
  }
}
