...
private const string k_SkipRemoveConfirmationPrefs =
	"PackageManager.SkipRemoveConfirmation";
private const string k_SkipDisableConfirmationPrefs =
	"PackageManager.SkipDisableConfirmation";
private const string k_ShowPackageDependenciesPrefs =
	"PackageManager.ShowPackageDependencies";
...
private static string projectIdentifier {
	get {
		// PlayerSettings.productGUID is already used as LocalProjectID by
		// Analytics, so we use it too
		return PlayerSettings.productGUID.ToString();
	}
}

private static string lastUsedFilterForProjectPerfs {
	get { return k_LastUsedFilterPrefsPrefix + projectIdentifier; }
}
...
public virtual PackageFilterTab? lastUsedPackageFilter {
	get {
		try {
			return (PackageFilterTab)
			       Enum.Parse(typeof(PackageFilterTab),
			                  EditorPrefs.GetString(lastUsedFilterForProjectPerfs,
			                                        defaultFilterTab.ToString()));
		} catch (Exception) {
			return null;
		}
	}
...
// The number of items per page is used to decide how many items to fetch in
// the initial refresh and it should always be a positive number When the
// number is set to 0, we consider this value not set (null)
