...
namespace UnityEditor.PackageManager.UI {
  internal class PackageLoadBar : VisualElement {
    ...
    private void UpdateLoadBarMessage() {
      if (!m_UnityConnect.isUserLoggedIn || m_Total == 0 ||
      ...
      if (m_Total == m_NumberOfPackagesShown) {
        ...
        m_LoadedText = m_Total == 1
                           ? L10n.Tr("One package shown")
                           : string.Format(L10n.Tr("All {0} packages shown"),
                                           m_NumberOfPackagesShown);
      } else {
        ...
        {
          // If the difference between the min and the max is bigger than
          // k_MinMaxDifference We show the two labels, else we only show that
          // one label of the number of packages left
        ...
        m_LoadedText = string.Format(L10n.Tr("{0} of {1}"),
                                     m_NumberOfPackagesShown, m_Total);
      }
      SetLabels();
    }
    ...
    private Label loadedLabel {
      get { return cache.Get<Label>("loadedLabel"); }
    }
    private Label loadMoreLabel {
      get { return cache.Get<Label>("loadMoreLabel"); }
    }
    private Label loadMinLabel {
      get { return cache.Get<Label>("loadMinLabel"); }
    }
    private Label loadMaxLabel {
      get { return cache.Get<Label>("loadMaxLabel"); }
    }
    private VisualElement loadBarContainer {
