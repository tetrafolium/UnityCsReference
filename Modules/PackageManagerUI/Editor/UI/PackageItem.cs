...
public IPackageVersion targetVersion {
	get { return package?.versions.primary; }
}
...
private IPackageVersion selectedVersion {
	get {
		if (package == null ||
		    string.IsNullOrEmpty(visualState?.selectedVersionId))
			return null;
		return package.versions.FirstOrDefault(
			v => v.uniqueId == visualState.selectedVersionId);
	}
}

internal IEnumerable<PackageVersionItem> versionItems {
	get { return versionList.Children().Cast<PackageVersionItem>(); }
}
...
public void UpdateVisualState(VisualState newVisualState = null) {
	...
	visualState = newVisualState?.Clone()
	              ?? visualState ?? new VisualState(package?.uniqueId);

	if (UIUtils.IsElementVisible(this) != visualState.visible)
		UIUtils.SetElementDisplay(this, visualState.visible);

	if (selectedVersion != null && visualState != null &&
	    selectedVersion != targetVersion)
		visualState.seeAllVersions =
			visualState.seeAllVersions ||
			!package.versions.key.Contains(selectedVersion);

	var expansionChanged =
		UIUtils.IsElementVisible(versionsContainer) != visualState.expanded;
	if (expansionChanged)
		UpdateExpanderUI(visualState.expanded);

	var needRefreshVersions =
		expansionChanged || seeAllVersionsOld != visualState.seeAllVersions;
	if (needRefreshVersions)
		RefreshVersions();

	if (needRefreshVersions ||
	    selectedVersionIdOld != visualState.selectedVersionId)
		RefreshSelection();
}
...
internal void SetPackage(IPackage package) {
	...
	// if the package gets updated while it's selected, we need to do some
	// special handling
	if (!string.IsNullOrEmpty(visualState?.selectedVersionId)) {
		// if the primary version was selected but there is a new primary
		// version select the new primary version to keep the main item selected
		if (visualState.selectedVersionId == oldDisplayVersion?.uniqueId &&
		    oldDisplayVersion?.uniqueId != displayVersion.uniqueId)
	...
	var showVersionList = !displayVersion.HasTag(PackageTag.BuiltIn) &&
...
public void RefreshState() {
	...
	if (state != PackageState.InProgress ||
		...
		var stateClass =
			state != PackageState.None ? state.ToString().ToLower() : null;
...
private void RefreshStateLabel(PackageState state) {
	if (state == PackageState.Installed ||
	    state == PackageState.InstalledAsDependency ||
	    state == PackageState.InDevelopment) {
		if (package.Is(PackageType.BuiltIn))
			stateLabel.text = L10n.Tr("Enabled");
		else {
			var installedVersion = package.versions.installed;
			var versionText = installedVersion.version?.ToString()
			                  ?? installedVersion.versionString;
			stateLabel.text =
				string.Format(L10n.Tr("Installed {0}"), versionText);
		}
...
private void RefreshVersions() {
	...
	var showToolbar =
		!seeAllVersions && allVersions.Count > keyVersions.Count;
...
// Note: ListItemMaxHeight is used because there is an issue with
// VisualElement where at construction time,
//          styling is not yet applied and max height returns 0 even though
//          the stylesheet has it at 156.
private void FixVersionListStyle(List<IPackageVersion> versions) {
	...
	var maxHeight =
		Math.Max(versionList.style.maxHeight.value.value, listItemMaxHeight);
	versionList.style.minHeight =
		Math.Min(versions.Count * listItemSpacing, maxHeight);

	// Hack until ScrollList has a better way to do the same -- Vertical
	// scroll bar is not yet visible
	var maxNumberOfItemBeforeScrollbar = 6;
	versionList.EnableClass("hasScrollBar",
	                        versions.Count > maxNumberOfItemBeforeScrollbar);
}
...
public void RefreshSelection() {
	...
	foreach (var version in versionItems)
		version.EnableClass(k_SelectedClassName,
		                    selectedVersion == version.targetVersion);
}
...
private Label nameLabel {
	get { return cache.Get<Label>("packageName"); }
}
private Label seeAllVersionsLabel {
	get { return cache.Get<Label>("seeAllVersions"); }
}
private VisualElement versionToolbar {
	get { return cache.Get<VisualElement>("versionsToolbar"); }
}
private VisualElement tagContainer =>
cache.Get<VisualElement>("tagContainer");
private VisualElement mainItem {
	get { return cache.Get<VisualElement>("mainItem"); }
}
private VisualElement stateIcon {
	get { return cache.Get<VisualElement>("stateIcon"); }
}
private Label stateLabel {
	get { return cache.Get<Label>("stateLabel"); }
}
private LoadingSpinner spinner {
	get { return cache.Get<LoadingSpinner>("packageSpinner"); }
}
private Toggle arrowExpander {
	get { return cache.Get<Toggle>("arrowExpander"); }
}
private Label expanderHidden {
	get { return cache.Get<Label>("expanderHidden"); }
}
private VisualElement versionsContainer {
	get { return cache.Get<VisualElement>("versionsContainer"); }
}
private ScrollView versionList {
	get { return cache.Get<ScrollView>("versionList"); }
}

private static readonly string[] k_TooltipsByState = {
	"", "This package is installed.",
	// Keep the error message for `installed` and `installedAsDependency`
	// the same for now as requested by the designer
	"This package is installed.", "This package is available for download.",
