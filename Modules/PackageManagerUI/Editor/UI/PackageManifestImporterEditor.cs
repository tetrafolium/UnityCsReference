...
namespace UnityEditor.PackageManager.UI {
  ...
  internal class PackageManifestImporterEditor : AssetImporterEditor {
    enum PackageVisibility { DependsOnType, AlwaysHidden, AlwaysVisible }

    private static readonly string s_LocalizedTitle =
        L10n.Tr("Package '{0}' Manifest");
    private static readonly string s_LocalizedMultipleTitle =
        L10n.Tr("{0} Package Manifests");
    private static readonly string s_LocalizedInvalidPackageManifest =
        L10n.Tr("Invalid Package Manifest");
    private static readonly string
        s_LocalizedPackageManagerUINotInstalledWarning = L10n.Tr(
            "Package Manager UI package is required to see selected packaged detail.");
    ...
    private static List<string> MajorUnityVersions {
      get {
        ...
        for (var majorVersion = kMinimalUnityMajorVersionSupported;
    ...
    private static readonly List<string> MinorUnityVersions =
    ...
    PackageManifestState packageState =>
        extraDataTarget as PackageManifestState;

    private static class Styles {
      public static readonly GUIContent information =
          EditorGUIUtility.TrTextContent("Information");
      public static readonly GUIContent name = EditorGUIUtility.TrTextContent(
          "Name", "Package name. Must be lowercase");
      public static readonly GUIContent organizationName =
          EditorGUIUtility.TrTextContent(
              "Organization name",
              "Package organization name. Must be lowercase and not include dots '.'");
      public static readonly GUIContent displayName =
          EditorGUIUtility.TrTextContent("Display name",
                                         "Display name used in UI.");
      public static readonly GUIContent version =
          EditorGUIUtility.TrTextContent(
              "Version",
              "Package Version, much follow SemVer (ex: 1.0.0-preview.1).");
      public static readonly GUIContent type =
          EditorGUIUtility.TrTextContent("Type", "Package Type (optional).");

      public static readonly GUIContent showAdvanced =
          EditorGUIUtility.TrTextContent("Advanced", "Show advanced settings.");

      public static readonly GUIContent visibility =
          EditorGUIUtility.TrTextContent("Visibility in Editor",
                                         "Package visibility in Editor.");

      public static readonly GUIContent unity =
          EditorGUIUtility.TrTextContent("Minimal Unity Version");
      public static readonly GUIContent unityMajor =
          EditorGUIUtility.TrTextContent("Major", "Major version of Unity");
      public static readonly GUIContent unityMinor =
          EditorGUIUtility.TrTextContent("Minor", "Minor version of Unity");
      public static readonly GUIContent unityRelease =
          EditorGUIUtility.TrTextContent("Release",
                                         "Specific release (ex: 0a9)");

      public static readonly GUIContent description =
          EditorGUIUtility.TrTextContent("Brief Description");

      public static readonly GUIContent dependencies =
          EditorGUIUtility.TrTextContent("Dependencies");
      public static readonly GUIContent package =
          EditorGUIUtility.TrTextContent("Package name",
                                         "Package name. Must be lowercase");

      public static readonly GUIContent viewInPackageManager =
          EditorGUIUtility.TrTextContent("View in Package Manager");
    }
...
#pragma warning restore 0649
    ...
    internal override string targetTitle {
      get {
        ...
        return string.Format(
            s_LocalizedTitle,
            packageState != null &&
                    packageState.isValidFile ? !IsNullOrEmptyTrim(
                        packageState.info.displayName)
                ? packageState.info.displayName.Trim()
                : packageState.info.packageName.completeName
    ...
    internal override void OnHeaderControlsGUI() {
      ...
      GUI.enabled = packageState != null && packageState.isValidFile &&
                    targets.Length == 1;
      if (GUILayout.Button(Styles.viewInPackageManager,
                           EditorStyles.miniButton)) {
        PackageManagerWindow.SelectPackageAndFilterStatic(
            packageState.info.packageName.completeName);
      }
    ...
    public override void OnEnable() {
      ...
      m_OrganizationName = extraDataSerializedObject.FindProperty(
          "info.packageName.organizationName");
      m_DisplayName =
          extraDataSerializedObject.FindProperty("info.displayName");
      m_Version = extraDataSerializedObject.FindProperty("info.version");
      m_UnityVersionEnabled =
          extraDataSerializedObject.FindProperty("info.unity.isEnable");
      ...
      m_UnityRelease =
          extraDataSerializedObject.FindProperty("info.unity.release");
      m_Description =
          extraDataSerializedObject.FindProperty("info.description");
      m_Type = extraDataSerializedObject.FindProperty("info.type");
      m_Advanced = extraDataSerializedObject.FindProperty("info.settings");
      m_Visibility =
          extraDataSerializedObject.FindProperty("info.settings.visibility");

      m_DependenciesList = new ReorderableList(
          extraDataSerializedObject,
          extraDataSerializedObject.FindProperty("dependencies"), true, false,
          true, true){drawElementCallback = DrawDependencyListElement,
    ...
    protected override void InitializeExtraDataInstance(Object extraData,
                                                        int targetIndex) {
      ReadPackageManifest(targets[targetIndex],
                          (PackageManifestState) extraData);
    }
    ...
    private void DrawDependencyListElement(Rect rect, int index, bool isActive,
                                           bool isFocused) {
      ...
      packageName.stringValue =
          EditorGUI.DelayedTextField(rect, packageName.stringValue);

      if (!IsNullOrEmptyTrim(packageName.stringValue) &&
          !PackageValidation.ValidateName(packageName.stringValue))
        errorMessages.Add(
            $"Invalid Dependency Package Name '{packageName.stringValue}'");

      using(new EditorGUI.DisabledScope(
          IsNullOrEmptyTrim(packageName.stringValue))) {
        rect.x += w / 3 * 2;
        rect.width = w / 3 - 4;
        version.stringValue =
            EditorGUI.DelayedTextField(rect, version.stringValue);

        if (!IsNullOrEmptyTrim(version.stringValue) &&
            !PackageValidation.ValidateVersion(version.stringValue))
          errorMessages.Add(
              $"Invalid Dependency Version '{version.stringValue}' for '{packageName.stringValue}'");
      }
    }

    protected override bool CanApply() { return errorMessages.Count == 0; }

    protected override void Apply() {
      base.Apply();

      for (int i = 0; i < targets.Length; i++) {
        WritePackageManifest(targets[i],
                             (PackageManifestState) extraDataTargets[i]);
      }
    }
    ...
    private void DoPackageInformationLayout() {
      using(new EditorGUILayout.VerticalScope(GUI.skin.box,
        ...
        m_OrganizationName.stringValue =
            EditorGUILayout.DelayedTextFieldDropDown(
                Styles.organizationName,
                m_OrganizationName.stringValue.ToLower(),
                Connect.UnityConnect.instance.userInfo.organizationNames);
        ...
        m_Type.stringValue = EditorGUILayout.DelayedTextFieldDropDown(
            Styles.type, m_Type.stringValue,
            PackageInfo.GetPredefinedPackageTypes());
        ...
        if (m_UnityVersionEnabled.boolValue) {
          ...
          m_UnityMajor.stringValue = EditorGUILayout.DelayedTextFieldDropDown(
              Styles.unityMajor, m_UnityMajor.stringValue,
              MajorUnityVersions.ToArray());

          EditorGUI.showMixedValue = m_UnityMinor.hasMultipleDifferentValues;
          m_UnityMinor.stringValue = EditorGUILayout.DelayedTextFieldDropDown(
              Styles.unityMinor, m_UnityMinor.stringValue,
              MinorUnityVersions.ToArray());

          EditorGUI.showMixedValue = m_UnityRelease.hasMultipleDifferentValues;
          m_UnityRelease.stringValue = EditorGUILayout.DelayedTextField(
              Styles.unityRelease, m_UnityRelease.stringValue);
    ...
    private void DoPackageDescriptionLayout() {
      using(new EditorGUILayout.VerticalScope(GUI.skin.box,
                                              GUILayout.ExpandWidth(true))) {
        using(var scrollView = new EditorGUILayout.VerticalScrollViewScope(
                  descriptionScrollViewPosition,
                  GUILayout.MinHeight(kMinHeightDescriptionScrollView))) {
          descriptionScrollViewPosition = scrollView.scrollPosition;
          m_Description.stringValue = EditorGUILayout.TextArea(
              m_Description.stringValue ?? "", GUILayout.ExpandHeight(true),
              GUILayout.ExpandWidth(true));
        }
      }
    }
    ...
    private void PerformValidation() {
      if (!PackageValidation.ValidateOrganizationName(
              m_OrganizationName.stringValue) &&
          !IsNullOrEmptyTrim(m_OrganizationName.stringValue))
        errorMessages.Add(
            $"Invalid Package Organization Name '{m_OrganizationName.stringValue}'");
      ...
      if (m_UnityVersionEnabled.boolValue) {
        if (!PackageValidation.ValidateUnityVersion(
                m_UnityMajor.stringValue, m_UnityMinor.stringValue,
                m_UnityRelease.stringValue)) {
          var unityVersion = string.Join(
              ".", new[]{m_UnityMajor.stringValue, m_UnityMinor.stringValue});
      ...
      if (IsNullOrEmptyTrim(m_DisplayName.stringValue) ||
          m_DisplayName.stringValue.Trim().Length == 0) {
        warningMessages.Add("Display name should be provided.");
      }

      if (IsNullOrEmptyTrim(m_Description.stringValue) ||
          m_Description.stringValue.Trim().Length == 0) {
        warningMessages.Add("Package description should be provided.");
      }

      if ((PackageVisibility)
              m_Visibility.intValue == PackageVisibility.AlwaysHidden) {
        warningMessages.Add(
            "This package and all its assets will be hidden by default in Editor because its visibility is set to 'Always Hidden'");
      }
      if ((PackageVisibility)
              m_Visibility.intValue == PackageVisibility.AlwaysVisible) {
        warningMessages.Add(
        ...
        else if (PackageInfo.GetPredefinedHiddenByDefaultPackageTypes()
                     .Contains(packageState.info.type))
          warningMessages.Add(
              $"This package and all its assets will be hidden by default in Editor because its type is '{packageState.info.type}'");
        else
          warningMessages.Add(
              $"This package and all its assets will be visible by default in Editor because its type is '{packageState.info.type}'");
      }
    }
    ...
    public override void OnInspectorGUI() {
      ...
      if (!m_IsValidFile.boolValue) {
        EditorGUILayout.HelpBox(s_LocalizedInvalidPackageManifest,
                                MessageType.Error);
        return;
      }
    ...
    private static void ReadPackageManifest(Object target,
                                            PackageManifestState packageState) {
      ...
      try {
        ...
        packageState.isValidFile = info != null && info.ContainsKey("name") &&
                                   info["name"] is string &&
        ...
        if (packageState.isValidFile) {
          ...
          var packageNameSplit =
              packageState.info.packageName.completeName.Split('.');
          ...
          if (packageNameSplitCount > 2) {
            ...
            packageState.info.packageName.organizationName =
                packageNameSplit[1];
            string domainAndOrganizationName =
                packageState.info.packageName.domain + "." +
                packageState.info.packageName.organizationName + ".";
            packageState.name =
                packageState.info.packageName.completeName.Replace(
                    domainAndOrganizationName, "");
            ...
            packageState.info.packageName.name =
                packageState.info.packageName.completeName;
          ...
          if (info.ContainsKey("hideInEditor") && info["hideInEditor"] is bool)
            packageState.info.settings.visibility =
                (bool) info["hideInEditor"] ? PackageVisibility
                    .AlwaysHidden : PackageVisibility.AlwaysVisible;
          else
            packageState.info.settings.visibility =
                PackageVisibility.DependsOnType;
          ...
          if (info.ContainsKey("unity") && info["unity"] is string) {
            ...
            packageState.info.unity = new PackageUnityVersion{
                isEnable = true, major = splitVersions[0],
                minor = splitVersions.Length > 1 ? splitVersions[1] : "",
                release = ""};

            if (info.ContainsKey("unityRelease") &&
                info["unityRelease"] is string)
              packageState.info.unity.release = (string) info["unityRelease"];
          } else {
            var unityVersion = InternalEditorUtility.GetUnityVersion();
            packageState.info.unity = new PackageUnityVersion{
                isEnable = false, major = unityVersion.Major.ToString(),
                minor = unityVersion.Minor.ToString(), release = ""};
          }
    ...
    private static void
    WritePackageManifest(Object target, PackageManifestState packageState) {
      var importer = target as PackageManifestImporter;
      if (importer == null)
        return;
      ...
      var organizationNameTrimmed =
          packageState.info.packageName.organizationName.Trim();
      var nameTrimmed = packageState.info.packageName.name.Trim();

      if (string.IsNullOrEmpty(domainTrimmed) &&
          string.IsNullOrEmpty(organizationNameTrimmed)) {
        json["name"] = nameTrimmed;
      } else if (string.IsNullOrEmpty(domainTrimmed) &&
                 !string.IsNullOrEmpty(organizationNameTrimmed)) {
        json["name"] =
            string.Join(".", new[]{organizationNameTrimmed, nameTrimmed});
      } else if (!string.IsNullOrEmpty(domainTrimmed) &&
                 !string.IsNullOrEmpty(organizationNameTrimmed)) {
        json["name"] = string.Join(
            ".", new[]{domainTrimmed, organizationNameTrimmed, nameTrimmed});
      }
      ...
      if (packageState.info.settings.visibility ==
          PackageVisibility.DependsOnType)
        json.Remove("hideInEditor");
      else
        json["hideInEditor"] = packageState.info.settings.visibility ==
                               PackageVisibility.AlwaysHidden;
      ...
      if (packageState.info.unity.isEnable) {
        if (!IsNullOrEmptyTrim(packageState.info.unity.major) &&
          ...
          json["unity"] =
              string.Join(".", new[]{packageState.info.unity.major.Trim(),
                                     packageState.info.unity.minor.Trim()});
