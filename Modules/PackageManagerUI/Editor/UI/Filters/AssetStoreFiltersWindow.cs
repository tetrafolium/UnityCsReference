...
namespace UnityEditor.PackageManager.UI {
  internal class AssetStoreFiltersWindow : PackageManagerFiltersWindow {
    ...
    internal static readonly string k_CategoriesFoldOutName =
        "categoriesFoldOut";
    ...
    internal static readonly string[] k_Statuses = {"Unlabeled", "Hidden",
                                                    "Deprecated"};
    ...
    protected override Vector2 GetSize() {
      ...
      var categories = m_CategoriesFoldOut?.Children().OfType<Toggle>()
          ?? Enumerable.Empty<Toggle>();
      height += (categories.Any() ? k_FoldOutHeight : 0) +
                categories.Count() * k_ToggleHeight;

      var labels = m_LabelsFoldOut?.Children().OfType<Toggle>()
          ?? Enumerable.Empty<Toggle>();
      var firstLabels = labels.Take(k_MaxDisplayLabels);
      height += (labels.Any() ? k_FoldOutHeight : 0) +
                firstLabels.Count() * k_ToggleHeight;
    ...
    protected override void ApplyFilters() {
      foreach (var toggle in m_StatusFoldOut.Children().OfType<Toggle>())
        toggle.SetValueWithoutNotify(toggle.name ==
                                     m_Filters.statuses?.FirstOrDefault());
        ...
        if (labels.Count() > Math.Max(k_MaxDisplayLabels, selectedLabels)) {
          m_ShowAllButton = new Button{text = L10n.Tr("Show all"),
                                       name = k_ShowAllButtonName};
    ...
    protected override void DoDisplay() {
      m_StatusFoldOut =
          new Foldout{text = L10n.Tr("Status"), name = k_StatusFoldOutName,
                      classList = {k_FoldoutClass}};
      foreach (var status in k_Statuses) {
        var toggle = new Toggle(status){name = status.ToLower(),
                                        classList = {k_ToggleClass}};
      ...
      if (m_Categories.Any()) {
        m_CategoriesFoldOut = new Foldout{text = L10n.Tr("Categories"),
                                          name = k_CategoriesFoldOutName,
                                          classList = {k_FoldoutClass}};
        ...
        m_CategoriesFoldOut.Query<Label>().ForEach(
            UIUtils.TextTooltipOnSizeChange);
        m_Container.Add(m_CategoriesFoldOut);
      }

      if (m_Labels.Any()) {
        m_LabelsFoldOut =
            new Foldout{text = L10n.Tr("Labels"), name = k_LabelsFoldOutName,
                        classList = {k_FoldoutClass}};
        var i = 0;
        foreach (var label in m_Labels) {
          var toggle = new Toggle(L10n.Tr(label)){name = label,
                                                  classList = {k_ToggleClass}};
          toggle.RegisterValueChangedCallback(evt => {
            if (evt.newValue) {
              // Uncheck Unlabeled if checked
              m_StatusFoldOut
                  .Q<Toggle>(k_Statuses [0]
                                 .ToLower())
                  .value = false;
            }
            UpdateFiltersIfNeeded();
    ...
    private void UpdateFiltersIfNeeded() {
      ...
      var selectedStatuses = m_StatusFoldOut.Children()
                                 .OfType<Toggle>()
                                 .Where(toggle => toggle.value)
                                 .Select(toggle => toggle.name.ToLower());
      filters.statuses = selectedStatuses.ToList();

      if (m_CategoriesFoldOut != null) {
        var selectedCategories = m_CategoriesFoldOut.Children()
                                     .OfType<Toggle>()
                                     .Where(toggle => toggle.value)
                                     .Select(toggle => toggle.name.ToLower());
      ...
      if (m_LabelsFoldOut != null) {
        var selectedLabels = m_LabelsFoldOut.Children()
                                 .OfType<Toggle>()
                                 .Where(toggle => toggle.value)
