  ...
  internal sealed class PackageEditor : Editor {
    ...
    internal override string targetTitle {
      get {
        ...
        return string.Format(
            ...
            m_Version != null ? string.IsNullOrEmpty(m_Version.displayName)
                ? m_Version.name
                : m_Version.displayName
    ...
    private static class Styles {
      ...
      public static readonly GUIContent name =
          EditorGUIUtility.TrTextContent("Name", "Package name");
      ...
      public static readonly GUIContent version =
          EditorGUIUtility.TrTextContent("Version", "Package Version");
      public static readonly GUIContent category =
          EditorGUIUtility.TrTextContent("Category", "Package Category");
      public static readonly GUIContent description =
          EditorGUIUtility.TrTextContent("Description");
      ...
      public static readonly GUIContent editPackage =
          EditorGUIUtility.TrTextContent("Edit");
      public static readonly GUIContent viewInPackageManager =
          EditorGUIUtility.TrTextContent("View in Package Manager");
    }

    private PackageSelectionObject packageSelectionObject =>
        target as PackageSelectionObject;
    ...
    private void OnPackagesChanged(IEnumerable<IPackage> added,
      ...
      if (string.IsNullOrEmpty(selectedPackageUniqueId))
        return;
      if (added.Concat(removed)
              .Concat(preUpdated)
              .Any(p => p.uniqueId == selectedPackageUniqueId))
        m_PackageDatabase.GetPackageAndVersion(
            packageSelectionObject.packageUniqueId,
            packageSelectionObject.versionUniqueId, out m_Package,
            out m_Version);
    }
    ...
    public override void OnInspectorGUI() {
      if (packageSelectionObject == null) {
        EditorGUILayout.HelpBox(
            L10n.Tr("This package is not accessible anymore."),
            MessageType.Error);
        return;
      }
      ...
      if (m_Package == null || m_Version == null) {
        ...
        if (!m_Version.isInstalled || m_AssetDatabase.GetAssetFolderInfo(
                                          "Packages/" + m_Package.name,
                                          out var rootFolder, out immutable))
      ...
      m_List = new ReorderableList(dependencies, typeof(DependencyInfo), false,
                                   true, false, false){
    ...
    internal override void OnHeaderControlsGUI() {
      ...
      GUI.enabled = targets.Length == 1 &&
                    m_Package?.state == PackageState.InDevelopment &&
                    (m_Version?.isInstalled ?? false);
      if (GUILayout.Button(Styles.editPackage, EditorStyles.miniButton)) {
        var path = m_Version.packageInfo.assetPath;
        var manifest = m_AssetDatabase.LoadAssetAtPath<PackageManifest>(
            $"{path}/package.json");
        if (manifest != null)
          m_Selection.activeObject = manifest;
      }
      GUI.enabled =
          targets.Length == 1 && m_Package != null && m_Version != null;
    ...
    internal override void OnForceReloadInspector() {
      ...
      if (packageSelectionObject != null &&
          (m_Package == null || m_Version == null))
        packageDatabase.GetPackageAndVersion(
            packageSelectionObject.packageUniqueId,
            packageSelectionObject.versionUniqueId, out m_Package,
            out m_Version);
    }
    ...
    private void DoPackageInformationLayout() {
      using(new EditorGUILayout.VerticalScope(GUI.skin.box,
        ...
        labels.AddRange(
            new[]{Styles.displayName, Styles.version, Styles.category});

        var contents = new List<string>();
        if (!m_Package.Is(PackageType.AssetStore))
          contents.Add(m_Version.name);
        contents.AddRange(new[]{m_Version.displayName,
    ...
    private void DoPackageDescriptionLabel() {
      ...
      var descriptionRect = GUILayoutUtility.GetRect(
          EditorGUIUtility.TempContent(m_Version.description), descriptionStyle,
          GUILayout.ExpandHeight(true), GUILayout.ExpandWidth(true));
      EditorGUI.SelectableLabel(descriptionRect, m_Version.description,
                                descriptionStyle);
    }
    ...
    private void DoPackageDescriptionLayout() {
      using(new EditorGUILayout.VerticalScope(GUI.skin.box,
        ...
        using(var scrollView = new EditorGUILayout.VerticalScrollViewScope(
                  m_ScrollPosition,
                  GUILayout.MinHeight(m_Package.Is(PackageType.AssetStore)
                                          ? kMinHeightForAssetStore
                                          : kMinHeightForOther))) {
    ...
    private static void SelectableLabelFields(IEnumerable<GUIContent> labels,
                                              IEnumerable<string> contents) {
      GUILayout.BeginHorizontal();
      GUILayout.BeginVertical();
      foreach (var label in labels)
        GUILayout.Label(label, GUILayout.MinWidth(kLabelMinWidth),
                        GUILayout.Height(EditorGUI.kSingleLineHeight));
      GUILayout.EndVertical();
      GUILayout.Space(EditorGUI.kSpacing);
      GUILayout.BeginVertical();
      foreach (var content in contents)
        EditorGUILayout.SelectableLabel(
            content, EditorStyles.textField,
            GUILayout.Height(EditorGUI.kSingleLineHeight));
      GUILayout.EndVertical();
