...
public static readonly string k_ResetPackagesMenuName =
	"Reset Packages to defaults";
public static readonly string k_ResetPackagesMenuPath =
	"Help/" + k_ResetPackagesMenuName;
...
public void OnEnable() {
	...
	m_Application.onInternetReachabilityChange +=
		OnInternetReachabilityChange;
}
...
private void DelayedSearchEvent() {
	if (DateTime.Now.Ticks - m_SearchTextChangeTimestamp >
	    k_SearchEventDelayTicks) {
		EditorApplication.update -= DelayedSearchEvent;
...
internal void SetFilter(PackageFilterTab filter) {
	...
	filterTabsMenu.text =
		L10n.Tr(string.Format("Packages: {0}", GetFilterDisplayName(filter)));
...
private void EnableMenuForCapability(PageCapability capability) {
	var enable =
		!(capability.requireUserLoggedIn && !m_UnityConnect.isUserLoggedIn) &&
...
private void UpdateFiltersMenuText(PageFilters filters) {
	...
	filtersMenu.text =
		filtersSet.Any() ? $"{L10n.Tr(" Filters ")} ({string.Join(",
      ", filtersSet.ToArray())})" : L10n.Tr("Filters");
}
...
private void SetupAddMenu() {
	addMenu.menu.AppendAction(L10n.Tr("Add package from disk..."), a => {
				var path = EditorUtility.OpenFilePanelWithFilters(
					L10n.Tr("Select package on disk"), "",
					new[] {"package.json file", "json"});
				if (Path.GetFileName(path) != "package.json") {
				        Debug.Log(L10n.Tr(
							  "Please select a valid package.json file in a package folder."));
				        return;
				}
	...
	addMenu.menu.AppendAction(L10n.Tr("Add package from tarball..."), a => {
				var path = EditorUtility.OpenFilePanelWithFilters(
					L10n.Tr("Select package on disk"), "",
					new[] {"Package tarball", "tgz, tar.gz"});
				if (!string.IsNullOrEmpty(path) &&
				    !m_PackageDatabase.isInstallOrUninstallInProgress) {
...
private void SetupFilterTabsMenu() {
	...
	filterTabsMenu.menu.AppendAction(
		L10n.Tr(GetFilterDisplayName(PackageFilterTab.All)),
		a => { SetFilterFromMenu(PackageFilterTab.All); },
		a => m_PackageFiltering.currentFilterTab ==
		PackageFilterTab.All ? DropdownMenuAction.Status.Checked
	  : DropdownMenuAction.Status.Normal);

	filterTabsMenu.menu.AppendAction(
		L10n.Tr(GetFilterDisplayName(PackageFilterTab.InProject)),
		a => { SetFilterFromMenu(PackageFilterTab.InProject); },
		a => m_PackageFiltering.currentFilterTab ==
		PackageFilterTab.InProject ? DropdownMenuAction.Status.Checked
	  : DropdownMenuAction.Status.Normal);

	filterTabsMenu.menu.AppendSeparator();
	filterTabsMenu.menu.AppendAction(
		L10n.Tr(GetFilterDisplayName(PackageFilterTab.AssetStore)),
		a => { SetFilterFromMenu(PackageFilterTab.AssetStore); },
		a => m_PackageFiltering.currentFilterTab ==
		PackageFilterTab.AssetStore ? DropdownMenuAction.Status.Checked
	  : DropdownMenuAction.Status.Normal);

	filterTabsMenu.menu.AppendSeparator();
	filterTabsMenu.menu.AppendAction(
		L10n.Tr(GetFilterDisplayName(PackageFilterTab.BuiltIn)),
		a => { SetFilterFromMenu(PackageFilterTab.BuiltIn); },
		a => m_PackageFiltering.currentFilterTab ==
		PackageFilterTab.BuiltIn ? DropdownMenuAction.Status.Checked
	  : DropdownMenuAction.Status.Normal);
...
private void UpdateOrdering(IPage page) {
	...
	foreach (var ordering in page.capability.orderingValues) {
		orderingMenu.menu.AppendAction(
			$"{L10n.Tr(ordering.displayName)} {k_Ascending}",
			a => {
			...
			a => {
					...
					!page.filters.isReverseOrder
		       ? DropdownMenuAction.Status
					.Checked : DropdownMenuAction.Status.Normal;
				});

		orderingMenu.menu.AppendAction(
			$"{L10n.Tr(ordering.displayName)} {k_Descending}",
			a => {
					orderingMenu.text = L10n.Tr("Sort: ") + a.name;
		...
		if (page.filters?.orderBy == ordering.orderBy) {
			orderingMenu.text =
				$"Sort: {L10n.Tr(ordering.displayName)} {(page.filters.isReverseOrder?k_Descending:k_Ascending)}";
		}
	}
}
...
private void SetupFilters() {
	...
	filtersMenu.clickable.clicked += () => {
		...
		var page = m_PageManager.GetCurrentPage();
		if (page != null &&
		    PackageManagerFiltersWindow.ShowAtPosition(
			    GUIUtility.GUIToScreenRect(filtersMenu.worldBound), page.tab,
			    page.filters)) {
...
private void SetupAdvancedMenu() {
	advancedMenu.menu.AppendAction(
		L10n.Tr("Show dependencies"),
		a => {
				ToggleDependencies();
				PackageManagerWindowAnalytics.SendEvent("toggleDependencies");
			},
		a => m_PackageManagerPrefs.showPackageDependencies ? DropdownMenuAction
		.Status.Checked
	  : DropdownMenuAction.Status.Normal);
	...
	advancedMenu.menu.AppendAction(
		L10n.Tr("Reset Packages to defaults"), a => {
...
private void ToggleDependencies() {
	m_PackageManagerPrefs.showPackageDependencies =
		!m_PackageManagerPrefs.showPackageDependencies;
}
...
private ToolbarMenu addMenu {
	get { return cache.Get<ToolbarMenu>("toolbarAddMenu"); }
}
private ToolbarMenu filterTabsMenu {
	get { return cache.Get<ToolbarMenu>("toolbarFilterTabsMenu"); }
}
private ToolbarMenu orderingMenu {
	get { return cache.Get<ToolbarMenu>("toolbarOrderingMenu"); }
}
private ToolbarWindowMenu filtersMenu {
	get { return cache.Get<ToolbarWindowMenu>("toolbarFiltersMenu"); }
}
private ToolbarButton clearFiltersButton {
	get { return cache.Get<ToolbarButton>("toolbarClearFiltersButton"); }
}
private ToolbarMenu advancedMenu {
	get { return cache.Get<ToolbarMenu>("toolbarAdvancedMenu"); }
}
private ToolbarSearchField searchToolbar {
	get { return cache.Get<ToolbarSearchField>("toolbarSearch"); }
}
}
}
