...
namespace UnityEditor.PackageManager.UI {
  internal class PackageManagerWindowRoot : VisualElement {
    ...
    private void
    ResolveDependencies(ResourceLoader resourceLoader, SelectionProxy selection,
    ...
    public PackageManagerWindowRoot(
        ResourceLoader resourceLoader, SelectionProxy selection,
        PackageFiltering packageFiltering,
        PackageManagerPrefs packageManagerPrefs,
        PackageDatabase packageDatabase, PageManager pageManager,
        PackageManagerProjectSettingsProxy settingsProxy,
        UnityConnectProxy unityConnectProxy) {
      ResolveDependencies(resourceLoader, selection, packageFiltering,
                          packageManagerPrefs, packageDatabase, pageManager,
                          settingsProxy, unityConnectProxy);
    ...
    public void OnEnable() {
      ...
      packageDetails.packageToolbarContainer.SetEnabled(
          !m_PackageDatabase.isEmpty);
      ...
      var newTab =
          m_PackageManagerPrefs.lastUsedPackageFilter ?? m_PackageManagerPrefs
              .defaultFilterTab;
      ...
      if (newTab != PackageFilterTab.All &&
          m_PageManager.GetRefreshTimestamp(PackageFilterTab.All) == 0)
    ...
    private void DelayRefresh(PackageFilterTab tab) {
      if (m_PackageManagerPrefs.numItemsPerPage == null ||
          tab == PackageFilterTab.AssetStore &&
              !m_UnityConnectProxy.isUserInfoReady) {
    ...
    private void OnFocusChanged(bool focus) {
      ...
      if (focus && canRefresh &&
        ...
        m_PageManager.Refresh(
            RefreshOptions.PurchasedOffline,
            m_PackageManagerPrefs.numItemsPerPage ?? PageManager
                .k_DefaultPageSize);
    }

    public void OnDisable() {
      m_PackageManagerPrefs.lastUsedPackageFilter =
    ...
    private void OnRefreshOperationFinish() {
      ...
      if (m_FilterToSelectAfterLoad != null &&
          m_PageManager.GetRefreshTimestamp(m_FilterToSelectAfterLoad) > 0)
    ...
    private void SelectPackageAndFilter() {
      ...
      if (!string.IsNullOrEmpty(m_PackageToSelectOnLoaded))
        m_PackageDatabase.GetPackageAndVersionByIdOrName(
            m_PackageToSelectOnLoaded, out package, out version);
      ...
      if (m_FilterToSelectAfterLoad == PackageFilterTab.AssetStore) {
        ...
        if (!string.IsNullOrEmpty(m_PackageToSelectOnLoaded)) {
          if (package == null || package is PlaceholderPackage)
            ...
            m_PageManager.GetPage(PackageFilterTab.AssetStore)
    ...
    public void OnFocus() { AddToClassList("focus"); }

    public void OnLostFocus() { RemoveFromClassList("focus"); }
    ...
    public void SelectPackageAndFilter(string packageToSelect,
                                       PackageFilterTab? filterTab = null,
                                       bool refresh = false,
                                       string searchText = "") {
      if (!string.IsNullOrEmpty(packageToSelect) || filterTab != null) {
        if (filterTab == null) {
          m_PackageDatabase.GetPackageAndVersionByIdOrName(
              packageToSelect, out var package, out var version);
          if (package != null)
            filterTab = m_PageManager.FindTab(package, version);
          else {
            var packageToSelectSplit = packageToSelect.Split('@');
            var versionString = packageToSelectSplit.Length == 2
                                    ? packageToSelectSplit[1]
                                    : string.Empty;

            // Package is not found in PackageDatabase but we can determine if
            // it's a preview package or not with it's version string.
            SemVersionParser.TryParse(versionString, out var semVersion);
            if (!m_SettingsProxy.enablePreviewPackages && semVersion.HasValue &&
                (semVersion.Value.Major == 0 ||
                 semVersion.Value.Prerelease.StartsWith("preview"))) {
              Debug.Log(
    ...
    internal PackageList packageList {
      get { return cache.Get<PackageList>("packageList"); }
    }
    internal PackageLoadBar packageLoadBar {
      get { return cache.Get<PackageLoadBar>("packageLoadBar"); }
    }
    private PackageDetails packageDetails {
      get { return cache.Get<PackageDetails>("packageDetails"); }
    }
    private PackageManagerToolbar packageManagerToolbar {
      get { return cache.Get<PackageManagerToolbar>("topMenuToolbar"); }
    }
    private PackageStatusBar packageStatusbar {
      get { return cache.Get<PackageStatusBar>("packageStatusBar"); }
    }
    private VisualSplitter mainContainer {
      get { return cache.Get<VisualSplitter>("mainContainer"); }
    }
  }
}
