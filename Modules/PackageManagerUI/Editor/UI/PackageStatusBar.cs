...
namespace UnityEditor.PackageManager.UI {
  internal class PackageStatusBar : VisualElement {
    ...
    internal static readonly string k_OfflineErrorMessage =
    ...
    private void UpdateStatusMessage() {
      ...
      if (!m_Application.isInternetReachable)
        ...
        var seeConsoleNotif =
            (UIError.Attribute.IsDetailInConsole & refreshError.attribute) != 0
                ? ", see console"
                : "";
      ...
      var label = timestamp == 0L
                      ? string.Empty
                      : L10n.Tr($"Last update {dt.ToString(" MMM d, HH
                                : mm ", CultureInfo.CreateSpecificCulture(" en -
                                      US "))}");
    ...
    private LoadingSpinner loadingSpinner {
      get { return cache.Get<LoadingSpinner>("loadingSpinner"); }
    }
    private Label errorIcon {
      get { return cache.Get<Label>("errorIcon"); }
    }
    private Label statusLabel {
      get { return cache.Get<Label>("statusLabel"); }
    }
    private DropdownButton refreshButton {
      get { return cache.Get<DropdownButton>("refreshButton"); }
    }
  }
}
