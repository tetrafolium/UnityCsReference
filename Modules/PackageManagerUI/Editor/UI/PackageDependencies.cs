...
public PackageDependencies() {
	...
	dependenciesExpander.RegisterValueChangedCallback(
		evt => SetExpanded(evt.newValue));
...
private string GetNameText(DependencyInfo dependency) {
	...
	return packageVersion !=
	       null ? packageVersion.displayName
	  : UpmPackageVersion.ExtractDisplayName(dependency.name);
}

private string GetStatusText(DependencyInfo dependency) {
	var installedVersion =
		m_PackageDatabase.GetPackage(dependency.name)?.versions.installed;
	...
	var statusText = installedVersion.HasTag(PackageTag.BuiltIn)
	                   ? L10n.Tr("enabled")
	                   : L10n.Tr("installed");
	return installedVersion.version?.ToString() ==
	       dependency.version
	       ? string.Format("({0})", statusText)
	  : string.Format("({0} {1})", installedVersion.version, statusText);
}

public void SetPackageVersion(IPackageVersion version) {
	var dependencies = version?.dependencies;
	var reverseDependencies =
		m_PackageDatabase.GetReverseDependencies(version);
	var showDependency =
		m_PackageManagerPrefs.showPackageDependencies &&
		(dependencies != null || reverseDependencies != null);
...
private void UpdateDependencies(DependencyInfo[] dependencies) {
	...
	foreach (var dependency in dependencies) {
		...
		var dependencyLowWidthItem = new PackageDependencySampleItemLowWidth(
			GetNameText(dependency), dependency.version.ToString(),
			BuildLabel(GetStatusText(dependency), "text"));
		dependenciesListLowWidth.Add(dependencyLowWidthItem);
	}
}

private void UpdateReverseDependencies(
	IEnumerable<IPackageVersion> reverseDependencies) {
	...
	UIUtils.SetElementDisplay(reverseDependenciesNames,
	                          hasReverseDependencies);
	UIUtils.SetElementDisplay(reverseDependenciesVersions,
	                          hasReverseDependencies);
	...
	foreach (var version in reverseDependencies) {
		reverseDependenciesNames.Add(
			BuildTextField(version.displayName ?? string.Empty, "text"));
		reverseDependenciesVersions.Add(
			BuildTextField(version.version.ToString(), "text"));

		var reverseDependencyLowWidthItem =
			new PackageDependencySampleItemLowWidth(
				version.displayName ?? string.Empty, version.version.ToString(),
				null);
...
private Toggle dependenciesExpander {
	get { return cache.Get<Toggle>("dependenciesExpander"); }
}
private VisualElement dependenciesOuterContainer {
	get { return cache.Get<VisualElement>("dependenciesOuterContainer"); }
}
private VisualElement dependenciesNormalView {
	get { return cache.Get<VisualElement>("dependenciesNormalView"); }
}
private Label noDependencies {
	get { return cache.Get<Label>("noDependencies"); }
}
private VisualElement dependenciesNames {
	get { return cache.Get<VisualElement>("dependenciesNames"); }
}
private VisualElement dependenciesVersions {
	get { return cache.Get<VisualElement>("dependenciesVersions"); }
}
private VisualElement dependenciesStatuses {
	get { return cache.Get<VisualElement>("dependenciesStatuses"); }
}
private VisualElement dependenciesListLowWidth {
	get { return cache.Get<VisualElement>("dependenciesListLowWidth"); }
}

private VisualElement reverseDependenciesNormalView {
	get { return cache.Get<VisualElement>("reverseDependenciesNormalView"); }
}
private Label noReverseDependencies {
	get { return cache.Get<Label>("noReverseDependencies"); }
}
private VisualElement reverseDependenciesNames {
	get { return cache.Get<VisualElement>("reverseDependenciesNames"); }
}
private VisualElement reverseDependenciesVersions {
	get { return cache.Get<VisualElement>("reverseDependenciesVersions"); }
}
private VisualElement reverseDependenciesListLowWidth {
	get {
		return cache.Get<VisualElement>("reverseDependenciesListLowWidth");
	}
}
}
}
