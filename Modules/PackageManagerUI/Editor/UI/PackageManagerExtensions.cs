...
internal static List<IPackageManagerExtension> Extensions {
	get {
		return extensions ??(extensions = new List<IPackageManagerExtension>());
	}
}
static List<IPackageManagerExtension> extensions;

internal static List<IPackageManagerToolbarExtension> ToolbarExtensions {
	get {
		return toolbarExtensions ??(
			toolbarExtensions = new List<IPackageManagerToolbarExtension>());
	}
}
static List<IPackageManagerToolbarExtension> toolbarExtensions;

internal static List<IPackageManagerMenuExtensions> MenuExtensions {
	get {
		return menuExtensions ??(menuExtensions =
						 new List<IPackageManagerMenuExtensions>());
	}
}
...
RegisterExtension(IPackageManagerToolbarExtension extension) {
	if (extension == null)
		return;
...
/// <param name="extension">A Package Manager UI toolbar extension</param>
internal static void
RegisterExtension(IPackageManagerMenuExtensions extension) {
	if (extension == null)
		return;
...
internal static void ExtensionCallback(Action action) {
	try {
		...
		Debug.LogError(string.Format(
				       L10n.Tr(
					       "[Package Manager Window] Package manager extension failed with error: {0}"),
				       exception.Message));
	}
}
}
