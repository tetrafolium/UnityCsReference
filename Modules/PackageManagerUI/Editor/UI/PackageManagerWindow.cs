...
namespace UnityEditor.PackageManager.UI {
  ...
  public static class Window {
    ...
    /// <item><description>packageId (e.g.
    /// com.unity.x@1.0.0)</description></item> <item><description>displayName
    /// (e.g. 2D Common)</description></item> <item><description>null (no
    /// specific package to focus)</description></item>
    /// </list>
    /// </summary>
    /// <param name="packageToSelect">packageToSelect can be identified by
    /// packageName, displayName, packageId, productId or null</param>
  ...
  internal class PackageManagerWindow : EditorWindow {
    internal static PackageManagerWindow instance {
      get;
      private set;
    }
    ...
    void OnEnable() {
      ...
      var settingsProxy =
          container.Resolve<PackageManagerProjectSettingsProxy>();
      var unityConnectProxy = container.Resolve<UnityConnectProxy>();

      m_Root = new PackageManagerWindowRoot(
          resourceLoader, selection, packageFiltering, packageManagerPrefs,
          packageDatabase, pageManager, settingsProxy, unityConnectProxy);
    ...
    void OnFocus() { m_Root?.OnFocus(); }

    void OnLostFocus() { m_Root?.OnLostFocus(); }
    ...
    internal static void OpenPackageManager(string packageToSelect) {
      ...
      if (!isWindowAlreadyVisible) {
        ...
        if (!string.IsNullOrEmpty(packageToSelect)) {
          ...
          IPackage package;
          packageDatabase.GetPackageAndVersionByIdOrName(
              packageToSelect, out package, out version);

          packageId =
              version?.uniqueId ?? package?.versions.primary.uniqueId ?? string
                  .Format("{0}@primary", packageToSelect);
        }
    ...
    internal static void OnPackageManagerResolve() {
      ...
      if (pageManager.GetRefreshTimestamp(RefreshOptions.UpmList |
                                          RefreshOptions.UpmListOffline) == 0)
        return;
    ...
    SelectPackageAndFilterStatic(string packageToSelect,
                                 PackageFilterTab? filterTab = null,
                                 bool refresh = false, string searchText = "") {
      instance = GetWindow<PackageManagerWindow>(typeof(SceneView));
      instance.minSize = new Vector2(800, 250);
      instance.m_Root.SelectPackageAndFilter(packageToSelect, filterTab,
                                             refresh, searchText);
      instance.Show();
    }
