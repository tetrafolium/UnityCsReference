...
namespace UnityEditor.PackageManager.UI {
  internal class PackageSampleItem {
    ...
    public PackageSampleItem(IPackageVersion version, Sample sample) {
      ...
      nameLabel.tooltip =
          sample.displayName; // add tooltip for when the label text is cut off
    ...
    private void OnImportButtonClicked() {
      ...
      if (previousImports.Count > 1) {
        warningMessage =
            L10n.Tr(
                "Different versions of the sample are already imported at") +
            "\n\n" + previousImportPaths + "\n" +
            L10n.Tr("They will be deleted when you update.");
      } else if (previousImports.Count == 1) {
        if (m_Sample.isImported) {
          warningMessage =
              L10n.Tr("The sample is already imported at") + "\n\n" +
              previousImportPaths + "\n" +
              L10n.Tr(
      ...
      PackageManagerWindowAnalytics.SendEvent("importSample",
                                              m_Version.uniqueId);
      ...
      if (m_Sample.Import(Sample.ImportOptions.OverridePreviousImports)) {
        ...
        if (m_Sample.isImported) {
          ...
          var importRelativePath =
              m_Sample.importPath.Replace(Application.dataPath, "Assets");
    ...
    internal Label importStatus {
      get {
        return m_ImportStatus ??(m_ImportStatus =
                                    new Label(){classList = {"importStatus"}});
      }
    }
    private Label m_NameLabel;
    internal Label nameLabel {
      get {
        return m_NameLabel ??(m_NameLabel =
                                 new Label(){classList = {"nameLabel"}});
      }
    }
    private Label m_SizeLabel;
    internal Label sizeLabel {
      get {
        return m_SizeLabel ??(m_SizeLabel =
                                 new Label(){classList = {"sizeLabel"}});
      }
    }
    private Button m_ImportButton;
    internal Button importButton {
      get {
        return m_ImportButton ??(m_ImportButton =
                                    new Button(){classList = {"importButton"}});
      }
    }
  }
}
