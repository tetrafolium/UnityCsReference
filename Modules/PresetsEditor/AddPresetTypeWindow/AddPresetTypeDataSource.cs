...
protected AdvancedDropdownItem RebuildTree() {
	...
	AdvancedDropdownItem root =
		new PresetTypeDropdownItem(L10n.Tr("Add Default Type"));
	...
	var presetTypes =
		...
		.Union(TypeCache.GetTypesDerivedFrom<ScriptedImporter>()
	...
	for (var i = 0; i < menuDictionary.Count; i++) {
		...
		for (var j = 0; j < paths.Length; j++) {
			...
			var group = (PresetTypeDropdownItem)
			            parent.children.SingleOrDefault(c => c.name == path);
			if (group == null) {
	...
	var scriptableObjectRoot =
		new PresetTypeDropdownItem(L10n.Tr("ScriptableObject"));
...
private int CompareItems(KeyValuePair<string, string> x,
