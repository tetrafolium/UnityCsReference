...
namespace UnityEditor.Presets {
  ...
  public sealed class Preset : Object {
    ...
    public Preset(Object source) { Internal_Create(this, source); }

    static extern void Internal_Create([ Writable ] Preset notSelf,
                                       [ NotNull ] Object source);

    public extern PropertyModification[] PropertyModifications {
      [NativeName("GetManagedPropertyModifications")] get;
    }

    public extern string[] excludedProperties {
      get;
      set;
    }

    public bool ApplyTo(Object target) { return ApplyTo(target, s_EmptyArray); }
    ...
    // Components returned by this method will have their GameObject hideFlags
    // set with DontAllowDestruction. The HideFlags must be set to
    // HideFlags.None before calling Destroy or DestroyImmediate on it.
    internal extern Object GetReferenceObject();

    [FreeFunction("GetDefaultPresetsForObject")]
    public static extern Preset[] GetDefaultPresetsForObject(
        [ NotNull ] Object target);

    [Obsolete(
        "Use GetDefaultPresetsForObject to get the full ordered list of default Presets that would be applied to that Object")]
    ...
    [Obsolete(
        "Use GetDefaultPresetsForType to get the full list of default Presets for a given PresetType.")]
    ...
    public static extern DefaultPreset[] GetDefaultPresetsForType(
        PresetType type);

    [FreeFunction("SetDefaultPresetsForType")]
    public static extern bool SetDefaultPresetsForType(PresetType type,
    ...
    public static bool SetAsDefault(Preset preset) {
      if (preset == null)
        throw new ArgumentNullException(nameof(preset));
      return SetDefaultPresetsForType(
          preset.GetPresetType(),
          new[]{new DefaultPreset{m_Filter = string.Empty, m_Preset = preset}});
    }
