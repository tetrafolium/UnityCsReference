...
namespace UnityEditor.Presets {
  ...
  internal class PresetEditor : Editor {
    static class Style {
      public static GUIContent presetType = EditorGUIUtility.TrTextContent(
          "Preset Type", "The Object type this Preset can be applied to.");
      ...
      public static GUIContent addToDefault = EditorGUIUtility.TrTextContent(
          "Add to {0} default",
          "The Preset will be added first in the default list with an empty filter.");
      public static GUIContent removeFromDefault = EditorGUIUtility.TrTextContent(
          "Remove from {0} default",
          "All entry using this Preset will be removed from the default list.");
      public static GUIContent enableProperty =
          EditorGUIUtility.TrTextContent("Include Property");
      public static GUIContent disableProperty =
          EditorGUIUtility.TrTextContent("Exclude Property");

      static Style() { inspectorBig.padding.bottom -= 1; }
    }
    ...
    static Dictionary<int, ReferenceCount> s_References =
        new Dictionary<int, ReferenceCount>();

    List<int> m_PresetsInstanceIds = new List<int>();
    bool m_DisplayErrorPreset;
    string m_SelectedPresetTypeName;
    Dictionary<string, List<Object>> m_InspectedTypes =
    ...
    public override void OnInspectorGUI() {
      if (m_DisplayErrorPreset) {
        EditorGUILayout.HelpBox(
            "Unable to load this Preset, the type is not supported.",
            MessageType.Error);
      } else {
        DrawPresetData();
      }
    ...
    void DrawPresetData() {
      string presetType = m_InspectedTypes.Count > 1
                              ? EditorGUI.mixedValueContent.text
                              : m_SelectedPresetTypeName;
      var rect = EditorGUI.PrefixLabel(EditorGUILayout.GetControlRect(true),
                                       Style.presetType);
    ...
    internal override void OnHeaderControlsGUI() {
      ...
      if (preset != null) {
        using(new EditorGUI.DisabledScope(
            targets.Length != 1 || !preset.GetPresetType().IsValidDefault())) {
          var defaultList =
              Preset.GetDefaultPresetsForType(preset.GetPresetType())
                  .Where(d => d.preset == preset);
          if (defaultList.Any()) {
            if (GUILayout.Button(
                    GUIContent.Temp(string.Format(Style.removeFromDefault.text,
                                                  preset.GetTargetTypeName()),
                                    Style.removeFromDefault.tooltip),
                    EditorStyles.miniButton, GUILayout.ExpandWidth(false))) {
              Undo.RecordObject(
                  Resources.FindObjectsOfTypeAll<PresetManager>().First(),
                  "Preset Manager");
            ...
            if (GUILayout.Button(
                    GUIContent.Temp(string.Format(Style.addToDefault.text,
                                                  preset.GetTargetTypeName()),
                                    Style.addToDefault.tooltip),
                    EditorStyles.miniButton, GUILayout.ExpandWidth(false))) {
              Undo.RecordObject(
                  Resources.FindObjectsOfTypeAll<PresetManager>().First(),
                  "Preset Manager");
              var list = Preset.GetDefaultPresetsForType(preset.GetPresetType())
                             .ToList();
              list.Insert(0, new DefaultPreset(string.Empty, preset));
              Preset.SetDefaultPresetsForType(preset.GetPresetType(),
                                              list.ToArray());
              Undo.FlushUndoRecordObjects();
            }
          }
    ...
    void GenerateInternalEditor() {
      if (m_InternalEditor == null) {
        ...
        for (var index = 0; index < targets.Length; index++) {
          ...
          if (!s_References.TryGetValue(p.GetInstanceID(), out reference)) {
            reference = new ReferenceCount(){
                count = 0, reference = p.GetReferenceObject()};
            if (reference.reference == null) {
              // fast exit on NULL targets as we do not support their inspector
              // in Preset.
              m_NotSupportedEditorName = p.GetTargetTypeName();
              m_UnsupportedIcon = EditorGUIUtility.LoadIcon(
                  m_NotSupportedEditorName.Replace('.', '/') + " Icon");
              if (m_UnsupportedIcon == null) {
                            ...
                            if (m_UnsupportedIcon == null) {
                              m_UnsupportedIcon =
                                  AssetPreview.GetMiniTypeThumbnailFromClassID(
                                      serializedObject
                                          .FindProperty(
                                              "m_TargetType.m_NativeTypeID")
                                          .intValue);
                              if (m_UnsupportedIcon == null) {
                                m_UnsupportedIcon =
        ...
        // Coming back from an assembly reload... our references are probably
        // broken and we need to fix them.
        for (var index = 0; index < targets.Length; index++) {
          var instanceID = targets [index]
                               .GetInstanceID();
          ReferenceCount reference = null;
          if (!s_References.TryGetValue(instanceID, out reference)) {
            reference = new ReferenceCount(){
                count = 0, reference = m_InternalEditor.targets[index]};
    ...
    void DestroyInternalEditor() {
      if (m_InternalEditor != null) {
        // Do not destroy anything if we are just reloading assemblies or
        // re-importing.
        bool shouldDestroyEverything =
            Unsupported.IsDestroyScriptableObject(this);
        if (shouldDestroyEverything)
          DestroyImmediate(m_InternalEditor);

        // On Destroy, look for instances id instead of target because they may
        // already be null.
          ...
          if (--s_References[instanceId].count == 0) {
            if (shouldDestroyEverything) {
              if (s_References[instanceId].reference is Component) {
                var go =
                    ((Component) s_References[instanceId].reference).gameObject;
                go.hideFlags =
                    HideFlags.None; // make sure we remove the don't destroy
                                    // flag before calling destroy
                DestroyImmediate(go);
              } else {
    ...
    void DrawInternalInspector() {
      using(var change = new EditorGUI.ChangeCheckScope()) {
        if (m_InternalEditor.target is Component) {
          bool wasVisible = InternalEditorUtility.GetIsInspectorExpanded(
              m_InternalEditor.target);
          bool isVisible =
              EditorGUILayout.InspectorTitlebar(wasVisible, m_InternalEditor);
          if (isVisible != wasVisible) {
            InternalEditorUtility.SetIsInspectorExpanded(
                m_InternalEditor.target, isVisible);
          }
        } else {
          m_InternalEditor.DrawHeader();
        }
        if (InternalEditorUtility.GetIsInspectorExpanded(
                m_InternalEditor.target) ||
            m_InternalEditor.HasLargeHeader()) {
          GUIStyle editorWrapper =
              (m_InternalEditor.UseDefaultMargins() &&
                       m_InternalEditor.CanBeExpandedViaAFoldoutWithoutUpdate()
    ...
    PropertyState GetPropertyState(string propertyPath,
      ...
      for (var index = 0; index < targets.Length && state != PropertyState.Both;
           index++) {
        Preset preset = (Preset) targets[index];
        if (checkChildren && preset.excludedProperties.Any(
                                 p => p.StartsWith(propertyPath + ".",
                                                   StringComparison.Ordinal)))
          return PropertyState.Both;
        if (preset.excludedProperties.Any(
                p => p == propertyPath ||
                     propertyPath.StartsWith(p + ".",
                                             StringComparison.Ordinal))) {
    ...
    void BeginProperty(Rect totalPosition, SerializedProperty property) {
      if (m_InternalEditor == null ||
          property.serializedObject != m_InternalEditor.serializedObject)
        return;
      ...
      if ((state & PropertyState.Excluded) == PropertyState.Excluded &&
        ...
        EditorGUI.DrawMarginLineForRect(
            totalPosition, new Color(240f / 255f, 81f / 255f, 60f / 255f));
    ...
    void AddExclusion(object path) {
      ...
      foreach (var preset in targets.OfType<Preset>()) {
        // set excludedProperties to an empty array for PropertyModifications to
        ...
        var childrenProperties = GetPresetProperties(preset).Where(
            p => p.StartsWith(propertyPath + ".", StringComparison.Ordinal));
        preset.excludedProperties =
            excluded.Except(childrenProperties).Concat(toAdd).ToArray();
      }
    ...
    void RemoveExclusion(object path) {
      ...
      foreach (var preset in targets.OfType<Preset>()) {
        ...
        var childrenAndSelf = properties.Where(
            p => p.StartsWith(propertyPath + ".", StringComparison.Ordinal) ||
                 p == propertyPath);
        ...
        if (removed.Count() != count) {
          ...
          // we need the first excluded parent and then exclude all its direct
          // children expect the selected path.
          ...
          // in the case of multi selection, we may have nothing to do on this
          // specific Preset.
          if (!removed.Contains(propPath))
            continue;

          var pathDepth = propPath.Count(c => c == '.');
          var toExclude = properties.Where(
              p =>
                  p.StartsWith(propPath + ".", StringComparison.Ordinal) &&
                  !p.StartsWith(propertyPath + ".", StringComparison.Ordinal) &&
                  p != propertyPath && p.Count(c => c == '.') == pathDepth + 1);

          preset.excludedProperties =
              removed.Except(new[]{propPath}).Concat(toExclude).ToArray();
        }
      }
    ...
    static IEnumerable<string> GetPresetProperties(Preset preset) {
      // We have to use PropertyModifications instead of directly a
      // SerializedProperty because some properties may be excluded from Preset
      // and we don't want them in excludedProperties
      return preset.PropertyModifications.Select(pm => pm.propertyPath)
    ...
    void DisableEnableProperty(GenericMenu menu, SerializedProperty property) {
      if (m_InternalEditor == null ||
          property.serializedObject.targetObject !=
              m_InternalEditor.serializedObject.targetObject)
        return;
      ...
      if ((state & PropertyState.Excluded) == PropertyState.Excluded) {
        menu.AddItem(Style.enableProperty, false, RemoveExclusion,
                     propertyPath);
      }
    }
    ...
    void DrawUnsupportedInspector() {
      ...
      EditorGUILayout.HelpBox(
          "Preset Inspectors are not supported for this type.",
          MessageType.Info);
    }
