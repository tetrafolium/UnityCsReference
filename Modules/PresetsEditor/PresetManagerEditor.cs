...
namespace UnityEditor.Presets {
  ...
  internal sealed class PresetManagerEditor : Editor {
    class Content {
      public static GUIContent presetManager =
          EditorGUIUtility.TrTextContent("Preset management");
    }

    static class Style {
      public static GUIContent managerIcon =
      ...
      public static GUIContent addDefault =
          EditorGUIUtility.TrTextContent("Add Default Preset");
    ...
    void OnEnable() { SetupDefaultList(); }

    void SetupDefaultList() {
      m_DefaultPresets = serializedObject.FindProperty("m_DefaultPresets");
      m_Defaults =
          new List<DefaultPresetReorderableList>(m_DefaultPresets.arraySize);
      for (int i = 0; i < m_DefaultPresets.arraySize; ++i) {
        SerializedProperty defaultPreset =
            m_DefaultPresets.GetArrayElementAtIndex(i);
        var presetType =
            new PresetType(defaultPreset.FindPropertyRelative("first"));
        var list = new DefaultPresetReorderableList(
            serializedObject, defaultPreset.FindPropertyRelative("second"),
            presetType);
        m_Defaults.Add(list);
      }
    ...
    public override void OnInspectorGUI() {
      if (serializedObject.UpdateIfRequiredOrScript() ||
          m_DefaultPresets.arraySize != m_Defaults.Count)
        SetupDefaultList();

      m_Search =
          EditorGUI.SearchField(EditorGUILayout.GetControlRect(), m_Search);
      EditorGUILayout.Space();

      using(var scope =
                new EditorGUILayout.VerticalScrollViewScope(m_ScrollPosition)) {
        for (int i = 0; i < m_DefaultPresets.arraySize; ++i) {
          if (string.IsNullOrEmpty(m_Search) ||
              m_Defaults[i].fullClassName.ToLower().Contains(
                  m_Search.ToLower()))
            m_Defaults [i]
                .DoLayoutList();
        }
      ...
      using(new EditorGUILayout.HorizontalScope()) {
        ...
        Rect rect = GUILayoutUtility.GetRect(Style.addDefault,
                                             Style.addComponentButtonStyle);
        if (EditorGUI.DropdownButton(rect, Style.addDefault, FocusType.Passive,
                                     Style.addComponentButtonStyle)) {
          if (AddPresetTypeWindow.Show(
                  rect, OnPresetTypeWindowSelection,
                  string.IsNullOrEmpty(m_Search) ? null : m_Search)) {
            GUIUtility.ExitGUI();
          }
        }
    ...
    static SettingsProvider CreatePresetManagerProvider() {
      ...
      provider.inspectorUpdateHandler += () => {
        if (provider.settingsEditor != null &&
            provider.settingsEditor.serializedObject
