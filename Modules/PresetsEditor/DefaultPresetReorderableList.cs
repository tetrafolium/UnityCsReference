...
public DefaultPresetReorderableList(SerializedObject serializedObject,
                                    SerializedProperty propertyArray,
                                    PresetType type)
	...
	drawHeaderCallback = rect =>
	                     DrawHeaderCallback(rect, content, fullNameContent);
	drawElementCallback = (rect, index, active, focused) =>
	                      DrawElementCallback(rect, index, active, focused, type,
	                                          serializedProperty);
...
static void DrawHeaderCallback(Rect rect, GUIContent content,
                               GUIContent fullNameContent) {
	...
	EditorGUI.SelectableLabel(firstLineRight, fullNameContent.text,
	                          Style.alignedRight);
	...
	Rect dividerRect =
		new Rect(filterRect.xMin - 5, filterRect.y + limitHeightOfDivider, 1f,
		         filterRect.height - 2 * limitHeightOfDivider);
...
static void DrawElementBackgroundCallback(Rect rect, int index,
...
static void DrawElementCallback(Rect rect, int index, bool isActive,
                                bool isFocused, PresetType keyType,
	...
	using (new EditorGUI.PropertyScope(enableRect, GUIContent.none,
	                                   disabledProperty)) {
		var toggleValue = EditorGUI.Toggle(enableRect, GUIContent.none,
		                                   !disabledProperty.boolValue);
	...
	using (new EditorGUI.DisabledScope(disabledProperty.boolValue)) {
		...
		DrawFilterField(filterRect,
		                defaultProperty.FindPropertyRelative("m_Filter"));
...
static void DrawPresetField(Rect rect, SerializedProperty presetProperty,
                            PresetType keyType, Preset presetObject) {
	// lets hack a bit the ObjectField because we want to open our own preset
	// selector.
	var buttonRect =
		new Rect(rect.xMax - rect.height, rect.y, rect.height, rect.height);
	...
	var controlID = GUIUtility.GetControlID(
		GUIContent.Temp(presetProperty.displayName), FocusType.Passive);
	var dropRect = rect;
	dropRect.xMax -= rect.height;
	EditorGUI.DoObjectField(rect, dropRect, controlID, typeof(Preset),
	                        presetProperty,
	                        (references, type, property, options) =>
	                        PresetFieldDropValidator(references, keyType),
	                        false);
}

static Object PresetFieldDropValidator(Object[] references,
...
static void DrawFilterField(Rect position, SerializedProperty filter) {
	...
	using (new EditorGUI.PropertyScope(fieldRect, GUIContent.none, filter)) {
		using (var changed = new EditorGUI.ChangeCheckScope()) {
			var newValue = EditorGUI.ToolbarSearchField(
				fieldRect, filter.stringValue, false);
			if (changed.changed)
