...
public static GUIContent presetIcon =
...
internal static PresetSelector get {
	get {
		if (s_SharedPresetSelector == null) {
			Object[] objs =
				Resources.FindObjectsOfTypeAll(typeof(PresetSelector));
...
public static bool DrawPresetButton(Rect rectangle, Object[] targets) {
	...
	if (!target || !new PresetType(target).IsValid() ||
	    (target.hideFlags & HideFlags.NotEditable) != 0)
		return false;

	if (EditorGUI.DropdownButton(rectangle, Style.presetIcon,
	                             FocusType.Passive,
...
public static void ShowSelector(Object[] targets, Preset currentSelection,
...
public static void ShowSelector(Object target, Preset currentSelection,
                                bool createNewAllowed,
                                PresetSelectorReceiver eventReceiver) {
	get.Init(target, currentSelection, createNewAllowed, eventReceiver);
}

public static void ShowSelector(PresetType presetType,
...
void Init(PresetType presetType, Preset currentSelection,
          bool createNewAllowed, PresetSelectorReceiver eventReceiver) {
	...
	UpdateSearchResult(currentSelection !=
	                   null ? currentSelection.GetInstanceID() : 0);
	...
	position = rect;
	minSize =
		new Vector2(kMinWidth, kMinTopSize + kPreviewExpandedAreaHeight +
		            2 * kPreviewMargin);
...
void Init(Object target, Preset currentSelection, bool createNewAllowed,
          PresetSelectorReceiver eventReceiver) {
	m_MainTarget = target;
	Init(new PresetType(target), currentSelection, createNewAllowed,
	     eventReceiver);
}

static IEnumerable<Preset> FindAllPresetsOfType(PresetType presetType) {
	return AssetDatabase.FindAssets("t:Preset")
	       .Select(a => AssetDatabase.LoadAssetAtPath<Preset>(
...
void OnGUI() {
	...
	var listPosition =
		EditorGUILayout.GetControlRect(true, GUILayout.ExpandHeight(true));
	int listKeyboardControlID = GUIUtility.GetControlID(FocusType.Keyboard);
	m_ListArea.OnGUI(
		new Rect(0, listPosition.y, position.width, listPosition.height),
		listKeyboardControlID);

	using (new EditorGUILayout.HorizontalScope(Style.bottomBarBg,
		...
		if (m_ListArea.CanShowThumbnails()) {
			using (var change = new EditorGUI.ChangeCheckScope()) {
				int newGridSize = (int) GUILayout.HorizontalSlider(
					m_ListArea.gridSize, m_ListArea.minGridSize,
					m_ListArea.maxGridSize, GUILayout.Width(55f));
				if (change.changed) {
...
void DrawSearchField() {
	using (var change = new EditorGUI.ChangeCheckScope()) {
		...
		var rect =
			EditorGUILayout.GetControlRect(false, 24f, Style.toolbarBack);
		rect.height = 40f;
		GUI.Label(rect, GUIContent.none, Style.toolbarBack);
		m_SearchField = EditorGUI.SearchField(
			new Rect(5f, 5f, position.width - 10f, 15f), m_SearchField);
		if (change.changed) {
...
static bool ApplyImportSettingsBeforeSavingPreset(ref Preset preset,
                                                  Object target) {
		...
		foreach (Editor e in activeEditor.activeEditors) {
			...
			if (editor != null && editor.target == target &&
				...
				if (EditorUtility.DisplayDialog(
					    ...
					    "Apply settings before creating a new preset", "Apply",
					    "Cancel")) {
					editor.ApplyAndImport();
					// after reimporting, the target object has changed, so update the
					// preset with the newly imported values.
...
static string CreatePresetDialog(ref Preset preset, Object target) {
	if (target is AssetImporter &&
	    ApplyImportSettingsBeforeSavingPreset(ref preset, target))
		return null;

	return EditorUtility.SaveFilePanelInProject(
		"New Preset", preset.GetTargetTypeName(), "preset", "",
...
static void CreatePreset(Object target) {
	...
	if (!string.IsNullOrEmpty(path)) {
		// If the asset already exist, we need to make sure we keep the same
		// PPtr valid in memory. To ensure that, we use CopySerialized on the
		// Asset instance instead of erasing the asset with CreateAsset.
