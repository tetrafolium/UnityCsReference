...
namespace UnityEditor.VFX {
  ...
  internal static class VFXExpressionHelper {
    [FreeFunction("VFX::GetTypeOfOperation",
                  IsThreadSafe = true)] extern public static VFXValueType
    GetTypeOfOperation(VFXExpressionOperation op, int data0, int data1,
                       int data2, int data3);

    [FreeFunction("VFX::GetSizeOfType",
                  IsThreadSafe = true)] extern public static int
    GetSizeOfType(VFXValueType type);

    [FreeFunction("VFX::GetTextureDimension",
                  IsThreadSafe = true)] extern public static TextureDimension
    GetTextureDimension(VFXValueType type);
  }
  ...
  internal struct VFXMappingTemporary {
    ...
    public VFXMappingTemporary(VFXMapping mapping, uint pastFrameIndex,
  ...
  internal sealed class VFXCPUBufferData : IDisposable {
    ...
    public VFXCPUBufferData() { m_Ptr = Internal_Create(); }
    ...
    [NativeMethod(IsThreadSafe = true)] extern static internal void
  ...
  internal struct VFXEditorTaskDesc {
    ...
    public int shaderSourceIndex {
      get { return m_ShaderSourceIndex; }
      set {
        processor = null;
  ...
  internal class VFXExpressionValueContainerDesc<T>
      : VFXExpressionValueContainerDesc {
    public T value = default(T);
  }

  internal class VFXExpressionObjectValueContainerDesc<T>
      : VFXExpressionValueContainerDesc {
  ...
  [UsedByNativeCode]
  [NativeHeader(
      "Modules/VFXEditor/Public/ScriptBindings/VisualEffectResourceBindings.h")]
  ...
  internal class VisualEffectResource : UnityObject {
    public VisualEffectResource() { CreateVisualEffectResource(this); }

    public const string Extension = ".vfx";
    extern private static void
    CreateVisualEffectResource([ Writable ] VisualEffectResource resource);
    extern public void ClearRuntimeData();

    private static VFXExpressionValuesSheetInternal
    CreateValueSheet(VFXExpressionValueContainerDesc[] values) {
      ...
      foreach (var group in values.GroupBy(o => o.GetType())) {
        if (group.Key == typeof(VFXExpressionValueContainerDesc<int>)) {
          ...
          internalSheet.intValuesExpressions =
              v.Select(o => o.expressionIndex).ToArray();
        } else if (group.Key ==
                   typeof(VFXExpressionValueContainerDesc<UInt32>)) {
          var v =
              group.Cast<VFXExpressionValueContainerDesc<UInt32>>().ToArray();
          internalSheet.uintValues = v.Select(o => o.value).ToArray();
          internalSheet.uintValuesExpressions =
              v.Select(o => o.expressionIndex).ToArray();
        } else if (group.Key ==
                   typeof(VFXExpressionValueContainerDesc<float>)) {
          var v =
              group.Cast<VFXExpressionValueContainerDesc<float>>().ToArray();
          internalSheet.floatValues = v.Select(o => o.value).ToArray();
          internalSheet.floatValuesExpressions =
              v.Select(o => o.expressionIndex).ToArray();
        } else if (group.Key ==
                   typeof(VFXExpressionValueContainerDesc<Vector2>)) {
          var v =
              group.Cast<VFXExpressionValueContainerDesc<Vector2>>().ToArray();
          internalSheet.vector2Values = v.Select(o => o.value).ToArray();
          internalSheet.vector2ValuesExpressions =
              v.Select(o => o.expressionIndex).ToArray();
        } else if (group.Key ==
                   typeof(VFXExpressionValueContainerDesc<Vector3>)) {
          var v =
              group.Cast<VFXExpressionValueContainerDesc<Vector3>>().ToArray();
          internalSheet.vector3Values = v.Select(o => o.value).ToArray();
          internalSheet.vector3ValuesExpressions =
              v.Select(o => o.expressionIndex).ToArray();
        } else if (group.Key ==
                   typeof(VFXExpressionValueContainerDesc<Vector4>)) {
          var v =
              group.Cast<VFXExpressionValueContainerDesc<Vector4>>().ToArray();
          internalSheet.vector4Values = v.Select(o => o.value).ToArray();
          internalSheet.vector4ValuesExpressions =
              v.Select(o => o.expressionIndex).ToArray();
        } else if (group.Key ==
                   typeof(VFXExpressionValueContainerDesc<Matrix4x4>)) {
          var v = group.Cast<VFXExpressionValueContainerDesc<Matrix4x4>>()
                      .ToArray();
          internalSheet.matrix4x4Values = v.Select(o => o.value).ToArray();
          internalSheet.matrix4x4ValuesExpressions =
              v.Select(o => o.expressionIndex).ToArray();
        } else if (group.Key ==
                   typeof(VFXExpressionObjectValueContainerDesc<Texture>)) {
          var v = group.Cast<VFXExpressionObjectValueContainerDesc<Texture>>()
                      .ToArray();
          internalSheet.textureValues = v.Select(o => o.instanceID).ToArray();
          internalSheet.textureValuesExpressions =
              v.Select(o => o.expressionIndex).ToArray();
        } else if (group.Key ==
                   typeof(VFXExpressionObjectValueContainerDesc<Mesh>)) {
          var v = group.Cast<VFXExpressionObjectValueContainerDesc<Mesh>>()
                      .ToArray();
          for (int i = 0; i < v.Length; ++i) {
          }
          internalSheet.meshValues = v.Select(o => o.instanceID).ToArray();
          internalSheet.meshValuesExpressions =
              v.Select(o => o.expressionIndex).ToArray();
        } else if (group.Key ==
                   typeof(VFXExpressionValueContainerDesc<Gradient>)) {
          var v =
              group.Cast<VFXExpressionValueContainerDesc<Gradient>>().ToArray();
          internalSheet.gradientValues = v.Select(o => o.value).ToArray();
          internalSheet.gradientValuesExpressions =
              v.Select(o => o.expressionIndex).ToArray();
        } else if (group.Key ==
                   typeof(VFXExpressionValueContainerDesc<AnimationCurve>)) {
          var v = group.Cast<VFXExpressionValueContainerDesc<AnimationCurve>>()
                      .ToArray();
          internalSheet.animationCurveValues = v.Select(o => o.value).ToArray();
          internalSheet.animationCurveValuesExpressions =
              v.Select(o => o.expressionIndex).ToArray();
          ...
          internalSheet.boolValuesExpressions =
              v.Select(o => o.expressionIndex).ToArray();
        }
        // For backward compatibility, Obsoleted by compile on import PR
        else if (group.Key ==
                 typeof(VFXExpressionValueContainerDesc<Texture>)) {
          var v =
              group.Cast<VFXExpressionValueContainerDesc<Texture>>().ToArray();
          internalSheet.textureValues =
              v.Select(o => o.value != null ? o.value.GetInstanceID() : 0)
                  .ToArray();
          internalSheet.textureValuesExpressions =
              v.Select(o => o.expressionIndex).ToArray();
        }
        // For backward compatibility, Obsoleted by compile on import PR
          ...
          throw new Exception(
              "Unknown VFXExpressionValueContainerDesc type : " + group.Key);
        }
      }
    ...
    public extern VFXShaderSourceDesc[] shaderSources {
      get;
      set;
    }
    ...
    [FreeFunction(Name = "VisualEffectResourceBindings::GetShaderSourceName",
                  ThrowsException = true, HasExplicitThis = true)]
    public extern string GetShaderSourceName(int index);
    [FreeFunction(Name = "VisualEffectResourceBindings::GetShaderSource",
                  ThrowsException = true, HasExplicitThis = true)]
    public extern string GetShaderSource(int index);

    public extern bool compileInitialVariants {
      get;
      set;
    }
    ...
    public void
    SetRuntimeData(VFXExpressionSheet sheet, VFXEditorSystemDesc[] systemDesc,
                   VFXEventDesc[] eventDesc, VFXGPUBufferDesc[] bufferDesc,
      ...
      internalSheet.expressionsPerSpawnEventAttribute =
          sheet.expressionsPerSpawnEventAttribute;
      internalSheet.values = CreateValueSheet(sheet.values);
      internalSheet.exposed = sheet.exposed;

      SetRuntimeData(internalSheet, systemDesc, eventDesc, bufferDesc,
                     temporaryBufferDesc, cpuBufferDesc, shaderSourceDesc,
                     shadowCastingMode, motionVectorGenerationMode, version);
    }
    ...
    SetRuntimeData(VFXExpressionSheet sheet, VFXEditorSystemDesc[] systemDesc,
      ...
      SetRuntimeDataDeprecated(internalSheet, systemDesc, eventDesc, bufferDesc,
                               temporaryBufferDesc, cpuBufferDesc,
                               this.shaderSources, defaultVersion);
    }

    [NativeThrows] extern private void SetRuntimeData(
        VFXExpressionSheetInternal sheet, VFXEditorSystemDesc[] systemDesc,
        VFXEventDesc[] eventDesc, VFXGPUBufferDesc[] bufferDesc,
        VFXTemporaryGPUBufferDesc[] temporaryBufferDesc,
        VFXCPUBufferDesc[] cpuBufferDesc,
        VFXShaderSourceDesc[] shaderSourceDesc,
        ShadowCastingMode shadowCastingMode,
        MotionVectorGenerationMode motionVectorGenerationMode, uint version);

    // This version is for backward compatilibity
    [NativeThrows] extern private void SetRuntimeDataDeprecated(
        VFXExpressionSheetInternal sheet, VFXEditorSystemDesc[] systemDesc,
        VFXEventDesc[] eventDesc, VFXGPUBufferDesc[] bufferDesc,
        VFXTemporaryGPUBufferDesc[] temporaryBufferDesc,
        VFXCPUBufferDesc[] cpuBufferDesc,
        VFXShaderSourceDesc[] shaderSourceDesc, uint version);
    extern public VFXRendererSettings rendererSettings {
      get;
      set;
    }
    extern public VFXUpdateMode updateMode {
      get;
      set;
    }
    extern public float preWarmDeltaTime {
      get;
      set;
    }
    extern public uint preWarmStepCount {
      get;
      set;
    }
    extern public string initialEventName {
      [FreeFunction("VisualEffectResourceBindings::GetInitialEventName",
                    HasExplicitThis = true)] get;
      [FreeFunction("VisualEffectResourceBindings::SetInitialEventName",
                    HasExplicitThis = true)] set;
    }
    extern public VFXCullingFlags cullingFlags {
      get;
      set;
    }
    ...
    public VisualEffectObject visualEffectObject {
      get {
        if (m_Object == null) {
          ...
          m_Object =
              AssetDatabase.LoadAssetAtPath<VisualEffectObject>(assetPath);
        }
        return m_Object;
      }
    }
    extern public ScriptableObject graph {
      get;
      set;
    }
