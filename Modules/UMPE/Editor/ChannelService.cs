...
internal static Dictionary<int, List<Action<int, byte[]> > > s_Handlers =
	new Dictionary<int, List<Action<int, byte[]> > >();

public static Action GetOrCreateChannel(string channelName,
...
public static Action RegisterMessageHandler(string channelName,
                                            Action<int, byte[]> handler) {
	...
	if (!s_Handlers.TryGetValue(channel.id, out handlers)) {
		handlers = new List<Action<int, byte[]> > {
			handler
		};
		s_Handlers.Add(channel.id, handlers);
	} else if (handlers.Contains(handler)) {
		throw new Exception(
			      "Cannot add already existing channel handler for channel: " +
			      channelName);
	} else {
		handlers.Add(handler);
	}

	return () => { UnregisterMessageHandler(channelName, handler); };
}

public static void UnregisterMessageHandler(string channelName,
                                            Action<int, byte[]> handler) {
	var channel = GetChannelFromName(channelName);
	if (ChannelInfo.invalidChannel == channel) {
		return;
...
private static void IncomingChannelServiceData(int channelId, int clientId,
                                               byte[] data) {
	List<Action<int, byte[]> > handlers = null;
...
public struct ChannelScope : IDisposable {
	...
	public ChannelScope(string channelName, Action<int, byte[]> handler,
