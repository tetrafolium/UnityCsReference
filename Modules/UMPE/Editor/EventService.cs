...
namespace UnityEditor.MPE {
  ...
  public static class EventService {
    ...
    internal static Dictionary<string, List<Func<string, object[], object>>>
        s_Events =
            new Dictionary<string, List<Func<string, object[], object>>>();
    internal static Dictionary<string, RequestData> s_Requests =
        new Dictionary<string, RequestData>();
    internal static ChannelClient m_Client;

    static EventService() { Start(); }
    ...
    RegisterEventHandler(string eventType, Action<string, object[]> handler) {
    ...
    public static Action
    RegisterEventHandler(string eventType,
      ...
      return () => { UnregisterEventHandler(eventType, handler); };
    }

    public static void
    UnregisterEventHandler(string eventType,
                           Func<string, object[], object> handler) {
      List<Func<string, object[], object>> handlers = null;
      if (s_Events.TryGetValue(eventType, out handlers)) {
    ...
    public static bool isConnected =>
        m_Client != null && m_Client.IsConnected();

    public static void Emit(string eventType, object args = null,
                            int targetId = -1,
                            EventDataSerialization eventDataSerialization =
                                EventDataSerialization.JsonUtility) {
      if (args == null)
    ...
    public static void Emit(string eventType, object[] args, int targetId = -1,
                            EventDataSerialization eventDataSerialization =
                                EventDataSerialization.JsonUtility) {
      const bool notifyWildcard = true;
      var req = CreateRequest(k_EventMsg, eventType, targetId, -1, args,
                              eventDataSerialization);

      // TODO: do we want to ensure that all local listeners received json as
      // payload? This means we could recycle handlers... If so we need to
      // serialize/deserialize... ugly. real ugly...
    ...
    public static bool CancelRequest(string eventType, string message = null) {
      ...
      Reject(request, new OperationCanceledException(
                          message ?? $"Request {eventType} canceled"));
      return true;
    }

    public static void Request(string eventType,
                               Action<Exception, object[]> promiseHandler,
                               object args = null,
                               long timeoutInMs = k_RequestDefaultTimeout,
                               EventDataSerialization eventDataSerialization =
                                   EventDataSerialization.JsonUtility) {
      if (args == null)
        Request(eventType, promiseHandler, null, timeoutInMs,
                eventDataSerialization);
      else
        Request(eventType, promiseHandler, new object[]{args}, timeoutInMs,
                eventDataSerialization);
    }
    ...
    public static void Request(string eventType,
      ...
      request =
          new RequestData{eventType = eventType,
                          promises = new List<Action<Exception, object[]>>(1),
      ...
      if (HasHandlers(eventType)) {
        ...
        var msg = CreateRequest(k_RequestMsg, eventType, -1, requestId, args,
                                eventDataSerialization);
    ...
    DeserializeEvent(string eventMsg, out RequestMessage deserializedMessage) {
      ...
      deserializedMessage.reqType = msg ["req"]
                                        .ToString();
      deserializedMessage.eventType = msg ["type"]
                                          .ToString();
      ...
      if (deserializedMessage.senderId ==
          m_Client.GetChannelClientInfo().connectionId) {
        return false;
      }
      ...
      if (msg.TryGetValue("dataInfos", out dataInfos))
        deserializedMessage.eventDataSerialization =
            EventDataSerialization.JsonUtility;
    ...
    private static void IncomingEvent(string eventMsg) {
      ...
      case k_RequestMsg: // Receiver
        ...
        if (HasHandlers(msg.eventType)) {
          var response =
              CreateRequestMsg(k_RequestAcknowledgeMsg, msg.eventType,
                               msg.senderId, msg.requestId, null, null);
          m_Client.Send(response);
        }
        break;
      case k_RequestAcknowledgeMsg: // Request emitter
        var pendingRequest =
            GetPendingRequest(msg.eventType, msg.requestId.Value);
        if (pendingRequest != null) {
          // A client is able to fulfill the request: proceed with request
          // execution:
          pendingRequest.isAcknowledged = true;
          pendingRequest.offerStartTime = Stopwatch.GetTimestamp();

          var message = CreateRequestMsgWithDataString(
              k_RequestExecuteMsg, msg.eventType, msg.senderId, msg.requestId,
              pendingRequest.data, pendingRequest.dataInfos);
      ...
      case k_RequestExecuteMsg: // Request receiver
      {
        ...
        var response =
            CreateRequest(k_RequestResultMsg, msg.eventType, msg.senderId,
                          msg.requestId, results, msg.eventDataSerialization);
        m_Client.Send(Json.Serialize(response));
        break;
      }
      case k_RequestResultMsg: // Request emitter
        var pendingRequestAwaitingResult =
            GetPendingRequest(msg.eventType, msg.requestId.Value);
        if (pendingRequestAwaitingResult != null) {
          var timeForSuccess =
              new TimeSpan(Stopwatch.GetTimestamp() -
                           pendingRequestAwaitingResult.offerStartTime)
                  .TotalMilliseconds;
          Console.WriteLine(
              $"[UMPE] Request {msg.eventType} successful in {timeForSuccess} ms");
    ...
    private static RequestData GetPendingRequest(string eventType,
                                                 int requestId) {
    ...
    private static bool HasHandlers(string eventType) {
      ...
      return s_Events.TryGetValue(eventType, out handlers) &&
             handlers.Count > 0;
    }
    ...
    public static void Tick() {
      ...
      if (s_Requests.Count > 0) {
        ...
        foreach (var request in pendingRequests) {
          var elapsedTime =
              new TimeSpan(now - request.offerStartTime).TotalMilliseconds;
          ...
          if (elapsedTime > request.timeoutInMs) {
            ...
            Reject(
                ...
                new TimeoutException(
                    $"Request timeout for {eventType} ({elapsedTime} > {request.timeoutInMs})"));
          }
        }
      }
    }

    private static object[] NotifyLocalListeners(string eventType,
                                                 object[] data,
                                                 bool notifyWildcard) {
      List<Func<string, object[], object>> handlers = null;
      ...
      if (s_Events.TryGetValue(eventType, out handlers)) {
        try {
          result =
              handlers.Select(handler => handler(eventType, data)).ToArray();
        } catch (Exception e) {
          result = new object[]{e};
          Console.WriteLine(e);
        }
    ...
    {
      var dataInfos = CreateDataInfosAndFormatDataForSerialization(
          args, eventDataSerialization);

      return CreateRequest(msgType, eventType, targetId, requestId, args,
                           dataInfos);
    }
    ...
    internal static string CreateRequestMsgWithDataString(string msgType, string eventType, int targetId, int? requestId, string args, object[] dataInfos)
    {
      ...
      var message = CreateRequestMsg(msgType, eventType, targetId, requestId,
                                     dataSearchString, dataInfos);
    ...
    {
      var request = CreateRequest(msgType, eventType, targetId, requestId, args,
                                  dataInfos);
    ...
    internal static object[] CreateDataInfosAndFormatDataForSerialization(
        object[] args, EventDataSerialization eventDataSerialization) {
      if (eventDataSerialization == EventDataSerialization.StandardJson)
        return null;

      // if the data is null we still send an empty dataInfo to know it uses
      // JsonUtility (useful on a request to keep that info when sending answer)
      if (args == null)
        return new object[]{};
      ...
      for (int i = 0; i < args.Length; i++) {
        ...
        if (!string.IsNullOrEmpty(dataString) && dataString != "{}" &&
          ...
          dataInfos.Add(new object[2]{i, args [i]
                                             .GetType()
    ...
    internal static object[] GetDataArray(object dataDeserialized,
                                          object dataSerializationInfos) {
      if (dataDeserialized != null) {
        object[] arrayDeserialized =
            dataDeserialized is List<object>
                ? (dataDeserialized as List<object>).ToArray()
                : new[]{dataDeserialized};
        object[] arraySerializationInfos =
            dataSerializationInfos is List<object>
                ? (dataSerializationInfos as List<object>).ToArray()
                : null;
    ...
    private static object[] GetDataArray(object[] arrayDeserialized,
                                         object[] dataSerializationInfos) {
      ...
      List<object> currentDataInfos =
          (List<object>) dataSerializationInfos[dataSerializationInfosIndex];
      for (int i = 0; i < arrayDeserialized.Length; ++i) {
        // Use JsonUtility
        if ((long) currentDataInfos[0] == i) {
          var typeName = (string) currentDataInfos[1];
          resultData[i] = JsonUtility.FromJson((string) arrayDeserialized[i],
                                               Type.GetType(typeName));
          dataSerializationInfosIndex++;
          if (dataSerializationInfosIndex < dataSerializationInfos.Length)
            currentDataInfos = (List<object>)
                dataSerializationInfos[dataSerializationInfosIndex];
        } else
