...
namespace UnityEditor.MPE {
  enum DataServiceEvent { AUTO_REFRESH }
  ...
  static class DataService {
    ...
    private class AssetEvents : AssetPostprocessor {
      ...
      OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets,
                             string[] movedAssets,
                             string[] movedFromAssetPaths) {
        if (!s_ImportRefreshEnabled)
          return;

        s_ImportedAssets = s_ImportedAssets.Concat(importedAssets)
    ...
    private static void EmitRefresh() {
      ...
      EventService.Emit(nameof(DataServiceEvent.AUTO_REFRESH),
                        s_ImportedAssets);
    ...
    private static void InitializeSlave() {
      EventService.RegisterEventHandler(
          nameof(DataServiceEvent.AUTO_REFRESH), (eventType, data) => {
            string[] paths = data.Cast<string>().ToArray();
            Console.WriteLine(
                $"Slave need to refresh the following assets: {String.Join(",
                ", paths)}");
