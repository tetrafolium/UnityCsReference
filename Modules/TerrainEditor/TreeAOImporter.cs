...
private static int PermuteCuboid(Vector3[] dirs, int offset, float x, float y,
                                 float z) {
...
public static void CalcSoftOcclusion(Mesh mesh) {
	...
	for (int i = 0; i < directions.Length; i++) {
		weights[i] =
			new Vector4(GetWeight(1, directions[i]), GetWeight(2, directions[i]),
			            GetWeight(3, directions[i]), GetWeight(0, directions[i]));
	}
	...
	for (int i = 0; i < verts.Length; i += 1) {
		...
		for (int j = 0; j < directions.Length; j++) {
			float occ = CountIntersections(
				v, go.transform.TransformDirection(directions[j]), 3);
...
static int CountIntersections(Vector3 v, Vector3 dist, float length) {
	...
	if (!kDebug) {
		return Physics.RaycastAll(v, dist, length, 1 << kWorkLayer).Length +
		       Physics
		       .RaycastAll(v + dist * length, -dist, length, 1 << kWorkLayer)
		       .Length;
	}
	...
	hits =
		Physics.RaycastAll(v + dist * length, -dist, length, 1 << kWorkLayer);
	if (hits.Length > 0) {
