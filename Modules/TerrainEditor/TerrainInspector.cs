...
namespace Experimental.TerrainAPI {
  public class TerrainToolShortcutContext : IShortcutToolContext {
    ...
    bool IShortcutToolContext.active {
      get {
        return !(TerrainInspector.s_activeTerrainInspector != 0 &&
                 TerrainInspector.s_activeTerrainInspector !=
                     terrainEditor.GetInstanceID());
      }
    }
...
internal class TerrainInspector : Editor {
  class Styles {
    ...
    public GUIStyle largeSquare = new GUIStyle("Button"){fixedHeight = 22};
    ...
    public readonly GUIContent[] toolIcons = {
        EditorGUIUtility.TrIconContent("TerrainInspector.TerrainToolAdd",
                                       "Create Neighbor Terrains"),
        EditorGUIUtility.TrIconContent("TerrainInspector.TerrainToolSplat",
                                       "Paint Terrain"),
        EditorGUIUtility.TrIconContent("TerrainInspector.TerrainToolTrees",
                                       "Paint Trees"),
        EditorGUIUtility.TrIconContent("TerrainInspector.TerrainToolPlants",
                                       "Paint Details"),
        EditorGUIUtility.TrIconContent("TerrainInspector.TerrainToolSettings",
                                       "Terrain Settings")};
    ...
    public readonly GUIContent[] toolNames = {
        ...
        EditorGUIUtility.TrTextContent("Paint Terrain",
    ...
    public readonly GUIContent brushSize = EditorGUIUtility.TrTextContent(
        "Brush Size", "Size of the brush used to paint.");
    public readonly GUIContent opacity = EditorGUIUtility.TrTextContent(
        "Opacity", "Strength of the applied effect.");
    public readonly GUIContent settings =
        EditorGUIUtility.TrTextContent("Settings");
    public readonly GUIContent mismatchedTerrainData =
        EditorGUIUtility.TextContentWithIcon(
    ...
    public readonly GUIContent assign =
        EditorGUIUtility.TrTextContent("Assign");
    public readonly GUIContent duplicateTab = EditorGUIUtility.TrTextContent(
        "This inspector tab is not the active Terrain inspector, paint functionality disabled.");
    public readonly GUIContent makeMeActive =
        EditorGUIUtility.TrTextContent("Activate this inspector");
    public readonly GUIContent gles2NotSupported =
        EditorGUIUtility.TrTextContentWithIcon(
            "Terrain editting is not supported in GLES2.", MessageType.Info);

    // Trees
    public readonly GUIContent trees = EditorGUIUtility.TrTextContent("Trees");
    public readonly GUIContent editTrees = EditorGUIUtility.TrTextContent(
        "Edit Trees...", "Add/remove tree types.");
    public readonly GUIContent treeDensity = EditorGUIUtility.TrTextContent(
        "Tree Density", "How dense trees are you painting");
    public readonly GUIContent treeHeight = EditorGUIUtility.TrTextContent(
        "Tree Height", "Height of the planted trees");
    public readonly GUIContent treeHeightRandomLabel =
        EditorGUIUtility.TrTextContent(
            "Random?", "Enable random variation in tree height (variation)");
    public readonly GUIContent treeHeightRandomToggle =
        EditorGUIUtility.TrTextContent(
            "", "Enable random variation in tree height (variation)");
    public readonly GUIContent lockWidth = EditorGUIUtility.TrTextContent(
        "Lock Width to Height", "Let the tree width be the same with height");
    public readonly GUIContent treeWidth = EditorGUIUtility.TrTextContent(
        "Tree Width", "Width of the planted trees");
    public readonly GUIContent treeWidthRandomLabel =
        EditorGUIUtility.TrTextContent(
            "Random?", "Enable random variation in tree width (variation)");
    public readonly GUIContent treeWidthRandomToggle =
        EditorGUIUtility.TrTextContent(
            "", "Enable random variation in tree width (variation)");
    public readonly GUIContent treeColorVar = EditorGUIUtility.TrTextContent(
        "Color Variation",
    ...
    public readonly GUIContent treeRotation = EditorGUIUtility.TrTextContent(
        "Random Tree Rotation",
        "Randomize tree rotation. This only works when the tree has an LOD group.");
    public readonly GUIContent treeRotationDisabled = EditorGUIUtility.TrTextContent(
        "The selected tree does not have an LOD group, so it will use the default impostor system and will not support rotation.");
    public readonly GUIContent treeHasChildRenderers = EditorGUIUtility.TrTextContent(
        "The selected tree does not have an LOD group, but has a hierarchy of MeshRenderers, only MeshRenderer on root GameObject in the trees hierarchy will be used. Use a tree with LOD group if you want a tree with hierarchy of MeshRenderers.");
    public readonly GUIContent massPlaceTrees = EditorGUIUtility.TrTextContent(
        "Mass Place Trees",
    ...
    public readonly GUIContent treeContributeGI = EditorGUIUtility.TrTextContent(
        "Tree Contribute Global Illumination",
        "The state of the Contribute GI flag for the tree prefab root GameObject. The flag can be changed on the prefab. When disabled, this tree will not be visible to the lightmapper. When enabled, any child GameObjects which also have the static flag enabled, will be present in lightmap calculations. Regardless of the value of the flag, each tree instance receives its own light probe and no lightmap texels.");

    // Details
    public readonly GUIContent details =
        EditorGUIUtility.TrTextContent("Details");
    public readonly GUIContent editDetails = EditorGUIUtility.TrTextContent(
        "Edit Details...", "Add/remove detail meshes");
    public readonly GUIContent detailTargetStrength =
        EditorGUIUtility.TrTextContent("Target Strength", "Target amount");

    // Heightmaps
    public readonly GUIContent textures =
        EditorGUIUtility.TrTextContent("Texture Resolutions (On Terrain Data)");
    public readonly GUIContent requireResampling =
        EditorGUIUtility.TrTextContent("Require resampling on change");
    public readonly GUIContent importRaw = EditorGUIUtility.TrTextContent(
        "Import Raw...",
        "The Import Raw button allows you to set the terrain's heightmap from an image file in the RAW grayscale format. RAW format can be generated by third party terrain editing tools (such as Bryce) and can also be opened, edited and saved by Photoshop. This allows for sophisticated generation and editing of terrains outside Unity.");
    public readonly GUIContent exportRaw = EditorGUIUtility.TrTextContent(
        "Export Raw...",
        "The Export Raw button allows you to save the terrain's heightmap to an image file in the RAW grayscale format. RAW format can be generated by third party terrain editing tools (such as Bryce) and can also be opened, edited and saved by Photoshop. This allows for sophisticated generation and editing of terrains outside Unity.");

    public readonly GUIContent bakeLightProbesForTrees =
        EditorGUIUtility.TrTextContent(
    ...
    public readonly GUIContent deringLightProbesForTrees =
        EditorGUIUtility.TrTextContent(
    ...
    public readonly GUIContent refresh = EditorGUIUtility.TrTextContent(
        "Refresh",
    ...
    // Settings
    public readonly GUIContent basicTerrain =
        EditorGUIUtility.TrTextContent("Basic Terrain");
    public readonly GUIContent groupingID = EditorGUIUtility.TrTextContent(
        "Grouping ID", "Grouping ID for auto connection");
    public readonly GUIContent allowAutoConnect = EditorGUIUtility.TrTextContent(
        "Auto Connect",
        "Allow the current terrain tile to automatically connect to neighboring tiles sharing the same grouping ID.");
    public readonly GUIContent attemptReconnect =
        EditorGUIUtility.TrTextContent(
            "Reconnect", "Will attempt to re-run auto connection");
    public readonly GUIContent drawTerrain = EditorGUIUtility.TrTextContent(
        "Draw", "Toggle the rendering of terrain");
    public readonly GUIContent drawInstancedTerrain =
        EditorGUIUtility.TrTextContent("Draw Instanced",
                                       "Toggle terrain instancing rendering");
    public readonly GUIContent pixelError = EditorGUIUtility.TrTextContent(
        "Pixel Error",
    ...
    public readonly GUIContent baseMapDist = EditorGUIUtility.TrTextContent(
        "Base Map Dist.",
    ...
    public readonly GUIContent castShadows = EditorGUIUtility.TrTextContent(
        "Cast Shadows", "Does the terrain cast shadows?");
    public readonly GUIContent createMaterial = EditorGUIUtility.TrTextContent(
        "Create...",
        "Create a new Material asset to be used by the terrain by duplicating the current default Terrain material.");
    public readonly GUIContent reflectionProbes = EditorGUIUtility.TrTextContent(
        "Reflection Probes",
    ...
    public readonly GUIContent preserveTreePrototypeLayers =
        EditorGUIUtility.TextContent(
            "Preserve Tree Prototype Layers|Enable this option if you want your tree instances to take on the layer values of their prototype prefabs, rather than the terrain GameObject's layer.");
    public readonly GUIContent treeAndDetails =
        EditorGUIUtility.TrTextContent("Tree & Detail Objects");
    public readonly GUIContent drawTrees = EditorGUIUtility.TrTextContent(
        "Draw", "Should trees, grass and details be drawn?");
    public readonly GUIContent detailObjectDistance =
        EditorGUIUtility.TrTextContent(
            "Detail Distance",
            "The distance (from camera) beyond which details will be culled.");
    public readonly GUIContent detailObjectDensity = EditorGUIUtility.TrTextContent(
        "Detail Density",
        "The number of detail/grass objects in a given unit of area. The value can be set lower to reduce rendering overhead.");
    public readonly GUIContent treeDistance = EditorGUIUtility.TrTextContent(
        "Tree Distance",
    ...
    public readonly GUIContent treeBillboardDistance = EditorGUIUtility.TrTextContent(
        "Billboard Start",
    ...
    public readonly GUIContent treeCrossFadeLength = EditorGUIUtility.TrTextContent(
        "Fade Length",
    ...
    public readonly GUIContent treeMaximumFullLODCount =
        EditorGUIUtility.TrTextContent(
            "Max Mesh Trees",
    ...
    public readonly GUIContent grassWindSettings =
        EditorGUIUtility.TrTextContent(
            "Wind Settings for Grass (On Terrain Data)");
    public readonly GUIContent wavingGrassStrength =
        EditorGUIUtility.TrTextContent(
            "Speed", "The speed of the wind as it blows grass.");
    public readonly GUIContent wavingGrassSpeed = EditorGUIUtility.TrTextContent(
        "Size",
        "The size of the 'ripples' on grassy areas as the wind blows over them.");
    public readonly GUIContent wavingGrassAmount =
        EditorGUIUtility.TrTextContent(
            "Bending",
            "The degree to which grass objects are bent over by the wind.");
    public readonly GUIContent wavingGrassTint = EditorGUIUtility.TrTextContent(
        "Grass Tint", "Overall color tint applied to grass objects.");
    public readonly GUIContent meshResolution =
        EditorGUIUtility.TrTextContent("Mesh Resolution (On Terrain Data)");
    public readonly GUIContent detailResolutionWarning =
        EditorGUIUtility.TrTextContent(
            "You may reduce CPU draw call overhead by setting the detail resolution per patch as high as possible, relative to detail resolution.");
    public readonly GUIContent holesSettings =
        EditorGUIUtility.TrTextContent("Holes Settings (On Terrain Data)");
    public readonly GUIContent holesCompressionToggle =
        EditorGUIUtility.TrTextContent(
            "Compress Holes Texture",
            "If enabled, holes texture will be compressed at runtime if compression supported.");
    public readonly GUIContent detailShadersMissing =
        EditorGUIUtility.TrTextContent(
            "The current render pipeline does not have all Detail shaders");
    public readonly GUIContent detailShadersUnsupported =
        EditorGUIUtility.TrTextContent(
            "The current render pipeline does not support Detail shaders");

    public static readonly GUIContent renderingLayerMask =
        EditorGUIUtility.TrTextContent(
            "Rendering Layer Mask",
            "Mask that can be used with SRP DrawRenderers command to filter renderers outside of the normal layering system.");

    public static readonly GUIContent heightmapResolution =
        EditorGUIUtility.TrTextContent(
    ...
    public static readonly GUIContent alphamapResolution =
        EditorGUIUtility.TrTextContent(
            "Control Texture Resolution",
            "Resolution of the \"splatmap\" that controls the blending of the different terrain materials.");
    ...
    public static readonly GUIContent basemapResolution =
        EditorGUIUtility.TrTextContent(
  ...
  static float ScaledSliderWithRounding(GUIContent content,
                                        float valueInPercent, float minVal,
                                        float maxVal, float scale,
                                        float precision) {
    ...
    v = Mathf.Clamp(v, minVal,
                    maxVal); // this keeps the slider knob from disappearing
  ...
  static float PowerSlider(GUIContent content, float value, float minVal,
                           float maxVal, float power,
                           GUILayoutOption[] options = null) {
    value = Mathf.Clamp(value, minVal, maxVal);
    EditorGUI.BeginChangeCheck();
    float newValue = EditorGUILayout.PowerSlider(content, value, minVal, maxVal,
                                                 power, options);
    if (EditorGUI.EndChangeCheck()) {
      return newValue;
    }
  ...
  float kOldMaxBrushSize {
    get {
      ...
      return Mathf.Floor(Mathf.Min(m_Terrain.terrainData.size.x,
  ...
  void GetBrushSizeLimits(out float minBrushSize, out float maxBrushSize,
    ...
    if (textureResolutionPerTile <= 0) {
      ...
      if (m_Terrain && m_Terrain.terrainData) {
        terrainSize = Mathf.Min(m_Terrain.terrainData.size.x,
                                m_Terrain.terrainData.size.z);
      }

      TerrainPaintUtility.GetBrushWorldSizeLimits(out minBrushSize,
                                                  out maxBrushSize, terrainSize,
  ...
  const float kMinBrushStrength =
      (1.1F / ushort.MaxValue) / kHeightmapBrushScale;
  ...
  static OnPaintContext onPaintEditContext =
      new OnPaintContext(new RaycastHit(), null, Vector2.zero, 0.0f, 0.0f);
  static OnInspectorGUIContext onInspectorGUIEditContext =
      new OnInspectorGUIContext();
  static OnSceneGUIContext onSceneGUIEditContext =
      new OnSceneGUIContext(null, new RaycastHit(), null, 0.0f, 0.0f);
  ...
  // It's defined as the last inspector that had one of its terrain tools
  // selected. If a terrain inspector is the only one when created, it also
  // becomes active.
  static internal int s_activeTerrainInspector = 0;
  static internal TerrainInspector s_activeTerrainInspectorInstance = null;

  List<ReflectionProbeBlendInfo> m_BlendInfoList =
      new List<ReflectionProbeBlendInfo>();
  ...
  static void ChangeTool(ShortcutArguments args,
                         Action<TerrainInspector> action) {
    TerrainToolShortcutContext context =
        (TerrainToolShortcutContext) args.context;
    TerrainInspector editor = context.terrainEditor;
    editor.lastTextureResolutionPerTile =
        0; // reset texture resolution, new tool may use different resolution
    action(editor);
  ...
  public void SelectPaintTool(Type toolType) {
    if (toolType.IsAbstract ||
      ...
      Debug.LogError(
          "SelectPaintTool: tool must be a subclass of TerrainPaintTool");
      ...
      var instanceProperty = toolType.GetProperty(
  ...
  [Shortcut("Terrain/Previous Brush", typeof(TerrainToolShortcutContext),
            KeyCode.Comma)]
  ...
  [Shortcut("Terrain/Next Brush", typeof(TerrainToolShortcutContext),
            KeyCode.Period)]
  ...
  [Shortcut("Terrain/Previous Detail", typeof(TerrainToolShortcutContext),
            KeyCode.Comma, ShortcutModifiers.Shift)]
  ...
  [Shortcut("Terrain/Next Detail", typeof(TerrainToolShortcutContext),
            KeyCode.Period, ShortcutModifiers.Shift)]
  static void NextDetail(ShortcutArguments args) {
    ChangeTool(args, editor => editor.DetailDelta(1));
  }

  static void ApplyClutchKeys(ShortcutArguments args, TerrainInspector editor,
                              ref float forwardTime, ref float backwardTime,
                              ref bool slow) {
    if (args.stage == ShortcutStage.Begin) {
      // if the backward-key is already pressed -- we keeps that direction but
      // slow it
      ...
      forwardTime = 0.0f;
      // if backwards is still pressed, disable slow, but reset the timer
  ...
  [ClutchShortcut("Terrain/Grow Brush Size", typeof(TerrainToolShortcutContext),
                  KeyCode.RightBracket)]
  static void GrowBrushSize(ShortcutArguments args) {
    TerrainToolShortcutContext context =
        (TerrainToolShortcutContext) args.context;
    TerrainInspector editor = context.terrainEditor;
    ApplyClutchKeys(args, editor, ref editor.m_GrowBrushTime,
                    ref editor.m_ShrinkBrushTime, ref editor.m_GrowShrinkSlow);
  }

  [FormerlyPrefKeyAs("Terrain/Shrink Brush Size", "LeftBracket")]
  [ClutchShortcut("Terrain/Shrink Brush Size",
                  typeof(TerrainToolShortcutContext), KeyCode.LeftBracket)]
  static void ShrinkBrushSize(ShortcutArguments args) {
    TerrainToolShortcutContext context =
        (TerrainToolShortcutContext) args.context;
    TerrainInspector editor = context.terrainEditor;
    ApplyClutchKeys(args, editor, ref editor.m_ShrinkBrushTime,
                    ref editor.m_GrowBrushTime, ref editor.m_GrowShrinkSlow);
  }

  [FormerlyPrefKeyAs("Terrain/Increase Brush Opacity", "Plus")]
  [ClutchShortcut("Terrain/Increase Brush Opacity",
                  typeof(TerrainToolShortcutContext), KeyCode.Equals)]
  static void IncreaseBrushOpacity(ShortcutArguments args) {
    TerrainToolShortcutContext context =
        (TerrainToolShortcutContext) args.context;
    TerrainInspector editor = context.terrainEditor;
    ApplyClutchKeys(args, editor, ref editor.m_IncreaseOpacityTime,
                    ref editor.m_DecreaseOpacityTime, ref editor.m_OpacitySlow);
  }

  [FormerlyPrefKeyAs("Terrain/Decrease Brush Opacity", "Minus")]
  [ClutchShortcut("Terrain/Decrease Brush Opacity",
                  typeof(TerrainToolShortcutContext), KeyCode.Minus)]
  static void DecreaseBrushOpacity(ShortcutArguments args) {
    TerrainToolShortcutContext context =
        (TerrainToolShortcutContext) args.context;
    TerrainInspector editor = context.terrainEditor;
    ApplyClutchKeys(args, editor, ref editor.m_DecreaseOpacityTime,
                    ref editor.m_IncreaseOpacityTime, ref editor.m_OpacitySlow);
  }
  ...
  float SmoothMaxBlended(float x, float max, float hardness) {
    // approximates Mathf.Max(x, max) -- see
    // https://www.desmos.com/calculator/zdwwpe7xwu
    float smoothPoint = max * hardness;
    return Mathf.Min(x, smoothPoint) +
           SmoothMax(Mathf.Max(x - smoothPoint, 0.0f), max - smoothPoint);
  }

  void BrushSizeHotkeyManipulation(ref float hotkeyTime, bool shrink,
                                   float deltaTime, float distance, bool slow) {
    if ((hotkeyTime > 0.0f) && (distance > 0.0f)) {
      ...
      float distanceScale =
          Mathf.Lerp(k_SlowDistanceScale, k_TargetDistanceScale, blend);
      if (slow)
        distanceScale = Mathf.Min(distanceScale, k_SlowDistanceScale);

      // Sqrt behaves great at large distances (speeds up adjust speed), but
      // gets super slow close up so we offset by a small bias to keep it in a
      // usable range
      float changePercentPerSecond =
          distanceScale * Mathf.Sqrt(distance + k_DistanceBias);
      ...
      float changeAdditive =
          k_AdditivePerSecond * deltaTime * changePercentPerSecond;

      // apply scale and offset, in the desired direction
      m_Size = shrink ?(m_Size - changeAdditive) /
                   changePercent : m_Size * changePercent +
               changeAdditive;
  ...
  void BrushOpacityHotkeyManipulation(ref float hotkeyTime, bool shrink,
                                      float deltaTime, bool slow) {
    if (hotkeyTime > 0.0f) {
      ...
      float blend = SmoothMaxBlended(hotkeyTime, 1.0f, 0.5f);
      float changeAmount =
          Mathf.Lerp(k_SlowOpacitySpeed, k_TargetOpacitySpeed, blend);
      if (slow)
      ...
      m_Strength =
          shrink ? m_Strength - changeAmount : m_Strength + changeAmount;
      m_Strength =
          Mathf.Clamp(m_Strength, kMinBrushStrength, kMaxBrushStrength);
  ...
  void ForceRepaintOnHotkeys() {
    if ((m_GrowBrushTime > 0.0f) || (m_ShrinkBrushTime > 0.0f) ||
        (m_IncreaseOpacityTime > 0.0f) || (m_DecreaseOpacityTime > 0.0f)) {
      Repaint();
      EditorApplication.SetSceneRepaintDirty();
    }
  }
  ...
  void HotkeyApply(float distance) {
    ...
    BrushSizeHotkeyManipulation(ref m_GrowBrushTime, false, deltaTime, distance,
                                m_GrowShrinkSlow);
    BrushSizeHotkeyManipulation(ref m_ShrinkBrushTime, true, deltaTime,
                                distance, m_GrowShrinkSlow);
    BrushOpacityHotkeyManipulation(ref m_IncreaseOpacityTime, false, deltaTime,
                                   m_OpacitySlow);
    BrushOpacityHotkeyManipulation(ref m_DecreaseOpacityTime, true, deltaTime,
                                   m_OpacitySlow);
  }
  ...
  void DetailDelta(int delta) {
    if (delta != 0) {
      ...
      case TerrainTool.PaintDetail:
        PaintDetailsTool.instance.selectedDetail =
            (int) Mathf.Repeat(PaintDetailsTool.instance.selectedDetail + delta,
                               m_Terrain.terrainData.detailPrototypes.Length);
        Event.current.Use();
        Repaint();
        break;
      case TerrainTool.PlaceTree:
        if (PaintTreesTool.instance.selectedTree >= 0)
          PaintTreesTool.instance.selectedTree =
              (int) Mathf.Repeat(PaintTreesTool.instance.selectedTree + delta,
                                 m_TreeContents.Length);
  ...
  void ResetPaintTools() {
    ...
    foreach (var klass in TypeCache.GetTypesDerivedFrom(
                 typeof(TerrainPaintTool<>))) {
      if (klass.IsAbstract || klass == typeof(PaintTreesTool) ||
          klass == typeof(PaintDetailsTool))
        continue;
      var instanceProperty = klass.GetProperty(
          "instance", BindingFlags.Public | BindingFlags.Static |
                          BindingFlags.FlattenHierarchy);
      var mi = instanceProperty.GetGetMethod();
      var tool = (ITerrainPaintTool) mi.Invoke(null, null);
      ...
      if (toolName.Equals("Create Neighbor Terrains")) {
        ...
        if (klass.Assembly
                .GetCustomAttributes(typeof(AssemblyIsEditorAssembly), false)
                .Length > 0)
          continue;
        else {
          m_CreateTool = tool;
  ...
  void LoadInspectorSettings() {
    ...
    PaintDetailsTool.instance.detailOpacity =
        EditorPrefs.GetFloat("TerrainDetailOpacity", 1.0f);
    PaintDetailsTool.instance.detailStrength =
        EditorPrefs.GetFloat("TerrainDetailStrength", 0.8f);

    int selected = EditorPrefs.GetInt("TerrainSelectedBrush", 0);
    PaintDetailsTool.instance.selectedDetail =
        EditorPrefs.GetInt("TerrainSelectedDetail", 0);

    m_ActivePaintToolIndex =
        EditorPrefs.GetInt("TerrainActivePaintToolIndex",
                           0); // TODO: this should be stored by name
    if (m_ActivePaintToolIndex > m_Tools.Length)
      m_ActivePaintToolIndex = 0;
  ...
  void SaveInspectorSettings() {
    EditorPrefs.SetInt("TerrainSelectedDetail",
                       PaintDetailsTool.instance.selectedDetail);
    EditorPrefs.SetInt("TerrainSelectedBrush", brushList.selectedIndex);

    EditorPrefs.SetFloat("TerrainDetailStrength",
                         PaintDetailsTool.instance.detailStrength);
    EditorPrefs.SetFloat("TerrainDetailOpacity",
                         PaintDetailsTool.instance.detailOpacity);
  ...
  static bool ShouldShowCreateMaterialButton(Material material) {
    return material == null ||
           GraphicsSettings.currentRenderPipeline != null &&
               material == GraphicsSettings.currentRenderPipeline
                               .defaultTerrainMaterial ||
           material == AssetDatabase.GetBuiltinExtraResource<Material>(
  ...
  public void OnEnable() {
    if (s_activeTerrainInspector == 0 ||
      ...
      foreach (var inspector in inspectors) {
        ...
        foreach (var editor in editors) {
          if (editor == this) {
            // Acquire active inspector ownership if there is no other inspector
            // active.
    ...
    m_ShowReflectionProbesGUI.value =
        terrain.reflectionProbeUsage != ReflectionProbeUsage.Off;
    m_ShowCreateMaterialButton =
        ShouldShowCreateMaterialButton(terrain.materialTemplate);
    ...
    m_Lighting.showLightingSettings =
        new SavedBool($"{target.GetType()}.ShowLightingSettings", true);
    m_Lighting.showLightmapSettings =
        new SavedBool($"{target.GetType()}.ShowLightmapSettings", true);
    m_Lighting.showBakedLightmap =
        new SavedBool($"{target.GetType()}.ShowBakedLightmapSettings", false);
    m_Lighting.showRealtimeLightmap = new SavedBool(
        $"{target.GetType()}.ShowRealtimeLightmapSettings", false);

    m_TerrainToolContext = new TerrainToolShortcutContext(this);
    ShortcutIntegration.instance.contextManager.RegisterToolContext(
        m_TerrainToolContext);
  ...
  public void OnDisable() {
    ShortcutIntegration.instance.contextManager.DeregisterToolContext(
        m_TerrainToolContext);
  ...
  SavedInt m_SelectedTool =
      new SavedInt("TerrainSelectedTool", (int) TerrainTool.Paint);
  TerrainToolShortcutContext m_TerrainToolContext;

  TerrainTool selectedTool {
    get {
      if (Tools.current == Tool.None &&
          GetInstanceID() == s_activeTerrainInspector)
  ...
  // this is a bunch of tracking to ensure we don't mess up the tool mode
  // callbacks
  ...
  // Ideally we would be notified when the active tool changes, but I can see no
  // way to do that So instead we will call this function everywhere, which
  // checks for it changing and does the proper notification callbacks
  ...
  public void MenuButton(GUIContent title, string menuName, int userData) {
    GUIContent t =
        new GUIContent(title.text, styles.settingsIcon, title.tooltip);
    ...
    if (GUI.Button(r, t, styles.largeSquare)) {
      ...
      EditorUtility.DisplayPopupMenu(new Rect(r.x, r.y, 0, 0), menuName,
                                     context);
    }
  }

  public static int AspectSelectionGrid(int selected, Texture[] textures,
                                        int approxSize, GUIStyle style,
    ...
    if (textures.Length != 0) {
      ...
      retval =
          GUI.SelectionGrid(r, System.Math.Min(selected, textures.Length - 1),
                            textures, (int) columns, style);
    } else {
  ...
  static Rect GetBrushAspectRect(int elementCount, int approxSize,
                                 int extraLineHeight, out int xCount) {
    xCount =
        (int) Mathf.Ceil((EditorGUIUtility.currentViewWidth - 20) / approxSize);
  ...
  public static int
  AspectSelectionGridImageAndText(int selected, GUIContent[] textures,
                                  int approxSize, GUIStyle style,
                                  string emptyString, out bool doubleClick) {
    EditorGUILayout.BeginVertical(GUIContent.none, EditorStyles.helpBox,
    ...
    if (textures.Length != 0) {
      int xCount = 0;
      Rect rect =
          GetBrushAspectRect(textures.Length, approxSize, 12, out xCount);
      ...
      retval = GUI.SelectionGrid(rect,
                                 System.Math.Min(selected, textures.Length - 1),
                                 textures, xCount, style);
    } else {
  ...
  public void ShowTrees() {
    ...
    bool doubleClick;
    PaintTreesTool.instance.selectedTree = AspectSelectionGridImageAndText(
        PaintTreesTool.instance.selectedTree, m_TreeContents, 64,
        styles.gridListText, "No trees defined", out doubleClick);
    ...
    if (doubleClick) {
      TerrainTreeContextMenus.EditTree(
          new MenuCommand(m_Terrain, PaintTreesTool.instance.selectedTree));
      GUIUtility.ExitGUI();
    }

    GUILayout.BeginHorizontal();
    using(new EditorGUI.DisabledScope(PaintTreesTool.instance.selectedTree ==
                                      PaintTreesTool.kInvalidTree)) {
      if (GUILayout.Button(styles.massPlaceTrees)) {
        TerrainMenus.MassPlaceTrees();
      }
    }
    GUILayout.FlexibleSpace();
    MenuButton(styles.editTrees, "CONTEXT/TerrainEngineTrees",
               PaintTreesTool.instance.selectedTree);
    ...
    // Placement distance
    PaintTreesTool.instance.brushSize = PowerSlider(
        styles.brushSize, PaintTreesTool.instance.brushSize, 1,
        Mathf.Min(m_Terrain.terrainData.size.x, m_Terrain.terrainData.size.z),
        4.0f);
    float oldDens = (3.3f - PaintTreesTool.instance.spacing) / 3f;
    float newDens = ScaledSliderWithRounding(styles.treeDensity, oldDens, 0.1f,
                                             1.0f, 100.0f, 1.0f);
    ...
    GUILayout.BeginHorizontal();
    GUILayout.Label(styles.treeHeight,
                    GUILayout.Width(EditorGUIUtility.labelWidth - 6));
    GUILayout.Label(styles.treeHeightRandomLabel, GUILayout.ExpandWidth(false));
    PaintTreesTool.instance.allowHeightVar = GUILayout.Toggle(
        PaintTreesTool.instance.allowHeightVar, styles.treeHeightRandomToggle,
        GUILayout.ExpandWidth(false));
    if (PaintTreesTool.instance.allowHeightVar) {
      EditorGUI.BeginChangeCheck();
      float min = PaintTreesTool.instance.treeHeight *
                  (1.0f - PaintTreesTool.instance.treeHeightVariation);
      float max = PaintTreesTool.instance.treeHeight *
                  (1.0f + PaintTreesTool.instance.treeHeightVariation);
      ...
      PaintTreesTool.instance.treeHeight = EditorGUILayout.Slider(
          PaintTreesTool.instance.treeHeight, 0.01f, 2.0f);
    ...
    PaintTreesTool.instance.lockWidthToHeight = EditorGUILayout.Toggle(
        styles.lockWidth, PaintTreesTool.instance.lockWidthToHeight);
    if (PaintTreesTool.instance.lockWidthToHeight) {
      PaintTreesTool.instance.treeWidth = PaintTreesTool.instance.treeHeight;
      PaintTreesTool.instance.treeWidthVariation =
          PaintTreesTool.instance.treeHeightVariation;
      PaintTreesTool.instance.allowWidthVar =
          PaintTreesTool.instance.allowHeightVar;
    }

    GUILayout.Space(5);

    using(new EditorGUI.DisabledScope(
        PaintTreesTool.instance.lockWidthToHeight)) {
      GUILayout.BeginHorizontal();
      GUILayout.Label(styles.treeWidth,
                      GUILayout.Width(EditorGUIUtility.labelWidth - 6));
      GUILayout.Label(styles.treeWidthRandomLabel,
                      GUILayout.ExpandWidth(false));
      PaintTreesTool.instance.allowWidthVar = GUILayout.Toggle(
          PaintTreesTool.instance.allowWidthVar, styles.treeWidthRandomToggle,
          GUILayout.ExpandWidth(false));
      if (PaintTreesTool.instance.allowWidthVar) {
        EditorGUI.BeginChangeCheck();
        float min = PaintTreesTool.instance.treeWidth *
                    (1.0f - PaintTreesTool.instance.treeWidthVariation);
        float max = PaintTreesTool.instance.treeWidth *
                    (1.0f + PaintTreesTool.instance.treeWidthVariation);
        EditorGUILayout.MinMaxSlider(ref min, ref max, 0.01f, 2.0f);
        if (EditorGUI.EndChangeCheck()) {
        ...
        PaintTreesTool.instance.treeWidth = EditorGUILayout.Slider(
            PaintTreesTool.instance.treeWidth, 0.01f, 2.0f);
    ...
    GameObject prefab =
        m_Terrain.terrainData
            .treePrototypes[PaintTreesTool.instance.selectedTree]
            .m_Prefab;

    bool randomRotationEnabled =
        TerrainEditorUtility.IsLODTreePrototype(prefab);
    using(new EditorGUI.DisabledScope(!randomRotationEnabled)) {
      PaintTreesTool.instance.randomRotation = EditorGUILayout.Toggle(
          styles.treeRotation, PaintTreesTool.instance.randomRotation);
    }
    if (!randomRotationEnabled)
      EditorGUILayout.HelpBox(styles.treeRotationDisabled.text,
                              MessageType.Info);
    ...
    if (prefab != null) {
      ...
      if (meshRenderers != null && meshRenderers.Length > 0) {
        if (meshRenderers.Length > 1 || !prefab.GetComponent<MeshRenderer>())
          EditorGUILayout.HelpBox(styles.treeHasChildRenderers.text,
                                  MessageType.Warning);
      }
    }

    // TODO: we should check if the shaders assigned to this 'tree' support
    // _TreeInstanceColor or not..  complicated check though
    PaintTreesTool.instance.treeColorAdjustment = EditorGUILayout.Slider(
        styles.treeColorVar, PaintTreesTool.instance.treeColorAdjustment, 0, 1);

    if (prefab != null) {
      StaticEditorFlags staticEditorFlags =
          GameObjectUtility.GetStaticEditorFlags(prefab);
      bool contributeGI =
          (staticEditorFlags & StaticEditorFlags.ContributeGI) != 0;
      using(new EditorGUI.DisabledScope(
          true)) // Always disabled, because we don't want to edit the prefab.
          contributeGI =
              EditorGUILayout.Toggle(styles.treeContributeGI, contributeGI);
    }
  }
  ...
  public void ShowDetailStats() {
    ...
    EditorGUILayout.HelpBox(styles.detailResolutionWarning.text,
                            MessageType.Warning);

    int maxMeshes = m_Terrain.terrainData.detailPatchCount *
                    m_Terrain.terrainData.detailPatchCount;
    EditorGUILayout.LabelField("Detail patches currently allocated: " +
                               maxMeshes);

    int maxDetails = maxMeshes * PaintDetailsUtils.GetMaxDetailInstances(
                                     m_Terrain.terrainData);
  ...
  public void ShowDetails() {
    ...
    if (renderPipelineAsset != null) {
      if (SupportedRenderingFeatures.active.terrainDetailUnsupported) {
        EditorGUILayout.HelpBox(styles.detailShadersUnsupported.text,
                                MessageType.Error);
                 ...
                 (renderPipelineAsset.terrainDetailGrassBillboardShader ==
                  null)) {
        EditorGUILayout.HelpBox(styles.detailShadersMissing.text,
                                MessageType.Error);
      }
    }
    ...
    bool doubleClick;
    PaintDetailsTool.instance.selectedDetail = AspectSelectionGridImageAndText(
        PaintDetailsTool.instance.selectedDetail, m_DetailContents, 64,
        styles.gridListText, "No Detail Objects defined", out doubleClick);
    if (doubleClick) {
      TerrainDetailContextMenus.EditDetail(
          new MenuCommand(m_Terrain, PaintDetailsTool.instance.selectedDetail));
      GUIUtility.ExitGUI();
    }
    ...
    GUILayout.BeginHorizontal();
    GUILayout.FlexibleSpace();
    MenuButton(styles.editDetails, "CONTEXT/TerrainEngineDetails",
               PaintDetailsTool.instance.selectedDetail);
    ShowRefreshPrototypes();
    GUILayout.EndHorizontal();
    ...
    PaintDetailsTool.instance.detailOpacity = EditorGUILayout.Slider(
        styles.opacity, PaintDetailsTool.instance.detailOpacity, 0, 1);

    // Strength
    PaintDetailsTool.instance.detailStrength =
        EditorGUILayout.Slider(styles.detailTargetStrength,
                               PaintDetailsTool.instance.detailStrength, 0, 1);
  }
  ...
  private void MarkTerrainDataDirty() {
    // In cases where terrain data is embedded in the scene (i.e. it's not an
    // asset), we need to dirty the scene if terrainData has changed.
    if (!EditorUtility.IsPersistent(m_Terrain.terrainData) &&
        !EditorApplication.isPlaying)
      SceneManagement.EditorSceneManager.MarkSceneDirty(
          m_Terrain.gameObject.scene);
  }

  private class DoCreateTerrainMaterial
    ...
    public override void Action(int instanceId, string pathName,
      ...
      AssetDatabase.CreateAsset(
          obj, AssetDatabase.GenerateUniqueAssetPath(pathName));
    ...
    public override void Cancelled(int instanceId, string pathName,
                                   string resourceFile) {
      Selection.activeObject = terrain;
    }
  }
  ...
  public void ShowSettings() {
    ...
    m_ShowBasicTerrainSettings = EditorGUILayout.BeginFoldoutHeaderGroup(
        m_ShowBasicTerrainSettings, styles.basicTerrain);
    ...
    if (m_ShowBasicTerrainSettings) {
      ...
      var groupingID =
          EditorGUILayout.IntField(styles.groupingID, m_Terrain.groupingID);

      EditorGUILayout.BeginHorizontal();
      var allowAutoConnect = EditorGUILayout.Toggle(styles.allowAutoConnect,
                                                    m_Terrain.allowAutoConnect);
      if (GUILayout.Button(styles.attemptReconnect))
        Terrain.SetConnectivityDirty();
      EditorGUILayout.EndHorizontal();

      var drawHeightmap =
          EditorGUILayout.Toggle(styles.drawTerrain, m_Terrain.drawHeightmap);
      var drawInstanced = EditorGUILayout.Toggle(styles.drawInstancedTerrain,
                                                 m_Terrain.drawInstanced);
      var heightmapPixelError = EditorGUILayout.Slider(
          styles.pixelError, m_Terrain.heightmapPixelError, 1,
          200); // former string formatting: ""
      var basemapDistance = PowerSlider(
          styles.baseMapDist, m_Terrain.basemapDistance, 0.0f, 20000.0f, 2.0f);
      var shadowCastingMode = (ShadowCastingMode) EditorGUILayout.EnumPopup(
          styles.castShadows, m_Terrain.shadowCastingMode);

      var reflectionProbeUsage =
          (ReflectionProbeUsage) EditorGUILayout.EnumPopup(
              styles.reflectionProbes, m_Terrain.reflectionProbeUsage);
      m_ShowReflectionProbesGUI.target =
      ...
      var materialTemplate =
          EditorGUILayout.ObjectField("Material", m_Terrain.materialTemplate,
                                      typeof(Material), false) as Material;
      if (EditorGUI.EndChangeCheck())
        m_ShowCreateMaterialButton =
            ShouldShowCreateMaterialButton(materialTemplate);
      if (m_ShowCreateMaterialButton &&
          GUILayout.Button(styles.createMaterial, EditorStyles.miniButton,
                           GUILayout.ExpandWidth(false))) {
        string defaultPath = "New Terrain Material.mat";
        if (materialTemplate == null) {
          ObjectSelector.get.Show(
              null, typeof(Shader), null, false, null, selection => {
                if (selection == null)
                  return;
                ...
                AssetDatabase.CreateAsset(
                    mat,
                    AssetDatabase.GetUniquePathNameAtSelectedPath(defaultPath));
          ...
          ProjectWindowUtil.StartNameEditingIfProjectWindowExists(
              mat.GetInstanceID(), createTerrainMat, defaultPath,
              AssetPreview.GetMiniThumbnail(mat), null);
        }
      }
      ...
      if (materialTemplate != null) {
        ...
        if (ShaderUtil.HasTangentChannel(s)) {
          GUIContent c = EditorGUIUtility.TrTextContent(
    ...
    m_ShowTreeAndDetailSettings = EditorGUILayout.BeginFoldoutHeaderGroup(
        m_ShowTreeAndDetailSettings, styles.treeAndDetails);

    if (m_ShowTreeAndDetailSettings) {
      ++EditorGUI.indentLevel;
      EditorGUI.BeginChangeCheck();

      var drawTreesAndFoliage = EditorGUILayout.Toggle(
          styles.drawTrees, m_Terrain.drawTreesAndFoliage);
      var bakeLightProbesForTrees = EditorGUILayout.Toggle(
          styles.bakeLightProbesForTrees, m_Terrain.bakeLightProbesForTrees);
      ...
      using(new EditorGUI.DisabledScope(!bakeLightProbesForTrees))
          deringLightProbesForTrees = EditorGUILayout.Toggle(
              styles.deringLightProbesForTrees, deringLightProbesForTrees);
      var preserveTreePrototypeLayers =
          EditorGUILayout.Toggle(styles.preserveTreePrototypeLayers,
                                 m_Terrain.preserveTreePrototypeLayers);
      var detailObjectDistance = EditorGUILayout.Slider(
          styles.detailObjectDistance, m_Terrain.detailObjectDistance, 0,
          250); // former string formatting: ""
      var detailObjectDensity =
          EditorGUILayout.Slider(styles.detailObjectDensity,
                                 m_Terrain.detailObjectDensity, 0.0f, 1.0f);
      var treeDistance =
          EditorGUILayout.Slider(styles.treeDistance, m_Terrain.treeDistance, 0,
                                 5000); // former string formatting: ""
      var treeBillboardDistance = EditorGUILayout.Slider(
          styles.treeBillboardDistance, m_Terrain.treeBillboardDistance, 5,
          2000); // former string formatting: ""
      var treeCrossFadeLength = EditorGUILayout.Slider(
          styles.treeCrossFadeLength, m_Terrain.treeCrossFadeLength, 0,
          200); // former string formatting: ""
      var treeMaximumFullLODCount = EditorGUILayout.IntSlider(
          styles.treeMaximumFullLODCount, m_Terrain.treeMaximumFullLODCount, 0,
          10000);
      ...
      if (Event.current.type == EventType.Layout) {
        ...
        for (int i = 0; i < terrainData.treePrototypes.Length; ++i) {
          if (TerrainEditorUtility.IsLODTreePrototype(
                  terrainData.treePrototypes[i].prefab)) {
      ...
      if (m_LODTreePrototypePresent)
        EditorGUILayout.HelpBox(
            "Tree Distance, Billboard Start, Fade Length and Max Mesh Trees have no effect on SpeedTree trees. Please use the LOD Group component on the tree prefab to control LOD settings.",
            MessageType.Info);
    ...
    m_ShowGrassWindSettings = EditorGUILayout.BeginFoldoutHeaderGroup(
        m_ShowGrassWindSettings, styles.grassWindSettings);

    if (m_ShowGrassWindSettings) {
      ++EditorGUI.indentLevel;
      EditorGUI.BeginChangeCheck();

      var wavingGrassStrength = EditorGUILayout.Slider(
          styles.wavingGrassStrength, terrainData.wavingGrassStrength, 0,
          1); // former string formatting: "%"
      var wavingGrassSpeed = EditorGUILayout.Slider(
          styles.wavingGrassSpeed, terrainData.wavingGrassSpeed, 0,
          1); // former string formatting: "%"
      var wavingGrassAmount = EditorGUILayout.Slider(
          styles.wavingGrassAmount, terrainData.wavingGrassAmount, 0,
          1); // former string formatting: "%"
      var wavingGrassTint = EditorGUILayout.ColorField(
          styles.wavingGrassTint, terrainData.wavingGrassTint);
  ...
  // this is a non-serializedProperty version of
  // RendererEditorBase.DrawRenderingLayer() if we switch to serializedProperty
  // multi-edit, we can just use that function directly instead
  private void ShowRenderingLayerMask(bool useMiniStyle = false) {
    ...
    if (useMiniStyle) {
      ...
      mask = EditorGUI.MaskField(rect, Styles.renderingLayerMask, mask,
                                 layerNames);
  ...
  public void ShowPaint() {
    if (m_Tools != null && m_Tools.Length >= 1 && m_ToolNames != null) {
      EditorGUI.BeginChangeCheck();
      int newPaintToolIndex =
          EditorGUILayout.Popup(m_ActivePaintToolIndex, m_ToolNames);
      if (EditorGUI.EndChangeCheck() &&
          (newPaintToolIndex != m_ActivePaintToolIndex)) {
  ...
  public void ShowBrushes(int spacing, bool showBrushes, bool showBrushEditor,
                          bool showBrushSize, bool showBrushStrength,
                          int textureResolutionPerTile) {
    EditorGUI.BeginDisabledGroup(s_activeTerrainInspector != GetInstanceID() ||
                                 s_activeTerrainInspectorInstance != this);
    ...
    if (showBrushSize) {
      ...
      GetBrushSizeLimits(out minBrushSize, out maxBrushSize,
                         textureResolutionPerTile);
      m_Size = PowerSlider(styles.brushSize, m_Size, minBrushSize, maxBrushSize,
                           4.0f);
    }

    if (showBrushStrength)
      m_Strength = ScaledSliderWithRounding(styles.opacity, m_Strength,
                                            kMinBrushStrength,
                                            kMaxBrushStrength, 100.0f, 0.1f);
  ...
  void ResizeControlTexture(int newResolution) {
    ...
    // we record the terrainData because we change
    // terrainData.alphamapResolution we also store a complete copy of the
    // alphamap -- because each alphamap is a separate asset, these are separate
    ...
    Material blitMaterial =
        TerrainPaintUtility
            .GetCopyTerrainLayerMaterial(); // special blit that forces copy
                                            // from highest mip only
    ...
    RenderTexture[] resizedAlphaMaps =
        new RenderTexture[terrainData.alphamapTextureCount];
    for (int i = 0; i < resizedAlphaMaps.Length; i++) {
      ...
      resizedAlphaMaps[i] = RenderTexture.GetTemporary(
          newResolution, newResolution, 0, oldAlphamap.graphicsFormat);
      ...
      TerrainPaintUtility.DrawQuad(
          new RectInt(0, 0, newResolution, newResolution),
          new RectInt(0, 0, sourceRezU, sourceRezV), oldAlphamap);
      GL.PopMatrix();
    }
    ...
    for (int i = 0; i < resizedAlphaMaps.Length; i++) {
      ...
      terrainData.CopyActiveRenderTextureToTexture(
          TerrainData.AlphamapTextureName, i,
          new RectInt(0, 0, newResolution, newResolution), Vector2Int.zero,
          false);
    }
  ...
  void ResizeHeightmap(int newResolution) {
    ...
    RenderTexture oldHeightmap = RenderTexture.GetTemporary(
        m_Terrain.terrainData.heightmapTexture.descriptor);
    Graphics.Blit(m_Terrain.terrainData.heightmapTexture, oldHeightmap);

    // TODO: Can this be optimized if there is no hole?
    RenderTexture oldHoles = RenderTexture.GetTemporary(
        m_Terrain.terrainData.holesRenderTexture.descriptor);
    ...
    // duv = (((suv * swidth - 0.5) / (swidth - 1)) * (dwidth - 1) + 0.5) /
    // dwidth k = (dwidth - 1) / (swidth - 1) / dwidth duv = suv * (swidth * k)
    // + 0.5 / dwidth - 0.5 * k
    ...
    Graphics.Blit(oldHeightmap, m_Terrain.terrainData.heightmapTexture, scale,
                  offset);
    RenderTexture.ReleaseTemporary(oldHeightmap);

    oldHoles.filterMode = FilterMode.Point;
    Graphics.Blit(oldHoles,
                  (RenderTexture) m_Terrain.terrainData.holesRenderTexture);
    RenderTexture.ReleaseTemporary(oldHoles);

    RenderTexture.active = oldRT;

    m_Terrain.terrainData.DirtyHeightmapRegion(
        new RectInt(0, 0, m_Terrain.terrainData.heightmapTexture.width,
                    m_Terrain.terrainData.heightmapTexture.height),
        TerrainHeightmapSyncControl.HeightAndLod);
    m_Terrain.terrainData.DirtyTextureRegion(
        TerrainData.HolesTextureName,
        new RectInt(0, 0, m_Terrain.terrainData.holesRenderTexture.width,
                    m_Terrain.terrainData.holesRenderTexture.height),
        false);
  ...
  public void ShowTextures() {
    m_ShowTextures = EditorGUILayout.BeginFoldoutHeaderGroup(m_ShowTextures,
                                                             styles.textures);
    ...
    int newHeightmapResolution = EditorGUILayout.IntPopup(
        Styles.heightmapResolution, m_Terrain.terrainData.heightmapResolution,
        Styles.heightmapResolutionStrings, Styles.heightmapResolutionInts);
    ...
    // alphamap resolution
    int newAlphamapResolution = EditorGUILayout.IntPopup(
        Styles.alphamapResolution, m_Terrain.terrainData.alphamapResolution,
        Styles.alphamapResolutionStrings, Styles.alphamapResolutionInts);
    ...
    // base texture resolution
    int newBasemapResolution = EditorGUILayout.IntPopup(
        Styles.basemapResolution, m_Terrain.terrainData.baseMapResolution,
        Styles.basemapResolutionStrings, Styles.basemapResolutionInts);
  ...
  public void ShowResolution(TerrainData terrainData) {
    m_ShowResolution = EditorGUILayout.BeginFoldoutHeaderGroup(
        m_ShowResolution, styles.meshResolution);
    ...
    terrainWidth = EditorGUILayout.DelayedFloatField(
        EditorGUIUtility.TrTextContent(
            "Terrain Width",
            $"Size of the terrain object in its X axis (in world units). Value range [1, {kMaxTerrainSize}]"),
        terrainWidth);
    if (terrainWidth <= 0)
      terrainWidth = 1;
    if (terrainWidth > kMaxTerrainSize)
      terrainWidth = kMaxTerrainSize;

    terrainLength = EditorGUILayout.DelayedFloatField(
        EditorGUIUtility.TrTextContent(
            "Terrain Length",
            $"Size of the terrain object in its Z axis (in world units). Value range [1, {kMaxTerrainSize}]"),
        terrainLength);
    if (terrainLength <= 0)
      terrainLength = 1;
    if (terrainLength > kMaxTerrainSize)
      terrainLength = kMaxTerrainSize;

    terrainHeight = EditorGUILayout.DelayedFloatField(
        EditorGUIUtility.TrTextContent(
            "Terrain Height",
            $"Difference in Y coordinate between the lowest possible heightmap value and the highest (in world units). Value range [1, {kMaxTerrainHeight}]"),
        terrainHeight);
    if (terrainHeight <= 0)
      terrainHeight = 1;
    if (terrainHeight > kMaxTerrainHeight)
      terrainHeight = kMaxTerrainHeight;

    const int kMinDetailResolutionPerPatch = 8;
    const int kMaxDetailResolutionPerPatch = 128;
    detailResolutionPerPatch = EditorGUILayout.DelayedIntField(
        EditorGUIUtility.TrTextContent(
            "Detail Resolution Per Patch",
            $"The number of cells in a single patch (mesh). This value is squared to form a grid of cells, and must be a divisor of the detail resolution. Value range [{kMinDetailResolutionPerPatch}, {kMaxDetailResolutionPerPatch}]"),
        detailResolutionPerPatch);
    detailResolutionPerPatch =
        Mathf.Clamp(detailResolutionPerPatch, kMinDetailResolutionPerPatch,
                    kMaxDetailResolutionPerPatch);

    const int kMinDetailResolution = 0;
    const int kMaxDetailResolution = 4048;
    detailResolution = EditorGUILayout.DelayedIntField(
        EditorGUIUtility.TrTextContent(
            "Detail Resolution",
            $"The number of cells available for placing details onto the terrain tile. This value is squared to make a grid of cells. Value range [{kMinDetailResolution}, {kMaxDetailResolution}]"),
        detailResolution);
    detailResolution = Mathf.Clamp(detailResolution, kMinDetailResolution,
                                   kMaxDetailResolution);
    ...
    if (EditorGUI.EndChangeCheck()) {
      ...
      if (resolutionChanged) {
        detailResolutionPerPatch =
            Mathf.Min(detailResolutionPerPatch, detailResolution);
      }

      bool resolutionPerPatchChanged =
          terrainData.detailResolutionPerPatch != detailResolutionPerPatch;
      if (resolutionPerPatchChanged) {
        detailResolution =
            Mathf.Max(detailResolution, detailResolutionPerPatch);
      }
  ...
  public void ShowHolesSettings(TerrainData terrainData) {
    m_ShowHolesSettings = EditorGUILayout.BeginFoldoutHeaderGroup(
        m_ShowHolesSettings, styles.holesSettings);
    ...
    bool enableHolesTextureCompression =
        EditorGUILayout.Toggle(styles.holesCompressionToggle,
  ...
  void ResizeDetailResolution(TerrainData terrainData, int resolution,
                              int resolutionPerPatch) {
    if (resolution == terrainData.detailResolution) {
      var layers = new List<int[, ]>();
      for (int i = 0; i < terrainData.detailPrototypes.Length; i++)
        layers.Add(terrainData.GetDetailLayer(0, 0, terrainData.detailWidth,
                                              terrainData.detailHeight, i));
  ...
  public void ShowUpgradeTreePrototypeScaleUI() {
    if (m_Terrain.terrainData != null &&
        m_Terrain.terrainData.NeedUpgradeScaledTreePrototypes()) {
  ...
  public void RenderLightingFields() { m_Lighting.RenderTerrainSettings(); }
  ...
  public override void OnInspectorGUI() {
    ...
    if (!m_Terrain.terrainData) {
      ...
      m_Terrain.terrainData = EditorGUILayout.ObjectField(
          "Assign:", m_Terrain.terrainData, typeof(TerrainData), false)
                                  as TerrainData;
      GUILayout.EndVertical();
      return;
    }
    ...
    if (m_TerrainCollider &&
      ...
      GUILayout.BeginVertical(EditorStyles.helpBox);
      GUILayout.Label(styles.mismatchedTerrainData,
    ...
    if (SystemInfo.graphicsDeviceType == GraphicsDeviceType.OpenGLES2) {
      ...
      GUILayout.BeginHorizontal();
      GUILayout
          .FlexibleSpace(); // flexible space on either end centers the toolbar
      GUI.changed = false;
      int newlySelectedTool =
          GUILayout.Toolbar(tool, styles.toolIcons, styles.command);
      ...
      if (newlySelectedTool != tool) {
        ...
        // Need to repaint other terrain inspectors as their previously selected
        // tool is now deselected.
    ...
    if (tool != (int) TerrainTool.Paint) {
      ...
      if (tool == (int) TerrainTool.CreateNeighbor) {
        ...
        if (!string.IsNullOrEmpty(m_CreateTool.GetDesc()))
          GUILayout.Label(m_CreateTool.GetDesc(),
                          EditorStyles.wordWrappedMiniLabel);
      } else if (tool > (int) TerrainTool.Paint &&
                 tool < styles.toolIcons.Length) {
        GUILayout.Label(styles.toolNames[tool].text);
        if (!string.IsNullOrEmpty(styles.toolNames[tool].tooltip))
          GUILayout.Label(styles.toolNames[tool].tooltip,
                          EditorStyles.wordWrappedMiniLabel);
      } else {
  ...
  public bool Raycast(out Vector2 uv, out Vector3 pos) {
    ...
    RaycastHit hit;
    if (m_Terrain.GetComponent<TerrainCollider>().Raycast(
            mouseRay, out hit, Mathf.Infinity, true)) {
      uv = hit.textureCoord;
      pos = hit.point;
      return true;
    }
  ...
  public bool HasFrameBounds() {
    // When selecting terrains using Scene search, they may be framed right
    // after selecting, when the editor is not initialized yet. Just return
    // empty bounds in that case.
  ...
  public Bounds OnGetFrameBounds() {
    ...
    // It's possible that something other the scene view invoked
    // OnGetFrameBounds (e.g. double clicking the terrain in the hierarchy) In
    // this case we can't do a raycast, because there is no active camera.
    if (Camera.current && m_Terrain.terrainData && Raycast(out uv, out pos)) {
      ...
      float brushSize =
          selectedTool ==
          TerrainTool.PlaceTree ? PaintTreesTool.instance.brushSize : m_Size;
      Vector3 size;
      size.x = brushSize / m_Terrain.terrainData.heightmapResolution *
               m_Terrain.terrainData.size.x;
      size.z = brushSize / m_Terrain.terrainData.heightmapResolution *
               m_Terrain.terrainData.size.z;
      ...
      if (selectedTool == TerrainTool.PaintDetail &&
          m_Terrain.terrainData.detailWidth != 0) {
        size.x = brushSize / m_Terrain.terrainData.detailWidth *
                 m_Terrain.terrainData.size.x;
        size.z = brushSize / m_Terrain.terrainData.detailHeight *
                 m_Terrain.terrainData.size.z;
        size.y = 0;
        bounds.size = size;
      }
      ...
      // We don't return bounds from the collider, because apparently they are
      // not immediately updated after changing the position. So if terrain is
      // repositioned, then bounds will still be wrong until after some times
      // PhysX updates them (it gets them through some pruning interface and not
      // directly from heightmap shape).
      ...
      float[, ] heights = m_Terrain.terrainData.GetHeights(
          0, 0, m_Terrain.terrainData.heightmapResolution,
  ...
  private Ray GUIPointToWorldRayPrecise(Vector2 guiPoint,
                                        float startZ = float.NegativeInfinity) {
    ...
    Vector2 screenPixelPos =
        HandleUtility.GUIPointToScreenPixelCoordinate(guiPoint);
    ...
    // first construct an arbitrary point that is on the ray through this screen
    // pixel (remap screen pixel point to clip space [-1, 1])
    Vector3 rayPointClipSpace = new Vector3(
        (screenPixelPos.x - viewport.x) * 2.0f / viewport.width - 1.0f,
        (screenPixelPos.y - viewport.y) * 2.0f / viewport.height - 1.0f, 0.95f);
    ...
    if (camera.orthographic) {
      ...
      rayDirectionWorldSpace =
          camToWorld.MultiplyVector(rayDirectionCameraSpace);
      rayDirectionWorldSpace.Normalize();

      // in camera space, the ray origin has the same XY coordinates as ANY
      // point on the ray so we just need to override the Z coordinate to startZ
      // to get the correct starting point (assuming camToWorld is a pure
      // rotation/offset, with no scale)
      ...
      // calculate the correct startZ offset from the camera by moving a
      // distance along the ray direction this assumes camToWorld is a pure
      // rotation/offset, with no scale, so we can use rayDirection.z to
      // calculate how far we need to move
      Vector3 cameraPositionWorldSpace = camToWorld.MultiplyPoint(Vector3.zero);
      Vector3 originOffsetWorldSpace =
          rayDirectionWorldSpace *
          Mathf.Abs(startZ / rayDirectionCameraSpace.z);
  ...
  private bool RaycastAllTerrains(out Terrain hitTerrain,
    ...
    foreach (Terrain terrain in Terrain.activeTerrains) {
      RaycastHit hit;
      if (terrain.GetComponent<TerrainCollider>().Raycast(
              mouseRay, out hit, Mathf.Infinity, true)) {
  ...
  public void OnSceneGUICallback(SceneView sceneView) {
    ...
    // If this is not the active terrain inspector, we shouldn't be affecting
    // the SceneGUI
    if (s_activeTerrainInspector != GetInstanceID() ||
        s_activeTerrainInspectorInstance != this)
      return;
    ...
    Texture brushTexture =
        selectedTool == TerrainTool.PlaceTree ? brushList.GetCircleBrush()
                            .texture : brushList.GetActiveBrush()
                            .texture;

    Vector2 uv = raycastHit.textureCoord;

    bool hitValidTerrain =
        (hitTerrain != null && hitTerrain.terrainData != null);
    if (hitValidTerrain) {
      HotkeyApply(raycastHit.distance);
    }

    Terrain lastActiveTerrain =
        hitValidTerrain ? hitTerrain : s_LastActiveTerrain;
    if (lastActiveTerrain) {
      activeTool.OnSceneGUI(
          lastActiveTerrain,
          onSceneGUIEditContext.Set(sceneView, hitValidTerrain, raycastHit,
                                    brushTexture, m_Strength, m_Size));
    }

    int id = GUIUtility.GetControlID(s_TerrainEditorHash, FocusType.Passive);
    var eventType = e.GetTypeForControl(id);
    if (!hitValidTerrain) {
      // if we release the mouse button outside the terrain we still need to
      // update the terrains. ( case 1089947 )
    ...
    case EventType.MouseDrag: {
      // Don't do anything at all if someone else owns the hotControl. Fixes
      // case 677541.
      ...
      if (activeTool.OnPaint(hitTerrain,
                             onPaintEditContext.Set(hitValidTerrain, raycastHit,
                                                    brushTexture, uv,
                                                    m_Strength, m_Size))) {
        if (selectedTool == TerrainTool.Paint) {
