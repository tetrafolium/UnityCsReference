...
static class Styles {
public static GUIContent readonlyText = EditorGUIUtility.TrTextContent(
	"One or more selected Brushes are read-only.");
public static GUIContent maskTexture = EditorGUIUtility.TrTextContent(
	"Mask Texture", "Texture Red channel controls the shape of the Brush");
public static GUIContent falloff = EditorGUIUtility.TrTextContent(
	"Falloff Curve",
	"Controls the Brush falloff curve, over the distance from the center of the Brush.");
public static GUIContent radiusScale = EditorGUIUtility.TrTextContent(
	"Falloff Radius Scale", "Controls the radius of the falloff curve.");
public static GUIContent remap = EditorGUIUtility.TrTextContent(
	"Brush Remap", "Remaps the grayscale values of the Brush");
public static GUIContent remapInvert = EditorGUIUtility.TrTextContent(
	"Brush Invert", "Inverts the Brush shape, swapping black and white");
}
...
protected virtual void OnEnable() {
	...
	m_BlackWhiteRemapMin =
		serializedObject.FindProperty("m_BlackWhiteRemapMin");
	m_BlackWhiteRemapMax =
		serializedObject.FindProperty("m_BlackWhiteRemapMax");
...
public override void OnInspectorGUI() {
	...
	Texture2D mask = (Texture2D) EditorGUILayout.ObjectField(
		Styles.maskTexture, origMask, typeof(Texture2D), false);
	if (mask == null) {
	...
	EditorGUILayout.MinMaxSlider(Styles.remap, ref blackWhiteRemapMin,
	                             ref blackWhiteRemapMax, 0.0f, 1.0f);
	if (m_BlackWhiteRemapMin.floatValue != blackWhiteRemapMin ||
...
public override void OnPreviewGUI(Rect r, GUIStyle background) {
	...
	if (m_HasChanged || m_PreviewTexture == null) {
		m_PreviewTexture = Brush.GenerateBrushTexture(
			mask, m_Falloff.animationCurveValue, m_RadiusScale.floatValue,
			m_BlackWhiteRemapMin.floatValue, m_BlackWhiteRemapMax.floatValue,
			m_InvertRemapRange.boolValue, texWidth, texHeight, true);
		m_HasChanged = false;
	}
...
public sealed override Texture2D RenderStaticPreview(
	...
	PreviewHelpers.AdjustWidthAndHeightForStaticPreview(
		brush.m_Mask.width, brush.m_Mask.height, ref width, ref height);
	return Brush.GenerateBrushTexture(
		brush.m_Mask, brush.m_Falloff, brush.m_RadiusScale,
		brush.m_BlackWhiteRemapMin, brush.m_BlackWhiteRemapMax,
		brush.m_InvertRemapRange, width, height, true);
}
}
}
