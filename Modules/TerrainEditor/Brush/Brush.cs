...
internal class Brush : ScriptableObject {
  ...
  public static void CreateNewDefaultBrush() {
    Brush b = CreateInstance(DefaultMask(), AnimationCurve.Linear(0, 0, 1, 1),
                             1.0f, false);
  ...
  internal const float kMaxRadiusScale =
      sqrt2 +
      (sqrt2 *
       0.05f); // take into account the smoothness from CreateBrush.shader (
               // circleFalloff = 1.0f - smoothstep(_BrushRadiusScale-0.05f,
               // _BrushRadiusScale, dist) )
  ...
  internal static Brush CreateInstance(Texture2D t, AnimationCurve f,
                                       float radiusScale, bool isReadOnly) {
  ...
  void UpdateTexture() {
    if (m_UpdateTexture || m_Texture == null) {
      ...
      m_Texture =
          GenerateBrushTexture(m_Mask, m_Falloff, m_RadiusScale,
                               m_BlackWhiteRemapMin, m_BlackWhiteRemapMax,
                               m_InvertRemapRange, m_Mask.width, m_Mask.height);
  ...
  void UpdateThumbnail() {
    if (m_UpdateThumbnail || m_Thumbnail == null) {
      ...
      m_Thumbnail = GenerateBrushTexture(
          m_Mask, m_Falloff, m_RadiusScale, m_BlackWhiteRemapMin,
          m_BlackWhiteRemapMax, m_InvertRemapRange, 64, 64, true);
  ...
  GenerateBrushTexture(Texture2D mask, AnimationCurve falloff,
                       float radiusScale, float blackWhiteRemapMin,
                       float blackWhiteRemapMax, bool invertRemapRange,
                       int width, int height, bool isThumbnail = false) {
    if (s_CreateBrushMaterial == null)
      s_CreateBrushMaterial = new Material(EditorGUIUtility.LoadRequired(
          "Brushes/CreateBrush.shader") as Shader);

    TextureFormat falloffFormat = TextureFormat.R16;

    // fallback for old platforms (GLES2).. ugly quantization but approximately
    // correct
    ...
    for (int i = 0; i < sampleCount; i++) {
      ...
      falloffPix[sampleCount - i - 1].r = falloffPix[sampleCount - i - 1].g =
          falloffPix[sampleCount - i - 1].b =
              falloffPix[sampleCount - i - 1].a = val;
    }
    ...
    GraphicsFormat outputRenderFormat =
        isThumbnail ? SystemInfo.GetGraphicsFormat(DefaultFormat.LDR)
        : Terrain.heightmapFormat;
    TextureFormat outputTexFormat =
        isThumbnail ? TextureFormat.ARGB32 : Terrain.heightmapTextureFormat;

    // build brush texture
    float blackRemap =
        invertRemapRange ? blackWhiteRemapMax : blackWhiteRemapMin;
    float whiteRemap =
        invertRemapRange ? blackWhiteRemapMin : blackWhiteRemapMax;

    Vector4 brushParams = new Vector4(
        radiusScale * 0.5f, mask.format != TextureFormat.Alpha8 ? 1.0f : 0.0f,
        blackRemap, whiteRemap);
    RenderTexture tempRT =
        RenderTexture.GetTemporary(width, height, 0, outputRenderFormat);
    ...
    Texture2D previewTexture =
        new Texture2D(width, height, outputTexFormat, false);
