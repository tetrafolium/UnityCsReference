...
internal class BrushList {
  ...
  internal static class Styles {
    ...
    public static GUIContent brushes =
        EditorGUIUtility.TrTextContent("Brushes");
  }
  ...
  public void LoadBrushes() {
    ...
    do {
      brush = (Brush) EditorGUIUtility.Load(EditorResources.brushesPath +
                                            "builtin_brush_" + idx + ".brush");
    ...
    do {
      ...
      if (t) {
        Brush b = Brush.CreateInstance(t, AnimationCurve.Constant(0, 1, 1),
                                       Brush.kMaxRadiusScale, true);
        arr.Add(b);
    ...
    // Load .brush files
    foreach (string assetPath in AssetDatabase.FindAssets(
                 $"t:{typeof(Brush).Name}")) {
      var b = AssetDatabase.LoadAssetAtPath(
          AssetDatabase.GUIDToAssetPath(assetPath), typeof(Brush)) as Brush;
  ...
  public Brush GetCircleBrush() { return m_BrushList[0]; }
  ...
  public bool ShowGUI() {
    ...
    {
      Rect brushPreviewRect = EditorGUILayout.GetControlRect(
          true, GUILayout.Width(128), GUILayout.Height(128));
      if (m_BrushList != null) {
        EditorGUI.DrawTextureAlpha(brushPreviewRect,
                                   GetActiveBrush().thumbnail);

        bool dummy;
        m_ScrollPos =
            EditorGUILayout.BeginScrollView(m_ScrollPos, GUILayout.Height(128));
        var missingBrush =
            EditorGUIUtility.TrTextContent("No brushes defined.");
        int newBrush =
            BrushSelectionGrid(m_SelectedBrush, m_BrushList, 32,
                               Styles.gridList, missingBrush, out dummy);
  ...
  public void ShowEditGUI() {
    ...
    if (m_BrushEditor) {
      ...
      bool newVisible = EditorGUI.DoObjectFoldout(
          m_ShowBrushSettings, titleRect, renderRect, m_FoldoutContent, id);
  ...
  int BrushSelectionGrid(int selected, Brush[] brushes, int approxSize,
                         GUIStyle style, GUIContent emptyString,
    ...
    if (brushes.Length != 0) {
      ...
      retval =
          GUI.SelectionGrid(r, System.Math.Min(selected, brushes.Length - 1),
                            m_Thumnails, (int) columns, style);
    } else
  ...
  internal void CreateBrush() {
    ObjectSelector.get.Show(
        null, typeof(Texture2D), null, false, null, selection => {
          ...
          var brushName = AssetDatabase.GenerateUniqueAssetPath(Path.Combine(
              ProjectWindowUtil.GetActiveFolderPath(), "NewBrush.brush"));
          var newBrush = Brush.CreateInstance((Texture2D) selection,
                                              AnimationCurve.Linear(0, 0, 1, 1),
                                              Brush.kMaxRadiusScale, false);
