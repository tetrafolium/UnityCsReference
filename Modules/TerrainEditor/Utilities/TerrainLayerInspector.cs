...
private static void CreateDefaultTerrainLayer() {
	...
	ProjectWindowUtil.CreateAsset(terrainLayer,
	                              "New Terrain Layer.terrainlayer");
}

private class Styles {
public readonly GUIContent diffuseTexture = EditorGUIUtility.TrTextContent(
	"Diffuse",
	"Depending on your terrain material type, the Alpha channel can be:\n\n  Built-in diffuse: Unused\n  Built-in specular: Gloss\n  Built-in standard or HDRP TerrainLit: Smoothness");
public readonly GUIContent diffuseTextureMaskMapEnabled =
	EditorGUIUtility.TrTextContent(
		"Diffuse",
		"Depending on your terrain material type, the Alpha channel can be:\n\n  Built-in diffuse: Unused\n  Built-in specular: Gloss\n  Built-in standard: Smoothness\n  HDRP TerrainLit: Density");
public readonly GUIContent normalMapTexture =
	EditorGUIUtility.TrTextContent("Normal Map");
public readonly GUIContent maskMapTexture = EditorGUIUtility.TrTextContent(
	"Mask Map",
	"HDRP TerrainLit shader uses this texture for:\n\n  R: Metallic\n  G: AO\n  B: Height\n  A: Smoothness (Diffuse Alpha becomes Density)\n\nCustom shaders can use this texture for whatever purpose.");
public readonly GUIContent channelRemapping =
	EditorGUIUtility.TrTextContent("Channel Remapping");
...
public readonly GUIContent min = EditorGUIUtility.TrTextContent(
	"Min", "The value when the texture channel is 0");
public readonly GUIContent max = EditorGUIUtility.TrTextContent(
	"Max", "The value when the texture channel is 1");
}
...
private GUIContent m_DiffuseMapMaskMapEnabledText =
	s_Styles.diffuseTextureMaskMapEnabled;
...
private void OnEnable() {
	...
	m_NormalMapHasCorrectTextureType =
		TerrainLayerUtility.CheckNormalMapTextureType(
			m_NormalMapTexture.objectReferenceValue as Texture2D);
}
...
private static void DoMinMaxLabels(GUIContent minLabel, GUIContent maxLabel,
                                   GUIStyle style) {
	...
	var r2 = new Rect(r.x + halfWidth + EditorGUI.kSpacing, r.y, halfWidth,
	                  r.height);
...
private static void DoMinMaxFloatFields(GUIContent label, GUIStyle labelStyle,
...
public override void OnInspectorGUI() {
	...
	var diffuseLabel =
		m_ShowMaskMap ? (maskMapUsed ? m_DiffuseMapMaskMapEnabledText
		       : m_DiffuseMapText)
	: s_Styles.diffuseTexture;
	EditorGUI.BeginProperty(r, diffuseLabel, m_DiffuseTexture);
	EditorGUI.BeginChangeCheck();
	var diffuseTexture = EditorGUI.ObjectField(
		r, diffuseLabel, m_DiffuseTexture.objectReferenceValue as Texture2D,
	...
	EditorGUI.BeginChangeCheck();
	var normalMapTexture = EditorGUI.ObjectField(
		r, s_Styles.normalMapTexture,
		m_NormalMapTexture.objectReferenceValue as Texture2D, typeof(Texture2D),
		false) as Texture2D;
	if (EditorGUI.EndChangeCheck()) {
		m_NormalMapTexture.objectReferenceValue = normalMapTexture;
		m_NormalMapHasCorrectTextureType =
			TerrainLayerUtility.CheckNormalMapTextureType(normalMapTexture);
	}
	EditorGUI.EndProperty();

	TerrainLayerUtility.ValidateNormalMapTextureUI(
		normalMapTexture, m_NormalMapHasCorrectTextureType);
	...
	if (m_ShowMaskMap) {
		...
		EditorGUI.BeginChangeCheck();
		var maskMapTexture = EditorGUI.ObjectField(
			r, m_MaskMapText, curMaskMap, typeof(Texture2D), false) as Texture2D;
		if (EditorGUI.EndChangeCheck())
		...
		if (maskMapUsed) {
			++EditorGUI.indentLevel;
			m_ShowMaskRemap =
				EditorGUILayout.Foldout(m_ShowMaskRemap, s_Styles.channelRemapping);
			if (m_ShowMaskRemap) {
				DoMinMaxLabels(s_Styles.min, s_Styles.max, EditorStyles.miniLabel);
				DoMinMaxFloatFields(m_MaskRemapRText, EditorStyles.miniLabel,
				                    m_MaskRemapMinR, m_MaskRemapMaxR,
				                    EditorStyles.miniTextField);
				DoMinMaxFloatFields(m_MaskRemapGText, EditorStyles.miniLabel,
				                    m_MaskRemapMinG, m_MaskRemapMaxG,
				                    EditorStyles.miniTextField);
				DoMinMaxFloatFields(m_MaskRemapBText, EditorStyles.miniLabel,
				                    m_MaskRemapMinB, m_MaskRemapMaxB,
				                    EditorStyles.miniTextField);
				DoMinMaxFloatFields(m_MaskRemapAText, EditorStyles.miniLabel,
				                    m_MaskRemapMinA, m_MaskRemapMaxA,
				                    EditorStyles.miniTextField);
			}
			--EditorGUI.indentLevel;
		}
...
public override Texture2D RenderStaticPreview(string assetPath,
	...
	TerrainLayer t =
		AssetDatabase.LoadMainAssetAtPath(assetPath) as TerrainLayer;
	...
	PreviewHelpers.AdjustWidthAndHeightForStaticPreview(texwidth, texheight,
