...
public static class TerrainLayerUtility {
  private class Styles {
    public readonly GUIContent terrainLayers =
        EditorGUIUtility.TrTextContent("Terrain Layers");
    public readonly GUIContent btnEditTerrainLayers =
        EditorGUIUtility.TrTextContentWithIcon(
            "Edit Terrain Layers...",
            "Allows adding / replacing or removing terrain layers",
            EditorGUIUtility.IconContent("SettingsIcon").image);
    public readonly GUIContent errNoLayersFound = EditorGUIUtility.TrTextContent(
        "No terrain layers founds. You can create a new terrain layer using the Asset/Create/Terrain Layer menu command.");
    public readonly GUIContent textureAssign = EditorGUIUtility.TrTextContent(
        "Assign a tiling texture",
        EditorGUIUtility.GetHelpIcon(MessageType.Warning));
    public readonly GUIContent textureMustHaveRepeatWrapMode =
        EditorGUIUtility.TrTextContent(
            "Texture wrap mode must be set to Repeat",
            EditorGUIUtility.GetHelpIcon(MessageType.Warning));
    public readonly GUIContent textureMustHaveMips =
        EditorGUIUtility.TrTextContent(
            "Texture must have mip maps",
            EditorGUIUtility.GetHelpIcon(MessageType.Warning));
    public readonly GUIContent normalMapIncorrectTextureType =
        EditorGUIUtility.TrTextContent(
            "Normal texture should be imported as Normal Map",
            EditorGUIUtility.GetHelpIcon(MessageType.Warning));
    public readonly GUIContent tilingSettings =
        EditorGUIUtility.TrTextContent("Tiling Settings");
    public readonly GUIContent tilingSize =
        EditorGUIUtility.TrTextContent("Size");
    public readonly GUIContent tilingOffset =
        EditorGUIUtility.TrTextContent("Offset");
  }
  private static Styles s_Styles = new Styles();

  public static int ShowTerrainLayersSelectionHelper(Terrain terrain,
                                                     int activeTerrainLayer) {
    ...
    if (terrain.terrainData.terrainLayers.Length > 0) {
      ...
      for (int i = 0; i < layerIcons.Length; ++i) {
        layerIcons[i] =
            (layers[i] == null || layers[i].diffuseTexture == null)
                ? EditorGUIUtility.whiteTexture
                : AssetPreview.GetAssetPreview(layers[i].diffuseTexture)
                ?? layers[i].diffuseTexture;
      }
      selectedTerrainLayer = TerrainInspector.AspectSelectionGrid(
          activeTerrainLayer, layerIcons, 64, new GUIStyle("GridList"),
          s_Styles.errNoLayersFound, out doubleClick);
    } else
    ...
    // menu button
    Rect r = GUILayoutUtility.GetRect(s_Styles.btnEditTerrainLayers,
                                      new GUIStyle("Button"));
    if (GUI.Button(r, s_Styles.btnEditTerrainLayers, new GUIStyle("Button"))) {
      ...
      EditorUtility.DisplayPopupMenu(new Rect(r.x, r.y, 0, 0),
                                     "CONTEXT /TerrainLayers", context);
    }
  ...
  public static void ValidateNormalMapTextureUI(Texture2D texture,
                                                bool normalMapTextureType) {
    if (texture != null && texture.wrapMode != TextureWrapMode.Repeat)
      EditorGUILayout.HelpBox(s_Styles.textureMustHaveRepeatWrapMode);
    if (texture != null && texture.mipmapCount <= 1)
      EditorGUILayout.HelpBox(s_Styles.textureMustHaveMips);
  ...
  public static void TilingSettingsUI(TerrainLayer terrainLayer) {
    ...
    terrainLayer.tileSize = EditorGUILayout.Vector2Field(s_Styles.tilingSize,
                                                         terrainLayer.tileSize);
    terrainLayer.tileOffset = EditorGUILayout.Vector2Field(
        s_Styles.tilingOffset, terrainLayer.tileOffset);
  }

  public static void TilingSettingsUI(SerializedProperty tileSize,
  ...
  internal static void AddTerrainLayer(Terrain terrain,
  ...
  internal static void ReplaceTerrainLayer(Terrain terrain, int index,
                                           TerrainLayer inputLayer) {
    if (inputLayer == null) {
    ...
    terrain.terrainData.SetTerrainLayersRegisterUndo(layers,
...
internal class TerrainLayersContextMenus {
  ...
  internal static void CreateLayer(MenuCommand item) {
    ObjectSelector.get.Show(
        null, typeof(Texture2D), null, false, null, selection => {
          if (selection == null)
            return;

          var layerName = AssetDatabase.GenerateUniqueAssetPath(
              Path.Combine(ProjectWindowUtil.GetActiveFolderPath(),
  ...
  internal static void AddLayer(MenuCommand item) {
    var terrain = (Terrain) item.context;
    ObjectSelector.get.Show(null, typeof(TerrainLayer), null, false,
                            null, selection => {
                              TerrainLayerUtility.AddTerrainLayer(
                                  terrain, (TerrainLayer) selection);
                            }, null);
  }
  ...
  internal static void ReplaceLayer(MenuCommand item) {
    ...
    ObjectSelector.get.Show(
        layer, typeof(TerrainLayer), null, false, null, null, selection => {
          TerrainLayerUtility.ReplaceTerrainLayer(terrain, (int) item.userData,
                                                  (TerrainLayer) selection);
        });
  }
  ...
  internal static bool ReplaceLayerCheck(MenuCommand item) {
    var terrain = (Terrain) item.context;
    return item.userData >= 0 &&
           item.userData < terrain.terrainData.terrainLayers.Length;
  }
