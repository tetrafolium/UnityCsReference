...
internal class TerrainMenus {
  ...
  static void CreateTerrain(MenuCommand menuCommand) {
    ...
    AssetDatabase.CreateAsset(
        terrainData,
        AssetDatabase.GenerateUniqueAssetPath("Assets/New Terrain.asset"));
  ...
  internal static void ImportRaw() {
    string saveLocation =
        EditorUtility.OpenFilePanel("Import Raw Heightmap", "", "raw");
    if (saveLocation != "") {
      ImportRawHeightmap wizard =
          TerrainWizard.DisplayTerrainWizard<ImportRawHeightmap>(
              "Import Heightmap", "Import");
      wizard.InitializeImportRaw(GetActiveTerrain(), saveLocation);
    }
  }

  internal static void ExportHeightmapRaw() {
    ExportRawHeightmap wizard =
        TerrainWizard.DisplayTerrainWizard<ExportRawHeightmap>(
            "Export Heightmap", "Export");
    wizard.InitializeDefaults(GetActiveTerrain());
  }

  internal static void MassPlaceTrees() {
    PlaceTreeWizard wizard =
        TerrainWizard.DisplayTerrainWizard<PlaceTreeWizard>("Place Trees",
                                                            "Place");
    wizard.InitializeDefaults(GetActiveTerrain());
  }

  internal static void Flatten() {
    FlattenHeightmap wizard =
        TerrainWizard.DisplayTerrainWizard<FlattenHeightmap>(
            "Flatten Heightmap", "Flatten");
    wizard.InitializeDefaults(GetActiveTerrain());
  }
  ...
  static Terrain GetActiveTerrain() {
    Object[] selection =
        Selection.GetFiltered(typeof(Terrain), SelectionMode.Editable);
...
class TerrainDetailContextMenus {
  ...
  static internal void AddDetailTexture(MenuCommand item) {
    DetailTextureWizard wizard =
        TerrainWizard.DisplayTerrainWizard<DetailTextureWizard>(
            "Add Grass Texture", "Add");
  ...
  static internal void AddDetailMesh(MenuCommand item) {
    DetailMeshWizard wizard =
        TerrainWizard.DisplayTerrainWizard<DetailMeshWizard>("Add Detail Mesh",
                                                             "Add");
  ...
  static internal void EditDetail(MenuCommand item) {
    ...
    DetailPrototype prototype =
        terrain.terrainData.detailPrototypes[item.userData];

    if (prototype.usePrototypeMesh) {
      DetailMeshWizard wizard =
          TerrainWizard.DisplayTerrainWizard<DetailMeshWizard>(
              "Edit Detail Mesh", "Apply");
      wizard.InitializeDefaults((Terrain) item.context, item.userData);
    } else {
      DetailTextureWizard wizard =
          TerrainWizard.DisplayTerrainWizard<DetailTextureWizard>(
              "Edit Grass Texture", "Apply");
      wizard.InitializeDefaults((Terrain) item.context, item.userData);
    }
  }
  ...
  static internal bool EditDetailCheck(MenuCommand item) {
    ...
    return item.userData >= 0 &&
           item.userData < terrain.terrainData.detailPrototypes.Length;
  }
...
class TerrainTreeContextMenus {
  ...
  static internal void AddTree(MenuCommand item) {
    TreeWizard wizard =
        TerrainWizard.DisplayTerrainWizard<TreeWizard>("Add Tree", "Add");
    wizard.InitializeDefaults((Terrain) item.context, -1);
  }
  ...
  static internal void EditTree(MenuCommand item) {
    TreeWizard wizard =
        TerrainWizard.DisplayTerrainWizard<TreeWizard>("Edit Tree", "Apply");
    wizard.InitializeDefaults((Terrain) item.context, item.userData);
  }
...
/*
    ...
    static void ImportHeightmap () {
        ImportTextureHeightmap wizard =
   ScriptableWizard.DisplayWizard<ImportTextureHeightmap>("Import Heightmap",
   "Import"); wizard.InitializeDefaults(GetActiveTerrain());
    }
*/
} // namespace
