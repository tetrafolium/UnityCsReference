...
internal class TerrainWizard : ScriptableWizard {
  ...
  internal static T DisplayTerrainWizard<T>(string title, string button) where T
      : TerrainWizard {
...
internal class ImportRawHeightmap : TerrainWizard {
  ...
  public void OnEnable() { minSize = new Vector2(400, 250); }
  ...
  void PickRawDefaults(string path) {
    ...
    if (terrainData.heightmapResolution * terrainData.heightmapResolution ==
      ...
      m_Depth = Depth.Bit8;
    } else if (terrainData.heightmapResolution *
  ...
  internal void OnWizardCreate() {
    ...
    if (m_Resolution > kMaxResolution) {
      isValid = false;
      errorString =
          "Heightmaps above 4097x4097 in resolution are not supported";
    ...
    if (m_Depth == Depth.Bit16) {
      ...
      for (int y = 0; y < heightmapRes; ++y) {
        for (int x = 0; x < heightmapRes; ++x) {
          int index = Mathf.Clamp(x, 0, m_Resolution - 1) +
                      Mathf.Clamp(y, 0, m_Resolution - 1) * m_Resolution;
          if ((m_ByteOrder == ByteOrder.Mac) ==
          ...
          ushort compressedHeight =
              System.BitConverter.ToUInt16(data, index * 2);
  ...
  internal void InitializeImportRaw(Terrain terrain, string path) {
    ...
    helpString =
        "Raw files must use a single channel and be either 8 or 16 bit.";
    OnWizardUpdate();
  }
}
...
internal class ExportRawHeightmap : TerrainWizard {
  ...
  public void OnEnable() { minSize = new Vector2(400, 200); }
  ...
  internal void OnWizardCreate() {
    ...
    string saveLocation =
        EditorUtility.SaveFilePanel("Save Raw Heightmap", "", "terrain", "raw");
    if (saveLocation != "") {
  ...
  void WriteRaw(string path) {
    ...
    int heightmapRes = terrainData.heightmapResolution;
    float[, ] heights =
        terrainData.GetHeights(0, 0, heightmapRes, heightmapRes);
    ...
    if (m_Depth == Depth.Bit16) {
      ...
      for (int y = 0; y < heightmapRes; ++y) {
        for (int x = 0; x < heightmapRes; ++x) {
          ...
          ushort compressedHeight =
              (ushort) Mathf.Clamp(height, 0, ushort.MaxValue);
...
internal enum NavMeshLodIndex { First, Last, Custom }
...
class TreeWizard : TerrainWizard {
  ...
  public void OnEnable() { minSize = new Vector2(400, 150); }
  ...
  internal void InitializeDefaults(Terrain terrain, int index) {
    ...
    if (m_PrototypeIndex == -1) {
      ...
      TreePrototype treePrototype =
          m_Terrain.terrainData.treePrototypes[m_PrototypeIndex];
  ...
  void OnWizardCreate() { DoApply(); }

  void OnWizardOtherButton() { DoApply(); }

  protected override bool DrawWizardGUI() {
    EditorGUI.BeginChangeCheck();

    bool allowSceneObjects = !EditorUtility.IsPersistent(
        m_Terrain.terrainData); // sometimes user prefers saving terrainData
                                // with the scene file
    m_Tree = (GameObject) EditorGUILayout.ObjectField(
        "Tree Prefab", m_Tree, typeof(GameObject), allowSceneObjects);
    if (m_Tree) {
      MeshRenderer meshRenderer = m_Tree.GetComponent<MeshRenderer>();
      if (meshRenderer) {
        EditorGUI.BeginDisabled(true);
        EditorGUILayout.EnumPopup("Cast Shadows",
    ...
    if (!TerrainEditorUtility.IsLODTreePrototype(m_Tree)) {
      ...
      navMeshLodIndex = (NavMeshLodIndex) EditorGUILayout.EnumPopup(
      ...
      if (navMeshLodIndex == NavMeshLodIndex.First)
        ...
        m_NavMeshLod = EditorGUILayout.IntSlider(
            m_NavMeshLod, 0, Mathf.Max(0, lodGroup.lodCount - 1));
...
enum DetailMeshRenderMode { VertexLit, Grass }
...
class DetailMeshWizard : TerrainWizard {
  ...
  public void OnEnable() { minSize = new Vector2(400, 400); }
...
class DetailTextureWizard : TerrainWizard {
  ...
  void DoApply() {
    ...
    prototypes[m_PrototypeIndex].healthyColor = m_HealthyColor;
    prototypes[m_PrototypeIndex].dryColor = m_DryColor;
    prototypes[m_PrototypeIndex].renderMode =
...
class PlaceTreeWizard : TerrainWizard {
  ...
  public void OnEnable() { minSize = new Vector2(250, 150); }

  void OnWizardCreate() {
    if (numberOfTrees > kMaxNumberOfTrees) {
      isValid = false;
      errorString =
          String.Format("Mass placing more than {0} trees is not supported",
                        kMaxNumberOfTrees);
      Debug.LogError(errorString);
      return;
    }
    PaintTreesTool.instance.MassPlaceTrees(m_Terrain.terrainData, numberOfTrees,
                                           true, keepExistingTrees);
    m_Terrain.Flush();
  }
}
...
class FlattenHeightmap : TerrainWizard {
  ...
  internal override void OnWizardUpdate() {
    if (terrainData)
      helpString =
          height + " meters (" + height / terrainData.size.y * 100 + "%)";
  }
...
class ImportTextureHeightmap : TerrainWizard
{
    ...
    /// Creates a heightmap from a heightmap texture and a given size of the
terrrain static public void ImportHeightmap (TerrainData theTerrain, Texture2D
texture, Vector3 importSize)
    {
...
// @TODO:
//      if (texture.format != TextureFormat.Alpha8 && texture.format !=
TextureFormat.RGB24 && texture.format != TextureFormat.ARGB32 && texture.format
!= TextureFormat.RGBA32) {
//          Debug.Log ("Heightmap texture must be an uncompressed texture");
//          return;
//      }

        float[,] heights = new float[theTerrain.heightmapHeight,
theTerrain.heightmapWidth]; for (int y=0;y<theTerrain.heightmapHeight;y++)
        {
