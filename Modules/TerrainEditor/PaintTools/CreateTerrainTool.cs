...
namespace UnityEditor.Experimental.TerrainAPI {
  internal class CreateTerrainTool : TerrainPaintTool<CreateTerrainTool> {
    private class Styles {
      public GUIContent fillHeightmapUsingNeighbors = EditorGUIUtility.TrTextContent(
          "Fill Heightmap Using Neighbors",
          "If selected, it will fill heightmap of the new terrain performing cross blend of heightmaps of its neighbors.");
      public GUIContent fillAddressMode = EditorGUIUtility.TrTextContent(
          "Fill Heightmap Address Mode",
          "Type of the terrain's neighbors sampling address mode.");
      public GUIContent terrainToolPropertyChange =
          EditorGUIUtility.TrTextContent("Terrain tool property change");
    }

    private enum FillAddressMode { Clamp = 0, Mirror = 1 }
    ...
    [SerializeField]
    private bool m_FillHeightmapUsingNeighbors = true;
    [SerializeField]
    private FillAddressMode m_FillAddressMode;
    private Material m_CrossBlendMaterial;

    private Material GetOrCreateCrossBlendMaterial() {
      if (m_CrossBlendMaterial == null)
        m_CrossBlendMaterial = new Material(
            Shader.Find("Hidden/TerrainEngine/CrossBlendNeighbors"));
      return m_CrossBlendMaterial;
    }

    public override string GetName() { return "Create Neighbor Terrains"; }

    public override string GetDesc() {
      return "Click the edges to create neighbor terrains";
    }

    public override void OnEnable() { LoadInspectorSettings(); }

    public override void OnDisable() { SaveInspectorSettings(); }

    private void LoadInspectorSettings() {
      m_FillHeightmapUsingNeighbors =
          EditorPrefs.GetBool("TerrainFillHeightmapUsingNeighbors", true);
      m_FillAddressMode =
          (FillAddressMode) EditorPrefs.GetInt("TerrainFillAddressMode", 0);
    }

    private void SaveInspectorSettings() {
      EditorPrefs.SetBool("TerrainFillHeightmapUsingNeighbors",
                          m_FillHeightmapUsingNeighbors);
    ...
    public override void OnInspectorGUI(Terrain terrain,
      ...
      bool fillHeightmapUsingNeighbors = EditorGUILayout.Toggle(
          s_Styles.fillHeightmapUsingNeighbors, m_FillHeightmapUsingNeighbors);
      EditorGUI.BeginDisabledGroup(!fillHeightmapUsingNeighbors);
      FillAddressMode fillAddressMode =
          (FillAddressMode) EditorGUILayout.EnumPopup(s_Styles.fillAddressMode,
                                                      m_FillAddressMode);
    ...
    Terrain CreateNeighbor(Terrain parent, Vector3 position) {
      ...
      if (parent.terrainData.terrainLayers != null &&
          parent.terrainData.terrainLayers.Length > 0) {
      ...
      terrainData.SetDetailResolution(
          parent.terrainData.detailResolution,
      ...
      string parentTerrainDataDir =
          Path.GetDirectoryName(AssetDatabase.GetAssetPath(parent.terrainData));

      var assetsToSave =
          new UnityEngine.Object[1 + terrainData.alphamapTextureCount];
      ...
      AssetDatabase.CreateAssetFromObjects(
          assetsToSave,
          Path.Combine(parentTerrainDataDir,
                       "TerrainData_" + terrainData.name + ".asset"));
    ...
    private void FillHeightmapUsingNeighbors(Terrain terrain) {
      ...
      Terrain[] nbrTerrains =
          new Terrain[4]{terrain.topNeighbor, terrain.bottomNeighbor,
                         terrain.leftNeighbor, terrain.rightNeighbor};
      ...
      TerrainNeighborInfo[] nbrInfos =
          new TerrainNeighborInfo[4]{top, bottom, left, right};
      ...
      for (int i = 0; i < 4; ++i) {
        ...
        if (nbrTerrain) {
          ...
          if (nbrInfo.terrainData) {
            ...
            nbrInfo.offset = (nbrTerrain.transform.position.y -
                              terrain.transform.position.y) /
                             (nbrInfo.terrainData.size.y * kNeightNormFactor);
          }
        }
      }

      RenderTexture heightmap = terrain.terrainData.heightmapTexture;
      Vector4 texCoordOffsetScale =
          new Vector4(-0.5f / heightmap.width, -0.5f / heightmap.height,
                      (float) heightmap.width / (heightmap.width - 1),
                      (float) heightmap.height / (heightmap.height - 1));

      Material crossBlendMat = GetOrCreateCrossBlendMaterial();
      Vector4 slopeEnableFlags =
          new Vector4(bottom.texture ? 0.0f : 1.0f, top.texture ? 0.0f : 1.0f,
                      left.texture ? 0.0f : 1.0f, right.texture ? 0.0f : 1.0f);
      ...
      crossBlendMat.SetVector(
          "_Offsets",
          new Vector4(bottom.offset, top.offset, left.offset, right.offset));
      ...
      terrain.terrainData.DirtyHeightmapRegion(
          new RectInt(0, 0, heightmap.width, heightmap.height),
    ...
    public override void OnSceneGUI(Terrain terrain, IOnSceneGUI editContext) {
      ...
      TerrainUtility.TerrainMap mapGroup =
          TerrainUtility.TerrainMap.CreateFromPlacement(terrain);
      if (mapGroup == null)
        return;

      foreach (TerrainUtility.TerrainMap.TileCoord coord in mapGroup
                   .m_terrainTiles.Keys) {
        int x = coord.tileX;
        int y = coord.tileZ;
        ...
        Vector3 pos =
            t.transform.position + 0.5f * new Vector3(size.x, 0, size.z);

        if ((bottom == null) &&
            Handles.Button(pos + new Vector3(0, 0, -size.z), rot, 0.5f * size.x,
                           0.5f * size.x, Handles.RectangleHandleCapWorldSpace))
          CreateNeighbor(terrain, t.transform.position + Vector3.back * size.z);
        if ((top == null) &&
            Handles.Button(pos + new Vector3(0, 0, size.z), rot, 0.5f * size.x,
                           0.5f * size.x, Handles.RectangleHandleCapWorldSpace))
          CreateNeighbor(terrain,
                         t.transform.position + Vector3.forward * size.z);
        if ((right == null) &&
            Handles.Button(pos + new Vector3(size.x, 0, 0), rot, 0.5f * size.x,
                           0.5f * size.x, Handles.RectangleHandleCapWorldSpace))
          CreateNeighbor(terrain,
                         t.transform.position + Vector3.right * size.x);
        if ((left == null) &&
            Handles.Button(pos + new Vector3(-size.x, 0, 0), rot, 0.5f * size.x,
                           0.5f * size.x, Handles.RectangleHandleCapWorldSpace))
