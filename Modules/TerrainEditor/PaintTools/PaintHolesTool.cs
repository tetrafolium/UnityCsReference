...
namespace UnityEditor.Experimental.TerrainAPI {
  internal class PaintHolesTool : TerrainPaintTool<PaintHolesTool> {
    [Shortcut("Terrain/Paint Holes", typeof(TerrainToolShortcutContext),
              KeyCode.F8)]
    ...
    public override string GetName() { return "Paint Holes"; }
    ...
    public override bool OnPaint(Terrain terrain, IOnPaint editContext) {
      Vector2 halfTexelOffset =
          new Vector2(0.5f / terrain.terrainData.holesResolution,
                      0.5f / terrain.terrainData.holesResolution);
      BrushTransform brushXform = TerrainPaintUtility.CalculateBrushTransform(
          terrain, editContext.uv - halfTexelOffset, editContext.brushSize,
          0.0f);
      PaintContext paintContext = TerrainPaintUtility.BeginPaintHoles(
          terrain, brushXform.GetBrushXYBounds());

      Material mat = TerrainPaintUtility.GetBuiltinPaintMaterial();

      float brushStrength =
          Event.current.shift ? editContext.brushStrength
      ...
      Graphics.Blit(
          ...
          (int) TerrainPaintUtility.BuiltinPaintMaterialPasses.PaintHoles);

      TerrainPaintUtility.EndPaintHoles(paintContext,
                                        "Terrain Paint - Paint Holes");
      return true;
    }
  }
}
