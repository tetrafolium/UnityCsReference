...
namespace UnityEditor.Experimental.TerrainAPI {
  internal class BrushRep {
    ...
    public void CreateFromBrush(Texture2D brushTex, int size) {
      ...
      if (mask != null) {
        ...
        if (!mask.isReadable) {
          readableTexture = new Texture2D(mask.width, mask.height, mask.format,
                                          mask.mipmapCount > 1);
        ...
        if (m_Size > kMinBrushSize) {
          for (int y = 0; y < m_Size; y++) {
            ...
            for (int x = 0; x < m_Size; x++) {
              float u = x / fSize;
              m_Strength[y * m_Size + x] =
                  readableTexture.GetPixelBilinear(u, v).r;
            }
          }
        } else {
  ...
  internal class PaintDetailsUtils {
    public static int FindDetailPrototype(Terrain terrain,
      ...
      DetailPrototype sourceDetailPrototype =
          sourceTerrain.terrainData.detailPrototypes[sourceDetail];
      for (int i = 0; i < terrain.terrainData.detailPrototypes.Length; ++i) {
        if (sourceDetailPrototype.Equals(
                terrain.terrainData.detailPrototypes[i]))
          return i;
      }
    ...
    public static int CopyDetailPrototype(Terrain terrain,
                                          Terrain sourceTerrain,
                                          int sourceDetail) {
      DetailPrototype sourceDetailPrototype =
          sourceTerrain.terrainData.detailPrototypes[sourceDetail];
      DetailPrototype[] newDetailPrototypesArray =
          new DetailPrototype[terrain.terrainData.detailPrototypes.Length + 1];
      System.Array.Copy(terrain.terrainData.detailPrototypes,
                        newDetailPrototypesArray,
                        terrain.terrainData.detailPrototypes.Length);
      newDetailPrototypesArray[newDetailPrototypesArray.Length - 1] =
          new DetailPrototype(sourceDetailPrototype);
  ...
  internal class PaintDetailsTool : TerrainPaintTool<PaintDetailsTool> {
    ...
    public float detailStrength {
      get { return m_DetailsStrength; }
      set {
        m_DetailsStrength =
            Mathf.Clamp01(Mathf.Round(value * TerrainData.maxDetailsPerRes) /
                          TerrainData.maxDetailsPerRes);
      }
    }
    ...
    public override bool OnPaint(Terrain terrain, IOnPaint editContext) {
      if (m_TargetTerrain == null || selectedDetail == kInvalidDetail ||
          selectedDetail >=
              m_TargetTerrain.terrainData.detailPrototypes.Length) {
        return false;
      }
      ...
      for (int t = 0; t < ctx.terrains.Length; ++t) {
        ...
        if (ctxTerrain != null) {
          int detailPrototype = PaintDetailsUtils.FindDetailPrototype(
              ctxTerrain, m_TargetTerrain, selectedDetail);
          if (detailPrototype == kInvalidDetail) {
            detailPrototype = PaintDetailsUtils.CopyDetailPrototype(
                ctxTerrain, m_TargetTerrain, selectedDetail);
          }
          ...
          TerrainPaintUtilityEditor.UpdateTerrainDataUndo(
              terrainData, "Terrain - Detail Edit");

          int size =
              (int) Mathf.Max(1.0f, editContext.brushSize *
                                        ((float) terrainData.detailResolution /
                                         terrainData.size.x));
          ...
          if (xmin >= terrainData.detailWidth ||
              ymin >= terrainData.detailHeight || xmax <= 0 || ymax <= 0) {
            continue;
          }
          ...
          for (int i = 0; i < layers.Length; i++) {
            int[, ] alphamap = terrainData.GetDetailLayer(xmin, ymin, width,
                                                          height, layers[i]);

            for (int y = 0; y < height; y++) {
              for (int x = 0; x < width; x++) {
                int xBrushOffset =
                    (xmin + x) - (xCenter - intRadius + intFraction);
                int yBrushOffset =
                    (ymin + y) - (yCenter - intRadius + intFraction);
                float opa = detailOpacity * m_BrushRep.GetStrengthInt(
                                                xBrushOffset, yBrushOffset);

                float targetValue = Mathf.Lerp(
                    alphamap[y, x],
                    targetStrength * TerrainData.maxDetailsPerRes, opa);
                alphamap[y, x] = Mathf.Min(
                    Mathf.RoundToInt(targetValue - .5f + Random.value),
                    TerrainData.maxDetailsPerRes);
              }
            }
    ...
    public override void OnEnterToolMode() {
      ...
      if (terrain != null && terrain.terrainData != null &&
        ...
        for (int i = 0; i < terrain.terrainData.detailPrototypes.Length; ++i) {
          if (m_LastSelectedDetailPrototype.Equals(
                  terrain.terrainData.detailPrototypes[i])) {
            selectedDetail = i;
            break;
          }
    ...
    public override void OnExitToolMode() {
      if (m_TargetTerrain != null && m_TargetTerrain.terrainData != null &&
          ...
          selectedDetail <
              m_TargetTerrain.terrainData.detailPrototypes.Length) {
        m_LastSelectedDetailPrototype = new DetailPrototype(
            m_TargetTerrain.terrainData.detailPrototypes[selectedDetail]);
      }

      selectedDetail = kInvalidDetail;
    }

    public override string GetName() { return "Paint Details"; }
