...
namespace UnityEditor.Experimental.TerrainAPI {
  ...
  public interface IOnPaint {
    ...
    [Obsolete(
        "IOnPaint.RepaintAllInspectors has been deprecated. Use IOnPaint.Repaint instead")]
  ...
  public abstract class TerrainPaintTool<T> : ScriptableSingleton<T>,
                                              ITerrainPaintTool where T
      : TerrainPaintTool<T> {
  ...
  internal class OnPaintContext : IOnPaint {
    ...
    public OnPaintContext(RaycastHit raycastHit, Texture brushTexture,
                          Vector2 uv, float brushStrength, float brushSize) {
      Set(false, raycastHit, brushTexture, uv, brushStrength, brushSize);
    }

    public OnPaintContext Set(bool hitValidTerrain, RaycastHit raycastHit,
                              Texture brushTexture, Vector2 uv,
                              float brushStrength, float brushSize) {
    ...
    public bool hitValidTerrain {
      get { return m_HitValidTerrain; }
    }
    public RaycastHit raycastHit {
      get { return m_RaycastHit; }
    }

    public void RepaintAllInspectors() { Repaint(RepaintFlags.UI); }
  ...
  internal class OnSceneGUIContext : IOnSceneGUI {
    ...
    public OnSceneGUIContext(SceneView sceneView, RaycastHit raycastHit,
                             Texture brushTexture, float brushStrength,
                             float brushSize) {
      Set(sceneView, false, raycastHit, brushTexture, brushStrength, brushSize);
    }

    public OnSceneGUIContext Set(SceneView sceneView, bool hitValidTerrain,
                                 RaycastHit raycastHit, Texture brushTexture,
                                 float brushStrength, float brushSize) {
  ...
  internal class OnInspectorGUIContext : IOnInspectorGUI {
    ...
    public void ShowBrushesGUI(int spacing) {
      ShowBrushesGUI(spacing, BrushGUIEditFlags.All);
    }
    public void ShowBrushesGUI(int spacing, BrushGUIEditFlags flags) {
      TerrainInspector.s_activeTerrainInspectorInstance.ShowBrushes(
          spacing, (flags & BrushGUIEditFlags.Select) != 0,
          (flags & BrushGUIEditFlags.Inspect) != 0,
          (flags & BrushGUIEditFlags.Size) != 0,
          (flags & BrushGUIEditFlags.Opacity) != 0, 0);
    }
    ...
    public void ShowBrushesGUI(int spacing, BrushGUIEditFlags flags,
      ...
      TerrainInspector.s_activeTerrainInspectorInstance.ShowBrushes(
          ...
          (flags & BrushGUIEditFlags.Inspect) != 0,
          (flags & BrushGUIEditFlags.Size) != 0,
          (flags & BrushGUIEditFlags.Opacity) != 0, textureResolutionPerTile);
    }
