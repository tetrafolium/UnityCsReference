...
namespace UnityEditor.Experimental.TerrainAPI {
  [FilePath("Library/TerrainTools/SetHeight",
            FilePathAttribute.Location.ProjectFolder)]
  internal class SetHeightTool : TerrainPaintTool<SetHeightTool> {
    private enum HeightSpace { World, Local }
    ...
    class Styles {
      ...
      public readonly GUIContent space = EditorGUIUtility.TrTextContent(
          "Space", "The heightmap space in which the painting operates.");
      public readonly GUIContent height = EditorGUIUtility.TrTextContent(
          "Height",
          "You can set the Height property manually or you can shift-click on the terrain to sample the height at the mouse position (rather like the 'eyedropper' tool in an image editor).");
      public readonly GUIContent flatten = EditorGUIUtility.TrTextContent(
          "Flatten Tile",
          "The Flatten button levels the whole terrain to the chosen height.");
      public readonly GUIContent flattenAll = EditorGUIUtility.TrTextContent(
          "Flatten All",
          "If selected, it will traverse all neighbors and flatten them too");
    }
    ...
    public override string GetName() { return toolName; }

    public override string GetDesc() { return GetStyles().description.text; }
      ...
      if (editContext.hitValidTerrain) {
        ...
        // draw result preview
        {
          ApplyBrushInternal(paintContext, editContext.brushStrength,
                             editContext.brushTexture, brushXform, terrain);
    ...
    private void ApplyBrushInternal(PaintContext paintContext,
      ...
      float brushTargetHeight =
          Mathf.Clamp01((m_TargetHeight - paintContext.heightWorldSpaceMin) /
                        paintContext.heightWorldSpaceSize);

      Vector4 brushParams = new Vector4(
          brushStrength * 0.01f,
          PaintContext.kNormalizedHeightScale * brushTargetHeight, 0.0f, 0.0f);
      ...
      Graphics.Blit(
          ...
          (int) TerrainPaintUtility.BuiltinPaintMaterialPasses.SetHeights);
    }

    public override bool OnPaint(Terrain terrain, IOnPaint editContext) {
      if (Event.current.shift) {
        m_TargetHeight = terrain.terrainData.GetInterpolatedHeight(
    ...
    void Flatten(Terrain terrain) {
      ...
      float terrainHeight =
          Mathf.Clamp01((m_TargetHeight - terrain.transform.position.y) /
    ...
    public override void OnInspectorGUI(Terrain terrain,
      ...
      EditorGUI.BeginChangeCheck();
      m_HeightSpace =
          (HeightSpace) EditorGUILayout.EnumPopup(styles.space, m_HeightSpace);
      if (EditorGUI.EndChangeCheck()) {
        if (m_HeightSpace == HeightSpace.Local)
          m_TargetHeight =
              Mathf.Clamp(m_TargetHeight, terrain.GetPosition().y,
                          terrain.terrainData.size.y + terrain.GetPosition().y);
      }

      if (m_HeightSpace == HeightSpace.Local)
        m_TargetHeight =
            EditorGUILayout.Slider(styles.height,
                                   m_TargetHeight - terrain.GetPosition().y, 0,
                                   terrain.terrainData.size.y) +
            terrain.GetPosition().y;
      else
        m_TargetHeight =
            EditorGUILayout.FloatField(styles.height, m_TargetHeight);
      if (EditorGUI.EndChangeCheck())
        Save(true);
