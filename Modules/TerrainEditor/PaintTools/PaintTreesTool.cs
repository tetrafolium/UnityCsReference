...
namespace UnityEditor.Experimental.TerrainAPI {
  internal class PaintTreesUtils {
    public static bool ValidateTreePrototype(Terrain terrain,
                                             int treePrototype) {
      int prototypeCount =
          TerrainInspectorUtil.GetPrototypeCount(terrain.terrainData);
      if (treePrototype == PaintTreesTool.kInvalidTree ||
          treePrototype >= prototypeCount)
        return false;

      if (!TerrainInspectorUtil.PrototypeIsRenderable(terrain.terrainData,
                                                      treePrototype))
        return false;
    ...
    public static int FindTreePrototype(Terrain terrain, Terrain sourceTerrain,
                                        int sourceTree) {
      ...
      TreePrototype sourceTreePrototype =
          sourceTerrain.terrainData.treePrototypes[sourceTree];
    ...
    public static int CopyTreePrototype(Terrain terrain, Terrain sourceTerrain,
                                        int sourceTree) {
      TreePrototype sourceTreePrototype =
          sourceTerrain.terrainData.treePrototypes[sourceTree];
      TreePrototype[] newTreePrototypesArray =
          new TreePrototype[terrain.terrainData.treePrototypes.Length + 1];
      System.Array.Copy(terrain.terrainData.treePrototypes,
                        newTreePrototypesArray,
                        terrain.terrainData.treePrototypes.Length);
      newTreePrototypesArray[newTreePrototypesArray.Length - 1] =
          new TreePrototype(sourceTreePrototype);
      terrain.terrainData.treePrototypes = newTreePrototypesArray;
      terrain.terrainData.RefreshPrototypes();
      return newTreePrototypesArray.Length - 1;
    }

    public static void PlaceTree(Terrain terrain, int treePrototype,
                                 Vector3 position, Color color, float height,
  ...
  internal class PaintTreesTool : TerrainPaintTool<PaintTreesTool> {
    ...
    = true;
    public bool allowHeightVar {
      get;
      set;
    }
    = true;
    public bool allowWidthVar {
      get;
      set;
    }
    = true;
    public float treeColorAdjustment {
      get;
      set;
    }
    = .4f;
    ...
    = 1;
    public float treeHeightVariation {
      get;
      set;
    }
    = .1f;
    ...
    = 1;
    public float treeWidthVariation {
      get;
      set;
    }
    = .1f;
    ...
    private void PlaceTrees(Terrain terrain, IOnPaint editContext) {
      if (m_TargetTerrain == null || selectedTree == kInvalidTree ||
          selectedTree >= m_TargetTerrain.terrainData.treePrototypes.Length) {
        return;
      }

      PaintTreesDetailsContext ctx =
          PaintTreesDetailsContext.Create(terrain, editContext.uv);

      int placedTreeCount = 0;

      int treePrototype = PaintTreesUtils.FindTreePrototype(
          terrain, m_TargetTerrain, selectedTree);
      if (treePrototype == kInvalidTree) {
        treePrototype = PaintTreesUtils.CopyTreePrototype(
            terrain, m_TargetTerrain, selectedTree);
      }
      ...
      if (PaintTreesUtils.ValidateTreePrototype(terrain, treePrototype)) {
        ...
        bool checkTreeDistance =
            Event.current.type == EventType.MouseDrag || brushSize > 1;
        if (!checkTreeDistance ||
            TerrainInspectorUtil.CheckTreeDistance(
                terrain.terrainData, position, treePrototype, spacing)) {
          TerrainPaintUtilityEditor.UpdateTerrainDataUndo(
              terrain.terrainData, "Terrain - Place Trees");

          PaintTreesUtils.PlaceTree(terrain, treePrototype, position,
                                    GetTreeColor(), GetTreeHeight(),
                                    lockWidthToHeight ? GetTreeHeight()
                                    : GetTreeWidth(), GetTreeRotation());
          ++placedTreeCount;
        }
      }
      ...
      for (int i = 0; i < ctx.terrains.Length; ++i) {
        ...
        if (ctxTerrain != null) {
          ...
          treePrototype = PaintTreesUtils.FindTreePrototype(
              ctxTerrain, m_TargetTerrain, selectedTree);
          if (treePrototype == kInvalidTree) {
            treePrototype = PaintTreesUtils.CopyTreePrototype(
                ctxTerrain, m_TargetTerrain, selectedTree);
          }

          if (PaintTreesUtils.ValidateTreePrototype(ctxTerrain,
                                                    treePrototype)) {
            Vector3 size = TerrainInspectorUtil.GetPrototypeExtent(
                ctxTerrain.terrainData, treePrototype);
            size.y = 0;
            float treeCountOneAxis =
                brushSize / (size.magnitude * spacing * .5f);
            int treeCount = (int)((treeCountOneAxis * treeCountOneAxis) * .5f);
            treeCount = Mathf.Clamp(treeCount, 0, 100);
            // Plant a bunch of trees
            for (int j = ctxTerrain == terrain ? 1 : 0;
                 j < treeCount && placedTreeCount < treeCount; ++j) {
              ...
              Vector3 position = new Vector3(ctxUV.x + randomOffset.x, 0,
                                             ctxUV.y + randomOffset.y);
              if (position.x >= 0 && position.x <= 1 && position.z >= 0 &&
                  position.z <= 1 &&
                  TerrainInspectorUtil.CheckTreeDistance(
                      ctxTerrain.terrainData, position, treePrototype,
                      spacing * .5f)) {
                TerrainPaintUtilityEditor.UpdateTerrainDataUndo(
                    ctxTerrain.terrainData, "Terrain - Place Trees");

                PaintTreesUtils.PlaceTree(ctxTerrain, treePrototype, position,
                                          GetTreeColor(), GetTreeHeight(),
                                          lockWidthToHeight ? GetTreeHeight()
                                          : GetTreeWidth(), GetTreeRotation());
                ++placedTreeCount;
      ...
      for (int i = 0; i < ctx.terrains.Length; ++i) {
        ...
        if (ctxTerrain != null) {
          ...
          if (!clearSelectedOnly || treePrototype != kInvalidTree) {
            TerrainPaintUtilityEditor.UpdateTerrainDataUndo(
                ctxTerrain.terrainData, "Terrain - Remove Trees");
    ...
    public void MassPlaceTrees(TerrainData terrainData, int numberOfTrees,
                               bool randomTreeColor, bool keepExistingTrees) {
      ...
      while (i < instances.Length) {
        ...
        if (terrainData.GetSteepness(instance.position.x, instance.position.z) <
            30) {
          ...
          instance.widthScale =
              lockWidthToHeight ? instance.heightScale : GetTreeWidth();
      ...
      if (keepExistingTrees) {
        ...
        var allTrees =
            new TreeInstance[existingTrees.Length + instances.Length];
        System.Array.Copy(existingTrees, 0, allTrees, 0, existingTrees.Length);
        System.Array.Copy(instances, 0, allTrees, existingTrees.Length,
                          instances.Length);
        instances = allTrees;
      }
    ...
    public override void OnEnterToolMode() {
      ...
      if (terrain != null && terrain.terrainData != null &&
        ...
        for (int i = 0; i < terrain.terrainData.treePrototypes.Length; ++i) {
          if (m_LastSelectedTreePrototype.Equals(
                  terrain.terrainData.treePrototypes[i])) {
            selectedTree = i;
            break;
          }
        }
    ...
    public override void OnExitToolMode() {
      if (m_TargetTerrain != null && m_TargetTerrain.terrainData != null &&
          ...
          selectedTree < m_TargetTerrain.terrainData.treePrototypes.Length) {
        m_LastSelectedTreePrototype = new TreePrototype(
            m_TargetTerrain.terrainData.treePrototypes[selectedTree]);
      }

      selectedTree = kInvalidTree;
    }

    public override string GetName() { return "Paint Trees"; }
    ...
    public override void OnSceneGUI(Terrain terrain, IOnSceneGUI editContext) {
      ...
      if (editContext.hitValidTerrain) {
        BrushTransform brushXform = TerrainPaintUtility.CalculateBrushTransform(
            terrain, editContext.raycastHit.textureCoord, brushSize, 0.0f);
