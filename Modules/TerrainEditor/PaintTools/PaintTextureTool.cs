...
namespace UnityEditor.Experimental.TerrainAPI {
  [FilePathAttribute("Library/TerrainTools/PaintTexture",
  ...
  internal class PaintTextureTool : TerrainPaintTool<PaintTextureTool> {
    ...
    [SerializeField] TerrainLayer m_SelectedTerrainLayer = null;

    // Keep this separate from m_SelectedTerrainLayer so that it allows
    // selecting null TerrainLayers (like those deleted from Assets).
    ...
    public override void OnEnterToolMode() { m_SelectedTerrainLayerIndex = -1; }
    ...
    public override bool OnPaint(Terrain terrain, IOnPaint editContext) {
      ...
      PaintContext paintContext = TerrainPaintUtility.BeginPaintTexture(
          terrain, brushXform.GetBrushXYBounds(), m_SelectedTerrainLayer);
      ...
      // apply brush
      float targetAlpha = 1.0f; // always 1.0 now -- no subtractive painting (we
                                // assume this in the ScatterAlphaMap)
      Vector4 brushParams =
          new Vector4(editContext.brushStrength, targetAlpha, 0.0f, 0.0f);
      ...
      Graphics.Blit(
          ...
          (int) TerrainPaintUtility.BuiltinPaintMaterialPasses.PaintTexture);
    ...
    public override void OnInspectorGUI(Terrain terrain,
      ...
      Editor.DrawFoldoutInspector(terrain.materialTemplate,
                                  ref m_TemplateMaterialEditor);
      EditorGUIUtility.fieldWidth = cacheFieldWidth;
      EditorGUIUtility.labelWidth = cacheLabelWIdth;
      EditorGUILayout.Space();

      if (m_SelectedTerrainLayerIndex == -1)
        m_SelectedTerrainLayerIndex = TerrainPaintUtility.FindTerrainLayerIndex(
            terrain, m_SelectedTerrainLayer);
      ...
      if (EditorGUI.EndChangeCheck()) {
        m_SelectedTerrainLayer =
            m_SelectedTerrainLayerIndex != -1
                ? terrain.terrainData.terrainLayers[m_SelectedTerrainLayerIndex]
                : null;
        Save(true);
      }

      terrain.materialTemplate.SetFloat(
          "_NumLayersCount", terrain.terrainData.terrainLayers.Length);
