...
namespace UnityEditor.Experimental.TerrainAPI {
  [FilePathAttribute("Library/TerrainTools/Stamp",
  ...
  internal class StampTool : TerrainPaintTool<StampTool> {
    ...
    class Styles {
      public readonly GUIContent description = EditorGUIUtility.TrTextContent(
          "Left click to stamp the brush onto the terrain.\n\nHold control and mousewheel to adjust height.\nHold shift to invert the stamp.");
      public readonly GUIContent height = EditorGUIUtility.TrTextContent(
          "Stamp Height",
          "You can set the Stamp Height manually or you can hold shift and mouse wheel on the terrain to adjust it.");
      public readonly GUIContent down = EditorGUIUtility.TrTextContent(
          "Subtract", "Subtract the stamp from the terrain.");
      public readonly GUIContent maxadd = EditorGUIUtility.TrTextContent(
          "Max <--> Add",
          "Blend between adding the heights and taking the maximum.");
    }
    ...
    private void ApplyBrushInternal(PaintContext paintContext,
                                    ...
                                    BrushTransform brushXform, Terrain terrain,
                                    bool negate) {
      ...
      Vector4 brushParams =
          new Vector4(brushStrength, 0.0f, height, m_MaxBlendAdd);
      ...
      Graphics.Blit(
          ...
          (int) TerrainPaintUtility.BuiltinPaintMaterialPasses.StampHeight);
    }
    ...
    public override void OnSceneGUI(Terrain terrain, IOnSceneGUI editContext) {
      ...
      if (evt.control && (evt.type == EventType.ScrollWheel)) {
        ...
        // we use distance to modify the scroll speed, so that when a user is up
        // close to the brush, they get fine adjustment, and when the user is
        // far from the brush, it adjusts quickly
        m_StampHeightTerrainSpace += Event.current.delta.y *
                                     k_mouseWheelToHeightRatio *
                                     editContext.raycastHit.distance;
        evt.Use();
        editContext.Repaint();
      }
    ...
    public override void OnInspectorGUI(Terrain terrain,
      ...
      EditorGUI.BeginChangeCheck();
      {
        ...
        height = EditorGUILayout.PowerSlider(styles.height, height, 0,
                                             terrain.terrainData.size.y, 2.0f);
      ...
      m_MaxBlendAdd =
          EditorGUILayout.Slider(styles.maxadd, m_MaxBlendAdd, 0.0f, 1.0f);
      if (EditorGUI.EndChangeCheck()) {
        Save(true);
      }
