...
internal unsafe struct Handle : IHandle<Handle> {
		...
		set {
			...
			if (!Valid)
				throw new InvalidOperationException(
					      "Handle is invalid or has been destroyed");
			if (AtomicNode->Id != Node.InvalidId)
				throw new InvalidOperationException(
					      $"Trying to overwrite id on live node {AtomicNode->Id}");
			AtomicNode->Id = value;
		}
	}
	...
	public Handle(Node *node) {
		...
		if (node->Id != Node.InvalidId)
			throw new InvalidOperationException(
				      $"Reusing unflushed node {node->Id}");
	...
	public void FlushNode() {
		if (!Valid)
			throw new InvalidOperationException(
				      "Attempting to flush invalid audio handle");
	...
	public override int GetHashCode() {
		unchecked { return ((int) m_Node * 397) ^ Version; }
	}

	public bool Valid =>
	m_Node != IntPtr.Zero && AtomicNode->Version == Version;
