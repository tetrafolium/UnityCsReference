...
namespace UnityEngine.XR.WSA {
  ...
  public class WorldManager {
    ...
    OnPositionalLocatorStateChangedDelegate(PositionalLocatorState oldState,
                                            PositionalLocatorState newState);

    // Notification on when the world tracking systems state has changed
    public static event OnPositionalLocatorStateChangedDelegate
        OnPositionalLocatorStateChanged;

    // If the user has registered notify of tracking loss
    [RequiredByNativeCode]
    private static void Internal_TriggerPositionalLocatorStateChanged(
    ...
    public static PositionalLocatorState state {
      get { return PositionalLocatorState.Unavailable; }
    }
