...
public class TreeData : ScriptableObject {
  ...
  // Get at index
  //
  private int GetNodeCount() { return nodes.Length; }
    ...
    for (int i = 0; i < fields.Length; i++) {
      if (fields[i].IsPublic) {
        if (fields[i].FieldType == typeof(TreeSpline)) {
          // Tree Spline
          TreeSpline origSpline = fields [i]
                                      .GetValue(n) as TreeSpline;
          fields [i]
              .SetValue(n2, new TreeSpline(origSpline));
        } else if (fields[i].FieldType == typeof(AnimationCurve)) {
          // Animation curve
          AnimationCurve origCurve = fields [i]
                                         .GetValue(n) as AnimationCurve;
          ...
          fields [i]
              .SetValue(n2, newCurve);
        } else {
          fields [i]
              .SetValue(n2, fields [i]
                                .GetValue(n));
        }
      }
    }
  ...
  public void SetGroupParent(TreeGroup g, TreeGroup parent) {
    ...
    if (origParent != null) {
      origParent.childGroupIDs =
          ArrayRemove(origParent.childGroupIDs, g.uniqueID);
  ...
  public void LockGroup(TreeGroup g) { g.Lock(); }
  ...
  public void DeleteNode(TreeNode n) { DeleteNode(n, true); }
  ...
  public void DeleteNode(TreeNode n, bool validate) {
    ...
    if (group != null) {
      ...
      for (int i = 0; i < group.childGroups.Count; i++) {
        ...
        // go backwards through list to make deletion a little cleaner. (would
        // require stepping j back on deletion otherwise)
        for (int j = kidGroup.nodes.Count - 1; j >= 0; j--) {
          if ((kidGroup.nodes[j] != null) &&
              (kidGroup.nodes[j].parentID == n.uniqueID)) {
  ...
  static public List<float> GetAdaptiveSamples(TreeGroup group, TreeNode node,
    ...
    float thresholdA =
        Mathf.Lerp(0.999f, 0.99999f, adaptiveQuality); // for optimize
    float thresholdB =
        Mathf.Lerp(0.5f, 0.985f, adaptiveQuality); // for subdivision
    float thresholdR = Mathf.Lerp(
        0.3f * rr, 0.1f * rr, adaptiveQuality); // for radius (un-affected by
                                                // actual radius, only curve)
    ...
    while (first < samplePoints.Count - 1) {
      for (int i = first; i < samplePoints.Count - 1; i++) {
        ...
        if (Vector3.Dot(upA, upB) < thresholdB)
          needSubDivision = true;
        if (Vector3.Dot(rightA, rightB) < thresholdB)
          needSubDivision = true;
        if (Vector3.Dot(frontA, frontB) < thresholdB)
          needSubDivision = true;
    ...
    if (node.capRange > 0.0f) {
      int capLoops =
          1 + Mathf.CeilToInt(node.capRange * 16.0f * adaptiveQuality);
  ...
  public void PreviewMesh(Matrix4x4 worldToLocalMatrix,
  ...
  public void UpdateMesh(Matrix4x4 worldToLocalMatrix,
                         out Material[] outMaterials) {
    outMaterials = null;
    ...
    // Update TreeData...
    UpdateMesh(worldToLocalMatrix, materials, verts, tris, aoSpheres,
               buildFlags, root.adaptiveLODQuality, root.aoDensity);

    // clear previous mesh data
    if (verts.Count > 65000) {
      // Too much to build
      Debug.LogWarning(
          "Tree mesh would exceed maximum vertex limit .. aborting");
      return;
    }
    mesh.Clear();
    ...
    // Copy mesh data to static arrays.. surely there must be a faster way to do
    // this
    ...
    List<Material> usedOptimizedMaterials =
        new List<Material>(kMaxOptimizedMaterials);
  ...
  private static void
  ExtractOptimizedShaders(List<TreeMaterial> materials,
                          out Shader optimizedSolidShader,
                          out Shader optimizedCutoutShader) {
    ...
    if (barkShaders.Count > 0)
      optimizedSolidShader =
          Shader.Find(TreeEditorHelper.GetOptimizedShaderName(barkShaders[0]));

    if (leafShaders.Count > 0)
      optimizedCutoutShader =
          Shader.Find(TreeEditorHelper.GetOptimizedShaderName(leafShaders[0]));
  ...
  public bool OptimizeMaterial(List<TreeMaterial> materials,
    ...
    ExtractOptimizedShaders(materials, out optimizedSolidShader,
                            out optimizedCutoutShader);
    ...
    for (int i = 0; i < materials.Count; i++) {
      ...
      // Debug.Log("material " + i + " area = " + materialArea[i] + " tileV = "
      // + materialTileV[i]);
    }
    ...
    for (int i = 0; i < materials.Count; i++) {
      ...
      atlas.AddTexture("tex" + i, diffuseTex, diffuseColor, normalTex, glossTex,
                       translucencyTex, shadowOffsetTex, shininess, scale, tile,
                       uvtiling);
    }
    ...
    for (int t = 0; t < triangles.Count; t++) {
      ...
      for (int v = 0; v < 3; v++) {
        ...
        if (!vertex.flag) {
          ...
          vertex.uv0.y =
              ((uvRect.y + vertex.uv0.y * uvRect.height) * uvTiling.y);
          vertex.flag = true;
        }
      }
  ...
  static Texture2D[] WriteOptimizedTextures(string treeAssetPath,
                                            Texture2D[] textures) {
    string[] fileNames = new string[textures.Length];

    string folderPath = Path.Combine(
        Path.GetDirectoryName(treeAssetPath),
        Path.GetFileNameWithoutExtension(treeAssetPath) + "_Textures");

    Directory.CreateDirectory(folderPath);

    // Write pngs into asset folder
    for (int i = 0; i < textures.Length; i++) {
      byte[] bytes = textures [i]
                         .EncodeToPNG();
    ...
    for (int i = 0; i < textures.Length; i++) {
      var textureImporter =
          AssetImporter.GetAtPath(fileNames[i]) as TextureImporter;
    ...
    for (int i = 0; i < textures.Length; i++) {
      textures[i] =
          AssetDatabase.LoadMainAssetAtPath(fileNames[i]) as Texture2D;
    }
  ...
  private void UpdateShadowTexture(Texture2D shadowTexture, int texWidth,
                                   int texHeight) {
    if (!shadowTexture)
      return;
    ...
    int shadowTextureSize = Mathf.Max(
        8, Mathf.ClosestPowerOfTwo((int)(Mathf.Min(texWidth, texHeight)) /
                                   divisors[root.shadowTextureQuality]));
  ...
  private bool UpdateTextures(TextureAtlas atlas,
    ...
    string hash = UnityEditorInternal.InternalEditorUtility
                      .CalculateHashForObjectsAndDependencies(materialArray);
    hash += atlas.GetHashCode();

    // early out if the source materials / textures match and all optimized
    // materials are hooked up
    if (materialHash == hash && allTexturesAvailable) {
      UpdateShadowTexture(optimizedCutoutMaterial.GetTexture("_ShadowTex")
                              as Texture2D,
    ...
    Texture2D diffuseTexture =
        new Texture2D(texWidth, texHeight, TextureFormat.ARGB32, true);
    Texture2D shadowTexture =
        new Texture2D(texWidth, texHeight, TextureFormat.RGB24, true);
    ...
    Texture2D defaultTranslucencyTex =
        new Texture2D(1, 1, TextureFormat.ARGB32, false, true);
    defaultTranslucencyTex.SetPixel(0, 0, Color.white);
    defaultTranslucencyTex.Apply();

    Material textureCombinerMaterial = EditorGUIUtility.LoadRequired(
        "Inspectors/TreeCreator/TreeTextureCombinerMaterial.mat") as Material;
    ...
    for (int blitMode = 0; blitMode < 4; blitMode++) {
      ...
      textureCombinerMaterial.SetVector("_TexSize",
                                        new Vector4(texWidth, texHeight, 0, 0));
      ...
      for (int i = 0; i < atlas.nodes.Count; i++) {
        ...
        case 0:
          ...
          if (rgbtex == null)
            rgbtex = defaultNormalTex;
          if (alphatex == null)
            alphatex = defaultShadowOffsetTexture;
          break;
        case 1:
          rgbtex = node.diffuseTexture;
          color = node.diffuseColor;
          if (rgbtex == null)
            rgbtex = defaultDiffuseTex;
          break;
        case 2:
          rgbtex = node.translucencyTexture;
          alphatex = node.glossTexture;
          if (rgbtex == null)
            rgbtex = defaultTranslucencyTex;
          if (alphatex == null)
            alphatex = defaultGlossTexture;
          break;
        case 3:
          alphatex = node.diffuseTexture;
          if (alphatex == null)
            alphatex = defaultDiffuseTex;
          break;
        }
        ...
        if (node.tileV) {
          ...
          for (float x = pushX; x > 0.0f; x--) {
            ...
            DrawTexture(leftRect, rgbtex, alphatex, textureCombinerMaterial,
                        color, blitMode);
            DrawTexture(rightRect, rgbtex, alphatex, textureCombinerMaterial,
                        color, blitMode);
          }
        }

        DrawTexture(nodeRect, rgbtex, alphatex, textureCombinerMaterial, color,
                    blitMode);
      }
      ...
      case 1:
        ...
        break;
      case 2:
        translucencyTexture.ReadPixels(new Rect(0, 0, texWidth, texHeight), 0,
                                       0);
    ...
    textures =
        WriteOptimizedTextures(AssetDatabase.GetAssetPath(this), textures);
  ...
  private void DrawTexture(Rect rect, Texture rgbTexture, Texture alphaTexture,
                           Material material, Color color, int pass) {
  ...
  public void UpdateMesh(Matrix4x4 matrix, List<TreeMaterial> materials,
    ...
    // gen mesh data
    root.UpdateMesh(materials, verts, tris, aoSpheres, buildFlags,
                    root.adaptiveLODQuality, root.aoDensity);
  ...
  private TreeGroupBranch[] ArrayAdd(TreeGroupBranch[] array,
                                     TreeGroupBranch value) {
  ...
  private TreeGroupBranch[] ArrayRemove(TreeGroupBranch[] array,
                                        TreeGroupBranch value) {
    List<TreeGroupBranch> temp = new List<TreeGroupBranch>(array);
    temp.Remove(value);
    return temp.ToArray();
  ...
  private TreeGroupLeaf[] ArrayRemove(TreeGroupLeaf[] array,
                                      TreeGroupLeaf value) {
    List<TreeGroupLeaf> temp = new List<TreeGroupLeaf>(array);
    temp.Remove(value);
    return temp.ToArray();
