...
public class TreeEditorHelper {
  ...
  private const string kDefaultLeafShaderName =
      "Nature/Tree Creator Leaves Fast";
  private const string kDefaultOptimizedBarkShaderName =
      "Hidden/Nature/Tree Creator Bark Optimized";
  private const string kDefaultOptimizedLeafShaderName =
      "Hidden/Nature/Tree Creator Leaves Optimized";

  private const string kOptimizedShaderDependency = "OptimizedShader";

  private readonly Dictionary<string, AnimBool> m_AnimBools =
      new Dictionary<string, AnimBool>();
  ...
  private readonly Dictionary<string, int> m_SelectedShader =
      new Dictionary<string, int>();

  private TreeData m_TreeData;

  internal static Shader DefaultOptimizedBarkShader {
    get { return Shader.Find(kDefaultOptimizedBarkShaderName); }
  }
  internal static Shader DefaultOptimizedLeafShader {
    get { return Shader.Find(kDefaultOptimizedLeafShaderName); }
  }

  static readonly Dictionary<string, GUIContent> s_Dictionary =
  ...
  private bool GUITooManyShaders(NodeType nodeType) {
    ...
    GUIContent message =
        nodeType ==
        NodeType.BarkNode ? GetGUIContent(
            "This tree uses multiple bark shaders but only one bark shader can be used on a tree. Select which bark shader to apply to all the bark materials used by this tree.|")
        : GetGUIContent(
              "This tree uses multiple leaf shaders but only one leaf shader can be used on a tree. Select which leaf shader to apply to all the leaf materials used by this tree.|");
    GUIContent button = GetGUIContent(
        "Apply|Will change the shader in all the materials on that node type to the one selected.");

    int selectedIndex = GUIShowError(uniqueID, shaders, message, button,
                                     ConsoleWindow.iconError);
  ...
  public bool GUIWrongShader(string uniqueID, Material value,
                             NodeType nodeType) {
    GUIContent message = GetGUIContent(
        "This material does not use a tree shader. A tree shader has to contain the word 'leaves' or 'bark' in the name and the line: Dependency \"OptimizedShader\" = \"OPTIMIZED_SHADER_NAME\" in the code.|");
    GUIContent button = GetGUIContent(
        "Apply|Will change the shader in the material to the one selected.");
    ...
    int selectedIndex = GUIShowError(uniqueID, recommendedShaders, message,
                                     button, ConsoleWindow.iconError);
  ...
  private int GUIShowError(string uniqueID, List<string> list,
                           GUIContent message, GUIContent button,
                           Texture2D icon) {
    int result = -1;
    if (m_AnimBools.ContainsKey(uniqueID)) {
      if (EditorGUILayout.BeginFadeGroup(m_AnimBools[uniqueID].faded)) {
        ...
        int selectedIndex =
            EditorGUILayout.Popup(GetSelectedIndex(uniqueID), list.ToArray());
  ...
  public void RefreshAllTreeShaders() {
    ...
    GetAllTreeShaders(m_TreeData, m_BarkShaders, m_LeafShaders,
                      m_TreeData.root);
  }

  private static void GetAllTreeShaders(TreeData treeData,
                                        List<string> barkShaders,
                                        List<string> leafShaders,
                                        TreeGroup group) {
  ...
  private List<string> GetRecommendedShaders(NodeType nodeType) {
    ...
    if (shaders.Count == 1 ||
        (shaders.Count == 2 && oppositeShaders.Count == 0)) {
  ...
  private static bool HasOptimizedShaderAndNameContains(Shader shader,
                                                        string name) {
  ...
  private static void AddShaderFromMaterial(Material material,
                                            List<string> barkShaders,
                                            List<string> leafShaders) {
    if (material && material.shader) {
      ...
      else if (IsTreeLeafShader(material.shader) &&
               !leafShaders.Contains(shader.name))
  ...
  private static void ChangeShaderOnMaterials(TreeData treeData, Shader shader,
                                              TreeGroup group,
                                              NodeType nodeType) {
  ...
  static public GUIContent GetGUIContent(string id) {
    ...
    GUIContent content =
        new GUIContent(ExtractLabel(uiString), ExtractTooltip(uiString));
