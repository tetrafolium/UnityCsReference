...
public void SetAnimationProperties(float primaryFactor, float secondaryFactor,
                                   float edgeFactor, float phase) {
...
public void BuildVertices(List<TreeVertex> verts) {
	...
	for (int i = 0; i <= segments; i++) {
		...
		if (spreadFactor > 0.0f) {
			...
			spreadRad =
				Mathf.Pow(Mathf.Abs(spreadFactor), 3.0f) * radius * spreadTop;
		}
		...
		v.pos = matrix.MultiplyPoint(
			new Vector3(Mathf.Sin(a) * (rad + (spreadRad * 0.25f)), 0.0f,
			            Mathf.Cos(a) * (rad + spreadRad)));

		v.uv0 = new Vector2(uvx, uvy);

		v.SetAnimationProperties(animParams.x, animParams.y, animParams.z,
		                         animParams.w);
	...
	for (int i = 0; i <= segments; i++) {
		...
		// Use the previous and the next vertices in the loop to create the
		// tangent. It will be tangent to the perfect ring loop in case of no
		// noise.
...
public void Cap(float sphereFactor, float noise, int mappingMode,
                float mappingScale, List<TreeVertex> verts,
                List<TreeTriangle> tris, int materialIndex) {
	// half number of segments for the cap, fade with spherefactor..
	int loops =
		Mathf.Max(1, (int)((segments / 2) * Mathf.Clamp01(sphereFactor)));
	int segs = segments;
	int vloops = loops;
	if (mappingMode == 1) {
	...
	Vector3 tmpTangent =
		Vector3.Normalize(matrix.MultiplyVector(Vector3.right));
	...
	for (int l = 0; l < vloops; l++) {
		...
		float normalBlend = (uvScale * Mathf.Clamp01(sphereFactor)) +
		...
		for (int i = 0; i < segs; i++) {
			...
			Vector3 uv =
				invMatrix.MultiplyPoint(copyV.pos).normalized * 0.5f * uvScale;

			TreeVertex newV = new TreeVertex();
			newV.pos = (copyV.pos * posBlend) + (centerPos * (1.0f - posBlend)) +
			           (upVector * cosine * sphereFactor * radius);
			newV.nor =
				((copyV.nor * normalBlend) + (upVector * (1.0f - normalBlend)))
				.normalized; // mix center and original vertex normal..
			newV.SetAnimationProperties(animParams.x, animParams.y, animParams.z,
			                            animParams.w);
			...
			if (mappingMode == 0) {
				...
				newV.uv0 =
					new Vector2((float)(i) / segments,
					            baseOffset + sphereFactor * cosine * mappingScale);
			}
	...
	for (int l = 0; l < loops; l++) {
		for (int i = 0; i < segs; i++) {
			if (l == (vloops - 1)) {
				...
				if (i == (segs - 1))
					v2 = voffset + (segs * l);
				tris.Add(new TreeTriangle(materialIndex, centerV, v1, v2, false,
				                          false, false));
			} else {
				...
				tris.Add(
					new TreeTriangle(materialIndex, v0, v1, v3, false, false, false));
				tris.Add(
					new TreeTriangle(materialIndex, v0, v3, v2, false, false, false));
			}
		}
	}
}

public void Connect(RingLoop other, List<TreeTriangle> tris,
                    int materialIndex, bool flipTris, bool lowres) {
	...
	if (lowres) {
		//
		// Lowres should always have an even number of vertices per ring-loop, and
		// the same amount for each loop
		//
		for (int i = 0; i < (segments / 2); i++) {
			...
			tris.Add(
				new TreeTriangle(materialIndex, a0, a1, b0, false, true, false));
			tris.Add(
				new TreeTriangle(materialIndex, a1, b1, b0, false, true, false));
		}
	} else {
		for (int i = 0; i < segments; i++) {
			// find matching vertices on the other ring loop
			int connect0 = Mathf.Min(
				(int) Mathf.Round((i / (float) segments) * other.segments),
				other.segments);
			int connect1 = Mathf.Min(
			...
			// add to triangle list
			tris.Add(
				new TreeTriangle(materialIndex, v0, v1, v2, false, true, false));
			tris.Add(
				new TreeTriangle(materialIndex, v3, v4, v5, false, true, false));
		}
	}
}
}
