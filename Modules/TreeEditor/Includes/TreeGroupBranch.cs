...
public enum GeometryMode { Branch = 0, BranchFrond = 1, Frond = 2 }

static class Styles {
public static string groupSeedString = LocalizationDatabase.GetLocalizedString(
	"Group Seed|The seed for this group of branches. Modify to vary procedural generation.");
public static string frequencyString = LocalizationDatabase.GetLocalizedString(
	"Frequency|Adjusts the number of branches created for each parent branch.");
public static string distributionModeString =
	LocalizationDatabase.GetLocalizedString(
		"Distribution|The way the branches are distributed along their parent.");
public static string twirlString = LocalizationDatabase.GetLocalizedString(
	"Twirl|Twirl around the parent branch.");
public static string whorledStepString = LocalizationDatabase.GetLocalizedString(
...
public static string growthScaleString = LocalizationDatabase.GetLocalizedString(
...
public static string growthAngleString = LocalizationDatabase.GetLocalizedString(
...
public static string mainWindString = LocalizationDatabase.GetLocalizedString(
...
public static string mainTurbulenceString =
	LocalizationDatabase.GetLocalizedString(
...
public static string edgeTurbulenceString =
	LocalizationDatabase.GetLocalizedString(
...
//[field: TreeAttribute("GeometryMode", "popup", "Branch Only,Branch +
//Fronds,Fronds Only")]
...
//[field: TreeAttribute("Radius", "slider", 0.1f, 5.0f, "radiusCurve",
//0.0f, 1.0f, "branchGeometry")]
public float radius = 0.5f;
public AnimationCurve radiusCurve = new AnimationCurve(
	new Keyframe(0, 1, -1, -1), new Keyframe(1, 0, -1, -1));

//[field: TreeAttribute("IsLengthRelative", "toggle", 0.0f, 1.0f,
//"branchGeometry")]
public bool radiusMode = true;

//[field: TreeAttribute("CapSmoothing", "slider", 0.0f, 1.0f,
//"branchGeometry")]
...
//[field: TreeAttribute("Crinklyness", "slider", 0.0f, 1.0f, "crinkCurve",
//0.0f, 1.0f, "notLocked")]
public float crinklyness = 0.1f;
public AnimationCurve crinkCurve =
	new AnimationCurve(new Keyframe(0.0f, 1.0f), new Keyframe(1.0f, 1.0f));

//[field: TreeAttribute("SeekSunGround", "slider", 0.0f, 1.0f, "seekCurve",
//-1.0f, 1.0f, "notLocked")]
...
//[field: TreeAttribute("Noise", "slider", 0.0f, 1.0f, "noiseCurve",
//0.0f, 1.0f, "branchGeometry")]
public float noise = 0.1f;
public AnimationCurve noiseCurve =
	new AnimationCurve(new Keyframe(0.0f, 1.0f), new Keyframe(1.0f, 1.0f));

//[field: TreeAttribute("NoiseScaleU", "slider", 0.0f, 1.0f,
//"branchGeometry")]
public float noiseScaleU = 0.2f;

//[field: TreeAttribute("NoiseScaleV", "slider", 0.0f, 1.0f,
//"branchGeometry")]
...
//[field: TreeAttribute("FlareRadius", "slider", 0.0f, 5.0f,
//"branchGeometry")]
public float flareSize = 0.0f;

//[field: TreeAttribute("FlareHeight", "slider", 0.01f, 1.0f,
//"branchGeometry")]
...
//[field: TreeAttribute("WeldHeight", "slider", 0.01f, 1.0f,
//"branchGeometry")]
public float weldHeight = 0.1f;

//[field: TreeAttribute("WeldSpreadTop", "slider", 0.0f, 1.0f,
//"branchGeometry")]
public float weldSpreadTop = 0.0f;

//[field: TreeAttribute("WeldSpreadBottom", "slider", 0.0f, 1.0f,
//"branchGeometry")]
...
//[field: TreeAttribute("FrondWidth", "slider", 0.1f, 10.0f, "frondCurve",
//0.0f, 1.0f)]
...
override internal bool HasExternalChanges() {
	...
	hash = UnityEditorInternal.InternalEditorUtility
	       .CalculateHashForObjectsAndDependencies(objects.ToArray());
...
private Vector3 GetFlareWeldAtTime(TreeNode node, float time) {
	...
	return new Vector3(flareFactor * flareSize,
	                   spreadFactor * weldSpreadTop * spreadMul,
	                   spreadFactor * weldSpreadBottom * spreadMul);
}

override public float GetRadiusAtTime(TreeNode node, float time,
                                      bool includeModifications) {
	// no radius when only fronds are displayed..
	if (geometryMode == GeometryMode.Frond)
		return 0.0f;
	...
	if (time > capStart) {
		// within cap area
		float angle =
			Mathf.Acos(Mathf.Clamp01((time - capStart) / node.capRange));
	...
	if (includeModifications) {
		...
		mainRadius +=
			Mathf.Max(flareWeldRad.x,
			          Mathf.Max(flareWeldRad.y, flareWeldRad.z) * 0.25f) *
			0.1f;
	}
...
override public void UpdateParameters() {
	...
	//
	if ((parentGroup == null) ||
	    (parentGroup.GetType() == typeof(TreeGroupRoot))) {
	...
	for (int n = 0; n < nodes.Count; n++) {
		...
		for (int x = 0; x < 5; x++) {
			...
			if ((Random.value <= breakingChance) && (breakingChance > 0.0f)) {
				node.breakOffset =
					breakingSpot.x + ((breakingSpot.y - breakingSpot.x) * bks);
		...
		if (!(parentGroup is TreeGroupRoot)) {
			...
			if ((node.parent != null) && (node.parent.spline != null)) {
				// Clamp to parent
				node.size = Mathf.Min(node.parent.group.GetRadiusAtTime(
							      node.parent, node.offset, false) *
				                      0.75f,
				                      node.size);
			}
		} else if (lockFlags == 0) {
			// special case for root, we don't want the radius to change if it's
		...
		/*
		   ...
		   {
		    float capRadius = Mathf.Clamp(radiusCurve.Evaluate(1.0f), 0.0f, 1.0f)
		 * node.size; float totalHeight = height * node.GetScale(); node.capRange =
		   (capRadius / totalHeight) * capSmoothing * 2.0f;
		   }
		 */
	}
...
public void UpdateSplines() {
	for (int n = 0; n < nodes.Count; n++) {
		...
		if (capSmoothing < 0.01f) {
			...
			float capRadius =
				Mathf.Clamp(radiusCurve.Evaluate(1.0f), 0.0f, 1.0f) * node.size;
			float totalHeight =
				Mathf.Max(node.spline.GetApproximateLength(), 0.00001f);
	...
	if (parentGroup == null) {
		...
		for (int i = 0; i < nodes.Count; i++) {
			...
			/*
			   ...
			   Vector3 pos = new Vector3(Mathf.Cos(ang) * dist, 0.0f, Mathf.Sin(ang) *
			   dist); Quaternion rot = Quaternion.Euler(node.pitch*-Mathf.Sin(ang), 0,
			   node.pitch * Mathf.Cos(ang)) * Quaternion.Euler(0, node.angle, 0);
		...
		for (int i = 0; i < nodes.Count; i++) {
			...
			float ang = node.angle * Mathf.Deg2Rad;
			Vector3 pos = new Vector3(Mathf.Cos(ang) * dist, -tgr.groundOffset,
			                          Mathf.Sin(ang) * dist);
			Quaternion rot = Quaternion.Euler(node.pitch * -Mathf.Sin(ang), 0,
			                                  node.pitch * Mathf.Cos(ang)) *
			                 Quaternion.Euler(0, node.angle, 0);
			node.matrix = Matrix4x4.TRS(pos, rot, Vector3.one);
		}
	} else {
		...
		for (int i = 0; i < nodes.Count; i++) {
			...
			// pitch matrix
			Matrix4x4 pit = Matrix4x4.TRS(Vector3.zero,
			                              Quaternion.Euler(node.pitch, 0.0f, 0.0f),
			                              Vector3.one);

			node.matrix = node.parent.matrix *
			              Matrix4x4.TRS(pos, rot, Vector3.one) * aog * pit;
...
override public void BuildAOSpheres(List<TreeAOSphere> aoSpheres) {
	...
	if (visible) {
		...
		for (int n = 0; n < nodes.Count; n++) {
			...
			while (t < 1.0f) {
				Vector3 pos =
					node.matrix.MultiplyPoint(node.spline.GetPositionAtTime(t));
				...
				if (includeFronds) {
					rad =
						Mathf.Max(rad, 0.95f * (Mathf.Clamp01(frondCurve.Evaluate(t)) *
						                        frondWidth * scale));
...
override public void UpdateMesh(List<TreeMaterial> materials,
                                List<TreeVertex> verts,
                                List<TreeTriangle> tris,
                                List<TreeAOSphere> aoSpheres, int buildFlags,
                                float adaptiveQuality, float aoDensity) {
	...
	if (geometryMode == GeometryMode.Branch ||
	    geometryMode == GeometryMode.BranchFrond) {
	...
	// Create mesh for nodes
	for (int i = 0; i < nodes.Count; i++) {
		UpdateNodeMesh(nodes[i], materials, verts, tris, aoSpheres, buildFlags,
		               adaptiveQuality, aoDensity);
	}

	// Update child groups..
	base.UpdateMesh(materials, verts, tris, aoSpheres, buildFlags,
	                adaptiveQuality, aoDensity);
}

private void UpdateNodeMesh(TreeNode node, List<TreeMaterial> materials,
                            List<TreeVertex> verts, List<TreeTriangle> tris,
                            List<TreeAOSphere> aoSpheres, int buildFlags,
                            float adaptiveQuality, float aoDensity) {
	// Clear tri range
	...
	float totalHeight =
		node.spline.GetApproximateLength(); // *node.GetScale(); //height *
	                                            // node.GetScale();
	...
	List<float> heightSamples =
		TreeData.GetAdaptiveSamples(this, node, lodQuality);
	...
	if ((parentGroup != null) &&
	    (parentGroup.GetType() == typeof(TreeGroupBranch))) {
		parentBranchGroup = (TreeGroupBranch) parentGroup;
	}

	if ((geometryMode == GeometryMode.BranchFrond) ||
	    (geometryMode == GeometryMode.Branch)) {
		int materialIndex = GetMaterialIndex(materialBranch, materials, true);

		float uvOffset = 0.0f;
		float uvBaseOffset = 0.0f;
		float uvStep =
			totalHeight / (GetRadiusAtTime(node, 0.0f, false) * Mathf.PI * 2.0f);
		bool uvAdapt = true;
		...
		for (int i = 0; i < heightSamples.Count; i++) {
			...
			Matrix4x4 m =
				node.matrix * Matrix4x4.TRS(pos, rot, new Vector3(1, 1, 1));
			...
			float radModify =
				Mathf.Max(flareWeldSpread.x,
				          Mathf.Max(flareWeldSpread.y, flareWeldSpread.z) * 0.25f);

			// keep the same number of vertices per ring for the cap.. to give a
			// nicer result
			if (t <= capStart) {
				radialSamples =
					TreeData.GetAdaptiveRadialSegments(rad + radModify, lodQuality);
			}
			...
			r.SetAnimationProperties(windFactors.x, windFactors.y, 0.0f,
			                         node.animSeed);
			r.SetSpread(flareWeldSpread.y, flareWeldSpread.z);
			r.SetNoise(noise * Mathf.Clamp01(noiseCurve.Evaluate(t)),
			           noiseScaleU * 10.0f, noiseScaleV * 10.0f);
			...
			if ((buildFlags & (int) BuildFlag.BuildWeldParts) != 0) {
				...
				float projectionBlend = Mathf.Pow(
					Mathf.Clamp01(((1.0f - t) - (1.0f - weldHeight)) / weldHeight),
					1.5f);
				...
				if (t < projectionRange) {
					if ((node.parent != null) && (node.parent.spline != null)) {
						...
						for (int v = vertStart; v < vertEnd; v++) {
							...
							for (int tri = node.parent.triStart; tri < node.parent.triEnd;
							     tri++) {
								object hit = MathUtils.IntersectRayTriangle(
									ray, verts[tris[tri].v[0]].pos, verts[tris[tri].v[1]].pos,
									verts[tris[tri].v[2]].pos, true);
								if (hit != null) {
									RaycastHit rayHit = ((RaycastHit) hit);
									if ((Mathf.Abs(rayHit.distance) < maxDist) &&
									    (rayHit.distance > minDist)) {
										maxDist = Mathf.Abs(rayHit.distance);
										verts[v].nor = (verts[tris[tri].v[0]].nor *
										                rayHit.barycentricCoordinate.x) +
										               (verts[tris[tri].v[1]].nor *
										                rayHit.barycentricCoordinate.y) +
										               (verts[tris[tri].v[2]].nor *
										                rayHit.barycentricCoordinate.z);

										verts[v].nor = (verts[v].nor * projectionBlend) +
										               (origNor * invProjectionBlend);

										verts[v].pos = (rayHit.point * projectionBlend) +
										               (origPos * invProjectionBlend);
									}
								}
							}
						}
			...
			if (ringloops[ringloops.Count - 1].radius > 0.025f) {
				if (node.breakOffset < 1.0f) {
					...
					int capMaterialIndex =
						GetMaterialIndex(tempCapMaterial, materials, false);

					ringloops [ringloops.Count - 1]
					.Cap(tempCapSpherical, tempCapNoise, tempCapMapping,
					     mappingScale, verts, tris, capMaterialIndex);
				}
			}
		}
		...
		for (int i = 0; i < (ringloops.Count - 1); i++) {
			ringloops [i]
			.Connect(ringloops[i + 1], tris, materialIndex, false, false);
		}
		node.triEnd = tris.Count;
	...
	if ((geometryMode == GeometryMode.BranchFrond) ||
	    (geometryMode == GeometryMode.Frond)) {
		if ((frondCount > 0) && (frondMin != frondMax)) {
			...
			for (int j = 0; j < frondCount; j++) {
				...
				float angle = ((frondRotation * 360.0f) +
				               (((float) j * 180.0f) / frondCount) - 90.0f) *
				              Mathf.Deg2Rad;
				float angleA = -angle - crease;
				float angleB = angle - crease;
				Vector3 directionA =
					new Vector3(Mathf.Sin(angleA), 0.0f, Mathf.Cos(angleA));
				Vector3 normalA = new Vector3(directionA.z, 0.0f, -directionA.x);
				Vector3 directionB =
					new Vector3(Mathf.Sin(angleB), 0.0f, -Mathf.Cos(angleB));
				...
				for (int i = 0; i < heightSamples.Count; i++) {
					float t = heightSamples[i];
					float v =
						(t - frondMappingMin) / (frondMappingMax - frondMappingMin);
					...
					if (t > capStart) {
						...
						directionA =
							new Vector3(Mathf.Sin(angleA) * sinCapAngle, cosCapAngle,
							            Mathf.Cos(angleA) * sinCapAngle);
						normalA = new Vector3(directionA.z, directionA.y, -directionA.x);
						directionB =
							new Vector3(Mathf.Sin(angleB) * sinCapAngle, cosCapAngle,
					...
					Quaternion rot = node.spline.GetRotationAtTime(t);
					float rad = (Mathf.Clamp01(frondCurve.Evaluate(t)) * frondWidth *
					             node.GetScale());
					...
					if (GenerateDoubleSidedGeometry) {
						// Generate double sided geometry to compensate for lack of VFACE
						// shader semantic Twice the poly count Split vertices along back
						// seam, to avoid bent normals.. 8 verts instead of 3
						...
						for (float side = -1; side < 2; side += 2) {
							...
							v0.SetAnimationProperties(windFactors.x, windFactors.y,
							                          animationEdge, node.animSeed);
							v1.SetAnimationProperties(
								windFactors.x, windFactors.y, 0.0f,
								node.animSeed); // no edge flutter for center vertex
							v2.SetAnimationProperties(
								windFactors.x, windFactors.y, 0.0f,
								node.animSeed); // no edge flutter for center vertex
							v3.SetAnimationProperties(windFactors.x, windFactors.y,
							                          animationEdge, node.animSeed);
						...
						if (i > 0) {
							...
							//
							// back
							TreeTriangle tri0 = new TreeTriangle(
								frondMaterialIndex, voffset - 4, voffset - 3, voffset - 11);
							TreeTriangle tri1 =
								new TreeTriangle(frondMaterialIndex, voffset - 4,
								                 voffset - 11, voffset - 12);
							tri0.flip();
							tri1.flip();

							// front
							TreeTriangle tri2 = new TreeTriangle(
								frondMaterialIndex, voffset - 8, voffset - 7, voffset - 15);
							TreeTriangle tri3 =
								new TreeTriangle(frondMaterialIndex, voffset - 8,
								                 voffset - 15, voffset - 16);
							...
							//
							// front
							TreeTriangle tri4 = new TreeTriangle(
								frondMaterialIndex, voffset - 2, voffset - 9, voffset - 1);
							TreeTriangle tri5 = new TreeTriangle(
								frondMaterialIndex, voffset - 2, voffset - 10, voffset - 9);

							// back
							TreeTriangle tri6 = new TreeTriangle(
								frondMaterialIndex, voffset - 6, voffset - 13, voffset - 5);
							TreeTriangle tri7 =
								new TreeTriangle(frondMaterialIndex, voffset - 6,
								                 voffset - 14, voffset - 13);
							tri6.flip();
							tri7.flip();
						...
						v1.SetAnimationProperties(
							windFactors.x, windFactors.y, 0.0f,
							node.animSeed); // no edge flutter for center vertex
						v2.SetAnimationProperties(windFactors.x, windFactors.y,
						                          animationEdge, node.animSeed);
						...
						if (i > 0) {
							...
							TreeTriangle tri0 = new TreeTriangle(
								frondMaterialIndex, voffset - 2, voffset - 3, voffset - 6);
							TreeTriangle tri1 = new TreeTriangle(
								frondMaterialIndex, voffset - 2, voffset - 6, voffset - 5);
							tris.Add(tri0);
							tris.Add(tri1);

							TreeTriangle tri2 = new TreeTriangle(
								frondMaterialIndex, voffset - 2, voffset - 4, voffset - 1);
							TreeTriangle tri3 = new TreeTriangle(
								frondMaterialIndex, voffset - 2, voffset - 5, voffset - 4);
							tris.Add(tri2);
							tris.Add(tri3);
						}
					}
	...
	if ((buildFlags & (int) BuildFlag.BuildAmbientOcclusion) != 0) {
		for (int i = vertOffset; i < verts.Count; i++) {
			verts [i]
			.SetAmbientOcclusion(ComputeAmbientOcclusion(
						     verts[i].pos, verts[i].nor, aoSpheres, aoDensity));
		}
	}
...
public void UpdateSpline(TreeNode node) {
	...
	Quaternion rotPhotropism = MathUtils.QuaternionFromMatrix(worldUp) *
	                           Quaternion.Euler(0.0f, node.angle, 0.0f);
	Quaternion rotGravitropism = MathUtils.QuaternionFromMatrix(worldUp) *
	                             Quaternion.Euler(-180.0f, node.angle, 0.0f);
	...
	for (int i = 0; i < count; i++) {
		...
		Quaternion c = Quaternion.Euler(
			new Vector3(180.0f * (Random.value - 0.5f), node.angle,
			            180.0f * (Random.value - 0.5f)));
