...
public class TreeGroupLeaf : TreeGroup {
  ...
  //[field: TreeAttribute("GeometryMode", "popup",
  //"Plane,Cross,Tri-Cross,Billboard,Mesh")]
  public int geometryMode = (int) GeometryMode.PLANE;

  //[field: TreeAttribute("Material", "material", 0.0f, 1.0f,
  //"primitiveGeometry")]
  ...
  //
  override public bool CanHaveSubGroups() { return false; }
    ...
    if (geometryMode == (int) GeometryMode.MESH) {
      hash = UnityEditorInternal.InternalEditorUtility
                 .CalculateHashForObjectsAndDependencies(
                     new Object[]{instanceMesh});
    } else {
      hash = UnityEditorInternal.InternalEditorUtility
                 .CalculateHashForObjectsAndDependencies(
                     new Object[]{materialLeaf});
    }
  ...
  override public void UpdateParameters() {
    if (lockFlags == 0) {
      for (int n = 0; n < nodes.Count; n++) {
        ...
        for (int x = 0; x < 5; x++) {
          float rx =
              (Random.value - 0.5f) * 180.0f * (1.0f - perpendicularAlign);
          float ry =
              (Random.value - 0.5f) * 180.0f * (1.0f - perpendicularAlign);
          float rz = 0.0f;
    ...
    if (parentGroup == null) {
      ...
      for (int n = 0; n < nodes.Count; n++) {
        ...
        if (tgr != null) {
          ...
          pos = new Vector3(Mathf.Cos(ang) * dist, -tgr.groundOffset,
                            Mathf.Sin(ang) * dist);
          rot = Quaternion.Euler(node.pitch * -Mathf.Sin(ang), 0,
                                 node.pitch * Mathf.Cos(ang)) *
                Quaternion.Euler(0, node.angle, 0);
        } else {
          // attached to branch node
          node.parent.GetPropertiesAtTime(node.offset, out pos, out rot,
                                          out rad);
        ...
        // pitch matrix
        Matrix4x4 pit = Matrix4x4.TRS(
            Vector3.zero,
            Quaternion.Euler(node.pitch + surfaceAngle, 0.0f, 0.0f),
            Vector3.one);
        ...
        // spin
        node.matrix *= Matrix4x4.TRS(new Vector3(0, 0, 0), node.rotation,
                                     new Vector3(1, 1, 1));
        ...
        if (horizontalAlign > 0.0f) {
          ...
          Quaternion newQuat =
              Quaternion.Slerp(MathUtils.QuaternionFromMatrix(node.matrix),
                               targetQuat, horizontalAlign);
  ...
  override public void UpdateMesh(List<TreeMaterial> materials,
      ...
      if (instanceMesh != null) {
        ...
        if (instanceMesh.GetComponent<Renderer>() != null) {
          Material[] sharedMaterials =
              instanceMesh.GetComponent<Renderer>().sharedMaterials;
    ...
    for (int n = 0; n < nodes.Count; n++) {
      UpdateNodeMesh(nodes[n], materials, verts, tris, aoSpheres, buildFlags,
  ...
  private static TreeVertex
  CreateBillboardVertex(TreeNode node, Quaternion billboardRotation,
                        Vector3 normalBase, float normalFix,
                        Vector3 tangentBase, Vector2 uv) {
    ...
    vertex.nor = (billboardRotation *
                  (new Vector3(uv.x * node.scale, uv.y * node.scale, 0.0f)));
    vertex.nor.z = normalFix;

    // normal
    Vector3 normal =
        (billboardRotation *
         (new Vector3(uv.x * normalBase.x, uv.y * normalBase.y, normalBase.z)))
            .normalized;

    // calculate tangent from the normal
    vertex.tangent =
        (tangentBase - normal * Vector3.Dot(tangentBase, normal)).normalized;
  ...
  private Vector2[] GetPlaneHullVertices(Material mat) {
    ...
    Vector2[] textureHull =
        UnityEditor.MeshUtility.ComputeTextureBoundingHull(tex, 4);
  ...
  private void UpdateNodeMesh(TreeNode node, List<TreeMaterial> materials,
    ...
    if (geometryMode == (int) GeometryMode.MESH) {
      ...
      for (int i = 0; i < cloneVerts.Length; i++) {
        ...
        Vector3 tangent =
            tformMatrix
                .MultiplyVector(new Vector3(
                    cloneTangents[i].x, cloneTangents[i].y, cloneTangents[i].z))
                .normalized;
        v0.tangent =
            new Vector4(tangent.x, tangent.y, tangent.z, cloneTangents[i].w);

        // wind
        float windEdge = (cloneVerts[i].magnitude / dist) * animationEdge;
        v0.SetAnimationProperties(windFactors.x, windFactors.y, windEdge,
                                  node.animSeed);

        // AO
        if ((buildFlags & (int) BuildFlag.BuildAmbientOcclusion) != 0) {
          v0.SetAmbientOcclusion(
              ComputeAmbientOcclusion(v0.pos, v0.nor, aoSpheres, aoDensity));
        }
      ...
      for (int s = 0; s < cloneMesh.subMeshCount; s++) {
        ...
        int materialIndex;
        if (instanceMesh.GetComponent<Renderer>() != null &&
            s < instanceMesh.GetComponent<Renderer>().sharedMaterials.Length) {
          materialIndex = GetMaterialIndex(
              instanceMesh.GetComponent<Renderer>().sharedMaterials[s],
              materials, false);
        } else {
        ...
        for (int i = 0; i < instanceTris.Length; i += 3) {
          TreeTriangle t0 =
              new TreeTriangle(materialIndex, instanceTris[i] + vertOffset,
                               instanceTris[i + 1] + vertOffset,
                               instanceTris[i + 2] + vertOffset);
          tris.Add(t0);
        }
      }
      ...
      // normal
      Vector3 normalBase = new Vector3(GenerateBendBillboardNormalFactor,
                                       GenerateBendBillboardNormalFactor, 1.0f);

      Vector3 tangentBase = billboardRotation * new Vector3(1, 0, 0);
      float normalFix = node.scale / (GenerateBendBillboardNormalFactor *
                                      GenerateBendBillboardNormalFactor);

      TreeVertex v0 =
          CreateBillboardVertex(node, billboardRotation, normalBase, normalFix,
                                tangentBase, new Vector2(0, 1));
      TreeVertex v1 =
          CreateBillboardVertex(node, billboardRotation, normalBase, normalFix,
                                tangentBase, new Vector2(0, 0));
      TreeVertex v2 =
          CreateBillboardVertex(node, billboardRotation, normalBase, normalFix,
                                tangentBase, new Vector2(1, 0));
      TreeVertex v3 =
          CreateBillboardVertex(node, billboardRotation, normalBase, normalFix,
                                tangentBase, new Vector2(1, 1));

      // wind
      v0.SetAnimationProperties(windFactors.x, windFactors.y, animationEdge,
                                node.animSeed);
      v1.SetAnimationProperties(windFactors.x, windFactors.y, animationEdge,
                                node.animSeed);
      v2.SetAnimationProperties(windFactors.x, windFactors.y, animationEdge,
                                node.animSeed);
      v3.SetAnimationProperties(windFactors.x, windFactors.y, animationEdge,
                                node.animSeed);
      ...
      if ((buildFlags & (int) BuildFlag.BuildAmbientOcclusion) != 0) {
        ...
        float a = 0.0f; // ComputeAmbientOcclusion(partID, v0.pos + pushU,
                        // Vector3.up, aoSpheres);
        // a += ComputeAmbientOcclusion(partID, v0.pos - pushU, -Vector3.up,
        // aoSpheres);
        a = ComputeAmbientOcclusion(v0.pos + pushR, Vector3.right, aoSpheres,
                                    aoDensity);
        a += ComputeAmbientOcclusion(v0.pos - pushR, -Vector3.right, aoSpheres,
                                     aoDensity);
        a += ComputeAmbientOcclusion(v0.pos + pushF, Vector3.forward, aoSpheres,
                                     aoDensity);
        a += ComputeAmbientOcclusion(v0.pos - pushF, -Vector3.forward,
                                     aoSpheres, aoDensity);
        a /= 4.0f;
      ...
      tris.Add(new TreeTriangle(materialIndex, index0, index0 + 2, index0 + 1,
                                true));
      tris.Add(new TreeTriangle(materialIndex, index0, index0 + 3, index0 + 2,
                                true));
    } else {
      ...
      Vector2[] rawHull = new Vector2[]{new Vector2(0, 1), new Vector2(0, 0),
                                        new Vector2(1, 0), new Vector2(1, 1)};
      Vector2[] textureHull = GetPlaneHullVertices(materialLeaf);
      if (textureHull == null)
        textureHull = rawHull;
      float ns = node.scale;
      Vector3[] positionsRaw =
          new Vector3[]{new Vector3(-ns, 0f, -ns), new Vector3(-ns, 0f, ns),
                        new Vector3(ns, 0f, ns), new Vector3(ns, 0f, -ns)};

      Vector3 normal =
          new Vector3(GenerateBendNormalFactor, 1.0f - GenerateBendNormalFactor,
      ...
      for (int ipl = 0; ipl < planes; ipl++) {
        ...
        TreeVertex[] tv = new TreeVertex[8]{
            new TreeVertex(), new TreeVertex(),
            new TreeVertex(), new TreeVertex(), // initial quad
            new TreeVertex(), new TreeVertex(),
            new TreeVertex(), new TreeVertex() // from bounding hull
        };
        ...
        for (int i = 0; i < 4; ++i) {
          ...
          tv [i]
              .SetAnimationProperties(windFactors.x, windFactors.y,
                                      animationEdge, node.animSeed);
          if ((buildFlags & (int) BuildFlag.BuildAmbientOcclusion) != 0) {
            tv [i]
                .SetAmbientOcclusion(ComputeAmbientOcclusion(
                    tv[i].pos, tv[i].nor, aoSpheres, aoDensity));
          }
        }
        ...
        tris.Add(
            new TreeTriangle(materialIndex, index0, index0 + 1, index0 + 2));
        tris.Add(
            new TreeTriangle(materialIndex, index0, index0 + 2, index0 + 3));

        Vector3 faceNormal =
            node.matrix.MultiplyVector(rot * new Vector3(0, 1, 0));
        ...
        if (GenerateDoubleSidedGeometry) {
          ...
          for (int i = 0; i < 4; ++i) {
            ...
            if ((buildFlags & (int) BuildFlag.BuildAmbientOcclusion) != 0) {
              tv2 [i]
                  .SetAmbientOcclusion(ComputeAmbientOcclusion(
                      tv2[i].pos, tv2[i].nor, aoSpheres, aoDensity));
            }
          }
          // now lerp positions into correct placed based on the bounding hull
          for (int i = 0; i < 4; ++i) {
            tv2 [i + 4]
                .Lerp4(tv2, textureHull[i]);
          ...
          for (int i = 0; i < 4; ++i)
            verts.Add(tv2[i + 4]);
          tris.Add(
              new TreeTriangle(materialIndex, index4, index4 + 2, index4 + 1));
          tris.Add(
              new TreeTriangle(materialIndex, index4, index4 + 3, index4 + 2));
        }
      }
    }
