...
public class TreeGroup {
  ...
  public enum LockFlag { LockPosition = 1, LockAlignment = 2, LockShape = 4 }

  public enum BuildFlag { BuildAmbientOcclusion = 1, BuildWeldParts = 2 }
  ...
  // Frequency, ie. the number of nodes that can be generated per node in the
  // parent group..
  public int distributionFrequency = 1;

  // Distribution properties, that modify actual distribution (ie. placement +
  // rotation)
  ...
  // Independent of actual distribution, so we don't need to sync distribution
  // completely when changing these..
  ...
  public AnimationCurve distributionScaleCurve =
      new AnimationCurve(new Keyframe(0, 1), new Keyframe(1, 0.3f));
  ...
  //
  virtual public float GetRadiusAtTime(TreeNode node, float t,
  ...
  //
  virtual public bool CanHaveSubGroups() { return true; }
  ...
  public void Lock() {
    ...
    lockFlags = (int) LockFlag.LockShape | (int) LockFlag.LockPosition |
                (int) LockFlag.LockAlignment;
  }

  public void Unlock() { lockFlags = 0; }

  internal virtual bool HasExternalChanges() { return false; }

  public bool CheckExternalChanges() {
    bool hasExternalChanges = HasExternalChanges();

    for (int i = 0; i < childGroups.Count; i++) {
      hasExternalChanges |= childGroups [i]
  ...
  public void UpdateFrequency(TreeData owner) {
    ...
    if (parentGroup == null) {
      ...
      for (int n = 0; n < parentGroup.nodes.Count; n++) {
        int tempFrequency =
            Mathf.RoundToInt(distributionFrequency * parentGroup
                                                         .nodes [n]
                                                         .GetScale());
  ...
  private float ComputeOffset(int index, int count, float distributionSum,
  ...
  public void UpdateDistribution(bool completeUpdate, bool updateSubGroups) {
    ...
    if (completeUpdate) {
      ...
      // sample distribution curve at X locations and store values in
      // distributionSamples store sum of samples in distributionSum
      ...
      for (int i = 0; i < nodes.Count; i++) {
        ...
        if (lockFlags == 0) {
          if ((i == 0) && (nodes.Count == 1) &&
            ...
            node.pitch = 0.0f;
            node.scale =
                Mathf.Clamp01(distributionScaleCurve.Evaluate(node.offset)) *
                    distributionScale +
            ...
            case DistributionMode.Random: {
              ...
              // hack, since the random values seem to be clustered -
              // investigate!
              for (int j = 0; j < 5; j++) {
                weight = Random.value * distributionSum;
              }

              // go through distributionSamples and subtract each from weight
              // until weight <= 0, which means we've hit a good spot
            ...
            case DistributionMode.Alternate: {
              // sample along distribution curve until we reach the next 'good'
              // offset
              float offset = ComputeOffset(nodeLocalIndex, nodeLocalCount,
                                           distributionSum, distributionStep);
            ...
            case DistributionMode.Opposite: {
              ...
              // offset
              float offset =
                  ComputeOffset(nodeLocalIndex / 2, nodeLocalCount / 2,
                                distributionSum, distributionStep);
              float angle = 90.0f * (nodeLocalIndex / 2) +
                            ((nodeLocalIndex % 2) * 180.0f);
              node.baseAngle = angle + (offset * distributionTwirl * 360.0f);
              node.offset = offset;
            } break;
            ...
            case DistributionMode.Whorled: {
              ...
              float offset = ComputeOffset(nodeLocalIndex / clusterCount,
                                           nodeLocalCount / clusterCount,
                                           distributionSum, distributionStep);
    ...
    for (int n = 0; n < nodes.Count; n++) {
      ...
      // Update individual node visibility, which has nothing to do with group
      // visibility..
      if (node.parent == null) {
        node.visible = true;
      } else {
      ...
      if (lockFlags == 0) {
        ...
        node.pitch = Mathf.Clamp(distributionPitchCurve.Evaluate(node.offset),
                                 -1.0f, 1.0f) *
                     -75.0f * distributionPitch;
      } else {
        node.angle = node.baseAngle;
      }
      node.scale = Mathf.Clamp01(distributionScaleCurve.Evaluate(node.offset)) *
                       distributionScale +
                   (1.0f - distributionScale);
    }
      ...
      for (int i = 0; i < nodes.Count; i++) {
        if (nodes[i].parent == null) {
          ...
          if (nodes[i].parent.animSeed == 0.0f) {
            // new seed
            nodes[i].animSeed =
                ((((float) nodes[i].seed) / 9.78f) % 1) + 0.001f;
          } else {
            // copy seed
    ...
    if (updateSubGroups) {
      ...
      for (int i = 0; i < childGroups.Count; i++) {
        childGroups [i]
            .UpdateDistribution(completeUpdate, updateSubGroups);
      }
    }
  ...
  virtual public void BuildAOSpheres(List<TreeAOSphere> aoSpheres) {
    ...
    for (int i = 0; i < childGroups.Count; i++) {
      childGroups [i]
          .BuildAOSpheres(aoSpheres);
    }

    Profiler.EndSample(); // BuildAOSpheres
  }

  virtual public void UpdateMesh(List<TreeMaterial> materials,
    ...
    for (int i = 0; i < childGroups.Count; i++) {
      childGroups [i]
          .UpdateMesh(materials, verts, tris, aoSpheres, buildFlags,
  ...
  GetMaterialIndex(Material m, List<TreeMaterial> materials, bool tileV) {
  ...
  protected static Vector4 CreateTangent(TreeNode node, Quaternion rot,
                                         Vector3 normal) {
  ...
  //
  protected static float ComputeAmbientOcclusion(Vector3 pos, Vector3 nor,
    ...
    for (int i = 0; i < aoSpheres.Count; i++) {
      total += aoSpheres [i]
                   .PointOcclusion(pos, nor) *
               aoSpheres[i].density * 0.25f;
    ...
    for (int i = 0; i < aoSpheres.Count; i++)
    {
        ...
        aoSpheres[i].flag = (delta.sqrMagnitude < ((aoSpheres[i].radius +
    range)*(aoSpheres[i].radius + range)));
    }
    ...
    for (int s = 0; s < samples; s++)
    {
        ...
        for (int i = 0; i < aoSpheres.Count; i++)
        {
            if (aoSpheres[i].flag)
            {
                if (MathUtils.IntersectRaySphere(ray, aoSpheres[i].position,
    aoSpheres[i].radius, ref t, ref hit))
                {
