...
public void AddTexture(string name, Texture2D diffuse, Color diffuseColor,
                       Texture2D normal, Texture2D gloss, Texture2D transtex,
                       Texture2D shadowOffsetTex, float shininess,
                       Vector2 scale, bool tileV, Vector2 uvTiling) {
...
public void Pack(ref int targetWidth, int targetHeight, int padding,
                 bool correctPow2) {
	//
	...
	for (int i = 0; i < nodes.Count; i++) {
		...
		node.packedRect.height = Mathf.Min(
			maxHeight, Mathf.Round(node.sourceRect.height * node.scale.y));
		...
		if (correctPow2) {
			node.packedRect.width =
				(float) Mathf.ClosestPowerOfTwo((int) node.packedRect.width);
			node.packedRect.height =
				(float) Mathf.ClosestPowerOfTwo((int) node.packedRect.height);
			//    Debug.Log("Corrected size:
			//    "+node.packedRect.width+","+node.packedRect.height);
		}
	}
	...
	nodes.Sort(
		delegate(TextureNode a, TextureNode b) { return a.CompareTo(b); });
	...
	for (int i = 0; i < nodes.Count; i++) {
			...
			for (int y = 0; y <= interiorh; y++) {
				...
				for (int j = 0; j < i; j++) {
					...
					if (TextureNode.Overlap(node, node2)) {
						good = false;
						// No point in searching for free place in top - bottom if
						// node2.tileV is true, so exit loop 'top - bottom'
						if (node2.tileV)
							y = interiorh;
						else
		...
		interiorw = Mathf.Max(interiorw,
		                      (int)(node.packedRect.x + node.packedRect.width));
		interiorh = Mathf.Max(interiorh,
		                      (int)(node.packedRect.y + node.packedRect.height));
	}
	...
	int finalWidth = Mathf.Clamp(Mathf.ClosestPowerOfTwo(interiorw),
	                             minimalWidth, targetWidth);
	...
	for (int i = 0; i < nodes.Count; i++) {
		...
		node.packedRect.width =
			Mathf.Round(node.packedRect.width); // - 0.5f;// +1.0f;// +0.5f;
		node.packedRect.height =
			Mathf.Round(node.packedRect.height); // - 0.5f;// +1.0f;// +0.5f;
