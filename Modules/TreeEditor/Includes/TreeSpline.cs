...
public class TreeSpline {
  ...
  public float tension =
      0.5f; // 0.0f = linear, 0.5f = catmull-rom spline, 1.0f = over shoot
  ...
  public void Reset() { nodes = new SplineNode[0]; }
  ...
  public void UpdateRotations() {
    ...
    for (int i = 1; i < nodes.Length; i++) {
      ...
      if (i == nodes.Length - 1) {
        ...
        upVec = (nodes[i].point - nodes[i - 1].point) / distA +
                (nodes[i + 1].point - nodes[i].point) / distB;
      }
      upVec.Normalize();
  ...
  private Quaternion GetRotationInternal(int idxFirstpoint, float t) {
    ...
    Quaternion T1 =
        new Quaternion(tension * (Q2.x - Q0.x), tension * (Q2.y - Q0.y),
                       tension * (Q2.z - Q0.z), tension * (Q2.w - Q0.w));
    Quaternion T2 =
        new Quaternion(tension * (Q3.x - Q1.x), tension * (Q3.y - Q1.y),
                       tension * (Q3.z - Q1.z), tension * (Q3.w - Q1.w));
  ...
  public Quaternion GetRotationAtTime(float timeParam) {
    ...
    if (timeParam <= nodes[0].time)
      return nodes[0].rot;
    if (timeParam >= nodes[nodes.Length - 1].time)
      return nodes[nodes.Length - 1].rot;
    ...
    int idx = c - 1;
    float param =
        (timeParam - nodes[idx].time) / (nodes[idx + 1].time - nodes[idx].time);
