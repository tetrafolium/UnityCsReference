...
[assembly:InternalsVisibleTo("Unity.InputSystem")] namespace UnityEngineInternal
    .Input {
  ...
  using NativeShouldRunUpdateCallback =
      System.Func<NativeInputUpdateType, bool>;
  internal unsafe delegate void NativeUpdateCallback(
      NativeInputUpdateType updateType, NativeInputEventBuffer * buffer);
  ...
  // We pass this as a struct to make it less painful to change the OnUpdate()
  // API if need be.
  [StructLayout(LayoutKind.Explicit, Size = structSize, Pack = 1)]
  internal unsafe struct NativeInputEventBuffer {
    public const int structSize = 20;
    // NOTE: Keep this as the first field in the struct. This avoids
    // alignment/packing issues
    //       on the C++ side due to the compiler wanting to align the 64bit
    //       pointer.
    [FieldOffset(0)]
  ...
  internal struct NativeInputEvent {
    ...
    [FieldOffset(8)]
    public double time; // Time on GetTimeSinceStartup() timeline in seconds.
                        // *NOT* on Time.time timeline.
    [FieldOffset(16)]
    public int eventId;

    public NativeInputEvent(NativeInputEventType type, int sizeInBytes,
                            int deviceId, double time) {
      this.type = type;
  ...
  internal partial class NativeInputSystem {
    // This generates warnings that these are not used. They are used, but only
    // by the input system package (which can access these via
    // InternalsVisibleTo). But since that is not available during unity build
    // time, and since these are marked as internal, we'd get the warning. So we
    // suppress these.
...
#pragma warning restore 649
    ...
    static NativeInputSystem() {
      // This property is backed by a native field, and so it's state is
      // preserved over domain reload. Reset it on initialization to keep it
      // current.
    ...
    [RequiredByNativeCode]
    internal static unsafe void NotifyUpdate(NativeInputUpdateType updateType,
                                             IntPtr eventBuffer) {
    ...
    [RequiredByNativeCode]
    internal static void NotifyDeviceDiscovered(int deviceId,
    ...
    [RequiredByNativeCode]
    internal static void ShouldRunUpdate(NativeInputUpdateType updateType,
                                         out bool retval) {
