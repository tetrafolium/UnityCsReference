...
namespace UnityEditor.Build.Reporting {
  public class StrippingInfo : ScriptableObject,
    ...
    public IEnumerable<string> GetReasonsForIncluding(string entityName) {
      HashSet<string> deps;
      return dependencies.TryGetValue(entityName, out deps)
                 ? deps
    ...
    [SerializeField]
    internal List<SerializedDependency> serializedDependencies;

    [SerializeField]
    internal List<string> modules = new List<string>();

    // Not needed any more since we have SerializedDependency.size now, but keep
    // it as long as the BuildReport UI uses it.
    [SerializeField]
    internal List<int> serializedSizes = new List<int>();

    [SerializeField]
    internal Dictionary<string, HashSet<string>> dependencies =
    ...
    [SerializeField]
    internal Dictionary<string, string> icons =
        new Dictionary<string, string>();

    [SerializeField]
    internal int totalSize = 0;

    void OnEnable() { SetIcon(RequiredByScripts, "class/MonoScript"); }
    ...
    void ISerializationCallbackReceiver.OnBeforeSerialize() {
      ...
      foreach (var dep in dependencies) {
        ...
        sd.value = list;
        sd.icon =
            icons.ContainsKey(dep.Key) ? icons[dep.Key] : "class/DefaultAsset";
