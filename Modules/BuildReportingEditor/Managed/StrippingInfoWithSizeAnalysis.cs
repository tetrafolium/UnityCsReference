...
namespace UnityEditor.Build.Reporting {
  internal abstract class StrippingInfoWithSizeAnalysis : StrippingInfo {
    ...
    GetIl2CPPAssemblyMapArtifacts(string path) {
    ...
    static private void OutputSizes(Dictionary<string, int> sizes,
                                    int totalLines) {
    ...
    static private void PrintSizesDictionary(Dictionary<string, int> sizes,
      ...
      myList.Sort(delegate(KeyValuePair<string, int> pair1,
                           KeyValuePair<string, int> pair2) {
        return pair2.Value.CompareTo(pair1.Value);
      });

      for (int i = 0; i < maxSize && i < myList.Count; i++)
        System.Console.WriteLine(myList[i].Value.ToString("D8") + " " +
                                 myList[i].Key);
    }
    ...
    protected Dictionary<string, int> folderSizes =
        new Dictionary<string, int>();
    protected Dictionary<string, int> moduleSizes =
        new Dictionary<string, int>();
    protected Dictionary<string, int> assemblySizes =
        new Dictionary<string, int>();
    protected Dictionary<string, int> objectSizes =
        new Dictionary<string, int>();
    ...
    public void Analyze() {
      ...
      var assemblyArtifacts = GetIl2CPPAssemblyMapArtifacts(
      ...
      foreach (var functionSize in functionSizes) {
        if (symbolArtifacts.ContainsKey(functionSize.Key)) {
          var objectFile = symbolArtifacts [functionSize.Key]
                               .Replace('\\', '/');
          ...
          if (objectFile.LastIndexOf('/') != -1) {
            var objectFolder =
                objectFile.Substring(0, objectFile.LastIndexOf('/'));
        ...
        if (moduleArtifacts.ContainsKey(functionSize.Key)) {
          ...
          objectFile = objectFile.Substring(0, objectFile.Length -
                                                   "Module_Dynamic.bc".Length);
      ...
      sizes["IL2CPP Unaccounted"] =
          moduleSizes["IL2CPP Generated"] - totalAssemblySize;
