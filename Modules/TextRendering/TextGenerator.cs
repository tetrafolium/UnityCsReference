...
[Flags] enum TextGenerationError {
  None = 0,
...
public struct TextGenerationSettings {
  ...
  public bool Equals(TextGenerationSettings other) {
    return CompareColors(color, other.color) && fontSize == other.fontSize &&
           ...
           fontStyle == other.fontStyle && richText == other.richText &&
           ...
           CompareVector2(pivot, other.pivot) && font == other.font;
  }
}

public partial class TextGenerator : IDisposable {
  // WARNING: Because this is a partial class, do not add any data members here;
  // there is no defined ordering between fields in multiple declarations of
  // partial class. All instance fields must be in the same declaration (for
  // this class, they are in the corresponding Bindings.txt file.
  ...
  ~TextGenerator() { ((IDisposable) this).Dispose(); }

  void IDisposable.Dispose() {
    lock(s_Instances) { s_Instances.Remove(m_Id); }
  ...
  private TextGenerationSettings
  ValidatedSettings(TextGenerationSettings settings) {
    ...
    if (settings.fontSize != 0 || settings.fontStyle != FontStyle.Normal) {
      if (settings.font != null)
        Debug.LogWarningFormat(
            settings.font,
            "Font size and style overrides are only supported for dynamic fonts. Font '{0}' is not dynamic.",
            settings.font.name);
      settings.fontSize = 0;
    ...
    if (settings.resizeTextForBestFit) {
      if (settings.font != null)
        Debug.LogWarningFormat(
            settings.font,
            "BestFit is only supported for dynamic fonts. Font '{0}' is not dynamic.",
            settings.font.name);
  ...
  public void Invalidate() { m_HasGenerated = false; }

  public void GetCharacters(List<UICharInfo> characters) {
    GetCharactersInternal(characters);
  }

  public void GetLines(List<UILineInfo> lines) { GetLinesInternal(lines); }
  ...
  public bool PopulateWithErrors(string str, TextGenerationSettings settings,
                                 GameObject context) {
    ...
    if ((error & TextGenerationError.CustomSizeOnNonDynamicFont) != 0)
      Debug.LogErrorFormat(
          context,
          "Font '{0}' is not dynamic, which is required to override its size",
          settings.font);
    if ((error & TextGenerationError.CustomStyleOnNonDynamicFont) != 0)
      Debug.LogErrorFormat(
          context,
          "Font '{0}' is not dynamic, which is required to override its style",
          settings.font);
    // if ((error & TextGenerationError.NoFont) == TextGenerationError.NoFont)
    //  Debug.LogErrorFormat(context, "Missing font, aborting text generation",
    //  settings.font);
    return false;
  }
  ...
  private TextGenerationError
  PopulateWithError(string str, TextGenerationSettings settings) {
    if (m_HasGenerated && str == m_LastString &&
        settings.Equals(m_LastSettings))
      return m_LastValid;
  ...
  private TextGenerationError PopulateAlways(string str,
    ...
    Populate_Internal(
        str, validSettings.font, validSettings.color, validSettings.fontSize,
        validSettings.scaleFactor, validSettings.lineSpacing,
        validSettings.fontStyle, validSettings.richText,
        validSettings.resizeTextForBestFit, validSettings.resizeTextMinSize,
        validSettings.resizeTextMaxSize, validSettings.verticalOverflow,
        validSettings.horizontalOverflow, validSettings.updateBounds,
        validSettings.textAnchor, validSettings.generationExtents,
