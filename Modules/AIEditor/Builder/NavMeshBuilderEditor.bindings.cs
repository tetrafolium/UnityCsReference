...
namespace UnityEditor.AI {
  ...
  [StaticAccessor("NavMeshBuilderEditorBindings",
  ...
  public sealed class NavMeshBuilder {
    ...
    public static extern bool isRunning {
      [StaticAccessor("NavMeshBuilder", StaticAccessorType.DoubleColon)]
      [NativeMethod("IsBuildingNavMeshAsync")] get;
    }
    ...
    public static void BuildNavMeshForMultipleScenes(string[] paths) {
      ...
      for (int i = 0; i < paths.Length; ++i) {
        if (EditorSceneManager.OpenScene(paths[i]).IsValid()) {
          ...
          // We only have one scene open so it is ok to simply get the active
          // scene here
    ...
    public static void
    CollectSourcesInStage(Bounds includedWorldBounds, int includedLayerMask,
                          NavMeshCollectGeometry geometry, int defaultArea,
                          List<NavMeshBuildMarkup> markups, Scene stageProxy,
      ...
      if (!stageProxy.IsValid())
        throw new ArgumentException(
            "Stage cannot be deduced from invalid scene.", nameof(stageProxy));
      ...
      var resultsArray = CollectSourcesInStageInternal(
          ...
          defaultArea, markups.ToArray(), stageProxy);
      results.Clear();
      results.AddRange(resultsArray);
    }

    public static void
    CollectSourcesInStage(Transform root, int includedLayerMask,
                          NavMeshCollectGeometry geometry, int defaultArea,
                          List<NavMeshBuildMarkup> markups, Scene stageProxy,
                          List<NavMeshBuildSource> results) {
      if (markups == null)
      ...
      var resultsArray = CollectSourcesInStageInternal(
          includedLayerMask, empty, root, false, geometry, defaultArea,
          markups.ToArray(), stageProxy);
      results.Clear();
      results.AddRange(resultsArray);
    }

    [FreeFunction(
        "NavMeshBuilderEditorBindings::CollectSourcesInStageInternal")]
    static extern NavMeshBuildSource[] CollectSourcesInStageInternal(
        ...
        NavMeshBuildMarkup[] markups, Scene stageProxy);
  }
}
