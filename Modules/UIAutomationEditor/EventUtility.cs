...
namespace UnityEditor.UIAutomation {
  // Note: all input mouse positions should be in GUIView coordinates as the
  // EventUtility class dispatches events as they were coming from GUIView. Use
  // ConvertEditorWindowCoordsToGuiViewCoords and
  // ConvertGuiViewCoordsToEditorWindowCoords to convert between coordic
  ...
  static class EventUtility {
    ...
    internal static bool Click(EditorWindow window, Vector2 mousePosition,
                               EventModifiers modifiers, int mouseButton) {
      var anyEventsUsed = false;

      var evt = new Event(){mousePosition = mousePosition,
                            modifiers = modifiers, button = mouseButton};
    ...
    public static void UpdateMouseMove(EditorWindow window,
    ...
    public static void DragAndDrop(EditorWindow window,
                                   Vector2 mousePositionStart,
                                   Vector2 mousePositionEnd) {
      DragAndDrop(window, mousePositionStart, mousePositionEnd,
                  EventModifiers.None);
    }
    ...
    public static void DragAndDrop(EditorWindow window,
      ...
      UpdateDragAndDrop(window,
                        (mousePositionStart + mousePositionStart) * 0.5f);
      EndDragAndDrop(window, mousePositionEnd, modifiers);
    }

    public static bool BeginDragAndDrop(EditorWindow window,
                                        Vector2 mousePosition) {
    ...
    public static bool BeginDragAndDrop(EditorWindow window,
      ...
      var evt =
          new Event(){mousePosition = mousePosition, modifiers = modifiers};
      ...
      // Some drag and drop handling requires to small mouse drag distance
      // before starting a drag and drop session
      evt.mousePosition =
          new Vector2(evt.mousePosition.x, evt.mousePosition.y + 20);
      anyEventsUsed |= window.SendEvent(evt);
    ...
    public static bool EndDragAndDrop(EditorWindow window,
                                      Vector2 mousePosition) {
    ...
    public static bool UpdateDragAndDrop(EditorWindow window,
                                         Vector2 mousePosition) {
    ...
    public static bool UpdateDragAndDrop(EditorWindow window,
      ...
      var evt =
          new Event(){type = EventType.DragUpdated,
                      mousePosition = mousePosition, modifiers = modifiers};
    ...
    public static void Drag(EditorWindow window, Vector2 mousePositionStart,
                            Vector2 mousePositionEnd) {
    ...
    public static void UpdateDrag(EditorWindow window, Vector2 mousePosition,
      ...
      window.SendEvent(new Event(){
          type = EventType.MouseDrag, mousePosition = mousePosition,
          modifiers = modifiers, delta = mousePosition - s_PrevMousePosition});
      s_PrevMousePosition = mousePosition;
    }

    // These are not public so hardcoded here
    const float kDockAreaTopMarginAndTabHeight =
        DockArea.kFloatingWindowTopBorderWidth + DockArea.kTabHeight + 1;

    public static Vector2
    ConvertEditorWindowCoordsToGuiViewCoords(Vector2 editorWindowPosition) {
      return new Vector2(editorWindowPosition.x,
                         editorWindowPosition.y +
                             kDockAreaTopMarginAndTabHeight);
    }

    public static Vector2
    ConvertGuiViewCoordsToEditorWindowCoords(Vector2 guiViewPosition) {
      return new Vector2(guiViewPosition.x,
                         guiViewPosition.y - kDockAreaTopMarginAndTabHeight);
    }

    static void UpdateFakeMouseCursorPosition(EditorWindow window,
                                              Vector2 mousePosition) {
      TestEditorWindow testWindow = window as TestEditorWindow;
      if (testWindow != null)
        testWindow.fakeCursor.position =
            ConvertGuiViewCoordsToEditorWindowCoords(mousePosition);
    }
  }
  ...
  internal class Easing {
    ...
    public class Quadratic {
      public static float In(float k) { return k * k; }

      public static float Out(float k) { return k * (2f - k); }
  ...
  struct Wait {
    ...
    public static void Seconds(double seconds) { s_Wait = new Wait(seconds); }
