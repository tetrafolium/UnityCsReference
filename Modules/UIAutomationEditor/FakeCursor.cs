...
public virtual void Draw() {
	if (shouldDrawFakeMouseCursor) {
		...
		if (cursor != null)
			...
			EditorGUI.DrawRect(new Rect(position.x, position.y, 5, 5),
			                   Color.white); // fallback rendering
	}
}
...
Rect GetCursorDrawRect(CursorType cursorType) {
	...
	return new Rect(position.x + offset.x, position.y + offset.y,
	                s_CursorSize.x, s_CursorSize.y);
}

void InitIfNeeded() {
	if (s_MouseCursors == null) {
		s_MouseCursors = new[] {new CursorData(){
