...
class AutomatedWindow<T> : IElementFinder,
	                   IDisposable where T : EditorWindow {
...
public AutomatedWindow(T windowToDrive) {
	...
	isAutomationVisible =
		EditorPrefs.GetBool("UnityEditor.PlaymodeTestsRunnerWindow" +
		                    ".visualizeUIAutomationEvents",
		                    false);

	// TODO: at the moment you can only debug one GUIView at a time, and
	// nothing prevents from someone else to change who's being debugged.
	...
	GUIViewDebuggerHelper.onViewInstructionsChanged +=
		m_Model.ViewContentsChanged;
	m_EditorWindow
	.RepaintImmediately();   // will create all the instructions, and will
	                         // trigger onViewInstructionsChanged
...
~AutomatedWindow() {
	if (!m_Disposed && m_DeveloperBuild)
		Debug.LogWarningFormat(
			"{0} instance finalized without being disposed. Create it inside a using block or manually call Dispose () when done.",
			this.GetType());
	Dispose();
}
...
public void Click(Vector2 editorWindowPosition,
                  EventModifiers modifiers = EventModifiers.None) {
	var GUIViewPosition =
		EventUtility.ConvertEditorWindowCoordsToGuiViewCoords(
...
public void Click(IAutomatedUIElement element,
                  EventModifiers modifiers = EventModifiers.None) {
	if (!element.hasRect) {
	...
	Click(EventUtility.ConvertGuiViewCoordsToEditorWindowCoords(
		      element.rect.center),
	      modifiers);
}
...
private void HandleMouseAutomationVisibility(Vector2 desiredMousePosition,
                                             bool sendMouseDrag = false) {
	...
	if (m_LastMousePosition != desiredMousePosition) {
		...
		float endTime =
			startTime +
			((m_LastMousePosition - desiredMousePosition).magnitude /
			 m_MouseMoveSpeed);
		...
		while (m_LastMousePosition != desiredMousePosition) {
			...
			float frac =
				Mathf.Clamp01((curtime - startTime) / (endTime - startTime));

			frac = Easing.Quadratic.InOut(frac);
			var mousePosition =
				Vector2.Lerp(mouseStart, desiredMousePosition, frac);

			// We currently send mouse moves to the window when visualizing the
			// interactions, But this might create different behaviour when
			// visualizing or not. This is also true for the Repaint event from
			// RepaintImmediately.
...
private void
HandleLastEventPauseVisibility(float minTimeBetweenEvents = .5f) {
	if (!isAutomationVisible)
		return;
	...
	if (deltaTime < minTimeBetweenEvents) {
		int remainderTime =
			Mathf.CeilToInt((minTimeBetweenEvents - deltaTime) * 100f);
		System.Threading.Thread.Sleep(remainderTime);
	}
	m_LastEventSent = (float) EditorApplication.timeSinceStartup;
}

public void DragAndDrop(Vector2 start, Vector2 end,
                        EventModifiers modifiers = EventModifiers.None) {
...
public void BeginDrag(Vector2 start,
                      EventModifiers modifiers = EventModifiers.None) {
...
public void EndDrop(Vector2 end,
                    EventModifiers modifiers = EventModifiers.None) {
...
public void KeyDownAndUp(KeyCode key,
                         EventModifiers modifiers = EventModifiers.None) {
	HandleLastEventPauseVisibility();
...
public IEnumerable<IAutomatedUIElement>
FindElementsByGUIStyle(GUIStyle style) {
	return m_Model.FindElementsByGUIStyle(style);
}

public IEnumerable<IAutomatedUIElement>
FindElementsByGUIContent(GUIContent guiContent) {
	return m_Model.FindElementsByGUIContent(guiContent);
}

public IEnumerable<IAutomatedUIElement>
FindElementsByNamedControl(string controlName) {
...
private readonly List<IMGUIInstruction> m_Instructions =
	new List<IMGUIInstruction>();
private readonly List<IMGUIClipInstruction> m_ClipList =
	new List<IMGUIClipInstruction>();
private readonly List<IMGUILayoutInstruction> m_LayoutList =
	new List<IMGUILayoutInstruction>();
private readonly List<IMGUINamedControlInstruction> m_NamedControlList =
	new List<IMGUINamedControlInstruction>();
private readonly List<IMGUIDrawInstruction> m_DrawInstructions =
	new List<IMGUIDrawInstruction>();

private AutomatedIMElement[] m_Elements;
private AutomatedIMElement m_Root;

public void Update() {
	// TODO: right now we simbolicate the stacktrace of all elements, but for
	// this scenario we are not super interested in it.
...
private void GenerateDom() {
	...
	for (int i = 0; i < total; ++i) {
		...
		if (i + 1 < total) {
			...
			if (nextInstruction.level < instruction.level) {
				for (int j = instruction.level - nextInstruction.level; j > 0;
				     --j) {
					var closingParent = ancestors.Pop();
					var closingParentIndex = ancestorsIndex.Pop();
					closingParent.descendants = new ArraySegment<AutomatedIMElement>(
						m_Elements, closingParentIndex + 1, i - closingParentIndex);
				}
			}
		}
	}
	...
	while (ancestors.Peek() != m_Root) {
		...
		var closingParentIndex = ancestorsIndex.Pop();
		closingParent.descendants = new ArraySegment<AutomatedIMElement>(
			m_Elements, closingParentIndex, (total - 1) - closingParentIndex);
	}
}

private AutomatedIMElement
CreateAutomatedElement(IMGUIInstruction imguiInstruction, int index) {
	...
	case InstructionType.kStyleDraw: {
		var drawInstruction =
			m_DrawInstructions[imguiInstruction.typeInstructionIndex];
	...
	case InstructionType.kLayoutBeginGroup: {
		var layoutInstruction =
			m_LayoutList[imguiInstruction.typeInstructionIndex];
	...
	case InstructionType.kLayoutNamedControl: {
		var namedControlInstruction =
			m_NamedControlList[imguiInstruction.typeInstructionIndex];
...
public void ViewContentsChanged() {
	// TODO: we dont need to update everytime the view actually change, we
	// just need to "dirty" the current state and update before we actually
	// need the date to be synced.
	Update();
}
}
...
IEnumerable<IAutomatedUIElement> FindElementsByGUIContent(
	GUIContent guiContent);
IEnumerable<IAutomatedUIElement> FindElementsByNamedControl(
	string controlName);
IAutomatedUIElement nextSibling {
	get;
}
}
...
IList<IAutomatedUIElement> children {
	get;
}
IAutomatedUIElement parent {
	get;
}
...
private Rect? m_Rect;
private List<IAutomatedUIElement> m_Children =
	new List<IAutomatedUIElement>();
	...
	get {
		if (!hasRect)
			throw new InvalidOperationException(
				      "Element does not contain rect info");
		return m_Rect.Value;
	}
	set { m_Rect = value; }
}
...
public ArraySegment<AutomatedIMElement> descendants {
	get;
	set;
}
...
protected virtual IEnumerable<IAutomatedUIElement>
FindElements(Func<IAutomatedUIElement, bool> predicate) {
	if (predicate == null)
...
private static bool GUIContentsAreEqual(GUIContent content1,
                                        GUIContent content2) {
	...
	// The native string type UTF16String does not differentiate between empty
	// and null strings Empty and null strings are considered equal here to
	// work around this issue
	return NullOrEmptyToNull(content1.text) ==
	       NullOrEmptyToNull(content2.text) &&
	       content1.image == content2.image &&
	       NullOrEmptyToNull(content1.tooltip) ==
	       NullOrEmptyToNull(content2.tooltip);
}

public IEnumerable<IAutomatedUIElement>
FindElementsByGUIContent(GUIContent guiContent) {
	return FindElements(
		element => GUIContentsAreEqual(element.guiContent, guiContent));
}
}
}
