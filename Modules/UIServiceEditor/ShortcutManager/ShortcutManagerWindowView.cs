...
namespace UnityEditor.ShortcutManagement {
  class ShortcutManagerWindowView : IShortcutManagerWindowView {
    ...
    public ShortcutManagerWindowView(
    ...
    public VisualElement GetVisualElementHierarchyRoot() { return m_Root; }
    ...
    public RebindResolution
    HandleRebindWillCreateConflict(ShortcutEntry entry,
                                   IList<KeyCombination> newBinding,
                                   IList<ShortcutEntry> conflicts) {
      var title = L10n.Tr("Binding Conflict");
      var message = string.Format(
          L10n.Tr(
              "The key {0} is already assigned to the \"{1}\" shortcut.\nDo you want to reassign this key?"),
          KeyCombination.SequenceToString(newBinding),
          conflicts[0].displayName);
      var result = EditorUtility.DisplayDialogComplex(
          title, message, L10n.Tr("Reassign"), L10n.Tr("Cancel"),
          L10n.Tr("Create Conflict"));
      switch (result) {
      case 0:
    ...
    VisualElement MakeItemForShortcutTable() {
      ...
      rebindControl.RegisterCallback<DetachFromPanelEvent>(
          OnRebindControlDetachedFromPanel);
      ...
      rowElement.RegisterCallback<GeometryChangedEvent>(
          OnGeometryChangedShortcutRow);
    ...
    void RebindControl_OnCancel() { EndRebind(); }
    ...
    void BindShortcutEntryItem(VisualElement shortcutElementTemplate,
                               int index) {
      ...
      bindingTextElement.text =
          KeyCombination.SequenceToString(shortcutEntry.combinations);
      bindingField.SetValueWithoutNotify(shortcutEntry.combinations.ToList());
      bindingField.RegisterValueChangedCallback(
          EditingShortcutEntryBindingChanged);
    ...
    void
    EditingShortcutEntryBindingChanged(ChangeEvent<List<KeyCombination>> evt) {
    ...
    void BindCategoriesTableItem(VisualElement categoryElementTemplate,
                                 int index) {
    ...
    void BuildSearchField(VisualElement root) {
      ...
      m_SearchTextField.menu.AppendAction(
          EditorGUIUtility.TrTextContent("Command").text,
          a => SearchOptionSelected(SearchOption.Name),
          a => m_ViewController.searchMode ==
               SearchOption.Name ? DropdownMenuAction.Status.Checked
          : DropdownMenuAction.Status.Normal);

      m_SearchTextField.menu.AppendAction(
          EditorGUIUtility.TrTextContent("Shortcut").text,
          a => SearchOptionSelected(SearchOption.Binding),
          a => m_ViewController.searchMode ==
               SearchOption.Binding ? DropdownMenuAction.Status.Checked
          : DropdownMenuAction.Status.Normal);
      ...
      m_KeyBindingSearchField.menu.AppendAction(
          EditorGUIUtility.TrTextContent("Command").text,
          a => SearchOptionSelected(SearchOption.Name),
          a => m_ViewController.searchMode ==
               SearchOption.Name ? DropdownMenuAction.Status.Checked
    ...
    void BuildProfileManagementRow(VisualElement header) {
      m_ActiveProfileDropdownButton =
          header.Q<TextElement>("activeProfileDropdownButtonText");
      m_ActiveProfileDropdownButton.text = m_ViewController.activeProfile;
      m_ActiveProfileDropdownButton.AddToClassList(
          PopupField<string>.textUssClassName);
      ...
      activeProfileDropdownButton.AddToClassList(
          BasePopupField<string, string>.inputUssClassName);
      activeProfileDropdownButton.RegisterCallback<MouseDownEvent>(
          OnProfileContextMenuMouseDown);

      var activeProfileDropdownArrow = activeProfileDropdownButton.Q("arrow");
      activeProfileDropdownArrow.AddToClassList(
    ...
    void OnCreateProfileClicked() {
      PromptWindow.Show(
          L10n.Tr("Create profile"), L10n.Tr("Create a shortcut profile"),
          L10n.Tr("Enter the name of the profile you want to create"),
          L10n.Tr("Profile Name:"), L10n.Tr("New profile"), L10n.Tr("Create"),
          m_ViewController.CanCreateProfile, m_ViewController.CreateProfile);
    }

    void OnRenameProfileClicked() {
      PromptWindow.Show(
          L10n.Tr("Rename profile"), L10n.Tr("Rename a shortcut profile"),
          string.Format(
              L10n.Tr("Enter the new name you want to give the profile '{0}'"),
              m_ViewController.activeProfile),
          L10n.Tr("Profile Name:"), m_ViewController.activeProfile,
          L10n.Tr("Rename"), m_ViewController.CanRenameActiveProfile,
          m_ViewController.RenameActiveProfile);
    }

    void OnDeleteProfileClicked() {
      DeleteShortcutProfileWindow.Show(
          m_ViewController.activeProfile,
    ...
    void OnProfileContextMenuMouseDown(MouseDownEvent evt) {
      ...
      foreach (var profile in m_ViewController.GetAvailableProfiles()) {
        genericMenu.AddItem(new GUIContent(profile), false,
      ...
      genericMenu.AddItem(
          EditorGUIUtility.TrTextContent("Create new profile..."), false,
          OnCreateProfileClicked);

      if (m_ViewController.CanRenameActiveProfile())
        genericMenu.AddItem(EditorGUIUtility.TrTextContent("Rename profile..."),
                            false, OnRenameProfileClicked);
      else
        genericMenu.AddDisabledItem(
            EditorGUIUtility.TrTextContent("Rename profile..."));

      if (m_ViewController.CanDeleteActiveProfile())
        genericMenu.AddItem(EditorGUIUtility.TrTextContent("Delete profile..."),
                            false, OnDeleteProfileClicked);
      else
        genericMenu.AddDisabledItem(
            EditorGUIUtility.TrTextContent("Delete profile..."));
    ...
    void BuildLegendRow(VisualElement root) {
      ...
      var labels = new[]{L10n.Tr("Unassigned Key"), L10n.Tr("Assigned Key"),
                         L10n.Tr("Global Key")};
    ...
    void UpdateShortcutTableSearchFilter() {
      ...
      // TODO: when I clear elements like this, do I need to unregister any
      // callbacks registered to them?
    ...
    void BuildVisualElementHierarchyRoot() {
      ...
      var headerTemplate =
          EditorResources.Load("UXML/ShortcutManager/ShortcutManagerView.uxml",
                               typeof(UnityEngine.Object)) as VisualTreeAsset;
      ...
      m_ShortcutsTableSearchFilterContainer =
          shortcutsTableContainer.Q("shortcutsTableSearchFilterContainer");
      m_SearchFiltersContainer =
          shortcutsTableContainer.Q("searchFiltersContainer");

      m_KeyboardElement = new Keyboard(
          m_BindingStateProvider, m_ViewController.GetSelectedKey(),
          m_ViewController.GetSelectedEventModifiers());
      ...
      m_CategoryTreeView =
          new ListView((IList) m_ViewController.GetCategories(),
                       k_ListItemHeight, MakeItemForCategoriesTable,
                       BindCategoriesTableItem){name = "categoryTreeView"};
      m_ShortcutsTable =
          new ListView((IList) m_ViewController.GetShortcutList(),
                       k_ListItemHeight, MakeItemForShortcutTable,
                       BindShortcutEntryItem){name = "shortcutsTable"};
      ...
      m_ShortcutsTable.RegisterCallback<MouseDownEvent>(
          ShortcutTableRightClickDown);
      m_ShortcutsTable.RegisterCallback<MouseUpEvent>(
          ShortcutTableRightClickUp);
      m_ShortcutsTable.RegisterCallback<GeometryChangedEvent>(
          ShortcutTableGeometryChanged);

      m_Root.AddToClassList("ShortcutManagerView");
      if (EditorGUIUtility.isProSkin)
        m_Root.AddToClassList("isProSkin");
      m_Root.AddStyleSheetPath(
          "StyleSheets/ShortcutManager/ShortcutManagerView.uss");

      // Set up resize handle manipulator
      // TODO: replace k_PixelsPadding with width of the text from the left
      // header element after UXML/USS migration.
      m_HorizontalColumnDragger =
          new HorizontalColumnDragger(k_PixelsPadding, m_ShortcutsTable);
    ...
    void SetLocalizedText() {
      m_Root.Q<TextElement>("categoryTableHeaderName").text =
          L10n.Tr("Category");
      m_Root.Q<TextElement>("shortcutsTableHeaderName").text =
          L10n.Tr("Command");
      m_Root.Q<TextElement>("shortcutsTableHeaderBindings").text =
          L10n.Tr("Shortcut");
    ...
    GenericMenu GetContextMenuForEntries(IEnumerable<ShortcutEntry> entries) {
      ...
      foreach (var entry in entries) {
        ...
        var mangledBinding =
            entry.combinations.FirstOrDefault().ToString().Replace("/",
                                                                   "Slash");
        var rootItemLabel = $"{mangledPath} ({mangledBinding})";
        if (entry.overridden)
          menu.AddItem(
              new GUIContent(
                  $"{rootItemLabel}/{L10n.Tr(" Reset to default ")}"),
              false, (x) => { m_ViewController.ResetToDefault(entry); }, entry);
        else
          menu.AddDisabledItem(new GUIContent(
              $"{rootItemLabel}/{L10n.Tr(" Reset to default ")}"));
        menu.AddItem(
            new GUIContent($"{rootItemLabel}/{L10n.Tr(" Remove shortcut ")}"),
            false, (x) => { m_ViewController.RemoveBinding(entry); }, entry);
      }

      return menu;
    }

    private GenericMenu GetContextMenuForKey(KeyCode keyCode,
                                             EventModifiers modifiers) {
      return GetContextMenuForEntries(
          m_ViewController.GetShortcutsBoundTo(keyCode, modifiers));
    }
    ...
    string GetToolTipForKey(KeyCode keyCode, EventModifiers modifiers) {
      ...
      if (builder.Length > 0) {
        while (builder[builder.Length - 1] == '\r' ||
    ...
    void StartRebind(ShortcutEntry entry, VisualElement row) {
      ...
      if (row != null) {
        ...
        HideElement(warningIcon);
        bindingInput.RegisterCallback<GeometryChangedEvent>(
            FocusElementDelayed);
      }
    }
    ...
    void EndRebind(bool refresh = true) {
      ...
      // TODO: this refresh causes issues when trying to double click another
      // binding, while a binding is being edited.
      if (refresh)
        m_ShortcutsTable.Refresh();
    ...
    void ShortcutTableRightClickDown(MouseDownEvent evt) {
      var slider =
          m_ShortcutsTable.Q<Scroller>(className
                                       : Scroller.verticalVariantUssClassName);
      var clickedIndex = (int)((evt.localMousePosition.y + slider.value) /
                               m_ShortcutsTable.resolvedItemHeight);
    ...
    void ShortcutTableRightClickUp(MouseUpEvent evt) {
      if (evt.button != 1 || m_ViewController.selectedEntry == null)
        return;
      GenericMenu menu =
          GetContextMenuForEntries(new[]{m_ViewController.selectedEntry});
      menu.ShowAsContext();
    }
    ...
    bool CanEntryBeAssignedToKey(KeyCode keyCode, EventModifiers eventModifier,
                                 ShortcutEntry entry) {
      return m_ViewController.CanEntryBeAssignedToKey(keyCode, eventModifier,
                                                      entry);
    }

    void OnKeyboardKeyDragPerformed(KeyCode keyCode,
    ...
    void OnMouseDownCategoryTable(MouseDownEvent evt) {
      ...
      visualElement.RegisterCallback<MouseLeaveEvent>(
          OnMouseLeaveWhileButtonDownCategoryTable);
    }
    ...
    void OnMouseUpCategoryTable(MouseUpEvent evt) {
      ...
      var visualElement = ((VisualElement) evt.currentTarget);
      visualElement.UnregisterCallback<MouseMoveEvent>(
          OnMouseMoveCategoryTable);
    ...
    void StartDrag(VisualElement target) {
      ...
      target.UnregisterCallback<MouseLeaveEvent>(
          OnMouseLeaveWhileButtonDownCategoryTable);
      m_StartedDrag = true;
      DragAndDrop.activeControlID =
          target.GetHashCode(); // TODO: how to handle activeControlID in
                                // UIELements
      DragAndDrop.PrepareStartDrag();
      DragAndDrop.SetGenericData(
          "ShortcutCommandItem",
          m_ViewController.GetShortcutList()[m_ShortcutsTable.selectedIndex]);
  ...
  class Keyboard : VisualElement {
    ...
    [NonSerialized] Dictionary<EventModifiers, List<Key>> m_ModifierKeys =
        new Dictionary<EventModifiers, List<Key>>();

    [NonSerialized] List<Key> m_AllKeys = new List<Key>();
    ...
    internal event
        Func<KeyCode, EventModifiers, GenericMenu> ContextMenuProvider;

    public Keyboard(
        IKeyBindingStateProvider keyBindingStateProvider,
        KeyCode initiallySelectedKey = KeyCode.None,
        EventModifiers initiallyActiveModifiers = EventModifiers.None) {
      ...
      var keysList = new List<KeyDef[]>{
          new[]{new KeyDef(KeyCode.Escape, "Esc"), new KeyDef(KeyCode.None),
                new KeyDef(KeyCode.F1), new KeyDef(KeyCode.F2),
                new KeyDef(KeyCode.F3), new KeyDef(KeyCode.F4),
                new KeyDef(KeyCode.None), new KeyDef(KeyCode.F5),
                new KeyDef(KeyCode.F6), new KeyDef(KeyCode.F7),
                new KeyDef(KeyCode.F8), new KeyDef(KeyCode.None),
                new KeyDef(KeyCode.F9), new KeyDef(KeyCode.F10),
                new KeyDef(KeyCode.F11), new KeyDef(KeyCode.F12)},
          new[]{
              new KeyDef(KeyCode.BackQuote, "`"),
              new KeyDef(KeyCode.Alpha1, "1"), new KeyDef(KeyCode.Alpha2, "2"),
              new KeyDef(KeyCode.Alpha3, "3"), new KeyDef(KeyCode.Alpha4, "4"),
              new KeyDef(KeyCode.Alpha5, "5"), new KeyDef(KeyCode.Alpha6, "6"),
              new KeyDef(KeyCode.Alpha7, "7"), new KeyDef(KeyCode.Alpha8, "8"),
              new KeyDef(KeyCode.Alpha9, "9"), new KeyDef(KeyCode.Alpha0, "0"),
              new KeyDef(KeyCode.Minus, "-"), new KeyDef(KeyCode.Equals, "="),
              new KeyDef(KeyCode.Backspace, "←")},
          new[]{new KeyDef(KeyCode.Tab), new KeyDef(KeyCode.Q),
                new KeyDef(KeyCode.W), new KeyDef(KeyCode.E),
                new KeyDef(KeyCode.R), new KeyDef(KeyCode.T),
                new KeyDef(KeyCode.Y), new KeyDef(KeyCode.U),
                new KeyDef(KeyCode.I), new KeyDef(KeyCode.O),
                new KeyDef(KeyCode.P), new KeyDef(KeyCode.LeftBracket, "["),
      ...
      var cursorControlKeysList = new List<KeyDef[]>(){
          new[]{new KeyDef(KeyCode.F13), new KeyDef(KeyCode.F14),
                new KeyDef(KeyCode.F15)},
          new[]{new KeyDef(KeyCode.Insert, "Ins"),
                new KeyDef(KeyCode.Home, "Hom"),
                new KeyDef(KeyCode.PageUp, "Pg Up")},
          new[]{new KeyDef(KeyCode.Delete, "Del"), new KeyDef(KeyCode.End),
      ...
      var compactContainer =
          new VisualElement(){name = "compactKeyboardContainer"};
      var cursorControlContainer =
          new VisualElement(){name = "cursorControlKeyboardContainer"};

      BuildKeyboardVisualTree(keysList, dictionaryKeyStyle, compactContainer);

      BuildKeyboardVisualTree(cursorControlKeysList, dictionaryKeyStyle,
                              cursorControlContainer);
    ...
    void HandleModifierKeysCommand<T>(CommandEventBase<T> evt) where T
        : CommandEventBase<T>,
          new () {
    ...
    void OnDragEnter(DragEnterEvent evt) { DoDragUpdate(evt.target); }

    void OnDragUpdated(DragUpdatedEvent evt) { DoDragUpdate(evt.target); }
    ...
    void DoDragUpdate(IEventHandler target) {
      ...
      var shortcutEntry =
          (ShortcutEntry) DragAndDrop.GetGenericData("ShortcutCommandItem");
      if (shortcutEntry == null)
        return;

      if (CanDrop != null &&
          CanDrop(keyElement.key, m_CurrentModifiers, shortcutEntry))
    ...
    void BuildKeyboardVisualTree(List<KeyDef[]> keysList,
                                 Dictionary<KeyCode, string> dictionaryKeyStyle,
          ...
          if (keyDef.keycode != KeyCode.None) {
            ...
            if (m_KeyBindingStateProvider.IsModifier(keyDef.keycode)) {
              EventModifiers modifier =
                  m_KeyBindingStateProvider.ModifierFromKeyCode(keyDef.keycode);
              List<Key> keyList;
    ...
    void StyleKey(Key key) {
      ...
      var bindingState =
          m_KeyBindingStateProvider.GetBindingStateForKeyWithModifiers(
              key.key, m_CurrentModifiers);
    ...
    void SetModifiers(EventModifiers modifiers) {
      ...
      foreach (var modifierPair in m_ModifierKeys) {
        var modifierPressed =
            (m_CurrentModifiers & modifierPair.Key) == modifierPair.Key;
    ...
    void OnMouseDown(MouseDownEvent evt) {
      ...
      if (keyElement != null) {
        ...
        case 1:
          var menu =
              ContextMenuProvider?.Invoke(keyElement.key, m_CurrentModifiers);
    ...
    void SetKeySelected(Key keyElement) {
      if (!m_KeyBindingStateProvider.CanBeSelected(keyElement.key)) {
        EventModifiers modifier =
            m_KeyBindingStateProvider.ModifierFromKeyCode(keyElement.key);
    ...
    public void Refresh() { RefreshKeyboardBoundVisuals(); }
  }
  ...
  class ShortcutTextField : TextInputBaseField<List<KeyCombination>> {
    ...
    public ShortcutTextField()
        : base(kMaxLengthNone, char.MinValue, new ShortcutInput()) {
      ...
      if (kde.keyCode != KeyCode.None) {
        if (!m_KeyDown.Contains(kde.keyCode)) {
          ...
          if (kde.keyCode == KeyCode.Escape) {
            ...
            Apply();
            kde.StopPropagation();
          } else if (!ShortcutManagerWindowViewController.IsModifier(
                         kde.keyCode)) {
  ...
  internal class ShortcutSearchField
      : SearchFieldBase<ShortcutTextField, List<KeyCombination>> {
    public new static readonly string ussClassName =
        "unity-shortcut-search-field";
    ...
    protected override void ClearTextField() { value = null; }
  ...
  internal class ShortcutPopupSearchField : ShortcutSearchField,
                                            IToolbarMenuElement {
  ...
  internal class HorizontalColumnDragger : MouseManipulator {
    const string k_ShortcutmanagerColumnposition =
        "ShortcutManager_ColumnPosition";
    Vector2 m_Start;
    bool m_Active;
    ...
    public HorizontalColumnDragger(float clampPositionPadding,
                                   ListView elements) {
      activators.Add(
          new ManipulatorActivationFilter{button = MouseButton.LeftMouse});
      m_Active = false;

      // TODO: Replace this with the maximum text width of the left and right
      // TextElements
      m_ClampPositionPadding = clampPositionPadding;
      m_Elements = elements;

      m_Elements.Q(className
                   : ScrollView.vScrollerUssClassName)
          .RegisterCallback<GeometryChangedEvent>(
              ShortcutsTableGeometryChanged);
    }
    ...
    public void UpdatePositions() {
      if (m_TargetPosition == 0) {
        m_TargetPosition = EditorPrefs.GetFloat(k_ShortcutmanagerColumnposition,
                                                target.parent.layout.width / 2);
      }

      m_TargetPosition =
          Mathf.Clamp(m_TargetPosition, m_ClampPositionPadding,
                      target.parent.layout.width - m_ClampPositionPadding -
    ...
    public void RequestUpdatePositions() { m_UpdatePositionsRequested = true; }
    ...
    void UpdateElement(VisualElement el, float handlePosition, float offset) {
      var totalPadding =
          el.ElementAt(0).computedStyle.paddingLeft.value.value +
          el.ElementAt(0).computedStyle.paddingRight.value.value +
          el.ElementAt(1).computedStyle.paddingLeft.value.value +
          el.ElementAt(1).computedStyle.paddingRight.value.value;
      var percentage =
          (handlePosition + (target.computedStyle.width.value.value) / 2 -
           totalPadding / 2) /
          (target.parent.layout.width - (totalPadding + offset));
