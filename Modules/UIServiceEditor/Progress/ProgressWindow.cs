...
internal class ProgressWindow : EditorWindow {
  internal const string ussPath =
      "StyleSheets/ProgressWindow/ProgressWindow.uss";
  internal const string ussPathDark =
      "StyleSheets/ProgressWindow/ProgressWindowDark.uss";
  internal const string ussPathLight =
      "StyleSheets/ProgressWindow/ProgressWindowLight.uss";
  ...
  internal static void ShowDetails(bool shouldReposition) {
    ...
    if (newWindowCreated && shouldReposition) {
      ...
      m_Window.position =
          new Rect(mainWindowRect.xMax - WindowWidth - 6,
                   mainWindowRect.yMax - WindowHeight - 50, size.x, size.y);
    ...
    foreach (var op in ops) {
      ...
      for (int i = 0; i < m_Elements.Count; ++i) {
        if (m_Elements[i].dataSource.id == op.id ||
            m_Elements[i].dataSource.id == op.parentId) {
          if (m_Elements[i].dataSource.id == op.id &&
              m_Elements[i].dataSource.status != op.status)
      ...
      if (statusWasChanged && op.parentId < 0 &&
          op.status == Progress.Status.Failed) {
  ...
  private int
  FindIndexFirstSucceededOrCanceledElement(List<ProgressElement> elements) {
    int firstSucceededOrCanceledIndex = elements.Count;
    for (int i = elements.Count - 1; i >= 0; i--) {
      if (elements[i].dataSource.status == Progress.Status.Succeeded ||
          elements[i].dataSource.status == Progress.Status.Canceled)
  ...
  private UIElements.ToolbarToggle
  CreateStatusFiler(VisualElement parent, string controlName, string iconName) {
    var filterToggle = new UIElements.ToolbarToggle();
    filterToggle.tooltip = controlName;
    filterToggle.value =
        EditorPrefs.GetBool($"{preferenceKey}{controlName}_filter", true);
    var toggleCheck = filterToggle.Q<VisualElement>("unity-checkmark");
    toggleCheck.style.backgroundImage =
        new StyleBackground(EditorGUIUtility.LoadIcon(iconName));
    ...
    filterToggle.RegisterCallback<ChangeEvent<bool>>((evt) => {
      EditorPrefs.SetBool($"{preferenceKey}{controlName}_filter",
                          filterToggle.value);
      UpdateStatusFilter();
    });
  ...
  private void ClearAll() {
    var finishedTasks = m_Elements.Where(el => !el.dataSource.running)
                            .Select(el => el.dataSource.id)
                            .ToArray();
  ...
  private void UpdateNbTasks() {
    if (m_Elements.Count > 1)
      titleContent = EditorGUIUtility.TrTextContent(
          $"Background Tasks ({m_Elements.Count})");
    else
  ...
  private ProgressElement AddElement(Progress.Item progressItem) {
    ...
    if (m_Elements.Count > 0 && progressItem.status == Progress.Status.Failed) {
      var insertIndex = -1;
      for (int i = 0; i < m_Elements.Count; i++) {
        if ((m_Elements[i].dataSource.status == Progress.Status.Failed &&
             progressItem.updateTime > m_Elements[i].dataSource.updateTime) ||
      ...
      m_ScrollView.Insert(m_LastParentFailedIndex + 1,
