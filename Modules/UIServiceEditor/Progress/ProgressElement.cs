...
internal class DisplayedTask {
  ...
  public DisplayedTask(Label name, Label progress,
                       VisualElement descriptionIcon, Label description,
                       Label elapsedTime, ProgressBar progressBar,
                       Button cancelButton) {
    nameLabel = name;
  ...
  public void SetIndefinite(bool indefinite) {
    if (indefinite) {
      var progressTotalWidth = float.IsNaN(progressBar.worldBound.width)
                                   ? 146
                                   : progressBar.worldBound.width;
  ...
  public void SetProgressStyleFull(bool styleFull) {
    var isAlreadyFull =
        progress.ClassListContains("unity-progress-bar__progress__full");
...
internal class ProgressElement {
  private const string k_UxmlProgressPath =
      "UXML/ProgressWindow/ProgressElement.uxml";
  private const string k_UxmlSubTaskPath =
      "UXML/ProgressWindow/SubTaskElement.uxml";
  ...
  public Progress.Item dataSource {
    get;
    private set;
  }
  ...
  public ProgressElement(Progress.Item dataSource) {
    ...
    if (s_VisualTreeBackgroundTask == null)
      s_VisualTreeBackgroundTask =
          EditorGUIUtility.Load(k_UxmlProgressPath) as VisualTreeAsset;
    ...
    if (s_VisualTreeSubTask == null)
      s_VisualTreeSubTask =
          EditorGUIUtility.Load(k_UxmlSubTaskPath) as VisualTreeAsset;
  ...
  internal void UpdateRunningTime() {
    ...
    if (dataSource.timeDisplayMode ==
            Progress.TimeDisplayMode.ShowRemainingTime &&
        !dataSource.finished)
      m_MainTask.elapsedTimeLabel.text =
          FormatRemainingTime(dataSource.remainingTime);
    else
      m_MainTask.elapsedTimeLabel.text =
          $"{m_MainTask.lastElapsedTime:0} seconds";
  }
  ...
  private static void UpdateResponsiveness(DisplayedTask task,
                                           Progress.Item dataSource) {
    if (dataSource.responding && !task.isResponding) {
      task.descriptionLabel.text = dataSource.description;
      if (task.progress.ClassListContains(
              "unity-progress-bar__progress__unresponding")) {
        task.progress.RemoveFromClassList(
            "unity-progress-bar__progress__unresponding");
      }
    } else if (!dataSource.responding && task.isResponding) {
      task.descriptionLabel.text =
          string.IsNullOrEmpty(dataSource.description)
              ? "(Not Responding)"
              : $"{dataSource.description} (Not Responding)";
      if (!task.progress.ClassListContains(
              "unity-progress-bar__progress__unresponding")) {
        task.progress.AddToClassList(
            "unity-progress-bar__progress__unresponding");
      }
    }
  ...
  private void UpdateDisplay(DisplayedTask task, Progress.Item dataSource) {
    ...
    if (dataSource.status == Progress.Status.Canceled) {
      ...
      task.progressLabel.style.unityBackgroundImageTintColor =
          new StyleColor(Color.green);

      // Update running time to force elapsed time to show when the task is set
      // to show ETA
      UpdateRunningTime();
  ...
  private static void UpdateStatusIcon(DisplayedTask task, string iconName) {
    task.descriptionIcon.style.backgroundImage =
        EditorGUIUtility.LoadIcon(iconName);
  ...
  private void SubTaskInitialization(Progress.Item subTaskSource) {
    ...
    DisplayedTask displayedSubTask =
        InitializeTask(subTaskSource, parentElement);
  ...
  private DisplayedTask InitializeTask(Progress.Item progressItem,
    ...
    displayedTask.cancelButton.clickable.clickedWithEventInfo +=
        CancelButtonClicked;
  ...
  private void OnCancelled() { UpdateDisplay(m_MainTask, dataSource); }
