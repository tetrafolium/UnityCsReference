...
void InitializeToolIcons() {
	...
	s_ToolIcons[index++] = EditorGUIUtility.TrIconContent(
		"TransformTool", "Move, Rotate or Scale selected objects.");
	...
	s_CustomToolIcon = EditorGUIUtility.TrIconContent(
		"CustomTool", "Available Custom Editor Tools");
	...
	s_ViewToolIcons = new GUIContent[] {
		...
		EditorGUIUtility.TrIconContent("ViewToolOrbit", viewToolsTooltipText),
		EditorGUIUtility.TrIconContent("ViewToolOrbit",
		                               "Orbit the Scene view."),
		EditorGUIUtility.TrIconContent("ViewToolOrbit On",
		                               viewToolsTooltipText),
	...
	s_LayerContent = EditorGUIUtility.TrTextContent(
		"Layers", "Which layers are visible in the Scene views.");
	...
	s_PreviewPackageContent =
		EditorGUIUtility.TrTextContent("Preview Packages in Use");
	s_PreviewPackageIcon = EditorGUIUtility.TrIconContent(
		"PreviewPackageInUse", "Preview Packages in Use");
...
public static readonly GUIStyle previewPackageInUseDropdown =
	"PreviewPackageInUse";
...
protected void OnEnable() {
	...
	m_IsPreviewPackagesInUse =
		PackageManager.PackageInfo.GetAll().FirstOrDefault(
			info => info.version.Contains("preview") ||
			info.version.StartsWith("0.")) != null;
	m_PackageManagerPrefs =
		ServicesContainer.instance.Resolve<PackageManagerPrefs>();
...
// The actual array we display. We build this every frame to make sure it
// looks correct i.r.t. selection :)
...
public override void OnGUI() {
	...
	bool isOrWillEnterPlaymode =
		EditorApplication.isPlayingOrWillChangePlaymode;
	GUI.color = isOrWillEnterPlaymode ? HostView.kPlayModeDarken : Color.white;

	if (Event.current.type == EventType.Repaint)
		Styles.appToolbar.Draw(new Rect(0, 0, position.width, position.height),
	...
	int playModeControlsStart =
		Mathf.RoundToInt((position.width - playPauseStopWidth) / 2);
	...
	if (UnityEditor.MPE.ProcessService.level ==
		...
		ReserveWidthLeft(dropdownWidth, ref pos);
		if (EditorGUI.DropdownButton(EditorToolGUI.GetThinArea(pos),
		                             s_AccountContent, FocusType.Passive,
		                             Styles.dropdown)) {
	...
	if (Unsupported.IsDeveloperBuild() && ModeService.hasSwitchableModes) {
		...
		ReserveWidthLeft(space, ref pos);
		ReserveWidthLeft(dropdownWidth, ref pos);
		var selectedModeIndex = EditorGUI.Popup(
			EditorToolGUI.GetThinArea(pos), ModeService.currentIndex,
			ModeService.modeNames, Styles.dropdown);
		if (EditorGUI.EndChangeCheck()) {
			EditorApplication.delayCall += () =>
			                               ModeService.ChangeModeByIndex(selectedModeIndex);
			GUIUtility.ExitGUI();
		}
	}

	if (m_IsPreviewPackagesInUse &&
	    !m_PackageManagerPrefs.dismissPreviewPackagesInUse) {
		ReserveWidthLeft(space, ref pos);

		var useIcon = Toolbar.get.mainToolbar.position.width <
		              k_MinWidthChangePreviewPackageInUseToIcon;
		ReserveWidthLeft(useIcon ? previewPackagesinUseIconWidth
		       : previewPackagesinUseWidth, ref pos);

		var dropDownCustomColor =
			new GUIStyle(Styles.previewPackageInUseDropdown);

		if (EditorGUI.DropdownButton(EditorToolGUI.GetThinArea(pos),
		                             useIcon ? s_PreviewPackageIcon
		                   : s_PreviewPackageContent, FocusType.Passive,
		                             dropDownCustomColor))
...
void ShowPreviewPackageInUseMenu(Rect rect) {
	...
	// Here we open the package manager, In-Project open and search field have
	// preview.
	menu.AddItem(EditorGUIUtility.TrTextContent("Show Preview Packages..."),
	             false, () => {
				PackageManagerWindow.SelectPackageAndFilterStatic(
					string.Empty, PackageFilterTab.InProject, true,
					"preview");
			});
	menu.AddSeparator("");
	...
	menu.AddItem(
		EditorGUIUtility.TrTextContent("Why am I seeing this?"), false, () => {
				m_ApplicationProxy.OpenURL(
					$"https://docs.unity3d.com/{m_ApplicationProxy.shortUnityVersion}/Documentation/Manual/pack-preview.html");
			});
	...
	if (!UnityConnect.instance.online ||
		...
		if (!Application.HasProLicense()) {
			...
			menu.AddDisabledItem(
				EditorGUIUtility.TrTextContent("Upgrade to Unity Plus or Pro"));
		}
	} else {
		string accountUrl =
			UnityConnect.instance.GetConfigurationURL(CloudConfigUrl.CloudPortal);
		if (UnityConnect.instance.loggedIn)
			menu.AddItem(EditorGUIUtility.TrTextContent("Go to account"), false,
			             () => UnityConnect.instance.OpenAuthorizedURLInWebBrowser(
					     accountUrl));
		else
			menu.AddDisabledItem(EditorGUIUtility.TrTextContent("Go to account"));
		...
		if (UnityConnect.instance.loggedIn) {
			...
			menu.AddItem(EditorGUIUtility.TrTextContent("Sign in..."), false,
		...
		if (!Application.HasProLicense()) {
			...
			menu.AddItem(
				EditorGUIUtility.TrTextContent("Upgrade to Unity Plus or Pro"),
				false, () => Application.OpenURL("https://store.unity.com/"));
		}
	}
...
void DoToolButtons(Rect rect) {
	...
	for (int i = 1; i < builtinIconsLength; i++) {
		s_ShownToolIcons[i] =
			s_ToolIcons[i - 1 +
			            (i == displayTool ? s_ShownToolIcons.Length - 1 : 0)];
	...
	if (lastCustomTool != null)
		s_ShownToolIcons[builtinIconsLength] =
			lastCustomTool.toolbarIcon ?? s_CustomToolIcon;
	else
		s_ShownToolIcons[builtinIconsLength] = s_CustomToolIcon;

	s_ShownToolIcons[0] =
		s_ViewToolIcons[(int) Tools.viewTool +
		                (displayTool == 0 ? s_ViewToolOnOffset : 0)];

	displayTool =
		GUI.Toolbar(rect, displayTool, s_ShownToolIcons, s_ToolControlNames,
		            Styles.command, GUI.ToolbarButtonSize.FitToContents);

	if (GUI.changed) {
		var evt = Event.current;

		if (displayTool == (int) Tool.Custom &&
		    (EditorToolContext.GetLastCustomTool() == null || evt.button == 1 ||
...
void DoToolSettings(Rect rect) {
	...
	EditorToolGUI.DoBuiltinToolSettings(rect, Styles.buttonLeft,
	                                    Styles.buttonRight);
}

void DoSnapButtons(Rect rect) {
	using (new EditorGUI.DisabledScope(
		       !EditorSnapSettings.activeToolSupportsGridSnap)) {
		...
		EditorSnapSettings.gridSnapEnabled =
			GUI.Toggle(rect, snap, icon, Styles.command);
	}
}
...
void DoPlayButtons(bool isOrWillEnterPlaymode) {
	...
	GUI.contentColor =
		new Color(1.0f / c.r, 1.0f / c.g, 1.0f / c.g, 1.0f / c.a);
	GUI.SetNextControlName("ToolbarPlayModePlayButton");
	GUILayout.Toggle(isOrWillEnterPlaymode, s_PlayIcons[buttonOffset],
	                 isPlaying ? Styles.commandLeftOn
	             : Styles.commandLeft);
	...
	bool isPaused =
		GUILayout.Toggle(EditorApplication.isPaused,
		                 s_PlayIcons[buttonOffset + 1], Styles.commandMid);
	if (GUI.changed) {
...
void DoLayersDropDown(Rect rect) {
	if (EditorGUI.DropdownButton(rect, s_LayerContent, FocusType.Passive,
	                             Styles.dropdown)) {
...
void DoLayoutDropDown(Rect rect) {
	...
	if (EditorGUI.DropdownButton(rect,
	                             GUIContent.Temp(Toolbar.lastLoadedLayoutName),
	                             FocusType.Passive, Styles.dropdown)) {
