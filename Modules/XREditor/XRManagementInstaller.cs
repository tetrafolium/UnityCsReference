...
namespace UnityEditor.XR {
  ...
  class XRManagementInstaller : SettingsProvider {
    ...
    static string s_ManagementPackagePath =
        $"Packages/{s_ManagementPackageId}/package.json";

    private PackageManager.Requests.AddRequest m_AddManagementRequest = null;

    static class Content {
      internal const string s_InstallationHelpText =
      ...
      internal static string s_SettingsRootTitle =
          "Project/XR Plugin Management";
      internal static GUIContent s_AddXrManagement =
          EditorGUIUtility.TrTextContent("Install XR Plugin Management");
      internal static GUIContent s_DownloadingText =
          new GUIContent("Downloading XR Management system...");
      internal static GUIContent s_InstallingText =
          new GUIContent("Installing XR Management system...");
    }

    [UnityEngine.Internal.ExcludeFromDocs] XRManagementInstaller(
        string path, SettingsScope scopes = SettingsScope.Project)
        : base(path, scopes) {}
    ...
    public override void OnGUI(string searchContext) {
      if (m_AddManagementRequest != null) {
        ...
        EditorGUILayout.HelpBox(Content.s_InstallationHelpText,
                                MessageType.Info);
        if (GUILayout.Button(Content.s_AddXrManagement)) {
          m_AddManagementRequest =
              PackageManager.Client.Add(s_ManagementPackageId);
        }
      }
    }
    ...
    static SettingsProvider CreateXRManagementInstallerProvider() {
      try {
        ...
        if (String.IsNullOrEmpty(packagePath) || !File.Exists(packagePath) ||
            String.Compare(s_ManagementPackagePath, packagePath, true) == 0)
