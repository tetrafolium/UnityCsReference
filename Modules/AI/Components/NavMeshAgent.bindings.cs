...
namespace UnityEngine.AI {
  ...
  public sealed class NavMeshAgent : Behaviour {
    // Sets or updates the destination. This triggers calculation for a new
    // path.
    ...
    public extern Vector3 nextPosition {
      get;
      set;
    }

    // The current steering target - usually the next corner or end point of the
    // current path. (RO)
    public extern Vector3 steeringTarget { get; }

    // The desired velocity of the agent including any potential contribution
    // from avoidance. (RO)
    public extern Vector3 desiredVelocity { get; }

    // Remaining distance along the current path - or infinity when not known.
    // (RO)
    ...
    public extern bool isOnOffMeshLink { [NativeName("IsOnOffMeshLink")] get; }
    ...
    [FreeFunction(
        "NavMeshAgentScriptBindings::GetCurrentOffMeshLinkDataInternal",
    ...
    public OffMeshLinkData nextOffMeshLinkData =>
        GetNextOffMeshLinkDataInternal();

    [FreeFunction("NavMeshAgentScriptBindings::GetNextOffMeshLinkDataInternal",
                  HasExplicitThis = true)]
    internal extern OffMeshLinkData GetNextOffMeshLinkDataInternal();

    // Terminate OffMeshLink occupation and transfer the agent to the closest
    // point on other side.
    ...
    public extern bool hasPath { [NativeName("HasPath")] get; }

    // A path is being computed, but not yet ready. (RO)
    public extern bool pathPending { [NativeName("PathPending")] get; }

    // Is the current path stale. (RO)
    public extern bool isPathStale { [NativeName("IsPathStale")] get; }
    ...
    public extern Vector3 pathEndPosition {
      get;
    }
    ...
    public extern bool isStopped {
      [FreeFunction("NavMeshAgentScriptBindings::GetIsStopped",
                    HasExplicitThis = true)] get;
      [FreeFunction("NavMeshAgentScriptBindings::SetIsStopped",
                    HasExplicitThis = true)] set;
    }

    // Clears the current path. Note that this agent will not start looking for
    // a new path until SetDestination is called.
    ...
    // Trace movement towards a target position in the NavMesh. Without moving
    // the agent.
    ...
    [FreeFunction("NavMeshAgentScriptBindings::CalculatePathInternal",
                  HasExplicitThis = true)] extern bool
    CalculatePathInternal(Vector3 targetPosition, [ NotNull ] NavMeshPath path);

    // Sample a position along the current path.
    public extern bool SamplePathPosition(int areaMask, float maxDistance,
                                          out NavMeshHit hit);
    ...
    [NativeName("GetCurrentPolygonOwner")] extern Object GetOwnerInternal();
    ...
    public extern bool isOnNavMesh { [NativeName("InCrowdSystem")] get; }
  }
}
