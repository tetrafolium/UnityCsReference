...
namespace UnityEngine.Experimental.AI {
  ...
  public enum PathQueryStatus {
    ...
    BufferTooSmall =
        1
        << 4, // Result buffer for the query was too small to store all results.
    OutOfNodes = 1 << 5, // Query ran out of nodes during search.
    PartialResult =
        1 << 6 // Query did not reach the end location, returning best guess.
  }

  // Flags describing polygon properties. Keep in sync with the enum declared in
  // NavMesh.h
  ...
  public struct NavMeshWorld {
    ...
    public bool IsValid() { return world != IntPtr.Zero; }
  ...
  public struct NavMeshQuery : IDisposable {
    ...
    const string k_NoBufferAllocatedErrorMessage =
        "This query has no buffer allocated for pathfinding operations. " +
    ...
    public void Dispose() {
      ...
      // Since it has been deallocated, we shouldn't deregister the query from
      // it... We need to extract removeQuery before disposing the handle,
    ...
    static extern void AddQuerySafety(IntPtr navMeshQuery,
                                      AtomicSafetyHandle handle);
    static extern void RemoveQuerySafety(IntPtr navMeshQuery,
                                         AtomicSafetyHandle handle);

    [ThreadSafe]
    static extern bool HasNodePool(IntPtr navMeshQuery);

    public unsafe PathQueryStatus
    BeginFindPath(NavMeshLocation start, NavMeshLocation end,
                  int areaMask = NavMesh.AllAreas,
                  NativeArray<float> costs = new NativeArray<float>()) {
      ...
      if (agentTypeEnd < 0)
        throw new ArgumentException(
            "The end location doesn't belong to any active NavMesh surface.",
            "end");
    ...
    public PathQueryStatus UpdateFindPath(int iterations,
                                          out int iterationsPerformed) {
      AtomicSafetyHandle.CheckWriteAndThrow(m_Safety);

      if (!HasNodePool(m_NavMeshQuery))
        throw new InvalidOperationException(k_NoBufferAllocatedErrorMessage);
      return UpdateFindPath(m_NavMeshQuery, iterations,
    ...
    [ThreadSafe]
    static extern unsafe PathQueryStatus BeginFindPath(IntPtr navMeshQuery,
    ...
    [ThreadSafe]
    static extern PathQueryStatus UpdateFindPath(IntPtr navMeshQuery,
                                                 int iterations,
                                                 out int iterationsPerformed);

    [ThreadSafe]
    static extern PathQueryStatus EndFindPath(IntPtr navMeshQuery,
                                              out int pathSize);

    [ThreadSafe]
    static extern unsafe int GetPathResult(IntPtr navMeshQuery, void *path,
                                           int maxPath);

    // If BeginFindPath/UpdateFindPath/EndFindPath existing NativeArray become
    // invalid...
    //      extern NavMeshPathStatus GetPath(out NativeArray<PolygonId>
    //      outputPath);
    ...
    [ThreadSafe]
    static extern int GetAgentTypeIdForPolygon(IntPtr navMeshQuery,
                                               PolygonId polygon);
    ...
    [ThreadSafe]
    static extern bool IsPositionInPolygon(IntPtr navMeshQuery,
                                           Vector3 position, PolygonId polygon);

    [ThreadSafe]
    static extern PathQueryStatus
    GetClosestPointOnPoly(IntPtr navMeshQuery, PolygonId polygon,
                          Vector3 position, out Vector3 nearest);

    public NavMeshLocation CreateLocation(Vector3 position, PolygonId polygon) {
      AtomicSafetyHandle.CheckReadAndThrow(m_Safety);
      Vector3 nearest;
      var status =
          GetClosestPointOnPoly(m_NavMeshQuery, polygon, position, out nearest);
      return (status & PathQueryStatus.Success) != 0
                 ? new NavMeshLocation(nearest, polygon)
                 : new NavMeshLocation();
    }

    [ThreadSafe]
    static extern NavMeshLocation
    MapLocation(IntPtr navMeshQuery, Vector3 position, Vector3 extents,
                int agentTypeID, int areaMask = NavMesh.AllAreas);
    public NavMeshLocation MapLocation(Vector3 position, Vector3 extents,
                                       int agentTypeID,
                                       int areaMask = NavMesh.AllAreas) {
      AtomicSafetyHandle.CheckReadAndThrow(m_Safety);
      return MapLocation(m_NavMeshQuery, position, extents, agentTypeID,
                         areaMask);
    }

    [ThreadSafe]
    static extern unsafe void MoveLocations(IntPtr navMeshQuery,
                                            void *locations, void *targets,
                                            void *areaMasks, int count);
    public unsafe void MoveLocations(NativeSlice<NavMeshLocation> locations,
                                     NativeSlice<Vector3> targets,
                                     NativeSlice<int> areaMasks) {
      if (locations.Length != targets.Length ||
          locations.Length != areaMasks.Length)
        throw new ArgumentException(
            "locations.Length, targets.Length and areaMasks.Length must be equal");

      AtomicSafetyHandle.CheckReadAndThrow(m_Safety);
      MoveLocations(m_NavMeshQuery, locations.GetUnsafePtr(),
                    targets.GetUnsafeReadOnlyPtr(),
                    areaMasks.GetUnsafeReadOnlyPtr(), locations.Length);
    }

    [ThreadSafe]
    static extern unsafe void
    MoveLocationsInSameAreas(IntPtr navMeshQuery, void *locations,
                             void *targets, int count, int areaMask);
    public unsafe void
    MoveLocationsInSameAreas(NativeSlice<NavMeshLocation> locations,
                             NativeSlice<Vector3> targets,
                             int areaMask = NavMesh.AllAreas) {
      if (locations.Length != targets.Length)
        throw new ArgumentException(
            "locations.Length and targets.Length must be equal");

      AtomicSafetyHandle.CheckReadAndThrow(m_Safety);
      MoveLocationsInSameAreas(m_NavMeshQuery, locations.GetUnsafePtr(),
                               targets.GetUnsafeReadOnlyPtr(), locations.Length,
                               areaMask);
    }

    [ThreadSafe]
    static extern NavMeshLocation MoveLocation(IntPtr navMeshQuery,
                                               NavMeshLocation location,
                                               Vector3 target, int areaMask);
    ...
    [ThreadSafe]
    static extern bool GetPortalPoints(IntPtr navMeshQuery, PolygonId polygon,
                                       PolygonId neighbourPolygon,
                                       out Vector3 left, out Vector3 right);
    public bool GetPortalPoints(PolygonId polygon, PolygonId neighbourPolygon,
                                out Vector3 left, out Vector3 right) {
      AtomicSafetyHandle.CheckReadAndThrow(m_Safety);
      return GetPortalPoints(m_NavMeshQuery, polygon, neighbourPolygon,
                             out left, out right);
    }

    [ThreadSafe]
    static extern Matrix4x4 PolygonLocalToWorldMatrix(IntPtr navMeshQuery,
                                                      PolygonId polygon);
    ...
    [ThreadSafe]
    static extern Matrix4x4 PolygonWorldToLocalMatrix(IntPtr navMeshQuery,
                                                      PolygonId polygon);
    ...
    [ThreadSafe]
    static extern NavMeshPolyTypes GetPolygonType(IntPtr navMeshQuery,
                                                  PolygonId polygon);
    ...
    // NavMeshStatus MoveAlongSurface(NavMeshLocation location, Vector3
    // targetPosition, int agentTypeID, int areaMask,
    //    out NavMeshLocation outputLocation, NativeArray<PolygonId>
    //    visitedBuffer, out int actualVisited);

    // Trace a ray between two points on the NavMesh.
    [ThreadSafe]
    static extern unsafe
        PathQueryStatus Raycast(IntPtr navMeshQuery, NavMeshLocation start,
                                Vector3 targetPosition, int areaMask,
                                void *costs, out NavMeshHit hit, void *path,
                                out int pathCount, int maxPath);

    public unsafe PathQueryStatus
    Raycast(out NavMeshHit hit, NavMeshLocation start, Vector3 targetPosition,
            int areaMask = NavMesh.AllAreas,
            NativeArray<float> costs = new NativeArray<float>()) {
      const int kAreaCount = 32;
      AtomicSafetyHandle.CheckReadAndThrow(m_Safety);
      ...
      var status = Raycast(m_NavMeshQuery, start, targetPosition, areaMask,
                           costsPtr, out hit, null, out pathCount, 0);
      status &= ~PathQueryStatus.BufferTooSmall;
      return status;
    }

    public unsafe PathQueryStatus
    Raycast(out NavMeshHit hit, NativeSlice<PolygonId> path, out int pathCount,
      ...
      var status = Raycast(m_NavMeshQuery, start, targetPosition, areaMask,
                           costsPtr, out hit, pathPtr, out pathCount, maxPath);
      return status;
    }

    [ThreadSafe]
    static extern unsafe PathQueryStatus GetEdgesAndNeighbors(
        IntPtr navMeshQuery, PolygonId node, int maxVerts, int maxNei,
        void *verts, void *neighbors, void *edgeIndices, out int vertCount,
        out int neighborsCount);

    public unsafe PathQueryStatus GetEdgesAndNeighbors(
        PolygonId node, NativeSlice<Vector3> edgeVertices,
        NativeSlice<PolygonId> neighbors, NativeSlice<byte> edgeIndices,
        out int verticesCount, out int neighborsCount) {
      AtomicSafetyHandle.CheckReadAndThrow(m_Safety);

      if (edgeIndices.Length != neighbors.Length && neighbors.Length > 0 &&
          edgeIndices.Length > 0) {
        throw new ArgumentException(
            $"The length of the {nameof(edgeIndices)} buffer ({edgeIndices.Length}) " +
      ...
      var vertPtr =
          edgeVertices.Length > 0 ? edgeVertices.GetUnsafePtr() : null;
      ...
      var maxNeighbors =
          neighbors.Length > 0 ? neighbors.Length : edgeIndices.Length;
      var status = GetEdgesAndNeighbors(m_NavMeshQuery, node, maxVertices,
                                        maxNeighbors, vertPtr, neiPtr, edgesPtr,
    ...
    // public void GetPolygon(NativeArray<PolygonId> polygonIDs,
    // NativeArray<NavMeshPolyData> polygons); public void
    // GetPolygons(MappedPosition position, float distance,
    // NativeList<NavMeshPolyData> polygons);

    // public static void LocalizePolygonIndices(NativeArray<NavMeshPolyData>
    // polygons);

    //// Segments
    // public NativeArray<NavMeshSegment> FindBoundarySegments (MappedPosition
    // position, float distance, Allocator allocator);

    //// Voxel rasterize
    // public void Rasterize (MappedPosition position, Quaternion orientation,
    // float cellWidth, float cellHeight, NativeArray2D<bool> grid);

    //// DetailMesh queries
    // void ProjectToDetailMesh(NativeArray<MappedPosition> positions,
    // NativeArray<Vector3> outputPositions);
  }
}
