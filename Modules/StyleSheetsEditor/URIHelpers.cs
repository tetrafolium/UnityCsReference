...
public static URIValidationResult
ValidAssetURL(string assetPath, string path, out string errorMessage,
	...
	} else if (Uri.TryCreate(path, UriKind.Absolute, out absoluteUri) ==
	           false) {
	...
	// Remove any leading "/" as this now used as a path relative to the
	// current directory
	...
	if (string.IsNullOrEmpty(projectRelativePath) ||
	    !File.Exists(projectRelativePath)) {
...
public static string InjectFileNameSuffix(string path, string suffix) {
	...
	string fileRenamed =
		Path.GetFileNameWithoutExtension(path) + suffix + ext;
