...
// Used by test
public StyleSheetImportErrors importErrors {
	get { return m_Errors; }
}
...
protected void VisitResourceFunction(GenericFunction funcTerm) {
	...
	if (argTerm == null) {
		m_Errors.AddSemanticError(
			StyleSheetImportErrorCode.MissingFunctionArgument, funcTerm.Name);
		return;
	}
...
static StyleSheetImportErrorCode
ConvertErrorCode(URIValidationResult result) {
	switch (result) {
...
protected void VisitUrlFunction(PrimitiveTerm term) {
	...
	URIValidationResult result = URIHelpers.ValidAssetURL(
		assetPath, path, out errorMessage, out projectRelativePath);

	if (result != URIValidationResult.OK) {
		m_Builder.AddValue(path, StyleValueType.MissingAssetReference);
		m_Errors.AddValidationWarning(errorMessage,
		                              m_Builder.currentProperty.line);
	} else {
		UnityEngine.Object asset =
			DeclareDependencyAndLoad(projectRelativePath);
		...
		if (isTexture || asset is Font || asset is VectorImage) {
			...
			if (isTexture) {
				string hiResImageLocation =
					URIHelpers.InjectFileNameSuffix(projectRelativePath, "@2x");

				if (File.Exists(hiResImageLocation)) {
					UnityEngine.Object hiResImage =
						DeclareDependencyAndLoad(hiResImageLocation);

					if (hiResImage is Texture2D) {
						m_Builder.AddValue(new ScalableImage(){
									normalImage = asset as Texture2D,
									highResolutionImage = hiResImage as Texture2D
								});
					} else {
						m_Errors.AddSemanticError(
							StyleSheetImportErrorCode.InvalidHighResolutionImage,
							string.Format(
								"Invalid asset type {0}, only Texture2D is supported for variants with @2x suffix",
								asset.GetType().Name));
					}
					return;
				}
				// If we didn't find an high res variant, tell ADB we depend on that
				// potential file existing
				DeclareDependencyAndLoad(hiResImageLocation);
			}
			m_Builder.AddValue(asset);
		} else {
			m_Errors.AddSemanticError(
				StyleSheetImportErrorCode.InvalidURIProjectAssetType,
				string.Format(
					"Invalid asset type {0}, only Font, Texture2D and VectorImage are supported",
					asset.GetType().Name));
		}
	}
}

private bool ValidateFunction(GenericFunction term,
                              out StyleValueFunction func) {
	func = StyleValueFunction.Unknown;
	if (term.Arguments.Length == 0) {
		m_Errors.AddSemanticError(
			StyleSheetImportErrorCode.MissingFunctionArgument, term.Name);
		return false;
	}
	...
	try {
		...
		m_Errors.AddValidationWarning(
			$"Unknown function {term.Name} in declaration {prop.name}: {term.Name}",
			prop.line);
		return false;
	}
	...
	for (int i = 0; i < argc; i++) {
		...
		if (!foundVar) {
			...
			if (string.IsNullOrEmpty(varName)) {
				...
				m_Errors.AddSemanticError(
					StyleSheetImportErrorCode.InvalidVarFunction,
					$"Variable {varName} is missing '--' prefix");
				return false;
			}
			if (varName.Length < 3) {
				m_Errors.AddSemanticError(
			...
			m_Errors.AddSemanticError(
				StyleSheetImportErrorCode.InvalidVarFunction, msg);
			return false;
		}
	}
...
protected void VisitValue(Term term) {
	...
	if (term == PrimitiveTerm.Inherit) {
		...
		case UnitType.Pixel:
			...
			m_Builder.AddValue(
				new Dimension(pixelValue.Value, Dimension.Unit.Pixel));
			break;
		case UnitType.Percentage:
			float? percentValue = primitiveTerm.GetFloatValue(UnitType.Pixel);
			m_Builder.AddValue(
				new Dimension(percentValue.Value, Dimension.Unit.Percent));
			break;
		case UnitType.Ident:
		...
		case UnitType.Uri:
			...
			break;
		default:
			m_Errors.AddSemanticError(StyleSheetImportErrorCode.UnsupportedUnit,
			                          primitiveTerm.ToString());
			return;
		}
	} else if (colorTerm != null) {
		var color = new Color(
			(float) colorTerm.R / 255.0f, (float) colorTerm.G / 255.0f,
			(float) colorTerm.B / 255.0f, (float) colorTerm.A / 255.0f);
		...
		m_Builder.AddValue(commaTerm.ToString(),
...
static bool TryParseKeyword(string rawStr, out StyleValueKeyword value) {
	if (s_NameCache == null) {
		...
		foreach (StyleValueKeyword kw in Enum.GetValues(
...
protected void ImportParserStyleSheet(UnityStyleSheet asset,
...
void VisitSheet(ParserStyleSheet styleSheet) {
	foreach (ParserStyleRule rule in styleSheet.StyleRules) {
		...
		foreach (Property property in rule.Declarations) {
			...
			// Note: we must rely on recursion to correctly handle parser types
			// here
...
void VisitComplexSelector(ComplexSelector complexSelector) {
	int fullSpecificity =
		CSSSpec.GetSelectorSpecificity(complexSelector.ToString());

	if (fullSpecificity == 0) {
		m_Errors.AddInternalError("Failed to calculate selector specificity " +
		                          complexSelector);
		return;
	}
		...
		foreach (CombinatorSelector selector in complexSelector) {
			...
			if (string.IsNullOrEmpty(simpleSelector)) {
				m_Errors.AddInternalError(
					"Expected simple selector inside complex selector " +
					simpleSelector);
				return;
			}
...
bool CheckSimpleSelector(string selector, out StyleSelectorPart[] parts) {
	if (!CSSSpec.ParseSelector(selector, out parts)) {
		m_Errors.AddSemanticError(
			StyleSheetImportErrorCode.UnsupportedSelectorFormat, selector);
		return false;
	}
	...
	if (parts.Any(p => p.type == StyleSelectorType.RecursivePseudoClass)) {
		m_Errors.AddSemanticError(
			StyleSheetImportErrorCode.RecursiveSelectorDetected, selector);
		return false;
	}
	return true;
}
