...
public static int GetSelectorSpecificity(StyleSelectorPart[] parts) {
	// always add 1 otherwise we wouldn't be able to distinguish between
	// default C# value for int (0) and an actual specificity (0 for *)
	int score = 1;
	for (int i = 0; i < parts.Length; i++) {
		...
		case StyleSelectorType.RecursivePseudoClass:
			throw new ArgumentException(
				      "Recursive pseudo classes are not supported");
...
public static bool ParseSelector(string selector,
                                 out StyleSelectorPart[] parts) {
