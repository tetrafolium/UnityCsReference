...
private static readonly int k_EnableHovering =
	"-unity-enable-hovering".GetHashCode();

static Dictionary<StyleState, StyleState[]> s_StatesCache =
	new Dictionary<StyleState, StyleState[]>();

internal static bool DrawStyle(GUIStyle gs, Rect position,
                               GUIContent content, DrawStates states) {
	if (Highlighter.IsSearchingForIdentifier())
		Highlighter.HighlightIdentifier(position, content?.text);

	if (gs == GUIStyle.none || gs.blockId == -1 ||
	    String.IsNullOrEmpty(gs.name) || gs.normal.background != null)
		return false;
	...
	if (gs.blockId == 0)
		gs.blockId = GUIStyleExtensions.StyleNameToBlockName(gs.name, false)
		             .GetHashCode();
...
internal static StyleBlock FindBlock(int name, DrawStates drawStates) {
	...
	if (GUI.enabled) {
		...
		if (drawStates.isHover && GUIUtility.hotControl == 0 &&
		    IsMouseOverGUIView())
	...
	if (!s_StatesCache.TryGetValue(stateFlags, out states)) {
		states = new[] {
			stateFlags, stateFlags &StyleState.disabled,
			stateFlags &StyleState.active, stateFlags &StyleState.@checked,
			stateFlags &StyleState.hover, stateFlags &StyleState.focus,
			StyleState.normal
		}
		.Distinct().Where(s => s != StyleState.none).ToArray();
...
private static readonly Dictionary<long, Texture2D> s_Gradients =
	new Dictionary<long, Texture2D>();
private static Texture2D GenerateGradient(StyleFunctionCall call,
                                          Rect rect) {
	...
	int key = call.blockKey ^ call.valueKey ^
	          ((int) rect.width * 30 + (int) rect.height * 8);
	...
	for (int i = 0; i < valueCount; ++i) {
		...
		if (values.Length == 1 && values[0].type == StyleValue.Type.Color) {
			...
			colorKeys[i].time = autoStep;
		           ...
		           values[0].type == StyleValue.Type.Color &&
		           values[1].type == StyleValue.Type.Number) {
			colorKeys[i].color = call.GetColor(valueIndex, 0);
	...
	for (int y = height - 1; y >= 0; --y) {
		for (int x = width - 1; x >= 0; --x) {
			...
			var recenteredPos =
				GetGradientCenterFrame(pixelPosition, width, height);
...
private static Matrix4x4 GetGradientScaleMatrix(Matrix4x4 matrix,
                                                Vector2 scaleVec) {
...
private static Vector2 GetGradientScaleVector(Matrix4x4 matrix, float deg,
...
private static Vector2 GetGradientCenterFrame(Vector2 pos, float width,
                                              float height) {
...
internal static void DrawBlock(GUIStyle basis, StyleBlock block,
                               Rect drawRect, GUIContent content,
                               DrawStates states) {
	...
	drawRect.width =
		basis.fixedWidth == 0f ? drawRect.width : basis.fixedWidth;
	drawRect.height =
		basis.fixedHeight == 0f ? drawRect.height : basis.fixedHeight;
	...
	if (!block.Execute(
		    StyleCatalogKeyword.background, DrawGradient,
		    new GradientParams(drawRect, border.radius, bgColorTint))) {
		// Draw background color
		var backgroundColor =
			block.GetColor(StyleCatalogKeyword.backgroundColor);
		if (backgroundColor.a > 0f) {
			var smoothCorners = !border.all;
			GUI.DrawTexture(drawRect, EditorGUIUtility.whiteTexture,
			                ScaleMode.StretchToFill, false, 0f,
			                backgroundColor * bgColorTint, Vector4.zero,
			                border.radius, smoothCorners);
		}
	}
	...
	if (content != null) {
		...
		if (block.GetKeyword(StyleCatalogKeyword.padding) ==
		...
		float opacity =
			hasImage ? block.GetFloat(StyleCatalogKeyword.opacity, 1f) : 1f;
		float contentImageOffsetX = hasImage ? block.GetFloat(
			StyleCatalogKeyword.contentImageOffsetX) : 0;
		float contentImageOffsetY = hasImage ? block.GetFloat(
			StyleCatalogKeyword.contentImageOffsetY) : 0;
		basis.Internal_DrawContent(
			contentRect, content, states.isHover, states.isActive, states.on,
			states.hasKeyboardFocus, states.hasTextInput,
			states.drawSelectionAsComposition, states.cursorFirst,
			states.cursorLast, states.cursorColor, states.selectionColor,
			guiContentColor * opacity, 0, 0, contentImageOffsetY,
			contentImageOffsetX, false, false);
	...
	if (border.any) {
		GUI.DrawTexture(
			drawRect, EditorGUIUtility.whiteTexture, ScaleMode.StretchToFill,
			true, 0f, border.borderLeftColor * colorTint,
			border.borderTopColor * colorTint,
			border.borderRightColor * colorTint,
			border.borderBottomColor * colorTint, border.widths, border.radius);
	}
...
// Note: Assign lambda to local variable to avoid the allocation caused by
// method group.
private static readonly Func<StyleFunctionCall, GradientParams, bool>
DrawGradient = (call, gp) => {
	...
	if (gradientTexture == null)
		return false;
	GUI.DrawTexture(gp.rect, gradientTexture, ScaleMode.ScaleAndCrop,
	                true, 0f, gp.colorTint, Vector4.zero, gp.radius);
	return true;
};

private static void DrawBackgroundImage(StyleBlock block, Rect position,
                                        Color colorTint) {
	var backgroundImage =
		block.GetTexture(StyleCatalogKeyword.backgroundImage, true);
	if (backgroundImage == null)
		return;

	var backgroundPosition = block.GetStruct<StyleBackgroundPosition>(
		StyleCatalogKeyword.backgroundPosition);
	var backgroundSize = block.GetRect(
		StyleCatalogKeyword.backgroundSize,
		StyleRect.Size(backgroundImage.width, backgroundImage.height));
	...
	var bgRect = new Rect(position.xMin, position.yMin, backgroundSize.width,
	                      backgroundSize.height);
	if (anchor.left < 1.0f)
		bgRect.xMin = position.xMin + position.width * anchor.left -
		              backgroundSize.width / 2f;
	else if (anchor.left >= 1.0f)
		bgRect.xMin = position.xMin + anchor.left;

	if (anchor.top < 1.0f)
		bgRect.yMin = position.yMin + position.height * anchor.top -
...
private static void GetContentCenteredRect(GUIStyle gs, GUIContent content,
                                           ref Rect contentRect) {
	var initialRect = contentRect;
	var contentSize = gs.CalcSize(content);
	contentRect.xMin = Mathf.Max(
		initialRect.xMin,
		initialRect.xMin + (initialRect.width - contentSize.x) / 2f);
	contentRect.xMax =
		Mathf.Min(initialRect.xMax, contentRect.xMin + contentSize.x);
	contentRect.yMin = Mathf.Max(
		initialRect.yMin,
		initialRect.yMin + (initialRect.height - contentSize.y) / 2f);
	contentRect.yMax =
		Mathf.Min(initialRect.yMax, contentRect.yMin + contentSize.y);
}
...
private readonly struct StyleBorder {
	// borderWidths The width of the borders(left, top, right and bottom). If
	// Vector4.zero, the full texture is drawn. borderRadiuses The radiuses
	// for rounded corners (top-left, top-right, bottom-right and
	// bottom-left). If Vector4.zero, corners will not be rounded.
	public StyleBorder(StyleBlock block) {
		if (block.HasValue(StyleCatalogKeyword.border)) {
			...
			widths = new Vector4(borderWidths.left, borderWidths.top,
			                     borderWidths.right, borderWidths.bottom);
			borderLeftColor =
				block.GetColor(StyleCatalogKeyword.borderLeftColor, defaultColor);
			borderTopColor =
				block.GetColor(StyleCatalogKeyword.borderTopColor, defaultColor);
			borderRightColor = block.GetColor(
				StyleCatalogKeyword.borderRightColor, defaultColor);
			borderBottomColor = block.GetColor(
				StyleCatalogKeyword.borderBottomColor, defaultColor);
		} else {
			widths = Vector4.zero;
			borderLeftColor = borderTopColor = borderRightColor =
				borderBottomColor = Color.cyan;
		}

		var borderRadius = block.GetRect(StyleCatalogKeyword.borderRadius);
		radius = new Vector4(borderRadius.top, borderRadius.right,
