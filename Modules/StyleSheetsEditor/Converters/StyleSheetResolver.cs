...
namespace UnityEditor.StyleSheets {
  internal class StyleSheetResolver {
    internal class ResolvingOptions {
      public bool ThrowIfCannotResolve {
        get;
        set;
      }
    ...
    internal class Value {
      ...
      public StyleValueKeyword AsKeyword() { return (StyleValueKeyword) Obj; }

      public bool AsBool() { return AsKeyword() == StyleValueKeyword.True; }

      public Color AsColor() { return (Color) Obj; }
      ...
      public Dimension AsDimension() { return (Dimension) Obj; }

      public string AsString() { return (string) Obj; }

      public UnityEngine.Object AsAssetReference() {
        return (UnityEngine.Object) Obj;
      }

      public ScalableImage AsScalableImage() { return (ScalableImage) Obj; }

      public override string ToString() { return Obj.ToString(); }

      public bool IsFloat() {
        return ValueType == StyleValueType.Float ||
               ValueType == StyleValueType.Dimension;
      }

      public bool IsString() { return ValueType == StyleValueType.String; }
      ...
      public Value Clone() { return new Value(ValueType, Obj); }
    }

    internal class Function : Value {
      public Function(string functionName)
          : base(StyleValueType.Function, functionName) {
        args = new List<Value[]>();
      }
    ...
    internal class Property {
      ...
      public Property Clone() {
        return new Property(Name){Values =
                                      Values.Select(v => v.Clone()).ToList()};
      }
    }
    ...
    internal class Rule {
      ...
      public List<string> PseudoStateRules {
        get;
        private set;
      }
    ...
    internal class ExtendData {
      ...
      public string ParentSelectorName {
        get;
        set;
      }
      public List<Rule> ChildrenRules {
        get;
        set;
      }
      ...
      public void ResolveParent(StyleSheetResolver resolver) {
        Rule parentRule;
        if (!resolver.Rules.TryGetValue(ParentSelectorName, out parentRule) &&
            resolver.Options.ThrowIfCannotResolve) {
    ...
    public Dictionary<string, ExtendData> ParentToChildren {
      get;
      private set;
    }
    public Dictionary<string, string> ChildToParent {
      get;
      private set;
    }
    public Dictionary<string, Property> Variables {
      get;
      private set;
    }
    public ResolvingOptions Options {
      get;
      private set;
    }
    ...
    public List<Rule> CleanRedundantProperties() {
      ...
      // Ensure we process derivations reverse topological order so we begin by
      // trimming the "grand children" before anything
      ...
      foreach (var derivationData in derivations) {
        foreach (var mainChildrenRule in derivationData.ChildrenRules) {
          var childrenExtendedRules =
              mainChildrenRule.ResolveExtendedRules(this);
          foreach (var extRule in childrenExtendedRules) {
            // Create the parent chain in which we will be looking for
            ...
            foreach (var childProperty in extRule.Properties.Values.ToList()) {
              // Navigate all parent rule (extended or not) to see if we are
              // overriding an existing values:
              foreach (var parentRule in parentChain) {
                Property parentProperty;
                if (parentRule.Properties.TryGetValue(childProperty.Name,
                                                      out parentProperty)) {
                  if (parentProperty.CompareTo(childProperty)) {
                    ...
                    // This is a property that is overridden in the children.
                    // Stop the parentChain:
                    break;
                  }
                }
                // else: property not in parent, continue to see if it exists
                // further in parent chain.
              }
            }
          }
        }
    ...
    public void ResolveExtend() {
      ResolveExtendData();
      // Ensure we process derivations in right order (topologic) so no
      // lingering extend stays:
      var derivations = TopologicalSort(ParentToChildren.Values.ToList());

      // Apply Derivations:
      foreach (var derivation in derivations) {
        if (derivation.ParentRule == null) {
          Debug.LogWarning("Cannot resolve parent rule: " +
                           derivation.ParentSelectorName);
          continue;
        }
        ...
        foreach (var childrenRule in derivation.ChildrenRules) {
          foreach (var replacementRule in extendedRules) {
            var childrenReplacementName = replacementRule.SelectorName.Replace(
                derivation.ParentSelectorName, childrenRule.SelectorName);
            if (childrenRule.SelectorName == childrenReplacementName) {
              ...
              if (!Rules.TryGetValue(childrenReplacementName,
                                     out pseudoChildrenRule)) {
                var childrenSelector = ConverterUtils.CreateSelectorFromSource(
                    replacementRule.Selector, childrenRule.SelectorName);
                pseudoChildrenRule =
                    new Rule(childrenReplacementName, childrenSelector, -1);
    ...
    class VariableDependencyNode {
      ...
      public VariableDependencyNode(Property variable) { Variable = variable; }

      public void Tranverse(Action<Property, int> action) {
        Tranverse(action, this, 0);
      }

      public static void Tranverse(Action<Property, int> action,
                                   VariableDependencyNode node, int depth) {
    ...
    static VariableDependencyNode BuildVariableDependencies(
        ...
        ref Dictionary<string, VariableDependencyNode> dependencies) {
          ...
          if (IsVariableValue(value)) {
            ...
            if (value.ValueType == StyleValueType.Function) {
              ...
              if (functionValue != null) {
                variables.TryGetValue(functionValue
                                          .args [0]
                                          [0]
                                          .AsString(),
                                      out variableProperty);
              }
            } else {
            ...
            var referredVarNode = BuildVariableDependencies(
                variableProperty, variables, ref dependencies);

            if (varNode.Parent ==
                null) // in case the the variable is referred multiple times.
            {
    ...
    static List<VariableDependencyNode>
    BuildVariableDependencies(Dictionary<string, Property> variables) {
      var topLevelDepNodes = new List<VariableDependencyNode>();
      var variableDependencies =
          new Dictionary<string, VariableDependencyNode>();

      foreach (var var in variables) {
        try {
          var node = BuildVariableDependencies(var.Value, variables,
                                               ref variableDependencies);
    ...
    public static void ResolveVariables(IEnumerable<Rule> rules,
                                        Dictionary<string, Property> variables,
                                        ResolvingOptions options) {
      var graph = BuildVariableDependencies(variables);

      foreach (var varDepNode in graph) {
        varDepNode.Tranverse((var, depth) =>
                                 ResolveValues(var, variables, options));
      }
    ...
    public static void PopulateSheet(StyleSheet sheet, IEnumerable<Rule> rules,
                                     ResolvingOptions options = null) {
      options = options ?? new ResolvingOptions();
    ...
    ConvertToStyleSheet(IEnumerable<Rule> rules,
                        ResolvingOptions options = null) {
    ...
    public static bool IsVariableValue(StyleSheetResolver.Value value) {
      return (value.ValueType == StyleValueType.Variable ||
              (value.ValueType == StyleValueType.Function &&
               value.AsString() == StyleValueFunctionExtension.k_Env));
    }

    public static List<Value>
    ResolveValues(Property property, Dictionary<string, Property> variables,
                  ResolvingOptions options) {
      for (var i = 0; i < property.Values.Count; ++i) {
        ...
        if (value.ValueType == StyleValueType.Function) {
          var functionValue = value as Function;
          if (variables.TryGetValue(functionValue
                                        .args [0]
                                        [0]
                                        .AsString(),
                                    out variableProperty)) {
            ...
            throw new Exception(
                $"Cannot resolve variable: \"{functionValue.args[0][0].AsString()}\"");
          }
        } else {
        ...
        if (variableValues != null) {
          ...
          throw new Exception("Cannot resolve variable: " + property
                                                                .Values [0]
                                                                .AsString());
        }
      }
    ...
    private List<Rule> GetParentChain(Rule childRule) {
      var isPseudoState =
          ConverterUtils.IsPseudoSelector(childRule.SelectorName);
      var childMainRuleName =
          ConverterUtils.GetNoPseudoSelector(childRule.SelectorName);
      ...
      while (parentRule != null) {
        if (isPseudoState) {
          ...
          var extendedParentRuleName = childRule.SelectorName.Replace(
              childMainRuleName, parentRule.SelectorName);
          Rule extentedParentRule;
          if (Rules.TryGetValue(extendedParentRuleName,
                                out extentedParentRule)) {
    ...
    private void RecordIfPseudoStateRule(Rule rule) {
      if (ConverterUtils.IsPseudoSelector(rule.SelectorName)) {
        var mainRuleName =
            ConverterUtils.GetNoPseudoSelector(rule.SelectorName);
        if (mainRuleName == "")
          return;
        Rule mainRule;
    ...
    private void ResolveRules(StyleSheet sheet) {
      foreach (var complexSelector in sheet.complexSelectors) {
        ...
        foreach (var property in complexSelector.rule.properties) {
          Property dstProp;
          if (aggregateRule.Properties.TryGetValue(property.name,
                                                   out dstProp)) {
        ...
        if (aggregateRule.Properties.TryGetValue(ConverterUtils.k_Extend,
                                                 out derivationProperty)) {
          var parentName = derivationProperty
                               .Values [0]
                               .AsString();
    ...
    private static List<Value> ToValues(StyleProperty srcProp,
                                        StyleSheet srcSheet) {
      return ToValues(srcProp.values, 0, srcProp.values.Length, srcSheet);
    }

    private static List<Value> ToValues(StyleValueHandle[] sourceValues,
                                        int from, int to, StyleSheet srcSheet) {
      ...
      for (int i = from; i < to; ++i) {
        ...
        if (valueHandle.valueType == StyleValueType.Function) {
          parsedValues.Add(
              ParseFunction(sourceValues, srcSheet, valueHandle, ref i));
        } else
          parsedValues.Add(new Value(valueHandle.valueType,
                                     GetPropertyValue(valueHandle, srcSheet)));
      }

      return parsedValues;
    }

    private static Function ParseFunction(StyleValueHandle[] sourceValues,
                                          StyleSheet srcSheet,
                                          StyleValueHandle valueHandle,
                                          ref int handleIndex) {
      ...
      for (int a = 0; a < funcArgCount; ++a) {
        ...
        if (argHandle.valueType == StyleValueType.Function) {
          argValues.Add(ParseFunction(sourceValues, srcSheet, argHandle,
                                      ref handleIndex));
        } else if (argHandle.valueType == StyleValueType.FunctionSeparator) {
          func.args.Add(argValues.ToArray());
          argValues.Clear();
        } else
          argValues.Add(new Value(argHandle.valueType,
                                  GetPropertyValue(argHandle, srcSheet)));
      }
    ...
    private static Property AddProperty(Rule aggregate, StyleProperty property,
                                        StyleSheet srcSheet) {
      var dstProperty =
          new Property(property.name){Values = ToValues(property, srcSheet)};
    ...
    private static object GetPropertyValue(StyleValueHandle valueHandle,
                                           StyleSheet srcSheet) {
      object value = null;
    ...
    private static void AddValues(StyleSheetBuilderHelper helper,
      ...
      foreach (var value in values) {
        ...
        case StyleValueType.Function:
          ...
          var functionValue = value as Function;
          helper.builder.AddValue(StyleValueFunctionExtension.FromUssString(
              functionValue.AsString()));
          var nbArgs = functionValue.args.Count - 1;
          for (var argIndex = 0; argIndex < functionValue.args.Count;
               ++argIndex) {
          ...
          for (var argIndex = 0; argIndex < functionValue.args.Count;
            ...
            if (argIndex < functionValue.args.Count - 1) {
              helper.builder.AddValue(value.AsString(),
                                      StyleValueType.FunctionSeparator);
            }
          }
          break;
        case StyleValueType.ScalableImage:
    ...
    private static List<ExtendData>
    TopologicalSort(List<ExtendData> derivations) {
      var graph = new Graph(derivations.Count);
      for (var i = 0; i < derivations.Count; ++i) {
        var derivation = derivations[i];
        foreach (var childrenRule in derivation.ChildrenRules) {
          var childIndex = derivations.FindIndex(
              d => d.ParentSelectorName == childrenRule.SelectorName);
          if (childIndex != -1) {
      ...
      var sortedDerivations =
          sortedEdges.Select(index => derivations[index]).ToList();
    ...
    private void Extend(Rule parentRule, Rule childrenRule) {
      foreach (var parentProperty in parentRule.Properties.Values) {
        if (Options.ThrowIfCannotResolve &&
            parentProperty.Name == ConverterUtils.k_Extend) {
          throw new Exception("Rule derivation not resolved: " +
                              parentRule.SelectorName);
          // Debug.Log("Rule derivation not resolved: " +
          // parentRule.SelectorName);
        }
