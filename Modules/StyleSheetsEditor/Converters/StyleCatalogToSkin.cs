...
internal static GUISkin CreateGUISkinFromSheetsFolder(
	SkinTarget target, IEnumerable<string> folders, GUISkin skin = null) {
	skin = skin ?? ConverterUtils.CreateDefaultGUISkin();
	PopulateSkin(target, folders, skin);
	return skin;
}

internal static void PopulateSkin(StyleSheetResolver resolver,
                                  GUISkin skin) {
	...
	// CustomStyles
	var customStyleBlockNames =
		resolver.Rules.Select(r => r.Key)
	...
	if (skin.customStyles != null) {
		...
		foreach (var customStyle in skin.customStyles) {
			...
			if (customStyle != null) {
				var blockName =
					GUIStyleExtensions.StyleNameToBlockName(customStyle.name);
	...
	foreach (var customStyleBlockName in customStyleBlockNames) {
		...
		// Check if we are overriding an existing style or if we are creating a
		// new custom style:
		if (!blockNameToStyleDict.TryGetValue(customStyleBlockName,
		                                      out customStyle)) {
	...
	if (settingsBlock.IsValid()) {
		skin.settings.selectionColor = settingsBlock.GetColor(
			ConverterUtils.k_SelectionColor, skin.settings.selectionColor);
		skin.settings.cursorColor = settingsBlock.GetColor(
			ConverterUtils.k_CursorColor, skin.settings.cursorColor);
		skin.settings.cursorFlashSpeed = settingsBlock.GetFloat(
			ConverterUtils.k_CursorFlashSpeed, skin.settings.cursorFlashSpeed);
		skin.settings.doubleClickSelectsWord =
			settingsBlock.GetBool(ConverterUtils.k_DoubleClickSelectsWord,
			                      skin.settings.doubleClickSelectsWord);
		skin.settings.tripleClickSelectsLine =
			settingsBlock.GetBool(ConverterUtils.k_TripleClickSelectsLine,
			                      skin.settings.tripleClickSelectsLine);
	}
}

internal static void
PopulateSkin(SkinTarget target, IEnumerable<string> folders, GUISkin skin) {
	var resolver = ConverterUtils.ResolveFromSheetsFolder(folders, target);
	resolver.Resolve();
	PopulateSkin(resolver, skin);
}

internal static void PopulateStyle(StyleCatalog catalog,
                                   string styleBlockName, GUIStyle style) {
