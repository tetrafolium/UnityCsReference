...
namespace UnityEditor.StyleSheets {
  class StyleSheetBuilderHelper {
    ...
    public UssExportOptions options {
      get;
      private set;
    }
    ...
    public void EndRule() { builder.EndRule(); }
    ...
    public void AddPropertyResource(string name, string value,
                                    string comment = "") {
      var property = builder.BeginProperty(name);
      options.AddComment(property, comment);
    ...
    public void AddProperty(string name, StyleValueKeyword value,
                            string comment = "") {
      var property = builder.BeginProperty(name);
      options.AddComment(property, comment);
      builder.AddValue(value);
...
#region StyleSheetManipulation
    public static void CopyProperty(StyleSheet sheet, UssComments comments,
                                    StyleProperty property,
                                    StyleSheetBuilderHelper helper) {
      ...
      foreach (var value in property.values) {
        ...
        case StyleValueType.ResourcePath:
          helper.builder.AddValue(sheet.ReadResourcePath(value),
                                  StyleValueType.ResourcePath);
          break;
        case StyleValueType.String:
          helper.builder.AddValue(sheet.ReadString(value),
    ...
    public static void
    PopulateProperties(IEnumerable<StyleProperty> properties,
                       Dictionary<string, PropertyPair> pairs, bool item1) {
    ...
    public static void BuildSelector(StyleComplexSelector complexSelector,
                                     StyleSheetBuilderHelper helper) {
      using(helper.builder.BeginComplexSelector(complexSelector.specificity)) {
        foreach (var selector in complexSelector.selectors) {
          helper.builder.AddSimpleSelector(selector.parts,
                                           selector.previousRelationship);
        }
      }
    }

    public static void CopySelector(StyleSheet sheet, UssComments comments,
