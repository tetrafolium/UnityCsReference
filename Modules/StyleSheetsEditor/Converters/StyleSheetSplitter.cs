...
#region ConversionAPI
    public SplitSheetData Split(StyleSheet s1, StyleSheet s2,
                                UssComments s1SrcComments = null,
                                UssComments s2SrcComments = null) {
      ...
      foreach (var selectorStr in allSelectors) {
        ...
        if (complexSelector1 != null) {
          if (complexSelector2 != null) {
            ...
            StyleSheetBuilderHelper.CopySelector(s1, s1Comments,
                                                 complexSelector1, s1Builder);
          }
        } else {
          // Rules only existing in S2: copy it straight
          StyleSheetBuilderHelper.CopySelector(s2, s2Comments, complexSelector2,
                                               s2Builder);
        }
      }
      ...
      var result =
          new SplitSheetData{common = commonBuilder.sheet, s1 = s1Builder.sheet,
                             s2 = s2Builder.sheet};
      return result;
    }
...
#region Implementation
    ...
    private void Split(StyleComplexSelector complexSelector1,
                       StyleComplexSelector complexSelector2) {
      ...
      commonBuilder.BeginRule(string.IsNullOrEmpty(comment1) ? comment2
                                                             : comment1);
      StyleSheetBuilderHelper.BuildSelector(complexSelector2, commonBuilder);

      // This is a common selector to both s1 and s2, for each properties
      // determine what is common:
      var properties = new Dictionary<string, PropertyPair>();
      StyleSheetBuilderHelper.PopulateProperties(
          complexSelector1.rule.properties, properties, true);
      StyleSheetBuilderHelper.PopulateProperties(
          complexSelector2.rule.properties, properties, false);
        ...
        if (propertyPair.p1 != null) {
          if (propertyPair.p2 != null) {
            ...
            if (propertyPair.p1.name == ConverterUtils.k_Extend) {
              StyleSheetBuilderHelper.CopyProperty(
                  s1Cache.sheet, s1Comments, propertyPair.p1, commonBuilder);
              StyleSheetBuilderHelper.CopyProperty(s1Cache.sheet, s1Comments,
                                                   propertyPair.p1, s1Builder);
              StyleSheetBuilderHelper.CopyProperty(s2Cache.sheet, s2Comments,
                                                   propertyPair.p2, s2Builder);
            }
