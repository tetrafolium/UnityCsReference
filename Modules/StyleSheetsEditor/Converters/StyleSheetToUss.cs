...
[assembly:InternalsVisibleTo("Assembly-CSharp-Editor-testable")]
  ...
  internal class UssComments {
    public Dictionary<StyleRule, string> ruleComments {
      get;
      private set;
    }
    public Dictionary<StyleProperty, string> propertyComments {
      get;
      private set;
    }
    public UssComments() {
  ...
  internal class UssExportOptions {
    ...
    public UssExportOptions(UssExportOptions opts) : base() {
  ...
  internal class StyleSheetToUss {
    static void AddComment(StringBuilder sb, string comment,
                           string indent = "") {
      sb.Append(indent);
    ...
    static int ColorComponent(float component) {
      return (int) Math.Round(component * byte.MaxValue, 0,
                              MidpointRounding.AwayFromZero);
    }

    public static string ToUssString(UnityEngine.Color color,
                                     bool useColorCode = false) {
      string str;
      string alpha =
          color.a.ToString("0.##", CultureInfo.InvariantCulture.NumberFormat);
      if (alpha != "1") {
        str = UnityString.Format(
            "rgba({0}, {1}, {2}, {3:F2})", ColorComponent(color.r),
            ColorComponent(color.g), ColorComponent(color.b), alpha);
      } else if (!useColorCode) {
        str = UnityString.Format("rgb({0}, {1}, {2})", ColorComponent(color.r),
    ...
    public static string ToUssString(StyleSheet sheet, UssExportOptions options,
    ...
    public static void ToUssString(StringBuilder sb, StyleSheet sheet,
                                   UssExportOptions options,
                                   StyleValueHandle[] values,
                                   ref int valueIndex, int valueCount = -1) {
      for (; valueIndex < values.Length && valueCount != 0; --valueCount) {
        ...
        if (valueIndex < values.Length &&
            values[valueIndex].valueType != StyleValueType.FunctionSeparator &&
            valueCount != 1) {
          sb.Append(" ");
        }
      }
    ...
    public static void ToUssString(StyleSheet sheet, UssExportOptions options,
                                   StyleRule rule, StringBuilder sb) {
    ...
    public static void
    ToUssString(StyleSelectorRelationship previousRelationship,
                StyleSelectorPart[] parts, StringBuilder sb) {
    ...
    public static void ToUssString(StyleSheet sheet, UssExportOptions options,
                                   ...
                                   StringBuilder sb) {
      options.comments.TryGet(complexSelector.rule,
                              comment => AddComment(sb, comment));
      foreach (var selector in complexSelector.selectors) {
        ToUssString(selector.previousRelationship, selector.parts, sb);
      }
    ...
    public static string ToUssString(StyleSheet sheet,
      ...
      if (sheet.complexSelectors != null) {
        for (var complexSelectorIndex = 0;
             complexSelectorIndex < sheet.complexSelectors.Length;
