...
public static implicit operator T(SVC<T> sc) {
	return sc.value;
}
...
internal struct StyleRect : IEquatable<StyleRect> {
	public static readonly StyleRect Nil =
		new StyleRect {
		left = float.NaN, right = float.NaN, top = float.NaN,
		bottom = float.NaN
	};
	...
	public static StyleRect Size(float width, float height) {
		return new StyleRect {
			       width = width, height = height, bottom = width,
			       left = height
		};
	}
...
internal struct StyleValue : IEquatable<StyleValue> {
	public static StyleValue Undefined(int key,
		...
		return new StyleValue {
			       key = key, state = state, type = Type.Undefined,
			       index = 0
		};
	}
...
ExpandRect(StyleState[] states, List<StyleValue> values,
	...
	for (int stateIndex = 0; stateIndex < states.Length; ++stateIndex) {
		...
		if (rect.left != float.MaxValue || rect.right != float.MaxValue ||
			...
			rectValues.Add(new StyleValue {
						key = rectKey, state = currentState,
		...
		if (rect.right != float.MaxValue)
			rectValues.Add(new StyleValue {
						key = rightKey, state = currentState,
						type = StyleValue.Type.Number,
		...
		if (rect.bottom != float.MaxValue)
			rectValues.Add(
				new StyleValue {
						key = bottomKey, state = currentState,
						type = StyleValue.Type.Number,
		...
		if (rect.left != float.MaxValue)
			rectValues.Add(new StyleValue {
						key = leftKey, state = currentState,
						type = StyleValue.Type.Number,
...
private StyleValue CompileValueGroup(
	...
	return new StyleValue {
		       key = propertyKey, state = stateFlags,
		       type = StyleValue.Type.Group,
		       index = SetIndex(groups, vg)
	};
}
