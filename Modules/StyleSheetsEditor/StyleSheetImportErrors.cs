...
namespace UnityEditor.StyleSheets {
  ...
  struct StyleSheetImportError {
    ...
    public StyleSheetImportError(StyleSheetImportErrorType error,
                                 StyleSheetImportErrorCode code,
                                 string assetPath, string message,
                                 int line = -1, bool isWarning = false) {
      this.error = error;
      this.code = code;
    ...
    public override string ToString() {
      ...
      return UnityString.Format(
          "StyleSheet import: type={0}, code={1} file={2}{3}\n    {4}", error,
          code, assetPath, lineStr, message);
    }
  }
  ...
  class StyleSheetImportErrors : IEnumerable<StyleSheetImportError> {
    ...
    public void AddSyntaxError(string message) {
      m_Errors.Add(new StyleSheetImportError(StyleSheetImportErrorType.Syntax,
                                             StyleSheetImportErrorCode.None,
                                             assetPath, message));
    }

    public void AddSemanticError(StyleSheetImportErrorCode code,
                                 string message) {
      m_Errors.Add(new StyleSheetImportError(StyleSheetImportErrorType.Semantic,
                                             code, assetPath, message));
    }

    public void AddInternalError(string message) {
      m_Errors.Add(new StyleSheetImportError(StyleSheetImportErrorType.Internal,
                                             StyleSheetImportErrorCode.None,
                                             assetPath, message));
    }
    ...
    public void AddValidationWarning(string message, int line) {
      m_Errors.Add(
          new StyleSheetImportError(StyleSheetImportErrorType.Validation,
                                    StyleSheetImportErrorCode.InvalidProperty,
                                    assetPath, message, line, true));
    }
    ...
    IEnumerator IEnumerable.GetEnumerator() { return m_Errors.GetEnumerator(); }

    public bool hasErrors {
      get { return m_Errors.Any(e => !e.isWarning); }
    }
