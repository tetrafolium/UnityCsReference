...
public class PaintContext
{
    ...
    public Terrain originTerrain {
        get;    // the terrain that defines the coordinate system and world space position of this PaintContext
    }
    public RectInt pixelRect {
        get;    // the rectangle, in target texture pixels on the originTerrain, that this paint context represents
    }
    ...
    public RenderTexture oldRenderTexture {
        get;    // active render texture at the time CreateRenderTargets() is called, restored on Cleanup()
        private set;
    }

    public int terrainCount {
        get {
            return m_TerrainTiles.Count;
        }
    }
    ...
    public interface ITerrainInfo
    {
        ...
        RectInt clippedTerrainPixels    {
            get;    // the region modified by the PaintContext, in target texture pixels
        }
        RectInt clippedPCPixels         {
            get;    // the region modified by the PaintContext, in PaintContext.sourceRenderTexture or destinationRenderTexture pixels
        }
        bool gatherEnable               {
            get;    // user tools can disable gathering of this terrain tile by setting this flag (default true)
            set;
        }
        bool scatterEnable              {
            get;    // user tools can disable scattering to this terrain tile by setting this flag (default true)
            set;
        }
        object userData                 {
    ...
    private class TerrainTile : ITerrainInfo
    {
        ...
        RectInt ITerrainInfo.clippedTerrainPixels    {
            get {
                return clippedTerrainPixels;
            }
        }
        RectInt ITerrainInfo.clippedPCPixels         {
            get {
                return clippedPCPixels;
            }
        }
