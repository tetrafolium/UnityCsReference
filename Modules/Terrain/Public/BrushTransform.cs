...
namespace UnityEngine.Experimental.TerrainAPI {
  // represents a linear 2D transform between brush UV space and some other
  // target XY space
  ...
  public struct BrushTransform {
    public Vector2 brushOrigin {
      get; // brush UV origin, in XY space
    }
    public Vector2 brushU {
      get; // brush U vector, in XY space
    }
    public Vector2 brushV {
      get; // brush V vector, in XY space
    }

    public Vector2 targetOrigin {
      get; // XY origin, in brush UV space
    }
    public Vector2 targetX {
      get; // X vector, in brush UV space
    }
    public Vector2 targetY {
      get; // Y vector, in brush UV space
    }
    ...
    public BrushTransform(Vector2 brushOrigin, Vector2 brushU, Vector2 brushV) {
      ...
      float invDet =
          Mathf.Approximately(det, 0.0f)
              ? 1.0f
              : 1.0f / det; // for non-invert-able matrices, we do 'something'
    ...
    public Rect GetBrushXYBounds() // get the XY bounding rectangle around the
                                   // Brush [0,1] UV space
    {
      ...
      float minX =
          Mathf.Min(Mathf.Min(brushOrigin.x, pU.x), Mathf.Min(pV.x, pUV.x));
      float maxX =
          Mathf.Max(Mathf.Max(brushOrigin.x, pU.x), Mathf.Max(pV.x, pUV.x));
      float minY =
          Mathf.Min(Mathf.Min(brushOrigin.y, pU.y), Mathf.Min(pV.y, pUV.y));
      float maxY =
          Mathf.Max(Mathf.Max(brushOrigin.y, pU.y), Mathf.Max(pV.y, pUV.y));
