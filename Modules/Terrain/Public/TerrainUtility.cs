...
namespace UnityEngine.Experimental.TerrainAPI {
  public static class TerrainUtility {
    public class TerrainMap {
      ...
      static public TerrainMap
      CreateFromConnectedNeighbors(Terrain originTerrain,
                                   TerrainFilter filter = null,
          ...
          if ((filter == null) || filter(cur.terrain)) {
            if (terrainMap.TryToAddTerrain(cur.tileX, cur.tileZ, cur.terrain)) {
              ...
              if (cur.terrain.leftNeighbor != null)
                todoQueue.Enqueue(new QueueElement(cur.tileX - 1, cur.tileZ,
                                                   cur.terrain.leftNeighbor));
              if (cur.terrain.bottomNeighbor != null)
                todoQueue.Enqueue(new QueueElement(cur.tileX, cur.tileZ - 1,
                                                   cur.terrain.bottomNeighbor));
              if (cur.terrain.rightNeighbor != null)
                todoQueue.Enqueue(new QueueElement(cur.tileX + 1, cur.tileZ,
                                                   cur.terrain.rightNeighbor));
              if (cur.terrain.topNeighbor != null)
                todoQueue.Enqueue(new QueueElement(cur.tileX, cur.tileZ + 1,
                                                   cur.terrain.topNeighbor));
            }
          }
        }
      ...
      // create a terrain map of ALL terrains, by using only their placement to
      // fit them to a grid the position and size of originTerrain defines the
      // grid alignment and origin.  if NULL, we use the first active terrain
      static public TerrainMap CreateFromPlacement(Terrain originTerrain,
                                                   TerrainFilter filter = null,
                                                   bool fullValidation = true) {
        if ((Terrain.activeTerrains == null) ||
            (Terrain.activeTerrains.Length == 0) || (originTerrain == null))
          return null;
        ...
        return CreateFromPlacement(new Vector2(gridOriginX, gridOriginZ),
                                   new Vector2(gridSizeX, gridSizeZ), filter,
                                   fullValidation);
      }
      ...
      // fit them to a grid the position and size of originTerrain defines the
      // grid alignment and origin.  if NULL, we use the first active terrain
      static public TerrainMap CreateFromPlacement(Vector2 gridOrigin,
                                                   Vector2 gridSize,
                                                   TerrainFilter filter = null,
                                                   bool fullValidation = true) {
        if ((Terrain.activeTerrains == null) ||
            (Terrain.activeTerrains.Length == 0))
          return null;
        ...
        foreach (Terrain terrain in Terrain.activeTerrains) {
          // some integration tests just create a terrain component without
          // terrain data
      ...
      Vector3
          m_patchSize; // size of the first terrain, used for consistency checks
      ...
      // attempt to place the specified terrain tile at the specified (x,z)
      // position, with consistency checks
      ...
      void ValidateTerrain(int tileX, int tileZ) {
        ...
        if (terrain != null) {
          ...
          {
            if (left) {
              if (!Mathf.Approximately(terrain.transform.position.x,
                                       left.transform.position.x +
                                           left.terrainData.size.x) ||
                  !Mathf.Approximately(terrain.transform.position.z,
    ...
    internal static bool HasValidTerrains() {
      return Terrain.activeTerrains != null &&
             Terrain.activeTerrains.Length > 0;
    }
    ...
    internal static TerrainGroups
    CollectTerrains(bool onlyAutoConnectedTerrains = true) {
      if (!HasValidTerrains())
        return null;
      ...
      foreach (Terrain t in Terrain.activeTerrains) {
        ...
        if (!TerrainGroups.ContainsKey(t.groupingID)) {
          TerrainMap map = TerrainMap.CreateFromPlacement(
              t, (x =>(x.groupingID == t.groupingID) &&
                      !(onlyAutoConnectedTerrains && !x.allowAutoConnect)));
          if (map != null)
