...
public static Vector2 GetCursorPosition(TextNativeSettings settings,
                                        Rect rect, int cursorIndex) {
...
public static unsafe NativeArray<TextVertex>
GetVertices(TextNativeSettings settings) {
	int vertexCount = 0;
	GetVertices(settings, IntPtr.Zero, UnsafeUtility.SizeOf<TextVertex>(),
	            ref vertexCount);

	var array = new NativeArray<TextVertex>(
		vertexCount, Allocator.Temp, NativeArrayOptions.UninitializedMemory);
	if (vertexCount > 0) {
		GetVertices(settings, (IntPtr) array.GetUnsafePtr(),
		            UnsafeUtility.SizeOf<TextVertex>(), ref vertexCount);
		Debug.Assert(vertexCount == array.Length);
	}
	return array;
}

public static Vector2 GetOffset(TextNativeSettings settings,
                                Rect screenRect) {
	if (settings.font == null) {
		Debug.LogError("Cannot process a null font.");
...
DoComputeTextHeight(TextNativeSettings settings);

[FreeFunction(Name = "TextNative::GetCursorPosition")]
private static extern Vector2
DoGetCursorPosition(TextNativeSettings settings, Rect rect,
                    int cursorPosition);

[FreeFunction(Name = "TextNative::GetVertices")]
private static extern void GetVertices(TextNativeSettings settings,
                                       IntPtr buffer, int vertexSize,
                                       ref int vertexCount);

[FreeFunction(Name = "TextNative::GetOffset")]
private static extern Vector2 DoGetOffset(TextNativeSettings settings,
                                          Rect rect);
}
}
