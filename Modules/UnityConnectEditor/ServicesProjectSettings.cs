...
/// Sets up NotificationManager by default, allows usage of RootVisualElement
/// and has a default activateHandler to set it all up.  Will call
/// ActivateAction when all the initial setup is done within the
/// activateHandler. Keep in mind that each time ActivateAction is called, the
/// UIElements window is brand new. Do not cache things related to UIElement
/// state in your extension.
/// </summary>
...
internal struct OpenPackageManager { public string packageName; }

protected abstract string serviceUssClassName {
	get;
}
...
protected abstract Notification.Topic[] notificationTopicsToSubscribe {
	get;
}

protected abstract SingleService serviceInstance {
	get;
}

protected virtual bool sendNotificationForNonStandardStates => true;

const string k_LoggedOutMessage =
	"You are not currently logged in. To enable this service, please login.";
const string k_UnboundMessage =
	"Project does not have a Unity project ID. To enable this service, go to Project Settings, Services and create a new project Id or link to an existing one.";
const string k_ProjectBindingBrokenMessage =
...
const string k_CoppaRequiredMessage =
	"COPPA compliance wasn't defined for this project. To enable this service, go to Project Settings, Services and configure COPPA compliance.";
const string k_CoppaPermissionMessage =
	"You do not have sufficient permissions to set COPPA Compliance.";
...
protected const string k_ServiceToggleContainerClassName =
...
protected static UserRole currentUserPermission {
	get;
	private set;
}
...
const Notification.Topic k_NonStandardStateTopic =
	Notification.Topic.ProjectBind;
const Notification.Severity k_NonStandardStateSeverity =
...
const string k_GeneralServicesTemplatePath =
	"UXML/ServicesWindow/GeneralProjectSettings.uxml";
...
// In order to minimize the number of calls to ActivateAction /
// DeactivateAction, we need to only achieve those if the project info
// really changed, thus keeping a cached copy...
...
protected ServicesProjectSettings(string path, SettingsScope scopes,
                                  string label,
	...
	m_ConnectingServicesState =
		new ConnectingServicesState(m_StateMachine, this);
	...
	activateHandler = (s, element) => {
		...
		// Create a child to make sure all the style sheets are not added to the
		// root.
		rootVisualElement = new ScrollView();
		rootVisualElement.AddToClassList(serviceUssClassName);
		rootVisualElement.AddStyleSheetPath(
			ServicesUtils.StylesheetPath.servicesWindowCommon);
		rootVisualElement.AddStyleSheetPath(
			EditorGUIUtility.isProSkin ? ServicesUtils.StylesheetPath
			.servicesWindowDark
	    : ServicesUtils.StylesheetPath.servicesWindowLight);
		rootVisualElement.AddStyleSheetPath(
			ServicesUtils.StylesheetPath.servicesCommon);
		rootVisualElement.AddStyleSheetPath(
			EditorGUIUtility.isProSkin ? ServicesUtils.StylesheetPath
			.servicesDark
	    : ServicesUtils.StylesheetPath.servicesLight);

		element.Add(rootVisualElement);

		m_SearchContext = s;
		this.label = label;
		m_GeneralTemplate = EditorGUIUtility.Load(k_GeneralServicesTemplatePath)
		                    as VisualTreeAsset;
		...
		if (settingsWindow.GetCurrentProvider() != this) {
			EditorAnalytics.SendEventShowService(new ShowServiceState(){
						service = GetServiceInstanceName(), page = "",
	...
	deactivateHandler = () => {
		...
		// Make sure to invalidate the actual root element for this provider
		// since this is not supposed to be the one active
...
bool IsProjectInfoChanged() {
	...
	// Doing something like: return (m_CachedProjectInfo !=
	// UnityConnect.instance.projectInfo); DOES NOT work. Also, we do not
	// consider "projectInfo.valid" as a property to identify if the info
	// changed.
	return (m_CachedProjectInfo.buildAllowed !=
	        UnityConnect.instance.projectInfo.buildAllowed) ||
	       (m_CachedProjectInfo.coppaLock !=
	        UnityConnect.instance.projectInfo.coppaLock) ||
	       (m_CachedProjectInfo.moveLock !=
	        UnityConnect.instance.projectInfo.moveLock) ||
	       (m_CachedProjectInfo.projectBound !=
	        UnityConnect.instance.projectInfo.projectBound) ||
	       (m_CachedProjectInfo.organizationId !=
	        UnityConnect.instance.projectInfo.organizationId) ||
	       (m_CachedProjectInfo.organizationName !=
	        UnityConnect.instance.projectInfo.organizationName) ||
	       (m_CachedProjectInfo.organizationForeignKey !=
	        UnityConnect.instance.projectInfo.organizationForeignKey) ||
	       (m_CachedProjectInfo.projectName !=
	        UnityConnect.instance.projectInfo.projectName) ||
	       (m_CachedProjectInfo.projectGUID !=
	        UnityConnect.instance.projectInfo.projectGUID) ||
	       (m_CachedProjectInfo.COPPA !=
	        UnityConnect.instance.projectInfo.COPPA);
}
...
protected static void SetupServiceToggleLabel(Toggle toggle, bool active) {
	...
	toggle.text =
		active ? s_ToggleOnLabelTranslated : s_ToggleOffLabelTranslated;
}

void OnRefreshRequired(ProjectInfo state) {
	// The check to reinitialize was not complete, we also have to take care
	// of the online/loggedin states If those states changed when the state of
	// the project info was not valid, we have to take note of that so that
	// the reinitialize is eventually called
	m_UnityConnectStateChanged =
		m_UnityConnectStateChanged ||
		(m_CachedOnlineState != UnityConnect.instance.online) ||
		(m_CachedLoggedInState != UnityConnect.instance.loggedIn);

	// Before reinitializing, we check that the actual state of the project
	// info is valid and
	//     that it has changed since the ActivateHandler.
	if (state.valid &&
	    (IsProjectInfoChanged() || m_UnityConnectStateChanged)) {
	...
	// Must make sure the state changed indicator comes back to false after
	// any valid state...
	if (state.valid) {
...
void ConfigureNotificationSubscriberForNonStandardStates() {
	m_NotificationSubscriber =
		new UIElementsNotificationSubscriber(rootVisualElement);
	var topics = notificationTopicsToSubscribe;
	if (!topics.Contains(Notification.Topic.ProjectBind)) {
		topics =
			new Notification.Topic[notificationTopicsToSubscribe.Length + 1];
		for (var i = 0; i < notificationTopicsToSubscribe.Length; i++) {
			topics[i] = notificationTopicsToSubscribe[i];
		}

		topics[notificationTopicsToSubscribe.Length] =
			Notification.Topic.ProjectBind;
	}
	var notifications = NotificationManager.instance.Subscribe(
		m_NotificationSubscriber, topics);
	foreach (var notification in notifications) {
		m_NotificationSubscriber.ReceiveNotification(notification);
	}
}

protected virtual INotificationSubscriber
ConfigureNotificationSubscriber() {
	var defaultNotificationSubscriber =
		new UIElementsNotificationSubscriber(rootVisualElement);
	var notifications = NotificationManager.instance.Subscribe(
		defaultNotificationSubscriber, notificationTopicsToSubscribe);
	foreach (var notification in notifications) {
...
void PublishConfigWarningNotification(string message) {
	if (sendNotificationForNonStandardStates) {
		NotificationManager.instance.Publish(
			k_NonStandardStateTopic, k_NonStandardStateSeverity, message);
	}
}

static void DismissWarningNotifications() {
	var dismissLogNotification = UnityConnect.instance.loggedIn;
	var dismissBrokenBindNotification =
		UnityConnect.instance.projectInfo.projectBound ||
		string.IsNullOrEmpty(UnityConnect.instance.projectInfo.projectGUID);
	var dismissUnboundNotification =
		UnityConnect.instance.projectInfo.projectBound;
	var dismissCoppaNotification = UnityConnect.instance.projectInfo.COPPA !=
	                               COPPACompliance.COPPAUndefined;

	var notifications =
		NotificationManager.instance.GetNotificationsForTopics(
			Notification.Topic.ProjectBind);
	foreach (var notification in notifications) {
		if (notification.severity == k_NonStandardStateSeverity &&
		    notification.topic == k_NonStandardStateTopic) {
			if ((notification.rawMessage == k_LoggedOutMessage &&
			     dismissLogNotification) ||
			    (notification.rawMessage == k_UnboundMessage &&
			     dismissUnboundNotification) ||
			    (notification.rawMessage == k_ProjectBindingBrokenMessage &&
			     dismissBrokenBindNotification) ||
			    (notification.rawMessage == k_CoppaRequiredMessage &&
			     dismissCoppaNotification)) {
...
protected void OpenDashboardForOrgKeyAndProjectGuid(string dashboardUrl) {
	...
	EditorAnalytics.SendOpenDashboardForService(new OpenDashboardForService(){
				serviceName = GetServiceInstanceName(), url = dashboardUrl,
				organizationId = orgForeignKey, projectId = projectGuid
			});
	Application.OpenURL(
		String.Format(dashboardUrl, orgForeignKey, projectGuid));
}
...
protected void OpenDashboardOrgAndProjectIds(string dashboardUrl) {
	...
	EditorAnalytics.SendOpenDashboardForService(new OpenDashboardForService(){
				serviceName = GetServiceInstanceName(), url = dashboardUrl,
				organizationId = orgSubUrl, projectId = projectSubUrl
			});
	Application.OpenURL(
		String.Format(dashboardUrl, orgSubUrl, projectSubUrl));
}

void RefreshCurrentUserRole() {
	ServicesConfiguration.instance.RequestCurrentProjectUsersApiUrl(
		currentProjectUsersApiUrl => {
				var getProjectUsersRequest = new UnityWebRequest(
					currentProjectUsersApiUrl, UnityWebRequest.kHttpVerbGET){
				        downloadHandler = new DownloadHandlerBuffer()
				};
				getProjectUsersRequest.suppressErrorsToConsole = true;
				getProjectUsersRequest.SetRequestHeader(
					"AUTHORIZATION",
					$"Bearer {UnityConnect.instance.GetUserInfo().accessToken}");
				...
				operation.completed += op => {
				        try {
								...
								getProjectUsersRequest.downloadHandler.text);
				                        var json = jsonParser.Parse();
				                        try {
				                                var currentUserId = UnityConnect.instance.userInfo.userId;
				                                var users = json.AsDict() [k_JsonUsersNodeName]
				                                            .AsList();
				                                foreach (var rawUser in users) {
				                                        var user = rawUser.AsDict();
				                                        if (currentUserId.Equals(user [k_JsonUserIdNodeName]
				                                                                 .AsString())) {
				                                                currentUserPermission =
											ConvertStringToUserRole(user [k_JsonRoleNodeName]
											                        .AsString());
				                                                InternalToggleRestrictedVisualElementsAvailability(
											currentUserPermission == UserRole.User);
				                                                break;
									}
								}
							} catch (Exception ex) {
...
void InternalToggleRestrictedVisualElementsAvailability(bool isUserAccess) {
	...
	var editModeContainer = coppaContainer?.Q(className
	                                          : k_ClassNameEditMode);
	if (editModeContainer != null) {
		editModeContainer.SetEnabled(!isUserAccess);
		if (isUserAccess) {
			NotificationManager.instance.Publish(
				Notification.Topic.CoppaCompliance, Notification.Severity.Warning,
				k_CoppaPermissionMessage);
		}
	}
...
ToggleRestrictedVisualElementsAvailability(bool enable);

/// <summary>
/// This method is called by the activateHandler after rootVisualElement
/// setups is done, but before NotificationManager setup is done. Load the
/// UXML within ActivateAction. If you need to setup specific things for
/// Notification Subscriber, override ConfigureNotificationSubscriber() Keep
/// in mind that each time ActivateAction is called, the UIElements window
/// is brand new. Do not cache things related to UIElement state in your
/// extension.
/// </summary>
/// <param name="searchContext">The search context of the Project Settings
/// window</param>
...
protected BaseState(SimpleStateMachine<Event> simpleStateMachine,
                    string name, ServicesProjectSettings provider)
...
public DeactivateState(SimpleStateMachine<Event> simpleStateMachine,
                       ServicesProjectSettings provider)
...
public InitialState(SimpleStateMachine<Event> simpleStateMachine,
                    ServicesProjectSettings provider)
	...
	ModifyActionForEvent(Event.GoingToConnectingServices,
	                     HandleGoingToConnectingServices);
...
public override void EnterState() {
	...
	if (!UnityConnect.instance.projectInfo.projectBound) {
		...
		provider.PublishConfigWarningNotification(
			k_ProjectBindingBrokenMessage);
...
SimpleStateMachine<Event>.State
HandleGoingToConnectingServices(Event arg) {
...
public StandardState(SimpleStateMachine<Event> simpleStateMachine,
                     ServicesProjectSettings provider)
...
public override void EnterState() {
	...
	provider.m_NotificationSubscriber =
		provider.ConfigureNotificationSubscriber();
}
...
public CoppaState(SimpleStateMachine<Event> simpleStateMachine,
                  ServicesProjectSettings provider)
...
public override void EnterState() {
	...
	contentContainer.AddStyleSheetPath(
	    ...
	    : ServicesUtils.StylesheetPath.servicesLight);
	var scrollContainer =
		provider.rootVisualElement.Q(null, k_ScrollContainerClassName);
	scrollContainer.Clear();
	provider.ConfigureNotificationSubscriberForNonStandardStates();

	m_CoppaManager = new CoppaManager(scrollContainer){
		saveButtonCallback =
			compliance => {stateMachine.ProcessEvent(Event.Initializing);}
		, exceptionCallback = (compliance, exception) => {
			NotificationManager.instance.Publish(Notification.Topic.CoppaCompliance,
...
public UnboundState(SimpleStateMachine<Event> simpleStateMachine,
                    ServicesProjectSettings provider)
...
public override void EnterState() {
	...
	contentContainer.AddStyleSheetPath(
		ServicesUtils.StylesheetPath.servicesCommon);
	contentContainer.AddStyleSheetPath(
		EditorGUIUtility.isProSkin ? ServicesUtils.StylesheetPath.servicesDark
	: ServicesUtils.StylesheetPath.servicesLight);
	var scrollContainer =
		provider.rootVisualElement.Q(null, k_ScrollContainerClassName);
	scrollContainer.Clear();
	...
	m_ProjectBindManager = new ProjectBindManager(scrollContainer){
		createButtonCallback =
			projectInfoData => {stateMachine.ProcessEvent(Event.Initializing);}
		, linkButtonCallback = projectInfoData => {
			stateMachine.ProcessEvent(Event.Initializing);
		}, exceptionCallback = (exception) => {
			NotificationManager.instance.Publish(Notification.Topic.ProjectBind,
			                                     Notification.Severity.Error,
			                                     L10n.Tr(exception.Message));
		}
	};
...
public BrokenBindingState(SimpleStateMachine<Event> simpleStateMachine,
                          ServicesProjectSettings provider)
...
public override void EnterState() {
	...
	var brokenBindingTemplate =
		EditorGUIUtility.Load(k_TemplatePath) as VisualTreeAsset;
	var scrollContainer =
		provider.rootVisualElement.Q(null, k_ScrollContainerClassName);
	scrollContainer.Clear();
	provider.ConfigureNotificationSubscriberForNonStandardStates();
	var brokenContentContainer =
		brokenBindingTemplate.CloneTree().contentContainer;
	ServicesUtils.TranslateStringsInTree(brokenContentContainer);
	scrollContainer.Add(brokenContentContainer);

	var projectBrokenBindContainer =
		provider.rootVisualElement.Q(k_ProjectBrokenBindContainerName);

	var newLinkButton =
		projectBrokenBindContainer.Q<Button>(k_NewLinkButtonName);
	if (newLinkButton != null) {
		newLinkButton.clicked +=
			() => { stateMachine.ProcessEvent(Event.Unbinding); };
	}
	var refreshAccessButton =
		projectBrokenBindContainer.Q<Button>(k_RefreshAccessButtonName);
	if (refreshAccessButton != null) {
		refreshAccessButton.clicked += () => {
			NotificationManager.instance.Publish(
				Notification.Topic.ProjectBind, Notification.Severity.Info,
				L10n.Tr(k_PermissionRefreshedMessage));
			UnityConnect.instance.RefreshProject();
		};
	}
}
...
public OfflineState(SimpleStateMachine<Event> simpleStateMachine,
                    ServicesProjectSettings provider)
...
public override void EnterState() {
	...
	if (refreshAccessButton != null) {
		refreshAccessButton.clicked += () => {
			NotificationManager.instance.Publish(
				Notification.Topic.ProjectBind, Notification.Severity.Info,
				L10n.Tr(k_ConnectionRefreshedMessage));
			UnityConnect.instance.RefreshProject();
		};
	}
}
...
const string k_ConnectingServicesContainerName =
	"ConnectingServicesContainer";
const string k_RefreshButtonName = "RefreshBtn";

const string k_ConnectionRefreshedMessage = "Attempting connection refresh.";
const string k_ConnectionFailedMessage =
	"Failed to connect to Services. Services are not reachable right now.";
...
public ConnectingServicesState(SimpleStateMachine<Event> simpleStateMachine,
                               ServicesProjectSettings provider)
...
public override void EnterState() {
	...
	var connectingServicesTemplate =
		EditorGUIUtility.Load(k_TemplatePath) as VisualTreeAsset;
	var scrollContainer =
		provider.rootVisualElement.Q(null, k_ScrollContainerClassName);
	scrollContainer.Clear();
	provider.ConfigureNotificationSubscriberForNonStandardStates();
	var connectingServicesTemplateContainer =
		connectingServicesTemplate.CloneTree().contentContainer;
	...
	var connectingServicesContainer =
		provider.rootVisualElement.Q(k_ConnectingServicesContainerName);

	var refreshAccessButton =
		connectingServicesContainer.Q<Button>(k_RefreshButtonName);
	if (refreshAccessButton != null) {
		refreshAccessButton.clicked += () => {
			NotificationManager.instance.Publish(
				Notification.Topic.ProjectBind, Notification.Severity.Info,
	...
	// It will probably never come to this since fetching URLs is very fast,
	// but...
	m_ScheduledVerify = contentContainer.schedule.Execute(VerifyPaths)
	                    .Every(k_VerifyDelay)
	                    .StartingIn(k_VerifyDelay);
}
...
void VerifyPaths() {
	...
	if (!ServicesConfiguration.instance.pathsReady &&
	    !ServicesConfiguration.instance.loadingConfigurations) {
	...
	if (m_VerifyRetries > k_MaxVerifyRetries) {
		...
		// Using the refresh btn will relaunch the verif for another
		// k_MaxVerifyRetries attempts
		NotificationManager.instance.Publish(Notification.Topic.ProjectBind,
		                                     Notification.Severity.Error,
		                                     L10n.Tr(k_ConnectionFailedMessage));
		ClearScheduledVerify();
	}
}
...
public LoggedOutState(SimpleStateMachine<Event> simpleStateMachine,
                      ServicesProjectSettings provider)
...
public override void EnterState() {
	...
	scrollContainer.Clear();
	provider.ConfigureNotificationSubscriberForNonStandardStates();
	var loggedOutTemplateContainer =
		loggedOutTemplate.CloneTree().contentContainer;
	ServicesUtils.TranslateStringsInTree(loggedOutTemplateContainer);
	scrollContainer.Add(loggedOutTemplateContainer);

	var loggedOutContainer =
		provider.rootVisualElement.Q(k_LoggedOutContainerName);

	var signInButton = loggedOutContainer.Q<Button>(k_SignInButtonName);
	if (signInButton != null) {
		signInButton.clicked += () => { UnityConnect.instance.ShowLogin(); };
	}
}
...
protected class GenericBaseState<T, U> : SimpleStateMachine<U>.State where T
...
protected const string k_NotLatestPackageInstalledInfo =
	"A newer version of the {0} exists and can be installed.";
protected const string k_DuplicateInstallWarning =
	"Installing both Asset Store and Package Manager packages will generate duplication.\nDo you want to continue?";
protected const string k_PackageInstallationHeadsup =
	"You are about to install the latest {0}.\nDo you want to continue?";
...
bool m_InstallingNewPackage;     // A switch to make sure to only call the
                                 // installation in a singleton fashion...
bool m_NotLatestPackageInfoHasBeenShown;

// Information to be set-up by the actual child classes
protected Notification.Topic topicForNotifications {
	get;
	set;
}
protected string notLatestPackageInstalledInfo {
	get;
	set;
}
protected string packageInstallationHeadsup {
	get;
	set;
}
protected string duplicateInstallWarning {
	get;
	set;
}
protected string packageInstallationDialogTitle {
	get;
	set;
}
protected bool assetStorePackageInstalled {
	get;
	set;
}
protected bool packmanPackageInstalled {
	get;
	set;
}
protected bool isLatestPackageInstalled {
	get;
	set;
}

protected string installPackageVersion {
	get;
	set;
}

// String containing the actual text for the version number label
protected string currentPackageVersion {
	get;
	private set;
}
protected string latestPackageVersion {
	get;
	private set;
}
...
protected void UpdatePackageUpdateButton() {
	...
	if ((currentPackageVersion != string.Empty) &&
	    (GetUpdatablePackageVersion() != string.Empty)) {
		if (currentPackageVersion.Equals(GetUpdatablePackageVersion())) {
			...
			// Display an info if a package is installed, but it is not the latest
			// one !
			if (!m_NotLatestPackageInfoHasBeenShown) {
				NotificationManager.instance.Publish(
					topicForNotifications, Notification.Severity.Info,
					L10n.Tr(notLatestPackageInstalledInfo));
...
protected void AddPackageProgress() {
	if (m_AddRequest.IsCompleted) {
		...
		if (m_AddRequest.Status >= StatusCode.Failure) {
			...
			NotificationManager.instance.Publish(topicForNotifications,
			                                     Notification.Severity.Error,
			                                     m_AddRequest.Error.message);
		} else {
			EditorAnalytics.SendImportServicePackageEvent(new ImportPackageInfo(){
							packageName = provider.serviceInstance.packageName,
							version = installPackageVersion
						});
		}
...
OnSearchPackageFound(PackageManager.PackageInfo package) {
...
protected void ListingCurrentPackageProgress() {
	if (m_Request.IsCompleted) {
		...
		if (m_Request.Status == StatusCode.Success) {
			...
			// Call the update button update independently of the actual presence of
			// the package
			UpdatePackageUpdateButton();
			PackageInformationUpdated();
...
protected void PreparePackageSection(VisualElement sectionRoot) {
	...
	if (gotoPackManWindow != null) {
		gotoPackManWindow.clicked += () => {
			...
			EditorAnalytics.SendOpenPackManFromServiceSettings(
				new OpenPackageManager(){
	...
	if (m_CurrentPackageVersionLabel != null) {
		m_CurrentPackageVersionLabel.text =
			m_CurrentPackageVersionLabel.text.ToUpper();
	}
	if (m_LatestPackageVersionLabel != null) {
		m_LatestPackageVersionLabel.text =
			m_LatestPackageVersionLabel.text.ToUpper();
	}
...
protected void InstallPackage(string messageForDialog) {
	if (!m_InstallingNewPackage) {
		if (EditorUtility.DisplayDialog(L10n.Tr(packageInstallationDialogTitle),
		                                messageForDialog, L10n.Tr(k_ChoiceYes),
		                                L10n.Tr(k_ChoiceNo))) {
...
public GenericBaseState(string name, SimpleStateMachine<U> simpleStateMachine,
                        T provider)
	: base(name, simpleStateMachine) {
