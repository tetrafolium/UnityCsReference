...
/// <summary>
/// The general section of the CloudServices project settings (actually simply
/// called Services in the ProjectSettings) Uses a simple state machine to
...
const string k_TemplatePath =
	"UXML/ServicesWindow/GeneralProjectSettingsStateBound.uxml";
...
const string k_UnlinkProjectDialogMessage =
	"Are you sure you want to unlink this project?";
const string k_ProjectUnlinkSuccessMessage =
	"Project was unlinked successfully.";
...
public static SettingsProvider CreateServicesProvider() {
	return new GeneralProjectSettings(
		k_ProjectSettingsPath, SettingsScope.Project,
		new List<string>(){
				L10n.Tr(k_KeywordOrganization), L10n.Tr(k_KeywordProject),
				L10n.Tr(k_KeywordUnlink), L10n.Tr(k_KeywordMembers),
				L10n.Tr(k_KeywordDashboard), L10n.Tr(k_KeywordId)
			});
}

GeneralProjectSettings(string path, SettingsScope scopes,
                       IEnumerable<string> keywords = null)
	: base(path, scopes, k_GeneralLabel, keywords) {
}

protected override Notification.Topic[] notificationTopicsToSubscribe =>
new[] {Notification.Topic.AdsService,
...
protected override void ActivateAction(string searchContext) {
	...
	var projectInfoOnBind = new ProjectInfoData(
	...
	InitializeFieldBlock(organizationFieldBlock,
	                     projectInfoOnBind.organizationName);
	...
	rootVisualElement.Q(k_DashboardBlockName)
	.AddManipulator(dashboardClickable);

	projectIdFieldBlock.Q<Button>(k_ProjectIdSubmitButtonName).clicked +=
		UnbindProject;
...
void SetupCoppaManager(VisualElement parentContainer) {
	...
	parentContainer.Q(CoppaManager.coppaContainerName)
	?.parent?.RemoveFromHierarchy();
	m_CoppaManager = new CoppaManager(parentContainer){
		exceptionCallback =
			(compliance, exception) => {NotificationManager.instance.Publish(
							    Notification.Topic.CoppaCompliance,
							    Notification.Severity.Error, L10n.Tr(exception.Message));}
	};
...
static void InitializeFieldBlock(VisualElement fieldBlock,
	...
	if (fieldBlock != null) {
		...
		var cancelButton = fieldBlock.Q<Button>(className
		                                        : k_CancelButtonClassName);
		...
		if (fieldValue != null) {
			...
			var valueTextField = fieldBlock.Q<TextField>(className
			                                             : k_FieldValueClassName);
...
void UnbindProject() {
	if (EditorUtility.DisplayDialog(L10n.Tr(k_UnlinkProjectDialogTitle),
		...
		EditorAnalytics.SendProjectServiceBindingEvent(
			new ProjectBindManager.ProjectBindState(){
		...
		NotificationManager.instance.Publish(
			...
			L10n.Tr(k_ProjectUnlinkSuccessMessage));
