...
namespace UnityEditor.Connect {
  internal class CloudBuildProjectSettings : ServicesProjectSettings {
    ...
    const string k_CloudBuildCommonUxmlPath =
        "UXML/ServicesWindow/CloudBuildProjectSettings.uxml";
    const string k_CloudBuildDisabledUxmlPath =
        "UXML/ServicesWindow/CloudBuildProjectSettingsStateDisabled.uxml";
    const string k_CloudBuildEnabledUxmlPath =
        "UXML/ServicesWindow/CloudBuildProjectSettingsStateEnabled.uxml";

    const string k_SubscriptionPersonal = "personal";
    const string k_SubscriptionTeamsBasic = "teams basic";
    // Other subscription possibility is "teams advanced", but we don't need it
    // for now.
    ...
    const string k_ServiceTargetContainerTitleClassName =
        "target-container-title";
    const string k_ServiceCloudBuildContainerClassName =
    ...
    const string k_MessageErrorForProjectData =
        "An unexpected error occurred while querying Cloud Build for current project data. See the console for more information.";
    const string k_MessageErrorForProjectTeamData =
        "An unexpected error occurred while querying Cloud Build for current project team data. See the console for more information.";
    const string k_MessageErrorForProjectBuildTargetsData =
        "An unexpected error occurred while querying Cloud Build for current project configured build targets. See the console for more information.";
    const string k_MessageProjectStateMismatch =
        "There is a mismatch between local and web configuration for Cloud Build. Please open the Cloud Build web dashboard and enable the current project.";
    const string k_MessageErrorForApiStatusData =
        "An unexpected error occurred while querying Cloud Build for api status. See the console for more information.";
    const string k_MessageErrorForBuildLaunch =
        "An unexpected error occurred while launching a build. See the console for more information.";
    const string k_CloudBuildPermissionMessage =
        "You do not have sufficient permissions to enable / disable Cloud Build service.";

    const string k_MessageLaunchingBuild = "Starting build {0}.";
    const string k_MessageLaunchedBuildSuccess =
        "Build #{0} {1} added to queue";
    const string k_MessageLaunchedBuildFailedWithMsg =
        "Build {0} wasn't launched: {1}";
    ...
    private struct BuildPostInfo { public string targetName; }

    [SettingsProvider]
    public static SettingsProvider CreateServicesProvider() {
      return new CloudBuildProjectSettings(
          BuildService.instance.projectSettingsPath, SettingsScope.Project);
    }
    ...
    public CloudBuildProjectSettings(string path, SettingsScope scopes,
    ...
    void OnDestroy() { DeactivateAction(); }

    protected override Notification.Topic[] notificationTopicsToSubscribe =>
        new[]{Notification.Topic.BuildService, Notification.Topic.ProjectBind};
    ...
    void SetupServiceToggle(SingleService singleService) {
      m_MainServiceToggle.SetProperty(k_ServiceNameProperty,
                                      singleService.name);
      ...
      if (singleService.displayToggle) {
        m_MainServiceToggle.RegisterValueChangedCallback(evt => {
          if (currentUserPermission != UserRole.Owner &&
              currentUserPermission != UserRole.Manager) {
    ...
    void UpdateServiceToggleAndDashboardLink(bool isEnabled) {
      if (m_GoToDashboard != null) {
        m_GoToDashboard.style.display =
            (isEnabled) ? DisplayStyle.Flex : DisplayStyle.None;
      }
    ...
    ToggleRestrictedVisualElementsAvailability(bool enable) {
      var serviceToggleContainer =
          rootVisualElement.Q(className
                              : k_ServiceToggleContainerClassName);
      var unityToggle = serviceToggleContainer?.Q(className
                                                  : k_UnityToggleClassName);
      if (unityToggle != null) {
        ...
        if (!enable) {
          ...
          NotificationManager.instance.Publish(Notification.Topic.BuildService,
    ...
    protected override void ActivateAction(string searchContext) {
      ...
      var mainTemplate =
          EditorGUIUtility.Load(k_CloudBuildCommonUxmlPath) as VisualTreeAsset;

      // To allow the save using the view data, we must provide a key on the
      // root element
      ...
      if (m_GoToDashboard != null) {
        var clickable = new Clickable(() => {
          ServicesConfiguration.instance.RequestBaseCloudBuildDashboardUrl(
              OpenDashboardOrgAndProjectIds);
        });
        m_GoToDashboard.AddManipulator(clickable);
      }

      m_MainServiceToggle =
          rootVisualElement.Q<Toggle>(className
                                      : k_ServiceToggleClassName);
      ...
      if (learnMore != null) {
        var clickable = new Clickable(() => {
          Application.OpenURL(
              ServicesConfiguration.instance.GetUnityTeamInfoUrl());
        });
    ...
    void RegisterEvent() {
      if (!m_EventHandlerInitialized) {
        ...
        BuildService.instance.serviceAfterDisableEvent +=
            ServiceIsDisablingEvent;
    ...
    sealed class DisabledState : SimpleStateMachine<CloudBuildEvent>.State {
      ...
      public DisabledState(
          SimpleStateMachine<CloudBuildEvent> simpleStateMachine,
          CloudBuildProjectSettings provider)
      ...
      public override void EnterState() {
        ...
        var generalTemplate = EditorGUIUtility.Load(
            k_CloudBuildDisabledUxmlPath) as VisualTreeAsset;
        var scrollContainer =
            m_Provider.rootVisualElement.Q(className
                                           : k_ServiceScrollContainerClassName);
        scrollContainer.Clear();
        if (generalTemplate != null) {
          var disabledStateContent =
              generalTemplate.CloneTree().contentContainer;
          ServicesUtils.TranslateStringsInTree(disabledStateContent);
          scrollContainer.Add(disabledStateContent);
          m_Provider.UpdateServiceToggleAndDashboardLink(
              m_Provider.serviceInstance.IsServiceEnabled());
        }

        var startUsing =
            m_Provider.rootVisualElement.Q(k_StartUsingCloudBuildLink);
        if (startUsing != null) {
          var clickable = new Clickable(() => {
            Application.OpenURL(
                ServicesConfiguration.instance.GetCloudBuildTutorialUrl());
          });
      ...
      SimpleStateMachine<CloudBuildEvent>.State
      HandleBinding(CloudBuildEvent raisedEvent) {
    ...
    sealed class EnabledState : SimpleStateMachine<CloudBuildEvent>.State {
      ...
      public override void EnterState() {
        ...
        var generalTemplate = EditorGUIUtility.Load(k_CloudBuildEnabledUxmlPath)
                                  as VisualTreeAsset;
        var scrollContainer =
            m_Provider.rootVisualElement.Q(className
                                           : k_ServiceScrollContainerClassName);
        ...
        m_Provider.rootVisualElement
            .Q(className
               : k_ServiceCloudBuildContainerClassName)
            .style.display = DisplayStyle.None;
        m_Provider.rootVisualElement.Q(k_PollFooterSectionName).style.display =
            DisplayStyle.None;

        var historyButton =
            m_Provider.rootVisualElement.Q<Button>(k_HistoryButtonName);
        if (historyButton != null) {
          historyButton.clicked += () => {
            ServicesConfiguration.instance
                .RequestCurrentCloudBuildProjectHistoryUrl(
                    currentCloudBuildProjectHistoryUrl => {
        ...
        var uploadButton =
            m_Provider.rootVisualElement.Q<Button>(k_UploadButtonName);
        if (uploadButton != null) {
          uploadButton.clicked += () => {
            ServicesConfiguration.instance
                .RequestCurrentCloudBuildProjectUploadUrl(
                    currentCloudBuildProjectUploadUrl => {
                      Application.OpenURL(currentCloudBuildProjectUploadUrl);
                    });
          };
        }

        var manageTargetButton =
            m_Provider.rootVisualElement.Q<Button>(k_ManageTargetButton);
        if (manageTargetButton != null) {
          manageTargetButton.clicked += () => {
            ServicesConfiguration.instance
                .RequestCurrentCloudBuildProjectTargetUrl(
                    currentCloudBuildProjectTargetUrl => {
        ...
        var addTargetButton =
            m_Provider.rootVisualElement.Q<Button>(k_AddTargetButton);
        if (addTargetButton != null) {
          addTargetButton.clicked += () => {
            ServicesConfiguration.instance.RequestCloudBuildAddTargetUrl(
                cloudBuildAddTargetUrl => {
      ...
      void GetProjectInfo() {
        ServicesConfiguration.instance.RequestCloudBuildApiCurrentProjectUrl(
            cloudBuildApiCurrentProjectUrl => {
              var getCurrentProjectRequest = new UnityWebRequest(
                  cloudBuildApiCurrentProjectUrl, UnityWebRequest.kHttpVerbGET){
                  downloadHandler = new DownloadHandlerBuffer()};
              getCurrentProjectRequest.suppressErrorsToConsole = true;
              getCurrentProjectRequest.SetRequestHeader(
                  "AUTHORIZATION",
      ...
      void GetProjectInfoRequestOnCompleted(AsyncOperation obj) {
        ...
        try {
          if (ServicesUtils.IsUnityWebRequestReadyForJsonExtract(
                  m_Provider.m_GetProjectRequest)) {
            try {
              var jsonParser = new JSONParser(
                  m_Provider.m_GetProjectRequest.downloadHandler.text);
              var json = jsonParser.Parse();
              if (json.AsDict() [k_JsonNodeNameDisabled]
                      .AsBool()) {
                ...
                var linksDict = json.AsDict() [k_JsonNodeNameLinks]
                                    .AsDict();
                ServicesConfiguration.instance.RequestCloudBuildApiUrl(
                    cloudBuildApiUrl => {
                      m_CloudBuildApiOrgProjectUrl =
                          cloudBuildApiUrl + linksDict [k_JsonNodeNameSelf]
                                                 .AsDict() [k_JsonNodeNameHref]
                                                 .AsString();
                      m_CloudBuildApiOrgProjectBillingPlanUrl =
                          m_CloudBuildApiOrgProjectUrl + k_UrlSuffixBillingPlan;
                      m_CloudBuildApiOrgProjectBuildTargetsUrl =
                          cloudBuildApiUrl + linksDict
                                             [k_JsonNodeNameListBuildTargets]
                                                 .AsDict() [k_JsonNodeNameHref]
                                                 .AsString();
                      m_CloudBuildApiOrgLatestBuilds =
                          cloudBuildApiUrl + linksDict
                                             [k_JsonNodeNameLatestBuilds]
                                                 .AsDict() [k_JsonNodeNameHref]
                                                 .AsString();
                      m_CloudBuildApiOrgLatestBuilds =
                          m_CloudBuildApiOrgLatestBuilds.Remove(
                              m_CloudBuildApiOrgLatestBuilds.Length - 1, 1) +
              ...
              NotificationManager.instance.Publish(
                  Notification.Topic.BuildService, Notification.Severity.Error,
      ...
      void GetProjectBillingPlan() {
        var getCurrentProjectBillingPlanRequest =
            new UnityWebRequest(m_CloudBuildApiOrgProjectBillingPlanUrl,
                                UnityWebRequest.kHttpVerbGET){
                downloadHandler = new DownloadHandlerBuffer()};
        getCurrentProjectBillingPlanRequest.suppressErrorsToConsole = true;
        getCurrentProjectBillingPlanRequest.SetRequestHeader(
            "AUTHORIZATION",
            $"Bearer {UnityConnect.instance.GetUserInfo().accessToken}");
        ...
        m_Provider.m_GetProjectBillingPlanRequest =
            getCurrentProjectBillingPlanRequest;
      ...
      void GetProjectBillingPlanRequestOnCompleted(AsyncOperation obj) {
        ...
        try {
          if (ServicesUtils.IsUnityWebRequestReadyForJsonExtract(
            ...
            try {
              var jsonParser =
                  new JSONParser(m_Provider.m_GetProjectBillingPlanRequest
                                     .downloadHandler.text);
              var json = jsonParser.Parse();
              m_BillingPlanLabel = json.AsDict() [k_JsonNodeNameEffective]
                                       .AsDict() [k_JsonNodeNameLabel]
                                       .AsString();
      ...
      void GetProjectBuildTargets() {
        var getCurrentProjectBuildTargetsRequest =
            new UnityWebRequest(m_CloudBuildApiOrgProjectBuildTargetsUrl,
                                UnityWebRequest.kHttpVerbGET){
                downloadHandler = new DownloadHandlerBuffer()};
        getCurrentProjectBuildTargetsRequest.suppressErrorsToConsole = true;
        getCurrentProjectBuildTargetsRequest.SetRequestHeader(
            "AUTHORIZATION",
            $"Bearer {UnityConnect.instance.GetUserInfo().accessToken}");
        ...
        m_Provider.m_GetProjectBuildTargetsRequest =
            getCurrentProjectBuildTargetsRequest;
      ...
      void GetBuildTargetsRequestOnCompleted(AsyncOperation obj) {
        ...
        try {
          if (ServicesUtils.IsUnityWebRequestReadyForJsonExtract(
            ...
            try {
              var jsonParser =
                  new JSONParser(m_Provider.m_GetProjectBuildTargetsRequest
                                     .downloadHandler.text);
              var json = jsonParser.Parse();
              ...
              if (buildEntryList.Count <= 0) {
                ...
                m_Provider.rootVisualElement.Q(k_NoTargetContainer)
                    .style.display = DisplayStyle.None;
                m_Provider.rootVisualElement.Q(k_AddTargetButton)
                    .style.display = DisplayStyle.None;
                m_Provider.rootVisualElement.Q(k_ManageTargetButton)
                    .style.display = DisplayStyle.Flex;
                m_Provider.rootVisualElement.Q(k_PollFooterName).style.display =
                    DisplayStyle.Flex;
                m_Provider.rootVisualElement.Q(k_PollFooterSectionName)
                    .style.display = DisplayStyle.Flex;

                var pollerToggle =
                    m_Provider.rootVisualElement.Q<Toggle>(k_PollToggleName);
                if (!CloudBuildPoller.instance.enabledOnce) {
                  CloudBuildPoller.instance.Enable(
                      m_CloudBuildApiOrgLatestBuilds);
                }
                pollerToggle.SetValueWithoutNotify(
                    CloudBuildPoller.instance.enabled);
                ...
                m_Provider.rootVisualElement
                    .Q<TextElement>(className
                                    : k_ServiceTargetContainerTitleClassName)
                    .text = L10n.Tr(k_LabelConfiguredTargets);
                var targetsContainer = m_Provider.rootVisualElement.Q(
                    className
                    : k_ServiceTargetContainerClassName);
              ...
              m_Provider.rootVisualElement
                  .Q(className
                     : k_ServiceCloudProgressClassName)
                  .style.display = DisplayStyle.None;
            } catch (Exception ex) {
              NotificationManager.instance.Publish(
                  Notification.Topic.BuildService, Notification.Severity.Error,
      ...
      void AddBuildTarget(VisualElement targetsContainer,
                          Dictionary<string, JSONValue> buildEntry) {
        if (buildEntry [k_JsonNodeNameEnabled]
                .AsBool()) {
          ServicesConfiguration.instance.RequestCloudBuildApiUrl(
              cloudBuildApiUrl => {
                var buildTargetName = buildEntry [k_JsonNodeNameName]
                                          .AsString();
                var buildTargetId = buildEntry [k_JsonNodeNameBuildTargetId]
                                        .AsString();
                var buildTargetUrls = buildEntry [k_JsonNodeNameLinks]
                                          .AsDict();
                var startBuildUrl = cloudBuildApiUrl +
                                    buildTargetUrls [k_JsonNodeNameStartBuilds]
                                        .AsDict() [k_JsonNodeNameHref]
                                        .AsString();
                ...
                if (m_BillingPlanLabel.ToLower() == k_SubscriptionPersonal ||
                    k_SubscriptionTeamsBasic.ToLower() ==
                        k_SubscriptionPersonal) {
                  buildButton.SetEnabled(false);
                }
                buildButton.text = L10n.Tr(k_LabelBuildButton);
                buildButton.clicked += () => {
                  var uploadHandler = new UploadHandlerRaw(
                      Encoding.UTF8.GetBytes(k_LaunchBuildPayload));
                  var launchBuildPostRequest = new UnityWebRequest(
                      startBuildUrl, UnityWebRequest.kHttpVerbPOST){
                      downloadHandler = new DownloadHandlerBuffer(),
                      uploadHandler = uploadHandler};
                  launchBuildPostRequest.suppressErrorsToConsole = true;
                  launchBuildPostRequest.SetRequestHeader(
                      "AUTHORIZATION",
                      $"Bearer {UnityConnect.instance.GetUserInfo().accessToken}");
                  launchBuildPostRequest.SetRequestHeader(
                      "Content-Type", "application/json;charset=utf-8");
                  m_Provider.m_BuildRequests.Add(launchBuildPostRequest);
                  var launchingMessage = string.Format(
                      L10n.Tr(k_MessageLaunchingBuild), buildTargetName);

                  Debug.Log(launchingMessage);
                  NotificationManager.instance.Publish(
                      Notification.Topic.BuildService,
                      Notification.Severity.Info, launchingMessage);

                  EditorAnalytics.SendLaunchCloudBuildEvent(
                      new BuildPostInfo(){targetName = buildTargetName});

                  var operation = launchBuildPostRequest.SendWebRequest();
                  operation.completed += asyncOperation => {
                    try {
                      if (ServicesUtils.IsUnityWebRequestReadyForJsonExtract(
                              launchBuildPostRequest)) {
                        try {
                          if (launchBuildPostRequest.responseCode ==
                              k_HttpResponseCodeAccepted) {
                            var jsonLaunchedBuildParser = new JSONParser(
                                launchBuildPostRequest.downloadHandler.text);
                            var launchedBuildJson =
                                jsonLaunchedBuildParser.Parse();
                            ...
                            foreach (var rawLaunchedBuild in launchedBuilds) {
                              ...
                              if (launchedBuild.ContainsKey(
                                      k_JsonNodeNameBuild)) {
                                var buildNumber = launchedBuild
                                                  [k_JsonNodeNameBuild]
                                                      .AsFloat()
                                                      .ToString();
                                var message = string.Format(
                                    L10n.Tr(k_MessageLaunchedBuildSuccess),
                                    buildNumber, buildTargetName);
                                Debug.Log(message);
                                NotificationManager.instance.Publish(
                                    Notification.Topic.BuildService,
                                    Notification.Severity.Info, message);
                              } else if (launchedBuild.ContainsKey(
                                             k_JsonNodeNameError)) {
                                var message = string.Format(
                                    L10n.Tr(
                                        k_MessageLaunchedBuildFailedWithMsg),
                                    buildTargetName,
                                    launchedBuild [k_JsonNodeNameError]
                                        .ToString());
                                Debug.LogError(message);
                                NotificationManager.instance.Publish(
                                    Notification.Topic.BuildService,
                                    Notification.Severity.Error, message);
                              }
                            }
                          } else {
                            var message =
                                L10n.Tr(k_MessageLaunchedBuildFailure);
                            Debug.LogError(message);
                            NotificationManager.instance.Publish(
                                Notification.Topic.BuildService,
                                Notification.Severity.Error, message);
      ...
      void GetApiStatus() {
        ServicesConfiguration.instance.RequestCloudBuildApiStatusUrl(
            cloudBuildApiStatusUrl => {
              var getCurrentProjectStatusRequest = new UnityWebRequest(
                  cloudBuildApiStatusUrl, UnityWebRequest.kHttpVerbGET){
                  downloadHandler = new DownloadHandlerBuffer()};
              getCurrentProjectStatusRequest.suppressErrorsToConsole = true;
              getCurrentProjectStatusRequest.SetRequestHeader(
                  "AUTHORIZATION",
                  $"Bearer {UnityConnect.instance.GetUserInfo().accessToken}");
      ...
      void GetApiStatusRequestOnCompleted(AsyncOperation obj) {
        ...
        try {
          if (ServicesUtils.IsUnityWebRequestReadyForJsonExtract(
                  m_Provider.m_GetApiStatusRequest)) {
            try {
              var jsonParser = new JSONParser(
                  m_Provider.m_GetApiStatusRequest.downloadHandler.text);
              var json = jsonParser.Parse();
              ...
              if (notificationEntryList.Count > 0) {
                foreach (var jsonNotificationEntry in notificationEntryList) {
                  ...
                  var notificationText = notificationEntry [k_JsonNodeNameText]
                                             .AsString();
                  var billingPlan = string.Empty;
                  if (notificationEntry.ContainsKey(
                          k_JsonNodeNameBillingPlan)) {
                    billingPlan = notificationEntry [k_JsonNodeNameBillingPlan]
                                      .AsString();
                  }

                  var notificationAlertType = notificationEntry
                                              [k_JsonNodeNameAlertType]
                                                  .AsString()
                                                  .ToLower();

                  if (string.IsNullOrEmpty(billingPlan) ||
                      m_BillingPlanLabel.ToLower().Equals(
                          billingPlan.ToLower())) {
                    var severity = Notification.Severity.Error;
                    if (notificationAlertType.Equals(
                            Notification.Severity.Warning.ToString()
                                .ToLower())) {
                      severity = Notification.Severity.Warning;
                    } else if (notificationAlertType.Equals(
                    ...
                    NotificationManager.instance.Publish(
                        Notification.Topic.BuildService, severity,
                        notificationText);
                  }
                }
              }
      ...
      SimpleStateMachine<CloudBuildEvent>.State
      HandleUnbinding(CloudBuildEvent raisedEvent) {
