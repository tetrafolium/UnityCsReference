...
namespace UnityEditor.Connect {
  /// <summary>
  /// The In App Purchasing section of the CloudServices project settings
  /// (actually simply called Services in the ProjectSettings) Uses a simple
  ...
  class PurchasingProjectSettings : ServicesProjectSettings {
    ...
    const string k_PurchasingServicesTemplatePath =
        "UXML/ServicesWindow/PurchasingProjectSettings.uxml";
    ...
    const string k_KeywordGooglePlay =
        "Google Play"; // So devs can find where to place their Google Play
                       // Public Key
    const string k_KeywordPublicKey =
        "public key"; // So devs can find where to place their Google Play
                      // Public Key

    const string k_GooglePlayKeyBtnUpdateLabel = "Update";
    const string k_GooglePlayKeyBtnVerifyLabel = "Verify";

    const string k_PurchasingPermissionMessage =
        "You do not have sufficient permissions to enable / disable Purchasing service.";
    ...
    public static SettingsProvider CreateServicesProvider() {
      return new PurchasingProjectSettings(
          k_ProjectSettingsPath, SettingsScope.Project,
          new List<string>(){
              L10n.Tr(k_KeywordPurchasing), L10n.Tr(k_KeywordInApp),
              L10n.Tr(k_KeywordPurchase), L10n.Tr(k_KeywordRevenue),
              L10n.Tr(k_KeywordPlatforms), L10n.Tr(k_KeywordGooglePlay),
    ...
    public PurchasingProjectSettings(string path, SettingsScope scopes,
      ...
      if (unityToggle != null) {
        ...
        if (!enable) {
          ...
          NotificationManager.instance.Publish(
              Notification.Topic.PurchasingService,
              Notification.Severity.Warning, k_PurchasingPermissionMessage);
        }
      }
    }
    ...
    protected override void ActivateAction(string searchContext) {
      ...
      var mainTemplate = EditorGUIUtility.Load(k_PurchasingServicesTemplatePath)
                             as VisualTreeAsset;
      ...
      if (m_GoToDashboard != null) {
        var clickable = new Clickable(() => {
          ServicesConfiguration.instance.RequestBasePurchasingDashboardUrl(
              OpenDashboardForProjectGuid);
        });
    ...
    void InitializeServiceCallbacks() {
      if (!m_CallbacksInitialized) {
        ...
        PurchasingService.instance.serviceAfterEnableEvent +=
            EnableOperationCompleted;
        PurchasingService.instance.serviceAfterDisableEvent +=
            DisableOperationCompleted;
    ...
    class BasePurchasingState
        : GenericBaseState<PurchasingProjectSettings, ServiceEvent> {
      ...
      protected const string k_SupportedPlatformsBlock =
          "SupportedPlatformsBlock";

      protected BasePurchasingState(
          string stateName, SimpleStateMachine<ServiceEvent> stateMachine,
          PurchasingProjectSettings provider)
          : base(stateName, stateMachine, provider) {}

      protected void LoadTemplateIntoScrollContainer(string templatePath) {
        var generalTemplate =
            EditorGUIUtility.Load(templatePath) as VisualTreeAsset;
        var rootElement = provider.rootVisualElement;
        if (rootElement != null) {
          var scrollContainer =
              provider.rootVisualElement.Q(className
                                           : k_ScrollContainerClass);
    ...
    sealed class DisabledState : BasePurchasingState {
      const string k_TemplatePath =
          "UXML/ServicesWindow/PurchasingProjectSettingsStateDisabled.uxml";

      public DisabledState(SimpleStateMachine<ServiceEvent> stateMachine,
                           PurchasingProjectSettings provider)
      ...
      public override void EnterState() {
        ...
        var scrollContainer =
            provider.rootVisualElement.Q(className
                                         : k_ScrollContainerClass);
        scrollContainer.Add(ServicesUtils.SetupSupportedPlatformsBlock(
            GetSupportedPlatforms()));

        provider.UpdateServiceToggleAndDashboardLink(
            provider.serviceInstance.IsServiceEnabled());

        provider.HandlePermissionRestrictedControls();
      }

      SimpleStateMachine<ServiceEvent>.State
      HandleEnabling(ServiceEvent raisedEvent) {
    ...
    sealed class EnabledState : BasePurchasingState {
      const string k_TemplatePath =
          "UXML/ServicesWindow/PurchasingProjectSettingsStateEnabled.uxml";
      ...
      // Popup Messages
      const string k_PackageMigrationHeadsup =
          "You are about to migrate to the more modern {0}. This will modify your project files. Be sure to make a backup first.\nDo you want to continue?";

      // Notification Messages
      const string k_AuthSignatureExceptionMessage =
          "Exception occurred trying to obtain authentication signature for project {0} and was not handled. Message: {1}";
      const string k_KeyParsingExceptionMessage =
          "Exception occurred trying to parse Google Play Key and was not handled. Message: {0}";

      string packageMigrationHeadsup {
        get;
        set;
      }
      ...
      enum ImportState { Unimported, VersionCheck, UpToDate, OutOfDate }
      ...
      public EnabledState(SimpleStateMachine<ServiceEvent> stateMachine,
                          PurchasingProjectSettings provider)
          : base(k_StateNameEnabled, stateMachine, provider) {
        topicForNotifications = Notification.Topic.PurchasingService;
        notLatestPackageInstalledInfo = string.Format(
            k_NotLatestPackageInstalledInfo, k_PurchasingPackageName);
        packageInstallationHeadsup = string.Format(k_PackageInstallationHeadsup,
                                                   k_PurchasingPackageName);
        packageMigrationHeadsup =
            string.Format(k_PackageMigrationHeadsup, k_PurchasingPackageName);
        duplicateInstallWarning = null;
        packageInstallationDialogTitle = string.Format(
            k_PackageInstallationDialogTitle, k_PurchasingPackageName);
      ...
      void SetupWelcomeIapBlock() {
        ...
        VisualElement welcomeToIapBlock =
            provider.rootVisualElement.Q(k_WelcomeToIapBlock);
        var bulletContainer = welcomeToIapBlock.Q(className
                                                  : k_BulletContainerClass);
      ...
      void SetupImportIapBlock() {
        Action requestLambda = () => { RequestImportOperation(); };
        ...
        // The check for a newer version should be done only when the service is
        // enabled, and at least once when entering the state ...
        PurchasingService.instance.RequestNotifyOnVersionCheck(
            OnVersionCheckComplete);
        PurchasingService.instance.GetLatestETag(
            PurchasingService.instance.OnGetLatestETag);
      }

      // Begin Migrate Block
      void SetupMigrateIapBlock() {
        m_MigrateIapBlock.Q<Button>(k_MigrateBtn).clicked +=
            InstallMigratePackage;

        m_MigratePackageVersionLabel =
            m_MigrateIapBlock.Q<Label>(k_MigrateVersionInfo);
        // Make sure version texts are upper case...
        if (m_MigratePackageVersionLabel != null) {
          m_MigratePackageVersionLabel.text =
              m_MigratePackageVersionLabel.text.ToUpper();
        }
      ...
      void VerifyImportTag() {
        ...
        } else if (PurchasingService.instance.latestETag ==
                   PurchasingService.instance.waitingOnPackage) {
          importState = ImportState.VersionCheck;
        } else if (importTag != PurchasingService.instance.unknownPackage) {
          importState = (importTag == PurchasingService.instance.latestETag)
                            ? ImportState.UpToDate
                            : ImportState.OutOfDate;
        } else {
          importState = ImportState.OutOfDate;
        }

        ToggleImportModeVisibility(m_ImportIapBlock, importState,
                                   m_LookForAssetStoreImport);
      ...
      void ToggleImportModeVisibility(VisualElement fieldBlock,
                                      ImportState importState, bool canImport) {
        if (fieldBlock != null) {
          if (canImport) {
            ...
            if (unimportedMode != null) {
              unimportedMode.style.display =
                  (importState == ImportState.Unimported) ? DisplayStyle.Flex
                                                          : DisplayStyle.None;
            }

            var versionCheckMode = fieldBlock.Q(k_VersionCheckMode);
            if (versionCheckMode != null) {
              versionCheckMode.style.display =
                  (importState == ImportState.VersionCheck) ? DisplayStyle.Flex
                                                            : DisplayStyle.None;
            }
            ...
            if (outOfDateMode != null) {
              outOfDateMode.style.display =
                  (importState == ImportState.OutOfDate) ? DisplayStyle.Flex
                                                         : DisplayStyle.None;
            }
          } else {
      ...
      void ToggleMigrateModeVisibility(VisualElement fieldBlock,
                                       bool canMigrate) {
        if (fieldBlock != null) {
          fieldBlock.style.display =
              canMigrate ? DisplayStyle.Flex : DisplayStyle.None;
        }
      }
      ...
      OnSearchPackageFound(PackageManager.PackageInfo package) {
        ...
        m_LatestPreMigrationPackageVersion = string.Empty;
        if (TryGetMajorVersion(currentPackageVersion,
                               out var currentMajorVer)) {
                ...
                if (TryGetMajorVersion(version, out var majorVer)) {
                  if (majorVer <= 2) {
                    if (string.IsNullOrEmpty(
                            m_LatestPreMigrationPackageVersion)) {
                      m_LatestPreMigrationPackageVersion = version;
                      break; // Should be the last version we need to test for,
                             // given Reverse order. Move if algorithm changes.
                    }
                  } else if (majorVer >= 3) {
      ...
      protected override void PackageInformationUpdated() {
        ...
        if (packmanPackageInstalled &&
            TryGetMajorVersion(currentPackageVersion,
                               out var currentMajorVer)) {
          if (currentMajorVer <= 2) {
      ...
      bool IsPreviewVersion(string versionName) {
        var previewTag =
            "preview"; // TODO: update to an array if more nomenclature exists.
      ...
      void OnImportComplete() { VerifyImportTag(); }

      void OnVersionCheckComplete() { VerifyImportTag(); }
      ...
      void SetupIapOptionsBlock() {
        ...
        m_IapOptionsBlock.Q<Button>(k_UpdateGooglePlayKeyBtn).clicked +=
            RequestUpdateOperation;

        m_IapOptionsBlock.Q<Button>(k_GooglePlayLink).clicked += () => {
          Application.OpenURL(
              PurchasingConfiguration.instance.googlePlayDevConsoleUrl);
        };
      }

      void ToggleGoogleKeyStateVisibility(VisualElement fieldBlock,
                                          GooglePlayKeyState importState) {
        if (fieldBlock != null) {
          var verifiedMode = fieldBlock.Q(k_VerifiedMode);
          if (verifiedMode != null) {
            var updateGooglePlayKeyBtn =
                m_IapOptionsBlock.Q<Button>(k_UpdateGooglePlayKeyBtn);
            if (importState == GooglePlayKeyState.Verified) {
              if (updateGooglePlayKeyBtn != null) {
                updateGooglePlayKeyBtn.text =
                    L10n.Tr(k_GooglePlayKeyBtnUpdateLabel);
              }
              verifiedMode.style.display = DisplayStyle.Flex;
            } else {
              if (updateGooglePlayKeyBtn != null) {
                updateGooglePlayKeyBtn.text =
                    L10n.Tr(k_GooglePlayKeyBtnVerifyLabel);
              }
          ...
          if (unVerifiedMode != null) {
            unVerifiedMode.style.display =
                (importState == GooglePlayKeyState.Verified)
                    ? DisplayStyle.None
                    : DisplayStyle.Flex;

            var badFormatError = fieldBlock.Q(k_ErrorKeyFormat);
            if (badFormatError != null) {
              badFormatError.style.display =
                  (importState == GooglePlayKeyState.InvalidFormat)
                      ? DisplayStyle.Flex
                      : DisplayStyle.None;
            }
            ...
            if (unauthUserError != null) {
              unauthUserError.style.display =
                  (importState == GooglePlayKeyState.UnauthorizedUser)
                      ? DisplayStyle.Flex
                      : DisplayStyle.None;
            }
            ...
            if (serverError != null) {
              serverError.style.display =
                  (importState == GooglePlayKeyState.ServerError)
                      ? DisplayStyle.Flex
                      : DisplayStyle.None;
            }
            ...
            if (cantFetchError != null) {
              cantFetchError.style.display =
                  (importState == GooglePlayKeyState.CantFetch)
                      ? DisplayStyle.Flex
                      : DisplayStyle.None;
            }
          }
      ...
      void RequestAuthSignature() {
        if (m_AuthSignatureRequest == null) {
          PurchasingService.instance.RequestAuthSignature(
              OnGetAuthSignature, out m_AuthSignatureRequest);
        }
      }

      void OnGetAuthSignature(AsyncOperation op) {
        if (op.isDone && (m_AuthSignatureRequest != null) &&
            m_AuthSignatureRequest.downloadHandler.isDone) {
          if ((m_AuthSignatureRequest.result !=
               UnityWebRequest.Result.ProtocolError) &&
              (m_AuthSignatureRequest.result !=
               UnityWebRequest.Result.ConnectionError)) {
            var jsonParser =
                new JSONParser(m_AuthSignatureRequest.downloadHandler.text);
            try {
              var json = jsonParser.Parse();

              m_ProjectAuthSignature = json.AsDict() [k_JsonKeyAuthSignature]
                                           .AsString();
              if (m_SettingKey) {
                SubmitGooglePlayKey();
              } else {
              ...
              NotificationManager.instance.Publish(
                  provider.serviceInstance.notificationTopic,
                  Notification.Severity.Error,
                  string.Format(
                      L10n.Tr(k_AuthSignatureExceptionMessage),
                      Connect.UnityConnect.instance.projectInfo.projectName,
                      ex.Message));
      ...
      void RetrieveGooglePlayKey() {
        PurchasingService.instance.GetGooglePlayKey(OnGetGooglePlayKey,
                                                    m_ProjectAuthSignature);
      }

      void OnGetGooglePlayKey(AsyncOperation op) {
        UnityWebRequestAsyncOperation webOp =
            (UnityWebRequestAsyncOperation) op;

        if (webOp != null && webOp.isDone) {
          UnityWebRequest completedRequest = webOp.webRequest;
          if (completedRequest != null) {
            if ((completedRequest.result !=
                 UnityWebRequest.Result.ProtocolError) &&
                (completedRequest.result !=
                 UnityWebRequest.Result.ConnectionError)) {
              var jsonParser =
                  new JSONParser(completedRequest.downloadHandler.text);
              try {
                var json = jsonParser.Parse();
                var rawKey = json.AsDict()[PurchasingService.instance
                                               .googleKeyJsonLabel];
                ...
                if (!String.IsNullOrEmpty(m_GooglePlayKey)) {
                  ...
                  m_IapOptionsBlock.Q<TextField>(k_GooglePlayKeyEntry)
                      .SetValueWithoutNotify(m_GooglePlayKey);
                } else {
                ...
                NotificationManager.instance.Publish(
                    provider.serviceInstance.notificationTopic,
                    Notification.Severity.Error,
                    string.Format(L10n.Tr(k_KeyParsingExceptionMessage),
                                  ex.Message));
      ...
      void SubmitGooglePlayKey() {
        string keyToSubmit =
            m_IapOptionsBlock.Q<TextField>(k_GooglePlayKeyEntry).text;
        PurchasingService.instance.SubmitGooglePlayKey(
            keyToSubmit, OnSubmitGooglePlayKey, m_ProjectAuthSignature);
      }
        ...
        if (webOp != null) {
          ...
          if (completedRequest != null) {
            if ((completedRequest.result !=
              ...
              verified = true;
              m_GooglePlayKeyState = GooglePlayKeyState.Verified;
            } else if (completedRequest.result ==
                       UnityWebRequest.Result.ProtocolError) {
              if (completedRequest.responseCode == 405 ||
                  completedRequest.responseCode == 500) {
                m_GooglePlayKeyState = GooglePlayKeyState.ServerError;
              } else if (completedRequest.responseCode == 401 ||
                         completedRequest.responseCode == 403) {
          ...
          EditorAnalytics.SendValidatePublicKeyEvent(
              new PublicKeyInfo(){key = "Google Play", success = verified});
        }
      ...
      SimpleStateMachine<ServiceEvent>.State
      HandleDisabling(ServiceEvent raisedEvent) {
