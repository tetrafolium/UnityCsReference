...
namespace UnityEditor.Connect {
  internal class CloudDiagProjectSettings : ServicesProjectSettings {
    ...
    // Related UXML files
    const string k_CloudDiagCommonUxmlPath =
        "UXML/ServicesWindow/CloudDiagProjectSettings.uxml";
    const string k_CloudDiagUserReportUxmlPath =
        "UXML/ServicesWindow/CloudDiagProjectSettingsUserReport.uxml";
    const string k_CloudDiagCrashCommonUxmlPath =
        "UXML/ServicesWindow/CloudDiagProjectSettingsCrash.uxml";
    const string k_CloudDiagCrashEnabledUxmlPath =
        "UXML/ServicesWindow/CloudDiagProjectSettingsCrashEnabled.uxml";
    ...
    const string k_CloudDiagUserReportContainerClassName =
        "cloud-diag-user-report";
    ...
    const string k_CloudDiagPermissionMessage =
        "You do not have sufficient permissions to enable / disable Cloud Diagnostic Crash service.";
    ...
    public static SettingsProvider CreateServicesProvider() {
      return new CloudDiagProjectSettings(
          CrashService.instance.projectSettingsPath, SettingsScope.Project);
    }

    public CloudDiagProjectSettings(string path, SettingsScope scopes,
    ...
    protected override Notification.Topic[] notificationTopicsToSubscribe =>
        new[]{Notification.Topic.CrashService, Notification.Topic.ProjectBind};
    ...
    void SetupServiceToggle() {
      m_CrashServiceToggle.SetProperty(k_ServiceNameProperty,
                                       CrashService.instance.name);
      m_CrashServiceToggle.SetEnabled(false);
      UpdateServiceToggleAndDashboardLink(
          CrashService.instance.IsServiceEnabled());
    ...
    void UpdateServiceToggleAndDashboardLink(bool isEnabled) {
      if (m_CrashServiceGoToDashboard != null) {
        m_CrashServiceGoToDashboard.style.display =
    ...
    ToggleRestrictedVisualElementsAvailability(bool enable) {
      ...
      if (unityToggle != null) {
        ...
        if (!enable) {
          ...
          NotificationManager.instance.Publish(Notification.Topic.CrashService,
    ...
    protected override void ActivateAction(string searchContext) {
      ...
      var mainTemplate =
          EditorGUIUtility.Load(k_CloudDiagCommonUxmlPath) as VisualTreeAsset;
      ...
      if (learnMore != null) {
        var clickable = new Clickable(() => {
          Application.OpenURL(
              ServicesConfiguration.instance.GetUnityCloudDiagnosticInfoUrl());
        });
        learnMore.AddManipulator(clickable);
      }
    }

    protected override void DeactivateAction() { UnregisterEvent(); }

    void RegisterEvent() {
      if (!m_EventHandlerInitialized) {
        CrashService.instance.serviceAfterEnableEvent += ServiceIsEnablingEvent;
        CrashService.instance.serviceAfterDisableEvent +=
    ...
    void SetupCrashDiag() {
      var crashDiagContainer =
          rootVisualElement.Q(className
                              : k_CloudDiagCrashContainerClassName);
      if (crashDiagContainer == null) {
        return;
      }

      var generalTemplate = EditorGUIUtility.Load(
          k_CloudDiagCrashCommonUxmlPath) as VisualTreeAsset;
      if (generalTemplate != null) {
        ...
        crashDiagContainer.Add(ServicesUtils.SetupSupportedPlatformsBlock(
            ServicesUtils.GetCloudDiagCrashSupportedPlatforms()));

        m_CrashServiceGoToDashboard =
            rootVisualElement.Q(k_CloudDiagCrashGoToDashboardName);
        if (m_CrashServiceGoToDashboard != null) {
          var clickable = new Clickable(() => {
            ServicesConfiguration.instance
                .RequestBaseCloudDiagCrashesDashboardUrl(
                    OpenDashboardOrgAndProjectIds);
          });
          m_CrashServiceGoToDashboard.AddManipulator(clickable);
        }

        m_CrashServiceToggle =
    ...
    void SetupUserReport() {
      var userReportContainer =
          rootVisualElement.Q(className
                              : k_CloudDiagUserReportContainerClassName);
      var generalTemplate = EditorGUIUtility.Load(k_CloudDiagUserReportUxmlPath)
                                as VisualTreeAsset;
      if ((generalTemplate != null) && (userReportContainer != null)) {
        ...
        userReportContainer.Add(ServicesUtils.SetupSupportedPlatformsBlock(
            ServicesUtils.GetCloudDiagUserReportSupportedPlatforms()));

        var downloadSdkButtonLink =
            rootVisualElement.Q(k_UserReportingDownloadSdk);
        if (downloadSdkButtonLink != null) {
          var clickable = new Clickable(() => {
            Application.OpenURL(
                ServicesConfiguration.instance
                    .GetUnityCloudDiagnosticUserReportingSdkUrl());
          });
    ...
    sealed class DisabledState : SimpleStateMachine<CloudDiagCrashEvent>.State {
      ...
      public DisabledState(
          SimpleStateMachine<CloudDiagCrashEvent> simpleStateMachine,
          CloudDiagProjectSettings provider)
      ...
      public override void EnterState() {
        var crashContainer =
            m_Provider.rootVisualElement.Q(k_CloudDiagCrashStateName);
      ...
      SimpleStateMachine<CloudDiagCrashEvent>.State
      HandleBinding(CloudDiagCrashEvent raisedEvent) {
    ...
    sealed class EnabledState : SimpleStateMachine<CloudDiagCrashEvent>.State {
      ...
      public override void EnterState() {
        ...
        var generalTemplate = EditorGUIUtility.Load(
            k_CloudDiagCrashEnabledUxmlPath) as VisualTreeAsset;
        var crashContainer =
            m_Provider.rootVisualElement.Q(k_CloudDiagCrashStateName);
        ...
        if ((generalTemplate != null) && (crashContainer != null)) {
          ...
          var capturePlayMode =
              crashContainer.Q<Toggle>(k_CrashCapturePlayMode);
          if (capturePlayMode != null) {
            capturePlayMode.SetValueWithoutNotify(
          ...
          var logBufferSize =
              crashContainer.Q<IntegerField>(k_CrashBufferLogCount);
          if (logBufferSize != null) {
            logBufferSize.SetValueWithoutNotify(
                (int) CrashReportingSettings.logBufferSize);
      ...
      SimpleStateMachine<CloudDiagCrashEvent>.State
      HandleUnbinding(CloudDiagCrashEvent raisedEvent) {
