...
namespace UnityEditor.Connect {
  class AnalyticsProjectSettings : ServicesProjectSettings {
    ...
    const string k_AnalyticsServicesTemplatePath =
        "UXML/ServicesWindow/AnalyticsProjectSettings.uxml";
    ...
    const string k_DataValidationExceptionMessage =
        "Exception occurred trying to validate analytics data for project {0} and was not handled. Message: {1}";
    ...
    const string k_AnalyticsPermissionMessage =
        "You do not have sufficient permissions to enable / disable Analytics service.";
    ...
    public static SettingsProvider CreateServicesProvider() {
      return new AnalyticsProjectSettings(k_ProjectSettingsPath,
    ...
    public AnalyticsProjectSettings(string path, SettingsScope scopes,
    ...
    protected override Notification.Topic[] notificationTopicsToSubscribe =>
        new[]{Notification.Topic.AnalyticsService,
    ...
    ToggleRestrictedVisualElementsAvailability(bool enable) {
      ...
      if (unityToggle != null) {
        ...
        if (!enable) {
          ...
          if (notifications.Any(notification => notification.rawMessage ==
          ...
          NotificationManager.instance.Publish(
              Notification.Topic.AnalyticsService,
              Notification.Severity.Warning, k_AnalyticsPermissionMessage);
        }
      }
    }
    ...
    protected override void ActivateAction(string searchContext) {
      ...
      var mainTemplate = EditorGUIUtility.Load(k_AnalyticsServicesTemplatePath)
                             as VisualTreeAsset;
      ...
      if (m_GoToDashboard != null) {
        var clickable = new Clickable(() => {
          ServicesConfiguration.instance.RequestBaseAnalyticsDashboardUrl(
    ...
    void EnableOperationCompleted(object sender, EventArgs args) {
      m_StateMachine.ProcessEvent(m_BasicDataValidated ? ServiceEvent.Enabled
                                  : ServiceEvent.Integrating);
    }
    ...
    void OnGetValidationData(string downloadedData) {
      ...
      try {
        ...
        basicStatus = json.AsDict() [k_JsonKeyBasic]
                          .AsString();
        customStatus = json.AsDict() [k_JsonKeyCustom]
                           .AsString();
        monetizationStatus = json.AsDict() [k_JsonKeyMonetization]
                                 .AsString();
      } catch (Exception ex) {
        NotificationManager.instance.Publish(
            serviceInstance.notificationTopic, Notification.Severity.Error,
            string.Format(L10n.Tr(k_DataValidationExceptionMessage),
      ...
      m_MonetizationDataIntegrated =
          (monetizationStatus == k_JsonValueCompleted);
    ...
    void RequestAuthSignature() {
      if (m_AuthSignatureRequest == null) {
        AnalyticsService.instance.RequestAuthSignature(
    ...
    void OnGetAuthSignature(AsyncOperation op) {
      if (op.isDone && m_AuthSignatureRequest != null &&
        ...
        if ((m_AuthSignatureRequest.result !=
          ...
          try {
            var json = jsonParser.Parse();
            m_ValidationPoller.Setup(json.AsDict() [k_JsonKeyAuthSignature]
                                         .AsString(),
                                     OnGetValidationData);
          } catch (Exception ex) {
            NotificationManager.instance.Publish(
    ...
    internal enum ServiceEvent {
      ...
      Integrating, // Service toggle is ON, but the dev team has not
                   // setup/interacted with the analytics backend
      Enabled, // Service toggle is ON, and the analytics backend has some basic
               // events to display
    }

    class BaseAnalyticsState
        : GenericBaseState<AnalyticsProjectSettings, ServiceEvent> {
      // Common uss class names
      protected const string k_ScrollContainerClass = "scroll-container";

      protected BaseAnalyticsState(
          string stateName, SimpleStateMachine<ServiceEvent> stateMachine,
          AnalyticsProjectSettings provider)
      ...
      protected void SetupSupportedPlatformsBlock() {
        ...
        scrollContainer.Add(ServicesUtils.SetupSupportedPlatformsBlock(
            ServicesUtils.GetAnalyticsSupportedPlatforms()));
      }
    }

    class DisabledState : BaseAnalyticsState {
      const string k_TemplatePath =
          "UXML/ServicesWindow/AnalyticsProjectSettingsStateDisabled.uxml";
      ...
      void SetupEnableBlock() {
        VisualElement enableBlock =
            provider.rootVisualElement.Q(k_EnableAnalyticsBlock);
      ...
      HandleIntegrating(ServiceEvent raisedEvent) {
    ...
    class IntegrationState : BaseAnalyticsState {
      const string k_TemplatePath =
          "UXML/ServicesWindow/AnalyticsProjectSettingsStateIntegrate.uxml";
      ...
      public IntegrationState(SimpleStateMachine<ServiceEvent> stateMachine,
                              AnalyticsProjectSettings provider)
      ...
      void SetupTroubleShootingBlock() {
        m_TroubleshootingBlock =
            provider.rootVisualElement.Q(k_TroubleshootingBlock);

        if (m_TroubleshootingBlock != null) {
          m_TroubleshootingBlock.Q<Button>(k_TroubleToggle).clicked +=
              ToggleTroubleshooting;

          var accessDashboard =
              m_TroubleshootingBlock.Q(k_AccessAnalyticsDashboardLink);
          if (accessDashboard != null) {
            var clickable = new Clickable(() => {
              ServicesConfiguration.instance.RequestBaseAnalyticsDashboardUrl(
                  provider.OpenDashboardForProjectGuid);
            });
      ...
      void CheckTroubleshootingModeVisibility(VisualElement fieldBlock) {
        if (fieldBlock != null) {
          ...
          if (troubleshootingMode != null) {
            troubleshootingMode.style.display =
                m_HavingTrouble ? DisplayStyle.Flex : DisplayStyle.None;
          }
        }
      }
    ...
    class EnabledState : BaseAnalyticsState {
      const string k_MainTemplatePath =
          "UXML/ServicesWindow/AnalyticsProjectSettingsStateEnabled.uxml";
      const string k_AdditionalEventsTemplatePath =
          "UXML/ServicesWindow/AnalyticsProjectSettingsAdditionalEventsTemplate.uxml";
      ...
      const string k_CustomDesc =
          "Understand player behavior and usage patterns with event data.";

      const string k_MonetizationTitleAndKey = "Monetization";
      const string k_MonetizationDesc =
          "Track in-game revenue and monitor fraudulent transactions.";
      ...
      public EnabledState(SimpleStateMachine<ServiceEvent> stateMachine,
        ...
        notLatestPackageInstalledInfo = string.Format(
            k_NotLatestPackageInstalledInfo, k_AnalyticsPackageName);
        packageInstallationHeadsup =
            string.Format(k_PackageInstallationHeadsup, k_AnalyticsPackageName);
        duplicateInstallWarning = null;
        packageInstallationDialogTitle = string.Format(
            k_PackageInstallationDialogTitle, k_AnalyticsPackageName);
        ...
        m_AdditionalEvents = new Dictionary<string, AdditionalEvent>(){
                [k_CustomKey] =
                    new AdditionalEvent(){
                        title = k_CustomTitle, description = k_CustomDesc,
                        learnUrl =
                            AnalyticsConfiguration.instance.customLearnUrl},
                [ k_MonetizationTitleAndKey ] =
                    new AdditionalEvent(){title = k_MonetizationTitleAndKey,
                                          description = k_MonetizationDesc,
                                          learnUrl =
                                              AnalyticsConfiguration.instance
                                                  .monetizationLearnUrl},
        };
      }

      public override void EnterState() {
        m_AdditionalEvents[k_CustomKey].integrated =
            provider.m_CustomDataIntegrated;
        m_AdditionalEvents[k_MonetizationTitleAndKey].integrated =
            provider.m_MonetizationDataIntegrated;
      ...
      void SetupIapWarningBlock() {
        ...
        if (iapWarningBlock != null) {
          iapWarningBlock.style.display =
              PurchasingService.instance.IsServiceEnabled() ? DisplayStyle.Flex
                                                            : DisplayStyle.None;
        }
      }
      ...
      void SetupAdditionalEventsBlock() {
        var additionalEventsBlock =
            provider.rootVisualElement.Q(k_AdditionalEventsBlock);
        if (additionalEventsBlock != null) {
          additionalEventsBlock.Clear();

          var eventTemplate = EditorGUIUtility.Load(
              k_AdditionalEventsTemplatePath) as VisualTreeAsset;
              ...
              if (eventBlock != null) {
                ...
                if (checkIcon != null) {
                  checkIcon.style.display =
                      additionalEvent.Value.integrated ? DisplayStyle
                          .Flex : DisplayStyle.None;
                }
                var preCheckIcon = eventBlock.Q(className
                                                : k_PreCheckIconClass);
                if (preCheckIcon != null) {
                  preCheckIcon.style.display =
                      !additionalEvent.Value.integrated ? DisplayStyle
                           .Flex : DisplayStyle.None;
                }

                // set text
                eventBlock.Q<Label>(k_AdditionalEventTitle).text =
                    additionalEvent.Value.title;
                eventBlock.Q<Label>(k_AdditionalEventDesc).text =
                    additionalEvent.Value.description;

                // Setup link url
                var learnMore = new Clickable(() => {
                  Application.OpenURL(additionalEvent.Value.learnUrl);
                });
                eventBlock.Q(k_AdditionalEventLearnUrl)
                    .AddManipulator(learnMore);
