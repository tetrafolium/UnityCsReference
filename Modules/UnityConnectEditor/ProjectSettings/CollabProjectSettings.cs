...
namespace UnityEditor.Connect {
  internal class CollabProjectSettings : ServicesProjectSettings {
    ...
    const string k_CollaborateCommonUxmlPath =
        "UXML/ServicesWindow/CollabProjectSettings.uxml";
    const string k_CollaborateDisabledUxmlPath =
        "UXML/ServicesWindow/CollabProjectSettingsDisabled.uxml";
    const string k_CollaborateEnabledUxmlPath =
        "UXML/ServicesWindow/CollabProjectSettingsEnabled.uxml";
    ...
    const string k_CollabPermissionMessage =
        "You do not have sufficient permissions to enable / disable Collaborate service.";
    ...
    public static SettingsProvider CreateServicesProvider() {
      return new CollabProjectSettings(
          CollabService.instance.projectSettingsPath, SettingsScope.Project);
    }
    ...
    public CollabProjectSettings(string path, SettingsScope scopes,
    ...
    protected override Notification.Topic[] notificationTopicsToSubscribe =>
        new[]{Notification.Topic.CollabService, Notification.Topic.ProjectBind};
    ...
    ToggleRestrictedVisualElementsAvailability(bool enable) {
      ...
      if (unityToggle != null) {
        ...
        if (!enable) {
          ...
          NotificationManager.instance.Publish(Notification.Topic.CollabService,
    ...
    void OnCollabStateChanged(CollabInfo info) {
      // Make sure to update the service state based on the information changed
      // in Collab...
    ...
    protected override void ActivateAction(string searchContext) {
      ...
      var mainTemplate =
          EditorGUIUtility.Load(k_CollaborateCommonUxmlPath) as VisualTreeAsset;
      ...
      if (m_GoToDashboard != null) {
        var clickable = new Clickable(() => {
          ServicesConfiguration.instance.RequestBaseCollabDashboardUrl(
      ...
      m_MainServiceToggle =
          rootVisualElement.Q<Toggle>(null, k_ServiceToggleClassName);
    ...
    void RegisterEvent() {
      if (!m_EventHandlerInitialized) {
        ...
        CollabService.instance.serviceAfterEnableEvent +=
            ServiceIsEnablingEvent;
        CollabService.instance.serviceAfterDisableEvent +=
    ...
    sealed class DisabledState
        : GenericBaseState<CollabProjectSettings, CollabEvent> {
      public DisabledState(SimpleStateMachine<CollabEvent> simpleStateMachine,
                           CollabProjectSettings provider)
      ...
      public override void EnterState() {
        var generalTemplate = EditorGUIUtility.Load(
            k_CollaborateDisabledUxmlPath) as VisualTreeAsset;
      ...
      SimpleStateMachine<CollabEvent>.State
      HandleBinding(CollabEvent raisedEvent) {
        return stateMachine.GetStateByName(k_StateNameEnabled);
      }
    }

    // Enabled state of the service
    sealed class EnabledState
        : GenericBaseState<CollabProjectSettings, CollabEvent> {
      VisualElement m_CollabPublishSection;
      VisualElement m_CollabHistorySection;

      public EnabledState(SimpleStateMachine<CollabEvent> simpleStateMachine,
                          CollabProjectSettings provider)
        ...
        notLatestPackageInstalledInfo =
            string.Format(k_NotLatestPackageInstalledInfo, k_CollabPackageName);
        packageInstallationHeadsup =
            string.Format(k_PackageInstallationHeadsup, k_CollabPackageName);
        duplicateInstallWarning = null;
        packageInstallationDialogTitle = string.Format(
            k_PackageInstallationDialogTitle, k_CollabPackageName);
      }
      ...
      public override void EnterState() {
        ...
        var generalTemplate = EditorGUIUtility.Load(
            k_CollaborateEnabledUxmlPath) as VisualTreeAsset;
        var scrollContainer = provider.rootVisualElement.Q(
            null, k_ServiceScrollContainerClassName);
        var newVisual = generalTemplate.CloneTree().contentContainer;
        ServicesUtils.TranslateStringsInTree(newVisual);
        ...
        var openHistory =
            provider.rootVisualElement.Q(k_OpenHistoryLink) as Button;
        if (openHistory != null) {
          openHistory.clicked += () => {
        ...
        Button openChangesLinkBtn =
            provider.rootVisualElement.Q(k_OpenChangesLink) as Button;
        ...
        if (gotoWebDashboard != null) {
          var clickable = new Clickable(() => {
            ServicesConfiguration.instance.RequestBaseCloudUsageDashboardUrl(
                provider.OpenDashboardOrgAndProjectIds);
          });
      ...
      SimpleStateMachine<CollabEvent>.State
      HandleUnbinding(CollabEvent raisedEvent) {
