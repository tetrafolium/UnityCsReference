...
namespace UnityEditor.Connect {
  internal class AdsProjectSettings : ServicesProjectSettings {
    ...
    const string k_AdsCommonUxmlPath =
        "UXML/ServicesWindow/AdsProjectSettings.uxml";
    const string k_AdsDisabledUxmlPath =
        "UXML/ServicesWindow/AdsProjectSettingsDisabled.uxml";
    const string k_AdsEnabledUxmlPath =
        "UXML/ServicesWindow/AdsProjectSettingsEnabled.uxml";
    ...
    const string k_AdsPermissionMessage =
        "You do not have sufficient permissions to enable / disable Ads service.";

    // Asset store GUIDs
    const string k_AdvertisingAndroidAdsDllGuid =
        "cad99f482ce25421196533fe02e6a13e";
    const string k_AdvertisingIosAdsDllGuid =
        "d6f3e2ade30154a80a137e0079f66a08";
    const string k_AdvertisingEditorDllGuid =
        "56921141d53fd4a5888445107b1b1286";

    const string k_AdsAssetStorePackageInstalledWarning =
    ...
    public static SettingsProvider CreateServicesProvider() {
      return new AdsProjectSettings(AdsService.instance.projectSettingsPath,
                                    SettingsScope.Project);
    }

    SimpleStateMachine<AdsEvent> m_StateMachine;
    public AdsProjectSettings(string path, SettingsScope scopes,
    ...
    void OnDestroy() { UnregisterEvent(); }

    protected override Notification.Topic[] notificationTopicsToSubscribe =>
        new[]{Notification.Topic.AdsService, Notification.Topic.ProjectBind,
    ...
    void SetUpGameId() {
      ...
      scrollContainer.Q<TextField>(name
                                   : k_AppleGameIdName)
          ?.SetValueWithoutNotify(appleGameId ?? unavailableGameId);
      scrollContainer.Q<TextField>(name
                                   : k_AndroidGameIdName)
          ?.SetValueWithoutNotify(androidGameId ?? unavailableGameId);
    }
    ...
    ToggleRestrictedVisualElementsAvailability(bool enable) {
      ...
      if (unityToggle != null) {
        ...
        if (!enable) {
          ...
          NotificationManager.instance.Publish(Notification.Topic.AdsService,
    ...
    protected override void ActivateAction(string searchContext) {
      ...
      var mainTemplate =
          EditorGUIUtility.Load(k_AdsCommonUxmlPath) as VisualTreeAsset;
      ...
      if (m_GoToDashboard != null) {
        var clickable = new Clickable(() => {
          OpenDashboardForOrgKeyAndProjectGuid(
              ServicesConfiguration.instance.adsDashboardUrl);
        });
    ...
    void GameIdsUpdatedEvent() { SetUpGameId(); }

    class BaseState : GenericBaseState<AdsProjectSettings, AdsEvent> {
      public BaseState(string name,
                       SimpleStateMachine<AdsEvent> simpleStateMachine,
                       AdsProjectSettings provider)
          : base(name, simpleStateMachine, provider) {}
    }

    // Disabled state of the service
    sealed class DisabledState : BaseState {
      public DisabledState(SimpleStateMachine<AdsEvent> simpleStateMachine,
                           AdsProjectSettings provider)
      ...
      public override void EnterState() {
        var generalTemplate =
            EditorGUIUtility.Load(k_AdsDisabledUxmlPath) as VisualTreeAsset;
        var scrollContainer = provider.rootVisualElement.Q(
            null, k_ServiceScrollContainerClassName);
        ...
        if (generalTemplate != null) {
          ...
          if (gettingStarted != null) {
            var clickable = new Clickable(() => {
              Application.OpenURL(
                  ServicesConfiguration.instance.adsGettingStartedUrl);
            });
        ...
        scrollContainer.Add(ServicesUtils.SetupSupportedPlatformsBlock(
            ServicesUtils.GetAdsSupportedPlatforms()));
    ...
    sealed class EnabledState : BaseState {
      ...
      public EnabledState(SimpleStateMachine<AdsEvent> simpleStateMachine,
                          AdsProjectSettings provider)
        ...
        notLatestPackageInstalledInfo =
            string.Format(k_NotLatestPackageInstalledInfo, k_AdsPackageName);
        packageInstallationHeadsup =
            string.Format(k_PackageInstallationHeadsup, k_AdsPackageName);
        ;
        duplicateInstallWarning = k_DuplicateInstallWarning;
        packageInstallationDialogTitle =
            string.Format(k_PackageInstallationDialogTitle, k_AdsPackageName);
      }

      void VerifyAssetStorePackageInstallation() {
        var assetStoreAndroidDllPath =
            AssetImporter.GetAtPath(
                AssetDatabase.GUIDToAssetPath(k_AdvertisingAndroidAdsDllGuid /* UnityEngine.Advertising.Android.dll */))
                as PluginImporter;
        var assetStoreIosDllPath = AssetImporter.GetAtPath(AssetDatabase.GUIDToAssetPath(
            k_AdvertisingIosAdsDllGuid /* UnityEngine.Advertising.iOS.dll */))
                                       as PluginImporter;
        var assetStoreEditorDllPath =
            AssetImporter
                .GetAtPath(
                    AssetDatabase.GUIDToAssetPath(k_AdvertisingEditorDllGuid /* UnityEngine.Advertising.Editor.dll */))
                    as PluginImporter;

        // Assume it is not installed...
        assetStorePackageInstalled = false;

        if ((assetStoreAndroidDllPath != null) ||
            (assetStoreIosDllPath != null) ||
            (assetStoreEditorDllPath != null)) {
      ...
      public override void EnterState() {
        ...
        if (assetStorePackageInstalled && !m_AssetStoreWarningHasBeenShown) {
          NotificationManager.instance.Publish(
              Notification.Topic.AdsService, Notification.Severity.Warning,
              L10n.Tr(k_AdsAssetStorePackageInstalledWarning));
          m_AssetStoreWarningHasBeenShown = true;
        }

        // If we haven't received new bound info, fetch them
        var generalTemplate =
            EditorGUIUtility.Load(k_AdsEnabledUxmlPath) as VisualTreeAsset;
        var scrollContainer = provider.rootVisualElement.Q(
            null, k_ServiceScrollContainerClassName);
        if (generalTemplate != null) {
          var newVisual = generalTemplate.CloneTree().contentContainer;
          ...
          if (learnMore != null) {
            var clickable = new Clickable(() => {
              Application.OpenURL(
                  ServicesConfiguration.instance.adsLearnMoreUrl);
            });
            learnMore.AddManipulator(clickable);
          }
          var toggleTestMode = scrollContainer.Q<Toggle>(k_ToggleTestModeName);
          if (toggleTestMode != null) {
            toggleTestMode.SetValueWithoutNotify(
                AdvertisementSettings.testMode);
            toggleTestMode.RegisterValueChangedCallback(
                evt => { AdvertisementSettings.testMode = evt.newValue; });
          }
