...
namespace UnityEditor.Connect {
  internal class ServicesEditorWindow : EditorWindow {
    const string k_PrivacyPolicyUrl =
        "https://unity3d.com/legal/privacy-policy";
    const string k_ServicesWindowUxmlPath =
        "UXML/ServicesWindow/ServicesWindow.uxml";
    const string k_ServiceTemplateUxmlPath =
        "UXML/ServicesWindow/ServiceTemplate.uxml";
    ...
    const string k_ServicePackageInstallContainerClassName =
        "service-package-install-container";
    ...
    const string k_ProjectNotBoundMessage =
        "Project is not bound. Either create a new project Id or bind your project to an existing project Id for the dashboard link to become available.";
    const string k_ConnectionFailedMessage =
        "Failed to connect to Services. Services are not reachable right now. Please try again later.";

    Dictionary<string, Label> m_StatusLabelByServiceName =
        new Dictionary<string, Label>();
    Dictionary<string, Clickable> m_ClickableByServiceName =
        new Dictionary<string, Clickable>();
    ...
    public UIElementsNotificationSubscriber notificationSubscriber {
      get;
      private set;
    }

    private struct CloseServiceWindowState { public int availableServices; }
    ...
    internal static void ShowServicesWindow() {
      ...
      EditorAnalytics.SendEventShowService(
          new ServicesProjectSettings.ShowServiceState(){
              service = k_WindowTitle, page = "",
              referrer = "window_menu_item"});
    }
    ...
    void OnCollabStateChanged(CollabInfo info) {
      if (CollabService.instance.IsServiceEnabled() !=
          Collab.instance.IsCollabEnabledForCurrentProject()) {
        CollabService.instance.EnableService(
            Collab.instance.IsCollabEnabledForCurrentProject());
      }
    }
    ...
    public void OnEnable() {
      ...
      // Collab is a specific case where the service can be enabled inside
      // another Editor Window by the Collab package code itself.
    ...
    void LoadWindow() {
      ...
      var mainTemplate =
          EditorGUIUtility.Load(k_ServicesWindowUxmlPath) as VisualTreeAsset;
      ...
      mainTemplate.CloneTree(rootVisualElement,
                             new Dictionary<string, VisualElement>(), null);
      notificationSubscriber =
          new UIElementsNotificationSubscriber(rootVisualElement);
      notificationSubscriber.Subscribe(
          Notification.Topic.AdsService, Notification.Topic.AnalyticsService,
          Notification.Topic.BuildService, Notification.Topic.CollabService,
          Notification.Topic.CoppaCompliance, Notification.Topic.CrashService,
          Notification.Topic.ProjectBind, Notification.Topic.PurchasingService);

      rootVisualElement.Q<Button>(k_ProjectSettingsBtnName).clicked += () => {
        ServicesUtils.OpenServicesProjectSettings(
            GeneralProjectSettings.generalProjectSettingsPath,
            typeof(GeneralProjectSettings).Name);
      };

      var dashboardClickable = new Clickable(() => {
        if (UnityConnect.instance.projectInfo.projectBound) {
          ServicesConfiguration.instance.RequestBaseDashboardUrl(
              baseDashboardUrl => {
                EditorAnalytics.SendOpenDashboardForService(
                    new ServicesProjectSettings.OpenDashboardForService(){
                        serviceName = k_WindowTitle, url = baseDashboardUrl,
                        organizationId =
                ...
                ServicesConfiguration.instance
                    .RequestCurrentProjectDashboardUrl(Application.OpenURL);
              });
        } else {
          NotificationManager.instance.Publish(
              Notification.Topic.ProjectBind, Notification.Severity.Warning,
              L10n.Tr(k_ProjectNotBoundMessage));
          ServicesUtils.OpenServicesProjectSettings(
              GeneralProjectSettings.generalProjectSettingsPath,
              typeof(GeneralProjectSettings).Name);
        }
      });
      rootVisualElement.Q(k_DashboardLinkName)
          .AddManipulator(dashboardClickable);
    ...
    void FinalizeServiceSetup() {
      var scrollContainer = rootVisualElement.Q(className
                                                : k_ScrollContainerClassName);
      var serviceTemplate =
          EditorGUIUtility.Load(k_ServiceTemplateUxmlPath) as VisualTreeAsset;
      ...
      var privacyClickable =
          new Clickable(() => { Application.OpenURL(k_PrivacyPolicyUrl); });
      scrollContainer.Q(k_PrivacyPolicyLinkName)
          .AddManipulator(privacyClickable);
    }

    void SetupService(VisualElement scrollContainer,
                      VisualTreeAsset serviceTemplate,
                      SingleService singleService) {
      scrollContainer.Add(serviceTemplate.CloneTree().contentContainer);
      var serviceIconAsset =
          EditorGUIUtility.Load(singleService.pathTowardIcon) as Texture2D;
      var serviceRoot = scrollContainer[scrollContainer.childCount - 1];
      serviceRoot.name = singleService.name;

      var serviceTitle = serviceRoot.Q<TextElement>(className
                                                    : k_ServiceTitleClassName);
      serviceTitle.text = singleService.title;
      serviceRoot.Q<TextElement>(className
                                 : k_ServiceDescriptionClassName)
          .text = singleService.description;
      serviceRoot.Q(className
                    : k_ServiceIconClassName)
          .style.backgroundImage = serviceIconAsset;

      Action openServiceSettingsLambda =
          () => { ServicesUtils.OpenServicesProjectSettings(singleService); };
      m_ClickableByServiceName.Add(singleService.name,
    ...
    void SetupPackageInstall(VisualElement serviceRoot,
                             SingleService singleService) {
      serviceRoot.AddManipulator(m_ClickableByServiceName[singleService.name]);

      var installButton =
          serviceRoot.Q<Button>(className
                                : k_ServicePackageInstallClassName);
      var installButtonContainer =
          serviceRoot.Q(className
                        : k_ServicePackageInstallContainerClassName);
      installButtonContainer.style.display = DisplayStyle.None;

      if (singleService.isPackage && (singleService.packageName != null) &&
          (m_PackageCollection != null)) {
        ...
        foreach (var info in m_PackageCollection) {
          if (info.name.Equals(singleService.packageName)) {
            packageFound = true;
            SetServiceToInstalledState(installButton, serviceRoot,
                                       singleService);
            break;
          }
        }
        ...
        if (!packageFound) {
          SetServiceToUninstalledState(installButton, serviceRoot,
                                       singleService);
          ...
          installButton.clicked += () => {
            ...
            EditorAnalytics.SendOpenPackManFromServiceSettings(
                new ServicesProjectSettings.OpenPackageManager(){
    ...
    void SetServiceToUninstalledState(Button installButton,
                                      VisualElement serviceRoot,
                                      SingleService singleService) {
      installButton.style.display = DisplayStyle.Flex;
      serviceRoot.RemoveManipulator(
          m_ClickableByServiceName[singleService.name]);
      serviceRoot [0]
          .RemoveFromClassList(k_EntryClassName);
      serviceRoot [0]
          .AddToClassList(k_UninstalledEntryClassName);
      if (singleService.displayToggle) {
        m_StatusLabelByServiceName[singleService.name].style.display =
            DisplayStyle.None;
      }
    }

    void SetServiceToInstalledState(Button installButton,
                                    VisualElement serviceRoot,
                                    SingleService singleService) {
      installButton.style.display = DisplayStyle.None;
      serviceRoot.AddManipulator(m_ClickableByServiceName[singleService.name]);
      serviceRoot [0]
          .RemoveFromClassList(k_UninstalledEntryClassName);
      serviceRoot [0]
          .AddToClassList(k_EntryClassName);
      if (singleService.displayToggle) {
        SetServiceStatusValue(singleService.name,
                              singleService.IsServiceEnabled());
        m_StatusLabelByServiceName[singleService.name].style.display =
            DisplayStyle.Flex;
      }
    }
    ...
    void SetupServiceStatusLabel(VisualElement serviceRoot,
                                 SingleService singleService) {
      var statusText = serviceRoot.Q<Label>(className
                                            : k_ServiceStatusClassName);
    ...
    public void SetServiceStatusValue(string serviceName, bool active) {
      if (m_StatusLabelByServiceName.ContainsKey(serviceName)) {
        m_StatusLabelByServiceName[serviceName].text =
            active ? L10n.Tr("ON") : L10n.Tr("OFF");
        if (active) {
          m_StatusLabelByServiceName [serviceName]
              .AddToClassList(k_ServiceStatusCheckedClassName);
        } else {
          m_StatusLabelByServiceName [serviceName]
              .RemoveFromClassList(k_ServiceStatusCheckedClassName);
        }
      }
    }
  }
