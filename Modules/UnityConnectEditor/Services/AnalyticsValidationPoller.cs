...
namespace UnityEditor.Connect {
  internal class AnalyticsValidationPoller {
    ...
    readonly TickTimerHelper m_Ticker =
        new TickTimerHelper(k_TickIntervalSeconds);
    ...
    public bool IsReady() { return m_ProjectAuthSignature != null; }
    ...
    // Can be called independently without Start/Stopping, however will not
    // adjust tick interval (it's private)
    public void Poll() {
      if (IsReady() && m_DataValidationRequest == null) {
        AnalyticsService.instance.RequestValidationData(
            OnPolled, m_ProjectAuthSignature, out m_DataValidationRequest);
      }
    }
    ...
    private void OnPolled(AsyncOperation op) {
      if (op.isDone) {
        if (m_DataValidationRequest != null) {
          if ((m_DataValidationRequest.result !=
               UnityWebRequest.Result.ProtocolError) &&
              (m_DataValidationRequest.result !=
