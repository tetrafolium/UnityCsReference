...
namespace UnityEditor.Connect {
  ...
  internal class PurchasingService : SingleService {
    ...
    public override string settingsProviderClassName =>
        nameof(PurchasingProjectSettings);
    ...
    static PurchasingService() { k_Instance = new PurchasingService(); }

    PurchasingService() {
      k_PackageUri =
          new Uri(PurchasingConfiguration.instance.purchasingPackageUrl);

      name = "Purchasing";
      title = L10n.Tr("In-App Purchasing");
      description = L10n.Tr("Simplify cross-platform IAP");
      pathTowardIcon =
          @"Builtin Skins\Shared\Images\ServicesWindow-ServiceIcon-Purchasing.png";
    ...
    protected override void InternalEnableService(bool enable,
      ...
      if (PurchasingSettings.enabled != enable) {
        ...
        EditorAnalytics.SendEventServiceInfo(
            new PurchasingServiceState(){iap = enable});
    ...
    public void InstallUnityPackage(Action onImport) {
      ...
      // Only OSX supports SSL certificate validation, disable checking on other
      // platforms.
      ...
      UnityWebRequest request =
          new UnityWebRequest(k_PackageUri, UnityWebRequest.kHttpVerbGET);
      ...
      operation.completed += (asyncOp) => {
        ...
        handler = () => {
          ...
          if ((request.result != UnityWebRequest.Result.ProtocolError) &&
            ...
            EditorAnalytics.SendImportServicePackageEvent(
                new ImportPackageInfo(){
                    packageName = Path.GetFileName(k_PackageUri.ToString()),
                    eTag = etag});

            // TODO: ImportPackage is a delayeed operation with no callback. See
            // if we can get a confirmation of successful installation
            //      before firing our callback, or even before Saving the ETag.
            onImport();
          } else {
            UnityEngine.Debug.LogError(
                L10n.Tr(
                    "Failed to download IAP package. Please check connectivity and retry. Web request Error: ") +
                request.error);
          }
        };
        EditorApplication.update += handler;
    ...
    internal void OnGetLatestETag(AsyncOperation op) {
      ...
      if (webOp != null) {
        string eTagHeader = ((webOp.webRequest.result !=
                              UnityWebRequest.Result.ProtocolError) &&
                             (webOp.webRequest.result !=
    ...
    public void RequestAuthSignature(Action<AsyncOperation> onGet,
                                     out UnityWebRequest request) {
      request = UnityWebRequest.Get(
          String.Format(AnalyticsConfiguration.instance.coreProjectsUrl,
    ...
    public void GetGooglePlayKey(Action<AsyncOperation> onGet,
                                 string authSignature) {
      var request =
          UnityWebRequest.Get(GetGoogleKeyResource() + k_GoogleKeyGetSuffix);
      request.suppressErrorsToConsole = true;

      var encodedAuthToken = ServicesUtils.Base64Encode((
          UnityConnect.instance.projectInfo.projectGUID + ":" + authSignature));
    ...
    public void SubmitGooglePlayKey(string submittedKey,
                                    Action<AsyncOperation> onSubmit,
                                    string authSignature) {
      var payload =
          "{\"" + k_GoogleKeyJsonLabel + "\": \"" + submittedKey + "\"}";
      var uploadHandler = new UploadHandlerRaw(Encoding.UTF8.GetBytes(payload));
      var request = new UnityWebRequest(
          GetGoogleKeyResource() + k_GoogleKeyPostSuffix,
          UnityWebRequest.kHttpVerbPOST){uploadHandler = uploadHandler};
      request.suppressErrorsToConsole = true;
    ...
    private string GetGoogleKeyResource() {
      return PurchasingConfiguration.instance.analyticsApiUrl +
             k_GoogleKeySubPath + UnityConnect.instance.projectInfo.projectGUID;
    }
  }
}
