...
namespace UnityEditor.Connect {
  ...
  internal class UDPService : SingleService {
    ...
    static UDPService() { k_Instance = new UDPService(); }

    UDPService() {
      name = "UDP";
      title = L10n.Tr("Unity Distribution Portal");
      description =
          L10n.Tr("Distribute to multiple app stores through a single hub.");
      pathTowardIcon =
          @"Builtin Skins\Shared\Images\ServicesWindow-ServiceIcon-UDP.png";
      projectSettingsPath = "Project/Services/Unity Distribution Portal";
      displayToggle = false;
      isPackage = true;
      packageName = "com.unity.purchasing.udp";
      // Due to really bad user experience, this service cannot be included in
      // the services window. See fogbugz case 1215216, with this entry removed
      // the user experience is not worst then on previous Unity Version
