...
namespace UnityEditor.Connect {
  ...
  internal class CloudBuildPoller {
    ...
    static CloudBuildPoller() { k_Instance = new CloudBuildPoller(); }
    ...
    void Update() {
      if (m_Timer.DoTick()) {
        var getCurrentBuildTargetStatusRequest =
            new UnityWebRequest(m_PollingUrl, UnityWebRequest.kHttpVerbGET){
                downloadHandler = new DownloadHandlerBuffer()};
        getCurrentBuildTargetStatusRequest.suppressErrorsToConsole = true;
        getCurrentBuildTargetStatusRequest.SetRequestHeader(
            "AUTHORIZATION",
        ...
        operation.completed += asyncOperation => {
          try {
            if (ServicesUtils.IsUnityWebRequestReadyForJsonExtract(
                    getCurrentBuildTargetStatusRequest)) {
              try {
                var jsonParser = new JSONParser(
                    getCurrentBuildTargetStatusRequest.downloadHandler.text);
                ...
                if (buildList.Count > 0) {
                  foreach (var rawBuild in buildList) {
                    ...
                    var buildNumber = build [k_JsonNodeNameBuild]
                                          .AsFloat()
                                          .ToString();
                    var buildId = build [k_JsonNodeNameBuildTargetId]
                                      .AsString() +
                                  "_" + buildNumber;
                    var buildStatus = build [k_JsonNodeNameBuildStatus]
                                          .AsString()
                                          .ToLower();
                    ...
                    if (m_BuildsToReportOn.Contains(buildId) &&
                      ...
                      var buildTargetName = build
                                            [k_JsonNodeNameBuildTargetName]
                                                .AsString();
                      ...
                      case k_BuildStatusCanceled:
                        ...
                        message = string.Format(
                            L10n.Tr(k_BuildFinishedWithStatusMsg), buildNumber,
                            buildTargetName, k_BuildStatusCanceled);
                      ...
                      case k_BuildStatusFailure:
                        ...
                        message = string.Format(
                            L10n.Tr(k_BuildFinishedWithStatusMsg), buildNumber,
                            buildTargetName, k_BuildStatusFailure);
                        Debug.LogError(message);
                        break;
                      case k_BuildStatusStarted:
                        message = string.Format(
                            L10n.Tr(k_BuildFinishedWithStatusMsg), buildNumber,
                            buildTargetName, k_BuildStatusStartedMessage);
                        Debug.Log(message);
                        break;
                      case k_BuildStatusSuccess:
                        message = string.Format(
                            L10n.Tr(k_BuildFinishedWithStatusMsg), buildNumber,
                            buildTargetName, k_BuildStatusSuccess);
                        Debug.Log(message);
                        break;
                      case k_BuildStatusUnknown:
                        message = string.Format(
                            L10n.Tr(k_BuildFinishedWithStatusMsg), buildNumber,
                            buildTargetName, k_BuildStatusUnknown);
                        Debug.LogWarning(message);
                        break;
                      }

                      NotificationManager.instance.Publish(
                          Notification.Topic.BuildService, severity, message);
                    } else if (!m_BuildsToReportOn.Contains(buildId) &&
                               (k_BuildStatusQueued.Equals(buildStatus) ||
                                k_BuildStatusStarted.Equals(buildStatus) ||
                                k_BuildStatusSentToBuilder.Equals(
                                    buildStatus) ||
                                k_BuildStatusSentRestarted.Equals(
                                    buildStatus))) {
                      if (k_BuildStatusSentRestarted.Equals(buildStatus)) {
                        ...
                        var message = string.Format(
                            L10n.Tr(k_BuildFinishedWithStatusMsg), buildNumber,
                            buildTargetName, k_BuildStatusSentRestarted);
                        Debug.Log(message);
                        NotificationManager.instance.Publish(
