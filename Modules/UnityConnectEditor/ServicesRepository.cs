...
namespace UnityEditor.Connect {
  ...
  internal class ServicesRepository {
    ...
    static readonly Dictionary<string, SingleService> k_Services =
        new Dictionary<string, SingleService>();
    ...
    const string k_NotInitializedMsg =
        "Service {0} was not initialized. See the console for more details.";
          ...
          if (!s_InitializedServices.Contains(serviceKey)) {
            ...
            try {
              ...
              NotificationManager.instance.Publish(
                  service.notificationTopic, Notification.Severity.Error,
                  string.Format(L10n.Tr(k_NotInitializedMsg),
                                L10n.Tr(service.title)));
    ...
    public static void DisableAllServices(bool shouldUpdateApiFlag) {
      GetServices().ForEach(
          service => service.EnableService(false, shouldUpdateApiFlag));
    }
    ...
    public static void SyncServicesOnProjectRebind() {
      k_Instance.RequestServicesApiFlags(flags => {
        if (flags != null) {
          GetServices().ForEach(service => {
            if (service.shouldSyncOnProjectRebind &&
                flags.ContainsKey(service.serviceFlagName)) {
    ...
    private void
    RequestServicesApiFlags(Action<Dictionary<string, bool>> callback) {
      ...
      ServicesConfiguration.instance.RequestCurrentProjectApiUrl(
          currentProjectApiUrl => {
            m_ServiceRequest = new UnityWebRequest(
                currentProjectApiUrl, UnityWebRequest.kHttpVerbGET){
                downloadHandler = new DownloadHandlerBuffer()};
            m_ServiceRequest.SetRequestHeader(
                "AUTHORIZATION",
              ...
              if (op.isDone) {
                ...
                if (ServicesUtils.IsUnityWebRequestReadyForJsonExtract(
                  ...
                  if (m_ServiceRequest.downloadHandler.text.Length != 0) {
                    var jsonParser =
                        new JSONParser(m_ServiceRequest.downloadHandler.text);
                    try {
                      ...
                      if (json.AsDict().ContainsKey(k_serviceFlagsKey)) {
                        jsonFlags = json.AsDict() [k_serviceFlagsKey]
                                        .AsDict();
                      }

                      if (jsonFlags != null) {
                        foreach (var key in jsonFlags.Keys) {
                          serviceFlags.Add(key, jsonFlags [key]
                                                    .AsBool());
                        }
                      }
                    } catch (Exception ex) {
                      ...
                      NotificationManager.instance.Publish(
                          ...
                          Notification.Severity.Error, ex.Message);
                    }
                  }
                }
