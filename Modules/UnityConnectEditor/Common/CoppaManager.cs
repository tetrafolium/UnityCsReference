...
namespace UnityEditor.Connect {
  ...
  internal class CoppaManager {
    ...
    const string k_CoppaCommonStyleSheetPath =
        "StyleSheets/ServicesWindow/CoppaCommon.uss";
    const string k_CoppaDarkStyleSheetPath =
        "StyleSheets/ServicesWindow/CoppaDark.uss";
    const string k_CoppaLightStyleSheetPath =
        "StyleSheets/ServicesWindow/CoppaLight.uss";
    const string k_CoppaLearnMoreUrl =
        "https://docs.unity3d.com/Manual/UnityAnalyticsCOPPA.html";
    const string k_CoppaComplianceEditorConfigurationExceptionMessage =
        "Unexpected UnityConnect SetCOPPACompliance behavior. " +
    ...
    public const string CoppaComplianceChangedMessage =
        "COPPA compliance changed";
    const string k_CoppaUnexpectedSaveRequestBehaviorMessage =
        "Unexpected save request behavior.";
    ...
    public SaveButtonCallback saveButtonCallback {
      private get;
      set;
    }
    public CancelButtonCallback cancelButtonCallback {
      private get;
      set;
    }
    ...
    private struct CoppaState { public bool isCompliant; }

    /// <summary>
    /// Configures a new Coppa manager to this within an existing EditorWindow
    /// </summary>
    /// <param name="rootVisualElement">visual element where the coppa content
    /// must be added</param>
    ...
    void InitializeCoppaManager(VisualElement rootVisualElement) {
      ...
      rootVisualElement.AddStyleSheetPath(
          EditorGUIUtility.isProSkin ? k_CoppaDarkStyleSheetPath
          : k_CoppaLightStyleSheetPath);
      var coppaTemplate =
          EditorGUIUtility.Load(k_CoppaTemplatePath) as VisualTreeAsset;
      ...
      var learnMoreClickable =
          new Clickable(() => { Application.OpenURL(k_CoppaLearnMoreUrl); });
      m_CoppaContainer.Q(k_CoppaLearnLinkBtnName)
          .AddManipulator(learnMoreClickable);
      ...
      persistContainer.Q<Button>(k_SaveBtnName).clicked += () => {
        try {
          ServicesConfiguration.instance.RequestCurrentProjectCoppaApiUrl(
              projectCoppaApiUrl => {
                var payload =
                    $"{{\"coppa\":\"{GetCompliancyJsonValueFromFieldValue(coppaField)}\"}}";
                var uploadHandler =
                    new UploadHandlerRaw(Encoding.UTF8.GetBytes(payload));
                var currentSaveRequest = new UnityWebRequest(
                    projectCoppaApiUrl, UnityWebRequest.kHttpVerbPUT){
                    uploadHandler = uploadHandler};
                currentSaveRequest.suppressErrorsToConsole = true;
                currentSaveRequest.SetRequestHeader(
                    "AUTHORIZATION",
                ...
                operation.completed += asyncOperation => {
                  try {
                    if (currentSaveRequest.responseCode ==
                        k_HttpStatusNoContent) {
                      try {
                        var newCompliancyValue =
                            GetCompliancyForFieldValue(coppaField);
                        if (!UnityConnect.instance.SetCOPPACompliance(
                                newCompliancyValue)) {
                          EditorAnalytics.SendCoppaComplianceEvent(
                              new CoppaState(){
                                  isCompliant =
                                      newCompliancyValue ==
                                      COPPACompliance.COPPACompliant});

                          SetCoppaFieldValue(originalCoppaValue, coppaField);
                          SetPersistContainerVisibility(
                              originalCoppaValue, persistContainer, coppaField);
                          exceptionCallback?.Invoke(
                              originalCoppaValue,
                              new CoppaComplianceEditorConfigurationException(
                                  k_CoppaComplianceEditorConfigurationExceptionMessage));
                        } else {
                      ...
                      try {
                        ...
                        exceptionCallback?.Invoke(
                            originalCoppaValue,
                            new CoppaComplianceWebConfigurationException(L10n.Tr(
                                k_CoppaUnexpectedSaveRequestBehaviorMessage)){
                                ...
                                responseHeaders =
                                    currentSaveRequest.GetResponseHeaders(),
                                responseCode = currentSaveRequest.responseCode,
                                isHttpError =
                                    (currentSaveRequest.result ==
          ...
          SetCoppaFieldValue(originalCoppaValue, coppaField);
          SetPersistContainerVisibility(originalCoppaValue, persistContainer,
                                        coppaField);
    ...
    static void SetEnabledCoppaControls(VisualElement coppaContainer,
                                        bool enable) {
    ...
    static void SetCoppaFieldValue(COPPACompliance coppaCompliance,
                                   PopupField<String> coppaField) {
      coppaField.SetValueWithoutNotify(
          GetFieldValueForCompliancy(coppaCompliance));
    }

    static void SetPersistContainerVisibility(COPPACompliance coppaCompliance,
                                              VisualElement persistContainer,
                                              PopupField<String> coppaField) {
      persistContainer.style.display =
          coppaField.GetValueToDisplay() !=
                  GetFieldValueForCompliancy(coppaCompliance)
              ? DisplayStyle.Flex
              : DisplayStyle.None;
    }
    ...
    GetCompliancyForFieldValue(PopupField<string> coppaField) {
    ...
    static string
    GetCompliancyJsonValueFromFieldValue(PopupField<string> coppaField) {
      return coppaField.value == L10n.Tr(k_Yes) ? k_CoppaCompliantJsonValue
                                                : k_CoppaNotCompliantJsonValue;
    }
    ...
    public delegate void ExceptionCallback(COPPACompliance coppaCompliance,
                                           Exception exception);
  }

  internal class CoppaComplianceEditorConfigurationException : Exception {
    public CoppaComplianceEditorConfigurationException(string message)
        : base(message) {}
  ...
  internal class CoppaComplianceWebConfigurationException : Exception {
    ...
    public CoppaComplianceWebConfigurationException(string message)
        : base(message) {}
  }
}
