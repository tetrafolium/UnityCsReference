...
namespace UnityEditor.Connect {
  ...
  internal class ProjectBindManager : IDisposable {
    const string k_ProjectBindTemplatePath =
        "UXML/ServicesWindow/ProjectBind.uxml";
    const string k_ProjectBindCommonStyleSheetPath =
        "StyleSheets/ServicesWindow/ProjectBindCommon.uss";
    const string k_ProjectBindDarkStyleSheetPath =
        "StyleSheets/ServicesWindow/ProjectBindDark.uss";
    const string k_ProjectBindLightStyleSheetPath =
        "StyleSheets/ServicesWindow/ProjectBindLight.uss";
    ...
    const string k_DialogConfirmationMessage =
        "Are you sure you want to link to the project '{0}' in organization '{1}'?";
    ...
    const string k_CouldNotObtainOrganizationsMessage =
        "Could not obtain organizations.";
    const string k_ProjectLinkSuccessMessage =
        "Project was linked successfully.";
    ...
    public CreateButtonCallback createButtonCallback {
      private get;
      set;
    }
    public LinkButtonCallback linkButtonCallback {
      private get;
      set;
    }
    ...
    public VisualElement projectBindContainer {
      get;
      private set;
    }
    ...
    static ProjectBindManager() {
      k_AnyRoleFilter =
          new List<string>(new[]{k_RoleOwner, k_RoleManager, k_RoleUser});
      k_AtLeastManagerFilter =
          new List<string>(new[]{k_RoleOwner, k_RoleManager});
    }

    /// <summary>
    /// Configures a new Project Bind manager to this within an existing
    /// EditorWindow
    /// </summary>
    /// <param name="rootVisualElement">visual element where the project bind
    /// content must be added</param>
    ...
    void Dispose(bool disposing) {
      if (disposing) {
        UnityConnect.instance.ProjectStateChanged -=
            OnProjectStateChangedAfterRebind;
        UnityConnect.instance.ProjectStateChanged -=
            OnProjectStateChangedAfterCreation;
    ...
    ~ProjectBindManager() { Dispose(false); }
    ...
    void InitializeProjectBindManager(VisualElement rootVisualElement) {
      ...
      rootVisualElement.AddStyleSheetPath(
          EditorGUIUtility.isProSkin ? k_ProjectBindDarkStyleSheetPath
          : k_ProjectBindLightStyleSheetPath);
      var projectBindTemplate =
          EditorGUIUtility.Load(k_ProjectBindTemplatePath) as VisualTreeAsset;
      ...
      m_CreateProjectIdBlock =
          projectBindContainer.Q(k_CreateProjectIdBlockName);
    ...
    void SetupCreateProjectIdBlock() {
      var popupField =
          BuildPopupField(m_CreateProjectIdBlock, k_OrganizationFieldName);
      popupField.RegisterValueChangedCallback(
          delegate(ChangeEvent<string> evt) {
            if (evt.newValue != m_LastCreateBlockOrganization) {
              ...
              m_CreateProjectIdBlock.Q(k_CreateProjectIdBtnName)
                  .SetEnabled(m_LastCreateBlockOrganization !=
      ...
      m_CreateProjectIdBlock.Q(k_ReuseProjectIdLinkBtnName)
          .AddManipulator(reuseProjectIdClickable);

      m_CreateProjectIdBlock.Q<Button>(k_CancelBtnName).style.display =
          DisplayStyle.None;

      var createProjectIdBtn =
          m_CreateProjectIdBlock.Q<Button>(k_CreateProjectIdBtnName);
      createProjectIdBtn.SetEnabled(false);
      createProjectIdBtn.clicked += () => {
        if (m_LastCreateBlockOrganization !=
            L10n.Tr(k_SelectOrganizationText)) {
          m_CreateIteration = 0;
    ...
    void SetupReuseProjectIdBlock() {
      var organizationPopupField =
          BuildPopupField(m_ReuseProjectIdBlock, k_OrganizationFieldName);
      var projectIdPopupField =
          BuildPopupField(m_ReuseProjectIdBlock, k_ProjectIdFieldName);

      organizationPopupField.RegisterValueChangedCallback(delegate(
          ChangeEvent<string> evt) {
        if (evt.newValue != m_LastReuseBlockOrganization) {
          m_LastReuseBlockOrganization = evt.newValue;
          if (m_LastReuseBlockOrganization ==
              L10n.Tr(k_SelectOrganizationText)) {
            var projectIdField = m_ReuseProjectIdBlock.Q<PopupField<string>>(
                k_ProjectIdFieldName);
            projectIdField.choices =
                new List<string>(){L10n.Tr(k_SelectProjectText)};
      ...
      projectIdPopupField.RegisterValueChangedCallback(
          delegate(ChangeEvent<string> evt) {
            if (evt.newValue != m_LastReuseBlockOrganization) {
              ...
              m_ReuseProjectIdBlock.Q<Button>(k_LinkBtnName)
                  .SetEnabled(m_LastReuseBlockProject !=
      ...
      m_ReuseProjectIdBlock.Q(k_CreateProjectIdLinkBtnName)
          .AddManipulator(createProjectIdClickable);

      m_ReuseProjectIdBlock.Q<Button>(k_CancelBtnName).style.display =
          DisplayStyle.None;
      ...
      linkBtn.clicked += () => {
        if (L10n.Tr(k_SelectProjectText) != m_LastReuseBlockProject) {
          ...
          if (EditorUtility.DisplayDialog(
                  L10n.Tr(k_LinkProjectWindowTitle),
                  string.Format(L10n.Tr(k_DialogConfirmationMessage),
                                projectInfo.name, projectInfo.organizationName),
                  L10n.Tr(k_Yes), L10n.Tr(k_No))) {
            try {
              // Only register before creation. Remove first in case it was
              // already added.
    ...
    void RequestCreateOperation() {
      ServicesConfiguration.instance.RequestOrganizationProjectsApiUrl(
          m_OrgIdByName[m_LastCreateBlockOrganization],
          organizationProjectsApiUrl => {
            var payload =
                $"{{\"name\":\"{Application.productName + GetProjectNameSuffix()}\", \"active\":true}}";
            var uploadHandler =
                new UploadHandlerRaw(Encoding.UTF8.GetBytes(payload));
            m_CurrentRequest = new UnityWebRequest(
                organizationProjectsApiUrl, UnityWebRequest.kHttpVerbPOST){
                downloadHandler = new DownloadHandlerBuffer(),
                uploadHandler = uploadHandler};
            m_CurrentRequest.suppressErrorsToConsole = true;
            m_CurrentRequest.SetRequestHeader(
                "AUTHORIZATION",
                $"Bearer {UnityConnect.instance.GetUserInfo().accessToken}");
            m_CurrentRequest.SetRequestHeader("Content-Type",
    ...
    void CreateOperationOnCompleted(AsyncOperation obj) {
      if (m_CurrentRequest == null) {
        // If we lost our m_CurrentRequest request reference, we can't risk
        // doing anything.
        return;
      }
      ...
      if (ServicesUtils.IsUnityWebRequestReadyForJsonExtract(
      ...
      } else if (m_CurrentRequest.responseCode ==
                 k_HttpStatusCodeUnprocessableEntity) {
        m_CurrentRequest?.Dispose();
        m_CurrentRequest = null;
        m_CreateIteration++;
        RequestCreateOperation();
        ...
        try {
          var ex = new UnityConnectWebRequestException(
              L10n.Tr(k_CouldNotCreateProjectMessage)){
              ...
              isHttpError = (m_CurrentRequest.result ==
                             UnityWebRequest.Result.ProtocolError),
              isNetworkError = (m_CurrentRequest.result ==
    ...
    void BindProject(ProjectInfoData projectInfo) {
      UnityConnect.instance.BindProject(projectInfo.guid, projectInfo.name,
                                        projectInfo.organizationId);
      EditorAnalytics.SendProjectServiceBindingEvent(
          new ProjectBindState(){bound = true, projectName = projectInfo.name});
      NotificationManager.instance.Publish(
          Notification.Topic.ProjectBind, Notification.Severity.Info,
          L10n.Tr(k_ProjectLinkSuccessMessage));
    }

    /// <summary>
    /// Using this method to simulate a callback after binding a project for
    /// creation and making sure the project info are fully loaded in
    /// UnityConnect.instance.projectInfo
    /// </summary>
    ...
    /// <summary>
    /// Using this method to simulate a callback after binding a project for
    /// rebinding an existing project and making sure the project info are fully
    /// loaded in UnityConnect.instance.projectInfo
    /// </summary>
    /// <param name="projectInfo"></param>
    ...
    void LoadProjectField(string organizationName,
                          PopupField<string> projectIdField) {
      ServicesConfiguration.instance.RequestOrganizationProjectsApiUrl(
          m_OrgIdByName[organizationName], organizationProjectsApiUrl => {
            var getProjectsRequest = new UnityWebRequest(
                organizationProjectsApiUrl, UnityWebRequest.kHttpVerbGET){
                downloadHandler = new DownloadHandlerBuffer()};
            getProjectsRequest.suppressErrorsToConsole = true;
            getProjectsRequest.SetRequestHeader(
                "AUTHORIZATION",
                $"Bearer {UnityConnect.instance.GetUserInfo().accessToken}");
            ...
            operation.completed += op => {
              try {
                if (ServicesUtils.IsUnityWebRequestReadyForJsonExtract(
                        getProjectsRequest)) {
                  var jsonParser =
                      new JSONParser(getProjectsRequest.downloadHandler.text);
                  var json = jsonParser.Parse();
                  try {
                    m_ProjectInfoByName =
                        new Dictionary<string, ProjectInfoData>();

                    var jsonProjects = json.AsDict() [k_JsonProjectsNodeName]
                                           .AsList();
                    foreach (var jsonProject in jsonProjects) {
                      if (!jsonProject
                               .AsDict() [k_JsonArchivedNodeName]
                               .AsBool()) {
                        var projectInfo =
                            ExtractProjectInfoFromJson(jsonProject);
                    ...
                    var sortedProjectNames =
                        new List<string>(m_ProjectInfoByName.Keys);
                    ...
                    if (exceptionCallback != null) {
                      ...
                      // If there is no exception callback, we have to at least
                      // log it
                      Debug.LogException(ex);
                    }
                  }
                  ...
                  var ex = new UnityConnectWebRequestException(
                      L10n.Tr(k_CouldNotObtainProjectMessage)){
                      ...
                      isHttpError = (getProjectsRequest.result ==
                                     UnityWebRequest.Result.ProtocolError),
                      isNetworkError = (getProjectsRequest.result ==
                                        UnityWebRequest.Result.ConnectionError),
                  };
                  if (exceptionCallback != null) {
                    exceptionCallback.Invoke(ex);
    ...
    static ProjectInfoData ExtractProjectInfoFromJson(JSONValue jsonProject) {
      return new ProjectInfoData(jsonProject
                                     .AsDict() [k_JsonOrgIdNodeName]
                                     .AsString(),
                                 jsonProject
                                     .AsDict() [k_JsonOrgNameNodeName]
                                     .AsString(),
                                 jsonProject
                                     .AsDict() [k_JsonNameNodeName]
                                     .AsString(),
                                 jsonProject
                                     .AsDict() [k_JsonGuidNodeName]
                                     .AsString(),
                                 jsonProject
                                     .AsDict() [k_JsonIdNodeName]
                                     .AsString());
    }

    /// <summary>
    /// To create a new project, the current user must be the owner or a manager
    /// of an organization
    /// </summary>
    /// <param name="organizationField"></param>
    /// <param name="projectIdField"></param>
    void LoadCreateOrganizationField(PopupField<string> organizationField,
                                     PopupField<string> projectIdField = null) {
      ServicesConfiguration.instance.RequestCurrentUserApiUrl(
          currentUserApiUrl => {
            var getOrganizationsRequest =
                new UnityWebRequest(currentUserApiUrl + "?include=orgs",
                                    UnityWebRequest.kHttpVerbGET){
                    downloadHandler = new DownloadHandlerBuffer()};
            getOrganizationsRequest.suppressErrorsToConsole = true;
            getOrganizationsRequest.SetRequestHeader(
                "AUTHORIZATION",
                $"Bearer {UnityConnect.instance.GetUserInfo().accessToken}");
                ...
                if (ServicesUtils.IsUnityWebRequestReadyForJsonExtract(
                  ...
                  try {
                    var jsonParser = new JSONParser(
                        getOrganizationsRequest.downloadHandler.text);
                    var json = jsonParser.Parse();
                    m_OrgIdByName.Clear();
                    var sortedOrganizationNames = new List<string>();
                    foreach (var rawOrg in json
                                 .AsDict() [k_JsonOrgsNodeName]
                                 .AsList()) {
                      var org = rawOrg.AsDict();
                      if (k_AtLeastManagerFilter.Contains(org
                                                          [k_JsonRoleNodeName]
                                                              .AsString())) {
                        sortedOrganizationNames.Add(org [k_JsonNameNodeName]
                                                        .AsString());
                        m_OrgIdByName.Add(org [k_JsonNameNodeName]
                                              .AsString(),
                                          org [k_JsonIdNodeName]
                                              .AsString());
                      }
                    }
                    ...
                    organizationField.SetValueWithoutNotify(
                        organizationField.choices[0]);
                    if (projectIdField != null) {
                      projectIdField.choices =
                          new List<string>{L10n.Tr(k_SelectProjectText)};
                      projectIdField.SetValueWithoutNotify(
                          L10n.Tr(k_SelectProjectText));
                      projectIdField.SetEnabled(false);
                    }
                  } catch (Exception ex) {
                  ...
                  var ex = new UnityConnectWebRequestException(
                      L10n.Tr(k_CouldNotObtainOrganizationsMessage)){
                      ...
                      responseHeaders =
                          getOrganizationsRequest.GetResponseHeaders(),
                      responseCode = getOrganizationsRequest.responseCode,
                      isHttpError = (getOrganizationsRequest.result ==
                                     UnityWebRequest.Result.ProtocolError),
                      isNetworkError = (getOrganizationsRequest.result ==
                                        UnityWebRequest.Result.ConnectionError),
                  };
                  if (exceptionCallback != null) {
                    exceptionCallback.Invoke(ex);
    ...
    /// <summary>
    /// To attach a project an existing project, we must collect all orgs the
    /// current user is a member of. In addition the current user may be a guest
    /// of a specific project, in which case we must also look at all projects
    /// to find organizations.
    /// </summary>
    /// <param name="organizationField"></param>
    /// <param name="projectIdField"></param>
    void LoadReuseOrganizationField(PopupField<string> organizationField,
                                    PopupField<string> projectIdField = null) {
      ServicesConfiguration.instance.RequestCurrentUserApiUrl(
          currentUserApiUrl => {
            var getOrganizationsRequest = new UnityWebRequest(
                currentUserApiUrl + "?include=orgs,projects",
              ...
              try {
                if (ServicesUtils.IsUnityWebRequestReadyForJsonExtract(
                  ...
                  try {
                    ...
                    foreach (var rawProject in json
                                 .AsDict() [k_JsonProjectsNodeName]
                                 .AsList()) {
                      var project = rawProject.AsDict();
                      if (!project [k_JsonArchivedNodeName]
                               .AsBool() &&
                          !sortedOrganizationNames.Contains(
                              project [k_JsonOrgNameNodeName]
                                  .AsString())) {
                        sortedOrganizationNames.Add(project
                                                    [k_JsonOrgNameNodeName]
                                                        .AsString());
                        m_OrgIdByName.Add(project [k_JsonOrgNameNodeName]
                                              .AsString(),
                                          project [k_JsonOrgIdNodeName]
                                              .AsString());
                      }
                    }
    ...
    static PopupField<string> BuildPopupField(VisualElement block,
                                              string anchorName) {
  ...
  internal class ProjectInfoData {
    ...
    public ProjectInfoData(string organizationId, string organizationName,
                           string name, string guid, string projectId) {
      this.guid = guid;
      this.name = name;
