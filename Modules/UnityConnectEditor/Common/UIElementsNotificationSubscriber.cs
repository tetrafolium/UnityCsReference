...
/// Uses UIElements, expects the presence of an empty VisualElement with class
/// "notification-container" where the messages will be displayed, usually at
/// the very top of a window. Don't forget to subscribe to the topics that are
/// of interest to your context and unsubscribed when closing window, etc.
/// </summary>
internal class UIElementsNotificationSubscriber : INotificationSubscriber {
const string k_NotificationTemplateUxmlPath =
	"UXML/ServicesWindow/NotificationTemplate.uxml";
const string k_NotificationTemplateCommonUssPath =
	"StyleSheets/ServicesWindow/NotificationCommon.uss";
const string k_NotificationTemplateDarkUssPath =
	"StyleSheets/ServicesWindow/NotificationDark.uss";
const string k_NotificationTemplateLightUssPath =
	"StyleSheets/ServicesWindow/NotificationLight.uss";
const string k_PagerTextTemplate = "{0} of {1}";
const string k_NullVisualElementExceptionMessage =
	"rootVisualElement cannot be null. It should be the rootVisualElement of the UI Toolkit layout.";
...
public UIElementsNotificationSubscriber(VisualElement rootVisualElement) {
	if (rootVisualElement == null) {
		throw new ArgumentException(
			      L10n.Tr(k_NullVisualElementExceptionMessage));
	}

	// This notification subscriber needs a specific container to work.
	// A specific service can create a container with the right class :
	// notification-container
	var notificationContainer =
		rootVisualElement.Q(null, k_NotificationContainerClassName);
	if (notificationContainer == null) {
		// Since the container does not exist, it is created and inserted as the
		// first element...
	...
	var notificationTemplate = EditorGUIUtility.Load(
		k_NotificationTemplateUxmlPath) as VisualTreeAsset;
	...
	rootVisualElement.AddStyleSheetPath(
		EditorGUIUtility.isProSkin
		                            ? k_NotificationTemplateDarkUssPath
	  : k_NotificationTemplateLightUssPath);
...
void InitializeNotification() {
	...
	m_MessageContainer =
		m_NotificationRoot.Q(null, k_MessageContainerClassName);
	m_DismissButton =
		m_NotificationRoot.Q<Button>(null, k_DismissButtonClassName);
	m_DismissButton.clicked += () => {
		NotificationManager.instance.Dismiss(
			m_Notifications[m_CurrentIndex].id);
	};
...
void InitializePager() {
	...
	m_PreviousButton =
		m_NotificationRoot.Q<Button>(null, k_PreviousButtonClassName);
	m_PageCount =
		m_NotificationRoot.Q<TextElement>(null, k_PageCountClassName);
	m_NextButton = m_NotificationRoot.Q<Button>(null, k_NextButtonClassName);

	m_PreviousButton.text = k_PreviousButtonText;
	m_PageCount.text =
		string.Format(L10n.Tr(k_PagerTextTemplate), k_PagerDefaultValue,
		              k_PagerDefaultValue);
...
public virtual void DismissNotification(long notificationId) {
	...
	if (dismissedNotification != null) {
		var dismissedNotificationIndex =
			m_Notifications.IndexOf(dismissedNotification);
		m_Notifications.Remove(dismissedNotification);
		if (m_Notifications.Count == 0) {
			m_NotificationRoot.style.display = DisplayStyle.None;
			m_CurrentIndex = -1;
		} else if (m_CurrentIndex > dismissedNotificationIndex ||
		           (m_CurrentIndex == dismissedNotificationIndex &&
		            m_CurrentIndex >= m_Notifications.Count)) {
			m_CurrentIndex -= 1;
		}
	...
	if (m_CurrentIndex > -1) {
		...
		default:
			throw new ArgumentOutOfRangeException(
				      k_NotificationSeverityParamName,
				      string.Format(L10n.Tr(k_UnknownSeverityExceptionMessage),
				                    m_Notifications[m_CurrentIndex].severity));
		}
...
void ConfigurePager() {
	if (m_Notifications.Count == 0) {
		...
		m_PageCount.text =
			string.Format(L10n.Tr(k_PagerTextTemplate), m_CurrentIndex + 1,
