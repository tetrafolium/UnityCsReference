...
namespace UnityEditor.PackageManager {
  internal static class PackageValidation {
    private static readonly Regex s_NameRegEx =
        new Regex(@"^([a-z][a-z\d\-\._]{0,99})$");
    private static readonly Regex s_OrganizationNameRegEx =
        new Regex(@"^([a-z][a-z\d\-_]{0,99})$");
    private static readonly Regex s_AllowedSemverRegEx =
        new Regex(@"^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(\-.+)?$");
    private static readonly Regex s_UnityMajorVersionRegEx =
        new Regex(@"^([1-9][0-9]{3})$");
    private static readonly Regex s_UnityMinorVersionRegEx =
        new Regex(@"^([1-9])$");
    private static readonly Regex s_UnityReleaseVersionRegEx =
        new Regex(@"^(0|[1-9]\d*)([abfp])(0|[1-9]\d*)$");
    ...
    public static bool ValidateOrganizationName(string organizationName) {
      return !string.IsNullOrEmpty(organizationName) &&
             s_OrganizationNameRegEx.IsMatch(organizationName);
    }

    public static bool ValidateVersion(string version) {
      return !string.IsNullOrEmpty(version) &&
             s_AllowedSemverRegEx.IsMatch(version);
    }
    ...
    public static bool ValidateUnityVersion(string majorVersion,
                                            string minorVersion,
                                            string releaseVersion = null) {
      return !string.IsNullOrEmpty(majorVersion) &&
             s_UnityMajorVersionRegEx.IsMatch(majorVersion) &&
             !string.IsNullOrEmpty(minorVersion) &&
             s_UnityMinorVersionRegEx.IsMatch(minorVersion) &&
