...
namespace UnityEditor.PackageManager {
  public static partial class Client {
    public static ListRequest List(bool offlineMode,
      ...
      long operationId;
      var status =
          List(out operationId, offlineMode, includeIndirectDependencies);
    ...
    public static ListRequest List() { return List(false, false); }
    ...
    public static EmbedRequest Embed(string packageName) {
      var packageInfo =
          PackageInfo.GetAll().FirstOrDefault(p => p.name == packageName);
      if (packageInfo == null)
        throw new InvalidOperationException(
            $"Cannot embed package [{packageName}] because it is not registered in the Asset Database.");

      Debug.Assert(packageInfo.entitlements.isAllowed,
                   "Expected [entitlements.isAllowed] flag to be true.");
    ...
    internal static GetCachedPackagesRequest
    GetCachedPackages(string registryId) {
      long operationId;
    ...
    public static SearchRequest Search(string packageIdOrName,
      ...
      long operationId;
      var status =
          GetPackageInfo(out operationId, packageIdOrName, offlineMode);
    ...
    public static SearchRequest SearchAll() { return SearchAll(false); }
