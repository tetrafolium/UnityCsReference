...
namespace UnityEditor.PackageManager.Requests {
  ...
  public abstract partial class Request : ISerializationCallbackReceiver {
    ...
    private void FetchError() {
      // We assume the request is empty when the Id is 0 and skip error
      // fetching. This happens during the serialization process as empty
      // instances are created in place of null references.
      if (Id == 0 || m_ErrorFetched ||
          NativeStatus.ConvertToManaged() != StatusCode.Failure) {
        return;
      }
    ...
    protected virtual void FetchNativeData() { FetchError(); }
    ...
    internal Request(long operationId, NativeStatusCode initialStatus) {
      ...
      Debug.AssertFormat(m_Status == NativeStatusCode.Error ? m_Id == 0
                         : m_Id > 0,
                           "Invalid operation id [status: {0}, id: {1}]",
                           m_Status, m_Id);
    }
  }

  /// <summary>
  /// Tracks the state of an asynchronous Upm server operation that returns a
  /// non-empty response
  /// </summary>
  public abstract class Request<T> : Request {
    ...
    private void FetchResult() {
      if (m_ResultFetched ||
          NativeStatus.ConvertToManaged() != StatusCode.Success) {
        return;
      }
