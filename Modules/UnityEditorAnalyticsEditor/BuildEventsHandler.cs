...
internal class BuildEventsHandlerPostProcess : IPostprocessBuildWithReport {
  ...
  private const string s_GradlePath =
      "Temp/gradleOut/build/intermediates/merged_manifests";
  ...
  private string
  SanitizePackageId(UnityEditor.PackageManager.PackageInfo packageInfo) {
    if (packageInfo.source == UnityEditor.PackageManager.PackageSource.Registry)
      return packageInfo.packageId;
    return packageInfo.name + "@" +
           Enum.GetName(typeof(UnityEditor.PackageManager.PackageSource),
                        packageInfo.source)
               .ToLower();
  }
  ...
  private void ReportBuildPackageIds(BuildFile[] buildFiles) {
    ...
    var matchingPackages =
        UnityEditor.PackageManager.PackageInfo.GetForAssemblyFilePaths(
            managedLibraries);
    var packageIds =
        matchingPackages.Select(item => SanitizePackageId(item)).ToArray();
    if (packageIds.Length > 0)
      EditorAnalytics.SendEventBuildPackageList(
          new BuildPackageIds(){package_ids = packageIds});
  }
  ...
  private void ReportSceneViewInfo() {
    ...
    if ((s_NumOfSceneViews != views.Length) ||
        (s_NumOf2dSceneViews != numOf2dSceneViews) || !s_EventSent) {
      s_EventSent = true;
      ...
      EditorAnalytics.SendEventSceneViewInfo(new SceneViewInfo(){
          total_scene_views = s_NumOfSceneViews,
          num_of_2d_views = s_NumOf2dSceneViews,
          is_default_2d_mode =
  ...
  private void ReportBuildTargetPermissions() {
    ...
    if (EditorUserBuildSettings.androidBuildSystem ==
      ...
      manifestFilePath =
          (EditorUserBuildSettings.androidBuildType == AndroidBuildType.Release)
              ? Paths.Combine(s_GradlePath,
                              "release/processReleaseManifest/merged",
                              s_AndroidManifest)
              : Paths.Combine(s_GradlePath, "debug/processDebugManifest/merged",
                              s_AndroidManifest);
    }
    ...
    if (File.Exists(manifestFilePath)) {
      ...
      XmlNodeList permissions =
          manifestFile.GetElementsByTagName("uses-permission");
      ...
      EditorAnalytics.SendEventBuildTargetPermissions(
          new AndroidBuildPermissions(){features = featuresList.ToArray(),
                                        permissions =
...
internal class BuildCompletionEventsHandler {
  [Serializable] struct BuildLibrariesInfo {
  ...
  public static void ReportPostBuildCompletionInfo(List<string> libraries) {
    if (libraries != null) {
      EditorAnalytics.SendEventBuildFrameworkList(new BuildLibrariesInfo(){
          ar_plugin_loaded =
              libraries.Contains("System/Library/Frameworks/ARKit.framework"),
