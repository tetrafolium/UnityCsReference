...
private static readonly string s_StyleSheetPath =
	"UIPackageResources/StyleSheets/UIElementsSamples/UIElementsSamples.uss";
private static readonly string s_DarkStyleSheetPath =
	"UIPackageResources/StyleSheets/UIElementsSamples/UIElementsSamplesDark.uss";
private static readonly string s_LightStyleSheetPath =
	"UIPackageResources/StyleSheets/UIElementsSamples/UIElementsSamplesLight.uss";
...
private readonly string k_TreeViewClassName =
	"unity-samples-explorer__tree-view";
private readonly string k_ContentPanelName = "content-container";
private readonly string k_ContentPanelClassName =
	"unity-samples-explorer__content-container";

private readonly string k_TreeItemClassName =
	"unity-samples-explorer__tree-item";
private readonly string k_TreeItemLabelClassName =
	"unity-samples-explorer__tree-item-label";

private readonly int k_TreeViewSelectionRestoreDelay = 400;
private readonly int k_TreeViewInitialSelectionDelay = 500;

private readonly string k_CategoryPanelClassName =
	"unity-samples-explorer__category_panel";
private readonly string k_CategoryTitleClassName =
	"unity-samples-explorer__category_title";
...
public static readonly string OpenWindowCommand =
	nameof(OpenUIElementsSamplesCommand);
...
public Func<SampleTreeItem, VisualElement> makeItem {
	get;
	private set;
}

public SampleTreeItem(string name,
                      Func<SampleTreeItem, VisualElement> makeItem,
...
public void OnEnable() {
	...
	var themedStyleSheet = EditorGUIUtility.isProSkin
	                       ? EditorGUIUtility
	                       .Load(s_DarkStyleSheetPath) as StyleSheet
	...
	var items = new List<ITreeViewItem>(){
		...
		new SampleTreeItem(
			"Numeric Fields", MakeNumericFieldsPanel,
			new List<TreeViewItem<string> >(){
					...
					new SampleTreeItem("MinMaxSlider",
					                   MinMaxSliderSnippet.Create),
					...
					new SampleTreeItem("Vector2Int",
					                   Vector2IntFieldSnippet.Create),
					new SampleTreeItem("Vector3Int",
					                   Vector3IntFieldSnippet.Create),
					new SampleTreeItem("RectInt", RectIntFieldSnippet.Create),
					new SampleTreeItem("BoundsInt",
					                   BoundsIntFieldSnippet.Create)
				}),
		new SampleTreeItem(
			"Value Fields", MakeValueFieldsPanel,
			new List<TreeViewItem<string> >(){
		...
		new SampleTreeItem(
			"Choice Fields", MakeChoiceFieldsPanel,
			new List<TreeViewItem<string> >(){
					...
					new SampleTreeItem("LayerMask",
					                   LayerMaskFieldSnippet.Create)
				}),
	};
	...
	// Force TreeView to call onSelectionChanged when it restores its own
	// selection from view data.
	treeView.schedule
	.Execute(() => { onSelectionChanged(treeView.selectedItems); })
...
private VisualElement MakeNumericFieldsPanel(SampleTreeItem item) {
	...
	container.Add(
		new Label("Numeric Fields"){
				classList = {k_CategoryTitleClassName}
			});

	container.Add(new IntegerField("Integer"));
	container.Add(new FloatField("Float"));
	container.Add(new LongField("Long"));
	container.Add(new MinMaxSlider("MinMaxSlider", 0, 20, -10,
	                               40){
...
private VisualElement MakeValueFieldsPanel(SampleTreeItem item) {
	...
	gradient.colorKeys =
		new GradientColorKey[] {new GradientColorKey(Color.red, 0),
		                        new GradientColorKey(Color.blue, 10),
		                        new GradientColorKey(Color.green, 20)};

	container.Add(new ColorField("Color"){
				value = Color.cyan
			});
	container.Add(new CurveField("Curve"){
				value = curve
			});
	container.Add(new GradientField("Gradient"){
				value = gradient
			});
