...
namespace UnityEditor.Experimental.GraphView {
  public class PlacematContainer : GraphView.Layer {
    public enum CycleDirection { Up, Down }
    ...
    GraphViewChange OnGraphViewChange(GraphViewChange graphViewChange) {
      if (graphViewChange.elementsToRemove != null)
        foreach (Placemat placemat in graphViewChange.elementsToRemove
                     .OfType<Placemat>())
    ...
    public bool GetPortCenterOverride(Port port,
      ...
      if (rootNode != null) {
        ...
        // Find the furthest placemat containing the rootNode and that is
        // collapsed (if any)
        Placemat placemat = m_Placemats.FirstOrDefault(
            p => p.Collapsed && p.WillDragNode(rootNode));
    ...
    public T CreatePlacemat<T>(Rect placematPosition, int zOrder,
                               string placematTitle) where T : Placemat,
                                                               new () {
      return InitAndAddPlacemat(new T(), placematPosition, zOrder,
                                placematTitle);
    }

    public T CreatePlacemat<T>(Func<T> creator, Rect placematPosition,
                               int zOrder, string placematTitle) where T
        : Placemat {
      return InitAndAddPlacemat(creator(), placematPosition, zOrder,
                                placematTitle);
    }

    T InitAndAddPlacemat<T>(T placemat, Rect placematPosition, int zOrder,
                            string placematTitle) where T : Placemat {
    ...
    public void HideCollapsedEdges() {
      // We need to hide edges in the reverse zOrder (topmost mats are collapsed
      // first)
