...
internal static readonly Vector2 k_DefaultCollapsedSize =
	new Vector2(200, 42);
...
internal void Init(GraphView graphView) {
	...
	var template = EditorGUIUtility.Load(
		"UXML/GraphView/PlacematElement.uxml") as VisualTreeAsset;
	if (template != null)
	...
	capabilities |= Capabilities.Deletable | Capabilities.Movable |
	                Capabilities.Selectable | Capabilities.Copiable;
...
public Rect ExpandedPosition =>
Collapsed ? new Rect(layout.position, UncollapsedSize)
        : GetPosition();
...
PlacematContainer Container => m_PlacematContainer ??(
	m_PlacematContainer = GetFirstAncestorOfType<PlacematContainer>());
...
SetCollapsedElements(IEnumerable<GraphElement> collapsedElements) {
	if (!Collapsed)
		return;
...
internal void HideCollapsedEdges() {
	...
	foreach (var edge in m_GraphView.edges.ToList())
		if (AnyNodeIsConnectedToPort(nodes, edge.input) &&
		    AnyNodeIsConnectedToPort(nodes, edge.output)) {
...
void RebuildCollapsedElements() {
	...
	var graphElements =
		...
		.Where(e => !(e is Edge) && (e.parent is GraphView.Layer) &&
		       (e.capabilities & Capabilities.Selectable) != 0)
		.ToList();

	var collapsedElementsElsewhere = new List<GraphElement>();
	RecurseRebuildCollapsedElements_LocalFunc(this, graphElements,
	                                          collapsedElementsElsewhere);
...
// TODO: Move to local function of Collapse once we move to C# 7.0 or
// higher.
void RecurseRebuildCollapsedElements_LocalFunc(
	Placemat currentPlacemat, IList<GraphElement> graphElements,
	...
	var currentActivePlacematRect = new Rect(
		currRect.x + k_SelectRectOffset, currRect.y + k_SelectRectOffset,
		...
		if (elem.layout.Overlaps(currentActivePlacematRect)) {
			...
			if (placemat != null && placemat.ZOrder > currentPlacemat.ZOrder) {
				if (placemat.Collapsed)
					...
					RecurseRebuildCollapsedElements_LocalFunc(
						placemat, graphElements, collapsedElementsElsewhere);
			}
...
static bool AnyNodeIsConnectedToPort(IEnumerable<Node> nodes, Port port) {
	foreach (var node in nodes) {
		...
		if (stackNode != null &&
		    stackNode.contentContainer.Children().Any(n => n == port.node))
			return true;
...
void ActOnGraphElementsOver(Action<GraphElement> act) {
	var graphElements = m_GraphView.graphElements.ToList().Where(
		e => !(e is Edge) && (e.parent is GraphView.Layer) &&
		(e.capabilities & Capabilities.Selectable) != 0);
...
internal bool ActOnGraphElementsOver(Func<GraphElement, bool> act,
                                     bool includePlacemats) {
	var graphElements =
		m_GraphView.graphElements.ToList()
		.Where(e => !(e is Edge) && e.parent is GraphView.Layer &&
		       (e.capabilities & Capabilities.Selectable) != 0)
	...
	return RecurseActOnGraphElementsOver_LocalFunc(this, graphElements, act,
	                                               includePlacemats);
}

// TODO: Move to local function of ActOnGraphElementsOver once we move to
// C# 7.0 or higher.
static bool RecurseActOnGraphElementsOver_LocalFunc(
	Placemat currentPlacemat, List<GraphElement> graphElements,
	...
	if (currentPlacemat.Collapsed) {
		foreach (var elem in currentPlacemat.CollapsedElements) {
			var placemat = elem as Placemat;
			if (placemat != null && placemat.ZOrder > currentPlacemat.ZOrder)
				if (RecurseActOnGraphElementsOver_LocalFunc(placemat, graphElements,
				                                            act, includePlacemats))
					return true;

			if (placemat == null ||
			    (includePlacemats && placemat.ZOrder > currentPlacemat.ZOrder))
				if (act(elem))
					return true;
		}
	} else {
		...
		foreach (var elem in graphElements) {
			if (elem.layout.Overlaps(currentActivePlacematRect)) {
				var placemat = elem as Placemat;
				if (placemat != null && placemat.ZOrder > currentPlacemat.ZOrder)
					if (RecurseActOnGraphElementsOver_LocalFunc(
						    placemat, graphElements, act, includePlacemats))
						return true;
...
internal void GrowToFitElements(List<GraphElement> elements) {
	...
	if (elements.Count > 0 &&
	    ComputeElementBounds(ref pos, elements,
	                         MinSizePolicy.DoNotEnsureMinSize)) {
		// We don't resize to be snug. In other words: we don't ever decrease in
		// size.
...
void ResizeToIncludeSelectedNodes() {
	List<GraphElement> nodes = m_GraphView.selection.OfType<GraphElement>()
	                           .Where(e => e is Node)
	                           .ToList();

	// Now include the selected nodes
	var pos = new Rect();
	if (ComputeElementBounds(ref pos, nodes,
	                         MinSizePolicy.DoNotEnsureMinSize)) {
		// We don't resize to be snug: we only resize enough to contain the
		// selected nodes.
...
internal void
GetElementsToMove(bool moveOnlyPlacemat,
                  HashSet<GraphElement> collectedElementsToMove) {
	if (Collapsed) {
...
BuildContextualMenu(ContextualMenuPopulateEvent evt) {
	...
	if (placemat != null) {
		...
		evt.menu.AppendAction("Change Color...", a => {
					ColorPicker.Show(c => placemat.Color = c, placemat.Color, showAlpha
					                 : false);
				});
		...
		evt.menu.AppendAction(placemat.Collapsed ? "Expand"
		              : "Collapse",
		                      a => placemat.Collapsed = !placemat.Collapsed);

		// Gather nodes here so that we don't recycle this code in the resize
		// functions.
		...
		evt.menu.AppendAction(
			"Resize/Grow To Fit Selection",
			a => placemat.ResizeToIncludeSelectedNodes(), s => {
		...
		var status = placemat.Container.Placemats.Any()
		         ? DropdownMenuAction.Status.Normal
		         : DropdownMenuAction.Status.Disabled;

		evt.menu.AppendAction("Order/Bring To Front",
		                      a => Container.BringToFront(placemat), status);
		evt.menu.AppendAction(
			"Order/Bring Forward",
			a => Container.CyclePlacemat(placemat,
			                             PlacematContainer.CycleDirection.Up),
			status);
		evt.menu.AppendAction(
			"Order/Send Backward",
			a => Container.CyclePlacemat(placemat,
			                             PlacematContainer.CycleDirection.Down),
			status);
		evt.menu.AppendAction("Order/Send To Back",
		                      a => Container.SendToBack(placemat), status);
	}
}
...
public void StartEditTitle() {
	// Focus field and select text (spare the user from having to type enter
	// before editing text).
...
internal bool GetPortCenterOverride(Port port,
	...
	var offset = port.orientation ==
	             Orientation.Horizontal
	             ? new Vector2(
		port.direction == Direction.Input ? -halfSize.x + xOffset
		       : halfSize.x - xOffset, 0)
	  : new Vector2(0, port.direction ==
	                Direction.Input ? -halfSize.y + yOffset
	                : halfSize.y - yOffset);
...
// Helper method that calculates how big a Placemat should be to fit the
// nodes on top of it currently.
internal const float k_Bounds = 9.0f;
internal const float k_BoundTop = 29.0f;     // Current height of Title

public enum MinSizePolicy { EnsureMinSize, DoNotEnsureMinSize }

// Returns false if bounds could not be computed.
public static bool ComputeElementBounds(
	ref Rect pos, List<GraphElement> elements,
	MinSizePolicy ensureMinSize = MinSizePolicy.EnsureMinSize) {
	...
	pos = new Rect(minX - k_Bounds, minY - (k_BoundTop + k_Bounds), width,
	               height);
...
// The next two values need to be the same as USS... however, we can't get
// the values from there as we need them in a static methods used to create
// new placemats
