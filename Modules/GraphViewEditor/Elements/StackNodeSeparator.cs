...
namespace UnityEditor.Experimental.GraphView {
  internal abstract class StackNodeInserter : VisualElement, IInsertLocation {
    public VisualElement stack {
      get { return GetFirstAncestorOfType<StackNode>(); }
    }

    public abstract void GetInsertInfo(Vector2 worldPosition,
                                       out InsertInfo insert);

    public StackNodeInserter() { this.AddManipulator(new Inserter()); }
  }
  ...
  internal class StackNodePlaceholder : StackNodeInserter {
    ...
    public override void GetInsertInfo(Vector2 worldPosition,
                                       out InsertInfo insertInfo) {
      insertInfo = new InsertInfo{
          target = stack, index = 0,
          localPosition = this.ChangeCoordinatesTo(stack, rect.center)};
    }
  }
  ...
  internal class StackNodeSeparator : StackNodeInserter {
    ...
    public Action<ContextualMenuPopulateEvent, int> menuEvent {
      get;
      set;
    }
    ...
    public override void GetInsertInfo(Vector2 worldPosition,
                                       out InsertInfo insertInfo) {
      insertInfo = new InsertInfo{
          target = stack, index = parent.IndexOf(this),
          localPosition = this.ChangeCoordinatesTo(stack, rect.center)};
    }
  ...
  internal class StackNodeContentContainer : StackNodeInserter {
    public override void GetInsertInfo(Vector2 worldPosition,
                                       out InsertInfo insertInfo) {
      insertInfo = new InsertInfo{target = stack, index = 0,
      ...
      foreach (VisualElement child in stack.Children()) {
        ...
        if (child.ContainsPoint(localPos)) {
          ...
          insertInfo.localPosition =
              child.ChangeCoordinatesTo(stack, child.rect.center);
        }
      }
    }
  }
