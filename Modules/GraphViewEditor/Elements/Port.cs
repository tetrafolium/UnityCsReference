...
namespace UnityEditor.Experimental.GraphView {
  public class Port : GraphElement {
    private static CustomStyleProperty<Color> s_PortColorProperty =
        new CustomStyleProperty<Color>("--port-color");
    private static CustomStyleProperty<Color> s_DisabledPortColorProperty =
        new CustomStyleProperty<Color>("--disabled-port-color");

    private static readonly Color s_DefaultColor =
        new Color(240 / 255f, 240 / 255f, 240 / 255f);
    private static readonly Color s_DefaultDisabledColor =
        new Color(70 / 255f, 70 / 255f, 70 / 255f);
    ...
    public string visualClass {
      ...
      set {
        ...
        // Add the given class if not null or empty. Use the auto class
        // otherwise.
        if (!string.IsNullOrEmpty(m_VisualClass))
    ...
    private void ManageTypeClassList(Type type,
                                     Action<string> classListAction) {
    ...
    public virtual void OnStartEdgeDragging() { highlight = false; }

    public virtual void OnStopEdgeDragging() { highlight = true; }
    ...
    public Edge ConnectTo(Port other) { return ConnectTo<Edge>(other); }

    public T ConnectTo<T>(Port other) where T : Edge, new () {
      if (other == null)
        throw new ArgumentNullException(
            "Port.ConnectTo<T>() other argument is null");
    ...
    public virtual void Disconnect(Edge edge) {
      if (edge == null)
        throw new ArgumentException(
            "The value passed to PortPresenter.Disconnect is null");
    ...
    private class DefaultEdgeConnectorListener : IEdgeConnectorListener {
      ...
      public void OnDrop(GraphView graphView, Edge edge) {
        ...
        if (graphView.graphViewChanged != null) {
          edgesToCreate =
              graphView.graphViewChanged(m_GraphViewChange).edgesToCreate;
        }
    ...
    // TODO This is a workaround to avoid having a generic type for the port as
    // generic types mess with USS.
    public static Port Create<TEdge>(Orientation orientation,
                                     Direction direction, Capacity capacity,
                                     Type type) where TEdge : Edge,
                                                              new () {
    ...
    protected Port(Orientation portOrientation, Direction portDirection,
                   Capacity portCapacity, Type type) {
      // currently we don't want to be styled as .graphElement since we're
      // contained in a Node
      ClearClassList();

      var tpl =
          EditorGUIUtility.Load("UXML/GraphView/Port.uxml") as VisualTreeAsset;
    ...
    public override Vector3 GetGlobalCenter() {
      ...
      if (m_GraphView != null &&
          m_GraphView.GetPortCenterOverride(this, out overriddenPosition)) {
    ...
    public override bool ContainsPoint(Vector2 localPoint) {
      ...
      if (direction == Direction.Input) {
        boxRect = new Rect(-lRect.xMin, -lRect.yMin, lRect.width + lRect.xMin,
                           rect.height);

        boxRect.width += m_ConnectorText.layout.xMin - lRect.xMax;
      } else {
        boxRect =
            new Rect(0, -lRect.yMin, rect.width - lRect.xMin, rect.height);
      ...
      return boxRect.Contains(
          this.ChangeCoordinatesTo(m_ConnectorBox, localPoint));
    }
    ...
    protected override void ExecuteDefaultAction(EventBase evt) {
      ...
      // Only update the box cap background if the port is enabled or
      // highlighted.
      if (highlight) {
    ...
    protected override void OnCustomStyleResolved(ICustomStyle styles) {
      ...
      if (!m_PortColorIsInline &&
          styles.TryGetValue(s_PortColorProperty, out portColorValue)) {
        m_PortColor = portColorValue;
        UpdateCapColor();
      }

      if (styles.TryGetValue(s_DisabledPortColorProperty,
                             out disableColorValue))
