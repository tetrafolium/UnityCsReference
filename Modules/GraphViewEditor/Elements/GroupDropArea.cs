...
namespace UnityEditor.Experimental.GraphView {
  internal class GroupDropArea : VisualElement, IDropTarget {
    public bool CanAcceptDrop(List<ISelectable> selection) {
      ...
      return !selection.Cast<GraphElement>().Any(ge =>
                                                     ge == null || ge is Group);
    }

    public bool DragLeave(DragLeaveEvent evt,
    ...
    public bool DragEnter(DragEnterEvent evt,
                          IEnumerable<ISelectable> selection,
    ...
    public bool DragPerform(DragPerformEvent evt,
                            IEnumerable<ISelectable> selection,
                            IDropTarget dropTarget, ISelection dragSource) {
      Group group = parent.GetFirstAncestorOfType<Group>();

      List<GraphElement> elemsToAdd =
          selection.Cast<GraphElement>()
              .Where(e => e != group && !group.containedElements.Contains(e) &&
                          !(e.GetContainingScope() is Group))
              .ToList(); // ToList required here as the enumeration might be
                         // done again *after* the elements are added to the
                         // group
    ...
    public bool DragUpdated(DragUpdatedEvent evt,
                            IEnumerable<ISelectable> selection,
                            IDropTarget dropTarget, ISelection dragSource) {
      Group group = parent.GetFirstAncestorOfType<Group>();
      bool canDrop = false;
