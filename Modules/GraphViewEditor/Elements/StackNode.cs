...
namespace UnityEditor.Experimental.GraphView {
  public partial class StackNode : Node {
    ...
    private static CustomStyleProperty<float> s_SeparatorHeight =
        new CustomStyleProperty<float>("--separator-height");
    private static CustomStyleProperty<float> s_SeparatorExtent =
        new CustomStyleProperty<float>("--separator-extent");
    ...
    public StackNode() : base("UXML/GraphView/StackNode.uxml") {
      VisualElement stackNodeContentContainerPlaceholder =
          this.Q("stackNodeContentContainerPlaceholder");
      ...
      m_PlaceholderContainer.Add(
          m_Placeholder = new StackNodePlaceholder("Spacebar to Add Node"));
    ...
    private bool AcceptsElementInternal(GraphElement element,
                                        ref int proposedIndex, int maxIndex) {
    ...
    protected virtual bool AcceptsElement(GraphElement element,
                                          ref int proposedIndex, int maxIndex) {
      return true;
    }
    ...
    private void UpdateSeparators() {
      ...
      if (m_SeparatorContainer.childCount < expectedSeparatorCount) {
        for (int i = m_SeparatorContainer.childCount;
             i < expectedSeparatorCount; ++i) {
      ...
      if (m_SeparatorContainer.childCount > expectedSeparatorCount) {
        for (int i = m_SeparatorContainer.childCount - 1;
             i >= expectedSeparatorCount; --i) {
          m_SeparatorContainer [i]
              .RemoveFromHierarchy();
        }
      }
      ...
      for (int i = 0; i < m_SeparatorContainer.childCount; ++i) {
        ...
        // .. for the other separators, use the spacing between the current and
        // the next separators
          ...
          separatorCenterY =
              (nextElement.layout.yMin + element.layout.yMax) / 2;
        }
        // .. for the last separator, use the bottom of the container
        else {
          separatorCenterY =
              m_SeparatorContainer.layout.height - separatorHeight / 2;
        }
        separator.style.top =
            separatorCenterY - separator.resolvedStyle.height / 2;
      }
    }
    ...
    private void
    ExecuteOnSeparatorContextualMenuEvent(ContextualMenuPopulateEvent evt,
                                          int separatorIndex) {
    ...
    OnSeparatorContextualMenuEvent(ContextualMenuPopulateEvent evt,
                                   int separatorIndex) {}
    ...
    public virtual int GetInsertionIndex(Vector2 worldPosition) {
      ...
      if (this == ve.GetFirstAncestorOfType<StackNode>()) {
        ...
        graphView.currentInsertLocation.GetInsertInfo(worldPosition,
                                                      out insertInfo);
    ...
    CollectElements(HashSet<GraphElement> collectedElementSet,
                    Func<GraphElement, bool> conditionFunc) {
      base.CollectElements(collectedElementSet, conditionFunc);
      GraphView.CollectElements(Children().OfType<GraphElement>(),
                                collectedElementSet, conditionFunc);
    }
  }
}
