...
namespace UnityEditor.Experimental.GraphView {
  public class Node : GraphElement, ICollectibleElement {
    ...
    public VisualElement titleButtonContainer {
      get;
      private set;
    }
    ...
    private void UpdateExpandedButtonState() {
      RemoveFromClassList(m_Expanded ? k_CollapsedStyleClass
                          : k_ExpandedStyleClass);
      AddToClassList(m_Expanded ? k_ExpandedStyleClass : k_CollapsedStyleClass);
    }

    public override Rect GetPosition() {
      if (resolvedStyle.position == Position.Absolute)
        return new Rect(resolvedStyle.left, resolvedStyle.top, layout.width,
                        layout.height);
      return layout;
    }
    ...
    public virtual Port InstantiatePort(Orientation orientation,
                                        Direction direction,
    ...
    private void SetElementVisible(VisualElement element, bool isVisible) {
      ...
      if (isVisible) {
        // Restore default value for visibility by setting it to
        // StyleKeyword.Null. Setting it to Visibility.Visible would make it
        // visible even if parent is hidden.
    ...
    public bool RefreshPorts() {
      ...
      bool outputVisible =
          outputCount > 0 || !AllElementsHidden(outputContainer);
    ...
    protected virtual void ToggleCollapse() { expanded = !expanded; }

    protected void UseDefaultStyling() {
      AddStyleSheetPath("StyleSheets/GraphView/Node.uss");
    }

    public Node() : this("UXML/GraphView/Node.uxml") { UseDefaultStyling(); }
    ...
    public Node(string uiFile) {
      ...
      if (borderContainer != null) {
        ...
        if (selection != null) {
          selection.style.overflow =
              Overflow.Visible; // fixes issues with selection border being
                                // clipped when zooming out
        }
      } else {
        mainContainer = main;
      }
      ...
      capabilities |= Capabilities.Selectable | Capabilities.Movable |
                      Capabilities.Deletable | Capabilities.Ascendable |
    ...
    void AddConnectionsToDeleteSet(VisualElement container,
                                   ref HashSet<GraphElement> toDelete) {
    ...
    void DisconnectAll(DropdownMenuAction a) {
      ...
      if (graphView != null) {
        ...
        Debug.Log(
            "Disconnecting nodes that are not in a GraphView will not work.");
      }
    }

    DropdownMenuAction.Status DisconnectAllStatus(DropdownMenuAction a) {
      VisualElement[] containers = {inputContainer, outputContainer};
    ...
    public virtual void BuildContextualMenu(ContextualMenuPopulateEvent evt) {
      if (evt.target is Node) {
        evt.menu.AppendAction("Disconnect all", DisconnectAll,
                              DisconnectAllStatus);
