...
public Group() {
	...
	var visualTree = EditorGUIUtility.Load("UXML/GraphView/GroupTitle.uxml")
	                 as VisualTreeAsset;
	...
	titleinput.RegisterCallback<FocusOutEvent>(
		e => { OnEditTitleFinished(); });
...
public override bool AcceptsElement(GraphElement element,
...
OnElementsAdded(IEnumerable<GraphElement> elements) {
	GraphView gv = GetFirstAncestorOfType<GraphView>();
...
internal void OnStartDragging(IMouseEvent evt,
                              IEnumerable<GraphElement> elements) {
	m_DropArea.OnStartDragging(evt, elements);
}

public void CollectElements(HashSet<GraphElement> collectedElementSet,
                            Func<GraphElement, bool> conditionFunc) {
	GraphView.CollectElements(containedElements, collectedElementSet,
	                          conditionFunc);
}
}
}
