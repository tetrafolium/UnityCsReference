...
namespace UnityEditor.Experimental.GraphView {
  public class Edge : GraphElement {
    ...
    private static CustomStyleProperty<int> s_EdgeWidthProperty =
        new CustomStyleProperty<int>("--edge-width");
    private static CustomStyleProperty<Color> s_SelectedEdgeColorProperty =
        new CustomStyleProperty<Color>("--selected-edge-color");
    private static CustomStyleProperty<Color> s_GhostEdgeColorProperty =
        new CustomStyleProperty<Color>("--ghost-edge-color");
    private static CustomStyleProperty<Color> s_EdgeColorProperty =
        new CustomStyleProperty<Color>("--edge-color");

    private static readonly int s_DefaultEdgeWidth = 2;
    private static readonly Color s_DefaultSelectedColor =
        new Color(240 / 255f, 240 / 255f, 240 / 255f);
    private static readonly Color s_DefaultColor =
        new Color(146 / 255f, 146 / 255f, 146 / 255f);
    private static readonly Color s_DefaultGhostColor =
        new Color(85 / 255f, 85 / 255f, 85 / 255f);
    ...
    public override bool Overlaps(Rect rectangle) {
      ...
      return edgeControl.Overlaps(
          this.ChangeCoordinatesTo(edgeControl, rectangle));
    }
    ...
    public override bool ContainsPoint(Vector2 localPoint) {
      ...
      var result = UpdateEdgeControl() &&
                   edgeControl.ContainsPoint(
                       this.ChangeCoordinatesTo(edgeControl, localPoint));
    ...
    public virtual void OnPortChanged(bool isInput) {
      edgeControl.outputOrientation = m_OutputPort?.orientation ??(
          m_InputPort?.orientation ?? Orientation.Horizontal);
      edgeControl.inputOrientation = m_InputPort?.orientation ??(
          m_OutputPort?.orientation ?? Orientation.Horizontal);
      UpdateEdgeControl();
    }
      ...
      if (selected) {
        ...
        if (isGhostEdge) {
          edgeControl.inputColor =
              new Color(edgeControl.inputColor.r, edgeControl.inputColor.g,
                        edgeControl.inputColor.b, 0.5f);
          edgeControl.outputColor =
              new Color(edgeControl.outputColor.r, edgeControl.outputColor.g,
                        edgeControl.outputColor.b, 0.5f);
        }
      }
    }
    ...
    public override void OnSelected() { UpdateEdgeControlColorsAndWidth(); }

    public override void OnUnselected() { UpdateEdgeControlColorsAndWidth(); }

    protected virtual EdgeControl CreateEdgeControl() {
      return new EdgeControl{capRadius = k_EndPointRadius,
    ...
    void TrackGraphElement(Port port) {
      if (port.panel !=
          null) // if the panel is null therefore the port is not yet attached
                // to its hierarchy, so postpone the register
      {
    ...
    void OnEdgeAttach(AttachToPanelEvent e) { UpdateEdgeControl(); }
    ...
    void DoTrackGraphElement(Port port) {
      ...
      while (current != null) {
        ...
        if (current !=
            port.node) // if we encounter our node ignore it but continue in the
                       // case there are nodes inside nodes
        {
    ...
    private void UpdateEdgeControlEndPoints() {
      ...
      m_GlobalCandidatePosition = this.WorldToLocal(m_CandidatePosition);
      if (m_OutputPort != null || m_InputPort != null) {
        edgeControl.to = (m_InputPort != null) ? GetPortPosition(m_InputPort)
                                               : m_GlobalCandidatePosition;
        edgeControl.from = (m_OutputPort != null)
                               ? GetPortPosition(m_OutputPort)
                               : m_GlobalCandidatePosition;
      }
