...
namespace UnityEditor.Experimental.GraphView {
  ...
  public class StickyNoteChangeEvent : EventBase<StickyNoteChangeEvent> {
    public static StickyNoteChangeEvent GetPooled(StickyNote target,
                                                  StickyNoteChange change) {
      var evt = GetPooled();
      evt.target = target;
      evt.change = change;
      return evt;
    ...
    public StickyNoteChange change {
      get;
      protected set;
    }
  }
  public enum StickyNoteTheme { Classic, Black }
  public enum StickyNoteFontSize { Small, Medium, Large, Huge }
  ...
  public class StickyNote : GraphElement, IResizable {
    ...
    public StickyNote(Vector2 position)
        : this("UXML/GraphView/StickyNote.uxml", position) {
      styleSheets.Add(EditorGUIUtility.Load(
          "StyleSheets/GraphView/Selectable.uss") as StyleSheet);
      styleSheets.Add(EditorGUIUtility.Load(
          "StyleSheets/GraphView/StickyNote.uss") as StyleSheet);
    }
    ...
    public StickyNote(string uiFile, Vector2 position) {
      ...
      capabilities = Capabilities.Movable | Capabilities.Deletable |
                     Capabilities.Ascendable | Capabilities.Selectable |
      ...
      if (m_TitleField != null) {
        ...
        m_TitleField.Q("unity-text-input")
            .RegisterCallback<BlurEvent>(OnTitleBlur);
      ...
      if (m_Contents != null) {
        ...
        if (m_ContentsField != null) {
          ...
          m_ContentsField.Q("unity-text-input")
              .RegisterCallback<BlurEvent>(OnContentsBlur);
        }
    ...
    void OnFitToText(DropdownMenuAction a) { FitText(false); }

    public void FitText(bool onlyIfSmaller) {
      Vector2 preferredTitleSize = Vector2.zero;
      if (!string.IsNullOrEmpty(m_Title.text))
        preferredTitleSize = m_Title.MeasureTextSize(
            m_Title.text, 0, MeasureMode.Undefined, 0,
            MeasureMode.Undefined); // This is the size of the string with the
                                    // current title font and such

      preferredTitleSize += AllExtraSpace(m_Title);
      preferredTitleSize.x +=
          m_Title.ChangeCoordinatesTo(this, Vector2.zero).x +
          resolvedStyle.width -
          m_Title
              .ChangeCoordinatesTo(this, new Vector2(m_Title.layout.width, 0))
              .x;

      Vector2 preferredContentsSizeOneLine = m_Contents.MeasureTextSize(
          m_Contents.text, 0, MeasureMode.Undefined, 0, MeasureMode.Undefined);

      Vector2 contentExtraSpace = AllExtraSpace(m_Contents);
      preferredContentsSizeOneLine += contentExtraSpace;

      Vector2 extraSpace =
          new Vector2(resolvedStyle.width, resolvedStyle.height) -
          m_Contents.ChangeCoordinatesTo(
              this,
              new Vector2(m_Contents.layout.width, m_Contents.layout.height));
      ...
      if (preferredContentsSizeOneLine.x <
          Mathf.Max(preferredTitleSize.x, resolvedStyle.width)) {
      ...
      } else // The width is not enough for the content: keep the width or use
             // the title width if bigger.
      {
        width =
            Mathf.Max(preferredTitleSize.x + extraSpace.x, resolvedStyle.width);
        float contextWidth = width - extraSpace.x - contentExtraSpace.x;
        Vector2 preferredContentsSize = m_Contents.MeasureTextSize(
            m_Contents.text, contextWidth, MeasureMode.Exactly, 0,
    ...
    void UpdateThemeClasses() {
      foreach (StickyNoteTheme value in System.Enum.GetValues(
                   typeof(StickyNoteTheme))) {
        if (m_Theme != value)
    ...
    void UpdateSizeClasses() {
      foreach (StickyNoteFontSize value in System.Enum.GetValues(
                   typeof(StickyNoteFontSize))) {
        if (m_FontSize != value)
    ...
    public void BuildContextualMenu(ContextualMenuPopulateEvent evt) {
      if (evt.target is StickyNote) {
        if (theme == StickyNoteTheme.Black)
          evt.menu.AppendAction("Light Theme", OnChangeTheme,
                                e => DropdownMenuAction.Status.Normal,
                                StickyNoteTheme.Classic);
        else
          evt.menu.AppendAction("Dark Theme", OnChangeTheme,
                                e => DropdownMenuAction.Status.Normal,
                                StickyNoteTheme.Black);

        foreach (StickyNoteFontSize value in System.Enum.GetValues(
                     typeof(StickyNoteFontSize))) {
          evt.menu.AppendAction(value.ToString() + " Text Size", OnChangeSize,
                                e => DropdownMenuAction.Status.Normal, value);
        }
        evt.menu.AppendSeparator();

        evt.menu.AppendAction("Fit To Text", OnFitToText,
                              e => DropdownMenuAction.Status.Normal);
        evt.menu.AppendSeparator();
      }
    }

    void OnTitleChange(EventBase e) { title = m_TitleField.value; }
    ...
    public override Rect GetPosition() {
      return new Rect(resolvedStyle.left, resolvedStyle.top,
    ...
    public virtual void OnResized() { NotifyChange(StickyNoteChange.Position); }
    ...
    void OnTitleRelayout(GeometryChangedEvent e) { UpdateTitleFieldRect(); }
    ...
    void UpdateTitleFieldRect() {
      ...
      m_TitleField.style.width = rect.width - m_Title.resolvedStyle.marginLeft -
                                 m_Title.resolvedStyle.marginRight;
      m_TitleField.style.height = rect.height -
                                  m_Title.resolvedStyle.marginTop -
                                  m_Title.resolvedStyle.marginBottom;
    }
    ...
    void NotifyChange(StickyNoteChange change) {
      panel.dispatcher.Dispatch(StickyNoteChangeEvent.GetPooled(this, change),
                                panel, DispatchMode.Queued);
    }
    ...
    static Vector2 AllExtraSpace(VisualElement element) {
      return new Vector2(
          element.resolvedStyle.marginLeft + element.resolvedStyle.marginRight +
          ...
          element.resolvedStyle.marginTop + element.resolvedStyle.marginBottom +
              element.resolvedStyle.paddingTop +
              element.resolvedStyle.paddingBottom +
              element.resolvedStyle.borderBottomWidth +
              element.resolvedStyle.borderTopWidth);
    }
  }
}
