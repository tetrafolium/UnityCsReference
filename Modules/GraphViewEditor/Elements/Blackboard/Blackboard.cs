...
namespace UnityEditor.Experimental.GraphView {
  public class Blackboard : GraphElement, ISelection {
    ...
    internal static readonly string StyleSheetPath =
        "StyleSheets/GraphView/Blackboard.uss";

    public Action<Blackboard> addItemRequested {
      get;
      set;
    }
    public Action<Blackboard, int, VisualElement> moveItemRequested {
      get;
      set;
    }
    public Action<Blackboard, VisualElement, string> editTextRequested {
      get;
      set;
    }
    ...
    public List<ISelectable> selection {
      get { return graphView?.selection; }
    }
    ...
    public bool scrollable {
      ...
      set {
        ...
        if (m_Scrollable) {
          ...
          // Remove the sections container from the content item and add it to
          // the scrollview
          m_ContentContainer.RemoveFromHierarchy();
          m_Root.Add(m_ScrollView);
          m_ScrollView.Add(m_ContentContainer);
          resizeRestriction =
              ResizeRestriction.None; // As both the width and height can be
                                      // changed by the user using a resizer

          AddToClassList("scrollable");
        } else {
          if (m_ScrollView != null) {
            // Remove the sections container from the scrollview and add it to
            // the content item
            resizeRestriction =
                ResizeRestriction
                    .FlexDirection; // As the height is automatically computed
                                    // from the content but the width can be
                                    // changed by the user using a resizer
    ...
    public Blackboard(GraphView associatedGraphView = null) {
      var tpl = EditorGUIUtility.Load("UXML/GraphView/Blackboard.uxml")
                    as VisualTreeAsset;
      ...
      // event interception to prevent GraphView manipulators from being
      // triggered when working with the blackboard
    ...
    public virtual void ClearSelection() { graphView?.ClearSelection(); }
