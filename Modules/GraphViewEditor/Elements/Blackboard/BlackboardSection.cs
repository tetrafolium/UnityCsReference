...
Blackboard blackboard {
	get {
		return m_Blackboard ??(m_Blackboard =
					       GetFirstAncestorOfType<Blackboard>());
	}
}

public delegate bool CanAcceptDropDelegate(ISelectable selected);

public CanAcceptDropDelegate canAcceptDrop {
	get;
	set;
}
...
public BlackboardSection() {
	var tpl = EditorGUIUtility.Load("UXML/GraphView/BlackboardSection.uxml")
	          as VisualTreeAsset;
...
private void OnDragUpdatedEvent(DragUpdatedEvent evt) {
	var selection =
		DragAndDrop.GetGenericData("DragSelection") as List<ISelectable>;
	...
	if (m_InsertIndex != -1) {
		...
		if (m_InsertIndex == childCount) {
			...
			indicatorY =
				...
				.ChangeCoordinatesTo(
					this,
					new Vector2(0, lastChild.layout.height +
					            lastChild.resolvedStyle.marginBottom))
				.y;
		} else {
			...
			indicatorY =
				...
				.ChangeCoordinatesTo(
					this, new Vector2(
						0, -childAtInsertIndex.resolvedStyle.marginTop))
				.y;
		}

		SetDragIndicatorVisible(true);

		m_DragIndicator.layout =
			new Rect(0, indicatorY - m_DragIndicator.layout.height / 2,
			         layout.width, m_DragIndicator.layout.height);
	} else {
		SetDragIndicatorVisible(false);
...
private void OnDragPerformEvent(DragPerformEvent evt) {
	...
	if (m_InsertIndex != -1) {
		List<Tuple<VisualElement, VisualElement> > draggedElements =
			new List<Tuple<VisualElement, VisualElement> >();

		foreach (ISelectable selectedElement in selection) {
			var draggedElement = selectedElement as VisualElement;

			if (draggedElement != null && Contains(draggedElement)) {
				draggedElements.Add(new Tuple<VisualElement, VisualElement>(
							    FindSectionDirectChild(draggedElement), draggedElement));
			}
		}
		...
		draggedElements.Sort((pair1, pair2) => {
					return IndexOf(pair1.Item1).CompareTo(IndexOf(pair2.Item1));
				});

		int insertIndex = m_InsertIndex;

		foreach (Tuple<VisualElement, VisualElement> draggedElement in
		         draggedElements) {
			VisualElement sectionDirectChild = draggedElement.Item1;
			int indexOfDraggedElement = IndexOf(sectionDirectChild);

			if (!((indexOfDraggedElement == insertIndex) ||
			      ((insertIndex - 1) == indexOfDraggedElement))) {
				if (blackboard.moveItemRequested != null) {
					blackboard.moveItemRequested(blackboard, m_InsertIndex,
					                             draggedElement.Item2);
				} else {
			...
			if (insertIndex >
			    indexOfDraggedElement) // No need to increment the insert index
				                   // for the next dragged element if the
				                   // current dragged element is above the
