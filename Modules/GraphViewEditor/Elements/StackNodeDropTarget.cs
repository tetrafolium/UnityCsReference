...
private int m_CurrentInsertIndex;     // The current index where the dragged
                                      // item will be inserted once dropped

private const string k_PreviewClass = "stack-node-preview";
private Func<GraphElement, VisualElement> m_DropPreviewTemplate;

private bool m_InstantAdd =
	false; // Temporarily set to true right after an item is detached from
               // the stack to show its preview right away (instead of being
               // animated)
...
private static CustomStyleProperty<int> s_AnimationDuration =
	new CustomStyleProperty<int>("--animation-duration");
...
protected bool dragEntered {
	...
	private set {
		...
		m_DragEntered = value;
		m_SeparatorContainer.visible =
			!value; // Hide the separators when while dragging
...
DefaultDropPreviewTemplate(GraphElement source) {
...
private void AddPreview(IList<VisualElement> previews, int index,
                        bool animated) {
	...
	if (animated) {
		...
		foreach (VisualElement preview in m_CurrentPreviews) {
			Vector2 size = new Vector2(preview.style.width.value.value,
			                           preview.style.height.value.value);
			Rect startRect = new Rect(Vector2.zero, size);
			Rect endRect = startRect;

			startRect.height = 0f;

			var addAnimation = preview.experimental.animation
			                   .Start(startRect, endRect, animationDuration,
			                          UpdatePreviewLayout)
			                   .KeepAlive();
	...
	if (animated) {
		...
		foreach (VisualElement preview in m_RemovedPreviews) {
			...
			var removeAnimations =
				preview.experimental.animation
				.Start(startRect, endRect, animationDuration,
				       UpdatePreviewLayout)
				.KeepAlive();
...
private void OnRemoveAnimationFinished() {
	...
	// If a new preview was added while another one was been removed and that
	// a drag exit occurred before the RemoveAnimation finished then removed
	// this added preview
...
private void HandleDragAndDropEvent(IMouseEvent evt) {
	if ((m_AddAnimations != null && m_AddAnimations.Any(a => a.isRunning)) ||
	    (m_RemoveAnimations != null &&
	     m_RemoveAnimations.Any(a => a.isRunning)))
		return;
	...
	int insertIndex = 0;
	Vector2 localMousePosition = graphView.ChangeCoordinatesTo(
		contentContainer, graphView.WorldToLocal(evt.mousePosition));
	int previewIndex = m_CurrentPreviews?.FirstOrDefault()
	                   ?.parent.IndexOf(m_CurrentPreviews.First())
	                   ?? -1;
	int maxIndex = 0;

	// If there is no child then add at index 0
	if (childCount != 0) {
		// Determine the insert index by traversing the stack from top to bottom
		// ignoring the previews and footer. For each child element, use the
		// vertical center as reference to determine if the dragged element
		// should be inserted above or below it.
		foreach (VisualElement child in Children()) {
			Rect rect = child.layout;

			if ((m_RemovedPreviews == null ||
			     !m_RemovedPreviews.Contains(child)) &&
			    (m_CurrentPreviews == null ||
			     !m_CurrentPreviews.Contains(child))) {
				maxIndex++;
	...
	// If there is already a preview being removed then adjust the insert
	// index of the new preview
	if (m_RemovedPreviews != null) {
		int removePreviewIndex = m_RemovedPreviews.FirstOrDefault()
		                         ?.parent.IndexOf(m_RemovedPreviews.First())
		                         ?? -1;
...
protected virtual bool hasMultipleSelectionSupport {
	get { return false; }
}
...
public virtual bool DragPerform(DragPerformEvent evt,
	...
	if (m_CurrentInsertIndex != -1) {
		// Notify the model that an element should be inserted at the specified
		// index
		if (graphView != null &&
		    graphView.elementsInsertedToStackNode != null) {
			graphView.elementsInsertedToStackNode(this, m_CurrentInsertIndex,
			                                      m_DraggedElements);
		}
...
public virtual bool DragUpdated(DragUpdatedEvent evt,
                                IEnumerable<ISelectable> selection,
                                IDropTarget dropTarget,
                                ISelection dragSource) {
	HandleDragAndDropEvent(evt);
...
public virtual bool DragEnter(DragEnterEvent evt,
                              IEnumerable<ISelectable> selection,
                              IDropTarget enteredTarget,
                              ISelection dragSource) {
	int proposedIndex = -1;
	int maxIndex = 1;
	// We just want to know which elements are accepted. The actual index is
	// not relevant to us at this point.
	m_DraggedElements =
		selection.OfType<GraphElement>()
		.Where(e => e != this && AcceptsElementInternal(
			       e, ref proposedIndex, maxIndex))
		.ToList();

	return true;
}

public virtual bool DragLeave(DragLeaveEvent evt,
                              IEnumerable<ISelectable> selection,
