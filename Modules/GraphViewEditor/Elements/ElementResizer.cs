...
namespace UnityEditor.Experimental.GraphView {
  internal class ElementResizer : MouseManipulator {
    ...
    public ElementResizer(VisualElement resizedElement,
                          ResizerDirection direction) {
    ...
    void OnMouseDown(MouseDownEvent e) {
      ...
      m_StartSize = new Vector2(resizedTarget.resolvedStyle.width,
                                resizedTarget.resolvedStyle.height);
      m_StartPosition = new Vector2(resizedTarget.resolvedStyle.left,
                                    resizedTarget.resolvedStyle.top);

      bool minWidthDefined =
          resizedTarget.resolvedStyle.minWidth != StyleKeyword.Auto;
      bool maxWidthDefined =
          resizedTarget.resolvedStyle.maxWidth != StyleKeyword.None;
      bool minHeightDefined =
          resizedTarget.resolvedStyle.minHeight != StyleKeyword.Auto;
      bool maxHeightDefined =
          resizedTarget.resolvedStyle.maxHeight != StyleKeyword.None;
      m_MinSize = new Vector2(
          minWidthDefined ? resizedTarget.resolvedStyle.minWidth.value
          : Mathf.NegativeInfinity,
            minHeightDefined ? resizedTarget.resolvedStyle.minHeight.value
          : Mathf.NegativeInfinity);
      m_MaxSize = new Vector2(
          maxWidthDefined ? resizedTarget.resolvedStyle.maxWidth.value
          : Mathf.Infinity,
            maxHeightDefined ? resizedTarget.resolvedStyle.maxHeight.value
          : Mathf.Infinity);
    ...
    void OnMouseMove(MouseMoveEvent e) {
      ...
      if (resizedTarget.isLayoutManual) {
        ...
        if ((direction & ResizerDirection.Right) != 0) {
          layout.width = Mathf.Min(
              m_MaxSize.x, Mathf.Max(m_MinSize.x, m_StartSize.x + mousePos.x -
                                                      m_StartMouse.x));
        ...
        if ((direction & ResizerDirection.Bottom) != 0) {
          layout.height = Mathf.Min(
              m_MaxSize.y, Mathf.Max(m_MinSize.y, m_StartSize.y + mousePos.y -
                                                      m_StartMouse.y));
        ...
        if ((direction & ResizerDirection.Right) != 0) {
          resizedTarget.style.width = Mathf.Min(
              m_MaxSize.x, Mathf.Max(m_MinSize.x, m_StartSize.x + mousePos.x -
                                                      m_StartMouse.x));
        } else if ((direction & ResizerDirection.Left) != 0) {
          float delta = mousePos.x - m_StartMouse.x;
        ...
        if ((direction & ResizerDirection.Bottom) != 0) {
          resizedTarget.style.height = Mathf.Min(
              m_MaxSize.y, Mathf.Max(m_MinSize.y, m_StartSize.y + mousePos.y -
                                                      m_StartMouse.y));
        } else if ((direction & ResizerDirection.Top) != 0) {
          float delta = mousePos.y - m_StartMouse.y;
    ...
    void OnMouseUp(MouseUpEvent e) {
      ...
      if (m_Active) {
        VisualElement resizedTarget = resizedElement.parent;
        if (resizedTarget.style.width != m_StartSize.x ||
            resizedTarget.style.height != m_StartSize.y) {
          if (resizedTarget is IResizable)
