...
namespace UnityEditor.Experimental.GraphView {
  public partial class Scope : GraphElement {
    ...
    private bool m_IsUpdatingGeometryFromContent; // To avoid recursive calls to
                                                  // UpdateGeometryFromContent

    internal bool hasPendingGeometryUpdate {
      get;
      private set;
    }
    ...
    public VisualElement headerContainer {
      get { return m_HeaderContainer; }
    }
    public IEnumerable<GraphElement> containedElements {
      get { return m_ContainedElements; }
    }
    public Rect containedElementsRect {
      get {
        return m_ContentContainer.ChangeCoordinatesTo(this,
                                                      m_ContentContainer.rect);
      }
    }

    public Scope() {
      var visualTree =
          EditorGUIUtility.Load("UXML/GraphView/Scope.uxml") as VisualTreeAsset;
      ...
      VisualElement contentContainerPlaceholder =
          m_MainContainer.Q(name
    ...
    internal static bool IsValidRect(Rect rect) {
      return !Single.IsNaN(rect.x + rect.y + rect.width + rect.height) &&
             rect.width > 0 && rect.height > 0;
    }

    public override bool HitTest(Vector2 localPoint) {
      Vector2 mappedPoint =
          this.ChangeCoordinatesTo(m_HeaderContainer, localPoint);
    ...
    public bool ContainsElement(GraphElement element) {
      return (m_ContainedElements != null)
                 ? m_ContainedElements.Contains(element)
                 : false;
    }

    public virtual bool AcceptsElement(GraphElement element,
    ...
    private void AddElementInternal(GraphElement element) {
      ...
      if (containedElements.Contains(element)) {
        throw new ArgumentException(
            "The specified element is already contained in this scope.");
      }

      string reasonWhyNotAccepted =
          "Cannot add the specified element to this scope.";
      ...
      // To update the scope geometry whenever the added element's geometry
      // changes
    ...
    public void
    RemoveElementsWithoutNotification(IEnumerable<GraphElement> elements) {
      foreach (GraphElement element in elements) {
    ...
    private void RemoveElementInternal(GraphElement element) {
      if (element == null)
        throw new ArgumentException(
            "Cannot remove null element from this scope");

      if (!m_ContainedElements.Contains(element))
        throw new ArgumentException(
            "This element is not contained in this scope");

      element.UnregisterCallback<GeometryChangedEvent>(
          OnSubElementGeometryChanged);
    ...
    OnElementsRemoved(IEnumerable<GraphElement> elements) {}
    ...
    public void UpdateGeometryFromContent() {
      ...
      try {
        // Dirty the layout of the content container to recompute the content
        // bounding rect
        ...
        // Need to mark actual children as dirty repaint so they properly move
        // with the changes in size/position of the scope. The number of actual
        // children should be low given the Scope (and Group) nodes do not
        // parent their child nodes. This is mostly to fix things like the title
        // element in the Group node.
        MarkChildrenDirtyRepaint(this);
        // We also need to mark the actual children bounding boxes as dirty,
        // since the element dimensions may have changed.
        MarkBoundingBoxesDirty(this);

        if (m_ContainedElements.Count > 0) {
          // Match the top left corner of the content container to the top left
          // corner of the bounding box of the contained elements
          Rect elemRectInLocalSpace = containedElementsRect;

          float xOffset = elemRectInLocalSpace.x;
          float yOffset = elemRectInLocalSpace.y;
          Vector2 newPosition =
              m_ContentContainer.contentRectInViewportSpace.position -
    ...
    public override void SetPosition(Rect newPos) {
      if ((!IsValidRect(newPos) && m_ContainedElements.Any()) ||
