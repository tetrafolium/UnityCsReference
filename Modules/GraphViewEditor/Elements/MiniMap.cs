...
namespace UnityEditor.Experimental.GraphView {
  public class MiniMap : GraphElement {
    ...
    Rect m_ContentRect; // Rect that represents the rect needed to encompass all
                        // Graph Elements
    Rect m_ContentRectLocal; // Rect that represents the rect needed to
                             // encompass all Graph Elements in local coords

    int titleBarOffset {
      get { return (int) resolvedStyle.paddingTop; }
    }
    ...
    void ToggleAnchorState(DropdownMenuAction a) { anchored = !anchored; }

    public virtual void BuildContextualMenu(ContextualMenuPopulateEvent evt) {
      if (!windowed)
        evt.menu.AppendAction(anchored ? "Make floating"
                              : "Anchor", ToggleAnchorState,
                                DropdownMenuAction.AlwaysEnabled);
    }

    public void OnResized() { Resize(); }
    ...
    void Resize() {
      ...
      // Relocate if partially visible on bottom or right side (left/top not
      // checked, only bottom/right affected by a size change)
      if (resolvedStyle.left + resolvedStyle.width >
          parent.layout.x + parent.layout.width) {
        var newPosition = layout;
        newPosition.x -= resolvedStyle.left + resolvedStyle.width -
                         (parent.layout.x + parent.layout.width);
        layout = newPosition;
      }

      if (resolvedStyle.top + resolvedStyle.height >
          parent.layout.y + parent.layout.height) {
        var newPosition = layout;
        newPosition.y -= resolvedStyle.top + resolvedStyle.height -
                         (parent.layout.y + parent.layout.height);
        layout = newPosition;
      }
    ...
    static void ChangeToMiniMapCoords(ref Rect rect, float factor,
                                      Vector3 translation) {
      // Apply factor
      rect.width *= factor;
      rect.height *= factor;
    ...
    void CalculateRects(VisualElement container) {
      ...
      var containerInvScale =
          new Vector2(containerInvTransform.m00, containerInvTransform.m11);
      ...
      SetZoomFactorText(UnityString.Format("{0:F2}", containerZoomFactor) +
                        "x");
      ...
      var minimapTranslation =
          new Vector3(-totalRect.x, titleBarOffset - totalRect.y);
      ChangeToMiniMapCoords(ref m_ViewportRect, minimapFactor,
                            minimapTranslation);
      ChangeToMiniMapCoords(ref m_ContentRect, minimapFactor,
                            minimapTranslation);

      // Diminish and center everything to fit vertically
      if (totalRect.height > (effectiveHeight - titleBarOffset)) {
        float totalRectFactor =
            (effectiveHeight - titleBarOffset) / totalRect.height;
        float totalRectOffsetX =
            (effectiveWidth - (totalRect.width * totalRectFactor)) / 2.0f;
        float totalRectOffsetY =
            titleBarOffset -
            ((totalRect.y + minimapTranslation.y) * totalRectFactor);
    ...
    Rect CalculateElementRect(GraphElement elem) {
      Rect rect =
          elem.ChangeCoordinatesTo(graphView.contentViewContainer, elem.rect);
      rect.x =
          m_ContentRect.x + ((rect.x - m_ContentRectLocal.x) *
                             m_ContentRect.width / m_ContentRectLocal.width);
      rect.y =
          m_ContentRect.y + ((rect.y - m_ContentRectLocal.y) *
                             m_ContentRect.height / m_ContentRectLocal.height);
      ...
      // (except yMin since we already have the titleBar offset which is enough
      // for clipping)
      var xMin = 2;
    ...
    private void OnGenerateVisualContent(MeshGenerationContext mgc) {
      ...
      // mode rendering at all. It should maintain its vertex/index lists and
      // only update affected vertices when their respective elements are
      // changed. This way the cost of GenerateVisualContent becomes effectively
      // only two memcpys.
      mgc.painter.DrawImmediate(DrawMinimapContent, true);
    }

    void DrawSolidRectangleWithOutline(ref Vector3[] cachedRect,
                                       Color faceColor, Color typeColor) {
    ...
    void DrawMinimapContent() {
      ...
      var containerScale =
          new Vector2(containerTransform.m00, containerTransform.m11);
      float containerWidth = parent.layout.width / containerScale.x;
      float containerHeight = parent.layout.height / containerScale.y;

      if (Mathf.Abs(containerWidth - m_PreviousContainerWidth) >
              Mathf.Epsilon ||
          Mathf.Abs(containerHeight - m_PreviousContainerHeight) >
              Mathf.Epsilon) {
    ...
    void DrawElements() {
      ...
      foreach (var placemat in placemats.Where(p => p.showInMiniMap &&
                                                    p.visible)) {
        var elemRect = CalculateElementRect(placemat);

        s_CachedRect [0]
            .Set(elemRect.xMin, elemRect.yMin, 0.0f);
        s_CachedRect [1]
            .Set(elemRect.xMax, elemRect.yMin, 0.0f);
        s_CachedRect [2]
            .Set(elemRect.xMax, elemRect.yMax, 0.0f);
        s_CachedRect [3]
            .Set(elemRect.xMin, elemRect.yMax, 0.0f);

        Color fillColor = placemat.resolvedStyle.backgroundColor;
        fillColor.a = 0.15f;

        DrawSolidRectangleWithOutline(ref s_CachedRect, fillColor,
                                      m_PlacematBorderColor);
      }
      ...
      graphView.graphElements.ForEach(elem => {
        ...
        DrawSolidRectangleWithOutline(ref s_CachedRect, elem.elementTypeColor,
                                      elem.elementTypeColor);
    ...
    private void EatMouseDown(MouseDownEvent e) {
      // The minimap should not let any left mouse down go through when it's not
      // movable.
