...
public ResizableElement(string uiFile) {
	...
	var sheet = EditorGUIUtility.Load("StyleSheets/GraphView/Resizable.uss")
	            as StyleSheet;
	styleSheets.Add(sheet);

	tpl.CloneTree(this);

	foreach (ResizerDirection value in System.Enum.GetValues(
	...
	foreach (ResizerDirection vertical in new[] {ResizerDirection.Top,
	                                             ResizerDirection.Bottom})
		foreach (ResizerDirection horizontal in new[] {ResizerDirection.Left,
		                                               ResizerDirection.Right}) {
			VisualElement resizer =
				this.Q(vertical.ToString().ToLower() + "-" +
				       horizontal.ToString().ToLower() + "-resize");
			if (resizer != null)
				resizer.AddManipulator(
					new ElementResizer(this, vertical | horizontal));
		}
}
}
}
