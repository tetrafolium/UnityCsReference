...
namespace UnityEditor.Experimental.GraphView {
  ...
  // TODO: This is a straight port from Canvas2D. I don't think that having to
  // check for types in the assembly using reflection is the way we want to go.
  public class NodeAdapter {
    ...
    public bool CanAdapt(object a, object b) {
      ...
      if (mi == null) {
        Debug.Log("adapter node not found for: " + a.GetType() + " -> " +
                  b.GetType());
      }
      return mi != null;
    }
    ...
    public bool Connect(object a, object b) {
      MethodInfo mi = GetAdapter(a, b);
      if (mi == null) {
        Debug.LogError("Attempt to connect 2 unadaptable types: " +
                       a.GetType() + " -> " + b.GetType());
        return false;
      }
    ...
    public MethodInfo GetAdapter(object a, object b) {
        ...
        foreach (Assembly assembly in currentDomain.GetAssemblies()) {
          ...
          foreach (MethodInfo method in methods) {
            ...
            if (methodParams.Length == 3) {
              string pa = methodParams[1].ParameterType +
                          methodParams[2].ParameterType.ToString();
      ...
      return s_NodeAdapterDictionary.TryGetValue(s.GetHashCode(),
                                                 out methodInfo)
                 ? methodInfo
                 : null;
    }
          ...
          try {
            foreach (Type temptype in assembly.GetTypes()) {
              ...
              foreach (MethodInfo i in methodInfos) {
                object[] allAttrs =
                    i.GetCustomAttributes(typeof(TypeAdapter), false);
                if (allAttrs.Any()) {
