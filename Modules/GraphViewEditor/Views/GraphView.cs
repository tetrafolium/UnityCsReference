...
namespace UnityEditor.Experimental.GraphView {
  ...
  internal class GraphViewUndoRedoSelection : ScriptableObject,
                                              IGraphViewSelection,
                                              ISerializationCallbackReceiver {
    [SerializeField]
    private int m_Version;

    [SerializeField] string[] m_SelectedElementsArray;
    ...
    public void OnAfterDeserialize() {
      ...
      if (m_SelectedElementsArray == null ||
          m_SelectedElementsArray.Length == 0)
        return;
  ...
  public abstract class GraphView : VisualElement, ISelection {
    ...
    internal IInsertLocation currentInsertLocation {
      get;
      set;
    }
    ...
    public GraphViewChanged graphViewChanged {
      get;
      set;
    }

    public Action<Group, string> groupTitleChanged {
      get;
      set;
    }
    public Action<Group, IEnumerable<GraphElement>> elementsAddedToGroup {
      get;
      set;
    }
    public Action<Group, IEnumerable<GraphElement>> elementsRemovedFromGroup {
      get;
      set;
    }

    public Action<StackNode, int, IEnumerable<GraphElement>>
        elementsInsertedToStackNode {
      get;
      set;
    }
    public Action<StackNode, IEnumerable<GraphElement>>
    ...
    public ElementResized elementResized {
      get;
      set;
    }

    public delegate void ViewTransformChanged(GraphView graphView);
    public ViewTransformChanged viewTransformChanged {
      get;
      set;
    }

    // BE AWARE: This is just a stopgap measure to get the minimap notified and
    // should not be used outside of it. This should also get ripped once the
    // minimap is re-written.
    internal Action redrawn {
      get;
      set;
    }
    ...
    private const string k_SelectionUndoRedoLabel =
        "Change GraphView Selection";
    ...
    class ContentViewContainer : VisualElement {
      public override bool Overlaps(Rect r) { return true; }
    }
    ...
    public void UpdateViewTransform(Vector3 newPosition, Vector3 newScale) {
      float validateFloat = newPosition.x + newPosition.y + newPosition.z +
    ...
    public enum FrameType { All = 0, Selection = 1, Origin = 2 }

    readonly int k_FrameBorder = 30;

    readonly Dictionary<int, Layer> m_ContainerLayers =
    ...
    public bool GetPortCenterOverride(Port port,
                                      out Vector2 overriddenPosition) {
    ...
    protected GraphView() {
      ...
      contentViewContainer = new ContentViewContainer{
          name = "contentViewContainer", pickingMode = PickingMode.Ignore,
          usageHints = UsageHints.GroupTransform};

      // make it absolute and 0 sized so it acts as a transform to move children
      // to and fro
      graphViewContainer.Add(contentViewContainer);

      AddStyleSheetPath("StyleSheets/GraphView/GraphView.uss");
      graphElements = contentViewContainer.Query<GraphElement>()
                          .Where(e => !(e is Port))
                          .Build();
      ...
      ports = contentViewContainer.Query()
                  .Children<Layer>()
                  .Descendents<Port>()
                  .Build();
    ...
    private void RestoreSavedSelection(IGraphViewSelection graphViewSelection) {
      if (m_PersistedSelection == null)
        return;
      if (graphViewSelection.selectedElements.Count == selection.Count &&
          graphViewSelection.version == m_SavedSelectionVersion)
        return;
    ...
    internal void RestorePersitentSelectionForElement(GraphElement element) {
      ...
      if (m_PersistedSelection.selectedElements.Count == selection.Count &&
          m_PersistedSelection.version == m_SavedSelectionVersion)
        return;
    ...
    private void RecordSelectionUndoPre() {
      ...
      Undo.RegisterCompleteObjectUndo(m_GraphViewUndoRedoSelection,
                                      k_SelectionUndoRedoLabel);
    }
    ...
    void AddLayer(Layer layer, int index) {
      ...
      int indexOfLayer = m_ContainerLayers.OrderBy(t => t.Key)
                             .Select(t => t.Value)
                             .ToList()
                             .IndexOf(layer);
    ...
    VisualElement GetLayer(int index) { return m_ContainerLayers[index]; }
    ...
    internal void ChangeLayer(GraphElement element) {
      ...
      if (selected)
        element.UnregisterCallback<DetachFromPanelEvent>(
    ...
    public UQueryState<GraphElement> graphElements {
      get;
      private set;
    }
    private UQueryState<GraphElement> allGraphElements { get; }
    public UQueryState<Node> nodes {
      get;
      private set;
    }
    public UQueryState<Port> ports;
    public UQueryState<Edge> edges {
      get;
      private set;
    }
    ...
    public GraphElement GetElementByGuid(string guid) {
      return allGraphElements.ToList().FirstOrDefault(e => e.viewDataKey ==
                                                           guid);
    }
    ...
    public Edge GetEdgeByGuid(string guid) {
      return graphElements.ToList().OfType<Edge>().FirstOrDefault(
          e => e.viewDataKey == guid);
    }
    ...
    public void SetupZoom(float minScaleSetup, float maxScaleSetup,
                          float scaleStepSetup, float referenceScaleSetup) {
    ...
    private void UpdatePersistedViewTransform() {
      ...
      m_PersistedViewTransform.position =
    ...
    internal override void OnViewDataReady() {
      ...
      m_PersistedViewTransform = GetOrCreateViewData<PersistedViewTransform>(
          m_PersistedViewTransform, key);

      m_PersistedSelection =
          GetOrCreateViewData<PersistedSelection>(m_PersistedSelection, key);

      UpdateViewTransform(m_PersistedViewTransform.position,
                          m_PersistedViewTransform.scale);
    ...
    public virtual void AddToSelection(ISelectable selectable) {
      ...
      if (ShouldRecordUndo()) {
        ...
        m_GraphViewUndoRedoSelection.selectedElements.Add(
            graphElement.viewDataKey);
    ...
    private void AddToSelectionNoUndoRecord(GraphElement graphElement) {
      ...
      // To ensure that the selected GraphElement gets unselected if it is
      // removed from the GraphView.
      graphElement.RegisterCallback<DetachFromPanelEvent>(
          OnSelectedElementDetachedFromPanel);
    ...
    private void RemoveFromSelectionNoUndoRecord(ISelectable selectable) {
      ...
      graphElement.UnregisterCallback<DetachFromPanelEvent>(
          OnSelectedElementDetachedFromPanel);
      graphElement.MarkDirtyRepaint();
    }
    ...
    public virtual void RemoveFromSelection(ISelectable selectable) {
      ...
      if (ShouldRecordUndo()) {
        RecordSelectionUndoPre();
        m_GraphViewUndoRedoSelection.selectedElements.Remove(
            graphElement.viewDataKey);
    ...
    public virtual void BuildContextualMenu(ContextualMenuPopulateEvent evt) {
      ...
      if (evt.target is GraphView || evt.target is Node ||
        ...
        evt.menu.AppendAction("Cut", (a) => { CutSelectionCallback(); },
                              (a) => {
                                return canCutSelection ? DropdownMenuAction
                                    .Status.Normal
      ...
      if (evt.target is GraphView || evt.target is Node ||
        ...
        evt.menu.AppendAction("Copy", (a) => { CopySelectionCallback(); },
                              (a) => {
                                return canCopySelection ? DropdownMenuAction
                                    .Status.Normal
                                    : DropdownMenuAction.Status.Disabled;
                              });
      }
      if (evt.target is GraphView) {
        evt.menu.AppendAction("Paste", (a) => { PasteCallback(); },
                              (a) => {
                                return canPaste ? DropdownMenuAction.Status
                                    .Normal
                                    : DropdownMenuAction.Status.Disabled;
                              });
      }
      if (evt.target is GraphView || evt.target is Node ||
        ...
        evt.menu.AppendAction(
            "Delete", (a) => { DeleteSelectionCallback(AskUser.DontAskUser); },
            (a) => {
              return canDeleteSelection ? DropdownMenuAction.Status
                  .Normal : DropdownMenuAction.Status.Disabled;
            });
      }
      if (evt.target is GraphView || evt.target is Node ||
          evt.target is Group) {
        ...
        evt.menu.AppendAction("Duplicate",
                              (a) => { DuplicateSelectionCallback(); },
                              (a) => {
                                return canDuplicateSelection ? DropdownMenuAction
                                    .Status.Normal
                                    : DropdownMenuAction.Status.Disabled;
                              });
        evt.menu.AppendSeparator();
    ...
    private void RequestNodeCreation(VisualElement target, int index,
                                     Vector2 position) {
    ...
    void OnContextualMenu(ContextualMenuPopulateEvent evt) {
      // If popping a contextual menu on a GraphElement, add the cut/copy
      // actions.
      BuildContextualMenu(evt);
    }

    private void OnBeforeUpdaterChange() { UpdateDrawChainRegistration(false); }

    private void OnAfterUpdaterChange() { UpdateDrawChainRegistration(true); }
    ...
    void OnEnterPanel(AttachToPanelEvent e) {
      ...
      if (p != null) {
        ...
        // Changing the updaters is assumed not to be a normal use case, except
        // maybe for Unity debugging purposes. For that reason, we don't track
        // updater changes.
      ...
      // Force DefaultCommonDark.uss since GraphView only has a dark style at
      // the moment
    ...
    EventPropagation OnInsertNodeKeyDown(KeyDownEvent evt) {
      ...
      if (currentInsertLocation != null) {
        currentInsertLocation.GetInsertInfo(evt.originalMousePosition,
                                            out insertInfo);

        if (insertInfo.target != null) {
          worldPosition =
              insertInfo.target.LocalToWorld(insertInfo.localPosition);
        }
      }
    ...
    public enum AskUser { AskUser, DontAskUser }
    ...
    protected internal virtual bool canCopySelection => selection.Any(
        s => s is Node || s is Group || s is Placemat || s is StickyNote);

    public static void
    CollectElements(IEnumerable<GraphElement> elements,
                    HashSet<GraphElement> collectedElementSet,
                    Func<GraphElement, bool> conditionFunc) {
      foreach (var element in elements.Where(
                   e => e != null && !collectedElementSet.Contains(e) &&
                        conditionFunc(e))) {
    ...
    CollectCopyableGraphElements(IEnumerable<GraphElement> elements,
      ...
      foreach (var placemat in elements.OfType<Placemat>()) {
        placemat.ActOnGraphElementsOver(el => {
          CollectElements(new[]{el}, elementsToCopySet, e => e.IsCopiable());
          return false;
        }, true);
      }
    }
    ...
    protected internal void CopySelectionCallback() {
      ...
      CollectCopyableGraphElements(selection.OfType<GraphElement>(),
                                   elementsToCopySet);
    ...
    protected internal virtual bool canCutSelection => selection.Any(
        s => s is Node || s is Group || s is Placemat || s is StickyNote);
    ...
    protected internal virtual bool canDeleteSelection {
      get {
        return selection.Cast<GraphElement>().Any(
            e => e != null && (e.capabilities & Capabilities.Deletable) != 0);
      }
    }
    ...
    SerializeGraphElementsDelegate(IEnumerable<GraphElement> elements);
    public delegate bool CanPasteSerializedDataDelegate(string data);
    public delegate void UnserializeAndPasteDelegate(string operationName,
                                                     string data);
    public delegate void DeleteSelectionDelegate(string operationName,
                                                 AskUser askUser);

    public SerializeGraphElementsDelegate serializeGraphElements {
      get;
      set;
    }
    public CanPasteSerializedDataDelegate canPasteSerializedData {
      get;
      set;
    }
    public UnserializeAndPasteDelegate unserializeAndPaste {
      get;
      set;
    }
    public DeleteSelectionDelegate deleteSelection {
      get;
      set;
    }

    const string m_SerializedDataMimeType =
    ...
    protected bool CanPasteSerializedData(string data) {
      if (canPasteSerializedData != null) {
        if (data.StartsWith(m_SerializedDataMimeType)) {
          return canPasteSerializedData(
              data.Substring(m_SerializedDataMimeType.Length + 1));
        } else {
    ...
    protected void UnserializeAndPasteOperation(string operationName,
                                                string data) {
    ...
    protected void DeleteSelectionOperation(string operationName,
                                            AskUser askUser) {
    ...
    public virtual List<Port> GetCompatiblePorts(Port startPort,
      ...
      return ports.ToList()
          .Where(nap => nap.direction != startPort.direction &&
                        ...
                        nodeAdapter.GetAdapter(nap.source, startPort.source) !=
                            null)
          .ToList();
    }
    ...
    public void RemoveElement(GraphElement graphElement) {
      // TODO : Find a better way to remove a graphElement from its scope when
      // it is removed from the GraphView.
    ...
    private void
    CollectDeletableGraphElements(IEnumerable<GraphElement> elements,
                                  HashSet<GraphElement> elementsToRemoveSet) {
      CollectElements(elements, elementsToRemoveSet,
                      e =>(e.capabilities & Capabilities.Deletable) ==
    ...
    public virtual EventPropagation DeleteSelection() {
      ...
      CollectDeletableGraphElements(selection.OfType<GraphElement>(),
                                    elementsToRemoveSet);

      var previouslyCollapsedElements = new HashSet<GraphElement>();
      // For each collapsed placemat, expand and keep list of elements over the
      // expanded placemat
      foreach (var placemat in elementsToRemoveSet.OfType<Placemat>().Where(
                   p => p.Collapsed)) {
      ...
      return (elementsToRemoveSet.Count > 0) ? EventPropagation.Stop
    ...
    public void DeleteElements(IEnumerable<GraphElement> elementsToRemove) {
      ...
      if (graphViewChanged != null) {
        elementsToRemoveList =
            graphViewChanged(m_GraphViewChange).elementsToRemove;
      }
    ...
    public EventPropagation FrameAll() { return Frame(FrameType.All); }

    public EventPropagation FrameSelection() {
      return Frame(FrameType.Selection);
    }

    public EventPropagation FrameOrigin() { return Frame(FrameType.Origin); }

    public EventPropagation FramePrev() {
      if (contentViewContainer.childCount == 0)
        return EventPropagation.Continue;

      List<GraphElement> childrenList =
          graphElements.ToList()
              .Where(e => e.IsSelectable() && !(e is Edge))
    ...
    public EventPropagation FramePrev(Func<GraphElement, bool> predicate) {
      ...
      List<GraphElement> list =
          graphElements.ToList().Where(predicate).ToList();
      list.Reverse();
    ...
    EventPropagation Frame(FrameType frameType) {
      ...
      if (frameType == FrameType.Selection &&
          (selection.Count == 0 ||
           !selection.Any(e => e.IsSelectable() && !(e is Edge))))
        frameType = FrameType.All;

      if (frameType == FrameType.Selection) {
        VisualElement graphElement = selection[0] as GraphElement;
        if (graphElement != null) {
          // Edges don't have a size. Only their internal EdgeControl have a
          // size.
          if (graphElement is Edge)
            graphElement = (graphElement as Edge).edgeControl;
          rectToFit = graphElement.ChangeCoordinatesTo(contentViewContainer,
                                                       graphElement.rect);
        }

        rectToFit = selection.Cast<GraphElement>().Aggregate(
            rectToFit, (current, currentGraphElement) => {
              ...
              return RectUtils.Encompass(
                  current, currentElement.ChangeCoordinatesTo(
                               contentViewContainer, currentElement.rect));
            });
        CalculateFrameTransform(rectToFit, layout, k_FrameBorder,
                                out frameTranslation, out frameScaling);
    ...
    public static void CalculateFrameTransform(Rect rectToFit, Rect clientRect,
                                               int border,
                                               out Vector3 frameTranslation,
                                               out Vector3 frameScaling) {
      // bring slightly smaller screen rect into GUI space
      var screenRect =
          new Rect{xMin = border, xMax = clientRect.width - border,
                   yMin = border, yMax = clientRect.height - border};
      ...
      // measure zoom level necessary to fit the canvas rect into the screen
      // rect
      float zoomLevel = Math.Min(identity.width / rectToFit.width,
                                 identity.height / rectToFit.height);

      // clamp
      zoomLevel = Mathf.Clamp(zoomLevel, ContentZoomer.DefaultMinScale, 1.0f);

      var transform = Matrix4x4.TRS(Vector3.zero, Quaternion.identity,
                                    new Vector3(zoomLevel, zoomLevel, 1.0f));

      var edge = new Vector2(clientRect.width, clientRect.height);
      var origin = new Vector2(0, 0);

      var r = new Rect{min = origin, max = edge};
    ...
    static readonly int s_EditorPixelsPerPointId =
        Shader.PropertyToID("_EditorPixelsPerPoint");
    static readonly int s_GraphViewScaleId =
        Shader.PropertyToID("_GraphViewScale");
    ...
    public virtual Blackboard GetBlackboard() { return null; }
