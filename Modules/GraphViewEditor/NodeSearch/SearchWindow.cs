...
namespace UnityEditor.Experimental.GraphView {
  ...
  public struct SearchWindowContext {
    ...
    public SearchWindowContext(Vector2 screenMousePosition,
  ...
  public class SearchWindow : EditorWindow {
    ...
    private List<SearchTreeGroupEntry> m_SelectionStack =
        new List<SearchTreeGroupEntry>();
    ...
    static SearchWindow() { s_DirtyList = true; }

    void OnEnable() { s_FilterWindow = this; }
    ...
    public static bool Open<T>(SearchWindowContext context, T provider) where T
        : ScriptableObject,
          ISearchWindowProvider {
      // If the window is already open, close it instead.
      UnityEngine.Object[] wins =
          Resources.FindObjectsOfTypeAll(typeof(SearchWindow));
      if (wins.Length > 0) {
        try {
    ...
    void Init(SearchWindowContext context, ScriptableObject provider) {
      ...
      Rect buttonRect =
          new Rect(context.screenMousePosition.x - width / 2,
                   context.screenMousePosition.y - k_WindowYOffset, width, 1);
    ...
    private void CreateSearchTree() {
      ...
      if (m_SelectionStack.Count == 0)
        ...
        while (true) {
          ...
          m_SelectionStack[level].selectedIndex =
              oldSearchTreeEntry.selectedIndex;
          ...
          SearchTreeEntry childMatch = children.FirstOrDefault(
              c => c.name == m_SelectionStack[level].name);
          if (childMatch != null && childMatch is SearchTreeGroupEntry) {
            match = childMatch as SearchTreeGroupEntry;
          } else {
            // If we couldn't find the child, remove all further
            // SearchTreeEntrys from the stack
    ...
    internal void OnGUI() {
      ...
      GUI.Label(new Rect(0, 0, position.width, position.height),
                GUIContent.none, s_Styles.background);
      ...
      string newSearch =
          EditorGUI.SearchField(searchRect, m_DelayedSearch ?? m_Search);

      if (EditorGUI.EndChangeCheck() &&
          (newSearch != m_Search || m_DelayedSearch != null)) {
        if (!isAnimating) {
      ...
      // Show lists
      ListGUI(activeTree, m_Anim, GetSearchTreeEntryRelative(0),
              GetSearchTreeEntryRelative(-1));
      if (m_Anim < 1)
        ListGUI(activeTree, m_Anim + 1, GetSearchTreeEntryRelative(-1),
                GetSearchTreeEntryRelative(-2));

      // Animate
      if (isAnimating && Event.current.type == EventType.Repaint) {
        long now = System.DateTime.Now.Ticks;
        float deltaTime =
            (now - m_LastTime) / (float) System.TimeSpan.TicksPerSecond;
        m_LastTime = now;
    ...
    private void HandleKeyboard() {
      ...
      if (evt.type == EventType.KeyDown) {
        ...
        if (evt.keyCode == KeyCode.DownArrow) {
          ...
          activeParent.selectedIndex =
              Mathf.Min(activeParent.selectedIndex,
    ...
    private void RebuildSearch() {
      if (!hasSearch) {
        ...
        if (m_SelectionStack[m_SelectionStack.Count - 1].name ==
            kSearchHeader) {
      ...
      // We keep two lists. Matches that matches the start of an item always get
      // first priority.
      ...
      foreach (SearchTreeEntry e in m_Tree) {
        ...
        for (int w = 0; w < searchWords.Length; w++) {
          ...
          if (name.Contains(search)) {
            // If the start of the item matches the first search word, make a
            // note of that.
      ...
      // Always select the first search result when search is changed (e.g. a
      // character was typed in or deleted), because it's usually the best
      // match.
    ...
    private void ListGUI(SearchTreeEntry[] tree, float anim,
                         SearchTreeGroupEntry parent,
      ...
      if (grandParent != null) {
        float yOffset =
            (headerRect.height - s_Styles.leftArrow.fixedHeight) / 2;
        Rect arrowRect =
            new Rect(headerRect.x + s_Styles.leftArrow.margin.left,
                     headerRect.y + yOffset, s_Styles.leftArrow.fixedWidth,
    ...
    private void ListGUI(SearchTreeEntry[] tree, SearchTreeGroupEntry parent) {
      ...
      for (int i = 0; i < children.Count; i++) {
        ...
        // Only do it on mouse move - keyboard controls are allowed to overwrite
        // this until the next time the mouse moves.
        ...
        if (Event.current.type == EventType.Repaint) {
          GUIStyle labelStyle = (e is SearchTreeGroupEntry)
                                    ? s_Styles.groupButton
                                    : s_Styles.componentButton;
          labelStyle.Draw(r, e.content, false, false, selected, selected);
          if ((e is SearchTreeGroupEntry)) {
            float yOffset = (r.height - s_Styles.rightArrow.fixedHeight) / 2;
            Rect arrowRect =
                new Rect(r.xMax - s_Styles.rightArrow.fixedWidth -
                             s_Styles.rightArrow.margin.right,
                         r.y + yOffset, s_Styles.rightArrow.fixedWidth,
    ...
    private List<SearchTreeEntry> GetChildren(SearchTreeEntry[] tree,
                                              SearchTreeEntry parent) {
