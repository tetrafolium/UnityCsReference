...
namespace UnityEditor.Experimental.GraphView {
  public class ContentZoomer : Manipulator {
    ...
    /// </summary>
    /// <remarks>
    /// Depending on the values of <c>minScale</c>, <c>maxScale</c> and
    /// <c>scaleStep</c>, it is not guaranteed that the first and last two scale
    /// steps will correspond exactly to the value specified in
    /// <c>scaleStep</c>.
    /// </remarks>
    ...
    [Obsolete(
        "ContentZoomer.keepPixelCacheOnZoom is deprecated and has no effect")]
    ...
    protected override void RegisterCallbacksOnTarget() {
      ...
      if (graphView == null) {
        throw new InvalidOperationException(
            "Manipulator can only be added to a GraphView");
      }
    ...
    private static float CalculateNewZoom(float currentZoom, float wheelDelta,
                                          float zoomStep, float referenceZoom,
                                          float minZoom, float maxZoom) {
      if (minZoom <= 0) {
        Debug.LogError(
            $"The minimum zoom ({minZoom}) must be greater than zero.");
        return currentZoom;
      }
      ...
      if (zoomStep < 0) {
        Debug.LogError(
            $"The zoom step ({zoomStep}) must be greater than or equal to zero.");
        return currentZoom;
      }
      ...
      // Except when the delta is zero, for each event, consider that the delta
      // corresponds to a rotation by a full notch. The scroll wheel abstraction
      // system is buggy and incomplete: with a regular mouse, the minimum wheel
      // movement is 0.1 on OS X and 3 on Windows. We can't simply accumulate
      // deltas like these, so we accumulate integers only. This may be
      // problematic with high resolution scroll wheels: many small events will
      // be fired. However, at this point, we have no way to differentiate a
      // high resolution scroll wheel delta from a non-accelerated scroll wheel
      // delta of one notch on OS X.
      ...
      // Do not assimilate again. Otherwise, points as far as 1.5 units away
      // could be stuck to the boundary because the wheel delta is either +1 or
      // -1.
    ...
    void OnWheel(WheelEvent evt) {
      ...
      // TODO: augment the data to have the position as well, so we don't have
      // to read in data from the target. 0-1 ranged center relative to size
      Vector2 zoomCenter = target.ChangeCoordinatesTo(
          graphView.contentViewContainer, evt.localMousePosition);
      ...
      // Apply the new zoom.
      float zoom = CalculateNewZoom(scale.y, -evt.delta.y, scaleStep,
