  ...
  public class EdgeManipulator : MouseManipulator {
    ...
    protected void OnMouseMove(MouseMoveEvent evt) {
      ...
      if (!alreadyDetached) {
        ...
        /// Determine which end is the nearest to the mouse position then detach
        /// it.
        Vector2 outputPos = new Vector2(m_Edge.output.GetGlobalCenter().x,
                                        m_Edge.output.GetGlobalCenter().y);
        Vector2 inputPos = new Vector2(m_Edge.input.GetGlobalCenter().x,
                                       m_Edge.input.GetGlobalCenter().y);
        ...
        m_ConnectedEdgeDragHelper =
            m_ConnectedPort.edgeConnector.edgeDragHelper;
        ...
        if (m_DetachedPort.allowMultiDrag) {
          foreach (var edge in m_DetachedPort.connections) {
            if (edge != m_Edge && edge.IsSelected(gv)) {
              var otherPort =
                  m_DetachedPort == edge.input ? edge.output : edge.input;
    ...
    protected void OnMouseUp(MouseUpEvent evt) {
      if (CanStopManipulation(evt)) {
        target.ReleaseMouse();
        if (m_Active) {
          // Restore the detached port before potentially delete or reconnect
          // it. This is to ensure that the edge has valid input and output so
          // it can be properly handled by the model.
          RestoreDetachedPort();
