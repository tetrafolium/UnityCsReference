...
namespace UnityEditor.Experimental.GraphView {
  public class ClickSelector : MouseManipulator {
    static bool WasSelectableDescendantHitByMouse(GraphElement currentTarget,
                                                  MouseDownEvent evt) {
      ...
      while (descendant != null && currentTarget != descendant) {
        ...
        if (selectableDescendant != null &&
            selectableDescendant.enabledInHierarchy &&
            selectableDescendant.pickingMode != PickingMode.Ignore &&
            selectableDescendant.IsSelectable()) {
          Vector2 localMousePosition = currentTarget.ChangeCoordinatesTo(
              descendant, evt.localMousePosition);
    ...
    protected void OnMouseDown(MouseDownEvent e) {
      ...
      if (CanStartManipulation(e) && graphElement.IsSelectable() &&
          graphElement.HitTest(e.localMousePosition) &&
        ...
        // Do not stop the propagation as it is common case for a parent start
        // to move the selection on a mouse down.
      }
    }
  }
}
