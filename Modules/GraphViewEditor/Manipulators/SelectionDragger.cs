...
namespace UnityEditor.Experimental.GraphView {
  public class SelectionDragger : Dragger {
    ...
    // selectedElement is used to store a unique selection candidate for cases
    // where user clicks on an item not to drag it but just to reset the
    // selection -- we only know this after the manipulation has ended
    GraphElement selectedElement {
      get;
      set;
    }
    GraphElement clickedElement {
      get;
      set;
    }
    ...
    private List<VisualElement> m_DropTargetPickList =
    ...
    static void SendDragAndDropEvent(IDragAndDropEvent evt,
      ...
      if (e.eventTypeId == DragExitedEvent.TypeId()) {
        ...
        dropTarget.DragEnter(evt as DragEnterEvent, selection, dropTarget,
                             dragSource);
      } else if (e.eventTypeId == DragLeaveEvent.TypeId()) {
        dropTarget.DragLeave(evt as DragLeaveEvent, selection, dropTarget,
                             dragSource);
      }
    ...
    private void OnMouseCaptureOutEvent(MouseCaptureOutEvent e) {
      if (m_Active) {
        ...
        // Stop processing the event sequence if the target has lost focus,
        // then.
    ...
    protected new void OnMouseDown(MouseDownEvent e) {
      ...
      if (CanStartManipulation(e)) {
        ...
        // Only start manipulating if the clicked element is movable, selected
        // and that the mouse is in its clickable region (it must be deselected
        // otherwise).
        if (!clickedElement.IsMovable() ||
            !clickedElement.HitTest(
                clickedElement.WorldToLocal(e.mousePosition)))
          return;

        // If we hit this, this likely because the element has just been
        // unselected It is important for this manipulator to receive the event
        // so the previous one did not stop it but we shouldn't let it propagate
        // to other manipulators to avoid a re-selection
        ...
        HashSet<GraphElement> elementsToMove = new HashSet<GraphElement>(
            m_GraphView.selection.OfType<GraphElement>());

        var selectedPlacemats =
            new HashSet<Placemat>(elementsToMove.OfType<Placemat>());
        ...
        foreach (GraphElement ce in elementsToMove) {
          ...
          Rect geometryInContentViewSpace =
              ce.hierarchy.parent.ChangeCoordinatesTo(
                  m_GraphView.contentViewContainer, geometry);
        ...
        if (m_PanSchedule == null) {
          m_PanSchedule = m_GraphView.schedule.Execute(Pan)
                              .Every(k_PanInterval)
                              .StartingIn(k_PanInterval);
          m_PanSchedule.Pause();
        }

        // Checking if the Graph Element we are moving has the snappable
        // Capability
        ...
        target.CaptureMouse(); // We want to receive events even when mouse is
                               // not over ourself.
        e.StopImmediatePropagation();
      }
    }
    ...
    internal Vector2 GetEffectivePanSpeed(Vector2 mousePos) {
      ...
      if (mousePos.x <= k_PanAreaWidth)
        effectiveSpeed.x =
            -(((k_PanAreaWidth - mousePos.x) / k_PanAreaWidth) + 0.5f) *
            k_PanSpeed;
      else if (mousePos.x >=
               m_GraphView.contentContainer.layout.width - k_PanAreaWidth)
        effectiveSpeed.x =
            (((mousePos.x -
               (m_GraphView.contentContainer.layout.width - k_PanAreaWidth)) /
              k_PanAreaWidth) +
      ...
      if (mousePos.y <= k_PanAreaWidth)
        effectiveSpeed.y =
            -(((k_PanAreaWidth - mousePos.y) / k_PanAreaWidth) + 0.5f) *
            k_PanSpeed;
      else if (mousePos.y >=
               m_GraphView.contentContainer.layout.height - k_PanAreaWidth)
        effectiveSpeed.y =
            (((mousePos.y -
               (m_GraphView.contentContainer.layout.height - k_PanAreaWidth)) /
              k_PanAreaWidth) +
    ...
    void ComputeSnappedRect(ref Rect selectedElementProposedGeom, float scale) {
      // Let the snapper compute a snapped position using the precomputed
      // position relatively to the geometries of all unselected GraphElements
      // in the GraphView.contentViewContainer's space.
      Rect geometryInContentViewContainerSpace =
          selectedElement.parent.ChangeCoordinatesTo(
              m_GraphView.contentViewContainer, selectedElementProposedGeom);
      geometryInContentViewContainerSpace =
          m_Snapper.GetSnappedRect(geometryInContentViewContainerSpace, scale);
      // Once the snapped position is computed in the
      // GraphView.contentViewContainer's space then translate it into the local
      // space of the parent of the selected element.
      selectedElementProposedGeom =
          m_GraphView.contentViewContainer.ChangeCoordinatesTo(
              selectedElement.parent, geometryInContentViewContainerSpace);
    }
    ...
    protected new void OnMouseMove(MouseMoveEvent e) {
      ...
      Vector2 gvMousePos = ve.ChangeCoordinatesTo(m_GraphView.contentContainer,
      ...
      // We need to monitor the mouse diff "by hand" because we stop positioning
      // the graph elements once the mouse has gone out.
      m_MouseDiff = m_originalMouse - e.mousePosition;

      var groupElementsDraggedOut =
          e.shiftKey ? new Dictionary<Group, List<GraphElement>>() : null;
      ...
      foreach (KeyValuePair<GraphElement, OriginalPos> v in m_OriginalPos) {
        ...
        // Protect against stale visual elements that have been deparented since
        // the start of the manipulation
        if (ce.hierarchy.parent == null)
          continue;

        if (!v.Value.dragStarted) {
          // TODO Would probably be a good idea to batch stack items as we do
          // for group ones.
          ...
          if (groupElementsDraggedOut != null) {
            ...
            if (groupParent != null) {
              ...
              groupElementsDraggedOut [groupParent]
                  .Add(ce);
            }
          }
      ...
      // Needed to ensure nodes can be dragged out of multiple groups all at
      // once.
      if (groupElementsDraggedOut != null) {
        foreach (KeyValuePair<Group, List<GraphElement>> kvp in
                     groupElementsDraggedOut) {
      ...
      // TODO: Replace with a temp drawing or something...maybe manipulator
      // could fake position all this to let operation know which element sits
      // under cursor...or is there another way to draw stuff that is being
      // dragged?

      IDropTarget dropTarget =
          GetDropTargetAt(e.mousePosition, selection.OfType<VisualElement>());
    ...
    void SnapOrMoveElement(KeyValuePair<GraphElement, OriginalPos> v,
                           Rect selectedElementGeom) {
      GraphElement ce = v.Key;
      if (EditorPrefs.GetBool("GraphSnapping")) {
        Vector2 geomDiff = selectedElementGeom.position -
                           m_OriginalPos[selectedElement].pos.position;
        Rect ceLayout = ce.GetPosition();
        ce.SetPosition(new Rect(v.Value.pos.x + geomDiff.x,
                                v.Value.pos.y + geomDiff.y, ceLayout.width,
                                ceLayout.height));
      } else {
    ...
    Rect GetSelectedElementGeom() {
      ...
      m_XScale = g.m00; // The scale on x is equal to the scale on y because the
                        // graphview is not distorted
      Rect selectedElementGeom = m_OriginalPos[selectedElement].pos;
      // Compute the new position of the selected element using the mouse delta
      // position and panning info
      selectedElementGeom.x =
          selectedElementGeom.x -
          (m_MouseDiff.x - m_ItemPanDiff.x) * panSpeed.x / m_XScale;
      selectedElementGeom.y =
          selectedElementGeom.y -
          (m_MouseDiff.y - m_ItemPanDiff.y) * panSpeed.y / m_XScale;
    ...
    void MoveElement(GraphElement element, Rect originalPos) {
      ...
      // Compute the new position of the selected element using the mouse delta
      // position and panning info
      newPos.x = originalPos.x - (m_MouseDiff.x - m_ItemPanDiff.x) *
                                     panSpeed.x / scale.x *
                                     element.transform.scale.x;
      newPos.y = originalPos.y - (m_MouseDiff.y - m_ItemPanDiff.y) *
                                     panSpeed.y / scale.y *
                                     element.transform.scale.y;

      element.SetPosition(m_GraphView.contentViewContainer.ChangeCoordinatesTo(
          element.hierarchy.parent, newPos));
    }
    ...
    protected new void OnMouseUp(MouseUpEvent evt) {
      ...
      if (CanStopManipulation(evt)) {
        if (m_Active) {
          if (m_Dragging) {
            foreach (IGrouping<StackNode, GraphElement> grouping in
                         m_OriginalPos.GroupBy(v => v.Value.stack,
                         v => v.Key)) {
              if (grouping.Key != null &&
                  m_GraphView.elementsRemovedFromStackNode != null)
                m_GraphView.elementsRemovedFromStackNode(grouping.Key,
                                                         grouping);
            ...
            if (graphView != null && graphView.graphViewChanged != null) {
              KeyValuePair<GraphElement, OriginalPos> firstPos =
                  m_OriginalPos.First();
              m_GraphViewChange.moveDelta =
                  firstPos.Key.GetPosition().position -
                  firstPos.Value.pos.position;
          ...
          if (selection.Count > 0 && m_PrevDropTarget != null) {
            if (m_PrevDropTarget.CanAcceptDrop(selection)) {
              ...
              using(DragExitedEvent dexit = DragExitedEvent.GetPooled(evt)) {
                SendDragAndDropEvent(dexit, selection, m_PrevDropTarget,
                                     m_GraphView);
              }
            }
          }
        ...
        if (originalPos.stack != null) {
          ...
          if (originalPos.scope != null) {
            if (!groupsElementsToReset.ContainsKey(originalPos.scope)) {
              groupsElementsToReset[originalPos.scope] =
