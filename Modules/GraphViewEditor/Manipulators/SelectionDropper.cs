...
namespace UnityEditor.Experimental.GraphView {
  ...
  internal class DragAndDropDelay {
    ...
    public bool CanStartDrag(Vector2 mousePosition) {
      return Vector2.Distance(mouseDownPosition, mousePosition) >
             k_StartDragTreshold;
    }
  }

  // Manipulates movable objects, can also initiate a Drag and Drop operation
  // FIXME: update this code once we have support for drag and drop events in
  // UIElements.
  public class SelectionDropper : Manipulator {
    ...
    protected void OnMouseDown(MouseDownEvent e) {
      ...
      if (selectionContainer == null) {
        // Keep for potential later use in OnMouseUp (where e.target might be
        // different then)
      ...
      // Since we didn't drag after all, update selection with current element
      // only
    ...
    protected void OnMouseMove(MouseMoveEvent e) {
      if (m_Active && !m_Dragging && selectionContainer != null) {
        ...
        if (selection.Count > 0) {
          ...
          if (canStartDrag &&
              m_DragAndDropDelay.CanStartDrag(e.localMousePosition)) {
            ...
            DragAndDrop.visualMode =
                e.actionKey ? DragAndDropVisualMode.Copy
    ...
    protected void OnMouseUp(MouseUpEvent e) {
      if (!m_Active || selectionContainer == null) {
        if (selectedElement != null && selectionContainer != null &&
            !m_Dragging) {
          if (selectedElement.IsSelected((VisualElement) selectionContainer) &&
              !e.actionKey && e.button == (int) activateButton) {
            // Reset to single selection
            selectionContainer.ClearSelection();
            selectedElement.Select((VisualElement) selectionContainer,
                                   e.actionKey);
          }
        }
      ...
      if (e.button == (int) activateButton) {
        ...
        if (!e.actionKey) {
          ...
          selectionContainer.AddToSelection(selectedElement);
                   ...
                   selectionContainer.selection.Contains(selectedElement)) {
