...
namespace UnityEditor.UIElements.GraphView {
  ...
  class SnapResult {
    ...
    public SnapReference sourceReference {
      get;
      set;
    }
    ...
    public SnapReference snappableReference {
      get;
      set;
    }
    public float offset {
  ...
  class SnapService {
    ...
    public SnapService() { snapDistance = k_DefaultSnapDistance; }
    ...
    virtual public void BeginSnap(List<Rect> snappableRects) {
      if (active) {
        throw new InvalidOperationException(
            "SnapService.BeginSnap: Already active. Call EndSnap() first.");
      }
      active = true;
    ...
    public Rect GetSnappedRect(Rect sourceRect, out List<SnapResult> results,
                               float scale = 1.0f) {
      if (!active) {
        throw new InvalidOperationException(
            "SnapService.GetSnappedRect: Already active. Call BeginSnap() first.");
      }
      ...
      foreach (SnapResult result in results) {
        result.indicatorLine =
            GetSnapLine(snappedRect, result.sourceReference,
                        result.snappableRect, result.snappableReference);
      }
      return snappedRect;
    }
    ...
    virtual public void EndSnap() {
      if (!active) {
        throw new InvalidOperationException(
            "SnapService.End: Already active. Call BeginSnap() first.");
      }
      m_SnappableRects.Clear();
      active = false;
    }

    SnapResult GetClosestSnapElement(Rect sourceRect, SnapReference sourceRef,
                                     Rect snappableRect,
      ...
      SnapResult minResult = new SnapResult{
          sourceRect = sourceRect, sourceReference = sourceRef,
          snappableRect = snappableRect,
          snappableReference = minSnappableReference, offset = minOffset};
    ...
    SnapResult GetClosestSnapElement(Rect sourceRect, SnapReference sourceRef,
      ...
      foreach (Rect snappableRect in m_SnappableRects) {
        SnapResult result = GetClosestSnapElement(
            sourceRect, sourceRef, snappableRect, startReference,
            centerReference, endReference);
    ...
    List<SnapResult> GetClosestSnapElements(Rect sourceRect,
      ...
      SnapReference startReference =
          orientation ==
          Orientation.Horizontal ? SnapReference.LeftEdge
          : SnapReference.TopEdge;
      SnapReference centerReference =
          orientation ==
          Orientation.Horizontal ? SnapReference.HorizontalCenter
          : SnapReference.VerticalCenter;
      SnapReference endReference =
          orientation ==
          Orientation.Horizontal ? SnapReference.RightEdge
          : SnapReference.BottomEdge;
      List<SnapResult> results = new List<SnapResult>(3);
      SnapResult result =
          GetClosestSnapElement(sourceRect, startReference, startReference,
                                centerReference, endReference);
      if (result != null)
        results.Add(result);
      result =
          GetClosestSnapElement(sourceRect, centerReference, startReference,
                                centerReference, endReference);
      if (result != null)
        results.Add(result);
      result = GetClosestSnapElement(sourceRect, endReference, startReference,
                                     centerReference, endReference);
      if (result != null)
        results.Add(result);
    ...
    List<SnapResult> GetClosestSnapElements(Rect sourceRect) {
      List<SnapResult> snapResults =
          GetClosestSnapElements(sourceRect, Orientation.Horizontal);
      return snapResults
          .Union(GetClosestSnapElements(sourceRect, Orientation.Vertical))
          .ToList();
    }
    ...
    Line2 GetSnapLine(Rect r1, SnapReference reference1, Rect r2,
      ...
      Vector2 p11 = line1.start, p12 = line1.end, p21 = line2.start,
              p22 = line2.end, start = Vector2.zero, end = Vector2.zero;
      if (horizontal) {
        float x = p21.x;
