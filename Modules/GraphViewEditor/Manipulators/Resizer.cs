...
void OnMouseDown(MouseDownEvent e) {
	...
	if (e.button == (int) activateButton) {
		...
		if (!parent.isLayoutManual &&
		    parent.resolvedStyle.position == Position.Relative) {
			Debug.LogWarning(
				"Attempting to resize an object with a non absolute position");
		}
...
void OnMouseMove(MouseMoveEvent e) {
	...
	if (m_Active) {
		Vector2 diff =
			this.ChangeCoordinatesTo(parent, e.localMousePosition) - m_Start;
		var newSize =
			new Vector2(m_StartPos.width + diff.x, m_StartPos.height + diff.y);
		float minWidth = ce.resolvedStyle.minWidth == StyleKeyword.Auto ? 0
	    : ce.resolvedStyle.minWidth.value;
		minWidth = Math.Max(minWidth, m_MinimumSize.x);
		float minHeight =
			ce.resolvedStyle.minHeight == StyleKeyword.Auto ? 0
	    : ce.resolvedStyle.minHeight.value;
		minHeight = Math.Max(minHeight, m_MinimumSize.y);
		float maxWidth =
			ce.resolvedStyle.maxWidth == StyleKeyword.None ? float.MaxValue
	    : ce.resolvedStyle.maxWidth.value;
		float maxHeight =
			ce.resolvedStyle.maxHeight == StyleKeyword.None ? float.MaxValue
	    : ce.resolvedStyle.maxHeight.value;

		newSize.x = (newSize.x < minWidth)
		        ? minWidth
		        : ((newSize.x > maxWidth) ? maxWidth : newSize.x);
		newSize.y = (newSize.y < minHeight)
		        ? minHeight
		        : ((newSize.y > maxHeight) ? maxHeight : newSize.y);
