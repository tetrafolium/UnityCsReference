...
public class EdgeDragHelper<TEdge> : EdgeDragHelper where TEdge : Edge,
	new () {
...
public override void HandleMouseUp(MouseUpEvent evt) {
	...
	// If it is an existing valid edge then delete and notify the model (using
	// DeleteElements()).
	if (edgeCandidate.input != null && edgeCandidate.output != null) {
		// Save the current input and output before deleting the edge as they
		// will be reset
	...
	// otherwise, if it is an temporary edge then just remove it as it is not
	// already known my the model
	else {
...
private Port GetEndPort(Vector2 mousePosition) {
	...
	foreach (Port compatiblePort in m_CompatiblePorts) {
		...
		if (compatiblePort.orientation == Orientation.Horizontal) {
			// Add extra padding for mouse check to the left of input port or
			// right of output port.
