...
bool active {
	get { return m_Service != null ? m_Service.active : false; }
}
public Snapper() {
}
...
internal void BeginSnap(GraphView graphView) {
	...
	m_LineView.layout =
		new Rect(0, 0, m_GraphView.layout.width, m_GraphView.layout.height);
...
internal List<Rect> GetNotSelectedElementRectsInView() {
	...
	// If dragging a group we need to find all elements inside and disregard
	// them For Groups they derive from Scope in which have containedElements.
	// If we are dragging around a node that are inside a Group,
	// the groups border might adjust and we don't want to snap to that.
	// We also check if there are any GraphElement children of the selected
	// graphelement And add them to the ignore list.
	foreach (GraphElement ge in m_GraphView.selection
	         .OfType<GraphElement>()) {
		if (ge is Group) {
			Group group = ge as Group;
			var grpElements = group.containedElements;
			foreach (GraphElement element in grpElements) {
				ignoreElements.AddRange(
					element.Query<GraphElement>("", "graphElement").ToList());
			}
		...
		ignoreElements.AddRange(
			ge.Query<GraphElement>("", "graphElement").ToList());
	}

	// Consider only the visible nodes.
	Rect rectToFit = m_GraphView.layout;
	// a copy is necessary because Add To selection might cause a
	// SendElementToFront which will change the order.
	...
	foreach (GraphElement element in checkOnlyInViewElements) {
		if (!element.IsSelected(m_GraphView) &&
		    (element.capabilities & Capabilities.Snappable) != 0 &&
		    !(ignoreElements.Contains(element))) {
			Rect geomtryInContentViewContainerSpace =
				element.parent.ChangeCoordinatesTo(
					m_GraphView.contentViewContainer, element.GetPosition());
...
internal Rect GetSnappedRect(Rect sourceRect, float scale = 1.0f) {
	...
	Rect snappedRect =
		m_Service.GetSnappedRect(sourceRect, out results, scale);
