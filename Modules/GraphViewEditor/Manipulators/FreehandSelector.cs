...
namespace UnityEditor.Experimental.GraphView {
  public class FreehandSelector : MouseManipulator {
      ...
      m_GraphView.graphElements.ForEach(element => {
        if (element.IsSelectable()) {
          for (int i = 1; i < m_FreehandElement.points.Count; i++) {
            // Apply offset
            Vector2 start = m_GraphView.ChangeCoordinatesTo(
                element, m_FreehandElement.points[i - 1]);
            Vector2 end = m_GraphView.ChangeCoordinatesTo(
                element, m_FreehandElement.points[i]);
    ...
    private class FreehandElement : ImmediateModeElement {
      ...
      private static CustomStyleProperty<float> s_SegmentSizeProperty =
          new CustomStyleProperty<float>("--segment-size");
      private static CustomStyleProperty<Color> s_SegmentColorProperty =
          new CustomStyleProperty<Color>("--segment-color");
      private static CustomStyleProperty<Color> s_DeleteSegmentColorProperty =
          new CustomStyleProperty<Color>("--delete-segment-color");
      ...
      private void OnCustomStyleResolved(CustomStyleResolvedEvent e) {
        ...
        if (styles.TryGetValue(s_DeleteSegmentColorProperty,
                               out deleteColorValue))
      ...
      private void DrawDottedLine(Vector3 p1, Vector3 p2,
