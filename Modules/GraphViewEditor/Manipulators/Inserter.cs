...
namespace UnityEditor.Experimental.GraphView {
  internal class Inserter : Manipulator {
    ...
    protected override void RegisterCallbacksOnTarget() {
      if (!(target is VisualElement && target is IInsertLocation)) {
        throw new InvalidOperationException(
            "Manipulator can only be added to an IInsertLocation VisualElement");
      }
    ...
    private void OnMouseOver(MouseOverEvent evt) {
      ...
      // Keep track of the graphview in case the target is removed while being
      // hovered over.
      m_GraphView =
          (target as VisualElement).GetFirstAncestorOfType<GraphView>();
    ...
    private void OnMouseOut(MouseOutEvent evt) { ResetInsertLocation(); }
  }
}
