...
namespace UnityEditor.Experimental.GraphView {
  public abstract class GraphViewToolWindow : EditorWindow {
    ...
    [SerializeField] EditorWindow m_SelectedWindow;

    [SerializeField] int m_SelectedGraphViewIdx;
    ...
    public override IEnumerable<Type> GetExtraPaneTypes() {
      return Assembly.GetAssembly(typeof(GraphViewToolWindow))
          .GetTypes()
          .Where(myType => myType.IsClass && !myType.IsAbstract &&
                           myType.IsSubclassOf(typeof(GraphViewToolWindow)));
    }

    public void SelectGraphViewFromWindow(GraphViewEditorWindow window,
                                          GraphView graphView,
                                          int graphViewIndexInWindow = 0) {
      var gvChoice = new GraphViewChoice{window = window, graphView = graphView,
                                         idx = graphViewIndexInWindow};
    ...
    protected void OnEnable() {
      ...
      m_SelectorMenu =
          new ToolbarMenu{name = "panelSelectPopup", text = "Select a panel"};

      var menu = m_SelectorMenu.menu;
      menu.AppendAction("None", OnSelectGraphView,
                        a => m_SelectedGraphView ==
                             null ? DropdownMenuAction.Status.Checked
    ...
    void Update() {
      // We need to wait until all the windows are created to re-assign a
      // potential graphView.
      if (m_FirstUpdate) {
        m_FirstUpdate = false;
        if (m_SelectedWindow != null) {
          var graphViewEditor = m_SelectedWindow as GraphViewEditorWindow;
          if (graphViewEditor != null && m_SelectedGraphViewIdx >= 0 &&
              m_SelectedGraphView == null) {
            m_SelectedGraphView =
                graphViewEditor.graphViews.ElementAt(m_SelectedGraphViewIdx);
    ...
    void RefreshPanelChoices() {
      ...
      using(var it = UIElementsUtility.GetPanelsIterator()) {
        ...
        while (it.MoveNext()) {
          var dockArea = guiViews.FirstOrDefault(
              v => v.GetInstanceID() == it.Current.Key) as DockArea;
          if (dockArea == null)
            continue;

          foreach (var graphViewTool in dockArea.m_Panes
                       .Where(p => p.GetType() == currentWindowType)
                       .Cast<GraphViewToolWindow>()) {
      ...
      using(var it = UIElementsUtility.GetPanelsIterator()) {
        while (it.MoveNext()) {
          ...
          foreach (var graphViewWindow in dockArea.m_Panes
                       .OfType<GraphViewEditorWindow>()) {
            int idx = 0;
            foreach (var graphView in graphViewWindow.graphViews.Where(
                         IsGraphViewSupported)) {
              m_GraphViewChoices.Add(new GraphViewChoice{
                  window = graphViewWindow, idx = idx++, graphView = graphView,
                  canUse = !usedGraphViews.Contains(graphView)});
            }
          }
        }
      }
      ...
      foreach (var graphView in m_GraphViewChoices) {
        menu.AppendAction(graphView.graphView.name, OnSelectGraphView, a => {
    ...
    protected virtual bool IsGraphViewSupported(GraphView gv) { return false; }
