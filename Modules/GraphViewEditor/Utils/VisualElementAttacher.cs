...
namespace UnityEditor.Experimental.GraphView {
  public class Attacher {
    ...
    private bool isAttached {
      get {
        return target != null && element != null && m_WatchedObjects != null &&
               m_WatchedObjects.Count > 0;
      }
    }
    ...
    public Attacher(VisualElement anchored, VisualElement target,
    ...
    public void Detach() { UnregisterCallbacks(); }
    ...
    private void RegisterCallbacks() {
      ...
      if (commonAncestor == target) {
        ...
        Debug.Log(
            "Attacher: An element can't be anchored to one of its descendants");
      } else if (commonAncestor == null) {
        Debug.Log(
            "Attacher: The element and its target must be in the same visual tree hierarchy");
      } else {
    ...
    private void OnTargetLayout(GeometryChangedEvent evt) { AlignOnTarget(); }

    private void AlignOnTarget() {
      Rect currentRect =
          new Rect(element.resolvedStyle.left, element.resolvedStyle.top,
                   element.resolvedStyle.width, element.resolvedStyle.height);
      Rect targetRect = target.rect;
      targetRect =
          target.ChangeCoordinatesTo(element.hierarchy.parent, targetRect);
      ...
      // we don't want the layout to be overwritten before styling has been
      // applied
