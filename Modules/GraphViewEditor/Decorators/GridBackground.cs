...
namespace UnityEditor.Experimental.GraphView {
  public class GridBackground : ImmediateModeElement {
    static CustomStyleProperty<float> s_SpacingProperty =
        new CustomStyleProperty<float>("--spacing");
    static CustomStyleProperty<int> s_ThickLinesProperty =
        new CustomStyleProperty<int>("--thick-lines");
    static CustomStyleProperty<Color> s_LineColorProperty =
        new CustomStyleProperty<Color>("--line-color");
    ...
    static readonly Color s_DefaultThickLineColor =
        new Color(0f, 0f, 0f, 0.38f);
    static readonly Color s_DefaultGridBackgroundColor =
    ...
    private Color lineColor =>
        m_LineColor * UIElementsUtility.editorPlayModeTintColor;

    Color m_ThickLineColor = s_DefaultThickLineColor;
    private Color thickLineColor =>
        m_ThickLineColor * UIElementsUtility.editorPlayModeTintColor;

    Color m_GridBackgroundColor = s_DefaultGridBackgroundColor;
    private Color gridBackgroundColor =>
        m_GridBackgroundColor * UIElementsUtility.editorPlayModeTintColor;
    ...
    private void OnCustomStyleResolved(CustomStyleResolvedEvent e) {
      ...
      if (customStyle.TryGetValue(s_ThickLineColorProperty,
                                  out thicklineColorValue))
      ...
      if (customStyle.TryGetValue(s_GridBackgroundColorProperty,
                                  out gridColorValue))
    ...
    protected override void ImmediateRepaint() {
      ...
      if (graphView == null) {
        throw new InvalidOperationException(
            "GridBackground can only be added to a GraphView");
      }
      m_Container = graphView.contentViewContainer;
      Rect clientRect = graphView.layout;

      // Since we're always stretch to parent size, we will use (0,0) as (x,y)
      // coordinates
      clientRect.x = 0;
      clientRect.y = 0;

      var containerScale =
          new Vector3(m_Container.transform.matrix.GetColumn(0).magnitude,
      ...
      var tx =
          Matrix4x4.TRS(containerTranslation, Quaternion.identity, Vector3.one);
      ...
      from.x = (from.x % (spacing * (containerScale.x)) -
                (spacing * (containerScale.x)));
      to.x = from.x;
      ...
      from.x = to.x =
          (thickGridLineX % (thickLineSpacing * (containerScale.x)) -
      ...
      from.y = to.y = (from.y % (spacing * (containerScale.y)) -
                       (spacing * (containerScale.y)));
      from.x = clientRect.x;
      ...
      from.y = to.y =
          (thickGridLineY % (thickLineSpacing * (containerScale.y)) -
