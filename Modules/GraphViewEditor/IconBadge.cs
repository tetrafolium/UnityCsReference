...
static CustomStyleProperty<int> s_DistanceProperty =
	new CustomStyleProperty<int>("--distance");
...
public IconBadge() {
	...
	var tpl = EditorGUIUtility.Load("GraphView/Badge/IconBadge.uxml")
	          as VisualTreeAsset;
...
private static readonly string defaultStylePath =
	"GraphView/Badge/IconBadge.uss";
...
private void LoadTemplate(VisualTreeAsset tpl) {
	...
	if (m_TextElement != null) {
		...
		// we need to add the style sheet to the Text element as well since it
		// will be parented elsewhere
		m_TextElement.AddStyleSheetPath(defaultStylePath);
		m_TextElement.style.whiteSpace = WhiteSpace.Normal;
		m_TextElement.RegisterCallback<GeometryChangedEvent>(
			(evt) => ComputeTextSize());
		m_TextElement.pickingMode = PickingMode.Ignore;
	}
}

private void InitBadgeComponent<ElementType>(
	string name, string ussClassName,
	ref ElementType outElement) where ElementType : VisualElement {
...
private void ComputeTextSize() {
	if (m_TextElement != null) {
		float maxWidth = m_TextElement.resolvedStyle.maxWidth ==
		                 StyleKeyword.None ? float.NaN
	    : m_TextElement.resolvedStyle.maxWidth.value;
		Vector2 newSize = m_TextElement.DoMeasure(maxWidth, MeasureMode.AtMost,
		                                          0, MeasureMode.Undefined);

		m_TextElement.style.width =
			newSize.x + m_TextElement.resolvedStyle.marginLeft +
		...
		float height = newSize.y + m_TextElement.resolvedStyle.marginTop +
...
private void ShowText() {
	if (m_TextElement != null && m_TextElement.hierarchy.parent == null) {
		...
		if (textParent != this) {
			if (m_TextAttacher == null) {
				m_TextAttacher = new Attacher(m_TextElement, m_IconElement,
...
private void PerformTipLayout() {
	...
	if (m_IconElement != null) {
		iconSize = m_IconElement.computedStyle.width ==
		           StyleKeyword.Auto ? contentWidth - arrowLength
	    : m_IconElement.computedStyle.width.value.value;
	}
	...
	if (tipAngle != m_CurrentTipAngle) {
		if (m_TipElement != null) {
			m_TipElement.transform.rotation =
				Quaternion.Euler(new Vector3(0, 0, tipAngle));
			m_TipElement.transform.position =
				new Vector3(tipTranslate.x, tipTranslate.y, 0);
		}
...
private void OnCustomStyleResolved(CustomStyleResolvedEvent e) {
	int dist = 0;
	if (!m_DistanceIsInline &&
	    e.customStyle.TryGetValue(s_DistanceProperty, out dist))
		m_Distance = dist;
}
}
}
