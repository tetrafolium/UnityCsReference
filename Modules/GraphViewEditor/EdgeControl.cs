...
namespace UnityEditor.Experimental.GraphView {
  public class EdgeControl : VisualElement {
    ...
    private static void RecycleCap(VisualElement cap) { capPool.Push(cap); }
    ...
    void UpdateEdgeCaps() {
      if (m_FromCap != null) {
        ...
        if ((size.x > 0) && (size.y > 0))
          m_FromCap.layout = new Rect(
              parent.ChangeCoordinatesTo(this, m_From) - (size / 2), size);
      }
      if (m_ToCap != null) {
        Vector2 size = m_ToCap.layout.size;
        if ((size.x > 0) && (size.y > 0))
          m_ToCap.layout = new Rect(
              parent.ChangeCoordinatesTo(this, m_To) - (size / 2), size);
      }
    }
    ...
    public override bool ContainsPoint(Vector2 localPoint) {
      ...
      // bounding box check succeeded, do more fine grained check by measuring
      // distance to bezier points exclude endpoints
        ...
        for (var i = 0; i < allPoints.Count - 1; i++) {
          ...
          if (distance < distanceLine && distanceNext < distanceLine) {
            ...
            var d = next2Current.y * localPoint.x -
                    next2Current.x * localPoint.y +
                    nextPoint.x * currentPoint.y - nextPoint.y * currentPoint.x;
    ...
    public override bool Overlaps(Rect rect) {
      if (base.Overlaps(rect)) {
        for (int a = 0; a < m_RenderPoints.Count - 1; a++) {
          if (RectUtils.IntersectsSegment(rect, m_RenderPoints[a],
                                          m_RenderPoints[a + 1]))
            return true;
        }
      }
    ...
    // The points that will be rendered. Expressed in coordinates local to the
    // element.
    ...
    public virtual void UpdateLayout() {
      ...
      if (m_ControlPointsDirty) {
        ComputeControlPoints(); // Computes the control points in parent ( graph
                                // ) coordinates
        ComputeLayout(); // Update the element layout based on the control
                         // points.
    ...
    void RenderStraightLines(Vector2 p1, Vector2 p2, Vector2 p3, Vector2 p4) {
      float safeSpan =
          outputOrientation == Orientation.Horizontal
                         ? Mathf
                                   .Abs((p1.x + k_EdgeLengthFromPort) -
                                        (p4.x - k_EdgeLengthFromPort))
          : Mathf.Abs((p1.y + k_EdgeLengthFromPort) -
      ...
      var offset =
          outputOrientation == Orientation.Horizontal
                     ? new Vector2(
                                   k_EdgeTurnDiameter - nodeToP2Dist, 0)
    ...
    protected virtual void UpdateRenderPoints() {
      ComputeControlPoints(); // This should have been updated before : make
                              // sure anyway.
      ...
      // We have to handle a special case of the edge when it is a straight
      // line, but not when going backwards in space (where the start point is
      // in front in y to the end point). We do this by turning the line into 3
      // linear segments with no curves. This also avoids possible NANs in later
      // angle calculations.
      bool sameOrientations = outputOrientation == inputOrientation;
      if (sameOrientations &&
          ((outputOrientation == Orientation.Horizontal &&
            Mathf.Abs(p1.y - p4.y) < 2 &&
            p1.x + k_EdgeLengthFromPort < p4.x - k_EdgeLengthFromPort) ||
           (outputOrientation == Orientation.Vertical &&
            Mathf.Abs(p1.x - p4.x) < 2 &&
      ...
      EdgeCornerSweepValues corner1 =
          GetCornerSweepValues(p1, p2, p3, diameter, Direction.Output);
      EdgeCornerSweepValues corner2 =
          GetCornerSweepValues(p2, p3, p4, diameter, Direction.Input);
      ...
      if (!ValidateCornerSweepValues(ref corner1, ref corner2)) {
        ...
        Vector2 px = (outputOrientation == Orientation.Horizontal)
                         ? new Vector2(p4.x, p1.y)
                         : new Vector2(p1.x, p4.y);
      ...
      if (!sameOrientations && renderBothCorners) {
        // if the 2 corners or endpoints are too close, the corner sweep angle
        // calculations can't handle different orientations
        ...
        if ((p3 - p2).sqrMagnitude < minDistance ||
          ...
          corner1 =
              GetCornerSweepValues(p1, px, p4, diameter, Direction.Output);
          renderBothCorners = false;
        }
      }
    ...
    private bool ValidateCornerSweepValues(ref EdgeCornerSweepValues corner1,
                                           ref EdgeCornerSweepValues corner2) {
      // Get the midpoint between the two corner circle centers.
      Vector2 circlesMidpoint =
          (corner1.circleCenter + corner2.circleCenter) / 2;

      // Find the angle to the corner circles midpoint so we can compare it to
      // the sweep angles of each corner.
      ...
      double angleToCirclesMid =
          outputOrientation ==
          Orientation.Horizontal
                                   ? Math.Atan2(p2CenterToCross1.y,
                                             p2CenterToCross1.x) -
      ...
      // We need the angle to the circles midpoint to match the turn direction
      // of the first corner's sweep angle.
      angleToCirclesMid =
          Math.Sign(angleToCirclesMid) * 2 * Mathf.PI - angleToCirclesMid;
      if (Mathf.Abs((float) angleToCirclesMid) > 1.5 * Mathf.PI)
        angleToCirclesMid = -1 * Math.Sign(angleToCirclesMid) * 2 * Mathf.PI +
                            angleToCirclesMid;

      // Calculate the maximum sweep angle so that both corner sweeps and with
      // the tangents of the 2 circles meeting each other.
      ...
      float maxSweepAngle =
          Mathf.Abs((float) corner1.sweepAngle) - p2AngleToMidTangent * 2;

      // If the angle to the circles midpoint is within the sweep angle, we need
      // to apply our maximum sweep angle calculated above, otherwise the
      // maximum sweep angle is irrelevant.
      if (Mathf.Abs((float) angleToCirclesMid) <
          Mathf.Abs((float) corner1.sweepAngle)) {
        corner1.sweepAngle =
            Math.Sign(corner1.sweepAngle) *
            Mathf.Min(maxSweepAngle, Mathf.Abs((float) corner1.sweepAngle));
        corner2.sweepAngle =
            Math.Sign(corner2.sweepAngle) *
    ...
    GetCornerSweepValues(Vector2 p1, Vector2 cornerPoint, Vector2 p2,
                         float diameter, Direction closestPortDirection) {
      EdgeCornerSweepValues corner = new EdgeCornerSweepValues();

      // Calculate initial radius. This radius can change depending on the
      ...
      // to the diameter and reduce the radius to match the segment. This is
      // what makes the corner turn radii get smaller as the edge corners get
      // tighter.
      if (segment > diameter) {
        segment = diameter;
      ...
      // Calculate both cross points (where the circle touches the
      // p1-cornerPoint line and the p2-cornerPoint line).
      ...
      corner.circleCenter =
          GetCornerCircleCenter(cornerPoint, corner.crossPoint1,
                                corner.crossPoint2, segment, corner.radius);

      // Calculate the starting and ending angles.
      corner.startAngle =
          Math.Atan2(corner.crossPoint1.y - corner.circleCenter.y,
                     corner.crossPoint1.x - corner.circleCenter.x);
      corner.endAngle =
          Math.Atan2(corner.crossPoint2.y - corner.circleCenter.y,
      ...
      // If we are computing the second corner (into the input port), we want to
      // start the sweep going backwards.
    ...
    private Vector2 GetCornerCircleCenter(Vector2 cornerPoint,
                                          Vector2 crossPoint1,
                                          Vector2 crossPoint2, float segment,
                                          float radius) {
      ...
      return new Vector2(cornerPoint.x - cornerToCenterVector.x * factor,
                         cornerPoint.y - cornerToCenterVector.y * factor);
    }

    private void GetRoundedCornerPoints(List<Vector2> points,
                                        EdgeCornerSweepValues corner,
                                        Direction closestPortDirection) {
      // Calculate the number of points that will sample the arc from the sweep
      // angle.
      int pointsCount = Mathf.CeilToInt(
          (float) Math.Abs(corner.sweepAngle * k_EdgeSweepResampleRatio));
      int sign = Math.Sign(corner.sweepAngle);
      bool backwards = (closestPortDirection == Direction.Input);

      for (int i = 0; i < pointsCount; ++i) {
        // If we are computing the second corner (into the input port), the
        // sweep is going backwards but we still need to add the points to the
        // list in the correct order.
        float sweepIndex = backwards ? i - pointsCount : i;

        double sweepedAngle =
            corner.startAngle + sign * sweepIndex / k_EdgeSweepResampleRatio;

        var pointX = (float)(corner.circleCenter.x +
                             Math.Cos(sweepedAngle) * corner.radius);
        var pointY = (float)(corner.circleCenter.y +
                             Math.Sin(sweepedAngle) * corner.radius);

        // Check if we overlap the previous point. If we do, we skip this point
        // so that we don't cause the edge polygons to twist.
        if (i == 0 && backwards) {
          if (outputOrientation == Orientation.Horizontal) {
            if (corner.sweepAngle < 0 && points[points.Count - 1].y > pointY)
              continue;
            else if (corner.sweepAngle >= 0 &&
                     points[points.Count - 1].y < pointY)
              continue;
          } else {
            if (corner.sweepAngle < 0 && points[points.Count - 1].x < pointX)
              continue;
            else if (corner.sweepAngle >= 0 &&
                     points[points.Count - 1].x > pointX)
              continue;
          }
        }
    ...
    protected virtual void ComputeControlPoints() {
      ...
      if (outputOrientation == Orientation.Horizontal)
        AssignControlPoint(ref m_ControlPoints[1],
                           new Vector2(from.x + offset, from.y));
      else
        AssignControlPoint(ref m_ControlPoints[1],
                           new Vector2(from.x, from.y + offset));

      if (inputOrientation == Orientation.Horizontal)
        AssignControlPoint(ref m_ControlPoints[2],
                           new Vector2(to.x - offset, to.y));
      else
        AssignControlPoint(ref m_ControlPoints[2],
                           new Vector2(to.x, to.y - offset));
    ...
    void ComputeLayout() {
      ...
      Rect rect = new Rect(
          Vector2.Min(to, from),
          new Vector2(Mathf.Abs(from.x - to.x), Mathf.Abs(from.y - to.y)));

      // Make sure any control points (including tangents, are included in the
      // rect)
      ...
      // Make sure that we have the place to display Edges with
      // EdgeControl.k_MinEdgeWidth at the lowest level of zoom.
      float margin =
          Mathf.Max(edgeWidth * 0.5f + 1,
                    EdgeControl.k_MinEdgeWidth / m_GraphView.minScale);
    ...
    static Material lineMat {
      get {
        if (s_LineMat == null)
          s_LineMat = new Material(EditorGUIUtility.LoadRequired(
              "GraphView/AAEdge.shader") as Shader);
        return s_LineMat;
      }
    }
    ...
    void DrawEdge(MeshGenerationContext mgc) {
      ...
      var md = mgc.Allocate((int) wantedLength, (int) indexCount, null, null,
                            MeshGenerationContext.MeshFlags.UVisDisplacement);
      if (md.vertexCount == 0)
        return;
      ...
      for (int i = 1; i < cpt; ++i)
        polyLineLength +=
            (m_RenderPoints[i - 1] - m_RenderPoints[i]).sqrMagnitude;
      ...
      for (int i = 0; i < cpt; ++i) {
        ...
        Vector2 uv =
            new Vector2(dir.y * halfWidth,
                        -dir.x * halfWidth); // Normal scaled by half width
        Color32 tint = Color.LerpUnclamped(outColor, inColor,
                                           currentLength / polyLineLength);

        md.SetNextVertex(new Vertex(){position = new Vector3(pos.x, pos.y, 1),
                                      uv = uv, tint = tint, idsFlags = flags});
        md.SetNextVertex(new Vertex(){position = new Vector3(pos.x, pos.y, -1),
                                      uv = uv, tint = tint, idsFlags = flags});
