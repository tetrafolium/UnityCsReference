...
namespace UnityEngine.Networking.Match {
  ...
  [Obsolete(
      "The matchmaker and relay feature will be removed in the future, minimal support will continue until this can be safely done.")]
  public class MatchInfo {
    ...
    public override string ToString() {
      return UnityString.Format("{0} @ {1}:{2} [{3},{4}]", networkId, address,
                                port, nodeId, usingRelay);
    }
  }
  ...
  public class MatchInfoSnapshot {
    ...
    public class MatchInfoDirectConnectSnapshot {
      ...
      internal MatchInfoDirectConnectSnapshot(
          MatchDirectConnectInfo matchDirectConnectInfo) {
  ...
  public class NetworkMatch : MonoBehaviour {
    public delegate void BasicResponseDelegate(bool success,
                                               string extendedInfo);
    public delegate void
    DataResponseDelegate<T>(bool success, string extendedInfo, T responseData);

    private delegate void InternalResponseDelegate<T, U>(T response,
                                                         U userCallback);
    ...
    // The methods that perform operations return an IEnumerator to pass to
    // StartCoroutine. The specified delegate function will be called when the
    // web request completed. For example:
    // - StartCoroutine(myMatchMaker.Create(create, OnMatchCreate));
    // - StartCoroutine(myMatchMaker.JoinMatch(myGuid, myMatch.matchId, "",
    // OnMatchJoined));
    ...
    public Coroutine CreateMatch(string matchName, uint matchSize,
                                 bool matchAdvertise, string matchPassword,
                                 string publicClientAddress,
                                 string privateClientAddress,
                                 int eloScoreForMatch, int requestDomain,
                                 DataResponseDelegate<MatchInfo> callback) {
      ...
      return CreateMatch(
          new CreateMatchRequest{
              name = matchName, size = matchSize, advertise = matchAdvertise,
              password = matchPassword, publicAddress = publicClientAddress,
              privateAddress = privateClientAddress,
              eloScore = eloScoreForMatch, domain = requestDomain},
          callback);
    }

    internal Coroutine CreateMatch(CreateMatchRequest req,
                                   DataResponseDelegate<MatchInfo> callback) {
      if (callback == null) {
      ...
      data.AddField("version",
                    UnityEngine.Networking.Match.Request.currentVersion);
      data.AddField("projectId", Application.cloudProjectId);
      data.AddField("sourceId", Utility.GetSourceID().ToString());
      data.AddField("accessTokenString",
                    0); // Set the access token to 0 for any new match request
      ...
      return StartCoroutine(
          ProcessMatchResponse<CreateMatchResponse,
                               DataResponseDelegate<MatchInfo>>(
              client, OnMatchCreate, callback));
    }

    internal virtual void
    OnMatchCreate(CreateMatchResponse response,
                  DataResponseDelegate<MatchInfo> userCallback) {
      if (response.success)
      ...
      userCallback(response.success, response.extendedInfo,
                   new MatchInfo(response));
    }

    public Coroutine JoinMatch(NetworkID netId, string matchPassword,
                               string publicClientAddress,
                               string privateClientAddress,
                               int eloScoreForClient, int requestDomain,
                               DataResponseDelegate<MatchInfo> callback) {
      return JoinMatch(
          new JoinMatchRequest{networkId = netId, password = matchPassword,
    ...
    internal Coroutine JoinMatch(JoinMatchRequest req,
                                 DataResponseDelegate<MatchInfo> callback) {
      if (callback == null) {
      ...
      var client = UnityWebRequest.Post(uri.ToString(), data);
      return StartCoroutine(
          ProcessMatchResponse<JoinMatchResponse,
                               DataResponseDelegate<MatchInfo>>(
              client, OnMatchJoined, callback));
    }

    internal void OnMatchJoined(JoinMatchResponse response,
                                DataResponseDelegate<MatchInfo> userCallback) {
      if (response.success)
        Utility.SetAccessTokenForNetwork(
            (NetworkID) response.networkId,
    ...
    public Coroutine DestroyMatch(NetworkID netId, int requestDomain,
                                  BasicResponseDelegate callback) {
      return DestroyMatch(
          new DestroyMatchRequest{networkId = netId, domain = requestDomain},
          callback);
    }

    // Start to destroy a match
    internal Coroutine DestroyMatch(DestroyMatchRequest req,
                                    BasicResponseDelegate callback) {
      if (callback == null) {
      ...
      data.AddField(
          "accessTokenString",
          Utility.GetAccessTokenForNetwork(req.networkId).GetByteString());
      data.AddField("domain", req.domain);
      ...
      var client = UnityWebRequest.Post(uri.ToString(), data);
      return StartCoroutine(
          ProcessMatchResponse<BasicResponse, BasicResponseDelegate>(
              client, OnMatchDestroyed, callback));
    }

    internal void OnMatchDestroyed(BasicResponse response,
                                   BasicResponseDelegate userCallback) {
      userCallback(response.success, response.extendedInfo);
    }

    public Coroutine DropConnection(NetworkID netId, NodeID dropNodeId,
                                    int requestDomain,
                                    BasicResponseDelegate callback) {
      return DropConnection(new DropConnectionRequest{networkId = netId,
                                                      nodeId = dropNodeId,
                                                      domain = requestDomain},
                            callback);
    }
    ...
    internal Coroutine DropConnection(DropConnectionRequest req,
                                      BasicResponseDelegate callback) {
      if (callback == null) {
        Debug.Log(
            "callback supplied is null, aborting DropConnection Request.");
        return null;
      }
      ...
      var client = UnityWebRequest.Post(uri.ToString(), data);
      return StartCoroutine(
          ProcessMatchResponse<DropConnectionResponse, BasicResponseDelegate>(
              client, OnDropConnection, callback));
    }

    internal void OnDropConnection(DropConnectionResponse response,
                                   BasicResponseDelegate userCallback) {
      userCallback(response.success, response.extendedInfo);
    }

    public Coroutine
    ListMatches(int startPageNumber, int resultPageSize, string matchNameFilter,
                bool filterOutPrivateMatchesFromResults, int eloScoreTarget,
                int requestDomain,
                DataResponseDelegate<List<MatchInfoSnapshot>> callback) {
      if (UnityEngine.Application.platform == RuntimePlatform.WebGLPlayer) {
        Debug.LogError("Matchmaking is not supported on WebGL player.");
        return null;
      } else
        return ListMatches(
            new ListMatchRequest{
                pageNum = startPageNumber, pageSize = resultPageSize,
                nameFilter = matchNameFilter,
                filterOutPrivateMatches = filterOutPrivateMatchesFromResults,
                eloScore = eloScoreTarget, domain = requestDomain},
            callback);
    }

    // Start getting a list of matches
    internal Coroutine
    ListMatches(ListMatchRequest req,
                DataResponseDelegate<List<MatchInfoSnapshot>> callback) {
      if (callback == null) {
      ...
      data.AddField("sourceId", Utility.GetSourceID().ToString());
      data.AddField("accessTokenString",
                    0); // Set access token to 0 for list requests
      data.AddField("domain", req.domain);

      data.AddField("pageSize", req.pageSize);
      data.AddField("pageNum", req.pageNum);
      data.AddField("nameFilter", req.nameFilter);
      data.AddField("filterOutPrivateMatches",
                    req.filterOutPrivateMatches.ToString());
      data.AddField("eloScore", req.eloScore.ToString());
      ...
      var client = UnityWebRequest.Post(uri.ToString(), data);
      return StartCoroutine(
          ProcessMatchResponse<ListMatchResponse,
                               DataResponseDelegate<List<MatchInfoSnapshot>>>(
              client, OnMatchList, callback));
    }

    internal void
    OnMatchList(ListMatchResponse response,
                DataResponseDelegate<List<MatchInfoSnapshot>> userCallback) {
    ...
    public Coroutine SetMatchAttributes(NetworkID networkId, bool isListed,
                                        int requestDomain,
                                        BasicResponseDelegate callback) {
      return SetMatchAttributes(
          new SetMatchAttributesRequest{networkId = networkId,
                                        isListed = isListed,
                                        domain = requestDomain},
          callback);
    }
    ...
    internal Coroutine SetMatchAttributes(SetMatchAttributesRequest req,
                                          BasicResponseDelegate callback) {
      if (callback == null) {
        Debug.Log(
            "callback supplied is null, aborting SetMatchAttributes Request.");
        return null;
      }
      ...
      return StartCoroutine(
          ProcessMatchResponse<BasicResponse, BasicResponseDelegate>(
              client, OnSetMatchAttributes, callback));
    }

    internal void OnSetMatchAttributes(BasicResponse response,
                                       BasicResponseDelegate userCallback) {
      userCallback(response.success, response.extendedInfo);
    }
    ...
    private IEnumerator
    ProcessMatchResponse<JSONRESPONSE, USERRESPONSEDELEGATETYPE>(
        UnityWebRequest client,
        InternalResponseDelegate<JSONRESPONSE, USERRESPONSEDELEGATETYPE>
            internalCallback,
        USERRESPONSEDELEGATETYPE userCallback) where JSONRESPONSE : Response,
                                                                    new () {
      ...
      if (client.result == UnityWebRequest.Result.Success) {
        try {
          JsonUtility.FromJsonOverwrite(client.downloadHandler.text,
                                        jsonInterface);
        } catch (ArgumentException exception) {
          jsonInterface.SetFailure(UnityString.Format(
              "ArgumentException:[{0}] ", exception.ToString()));
        }
      } else {
        jsonInterface.SetFailure(
            UnityString.Format("Request error:[{0}] Raw response:[{1}]",
                               client.error, client.downloadHandler.text));
      }
