...
public enum NetworkError {
	...
	WrongConnection, // connection id mismatch or connection is has not
	                 // connected yet
	...
	Timeout,
	// system cannot accomplish operation in time and connection will be closed,
	// or connection to slow, or % of drop packet is high
	MessageToLong,
	// sending message it too long for sending channel qos or providing buffer
	// is to small for receiving message
	WrongOperation, // attempt to start sending message to group before previous
	...
	UsageError // if any function is called with the wrong type of arguments or
	           // sizes
}

public enum ReactorModel { SelectReactor = 0, FixRateReactor = 1 }
...
public bool BelongsToSharedOrderChannel {
	get { return m_BelongsSharedOrderChannel; }
}
}

// connection configuration proxy class
// contains connection profile + channel definitions
// to allow user manipulate channel info via property with parameter (using
// array interface) all fields are defined for direct access from HLAPI (it it
// needed)
[Serializable]
[Obsolete(
	 "The UNET transport will be removed in the future as soon a replacement is ready.")]
...
public ConnectionConfig(ConnectionConfig config) {
	...
	m_MaxCombinedReliableMessageCount =
		config.m_MaxCombinedReliableMessageCount;
...
public static void Validate(ConnectionConfig config) {
	if (config.m_PacketSize <
	    g_MinPacketSize) // rough estimation of largest header, as it is
		             // defined in c++; we do not have access to it
		throw new ArgumentOutOfRangeException("PacketSize should be > " +
		                                      g_MinPacketSize);
	if (config.m_FragmentSize >= config.m_PacketSize - g_MinPacketSize)
		throw new ArgumentOutOfRangeException(
			      "FragmentSize should be < PacketSize - " + g_MinPacketSize);
	if (config.m_Channels.Count > 255)
		throw new ArgumentOutOfRangeException(
			      "Channels number should be less than 256");
}

// properties, it should be defines because set methods (which has not
// implemented yet) should check parameters correctnesses
...
public uint MinUpdateTimeout {
	...
	set {
		if (value == 0)
			throw new ArgumentOutOfRangeException(
				      "Minimal update timeout should be > 0");
...
[Obsolete(
	 "IsAcksLong is deprecated. Use AcksType = ConnectionAcksType.Acks64",
	 false)]
...
public bool UsePlatformSpecificProtocols {
	...
	set {
		if ((value == true) &&
		    (UnityEngine.Application.platform != RuntimePlatform.PS4))
			throw new ArgumentOutOfRangeException(
				      "Platform specific protocols are not supported on this platform");
...
public byte AddChannel(QosType value) {
	if (m_Channels.Count > 255)
		throw new ArgumentOutOfRangeException(
			      "Channels Count should be less than 256");
	if (!Enum.IsDefined(typeof(QosType), value)) {
		throw new ArgumentOutOfRangeException(
			      "requested qos type doesn't exist: " + (int) value);
	}
...
public void MakeChannelsSharedOrder(List<byte> channelIndices) {
	...
	if (channelIndices.Count == 0)
		throw new ArgumentOutOfRangeException(
			      "Received empty list of shared order channel indexes");
	for (byte i = 0; i < channelIndices.Count; i++) {
		byte channelId = channelIndices[i];
		if (channelId >= m_Channels.Count)
			throw new ArgumentOutOfRangeException(
				      "Shared order channel list contains wrong channel index " +
				      channelId);
		ChannelQOS channel = m_Channels[channelId];
		if (channel.BelongsToSharedOrderChannel)
			throw new ArgumentException(
				      "Channel with index " + channelId +
				      " has been already included to other shared order channel");
		if (!(channel.QOS == QosType.Reliable ||
		      channel.QOS == QosType.Unreliable))
			throw new ArgumentException(
				      "Only Reliable and Unreliable QoS are allowed for shared order channel, wrong channel is with index " +
				      channelId);
	}
	for (byte i = 0; i < channelIndices.Count; i++) {
		byte channelId = channelIndices[i];
...
public QosType GetChannel(byte idx) {
	if (idx >= m_Channels.Count)
		throw new ArgumentOutOfRangeException(
			      "requested index greater than maximum channels count");
...
public IList<byte> GetSharedOrderChannels(byte idx) {
	if (idx >= m_SharedOrderChannels.Count)
		throw new ArgumentOutOfRangeException(
			      "requested index greater than maximum shared order channels count");
	return m_SharedOrderChannels [idx]
	       .AsReadOnly();
}
...
// it includes maximum default connections with them configs (parameters +
// channels configuration) and array of special connection (with special
// configuration) AddSpecialConnection will return connection id which user
// should use when he call connect to identify special configuration for this
// connection
[Serializable]
[Obsolete(
	 "The UNET transport will be removed in the future as soon a replacement is ready.")]
...
private ushort m_ReceivedMessagePoolSize =
	1024; // TODO: default values should be initialized from c++ default
              // constants
[SerializeField]
...
public HostTopology(ConnectionConfig defaultConfig,
                    int maxDefaultConnections) {
	if (defaultConfig == null)
		throw new NullReferenceException("config is not defined");
	if (maxDefaultConnections <= 0)
		throw new ArgumentOutOfRangeException(
			      "maxConnections", "Number of connections should be > 0");
	if (maxDefaultConnections >= 65535)
		throw new ArgumentOutOfRangeException(
			      "maxConnections", "Number of connections should be < 65535");
...
public ConnectionConfig GetSpecialConnectionConfig(int i) {
	if (i > m_SpecialConnections.Count || i == 0)
		throw new ArgumentException(
			      "special configuration index is out of valid range");
...
public float MessagePoolSizeGrowthFactor {
	...
	set {
		if (value <= 0.5 || value > 1.0)
			throw new ArgumentException(
				      "pool growth factor should be varied between 0.5 and 1.0");
...
private const uint g_MaxTimerTimeout =
	12000; // before changing check UNETConfiguration.h file
...
public uint ThreadAwakeTimeout {
	...
	set {
		if (value == 0)
			throw new ArgumentOutOfRangeException(
				      "Minimal thread awake timeout should be > 0");
...
public ushort MaxHosts {
	...
	set {
		if (value == 0)
			throw new ArgumentOutOfRangeException(
				      "MaxHosts", "Maximum hosts number should be > 0");
		if (value > g_MaxHosts)
			throw new ArgumentOutOfRangeException(
				      "MaxHosts", "Maximum hosts number should be <= " + g_MaxHosts);
		m_MaxHosts = value;
	}
}
...
public uint MinTimerTimeout {
	...
	set {
		if (value > MaxTimerTimeout)
			throw new ArgumentOutOfRangeException(
				      "MinTimerTimeout should be < MaxTimerTimeout");
		if (value == 0)
			throw new ArgumentOutOfRangeException(
				      "MinTimerTimeout should be > 0");
...
public uint MaxTimerTimeout {
	...
	set {
		if (value == 0)
			throw new ArgumentOutOfRangeException(
				      "MaxTimerTimeout should be > 0");
		if (value > g_MaxTimerTimeout)
			throw new ArgumentOutOfRangeException("MaxTimerTimeout should be <=" +
			                                      g_MaxTimerTimeout);
...
public uint MinNetSimulatorTimeout {
	...
	set {
		if (value > MaxNetSimulatorTimeout)
			throw new ArgumentOutOfRangeException(
				      "MinNetSimulatorTimeout should be < MaxTimerTimeout");
		if (value == 0)
			throw new ArgumentOutOfRangeException(
				      "MinNetSimulatorTimeout should be > 0");
...
public uint MaxNetSimulatorTimeout {
	...
	set {
		if (value == 0)
			throw new ArgumentOutOfRangeException(
				      "MaxNetSimulatorTimeout should be > 0");
		if (value > g_MaxNetSimulatorTimeout)
			throw new ArgumentOutOfRangeException(
				      "MaxNetSimulatorTimeout should be <=" + g_MaxNetSimulatorTimeout);
...
public ConnectionSimulatorConfig(int outMinDelay, int outAvgDelay,
                                 int inMinDelay, int inAvgDelay,
