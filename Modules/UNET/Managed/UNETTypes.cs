...
namespace UnityEngine.Networking {
  ...
  public class NetworkAccessToken {
    ...
    public NetworkAccessToken() { array = new byte[NETWORK_ACCESS_TOKEN_SIZE]; }

    public NetworkAccessToken(byte[] array) { this.array = array; }
    ...
    public string GetByteString() { return Convert.ToBase64String(array); }

    public bool IsValid() {
      // This contains a valid token if we have an array, it's size is correct,
      // and it is not filled with 0
  ...
  // FIXME: Yank these when we can properly enumerate them from other sources
  // like Cloud code
  public partial class Utility {
    [Obsolete("This property is unused and should not be referenced in code.",
              true)]
    ...
    private static Dictionary<Types.NetworkID, Types.NetworkAccessToken>
        s_dictTokens =
            new Dictionary<Types.NetworkID, Types.NetworkAccessToken>();
    ...
    [Obsolete(
        "This function is unused and should not be referenced in code. Please sign in and setup your project in the editor instead.",
        true)]
    ...
    public static void
    SetAccessTokenForNetwork(Types.NetworkID netId,
                             Types.NetworkAccessToken accessToken) {
      // If we're updating an existing token we need to remove the stale one
      // first
    ...
    public static Types.NetworkAccessToken
    GetAccessTokenForNetwork(Types.NetworkID netId) {
