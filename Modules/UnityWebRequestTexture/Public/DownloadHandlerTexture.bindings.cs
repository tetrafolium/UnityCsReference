...
namespace UnityEngine.Networking {
  ...
  [NativeHeader(
      "Modules/UnityWebRequestTexture/Public/DownloadHandlerTexture.h")]
  public sealed class DownloadHandlerTexture : DownloadHandler {
    private Texture2D mTexture;
    private bool mHasTexture;
    private bool mNonReadable;

    private static extern IntPtr Create(DownloadHandlerTexture obj,
                                        bool readable);
    ...
    public DownloadHandlerTexture() { InternalCreateTexture(true); }
    ...
    private Texture2D InternalGetTexture() {
      if (mHasTexture) {
        if (mTexture == null) {
          // this is corner case when this DH survives scene reload, while
          // texture does not
