...
public static string ServiceTokenUrl {
	get {
		if (!string.IsNullOrEmpty(ServiceBaseUrl) &&
			...
			return new Uri(
				new Uri(ServiceBaseUrl),
				string.Format("token/{0}", CloudProjectSettings.projectId))
			       .ToString();
		}
...
public static string GetUsymUploadAuthToken() {
	...
	try {
		string environmentValue =
			Environment.GetEnvironmentVariable("USYM_UPLOAD_AUTH_TOKEN");
		...
		// Only OSX supports SSL certificate validation, disable checking on
		// other platforms. Fix when a Unity Web framework supports SSL.
...
static UploadPlatformConfig GetUploadPlatformConfig() {
	...
	if (Application.platform == RuntimePlatform.WindowsEditor) {
		config.UsymtoolPath = Paths.Combine(
			EditorApplication.applicationContentsPath, "Tools", "usymtool.exe");
		config.LzmaPath = Paths.Combine(
			EditorApplication.applicationContentsPath, "Tools", "lzma.exe");
		config.LogFilePath =
			Paths.Combine(Environment.GetFolderPath(
					      Environment.SpecialFolder.LocalApplicationData),
			              "Unity", "Editor", "symbol_upload.log");
	} else if (Application.platform == RuntimePlatform.OSXEditor) {
		config.UsymtoolPath =
			Paths.Combine(EditorApplication.applicationContentsPath, "Tools",
			              "macosx", "usymtool");
		config.LzmaPath = Paths.Combine(
			EditorApplication.applicationContentsPath, "Tools", "lzma");
		config.LogFilePath =
			Paths.Combine(Environment.GetEnvironmentVariable("HOME"), "Library",
			              "Logs", "Unity", "symbol_upload.log");
	} else if (Application.platform == RuntimePlatform.LinuxEditor) {
		config.UsymtoolPath = Paths.Combine(
			EditorApplication.applicationContentsPath, "Tools", "usymtool");
		config.LzmaPath = Paths.Combine(
			EditorApplication.applicationContentsPath, "Tools", "lzma-linux64");
		config.LogFilePath =
			Paths.Combine(Environment.GetEnvironmentVariable("HOME"), ".config",
			              "unity3d", "symbol_upload.log");
	}

	return config;
}

public static void UploadSymbolsInPath(string authToken, string symbolPath,
	...
	try {
		...
		string args = string.Format(
			"-symbolPath \"{0}\" -log \"{1}\" -filter \"{2}\" -excludeFilter \"{3}\"",
			symbolPath, platformConfig.LogFilePath, includeFilter,
			excludeFilter);
		...
		System.Diagnostics.Process nativeProgram =
		...
		Debug.LogWarningFormat(
			"Exception occurred attempting to upload symbols to Unity Performance Reporting service.  Native symbols will not be available for this build. Exception details:\n{0}\n{1}",
			ex.ToString(), ex.StackTrace);
	}
}
}
