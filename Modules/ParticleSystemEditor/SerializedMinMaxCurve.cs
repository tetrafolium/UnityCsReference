...
private string m_Name;   // Curve name. Used for creating unique name based on
                         // ParticleSystem name + Module name + m_Name
public GUIContent m_DisplayName;
private bool m_SignedRange;          // True if curve can have negative values
public float m_DefaultCurveScalar;   // Used when switching to curve from scalar
                                     // and scalar is 0. Ensures a valid y axis
                                     // range (must be positive)
public float
        m_RemapValue; // Used for remap UI values e.g for rotation: state is in
                      // radians but we want UI to show it in degrees
...
public SerializedMinMaxCurve(ModuleUI m, GUIContent displayName, string name,
                             bool signedRange, bool useProp0) {
	Init(m, displayName, name, signedRange, useProp0, true);
}

public SerializedMinMaxCurve(ModuleUI m, GUIContent displayName, string name,
                             bool signedRange, bool useProp0,
                             bool addCurveIfNeeded) {
	Init(m, displayName, name, signedRange, useProp0, addCurveIfNeeded);
}

void Init(ModuleUI m, GUIContent displayName, string uniqueName,
          bool signedRange, bool useProp0, bool addCurveIfNeeded) {
	m_Module = m;
	...
	scalar = useProp0 ? m.GetProperty0(m_Name, "scalar")
	: m.GetProperty(m_Name, "scalar");
	minScalar = useProp0 ? m.GetProperty0(m_Name, "minScalar")
	: m.GetProperty(m_Name, "minScalar");
	maxCurve = useProp0 ? m.GetProperty0(m_Name, "maxCurve")
	: m.GetProperty(m_Name, "maxCurve");
	minCurve = useProp0 ? m.GetProperty0(m_Name, "minCurve")
	: m.GetProperty(m_Name, "minCurve");
	minMaxState = useProp0 ? m.GetProperty0(m_Name, "minMaxState")
	: m.GetProperty(m_Name, "minMaxState");

	// Reconstruct added curves when we initialize
	if (addCurveIfNeeded) {
		if (state == MinMaxCurveState.k_Curve ||
		    state == MinMaxCurveState.k_TwoCurves) {
			if (m_Module.m_ParticleSystemUI.m_ParticleEffectUI
			    .IsParticleSystemUIVisible(m_Module.m_ParticleSystemUI))
				m.GetParticleSystemCurveEditor().AddCurveDataIfNeeded(
					GetUniqueCurveName(), CreateCurveData(Color.black));
		}
	}
...
public float minConstant {
	get { return minScalar.floatValue; }
...
public Vector2 GetAxisScalars() {
	return new Vector2(m_Module.GetXAxisScalar(),
	                   scalar.floatValue * m_RemapValue);
}

// Callback for Curve Editor to set axis labels back
public void SetAxisScalars(Vector2 axisScalars) {
	// X axis: TODO: We do not support changing the X values in the curve editor
	// yet
...
public bool OnCurveAreaMouseDown(int button, Rect drawRect,
                                 Rect curveRanges) {
	if (button == 0) {
		ToggleCurveInEditor();
		return true;
	...
	if (button == 1) {
		...
		AnimationCurveContextMenu.Show(drawRect, maxCurve != null ? maxCurve.Copy()
		                     : null, minCurve != null ? minCurve.Copy()
		                     : null, scalar != null ? scalar.Copy()
		                     : null, curveRanges,
		                               m_Module.GetParticleSystemCurveEditor());
		return true;
	}
...
public ParticleSystemCurveEditor.CurveData CreateCurveData(Color color) {
	System.Diagnostics.Debug.Assert(
		state != MinMaxCurveState
		.k_Scalar);      // We should not create curve data for scalars

	return new ParticleSystemCurveEditor.CurveData(
		GetUniqueCurveName(), m_DisplayName, GetMinCurve(), maxCurve, color,
		m_SignedRange, GetAxisScalars, SetAxisScalars, m_Module.foldout);
}
...
public void SetMinMaxState(MinMaxCurveState newState, bool addToCurveEditor) {
	...
	// Assign state AFTER matching data to new state AND removing curve from
	// curveEditor since it uses current 'state'
...
public string GetUniqueCurveName() {
	return SerializedModule.Concat(
		m_Module.GetUniqueModuleName(m_Module.serializedObject.targetObject),
		m_Name);
}
...
static bool AnimationCurveSupportsProcedural(AnimationCurve curve,
	...
	case AnimationUtility.PolynomialValid.InvalidPreWrapMode:
		failureReason =
			"Unsupported curve pre-wrap mode. Loop and ping-pong do not support procedural mode.";
		break;
	case AnimationUtility.PolynomialValid.InvalidPostWrapMode:
		failureReason =
			"Unsupported curve post-wrap mode. Loop and ping-pong do not support procedural mode.";
		break;
	case AnimationUtility.PolynomialValid.TooManySegments:
		failureReason =
			"Curve uses too many keys. Procedural mode does not support more than " +
			AnimationUtility.GetMaxNumPolynomialSegmentsSupported() + " keys";
		if (curve.keys[0].time != 0.0f ||
		    curve.keys[curve.keys.Length - 1].time != 1.0f)
			failureReason +=
				" (Additional keys are added to curves that do not start at 0, or do not end at 1)";
		failureReason += ".";
		break;
	}
	return false;
...
public bool SupportsProcedural(ref string failureReason) {
	...
	bool isMaxCurveValid = AnimationCurveSupportsProcedural(
		maxCurve.animationCurveValue, ref maxCurveFailureReason);
	if (!isMaxCurveValid)
		failureReason = maxCurveFailureReason;

	if ((state != MinMaxCurveState.k_TwoCurves) &&
	    (state != MinMaxCurveState.k_TwoScalars))
		return isMaxCurveValid;
	else {
		string minCurveFailureReason = "Min Curve: ";
		bool isMinCurveValid = AnimationCurveSupportsProcedural(
			minCurve.animationCurveValue, ref minCurveFailureReason);
		if (isMinCurveValid)
