...
internal class ParticleSystemUI {
  ...
  private string
      m_SupportsCullingTextLabel; // Cached version including bullet points
  ...
  private static PrefColor s_BoundsColor = new PrefColor(
      "Particle System/Bounds", 1.0f, 235.0f / 255.0f, 4.0f / 255.0f, 1.0f);

  protected class Texts {
    public GUIContent addModules =
        EditorGUIUtility.TrTextContent("", "Show/Hide Modules");
  ...
  public bool multiEdit {
    get {
      return (m_ParticleSystems != null) && (m_ParticleSystems.Length > 1);
    }
  }
  ...
  public void Init(ParticleEffectUI owner, ParticleSystem[] systems) {
    ...
    bool anyWithoutRenderers =
        m_ParticleSystems.FirstOrDefault(
            o => o.GetComponent<ParticleSystemRenderer>() == null) != null;
  ...
  private void InitRendererUI() {
    ...
    if (renderers.Count > 0) {
      System.Diagnostics.Debug.Assert(
          m_Modules.Last() ==
          null); // If hitting this assert we have either not cleaned up the
                 // previous renderer or hitting another module

      m_RendererSerializedObject = new SerializedObject(renderers.ToArray());
      m_Modules[m_Modules.Length - 1] = new RendererModuleUI(
          this, m_RendererSerializedObject, s_ModuleNames.Last());
    }
  }
  ...
  public void OnGUI(float width, bool fixedWidth) {
...

    {
      ...
      for (int i = 0; i < m_Modules.Length; ++i) {
        ...
        // Skip if not visible (except initial module which should always be
        // visible)
        ...
        if (module.foldout) {
          using(new EditorGUI.DisabledScope(!module.enabled)) {
            Rect moduleSize = EditorGUILayout.BeginVertical(
                ParticleSystemStyles.Get().modulePadding);
            {
              moduleSize.y -= 4; // pull background 'up' behind title to fill
                                 // rounded corners.
              moduleSize.height += 4;
              GUI.Label(moduleSize, GUIContent.none,
                        ParticleSystemStyles.Get().moduleBgStyle);
        ...
        // TODO: Get Texture instead of static preview. Render Icon (below
        // titlebar due to rounded corners)
        if (initialModule) {
          // Get preview of material or mesh
          ParticleSystemRenderer renderer =
              m_ParticleSystems[0].GetComponent<ParticleSystemRenderer>();
          float iconSize = 21;
          Rect iconRect = new Rect(moduleHeaderRect.x + 4,
                                   moduleHeaderRect.y + 2, iconSize, iconSize);
          ...
          if (isRepaintEvent && renderer != null) {
            ...
            if (!multiEdit) {
              if (renderer.renderMode == ParticleSystemRenderMode.Mesh) {
                if (renderer.mesh != null) {
                  ...
                  // If the asset is dirty we ensure to get a updated one by
                  // clearing cache of temporary previews
                  if (m_CachedMeshInstanceID != instanceID) {
                    m_CachedMeshInstanceID = instanceID;
                    m_CachedMeshDirtyCount = -1;
                  }
                  if (EditorUtility.GetDirtyCount(instanceID) !=
                      m_CachedMeshDirtyCount) {
            ...
            if (multiEdit ||
                (renderer.gameObject.hideFlags & HideFlags.NotEditable) != 0) {
              // Presets should use the default material.
              instanceID =
                  Material.GetDefaultParticleMaterial().GetInstanceID();
            }
            ...
            // Fill so we do not see the background when we have no icon (eg
            // multi-edit)
            if (!iconRendered) {
              GUI.Label(iconRect, GUIContent.none,
                        ParticleSystemStyles.Get().moduleBgStyle);
            }
          }
          ...
          if (!multiEdit &&
              EditorGUI.DropdownButton(iconRect, GUIContent.none,
            ...
            if (EditorGUI.actionKey) {
              ...
              Selection.activeInstanceID =
                  m_ParticleSystems[0].gameObject.GetInstanceID();
            }
          }
        }
        ...
        Rect checkMarkRect =
            new Rect(moduleHeaderRect.x + 2, moduleHeaderRect.y + 1, 13, 13);
        if (!initialModule &&
            GUI.Button(checkMarkRect, GUIContent.none, GUIStyle.none))
          module.enabled = !module.enabled;

        // Button logic for plus/minus (see below for UI)
        Rect plusRect =
            new Rect(moduleHeaderRect.x + moduleHeaderRect.width - 10,
                     moduleHeaderRect.y + moduleHeaderRect.height - 10, 10, 10);
        Rect plusRectInteract =
            new Rect(plusRect.x - 4, plusRect.y - 4, plusRect.width + 4,
                     plusRect.height + 4);
        Rect infoRect = new Rect(plusRect.x - 23, plusRect.y - 8, 20, 20);

        if (initialModule &&
            EditorGUI.DropdownButton(plusRectInteract, s_Texts.addModules,
                                     FocusType.Passive, GUIStyle.none))
          ShowAddModuleMenu();
        ...
        if (!initialModule) {
          ...
          GUIStyle style = EditorGUI.showMixedValue ? ParticleSystemStyles.Get()
                               .toggleMixed : ParticleSystemStyles.Get()
                               .toggle;
        ...
        if (initialModule &&
            !string.IsNullOrEmpty(m_SupportsCullingTextLabel)) {
          var supportsCullingText =
              new GUIContent("", ParticleSystemStyles.Get().warningIcon,
                             m_SupportsCullingTextLabel);
      ...
      foreach (ParticleSystem child in children) {
        if (child != ps) {
          bool alreadySelected =
              m_ParticleSystems.FirstOrDefault(
                  o => ParticleSystemEditorUtils.GetRoot(o) == child) != null;
          if (!alreadySelected)
            ShowBounds(child);
        }
      }
  ...
  public void ApplyProperties() {
    bool hasModifiedProperties =
        m_ParticleSystemSerializedObject.hasModifiedProperties;
    ...
    if (hasModifiedProperties) {
      ...
      foreach (ParticleSystem ps in m_ParticleSystems) {
        ...
        if (!ParticleEffectUI.IsStopped(root) &&
    ...
    if (m_RendererSerializedObject != null &&
        m_RendererSerializedObject.targetObject != null)
  ...
  void UpdateParticleSystemInfoString() {
    ...
    if (supportsCullingText != string.Empty) {
      if (supportsCullingText != m_SupportsCullingText ||
          m_SupportsCullingTextLabel == null) {
        m_SupportsCullingText = supportsCullingText;
        m_SupportsCullingTextLabel =
            "Procedural simulation is not supported because: " +
            supportsCullingText.Replace("\n", "\n" + s_Texts.bulletPoint);
      }
    } else {
      m_SupportsCullingText = null;
      m_SupportsCullingTextLabel = null;
  ...
  void ShowAddModuleMenu() {
    ...
    for (int i = 0; i < s_ModuleNames.Length; ++i) {
      if (m_Modules[i] == null || !m_Modules[i].visibleUI)
        menu.AddItem(new GUIContent(s_ModuleNames[i]), false, AddModuleCallback,
                     i);
      else
    ...
    menu.AddSeparator("");
    menu.AddItem(EditorGUIUtility.TrTextContent("Show All Modules"),
                 ParticleEffectUI.GetAllModulesVisible(), AddModuleCallback,
                 10000);
    menu.ShowAsContext();
    Event.current.Use();
  }
  ...
  void AddModuleCallback(object obj) {
    ...
    if (index >= 0 && index < m_Modules.Length) {
      ...
      m_ParticleEffectUI.SetAllModulesVisible(
          !ParticleEffectUI.GetAllModulesVisible());
    }
    ApplyProperties();
  }
  ...
  void ShowModuleMenu(int moduleIndex) {
    ...
    if (!ParticleEffectUI.GetAllModulesVisible())
      menu.AddItem(EditorGUIUtility.TrTextContent("Remove"), false,
                   ModuleMenuCallback, moduleIndex);
    else
      menu.AddDisabledItem(EditorGUIUtility.TrTextContent(
          "Remove")); // Do not allow remove module when always show modules is
                      // enabled
    EditorGUI.DoPropertyContextMenu(m_Modules[moduleIndex].m_Enabled, null,
                                    menu);
    Event.current.Use();
  }
  ...
  void EmitterMenuCallback(object obj) {
    ...
    switch (userData) {
    case 0:
      m_ParticleEffectUI.CreateParticleSystem(m_ParticleSystems[0],
  ...
  void ShowEmitterMenu() {
    ...
    menu.AddItem(EditorGUIUtility.TrTextContent("Show Location"), false,
                 EmitterMenuCallback, 2);
    menu.AddSeparator("");
    if (m_ParticleSystems[0].gameObject.activeInHierarchy)
      menu.AddItem(EditorGUIUtility.TrTextContent("Create Particle System"),
                   false, EmitterMenuCallback, 0);
    else
      menu.AddDisabledItem(
          EditorGUIUtility.TrTextContent("Create new Particle System"));

    menu.AddItem(EditorGUIUtility.TrTextContent("Reset"), false,
                 EmitterMenuCallback, 1);
    menu.ShowAsContext();
    Event.current.Use();
  }

  private static ModuleUI[] CreateUIModules(ParticleSystemUI e,
                                            SerializedObject so) {
    int index = 0;
    ...
    return new ModuleUI[]{
        ...
        null, // RendererModule is created separately in InitRendererUI (it can
              // be added/removed)
    };
  }
