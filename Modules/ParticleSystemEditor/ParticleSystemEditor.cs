...
internal class ParticleSystemInspector : Editor,
                                         ParticleEffectUIOwner {
  ParticleEffectUI m_ParticleEffectUI;
  GUIContent m_PreviewTitle =
      EditorGUIUtility.TrTextContent("Particle System Curves");
  ...
  private bool selectedInParticleSystemWindow {
    get {
      ...
      if (ParticleSystemEditorUtils.lockedParticleSystem == null)
        ...
        currentGameObject =
            ParticleSystemEditorUtils.lockedParticleSystem.gameObject;
  ...
  private void HierarchyOrProjectWindowWasChanged() {
    // target can be null if child particle system was deleted when
    // ParticleSystemWindow is open
  ...
  private void Init(bool forceInit) {
    IEnumerable<ParticleSystem> systems =
        from p in targets.OfType<ParticleSystem>() where(p != null) select p;
  ...
  void ShowEditorButtonGUI() {
    GUILayout.BeginHorizontal();
    {
      ...
      if (m_ParticleEffectUI == null || !m_ParticleEffectUI.multiEdit) {
        ...
        if (window)
          window.customEditor =
              this; // window can be created by LoadWindowLayout, when Editor
                    // starts up, so always make sure the custom editor member
                    // is set up (case 930005)
        ...
        if (GUILayout.Button(text, EditorStyles.miniButton,
          ...
          if (window && window.IsVisible() && alreadySelected) {
            ...
            if (window) {
              ...
              // Kill inspector gui first to ensure playback time is cached
              // properly
              Clear();
  ...
  private bool ShouldShowInspector() {
    // Only show the inspector GUI if we are not showing the
  ...
  public void OnSceneViewGUI(SceneView sceneView) {
    if (ShouldShowInspector()) {
      Init(false); // Here because can be called before inspector GUI so to
                   // prevent blinking GUI when selecting ps.
      if (m_ParticleEffectUI != null) {
  ...
  public override bool HasPreviewGUI() { return ShouldShowInspector(); }

  public override void DrawPreview(Rect previewArea) {
    ObjectPreview.DrawPreview(
        this, previewArea,
        new Object[]{targets[0]}); // only draw the first preview - all the
                                   // selected systems share it
  }
  ...
  public override GUIContent GetPreviewTitle() { return m_PreviewTitle; }
