...
partial class ModuleUI {
  ...
  protected static readonly Rect kUnsignedRange =
      new Rect(0, 0, 1, 1); // Vertical range 0...1
  protected static readonly Rect kSignedRange =
      new Rect(0, -1, 1, 2); // Vertical range -1...1
  ...
  public static readonly GUIStyle s_ControlRectStyle =
      new GUIStyle{margin = new RectOffset(0, 0, 2, 2)};

  // Alternative to BeginProperty when dealing with properties that should be
  // logically combined but are not parent child properties.
  public class PropertyGroupScope : GUI.Scope {
    ...
    public PropertyGroupScope(params SerializedProperty[] properties) {
      ...
      foreach (var serializedProperty in properties) {
        if (serializedProperty.serializedObject.targetObjectsCount == 1 &&
  ...
  protected static Rect GetControlRect(int height,
    ...
    return GUILayoutUtility.GetRect(0, height, s_ControlRectStyle,
                                    layoutOptions);
  }

  protected static Rect FieldPosition(Rect totalPosition,
                                      out Rect labelPosition) {
    labelPosition = new Rect(
        totalPosition.x + EditorGUI.indent, totalPosition.y,
        EditorGUIUtility.labelWidth - EditorGUI.indent, kSingleLineHeight);
    return new Rect(totalPosition.x + EditorGUIUtility.labelWidth,
                    totalPosition.y,
                    totalPosition.width - EditorGUIUtility.labelWidth,
  ...
  protected static bool PlusButton(Rect position) {
    return GUI.Button(new Rect(position.x - 2, position.y - 2, 12, 13),
                      GUIContent.none, "OL Plus");
  }

  protected static bool MinusButton(Rect position) {
    return GUI.Button(new Rect(position.x - 2, position.y - 2, 12, 13),
                      GUIContent.none, "OL Minus");
  }

  private static float FloatDraggable(Rect rect, SerializedProperty floatProp,
                                      float remap, float dragWidth) {
    return FloatDraggable(rect, floatProp, remap, dragWidth, kFormatString);
  }

  public static float FloatDraggable(Rect rect, float floatValue, float remap,
                                     float dragWidth, string formatString) {
    int id =
        EditorGUIUtility.GetControlID(1658656233, FocusType.Keyboard, rect);
    ...
    return EditorGUI.DoFloatField(
               EditorGUI.s_RecycledEditor, floatFieldRect, dragZoneRect, id,
               floatValue * remap, formatString,
               ParticleSystemStyles.Get().numberField, true) /
           remap;
  }

  public static float FloatDraggable(Rect rect, SerializedProperty floatProp,
                                     float remap, float dragWidth,
                                     string formatString) {
    EditorGUI.BeginProperty(rect, GUIContent.none, floatProp);

    EditorGUI.BeginChangeCheck();
    float floatValue = FloatDraggable(rect, floatProp.floatValue, remap,
                                      dragWidth, formatString);
  ...
  GUIVector3Field(GUIContent guiContent, SerializedProperty vecProp,
                  params GUILayoutOption[] layoutOptions) {
  ...
  public static float GUIFloat(GUIContent guiContent,
  ...
  public static float GUIFloat(Rect rect, GUIContent guiContent,
                               SerializedProperty floatProp,
                               string formatString = kFormatString) {
    guiContent = EditorGUI.BeginProperty(rect, guiContent, floatProp);
    PrefixLabel(rect, guiContent);
    float val = FloatDraggable(rect, floatProp, 1f, EditorGUIUtility.labelWidth,
                               formatString);
    EditorGUI.EndProperty();
    return val;
  }

  public static float GUIFloat(GUIContent guiContent, float floatValue,
                               string formatString,
                               params GUILayoutOption[] layoutOptions) {
    Rect rect = GetControlRect(kSingleLineHeight, layoutOptions);
    PrefixLabel(rect, guiContent);
    return FloatDraggable(rect, floatValue, 1f, EditorGUIUtility.labelWidth,
                          formatString);
  }

  public static void GUIButtonGroup(EditMode.SceneViewEditMode[] modes,
                                    GUIContent[] guiContents,
                                    GetBoundsFunc getBoundsOfTargets,
                                    Editor caller) {
  ...
  public static void
  GUISortingLayerField(GUIContent guiContent, SerializedProperty sortProperty,
                       params GUILayoutOption[] layoutOptions) {
    Rect rect = GetControlRect(kSingleLineHeight, layoutOptions);
    var label = EditorGUI.BeginProperty(rect, guiContent, sortProperty);
    EditorGUI.SortingLayerField(rect, label, sortProperty,
  ...
  private static bool Toggle(Rect rect, SerializedProperty boolProp) {
    ...
    EditorGUI.BeginChangeCheck();
    bool newValue = EditorGUI.Toggle(rect, boolProp.boolValue,
                                     ParticleSystemStyles.Get().toggle);
    if (EditorGUI.EndChangeCheck())
  ...
  public static bool GUIToggle(GUIContent guiContent,
  ...
  public static void GUILayerMask(GUIContent guiContent,
                                  SerializedProperty layerMaskProp,
                                  params GUILayoutOption[] layoutOptions) {
    Rect rect = GetControlRect(kSingleLineHeight, layoutOptions);
    guiContent = EditorGUI.BeginProperty(rect, guiContent, layerMaskProp);
    rect = PrefixLabel(rect, guiContent);
    EditorGUI.LayerMaskField(rect, layerMaskProp, GUIContent.none,
                             ParticleSystemStyles.Get().popup);
    EditorGUI.EndProperty();
  }
  ...
  public static bool GUIToggle(GUIContent guiContent, bool boolValue,
    ...
    rect = PrefixLabel(rect, guiContent);
    boolValue =
        EditorGUI.Toggle(rect, boolValue, ParticleSystemStyles.Get().toggle);
    return boolValue;
  }

  public static void
  GUIToggleWithFloatField(string name, SerializedProperty boolProp,
                          SerializedProperty floatProp, bool invertToggle,
                          params GUILayoutOption[] layoutOptions) {
    GUIToggleWithFloatField(EditorGUIUtility.TempContent(name), boolProp,
                            floatProp, invertToggle, layoutOptions);
  }

  public static void
  GUIToggleWithFloatField(GUIContent guiContent, SerializedProperty boolProp,
                          SerializedProperty floatProp, bool invertToggle,
                          params GUILayoutOption[] layoutOptions) {
    ...
    if (toggleValue) {
      float dragWidth = 25f;
      Rect floatDragRect =
          new Rect(rect.x + EditorGUIUtility.labelWidth + k_toggleWidth, rect.y,
                   rect.width - k_toggleWidth, rect.height);
  ...
  public static void
  GUIToggleWithIntField(string name, SerializedProperty boolProp,
                        SerializedProperty floatProp, bool invertToggle,
                        params GUILayoutOption[] layoutOptions) {
    GUIToggleWithIntField(EditorGUIUtility.TempContent(name), boolProp,
                          floatProp, invertToggle, layoutOptions);
  }

  public static void
  GUIToggleWithIntField(GUIContent guiContent, SerializedProperty boolProp,
                        SerializedProperty intProp, bool invertToggle,
                        params GUILayoutOption[] layoutOptions) {
    ...
    if (toggleValue) {
      float dragWidth = 25f;
      Rect intDragRect = new Rect(
          toggleRect.xMax, lineRect.y,
          lineRect.width - toggleRect.xMax + k_toggleWidth, lineRect.height);
      EditorGUI.BeginChangeCheck();
      int newValue =
          IntDraggable(intDragRect, null, intProp.intValue, dragWidth);
      if (EditorGUI.EndChangeCheck())
  ...
  public static void GUIObject(Rect rect, GUIContent label,
                               SerializedProperty objectProp,
                               System.Type objType) {
    label = EditorGUI.BeginProperty(rect, label, objectProp);
    rect = PrefixLabel(rect, label);
    EditorGUI.ObjectField(rect, objectProp, objType, GUIContent.none,
                          ParticleSystemStyles.Get().objectField);
    EditorGUI.EndProperty();
  }

  public static void
  GUIObjectFieldAndToggle(GUIContent label, SerializedProperty objectProp,
                          SerializedProperty boolProp,
                          params GUILayoutOption[] layoutOptions) {
    Rect rect = GetControlRect(kSingleLineHeight, layoutOptions);
    label = EditorGUI.BeginProperty(rect, label, objectProp);
  ...
  internal Object ParticleSystemValidator(Object[] references,
                                          System.Type objType,
  ...
  public int GUIListOfFloatObjectToggleFields(
      GUIContent label, SerializedProperty[] objectProps,
      EditorGUI.ObjectFieldValidator validator, GUIContent buttonTooltip,
      bool allowCreation, params GUILayoutOption[] layoutOptions) {
    int buttonPressed = -1;
    int numObjects = objectProps.Length;
    Rect rect = GUILayoutUtility.GetRect(
        0, (kSingleLineHeight + 2) * numObjects,
        layoutOptions); // the +1 is that label is on it own line
    rect.height = kSingleLineHeight;

    float indent = 10f;
    float floatFieldWidth = 35f;
    float space = 10f;
    float objectFieldWidth =
        rect.width - indent - floatFieldWidth - space * 2 - k_toggleWidth;
    ...
    using(new PropertyGroupScope(objectProps)) {
      ...
      for (int i = 0; i < numObjects; ++i) {
        ...
        Rect r2 = new Rect(rect.x + indent + floatFieldWidth + space, rect.y,
                           objectFieldWidth, rect.height);
        int id = EditorGUIUtility.GetControlID(1235498, FocusType.Keyboard, r2);

        EditorGUI.BeginProperty(r2, GUIContent.none, objectProp);
        EditorGUI.DoObjectField(r2, r2, id, null, objectProp, validator, true,
                                ParticleSystemStyles.Get().objectField);
        EditorGUI.EndProperty();

        if (objectProp.objectReferenceValue == null) {
          r2 = new Rect(rect.xMax - k_toggleWidth, rect.y + 3, k_toggleWidth,
                        k_toggleWidth);
          if (!allowCreation || GUI.Button(r2, buttonTooltip ?? GUIContent.none,
                                           ParticleSystemStyles.Get().plus))
            buttonPressed = i;
        }
  ...
  public static void GUIIntDraggableX2(GUIContent mainLabel, GUIContent label1,
  ...
  public static int IntDraggable(Rect rect, GUIContent label,
                                 SerializedProperty intProp, float dragWidth) {
  ...
  public static int GUIInt(GUIContent guiContent, SerializedProperty intProp,
                           params GUILayoutOption[] layoutOptions) {
    Rect rect = GUILayoutUtility.GetRect(0, kSingleLineHeight, layoutOptions);
    ...
    EditorGUI.BeginChangeCheck();
    int newValue =
        IntDraggable(rect, null, intProp.intValue, EditorGUIUtility.labelWidth);
    if (EditorGUI.EndChangeCheck())
      intProp.intValue = newValue;
  ...
  public static int IntDraggable(Rect rect, GUIContent label, int value,
                                 float dragWidth) {
    ...
    // copied
    float dragSensitity =
        Mathf.Max(1, Mathf.Pow(Mathf.Abs((float) value), 0.5f) * .03f);
    // float dragSensitity = Mathf.Clamp01(Mathf.Max(1,
    // Mathf.Pow(Mathf.Abs((float)value), 0.5f)) * 0.03f );
    return EditorGUI.DoIntField(
        EditorGUI.s_RecycledEditor, intFieldRect, dragZoneRect, id, value,
        EditorGUI.kIntFieldFormatString, ParticleSystemStyles.Get().numberField,
        true, dragSensitity);
  }

  public static void GUIMinMaxRange(GUIContent label,
                                    SerializedProperty vec2Prop,
                                    params GUILayoutOption[] layoutOptions) {
    Rect rect = GetControlRect(kSingleLineHeight, layoutOptions);
    label = EditorGUI.BeginProperty(rect, label, vec2Prop);
    rect = SubtractPopupWidth(
        rect); // There is actually no popup in this control but the layout is
               // nicer if the fields line up.
    rect = PrefixLabel(rect, label);
  ...
  public static bool GUIBoolAsPopup(GUIContent label,
                                    SerializedProperty boolProp,
                                    string[] options,
                                    params GUILayoutOption[] layoutOptions) {
    ...
    EditorGUI.BeginChangeCheck();
    int newValue = EditorGUI.Popup(rect, null, boolProp.boolValue ? 1 : 0,
                                   EditorGUIUtility.TempContent(options),
                                   ParticleSystemStyles.Get().popup);
    if (EditorGUI.EndChangeCheck())
  ...
  public static void
  GUIEnumMaskUVChannelFlags(GUIContent label, SerializedProperty enumProperty,
                            params GUILayoutOption[] layoutOptions) {
    Rect rect = GetControlRect(kSingleLineHeight, layoutOptions);
    label = EditorGUI.BeginProperty(rect, label, enumProperty);
    rect = PrefixLabel(rect, label);
    EditorGUI.BeginChangeCheck();
    int enumVal = (int)(UVChannelFlags) EditorGUI.EnumFlagsField(
        rect, (UVChannelFlags) enumProperty.intValue,
        ParticleSystemStyles.Get().popup);
    if (EditorGUI.EndChangeCheck())
  ...
  public static void GUIMask(GUIContent label, SerializedProperty intProp,
                             string[] options,
                             params GUILayoutOption[] layoutOptions) {
    Rect rect = GetControlRect(kSingleLineHeight, layoutOptions);
  ...
  public static void GUIMask(Rect rect, GUIContent label,
                             SerializedProperty intProp, string[] options) {
    label = EditorGUI.BeginProperty(rect, label, intProp);
    rect = PrefixLabel(rect, label);

    EditorGUI.BeginChangeCheck();
    int newValue = EditorGUI.MaskField(rect, intProp.intValue, options,
                                       ParticleSystemStyles.Get().popup);
    if (EditorGUI.EndChangeCheck())
      intProp.intValue = newValue;
  ...
  public static int GUIPopup(GUIContent label, SerializedProperty intProp,
                             GUIContent[] options,
                             params GUILayoutOption[] layoutOptions) {
    Rect rect = GetControlRect(kSingleLineHeight, layoutOptions);
  ...
  public static int GUIPopup(Rect rect, GUIContent label,
                             SerializedProperty intProp, GUIContent[] options) {
    label = EditorGUI.BeginProperty(rect, label, intProp);
    rect = PrefixLabel(rect, label);

    EditorGUI.BeginChangeCheck();
    int newValue = EditorGUI.Popup(rect, null, intProp.intValue, options,
                                   ParticleSystemStyles.Get().popup);
    if (EditorGUI.EndChangeCheck())
      intProp.intValue = newValue;
  ...
  public static int GUIPopup(GUIContent label, int intValue,
                             GUIContent[] options,
                             params GUILayoutOption[] layoutOptions) {
    Rect rect = GetControlRect(kSingleLineHeight, layoutOptions);
    rect = PrefixLabel(rect, label);
    var index = EditorGUI.Popup(rect, intValue, options,
                                ParticleSystemStyles.Get().popup);
    return index;
  }

  public static int GUIPopup(GUIContent label, int intValue,
                             GUIContent[] options, SerializedProperty property,
                             params GUILayoutOption[] layoutOptions) {
    Rect rect = GetControlRect(kSingleLineHeight, layoutOptions);
  ...
  public static Enum GUIEnumPopup(GUIContent label, Enum selected,
                                  SerializedProperty property,
                                  params GUILayoutOption[] layoutOptions) {
    Rect rect = GetControlRect(kSingleLineHeight, layoutOptions);
    label = EditorGUI.BeginProperty(rect, label, property);
    rect = PrefixLabel(rect, label);
    var e =
        EditorGUI.EnumPopup(rect, selected, ParticleSystemStyles.Get().popup);
    EditorGUI.EndProperty();
    return e;
  }

  private static Color GetColor(SerializedMinMaxCurve mmCurve) {
    return mmCurve.m_Module.m_ParticleSystemUI.m_ParticleEffectUI
        .GetParticleSystemCurveEditor()
        .GetCurveColor(mmCurve.maxCurve);
  }

  // Should return true to use the mousedown event
  public delegate bool CurveFieldMouseDownCallback(int button, Rect drawRect,
                                                   Rect curveRanges);

  // returns buttonID if clicked otherwise -1
  private static void
  GUICurveField(Rect position, SerializedProperty maxCurve,
                SerializedProperty minCurve, Color color, Rect ranges,
                CurveFieldMouseDownCallback mouseDownCallback) {
    int id =
        EditorGUIUtility.GetControlID(1321321231, FocusType.Keyboard, position);
    ...
    case EventType.Repaint: {
      ...
      if (minCurve == null) {
        EditorGUIUtility.DrawCurveSwatch(position2, null, maxCurve, color,
                                         EditorGUI.kCurveBGColor, ranges);
      } else {
        EditorGUIUtility.DrawRegionSwatch(position2, maxCurve, minCurve, color,
                                          EditorGUI.kCurveBGColor, ranges);
      }
    ...
    case EventType.MouseDown:
      if (position.Contains(evt.mousePosition)) {
        if (mouseDownCallback != null &&
            mouseDownCallback(evt.button, position, ranges))
          evt.Use();
      }
      break;
    }
  ...
  public static void GUIMinMaxCurve(GUIContent label,
                                    SerializedMinMaxCurve mmCurve,
                                    params GUILayoutOption[] layoutOptions) {
    GUIMinMaxCurve(label, mmCurve, null, layoutOptions);
  }

  public static void GUIMinMaxCurve(SerializedProperty editableLabel,
                                    SerializedMinMaxCurve mmCurve,
                                    params GUILayoutOption[] layoutOptions) {
    GUIMinMaxCurve(null, mmCurve, editableLabel, layoutOptions);
  }

  internal static void GUIMinMaxCurve(GUIContent label,
                                      SerializedMinMaxCurve mmCurve,
                                      SerializedProperty editableLabel,
                                      params GUILayoutOption[] layoutOptions) {
    ...
    if (editableLabel != null) {
      ...
      // Draw with the minimum width so we don't lose the draggable
      // functionality to the left of the float field
      float textWidth =
          ParticleSystemStyles.Get()
              .editableLabel
              .CalcSize(GUIContent.Temp(editableLabel.stringValue))
              .x;
      labelPosition.width =
          Mathf.Min(labelPosition.width, textWidth + kSpacingSubLabel);

      EditorGUI.BeginProperty(labelPosition, GUIContent.none, editableLabel);

      EditorGUI.BeginChangeCheck();
      string newString = EditorGUI.TextFieldInternal(
          GUIUtility.GetControlID(FocusType.Passive, labelPosition),
          labelPosition, editableLabel.stringValue,
          ParticleSystemStyles.Get().editableLabel);
      if (EditorGUI.EndChangeCheck())
    ...
    if (mixedState) {
      ...
      if (state == MinMaxCurveState.k_Scalar) {
        EditorGUI.BeginChangeCheck();
        float newValue =
            FloatDraggable(rect, mmCurve.scalar, mmCurve.m_RemapValue,
        ...
        EditorGUI.BeginChangeCheck();
        float newMinValue =
            FloatDraggable(halfRectWithDragger, mmCurve.minScalar,
                           mmCurve.m_RemapValue, kDragSpace, "g5");
        if (EditorGUI.EndChangeCheck() && !mmCurve.signedRange)
        ...
        EditorGUI.BeginChangeCheck();
        float newMaxValue =
            FloatDraggable(halfRectWithDragger, mmCurve.scalar,
                           mmCurve.m_RemapValue, kDragSpace, "g5");
        if (EditorGUI.EndChangeCheck() && !mmCurve.signedRange)
        ...
        SerializedProperty minCurve =
            (state == MinMaxCurveState.k_TwoCurves) ? mmCurve.minCurve : null;
        GUICurveField(controlRect, mmCurve.maxCurve, minCurve,
                      GetColor(mmCurve), previewRange,
                      mmCurve.OnCurveAreaMouseDown);
      }
    }
  ...
  public static Rect GUIMinMaxCurveInline(Rect rect,
                                          SerializedMinMaxCurve mmCurve,
                                          float dragWidth) {
    ...
    if (mixedState) {
      ...
      if (state == MinMaxCurveState.k_Scalar) {
        EditorGUI.BeginChangeCheck();
        float newValue = FloatDraggable(rect, mmCurve.scalar,
                                        mmCurve.m_RemapValue, dragWidth, "n0");
        if (EditorGUI.EndChangeCheck() && !mmCurve.signedRange)
          mmCurve.scalar.floatValue = Mathf.Max(newValue, 0f);
      } else if (state == MinMaxCurveState.k_TwoScalars) {
        Rect halfRect = rect;
        ...
        SerializedProperty minCurve =
            (state == MinMaxCurveState.k_TwoCurves) ? mmCurve.minCurve : null;
        GUICurveField(rect, mmCurve.maxCurve, minCurve, GetColor(mmCurve),
                      previewRange, mmCurve.OnCurveAreaMouseDown);
      }
    }
  ...
  public void GUIMinMaxGradient(GUIContent label,
                                SerializedMinMaxGradient minMaxGradient,
                                bool hdr,
                                params GUILayoutOption[] layoutOptions) {
    GUIMinMaxGradient(label, minMaxGradient, null, hdr, layoutOptions);
  }

  public void GUIMinMaxGradient(SerializedProperty editableLabel,
                                SerializedMinMaxGradient minMaxGradient,
                                bool hdr,
                                params GUILayoutOption[] layoutOptions) {
  ...
  internal void GUIMinMaxGradient(GUIContent label,
                                  SerializedMinMaxGradient minMaxGradient,
                                  SerializedProperty editableLabel, bool hdr,
                                  params GUILayoutOption[] layoutOptions) {
    ...
    bool useRandomness =
        !mixedState &&
        ((state == MinMaxGradientState.k_RandomBetweenTwoColors) ||
         (state == MinMaxGradientState.k_RandomBetweenTwoGradients));

    Rect rect = GUILayoutUtility.GetRect(0, useRandomness ? 2 * kSingleLineHeight
                                         : kSingleLineHeight, layoutOptions);
    ...
    if (mixedState) {
      ...
      case MinMaxGradientState.k_Color:
        // The color field looks too large compared to the other fields because
      ...
      case MinMaxGradientState.k_RandomColor:
        EditorGUI.BeginProperty(gradientRect, GUIContent.none,
                                minMaxGradient.m_MaxGradient);
        EditorGUI.GradientField(gradientRect, minMaxGradient.m_MaxGradient,
                                hdr);
        EditorGUI.EndProperty();
        break;
      ...
      case MinMaxGradientState.k_RandomBetweenTwoGradients:
        ...
        EditorGUI.BeginProperty(gradientRect, GUIContent.none,
                                minMaxGradient.m_MinGradient);
        EditorGUI.GradientField(gradientRect, minMaxGradient.m_MinGradient,
                                hdr);
        EditorGUI.EndProperty();
        break;
      }
  ...
  private static void GUIColor(Rect rect, SerializedProperty colorProp,
    ...
    Color newValue = EditorGUI.ColorField(
        rect, GUIContent.none, colorProp.colorValue, false, true, hdr);
    if (EditorGUI.EndChangeCheck())
      colorProp.colorValue = newValue;
    EditorGUI.EndProperty();
  }

  public void GUITripleMinMaxCurve(GUIContent label, GUIContent x,
                                   SerializedMinMaxCurve xCurve, GUIContent y,
                                   SerializedMinMaxCurve yCurve, GUIContent z,
                                   SerializedMinMaxCurve zCurve,
                                   SerializedProperty randomizePerFrame,
                                   params GUILayoutOption[] layoutOptions) {
    using(new PropertyGroupScope(xCurve.rootProperty, yCurve.rootProperty,
                                 zCurve.rootProperty)) {
      ...
      float[] labelWidth = new float[3]{
          ParticleSystemStyles.Get().label.CalcSize(GUIContent.Temp(x.text)).x +
              labelBorder,
          ParticleSystemStyles.Get().label.CalcSize(GUIContent.Temp(y.text)).x +
              labelBorder,
          ParticleSystemStyles.Get().label.CalcSize(GUIContent.Temp(z.text)).x +
              labelBorder};

      float fieldWidth =
          (rect.width - labelWidth[0] - labelWidth[1] - labelWidth[2]) / 3.0f;
      ...
      if (mixedState) {
        ...
        if (state == MinMaxCurveState.k_Scalar) {
          for (int i = 0; i < curves.Length; ++i) {
            ...
            Label(r, labels[i]);
            EditorGUI.BeginChangeCheck();
            float newValue = FloatDraggable(
                r, curves[i].scalar, curves[i].m_RemapValue, labelWidth[i]);
          ...
          for (int i = 0; i < curves.Length; ++i) {
            ...
            EditorGUI.BeginChangeCheck();
            maxConstant = FloatDraggable(r, maxConstant, curves[i].m_RemapValue,
                                         labelWidth[i], "g5");
            if (EditorGUI.EndChangeCheck()) {
            ...
            EditorGUI.BeginChangeCheck();
            minConstant = FloatDraggable(r, minConstant, curves[i].m_RemapValue,
                                         labelWidth[i], "g5");
            if (EditorGUI.EndChangeCheck()) {
          ...
          for (int i = 0; i < curves.Length; ++i) {
            r.width = fieldWidth + labelWidth[i] - labelBorder * 2;
            SerializedProperty minCurve =
                (state == MinMaxCurveState.k_TwoCurves) ? curves[i].minCurve
                                                        : null;

            using(minCurve == null ? new EditorGUI.PropertyScope(
                                  r, labels[i], curves[i].maxCurve)
                  : (GUI.Scope) new PropertyGroupScope(curves[i].maxCurve,
                                                       minCurve)) {
              Label(r, labels[i]);
              Rect r2 = r;
              r2.xMin += labelWidth[i];

              GUICurveField(r2, curves[i].maxCurve, minCurve,
                            GetColor(curves[i]), previewRange,
                            curves[i].OnCurveAreaMouseDown);
              r.x += fieldWidth + labelWidth[i] + labelBorder;
            }
          }
        }
  ...
  private class CurveStateCallbackData {
    public CurveStateCallbackData(MinMaxCurveState state,
                                  SerializedMinMaxCurve[] curves) {
      minMaxCurves = curves;
      selectedState = state;
    }
  ...
  static void SelectMinMaxCurveStateCallback(object obj) {
    ...
    // Get SerializedObject (through any SerializedProperty will do) and mark
    // override dirty.
    SerializedObject serializedObject =
        data.minMaxCurves[0].minScalar.serializedObject;
    serializedObject.SetIsDifferentCacheDirty();
    serializedObject.ApplyModifiedProperties();
  }

  public static void GUIMMCurveStateList(Rect rect,
  ...
  public static void GUIMMCurveStateList(Rect rect,
    ...
    if (EditorGUI.DropdownButton(
            rect, GUIContent.none, FocusType.Passive,
            ParticleSystemStyles.Get().minMaxCurveStateDropDown)) {
      ...
      MinMaxCurveState[] states = {
          MinMaxCurveState.k_Scalar, MinMaxCurveState.k_Curve,
          MinMaxCurveState.k_TwoScalars, MinMaxCurveState.k_TwoCurves};
      bool[] allowState = {
          minMaxCurves[0].m_AllowConstant, minMaxCurves[0].m_AllowCurves,
      ...
      for (int i = 0; i < texts.Length; ++i) {
        if (allowState[i])
          menu.AddItem(texts[i],
                       allowHighlight && (minMaxCurves[0].state == states[i]),
                       SelectMinMaxCurveStateCallback,
                       new CurveStateCallbackData(states[i], minMaxCurves));
      }
      menu.DropDown(rect);
      Event.current.Use();
    }
  ...
  private class GradientCallbackData {
    public GradientCallbackData(MinMaxGradientState state,
                                SerializedMinMaxGradient p) {
      gradientProp = p;
      selectedState = state;
    }
  ...
  static void SelectMinMaxGradientStateCallback(object obj) {
    ...
    // override dirty.
    SerializedObject serializedObject =
        data.gradientProp.m_MinColor.serializedObject;
    serializedObject.SetIsDifferentCacheDirty();
    serializedObject.ApplyModifiedProperties();
  }

  public static void GUIMMGradientPopUp(Rect rect,
                                        SerializedMinMaxGradient gradientProp) {
    if (EditorGUI.DropdownButton(
            rect, GUIContent.none, FocusType.Passive,
            ParticleSystemStyles.Get().minMaxCurveStateDropDown)) {
      GUIContent[] texts = {
      ...
      MinMaxGradientState[] states = {
          MinMaxGradientState.k_Color, MinMaxGradientState.k_Gradient,
      ...
      for (int i = 0; i < texts.Length; ++i) {
        if (allowState[i])
          menu.AddItem(texts[i],
                       allowHighlight && (gradientProp.state == states[i]),
                       SelectMinMaxGradientStateCallback,
                       new GradientCallbackData(states[i], gradientProp));
      }
      menu.ShowAsContext();
      Event.current.Use();
    }
  ...
  public static void GUIMMColorPopUp(Rect rect, SerializedProperty boolProp) {
    if (EditorGUI.DropdownButton(
      ...
      GenericMenu menu = new GenericMenu();
      GUIContent[] texts = {
          EditorGUIUtility.TrTextContent("Constant Color"),
          EditorGUIUtility.TrTextContent("Random Between Two Colors")};
      bool[] states = {false, true};

      for (int i = 0; i < texts.Length; ++i) {
        menu.AddItem(texts[i], (boolProp.boolValue == states[i]),
                     SelectMinMaxColorStateCallback,
                     new ColorCallbackData(states[i], boolProp));
      }
      menu.ShowAsContext();
      Event.current.Use();
    }
