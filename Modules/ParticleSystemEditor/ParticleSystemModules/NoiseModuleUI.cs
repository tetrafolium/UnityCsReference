...
class Texts {
public GUIContent separateAxes = EditorGUIUtility.TrTextContent(
	"Separate Axes",
	"If enabled, you can control the noise separately for each axis.");
public GUIContent strength = EditorGUIUtility.TrTextContent(
	"Strength", "How strong the overall noise effect is.");
public GUIContent frequency = EditorGUIUtility.TrTextContent(
	"Frequency",
	"Low values create soft, smooth noise, and high values create rapidly changing noise.");
public GUIContent damping = EditorGUIUtility.TrTextContent(
	"Damping", "If enabled, strength is proportional to frequency.");
public GUIContent octaves = EditorGUIUtility.TrTextContent(
	"Octaves",
	"Layers of noise that combine to produce final noise (Adding octaves increases the performance cost substantially!)");
public GUIContent octaveMultiplier = EditorGUIUtility.TrTextContent(
	"Octave Multiplier",
	"When combining each octave, scale the intensity by this amount.");
public GUIContent octaveScale = EditorGUIUtility.TrTextContent(
	"Octave Scale", "When combining each octave, zoom in by this amount.");
public GUIContent quality = EditorGUIUtility.TrTextContent(
	"Quality", "Generate 1D, 2D or 3D noise.");
public GUIContent scrollSpeed = EditorGUIUtility.TrTextContent(
	"Scroll Speed", "Scroll the noise map over the particle system.");
public GUIContent remap = EditorGUIUtility.TrTextContent(
	"Remap", "Remap the final noise values into a new range.");
public GUIContent remapCurve =
	EditorGUIUtility.TrTextContent("Remap Curve");
public GUIContent positionAmount = EditorGUIUtility.TrTextContent(
	"Position Amount",
	"What proportion of the noise is applied to the particle positions.");
public GUIContent rotationAmount = EditorGUIUtility.TrTextContent(
	"Rotation Amount",
	"What proportion of the noise is applied to the particle rotations, in degrees per second.");
public GUIContent sizeAmount = EditorGUIUtility.TrTextContent(
	"Size Amount",
	"Multiply the size of the particle by a proportion of the noise.");
public GUIContent x = EditorGUIUtility.TextContent("X");
public GUIContent y = EditorGUIUtility.TextContent("Y");
public GUIContent z = EditorGUIUtility.TextContent("Z");
public GUIContent previewTexture = EditorGUIUtility.TrTextContent(
	"Preview", "Preview the noise as a texture.");
public GUIContent previewTextureMultiEdit = EditorGUIUtility.TrTextContent(
	"Preview (Disabled)",
	"Preview is disabled in multi-object editing mode.");

public GUIContent[] qualityDropdown =
	new GUIContent[] {EditorGUIUtility.TrTextContent("Low (1D)"),
	                  EditorGUIUtility.TrTextContent("Medium (2D)"),
	                  EditorGUIUtility.TrTextContent("High (3D)")};
}
private static Texts s_Texts;

public NoiseModuleUI(ParticleSystemUI owner, SerializedObject o,
                     string displayName)
...
protected override void Init() {
	...
	m_StrengthX =
		new SerializedMinMaxCurve(this, s_Texts.x, "strength", kUseSignedRange);
	m_StrengthY = new SerializedMinMaxCurve(this, s_Texts.y, "strengthY",
	                                        kUseSignedRange);
	m_StrengthZ = new SerializedMinMaxCurve(this, s_Texts.z, "strengthZ",
	                                        kUseSignedRange);
	...
	m_ScrollSpeed = new SerializedMinMaxCurve(this, s_Texts.scrollSpeed,
	                                          "scrollSpeed", kUseSignedRange);
	m_ScrollSpeed.m_AllowRandom = false;
	m_RemapX =
		new SerializedMinMaxCurve(this, s_Texts.x, "remap", kUseSignedRange);
	m_RemapY =
		new SerializedMinMaxCurve(this, s_Texts.y, "remapY", kUseSignedRange);
	m_RemapZ =
		new SerializedMinMaxCurve(this, s_Texts.z, "remapZ", kUseSignedRange);
	...
	m_PositionAmount = new SerializedMinMaxCurve(
		this, s_Texts.positionAmount, "positionAmount", kUseSignedRange);
	m_RotationAmount = new SerializedMinMaxCurve(
		this, s_Texts.rotationAmount, "rotationAmount", kUseSignedRange);
	m_SizeAmount = new SerializedMinMaxCurve(this, s_Texts.sizeAmount,
	                                         "sizeAmount", kUseSignedRange);

	if (s_PreviewTexture == null) {
		s_PreviewTexture = new Texture2D(k_PreviewSize, k_PreviewSize,
...
override public void OnInspectorGUI(InitialModuleUI initial) {
	// values take 1 frame to appear in the module, so delay our generation of
	// the preview texture using this bool
	if (s_PreviewTextureDirty) {
		if (m_ParticleSystemUI.multiEdit) {
			Color32[] pixels =
				new Color32[s_PreviewTexture.width * s_PreviewTexture.height];
			...
			m_ParticleSystemUI
			.m_ParticleSystems [0]
			.GenerateNoisePreviewTexture(s_PreviewTexture);
		}
	...
	if (separateAxes) {
		...
		GUITripleMinMaxCurve(GUIContent.none, s_Texts.x, m_StrengthX, s_Texts.y,
		                     m_StrengthY, s_Texts.z, m_StrengthZ, null);
	} else {
	...
	using (new EditorGUI.DisabledScope(remapEnabled == false)) {
		if (separateAxes) {
			...
			GUITripleMinMaxCurve(GUIContent.none, s_Texts.x, m_RemapX, s_Texts.y,
			                     m_RemapY, s_Texts.z, m_RemapZ, null);
		} else {
	...
	if (EditorGUI.EndChangeCheck() || m_ScrollSpeed.scalar.floatValue != 0.0f ||
	    remapEnabled || separateAxesChanged) {
		s_PreviewTextureDirty = true;
		m_ParticleSystemUI.m_ParticleEffectUI.m_Owner.Repaint();
	}

	if (m_ParticleSystemUI.multiEdit)
		GUILayout.Label(s_Texts.previewTextureMultiEdit, previewTextureStyle,
		                GUILayout.ExpandWidth(true),
		                GUILayout.ExpandHeight(false));
	else
		GUILayout.Label(s_Texts.previewTexture, previewTextureStyle,
		                GUILayout.ExpandWidth(true),
		                GUILayout.ExpandHeight(false));
