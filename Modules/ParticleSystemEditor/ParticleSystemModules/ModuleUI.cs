...
public bool visibleUI {
	...
	set {
		SetVisibilityState(value ? VisibilityState.VisibleAndFolded
...
public bool foldout {
	...
	set {
		SetVisibilityState(value ? VisibilityState.VisibleAndFoldedOut
		         : VisibilityState.VisibleAndFolded);
	}
}

public bool enabled {
	get { return m_Enabled.boolValue; }
	set {
...
public bool isWindowView {
	get {
		return m_ParticleSystemUI.m_ParticleEffectUI.m_Owner is
		       ParticleSystemWindow;
	}
}

public ModuleUI(ParticleSystemUI owner, SerializedObject o, string name,
                string displayName)
	: base(o, name) {
...
public ModuleUI(ParticleSystemUI owner, SerializedObject o, string name,
                string displayName, VisibilityState initialVisibilityState)
	: base(o, name) {
	Setup(owner, o, displayName, initialVisibilityState);
}

private void Setup(ParticleSystemUI owner, SerializedObject o,
                   string displayName,
	...
	foreach (Object obj in o.targetObjects) {
		VisibilityState state = (VisibilityState) SessionState.GetInt(
			GetUniqueModuleName(obj), (int) defaultVisibilityState);
		m_VisibilityState = (VisibilityState) Mathf.Max(
			(int) state, (int) m_VisibilityState); // use most visible state
	}
...
}   // Like OnSceneGUI but only called once when in multi edit mode where
    // OnSceneGUI is called for each target.
...
public virtual void UndoRedoPerformed() {
	// If the undo operation has changed the module to now be disabled then we
	// should remove any of its curves from the curve editor. (case 861424)
	if (!enabled)
		OnModuleDisable();
}
...
protected virtual void OnModuleDisable() {
	Undo.undoRedoPerformed -= UndoRedoPerformed;
	ParticleSystemCurveEditor psce =
		m_ParticleSystemUI.m_ParticleEffectUI.GetParticleSystemCurveEditor();
...
internal void CheckVisibilityState() {
	...
	// Ensure disabled modules are only visible if show all modules is true.
	// Except the renderer module, we want that to be shown always if the module
	// is there which means that we have a ParticleSystemRenderer
	if (!isRendererModule && !m_Enabled.boolValue &&
...
protected virtual void SetVisibilityState(VisibilityState newState) {
	if (newState != m_VisibilityState) {
		if (newState == VisibilityState.VisibleAndFolded) {
			// Remove curves from the curveeditor when closing modules (and put them
			// back when folding out again)
			ParticleSystemCurveEditor psce = m_ParticleSystemUI.m_ParticleEffectUI
...
public virtual bool DrawHeader(Rect rect, GUIContent label) {
	...
	var toggleState = GUI.Toggle(rect, foldout, label,
	                             ParticleSystemStyles.Get().moduleHeaderStyle);
