...
public GUIContent dampen = EditorGUIUtility.TrTextContent(
	"Dampen",
	"Controls how much the velocity that exceeds the velocity limit should be dampened. A value of 0.5 will dampen the exceeding velocity by 50%.");
public GUIContent magnitude = EditorGUIUtility.TrTextContent(
	"Speed", "The speed limit of particles over the particle lifetime.");
...
public GUIContent drag = EditorGUIUtility.TrTextContent(
	"Drag",
	"Control the amount of drag applied to each particle during its lifetime.");
public GUIContent multiplyDragByParticleSize =
	EditorGUIUtility.TrTextContent(
		"Multiply by Size",
		"Adjust the drag based on the size of the particles.");
public GUIContent multiplyDragByParticleVelocity =
	EditorGUIUtility.TrTextContent(
...
public ClampVelocityModuleUI(ParticleSystemUI owner, SerializedObject o,
                             string displayName)
...
protected override void Init() {
	...
	m_Magnitude =
		new SerializedMinMaxCurve(this, s_Texts.magnitude, "magnitude");
	...
	m_MultiplyDragByParticleVelocity =
		GetProperty("multiplyDragByParticleVelocity");
}
...
override public void OnInspectorGUI(InitialModuleUI initial) {
	...
	if (separateAxes) {
		GUITripleMinMaxCurve(GUIContent.none, s_Texts.x, m_X, s_Texts.y, m_Y,
		                     s_Texts.z, m_Z, null);
	...
	GUIToggle(s_Texts.multiplyDragByParticleVelocity,
	          m_MultiplyDragByParticleVelocity);
	EditorGUI.indentLevel--;
}
