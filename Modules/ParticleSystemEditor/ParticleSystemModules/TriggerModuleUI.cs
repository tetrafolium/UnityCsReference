...
class TriggerModuleUI : ModuleUI {
  ...
  SerializedProperty[] m_CollisionShapes =
      new SerializedProperty[k_MaxNumCollisionShapes];
  ...
  class Texts {
    public GUIContent collisionShapes = EditorGUIUtility.TrTextContent(
        "Colliders", "The list of collision shapes to use for the trigger.");
    public GUIContent createCollisionShape = EditorGUIUtility.TrTextContent(
        "",
        "Create a GameObject containing a sphere collider and assigns it to the list.");
    public GUIContent inside = EditorGUIUtility.TrTextContent(
        "Inside",
        "What to do for particles that are inside the collision volume.");
    public GUIContent outside = EditorGUIUtility.TrTextContent(
        "Outside",
        "What to do for particles that are outside the collision volume.");
    public GUIContent enter = EditorGUIUtility.TrTextContent(
        "Enter", "Triggered once when particles enter the collision volume.");
    public GUIContent exit = EditorGUIUtility.TrTextContent(
        "Exit", "Triggered once when particles leave the collision volume.");
    ...
    public GUIContent[] overlapOptions =
        new GUIContent[]{EditorGUIUtility.TrTextContent("Ignore"),
                         EditorGUIUtility.TrTextContent("Kill"),
                         EditorGUIUtility.TrTextContent("Callback")};
  }
  private static Texts s_Texts;

  public TriggerModuleUI(ParticleSystemUI owner, SerializedObject o,
                         string displayName)
      : base(owner, o, "TriggerModule", displayName) {
    m_ToolTip =
        "Allows you to execute script code based on whether particles are inside or outside the collision shapes.";
  }
  ...
  protected override void Init() {
    ...
    List<SerializedProperty> shownCollisionShapes =
        new List<SerializedProperty>();
    for (int i = 0; i < m_CollisionShapes.Length; ++i) {
      m_CollisionShapes [i]
      = GetProperty(
          "collisionShape" +
          i); // Keep name in sync with transfer func in TriggerModule.h
  ...
  CreateDefaultCollider(string name, ParticleSystem parentOfGameObject) {
  ...
  private void DoListOfCollisionShapesGUI() {
    ...
    if (m_ParticleSystemUI.multiEdit) {
      for (int shapeIndex = 0; shapeIndex < k_MaxNumCollisionShapes;
           shapeIndex++) {
        int hasShape = -1;
        foreach (ParticleSystem ps in m_ParticleSystemUI.m_ParticleSystems) {
          ...
          if (hasShape == -1) {
            ...
            EditorGUILayout.HelpBox(
                "Collider list editing is only available when all selected systems contain the same number of colliders",
    ...
    int buttonPressedIndex = GUIListOfFloatObjectToggleFields(
        s_Texts.collisionShapes, m_ShownCollisionShapes, null,
        s_Texts.createCollisionShape, !m_ParticleSystemUI.multiEdit);
    if (buttonPressedIndex >= 0 && !m_ParticleSystemUI.multiEdit) {
      GameObject go =
          CreateDefaultCollider("Collider " + (buttonPressedIndex + 1),
                                m_ParticleSystemUI.m_ParticleSystems[0]);
      go.transform.localPosition = new Vector3(
          0, 0,
          10 + buttonPressedIndex); // ensure each collider is not at same pos
    ...
    if (m_ShownCollisionShapes.Length > 1) {
      if (MinusButton(rect)) {
        m_ShownCollisionShapes[m_ShownCollisionShapes.Length - 1]
            .objectReferenceValue = null;

        List<SerializedProperty> shownCollisionShapes =
            new List<SerializedProperty>(m_ShownCollisionShapes);
