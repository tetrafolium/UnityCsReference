...
class CollisionModuleUI : ModuleUI {
  ...
  static Transform
      s_SelectedTransform; // static so to ensure only one selected Transform
                           // across multiple particle systems
  internal static PrefColor s_CollisionBoundsColor =
      new PrefColor("Particle System/Collision Bounds", 0.0f, 1.0f, 0.0f, 1.0f);
  ...
  static readonly string s_UndoCollisionPlaneString =
      L10n.Tr("Modified Collision Plane Transform");

  class Texts {
    public GUIContent lifetimeLoss = EditorGUIUtility.TrTextContent(
        "Lifetime Loss",
        "When particle collides, it will lose this fraction of its Start Lifetime");
    public GUIContent planes = EditorGUIUtility.TrTextContent(
        "Planes",
        "Planes are defined by assigning a reference to a transform. This transform can be any transform in the scene and can be animated. Multiple planes can be used. Note: the Y-axis is used as the plane normal.");
    public GUIContent createPlane = EditorGUIUtility.TrTextContent(
        "", "Create an empty GameObject and assign it as a plane.");
    public GUIContent minKillSpeed = EditorGUIUtility.TrTextContent(
        "Min Kill Speed",
        "When particles collide and their speed is lower than this value, they are killed.");
    public GUIContent maxKillSpeed = EditorGUIUtility.TrTextContent(
        "Max Kill Speed",
        "When particles collide and their speed is higher than this value, they are killed.");
    public GUIContent dampen = EditorGUIUtility.TrTextContent(
        "Dampen",
        "When particle collides, it will lose this fraction of its speed. Unless this is set to 0.0, particle will become slower after collision.");
    public GUIContent bounce = EditorGUIUtility.TrTextContent(
        "Bounce",
        "When particle collides, the bounce is scaled with this value. The bounce is the upwards motion in the plane normal direction.");
    public GUIContent radiusScale = EditorGUIUtility.TrTextContent(
        "Radius Scale",
        "Scale particle bounds by this amount to get more precise collisions.");
    public GUIContent visualization = EditorGUIUtility.TrTextContent(
        "Visualization",
        "Only used for visualizing the planes: Wireframe or Solid.");
    public GUIContent scalePlane = EditorGUIUtility.TrTextContent(
        "Scale Plane", "Resizes the visualization planes.");
    public GUIContent visualizeBounds = EditorGUIUtility.TrTextContent(
        "Visualize Bounds", "Render the collision bounds of the particles.");
    public GUIContent collidesWith = EditorGUIUtility.TrTextContent(
        "Collides With",
        "Collides the particles with colliders included in the layermask.");
    public GUIContent collidesWithDynamic = EditorGUIUtility.TrTextContent(
        "Enable Dynamic Colliders",
        "Should particles collide with dynamic objects?");
    public GUIContent maxCollisionShapes = EditorGUIUtility.TrTextContent(
        "Max Collision Shapes",
        "How many collision shapes can be considered for particle collisions. Excess shapes will be ignored. Terrains take priority.");
    public GUIContent quality = EditorGUIUtility.TrTextContent(
        "Collision Quality",
        "Quality of world collisions. Medium and low quality are approximate and may leak particles.");
    public GUIContent voxelSize = EditorGUIUtility.TrTextContent(
        "Voxel Size",
        "Size of voxels in the collision cache. Smaller values improve accuracy but require higher memory usage and are less efficient.");
    public GUIContent collisionMessages = EditorGUIUtility.TrTextContent(
        "Send Collision Messages", "Send collision callback messages.");
    public GUIContent collisionType = EditorGUIUtility.TrTextContent(
        "Type", "Collide with a list of Planes, or the Physics World.");
    public GUIContent collisionMode =
        EditorGUIUtility.TrTextContent("Mode", "Use 3D Physics or 2D Physics.");
    public GUIContent colliderForce = EditorGUIUtility.TrTextContent(
        "Collider Force",
        "Control the strength of particle forces on colliders.");
    public GUIContent multiplyColliderForceByCollisionAngle =
        EditorGUIUtility.TrTextContent(
            "Multiply by Collision Angle",
            "Should the force be proportional to the angle of the particle collision?  A particle collision directly along the collision normal produces all the specified force whilst collisions away from the collision normal produce less force.");
    public GUIContent multiplyColliderForceByParticleSpeed =
        EditorGUIUtility.TrTextContent(
            "Multiply by Particle Speed",
            "Should the force be proportional to the particle speed?");
    public GUIContent multiplyColliderForceByParticleSize =
        EditorGUIUtility.TrTextContent(
            "Multiply by Particle Size",
            "Should the force be proportional to the particle size?");

    public GUIContent[] collisionTypes =
        new GUIContent[]{EditorGUIUtility.TrTextContent("Planes"),
                         EditorGUIUtility.TrTextContent("World")};

    public GUIContent[] collisionModes =
        new GUIContent[]{EditorGUIUtility.TrTextContent("3D"),
    ...
    public GUIContent[] planeVizTypes =
        new GUIContent[]{EditorGUIUtility.TrTextContent("Grid"),
    ...
    public GUIContent[] toolContents = {
        ...
        EditorGUIUtility.TrIconContent("RotateTool",
                                       "Rotate plane editing mode.")};
  }
  private static Texts s_Texts;

  public CollisionModuleUI(ParticleSystemUI owner, SerializedObject o,
                           string displayName)
      : base(owner, o, "CollisionModule", displayName) {
    m_ToolTip =
        "Allows you to specify multiple collision planes that the particle can collide with.";
  }

  private bool editingPlanes {
    get {
      return (
          (EditMode.editMode == EditMode.SceneViewEditMode
  ...
  protected override void Init() {
    ...
    for (int i = 0; i < m_Planes.Length; ++i) {
      ...
      = GetProperty(
          "plane" +
          i); // Keep name in sync with transfer func in CollisionModule.h
    ...
    m_LifetimeLossOnCollision = new SerializedMinMaxCurve(
        this, s_Texts.lifetimeLoss, "m_EnergyLossOnCollision");
    ...
    m_PlaneVisualizationType = (PlaneVizType) EditorPrefs.GetInt(
        "PlaneColisionVizType", (int) PlaneVizType.Solid);
    ...
    m_MultiplyColliderForceByCollisionAngle =
        GetProperty("multiplyColliderForceByCollisionAngle");
    m_MultiplyColliderForceByParticleSpeed =
        GetProperty("multiplyColliderForceByParticleSpeed");
  ...
  override public void OnInspectorGUI(InitialModuleUI initial) {
    ...
    CollisionTypes type = (CollisionTypes)
        GUIPopup(s_Texts.collisionType, m_Type, s_Texts.collisionTypes);
    ...
    if (type == CollisionTypes.Plane) {
      ...
      EditorGUI.BeginChangeCheck();
      m_PlaneVisualizationType = (PlaneVizType)
          GUIPopup(s_Texts.visualization, (int) m_PlaneVisualizationType,
                   s_Texts.planeVizTypes);
      if (EditorGUI.EndChangeCheck()) {
        EditorPrefs.SetInt("PlaneColisionVizType",
                           (int) m_PlaneVisualizationType);
      }
      ...
      GUIButtonGroup(
          s_SceneViewEditModes, s_Texts.toolContents,
    ...
    if (type == CollisionTypes.World) {
      ...
      EditorGUI.indentLevel++;
      GUIToggle(s_Texts.multiplyColliderForceByCollisionAngle,
                m_MultiplyColliderForceByCollisionAngle);
      GUIToggle(s_Texts.multiplyColliderForceByParticleSpeed,
                m_MultiplyColliderForceByParticleSpeed);
      GUIToggle(s_Texts.multiplyColliderForceByParticleSize,
                m_MultiplyColliderForceByParticleSize);
      EditorGUI.indentLevel--;
    }
  ...
  private void SyncVisualization() {
    ...
    if (m_ParticleSystemUI.multiEdit) {
      ...
      for (int i = 0; i < m_ShownPlanes.Length; ++i) {
        Transform transform =
            m_ShownPlanes[i].objectReferenceValue as Transform;
        if (transform != null && !m_ScenePlanes.Contains(transform))
          m_ScenePlanes.Add(transform);
      }
    }
  ...
  CreateEmptyGameObject(string name, ParticleSystem parentOfGameObject) {
  ...
  private void DoListOfPlanesGUI() {
    ...
    if (!IsListOfPlanesValid()) {
      EditorGUILayout.HelpBox(
          "Plane list editing is only available when all selected systems contain the same number of planes",
          MessageType.Info, true);
      return;
    }

    EditorGUI.BeginChangeCheck();
    int buttonPressedIndex = GUIListOfFloatObjectToggleFields(
        s_Texts.planes, m_ShownPlanes, null, s_Texts.createPlane,
    ...
    if (buttonPressedIndex >= 0 && !m_ParticleSystemUI.multiEdit) {
      GameObject go =
          CreateEmptyGameObject("Plane Transform " + (buttonPressedIndex + 1),
                                m_ParticleSystemUI.m_ParticleSystems[0]);
      go.transform.localPosition = new Vector3(
          0, 0,
          10 + buttonPressedIndex); // ensure each plane is not at same pos
      go.transform.localEulerAngles =
          (new Vector3(-90, 0, 0)); // make the plane normal point towards the
                                    // forward axis of the particle system
    ...
    Rect rect = GUILayoutUtility.GetRect(
        0, EditorGUI.kSingleLineHeight); // GUILayoutUtility.GetLastRect();
    rect.x =
    ...
    if (m_ShownPlanes.Length > 1) {
      if (MinusButton(rect)) {
        ...
        List<SerializedProperty> shownPlanes =
            new List<SerializedProperty>(m_ShownPlanes);
  ...
  private void CollisionPlanesSceneGUI() {
    ...
    for (int i = 0; i < m_ScenePlanes.Count; ++i) {
      ...
      bool isPlayingAndStatic =
          EditorApplication.isPlaying && transform.gameObject.isStatic;
      if (editingPlanes) {
        if (Object.ReferenceEquals(s_SelectedTransform, transform)) {
          ...
          // we want ignored mouse up events to check for dragging off of scene
          // view
          if (evt.type == EventType.Ignore && evt.rawType == EventType.MouseUp)
            oldEventType = evt.rawType;

          Handles.FreeMoveHandle(position, Quaternion.identity, handleSize,
          ...
          if (oldEventType == EventType.MouseDown &&
            ...
            GUIUtility.hotControl =
                0; // Reset hot control or the FreeMoveHandle will prevent input
                   // to the new Handles. (case 873514)
          }
        }
      }
  ...
  void RenderCollisionBounds() {
    ...
    Handles.SetDiscSectionPoints(points0, Vector3.zero, Vector3.forward,
                                 Vector3.right, 360, 1.0f);
    Handles.SetDiscSectionPoints(points1, Vector3.zero, Vector3.up,
                                 -Vector3.right, 360, 1.0f);
    Handles.SetDiscSectionPoints(points2, Vector3.zero, Vector3.right,
                                 Vector3.up, 360, 1.0f);

    Vector3[] points =
        new Vector3[points0.Length + points1.Length + points2.Length];
    ...
    foreach (ParticleSystem ps in m_ParticleSystemUI.m_ParticleSystems) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        float radius =
            System.Math.Max(size.x, System.Math.Max(size.y, size.z)) * 0.5f *
            ps.collision.radiusScale;
        Handles.matrix =
            transform * Matrix4x4.TRS(particle.position, Quaternion.identity,
  ...
  private static void DrawSolidPlane(Vector3 pos, Quaternion rot,
                                     Color faceColor, Color edgeColor) {
    if (Event.current.type != EventType.Repaint)
      return;

    var oldMatrix = Handles.matrix;
    float scale = 10 * m_ScaleGrid;
    Handles.matrix = Matrix4x4.TRS(pos, rot, new Vector3(scale, scale, scale)) *
                     Matrix4x4.TRS(Vector3.zero, Quaternion.Euler(90, 0, 0),
                                   Vector3.one); // Rotate plane by 90
    Handles.DrawSolidRectangleWithOutline(new Rect(-0.5f, -0.5f, 1, 1),
                                          faceColor, edgeColor);
    Handles.DrawLine(Vector3.zero, Vector3.back / scale);
    Handles.matrix = oldMatrix;
  }

  private static void DrawGrid(Vector3 pos, Vector3 axis1, Vector3 axis2,
