...
public GUIContent mode = EditorGUIUtility.TrTextContent(
	"Mode", "Select how trails are generated on the particles.");
public GUIContent ratio = EditorGUIUtility.TrTextContent(
	"Ratio", "Choose what proportion of particles will receive a trail.");
public GUIContent lifetime = EditorGUIUtility.TrTextContent(
	"Lifetime",
	"How long each trail will last, relative to the life of the particle.");
public GUIContent minVertexDistance = EditorGUIUtility.TrTextContent(
	"Minimum Vertex Distance",
	"The minimum distance each trail can travel before adding a new vertex.");
public GUIContent textureMode = EditorGUIUtility.TrTextContent(
	"Texture Mode", "Should the U coordinate be stretched or tiled?");
public GUIContent worldSpace = EditorGUIUtility.TrTextContent(
	"World Space",
	"Trail points will be dropped in world space, even if the particle system is simulating in local space.");
public GUIContent dieWithParticles = EditorGUIUtility.TrTextContent(
	"Die with Particles",
	"The trails will disappear when their owning particles die.");
public GUIContent sizeAffectsWidth = EditorGUIUtility.TrTextContent(
	"Size affects Width",
	"The trails will use the particle size to control their width.");
public GUIContent sizeAffectsLifetime = EditorGUIUtility.TrTextContent(
	"Size affects Lifetime",
	"The trails will use the particle size to control their lifetime.");
public GUIContent inheritParticleColor = EditorGUIUtility.TrTextContent(
	"Inherit Particle Color",
	"The trails will use the particle color as their base color.");
public GUIContent colorOverLifetime = EditorGUIUtility.TrTextContent(
	"Color over Lifetime",
	"The color of the trails during the lifetime of the particle they are attached to.");
public GUIContent widthOverTrail = EditorGUIUtility.TrTextContent(
	"Width over Trail",
	"Select a width for the trail from its start to end vertex.");
public GUIContent colorOverTrail = EditorGUIUtility.TrTextContent(
	"Color over Trail",
	"Select a color for the trail from its start to end vertex.");
public GUIContent generateLightingData = EditorGUIUtility.TrTextContent(
...
public GUIContent ribbonCount = EditorGUIUtility.TrTextContent(
	"Ribbon Count",
	"Select how many ribbons to render throughout the Particle System.");
public GUIContent splitSubEmitterRibbons = EditorGUIUtility.TrTextContent(
...
public GUIContent attachRibbonsToTransform = EditorGUIUtility.TrTextContent(
	"Attach Ribbons to Transform",
	"Connect each ribbon to the position of the Transform Component.");
...
public GUIContent[] textureModeOptions =
	new GUIContent[] {EditorGUIUtility.TrTextContent("Stretch"),
...
public TrailModuleUI(ParticleSystemUI owner, SerializedObject o,
                     string displayName)
...
protected override void Init() {
	...
	m_ColorOverLifetime =
		new SerializedMinMaxGradient(this, "colorOverLifetime");
	m_WidthOverTrail = new SerializedMinMaxCurve(this, s_Texts.widthOverTrail,
	                                             "widthOverTrail");
...
override public void OnInspectorGUI(InitialModuleUI initial) {
	ParticleSystemTrailMode mode = (ParticleSystemTrailMode)
	                               GUIPopup(s_Texts.mode, m_Mode, s_Texts.trailModeOptions);
	...
	// Add a warning message when no trail material is assigned, telling users
	// where to find it
