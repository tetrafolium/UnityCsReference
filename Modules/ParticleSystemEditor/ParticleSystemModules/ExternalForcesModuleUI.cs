...
class Texts {
public GUIContent multiplier = EditorGUIUtility.TrTextContent(
	"Multiplier",
	"Used to scale the force applied to this particle system.");
public GUIContent influenceFilter = EditorGUIUtility.TrTextContent(
	"Influence Filter",
	"Use either a LayerMask or a List, to decide which Force Fields affect this Particle System.");
public GUIContent influenceMask = EditorGUIUtility.TrTextContent(
	"Influence Mask",
	"Select a global mask of which GameObjects can affect this Particle System.");

public GUIContent[] influenceFilters =
	new GUIContent[] {EditorGUIUtility.TrTextContent("Layer Mask"),
	                  EditorGUIUtility.TrTextContent("List"),
	                  EditorGUIUtility.TrTextContent("Layer Mask and List")};
}
static Texts s_Texts;

public ExternalForcesModuleUI(ParticleSystemUI owner, SerializedObject o,
                              string displayName)
...
protected override void Init() {
	...
	m_Multiplier =
		new SerializedMinMaxCurve(this, s_Texts.multiplier, "multiplierCurve");
	...
	m_InfluenceListView = new ReorderableList(serializedObject, m_InfluenceList,
	                                          true, true, true, true);
...
override public void OnInspectorGUI(InitialModuleUI initial) {
	...
	ParticleSystemGameObjectFilter filter = (ParticleSystemGameObjectFilter)
	                                        GUIPopup(s_Texts.influenceFilter, m_InfluenceFilter,
	                                                 s_Texts.influenceFilters);
	if (m_InfluenceFilter.hasMultipleDifferentValues) {
		EditorGUILayout.HelpBox(
			"Influence List editing is only available when all selected systems have the same filter type",
...
override public void OnSceneViewGUI() {
	using (new Handles.DrawingScope(Handles.matrix)) {
		ParticleSystemForceField[] forceFields =
...
private void DrawInfluenceListElementCallback(Rect rect, int index,
                                              bool isActive, bool isFocused) {
	SerializedProperty forceField =
		m_InfluenceList.GetArrayElementAtIndex(index);
	EditorGUI.ObjectField(rect, forceField, null, GUIContent.none,
