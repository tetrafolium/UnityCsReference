...
class ShapeModuleUI : ModuleUI {
  struct MultiModeParameter {
    ...
    public static MultiModeParameter GetProperty(ModuleUI ui, string name,
                                                 GUIContent speed,
      ...
      result.m_Speed = new SerializedMinMaxCurve(ui, speed, name + ".speed",
                                                 kUseSignedRange);
      result.m_Speed.m_AllowRandom = false;
      return result;
    }

    public void OnInspectorGUI(MultiModeTexts text, GUIContent[] modesText,
                               bool showSpread = true, bool showSpeed = true) {
      if (m_Value != null)
  ...
  private static readonly Matrix4x4 s_ArcHandleOffsetMatrix =
  ...
  private static PrefColor s_GizmoColor =
      new PrefColor("Particle System/Shape Module Gizmos", 148f / 255f,
                    229f / 255f, 1f, 0.9f);
  private static Color s_ShapeGizmoThicknessTint =
      new Color(0.7f, 0.7f, 0.7f, 1.0f);

  private Mesh m_SpriteMesh;

  private readonly ParticleSystemShapeType[] m_GuiTypes =
      new[]{ParticleSystemShapeType.Sphere,
  ...
  private readonly int[] m_TypeToGuiTypeIndex =
      new[]{0, 0, 1, 1, 2, 4, 5, 2, 2, 2, 10, 10, 11, 6, 7, 4, 4, 3, 12, 8, 9};

  private readonly ParticleSystemShapeType[] boxShapes =
      new ParticleSystemShapeType[]{ParticleSystemShapeType.Box,
                                    ParticleSystemShapeType.BoxShell,
                                    ParticleSystemShapeType.BoxEdge};
  private readonly ParticleSystemShapeType[] coneShapes =
      new ParticleSystemShapeType[]{ParticleSystemShapeType.Cone,
                                    ParticleSystemShapeType.ConeVolume};

  static EditMode.SceneViewEditMode[] s_SceneViewEditModes =
      new[]{EditMode.SceneViewEditMode.ParticleSystemShapeModuleGizmo,
  ...
  class Texts {
    public GUIContent shape = EditorGUIUtility.TrTextContent(
        "Shape",
        "Defines the shape of the volume from which particles can be emitted, and the direction of the start velocity.");
    public GUIContent radius =
        EditorGUIUtility.TrTextContent("Radius", "Radius of the shape.");
    public GUIContent radiusThickness = EditorGUIUtility.TrTextContent(
        "Radius Thickness",
        "Control the thickness of the spawn volume, from 0 to 1.");
    public GUIContent coneAngle =
        EditorGUIUtility.TrTextContent("Angle", "Angle of the cone.");
    public GUIContent coneLength =
        EditorGUIUtility.TrTextContent("Length", "Length of the cone.");
    public GUIContent boxThickness = EditorGUIUtility.TrTextContent(
        "Box Thickness",
        "When using shell/edge modes, control the thickness of the spawn volume, from 0 to 1.");
    public GUIContent meshType = EditorGUIUtility.TrTextContent(
        "Type", "Generate particles from vertices, edges or triangles.");
    public GUIContent mesh = EditorGUIUtility.TrTextContent(
        "Mesh", "Mesh that the particle system will emit from.");
    public GUIContent meshRenderer = EditorGUIUtility.TrTextContent(
        "Mesh", "MeshRenderer that the particle system will emit from.");
    public GUIContent skinnedMeshRenderer = EditorGUIUtility.TrTextContent(
        "Mesh", "SkinnedMeshRenderer that the particle system will emit from.");
    public GUIContent sprite = EditorGUIUtility.TrTextContent(
        "Sprite", "Sprite that the particle system will emit from.");
    public GUIContent spriteRenderer = EditorGUIUtility.TrTextContent(
        "Sprite", "SpriteRenderer that the particle system will emit from.");
    public GUIContent meshMaterialIndex = EditorGUIUtility.TrTextContent(
        "Single Material", "Only emit from a specific material of the mesh.");
    public GUIContent useMeshColors = EditorGUIUtility.TrTextContent(
        "Use Mesh Colors",
        "Modulate particle color with mesh vertex colors, or if they don't exist, use the shader color property \"_Color\" or \"_TintColor\" from the material. Does not read texture colors.");
    public GUIContent meshNormalOffset = EditorGUIUtility.TrTextContent(
        "Normal Offset",
        "Offset particle spawn positions along the mesh normal.");
    public GUIContent texture = EditorGUIUtility.TrTextContent(
        "Texture", "Texture that the particles will sample their color from.");
    public GUIContent textureClipChannel = EditorGUIUtility.TrTextContent(
        "Clip Channel", "Select a channel to use for discarding particles.");
    public GUIContent textureClipThreshold = EditorGUIUtility.TrTextContent(
        "Clip Threshold",
        "Only emit from parts of the texture where the Clip Channel is greater than or equal to this value.");
    public GUIContent textureColorAffectsParticles =
        EditorGUIUtility.TrTextContent(
            "Color affects Particles",
            "Multiply the particle color by the texture RGB value.");
    public GUIContent textureAlphaAffectsParticles =
        EditorGUIUtility.TrTextContent(
            "Alpha affects Particles",
            "Multiply the particle alpha by the texture alpha value.");
    public GUIContent textureBilinearFiltering = EditorGUIUtility.TrTextContent(
        "Bilinear Filtering", "Blend between pixels on the texture.");
    public GUIContent textureUVChannel = EditorGUIUtility.TrTextContent(
        "UV Channel",
        "Use the selected UV channel from the source mesh, for reading the texture.");
    public GUIContent alignToDirection = EditorGUIUtility.TrTextContent(
        "Align To Direction",
        "Automatically align particles based on their initial direction of travel.");
    public GUIContent randomDirectionAmount = EditorGUIUtility.TrTextContent(
        "Randomize Direction",
        "Override the initial direction of travel with a random direction.");
    public GUIContent sphericalDirectionAmount = EditorGUIUtility.TrTextContent(
        "Spherize Direction",
        "Override the initial direction of travel with a direction that projects particles outwards from the center of the Shape Transform.");
    public GUIContent randomPositionAmount = EditorGUIUtility.TrTextContent(
        "Randomize Position",
        "Move the starting positions by a random amount, up to this maximum value.");
    public GUIContent emitFrom = EditorGUIUtility.TrTextContent(
        "Emit from:", "Specifies from where particles are emitted.");
    public GUIContent donutRadius = EditorGUIUtility.TrTextContent(
        "Donut Radius",
        "The radius of the donut. Used to control the thickness of the ring.");
    public GUIContent position = EditorGUIUtility.TrTextContent(
        "Position", "Translate the emission shape.");
    public GUIContent rotation = EditorGUIUtility.TrTextContent(
        "Rotation", "Rotate the emission shape.");
    public GUIContent scale =
        EditorGUIUtility.TrTextContent("Scale", "Scale the emission shape.");

    public readonly string undoSphereThickness =
        L10n.Tr("Sphere Thickness Handle Change");
    public readonly string undoSphere = L10n.Tr("Sphere Handle Change");
    public readonly string undoCircleThickness =
        L10n.Tr("Circle Thickness Handle Change");
    public readonly string undoCircle = L10n.Tr("Circle Handle Change");
    public readonly string undoHemisphereThickness =
        L10n.Tr("Hemisphere Thickness Handle Change");
    public readonly string undoHemisphere = L10n.Tr("Hemisphere Handle Change");
    public readonly string undoConeThickness =
        L10n.Tr("Cone Thickness Handle Change");
    public readonly string undoCone = L10n.Tr("Cone Handle Change");
    public readonly string undoConeVolumeThickness =
        L10n.Tr("Cone Volume Thickness Handle Change");
    public readonly string undoConeVolume =
        L10n.Tr("Cone Volume Handle Change");
    public readonly string undoBox = L10n.Tr("Box Handle Change");
    public readonly string undoDonut = L10n.Tr("Donut Handle Change");
    public readonly string undoDonutRadiusThickness =
        L10n.Tr("Donut Radius Thickness Handle Change");
    public readonly string undoDonutRadius =
        L10n.Tr("Donut Radius Handle Change");
    ...
    public GUIContent[] boxTypes =
        new GUIContent[]{EditorGUIUtility.TrTextContent("Volume"),
                         EditorGUIUtility.TrTextContent("Shell"),
                         EditorGUIUtility.TrTextContent("Edge")};

    public GUIContent[] coneTypes =
        new GUIContent[]{EditorGUIUtility.TrTextContent("Base"),
                         EditorGUIUtility.TrTextContent("Volume")};

    public GUIContent[] meshTypes =
        new GUIContent[]{EditorGUIUtility.TrTextContent("Vertex"),
                         EditorGUIUtility.TrTextContent("Edge"),
                         EditorGUIUtility.TrTextContent("Triangle")};

    public GUIContent[] emissionModes =
        new GUIContent[]{EditorGUIUtility.TrTextContent("Random"),
                         EditorGUIUtility.TrTextContent("Loop"),
                         EditorGUIUtility.TrTextContent("Ping-Pong"),
                         EditorGUIUtility.TrTextContent("Burst Spread")};

    public GUIContent[] emissionModesMesh =
        new GUIContent[]{EditorGUIUtility.TrTextContent("Random"),
                         EditorGUIUtility.TrTextContent("Loop"),
                         EditorGUIUtility.TrTextContent("Ping-Pong")};

    public GUIContent[] textureClipChannels =
        new GUIContent[]{EditorGUIUtility.TrTextContent("Red"),
    ...
    public GUIContent[] toolContents = {
        EditorGUIUtility.TrIconContent("ParticleShapeTool",
                                       "Shape gizmo editing mode."),
        EditorGUIUtility.TrIconContent(
            "MoveTool", "Shape transform position editing mode."),
        EditorGUIUtility.TrIconContent(
            "RotateTool", "Shape transform rotation editing mode."),
        EditorGUIUtility.TrIconContent("ScaleTool",
  ...
  class MultiModeTexts {
    public MultiModeTexts(string _value, string _mode, string _spread,
                          string _speed) {
  ...
  static MultiModeTexts s_RadiusTexts = new MultiModeTexts(
      ...
      /*_spread:*/
      "Spread|Spawn particles only at specific positions along the radius (0 to disable).",
      /*_speed:*/
      "Speed|Control the speed that the emission position moves along the radius.");
  ...
  static MultiModeTexts s_ArcTexts = new MultiModeTexts(
      ...
      /*_spread:*/
      "Spread|Spawn particles only at specific angles around the arc (0 to disable).",
      /*_speed:*/
      "Speed|Control the speed that the emission position moves around the arc.");

  static MultiModeTexts s_MeshTexts = new MultiModeTexts(
      /*_value:*/ null,
      /*_mode:*/ "Mode|Control how particles are spawned across the mesh.",
      /*_spread:*/
      "Spread|Spawn particles only at specific positions across the mesh (0 to disable).",
      /*_speed:*/
      "Speed|Control the speed that the emission position moves along the edges of the mesh.");

  public ShapeModuleUI(ParticleSystemUI owner, SerializedObject o,
                       string displayName)
      : base(owner, o, "ShapeModule", displayName,
  ...
  private bool editToolActive {
    get {
      if (EditMode.IsOwner(
              m_ParticleSystemUI.m_ParticleEffectUI.m_Owner.customEditor)) {
  ...
  protected override void Init() {
    ...
    m_Radius =
        MultiModeParameter.GetProperty(this, "radius", s_RadiusTexts.speed);
    ...
    m_MeshSpawn = MultiModeParameter.GetProperty(this, "m_MeshSpawn",
                                                 s_MeshTexts.speed, true);
    ...
    m_TextureColorAffectsParticles =
        GetProperty("m_TextureColorAffectsParticles");
    ...
    if (!s_TextureMaterial) {
      s_TextureMaterial = new Material((Shader) EditorGUIUtility.Load(
          "SceneView/ParticleShapeGizmo.shader"));
      s_TextureMaterial.hideFlags = HideFlags.HideAndDontSave;
    }
    if (!s_SphereTextureMaterial) {
      s_SphereTextureMaterial = new Material((Shader) EditorGUIUtility.Load(
          "SceneView/ParticleShapeGizmoSphere.shader"));
      s_SphereTextureMaterial.hideFlags = HideFlags.HideAndDontSave;
    }
    if (!s_CircleMesh)
      s_CircleMesh =
          ((GameObject) EditorGUIUtility.Load("SceneView/Circle.fbx"))
    ...
    if (!s_SphereMesh)
      s_SphereMesh =
          Resources.GetBuiltinResource(typeof(Mesh), "New-Sphere.fbx") as Mesh;
    if (!s_HemisphereMesh)
      s_HemisphereMesh =
          ((GameObject) EditorGUIUtility.Load("SceneView/Hemisphere.fbx"))
              .transform.GetComponent<MeshFilter>()
              .sharedMesh;
  }
  ...
  override public void OnInspectorGUI(InitialModuleUI initial) {
    ...
    if (!m_Type.hasMultipleDifferentValues) {
      ...
      case ParticleSystemShapeType.Box: {
        int emitFrom =
            ConvertBoxTypeToBoxEmitFrom((ParticleSystemShapeType) type);
        emitFrom =
            GUIPopup(s_Texts.emitFrom, emitFrom, s_Texts.boxTypes, m_Type);
        type = (int) ConvertBoxEmitFromToBoxType(emitFrom);

        if (type == (int) ParticleSystemShapeType.BoxShell ||
            type == (int) ParticleSystemShapeType.BoxEdge)
      ...
      case ParticleSystemShapeType.Cone: {
        ...
        int emitFrom =
            ConvertConeTypeToConeEmitFrom((ParticleSystemShapeType) type);
        emitFrom =
            GUIPopup(s_Texts.emitFrom, emitFrom, s_Texts.coneTypes, m_Type);
      ...
      case ParticleSystemShapeType.SkinnedMeshRenderer: {
        ParticleSystemMeshShapeType placementMode =
            (ParticleSystemMeshShapeType)
                GUIPopup(s_Texts.meshType, m_PlacementMode, s_Texts.meshTypes);

        if (!m_PlacementMode.hasMultipleDifferentValues &&
            placementMode != ParticleSystemMeshShapeType.Triangle) {
          bool showSpeedAndSpread =
              (placementMode != ParticleSystemMeshShapeType.Vertex);
          m_MeshSpawn.OnInspectorGUI(s_MeshTexts, s_Texts.emissionModesMesh,
                                     showSpeedAndSpread, showSpeedAndSpread);
        }
        ...
        if (guiType == ParticleSystemShapeType.Mesh) {
          ...
          MeshRenderer mesh =
              (MeshRenderer) m_MeshRenderer.objectReferenceValue;
          if (mesh) {
          ...
          SkinnedMeshRenderer mesh =
              (SkinnedMeshRenderer) m_SkinnedMeshRenderer.objectReferenceValue;
          if (mesh) {
            material = mesh.sharedMaterial;
        ...
        GUIToggleWithIntField(s_Texts.meshMaterialIndex, m_UseMeshMaterialIndex,
                              m_MeshMaterialIndex, false);
        ...
        if (useMeshColors) {
          if (material != null && srcMesh != null) {
            ...
            if (!material.HasProperty(colorName) &&
                !material.HasProperty(tintColorName) &&
                !srcMesh.HasVertexAttribute(VertexAttribute.Color)) {
              GUIContent warning = EditorGUIUtility.TrTextContent(
                  "To use mesh colors, your source mesh must either provide vertex colors, or its shader must contain a color property named \"_Color\" or \"_TintColor\".");
    ...
    GUIButtonGroup(s_SceneViewEditModes, s_Texts.toolContents,
                   m_ParticleSystemUI.GetBounds,
                   m_ParticleSystemUI.m_ParticleEffectUI.m_Owner.customEditor);
  }
  ...
  private void OnTextureInspectorGUI() {
    if (m_Texture.hasMultipleDifferentValues ||
      ...
      GUIPopup(s_Texts.textureClipChannel, m_TextureClipChannel,
               s_Texts.textureClipChannels);
      GUIFloat(s_Texts.textureClipThreshold, m_TextureClipThreshold);
      GUIToggle(s_Texts.textureColorAffectsParticles,
                m_TextureColorAffectsParticles);
      GUIToggle(s_Texts.textureAlphaAffectsParticles,
                m_TextureAlphaAffectsParticles);
      GUIToggle(s_Texts.textureBilinearFiltering, m_TextureBilinearFiltering);

      if (!m_Type.hasMultipleDifferentValues) {
        if ((m_Type.intValue == (int) ParticleSystemShapeType.Mesh) ||
            (m_Type.intValue == (int) ParticleSystemShapeType.MeshRenderer) ||
            (m_Type.intValue ==
             (int) ParticleSystemShapeType.SkinnedMeshRenderer))
  ...
  override public void OnSceneViewGUI() {
    ...
    foreach (ParticleSystem ps in m_ParticleSystemUI.m_ParticleSystems) {
      ...
      if (mainModule.scalingMode == ParticleSystemScalingMode.Local)
        transformMatrix.SetTRS(ps.transform.position, ps.transform.rotation,
                               ps.transform.localScale);
      else
        transformMatrix = ps.transform.localToWorldMatrix;

      bool isBox = (type == ParticleSystemShapeType.Box ||
                    type == ParticleSystemShapeType.BoxShell ||
                    type == ParticleSystemShapeType.BoxEdge ||
                    type == ParticleSystemShapeType.Rectangle);

      Vector3 emitterScale = isBox ? Vector3.one : shapeModule.scale;
      Matrix4x4 emitterMatrix =
          Matrix4x4.TRS(shapeModule.position,
                        Quaternion.Euler(shapeModule.rotation), emitterScale);
      Matrix4x4 combinedMatrix = transformMatrix * emitterMatrix;
      Handles.matrix = combinedMatrix;

      Color gizmoEditingColor =
          new Color(1.0f, 1.0f, 1.0f,
                    allowGizmoEditing && !Event.current.alt ? 1.0f : 0.0f);

      if (type == ParticleSystemShapeType.Sphere) {
        // Thickness
        Handles.color *= s_ShapeGizmoThicknessTint;
        EditorGUI.BeginChangeCheck();
        float radiusThickness = Handles.DoSimpleRadiusHandle(
            Quaternion.identity, Vector3.zero,
            shapeModule.radius * (1.0f - shapeModule.radiusThickness), false,
            shapeModule.arc, allowGizmoEditing);
        if (EditorGUI.EndChangeCheck()) {
          Undo.RecordObject(ps, s_Texts.undoSphereThickness);
          shapeModule.radiusThickness =
              1.0f - (radiusThickness / shapeModule.radius);
        }

        // Sphere
        Handles.color = s_GizmoColor;
        EditorGUI.BeginChangeCheck();
        float radius = Handles.DoSimpleRadiusHandle(
            Quaternion.identity, Vector3.zero, shapeModule.radius, false,
            shapeModule.arc, allowGizmoEditing);
        if (EditorGUI.EndChangeCheck()) {
        ...
        Matrix4x4 textureTransform =
            ...
            Matrix4x4.TRS(Vector3.zero, Quaternion.identity,
                          Vector3.one * shapeModule.radius * 2.0f);
        OnSceneViewTextureGUI(shapeModule, s_SphereMesh, false,
                              s_SphereTextureMaterial, textureTransform);
        ...
        m_ArcHandle.radius =
            shapeModule.radius * (1.0f - shapeModule.radiusThickness);
        ...
        using(new Handles.DrawingScope(
            Handles.matrix * s_ArcHandleOffsetMatrix)) m_ArcHandle.DrawHandle();

        if (EditorGUI.EndChangeCheck()) {
          Undo.RecordObject(ps, s_Texts.undoCircleThickness);
          shapeModule.radiusThickness =
              1.0f - (m_ArcHandle.radius / shapeModule.radius);
        }
        ...
        Matrix4x4 textureTransform =
            combinedMatrix *
            Matrix4x4.TRS(Vector3.zero, Quaternion.Euler(90.0f, 0.0f, 180.0f),
                          Vector3.one * shapeModule.radius * 2.0f);
        OnSceneViewTextureGUI(shapeModule, s_CircleMesh, true,
                              s_TextureMaterial, textureTransform);
        ...
        float radiusThickness = Handles.DoSimpleRadiusHandle(
            Quaternion.identity, Vector3.zero,
            shapeModule.radius * (1.0f - shapeModule.radiusThickness), true,
            shapeModule.arc, allowGizmoEditing);
        if (EditorGUI.EndChangeCheck()) {
        ...
        EditorGUI.BeginChangeCheck();
        float radius = Handles.DoSimpleRadiusHandle(
            Quaternion.identity, Vector3.zero, shapeModule.radius, true,
            shapeModule.arc, allowGizmoEditing);
        if (EditorGUI.EndChangeCheck()) {
        ...
        Matrix4x4 textureTransform =
            ...
            Matrix4x4.TRS(Vector3.zero, Quaternion.identity,
                          Vector3.one * shapeModule.radius * 2.0f);
        OnSceneViewTextureGUI(shapeModule, s_HemisphereMesh, false,
                              s_SphereTextureMaterial, textureTransform);
      } else if (type == ParticleSystemShapeType.Cone) {
        Handles.ConeHandles coneHandlesMask =
            allowGizmoEditing ? Handles.ConeHandles.All : 0;

        // Thickness
        Handles.color *= s_ShapeGizmoThicknessTint;
        EditorGUI.BeginChangeCheck();
        float angleThickness =
            Mathf.Lerp(shapeModule.angle, 0.0f, shapeModule.radiusThickness);
        Vector3 radiusThicknessAngleRange = new Vector3(
            shapeModule.radius * (1.0f - shapeModule.radiusThickness),
            angleThickness, mainModule.startSpeedMultiplier);
        radiusThicknessAngleRange = Handles.ConeFrustrumHandle(
            Quaternion.identity, Vector3.zero, radiusThicknessAngleRange,
            Handles.ConeHandles.Radius & coneHandlesMask);
        if (EditorGUI.EndChangeCheck()) {
          Undo.RecordObject(ps, s_Texts.undoConeThickness);
          shapeModule.radiusThickness =
              1.0f - (radiusThicknessAngleRange.x / shapeModule.radius);
        }
        ...
        Vector3 radiusAngleRange =
            new Vector3(shapeModule.radius, shapeModule.angle,
                        mainModule.startSpeedMultiplier);
        radiusAngleRange =
            Handles.ConeFrustrumHandle(Quaternion.identity, Vector3.zero,
                                       radiusAngleRange, coneHandlesMask);
        if (EditorGUI.EndChangeCheck()) {
        ...
        float angleThickness =
            Mathf.Lerp(shapeModule.angle, 0.0f, shapeModule.radiusThickness);
        Vector3 radiusThicknessAngleLength = new Vector3(
            shapeModule.radius * (1.0f - shapeModule.radiusThickness),
            angleThickness, shapeModule.length);
        radiusThicknessAngleLength = Handles.ConeFrustrumHandle(
            Quaternion.identity, Vector3.zero, radiusThicknessAngleLength,
            Handles.ConeHandles.Radius & coneHandlesMask);
        if (EditorGUI.EndChangeCheck()) {
          Undo.RecordObject(ps, s_Texts.undoConeVolumeThickness);
          shapeModule.radiusThickness =
              1.0f - (radiusThicknessAngleLength.x / shapeModule.radius);
        }

        // Cone
        Handles.color = s_GizmoColor;
        EditorGUI.BeginChangeCheck();
        Vector3 radiusAngleLength = new Vector3(
            shapeModule.radius, shapeModule.angle, shapeModule.length);
        radiusAngleLength =
            Handles.ConeFrustrumHandle(Quaternion.identity, Vector3.zero,
                                       radiusAngleLength, coneHandlesMask);
        if (EditorGUI.EndChangeCheck()) {
        ...
        OnSceneViewTextureGUI(shapeModule, s_CircleMesh, true,
                              s_TextureMaterial, textureTransform);
      } else if (type == ParticleSystemShapeType.Box ||
                 type == ParticleSystemShapeType.BoxShell ||
                 type == ParticleSystemShapeType.BoxEdge) {
        EditorGUI.BeginChangeCheck();
        ...
        Matrix4x4 textureTransform =
            combinedMatrix *
            Matrix4x4.TRS(
                new Vector3(0.0f, 0.0f, -m_BoxBoundsHandle.size.z * 0.5f),
                Quaternion.identity, m_BoxBoundsHandle.size);
        OnSceneViewTextureGUI(shapeModule, s_QuadMesh, true, s_TextureMaterial,
                              textureTransform);
        ...
        m_ArcHandle.radius = shapeModule.radius;
        m_ArcHandle.angle = shapeModule.arc;
        m_ArcHandle.SetColorWithRadiusHandle(Color.white * gizmoEditingColor,
                                             0f);
        ...
        using(new Handles.DrawingScope(Handles.matrix *
                                       s_ArcHandleOffsetMatrix)) {
          float excessAngle = shapeModule.arc % 360f;
          float angle = Mathf.Abs(shapeModule.arc) >= 360f ? 360f : excessAngle;

          Handles.DrawWireArc(new Vector3(0.0f, shapeModule.donutRadius, 0.0f),
                              Vector3.up, Vector3.forward, angle,
                              shapeModule.radius);
          Handles.DrawWireArc(new Vector3(0.0f, -shapeModule.donutRadius, 0.0f),
                              Vector3.up, Vector3.forward, angle,
                              shapeModule.radius);
          Handles.DrawWireArc(Vector3.zero, Vector3.up, Vector3.forward, angle,
                              shapeModule.radius + shapeModule.donutRadius);
          Handles.DrawWireArc(Vector3.zero, Vector3.up, Vector3.forward, angle,
                              shapeModule.radius - shapeModule.donutRadius);

          if (shapeModule.arc != 360.0f) {
            Quaternion arcRotation =
                Quaternion.AngleAxis(shapeModule.arc, Vector3.up);
            Vector3 capCenter =
                arcRotation * Vector3.forward * shapeModule.radius;
            Handles.DrawWireDisc(capCenter, arcRotation * Vector3.right,
                                 shapeModule.donutRadius);
          }
        }

        // Donut thickness
        m_SphereBoundsHandle.axes =
            PrimitiveBoundsHandle.Axes.X | PrimitiveBoundsHandle.Axes.Y;
        m_SphereBoundsHandle.radius =
            shapeModule.donutRadius * (1.0f - shapeModule.radiusThickness);
        ...
        int numOuterRadii =
            Mathf.Max(1, (int) Mathf.Ceil(shapeModule.arc / handleInterval));
        Matrix4x4 donutRadiusStartMatrix =
            Matrix4x4.TRS(new Vector3(shapeModule.radius, 0.0f, 0.0f),
        ...
        for (int i = 0; i < numOuterRadii; i++) {
          ...
          using(new Handles.DrawingScope(Handles.matrix *
                                         (Matrix4x4.Rotate(Quaternion.Euler(
                                              0.0f, 0.0f, handleInterval * i)) *
                                          donutRadiusStartMatrix)))
              m_SphereBoundsHandle.DrawHandle();
          if (EditorGUI.EndChangeCheck()) {
            Undo.RecordObject(ps, s_Texts.undoDonutRadiusThickness);
            shapeModule.radiusThickness =
                1.0f - (m_SphereBoundsHandle.radius / shapeModule.donutRadius);
          }
        }
        ...
        Matrix4x4 textureTransform =
            combinedMatrix *
            Matrix4x4.TRS(new Vector3(shapeModule.radius, 0.0f, 0.0f),
                          Quaternion.Euler(180.0f, 0.0f, 180.0f),
                          Vector3.one * shapeModule.donutRadius * 2.0f);
        OnSceneViewTextureGUI(shapeModule, s_CircleMesh, true,
                              s_TextureMaterial, textureTransform);
      } else if (type == ParticleSystemShapeType.SingleSidedEdge) {
        EditorGUI.BeginChangeCheck();
        float radius =
            Handles.DoSimpleEdgeHandle(Quaternion.identity, Vector3.zero,
                                       shapeModule.radius, allowGizmoEditing);
        if (EditorGUI.EndChangeCheck()) {
        ...
        if (mesh) {
          ...
          OnSceneViewTextureGUI(shapeModule, mesh, false, s_TextureMaterial,
                                combinedMatrix);
        }
        ...
        m_BoxBoundsHandle.center = Vector3.zero;
        m_BoxBoundsHandle.size =
            new Vector3(shapeModule.scale.x, shapeModule.scale.y, 0.0f);
        m_BoxBoundsHandle.handleColor = gizmoEditingColor;
        m_BoxBoundsHandle.DrawHandle();

        if (EditorGUI.EndChangeCheck()) {
          Undo.RecordObject(ps, s_Texts.undoRectangle);
          shapeModule.scale = new Vector3(m_BoxBoundsHandle.size.x,
                                          m_BoxBoundsHandle.size.y, 0.0f);
        }

        OnSceneViewTextureGUI(shapeModule, s_QuadMesh, true, s_TextureMaterial,
                              combinedMatrix *
                                  Matrix4x4.Scale(m_BoxBoundsHandle.size));
        ...
        if (sprite) {
          ...
          m_SpriteMesh.vertices =
              Array.ConvertAll(sprite.vertices, i =>(Vector3) i);
          m_SpriteMesh.uv = sprite.uv;
          m_SpriteMesh.triangles =
              Array.ConvertAll(sprite.triangles, i =>(int) i);
          ...
          OnSceneViewTextureGUI(shapeModule, m_SpriteMesh, false,
                                s_TextureMaterial, combinedMatrix);
        }
      }
      ...
      if (EditMode.editMode ==
        ...
        Quaternion rotation =
            transformMatrix.rotation * Quaternion.Euler(shapeModule.rotation);
        ...
        if (EditorGUI.EndChangeCheck()) {
          ...
          shapeModule.rotation =
              (inverseTransformMatrix.rotation * rotation).eulerAngles;
  ...
  private void OnSceneViewTextureGUI(ParticleSystem.ShapeModule shapeModule,
                                     Mesh mesh, bool twoSided, Material mat,
                                     Matrix4x4 transform) {
    ...
    if (texture) {
      ...
      mat.SetFloat("_Cull", twoSided ?(float) UnityEngine.Rendering.CullMode.Off
  ...
  override public void UpdateCullingSupportedString(ref string text) {
    ...
    foreach (ParticleSystem ps in m_ParticleSystemUI.m_ParticleSystems) {
      ...
      case ParticleSystemShapeType.Circle:
        animatedShape =
            (ps.shape.arcMode != ParticleSystemShapeMultiModeValue.Random);
        break;
      case ParticleSystemShapeType.SingleSidedEdge:
        animatedShape =
            (ps.shape.radiusMode != ParticleSystemShapeMultiModeValue.Random);
        break;
      default:
        break;
      }
