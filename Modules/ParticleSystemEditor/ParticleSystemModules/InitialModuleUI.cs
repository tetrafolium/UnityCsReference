...
class Texts {
public GUIContent duration = EditorGUIUtility.TrTextContent(
	"Duration",
...
public GUIContent looping = EditorGUIUtility.TrTextContent(
	"Looping",
	"If true, the emission cycle will repeat after the duration.");
public GUIContent prewarm = EditorGUIUtility.TrTextContent(
	"Prewarm",
...
public GUIContent startDelay = EditorGUIUtility.TrTextContent(
	"Start Delay",
...
public GUIContent maxParticles = EditorGUIUtility.TrTextContent(
	"Max Particles",
...
public GUIContent lifetime = EditorGUIUtility.TrTextContent(
	"Start Lifetime",
	"Start lifetime in seconds, particle will die when its lifetime reaches 0.");
public GUIContent speed = EditorGUIUtility.TrTextContent(
	"Start Speed",
	"The start speed of particles, applied in the starting direction.");
public GUIContent color = EditorGUIUtility.TrTextContent(
	"Start Color", "The start color of particles.");
public GUIContent size3D = EditorGUIUtility.TrTextContent(
	"3D Start Size",
	"If enabled, you can control the size separately for each axis.");
public GUIContent size = EditorGUIUtility.TrTextContent(
	"Start Size", "The start size of particles.");
public GUIContent rotation3D = EditorGUIUtility.TrTextContent(
	"3D Start Rotation",
	"If enabled, you can control the rotation separately for each axis.");
public GUIContent rotation = EditorGUIUtility.TrTextContent(
	"Start Rotation", "The start rotation of particles in degrees.");
public GUIContent randomizeRotationDirection = EditorGUIUtility.TrTextContent(
	"Flip Rotation",
	"Cause some particles to spin in the opposite direction. (Set between 0 and 1, where a higher value causes more to flip)");
public GUIContent autoplay = EditorGUIUtility.TrTextContent(
	"Play On Awake*",
	"If enabled, the system will start playing automatically. Note that this setting is shared between all Particle Systems in the current particle effect.");
public GUIContent gravity = EditorGUIUtility.TrTextContent(
	"Gravity Modifier", "Scales the gravity defined in Physics Manager");
public GUIContent scalingMode = EditorGUIUtility.TrTextContent(
	"Scaling Mode",
	"Use the combined scale from our entire hierarchy, just this local particle node, or only apply scale to the shape module.");
public GUIContent simulationSpace = EditorGUIUtility.TrTextContent(
	"Simulation Space",
	"Makes particle positions simulate in world, local or custom space. In local space they stay relative to their own Transform, and in custom space they are relative to the custom Transform.");
public GUIContent customSimulationSpace = EditorGUIUtility.TrTextContent(
	"Custom Simulation Space",
	"Makes particle positions simulate relative to a custom Transform component.");
public GUIContent simulationSpeed = EditorGUIUtility.TrTextContent(
	"Simulation Speed", "Scale the playback speed of the Particle System.");
public GUIContent deltaTime = EditorGUIUtility.TrTextContent(
	"Delta Time",
	"Use either the Delta Time or the Unscaled Delta Time. Useful for playing effects whilst paused.");
public GUIContent autoRandomSeed = EditorGUIUtility.TrTextContent(
	"Auto Random Seed",
	"Simulate differently each time the effect is played.");
public GUIContent randomSeed = EditorGUIUtility.TrTextContent(
	"Random Seed",
	"Randomize the look of the Particle System. Using the same seed will make the Particle System play identically each time. After changing this value, restart the Particle System to see the changes, or check the Resimulate box.");
public GUIContent emitterVelocity = EditorGUIUtility.TrTextContent(
	"Emitter Velocity",
	"When the Particle System is moving, should we use its Transform, or Rigidbody Component, to calculate its velocity?");
public GUIContent stopAction = EditorGUIUtility.TrTextContent(
	"Stop Action",
	"When the Particle System is stopped and all particles have died, should the GameObject automatically disable/destroy itself?");
public GUIContent cullingMode = EditorGUIUtility.TrTextContent(
	"Culling Mode",
	"Choose whether to continue simulating the Particle System when offscreen. Catch-up mode pauses offscreen simulations but performs a large simulation step when they become visible, giving the appearance that they were never paused. Automatic uses Pause mode for looping systems, and AlwaysSimulate if not looping.");
public GUIContent ringBufferMode = EditorGUIUtility.TrTextContent(
	"Ring Buffer Mode",
	"Rather than dying when their lifetime has elapsed, particles will remain alive until the Max Particles buffer is full, at which point new particles will replace the oldest.");
public GUIContent ringBufferLoopRange = EditorGUIUtility.TrTextContent(
	"Loop Range",
	"Particle lifetimes may loop between a fade-in and fade-out time, in order to use curves for the entire time they are alive. Values are in the 0-1 range.");
public GUIContent x = EditorGUIUtility.TextContent("X");
public GUIContent y = EditorGUIUtility.TextContent("Y");
public GUIContent z = EditorGUIUtility.TextContent("Z");

public GUIContent[] simulationSpaces =
...
public GUIContent[] scalingModes =
	new GUIContent[] {EditorGUIUtility.TrTextContent("Hierarchy"),
	                  EditorGUIUtility.TrTextContent("Local"),
	                  EditorGUIUtility.TrTextContent("Shape")};

public GUIContent[] stopActions =
...
public GUIContent[] cullingModes =
	new GUIContent[] {EditorGUIUtility.TrTextContent("Automatic"),
...
public GUIContent[] ringBufferModes =
	new GUIContent[] {EditorGUIUtility.TrTextContent("Disabled"),
	                  EditorGUIUtility.TrTextContent("Pause Until Replaced"),
	                  EditorGUIUtility.TrTextContent("Loop Until Replaced")};
}
private static Texts s_Texts;

public InitialModuleUI(ParticleSystemUI owner, SerializedObject o,
                       string displayName)
	: base(owner, o, "InitialModule", displayName,
	       VisibilityState.VisibleAndFoldedOut) {
	Init(); // Should always be initialized since it is used by other modules
	        // (see ShapeModule)
}

public override bool DrawHeader(Rect rect, GUIContent label) {
	// When displaying prefab overrides, the whole UI is disabled, but we still
	// need to be able to expand the modules to see the settings - this doesn't
	// modify the asset
	bool wasEnabled = GUI.enabled;
	GUI.enabled = true;

	var toggleState = GUI.Toggle(rect, foldout, label,
	                             ParticleSystemStyles.Get().emitterHeaderStyle);
...
protected override void Init() {
	...
	m_StartDelay = new SerializedMinMaxCurve(this, s_Texts.startDelay,
	                                         "startDelay", false, true);
	...
	// module properties
	m_LifeTime =
		new SerializedMinMaxCurve(this, s_Texts.lifetime, "startLifetime");
	m_Speed = new SerializedMinMaxCurve(this, s_Texts.speed, "startSpeed",
	                                    kUseSignedRange);
	...
	m_SizeY = new SerializedMinMaxCurve(this, s_Texts.y, "startSizeY", false,
	                                    false, m_Size3D.boolValue);
	m_SizeZ = new SerializedMinMaxCurve(this, s_Texts.z, "startSizeZ", false,
	                                    false, m_Size3D.boolValue);
	m_Rotation3D = GetProperty("rotation3D");
	m_RotationX = new SerializedMinMaxCurve(this, s_Texts.x, "startRotationX",
	                                        kUseSignedRange, false,
	                                        m_Rotation3D.boolValue);
	m_RotationY = new SerializedMinMaxCurve(this, s_Texts.y, "startRotationY",
	                                        kUseSignedRange, false,
	                                        m_Rotation3D.boolValue);
	m_RotationZ = new SerializedMinMaxCurve(this, s_Texts.z, "startRotation",
	                                        kUseSignedRange);
	...
	m_GravityModifier = new SerializedMinMaxCurve(
		this, s_Texts.gravity, "gravityModifier", kUseSignedRange);
...
override public void OnInspectorGUI(InitialModuleUI initial) {
	...
	using (new EditorGUI.DisabledScope(m_Prewarm.boolValue &&
	                                   m_Looping.boolValue)) {
	...
	if (size3D) {
		...
		GUITripleMinMaxCurve(GUIContent.none, s_Texts.x, m_SizeX, s_Texts.y,
		                     m_SizeY, s_Texts.z, m_SizeZ, null);
	} else {
	...
	if (rotation3D) {
		...
		GUITripleMinMaxCurve(GUIContent.none, s_Texts.x, m_RotationX, s_Texts.y,
		                     m_RotationY, s_Texts.z, m_RotationZ, null);
	} else {
	...
	int space = GUIPopup(s_Texts.simulationSpace, m_SimulationSpace,
	                     s_Texts.simulationSpaces);
	...
	GUIBoolAsPopup(s_Texts.deltaTime, m_UseUnscaledTime,
	               new string[] {"Scaled", "Unscaled"});
	...
	bool anyNonMesh =
		m_ParticleSystemUI.m_ParticleSystems.FirstOrDefault(
			...
			(o.shape.shapeType !=
			 ...
			 o.shape.shapeType != ParticleSystemShapeType.MeshRenderer)) !=
		null;
	if (anyNonMesh)
	...
	GUIBoolAsPopup(s_Texts.emitterVelocity, m_EmitterVelocity,
	               new string[] {"Transform", "Rigidbody"});

	GUIInt(s_Texts.maxParticles, m_MaxNumParticles);

	bool autoRandomSeed = GUIToggle(s_Texts.autoRandomSeed, m_AutoRandomSeed);
	if (!autoRandomSeed) {
		bool isInspectorView = m_ParticleSystemUI.m_ParticleEffectUI.m_Owner is
		                       ParticleSystemInspector;
		if (isInspectorView) {
			GUILayout.BeginHorizontal();
			GUIInt(s_Texts.randomSeed, m_RandomSeed);
			if (!m_ParticleSystemUI.multiEdit &&
			    GUILayout.Button("Reseed", EditorStyles.miniButton,
			                     GUILayout.Width(60)))
				m_RandomSeed.intValue = (int) m_ParticleSystemUI
				                        .m_ParticleSystems [0]
				                        .GenerateRandomSeed();
	...
	ParticleSystemRingBufferMode ringBufferMode = (ParticleSystemRingBufferMode)
	                                              GUIPopup(s_Texts.ringBufferMode, m_RingBufferMode,
	                                                       s_Texts.ringBufferModes);
	if (!m_RingBufferMode.hasMultipleDifferentValues &&
	    ringBufferMode == ParticleSystemRingBufferMode.LoopUntilReplaced) {
