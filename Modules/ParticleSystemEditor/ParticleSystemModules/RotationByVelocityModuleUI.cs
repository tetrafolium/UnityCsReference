...
public GUIContent velocityRange = EditorGUIUtility.TrTextContent(
	"Speed Range",
	"Maps the speed to a value along the curve, when using one of the curve modes.");
public GUIContent rotation = EditorGUIUtility.TrTextContent(
	"Angular Velocity",
	"Controls the angular velocity of each particle based on its speed.");
...
public RotationByVelocityModuleUI(ParticleSystemUI owner, SerializedObject o,
                                  string displayName)
...
protected override void Init() {
	...
	m_X = new SerializedMinMaxCurve(this, s_Texts.x, "x", kUseSignedRange,
	                                false, m_SeparateAxes.boolValue);
