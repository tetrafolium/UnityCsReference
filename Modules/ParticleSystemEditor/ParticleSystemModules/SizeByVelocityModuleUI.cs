...
class SizeByVelocityModuleUI : ModuleUI {
  class Texts {
    public GUIContent velocityRange = EditorGUIUtility.TrTextContent(
        "Speed Range", "Remaps speed in the defined range to a size.");
    public GUIContent size = EditorGUIUtility.TrTextContent(
        "Size", "Controls the size of each particle based on its speed.");
    public GUIContent separateAxes = EditorGUIUtility.TrTextContent(
        "Separate Axes",
        "If enabled, you can control the angular velocity limit separately for each axis.");
  ...
  public SizeByVelocityModuleUI(ParticleSystemUI owner, SerializedObject o,
                                string displayName)
  ...
  protected override void Init() {
    ...
    m_X = new SerializedMinMaxCurve(
        this, s_Texts.x,
        "curve"); // use "curve" instead of "x" for backwards compatibility
                  // reasons: the old system used to only support one axis, and
                  // this was its name
    m_X.m_AllowConstant = false;
    m_Y = new SerializedMinMaxCurve(this, s_Texts.y, "y", false, false,
                                    m_SeparateAxes.boolValue);
    m_Y.m_AllowConstant = false;
    m_Z = new SerializedMinMaxCurve(this, s_Texts.z, "z", false, false,
                                    m_SeparateAxes.boolValue);
  ...
  override public void OnInspectorGUI(InitialModuleUI initial) {
    ...
    using(
        new EditorGUI.DisabledScope((state == MinMaxCurveState.k_Scalar) ||
