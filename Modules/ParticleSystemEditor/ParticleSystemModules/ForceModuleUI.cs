...
class ForceModuleUI : ModuleUI {
  ...
  class Texts {
    ...
    public GUIContent randomizePerFrame = EditorGUIUtility.TrTextContent(
        "Randomize",
        "Randomize force every frame. Only available when using random between two constants or random between two curves.");
  ...
  public ForceModuleUI(ParticleSystemUI owner, SerializedObject o,
                       string displayName)
  ...
  override public void OnInspectorGUI(InitialModuleUI initial) {
    ...
    GUITripleMinMaxCurve(GUIContent.none, s_Texts.x, m_X, s_Texts.y, m_Y,
                         s_Texts.z, m_Z, m_RandomizePerFrame);

    GUIBoolAsPopup(s_Texts.space, m_InWorldSpace, s_Texts.spaces);

    using(
        new EditorGUI.DisabledScope((state != MinMaxCurveState.k_TwoScalars) &&
