...
internal class ParticleEffectUI {
  public ParticleEffectUIOwner
      m_Owner; // Can be InspectorWindow or ParticleSystemWindow
  public ParticleSystemUI[] m_Emitters; // Contains UI for all ParticleSystem
                                        // children of the root ParticleSystem
                                        // for this effect
  bool m_EmittersActiveInHierarchy;
  ParticleSystemCurveEditor
      m_ParticleSystemCurveEditor; // The curve editor used by ParticleSystem
                                   // modules
  List<ParticleSystem>
      m_SelectedParticleSystems; // This is the array of selected particle
                                 // systems and used to find the root
                                 // ParticleSystem and for the inspector
  ...
  float m_EmitterAreaWidth =
      230; // Only used in ParticleSystemWindow for horizontal layout
  float m_CurveEditorAreaHeight =
      330; // Only used in ParticleSystemWindow for vertical layout
  Vector2 m_EmitterAreaScrollPos = Vector2.zero;
  static readonly Color k_DarkSkinDisabledColor =
      new Color(0.66f, 0.66f, 0.66f, 0.95f);
  static readonly Color k_LightSkinDisabledColor =
      new Color(0.84f, 0.84f, 0.84f, 0.95f);

  private enum OwnerType { Inspector, ParticleSystemWindow }

  internal class Texts {
    public GUIContent previewSpeed = EditorGUIUtility.TrTextContent(
        "Playback Speed",
        "Playback Speed is also affected by the Time Scale setting in the Time Manager.");
    public GUIContent previewSpeedDisabled = EditorGUIUtility.TrTextContent(
        "Playback Speed",
        "Playback Speed is locked to 0.0, because the Time Scale in the Time Manager is set to 0.0.");
    public GUIContent previewTime =
        EditorGUIUtility.TrTextContent("Playback Time");
    public GUIContent particleCount =
        EditorGUIUtility.TrTextContent("Particles");
    public GUIContent subEmitterParticleCount =
        EditorGUIUtility.TrTextContent("Sub Emitter Particles");
    public GUIContent particleSpeeds =
        EditorGUIUtility.TrTextContent("Speed Range");
    public GUIContent play = EditorGUIUtility.TrTextContent("Play");
    public GUIContent playDisabled = EditorGUIUtility.TrTextContent(
        "Play",
        "Play is disabled, because the Time Scale in the Time Manager is set to 0.0.");
    ...
    public GUIContent addParticleSystem =
        EditorGUIUtility.TrTextContent("", "Create Particle System");
    public GUIContent showBounds = EditorGUIUtility.TrTextContent(
        "Show Bounds", "Show world space bounding boxes.");
    public GUIContent showOnlySelected = EditorGUIUtility.TrTextContent(
        "Show Only Selected",
        "Hide all unselected Particle Systems in the current Effect.");
    public GUIContent resimulation = EditorGUIUtility.TrTextContent(
        "Resimulate",
        "If resimulate is enabled, the Particle System will show changes made to the system immediately (including changes made to the Particle System Transform).");
    public GUIContent previewLayers = EditorGUIUtility.TrTextContent(
        "Simulate Layers",
        "Automatically preview all looping Particle Systems on the chosen layers, in addition to the selected Game Objects.");
  ...
  static Event CreateCommandEvent(string commandName) {
    return new Event{type = EventType.ExecuteCommand,
                     commandName = "ParticleSystem/" + commandName};
  }
  ...
  [Shortcut("ParticleSystem/Play",
            typeof(ParticleSystemInspector.ShortcutContext), KeyCode.Comma)]
  ...
  [Shortcut("ParticleSystem/Stop",
            typeof(ParticleSystemInspector.ShortcutContext), KeyCode.Period)]
  static void StopShortcut(ShortcutArguments args) {
    DispatchShortcutEvent(s_StopEvent);
  }

  [Shortcut("ParticleSystem/Restart",
            typeof(ParticleSystemInspector.ShortcutContext), KeyCode.Slash)]
  ...
  [ClutchShortcut("ParticleSystem/Forward",
                  typeof(ParticleSystemInspector.ShortcutContext), KeyCode.M)]
  static void ForwardShortcut(ShortcutArguments args) {
    DispatchShortcutEvent(args.stage == ShortcutStage.Begin ? s_ForwardBeginEvent
                          : s_ForwardEndEvent);
  }

  [FormerlyPrefKeyAs("ParticleSystem/Reverse", "n")]
  [ClutchShortcut("ParticleSystem/Reverse",
                  typeof(ParticleSystemInspector.ShortcutContext), KeyCode.N)]
  static void ReverseShortcut(ShortcutArguments args) {
    DispatchShortcutEvent(args.stage == ShortcutStage.Begin ? s_ReverseBeginEvent
                          : s_ReverseEndEvent);
  }

  public ParticleEffectUI(ParticleEffectUIOwner owner) {
    m_Owner = owner;
    System.Diagnostics.Debug.Assert(m_Owner is ParticleSystemInspector ||
                                    m_Owner is ParticleSystemWindow);
  }

  public bool multiEdit {
    get {
      return (m_SelectedParticleSystems != null) &&
             (m_SelectedParticleSystems.Count > 1);
    }
  }
  ...
  static Color GetDisabledColor() {
    return (!EditorGUIUtility.isProSkin) ? k_LightSkinDisabledColor
                                         : k_DarkSkinDisabledColor;
  }
  ...
  static private void GetDirectParticleSystemChildrenRecursive(
      Transform transform, List<ParticleSystem> particleSystems) {
    foreach (Transform childTransform in transform) {
      ParticleSystem ps =
          childTransform.gameObject.GetComponent<ParticleSystem>();
      if (ps != null) {
        // Note: we do not check for if the gameobject is active (we want
        // inactive particle systems as well due prefabs)
        particleSystems.Add(ps);
        GetDirectParticleSystemChildrenRecursive(childTransform,
                                                 particleSystems);
      }
    }
  }

  // Should be called often to ensure we catch if selected Particle System is
  // dragged in/out of root hierarchy
  public bool InitializeIfNeeded(IEnumerable<ParticleSystem> systems) {
    ...
    foreach (ParticleSystem shuriken in allSystems) {
      ...
      if (!usingMultiEdit) {
        ...
        if (m_SelectedParticleSystems != null &&
            m_SelectedParticleSystems.Count > 0) {
          if (root ==
              ParticleSystemEditorUtils.GetRoot(m_SelectedParticleSystems[0])) {
            if (m_ParticleSystemCurveEditor != null && m_Emitters != null &&
                shurikens.Length == m_Emitters.Length &&
                shuriken.gameObject.activeInHierarchy ==
              ...
              if (ParticleEffectUI.m_ShowOnlySelected)
                SetShowOnlySelectedMode(
                    ParticleEffectUI.m_ShowOnlySelected); // always refresh
        ...
        // in multi-edit mode, we explicitly choose the systems to edit, so
        // don't automatically add child systems or search for the root
      ...
      if (!usingMultiEdit) {
        // Init CurveEditor before modules (they may add curves during
        // construction)
        ...
        if (numEmitters > 0) {
          ...
          for (int i = 0; i < numEmitters; ++i) {
            ...
            m_Emitters [i]
                .Init(this, new ParticleSystem[]{shurikens[i]});
          }
    ...
    if (initializeRequired) {
      ...
      if (usingMultiEdit) {
        ...
        if (numEmitters > 0) {
          ...
          m_Emitters [0]
              .Init(this, m_SelectedParticleSystems.ToArray());
          m_EmittersActiveInHierarchy =
              m_SelectedParticleSystems[0].gameObject.activeInHierarchy;
        }
      }

      // Allow modules to validate their state (the user can have moved emitters
      // around in the hierarchy)
      ...
      m_EmitterAreaWidth = EditorPrefs.GetFloat(
          "ParticleSystemEmitterAreaWidth", k_MinEmitterAreaSize.x);
      m_CurveEditorAreaHeight = EditorPrefs.GetFloat(
          "ParticleSystemCurveEditorAreaHeight", k_MinCurveAreaSize.y);

      SetShowOnlySelectedMode(ParticleEffectUI.m_ShowOnlySelected);

      m_EmitterAreaScrollPos.x =
          SessionState.GetFloat("CurrentEmitterAreaScroll", 0.0f);

      if (ShouldManagePlaybackState(mainSystem)) {
        // Restore lastPlayBackTime if available in session cache
        Vector3 simulationState = SessionState.GetVector3(
            k_SimulationStateId + mainSystem.GetInstanceID(), Vector3.zero);
  ...
  public void Clear() {
    ParticleSystem root = ParticleSystemEditorUtils.GetRoot(
        m_SelectedParticleSystems[0]); // root can have been deleted
    if (ShouldManagePlaybackState(root)) {
      // Store simulation state of current effect as Vector3 (rootInstanceID,
      ...
      if (root != null) {
        ...
        SessionState.SetVector3(
            k_SimulationStateId + rootInstanceId,
            new Vector3(rootInstanceId, (int) playState,
                        ParticleSystemEditorUtils.playbackTime));
      }
  ...
  public string GetNextParticleSystemName() {
    ...
    for (int i = 2; i < 50; ++i) {
      ...
      bool found = false;
      foreach (ParticleSystemUI e in m_Emitters) {
        if (e.m_ParticleSystems.FirstOrDefault(o => o.name == nextName) !=
            null) {
          found = true;
          break;
        }
  ...
  public bool IsParticleSystemUIVisible(ParticleSystemUI psUI) {
    OwnerType ownerType =
        m_Owner is ParticleSystemInspector ? OwnerType.Inspector
        : OwnerType.ParticleSystemWindow;
  ...
  public GameObject
  CreateParticleSystem(ParticleSystem parentOfNewParticleSystem,
                       SubModuleUI.SubEmitterType defaultType) {
    ...
    if (go) {
      ...
      SessionState.SetFloat("CurrentEmitterAreaScroll",
                            m_EmitterAreaScrollPos.x);
      ...
      if (particleMat == null)
        particleMat = AssetDatabase.GetBuiltinExtraResource<Material>(
  ...
  public void OnSceneViewGUI() {
    if (m_OverlayWindow == null) {
      m_OverlayWindow = new OverlayWindow(
          ParticleSystemInspector.playBackTitle, SceneViewGUICallback,
          (int) SceneViewOverlay.Ordering.ParticleEffect, null,
  ...
  internal void PlayBackInfoGUI(bool isPlayMode) {
    ...
    if (!isPlayMode) {
      ...
      if (Time.timeScale == 0.0f) {
        ...
        ParticleSystemEditorUtils.simulationSpeed =
            Mathf.Clamp(EditorGUILayout.FloatField(
                            s_Texts.previewSpeed,
                            ParticleSystemEditorUtils.simulationSpeed),
                        0f, 10f);
      }
      ...
      EditorGUI.kFloatFieldFormatString = s_Texts.secondsFloatFieldFormatString;
      float playbackTime = EditorGUILayout.FloatField(
          s_Texts.previewTime, ParticleSystemEditorUtils.playbackTime);
      EditorGUI.kFloatFieldFormatString = oldFormat;
      ...
      if (oldEventType == EventType.MouseDown &&
          GUIUtility.hotControl != oldHotControl) {
        m_IsDraggingTimeHotControlID = GUIUtility.hotControl;
        ParticleSystemEditorUtils.playbackIsScrubbing = true;
      }

      // Detect stop dragging
      if (m_IsDraggingTimeHotControlID != -1 &&
          GUIUtility.hotControl != m_IsDraggingTimeHotControlID) {
    ...
    foreach (ParticleSystem ps in m_SelectedParticleSystems) {
      ps.CalculateEffectUIData(ref particleCount, ref fastestParticle,
                               ref slowestParticle);
    }
    EditorGUILayout.LabelField(s_Texts.particleCount,
                               GUIContent.Temp(particleCount.ToString()));

    bool hasSubEmitters = false;
    int subEmitterParticles = 0;
    foreach (ParticleSystem ps in m_SelectedParticleSystems) {
      int subEmitterParticlesCurrent = 0;
      if (ps.CalculateEffectUISubEmitterData(ref subEmitterParticlesCurrent,
                                             ref fastestParticle,
                                             ref slowestParticle)) {
        hasSubEmitters = true;
    ...
    if (hasSubEmitters)
      EditorGUILayout.LabelField(
          s_Texts.subEmitterParticleCount,
          GUIContent.Temp(subEmitterParticles.ToString()));

    if (fastestParticle >= slowestParticle)
      EditorGUILayout.LabelField(
          s_Texts.particleSpeeds,
          GUIContent.Temp(slowestParticle.ToString(
                              s_Texts.speedFloatFieldFormatString,
                              CultureInfo.InvariantCulture.NumberFormat) +
                          " - " +
                          fastestParticle.ToString(
                              s_Texts.speedFloatFieldFormatString,
                              CultureInfo.InvariantCulture.NumberFormat)));
    else
      EditorGUILayout.LabelField(s_Texts.particleSpeeds,
                                 GUIContent.Temp("0.0 - 0.0"));

    if (!EditorApplication.isPlaying) {
      ParticleSystemEditorUtils.previewLayers = EditorGUILayout.LayerMaskField(
          ParticleSystemEditorUtils.previewLayers, s_Texts.previewLayers);
      ParticleSystemEditorUtils.resimulation =
          GUILayout.Toggle(ParticleSystemEditorUtils.resimulation,
                           s_Texts.resimulation, EditorStyles.toggle);
    }

    ParticleEffectUI.m_ShowBounds = GUILayout.Toggle(
        ParticleEffectUI.m_ShowBounds, ParticleEffectUI.texts.showBounds,
        EditorStyles.toggle);

    EditorGUI.BeginChangeCheck();
    ParticleEffectUI.m_ShowOnlySelected = GUILayout.Toggle(
        ParticleEffectUI.m_ShowOnlySelected,
        ParticleEffectUI.texts.showOnlySelected, EditorStyles.toggle);
    if (EditorGUI.EndChangeCheck())
      SetShowOnlySelectedMode(ParticleEffectUI.m_ShowOnlySelected);
  ...
  void HandleScrubbing() {
    if ((!m_ScrubForward && !m_ScrubReverse) ||
        Time.realtimeSinceStartup < m_ScrubNextUpdate)
      return;
    ...
    float previewSpeed = ParticleSystemEditorUtils.simulationSpeed;
    float timeDiff =
        (evt.shift ? 3f : 1f) * m_TimeHelper.deltaTime * (changeTime * 3f);
    ParticleSystemEditorUtils.playbackTime = Mathf.Max(
        0f, ParticleSystemEditorUtils.playbackTime + timeDiff * (previewSpeed));
  ...
  internal static bool IsStopped(ParticleSystem root) {
    return (!ParticleSystemEditorUtils.playbackIsPlaying &&
            !ParticleSystemEditorUtils.playbackIsPaused) &&
           !ParticleSystemEditorUtils.playbackIsScrubbing;
  }

  internal bool IsPaused() {
    return !IsPlaying() && !IsStopped(ParticleSystemEditorUtils.GetRoot(
                               m_SelectedParticleSystems[0]));
  }
  ...
  internal void PlayStopGUI() {
    ...
    GUIContent playText =
        disablePlayButton ? s_Texts.playDisabled : s_Texts.play;
    ...
    if (!EditorApplication.isPlaying) {
      ...
      GUILayout.BeginHorizontal(GUILayout.Width(220.0f));
      {
        using(new EditorGUI.DisabledScope(disablePlayButton)) {
          bool isPlaying = ParticleSystemEditorUtils.playbackIsPlaying &&
                           !ParticleSystemEditorUtils.playbackIsPaused &&
                           !disablePlayButton;
          if (GUILayout.Button(isPlaying ? s_Texts.pause
                               : playText, "ButtonLeft")) {
            if (isPlaying)
              Pause();
            else
      ...
      // Play mode: we only handle play/stop (due to problems with determining
      // if a system with subemitters is playing we cannot pause)
    ...
    // Handle shortcut keys last so we do not activate them if inputfield has
    // used the event
  ...
  private void InspectorParticleSystemGUI() {
    ...
    {
      ParticleSystem selectedSystem = (m_SelectedParticleSystems.Count > 0)
                                          ? m_SelectedParticleSystems[0]
                                          : null;
      if (selectedSystem != null) {
        ParticleSystemUI psUI = m_Emitters.FirstOrDefault(
            o => o.m_ParticleSystems[0] == selectedSystem);
        if (psUI != null) {
          float width = GUIClip.visibleRect.width -
                        18; // -10 is effect_bg padding, -8 is inspector padding
  ...
  private void DrawSelectionMarker(Rect rect) {
    ...
    rect.width -= 2;
    rect.height -= 2;
    ParticleSystemStyles.Get().selectionMarker.Draw(rect, GUIContent.none,
  ...
  private List<ParticleSystemUI> GetSelectedParticleSystemUIs() {
    ...
    foreach (ParticleSystemUI psUI in m_Emitters) {
      if (selectedInstanceIDs.Contains(
              psUI.m_ParticleSystems[0].gameObject.GetInstanceID()))
        result.Add(psUI);
    }
    return result;
  }
  ...
  private void MultiParticleSystemGUI(bool verticalLayout) {
    ...
    m_EmitterAreaScrollPos =
        EditorGUILayout.BeginScrollView(m_EmitterAreaScrollPos);
    {
      Rect emitterAreaRect = EditorGUILayout.BeginVertical();
      {
        // Click-Drag with Alt pressed in entire area
        m_EmitterAreaScrollPos -=
            EditorGUI.MouseDeltaReader(emitterAreaRect, Event.current.alt);
        // Top padding
        GUILayout.Space(3);
        ...
        for (int i = 0; i < m_Emitters.Length; ++i) {
          ...
          ModuleUI rendererModuleUI = m_Emitters [i]
                                          .GetParticleSystemRendererModuleUI();
          if (isRepaintEvent && rendererModuleUI != null &&
              !rendererModuleUI.enabled)
          ...
          Rect psRect = EditorGUILayout.BeginVertical();
          {
            ...
            m_Emitters [i]
                .OnGUI(ModuleUI.k_CompactFixedModuleWidth, true);
          }
        ...
        GUILayout.Space(5);
        if (GUILayout.Button(s_Texts.addParticleSystem, "OL Plus",
                             GUILayout.Width(20))) {
          // Store state of inspector before creating new particle system that
          // will reload the inspector (new selected object)
          // SessionState.SetFloat("CurrentEmitterAreaScroll",
          // m_EmitterAreaScrollPos.x);
          CreateParticleSystem(
              ParticleSystemEditorUtils.GetRoot(m_SelectedParticleSystems[0]),
              SubModuleUI.SubEmitterType.None);
        }
        ...
        m_EmitterAreaScrollPos -=
            EditorGUI.MouseDeltaReader(emitterAreaRect, true);

        GUILayout.FlexibleSpace(); // Makes the emitter area background extend
                                   // to bottom
      }
    ...
    // GUILayout.FlexibleSpace();    // Makes the emitter area background align
    // to bottom of highest emitter
  ...
  private void WindowCurveEditorGUI(bool verticalLayout) {
    ...
    if (verticalLayout) {
      rect = GUILayoutUtility.GetRect(
          13, m_CurveEditorAreaHeight,
          GUILayout.MinHeight(m_CurveEditorAreaHeight));
    } else {
      EditorWindow win = (EditorWindow) m_Owner;
      System.Diagnostics.Debug.Assert(win != null);
      rect = GUILayoutUtility.GetRect(win.position.width - m_EmitterAreaWidth,
                                      win.position.height - 17);
    }
  ...
  void ResizeHandling(bool verticalLayout) {
    ...
    if (verticalLayout) {
      ...
      // For horizontal layout we add a vertical size controller to adjust
      // emitter area width
      float deltaY = EditorGUI.MouseDeltaReader(dragRect, true).y;
      if (deltaY != 0f) {
        m_CurveEditorAreaHeight -= deltaY;
        ClampWindowContentSizes();
        EditorPrefs.SetFloat("ParticleSystemCurveEditorAreaHeight",
                             m_CurveEditorAreaHeight);
      }
      ...
      // For horizontal layout we add a vertical size controller to adjust
      // emitter area width
      dragRect = new Rect(m_EmitterAreaWidth - dragWidth, 0, dragWidth,
                          GUIClip.visibleRect.height);
      float deltaX = EditorGUI.MouseDeltaReader(dragRect, true).x;
      if (deltaX != 0f) {
        m_EmitterAreaWidth += deltaX;
        ClampWindowContentSizes();
        EditorPrefs.SetFloat("ParticleSystemEmitterAreaWidth",
                             m_EmitterAreaWidth);
      }
  ...
  void ClampWindowContentSizes() {
    ...
    if (type != EventType.Layout) {
      ...
      if (verticalLayout)
        m_CurveEditorAreaHeight =
            Mathf.Clamp(m_CurveEditorAreaHeight, k_MinCurveAreaSize.y,
                        height - k_MinEmitterAreaSize.y);
      else
        m_EmitterAreaWidth =
            Mathf.Clamp(m_EmitterAreaWidth, k_MinEmitterAreaSize.x,
                        width - k_MinCurveAreaSize.x);
    }
  }
  ...
  public void OnGUI() {
    ...
    case OwnerType.ParticleSystemWindow: {
      ClampWindowContentSizes();
      bool verticalLayout = m_VerticalLayout; // GUIClip.visibleRect.width <
                                              // GUIClip.visibleRect.height;
    ...
    case OwnerType.Inspector:
      // The inspector window already has added a vertical scrollview so no need
      // to do it here
        ...
        if (moduleUi != null) {
          if (showAll) {
            ...
            if (moduleUi as RendererModuleUI != null) {
              if (particleSystemUI.m_ParticleSystems.FirstOrDefault(
                      o => o.GetComponent<ParticleSystemRenderer>() == null) ==
                  null)
                allowHiding = false;
            }
      ...
      foreach (ParticleSystem selected in psUI.m_ParticleSystems) {
        if (selected != null) {
          ...
          foreach (ParticleSystem ps in allSystems) {
            ...
            if (psRenderer != null) {
              if (enabled)
                psRenderer.editorEnabled = selectedInstanceIDs.Contains(
                    psRenderer.gameObject.GetInstanceID());
              else
