...
internal class ParticleSystemCurveEditor {
  ...
  internal class Styles {
    ...
    public GUIStyle yAxisHeader =
        new GUIStyle(ParticleSystemStyles.Get().label);
    public GUIContent optimizeCurveText = EditorGUIUtility.TrTextContent(
        "Optimize",
        "Click to optimize curve. Optimized curves are defined by having at most 3 keys, with a key at both ends, and do not support loop or ping pong wrapping.");
    public GUIContent removeCurveText =
        EditorGUIUtility.TrTextContent("Remove", "Remove selected curve(s)");
    public GUIContent curveLibraryPopup =
        EditorGUIUtility.TrTextContent("", "Open curve library");
  ...
  public class CurveData {
    ...
    public CurveData(string name, GUIContent displayName,
                     SerializedProperty min, SerializedProperty max,
                     Color color, bool signedRange,
                     CurveWrapper.GetAxisScalarsCallback getAxisScalars,
                     CurveWrapper.SetAxisScalarsCallback setAxisScalars,
                     bool visible) {
      m_UniqueName = name;
      ...
      if (m_Max == null || m_MaxId == 0) {
        Debug.LogError(
            "Max curve should always be valid! (Min curve can be null)");
      }
    }

    public bool IsRegion() { return m_Min != null; }
  }
  ...
  public void Init() {
    ...
    m_CurveEditorSettings.rectangleToolFlags =
        CurveEditorSettings.RectangleToolFlags.MiniRectangleTool;
    ...
    m_CurveEditor = new CurveEditor(new Rect(0, 0, 1000, 100),
                                    CreateCurveWrapperArray(), false);
    m_CurveEditor.settings = m_CurveEditorSettings;
    m_CurveEditor.leftmargin = 40;
    m_CurveEditor.rightmargin = m_CurveEditor.topmargin =
        m_CurveEditor.bottommargin = 25;
    m_CurveEditor.SetShownHRangeInsideMargins(m_CurveEditorSettings.hRangeMin,
                                              m_CurveEditorSettings.hRangeMax);
    m_CurveEditor.SetShownVRangeInsideMargins(m_CurveEditorSettings.vRangeMin,
                                              m_CurveEditorSettings.hRangeMax);
  ...
  void UndoRedoPerformed() { ContentChanged(); }
  ...
  public void AddCurve(CurveData curveData) { Add(curveData); }

  public void RemoveCurve(SerializedProperty max) { RemoveCurve(null, max); }
  ...
  public void AddCurveDataIfNeeded(string curveName, CurveData curveData) {
    // We check if a curve was added by checking our inspector state if a color
    // was set for curveName
  ...
  static bool SameColor(Color c1, Color c2) {
    return Mathf.Abs(c1.r - c2.r) < 0.01f && Mathf.Abs(c1.g - c2.g) < 0.01f &&
  ...
  private void Add(CurveData cd) {
    m_CurveEditor.SelectNone(); // ensures that newly added curves are moved to
                                // front (selected keys will stay on top)
    ...
    SessionState.SetVector3(
        cd.m_UniqueName, new Vector3(cd.m_Color.r, cd.m_Color.g, cd.m_Color.b));
  ...
  private bool Remove(int index) {
    if (index >= 0 && index < m_AddedCurves.Count) {
      ...
      if (m_AddedCurves.Count == 0) {
        ...
        //  Debug.LogError ("Color count mismatch : " + m_AvailableColors.Count
        //  + " != " + m_Colors.Length);
  ...
  public void OnGUI(Rect rect) {
    ...
    Rect curveEditorRect =
        new Rect(rect.x, rect.y, rect.width, rect.height - k_PresetsHeight);
    Rect presetRect = new Rect(rect.x, rect.y + curveEditorRect.height,
                               rect.width, k_PresetsHeight);
    ...
    foreach (CurveWrapper cw in m_CurveEditor.animationCurves) {
      if (cw.getAxisUiScalarsCallback != null &&
          cw.setAxisUiScalarsCallback != null) {
    ...
    DoRemoveSelectedButton(new Rect(curveEditorRect.x, curveEditorRect.y,
                                    curveEditorRect.width, 24));
    DoOptimizeCurveButton(new Rect(curveEditorRect.x, curveEditorRect.y,
                                   curveEditorRect.width, 24));
    presetRect.x += 30;
  ...
  void DoLabelForTopMostCurve(Rect rect) {
    if (m_CurveEditor.IsDraggingCurveOrRegion() ||
        m_CurveEditor.selectedCurves.Count <= 1) {
      int curveID;

      if (m_CurveEditor.GetTopMostCurveID(out curveID)) {
        for (int i = 0; i < m_AddedCurves.Count; ++i) {
          if (m_AddedCurves[i].m_MaxId == curveID ||
              m_AddedCurves[i].m_MinId == curveID) {
            s_Styles.yAxisHeader.normal.textColor = m_AddedCurves[i].m_Color;
            GUI.Label(rect, m_AddedCurves[i].m_DisplayName,
                      s_Styles.yAxisHeader);
            return;
          }
        }
      }
      ...
      for (int j = 0; j < m_AddedCurves.Count; ++j) {
        ...
        if (cd.m_MaxId == topMostCurveID || cd.m_MinId == topMostCurveID) {
          if (doubleCurve.signedRange == cd.m_SignedRange) {
            if (cd.m_MaxId > 0) {
              SetCurve(m_CurveEditor.GetCurveWrapperFromID(cd.m_MaxId),
                       doubleCurve.maxCurve);
            }

            if (cd.m_MinId > 0) {
              SetCurve(m_CurveEditor.GetCurveWrapperFromID(cd.m_MinId),
                       doubleCurve.minCurve);
            }
          } else {
            Debug.LogWarning(
                "Cannot assign a curves with different signed range");
          }
        }
      }
    }
  ...
  void PresetDropDown(Rect rect) {
    if (EditorGUI.DropdownButton(rect, EditorGUI.GUIContents.titleSettingsIcon,
                                 FocusType.Passive,
                                 EditorStyles.inspectorTitlebarText)) {
      DoubleCurve doubleCurveToSaveAsPreset =
          CreateDoubleCurveFromTopMostCurve();
      if (doubleCurveToSaveAsPreset != null) {
        InitDoubleCurvePresets();
        if (m_DoubleCurvePresets != null) {
          m_DoubleCurvePresets.doubleCurveToSave =
              CreateDoubleCurveFromTopMostCurve();
          // We prioritize left first for normal inspector layout (where the
  ...
  void InitDoubleCurvePresets() {
    ...
    if (m_CurveEditor.GetTopMostCurveID(out topMostCurveID)) {
      if (m_DoubleCurvePresets == null ||
        ...
        System.Action<DoubleCurve> presetSelectedCallback =
            delegate(DoubleCurve presetDoubleCurve) {
          SetTopMostCurve(presetDoubleCurve);
          UnityEditorInternal.InternalEditorUtility.RepaintAllViews();
        };
        DoubleCurve doubleCurveToSaveAsPreset =
            CreateDoubleCurveFromTopMostCurve();
        m_DoubleCurvePresets = new DoubleCurvePresetsContentsForPopupWindow(
            doubleCurveToSaveAsPreset, presetSelectedCallback);
  ...
  void PresetCurveButtons(Rect position, Rect curveEditorRect) {
    ...
    DoubleCurvePresetLibrary curveLibrary =
        m_DoubleCurvePresets.GetPresetLibraryEditor().GetCurrentLib();
    ...
    float presetButtonsWidth = showNumPresets * swatchWidth +
                               (showNumPresets - 1) * spaceBetweenSwatches;
    ...
    for (int i = 0; i < showNumPresets; i++) {
      ...
      if (GUI.Button(swatchRect, s_Styles.presetTooltip, GUIStyle.none)) {
        DoubleCurve presetDoubleCurve =
            curveLibrary.GetPreset(i) as DoubleCurve;
  ...
  // Since we don't enforce the layout, we have a button that enforces the curve
  // layout instead.
  void DoOptimizeCurveButton(Rect rect) {
    bool optimizeButtonShown = false;
    Vector2 buttonSize = new Vector2(64, 18);
    Rect buttonRect = new Rect(rect.xMax - 80 - buttonSize.x,
    ...
    if (!m_CurveEditor.IsDraggingCurveOrRegion()) {
      ...
      if (selection.Count > 0) {
        for (int j = 0; j < selection.Count; ++j) {
          CurveWrapper cw =
              m_CurveEditor.GetCurveWrapperFromSelection(selection[j]);
          numValidPolynomialCurve +=
              AnimationUtility.IsValidOptimizedPolynomialCurve(cw.curve) ? 1
                                                                         : 0;
        }
          ...
          if (GUI.Button(buttonRect, s_Styles.optimizeCurveText)) {
            for (int j = 0; j < selection.Count; ++j) {
              ...
              if (!AnimationUtility.IsValidOptimizedPolynomialCurve(cw.curve)) {
                ...
                cw.changed =
                    true; // Used in SaveChangedCurves () later in OnGUI
              }
            }
            m_CurveEditor.SelectNone();
          }
  ...
  void DoRemoveSelectedButton(Rect rect) {
    using(new EditorGUI.DisabledScope(m_CurveEditor.animationCurves.Length ==
                                      0)) {
      Vector2 buttonSize = new Vector2(64, 18);
      Rect clearCurvesRect =
          new Rect(rect.x + rect.width - buttonSize.x - 10,
                   rect.y + (rect.height - buttonSize.y) * 0.5f, buttonSize.x,
                   buttonSize.y);
  ...
  void SaveCurve(SerializedProperty prop, CurveWrapper cw) {
    if (cw.curve.keys.Length == 1) {
      ...
      cw.wrapColorMultiplier =
          Color.clear; // Hide wrapping when we only have 1 key
    } else {
  ...
  CurveWrapper CreateCurveWrapper(
      SerializedProperty curve, int id, int regionId, Color color,
      bool signedRange,
      CurveWrapper.GetAxisScalarsCallback getAxisScalarsCallback,
      CurveWrapper.SetAxisScalarsCallback setAxisScalarsCallback) {
    CurveWrapper cw = new CurveWrapper();
    cw.id = id;
    cw.regionId = regionId;
    cw.color = color;
    cw.renderer = new NormalCurveRenderer(curve.animationCurveValue);
    cw.renderer.SetWrap(curve.animationCurveValue.preWrapMode,
                        curve.animationCurveValue.postWrapMode);
    if (cw.curve.keys.Length == 1) {
      cw.renderer.SetCustomRange(0.0f, 1.0f);
      cw.wrapColorMultiplier =
          Color.clear; // Hide wrapping when we only have 1 key
  ...
  CurveWrapper[] CreateCurveWrapperArray() {
    ...
    for (int i = 0; i < m_AddedCurves.Count; i++) {
      ...
      if (cd.m_Visible) {
        ...
        if (cd.m_Max != null) {
          curveWrappers.Add(CreateCurveWrapper(
              cd.m_Max, cd.m_MaxId, regionId, cd.m_Color, cd.m_SignedRange,
              cd.m_GetAxisScalarsCallback, cd.m_SetAxisScalarsCallback));
        }

        if (cd.m_Min != null) {
          curveWrappers.Add(CreateCurveWrapper(
              cd.m_Min, cd.m_MinId, regionId, cd.m_Color, cd.m_SignedRange,
              cd.m_GetAxisScalarsCallback, cd.m_SetAxisScalarsCallback));
        }
      }
    }
  ...
  void ContentChanged() {
    ...
    m_CurveEditorSettings.showAxisLabels =
        m_CurveEditor.animationCurves.Length > 0;
  }
}
