...
internal class ParticleSystemWindow : EditorWindow, ParticleEffectUIOwner {
  ...
  class Texts {
    public GUIContent lockParticleSystem = EditorGUIUtility.TrTextContent(
        "", "Lock the current selected Particle System");
  }
  ...
  internal static ParticleSystemWindow GetInstance() { return s_Instance; }

  internal bool IsVisible() { return m_IsVisible; }
  ...
  void OnEnable() {
    ...
    ParticleEffectUI.m_VerticalLayout =
        EditorPrefs.GetBool("ShurikenVerticalLayout", false);

    // Get notified when hierarchy- or project window has changes so we can
    // detect if particle systems have been dragged in or out.
  ...
  void OnPauseStateChanged(PauseState state) { Repaint(); }
  ...
  private void OnHierarchyOrProjectWindowWasChanged() { InitEffectUI(); }
  ...
  void DoToolbarGUI() {
    ...
    using(new EditorGUI.DisabledScope(m_ParticleEffectUI == null)) {
      if (!EditorApplication.isPlaying) {
        ...
        if (GUILayout.Button(isPlaying ? ParticleEffectUI.texts.pause
                             : ParticleEffectUI.texts.play, "ToolbarButton",
                               GUILayout.Width(65))) {
          if (m_ParticleEffectUI != null) {
            if (isPlaying)
        ...
        if (GUILayout.Button(ParticleEffectUI.texts.play, "ToolbarButton",
                             GUILayout.Width(65))) {
          if (m_ParticleEffectUI != null) {
            m_ParticleEffectUI.Stop();
            m_ParticleEffectUI.Play();
      ...
      // Resimulation toggle
      ParticleSystemEditorUtils.resimulation = GUILayout.Toggle(
          ParticleSystemEditorUtils.resimulation,
          ParticleEffectUI.texts.resimulation, "ToolbarButton");

      // Bounds toggle
      ParticleEffectUI.m_ShowBounds =
          GUILayout.Toggle(ParticleEffectUI.m_ShowBounds,
                           ParticleEffectUI.texts.showBounds, "ToolbarButton");

      // Editor layout
      if (GUILayout.Button(ParticleEffectUI.m_VerticalLayout ? s_Icons[0]
                           : s_Icons[1], "ToolbarButton")) {
        ParticleEffectUI.m_VerticalLayout = !ParticleEffectUI.m_VerticalLayout;
        EditorPrefs.SetBool("ShurikenVerticalLayout",
                            ParticleEffectUI.m_VerticalLayout);
        { Clear(); }
      }
      ...
      GUILayout.Space(2);
      ParticleSystem lockedParticleSystem =
          ParticleSystemEditorUtils.lockedParticleSystem;
      bool isLocked = lockedParticleSystem != null;
      bool newLocked = GUILayout.Toggle(isLocked, s_Texts.lockParticleSystem,
                                        "IN LockButton");
  ...
  void OnGUI() {
    ...
    if (s_Icons == null)
      s_Icons =
          new GUIContent[]{EditorGUIUtility.IconContent("HorizontalSplit"),
