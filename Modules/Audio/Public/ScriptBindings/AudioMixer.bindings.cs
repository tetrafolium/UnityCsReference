...
namespace UnityEngine.Audio {
  ...
  /// An AudioMixer defines the routing graph for audio signals from
  /// [[AudioSources]] through to the listener [[AudioListener]] AudioMixers are
  /// referenced by AudioListeners and are used to apply effects, attenuation,
  /// submixing etc to the audio signal path.
  ///
  /// SA: [[class-AudioListener|AudioListener component]] in the Components
  /// Reference
  [ExcludeFromPreset]
  ...
  public partial class AudioMixer : Object {
    ...
    [NativeMethod("AudioMixerBindings::FindMatchingGroups",
                  IsFreeFunction = true, HasExplicitThis = true)]
    public extern AudioMixerGroup[] FindMatchingGroups(string subPath);

    internal void TransitionToSnapshot(AudioMixerSnapshot snapshot,
                                       float timeToReach) {
      if (snapshot == null)
        throw new ArgumentException(
            "null Snapshot passed to AudioMixer.TransitionToSnapshot of AudioMixer '" +
            name + "'");
    ...
    private extern void
    TransitionToSnapshotInternal(AudioMixerSnapshot snapshot,
                                 float timeToReach);

    [NativeMethod("AudioMixerBindings::TransitionToSnapshots",
                  IsFreeFunction = true, HasExplicitThis = true,
                  ThrowsException = true)]
    public extern void TransitionToSnapshots(AudioMixerSnapshot[] snapshots,
                                             float[] weights,
                                             float timeToReach);
