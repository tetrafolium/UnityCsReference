...
[NativeHeader(
	 "Modules/Audio/Public/ScriptBindings/AudioMixerPlayable.bindings.h")]
...
public partial struct AudioMixerPlayable : IPlayable,
       IEquatable<AudioMixerPlayable> {
	PlayableHandle m_Handle;

	public static AudioMixerPlayable
	Create(PlayableGraph graph, int inputCount = 0,
	...
	private static PlayableHandle CreateHandle(PlayableGraph graph,
	                                           int inputCount,
	                                           bool normalizeInputVolumes) {
		PlayableHandle handle = PlayableHandle.Null;
		if (!CreateAudioMixerPlayableInternal(ref graph, normalizeInputVolumes,
		                                      ref handle))
		...
		if (handle.IsValid()) {
			if (!handle.IsPlayableOfType<AudioMixerPlayable>())
				throw new InvalidCastException(
					      "Can't set handle: the playable is not an AudioMixerPlayable.");
		}
	...
	//[NativeThrows]
	// extern private static bool GetAutoNormalizeInternal(ref PlayableHandle
	// hdl);

	//[NativeThrows]
	// extern private static void SetAutoNormalizeInternal(ref PlayableHandle
	// hdl, bool normalise);

	[NativeThrows] extern private static bool
	CreateAudioMixerPlayableInternal(ref PlayableGraph graph,
	                                 bool normalizeInputVolumes,
