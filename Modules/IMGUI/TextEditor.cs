...
public class TextEditor {
  ...
  public Vector2 scrollOffset =
      Vector2.zero; // The text field can have a scroll offset in order to
  ...
  void GrabGraphicalCursorPos() {
    if (!hasHorizontalCursorPos) {
      graphicalCursorPos =
          style.GetCursorPixelPosition(localPosition, m_Content, cursorIndex);
      graphicalSelectCursorPos =
          style.GetCursorPixelPosition(localPosition, m_Content, selectIndex);
      hasHorizontalCursorPos = false;
    }
  }
  ...
  public bool HandleKeyEvent(Event e) { return HandleKeyEvent(e, false); }
  ...
  public bool Delete() {
    if (hasSelection) {
      ...
      m_Content.text = text.Remove(
          cursorIndex, NextCodePointIndex(cursorIndex) - cursorIndex);
      return true;
    }
    return false;
  }

  public bool CanPaste() { return GUIUtility.systemCopyBuffer.Length != 0; }
  ...
  public bool hasSelection {
    get { return cursorIndex != selectIndex; }
  }
  ...
  public bool DeleteSelection() {
    if (cursorIndex == selectIndex)
      return false;
    if (cursorIndex < selectIndex) {
      m_Content.text = text.Substring(0, cursorIndex) +
                       text.Substring(selectIndex, text.Length - selectIndex);
      selectIndex = cursorIndex;
    } else {
      m_Content.text = text.Substring(0, selectIndex) +
  ...
  /// Replace the selection with /replace/. If there is no selection, /replace/
  /// is inserted at the current cursor point.
  ...
  public void Insert(char c) { ReplaceSelection(c.ToString()); }
  ...
  public void MoveUp() {
    ...
    cursorIndex = selectIndex = style.GetCursorStringIndex(
        localPosition, m_Content, graphicalCursorPos);
    if (cursorIndex <= 0)
      ClearCursorPos();
  }
  ...
  /// Move to the start of the current graphical line. This takes word-wrapping
  /// into consideration.
  public void MoveGraphicalLineStart() {
    cursorIndex = selectIndex =
        GetGraphicalLineStart(cursorIndex < selectIndex ? cursorIndex
                              : selectIndex);
  }

  /// Move to the end of the current graphical line. This takes word-wrapping
  /// into consideration.
  public void MoveGraphicalLineEnd() {
    cursorIndex = selectIndex =
        GetGraphicalLineEnd(cursorIndex > selectIndex ? cursorIndex
                            : selectIndex);
  }

  /// Moves the cursor to the beginning of the text
  public void MoveTextStart() { selectIndex = cursorIndex = 0; }

  /// Moves the cursor to the end of the text
  public void MoveTextEnd() { selectIndex = cursorIndex = text.Length; }
  ...
  // Move the cursor to a graphical position. Used for moving the cursor on
  // MouseDown events.
  ...
  // Move the cursor to a graphical position. Used for moving the cursor on
  // MouseDown events.
  protected internal void MoveCursorToPosition_Internal(Vector2 cursorPosition,
                                                        bool shift) {
    selectIndex = style.GetCursorStringIndex(localPosition, m_Content,
  ...
  public void MoveAltCursorToPosition(Vector2 cursorPosition) {
    int index = style.GetCursorStringIndex(localPosition, m_Content,
                                           cursorPosition + scrollOffset);
  ...
  public bool IsOverSelection(Vector2 cursorPosition) {
    int p = style.GetCursorStringIndex(localPosition, m_Content,
                                       cursorPosition + scrollOffset);
    return ((p < Mathf.Max(cursorIndex, selectIndex)) &&
            (p > Mathf.Min(cursorIndex, selectIndex)));
  }

  // Do a drag selection. Used to expand the selection in MouseDrag events.
  public void SelectToPosition(Vector2 cursorPosition) {
    if (!m_MouseDragSelectsWholeWords)
      cursorIndex = style.GetCursorStringIndex(localPosition, m_Content,
                                               cursorPosition + scrollOffset);
    else // snap to words/paragraphs
    {
      ...
      if (m_DblClickSnap == DblClickSnapping.WORDS) {
        if (p < m_DblClickInitPos) {
          ...
          selectIndex =
              FindEndOfClassification(m_DblClickInitPos, Direction.Forward);
        } else {
          cursorIndex = FindEndOfClassification(p, Direction.Forward);
          selectIndex =
              FindEndOfClassification(m_DblClickInitPos, Direction.Backward);
        }
      } // paragraph
      else {
        if (p < m_DblClickInitPos) {
  ...
  public void SelectLeft() {
    if (m_bJustSelected)
      if (cursorIndex > selectIndex) { // swap
        int tmp = cursorIndex;
        cursorIndex = selectIndex;
        selectIndex = tmp;
      }
  ...
  public void SelectTextEnd() {
    // This is not quite like the mac - there, when you select to end of text,
    // the position of the cursor becomes somewhat i'll defined Hard to explain.
    // In textedit, try: CMD-SHIFT-down, SHIFT-LEFT for case 1. then do
    // CMD-SHIFT-down, SHIFT-RIGHT, SHIFT-LEFT for case 2. Anyways, it's wrong
    // so we won't do that
    cursorIndex = text.Length;
  }
  ...
  int GetGraphicalLineStart(int p) {
    ...
    point.y +=
        1.0f /
        GUIUtility.pixelsPerPoint; // we make sure no floating point errors can
                                   // make us land on another line
  ...
  int FindNextSeperator(int startPos) {
    ...
    while (startPos < textLen &&
           ClassifyChar(startPos) != CharacterType.LetterLike)
  ...
  enum CharacterType { LetterLike, Symbol, Symbol2, WhiteSpace }
  ...
  /// If the cursor is over a whitespace, it's moved forwards ''till the first
  /// non-whitespace character If the cursor is over an alphanumeric character,
  /// it''s moved forward 'till it encounters space or a punctuation mark. If
  /// the stopping character is a space, this is skipped as well. If the cursor
  /// is over an punctuation mark, it's moved forward ''till it a letter or a
  /// space of a punctuation mark. If the stopping character is a space, this is
  /// skipped as well
    ...
    if (text[p] == ' ') // If we're at a space, skip over any number of spaces
    {
      while (p < textLen && ClassifyChar(p) == CharacterType.WhiteSpace)
        p = NextCodePointIndex(p);
               ...
               text[p] == '\n') // If we're at a tab or a newline, just step one
                                // char ahead
    {
      return p;
    }
  ...
  internal void UpdateScrollOffset() {
    ...
    graphicalCursorPos = style.GetCursorPixelPosition(
        new Rect(0, 0, position.width, position.height), m_Content, cursorPos);
    ...
    Vector2 contentSize =
        new Vector2(style.CalcSize(m_Content).x,
    ...
    if (contentSize.y < viewRect.height) {
      ...
      // go down
      if (localGraphicalCursorPos.y + style.lineHeight >
          scrollOffset.y + viewRect.height)
        scrollOffset.y =
            localGraphicalCursorPos.y - viewRect.height + style.lineHeight;
      // go up
    ...
    // 1. Text field has more text than it can fit vertically, and the cursor is
    // at the very bottom (text field is scrolled down)
    // 2. user e.g. deletes some lines of text at the bottom (backspace or
    // select+delete)
    // 3. now suddenly we have space at the bottom of text field, that is now
    // not filled with any content
    // 4. scroll text field up to fill in that space (this is what other text
    // editors do)
  ...
  public void DrawCursor(string newText) {
    ...
    if (GUIUtility.compositionString.Length > 0) {
      m_Content.text = newText.Substring(0, cursorIndex) +
    ...
    GUIUtility.compositionCursorPos = GUIClip.UnclipToWindow(
        graphicalCursorPos +
        new Vector2(position.x, position.y + style.lineHeight) - scrollOffset);

    if (GUIUtility.compositionString.Length > 0)
      style.DrawWithTextSelection(
          position, m_Content, controlID, cursorIndex,
          cursorIndex + GUIUtility.compositionString.Length, true);
    else
      style.DrawWithTextSelection(position, m_Content, controlID, cursorIndex,
                                  selectIndex);
  ...
  bool PerformOperation(TextEditOp operation, bool textIsReadOnly) {
    ...
    case TextEditOp.SelectGraphicalLineEnd:
      ...
      break;
    //      case TextEditOp.SelectPageUp:                   return SelectPageUp
    //      (); break; case TextEditOp.SelectPageDown:             return
    //      SelectPageDown (); break;
    ...
    case TextEditOp.SelectNone:
      SelectNone();
      break;
    //      case TextEditOp.ScrollStart:            return ScrollStart ();
    //      break; case TextEditOp.ScrollEnd:          return ScrollEnd ();
    //      break; case TextEditOp.ScrollPageUp:       return ScrollPageUp ();
    //      break; case TextEditOp.ScrollPageDown:     return ScrollPageDown ();
    //      break;
  ...
  public void Copy() {
    ...
    string selectedRenderedText = style.Internal_GetSelectedRenderedText(
        localPosition, m_Content, selectIndex, cursorIndex);
  ...
  static string ReplaceNewlinesWithSpaces(string value) {
    // First get rid of Windows style new lines and then *nix so we don't leave
    // '\r' around.
    value = value.Replace("\r\n", " ");
    value = value.Replace('\n', ' ');
  ...
  public void DetectFocusChange() { OnDetectFocusChange(); }
