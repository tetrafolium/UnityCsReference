...
// Use this define to debug controlID consistency together with 's_LogControlID'
// (default false) to enable logging in a codepath thats needs tested for
// consistency. E.g:
...
public sealed class ExitGUIException : Exception {
  public ExitGUIException() { GUIUtility.guiIsExiting = true; }
...
public partial class GUIUtility {
  ...
  public static int hotControl {
    ...
    set { Internal_SetHotControl(value); }
  }
  ...
  public static int keyboardControl {
    ...
    set { Internal_SetKeyboardControl(value); }
  }
  ...
  internal static bool HasKeyFocus(int controlID) {
    return controlID == GUIUtility.keyboardControl &&
           (s_HasCurrentWindowKeyFocusFunc !=
                null ? s_HasCurrentWindowKeyFocusFunc()
            : true);
  }

  //*undocumented*
  public static void ExitGUI() {
    // We have to always throw the ExitGUIException otherwise the exiting out of
    // recursive on GUI will not work.
  ...
  [RequiredByNativeCode]
  internal static void ProcessEvent(int instanceID, IntPtr nativeEventPtr,
                                    out bool result) {
    result = false;
  ...
  internal static void CleanupRoots() { cleanupRoots?.Invoke(); }

  [RequiredByNativeCode]
  internal static void BeginGUI(int skinMode, int instanceID,
                                int useGUILayout) {
    s_SkinMode = skinMode;
    ...
    // Switch to the correct ID list & clear keyboard loop if we're about to
    // layout (we rebuild it during layout, so we want it cleared beforehand)
    if (useGUILayout != 0) {
  ...
  internal static float RoundToPixelGrid(float v) {
    ...
    return Mathf.Floor((v * GUIUtility.pixelsPerPoint) +
                       kNearestRoundingOffset) /
  ...
  public static Rect ScreenToGUIRect(Rect screenRect) {
    Vector2 guiPoint =
        ScreenToGUIPoint(new Vector2(screenRect.x, screenRect.y));
  ...
  public static void RotateAroundPivot(float angle, Vector2 pivotPoint) {
    ...
    Matrix4x4 newMat =
        Matrix4x4.TRS(point, Quaternion.Euler(0, 0, angle), Vector3.one) *
  ...
  public static void ScaleAroundPivot(Vector2 scale, Vector2 pivotPoint) {
    Matrix4x4 mat = GUI.matrix;
    Vector2 point = GUIClip.Unclip(pivotPoint);
    Matrix4x4 newMat = Matrix4x4.TRS(point, Quaternion.identity,
  ...
  internal static bool HitTest(Rect rect, Vector2 point, int offset) {
    return (point.x >= rect.xMin - offset) && (point.x < rect.xMax + offset) &&
           (point.y >= rect.yMin - offset) && (point.y < rect.yMax + offset);
  }
...
internal sealed partial class GUIClip {
  ...
  internal static void Push(Rect screenRect, Vector2 scrollOffset,
                            Vector2 renderOffset, bool resetOffset) {
    Internal_Push(screenRect, scrollOffset, renderOffset, resetOffset);
  }

  // Removes the topmost clipping rectangle, undoing the effect of the latest
  // GUIClip.Push
  internal static void Pop() { Internal_Pop(); }

  // Unclips /pos/ to IMGUI container coordinates.
  public static Vector2 Unclip(Vector2 pos) { return Unclip_Vector2(pos); }

  // Unclips /rect/ to IMGUI container coordinates.
  public static Rect Unclip(Rect rect) { return Unclip_Rect(rect); }
