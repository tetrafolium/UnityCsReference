...
public enum ScaleMode {
  // Stretches the texture to fill the complete rectangle passed in to
  // GUI.DrawTexture
  StretchToFill = 0,
  // Scales the texture, maintaining aspect ratio, so it completely covers the
  // /position/ rectangle passed to GUI.DrawTexture. If the texture is being
...
// Used by GUIUtility.GetcontrolID to inform the UnityGUI system if a given
// control can get keyboard focus.
public enum FocusType {
      [Obsolete(
          "FocusType.Native now behaves the same as FocusType.Passive in all OS cases. (UnityUpgradable) -> Passive",
          false)] Native = 0,
      // This is a proper keyboard control. It can have input focus on all
      // platforms. Used for TextField and TextArea controls
      Keyboard = 1,
