...
static readonly Dictionary<int, LayoutCache> s_StoredLayouts =
	new Dictionary<int, LayoutCache>();
static readonly Dictionary<int, LayoutCache> s_StoredWindows =
	new Dictionary<int, LayoutCache>();
...
internal static LayoutCache SelectIDList(int instanceID, bool isWindow) {
	Dictionary<int, LayoutCache> store =
		isWindow ? s_StoredWindows : s_StoredLayouts;
	LayoutCache cache;
...
internal static void BeginContainer(LayoutCache cache) {
	// Make a vertical group to encompass the whole thing
	if (Event.current.type == EventType.Layout) {
		// Make sure to update all the cached values of the LayoutCache when doing
		// a Layout
...
internal static void BeginWindow(int windowID, GUIStyle style,
...
internal static void Layout() {
	if (current.topLevel.windowID == -1) {
		...
		current.topLevel.SetHorizontal(
			0, Mathf.Min(Screen.width / GUIUtility.pixelsPerPoint,
			             current.topLevel.maxWidth));
		current.topLevel.CalcHeight();
		current.topLevel.SetVertical(
			0, Mathf.Min(Screen.height / GUIUtility.pixelsPerPoint,
			             current.topLevel.maxHeight));
...
// Global layout function. Called from EditorWindows (differs from game view
// in that they use the full window size and try to stretch GUI
internal static void LayoutFromEditorWindow() {
	if (current.topLevel != null) {
		current.topLevel.CalcWidth();
		current.topLevel.SetHorizontal(0,
		                               Screen.width / GUIUtility.pixelsPerPoint);
		current.topLevel.CalcHeight();
		current.topLevel.SetVertical(0,
		                             Screen.height / GUIUtility.pixelsPerPoint);
		...
		Debug.LogError(
			"GUILayout state invalid. Verify that all layout begin/end calls match.");
	}
}
...
// Is public so we can access it from editor inspectors, but not supported by
// public stuff
...
static void LayoutSingleGroup(GUILayoutGroup i) {
	if (!i.isWindow) {
		// CalcWidth knocks out minWidth with the calculated sizes from its
		// children. Normally, this is fine, but since we're in a fixed-size area,
		...
		i.SetHorizontal(i.rect.x,
		                Mathf.Clamp(i.maxWidth, origMinWidth, origMaxWidth));
		...
		i.SetVertical(i.rect.y,
		              Mathf.Clamp(i.maxHeight, origMinHeight, origMaxHeight));
	} else {
		// Figure out the group's min & maxWidth.
		i.CalcWidth();
		...
		// Make it as wide as possible, but the Rect supplied takes precedence...
		i.SetHorizontal(winRect.x,
		                Mathf.Clamp(winRect.width, i.minWidth, i.maxWidth));

		i.CalcHeight();

		// Make it as high as possible, but the Rect supplied takes precedence...
		i.SetVertical(winRect.y,
		              Mathf.Clamp(winRect.height, i.minHeight, i.maxHeight));
...
static GUILayoutGroup CreateGUILayoutGroupInstanceOfType(Type LayoutType) {
	if (!typeof(GUILayoutGroup).IsAssignableFrom(LayoutType))
		throw new ArgumentException(
			      "LayoutType needs to be of type GUILayoutGroup", nameof(LayoutType));
	return (GUILayoutGroup) Activator.CreateInstance(LayoutType);
}

// Generic helper - use this when creating a layoutgroup. It will make sure
...
BeginLayoutGroup(GUIStyle style, GUILayoutOption[] options, Type layoutType) {
	...
	default:
		...
		if (g == null)
			throw new ExitGUIException("GUILayout: Mismatched LayoutGroup." +
			                           Event.current.type);
		g.ResetCursor();
		GUIDebugger.LogLayoutGroupEntry(g.rect, g.marginLeft, g.marginRight,
		                                g.marginTop, g.marginBottom, g.style,
		                                g.isVertical);
		break;
	}
...
internal static void EndLayoutGroup() {
	if (current.layoutGroups.Count == 0 || Event.current == null) {
...
// Generic helper - use this when creating a layout group. It will make sure
// everything is wired up correctly.
internal static GUILayoutGroup BeginLayoutArea(GUIStyle style,
                                               Type layoutType) {
	GUILayoutGroup g;
	switch (Event.current.type) {
	case EventType.Used:
...
internal static GUILayoutGroup DoBeginLayoutArea(GUIStyle style,
                                                 Type layoutType) {
...
static Rect DoGetRect(GUIContent content, GUIStyle style,
                      GUILayoutOption[] options) {
	...
	case EventType.Layout:
		if (style.isHeightDependantOnWidth) {
			...
			// This is needed on non-integer scale ratios to avoid errors to
			// accumulate in further layout calculations
			size.x = Mathf.Ceil(size.x);
			size.y = Mathf.Ceil(size.y);
			current.topLevel.Add(
				new GUILayoutEntry(size.x, size.x, size.y, size.y, style, options));
		}
		return kDummyRect;
	...
	default:
		...
		GUIDebugger.LogLayoutEntry(entry.rect, entry.marginLeft,
		                           entry.marginRight, entry.marginTop,
		                           entry.marginBottom, entry.style);
		return entry.rect;
	}
}
...
public static Rect GetRect(float minWidth, float maxWidth, float minHeight,
                           float maxHeight) {
	return DoGetRect(minWidth, maxWidth, minHeight, maxHeight, GUIStyle.none,
	                 null);
}

public static Rect GetRect(float minWidth, float maxWidth, float minHeight,
                           float maxHeight, GUIStyle style) {
...
static Rect DoGetRect(float minWidth, float maxWidth, float minHeight,
                      float maxHeight, GUIStyle style,
                      GUILayoutOption[] options) {
	switch (Event.current.type) {
	case EventType.Layout:
		current.topLevel.Add(new GUILayoutEntry(minWidth, maxWidth, minHeight,
		                                        maxHeight, style, options));
		return kDummyRect;
	case EventType.Used:
		return kDummyRect;
	default:
