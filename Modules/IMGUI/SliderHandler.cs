...
internal struct SliderHandler {
  ...
  public SliderHandler(Rect position, float currentValue, float size,
                       float start, float end, GUIStyle slider, GUIStyle thumb,
                       bool horiz, int id, GUIStyle thumbExtent = null) {
  ...
  private float OnMouseDown() {
    ...
    if (IsEmptySlider() ||
        (!GUIUtility.HitTest(boundingRect, CurrentEvent()) && !mouseOverThumb))
      return currentValue;
    ...
    if (SupportsPageMovements()) {
      ...
      GUI.nextScrollStepTime =
          SystemClock.now.AddMilliseconds(ScrollWaitDefinitions.firstWait);
  ...
  private float OnMouseDrag() {
    ...
    // that values are relative to the click point - no matter where inside the
    // trough the original value was. Also means user can get back original
    // value if he drags back to start position.
  ...
  private float OnRepaint() {
    ...
    if (!IsEmptySlider() && currentValue >= Mathf.Min(start, end) &&
        currentValue <= Mathf.Max(start, end)) {
      if (thumbExtent != null)
    ...
    if (GUIUtility.HitTest(ThumbRect(), CurrentEvent())) {
      if (GUI.scrollTroughSide !=
          0) // if was scrolling with "trough" and the thumb reached mouse -
             // sliding action over
      {
        GUIUtility.hotControl = 0;
      }
    ...
    GUI.nextScrollStepTime =
        SystemClock.now.AddMilliseconds(ScrollWaitDefinitions.regularWait);
  ...
  private int CurrentScrollTroughSide() {
    float mousePos =
        horiz ? CurrentEvent().mousePosition.x : CurrentEvent().mousePosition.y;
    float thumbPos = horiz ? ThumbRect().x : ThumbRect().y;

    return mousePos > thumbPos ? 1 : -1;
  }

  private bool IsEmptySlider() { return start == end; }
  ...
  private float ValueForCurrentMousePosition() {
    if (horiz)
      return (MousePosition() - ThumbRect().width * .5f) / ValuesPerPixel() +
             start - size * .5f;
    return (MousePosition() - ThumbRect().height * .5f) / ValuesPerPixel() +
           start - size * .5f;
  }
  ...
  private Rect ThumbExtRect() {
    var rect = new Rect(0, 0, thumbExtent.fixedWidth,
                        thumbExtent.fixedHeight){center = ThumbRect().center};
    return rect;
  }
  ...
  private Rect VerticalThumbRect() {
    ...
    if (start < end) {
      return new Rect(contentRect.x,
                      (ClampedCurrentValue() - start) * valuesPerPixel +
                          contentRect.y,
                      thumbWidth, size * valuesPerPixel + thumbSize);
    } else {
      return new Rect(contentRect.x,
                      (ClampedCurrentValue() + size - start) * valuesPerPixel +
                          contentRect.y,
                      thumbWidth, size * -valuesPerPixel + thumbSize);
    }
  }
  ...
  private Rect HorizontalThumbRect() {
    ...
    if (start < end)
      return new Rect(
          ...
          contentRect.y, size * valuesPerPixel + thumbSize, thumbHeight);
  ...
  private float ValuesPerPixel() {
    if (horiz)
      return (position.width - slider.padding.horizontal - ThumbSize()) /
             (end - start);
    return (position.height - slider.padding.vertical - ThumbSize()) /
           (end - start);
  }
  ...
  private float MaxValue() { return Mathf.Max(start, end) - size; }
