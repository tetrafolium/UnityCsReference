...
public sealed partial class Event {
  public Event() { m_Ptr = Internal_Create(0); }

  public Event(int displayIndex) { m_Ptr = Internal_Create(displayIndex); }
  ...
  internal static void CleanupRoots() {
    // Required for application quite, so we can force GC to collect root
    // objects before Unity managers are destroyed
    s_Current = null;
    s_MasterEvent = null;
  }
  ...
  [Obsolete(
      "Use HandleUtility.GUIPointToWorldRay(Event.current.mousePosition);",
      true)]
  public Ray mouseRay {
    get { return new Ray(Vector3.up, Vector3.up); }
    set {}
  }
  ...
  // manipulates elements (like a touch screen), without any cursor acting as an
  // intermediate.
  ...
  public static Event KeyboardEvent(string key) {
    ...
    string subStr =
        key.Substring(startIdx, key.Length - startIdx).ToLowerInvariant();
    switch (subStr) {
    case "[0]":
      evt.character = '0';
    ...
    default:
      if (subStr.Length != 1) {
        try {
          ...
          Debug.LogError(UnityString.Format(
              "Unable to find key name that matches '{0}'", subStr));
        }
      } else {
  ...
  public override bool Equals(object obj) {
    ...
    Event rhs = (Event) obj;
    // We are ignoring Caps Lock for modifiers, so that Key Combinations will
    // still work when Caps Lock is down.
    if (type != rhs.type || (modifiers & ~EventModifiers.CapsLock) !=
                                (rhs.modifiers & ~EventModifiers.CapsLock))
      return false;
    if (isKey)
    ...
    if (isKey) {
      if (character == 0)
        return UnityString.Format(
            "Event:{0}   Character:\\0   Modifiers:{1}   KeyCode:{2}", type,
            modifiers, keyCode);

      return "Event:" + type + "   Character:" + (int)(character) +
             "   Modifiers:" + modifiers + "   KeyCode:" + keyCode;
    }
    if (isMouse)
      return UnityString.Format("Event: {0}   Position: {1} Modifiers: {2}",
                                type, mousePosition, modifiers);
  ...
  public void Use() {
    if (type == EventType.Repaint || type == EventType.Layout)
      Debug.LogWarning(UnityString.Format(
          "Event.Use() should not be called for events of type {0}", type));
    Internal_Use();
  }
}
}
