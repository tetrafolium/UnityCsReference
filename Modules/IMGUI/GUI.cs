...
public partial class GUI {
  ...
  internal static void CleanupRoots() {
    ...
    // Required for Windows Store Apps when cleaning up everything on
    // application quit.
    //  Otherwise managed roots are being freed after Unity managers are
    //  destroyed, in this case when finalizers are invoked for GUI* objects
    //  they're accessing non existent managers.
    s_Skin = null;

    // GUILayoutUtility.CleanupRoots indirectly invokes GUILayoutUtility ctor,
    // which indirectly sets GUIStyle s_None, that's why we need to invoke
    // GUILayoutUtility.CleanupRoots before GUIStyle.CleanupRoots
    ...
    //    UnityEngine.DLL!UnityEngine.GUILayoutGroup.GUILayoutGroup() Line 592
    //    C#
    //    UnityEngine.DLL!UnityEngine.GUILayoutUtility.LayoutCache.LayoutCache()
    //    Line 19    C#
    //    UnityEngine.DLL!UnityEngine.GUILayoutUtility.GUILayoutUtility() Line
    //    43   C# [Native to Managed Transition]
  ...
  // The tooltip of the control the mouse is currently over, or which has
  // keyboard focus. (RO).
  ...
  public static void DrawTexture(Rect position, Texture image,
                                 ScaleMode scaleMode, bool alphaBlend,
                                 float imageAspect) {
    DrawTexture(position, image, scaleMode, alphaBlend, imageAspect, GUI.color,
                0, 0);
  }
  ...
  public static void DrawTexture(Rect position, Texture image,
                                 ScaleMode scaleMode, bool alphaBlend,
                                 float imageAspect, Color color,
                                 float borderWidth, float borderRadius) {
    var borderWidths = Vector4.one * borderWidth;
    DrawTexture(position, image, scaleMode, alphaBlend, imageAspect, color,
                borderWidths, borderRadius);
  }
  ...
  public static void DrawTexture(Rect position, Texture image,
                                 ScaleMode scaleMode, bool alphaBlend,
                                 float imageAspect, Color color,
                                 Vector4 borderWidths, float borderRadius) {
  ...
  public static void DrawTexture(Rect position, Texture image,
                                 ScaleMode scaleMode, bool alphaBlend,
                                 float imageAspect, Color color,
                                 Vector4 borderWidths, Vector4 borderRadiuses) {
    DrawTexture(position, image, scaleMode, alphaBlend, imageAspect, color,
                borderWidths, borderRadiuses, true);
  }

  internal static void DrawTexture(Rect position, Texture image,
                                   ScaleMode scaleMode, bool alphaBlend,
                                   float imageAspect, Color color,
                                   Vector4 borderWidths, Vector4 borderRadiuses,
                                   bool drawSmoothCorners) {
    DrawTexture(position, image, scaleMode, alphaBlend, imageAspect, color,
                color, color, color, borderWidths, borderRadiuses,
                drawSmoothCorners);
  }
  ...
  internal static void DrawTexture(Rect position, Texture image,
                                   ScaleMode scaleMode, bool alphaBlend,
                                   float imageAspect, Color leftColor,
                                   Color topColor, Color rightColor,
                                   Color bottomColor, Vector4 borderWidths,
                                   Vector4 borderRadiuses) {
    DrawTexture(position, image, scaleMode, alphaBlend, imageAspect, leftColor,
                topColor, rightColor, bottomColor, borderWidths, borderRadiuses,
                true);
  }
  ...
  internal static void DrawTexture(Rect position, Texture image,
    ...
    if (Event.current.type == EventType.Repaint) {
      ...
      if (borderWidths != Vector4.zero) {
        if ((leftColor != topColor) || (leftColor != rightColor) ||
      ...
      Internal_DrawTextureArguments arguments =
          new Internal_DrawTextureArguments{leftBorder = 0,
                                            rightBorder = 0,
                                            topBorder = 0,
                                            bottomBorder = 0,
                                            color = leftColor,
                                            ...
                                            mat = mat};
      CalculateScaledTextureRects(position, scaleMode, imageAspect,
                                  ref arguments.screenRect,
                                  ref arguments.sourceRect);
      Graphics.Internal_DrawTexture(ref arguments);
    }
  }

  // Calculate screenrect and sourcerect for different scalemodes
  internal static bool CalculateScaledTextureRects(Rect position,
                                                   ScaleMode scaleMode,
                                                   float imageAspect,
    ...
    case ScaleMode.ScaleToFit:
      if (destAspect > imageAspect) {
        float stretch = imageAspect / destAspect;
        outScreenRect =
            new Rect(position.xMin + position.width * (1.0f - stretch) * .5f,
                     position.yMin, stretch * position.width, position.height);
        outSourceRect = new Rect(0, 0, 1, 1);
        ret = true;
      } else {
        float stretch = destAspect / imageAspect;
        outScreenRect =
            new Rect(position.xMin,
                     position.yMin + position.height * (1.0f - stretch) * .5f,
                     position.width, stretch * position.height);
        outSourceRect = new Rect(0, 0, 1, 1);
        ret = true;
      }
      break;
  ...
  // Draw a texture within a rectangle with the given texture coordinates. Use
  // this function for clipping or tiling the image within the given rectangle.
  public static void DrawTextureWithTexCoords(Rect position, Texture image,
                                              Rect texCoords) {
  ...
  // this function for clipping or tiling the image within the given rectangle.
  public static void DrawTextureWithTexCoords(Rect position, Texture image,
                                              Rect texCoords, bool alphaBlend) {
    GUIUtility.CheckOnGUI();
    ...
    if (Event.current.type == EventType.Repaint) {
      ...
      Internal_DrawTextureArguments arguments =
          new Internal_DrawTextureArguments();
  ...
  internal static bool Button(Rect position, int id, GUIContent content,
                              GUIStyle style) {
    GUIUtility.CheckOnGUI();
  ...
  public static bool RepeatButton(Rect position, string text) {
    return DoRepeatButton(position, GUIContent.Temp(text), s_Skin.button,
                          FocusType.Passive);
  }

  public static bool RepeatButton(Rect position, Texture image) {
    return DoRepeatButton(position, GUIContent.Temp(image), s_Skin.button,
                          FocusType.Passive);
  }
  ...
  public static bool RepeatButton(Rect position, string text, GUIStyle style) {
    return DoRepeatButton(position, GUIContent.Temp(text), style,
                          FocusType.Passive);
  }

  public static bool RepeatButton(Rect position, Texture image,
                                  GUIStyle style) {
    return DoRepeatButton(position, GUIContent.Temp(image), style,
                          FocusType.Passive);
  }
  ...
  private static bool DoRepeatButton(Rect position, GUIContent content,
                                     GUIStyle style, FocusType focusType) {
    GUIUtility.CheckOnGUI();
  ...
  public static string TextField(Rect position, string text) {
    ...
    DoTextField(position, GUIUtility.GetControlID(FocusType.Keyboard, position),
                t, false, -1, GUI.skin.textField);
    return t.text;
  }
  ...
  public static string TextField(Rect position, string text, int maxLength) {
    GUIContent t = GUIContent.Temp(text);
    DoTextField(position, GUIUtility.GetControlID(FocusType.Keyboard, position),
                t, false, maxLength, GUI.skin.textField);
    return t.text;
  }
  ...
  public static string PasswordField(Rect position, string password,
                                     char maskChar, int maxLength) {
    return PasswordField(position, password, maskChar, maxLength,
                         GUI.skin.textField);
  }

  public static string PasswordField(Rect position, string password,
                                     char maskChar, GUIStyle style) {
  ...
  public static string PasswordField(Rect position, string password,
    ...
    if (TouchScreenKeyboard.isSupported &&
      ...
      DoTextField(position, GUIUtility.GetControlID(FocusType.Keyboard), t,
                  false, maxLength, style, password, maskChar);
    else
      DoTextField(position,
                  GUIUtility.GetControlID(FocusType.Keyboard, position), t,
                  false, maxLength, style);
  ...
  internal static string PasswordFieldGetStrToShow(string password,
                                                   char maskChar) {
    return (Event.current.type == EventType.Repaint ||
            Event.current.type == EventType.MouseDown)
               ? "".PadRight(password.Length, maskChar)
               : password;
  }
  ...
  public static string TextArea(Rect position, string text, int maxLength) {
    GUIContent t = GUIContent.Temp(text);
    DoTextField(position, GUIUtility.GetControlID(FocusType.Keyboard, position),
                t, true, maxLength, GUI.skin.textArea);
    return t.text;
  }
  ...
  public static string TextArea(Rect position, string text, int maxLength,
    ...
    GUIContent t = GUIContent.Temp(text);
    DoTextField(position, GUIUtility.GetControlID(FocusType.Keyboard, position),
                t, true, maxLength, style);
    return t.text;
  }

  internal static void DoTextField(Rect position, int id, GUIContent content,
                                   bool multiline, int maxLength,
                                   GUIStyle style) {
  ...
  internal static void DoTextField(Rect position, int id, GUIContent content,
                                   bool multiline, int maxLength,
                                   GUIStyle style, string secureText) {
    DoTextField(position, id, content, multiline, maxLength, style, secureText,
                '\0');
  }
  ...
  internal static void DoTextField(Rect position, int id, GUIContent content,
    ...
    TextEditor editor =
        (TextEditor) GUIUtility.GetStateObject(typeof(TextEditor), id);
    ...
    if (TouchScreenKeyboard.isSupported &&
        !TouchScreenKeyboard.isInPlaceEditingAllowed) {
      HandleTextFieldEventForTouchscreen(position, id, content, multiline,
                                         maxLength, style, secureText, maskChar,
                                         editor);
    } else // Not supported means we have a physical keyboard attached
    {
      HandleTextFieldEventForDesktop(position, id, content, multiline,
                                     maxLength, style, editor);
    }
  ...
  private static void HandleTextFieldEventForTouchscreen(
      Rect position, int id, GUIContent content, bool multiline, int maxLength,
    ...
    case EventType.MouseDown:
      if (position.Contains(evt.mousePosition)) {
        ...
        if (s_HotTextField != -1 && s_HotTextField != id) {
          TextEditor currentEditor = (TextEditor) GUIUtility.GetStateObject(
              typeof(TextEditor), s_HotTextField);
          currentEditor.keyboardOnScreen = null;
        }

        s_HotTextField = id;

        // in player setting keyboard control calls OnFocus every time, don't
        // want that. In editor it does not do that for some reason
        if (GUIUtility.keyboardControl != id)
          GUIUtility.keyboardControl = id;

        editor.keyboardOnScreen = TouchScreenKeyboard.Open(
            secureText ?? content.text, TouchScreenKeyboardType.Default,
            true, // autocorrection
            multiline, (secureText != null));
    ...
    case EventType.Repaint:
      if (editor.keyboardOnScreen != null) {
        ...
        if (editor.keyboardOnScreen.status !=
      ...
      // if we use system keyboard we will have normal text returned (hiding
      // symbols is done inside os) so before drawing make sure we hide them
      // ourselves
  ...
  private static void
  HandleTextFieldEventForDesktop(Rect position, int id, GUIContent content,
    ...
    case EventType.KeyDown:
      ...
      // Simplest test: only allow the character if the display font supports
      // it.
      ...
      // On windows, keypresses also send events with keycode but no character.
      // Eat them up here.
      if (c == 0) {
        // if we have a composition string, make sure we clear the previous
        // selection.
  ...
  public static bool Toggle(Rect position, bool value, GUIContent content,
    ...
    return DoToggle(
        position,
        GUIUtility.GetControlID(s_ToggleHash, FocusType.Passive, position),
        value, content, style);
  }

  public static bool Toggle(Rect position, int id, bool value,
                            GUIContent content, GUIStyle style) {
    GUIUtility.CheckOnGUI();
    return DoToggle(position, id, value, content, style);
  }

  public enum ToolbarButtonSize { Fixed, FitToContents }
  ...
  public static int Toolbar(Rect position, int selected,
                            GUIContent[] contents) {
  ...
  public static int Toolbar(Rect position, int selected, GUIContent[] contents,
    ...
    return Toolbar(position, selected, contents, null, style,
                   ToolbarButtonSize.Fixed);
  }

  // Make a toolbar
  public static int Toolbar(Rect position, int selected, GUIContent[] contents,
                            GUIStyle style, ToolbarButtonSize buttonSize) {
    return Toolbar(position, selected, contents, null, style, buttonSize);
  }

  internal static int Toolbar(Rect position, int selected,
                              GUIContent[] contents, string[] controlNames,
                              GUIStyle style, ToolbarButtonSize buttonSize,
    ...
    FindStyles(ref style, out firstStyle, out midStyle, out lastStyle, "left",
               "mid", "right");

    return DoButtonGrid(position, selected, contents, controlNames,
                        contents.Length, style, firstStyle, midStyle, lastStyle,
                        buttonSize, contentsEnabled);
  }

  public static int SelectionGrid(Rect position, int selected, string[] texts,
                                  int xCount) {
    return SelectionGrid(position, selected, GUIContent.Temp(texts), xCount,
                         null);
  }

  public static int SelectionGrid(Rect position, int selected, Texture[] images,
                                  int xCount) {
    return SelectionGrid(position, selected, GUIContent.Temp(images), xCount,
                         null);
  }

  public static int SelectionGrid(Rect position, int selected,
                                  GUIContent[] content, int xCount) {
    return SelectionGrid(position, selected, content, xCount, null);
  }

  public static int SelectionGrid(Rect position, int selected, string[] texts,
                                  int xCount, GUIStyle style) {
    return SelectionGrid(position, selected, GUIContent.Temp(texts), xCount,
                         style);
  }
  ...
  public static int SelectionGrid(Rect position, int selected,
                                  GUIContent[] contents, int xCount,
                                  GUIStyle style) {
    if (style == null)
      style = s_Skin.button;
    return DoButtonGrid(position, selected, contents, null, xCount, style,
                        style, style, style, ToolbarButtonSize.Fixed);
  }

  // Find many GUIStyles from style.name permutations (Helper function for
  // toolbars).
  internal static void FindStyles(ref GUIStyle style, out GUIStyle firstStyle,
                                  out GUIStyle midStyle, out GUIStyle lastStyle,
  ...
  internal static int CalcTotalHorizSpacing(int xCount, GUIStyle style,
                                            GUIStyle firstStyle,
                                            GUIStyle midStyle,
                                            GUIStyle lastStyle) {
    if (xCount < 2)
    ...
    return Mathf.Max(firstStyle.margin.right, midStyle.margin.left) +
           Mathf.Max(midStyle.margin.right, lastStyle.margin.left) +
           internalSpace * (xCount - 3);
  }

  private static bool DoControl(Rect position, int id, bool on, bool hover,
                                GUIContent content, GUIStyle style) {
    var evt = Event.current;
    switch (evt.type) {
    case EventType.Repaint:
    ...
    case EventType.KeyDown:
      ...
      if ((evt.keyCode == KeyCode.Space || evt.keyCode == KeyCode.Return ||
          ...
          !anyModifiers && GUIUtility.keyboardControl == id) {
        evt.Use();
        changed = true;
        return !on;
      }
  ...
  private static void DoLabel(Rect position, GUIContent content,
                              GUIStyle style) {
    var evt = Event.current;
    ...
    // Is inside label AND inside guiclip visible rect (prevents tooltips on
    // labels that are clipped)
    if (!String.IsNullOrEmpty(content.tooltip) &&
        position.Contains(evt.mousePosition) &&
        GUIClip.visibleRect.Contains(evt.mousePosition))
      GUIStyle.SetMouseTooltip(content.tooltip, position);
  }

  internal static bool DoToggle(Rect position, int id, bool value,
                                GUIContent content, GUIStyle style) {
    return DoControl(position, id, value,
                     position.Contains(Event.current.mousePosition), content,
                     style);
  }

  internal static bool DoButton(Rect position, int id, GUIContent content,
                                GUIStyle style) {
    return DoControl(position, id, false,
                     position.Contains(Event.current.mousePosition), content,
                     style);
  }
  ...
  // Make a button grid
  private static int
  DoButtonGrid(Rect position, int selected, GUIContent[] contents,
               string[] controlNames, int xCount, GUIStyle style,
               GUIStyle firstStyle, GUIStyle midStyle, GUIStyle lastStyle,
               ToolbarButtonSize buttonSize, bool[] contentsEnabled = null) {
    GUIUtility.CheckOnGUI();
    ...
    if (count % xCount != 0)
      rows++;
    float totalHorizSpacing =
        CalcTotalHorizSpacing(xCount, style, firstStyle, midStyle, lastStyle);
    float totalVerticalSpacing =
        Mathf.Max(style.margin.top, style.margin.bottom) * (rows - 1);
    ...
    Rect[] buttonRects =
        CalcMouseRects(position, contents, xCount, elemWidth, elemHeight, style,
                       firstStyle, midStyle, lastStyle, false, buttonSize);
    ...
    for (int buttonIndex = 0; buttonIndex < count; ++buttonIndex) {
      ...
      var id = GUIUtility.GetControlID(s_ButtonGridHash, FocusType.Passive,
                                       buttonRect);
      ...
      case EventType.Repaint:
        var buttonStyle =
            count == 1
                ...
                : (buttonIndex == 0
                       ? firstStyle
                       : (buttonIndex == count - 1 ? lastStyle : midStyle));
        ...
        if (!isSelected)
          buttonStyle.Draw(buttonRect, content,
                           isMouseOver && (enabled || isHotControl) &&
                               (isHotControl || GUIUtility.hotControl == 0),
                           enabled && isHotControl, false, false);
        else
    ...
    if (selectedButtonStyle != null) {
      ...
      selectedButtonStyle.Draw(buttonRect, content,
                               isMouseOver && (enabled || isHotControl) &&
                                   (isHotControl || GUIUtility.hotControl == 0),
                               enabled && isHotControl, true, false);
      enabled = wasEnabled;
    }
  ...
  private static Rect[] CalcMouseRects(Rect position, GUIContent[] contents,
                                       int xCount, float elemWidth,
                                       float elemHeight, GUIStyle style,
                                       GUIStyle firstStyle, GUIStyle midStyle,
                                       GUIStyle lastStyle, bool addBorders,
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!addBorders)
        ...
        retval[i] =
            currentStyle.margin.Add(new Rect(xPos, yPos, w, elemHeight));
      ...
      xPos = retval[i].xMax +
             Mathf.Max(currentStyle.margin.right, nextStyle.margin.left);
  ...
  public static float HorizontalSlider(Rect position, float value,
                                       float leftValue, float rightValue) {
    return Slider(position, value, 0, leftValue, rightValue,
                  skin.horizontalSlider, skin.horizontalSliderThumb, true, 0,
                  skin.horizontalSliderThumbExtent);
  }
  ...
  public static float HorizontalSlider(Rect position, float value,
    ...
    return Slider(position, value, 0, leftValue, rightValue, slider, thumb,
                  true, 0, null);
  }
  ...
  public static float HorizontalSlider(Rect position, float value,
                                       float leftValue, float rightValue,
                                       GUIStyle slider, GUIStyle thumb,
                                       GUIStyle thumbExtent) {
    return Slider(
        position, value, 0, leftValue, rightValue, slider, thumb, true, 0,
        (thumbExtent == null && thumb == GUI.skin.horizontalSliderThumb)
            ? GUI.skin.horizontalSliderThumbExtent
            : thumbExtent);
  }

  public static float VerticalSlider(Rect position, float value, float topValue,
                                     float bottomValue) {
    return Slider(position, value, 0, topValue, bottomValue,
                  skin.verticalSlider, skin.verticalSliderThumb, false, 0,
                  skin.verticalSliderThumbExtent);
  }

  // A vertical slider the user can drag to change a value between a min and a
  // max.
  public static float VerticalSlider(Rect position, float value, float topValue,
                                     float bottomValue, GUIStyle slider,
                                     GUIStyle thumb) {
    return Slider(position, value, 0, topValue, bottomValue, slider, thumb,
                  false, 0, null);
  }

  public static float VerticalSlider(Rect position, float value, float topValue,
                                     float bottomValue, GUIStyle slider,
                                     GUIStyle thumb, GUIStyle thumbExtent) {
    return Slider(position, value, 0, topValue, bottomValue, slider, thumb,
                  false, 0,
                  (thumbExtent == null && thumb == GUI.skin.verticalSliderThumb)
                      ? GUI.skin.verticalSliderThumbExtent
                      : thumbExtent);
  }

  // Main slider function.
  // Handles scrollbars & sliders in both horizontal & vertical directions.
  public static float Slider(Rect position, float value, float size,
                             float start, float end, GUIStyle slider,
                             GUIStyle thumb, bool horiz, int id,
                             GUIStyle thumbExtent = null) {
    GUIUtility.CheckOnGUI();
    if (id == 0) {
      id = GUIUtility.GetControlID(s_SliderHash, FocusType.Passive, position);
    }
    return new SliderHandler(position, value, size, start, end, slider, thumb,
                             horiz, id, thumbExtent)
        .Handle();
  }

  public static float HorizontalScrollbar(Rect position, float value,
                                          float size, float leftValue,
                                          float rightValue) {
    return Scroller(position, value, size, leftValue, rightValue,
                    skin.horizontalScrollbar, skin.horizontalScrollbarThumb,
                    skin.horizontalScrollbarLeftButton,
                    skin.horizontalScrollbarRightButton, true);
  }

  // Make a horizontal scrollbar. Scrollbars are what you use to scroll through
  // a document. Most likely, you want to use scrollViews instead.
  public static float HorizontalScrollbar(Rect position, float value,
                                          float size, float leftValue,
                                          float rightValue, GUIStyle style) {
    return Scroller(position, value, size, leftValue, rightValue, style,
                    skin.GetStyle(style.name + "thumb"),
                    skin.GetStyle(style.name + "leftbutton"),
                    skin.GetStyle(style.name + "rightbutton"), true);
  }

  internal static bool ScrollerRepeatButton(int scrollerID, Rect rect,
                                            GUIStyle style) {
    ...
    if (DoRepeatButton(rect, GUIContent.none, style, FocusType.Passive)) {
      ...
      if (firstClick) {
        hasChanged = true;
        nextScrollStepTime =
            DateTime.Now.AddMilliseconds(ScrollWaitDefinitions.firstWait);
      } else {
        if (DateTime.Now >= nextScrollStepTime) {
          hasChanged = true;
          nextScrollStepTime =
              DateTime.Now.AddMilliseconds(ScrollWaitDefinitions.regularWait);
        }
      }
  ...
  public static float VerticalScrollbar(Rect position, float value, float size,
                                        float topValue, float bottomValue) {
    return Scroller(position, value, size, topValue, bottomValue,
                    skin.verticalScrollbar, skin.verticalScrollbarThumb,
                    skin.verticalScrollbarUpButton,
                    skin.verticalScrollbarDownButton, false);
  }
  ...
  public static float VerticalScrollbar(Rect position, float value, float size,
    ...
    return Scroller(position, value, size, topValue, bottomValue, style,
                    skin.GetStyle(style.name + "thumb"),
                    skin.GetStyle(style.name + "upbutton"),
                    skin.GetStyle(style.name + "downbutton"), false);
  }

  internal static float Scroller(Rect position, float value, float size,
                                 float leftValue, float rightValue,
                                 GUIStyle slider, GUIStyle thumb,
                                 GUIStyle leftButton, GUIStyle rightButton,
                                 bool horiz) {
    GUIUtility.CheckOnGUI();
    ...
    if (horiz) {
      sliderRect = new Rect(position.x + leftButton.fixedWidth, position.y,
                            position.width - leftButton.fixedWidth -
                                rightButton.fixedWidth,
                            position.height);
      minRect = new Rect(position.x, position.y, leftButton.fixedWidth,
                         position.height);
      maxRect = new Rect(position.xMax - rightButton.fixedWidth, position.y,
                         rightButton.fixedWidth, position.height);
    } else {
      sliderRect = new Rect(
          position.x, position.y + leftButton.fixedHeight, position.width,
          position.height - leftButton.fixedHeight - rightButton.fixedHeight);
      minRect = new Rect(position.x, position.y, position.width,
                         leftButton.fixedHeight);
      maxRect = new Rect(position.x, position.yMax - rightButton.fixedHeight,
                         position.width, rightButton.fixedHeight);
    }

    value = Slider(sliderRect, value, size, leftValue, rightValue, slider,
                   thumb, horiz, id);
    ...
    if (wasMouseUpEvent &&
        Event.current.type ==
            EventType
                .Used) // repeat buttons ate mouse up event - release scrolling
      s_ScrollControlId = 0;
  ...
  public static void BeginClip(Rect position, Vector2 scrollOffset,
                               Vector2 renderOffset, bool resetOffset) {
    GUIUtility.CheckOnGUI();
  ...
  internal static void BeginGroup(Rect position, GUIContent content,
                                  GUIStyle style, Vector2 scrollOffset) {
    GUIUtility.CheckOnGUI();
  ...
  // Similar to BeginGroup but does not use GUIUtility.GetControlID () for style
  // rendering and can therefore be used in Repaint events only. BeginGroup
  // needs to be called on every event for consistent controlIDs.
  ...
  internal static UnityEngineInternal.GenericStack scrollViewStates {
    get;
    set;
  }
  = new UnityEngineInternal.GenericStack();

  public static Vector2 BeginScrollView(Rect position, Vector2 scrollPosition,
                                        Rect viewRect) {
    return BeginScrollView(position, scrollPosition, viewRect, false, false,
                           skin.horizontalScrollbar, skin.verticalScrollbar,
                           GUI.skin.scrollView);
  }
  ...
  public static Vector2 BeginScrollView(Rect position, Vector2 scrollPosition,
    ...
    return BeginScrollView(position, scrollPosition, viewRect,
  ...
  DoBeginScrollView(Rect position, Vector2 scrollPosition, Rect viewRect,
                    bool alwaysShowHorizontal, bool alwaysShowVertical,
                    GUIStyle horizontalScrollbar, GUIStyle verticalScrollbar,
                    GUIStyle background) {
    return BeginScrollView(position, scrollPosition, viewRect,
  ...
  BeginScrollView(Rect position, Vector2 scrollPosition, Rect viewRect,
    ...
    ScrollViewState state = (ScrollViewState) GUIUtility.GetStateObject(
        typeof(ScrollViewState), id);
    ...
    case EventType.Used:
      break;
    default:
      bool needsVertical = alwaysShowVertical,
           needsHorizontal = alwaysShowHorizontal;

      // Check if we need a horizontal scrollbar
      if (needsHorizontal || viewRect.width > clipRect.width) {
        state.visibleRect.height = position.height -
                                   horizontalScrollbar.fixedHeight +
                                   horizontalScrollbar.margin.top;
        clipRect.height -=
            horizontalScrollbar.fixedHeight + horizontalScrollbar.margin.top;
        needsHorizontal = true;
      }
      if (needsVertical || viewRect.height > clipRect.height) {
        state.visibleRect.width = position.width -
                                  verticalScrollbar.fixedWidth +
                                  verticalScrollbar.margin.left;
        clipRect.width -=
            verticalScrollbar.fixedWidth + verticalScrollbar.margin.left;
        needsVertical = true;
        if (!needsHorizontal && viewRect.width > clipRect.width) {
          state.visibleRect.height = position.height -
                                     horizontalScrollbar.fixedHeight +
      ...
      if (Event.current.type == EventType.Repaint &&
        ...
        background.Draw(position,
                        position.Contains(Event.current.mousePosition), false,
                        needsHorizontal && needsVertical, false);
      }
      if (needsHorizontal && horizontalScrollbar != GUIStyle.none) {
        scrollPosition.x = HorizontalScrollbar(
            new Rect(position.x,
                     ...
                     clipRect.width, horizontalScrollbar.fixedHeight),
            scrollPosition.x, Mathf.Min(clipRect.width, viewRect.width), 0,
            viewRect.width, horizontalScrollbar);
      } else {
        GUIUtility.GetControlID(s_SliderHash, FocusType.Passive);
        GUIUtility.GetControlID(s_RepeatButtonHash, FocusType.Passive);
        GUIUtility.GetControlID(s_RepeatButtonHash, FocusType.Passive);
        scrollPosition.x = horizontalScrollbar != GUIStyle.none ? 0
            : Mathf.Clamp(scrollPosition.x, 0,
                          Mathf.Max(viewRect.width - position.width, 0));
      }

      if (needsVertical && verticalScrollbar != GUIStyle.none) {
        scrollPosition.y = VerticalScrollbar(
            new Rect(clipRect.xMax + verticalScrollbar.margin.left, clipRect.y,
                     verticalScrollbar.fixedWidth, clipRect.height),
            scrollPosition.y, Mathf.Min(clipRect.height, viewRect.height), 0,
            viewRect.height, verticalScrollbar);
      } else {
        GUIUtility.GetControlID(s_SliderHash, FocusType.Passive);
        GUIUtility.GetControlID(s_RepeatButtonHash, FocusType.Passive);
        GUIUtility.GetControlID(s_RepeatButtonHash, FocusType.Passive);
        scrollPosition.y = verticalScrollbar != GUIStyle.none ? 0
            : Mathf.Clamp(scrollPosition.y, 0,
                          Mathf.Max(viewRect.height - position.height, 0));
      }
      break;
    }
    GUIClip.Push(clipRect,
                 new Vector2(Mathf.Round(-scrollPosition.x - viewRect.x),
  ...
  public static void EndScrollView() { EndScrollView(true); }
  ...
  public static void EndScrollView(bool handleScrollWheel) {
    ...
    float deltaTime =
        Time.realtimeSinceStartup - state.previousTimeSinceStartup;
    state.previousTimeSinceStartup = Time.realtimeSinceStartup;
    // If touch scroll, then handle inertia
    if (Event.current.type == EventType.Repaint &&
        state.velocity != Vector2.zero) {
      for (int axis = 0; axis < 2; axis++) {
        state.velocity[axis] *= Mathf.Pow(
            0.1f, deltaTime); // Decrease in a timely fashion (~/10 per second)
        ...
        if (Mathf.Abs(state.velocity[axis]) < velocityToSubstract)
          ...
          needApply = true;
          // Reset the scrolling start info so that dragging works fine after
          // inertia
    ...
    // This is the mac way of handling things: if the mouse is over a
    // scrollview, the scrollview gets the event.
    if (handleScrollWheel &&
        ...
        && (state.viewRect.width > state.visibleRect.width ||
            state.viewRect.height > state.visibleRect.height)) {
      // Using scrollwheel
      if (Event.current.type == EventType.ScrollWheel
          // avoid eating scroll events if a scroll view is not necessary
          && ((state.viewRect.width > state.visibleRect.width &&
               !Mathf.Approximately(0f, Event.current.delta.x)) ||
              (state.viewRect.height > state.visibleRect.height &&
               !Mathf.Approximately(0f, Event.current.delta.y))) &&
          state.position.Contains(Event.current.mousePosition)) {
        state.scrollPosition.x =
            Mathf.Clamp(state.scrollPosition.x + (Event.current.delta.x * 20f),
                        0f, state.viewRect.width - state.visibleRect.width);
        state.scrollPosition.y =
            Mathf.Clamp(state.scrollPosition.y + (Event.current.delta.y * 20f),
                        0f, state.viewRect.height - state.visibleRect.height);
        Event.current.Use();

        needApply = true;
      }
      // Using touch
      else if (Event.current.type == EventType.TouchDown &&
        ...
        GUIUtility.hotControl = GUIUtility.GetControlID(
            s_ScrollviewHash, FocusType.Passive, state.position);
        ;
        Event.current.Use();
      } else if (state.isDuringTouchScroll &&
                 Event.current.type == EventType.TouchUp)
        ...
        state.scrollPosition.x =
            Mathf.Clamp(state.touchScrollStartPosition.x -
                            (Event.current.mousePosition.x -
                             state.touchScrollStartMousePosition.x),
                        0f, state.viewRect.width - state.visibleRect.width);
        state.scrollPosition.y =
            Mathf.Clamp(state.touchScrollStartPosition.y -
                            (Event.current.mousePosition.y -
                             state.touchScrollStartMousePosition.y),
                        0f, state.viewRect.height - state.visibleRect.height);
        Event.current.Use();

        // Sets the new volicity
        Vector2 newVelocity =
            (state.scrollPosition - previousPosition) / deltaTime;
        state.velocity =
            Vector2.Lerp(state.velocity, newVelocity, deltaTime * 10);

        needApply = true;
      }
    }
    if (needApply) {
      // If one of the visible rect dimensions is larger than the view rect
      // dimensions
  ...
  public static Rect Window(int id, Rect clientRect, WindowFunction func,
                            string text) {
    GUIUtility.CheckOnGUI();
    return DoWindow(id, clientRect, func, GUIContent.Temp(text),
  ...
  public static Rect Window(int id, Rect clientRect, WindowFunction func,
                            Texture image) {
    GUIUtility.CheckOnGUI();
    return DoWindow(id, clientRect, func, GUIContent.Temp(image),
                    GUI.skin.window, GUI.skin, true);
  }
  ...
  public static Rect Window(int id, Rect clientRect, WindowFunction func,
                            GUIContent content) {
    GUIUtility.CheckOnGUI();
    return DoWindow(id, clientRect, func, content, GUI.skin.window, GUI.skin,
                    true);
  }
  ...
  public static Rect Window(int id, Rect clientRect, WindowFunction func,
    ...
    GUIUtility.CheckOnGUI();
    return DoWindow(id, clientRect, func, GUIContent.Temp(text), style,
                    GUI.skin, true);
  }

  public static Rect Window(int id, Rect clientRect, WindowFunction func,
                            Texture image, GUIStyle style) {
    GUIUtility.CheckOnGUI();
    return DoWindow(id, clientRect, func, GUIContent.Temp(image), style,
                    GUI.skin, true);
  }
  ...
  public static Rect ModalWindow(int id, Rect clientRect, WindowFunction func,
                                 string text) {
    GUIUtility.CheckOnGUI();
    return DoModalWindow(id, clientRect, func, GUIContent.Temp(text),
                         GUI.skin.window, GUI.skin);
  }

  public static Rect ModalWindow(int id, Rect clientRect, WindowFunction func,
                                 Texture image) {
    GUIUtility.CheckOnGUI();
    return DoModalWindow(id, clientRect, func, GUIContent.Temp(image),
                         GUI.skin.window, GUI.skin);
  }
  ...
  public static Rect ModalWindow(int id, Rect clientRect, WindowFunction func,
                                 GUIContent content) {
    GUIUtility.CheckOnGUI();
    return DoModalWindow(id, clientRect, func, content, GUI.skin.window,
                         GUI.skin);
  }
  ...
  public static Rect ModalWindow(int id, Rect clientRect, WindowFunction func,
                                 string text, GUIStyle style) {
    GUIUtility.CheckOnGUI();
    return DoModalWindow(id, clientRect, func, GUIContent.Temp(text), style,
                         GUI.skin);
  }
  ...
  public static Rect ModalWindow(int id, Rect clientRect, WindowFunction func,
                                 Texture image, GUIStyle style) {
    GUIUtility.CheckOnGUI();
    return DoModalWindow(id, clientRect, func, GUIContent.Temp(image), style,
                         GUI.skin);
  }
  ...
  private static Rect DoWindow(int id, Rect clientRect, WindowFunction func,
                               GUIContent title, GUIStyle style, GUISkin skin,
                               bool forceRectOnLayout) {
    return Internal_DoWindow(id, GUIUtility.s_OriginalID, clientRect, func,
                             title, style, skin, forceRectOnLayout);
  }

  private static Rect DoModalWindow(int id, Rect clientRect,
                                    WindowFunction func, GUIContent content,
                                    GUIStyle style, GUISkin skin) {
    return Internal_DoModalWindow(id, GUIUtility.s_OriginalID, clientRect, func,
                                  content, style, skin);
  }

  [RequiredByNativeCode]
  internal static void
  CallWindowDelegate(WindowFunction func, int id, int instanceID, GUISkin _skin,
                     int forceRect, float width, float height, GUIStyle style) {
    GUILayoutUtility.SelectIDList(id, true);
    GUISkin temp = skin;
    if (Event.current.type == EventType.Layout) {
      if (forceRect != 0) {
        GUILayoutOption[] options = {GUILayout.Width(width),
                                     GUILayout.Height(height)};

        // Tell the GUILayout system we're starting a window, our style and our
        // size. Then layouting is just the same as anything else
        GUILayoutUtility.BeginWindow(id, style, options);
      } else {
        // If we don't want to force the rect (which is when we come from
        // GUILayout.window), don't pass in the fixedsize options
  ...
  internal static void BeginWindows(int skinMode, int editorWindowInstanceID) {
    ...
    UnityEngineInternal.GenericStack oldLayoutGroups =
        GUILayoutUtility.current.layoutGroups;
...
public partial class GUI {
  public abstract class Scope : IDisposable {
    ...
    ~Scope() {
      if (!m_Disposed && !GUIUtility.guiIsExiting)
        Console.WriteLine(
            $"{GetType().Name} was not disposed! You should use the 'using' keyword or manually call Dispose.");
      Dispose(false);
    }
  ...
  public class ScrollViewScope : Scope {
    ...
    public ScrollViewScope(Rect position, Vector2 scrollPosition,
                           Rect viewRect) {
    ...
    public ScrollViewScope(Rect position, Vector2 scrollPosition, Rect viewRect,
      ...
      handleScrollWheel = true;
      this.scrollPosition =
          BeginScrollView(position, scrollPosition, viewRect,
                          alwaysShowHorizontal, alwaysShowVertical);
    }
    ...
    internal ScrollViewScope(Rect position, Vector2 scrollPosition,
                             Rect viewRect, bool alwaysShowHorizontal,
                             bool alwaysShowVertical,
                             GUIStyle horizontalScrollbar,
  ...
  internal struct ColorScope : IDisposable {
    ...
    public ColorScope(float r, float g, float b, float a = 1.0f)
  ...
  internal struct BackgroundColorScope : IDisposable {
    ...
    public BackgroundColorScope(float r, float g, float b, float a = 1.0f)
        : this(new Color(r, g, b, a)) {}
