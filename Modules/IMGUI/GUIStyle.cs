...
public sealed partial class GUIStyleState {
  ...
  // If NULL, it means we own m_Ptr and need to delete it when this gets
  // disposed
  readonly GUIStyle m_SourceStyle;

  public GUIStyleState() { m_Ptr = Init(); }
  ...
  internal static GUIStyleState GetGUIStyleState(GUIStyle sourceStyle,
                                                 IntPtr source) {
    GUIStyleState newState = new GUIStyleState(sourceStyle, source);
    return newState;
  }
...
public sealed partial class GUIStyle {
  ...
  public GUIStyle() { m_Ptr = Internal_Create(this); }
  ...
  internal void InternalOnAfterDeserialize() {
    m_Normal = GUIStyleState.ProduceGUIStyleStateFromDeserialization(
        this, GetStyleStatePtr(0));
    m_Hover = GUIStyleState.ProduceGUIStyleStateFromDeserialization(
        this, GetStyleStatePtr(1));
    m_Active = GUIStyleState.ProduceGUIStyleStateFromDeserialization(
        this, GetStyleStatePtr(2));
    m_Focused = GUIStyleState.ProduceGUIStyleStateFromDeserialization(
        this, GetStyleStatePtr(3));
    m_OnNormal = GUIStyleState.ProduceGUIStyleStateFromDeserialization(
        this, GetStyleStatePtr(4));
    m_OnHover = GUIStyleState.ProduceGUIStyleStateFromDeserialization(
        this, GetStyleStatePtr(5));
    m_OnActive = GUIStyleState.ProduceGUIStyleStateFromDeserialization(
        this, GetStyleStatePtr(6));
    m_OnFocused = GUIStyleState.ProduceGUIStyleStateFromDeserialization(
        this, GetStyleStatePtr(7));
  }

  [NonSerialized]
  internal IntPtr m_Ptr;

  [NonSerialized] GUIStyleState m_Normal, m_Hover, m_Active, m_Focused,
      m_OnNormal, m_OnHover, m_OnActive, m_OnFocused;

  [NonSerialized] RectOffset m_Border, m_Padding, m_Margin, m_Overflow;

  [NonSerialized] string m_Name;
  ...
  public GUIStyleState normal {
    get {
      ...
      // since constructors can be called within and outside a serialization
      // operation So we delay the initialization here where we know we will be
      // on the main thread, outside any loading operation.
      return m_Normal ??(
  ...
  public GUIStyleState onNormal {
    get {
      return m_OnNormal ??(m_OnNormal = GUIStyleState.GetGUIStyleState(
                              this, GetStyleStatePtr(4)));
    }
    set { AssignStyleState(4, value.m_Ptr); }
  }

  // Rendering settings for when the control is turned on and the mouse is
  // hovering it.
  public GUIStyleState onHover {
    get {
      return m_OnHover ??(m_OnHover = GUIStyleState.GetGUIStyleState(
                             this, GetStyleStatePtr(5)));
    }
  ...
  public GUIStyleState onActive {
    get {
      return m_OnActive ??(m_OnActive = GUIStyleState.GetGUIStyleState(
                              this, GetStyleStatePtr(6)));
    }
  ...
  public GUIStyleState focused {
    get {
      return m_Focused ??(m_Focused = GUIStyleState.GetGUIStyleState(
                             this, GetStyleStatePtr(3)));
    }
  ...
  public GUIStyleState onFocused {
    get {
      return m_OnFocused ??(m_OnFocused = GUIStyleState.GetGUIStyleState(
                               this, GetStyleStatePtr(7)));
    }
  ...
  public RectOffset overflow {
    get {
      return m_Overflow ??(m_Overflow =
                              new RectOffset(this, GetRectOffsetPtr(3)));
    }
  ...
  public void Draw(Rect position, bool isHover, bool isActive, bool on,
                   bool hasKeyboardFocus) {
    Draw(position, GUIContent.none, -1, isHover, isActive, on,
         hasKeyboardFocus);
  }

  // Draw the GUIStyle with a text string inside.
  public void Draw(Rect position, string text, bool isHover, bool isActive,
                   bool on, bool hasKeyboardFocus) {
    Draw(position, GUIContent.Temp(text), -1, isHover, isActive, on,
         hasKeyboardFocus);
  }
  ...
  public void Draw(Rect position, Texture image, bool isHover, bool isActive,
                   bool on, bool hasKeyboardFocus) {
    Draw(position, GUIContent.Temp(image), -1, isHover, isActive, on,
         hasKeyboardFocus);
  }

  // Draw the GUIStyle with text and an image inside. If the image is too large
  // to fit within the content area of the style it is scaled down.
  public void Draw(Rect position, GUIContent content, bool isHover,
  ...
  public void Draw(Rect position, GUIContent content, int controlID, bool on,
                   bool hover) {
    Draw(position, content, controlID, hover,
         GUIUtility.hotControl == controlID, on,
         GUIUtility.HasKeyFocus(controlID));
  }

  private void Draw(Rect position, GUIContent content, int controlId,
                    bool isHover, bool isActive, bool on,
                    bool hasKeyboardFocus) {
    ...
    if (onDraw == null || !onDraw(this, position, content, drawStates)) {
      if (controlId == -1)
        Internal_Draw(position, content, isHover, isActive, on,
                      hasKeyboardFocus);
      else
  ...
  // The normal draw methods use MonoGUIContentToTempNative which means they all
  // share the same temp GUIContent on the native side. A native IMGUI control
  // such as GUIButton is already using this temp GUIContent when it calls
  // GetControlID, which, because of the delayed feature in PrefixLabel, can end
  // up calling a style draw function again to draw the PrefixLabel. This draw
  // call cannot use the same temp GUIContent that is already needed for the
  // GUIButton control itself, so it has to use this alternative code path that
  // uses a different GUIContent to store the content in. We can all agree this
  // workaround is not nice at all. But nobody seemed to be able to come up with
  // something better.
  internal void DrawPrefixLabel(Rect position, GUIContent content,
                                int controlID) {
    if (content != null) {
      var drawStates = new DrawStates(
          controlID, position.Contains(Event.current.mousePosition), false,
          false, GUIUtility.HasKeyFocus(controlID));
      if (onDraw == null || !onDraw(this, position, content, drawStates))
        Internal_DrawPrefixLabel(position, content, controlID, false);
    } else
      Debug.LogError(
          "Style.DrawPrefixLabel may not be called with GUIContent that is null.");
  }

  // Does the ID-based Draw function show keyboard focus? Disabled by windows
  // when they don't have keyboard focus
  internal static bool showKeyboardFocus = true;

  // Draw this GUIStyle with selected content.
  public void DrawCursor(Rect position, GUIContent content, int controlID,
                         int character) {
  ...
  internal void DrawWithTextSelection(Rect position, GUIContent content,
                                      bool isActive, bool hasKeyboardFocus,
                                      int firstSelectedCharacter,
                                      int lastSelectedCharacter,
                                      bool drawSelectionAsComposition,
    ...
    var drawStates =
        new DrawStates(-1, hovered, isActive, false, hasKeyboardFocus,
                       drawSelectionAsComposition, firstSelectedCharacter,
                       lastSelectedCharacter, cursorColor, selectionColor);
    if (onDraw == null || !onDraw(this, position, content, drawStates)) {
      Internal_DrawWithTextSelection(
          position, content, hovered, isActive, false, hasKeyboardFocus,
          drawSelectionAsComposition, firstSelectedCharacter,
          lastSelectedCharacter, cursorColor, selectionColor);
    }
  }
  ...
  internal void DrawWithTextSelection(Rect position, GUIContent content,
                                      int controlID, int firstSelectedCharacter,
                                      int lastSelectedCharacter,
                                      bool drawSelectionAsComposition) {
    DrawWithTextSelection(
        position, content, controlID == GUIUtility.hotControl,
        controlID == GUIUtility.keyboardControl && showKeyboardFocus,
        firstSelectedCharacter, lastSelectedCharacter,
        drawSelectionAsComposition, GUI.skin.settings.selectionColor);
  }

  // Draw this GUIStyle with selected content.
  public void DrawWithTextSelection(Rect position, GUIContent content,
                                    int controlID, int firstSelectedCharacter,
                                    int lastSelectedCharacter) {
    DrawWithTextSelection(position, content, controlID, firstSelectedCharacter,
                          lastSelectedCharacter, false);
  }
  ...
  public Vector2 GetCursorPixelPosition(Rect position, GUIContent content,
                                        int cursorStringIndex) {
    return Internal_GetCursorPixelPosition(position, content,
                                           cursorStringIndex);
  }

  // Get the cursor position (indexing into contents.text) when the user clicked
  // at cursorPixelPosition
  public int GetCursorStringIndex(Rect position, GUIContent content,
                                  Vector2 cursorPixelPosition) {
    return Internal_GetCursorStringIndex(position, content,
                                         cursorPixelPosition);
  }

  // Returns number of characters that can fit within width, returns -1 if fails
  // due to missing font
  ...
  // Calculate the size of a some content if it is rendered with this style.
  internal Vector2 CalcSizeWithConstraints(GUIContent content,
                                           Vector2 constraints) {
    return Internal_CalcSizeWithConstraints(content, constraints);
  }

  // Calculate the size of an element formatted with this style, and a given
  // space to content.
  public Vector2 CalcScreenSize(Vector2 contentSize) {
    return new Vector2(
        (fixedWidth != 0.0f
             ? fixedWidth
             : Mathf.Ceil(contentSize.x + padding.left + padding.right)),
        (fixedHeight != 0.0f
             ? fixedHeight
             : Mathf.Ceil(contentSize.y + padding.top + padding.bottom)));
  }

  // How tall this element will be when rendered with /content/ and a specific
  // /width/.
  ...
  public bool isHeightDependantOnWidth =>
      fixedHeight == 0 &&
      (wordWrap && imagePosition != ImagePosition.ImageOnly);

  // Calculate the minimum and maximum widths for this style rendered with
  // /content/.
  public void CalcMinMaxWidth(GUIContent content, out float minWidth,
                              out float maxWidth) {
...
// Different methods for how the GUI system handles text being too large to fit
// the rectangle allocated.
