...
internal class GUILayoutEntry {
  ...
  public GUILayoutEntry(float _minWidth, float _maxWidth, float _minHeight,
                        float _maxHeight, GUIStyle _style) {
    minWidth = _minWidth;
    maxWidth = _maxWidth;
  ...
  public virtual void ApplyOptions(GUILayoutOption[] options) {
    ...
    foreach (GUILayoutOption i in options) {
      ...
      case GUILayoutOption.Type.fixedWidth:
        minWidth = maxWidth = (float) i.value;
        stretchWidth = 0;
        break;
      case GUILayoutOption.Type.fixedHeight:
        minHeight = maxHeight = (float) i.value;
        stretchHeight = 0;
        break;
      case GUILayoutOption.Type.minWidth:
        minWidth = (float) i.value;
        if (maxWidth < minWidth)
          maxWidth = minWidth;
        break;
      case GUILayoutOption.Type.maxWidth:
        maxWidth = (float) i.value;
        if (minWidth > maxWidth)
          minWidth = maxWidth;
        stretchWidth = 0;
        break;
      case GUILayoutOption.Type.minHeight:
        minHeight = (float) i.value;
        if (maxHeight < minHeight)
          maxHeight = minHeight;
        break;
      case GUILayoutOption.Type.maxHeight:
        maxHeight = (float) i.value;
        if (minHeight > maxHeight)
          minHeight = maxHeight;
        stretchHeight = 0;
        break;
      case GUILayoutOption.Type.stretchWidth:
        stretchWidth = (int) i.value;
        break;
      case GUILayoutOption.Type.stretchHeight:
        stretchHeight = (int) i.value;
        break;
      }
    }
  ...
  public override string ToString() {
    ...
    return space +
           UnityString.Format(
               "{1}-{0} (x:{2}-{3}, y:{4}-{5})", style != null ? style.name
               : "NULL", GetType(), rect.x, rect.xMax, rect.y, rect.yMax) +
           "   -   W: " + minWidth + "-" + maxWidth +
           (stretchWidth != 0 ? "+" : "") + ", H: " + minHeight + "-" +
           maxHeight + (stretchHeight != 0 ? "+" : "");
  }
}

// Layouter that makes elements which sizes will always conform to a specific
// aspect ratio.
internal sealed class GUIAspectSizer : GUILayoutEntry {
  float aspect;

  public GUIAspectSizer(float aspect, GUILayoutOption[] options)
      : base(0, 0, 0, 0, GUIStyle.none) {
    this.aspect = aspect;
    ApplyOptions(options);
  }
...
internal sealed class GUIGridSizer : GUILayoutEntry {
  ...
  public static Rect GetRect(GUIContent[] contents, int xCount, GUIStyle style,
                             GUILayoutOption[] options) {
    Rect r = new Rect(0, 0, 0, 0);
    switch (Event.current.type) {
    case EventType.Layout:
      GUILayoutUtility.current.topLevel.Add(
          new GUIGridSizer(contents, xCount, style, options));
      break;
    case EventType.Used:
      return kDummyRect;
    default:
  ...
  private GUIGridSizer(GUIContent[] contents, int xCount, GUIStyle buttonStyle,
                       GUILayoutOption[] options)
      : base(0, 0, 0, 0, GUIStyle.none) {
    m_Count = contents.Length;
    m_XCount = xCount;

    // Most settings comes from the button style (can we stretch, etc). Hence, I
    // apply the style here
    ApplyStyleSettings(buttonStyle);

    // We can have custom options coming from userland. We apply this last so it
    // overrides
    ApplyOptions(options);
    ...
    float totalHorizSpacing =
        Mathf.Max(buttonStyle.margin.left, buttonStyle.margin.right) *
        (m_XCount - 1);
    //          Debug.Log (String.Format ("margins: {0}, {1}   totalHoriz: {2}",
    //          buttonStyle.margin.left, buttonStyle.margin.right,
    //          totalHorizSpacing));
    // internal horizontal margins
    float totalVerticalSpacing =
        Mathf.Max(buttonStyle.margin.top, buttonStyle.margin.bottom) *
        (rows - 1);

    // Handle fixedSize buttons
    if (buttonStyle.fixedWidth != 0)
      m_MinButtonWidth = m_MaxButtonWidth = buttonStyle.fixedWidth;
    //          Debug.Log ("buttonStyle.fixedHeight " +
    ...
    //          Debug.Log (String.Format ("minButtonWidth {0}, maxButtonWidth
    //          {1}, minButtonHeight {2}, maxButtonHeight{3}", minButtonWidth,
    //          maxButtonWidth, minButtonHeight, maxButtonHeight));

    // if anything is left unknown, we need to iterate over all elements and
    // figure out the sizes.
    if (m_MinButtonHeight == -1 || m_MaxButtonHeight == -1 ||
        m_MinButtonWidth == -1 || m_MaxButtonWidth == -1) {
      // figure out the max size. Since the buttons are in a grid, the max size
      // determines stuff.
      ...
      if (m_MinButtonWidth == -1) {
        // if the user has supplied a maxButtonWidth, the buttons can never get
        // larger.
      ...
      if (m_MaxButtonWidth == -1) {
        // if the user has supplied a minButtonWidth, the buttons can never get
        // smaller.
        if (m_MinButtonWidth != -1)
...
// Class that can handle word-wrap sizing. this is specialcased as setting width
// can make the text wordwrap, which would then increase height...
internal sealed class GUIWordWrapSizer : GUILayoutEntry {
  readonly GUIContent m_Content;
  // We need to differentiate between min & maxHeight we calculate for ourselves
  // and one that is forced by the user (When inside a scrollview, we can be
  // told to layout twice, so we need to know the difference)
  readonly float m_ForcedMinHeight;
  readonly float m_ForcedMaxHeight;

  public GUIWordWrapSizer(GUIStyle style, GUIContent content,
                          GUILayoutOption[] options)
      : base(0, 0, 0, 0, style) {
  ...
  public override void CalcWidth() {
    if (minWidth == 0 || maxWidth == 0) {
      ...
      // Further layout calculations might round this value to nearest which
      // could force the text over several lines when rounding down. Make sure
      // to actual give enough space by always rounding up (case 1047812).
  ...
  public override void CalcHeight() {
    // When inside a scrollview, this can get called twice (as vertical
    // scrollbar reduces width, which causes a reflow). Hence, we need to use
    // the separately cached values for min & maxHeight coming from the user...
