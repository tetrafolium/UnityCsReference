...
internal class GUILayoutGroup : GUILayoutEntry {
  ...
  public bool resetCoords = false; // Reset coordinate for GetRect. Used for
                                   // groups that are part of a window
  ...
  public int windowID = -1; // Optional window ID for toplevel windows. Used by
                            // Layout to tell GUI.Window of size changes...
  int m_Cursor = 0;
  ...
  // The summed sizes of the children. This is used to determine whether or not
  // the children should be stretched
  ...
  private static readonly GUILayoutEntry none =
      new GUILayoutEntry(0, 1, 0, 1, GUIStyle.none);
  ...
  public override void ApplyOptions(GUILayoutOption[] options) {
    ...
    foreach (GUILayoutOption i in options) {
      ...
      case GUILayoutOption.Type.maxHeight:
        ...
        break;
      // TODO:
      //              case GUILayoutOption.Type.alignStart:       align =
      //              Align.start; break; case GUILayoutOption.Type.alignMiddle:
      //              align = Align.middle; break; case
      //              GUILayoutOption.Type.alignEnd:     align = Align.end;
      //              break; case GUILayoutOption.Type.alignJustify:     align =
      //              Align.justify; break; case GUILayoutOption.Type.equalSize:
      //              equalSize = true; break;
  ...
  public Rect PeekNext() {
    ...
    if (Event.current.type == EventType.Repaint)
      throw new ArgumentException("Getting control " + m_Cursor +
                                  "'s position in a group with only " +
                                  entries.Count + " controls when doing " +
  ...
  public Rect GetLast() {
    ...
    if (Event.current.type == EventType.Repaint)
      Debug.LogError("Getting control " + m_Cursor +
                     "'s position in a group with only " + entries.Count +
                     " controls when doing " + Event.current.rawType);
    return kDummyRect;
  }

  public void Add(GUILayoutEntry e) { entries.Add(e); }
  ...
  public override void CalcWidth() {
    ...
    if (isVertical) {
      foreach (GUILayoutEntry i in entries) {
        ...
        if (i.consideredForMargin) {
          ...
          m_ChildMinWidth =
              Mathf.Max(i.minWidth + i.marginHorizontal, m_ChildMinWidth);
          m_ChildMaxWidth =
              Mathf.Max(i.maxWidth + i.marginHorizontal, m_ChildMaxWidth);
        }
        m_StretchableCountX += i.stretchWidth;
      }
      // Before, we added the margins to the width, now we want to suptract them
      // again.
      ...
      foreach (GUILayoutEntry i in entries) {
        ...
        if (i.consideredForMargin) {
          if (!first)
            ...
            // the first element's margins are handles _leftMarginMin and should
            // not be added to the children's sizes
            margin = 0;
            first = false;
          }
    ...
    if (style != GUIStyle.none || m_UserSpecifiedWidth) {
      ...
      // If we don't have a GUIStyle, we pop the min of margins outward from
      // children on to us.
    ...
    minWidth =
        Mathf.Max(minWidth, m_ChildMinWidth + leftPadding + rightPadding);

    if (maxWidth ==
        0) // if we don't have a max width, take the one that was calculated
    {
    ...
    // Finally, if our minimum width is greater than our maximum width, minWidth
    // wins
  ...
  public override void SetHorizontal(float x, float width) {
    ...
    if (isVertical) {
      // If we have a GUIStyle here, spacing from our edges to children are max
      // (our padding, their margins)
      if (style != GUIStyle.none) {
        foreach (GUILayoutEntry i in entries) {
          // NOTE: we can't use .horizontal here (As that could make things like
          // right button margin getting eaten by large left padding - so we
          // need to split up in left and right
          float leftMar = Mathf.Max(i.marginLeft, padding.left);
          float thisX = x + leftMar;
          float thisWidth =
              width - Mathf.Max(i.marginRight, padding.right) - leftMar;
          if (i.stretchWidth != 0)
            i.SetHorizontal(thisX, thisWidth);
          else
            i.SetHorizontal(thisX,
                            Mathf.Clamp(thisWidth, i.minWidth, i.maxWidth));
        }
      } else {
        // If not, PART of the subelements' margins have already been propagated
        // upwards to this group, so we need to subtract that  from what we
        // apply
        float thisX = x - marginLeft;
        float thisWidth = width + marginHorizontal;
        foreach (GUILayoutEntry i in entries) {
          if (i.stretchWidth != 0) {
            i.SetHorizontal(thisX + i.marginLeft,
                            thisWidth - i.marginHorizontal);
          } else
            i.SetHorizontal(thisX + i.marginLeft,
                            Mathf.Clamp(thisWidth - i.marginHorizontal,
                                        i.minWidth, i.maxWidth));
        }
      }
    } else { // we're horizontally laid out:
      // apply margins/padding here
      // If we have a style, adjust the sizing to take care of padding (if we
      // don't the horizontal margins have been propagated fully up the
      // hierarchy)...
      if (style != GUIStyle.none) {
        ...
        if (entries.Count != 0) {
          ...
          rightMar =
              Mathf.Max(rightMar, entries[entries.Count - 1].marginRight);
        }
        x += leftMar;
      ...
      if (m_ChildMinWidth != m_ChildMaxWidth)
        minMaxScale = Mathf.Clamp((widthToDistribute - m_ChildMinWidth) /
                                      (m_ChildMaxWidth - m_ChildMinWidth),
                                  0, 1);

      // Handle stretching
      float perItemStretch = 0;
      if (widthToDistribute > m_ChildMaxWidth) // If we have too much space, we
                                               // need to distribute it.
      {
        if (m_StretchableCountX > 0) {
          perItemStretch = (widthToDistribute - m_ChildMaxWidth) /
                           (float) m_StretchableCountX;
        }
      }
      ...
      //          Debug.Log ("" + x + ", " + width + "   perItemStretch:" +
      //          perItemStretch); Debug.Log ("MinMaxScale"+ minMaxScale);
      foreach (GUILayoutEntry i in entries) {
  ...
  public override void CalcHeight() {
    ...
    if (style != GUIStyle.none || m_UserSpecifiedHeight) {
      ...
      // If we don't have a GUIStyle, we bubble the margins outward from
      // children on to us.
      m_MarginTop = topMarginMin;
      m_MarginBottom = bottomMarginMin;
      firstPadding = lastPadding = 0;
    }
    // Debug.Log ("Margins: " + _topMarginMin + ", " + _bottomMarginMin + "
    // childHeights:" + childMinHeight + ", " + childMaxHeight);
    // If we have a specified minheight, take that into account...
    minHeight =
        Mathf.Max(minHeight, m_ChildMinHeight + firstPadding + lastPadding);

    if (maxHeight ==
        0) // if we don't have a max height, take the one that was calculated
    {
    ...
    // Finally, if out minimum height is greater than our maximum height,
    // minHeight wins
  ...
  public override void SetVertical(float y, float height) {
    ...
    if (isVertical) {
      // If we have a skin, adjust the sizing to take care of padding (if we
      // don't have a skin the vertical margins have been propagated fully up
      // the hierarchy)...
      if (style != GUIStyle.none) {
        ...
        if (entries.Count != 0) {
          ...
          bottomMar =
              Mathf.Max(bottomMar, entries[entries.Count - 1].marginBottom);
        }
        y += topMar;
      ...
      if (m_ChildMinHeight != m_ChildMaxHeight)
        minMaxScale = Mathf.Clamp((heightToDistribute - m_ChildMinHeight) /
                                      (m_ChildMaxHeight - m_ChildMinHeight),
                                  0, 1);

      // Handle stretching
      float perItemStretch = 0;
      if (heightToDistribute >
          m_ChildMaxHeight) // If we have too much space - stretch any
                            // stretchable children
      {
        if (m_StretchableCountY > 0)
          perItemStretch = (heightToDistribute - m_ChildMaxHeight) /
                           (float) m_StretchableCountY;
      }
      ...
      if (style != GUIStyle.none) {
        foreach (GUILayoutEntry i in entries) {
          ...
          float thisHeight =
              height - Mathf.Max(i.marginBottom, padding.bottom) - topMar;

          if (i.stretchHeight != 0)
            i.SetVertical(thisY, thisHeight);
          else
            i.SetVertical(thisY,
                          Mathf.Clamp(thisHeight, i.minHeight, i.maxHeight));
        }
      } else {
        // If not, the subelements' margins have already been propagated upwards
        // to this group, so we can safely ignore them
        ...
        foreach (GUILayoutEntry i in entries) {
          if (i.stretchHeight != 0)
            ...
            i.SetVertical(thisY + i.marginTop,
                          Mathf.Clamp(thisHeight - i.marginVertical,
                                      i.minHeight, i.maxHeight));
        }
      }
    }
  }
...
internal sealed class GUIScrollGroup : GUILayoutGroup {
  ...
  public override void CalcWidth() {
    // Save the size values & reset so we calc the sizes of children without any
    // contraints
    ...
    if (allowHorizontalScroll) {
      // Set an explicit small minWidth so it will correctly scroll when place
      // inside horizontal groups
      if (minWidth > 32)
        minWidth = 32;
  ...
  public override void SetHorizontal(float x, float width) {
    float _cWidth = needsVerticalScrollbar ? width -
                    verticalScrollbar.fixedWidth -
                    verticalScrollbar.margin.left : width;
    // if (allowVerticalScroll == false)
    //  Debug.Log ("width " + width);
    // If we get a vertical scrollbar, the width changes, so we need to do a
    // recalculation with the new width.
    if (allowHorizontalScroll && _cWidth < calcMinWidth) {
      ...
      // set the min and max width we calculated for the children so
      // SetHorizontal works correctly
  ...
  public override void CalcHeight() {
    // Save the values & reset so we calc the sizes of children without any
    // contraints
    ...
    // otherwise we get a vertical scrollbar and leftover space beneath the
    // scrollview.
  ...
  public override void SetVertical(float y, float height) {
    ...
    // Now we know how much height we have, and hence how much vertical space to
    // distribute. If we get a vertical scrollbar, the width changes, so we need
    // to do a recalculation with the new width.
    if (allowVerticalScroll && availableHeight < calcMinHeight) {
      // We're too small vertically, so we need a vertical scrollbar.
      // This means that we have less horizontal space, which can change the
      // vertical size.
      if (!needsHorizontalScrollbar && !needsVerticalScrollbar) {
        // Subtract scrollbar width from the size...
        clientWidth = rect.width - verticalScrollbar.fixedWidth -
        ...
        // This can have caused a reflow, so we need to recalclate from here on
        // down (we already know we need a vertical scrollbar, so this size
        // change cannot bubble upwards.
        CalcHeight();

        rect.width = outsideWidth;
      }

      // set the min and max height we calculated for the children so
      // SetVertical works correctly
