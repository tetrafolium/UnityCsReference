...
internal class ScrollViewState {
  ...
  public void ScrollTo(Rect pos) { ScrollTowards(pos, Mathf.Infinity); }
  ...
  private Vector2 ScrollNeeded(Rect pos) {
    ...
    actualViewRect.height =
        Mathf.Max(actualViewRect.height, visibleRect.height);
    scrollVector.x =
        Mathf.Clamp(scrollVector.x, actualViewRect.xMin - scrollPosition.x,
                    actualViewRect.xMax - visibleRect.width - scrollPosition.x);
    scrollVector.y = Mathf.Clamp(
        scrollVector.y, actualViewRect.yMin - scrollPosition.y,
        actualViewRect.yMax - visibleRect.height - scrollPosition.y);
