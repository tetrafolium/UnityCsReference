...
internal enum EventType {
	UmpProfilerOpenPlayerConnection, // Profiler > Editor: Sent when the
	                                 // profiler OOP is ready and wants the
	                                 // master editor process to enable its
	                                 // profile and player connection
	UmpProfilerRecordingStateChanged, // Profiler > Editor: Sent from the
	                                  // profiler OOP to have the master editor
	                                  // synchronize recording state changes in
	                                  // OOPP.
	UmpProfilerDeepProfileChanged, // Profiler > Editor: Sent from the profiler
	                               // OOP to have the master synchronize deep
	                               // profile state and reload scripts.
	UmpProfilerMemRecordModeChanged, // Profiler > Editor: Sent from the
	                                 // profiler OOP to have the master
	                                 // synchronize memory record mode.
	UmpProfilerCurrentFrameChanged, // Profiler > Editor: The OOPP notifies the
	                                // master editor that the user has selected
	                                // a specific frame in the profiler chart.
	UmpProfilerRecordToggle, // Editor > Profiler: The master editor requests
	                         // the OOPP to start or end recording (i.e. used
	                         // with F9)
	UmpProfilerAboutToQuit, // Editor > Profiler: The master editor notifies the
	                        // OOPP that he needs to quit/exit.
	UmpProfilerExit, // Editor > Profiler: Request the OOPP to exit (used when
	                 // the main editor is about to quit)
	UmpProfilerPing, // Used for regression testing.
	UmpProfilerRequestRecordState, // Used for regression testing.
}

[Serializable] struct PlayerConnectionInfo {
	public bool recording;
	public bool profileEditor;
}

// Represents the Profiler (slave) Out-Of-Process is launched by the
// MainEditorProcess and connects to its EventServer at startup.
static class ProfilerProcess {
static bool s_ProfilerDriverSetup = false;
static ProfilerWindow s_SlaveProfilerWindow;
static string userPrefProfilerLayoutPath = Path.Combine(
	WindowLayout.layoutsDefaultModePreferencesPath, "Profiler.dwlt");
static string systemProfilerLayoutPath =
	Path.Combine(EditorApplication.applicationContentsPath,
	             "Resources/Layouts/Profiler.dwlt");
...
static void InitializeProfilerSlaveProcess() {
	if (!File.Exists(userPrefProfilerLayoutPath) ||
	    File.GetLastWriteTime(systemProfilerLayoutPath) >
	    File.GetLastWriteTime(userPrefProfilerLayoutPath)) {
...
static void InitializeProfilerSlaveProcessDomain() {
	Console.WriteLine(
		"[UMPE] Initialize Profiler Slave Process Domain Triggered");
	...
	EventService.RegisterEventHandler(
		nameof(EventType.UmpProfilerRecordToggle), HandleToggleRecording);
	EventService.RegisterEventHandler(
		nameof(EventType.UmpProfilerRequestRecordState),
		HandleRequestRecordState);
	EventService.RegisterEventHandler(nameof(EventType.UmpProfilerPing),
	                                  HandlePingEvent);
	EventService.RegisterEventHandler(nameof(EventType.UmpProfilerExit),
	                                  HandleExitEvent);
	...
	Console.WriteLine(
		"[UMPE] Initialize Profiler Slave Process Domain Completed");
}
...
static void SetupProfilerWindow(ProfilerWindow profilerWindow) {
	...
	profilerWindow.recordingStateChanged -=
		OnProfilerWindowRecordingStateChanged;
	profilerWindow.deepProfileChanged -= OnProfilerWindowDeepProfileChanged;
	profilerWindow.memoryRecordingModeChanged -=
		OnProfilerWindowMemoryRecordModeChanged;
...
static void SetupProfilerDriver() {
	...
	ProfilerDriver.profileEditor = ProfilerUserSettings.defaultTargetMode ==
	...
	if (!SessionState.GetBool("OOPP.PlayerConnectionOpened", false))
		EventService.Request(nameof(EventType.UmpProfilerOpenPlayerConnection),
		                     HandlePlayerConnectionOpened, playerConnectionInfo,
		                     5000L);
...
static void SetupProfiledConnection(int connId) {
	ProfilerDriver.connectedProfiler =
		ProfilerDriver.GetAvailableProfilers().FirstOrDefault(id => id ==
		                                                      connId);
...
static void SetProfilerWindowTitle(ApplicationTitleDescriptor desc) {
	...
	if (s_SlaveProfilerWindow.IsRecording()) {
		...
		desc.title =
			$"Profiler [RECORDING {playStateHint}] - {desc.projectName} - {desc.unityVersion}";
	} else
...
static void OnProfilerWindowRecordingStateChanged(bool recording) {
	EventService.Emit(nameof(EventType.UmpProfilerRecordingStateChanged),
	                  new object[] {recording, ProfilerDriver.profileEditor});
...
static void OnProfilerWindowDeepProfileChanged(bool deepProfiling) {
	EventService.Request(
		nameof(EventType.UmpProfilerDeepProfileChanged), (err, results) => {
					if (err != null)
						throw err;
					...
					Menu.SetChecked("Edit/Deep Profiling",
					                ProfilerDriver.deepProfiling);
				}, deepProfiling);
}

static void OnProfilerWindowMemoryRecordModeChanged(
	ProfilerMemoryRecordMode memRecordMode) {
	EventService.Emit(nameof(EventType.UmpProfilerMemRecordModeChanged),
	                  (int) memRecordMode);
}
...
static void OnProfilerCurrentFrameChanged(int frame, bool paused) {
	if (frame == -1)
		return;
	EventService.Emit(nameof(EventType.UmpProfilerCurrentFrameChanged),
	                  new object[] {frame, paused});
}
...
static object HandleExitEvent(string type, object[] args) {
	EditorApplication.delayCall += () =>
	                               EditorApplication.Exit(Convert.ToInt32(args[0]));
	return null;
}
...
[UsedImplicitly,
 CommandHandler("Profiler/OpenProfileData", CommandHint.Menu)]
static void OnLoadProfileDataFileCommand(CommandExecuteContext ctx) {
	s_SlaveProfilerWindow.LoadProfilingData(false);
}

[UsedImplicitly,
 CommandHandler("Profiler/SaveProfileData", CommandHint.Menu)]
...
static void OnRecordCommand(CommandExecuteContext ctx) {
	s_SlaveProfilerWindow.SetRecordingEnabled(
		!s_SlaveProfilerWindow.IsSetToRecord());
	Menu.SetChecked("Edit/Record", s_SlaveProfilerWindow.IsSetToRecord());
}

[UsedImplicitly,
 CommandHandler("Profiler/EnableDeepProfiling", CommandHint.Menu)]
...
static void InitializeProfilerMasterProcess() {
	...
	EventService.RegisterEventHandler(
		nameof(EventType.UmpProfilerAboutToQuit), OnProfilerExited);
	EventService.RegisterEventHandler(
		nameof(EventType.UmpProfilerOpenPlayerConnection),
		OnOpenPlayerConnectionRequested);
	EventService.RegisterEventHandler(
		nameof(EventType.UmpProfilerCurrentFrameChanged),
		OnProfilerCurrentFrameChanged);
	EventService.RegisterEventHandler(
		nameof(EventType.UmpProfilerRecordingStateChanged),
		OnProfilerRecordingStateChanged);
	EventService.RegisterEventHandler(
		nameof(EventType.UmpProfilerDeepProfileChanged),
		OnProfilerDeepProfileChanged);
	EventService.RegisterEventHandler(
		nameof(EventType.UmpProfilerMemRecordModeChanged),
		OnProfilerMemoryRecordModeChanged);
}

[UsedImplicitly,
 CommandHandler("ProfilerRecordToggle", CommandHint.Shortcut)]
static void RecordToggle(CommandExecuteContext context) {
	if (ProcessService.level == ProcessLevel.Master &&
	    ...
	    ProcessService.GetSlaveProcessState(s_SlaveProcessId) ==
	    ProcessState.Running &&
	    EventService.isConnected) {
		EventService.Request(
			nameof(EventType.UmpProfilerRecordToggle), (err, args) => {
...
static object OnProfilerCurrentFrameChanged(string eventType,
                                            object[] args) {
...
static object OnProfilerRecordingStateChanged(string eventType,
                                              object[] args) {
...
static object OnProfilerDeepProfileChanged(string eventType,
                                           object[] args) {
...
static object OnProfilerMemoryRecordModeChanged(string eventType,
                                                object[] args) {
	ProfilerDriver.memoryRecordMode =
		(ProfilerMemoryRecordMode) Convert.ToInt32(args[0]);
	return null;
}
...
static object OnOpenPlayerConnectionRequested(string eventType,
                                              object[] args) {
	var info = (PlayerConnectionInfo) args[0];
	var connectionId =
		ProcessService.EnableProfileConnection(Application.dataPath);
...
internal static bool IsRunning() {
	if (s_SlaveProcessId == -1)
		return false;
	return ProcessService.GetSlaveProcessState(s_SlaveProcessId) ==
	       ProcessState.Running;
}
...
internal static int LaunchProfilerSlave() {
	if (IsRunning()) {
		Debug.LogWarning(
			$"You've already launched the profiler out-of-process ({s_SlaveProcessId}), please wait a few seconds...");
		return s_SlaveProcessId;
	}

	const string umpCap = "ump-cap";
	const string umpWindowTitleSwitch = "ump-window-title";
	s_SlaveProcessId = ProcessService.LaunchSlave(
		k_RoleName, umpWindowTitleSwitch, "Profiler", umpCap,
		"disable-extra-resources", umpCap, "menu_bar", "editor-mode",
		k_RoleName, "disableManagedDebugger", "true");
	return s_SlaveProcessId;
}
}
}
