...
internal class Chart {
  ...
  private static readonly Color s_OverlayBackgroundDimFactor =
      new Color(0.9f, 0.9f, 0.9f, 0.4f);
  protected Action<bool> onDoSeriesToggle;
  string m_ChartSettingsName;
  int m_chartControlID;

  public void LoadAndBindSettings(string chartSettingsName,
                                  ChartViewData cdata) {
  ...
  static class Styles {
    ...
    public static readonly GUIStyle noDataOverlayBox =
    ...
    public static readonly Color noDataSeparatorColor =
        new Color(.8f, .8f, .8f, 0.5f);
  }
  ...
  public virtual void Close() { closed(this); }

  private int MoveSelectedFrame(int selectedFrame, ChartViewData cdata,
                                int direction) {
    int length = cdata.GetDataDomainLength();
    int newSelectedFrame = selectedFrame + direction;
    if (newSelectedFrame < cdata.firstSelectableFrame ||
        newSelectedFrame > cdata.chartDomainOffset + length)
      return selectedFrame;

    return newSelectedFrame;
  }

  private int DoFrameSelectionDrag(float x, Rect r, ChartViewData cdata,
                                   int len) {
    int frame = Mathf.RoundToInt((x - r.x) / r.width * len - 0.5f);
    GUI.changed = true;
    return Mathf.Clamp(frame + cdata.chartDomainOffset,
                       cdata.firstSelectableFrame,
                       cdata.chartDomainOffset + len);
  }

  private int HandleFrameSelectionEvents(int selectedFrame, int chartControlID,
                                         Rect chartFrame, ChartViewData cdata) {
    ...
    case EventType.MouseDown:
      if (chartFrame.Contains(evt.mousePosition)) {
        ...
        selectedFrame =
            DoFrameSelectionDrag(evt.mousePosition.x, chartFrame, cdata, len);
        evt.Use();
      }
      break;
  ...
  protected virtual void DoLegendGUI(Rect position, ChartType type,
                                     ChartViewData cdata, EventType evtType,
                                     bool active) {
    if (Event.current.type == EventType.Repaint)
      Styles.legendBackground.Draw(position, GUIContent.none, false, false,
                                   active, false);
    ...
    // Leave space for the GPU Profiler's Warning Icon, 16 pixels wide, 2 pixels
    // margin = 20 pixels. Without this spacer, the tooltip of the header would
    // be displayed instead of the one for the Warning Icon.
    headerRect.xMax -= 20;
  ...
  public int DoGUI(ChartType type, int selectedFrame, ChartViewData cdata,
                   bool active) {
    if (cdata == null)
      return selectedFrame;
    ...
    var chartHeight = GUILayout.MinHeight(
        Math.Max(kLabelOffset + ((cdata.numSeries + 1) * kLabelHeight) +
                     kDistFromTopToFirstLabel,
                 kChartMinHeight));
    Rect chartRect = GUILayoutUtility.GetRect(GUIContent.none,
                                              Styles.background, chartHeight);
    ...
    if (evtType == EventType.MouseDown &&
        chartRect.Contains(evt.mousePosition) && selected != null)
      ChartSelected();

    // if we are not dragging labels, handle graph frame selection
    if (m_DragItemIndex == -1)
      selectedFrame =
          HandleFrameSelectionEvents(selectedFrame, m_chartControlID, r, cdata);
    ...
    if (evt.type == EventType.Repaint &&
        GUIClip.visibleRect.Overlaps(chartRect)) {
  ...
  public void ChartSelected() { selected(this); }

  private void DrawSelectedFrame(int selectedFrame, ChartViewData cdata,
                                 Rect r) {
    if (cdata.firstSelectableFrame != -1 &&
        selectedFrame - cdata.firstSelectableFrame >= 0)
      DrawVerticalLine(selectedFrame, cdata, r, Styles.selectedFrameColor,
                       1.0f);
  }

  internal static void DrawVerticalLine(int frame, ChartViewData cdata, Rect r,
                                        Color color, float minWidth,
                                        float maxWidth = 0) {
    if (Event.current.type != EventType.Repaint)
      return;
  ...
  private void DrawMaxValueScale(ChartViewData cdata, Rect r) {
    Handles.Label(new Vector3(r.x + r.width / 2 - 20, r.yMin + 2, 0),
                  "Scale: " + cdata.maxValue);
  }
  ...
  void DrawOverlayBoxes(ChartViewData cdata, Rect r, bool chartActive) {
    ...
    if (Event.current.type == EventType.Repaint &&
      ...
      for (int frame = 0; frame < frameDataLength; frame++) {
        bool hasDataForFrame =
            (cdata.dataAvailable[frame] & ChartViewData.dataAvailableBit) ==
            ChartViewData.dataAvailableBit;
        if (hasDataForFrame) {
          if (lastFrameBeforeStatisticsAvailabilityChanged < frame - 1) {
            DrawOverlayBox(r, lastFrameBeforeStatisticsAvailabilityChanged,
                           frame, frameDataLength, chartActive,
                           Styles.noDataOverlayBox, lastStatisticsState);
          }
          ...
          // Not bitwise comparison because this here just checks that the
          // previous frame didn't just contain normal available data
          if (lastStatisticsState != ChartViewData.dataAvailableBit) {
            // a new reason for missing data has started here, flush out the old
            // one
            DrawOverlayBox(r, lastFrameBeforeStatisticsAvailabilityChanged,
                           frame, frameDataLength, chartActive,
                           Styles.noDataOverlayBox, lastStatisticsState);
            m_cachedFramesWithSeparatorLines.Add(frame +
                                                 cdata.chartDomainOffset);
          }
          lastFrameBeforeStatisticsAvailabilityChanged = frame;
      ...
      if (lastFrameBeforeStatisticsAvailabilityChanged < frameDataLength - 1) {
        DrawOverlayBox(r, lastFrameBeforeStatisticsAvailabilityChanged,
                       frameDataLength - 1, frameDataLength, chartActive,
                       Styles.noDataOverlayBox, lastStatisticsState);
      }
    }
  ...
  void DrawOverlayBox(Rect r, int startFrame, int endFrame, int frameDataLength,
                      bool chartActive, GUIStyle style,
                      int statisticsAvailabilityState) {
    float gracePixels = -1;
    float domainSize = frameDataLength - 1;
    float startXOffest =
        Mathf.RoundToInt(r.width * startFrame / domainSize) + gracePixels;
    float endXOffest =
        Mathf.RoundToInt(r.width * endFrame / domainSize) - gracePixels;
    Rect noDataRect = r;
    noDataRect.x += Mathf.Max(startXOffest, 0);
    noDataRect.width =
        Mathf.Min(endXOffest - startXOffest, r.width - (noDataRect.x - r.x));
    ...
    if (!string.IsNullOrEmpty(message)) {
      ...
      var position = new Vector2(noDataRect.width / 2 - size.x / 2,
                                 noDataRect.height / 2 - size.y / 2);
  ...
  private void DrawChartStacked(int selectedFrame, ChartViewData cdata, Rect r,
    ...
    if (selectedName.Length > 0) {
      ...
      GUIContent content =
          EditorGUIUtility.TempContent("Selected: " + selectedName);
      Vector2 size = EditorStyles.whiteBoldLabel.CalcSize(content);
      EditorGUI.DropShadowLabel(
          new Rect(r.x + r.width - size.x - 3.0f, r.y + 3.0f, size.x, size.y),
          content, Styles.selectedLabel);
    }
  }
  ...
  internal static void DoLabel(float x, float y, string text, float alignment) {
    ...
    EditorGUI.DoDropShadowLabel(r, content, Styles.whiteLabel,
                                Styles.labelDropShadowOpacity);
  }

  private void DrawGridStacked(Rect r, ChartViewData cdata) {
    if (Event.current.type != EventType.Repaint || cdata.grid == null ||
        cdata.gridLabels == null)
      return;

    GL.Begin(GL.LINES);
    GL.Color(new Color(1, 1, 1, 0.2f));
    float rangeScale =
        cdata.series[0].rangeAxis.sqrMagnitude == 0f
            ? 0f
            : 1f / (cdata.series[0].rangeAxis.y - cdata.series[0].rangeAxis.x) *
                  r.height;
    float rectBottom = r.y + r.height;
    for (int i = 0; i < cdata.grid.Length; ++i) {
      float y = rectBottom -
                (cdata.grid[i] - cdata.series[0].rangeAxis.x) * rangeScale;
      if (y > r.y) {
  ...
  private readonly List<LabelLayoutData> m_LabelData =
      new List<LabelLayoutData>(16);
  ...
  private void DrawLabels(Rect chartPosition, ChartViewData data,
                          int selectedFrame, ChartType chartType) {
    ...
    if (selectedFrame < data.firstSelectableFrame ||
    ...
    var selectedFrameMidline =
        chartPosition.x +
        chartPosition.width * ((selectedIndex + 0.5f) / (domain.y - domain.x));
    ...
    for (int s = 0; s < data.numSeries; ++s) {
      ...
      if (chartData.enabled && value >= labelRange.x && value <= labelRange.y) {
        ...
        var rangeSize = rangeAxis.sqrMagnitude == 0f
                            ? 1f
                            : rangeAxis.y * chartData.yScale - rangeAxis.x;
        ...
        if (stacked) {
          ...
          for (int i = currentChartIndex - 1; i >= 0; --i) {
            ...
            var otherChartData =
                data.hasOverlay ? data.overlays[otherSeriesIdx]
                : data.series[otherSeriesIdx];
            bool enabled = data.series[otherSeriesIdx].enabled;

            if (enabled) {
              accumulatedValues +=
                  otherChartData.yValues[selectedIndex] * otherChartData.yScale;
            }
          }
        ...
        var position = new Vector2(
            // offset by half a point so there is a 1-point gap down the midline
            // if labels are on both sides
            selectedFrameMidline + 0.5f,
            chartPosition.y +
                chartPosition.height *
                    (1.0f -
                     ((value * chartData.yScale - rangeAxis.x) / rangeSize)));
        var size = Styles.whiteLabel.CalcSize(
            EditorGUIUtility.TempContent(data.selectedLabels[s]));
        position.y -= 0.5f * size.y;
        position.y = Mathf.Clamp(position.y, chartPosition.yMin,
                                 chartPosition.yMax - size.y);
    ...
    // right align labels to the selected frame midline if approaching right
    // border
    if (selectedFrameMidline >
        chartPosition.x + chartPosition.width - maxLabelWidth) {
      for (int s = 0; s < data.numSeries; ++s) {
    ...
    for (int it = 0; it < k_LabelLayoutMaxIterations; ++it) {
      ...
      for (int s1 = 0; s1 < data.numSeries; ++s1) {
        ...
        for (int s2 = 0; s2 < data.numSeries; ++s2) {
          ...
          if (s1 != s2 &&
              m_LabelData[s1].position.Overlaps(m_LabelData[s2].position))
      ...
      float totalHeight;
      var geometricCenter = GetGeometricCenter(m_MostOverlappingLabels,
                                               m_LabelData, out totalHeight);
      ...
      while (foundOverlaps) {
        ...
        for (int s = 0; s < data.numSeries; ++s) {
          ...
          var x = testRect.xMax < selectedFrameMidline ? testRect.xMax
              : testRect.xMin;
          if (testRect.Contains(new Vector2(x, minY)) ||
        ...
        GetGeometricCenter(m_MostOverlappingLabels, m_LabelData,
                           out totalHeight);
      ...
      // separate overlapping rects and distribute them away from their
      // geometric center
      ...
      for (int i = 0; i < m_MostOverlappingLabels.Count; ++i) {
        ...
        var label = m_LabelData[labelIndex];
        label.position.y =
            geometricCenter - totalHeight * 0.5f + heightAllotted;
        m_LabelData[labelIndex] = label;
    ...
    for (int s = 0; s < data.numSeries; ++s) {
      ...
      GUI.contentColor = Color.Lerp(data.series[labelIndex].color, Color.white,
                                    Styles.labelLerpToWhiteAmount);
      ...
      EditorGUI.DoDropShadowLabel(
          ...
          Styles.whiteLabel, Styles.labelDropShadowOpacity);
    }
    GUI.contentColor = oldContentColor;
  }

  private int SortLineLabelIndices(int index1, int index2) {
    return -m_LabelData[index1].desiredYPosition.CompareTo(
        m_LabelData[index2].desiredYPosition);
  }

  private int SortOverlappingRectIndices(int index1, int index2) {
    return -m_LabelOrder.IndexOf(index1).CompareTo(
        m_LabelOrder.IndexOf(index2));
  }

  private float GetGeometricCenter(List<int> overlappingRects,
                                   List<LabelLayoutData> labelData,
  ...
  private void DrawChartItemLine(Rect r, ChartViewData cdata, int index) {
    ...
    if (m_LineDrawingPoints == null ||
        series.numDataPoints > m_LineDrawingPoints.Length)
    ...
    float rangeScale = cdata.series[index].rangeAxis.sqrMagnitude == 0f
                           ? 0f
                           : 1f /
                                 (cdata.series[index].rangeAxis.y -
                                  cdata.series[index].rangeAxis.x) *
                                 r.height;
    float rectBottom = r.y + r.height;
    ...
    for (int i = 0; i < series.numDataPoints; ++i) {
      ...
      if (yValue == -1f) {
        ...
        // once we started drawing the points in the line, we have to keep
        // going, otherwise the line will interpolate over frames with missing
        // data, leading to misleading visuals. alternatively we'd have to split
        // the line here, which might be better. consider splitting in a future
        // version
        yValue = 0;
      }
      ...
      if (!passedFirstValidValue) {
        ...
        // first valid point found. Set all skipped points in the line point
        // array to the current one, the line starts here
        for (int k = 0; k < i; k++) {
          m_LineDrawingPoints [k]
              .Set((series.xValues[i] - domain.x + 0.5f) * domainScale + r.x,
                   rectBottom -
                       (Mathf.Clamp(yValue, graphRange.x, graphRange.y) -
                        series.rangeAxis.x) *
                           rangeScale,
                   0f);
        }
      }
      m_LineDrawingPoints [i]
          .Set((series.xValues[i] - domain.x + 0.5f) * domainScale + r.x,
               rectBottom - (Mathf.Clamp(yValue, graphRange.x, graphRange.y) -
                             series.rangeAxis.x) *
                                rangeScale,
               0f);
    }
    ...
    using(new Handles.DrawingScope(cdata.series[index].color))
        Handles.DrawAAPolyLine(k_LineWidth, series.numDataPoints,
                               m_LineDrawingPoints);
  }

  private void DrawChartItemStacked(Rect r, int index, ChartViewData cdata,
                                    float[] stackedSampleSums) {
    int numSamples = cdata.GetDataDomainLength();
    ...
    for (int i = 0; i < numSamples; i++, x += step) {
      ...
      if (value == -1f) {
        ...
        // once we started drawing the vertices, we have to keep going,
        // otherwise the mesh will interpolate over frames with missing data,
  ...
  private void DrawChartItemStackedOverlay(Rect r, int index,
                                           ChartViewData cdata,
                                           float[] stackedSampleSums) {
    int numSamples = cdata.GetDataDomainLength();
    float step = r.width / numSamples;
  ...
  protected virtual Rect DoSeriesList(Rect position, int chartControlID,
                                      ChartType chartType,
                                      ChartViewData cdata) {
    ...
    if (m_DragItemIndex != -1) {
      ...
      case EventType.KeyDown:
        if (evt.keyCode == KeyCode.Escape) {
          GUIUtility.hotControl = 0;
          System.Array.Copy(m_OldChartOrder, cdata.order,
                            m_OldChartOrder.Length);
          m_DragItemIndex = -1;
          evt.Use();
        }
        break;
    ...
    for (int i = cdata.numSeries - 1; i >= 0; --i) {
      ...
      GUIContent elementLabel =
          EditorGUIUtility.TempContent(cdata.series[orderIdx].name);
      elementPosition.height =
          Styles.seriesLabel.CalcHeight(elementLabel, elementPosition.width);
      ...
      if (chartType == ChartType.StackedFill) {
        ...
        dragHandlePosition.xMin =
            dragHandlePosition.xMax - elementPosition.height;
        switch (eventType) {
        case EventType.Repaint:
          // Properly center the drag handle vertically
          Rect dragHandleRenderedPosition = dragHandlePosition;
          dragHandleRenderedPosition.height =
              Styles.seriesDragHandle.fixedHeight;
          dragHandleRenderedPosition.y +=
              (dragHandlePosition.height - dragHandleRenderedPosition.height) /
              2;
          Styles.seriesDragHandle.Draw(dragHandleRenderedPosition, false, false,
                                       false, false);
          break;
        case EventType.MouseDown:
          if (dragHandlePosition.Contains(mousePosition)) {
            ...
            System.Array.Copy(cdata.order, m_OldChartOrder,
                              m_OldChartOrder.Length);
        ...
        case EventType.MouseDrag:
          if (i == m_DragItemIndex) {
            ...
            if (moveDn || moveUp) {
              ...
              int targetIdx =
                  moveUp ? Mathf.Min(cdata.numSeries - 1, i + 1)
                  : Mathf.Max(0, i - 1);
      ...
      DoSeriesToggle(controlPosition, elementLabel,
                     ref cdata.series[orderIdx].enabled,
                     cdata.series[orderIdx].color, cdata);
  ...
  protected void DoSeriesToggle(Rect position, GUIContent label,
                                ref bool enabled, Color color,
                                ChartViewData cdata) {
...
internal class ChartViewData {
  ...
  public void Assign(ChartSeriesViewData[] series, int firstFrame,
                     int firstSelectableFrame) {
    this.series = series;
  ...
  public Vector2 GetDataDomain() {
    // TODO: this currently assumes data points are in order and first series
    ...
    for (int i = 0; i < numSeries; ++i) {
      ...
      result.y =
          Mathf.Max(result.y, series[i].xValues[series[i].numDataPoints - 1]);
    }
    return result;
  }
  ...
  public int GetDataDomainLength() {
    ...
    // the domain is a range of indices, logically starting at 0. The Length is
    // therefore the (lastIndex - firstIndex + 1)
