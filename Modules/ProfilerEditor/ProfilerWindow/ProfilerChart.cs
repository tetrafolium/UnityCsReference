...
private static readonly GUIContent performanceWarning = new GUIContent(
	"", EditorGUIUtility.LoadIcon("console.warnicon.sml"),
	L10n.Tr(
		"Collecting GPU Profiler data might have overhead. Close graph if you don't need its data"));
...
// For some charts, every line is scaled individually, so every data series
// gets their own range based on their own max scale. For charts that share
// their scale (like the Networking charts) all series get adjusted to the
// total max of the chart. Shared scale is only used for line charts and
// should only be used when every series shares the same data unit.
...
public ProfilerChart(ProfilerArea area, Chart.ChartType type, float dataScale,
                     int seriesCount)
	: base() {
	...
	if (s_LocalizedChartNames == null) {
		s_LocalizedChartNames = new string[] {
			LocalizationDatabase.GetLocalizedString(
				"CPU Usage|Graph out the various CPU areas"),
			LocalizationDatabase.GetLocalizedString(
				"GPU Usage|Graph out the various GPU areas"),
			LocalizationDatabase.GetLocalizedString("Rendering"),
			LocalizationDatabase.GetLocalizedString(
				"Memory|Graph out the various memory usage areas"),
			...
			LocalizationDatabase.GetLocalizedString(
				"Global Illumination|Graph of the Precomputed Realtime Global Illumination system resource usage."),
		};
	}
	UnityEngine.Debug.Assert(s_LocalizedChartNames.Length ==
	                         Profiler.areaCount);
	return s_LocalizedChartNames[(int) m_Area];
}

protected override void DoLegendGUI(Rect position, ChartType type,
	...
	if (m_Area == ProfilerArea.GPU) {
		...
		GUI.Label(new Rect(position.xMax - width - rightMmargin,
		                   position.y + topMargin, width,
		                   iconSize + padding.vertical),
		          performanceWarning);
	}
}
...
public virtual int DoChartGUI(int currentFrame, bool active) {
	if (Event.current.type == EventType.Repaint) {
		...
		for (int s = 0; s < m_Series.Length; s++) {
			string name =
				m_Data.hasOverlay ? "Selected" + m_Series[s].name : m_Series[s].name;
			labels[s] =
				ProfilerDriver.GetFormattedCounterValue(currentFrame, m_Area, name);
		}
	...
	if (legendHeaderLabel == null) {
		string iconName = string.Format(
			"Profiler.{0}", System.Enum.GetName(typeof(ProfilerArea), m_Area));
		legendHeaderLabel = EditorGUIUtility.TextContentWithIcon(
			GetLocalizedChartName(), iconName);
	}
...
private void ApplyActiveState() {
	// Opening/Closing CPU chart should not set the CPU area as that would set
	// Profiler.enabled.
