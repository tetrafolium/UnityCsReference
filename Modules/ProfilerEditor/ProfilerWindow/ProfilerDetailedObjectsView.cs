...
namespace UnityEditorInternal.Profiling {
  ...
  internal class ProfilerDetailedObjectsView : ProfilerDetailedView {
    static readonly string kMetadataText =
        LocalizationDatabase.GetLocalizedString("Metadata:");
    static readonly string kCallstackText =
        LocalizationDatabase.GetLocalizedString("Callstack:");
    static readonly string kNoMetadataOrCallstackText =
        LocalizationDatabase.GetLocalizedString(
            "No metadata or callstack is available for the selected sample.");

    [NonSerialized] bool m_Initialized;
    [NonSerialized] List<ulong> m_CachedCallstack = new List<ulong>();
    ...
    [SerializeField] ProfilerFrameDataMultiColumnHeader m_MultiColumnHeader;
    ...
    class ObjectsTreeView : TreeView {
      ...
      public ObjectsTreeView(
          ...
          ProfilerFrameDataMultiColumnHeader multicolumnHeader)
      ...
      protected override TreeViewItem BuildRoot() {
        ...
        if (m_ObjectsData != null && m_ObjectsData.Count != 0) {
          ...
          allItems.Add(
              new TreeViewItem{id = 0, depth = 0, displayName = kNoneText});
        }
      ...
      void CellGUI(Rect cellRect, TreeViewItem item, int column,
                   ref RowGUIArgs args) {
        var objData = m_ObjectsData[args.item.id];

        CenterRectUsingSingleLineHeight(ref cellRect);
        DefaultGUI.Label(cellRect, objData.columnStrings[column], args.selected,
                         args.focused);
      }
      ...
      void OnSortingChanged(MultiColumnHeader header) {
        ...
        if (m_ObjectsData != null) {
          ...
          Comparison<ObjectInformation> comparison = (objData1, objData2) =>
              objData1
                  .columnStrings [header.sortedColumnIndex]
                  .CompareTo(objData2.columnStrings[header.sortedColumnIndex]) *
              orderMultiplier;
    ...
    void InitIfNeeded() {
      ...
      var cpuDetailColumns = new[]{HierarchyFrameDataView.columnObjectName,
                                   HierarchyFrameDataView.columnTotalPercent,
                                   HierarchyFrameDataView.columnGcMemory,
                                   HierarchyFrameDataView.columnTotalTime};
      var gpuDetailColumns = new[]{HierarchyFrameDataView.columnObjectName,
      ...
      var columns =
          ProfilerFrameDataHierarchyView.CreateColumns(profilerColumns);
      var headerState =
          ProfilerFrameDataHierarchyView.CreateDefaultMultiColumnHeaderState(
      ...
      m_MultiColumnHeader = new ProfilerFrameDataMultiColumnHeader(
      ...
      if (m_VertSplit == null || !m_VertSplit.IsValid())
        m_VertSplit = SplitterState.FromRelative(
            new[]{SessionState.GetFloat(splitter0StatePrefKey, 60f),
                  SessionState.GetFloat(splitter1StatePrefKey, 40f)},
            new[]{50f, 50f}, null);
    ...
    public void DoGUI(GUIStyle headerStyle,
      ...
      var selectedMergedSampleIndex =
          m_TreeView.GetSelectedFrameDataViewMergedSampleIndex();
      var selectedSampleMetadataCount = 0;
      if (selectedSampleId != -1) {
        frameDataView.GetItemMergedSampleCallstack(
            selectedSampleId, selectedMergedSampleIndex, m_CachedCallstack);
        selectedSampleMetadataCount =
            frameDataView.GetItemMergedSamplesMetadataCount(
                selectedSampleId, selectedMergedSampleIndex);
      }
      ...
      // Display active text (We want word wrapped text with a vertical
      // scrollbar)
      m_CallstackScrollViewPos = EditorGUILayout.BeginScrollView(
          m_CallstackScrollViewPos, Styles.callstackScroll);

      var sb = new StringBuilder();

      if (showMetadata || showCallstack) {
        if (showMetadata) {
          var metadataInfo = frameDataView.GetMarkerMetadataInfo(
              frameDataView.GetItemMarkerID(selectedSampleId));
          ...
          for (var i = 0; i < selectedSampleMetadataCount; ++i) {
            if (metadataInfo != null && i < metadataInfo.Length)
              ...
              sb.Append(i);
            sb.Append(": ");
            sb.Append(frameDataView.GetItemMergedSamplesMetadata(
                selectedSampleId, selectedMergedSampleIndex, i));
            sb.Append('\n');
          }
          sb.Append('\n');
        }
          ...
          foreach (var addr in m_CachedCallstack) {
            ...
            if (string.IsNullOrEmpty(methodInfo.methodName)) {
              ...
              if (methodInfo.sourceFileLine == 0)
                sb.AppendFormat(
                    "0x{0:X}\t\t{1}\t<a href=\"{2}\" line=\"1\">{2}</a>\n",
                    addr, methodInfo.methodName, normalizedPath);
              else
                sb.AppendFormat(
                    "0x{0:X}\t\t{1}\t<a href=\"{2}\" line=\"{3}\">{2}:{3}</a>\n",
                    addr, methodInfo.methodName, normalizedPath,
                    methodInfo.sourceFileLine);
            }
          }
        }
      } else {
      ...
      Styles.callstackTextArea.CalcMinMaxWidth(GUIContent.Temp(metadataText),
                                               out _, out var maxWidth);
      float minHeight = Styles.callstackTextArea.CalcHeight(
          GUIContent.Temp(metadataText), maxWidth);
      EditorGUILayout.SelectableLabel(
          metadataText, Styles.callstackTextArea, GUILayout.ExpandWidth(true),
          GUILayout.ExpandHeight(true), GUILayout.MinWidth(maxWidth + 10),
          GUILayout.MinHeight(minHeight + 10));
    ...
    void UpdateIfNeeded(HierarchyFrameDataView frameDataView, int selectedId) {
      ...
      for (var i = 0; i < columnsCount; i++) {
        ...
        m_FrameDataView.GetItemMergedSamplesColumnData(
            selectedId, m_MultiColumnHeader.columns[i].profilerColumn,
            objectsDatas[i]);
      }
      ...
      for (var i = 0; i < samplesCount; i++) {
        var objData =
            new ObjectInformation(){columnStrings = new string[columnsCount]};
        objData.id = selectedId;
        objData.sampleIndex = i;

        objData.instanceId = (i < instanceIDs.Count) ? instanceIDs[i] : 0;
        for (var j = 0; j < columnsCount; j++)
          objData.columnStrings[j] = (i < objectsDatas[j].Count) ? objectsDatas
                                         [j]
                                         [i]
                                                                 : string.Empty;
