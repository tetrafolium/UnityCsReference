...
internal class ProfilerWindow : EditorWindow,
  ...
  internal static class Styles {
    public static readonly GUIContent addArea = EditorGUIUtility.TrTextContent(
        "Profiler Modules", "Add and remove profiler modules");
    public static readonly GUIContent deepProfile =
        EditorGUIUtility.TrTextContent(
            "Deep Profile",
            "Instrument all scripting method calls to investigate scripts");
    public static readonly GUIContent deepProfileNotSupported =
        EditorGUIUtility.TrTextContent(
            "Deep Profile",
            "Build a Player with Deep Profiling Support to be able to enable instrumentation of all scripting methods in a Player.");
    public static readonly GUIContent noData =
        EditorGUIUtility.TrTextContent("No frame data available");
    public static readonly GUIContent noActiveModules =
        EditorGUIUtility.TrTextContent(
            "No Profiler Modules are active. Activate modules from the top left-hand drop-down.");

    public static readonly string enableDeepProfilingWarningDialogTitle =
        L10n.Tr("Enable deep script profiling");
    public static readonly string enableDeepProfilingWarningDialogContent =
        L10n.Tr("Enabling deep profiling requires reloading scripts.");
    public static readonly string disableDeepProfilingWarningDialogTitle =
        L10n.Tr("Disable deep script profiling");
    public static readonly string disableDeepProfilingWarningDialogContent =
        L10n.Tr("Disabling deep profiling requires reloading all scripts.");
    ...
    public static readonly GUIContent recordCallstacks = EditorGUIUtility.TrTextContent(
        "Call Stacks",
        "Record call stacks for special samples such as \"GC.Alloc\". " +
    ...
    public static readonly string[] recordCallstacksOptions = {
        L10n.Tr("GC.Alloc"), L10n.Tr("UnsafeUtility.Malloc(Persistent)"),
    ...
    public static readonly
        ProfilerMemoryRecordMode[] recordCallstacksEnumValues = {
    ...
    public static readonly GUIContent profilerRecordOff =
        EditorGUIUtility.TrIconContent("Record Off",
                                       "Record profiling information");
    public static readonly GUIContent profilerRecordOn =
        EditorGUIUtility.TrIconContent("Record On",
                                       "Record profiling information");
    ...
    public static readonly GUIContent prevFrame =
        EditorGUIUtility.TrIconContent("Animation.PrevKey", "Previous frame");
    public static readonly GUIContent nextFrame =
        EditorGUIUtility.TrIconContent("Animation.NextKey", "Next frame");
    public static readonly GUIContent currentFrame =
        EditorGUIUtility.TrIconContent("Animation.LastKey", "Current frame");
    public static readonly GUIContent frame = EditorGUIUtility.TrTextContent(
        "Frame: ", "Selected frame / Total number of frames");
    public static readonly GUIContent clearOnPlay = EditorGUIUtility.TrTextContent(
        "Clear on Play",
        "Clear the captured data on entering Play Mode, or connecting to a new Player");
    public static readonly GUIContent clearData =
        EditorGUIUtility.TrTextContent("Clear", "Clear the captured data");
    public static readonly GUIContent saveWindowTitle =
        EditorGUIUtility.TrTextContent("Save Window");
    public static readonly GUIContent saveProfilingData =
        EditorGUIUtility.TrIconContent(
            "SaveAs", "Save current profiling information to a binary file");
    public static readonly GUIContent loadWindowTitle =
        EditorGUIUtility.TrTextContent("Load Window");
    public static readonly GUIContent loadProfilingData =
        EditorGUIUtility.TrIconContent(
            "Import",
            "Load binary profiling information from a file. Shift click to append to the existing data");
    public static readonly string[] loadProfilingDataFileFilters = new string[]{
        L10n.Tr("Profiler files"), "data,raw", L10n.Tr("All files"), "*"};
    ...
    public static readonly GUILayoutOption chartWidthOption =
        GUILayout.Width(Chart.kSideWidth - 1);
  ...
  private static readonly ProfilerArea[] ms_StackedAreas = {
      ProfilerArea.CPU, ProfilerArea.GPU, ProfilerArea.UI,
      ProfilerArea.GlobalIllumination};

  [NonSerialized] bool m_Initialized;

  [SerializeField] SplitterState m_VertSplit;

  [NonSerialized] float m_FrameCountLabelMinWidth = 0;

  const string k_VertSplitterPercentageElement0PrefKey =
      "ProfilerWindow.VerticalSplitter.Relative[0]";
  const string k_VertSplitterPercentageElement1PrefKey =
      "ProfilerWindow.VerticalSplitter.Relative[1]";
  ...
  [SerializeField] string m_ActiveNativePlatformSupportModule = null;
  ...
  // through reflection
  [SerializeField] ProfilerArea m_CurrentArea = k_InvalidArea;
  const ProfilerArea k_InvalidArea =
      unchecked((ProfilerArea) Profiler.invalidProfilerArea);
  ...
  // used by Tests/PerformanceTests/Profiler ProfilerWindowTests.CPUViewTests
  // through reflection
  [SerializeReference] ProfilerModuleBase[] m_ProfilerModules;

  // used by Tests/PerformanceTests/Profiler
  // ProfilerWindowTests.CPUViewTests.SelectAndDisplayDetailsForAFrame_WithSearchFiltering
  // to avoid brittle tests due to reflection
  internal T GetProfilerModule<T>(ProfilerArea area) where T
      : ProfilerModuleBase {
    var index = (int) area;
  ...
  ProfilerMemoryRecordMode m_CurrentCallstackRecordMode =
      ProfilerMemoryRecordMode.None;
  [SerializeField] ProfilerMemoryRecordMode m_CallstackRecordMode =
      ProfilerMemoryRecordMode.None;

  public string ConnectedTargetName => m_AttachProfilerState.connectionName;

  public bool ConnectedToEditor =>
      m_AttachProfilerState.connectedToTarget == ConnectionTarget.Editor;

  [SerializeField]
  private bool m_ClearOnPlay;

  const string kProfilerRecentSaveLoadProfilePath =
      "ProfilerRecentSaveLoadProfilePath";
  const string kProfilerEnabledSessionKey = "ProfilerEnabled";
  const string kProfilerEditorTargetModeEnabledSessionKey =
      "ProfilerTargetMode";
  const string kProfilerDeepProfilingWarningSessionKey =
      "ProfilerDeepProfilingWarning";
  ...
  internal event Action<ProfilerMemoryRecordMode> memoryRecordingModeChanged =
      delegate{};

  // use this when iterating over arrays of history length. This +
  // iterationIndex < 0 means no data for this frame, for anything else, this is
  // the same as ProfilerDriver.firstFrame.
  int firstFrameIndexWithHistoryOffset {
    get {
      return ProfilerDriver.lastFrameIndex + 1 -
             ProfilerUserSettings.frameCount;
    }
  }
  ...
  public int GetActiveVisibleFrameIndex() {
    ...
    // otherwise it looks weird when it is rapidly jumping around when we have a
    // lot of repaints
    return m_CurrentFrame == -1 ? m_LastFrameFromTick : m_CurrentFrame;
  }

  public bool IsRecording() {
    return IsSetToRecord() &&
           ((EditorApplication.isPlaying && !EditorApplication.isPaused) ||
            ProfilerDriver.profileEditor ||
            !ProfilerDriver.IsConnectionEditor());
  }

  public bool IsSetToRecord() { return m_Recording; }
  ...
  static readonly string[] k_DefaultMemoryAreaCounterNames = {
      "Total Used Memory",     "Texture Memory", "Mesh Memory",
      "Material Count",        "Object Count",   "GC Used Memory",
      "GC Allocated In Frame",
  };

  static readonly string[] k_PS4MemoryAreaCounterNames =
      k_DefaultMemoryAreaCounterNames
          .Concat(new string[]{"GARLIC heap allocs", "ONION heap allocs"})
          .ToArray();
  ...
  void Initialize() {
    // When reinitializing (e.g. because Colorblind mode or PlatformModule
    // changed) we don't need a new state
    if (m_AttachProfilerState == null)
      m_AttachProfilerState = PlayerConnectionGUIUtility.GetConnectionState(
          this, OnTargetedEditorConnectionChanged, IsEditorConnectionTargeted,
          (player) => ClearFramesCallback()) as IConnectionStateInternal;
    ...
    for (int i = 0; i < Profiler.areaCount; i++) {
      ...
      ProfilerChart chart =
          CreateProfilerChart((ProfilerArea) i, chartType, scale, length);
      ...
      if (chart.m_Area == ProfilerArea.GPU) {
        chart.statisticsAvailabilityMessage =
            GPUProfilerModule.GetStatisticsAvailabilityStateReason;
      }

      for (int s = 0; s < length; s++) {
        chart.m_Series[s] = new ChartSeriesViewData(
            statisticsNames[s], historySize,
            chartAreaColors[s % chartAreaColors.Length]);
    ...
    m_CurrentArea = (ProfilerArea) SessionState.GetInt(k_CurrentAreaPrefKey,
                                                       (int) k_InvalidArea);

    if (m_CurrentArea == k_InvalidArea ||
        !m_Charts[(int) m_CurrentArea].active) {
    ...
    if (m_VertSplit == null || !m_VertSplit.IsValid())
      m_VertSplit = SplitterState.FromRelative(
          new[]{EditorPrefs.GetFloat(k_VertSplitterPercentageElement0PrefKey,
                                     50f),
                EditorPrefs.GetFloat(k_VertSplitterPercentageElement1PrefKey,
                                     50f)},
          new float[]{k_VertSplitterMinSizes, k_VertSplitterMinSizes}, null);
    // 2 times the min splitter size plus one line height for the toolbar up top
    minSize = new Vector2(Chart.kSideWidth + k_RightPaneMinSize,
                          k_VertSplitterMinSizes * m_VertSplit.minSizes.Length +
                              k_LineHeight);

    // TODO: only create modules for active charts and otherwise lazy initialize
    // them.
  ...
  void OnToggleCPUChartSeries(bool wasToggled) {
    if (wasToggled) {
      ...
      int firstFrame =
          Mathf.Max(ProfilerDriver.firstFrameIndex, firstEmptyFrame);

      ComputeChartScaleValue(ProfilerArea.CPU, historyLength, firstEmptyFrame,
                             firstFrame);
    }
  }

  ProfilerChart CreateProfilerChart(ProfilerArea i, Chart.ChartType chartType,
                                    float scale, int length) {
    ProfilerChart newChart =
        (i == ProfilerArea.UIDetails)
            ? new UISystemProfilerChart(chartType, scale, length)
            : new ProfilerChart(i, chartType, scale, length);

    if (i == ProfilerArea.NetworkMessages ||
        i == ProfilerArea.NetworkOperations) {
  ...
  void OnChartClosed(Chart sender) {
    ...
    m_ProfilerModules [(int) profilerChart.m_Area]
        .OnDisable();
    m_ProfilerModules [(int) profilerChart.m_Area]
        .OnClosed();
    m_CurrentArea = k_InvalidArea;
  ...
  void OnChartSelected(Chart sender) {
    ...
    if (oldArea != k_InvalidArea) {
      m_ProfilerModules [(int) oldArea]
          .OnDisable();
    }

    m_ProfilerModules [(int) newArea]
        .OnEnable(this);
  ...
  void CheckForPlatformModuleChange() {
    if (m_ActiveNativePlatformSupportModule == null) {
      m_ActiveNativePlatformSupportModule =
          EditorUtility.GetActiveNativePlatformSupportModuleName();
      return;
    }

    if (m_ActiveNativePlatformSupportModule !=
        EditorUtility.GetActiveNativePlatformSupportModuleName()) {
  ...
  void SaveViewSettings() {
    ...
    if (m_VertSplit != null && m_VertSplit.relativeSizes != null) {
      EditorPrefs.SetFloat(k_VertSplitterPercentageElement0PrefKey,
                           m_VertSplit.relativeSizes[0]);
      EditorPrefs.SetFloat(k_VertSplitterPercentageElement1PrefKey,
                           m_VertSplit.relativeSizes[1]);
    }
  ...
  void Awake() {
    ...
    if (ProfilerUserSettings.rememberLastRecordState)
      m_Recording = EditorPrefs.GetBool(
          kProfilerEnabledSessionKey, ProfilerUserSettings.defaultRecordState);
    else
      m_Recording = SessionState.GetBool(
          kProfilerEnabledSessionKey, ProfilerUserSettings.defaultRecordState);

    // This event gets called every time when some other window is maximized and
    // then unmaximized
    ProfilerDriver.enabled = m_Recording;
    ProfilerDriver.profileEditor =
        SessionState.GetBool(kProfilerEditorTargetModeEnabledSessionKey,
                             ProfilerUserSettings.defaultTargetMode ==
                                     ProfilerEditorTargetMode.Editmode ||
                                 ProfilerDriver.profileEditor);
  ...
  void OnFocus() {
    // set the real state of profiler. OnDestroy is called not only when window
    // is destroyed, but also when maximized state is changed
    if (Profiler.supported)
      ProfilerDriver.enabled = m_Recording;
  }
  ...
  void OnLostFocus() {
    if (GUIUtility.hotControl != 0) {
      // The chart may not have had the chance to release the hot control before
      // we lost focus. This happens when changing the selected frame, which may
      // pause the game and switch the focus to another view.
  ...
  public virtual void AddItemsToMenu(GenericMenu menu) {
    menu.AddItem(Styles.accessibilityModeLabel,
                 UserAccessiblitySettings.colorBlindCondition !=
  ...
  void OnToggleColorBlindMode() {
    UserAccessiblitySettings.colorBlindCondition =
        ...
        ColorBlindCondition.Default
                ? ColorBlindCondition
            .Deuteranopia : ColorBlindCondition.Default;
  }

  void OnToggleShowStatsLabelsOnCurrentFrame() {
    ProfilerUserSettings.showStatsLabelsOnCurrentFrame =
        !ProfilerUserSettings.showStatsLabelsOnCurrentFrame;
  }
  ...
  static void ShowProfilerOOP() {
    if (EditorUtility.DisplayDialog(
            ...
            "OK", DialogOptOutDecisionType.ForThisMachine,
            "UseOutOfProcessProfiler")) {
  ...
  static void RecordToggle() {
    ...
    if (CommandService.Exists("ProfilerRecordToggle")) {
      var result =
          CommandService.Execute("ProfilerRecordToggle", CommandHint.Shortcut);
    ...
    if (!commandHandled) {
      ...
      using(var state =
                PlayerConnectionGUIUtility.GetConnectionState(null, null)) {
        var connectionName = "";
        ...
        if (ProfilerDriver.enabled)
          Debug.LogFormat(
              ...
              $"Recording {GetRecordingStateName(connectionName)} has started...");
        else
          Debug.LogFormat(
              LogType.Log, LogOption.NoStacktrace, null,
  ...
  private static void EditorGUI_HyperLinkClicked(object sender, EventArgs e) {
    EditorGUILayout.HyperLinkClickedEventArgs args =
        (EditorGUILayout.HyperLinkClickedEventArgs) e;
  ...
  static void RepaintAllProfilerWindows() {
    foreach (ProfilerWindow window in m_ProfilerWindows) {
      // This is useful hack when you need to profile in the editor and dont
      // want it to affect your framerate... NOTE: we should make this an option
      // in the UI somehow...
  ...
  void SetProfileDeepScripts(bool deep) {
    ...
    if (ProfilerDriver.IsConnectionEditor()) {
      ...
      // When connected to the player, we send deep profiler mode command
      // immediately.
  ...
  string PickFrameLabel() {
    ...
    return ((m_CurrentFrame == -1) ? lastFrame : m_CurrentFrame + 1) + " / " +
           lastFrame;
  }
  ...
  public HierarchyFrameDataView
  GetFrameDataView(string threadName, HierarchyFrameDataView.ViewModes viewMode,
    ...
    if (m_FrameDataView != null && m_FrameDataView.valid) {
      if (m_FrameDataView.frameIndex == frameIndex &&
          m_FrameDataView.threadIndex == threadIndex &&
          m_FrameDataView.viewMode == viewMode)
    ...
    m_FrameDataView = new HierarchyFrameDataView(
        frameIndex, threadIndex, viewMode, profilerSortColumn, sortAscending);
    return m_FrameDataView;
  }
  ...
  private static void UpdateChartGrid(float timeMax, ChartViewData data) {
    if (timeMax < 1500) {
      ...
      data.SetGrid(new float[]{8333, 4000, 1000},
                   new[]{"8ms (120FPS)", "4ms (250FPS)", "1ms (1000FPS)"});
    } else if (timeMax < 30000) {
      data.SetGrid(new float[]{16667, 10000, 5000},
                   new[]{"16ms (60FPS)", "10ms (100FPS)", "5ms (200FPS)"});
    } else if (timeMax < 100000) {
      data.SetGrid(new float[]{66667, 33333, 16667},
                   new[]{"66ms (15FPS)", "33ms (30FPS)", "16ms (60FPS)"});
    } else {
      data.SetGrid(new float[]{500000, 200000, 66667},
  ...
  private void UpdateCharts() {
    ...
    bool hasCPUOverlay =
        (selectedName != string.Empty) && m_CurrentArea == ProfilerArea.CPU;
    ...
    if (hasCPUOverlay) {
      ...
      for (int i = 0; i < numCharts; ++i) {
        ...
        cpuChart.m_Data.overlays[i] = new ChartSeriesViewData(
            chart.name, chart.yValues.Length, chart.color);
        for (int frameIdx = 0; frameIdx < chart.yValues.Length; ++frameIdx)
          cpuChart.m_Data.overlays[i].xValues[frameIdx] = (float) frameIdx;
        float maxValue;
        ProfilerDriver.GetCounterValuesBatch(
            ProfilerArea.CPU, UnityString.Format("Selected{0}", chart.name),
            firstEmptyFrame, 1.0f, cpuChart.m_Data.overlays[i].yValues,
            out maxValue);
    ...
    for (int i = 0; i < ms_StackedAreas.Length; i++)
      ComputeChartScaleValue(ms_StackedAreas[i], historyLength, firstEmptyFrame,
                             firstFrame);
  }

  private void ComputeChartScaleValue(ProfilerArea i, int historyLength,
                                      int firstEmptyFrame, int firstFrame) {
    ...
    for (int k = 0; k < historyLength; k++) {
      ...
      if (timeNow > timeMax)
        timeMax = timeNow;
      if (timeNow > timeMaxExcludeFirst &&
          k + firstEmptyFrame >= firstFrame + 1)
        timeMaxExcludeFirst = timeNow;
    }
    if (timeMaxExcludeFirst != 0.0f)
      timeMax = timeMaxExcludeFirst;

    timeMax = Mathf.Clamp(timeMax * chart.m_DataScale, k_ChartMinClamp,
                          k_ChartMaxClamp);
  ...
  internal void UpdateSingleChart(ProfilerChart chart, int firstEmptyFrame,
                                  int firstFrame) {
    float totalMaxValue = 1;
    for (int i = 0, count = chart.m_Series.Length; i < count; ++i) {
      ProfilerDriver.GetCounterValuesBatch(
          chart.m_Area, chart.m_Series[i].name, firstEmptyFrame, 1.0f,
          chart.m_Series[i].yValues, out var maxValue);
      ...
      if (chart.m_Type == Chart.ChartType.Line) {
        // Scale line charts so they never hit the top. Scale them slightly
        // differently for each line so that in "no stuff changing" case they
        // will not end up being exactly the same.
    ...
    if (chart.m_SharedScale && chart.m_Type == Chart.ChartType.Line) {
      // For some charts, every line is scaled individually, so every data
      // series gets their own range based on their own max scale. For charts
      // that share their scale (like the Networking charts) all series get
      // adjusted to the total max of the chart.
      for (int i = 0, count = chart.m_Series.Length; i < count; ++i)
        chart.m_Series[i].rangeAxis =
            new Vector2(0f, (1.05f + i * 0.05f) * totalMaxValue);
      chart.m_Data.maxValue = totalMaxValue;
    }
    chart.m_Data.Assign(chart.m_Series, firstEmptyFrame, firstFrame);

    ProfilerDriver.GetStatisticsAvailabilityStates(
        chart.m_Area, firstEmptyFrame, chart.m_Data.dataAvailable);

    if (chart is UISystemProfilerChart)
      ((UISystemProfilerChart) chart)
          .Update(firstFrame, ProfilerUserSettings.frameCount);
  }
  ...
  void ToggleCallstackRecordModeFlag(object userData, string[] options,
                                     int selected) {
  ...
  internal void SaveProfilingData() {
    ...
    string filename =
        string.IsNullOrEmpty(recent) ? "" : System.IO.Path.GetFileName(recent);

    string selected = EditorUtility.SaveFilePanel(Styles.saveWindowTitle.text,
  ...
  internal void LoadProfilingData(bool keepExistingData) {
    ...
    string selected = EditorUtility.OpenFilePanelWithFilters(
        Styles.loadWindowTitle.text, recent,
        Styles.loadProfilingDataFileFilters);
...
#pragma warning restore
  ...
  private void DrawMainToolbar() {
    ...
    Rect popupRect = GUILayoutUtility.GetRect(
        Styles.addArea, EditorStyles.toolbarDropDown, Styles.chartWidthOption);
    if (EditorGUI.DropdownButton(popupRect, Styles.addArea, FocusType.Passive,
      ...
      EditorUtility.DisplayCustomMenu(popupRect, names, enabled, selected,
                                      AddAreaClick, null);
    }

    // Engine attach
    PlayerConnectionGUILayout.ConnectionTargetSelectionDropdown(
        m_AttachProfilerState, EditorStyles.toolbarDropDown);

    // Record
    var profilerEnabled = GUILayout.Toggle(
        m_Recording, m_Recording ? Styles.profilerRecordOn
        : Styles.profilerRecordOff, EditorStyles.toolbarButton);
    ...
    // Clear on Play
    SetClearOnPlay(GUILayout.Toggle(GetClearOnPlay(), Styles.clearOnPlay,
    ...
    using(new EditorGUI.DisabledScope(!deepProfilerSupported)) {
      SetProfileDeepScripts(GUILayout.Toggle(
          ProfilerDriver.deepProfiling,
          deepProfilerSupported ? Styles.deepProfile
          : Styles.deepProfileNotSupported, EditorStyles.toolbarButton));
    }

    // Allocation call stacks
    AllocationCallstacksToolbarItem();

    // keep more space between the toggles and the overflow/help icon buttons on
    // the far right, keep deep profiling closer to the other controls
    GUILayout.FlexibleSpace();
    GUILayout.FlexibleSpace();
    GUILayout.FlexibleSpace();
    ...
    // Save profile
    using(new EditorGUI.DisabledScope(ProfilerDriver.lastFrameIndex == -1)) {
      if (GUILayout.Button(Styles.saveProfilingData,
                           EditorStyles.toolbarButton)) {
        SaveProfilingData();
    ...
    // Open Manual
    if (GUILayout.Button(Styles.helpButtonContent,
                         EditorStyles.toolbarButton)) {
      Application.OpenURL(Styles.linkToManual);
    }

    // Overflow Menu
    var overflowMenuRect = GUILayoutUtility.GetRect(
        Styles.optionsButtonContent, EditorStyles.toolbarButtonRight);
    if (GUI.Button(overflowMenuRect, Styles.optionsButtonContent,
      ...
      menu.AddItem(Styles.accessibilityModeLabel,
                   UserAccessiblitySettings.colorBlindCondition !=
                       ColorBlindCondition.Default,
                   OnToggleColorBlindMode);
      menu.AddItem(Styles.showStatsLabelsOnCurrentFrameLabel,
                   ProfilerUserSettings.showStatsLabelsOnCurrentFrame,
                   OnToggleShowStatsLabelsOnCurrentFrame);
      menu.AddSeparator("");
      menu.AddItem(Styles.preferencesButtonContent, false,
                   OpenProfilerPreferences);
  ...
  void OpenProfilerPreferences() {
    var settings = SettingsWindow.Show(SettingsScope.User,
  ...
  void AllocationCallstacksToolbarItem() {
    // Whenever we unset all flags, we fallback to the default GC Alloc
    // callstacks.
    ...
    if (EditorGUILayout.DropDownToggle(ref toggled, Styles.recordCallstacks,
                                       EditorStyles.toolbarDropDownToggle)) {
      var rect = GUILayoutUtility.topLevel.GetLast();
      var names = Unsupported.IsDeveloperMode()
                      ? Styles.recordCallstacksDevelopmentOptions
                      : Styles.recordCallstacksOptions;
      var selected = new List<int>();
      for (var i = 0; i < names.Length; ++i) {
        if ((m_CallstackRecordMode & Styles.recordCallstacksEnumValues[i]) != 0)
          selected.Add(i);
      }
      EditorUtility.DisplayCustomMenu(rect, names, selected.ToArray(),
                                      ToggleCallstackRecordModeFlag, null);
      GUIUtility.ExitGUI();
    }
    if (toggled != oldToggleState) {
      selectedMemRecordMode =
          m_CurrentCallstackRecordMode !=
          ProfilerMemoryRecordMode.None ? ProfilerMemoryRecordMode.None
          : m_CallstackRecordMode;
      SetCallstackRecordMode(selectedMemRecordMode);
    }
  }

  void Clear() {
    m_ProfilerModules [(int) ProfilerArea.CPU]
        .Clear();
    m_ProfilerModules [(int) ProfilerArea.GPU]
        .Clear();
...
#pragma warning restore
  ...
  private void FrameNavigationControls() {
    ...
    using(new EditorGUI.DisabledScope(
        ProfilerDriver.GetPreviousFrameIndex(m_CurrentFrame) == -1)) {
    ...
    using(new EditorGUI.DisabledScope(
        ProfilerDriver.GetNextFrameIndex(m_CurrentFrame) == -1)) {
    ...
    using(new EditorGUI.DisabledScope(ProfilerDriver.lastFrameIndex < 0)) {
      if (GUILayout.Toggle(ProfilerDriver.lastFrameIndex >= 0 &&
                               m_CurrentFrame == -1,
                           Styles.currentFrame, EditorStyles.toolbarButton)) {
    ...
    EditorStyles.toolbarLabel.CalcMinMaxWidth(frameCountLabel, out minWidth,
                                              out maxWidth);
    if (minWidth > m_FrameCountLabelMinWidth)
      // to avoid increasing the size in too fine graned intervals, add a 10
      // pixel buffer.
      m_FrameCountLabelMinWidth = minWidth + 10;
    GUILayout.Label(frameCountLabel, EditorStyles.toolbarLabel,
                    GUILayout.MinWidth(m_FrameCountLabelMinWidth));
  }

  void SetCurrentFrameDontPause(int frame) { m_CurrentFrame = frame; }

  void SetCurrentFrame(int frame) {
    bool shouldPause = frame != -1 && ProfilerDriver.enabled &&
                       !ProfilerDriver.profileEditor && m_CurrentFrame != frame;
  ...
  void OnGUI() {
    ...
    m_GraphPos = EditorGUILayout.BeginScrollView(
        m_GraphPos, Styles.profilerGraphBackground);
    ...
    for (int c = 0; c < m_Charts.Length; ++c) {
      ...
      newCurrentFrame =
          chart.DoChartGUI(newCurrentFrame, m_CurrentArea == chart.m_Area);
    }
    ...
    if (m_CurrentArea != k_InvalidArea) {
      var detailViewPosition =
          new Rect(0, m_VertSplit.realSizes[0] + EditorGUI.kWindowToolbarHeight,
                   position.width, m_VertSplit.realSizes[1]);
      var detailViewToolbar = detailViewPosition;
      detailViewToolbar.height =
          EditorStyles.contentToolbar.CalcHeight(GUIContent.none, 10.0f);
      m_ProfilerModules [(int) m_CurrentArea]
          .DrawToolbar(detailViewPosition);

      detailViewPosition.yMin += detailViewToolbar.height;
      m_ProfilerModules [(int) m_CurrentArea]
          .DrawView(detailViewPosition);

      // Draw separator
      var lineRect = new Rect(
          0, m_VertSplit.realSizes[0] + EditorGUI.kWindowToolbarHeight - 1,
          position.width, 1);
  ...
  public void SetClearOnPlay(bool enabled) { m_ClearOnPlay = enabled; }

  public bool GetClearOnPlay() { return m_ClearOnPlay; }
  ...
  void OnTargetedEditorConnectionChanged(EditorConnectionTarget change) {
    ...
    SessionState.SetBool(kProfilerEditorTargetModeEnabledSessionKey,
                         ProfilerDriver.profileEditor);
  }
  ...
  internal static bool SetEditorDeepProfiling(bool deep) {
    ...
    // When enabling / disabling deep script profiling we need to reload
    // scripts. In play mode this might be intrusive. So ask the user first.
    if (EditorApplication.isPlaying) {
      if (deep)
        doApply = EditorUtility.DisplayDialog(
            Styles.enableDeepProfilingWarningDialogTitle,
            Styles.enableDeepProfilingWarningDialogContent,
            Styles.domainReloadWarningDialogButton, Styles.cancelDialogButton,
            DialogOptOutDecisionType.ForThisSession,
            kProfilerDeepProfilingWarningSessionKey);
      else
        doApply = EditorUtility.DisplayDialog(
            Styles.disableDeepProfilingWarningDialogTitle,
            Styles.disableDeepProfilingWarningDialogContent,
            Styles.domainReloadWarningDialogButton, Styles.cancelDialogButton,
            DialogOptOutDecisionType.ForThisSession,
            kProfilerDeepProfilingWarningSessionKey);
    }
