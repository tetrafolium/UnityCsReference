...
internal class ProfilerInstrumentationPopup : PopupWindowContent {
  private const string kAutoInstrumentSettingKey =
      "ProfilerAutoInstrumentedAssemblyTypes";
  private const int kAutoInstrumentButtonHeight = 20;
  private const int kAutoInstrumentButtonsHeight =
      1 * kAutoInstrumentButtonHeight;

  private static GUIContent s_AutoInstrumentScriptsContent =
      EditorGUIUtility.TrTextContent("Auto instrument " +
                                     InstrumentedAssemblyTypes.Script +
                                     " assemblies");
  ...
  class InputData : PopupList.InputData {
    public override IEnumerable<PopupList.ListElement>
    BuildQuery(string prefix) {
      if (prefix == "")
  ...
  public ProfilerInstrumentationPopup(Dictionary<string, int> functions,
                                      bool showAllCheckbox,
                                      bool showAutoInstrumemtationParams) {
    m_ShowAutoInstrumemtationParams = showAutoInstrumemtationParams;
    m_ShowAllCheckbox = showAllCheckbox;
    m_AutoInstrumentedAssemblyTypes =
        (InstrumentedAssemblyTypes)
            SessionState.GetInt(kAutoInstrumentSettingKey, 0);
    ...
    m_FunctionsListInputData.m_OnSelectCallback =
        ProfilerInstrumentationPopupCallback;
  ...
  private void SetFunctions(Dictionary<string, int> functions) {
    ...
    if (functions == null) {
      // Querying functions
      var element = m_FunctionsListInputData.NewOrMatchingElement(
          "Querying instrumentable functions...");
      element.enabled = false;
    } else if (functions.Count == 0) {
      var element = m_FunctionsListInputData.NewOrMatchingElement(
          "No instrumentable child functions found");
      element.enabled = false;
    } else {
      m_FunctionsListInputData.m_MaxCount =
          Mathf.Clamp(functions.Count + 1, 0, 30);

      if (m_ShowAllCheckbox) {
        m_AllCheckbox =
            new PopupList.ListElement(" All", false, float.MaxValue);
  ...
  public override void OnGUI(Rect rect) {
    ...
    if (m_ShowAutoInstrumemtationParams) {
      ...
      if (GUI.Toggle(toggleRect,
                     (m_AutoInstrumentedAssemblyTypes &
                      InstrumentedAssemblyTypes.Script) != 0,
                     s_AutoInstrumentScriptsContent))
      ...
      if (newAutoInstrumentedAssemblies != m_AutoInstrumentedAssemblyTypes) {
        ...
        ProfilerDriver.SetAutoInstrumentedAssemblies(
            m_AutoInstrumentedAssemblyTypes);
        SessionState.SetInt(kAutoInstrumentSettingKey,
                            (int) m_AutoInstrumentedAssemblyTypes);
      }
  ...
  public override void OnClose() { m_FunctionsList.OnClose(); }
  ...
  static void SetFunctionNamesFromUnity(bool allFunction,
                                        string[] functionNames,
                                        int[] isInstrumentedFlags) {
  ...
  public static void Show(Rect r) {
    var popup = new ProfilerInstrumentationPopup(s_InstrumentableFunctions,
                                                 false, true);
  ...
  static public bool FunctionHasInstrumentationPopup(string funcName) {
    return s_InstrumentableFunctions != null &&
           s_InstrumentableFunctions.ContainsKey(funcName);
  }
