...
namespace UnityEditorInternal.Profiling {
  ...
  internal class ProfilerFrameDataViewBase {
    protected static class BaseStyles {
      public static readonly GUIContent noData = EditorGUIUtility.TrTextContent(
          "No frame data available. Select a frame from the charts above to see its details here.");
      public static GUIContent disabledSearchText = EditorGUIUtility.TrTextContent(
          "Showing search results are disabled while recording with deep profiling.\nStop recording to view search results.");
      public static GUIContent cpuGPUTime =
          EditorGUIUtility.TrTextContent("CPU:{0}ms   GPU:{1}ms");
      ...
      public static readonly GUIStyle viewTypeToolbarDropDown =
          new GUIStyle(EditorStyles.toolbarDropDownLeft);
      public static readonly GUIStyle threadSelectionToolbarDropDown =
          new GUIStyle(EditorStyles.toolbarDropDown);
      public static readonly GUIStyle detailedViewTypeToolbarDropDown =
          new GUIStyle(EditorStyles.toolbarDropDown);
      public static readonly GUIContent updateLive = EditorGUIUtility.TrTextContent(
          "Live",
          "Display the current or selected frame while recording Playmode or Editor. This increases the overhead in the EditorLoop when the Profiler Window is repainted.");
      public static readonly GUIContent liveUpdateMessage =
          EditorGUIUtility.TrTextContent(
              "Displaying of frame data disabled while recording Playmode or Editor. To see the data, pause recording, or toggle \"Live\" display mode on. " +
    ...
    static readonly GUIContent[] kCPUProfilerViewTypeNames =
        new GUIContent[]{EditorGUIUtility.TrTextContent("Timeline"),
    ...
    static readonly int[] kCPUProfilerViewTypes = new int[]{
        (int) ProfilerViewType.Timeline, (int) ProfilerViewType.Hierarchy,
        (int) ProfilerViewType.RawHierarchy};

    static readonly GUIContent[] kGPUProfilerViewTypeNames =
        new GUIContent[]{EditorGUIUtility.TrTextContent("Hierarchy"),
                         EditorGUIUtility.TrTextContent("Raw Hierarchy")};
    static readonly int[] kGPUProfilerViewTypes = new int[]{
        (int) ProfilerViewType.Hierarchy, (int) ProfilerViewType.RawHierarchy};
    ...
    public CPUorGPUProfilerModule cpuModule {
      get;
      private set;
    }
    ...
    public virtual void OnEnable(CPUorGPUProfilerModule cpuOrGpuModule,
    ...
    protected void DrawViewTypePopup(ProfilerViewType viewType) {
      ...
      if (!gpuView) {
        newViewType = (ProfilerViewType) EditorGUILayout.IntPopup(
            (int) viewType, kCPUProfilerViewTypeNames, kCPUProfilerViewTypes,
            BaseStyles.viewTypeToolbarDropDown,
            GUILayout.Width(BaseStyles.viewTypeToolbarDropDown.fixedWidth));
      } else {
        if (viewType == ProfilerViewType.Timeline)
          viewType = ProfilerViewType.Hierarchy;
        newViewType = (ProfilerViewType) EditorGUILayout.IntPopup(
            (int) viewType, kGPUProfilerViewTypeNames, kGPUProfilerViewTypes,
            BaseStyles.viewTypeToolbarDropDown,
            GUILayout.Width(BaseStyles.viewTypeToolbarDropDown.fixedWidth));
      }
    ...
    protected void DrawLiveUpdateToggle(ref bool updateViewLive) {
      using(new EditorGUI.DisabledScope(ProcessService.level !=
                                        ProcessLevel.Master)) {
        // This button is only needed in the Master Process
        updateViewLive = GUILayout.Toggle(updateViewLive, BaseStyles.updateLive,
    ...
    protected void DrawCPUGPUTime(float cpuTimeMs, float gpuTimeMs) {
      var cpuTime =
          cpuTimeMs > 0 ? UnityString.Format("{0:N2}", cpuTimeMs) : "--";
      var gpuTime =
          gpuTimeMs > 0 ? UnityString.Format("{0:N2}", gpuTimeMs) : "--";
      GUILayout.Label(
          UnityString.Format(BaseStyles.cpuGPUTime.text, cpuTime, gpuTime),
    ...
    protected void ShowLargeTooltip(Vector2 pos, Rect fullRect, string text,
      ...
      // Flip tooltip if too close to bottom (but do not flip if flipping would
      // mean the tooltip is too high up)
      var flipRectAdjust = (lineHeight + rect.height + 2 * arrowRect.height);
      var flipped =
          (pos.y + size.y + 6 > fullRect.yMax) && (rect.y - flipRectAdjust > 0);
      if (flipped) {
      ...
      if (flipped)
        GUIUtility.ScaleAroundPivot(
            ...
            new Vector2(arrowRect.width * 0.5f, arrowRect.height));
      GUI.Label(new Rect(0, 0, arrowRect.width, arrowRect.height),
                GUIContent.none, BaseStyles.tooltipArrow);
