...
[SerializeField] SplitterState m_NetworkSplit;

static SVC<Color> s_SeparatorColor =
...
public override void OnEnable(IProfilerWindowController profilerWindow) {
	...
	if (m_NetworkSplit == null || !m_NetworkSplit.IsValid())
		m_NetworkSplit = SplitterState.FromRelative(new[] {20f, 80f},
		                                            new[] {100f, 100f}, null);
}
...
[SerializeField]
private String[] msgNames = {"UserMessage",
	                     "ObjectDestroy",
	                     "ClientRpc",
	                     "ObjectSpawn",
	                     "Owner",
...
#pragma warning disable CS0618
		...
		msgFoldouts[msgId] =
			EditorGUILayout.Foldout(msgFoldouts[msgId], msgNames[msgId] + ":");
...
#pragma warning restore
			...
			foreach (var entryName in detail.m_Entries.Keys) {
				...
				if (entry.m_IncomingTotal > 0) {
					...
					EditorGUILayout.LabelField(
						entry.m_IncomingSequence.GetFiveTick(tick).ToString());
					EditorGUILayout.LabelField(
						entry.m_IncomingSequence.GetTenTick(tick).ToString());
				...
				if (entry.m_OutgoingTotal > 0) {
					...
					EditorGUILayout.LabelField(
						entry.m_OutgoingSequence.GetFiveTick(tick).ToString());
					EditorGUILayout.LabelField(
						entry.m_OutgoingSequence.GetTenTick(tick).ToString());
	...
	// Draw separator
	var lineRect =
		new Rect(m_NetworkSplit.realSizes[0] + labelRect.xMin,
		         position.y - EditorGUI.kWindowToolbarHeight - 1, 1,
