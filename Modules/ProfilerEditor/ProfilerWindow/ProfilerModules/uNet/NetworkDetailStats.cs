...
[Obsolete(
	 "This profiler helper class is deprecated, it's here to support the high level API package until it no longer depends on it and will then be removed.")]
public class NetworkDetailStats {
public enum NetworkDirection { Incoming, Outgoing }
...
public int GetFiveTick(int tick) {
	...
	int count = 0;
	for (int i = 0; i < 5; i++) {
		count += m_MessagesPerTick[(tick - i + kPacketHistoryTicks) %
		                           kPacketHistoryTicks];
	}
	return (int)(count / 5);
}
...
// detailed stats for one entry of a network operation, such as a command or
// rpc-call
...
public Dictionary<string, NetworkOperationEntryDetails> m_Entries =
	new Dictionary<string, NetworkOperationEntryDetails>();
...
public void SetStat(NetworkDirection direction, string entryName,
                    int amount) {
...
public void IncrementStat(NetworkDirection direction, string entryName,
                          int amount) {
	NetworkOperationEntryDetails entry;
	if (m_Entries.ContainsKey(entryName)) {
		entry = m_Entries[entryName];
...
static internal Dictionary<short, NetworkOperationDetails>
m_NetworkOperations = new Dictionary<short, NetworkOperationDetails>();
...
static public void SetStat(NetworkDirection direction, short msgId,
                           string entryName, int amount) {
...
static public void IncrementStat(NetworkDirection direction, short msgId,
                                 string entryName, int amount) {
	NetworkOperationDetails op;
	if (m_NetworkOperations.ContainsKey(msgId)) {
		op = m_NetworkOperations[msgId];
