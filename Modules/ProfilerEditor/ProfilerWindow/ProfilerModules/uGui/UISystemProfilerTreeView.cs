...
public UISystemProfilerTreeView(State state,
...
protected override IList<TreeViewItem> BuildRows(TreeViewItem root) {
	...
	profilerState.lastFrame =
		profilerState.profilerWindow.GetActiveVisibleFrameIndex();
	...
	if (UISystemData != null) {
		...
		foreach (var data in UISystemData) {
			...
			if (data.isBatch) {
				...
				canvasTreeViewItem = new BatchTreeViewItem(
					data, parent.depth + 1, name, allBatchesInstanceIDs);
			} else {
				name =
					property.GetUISystemProfilerNameByOffset(data.objectNameOffset);
				canvasTreeViewItem =
					new CanvasTreeViewItem(data, parent.depth + 1, name);
				batchIndex = 0;
		...
		m_Comparer.isAscending =
			multiColumnHeader.GetColumn((int) m_Comparer.Col).sortedAscending;
	...
	for (int i = 0, count = args.GetNumVisibleColumns(); i < count; i++) {
		...
		if (column == (int) Column.Element) {
			...
			lineStyle.padding.left =
				icon == null ? 0 : iconRectWidth + kSpaceBetweenIconAndText;
			lineStyle.Draw(rect, args.item.displayName, false, false, args.selected,
			               args.focused);
		...
		if (content != null) {
			DefaultGUI.LabelRightAligned(rect, content, args.selected,
			                             args.focused);
		} else {
			// var c = GUI.color;
			// var f = .65F;
...
protected override void ContextClickedItem(int id) {
	...
	pm.AddItem(EditorGUIUtility.TrTextContent("Find matching objects in scene"),
	           false, () => DoubleClickedItem(id));

	pm.ShowAsContext();
}

protected override void DoubleClickedItem(int id) {
	IList<TreeViewItem> rows = GetRowsFromIDs(new List<int> {
				id
			});
...
private static void HighlightRowsMatchingObjects(IList<TreeViewItem> rows) {
	...
	foreach (var row in rows) {
		...
		if (canvasRow == null)
			continue;
		Canvas canvas = EditorUtility.InstanceIDToObject(
			canvasRow.info.objectInstanceId) as Canvas;
...
public IProfilerWindowController profilerWindow;
}
...
protected static readonly Texture2D s_CanvasIcon =
	EditorGUIUtility.LoadIcon("RectTool On");
public UISystemProfilerInfo info;
public int renderDataIndex;

internal BaseTreeViewItem(UISystemProfilerInfo info, int depth,
                          string displayName)
...
public CanvasTreeViewItem(UISystemProfilerInfo info, int depth,
                          string displayName)
...
public BatchTreeViewItem(UISystemProfilerInfo info, int depth,
                         string displayName, int[] allBatchesInstanceIDs)
	: base(info, depth, displayName) {
	icon = null;
	instanceIDs = new int[info.instanceIDsCount];
	Array.Copy(allBatchesInstanceIDs, info.instanceIDsIndex, instanceIDs, 0,
	           info.instanceIDsCount);
