...
internal class UISystemProfilerChart : ProfilerChart {
  ...
  public UISystemProfilerChart(Chart.ChartType type, float dataScale,
                               int seriesCount)
      : base(ProfilerArea.UIDetails, type, dataScale, seriesCount) {}

  public void Update(int firstFrame, int historyLength) {
    int count =
        ProfilerDriver.GetUISystemEventMarkersCount(firstFrame, historyLength);
    if (count == 0)
      return;
    m_Markers = new EventMarker[count];
    m_MarkerNames = new string[count];
    ProfilerDriver.GetUISystemEventMarkersBatch(firstFrame, historyLength,
                                                m_Markers, m_MarkerNames);
  }
  ...
  public override int DoChartGUI(int currentFrame, bool active) {
    ...
    if (m_Markers != null && showMarkers) {
      ...
      for (int index = 0; index < m_Markers.Length; index++) {
        ...
        Color color =
            ProfilerColors
                .chartAreaColors[(uint) m_Series.Length %
                                 ProfilerColors.chartAreaColors.Length];
        Chart.DrawVerticalLine(marker.frame, m_Data, rect,
                               color.AlphaMultiplied(0.4f), 1.0f);
      }
  ...
  private void DrawMarkerLabels(ChartViewData cdata, Rect r,
                                EventMarker[] markers, string[] markerNames) {
    ...
    if (maxCount != 0) {
      ...
      for (int s = 0; s < markers.Length; ++s) {
        ...
        if (!markerLastFrame.TryGetValue(markers[s].nameOffset, out lframe) ||
            lframe != frame - 1 || lframe < cdata.chartDomainOffset) {
          ...
          if (frame >= 0) {
            ...
            Chart.DoLabel(xpos + offset,
                          r.y + r.height - (s % maxCount + 1) * labelPositions,
                          markerNames[s], 0);
          }
        }
  ...
  protected override Rect DoSeriesList(Rect position, int chartControlID,
                                       ChartType chartType,
                                       ChartViewData cdata) {
    Rect elementPosition =
        base.DoSeriesList(position, chartControlID, chartType, cdata);
    GUIContent label = EditorGUIUtility.TempContent("Markers");
    Color color =
        ProfilerColors.chartAreaColors[cdata.numSeries %
                                       ProfilerColors.chartAreaColors.Length];
