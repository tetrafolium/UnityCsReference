...
internal class UISystemProfiler {
  private readonly SplitterState m_TreePreviewHorizontalSplitState =
      SplitterState.FromRelative(new[]{70f, 30f}, new[]{100f, 100f}, null);
  ...
  public UISystemProfiler() { setupPreviewTextureMaterial(); }
  ...
  internal void DrawUIPane(IProfilerWindowController win) {
    ...
    var treeRect = EditorGUILayout.GetControlRect(GUILayout.ExpandWidth(true),
    ...
    if (m_TreeViewControl.property == null ||
        (m_UGUIProfilerTreeViewState != null &&
    ...
    if (m_TreeViewControl.property != null &&
        !m_TreeViewControl.property.frameDataReady) {
      m_TreeViewControl.property.Dispose();
    ...
    if (!detachPreview) {
      using(new EditorGUILayout.VerticalScope()) {
        using(new EditorGUILayout.HorizontalScope(
            EditorStyles.toolbar, GUILayout.ExpandWidth(true))) {
          detachPreview =
              GUILayout.Button(Styles.contentDetachRender,
      ...
      SplitterGUILayout
          .EndHorizontalSplit(); // m_TreePreviewHorizontalSplitState

      // Draw separator
      EditorGUI.DrawRect(
          new Rect(m_TreePreviewHorizontalSplitState.realSizes[0] +
                       treeRect.xMin,
                   treeRect.y, 1, treeRect.height),
  ...
  internal static void DrawPreviewToolbarButtons() {
    PreviewBackground = (Styles.PreviewBackgroundType) EditorGUILayout.IntPopup(
        GUIContent.none, (int) PreviewBackground, Styles.backgroundOptions,
        Styles.backgroundValues, EditorStyles.toolbarDropDown,
        GUILayout.MaxWidth(110));

    PreviewRenderMode = (Styles.RenderMode) EditorGUILayout.IntPopup(
        GUIContent.none, (int) PreviewRenderMode, Styles.rendermodeOptions,
        Styles.rendermodeValues, EditorStyles.toolbarDropDown,
        GUILayout.MaxWidth(150));
  }

  private static Styles.RenderMode PreviewRenderMode {
    get {
      return (Styles.RenderMode) EditorPrefs.GetInt(Styles.PrefOverdraw, 0);
    }
  ...
  private static Styles.PreviewBackgroundType PreviewBackground {
    get {
      return (Styles.PreviewBackgroundType)
          EditorPrefs.GetInt(Styles.PrefCheckerBoard, 0);
    }
  ...
  internal void DrawRenderUI() {
    ...
    var previewRect = GUILayoutUtility.GetRect(
        GUIContent.none, s_PreviewBackground, GUILayout.ExpandWidth(true),
        GUILayout.ExpandHeight(true));

    GUI.Box(previewRect, GUIContent.none, s_PreviewBackground);
    m_ZoomablePreview.BeginViewGUI();
    bool first = true;
    if (m_UGUIProfilerTreeViewState != null &&
        Event.current.type == EventType.Repaint) {
      IList<int> selection = m_TreeViewControl.GetSelection();
      if (selection.Count > 0) {
        IList<TreeViewItem> selectedRows =
            m_TreeViewControl.GetRowsFromIDs(selection);
        foreach (TreeViewItem row in selectedRows) {
          ...
          if (batch != null) {
            image = m_RenderService.GetThumbnail(
                currentFrame, batch.renderDataIndex, 1,
                previewRenderMode != Styles.RenderMode.Standard);
          }
          var canvas = row as UISystemProfilerTreeView.CanvasTreeViewItem;
          if (canvas != null) {
            image = m_RenderService.GetThumbnail(
                currentFrame, canvas.info.renderDataIndex,
                canvas.info.renderDataCount,
                previewRenderMode != Styles.RenderMode.Standard);
          }
          ...
          if (image) {
            ...
            float scaleFactor =
                Math.Min(previewRect.width / w, previewRect.height / h);
            w *= scaleFactor;
            h *= scaleFactor;
            var imageRect =
                new Rect(previewRect.x + (previewRect.width - w) / 2,
                         previewRect.y + (previewRect.height - h) / 2, w, h);
            ...
            if (first) {
              ...
              if (previewBackground ==
                  Styles.PreviewBackgroundType.Checkerboard)
                EditorGUI.DrawTransparencyCheckerTexture(
                    m_ZoomablePreview.drawRect, ScaleMode.ScaleAndCrop, 0f);
              else
                EditorGUI.DrawRect(
                    m_ZoomablePreview.drawRect,
                    previewBackground ==
                        Styles.PreviewBackgroundType.Black ? Color.black
                    : Color.white);
            }

            Graphics.DrawTexture(m_ZoomablePreview.drawRect, image,
                                 m_ZoomablePreview.shownArea, 0, 0, 0, 0,
                                 previewRenderMode ==
                                     Styles.RenderMode.CompositeOverdraw ?
                                         m_CompositeOverdrawMaterial
                                 : previewTextureMaterial);
          }
  ...
  private void InitIfNeeded(IProfilerWindowController win) {
    if (m_ZoomablePreview != null)
      return;

    m_ZoomablePreview = new ZoomableArea(true, false){
        hRangeMin = 0.0f, vRangeMin = 0.0f, hRangeMax = 1.0f, vRangeMax = 1.0f};
    ...
    m_MulticolumnHeaderState = new MultiColumnHeaderState(new[]{
        new MultiColumnHeaderState.Column{
            headerContent = EditorGUIUtility.TrTextContent("Object"),
            width = 220, maxWidth = 400, canSort = true},
        new MultiColumnHeaderState.Column{
            headerContent = EditorGUIUtility.TrTextContent("Self Batch Count"),
            width = initwidth, maxWidth = maxWidth},
        new MultiColumnHeaderState.Column{
            headerContent =
                EditorGUIUtility.TrTextContent("Cumulative Batch Count"),
            width = initwidth, maxWidth = maxWidth},
        new MultiColumnHeaderState.Column{
            headerContent = EditorGUIUtility.TrTextContent("Self Vertex Count"),
            width = initwidth, maxWidth = maxWidth},
        new MultiColumnHeaderState.Column{
            headerContent =
                EditorGUIUtility.TrTextContent("Cumulative Vertex Count"),
            width = initwidth, maxWidth = maxWidth},
        new MultiColumnHeaderState.Column{
            headerContent =
                EditorGUIUtility.TrTextContent("Batch Breaking Reason"),
            width = 220, maxWidth = 400, canSort = false},
        new MultiColumnHeaderState.Column{
            headerContent = EditorGUIUtility.TrTextContent("GameObject Count"),
            width = initwidth, maxWidth = 400},
        new MultiColumnHeaderState.Column{
            headerContent = EditorGUIUtility.TrTextContent("GameObjects"),
            width = 150, maxWidth = 400, canSort = false},
    });
    foreach (var column in m_MulticolumnHeaderState.columns) {
      column.sortingArrowAlignment = TextAlignment.Right;
    }

    m_UGUIProfilerTreeViewState =
        new UISystemProfilerTreeView.State{profilerWindow = win};
    var multiColumnHeader =
        new Headers(m_MulticolumnHeaderState){canSort = true, height = 21};
    multiColumnHeader.sortingChanged +=
        header => { m_TreeViewControl.Reload(); };
    m_TreeViewControl = new UISystemProfilerTreeView(
        m_UGUIProfilerTreeViewState, multiColumnHeader);
    m_TreeViewControl.Reload();
  }
  ...
  internal class Headers : MultiColumnHeader {
    ...
    ColumnHeaderGUI(MultiColumnHeaderState.Column column, Rect headerRect,
                    int columnIndex) {
      GUIStyle style = GetStyleWrapped(column.headerTextAlignment);

      float labelHeight =
          style.CalcHeight(column.headerContent, headerRect.width);
    ...
    internal override void DrawDivider(Rect dividerRect,
                                       MultiColumnHeaderState.Column column) {
      // do NOT draw the divider
      // base.DrawDivider(dividerRect, column);
    }

    internal override Rect GetArrowRect(MultiColumnHeaderState.Column column,
                                        Rect headerRect) {
      return new Rect(headerRect.xMax - DefaultStyles.arrowStyle.fixedWidth,
                      headerRect.y + 5, DefaultStyles.arrowStyle.fixedWidth,
    ...
    private GUIStyle GetStyleWrapped(TextAlignment alignment) {
      ...
      case TextAlignment.Left:
        return Styles.columnHeader;
      case TextAlignment.Center:
        return Styles.columnHeaderCenterAligned;
      case TextAlignment.Right:
        return Styles.columnHeaderRightAligned;
      default:
        return Styles.columnHeader;
      }
    }
  ...
  internal static class Styles {
    ...
    public static Color separatorColor {
      get {
        return EditorGUIUtility.isProSkin ? m_SeparatorColorPro
            : m_SeparatorColorNonPro;
      }
    }
    ...
    static Styles() {
      ...
      columnHeaderCenterAligned =
          new GUIStyle(columnHeader){alignment = TextAnchor.MiddleCenter};
      columnHeaderRightAligned =
          new GUIStyle(columnHeader){alignment = TextAnchor.MiddleRight};

      background = "OL Box";
      header = "OL title";
      header.alignment = TextAnchor.MiddleLeft;

      noData = EditorGUIUtility.TrTextContent(
          "No frame data available - UI profiling is only available when profiling in the editor");

      contentDetachRender = EditorGUIUtility.TrTextContent("Detach");

      backgroundOptions = new[]{EditorGUIUtility.TrTextContent("Checkerboard"),
                                EditorGUIUtility.TrTextContent("Black"),
                                EditorGUIUtility.TrTextContent("White")};
      backgroundValues = new[]{(int) PreviewBackgroundType.Checkerboard,
                               (int) PreviewBackgroundType.Black,
                               (int) PreviewBackgroundType.White};

      rendermodeOptions =
          new[]{EditorGUIUtility.TrTextContent("Standard"),
                EditorGUIUtility.TrTextContent("Overdraw"),
                EditorGUIUtility.TrTextContent("Composite overdraw")};
      rendermodeValues =
          new[]{(int) RenderMode.Standard, (int) RenderMode.Overdraw,
                (int) RenderMode.CompositeOverdraw};
      m_SeparatorColorPro = new Color(0.15f, 0.15f, 0.15f);
      m_SeparatorColorNonPro = new Color(0.6f, 0.6f, 0.6f);
    }

    internal enum RenderMode { Standard, Overdraw, CompositeOverdraw }

    internal enum PreviewBackgroundType { Checkerboard, Black, White }
  }

  public void CurrentAreaChanged(ProfilerArea? newArea) {
    if (!newArea.HasValue || newArea.Value != ProfilerArea.UI &&
                                 newArea.Value != ProfilerArea.UIDetails) {
      if (m_DetachedPreview) {
