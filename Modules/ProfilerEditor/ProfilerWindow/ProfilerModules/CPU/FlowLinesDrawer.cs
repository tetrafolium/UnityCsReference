...
readonly Color[] m_CachedSelectedLineColors = new Color[] {
	Styles.selectedColor, Styles.selectedColor, Styles.selectedColor
};
Vector2 m_BeginEventPosition = k_InvalidPosition;
Dictionary<int, Vector2> m_LatestNextEventPositionsPerThread =
	new Dictionary<int, Vector2>();
Dictionary<uint, Vector2> m_LatestNextEventPositionsPerFlow =
	new Dictionary<uint, Vector2>();
List<Vector2> m_EndEventPositions = new List<Vector2>();
Dictionary<uint, int> m_LatestEndEventPositionIndexesPerFlow =
	new Dictionary<uint, int>();
SelectedEventData m_SelectedEvent;

public bool hasSelectedEventPosition {
	get { return m_SelectedEvent != null; }
}

bool hasBeginEventPosition {
	get { return (m_BeginEventPosition != k_InvalidPosition); }
}

bool hasAtLeastOneNextEventPosition {
	get { return (m_LatestNextEventPositionsPerThread.Count > 0); }
}

bool hasEndEventPosition {
	get { return (m_EndEventPositions.Count > 0); }
}

bool canDraw {
	get { return (hasBeginEventPosition && hasAtLeastOneNextEventPosition); }
}

public void
AddFlowEvent(ProfilerTimelineGUI.ThreadInfo.FlowEventData flowEventData,
             int threadIndex, Rect sampleRect, bool isSelectedSample) {
	...
	case ProfilerFlowEventType.Next:
		ProcessSampleRectForNextEvent(sampleRect, threadIndex,
		                              flowEvent.FlowId);
		break;
	...
	if (flowEventType != ProfilerFlowEventType.Begin) {
		if (isSelectedSample && !hasSelectedEventPosition) {
			...
			switch (flowEventType) {
			case ProfilerFlowEventType.Next:
				selectedFlowEventPosition =
					new Vector2(sampleRect.xMin, sampleRect.yMax);
				break;

			case ProfilerFlowEventType.End:
				selectedFlowEventPosition =
					new Vector2(sampleRect.xMax, sampleRect.yMax);
				break;
...
public void Draw() {
	...
	var verticalLineOrigin =
		new Vector2(m_BeginEventPosition.x + (beginIndicatorSize.x * 0.5f),
		            m_BeginEventPosition.y + beginIndicatorSize.y);
...
void ProcessSampleRectForNextEvent(Rect sampleRect, int threadIndex,
                                   uint flowEventId) {
	// Store the position of the latest next event per thread. We use this to
	// draw a single horizontal line to the latest next event. We can use the
	// bottom-left position (where we draw to) as samples won't overlap on a
	// single thread.
	var bottomLeftSampleRectPosition =
		new Vector2(sampleRect.xMin, sampleRect.yMax);
	Vector2 latestNextEventPositionForThread;
	if (m_LatestNextEventPositionsPerThread.TryGetValue(
		    threadIndex, out latestNextEventPositionForThread)) {
		if (bottomLeftSampleRectPosition.x >
		    latestNextEventPositionForThread.x) {
			m_LatestNextEventPositionsPerThread[threadIndex] =
				bottomLeftSampleRectPosition;
		}
	} else {
		m_LatestNextEventPositionsPerThread[threadIndex] =
			bottomLeftSampleRectPosition;
	...
	// Store the position of the latest next event per flow. We use this when
	// drawing each flow's end line.
	var bottomRightSampleRectPosition =
		new Vector2(sampleRect.xMax, sampleRect.yMax);
	Vector2 latestNextEventPositionForFlow;
	if (m_LatestNextEventPositionsPerFlow.TryGetValue(
		    flowEventId, out latestNextEventPositionForFlow)) {
		if (bottomRightSampleRectPosition.x >
		    latestNextEventPositionForFlow.x) {
			m_LatestNextEventPositionsPerFlow[flowEventId] =
				bottomRightSampleRectPosition;
		}
	} else {
		m_LatestNextEventPositionsPerFlow[flowEventId] =
			bottomRightSampleRectPosition;
...
void ProcessSampleRectForEndEvent(Rect sampleRect, uint flowEventId) {
	...
	// Store the position of the latest end event per flow. We use this when
	// drawing each flow's end line.
	int latestEndEventPositionIndexForFlow;
	if (m_LatestEndEventPositionIndexesPerFlow.TryGetValue(
		    flowEventId, out latestEndEventPositionIndexForFlow)) {
		var latestEndEventPositionForFlow =
			m_EndEventPositions[latestEndEventPositionIndexForFlow];
...
void DrawBeginToNextFlowLines(Vector2 verticalLineOrigin) {
	...
	var horizontalLinePositions =
		CollectHorizontalBeginToNextFlowLinePositionsAndTrackVerticalRange(
			verticalLineOrigin.x, ref lowestVerticalSamplePosition,
			ref highestVerticalSamplePosition);
	DrawLines(horizontalLinePositions);

	DrawVerticalBeginToNextFlowLines(verticalLineOrigin,
	                                 lowestVerticalSamplePosition,
	                                 highestVerticalSamplePosition);
}

Vector3
[] CollectHorizontalBeginToNextFlowLinePositionsAndTrackVerticalRange(
	float horizontalOrigin, ref float lowestVerticalSamplePosition,
	ref float highestVerticalSamplePosition) {
	const int pointsPerLine = 2;
	Vector3[] linePositions =
		new Vector3[m_LatestNextEventPositionsPerThread.Count *
		            pointsPerLine];

	int index = 0;
	foreach (KeyValuePair<int, Vector2> kvp in
	         m_LatestNextEventPositionsPerThread) {
		var highestHorizontalNextEventSamplePosition = kvp.Value;
		var origin = new Vector2(horizontalOrigin,
		                         highestHorizontalNextEventSamplePosition.y);
...
void DrawVerticalBeginToNextFlowLines(Vector2 origin,
                                      float lowestVerticalSamplePosition,
                                      float highestVerticalSamplePosition) {
...
void DrawNextToEndFlowLines() {
	...
	Vector3[] linePositions =
		CollectLatestNextEventToLatestEndEventLinePositionsPerFlow();
...
Vector3[] CollectLatestNextEventToLatestEndEventLinePositionsPerFlow() {
	...
	Vector3[] linePositions =
		new Vector3[m_LatestEndEventPositionIndexesPerFlow.Count *
		            k_NumberOfPointsPerEndEvent];
	int index = 0;
	foreach (KeyValuePair<uint, int> latestEndEventPositionIndexKvp in
	         m_LatestEndEventPositionIndexesPerFlow) {
		var flowId = latestEndEventPositionIndexKvp.Key;
		var latestEndEventPositionIndex = latestEndEventPositionIndexKvp.Value;

		Vector2 latestNextEventPositionForFlow;
		if (m_LatestNextEventPositionsPerFlow.TryGetValue(
			    flowId, out latestNextEventPositionForFlow)) {
			var latestEndEventPositionForFlow =
				m_EndEventPositions[latestEndEventPositionIndex];
			var horizontalOrigin = latestNextEventPositionForFlow;
			linePositions[index] = horizontalOrigin;
			var horizontalDestination =
				new Vector2(latestEndEventPositionForFlow.x,
				            latestNextEventPositionForFlow.y);
			linePositions[index + 1] = horizontalDestination;
			var verticalOrigin = new Vector2(latestEndEventPositionForFlow.x,
			                                 latestNextEventPositionForFlow.y);
...
void DrawSelectedLineIfNecessary(Vector2 beginEventLineOrigin) {
	if (hasSelectedEventPosition) {
		...
		case ProfilerFlowEventType.Next: {
			// If a next event is selected, draw from the begin event to the
			// selected event.
		...
		case ProfilerFlowEventType.End: {
			// If an end event is selected, draw from the last next event for this
			// flow to the selected event.
...
void DrawSelectedLine(Vector2 origin) {
	...
	if (midPoint != k_InvalidPosition) {
		...
		Handles.DrawAAPolyLine(EditorGUIUtility.whiteTexture,
		                       Styles.selectedWidth,
		                       m_CachedSelectedLinePoints);
	}
}
...
public Vector2 flowEventPosition {
	get;
	set;
}
