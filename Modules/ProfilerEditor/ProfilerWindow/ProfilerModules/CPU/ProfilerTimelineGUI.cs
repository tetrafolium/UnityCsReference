...
internal class ProfilerTimelineGUI : ProfilerFrameDataViewBase {
  ...
  static readonly float[] k_TickModulos = {
      0.001f, 0.005f, 0.01f, 0.05f, 0.1f, 0.5f,  1,     5,    10,
  ...
  internal class ThreadInfo : IComparable<ThreadInfo> {
    ...
    public ThreadInfo(string name, int threadIndex, int maxDepth,
                      int linesToDisplay) {
      this.name = name;
    ...
    public struct FlowEventData {
      ...
      public bool hasParentSampleIndex {
        get { return (flowEvent.ParentSampleIndex > 0); }
      }
    }
  }
  ...
  internal class GroupInfo {
    ...
    public GroupInfo(string name,
                     UnityEngine.Events.UnityAction foldoutStateChangedCallback)
        : this(name, foldoutStateChangedCallback, false) {}

    public GroupInfo(
        string name, UnityEngine.Events.UnityAction foldoutStateChangedCallback,
        bool expanded,
        int defaultLineCountPerThread = k_DefaultLineCountPerThread,
        float height = k_GroupHeight) {
      this.name = name;
      this.height = height;
  ...
  internal class Styles {
    ...
    public Color rangeSelectionColor =>
        EditorGUIUtility.isProSkin ? m_RangeSelectionColorDark
        : m_RangeSelectionColorLight;
    Color m_OutOfRangeColorLight = new Color32(160, 160, 160, 127);
    Color m_OutOfRangeColorDark = new Color32(40, 40, 40, 127);
    public Color outOfRangeColor =>
        EditorGUIUtility.isProSkin ? m_OutOfRangeColorDark
        : m_OutOfRangeColorLight;
  }
  ...
  class EntryInfo {
    ...
    public int nativeIndex =
        -1; // Uniquely identifies the sample for the thread and frame.
    ...
    public bool Equals(int frameId, int threadId, int nativeIndex) {
      return frameId == this.frameId && threadId == this.threadId &&
  ...
  class SelectedEntryInfo : EntryInfo {
    // The associated GameObjects instance ID. Negative means Native Object,
    // Positive means Managed Object, 0 means not set (as in, no object
    // associated)
    ...
    List<RawFrameDataView.FlowEvent> m_FlowEvents =
        new List<RawFrameDataView.FlowEvent>();

    public List<RawFrameDataView.FlowEvent> FlowEvents {
      get { return m_FlowEvents; }
    }
  ...
  [NonSerialized] ZoomableArea m_TimeArea;
  ...
  [NonSerialized] List<RawFrameDataView.FlowEvent> m_CachedThreadFlowEvents =
      new List<RawFrameDataView.FlowEvent>();

  [Flags] enum ProcessedInputs {
    MouseDown = 1 << 0,
  ...
  static readonly ProfilerMarker m_DoGUIMarker =
      new ProfilerMarker(nameof(ProfilerTimelineGUI) + ".DoGUI");
  ...
  public override void OnEnable(CPUorGPUProfilerModule cpuOrGpuModule,
                                IProfilerWindowController profilerWindow,
                                bool isGpuView) {
    base.OnEnable(cpuOrGpuModule, profilerWindow, isGpuView);
    if (m_Groups != null) {
      for (int i = 0; i < m_Groups.Count; i++) {
        m_Groups[i].expanded.value = SessionState.GetBool(
            $"Profiler.Timeline.GroupExpanded.{m_Groups[i].name}", false);
      }
    }
  }

  void RepaintProfilerWindow() { m_ProfilerWindow?.Repaint(); }

  void UpdateGroupAndThreadInfo(ProfilerFrameDataIterator iter,
                                int frameIndex) {
    ...
    for (int i = 0; i < threadCount; ++i) {
      ...
      if (thread == null) {
        // ProfilerFrameDataIterator.maxDepth includes the thread sample which
        // is not getting displayed, so we store it at -1 for all intents and
        // purposes
        thread = new ThreadInfo(iter.GetThreadName(), i, iter.maxDepth - 1,
                                group.defaultLineCountPerThread);
  ...
  float CalculateHeightForAllBars(Rect fullRect, out float combinedHeaderHeight,
                                  out float combinedThreadHeight) {
    ...
    for (int i = 0; i < m_Groups.Count; i++) {
      ...
      if (mainGroup) {
        ...
        group.height = group.expanded.value ? k_GroupHeight
            : Math.Max(group.height,
                       group.threads.Count * k_ThreadMinHeightCollapsed);
      }
  ...
  bool DrawBar(Rect r, float y, float height, string name, bool group,
               bool expanded, bool indent) {
    Rect leftRect =
        new Rect(r.x - Chart.kSideWidth, y, Chart.kSideWidth, height);
    ...
    foreach (var groupInfo in m_Groups) {
      ...
      if (!mainGroup) {
        ...
        var newExpandedState =
            DrawBar(r, y, height, groupInfo.name, true, expandedState, false);

        if (newExpandedState != expandedState) {
          SessionState.SetBool(
              $"Profiler.Timeline.GroupExpanded.{groupInfo.name}",
              newExpandedState);
  ...
  void DoNativeProfilerTimeline(Rect r, int frameIndex,
                                int maxContextFramesToShow, int threadIndex,
                                float timeOffset, bool ghost,
                                float scaleForThreadHeight) {
    // Add some margins to each thread view.
    Rect clipRect = r;
    float topMargin =
        Math.Min(clipRect.height * 0.25f,
                 1); // Reduce margin when drawing thin timelines (to more
                     // easily get an overview over collapsed threadgroups)
    ...
    GUI.BeginGroup(clipRect);
    {
      ...
      if (Event.current.type == EventType.Repaint) {
        DrawNativeProfilerTimeline(localRect, frameIndex, threadIndex,
                                   timeOffset, ghost);
      } else if (Event.current.type == EventType.MouseDown &&
                 !ghost) // Ghosts are not clickable
      {
        HandleNativeProfilerTimelineInput(
            localRect, frameIndex, maxContextFramesToShow, threadIndex,
            timeOffset, topMargin, scaleForThreadHeight);
      }
    }
    GUI.EndGroup();
  }

  void DrawNativeProfilerTimeline(Rect threadRect, int frameIndex,
                                  int threadIndex, float timeOffset,
                                  bool ghost) {
    bool hasSelection = m_SelectedEntry.threadId == threadIndex &&
                        m_SelectedEntry.frameId == frameIndex;

    NativeProfilerTimeline_DrawArgs drawArgs =
        new NativeProfilerTimeline_DrawArgs();
    drawArgs.Reset();
    ...
    drawArgs.threadRect.yMax =
        Mathf.Min(drawArgs.threadRect.yMax,
                  m_TimeArea.shownArea.height - m_TimeArea.hSliderHeight);
    drawArgs.shownAreaRect = m_TimeArea.shownArea;
    drawArgs.selectedEntryIndex =
        hasSelection ? m_SelectedEntry.nativeIndex : -1;
    drawArgs.mousedOverEntryIndex = -1;

    NativeProfilerTimeline.Draw(ref drawArgs);
  }

  void HandleNativeProfilerTimelineInput(Rect threadRect, int frameIndex,
                                         int maxContextFramesToShow,
                                         int threadIndex, float timeOffset,
                                         float topMargin,
                                         float scaleForThreadHeight) {
    // Only let this thread view change mouse state if it contained the mouse
    // pos
    ...
    bool singleClick = Event.current.clickCount == 1 &&
                       Event.current.type == EventType.MouseDown;
    bool doubleClick = Event.current.clickCount == 2 &&
                       Event.current.type == EventType.MouseDown;
    ...
    NativeProfilerTimeline_GetEntryAtPositionArgs posArgs =
        new NativeProfilerTimeline_GetEntryAtPositionArgs();
    posArgs.Reset();
    ...
    if (mouseOverIndex != -1) {
      bool selectedChanged =
          !m_SelectedEntry.Equals(frameIndex, threadIndex, mouseOverIndex);
      if (selectedChanged) {
        // Read out timing info
        NativeProfilerTimeline_GetEntryTimingInfoArgs timingInfoArgs =
            new NativeProfilerTimeline_GetEntryTimingInfoArgs();
        ...
        NativeProfilerTimeline_GetEntryInstanceInfoArgs instanceInfoArgs =
            new NativeProfilerTimeline_GetEntryInstanceInfoArgs();
        ...
        m_SelectedEntry.totalDuration =
            timingInfoArgs.out_TotalDurationForFrame;
        m_SelectedEntry.instanceCount =
            timingInfoArgs.out_InstanceCountForFrame;
        ...
        if ((cpuModule.ViewOptions &
             CPUorGPUProfilerModule.ProfilerViewFilteringOptions
                 .ShowExecutionFlow) != 0) {
          using(var frameData = ProfilerDriver.GetRawFrameDataView(
                    frameIndex, threadIndex)) {
            // posArgs.out_EntryIndex is a MeshCache index which differs from
            // sample index by 1 as root is not included into MeshCache.
            frameData.GetSampleFlowEvents(mouseOverIndex + 1,
                                          m_SelectedEntry.FlowEvents);
            UpdateActiveFlowEventsForAllThreadsInAllVisibleFrames(
                frameIndex, maxContextFramesToShow, m_SelectedEntry.FlowEvents);
          }
        }
      }
      ...
      m_CurrentlyProcessedInputs |=
          ProcessedInputs.MouseDown | ProcessedInputs.FrameSelection;
    } else {
  ...
  public override void Clear() { InitializeNativeTimeline(); }
  ...
  void PerformFrameSelected(float frameMS, bool verticallyFrameSelected = true,
                            bool hFrameAll = false) {
    float t;
    float dt;
    ...
    if (m_SelectedEntry.instanceCount >= 0 && verticallyFrameSelected) {
      if (m_SelectedEntry.relativeYPos > m_SelectedThread.height) {
        ...
        if (selectedThread != null) {
          selectedThread.linesToDisplay =
              CalculateLineCount(m_SelectedEntry.relativeYPos + k_LineHeight);
      ...
      float yMinPosition =
          m_SelectedThreadYRange + m_SelectedEntry.relativeYPos - k_LineHeight;
      float yMaxPosition =
          m_SelectedThreadYRange + m_SelectedEntry.relativeYPos;

      if (yMinPosition < m_TimeArea.shownAreaInsideMargins.yMin) {
        yMin = yMinPosition;
        yMax = Mathf.Min(yMin + m_TimeArea.shownArea.height,
                         m_TimeArea.vBaseRangeMax);
      }

      if (yMaxPosition >
          m_TimeArea.shownAreaInsideMargins.yMax - m_TimeArea.hSliderHeight) {
        yMax = yMaxPosition + m_TimeArea.hSliderHeight;
        yMin = Mathf.Max(yMax - m_TimeArea.shownArea.height,
                         m_TimeArea.vBaseRangeMin);
      }
  ...
  void DoProfilerFrame(int frameIndex, int maxContextFramesToShow,
                       Rect fullRect, bool ghost, int threadCount, float offset,
                       float scaleForThreadHeight) {
    using(var iter = new ProfilerFrameDataIterator()) {
      ...
      foreach (var groupInfo in m_Groups) {
        ...
        // When group is not expanded its header still occupies at least
        // groupInfo.height
        ...
        foreach (var threadInfo in groupInfo.threads) {
          r.y = y;
          r.height = expanded ? threadInfo.height * scaleForThreadHeight
              : Math.Max(groupInfo.height / groupThreadCount,
                         k_ThreadMinHeightCollapsed);

          var tr = r;
          tr.y -= fullRect.y;
          if (tr.yMin < m_TimeArea.shownArea.yMax &&
              tr.yMax > m_TimeArea.shownArea.yMin) {
            iter.SetRoot(frameIndex, threadInfo.threadIndex);
            DoNativeProfilerTimeline(r, frameIndex, maxContextFramesToShow,
                                     threadInfo.threadIndex, offset, ghost,
                                     scaleForThreadHeight);
          }

          // Save the y pos and height of the selected thread each time we draw,
          // since it can change
          bool containsSelected =
              m_SelectedEntry.IsValid() &&
              (m_SelectedEntry.frameId == frameIndex) &&
              (m_SelectedEntry.threadId == threadInfo.threadIndex);
          if (containsSelected) {
            m_SelectedThreadY = y;
  ...
  void DoFlowEvents(int currentFrameIndex, int firstDrawnFrameIndex,
                    int lastDrawnFrameIndex, Rect fullRect,
                    float scaleForThreadHeight) {
    if ((cpuModule.ViewOptions &
         CPUorGPUProfilerModule.ProfilerViewFilteringOptions
             .ShowExecutionFlow) == 0) {
    ...
    bool hasSelectedSampleWithFlowEvents =
        (m_SelectedEntry.FlowEvents.Count > 0) &&
        SelectedSampleIsVisible(firstDrawnFrameIndex, lastDrawnFrameIndex);
    FlowLinesDrawer activeFlowLinesDrawer =
        (hasSelectedSampleWithFlowEvents) ? new FlowLinesDrawer() : null;
    ForEachThreadInEachGroup(
        fullRect, scaleForThreadHeight, (groupInfo, threadInfo, threadRect) => {
          var groupIsExpanded = groupInfo.expanded.value;
          DrawIndicatorsForAllFlowEventsInFrameOnThread(
              currentFrameIndex, threadInfo, threadRect, fullRect,
              groupIsExpanded, hasSelectedSampleWithFlowEvents);

          if (hasSelectedSampleWithFlowEvents) {
            ProcessActiveFlowEventsOnThread(ref activeFlowLinesDrawer,
                                            threadInfo, threadRect, fullRect,
                                            currentFrameIndex, groupIsExpanded);
          }
        });
  ...
  void DrawIndicatorsForAllFlowEventsInFrameOnThread(
      int currentFrameIndex, ThreadInfo threadInfo, Rect threadRect,
      Rect fullRect, bool groupIsExpanded,
      bool hasSelectedEntryWithFlowEvents) {
    m_SamplesWithDrawnFlowEventsPerThreadCache.Clear();

    using(var frameData = ProfilerDriver.GetRawFrameDataView(
              currentFrameIndex, threadInfo.threadIndex)) {
      frameData.GetFlowEvents(m_CachedThreadFlowEvents);

      var localViewport = new Rect(Vector2.zero, fullRect.size);
      var indicatorMode =
          (hasSelectedEntryWithFlowEvents)
              ? FlowIndicatorDrawer.IndicatorAppearanceMode.Inactive
              : FlowIndicatorDrawer.IndicatorAppearanceMode.NoActiveEvents;
      foreach (var flowEvent in m_CachedThreadFlowEvents) {
        if (flowEvent.ParentSampleIndex != 0) {
          // A sample can have multiple begin flow events. Check an indicator
          // hasn't already been drawn for this sample on this thread.
          if (!SampleAlreadyHasFlowEventType(flowEvent.ParentSampleIndex,
                                             flowEvent.FlowEventType)) {
            var sampleRect = RectForSampleOnFrameInThread(
                flowEvent.ParentSampleIndex, currentFrameIndex, threadInfo,
                threadRect, 0f, m_TimeArea.shownArea,
                (groupIsExpanded == false));
            if (sampleRect.Overlaps(localViewport)) {
              FlowIndicatorDrawer.DrawFlowIndicatorForFlowEvent(
                  flowEvent, sampleRect, indicatorMode);

              // A sample can only have multiple Begin flow events so only use
              // the cache for begin events.
              if (flowEvent.FlowEventType == ProfilerFlowEventType.Begin) {
                m_SamplesWithDrawnFlowEventsPerThreadCache.Add(
                    flowEvent.ParentSampleIndex);
              }
            }
          }
        }
  ...
  void ProcessActiveFlowEventsOnThread(ref FlowLinesDrawer flowLinesDrawer,
                                       ThreadInfo threadInfo, Rect threadRect,
                                       Rect fullRect, int currentFrameIndex,
                                       bool groupIsExpanded) {
    foreach (var flowEventData in threadInfo.ActiveFlowEvents) {
      ...
      var timeOffset = ProfilerFrameTimingUtility.TimeOffsetBetweenFrames(
          currentFrameIndex, flowEventData.frameIndex);
      var sampleRect = RectForSampleOnFrameInThread(
          flowEvent.ParentSampleIndex, flowEventFrameIndex, threadInfo,
          threadRect, timeOffset, m_TimeArea.shownArea,
          (groupIsExpanded == false));

      // A flow event can have no parent sample if it is not enclosed within a
      // PROFILER_AUTO scope.
      if (flowEventData.hasParentSampleIndex) {
        ...
        if (!flowLinesDrawer.hasSelectedEventPosition) {
          isSelectedSample =
              (m_SelectedEntry.threadId == threadInfo.threadIndex) &&
              (m_SelectedEntry.frameId == flowEventFrameIndex) &&
              m_SelectedEntry.FlowEvents.Contains(flowEvent);
        }
        flowLinesDrawer.AddFlowEvent(flowEventData, threadInfo.threadIndex,
                                     sampleRect, isSelectedSample);

        // Draw indicator for this active flow event.
        var localViewport = new Rect(Vector2.zero, fullRect.size);
        if (sampleRect.Overlaps(localViewport)) {
          FlowIndicatorDrawer.DrawFlowIndicatorForFlowEvent(
              flowEvent, sampleRect,
              FlowIndicatorDrawer.IndicatorAppearanceMode.Active);
        }
      }
    }
  }

  bool SampleAlreadyHasFlowEventType(int sampleIndex,
                                     ProfilerFlowEventType flowEventType) {
  ...
  /// <summary>
  /// Iterate over each group's threads and execute the provided <paramref
  /// name="action"/> for each thread. Passes the thread's rect to the provided
  /// action, calculated by summing all previous thread rects.
  /// </summary>
  /// <param name="fullRect"></param>
  /// <param name="scaleForThreadHeight"></param>
  /// <param name="action"></param>
  void ForEachThreadInEachGroup(Rect fullRect, float scaleForThreadHeight,
                                Action<GroupInfo, ThreadInfo, Rect> action) {
    float y = fullRect.y;
    // The unscaled y value equating to the timeline Y range value.
    float rangeY = 0;
    foreach (var groupInfo in m_Groups) {
      ...
      foreach (var threadInfo in groupInfo.threads) {
        threadRect.y = y;
        threadRect.height =
            groupIsExpanded ? threadInfo.height * scaleForThreadHeight
            : Math.Max(groupInfo.height / groupThreadCount,
                       k_ThreadMinHeightCollapsed);
  ...
  Rect RectForSampleOnFrameInThread(int sampleIndex, int frameIndex,
                                    ThreadInfo threadInfo, Rect threadRect,
                                    float timeOffset, Rect shownAreaRect,
                                    bool isGroupCollapsed) {
    var positionInfoArgs = RawPositionInfoForSample(
        sampleIndex, frameIndex, threadInfo.threadIndex, threadRect, timeOffset,
        shownAreaRect);
    CorrectRawPositionInfo(isGroupCollapsed, threadInfo, threadRect,
                           ref positionInfoArgs);

    var sampleRect = new Rect(new Vector2(positionInfoArgs.out_Position.x,
                                          positionInfoArgs.out_Position.y),
                              positionInfoArgs.out_Size);
    return sampleRect;
  }

  NativeProfilerTimeline_GetEntryPositionInfoArgs
  RawPositionInfoForSample(int sampleIndex, int frameIndex, int threadIndex,
                           Rect threadRect, float timeOffset,
                           Rect shownAreaRect) {
    var positionInfoArgs =
        new NativeProfilerTimeline_GetEntryPositionInfoArgs();
  ...
  // NativeProfilerTimeline.GetEntryPositionInfo appears to return rects
  // relative to the thread rect. It also appears to not account for collapsed
  // groups and threads.
  void CorrectRawPositionInfo(
      bool isGroupCollapsed, ThreadInfo threadInfo, Rect threadRect,
      ref NativeProfilerTimeline_GetEntryPositionInfoArgs positionInfo) {
    ...
    // If the rect is below the bottom of the thread rect (i.e. the thread
    // collapsed), clamp the rect to the bottom of the thread rect.
    var positionInfoMaxY =
        positionInfo.out_Position.y + positionInfo.out_Size.y;
  ...
  bool SelectedSampleIsVisible(int firstDrawnFrameIndex,
                               int lastDrawnFrameIndex) {
    return ((m_SelectedEntry.frameId >= firstDrawnFrameIndex) &&
            (m_SelectedEntry.frameId <= lastDrawnFrameIndex));
  }
  ...
  void DoSelectionTooltip(int frameIndex, Rect fullRect) {
    ...
    string durationString = UnityString.Format(
        m_SelectedEntry.duration >= 1.0 ? "{0:f2}ms" : "{0:f3}ms",
        m_SelectedEntry.duration);

    System.Text.StringBuilder text = new System.Text.StringBuilder();
    text.Append(
        UnityString.Format("{0}\n{1}", m_SelectedEntry.name, durationString));

    // Show total duration if more than one instance
    if (m_SelectedEntry.instanceCount > 1) {
      string totalDurationString = UnityString.Format(
          m_SelectedEntry.totalDuration >= 1.0 ? "{0:f2}ms" : "{0:f3}ms",
          m_SelectedEntry.totalDuration);
      text.Append(
          string.Format("\n{0}: {1} ({2} {3})", styles.localizedStringTotal,
                        totalDurationString, m_SelectedEntry.instanceCount,
                        styles.localizedStringInstances));
    }
    ...
    float maxYPosition = Mathf.Min(fullRect.yMax, selectedThreadYOffset +
                                                      m_SelectedThread.height);

    // calculate how much of the line height is visible (needed for calculating
    // the offset of the tooltip when flipping)
    float selectedLineHeightVisible =
        Mathf.Clamp(maxYPosition - (selectedY - k_LineHeight), 0, k_LineHeight);

    // keep the popup within the drawing area and thread rect
    selectedY = Mathf.Clamp(selectedY, fullRect.y, maxYPosition);

    float x = m_TimeArea.TimeToPixel(
        m_SelectedEntry.time + m_SelectedEntry.duration * 0.5f, fullRect);
    ShowLargeTooltip(new Vector2(x, selectedY), fullRect, text.ToString(),
                     selectedLineHeightVisible);
  }
  ...
  void PrepareTicks() {
    m_HTicks.SetRanges(m_TimeArea.shownArea.xMin, m_TimeArea.shownArea.xMax,
                       m_TimeArea.drawRect.xMin, m_TimeArea.drawRect.xMax);
    m_HTicks.SetTickStrengths(TimeArea.kTickRulerDistMin,
                              TimeArea.kTickRulerDistFull, true);
  }
  ...
  void DrawGrid(Rect rect, float frameTime) {
    ...
    TimeArea.DrawVerticalLineFast(m_TimeArea.TimeToPixel(0, rect), 0,
                                  rect.height, styles.frameDelimiterColor);
    TimeArea.DrawVerticalLineFast(m_TimeArea.TimeToPixel(frameTime, rect), 0,
                                  rect.height, styles.frameDelimiterColor);
  ...
  void DoTimeRulerGUI(Rect timeRulerRect, float sideWidth, float frameTime) {
    // m_TimeArea shouldn't ever be null when this method is called, but just to
    // make sure in case the call to this changes.
    if (Event.current.type != EventType.Repaint || m_TimeArea == null)
      return;

    Rect sidebarLeftOfTimeRulerRect = new Rect(
        timeRulerRect.x - sideWidth, timeRulerRect.y, sideWidth, k_LineHeight);
    timeRulerRect.width -= m_TimeArea.vSliderWidth;
    Rect spaceRightOftimeRulerRect =
        new Rect(timeRulerRect.xMax, timeRulerRect.y, m_TimeArea.vSliderWidth,
                 timeRulerRect.height);

    styles.leftPane.Draw(sidebarLeftOfTimeRulerRect, GUIContent.none, false,
                         false, false, false);

    styles.leftPane.Draw(spaceRightOftimeRulerRect, GUIContent.none, false,
                         false, false, false);
    ...
    if (Event.current.type == EventType.Repaint) {
      ...
      for (int l = 0; l < m_HTicks.tickLevels; l++) {
        ...
        for (int i = 0; i < ticks.Length; i++) {
          ...
          var height = timeRulerRect.height * Mathf.Min(1, strength) *
                       TimeArea.kTickRulerHeightMax;
          var color =
              new Color(1, 1, 1, strength / TimeArea.kTickRulerFatThreshold) *
              baseColor;
          TimeArea.DrawVerticalLineFast(x, timeRulerRect.height - height + 0.5f,
                                        timeRulerRect.height - 0.5f, color);
        }
      }
    ...
    for (int i = 0; i < labelTicks.Length; i++) {
      ...
      GUI.Label(new Rect(labelpos + 3, -3, labelWidth, 20), label,
                styles.timelineTick);
    }
  ...
  string FormatTickLabel(float time, int level) {
    ...
    return UnityString.Format(
        format, time.ToString("N" + Mathf.Max(0, -log10),
  ...
  void DrawOutOfRangeOverlay(Rect rect, float frameTime) {
    ...
    if (frameStartPixel > rect.xMin) {
      var startRect =
          Rect.MinMaxRect(rect.xMin, rect.yMin,
                          Mathf.Min(frameStartPixel, rect.xMax), rect.yMax);
      EditorGUI.DrawRect(startRect, color);
      TimeArea.DrawVerticalLine(startRect.xMax, startRect.yMin, startRect.yMax,
                                lineColor);
    }

    // Rect shaded shape drawn after selected frame
    if (frameEndPixel < rect.xMax) {
      var endRect = Rect.MinMaxRect(Mathf.Max(frameEndPixel, rect.xMin),
                                    rect.yMin, rect.xMax, rect.yMax);
      EditorGUI.DrawRect(endRect, color);
      TimeArea.DrawVerticalLine(endRect.xMin, endRect.yMin, endRect.yMax,
                                lineColor);
    }
  }
  ...
  void DrawRangeSelectionOverlay(Rect rect) {
    ...
    var selectionRect =
        Rect.MinMaxRect(Mathf.Max(rect.xMin, startPixel), rect.yMin,
                        Mathf.Min(rect.xMax, endPixel), rect.yMax);
    EditorGUI.DrawRect(selectionRect, styles.rangeSelectionColor);

    // Duration label
    var labelText = UnityString.Format(
        k_TickFormatMilliseconds,
        m_RangeSelection.duration.ToString(
            "N3", CultureInfo.InvariantCulture.NumberFormat));
    Chart.DoLabel(startPixel + (endPixel - startPixel) / 2, rect.yMin + 3,
                  labelText, -0.5f);
  }
  ...
  void DoTimeArea() {
    ...
    if (previousHotControl != GUIUtility.hotControl &&
      ...
      m_CurrentlyProcessedInputs |=
          ProcessedInputs.MouseDown | ProcessedInputs.PanningOrZooming;
  }

  void DoThreadSplitters(Rect fullThreadsRect,
                         Rect fullThreadsRectWithoutSidebar, int frame,
                         ThreadSplitterCommand command) {
    ...
    foreach (var group in m_Groups) {
      ...
      case ThreadSplitterCommand.HandleThreadSplitter:
        // Another Splitter for the last thread of the previous group on the
        // bottom line of this groups header the first group is always main and
        // has no header so that case can be ignored
        if (lastThreadInLastExpandedGroup != null && group.height > 0f)
          HandleThreadSplitter(lastExpandedGroup, lastThreadInLastExpandedGroup,
                               fullThreadsRect,
                               headerHeight + threadHeight -
                                   lastThreadInLastExpandedGroup.height);
        break;
      ...
      foreach (var thread in group.threads) {
        if (group.expanded.value) {
          switch (command) {
          case ThreadSplitterCommand.HandleThreadSplitter:
            HandleThreadSplitter(group, thread, fullThreadsRect,
                                 headerHeight + threadHeight);
            HandleThreadSplitterFoldoutButtons(
                group, thread, fullThreadsRectWithoutSidebar,
                headerHeight + threadHeight,
                HandleThreadSplitterFoldoutButtonsCommand.OnlyHandleInput);
            break;
          case ThreadSplitterCommand.HandleThreadSplitterFoldoutButtons:
            HandleThreadSplitterFoldoutButtons(
                group, thread, fullThreadsRectWithoutSidebar,
                headerHeight + threadHeight,
                HandleThreadSplitterFoldoutButtonsCommand.OnlyDraw);
            break;
          default:
            break;
          }
      ...
      lastThreadInLastExpandedGroup =
          group.expanded.value ? lastThread : lastThreadInLastExpandedGroup;
      lastExpandedGroup = group.expanded.value ? group : lastExpandedGroup;
    }
  }

  float CalculateMaxYPositionForThread(float lineCount,
                                       float yPositionOfFirstThread,
                                       float yOffsetForThisThread) {
    float threadHeight = CalculateThreadHeight(lineCount);
    threadHeight *= m_TimeArea.scale.y;
    yOffsetForThisThread *= m_TimeArea.scale.y;
    return yPositionOfFirstThread + scrollOffsetY + yOffsetForThisThread +
           threadHeight;
  }

  float CalculateThreadHeight(float lineCount) {
    // add a bit of extra height through kExtraHeightPerThread to give a sneak
    // peek at the next line
    return lineCount * k_FullThreadLineHeight + k_ExtraHeightPerThread;
  }

  int CalculateLineCount(float threadHeight) {
    return Mathf.RoundToInt((threadHeight - k_ExtraHeightPerThread) /
                            k_FullThreadLineHeight);
  }

  static bool CheckForExclusiveSplitterInput(ProcessedInputs inputs) {
    // check if there is a MouseDown event happening that is just moving a
    // splitter and doing nothing else
    return inputs > 0 && (inputs & ProcessedInputs.SplitterMoving) > 0 &&
           (inputs & ProcessedInputs.MouseDown) > 0 &&
           inputs - (inputs & ProcessedInputs.SplitterMoving) -
                   (inputs & ProcessedInputs.MouseDown) ==
               0;
  }

  void HandleThreadSplitter(GroupInfo group, ThreadInfo thread,
                            Rect fullThreadsRect, float yOffsetForThisThread) {
    Rect splitterRect = new Rect(
        fullThreadsRect.x + 1,
        CalculateMaxYPositionForThread(thread.linesToDisplay, fullThreadsRect.y,
                                       yOffsetForThisThread) -
            (k_ThreadSplitterHandleSize / 2f),
        fullThreadsRect.width - 2, k_ThreadSplitterHandleSize);

    // Handle the mouse cursor look
    if (Event.current.type == EventType.Repaint) {
      // we're in Repaint so m_CurrentlyProcessedInputs is already filled with
      // all the inputs since the last repaint
      if (CheckForExclusiveSplitterInput(m_CurrentlyProcessedInputs)) {
        // while dragging the splitter, the cursor should be
        // MouseCursor.SplitResizeUpDown regardless of where the mouse is in the
        // view
        EditorGUIUtility.AddCursorRect(fullThreadsRect,
                                       MouseCursor.SplitResizeUpDown);
      } else if (fullThreadsRect.Contains(splitterRect.center) &&
                 ((m_CurrentlyProcessedInputs & ProcessedInputs.MouseDown) ==
                      0 ||
                  (m_CurrentlyProcessedInputs - ProcessedInputs.MouseDown) >
                      0)) {
        // unless a splitter is getting dragged, only change the cursor if this
        // splitter would be in view and no other input is occurring
    ...
    if (previousHotControl != GUIUtility.hotControl) {
      ...
      if (GUIUtility.hotControl != 0) {
        // ... and took it
        m_CurrentlyProcessedInputs |=
            ProcessedInputs.MouseDown | ProcessedInputs.SplitterMoving;
        int defaultLineCountForThisThread =
            group.defaultLineCountPerThread *
            (group.name.Equals(k_MainGroupName) && thread.threadIndex == 0 ? 2
                                                                           : 1);
        m_MaxLinesToDisplayForTheCurrentlyModifiedSplitter = Mathf.Max(
            thread.linesToDisplay,
            Mathf.Max(thread.maxDepth, defaultLineCountForThisThread));
      } else {
        // ... and released it
        // so reset the lines to the nearest integer
        thread.linesToDisplay =
            Mathf.Clamp(Mathf.RoundToInt(thread.linesToDisplay), 1,
                        m_MaxLinesToDisplayForTheCurrentlyModifiedSplitter);
        m_CurrentlyProcessedInputs -=
            m_CurrentlyProcessedInputs &
            (ProcessedInputs.MouseDown | ProcessedInputs.SplitterMoving);
      }
    } else if (CheckForExclusiveSplitterInput(m_LastRepaintProcessedInputs) &&
               GUIUtility.hotControl != 0) {
      // continuous dragging
      m_CurrentlyProcessedInputs |=
          ProcessedInputs.MouseDown | ProcessedInputs.SplitterMoving;
    ...
    if (deltaY != 0f) {
      thread.linesToDisplay = Mathf.Clamp(
          (thread.linesToDisplay * k_FullThreadLineHeight + deltaY) /
              k_FullThreadLineHeight,
          1, m_MaxLinesToDisplayForTheCurrentlyModifiedSplitter);
    }
  }

  void HandleThreadSplitterFoldoutButtons(
      GroupInfo group, ThreadInfo thread, Rect fullThreadsRectWithoutSidebar,
      float yOffsetForThisThread,
      HandleThreadSplitterFoldoutButtonsCommand command) {
    int roundedLineCount = Mathf.RoundToInt(thread.linesToDisplay);
    int defaultLineCountForThisThread =
        group.defaultLineCountPerThread *
        (group.name.Equals(k_MainGroupName) && thread.threadIndex == 0 ? 2 : 1);

    // only show the button if clicking it would change anything at all
    if (thread.maxDepth > defaultLineCountForThisThread ||
        thread.maxDepth > roundedLineCount) {
      ...
      float threadYMax = CalculateMaxYPositionForThread(
          roundedLineCount, fullThreadsRectWithoutSidebar.y,
          yOffsetForThisThread);
      Vector2 expandCollapsButtonSize =
          expandCollapseButtonStyle.CalcSize(GUIContent.none);
      Rect expandCollapseButtonRect =
          new Rect(fullThreadsRectWithoutSidebar.x +
                       fullThreadsRectWithoutSidebar.width / 2 -
                       expandCollapsButtonSize.x / 2,
                   threadYMax - expandCollapsButtonSize.y,
                   expandCollapsButtonSize.x, expandCollapsButtonSize.y);
      ...
      if (GUIClip.visibleRect.Overlaps(expandCollapseButtonRect)) {
        ...
        case HandleThreadSplitterFoldoutButtonsCommand.OnlyHandleInput:
          if (GUI.Button(expandCollapseButtonRect, GUIContent.none,
                         GUIStyle.none)) {
            // Expand or collapse button expands to show all or collapses to
            // default line height
            if (expandOnButtonClick) {
              ...
              thread.linesToDisplay =
                  group.defaultLineCountPerThread *
                  (group.name.Equals(k_MainGroupName) && thread.threadIndex == 0
                       ? 2
                       : 1);
            }
        ...
        case HandleThreadSplitterFoldoutButtonsCommand.OnlyDraw:
          if (Event.current.type == EventType.Repaint && expandOnButtonClick) {
            float height = styles.bottomShadow.CalcHeight(
                GUIContent.none, fullThreadsRectWithoutSidebar.width);
            styles.bottomShadow.Draw(
                new Rect(fullThreadsRectWithoutSidebar.x, threadYMax - height,
                         fullThreadsRectWithoutSidebar.width, height),
                GUIContent.none, 0);
          }

          if (Event.current.type == EventType.Repaint) {
            expandCollapseButtonStyle.Draw(
                expandCollapseButtonRect, GUIContent.none,
                expandCollapseButtonRect.Contains(Event.current.mousePosition),
                false, false, false);
          }
  ...
  public void DoGUI(int frameIndex, Rect position, bool fetchData,
                    ref bool updateViewLive) {
    using(var iter = fetchData ? new ProfilerFrameDataIterator() : null) {
      ...
      using(m_DoGUIMarker.Auto()) {
        ...
        position.yMin -=
            1; // Workaround: Adjust the y position as a temporary fix to a 1px
               // vertical offset that need to be investigated.
        Rect fullRect = position;
        float sideWidth = Chart.kSideWidth;

        Rect timeRulerRect = new Rect(fullRect.x + sideWidth, fullRect.y,
                                      fullRect.width - sideWidth, k_LineHeight);

        Rect timeAreaRect = new Rect(
            fullRect.x + sideWidth, fullRect.y + timeRulerRect.height,
            fullRect.width - sideWidth, fullRect.height - timeRulerRect.height);
        ...
        Rect bottomLeftFillRect =
            new Rect(0, position.yMax - m_TimeArea.vSliderWidth, sideWidth - 1,
                     m_TimeArea.vSliderWidth);

        if (Event.current.type == EventType.Repaint) {
          styles.profilerGraphBackground.Draw(fullRect, false, false, false,
                                              false);
        }
        ...
        float heightForAllBars = CalculateHeightForAllBars(
            fullRect, out combinedHeaderHeight, out combinedThreadHeight);
        float emptySpaceBelowBars = k_LineHeight * 3f;

        // if needed, take up more empty space below, to fill up the
        // ZoomableArea
        emptySpaceBelowBars = Mathf.Max(
            emptySpaceBelowBars,
            timeAreaRect.height - m_TimeArea.hSliderHeight - heightForAllBars);
        ...
        if (Mathf.Abs(heightForAllBars - m_LastHeightForAllBars) >= 0.5f ||
            Mathf.Abs(fullRect.height - m_LastFullRectHeight) >= 0.5f) {
          m_LastHeightForAllBars = heightForAllBars;
          m_LastFullRectHeight = fullRect.height;

          // set V range to enforce scale of 1 and to shift the shown area up in
          // case the drawn area shrunk down
          m_TimeArea.SetShownVRange(m_TimeArea.shownArea.y,
                                    m_TimeArea.shownArea.y +
                                        m_TimeArea.drawRect.height);
        }
        ...
        Rect fullThreadsRect = new Rect(
            fullRect.x, fullRect.y + timeRulerRect.height,
            fullRect.width - m_TimeArea.vSliderWidth,
            fullRect.height - timeRulerRect.height - m_TimeArea.hSliderHeight);
        ...
        // The splitters need to be handled after the time area so that they
        // don't interfere with the input for panning/scrolling the ZoomableArea
        DoThreadSplitters(fullThreadsRect, fullThreadsRectWithoutSidebar,
                          frameIndex,
                          ThreadSplitterCommand.HandleThreadSplitter);

        Rect barsUIRect = m_TimeArea.drawRect;

        DrawGrid(barsUIRect, iter.frameTimeMS);

        Rect barsAndSidebarUIRect =
            new Rect(barsUIRect.x - sideWidth, barsUIRect.y,
                     barsUIRect.width + sideWidth, barsUIRect.height);

        if (Event.current.type == EventType.Repaint) {
          Rect leftSideRect =
              new Rect(0, position.y, sideWidth, position.height);

          styles.leftPane.Draw(leftSideRect, false, false, false, false);

          // The bar in the lower left side that fills the space next to the
          // horizontal scrollbar.
          EditorStyles.toolbar.Draw(bottomLeftFillRect, false, false, false,
                                    false);
        }
        ...
        // since the scale is not applied to the group headers, there would be
        // some height unaccounted for this calculation applies that height to
        // the threads via scaleForThreadHeight
        float heightUnaccountedForDueToNotScalingHeaders =
            m_TimeArea.scale.y * combinedHeaderHeight - combinedHeaderHeight;
        float scaleForThreadHeight =
            (combinedThreadHeight * m_TimeArea.scale.y +
             heightUnaccountedForDueToNotScalingHeaders) /
            combinedThreadHeight;
        ...
        } while (currentTime > m_TimeArea.shownArea.x &&
                 numContextFramesToShow > 0);
        ...
        while (currentFrame != -1 && currentFrame != frameIndex) {
          ...
          DoProfilerFrame(currentFrame, maxContextFramesToShow, shownBarsUIRect,
                          true, threadCount, currentTime, scaleForThreadHeight);
          currentTime += iter.frameTimeMS;
        ...
        while (currentTime <
                   m_TimeArea.shownArea.x + m_TimeArea.shownArea.width &&
               numContextFramesToShow >= 0) {
          if (frameIndex != currentFrame) {
            DoProfilerFrame(currentFrame, maxContextFramesToShow,
                            shownBarsUIRect, true, threadCount, currentTime,
                            scaleForThreadHeight);
        ...
        threadCount = 0;
        currentTime = 0;
        DoProfilerFrame(frameIndex, maxContextFramesToShow, shownBarsUIRect,
                        false, threadCount, currentTime, scaleForThreadHeight);
        DoFlowEvents(frameIndex, firstDrawnFrame, lastDrawnFrame,
                     shownBarsUIRect, scaleForThreadHeight);

        GUI.EndClip();

        // Draw Foldout Buttons on top of natively drawn bars
        DoThreadSplitters(
            fullThreadsRect, fullThreadsRectWithoutSidebar, frameIndex,
            ThreadSplitterCommand.HandleThreadSplitterFoldoutButtons);

        // Draw tooltips on top of clip to be able to extend outside of timeline
        // area
  ...
  void InitializeNativeTimeline() {
    ...
    args.profilerColorDescriptors =
        new ProfilerColorDescriptor[timelineColors.Length];

    for (int i = 0; i < timelineColors.Length; ++i) {
      args.profilerColorDescriptors[i] =
          new ProfilerColorDescriptor(timelineColors[i]);
    }

    args.showFullScriptingMethodNames =
        ((cpuModule.ViewOptions &
          CPUorGPUProfilerModule.ProfilerViewFilteringOptions
              .ShowFullScriptingMethodNames) != 0)
            ? 1
            : 0;
  ...
  void DoRangeSelection(Rect rect) {
    var controlID = EditorGUIUtility.GetControlID(
        RangeSelectionInfo.controlIDHint, FocusType.Passive);
    var evt = Event.current;

    switch (evt.GetTypeForControl(controlID)) {
    case EventType.MouseDown:
      if (EditorGUIUtility.hotControl == 0 && evt.button == 0 &&
          rect.Contains(evt.mousePosition)) {
        var delay = (DragAndDropDelay) GUIUtility.GetStateObject(
            typeof(DragAndDropDelay), controlID);
        delay.mouseDownPosition = evt.mousePosition;
        m_RangeSelection.mouseDown = true;
        m_RangeSelection.active = false;
        m_CurrentlyProcessedInputs |=
            ProcessedInputs.MouseDown | ProcessedInputs.RangeSelection;
      }
    ...
    case EventType.MouseDrag:
      if (EditorGUIUtility.hotControl == 0 && m_RangeSelection.mouseDown) {
        ...
        if (delay.CanStartDrag()) {
          ...
          m_RangeSelection.mouseDownTime =
              m_TimeArea.PixelToTime(delay.mouseDownPosition.x, rect);
          m_RangeSelection.startTime = m_RangeSelection.endTime =
              m_RangeSelection.mouseDownTime;
          ClearSelection();
    ...
    case EventType.MouseUp:
      if (EditorGUIUtility.hotControl == controlID && evt.button == 0) {
        ...
        m_CurrentlyProcessedInputs -=
            (m_CurrentlyProcessedInputs & ProcessedInputs.MouseDown) |
            (m_CurrentlyProcessedInputs & ProcessedInputs.RangeSelection);
        evt.Use();
      }
    ...
    case EventType.Repaint:
      if (m_RangeSelection.active) {
        var startPixel =
            m_TimeArea.TimeToPixel(m_RangeSelection.startTime, rect);
        var endPixel = m_TimeArea.TimeToPixel(m_RangeSelection.endTime, rect);
        if (startPixel > rect.xMax || endPixel < rect.xMin)
          break;
        ...
        var selectionRect =
            Rect.MinMaxRect(Mathf.Max(rect.xMin, startPixel), rect.yMin,
                            Mathf.Min(rect.xMax, endPixel), rect.yMax);
        styles.rectangleToolSelection.Draw(selectionRect, false, false, false,
                                           false);
      }
  ...
  internal void DrawToolbar(ProfilerFrameDataIterator frameDataIterator,
    ...
    if (frameDataIterator != null)
      DrawCPUGPUTime(frameDataIterator.frameTimeMS,
                     frameDataIterator.frameGpuTimeMS);
  ...
  void UpdateActiveFlowEventsForAllThreadsInAllVisibleFrames(
      int frameIndex, int maximumNumberOfContextFramesToShow,
      List<RawFrameDataView.FlowEvent> activeFlowEvents) {
    if (activeFlowEvents.Count == 0) {
      return;
    }

    int firstContextFrame = IndexOfFirstContextFrame(
        frameIndex, maximumNumberOfContextFramesToShow);
    ...
    while ((currentFrame != -1) && (currentFrame <= lastContextFrame)) {
      ...
      UpdateActiveFlowEventsForAllThreadsAtFrame(
          currentFrame, clearActiveFlowEvents, activeFlowEvents);

      currentFrame = ProfilerDriver.GetNextFrameIndex(currentFrame);
    }
  }

  int IndexOfFirstContextFrame(int currentFrame,
                               int maximumNumberOfContextFramesToShow) {
  ...
  void UpdateActiveFlowEventsForAllThreadsAtFrame(
      int frameIndex, bool clearThreadActiveFlowEvents,
      List<RawFrameDataView.FlowEvent> activeFlowEvents) {
    foreach (var groupInfo in m_Groups) {
      foreach (var threadInfo in groupInfo.threads) {
        ...
        using(var frameData = ProfilerDriver.GetRawFrameDataView(
                  frameIndex, threadInfo.threadIndex)) {
          frameData.GetFlowEvents(m_CachedThreadFlowEvents);
          foreach (var threadFlowEvent in m_CachedThreadFlowEvents) {
            ...
            if (existsInActiveFlowEvents) {
              var flowEventData = new ThreadInfo.FlowEventData{
                  flowEvent = threadFlowEvent, frameIndex = frameIndex};
