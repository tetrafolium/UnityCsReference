...
internal class AudioProfilerGroupInfoWrapper {
  ...
  public AudioProfilerGroupInfoWrapper(AudioProfilerGroupInfo info,
                                       string assetName, string objectName,
                                       bool addToRoot) {
    this.info = info;
...
internal class AudioProfilerGroupInfoHelper {
  ...
  public class AudioProfilerGroupInfoComparer
      : IComparer<AudioProfilerGroupInfoWrapper> {
    public ColumnIndices primarySortKey;
    public ColumnIndices secondarySortKey;
    public bool sortByDescendingOrder;

    public AudioProfilerGroupInfoComparer(ColumnIndices primarySortKey,
                                          ColumnIndices secondarySortKey,
                                          bool sortByDescendingOrder) {
      this.primarySortKey = primarySortKey;
      this.secondarySortKey = secondarySortKey;
      this.sortByDescendingOrder = sortByDescendingOrder;
    }

    private int CompareInternal(AudioProfilerGroupInfoWrapper a,
                                AudioProfilerGroupInfoWrapper b,
                                ColumnIndices key) {
      int res = 0;
      switch (key) {
      case ColumnIndices.ObjectName:
        res = a.objectName.CompareTo(b.objectName);
        break;
      case ColumnIndices.AssetName:
        res = a.assetName.CompareTo(b.assetName);
        break;
      case ColumnIndices.Volume:
        res = a.info.volume.CompareTo(b.info.volume);
        break;
      case ColumnIndices.Audibility:
        res = a.info.audibility.CompareTo(b.info.audibility);
        break;
      case ColumnIndices.PlayCount:
        res = a.info.playCount.CompareTo(b.info.playCount);
        break;
      case ColumnIndices.Is3D:
        res = (a.info.flags & AUDIOPROFILER_FLAGS_3D)
                  .CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_3D) +
              (a.info.flags & AUDIOPROFILER_FLAGS_ISSPATIAL)
                      .CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_ISSPATIAL) *
                  2;
        break;
      case ColumnIndices.IsPaused:
        res = (a.info.flags & AUDIOPROFILER_FLAGS_PAUSED)
                  .CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_PAUSED);
        break;
      case ColumnIndices.IsMuted:
        res = (a.info.flags & AUDIOPROFILER_FLAGS_MUTED)
                  .CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_MUTED);
        break;
      case ColumnIndices.IsVirtual:
        res = (a.info.flags & AUDIOPROFILER_FLAGS_VIRTUAL)
                  .CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_VIRTUAL);
        break;
      case ColumnIndices.IsOneShot:
        res = (a.info.flags & AUDIOPROFILER_FLAGS_ONESHOT)
                  .CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_ONESHOT);
        break;
      case ColumnIndices.IsStream:
        res = (a.info.flags & AUDIOPROFILER_FLAGS_STREAM)
                  .CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_STREAM);
        break;
      case ColumnIndices.IsCompressed:
        res = (a.info.flags & AUDIOPROFILER_FLAGS_COMPRESSED)
                  .CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_COMPRESSED);
        break;
      case ColumnIndices.IsLooped:
        res = (a.info.flags & AUDIOPROFILER_FLAGS_LOOPED)
                  .CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_LOOPED);
        break;
      case ColumnIndices.IsOpenMemory:
        res = (a.info.flags & AUDIOPROFILER_FLAGS_OPENMEMORY)
                  .CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_OPENMEMORY);
        break;
      case ColumnIndices.IsOpenMemoryPoint:
        res =
            (a.info.flags & AUDIOPROFILER_FLAGS_OPENMEMORYPOINT)
                .CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_OPENMEMORYPOINT);
        break;
      case ColumnIndices.IsOpenUser:
        res = (a.info.flags & AUDIOPROFILER_FLAGS_OPENUSER)
                  .CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_OPENUSER);
        break;
      case ColumnIndices.IsNonBlocking:
        res = (a.info.flags & AUDIOPROFILER_FLAGS_NONBLOCKING)
                  .CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_NONBLOCKING);
        break;
      case ColumnIndices.DistanceToListener:
        res = a.info.distanceToListener.CompareTo(b.info.distanceToListener);
        break;
      case ColumnIndices.MinDist:
        res = a.info.minDist.CompareTo(b.info.minDist);
        break;
      case ColumnIndices.MaxDist:
        res = a.info.maxDist.CompareTo(b.info.maxDist);
        break;
      case ColumnIndices.Time:
        res = a.info.time.CompareTo(b.info.time);
        break;
      case ColumnIndices.Duration:
        res = a.info.duration.CompareTo(b.info.duration);
        break;
      case ColumnIndices.Frequency:
        res = a.info.frequency.CompareTo(b.info.frequency);
        break;
      }
      return (sortByDescendingOrder) ? -res : res;
    }

    public int Compare(AudioProfilerGroupInfoWrapper a,
                       AudioProfilerGroupInfoWrapper b) {
  ...
  public static string GetColumnString(AudioProfilerGroupInfoWrapper info,
                                       ColumnIndices index) {
    ...
    case ColumnIndices.Audibility:
      return isGroup ? "" : FormatDb(info.info.audibility);
    case ColumnIndices.PlayCount:
      return isGroup ? "" : info.info.playCount.ToString();
    case ColumnIndices.Is3D:
      return isGroup ? ""
          : is3D ?((info.info.flags & AUDIOPROFILER_FLAGS_ISSPATIAL) != 0
                      ? "Spatial"
                      : "YES")
          : "NO";
    case ColumnIndices.IsPaused:
      return isGroup ? "" : (info.info.flags & AUDIOPROFILER_FLAGS_PAUSED) != 0
                 ? "YES"
                 : "NO";
    case ColumnIndices.IsMuted:
      return isGroup ? "" : (info.info.flags & AUDIOPROFILER_FLAGS_MUTED) != 0
                 ? "YES"
                 : "NO";
    case ColumnIndices.IsVirtual:
      return isGroup ? "" : (info.info.flags & AUDIOPROFILER_FLAGS_VIRTUAL) != 0
                 ? "YES"
                 : "NO";
    case ColumnIndices.IsOneShot:
      return isGroup ? "" : (info.info.flags & AUDIOPROFILER_FLAGS_ONESHOT) != 0
                 ? "YES"
                 : "NO";
    case ColumnIndices.IsStream:
      return isGroup ? "" : (info.info.flags & AUDIOPROFILER_FLAGS_STREAM) != 0
                 ? "YES"
                 : "NO";
    case ColumnIndices.IsCompressed:
      return isGroup ? ""
          : (info.info.flags & AUDIOPROFILER_FLAGS_COMPRESSED) != 0
                 ? "YES"
                 : "NO";
    case ColumnIndices.IsLooped:
      return isGroup ? "" : (info.info.flags & AUDIOPROFILER_FLAGS_LOOPED) != 0
                 ? "YES"
                 : "NO";
    case ColumnIndices.IsOpenMemory:
      return isGroup ? ""
          : (info.info.flags & AUDIOPROFILER_FLAGS_OPENMEMORY) != 0
                 ? "YES"
                 : "NO";
    case ColumnIndices.IsOpenMemoryPoint:
      return isGroup ? ""
          : (info.info.flags & AUDIOPROFILER_FLAGS_OPENMEMORYPOINT) != 0
                 ? "YES"
                 : "NO";
    case ColumnIndices.IsOpenUser:
      return isGroup ? "" : (info.info.flags & AUDIOPROFILER_FLAGS_OPENUSER) != 0
                 ? "YES"
                 : "NO";
    case ColumnIndices.IsNonBlocking:
      return isGroup ? ""
          : (info.info.flags & AUDIOPROFILER_FLAGS_NONBLOCKING) != 0
                 ? "YES"
                 : "NO";
    case ColumnIndices.DistanceToListener:
      return isGroup ? "" : !is3D ? "N/A"
          : (info.info.distanceToListener >= 1000.0f)
            ? UnityString.Format("{0:0.00} km",
                                 info.info.distanceToListener * 0.001f)
            : UnityString.Format("{0:0.00} m", info.info.distanceToListener);
    case ColumnIndices.MinDist:
      return isGroup ? "" : !is3D ? "N/A"
          : (info.info.minDist >= 1000.0f)
            ? UnityString.Format("{0:0.00} km", info.info.minDist * 0.001f)
            : UnityString.Format("{0:0.00} m", info.info.minDist);
    case ColumnIndices.MaxDist:
      return isGroup ? "" : !is3D ? "N/A"
          : (info.info.maxDist >= 1000.0f)
            ? UnityString.Format("{0:0.00} km", info.info.maxDist * 0.001f)
            : UnityString.Format("{0:0.00} m", info.info.maxDist);
    case ColumnIndices.Time:
      return isGroup ? "" : UnityString.Format("{0:0.00} s", info.info.time);
    case ColumnIndices.Duration:
      return isGroup ? "" : UnityString.Format("{0:0.00} s", info.info.duration);
    case ColumnIndices.Frequency:
      return isGroup ? UnityString.Format("{0:0.00} x", info.info.frequency)
          : (info.info.frequency >= 1000.0f)
                 ? UnityString.Format("{0:0.00} kHz",
                                      info.info.frequency * 0.001f)
                 : UnityString.Format("{0:0.00} Hz", info.info.frequency);
    }
    return "Unknown";
  }

  public static int GetLastColumnIndex() {
    return Unsupported.IsDeveloperMode() ? ((int) ColumnIndices._LastColumn - 1)
                                         : (int) ColumnIndices.Duration;
  }
}
...
internal class AudioProfilerGroupViewBackend {
  ...
  public void UpdateSorting() {
    items.Sort(new AudioProfilerGroupInfoHelper.AudioProfilerGroupInfoComparer(
        (AudioProfilerGroupInfoHelper.ColumnIndices)
            m_TreeViewState.selectedColumn,
        (AudioProfilerGroupInfoHelper.ColumnIndices)
            m_TreeViewState.prevSelectedColumn,
        m_TreeViewState.sortByDescendingOrder));
    if (OnUpdate != null)
      OnUpdate();
  }
}
...
internal class AudioProfilerGroupTreeViewState : TreeViewState {
  ...
  public int selectedColumn =
      (int) AudioProfilerGroupInfoHelper.ColumnIndices.Audibility;

  [SerializeField]
  public int prevSelectedColumn =
      (int) AudioProfilerGroupInfoHelper.ColumnIndices.Is3D;
...
internal class AudioProfilerGroupView {
  ...
  public int GetNumItemsInData() { return m_Backend.items.Count; }

  public AudioProfilerGroupView(EditorWindow editorWindow,
                                AudioProfilerGroupTreeViewState state) {
  ...
  public void Init(Rect rect, AudioProfilerGroupViewBackend backend) {
    ...
    // Default widths
    if (m_TreeViewState.columnWidths == null ||
        m_TreeViewState.columnWidths.Length == 0) {
      ...
      for (int n = 2; n < numCols; n++)
        m_TreeViewState.columnWidths[n] =
            (n == 2 || n == 3 || (n >= 11 && n <= 16)) ? 75 : 60;
    ...
    ITreeViewDataSource dataSource =
        new AudioProfilerDataSource(m_TreeView, m_Backend);
    m_TreeView.Init(rect, dataSource, gui, null);

    m_ColumnHeader =
        new AudioProfilerGroupViewColumnHeader(m_TreeViewState, m_Backend);
  ...
  public void OnGUI(Rect rect, bool allowSorting) {
    ...
    Rect headerRect =
        new Rect(rect.x, rect.y, rect.width, m_HeaderStyle.fixedHeight);
  ...
  internal class AudioProfilerGroupTreeViewItem : TreeViewItem {
    ...
    public AudioProfilerGroupTreeViewItem(int id, int depth,
                                          TreeViewItem parent,
                                          string displayName,
                                          AudioProfilerGroupInfoWrapper info)
  ...
  internal class AudioProfilerDataSource : TreeViewDataSource {
    ...
    public AudioProfilerDataSource(TreeViewController treeView,
                                   AudioProfilerGroupViewBackend backend)
        : base(treeView) {
      m_Backend = backend;
    ...
    private void FillTreeItems(AudioProfilerGroupTreeViewItem parentNode,
                               int depth, int parentId,
                               List<AudioProfilerGroupInfoWrapper> items) {
      int numChildren = 0;
      ...
      if (numChildren > 0) {
        ...
        foreach (var s in items) {
          if (parentId == (s.addToRoot ? 0 : s.info.parentId)) {
            var childNode = new AudioProfilerGroupTreeViewItem(
                s.info.uniqueId, s.addToRoot ? 1
                : depth, parentNode, s.objectName, s);
    ...
    public override void FetchData() {
      var root = new AudioProfilerGroupTreeViewItem(
          1, 0, null, "ROOT",
          new AudioProfilerGroupInfoWrapper(new AudioProfilerGroupInfo(),
                                            "ROOT", "ROOT", false));
  ...
  internal class AudioProfilerGroupViewColumnHeader {
    ...
    string[] headers = new[]{
        "Object",   "Asset",     "Volume",     "Audibility", "Plays",
        "3D",       "Paused",    "Muted",      "Virtual",    "OneShot",
        "Looped",   "Distance",  "MinDist",    "MaxDist",    "Time",
        "Duration", "Frequency", "Stream",     "Compressed", "NonBlocking",
        "User",     "Memory",    "MemoryPoint"};

    public AudioProfilerGroupViewColumnHeader(
        AudioProfilerGroupTreeViewState state,
        AudioProfilerGroupViewBackend backend) {
      m_TreeViewState = state;
      m_Backend = backend;
    ...
    public void OnGUI(Rect rect, bool allowSorting, GUIStyle headerStyle) {
      ...
      for (int i = 0; i <= lastColumnIndex; ++i) {
        Rect columnRect =
            new Rect(columnPos, 0, columnWidths[i], rect.height - 1);
        columnPos += columnWidths[i];
        Rect dragRect =
            new Rect(columnPos - dragWidth / 2, 0, dragAreaWidth, rect.height);
        ...
        if (allowSorting && Event.current.type == EventType.MouseDown &&
        ...
        if (Event.current.type == EventType.Repaint)
          EditorGUIUtility.AddCursorRect(dragRect,
  ...
  internal class AudioProfilerGroupViewGUI : TreeViewGUI {
    ...
    protected override void OnContentGUI(Rect rect, int row, TreeViewItem item,
                                         string label, bool selected,
                                         bool focused, bool useBoldFont,
                                         bool isPinging) {
      if (Event.current.type != EventType.Repaint)
        return;
      ...
      int margin = 2;
      base.OnContentGUI(
          new Rect(rect.x, rect.y, columnWidths[0] - margin, rect.height), row,
          item, label, selected, focused, useBoldFont, isPinging);
      ...
      for (int i = 1; i < columnWidths.Length; i++) {
        ...
        lineStyle.Draw(rect,
                       AudioProfilerGroupInfoHelper.GetColumnString(
                           profilerItem.info,
                           (AudioProfilerGroupInfoHelper.ColumnIndices) i),
                       false, false, selected, focused);
        Handles.color = Color.black;
        Handles.DrawLine(
            new Vector3(rect.x - margin + 1, rect.y, 0),
            new Vector3(rect.x - margin + 1, rect.y + rect.height, 0));
    ...
    float[] columnWidths {
      get {
        return ((AudioProfilerGroupTreeViewState) m_TreeView.state)
            .columnWidths;
      }
    }
  }
...
internal class AudioProfilerDSPView {
  ...
  private class AudioProfilerDSPNode {
    public AudioProfilerDSPNode(AudioProfilerDSPNode firstTarget,
                                AudioProfilerDSPInfo info, int x, int y,
                                int level) {
      ...
      this.level = level;
      this.maxY = y;
      audible = (info.flags & AUDIOPROFILER_DSPFLAGS_ACTIVE) != 0 &&
                (info.flags & AUDIOPROFILER_DSPFLAGS_BYPASS) == 0;
      if (firstTarget != null)
...
#pragma warning disable 649
  ...
  private class AudioProfilerDSPWire {
    public AudioProfilerDSPWire(AudioProfilerDSPNode source,
                                AudioProfilerDSPNode target,
                                AudioProfilerDSPInfo info) {
      this.source = source;
      this.target = target;
      this.info = info;
  ...
  private void DrawRectClipped(Rect r, Color col, string name, Rect c,
                               float zoomFactor) {
    Rect s = new Rect(r.x * zoomFactor, r.y * zoomFactor, r.width * zoomFactor,
                      r.height * zoomFactor);
  ...
  public void OnGUI(Rect clippingRect, ProfilerProperty property,
                    bool showInactiveDSPChains, bool highlightAudibleDSPChains,
                    ref float zoomFactor, ref Vector2 scrollPos) {
    if (Event.current.type == EventType.ScrollWheel &&
        clippingRect.Contains(Event.current.mousePosition) &&
        Event.current.shift) {
      float zoomStep = 1.05f;
      var oldZoomFactor = zoomFactor;
      zoomFactor *= (Event.current.delta.y > 0) ? zoomStep : (1.0f / zoomStep);
      scrollPos += (Event.current.mousePosition - scrollPos) *
    ...
    // Figure out where to place all this stuff and how much screen estate the
    // branches need.
    bool isRootNode = true;
    foreach (var info in dspInfo) {
      if (!showInactiveDSPChains &&
          (info.flags & AUDIOPROFILER_DSPFLAGS_ACTIVE) == 0)
        continue;

      if (!nodes.ContainsKey(info.id)) {
        var firstTarget =
            nodes.ContainsKey(info.target) ? nodes[info.target] : null;
        if (firstTarget != null) {
          nodes[info.id] = new AudioProfilerDSPNode(
              firstTarget, info, firstTarget.x + w + xspacing, firstTarget.maxY,
              firstTarget.level + 1);
          ...
          isRootNode = false;
          nodes[info.id] = new AudioProfilerDSPNode(
              firstTarget, info, 10 + (w / 2), 10 + (h / 2), 1);
        }
        if (firstTarget != null)
          wires.Add(
              new AudioProfilerDSPWire(nodes[info.id], firstTarget, info));
      } else {
        // Same node, but containing an additional target connection (i.e. for
        // reverb mix connections)
        wires.Add(
            new AudioProfilerDSPWire(nodes[info.id], nodes[info.target], info));
      }
    }
    ...
    foreach (var _node in nodes) {
      ...
      node.y +=
          ((node.maxY == node.y) ? (h + yspacing) : (node.maxY - node.y)) / 2;
    }
    ...
    foreach (var wire in wires) {
      ...
      var info = wire.info;
      var p1 = new Vector3((source.x - w * 0.5f) * zoomFactor,
                           source.y * zoomFactor, 0.0f);
      var p2 = new Vector3(
          (target.x + w * 0.5f) * zoomFactor,
          (target.y + wire.targetPort * portSpacing) * zoomFactor, 0.0f);
      var c1 = GetOutCode(p1, clippingRect);
      var c2 = GetOutCode(p2, clippingRect);
      if ((c1 & c2) == 0) {
        float thickness = 3.0f;
        Handles.color = new Color(
            info.weight, 0.0f, 0.0f,
            (!highlightAudibleDSPChains || source.audible) ? 1.0f : 0.4f);
    ...
    foreach (var wire in wires) {
      ...
      if (info.weight != 1.0f) {
        ...
        int cy = (target != null)
                     ? (int)(target.y + ((cx - target.x - w * 0.5f) *
                                         (float)(source.y - target.y) /
                                         (float)(source.x - target.x - w)))
                     : target.y;
        DrawRectClipped(new Rect(cx - (cw / 2), cy - (ch / 2), cw, ch),
                        new Color(1.0f, 0.3f, 0.2f,
                                  (!highlightAudibleDSPChains || source.audible)
                                      ? 1.0f
                                      : 0.4f),
                        UnityString.Format("{0:0.00}%", 100.0f * info.weight),
                        clippingRect, zoomFactor);
      }
    }
    ...
    foreach (var _node in nodes) {
      var node = _node.Value;
      var info = node.info;
      if (!nodes.ContainsKey(info.target) ||
          node.firstTarget != nodes[info.target])
        continue;
      ...
      var color = new Color(
          (!active || bypass)
              ? 0.5f
              : Mathf.Clamp(2.0f * cpuColoring * cpuLoad, 0.0f, 1.0f),
          (!active || bypass)
              ? 0.5f
              : Mathf.Clamp(2.0f - 2.0f * cpuColoring * cpuLoad, 0.0f, 1.0f),
          bypass ? 1.0f
          : active ? 0.0f : 0.5f,
      ...
      DrawRectClipped(new Rect(node.x - w * 0.5f, node.y - h * 0.5f, w, h),
                      color, name, clippingRect, zoomFactor);
      if (node.audible) {
        if (info.numLevels >= 1) {
          float p = (h - 6) * Mathf.Clamp(info.level1, 0.0f, 1.0f);
          DrawRectClipped(
              new Rect(node.x - w * 0.5f + 3, node.y - h * 0.5f + 3, 4, h - 6),
              Color.black, null, clippingRect, zoomFactor);
          DrawRectClipped(new Rect(node.x - w * 0.5f + 3,
                                   node.y - h * 0.5f - 3 + h - p, 4, p),
                          Color.red, null, clippingRect, zoomFactor);
        }
        if (info.numLevels >= 2) {
          float p = (h - 6) * Mathf.Clamp(info.level2, 0.0f, 1.0f);
          DrawRectClipped(
              new Rect(node.x - w * 0.5f + 8, node.y - h * 0.5f + 3, 4, h - 6),
              Color.black, null, clippingRect, zoomFactor);
          DrawRectClipped(new Rect(node.x - w * 0.5f + 8,
                                   node.y - h * 0.5f - 3 + h - p, 4, p),
                          Color.red, null, clippingRect, zoomFactor);
        }
      }
...
internal class AudioProfilerClipInfoWrapper {
  ...
  public AudioProfilerClipInfoWrapper(AudioProfilerClipInfo info,
                                      string assetName) {
    this.info = info;
    this.assetName = assetName;
  }
...
internal class AudioProfilerClipInfoHelper {
  ...
  public class AudioProfilerClipInfoComparer
      : IComparer<AudioProfilerClipInfoWrapper> {
    public ColumnIndices primarySortKey;
    public ColumnIndices secondarySortKey;
    public bool sortByDescendingOrder;

    public AudioProfilerClipInfoComparer(ColumnIndices primarySortKey,
                                         ColumnIndices secondarySortKey,
                                         bool sortByDescendingOrder) {
      this.primarySortKey = primarySortKey;
      this.secondarySortKey = secondarySortKey;
    ...
    private int CompareInternal(AudioProfilerClipInfoWrapper a,
                                AudioProfilerClipInfoWrapper b,
                                ColumnIndices key) {
      int res = 0;
      switch (key) {
      case ColumnIndices.AssetName:
      ...
      case ColumnIndices.LoadState:
        res = a.info.loadState.CompareTo(b.info.loadState);
        break;
      case ColumnIndices.InternalLoadState:
        res = a.info.internalLoadState.CompareTo(b.info.internalLoadState);
        break;
      case ColumnIndices.Age:
        res = a.info.age.CompareTo(b.info.age);
        break;
      case ColumnIndices.Disposed:
        res = a.info.disposed.CompareTo(b.info.disposed);
        break;
      case ColumnIndices.NumChannelInstances:
        res = a.info.numChannelInstances.CompareTo(b.info.numChannelInstances);
        break;
      }
      return (sortByDescendingOrder) ? -res : res;
    }

    public int Compare(AudioProfilerClipInfoWrapper a,
                       AudioProfilerClipInfoWrapper b) {
      int res = CompareInternal(a, b, primarySortKey);
      return (res == 0) ? CompareInternal(a, b, secondarySortKey) : res;
    }
  ...
  static string[] m_LoadStateNames = new string[]{
      "Unloaded", "Loading Base", "Loading Sub", "Loaded", "Failed"};
  static string[] m_InternalLoadStateNames =
      new string[]{"Pending", "Loaded", "Failed"};

  public static string GetColumnString(AudioProfilerClipInfoWrapper info,
                                       ColumnIndices index) {
    switch (index) {
    case ColumnIndices.AssetName:
      return info.assetName;
    ...
    case ColumnIndices.Disposed:
      return (info.info.disposed != 0) ? "YES" : "NO";
    case ColumnIndices.NumChannelInstances:
...
internal class AudioProfilerClipViewBackend {
  ...
  public void UpdateSorting() {
    items.Sort(new AudioProfilerClipInfoHelper.AudioProfilerClipInfoComparer(
...
internal class AudioProfilerClipTreeViewState : TreeViewState {
  [SerializeField]
  public int selectedColumn =
      (int) AudioProfilerClipInfoHelper.ColumnIndices.InternalLoadState;

  [SerializeField]
  public int prevSelectedColumn =
      (int) AudioProfilerClipInfoHelper.ColumnIndices.LoadState;
...
internal class AudioProfilerClipView {
  ...
  public AudioProfilerClipView(EditorWindow editorWindow,
                               AudioProfilerClipTreeViewState state) {
    m_EditorWindow = editorWindow;
    m_TreeViewState = state;
  }
  ...
  public void Init(Rect rect, AudioProfilerClipViewBackend backend) {
    ...
    m_ColumnHeader =
        new AudioProfilerClipViewColumnHeader(m_TreeViewState, m_Backend);
    m_ColumnHeader.columnWidths = m_TreeViewState.columnWidths;
    m_ColumnHeader.minColumnWidth = 30f;
  ...
  internal class AudioProfilerClipTreeViewItem : TreeViewItem {
    ...
    public AudioProfilerClipTreeViewItem(int id, int depth, TreeViewItem parent,
                                         string displayName,
                                         AudioProfilerClipInfoWrapper info)
        : base(id, depth, parent, displayName) {
      this.info = info;
    }
  }
  ...
  internal class AudioProfilerDataSource : TreeViewDataSource {
    ...
    public AudioProfilerDataSource(TreeViewController treeView,
                                   AudioProfilerClipViewBackend backend)
        : base(treeView) {
      m_Backend = backend;
      m_Backend.OnUpdate = FetchData;
      showRootItem = false;
    ...
    private void FillTreeItems(AudioProfilerClipTreeViewItem parentNode,
                               int depth, int parentId,
                               List<AudioProfilerClipInfoWrapper> items) {
      parentNode.children = new List<TreeViewItem>(items.Count);

      int uniqueId = 1;
      foreach (var s in items) {
        var childNode = new AudioProfilerClipTreeViewItem(
            ++uniqueId, 1, parentNode, s.assetName, s);
        parentNode.children.Add(childNode);
      }
    }

    public override void FetchData() {
      var root = new AudioProfilerClipTreeViewItem(
          1, 0, null, "ROOT",
          new AudioProfilerClipInfoWrapper(new AudioProfilerClipInfo(),
                                           "ROOT"));
      FillTreeItems(root, 1, 0, m_Backend.items);
      m_RootItem = root;
      // SetExpanded (m_RootItem, true);
  ...
  internal class AudioProfilerClipViewColumnHeader {
    ...
    string[] headers = new[]{"Asset", "Load State", "Internal Load State",
                             "Age",   "Disposed",   "Num Voices"};

    public AudioProfilerClipViewColumnHeader(
        AudioProfilerClipTreeViewState state,
        AudioProfilerClipViewBackend backend) {
      m_TreeViewState = state;
      m_Backend = backend;
  ...
  internal class AudioProfilerClipViewGUI : TreeViewGUI {
    ...
    protected override void OnContentGUI(Rect rect, int row, TreeViewItem item,
      ...
      for (int i = 1; i < columnWidths.Length; i++) {
        ...
        lineStyle.alignment = TextAnchor.MiddleRight;
        lineStyle.Draw(rect,
                       AudioProfilerClipInfoHelper.GetColumnString(
                           profilerItem.info,
                           (AudioProfilerClipInfoHelper.ColumnIndices) i),
                       false, false, selected, focused);
        Handles.color = Color.black;
        Handles.DrawLine(
            new Vector3(rect.x - margin + 1, rect.y, 0),
    ...
    float[] columnWidths {
      get {
        return ((AudioProfilerClipTreeViewState) m_TreeView.state).columnWidths;
      }
    }
  }
}
