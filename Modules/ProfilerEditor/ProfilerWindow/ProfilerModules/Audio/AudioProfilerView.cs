...
internal class AudioProfilerGroupInfoHelper
{
    ...
    public class AudioProfilerGroupInfoComparer : IComparer<AudioProfilerGroupInfoWrapper>
    {
        ...
        private int CompareInternal(AudioProfilerGroupInfoWrapper a, AudioProfilerGroupInfoWrapper b, ColumnIndices key)
        {
            ...
            case ColumnIndices.Is3D:
                res = (a.info.flags & AUDIOPROFILER_FLAGS_3D).CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_3D) + (a.info.flags & AUDIOPROFILER_FLAGS_ISSPATIAL).CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_ISSPATIAL) * 2;
                break;
            case ColumnIndices.IsPaused:
                res = (a.info.flags & AUDIOPROFILER_FLAGS_PAUSED).CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_PAUSED);
                break;
            case ColumnIndices.IsMuted:
                res = (a.info.flags & AUDIOPROFILER_FLAGS_MUTED).CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_MUTED);
                break;
            case ColumnIndices.IsVirtual:
                res = (a.info.flags & AUDIOPROFILER_FLAGS_VIRTUAL).CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_VIRTUAL);
                break;
            case ColumnIndices.IsOneShot:
                res = (a.info.flags & AUDIOPROFILER_FLAGS_ONESHOT).CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_ONESHOT);
                break;
            case ColumnIndices.IsStream:
                res = (a.info.flags & AUDIOPROFILER_FLAGS_STREAM).CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_STREAM);
                break;
            case ColumnIndices.IsCompressed:
                res = (a.info.flags & AUDIOPROFILER_FLAGS_COMPRESSED).CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_COMPRESSED);
                break;
            case ColumnIndices.IsLooped:
                res = (a.info.flags & AUDIOPROFILER_FLAGS_LOOPED).CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_LOOPED);
                break;
            case ColumnIndices.IsOpenMemory:
                res = (a.info.flags & AUDIOPROFILER_FLAGS_OPENMEMORY).CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_OPENMEMORY);
                break;
            case ColumnIndices.IsOpenMemoryPoint:
                res = (a.info.flags & AUDIOPROFILER_FLAGS_OPENMEMORYPOINT).CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_OPENMEMORYPOINT);
                break;
            case ColumnIndices.IsOpenUser:
                res = (a.info.flags & AUDIOPROFILER_FLAGS_OPENUSER).CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_OPENUSER);
                break;
            case ColumnIndices.IsNonBlocking:
                res = (a.info.flags & AUDIOPROFILER_FLAGS_NONBLOCKING).CompareTo(b.info.flags & AUDIOPROFILER_FLAGS_NONBLOCKING);
                break;
    ...
    public static string GetColumnString(AudioProfilerGroupInfoWrapper info, ColumnIndices index)
    {
        ...
        case ColumnIndices.Is3D:
            return isGroup ? "" : is3D ? ((info.info.flags & AUDIOPROFILER_FLAGS_ISSPATIAL) != 0 ? "Spatial" : "YES") : "NO";
        case ColumnIndices.IsPaused:
            return isGroup ? "" : (info.info.flags & AUDIOPROFILER_FLAGS_PAUSED) != 0 ? "YES" : "NO";
        case ColumnIndices.IsMuted:
            return isGroup ? "" : (info.info.flags & AUDIOPROFILER_FLAGS_MUTED) != 0 ? "YES" : "NO";
        case ColumnIndices.IsVirtual:
            return isGroup ? "" : (info.info.flags & AUDIOPROFILER_FLAGS_VIRTUAL) != 0 ? "YES" : "NO";
        case ColumnIndices.IsOneShot:
            return isGroup ? "" : (info.info.flags & AUDIOPROFILER_FLAGS_ONESHOT) != 0 ? "YES" : "NO";
        case ColumnIndices.IsStream:
            return isGroup ? "" : (info.info.flags & AUDIOPROFILER_FLAGS_STREAM) != 0 ? "YES" : "NO";
        case ColumnIndices.IsCompressed:
            return isGroup ? "" : (info.info.flags & AUDIOPROFILER_FLAGS_COMPRESSED) != 0 ? "YES" : "NO";
        case ColumnIndices.IsLooped:
            return isGroup ? "" : (info.info.flags & AUDIOPROFILER_FLAGS_LOOPED) != 0 ? "YES" : "NO";
        case ColumnIndices.IsOpenMemory:
            return isGroup ? "" : (info.info.flags & AUDIOPROFILER_FLAGS_OPENMEMORY) != 0 ? "YES" : "NO";
        case ColumnIndices.IsOpenMemoryPoint:
            return isGroup ? "" : (info.info.flags & AUDIOPROFILER_FLAGS_OPENMEMORYPOINT) != 0 ? "YES" : "NO";
        case ColumnIndices.IsOpenUser:
            return isGroup ? "" : (info.info.flags & AUDIOPROFILER_FLAGS_OPENUSER) != 0 ? "YES" : "NO";
        case ColumnIndices.IsNonBlocking:
            return isGroup ? "" : (info.info.flags & AUDIOPROFILER_FLAGS_NONBLOCKING) != 0 ? "YES" : "NO";
        case ColumnIndices.DistanceToListener:
            return isGroup ? "" : !is3D ? "N/A" : (info.info.distanceToListener >= 1000.0f) ? UnityString.Format("{0:0.00} km", info.info.distanceToListener * 0.001f) : UnityString.Format("{0:0.00} m", info.info.distanceToListener);
        case ColumnIndices.MinDist:
            return isGroup ? "" : !is3D ? "N/A" : (info.info.minDist >= 1000.0f) ? UnityString.Format("{0:0.00} km", info.info.minDist * 0.001f) : UnityString.Format("{0:0.00} m", info.info.minDist);
        case ColumnIndices.MaxDist:
            return isGroup ? "" : !is3D ? "N/A" : (info.info.maxDist >= 1000.0f) ? UnityString.Format("{0:0.00} km", info.info.maxDist * 0.001f) : UnityString.Format("{0:0.00} m", info.info.maxDist);
        case ColumnIndices.Time:
            return isGroup ? "" : UnityString.Format("{0:0.00} s", info.info.time);
        case ColumnIndices.Duration:
            return isGroup ? "" : UnityString.Format("{0:0.00} s", info.info.duration);
        case ColumnIndices.Frequency:
            return isGroup ? UnityString.Format("{0:0.00} x", info.info.frequency) : (info.info.frequency >= 1000.0f) ? UnityString.Format("{0:0.00} kHz", info.info.frequency * 0.001f) : UnityString.Format("{0:0.00} Hz", info.info.frequency);
        }
        return "Unknown";
    }
...
internal class AudioProfilerGroupView
{
    ...
    internal class AudioProfilerGroupViewGUI : TreeViewGUI
    {
        ...
        float[] columnWidths {
            get {
                return ((AudioProfilerGroupTreeViewState)m_TreeView.state).columnWidths;
            }
        }
    }
}
