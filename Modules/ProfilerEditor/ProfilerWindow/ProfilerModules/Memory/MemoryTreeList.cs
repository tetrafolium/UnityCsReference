...
internal class MemoryTreeList {
  ...
  public MemoryTreeList(IProfilerWindowController profilerWindow,
                        MemoryTreeList detailview) {
  ...
  protected virtual void SetupSplitter() {
    ...
    m_Splitter = SplitterState.FromRelative(splitterRelativeSizes,
                                            splitterMinWidths, null);
  }
  ...
  public void OnGUI() {
    ...
    m_ScrollPosition =
        GUILayout.BeginScrollView(m_ScrollPosition, styles.background);
    int row = 0;
  ...
  private bool FindNamedChild(string name, List<MemoryElement> list,
                              out MemoryElement outChild) {
  ...
  public MemoryElement GetRoot() { return m_Root; }
  ...
  protected static void DrawBackground(int row, bool selected) {
    ...
    if (Event.current.type == EventType.Repaint)
      background.Draw(currentRect, GUIContent.none, false, false, selected,
                      false);
  }
  ...
  protected static Rect GenerateRect(int row) {
    var rect =
        new Rect(1, kRowHeight * row, GUIClip.visibleRect.width, kRowHeight);
    return rect;
  }

  protected virtual void DrawData(Rect rect, MemoryElement memoryElement,
                                  int indent, int row, bool selected) {
    if (Event.current.type != EventType.Repaint)
      return;
    string displayName =
        memoryElement.name + "(" + memoryElement.memoryInfo.className + ")";
    styles.numberLabel.Draw(rect, displayName, false, false, false, selected);
  }

  protected void DrawRecursiveData(MemoryElement element, ref int row,
                                   int indent) {
  ...
  protected virtual void DrawItem(MemoryElement memoryElement, ref int row,
                                  int indent) {
    ...
    if (memoryElement.ChildCount() > 0)
      memoryElement.expanded = GUI.Toggle(toggleRect, memoryElement.expanded,
                                          GUIContent.none, styles.foldout);
  ...
  protected void RowClicked(Event evt, MemoryElement memoryElement) {
    ...
    if (evt.clickCount == 2 && memoryElement.memoryInfo != null &&
        memoryElement.memoryInfo.instanceId != 0) {
    ...
    if (m_DetailView != null)
      m_DetailView.SetRoot(
          memoryElement.memoryInfo == null ? null
          : new MemoryElement(memoryElement.memoryInfo, false));
    ...
    case KeyCode.Return:
      if (m_MemorySelection.Selected.memoryInfo != null) {
        ...
        Selection.activeInstanceID =
            m_MemorySelection.Selected.memoryInfo.instanceId;
      }
      break;
    default:
      return;
  ...
  protected void EnsureVisible() {
    ...
    m_ScrollPosition.y =
        Clamp(m_ScrollPosition.y, m_SelectionOffset - m_VisibleHeight,
              m_SelectionOffset - kRowHeight);
  }
}

class MemoryTreeListClickable : MemoryTreeList {
  public MemoryTreeListClickable(IProfilerWindowController profilerWindow,
                                 MemoryTreeList detailview)
  ...
  protected override void DrawData(Rect rect, MemoryElement memoryElement,
                                   int indent, int row, bool selected) {
    if (Event.current.type != EventType.Repaint)
      return;
    ...
    styles.numberLabel.Draw(
        rect, EditorUtility.FormatBytes(memoryElement.totalMemory), false,
        false, false, selected);
    rect.x += m_Splitter.realSizes[currentColumn++];
    rect.width = m_Splitter.realSizes[currentColumn] - kSmallMargin;

    if (memoryElement.ReferenceCount() > 0)
      styles.numberLabel.Draw(rect, memoryElement.ReferenceCount().ToString(),
