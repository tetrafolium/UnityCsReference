...
internal class MemoryElementDataManager {
  ...
  static List<MemoryElement> GenerateObjectTypeGroups(ObjectInfo[] memory,
    ...
    foreach (MemoryElement typeGroup in objectGroups) {
      ...
      if (filter == ObjectTypeFilter.Other &&
          !HasValidNames(typeGroup.children))
  ...
  static public MemoryElement GetTreeRoot(ObjectMemoryInfo[] memoryObjectList,
                                          int[] referencesIndices) {
    // run through all objects and do conversion from references to
    // referenced_by
    ...
    for (int i = 0; i < memoryObjectList.Length; i++) {
      for (int j = 0; j < memoryObjectList[i].count; j++) {
        ...
        if (allObjectInfo[referencedObjectIndex].referencedBy == null)
          allObjectInfo[referencedObjectIndex].referencedBy =
              new List<ObjectInfo>();
    ...
    root.AddChild(new MemoryElement(
        "Scene Memory",
        GenerateObjectTypeGroups(allObjectInfo, ObjectTypeFilter.Scene)));
    root.AddChild(new MemoryElement(
        "Assets",
        GenerateObjectTypeGroups(allObjectInfo, ObjectTypeFilter.Asset)));
    root.AddChild(new MemoryElement(
        "Builtin Resources",
        GenerateObjectTypeGroups(allObjectInfo,
                                 ObjectTypeFilter.BuiltinResource)));
    root.AddChild(new MemoryElement(
        "Not Saved",
        GenerateObjectTypeGroups(allObjectInfo, ObjectTypeFilter.DontSave)));
    root.AddChild(new MemoryElement(
        "Other",
        GenerateObjectTypeGroups(allObjectInfo, ObjectTypeFilter.Other)));
