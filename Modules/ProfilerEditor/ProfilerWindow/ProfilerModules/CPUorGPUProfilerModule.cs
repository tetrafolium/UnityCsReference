...
protected bool fetchData {
	get {
		return !(m_ProfilerWindow == null ||
		         (m_ProfilerWindow.IsRecording() &&
...
protected abstract string SettingsKeyPrefix {
	get;
}
string ViewTypeSettingsKey {
	get { return SettingsKeyPrefix + k_ViewTypeSettingsKey; }
}
string HierarchyViewSettingsKeyPrefix {
	get { return SettingsKeyPrefix + k_HierarchyViewSettingsKeyPrefix; }
}

string ProfilerViewFilteringOptionsKey =>
SettingsKeyPrefix + nameof(m_ProfilerViewFilteringOptions);

protected abstract ProfilerViewType DefaultViewTypeSetting {
	get;
}
...
public enum ProfilerViewFilteringOptions {
	None = 0,
	CollapseEditorBoundarySamples =
		1 << 0, // Session based override, default to off
...
static readonly GUIContent[] k_ProfilerViewFilteringOptions = {
	EditorGUIUtility.TrTextContent(
		"Collapse EditorOnly Samples",
		"Samples that are only created due to profiling the editor are collapsed by default, renamed to EditorOnly [<FunctionName>] and any GC Alloc incurred by them will not be accumulated."),
	EditorGUIUtility.TrTextContent(
		"Show Full Scripting Method Names",
		"Display fully qualified method names including assembly name and namespace."),
	EditorGUIUtility.TrTextContent(
		"Show Flow Events", "Visualize job scheduling and execution."),
};

[SerializeField] int m_ProfilerViewFilteringOptions =
	(int) ProfilerViewFilteringOptions.CollapseEditorBoundarySamples;
...
internal ProfilerFrameDataHierarchyView FrameDataHierarchyView =>
m_FrameDataHierarchyView;

internal ProfilerViewFilteringOptions ViewOptions =>
(ProfilerViewFilteringOptions) m_ProfilerViewFilteringOptions;
...
public override void OnEnable(IProfilerWindowController profilerWindow) {
	...
	if (m_FrameDataHierarchyView == null)
		m_FrameDataHierarchyView =
			new ProfilerFrameDataHierarchyView(HierarchyViewSettingsKeyPrefix);
	...
	m_ProfilerWindow.selectionChanged +=
		m_FrameDataHierarchyView.SetSelectionFromLegacyPropertyPath;
	m_ViewType = (ProfilerViewType) EditorPrefs.GetInt(
		ViewTypeSettingsKey, (int) DefaultViewTypeSetting);
	m_ProfilerViewFilteringOptions = SessionState.GetInt(
		ProfilerViewFilteringOptionsKey, m_ProfilerViewFilteringOptions);
}

public override void SaveViewSettings() {
	base.SaveViewSettings();
	EditorPrefs.SetInt(ViewTypeSettingsKey, (int) m_ViewType);
	SessionState.SetInt(ProfilerViewFilteringOptionsKey,
	                    m_ProfilerViewFilteringOptions);
...
public void DrawOptionsMenuPopup() {
	var position =
		GUILayoutUtility.GetRect(ProfilerWindow.Styles.optionsButtonContent,
		                         EditorStyles.toolbarButton);
	if (GUI.Button(position, ProfilerWindow.Styles.optionsButtonContent,
	               EditorStyles.toolbarButton)) {
		...
		for (var i = 0; i < k_ProfilerViewFilteringOptions.Length; i++) {
			...
			if (ViewType == ProfilerViewType.Timeline &&
			    option ==
			    ProfilerViewFilteringOptions.CollapseEditorBoundarySamples)
				continue;

			if (option == ProfilerViewFilteringOptions.ShowExecutionFlow &&
			    ViewType != ProfilerViewType.Timeline)
				continue;

			pm.AddItem(k_ProfilerViewFilteringOptions [i]
			           ,
			           OptionEnabled(option), () => ToggleOption(option));
		}
		pm.Popup(position, -1);
	}
}
...
bool OptionEnabled(ProfilerViewFilteringOptions option) {
	return (option & (ProfilerViewFilteringOptions)
	        m_ProfilerViewFilteringOptions) !=
	       ProfilerViewFilteringOptions.None;
}

protected virtual void ToggleOption(ProfilerViewFilteringOptions option) {
	m_ProfilerViewFilteringOptions =
		(int)((ProfilerViewFilteringOptions) m_ProfilerViewFilteringOptions ^
		      option);
	SessionState.SetInt(ProfilerViewFilteringOptionsKey,
	                    m_ProfilerViewFilteringOptions);
...
public override void DrawView(Rect position) {
	m_FrameDataHierarchyView.DoGUI(fetchData ? GetFrameDataView()
	                             : null, fetchData, ref updateViewLive);
}
...
HierarchyFrameDataView GetFrameDataView() {
	...
	return m_ProfilerWindow.GetFrameDataView(
		m_FrameDataHierarchyView.threadName, viewMode | GetFilteringMode(),
...
const int k_AnyFullManagedMarker =
	(int)(MarkerFlags.ScriptInvoke | MarkerFlags.ScriptDeepProfiler);
public string GetItemName(HierarchyFrameDataView frameData, int itemId) {
	var name = frameData.GetItemName(itemId);
	if ((ViewOptions &
	     ProfilerViewFilteringOptions.ShowFullScriptingMethodNames) != 0)
		return name;
	...
	if (namespaceDelimiterIndex < name.Length &&
	    name[namespaceDelimiterIndex] == ':')
