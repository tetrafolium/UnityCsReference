...
[SerializeField] bool m_Serialized;

[NonSerialized] int m_FrameIndex = -1;

[SerializeField] TreeViewState m_TreeViewState;

[SerializeField] MultiColumnHeaderState m_MultiColumnHeaderState;

[SerializeField] int m_ThreadIndex = 0;
...
[NonSerialized] List<ThreadInfo> m_ThreadInfoCache;
[NonSerialized] string[] m_ThreadNames;

[SerializeField] DetailedViewType m_DetailedViewType =
	DetailedViewType.None;

[SerializeField] SplitterState m_DetailedViewSpliterState;

SearchField m_SearchField;
ProfilerFrameDataTreeView m_TreeView;

[SerializeField] ProfilerDetailedObjectsView m_DetailedObjectsView;

[SerializeField] ProfilerDetailedCallsView m_DetailedCallsView;
...
[SerializeField] string m_ThreadName;
...
public int sortedProfilerColumn {
	get {
		return m_TreeView ==
		       null ? HierarchyFrameDataView.columnDontSort
	    : m_TreeView.sortedProfilerColumn;
	}
}

public bool sortedProfilerColumnAscending {
	get {
		return m_TreeView == null ? false
	    : m_TreeView.sortedProfilerColumnAscending;
	}
}
...
static readonly ProfilerMarker m_DoGUIMarker =
	new ProfilerMarker(nameof(ProfilerFrameDataHierarchyView) + ".DoGUI");

readonly string k_SerializationPrefKeyPrefix;
string multiColumnHeaderStatePrefKey =>
k_SerializationPrefKeyPrefix + "MultiColumnHeaderState";
string splitter0StatePrefKey =>
k_SerializationPrefKeyPrefix + "Splitter.Relative[0]";
string splitter1StatePrefKey =>
k_SerializationPrefKeyPrefix + "Splitter.Relative[1]";
string detailedViewTypeStatePrefKey =>
k_SerializationPrefKeyPrefix + "DetailedViewTypeState";

string detailedObjectsViewPrefKeyPrefix =>
k_SerializationPrefKeyPrefix + "DetailedObjectsView.";
string detailedCallsViewPrefKeyPrefix =>
k_SerializationPrefKeyPrefix + "DetailedCallsView.";
...
void InitIfNeeded() {
	...
	var cpuHierarchyColumns =
		new[] {HierarchyFrameDataView.columnName,
	...
	var defaultSortColumn = gpuView ? HierarchyFrameDataView
	                        .columnTotalGpuTime
	  : HierarchyFrameDataView.columnTotalTime;
	...
	var headerState =
		CreateDefaultMultiColumnHeaderState(columns, defaultSortColumn);
	...
	var multiColumnHeader = new ProfilerFrameDataMultiColumnHeader(
		m_MultiColumnHeaderState, columns){
		height = 25
	};
	if (firstInit)
	...
	// Check if it already exists (deserialized from window layout file or
	// scriptable object)
	if (m_TreeViewState == null)
		m_TreeViewState = new TreeViewState();
	m_TreeView = new ProfilerFrameDataTreeView(m_TreeViewState,
	                                           multiColumnHeader, cpuModule);
	...
	if (m_DetailedObjectsView == null)
		m_DetailedObjectsView =
			new ProfilerDetailedObjectsView(detailedObjectsViewPrefKeyPrefix);
	m_DetailedObjectsView.gpuView = gpuView;
	m_DetailedObjectsView.frameItemEvent += FrameItem;
	if (m_DetailedCallsView == null) {
		m_DetailedCallsView =
			new ProfilerDetailedCallsView(detailedCallsViewPrefKeyPrefix);
		m_DetailedCallsView.profilerSampleNameProvider = cpuModule;
	}
	m_DetailedCallsView.frameItemEvent += FrameItem;
	if (m_DetailedViewSpliterState == null ||
	    !m_DetailedViewSpliterState.IsValid())
		m_DetailedViewSpliterState = SplitterState.FromRelative(
			new[] {SessionState.GetFloat(splitter0StatePrefKey, 70f),
			       SessionState.GetFloat(splitter1StatePrefKey, 30f)},
			new[] {450f, 50f}, null);
	if (!m_Serialized)
		m_DetailedViewType = (DetailedViewType) SessionState.GetInt(
			detailedViewTypeStatePrefKey, (int) DetailedViewType.None);

	m_Serialized = true;
	m_Initialized = true;
}

public override void OnEnable(CPUorGPUProfilerModule cpuModule,
...
public static ProfilerFrameDataMultiColumnHeader.Column[] CreateColumns(
	int[] profilerColumns) {
	var columns =
		new ProfilerFrameDataMultiColumnHeader.Column[profilerColumns.Length];
	for (var i = 0; i < profilerColumns.Length; ++i) {
		...
		var content =
			...
			HierarchyFrameDataView.columnWarningCount
			  ? EditorGUIUtility
			.IconContent("ProfilerColumn.WarningCount", columnName)
	    : new GUIContent(columnName);
		var column = new ProfilerFrameDataMultiColumnHeader.Column {
			profilerColumn = profilerColumns[i], headerLabel = content
		};
		columns[i] = column;
	}
...
public static MultiColumnHeaderState CreateDefaultMultiColumnHeaderState(
	ProfilerFrameDataMultiColumnHeader.Column[] columns,
	...
	var state = new MultiColumnHeaderState(headerColumns){
		sortedColumnIndex =
			ProfilerFrameDataMultiColumnHeader.GetMultiColumnHeaderIndex(
...
public void DoGUI(HierarchyFrameDataView frameDataView, bool fetchData,
	...
	using (m_DoGUIMarker.Auto()) {
		...
		var collectingSamples =
			ProfilerDriver.enabled &&
			(ProfilerDriver.profileEditor || EditorApplication.isPlaying);
		var isSearchAllowed =
			string.IsNullOrEmpty(treeView.searchString) ||
			!(collectingSamples && ProfilerDriver.deepProfiling);

		var isDataAvailable = frameDataView != null && frameDataView.valid;

		var showDetailedView =
			isDataAvailable && m_DetailedViewType != DetailedViewType.None;
		if (showDetailedView)
		...
		if (showDetailedView) {
			...
			case DetailedViewType.Objects:
				detailedObjectsView.DoGUI(BaseStyles.header, frameDataView,
				                          m_TreeView.GetSelection());
				break;
			case DetailedViewType.CallersAndCallees:
				detailedCallsView.DoGUI(BaseStyles.header, frameDataView,
				                        m_TreeView.GetSelection());
				break;
			}
...
void DrawSearchBar() {
	var rect = GUILayoutUtility.GetRect(
		50f, 300f, EditorGUI.kSingleLineHeight, EditorGUI.kSingleLineHeight,
		EditorStyles.toolbarSearchField);
	treeView.searchString =
		m_SearchField.OnToolbarGUI(rect, treeView.searchString);
}

void DrawToolbar(HierarchyFrameDataView frameDataView,
                 bool showDetailedView, ref bool updateViewLive) {
	EditorGUILayout.BeginHorizontal(BaseStyles.toolbar);

	if (frameDataView != null && frameDataView.valid)
		DrawViewTypePopup(
			(frameDataView.viewMode &
			 HierarchyFrameDataView.ViewModes.MergeSamplesWithTheSameName) != 0
		? ProfilerViewType.Hierarchy
		: ProfilerViewType.RawHierarchy);
	else
	...
	if (!gpuView) {
		using (new EditorGUI.DisabledScope(
			       !(frameDataView != null && frameDataView.valid))) {
...
void UpdateThreadNamesAndThreadIndex(HierarchyFrameDataView frameDataView) {
	...
	using (var frameIterator = new ProfilerFrameDataIterator()) {
		...
		if (m_ThreadInfoCache == null ||
		    m_ThreadInfoCache.Capacity < threadCount)
...
UnityEditor.Tuple<int, string[]>
GetThreadNamesLazy(HierarchyFrameDataView frameDataView) {
...
private void DrawThreadPopup(HierarchyFrameDataView frameDataView) {
	if (!(frameDataView != null && frameDataView.valid)) {
		...
		EditorGUILayout.AdvancedPopup(
			0, disabledValues, BaseStyles.threadSelectionToolbarDropDown,
			GUILayout.MinWidth(
				BaseStyles.detailedViewTypeToolbarDropDown.fixedWidth));
		return;
	}

	var newThreadIndex = 0;
	if (threadIndex == 0) {
		newThreadIndex = EditorGUILayout.AdvancedLazyPopup(
			m_ThreadName, threadIndex,
			(() => GetThreadNamesLazy(frameDataView)),
			BaseStyles.threadSelectionToolbarDropDown,
			GUILayout.MinWidth(
				BaseStyles.detailedViewTypeToolbarDropDown.fixedWidth));
	} else {
		float minWidth, maxWidth;
		var content = new GUIContent(m_ThreadName);
		BaseStyles.threadSelectionToolbarDropDown.CalcMinMaxWidth(
			content, out minWidth, out maxWidth);
		UpdateThreadNamesAndThreadIndex(frameDataView);
		newThreadIndex = EditorGUILayout.AdvancedPopup(
			threadIndex, m_ThreadNames,
			BaseStyles.threadSelectionToolbarDropDown,
			GUILayout.MinWidth(
				Math.Max(BaseStyles.detailedViewTypeToolbarDropDown.fixedWidth,
				         minWidth)));
	}
...
void DrawDetailedViewPopup() {
	m_DetailedViewType = (DetailedViewType) EditorGUILayout.IntPopup(
		(int) m_DetailedViewType, kDetailedViewTypeNames, kDetailedViewTypes,
		BaseStyles.detailedViewTypeToolbarDropDown,
		GUILayout.Width(
			BaseStyles.detailedViewTypeToolbarDropDown.fixedWidth));
}
...
void FrameItem(int id) {
	m_TreeView.SetFocus();
	m_TreeView.SetSelection(new List<int>(1){
				id
			});
...
public void
SetSelectionFromLegacyPropertyPath(string selectedPropertyPath) {
	InitIfNeeded();
...
public override void Clear() {
	...
	if (m_TreeView != null) {
		if (m_TreeView.multiColumnHeader != null) {
			m_TreeView.multiColumnHeader.visibleColumnsChanged -=
				OnMultiColumnHeaderChanged;
			m_TreeView.multiColumnHeader.sortingChanged -=
				OnMultiColumnHeaderChanged;
		}
		m_TreeView.Clear();
	}
...
public void SaveViewSettings() {
	if (m_DetailedViewSpliterState != null &&
	    m_DetailedViewSpliterState.relativeSizes != null &&
	    m_DetailedViewSpliterState.relativeSizes.Length >= 2) {
		SessionState.SetFloat(splitter0StatePrefKey,
		                      m_DetailedViewSpliterState.relativeSizes[0]);
		SessionState.SetFloat(splitter1StatePrefKey,
		                      m_DetailedViewSpliterState.relativeSizes[1]);
	}
	SessionState.SetInt(detailedViewTypeStatePrefKey,
	                    (int) m_DetailedViewType);
