...
internal class ProfilerColors {
  static ProfilerColors() {
    s_DefaultColors = new Color[]{
        ...
        HierarchyFrameDataView.GetMarkerCategoryColor(
            31), // "Virtual Texturing"
    };
    s_DefaultColorsLuminanceValues = new float[s_DefaultColors.Length];
    VisionUtility.GetLuminanceValuesForPalette(
        s_DefaultColors, ref s_DefaultColorsLuminanceValues);
    // Areas are defined by stats in ProfilerStats.cpp file.
    // Color are driven by CPU profiler chart area colors and must be consistent
    // with CPU timeline sample colors. Sample color is defined by ProfilerGroup
    // (category) and defined in s_ProfilerGroupInfos table.
    ...
    VisionUtility.GetColorBlindSafePalette(s_ColorBlindSafeChartColors, 0.3f,
                                           1f);
    ...
    s_ColorBlindSafeColorsLuminanceValues =
        new float[s_ColorBlindSafeColors.Length];
    VisionUtility.GetLuminanceValuesForPalette(
        s_ColorBlindSafeColors, ref s_ColorBlindSafeColorsLuminanceValues);
  }

  public static Color[] chartAreaColors {
    get {
      return UserAccessiblitySettings.colorBlindCondition ==
  ...
  public static Color[] timelineColors {
    get {
      return UserAccessiblitySettings.colorBlindCondition ==
             ColorBlindCondition.Default ? s_DefaultColors
          : s_ColorBlindSafeColors;
    }
  }
