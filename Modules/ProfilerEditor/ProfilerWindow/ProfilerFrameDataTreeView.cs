...
internal class ProfilerFrameDataTreeView : TreeView {
  public static readonly GUIContent kFrameTooltip =
      EditorGUIUtility.TrTextContent("", "Press 'F' to frame selection");
  ...
  // Each level has a set of expanded marker ids, which are equivalent to sample
  // name.
  class ExpandedMarkerIdHierarchy {
    public Dictionary<int, ExpandedMarkerIdHierarchy> expandedMarkers;
  }
  [NonSerialized] ExpandedMarkerIdHierarchy m_ExpandedMarkersHierarchy;

  [NonSerialized] List<TreeViewItem> m_RowsPool = new List<TreeViewItem>();
  [NonSerialized] Stack<List<TreeViewItem>> m_ChildrenPool =
      new Stack<List<TreeViewItem>>();
  [NonSerialized] LinkedList<TreeTraversalState> m_ReusableVisitList =
      new LinkedList<TreeTraversalState>();
  [NonSerialized] List<int> m_ReusableChildrenIds = new List<int>(1024);
  [NonSerialized] Stack<LinkedListNode<TreeTraversalState>>
      m_TreeTraversalStatePool =
          new Stack<LinkedListNode<TreeTraversalState>>();
  ...
  class FrameDataTreeViewItem : TreeViewItem {
    ...
    public int samplesCount {
      get { return m_FrameDataView.GetItemMergedSamplesCount(id); }
    }

    public FrameDataTreeViewItem(HierarchyFrameDataView frameDataView, int id,
                                 int depth, TreeViewItem parent)
        : base(id, depth, parent, null) {
      m_FrameDataView = frameDataView;
      m_Initialized = false;
    }

    internal void Init(HierarchyFrameDataView frameDataView, int id, int depth,
                       TreeViewItem parent) {
      this.id = id;
      this.depth = depth;
      this.parent = parent;
    ...
    public void Init(ProfilerFrameDataMultiColumnHeader.Column[] columns,
      ...
      for (var i = 0; i < columns.Length; i++) {
        ...
        if (columns[i].profilerColumn == HierarchyFrameDataView.columnName) {
          ...
          data =
              m_FrameDataView.GetItemColumnData(id, columns[i].profilerColumn);
        }
  ...
  public ProfilerFrameDataTreeView(
      TreeViewState state, ProfilerFrameDataMultiColumnHeader multicolumnHeader,
      IProfilerSampleNameProvider profilerSampleNameProvider)
  ...
  public void SetFrameDataView(HierarchyFrameDataView frameDataView) {
    ...
    var needSorting =
        ...
        (frameDataView.sortColumn != m_MultiColumnHeader.sortedProfilerColumn ||
         frameDataView.sortColumnAscending !=
             m_MultiColumnHeader.sortedProfilerColumnAscending);
    ...
    m_FrameDataView = frameDataView;
    if (needSorting)
      m_FrameDataView.Sort(m_MultiColumnHeader.sortedProfilerColumn,
                           m_MultiColumnHeader.sortedProfilerColumnAscending);
  ...
  void
  AddExpandedChildrenRecursively(TreeViewItem item,
                                 ExpandedMarkerIdHierarchy expandedHierarchy) {
    if (item.children == null)
      return;

    for (var i = 0; i < item.children.Count; ++i) {
      var childItem = item.children[i];
      // Inlining !IsChildListForACollapsedParent without childList.Count == 1
      // check, as we only create list if we have children
      if (childItem.children != null && childItem.children[0] != null) {
        ...
        if (expandedHierarchy.expandedMarkers == null)
          expandedHierarchy.expandedMarkers =
              new Dictionary<int, ExpandedMarkerIdHierarchy>();
        try {
          expandedHierarchy.expandedMarkers.Add(
              m_FrameDataView.GetItemMarkerID(childItem.id), subHierarchy);
  ...
  public void SetSelectionFromLegacyPropertyPath(string selectedPropertyPath) {
    ...
    if (m_SelectedItemMarkerIdPath != null &&
        PropertyPathMatchesSelectedIDs(selectedPropertyPath, state.selectedIDs))
      return;
  ...
  private bool PropertyPathMatchesSelectedIDs(string legacyPropertyPath,
                                              List<int> selectedIDs) {
    if (m_FrameDataView == null || !m_FrameDataView.valid)
      return false;

    if (string.IsNullOrEmpty(legacyPropertyPath) || selectedIDs == null ||
        selectedIDs.Count == 0) {
      return string.IsNullOrEmpty(legacyPropertyPath) &&
  ...
  void StoreSelectedState() {
    ...
    m_FrameDataView.GetItemMarkerIDPath(oldSelection[0],
                                        m_SelectedItemMarkerIdPath);
  }
  ...
  void MigrateSelectedState(bool expandIfNecessary) {
    ...
    var newSelection =
        (newSelectedId == 0) ? new List<int>() : new List<int>(){newSelectedId};
    state.selectedIDs = newSelection;

    // Framing invalidates expanded state and this is very expensive operation
    // to perform each frame. Thus we auto frame selection only when we are not
    // profiling.
    ...
    if (newSelectedId != 0 && isInitialized && isFramingAllowed &&
        (selectedItemsPathIsExpanded || expandIfNecessary))
  ...
  protected override IList<TreeViewItem> BuildRows(TreeViewItem root) {
    ...
    var newExpandedIds = m_ExpandedMarkersHierarchy == null ? null
        : new List<int>(state.expandedIDs.Count);
    if (!string.IsNullOrEmpty(searchString)) {
      Search(root, searchString, m_Rows);
    } else {
      AddAllChildren((FrameDataTreeViewItem) root, m_ExpandedMarkersHierarchy,
                     m_Rows, newExpandedIds);
    }
  ...
  void Search(TreeViewItem searchFromThis, string search,
              List<TreeViewItem> result) {
    if (searchFromThis == null)
      throw new ArgumentException("Invalid searchFromThis: cannot be null",
                                  "searchFromThis");
    ...
    while (stack.Count > 0) {
      ...
      // Matches search?
      var functionName =
          m_ProfilerSampleNameProvider.GetItemName(m_FrameDataView, current);
      if (functionName.IndexOf(search, StringComparison.OrdinalIgnoreCase) >=
          0) {
        var item = AcquireFrameDataTreeViewItem(m_FrameDataView, current,
                                                kItemDepth, searchFromThis);
  ...
  // This way we follow samples and expansion hierarchy simultaneously avoiding
  // expensive expansion lookup by a full sample path in a global table.
  struct TreeTraversalState {
    public FrameDataTreeViewItem item;
    public ExpandedMarkerIdHierarchy expandedHierarchy;
  }

  LinkedListNode<TreeTraversalState>
  AcquireTreeTraversalStateNode(FrameDataTreeViewItem item,
                                ExpandedMarkerIdHierarchy expandedHierarchy) {
    ...
    node.Value = new TreeTraversalState(){item = item, expandedHierarchy =
                                                           expandedHierarchy};
    return node;
  }

  FrameDataTreeViewItem
  AcquireFrameDataTreeViewItem(HierarchyFrameDataView frameDataView, int id,
                               int depth, TreeViewItem parent) {
    if (m_RowsPool.Count > 0) {
      FrameDataTreeViewItem child =
          (FrameDataTreeViewItem) m_RowsPool[m_RowsPool.Count - 1];
  ...
  void AddAllChildren(FrameDataTreeViewItem parent,
                      ExpandedMarkerIdHierarchy parentExpandedHierararchy,
                      IList<TreeViewItem> newRows, List<int> newExpandedIds) {
    m_ReusableVisitList.AddFirst(
        AcquireTreeTraversalStateNode(parent, parentExpandedHierararchy));

    // Depth-first traversal.
    // Think of it as an unrolled recursion where stack state is defined by
    // TreeTraversalState.
    while (m_ReusableVisitList.First != null) {
      ...
      m_FrameDataView.GetItemChildren(currentItem.item.id,
                                      m_ReusableChildrenIds);
      var childrenCount = m_ReusableChildrenIds.Count;
      if (childrenCount == 0)
        continue;

      if (currentItem.item.depth != -1) {
        // Check expansion state from a previous frame view state (marker id
        // path) or current tree view state (frame-specific id).
        bool needsExpansion;
        if (m_ExpandedMarkersHierarchy == null) {
          ...
          // When we switch to another frame, we rebuild expanded state based on
          // stored m_ExpandedMarkersHierarchy which represents tree of expanded
          // nodes.
      ...
      for (var i = 0; i < childrenCount; ++i) {
        var child = AcquireFrameDataTreeViewItem(
            m_FrameDataView, m_ReusableChildrenIds[i],
            currentItem.item.depth + 1, currentItem.item);
        currentItem.item.children.Add(child);
      }

      // Add children to the traversal list.
      // We add all of them in front, so it is depth search, but with preserved
      // siblings order.
      ...
      foreach (var child in currentItem.item.children) {
        ...
        if (currentItem.expandedHierarchy != null &&
            currentItem.expandedHierarchy.expandedMarkers != null)
          currentItem.expandedHierarchy.expandedMarkers.TryGetValue(
              childMarkerId, out childExpandedHierarchy);

        var traversalState = AcquireTreeTraversalStateNode(
            (FrameDataTreeViewItem) child, childExpandedHierarchy);
        if (prev == null)
  ...
  protected override void SelectionChanged(IList<int> selectedIds) {
    ...
    // we still want to be able to frame and select proper sample once it is
    // present again. Thus we invalidate selection only if user selected new
    // item. Same applies to expanded state.
    ...
    if (selectedIds.Count > 0)
      // Selection change event handling will set this path and cause a
      // selection migration, which is unnecessary and will fail in Raw
      // Hierarchy
      m_LegacySelectedItemMarkerNamePath = null;
  }
  ...
  void CellGUI(Rect cellRect, FrameDataTreeViewItem item, bool needsIndent,
               int column, ref RowGUIArgs args) {
    if (needsIndent) {
    ...
    DefaultStyles.label.Draw(cellRect, content, false, false, args.selected,
                             args.focused);
  }
}
...
public class ProfilerFrameDataMultiColumnHeader : MultiColumnHeader {
  ...
  public ProfilerFrameDataMultiColumnHeader(MultiColumnHeaderState state,
                                            Column[] columns)
      : base(state) {
  ...
  public static int GetMultiColumnHeaderIndex(Column[] columns,
                                              int profilerColumn) {
    for (var i = 0; i < columns.Length; ++i) {
