...
//@TODO: This should be renamed to LoadedObjectMemoryType like on C++. But
//should be done on the memory profiler branch.
...
public enum ProfilerMemoryView { Simple = 0, Detailed = 1 }

public enum ProfilerAudioView {
	[Obsolete(
		 "This has been made obsolete. Audio stats are now shown on every subpane.",
		 true)] Stats = 0,
	Channels = 1,
	Groups = 2,
	ChannelsAndGroups = 3,
...
// The following is a better solution for getting the constant rather than
// hard coding it Unfortunately, this will not work with our test framework
// due to it only pulling managed symbols and not native, which causes a
// runtime exception when the property is accessed
// [NativeProperty("\"PLAYER_DIRECTCONNECT_PORT\"", true, TargetType.Field,
// IsThreadSafe = true)] Must stay consistent with PLAYER_DIRECTCONNECT_PORT
// in GeneralConnection.h

static public string directConnectionPort = "34999";

[StaticAccessor("profiling::GetProfilerSessionPtr()->GetProfilerHistory()",
...
[StaticAccessor("profiling::GetProfilerSessionPtr()->GetProfilerHistory()",
                StaticAccessorType.Arrow)]
static public extern int firstFrameIndex {
	[NativeMethod("GetFirstFrameIndex")] get;
}

[StaticAccessor("profiling::GetProfilerSessionPtr()->GetProfilerHistory()",
                StaticAccessorType.Arrow)]
static public extern int lastFrameIndex {
	[NativeMethod("GetLastFrameIndex")] get;
}
...
static public extern string selectedPropertyPath {
	[NativeMethod("GetSelectedPropertyPath")] get;
	[NativeMethod("SetSelectedPropertyPath")] set;
}

[NativeMethod("SetProfileArea")]
[StaticAccessor("profiling::GetProfilerSessionPtr()",
                StaticAccessorType.Arrow)]
...
static public extern bool enabled {
	[NativeMethod("IsProfilingEnabled")] get;
	[NativeMethod("SetProfilingEnabled")] set;
}

static public bool profileGPU {
	get { return IsAreaEnabled(ProfilerArea.GPU); }
	set { SetAreaEnabled(ProfilerArea.GPU, value); }
}

[StaticAccessor("profiling::GetProfilerSessionPtr()",
                StaticAccessorType.Arrow)]
static public extern bool profileEditor {
	[NativeMethod("IsEditorProfilingEnabled")] get;
	[NativeMethod("SetEditorProfilingEnabled")] set;
}

[StaticAccessor("profiling::GetProfilerSessionPtr()",
                StaticAccessorType.Arrow)]
static public extern bool deepProfiling {
	[NativeMethod("IsDeepProfilingEnabled")] get;
	[NativeMethod("SetDeepProfilingEnabled")] set;
}

static public extern ProfilerMemoryRecordMode memoryRecordMode {
	[FreeFunction("profiler_get_memory_record_mode")] get;
	[FreeFunction("profiler_set_memory_record_mode")] set;
}
...
[StaticAccessor("profiling::GetProfilerSessionPtr()->GetProfilerHistory()",
                StaticAccessorType.Arrow)]
[Obsolete(
	 "Use GetFormattedCounterValue to get the stats including Profiler Counter data",
	 false)]
static public extern string GetFormattedStatisticsValue(int frame,
                                                        int identifier);

[StaticAccessor("profiling::GetProfilerSessionPtr()->GetProfilerHistory()",
                StaticAccessorType.Arrow)]
static public extern int GetUISystemEventMarkersCount(int firstFrame,
                                                      int frameCount);

[StaticAccessor("profiling::GetProfilerSessionPtr()->GetProfilerHistory()",
                StaticAccessorType.Arrow)]
static public extern void
GetUISystemEventMarkersBatch(int firstFrame, int frameCount,
                             [ Out ] EventMarker[] buffer,
                             [ Out ] string[] names);
...
[Obsolete(
	 "Use GetCounterValuesBatch to read the stats including Profiler Counter data",
	 false)]
static public extern void
GetStatisticsValues(int identifier, int firstFrame, float scale,
                    [ Out ] float[] buffer, out float maxValue);

[StaticAccessor("profiling::GetProfilerSessionPtr()->GetProfilerHistory()",
                StaticAccessorType.Arrow)]
static public extern string
GetFormattedCounterValue(int frame, ProfilerArea area, string name);

[StaticAccessor("profiling::GetProfilerSessionPtr()->GetProfilerHistory()",
                StaticAccessorType.Arrow)]
public static extern void
GetCounterValuesBatch(ProfilerArea area, string name, int firstFrame,
                      float scale, [ Out ] float[] buffer,
                      out float maxValue);

static public void GetGpuStatisticsAvailabilityStates(int firstFrame, [
							      Out
                                                      ] GpuProfilingStatisticsAvailabilityStates[] buffer) {
	unsafe {
		fixed(GpuProfilingStatisticsAvailabilityStates *b = buffer) {
			GetGpuStatisticsAvailabilityStatesInternal(firstFrame, b,
			                                           buffer.Length);
		}
	}
}
...
static public extern GpuProfilingStatisticsAvailabilityStates
GetGpuStatisticsAvailabilityState(int frame);

[StaticAccessor("profiling::GetProfilerSessionPtr()->GetProfilerHistory()",
                StaticAccessorType.Arrow)]
[NativeMethod("GetGpuStatisticsAvailabilityStates")]
static unsafe extern void GetGpuStatisticsAvailabilityStatesInternal(
	int firstFrame, GpuProfilingStatisticsAvailabilityStates *buffer,
	int size);

[StaticAccessor("profiling::GetProfilerSessionPtr()->GetProfilerHistory()",
                StaticAccessorType.Arrow)]
[NativeMethod("GetStatisticsAvailabilityState")]
static internal extern int GetStatisticsAvailabilityState(ProfilerArea area,
                                                          int frame);

[StaticAccessor("profiling::GetProfilerSessionPtr()->GetProfilerHistory()",
                StaticAccessorType.Arrow)]
[NativeMethod("GetStatisticsAvailabilityStates")]
static internal extern void
GetStatisticsAvailabilityStates(ProfilerArea area, int firstFrame,
                                [ Out ] int[] buffer);

static public HierarchyFrameDataView
GetHierarchyFrameDataView(int frameIndex, int threadIndex,
                          HierarchyFrameDataView.ViewModes viewMode,
                          int sortColumn, bool sortAscending) {
	return new HierarchyFrameDataView(frameIndex, threadIndex, viewMode,
	                                  sortColumn, sortAscending);
}

static public RawFrameDataView GetRawFrameDataView(int frameIndex,
                                                   int threadIndex) {
...
[RequiredByNativeCode]
static void InvokeNewProfilerFrameRecorded(int connectionId,
                                           int newFrameIndex) {
...
[StaticAccessor("profiling::GetProfilerSessionPtr()->GetProfilerHistory()",
                StaticAccessorType.Arrow)]
static public extern string[] GetGraphStatisticsPropertiesForArea(
	ProfilerArea area);

[Obsolete(
	 "Use GetStatisticsIdentifierForArea that takes ProfilerArea as first argument",
	 false)]
static public int GetStatisticsIdentifier(string propertyName) {
	return GetStatisticsIdentifierForArea((ProfilerArea) Profiler.areaCount,
	                                      propertyName);
}

[StaticAccessor("profiling::GetProfilerSessionPtr()->GetProfilerHistory()",
                StaticAccessorType.Arrow)]
static public extern void GetStatisticsAvailable(ProfilerArea profilerArea,
                                                 int firstFrame,
                                                 [ Out ] int[] buffer);

[StaticAccessor("profiling::GetProfilerSessionPtr()->GetProfilerHistory()",
                StaticAccessorType.Arrow)]
static extern void GetStatisticsAvailableInternal(ProfilerArea profilerArea,
                                                  int firstFrame,
                                                  IntPtr buffer, int size);

[Obsolete(
	 "GetStatisticsAvailable with bool buffer is obsolete, use with int array instead. (x & 1) == 1 is the same as true")]
static public void GetStatisticsAvailable(ProfilerArea profilerArea,
                                          int firstFrame,
                                          [ Out ] bool[] buffer) {
	unsafe {
		var intBuffer = stackalloc int[buffer.Length];
		GetStatisticsAvailableInternal(profilerArea, firstFrame,
		                               new IntPtr(intBuffer), buffer.Length);
...
GetStatisticsIdentifierForArea(ProfilerArea profilerArea,
                               string propertyName);
...
static public extern int connectedProfiler {
	[NativeMethod("GetConnectedProfiler")] get;
	[NativeMethod("SetConnectedProfiler")] set;
}
...
static public extern string GetOverviewText(ProfilerArea profilerArea,
                                            int frame);

static public extern uint usedHeapSize {
	[FreeFunction("GetUsedHeapSize")] get;
}

static public extern uint objectCount {
	[StaticAccessor("Object", StaticAccessorType.DoubleColon)]
	[NativeMethod("GetLoadedObjectCount")] get;
}
...
static public extern bool isGPUProfilerSupportedByOS {
	[StaticAccessor("profiling::GetProfilerSessionPtr()->GetProfilerHistory()",
	                StaticAccessorType.Arrow)]
	[NativeMethod("IsGPUProfilerSupportedByOS")] get;
}
...
static public extern bool isGPUProfilerSupported {
	[StaticAccessor("profiling::GetProfilerSessionPtr()->GetProfilerHistory()",
	                StaticAccessorType.Arrow)]
	[NativeMethod("IsGPUProfilerSupported")] get;
}

[Obsolete(
	 "Deprecated API, it will always return false, use GetGPUStatisticsAvailabilityState instead.")]
...
static public extern void
RequestObjectMemoryInfo(bool gatherObjectReferences);
...
static public extern void
SetAutoInstrumentedAssemblies(InstrumentedAssemblyTypes fullname);
