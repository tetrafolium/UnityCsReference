...
namespace UnityEditorInternal.Profiling.Memory.Experimental {
  ...
  public class MemorySnapshotFileReader : ISerializationCallbackReceiver {
    ...
    [SerializeField] string m_AbsoluteFilePath;

    [NonSerialized] Chapter[] m_Chapters;

    [NonSerialized] BinaryReader m_Reader;

    [NonSerialized] ulong m_ChapterOffset;

    [NonSerialized] Block[] m_Blocks;

    public MemorySnapshotFileReader(string filePath) { Open(filePath); }

    public MemorySnapshotFileReader() {}

    public void Dispose() { Close(); }

    public string GetFilePath() { return m_AbsoluteFilePath; }

    public void Open(string filePath) {
      m_AbsoluteFilePath = filePath;

      FileStream stream =
          new FileStream(m_AbsoluteFilePath, FileMode.Open, FileAccess.Read);
    ...
    public void OnAfterDeserialize() { Open(m_AbsoluteFilePath); }

    internal void GetDataByteArray(FileFormat.EntryType entryType,
                                   uint entryIndex, uint numEntries,
                                   ref byte[][] dataOut) {
      ...
      for (uint i = 0; i < numEntries; ++i) {
        ...
        m_Blocks [blockIndex]
            .GetData(offset, entrySize, ref dataOut[i], m_Reader);

        offset += entrySize;
      }
    }

    public void GetDataArray<T>(FileFormat.EntryType entryType, uint entryIndex,
                                uint numEntries, ref T[] dataOut,
      ...
      while (i < numEntries) {
        ...
        m_Blocks [blockIndex]
            .GetData(offset, cacheMemory, ref data, m_Reader);
        ...
        while (i < element) {
          uint entrySize = chapter.GetSizeForEntryIndex(entryIndex + i);
          dataOut [i]
          = getItemFunc(data, dataOffset, entrySize);
    ...
    public T GetDataSingle<T>(FileFormat.EntryType entryType,
                              GetItem<T> getItemFunc) {
      Chapter chapter = GetChapter(entryType);
      ...
      m_Blocks [blockIndex]
          .GetData(blockOffset, dataSize, ref data, m_Reader);
    ...
    internal void BuildChapters() {
      ...
      if (m_Reader.ReadUInt32() != kMemorySnapshotHeadSignature) {
        throw new IOException(m_AbsoluteFilePath +
                              " Corrupted head signature.");
      }
      ...
      if (m_Reader.ReadUInt32() != kMemorySnapshotTailSignature) {
        throw new IOException(m_AbsoluteFilePath +
                              " Corrupted tail signature.");
      }

      m_Reader.BaseStream.Seek(-sizeof(UInt32) - sizeof(UInt64),
                               SeekOrigin.End);
