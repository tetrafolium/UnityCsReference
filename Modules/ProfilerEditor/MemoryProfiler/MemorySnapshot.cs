...
namespace UnityEditor.Profiling.Memory.Experimental {
  ...
  public class PackedMemorySnapshot : IDisposable {
    ...
    public static PackedMemorySnapshot Load(string path) {
      ...
      UInt32 ver = reader.GetDataSingle(EntryType.Metadata_Version,
                                        ConversionFunctions.ToUInt32);

      if (ver < kMinSupportedVersion) {
        throw new Exception(string.Format(
            "Memory snapshot at {0}, is using an older format version: {1}",
            new object[]{reader.GetFilePath(), ver.ToString()}));
      }
    ...
    public static bool
    Convert(UnityEditor.MemoryProfiler.PackedMemorySnapshot snapshot,
            string writePath) {
      ...
      writer.WriteEntry(EntryType.Metadata_RecordDate,
      ...
      byte[] metaDataBytes =
          new byte[stringDataLength +
                   (sizeof(int) *
                    3)]; // add space for serializing the lengths of the strings

      int offset = 0;
      offset = ExperimentalMemoryProfiler.WriteIntToByteArray(
          metaDataBytes, offset, content.Length);
      offset = ExperimentalMemoryProfiler.WriteStringToByteArray(
          metaDataBytes, offset, content);

      offset = ExperimentalMemoryProfiler.WriteIntToByteArray(
          metaDataBytes, offset, platform.Length);
      offset = ExperimentalMemoryProfiler.WriteStringToByteArray(
          metaDataBytes, offset, platform);

      offset = ExperimentalMemoryProfiler.WriteIntToByteArray(metaDataBytes,
                                                              offset, 0);
      ...
      writer.WriteEntry(
          EntryType.Metadata_CaptureFlags,
          (UInt32) CaptureFlags.ManagedObjects); // capture just managed
      ...
      for (int i = 0; i < snapshot.managedHeapSections.Length; ++i) {
        ...
        writer.WriteEntry(EntryType.ManagedHeapSections_StartAddress,
                          heapSection.startAddress);
        writer.WriteEntryArray(EntryType.ManagedHeapSections_Bytes,
                               heapSection.m_Bytes);
      }
      ...
      for (int i = 0; i < snapshot.typeDescriptions.Length; ++i) {
        ...
        writer.WriteEntry(EntryType.TypeDescriptions_Flags,
                          (UInt32) type.m_Flags);
        writer.WriteEntry(EntryType.TypeDescriptions_BaseOrElementTypeIndex,
                          type.baseOrElementTypeIndex);
        ...
        if (!type.isArray) {
          ...
          for (int j = 0; j < typeFields.Length; ++j) {
            ...
            writer.WriteEntry(EntryType.FieldDescriptions_TypeIndex,
                              field.typeIndex);
            writer.WriteEntry(EntryType.FieldDescriptions_Name, field.name);
            writer.WriteEntry(EntryType.FieldDescriptions_IsStatic,
                              field.isStatic);
          }

          writer.WriteEntryArray(EntryType.TypeDescriptions_FieldIndices,
                                 fieldIndices);
          writer.WriteEntryArray(EntryType.TypeDescriptions_StaticFieldBytes,
                                 type.staticFieldBytes);
        } else {
          writer.WriteEntryArray(EntryType.TypeDescriptions_FieldIndices,
                                 new int[0]);
          writer.WriteEntryArray(EntryType.TypeDescriptions_StaticFieldBytes,
        ...
        writer.WriteEntry(EntryType.TypeDescriptions_TypeInfoAddress,
      ...
      for (int i = 0; i < snapshot.nativeTypes.Length; ++i) {
        ...
        writer.WriteEntry(EntryType.NativeTypes_NativeBaseTypeArrayIndex,
                          nativeType.nativeBaseTypeArrayIndex);
        writer.WriteEntry(EntryType.NativeTypes_Name, nativeType.name);
      }

      // write stub root reference for all native objects to point to, as the
      // old format did not capture these
      writer.WriteEntry(EntryType.NativeRootReferences_AreaName,
                        "Invalid Root");
      writer.WriteEntry(EntryType.NativeRootReferences_AccumulatedSize,
                        (ulong) 0);
      writer.WriteEntry(EntryType.NativeRootReferences_Id, (long) 0);
      writer.WriteEntry(EntryType.NativeRootReferences_ObjectName,
                        "Invalid Root Object");
      ...
      for (int i = 0; i < snapshot.nativeObjects.Length; ++i) {
        ...
        writer.WriteEntry(EntryType.NativeObjects_InstanceId,
                          nativeObject.instanceId);
        writer.WriteEntry(EntryType.NativeObjects_Size,
                          (ulong) nativeObject.size);
        writer.WriteEntry(EntryType.NativeObjects_NativeTypeArrayIndex,
                          nativeObject.nativeTypeArrayIndex);
        writer.WriteEntry(EntryType.NativeObjects_HideFlags,
                          (UInt32) nativeObject.hideFlags);
        writer.WriteEntry(EntryType.NativeObjects_Flags,
                          (UInt32) nativeObject.m_Flags);
        writer.WriteEntry(EntryType.NativeObjects_NativeObjectAddress,
                          (ulong) nativeObject.nativeObjectAddress);
        writer.WriteEntry(EntryType.NativeObjects_RootReferenceId, (long) 0);
      }

      // write virtual machine information
      writer.WriteEntry(EntryType.Metadata_VirtualMachineInformation,
    ...
    public static void Save(PackedMemorySnapshot snapshot, string writePath) {
      ...
      if (!System.IO.File.Exists(path)) {
        throw new UnityException("Failed to save snapshot. Snapshot file: " +
                                 snapshot.filePath + " is missing.");
      }
    ...
    public ConnectionEntries connections {
      get;
      internal set;
    }
    public FieldDescriptionEntries fieldDescriptions {
      get;
      internal set;
    }
    public GCHandleEntries gcHandles {
      get;
      internal set;
    }
    public ManagedMemorySectionEntries managedHeapSections {
      get;
      internal set;
    }
    public ManagedMemorySectionEntries managedStacks {
      get;
      internal set;
    }
    public NativeAllocationEntries nativeAllocations {
      get;
      internal set;
    }
    public NativeAllocationSiteEntries nativeAllocationSites {
      get;
      internal set;
    }
    public NativeCallstackSymbolEntries nativeCallstackSymbols {
      get;
      internal set;
    }
    public NativeMemoryLabelEntries nativeMemoryLabels {
      get;
      internal set;
    }
    public NativeMemoryRegionEntries nativeMemoryRegions {
      get;
      internal set;
    }
    public NativeObjectEntries nativeObjects {
      get;
      internal set;
    }
    public NativeRootReferenceEntries nativeRootReferences {
      get;
      internal set;
    }
    public NativeTypeEntries nativeTypes {
      get;
      internal set;
    }
    public TypeDescriptionEntries typeDescriptions {
      get;
      internal set;
    }
    ...
    internal void BuildEntries() {
      ...
      managedHeapSections = new ManagedMemorySectionEntries(
          m_Reader, EntryType.ManagedHeapSections_StartAddress);
      managedStacks = new ManagedMemorySectionEntries(
          m_Reader, EntryType.ManagedStacks_StartAddress);
      ...
      nativeObjects =
          new NativeObjectEntries(m_Reader, version == kCurrentVersion);
    ...
    internal MemorySnapshotFileReader GetReader() { return m_Reader; }

    public UInt32 version {
      get {
        return m_Reader.GetDataSingle(EntryType.Metadata_Version,
                                      ConversionFunctions.ToUInt32);
      }
    }
    public MetaData metadata {
      get {
        byte[] array = m_Reader.GetDataSingle(EntryType.Metadata_UserMetadata,
                                              ConversionFunctions.ToByteArray);
        // decoded as
        //   content_data_length
        //   content_data
        ...
        offset = ReadStringFromByteArray(array, offset, dataLength,
                                         out data.content);
        offset = ReadIntFromByteArray(array, offset, out dataLength);
        offset = ReadStringFromByteArray(array, offset, dataLength,
                                         out data.platform);
    ...
    private static int ReadStringFromByteArray(byte[] array, int offset,
                                               int stringLength,
                                               out string value) {
      if (stringLength == 0) {
        value = "";
    ...
    public string filePath {
      get { return m_Reader.GetFilePath(); }
    }

    public DateTime recordDate {
      get {
        long ticks = m_Reader.GetDataSingle(EntryType.Metadata_RecordDate,
    ...
    public CaptureFlags captureFlags {
      get {
        return (CaptureFlags) m_Reader.GetDataSingle(
    ...
    public VirtualMachineInformation virtualMachineInformation {
      get {
        return m_Reader.GetDataSingle(
            ...
            ConversionFunctions.ToVirtualMachineInformation);
      }
    }

    ~PackedMemorySnapshot() { Dispose(false); }
