...
[NativeHeader(
	 "Modules/AssetDatabase/Editor/Public/AssetDatabaseExperimental.h")]
...
public enum OnDemandMode { Off = 0, Lazy = 1, Background = 2 }
...
public extern static AssetDatabaseCounters counters {
	get;
}
...
public static event Action<CacheServerConnectionChangedParameters>
...
[Obsolete("Has been replaced by AssetDatabaseExperimental.RefreshSettings",
          true)]
public static void ReconnectToCacheServer() {
	throw new NotSupportedException(
		      "Please use AssetdatabaseExperimental.RefreshSettings instead.");
}
...
[NativeHeader(
	 "Modules/AssetDatabase/Editor/V2/Virtualization/Virtualization.h")]
...
public extern static ArtifactID
ProduceArtifactAsync(ArtifactKey artifactKey);
public extern static ArtifactID[] ProduceArtifactsAsync(
	GUID[] artifactKey,
	[ uei.DefaultValue("null") ] Type importerType = null);
public extern static ArtifactID
ForceProduceArtifact(ArtifactKey artifactKey);

[Obsolete(
	 "GetArtifactHash() has been removed. Use LookupArtifact(), ProduceArtifact() or ForceProduceArtifact() instead.")]
[uei.ExcludeFromDocs]
public static Hash128
GetArtifactHash(string guid, ImportSyncMode mode = ImportSyncMode.Block) {
	switch (mode) {
...
[Obsolete(
	 "GetArtifactHash() has been removed. Use LookupArtifact(), ProduceArtifact() or ForceProduceArtifact() instead.")]
public static Hash128
GetArtifactHash(string guid, [ uei.DefaultValue("null") ] Type importerType,
                ImportSyncMode mode = ImportSyncMode.Block) {
	switch (mode) {
	case ImportSyncMode.Block:
		return ProduceArtifact(new ArtifactKey(new GUID(guid), importerType))
		       .value;
	case ImportSyncMode.Poll:
		return LookupArtifact(new ArtifactKey(new GUID(guid), importerType))
		       .value;
	case ImportSyncMode.Queue:
		return ProduceArtifactAsync(
			new ArtifactKey(new GUID(guid), importerType))
		       .value;
	}
...
[Obsolete(
	 "GetArtifactPaths(Hash128, out string[]) has been removed. Use GetArtifactPaths(ArtifactID, out string[]) instead.")]
public static bool GetArtifactPaths(Hash128 hash, out string[] paths) {
	return GetArtifactPaths(new ArtifactID(){
				value = hash
			}, out paths);
}

[Obsolete(
	 "GetArtifactHashes() has been removed. Use LookupArtifact(), ProduceArtifact() or ForceProduceArtifact() instead.")]
public static Hash128[] GetArtifactHashes(
	string[] guids, ImportSyncMode mode = ImportSyncMode.Block) {
	...
	case ImportSyncMode.Queue:
		return ProduceArtifactsAsync(_guids, null)
		       .Select(a => a.value)
		       .ToArray();
	}
...
private extern static string[] GetArtifactPathsImpl(ArtifactID hash,
                                                    out bool success);

public extern static OnDemandProgress
GetOnDemandArtifactProgress(ArtifactKey artifactKey);

[Obsolete(
	 "GetOnDemandArtifactProgress(string) has been removed. Use GetOnDemandArtifactProgress(ArtifactKey) instead.")]
...
[Obsolete(
	 "GetOnDemandArtifactProgress(string,Type) has been removed. Use GetOnDemandArtifactProgress(ArtifactKey) instead.")]
public static OnDemandProgress
GetOnDemandArtifactProgress(string guid, Type importerType) {
	return GetOnDemandArtifactProgress(
		new ArtifactKey(new GUID(guid), importerType));
}

[RequiredByNativeCode]
static string[] OnSourceAssetsModified(string[] changedAssets,
                                       string[] addedAssets,
	...
	for (int i = 0; i < movedAssets.Length; i++)
		assetMoveInfo[i] =
			new AssetMoveInfo(movedFromAssetPaths[i], movedAssets[i]);

	var assetsReportedChanged = new HashSet<string>();

	foreach (Type type in TypeCache
	         .GetTypesDerivedFrom<AssetsModifiedProcessor>()) {
		var assetPostprocessor =
			Activator.CreateInstance(type) as AssetsModifiedProcessor;
		assetPostprocessor.assetsReportedChanged = assetsReportedChanged;
		assetPostprocessor.Internal_OnAssetsModified(
			changedAssets, addedAssets, deletedAssets, assetMoveInfo);
...
public extern static void RegisterCustomDependency(string dependency,
                                                   Hash128 hashOfValue);

[FreeFunction(
	 "AssetDatabaseExperimental::UnregisterCustomDependencyPrefixFilter")]
public extern static UInt32
UnregisterCustomDependencyPrefixFilter(string prefixFilter);
