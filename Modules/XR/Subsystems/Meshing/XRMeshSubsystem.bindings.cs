...
public struct MeshId : IEquatable<MeshId> {
	public override string ToString() {
		return string.Format("{0}-{1}", m_SubId1.ToString("X16"),
...
public struct MeshGenerationResult : IEquatable<MeshGenerationResult> {
	...
	public bool Equals(MeshGenerationResult other) {
		return (MeshId.Equals(other.MeshId)) && (Mesh.Equals(other.Mesh)) &&
		       (MeshCollider.Equals(other.MeshCollider)) &&
		       (Status.Equals(other.Status)) &&
		       (Attributes.Equals(other.Attributes));
	}

	public static bool operator ==(MeshGenerationResult lhs,
	                               MeshGenerationResult rhs) {
	...
	public override int GetHashCode() {
		return HashCodeHelper.Combine(
			HashCodeHelper.Combine(
				HashCodeHelper.Combine(
					HashCodeHelper.Combine(MeshId.GetHashCode(),
					                       Mesh.GetHashCode()),
...
public class XRMeshSubsystem
	: IntegratedSubsystem<XRMeshSubsystemDescriptor> {
...
public extern void
GenerateMeshAsync(MeshId meshId, Mesh mesh, MeshCollider meshCollider,
