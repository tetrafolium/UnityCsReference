...
public struct ParticleSystemNativeArray3 {
	...
	public Vector3 this[int index] {
		get { return new Vector3(x[index], y[index], z[index]); }
...
public struct ParticleSystemNativeArray4 {
	...
	public Vector4 this[int index] {
		get { return new Vector4(x[index], y[index], z[index], w[index]); }
...
public struct ParticleSystemJobData {
	...
	public NativeArray<UInt32> randomSeeds {
		get;
	}
	...
	unsafe internal ParticleSystemJobData(ref NativeParticleData nativeData)
		: this() {
		...
		rotationalSpeeds =
			CreateNativeArray3(ref nativeData.rotationalSpeeds, count);
		sizes = CreateNativeArray3(ref nativeData.sizes, count);
		startColors = CreateNativeArray<Color32>(nativeData.startColors, count);
		aliveTimePercent =
			CreateNativeArray<float>(nativeData.aliveTimePercent, count);
		inverseStartLifetimes =
			CreateNativeArray<float>(nativeData.inverseStartLifetimes, count);
	...
	unsafe internal NativeArray<T> CreateNativeArray<T>(void *src,
	                                                    int count) where T
	: struct {
		var arr = NativeArrayUnsafeUtility.ConvertExistingDataToNativeArray<T>(
			src, count, Allocator.Invalid);
	...
	unsafe internal ParticleSystemNativeArray3
	CreateNativeArray3(ref NativeParticleData.Array3 ptrs, int count) {
	...
	unsafe internal ParticleSystemNativeArray4
	CreateNativeArray4(ref NativeParticleData.Array4 ptrs, int count) {
...
internal struct ParticleSystemJobStruct<T> where T : struct,
                                                 IJobParticleSystem {
	...
	public delegate void ExecuteJobFunction(ref T data, IntPtr listDataPtr,
	                                        IntPtr unusedPtr,
	                                        ref JobRanges ranges, int jobIndex);
	public static unsafe void Execute(ref T data, IntPtr listDataPtr,
	                                  IntPtr unusedPtr, ref JobRanges ranges,
	                                  int jobIndex) {
		...
		ParticleSystemJobData jobData =
			new ParticleSystemJobData(ref particleData);
...
internal struct ParticleSystemParallelForJobStruct<T> where T
: struct,
	...
	public static unsafe void Execute(ref T data, IntPtr listDataPtr,
		...
		while (true) {
			int begin;
			int end;
			if (!JobsUtility.GetWorkStealingRange(ref ranges, jobIndex, out begin,
			                                      out end))
				break;
...
internal struct ParticleSystemParallelForBatchJobStruct<T> where T
: struct,
