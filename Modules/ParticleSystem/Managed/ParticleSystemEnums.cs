...
public enum ParticleSystemRenderMode {
  ...
  HorizontalBillboard =
      2, // Render particles as billboards always facing up along the y-Axis.
  VerticalBillboard = 3, // Render particles as billboards always facing the
                         // player, but not pitching along the x-Axis.
  Mesh = 4,              // Render particles as meshes.
  None = 5 // Don't render particles. (e.g. useful when using the Trail or
           // Lights Module)
}
...
public enum ParticleSystemCollisionQuality { High = 0, Medium = 1, Low = 2 }
...
public enum ParticleSystemShapeType {
  ...
  [ Obsolete(
      "SphereShell is deprecated and does nothing. Please use ShapeModule.radiusThickness instead, to control edge emission.",
      false) ] SphereShell = 1, // Emit from the surface of a sphere.
  Hemisphere = 2,               // Emit from the volume of a half-sphere.
  [ Obsolete(
      "HemisphereShell is deprecated and does nothing. Please use ShapeModule.radiusThickness instead, to control edge emission.",
      false) ] HemisphereShell = 3, // Emit from the surface of a half-sphere.
  Cone = 4,                         // Emit from the base surface of a cone.
  Box = 5,                          // Emit from the volume of a box.
  Mesh = 6,                         // Emit from a mesh.
  [ Obsolete(
      "ConeShell is deprecated and does nothing. Please use ShapeModule.radiusThickness instead, to control edge emission.",
      false) ] ConeShell = 7, // Emit from the base surface of a cone.
  ConeVolume = 8,             // Emit from the volume of a cone.
  [ Obsolete(
      "ConeVolumeShell is deprecated and does nothing. Please use ShapeModule.radiusThickness instead, to control edge emission.",
      false) ] ConeVolumeShell = 9, // Emit from the surface of a cone.
  Circle = 10,                      // Emit from a circle.
  [ Obsolete(
      "CircleEdge is deprecated and does nothing. Please use ShapeModule.radiusThickness instead, to control edge emission.",
      false) ] CircleEdge = 11, // Emit from the edge of a circle.
...
// The animation type
public enum ParticleSystemAnimationType { WholeSheet = 0, SingleRow = 1 }
...
// The collision type
public enum ParticleSystemCollisionType { Planes = 0, World = 1 }

// The collision mode
public enum ParticleSystemCollisionMode { Collision3D = 0, Collision2D = 1 }

// The overlap action
public enum ParticleSystemOverlapAction { Ignore = 0, Kill = 1, Callback = 2 }
...
public enum ParticleSystemSimulationSpace {
  ...
  Custom = 2 // Use custom simulation space, relative to a custom transform
             // component.
}
...
public enum ParticleSystemInheritVelocityMode {
  ...
  Current = 1  // Emitter velocity is inherited over the particle's lifetime
               // using the current emitter velocity.
}
...
public enum ParticleSystemCustomData { Custom1, Custom2 }

// The custom stream modes
public enum ParticleSystemCustomDataMode { Disabled, Vector, Color }
...
public enum ParticleSystemSubEmitterProperties {
  ...
  InheritEverything = InheritColor | InheritSize | InheritRotation |
                      InheritLifetime | InheritDuration,
  InheritColor = 1 << 0,
  InheritSize = 1 << 1,
  InheritRotation = 1 << 2,
  InheritLifetime = 1 << 3,
...
public enum ParticleSystemTrailTextureMode {
  ...
  DistributePerSegment =
      2, // Stretch the texture over the entire trail, but treat each segment as
         // though it is of equal length.
  RepeatPerSegment = 3 // Repeat the texture along the trail, at a rate of one
                       // repetition per segment.
}
...
public enum ParticleSystemShapeMultiModeValue {
  ...
  PingPong = 2,   // Animate the emission point around the shape, alternating
...
public enum ParticleSystemRingBufferMode {
  Disabled = 0,
  PauseUntilReplaced =
      1, // When particles reach the end of their life, pause until replaced.
  LoopUntilReplaced = 2 // When particles reach the fade out time, loop back to
                        // the fade in time. When replaced, play to the end of
                        // their life before actually being replaced.
}

// Select whether to use a layer mask or an explicit list when deciding which
// GameObjects to use with certain Particle System effects.
