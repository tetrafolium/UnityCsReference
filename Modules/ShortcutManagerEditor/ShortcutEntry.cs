...
namespace UnityEditor.ShortcutManagement {
  ...
  [Serializable] struct Identifier {
    public string path;
    ...
    public Identifier(string path) { this.path = path; }
  ...
  class ShortcutEntry {
    ...
    readonly List<KeyCombination> m_DefaultCombinations =
    ...
    internal ShortcutEntry(Identifier id,
                           IEnumerable<KeyCombination> defaultCombination,
                           Action<ShortcutArguments> action, Type context,
                           ShortcutType type, string displayName = null) {
      m_Identifier = id;
      ...
      if (typeof(IShortcutToolContext).IsAssignableFrom(m_Context))
        foreach (var attribute in m_Context.GetCustomAttributes(
                     typeof(ReserveModifiersAttribute), true))
          m_ReservedModifier |=
              (attribute as ReserveModifiersAttribute).modifier;
    }

    public override string ToString() {
      return $"{string.Join(",
             ", combinations.Select(c => c.ToString()).ToArray())} [{context?.Name}]";
    }
    ...
    internal void ResetToDefault() { m_OverriddenCombinations = null; }
    ...
    internal void SetOverride(IEnumerable<KeyCombination> newKeyCombinations) {
      ...
      if (m_Type == ShortcutType.Menu) {
        var newMenuKey = m_OverriddenCombinations.Any()
                             ? m_OverriddenCombinations [0]
                                   .ToMenuShortcutString()
                             : "";
        Menu.SetHotkey(m_Identifier.path.Substring(
                           Discovery.k_MainMenuShortcutPrefix.Length),
                       newMenuKey);
      }
    }
