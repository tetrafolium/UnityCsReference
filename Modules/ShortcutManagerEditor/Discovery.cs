...
public Discovery(
	IEnumerable<IDiscoveryShortcutProvider> shortcutProviders,
	IBindingValidator bindingValidator,
	IDiscoveryInvalidShortcutReporter invalidShortcutReporter = null) {
...
public IEnumerable<ShortcutEntry> GetAllShortcuts() {
	...
	foreach (var discoveryModule in m_ShortcutProviders) {
		...
		foreach (var discoveredEntry in shortcuts) {
			...
			if (shortcutEntry.identifier.path != null &&
			    shortcutEntry.type != ShortcutType.Menu &&
			    shortcutEntry.identifier.path.StartsWith(
				    k_MainMenuShortcutPrefix)) {
				m_InvalidShortcutReporter?.ReportReservedIdentifierPrefixConflict(
					discoveredEntry, k_MainMenuShortcutPrefix);
				continue;
			}

			if (shortcutEntry.displayName != null &&
			    shortcutEntry.type != ShortcutType.Menu &&
			    shortcutEntry.displayName.StartsWith(k_MainMenuShortcutPrefix)) {
				m_InvalidShortcutReporter?.ReportReservedDisplayNamePrefixConflict(
					discoveredEntry, k_MainMenuShortcutPrefix);
				continue;
			}

			if (identifier2ShortcutEntry.Contains(shortcutEntry.identifier)) {
				m_InvalidShortcutReporter?.ReportIdentifierConflict(
					discoveredEntry);
				continue;
			}

			if (displayName2ShortcutEntry.Contains(shortcutEntry.displayName)) {
				m_InvalidShortcutReporter?.ReportDisplayNameConflict(
					discoveredEntry);
				continue;
			}
			...
			if (!m_BindingValidator.IsBindingValid(shortcutEntry.combinations,
			                                       out invalidBindingMessage)) {
				m_InvalidShortcutReporter?.ReportInvalidBinding(
					discoveredEntry, invalidBindingMessage);

				// Replace invalid binding with empty binding
				var emptyBinding = Enumerable.Empty<KeyCombination>();
				shortcutEntry = new ShortcutEntry(
					shortcutEntry.identifier, emptyBinding, shortcutEntry.action,
	...
	if (isEditorWindow) {
		if (context != typeof(EditorWindow) &&!isIShortcutToolContext)
			return true;
	           ...
	           context != typeof(IShortcutToolContext))
		return true;
