...
namespace UnityEditor.ShortcutManagement {
  class Directory : IDirectory {
    ...
    public void Initialize(IEnumerable<ShortcutEntry> entries) {
      ...
      m_IndexedShortcutEntries =
          new List<ShortcutEntry>[ (int) KeyCode.Mouse0 ];
    ...
    private List<ShortcutEntry>
    GetShortcutEntriesForPrimaryKey(List<KeyCombination> combinationSequence) {
      if (combinationSequence == null || combinationSequence.Count < 1)
        return null;
      return m_IndexedShortcutEntries[(int) combinationSequence[0].keyCode];
    }

    // These two overloads have some duplication to avoid creating predicates
    // etc.
    public void FindShortcutEntries(List<KeyCombination> combinationSequence,
                                    Type[] context,
                                    List<ShortcutEntry> outputShortcuts) {
      outputShortcuts.Clear();
      List<ShortcutEntry> entries =
          GetShortcutEntriesForPrimaryKey(combinationSequence);
      if (entries == null)
        return;
      foreach (var entry in entries)
        if (entry.StartsWith(combinationSequence) &&
            ShortcutEntryMatchesAnyContext(entry.context, context))
    ...
    public void FindShortcutEntries(List<KeyCombination> combinationSequence,
      ...
      foreach (var entry in entries)
        if (entry.StartsWith(combinationSequence) &&
            ShortcutEntrySatisfiesContextManager(contextManager, entry))
          outputShortcuts.Add(entry);
    }
    ...
    public ShortcutEntry FindShortcutEntry(Identifier identifier) {
      return m_ShortcutEntries.FirstOrDefault(
          s => s.identifier.Equals(identifier));
    }
    ...
    public void FindPotentialConflicts(Type context,
                                       IList<KeyCombination> binding,
      ...
      output.Clear();
      List<ShortcutEntry> entries =
          m_IndexedShortcutEntries[(int) binding[0].keyCode];
    ...
    public void FindShortcutsWithConflicts(List<ShortcutEntry> output,
                                           IContextManager contextManager) {
      foreach (List<ShortcutEntry> entries in m_IndexedShortcutEntries) {
        ...
        for (int i = 0; i < entries.Count; ++i) {
          for (int j = i + 1; j < entries.Count; ++j) {
            if (DoShortcutEntriesConflict(entries[i], entries[j],
                                          contextManager)) {
    ...
    void AddEntry(KeyCode keyCode, ShortcutEntry entry) {
      ...
      List<ShortcutEntry> entriesForKeyCode =
          m_IndexedShortcutEntries[keyCodeValue];
      if (entriesForKeyCode == null)
        m_IndexedShortcutEntries[keyCodeValue] = entriesForKeyCode =
            new List<ShortcutEntry>();
      entriesForKeyCode.Add(entry);
    }

    bool DoShortcutEntriesConflict(ShortcutEntry shortcutEntry1,
                                   ShortcutEntry shortcutEntry2,
                                   IContextManager contextManager) {
      return contextManager.DoContextsConflict(shortcutEntry1.context,
                                               shortcutEntry2.context) &&
    ...
    //////////////////////////////
    static bool ShortcutEntryMatchesAnyContext(Type shortcutEntryContext,
                                               Type[] contextList) {
      if (contextList == null)
        return true;
    ...
    static bool ShortcutEntryMatchesContext(Type shortcutEntryContext,
                                            Type context) {
      return context == shortcutEntryContext ||
             (shortcutEntryContext != null &&
              shortcutEntryContext.IsAssignableFrom(context));
    }

    static bool
    ShortcutEntrySatisfiesContextManager(IContextManager contextManager,
                                         ShortcutEntry entry) {
      return contextManager.HasActiveContextOfType(entry.context) &&
             // Emulate old play mode shortcut behavior
             // * Non-menu shortcuts do NOT apply only when the Game View is
             // focused, the editor is playing, and is NOT paused
