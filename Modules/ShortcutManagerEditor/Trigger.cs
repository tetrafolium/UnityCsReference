...
namespace UnityEditor.ShortcutManagement {
  class Trigger {
    ...
    Dictionary<KeyCode, KeyValuePair<ShortcutEntry, object>> m_ActiveClutches =
        new Dictionary<KeyCode, KeyValuePair<ShortcutEntry, object>>();
    ...
    public void HandleKeyEvent(Event evt, IContextManager contextManager) {
      ...
      if (evt.type == EventType.KeyUp) {
        ...
        if (m_ActiveClutches.TryGetValue(evt.keyCode, out clutchKeyValuePair)) {
          ...
          var args = new ShortcutArguments{context = clutchContext,
      ...
      // Use the event and return if the key is currently used in an active
      // clutch
      ...
      m_Directory.FindShortcutEntries(m_KeyCombinationSequence, contextManager,
                                      m_Entries);
      ...
      case 1:
        ...
        if (ShortcutFullyMatchesKeyCombination(shortcutEntry)) {
          ...
          args.context =
              contextManager.GetContextInstanceOfType(shortcutEntry.context);
          ...
          case ShortcutType.Clutch:
            if (!m_ActiveClutches.ContainsKey(evt.keyCode)) {
              m_ActiveClutches.Add(evt.keyCode,
                                   new KeyValuePair<ShortcutEntry, object>(
                                       shortcutEntry, args.context));
    ...
    public bool HasAnyEntries() { return m_Entries.Any(); }

    // filtered entries are expected to all be in the same context and/or null
    // context and they all are known to share the prefix
    bool HasConflicts(IEnumerable<ShortcutEntry> filteredEntries,
                      List<KeyCombination> prefix) {
    ...
    void Reset() { m_KeyCombinationSequence.Clear(); }
