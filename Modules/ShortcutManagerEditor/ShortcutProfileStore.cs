...
namespace UnityEditor.ShortcutManagement {
  ...
  class ShortcutProfileStore : IShortcutProfileStore {
    public bool ValidateProfileId(string id) {
      return !string.IsNullOrEmpty(id) && id.Length <= 127 &&
    ...
    public static string GetShortcutFolderPath() {
      return Paths.Combine(InternalEditorUtility.unityPreferencesFolder,
                           "shortcuts", ModeService.currentId);
    }
    ...
    static IEnumerable<string> GetAllShortcutProfilePaths() {
      ...
      if (ModeService.currentId == ModeService.k_DefaultModeId) {
        var legacyShortcutFolder = Paths.Combine(
            InternalEditorUtility.unityPreferencesFolder, "shortcuts");
        if (System.IO.Directory.Exists(legacyShortcutFolder)) {
          var legacyShortcutFiles =
              System.IO.Directory
                  .GetFiles(legacyShortcutFolder, "*.shortcut",
                            System.IO.SearchOption.TopDirectoryOnly)
                  .ToArray();
          if (legacyShortcutFiles.Length > 0 &&
              !System.IO.Directory.Exists(shortcutsFolderPath))
