...
namespace UnityEditor.ShortcutManagement {
  class ShortcutManagerWindowViewController
      : IShortcutManagerWindowViewController,
        IKeyBindingStateProvider {
    ...
    Dictionary<string, List<ShortcutEntry>> m_CategoryToEntriesList =
        new Dictionary<string, List<ShortcutEntry>>();
    Dictionary<KeyCombination, List<ShortcutEntry>>
        m_KeyBindingRootToBoundEntries;

    List<string> m_Categories = new List<string>();
    int m_SelectedCategoryIndex;

    List<ShortcutEntry> m_SelectedCategoryShortcutList =
    ...
    List<ShortcutEntry> m_ShortcutsBoundToSelectedKey =
        new List<ShortcutEntry>();

    public ShortcutManagerWindowViewController(
        SerializedShortcutManagerWindowState state, IDirectory directory,
        IBindingValidator bindingValidator,
        IShortcutProfileManager profileManager, IContextManager contextManager,
        IAvailableShortcutsChangedNotifier availableShortcutsChangedNotifier) {
    ...
    void InitializeCategoryList() {
      ...
      selectedEntry = m_Directory.FindShortcutEntry(
          m_SerializedState.selectedEntryIdentifier);
    }
    ...
    public void OnEnable() {
      ...
      m_AvailableShortcutsChangedNotifier.availableShortcutsChanged +=
          OnAvailableShortcutsChanged;
    }
    ...
    void OnActiveProfileChanged(IShortcutProfileManager shortcutProfileManager,
                                ShortcutProfile previousActiveProfile,
                                ShortcutProfile currentActiveProfile) {
      UpdateInternalCache();
    }
    ...
    void
    OnLoadedProfilesChanged(IShortcutProfileManager shortcutProfileManager) {
      m_ShortcutManagerWindowView.RefreshProfiles();
    }

    // There is a mutual dependecy between the view and the viewcontroller,
    // Ideally we would inject dependecies via the constructor, but since we
    // have a circular dependecy that is not possible, so until I find a better
    // solution, we inject the dependency here vie this method.
    ...
    public void ResetToDefault(ShortcutEntry entry) {
      ...
      if (conflicts.Count == 0) {
        ...
        var howToHandle =
            m_ShortcutManagerWindowView.HandleRebindWillCreateConflict(
                entry, newBinding, conflicts);
        switch (howToHandle) {
    ...
    void BuildCategoryList() {
      ...
      foreach (var entry in entries) {
        ...
        if (entry.type == ShortcutType.Menu) {
          ...
          if (index > -1) {
            ...
            m_CategoryToEntriesList [category]
                .Add(entry);
          }
        }
      }
      ...
      m_CategoryToEntriesList.Add(m_CommandsWithConflicts,
                                  new List<ShortcutEntry>());
    ...
    void UpdateCommandsWithConflicts() {
      GetAllCommandsWithConflicts(
          m_CategoryToEntriesList[m_CommandsWithConflicts]);
    }
    ...
    public bool CanCreateProfile(string newProfileId) {
      return m_ShortcutProfileManager.CanCreateProfile(newProfileId) ==
             CanCreateProfileResult.Success;
    }
    ...
    public bool CanRenameActiveProfile() { return CanDeleteActiveProfile(); }

    public bool CanRenameActiveProfile(string newProfileId) {
      if (m_ShortcutProfileManager.activeProfile == null)
        return false;

      return m_ShortcutProfileManager.CanRenameProfile(
                 m_ShortcutProfileManager.activeProfile, newProfileId) ==
             CanRenameProfileResult.Success;
    }
    ...
    public void RenameActiveProfile(string newProfileId) {
      ...
      m_ShortcutProfileManager.RenameProfile(
          m_ShortcutProfileManager.activeProfile, newProfileId);
    }
    ...
    public bool CanDeleteActiveProfile() {
      ...
      return m_ShortcutProfileManager.CanDeleteProfile(
                 m_ShortcutProfileManager.activeProfile) ==
             CanDeleteProfileResult.Success;
    }
    ...
    public void DeleteActiveProfile() {
      ...
      m_ShortcutProfileManager.DeleteProfile(
          m_ShortcutProfileManager.activeProfile);
    }

    public IList<string> GetCategories() { return m_Categories; }

    public int categorySeparatorIndex { get; }
    = k_MainMenuIndex;
    ...
    string selectedCategory {
      ...
      set {
        ...
        m_SerializedState.selectedCategory =
            m_Categories[m_SelectedCategoryIndex];
        PopulateShortcutList();
      }
    }
    ...
    public string GetSearch() { return m_SerializedState.search; }
    ...
    public void SetSelectedSearchFilter(string filter) {
      if (filter == m_AllUnityCommands)
        m_SerializedState.searchCategoryFilter =
            SearchCategoryFilter.IgnoreCategory;
      else if (filter == m_Categories[m_SelectedCategoryIndex])
        m_SerializedState.searchCategoryFilter =
            SearchCategoryFilter.SearchWithinSelectedCategory;
      else
    ...
    void PopulateShortcutList() {
      ...
      if (IsSearching() && m_SerializedState.searchCategoryFilter ==
                               SearchCategoryFilter.IgnoreCategory)
      ...
      foreach (var indentifier in identifierList) {
        ...
        if (BelongsToSearch(entry)) {
          ...
          m_SelectedCategoryShortcutPathList.Add(
              entry.displayName.StartsWith(category)
                  ? entry.displayName.Substring(category.Length + 1)
                  : entry.displayName);
        }
      }
    }
    ...
    bool BelongsToSearch(ShortcutEntry entry) {
      ...
      case SearchOption.Name:
        return entry.displayName.IndexOf(
                   m_SerializedState.search,
    ...
    public ShortcutEntry selectedEntry {
      ...
      private set {
        ...
        if (m_SelectedEntry != null) {
          m_SerializedState.selectedEntryIdentifier =
              m_SelectedEntry.identifier;
          m_SelectedEntryIndex = m_SelectedCategoryShortcutList.FindIndex(
              entry => entry == m_SelectedEntry);
        } else {
    ...
    public void DragEntryAndDropIntoKey(KeyCode keyCode,
      ...
      keyCombination.Add(
          KeyCombination.FromKeyboardInput(keyCode, eventModifier));
    ...
    string GetUniqueProfileId() {
      ...
      int index;
      for (index = 1;
           existingProfileIds.Contains(desiredId) && index != int.MaxValue;
           index++) {
    ...
    void RebindEntry(ShortcutEntry entry, List<KeyCombination> keyCombination) {
      ...
      if (m_ShortcutProfileManager.activeProfile == null) {
        ...
        m_ShortcutProfileManager.activeProfile =
            m_ShortcutProfileManager.CreateProfile(uniqueProfileId);
      }

      m_ShortcutProfileManager.ModifyShortcutEntry(entry.identifier,
                                                   keyCombination);
      UpdateCommandsWithConflicts();
      BuildKeyMapBindingStateData();
      PopulateShortcutList();
    ...
    public bool CanEntryBeAssignedToKey(KeyCode keyCode,
                                        EventModifiers eventModifier,
                                        ShortcutEntry entry) {
      ...
      if (m_KeyBindingRootToBoundEntries.TryGetValue(keycombination,
                                                     out entries)) {
        foreach (var boundEntry in entries) {
          if (IsGlobalContext(entry))
            return false;

          if (boundEntry.context.IsAssignableFrom(entry.context) ||
              entry.context.IsAssignableFrom(boundEntry.context))
            return false;
        }
      }
    ...
    void PopulateShortcutsBoundToSelectedKey() {
      ...
      var keycombination = KeyCombination.FromKeyboardInput(
          m_SerializedState.selectedKey, m_SerializedState.selectedModifiers);
      List<ShortcutEntry> entries;
      if (m_KeyBindingRootToBoundEntries.TryGetValue(keycombination,
                                                     out entries)) {
    ...
    public KeyCode GetSelectedKey() { return m_SerializedState.selectedKey; }
    ...
    public void RequestRebindOfSelectedEntry(List<KeyCombination> newbinding) {
      ...
      if (conflicts.Count == 0) {
        ...
        var howToHandle =
            m_ShortcutManagerWindowView.HandleRebindWillCreateConflict(
                selectedEntry, newbinding, conflicts);
        switch (howToHandle) {
        case RebindResolution.DoNotRebind:
          break;
        case RebindResolution.CreateConflict:
    ...
    public IList<ShortcutEntry> GetSelectedEntryConflictsForGivenKeyCombination(
        List<KeyCombination> temporaryCombination) {
    ...
    public IList<ShortcutEntry> GetShortcutsBoundTo(KeyCode keyCode,
    ...
    public bool IsEntryPartOfConflict(ShortcutEntry shortcutEntry) {
      return m_CategoryToEntriesList [m_CommandsWithConflicts]
          .Contains(shortcutEntry);
    }

    // TODO: find a better place for this logic, Directory maybe?
    IList<ShortcutEntry>
    FindConflictsIfRebound(ShortcutEntry entry,
                           List<KeyCombination> newCombination) {
      var conflictingShortcuts = new List<ShortcutEntry>();
      m_Directory.FindPotentialConflicts(entry.context, newCombination,
    ...
    void BuildKeyMapBindingStateData() {
      m_KeyBindingRootToBoundEntries =
          new Dictionary<KeyCombination, List<ShortcutEntry>>();
      foreach (var entry in m_AllEntries) {
        ...
        if (binding != null && binding.Any()) {
          ...
          if (!m_KeyBindingRootToBoundEntries.ContainsKey(firstKeyBinding)) {
            m_KeyBindingRootToBoundEntries[firstKeyBinding] =
                new List<ShortcutEntry>();
          }
          m_KeyBindingRootToBoundEntries [firstKeyBinding]
              .Add(entry);
    ...
    public BindingState
    GetBindingStateForKeyWithModifiers(KeyCode keyCode,
                                       EventModifiers modifiers) {
      var keycombination = KeyCombination.FromKeyboardInput(keyCode, modifiers);
      List<ShortcutEntry> entries;
      if (m_KeyBindingRootToBoundEntries.TryGetValue(keycombination,
    ...
    public static EventModifiers ModifierFromKeyCode(KeyCode k) {
      ...
      case KeyCode.RightControl:
        return SystemInfo.operatingSystemFamily ==
               OperatingSystemFamily.MacOSX ? EventModifiers
                   .None : EventModifiers.Control;
      case KeyCode.LeftCommand:
      case KeyCode.RightCommand:
        return SystemInfo.operatingSystemFamily ==
               OperatingSystemFamily.MacOSX ? EventModifiers
                   .Command : EventModifiers.None;
      default:
  ...
  class ConflictComparer : IComparer<ShortcutEntry> {
    ...
    public int Compare(ShortcutEntry x, ShortcutEntry y) {
      return m_KeyCombinationSequenceComprarer.Compare(x?.combinations,
                                                       y?.combinations);
    }
  }

  class KeyCombinationSequenceComparer
      : IComparer<IEnumerable<KeyCombination>> {
    ...
    public int Compare(IEnumerable<KeyCombination> x,
                       IEnumerable<KeyCombination> y) {
      if (x == null && y != null)
        return -1;
      if (x != null && y == null)
        return 1;
      ...
      while (xValid && yValid) {
        var elementCompare =
            m_KeyCombinationComparer.Compare(xEnum.Current, yEnum.Current);
        if (elementCompare != 0) {
          xEnum.Dispose();
          yEnum.Dispose();
