...
namespace UnityEditor.ShortcutManagement {
  interface IConflictResolverView {
    void Show(IConflictResolver conflictResolver,
              IEnumerable<KeyCombination> keyCombinationSequence,
              IEnumerable<ShortcutEntry> entries);
  }

  class ConflictResolverView : IConflictResolverView {
    public void Show(IConflictResolver conflictResolver,
                     IEnumerable<KeyCombination> keyCombinationSequence,
                     IEnumerable<ShortcutEntry> entries) {
      ConflictResolverWindow.Show(conflictResolver, keyCombinationSequence,
                                  entries);
    }
  }
  ...
  class ConflictResolverWindow : EditorWindow {
    ...
    internal static ConflictResolverWindow
    Show(IConflictResolver conflictResolver,
         IEnumerable<KeyCombination> keyCombinationSequence,
         IEnumerable<ShortcutEntry> entries) {
    ...
    static class Contents {
      public static GUIContent description = EditorGUIUtility.TrTextContent(
          "You can choose to perform a single command, rebind the shortcut to the selected command, or resolve the conflict in the Shortcut Manager.");
      public static GUIContent cancel =
          EditorGUIUtility.TrTextContent("Cancel");
      public static GUIContent perform =
          EditorGUIUtility.TrTextContent("Perform Selected");
      public static GUIContent rebind =
          EditorGUIUtility.TrTextContent("Rebind Selected");

      public static GUIContent itemName =
          EditorGUIUtility.TrTextContent("Name");
      public static GUIContent itemType =
          EditorGUIUtility.TrTextContent("Type");
      public static GUIContent itemBindings =
          EditorGUIUtility.TrTextContent("Shortcut");

      public static GUIContent windowTitle =
          EditorGUIUtility.TrTextContent("Shortcut Conflict");
      public static GUIContent rebindToSelectedCommand =
          EditorGUIUtility.TrTextContent("Rebind to selected command");
      public static GUIContent SelectCommandHeading =
          EditorGUIUtility.TrTextContent("Select a command to perform:");
      public static GUIContent OpenShortcutManager =
          EditorGUIUtility.TrTextContent("Resolve Conflict...");

      public static Texture2D warningIcon =
          (Texture2D) EditorGUIUtility.LoadRequired(
              "Icons/ShortcutManager/alertDialog.png");
    }

    class ConflictListView : TreeView {
      List<ShortcutEntry> m_Entries;

      enum MyColumns { Name, Type, Binding }
      ...
      protected override TreeViewItem BuildRoot() {
        ...
        for (var index = 0; index < m_Entries.Count; index++) {
          ...
          allItems.Add(new TreeViewItem{
              id = index, depth = 0, displayName = shortcutEntry.displayName});
        }
      ...
      protected override void RowGUI(RowGUIArgs args) {
        ...
        for (int i = 0; i < args.GetNumVisibleColumns(); ++i) {
          CellGUI(args.GetCellRect(i), item, (MyColumns) args.GetColumn(i),
                  ref args);
        }
      }

      static void CellGUI(Rect getCellRect, ShortcutEntry item,
                          MyColumns getColumn, ref RowGUIArgs args) {
        switch (getColumn) {
        case MyColumns.Name: {
        ...
        case MyColumns.Binding: {
          GUI.Label(getCellRect,
                    KeyCombination.SequenceToString(item.combinations));
          break;
        }
        }
      }
    ...
    enum ActionSelected { Cancel, ExecuteOnce, ExecuteAlways }
    ...
    internal void Init(IConflictResolver conflictResolver,
                       IEnumerable<KeyCombination> keyCombinationSequence,
                       IEnumerable<ShortcutEntry> entries,
                       GUIView previouslyFocusedView) {
      ...
      m_ConflictListView =
          new ConflictListView(m_TreeViewState, multiColumnHeader, m_Entries);

      m_Header = string.Format(
          L10n.Tr("The binding \"{0}\" conflicts with multiple commands."),
          KeyCombination.SequenceToString(keyCombinationSequence));
    }
    ...
    private void OnEnable() {
      ...
      var columns = new[]{new MultiColumnHeaderState.Column{
    ...
    private void OnDisable() {
      ...
      // We need to delay this action, since actions can depend on the right
      // view having focus, and when closing a window that will change the
      // current focused view to null.
    ...
    private void OnGUI() {
      ...
      EditorGUILayout.BeginHorizontal();
      {
        ...
        EditorGUILayout.BeginVertical(Styles.commandsArea);
        {
          ...
          GUILayout.Label(Contents.SelectCommandHeading,
          ...
          m_Rebind =
              GUILayout.Toggle(m_Rebind, Contents.rebindToSelectedCommand);
        }
      ...
      EditorGUILayout.BeginHorizontal();
      {
        ...
        using(new EditorGUI.DisabledScope(!hasSelection)) {
          ...
          using(new EditorGUI.DisabledScope(
              entry == null ||
              (entry.type == ShortcutType.Clutch && !m_Rebind))) {
            var buttonLabel =
                hasSelection &&
                entry.type == ShortcutType.Clutch ? Contents.rebind
                : Contents.perform;
