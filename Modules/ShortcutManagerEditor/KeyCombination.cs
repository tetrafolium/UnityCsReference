...
namespace UnityEditor.ShortcutManagement {
  ...
  public struct KeyCombination : IEquatable<KeyCombination> {
    ...
    [SerializeField] ShortcutModifiers m_Modifiers;

    public KeyCode keyCode => m_KeyCode;
    public ShortcutModifiers modifiers => m_Modifiers;

    static Dictionary<KeyCode, string> s_KeyCodeToMenuItemKeyCodeString =
        new Dictionary<KeyCode, string>(){{KeyCode.LeftArrow, "LEFT"},
                                          {KeyCode.UpArrow, "UP"},
    ...
    static KeyCombination() {
      // Populate s_MenuItemKeyCodeStringToKeyCode by reversing
      // s_KeyCodeToMenuItemKeyCodeString
      s_MenuItemKeyCodeStringToKeyCode = new Dictionary<string, KeyCode>(
          s_KeyCodeToMenuItemKeyCodeString.Count);
      foreach (var entry in s_KeyCodeToMenuItemKeyCodeString) {
        s_MenuItemKeyCodeStringToKeyCode.Add(entry.Value, entry.Key);
      }
    }

    public KeyCombination(KeyCode keyCode, ShortcutModifiers shortcutModifiers =
    ...
    internal static KeyCombination FromKeyboardInput(KeyCode keyCode,
                                                     EventModifiers modifiers) {
      return new KeyCombination(
          keyCode, ConvertEventModifiersToShortcutModifiers(modifiers, false));
    }

    internal static KeyCombination FromPrefKeyKeyboardEvent(Event evt) {
      return new KeyCombination(
          evt.keyCode,
          ConvertEventModifiersToShortcutModifiers(evt.modifiers, true));
    }

    static ShortcutModifiers
    ConvertEventModifiersToShortcutModifiers(EventModifiers eventModifiers,
                                             bool coalesceCommandAndControl) {
      ...
      if (coalesceCommandAndControl) {
        if ((eventModifiers &
             (EventModifiers.Command | EventModifiers.Control)) != 0)
          modifiers |= ShortcutModifiers.Action;
      } else if (Application.platform == RuntimePlatform.OSXEditor &&
                 (eventModifiers & EventModifiers.Command) != 0)
        modifiers |= ShortcutModifiers.Action;
      else if (Application.platform != RuntimePlatform.OSXEditor &&
    ...
    public bool alt =>
        (modifiers & ShortcutModifiers.Alt) == ShortcutModifiers.Alt;
    public bool action =>
        (modifiers & ShortcutModifiers.Action) == ShortcutModifiers.Action;
    public bool shift =>
        (modifiers & ShortcutModifiers.Shift) == ShortcutModifiers.Shift;
    ...
    SequenceToString(IEnumerable<KeyCombination> keyCombinations) {
    ...
    static void KeyCodeToMenuItemKeyCodeString(KeyCode keyCode,
                                               StringBuilder builder) {
      string keyCodeString;
      if (s_KeyCodeToMenuItemKeyCodeString.TryGetValue(keyCode,
                                                       out keyCodeString)) {
    ...
    TryParseMenuItemBindingString(string menuItemBindingString,
                                  out KeyCombination keyCombination) {
      ...
      var keyCodeString = menuItemBindingString.Substring(
          startIndex, menuItemBindingString.Length - startIndex);
      KeyCode keyCode;
      ShortcutModifiers additionalModifiers;
      if (!TryParseMenuItemKeyCodeString(keyCodeString, out keyCode,
                                         out additionalModifiers)) {
        keyCombination = default(KeyCombination);
        return false;
      }
    ...
    static bool
    TryParseMenuItemKeyCodeString(string keyCodeString, out KeyCode keyCode,
                                  out ShortcutModifiers additionalModifiers) {
      ...
      if (s_MenuItemKeyCodeStringToKeyCode.TryGetValue(keyCodeString,
                                                       out keyCode))
        return true;
    ...
    internal static string
    SequenceToMenuString(IEnumerable<KeyCombination> keyCombinations) {
      if (!keyCombinations.Any())
        return "";

      // TODO: once we start supporting chords we need to figure out how to
      // represent that for menus.
      return keyCombinations.Single().ToMenuShortcutString();
    }

    static void VisualizeModifiers(ShortcutModifiers modifiers,
                                   StringBuilder builder) {
    ...
    static Dictionary<KeyCode, string> s_KeyCodeNamesMacOS =
        new Dictionary<KeyCode, string>{
            {KeyCode.Backspace, "⌫"}, {KeyCode.Tab, "⇥"},
            {KeyCode.Return, "↩"},    {KeyCode.Escape, "⎋"},
            {KeyCode.Delete, "⌦"},    {KeyCode.UpArrow, "↑"},
            {KeyCode.DownArrow, "↓"}, {KeyCode.RightArrow, "→"},
            {KeyCode.LeftArrow, "←"}, {KeyCode.Home, "↖"},
            {KeyCode.End, "↘"},       {KeyCode.PageUp, "⇞"},
            {KeyCode.PageDown, "⇟"},
        };

    static Dictionary<KeyCode, string> s_KeyCodeNamesNotMacOS =
        new Dictionary<KeyCode, string>{
    ...
    static Dictionary<KeyCode, string> s_KeyCodeNamesCommon =
        new Dictionary<KeyCode, string>{
            {KeyCode.Exclaim, "!"},
            {KeyCode.DoubleQuote, "\""},
            {KeyCode.Hash, "#"},
    ...
    public override int GetHashCode() {
      unchecked { return ((int) m_KeyCode * 397) ^ (int) m_Modifiers; }
    }
  }
}
