...
namespace UnityEditor.ShortcutManagement {
  ...
  interface IShortcutProfileManager {
    event Action<IShortcutProfileManager, Identifier, ShortcutBinding,
                 ShortcutBinding> shortcutBindingChanged;
    event Action<IShortcutProfileManager, ShortcutProfile, ShortcutProfile>
    ...
    CanCreateProfileResult CanCreateProfile(
        string profileId, ShortcutProfile parentProfile = null);
    ShortcutProfile CreateProfile(string profileId,
                                  ShortcutProfile parentProfile = null);
    CanDeleteProfileResult CanDeleteProfile(ShortcutProfile profile);
    void DeleteProfile(ShortcutProfile profile);
    CanRenameProfileResult CanRenameProfile(ShortcutProfile profile,
                                            string newProfileId);
    void RenameProfile(ShortcutProfile profile, string newProfileId);

    IEnumerable<ShortcutEntry>
    GetAllShortcuts(); // Preferences UI relies on this order being stable
    void ModifyShortcutEntry(Identifier identifier,
                             IEnumerable<KeyCombination> combinationSequence);
    void ClearShortcutOverride(Identifier identifier);
    void ResetToDefault(); // TODO: Remove this when current UI is replaced. Set
                           // activeProfile to null instead.
  ...
  class ShortcutProfileManager : IShortcutProfileManager {
    ...
    Dictionary<string, ShortcutProfile> m_LoadedProfiles =
        new Dictionary<string, ShortcutProfile>();

    public event Action<IShortcutProfileManager, Identifier, ShortcutBinding,
                        ShortcutBinding> shortcutBindingChanged;
    public event Action<IShortcutProfileManager, ShortcutProfile,
                        ShortcutProfile> activeProfileChanged;
    ...
    public ShortcutProfileManager(IEnumerable<ShortcutEntry> baseProfile,
                                  ...
                                  IShortcutProfileStore profileStore) {
    ...
    public ShortcutProfile activeProfile {
      ...
      set {
        ...
        if (m_ActiveProfile != lastActiveProfile)
          activeProfileChanged?.Invoke(this, lastActiveProfile,
                                       m_ActiveProfile);
      }
    }
    ...
    public CanCreateProfileResult
    CanCreateProfile(string profileId, ShortcutProfile parentProfile = null) {
    ...
    public ShortcutProfile CreateProfile(string profileId,
      ...
      case CanCreateProfileResult.MissingParentProfile:
        throw new ArgumentException("Missing parent profile",
                                    nameof(parentProfile));
    ...
    public void DeleteProfile(ShortcutProfile profile) {
      ...
      case CanDeleteProfileResult.ProfileHasDependencies:
        throw new ArgumentException("Profile has dependencies",
                                    nameof(profile));
    ...
    public CanRenameProfileResult CanRenameProfile(ShortcutProfile profile,
                                                   string newProfileId) {
      // Profile must exist
      if (!m_LoadedProfiles.ContainsKey(profile.id))
    ...
    public void RenameProfile(ShortcutProfile profile, string newProfileId) {
      ...
      case CanRenameProfileResult.Success: {
        var newProfile = new ShortcutProfile(newProfileId, profile.entries,
                                             profile.parentId);
        m_LoadedProfiles.Add(newProfile.id, newProfile);
        SaveShortcutProfile(newProfile);
        foreach (var childProfile in m_LoadedProfiles.Values.Where(
                     childProfile => childProfile.parent == profile)) {
      ...
      case CanRenameProfileResult.DuplicateProfileId:
        throw new ArgumentException("Duplicate profile id",
                                    nameof(newProfileId));
    ...
    public IEnumerable<ShortcutEntry> GetAllShortcuts() { return m_Entries; }

    public void
    ModifyShortcutEntry(Identifier identifier,
                        IEnumerable<KeyCombination> combinationSequence) {
      if (activeProfile == null)
        throw new InvalidOperationException("No active profile");

      string invalidBindingMessage;
      if (!m_BindingValidator.IsBindingValid(combinationSequence,
                                             out invalidBindingMessage))
        throw new ArgumentException(invalidBindingMessage,
                                    nameof(combinationSequence));

      var shortcutEntry =
          m_Entries.FirstOrDefault(e => e.identifier.Equals(identifier));
      ...
      foreach (var activeProfileEntry in m_ActiveProfile.entries) {
        if (activeProfileEntry.identifier.Equals(identifier)) {
          ...
          profileEntry.combinations =
              new List<KeyCombination>(combinationSequence);
          break;
        }
      }
    ...
    // TODO: Make this private when current UI is replaced. Set activeProfile to
    // null instead.
    ...
    public void ResetToDefault(ShortcutEntry entry) {
      ...
      shortcutBindingChanged?.Invoke(this, entry.identifier, oldBinding,
                                     newBinding);
    }
    ...
    void ResetActiveProfile() {
      var newProfile =
          new ShortcutProfile(m_ActiveProfile.id, m_ActiveProfile.parentId){
    ...
    void ApplySingleProfile(ShortcutProfile shortcutProfile) {
      foreach (var shortcutOverride in shortcutProfile.entries) {
        var entry = m_Entries.FirstOrDefault(
            e => e.identifier.Equals(shortcutOverride.identifier));
        if (entry != null) {
          string invalidBindingMessage;
          if (!m_BindingValidator.IsBindingValid(shortcutOverride.combinations,
                                                 out invalidBindingMessage)) {
            ...
            Debug.LogWarning(
                $"Ignoring shortcut {nameSnippet} in profile \"{shortcutProfile.id}\" with invalid binding.\n{invalidBindingMessage}.");
            continue;
          }
    ...
    void InitializeLoadedProfilesDictionary() {
      ...
      foreach (var profile in foundShortcutProfiles) {
        if (m_LoadedProfiles.ContainsKey(profile.id)) {
          Debug.LogWarning(
              $"Found multiple shortcut profiles that share the id '{profile.id}'!");
        } else {
    ...
    void ResolveProfileParentReference(ShortcutProfile profile) {
      ...
      if (m_LoadedProfiles.ContainsKey(profile.parentId))
        ...
        Debug.LogWarning(
            $"Shortcut profile with id '{profile.id}' references unknown parent profile with id '{profile.parentId}'. Breaking parent link.");
    ...
    static void BreakProfileParentCyclicDependency(ShortcutProfile profile) {
      ...
      do {
        ...
        if (seenProfiles.Contains(profile.parent)) {
          Debug.LogWarning(
              $"Cyclic dependency between shortcut profiles found! Breaking parent link at '{profile.id}'.");
          profile.BreakParentLink();
          return;
        }
    ...
    void SaveShortcutProfile(ShortcutProfile profile) {
      m_ProfileStore.SaveShortcutProfileJson(profile.id,
    ...
    ShortcutProfile LoadProfile(string id) {
      ...
      throw new FileNotFoundException(
          string.Format("'{0}.shortcut' does not exist!", id));
    }
    ...
    void LoadAndApplyJsonFile(string id, ShortcutProfile instance) {
      ...
      if (id != instance.id) {
        Debug.LogWarning(string.Format(
            "The identifier '{0}' doesn't match the filename of '{1}.shortcut'!",
            id, instance.id));
      }
    }
  }
