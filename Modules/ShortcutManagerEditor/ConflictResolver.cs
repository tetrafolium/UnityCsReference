...
public ConflictResolver(IShortcutProfileManager profileManager,
                        IContextManager contextManager,
                        IConflictResolverView conflictResolverView) {
...
public void
ResolveConflict(IEnumerable<KeyCombination> keyCombinationSequence,
	...
	foreach (var entry in entries) {
		m_Contexts.Add(
			m_ContextManager.GetContextInstanceOfType(entry.context));
	}
...
public void ExecuteAlways(ShortcutEntry entry) {
	...
	foreach (var shortcutEntry in m_Entries) {
		if (shortcutEntry != entry) {
			m_ProfileManager.ModifyShortcutEntry(shortcutEntry.identifier,
			                                     new List<KeyCombination>(0));
		}
	}
...
void Execute(ShortcutEntry entry) {
	if (entry.type == ShortcutType.Clutch)
		throw new InvalidOperationException(
			      "Clutches cannot be activated through conflict resolution");
