...
namespace UnityEditor.ShortcutManagement {
  ...
  class ContextManager : IContextManager {
    ...
    List<IShortcutToolContext> m_PriorityContexts =
        new List<IShortcutToolContext>();

    List<IShortcutToolContext> m_ToolContexts =
        new List<IShortcutToolContext>();
    static Dictionary<Type, bool> s_IsPriorityContextCache =
        new Dictionary<Type, bool>();

    public int activeContextCount => 1 + ((focusedWindow != null) ? 1 : 0) +
                                     m_PriorityContexts.Count(c => c.active) +
                                     m_ToolContexts.Count(c => c.active);

    public bool playModeContextIsActive => focusedWindow is GameView &&
    ...
    static bool IsPriorityContext(Type context) {
      ...
      if (!s_IsPriorityContextCache.TryGetValue(context, out result)) {
        result = Attribute.GetCustomAttribute(
                     context, typeof(PriorityContextAttribute)) != null;
    ...
    public bool DoContextsConflict(Type context1, Type context2) {
      ...
      if (context1.IsAssignableFrom(context2) ||
          context2.IsAssignableFrom(context1))
        return true;
    ...
    public bool HasAnyPriorityContext() { return m_PriorityContexts.Count > 0; }
    ...
    public object GetContextInstanceOfType(Type type) {
      ...
      if (m_FocusedWindow != null && m_FocusedWindow.IsAlive &&
          type.IsInstanceOfType(m_FocusedWindow.Target))
