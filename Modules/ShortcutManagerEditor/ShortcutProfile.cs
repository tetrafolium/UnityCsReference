...
namespace UnityEditor.ShortcutManagement {
  ...
  class SerializableShortcutEntry {
    ...
    internal
    SerializableShortcutEntry(Identifier id,
                              IEnumerable<KeyCombination> combinations) {
      identifier = id;
  ...
  class ShortcutProfile {
    ...
    [SerializeField] string m_ParentId;

    [SerializeField] List<SerializableShortcutEntry> m_Entries =
        new List<SerializableShortcutEntry>();

    public ShortcutProfile parent {
      get;
      internal set;
    }
    ...
    internal ShortcutProfile(string id,
                             IEnumerable<SerializableShortcutEntry> entries,
                             string parentId = "") {
      m_Id = id;
      m_ParentId = parentId;
    ...
    public void Add(ShortcutEntry profileEntry) {
      var existingEntry = m_Entries.SingleOrDefault(
          entry => entry.identifier.Equals(profileEntry.identifier));
      if (existingEntry != null) {
        throw new ArgumentException(
            "This profile already contains an existing entry with matching Identifier!",
            nameof(profileEntry));
      }
    ...
    public void Remove(Identifier identifier) {
      var existingEntry = m_Entries.FirstOrDefault(
          entry => entry.identifier.Equals(identifier));
