...
public struct ActiveProfileChangedEventArgs {
	public string previousActiveProfileId {
		get;
	}
	public string currentActiveProfileId {
		get;
	}
...
public struct ShortcutBindingChangedEventArgs {
	...
	public ShortcutBinding oldBinding {
		get;
	}
	public ShortcutBinding newBinding {
		get;
	}
...
public static IShortcutManager instance {
	get;
}
        = new ShortcutManagerImplementation(
		  ShortcutIntegration.instance.profileManager);
}
...
internal
ShortcutManagerImplementation(IShortcutProfileManager profileManager) {
	m_ShortcutProfileManager = profileManager;

	// TODO: Not sure if these events mean the same
	m_ShortcutProfileManager.activeProfileChanged +=
		RaiseActiveProfileChanged;
	m_ShortcutProfileManager.shortcutBindingChanged +=
		RaiseShortcutBindingChanged;
}

void
RaiseActiveProfileChanged(IShortcutProfileManager shortcutProfileManager,
                          ShortcutProfile oldActiveProfile,
                          ShortcutProfile newActiveProfile) {
	var oldActiveProfileId =
		oldActiveProfile?.id ?? ShortcutManager.defaultProfileId;
	var newActiveProfileId =
		newActiveProfile?.id ?? ShortcutManager.defaultProfileId;
	var eventArgs = new ActiveProfileChangedEventArgs(oldActiveProfileId,
	                                                  newActiveProfileId);
	activeProfileChanged?.Invoke(eventArgs);
}

void RaiseShortcutBindingChanged(
	IShortcutProfileManager shortcutProfileManager, Identifier identifier,
	ShortcutBinding oldBinding, ShortcutBinding newBinding) {
	var eventArgs = new ShortcutBindingChangedEventArgs(
		identifier.path, oldBinding, newBinding);
...
public string activeProfileId {
	get {
		return m_ShortcutProfileManager.activeProfile?.id ?? ShortcutManager
		       .defaultProfileId;
	}
	set {
		if (value == null)
...
public IEnumerable<string> GetAvailableProfileIds() {
	...
	foreach (var id in m_ShortcutProfileManager.GetProfiles().Select(
			 profile => profile.id))
		yield return id;
}
...
public bool IsProfileIdValid(string profileId) {
	...
	return !profileId.Equals(string.Empty) && profileId.Length <= 127 &&
	       profileId.IndexOfAny(Path.GetInvalidFileNameChars()) == -1;
}
...
public void DeleteProfile(string profileId) {
	if (IsProfileReadOnly(profileId))
		throw new ArgumentException("Cannot delete read-only profile",
		                            nameof(profileId));
...
public void RenameProfile(string profileId, string newProfileId) {
	if (IsProfileReadOnly(profileId))
		throw new ArgumentException("Cannot rename read-only profile",
		                            nameof(profileId));
	...
	switch (
		m_ShortcutProfileManager.CanRenameProfile(profile, newProfileId)) {
...
public IEnumerable<string> GetAvailableShortcutIds() {
	return m_ShortcutProfileManager.GetAllShortcuts().Select(
		entry => entry.identifier.path);
}
...
public ShortcutBinding GetShortcutBinding(string shortcutId) {
	...
	var shortcutEntry = shortcutEntries.FirstOrDefault(
		entry => entry.identifier.path == shortcutId);
...
public void RebindShortcut(string shortcutId, ShortcutBinding binding) {
	...
	m_ShortcutProfileManager.ModifyShortcutEntry(
		shortcutEntry.identifier, binding.keyCombinationSequence);
}
...
public void ClearShortcutOverride(string shortcutId) {
	...
	if (IsProfileReadOnly(activeProfileId))
		throw new InvalidOperationException(
			      "Cannot clear shortcut override on read-only profile");
