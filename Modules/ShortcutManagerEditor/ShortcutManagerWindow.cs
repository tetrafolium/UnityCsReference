...
enum RebindResolution { DoNotRebind, CreateConflict, UnassignExistingAndBind }
...
RebindResolution HandleRebindWillCreateConflict(
	ShortcutEntry entry, IList<KeyCombination> newBinding,
	IList<ShortcutEntry> conflicts);
}

enum BindingState { NotBound = 0, BoundGlobally = 1, BoundToContext = 2 }

enum SearchOption { Name, Binding }
...
void DragEntryAndDropIntoKey(KeyCode keyCode, EventModifiers eventModifier,
                             ShortcutEntry entry);
...
IList<ShortcutEntry> GetSelectedEntryConflictsForGivenKeyCombination(
	List<KeyCombination> temporaryCombination);
IList<ShortcutEntry> GetShortcutsBoundTo(KeyCode keyCode,
                                         EventModifiers modifiers);
bool IsEntryPartOfConflict(ShortcutEntry shortcutEntry);
}

interface IKeyBindingStateProvider {
BindingState GetBindingStateForKeyWithModifiers(KeyCode keyCode,
                                                EventModifiers modifiers);
...
[SerializeField]
internal SearchCategoryFilter searchCategoryFilter =
...
enum SearchCategoryFilter { IgnoreCategory, SearchWithinSelectedCategory }

class ShortcutManagerWindow : EditorWindow {
SerializedShortcutManagerWindowState m_State =
	new SerializedShortcutManagerWindowState();
...
void OnEnable() {
	...
	// Workaround for the rootVisualContainer not having a height set on
	// AuxWindows:
	...
	m_ViewController = new ShortcutManagerWindowViewController(
		m_State, directory, bindingValidator, profileManager, contextManager,
		ShortcutIntegration.instance);
	var view = EditorUIService.instance.CreateShortcutManagerWindowView(
...
// On Windows, shift key down does not send its own KeyDown event, so we
// need to listen to this event to workaround that.
void OnModifiersMightHaveChanged() {
	if (focusedWindow == this)
		SendEvent(EditorGUIUtility.CommandEvent(
				  EventCommandNames.ModifierKeysChanged));
}
