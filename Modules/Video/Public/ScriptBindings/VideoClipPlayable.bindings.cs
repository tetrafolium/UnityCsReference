...
namespace UnityEngine.Experimental.Video {
  ...
  public struct VideoClipPlayable : IPlayable,
                                    IEquatable<VideoClipPlayable> {
    PlayableHandle m_Handle;

    public static VideoClipPlayable Create(PlayableGraph graph, VideoClip clip,
                                           bool looping) {
    ...
    private static PlayableHandle CreateHandle(PlayableGraph graph,
                                               VideoClip clip, bool looping) {
      PlayableHandle handle = PlayableHandle.Null;
      if (!InternalCreateVideoClipPlayable(ref graph, clip, looping,
                                           ref handle))
      ...
      if (handle.IsValid()) {
        if (!handle.IsPlayableOfType<VideoClipPlayable>())
          throw new InvalidCastException(
              "Can't set handle: the playable is not an VideoClipPlayable.");
      }
    ...
    public VideoClip GetClip() { return GetClipInternal(ref m_Handle); }
    ...
    public bool IsPlaying() { return GetIsPlayingInternal(ref m_Handle); }
    ...
    internal void GetPauseDelay(double value) {
      ...
      if (m_Handle.GetPlayState() == PlayState.Playing &&
        ...
        throw new ArgumentException(
            "VideoClipPlayable.pauseDelay: Setting new delay when existing delay is too small or 0.0 (" +
            currentDelay +
            "), Video system will not be able to change in time");
    ...
    private void ValidateStartDelayInternal(double startDelay) {
      ...
      if (IsPlaying() &&
          (startDelay < validEndDelay ||
           (currentDelay >= validStartDelay && currentDelay < validEndDelay))) {
        Debug.LogWarning(
            "VideoClipPlayable.StartDelay: Setting new delay when existing delay is too small or 0.0 (" +
            currentDelay +
            "), Video system will not be able to change in time");
      }
    }

    [NativeThrows] extern private static VideoClip
    GetClipInternal(ref PlayableHandle hdl);

    [NativeThrows] extern private static void
    SetClipInternal(ref PlayableHandle hdl, VideoClip clip);
    ...
    [NativeThrows] extern private static bool
    GetIsPlayingInternal(ref PlayableHandle hdl);
    ...
    [NativeThrows] extern private static bool
    InternalCreateVideoClipPlayable(ref PlayableGraph graph, VideoClip clip,
