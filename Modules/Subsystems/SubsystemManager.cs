...
public static void GetSubsystemDescriptors<T>(List<T> descriptors) where T
...
static void AddSubsystemSubset<TBaseTypeInList, TQueryType>(
	List<TBaseTypeInList> copyFrom, List<TQueryType> copyTo) where TQueryType
: ISubsystem where TBaseTypeInList : ISubsystem {
...
internal static void RemoveIntegratedSubsystemByPtr(IntPtr ptr) {
	for (int finderIndex = 0; finderIndex < s_IntegratedSubsystems.Count;
	     ++finderIndex) {
...
RemoveStandaloneSubsystem(SubsystemWithProvider subsystem) {
	return s_StandaloneSubsystems.Remove(subsystem);
}

internal static SubsystemWithProvider FindStandaloneSubsystemByDescriptor(
...
static List<IntegratedSubsystem> s_IntegratedSubsystems =
	new List<IntegratedSubsystem>();
static List<SubsystemWithProvider> s_StandaloneSubsystems =
	new List<SubsystemWithProvider>();
