...
internal class VideoPlayerEditor : Editor {
  class Styles {
    public readonly GUIContent dataSourceContent =
        EditorGUIUtility.TrTextContent(
            "Source", "Type of source the movie will be read from.");
    public readonly GUIContent videoClipContent =
        EditorGUIUtility.TrTextContent(
            "Video Clip",
            "VideoClips can be imported using the asset pipeline.");
    public readonly GUIContent urlContent = EditorGUIUtility.TrTextContent(
        "URL", "URLs"); // can be http:// or file://. File URLs can be relative
                        // [file://] or absolute [file:///].  For file URLs, the
                        // prefix is optional.");
    public readonly GUIContent browseContent = EditorGUIUtility.TrTextContent(
        "Browse...",
        "Click to set a file:// URL.  http:// URLs have to be written or copy-pasted manually.");
    public readonly GUIContent playOnAwakeContent =
        EditorGUIUtility.TrTextContent(
            "Play On Awake", "Start playback as soon as the game is started.");
    public readonly GUIContent waitForFirstFrameContent =
        EditorGUIUtility.TrTextContent(
            "Wait For First Frame",
            "Wait for first frame to be ready before starting playback. When on, player time will only start increasing when the first image is ready.  When off, the first few frames may be skipped while clip preparation is ongoing.");
    public readonly GUIContent loopContent = EditorGUIUtility.TrTextContent(
        "Loop", "Start playback at the beginning when end is reached.");
    public readonly GUIContent skipOnDropContent =
        EditorGUIUtility.TrTextContent(
            "Skip On Drop",
            "Allow to skip frames to catch up with current time.");
    public readonly GUIContent playbackSpeedContent =
        EditorGUIUtility.TrTextContent(
            "Playback Speed",
            "Increase or decrease the playback speed. 1.0 is the normal speed.");
    public readonly GUIContent renderModeContent =
        EditorGUIUtility.TrTextContent(
            "Render Mode",
            "Type of object on which the played images will be drawn.");
    public readonly GUIContent cameraContent = EditorGUIUtility.TrTextContent(
        "Camera",
        "Camera where the images will be drawn, behind (Back Plane) or in front of (Front Plane) of the scene.");
    public readonly GUIContent textureContent = EditorGUIUtility.TrTextContent(
        "Target Texture",
        "RenderTexture where the images will be drawn.  RenderTextures can be created under the Assets folder and then used on other objects.");
    public readonly GUIContent alphaContent = EditorGUIUtility.TrTextContent(
        "Alpha",
        "A value less than 1.0 will reveal the content behind the video.");
    public readonly GUIContent camera3DLayout = EditorGUIUtility.TrTextContent(
        "3D Layout", "Layout of 3D content in the source video.");
    public readonly GUIContent audioOutputModeContent =
        EditorGUIUtility.TrTextContent(
            "Audio Output Mode",
            "Where the audio in the movie will be output.");
    public readonly GUIContent audioSourceContent = EditorGUIUtility.TrTextContent(
        "Audio Source",
        "AudioSource component that will receive this track's audio samples.");
    public readonly GUIContent aspectRatioLabel =
        EditorGUIUtility.TrTextContent("Aspect Ratio");
    public readonly GUIContent muteLabel =
        EditorGUIUtility.TrTextContent("Mute");
    public readonly GUIContent volumeLabel =
        EditorGUIUtility.TrTextContent("Volume");
    public readonly GUIContent controlledAudioTrackCountContent =
    ...
    public readonly string[] selectMovieFileFilter = {
        L10n.Tr("Movie files"),
        "asf,avi,dv,m4v,mp4,mov,mpg,mpeg,m4v,ogv,vp8,webm,wmv",
        L10n.Tr("All files"), "*"};
    ...
    public static readonly int ObjectFieldControlID =
        "VideoPlayerAudioSourceObjectFieldHash".GetHashCode();
  }
  ...
  int m_MaterialPropertyPopupSelection,
      m_MaterialPropertyPopupInvalidSelections;
  ...
  void OnEnable() {
    ...
    m_TargetMaterialRenderer =
        serializedObject.FindProperty("m_TargetMaterialRenderer");
    m_TargetMaterialProperty =
        serializedObject.FindProperty("m_TargetMaterialProperty");
    ...
    m_TargetCamera3DLayout =
        serializedObject.FindProperty("m_TargetCamera3DLayout");
    m_AudioOutputMode = serializedObject.FindProperty("m_AudioOutputMode");
    m_ControlledAudioTrackCount =
        serializedObject.FindProperty("m_ControlledAudioTrackCount");
    m_EnabledAudioTracks =
        serializedObject.FindProperty("m_EnabledAudioTracks");
    m_TargetAudioSources =
        serializedObject.FindProperty("m_TargetAudioSources");
    m_DirectAudioVolumes =
        serializedObject.FindProperty("m_DirectAudioVolumes");
    m_DirectAudioMutes = serializedObject.FindProperty("m_DirectAudioMutes");

    m_ShowRenderTexture.value =
        m_RenderMode.intValue == (int) VideoRenderMode.RenderTexture;
    ...
    m_ShowRenderer.value =
        m_RenderMode.intValue == (int) VideoRenderMode.MaterialOverride;
    ...
    m_ShowMaterialProperty.value =
        targets.Length > 1 || (m_MaterialPropertyPopupSelection >= 0 &&
                               m_MaterialPropertyPopupContent.Length > 0);

    m_DataSourceIsClip.value =
        m_DataSource.intValue == (int) VideoSource.VideoClip;
    m_DataSourceIsUrl.value = m_DataSource.intValue == (int) VideoSource.Url;
    m_ShowAspectRatio.value =
        (m_RenderMode.intValue != (int) VideoRenderMode.MaterialOverride) &&
        (m_RenderMode.intValue != (int) VideoRenderMode.APIOnly);
    m_ShowAudioControls.value =
        m_AudioOutputMode.intValue != (int) VideoAudioOutputMode.None;
  ...
  public override void OnInspectorGUI() {
    ...
    EditorGUILayout.PropertyField(m_WaitForFirstFrame,
                                  s_Styles.waitForFirstFrameContent);
    ...
    EditorGUILayout.Slider(m_PlaybackSpeed, 0.0f, 10.0f,
                           s_Styles.playbackSpeedContent);
    EditorGUILayout.Space();

    EditorGUILayout.PropertyField(m_RenderMode, s_Styles.renderModeContent);
    if (m_RenderMode.hasMultipleDifferentValues) {
      EditorGUILayout.HelpBox(s_Styles.selectUniformVideoRenderModeHelp,
                              MessageType.Warning, false);
    } else {
      VideoRenderMode currentRenderMode =
          (VideoRenderMode) m_RenderMode.intValue;
  ...
  private void HandleDataSourceField() {
    ...
    if (m_DataSource.hasMultipleDifferentValues)
      ...
      if (EditorGUILayout.BeginFadeGroup(m_DataSourceIsUrl.faded)) {
        ...
        Rect browseRect =
            EditorGUILayout.GetControlRect(true, EditorGUI.kSingleLineHeight);
        browseRect.xMin += EditorGUIUtility.labelWidth;
        browseRect.xMax = browseRect.xMin +
                          GUI.skin.label.CalcSize(s_Styles.browseContent).x +
                          10;
        if (EditorGUI.DropdownButton(browseRect, s_Styles.browseContent,
                                     FocusType.Passive,
  ...
  private static int
  GetMaterialPropertyPopupHash(UnityEngine.Object[] objects) {
    int hash = 0;
  ...
  GetMaterialPropertyNames(UnityEngine.Object obj, bool multiSelect,
                           out int selection, out bool invalidSelection) {
    selection = -1;
  ...
  private delegate List<string> EntryGenerator(UnityEngine.Object obj,
                                               bool multiSelect,
                                               out int selection,
                                               out bool invalidSelection);
  private static GUIContent[] BuildPopupEntries(UnityEngine.Object[] objects,
                                                EntryGenerator func,
                                                out int selection,
                                                out int invalidSelections) {
    selection = -1;
    invalidSelections = 0;
    ...
    foreach (UnityEngine.Object o in objects) {
      ...
      List<string> newEntries =
          func(o, objects.Length > 1, out newSelection, out invalidSelection);
      if (newEntries != null) {
        if (invalidSelection)
          ++invalidSelections;
        List<string> mergedEntries = entries == null ? newEntries
            : new List<string>(entries.Intersect(newEntries));
        selection = entries == null ? newSelection : selection < 0 ||
                    newSelection < 0 ||
                    entries[selection] != newEntries[newSelection] ? - 1
            : mergedEntries.IndexOf(entries[selection]);
  ...
  private static void HandlePopup(GUIContent content,
                                  SerializedProperty property,
                                  GUIContent[] entries, int selection) {
    Rect pos =
        EditorGUILayout.GetControlRect(true, EditorGUI.kSingleLineHeight);
  ...
  private void HandleTargetField(VideoRenderMode currentRenderMode) {
    m_ShowRenderTexture.target =
        currentRenderMode == VideoRenderMode.RenderTexture;
    ...
    m_ShowTargetCamera.target =
        (currentRenderMode == VideoRenderMode.CameraFarPlane) ||
    ...
    if (EditorGUILayout.BeginFadeGroup(m_ShowTargetCamera.faded)) {
      ...
      EditorGUILayout.Slider(m_TargetCameraAlpha, 0.0f, 1.0f,
                             s_Styles.alphaContent);
      // If VR is enabled in PlayerSettings on ANY platform, show the 3D layout
      // option
      foreach (BuildPlatform cur in BuildPlatforms.instance.buildPlatforms) {
        if (UnityEditorInternal.VR.VREditor.GetVREnabledOnTargetGroup(
                cur.targetGroup)) {
          EditorGUILayout.PropertyField(m_TargetCamera3DLayout,
                                        s_Styles.camera3DLayout);
          break;
        }
      }
    }
    ...
    if (EditorGUILayout.BeginFadeGroup(m_ShowRenderer.faded)) {
      ...
      if (hasMultipleSelection)
        EditorGUILayout.PropertyField(m_TargetMaterialRenderer,
                                      s_Styles.materialRendererContent);
      else {
        Rect rect =
            EditorGUILayout.GetControlRect(true, EditorGUI.kSingleLineHeight);
        ...
        var newRenderer = EditorGUI.ObjectField(
            rect, label, GetTargetRenderer((VideoPlayer) target),
      ...
      if (m_MaterialPropertyPopupContentHash != curHash)
        m_MaterialPropertyPopupContent =
            BuildPopupEntries(targets, GetMaterialPropertyNames,
                              out m_MaterialPropertyPopupSelection,
                              out m_MaterialPropertyPopupInvalidSelections);
      HandlePopup(s_Styles.materialPropertyContent, m_TargetMaterialProperty,
                  m_MaterialPropertyPopupContent,
                  m_MaterialPropertyPopupSelection);
      if (m_MaterialPropertyPopupInvalidSelections > 0 ||
          m_MaterialPropertyPopupContent.Length == 0) {
        ...
        if (m_MaterialPropertyPopupContent.Length == 0) {
          if (!hasMultipleSelection)
            EditorGUILayout.HelpBox(
                s_Styles.rendererMaterialsHaveNoTexPropsHelp,
                MessageType.Warning);
          else
            EditorGUILayout.HelpBox(
                s_Styles.someRendererMaterialsHaveNoTexPropsHelp,
                MessageType.Warning);
        } else if (!hasMultipleSelection)
          EditorGUILayout.HelpBox(s_Styles.invalidTexPropSelectionHelp,
                                  MessageType.Warning);
        else if (m_MaterialPropertyPopupInvalidSelections == 1)
          EditorGUILayout.HelpBox(s_Styles.oneInvalidTexPropSelectionHelp,
                                  MessageType.Warning);
        else
          EditorGUILayout.HelpBox(
              string.Format(s_Styles.someInvalidTexPropSelectionsHelp,
                            m_MaterialPropertyPopupInvalidSelections),
              MessageType.Warning);
        GUILayout.EndHorizontal();
      } else
        DisplayMultiMaterialInformation(m_MaterialPropertyPopupContentHash !=
                                        curHash);
  ...
  private string GenerateMultiMaterialinformation() {
    ...
    return string.Format(s_Styles.texPropInSomeMaterialsHelp,
  ...
  private void HandleAudio() {
    EditorGUILayout.Space();
    EditorGUILayout.PropertyField(m_AudioOutputMode,
                                  s_Styles.audioOutputModeContent);

    m_ShowAudioControls.target =
        (VideoAudioOutputMode)
            m_AudioOutputMode.intValue != VideoAudioOutputMode.None;
    if (EditorGUILayout.BeginFadeGroup(m_ShowAudioControls.faded)) {
      ...
      if (serializedObject.isEditingMultipleObjects)
        EditorGUILayout.HelpBox(s_Styles.audioControlsNotEditableHelp,
                                MessageType.Warning, false);
      else if (m_AudioOutputMode.hasMultipleDifferentValues)
        EditorGUILayout.HelpBox(s_Styles.selectUniformAudioOutputModeHelp,
                                MessageType.Warning, false);
      else {
        ...
        if (useControlledAudioTrackCount &&
            m_ControlledAudioTrackCount.hasMultipleDifferentValues)
          EditorGUILayout.HelpBox(s_Styles.selectUniformAudioTracksHelp,
                                  MessageType.Warning, false);
        else {
          VideoAudioOutputMode audioOutputMode =
              (VideoAudioOutputMode) m_AudioOutputMode.intValue;
          ...
          if (useControlledAudioTrackCount) {
            trackCount =
                (ushort) Math.Min((ushort) m_ControlledAudioTrackCount.intValue,
                                  trackCountBefore);
            trackCount =
                (ushort) Math.Min(trackCount, m_EnabledAudioTracks.arraySize);
          } else {
          ...
          for (ushort trackIdx = 0; trackIdx < trackCount; ++trackIdx) {
            ...
            if (audioOutputMode == VideoAudioOutputMode.AudioSource) {
              ...
              int id = GUIUtility.GetControlID(Styles.ObjectFieldControlID,
                                               FocusType.Keyboard, rect);
              var label = EditorGUI.BeginProperty(
                  rect, s_Styles.audioSourceContent, property);
              rect = EditorGUI.PrefixLabel(rect, id, label);
              EditorGUI.BeginChangeCheck();

              var result = EditorGUI.DoObjectField(
                  rect, rect, id, property.objectReferenceValue,
                  property.serializedObject.targetObject, typeof(AudioSource),
                  null, true);
              if (EditorGUI.EndChangeCheck()) {
                if (!EditorUtility.IsPersistent(result))
                  ...
                  Debug.LogWarning(
                      "Invalid AudioSource for VideoPlayer. Use an instance of an AudioSource in the scene.");
              }
              ...
              EditorGUILayout.Slider(
                  m_DirectAudioVolumes.GetArrayElementAtIndex(trackIdx), 0.0f,
                  1.0f, s_Styles.volumeLabel);
            }
            EditorGUI.indentLevel--;
          }
        }
  ...
  GUIContent GetAudioTrackEnabledContent(ushort trackIdx) {
    ...
    ushort channelCount =
        player ? player.GetAudioChannelCount(trackIdx) : (ushort) 0;

    if (language != info.language || channelCount != info.channelCount ||
        info.content == null) {
      string trackDetails = "";
      ...
      info.content =
          EditorGUIUtility.TextContent("Track " + trackIdx + trackDetails);
  ...
  private bool HandleControlledAudioTrackCount() {
    ...
    ushort audioTrackCount =
        serializedObject.isEditingMultipleObjects ?(ushort) 0
    ...
    if (audioTrackCount == 0) {
      ...
      controlledAudioTrackCountContent =
          s_Styles.controlledAudioTrackCountContent;
    } else {
      ...
      if (audioTrackCount != m_AudioTrackCountCached) {
        ...
        m_ControlledAudioTrackCountContent = EditorGUIUtility.TextContent(
            s_Styles.controlledAudioTrackCountContent.text + " [" +
            audioTrackCount + " found]");
        m_ControlledAudioTrackCountContent.tooltip =
            s_Styles.controlledAudioTrackCountContent.tooltip;
      }
      controlledAudioTrackCountContent = m_ControlledAudioTrackCountContent;
    }

    EditorGUILayout.PropertyField(m_ControlledAudioTrackCount,
                                  controlledAudioTrackCountContent);
    return true;
  }

  private void PrepareCompleted(VideoPlayer vp) { Repaint(); }
