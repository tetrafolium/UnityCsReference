...
namespace UnityEngine.SocialPlatforms.Impl {
  public class LocalUser : UserProfile, ILocalUser {
    ...
    public void SetFriends(IUserProfile[] friends) { m_Friends = friends; }

    public void SetAuthenticated(bool value) { m_Authenticated = value; }

    public void SetUnderage(bool value) { m_Underage = value; }
  ...
  public class UserProfile : IUserProfile {
    ...
    public UserProfile(string name, string teamId, string gameId, bool friend,
                       UserState state, Texture2D image) {
      m_UserName = name;
      m_teamID = teamId;
      m_gameID = gameId;
      m_IsFriend = friend;
    ...
    public void SetUserID(string id) { m_teamID = id; }

    public void SetUserGameID(string id) { m_gameID = id; }

    public void SetImage(Texture2D image) { m_Image = image; }

    public void SetIsFriend(bool value) { m_IsFriend = value; }

    public void SetState(UserState state) { m_State = state; }
  ...
  public class Achievement : IAchievement {
    ...
    public Achievement(string id, double percentCompleted, bool completed,
                       bool hidden, DateTime lastReportedDate) {
      this.id = id;
    ...
    public override string ToString() {
      return id + " - " + percentCompleted + " - " + completed + " - " +
             hidden + " - " + lastReportedDate;
    }
    ...
    // Setters for implementors
    public void SetCompleted(bool value) { m_Completed = value; }

    public void SetHidden(bool value) { m_Hidden = value; }
  ...
  public class AchievementDescription : IAchievementDescription {
    ...
    public AchievementDescription(string id, string title, Texture2D image,
                                  string achievedDescription,
                                  string unachievedDescription, bool hidden,
                                  int points) {
      this.id = id;
      m_Title = title;
      m_Image = image;
    ...
    public override string ToString() {
      return id + " - " + title + " - " + achievedDescription + " - " +
    ...
    public string achievedDescription {
      get { return m_AchievedDescription; }
    }
    public string unachievedDescription {
      get { return m_UnachievedDescription; }
    }
    public bool hidden {
      get { return m_Hidden; }
    }
  ...
  public class Score : IScore {
    ...
    public Score(string leaderboardID, Int64 value, string userID,
                 DateTime date, string formattedValue, int rank) {
    ...
    public override string ToString() {
      return "Rank: '" + m_Rank + "' Value: '" + value + "' Category: '" +
             leaderboardID + "' PlayerID: '" + m_UserID + "' Date: '" + m_Date;
    }
    ...
    public void SetDate(DateTime date) { m_Date = date; }

    public void SetFormattedValue(string value) { m_FormattedValue = value; }
    ...
    // TODO: This is just an int64 here, but should be able to represent all
    // supported formats, except for float type scores ...
    public Int64 value {
      get;
      set;
    }
  ...
  public class Leaderboard : ILeaderboard {
    ...
    public void SetUserFilter(string[] userIDs) { m_UserIDs = userIDs; }

    public override string ToString() {
      return "ID: '" + id + "' Title: '" + m_Title + "' Loading: '" +
             m_Loading + "' Range: [" + range.from + "," + range.count +
             "] MaxRange: '" + m_MaxRange + "' Scores: '" + m_Scores.Length +
             "' UserScope: '" + userScope + "' TimeScope: '" + timeScope +
             "' UserFilter: '" + m_UserIDs.Length;
    }
    ...
    // Setters for implementors
    public void SetLocalUserScore(IScore score) { m_LocalUserScore = score; }

    public void SetMaxRange(uint maxRange) { m_MaxRange = maxRange; }

    public void SetScores(IScore[] scores) { m_Scores = scores; }

    public void SetTitle(string title) { m_Title = title; }

    public string[] GetUserFilter() { return m_UserIDs; }
...
namespace UnityEngine.SocialPlatforms {
  ...
  public class Local : ISocialPlatform {
    ...
    void ISocialPlatform.Authenticate(ILocalUser user,
    ...
    void ISocialPlatform.Authenticate(ILocalUser user,
      ...
      ((ISocialPlatform) this)
          .Authenticate(user, success => callback(success, null));
    }

    void ISocialPlatform.LoadFriends(ILocalUser user,
                                     System.Action<bool> callback) {
      if (!VerifyUser())
        return;
    ...
    public void ReportProgress(string id, double progress,
      ...
      foreach (Achievement achoo in m_Achievements) {
        // TODO: Only allow increase in progress, figure out if xbox/gc report
        // errors when lower progress is reported
      ...
      // If it's not been reported already check if it's a valid achievement
      // description ID and add it, else it's an error
      foreach (AchievementDescription achoo in m_AchievementDescriptions) {
        ...
        if (achoo.id == id) {
          ...
          Achievement newAchievement =
              new Achievement(id, progress, completed, false, DateTime.Now);
    ...
    public void LoadAchievementDescriptions(
        System.Action<IAchievementDescription[]> callback) {
      if (!VerifyUser())
        return;
      if (callback != null)
    ...
    public void ReportScore(Int64 score, string board,
                            System.Action<bool> callback) {
      if (!VerifyUser())
        return;
      foreach (Leaderboard current in m_Leaderboards) {
        if (current.id == board) {
          // TODO: IIRC GameCenter only adds scores if they are higher than the
          // users previous score, maybe mirror this here.
          List<Score> scores = new List<Score>((Score[]) current.scores);
          scores.Add(new Score(board, score, localUser.id, DateTime.Now,
                               score + " points", 0));
    ...
    void ISocialPlatform.LoadScores(ILeaderboard board,
                                    System.Action<bool> callback) {
      if (!VerifyUser())
        return;
      ...
      foreach (Leaderboard lb in m_Leaderboards) {
        // TODO: In the real world the board might have thousands of scores but
        // only 100 are returned, for now we can assume they are less than 100
        // here
        // TODO: Apply the filters which the leaderboard has, right now it
        // always returns everything found
    ...
    private void PopulateStaticData() {
      m_Friends.Add(new UserProfile("Fred", "1001", true, UserState.Online,
                                    m_DefaultTexture));
      m_Friends.Add(new UserProfile("Julia", "1002", true, UserState.Online,
                                    m_DefaultTexture));
      m_Friends.Add(new UserProfile("Jeff", "1003", true, UserState.Online,
                                    m_DefaultTexture));
      m_Users.Add(new UserProfile("Sam", "1004", false, UserState.Offline,
                                  m_DefaultTexture));
      m_Users.Add(new UserProfile("Max", "1005", false, UserState.Offline,
                                  m_DefaultTexture));
      m_AchievementDescriptions.Add(new AchievementDescription(
          "Achievement01", "First achievement", m_DefaultTexture,
          "Get first achievement", "Received first achievement", false, 10));
      m_AchievementDescriptions.Add(new AchievementDescription(
          "Achievement02", "Second achievement", m_DefaultTexture,
          "Get second achievement", "Received second achievement", false, 20));
      m_AchievementDescriptions.Add(new AchievementDescription(
          "Achievement03", "Third achievement", m_DefaultTexture,
          "Get third achievement", "Received third achievement", false, 15));
      Leaderboard board = new Leaderboard();
      board.SetTitle("High Scores");
      board.id = "Leaderboard01";
      List<Score> scores = new List<Score>();
      scores.Add(new Score("Leaderboard01", 300, "1001",
                           DateTime.Now.AddDays(-1), "300 points", 1));
      scores.Add(new Score("Leaderboard01", 255, "1002",
                           DateTime.Now.AddDays(-1), "255 points", 2));
      scores.Add(new Score("Leaderboard01", 55, "1003",
                           DateTime.Now.AddDays(-1), "55 points", 3));
      scores.Add(new Score("Leaderboard01", 10, "1004",
                           DateTime.Now.AddDays(-1), "10 points", 4));
