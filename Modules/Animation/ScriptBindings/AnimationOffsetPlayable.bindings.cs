...
namespace UnityEngine.Animations {
  ...
  internal struct AnimationOffsetPlayable
      ...
      if (handle.IsValid()) {
        if (!handle.IsPlayableOfType<AnimationOffsetPlayable>())
          throw new InvalidCastException(
              "Can't set handle: the playable is not an AnimationOffsetPlayable.");
      }
    ...
    public Vector3 GetPosition() { return GetPositionInternal(ref m_Handle); }
    ...
    CreateHandleInternal(PlayableGraph graph, Vector3 position,
                         Quaternion rotation, ref PlayableHandle handle);

    [NativeThrows] extern static private Vector3
    GetPositionInternal(ref PlayableHandle handle);

    [NativeThrows] extern static private void
    SetPositionInternal(ref PlayableHandle handle, Vector3 value);

    [NativeThrows] extern static private Quaternion
    GetRotationInternal(ref PlayableHandle handle);

    [NativeThrows] extern static private void
    SetRotationInternal(ref PlayableHandle handle, Quaternion value);
  }
}
