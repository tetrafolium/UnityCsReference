...
public enum PlayMode {
	// Will stop all animations that were started in the same layer. This is the
	// default when playing animations.
	StopSameLayer = 0,
	// Will stop all animations that were started with this component before
	// playing
	StopAll = 4,
}
...
public enum QueueMode {
	...
	CompleteOthers = 0,
	// Starts playing immediately. This can be used if you just want to quickly
...
public enum AnimationCullingType {
	...
	[
		System.Obsolete(
			"Enum member AnimatorCullingMode.BasedOnClipBounds has been deprecated. Use AnimationCullingType.AlwaysAnimate or AnimationCullingType.BasedOnRenderers instead")
	] BasedOnClipBounds = 2,
	// Animation is disabled when localBounds are not visible.
	[
		System.Obsolete(
			"Enum member AnimatorCullingMode.BasedOnUserBounds has been deprecated. Use AnimationCullingType.AlwaysAnimate or AnimationCullingType.BasedOnRenderers instead")
	] BasedOnUserBounds = 3
}
...
public bool
Play([ uei.DefaultValue("PlayMode.StopSameLayer") ] PlayMode mode) {
...
Play(string animation,
     [ uei.DefaultValue("PlayMode.StopSameLayer") ] PlayMode mode);
...
extern public void
CrossFade(string animation, [ uei.DefaultValue("0.3F") ] float fadeLength,
          [ uei.DefaultValue("PlayMode.StopSameLayer") ] PlayMode mode);

[uei.ExcludeFromDocs]
public void Blend(string animation) {
	Blend(animation, 1.0f);
}
[uei.ExcludeFromDocs]
public void Blend(string animation, float targetWeight) {
	Blend(animation, targetWeight, 0.3f);
}
extern public void Blend(string animation,
                         [ uei.DefaultValue("1.0F") ] float targetWeight,
                         [ uei.DefaultValue("0.3F") ] float fadeLength);
...
public AnimationState CrossFadeQueued(string animation, float fadeLength,
                                      QueueMode queue) {
	return CrossFadeQueued(animation, fadeLength, queue,
	                       PlayMode.StopSameLayer);
}
[FreeFunction("AnimationBindings::CrossFadeQueuedImpl",
              HasExplicitThis = true)] extern public AnimationState
CrossFadeQueued(
	string animation, [ uei.DefaultValue("0.3F") ] float fadeLength,
	[ uei.DefaultValue("QueueMode.CompleteOthers") ] QueueMode queue,
	[ uei.DefaultValue("PlayMode.StopSameLayer") ] PlayMode mode);
...
[FreeFunction("AnimationBindings::PlayQueuedImpl",
              HasExplicitThis = true)] extern public AnimationState
...
public void AddClip(AnimationClip clip, string newName) {
	AddClip(clip, newName, Int32.MinValue, Int32.MaxValue);
}
[uei.ExcludeFromDocs]
public void AddClip(AnimationClip clip, string newName, int firstFrame,
                    int lastFrame) {
	AddClip(clip, newName, firstFrame, lastFrame, false);
}
extern public void AddClip(AnimationClip clip, string newName, int firstFrame,
                           int lastFrame,
                           [ uei.DefaultValue("false") ] bool addLoopFrame);
...
[NativeName("RemoveClip")] extern private void
RemoveClipNamed(string clipName);
...
[FreeFunction("AnimationBindings::GetState",
              HasExplicitThis = true)] extern internal
AnimationState GetState(string name);

[FreeFunction("AnimationBindings::GetStateAtIndex", HasExplicitThis = true,
...
public extern bool animateOnlyIfVisible {
	[FreeFunction("AnimationBindings::GetAnimateOnlyIfVisible",
	              HasExplicitThis = true)] get;
	[FreeFunction("AnimationBindings::SetAnimateOnlyIfVisible",
	              HasExplicitThis = true)] set;
}

extern public AnimationCullingType cullingType {
	get;
	set;
}
extern public Bounds localBounds {
	[NativeName("GetLocalAABB")] get;
...
extern public float weight {
	get;
	set;
}
extern public WrapMode wrapMode {
	get;
	set;
}
...
extern public float normalizedTime {
	get;
	set;
}
...
extern public float normalizedSpeed {
	get;
	set;
}
...
extern public int layer {
	get;
	set;
}
extern public AnimationClip clip {
	get;
}
...
extern public AnimationBlendMode blendMode {
	get;
	set;
}
...
public void AddMixingTransform(Transform mix) {
	AddMixingTransform(mix, true);
}
extern public void
AddMixingTransform(Transform mix,
                   [ uei.DefaultValue("true") ] bool recursive);
...
public Object objectReferenceParameter {
	get { return m_ObjectReferenceParameter; }
...
public SendMessageOptions messageOptions {
	get { return (SendMessageOptions) m_MessageOptions; }
	set { m_MessageOptions = (int) value; }
}

public bool isFiredByLegacy {
	get { return m_Source == AnimationEventSource.Legacy; }
}
public bool isFiredByAnimator {
	get { return m_Source == AnimationEventSource.Animator; }
}
...
public AnimationState animationState {
	get {
		if (!isFiredByLegacy)
			Debug.LogError(
				"AnimationEvent was not fired by Animation component, you shouldn't use AnimationEvent.animationState");
...
public AnimatorStateInfo animatorStateInfo {
	get {
		if (!isFiredByAnimator)
			Debug.LogError(
				"AnimationEvent was not fired by Animator component, you shouldn't use AnimationEvent.animatorStateInfo");
...
public AnimatorClipInfo animatorClipInfo {
	get {
		if (!isFiredByAnimator)
			Debug.LogError(
				"AnimationEvent was not fired by Animator component, you shouldn't use AnimationEvent.animatorClipInfo");
