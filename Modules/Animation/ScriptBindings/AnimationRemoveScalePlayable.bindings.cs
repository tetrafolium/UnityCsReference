...
namespace UnityEngine.Animations {
  ...
  internal struct AnimationRemoveScalePlayable
      ...
      if (handle.IsValid()) {
        if (!handle.IsPlayableOfType<AnimationRemoveScalePlayable>())
          throw new InvalidCastException(
              "Can't set handle: the playable is not an AnimationRemoveScalePlayable.");
      }
    ...
    operator Playable(AnimationRemoveScalePlayable playable) {
      return new Playable(playable.GetHandle());
    }

    public static explicit
    operator AnimationRemoveScalePlayable(Playable playable) {
