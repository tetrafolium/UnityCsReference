...
public virtual void OnDestroy(IWindowModel model) {
	...
	// Here we assume m_Model == model. We should probably make the ignored
	// OnDestroy argument obsolete.
	m_Model = null;
	m_Panel.Dispose();
}

public bool GetTooltip(Vector2 windowMouseCoordinates, out string tooltip,
	...
	if (target != null) {
		using (var tooltipEvent = TooltipEvent.GetPooled()) {
			...
			if (!string.IsNullOrEmpty(tooltipEvent.tooltip) &&
			    !tooltipEvent.isDefaultPrevented) {
