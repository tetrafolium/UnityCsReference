...
namespace UnityEditor.UIElements {
  class DefaultEditorWindowBackend : DefaultWindowBackend,
    ...
    protected IEditorWindowModel editorWindowModel =>
        m_Model as IEditorWindowModel;

    public override void OnCreate(IWindowModel model) {
      base.OnCreate(model);

      m_PlayModeDarkenColor = UIElementsUtility.editorPlayModeTintColor =
          EditorApplication.isPlayingOrWillChangePlaymode ? editorWindowModel
              .playModeTintColor : Color.white;

      EditorApplication.playModeStateChanged += PlayModeStateChangedCallback;
      AnimationMode.onAnimationRecordingStart +=
          RefreshStylesAfterExternalEvent;
      ...
      // Window is non-null when set by deserialization; it's usually null when
      // OnCreate is called.
    ...
    void IEditorWindowBackend.ViewMarginsChanged() { UpdateStyleMargins(); }
    ...
    void IEditorWindowBackend.OnRegisterWindow() { RegisterWindow(); }

    void IEditorWindowBackend.OnUnregisterWindow() { UnregisterWindow(); }
    ...
    private void RegisterImguiContainerGUICallbacks() {
      ...
      root.RegisterCallback<MouseDownEvent>(SendEventToSplitterGUI,
                                            k_TricklePhase);
      root.RegisterCallback<MouseUpEvent>(SendEventToSplitterGUI,
                                          k_TricklePhase);
      root.RegisterCallback<MouseMoveEvent>(SendEventToSplitterGUI,
                                            k_TricklePhase);
    ...
    private void UnregisterImguiContainerGUICallbacks() {
      ...
      if (root == m_RegisteredRoot) {
        ...
        root.UnregisterCallback<MouseDownEvent>(SendEventToSplitterGUI,
                                                k_TricklePhase);
        root.UnregisterCallback<MouseUpEvent>(SendEventToSplitterGUI,
                                              k_TricklePhase);
        root.UnregisterCallback<MouseMoveEvent>(SendEventToSplitterGUI,
                                                k_TricklePhase);
      }
    }
    ...
    private void SendEventToSplitterGUI(EventBase ev) {
      ...
      imguiContainer.HandleIMGUIEvent(
          ev.imguiEvent, editorWindowModel.onSplitterGUIHandler, false);
    ...
    void UpdatePlayModeColor(Color newColorToUse) {
      ...
      if (m_PlayModeDarkenColor != newColorToUse) {
        ...
        // Make sure to dirty the right imgui container in this HostView (and
        // all its children / parents) The MarkDirtyRepaint() function is
        // dirtying the element itself and its parent, but not the children
        // explicitly.
        // ... and in the repaint function, it check for the current rendered
        // element, not the parent. Since the HostView "hosts" an IMGUIContainer
        // or any VisualElement, we have to make sure to dirty everything here.
    ...
    void IEditorWindowBackend.Focused() { m_Panel.Focus(); }

    void IEditorWindowBackend.Blurred() { m_Panel.Blur(); }
    ...
    private void AddUIElementsDebuggerToMenu(GenericMenu menu) {
      ...
      var shortcut = ShortcutIntegration.instance.directory.FindShortcutEntry(
          UIElementsDebugger.k_WindowPath);
      if (shortcut != null && shortcut.combinations.Any())
        itemContent +=
            $" {KeyCombination.SequenceToMenuString(shortcut.combinations)}";

      menu.AddItem(EditorGUIUtility.TextContent(itemContent), false,
                   DebugWindow, editorWindowModel.window);
    }
    ...
    private void DebugWindow(object userData) {
      ...
      if (CommandService.Exists(UIElementsDebugger.OpenWindowCommand))
        CommandService.Execute(UIElementsDebugger.OpenWindowCommand,
                               CommandHint.Menu, window);
      else {
    ...
    private void RefreshStylesAfterExternalEvent() {
      ...
      var updater = panel.GetUpdater(VisualTreeUpdatePhase.Bindings)
                        as VisualTreeBindingsUpdater;
      if (updater == null)
        return;

      updater.PollElementsWithBindings(
          (e, b) => BindingExtensions.HandleStyleUpdate(e));
    }
  }
}
