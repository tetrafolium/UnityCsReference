...
namespace UnityEditor.UIElements {
  internal class EditorElement : VisualElement, IEditorElement {
    ...
    Editor[] m_Editors {
      get {
        if (m_EditorCache == null || m_EditorIndex >= m_EditorCache.Length ||
            !m_EditorCache[m_EditorIndex]) {
          PopulateCache();
        }
        return m_EditorCache;
      }
    ...
    internal InspectorElement m_InspectorElement {
      get;
      private set;
    }
    ...
    static class Styles {
      public static GUIStyle importedObjectsHeaderStyle =
          new GUIStyle("IN BigTitle");
    ...
    internal EditorElement(int editorIndex, IPropertyView iw) {
      ...
      Add(m_Header);
      // If the editor targets contain many target and the multi editing is not
      // supported, we should not add this inspector. However, the header and
      // footer are kept since these are showing information regarding this
      // state.
      if ((editor.targets.Length <= 1) ||
          (iw.IsMultiEditingSupported(editor, editor.target))) {
    ...
    void Init() {
      ...
      var inspectorElementMode = InspectorElement.GetModeFromInspectorMode(
          inspectorWindow.inspectorMode);
      if (inspectorWindow.useUIElementsDefaultInspector)
        inspectorElementMode &= ~(InspectorElement.Mode.IMGUIDefault);

      m_InspectorElement =
          new InspectorElement(editor, inspectorElementMode){focusable = false};
      ...
      m_InspectorElement.style.paddingBottom =
          InspectorWindow.kEditorElementPaddingBottom;
...
#region Header
    ...
    IMGUIContainer BuildHeaderElement(string editorTitle) {
      ...
      // This also needs to validate the state of the editor tracker (stuff that
      // was already done in the original DrawEditors
      var headerElement = inspectorWindow.CreateIMGUIContainer(
          HeaderOnGUI, editorTitle + "Header");
      return headerElement;
    }

    private static UQueryState<IMGUIContainer> ImguiContainersQuery =
        new UQueryBuilder<IMGUIContainer>(null).SingleBaseType().Build();
    ...
    void HeaderOnGUI() {
      ...
      // Avoid drawing editor if native target object is not alive, unless it's
      // a MonoBehaviour/ScriptableObject We want to draw the generic editor
      // with a warning about missing/invalid script Case 891450:
      // - ActiveEditorTracker will automatically create editors for materials
      // of components on tracked game objects
      // - UnityEngine.UI.Mask will destroy this material in OnDisable (e.g.
      // disabling it with the checkbox) causing problems when drawing the
      // material editor
      if (target == null &&
          !NativeClassExtensionUtilities.ExtendsANativeType(target)) {
        SetElementVisible(m_InspectorElement, false);
        return;
      }

      m_WasVisible =
          inspectorWindow.WasEditorVisible(editors, m_EditorIndex, target);
      ...
      if (GUI.changed) {
        // If the header changed something, we must trigger a layout calculating
        // on imgui children Fixes Material editor toggling layout issues (case
        // 1148706)
      ...
      var multiEditingSupported =
          inspectorWindow.IsMultiEditingSupported(editor, target);
    ...
    Rect DrawEditorHeader(Editor[] editors, Object target,
                          ref bool wasVisible) {
    ...
    bool DrawEditorLargeHeader(Editor[] editors, ref bool wasVisible) {
      ...
      bool largeHeader =
          InspectorWindow.EditorHasLargeHeader(m_EditorIndex, editors);

      // Draw large headers before we do the culling of unsupported editors
      // below, so the large header is always shown even when the editor can't
      // be.
      if (largeHeader) {
        bool IsOpenForEdit = editor.IsOpenForEdit();
        wasVisible = true;

        if (inspectorWindow.editorsWithImportedObjectLabel.Contains(
                m_EditorIndex)) {
          ...
          if (editors.Length > 1) {
            if (editors[0] is PrefabImporterEditor &&
                editors[1] is GameObjectInspector)
              headerText = "Root in Prefab Asset";
          }

          GUILayout.Label(headerText, Styles.importedObjectsHeaderStyle,
                          GUILayout.ExpandWidth(true));
          GUILayout.Space(-7f); // Ensures no spacing between this header and
                                // the next header
        }

        // Header
        using(new EditorGUI.DisabledScope(
            !IsOpenForEdit)) // Only disable the entire header if the asset is
                             // locked by VCS
        {
          editor.DrawHeader();
        }
    ...
    // Draw small headers (the header above each component) after the culling
    // above so we don't draw a component header for all the components that
    // can't be shown.
    Rect DrawEditorSmallHeader(Editor[] editors, Object target,
                               bool wasVisible) {
      ...
      using(new EditorGUI.DisabledScope(!currentEditor.IsEnabled())) {
        bool isVisible =
            EditorGUILayout.InspectorTitlebar(wasVisible, currentEditor);
    ...
    static void
    SetInspectorElementChildIMGUIContainerFocusable(InspectorElement ve,
                                                    bool focusable) {
...
#region Footer
    ...
    IMGUIContainer BuildFooterElement(string editorTitle) {
      IMGUIContainer footerElement = inspectorWindow.CreateIMGUIContainer(
          FooterOnGUI, editorTitle + "Footer");
    ...
    void FooterOnGUI() {
      ...
      inspectorWindow.editorDragging.HandleDraggingToEditor(
          editors, m_EditorIndex, m_DragRect, m_ContentRect);
      HandleComponentScreenshot(m_ContentRect, ed);

      var comp = target as Component;

      if (EditorGUI.ShouldDrawOverrideBackground(ed.targets, Event.current,
                                                 comp)) {
        var rect = GUILayoutUtility.kDummyRect;
        bool wasVisible =
            inspectorWindow.WasEditorVisible(editors, m_EditorIndex, target);
        // if the inspector is currently visible then the override background
        // drawn by the footer needs to be slightly larger than if the inspector
        // is collapsed
        if (wasVisible) {
          rect.y -= 1;
          rect.height += 1;
      ...
      if (ScreenShots.s_TakeComponentScreenshot) {
        ...
        if (content.Contains(Event.current.mousePosition)) {
          ...
          globalComponentRect.position =
              globalComponentRect.position +
              inspectorWindow.parent.screenPosition.position;
    ...
    internal bool EditorNeedsVerticalOffset(Editor[] editors, Object target) {
      return m_EditorIndex > 0 && IsEditorValid() &&
             editors[m_EditorIndex - 1].target is GameObject &&
             target is Component;
    }
  }
}
