...
namespace UnityEditor.UIElements {
  public class InspectorElement : BindableElement {
    ...
    public static readonly string iMGUIContainerUssClassName =
        ussClassName + "__imgui-container";
    ...
    public new class UxmlTraits : BindableElement.UxmlTraits {
      public UxmlTraits() { m_PickingMode.defaultValue = PickingMode.Ignore; }
    }
    ...
    internal string trackerName =>
        m_TrackerName ??(m_TrackerName = GetInspectorTrackerName(this));

    internal bool ownsEditor {
      get;
      private set;
    }
    = false;

    internal SerializedObject boundObject {
      get;
      private set;
    }

    internal VisualElement prefabOverrideBlueBarsContainer {
      get;
      private set;
    }
    ...
    void OnDetachFromPanel(DetachFromPanelEvent evt) { DestroyOwnedEditor(); }
    ...
    private void Reset(SerializedObject bindObject) {
      ...
      prefabOverrideBlueBarsContainer.name =
          BindingExtensions.prefabOverrideBarContainerName;
    ...
    private Editor GetOrCreateEditor(SerializedObject serializedObject) {
      ...
      foreach (var inspectorWindow in InspectorWindow.GetInspectors()) {
        foreach (var trackerEditor in inspectorWindow.tracker.activeEditors) {
          if (trackerEditor.target == target ||
              trackerEditor.serializedObject == serializedObject) {
    ...
    CreateDefaultInspector(SerializedObject serializedObject) {
    ...
    bool AddMissingScriptLabel(SerializedObject serializedObject) {
      ...
      if (scriptProperty != null) {
        hierarchy.Add(new IMGUIContainer(
            () => GenericInspector.ShowScriptNotLoadedWarning(scriptProperty)));
        return true;
      }
    ...
    internal static bool SetWideModeForWidth(VisualElement displayElement) {
      ...
      // or when the inspector display is changed from none to flex, the width
      // will be zero during the measuring phase. we try to find a parent with a
      // a width. If none are found, we'll set wideMode to true to avoid
      // computing too tall an inspector on the first layout calculation
      while (displayElement != null &&
             (float.IsNaN(inspectorWidth) || inspectorWidth == 0)) {
    ...
    private VisualElement
    CreateIMGUIInspectorFromEditor(SerializedObject serializedObject,
                                   Editor editor, bool reuseIMGUIContainer) {
      if ((mode & (Mode.IMGUICustom | Mode.IMGUIDefault)) == 0)
        return null;

      if ((mode & Mode.IMGUICustom) > 0 && (mode & Mode.IMGUIDefault) == 0 &&
          editor is GenericInspector)
        return null;

      if ((mode & Mode.IMGUICustom) == 0 && (mode & Mode.IMGUIDefault) > 0 &&
          !(editor is GenericInspector) && !(editor is AssetImporterEditor) &&
          !(editor is GameObjectInspector)) {
      ...
      // Reusing the existing IMGUIContainer allows us to re-use the existing
      // gui state, when we are drawing the same inspector this will let us keep
      // the same control ids
      ...
      inspector.onGUIHandler = () => {
        if (editor && editor.target &&
            (editor.target.hideFlags & HideFlags.HideInInspector) ==
                HideFlags.HideInInspector)
          return;

        // It's possible to run 2-3 frames after the tracker of this inspector
        // window has been recreated, and with it the Editor and its
        // SerializedObject. One example of when this happens is when the
        // Preview window is detached from a *second* instance of an
        // InspectorWindow and re-attached.
        //
        // This is only a problem for the *second* (or third, forth, etc)
        // instance of the InspectorWindow because only the first instance can
        // use the ActiveEditorTracker.sharedTracker in
        // InspectorWindow.CreateTracker(). The other instances have to create a
        // new tracker...each time.
        //
        // Not an ideal solution, but basically we temporarily hold the printing
        // to console for errors for which GUIUtility.ShouldRethrowException(e)
        // returns false. The errors that may occur during this brief "bad
        // state" are SerializedProperty errors. If the custom Editor created
        // and remembered references to some SerializedProperties during its
        // OnEnable(), those references will be invalid when the tracker is
        // refreshed, until this GUIHandler is reassigned. This fix just ignores
        // those errors.
        //
        // We don't simply early return here because that can break some tests
        // that rely heavily on yields and timing of UI redraws. Yes..
        //
        // case 1119612
        ...
        if ((editor.target == null &&
             !GenericInspector.ObjectIsMonoBehaviourOrScriptableObject(
                 editor.target)) ||
        ...
        using(new EditorGUI.DisabledScope(!editor.IsEnabled())) {
          ...
          ScriptAttributeUtility.propertyHandlerCache =
              editor.propertyHandlerCache;
          ...
          GUIStyle editorWrapper =
              (editor.UseDefaultMargins() &&
                       editor.CanBeExpandedViaAFoldoutWithoutUpdate()
          ...
          try {
            ...
            using(new InspectorWindowUtils.LayoutGroupChecker()) {
              ...
              {
                // we have no guarantees regarding what happens in the try/catch
                // block below, so we need to save state here and restore it
                // afterwards, the natural thing to do would be using
                // SavedGUIState, but it implicitly resets keyboards bindings
                // and it breaks functionality. We have identified issues with
                // layout so we just save that for the time being.
                var layoutCache =
                    new GUILayoutUtility.LayoutCache(GUILayoutUtility.current);
                try {
                  var rebuildOptimizedGUIBlocks =
                      GetRebuildOptimizedGUIBlocks(editor.target);
                  rebuildOptimizedGUIBlocks |= editor.isInspectorDirty;
                  float height;
                  if (editor.GetOptimizedGUIBlock(rebuildOptimizedGUIBlocks,
                                                  visible, out height)) {
                    var contentRect =
                        GUILayoutUtility.GetRect(0, visible ? height : 0);

                    // Layout events are ignored in the optimized code path
                    // The exception is when we are drawing a GenericInspector,
                    // they always use the optimized path and must therefore run
                    // at least one layout calculation in it
                    if (Event.current.type == EventType.Layout &&
                        !(editor is GenericInspector)) {
                      return;
                    }

                    InspectorWindowUtils.DrawAddedComponentBackground(
                        contentRect, editor.targets);
            ...
            if (GUI.changed) {
              // This forces a relayout of all imguicontainers in this inspector
              // window. fixes part of case 1148706
    ...
    private VisualElement
    CreateInspectorElementFromEditor(Editor editor,
                                     bool reuseIMGUIContainer = false) {
      ...
      if (inspectorElement == null)
        inspectorElement = CreateIMGUIInspectorFromEditor(
            serializedObject, editor, reuseIMGUIContainer);
      ...
      if (inspectorElement == null) {
        ...
        AddToClassList(uIEInspectorVariantUssClassName);
        inspectorElement =
            new Label("No inspector found given the current Inspector.Mode.");
      }
    ...
    private bool GetRebuildOptimizedGUIBlocks(Object inspectedObject) {
      ...
      if (Event.current.type == EventType.Repaint) {
        if (inspectedObject != null &&
            m_IsOpenForEdit !=
                Editor.IsAppropriateFileOpenForEdit(inspectedObject)) {
        ...
        if (m_InvalidateGUIBlockCache) {
          ...
          m_InvalidateGUIBlockCache = false;
                 ...
                 Event.current.commandName ==
                     EventCommandNames.EyeDropperUpdate) {
        rebuildOptimizedGUIBlocks = true;
      }
