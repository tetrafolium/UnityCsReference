...
private static string GetCurrentFolder() {
	...
	if (Selection.assetGUIDs.Length == 0) {
		...
		// Get the file extension of the selected asset as it might need to be
		// removed.
...
public static void CreateUXMLTemplate() {
	if (CommandService.Exists(nameof(CreateUXMLTemplate)))
		...
		var path = AssetDatabase.GenerateUniqueAssetPath(
			folder + "/NewUXMLTemplate.uxml");
		var contents = CreateUXMLTemplate(folder);
		var icon =
			EditorGUIUtility.IconContent<VisualTreeAsset>().image as Texture2D;
...
public static string CreateUXMLTemplate(string folder,
	...
	Dictionary<string, string> namespacePrefix =
		UxmlSchemaGenerator.GetNamespacePrefixDictionary();
