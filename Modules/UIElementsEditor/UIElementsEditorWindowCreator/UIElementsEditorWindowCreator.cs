...
const string k_UIElementsEditorWindowCreatorStyleSheetPath =
	"UIPackageResources/StyleSheets/UIElementsEditorWindowCreator.uss";
const string k_UIElementsEditorWindowCreatorUxmlPath =
	"UIPackageResources/UXML/UIElementsEditorWindowCreator.uxml";
...
public static void CreateTemplateEditorWindow() {
	if (CommandService.Exists(nameof(CreateTemplateEditorWindow)))
		CommandService.Execute(nameof(CreateTemplateEditorWindow),
		                       CommandHint.Menu);
	else {
		UIElementsEditorWindowCreator editorWindow =
			GetWindow<UIElementsEditorWindowCreator>(
				true, "UI Toolkit Editor Window Creator");
		editorWindow.maxSize =
			new Vector2(Styles.K_WindowWidth, Styles.K_WindowHeight);
...
public void OnEnable() {
	// After the c# file has been created and the domain.reload executed, we
...
void SetupLayout() {
	...
	var visualTree = EditorGUIUtility.Load(
		k_UIElementsEditorWindowCreatorUxmlPath) as VisualTreeAsset;
	...
	cSharpTextField.RegisterCallback<ChangeEvent<string> >(
		OnCSharpValueChanged);
	...
	m_Root.Q<Button>("confirmButton").clickable.clicked +=
		CreateNewTemplatesFiles;
	m_ErrorMessageBox.Q<Image>("warningIcon").image =
...
void ShowErrorMessage() {
	if (m_ErrorMessageBox.resolvedStyle.visibility == Visibility.Hidden) {
		...
		maxSize = new Vector2(Styles.K_WindowWidth,
		                      Styles.K_WindowHeight +
		                      m_ErrorMessageBox.contentRect.height);
		minSize = new Vector2(Styles.K_WindowWidth,
		                      Styles.K_WindowHeight +
		                      m_ErrorMessageBox.contentRect.height);
	}
}
...
void CreateNewTemplatesFiles() {
	if (IsInputValid()) {
		...
		if (m_IsUxmlEnable) {
			...
			File.WriteAllText(
				uxmlPath,
				UIElementsTemplate.CreateUXMLTemplate(
					m_Folder,
		...
		if (m_IsCSharpEnable) {
			...
			File.WriteAllText(cSharpPath,
			                  UIElementsTemplate.CreateCSharpTemplate(
						  m_CSharpName, m_UxmlName, m_UssName, m_Folder));
...
bool IsValidPath() {
	if (m_Folder.Split('/').Contains("Editor") == false) {
		m_ErrorMessage =
			"The target path must be located inside an Editor folder";
		return false;
	}
...
bool IsAtLeastOneFileCreated() {
	bool isAtLeastOneFileCreated =
		m_IsCSharpEnable || m_IsUssEnable || m_IsUxmlEnable;
	if (!isAtLeastOneFileCreated) {
...
bool Validate(string fileName, string extension) {
	...
	if (fileName.Length <= 0) {
		...
		isValid = false;
	} else if (!System.CodeDom.Compiler.CodeGenerator
	           .IsValidLanguageIndependentIdentifier(fileName)) {
...
bool ClassExists() {
	bool classExists = AppDomain.CurrentDomain.GetAssemblies().Any(
		a => a.GetType(m_CSharpName, false) != null);
	if (classExists) {
...
void OpenNewlyCreatedFiles() {
	if (m_IsCSharpEnable) {
		InternalEditorUtility.OpenFileAtLineExternal(
			Path.Combine(m_Folder, m_CSharpName + ".cs"), -1, -1);
	}

	if (m_IsUxmlEnable) {
		InternalEditorUtility.OpenFileAtLineExternal(
			Path.Combine(m_Folder, m_UxmlName + ".uxml"), -1, -1);
	}

	if (m_IsUssEnable) {
		InternalEditorUtility.OpenFileAtLineExternal(
			Path.Combine(m_Folder, m_UssName + ".uss"), -1, -1);
	}
}
}
