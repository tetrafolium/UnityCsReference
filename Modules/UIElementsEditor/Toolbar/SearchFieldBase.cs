...
public abstract class SearchFieldBase<TextInputType, T>
	: VisualElement,
	  INotifyValueChanged<T> where TextInputType : TextInputBaseField<T>,
	new () {
private readonly Button m_SearchButton;
private readonly Button m_CancelButton;
private readonly TextInputType m_TextField;
protected TextInputType textInputField {
...
private void OnValueChanged(ChangeEvent<T> change) {
	UpdateCancelButton(); // We need to update it on value changed because in
	                      // most cases the TextField is modified directly
}
...
public virtual void SetValueWithoutNotify(T newValue) {
	...
	UpdateCancelButton(); // We need to update it in that case because
...
private void UpdateCancelButton() {
	m_CancelButton.EnableInClassList(cancelButtonOffVariantUssClassName,
