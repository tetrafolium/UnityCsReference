...
namespace UnityEditor.UIElements {
  ...
  internal class VisualTreeAssetEditor : ScriptableObjectAssetEditor {
    ...
    public void Render(VisualTreeAsset vta, Rect r, GUIStyle background) {
      ...
      var viewportRect =
          GUIClip.UnclipToWindow(r); // Still in points, not pixels
      var viewportRectPx =
          new Rect(viewportRect.min * dpiScale, viewportRect.size * dpiScale);
      m_Panel.visualTree.SetSize(
          viewportRect.size); // We will draw relative to a viewport covering
                              // the preview area, so draw at 0,0
      ...
      RectInt oldViewport =
          UnityEngine.UIElements.UIR.Utility.GetActiveViewport();
      GL.Viewport(new Rect(
          new Vector2(viewportRectPx.xMin, Screen.height - viewportRectPx.yMin -
                                               viewportRectPx.height),
          viewportRectPx.size));

      m_Panel.Repaint(Event.current);

      GL.Viewport(new Rect(oldViewport.xMin, oldViewport.yMin,
