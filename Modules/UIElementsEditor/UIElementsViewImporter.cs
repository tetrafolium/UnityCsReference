...
namespace UnityEditor.UIElements {
  ...
  [ScriptedImporter(version : 7, ext
                    : "uxml", importQueueOffset : 1100)]
  [ExcludeFromPreset]
  internal class UIElementsViewImporter : ScriptedImporter {
    ...
    static string[] GatherDependenciesFromSourceFile(string assetPath) {
      ...
      try {
        doc = XDocument.Parse(File.ReadAllText(assetPath),
                              LoadOptions.SetLineInfo);
      } catch (Exception) {
        // We want to be silent here, all XML syntax errors will be reported
  ...
  internal class UXMLImporterImpl : StyleValueImporter {
    internal struct Error {
      ...
      public Error(ImportErrorType error, ImportErrorCode code, object context,
                   Level level, string filePath, IXmlLineInfo xmlLineInfo) {
      ...
      private static string ErrorMessage(ImportErrorCode errorCode) {
        ...
        case ImportErrorCode.InvalidRootElement:
          return "Expected the XML Root element name to be '" + k_RootNode +
                 "', found '{0}'";
        case ImportErrorCode.TemplateHasEmptyName:
          return "'" + k_TemplateNode + "' declaration requires a non-empty '" +
                 k_TemplateNameAttr + "' attribute";
        case ImportErrorCode.TemplateInstanceHasEmptySource:
          return "'" + k_TemplateInstanceNode +
                 "' declaration requires a non-empty '" +
                 k_TemplateInstanceSourceAttr + "' attribute";
        case ImportErrorCode.TemplateMissingPathOrSrcAttribute:
          return "'" + k_TemplateNode + "' declaration requires a '" +
                 k_GenericPathAttr + "' or '" + k_GenericSrcAttr +
                 "' attribute referencing another UXML file";
        case ImportErrorCode.TemplateSrcAndPathBothSpecified:
          return "'" + k_TemplateNode + "' declaration does not accept both '" +
                 k_GenericSrcAttr + "' and '" + k_GenericPathAttr +
                 "' attributes";
        ...
        case ImportErrorCode.StyleReferenceEmptyOrMissingPathOrSrcAttr:
          return "'" + k_StyleReferenceNode + "' declaration requires a '" +
                 k_GenericPathAttr + "' or '" + k_GenericSrcAttr +
                 "' attribute referencing a USS file";
        case ImportErrorCode.StyleReferenceSrcAndPathBothSpecified:
          return "'" + k_StyleReferenceNode +
                 "' declaration does not accept both '" + k_GenericSrcAttr +
                 "' and '" + k_GenericPathAttr + "' attributes";
        ...
        default:
          throw new ArgumentOutOfRangeException("Unhandled error code " +
                                                errorCode);
        }
      }
      ...
      public override string ToString() {
        ...
        string lineInfo = xmlLineInfo == null
                              ? ""
            : UnityString.Format(" ({0},{1})", xmlLineInfo.LineNumber,
                                 xmlLineInfo.LinePosition);
        return UnityString.Format(
            "{0}{1}: {2} - {3}", filePath, lineInfo, error,
            UnityString.Format(message, context == null ? "<null>"
                               : context.ToString()));
      }
    }
    ...
    internal class DefaultLogger {
      ...
      internal virtual void LogError(ImportErrorType error,
                                     ImportErrorCode code, object context,
                                     Error.Level level,
                                     IXmlLineInfo xmlLineInfo) {
        m_Errors.Add(
            new Error(error, code, context, level, m_Path, xmlLineInfo));
      }
      ...
      internal virtual void BeginImport(string path) {
        ...
        // some files are unexpectedly getting imported before their
        // dependencies.
      ...
      internal virtual void FinishImport() {
        Dictionary<string, VisualTreeAsset> cache =
            new Dictionary<string, VisualTreeAsset>();

        foreach (var error in m_Errors) {
          VisualTreeAsset obj;
          if (!cache.TryGetValue(error.filePath, out obj))
            cache.Add(error.filePath,
                      obj = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(
                          error.filePath));
    ...
    const string k_GenericPathAttr =
        UxmlGenericAttributeNames.k_PathAttributeName;
    const string k_GenericSrcAttr =
        UxmlGenericAttributeNames.k_SrcAttributeName;
    ...
    const string k_TemplateNameAttr =
        UxmlGenericAttributeNames.k_NameAttributeName;
    const string k_TemplateInstanceNode =
        TemplateContainer.UxmlFactory.k_ElementName;
    const string k_TemplateInstanceSourceAttr =
        TemplateContainer.UxmlTraits.k_TemplateAttributeName;
    ...
    const string k_AttributeOverridesNode =
        UxmlAttributeOverridesFactory.k_ElementName;
    const string k_AttributeOverridesElementNameAttr =
        UxmlAttributeOverridesTraits.k_ElementNameAttributeName;
    ...
    public void Import(out VisualTreeAsset asset) {
      ...
      // This guarantees that all files were imported in the correct order
      // before logging any errors.
    ...
    void ImportXml(string xmlPath, out VisualTreeAsset vta) {
      ...
      try {
        ...
        logger.LogError(ImportErrorType.Syntax, ImportErrorCode.InvalidXml, e,
                        Error.Level.Fatal, null);
        return;
      }
    ...
    void LoadXmlRoot(XDocument doc, VisualTreeAsset vta) {
      ...
      if (!string.Equals(elt.Name.LocalName, k_RootNode, k_Comparison)) {
        logger.LogError(ImportErrorType.Semantic,
                        ImportErrorCode.InvalidRootElement, elt.Name,
    ...
    void LoadTemplateNode(VisualTreeAsset vta, XElement elt, XElement child) {
      ...
      foreach (var xAttribute in child.Attributes()) {
        ...
        case k_TemplateNameAttr:
          ...
          if (String.IsNullOrEmpty(name)) {
            logger.LogError(ImportErrorType.Semantic,
                            ImportErrorCode.TemplateHasEmptyName, child,
                            Error.Level.Fatal, child);
          }
          break;
        default:
          logger.LogError(ImportErrorType.Semantic,
                          ImportErrorCode.UnknownAttribute,
                          xAttribute.Name.LocalName, Error.Level.Fatal, child);
          break;
        }
      }
      ...
      if (hasPath == hasSrc) {
        logger.LogError(ImportErrorType.Semantic,
                        hasPath ? ImportErrorCode.TemplateSrcAndPathBothSpecified
                        : ImportErrorCode.TemplateMissingPathOrSrcAttribute,
                          null, Error.Level.Fatal, elt);
        return;
      }
      ...
      if (vta.TemplateExists(name)) {
        logger.LogError(ImportErrorType.Semantic,
                        ImportErrorCode.DuplicateTemplateName, name,
                        Error.Level.Fatal, elt);
        return;
      }
      ...
      if (hasPath) {
        ...
        URIValidationResult result = URIHelpers.ValidAssetURL(
            assetPath, src, out errorMessage, out projectRelativePath);

        if (result != URIValidationResult.OK) {
          logger.LogError(ImportErrorType.Semantic, ConvertErrorCode(result),
                          errorMessage, Error.Level.Fatal, elt);
        } else {
          ...
          if (asset is VisualTreeAsset) {
            ...
            logger.LogError(ImportErrorType.Semantic,
                            ImportErrorCode.ReferenceInvalidAssetType,
                            projectRelativePath, Error.Level.Fatal, elt);
          }
        }
      }
    }
    ...
    static void AddDependency(string assetPath, XElement templateNode,
    ...
    internal static void PopulateDependencies(string assetPath, XElement elt,
                                              List<string> dependencies) {
    ...
    void LoadXml(XElement elt, VisualElementAsset parent, VisualTreeAsset vta,
      ...
      // id includes the parent id, meaning it's dependent on the whole direct
      // hierarchy
      vea.id = (vta.GetNextChildSerialNumber() + 585386304) * -1521134295 +
               parentHash;
        ...
        foreach (XElement child in elt.Elements()) {
          if (child.Name.LocalName == k_TemplateNode)
            ...
            LoadStyleReferenceNode(vea, child);
                   ...
                   child.Name.LocalName == k_AttributeOverridesNode)
    ...
    void LoadStyleReferenceNode(VisualElementAsset vea, XElement styleElt) {
      ...
      if (hasPath == hasSrc) {
        logger.LogError(
            ...
            hasPath ? ImportErrorCode.StyleReferenceSrcAndPathBothSpecified
            : ImportErrorCode.StyleReferenceEmptyOrMissingPathOrSrcAttr, null,
              Error.Level.Warning, styleElt);
        return;
      }
      ...
      if (hasPath) {
        ...
        URIValidationResult result =
            URIHelpers.ValidAssetURL(assetPath, srcAttr.Value, out errorMessage,
                                     out projectRelativePath);

        if (result != URIValidationResult.OK) {
          logger.LogError(ImportErrorType.Semantic, ConvertErrorCode(result),
                          errorMessage, Error.Level.Fatal, styleElt);
        } else {
          Object asset = DeclareDependencyAndLoad(projectRelativePath);
          ...
          if (asset is StyleSheet) {
            ...
            logger.LogError(ImportErrorType.Semantic,
                            ImportErrorCode.ReferenceInvalidAssetType,
                            projectRelativePath, Error.Level.Fatal, styleElt);
          }
        }
      }
    }

    void LoadAttributeOverridesNode(TemplateAsset templateAsset,
                                    XElement attributeOverridesElt) {
      var elementNameAttr =
          attributeOverridesElt.Attribute(k_AttributeOverridesElementNameAttr);
      if (elementNameAttr == null ||
          String.IsNullOrEmpty(elementNameAttr.Value)) {
        logger.LogError(
            ImportErrorType.Semantic,
            ImportErrorCode.AttributeOverridesMissingElementNameAttr, null,
            Error.Level.Warning, attributeOverridesElt);
        return;
      }
    ...
    VisualElementAsset ResolveType(XElement elt,
                                   VisualTreeAsset visualTreeAsset) {
      string elementNamespaceName = elt.Name.NamespaceName;

      if (elementNamespaceName.StartsWith(
              "UnityEditor.Experimental.UIElements") ||
          elementNamespaceName.StartsWith(
              "UnityEngine.Experimental.UIElements")) {
        elementNamespaceName = elementNamespaceName.Replace(
            ".Experimental.UIElements", ".UIElements");
      }
      ...
      if (elt.Name.LocalName == k_TemplateInstanceNode &&
          elementNamespaceName == typeof(TemplateContainer).Namespace) {
        XAttribute sourceAttr = elt.Attribute(k_TemplateInstanceSourceAttr);
        if (sourceAttr == null || String.IsNullOrEmpty(sourceAttr.Value)) {
          logger.LogError(ImportErrorType.Semantic,
                          ImportErrorCode.TemplateInstanceHasEmptySource, null,
                          Error.Level.Fatal, elt);
          return null;
        }
        ...
        if (!visualTreeAsset.TemplateExists(templateName)) {
          logger.LogError(ImportErrorType.Semantic,
                          ImportErrorCode.UnknownTemplate, templateName,
                          Error.Level.Fatal, elt);
          return null;
        }
    ...
    bool ParseAttributes(XElement elt, VisualElementAsset res,
                         VisualTreeAsset vta, VisualElementAsset parent) {
      ...
      foreach (XAttribute xattr in elt.Attributes()) {
        ...
        case "contentContainer":
          ...
          if (vta.contentContainerId != 0) {
            logger.LogError(ImportErrorType.Semantic,
                            ImportErrorCode.DuplicateContentContainer, null,
                            Error.Level.Fatal, elt);
            continue;
          }
        ...
        case k_SlotDefinitionAttr:
          logger.LogError(ImportErrorType.Syntax,
                          ImportErrorCode.SlotsAreExperimental, null,
                          Error.Level.Warning, elt);
          if (String.IsNullOrEmpty(xattr.Value))
            logger.LogError(ImportErrorType.Semantic,
                            ImportErrorCode.SlotDefinitionHasEmptyName, null,
                            Error.Level.Fatal, elt);
          else if (!vta.AddSlotDefinition(xattr.Value, res.id))
            logger.LogError(ImportErrorType.Semantic,
                            ImportErrorCode.DuplicateSlotDefinition,
                            xattr.Value, Error.Level.Fatal, elt);
          continue;
        case k_SlotUsageAttr:
          ...
          if (templateAsset == null) {
            logger.LogError(ImportErrorType.Semantic,
                            ImportErrorCode.SlotUsageInNonTemplate, parent,
                            Error.Level.Fatal, elt);
            continue;
          }
          if (string.IsNullOrEmpty(xattr.Value)) {
            logger.LogError(ImportErrorType.Semantic,
                            ImportErrorCode.SlotUsageHasEmptyName, null,
                            Error.Level.Fatal, elt);
            continue;
          }
        ...
        case "style":
          ...
          ExCSS.StyleSheet parsed =
              new Parser().Parse("* { " + xattr.Value + " }");
          if (parsed.Errors.Count != 0) {
            logger.LogError(ImportErrorType.Semantic,
                            ImportErrorCode.InvalidCssInStyleAttribute,
                            parsed.Errors.Aggregate(
                                "", (s, error) => s + error.ToString() + "\n"),
                            Error.Level.Warning, xattr);
            continue;
          }
          if (parsed.StyleRules.Count != 1) {
            logger.LogError(ImportErrorType.Semantic,
                            ImportErrorCode.InvalidCssInStyleAttribute,
                            "Expected one style rule, found " +
                                parsed.StyleRules.Count,
                            Error.Level.Warning, xattr);
            continue;
          }
          ...
          // Don't call m_Builder.EndRule() here, it's done in LoadXml to get
          // the rule index at the same time !
          continue;
        }
  ...
  internal enum ImportErrorType { Syntax, Semantic }
}
