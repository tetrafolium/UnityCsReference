  ...
  public class ObjectField : BaseField<Object> {
    ...
    public new class UxmlTraits : BaseField<Object>.UxmlTraits {
      UxmlBoolAttributeDescription m_AllowSceneObjects =
          new UxmlBoolAttributeDescription{name = "allow-scene-objects",
                                           defaultValue = true};
      UxmlTypeAttributeDescription<Object> m_ObjectType =
          new UxmlTypeAttributeDescription<Object>{name = "type"};

      public override void Init(VisualElement ve, IUxmlAttributes bag,
                                CreationContext cc) {
        base.Init(ve, bag, cc);

        ((ObjectField) ve).allowSceneObjects =
            m_AllowSceneObjects.GetValueFromBag(bag, cc);
    ...
    public override void SetValueWithoutNotify(Object newValue) {
      ...
      var valueChanged =
          !EqualityComparer<Object>.Default.Equals(this.value, newValue);
    ...
    private class ObjectFieldDisplay : VisualElement {
      ...
      public static readonly string acceptDropVariantUssClassName =
          ussClassName + "--accept-drop";

      public ObjectFieldDisplay(ObjectField objectField) {
        AddToClassList(ussClassName);
        m_ObjectIcon = new Image{scaleMode = ScaleMode.ScaleAndCrop,
      ...
      public void Update() {
        GUIContent content = EditorGUIUtility.ObjectContent(
            m_ObjectField.value, m_ObjectField.objectType);
      ...
      private void OnDragLeave() {
        // Make sure we've cleared the accept drop look, whether we we in a drop
        // operation or not.
      ...
      private void OnMouseDown(MouseDownEvent evt) {
        ...
        // Double click opens the asset in external app or changes selection to
        // referenced object
      ...
      private Object DNDValidateObject() {
        ...
        Object validatedObject = EditorGUI.ValidateObjectFieldAssignment(
            references, m_ObjectField.objectType, null,
            EditorGUI.ObjectFieldValidatorOptions.None);

        if (validatedObject != null) {
          // If scene objects are not allowed and object is a scene object then
          // clear
          if (!m_ObjectField.allowSceneObjects &&
    ...
    public static readonly string objectUssClassName =
        ussClassName + "__object";
    public static readonly string selectorUssClassName =
        ussClassName + "__selector";

    public ObjectField() : this(null) {}

    public ObjectField(string label) : base(label, null) {
    ...
    internal void ShowObjectSelector() {
      // Since we have nothing useful to do on the object selector closing
      // action, we just do not assign any callback All the object changes will
      // be notified through the OnObjectChanged and a "cancellation" (Escape
      // key) on the ObjectSelector is calling the closing callback without any
      // good object
      ObjectSelector.get.Show(value, objectType, null, allowSceneObjects, null,
                              null, OnObjectChanged);
    }
