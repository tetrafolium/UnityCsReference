...
namespace UnityEditor.UIElements {
  public enum DeltaSpeed { Fast, Normal, Slow }
  ...
  public abstract class TextValueField<TValueType>
      : TextInputBaseField<TValueType>, IValueField<TValueType> {
    ...
    protected TextValueField(string label, int maxLength,
                             TextValueInput textValueInput)
        : base(label, maxLength, Char.MinValue, textValueInput) {
      SetValueWithoutNotify(default(TValueType));
    }

    public abstract void ApplyInputDeviceDelta(Vector3 delta, DeltaSpeed speed,
                                               TValueType startValue);
    public void StartDragging() { textValueInput.StartDragging(); }

    public void StopDragging() { textValueInput.StopDragging(); }
    ...
    protected void AddLabelDragger<TDraggerType>() {
      var dragger =
          new FieldMouseDragger<TDraggerType>((IValueField<TDraggerType>) this);
    ...
    public override void SetValueWithoutNotify(TValueType newValue) {
      ...
      if (textValueInput.m_UpdateTextFromValue) {
        ...
        // In the case of an expression like 2+2, the text might not be equal to
        // the result
        text = ValueToString(rawValue);
      }
    }
    ...
    protected abstract class TextValueInput : TextInputBase {
      TextValueField<TValueType> textValueFieldParent =>
          (TextValueField<TValueType>) parent;

      protected TextValueInput() { m_UpdateTextFromValue = true; }

      internal bool m_UpdateTextFromValue;

      internal override bool AcceptCharacter(char c) {
        return base.AcceptCharacter(c) && c != 0 &&
               allowedCharacters.IndexOf(c) != -1;
      }
      ...
      public abstract void ApplyInputDeviceDelta(Vector3 delta,
                                                 DeltaSpeed speed,
                                                 TValueType startValue);
      ...
      protected override void ExecuteDefaultActionAtTarget(EventBase evt) {
        ...
        if (evt.eventTypeId == KeyDownEvent.TypeId()) {
          ...
          if ((kde?.character == 3) ||  // KeyCode.KeypadEnter
              (kde?.character == '\n')) // KeyCode.Return
          {
            // Here we should update the value, but it will be done when the
            // blur event will be handled...
            parent.Focus();
          } else if (!isReadOnly) {
            hasChanged = true;
          }
        } else if (!isReadOnly &&
                   evt.eventTypeId == ExecuteCommandEvent.TypeId()) {
          ExecuteCommandEvent commandEvt = evt as ExecuteCommandEvent;
          string cmdName = commandEvt.commandName;
          if (cmdName == EventCommandNames.Paste ||
        ...
        if (!textValueFieldParent.isDelayed && hasChanged) {
          ...
          // This allow expression (2+2) or string like 00123 to remain as typed
          // in the TextField until enter is pressed
  ...
  // Derive from BaseFieldTraits in order to not inherit from TextInputBaseField
  // UXML attributes.
  public class TextValueFieldTraits<TValueType, TValueUxmlAttributeType>
      : BaseFieldTraits<TValueType, TValueUxmlAttributeType> where
            TValueUxmlAttributeType : TypedUxmlAttributeDescription<TValueType>,
                                      new () {
