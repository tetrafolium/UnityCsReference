...
namespace UnityEditor.UIElements {
  public abstract class BaseCompositeField<TValueType, TField, TFieldValue>
      : BaseField<TValueType> where TField : TextValueField<TFieldValue>,
                                             new () {
    internal struct FieldDescription {
      public delegate void WriteDelegate(ref TValueType val,
      ...
      public FieldDescription(string name, string ussName,
                              Func<TValueType, TFieldValue> read,
                              WriteDelegate write) {
        this.name = name;
    ...
    protected BaseCompositeField(string label, int fieldsByLine)
      ...
      for (int i = 0; i < numberOfLines; i++) {
        ...
        for (int j = i * fieldsByLine; j < ((i * fieldsByLine) + fieldsByLine);
             j++) {
          var desc = fieldDescriptions[j];
          var field = new TField(){name = desc.ussName};
          ...
          field.RegisterValueChangedCallback(e => {
            ...
            // Here, just check and make sure the text is updated in the basic
            // field and is the same as the value... For example, backspace done
            // on a selected value will empty the field (text == "") but the
            // value will be 0. Or : a text of "2+3" is valid until enter is
            // pressed, so not equal to a value of "5".
            var valueString = e.newValue.ToString();
            var textString = ((TField) e.currentTarget).text;
            // If text is different or value changed because of an explicit
            // value set
    ...
    public override void SetValueWithoutNotify(TValueType newValue) {
      var displayNeedsUpdate =
          m_ShouldUpdateDisplay &&
          !EqualityComparer<TValueType>.Default.Equals(rawValue, newValue);
