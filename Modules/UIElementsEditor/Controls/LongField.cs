...
namespace UnityEditor.UIElements {
  public class LongField : TextValueField<long> {
    ...
    public new class UxmlTraits
        : TextValueFieldTraits<long, UxmlLongAttributeDescription> {}
    ...
    public LongField() : this((string) null) {}
    ...
    CanTryParse(string textString) => long.TryParse(textString, out _);

    public override void ApplyInputDeviceDelta(Vector3 delta, DeltaSpeed speed,
                                               long startValue) {
    ...
    class LongInput : TextValueInput {
      ...
      internal LongInput() { formatString = EditorGUI.kIntFieldFormatString; }
      ...
      ApplyInputDeviceDelta(Vector3 delta, DeltaSpeed speed, long startValue) {
        double sensitivity =
            NumericFieldDraggerUtility.CalculateIntDragSensitivity(startValue);
        float acceleration = NumericFieldDraggerUtility.Acceleration(
            speed == DeltaSpeed.Fast, speed == DeltaSpeed.Slow);
        long v = StringToValue(text);
        v += (long) Math.Round(
            NumericFieldDraggerUtility.NiceDelta(delta, acceleration) *
            sensitivity);
