  ...
  public class ColorField : BaseField<Color> {
    ...
    public new class UxmlTraits
      ...
      UxmlBoolAttributeDescription m_ShowEyeDropper =
          new UxmlBoolAttributeDescription{name = "show-eye-dropper",
                                           defaultValue = true};
      UxmlBoolAttributeDescription m_ShowAlpha =
          new UxmlBoolAttributeDescription{name = "show-alpha",
                                           defaultValue = true};
      UxmlBoolAttributeDescription m_Hdr =
          new UxmlBoolAttributeDescription{name = "hdr"};

      public override void Init(VisualElement ve, IUxmlAttributes bag,
                                CreationContext cc) {
        base.Init(ve, bag, cc);

        ((ColorField) ve).showEyeDropper =
            m_ShowEyeDropper.GetValueFromBag(bag, cc);
    ...
    public ColorField(string label) : base(label, null) {
      ...
      // The focus on a color field is implemented like a BaseCompoundField :
      // the ColorField and its inner child are both put in the focus ring. When
      // the ColorField is receiving the Focus, it is "delegating" it to the
      // inner child, which is, in this case, the IMGUIContainer.
      m_ColorField = new IMGUIContainer(OnGUIHandler){
    ...
    private void OnGUIHandler() {
      // Dirty repaint on eye dropper update to preview the color under the
      // cursor
      if (Event.current.type == EventType.ExecuteCommand &&
          Event.current.commandName == EventCommandNames.EyeDropperUpdate) {
        IncrementVersion(VersionChangeType.Repaint);
      }

      Color newColor = EditorGUILayout.ColorField(
          GUIContent.none, value, showEyeDropper, showAlpha, hdr);
      value = newColor;
    }
  }
}
