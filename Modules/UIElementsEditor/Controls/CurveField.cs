...
namespace UnityEditor.UIElements {
  public class CurveField : BaseField<AnimationCurve> {
    ...
    public static readonly string contentUssClassName =
        ussClassName + "__content";
    public static readonly string borderUssClassName =
        ussClassName + "__border";

    private static CustomStyleProperty<Color> s_CurveColorProperty =
        new CustomStyleProperty<Color>("--unity-curve-color");
    public Rect ranges {
      get;
      set;
    }
    ...
    public enum RenderMode { Texture, Mesh, Default = Texture }
      ...
      set {
        if (m_RenderMode != value) {
          ...
          if (renderMode == RenderMode.Mesh) {
            ...
            m_ZeroIndicator = new VisualElement(){
                style = {height = 1, backgroundColor = Color.black}};
    ...
    public override AnimationCurve value {
      ...
      set {
        // I need to have total ownership of the curve, I won't be able to know
        // if it is changed outside. so I'm duplicating it.
        if (value != null ||
            !m_ValueNull) // let's not reinitialize an initialized curve
        {
          if (panel != null) {
            using(ChangeEvent<AnimationCurve> evt =
                      ChangeEvent<AnimationCurve>.GetPooled(rawValue, value)) {
              evt.target = this;
    ...
    public CurveField() : this(null) {}

    public CurveField(string label) : base(label, null) {
      ...
      VisualElement borderElement = new VisualElement(){
          name = "unity-border", pickingMode = PickingMode.Ignore};
    ...
    public override void SetValueWithoutNotify(AnimationCurve newValue) {
      ...
      if (CurveEditorWindow.visible &&
          Object.ReferenceEquals(CurveEditorWindow.curve, rawValue)) {
    ...
    private void OnCustomStyleResolved(CustomStyleResolvedEvent e) {
      ...
      if (m_CurveColor != oldColor && renderMode == RenderMode.Texture) {
        // The mesh texture is updated at each repaint, the standard texture
        // should however be regenerated
        m_TextureDirty = true;
      }
    }
    ...
    protected override void ExecuteDefaultAction(EventBase evt) {
      ...
      if (kde != null) {
        ...
        else if ((evt as MouseDownEvent)?.button == (int)MouseButton.LeftMouse)
        {
          ...
          if (visualInput.ContainsPoint(
                  visualInput.WorldToLocal(mde.mousePosition))) {
            showCurveEditor = true;
          }
        }
    ...
    void FillCurveData() {
      ...
      if (vertices == null ||
          vertices.Length != k_HorizontalCurveResolution * 2) {
      ...
      for (int k = 1; k < keyCount; ++k) {
        ...
        int sliceSampleCount = Mathf.FloorToInt((float) noKeySampleCount *
        ...
        for (int i = 1; i < sliceSampleCount + 1; ++i) {
          float time =
              sliceStartTime + i * sliceDuration / (sliceSampleCount + 1);
      ...
      Vector3 scale =
          new Vector3(m_Content.layout.width, m_Content.layout.height);

      var yStartValue =
          (!Mathf.Approximately(minValue, maxValue)) ? 1.0f : 0.5f;
      vertices[0] = vertices[1] = Vector3.Scale(
          new Vector3(0,
                      yStartValue -
                          Mathf.InverseLerp(minValue, maxValue, valueCache[0]),
                      0),
          scale);

      Vector3 secondPoint = Vector3.Scale(
          new Vector3(1.0f / k_HorizontalCurveResolution,
                      yStartValue -
                          Mathf.InverseLerp(minValue, maxValue, valueCache[1]),
                      0),
          scale);
      ...
      for (int i = 1; i < k_HorizontalCurveResolution - 1; ++i) {
        ...
        Vector3 nextPoint = Vector3.Scale(
            new Vector3(Mathf.InverseLerp(startTime, endTime, timeCache[i + 1]),
                        yStartValue - Mathf.InverseLerp(minValue, maxValue,
                                                        valueCache[i + 1]),
                        0),
            scale);
      ...
      vertices[(k_HorizontalCurveResolution - 1) * 2] =
          vertices[(k_HorizontalCurveResolution - 1) * 2 + 1] = currentPoint;

      norm = new Vector3(prevDir.y, -prevDir.x, 1);
      normals[(k_HorizontalCurveResolution - 1) * 2] =
          -norm * k_VertexHalfWidth;
      normals[(k_HorizontalCurveResolution - 1) * 2 + 1] =
          norm * k_VertexHalfWidth;
      ...
      if (Mathf.Approximately(minValue, maxValue)) {
        m_ZeroIndicator.style.top =
            m_Content.layout.height * Mathf.InverseLerp(-1, 1, minValue);
      } else {
        m_ZeroIndicator.style.top =
            m_Content.layout.height *
            (yStartValue - Mathf.InverseLerp(minValue, maxValue, 0));
      }
    }
    ...
    void SetupStandardRepaint() {
      ...
      // This will mimic the IMGUI curve since the range will be calculated by
      // the CurvePreview if the range is at the default value...
      ...
      if (previewHeight > 1 && previewWidth > 1) {
        if (!m_ValueNull) {
          m_Texture = AnimationCurvePreviewCache.GenerateCurvePreview(
              previewWidth, previewHeight, rangeRect, rawValue, curveColor,
              m_Texture);
        } else {
          m_Texture = null;
        }
    ...
    private void OnGenerateVisualContent(MeshGenerationContext mgc) {
      if (renderMode == RenderMode.Mesh) {
        ...
        if (m_Texture != null) {
          var rectParams =
              MeshGenerationContextUtils.RectangleParams.MakeTextured(
                  new Rect(0, 0, m_Texture.width, m_Texture.height),
                  new Rect(0, 0, 1, 1), m_Texture, ScaleMode.StretchToFill,
                  panel.contextType);
    ...
    class CurveFieldContent : ImmediateModeElement {
      ...
      public void SetMesh(Mesh mesh) { m_Mesh = mesh; }

      public CurveFieldContent() { pickingMode = PickingMode.Ignore; }
      ...
      protected override void ImmediateRepaint() {
        if (m_Mesh != null) {
          if (m_Mat == null) {
            m_Mat = new Material(EditorGUIUtility.LoadRequired(
                "Shaders/UIElements/AACurveField.shader") as Shader);
      ...
      void DrawMesh() {
        ...
        // Send the view zoom factor so that the antialias width do not grow
        // when zooming in.
        m_Mat.SetFloat("_ZoomFactor", scale * realWidth /
                                          CurveField.k_EdgeWidth *
                                          EditorGUIUtility.pixelsPerPoint);

        // Send the view zoom correction so that the vertex shader can scale the
        // edge triangles when below m_MinWidth.
