...
static readonly GradientColorKey k_WhiteKeyBegin =
	new GradientColorKey(Color.white, 0);
static readonly GradientColorKey k_WhiteKeyEnd =
	new GradientColorKey(Color.white, 1);
static readonly GradientAlphaKey k_AlphaKeyBegin =
...
public override Gradient value {
	...
	set {
		if (value != null ||
		    !m_ValueNull) // let's not reinitialize an initialized gradient
		{
			using (ChangeEvent<Gradient> evt =
				       ChangeEvent<Gradient>.GetPooled(rawValue, value)) {
				evt.target = this;
...
public GradientField(string label) : base(label, null) {
	AddToClassList(ussClassName);
	labelElement.AddToClassList(labelUssClassName);
	visualInput.AddToClassList(inputUssClassName);

	m_GradientTextureImage =
		new VisualElement(){
...
void UpdateGradientTexture() {
	if (m_ValueNull) {
		...
		Texture2D gradientTexture =
			UnityEditorInternal.GradientPreviewCache.GenerateGradientPreview(
				value, computedStyle.backgroundImage.value.texture);
		m_GradientTextureImage.style.backgroundImage = gradientTexture;

		IncrementVersion(
			VersionChangeType
			.Repaint); // since the Texture2D object can be reused, force
		                   // dirty because the backgroundImage change will only
		                   // trigger the Dirty if the Texture2D objects are
		                   // different.
	}
}
...
void OnGradientChanged(Gradient newValue) {
	...
	.ClearCache();   // needed because GradientEditor itself uses the cache
	                 // and will no invalidate it on changes.
