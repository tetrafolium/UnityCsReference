...
namespace UnityEditor.UIElements {
  static class EnumFieldHelpers {
    internal static readonly UxmlTypeAttributeDescription<Enum> type =
        new UxmlTypeAttributeDescription<Enum>{name = "type"};
    internal static readonly UxmlStringAttributeDescription value =
        new UxmlStringAttributeDescription{name = "value"};
    internal static readonly UxmlBoolAttributeDescription
        includeObsoleteValues = new UxmlBoolAttributeDescription(){
            name = "include-obsolete-values", defaultValue = false};

    internal static bool ExtractValue(IUxmlAttributes bag, CreationContext cc,
                                      out Enum resEnumValue,
                                      out bool resIncludeObsoleteValues) {
      ...
      if (value.TryGetValueFromBag(bag, cc, ref specifiedValue) &&
          !Enum.IsDefined(systemType, specifiedValue)) {
        Debug.LogErrorFormat(
            "EnumField: Could not parse value of '{0}', because it isn't defined in the {1} enum.",
            specifiedValue, systemType.FullName);
        return false;
      }

      resEnumValue =
          specifiedValue != null ?(Enum)
                                Enum.Parse(systemType, specifiedValue)
          : (Enum) Enum.ToObject(systemType, 0);
...
#pragma warning disable 414
      ...
      private UxmlBoolAttributeDescription m_IncludeObsoleteValues =
          EnumFieldHelpers.includeObsoleteValues;
#pragma warning restore 414
      ...
      public override void Init(VisualElement ve, IUxmlAttributes bag,
        ...
        if (EnumFieldHelpers.ExtractValue(bag, cc, out resEnumValue,
                                          out resIncludeObsoleteValues)) {
    ...
    public EnumFlagsField() : this(null, null, false) {}
    ...
    public void Init(Enum defaultValue, bool includeObsoleteValues = false) {
      ...
      m_EnumData =
          EnumDataUtility.GetCachedEnumData(m_EnumType, !includeObsoleteValues);

      if (!m_EnumData.flags)
        Debug.LogWarning(
            "EnumMaskField is not bound to enum type with the [Flags] attribute");
    ...
    public EnumFlagsField(string label) : base(label) {
