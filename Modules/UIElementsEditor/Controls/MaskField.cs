...
namespace UnityEditor.UIElements {
  public abstract class BaseMaskField<TChoice>
      : BasePopupField<TChoice, string> {
    ...
    // This is the list of masks for every specific choices... if null, the mask
    // will be computed with the default values More details about this :
    //   In IMGUI, the MaskField is only allowing the creation of the field with
    //   an array of choices that will have a mask
    //    based on power of 2 value starting from 1.
    //   However, the LayerMaskField is created based on the Layers and do not
    //   necessarily has power of 2 consecutive masks.
    //     Therefore, this specific internal field (in IMGUI...) is requiring a
    //     specific array to contain the mask value of the actual list of
    //     choices.
    List<int> m_UserChoicesMasks;

    // This is containing a mask to cover all the choices from the list.
    // Computed with the help of m_UserChoicesMasks
    ...
    void ComputeFullChoiceMask() {
      // Compute the full mask for all the items... it is not necessarily ~0
      // (which is all bits set to 1)
      if (m_UserChoices.Count == 0) {
        m_FullChoiceMask = 0;
      } else {
        if ((m_UserChoicesMasks != null) &&
            (m_UserChoicesMasks.Count == m_UserChoices.Count)) {
    ...
    public override void SetValueWithoutNotify(TChoice newValue) {
      base.SetValueWithoutNotify(
          MaskToValue(UpdateMaskIfEverything(ValueToMask(newValue))));
    }
    ...
    internal override void AddMenuItems(GenericMenu menu) {
      ...
      foreach (var item in m_Choices) {
        ...
        menu.AddItem(
            new GUIContent(GetListItemToDisplay(MaskToValue(maskOfItem))),
            isSelected, () => ChangeValueFromMenu(item));
      }
    }

    // Based on the current mask, this is updating the value of the actual mask
    // to use vs the full mask. This is returning ~0 if all the values are
    // selected...
  ...
  public class MaskField : BaseMaskField<int> {
    ...
    {
      UxmlStringAttributeDescription m_MaskChoices =
          new UxmlStringAttributeDescription{name = "choices"};
      UxmlIntAttributeDescription m_MaskValue =
          new UxmlIntAttributeDescription{name = "value"};
    ...
    internal bool ShouldFormatSelectedValue() {
      return rawValue != 0 && rawValue != -1 &&
             m_FormatSelectedValueCallback != null && IsPowerOf2(rawValue);
    }
    ...
    public MaskField(List<string> choices, int defaultMask,
        ...
        : this(null, choices, defaultMask, formatSelectedValueCallback,
    ...
    public MaskField(string label) : base(label) {
