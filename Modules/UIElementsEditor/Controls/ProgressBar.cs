...
namespace UnityEditor.UIElements {
  public class ProgressBar : BindableElement, INotifyValueChanged<float> {
    ...
    public static readonly string titleContainerUssClassName =
        ussClassName + "__title-container";
    public static readonly string progressUssClassName =
        ussClassName + "__progress";
    ...
    internal float lowValue {
      get;
      private set;
    }
    internal float highValue {
      get;
      private set;
    }
    = 100f;
    ...
    public ProgressBar() {
      var tpl = EditorGUIUtility.Load(
          "UIPackageResources/UXML/ProgressBar.uxml") as VisualTreeAsset;
        ...
        if (!EqualityComparer<float>.Default.Equals(m_Value, value)) {
          if (panel != null) {
            using(ChangeEvent<float> evt =
                      ChangeEvent<float>.GetPooled(m_Value, value)) {
              evt.target = this;
    ...
    float CalculateProgressWidth(float width) {
      ...
      return maxWidth -
             Mathf.Max((maxWidth) * width / highValue, minVisibleProgress);
    }
  }
}
