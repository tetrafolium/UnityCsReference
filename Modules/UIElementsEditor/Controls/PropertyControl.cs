...
namespace UnityEditor.UIElements {
  ...
  internal class PropertyControl<TType> : BaseField<TType> {
    public new class UxmlFactory
        : UxmlFactory<PropertyControl<TType>, UxmlTraits> {
    ...
    public new class UxmlTraits : BaseField<TType>.UxmlTraits {
      UxmlStringAttributeDescription m_TypeOf =
          new UxmlStringAttributeDescription{
              name = "value-type", obsoleteNames = new[]{"typeOf"},
      ...
      public UxmlTraits() {
        m_TypeOf.restriction = new UxmlEnumeration{
            values = Enum.GetValues(typeof(DataType))
                         .Cast<DataType>()
      ...
      public override void Init(VisualElement ve, IUxmlAttributes bag,
        ...
        ((PropertyControl<TType>) ve)
            .SetValueWithoutNotify(
                ((PropertyControl<TType>) ve).StringToValue(initValue));
    ...
    enum DataType { Long, Double, Int, Float, String, Unsupported }
    ...
    public static readonly string controlUssClassName =
        ussClassName + "__control";

    public PropertyControl() : this("") {}

    public PropertyControl(string labelText) : base(null) {
      Init("", labelText);
    }
    ...
    private void CacheType() {
      ...
      if (ttype == typeof(long))
        ...
        throw new NotSupportedException(
            $"Unsupported type for PropertyControl {ttype}");
      }
    }
    ...
    private void AddLabelDragger<TDraggerType>() {
      var dragger = new FieldMouseDragger<TDraggerType>(
          (IValueField<TDraggerType>) m_Field);
    ...
    private void CreateControl<TControlType, TDataType>() where TControlType
        : BaseField<TDataType>,
          new () {
    ...
    private void CreateControl() {
      ...
      if (m_Field == null)
        throw new NotSupportedException(
            $"Unsupported type attribute: {typeof(TType)}");
    ...
    private void OnFieldValueChanged(ChangeEvent<TType> evt) {
      using(ChangeEvent<TType> newEvent =
                ChangeEvent<TType>.GetPooled(evt.previousValue, evt.newValue)) {
