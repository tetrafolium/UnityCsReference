...
namespace UnityEditor.UIElements {
  public class LayerMaskField : MaskField {
    ...
    {
      readonly UxmlIntAttributeDescription m_MaskValue =
      ...
      public override void Init(VisualElement ve, IUxmlAttributes bag,
        ...
        layerMaskField.SetValueWithoutNotify(
            m_MaskValue.GetValueFromBag(bag, cc));
        base.Init(ve, bag, cc);
      }
    }
      ...
      set {
        if (value != null) {
          Debug.LogWarning(L10n.Tr(
              "LayerMaskField doesn't support the formatting of the selected value."));
        }
    ...
    public override Func<string, string> formatListItemCallback {
      ...
      set {
        if (value != null) {
          Debug.LogWarning(L10n.Tr(
              "LayerMaskField doesn't support the formatting of the list items."));
        }
    ...
    public LayerMaskField(int defaultMask) : this(null, defaultMask) {}

    public LayerMaskField(string label, int defaultMask) : this(label) {
      SetValueWithoutNotify(defaultMask);
    }

    public LayerMaskField() : this(null) {}

    public LayerMaskField(string label) : base(label) {
    ...
    internal override void AddMenuItems(GenericMenu menu) {
      // We must update the choices and the values since we don't know if they
      // changed...
      UpdateLayersInfo();
