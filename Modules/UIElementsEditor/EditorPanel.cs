...
namespace UnityEditor.UIElements {
  sealed class EditorPanel : Panel {
    ...
    static EditorContextualMenuManager s_ContextualMenuManager =
        new EditorContextualMenuManager();
    static Shader s_EditorShader;
    static readonly int s_EditorColorSpaceID =
        Shader.PropertyToID("_EditorColorSpace");
    [InitializeOnLoad]
    static class EditorShaderLoader {
      static EditorShaderLoader() {
        // TODO: Remove this once case 1148851 has been fixed.
        UnityEngine.UIElements.UIR.UIRenderDevice.getEditorShader = () =>
            EditorShader;
      }
    }
    // Case 1183719 - The delegate getEditorShader is being reset upon domain
    // reload and InitializeOnLoad is not rerun Hence a static constructor to
    // Initialize the Delegate. EditorShaderLoader is still needed for Batch
    // mode where EditorPanel may not be created
    static EditorPanel() {
      // TODO: Remove this once case 1148851 has been fixed.
      UnityEngine.UIElements.UIR.UIRenderDevice.getEditorShader = () =>
          EditorShader;
    ...
    EditorPanel(ScriptableObject ownerObject)
        : base(ownerObject, ContextType.Editor,
    ...
    static void
    OnBeforeDrawChain(UnityEngine.UIElements.UIR.RenderChain renderChain) {
      Material mat = renderChain.GetStandardMaterial();
      mat.SetFloat(s_EditorColorSpaceID, QualitySettings.activeColorSpace ==
