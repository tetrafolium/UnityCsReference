...
private const string k_UielementsUxmllivereloadPrefsKey =
	"UIElements_UXMLLiveReload";

internal static bool UxmlLiveReloadIsEnabled {
	get {
		return EditorPrefs.GetBool(k_UielementsUxmllivereloadPrefsKey, false);
	}
...
static RetainedMode() {
	...
	Panel.TimeSinceStartup = () =>
	                         (long)(EditorApplication.timeSinceStartup * 1000.0f);
}
	...
	try {
		...
		while (it.MoveNext()) {
			...
			if (view != null && view.actualView != null &&
			    !(view.actualView is UIElementsDebugger)) {
