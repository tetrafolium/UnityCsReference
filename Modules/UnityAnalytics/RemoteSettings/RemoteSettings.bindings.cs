...
public static class RemoteSettings {
  ...
  RemoteSettingsUpdateCompleted(bool wasLastUpdatedFromServer,
                                bool settingsChanged, int response) {
  ...
  [Obsolete(
      "Calling CallOnUpdate() is not necessary any more and should be removed. Use RemoteSettingsUpdated instead",
      true)]
  public static void CallOnUpdate() {
    throw new NotSupportedException(
        "Calling CallOnUpdate() is not necessary any more and should be removed.");
  }
  ...
  // Returns the value corresponding to /key/ in the preference file if it
  // exists.
  [uei.ExcludeFromDocs]
  ...
  GetInt(string key,
         [ UnityEngine.Internal.DefaultValue("0") ] int defaultValue);
  ...
  public extern static long
  GetLong(string key,
          [ UnityEngine.Internal.DefaultValue("0") ] long defaultValue);
  ...
  public extern static float
  GetFloat(string key,
           [ UnityEngine.Internal.DefaultValue("0.0F") ] float defaultValue);
  ...
  GetString(string key,
            [ UnityEngine.Internal.DefaultValue("\"\"") ] string defaultValue);
  ...
  GetBool(string key,
          [ UnityEngine.Internal.DefaultValue("false") ] bool defaultValue);
  ...
  public static object GetObject(Type type, string key = "") {
    ...
    if (type.IsAbstract || type.IsSubclassOf(typeof(UnityEngine.Object)))
      throw new ArgumentException(
          "Cannot deserialize to new instances of type '" + type.Name + ".'");
  ...
  internal static extern object
  GetAsScriptingObject(Type t, object defaultValue, string key);

  public static IDictionary<string, object> GetDictionary(string key = "") {
    UseSafeLock();
    IDictionary<string, object> dict =
        RemoteConfigSettingsHelper.GetDictionary(GetSafeTopMap(), key);
    ReleaseSafeLock();
    return dict;
  }
...
public class RemoteConfigSettings : IDisposable {
  ...
  internal static extern IntPtr Internal_Create(RemoteConfigSettings rcs,
                                                string configKey);
  [ThreadSafe]
  ...
  [RequiredByNativeCode]
  internal static void
  RemoteConfigSettingsUpdated(RemoteConfigSettings rcs,
  ...
  public extern static bool QueueConfig(string name, object param, int ver = 1,
                                        string prefix = "");
  ...
  internal extern object GetAsScriptingObject(Type t, object defaultValue,
                                              string key);
...
internal static class RemoteConfigSettingsHelper {
  ...
  internal extern static long GetSafeNumber(IntPtr m, string key,
                                            long defaultValue);
  internal extern static float GetSafeFloat(IntPtr m, string key,
                                            float defaultValue);
  internal extern static bool GetSafeBool(IntPtr m, string key,
                                          bool defaultValue);
  internal extern static string GetSafeStringValue(IntPtr m, string key,
                                                   string defaultValue);
  ...
  internal static IDictionary<string, object> GetArrayMapEntries(IntPtr a,
                                                                 long i) {
    return GetDictionary(GetSafeArrayMap(a, i));
  }

  internal static T[] GetArrayEntriesType<T>(IntPtr a, long size,
                                             Func<IntPtr, long, T> f) {
    T[] r = new T[size];
  ...
  internal static object GetArrayEntries(IntPtr a) {
    ...
    case Tag.kIntVal:
    case Tag.kInt64Val:
      return GetArrayEntriesType<long>(a, size, GetSafeNumberArray);
    case Tag.kDoubleVal:
      return GetArrayEntriesType<float>(a, size, GetSafeArrayFloat);
    case Tag.kBoolVal:
      return GetArrayEntriesType<bool>(a, size, GetSafeArrayBool);
    case Tag.kStringVal:
      return GetArrayEntriesType<string>(a, size, GetSafeArrayStringValue);
    case Tag.kArrayVal:
      return GetArrayEntriesType<object>(a, size, GetArrayArrayEntries);
    case Tag.kMapVal:
      return GetArrayEntriesType<IDictionary<string, object>>(
          a, size, GetArrayMapEntries);
    }
    return null;
  }
  ...
  internal static object GetMixedArrayEntries(IntPtr a) {
    ...
    for (long i = 0; i < size; i++) {
      ...
      case Tag.kInt64Val:
        r [i]
        = GetSafeNumberArray(a, i);
        break;
      case Tag.kDoubleVal:
        r [i]
        = GetSafeArrayFloat(a, i);
        break;
      case Tag.kBoolVal:
        r [i]
        = GetSafeArrayBool(a, i);
        break;
      case Tag.kStringVal:
        r [i]
        = GetSafeArrayStringValue(a, i);
        break;
      case Tag.kArrayVal:
        r [i]
        = GetArrayArrayEntries(a, i);
        break;
      case Tag.kMapVal:
        r [i]
        = GetArrayMapEntries(a, i);
        break;
      }
    }
    return r;
  ...
  internal static void SetDictKeyType(IntPtr m,
                                      IDictionary<string, object> dict,
                                      string key, Tag tag) {
    switch (tag) {
    case Tag.kIntVal:
    case Tag.kInt64Val:
      dict [key]
      = GetSafeNumber(m, key, 0);
      break;
    case Tag.kDoubleVal:
      dict [key]
      = GetSafeFloat(m, key, 0);
      break;
    case Tag.kBoolVal:
      dict [key]
      = GetSafeBool(m, key, false);
      break;
    case Tag.kStringVal:
      dict [key]
      = GetSafeStringValue(m, key, "");
      break;
    case Tag.kArrayVal:
      dict [key]
      = GetArrayEntries(GetSafeArray(m, key));
      break;
    case Tag.kMixedArrayVal:
      dict [key]
      = GetMixedArrayEntries(GetSafeArray(m, key));
      break;
    case Tag.kMapVal:
      dict [key]
      = GetDictionary(GetSafeMap(m, key));
      break;
    }
  }
}
