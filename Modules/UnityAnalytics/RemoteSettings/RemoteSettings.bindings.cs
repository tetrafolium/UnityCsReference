...
internal static class RemoteConfigSettingsHelper
{
    ...
    internal static object GetArrayEntries(IntPtr a)
    {
        ...
        case Tag.kMapVal:
            return GetArrayEntriesType<IDictionary<string, object>>(a, size, GetArrayMapEntries);
        }
        return null;
    }
    ...
    internal static object GetMixedArrayEntries(IntPtr a)
    {
        ...
        for (long i = 0; i < size; i++)
        {
            ...
            case Tag.kIntVal:
            case Tag.kInt64Val:
                r[i] = GetSafeNumberArray(a, i);
                break;
            case Tag.kDoubleVal:
                r[i] = GetSafeArrayFloat(a, i);
                break;
            case Tag.kBoolVal:
                r[i] = GetSafeArrayBool(a, i);
                break;
            case Tag.kStringVal:
                r[i] = GetSafeArrayStringValue(a, i);
                break;
            case Tag.kArrayVal:
                r[i] = GetArrayArrayEntries(a, i);
                break;
            case Tag.kMapVal:
                r[i] = GetArrayMapEntries(a, i);
                break;
            }
        }
        return r;
    ...
    internal static void SetDictKeyType(IntPtr m, IDictionary<string, object> dict, string key, Tag tag)
    {
        ...
        case Tag.kIntVal:
        case Tag.kInt64Val:
            dict[key] = GetSafeNumber(m, key, 0);
            break;
        case Tag.kDoubleVal:
            dict[key] = GetSafeFloat(m, key, 0);
            break;
        case Tag.kBoolVal:
            dict[key] = GetSafeBool(m, key, false);
            break;
        case Tag.kStringVal:
