...
public enum AnalyticsEventPriority {
	...
	HighestPriorityEvent_NoRetryNoCaching =
		FlushQueueFlag | NoCachingFlag | NoRetryFlag
}
...
public static AnalyticsResult FlushEvents() {
	...
	return FlushArchivedEvents() ? AnalyticsResult.Ok
...
public static AnalyticsResult SetUserId(string userId) {
	if (string.IsNullOrEmpty(userId))
		throw new ArgumentException(
			      "Cannot set userId to an empty or null string");
	return SendUserInfoEvent(new UserInfo(){
...
public static AnalyticsResult SetUserGender(Gender gender) {
	return SendUserInfoEvent(new UserInfo(){
				sex = gender == Gender.Male ? "M" : gender == Gender.Female ?
				      "F" : "U"
			});
}
...
public static AnalyticsResult SetUserBirthYear(int birthYear) {
	return SendUserInfoEvent(new UserInfoBirthYear(){
...
public static AnalyticsResult Transaction(string productId, decimal amount,
                                          string currency) {
...
public static AnalyticsResult Transaction(string productId, decimal amount,
                                          ...
                                          string signature) {
	return Transaction(productId, amount, currency, receiptPurchaseData,
	                   signature, false);
}
...
public static AnalyticsResult Transaction(string productId, decimal amount,
	...
	if (string.IsNullOrEmpty(productId))
		throw new ArgumentException(
			      "Cannot set productId to an empty or null string");
	...
	return Transaction(productId, Convert.ToDouble(amount), currency,
	                   receiptPurchaseData, signature, usingIAPService);
}

public static AnalyticsResult CustomEvent(string customEventName) {
	if (string.IsNullOrEmpty(customEventName))
		throw new ArgumentException(
			      "Cannot set custom event name to an empty or null string");
	if (!IsInitialized())
		return AnalyticsResult.NotInitialized;
...
public static AnalyticsResult
CustomEvent(string customEventName, IDictionary<string, object> eventData) {
	if (string.IsNullOrEmpty(customEventName))
		throw new ArgumentException(
			      "Cannot set custom event name to an empty or null string");
	if (!IsInitialized())
...
public static AnalyticsResult EnableCustomEvent(string customEventName,
                                                bool enabled) {
	if (string.IsNullOrEmpty(customEventName))
		throw new ArgumentException(
			      "Cannot set event name to an empty or null string");
	if (!IsInitialized())
		return AnalyticsResult.NotInitialized;
...
public static AnalyticsResult
RegisterEvent(string eventName, int maxEventPerHour, int maxItems,
              string vendorKey = "", string prefix = "") {
	string n = String.Empty;
	n = Assembly.GetCallingAssembly().FullName;
	return RegisterEvent(eventName, maxEventPerHour, maxItems, vendorKey, 1,
	                     prefix, n);
}
...
RegisterEvent(string eventName, int maxEventPerHour, int maxItems,
	...
	string n = String.Empty;
	n = Assembly.GetCallingAssembly().FullName;
	return RegisterEvent(eventName, maxEventPerHour, maxItems, vendorKey, ver,
	                     prefix, n);
}

private static AnalyticsResult RegisterEvent(string eventName,
                                             int maxEventPerHour,
                                             int maxItems, string vendorKey,
	...
	if (!IsInitialized())
		return AnalyticsResult.NotInitialized;
	return RegisterEventWithLimit(eventName, maxEventPerHour, maxItems,
	                              vendorKey, ver, prefix, assemblyInfo, true);
}

public static AnalyticsResult SendEvent(string eventName, object parameters,
                                        int ver = 1, string prefix = "") {
	if (string.IsNullOrEmpty(eventName))
		throw new ArgumentException(
			      "Cannot set event name to an empty or null string");
	if (parameters == null)
...
public static AnalyticsResult SetEventEndPoint(string eventName,
                                               string endPoint, int ver = 1,
                                               string prefix = "") {
	if (string.IsNullOrEmpty(eventName))
		throw new ArgumentException(
			      "Cannot set event name to an empty or null string");
...
public static AnalyticsResult
SetEventPriority(string eventName, AnalyticsEventPriority eventPriority,
                 int ver = 1, string prefix = "") {
	if (string.IsNullOrEmpty(eventName))
		throw new ArgumentException(
			      "Cannot set event name to an empty or null string");
...
public static AnalyticsResult EnableEvent(string eventName, bool enabled,
                                          int ver = 1, string prefix = "") {
	if (string.IsNullOrEmpty(eventName))
		throw new ArgumentException(
			      "Cannot set event name to an empty or null string");
...
public static AnalyticsResult IsEventEnabled(string eventName, int ver = 1,
                                             string prefix = "") {
	if (string.IsNullOrEmpty(eventName))
		throw new ArgumentException(
			      "Cannot set event name to an empty or null string");
