...
namespace UnityEditor.Experimental.AssetImporters {
  public class SketchupMaterialDescriptionPreprocessor : AssetPostprocessor {
    ...
    void CreateFromSketchupMaterial(MaterialDescription description,
                                    Material material, AnimationClip[] clips) {
      var shader = Shader.Find("Standard");
      if (shader == null) {
        context.LogImportError(
            "SketchupMaterialDescriptionPreprocessor cannot find a shader named 'Standard'.");
        return;
      }
      ...
      if (description.TryGetProperty("IsTransparent", out floatProperty) &&
          floatProperty == 1.0f) {
