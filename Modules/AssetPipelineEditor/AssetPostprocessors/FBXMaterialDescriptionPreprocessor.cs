...
namespace UnityEditor.Experimental.AssetImporters {
  public class FBXMaterialDescriptionPreprocessor : AssetPostprocessor {
    ...
    public override int GetPostprocessOrder() { return k_Order; }
    ...
    public void OnPreprocessMaterialDescription(MaterialDescription description,
      ...
      if (lowerCaseExtension == ".fbx" || lowerCaseExtension == ".dae" ||
          lowerCaseExtension == ".obj" || lowerCaseExtension == ".blend" ||
          lowerCaseExtension == ".mb" || lowerCaseExtension == ".ma" ||
        ...
        if (IsAutodeskInteractiveMaterial(description))
          ...
          CreateFromMayaArnoldStandardSurfaceMaterial(description, material,
                                                      clips);
        else if (Is3DsMaxArnoldStandardSurfaceMaterial(description))
          CreateFrom3DsMaxArnoldStandardSurfaceMaterial(description, material,
                                                        clips);
    ...
    static bool
    IsMayaArnoldStandardSurfaceMaterial(MaterialDescription description) {
      float typeId;
    ...
    static bool
    Is3DsMaxArnoldStandardSurfaceMaterial(MaterialDescription description) {
      float classIdA;
      float classIdB;
      description.TryGetProperty("ClassIDa", out classIdA);
      description.TryGetProperty("ClassIDb", out classIdB);
    ...
    static bool IsAutodeskInteractiveMaterial(MaterialDescription description) {
      string stringValue;
      return description.TryGetProperty("renderAPI", out stringValue) &&
             stringValue == "SFX_PBS_SHADER";
    }

    void CreateFrom3DsMaxArnoldStandardSurfaceMaterial(
        MaterialDescription description, Material material,
        AnimationClip[] clips) {
      ...
      if (shader == null) {
        context.LogImportError(
            "FBXMaterialDescriptionPreprocessor cannot find a shader named 'Autodesk Interactive'.");
        return;
      }
      ...
      if (description.TryGetProperty("transmission", out floatProperty) &&
          floatProperty > 0.0f) {
      ...
      if (description.TryGetProperty("base_color.shader",
                                     out textureProperty)) {
      ...
      if (description.TryGetProperty("emission", out floatProperty) &&
        ...
        if (description.TryGetProperty("emission_color.shader",
                                       out textureProperty)) {
        ...
        } else if (description.TryGetProperty("emission_color",
                                              out vectorProperty)) {
    ...
    void
    CreateFromMayaArnoldStandardSurfaceMaterial(MaterialDescription description,
                                                Material material,
                                                AnimationClip[] clips) {
      var shader = Shader.Find("Autodesk Interactive");
      if (shader == null) {
      ...
      if (description.HasAnimationCurve("transmission")) {
        for (int i = 0; i < clips.Length; i++) {
          ...
          description.TryGetAnimationCurve(clips[i].name, "transmission",
                                           out opacityCurve);
          clips [i]
              .SetCurve("", typeof(Material), "_Color.a", opacityCurve);

          if (!description.HasAnimationCurveInClip(clips[i].name,
                                                   "baseColor.x")) {
            Vector4 diffuseColor;
            description.TryGetProperty("baseColor", out diffuseColor);
            clips [i]
                .SetCurve("", typeof(Material), "_Color.r",
                          AnimationCurve.Constant(0.0f, 1.0f, diffuseColor.x));
            clips [i]
                .SetCurve("", typeof(Material), "_Color.g",
                          AnimationCurve.Constant(0.0f, 1.0f, diffuseColor.y));
            clips [i]
                .SetCurve("", typeof(Material), "_Color.b",
      ...
      if (description.TryGetProperty("specularRoughness",
                                     out textureProperty)) {
        SetMaterialTextureProperty("_SpecGlossMap", material, textureProperty);
        material.EnableKeyword("_SPECGLOSSMAP");
      } else if (description.TryGetProperty("specularRoughness",
                                            out floatProperty)) {
        material.SetFloat("_Glossiness", floatProperty);
      ...
      if (description.TryGetProperty("emission", out floatProperty) &&
        ...
        if (description.TryGetProperty("emissionColor", out textureProperty)) {
          ...
          SetMaterialTextureProperty("_EmissionMap", material, textureProperty);
          material.SetColor("_EmissionColor", emissiveColor);
        } else if (description.TryGetProperty("emissionColor",
                                              out vectorProperty)) {
          emissiveColor = vectorProperty * floatProperty;
          material.SetColor("_EmissionColor", emissiveColor);
        }
      ...
      if (description.HasAnimationCurve("emissionColor.x")) {
        if (description.HasAnimationCurve("emission")) {
          ...
          for (int i = 0; i < clips.Length; i++) {
            ...
            description.TryGetAnimationCurve(clips[i].name, "emission",
                                             out intensityCurve);

            description.TryGetAnimationCurve(clips[i].name, "emissionColor.x",
                                             out curve);
            MultiplyCurves(curve, intensityCurve);
            clips [i]
                .SetCurve("", typeof(Material), "_EmissionColor.r", curve);

            description.TryGetAnimationCurve(clips[i].name, "emissionColor.y",
                                             out curve);
            MultiplyCurves(curve, intensityCurve);
            clips [i]
                .SetCurve("", typeof(Material), "_EmissionColor.g", curve);

            description.TryGetAnimationCurve(clips[i].name, "emissionColor.z",
                                             out curve);
            MultiplyCurves(curve, intensityCurve);
            clips [i]
                .SetCurve("", typeof(Material), "_EmissionColor.b", curve);
          }
        } else {
        ...
        for (int i = 0; i < clips.Length; i++) {
          ...
          ConvertAndCopyKeys(curveR, curve,
                             value =>
                                 ConvertFloatMultiply(emissiveColor.x, value));
          clips [i]
              .SetCurve("", typeof(Material), "_EmissionColor.r", curveR);

          AnimationCurve curveG = new AnimationCurve();
          ConvertAndCopyKeys(curveG, curve,
                             value =>
                                 ConvertFloatMultiply(emissiveColor.y, value));
          clips [i]
              .SetCurve("", typeof(Material), "_EmissionColor.g", curveG);

          AnimationCurve curveB = new AnimationCurve();
          ConvertAndCopyKeys(curveB, curve,
                             value =>
                                 ConvertFloatMultiply(emissiveColor.z, value));
          clips [i]
              .SetCurve("", typeof(Material), "_EmissionColor.b", curveB);
        }
      }
    }

    void CreateFrom3DsMaxPhysicalMaterial(MaterialDescription description,
                                          Material material,
                                          AnimationClip[] clips) {
      var shader = Shader.Find("Autodesk Interactive");
      if (shader == null) {
      ...
      if (description.TryGetProperty("base_color_map", out textureProperty)) {
        ...
        material.SetColor("_Color",
                          new Color(1.0f, 1.0f, 1.0f, 1.0f - transparency));
      } else if (description.TryGetProperty("base_color", out vectorProperty)) {
    ...
    void CreateFromAutodeskInteractiveMaterial(MaterialDescription description,
                                               Material material,
                                               AnimationClip[] clips) {
      var shader = Shader.Find("Autodesk Interactive");
      if (shader == null) {
      ...
      for (int i = 0; i < clips.Length; i++) {
        if (description.HasAnimationCurveInClip(clips[i].name, "uv_scale.x") ||
          ...
          if (description.TryGetAnimationCurve(clips[i].name, "uv_scale.x",
                                               out curve))
            clips [i]
                .SetCurve("", typeof(Material), "_MainTex_ST.x", curve);
          else
            clips [i]
                .SetCurve("", typeof(Material), "_MainTex_ST.x",
                          AnimationCurve.Constant(0.0f, 1.0f, 1.0f));

          if (description.TryGetAnimationCurve(clips[i].name, "uv_scale.y",
                                               out curve))
            clips [i]
                .SetCurve("", typeof(Material), "_MainTex_ST.y", curve);
          else
            clips [i]
                .SetCurve("", typeof(Material), "_MainTex_ST.y",
                          AnimationCurve.Constant(0.0f, 1.0f, 1.0f));

          if (description.TryGetAnimationCurve(clips[i].name, "uv_offset.x",
                                               out curve))
            clips [i]
                .SetCurve("", typeof(Material), "_MainTex_ST.z", curve);
          else
            clips [i]
                .SetCurve("", typeof(Material), "_MainTex_ST.z",
                          AnimationCurve.Constant(0.0f, 1.0f, 0.0f));

          if (description.TryGetAnimationCurve(clips[i].name, "uv_offset.y",
                                               out curve)) {
            ConvertKeys(curve, ConvertFloatNegate);
            clips [i]
                .SetCurve("", typeof(Material), "_MainTex_ST.w", curve);
          } else
            clips [i]
                .SetCurve("", typeof(Material), "_MainTex_ST.w",
                          AnimationCurve.Constant(0.0f, 1.0f, 0.0f));
        }
      }
      ...
      if (alphaThreshold > 0.0f ||
          description.HasAnimationCurve("mask_threshold")) {
        ...
        material.renderQueue =
            (int) UnityEngine.Rendering.RenderQueue.Transparent;
      } else if (opacity < 1.0f || description.HasAnimationCurve("opacity") ||
                 (description.TryGetProperty("use_opacity_map",
                                             out floatProperty) &&
                  floatProperty == 1.0f)) {
        material.SetInt("_Mode", (int) StandardShaderGUI.BlendMode.Transparent);
        material.SetOverrideTag("RenderType", "Transparent");
      ...
      if (description.TryGetProperty("use_color_map", out floatProperty) &&
              floatProperty == 1.0f ||
          description.TryGetProperty("use_opacity_map", out floatProperty) &&
              floatProperty == 1.0f) {
      ...
      if (description.HasAnimationCurve("opacity")) {
        for (int i = 0; i < clips.Length; i++) {
          AnimationCurve opacityCurve;
          description.TryGetAnimationCurve(clips[i].name, "opacity",
                                           out opacityCurve);
          clips [i]
              .SetCurve("", typeof(Material), "_Color.a", opacityCurve);
      ...
      if (description.TryGetProperty("use_normal_map", out floatProperty) &&
          floatProperty == 1.0f) {
      ...
      if (description.TryGetProperty("use_metallic_map", out floatProperty) &&
          floatProperty == 1.0f) {
        if (description.TryGetProperty("TEX_metallic_map",
                                       out textureProperty)) {
      ...
      if (description.TryGetProperty("use_roughness_map", out floatProperty) &&
          floatProperty == 1.0f) {
        if (description.TryGetProperty("TEX_roughness_map",
                                       out textureProperty)) {
      ...
      if (description.TryGetProperty("use_emissive_map", out floatProperty) &&
          floatProperty == 1.0f) {
        if (description.TryGetProperty("TEX_emissive_map",
                                       out textureProperty)) {
          Vector4 emissiveColor = new Vector4(1.0f, 1.0f, 1.0f, 1.0f);
          if (description.TryGetProperty("emissive_intensity",
                                         out floatProperty)) {
            emissiveColor *= floatProperty;
          }
          material.SetColor("_EmissionColor", emissiveColor);
        ...
        if (description.HasAnimationCurve("emissive_intensity")) {
          ...
          for (int i = 0; i < clips.Length; i++) {
            description.TryGetAnimationCurve(clips[i].name,
                                             "emissive_intensity", out curve);
            // remap emissive intensity to emission color
            clips [i]
                .SetCurve("", typeof(Material), "_EmissionColor.r", curve);
            clips [i]
        ...
        if (vectorProperty.x > 0.0f || vectorProperty.y > 0.0f ||
            vectorProperty.z > 0.0f) {
          material.globalIlluminationFlags |=
              MaterialGlobalIlluminationFlags.RealtimeEmissive;
          material.EnableKeyword("_EMISSION");
        }
        ...
        if (description.HasAnimationCurve("emissive.x")) {
          if (description.HasAnimationCurve("emissive_intensity")) {
            ...
            for (int i = 0; i < clips.Length; i++) {
              ...
              description.TryGetAnimationCurve(
                  clips[i].name, "emissive_intensity", out intensityCurve);

              description.TryGetAnimationCurve(clips[i].name, "emissive.x",
                                               out curve);
              MultiplyCurves(curve, intensityCurve);
              clips [i]
                  .SetCurve("", typeof(Material), "_EmissionColor.r", curve);

              description.TryGetAnimationCurve(clips[i].name, "emissive.y",
                                               out curve);
              MultiplyCurves(curve, intensityCurve);
              clips [i]
                  .SetCurve("", typeof(Material), "_EmissionColor.g", curve);

              description.TryGetAnimationCurve(clips[i].name, "emissive.z",
                                               out curve);
              MultiplyCurves(curve, intensityCurve);
              clips [i]
                  .SetCurve("", typeof(Material), "_EmissionColor.b", curve);
          ...
          for (int i = 0; i < clips.Length; i++) {
            ...
            ConvertAndCopyKeys(
                curveR, curve,
                value => ConvertFloatMultiply(emissiveColor.x, value));
            clips [i]
                .SetCurve("", typeof(Material), "_EmissionColor.r", curveR);
            ...
            ConvertAndCopyKeys(
                curveG, curve,
                value => ConvertFloatMultiply(emissiveColor.y, value));
            clips [i]
                .SetCurve("", typeof(Material), "_EmissionColor.g", curveG);
            ...
            ConvertAndCopyKeys(
                curveB, curve,
                value => ConvertFloatMultiply(emissiveColor.z, value));
            clips [i]
                .SetCurve("", typeof(Material), "_EmissionColor.b", curveB);
          }
        }
      ...
      if (description.TryGetProperty("use_ao_map", out floatProperty) &&
          floatProperty == 1.0f) {
    ...
    void CreateFromStandardMaterial(MaterialDescription description,
                                    Material material, AnimationClip[] clips) {
      var shader = Shader.Find("Standard");
      if (shader == null) {
        context.LogImportError(
            "FBXMaterialDescriptionPreprocessor cannot find a shader named 'Standard'.");
        return;
      }
      material.shader = shader;
      ...
      if (!description.TryGetProperty("Opacity", out opacity)) {
        if (description.TryGetProperty("TransparencyFactor",
                                       out transparencyFactor)) {
          opacity =
              transparencyFactor == 1.0f ? 1.0f : 1.0f - transparencyFactor;
        }
        if (opacity == 1.0f && description.TryGetProperty("TransparentColor",
                                                          out vectorProperty)) {
          opacity = vectorProperty.x == 1.0f ? 1.0f : 1.0f - vectorProperty.x;
        }
      }
      if (opacity < 1.0f ||
          (opacity == 1.0f && description.TryGetProperty(
                                  "TransparentColor", out textureProperty))) {
        isTransparent = true;
      ...
      if (description.TryGetProperty("DiffuseColor", out textureProperty)) {
        ...
        SetMaterialTextureProperty("_MainTex", material, textureProperty);
        material.SetColor("_Color", PlayerSettings.colorSpace ==
                                        ColorSpace.Gamma ? diffuseColor.gamma
                          : diffuseColor);
      } else if (description.TryGetProperty("DiffuseColor",
                                            out vectorProperty)) {
      ...
      } else if (description.TryGetProperty("EmissiveColor",
                                            out vectorProperty) &&
      ...
      if (PlayerSettings.colorSpace == ColorSpace.Gamma)
        RemapAndTransformColorCurves(description, clips, "DiffuseColor",
                                     "_Color", ConvertFloatLinearToGamma);
      else
    ...
    static void RemapTransparencyCurves(MaterialDescription description,
                                        AnimationClip[] clips) {
      // For some reason, Opacity is never animated, we have to use
      // TransparencyFactor and TransparentColor
      for (int i = 0; i < clips.Length; i++) {
        bool foundTransparencyCurve = false;
        AnimationCurve curve;
        if (description.TryGetAnimationCurve(clips[i].name,
                                             "TransparencyFactor", out curve)) {
          ConvertKeys(curve, ConvertFloatOneMinus);
          clips [i]
              .SetCurve("", typeof(Material), "_Color.a", curve);
          foundTransparencyCurve = true;
        } else if (description.TryGetAnimationCurve(
                       clips[i].name, "TransparentColor.x", out curve)) {
          ConvertKeys(curve, ConvertFloatOneMinus);
          clips [i]
              .SetCurve("", typeof(Material), "_Color.a", curve);
          foundTransparencyCurve = true;
        ...
        if (foundTransparencyCurve && !description.HasAnimationCurveInClip(
                                          clips[i].name, "DiffuseColor")) {
          Vector4 diffuseColor;
    ...
    static void RemapCurve(MaterialDescription description,
                           AnimationClip[] clips, string originalPropertyName,
                           string newPropertyName) {
      AnimationCurve curve;
      for (int i = 0; i < clips.Length; i++) {
        if (description.TryGetAnimationCurve(clips[i].name,
                                             originalPropertyName, out curve)) {
          clips [i]
              .SetCurve("", typeof(Material), newPropertyName, curve);
        }
      }
    }

    static void RemapColorCurves(MaterialDescription description,
                                 AnimationClip[] clips,
                                 string originalPropertyName,
                                 string newPropertyName) {
      AnimationCurve curve;
      for (int i = 0; i < clips.Length; i++) {
        if (description.TryGetAnimationCurve(
                clips[i].name, originalPropertyName + ".x", out curve)) {
          clips [i]
              .SetCurve("", typeof(Material), newPropertyName + ".r", curve);
        }

        if (description.TryGetAnimationCurve(
                clips[i].name, originalPropertyName + ".y", out curve)) {
          clips [i]
              .SetCurve("", typeof(Material), newPropertyName + ".g", curve);
        }

        if (description.TryGetAnimationCurve(
                clips[i].name, originalPropertyName + ".z", out curve)) {
          clips [i]
              .SetCurve("", typeof(Material), newPropertyName + ".b", curve);
        }
      }
    }

    static void RemapAndTransformColorCurves(
        MaterialDescription description, AnimationClip[] clips,
    ...
    static float ConvertFloatOneMinus(float value) { return 1.0f - value; }

    static float ConvertFloatNegate(float value) { return -value; }

    static void MultiplyCurves(AnimationCurve curve,
                               AnimationCurve curveMultiplier) {
    ...
    static void
    ConvertAndCopyKeys(AnimationCurve curveDest, AnimationCurve curveSource,
                       System.Func<float, float> convertionDelegate) {
      for (int i = 0; i < curveSource.keys.Length; i++) {
        var sourceKey = curveSource.keys[i];
        curveDest.AddKey(new Keyframe(sourceKey.time,
                                      convertionDelegate(sourceKey.value),
                                      sourceKey.inTangent, sourceKey.outTangent,
                                      sourceKey.inWeight, sourceKey.outWeight));
      }
    }

    static void ConvertKeys(AnimationCurve curve,
                            System.Func<float, float> convertionDelegate) {
      Keyframe[] keyframes = curve.keys;
      for (int i = 0; i < keyframes.Length; i++) {
    ...
    static void
    SetMaterialTextureProperty(string propertyName, Material material,
                               TexturePropertyDescription textureProperty) {
