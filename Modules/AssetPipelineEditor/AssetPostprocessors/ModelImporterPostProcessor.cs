...
static void OnPostprocessAllAssets(string[] importedAssets,
	...
	if (UnityEditor.Experimental.AssetDatabaseExperimental.ActiveOnDemandMode !=
	    UnityEditor.Experimental.AssetDatabaseExperimental.OnDemandMode
	    .Off ||
	    UnityEditor.Experimental.AssetDatabaseExperimental
	    .VirtualizationEnabled) {
		// This PostProcessAllAssets will forcefully import everything in
		// on-demand mode We need to find a better way of filtering on asset type
	...
	try {
		foreach (var assetPath in importedAssets) {
			...
			if (isMaterialAsset == true || importer is ModelImporter ||
			    importer is SpeedTreeImporter) {
				if (!AssetDatabase.IsMainAssetAtPathLoaded(assetPath)) {
					loadedAssets.AddRange(
						AssetDatabase.LoadAllAssetsAtPath(assetPath).Where(
							o => !(o is GameObject || o is Component ||
							       o is MonoBehaviour || o is ScriptableObject)));
				}

				var embeddedMaterials =
					AssetDatabase.LoadAllAssetsAtPath(assetPath).OfType<Material>();
		...
		if (oneFound && !AskedForBumpMap) {
			...
			// We cannot open the BumpMapTexturesWindow here because the Editor
			// Layout may not have been loaded yet and will destroy the window when
			// doing so. So lets wait for the first frame to open it.
