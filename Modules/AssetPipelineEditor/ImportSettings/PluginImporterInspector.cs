...
internal class PluginImporterInspector : AssetImporterEditor {
  ...
  // Unlike other platforms, Standalone targets (OSX, Windows, Linux) are
  // bundled as one target ("Standalone") in platform settings That's why value
  // m_CompatibleWithPlatform for <OSX, Windows, Linux> is controlled from two
  // places:
  // * ShowGeneralOptions (when clicking on 'Standalone' toggle, it enables or
  // disables all those targets)
  // * DesktopPluginImporterExtension (where it's possible to individually
  // enable or disable specific Standalone target)

  public static readonly GUIContent defineConstraints =
      EditorGUIUtility.TrTextContent("Define Constraints");
  public static readonly GUIContent loadSettings =
      EditorGUIUtility.TrTextContent("Plugin load settings");
  public static readonly GUIContent preload = EditorGUIUtility.TrTextContent(
      "Load on startup",
      "Always load plugin during startup instead of on-demand");
  ...
  private Compatibility[] m_CompatibleWithPlatform =
      new Compatibility[GetPlatformGroupArraySize()];
  private List<DefineConstraint> m_DefineConstraintState =
      new List<DefineConstraint>();
  ...
  internal IPluginImporterExtension[] additionalExtensions {
    get {
      return new IPluginImporterExtension[]{editorExtension, desktopExtension};
    }
  }
  ...
  private static bool IgnorePlatform(BuildTarget platform) { return false; }

  private bool IsEditingPlatformSettingsSupported() {
    // We don't support editing platform settings when multiple objects are
    // selected
  ...
  private static int GetPlatformGroupArraySize() {
    int max = 0;
    foreach (BuildTarget platform in typeof(BuildTarget)
                 .EnumGetNonObsoleteValues())
      if (max < (int) platform + 1)
        max = (int) platform + 1;
    return max;
  }
  ...
  // Used by extensions, for ex., Standalone where we have options for
  // enabling/disabling platform in platform specific extensions
  ...
  internal void SetPlatformCompatibility(string platformName, bool compatible) {
    SetPlatformCompatibility(platformName, compatible ? Compatibility.Compatible
                             : Compatibility.NotCompatible);
  }

  internal void SetPlatformCompatibility(string platformName,
  ...
  private static List<BuildTarget> GetValidBuildTargets() {
    ...
    foreach (BuildTarget platform in typeof(BuildTarget)
                 .EnumGetNonObsoleteValues()) {
      // We have some special enums with negative values which are not actual
      ...
      // Ignore platforms which don't have module extensions loaded, accept
      // standalone targets by default, as they don't have extensions
  ...
  private BuildPlatform[] GetBuildPlayerValidPlatforms() {
    List<BuildPlatform> validPlatforms =
        BuildPlatforms.instance.GetValidPlatforms();
    List<BuildPlatform> filtered = new List<BuildPlatform>();

    if (m_CompatibleWithEditor > Compatibility.NotCompatible) {
      BuildPlatform editorPlatform = new BuildPlatform(
          "Editor settings", "Editor Settings", "BuildSettings.Editor",
          BuildTargetGroup.Unknown, BuildTarget.NoTarget, true);
    ...
    foreach (BuildPlatform bp in validPlatforms) {
      ...
      if (bp.targetGroup == BuildTargetGroup.Standalone) {
        ...
        if (m_CompatibleWithPlatform[(int) bp.defaultTarget] <
            Compatibility.Compatible)
          continue;

        IPluginImporterExtension extension =
            ModuleManager.GetPluginImporterExtension(bp.targetGroup);
        if (extension == null)
          continue;
      }
  ...
  private void ResetCompatability(ref Compatibility value,
                                  Func<PluginImporter, bool> getComptability) {
    value = getComptability(importer) ? Compatibility.Compatible
                                      : Compatibility.NotCompatible;
    foreach (var imp in importers) {
      if (value != (getComptability(imp) ? Compatibility.Compatible
                                         : Compatibility.NotCompatible)) {
  ...
  protected override void ResetValues() {
    ...
    foreach (var pluginImporter in importers) {
      var importerDefineConstraints =
          pluginImporter.DefineConstraints.Take(minSizeOfDefines).ToList();

      for (var i = 0; i < importerDefineConstraints.Count; i++) {
        var importerDefineConstraint = importerDefineConstraints[i];
        try {
          var symbolName =
              importerDefineConstraint.StartsWith(DefineConstraintsHelper.Not)
                  ? importerDefineConstraint.Substring(1)
                  : importerDefineConstraint;
          if (!SymbolNameRestrictions.IsValid(symbolName)) {
            throw new PrecompiledAssemblyException(
                $"Invalid define constraint {symbolName}", symbolName);
          }

          Compatibility mixedValue =
              importerDefineConstraints[i] !=
              baseImporterDefineConstraints[i] ? Compatibility
    ...
    ResetCompatability(ref m_CompatibleWithAnyPlatform,
                       (imp => imp.GetCompatibleWithAnyPlatform()));
    ResetCompatability(ref m_CompatibleWithEditor,
                       (imp => imp.GetCompatibleWithEditor()));
    ResetCompatability(ref m_AutoReferenced,
                       (imp => !imp.IsExplicitlyReferenced));
    ...
    if (m_CompatibleWithAnyPlatform < Compatibility.Compatible) {
      ...
      foreach (BuildTarget platform in GetValidBuildTargets()) {
        ResetCompatability(ref m_CompatibleWithPlatform[(int) platform],
                           (imp => imp.GetCompatibleWithPlatform(platform)));
      }
    } else {
      ResetCompatability(ref m_CompatibleWithEditor,
                         (imp => !imp.GetExcludeEditorFromAnyPlatform()));

      foreach (BuildTarget platform in GetValidBuildTargets()) {
        ResetCompatability(ref m_CompatibleWithPlatform[(int) platform],
                           (imp => !imp.GetExcludeFromAnyPlatform(platform)));
      }
    }
    ...
    foreach (BuildTarget platform in GetValidBuildTargets()) {
      IPluginImporterExtension extension =
          ModuleManager.GetPluginImporterExtension(platform);
      if (extension != null)
        extension.ResetValues(this);
    }
  }
  ...
  protected override void Apply() {
    var constraints =
        m_DefineConstraintState.Where(x => x.displayValue > Compatibility.Mixed)
            .Select(x => x.name)
            .ToArray();
    foreach (var imp in importers) {
      ...
      if (m_CompatibleWithAnyPlatform > Compatibility.Mixed)
        imp.SetCompatibleWithAnyPlatform(m_CompatibleWithAnyPlatform ==
                                         Compatibility.Compatible);
      if (m_CompatibleWithEditor > Compatibility.Mixed)
        imp.SetCompatibleWithEditor(m_CompatibleWithEditor ==
                                    Compatibility.Compatible);

      if (m_AutoReferenced > Compatibility.Mixed)
        imp.IsExplicitlyReferenced =
            m_AutoReferenced != Compatibility.Compatible;

      if (m_ValidateReferences > Compatibility.Mixed)
        imp.ValidateReferences =
            m_ValidateReferences == Compatibility.Compatible;

      foreach (BuildTarget platform in GetValidBuildTargets()) {
        if (m_CompatibleWithPlatform[(int) platform] > Compatibility.Mixed)
          imp.SetCompatibleWithPlatform(
              platform, m_CompatibleWithPlatform[(int) platform] ==
                            Compatibility.Compatible);
      }

      if (m_CompatibleWithEditor > Compatibility.Mixed)
        imp.SetExcludeEditorFromAnyPlatform(m_CompatibleWithEditor ==
                                            Compatibility.NotCompatible);

      foreach (BuildTarget platform in GetValidBuildTargets()) {
        if (m_CompatibleWithPlatform[(int) platform] > Compatibility.Mixed)
          imp.SetExcludeFromAnyPlatform(
              platform, m_CompatibleWithPlatform[(int) platform] ==
                            Compatibility.NotCompatible);
      }
  ...
  public override void OnEnable() {
    ...
    m_DefineConstraints =
        new ReorderableList(m_DefineConstraintState, typeof(DefineConstraint),
    ...
    if (this.importer) {
      ...
      m_PluginInformation["Type"] =
          importer.isNativePlugin ? "Native" : "Managed";
      if (!importer.isNativePlugin) {
        string info;
        switch (importer.dllType) {
        case DllType.ManagedNET35:
          info = "Targets .NET 3.5";
          break;
        case DllType.ManagedNET40:
          info = "Targets .NET 4.x";
          break;
        case DllType.UnknownManaged:
          info = "Targets Unknown .NET";
          break;
  ...
  private void DrawDefineConstraintListElement(Rect rect, int index,
    ...
    var textFieldValue =
        EditorGUI.TextField(rect, mixed ? L10n.Tr("(Multiple Values)")
                            : label);
  ...
  private Compatibility ToggleWithMixedValue(Compatibility value,
                                             string title) {
    ...
    bool newBoolValue =
        EditorGUILayout.Toggle(EditorGUIUtility.TrTempContent(title),
                               value == Compatibility.Compatible);
    if (EditorGUI.EndChangeCheck())
      return newBoolValue ? Compatibility.Compatible
          : Compatibility.NotCompatible;
  ...
  private void ShowPlatforms(ValueSwitcher switcher) {
    // Note: We use m_CompatibleWithEditor & m_CompatibleWithPlatform for
    // displaying both Include & Exclude platforms
    m_CompatibleWithEditor = switcher(
        ToggleWithMixedValue(switcher(m_CompatibleWithEditor), "Editor"));
    EditorGUI.BeginChangeCheck();
    Compatibility value =
        ToggleWithMixedValue(switcher(compatibleWithStandalone), "Standalone");
    // We only want to change compatibleWithStandalone value, if user actually
    // clicks on it
    if (EditorGUI.EndChangeCheck()) {
    ...
    foreach (BuildTarget platform in GetValidBuildTargets()) {
      ...
      m_CompatibleWithPlatform [(int) platform]
      = switcher(ToggleWithMixedValue(
          switcher(m_CompatibleWithPlatform[(int) platform]),
          platform.ToString()));
    }
  }

  private Compatibility SwitchToInclude(Compatibility value) { return value; }

  private Compatibility SwitchToExclude(Compatibility value) {
    switch (value) {
    case Compatibility.Mixed:
      return Compatibility.Mixed;
  ...
  private void ShowGeneralOptions() {
    ...
    m_CompatibleWithAnyPlatform =
        ToggleWithMixedValue(m_CompatibleWithAnyPlatform, "Any Platform");
  ...
  private void ShowPlatformSettings() {
    ...
    if (validPlatforms.Length > 0) {
      ...
      if (validPlatforms[platformIndex].name ==
          BuildPipeline.GetEditorTargetName()) {
        ShowEditorSettings();
      } else {
        BuildTargetGroup targetGroup =
            validPlatforms[platformIndex].targetGroup;
        if (targetGroup == BuildTargetGroup.Standalone) {
          desktopExtension.OnPlatformSettingsGUI(this);
        } else {
          IPluginImporterExtension extension =
              ModuleManager.GetPluginImporterExtension(targetGroup);
          if (extension != null)
  ...
  private void ShowReferenceOptions() {
    GUILayout.Label(EditorGUIUtility.TrTempContent("General"),
                    EditorStyles.boldLabel);
    EditorGUILayout.BeginVertical(GUI.skin.box);
    EditorGUI.BeginChangeCheck();
    m_AutoReferenced = ToggleWithMixedValue(m_AutoReferenced, "Auto Reference");
    m_ValidateReferences =
        ToggleWithMixedValue(m_ValidateReferences, "Validate References");
  ...
  public override void OnInspectorGUI() {
    ...
    using(new EditorGUI.DisabledScope(false)) {
      var isManagedPlugin =
          importers.All(x => x.dllType == DllType.ManagedNET35 ||
                             x.dllType == DllType.ManagedNET40);
      if (isManagedPlugin) {
    ...
    foreach (var prop in m_PluginInformation) {
      ...
      EditorGUILayout.SelectableLabel(
          prop.Value, GUILayout.Height(EditorGUI.kSingleLineHeight));
    ...
    // Once Mono loads a native plugin, it never releases a handle, thus plugin
    // is never unloaded.
    if (importer.isNativePlugin)
