...
static class Style {
public static readonly GUIContent[] kSampleRateStrings =
	new[] {"8,000 Hz",  "11,025 Hz", "22,050 Hz", "44,100 Hz",
	       "48,000 Hz", "96,000 Hz", "192,000 Hz"}
.Select(s => new GUIContent(s))
.ToArray();
public static readonly int[] kSampleRateValues = {
	8000, 11025, 22050, 44100, 48000, 96000, 192000
};

public static GUIContent LoadType =
	EditorGUIUtility.TrTextContent("Load Type");
public static GUIContent PreloadAudioData =
	EditorGUIUtility.TrTextContent("Preload Audio Data*");
public static GUIContent CompressionFormat =
	EditorGUIUtility.TrTextContent("Compression Format");
public static GUIContent Quality =
	EditorGUIUtility.TrTextContent("Quality");
public static GUIContent SampleRateSetting =
	EditorGUIUtility.TrTextContent("Sample Rate Setting");
public static GUIContent SampleRate =
	EditorGUIUtility.TrTextContent("Sample Rate");
public static GUIContent DefaultPlatform =
	EditorGUIUtility.TrTextContent("Default");
public static GUIContent SharedSettingInformation =
...
protected override void InitializeExtraDataInstance(Object extraData,
	...
	if (settings != null && audioImporter != null) {
		// We need to sort them so every extraDataTarget have them ordered
		// correctly and we can use serializedProperties.
		var validPlatforms = BuildPlatforms.instance.GetValidPlatforms().OrderBy(
			platform => platform.targetGroup);
		settings.sampleSettingOverrides =
			new List<AudioImporterPlatformSettings>(validPlatforms.Count());
		foreach (BuildPlatform platform in validPlatforms) {
			...
			settings.sampleSettingOverrides.Add(new AudioImporterPlatformSettings(){
						platform = platform.targetGroup,
						isOverridden =
							audioImporter.ContainsSampleSettingsOverride(groupName),
						settings = sample
					});
		}
	}
		...
		if (settings != null && audioImporter != null) {
			foreach (var setting in settings.sampleSettingOverrides) {
				if (setting.isOverridden) {
					audioImporter.SetOverrideSampleSettings(setting.platform.ToString(),
					                                        setting.settings);
				} else if (audioImporter.ContainsSampleSettingsOverride(
						   setting.platform.ToString())) {
...
public bool CurrentPlatformHasAutoTranslatedCompression() {
	...
	foreach (AudioImporter importer in GetAllAudioImporterTargets()) {
		AudioCompressionFormat defaultCompressionFormat =
			importer.defaultSampleSettings.compressionFormat;
		// Because we only want to query if the importer does not have an
		// override.
		if (!importer.Internal_ContainsSampleSettingsOverride(targetGroup)) {
			AudioImporterSampleSettings overrideSettings =
				importer.Internal_GetOverrideSampleSettings(targetGroup);
			AudioCompressionFormat overrideCompressionFormat =
				overrideSettings.compressionFormat;

			// If we dont have an override, but the translated compression format is
			// different, this means we have audio translate happening.
...
public override void OnEnable() {
	...
	m_DefaultSampleSettings =
		serializedObject.FindProperty("m_DefaultSettings");
...
public override void OnInspectorGUI() {
	...
	EditorGUILayout.HelpBox(
		"Original Size: \t" + EditorUtility.FormatBytes(origSize) +
		"\nImported Size: \t" + EditorUtility.FormatBytes(compSize) + "\n" +
		"Ratio: \t\t" +
		(100.0f * (float) compSize / (float) origSize)
		.ToString("0.00", CultureInfo.InvariantCulture.NumberFormat) +
		"%",
		MessageType.Info);
	...
	if (CurrentPlatformHasAutoTranslatedCompression()) {
		GUILayout.Space(10);
		EditorGUILayout.HelpBox(
			"The selection contains different compression formats to the default settings for the current build platform.",
			MessageType.Info);
	}
	...
	if (CurrentSelectionContainsHardwareSounds()) {
		GUILayout.Space(10);
		EditorGUILayout.HelpBox(
			"The selection contains sounds that are decompressed in hardware. Advanced mixing is not available for these sounds.",
			MessageType.Info);
	}
...
private List<AudioCompressionFormat>
GetFormatsForPlatform(BuildTargetGroup platform) {
	List<AudioCompressionFormat> allowedFormats =
		new List<AudioCompressionFormat>();
...
private void
OnSampleSettingGUI(BuildTargetGroup platform,
                   SerializedProperty audioImporterSampleSettings,
                   bool selectionContainsTrackerFile) {
	// Load Type
	var loadTypeProperty =
		audioImporterSampleSettings.FindPropertyRelative("loadType");
	using (var horizontal = new EditorGUILayout.HorizontalScope()) {
		using (var propertyScope = new EditorGUI.PropertyScope(
			       horizontal.rect, Style.LoadType, loadTypeProperty)) {
			...
			using (var changed = new EditorGUI.ChangeCheckScope()) {
				var newValue = (AudioClipLoadType) EditorGUILayout.EnumPopup(
					...
					(AudioClipLoadType) loadTypeProperty.intValue);
				if (changed.changed) {
	...
	// Preload Audio Data
	// If the loadtype is streaming on the selected platform, gray out the
	// "Preload Audio Data" option and show the checkbox as unchecked.
	bool disablePreloadAudioDataOption =
		(AudioClipLoadType)
		loadTypeProperty.intValue == AudioClipLoadType.Streaming;
	using (new EditorGUI.DisabledScope(disablePreloadAudioDataOption)) {
		if (disablePreloadAudioDataOption)
			...
			EditorGUILayout.PropertyField(m_PreloadAudioData,
			                              Style.PreloadAudioData);
	}

	if (!selectionContainsTrackerFile) {
		// Compression format
		var compressionFormatProperty =
			audioImporterSampleSettings.FindPropertyRelative("compressionFormat");
		var allowedFormats = GetFormatsForPlatform(platform);
		using (var horizontal = new EditorGUILayout.HorizontalScope()) {
			using (var propertyScope = new EditorGUI.PropertyScope(
				       horizontal.rect, Style.CompressionFormat,
				       compressionFormatProperty)) {
				EditorGUI.showMixedValue =
					compressionFormatProperty.hasMultipleDifferentValues;
				using (var changed = new EditorGUI.ChangeCheckScope()) {
					var newValue = (AudioCompressionFormat) EditorGUILayout.IntPopup(
						propertyScope.content, compressionFormatProperty.intValue,
						allowedFormats.Select(a => new GUIContent(a.ToString()))
						.ToArray(),
		...
		// Quality
		if (!compressionFormatProperty.hasMultipleDifferentValues &&
		    CompressionFormatHasQuality((AudioCompressionFormat)
		                                compressionFormatProperty.intValue)) {
			using (var horizontal = new EditorGUILayout.HorizontalScope()) {
				var property =
					audioImporterSampleSettings.FindPropertyRelative("quality");
				using (var propertyScope = new EditorGUI.PropertyScope(
					       horizontal.rect, Style.Quality, property)) {
					EditorGUI.showMixedValue = property.hasMultipleDifferentValues;
					using (var changed = new EditorGUI.ChangeCheckScope()) {
						var newValue = EditorGUILayout.IntSlider(
							propertyScope.content,
							(int) Mathf.Clamp(property.floatValue * 100.0f + 0.5f, 1.0f,
							                  100.0f),
							1, 100);
						if (changed.changed) {
		...
		if (platform != BuildTargetGroup.WebGL) {
			...
			var sampleRateSettingProperty =
				audioImporterSampleSettings.FindPropertyRelative(
					"sampleRateSetting");
			using (var horizontal = new EditorGUILayout.HorizontalScope()) {
				using (var propertyScope = new EditorGUI.PropertyScope(
					       horizontal.rect, Style.SampleRateSetting,
					       sampleRateSettingProperty)) {
					EditorGUI.showMixedValue =
						sampleRateSettingProperty.hasMultipleDifferentValues;
					using (var changed = new EditorGUI.ChangeCheckScope()) {
						var newValue = (AudioSampleRateSetting) EditorGUILayout.EnumPopup(
							propertyScope.content,
							(AudioSampleRateSetting) sampleRateSettingProperty.intValue);
						if (changed.changed) {
			...
			if (!sampleRateSettingProperty.hasMultipleDifferentValues &&
			    (AudioSampleRateSetting) sampleRateSettingProperty.intValue ==
			    AudioSampleRateSetting.OverrideSampleRate) {
				using (var horizontal = new EditorGUILayout.HorizontalScope()) {
					var property = audioImporterSampleSettings.FindPropertyRelative(
						"sampleRateOverride");
					using (var propertyScope = new EditorGUI.PropertyScope(
						       horizontal.rect, Style.SampleRate, property)) {
						EditorGUI.showMixedValue = property.hasMultipleDifferentValues;
						using (var changed = new EditorGUI.ChangeCheckScope()) {
							var newValue = EditorGUILayout.IntPopup(
								propertyScope.content, property.intValue,
		...
		EditorGUILayout.LabelField(Style.SharedSettingInformation,
...
private void OnAudioImporterGUI(bool selectionContainsTrackerFile) {
	...
	int shownSettingsPage = EditorGUILayout.BeginPlatformGrouping(
		validPlatforms, Style.DefaultPlatform);

	if (shownSettingsPage == -1) {
		OnSampleSettingGUI(BuildTargetGroup.Unknown, m_DefaultSampleSettings,
		                   selectionContainsTrackerFile);
	} else {
		BuildTargetGroup platform = validPlatforms[shownSettingsPage].targetGroup;
		SerializedProperty platformProperty =
			extraDataSerializedObject.FindProperty(
				$"sampleSettingOverrides.Array.data[{shownSettingsPage}]");
		var isOverriddenProperty =
			platformProperty.FindPropertyRelative("isOverridden");

		// Define the UI state of the override here.
		using (var horizontal = new EditorGUILayout.HorizontalScope()) {
			var label = EditorGUIUtility.TrTempContent(
				"Override for " + validPlatforms[shownSettingsPage].title.text);
			using (var propertyScope = new EditorGUI.PropertyScope(
				       horizontal.rect, label, isOverriddenProperty)) {
				EditorGUI.showMixedValue =
					isOverriddenProperty.hasMultipleDifferentValues;
				using (var changed = new EditorGUI.ChangeCheckScope()) {
					var newValue = EditorGUILayout.ToggleLeft(
						propertyScope.content, isOverriddenProperty.boolValue);
					if (changed.changed) {
		...
		using (new EditorGUI.DisabledScope(
			       isOverriddenProperty.hasMultipleDifferentValues ||
			       !isOverriddenProperty.boolValue)) {
			OnSampleSettingGUI(platform,
			                   platformProperty.FindPropertyRelative("settings"),
			                   selectionContainsTrackerFile);
		}
	}
