...
internal class ModelImporterMaterialEditor : BaseAssetImporterTabUI {
  ...
  Dictionary<Tuple<string, string>, ExternalObjectCache>
      m_ExternalObjectsCache =
          new Dictionary<Tuple<string, string>, ExternalObjectCache>();
  Object m_CacheCurrentTarget = null;

  static class Styles {
    public static GUIContent MaterialLocation =
        EditorGUIUtility.TrTextContent("Location");

    public static GUIContent MaterialName =
        EditorGUIUtility.TrTextContent("Naming");
    ...
    public static GUIContent MaterialSearch =
        EditorGUIUtility.TrTextContent("Search");
    ...
    public static GUIContent NoMaterialHelp = EditorGUIUtility.TrTextContent(
        "Do not generate materials. Use Unity's default material instead.");

    public static GUIContent ExternalMaterialHelpStart =
        EditorGUIUtility.TrTextContent(
            "For each imported material, Unity first looks for an existing material named %MAT%.");
    public static GUIContent[] ExternalMaterialNameHelp = {
        ...
        EditorGUIUtility.TrTextContent(
            "[BaseTextureName] or [ModelFileName]-[MaterialName] if no base texture can be found"),
    };
    public static GUIContent[] ExternalMaterialSearchHelp = {
        EditorGUIUtility.TrTextContent(
            "Unity will look for it in the local Materials folder."),
        EditorGUIUtility.TrTextContent(
            "Unity will do a recursive-up search for it in all Materials folders up to the Assets folder."),
        EditorGUIUtility.TrTextContent(
            "Unity will search for it anywhere inside the Assets folder.")};
    public static GUIContent ExternalMaterialHelpEnd = EditorGUIUtility.TrTextContent(
        "If it doesn't exist, a new one is created in the local Materials folder.");

    public static GUIContent InternalMaterialHelp =
        EditorGUIUtility.TrTextContent(
            "Materials are embedded inside the imported asset.");

    public static GUIContent MaterialAssignmentsHelp =
        EditorGUIUtility.TrTextContent(
            "Material assignments can be remapped below.");

    public static GUIContent ExternalMaterialMappings =
        EditorGUIUtility.TrTextContent(
            "Remapped Materials",
            "External materials to use for each embedded material.");

    public static GUIContent NoMaterialMappingsHelp =
        EditorGUIUtility.TrTextContent(
            "Re-import the asset to see the list of used materials.");

    public static GUIContent Textures =
        EditorGUIUtility.TrTextContent("Textures");
    public static GUIContent ExtractEmbeddedTextures =
        EditorGUIUtility.TrTextContent(
            "Extract Textures...",
            "Click on this button to extract the embedded textures.");

    public static GUIContent Materials =
        EditorGUIUtility.TrTextContent("Materials");
    public static GUIContent ExtractEmbeddedMaterials =
        EditorGUIUtility.TrTextContent(
            "Extract Materials...",
            "Click on this button to extract the embedded materials.");

    public static GUIContent RemapOptions =
        EditorGUIUtility.TrTextContent("On Demand Remap");
    public static GUIContent RemapMaterialsInProject = EditorGUIUtility.TrTextContent(
        "Search and Remap",
        "Click on this button to search and remap the materials from the project.");

    public static GUIContent SRGBMaterialColor = EditorGUIUtility.TrTextContent(
        "sRGB Albedo Colors",
        "Albedo colors in gamma space. Disable this for projects using linear color space.");

    public static GUIContent MaterialCreationMode = EditorGUIUtility.TrTextContent(
        "Material Creation Mode",
        "Select the method used to generate materials during the import process.");
  }
  ...
  private void UpdateShowAllMaterialNameOptions() {
    // We need to display BasedOnTextureName_Or_ModelNameAndMaterialName
    // obsolete option for objects which use this option
#pragma warning disable 618
    m_ShowAllMaterialNameOptions =
        (m_MaterialName.intValue ==
         (int) ModelImporterMaterialName
...
#pragma warning restore 618
  ...
  bool CanExtractEmbeddedMaterials() {
    ...
    // If the m_ExternalObjects map has any un-applied changes, keep the state
    // of the button as is
    ...
    foreach (var t in m_ExternalObjects.serializedObject.targetObjects) {
      ...
      int mappedMaterialCount = externalObjectMap.Count(
          x => x.Key.type == typeof(Material) &&x.Value != null &&
               Array.Exists(materialsList, y => y.name == x.Key.name));
  ...
  internal override void OnEnable() {
    ...
    m_HasEmbeddedTextures =
        serializedObject.FindProperty("m_HasEmbeddedTextures");

    m_SupportsEmbeddedMaterials =
        serializedObject.FindProperty("m_SupportsEmbeddedMaterials");

    m_UseSRGBMaterialColor =
        serializedObject.FindProperty("m_UseSRGBMaterialColor");

    m_MaterialImportMode =
        serializedObject.FindProperty("m_MaterialImportMode");
  ...
  private void BuildMaterialsCache() {
    ...
    for (int materialIdx = 0; materialIdx < m_Materials.arraySize;
         ++materialIdx) {
  ...
  private void BuildExternalObjectsCache() {
    ...
    for (int externalObjectIdx = 0, count = m_ExternalObjects.arraySize;
         externalObjectIdx < count; ++externalObjectIdx) {
      ...
      m_ExternalObjectsCache.Add(
          new Tuple<string, string>(externalName, externalType), cachedObject);
    }
  }

  public override void OnInspectorGUI() { DoMaterialsGUI(); }
  ...
  private void ExtractTexturesGUI() {
    using(new EditorGUILayout.HorizontalScope()) {
      ...
      using(new EditorGUI.DisabledScope(
          !m_HasEmbeddedTextures.boolValue &&
          !m_HasEmbeddedTextures.hasMultipleDifferentValues)) {
        if (GUILayout.Button(Styles.ExtractEmbeddedTextures)) {
          // when extracting textures, we must handle the case when multiple
          // selected assets could generate textures with the same name at the
          // user supplied path we proceed as follows:
          ...
          // 3. we generate unique names for each asset and move them to the
          // user supplied path
          // 4. we re-import all the assets to have the internal materials
          // linked to the newly extracted textures

          List<Tuple<Object, string>> outputsForTargets =
              new List<Tuple<Object, string>>();
          // use the first target for selecting the destination folder, but
          // apply that path for all targets
          ...
          try {
            ...
            foreach (var t in targets) {
              ...
              tempPath = tempPath.Replace(
                  "Temp",
                  UnityEditorInternal.InternalEditorUtility.GetAssetsFolder());
          ...
          try {
            ...
            foreach (var item in outputsForTargets) {
              ...
              foreach (var guid in guids) {
                ...
                importer.AddRemap(new AssetImporter.SourceAssetIdentifier(tex),
                                  tex);

                var newPath = Path.Combine(destinationPath,
                                           FileUtil.UnityGetFileName(path));
    ...
    using(new EditorGUILayout.HorizontalScope()) {
      ...
      using(new EditorGUI.DisabledScope(!CanExtractEmbeddedMaterials())) {
        if (GUILayout.Button(Styles.ExtractEmbeddedMaterials)) {
          ...
          // database, we need to reimport them
          if (!AllEmbeddedMaterialsAreImported()) {
            if (EditorUtility.DisplayDialog(
                    L10n.Tr(
                        "Are you sure you want to re-extract the Materials?"),
                    L10n.Tr(
                        "In order to re-extract the Materials we'll need to reimport the mesh, this might take a while. Do you want to continue?"),
          ...
          // AssetDatabase.StopAssetEditing() invokes OnEnable(), which
          // invalidates all the serialized properties, so we must return.
          return true;
        }
      }
    }
  ...
  public bool AllEmbeddedMaterialsAreImported() {
    foreach (ModelImporter modelImporter in m_ExternalObjects.serializedObject
                 .targetObjects) {
      // Find the names of embedded materials - the source materials that are
      // not re-mapped in the externalObjectsCache
      IEnumerable<string> namesOfEmbeddedMaterials =
          modelImporter.sourceMaterials
              .Where(x => !m_ExternalObjectsCache.Any(
                         y => y.Key.Item1 == x.name &&
                              y.Value.property != null &&
                              y.Value.property.objectReferenceValue != null))
              .Select(x => x.name);

      // Find the names of embedded materials in the AssetDatabase
      IEnumerable<string> namesOfMaterialsInAssetDatabase =
          AssetDatabase.LoadAllAssetsAtPath(modelImporter.assetPath)
              .Where(x => x.GetType() == typeof(Material))
              .Select(x => x.name);

      // Are there any embedded materials that *arent* in the AssetDatabase?
      if (namesOfEmbeddedMaterials.Except(namesOfMaterialsInAssetDatabase)
              .Any())
        return false;
    }
  ...
  public void ReimportEmbeddedMaterials() {
    ...
    int[] missingMaterialIndexes =
        m_ExternalObjectsCache.Values
            .Select((extObj, index) => new {extObj, index})
            .Where(x => x.extObj.property != null &&
                        x.extObj.property.objectReferenceValue == null &&
                        x.extObj.property.objectReferenceInstanceIDValue != 0)
            .Select(x => x.index)
            .ToArray();
    ...
    // Force a reimport - any materials marked "None" (including former missing
    // Materials), will now be assigned an embedded material
  ...
  private bool MaterialRemapOptions() {
    m_ShowMaterialRemapOptions = EditorGUILayout.Foldout(
        m_ShowMaterialRemapOptions, Styles.RemapOptions, true);
    if (m_ShowMaterialRemapOptions) {
      EditorGUI.indentLevel++;
      EditorGUILayout.Popup(
          m_MaterialName,
          m_ShowAllMaterialNameOptions ? Styles.MaterialNameOptAll
          : Styles.MaterialNameOptMain, Styles.MaterialName);
      EditorGUILayout.Popup(m_MaterialSearch, Styles.MaterialSearchOpt,
                            Styles.MaterialSearch);

      string searchHelp =
          Styles.ExternalMaterialHelpStart.text.Replace(
              "%MAT%",
      ...
      using(new EditorGUILayout.HorizontalScope()) {
        ...
        using(new EditorGUI.DisabledScope(assetTarget == null)) {
          if (GUILayout.Button(Styles.RemapMaterialsInProject)) {
            ...
            foreach (var t in targets) {
              var importer = t as ModelImporter;
              // SearchAndReplaceMaterials will ensure the material name and
              // search options get saved, while all other pending changes stay
              // pending.
              importer.SearchAndRemapMaterials(
                  (ModelImporterMaterialName) m_MaterialName.intValue,
                  (ModelImporterMaterialSearch) m_MaterialSearch.intValue);
            }
  ...
  void DoMaterialsGUI() {
    if (targets.Length > 1) {
      EditorGUILayout.HelpBox(
          L10n.Tr("Material Editing is not supported on multiple selection"),
          MessageType.Info);
      return;
    }
    ...
    using(var horizontal = new EditorGUILayout.HorizontalScope()) {
      using(var prop = new EditorGUI.PropertyScope(horizontal.rect,
                                                   Styles.MaterialCreationMode,
                                                   m_MaterialImportMode)) {
        EditorGUI.BeginChangeCheck();
        var newValue =
            (int)(ModelImporterMaterialImportMode) EditorGUILayout.EnumPopup(
                prop.content, (ModelImporterMaterialImportMode)
                                  m_MaterialImportMode.intValue);
    ...
    if (!m_MaterialImportMode.hasMultipleDifferentValues) {
      if (m_MaterialImportMode.intValue !=
          (int) ModelImporterMaterialImportMode.None) {
        if (m_MaterialImportMode.intValue ==
            (int) ModelImporterMaterialImportMode.ImportStandard) {
          EditorGUILayout.PropertyField(m_UseSRGBMaterialColor,
                                        Styles.SRGBMaterialColor);
        }

        using(var horizontal = new EditorGUILayout.HorizontalScope()) {
          using(var prop = new EditorGUI.PropertyScope(horizontal.rect,
                                                       Styles.MaterialLocation,
                                                       m_MaterialLocation)) {
            EditorGUI.BeginChangeCheck();
            var newValue =
                (int)(ModelImporterMaterialLocation) EditorGUILayout.EnumPopup(
                    prop.content, (ModelImporterMaterialLocation)
                                      m_MaterialLocation.intValue);
            if (EditorGUI.EndChangeCheck()) {
        ...
        if (!m_MaterialLocation.hasMultipleDifferentValues) {
          if (m_MaterialLocation.intValue == 0) {
            ...
            materialHelp =
                Styles.ExternalMaterialHelpStart.text.Replace(
                    "%MAT%",
                    Styles.ExternalMaterialNameHelp[m_MaterialName.intValue]
                        .text) +
                "\n" +
                Styles.ExternalMaterialSearchHelp[m_MaterialSearch.intValue]
                    .text +
                "\n" + Styles.ExternalMaterialHelpEnd.text;
          } else if (m_Materials.arraySize > 0 &&
                     CanExtractEmbeddedMaterials()) {
    ...
    if ((targets.Length == 1 ||
         m_SupportsEmbeddedMaterials.hasMultipleDifferentValues == false) &&
        m_SupportsEmbeddedMaterials.boolValue == false &&
        m_MaterialLocation.intValue != 0 &&
        !m_MaterialLocation.hasMultipleDifferentValues) {
      EditorGUILayout.Space();
      EditorGUILayout.HelpBox(Styles.NoMaterialMappingsHelp.text,
                              MessageType.Warning);
    }
    ...
    if (m_MaterialImportMode.intValue !=
            (int) ModelImporterMaterialImportMode.None &&
        targets.Length == 1 && m_Materials.arraySize > 0 &&
        m_MaterialLocation.intValue != 0 &&
        !m_MaterialLocation.hasMultipleDifferentValues &&
        !m_Materials.hasMultipleDifferentValues &&
  ...
  void DoMaterialRemapList() {
    // OnEnabled is not called consistently when the asset gets reimported, we
    // need to rebuild the cache here if it's outdated.
    if (m_ExternalObjects.arraySize != m_ExternalObjectsCache.Count())
      ResetValues();
    // The list of material names is immutable, whereas the map of external
    // objects can change based on user actions. For each material name, map the
    // external object associated with it where one exists.
    for (int materialIdx = 0; materialIdx < m_MaterialsCache.Count;
         ++materialIdx) {
      var mat = m_MaterialsCache[materialIdx];

      bool hasMatchingCachedObject =
          m_ExternalObjectsCache.TryGetValue(
              new Tuple<string, string>(mat.name, mat.type),
              out var cachedExternalObject) &&
          cachedExternalObject != null;
      ...
      if (hasMatchingCachedObject) {
        ...
        // The material doesn't have a serialized property, so it's going to
        // have to draw the GUI a different way!
  ...
  void MaterialPropertyGUI(MaterialCache materialCache,
    ...
    if (EditorGUI.EndChangeCheck()) {
      if (material == null)
        m_ExternalObjects.DeleteArrayElementAtIndex(
            externalObjectCache.propertyIdx);
      else {
        var pair = m_ExternalObjects.GetArrayElementAtIndex(
            externalObjectCache.propertyIdx);
  ...
  void MaterialPropertyGUI(MaterialCache materialCache) {
    ...
    EditorGUI.BeginChangeCheck();
    Material material =
        ObjectFieldWithPPtrHashID(nameLabel, null, typeof(Material), false)
            as Material;
    if (EditorGUI.EndChangeCheck() && material != null) {
      m_ExternalObjects.arraySize++;
      var pair = m_ExternalObjects.GetArrayElementAtIndex(
          m_ExternalObjects.arraySize - 1);
      ...
      pair.FindPropertyRelative("first.assembly").stringValue =
          materialCache.assembly;
      pair.FindPropertyRelative("second").objectReferenceValue = material;

      // ExternalObjects is serialized as a map, so items are reordered when
      // deserializing. We need to update the serializedObject to trigger the
      // reordering before rebuilding the cache.
      serializedObject.ApplyModifiedProperties();
      serializedObject.Update();
  ...
  ObjectFieldWithPPtrHashID(GUIContent label, Object obj, Type objType,
    ...
    return EditorGUI.DoObjectField(position, position, id, obj, null, objType,
