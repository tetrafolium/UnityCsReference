...
internal class ModelImporterClipEditor : BaseAssetImporterTabUI {
  ...
  ModelImporter singleImporter {
    get { return targets[0] as ModelImporter; }
  }

  internal const string ActiveClipIndex =
      "ModelImporterClipEditor.ActiveClipIndex";
  ...
  private class ClipInformation {
    ...
    public AnimationClipInfoProperties property => m_Property ??(
        m_Property = new AnimationClipInfoProperties(animationClipProperty));
    ...
    public string firstFrame {
      get {
        return prop.FindPropertyRelative("firstFrame")
    ...
    public string lastFrame {
      get {
        return prop.FindPropertyRelative("lastFrame")
            .floatValue.ToString("0.0",
                                 CultureInfo.InvariantCulture.NumberFormat);
      }
    }
...
#pragma warning disable 0649
  [CacheProperty] SerializedProperty m_AnimationType;

  [CacheProperty] SerializedProperty m_ImportAnimation;
  [CacheProperty] SerializedProperty m_ClipAnimations;
  [CacheProperty] SerializedProperty m_BakeSimulation;
  [CacheProperty] SerializedProperty m_ResampleCurves;
  [CacheProperty] SerializedProperty m_AnimationCompression;
  [CacheProperty] SerializedProperty m_AnimationRotationError;
  [CacheProperty] SerializedProperty m_AnimationPositionError;
  [CacheProperty] SerializedProperty m_AnimationScaleError;
  [CacheProperty] SerializedProperty m_AnimationWrapMode;
  [CacheProperty] SerializedProperty m_LegacyGenerateAnimations;
  [CacheProperty] SerializedProperty m_ImportAnimatedCustomProperties;
  [CacheProperty] SerializedProperty m_ImportConstraints;

  [CacheProperty] SerializedProperty m_MotionNodeName;
...
#pragma warning restore 0649
  ...
  private ModelImporterGenerateAnimations legacyGenerateAnimations {
    get {
      return (ModelImporterGenerateAnimations)
          m_LegacyGenerateAnimations.intValue;
    }
    set { m_LegacyGenerateAnimations.intValue = (int) value; }
  }

  private class Styles {
    public GUIContent ErrorsFoundWhileImportingThisAnimation =
        EditorGUIUtility.TrTextContentWithIcon(
            "Error(s) found while importing this animation file. Open \"Import Messages\" foldout below for more details.",
            MessageType.Error);
    public GUIContent WarningsFoundWhileImportingRig =
        EditorGUIUtility.TrTextContentWithIcon(
            "Warning(s) found while importing rig in this animation file. Open \"Rig\" tab for more details.",
            MessageType.Warning);
    public GUIContent WarningsFoundWhileImportingThisAnimation =
        EditorGUIUtility.TrTextContentWithIcon(
            "Warning(s) found while importing this animation file. Open \"Import Messages\" foldout below for more details.",
            MessageType.Warning);
    public GUIContent ImportAnimations = EditorGUIUtility.TrTextContent(
        "Import Animation", "Controls if animations are imported.");

    public GUIStyle numberStyle = new GUIStyle(EditorStyles.label);

    public GUIContent AnimWrapModeLabel = EditorGUIUtility.TrTextContent(
        "Wrap Mode",
    ...
    public GUIContent BakeIK = EditorGUIUtility.TrTextContent(
        "Bake Animations",
    ...
    public GUIContent ResampleCurves = EditorGUIUtility.TrTextContent(
        "Resample Curves ",
        " Curves will be resampled on every frame. Use this if you're having issues with the interpolation between keys in your original animation. Disable this to keep curves as close as possible to how they were originally authored.");
    public GUIContent AnimCompressionLabel = EditorGUIUtility.TrTextContent(
        "Anim. Compression",
    ...
    public GUIContent[] AnimCompressionOpt = {
        ...
        EditorGUIUtility.TrTextContent(
            "Optimal",
            "Reduces keyframes on import and choose between different curve representations to reduce memory usage at runtime. This affects the runtime memory size and how curves are evaluated.")};

    public GUIContent AnimRotationErrorLabel = EditorGUIUtility.TrTextContent(
        "Rotation Error",
        "Defines how much rotation curves should be reduced. The smaller value you use - the higher precision you get.");
    public GUIContent AnimPositionErrorLabel = EditorGUIUtility.TrTextContent(
        "Position Error",
        "Defines how much position curves should be reduced. The smaller value you use - the higher precision you get.");
    public GUIContent AnimScaleErrorLabel = EditorGUIUtility.TrTextContent(
        "Scale Error",
        "Defines how much scale curves should be reduced. The smaller value you use - the higher precision you get.");
    public GUIContent AnimationCompressionHelp = EditorGUIUtility.TrTextContent(
        "Rotation error is defined as maximum angle deviation allowed in degrees, for others it is defined as maximum distance/delta deviation allowed in percents");
    public GUIContent clipMultiEditInfo = EditorGUIUtility.TrTextContent(
        "Multi-object editing of clips not supported.");

    public GUIContent updateMuscleDefinitionFromSource =
        EditorGUIUtility.TrTextContent(
            "Update",
            "Update the copy of the muscle definition from the source.");

    public GUIContent MotionSetting = EditorGUIUtility.TrTextContent(
        "Motion", "Advanced setting for root motion and blending pivot");
    public GUIContent MotionNode = EditorGUIUtility.TrTextContent(
        "Root Motion Node",
        "Define a transform node that will be used to create root motion curves");
    public GUIContent ImportMessages =
        EditorGUIUtility.TrTextContent("Import Messages");

    public GUIContent GenerateRetargetingWarnings =
        EditorGUIUtility.TrTextContent("Generate Retargeting Quality Report");
    public GUIContent RetargetingQualityCompares =
        EditorGUIUtility.TrTextContentWithIcon(
            "Retargeting Quality compares retargeted with original animation. It reports average and maximum position/orientation difference for body parts. It may slow down import time of this file.",
            MessageType.Info);
    public GUIContent AnimationDataWas = EditorGUIUtility.TrTextContentWithIcon(
        "Animation data was imported using a deprecated Generation option in the Rig tab. Please switch to a non-deprecated import mode in the Rig tab to be able to edit the animation import settings.",
        MessageType.Info);
    public GUIContent TheAnimationsSettingsCanBe =
        EditorGUIUtility.TrTextContentWithIcon(
            "The animations settings can be edited after clicking Apply.",
            MessageType.Info);
    public GUIContent ErrorsFoundWhileImporting =
        EditorGUIUtility.TrTextContentWithIcon(
            "Error(s) found while importing rig in this animation file. Open \"Rig\" tab for more details.",
            MessageType.Error);
    public GUIContent NoAnimationDataAvailable =
        EditorGUIUtility.TrTextContentWithIcon(
            "No animation data available in this model.", MessageType.Info);
    public GUIContent TheRigsOfTheSelectedModelsHave =
        EditorGUIUtility.TrTextContentWithIcon(
            "The rigs of the selected models have different Animation Types.",
            MessageType.Info);
    public GUIContent TheRigsOfTheSelectedModelsAre =
        EditorGUIUtility.TrTextContentWithIcon(
            "The rigs of the selected models are not setup to handle animation. Change the Animation Type in the Rig tab and click Apply.",
            MessageType.Info);
    ...
    public GUIContent MaskHasAPath = EditorGUIUtility.TrTextContent(
        "Mask has a path that does not match the transform hierarchy. Animation may not import correctly.");
    public GUIContent UpdateMask =
        EditorGUIUtility.TrTextContent("Update Mask");
    public GUIContent SourceMaskHasChanged = EditorGUIUtility.TrTextContent(
        "Source Mask has changed since last import and must be updated.");
    public GUIContent SourceMaskHasAPath = EditorGUIUtility.TrTextContent(
        "Source Mask has a path that does not match the transform hierarchy. Animation may not import correctly.");

    public GUIContent Mask = EditorGUIUtility.TrTextContent(
        "Mask",
        "Configure the mask for this clip to remove unnecessary curves.");

    public GUIContent ImportAnimatedCustomProperties =
        EditorGUIUtility.TrTextContent(
            "Animated Custom Properties",
            "Controls if animated custom properties are imported.");
    public GUIContent ImportConstraints = EditorGUIUtility.TrTextContent(
        "Import Constraints", "Controls if the constraints are imported.");

    public Styles() { numberStyle.alignment = TextAnchor.UpperRight; }
  }
  ...
  public ModelImporterClipEditor(AssetImporterEditor panelContainer)
    ...
    m_ClipList = new ReorderableList(new List<ClipInformation>(),
  ...
  internal override void OnEnable() {
    ...
    // caching errors values now as they can't change until next re-import that
    // will triggers a new OnEnable
    ...
    // Sometimes we dont want to start at the 0th index, this is where we're
    // editing a clip - see
    ...
    motionNodeIndex =
        ArrayUtility.FindIndex(m_MotionNodeList, delegate(GUIContent content) {
          return content.text == m_MotionNodeName.stringValue;
        });
  ...
  void SyncClipEditor(AnimationClipInfoProperties info) {
    ...
    // It mandatory to set clip info into mask inspector first, this will update
    // m_Mask.
  ...
  private void SetupDefaultClips() {
    ...
    m_ClipAnimations =
        m_DefaultClipsSerializedObject.FindProperty("m_ClipAnimations");
    m_AnimationType =
        m_DefaultClipsSerializedObject.FindProperty("m_AnimationType");
  ...
  // When switching to explicitly defined clips, we must fix up the internalID's
  // to not lose AnimationClip references. When m_ClipAnimations is defined, the
  // clips are identified by the clipName When m_ClipAnimations is not defined,
  // the clips are identified by the takeName
  void PatchDefaultClipTakeNamesToSplitClipNames() {
    foreach (TakeInfo takeInfo in singleImporter.importedTakeInfos) {
      UnityType animationClipType = UnityType.FindTypeByName("AnimationClip");
      ImportSettingInternalID.Rename(serializedObject, animationClipType,
                                     takeInfo.name, takeInfo.defaultClipName);
    }
  }

  // A dummy SerializedObject is created when there are no explicitly defined
  // clips. When the user modifies any settings these clips must be transferred
  // to the model importer.
  private void TransferDefaultClipsToCustomClips() {
    if (m_DefaultClipsSerializedObject == null)
      return;

    bool wasEmpty =
        serializedObject.FindProperty("m_ClipAnimations").arraySize == 0;
    if (!wasEmpty)
      Debug.LogError(
    ...
    if (m_ClipList.index >= 0)
      SyncClipEditor(
          ((ClipInformation) m_ClipList.list[m_ClipList.index]).property);
  }

  internal override void OnDestroy() { DestroyEditorsAndData(); }
  ...
  void AnimationClipGUI() {
    if (m_Errors.Length > 0) {
      ...
      if (m_Warnings.Length > 0) {
        EditorGUILayout.HelpBox(
            styles.WarningsFoundWhileImportingThisAnimation);
      }
    }
    ...
    s_ImportMessageFoldout = EditorGUILayout.Foldout(
        s_ImportMessageFoldout, styles.ImportMessages, true);
    ...
    if (s_ImportMessageFoldout) {
      ...
      if (animationType == ModelImporterAnimationType.Human) {
        EditorGUILayout.PropertyField(m_AnimationDoRetargetingWarnings,
                                      styles.GenerateRetargetingWarnings);

        if (m_AnimationDoRetargetingWarnings.boolValue) {
          if (m_RetargetWarnings.Length > 0) {
            EditorGUILayout.HelpBox(L10n.Tr(m_RetargetWarnings),
                                    MessageType.Info);
          }
        } else {
  ...
  public override void OnInspectorGUI() {
    ...
    EditorGUILayout.PropertyField(m_ImportConstraints,
                                  styles.ImportConstraints);
    ...
    if (m_ImportAnimation.boolValue &&
        !m_ImportAnimation.hasMultipleDifferentValues) {
      bool hasNoValidAnimationData =
          targets.Length == 1 && singleImporter.importedTakeInfos.Length == 0 &&
          singleImporter.animationType != ModelImporterAnimationType.None;
  ...
  void AnimationSettings() {
    ...
    if (animationType == ModelImporterAnimationType.Legacy) {
      ...
      EditorGUILayout.Popup(m_AnimationWrapMode, styles.AnimWrapModeOpt,
                            styles.AnimWrapModeLabel);
      EditorGUI.showMixedValue = false;

      // Compression
      int[] kCompressionValues = {
          (int) ModelImporterAnimationCompression.Off,
          (int) ModelImporterAnimationCompression.KeyframeReduction,
          (int) ModelImporterAnimationCompression
              .KeyframeReductionAndCompression};
      EditorGUILayout.IntPopup(m_AnimationCompression,
                               styles.AnimCompressionOptLegacy,
                               kCompressionValues, styles.AnimCompressionLabel);
    } else {
      // Compression
      int[] kCompressionValues = {
          (int) ModelImporterAnimationCompression.Off,
          (int) ModelImporterAnimationCompression.KeyframeReduction,
          (int) ModelImporterAnimationCompression.Optimal};
      EditorGUILayout.IntPopup(m_AnimationCompression,
                               styles.AnimCompressionOpt, kCompressionValues,
                               styles.AnimCompressionLabel);
    }

    if (m_AnimationCompression.intValue >
        (int) ModelImporterAnimationCompression.Off) {
      // keyframe reduction settings
      EditorGUILayout.PropertyField(m_AnimationRotationError,
                                    styles.AnimRotationErrorLabel);
      EditorGUILayout.PropertyField(m_AnimationPositionError,
                                    styles.AnimPositionErrorLabel);
      EditorGUILayout.PropertyField(m_AnimationScaleError,
                                    styles.AnimScaleErrorLabel);
      GUILayout.Label(styles.AnimationCompressionHelp, EditorStyles.helpBox);
    }

    EditorGUILayout.PropertyField(m_ImportAnimatedCustomProperties,
                                  styles.ImportAnimatedCustomProperties);
  }

  void RootMotionNodeSettings() {
    if (animationType == ModelImporterAnimationType.Human ||
        animationType == ModelImporterAnimationType.Generic) {
      s_MotionNodeFoldout = EditorGUILayout.Foldout(s_MotionNodeFoldout,
                                                    styles.MotionSetting, true);

      if (s_MotionNodeFoldout) {
        EditorGUI.BeginChangeCheck();
        motionNodeIndex = EditorGUILayout.Popup(
            styles.MotionNode, motionNodeIndex, m_MotionNodeList);

        if (EditorGUI.EndChangeCheck()) {
          if (motionNodeIndex > 0 &&
              motionNodeIndex < m_MotionNodeList.Length) {
            m_MotionNodeName.stringValue =
                m_MotionNodeList[motionNodeIndex].text;
          } else {
            m_MotionNodeName.stringValue = "";
          }
        }
  ...
  void SelectClip(int selected) {
    // If you were editing Clip Name (delayed text field had focus) and then
    // selected a new clip from the clip list, the active string in the delayed
    // text field would get applied to the new selected clip instead of the old.
    // HACK: Calling EndGUI here on the recycled delayed text editor seems to
    // fix this issue. Sometime we should reimplement delayed text field code to
    // not be super confusing and then fix the issue more properly.
    ...
    AnimationClipInfoProperties info =
        ((ClipInformation) m_ClipList.list[m_ClipList.index]).property;
    AnimationClip clip =
        singleImporter.GetPreviewAnimationClipForTake(info.takeName);
    if (clip != null) {
      m_AnimationClipEditor = (AnimationClipEditor) Editor.CreateEditor(
          clip, typeof(AnimationClipEditor));
      InitMask(info);
      SyncClipEditor(info);
    }
  }
  ...
  void UpdateList() {
    ...
    m_ClipList.index =
        Mathf.Clamp(m_ClipList.index, -1, m_ClipAnimations.arraySize - 1);
  }
  ...
  void SelectClipInList(ReorderableList list) { SelectClip(list.index); }

  const int kFrameColumnWidth = 45;

  private void DrawClipElement(Rect rect, int index, bool selected,
                               bool focused) {
  ...
  void AnimationSplitTable() {
    ...
    // Show selected clip info
    {
      ...
      if (m_AnimationClipEditor != null) {
        ...
        AnimationClip actualClip =
            m_AnimationClipEditor.target as AnimationClip;
        ...
        m_AnimationClipEditor.takeIndex =
            ArrayUtility.IndexOf(takeNames, clip.takeName);
        ...
        if (EditorGUI.EndChangeCheck()) {
          ...
          // We renamed the clip name, try to maintain the localIdentifierInFile
          ...
          if (clip.name != currentName) {
            ...
            TransferDefaultClipsToCustomClips();
            UnityType animationClipType =
                UnityType.FindTypeByName("AnimationClip");
            ImportSettingInternalID.Rename(serializedObject, animationClipType,
        ...
        if (EditorGUI.EndChangeCheck() ||
            m_AnimationClipEditor.needsToGenerateClipInfo) {
          TransferDefaultClipsToCustomClips();
  ...
  public override bool HasPreviewGUI() {
    return m_ImportAnimation.boolValue && m_AnimationClipEditor != null &&
           m_AnimationClipEditor.HasPreviewGUI();
  }
  ...
  bool IsDeprecatedMultiAnimationRootImport() {
    if (animationType == ModelImporterAnimationType.Legacy)
      return legacyGenerateAnimations ==
  ...
  AnimationClipInfoProperties GetSelectedClipInfo() {
    // If it doesn't have a selected clip. return null - there is nothing to
    // select!
    return m_ClipList.index >= 0 &&
           m_ClipList.index <
               m_ClipList
                   .count ?((ClipInformation) m_ClipList.list[m_ClipList.index])
                   .property : null;
  }
  ...
  /// <param name="number">Number between the brackets (-1 if no brackets were
  /// found).</param>
  ...
  string FindNextAvailableName(string baseName) {
    ...
    for (int i = 0; i < m_ClipAnimations.arraySize; ++i) {
      AnimationClipInfoProperties clip =
          ((ClipInformation) m_ClipList.list[i]).property;
  ...
  void SetupTakeNameAndFrames(AnimationClipInfoProperties info,
                              TakeInfo takeInfo) {
    info.takeName = takeInfo.name;
    info.firstFrame =
        (int) Mathf.Round(takeInfo.bakeStartTime * takeInfo.sampleRate);
    info.lastFrame =
        (int) Mathf.Round(takeInfo.bakeStopTime * takeInfo.sampleRate);
  }
  ...
  void AddClip(TakeInfo takeInfo) {
    ...
    var property =
        m_ClipAnimations.GetArrayElementAtIndex(m_ClipAnimations.arraySize - 1);
    AnimationClipInfoProperties info =
        new AnimationClipInfoProperties(property);
  ...
  private void AvatarMaskSettings(AnimationClipInfoProperties clipInfo) {
    if (clipInfo != null && m_AnimationClipEditor != null) {
      ...
      if (maskType == ClipAnimationMaskType.CreateFromThisModel ||
          maskType == ClipAnimationMaskType.CopyFromOther) {
        upToDate = m_MaskInspector.IsMaskUpToDate();
      }

      if (maskType == ClipAnimationMaskType.CreateFromThisModel && !upToDate &&
          !m_MaskInspector.IsMaskEmpty()) {
        GUILayout.BeginHorizontal(EditorStyles.helpBox);
        GUILayout.Label(styles.MaskHasAPath, EditorStyles.wordWrappedMiniLabel);
  ...
  private void InitMask(AnimationClipInfoProperties clipInfo) {
    if (m_Mask == null) {
      ...
      m_MaskInspector = (AvatarMaskInspector) Editor.CreateEditor(
          m_Mask, typeof(AvatarMaskInspector));
  ...
  private void
  SetTransformMaskFromReference(AnimationClipInfoProperties clipInfo) {
    string[] transformPaths = referenceTransformPaths;
    string[] humanTransforms =
        animationType == ModelImporterAnimationType.Human ? AvatarMaskUtility
                             .GetAvatarHumanAndActiveExtraTransforms(
                                 serializedObject,
                                 clipInfo.transformMaskProperty, transformPaths)
        : AvatarMaskUtility.GetAvatarInactiveTransformMaskPaths(
              clipInfo.transformMaskProperty);

    AvatarMaskUtility.UpdateTransformMask(
        clipInfo.transformMaskProperty, transformPaths, humanTransforms,
  ...
  void RegisterListeners() {
    // Ensures that the ClipList and the Serialized copy of the clip remain in
    // sync when an Undo/Redo is performed.
    if (!serializedObject.isEditingMultipleObjects)
  ...
  void DeserializeClips() {
    ...
    // Set the active clip within a valid range, -1 ONLY if there are no
    // possible clips to select.
    int selectedClip = m_ClipList.count > 0
                           ? Mathf.Clamp(m_ClipList.index, 0, m_ClipList.count)
                           : -1;
