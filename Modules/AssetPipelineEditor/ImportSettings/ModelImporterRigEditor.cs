...
#pragma warning disable 0649
  ...
  [CacheProperty] SerializedProperty m_AvatarSetup;
  [CacheProperty("m_LastHumanDescriptionAvatarSource")] SerializedProperty
      m_AvatarSource;
  [CacheProperty] SerializedProperty m_LegacyGenerateAnimations;
  [CacheProperty] SerializedProperty m_AnimationCompression;
  [CacheProperty("skinWeightsMode")] SerializedProperty m_SkinWeightsMode;
  [CacheProperty("maxBonesPerVertex")] SerializedProperty m_MaxBonesPerVertex;
  [CacheProperty("minBoneWeight")] SerializedProperty m_MinBoneWeight;
  [CacheProperty] SerializedProperty m_OptimizeGameObjects;

  [CacheProperty("m_HumanDescription.m_RootMotionBoneName")] SerializedProperty
      m_RootMotionBoneName;

  [CacheProperty("m_HasExtraRoot")] SerializedProperty m_SrcHasExtraRoot;
  [CacheProperty("m_HumanDescription.m_HasExtraRoot")] SerializedProperty
      m_DstHasExtraRoot;

  [CacheProperty] SerializedProperty m_RigImportErrors;
  [CacheProperty] SerializedProperty m_RigImportWarnings;

  [CacheProperty("m_HumanDescription.m_Human")] SerializedProperty
      m_HumanBoneArray;
  [CacheProperty("m_HumanDescription.m_Skeleton")] SerializedProperty
      m_Skeleton;
#pragma warning restore 0649
  ...
  static class Styles {
    public static GUIContent AnimationType = EditorGUIUtility.TrTextContent(
        "Animation Type", "The type of animation to support / import.");
    public static GUIContent[] AnimationTypeOpt = {
        ...
        EditorGUIUtility.TrTextContent("Humanoid",
                                       "Humanoid Mecanim animation system.")};

    public static GUIContent SaveAvatar = EditorGUIUtility.TrTextContent(
        "Save Avatar", "Saves the generated Avatar as a sub-asset.");

    public static GUIContent AnimLabel = EditorGUIUtility.TrTextContent(
        "Generation", "Controls how animations are imported.");
    public static GUIContent[] AnimationsOpt = {
        EditorGUIUtility.TrTextContent("Don't Import",
                                       "No animation or skinning is imported."),
        EditorGUIUtility.TrTextContent(
            "Store in Original Roots (Deprecated)",
    ...
    public static GUIContent avatar =
        EditorGUIUtility.TrTextContent("Animator");
    public static GUIContent configureAvatar =
        EditorGUIUtility.TrTextContent("Configure...");
    public static GUIContent avatarValid =
        EditorGUIUtility.TrTextContent("\u2713");
    public static GUIContent avatarInvalid =
        EditorGUIUtility.TrTextContent("\u2715");
    public static GUIContent avatarPending =
        EditorGUIUtility.TrTextContent("...");

    public static GUIContent UpdateMuscleDefinitionFromSource =
        EditorGUIUtility.TrTextContent(
            "Update",
            "Update the copy of the muscle definition from the source.");
    public static GUIContent RootNode = EditorGUIUtility.TrTextContent(
        "Root node",
        "Specify the root node used to extract the animation translation.");

    public static GUIContent AvatarDefinition = EditorGUIUtility.TrTextContent(
        "Avatar Definition",
        "Choose between Create From This Model or Copy From Other Avatar. The first one creates an Avatar for this file and the second one uses an Avatar from another file to import animation.");

    public static GUIContent SkinWeightsMode = EditorGUIUtility.TrTextContent(
        "Skin Weights", "Control how many bone weights are imported.");
    public static GUIContent[] SkinWeightsModeOpt = {
        EditorGUIUtility.TrTextContent(
            "Standard (4 Bones)", "Import a maximum of 4 bones per vertex."),
        EditorGUIUtility.TrTextContent(
            "Custom", "Import a custom number of bones per vertex.")};
    public static GUIContent MaxBonesPerVertex = EditorGUIUtility.TrTextContent(
        "Max Bones/Vertex", "Number of bones that can affect each vertex.");
    public static GUIContent MinBoneWeight = EditorGUIUtility.TrTextContent(
        "Min Bone Weight",
        "Bone weights smaller than this value are rejected. The remaining weights are scaled to add up to 1.0.");

    public static GUIContent UpdateReferenceClips = EditorGUIUtility.TrTextContent(
        "Update reference clips",
        "Click on this button to update all the @convention files referencing this file. Should set all these files to Copy From Other Avatar, set the source Avatar to this one and reimport all these files.");

    public static GUIContent ImportMessages =
        EditorGUIUtility.TrTextContent("Import Messages");
    public static GUIContent ExtraExposedTransform = EditorGUIUtility.TrTextContent(
        "Extra Transforms to Expose",
        "Select the list of transforms to expose in the optimized GameObject hierarchy.");
  }
  ...
  internal override void OnEnable() {
    ...
    m_ExposeTransformEditor.OnEnable(singleImporter.transformPaths,
                                     serializedObject);
  ...
  private void UpdateBipedMappingReport() {
    if (m_AnimationType.intValue == (int) ModelImporterAnimationType.Human) {
      ...
      if (go != null) {
        m_IsBiped =
            AvatarBipedMapper.IsBiped(go.transform, m_BipedMappingReport);
      }
    }
  }
  ...
  private bool CanMultiEditTransformList() {
    ...
    for (int i = 1; i < targets.Length; ++i) {
      ...
      if (!ArrayUtility.ArrayEquals(transformPaths,
  ...
  void CheckIfAvatarCopyIsUpToDate() {
    ...
    // Get SerializedObject of this importer and the importer of the source
    // avatar
    string path =
        AssetDatabase.GetAssetPath(m_AvatarSource.objectReferenceValue);
    ModelImporter sourceImporter =
        AssetImporter.GetAtPath(path) as ModelImporter;

    m_AvatarCopyIsUpToDate =
        DoesHumanDescriptionMatch(singleImporter, sourceImporter);
  }
  ...
  void LegacyGUI() {
    EditorGUILayout.Popup(m_LegacyGenerateAnimations, Styles.AnimationsOpt,
                          Styles.AnimLabel);
    // Show warning and fix button for deprecated import formats
    if (m_LegacyGenerateAnimations.intValue == 1 ||
        m_LegacyGenerateAnimations.intValue == 2 ||
        m_LegacyGenerateAnimations.intValue == 3)
      EditorGUILayout.HelpBox(
          "The animation import setting \"" +
              Styles.AnimationsOpt[m_LegacyGenerateAnimations.intValue].text +
              "\" is deprecated.",
          MessageType.Warning);
  }
  ...
  void GenericGUI() {
    using(var horizontal = new EditorGUILayout.HorizontalScope()) {
      using(var propertyField = new EditorGUI.PropertyScope(
                horizontal.rect, Styles.AvatarDefinition, m_AvatarSetup)) {
        ...
        using(var change = new EditorGUI.ChangeCheckScope()) {
          var value = (ModelImporterAvatarSetup) EditorGUILayout.EnumPopup(
              ...
              (ModelImporterAvatarSetup) m_AvatarSetup.intValue);
          if (change.changed)
    ...
    if (!m_AvatarSetup.hasMultipleDifferentValues) {
      if (m_AvatarSetup.intValue ==
          (int) ModelImporterAvatarSetup.CreateFromThisModel) {
        ...
        using(new EditorGUI.DisabledScope(!m_CanMultiEditTransformList)) {
          if (assetTarget == null) {
            m_RootMotionBoneName.stringValue = EditorGUILayout.TextField(
                Styles.RootNode, m_RootMotionBoneName.stringValue);
          } else {
            EditorGUI.BeginChangeCheck();
            var currentIndex = ArrayUtility.FindIndex(
                m_RootMotionBoneList,
                content => FileUtil.GetLastPathNameComponent(content.text) ==
                           m_RootMotionBoneName.stringValue);
            currentIndex = currentIndex < 1 ? 0 : currentIndex;
            currentIndex = EditorGUILayout.Popup(Styles.RootNode, currentIndex,
                                                 m_RootMotionBoneList);
            if (EditorGUI.EndChangeCheck()) {
              if (currentIndex > 0 &&
                  currentIndex < m_RootMotionBoneList.Length) {
                m_RootMotionBoneName.stringValue =
                    FileUtil.GetLastPathNameComponent(
                        m_RootMotionBoneList[currentIndex].text);
              } else {
                m_RootMotionBoneName.stringValue = "";
              }
            }
      ...
      } else if (m_AvatarSetup.intValue ==
                 (int) ModelImporterAvatarSetup.CopyFromOther)
        CopyAvatarGUI();
    }
  }
  ...
  void HumanoidGUI() {
    using(var horizontal = new EditorGUILayout.HorizontalScope()) {
      using(var propertyField = new EditorGUI.PropertyScope(
        ...
        using(var change = new EditorGUI.ChangeCheckScope()) {
          ...
          var value = (ModelImporterAvatarSetup) EditorGUI.EnumPopup(
              r, propertyField.content,
              (ModelImporterAvatarSetup) m_AvatarSetup.intValue,
              e =>(ModelImporterAvatarSetup)
                      e != ModelImporterAvatarSetup.NoAvatar);
          if (change.changed)
            m_AvatarSetup.intValue = (int) value;
        }
    ...
    if (m_IsBiped) {
      if (m_BipedMappingReport.Count > 0) {
        ...
        for (int reportIter = 0; reportIter < m_BipedMappingReport.Count;
             reportIter++) {
  ...
  void ConfigureAvatarGUI() {
    if (targets.Length > 1) {
      GUILayout.Label("Can't configure avatar in multi-editing mode",
    ...
    if (singleImporter.transformPaths.Length <= HumanTrait.RequiredBoneCount) {
      GUILayout.Label(
          string.Format(
              "Not enough bones to create human avatar (requires {0})",
    ...
    GUI.Label(new Rect(r.xMax - buttonWidth - 18, r.y, 18, r.height),
              validationContent, EditorStyles.label);

    // Configure button
    using(new EditorGUI.DisabledScope(m_Avatar == null)) {
      if (GUI.Button(new Rect(r.xMax - buttonWidth, r.y + 1, buttonWidth,
                              r.height - 1),
                     Styles.configureAvatar, EditorStyles.miniButton)) {
        if (!isLocked) {
    ...
    if (sourceAvatar != null) {
      if (sourceAvatar.isHuman &&
        ...
        if (EditorUtility.DisplayDialog(
                "Assigning a Humanoid Avatar on a Generic Rig",
                "Do you want to change Animation Type to Humanoid ?", "Yes",
                "No")) {
          animationType = ModelImporterAnimationType.Human;
        } else {
          m_AvatarSource.objectReferenceValue = null;
        }
      } else if (!sourceAvatar.isHuman &&
                 (animationType != ModelImporterAnimationType.Generic)) {
        if (EditorUtility.DisplayDialog(
                "Assigning a Generic Avatar on a Humanoid Rig",
                "Do you want to change Animation Type to Generic ?", "Yes",
                "No")) {
          animationType = ModelImporterAnimationType.Generic;
        } else {
          m_AvatarSource.objectReferenceValue = null;
...
With this option, this model will not create any avatar but only import animations.", EditorStyles.helpBox);
      ...
      m_AvatarCopyIsUpToDate = true;
        ...
        {
      if (GUILayout.Button(Styles.UpdateMuscleDefinitionFromSource,
  ...
  Object ValidateAvatarSource(Object[] references, Type objType,
  ...
  void ShowUpdateReferenceClip() {
    if (targets.Length > 1 ||
        m_AvatarSetup.intValue == (int)
                                      ModelImporterAvatarSetup.CopyFromOther ||
        !m_Avatar || !m_Avatar.isValid)
      return;
    ...
    if (paths.Length > 0 &&
        GUILayout.Button(Styles.UpdateReferenceClips, GUILayout.Width(150))) {
  ...
  public override void OnInspectorGUI() {
    ...
    // Animation type
    EditorGUI.BeginChangeCheck();
    EditorGUILayout.Popup(m_AnimationType, Styles.AnimationTypeOpt,
                          Styles.AnimationType);
    if (EditorGUI.EndChangeCheck()) {
      m_AvatarSource.objectReferenceValue = null;

      if (animationType == ModelImporterAnimationType.Legacy)
        m_AnimationCompression.intValue =
    ...
    if (animationType != ModelImporterAnimationType.None ||
        m_AnimationType.hasMultipleDifferentValues) {
      EditorGUILayout.Popup(m_SkinWeightsMode, Styles.SkinWeightsModeOpt,
                            Styles.SkinWeightsMode);

      if (m_SkinWeightsMode.intValue == (int) ModelImporterSkinWeights.Custom) {
        EditorGUILayout.IntSlider(m_MaxBonesPerVertex, 1, 255,
                                  Styles.MaxBonesPerVertex);
        EditorGUILayout.Slider(m_MinBoneWeight, 0.001f, 0.5f,
                               Styles.MinBoneWeight);
      }
    }

    ShowUpdateReferenceClip();

    // OptimizeGameObject is only supported on our own avatar when animation
    // type is not Legacy.
    if (m_AvatarSetup.intValue ==
            (int) ModelImporterAvatarSetup.CreateFromThisModel &&
      ...
      if (m_OptimizeGameObjects.boolValue &&
          serializedObject.targetObjectsCount ==
              1) // SerializedProperty can't handle multiple string arrays
                 // properly.
      {
        bool wasChanged = GUI.changed;
        m_ExtraExposedTransformFoldout = EditorGUILayout.Foldout(
            m_ExtraExposedTransformFoldout, Styles.ExtraExposedTransform, true);
        GUI.changed = wasChanged;
        if (m_ExtraExposedTransformFoldout) {
          // Do not allow multi edit of exposed transform list if all rigs
          // doesn't match
          using(new EditorGUI.DisabledScope(
              !m_CanMultiEditTransformList)) using(new EditorGUI
                                                       .IndentLevelScope())
    ...
    if (errors.Length > 0 || warnings.Length > 0) {
      ...
      importMessageFoldout = EditorGUILayout.Foldout(
          importMessageFoldout, Styles.ImportMessages, true);
  ...
  static bool DoesHumanDescriptionMatch(ModelImporter importer,
                                        ModelImporter otherImporter) {
    SerializedObject so =
        new SerializedObject(new Object[]{importer, otherImporter});

    so.maxArraySizeForMultiEditing = Math.Max(
        importer.transformPaths.Length, otherImporter.transformPaths.Length);
  ...
  static void CopyHumanDescriptionToDestination(SerializedObject sourceObject,
  ...
  private void CopyHumanDescriptionFromOtherModel(Avatar sourceAvatar) {
    ...
    SerializedObject srcImporter =
        GetModelImporterSerializedObject(srcAssetPath);
  ...
  private void SetupReferencedClip(string otherModelImporterPath) {
    SerializedObject targetImporter =
        GetModelImporterSerializedObject(otherModelImporterPath);

    // We may receive a path that doesn't have a importer.
    if (targetImporter != null) {
      targetImporter.CopyFromSerializedProperty(
          serializedObject.FindProperty("m_AnimationType"));

      SerializedProperty copyAvatar =
          targetImporter.FindProperty("m_CopyAvatar");
      if (copyAvatar != null)
        copyAvatar.boolValue = true;

      SerializedProperty avatar =
          targetImporter.FindProperty("m_LastHumanDescriptionAvatarSource");
      if (avatar != null)
    ...
    get {
      foreach (InspectorWindow i in InspectorWindow.GetAllInspectorWindows()) {
        ...
        foreach (Editor e in activeEditor.activeEditors) {
          // the tab is no longer an editor, so we must always refer to the
          // panel container
          if (e is ModelImporterEditor &&
              ((ModelImporterEditor) e).activeTab == this) {
            return i.isLocked;
          }
        }
      }
  ...
  internal override void PostApply() { UpdateBipedMappingReport(); }
}
}
