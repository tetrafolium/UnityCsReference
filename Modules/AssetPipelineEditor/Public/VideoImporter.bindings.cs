...
[NativeHeader("Modules/AssetPipelineEditor/Public/VideoClipImporter.h")]
[NativeHeader(
    "Modules/AssetPipelineEditor/Public/VideoClipImporter.bindings.h")]
public partial class VideoClipImporter : AssetImporter {
  [System.ComponentModel.EditorBrowsable(
      System.ComponentModel.EditorBrowsableState.Never)]
  [Obsolete(
      "VideoClipImporter.quality has no effect anymore (was only used for MovieTexture which is removed)",
      false)]
  ...
  [System.ComponentModel.EditorBrowsable(
      System.ComponentModel.EditorBrowsableState.Never)]
  [Obsolete(
      "VideoClipImporter.linearColor has no effect anymore (was only used for MovieTexture which is removed)",
      false)]
  ...
  [System.ComponentModel.EditorBrowsable(
      System.ComponentModel.EditorBrowsableState.Never)]
  [Obsolete(
      "VideoClipImporter.useLegacyImporter has no effect anymore (was only used for MovieTexture which is removed)",
      false)]
  public bool useLegacyImporter {
    get { return false; }
    set {}
  }
  ...
  public VideoImporterTargetSettings GetTargetSettings(string platform) {
    BuildTargetGroup platformGroup =
        GetBuildTargetGroup("GetTargetSetting", platform);
    return Internal_GetTargetSettings(platformGroup);
  }

  internal VideoImporterTargetSettings
  Internal_GetTargetSettings(BuildTargetGroup group) {
    return Private_GetTargetSettings(group) as VideoImporterTargetSettings;
  }

  [FreeFunction(Name = "VideoImporterBindings::GetTargetSettings",
                HasExplicitThis = true)]
  private extern object Private_GetTargetSettings(BuildTargetGroup group);

  public void SetTargetSettings(string platform,
  ...
  Internal_SetTargetSettings(BuildTargetGroup group,
                             VideoImporterTargetSettings settings);

  public void ClearTargetSettings(string platform) {
    var platformGroup =
        GetBuildTargetGroup("ClearTargetSettings", platform, false);
    Internal_ClearTargetSettings(platformGroup);
  }

  private BuildTargetGroup GetBuildTargetGroup(string methodName,
                                               string platform,
                                               bool acceptDefault = true) {
    if (!acceptDefault && platform.Equals(VideoClipImporter.defaultTargetName,
                                          StringComparison.OrdinalIgnoreCase))
      throw new ArgumentException(
          "Cannot call VideoClipImporter." + methodName +
          " for the default VideoImporterTargetSettings.");

    BuildTargetGroup platformGroup =
        BuildPipeline.GetBuildTargetGroupByName(platform);
    if (!platform.Equals(VideoClipImporter.defaultTargetName,
                         StringComparison.OrdinalIgnoreCase) &&
        platformGroup == BuildTargetGroup.Unknown) {
      var platformList =
          "'Standalone', 'Android', 'iOS', 'Lumin', 'PS4', 'Switch', 'tvOS', 'WebGL', 'WSA', 'WebGL' or 'XboxOne'";
      if (acceptDefault)
        platformList = "'Default', " + platformList;

      throw new ArgumentException(
          "Unknown platform passed to VideoClipImporter." + methodName + " (" +
          platform + "), please use one of " + platformList + ".");
    }
  ...
  public extern bool isPlayingPreview { [NativeName("Started")] get; }
  public extern Texture GetPreviewTexture();

  internal extern static string defaultTargetName {
    [NativeName("DefaultSettingsName")] get;
  }

  [FreeFunction("VideoImporterBindings::EqualsDefaultTargetSettings",
                HasExplicitThis = true)]
  internal extern bool
  EqualsDefaultTargetSettings(VideoImporterTargetSettings settings);
  ...
  [NativeMethod("operator==")] extern public bool Equals(VideoClipImporter rhs);
}
