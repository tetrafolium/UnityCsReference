...
static NativeFormatImporterUtility() {
	foreach (var type in TypeCache
	         .GetTypesWithAttribute<AssetFileNameExtensionAttribute>()) {
		var attr =
			type.GetCustomAttributes(typeof(AssetFileNameExtensionAttribute),
			                         false)[0] as AssetFileNameExtensionAttribute;
		try {
			RegisterExtensionForType(type, attr.preferredExtension,
			                         attr.otherExtensions.ToArray());
...
static readonly Dictionary<Type, string[]> s_RegisteredExtensionsByType =
	new Dictionary<Type, string[]>();

internal static void
RegisterExtensionForType(Type type, string preferredExtension,
                         params string[] otherExtensions) {
...
static string ValidateExtension(string extension, Type requestingType) {
	...
	var registeredType = Internal_GetNativeTypeForExtension(
		extension, out nativeExtensionRegistered);
...
[FreeFunction(
	 Name = "NativeFormatImporter::Internal_GetNativeTypeForExtension",
	 IsThreadSafe = true)]
static extern Type Internal_GetNativeTypeForExtension(string extension,
                                                      out bool registered);
...
internal static string GetExtensionForAsset(UnityObject asset) {
	...
	// prefer the most specific type that matches (matches native
	// implementation)
...
[FreeFunction(
	 Name = "NativeFormatImporter::Internal_GetExtensionForNativeAsset",
	 IsThreadSafe = true)]
