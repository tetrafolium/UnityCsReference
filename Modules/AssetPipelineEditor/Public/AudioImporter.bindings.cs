...
[System.Obsolete(
	 "UnityEditor.AudioImporterFormat has been deprecated. Use UnityEngine.AudioCompressionFormat instead.")]
...
[System.ComponentModel.EditorBrowsable(
	 System.ComponentModel.EditorBrowsableState.Never)]
[System.Obsolete(
	 "UnityEditor.AudioImporterLoadType has been deprecated. Use UnityEngine.AudioClipLoadType instead (UnityUpgradable) -> [UnityEngine] UnityEngine.AudioClipLoadType",
	 true)]
public enum AudioImporterLoadType {
	DecompressOnLoad = -1,
	CompressedInMemory = -1,
	[ System.Obsolete(
		  "UnityEditor.AudioImporterLoadType.StreamFromDisc has been deprecated. Use UnityEngine.AudioClipLoadType.Streaming instead (UnityUpgradable) -> UnityEngine.AudioClipLoadType.Streaming",
		  true) ] StreamFromDisc = -1
}

[System.Obsolete(
	 "Setting and getting import channels is not used anymore (use forceToMono instead)",
	 true)]
...
// Audio importer lets you modify [[AudioClip]] import settings from editor
// scripts.
...
public bool ContainsSampleSettingsOverride(string platform) {
	BuildTargetGroup platformGroup =
		BuildPipeline.GetBuildTargetGroupByName(platform);
	if (platformGroup == BuildTargetGroup.Unknown) {
		Debug.LogError(
			"Unknown platform passed to AudioImporter.ContainsSampleSettingsOverride (" +
			platform + "), please use one of " +
...
Internal_ContainsSampleSettingsOverride(BuildTargetGroup platformGroup);

public AudioImporterSampleSettings
GetOverrideSampleSettings(string platform) {
	BuildTargetGroup platformGroup =
		BuildPipeline.GetBuildTargetGroupByName(platform);
	if (platformGroup == BuildTargetGroup.Unknown) {
		Debug.LogError(
			"Unknown platform passed to AudioImporter.GetOverrideSampleSettings (" +
			platform + "), please use one of " +
			"'Web', 'Standalone', 'iOS', 'Android', 'WebGL', 'PS4', 'XboxOne' or 'WSA'");
...
internal extern AudioImporterSampleSettings
...
public bool SetOverrideSampleSettings(string platform,
	...
	if (platformGroup == BuildTargetGroup.Unknown) {
		Debug.LogError(
			"Unknown platform passed to AudioImporter.SetOverrideSampleSettings (" +
			platform + "), please use one of " +
			"'Web', 'Standalone', 'iOS', 'Android', 'WebGL', 'PS4', 'XboxOne' or 'WSA'");
		return false;
	}
...
public bool ClearSampleSettingOverride(string platform) {
	...
	if (platformGroup == BuildTargetGroup.Unknown) {
		Debug.LogError(
			"Unknown platform passed to AudioImporter.ClearSampleSettingOverride (" +
			platform + "), please use one of " +
			"'Web', 'Standalone', 'iOS', 'Android', 'WebGL', 'PS4', 'XboxOne' or 'WSA'");
		return false;
	}
...
internal extern bool
Internal_ClearSampleSettingOverride(BuildTargetGroup platform);
...
[System.Obsolete(
	 "Setting and getting the compression format is not used anymore (use compressionFormat in defaultSampleSettings instead). Source audio file is assumed to be PCM Wav.")] AudioImporterFormat
format {
	get {
		return (defaultSampleSettings.compressionFormat ==
	...
	set {
		...
		settings.compressionFormat = (value == AudioImporterFormat.Native)
		                       ? AudioCompressionFormat.PCM
...
// Compression bitrate.
[System.Obsolete("AudioImporter.compressionBitrate is no longer supported",
                 true)]
...
[System.Obsolete(
	 "AudioImporter.loopable is no longer supported. All audio assets encoded by Unity are by default loopable.")]
...
[System.Obsolete(
	 "AudioImporter.hardware is no longer supported. All mixing of audio is done by software and only some platforms use hardware acceleration to perform decoding.")]
...
[System.Obsolete(
	 "Setting/Getting decompressOnLoad is deprecated. Use AudioImporterSampleSettings.loadType instead.")] bool
decompressOnLoad {
	get {
		return (defaultSampleSettings.loadType ==
		        AudioClipLoadType.DecompressOnLoad);
	}
	set {
		AudioImporterSampleSettings settings = defaultSampleSettings;
		settings.loadType =
			value ? AudioClipLoadType.DecompressOnLoad
...
[System.Obsolete(
	 "AudioImporter.quality is no longer supported. Use AudioImporterSampleSettings.")] float
quality {
	get { return defaultSampleSettings.quality; }
	set {
		AudioImporterSampleSettings settings = defaultSampleSettings;
...
//*undocumented* Update/cache audio info. important to call this before any of
//the next. you only need to call it once per audiofile
...
internal int origChannelCount {
	get { return 0; }
}

// Is original source compressible to Ogg/MP3 (depending on platform)?  call
// updateOrigData before
[System.Obsolete("AudioImporter.origIsCompressible is deprecated.", true)]
internal bool origIsCompressible {
	get { return false; }
}
...
internal bool origIsMonoForcable {
	get { return false; }
}

//*undocumented* Min bitrate for ogg/mp3 compression.  call updateOrigData
//before
...
//*undocumented* Max bitrate for ogg/mp3 compression. call updateOrigData
//before
