...
// Helper class to generate form data to post to web servers using the [[WWW]]
// class.
public class WWWForm {
  ...
  public WWWForm() {
    ...
    for (int i = 0; i < 40; i++) {
      ...
      if (randomChar >
          57) // skip unprintable chars between 57 and 64 (inclusive)
        randomChar += 7;
  ...
  // Add binary data to the form.
  public void AddBinaryData(string fieldName, byte[] contents,
                            [ DefaultValue("null") ] string fileName,
                            [ DefaultValue("null") ] string mimeType) {
    containsFiles = true;

    // We handle png files automatically as we suspect people will be uploading
    // png files a lot due to the new screen shot feature. If we want to add
    // support for detecting other file types, we will need to do it in a more
    // extensible way.
    bool isPng =
        contents.Length > 8 && contents[0] == 0x89 && contents[1] == 0x50 &&
        contents[2] == 0x4e && contents[3] == 0x47 && contents[4] == 0x0d &&
        contents[5] == 0x0a && contents[6] == 0x1a && contents[7] == 0x0a;
    if (fileName == null) {
  ...
  // (RO) Returns the correct request headers for posting the form using the
  // [[WWW]] class.
    ...
    get {
      if (containsFiles) {
        ...
        byte[] dispositionHeader =
            DefaultEncoding.GetBytes("Content-disposition: form-data; name=\"");
        ...
        using(MemoryStream memStream = new MemoryStream(1024)) {
          for (int i = 0; i < formData.Count; i++) {
            ...
            memStream.Write(crlf, 0, (int) crlf.Length);
            memStream.Write(contentTypeHeader, 0,
                            (int) contentTypeHeader.Length);

            byte[] type = System.Text.Encoding.UTF8.GetBytes((string) types[i]);
            memStream.Write(type, 0, (int) type.Length);
            memStream.Write(crlf, 0, (int) crlf.Length);
            memStream.Write(dispositionHeader, 0,
                            (int) dispositionHeader.Length);

            string headerName = System.Text.Encoding.UTF8.HeaderName;
            // Headers must be 7 bit clean, so encode as per rfc1522 using
            // quoted-printable if needed.
            string encodedFieldName = (string) fieldNames[i];
            if (!WWWTranscoder.SevenBitClean(encodedFieldName,
                                             System.Text.Encoding.UTF8) ||
                encodedFieldName.IndexOf("=?") > -1) {
              encodedFieldName =
                  "=?" + headerName + "?Q?" +
                  WWWTranscoder.QPEncode(encodedFieldName,
                                         System.Text.Encoding.UTF8) +
                  "?=";
            }
            ...
            if (fileNames[i] != null) {
              ...
              if (!WWWTranscoder.SevenBitClean(encodedFileName,
                                               System.Text.Encoding.UTF8) ||
                  encodedFileName.IndexOf("=?") > -1) {
                encodedFileName =
                    "=?" + headerName + "?Q?" +
                    WWWTranscoder.QPEncode(encodedFileName,
                                           System.Text.Encoding.UTF8) +
                    "?=";
              }
              byte[] fileName =
                  System.Text.Encoding.UTF8.GetBytes(encodedFileName);
        ...
        using(MemoryStream memStream = new MemoryStream(1024)) {
          for (int i = 0; i < formData.Count; i++) {
            byte[] name = WWWTranscoder.DataEncode(
                System.Text.Encoding.UTF8.GetBytes((string) fieldNames[i]));
            byte[] formBytes = (byte[]) formData[i];
            byte[] value = WWWTranscoder.DataEncode(formBytes);

            if (i > 0)
              memStream.Write(ampersand, 0, (int) ampersand.Length);
            memStream.Write(name, 0, (int) name.Length);
...
internal class WWWTranscoder {
  private static byte[] ucHexChars =
      WWWForm.DefaultEncoding.GetBytes("0123456789ABCDEF");
  ...
  public static string URLEncode(string toEncode, Encoding e) {
    byte[] data = Encode(e.GetBytes(toEncode), urlEscapeChar, urlSpace,
                         urlForbidden, false);
  ...
  public static string DataEncode(string toEncode, Encoding e) {
    byte[] data = Encode(e.GetBytes(toEncode), urlEscapeChar, dataSpace,
                         urlForbidden, false);
    return WWWForm.DefaultEncoding.GetString(data, 0, data.Length);
  }
  ...
  public static string QPEncode(string toEncode, Encoding e) {
    byte[] data =
        Encode(e.GetBytes(toEncode), qpEscapeChar, qpSpace, qpForbidden, true);
    return WWWForm.DefaultEncoding.GetString(data, 0, data.Length);
  }
  ...
  public static byte[] Encode(byte[] input, byte escapeChar, byte[] space,
                              byte[] forbidden, bool uppercase) {
      ...
      for (int i = 0; i < input.Length; i++) {
        if (input[i] == 32) {
          memStream.Write(space, 0, space.Length);
                   ...
                   ByteArrayContains(forbidden, input[i])) {
          memStream.WriteByte(escapeChar);
          memStream.Write(Byte2Hex(input[i], uppercase ? ucHexChars
                                   : lcHexChars),
                          0, 2);
        } else {
  ...
  public static string URLDecode(string toEncode, Encoding e) {
    byte[] data = Decode(WWWForm.DefaultEncoding.GetBytes(toEncode),
                         urlEscapeChar, urlSpace);
  ...
  public static string DataDecode(string toDecode, Encoding e) {
    byte[] data = Decode(WWWForm.DefaultEncoding.GetBytes(toDecode),
                         urlEscapeChar, dataSpace);
    return e.GetString(data, 0, data.Length);
  }
  ...
  private static bool ByteSubArrayEquals(byte[] array, int index,
                                         byte[] comperand) {
...
static class WebRequestUtils {
  ...
  internal static string RedirectTo(string baseUri, string redirectUri) {
    Uri redirectURI;
    // On UNIX systems URI starting with / is misidentified as absolute path and
    // is considered absolute while it is actually a relative URI. Enforce that.
    if (redirectUri[0] == '/')
  ...
  internal static string MakeInitialUrl(string targetUrl, string localUrl) {
    ...
    try {
      // If URL starts with dot, it is relative and this would throw, skip to
      // combining
      if (targetUri == null && targetUrl[0] != '.')
        targetUri = new System.Uri(targetUrl);
    } catch (FormatException e1) {
      // Technically, this should be UriFormatException but MSDN says WSA/PCL
      // doesn't support UriFormatException, and recommends FormatException
      // instead See:
      // https://msdn.microsoft.com/en-us/library/system.uriformatexception%28v=vs.110%29.aspx
      ex = e1;
    }
  ...
  internal static string MakeUriString(Uri targetUri, string targetUrl,
    ...
    // Special handling for URIs like jar:file (Android), blob:http (WebGL and
    // similar Uri.AbsoluteUri class in those cases results in jar:file/path,
    // which is incorrect because of only one slash Uri.Scheme also returns
    // scheme part before the colon (jar, blob) so if we didn't prepend the
    // scheme and scheme has colon it it, construct the URI from it's parts
    var scheme = targetUri.Scheme;
    if (!prependProtocol && (targetUrl.Length >= scheme.Length + 2) &&
        targetUrl[scheme.Length + 1] != '/') {
      ...
      // jar:file URIs should be treated like file URIs (unescaped and stripped
      // of query&fragment)
      if (scheme == "jar") {
        string path = targetUri.AbsolutePath;
        if (path.Contains("%"))
        ...
        // common error when using Uri class and converting to string: URI will
        // be jar:file:/path instead of jar:file:///path
    ...
    // if URL contains '%', assume it is properly escaped, otherwise '%2f' gets
    // unescaped as '/' (which may not be correct) otherwise escape it, i.e.
