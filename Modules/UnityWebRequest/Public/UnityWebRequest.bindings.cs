...
namespace UnityEngine.Networking {
  ...
  [UsedByNativeCode]
  [NativeHeader(
      "Modules/UnityWebRequest/Public/UnityWebRequestAsyncOperation.h")]
  ...
  public partial class UnityWebRequest : IDisposable {
    ...
    public static void ClearCookieCache() { ClearCookieCache(null, null); }
    ...
    private extern UnityWebRequestError
    SetMethod(UnityWebRequestMethod methodType);

    internal void InternalSetMethod(UnityWebRequestMethod methodType) {
      if (!isModifiable)
        throw new InvalidOperationException(
            "UnityWebRequest has already been sent and its request method can no longer be altered");

      UnityWebRequestError ret = SetMethod(methodType);
      if (ret != UnityWebRequestError.OK)
        throw new InvalidOperationException(
            UnityWebRequest.GetWebErrorString(ret));
    }

    private extern UnityWebRequestError
    SetCustomMethod(string customMethodName);
      ...
      set {
        if (String.IsNullOrEmpty(value)) {
          throw new ArgumentException(
              "Cannot set a UnityWebRequest's method to an empty or null string");
        }
    ...
    public string error {
      get {
        ...
        case Result.ProtocolError:
          return string.Format("HTTP/1.1 {0} {1}", responseCode,
                               GetHTTPStatusString(responseCode));
        default:
    ...
    public bool useHttpContinue {
      ...
      set {
        if (!isModifiable)
          throw new InvalidOperationException(
              "UnityWebRequest has already been sent and its 100-Continue setting cannot be altered");
    ...
    public string url {
      ...
      set {
        ...
        // This permits us to allow relative URLs and correct minor user
        // mistakes.
    ...
    public Uri uri {
      ...
      set {
        ...
        InternalSetUrl(
            WebRequestUtils.MakeUriString(value, value.OriginalString, false));
        m_Uri = value;
      }
    }
    ...
    private void InternalSetUrl(string url) {
      if (!isModifiable)
        throw new InvalidOperationException(
            "UnityWebRequest has already been sent and its URL cannot be altered");
    ...
    public bool isDone {
      get { return result != Result.InProgress; }
    }
    // These two are referenced by packages, deprecate after packages get
    // updated
    [System.Obsolete(
    ...
    [Obsolete(
        "HTTP/2 and many HTTP/1.1 servers don't support this; we recommend leaving it set to false (default).",
        false)]
    public bool chunkedTransfer {
      ...
      set {
        if (!isModifiable)
          throw new InvalidOperationException(
              "UnityWebRequest has already been sent and its chunked transfer encoding setting cannot be altered");
    ...
    internal extern UnityWebRequestError InternalSetRequestHeader(string name,
                                                                  string value);

    public void SetRequestHeader(string name, string value) {
      if (String.IsNullOrEmpty(name))
        throw new ArgumentException(
            "Cannot set a Request Header with a null or empty name");

      // Only check for null here, as in general header value can be empty, i.e.
      // Accept-Encoding can have empty value according spec.
      if (value == null)
        throw new ArgumentException("Cannot set a Request header with a null");
      if (!isModifiable)
        throw new InvalidOperationException(
            "UnityWebRequest has already been sent and its request headers cannot be altered");
    ...
    public Dictionary<string, string> GetResponseHeaders() {
      ...
      Dictionary<string, string> headers = new Dictionary<string, string>(
          headerKeys.Length, StringComparer.OrdinalIgnoreCase);
    ...
    public UploadHandler uploadHandler {
      get { return m_UploadHandler; }
      set {
        if (!isModifiable)
          throw new InvalidOperationException(
              "UnityWebRequest has already been sent; cannot modify the upload handler");
    ...
    public DownloadHandler downloadHandler {
      ...
      set {
        if (!isModifiable)
          throw new InvalidOperationException(
              "UnityWebRequest has already been sent; cannot modify the download handler");
    ...
    private extern UnityWebRequestError
    SetCertificateHandler(CertificateHandler ch);

    public CertificateHandler certificateHandler {
      get { return m_CertificateHandler; }
      set {
        if (!isModifiable)
          throw new InvalidOperationException(
              "UnityWebRequest has already been sent; cannot modify the certificate handler");
    ...
    private extern UnityWebRequestError
    SetSuppressErrorsToConsole(bool suppress);
