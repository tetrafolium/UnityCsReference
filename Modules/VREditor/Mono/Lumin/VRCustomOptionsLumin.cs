...
static GUIContent[] s_DepthOptions = {new GUIContent("16-bit depth"),
	                              new GUIContent("24-bit depth")};

static GUIContent[] s_FrameTimingOptions = {
	new GUIContent("Unspecified"), new GUIContent("Maximum"),
	new GUIContent("60Hz"), new GUIContent("120Hz")
};
...
static GUIContent s_GlBlobCacheLabel =
	new GUIContent("Enable OpenGL Shader Cache File");
static GUIContent s_GlBlobMaxSizeLabel =
	new GUIContent("Max GL Blob Size in bytes");
static GUIContent s_GlFileMaxSizeLabel =
	new GUIContent("Max GL Cache File Size in bytes");

const string s_GLBlobToolTip =
	"Select to optimize application and use cached shader data.";
const string s_GlBlobMaxSizeToolTip =
	"The maximium size for each shader blob data, units in bytes.";
const string s_GlFileMaxSizeToolTip =
	"The maximium size for shader cache file, units in bytes.";
...
public override Rect Draw(BuildTargetGroup target, Rect rect) {
	...
	EditorGUI.BeginChangeCheck();
	intValue = EditorGUI.Popup(rect, label, m_FrameTiming.intValue,
	                           s_FrameTimingOptions);
	if (EditorGUI.EndChangeCheck()) {
	...
	label =
		EditorGUI.BeginProperty(rect, s_GlBlobCacheLabel, m_EnableGLCache);
	label.tooltip = s_GLBlobToolTip;
	EditorGUI.BeginChangeCheck();
	bool glCacheEnabled =
		EditorGUI.Toggle(rect, label, m_EnableGLCache.boolValue);
	if (EditorGUI.EndChangeCheck()) {
	...
	label =
		EditorGUI.BeginProperty(rect, s_GlBlobMaxSizeLabel, m_GLMaxBlobSize);
	label.tooltip = s_GlBlobMaxSizeToolTip;
	EditorGUI.BeginChangeCheck();
	int glMaxBlobSize =
		EditorGUI.IntField(rect, label, m_GLMaxBlobSize.intValue);
	if (EditorGUI.EndChangeCheck()) {
	...
	label =
		EditorGUI.BeginProperty(rect, s_GlFileMaxSizeLabel, m_GLMaxFileSize);
	label.tooltip = s_GlFileMaxSizeToolTip;
	EditorGUI.BeginChangeCheck();
	int glMaxFileSize =
		EditorGUI.IntField(rect, label, m_GLMaxFileSize.intValue);
	if (EditorGUI.EndChangeCheck()) {
