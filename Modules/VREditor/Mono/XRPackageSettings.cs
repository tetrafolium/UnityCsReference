...
namespace UnityEditorInternal.VR {
  internal class XRProjectSettings : ScriptableObject {
    internal static class KnownSettings {
      internal static readonly string k_VRDeviceDidAlertUser =
          "VR Device User Alert";
      internal static readonly string k_VRDeviceDisabled = "VR Device Disabled";
      internal static readonly string k_VRDeviceTransitionGroups =
      ...
      get {
        if (s_PackageSettings == null) {
          lock(s_Lock) {
            if (s_PackageSettings == null) {
              s_PackageSettings =
                  ScriptableObject.CreateInstance<XRProjectSettings>();
            }
          }
        }
    ...
    void OnEnable() { Internal_LoadSettings(); }

    void OnDisable() { Internal_SaveSettings(); }

    void OnDestroy() { Internal_SaveSettings(); }

    private string GetStorageFilePath() {
      if (String.IsNullOrEmpty(m_StorageFilePath))
        m_StorageFilePath =
            Path.Combine(k_ProjectSettingsPath, k_ProjectSettingsFileName);
    ...
    internal static void SaveSettings() { Instance.Internal_SaveSettings(); }
