...
private static Dictionary<BuildTargetGroup, bool> dirtyDeviceLists =
	new Dictionary<BuildTargetGroup, bool>();
...
public static VRDeviceInfoEditor[] GetEnabledVRDeviceInfo(
	BuildTargetGroup targetGroup) {
	string[] enabledVRDevices = GetVREnabledDevicesOnTargetGroup(targetGroup);
	return GetAllVRDeviceInfo(targetGroup)
	       .Where(d => enabledVRDevices.Contains(d.deviceNameKey))
	       .ToArray();
}
...
public static VRDeviceInfoEditor[] GetEnabledVRDeviceInfo(
	...
	return GetAllVRDeviceInfoByTarget(target)
	       .Where(d => enabledVRDevices.Contains(d.deviceNameKey))
	       .ToArray();
}

public static bool IsVRDeviceEnabledForBuildTarget(BuildTarget target,
                                                   string deviceName) {
...
public static string[] GetAvailableVirtualRealitySDKs(
	BuildTargetGroup targetGroup) {
...
public static void SetVirtualRealitySDKs(BuildTargetGroup targetGroup,
                                         string[] sdks) {
...
public static void SetVirtualRealitySupported(BuildTargetGroup targetGroup,
                                              bool value) {
