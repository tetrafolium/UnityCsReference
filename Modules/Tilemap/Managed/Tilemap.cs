  ...
  public partial class Tilemap {
    ...
    internal bool bufferSyncTile {
      ...
      set {
        if (value == false && m_BufferSyncTile != value &&
            HasSyncTileCallback() && syncTileBuffer.Count > 0)
    ...
    private void SendTilemapTileChangedCallback(SyncTile[] syncTiles) {
      try {
        ...
        // Case 1215834: Log user exception/s and ensure engine code continues
        // to run
    ...
    SetSyncTileCallback(Action<Tilemap, SyncTile[]> callback) {
      Tilemap.tilemapTileChanged += callback;
    }

    internal static void
    RemoveSyncTileCallback(Action<Tilemap, SyncTile[]> callback) {
      Tilemap.tilemapTileChanged -= callback;
    }
  }
}
