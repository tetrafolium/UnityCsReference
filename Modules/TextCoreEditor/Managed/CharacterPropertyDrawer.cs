...
namespace UnityEditor.TextCore {
  ...
  class CharacterPropertyDrawer : PropertyDrawer {
    ...
    public override void OnGUI(Rect position, SerializedProperty property,
                               GUIContent label) {
      SerializedProperty prop_Unicode =
          property.FindPropertyRelative("m_Unicode");
      SerializedProperty prop_GlyphIndex =
          property.FindPropertyRelative("m_GlyphIndex");
      ...
      if (GUI.enabled == false) {
        ...
        EditorGUI.LabelField(new Rect(rect.x, rect.y, 120f, 18),
                             new GUIContent("Unicode: <color=#FFFF80>0x" +
                                            unicode.ToString("X") + "</color>"),
                             style);
        EditorGUI.LabelField(
            new Rect(rect.x + 115, rect.y, 120f, 18),
            unicode <= 0xFFFF
                ? new GUIContent("UTF16: <color=#FFFF80>\\u" +
                                 unicode.ToString("X4") + "</color>")
                : new GUIContent("UTF32: <color=#FFFF80>\\U" +
                                 unicode.ToString("X8") + "</color>"),
            style);
        EditorGUI.LabelField(new Rect(rect.x, rect.y + 18, 120, 18),
                             new GUIContent("Glyph ID: <color=#FFFF80>" +
                                            prop_GlyphIndex.intValue +
                                            "</color>"),
                             style);
        EditorGUI.LabelField(new Rect(rect.x, rect.y + 36, 80, 18),
                             new GUIContent("Scale: <color=#FFFF80>" +
                                            prop_Scale.floatValue + "</color>"),
                             style);
      ...
      } else // Display editable fields
      {
        ...
        string unicode = EditorGUI.TextField(
            new Rect(rect.x, rect.y, 120, 18),
            "Unicode:", prop_Unicode.intValue.ToString("X"));
        ...
        // Cache current glyph index in case it needs to be restored if the new
        // glyph index is invalid.
        int currentGlyphIndex = prop_GlyphIndex.intValue;

        EditorGUIUtility.labelWidth = 59f;
        EditorGUI.BeginChangeCheck();
        EditorGUI.DelayedIntField(new Rect(rect.x, rect.y + 18, 100, 18),
                                  prop_GlyphIndex, new GUIContent("Glyph ID:"));
        if (EditorGUI.EndChangeCheck()) {
          // Get a reference to the font asset
          FontAsset fontAsset =
              property.serializedObject.targetObject as FontAsset;

          // Make sure new glyph index is valid.
          int elementIndex = fontAsset.glyphTable.FindIndex(
              item => item.index == prop_GlyphIndex.intValue);
        ...
        if (GUI.Button(new Rect(rect.x + 120, rect.y + 18, 75, 18),
                       new GUIContent("Edit Glyph"))) {
        ...
        if (glyphIndex == m_GlyphSelectedForEditing && GUI.enabled) {
          ...
          if (fontAsset != null) {
            ...
            if (elementIndex != -1) {
              SerializedProperty prop_GlyphTable =
                  property.serializedObject.FindProperty("m_GlyphTable");
              SerializedProperty prop_Glyph =
                  prop_GlyphTable.GetArrayElementAtIndex(elementIndex);

              SerializedProperty prop_GlyphMetrics =
                  prop_Glyph.FindPropertyRelative("m_Metrics");
              SerializedProperty prop_GlyphRect =
                  prop_Glyph.FindPropertyRelative("m_GlyphRect");

              Rect newRect = EditorGUILayout.GetControlRect(false, 115);
              EditorGUI.DrawRect(new Rect(newRect.x + 52, newRect.y - 20,
                                          newRect.width - 52,
                                          newRect.height - 5),
                                 new Color(0.1f, 0.1f, 0.1f, 0.45f));
              EditorGUI.DrawRect(new Rect(newRect.x + 53, newRect.y - 19,
                                          newRect.width - 54,
                                          newRect.height - 7),
        ...
        EditorGUI.PropertyField(new Rect(rect.x, rect.y + 36, 80, 18),
                                prop_Scale, new GUIContent("Scale:"));
    ...
    void DrawGlyph(Rect position, SerializedProperty property) {
      ...
      SerializedProperty glyphTableProperty =
          property.serializedObject.FindProperty("m_GlyphTable");
      ...
      SerializedProperty glyphProperty =
          glyphTableProperty.GetArrayElementAtIndex(elementIndex);

      // Get reference to atlas texture.
      int atlasIndex =
          glyphProperty.FindPropertyRelative("m_AtlasIndex").intValue;
      SerializedProperty atlasTextureProperty =
          so.FindProperty("m_AtlasTextures");
      Texture2D atlasTexture =
          atlasTextureProperty.GetArrayElementAtIndex(atlasIndex)
      ...
      GlyphRenderMode atlasRenderMode =
          (GlyphRenderMode) so.FindProperty("m_AtlasRenderMode").intValue;
      int atlasPadding = so.FindProperty("m_AtlasPadding").intValue;

      if (((GlyphRasterModes) atlasRenderMode &
      ...
      int glyphOriginX =
          glyphRectProperty.FindPropertyRelative("m_X").intValue - padding;
      int glyphOriginY =
          glyphRectProperty.FindPropertyRelative("m_Y").intValue - padding;
      int glyphWidth =
      ...
      float ascentLine =
          faceInfoProperty.FindPropertyRelative("m_AscentLine").floatValue;
      float descentLine =
          faceInfoProperty.FindPropertyRelative("m_DescentLine").floatValue;
      ...
      if (Event.current.type == EventType.Repaint) {
        glyphDrawPosition.x +=
            (glyphDrawPosition.width - glyphWidth * scale) / 2;
        glyphDrawPosition.y +=
            (glyphDrawPosition.height - glyphHeight * scale) / 2;
        glyphDrawPosition.width = glyphWidth * scale;
        glyphDrawPosition.height = glyphHeight * scale;

        // Could switch to using the default material of the font asset which
        // would require passing scale to the shader.
        Graphics.DrawTexture(glyphDrawPosition, atlasTexture, texCoords, 0, 0,
                             0, 0, new Color(1f, 1f, 1f), mat);
      }
    }
    ...
    int GetElementIndex(SerializedProperty glyphTableProperty, int glyphIndex) {
      ...
      for (int i = 0; i < elementCount; i++) {
        SerializedProperty glyphProperty =
            glyphTableProperty.GetArrayElementAtIndex(i);

        if (glyphIndex ==
            glyphProperty.FindPropertyRelative("m_Index").intValue)
          return i;
      }
