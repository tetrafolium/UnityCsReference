...
namespace UnityEditor.TextCore {
  static class TextSpriteAssetMenu {
    // Add a Context Menu to the Sprite Asset Editor Panel to Create and Add a
    // Default Material.
    ...
    // Add a Context Menu to the Sprite Asset Editor Panel to update existing
    // sprite assets.
    ...
    static void UpdateSpriteAsset(MenuCommand command) {
      ...
      // Get a list of all the sprites contained in the texture referenced by
      // the sprite asset. This only works if the texture is set to sprite mode.
      ...
      var tempGlyphTable =
          spriteGlyphTable.OrderBy(glyph => glyph.index).ToList();
      List<uint> availableGlyphIndexes = new List<uint>();

      int elementIndex = 0;
      for (uint i = 0; i < tempGlyphTable[tempGlyphTable.Count - 1].index;
           i++) {
      ...
      // Iterate over each of the sprites in the texture to try to match them to
      // existing sprites in the sprite asset.
      for (int i = 0; i < sprites.Length; i++) {
        int id = sprites [i]
                     .GetInstanceID();

        int glyphIndex = spriteGlyphTable.FindIndex(
            item => item.sprite.GetInstanceID() == id);
        ...
        if (glyphIndex == -1) {
          ...
          spriteGlyph.metrics = new GlyphMetrics(
              sprite.rect.width, sprite.rect.height, -sprite.pivot.x,
          ...
          if (spriteGlyph.glyphRect.x != sprite.rect.x ||
    ...
    static void CreateTextSpriteAsset() {
      ...
      if (target == null || target.GetType() != typeof(Texture2D)) {
        Debug.LogWarning(
            "A texture which contains sprites must first be selected in order to create a TextSprite asset.");
        return;
      }
      ...
      string fileNameWithoutExtension =
          Path.GetFileNameWithoutExtension(filePathWithName);
      string filePath = filePathWithName.Replace(fileNameWithExtension, "");

      // Check if Sprite Asset already exists
      TextSpriteAsset spriteAsset = AssetDatabase.LoadAssetAtPath(
          filePath + fileNameWithoutExtension + ".asset",
          typeof(TextSpriteAsset)) as TextSpriteAsset;
      ...
      if (isNewAsset) {
        ...
        AssetDatabase.CreateAsset(
            spriteAsset, filePath + fileNameWithoutExtension + ".asset");

        spriteAsset.version = "1.1.0";

        // Compute the hash code for the sprite asset.
        spriteAsset.hashCode =
            TextUtilities.GetHashCodeCaseInSensitive(spriteAsset.name);

        // Assign new Sprite Sheet texture to the Sprite Asset.
        spriteAsset.spriteSheet = sourceTex;

        List<SpriteGlyph> spriteGlyphTable = new List<SpriteGlyph>();
        List<SpriteCharacter> spriteCharacterTable =
            new List<SpriteCharacter>();

        PopulateSpriteTables(sourceTex, ref spriteCharacterTable,
                             ref spriteGlyphTable);
      ...
      AssetDatabase.ImportAsset(AssetDatabase.GetAssetPath(
          spriteAsset)); // Re-import font asset to get the new updated version.

      // AssetDatabase.Refresh();
    }

    static void
    PopulateSpriteTables(Texture source,
                         ref List<SpriteCharacter> spriteCharacterTable,
                         ref List<SpriteGlyph> spriteGlyphTable) {
