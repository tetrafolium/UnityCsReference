  ...
  static class FontAssetCreationMenu {
    ...
    public static void CreateFontAsset() {
      ...
      if (target == null || target.GetType() != typeof(Font)) {
        Debug.LogWarning(
            "A Font file must first be selected in order to create a Font Asset.");
        return;
      }
      ...
      if (FontEngine.LoadFontFace(sourceFont, 90) != FontEngineError.Success) {
        Debug.LogWarning("Unable to load font face for [" + sourceFont.name +
                             "].",
                         sourceFont);
        return;
      }
      ...
      fontAsset.m_SourceFontFileGUID =
          AssetDatabase.AssetPathToGUID(sourceFontFilePath);
      ...
      int packingModifier = ((GlyphRasterModes) fontAsset.atlasRenderMode &
      ...
      fontAsset.fontAssetCreationSettings = new FontAssetCreationSettings(
          fontAsset.m_SourceFontFileGUID, fontAsset.faceInfo.pointSize, 0,
          atlasPadding, 0, 1024, 1024, 7, string.Empty,
          (int) GlyphRenderMode.SDFAA);
