...
class FontAssetEditor : Editor
{
    ...
    internal static Material internalSDFMaterial
    {
        get
        {
            if (s_InternalSDFMaterial == null)
                s_InternalSDFMaterial = new Material(ShaderUtilities.ShaderRef_MobileSDF);
    ...
    internal static Material internalBitmapMaterial
    {
        get
        {
            if (s_InternalBitmapMaterial == null)
                s_InternalBitmapMaterial = new Material(ShaderUtilities.ShaderRef_MobileBitmap);
    ...
    public override void OnInspectorGUI()
    {
        ...
        // FACE INFO PANEL
        #region Face info
        GUI.Label(rect, new GUIContent("<b>Face Info</b> - v" + m_fontAsset.version), TextCoreEditorStyles.sectionHeader);
        ...
        {
            //TMPro_FontAssetCreatorWindow.ShowFontAtlasCreatorWindow(target as FontAsset);
        }
        ...
        if (GUI.Button(rect, new GUIContent("<b>Generation Settings</b>"), TextCoreEditorStyles.sectionHeader))
            UI_PanelState.generationSettingsPanel = !UI_PanelState.generationSettingsPanel;

        GUI.Label(rect, (UI_PanelState.generationSettingsPanel ? "" : s_UiStateLabel[1]), TextCoreEditorStyles.rightLabel);
        ...
        if (GUI.Button(rect, new GUIContent("<b>Atlas & Material</b>"), TextCoreEditorStyles.sectionHeader))
            UI_PanelState.fontAtlasInfoPanel = !UI_PanelState.fontAtlasInfoPanel;

        GUI.Label(rect, (UI_PanelState.fontAtlasInfoPanel ? "" : s_UiStateLabel[1]), TextCoreEditorStyles.rightLabel);
        ...
        if (GUI.Button(rect, new GUIContent("<b>Font Weights</b>", "The Font Assets that will be used for different font weights and the settings used to simulate a typeface when no asset is available."), TextCoreEditorStyles.sectionHeader))
            UI_PanelState.fontWeightPanel = !UI_PanelState.fontWeightPanel;

        GUI.Label(rect, (UI_PanelState.fontWeightPanel ? "" : s_UiStateLabel[1]), TextCoreEditorStyles.rightLabel);
        ...
        rect = EditorGUILayout.GetControlRect(false, 24);
        EditorGUI.indentLevel = 0;
        if (GUI.Button(rect, new GUIContent("<b>Fallback Font Assets</b>", "Select the Font Assets that will be searched and used as fallback when characters are missing from this font asset."), TextCoreEditorStyles.sectionHeader))
            UI_PanelState.fallbackFontAssetPanel = !UI_PanelState.fallbackFontAssetPanel;

        GUI.Label(rect, (UI_PanelState.fallbackFontAssetPanel ? "" : s_UiStateLabel[1]), TextCoreEditorStyles.rightLabel);
        ...
        if (GUI.Button(rect, new GUIContent("<b>Character Table</b>", "List of characters contained in this font asset."), TextCoreEditorStyles.sectionHeader))
            UI_PanelState.characterTablePanel = !UI_PanelState.characterTablePanel;

        GUI.Label(rect, (UI_PanelState.characterTablePanel ? "" : s_UiStateLabel[1]), TextCoreEditorStyles.rightLabel);
            ...
            if (arraySize > 0)
            {
                ...
                for (int i = itemsPerPage * m_CurrentCharacterPage; i < arraySize && i < itemsPerPage * (m_CurrentCharacterPage + 1); i++)
                {
                    ...
                    if (m_SelectedCharacterRecord == i)
                    {
                        ...
                        // Placeholder text
                        EditorGUI.LabelField(position, new GUIContent(m_unicodeHexLabel, "The Unicode (Hex) ID of the duplicated Character"), TextCoreEditorStyles.label);
        ...
        if (GUI.Button(rect, new GUIContent("<b>Glyph Table</b>", "List of glyphs contained in this font asset."), TextCoreEditorStyles.sectionHeader))
            UI_PanelState.glyphTablePanel = !UI_PanelState.glyphTablePanel;

        GUI.Label(rect, (UI_PanelState.glyphTablePanel ? "" : s_UiStateLabel[1]), TextCoreEditorStyles.rightLabel);
            ...
            if (arraySize > 0)
            {
                ...
                for (int i = itemsPerPage * m_CurrentGlyphPage; i < arraySize && i < itemsPerPage * (m_CurrentGlyphPage + 1); i++)
                {
                    ...
                    if (m_SelectedGlyphRecord == i)
                    {
                        ...
                        // Placeholder text
                        EditorGUI.LabelField(position, new GUIContent(m_GlyphIDLabel, "The Glyph ID of the duplicated Glyph"), TextCoreEditorStyles.label);
        ...
        if (GUI.Button(rect, new GUIContent("<b>Glyph Adjustment Table</b>", "List of glyph adjustment / advanced kerning pairs."), TextCoreEditorStyles.sectionHeader))
            UI_PanelState.kerningInfoPanel = !UI_PanelState.kerningInfoPanel;

        GUI.Label(rect, (UI_PanelState.kerningInfoPanel ? "" : s_UiStateLabel[1]), TextCoreEditorStyles.rightLabel);
        ...
        if (UI_PanelState.kerningInfoPanel)
        {
            ...
            if (errorCode == -1)
            {
                ...
                GUILayout.Label("Kerning Pair already <color=#ffff00>exists!</color>", TextCoreEditorStyles.label);
                GUILayout.FlexibleSpace();
    ...
    {
        m_fontAsset.m_FontAssetCreationSettings.pointSize = m_SamplingPointSize_prop.intValue;
        m_fontAsset.m_FontAssetCreationSettings.renderMode = m_AtlasRenderMode_prop.intValue;
        m_fontAsset.m_FontAssetCreationSettings.padding = m_AtlasPadding_prop.intValue;
        m_fontAsset.m_FontAssetCreationSettings.atlasWidth = m_AtlasWidth_prop.intValue;
        m_fontAsset.m_FontAssetCreationSettings.atlasHeight = m_AtlasHeight_prop.intValue;
    }
    ...
    void DisplayPageNavigation(ref int currentPage, int arraySize, int itemsPerPage)
    {
        ...
        GUI.Label(pagePos, "Page " + (currentPage + 1) + " / " + totalPages, TextCoreEditorStyles.centeredLabel);
