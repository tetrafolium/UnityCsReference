...
public partial class WWW : CustomYieldInstruction,
       IDisposable {
...
[Obsolete(
	 "This overload is deprecated. Use UnityEngine.WWW.WWW(string, byte[], System.Collections.Generic.Dictionary<string, string>) instead.")]
public WWW(string url, byte[] postData, Hashtable headers) {
	var verb = postData == null ? UnityWebRequest.kHttpVerbGET
...
internal WWW(string url, string name, Hash128 hash, uint crc) {
	_uwr = UnityWebRequestAssetBundle.GetAssetBundle(
		url, new CachedAssetBundle(name, hash), crc);
	_uwr.SendWebRequest();
}
...
[Obsolete(
	 "Obsolete msg (UnityUpgradable) -> * UnityEngine.WWW.GetAudioClip()",
	 true)]
...
[System.ComponentModel.EditorBrowsable(
	 System.ComponentModel.EditorBrowsableState.Never)]
[Obsolete(
	 "Obsolete msg (UnityUpgradable) -> * UnityEngine.WWW.GetMovieTexture()",
	 true)]
public Object movie {
	get { return null; }
}
...
public string error {
	get {
		...
		if (_uwr.responseCode >= 400) {
			var statusString =
				UnityWebRequest.GetHTTPStatusString(_uwr.responseCode);
...
public float progress {
	get {
		...
		// UWR returns negative if not sent yet, WWW always returns between 0 and
		// 1
		if (progress < 0)
			progress = 0.0f;
		return progress;
	...
	get {
		...
		if (_responseHeaders == null) {
			...
			if (_responseHeaders != null) {
				var statusString =
					UnityWebRequest.GetHTTPStatusString(_uwr.responseCode);
				_responseHeaders["STATUS"] = string.Format(
					"HTTP/1.1 {0} {1}", _uwr.responseCode, statusString);
			} else
...
public Texture2D texture {
	get { return CreateTextureFromDownloadedData(false); }
}

public Texture2D textureNonReadable {
	get { return CreateTextureFromDownloadedData(true); }
}
...
public override bool keepWaiting {
	get { return _uwr == null ? false : !_uwr.isDone; }
}
...
internal Object GetAudioClipInternal(bool threeD, bool stream,
                                     bool compressed, AudioType audioType) {
	return WebRequestWWW.InternalCreateAudioClipUsingDH(
		_uwr.downloadHandler, _uwr.url, stream, compressed, audioType);
}
...
public MovieTexture GetMovieTexture() {
	throw new Exception(
		      "MovieTexture has been removed from Unity. Use VideoPlayer instead.");
}
