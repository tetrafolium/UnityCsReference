...
namespace UnityEditor.DeploymentTargets {
  internal struct DeploymentTargetId {
    // Meta-targets, to be able to select targets that does not depend on the
    // current connection state.
    //  For example: selecting "all" should launch on all connected targets,
    //  even if the number of connected targets changes between launches.
    internal static readonly DeploymentTargetId kDefault =
        new DeploymentTargetId("__builtin__target_default");
    internal static readonly DeploymentTargetId kAll =
        new DeploymentTargetId("__builtin__target_all");
  ...
  internal enum CheckStatus { Ok = 0, Failed }
  ...
  internal struct TargetCheckResult {
    ...
    public bool Passed() {
      return hardware.status == CheckStatus.Ok && sdk.status == CheckStatus.Ok;
    }
  }
  ...
  internal interface IDeploymentTargetInfo {
    ...
    TargetCheckResult CheckTarget(
        DeploymentTargetRequirements targetRequirements);
  ...
  internal class DeploymentOperationFailedException : Exception {
    ...
    public DeploymentOperationFailedException(string title, string message,
  ...
  internal class CorruptBuildException : DeploymentOperationFailedException {
    public CorruptBuildException(string message = "Corrupt build.",
                                 Exception inner = null)
        : base("Corrupt build", message, inner) {}
  }

  internal interface IDeploymentTargetsExtension {
    // Returns context object populated with data that other methods can access
    // from any thread.
    //  setup - Pass true to setup platform if it hasn't been already setup
    //  (e.g. display dialog box to select SDK path). Throws exception if user
    //  cancels setup process.
    //          Pass false to return null if platform hasn't been already setup.
    //          This is useful when this method is called from UI draw event.
    //          Displaying dialog boxes at that point breaks UI. Let the user
    //          know what's going on by displaying informational label or
    //          something like that.
    ...
    List<DeploymentTargetIdAndStatus> GetKnownTargets(
        IDeploymentTargetsMainThreadContext context,
        ProgressHandler progressHandler = null);

    // Returns info for a target
    // Throws DeploymentOperationFailedException (or one of its subclasses) is
    // something goes wrong. Throws DeploymentOperationAbortedException if
    // process is cancelled by the user.
    //  Can be called from a background thread
    IDeploymentTargetInfo GetTargetInfo(
        IDeploymentTargetsMainThreadContext context,
        DeploymentTargetId targetId, ProgressHandler progressHandler = null);

    // Returns device logger specific to target platform
    // Throws DeploymentOperationFailedException (or one of its subclasses) is
    // something goes wrong. Throws DeploymentOperationAbortedException if
    // process is cancelled by the user.
    DeploymentTargetLogger GetTargetLogger(
        IDeploymentTargetsMainThreadContext context,
    ...
    // process is cancelled by the user.
    //  Can be called from a background thread
    void LaunchBuildOnTarget(IDeploymentTargetsMainThreadContext context,
