...
namespace UnityEditor.DeploymentTargets {
  class DeploymentTargetManager {
    const string k_ExtensionErrorMessage =
    ...
    public IDeploymentTargetsMainThreadContext Context {
      get { return m_Context; }
    }

    public static DeploymentTargetManager
    CreateInstance(BuildTargetGroup targetGroup, BuildTarget buildTarget,
                   bool setup = true) {
      var extension = GetExtension(targetGroup, buildTarget);
      var context = extension.GetMainThreadContext(setup);
      return context != null ? new DeploymentTargetManager(extension, context)
          : null;
    }

    DeploymentTargetManager(IDeploymentTargetsExtension extension,
    ...
    GetExtension(BuildTargetGroup targetGroup, BuildTarget buildTarget) {
      var extension =
          ModuleManager.GetDeploymentTargetsExtension(targetGroup, buildTarget);
      if (extension == null)
    ...
    public void LaunchBuildOnTarget(BuildProperties buildProperties,
                                    DeploymentTargetId targetId,
                                    ProgressHandler progressHandler = null) {
      m_Extension.LaunchBuildOnTarget(m_Context, buildProperties, targetId,
                                      progressHandler);
    }
    ...
    internal
        DeploymentTargetLogger GetTargetLogger(DeploymentTargetId targetId) {
      return m_Extension.GetTargetLogger(m_Context, targetId);
    }

    // Launch a build on any target on a platform
    public List<DeploymentTargetId>
    FindValidTargetsForLaunchBuild(BuildProperties buildProperties) {
