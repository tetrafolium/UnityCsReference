...
public class SerializedProperty : IDisposable {
  ...
  // This is so the garbage collector won't clean up SerializedObject behind the
  // scenes.
  ...
  public UnityObject exposedReferenceValue {
    get {
      ...
      var exposedPropertyTable =
          serializedObject.context as IExposedPropertyTable;
      if (exposedPropertyTable != null) {
        ...
        var objReference = exposedPropertyTable.GetReferenceValue(
            propertyName, out propertyFoundInTable);
  ...
  // This is useful if you want to keep a reference to the current property but
  ...
  // Retrieves the SerializedProperty at a relative path to the current
  // property.
  ...
  // Retrieves an iterator that allows you to iterator over the current nexting
  // of a serialized property.
  public System.Collections.IEnumerator GetEnumerator() {
    if (isArray) {
      ...
      while (NextVisible(true) &&
             !SerializedProperty.EqualContents(this, end)) {
        yield return this;
      }
    }
  }
  ...
  internal void SetToValueOfTarget(UnityObject target) {
    ...
    if (targetProperty == null) {
      Debug.LogError(target.name + " does not have the property " +
                     propertyPath);
      return;
    }
    switch (propertyType) {
    case SerializedPropertyType.Integer:
      intValue = targetProperty.intValue;
      break;
    case SerializedPropertyType.Boolean:
      boolValue = targetProperty.boolValue;
      break;
    case SerializedPropertyType.Float:
      floatValue = targetProperty.floatValue;
      break;
    case SerializedPropertyType.String:
      stringValue = targetProperty.stringValue;
      break;
    case SerializedPropertyType.Color:
      colorValue = targetProperty.colorValue;
      break;
    case SerializedPropertyType.ObjectReference:
      objectReferenceValue = targetProperty.objectReferenceValue;
      break;
    case SerializedPropertyType.LayerMask:
      intValue = targetProperty.intValue;
      break;
    case SerializedPropertyType.Enum:
      enumValueIndex = targetProperty.enumValueIndex;
      break;
    case SerializedPropertyType.Vector2:
      vector2Value = targetProperty.vector2Value;
      break;
    case SerializedPropertyType.Vector3:
      vector3Value = targetProperty.vector3Value;
      break;
    case SerializedPropertyType.Vector4:
      vector4Value = targetProperty.vector4Value;
      break;
    case SerializedPropertyType.Vector2Int:
      vector2IntValue = targetProperty.vector2IntValue;
      break;
    case SerializedPropertyType.Vector3Int:
      vector3IntValue = targetProperty.vector3IntValue;
      break;
    case SerializedPropertyType.Rect:
      rectValue = targetProperty.rectValue;
      break;
    case SerializedPropertyType.RectInt:
      rectIntValue = targetProperty.rectIntValue;
      break;
    ...
    case SerializedPropertyType.AnimationCurve:
      animationCurveValue = targetProperty.animationCurveValue;
      break;
    case SerializedPropertyType.Bounds:
      boundsValue = targetProperty.boundsValue;
      break;
    case SerializedPropertyType.BoundsInt:
      boundsIntValue = targetProperty.boundsIntValue;
      break;
    case SerializedPropertyType.Gradient:
      gradientValue = targetProperty.gradientValue;
      break;
    case SerializedPropertyType.ExposedReference:
      exposedReferenceValue = targetProperty.exposedReferenceValue;
      break;
    }
  }
  ...
  internal void Verify(VerifyFlags verifyFlags = VerifyFlags.None) {
    ...
    if (m_NativePropertyPtr == IntPtr.Zero || m_SerializedObject == null ||
        m_SerializedObject.m_NativeObjectPtr == IntPtr.Zero)
      throw new NullReferenceException(
          "SerializedObject of SerializedProperty has been Disposed.");

    SyncSerializedObjectVersion();

    if ((verifyFlags & VerifyFlags.IteratorNotAtEnd) ==
            VerifyFlags.IteratorNotAtEnd &&
        EndOfData())
      throw new InvalidOperationException(
          "The operation is not possible when moved past all properties (Next returned false)");
  }
  ...
  [NativeName("NextVisible")] extern private bool
  NextVisibleInternal(bool enterChildren);
  ...
  [FreeFunction(Name = "SerializedPropertyBindings::ClearArray",
                HasExplicitThis = true)] extern private void
  ClearArrayInternal();

  [NativeName("FindProperty")] extern internal bool
  FindPropertyInternal(string propertyPath);
  ...
  [FreeFunction("SerializedPropertyBindings::Internal_Destroy",
                IsThreadSafe = true)]
  ...
  private extern static bool EqualContentsInternal(SerializedProperty x,
  ...
  private extern static bool DataEqualsInternal(SerializedProperty x,
                                                SerializedProperty y);
  ...
  private extern void SetBitAtIndexForAllTargetsImmediateInternal(int index,
                                                                  bool value);
  ...
  [FreeFunction("SerializedPropertyBindings::GetNameInternal",
  ...
  /// In the context of polymorphic serialization, those elements might be
  /// dynamic instances not statically discoverable from the class type. We need
  /// to take a very specific code path when we try to get the type of a field
  /// inside such a dynamic instance through a SerializedProperty.
  ///
  /// @see
  /// UnityEditor.ScriptAttributeUtility.GetFieldInfoAndStaticTypeFromProperty
  /// </summary>
  ...
  /// <summary>
  /// Returns the FQN in the format "<assembly name> <full class name>" for the
  /// current dynamic managed reference.
  /// </summary>
  /// <returns></returns>
  // Useful in the same context as 'isReferencingAManagedReferenceField'.
  ...
  // Is property a default override property which is enforced to always be
  // overridden? (RO)
  ...
  public object managedReferenceValue {
    set {
      ...
      // Make sure that the underlying base type is compatible with the current
      // object
      Type type;
      var fieldInfo =
          UnityEditor.ScriptAttributeUtility
              .GetFieldInfoAndStaticTypeFromProperty(this, out type);
      var propertyBaseType = type;

      if (value != null) {
        var valueType = value.GetType();
        if (valueType == typeof(UnityObject) ||
            valueType.IsSubclassOf(typeof(UnityObject))) {
          throw new System.InvalidOperationException(
    ...
    // No getter for managed reference since it adds a few notions that might be
    // hard to reason about from the user's perspective. A serializedobject is a
    // serialized version of a UnityObject. A serialized property is a view on
    // that serialized object. All operations and all that a serialized knows
    // about are around that serialzied view of the managed world. Managed
    // references in the context of SO/SP does not have the same semantics as C#
    // references. All the references are serialized properly but the original
    // C# reference is lost in the process and cannot be retrieved unless e.g.
    // cached locally on the C# side (i.e. in this file). BUT this would not
    // world properly or involve tricky reconstruction with domain reloads after
    // which all those C# references would have to be reconstructed etc. So for
    // now we dont allow getting a managed reference.

    // If we ever wanted to add one though, the approach would be to add an
    // explicit setting that takes an reference ID and an object instance as
    // input, this would allow serialized properties to edit relations between
    // managed references. We would also need a method to get an instance from a
    // managed reference id along with a nice safe API around it.
  }
  ...
  [FreeFunction(
      Name = "SerializedPropertyBindings::SetPPtrValueFromInstanceIDInternal",
      HasExplicitThis = true)]
  ...
  [FreeFunction(Name = "SerializedPropertyBindings::GetValueVector2Internal",
                HasExplicitThis = true)] extern private Vector2
  GetValueVector2Internal();

  [FreeFunction(Name = "SerializedPropertyBindings::SetValueVector2Internal",
                HasExplicitThis = true)] extern private void
  SetValueVector2Internal(Vector2 value);
  ...
  [FreeFunction(Name = "SerializedPropertyBindings::GetValueVector3Internal",
                HasExplicitThis = true)] extern private Vector3
  GetValueVector3Internal();

  [FreeFunction(Name = "SerializedPropertyBindings::SetValueVector3Internal",
                HasExplicitThis = true)] extern private void
  SetValueVector3Internal(Vector3 value);
  ...
  [FreeFunction(Name = "SerializedPropertyBindings::GetValueVector4Internal",
                HasExplicitThis = true)] extern private Vector4
  GetValueVector4Internal();

  [FreeFunction(Name = "SerializedPropertyBindings::SetValueVector4Internal",
                HasExplicitThis = true)] extern private void
  SetValueVector4Internal(Vector4 value);
  ...
  [FreeFunction(Name = "SerializedPropertyBindings::GetValueVector2IntInternal",
                HasExplicitThis = true)] extern private Vector2Int
  GetValueVector2IntInternal();

  [FreeFunction(Name = "SerializedPropertyBindings::SetValueVector2IntInternal",
                HasExplicitThis = true)] extern private void
  SetValueVector2IntInternal(Vector2Int value);
  ...
  [FreeFunction(Name = "SerializedPropertyBindings::GetValueVector3IntInternal",
                HasExplicitThis = true)] extern private Vector3Int
  GetValueVector3IntInternal();

  [FreeFunction(Name = "SerializedPropertyBindings::SetValueVector3IntInternal",
                HasExplicitThis = true)] extern private void
  SetValueVector3IntInternal(Vector3Int value);
  ...
  [FreeFunction(Name = "SerializedPropertyBindings::GetValueQuaternionInternal",
                HasExplicitThis = true)] extern private Quaternion
  GetValueQuaternionInternal();

  [FreeFunction(Name = "SerializedPropertyBindings::SetValueQuaternionInternal",
                HasExplicitThis = true)] extern private void
  SetValueQuaternionInternal(Quaternion value);
  ...
  [FreeFunction(Name = "SerializedPropertyBindings::GetValueRectInternal",
                HasExplicitThis = true)] extern private Rect
  GetValueRectInternal();

  [FreeFunction(Name = "SerializedPropertyBindings::SetValueRectInternal",
                HasExplicitThis = true)] extern private void
  SetValueRectInternal(Rect value);
  ...
  [FreeFunction(Name = "SerializedPropertyBindings::GetValueRectIntInternal",
                HasExplicitThis = true)] extern private RectInt
  GetValueRectIntInternal();

  [FreeFunction(Name = "SerializedPropertyBindings::SetValueRectIntInternal",
                HasExplicitThis = true)] extern private void
  SetValueRectIntInternal(RectInt value);
  ...
  [FreeFunction(Name = "SerializedPropertyBindings::GetValueBoundsInternal",
                HasExplicitThis = true)] extern private Bounds
  GetValueBoundsInternal();

  [FreeFunction(Name = "SerializedPropertyBindings::SetValueBoundsInternal",
                HasExplicitThis = true)] extern private void
  SetValueBoundsInternal(Bounds value);
  ...
  [FreeFunction(Name = "SerializedPropertyBindings::GetValueBoundsIntInternal",
                HasExplicitThis = true)] extern private BoundsInt
  GetValueBoundsIntInternal();

  [FreeFunction(Name = "SerializedPropertyBindings::SetValueBoundsIntInternal",
                HasExplicitThis = true)] extern private void
  SetValueBoundsIntInternal(BoundsInt value);
  ...
  [FreeFunction(Name = "SerializedPropertyBindings::NextInternal",
                HasExplicitThis = true)] extern private bool
  NextInternal(bool enterChildren);
  ...
  [FreeFunction(Name = "SerializedPropertyBindings::ResetInternal",
                HasExplicitThis = true)] extern private void ResetInternal();
  ...
  [NativeName("CountRemaining")] extern private int CountRemainingInternal();
  ...
  [NativeName("CountInProperty")] extern private int CountInPropertyInternal();
  ...
  [FreeFunction(Name = "SerializedPropertyBindings::CopyInternal",
  ...
  [NativeName("DuplicateCommand")] extern private bool
  DuplicateCommandInternal();
  ...
  [NativeName("DeleteCommand")] extern private bool DeleteCommandInternal();

  // Retrieves the SerializedProperty that defines the end range of this
  // property.
  public SerializedProperty GetEndProperty() { return GetEndProperty(false); }
  ...
  [NativeName("GetArrayElementAtIndex")] extern private bool
  GetArrayElementAtIndexImpl(int index);
  ...
  [NativeName("InsertArrayElementAtIndex")] extern private void
  InsertArrayElementAtIndexInternal(int index);
  ...
  [NativeName("DeleteArrayElementAtIndex")] extern private void
  DeleteArrayElementAtIndexInternal(int index);
  ...
  [NativeName("MoveArrayElement")] extern private bool
  MoveArrayElementInternal(int srcIndex, int dstIndex);
  ...
  [NativeName("GetFixedBufferElementAtIndex")] extern private bool
  GetFixedBufferAtIndexInternal(int index);

  [NativeName("AnimationCurveValueEquals")] extern private bool
  AnimationCurveValueEquals(AnimationCurve curve);
  ...
  [NativeName("GradientValueEquals")] extern private bool
  GradientValueEquals(Gradient gradient);
  ...
  [NativeName("StringValueEquals")] extern private bool
  StringValueEquals(string value);
