...
namespace UnityEditor.Sprites {
  public sealed partial class Packer {
    public enum Execution { Normal = 0, ForceRegroup }
      ...
      set {
        ...
        if (!m_policies.Contains(value))
          throw new ArgumentException(
              "Specified policy {0} is not in the policy list.", value);
      ...
      foreach (var t in types) {
        if (m_policyTypeCache.ContainsKey(t.Name)) {
          ...
          Debug.LogError(string.Format(
              "Duplicate Sprite Packer policies found: {0} and {1}. Please rename one.",
              t.FullName, otherT.FullName));
        } else
          m_policyTypeCache[t.Name] = t;
      }

      m_selectedPolicy = String.IsNullOrEmpty(PlayerSettings.spritePackerPolicy)
                             ? kDefaultPolicy
                             : PlayerSettings.spritePackerPolicy;
    ...
    internal static string GetSelectedPolicyId() {
      ...
      string versionString = string.Format("{0}::{1}", t.AssemblyQualifiedName,
                                           policy.GetVersion());
    ...
    ExecuteSelectedPolicy(BuildTarget target,
                          int[] textureImporterInstanceIDs) {
      RegenerateList();

      Type t = m_policyTypeCache[m_selectedPolicy];
      IPackerPolicy policy = Activator.CreateInstance(t) as IPackerPolicy;
      policy.OnGroupAtlases(target, new PackerJob(),
        ...
        foreach (Editor e in activeEditor.activeEditors) {
          ...
          if (EditorUtility.DisplayDialog("Unapplied import settings",
                                          "Unapplied import settings for \'" +
                                              importer.assetPath + "\'",
                                          "Apply", "Revert")) {
            inspector.ApplyAndImport(); // No way to apply/revert only some
                                        // assets. Bug: 564192.
          }
        }
      }
  ...
  public interface IPackerPolicy {
    ...
    void OnGroupAtlases(BuildTarget target, PackerJob job,
                        int[] textureImporterInstanceIDs);
    int GetVersion();
  }
}
