...
internal static class SpriteUtility {
  static class SpriteUtilityStrings {
    public static readonly GUIContent saveAnimDialogMessage =
        EditorGUIUtility.TrTextContent(
            "Create a new animation for the game object '{0}':");
    public static readonly GUIContent saveAnimDialogTitle =
        EditorGUIUtility.TrTextContent("Create New Animation");
    public static readonly GUIContent saveAnimDialogName =
        EditorGUIUtility.TrTextContent("New Animation");
    public static readonly GUIContent unableToFindSpriteRendererWarning =
        EditorGUIUtility.TrTextContent(
            "There should be a SpriteRenderer in dragged object");
    public static readonly GUIContent unableToAddSpriteRendererWarning =
        EditorGUIUtility.TrTextContent(
            "Unable to add SpriteRenderer into Gameobject.");
    public static readonly GUIContent failedToCreateAnimationError =
        EditorGUIUtility.TrTextContent(
            "Failed to create animation for dragged object");
  }
  ...
  public delegate string ShowFileDialogDelegate(string title,
                                                string defaultName,
                                                string extension,
                                                string message,
                                                string defaultPath);
  public static void OnSceneDrag(SceneView sceneView) {
    HandleSpriteSceneDrag(sceneView, new UnityEngine.U2D.Interface.Event(),
                          DragAndDrop.objectReferences, DragAndDrop.paths,
                          EditorUtility.SaveFilePanelInProject);
  }

  public static void
  HandleSpriteSceneDrag(SceneView sceneView, IEvent evt,
                        Object[] objectReferences, string[] paths,
                        ShowFileDialogDelegate saveFileDialog) {
    if (evt.type != EventType.DragUpdated &&
        evt.type != EventType.DragPerform && evt.type != EventType.DragExited)
      return;

    // Return if any of the dragged objects are null, e.g. a MonoBehaviour
    // without a managed instance
    if (objectReferences.Any(obj => obj == null))
      return;

    // Regardless of EditorBehaviorMode or SceneView mode we don't handle if
    // texture is dragged over a GO with renderer
    if (objectReferences.Length == 1 &&
        objectReferences[0] as UnityTexture2D != null) {
    ...
    case (EventType.DragUpdated):
      DragType newDragType =
          evt.alt ? DragType.CreateMultiple : DragType.SpriteAnimation;

      if (s_DragType != newDragType ||
          s_SceneDragObjects == null) // Either this is first time we are here
                                      // OR evt.alt changed during drag
      {
        if (!ExistingAssets(objectReferences) &&
            PathsAreValidTextures(
                paths)) // External drag with images that are not in the project
        {
        ...
        {
          List<Sprite> assets =
              GetSpriteFromPathsOrObjects(objectReferences, paths, evt.type);

          if (assets.Count == 0)
            return;

          if (s_DragType !=
              DragType.NotInitialized) // evt.alt changed during drag, so we
                                       // need to cleanup and start over
            CleanUp(true);
      ...
      PositionSceneDragObjects(s_SceneDragObjects, sceneView,
                               evt.mousePosition);
    ...
    case (EventType.DragPerform):
      ...
      if (sprites.Count > 0 && s_SceneDragObjects != null) {
        // Store current undoIndex to undo all operations done if any part of
        // sprite creation fails
        int undoIndex = Undo.GetCurrentGroup();

        // For external drags, we have delayed all creation to DragPerform
        // because only now we have the imported sprite assets
        ...
        foreach (GameObject dragGO in s_SceneDragObjects) {
          ...
          // When in e.g Prefab Mode ensure to reparent dragged objects under
          // the prefab root
          Transform defaultObjectTransform =
              SceneView.GetDefaultParentObjectIfSet();
          if (defaultObjectTransform != null)
            ...
            dragGO.transform.SetParent(sceneView.customParentForDraggedObjects,
                                       true);
        ...
        if (s_DragType == DragType.SpriteAnimation && sprites.Count > 1) {
          createGameObject =
              AddAnimationToGO((GameObject) s_SceneDragObjects[0],
                               sprites.ToArray(), saveFileDialog);
        }

        if (createGameObject) {
          Selection.objects = s_SceneDragObjects.ToArray();
        } else {
          // Revert all Create Sprite actions if animation failed to be created
          // or was cancelled
  ...
  private static void PositionSceneDragObjects(List<Object> objects,
    ...
    if (sceneView.in2DMode) {
      ...
      DragAndDrop.visualMode = DragAndDropVisualMode.Copy;
      object hit = HandleUtility.RaySnap(
  ...
  static bool CreateAnimation(GameObject gameObject, Object[] frames,
    ...
    Animator animator =
        AnimationWindowUtility.EnsureActiveAnimationPlayer(gameObject)
            ? AnimationWindowUtility.GetClosestAnimatorInParents(
    ...
    if (animator != null) {
      ...
      string message = string.Format(
          SpriteUtilityStrings.saveAnimDialogMessage.text, gameObject.name);
      string newClipDirectory = ProjectWindowUtil.GetActiveFolderPath();
      string newClipPath =
          saveFileDialog(SpriteUtilityStrings.saveAnimDialogTitle.text,
                         SpriteUtilityStrings.saveAnimDialogName.text, "anim",
                         message, newClipDirectory);
      ...
      if (string.IsNullOrEmpty(newClipPath)) {
        ...
        AnimationClip newClip =
            AnimationWindowUtility.CreateNewClipAtPath(newClipPath);
        if (newClip != null) {
          AddSpriteAnimationToClip(newClip, frames);
          createSuccess = AnimationWindowUtility.AddClipToAnimatorComponent(
              animator, newClip);
        }
      }
    }
  ...
  GetSpriteFromPathsOrObjects(Object[] objects, string[] paths,
    ...
    // Fix case 742896. If any of the drag objects is already in the
    // AssetDatabase, means we don't have to handle external drags. Fix case
    // 857231. We only handle external drag if default behaviour mode is 2D
  ...
  private static void HandleExternalDrag(string[] paths, bool perform,
                                         ref List<Sprite> result) {
    foreach (var path in paths) {
      ...
      var newPath = AssetDatabase.GenerateUniqueAssetPath(
          Path.Combine("Assets", FileUtil.GetLastPathNameComponent(path)));
      if (newPath.Length <= 0)
        continue;
      ...
      Sprite defaultSprite = GenerateDefaultSprite(
          AssetDatabase.LoadMainAssetAtPath(newPath) as UnityTexture2D);
  ...
  private static Sprite GenerateDefaultSprite(UnityTexture2D texture) {
    ...
    if (textureImporter ==
        null) // could be DDS importer or other non-TextureImporter type
      return null;
    ...
    firstSprite = AssetDatabase.LoadAllAssetsAtPath(assetPath).FirstOrDefault(
        t => t is Sprite);
  ...
  public static GameObject CreateDragGO(Sprite frame, Vector3 position,
    ...
    // ObjectFactory registers an Undo for the GameObject created, which we do
    // not want for the drag preview. We register an Undo only when the user
    // does a DragPerform to confirm the creation of the Sprite GameObject. The
    // GameObject is cloned and returned while the original is destroyed to
    // remove the Undo operation.
    GameObject go =
        ObjectFactory.CreateGameObject(name, typeof(SpriteRenderer));
    ...
    // Ensure that the dragged gameobjects gets the correct scene culling mask
    // so it can be rendered in the SceneView (when the SceneView camera is
    // culling on a custom scene)
  ...
  public static bool AddAnimationToGO(GameObject go, Sprite[] frames,
                                      ShowFileDialogDelegate saveFileDialog) {
    SpriteRenderer spriteRenderer = go.GetComponent<SpriteRenderer>();
    if (spriteRenderer == null) {
      Debug.LogWarning(
          SpriteUtilityStrings.unableToFindSpriteRendererWarning.text);
      spriteRenderer = (SpriteRenderer) ObjectFactory.AddComponent(
          go, typeof(SpriteRenderer));
      if (spriteRenderer == null) {
        Debug.LogWarning(
            SpriteUtilityStrings.unableToAddSpriteRendererWarning.text);
        return false;
      }
    }
  ...
  public static GameObject DropSpriteToSceneToCreateGO(Sprite sprite,
                                                       Vector3 position) {
    GameObject go = new GameObject(
        string.IsNullOrEmpty(sprite.name) ? "Sprite" : sprite.name);
    SpriteRenderer spriteRenderer =
        (SpriteRenderer) ObjectFactory.AddComponent(go, typeof(SpriteRenderer));
  ...
  public static Sprite RemapObjectToSprite(Object obj) {
    ...
    if (obj is UnityTexture2D) {
      Object[] assets =
          AssetDatabase.LoadAllAssetsAtPath(AssetDatabase.GetAssetPath(obj));
  ...
  public static List<Sprite> TextureToSprites(UnityTexture2D tex) {
    Object[] assets =
        AssetDatabase.LoadAllAssetsAtPath(AssetDatabase.GetAssetPath(tex));
    List<Sprite> result = new List<Sprite>();
  ...
  private static bool ValidPathForTextureAsset(string path) {
    ...
    return ext == "jpg" || ext == "jpeg" || ext == "tif" || ext == "tiff" ||
           ext == "tga" || ext == "gif" || ext == "png" || ext == "psd" ||
           ext == "bmp" || ext == "iff" || ext == "pict" || ext == "pic" ||
           ext == "pct" || ext == "exr" || ext == "hdr";
  }

  public static UnityTexture2D RenderStaticPreview(Sprite sprite, Color color,
                                                   int width, int height) {
    return RenderStaticPreview(sprite, color, width, height,
                               Matrix4x4.identity);
  }
  ...
  public static UnityTexture2D RenderStaticPreview(Sprite sprite, Color color,
    ...
    PreviewHelpers.AdjustWidthAndHeightForStaticPreview(
        (int) sprite.rect.width, (int) sprite.rect.height, ref width,
        ref height);
    ...
    UnityTexture2D copy =
        new UnityTexture2D(width, height, TextureFormat.ARGB32, false);
  ...
  internal static void RenderSpriteImmediate(Sprite sprite, Color color,
                                             Matrix4x4 transform) {
  ...
  public static UnityTexture2D CreateTemporaryDuplicate(UnityTexture2D original,
    ...
    // If the user system doesn't support this texture size, force it to use
    // mipmap
    bool forceUseMipMap = width >= SystemInfo.maxTextureSize ||
                          height >= SystemInfo.maxTextureSize;

    UnityTexture2D copy =
        new UnityTexture2D(width, height, TextureFormat.RGBA32,
                           original.mipmapCount > 1 || forceUseMipMap);
    copy.ReadPixels(new Rect(0, 0, width, height), 0, 0);
    copy.Apply();
    RenderTexture.ReleaseTemporary(tmp);
