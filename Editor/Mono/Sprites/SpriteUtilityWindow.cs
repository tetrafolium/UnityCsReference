...
internal class SpriteUtilityWindow : EditorWindow {
  protected class Styles {
    ...
    public static readonly GUIContent okText =
  ...
  protected float GetMinZoom() {
    if (m_Texture == null)
      return 1.0f;
    // Case 654327: Add k_MaxZoom size to min check to ensure that min zoom is
    // smaller than max zoom
    return Mathf.Min(m_TextureViewRect.width / m_Texture.width,
                     m_TextureViewRect.height / m_Texture.height, k_MaxZoom) *
           k_MinZoomPercentage;
  }
    ...
    if (Event.current.type == EventType.ScrollWheel ||
      ...
      float zoomMultiplier =
          ...
          Event.current.delta.y *
              (Event.current.type == EventType.ScrollWheel ? k_WheelZoomSpeed
               : -k_MouseZoomSpeed);
      ...
      if (currentZoom != m_Zoom) {
        ...
        Vector3 textureHalfSize =
            new Vector2(m_Texture.width, m_Texture.height) * 0.5f;
        Vector3 mousePositionWorld = Handles.inverseMatrix.MultiplyPoint3x4(
            Event.current.mousePosition + m_ScrollPosition);
        Vector3 delta =
            (mousePositionWorld - textureHalfSize) * (zoomMultiplier - 1f);
  ...
  protected void HandlePanning() {
    // You can pan by holding ALT and using left button or NOT holding ALT and
    // using right button. ALT + right is reserved for zooming.
    bool panMode = (!Event.current.alt && Event.current.button > 0 ||
    ...
    // We need to repaint when entering or exiting the pan mode, so the mouse
    // cursor gets refreshed.
    if (((Event.current.type == EventType.MouseUp ||
          Event.current.type == EventType.MouseDown) &&
         panMode) ||
        (Event.current.type == EventType.KeyUp ||
  ...
  // Bounding values for scrollbars. Changes with zoom, because we want min/max
  // scroll to stop at texture edges.
  protected Rect maxScrollRect {
    get {
      ...
      return new Rect(-halfWidth, -halfHeight,
                      m_TextureViewRect.width + halfWidth * 2f,
                      m_TextureViewRect.height + halfHeight * 2f);
    }
  }
  ...
  protected void DrawTexturespaceBackground() {
    ...
    for (float v = 0; v <= size; v += gridSize)
      SpriteEditorUtility.DrawLine(
          new Vector2(-halfSize + v, halfSize + v) + offset,
          new Vector2(halfSize + v, -halfSize + v) + offset);
  ...
  protected void DrawTexture() {
    float mipLevel =
        Mathf.Min(m_MipLevel, TextureUtil.GetMipmapCount(m_Texture) - 1);
  ...
  protected void DrawScreenspaceBackground() {
    if (Event.current.type == EventType.Repaint)
      m_Styles.preBackground.Draw(m_TextureViewRect, false, false, false,
                                  false);
  }

  protected void HandleScrollbars() {
    Rect horizontalScrollBarPosition =
        new Rect(m_TextureViewRect.xMin, m_TextureViewRect.yMax,
                 m_TextureViewRect.width, k_ScrollbarMargin);
    m_ScrollPosition.x = GUI.HorizontalScrollbar(
        horizontalScrollBarPosition, m_ScrollPosition.x,
        m_TextureViewRect.width, maxScrollRect.xMin, maxScrollRect.xMax);

    Rect verticalScrollBarPosition =
        new Rect(m_TextureViewRect.xMax, m_TextureViewRect.yMin,
                 k_ScrollbarMargin, m_TextureViewRect.height);
    m_ScrollPosition.y = GUI.VerticalScrollbar(
        verticalScrollBarPosition, m_ScrollPosition.y, m_TextureViewRect.height,
  ...
  protected void SetupHandlesMatrix() {
    ...
    // Handle matrix is for converting between view and texture space
    // coordinates, without taking account the scroll position. Scroll position
    // is added separately so we can use it with GUIClip.
    Handles.matrix =
        Matrix4x4.TRS(handlesPos, Quaternion.identity, handlesScale);
  }
  ...
  protected Rect DoAlphaZoomToolbarGUI(Rect area) {
    ...
    using(new EditorGUI.DisabledScope(mipCount == 1)) {
      ...
      drawArea.x -= drawArea.width;
      m_MipLevel = Mathf.Round(
          GUI.HorizontalSlider(drawArea, m_MipLevel, mipCount - 1, 0,
    ...
    drawArea.width = EditorGUI.kSliderMinW;
    drawArea.x -= drawArea.width;
    m_Zoom = GUI.HorizontalSlider(drawArea, m_Zoom, GetMinZoom(), k_MaxZoom,
                                  m_Styles.preSlider, m_Styles.preSliderThumb);

    drawArea.width = EditorGUI.kObjectFieldMiniThumbnailWidth;
    drawArea.x -= drawArea.width + EditorGUI.kSpacing;
    m_ShowAlpha =
        GUI.Toggle(drawArea, m_ShowAlpha, m_ShowAlpha ? m_Styles.alphaIcon
                   : m_Styles.RGBIcon, "toolbarButton");
  ...
  protected void DoTextureGUI() {
    ...
    // Handle this after DoTextureGUIExtras in case user wants any event that is
    // handled by Zoom or Panning
    HandleZoom();
    HandlePanning();
  }
  ...
  internal static void DrawToolBarWidget(ref Rect drawRect,
  ...
  SetShowSpriteEditorWindow(Func<bool> spriteEditorWindow) {
    if (spriteEditorWindow != null)
      showSpriteEditorWindow = (x) => spriteEditorWindow();
  }

  internal static void SetShowSpriteEditorWindowWithObject(
      Func<UnityEngine.Object, bool> spriteEditorWindow) {
    if (spriteEditorWindow != null)
  ...
  static Func<UnityEngine.Object, bool> showSpriteEditorWindow = (_) => {
    EditorUtility.DisplayDialog(Styles.noSpriteEditorWindowTitle.text,
                                Styles.noSpriteEditorWindow.text,
                                Styles.okText.text);
    return false;
  };
} // class
