  ...
  internal class PackerWindow : SpriteUtilityWindow {
    private class PackerWindowStyle {
      public static readonly GUIContent packLabel =
          EditorGUIUtility.TrTextContent("Pack");
      public static readonly GUIContent repackLabel =
          EditorGUIUtility.TrTextContent("Repack");
      public static readonly GUIContent viewAtlasLabel =
          EditorGUIUtility.TrTextContent("View Atlas:");
      ...
      public static readonly GUIContent pageContentLabel =
          EditorGUIUtility.TrTextContent("Page {0}");
      public static readonly GUIContent packingDisabledLabel =
          EditorGUIUtility.TrTextContent(
              "Legacy sprite packing is disabled. Enable it in Edit > Project Settings > Editor.");
      public static readonly GUIContent openProjectSettingButton =
          EditorGUIUtility.TrTextContent("Open Project Editor Settings");
    }
    ...
    struct Edge {
      ...
      public override bool Equals(object obj) {
        ...
        if (obj.GetType() != GetType())
          return false;
        Edge item = (Edge) obj;
        return (v0 == item.v0 && v1 == item.v1) ||
               (v0 == item.v1 && v1 == item.v0);
      }
    ...
    private static string[] s_AtlasNamesEmpty =
        new string[1]{"Sprite atlas cache is empty"};
    ...
    private void RefreshAtlasPageList() {
      if (m_AtlasNames.Length > 0) {
        ...
        for (int i = 0; i < textures.Length; ++i)
          m_PageNames[i] =
              string.Format(PackerWindowStyle.pageContentLabel.text, i + 1);
      } else {
    ...
    private void OnAtlasNameListChanged() {
      if (m_AtlasNames.Length > 0) {
        ...
        string newAtlasName = (atlasNames.Length <= m_SelectedAtlas)
                                  ? null
                                  : atlasNames[m_SelectedAtlas];
    ...
    private bool ValidateIsPackingEnabled() { return true; }
    ...
    private Rect DoToolbarGUI() {
      ...
      using(new EditorGUI.DisabledScope(Application.isPlaying)) {
        drawRect.width =
            EditorStyles.toolbarButton.CalcSize(PackerWindowStyle.packLabel).x;
        DrawToolBarWidget(ref drawRect, ref toolbarRect, (adjustedDrawRect) => {
          if (GUI.Button(adjustedDrawRect, PackerWindowStyle.packLabel,
                         EditorStyles.toolbarButton)) {
            Packer.RebuildAtlasCacheIfNeededInternal(
                EditorUserBuildSettings.activeBuildTarget, true,
                Packer.Execution.Normal);
            m_SelectedSprite = null;
            RefreshAtlasPageList();
            RefreshState();
          }
        ...
        using(new EditorGUI.DisabledScope(Packer.SelectedPolicy ==
                                          Packer.kDefaultPolicy)) {
          drawRect.x += drawRect.width;
          drawRect.width =
              EditorStyles.toolbarButton.CalcSize(PackerWindowStyle.repackLabel)
                  .x;
          DrawToolBarWidget(
              ref drawRect, ref toolbarRect, (adjustedDrawRect) => {
                if (GUI.Button(adjustedDrawRect, PackerWindowStyle.repackLabel,
                               EditorStyles.toolbarButton)) {
                  Packer.RebuildAtlasCacheIfNeededInternal(
                      EditorUserBuildSettings.activeBuildTarget, true,
      ...
      float viewAtlasWidth =
          GUI.skin.label.CalcSize(PackerWindowStyle.viewAtlasLabel).x;
      float totalWidth =
          viewAtlasWidth + kAtlasNameWidth + kPagesWidth + kPolicyWidth;

      drawRect.x +=
          EditorGUI
              .kSpacing; // leave some space from previous control for cosmetic
      ...
      using(new EditorGUI.DisabledScope(m_AtlasNames.Length == 0)) {
        ...
        DrawToolBarWidget(ref drawRect, ref toolbarRect, (adjustedDrawArea) => {
          m_SelectedAtlas =
              EditorGUI.Popup(adjustedDrawArea, m_SelectedAtlas, m_AtlasNames,
        ...
        DrawToolBarWidget(ref drawRect, ref toolbarRect, (adjustedDrawArea) => {
          m_SelectedPage =
              EditorGUI.Popup(adjustedDrawArea, m_SelectedPage, m_PageNames,
                              EditorStyles.toolbarPopup);
        });
      ...
      DrawToolBarWidget(ref drawRect, ref toolbarRect, (adjustedDrawArea) => {
        selectedPolicy = EditorGUI.Popup(adjustedDrawArea, selectedPolicy,
                                         policies, EditorStyles.toolbarPopup);
      });
    ...
    void OnSelectionChange() {
      ...
      if (selectedSprite != m_SelectedSprite) {
        if (selectedSprite != null) {
          ...
          Packer.GetAtlasDataForSprite(selectedSprite, out selAtlasName,
                                       out selAtlasTexture);

          int selAtlasIndex = m_AtlasNames.ToList().FindIndex(
              delegate(string s) { return selAtlasName == s; });
          if (selAtlasIndex == -1)
            return;
          int selAtlasPage = Packer.GetTexturesForAtlas(selAtlasName)
                                 .ToList()
                                 .FindIndex(delegate(Texture2D t) {
                                   return selAtlasTexture == t;
                                 });
          if (selAtlasPage == -1)
            return;
    ...
    private void RefreshState() {
      ...
      // check if the atlas has alpha as an external texture (as in ETC1 atlases
      // with alpha)
      Texture2D[] alphaTextures = Packer.GetAlphaTexturesForAtlas(curAtlasName);
      Texture2D selectedAlphaTexture = (m_SelectedPage < alphaTextures.Length)
                                           ? alphaTextures[m_SelectedPage]
                                           : null;
    ...
    public void OnGUI() {
      ...
      m_TextureViewRect =
          new Rect(0f, toolbarRect.yMax, position.width - k_ScrollbarMargin,
                   position.height - k_ScrollbarMargin - toolbarRect.height);
      GUILayout.FlexibleSpace();
      DoTextureGUI();
      string info = string.Format(
          "{1}x{2}, {0}", TextureUtil.GetTextureFormatString(m_Texture.format),
          m_Texture.width, m_Texture.height);
      EditorGUI.DropShadowLabel(new Rect(m_TextureViewRect.x,
                                         m_TextureViewRect.y + 10,
                                         m_TextureViewRect.width, 20),
                                info);
    ...
    private void DrawLineUtility(Vector2 from, Vector2 to) {
      SpriteEditorUtility.DrawLine(
          new Vector3(from.x * m_Texture.width + 1f / m_Zoom,
                      from.y * m_Texture.height + 1f / m_Zoom, 0.0f),
          new Vector3(to.x * m_Texture.width + 1f / m_Zoom,
                      to.y * m_Texture.height + 1f / m_Zoom, 0.0f));
    }
    ...
    private Edge[] FindUniqueEdges(UInt16[] indices) {
      ...
      Edge[] uniqueEdges = allEdges.GroupBy(x => x)
                               .Where(x => x.Count() == 1)
                               .Select(x => x.First())
                               .ToArray();
      return uniqueEdges;
    ...
    protected override void DrawGizmos() {
      if (m_SelectedSprite != null && m_Texture != null) {
        ...
        Edge[] uniqueEdges = FindUniqueEdges(
            indices); // Assumes that our mesh has no duplicate vertices

        SpriteEditorUtility.BeginLines(
            new Color(0.3921f, 0.5843f, 0.9294f, 0.75f)); // Cornflower blue :)
