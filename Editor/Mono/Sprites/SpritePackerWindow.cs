    ...
    void OnSelectionChange()
    {
        ...
        if (selectedSprite != m_SelectedSprite)
        {
            if (selectedSprite != null)
            {
                ...
                int selAtlasIndex = m_AtlasNames.ToList().FindIndex(delegate(string s) {
                    return selAtlasName == s;
                });
                if (selAtlasIndex == -1)
                    return;
                int selAtlasPage = Packer.GetTexturesForAtlas(selAtlasName).ToList().FindIndex(delegate(Texture2D t) {
