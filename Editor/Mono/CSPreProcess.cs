...
static bool Run(out CompilerMessage[] messages, bool includeModules) {
	int exitcode;
	messages = ParseResults(
		GetOutputStream(GetJamStartInfo(includeModules), out exitcode));
	return exitcode == 0;
}
...
private static ProcessStartInfo GetJamStartInfo(bool includeModules) {
	...
	var psi = new ProcessStartInfo {
		...
		// The only reason jam.pl exists is that I cannot figure out how to call
		// jam.bat, or jam.exe directly. magic.
		Arguments = moduleArgs.ToString(),
		FileName = "perl",
	};

	var path = Environment.GetEnvironmentVariable("PATH") ?? "";
	// on macOS the typical mercurial path might not be in our environment
	// variable, so add it for executing jam
...
private static string GetOutputStream(ProcessStartInfo startInfo,
                                      out int exitCode) {
	...
	var output = startInfo.RedirectStandardError ? sbStandardError
	             .ToString()
	: sbStandardOut.ToString();
	exitCode = p.ExitCode;
	return output;
}
}
