...
[SerializeField]
private float[] m_TickModulos =
	new float[] {}; // array with possible modulo numbers to choose from
[SerializeField]
private float[] m_TickStrengths =
	new float[] {}; // array with current strength of each modulo number
[SerializeField]
private int m_SmallestTick =
	0; // index of the currently smallest modulo number used to draw ticks
[SerializeField]
private int m_BiggestTick =
	-1; // index of the currently biggest modulo number used to draw ticks
[SerializeField]
private float m_MinValue = 0;   // shownArea min (in curve space)
[SerializeField]
private float m_MaxValue = 1;   // shownArea max (in curve space)
[SerializeField]
private float m_PixelRange = 1;   // total width/height of curveeditor

public int tickLevels {
	get { return m_BiggestTick - m_SmallestTick + 1; }
}
...
public List<float> GetTickModulosForFrameRate(float frameRate) {
	...
	// Make frames multiples of 5 and 10, if frameRate is too high (avoid
	// overflow) or not an even number
	if (frameRate > int.MaxValue / 2.0f ||
	    frameRate != Mathf.Round(frameRate)) {
		modulos = new List<float> {
			1f / frameRate,     5f / frameRate,      10f / frameRate,
			50f / frameRate,    100f / frameRate,    500f / frameRate,
			1000f / frameRate,  5000f / frameRate,   10000f / frameRate,
			50000f / frameRate, 100000f / frameRate, 500000f / frameRate
		};
...
public void SetRanges(float minValue, float maxValue, float minPixel,
                      float maxPixel) {
	m_MinValue = minValue;
	m_MaxValue = maxValue;
...
public void GetTicksAtLevel(int level, bool excludeTicksFromHigherlevels,
                            List<float> list) {
	if (list == null)
	...
	for (int i = startTick; i <= endTick; i++) {
		...
		if (excludeTicksFromHigherlevels && l < m_BiggestTick &&
...
public float GetPeriodOfLevel(int level) {
	return m_TickModulos[Mathf.Clamp(m_SmallestTick + level, 0,
	                                 m_TickModulos.Length - 1)];
}
...
public int GetLevelWithMinSeparation(float pixelSeparation) {
	for (int i = 0; i < m_TickModulos.Length; i++) {
		...
		float tickSpacing =
			m_TickModulos[i] * m_PixelRange / (m_MaxValue - m_MinValue);
...
public void SetTickStrengths(float tickMinSpacing, float tickMaxSpacing,
	...
	for (int i = m_TickModulos.Length - 1; i >= 0; i--) {
		...
		// Beyond kTickHeightFatThreshold the ticks don't get any bigger or
		// fatter, so ignore them, since they are already covered by smalle modulo
		// ticks anyway:
		if (m_TickStrengths[i] >= 1)
			m_BiggestTick = i;

		// Do not show tick markers less than 3 pixels apart:
		if (tickSpacing <= tickMinSpacing) {
			m_SmallestTick = i;
			break;
		}
	}
