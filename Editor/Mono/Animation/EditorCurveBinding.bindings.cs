...
public struct EditorCurveBinding : IEquatable<EditorCurveBinding> {
  ...
  public bool isPPtrCurve {
    get { return m_isPPtrCurve != 0; }
  }
  public bool isDiscreteCurve {
    get { return m_isDiscreteCurve != 0; }
  }
  internal bool isPhantom {
    get { return m_isPhantom != 0; }
    set { m_isPhantom = value == true ? 1 : 0; }
  }

  public static bool operator ==(EditorCurveBinding lhs,
                                 EditorCurveBinding rhs) {
    // Only if classID actually has been setup do we compare it (It might only
    // be type)
    if (lhs.m_ClassID != 0 && rhs.m_ClassID != 0) {
      if (lhs.m_ClassID != rhs.m_ClassID ||
          lhs.m_ScriptInstanceID != rhs.m_ScriptInstanceID)
        return false;
    }

    return lhs.m_isPPtrCurve == rhs.m_isPPtrCurve &&
           lhs.m_isDiscreteCurve == rhs.m_isDiscreteCurve &&
  ...
  public override int GetHashCode() {
    return String.Format("{0}:{1}:{2}", path, type.Name, propertyName)
        .GetHashCode();
  }
  ...
  public bool Equals(EditorCurveBinding other) { return this == other; }
  ...
  static private void BaseCurve(string inPath, System.Type inType,
  ...
  static public EditorCurveBinding FloatCurve(string inPath, System.Type inType,
                                              string inPropertyName) {
  ...
  static public EditorCurveBinding PPtrCurve(string inPath, System.Type inType,
                                             string inPropertyName) {
    EditorCurveBinding binding;
    BaseCurve(inPath, inType, inPropertyName, out binding);
  ...
  DiscreteCurve(string inPath, System.Type inType, string inPropertyName) {
    EditorCurveBinding binding;
    BaseCurve(inPath, inType, inPropertyName, out binding);
    binding.m_isPPtrCurve = 0;
    binding.m_isDiscreteCurve = 1;
