...
// will not work and will start failing miserably (mouse events not hitting it,
// etc.). That's because ZoomableArea is using GUILayout itself, and needs an
// actual physical rectangle.

[System.Serializable]
internal class ZoomableArea {
  // Global state
  private static Vector2 m_MouseDownPosition =
      new Vector2(-1000000, -1000000); // in transformed space
  ...
  public float hRangeMin {
    get {
      return (hAllowExceedBaseRangeMin ? Mathf.NegativeInfinity : hBaseRangeMin);
    }
    set {
      SetAllowExceed(ref m_HBaseRangeMin, ref m_HAllowExceedBaseRangeMin,
                     value);
    }
  }
  public float hRangeMax {
    ...
    set {
      SetAllowExceed(ref m_HBaseRangeMax, ref m_HAllowExceedBaseRangeMax,
                     value);
    }
  }
  public float vRangeMin {
    get {
      return (vAllowExceedBaseRangeMin ? Mathf.NegativeInfinity : vBaseRangeMin);
    }
    set {
      SetAllowExceed(ref m_VBaseRangeMin, ref m_VAllowExceedBaseRangeMin,
                     value);
    }
  }
  public float vRangeMax {
    ...
    set {
      SetAllowExceed(ref m_VBaseRangeMax, ref m_VAllowExceedBaseRangeMax,
                     value);
    }
  }
  private void SetAllowExceed(ref float rangeEnd, ref bool allowExceed,
                              float value) {
  ...
  // if the min and max scaling does not allow for actual zooming, there is no
  // point in allowing it
  protected bool allowSliderZoomHorizontal {
    get { return m_EnableSliderZoomHorizontal && m_HScaleMin < m_HScaleMax; }
  }
  protected bool allowSliderZoomVertical {
    get { return m_EnableSliderZoomVertical && m_VScaleMin < m_VScaleMax; }
  }
  ...
  // This is optional now, but used to be default behaviour because
  // ZoomableAreas are mostly graphs with +Y being up
  public enum YDirection { Positive, Negative }
  [SerializeField]
  ...
  public float vSliderWidth {
    get { return vSlider ? styles.sliderWidth : 0f; }
  }
  public float hSliderHeight {
    get { return hSlider ? styles.sliderWidth : 0f; }
  }
  ...
  public class Styles {
    ...
    public GUIStyle horizontalMinMaxScrollbarThumb {
      get { return GetSliderAxisStyle(enableSliderZoomHorizontal).horizontal; }
    }
    public GUIStyle horizontalScrollbarLeftButton;
    public GUIStyle horizontalScrollbarRightButton;
    public GUIStyle verticalScrollbar;
    public GUIStyle verticalMinMaxScrollbarThumb {
      get { return GetSliderAxisStyle(enableSliderZoomVertical).vertical; }
    }
    ...
    private SliderTypeStyles.SliderAxisStyles
    GetSliderAxisStyle(bool enableSliderZoom) {
      if (minimalGUI)
        return enableSliderZoom ? minimalSliderStyles
            .minMaxSliders : minimalSliderStyles.scrollbar;
      else
        return enableSliderZoom ? normalSliderStyles
            .minMaxSliders : normalSliderStyles.scrollbar;
    }
    ...
    public void InitGUIStyles(bool minimalGUI, bool enableSliderZoomHorizontal,
                              bool enableSliderZoomVertical) {
      ...
      if (minimalGUI) {
        if (minimalSliderStyles == null) {
          minimalSliderStyles = new SliderTypeStyles(){
              scrollbar =
                  new SliderTypeStyles.SliderAxisStyles(){
  ...
  public Rect rect {
    get {
      return new Rect(
          ...
          drawRect.width + (m_VSlider ? styles.visualSliderWidth : 0),
          drawRect.height + (m_HSlider ? styles.visualSliderWidth : 0));
    }
    set {
      Rect newDrawArea =
          new Rect(value.x, value.y,
                   value.width - (m_VSlider ? styles.visualSliderWidth : 0),
                   value.height - (m_HSlider ? styles.visualSliderWidth : 0));
      if (newDrawArea != m_DrawArea) {
        if (m_ScaleWithWindow) {
          ...
          m_Translation +=
              new Vector2((newDrawArea.width - m_DrawArea.width) / 2,
                          (newDrawArea.height - m_DrawArea.height) / 2);
          m_DrawArea = newDrawArea;
        }
      }
  ...
  public void SetShownHRangeInsideMargins(float min, float max) {
    ...
    if (widthInsideMargins < kMinWidth)
      widthInsideMargins = kMinWidth;
  ...
  public void SetShownVRangeInsideMargins(float min, float max) {
    ...
    if (heightInsideMargins < kMinHeight)
      heightInsideMargins = kMinHeight;
  ...
  public Rect shownArea {
    ...
    get {
      if (m_UpDirection == YDirection.Positive) {
        return new Rect(-m_Translation.x / m_Scale.x,
                        -(m_Translation.y - drawRect.height) / m_Scale.y,
                        drawRect.width / m_Scale.x,
                        drawRect.height / -m_Scale.y);
      } else {
        return new Rect(
            -m_Translation.x / m_Scale.x, -m_Translation.y / m_Scale.y,
  ...
  float GetWidthInsideMargins(float widthWithMargins,
                              bool substractSliderWidth = false) {
    float width = (widthWithMargins < kMinWidth) ? kMinWidth : widthWithMargins;
    float widthInsideMargins =
        width - leftmargin - rightmargin -
        (substractSliderWidth ?(m_VSlider ? styles.visualSliderWidth : 0) : 0);
    return Mathf.Max(widthInsideMargins, kMinWidth);
  }

  float GetHeightInsideMargins(float heightWithMargins,
                               bool substractSliderHeight = false) {
    float height =
        (heightWithMargins < kMinHeight) ? kMinHeight : heightWithMargins;
    float heightInsideMargins =
        height - topmargin - bottommargin -
        (substractSliderHeight ?(m_HSlider ? styles.visualSliderWidth : 0) : 0);
    return Mathf.Max(heightInsideMargins, kMinHeight);
  }

  public virtual Bounds drawingBounds {
    get {
      return new Bounds(new Vector3((hBaseRangeMin + hBaseRangeMax) * 0.5f,
                                    (vBaseRangeMin + vBaseRangeMax) * 0.5f, 0),
                        new Vector3(hBaseRangeMax - hBaseRangeMin,
                                    vBaseRangeMax - vBaseRangeMin, 1));
    }
  }
  ...
  public Matrix4x4 drawingToViewMatrix {
    get {
      return Matrix4x4.TRS(m_Translation, Quaternion.identity,
                           new Vector3(m_Scale.x, m_Scale.y, 1));
    }
  }

  public Vector2 DrawingToViewTransformPoint(Vector2 lhs) {
    return new Vector2(lhs.x * m_Scale.x + m_Translation.x,
                       lhs.y * m_Scale.y + m_Translation.y);
  }
  public Vector3 DrawingToViewTransformPoint(Vector3 lhs) {
    return new Vector3(lhs.x * m_Scale.x + m_Translation.x,
                       lhs.y * m_Scale.y + m_Translation.y, 0);
  }

  public Vector2 ViewToDrawingTransformPoint(Vector2 lhs) {
    return new Vector2((lhs.x - m_Translation.x) / m_Scale.x,
                       (lhs.y - m_Translation.y) / m_Scale.y);
  }
  public Vector3 ViewToDrawingTransformPoint(Vector3 lhs) {
    return new Vector3((lhs.x - m_Translation.x) / m_Scale.x,
                       (lhs.y - m_Translation.y) / m_Scale.y, 0);
  }
  ...
  private bool IsZoomEvent() {
    return ((Event.current.button == 1 && Event.current.alt) // right+alt drag
            //|| (Event.current.button == 0 && Event.current.command) //
            //left+commend drag
  ...
  private bool IsPanEvent() {
    return ((Event.current.button == 0 && Event.current.alt) // left+alt drag
            ||
            (Event.current.button == 2 && !Event.current.command) // middle drag
    );
  }

  public ZoomableArea() { m_MinimalGUI = false; }

  public ZoomableArea(bool minimalGUI) { m_MinimalGUI = minimalGUI; }

  public ZoomableArea(bool minimalGUI, bool enableSliderZoom)
      : this(minimalGUI, enableSliderZoom, enableSliderZoom) {}

  public ZoomableArea(bool minimalGUI, bool enableSliderZoomHorizontal,
                      bool enableSliderZoomVertical) {
  ...
  public void BeginViewGUI() {
    if (styles.horizontalScrollbar == null)
      styles.InitGUIStyles(m_MinimalGUI, allowSliderZoomHorizontal,
                           allowSliderZoomVertical);

    if (enableMouseInput)
      HandleZoomAndPanEvents(m_DrawArea);

    horizontalScrollbarID = GUIUtility.GetControlID(
        EditorGUIExt.s_MinMaxSliderHash, FocusType.Passive);
    verticalScrollbarID = GUIUtility.GetControlID(
        EditorGUIExt.s_MinMaxSliderHash, FocusType.Passive);
  ...
  void SliderGUI() {
    ...
    using(new EditorGUI.DisabledScope(!enableMouseInput)) {
      ...
      if (m_HSlider) {
        Rect hRangeSliderRect =
            new Rect(drawRect.x + 1, drawRect.yMax - inset,
                     drawRect.width - otherInset, styles.sliderWidth);
        float shownXRange = area.width;
        float shownXMin = area.xMin;
        if (allowSliderZoomHorizontal) {
          EditorGUIExt.MinMaxScroller(
              hRangeSliderRect, horizontalScrollbarID, ref shownXMin,
              ref shownXRange, editorBounds.min.x, editorBounds.max.x,
              Mathf.NegativeInfinity, Mathf.Infinity,
              styles.horizontalScrollbar, styles.horizontalMinMaxScrollbarThumb,
              styles.horizontalScrollbarLeftButton,
              styles.horizontalScrollbarRightButton, true);
        } else {
          shownXMin = GUI.Scroller(hRangeSliderRect, shownXMin, shownXRange,
                                   editorBounds.min.x, editorBounds.max.x,
                                   styles.horizontalScrollbar,
                                   styles.horizontalMinMaxScrollbarThumb,
                                   styles.horizontalScrollbarLeftButton,
                                   styles.horizontalScrollbarRightButton, true);
        }
        min = shownXMin;
      ...
      // Reverse y values since y increses upwards for the draw area but
      // downwards for the slider
      if (m_VSlider) {
        if (m_UpDirection == YDirection.Positive) {
          Rect vRangeSliderRect =
              new Rect(drawRect.xMax - inset, drawRect.y, styles.sliderWidth,
                       drawRect.height - otherInset);
          float shownYRange = area.height;
          float shownYMin = -area.yMax;
          if (allowSliderZoomVertical) {
            EditorGUIExt.MinMaxScroller(
                vRangeSliderRect, verticalScrollbarID, ref shownYMin,
                ref shownYRange, -editorBounds.max.y, -editorBounds.min.y,
                Mathf.NegativeInfinity, Mathf.Infinity,
            ...
            shownYMin = GUI.Scroller(vRangeSliderRect, shownYMin, shownYRange,
          ...
          float rectHeightWithinMargins =
              GetHeightInsideMargins(rect.height, true);
          if (min > area.yMin)
          ...
          if (allowSliderZoomVertical) {
            EditorGUIExt.MinMaxScroller(
                vRangeSliderRect, verticalScrollbarID, ref shownYMin,
                ref shownYRange, editorBounds.min.y, editorBounds.max.y,
                Mathf.NegativeInfinity, Mathf.Infinity,
                styles.verticalScrollbar, styles.verticalMinMaxScrollbarThumb,
                styles.verticalScrollbarUpButton,
                styles.verticalScrollbarDownButton, false);
  ...
  private void HandleZoomEvent(Vector2 zoomAround, bool scrollwhell) {
    ...
    SetScaleFocused(zoomAround, scale * m_Scale, Event.current.shift,
                    EditorGUI.actionKey);
  }
  ...
  public void SetScaleFocused(Vector2 focalPoint, Vector2 newScale,
                              bool lockHorizontal, bool lockVertical) {
    if (uniformScale)
      lockHorizontal = lockVertical = false;
    else {
      // if an axis is locked by default, it is as if that modifier key is
      // permanently held down actually pressing the key then lifts the lock. In
      // other words, LockedByDefault acts like an inversion.
  ...
  public void EnforceScaleAndRange() {
    ...
    if (!Mathf.Approximately(newArea.width, oldArea.width)) {
      ...
      if (newArea.width < oldArea.width) {
        // The shown area decreasing in size means the scale is increasing. This
        // happens e.g. while zooming in. Only the max scale restricts the shown
        // area size here, range has no influence.
        constrainedWidth =
            GetWidthInsideMargins(drawRect.width / m_HScaleMax, false);
      } else {
        constrainedWidth =
            GetWidthInsideMargins(drawRect.width / m_HScaleMin, false);
      ...
      float xLerp =
          Mathf.InverseLerp(oldArea.width, newArea.width, constrainedWidth);
      float newWidth = Mathf.Lerp(oldArea.width, newArea.width, xLerp);
      float widthChange = Mathf.Abs(newWidth - newArea.width);
      newArea = new Rect(
          // only affect the position if there was any significant change in
          // width this fixes an issue where if width was only different due to
          // rounding issues, position changes are ignored as xLerp comes back 0
          // (or very nearly 0)
          widthChange > minChange ? Mathf.Lerp(oldArea.x, newArea.x, xLerp)
          : newArea.x, newArea.y, newWidth, newArea.height);
    }
    if (!Mathf.Approximately(newArea.height, oldArea.height)) {
      ...
      if (newArea.height < oldArea.height) {
        ...
        // area size here, range has no influence.
        constrainedHeight =
            GetHeightInsideMargins(drawRect.height / m_VScaleMax, false);
      } else {
        constrainedHeight =
            GetHeightInsideMargins(drawRect.height / m_VScaleMin, false);
      ...
      float yLerp =
          Mathf.InverseLerp(oldArea.height, newArea.height, constrainedHeight);
      float newHeight = Mathf.Lerp(oldArea.height, newArea.height, yLerp);
      float heightChange = Mathf.Abs(newHeight - newArea.height);
      newArea = new Rect(
          newArea.x,
          // only affect the position if there was any significant change in
          // height this fixes an issue where if height was only different due
          // to rounding issues, position changes are ignored as yLerp comes
          // back 0 (or very nearly 0)
          heightChange > minChange ? Mathf.Lerp(oldArea.y, newArea.y, yLerp)
          : newArea.y, newArea.width, newHeight);
    }
  ...
  public void UpdateZoomScale(float fMaxScaleValue, float fMinScaleValue) {
    // Update/reset the values of the scale to new zoom range, if the current
    // values do not fall in the range of the new resolution
