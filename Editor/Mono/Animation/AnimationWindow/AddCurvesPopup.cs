...
Vector2 GetWindowSize() {
	...
	float width = Mathf.Clamp(contentWidth + k_SpaceForSlider + k_WindowPadding,
	                          k_WindowMinWidth, k_WindowMaxWidth);
	return new Vector2(width, k_WindowFixedHeight);
}

void Init(Rect buttonRect) {
	s_Hierarchy = new AddCurvesPopupHierarchy();
	s_Hierarchy.InitIfNeeded(
		this, new Rect(0, 0, k_WindowMinWidth, k_WindowFixedHeight));
...
internal static bool ShowAtPosition(Rect buttonRect,
                                    ...
                                    OnNewCurveAdded newCurveCallback) {
...
internal void OnGUI() {
	...
	Rect rect = new Rect(1, 1, windowSize.x - k_WindowPadding,
	                     windowSize.y - k_WindowPadding);
	GUI.Box(new Rect(0, 0, windowSize.x, windowSize.y), GUIContent.none,
	        "grey_border");
