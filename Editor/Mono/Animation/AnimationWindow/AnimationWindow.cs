...
internal class AnimationWindow : EditorWindow,
  ...
  private static List<AnimationWindow> s_AnimationWindows =
      new List<AnimationWindow>();
  ...
  public void OnEnable() {
    ...
    m_RecordTitleContent = EditorGUIUtility.TextContentWithIcon(
        titleContent.text, "Animation.Record");
  ...
  public void OnDestroy() { DestroyImmediate(m_AnimEditor); }
  ...
  public void OnGUI() {
    ...
    titleContent = m_AnimEditor.state.recording ? m_RecordTitleContent
        : m_DefaultTitleContent;
  ...
  public void OnFocus() { OnSelectionChange(); }
  ...
  public bool EditAnimationClip(AnimationClip animationClip) {
    ...
    EditAnimationClipInternal(animationClip, (Object) null,
                              (IAnimationWindowControl) null);
    return true;
  }

  public bool EditSequencerClip(AnimationClip animationClip,
                                Object sourceObject,
  ...
  public void UnlinkSequencer() {
    if (state.linkedWithSequencer) {
      ...
      // Selected object could have been changed when unlocking the animation
      // window
  ...
  private bool
  EditGameObjectInternal(GameObject gameObject,
                         IAnimationWindowControl controlInterface) {
    ...
    if (ShouldUpdateGameObjectSelection(newSelection)) {
      ...
      m_LastSelectedObjectID =
          gameObject != null ? gameObject.GetInstanceID() : 0;
    } else
  ...
  private void
  EditAnimationClipInternal(AnimationClip animationClip, Object sourceObject,
                            IAnimationWindowControl controlInterface) {
    var newSelection =
        AnimationClipSelectionItem.Create(animationClip, sourceObject);
    if (ShouldUpdateSelection(newSelection)) {
      m_AnimEditor.selection = newSelection;
      m_AnimEditor.overrideControlInterface = controlInterface;

      m_LastSelectedObjectID =
          animationClip != null ? animationClip.GetInstanceID() : 0;
    } else
      m_AnimEditor.OnSelectionUpdated();
  }
  ...
  private bool
  ShouldUpdateGameObjectSelection(GameObjectSelectionItem selectedItem) {
    ...
    if (currentSelection.rootGameObject != null) {
      AnimationClip[] allClips =
          AnimationUtility.GetAnimationClips(currentSelection.rootGameObject);
  ...
  private bool
  ShouldUpdateSelection(AnimationWindowSelectionItem selectedItem) {
    if (m_LockTracker.isLocked)
      return false;
  ...
  private void UndoRedoPerformed() { Repaint(); }
