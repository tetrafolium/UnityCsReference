...
internal class DopeSheetEditor : TimeArea,
  ...
  // How much rendered keyframe left edge is visually offset when compared to
  // the time it represents. A diamond shape left edge isn't representing the
  // time, the middle part is.
  private const float k_KeyframeOffset = -6.5f;
  // Pptr keyframe preview also needs 1px offset so it sits more tightly in the
  // grid
  ...
  class DopeSheetControlPointRenderer {
    ...
    private List<DrawElement> m_UnselectedKeysDrawBuffer =
        new List<DrawElement>();
    private List<DrawElement> m_SelectedKeysDrawBuffer =
        new List<DrawElement>();
    private List<DrawElement> m_DragDropKeysDrawBuffer =
        new List<DrawElement>();
    ...
    public void AddUnselectedKey(DrawElement element) {
      ...
      if (element.texture != null) {
        ...
        rect.size = new Vector2(m_DefaultDopeKeyIcon.width,
                                m_DefaultDopeKeyIcon.height);
  ...
  public float contentHeight {
    get {
      ...
      foreach (DopeLine dopeline in state.dopelines)
        height += dopeline.tallMode ? AnimationWindowHierarchyGUI
                      .k_DopeSheetRowHeightTall
            : AnimationWindowHierarchyGUI.k_DopeSheetRowHeight;
  ...
  public bool spritePreviewLoading {
    get { return m_SpritePreviewLoading; }
  }

  public DopeSheetEditor(EditorWindow owner) : base(false) { m_Owner = owner; }
  ...
  public void RecalculateBounds() {
    if (!state.disabled) {
      ...
      m_Bounds.SetMinMax(new Vector3(timeRange.x, 0, 0),
                         new Vector3(timeRange.y, 0, 0));
    }
  }
  ...
  private Rect DopelinesGUI(Rect position, Vector2 scrollPosition) {
    ...
    // Workaround for cases when mouseup happens outside the window. Apparently
    // the mouseup event is lost (not true on OSX, though).
    ...
    for (int i = 0; i < dopelines.Count; ++i) {
      ...
      dopeLine.position.height =
          (dopeLine.tallMode ? AnimationWindowHierarchyGUI
               .k_DopeSheetRowHeightTall
           : AnimationWindowHierarchyGUI.k_DopeSheetRowHeight);

      // Cull out dopelines that are not visible
      if (dopeLine.position.yMin + scrollPosition.y >= position.yMin &&
              dopeLine.position.yMin + scrollPosition.y <= position.yMax ||
          dopeLine.position.yMax + scrollPosition.y >= position.yMin &&
              dopeLine.position.yMax + scrollPosition.y <= position.yMax) {
    ...
    Rect dopelinesRect = new Rect(position.xMin, position.yMin, position.width,
                                  linePosition.yMax - position.yMin);
  ...
  private void RectangleToolGUI() { m_RectangleTool.OnGUI(); }

  public void DrawMasterDopelineBackground(Rect position) {
    if (Event.current.type != EventType.Repaint)
      return;

    AnimationWindowStyles.eventBackground.Draw(position, false, false, false,
                                               false);
  }
  ...
  void UpdateSpritePreviewCacheSize() {
    ...
    if (newPreviewCacheSize > m_SpritePreviewCacheSize) {
      AssetPreview.SetPreviewTextureCacheSize(newPreviewCacheSize,
                                              assetPreviewManagerID);
  ...
  private void DopeLineRepaint(DopeLine dopeline) {
    ...
    AnimationWindowHierarchyNode node =
        (AnimationWindowHierarchyNode)
            state.hierarchyData.FindItem(dopeline.hierarchyNodeID);
    bool isChild = node != null && node.depth > 0;
    Color color = isChild ? Color.gray.AlphaMultiplied(0.05f)
        : Color.gray.AlphaMultiplied(0.16f);
    ...
    for (int i = 0; i < length; i++) {
      ...
      color = dopeline.isMasterDopeline ? Color.gray.RGBMultiplied(0.85f)
          : Color.gray.RGBMultiplied(1.2f);
      Texture2D texture = null;

      if (keyframe.isPPtrCurve && dopeline.tallMode)
        texture = keyframe.value == null ? null
            : AssetPreview.GetAssetPreview(
                  ((Object) keyframe.value).GetInstanceID(),
                  assetPreviewManagerID);

      if (texture != null) {
        rect = GetPreviewRectFromKeyFrameRect(rect);
        color = Color.white.AlphaMultiplied(0.5f);
      } else if (keyframe.value != null && keyframe.isPPtrCurve &&
                 dopeline.tallMode) {
    ...
    if (DoDragAndDrop(dopeline, dopeline.position, false)) {
      float time = Mathf.Max(
          state.PixelToTime(Event.current.mousePosition.x,
                            AnimationWindowState.SnapMode.SnapToClipFrame),
          0f);
      ...
      foreach (Object obj in GetSortedDragAndDropObjectReferences()) {
        ...
        if (dopeline.isPptrDopeline && dopeline.tallMode)
          texture = AssetPreview.GetAssetPreview(obj.GetInstanceID(),
                                                 assetPreviewManagerID);
        ...
        m_PointRenderer.AddDragDropKey(
            new DrawElement(rect, keyColor, texture));
  ...
  private Rect GetDragAndDropRect(DopeLine dopeline, float time) {
    ...
    rect.center = new Vector2(
        state.TimeToPixel(time) + rect.width * .5f + offsetX, rect.center.y);
    return rect;
  }
  ...
  private GenericMenu GenerateMenu(DopeLine dopeline) {
    ...
    List<AnimationWindowKeyframe> hoveringKeys =
    ...
    AnimationKeyTime mouseKeyTime = AnimationKeyTime.Time(
        state.PixelToTime(Event.current.mousePosition.x,
                          AnimationWindowState.SnapMode.SnapToClipFrame),
        state.frameRate);
    ...
    if (dopeline.isEditable && hoveringKeys.Count == 0)
      menu.AddItem(new GUIContent(str), false, AddKeyToDopeline,
                   new AddKeyToDopelineContext{dopeline = dopeline,
                                               time = mouseKeyTime});
    else
      menu.AddDisabledItem(new GUIContent(str));

    str = state.selectedKeys.Count > 1 ? "Delete Keys" : "Delete Key";
    if (dopeline.isEditable &&
        (state.selectedKeys.Count > 0 || hoveringKeys.Count > 0))
      menu.AddItem(new GUIContent(str), false, DeleteKeys,
                   state.selectedKeys.Count > 0 ? state.selectedKeys
                                                : hoveringKeys);
    else
      menu.AddDisabledItem(new GUIContent(str));
    ...
    if (dopeline.isEditable &&
        AnimationWindowUtility.ContainsFloatKeyframes(state.selectedKeys)) {
      ...
      foreach (AnimationWindowKeyframe key in state.selectedKeys) {
        ...
        int index = key.curve.GetKeyframeIndex(
            AnimationKeyTime.Time(key.time, state.frameRate));
        if (index == -1)
          continue;
        ...
        if (curve == null) {
          curve = AnimationUtility.GetEditorCurve(key.curve.clip,
                                                  key.curve.binding);
          if (curve == null)
  ...
  private void HandleDragging() {
    int id = EditorGUIUtility.GetControlID("dopesheetdrag".GetHashCode(),
                                           FocusType.Passive, new Rect());
    EventType eventType = Event.current.GetTypeForControl(id);

    if ((eventType == EventType.MouseDrag || eventType == EventType.MouseUp) &&
        m_MousedownOnKeyframe) {
      if (eventType == EventType.MouseDrag && !EditorGUI.actionKey &&
          !Event.current.shift) {
        if (!m_IsDragging && state.selectedKeys.Count > 0) {
          ...
          m_RectangleTool.OnStartMove(new Vector2(m_DragStartTime, 0f),
                                      state.rippleTime);
          Event.current.Use();
        }
      }

      // What is the distance from first selected key to zero time. We need this
      // in order to make sure no key goes to negative time while dragging.
      float firstSelectedKeyTime = float.MaxValue;
      foreach (AnimationWindowKeyframe selectedKey in state.selectedKeys)
        firstSelectedKeyTime =
            Mathf.Min(selectedKey.time, firstSelectedKeyTime);

      float currentTime =
          state.SnapToFrame(state.PixelToTime(Event.current.mousePosition.x),
                            AnimationWindowState.SnapMode.SnapToClipFrame);
    ...
    if (m_IsDraggingPlayheadStarted && eventType == EventType.MouseDrag &&
        Event.current.button == 1) {
      m_IsDraggingPlayhead = true;

      // int frame = state.m_Frame;
      // if (!m_IsDragging)
      //  frame = state.TimeToFrameFloor(state.SnapToFrame (state.PixelToTime
    ...
    if (m_IsDragging) {
      ...
      Rect mouseRect =
          new Rect(mousePosition.x - 10, mousePosition.y - 10, 20, 20);
  ...
  private void HandleDelete() {
    ...
    case EventType.ExecuteCommand:
      if ((Event.current.commandName == EventCommandNames.SoftDelete ||
  ...
  private void HandleDragAndDropToEmptyArea() {
    ...
    if (DragAndDrop
                .objectReferences [0]
                .GetType() == typeof(Sprite) ||
        DragAndDrop
                .objectReferences [0]
                .GetType() == typeof(Texture2D)) {
        ...
        if (!DopelineForValueTypeExists(typeof(Sprite))) {
          if (evt.type == EventType.DragPerform) {
            EditorCurveBinding? spriteBinding =
                CreateNewPptrDopeline(state.selection, typeof(Sprite));
            if (spriteBinding != null) {
              DoSpriteDropAfterGeneratingNewDopeline(state.activeAnimationClip,
                                                     spriteBinding);
            }
          }
  ...
  private void
  DoSpriteDropAfterGeneratingNewDopeline(AnimationClip animationClip,
                                         EditorCurveBinding? spriteBinding) {
    // Create the new curve for our sprites
    AnimationWindowCurve newCurve = new AnimationWindowCurve(
        animationClip, (EditorCurveBinding) spriteBinding, typeof(Sprite));

    // And finally perform the drop onto the curve
    PerformDragAndDrop(newCurve, 0f);
  }

  private void HandleRectangleToolEvents() { m_RectangleTool.HandleEvents(); }

  private bool DopelineForValueTypeExists(Type valueType) {
    return state.allCurves.Exists(curve => curve.valueType == valueType);
  }

  private EditorCurveBinding?
  CreateNewPptrDopeline(AnimationWindowSelectionItem selectedItem,
                        Type valueType) {
    List<EditorCurveBinding> potentialBindings = null;
    if (selectedItem.rootGameObject != null) {
      potentialBindings = AnimationWindowUtility.GetAnimatableProperties(
          selectedItem.rootGameObject, selectedItem.rootGameObject, valueType);
      if (potentialBindings.Count == 0 &&
          valueType == typeof(Sprite)) // No animatable properties for Sprite
                                       // available. Default as SpriteRenderer.
      {
      ...
      potentialBindings = AnimationWindowUtility.GetAnimatableProperties(
          selectedItem.scriptableObject, valueType);
    }

    if (potentialBindings == null || potentialBindings.Count == 0)
      return null;

    if (potentialBindings.Count ==
        1) // Single property for this valuetype, return it
    {
    ...
    } else // Multiple properties, dropdown selection
    {
      ...
      Rect r = new Rect(Event.current.mousePosition.x,
                        Event.current.mousePosition.y, 1, 1);
      EditorUtility.DisplayCustomMenu(
          r, EditorGUIUtility.TempContent(menuItems.ToArray()), -1,
          SelectTypeForCreatingNewPptrDopeline, userDataList);
      return null; // We return null, but creation is handled via dropdown
                   // callback code
    }
  }

  private void SelectTypeForCreatingNewPptrDopeline(object userData,
                                                    string[] options,
                                                    int selected) {
    ...
    List<EditorCurveBinding> bindings =
        userDataList[1] as List<EditorCurveBinding>;
  ...
  private EditorCurveBinding?
  CreateNewSpriteRendererDopeline(GameObject targetGameObject,
    ...
    List<EditorCurveBinding> curveBindings =
        AnimationWindowUtility.GetAnimatableProperties(
            targetGameObject, rootGameObject, typeof(SpriteRenderer),
            typeof(Sprite));
  ...
  private void HandleDragAndDrop(DopeLine dopeline) {
    ...
    if (DoDragAndDrop(dopeline, dopeline.position,
  ...
  private void HandleMouseDown(DopeLine dopeline) {
    ...
    // If there are no selected keyframe in click without shift or
    // EditorGUI.actionKey, then clear all other selections
      ...
      if (r.Contains(evt.mousePosition)) {
        ...
        if (canUnselectKeys) {
          ...
          if (!state.KeyIsSelected(keyframe)) {
            ...
            if (!dopeline.isMasterDopeline)
              state.SelectHierarchyItem(dopeline,
                                        EditorGUI.actionKey || evt.shift);
          }
        }
    ...
    if (dopeline.isMasterDopeline) {
      HashSet<int> hierarchyIDs =
          state.GetAffectedHierarchyIDs(state.selectedKeys);
      state.SelectHierarchyItems(hierarchyIDs, true, true);
    }

    if (evt.clickCount == 2 && evt.button == 0 && !Event.current.shift &&
        !EditorGUI.actionKey)
  ...
  private void HandleDopelineDoubleclick(DopeLine dopeline) {
    float timeAtMousePosition =
        state.PixelToTime(Event.current.mousePosition.x,
                          AnimationWindowState.SnapMode.SnapToClipFrame);
    AnimationKeyTime mouseKeyTime =
        AnimationKeyTime.Time(timeAtMousePosition, state.frameRate);
  ...
  private Rect GetKeyframeRect(DopeLine dopeline,
                               AnimationWindowKeyframe keyframe) {
    float time = keyframe != null ? keyframe.time : 0f;

    float width = 10f;
    if (dopeline.isPptrDopeline && dopeline.tallMode &&
        (keyframe == null || keyframe.value != null))
      width = dopeline.position.height;

    return new Rect(state.TimeToPixel(state.SnapToFrame(
                        time, AnimationWindowState.SnapMode.SnapToClipFrame)) +
                        GetKeyframeOffset(dopeline, keyframe),
                    dopeline.position.yMin, width, dopeline.position.height);
  }

  // This means "how much is the rendered keyframe offset in pixels for x-axis".
  // Say you are rendering keyframe to some time t. The time t relates to some
  // pixel x, but you then need to offset because keyframe diamond center
  // represents the time, not the left edge However for pptr keyframes, the time
  // is represented by left edge
      ...
      if (dopelinesSelected) {
        foreach (AnimationWindowCurve curve in state.activeCurves) {
          ...
          if (keyCount > 1) {
            ...
            Vector2 pt2 =
                new Vector2(curve.m_Keyframes[keyCount - 1].time, 0.0f);
  ...
  private bool DoDragAndDrop(DopeLine dopeLine, Rect position, bool perform) {
    ...
    System.Type targetType = DragAndDrop
                                 .objectReferences [0]
                                 .GetType();
    ...
    if (dopeLine.valueType == targetType) {
      ...
      // dopeline ValueType wasn't exact match. We can still look for a curve
      // that accepts our drop object type
      foreach (AnimationWindowCurve dopelineCurve in dopeLine.curves) {
        if (dopelineCurve.isPPtrCurve) {
          ...
          List<Sprite> sprites = SpriteUtility.GetSpriteFromPathsOrObjects(
              DragAndDrop.objectReferences, DragAndDrop.paths,
              Event.current.type);
  ...
  private void PerformDragAndDrop(AnimationWindowCurve targetCurve,
                                  float time) {
    ...
    int startFrame =
        AnimationKeyTime.Time(time, targetCurve.clip.frameRate).frame;
    ...
    for (int i = 0; i < objectReferences.Length; ++i) {
      ...
      CreateNewPPtrKeyframe(
          AnimationKeyTime.Frame(startFrame + i, targetCurve.clip.frameRate)
              .time,
          value, targetCurve);
    }
  ...
  private Object[] GetSortedDragAndDropObjectReferences() {
    ...
    System.Array.Sort(objectReferences,
                      (a, b) => EditorUtility.NaturalCompare(a.name, b.name));

    return objectReferences;
  }

  private void CreateNewPPtrKeyframe(float time, Object value,
                                     AnimationWindowCurve targetCurve) {
    ...
    AnimationWindowKeyframe keyframe =
        new AnimationWindowKeyframe(targetCurve, referenceKeyframe);
    AnimationKeyTime newTime =
        AnimationKeyTime.Time(keyframe.time, state.frameRate);
    targetCurve.AddKeyframe(keyframe, newTime);
    state.SelectKey(keyframe);
  }

  // if targetType == null, it means that all types are fine (as long as they
  // are all of the same type)
  private static bool ValidateDragAndDropObjects() {
    if (DragAndDrop.objectReferences.Length == 0)
      return false;

    // Let's be safe and early out if any of the objects are null or if they
    // aren't all of the same type (exception beign sprite vs. texture2D, which
    ...
    for (int i = 0; i < DragAndDrop.objectReferences.Length; i++) {
      ...
      if (i < DragAndDrop.objectReferences.Length - 1) {
        ...
        bool bothAreSpritesOrTextures =
            (obj is Texture2D || obj is Sprite) &&
            (nextObj is Texture2D || nextObj is Sprite);
  ...
  private AnimationWindowCurve GetCurveOfType(DopeLine dopeLine,
                                              System.Type type) {
  ...
  // For optimizing. Starting from keyIndex, we check through any key with same
  // time and see if any are selected
  ...
  private void AddKeyToDopeline(object obj) {
    AddKeyToDopeline((AddKeyToDopelineContext) obj);
  }
  private void AddKeyToDopeline(AddKeyToDopelineContext context) {
    AnimationWindowUtility.AddKeyframes(state, context.dopeline.curves,
                                        context.time);
  }

  private void DeleteKeys(object obj) {
    DeleteKeys((List<AnimationWindowKeyframe>) obj);
  }
  ...
  internal class DopeSheetSelectionRect {
    ...
    public DopeSheetSelectionRect(DopeSheetEditor owner) { this.owner = owner; }
    ...
    public void OnGUI(Rect position) {
      ...
      case EventType.Repaint:
        if (GUIUtility.hotControl == id && m_ValidRect)
          EditorStyles.selectionRect.Draw(GetCurrentPixelRect(),
                                          GUIContent.none, false, false, false,
                                          false);
        break;
      ...
      case EventType.MouseUp:
        if (GUIUtility.hotControl == id && evt.button == 0) {
          if (m_ValidRect) {
            ...
            HashSet<AnimationWindowKeyframe> toBeUnselected =
                new HashSet<AnimationWindowKeyframe>();
            HashSet<AnimationWindowKeyframe> toBeSelected =
                new HashSet<AnimationWindowKeyframe>();

            foreach (DopeLine dopeline in owner.state.dopelines) {
              if (dopeline.position.yMin >= timeRect.yMin &&
                  dopeline.position.yMax <= timeRect.yMax) {
                foreach (AnimationWindowKeyframe keyframe in dopeline.keys) {
                  AnimationKeyTime startTime =
                      AnimationKeyTime.Time(timeRect.xMin, frameRate);
                  AnimationKeyTime endTime =
                      AnimationKeyTime.Time(timeRect.xMax, frameRate);

                  AnimationKeyTime keyTime =
                      AnimationKeyTime.Time(keyframe.time, frameRate);
                  // for dopeline tallmode, we don't want to select the sprite
                  // at the end. It just feels wrong.
                  if (!dopeline.tallMode && keyTime.frame >= startTime.frame &&
                          keyTime.frame <= endTime.frame ||
                      dopeline.tallMode && keyTime.frame >= startTime.frame &&
                          keyTime.frame < endTime.frame) {
                    if (!toBeSelected.Contains(keyframe) &&
                        !toBeUnselected.Contains(keyframe)) {
            ...
            // Only if all the keys inside rect are selected, we want to
            // unselect them.
    ...
    public Rect GetCurrentPixelRect() {
      ...
      Rect r = AnimationWindowUtility.FromToRect(m_SelectStartPoint,
                                                 m_SelectMousePoint);
      r.xMin = owner.state.TimeToPixel(
          owner.state.PixelToTime(
              r.xMin, AnimationWindowState.SnapMode.SnapToClipFrame),
          AnimationWindowState.SnapMode.SnapToClipFrame);
      r.xMax = owner.state.TimeToPixel(
          owner.state.PixelToTime(
              r.xMax, AnimationWindowState.SnapMode.SnapToClipFrame),
          AnimationWindowState.SnapMode.SnapToClipFrame);
    ...
    public Rect GetCurrentTimeRect() {
      ...
      Rect r = AnimationWindowUtility.FromToRect(m_SelectStartPoint,
                                                 m_SelectMousePoint);
      r.xMin = owner.state.PixelToTime(
          r.xMin, AnimationWindowState.SnapMode.SnapToClipFrame);
      r.xMax = owner.state.PixelToTime(
          r.xMax, AnimationWindowState.SnapMode.SnapToClipFrame);
      r.yMin = Mathf.Floor(r.yMin / height) * height;
      r.yMax = (Mathf.Floor(r.yMax / height) + 1) * height;
      return r;
  ...
  public void UpdateCurves(List<ChangedCurve> changedCurves, string undoText) {
    ...
    foreach (ChangedCurve changedCurve in changedCurves) {
      AnimationWindowCurve curve =
          state.allCurves.Find(c => changedCurve.curveId == c.GetHashCode());
      if (curve != null) {
        AnimationUtility.SetEditorCurve(curve.clip, changedCurve.binding,
                                        changedCurve.curve);
      } else {
        Debug.LogError(
            "Could not match ChangedCurve data to destination curves.");
      }
    }
  }
}
