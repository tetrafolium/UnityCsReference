...
internal class NormalCurveRenderer : CurveRenderer {
  ...
  private float rangeStart {
    get {
      return (m_CustomRangeStart == 0 && m_CustomRangeEnd == 0 &&
                      m_Curve.length > 0
                  ? m_Curve.keys[0].time
                  : m_CustomRangeStart);
    }
  ...
  private float rangeEnd {
    get {
      return (m_CustomRangeStart == 0 && m_CustomRangeEnd == 0 &&
                      m_Curve.length > 0
                  ? m_Curve.keys[m_Curve.length - 1].time
                  : m_CustomRangeEnd);
    }
  }
    ...
    get {
      if (!s_CurveMaterial) {
        Shader shader = (Shader) EditorGUIUtility.LoadRequired(
            "Editors/AnimationWindow/Curve.shader");
  ...
  public AnimationCurve GetCurve() { return m_Curve; }
  ...
  public virtual float ClampedValue(float value) { return value; }

  public virtual float EvaluateCurveSlow(float time) {
    return m_Curve.Evaluate(time);
  }

  // TODO: Implement proper analytic evaluation of curve delta instead of this
  // numeric hack
  public float EvaluateCurveDeltaSlow(float time) {
    float epsilon = 0.0001f;
    return (EvaluateCurveSlow(time + epsilon) -
            EvaluateCurveSlow(time - epsilon)) /
           (epsilon * 2);
  }

  private Vector3[] GetPoints() { return GetPoints(rangeStart, rangeEnd); }
  ...
  public static float[, ] CalculateRanges(float minTime, float maxTime,
                                          float rangeStart, float rangeEnd,
                                          WrapMode preWrapMode,
                                          WrapMode postWrapMode) {
    // Don't want to deal with optimizing for different pre and post wrap mode
    // for now
    ...
    if (wrap == WrapMode.Loop) {
      // If we are covering a range longer than a full loop, just add all
      // points:
      ...
      // Else, only add the needed range(s)
        ...
        // Find the start and end of needed shown range repeated into the range
        // of the curve
        minTime = Mathf.Repeat(minTime - rangeStart, rangeEnd - rangeStart) +
                  rangeStart;
        maxTime = Mathf.Repeat(maxTime - rangeStart, rangeEnd - rangeStart) +
                  rangeStart;
        if (minTime < maxTime)
  ...
  protected virtual int GetSegmentResolution(float minTime, float maxTime,
                                             float keyTime, float nextKeyTime) {
    float fullTimeRange = maxTime - minTime;
    float keyTimeRange = nextKeyTime - keyTime;
    int count = Mathf.RoundToInt(kSegmentWindowResolution *
                                 (keyTimeRange / fullTimeRange));
    return Mathf.Clamp(count, 1, kMaximumSampleCount);
  }

  protected virtual void AddPoint(ref List<Vector3> points, ref float lastTime,
                                  float sampleTime, ref float lastValue,
                                  float sampleValue) {
  ...
  private void AddPoints(ref List<Vector3> points, float minTime, float maxTime,
                         float visibleMinTime, float visibleMaxTime) {
    ...
    for (int i = 0; i < m_Curve.length - 1; i++) {
      ...
      // Get first value from actual key rather than evaluating curve (to
      // correctly handle stepped interpolation)
      points.Add(new Vector3(key.time, key.value));

      // Place second sample very close to first one (to correctly handle
      // stepped interpolation)
      int segmentResolution = GetSegmentResolution(
          visibleMinTime, visibleMaxTime, key.time, nextKey.time);
      float newTime =
          Mathf.Lerp(key.time, nextKey.time, 0.001f / segmentResolution);
      ...
      // Place second last sample very close to last one (to correctly handle
      // stepped interpolation)
      newTime =
          Mathf.Lerp(key.time, nextKey.time, 1 - 0.001f / segmentResolution);
      value = EvaluateCurveSlow(newTime);
      AddPoint(ref points, ref lastTime, newTime, ref lastValue, value);

      // Get last value from actual key rather than evaluating curve (to
      // correctly handle stepped interpolation)
  ...
  public void DrawCurve(float minTime, float maxTime, Color color,
                        Matrix4x4 transform, Color wrapColor) {
    BuildCurveMesh();
    ...
    if (keys.Length > 0) {
      ...
      DrawCurveWrapped(minTime, maxTime, rangeStart, rangeEnd, preWrapMode,
                       postWrapMode, m_CurveMesh, firstPoint, lastPoint,
                       transform, color, wrapColor);
    }
  }

  public static void DrawPolyLine(Matrix4x4 transform, float minDistance,
  ...
  public static void DrawCurveWrapped(float minTime, float maxTime,
                                      float rangeStart, float rangeEnd,
                                      WrapMode preWrap, WrapMode postWrap,
                                      Mesh mesh, Vector3 firstPoint,
                                      Vector3 lastPoint, Matrix4x4 transform,
                                      Color color, Color wrapColor) {
    ...
    if (rangeEnd - rangeStart != 0) {
      minRep =
          Mathf.FloorToInt((minTime - rangeStart) / (rangeEnd - rangeStart));
    ...
    // Draw wrapping
    mat.SetColor("_Color", new Color(wrapColor.r, wrapColor.g, wrapColor.b,
                                     wrapColor.a * color.a));
    Handles.color =
        new Color(wrapColor.r, wrapColor.g, wrapColor.b, wrapColor.a * color.a);

    // Draw pre wrapping
    if (preWrap == WrapMode.Loop) {
      Matrix4x4 firstTransform =
          Handles.matrix * transform *
          Matrix4x4.TRS(
              new Vector3(minRep * (rangeEnd - rangeStart), 0.0f, 0.0f),
              Quaternion.identity, Vector3.one);
      Matrix4x4 iterTransform =
          Matrix4x4.TRS(new Vector3(rangeEnd - rangeStart, 0.0f, 0.0f),
                        Quaternion.identity, Vector3.one);
      ...
      for (int r = minRep; r < 0; ++r) {
        ...
        Handles.DrawLine(curTransform.MultiplyPoint(lastPoint),
      ...
      for (int r = minRep; r < 0; ++r) {
        //  Even loop
        if (r % 2 == 0) {
          Matrix4x4 shiftTransform = Matrix4x4.TRS(
              new Vector3(r * (rangeEnd - rangeStart), 0.0f, 0.0f),
              Quaternion.identity, Vector3.one);
        ...
        //  Odd loop
          ...
          Matrix4x4 shiftTransform = Matrix4x4.TRS(
              new Vector3((r + 1) * (rangeEnd - rangeStart) + rangeStart * 2,
                          0.0f, 0.0f),
      ...
      if (minRep < 0) {
        Handles.DrawLine(
            ...
            transform.MultiplyPoint(new Vector3(
                Mathf.Min(maxTime, firstPoint.x), firstPoint.y, 0)));
      }
    }
    ...
    if (postWrap == WrapMode.Loop) {
      ...
      for (int r = 1; r <= maxRep; ++r) {
        ...
        Handles.DrawLine(prevTransform.MultiplyPoint(lastPoint),
      ...
      if (maxRep > 0) {
        Handles.DrawLine(
            transform.MultiplyPoint(
                new Vector3(Mathf.Max(minTime, lastPoint.x), lastPoint.y, 0)),
  ...
  public static void DrawCurveWrapped(float minTime, float maxTime,
                                      float rangeStart, float rangeEnd,
                                      WrapMode preWrap, WrapMode postWrap,
                                      Color color, Matrix4x4 transform,
                                      Vector3[] points, Color wrapColor) {
    if (points.Length == 0)
      return;
    ...
    if (minRep <= 0 && maxRep >= 0) {
      // Use line drawing with minimum segment length. Faster for large data
      // sets
    ...
    if (preWrap == WrapMode.Loop) {
      ...
      for (int r = minRep; r < 0; r++) {
        for (int i = 0; i < points.Length; i++) {
          if (r / 2 == r / 2f) {
            ...
            point.x =
                -point.x + (r + 1) * (rangeEnd - rangeStart) + rangeStart * 2;
            point = transform.MultiplyPoint(point);
            line.Add(point);
          }
        }
      ...
      if (minRep < 0) {
        Handles.DrawPolyLine(new Vector3[]{
            ...
            transform.MultiplyPoint(
                new Vector3(Mathf.Min(maxTime, points[0].x), points[0].y, 0))});
      }
    }
    ...
    if (postWrap == WrapMode.Loop) {
      ...
      if (maxRep > 0) {
        Handles.DrawPolyLine(new Vector3[]{
            transform.MultiplyPoint(new Vector3(
                Mathf.Max(minTime, points[last].x), points[last].y, 0)),
  ...
  public Bounds GetBounds(float minTime, float maxTime) {
    ...
    return new Bounds(
        new Vector3((maxTime + minTime) * 0.5f, (max + min) * 0.5f, 0),
        new Vector3(maxTime - minTime, max - min, 0));
  }

  public void FlushCache() { Object.DestroyImmediate(m_CurveMesh); }
}
} // namespace
