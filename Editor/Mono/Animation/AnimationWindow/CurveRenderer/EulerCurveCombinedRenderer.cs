...
private float rangeEnd {
	get {
		return (m_CustomRangeStart == 0 && m_CustomRangeEnd == 0 &&
		        eulerX.length > 0
		  ? eulerX.keys[eulerX.length - 1].time
		  : m_CustomRangeEnd);
	}
}
...
public EulerCurveCombinedRenderer(
	AnimationCurve quaternionX, AnimationCurve quaternionY,
	AnimationCurve quaternionZ, AnimationCurve quaternionW,
	AnimationCurve eulerX, AnimationCurve eulerY, AnimationCurve eulerZ) {
	this.quaternionX = (quaternionX == null ? new AnimationCurve()
	                : quaternionX);
	this.quaternionY = (quaternionY == null ? new AnimationCurve()
	                : quaternionY);
	this.quaternionZ = (quaternionZ == null ? new AnimationCurve()
	                : quaternionZ);
	this.quaternionW = (quaternionW == null ? new AnimationCurve()
	                : quaternionW);
...
private Vector3 GetValues(float time, bool keyReference) {
	...
	if (quaternionX.length != 0 && quaternionY.length != 0 &&
	    quaternionZ.length != 0 && quaternionW.length != 0) {
		q = EvaluateQuaternionCurvesDirectly(time);
		if (keyReference)
			refEuler = EvaluateEulerCurvesDirectly(time);
		refEuler =
			QuaternionCurveTangentCalculation.GetEulerFromQuaternion(q, refEuler);
...
private Quaternion EvaluateQuaternionCurvesDirectly(float time) {
	return new Quaternion(
		quaternionX.Evaluate(time), quaternionY.Evaluate(time),
		quaternionZ.Evaluate(time), quaternionW.Evaluate(time));
}

private Vector3 EvaluateEulerCurvesDirectly(float time) {
	return new Vector3(eulerX.Evaluate(time), eulerY.Evaluate(time),
...
private void CalculateCurves(float minTime, float maxTime) {
	...
	float[, ] ranges = NormalCurveRenderer.CalculateRanges(
		minTime, maxTime, rangeStart, rangeEnd, preWrapMode, postWrapMode);
...
private void AddPoints(float minTime, float maxTime) {
	...
	for (int i = 0; i < refCurve.length - 1; i++) {
		...
		points [newTime]
		        = GetValues(newTime, true);

		// Iterate forwards through curve segments
		for (float j = 1; j <= kSegmentResolution / 2; j++) {
			newTime = Mathf.Lerp(refCurve[i].time, refCurve[i + 1].time,
			                     (j - 0.001f) / kSegmentResolution);
			points [newTime]
			        = GetValues(newTime, false);
		}
...
public float EvaluateCurveDeltaSlow(float time, int component) {
	if (quaternionX == null)
		return 0;
	return (EvaluateCurveSlow(time + epsilon, component) -
	        EvaluateCurveSlow(time - epsilon, component)) /
	       (epsilon * 2);
}
...
public float EvaluateCurveSlow(float time, int component) {
	...
	if (time < cachedRangeStart || time > cachedRangeEnd) {
		// if an evaluate call is outside of cached range we might as well
		// calculate whole range
	...
	Debug.LogError(
		"List of euler curve points is empty, probably caused by lack of euler curve key synching");
	return 0;
}

public void DrawCurve(float minTime, float maxTime, Color color,
                      Matrix4x4 transform, int component, Color wrapColor) {
	...
	NormalCurveRenderer.DrawCurveWrapped(
		minTime, maxTime, rangeStart, rangeEnd, preWrapMode, postWrapMode,
		color, transform, polyLine.ToArray(), wrapColor);
}
