...
readonly GUIContent k_AnimatePropertyLabel =
	EditorGUIUtility.TrTextContent("Add Property");
...
public static readonly float k_DopeSheetRowHeight =
	EditorGUI.kSingleLineHeight;
public static readonly float k_DopeSheetRowHeightTall =
	k_DopeSheetRowHeight * 2f;
...
private readonly static Color k_KeyColorInDopesheetMode =
	new Color(0.7f, 0.7f, 0.7f, 1);
private readonly static Color k_KeyColorForNonCurves =
...
public AnimationWindowHierarchyGUI(TreeViewController treeView,
...
protected void DoNodeGUI(Rect rect, AnimationWindowHierarchyNode node,
                         bool selected, bool focused, int row) {
	InitStyles();
	...
	if (node is AnimationWindowHierarchyAddButtonNode) {
		...
		bool enabled = false;
		if (node.curves != null) {
			enabled = !Array.Exists(node.curves,
			                        curve => curve.animationIsEditable == false);
		}
...
public override void BeginRowGUI() {
	...
	// This is required to avoid changing control ids mapping as we scroll in
	// the tree view and change items visibility.  Hopefully, we should be able
	// to remove it entirely if we isolate the animation window layouts in
	// separate IMGUIContainers...
	...
	for (int i = 0; i < rowCount; ++i) {
		var propertyNode =
			m_TreeView.data.GetItem(i) as AnimationWindowHierarchyPropertyNode;
		if (propertyNode != null && !propertyNode.isPptrNode)
			m_HierarchyItemValueControlIDs[i] =
				GUIUtility.GetControlID(FocusType.Keyboard);
		else
			m_HierarchyItemValueControlIDs[i] = 0; // not needed.

		m_HierarchyItemFoldControlIDs[i] =
			GUIUtility.GetControlID(FocusType.Passive);
		m_HierarchyItemButtonControlIDs[i] =
			GUIUtility.GetControlID(FocusType.Passive);
	}
}

private void DoAddCurveButton(Rect rect, AnimationWindowHierarchyNode node,
                              int row) {
	...
	Rect rectWithMargin =
		new Rect(rect.xMin + xMargin, rect.yMin + yMargin,
		         rect.width - xMargin * 2f, rect.height - yMargin * 2f);
	...
	if (DoTreeViewButton(m_HierarchyItemButtonControlIDs[row], rectWithMargin,
	                     k_AnimatePropertyLabel, GUI.skin.button)) {
...
// Draw foldout (after text content above to ensure drop down icon is rendered
// above selection highlight)
private void DoFoldout(AnimationWindowHierarchyNode node, Rect rect,
                       float indent, int row) {
	if (m_TreeView.data.IsExpandable(node)) {
		Rect toggleRect = rect;
		toggleRect.x = indent;
		toggleRect.width = foldoutStyleWidth;
		EditorGUI.BeginChangeCheck();
		bool newExpandedValue = GUI.Toggle(
			toggleRect, m_HierarchyItemFoldControlIDs[row],
			m_TreeView.data.IsExpanded(node), GUIContent.none, foldoutStyle);
		...
		AnimationWindowHierarchyPropertyNode hierarchyPropertyNode =
			node as AnimationWindowHierarchyPropertyNode;
		AnimationWindowHierarchyState hierarchyState =
			m_TreeView.state as AnimationWindowHierarchyState;
		...
		if (hierarchyPropertyNode != null && hierarchyPropertyNode.isPptrNode) {
			...
			tallMode = GUI.Toggle(toggleRect, m_HierarchyItemFoldControlIDs[row],
			                      tallMode, GUIContent.none, foldoutStyle);
			if (EditorGUI.EndChangeCheck())
...
private void DoIconAndName(Rect rect, AnimationWindowHierarchyNode node,
                           bool selected, bool focused, float indent) {
	EditorGUIUtility.SetIconSize(new Vector2(
					     13,
					     13)); // If not set we see icons scaling down if text is being cropped
	...
	if (Event.current.type == EventType.Repaint) {
		...
		if (isPhantom) {
			...
			tooltipText =
				"Transform position, rotation and scale can't be partially animated. This value will be animated to the default value";
		}
		if (isLeftOverCurve) {
			warningText = " (Missing!)";
			tooltipText =
				"The GameObject or Component is missing (" + node.path + ")";
		}
		if (isAmbiguous) {
			warningText = " (Duplicate GameObject name!)";
			tooltipText =
				"Target for curve is ambiguous since there are multiple GameObjects with same name (" +
				node.path + ")";
		}
		...
		if (node.depth == 0) {
			...
			if (node.curves.Length > 0) {
				...
				string gameObjectName = GetGameObjectName(
					selectionBinding != null ? selectionBinding.rootGameObject
	      : null, node.path);
				nodePrefix = string.IsNullOrEmpty(gameObjectName)
			   ? ""
			   : gameObjectName + " : ";
			}

			Styles.content =
				new GUIContent(nodePrefix + node.displayName + warningText,
				               GetIconForItem(node), tooltipText);

			textColor =
				EditorGUIUtility.isProSkin ? Color.gray * 1.35f : Color.black;
		} else {
			Styles.content = new GUIContent(node.displayName + warningText,
			                                GetIconForItem(node), tooltipText);

			textColor = EditorGUIUtility.isProSkin ? Color.gray
	    : m_LightSkinPropertyTextColor;

			var phantomColor = selected ? m_PhantomCurveColor *
			                   k_SelectedPhantomCurveColorMultiplier
	    : m_PhantomCurveColor;
			textColor = isPhantom ? phantomColor : textColor;
		}
		textColor =
			isLeftOverCurve || isAmbiguous ? k_LeftoverCurveColor : textColor;
	...
	if (IsRenaming(node.id) && Event.current.type != EventType.Layout)
		GetRenameOverlay().editFieldRect =
			new Rect(rect.x + k_IndentWidth, rect.y,
			         rect.width - k_IndentWidth - 1, rect.height);
}
...
private void DoValueField(Rect rect, AnimationWindowHierarchyNode node,
                          int row) {
	...
	if (node is AnimationWindowHierarchyPropertyNode) {
		...
		if (objectValue is float) {
			...
			Rect valueFieldDragRect = new Rect(
				rect.xMax - k_ValueFieldOffsetFromRightSide - k_ValueFieldDragWidth,
				rect.y, k_ValueFieldDragWidth, rect.height);
			Rect valueFieldRect =
				new Rect(rect.xMax - k_ValueFieldOffsetFromRightSide, rect.y,
				         k_ValueFieldWidth, rect.height);

			if (Event.current.type == EventType.MouseMove &&
			    valueFieldRect.Contains(Event.current.mousePosition))
			...
			if (curve.valueType == typeof(bool)) {
				value =
					GUI.Toggle(valueFieldRect, m_HierarchyItemValueControlIDs[row],
					           value != 0, GUIContent.none, EditorStyles.toggle)
		  ? 1
		  : 0;
			} else {
				...
				//  TreeView forces keyboard focus on itself at mouse down and we lose
				//  focus here.
				if (EditorGUI.s_RecycledEditor.controlID == id &&
				...
				value = EditorGUI.DoFloatField(
					EditorGUI.s_RecycledEditor, valueFieldRect, valueFieldDragRect,
					id, value, "g5", m_AnimationSelectionTextField, true);
				if (enterInTextField) {
					GUI.changed = true;
					Event.current.Use();
				}
			...
			if (EditorGUI.EndChangeCheck()) {
				...
				AnimationKeyTime newAnimationKeyTime =
					AnimationKeyTime.Time(state.currentTime, curve.clip.frameRate);
				...
				if (existingKeyframe == null)
					AnimationWindowUtility.AddKeyframeToCurve(
						curve, value, curve.valueType, newAnimationKeyTime);
				else
...
private bool DoTreeViewButton(int id, Rect position, GUIContent content,
                              GUIStyle style) {
...
private void DoCurveDropdown(Rect rect, AnimationWindowHierarchyNode node,
	...
	rect = new Rect(rect.xMax - k_RowRightOffset - 12, rect.yMin + 2, 22, 12);

	// case 767863.
	// This control id is unique to the hierarchy node it refers to.
	// The tree view only renders the elements that are visible, and will cause
	// the control id counter to shift when scrolling through the view.
	if (DoTreeViewButton(m_HierarchyItemButtonControlIDs[row], rect,
	                     GUIContent.none, m_AnimationCurveDropdown)) {
		state.SelectHierarchyItem(node.id, false, false);
		GenericMenu menu = GenerateMenu(
			new AnimationWindowHierarchyNode[] {node}.ToList(), enabled);
		menu.DropDown(rect);
		Event.current.Use();
	}
}

private void DoCurveColorIndicator(Rect rect,
	...
	if (!state.showCurveEditor)
		...
		GUI.color =
			CurveUtility.GetPropertyColor(node.curves[0].binding.propertyName);
	else
	...
	Texture icon =
		hasKey ? CurveUtility.GetIconKey() : CurveUtility.GetIconCurve();
	rect = new Rect(rect.xMax - k_RowRightOffset - (icon.width / 2) - 5,
	                rect.yMin + k_ColorIndicatorTopMargin, icon.width,
	                icon.height);
...
private GenericMenu
GenerateMenu(List<AnimationWindowHierarchyNode> interactedNodes,
             bool enabled) {
	List<AnimationWindowCurve> curves =
		GetCurvesAffectedByNodes(interactedNodes, false);
	// Linked curves are like regular affected curves but always include
	// transform siblings
	List<AnimationWindowCurve> linkedCurves =
		GetCurvesAffectedByNodes(interactedNodes, true);

	bool forceGroupRemove =
		curves.Count == 1
	    ? AnimationWindowUtility.ForceGrouping(curves[0].binding)
	    : false;

	GenericMenu menu = new GenericMenu();

	// Remove curves
	GUIContent removePropertyContent =
		new GUIContent(curves.Count > 1 || forceGroupRemove ? "Remove Properties"
		       : "Remove Property");
	if (!enabled)
	...
	EditorCurveBinding[] curveBindings =
		new EditorCurveBinding[linkedCurves.Count];
	for (int i = 0; i < linkedCurves.Count; i++)
		curveBindings[i] = linkedCurves[i].binding;
	RotationCurveInterpolation.Mode rotationInterpolation =
		GetRotationInterpolationMode(curveBindings);
	...
	if (showInterpolation) {
		string legacyWarning = state.activeAnimationClip.legacy ?
		                       " (Not fully supported in Legacy)" : "";
		GenericMenu.MenuFunction2 nullMenuFunction2 = null;
		menu.AddItem(
			EditorGUIUtility.TrTextContent("Interpolation/Euler Angles" +
			                               legacyWarning),
			rotationInterpolation == RotationCurveInterpolation.Mode.RawEuler,
			enabled ? ChangeRotationInterpolation
	  : nullMenuFunction2, RotationCurveInterpolation.Mode.RawEuler);
		menu.AddItem(EditorGUIUtility.TrTextContent(
				     "Interpolation/Euler Angles (Quaternion)"),
		             rotationInterpolation ==
		             RotationCurveInterpolation.Mode.Baked,
		             enabled ? ChangeRotationInterpolation
		   : nullMenuFunction2, RotationCurveInterpolation.Mode.Baked);
		menu.AddItem(
			EditorGUIUtility.TrTextContent("Interpolation/Quaternion"),
			rotationInterpolation == RotationCurveInterpolation.Mode.NonBaked,
			enabled ? ChangeRotationInterpolation
	...
	if (state.previewing) {
		...
		if (noneHaveKeys || !enabled)
			...
			menu.AddItem(new GUIContent(str), false, DeleteKeysAtCurrentTime,
			             curves);
	}
...
private void AddKeysAtCurrentTime(object obj) {
	AddKeysAtCurrentTime((List<AnimationWindowCurve>)obj);
}
...
private void DeleteKeysAtCurrentTime(object obj) {
	DeleteKeysAtCurrentTime((List<AnimationWindowCurve>)obj);
}
...
private void ChangeRotationInterpolation(System.Object interpolationMode) {
	RotationCurveInterpolation.Mode mode =
		(RotationCurveInterpolation.Mode)interpolationMode;

	AnimationWindowCurve[] activeCurves = state.activeCurves.ToArray();
	EditorCurveBinding[] curveBindings =
		new EditorCurveBinding[activeCurves.Length];

	for (int i = 0; i < activeCurves.Length; i++) {
		curveBindings[i] = activeCurves[i].binding;
	}

	RotationCurveInterpolation.SetInterpolation(state.activeAnimationClip,
	                                            curveBindings, mode);
...
private void RemoveCurvesFromNodes(List<AnimationWindowHierarchyNode> nodes) {
	...
	foreach (var node in nodes) {
		...
		if (hierarchyNode.parent is AnimationWindowHierarchyPropertyGroupNode &&
		    hierarchyNode.binding != null &&
		    AnimationWindowUtility.ForceGrouping((EditorCurveBinding)
		                                         hierarchyNode.binding))
		...
		if (hierarchyNode is AnimationWindowHierarchyPropertyGroupNode ||
		    hierarchyNode is AnimationWindowHierarchyPropertyNode)
			curves = AnimationWindowUtility.FilterCurves(
				hierarchyNode.curves.ToArray(), hierarchyNode.path,
				hierarchyNode.animatableObjectType, hierarchyNode.propertyName);
		else
			curves = AnimationWindowUtility.FilterCurves(
				hierarchyNode.curves.ToArray(), hierarchyNode.path,
				hierarchyNode.animatableObjectType);
...
private List<AnimationWindowCurve>
GetCurvesAffectedByNodes(List<AnimationWindowHierarchyNode> nodes,
                         bool includeLinkedCurves) {
	...
	foreach (var node in nodes) {
		...
		if (hierarchyNode.curves.Length > 0) {
			...
			if (hierarchyNode is AnimationWindowHierarchyPropertyGroupNode ||
			    hierarchyNode is AnimationWindowHierarchyPropertyNode)
				curves.AddRange(AnimationWindowUtility.FilterCurves(
							hierarchyNode.curves, hierarchyNode.path,
							hierarchyNode.animatableObjectType, hierarchyNode.propertyName));
			else
				curves.AddRange(AnimationWindowUtility.FilterCurves(
							hierarchyNode.curves, hierarchyNode.path,
...
// Propertynames are used in treeview node IDs, so we need to anticipate the
// new IDs by injecting them into treeview state This way treeview state
// (selection and expanding) will be preserved once the curve data is
// eventually reloaded
private void MaintainTreeviewStateAfterRotationInterpolation(
	RotationCurveInterpolation.Mode newMode) {
	...
	for (int i = 0; i < selectedInstaceIDs.Count; i++) {
		AnimationWindowHierarchyNode node = state.hierarchyData.FindItem(
			selectedInstaceIDs[i]) as AnimationWindowHierarchyNode;

		if (node != null &&
		    !node.propertyName.Equals(
			    RotationCurveInterpolation.GetPrefixForInterpolation(newMode))) {
			string oldPrefix = node.propertyName.Split('.')[0];
			string newPropertyName = node.propertyName.Replace(
				oldPrefix,
				RotationCurveInterpolation.GetPrefixForInterpolation(newMode));
			...
			newIds.Add(
				(node.path + node.animatableObjectType.Name + newPropertyName)
				.GetHashCode());
		}
	}
...
private RotationCurveInterpolation.Mode
GetRotationInterpolationMode(EditorCurveBinding[] curves) {
	if (curves == null || curves.Length == 0)
		return RotationCurveInterpolation.Mode.Undefined;

	RotationCurveInterpolation.Mode mode =
		RotationCurveInterpolation.GetModeFromCurveData(curves[0]);
	for (int i = 1; i < curves.Length; i++) {
		RotationCurveInterpolation.Mode nextMode =
			RotationCurveInterpolation.GetModeFromCurveData(curves[i]);
		if (mode != nextMode)
			return RotationCurveInterpolation.Mode.Undefined;
	}
...
public override void GetFirstAndLastRowVisible(out int firstRowVisible,
                                               out int lastRowVisible) {
	firstRowVisible = 0;
...
public float GetNodeHeight(AnimationWindowHierarchyNode node) {
	...
	AnimationWindowHierarchyState hierarchyState =
		m_TreeView.state as AnimationWindowHierarchyState;
	return hierarchyState.GetTallMode(node) ? k_DopeSheetRowHeightTall
	                                    : k_DopeSheetRowHeight;
}
...
public override Vector2 GetTotalSize() {
	...
	for (int i = 0; i < rows.Count; i++) {
		AnimationWindowHierarchyNode node =
			rows[i] as AnimationWindowHierarchyNode;
...
public override Rect GetRowRect(int row, float rowWidth) {
	var rows = m_TreeView.data.GetRows();
	AnimationWindowHierarchyNode hierarchyNode =
		rows[row] as AnimationWindowHierarchyNode;
...
public override void OnRowGUI(Rect rowRect, TreeViewItem node, int row,
		...
		if (newName != oldName) {
			...
			foreach (AnimationWindowCurve curve in m_RenamedNode.curves) {
				EditorCurveBinding newBinding =
					AnimationWindowUtility.GetRenamedBinding(curve.binding, newName);

				if (AnimationWindowUtility.CurveExists(newBinding,
				                                       state.allCurves.ToArray())) {
