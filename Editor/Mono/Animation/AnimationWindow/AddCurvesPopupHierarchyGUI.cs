...
internal class AddCurvesPopupHierarchyGUI : TreeViewGUI {
  ...
  private GUIContent addPropertiesContent =
      EditorGUIUtility.TrTextContent("Add Properties");
  private const float plusButtonWidth = 17;

  static Texture2D warningIcon = (Texture2D) EditorGUIUtility.LoadRequired(
      "Icons/ShortcutManager/alertDialog.png");
  public static GUIStyle warningIconStyle;

  public AddCurvesPopupHierarchyGUI(TreeViewController treeView,
                                    EditorWindow owner)
      : base(treeView, true) {
    this.owner = owner;
  ...
  public override void OnRowGUI(Rect rowRect, TreeViewItem node, int row,
    ...
    AddCurvesPopupGameObjectNode addCurvesPopupNode =
        node as AddCurvesPopupGameObjectNode;
    if (addCurvesPopupNode != null) {
      propertyPathMismatchWithHumanAvatar =
          addCurvesPopupNode.propertyPathMismatchWithHumanAvatar;
    }
    ...
    if (propertyPathMismatchWithHumanAvatar) {
      Rect iconRect = new Rect(rowRect.width - plusButtonWidth, rowRect.yMin,
                               plusButtonWidth, buttonStyle.fixedHeight);
      GUI.Label(
          iconRect,
          new GUIContent(
              warningIcon,
              "The Avatar definition does not match the property path. Please author using a hierarchy the Avatar was built with."),
          warningIconStyle);
    }
  }
  ...
  private void DoAddCurveButton(Rect rowRect, TreeViewItem node) {
    ...
    AddCurvesPopupPropertyNode hierarchyNode =
        node as AddCurvesPopupPropertyNode;
    if (hierarchyNode == null || hierarchyNode.curveBindings == null ||
        hierarchyNode.curveBindings.Length == 0)
      return;

    Rect buttonRect = new Rect(rowRect.width - plusButtonWidth, rowRect.yMin,
                               plusButtonWidth, buttonStyle.fixedHeight);
  ...
  override protected bool IsRenaming(int id) { return false; }
