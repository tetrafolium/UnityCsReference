...
class AnimationWindowClipPopup {
  ...
  internal sealed class ClipPopupCallbackInfo {
    ...
    public static AnimationClip GetSelectedClipForControl(int controlID,
                                                          AnimationClip clip) {
    ...
    public static void SendEvent() {
      ...
      instance.m_SourceView.SendEvent(
          EditorGUIUtility.CommandEvent(kPopupMenuChangedMessage));
    }
  }

  private void DisplayClipMenu(Rect position, int controlID,
                               AnimationClip clip) {
    ...
    if (Application.platform == RuntimePlatform.OSXEditor) {
      position.y =
          position.y - selected * EditorGUI.kSingleLineHeight - kMenuOffsetMac;
    }

    ClipPopupCallbackInfo.instance = new ClipPopupCallbackInfo(controlID);

    EditorUtility.DisplayCustomMenu(
        position, menuContent, null, selected, (userData, options, index) => {
          if (index < clips.Length) {
            ClipPopupCallbackInfo.SetSelectedClip(clips[index]);
          } else {
            AnimationClip newClip = AnimationWindowUtility.CreateNewClip(
                state.selection.rootGameObject.name);
            if (newClip) {
              AnimationWindowUtility.AddClipToAnimationPlayerComponent(
                  state.activeAnimationPlayer, newClip);
  ...
  // (case 1029160) Modified version of EditorGUI.DoPopup to fit large data list
  // query.
  private AnimationClip DoClipPopup(AnimationClip clip, GUIStyle style) {
    Rect position = EditorGUILayout.GetControlRect(
        false, EditorGUI.kSingleLineHeight, style);
    int controlID =
        GUIUtility.GetControlID(s_ClipPopupHash, FocusType.Keyboard, position);
    ...
    case EventType.Repaint:
      ...
      GUIContent buttonContent =
          EditorGUIUtility.TempContent(CurveUtility.GetClipName(clip));
  ...
  public void OnGUI() {
    if (state.selection.canChangeAnimationClip) {
      ...
      var newClip = DoClipPopup(state.activeAnimationClip,
                                AnimationWindowStyles.animClipToolbarPopup);
      ...
      Rect r = EditorGUILayout.GetControlRect(
          ...
          AnimationWindowStyles.toolbarLabel);
      EditorGUI.LabelField(r,
                           CurveUtility.GetClipName(state.activeAnimationClip),
                           AnimationWindowStyles.toolbarLabel);
    }
  }
  ...
  private AnimationClip[] GetOrderedClipList() {
    ...
    Array.Sort(clips, (AnimationClip clip1, AnimationClip clip2) =>
                          CurveUtility.GetClipName(clip1).CompareTo(
                              CurveUtility.GetClipName(clip2)));
