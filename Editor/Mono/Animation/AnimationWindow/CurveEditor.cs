...
internal class CurveWrapper {
  ...
  internal enum SelectionMode { None = 0, Selected = 1, SemiSelected = 2 }
  ...
  public GameObject rootGameObjet {
    get {
      return m_SelectionBinding != null ? m_SelectionBinding.rootGameObject
          : null;
    }
  }
  public AnimationClip animationClip {
    get {
      return m_SelectionBinding != null ? m_SelectionBinding.animationClip
          : null;
    }
  }
  public bool clipIsEditable {
    get {
      return m_SelectionBinding != null ? m_SelectionBinding.clipIsEditable
          : true;
    }
  }
  public bool animationIsEditable {
    get {
      return m_SelectionBinding != null ? m_SelectionBinding.animationIsEditable
          : true;
    }
  }
  public int selectionID {
    get { return m_SelectionBinding != null ? m_SelectionBinding.id : 0; }
  }

  public ISelectionBinding selectionBindingInterface {
    get { return m_SelectionBinding; }
    set { m_SelectionBinding = value; }
  }

  public Bounds bounds {
    get { return renderer.GetBounds(); }
  }
  ...
  public int groupId;
  public int regionId; // Regions are defined by two curves added after each
                       // other with the same regionId.
  public Color color;
  public Color wrapColorMultiplier = Color.white;
  public bool readOnly;
  public bool hidden;
  public GetAxisScalarsCallback
      getAxisUiScalarsCallback; // Delegate used to fetch values that are
                                // multiplied on x and y axis ui values
  public SetAxisScalarsCallback
      setAxisUiScalarsCallback; // Delegate used to set values back that has
                                // been changed by this curve editor

  public PreProcessKeyMovement
      preProcessKeyMovementDelegate; // Delegate used limit key manipulation to
                                     // fit curve constraints
  ...
  internal Bounds ComputeBoundsBetweenTime(float start, float end) =>
      renderer.GetBounds(start, end);

  // An additional vertical min / max range clamp when editing multiple curves
...
class CurveControlPointRenderer {
  ...
  public CurveControlPointRenderer() {
    m_UnselectedPointRenderer =
        new ControlPointRenderer(CurveEditor.Styles.pointIcon);
    m_SelectedPointRenderer =
        new ControlPointRenderer(CurveEditor.Styles.pointIconSelected);
    m_SelectedPointOverlayRenderer =
        new ControlPointRenderer(CurveEditor.Styles.pointIconSelectedOverlay);
    m_SemiSelectedPointOverlayRenderer = new ControlPointRenderer(
        CurveEditor.Styles.pointIconSemiSelectedOverlay);
    m_WeightedPointRenderer =
        new ControlPointRenderer(CurveEditor.Styles.pointIconWeighted);
  }
...
[System.Serializable]
internal class CurveEditor : TimeArea,
                             CurveUpdater {
  ...
  public CurveWrapper[] animationCurves {
    ...
    set {
      ...
      for (int i = 0; i < m_AnimationCurves.Length; ++i) {
        ...
        m_EnableCurveGroups =
            m_EnableCurveGroups || (m_AnimationCurves[i].groupId != -1);
      }
      SyncDrawOrder();
      SyncSelection();
      ValidateCurveList();
  ...
  private List<int> m_DrawOrder =
      new List<int>(); // contains curveIds (last element topmost)
  void SyncDrawOrder() {
    // Init
    if (m_DrawOrder.Count == 0) {
    ...
    // Fallback if invalid setup with multiple curves with same curveID (see
    // case 482048)
    if (m_DrawOrder.Count != m_AnimationCurves.Length) {
      ...
      //      Cylinder
      //          InnerCyl
      // Since we cannot generate unique curve ids for identical paths like
      // Cylinder and InnerCyl.
      m_DrawOrder = m_AnimationCurves.Select(cw => cw.id).ToList();
    }
  }
  ...
  internal Bounds m_DefaultBounds =
      new Bounds(new Vector3(0.5f, 0.5f, 0), new Vector3(1, 1, 0));
  ...
  /// 1/time to snap all keyframes to while dragging. Set to 0 for no snap
  /// (default)
  ...
  internal List<CurveSelection> selectedCurves {
    get { return selection.selectedCurves; }
    set {
  ...
  public bool hasSelection {
    get { return selectedCurves.Count != 0; }
  }

  bool m_InRangeSelection = false;

  internal void BeginRangeSelection() { m_InRangeSelection = true; }
  ...
  internal
      CurveWrapper GetCurveWrapperFromSelection(CurveSelection curveSelection) {
  ...
  // Time range selection, is it active and what was the mousedown time (start)
  // and the current end time.
  ...
  class SavedCurve {
    ...
    public class SavedKeyFrameComparer : IComparer<float> {
      public static SavedKeyFrameComparer Instance =
          new SavedKeyFrameComparer();

      public int Compare(float time1, float time2) {
        float cmp = time1 - time2;
        return cmp < -kCurveTimeEpsilon ? - 1
            : (cmp >= kCurveTimeEpsilon ? 1 : 0);
      }
    }
    ...
    public delegate SavedKeyFrame KeyFrameOperation(SavedKeyFrame keyframe,
                                                    SavedCurve curve);
  }
  ...
  // The mouse will select a key if it's within this distance from the key
  // point.
  ...
  public CurveEditor(Rect rect, CurveWrapper[] curves, bool minimalGUI)
    ...
    float[] modulos = new float[]{
        0.0000001f, 0.0000005f, 0.000001f, 0.000005f, 0.00001f, 0.00005f,
        0.0001f,    0.0005f,    0.001f,    0.005f,    0.01f,    0.05f,
        0.1f,       0.5f,       1,         5,         10,       50,
  ...
  private void ValidateCurveList() {
    // Validate that regions are valid (they should consist of two curves after
    // each other with same regionId)
    for (int i = 0; i < m_AnimationCurves.Length; ++i) {
      ...
      if (regId1 >= 0) {
        if (i == m_AnimationCurves.Length - 1) {
          Debug.LogError(
              "Region has only one curve last! Regions should be added as two curves after each other with same regionId");
          return;
        }
    ...
    if (m_DrawOrder.Count != m_AnimationCurves.Length) {
      Debug.LogError("DrawOrder and AnimationCurves mismatch: DrawOrder " +
                     m_DrawOrder.Count +
                     ", AnimationCurves: " + m_AnimationCurves.Length);
      return;
    }
    ...
    /*
    string info = "Count: " + m_AnimationCurves.Length + " (Click me for more
    info)\n"; foreach (CurveWrapper cw in m_AnimationCurves) info += ("Curve: id
    " + cw.id + ", regionId " + cw.regionId + ", hidden " + cw.hidden + "\n");
    Debug.Log(info);
    */
  }
  ...
  private void SyncSelection() {
    ...
    List<CurveSelection> newSelection =
        new List<CurveSelection>(selectedCurves.Count);
  ...
  public void InvalidateBounds() { m_BoundsAreDirty = true; }
  ...
  private void RecalculateBounds() {
    ...
    if (state != null) {
      m_CurveBounds.SetMinMax(new Vector3(state.timeRange.x, 0f, 0f),
                              new Vector3(state.timeRange.y, 1f, 0f));
    ...
    //  Calculate bounds based on curve bounds if bound is not set by
    //  hRangeMin/hRangeMax vRangeMin/vRangeMax.
    float minx =
        hRangeMin != Mathf.NegativeInfinity ? hRangeMin : m_CurveBounds.min.x;
    float miny =
        vRangeMin != Mathf.NegativeInfinity ? vRangeMin : m_CurveBounds.min.y;
    ...
    m_DrawingBounds.SetMinMax(new Vector3(minx, miny, m_CurveBounds.min.z),
                              new Vector3(maxx, maxy, m_CurveBounds.max.z));

    // Enforce minimum size of bounds
    m_DrawingBounds.size =
        new Vector3(Mathf.Max(m_DrawingBounds.size.x, kMinRange),
                    Mathf.Max(m_DrawingBounds.size.y, kMinRange), 0);
    m_CurveBounds.size =
        new Vector3(Mathf.Max(m_CurveBounds.size.x, kMinRange),
                    Mathf.Max(m_CurveBounds.size.y, kMinRange), 0);
  ...
  private void RecalculateSelectionBounds() {
    ...
    if (hasSelection) {
      ...
      m_SelectionBounds =
          new Bounds(new Vector2(keyframe.time, keyframe.value), Vector2.zero);

      for (int i = 1; i < selected.Count; ++i) {
        keyframe = GetKeyframeFromSelection(selected[i]);
        m_SelectionBounds.Encapsulate(
            new Vector2(keyframe.time, keyframe.value));
      }
    } else {
  ...
  private void RecalculateSelectionWithCurvesBounds() {
    ...
    if (hasSelection) {
      CurveSelection[] selected =
          selectedCurves.OrderBy(p => p.curveID).ThenBy(p => p.key).ToArray();
      ...
      for (int i = 0; i < selected.Length - 1; i++) {
        ...
        Bounds inBetweenBounds = currentCurveWrapper.ComputeBoundsBetweenTime(
            keyframeStart.time, keyframeEnd.time);
  ...
  public Bounds GetClipBounds() { return curveBounds; }
  ...
  public Bounds GetSelectionBounds() {
    ...
    if (selectedCurves.Count == 1) {
      ...
      frameBounds =
          new Bounds(new Vector2(cw.curve[cs.key].time, cw.curve[cs.key].value),
                     Vector2.zero);

      // Include neighboring keys in bounds
      if (cs.key - 1 >= 0)
        frameBounds.Encapsulate(
            new Vector2(cw.curve[cs.key - 1].time, cw.curve[cs.key - 1].value));
      ...
      if (cs.key - 1 >= 0)
        frameBounds.Encapsulate(cw.ComputeBoundsBetweenTime(
            cw.curve[cs.key - 1].time, cw.curve[cs.key].time));
      if (cs.key + 1 < cw.curve.length)
        frameBounds.Encapsulate(cw.ComputeBoundsBetweenTime(
            cw.curve[cs.key].time, cw.curve[cs.key + 1].time));
    } else {
      frameBounds = selectionWithCurvesBounds;
    }

    // Enforce minimum size of bounds
    frameBounds.size = new Vector3(Mathf.Max(frameBounds.size.x, 0.1F),
                                   Mathf.Max(frameBounds.size.y, 0.1F), 0);
  ...
  public void UpdateCurves(List<ChangedCurve> changedCurves, string undoText) {
    UpdateCurves(new List<int>(changedCurves.Select(curve => curve.curveId)),
                 undoText);
  }

  public void StartLiveEdit() { MakeCurveBackups(); }

  public void EndLiveEdit() { m_CurveBackups = null; }

  public bool InLiveEdit() { return m_CurveBackups != null; }
  ...
  public void CurveGUI() {
    ...
    // Because this uses a keyboard input field, it must be allowed to handle
    // events first
    ...
    case EventType.ContextClick:
      ...
      if (mouseKey != null) {
        ...
        foreach (CurveSelection sel in selectedCurves) {
          keyList.Add(new KeyIdentifier(GetCurveFromSelection(sel), sel.curveID,
                                        sel.key));
        ...
        if (!inSelected) {
          keyList.Clear();
          keyList.Add(new KeyIdentifier(GetCurveFromSelection(mouseKey),
                                        mouseKey.curveID, mouseKey.key));
          ClearSelection();
          AddSelection(mouseKey);
        }

        bool isEditable = !selectedCurves.Exists(
            sel => !GetCurveWrapperFromSelection(sel).animationIsEditable);

        m_MenuManager = new CurveMenuManager(this);
        GenericMenu menu = new GenericMenu();

        string deleteKeyLabel =
            keyList.Count > 1 ? "Delete Keys" : "Delete Key";
        string editKeyLabel =
            keyList.Count > 1 ? "Edit Keys..." : "Edit Key...";

        if (isEditable) {
          menu.AddItem(new GUIContent(deleteKeyLabel), false, DeleteKeys,
                       keyList);
          menu.AddItem(new GUIContent(editKeyLabel), false,
  ...
  void RecalcCurveSelection() {
    ...
    foreach (CurveSelection cs in selectedCurves) {
      CurveWrapper cw = GetCurveWrapperFromSelection(cs);
      if (cw != null)
        cw.selected = cs.semiSelected ? CurveWrapper.SelectionMode
                          .SemiSelected : CurveWrapper.SelectionMode.Selected;
    }
  }

  void RecalcSecondarySelection() {
    // No need to recalculate secondary selection if there are no curves with a
    // valid groupId.
    ...
    // Go through selection, find curveselections that need syncing, add those
    // for the sync points.
    foreach (CurveSelection cs in selectedCurves) {
      CurveWrapper cw = GetCurveWrapperFromSelection(cs);
      if (cw == null)
    ...
    // the selection can contain duplicate keys. We go through the selection and
    // remove any duplicates we find. Since the selection is already sorted, the
    ...
    for (int i = 0; i < newSelection.Count - 1;) {
      ...
      if (cs1.curveID == cs2.curveID && cs1.key == cs2.key) {
        // If we have a collision, one can be fully selected, while the other
        // can be semiselected. Make sure we always get the most selected one.
  ...
  void DragTangents() {
    Event evt = Event.current;
    int tangentId =
        GUIUtility.GetControlID(s_TangentControlIDHash, FocusType.Passive);
      ...
      if (evt.button == 0 && !evt.alt) {
        ...
        foreach (CurveSelection cs in selectedCurves) {
          ...
          if (IsLeftTangentEditable(cs)) {
            CurveSelection tangent = new CurveSelection(
                cs.curveID, cs.key, CurveSelection.SelectionType.InTangent);
            float d =
                (DrawingToViewTransformPoint(GetPosition(tangent)) - mousePos)
                    .sqrMagnitude;
            if (d <= nearestDist) {
          ...
          if (IsRightTangentEditable(cs)) {
            CurveSelection tangent = new CurveSelection(
                cs.curveID, cs.key, CurveSelection.SelectionType.OutTangent);
            float d =
                (DrawingToViewTransformPoint(GetPosition(tangent)) - mousePos)
                    .sqrMagnitude;
            if (d <= nearestDist) {
        ...
        if ((curveWrapper != null) && curveWrapper.animationIsEditable) {
          ...
          if (dragged.type == CurveSelection.SelectionType.InTangent) {
            ...
            Vector2 tangentDirection =
                newPosition - new Vector2(key.time, key.value);
            if (tangentDirection.x < -0.0001F) {
              key.inTangent = tangentDirection.y / tangentDirection.x;
              key.inWeight =
                  Mathf.Clamp(Mathf.Abs(tangentDirection.x / dx), 0f, 1f);
            } else {
            ...
            if (!AnimationUtility.GetKeyBroken(key)) {
              ...
              AnimationUtility.SetKeyRightTangentMode(ref key,
                                                      TangentMode.Free);
            }
          ...
          AnimationUtility.UpdateTangentsFromModeSurrounding(curveWrapper.curve,
                                                             dragged.key);
    ...
    case EventType.Repaint:
      if (GUIUtility.hotControl == tangentId) {
        Rect mouseRect = new Rect(evt.mousePosition.x - 10,
                                  evt.mousePosition.y - 10, 20, 20);
  ...
  internal void TranslateSelectedKeys(Vector2 movement) {
    ...
    UpdateCurvesFromPoints(delegate(SavedCurve.SavedKeyFrame keyframe,
                                    SavedCurve curve) {
      if (keyframe.selected != CurveWrapper.SelectionMode.None) {
        ...
        duplicateKeyframe.key.time = Mathf.Clamp(
            duplicateKeyframe.key.time + movement.x, hRangeMin, hRangeMax);

        // if it's fully selected, also move on Y
        if (duplicateKeyframe.selected == CurveWrapper.SelectionMode.Selected)
          duplicateKeyframe.key.value = ClampVerticalValue(
              duplicateKeyframe.key.value + movement.y, curve.curveId);
  ...
  internal void SetSelectedKeyPositions(float newTime, float newValue,
                                        bool updateTime, bool updateValue) {
    ...
    UpdateCurvesFromPoints(
        delegate(SavedCurve.SavedKeyFrame keyframe, SavedCurve curve) {
          if (keyframe.selected != CurveWrapper.SelectionMode.None) {
            ...
            if (updateTime) {
              duplicateKeyframe.key.time =
                  Mathf.Clamp(newTime, hRangeMin, hRangeMax);
            }
            if (updateValue) {
              duplicateKeyframe.key.value =
                  ClampVerticalValue(newValue, curve.curveId);
            }
  ...
  internal void TransformSelectedKeys(Matrix4x4 matrix, bool flipX,
                                      bool flipY) {
    bool inLiveEdit = InLiveEdit();
    if (!inLiveEdit)
      StartLiveEdit();
    ...
    UpdateCurvesFromPoints(
        ...
        delegate(SavedCurve.SavedKeyFrame keyframe, SavedCurve curve) {
          if (keyframe.selected != CurveWrapper.SelectionMode.None) {
            ...
            Vector3 v = new Vector3(duplicateKeyframe.key.time,
                                    duplicateKeyframe.key.value, 0f);
            ...
            if (flipX) {
              duplicateKeyframe.key.inTangent =
                  (keyframe.key.outTangent != Mathf.Infinity)
                      ? -keyframe.key.outTangent
                      : Mathf.Infinity;
              duplicateKeyframe.key.outTangent =
                  (keyframe.key.inTangent != Mathf.Infinity)
                      ? -keyframe.key.inTangent
                      : Mathf.Infinity;
            ...
            // if it's fully selected, also move on Y
            if (duplicateKeyframe.selected ==
                CurveWrapper.SelectionMode.Selected) {
              duplicateKeyframe.key.value =
                  ClampVerticalValue(v.y, curve.curveId);

              if (flipY) {
                duplicateKeyframe.key.inTangent =
                    (duplicateKeyframe.key.inTangent != Mathf.Infinity)
                        ? -duplicateKeyframe.key.inTangent
                        : Mathf.Infinity;
                duplicateKeyframe.key.outTangent =
                    (duplicateKeyframe.key.outTangent != Mathf.Infinity)
                        ? -duplicateKeyframe.key.outTangent
                        : Mathf.Infinity;
              }
            }
  ...
  internal void TransformRippleKeys(Matrix4x4 matrix, float t1, float t2,
    ...
    UpdateCurvesFromPoints(
        ...
        delegate(SavedCurve.SavedKeyFrame keyframe, SavedCurve curve) {
          ...
          if (keyframe.selected != CurveWrapper.SelectionMode.None) {
            ...
            duplicateKeyframe.key.time =
                SnapTime(Mathf.Clamp(newTime, hRangeMin, hRangeMax));
  ...
  void UpdateCurvesFromPoints(SavedCurve.KeyFrameOperation action) {
    ...
    foreach (SavedCurve sc in m_CurveBackups) {
      ...
      var working = new SortedList<float, SavedCurve.SavedKeyFrame>(
          SavedCurve.SavedKeyFrameComparer.Instance);
      ...
      foreach (SavedCurve.SavedKeyFrame keyframe in sc.keys) {
        if (keyframe.selected == CurveWrapper.SelectionMode.None) {
          ...
          // We might have moved keys around, let's add new key or replace
          // existing key.
          working[newKeyframe.key.time] = newKeyframe;
        }
      }

      //  Add all modified key frames to the working collection and remove
      //  duplicates.
      foreach (SavedCurve.SavedKeyFrame keyframe in sc.keys) {
        if (keyframe.selected != CurveWrapper.SelectionMode.None) {
          var newKeyframe = action(keyframe, sc);
      ...
      // Each point has it's selection set, but m_DisplayCurves has a more
      // traditional key array. Go through the working points and sort those for
      // display.
      int idx = 0;
      ...
      // We now have the list of keys to assign - let's get them back into the
      // animation clip
  ...
  new internal static class Styles {
    public static Texture2D pointIcon =
        EditorGUIUtility.LoadIcon("curvekeyframe");
    public static Texture2D pointIconWeighted =
        EditorGUIUtility.LoadIcon("curvekeyframeweighted");
    public static Texture2D pointIconSelected =
        EditorGUIUtility.LoadIcon("curvekeyframeselected");
    public static Texture2D pointIconSelectedOverlay =
        EditorGUIUtility.LoadIcon("curvekeyframeselectedoverlay");
    public static Texture2D pointIconSemiSelectedOverlay =
        EditorGUIUtility.LoadIcon("curvekeyframesemiselectedoverlay");
  ...
  Rect GetCurveRect(CurveWrapper cw) {
    ...
    return Rect.MinMaxRect(bounds.min.x, bounds.min.y, bounds.max.x,
                           bounds.max.y);
  }

  Vector2 s_StartMouseDragPosition, s_EndMouseDragPosition,
      s_StartKeyDragPosition;
  PickMode s_PickMode;
  ...
  int GetCurveAtPosition(Vector2 viewPos, out Vector2 closestPointOnCurve) {
    ...
    for (int i = m_DrawOrder.Count - 1; i >= 0; --i) {
      ...
      // Sample the curves at pixel intervals in the area around the desired
      // time, corresponding to the max cursor distance allowed.
      Vector2 valL;
      ...
      for (int x = -maxPixelDist; x < maxPixelDist; x++) {
        ...
        if (dist < smallestDist) {
          ...
          closestPointOnCurve =
              HandleUtility.ProjectPointLine(viewPos, valL, valR);
        }
  ...
  List<int> CreateKeyFromClick(Vector2 viewPos) {
    ...
    if (singleCurveIndex >= 0) {
      // If there is only one curve, allow creating keys on it by
      // double/right-clicking anywhere if the click is to the left or right of
      // the existing keys, or if there are no existing keys.
      CurveWrapper cw = m_AnimationCurves[singleCurveIndex];
      Vector2 localPos = ViewToDrawingTransformPoint(viewPos);
      float time = localPos.x;
      if (cw.curve.keys.Length == 0 || time < cw.curve.keys[0].time ||
          time > cw.curve.keys[cw.curve.keys.Length - 1].time) {
  ...
  bool CreateKeyFromClick(int curveIndex, Vector2 localPos) {
    localPos.x =
        Mathf.Clamp(localPos.x, settings.hRangeMin, settings.hRangeMax);
  ...
  CurveSelection AddKeyAtTime(CurveWrapper cw, float time) {
    ...
    if (invSnap != 0.0f)
      ...
      halfFrame = 0.0001f;
    if (CurveUtility.HaveKeysInRange(cw.curve, time - halfFrame,
                                     time + halfFrame))
      return null;
  ...
  CurveSelection AddKeyAtPosition(CurveWrapper cw, Vector2 position) {
    ...
    if (invSnap != 0.0f)
      ...
      halfFrame = 0.0001f;
    if (CurveUtility.HaveKeysInRange(cw.curve, position.x - halfFrame,
                                     position.x + halfFrame))
      return null;

    // Add the key
    float slope = 0;
    Keyframe key =
        new Keyframe(position.x, SnapValue(position.y), slope, slope);
  ...
  // Find keyframe nearest to the mouse. We use the draw order to ensure to
  // return the key that is topmost rendered if several keys are overlapping.
  // The user can click on another curve to bring it to front and hereby be able
  // to better select its keys. Returns null if nothing is within
  // Sqrt(kMaxPickDistSqr) pixels.
  CurveSelection FindNearest() {
    ...
    for (int index = m_DrawOrder.Count - 1; index >= 0; --index) {
      ...
      for (int i = 0; i < cw.curve.keys.Length; ++i) {
        ...
        float d = (GetGUIPoint(cw, new Vector2(k.time, k.value)) - mousePos)
                      .sqrMagnitude;
      ...
      // If top curve has key within range make it harder for keys below to get
      // selected
  ...
  public bool IsDraggingKey() { return m_DraggingKey != null; }
  ...
  public bool IsDraggingCurve(CurveWrapper cw) {
    return (m_DraggingCurveOrRegion != null &&
            m_DraggingCurveOrRegion.Length == 1 &&
            m_DraggingCurveOrRegion[0] == cw);
  }

  public bool IsDraggingRegion(CurveWrapper cw1, CurveWrapper cw2) {
    return (m_DraggingCurveOrRegion != null &&
            m_DraggingCurveOrRegion.Length == 2 &&
            (m_DraggingCurveOrRegion[0] == cw1 ||
             m_DraggingCurveOrRegion[0] == cw2));
  }

  bool HandleCurveAndRegionMoveToFrontOnMouseDown(ref Vector2 timeValue,
                                                  ref CurveWrapper[] curves) {
    // Did we click on a curve
    Vector2 closestPointOnCurve;
    int clickedCurveIndex = GetCurveAtPosition(Event.current.mousePosition,
                                               out closestPointOnCurve);
  ...
  void SelectPoints() {
    ...
    case EventType.Layout:
      float distance = HandleUtility.DistanceToRectangleInternal(
          drawRect.position, Quaternion.identity, drawRect.size);
    ...
    case EventType.ContextClick:
      ...
      if (drawRectAtOrigin.Contains(Event.current.mousePosition)) {
        ...
        int curveIndex = GetCurveAtPosition(Event.current.mousePosition,
                                            out closestPositionOnCurve);
        if (curveIndex >= 0) {
          GenericMenu menu = new GenericMenu();
          if (m_AnimationCurves[curveIndex].animationIsEditable)
            menu.AddItem(EditorGUIUtility.TrTextContent("Add Key"), false,
                         CreateKeyFromClick, Event.current.mousePosition);
          else
    ...
    case EventType.MouseDown:
      if (evt.clickCount == 2 && evt.button == 0) {
        ...
        if (selectedPoint != null) {
          ...
          if (curve != null) {
            ...
            for (int keyIndex = 0; keyIndex < curve.keys.Length; ++keyIndex) {
              if (!selectedCurves.Any(x => x.curveID == selectedPoint.curveID &&
                                           x.key == keyIndex)) {
                var keySelection =
                    new CurveSelection(selectedPoint.curveID, keyIndex);
        ...
        if (selectedPoint == null || selectedPoint.semiSelected) {
          ...
          var curveOrRegionClicked = HandleCurveAndRegionMoveToFrontOnMouseDown(
              ref timeValue, ref curves);
          ...
          s_StartKeyDragPosition =
              new Vector2(selectedKeyframe.time, selectedKeyframe.value);
          ...
          if (addToSelection) {
            ...
            if (!addRangeToSelection) {
              ...
              // Try and add all keys on the same curve in the range [keyMin,
              // keyMax]
              BeginRangeSelection();
              for (var keyIndex = keyMin; keyIndex <= keyMax; ++keyIndex) {
                if (!selectedCurves.Any(x => x.curveID ==
                                                 selectedPoint.curveID &&
                                             x.key == keyIndex)) {
                  var rangeSelection =
                      new CurveSelection(selectedPoint.curveID, keyIndex);
    ...
    case EventType.MouseDrag:
      if (GUIUtility.hotControl == id) {
        ...
        if (s_PickMode == PickMode.Click) {
          ...
          Rect r = EditorGUIExt.FromToRect(s_StartMouseDragPosition,
                                           evt.mousePosition);

          List<CurveSelection> newSelection =
              new List<CurveSelection>(s_SelectionBackup);
          for (int i = 0; i < m_AnimationCurves.Length; ++i) {
            CurveWrapper cw = m_AnimationCurves[i];
            if (cw.readOnly || cw.hidden)
              continue;
          ...
          // We need to sort selection since we're mixing existing selection
          // with new selection.
    ...
    if (s_PickMode == PickMode.Marquee) {
      GUI.Label(EditorGUIExt.FromToRect(s_StartMouseDragPosition,
                                        s_EndMouseDragPosition),
                GUIContent.none, Styles.selectionRect);
    }
  }
  ...
  private void EditAxisLabels() {
    ...
    bool isEditable = axisUiScalars.y >= 0 && curvesInSameSpace.Count > 0 &&
                      curvesInSameSpace[0].setAxisUiScalarsCallback != null;
    if (!isEditable)
      return;
      ...
      if (evt.button == 0) {
        ...
        if (!editRect.Contains(Event.current.mousePosition))
          GUIUtility.keyboardControl =
              0; // If not hitting our FloatField ensure it loses focus
      }
      break;

    case EventType.MouseDrag:
      if (GUIUtility.hotControl == id) {
        float dragSensitity = Mathf.Clamp01(
            Mathf.Max(axisUiScalars.y,
                      Mathf.Pow(Mathf.Abs(axisUiScalars.y), 0.5f)) *
            .01f);
        axisUiScalars.y += HandleUtility.niceMouseDelta * dragSensitity;
        if (axisUiScalars.y < 0.001f)
          axisUiScalars.y =
              0.001f; // Since the scalar is a magnitude we do not want to drag
                      // to 0 and below.. find nicer solution
    ...
    float newValue = EditorGUI.FloatField(editRect, axisUiScalars.y,
                                          Styles.axisLabelNumberField);
    if (axisUiScalars.y != newValue)
      SetAxisUiScalars(new Vector2(axisUiScalars.x, newValue),
                       curvesInSameSpace);
  ...
  public void TimeRangeSelectTo(float time) {
    if (!s_TimeRangeSelectionActive) {
      Debug.LogError(
          "TimeRangeSelectTo can only be called after BeginTimeRangeSelection");
      return;
    }
    ...
    float minTime =
        Mathf.Min(s_TimeRangeSelectionStart, s_TimeRangeSelectionEnd);
    float maxTime =
        Mathf.Max(s_TimeRangeSelectionStart, s_TimeRangeSelectionEnd);
    foreach (CurveWrapper cw in m_AnimationCurves) {
      if (cw.readOnly || cw.hidden)
        continue;
      int i = 0;
  ...
  public void EndTimeRangeSelection() {
    if (!s_TimeRangeSelectionActive) {
      Debug.LogError(
          "EndTimeRangeSelection can only be called after BeginTimeRangeSelection");
      return;
    }
  ...
  public void CancelTimeRangeSelection() {
    if (!s_TimeRangeSelectionActive) {
      Debug.LogError(
          "CancelTimeRangeSelection can only be called after BeginTimeRangeSelection");
      return;
    }
  ...
  void StartEditingSelectedPoints(Vector2 fieldPosition) {
    ...
    if (selectedCurves.All(x => GetKeyframeFromSelection(x).time ==
                                keyframe.time))
      m_NewTime = keyframe.time;
    if (selectedCurves.All(x => GetKeyframeFromSelection(x).value ==
                                keyframe.value))
  ...
  void FinishEditingPoints() { m_EditingPoints = false; }
  ...
  void EditSelectedPoints() {
    ...
    if (evt.type == EventType.KeyDown) {
      if (evt.keyCode == KeyCode.KeypadEnter || evt.keyCode == KeyCode.Return) {
        if (hasSelection && !m_EditingPoints) {
          ...
          SetSelectedKeyPositions(m_NewTime, m_NewValue, m_TimeWasEdited,
                                  m_ValueWasEdited);
          FinishEditingPoints();
          GUI.changed = true;
          evt.Use();
    ...
    var fieldPosition =
        DrawingToViewTransformPoint(m_PointEditingFieldPosition);

    const float kFieldHeight = 18f;
    const float kFieldWidth = 95f;

    // Keep fields in the drawing margins
    var drawAreaInMargins =
        Rect.MinMaxRect(leftmargin, topmargin, rect.width - rightmargin,
                        rect.height - bottommargin);
    fieldPosition.x = Mathf.Clamp(fieldPosition.x, drawAreaInMargins.xMin,
                                  drawAreaInMargins.xMax - kFieldWidth);
    fieldPosition.y = Mathf.Clamp(fieldPosition.y, drawAreaInMargins.yMin,
                                  drawAreaInMargins.yMax - kFieldHeight * 2);
    ...
    if (curveWrapper.useScalingInKeyEditor &&
        curveWrapper.getAxisUiScalarsCallback != null) {
    ...
    m_NewTime =
        PointFieldForSelection(
            new Rect(fieldPosition.x, fieldPosition.y, kFieldWidth,
                     kFieldHeight),
            1, m_NewTime * scale.x, x => GetKeyframeFromSelection(x).time,
            (r, id, time) => TimeField(r, id, time, invSnap, timeFormat),
            curveWrapper.xAxisLabel == null ? settings.xAxisLabel
            : curveWrapper.xAxisLabel) /
        scale.x;
    ...
    m_NewValue =
        PointFieldForSelection(
            new Rect(fieldPosition.x, fieldPosition.y + kFieldHeight,
                     kFieldWidth, kFieldHeight),
            2, m_NewValue * scale.y, x => GetKeyframeFromSelection(x).value,
            (r, id, value) => ValueField(r, id, value),
            curveWrapper.yAxisLabel == null ? settings.yAxisLabel
            : curveWrapper.yAxisLabel) /
        scale.y;
    if (EditorGUI.EndChangeCheck()) {
    ...
    if (evt.type == EventType.KeyDown) {
      ...
      const char endOfMediumCharacter =
          (char) 25; // ASCII 25: "End Of Medium" on pressing shift tab

      // Override Unity's Tab and Shift+Tab handling.
      if (evt.character == tabCharacter ||
          evt.character == endOfMediumCharacter) {
        ...
        m_FocusedPointField = GUI.GetNameOfFocusedControl() ==
                              kPointValueFieldName ? kPointTimeFieldName
            : kPointValueFieldName;
        evt.Use();
      }
    }
  ...
  float
  PointFieldForSelection(Rect rect, int customID, float value,
                         System.Func<CurveSelection, float> memberGetter,
                         System.Func<Rect, int, float, float> fieldCreator,
                         string label) {
    float firstSelectedValue = memberGetter(selectedCurves[0]);
    bool sameValues =
        selectedCurves.All(x => memberGetter(x) == firstSelectedValue);
    if (!sameValues)
  ...
  // m_DraggingCurveOrRegion is null if nothing is being dragged, has one entry
  // if single curve being dragged or has two entries if region is being dragged
  CurveWrapper[] m_DraggingCurveOrRegion = null;

  void SetupKeyOrCurveDragging(Vector2 timeValue, CurveWrapper cw, int id,
                               Vector2 mousePos) {
  ...
  public Vector2 MovePoints() {
    ...
    case EventType.MouseDown:
      if (evt.button == 0) {
        ...
        foreach (CurveSelection cs in selectedCurves) {
          ...
          if ((DrawingToViewTransformPoint(GetPosition(cs)) - evt.mousePosition)
                  .sqrMagnitude <= kMaxPickDistSqr) {
            Keyframe keyframe = GetKeyframeFromSelection(cs);
            SetupKeyOrCurveDragging(new Vector2(keyframe.time, keyframe.value),
                                    curveWrapper, id, evt.mousePosition);
        ...
        // Curve dragging. Moving keys has highest priority, therefore we check
        // curve/region dragging AFTER key dragging above
        if (evt.shift && settings.allowDraggingCurvesAndRegions &&
            m_DraggingKey == null) {
          // We use the logic as for moving keys when we drag entire curves or
          // regions: We just select all keyFrames in a curve or region before
          // dragging and ensure to hide tangents when drawing.
          Vector2 timeValue = Vector2.zero;
          CurveWrapper[] curves = null;
          if (HandleCurveAndRegionMoveToFrontOnMouseDown(ref timeValue,
                                                         ref curves)) {
            ...
            SetupKeyOrCurveDragging(timeValue, curves[0], id,
                                    evt.mousePosition);
    ...
    case EventType.MouseDrag:
      if (GUIUtility.hotControl == id) {
        ...
        if (evt.shift && m_AxisLock == AxisLock.None)
          m_AxisLock =
              Mathf.Abs(delta.x) > Mathf.Abs(delta.y) ? AxisLock.X : AxisLock.Y;
        ...
        if (m_DraggingCurveOrRegion != null) {
          ...
          motion.y = SnapValue(motion.y + s_StartKeyDragPosition.y) -
                     s_StartKeyDragPosition.y;
        } else {
          switch (m_AxisLock) {
          case AxisLock.None:
            motion = ViewToDrawingTransformVector(delta);
            motion.x = SnapTime(motion.x + s_StartKeyDragPosition.x) -
                       s_StartKeyDragPosition.x;
            motion.y = SnapValue(motion.y + s_StartKeyDragPosition.y) -
                       s_StartKeyDragPosition.y;
            break;
          case AxisLock.X:
    ...
    case EventType.Repaint:
      Rect mouseRect =
          new Rect(evt.mousePosition.x - 10, evt.mousePosition.y - 10, 20, 20);
      if (m_DraggingCurveOrRegion != null)
  ...
  void ResetDragging() {
    // If we are dragging entire curve we have selected all keys we therefore
    // ensure to deselect them again...
    if (m_DraggingCurveOrRegion != null) {
  ...
  void MakeCurveBackups() {
    ...
    for (int i = 0; i < selectedCurves.Count; i++) {
      CurveSelection cs = selectedCurves[i];
      // if it's a different curve than last point, we need to back up this
      // curve.
      if (cs.curveID != lastCurveID) {
        AnimationCurve curve = GetCurveFromSelection(cs);
        if (curve != null) {
          // Make a new saved curve with copy of all keyframes. No need to mark
          // them as selected
          sc = new SavedCurve();
          ...
          foreach (Keyframe k in keys)
            sc.keys.Add(new SavedCurve.SavedKeyFrame(
                k, CurveWrapper.SelectionMode.None));
          m_CurveBackups.Add(sc);
        }
      }

      // Mark them as selected
      sc.keys[cs.key].selected = cs.semiSelected ? CurveWrapper.SelectionMode
                                     .SemiSelected
          : CurveWrapper.SelectionMode.Selected;
    }
  }
  ...
  // Get the position of a CurveSelection. This will correctly offset tangent
  // handles
  Vector2 GetPosition(CurveSelection selection) {
    ...
    if (selection.type == CurveSelection.SelectionType.InTangent) {
      ...
      if ((key.weightedMode & WeightedMode.In) == WeightedMode.In) {
        Keyframe prevKey = (selection.key >= 1 && selection.key < curve.length)
                               ? curve[selection.key - 1]
                               : key;
        float dx = key.time - prevKey.time;
      ...
      if ((key.weightedMode & WeightedMode.Out) == WeightedMode.Out) {
        Keyframe nextKey =
            (selection.key >= 0 && selection.key < (curve.length - 1))
                ? curve[selection.key + 1]
                : key;
        float dx = nextKey.time - key.time;
  ...
  bool IsRegionCurveSelected(CurveWrapper cw1, CurveWrapper cw2) {
    return IsCurveSelected(cw1) || IsCurveSelected(cw2);
  }
  ...
  void DrawCurvesAndRegion(CurveWrapper cw1, CurveWrapper cw2,
                           List<CurveSelection> selection, bool hasFocus) {
  ...
  void DrawCurveAndPoints(CurveWrapper cw, List<CurveSelection> selection,
                          bool hasFocus) {
    DrawCurve(cw, hasFocus);
    DrawPointsOnCurve(cw, selection, hasFocus);
  }

  bool ShouldCurveHaveFocus(int indexIntoDrawOrder, CurveWrapper cw1,
                            CurveWrapper cw2) {
    bool focus = false;
  ...
  void DrawCurves() {
    ...
    for (int i = 0; i < m_DrawOrder.Count; ++i) {
      ...
      if (IsRegion(cw, cw2)) {
        ...
        DrawCurvesAndRegion(
            cw, cw2, IsRegionCurveSelected(cw, cw2) ? selectedCurves : null,
            focus);
      } else {
        bool focus = ShouldCurveHaveFocus(i, cw, null);
        DrawCurveAndPoints(cw, IsCurveSelected(cw) ? selectedCurves : null,
                           focus);
      }
    }
  ...
  void DrawCurvesTangents() {
    ...
    for (int i = 0; i < selectedCurves.Count; ++i) {
      ...
      if (IsLeftTangentEditable(sel) &&
          GetKeyframeFromSelection(sel).time != curve.keys[0].time) {
        Vector2 leftTangent = GetPosition(new CurveSelection(
            sel.curveID, sel.key, CurveSelection.SelectionType.InTangent));
        DrawLine(leftTangent, keyPoint);
      }

      if (IsRightTangentEditable(sel) &&
          GetKeyframeFromSelection(sel).time !=
              curve.keys[curve.keys.Length - 1].time) {
        Vector2 rightTangent = GetPosition(new CurveSelection(
            sel.curveID, sel.key, CurveSelection.SelectionType.OutTangent));
    ...
    for (int i = 0; i < selectedCurves.Count; ++i) {
      ...
      if (IsLeftTangentEditable(sel) &&
          GetKeyframeFromSelection(sel).time != curve.keys[0].time) {
        Vector2 leftTangent =
            DrawingToViewTransformPoint(GetPosition(new CurveSelection(
                sel.curveID, sel.key, CurveSelection.SelectionType.InTangent)));
        DrawTangentPoint(leftTangent,
                         (GetKeyframeFromSelection(sel).weightedMode &
                          WeightedMode.In) == WeightedMode.In);
      }

      if (IsRightTangentEditable(sel) &&
          GetKeyframeFromSelection(sel).time !=
              curve.keys[curve.keys.Length - 1].time) {
        Vector2 rightTangent = DrawingToViewTransformPoint(GetPosition(
            new CurveSelection(sel.curveID, sel.key,
                               CurveSelection.SelectionType.OutTangent)));
        DrawTangentPoint(rightTangent,
                         (GetKeyframeFromSelection(sel).weightedMode &
                          WeightedMode.Out) == WeightedMode.Out);
      }
    }
  ...
  void DrawCurvesOverlay() {
    ...
    if (m_DraggingKey != null &&
        settings.rectangleToolFlags == RectangleToolFlags.NoRectangleTool) {
      GUI.color = Color.white;

      // Clamp only using the currently dragged curve (we could have more
      // selected but we only show the coord info for this one).
      ...
      newPoint.y =
          Mathf.Clamp(newPoint.y, smallestVRangeMin, smallestVRangeMax);
      Vector2 p = DrawingToViewTransformPoint(newPoint);

      Vector2 axisUiScalars =
          m_DraggingKey.getAxisUiScalarsCallback !=
          null ? m_DraggingKey.getAxisUiScalarsCallback() : Vector2.one;
      if (axisUiScalars.x >= 0f)
        newPoint.x *= axisUiScalars.x;
      if (axisUiScalars.y >= 0f)
        newPoint.y *= axisUiScalars.y;
      GUIContent content = new GUIContent(
          string.Format("{0}, {1}", FormatTime(newPoint.x, invSnap, timeFormat),
                        FormatValue(newPoint.y)));
      Vector2 size = Styles.dragLabel.CalcSize(content);
      EditorGUI.DoDropShadowLabel(new Rect(p.x, p.y - size.y, size.x, size.y),
                                  content, Styles.dragLabel, 0.3f);
    }
  }

  List<Vector3> CreateRegion(CurveWrapper minCurve, CurveWrapper maxCurve,
                             float deltaTime) {
    ...
    // To handle constant curves (high gradient) we add key time samples on both
    // side of the keys as well the key time itself.
    ...
    Vector3 prevA =
        new Vector3(range.x, maxCurve.renderer.EvaluateCurveSlow(0.0f), 0.0f);
    Vector3 prevB =
        new Vector3(range.x, minCurve.renderer.EvaluateCurveSlow(0.0f), 0.0f);
    ...
    for (int i = 0; i < sampleTimes.Count; ++i) {
      ...
      Vector3 valueA =
          new Vector3(time, maxCurve.renderer.EvaluateCurveSlow(time), 0.0f);
      Vector3 valueB =
          new Vector3(time, minCurve.renderer.EvaluateCurveSlow(time), 0.0f);
      ...
      if (prevA.y >= prevB.y && valueA.y >= valueB.y) {
        ...
        if (Mathf.LineIntersection(screenPrevA, screenValueA, screenPrevB,
                                   screenValueB, ref intersection)) {
          region.Add(screenPrevA);
  ...
  public void DrawRegion(CurveWrapper curve1, CurveWrapper curve2,
                         bool hasFocus) {
    if (Event.current.type != EventType.Repaint)
      return;
  ...
  void DrawCurve(CurveWrapper cw, bool hasFocus) {
    ...
    renderer.DrawCurve(framed.xMin, framed.xMax, color, drawingToViewMatrix,
                       settings.wrapColor * cw.wrapColorMultiplier);
  }

  void DrawPointsOnCurve(CurveWrapper cw, List<CurveSelection> selected,
                         bool hasFocus) {
    if (selected == null) {
      Color color = cw.color;
      ...
      for (int i = 0; i < keys.Length; ++i) {
        Keyframe k = keys[i];
        DrawPoint(DrawingToViewTransformPoint(new Vector2(k.time, k.value)),
      ...
      int selectionIdx = 0;
      // Find the point in selected curves that matches the curve we're about to
      // draw.
      while (selectionIdx < selected.Count &&
             selected[selectionIdx].curveID != cw.id)
        selectionIdx++;
      ...
      for (int i = 0; i < keys.Length; ++i) {
        Keyframe k = keys[i];
        if (selectionIdx < selected.Count &&
            selected[selectionIdx].key == pointIdx &&
            selected[selectionIdx].curveID == cw.id) {
          if (selected[selectionIdx].semiSelected)
            DrawPoint(DrawingToViewTransformPoint(new Vector2(k.time, k.value)),
                      CurveWrapper.SelectionMode.SemiSelected);
          else
            DrawPoint(
                DrawingToViewTransformPoint(new Vector2(k.time, k.value)),
                CurveWrapper.SelectionMode.Selected,
                settings.rectangleToolFlags ==
                    RectangleToolFlags.NoRectangleTool ? MouseCursor.MoveArrow
                : MouseCursor.Arrow);
          selectionIdx++;
        } else
          DrawPoint(DrawingToViewTransformPoint(new Vector2(k.time, k.value)),
  ...
  void DrawPoint(Vector2 viewPos, CurveWrapper.SelectionMode selected,
    ...
    // Important to take floor of positions of GUI stuff to get pixel correct
    // alignment of stuff drawn with both GUI and Handles/GL. Otherwise things
    // are off by one pixel half the time.
    var rect = new Rect(Mathf.Floor(viewPos.x) - 4, Mathf.Floor(viewPos.y) - 4,
                        Styles.pointIcon.width, Styles.pointIcon.height);
    ...
    // Changing the cursor for every point in the selection can be awfully
    // costly.
  ...
  WrapMode WrapModeIconPopup(Keyframe key, WrapMode oldWrap, float hOffset) {
    ...
    var r = new Rect(keyPosition.x + buttonSize * hOffset, keyPosition.y,
    ...
    Enum[] enumValues =
        Enum.GetValues(typeof(WrapModeFixedCurve)).Cast<Enum>().ToArray();
    var stringNames = Enum.GetNames(typeof(WrapModeFixedCurve))
    ...
    int controlID = GUIUtility.GetControlID("WrapModeIconPopup".GetHashCode(),
                                            FocusType.Keyboard, r);

    // DoPopup begins

    var selectedPopupIndex =
        EditorGUI.PopupCallbackInfo.GetSelectedValueForControl(controlID, i);
    ...
    case EventType.MouseDown:
      if (evt.button == 0 && r.Contains(evt.mousePosition)) {
        ...
        EditorGUI.PopupCallbackInfo.instance =
            new EditorGUI.PopupCallbackInfo(controlID);
        EditorUtility.DisplayCustomMenu(
            r, popupStrings, selectedPopupIndex,
            EditorGUI.PopupCallbackInfo.instance.SetEnumValueDelegate, null);
  ...
  // The return value for each axis can be -1, if so then we do not have any
  // proper value to use.
  private Vector2
  GetAxisUiScalars(List<CurveWrapper> curvesWithSameParameterSpace) {
    // If none or just one selected curve then use top most rendered curve value
    if (selectedCurves.Count <= 1) {
      if (m_DrawOrder.Count > 0) {
        CurveWrapper cw =
            GetCurveWrapperFromID(m_DrawOrder[m_DrawOrder.Count - 1]);
    ...
    // If multiple curves selected we have to check if they are in the same
    // value space
  ...
  private void SetAxisUiScalars(Vector2 newScalars,
                                List<CurveWrapper> curvesInSameSpace) {
    foreach (CurveWrapper cw in curvesInSameSpace) {
      // Only set valid values (-1 indicate invalid value, if so use original
      // value)
  ...
  public void GridGUI() {
    ...
    hTicks.SetRanges(shownRect.xMin * axisUiScalars.x,
                     shownRect.xMax * axisUiScalars.x, drawRect.xMin,
                     drawRect.xMax);
    vTicks.SetRanges(shownRect.yMin * axisUiScalars.y,
                     shownRect.yMax * axisUiScalars.y, drawRect.yMin,
                     drawRect.yMax);
    ...
    hTicks.SetTickStrengths(settings.hTickStyle.distMin,
                            settings.hTickStyle.distFull, false);
    ...
    for (int l = 0; l < hTicks.tickLevels; l++) {
      ...
      if (strength > 0f) {
        GL.Color(settings.hTickStyle.tickColor * new Color(1, 1, 1, strength) *
        ...
        for (int j = 0; j < ticks.Length; j++) {
          ...
          if (ticks[j] > hRangeMin && ticks[j] < hRangeMax)
            DrawLine(new Vector2(ticks[j], lineStart),
                     new Vector2(ticks[j], lineEnd));
        }
      }
    }

    // Draw bounds of allowed range
    GL.Color(settings.hTickStyle.tickColor * new Color(1, 1, 1, 1) *
             new Color(1, 1, 1, 0.75f));
    if (hRangeMin != Mathf.NegativeInfinity)
      DrawLine(new Vector2(hRangeMin, lineStart),
               new Vector2(hRangeMin, lineEnd));
    if (hRangeMax != Mathf.Infinity)
      DrawLine(new Vector2(hRangeMax, lineStart),
               new Vector2(hRangeMax, lineEnd));

    vTicks.SetTickStrengths(settings.vTickStyle.distMin,
                            settings.vTickStyle.distFull, false);
    ...
    for (int l = 0; l < vTicks.tickLevels; l++) {
      ...
      if (strength > 0f) {
        GL.Color(settings.vTickStyle.tickColor * new Color(1, 1, 1, strength) *
                 new Color(1, 1, 1, 0.75f));
        float[] ticks = vTicks.GetTicksAtLevel(l, true);
        for (int j = 0; j < ticks.Length; j++) {
          ticks[j] /= axisUiScalars.y;
          if (ticks[j] > vRangeMin && ticks[j] < vRangeMax)
            DrawLine(new Vector2(lineStart, ticks[j]),
                     new Vector2(lineEnd, ticks[j]));
        }
      }
    }
    // Draw bounds of allowed range
    GL.Color(settings.vTickStyle.tickColor * new Color(1, 1, 1, 1) *
             new Color(1, 1, 1, 0.75f));
    if (vRangeMin != Mathf.NegativeInfinity)
      DrawLine(new Vector2(lineStart, vRangeMin),
               new Vector2(lineEnd, vRangeMin));
    if (vRangeMax != Mathf.Infinity)
      DrawLine(new Vector2(lineStart, vRangeMax),
               new Vector2(lineEnd, vRangeMax));
    ...
    if (settings.showAxisLabels) {
      ...
      if (settings.hTickStyle.distLabel > 0 && axisUiScalars.x > 0f) {
        ...
        int labelLevel =
            hTicks.GetLevelWithMinSeparation(settings.hTickStyle.distLabel);

        // Calculate how many decimals are needed to show the differences
        // between the labeled ticks
        int decimals = MathUtils.GetNumberOfDecimalsForMinimumDifference(
            hTicks.GetPeriodOfLevel(labelLevel));
        ...
        for (int i = 0; i < ticks.Length; i++) {
          ...
          if (ticksPos[i] < hRangeMin || ticksPos[i] > hRangeMax)
            continue;
          Vector2 pos =
              DrawingToViewTransformPoint(new Vector2(ticksPos[i], 0));
          // Important to take floor of positions of GUI stuff to get pixel
          // correct alignment of stuff drawn with both GUI and Handles/GL.
          // Otherwise things are off by one pixel half the time.
          ...
          if (settings.hTickStyle.centerLabel) {
            ...
            labelRect =
                new Rect(pos.x, pos.y - 16 - settings.hTickLabelOffset, 1, 16);
          } else {
            wantedAlignment = TextAnchor.UpperLeft;
            labelRect =
                new Rect(pos.x, pos.y - 16 - settings.hTickLabelOffset, 50, 16);
          }
          ...
          GUI.Label(labelRect,
                    uiValue.ToString("n" + decimals) + settings.hTickStyle.unit,
                    Styles.labelTickMarksX);
        }
      }
      ...
      if (settings.vTickStyle.distLabel > 0 && axisUiScalars.y > 0f) {
        ...
        int labelLevel =
            vTicks.GetLevelWithMinSeparation(settings.vTickStyle.distLabel);
        ...
        // between the labeled ticks
        int decimals = MathUtils.GetNumberOfDecimalsForMinimumDifference(
            vTicks.GetPeriodOfLevel(labelLevel));
        ...
        if (!settings.vTickStyle.stubs && ticks.Length > 1) {
          ...
          labelSize =
              Mathf.Max(
                  ...
                  Styles.labelTickMarksY.CalcSize(new GUIContent(maxNumber))
                      .x) +
              6;
        }
        ...
        for (int i = 0; i < ticks.Length; i++) {
          ...
          if (ticksPos[i] < vRangeMin || ticksPos[i] > vRangeMax)
            continue;
          Vector2 pos =
              DrawingToViewTransformPoint(new Vector2(0, ticksPos[i]));
          // Important to take floor of positions of GUI stuff to get pixel
          // correct alignment of stuff drawn with both GUI and Handles/GL.
          // Otherwise things are off by one pixel half the time.
          ...
          if (settings.vTickStyle.centerLabel)
            labelRect = new Rect(
                0, pos.y - 8, leftmargin - 4,
                16); // text expands to the left starting from where grid starts
                     // (leftmargin size must ensure text is visible)
          else
            labelRect = new Rect(0, pos.y - 13, labelSize,
                                 16); // text expands to the right starting from
                                      // left side of window

          GUI.Label(labelRect,
                    uiValue.ToString(format) + settings.vTickStyle.unit,
                    Styles.labelTickMarksY);
        }
      }
    }
    // Cleanup
