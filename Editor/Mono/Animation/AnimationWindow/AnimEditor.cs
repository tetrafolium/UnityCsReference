...
internal class AnimEditor : ScriptableObject {
  ...
  private float hierarchyWidth {
    get { return m_HorizontalSplitter.realSizes[0]; }
  }
  private float contentWidth {
    get { return m_HorizontalSplitter.realSizes[1]; }
  }

  internal static PrefColor kEulerXColor =
      new PrefColor("Animation/EulerX", 1.0f, 0.0f, 1.0f, 1.0f);
  internal static PrefColor kEulerYColor =
      new PrefColor("Animation/EulerY", 1.0f, 1.0f, 0.0f, 1.0f);
  internal static PrefColor kEulerZColor =
      new PrefColor("Animation/EulerZ", 0.0f, 1.0f, 1.0f, 1.0f);

  static private Color s_SelectionRangeColorLight =
      new Color32(255, 255, 255, 90);
  static private Color s_SelectionRangeColorDark =
      new Color32(200, 200, 200, 40);

  static private Color selectionRangeColor {
    get {
      return EditorGUIUtility.isProSkin ? s_SelectionRangeColorDark
          : s_SelectionRangeColorLight;
    }
  }
  ...
  static private Color outOfRangeColor {
    get {
      return EditorGUIUtility.isProSkin ? s_OutOfRangeColorDark
          : s_OutOfRangeColorLight;
    }
  }
  ...
  static private Color inRangeColor {
    get {
      return EditorGUIUtility.isProSkin ? s_InRangeColorDark
          : s_InRangeColorLight;
    }
  }

  static private Color s_FilterBySelectionColorLight =
      new Color(0.82f, 0.97f, 1.00f, 1.00f);
  static private Color s_FilterBySelectionColorDark =
      new Color(0.54f, 0.85f, 1.00f, 1.00f);

  static private Color filterBySelectionColor {
    get {
      return EditorGUIUtility.isProSkin ? s_FilterBySelectionColorDark
          : s_FilterBySelectionColorLight;
    }
  }
  ...
  internal static FrameRateMenuEntry[] kAvailableFrameRates =
      new FrameRateMenuEntry[]{
          new FrameRateMenuEntry(
              EditorGUIUtility.TextContent("Set Sample Rate/24"), 24f),
          new FrameRateMenuEntry(
              EditorGUIUtility.TextContent("Set Sample Rate/25"), 25f),
          new FrameRateMenuEntry(
              EditorGUIUtility.TextContent("Set Sample Rate/30"), 30f),
          new FrameRateMenuEntry(
              EditorGUIUtility.TextContent("Set Sample Rate/50"), 50f),
          new FrameRateMenuEntry(
              EditorGUIUtility.TextContent("Set Sample Rate/60"), 60f)};
  ...
  public AnimationWindowSelectionItem selection {
    get { return m_State.selection; }

    set { m_State.selection = value; }
  }

  public IAnimationWindowControl controlInterface {
    get { return state.controlInterface; }
  }

  public IAnimationWindowControl overrideControlInterface {
    get { return state.overrideControlInterface; }

    set { state.overrideControlInterface = value; }
  }
  ...
  public void OnAnimEditorGUI(EditorWindow parent, Rect position) {
    ...
    using(new EditorGUI.DisabledScope(m_State.disabled ||
      ...
      using(
          new GUILayout.HorizontalScope(AnimationWindowStyles.toolbarBottom)) {
        TabSelectionOnGUI();
      }
      ...
      // Acquire Rects
      Rect timerulerRect =
          GUILayoutUtility.GetRect(contentWidth, layoutRowHeight);
      Rect eventsRect =
          GUILayoutUtility.GetRect(contentWidth, layoutRowHeight - 1);
      Rect contentLayoutRect = GUILayoutUtility.GetRect(
          contentWidth, contentWidth, 0f, float.MaxValue,
      ...
      if (Event.current.type == EventType.Repaint) {
        OptionsOnGUI(optionsID);
        AnimationWindowStyles.separator.Draw(
            new Rect(hierarchyWidth, 0, 1, position.height), false, false,
            false, false);
      }
  ...
  void MainContentOnGUI(Rect contentLayoutRect) {
    ...
    if (m_State.animatorIsOptimized) {
      GUI.Label(contentLayoutRect, GUIContent.none,
                AnimationWindowStyles.dopeSheetBackground);

      Vector2 textSize =
          GUI.skin.label.CalcSize(AnimationWindowStyles.animatorOptimizedText);
      Rect labelRect =
          new Rect(contentLayoutRect.x + contentLayoutRect.width * .5f -
                       textSize.x * .5f,
                   contentLayoutRect.y + contentLayoutRect.height * .5f -
                       textSize.y * .5f,
    ...
    if (m_State.disabled) {
      ...
      if (evt.type == EventType.MouseDown &&
          contentLayoutRect.Contains(evt.mousePosition))
  ...
  private void OverlayEventOnGUI() {
    ...
    Rect overlayRect =
        new Rect(hierarchyWidth - 1, 0f, contentWidth - kSliderThickness,
                 m_Position.height - kSliderThickness);
  ...
  private void OverlayOnGUI(Rect contentRect) {
    ...
    Rect contentRectNoSliders = new Rect(contentRect.xMin, contentRect.yMin,
                                         contentRect.width - kSliderThickness,
                                         contentRect.height - kSliderThickness);
    Rect overlayRectNoSliders =
        new Rect(hierarchyWidth - 1, 0f, contentWidth - kSliderThickness,
                 m_Position.height - kSliderThickness);
    ...
    Rect localRect =
        new Rect(0, 0, overlayRectNoSliders.width, overlayRectNoSliders.height);
  ...
  public void OnEnable() {
    ...
    if (m_State == null) {
      m_State =
          CreateInstance(typeof(AnimationWindowState)) as AnimationWindowState;
    ...
    m_State.timeArea =
        m_State.showCurveEditor ?(TimeArea) m_CurveEditor : m_DopeSheet;
  ...
  public void OnSelectionChanged() {
    triggerFraming = true; // Framing of clip can only be done after Layout.
                           // Here we just order it to happen later on.
    Repaint();
  }
  ...
  public void OnStartLiveEdit() { SaveCurveEditorKeySelection(); }
  ...
  public void OnLostFocus() {
    // RenameOverlay might still be active, close before switching to another
    // window.
    if (m_Hierarchy != null)
      m_Hierarchy.EndNameEditing(true);

    // Stop text editing.  FrameRate ui or Frame ui may still be in edition
    // mode.
  ...
  private void SetupWizardOnGUI(Rect position) {
    GUI.Label(position, GUIContent.none,
              AnimationWindowStyles.dopeSheetBackground);

    Rect positionWithoutScrollBar =
        new Rect(position.x, position.y, position.width - kSliderThickness,
    ...
    bool animatableObject =
        m_State.activeGameObject &&
        !EditorUtility.IsPersistent(m_State.activeGameObject);

    if (animatableObject) {
      var missingObjects =
          (!m_State.activeRootGameObject && !m_State.activeAnimationClip)
              ? AnimationWindowStyles.animatorAndAnimationClip.text
              : AnimationWindowStyles.animationClip.text;

      string txt = String.Format(AnimationWindowStyles.formatIsMissing.text,
                                 m_State.activeGameObject.name, missingObjects);
      ...
      Rect labelRect =
          new Rect(positionWithoutScrollBar.width * .5f - textSize.x * .5f,
                   positionWithoutScrollBar.height * .5f - textSize.y * .5f,
                   textSize.x, textSize.y);
      GUI.Label(labelRect, textContent);

      Rect buttonRect =
          new Rect(positionWithoutScrollBar.width * .5f - buttonWidth * .5f,
                   labelRect.yMax + buttonPadding, buttonWidth, buttonHeight);

      if (GUI.Button(buttonRect, AnimationWindowStyles.create)) {
        if (AnimationWindowUtility.InitializeGameobjectForAnimation(
                m_State.activeGameObject)) {
          Component animationPlayer =
              AnimationWindowUtility
                  .GetClosestAnimationPlayerComponentInParents(
                      m_State.activeGameObject.transform);
          m_State.activeAnimationClip =
              AnimationUtility.GetAnimationClips(animationPlayer.gameObject)[0];
        }
      ...
      Vector2 textSize = GUI.skin.label.CalcSize(
          AnimationWindowStyles.noAnimatableObjectSelectedText);
      Rect labelRect =
          new Rect(positionWithoutScrollBar.width * .5f - textSize.x * .5f,
                   positionWithoutScrollBar.height * .5f - textSize.y * .5f,
                   textSize.x, textSize.y);
    ...
    GUI.EndClip();
    GUI.enabled = false; // Reset state to false. It's always false originally
                         // for SetupWizardOnGUI.
  }

  private void EventLineOnGUI(Rect eventsRect) {
    eventsRect.width -= kSliderThickness;
    GUI.Label(eventsRect, GUIContent.none,
              AnimationWindowStyles.eventBackground);
  ...
  private void RenderEventTooltip() { m_Events.DrawInstantTooltip(m_Position); }

  private void TabSelectionOnGUI() {
    GUILayout.FlexibleSpace();
    EditorGUI.BeginChangeCheck();
    GUILayout.Toggle(!m_State.showCurveEditor, AnimationWindowStyles.dopesheet,
                     AnimationWindowStyles.miniToolbarButton,
                     GUILayout.Width(kToggleButtonWidth));
    GUILayout.Toggle(m_State.showCurveEditor, AnimationWindowStyles.curves,
  ...
  private void HierarchyOnGUI() {
    Rect hierarchyLayoutRect =
        GUILayoutUtility.GetRect(hierarchyWidth, hierarchyWidth, 0f,
                                 float.MaxValue, GUILayout.ExpandHeight(true));

    if (!m_State.showReadOnly && !m_State.selection.animationIsEditable) {
      Vector2 labelSize = GUI.skin.label.CalcSize(
          AnimationWindowStyles.readOnlyPropertiesLabel);
      ...
      Rect labelRect =
          new Rect(hierarchyLayoutRect.x + hierarchyLayoutRect.width * .5f -
                       labelSize.x * .5f,
                   hierarchyLayoutRect.y + hierarchyLayoutRect.height * .5f -
                       labelSize.y,
      ...
      if (GUI.Button(buttonRect,
                     AnimationWindowStyles.readOnlyPropertiesButton)) {
  ...
  private void FrameRateInputFieldOnGUI() {
    ...
    using(new EditorGUI.DisabledScope(!selection.animationIsEditable)) {
      ...
      EditorGUI.BeginChangeCheck();
      int clipFrameRate = EditorGUILayout.DelayedIntField(
          (int) m_State.clipFrameRate, EditorStyles.toolbarTextField,
          GUILayout.Width(kIntFieldWidth));
      if (EditorGUI.EndChangeCheck()) {
  ...
  private void ClipSelectionDropDownOnGUI() { m_ClipPopup.OnGUI(); }

  private void DopeSheetOnGUI(Rect position) {
    Rect noVerticalSliderRect =
        new Rect(position.xMin, position.yMin,
                 position.width - kSliderThickness, position.height);
    ...
    Rect noSlidersRect = new Rect(position.xMin, position.yMin,
                                  position.width - kSliderThickness,
                                  position.height - kSliderThickness);
    ...
    if (!m_State.disabled) {
      m_DopeSheet.TimeRuler(noSlidersRect, m_State.frameRate, false, true,
                            kDisabledRulerAlpha, m_State.timeFormat); // grid
    }
    m_DopeSheet.OnGUI(noSlidersRect, m_State.hierarchyState.scrollPos * -1);

    m_DopeSheet.EndViewGUI();

    Rect verticalScrollBarPosition =
        new Rect(noVerticalSliderRect.xMax, noVerticalSliderRect.yMin,
                 kSliderThickness, noSlidersRect.height);

    float visibleHeight = m_Hierarchy.GetTotalRect().height;
    float contentHeight =
        Mathf.Max(visibleHeight, m_Hierarchy.GetContentSize().y);

    m_State.hierarchyState.scrollPos.y = GUI.VerticalScrollbar(
        verticalScrollBarPosition, m_State.hierarchyState.scrollPos.y,
        visibleHeight, 0f, contentHeight);
  ...
  private void CurveEditorOnGUI(Rect position) {
    ...
    if (!m_State.disabled) {
      GUI.Box(noSlidersRect, GUIContent.none,
              AnimationWindowStyles.curveEditorBackground);
  ...
  private void TimeRulerOnGUI(Rect timeRulerRect) {
    Rect timeRulerRectNoScrollbar =
        new Rect(timeRulerRect.xMin, timeRulerRect.yMin,
                 timeRulerRect.width - kSliderThickness, timeRulerRect.height);
    Rect timeRulerBackgroundRect = timeRulerRectNoScrollbar;

    GUI.Box(timeRulerBackgroundRect, GUIContent.none,
            AnimationWindowStyles.timeRulerBackground);
    ...
    m_State.timeArea.TimeRuler(timeRulerRectNoScrollbar, m_State.frameRate,
                               true, false, 1f, m_State.timeFormat);
  ...
  private GenericMenu GenerateOptionsMenu() {
    ...
    menu.AddItem(EditorGUIUtility.TextContent("Seconds"),
                 m_State.timeFormat == TimeArea.TimeFormat.TimeFrame,
                 () => m_State.timeFormat = TimeArea.TimeFormat.TimeFrame);
    menu.AddItem(EditorGUIUtility.TextContent("Frames"),
                 m_State.timeFormat == TimeArea.TimeFormat.Frame,
                 () => m_State.timeFormat = TimeArea.TimeFormat.Frame);

    menu.AddSeparator("");

    menu.AddItem(EditorGUIUtility.TextContent("Ripple"), m_State.rippleTime,
                 () => m_State.rippleTime = !m_State.rippleTime);

    menu.AddSeparator("");

    menu.AddItem(EditorGUIUtility.TextContent("Show Sample Rate"),
                 m_State.showFrameRate,
                 () => m_State.showFrameRate = !m_State.showFrameRate);
    ...
    for (int i = 0; i < kAvailableFrameRates.Length; ++i) {
      ...
      menu.AddItem(entry.content, isActive, isAnimatable ? SetFrameRate
                   : nullMenuFunction2, entry.value);
    }

    menu.AddSeparator("");

    menu.AddItem(EditorGUIUtility.TextContent("Show Read-only Properties"),
                 m_State.showReadOnly,
                 () => m_State.showReadOnly = !m_State.showReadOnly);
  ...
  private void OptionsOnGUI(int controlID) {
    Rect layoutRect =
        new Rect(hierarchyWidth, 0f, contentWidth, layoutRowHeight);

    GUI.BeginGroup(layoutRect);

    Vector2 optionsSize = EditorStyles.toolbarButtonRight.CalcSize(
        AnimationWindowStyles.optionsContent);
    Rect optionsRect = new Rect(layoutRect.width - kSliderThickness, 0f,
                                optionsSize.x, optionsSize.y);
    GUI.Box(optionsRect, GUIContent.none,
            AnimationWindowStyles.animPlayToolBar);
    if (EditorGUI.DropdownButton(controlID, optionsRect,
                                 AnimationWindowStyles.optionsContent,
                                 AnimationWindowStyles.optionsButton)) {
  ...
  private void FilterBySelectionButtonOnGUI() {
    ...
    bool filterBySelection =
        GUILayout.Toggle(m_State.filterBySelection,
                         AnimationWindowStyles.filterBySelectionContent,
  ...
  private void AddEventButtonOnGUI() {
    using(new EditorGUI.DisabledScope(!selection.animationIsEditable)) {
      if (GUILayout.Button(AnimationWindowStyles.addEventContent,
                           AnimationWindowStyles.animClipToolbarButton))
        m_Events.AddEvent(m_State.currentTime, selection.rootGameObject,
                          selection.animationClip);
    }
  }

  private void AddKeyframeButtonOnGUI() {
    bool canAddKey =
        selection.animationIsEditable && m_State.allCurves.Count != 0;
    using(new EditorGUI.DisabledScope(!canAddKey)) {
      if (GUILayout.Button(AnimationWindowStyles.addKeyframeContent,
                           AnimationWindowStyles.animClipToolbarButton)) {
        SaveCurveEditorKeySelection();
        var keyTime =
            AnimationKeyTime.Time(m_State.currentTime, m_State.frameRate);
        AnimationWindowUtility.AddSelectedKeyframes(m_State, keyTime);
        UpdateSelectedKeysToCurveEditor();

        // data is scheduled for an update, bail out now to avoid using out of
        // date data.
        EditorGUIUtility.ExitGUI();
      }
    }
  ...
  private void PlayControlsOnGUI() {
    ...
    if (GUILayout.Button(AnimationWindowStyles.firstKeyContent,
                         EditorStyles.toolbarButton)) {
      controlInterface.GoToFirstKeyframe();

      // Stop text editing.  User may be editing frame navigation ui which will
      // not update until we exit text editing.
      EditorGUI.EndEditingActiveTextField();
    }

    if (GUILayout.Button(AnimationWindowStyles.prevKeyContent,
                         EditorStyles.toolbarButton)) {
    ...
    if (GUILayout.Button(AnimationWindowStyles.nextKeyContent,
                         EditorStyles.toolbarButton)) {
    ...
    if (GUILayout.Button(AnimationWindowStyles.lastKeyContent,
                         EditorStyles.toolbarButton)) {
    ...
    EditorGUI.BeginChangeCheck();
    int newFrame = EditorGUILayout.DelayedIntField(
        m_State.currentFrame, EditorStyles.toolbarTextField,
        GUILayout.Width(kIntFieldWidth));
    if (EditorGUI.EndChangeCheck()) {
  ...
  private void LinkOptionsOnGUI() {
    if (m_State.linkedWithSequencer) {
      if (GUILayout.Toggle(true, AnimationWindowStyles.sequencerLinkContent,
                           EditorStyles.toolbarButton) == false) {
  ...
  static void ExecuteShortcut(ShortcutArguments args,
                              Action<IAnimationWindowControl> exp) {
  ...
  static void ShowCurves(ShortcutArguments args) {
    ExecuteShortcut(
        args, animEditor => { animEditor.SwitchBetweenCurvesAndDopesheet(); });
  }
  ...
  static void PreviousFrame(ShortcutArguments args) {
    ExecuteShortcut(args,
                    controlInterface => controlInterface.GoToPreviousFrame());
  }

  [FormerlyPrefKeyAs("Animation/Previous Keyframe", "&,")]
  [Shortcut("Animation/Previous Keyframe", typeof(AnimationWindow),
            KeyCode.Comma, ShortcutModifiers.Alt)]
  ...
  [Shortcut("Animation/Next Keyframe", typeof(AnimationWindow), KeyCode.Period,
            ShortcutModifiers.Alt)]
  static void NextKeyFrame(ShortcutArguments args) {
    ExecuteShortcut(args,
                    controlInterface => controlInterface.GoToNextKeyframe());
  }

  [FormerlyPrefKeyAs("Animation/First Keyframe", "#,")]
  [Shortcut("Animation/First Keyframe", typeof(AnimationWindow), KeyCode.Comma,
            ShortcutModifiers.Shift)]
  static void FirstKeyFrame(ShortcutArguments args) {
    ExecuteShortcut(args,
                    controlInterface => controlInterface.GoToFirstKeyframe());
  }

  [FormerlyPrefKeyAs("Animation/Last Keyframe", "#.")]
  [Shortcut("Animation/Last Keyframe", typeof(AnimationWindow), KeyCode.Period,
            ShortcutModifiers.Shift)]
  static void LastKeyFrame(ShortcutArguments args) {
    ExecuteShortcut(args,
                    controlInterface => controlInterface.GoToLastKeyframe());
  }
  ...
  static void KeySelected(ShortcutArguments args) {
    AnimationWindow animationWindow =
        AnimationWindow.GetAllAnimationWindows().Find(
            aw =>(aw.state.previewing || aw == EditorWindow.focusedWindow));
    ...
    AnimationWindowUtility.AddSelectedKeyframes(
        animEditor.m_State, animEditor.controlInterface.time);
  ...
  [Shortcut("Animation/Toggle Ripple", typeof(AnimationWindow), KeyCode.Alpha2,
            ShortcutModifiers.Shift)]
  static void ToggleRipple(ShortcutArguments args) {
    ExecuteShortcut(args, animEditor => {
      animEditor.state.rippleTime = !animEditor.state.rippleTime;
    });
  }

  [ClutchShortcut("Animation/Ripple (Clutch)", typeof(AnimationWindow),
                  KeyCode.Alpha2)]
  static void ClutchRipple(ShortcutArguments args) {
    ExecuteShortcut(args, animEditor => {
      animEditor.state.rippleTimeClutch = args.stage == ShortcutStage.Begin;
    });
  }
  ...
  private void PlayButtonOnGUI() {
    EditorGUI.BeginChangeCheck();
    bool playbackEnabled = GUILayout.Toggle(controlInterface.playing,
                                            AnimationWindowStyles.playContent,
                                            EditorStyles.toolbarButton);
    if (EditorGUI.EndChangeCheck()) {
      if (playbackEnabled)
        controlInterface.StartPlayback();
  ...
  private void PreviewButtonOnGUI() {
    ...
    bool recordingEnabled = GUILayout.Toggle(
        controlInterface.previewing, AnimationWindowStyles.previewContent,
        EditorStyles.toolbarButton);
    if (EditorGUI.EndChangeCheck()) {
      if (recordingEnabled)
  ...
  private void RecordButtonOnGUI() {
    ...
    bool recordingEnabled = GUILayout.Toggle(
        controlInterface.recording, AnimationWindowStyles.recordContent,
        EditorStyles.toolbarButton);
    if (EditorGUI.EndChangeCheck()) {
      if (recordingEnabled)
        ...
        // Force refresh in inspector as stopping recording does not invalidate
        // any data.
  ...
  private void RenderSelectionOverlay(Rect rect) {
    ...
    Bounds bounds =
        m_State.showCurveEditor ? m_CurveEditor.selectionBounds
        : m_State.selectionBounds;
    ...
    AnimationWindowUtility.DrawSelectionOverlay(rect, selectionRangeColor,
                                                startPixel, endPixel);
  }
  ...
  private void RenderInRangeOverlay(Rect rect) {
    ...
    AnimationWindowUtility.DrawInRangeOverlay(
        rect, color, m_State.TimeToPixel(timeRange.x) + rect.xMin,
        m_State.TimeToPixel(timeRange.y) + rect.xMin);
  }
  ...
  private void RenderOutOfRangeOverlay(Rect rect) {
    ...
    Vector2 timeRange = m_State.timeRange;
    AnimationWindowUtility.DrawOutOfRangeOverlay(
        rect, color, m_State.TimeToPixel(timeRange.x) + rect.xMin,
        m_State.TimeToPixel(timeRange.y) + rect.xMin);
  }

  private void SynchronizeLayout() {
    m_HorizontalSplitter.realSizes[1] =
        (int) Mathf.Min(m_Position.width - m_HorizontalSplitter.realSizes[0],
                        m_HorizontalSplitter.realSizes[1]);
  ...
  // Curve editor changes curves, but we are in charge of saving them into the
  // clip
    ...
    for (int i = 0; i < m_CurveEditor.animationCurves.Length; ++i) {
      ...
      if (curveWrapper.changed) {
        ...
        if (curveWrapper.animationClip != null) {
          if (curvesToUpdate.TryGetValue(curveWrapper.animationClip,
                                         out changedCurves)) {
            changedCurves.bindings.Add(curveWrapper.binding);
            changedCurves.curves.Add(
                curveWrapper.curve.length > 0 ? curveWrapper.curve : null);
          } else {
    ...
    if (curvesToUpdate.Count > 0) {
      foreach (var kvp in curvesToUpdate) {
        Undo.RegisterCompleteObjectUndo(
            kvp.Key, AnimationWindowState.kEditCurveUndoLabel);
        AnimationWindowUtility.SaveCurves(kvp.Key, kvp.Value.bindings,
                                          kvp.Value.curves);
      }
  ...
  private void UpdateSelectedKeysFromCurveEditor() {
    ...
    foreach (CurveSelection curveSelection in m_CurveEditor.selectedCurves) {
      AnimationWindowKeyframe keyFrame =
          AnimationWindowUtility.CurveSelectionToAnimationWindowKeyframe(
              curveSelection, m_State.allCurves);
      if (keyFrame != null)
  ...
  private void UpdateSelectedKeysToCurveEditor() {
    ...
    foreach (AnimationWindowKeyframe keyframe in m_State.selectedKeys) {
      CurveSelection curveSelection =
          AnimationWindowUtility.AnimationWindowKeyframeToCurveSelection(
              keyframe, m_CurveEditor);
  ...
  private void SaveCurveEditorKeySelection() {
    // Synchronize current selection in curve editor and save selection snapshot
    // in undo redo.
    if (m_State.showCurveEditor)
      UpdateSelectedKeysFromCurveEditor();
    else
  ...
  public void EndKeyModification() { UpdateSelectedKeysToCurveEditor(); }
  ...
  private void InitializeCurveEditor() {
    ...
    settings.hTickStyle.distMin = 30; // min distance between vertical lines
                                      // before they disappear completely
    settings.hTickStyle.distFull =
        80; // distance between vertical lines where they gain full strength
    settings.hTickStyle.distLabel =
        0; // min distance between vertical lines labels
    if (EditorGUIUtility.isProSkin) {
      settings.vTickStyle.tickColor.color =
          new Color(1, 1, 1,
                    settings.vTickStyle.tickColor.color
                        .a); // color and opacity of horizontal lines
      settings.vTickStyle.labelColor.color =
          new Color(1, 1, 1,
                    settings.vTickStyle.labelColor.color
                        .a); // color and opacity of horizontal line labels
    }
    settings.vTickStyle.distMin = 15; // min distance between horizontal lines
                                      // before they disappear completely
    settings.vTickStyle.distFull =
        40; // distance between horizontal lines where they gain full strength
    settings.vTickStyle.distLabel =
        30; // min distance between horizontal lines labels
    ...
    settings.rectangleToolFlags =
        CurveEditorSettings.RectangleToolFlags.FullRectangleTool;
    settings.undoRedoSelection = true;
    settings.flushCurveCache =
        false; // Curve Wrappers are cached in AnimationWindowState.
  ...
  private void InitializeHorizontalSplitter() {
    m_HorizontalSplitter = SplitterState.FromRelative(
        new float[]{kHierarchyMinWidth, kHierarchyMinWidth * 3},
        new float[]{kHierarchyMinWidth, kHierarchyMinWidth}, null);
    m_HorizontalSplitter.realSizes[0] = kHierarchyMinWidth;
    m_HorizontalSplitter.realSizes[1] = kHierarchyMinWidth;
  }

  // Called once during initialization of m_State
  private void InitializeOverlay() { m_Overlay = new AnimEditorOverlay(); }

  // Called during initialization, even when m_State already exists
  private void InitializeNonserializedValues() {
    // Since CurveEditor doesn't know about AnimationWindowState, a delegate
    // allows it to reflect frame rate changes
