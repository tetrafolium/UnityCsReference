...
public Rect contentRect {
	get {
		return new Rect(0, 0, m_TimeArea.drawRect.width,
...
public bool CalculateScaleTimeMatrix(float fromTime, float toTime,
                                     float offsetTime, float pivotTime,
                                     float frameRate, out Matrix4x4 transform,
                                     out bool flipKeys) {
	transform = Matrix4x4.identity;
	flipKeys = false;

	float thresholdTime =
		(Mathf.Approximately(frameRate, 0f)) ? 0.001f : 1f / frameRate;
	...
	dtNum = (Mathf.Sign(dtNum) == Mathf.Sign(dtDenum)) ? dtNum - offsetTime
	                                               : dtNum + offsetTime;

	if (Mathf.Approximately(dtDenum, 0f)) {
		transform.SetTRS(new Vector3(dtNum, 0f, 0f), Quaternion.identity,
		                 Vector3.one);
		flipKeys = false;
	...
	transform.SetTRS(new Vector3(pivotTime, 0f, 0f), Quaternion.identity,
	                 Vector3.one);
	transform = transform * Matrix4x4.TRS(Vector3.zero, Quaternion.identity,
	                                      new Vector3(scaleTime, 1f, 1f));
	transform = transform * Matrix4x4.TRS(new Vector3(-pivotTime, 0f),
...
public bool CalculateScaleValueMatrix(float fromValue, float toValue,
                                      float offsetValue, float pivotValue,
	...
	dvNum = (Mathf.Sign(dvNum) == Mathf.Sign(dvDenum)) ? dvNum - offsetValue
	                                               : dvNum + offsetValue;

	if (Mathf.Approximately(dvDenum, 0f)) {
		transform.SetTRS(new Vector3(0f, dvNum, 0f), Quaternion.identity,
		                 Vector3.one);
		flipKeys = false;
	...
	transform.SetTRS(new Vector3(0f, pivotValue, 0f), Quaternion.identity,
	                 Vector3.one);
	transform = transform * Matrix4x4.TRS(Vector3.zero, Quaternion.identity,
	                                      new Vector3(1f, scaleValue, 1f));
	transform = transform * Matrix4x4.TRS(new Vector3(0f, -pivotValue, 0f),
	                                      Quaternion.identity, Vector3.one);
...
public float PixelToTime(float pixelTime, float frameRate) {
	...
	float visibleTimeSpan =
		m_TimeArea.shownArea.xMax - m_TimeArea.shownArea.xMin;
