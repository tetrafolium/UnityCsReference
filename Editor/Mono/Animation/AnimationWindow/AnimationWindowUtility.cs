...
static internal class AnimationWindowUtility {
  public static void SaveCurve(AnimationClip clip, AnimationWindowCurve curve) {
    ...
    if (curve.isPPtrCurve) {
      ...
      AnimationUtility.SetObjectReferenceCurve(clip, curve.binding,
                                               objectCurve);
    } else {
  ...
  public static void SaveCurves(AnimationClip clip,
                                IEnumerable<AnimationWindowCurve> curves) {
    foreach (AnimationWindowCurve curve in curves) {
      ...
      if (curve.isPPtrCurve) {
        ...
        AnimationUtility.SetObjectReferenceCurveNoSync(clip, curve.binding,
                                                       objectCurve);
      } else {
        AnimationCurve animationCurve = curve.ToAnimationCurve();
        ...
        AnimationUtility.SetEditorCurveNoSync(clip, curve.binding,
                                              animationCurve);
      }
    }
  ...
  public static void SaveCurves(AnimationClip clip,
                                IList<EditorCurveBinding> bindings,
                                IList<AnimationCurve> curves) {
    if (bindings.Count != curves.Count)
      throw new ArgumentException(
          "bindings and curves array sizes do not match");
  ...
  public static void CreateDefaultCurves(AnimationWindowState state,
                                         EditorCurveBinding[] properties) {
  ...
  public static AnimationWindowCurve
  CreateDefaultCurve(AnimationWindowState state, AnimationClip animationClip,
                     EditorCurveBinding binding) {
    Type type = state.selection.GetEditorCurveValueType(binding);

    AnimationWindowCurve curve =
        new AnimationWindowCurve(animationClip, binding, type);

    object currentValue = CurveBindingUtility.GetCurrentValue(
        state.activeRootGameObject, binding);
    if (animationClip.length == 0.0F) {
      AddKeyframeToCurve(curve, currentValue, type,
                         AnimationKeyTime.Time(0.0F, animationClip.frameRate));
    } else {
      AddKeyframeToCurve(curve, currentValue, type,
                         AnimationKeyTime.Time(0.0F, animationClip.frameRate));
      AddKeyframeToCurve(
          curve, currentValue, type,
          AnimationKeyTime.Time(animationClip.length, animationClip.frameRate));
    }
  ...
  public static bool
  ShouldShowAnimationWindowCurve(EditorCurveBinding curveBinding) {
    // We don't want to convert the w component of rotation curves to be shown
    // in animation window
  ...
  public static bool IsNodeLeftOverCurve(AnimationWindowHierarchyNode node) {
    if (node.binding != null) {
      if (node.curves.Length > 0) {
        AnimationWindowSelectionItem selectionBinding =
            node.curves[0].selectionBinding;
        if (selectionBinding != null) {
          if (selectionBinding.rootGameObject == null &&
              selectionBinding.scriptableObject == null)
            return false;

          return selectionBinding.GetEditorCurveValueType(
                     (EditorCurveBinding) node.binding) == null;
        }
      }
    }
  ...
  public static bool IsNodeAmbiguous(AnimationWindowHierarchyNode node) {
    if (node.binding != null) {
      if (node.curves.Length > 0) {
        ...
        if (selectionBinding != null) {
          if (selectionBinding.rootGameObject != null)
            return AnimationUtility.AmbiguousBinding(
                node.binding.Value.path, node.binding.Value.m_ClassID,
                selectionBinding.rootGameObject.transform);
        }
      }
    }
  ...
  public static void AddSelectedKeyframes(AnimationWindowState state,
                                          AnimationKeyTime time) {
    List<AnimationWindowCurve> curves =
        state.activeCurves.Count > 0 ? state.activeCurves : state.allCurves;
    AddKeyframes(state, curves, time);
  }

  public static void AddKeyframes(AnimationWindowState state,
                                  IList<AnimationWindowCurve> curves,
                                  AnimationKeyTime time) {
    ...
    foreach (AnimationWindowCurve curve in curves) {
      ...
      AnimationKeyTime shiftedMouseKeyTime =
          AnimationKeyTime.Time(time.time, time.frameRate);

      object value = CurveBindingUtility.GetCurrentValue(state, curve);
      AnimationWindowKeyframe keyframe =
          AnimationWindowUtility.AddKeyframeToCurve(
              curve, value, curve.valueType, shiftedMouseKeyTime);
  ...
  public static void RemoveKeyframes(AnimationWindowState state,
                                     IList<AnimationWindowCurve> curves,
                                     AnimationKeyTime time) {
  ...
  public static AnimationWindowKeyframe
  AddKeyframeToCurve(AnimationWindowCurve curve, object value, Type type,
                     AnimationKeyTime time) {
    ...
    if (curve.isPPtrCurve) {
      ...
      if (type == typeof(bool)) {
        AnimationUtility.SetKeyLeftTangentMode(ref tempKey,
                                               TangentMode.Constant);
        AnimationUtility.SetKeyRightTangentMode(ref tempKey,
                                                TangentMode.Constant);
        ...
        if (animationCurve.length <= 1) {
          TangentMode tangentMode =
              curve.isDiscreteCurve ? TangentMode.Constant : TangentMode.Linear;
        ...
        int keyIndex = animationCurve.AddKey(tempKey);
        if (keyIndex != -1) {
          // Make sure tangent slopes default to ClampedAuto.  Tangent mode will
          // be modified afterwards.
          AnimationUtility.SetKeyLeftTangentMode(animationCurve, keyIndex,
                                                 TangentMode.ClampedAuto);
          AnimationUtility.SetKeyRightTangentMode(animationCurve, keyIndex,
                                                  TangentMode.ClampedAuto);
          AnimationUtility.UpdateTangentsFromModeSurrounding(animationCurve,
                                                             keyIndex);
  ...
  public static List<AnimationWindowCurve>
  FilterCurves(AnimationWindowCurve[] curves, string path,
               bool entireHierarchy) {
    List<AnimationWindowCurve> results = new List<AnimationWindowCurve>();

    if (curves != null) {
      foreach (AnimationWindowCurve curve in curves)
        if (curve.path.Equals(path) ||
            (entireHierarchy && curve.path.Contains(path)))
          results.Add(curve);
    }
  ...
  public static bool IsCurveCreated(AnimationClip clip,
                                    EditorCurveBinding binding) {
  ...
  HasOtherRotationCurve(AnimationClip clip,
  ...
  internal static bool IsRotationCurve(EditorCurveBinding curveBinding) {
    ...
    return propertyName == "m_LocalRotation" ||
           propertyName == "localEulerAnglesRaw";
  }

  public static bool IsRectTransformPosition(EditorCurveBinding curveBinding) {
    return curveBinding.type ==
           typeof(RectTransform) &&GetPropertyGroupName(
               curveBinding.propertyName) == "m_LocalPosition";
  }

  public static bool
  ContainsFloatKeyframes(List<AnimationWindowKeyframe> keyframes) {
  ...
  public static List<AnimationWindowCurve>
  FilterCurves(AnimationWindowCurve[] curves, string path,
               Type animatableObjectType, string propertyName) {
    List<AnimationWindowCurve> results = new List<AnimationWindowCurve>();
    ...
    if (curves != null) {
      ...
      foreach (AnimationWindowCurve curve in curves) {
        bool propertyNameMatches =
            isPropertyGroup ? GetPropertyGroupName(curve.propertyName)
                .Equals(propertyGroupName)
            : curve.propertyName.Equals(propertyName);
        if (curve.path.Equals(path) && curve.type == animatableObjectType &&
            propertyNameMatches)
          results.Add(curve);
      }
  ...
  public static object GetCurrentValue(GameObject rootGameObject,
                                       EditorCurveBinding curveBinding) {
    if (curveBinding.isPPtrCurve) {
      Object value;
      AnimationUtility.GetObjectReferenceValue(rootGameObject, curveBinding,
                                               out value);
      return value;
    } else {
      float value;
  ...
  public static List<EditorCurveBinding>
  GetAnimatableProperties(GameObject gameObject, GameObject root,
                          Type valueType) {
    EditorCurveBinding[] animatable =
        AnimationUtility.GetAnimatableBindings(gameObject, root);
  ...
  public static List<EditorCurveBinding>
  GetAnimatableProperties(GameObject gameObject, GameObject root,
                          Type objectType, Type valueType) {
    EditorCurveBinding[] animatable =
        AnimationUtility.GetAnimatableBindings(gameObject, root);

    List<EditorCurveBinding> result = new List<EditorCurveBinding>();
    foreach (EditorCurveBinding binding in animatable)
      if (binding.type == objectType &&
          AnimationUtility.GetEditorCurveValueType(root, binding) == valueType)
        result.Add(binding);
  ...
  public static List<EditorCurveBinding>
  GetAnimatableProperties(ScriptableObject scriptableObject, Type valueType) {
    EditorCurveBinding[] animatable =
        AnimationUtility.GetAnimatableBindings(scriptableObject);

    List<EditorCurveBinding> result = new List<EditorCurveBinding>();
    foreach (EditorCurveBinding binding in animatable)
      if (AnimationUtility.GetEditorCurveValueType(scriptableObject, binding) ==
          valueType)
        result.Add(binding);
  ...
  public static bool PropertyIsAnimatable(Object targetObject,
                                          string propertyPath,
                                          Object rootObject) {
    if (targetObject is ScriptableObject) {
      ScriptableObject scriptableObject = (ScriptableObject) targetObject;
      EditorCurveBinding[] allCurveBindings =
          AnimationUtility.GetAnimatableBindings(scriptableObject);
      return Array.Exists(allCurveBindings,
                          binding => binding.propertyName == propertyPath);
    } else {
      ...
      if (gameObject != null) {
        ...
        return AnimationUtility.PropertyModificationToEditorCurveBinding(
                   dummyModification, rootObject == null ? gameObject
                   : (GameObject) rootObject, out binding) != null;
      }
    }
  ...
  public static PropertyModification
      [] SerializedPropertyToPropertyModifications(
    ...
    if (property.hasChildren) {
      ...
      while (iter.Next(true) && !SerializedProperty.EqualContents(iter, end) &&
             iter.propertyPath.StartsWith(property.propertyPath)) {
    ...
    if (property.propertyPath.StartsWith("m_LocalRotation")) {
      ...
      if (serializedObject.targetObject is Transform) {
        SerializedProperty eulerHintProperty =
            serializedObject.FindProperty("m_LocalEulerAnglesHint");
        if (eulerHintProperty != null && eulerHintProperty.hasChildren) {
          ...
          while (iter.Next(true) &&
                 ...
                 iter.propertyPath.StartsWith(eulerHintProperty.propertyPath)) {
            properties.Add(iter.Copy());
          }
        }
      }
    ...
    for (int i = 0; i < properties.Count; ++i) {
      ...
      var isObject =
          propertyIter.propertyType == SerializedPropertyType.ObjectReference;
      ...
      if (isObject || isFloat || isBool || isInt || isEnum) {
        ...
        if (propertyIter.hasMultipleDifferentValues) {
          for (int j = 0; j < targetObjects.Length; ++j) {
            ...
            SerializedProperty singleProperty =
                singleObject.FindProperty(propertyIter.propertyPath);
        ...
        // fast path
          ...
          if (isObject)
            ...
            value =
                propertyIter.floatValue.ToString(CultureInfo.InvariantCulture);
          else if (isInt)
  ...
  public static EditorCurveBinding[] PropertyModificationsToEditorCurveBindings(
      PropertyModification[] modifications, GameObject rootGameObject,
      AnimationClip animationClip) {
    ...
    for (int i = 0; i < modifications.Length; ++i) {
      ...
      if (AnimationUtility.PropertyModificationToEditorCurveBinding(
              modifications[i], rootGameObject, out binding) != null) {
        EditorCurveBinding[] additionalBindings =
            RotationCurveInterpolation.RemapAnimationBindingForAddKey(
                binding, animationClip);
  ...
  public static EditorCurveBinding[] SerializedPropertyToEditorCurveBindings(
      SerializedProperty property, GameObject rootGameObject,
      AnimationClip animationClip) {
    PropertyModification[] modifications =
        AnimationWindowUtility.SerializedPropertyToPropertyModifications(
            property);
    return PropertyModificationsToEditorCurveBindings(
        modifications, rootGameObject, animationClip);
  }

  public static bool CurveExists(EditorCurveBinding binding,
  ...
  public static EditorCurveBinding GetRenamedBinding(EditorCurveBinding binding,
                                                     string newPath) {
  ...
  public static void RenameCurvePath(AnimationWindowCurve curve,
    ...
    if (curve.isPPtrCurve) {
      ...
      // Add new curve
      AnimationUtility.SetObjectReferenceCurve(clip, newBinding,
                                               curve.ToObjectCurve());
    } else {
      // Delete old curve
      ...
      // Add new curve
      AnimationUtility.SetEditorCurve(clip, newBinding,
  ...
  public static string GetPropertyDisplayName(string propertyName) {
    ...
    propertyName =
        propertyName.Replace("m_Materials.Array.data", "Material Reference");
  ...
  public static bool ShouldPrefixWithTypeName(Type animatableObjectType,
                                              string propertyName) {
    if (animatableObjectType == typeof(Transform) ||
        animatableObjectType == typeof(RectTransform))
      return false;

    if (animatableObjectType == typeof(SpriteRenderer) &&propertyName ==
        "m_Sprite")
      return false;
  ...
  public static string GetNicePropertyDisplayName(Type animatableObjectType,
                                                  string propertyName) {
    if (ShouldPrefixWithTypeName(animatableObjectType, propertyName))
      return ObjectNames.NicifyVariableName(animatableObjectType.Name) + "." +
  ...
  GetNicePropertyGroupDisplayName(Type animatableObjectType,
                                  string propertyGroupName) {
    if (ShouldPrefixWithTypeName(animatableObjectType, propertyGroupName))
      return ObjectNames.NicifyVariableName(animatableObjectType.Name) + "." +
             NicifyPropertyGroupName(animatableObjectType, propertyGroupName);
    else
  ...
  public static string NicifyPropertyGroupName(Type animatableObjectType,
                                               string propertyGroupName) {
    string result =
        GetPropertyGroupName(GetPropertyDisplayName(propertyGroupName));

    // Workaround for uGUI RectTransform which only animates position.z
    if (animatableObjectType ==
        typeof(RectTransform) &result.Equals("Position"))
      result = "Position (Z)";
  ...
  public static float GetNextKeyframeTime(AnimationWindowCurve[] curves,
                                          float currentTime, float frameRate) {
    AnimationKeyTime candidateKeyTime =
        AnimationKeyTime.Frame(int.MaxValue, frameRate);
    AnimationKeyTime time = AnimationKeyTime.Time(currentTime, frameRate);
    AnimationKeyTime nextTime =
        AnimationKeyTime.Frame(time.frame + 1, frameRate);
    bool found = false;

    foreach (AnimationWindowCurve curve in curves) {
      foreach (AnimationWindowKeyframe keyframe in curve.m_Keyframes) {
        AnimationKeyTime keyTime =
            AnimationKeyTime.Time(keyframe.time, frameRate);
        if (keyTime.frame <= candidateKeyTime.frame &&
            keyTime.frame >= nextTime.frame) {
  ...
  public static float GetPreviousKeyframeTime(AnimationWindowCurve[] curves,
                                              float currentTime,
                                              float frameRate) {
    AnimationKeyTime candidateKeyTime =
        AnimationKeyTime.Time(float.MinValue, frameRate);
    AnimationKeyTime time = AnimationKeyTime.Time(currentTime, frameRate);
    AnimationKeyTime previousTime =
        AnimationKeyTime.Frame(time.frame - 1, frameRate);
    ...
    foreach (AnimationWindowCurve curve in curves) {
      foreach (AnimationWindowKeyframe keyframe in curve.m_Keyframes) {
        AnimationKeyTime keyTime =
            AnimationKeyTime.Time(keyframe.time, frameRate);
        if (keyTime.frame >= candidateKeyTime.frame &&
            keyTime.frame <= previousTime.frame) {
  ...
  // Add animator, controller and clip to gameobject if they are missing to make
  // this gameobject animatable
  public static bool
  InitializeGameobjectForAnimation(GameObject animatedObject) {
    Component animationPlayer =
        GetClosestAnimationPlayerComponentInParents(animatedObject.transform);
    if (animationPlayer == null) {
      ...
      bool success =
          AddClipToAnimationPlayerComponent(animationPlayer, newClip);
  ...
  // Ensures that the gameobject or it's parents have an animation player
  // component. If not try to create one.
  public static Component
  EnsureActiveAnimationPlayer(GameObject animatedObject) {
    Component closestAnimator =
        GetClosestAnimationPlayerComponentInParents(animatedObject.transform);
  ...
  private static bool EnsureAnimationPlayerHasClip(Component animationPlayer) {
    ...
    if (AnimationUtility.GetAnimationClips(animationPlayer.gameObject).Length >
        0)
      return true;
    ...
    // End animation mode before adding or changing animation component to
    // object
  ...
  public static bool
  AddClipToAnimationPlayerComponent(Component animationPlayer,
  ...
  public static bool AddClipToAnimatorComponent(Animator animator,
                                                AnimationClip newClip) {
    UnityEditor.Animations.AnimatorController controller =
        UnityEditor.Animations.AnimatorController
            .GetEffectiveAnimatorController(animator);
    if (controller == null) {
      controller =
          UnityEditor.Animations.AnimatorController
              .CreateAnimatorControllerForClip(newClip, animator.gameObject);
      UnityEditor.Animations.AnimatorController.SetAnimatorController(
          animator, controller);
      ...
      ChildAnimatorState childAnimatorState =
          controller.layers[0].stateMachine.FindState(newClip.name);
      ...
      else if (childAnimatorState.state &&
               childAnimatorState.state.motion == null)
  ...
  public static bool AddClipToAnimationComponent(Animation animation,
                                                 AnimationClip newClip) {
  ...
  internal static AnimationClip CreateNewClip(string gameObjectName) {
    ...
    string message = string.Format(
        "Create a new animation for the game object '{0}':", gameObjectName);
    ...
    string newClipPath = EditorUtility.SaveFilePanelInProject(
        "Create New Animation", "New Animation", "anim", message,
        newClipDirectory);
  ...
  // The clipPath parameter must be a full asset path ending with '.anim'. e.g.
  // "Assets/Animations/New Clip.anim" This function will overwrite existing
  // .anim files.
  internal static AnimationClip CreateNewClipAtPath(string clipPath) {
    ...
    AnimationClip asset =
        AssetDatabase.LoadMainAssetAtPath(clipPath) as AnimationClip;
  ...
  public static int GetPropertyNodeID(int setId, string path, System.Type type,
                                      string propertyName) {
    return (setId + path + type.Name + propertyName).GetHashCode();
  }

  // What is the first animation player component (Animator or Animation) when
  // recursing parent tree toward root
  public static Component
  GetClosestAnimationPlayerComponentInParents(Transform tr) {
    while (true) {
  ...
  // What is the first animation component when recursing parent tree toward
  // root
  ...
  public static void DrawInRangeOverlay(Rect rect, Color color,
                                        float startOfClipPixel,
                                        float endOfClipPixel) {
    // Rect shaded shape drawn inside range
    if (endOfClipPixel >= rect.xMin) {
      if (color.a > 0f) {
        Rect inRect =
            Rect.MinMaxRect(Mathf.Max(startOfClipPixel, rect.xMin), rect.yMin,
                            Mathf.Min(endOfClipPixel, rect.xMax), rect.yMax);
        DrawRect(inRect, color);
      }
    }
  }

  public static void DrawOutOfRangeOverlay(Rect rect, Color color,
                                           float startOfClipPixel,
                                           float endOfClipPixel) {
    ...
    if (startOfClipPixel > rect.xMin) {
      Rect startRect =
          Rect.MinMaxRect(rect.xMin, rect.yMin,
                          Mathf.Min(startOfClipPixel, rect.xMax), rect.yMax);
    ...
    Rect endRect = Rect.MinMaxRect(Mathf.Max(endOfClipPixel, rect.xMin),
  ...
  public static void DrawSelectionOverlay(Rect rect, Color color,
                                          float startPixel, float endPixel) {
    startPixel = Mathf.Max(startPixel, rect.xMin);
    endPixel = Mathf.Max(endPixel, rect.xMin);

    Rect labelRect =
        Rect.MinMaxRect(startPixel, rect.yMin, endPixel, rect.yMax);
  ...
  private static CurveRenderer
  CreateRendererForCurve(AnimationWindowCurve curve) {
  ...
  private static CurveWrapper.PreProcessKeyMovement
  CreateKeyPreprocessorForCurve(AnimationWindowCurve curve) {
    CurveWrapper.PreProcessKeyMovement method;
    switch (System.Type.GetTypeCode(curve.valueType)) {
    case TypeCode.Int32:
      method =
          (ref Keyframe key) => { key.value = Mathf.Floor(key.value + 0.5f); };
      break;
    case TypeCode.Boolean:
      method =
          (ref Keyframe key) => { key.value = key.value > 0.5f ? 1.0f : 0.0f; };
      break;
    default:
      method = null;
      break;
  ...
  public static CurveWrapper GetCurveWrapper(AnimationWindowCurve curve,
                                             AnimationClip clip) {
    CurveWrapper curveWrapper = new CurveWrapper();
    curveWrapper.renderer = CreateRendererForCurve(curve);
    curveWrapper.preProcessKeyMovementDelegate =
        CreateKeyPreprocessorForCurve(curve);
    curveWrapper.renderer.SetWrap(WrapMode.Clamp, clip.isLooping ? WrapMode.Loop
                                  : WrapMode.Clamp);
  ...
  // Convert keyframe from curve editor representation (CurveSelection) to
  // animation window representation (AnimationWindowKeyframe)
  public static AnimationWindowKeyframe CurveSelectionToAnimationWindowKeyframe(
      CurveSelection curveSelection, List<AnimationWindowCurve> allCurves) {
  ...
  // Convert keyframe from animation window representation
  // (AnimationWindowKeyframe) to curve editor representation (CurveSelection)
  // to animation window representation (AnimationWindowKeyframe)
  public static CurveSelection
  AnimationWindowKeyframeToCurveSelection(AnimationWindowKeyframe keyframe,
  ...
  public static AnimationWindowCurve
  BestMatchForPaste(EditorCurveBinding binding,
                    List<AnimationWindowCurve> clipboardCurves,
                    List<AnimationWindowCurve> targetCurves) {
    // Exact match
    ...
    foreach (AnimationWindowCurve targetCurve in targetCurves) {
      if (targetCurve.binding.propertyName == binding.propertyName) {
        // Only match if key in binding is not already being pasted itself in
        // clipboardCurves.
        if (!clipboardCurves.Exists(clipboardCurve => clipboardCurve.binding ==
                                                      targetCurve.binding)) {
          return targetCurve;
        }
      }
  ...
  public static bool IsActualTransformCurve(EditorCurveBinding curveBinding) {
    return curveBinding.type == typeof(Transform) ||
           curveBinding.type ==
  ...
  public static bool ForceGrouping(EditorCurveBinding binding) {
    ...
    if (binding.type == typeof(RectTransform)) {
      ...
      return group == "m_LocalPosition" || group == "m_LocalScale" ||
             group == "m_LocalRotation" || group == "localEulerAnglesBaked" ||
             group == "localEulerAngles" || group == "localEulerAnglesRaw";
    }
  ...
  public static void ControllerChanged() {
    foreach (AnimationWindow animationWindow in AnimationWindow
                 .GetAllAnimationWindows())
