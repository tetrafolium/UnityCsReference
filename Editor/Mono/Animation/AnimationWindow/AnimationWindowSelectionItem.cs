...
internal class AnimationWindowSelectionItem
    : ScriptableObject,
      System.IEquatable<AnimationWindowSelectionItem>,
      ISelectionBinding {
  [SerializeField]
  private int m_Id;
  [SerializeField]
  ...
  public virtual ScriptableObject scriptableObject {
    ...
    set { m_ScriptableObject = value; }
  }

  public virtual Object sourceObject {
    get {
      return (gameObject != null) ? (Object) gameObject
  ...
  public virtual Component animationPlayer {
    get {
      if (gameObject != null)
        return AnimationWindowUtility
            .GetClosestAnimationPlayerComponentInParents(gameObject.transform);
      return null;
    }
  }
  ...
  public virtual bool animationIsEditable {
    get {
      ...
      if (animationClip &&
          (animationClip.hideFlags & HideFlags.NotEditable) != 0)
        return false;
  ...
  public virtual bool clipIsEditable {
    get {
      ...
      if ((animationClip.hideFlags & HideFlags.NotEditable) != 0)
        return false;
      if (!AssetDatabase.IsOpenForEdit(animationClip,
  ...
  public virtual bool canChangeAnimationClip {
    get { return rootGameObject != null; }
  }
    ...
    get {
      if (m_CurvesCache == null) {
        ...
        if (animationClip != null) {
          ...
          List<AnimationWindowCurve> transformCurves =
              new List<AnimationWindowCurve>();

          foreach (EditorCurveBinding curveBinding in curveBindings) {
            if (AnimationWindowUtility.ShouldShowAnimationWindowCurve(
                    curveBinding)) {
              AnimationWindowCurve curve = new AnimationWindowCurve(
                  animationClip, curveBinding,
                  GetEditorCurveValueType(curveBinding));
          ...
          foreach (EditorCurveBinding curveBinding in objectCurveBindings) {
            AnimationWindowCurve curve =
                new AnimationWindowCurve(animationClip, curveBinding,
                                         GetEditorCurveValueType(curveBinding));
            curve.selectionBinding = this;
        ...
        // Curves need to be sorted with path/type/property name so it's
        // possible to construct hierarchy from them Sorting logic in
        // AnimationWindowCurve.CompareTo()
        m_CurvesCache.Sort();
      }
  ...
  private void
  FillInMissingTransformCurves(List<AnimationWindowCurve> transformCurves,
                               ref List<AnimationWindowCurve> curvesCache) {
    ...
    foreach (var transformCurve in transformCurves) {
      ...
      if (transformBinding.path != lastBinding.path ||
          AnimationWindowUtility.GetPropertyGroupName(
              transformBinding.propertyName) !=
              AnimationWindowUtility.GetPropertyGroupName(
                  lastBinding.propertyName)) {
        propertyGroupName = AnimationWindowUtility.GetPropertyGroupName(
            lastBinding.propertyName);

        FillPropertyGroup(ref propertyGroup, lastBinding, propertyGroupName,
                          ref curvesCache);
    ...
    FillPropertyGroup(
        ref propertyGroup, lastBinding,
        AnimationWindowUtility.GetPropertyGroupName(lastBinding.propertyName),
        ref curvesCache);
  }

  private void FillPropertyGroup(ref EditorCurveBinding?[] propertyGroup,
                                 EditorCurveBinding lastBinding,
                                 string propertyGroupName,
                                 ref List<AnimationWindowCurve> curvesCache) {
    ...
    if (!propertyGroup[0].HasValue) {
      ...
      AnimationWindowCurve curve = new AnimationWindowCurve(
          animationClip, newBinding, GetEditorCurveValueType(newBinding));
      curve.selectionBinding = this;
  ...
  private void
  AssignBindingToRightSlot(EditorCurveBinding transformBinding,
                           ref EditorCurveBinding?[] propertyGroup) {
  ...
  public static AnimationWindowSelectionItem Create() {
    AnimationWindowSelectionItem selectionItem =
        CreateInstance(typeof(AnimationWindowSelectionItem))
            as AnimationWindowSelectionItem;
  ...
  public void ClearCache() { m_CurvesCache = null; }
  ...
  public bool Equals(AnimationWindowSelectionItem other) {
    return id == other.id && animationClip == other.animationClip &&
  ...
  public Type GetEditorCurveValueType(EditorCurveBinding curveBinding) {
    if (rootGameObject != null) {
      return AnimationUtility.GetEditorCurveValueType(rootGameObject,
                                                      curveBinding);
    } else if (scriptableObject != null) {
      return AnimationUtility.GetEditorCurveValueType(scriptableObject,
                                                      curveBinding);
    } else {
