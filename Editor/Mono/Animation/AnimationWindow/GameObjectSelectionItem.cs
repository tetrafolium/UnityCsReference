...
internal class GameObjectSelectionItem : AnimationWindowSelectionItem {
  public static GameObjectSelectionItem Create(GameObject gameObject) {
    GameObjectSelectionItem selectionItem =
        CreateInstance(typeof(GameObjectSelectionItem))
            as GameObjectSelectionItem;
    ...
    if (selectionItem.rootGameObject != null) {
      AnimationClip[] allClips =
          AnimationUtility.GetAnimationClips(selectionItem.rootGameObject);

      if (selectionItem.animationClip == null &&
          selectionItem.gameObject !=
              null) // there is activeGO but clip is still null
        selectionItem.animationClip = allClips.Length > 0 ? allClips[0] : null;
      else if (!Array.Exists(
                   allClips,
                   x => x ==
                        selectionItem.animationClip)) // clip doesn't belong to
                                                      // the currently active GO
        selectionItem.animationClip = allClips.Length > 0 ? allClips[0] : null;
    }
  ...
  public override void Synchronize() {
    if (rootGameObject != null) {
      AnimationClip[] allClips =
          AnimationUtility.GetAnimationClips(rootGameObject);
