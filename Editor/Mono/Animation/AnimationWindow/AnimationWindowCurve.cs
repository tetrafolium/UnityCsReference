...
internal class AnimationWindowCurve : IComparable<AnimationWindowCurve>,
  ...
  public bool isPPtrCurve {
    get { return m_Binding.isPPtrCurve; }
  }
  public bool isDiscreteCurve {
    get { return m_Binding.isDiscreteCurve; }
  }
  public bool isPhantom {
    get { return m_Binding.isPhantom; }
  }
  public string propertyName {
    get { return m_Binding.propertyName; }
  }
  public string path {
    get { return m_Binding.path; }
  }
  public System.Type type {
    get { return m_Binding.type; }
  }
  ...
  public int depth {
    get { return path.Length > 0 ? path.Split('/').Length : 0; }
  }
  ...
  public ScriptableObject scriptableObject {
    get {
      return m_SelectionBinding != null ? m_SelectionBinding.scriptableObject
          : null;
    }
  }
  ...
  public AnimationWindowCurve(AnimationClip clip, EditorCurveBinding binding,
                              System.Type valueType) {
    binding = RotationCurveInterpolation.RemapAnimationBindingForRotationCurves(
        binding, clip);
  ...
  public override int GetHashCode() {
    ...
    return unchecked(selectionID * 92821 ^ clipID * 19603 ^
                     GetBindingHashCode());
  }

  public int GetBindingHashCode() { return m_BindingHashCode; }
  ...
  public int CompareTo(AnimationWindowCurve obj) {
    ...
    bool sameTransformComponent =
        type == typeof(Transform) &&obj.type == typeof(Transform);
    bool oneIsTransformComponent =
        (type == typeof(Transform) || obj.type == typeof(Transform));

    // We want to sort position before rotation
    if (sameTransformComponent) {
      string propertyGroupA =
          AnimationWindowUtility.GetNicePropertyGroupDisplayName(
              typeof(Transform),
              AnimationWindowUtility.GetPropertyGroupName(propertyName));
      string propertyGroupB =
          AnimationWindowUtility.GetNicePropertyGroupDisplayName(
              typeof(Transform),
              AnimationWindowUtility.GetPropertyGroupName(obj.propertyName));

      if (propertyGroupA.Contains("Position") &&
          propertyGroupB.Contains("Rotation"))
        return -1;
      if (propertyGroupA.Contains("Rotation") &&
          propertyGroupB.Contains("Position"))
        return 1;
    }
    ...
    if (obj.type == type) {
      ...
      if (lhsIndex != -1 && rhsIndex != -1 &&
          propertyName.Substring(0, propertyName.Length - 2) ==
              obj.propertyName.Substring(0, obj.propertyName.Length - 2))
        return rhsIndex - lhsIndex;
    }

    return string.Compare((path + type + propertyName),
                          obj.path + obj.type + obj.propertyName,
  ...
  int ComparePaths(string otherPath) {
    ...
    for (int i = 0; i < smallerLength; ++i) {
      int compare =
          string.Compare(thisPath[i], objPath[i], StringComparison.Ordinal);
      if (compare == 0) {
        continue;
      }
  ...
  public AnimationCurve ToAnimationCurve() {
    ...
    for (int i = 0; i < length; i++) {
      // Make sure we don't get two keyframes in an exactly the same time. We
      // just ignore those.
      if (Mathf.Abs(m_Keyframes[i].time - lastFrameTime) >
          AnimationWindowCurve.timeEpsilon) {
        Keyframe newKeyframe = m_Keyframes [i]
                                   .ToKeyframe();
  ...
  public ObjectReferenceKeyframe[] ToObjectCurve() {
    ...
    for (int i = 0; i < length; i++) {
      ...
      if (Mathf.Abs(m_Keyframes[i].time - lastFrameTime) >
          AnimationWindowCurve.timeEpsilon) {
        ObjectReferenceKeyframe newKeyframe = m_Keyframes [i]
                                                  .ToObjectReferenceKeyframe();
  ...
  public void AddKeyframe(AnimationWindowKeyframe key,
  ...
  public void Clear() { m_Keyframes.Clear(); }
}
}
