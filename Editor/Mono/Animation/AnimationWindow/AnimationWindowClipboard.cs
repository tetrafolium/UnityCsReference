...
internal sealed class AnimationWindowEventClipboard {
  ...
  public AnimationWindowEventClipboard(AnimationEvent e) {
    ...
    objectParam = e.objectReferenceParameter ? e.objectReferenceParameter
                      .GetInstanceID() : 0;
  ...
  public static AnimationEvent FromClipboard(AnimationWindowEventClipboard e) {
    return new AnimationEvent{
        ...
        objectReferenceParameter =
            InternalEditorUtility.GetObjectFromInstanceID(e.objectParam),
...
internal class AnimationWindowEventsClipboard {
  ...
  internal static void CopyEvents(IList<AnimationEvent> allEvents,
                                  bool[] selected, int explicitIndex = -1) {
    ...
    // Else, only copy the clicked animation event
      ...
      copyEvents.Add(
          new AnimationWindowEventClipboard(allEvents[explicitIndex]));
    }
    var data =
        new AnimationWindowEventsClipboard{events = copyEvents.ToArray()};
    Clipboard.SetCustomValue(data);
  }

  internal static AnimationEvent[] AddPastedEvents(AnimationEvent[] events,
                                                   float time,
                                                   out bool[] selected) {
    selected = null;
