...
internal class MinMaxCurveEditorWindow : EditorWindow {
  ...
  [SerializeField] GUIView delegateView;

  public static MinMaxCurveEditorWindow instance {
    get {
      if (!s_SharedMinMaxCurveEditor)
        s_SharedMinMaxCurveEditor =
            ScriptableObject.CreateInstance<MinMaxCurveEditorWindow>();
  ...
  void Init(CurveEditorSettings settings) {
    m_CurveEditor =
        new CurveEditor(GetCurveEditorRect(), GetCurveWrapperArray(), true);
    ...
    m_CurveEditor.settings.rectangleToolFlags =
    ...
    // For each of horizontal and vertical axis, if we have a finite range for
    // that axis, use that range, otherwise use framing logic to determine shown
    // range for that axis.
    bool frameH = true;
    bool frameV = true;
    if (!float.IsNegativeInfinity(m_CurveEditor.settings.hRangeMin) &&
        !float.IsInfinity(m_CurveEditor.settings.hRangeMax)) {
      m_CurveEditor.SetShownHRangeInsideMargins(
          m_CurveEditor.settings.hRangeMin, m_CurveEditor.settings.hRangeMax);
      frameH = false;
    }
    if (!float.IsNegativeInfinity(m_CurveEditor.settings.vRangeMin) &&
        !float.IsInfinity(m_CurveEditor.settings.vRangeMax)) {
      m_CurveEditor.SetShownVRangeInsideMargins(
          m_CurveEditor.settings.vRangeMin, m_CurveEditor.settings.vRangeMax);
      frameV = false;
    }
  ...
  void InitCurvePresets() {
    if (m_CurvePresets == null) {
      ...
      AnimationCurve min = m_CurveEditor.animationCurves.Length > 1
                               ? m_CurveEditor.animationCurves[1].curve
                               : new AnimationCurve();

      // Selection callback for library window
      System.Action<DoubleCurve> presetSelectedCallback =
          delegate(DoubleCurve presetCurve) {
        var doubleCurve = new DoubleCurve(min, max, true);
        doubleCurve.minCurve.keys = CurveEditorWindow.GetNormalizedKeys(
            presetCurve.minCurve.keys, m_CurveEditor);
        ...
        doubleCurve.maxCurve.keys = CurveEditorWindow.GetNormalizedKeys(
            presetCurve.maxCurve.keys, m_CurveEditor);
      ...
      // We set the curve to save when showing the popup to ensure to scale the
      // current state of the curve
      m_CurvePresets = new DoubleCurvePresetsContentsForPopupWindow(
          new DoubleCurve(min, max, true), presetSelectedCallback);
      m_CurvePresets.InitIfNeeded();
      m_CurvePresets.GetPresetLibraryEditor().GetCurrentLib().useRanges = false;
    }
  }

  public static void SetCurves(SerializedProperty max, SerializedProperty min,
                               SerializedProperty multiplier, Color color) {
    instance.m_Color = color;

    if (max == null)
      instance.m_MaxCurve = null;
    else
      instance.m_MaxCurve = max.hasMultipleDifferentValues ? new AnimationCurve()
          : max.animationCurveValue;

    if (min == null)
      instance.m_MinCurve = null;
    else
      instance.m_MinCurve = min.hasMultipleDifferentValues ? new AnimationCurve()
          : min.animationCurveValue;
  ...
  void SetAxisUiScalarsCallback(Vector2 newAxisScalars) {
    ...
    // We must apply the changes as this is called outside of the OnGUI code and
  ...
  Vector2 GetAxisUiScalarsCallback() {
    ...
    if (m_MultiplierProperty.floatValue < 0) {
      m_MultiplierProperty.floatValue =
          Mathf.Abs(m_MultiplierProperty.floatValue);
      m_MultiplierProperty.serializedObject.ApplyModifiedProperties();
    }
  ...
  void DrawPresetSwatchArea() {
    GUI.Box(new Rect(0, position.height - k_PresetsHeight, position.width,
                     k_PresetsHeight),
            "", s_Styles.curveSwatchArea);
    Color curveColor = m_Color;
    curveColor.a *= 0.6f;
    float yPos = position.height - k_PresetsHeight +
                 (k_PresetsHeight - k_PresetSwatchHeight) * 0.5f;
    InitCurvePresets();
    ...
    if (curveLibrary != null) {
      ...
      for (int i = 0; i < curveLibrary.Count(); i++) {
        Rect swatchRect =
            new Rect(k_PresetSwatchMargin +
                         (k_PresetSwatchWidth + k_PresetSwatchSeperation) * i,
                     yPos, k_PresetSwatchWidth, k_PresetSwatchHeight);
        ...
        if (GUI.Button(swatchRect, guiContent, s_Styles.curveSwatch)) {
          ...
          max.keys = CurveEditorWindow.GetDenormalizedKeys(
              animCurve.maxCurve.keys, m_CurveEditor);
          max.postWrapMode = animCurve.maxCurve.postWrapMode;
          max.preWrapMode = animCurve.maxCurve.preWrapMode;

          if (min != null) {
            min.keys = CurveEditorWindow.GetDenormalizedKeys(
                animCurve.minCurve.keys, m_CurveEditor);
    ...
    // Dropdown
    Rect presetDropDownButtonRect =
        new Rect(k_PresetSwatchMargin - k_PresetsDropdownButtonSize,
                 yPos + k_PresetSwatchSeperation, k_PresetsDropdownButtonSize,
                 k_PresetsDropdownButtonSize);
    if (EditorGUI.DropdownButton(
            presetDropDownButtonRect, EditorGUI.GUIContents.titleSettingsIcon,
            FocusType.Passive, EditorStyles.inspectorTitlebarText)) {
      if (m_MaxCurve != null) {
        AnimationCurve max = m_CurveEditor.animationCurves[0].curve;
        AnimationCurve maxCopy = new AnimationCurve(
            CurveEditorWindow.GetNormalizedKeys(max.keys, m_CurveEditor));
        ...
        if (m_MinCurve != null) {
          ...
          minCopy = new AnimationCurve(
              CurveEditorWindow.GetNormalizedKeys(min.keys, m_CurveEditor));
          minCopy.postWrapMode = min.postWrapMode;
          minCopy.preWrapMode = min.preWrapMode;
        }

        m_CurvePresets.doubleCurveToSave =
            new DoubleCurve(minCopy, maxCopy, true);
  ...
  void OnGUI() {
    ...
    GUI.Label(m_CurveEditor.drawRect, GUIContent.none,
              s_Styles.curveEditorBackground);
    m_CurveEditor.OnGUI();
  ...
  public void UpdateCurve() { DoUpdateCurve(false); }

  void DoUpdateCurve(bool exitGUI) {
    bool minChanged = m_CurveEditor.animationCurves.Length > 0 &&
                      m_CurveEditor.animationCurves[0] != null &&
                      m_CurveEditor.animationCurves[0].changed;
    bool maxChanged = m_CurveEditor.animationCurves.Length > 1 &&
                      m_CurveEditor.animationCurves[1] != null &&
                      m_CurveEditor.animationCurves[1].changed;
