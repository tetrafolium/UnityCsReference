...
internal static class CurveUtility {
  ...
  public static bool HaveKeysInRange(AnimationCurve curve, float beginTime,
                                     float endTime) {
    // Loop backwards
  ...
  public static void RemoveKeysInRange(AnimationCurve curve, float beginTime,
                                       float endTime) {
  ...
  public static void SetKeyModeFromContext(AnimationCurve curve, int keyIndex) {
    ...
    if (keyIndex > 0) {
      ...
      TangentMode prevTangentMode =
          AnimationUtility.GetKeyRightTangentMode(curve[keyIndex - 1]);
      if (prevTangentMode == TangentMode.ClampedAuto ||
          prevTangentMode == TangentMode.Auto)
        smoothTangent = true;
    }
    if (keyIndex < curve.length - 1) {
      if (AnimationUtility.GetKeyBroken(curve[keyIndex + 1]))
        broken = true;
      TangentMode nextTangentMode =
          AnimationUtility.GetKeyLeftTangentMode(curve[keyIndex + 1]);
      if (nextTangentMode == TangentMode.ClampedAuto ||
          nextTangentMode == TangentMode.Auto)
        smoothTangent = true;
    }
    ...
    if (broken && !smoothTangent) {
      ...
      // Keys at boundaries.  Make sure left and right tangent modes are the
      // same.
      if (keyIndex == 0)
        AnimationUtility.SetKeyLeftTangentMode(
            ref key, AnimationUtility.GetKeyRightTangentMode(key));
      if (keyIndex == curve.length - 1)
        AnimationUtility.SetKeyRightTangentMode(
            ref key, AnimationUtility.GetKeyLeftTangentMode(key));
    } else {
      // If both neighbors or only neighbor are set to TangentMode.Auto or
      // TangentMode.ClampedAuto, set new key to this mode as well. If there are
      // no neighbors, set new key to TangentMode.ClampedAuto. Otherwise, fall
      // back to TangentMode.Free.
      TangentMode mode = TangentMode.Free;
      if ((keyIndex == 0 ||
           AnimationUtility.GetKeyRightTangentMode(curve[keyIndex - 1]) ==
               TangentMode.ClampedAuto) &&
          (keyIndex == curve.length - 1 ||
           AnimationUtility.GetKeyLeftTangentMode(curve[keyIndex + 1]) ==
               TangentMode.ClampedAuto)) {
        mode = TangentMode.ClampedAuto;
      } else if ((keyIndex == 0 ||
                  AnimationUtility.GetKeyRightTangentMode(
                      curve[keyIndex - 1]) == TangentMode.Auto) &&
                 (keyIndex == curve.length - 1 ||
                  AnimationUtility.GetKeyLeftTangentMode(curve[keyIndex + 1]) ==
                      TangentMode.Auto)) {
  ...
  public static Color GetBalancedColor(Color c) {
    return new Color(0.15f + 0.75f * c.r, 0.20f + 0.60f * c.g,
                     0.10f + 0.90f * c.b);
  }
  ...
  public static Color GetPropertyColor(string name) {
    ...
    if (type == 1) {
      ...
      if (name.EndsWith(".x"))
        col = AnimEditor.kEulerXColor;
      else if (name.EndsWith(".y"))
        col = AnimEditor.kEulerYColor;
      else if (name.EndsWith(".z"))
        col = AnimEditor.kEulerZColor;
    } else if (type == 3) {
      if (name.EndsWith(".x"))
        col = GetBalancedColor(new Color(0.7f, 0.4f, 0.4f));
      else if (name.EndsWith(".y"))
        col = GetBalancedColor(new Color(0.4f, 0.7f, 0.4f));
      else if (name.EndsWith(".z"))
        col = GetBalancedColor(new Color(0.4f, 0.4f, 0.7f));
      ...
      col = new Color(1.0f, 0.5f, 0.0f);
    else if (name.EndsWith(".r"))
      col = GetBalancedColor(Color.red);
    else if (name.EndsWith(".g"))
      col = GetBalancedColor(Color.green);
    else if (name.EndsWith(".b"))
      col = GetBalancedColor(Color.blue);
    else if (name.EndsWith(".a"))
      col = GetBalancedColor(Color.yellow);
