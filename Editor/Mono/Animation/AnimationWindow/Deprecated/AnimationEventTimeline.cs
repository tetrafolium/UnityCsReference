...
public EventLineContextMenuObject(GameObject animated, AnimationClip clip,
                                  float time, int index, bool[] selected) {
	m_Animated = animated;
	m_Clip = clip;
	m_Time = time;
	m_Index = index;
...
public void AddEvent(float time, GameObject gameObject,
                     AnimationClip animationClip) {
	AnimationWindowEvent awEvent =
		AnimationWindowEvent.CreateAndEdit(gameObject, animationClip, time);
	Selection.activeObject = awEvent;
}

public void EditEvents(GameObject gameObject, AnimationClip clip,
...
public void EditEvent(GameObject gameObject, AnimationClip clip, int index) {
	AnimationWindowEvent awEvent =
		AnimationWindowEvent.Edit(gameObject, clip, index);
	Selection.activeObject = awEvent;
}

public void ClearSelection() {
	// Do not unecessarily clear selection.  Only clear if selection already is
	// animation window event.
...
void CopyEvents(AnimationClip clip, bool[] selected, int explicitIndex = -1) {
	var allEvents =
		new List<AnimationEvent>(AnimationUtility.GetAnimationEvents(clip));
	AnimationWindowEventsClipboard.CopyEvents(allEvents, selected,
	                                          explicitIndex);
}
...
void PasteEvents(GameObject animated, AnimationClip clip, float time) {
	...
	var newEvents = AnimationWindowEventsClipboard.AddPastedEvents(
		oldEvents, time, out var selected);
	if (newEvents == null)
		return;
...
public void EventLineGUI(Rect rect, AnimationWindowState state) {
	...
	float mousePosTime =
		Mathf.Max(Mathf.RoundToInt(
				  state.PixelToTime(Event.current.mousePosition.x, rect) *
				  state.frameRate) /
		          state.frameRate,
		          0.0f);
	...
	if (clip != null) {
		...
		for (int i = 0; i < events.Length; i++) {
			...
			float keypos =
				Mathf.Floor(state.FrameToPixel(evt.time * clip.frameRate, rect));
			int sharedOffset = 0;
			if (shared > 1) {
				float spread = Mathf.Min(
					(shared - 1) * (eventMarker.width - 1),
					(int)(state.FrameDeltaToPixel(rect) - eventMarker.width * 2));
				sharedOffset = Mathf.FloorToInt(
					Mathf.Max(0, spread - (eventMarker.width - 1) * (sharedLeft)));
			}

			Rect r =
				new Rect(keypos + sharedOffset - eventMarker.width / 2,
		...
		if (m_DirtyTooltip) {
			if (m_HoverEvent >= 0 && m_HoverEvent < hitRects.Length) {
				m_InstantTooltipText = AnimationWindowEventInspector.FormatEvent(
					animated, events[m_HoverEvent]);
				m_InstantTooltipPoint = new Vector2(
					hitRects[m_HoverEvent].xMin +
					(int)(hitRects[m_HoverEvent].width / 2) + rect.x - 30,
					rect.yMax);
			}
		...
		HighLevelEvent hEvent = EditorGUIExt.MultiSelection(
			...
			ref selectedEvents, null, out clickedIndex, out offset,
		...
		if (hEvent != HighLevelEvent.None) {
			...
			case HighLevelEvent.DoubleClick:
				...
				if (clickedIndex != -1)
					...
					EventLineContextMenuAdd(new EventLineContextMenuObject(
									animated, clip, mousePosTime, -1, selectedEvents));
				break;
			case HighLevelEvent.Drag:
				for (int i = events.Length - 1; i >= 0; i--) {
					if (selectedEvents[i]) {
						...
						evt.time =
							m_EventTimes[i] + offset.x * state.PixelDeltaToTime(rect);
						evt.time = Mathf.Max(0.0F, evt.time);
						evt.time =
							Mathf.RoundToInt(evt.time * clip.frameRate) / clip.frameRate;
					}
				}
			...
			case HighLevelEvent.ContextClick:
				CreateContextMenu(animated, clip, events[clickedIndex].time,
				                  clickedIndex, selectedEvents);

				// Mouse may move while context menu is open - make sure instant
				// tooltip is handled
		...
		if (Event.current.type == EventType.ContextClick &&
		    eventLineRect.Contains(Event.current.mousePosition)) {
			Event.current.Use();
...
void CreateContextMenu(GameObject animatedGo, AnimationClip clip, float time,
                       int eventIndex, bool[] selectedEvents) {
	GenericMenu menu = new GenericMenu();
	var contextData = new EventLineContextMenuObject(
		animatedGo, clip, time, eventIndex, selectedEvents);
	var selectedCount = selectedEvents.Count(selected => selected);

	menu.AddItem(Styles.textAddEvent, false, EventLineContextMenuAdd,
	             contextData);
	if (selectedCount > 0 || eventIndex != -1) {
		menu.AddItem(selectedCount > 1 ? Styles.textDeleteEvents
		                     : Styles.textDeleteEvent,
		             false, EventLineContextMenuDelete, contextData);
		menu.AddItem(Styles.textCopyEvents, false, EventLineContextMenuCopy,
		             contextData);
	} else {
	...
	if (AnimationWindowEventsClipboard.CanPaste())
		menu.AddItem(Styles.textPasteEvents, false, EventLineContextMenuPaste,
		             contextData);
	else
...
public void DrawInstantTooltip(Rect position) {
	if (!string.IsNullOrEmpty(m_InstantTooltipText)) {
		...
		Rect rect = new Rect(m_InstantTooltipPoint.x - size.x * .5f,
		                     m_InstantTooltipPoint.y + 24, size.x, size.y);
		...
		rect = new Rect(m_InstantTooltipPoint.x - 33, m_InstantTooltipPoint.y, 7,
		                25);
		...
		for (int i = hitRects.Length - 1; i >= 0; i--) {
			if (hitRects [i]
				...
				if (m_HoverEvent != i) {
					...
					m_InstantTooltipPoint = new Vector2(
						hitRects[m_HoverEvent].xMin +
						(int)(hitRects[m_HoverEvent].width / 2) + eventLineRect.x,
						eventLineRect.yMax);
					m_DirtyTooltip = true;
				}
			}
