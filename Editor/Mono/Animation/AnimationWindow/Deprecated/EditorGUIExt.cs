...
internal class EditorGUIExt {
  ...
  static bool DoRepeatButton(Rect position, GUIContent content, GUIStyle style,
                             FocusType focusType) {
  ...
  public static void MinMaxScroller(Rect position, int id, ref float value,
                                    ref float size, float visualStart,
                                    float visualEnd, float startLimit,
                                    float endLimit, GUIStyle slider,
                                    GUIStyle thumb, GUIStyle leftButton,
    ...
    MinMaxSlider(sliderRect, ref value, ref size, newVisualStart, newVisualEnd,
                 newVisualStart, newVisualEnd, slider, thumb, horiz);
  ...
  class MinMaxSliderState {
    ...
    public int whereWeDrag = -1; // which part are we dragging? 0 = middle, 1 =
                                 // min, 2 = max, 3 = min trough, 4 = max trough
  }

  static MinMaxSliderState s_MinMaxSliderState;
  static int kFirstScrollWait = 250; // ms
  static int kScrollWait = 30;       // ms
  static System.DateTime s_NextScrollStepTime =
      System.DateTime.Now; // whatever but null

  // Mouse down position for
  private static Vector2 s_MouseDownPos = Vector2.zero;
  // Are we doing a drag selection (as opposed to when the mousedown was over a
  // selection rect)
  enum DragSelectionState { None, DragSelecting, Dragging }
  static DragSelectionState s_MultiSelectDragSelection =
      DragSelectionState.None;
  ...
  /// Make a double-draggable slider that will let you specify a range of
  /// values.
  /// @param position where to draw it
  /// @param value the current start position
  /// @param size the size of the covered range
  /// @param visualStart what is displayed as the start of the range. The user
  /// can drag beyond this, but the displays shows this as the limit. Set this
  /// to be the start of the relevant data.
  /// @param visualEnd what is displayed as the end of the range. The user can
  /// drag beyond this, but the displays shows this as the limit. Set this to be
  /// the end of the relevant data.
  /// @param startLimit what is the lowest possible value? The user can never
  /// slide beyond this in the minimum direction. If you don't want a limit, set
  /// it to -Mathf.Infinity
  /// @param endLimit what is the highes possible value? The user can never
  /// slide beyond this in the maximum direction. If you don't want a limit, set
  /// it to Mathf.Infinity
  public static void MinMaxSlider(Rect position, ref float value,
                                  ref float size, float visualStart,
                                  float visualEnd, float startLimit,
                                  float endLimit, GUIStyle slider,
                                  GUIStyle thumb, bool horiz) {
    DoMinMaxSlider(
        position,
        GUIUtility.GetControlID(s_MinMaxSliderHash, FocusType.Passive),
        ref value, ref size, visualStart, visualEnd, startLimit, endLimit,
        slider, thumb, horiz);
  }

  private static float ThumbSize(bool horiz, GUIStyle thumb) {
    if (horiz)
      return thumb.fixedWidth != 0 ? thumb.fixedWidth
                                   : thumb.padding.horizontal;
    return thumb.fixedHeight != 0 ? thumb.fixedHeight : thumb.padding.vertical;
  }

  internal static void DoMinMaxSlider(Rect position, int id, ref float value,
                                      ref float size, float visualStart,
                                      float visualEnd, float startLimit,
                                      float endLimit, GUIStyle slider,
                                      GUIStyle thumb, bool horiz) {
    ...
    float displaySize =
        Mathf.Clamp(value + size, minVisual, maxVisual) - displayValue;
    ...
    if (horiz) {
      float thumbHeight =
          (thumb.fixedHeight != 0 ? thumb.fixedHeight : contentRect.height);

      pixelsPerValue =
          (position.width - slider.padding.horizontal - thumbSize) /
          (maxVisual - minVisual);
      thumbRect = new Rect(
          (displayValue - minVisual) * pixelsPerValue + contentRect.x,
          contentRect.y, displaySize * pixelsPerValue + thumbSize, thumbHeight);
      thumbMinRect = new Rect(thumbRect.x, thumbRect.y, thumb.padding.left,
                              thumbRect.height);
      thumbMaxRect = new Rect(thumbRect.xMax - thumb.padding.right, thumbRect.y,
                              thumb.padding.right, thumbRect.height);
      mousePosition = evt.mousePosition.x - position.x;
    } else {
      float thumbWidth =
          (thumb.fixedWidth != 0 ? thumb.fixedWidth : contentRect.width);

      pixelsPerValue = (position.height - slider.padding.vertical - thumbSize) /
                       (maxVisual - minVisual);
      thumbRect =
          new Rect(contentRect.x,
                   (displayValue - minVisual) * pixelsPerValue + contentRect.y,
                   thumbWidth, displaySize * pixelsPerValue + thumbSize);
      thumbMinRect = new Rect(thumbRect.x, thumbRect.y, thumbRect.width,
                              thumb.padding.top);
      thumbMaxRect =
          new Rect(thumbRect.x, thumbRect.yMax - thumb.padding.bottom,
                   thumbRect.width, thumb.padding.bottom);
    ...
    case EventType.MouseDown:
      ...
      if (evt.button != 0 || !position.Contains(evt.mousePosition) ||
          minVisual - maxVisual == 0)
        return;
      if (state == null)
        state = s_MinMaxSliderState = new MinMaxSliderState();

      // These are required to be set whenever we grab hotcontrol, regardless of
      // if we actually drag or not. (case 585577)
      ...
      if (thumbRect.Contains(evt.mousePosition)) {
        ...
        if (size != 0 && usePageScrollbars) {
          ...
          s_NextScrollStepTime =
              System.DateTime.Now.AddMilliseconds(kFirstScrollWait);
          ...
          if (horiz)
            value = ((float) mousePosition - thumbRect.width * .5f) /
                        pixelsPerValue +
                    minVisual - size * .5f;
          else
            value = ((float) mousePosition - thumbRect.height * .5f) /
                        pixelsPerValue +
                    minVisual - size * .5f;
          state.dragStartPos = mousePosition;
          state.dragStartValue = value;
    ...
    case EventType.MouseDrag:
      ...
      // Recalculate the value from the mouse position. this has the side effect
      // that values are relative to the click point - no matter where inside
      // the trough the original value was. Also means user can get back
      // original value if he drags back to start position.
      float deltaVal =
          (mousePosition - state.dragStartPos) / state.dragStartValuesPerPixel;
      switch (state.whereWeDrag) {
      case 0: // normal drag
        value = Mathf.Clamp(state.dragStartValue + deltaVal, minLimit,
                            maxLimit - size);
        break;
    ...
    case EventType.Repaint:
      ...
      EditorGUIUtility.AddCursorRect(
          thumbMinRect, horiz ? MouseCursor.ResizeHorizontal
          : MouseCursor.ResizeVertical,
            state != null && state.whereWeDrag == 1 ? id : -1);
      EditorGUIUtility.AddCursorRect(
          thumbMaxRect, horiz ? MouseCursor.ResizeHorizontal
          : MouseCursor.ResizeVertical,
            state != null && state.whereWeDrag == 2 ? id : -1);
      ...
      if (thumbRect.Contains(evt.mousePosition)) {
        if (state != null &&
            (state.whereWeDrag == 3 ||
             state.whereWeDrag ==
                 4)) // if was scrolling with "through" and the thumb reached
                     // mouse - sliding action over
          GUIUtility.hotControl = 0;
        return;
      }
  ...
  public static bool DragSelection(Rect[] positions, ref bool[] selections,
                                   GUIStyle style) {
      ...
      if (GUIUtility.hotControl == id) {
        if (evt.button == 0) {
          if (selectedIndex < 0) {
            ...
            Rect dummyRect = new Rect(positions[0].x, positions[0].y - 200,
                                      positions[0].width, 200);
  ...
  public static HighLevelEvent
  MultiSelection(Rect rect, Rect[] positions, GUIContent content,
                 Rect[] hitPositions, ref bool[] selections, bool[] readOnly,
                 out int clickedIndex, out Vector2 offset,
                 out float startSelect, out float endSelect, GUIStyle style) {
  ...
  static int GetIndexUnderMouse(Rect[] hitPositions, bool[] readOnly) {
    ...
    for (int i = hitPositions.Length - 1; i >= 0; i--)
      if ((readOnly == null || !readOnly[i]) && hitPositions [i]
                                                    .Contains(mousePos))
        return i;
  ...
  // Small helper: Make a rect from MinMax values and make sure they're positive
  // sizes
