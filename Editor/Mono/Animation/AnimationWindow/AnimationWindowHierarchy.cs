...
internal class AnimationWindowHierarchyState : TreeViewState {
  ...
  public int GetTallInstancesCount() { return m_TallInstanceIDs.Count; }
...
internal class AnimationWindowHierarchy {
  ...
  public Vector2 GetContentSize() { return m_TreeView.GetContentSize(); }

  public Rect GetTotalRect() { return m_TreeView.GetTotalRect(); }

  public AnimationWindowHierarchy(AnimationWindowState state,
                                  EditorWindow owner, Rect position) {
    m_State = state;
    Init(owner, position);
  }
  ...
  public void Init(EditorWindow owner, Rect rect) {
    ...
    m_State.hierarchyData =
        new AnimationWindowHierarchyDataSource(m_TreeView, m_State);
    m_TreeView.Init(rect, m_State.hierarchyData,
                    new AnimationWindowHierarchyGUI(m_TreeView, m_State), null);
  ...
  virtual internal bool IsRenamingNodeAllowed(TreeViewItem node) {
    return true;
  }
