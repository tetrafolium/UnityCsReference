...
internal class CurveEditorWindow : EditorWindow {
  ...
  public static CurveEditorWindow instance {
    get {
      if (!s_SharedCurveEditor)
        s_SharedCurveEditor =
            ScriptableObject.CreateInstance<CurveEditorWindow>();
  ...
  void Init(CurveEditorSettings settings) {
    ...
    // As there is no guarantee animation curve changes are recorded in undo
    // redo, we can't really handle curve selection in undo redo either.
    ...
    bool frameH = true;
    bool frameV = true;
    if (m_CurveEditor.settings.hRangeMin != Mathf.NegativeInfinity &&
        m_CurveEditor.settings.hRangeMax != Mathf.Infinity) {
      m_CurveEditor.SetShownHRangeInsideMargins(
          m_CurveEditor.settings.hRangeMin, m_CurveEditor.settings.hRangeMax);
      frameH = false;
    }
    if (m_CurveEditor.settings.vRangeMin != Mathf.NegativeInfinity &&
        m_CurveEditor.settings.vRangeMax != Mathf.Infinity) {
  ...
  static bool GetNormalizationRect(out Rect normalizationRect,
    ...
    normalizationRect = new Rect(
        curveEditor.settings.hRangeMin, curveEditor.settings.vRangeMin,
  ...
  static Keyframe[] CopyAndScaleCurveKeys(Keyframe[] orgKeys, Rect rect,
    ...
    if (rect.width == 0f || rect.height == 0f ||
        Single.IsInfinity(rect.width) || Single.IsInfinity(rect.height)) {
  ...
  void InitCurvePresets() {
    if (m_CurvePresets == null) {
      ...
      Action<AnimationCurve> presetSelectedCallback =
          delegate(AnimationCurve presetCurve) {
      ...
      m_CurvePresets = new CurvePresetsContentsForPopupWindow(
          curveToSaveAsPreset, curveLibraryType, presetSelectedCallback);
  ...
  public void Show(Action<AnimationCurve> onCurveChanged,
  ...
  public void FrameSelected() { m_CurveEditor.FrameSelected(true, true); }
  ...
  Rect GetCurveEditorRect() {
    // return new Rect(0, kToolbarHeight, position.width,
    // position.height-kToolbarHeight);
  ...
  public static Keyframe[] NormalizeKeys(Keyframe[] sourceKeys,
                                         NormalizationMode normalization,
                                         CurveEditor curveEditor) {
  ...
  public static Keyframe[] GetDenormalizedKeys(Keyframe[] sourceKeys,
                                               CurveEditor curveEditor) {
  ...
  public static Keyframe[] GetNormalizedKeys(Keyframe[] sourceKeys,
                                             CurveEditor curveEditor) {
  ...
  void OnGUI() {
    ...
    GUI.Label(m_CurveEditor.drawRect, GUIContent.none,
              ms_Styles.curveEditorBackground);
    m_CurveEditor.OnGUI();

    // Preset swatch area
    GUI.Box(new Rect(0, position.height - kPresetsHeight, position.width,
                     kPresetsHeight),
            "", ms_Styles.curveSwatchArea);
    ...
    float yPos =
        position.height - kPresetsHeight + (kPresetsHeight - height) * 0.5f;
    InitCurvePresets();
    CurvePresetLibrary curveLibrary =
        m_CurvePresets.GetPresetLibraryEditor().GetCurrentLib();
    if (curveLibrary != null) {
      for (int i = 0; i < curveLibrary.Count(); i++) {
        Rect swatchRect =
            new Rect(margin + (width + 5f) * i, yPos, width, height);
        m_GUIContent.tooltip = curveLibrary.GetName(i);
        if (GUI.Button(swatchRect, m_GUIContent, ms_Styles.curveSwatch)) {
          AnimationCurve animCurve =
              curveLibrary.GetPreset(i) as AnimationCurve;
    ...
    // if (EditorGUI.DropdownButton(new Rect (position.width -26, yPos, 20, 20),
    // GUIContent.none, FocusType.Passive, "OL Plus"))
  ...
  void PresetDropDown(Rect rect) {
    if (EditorGUI.DropdownButton(rect, EditorGUI.GUIContents.titleSettingsIcon,
      ...
      if (m_Curve != null) {
        ...
        AnimationCurve copy =
            new AnimationCurve(GetNormalizedKeys(m_Curve.keys));
  ...
  void ValidateCurveLibraryTypeAndScale() {
    ...
    if (GetNormalizationRect(out normalizationRect, m_CurveEditor)) {
      if (curveLibraryType != CurveLibraryType.NormalizedZeroToOne)
        Debug.LogError(
            "When having a normalize rect we should be using curve library type: NormalizedZeroToOne (normalizationRect: " +
            normalizationRect + ")");
    } else {
      if (curveLibraryType != CurveLibraryType.Unbounded)
        Debug.LogError(
            "When NOT having a normalize rect we should be using library type: Unbounded");
    }
  }
