...
internal class AddCurvesPopupHierarchyDataSource : TreeViewDataSource {
  ...
  public override void FetchData() {
    ...
    if (AddCurvesPopup.s_State.selection.canAddCurves) {
      ...
      ScriptableObject scriptableObject =
          AddCurvesPopup.s_State.activeScriptableObject;

      if (rootGameObject != null) {
        AddGameObjectToHierarchy(rootGameObject, rootGameObject,
                                 AddCurvesPopup.s_State.activeAnimationClip,
                                 m_RootItem);
      } else if (scriptableObject != null) {
        AddScriptableObjectToHierarchy(
            scriptableObject, AddCurvesPopup.s_State.activeAnimationClip,
            m_RootItem);
      }
    }
  ...
  private TreeViewItem AddGameObjectToHierarchy(GameObject gameObject,
                                                GameObject rootGameObject,
                                                AnimationClip animationClip,
                                                TreeViewItem parent) {
    string path = AnimationUtility.CalculateTransformPath(
        gameObject.transform, rootGameObject.transform);
    AddCurvesPopupGameObjectNode node =
        new AddCurvesPopupGameObjectNode(gameObject, parent, gameObject.name);
    ...
    EditorCurveBinding[] allCurveBindings =
        AnimationUtility.GetAnimatableBindings(gameObject, rootGameObject);
    List<EditorCurveBinding> singleObjectBindings =
      ...
      if (curveBinding.propertyName == "m_IsActive") {
        ...
        if (curveBinding.path != "") {
          TreeViewItem newNode =
              CreateNode(singleObjectBindings.ToArray(), node);
          if (newNode != null)
        ...
        if (!isLastItemOverall)
          isLastItemOnThisGroup =
              (allCurveBindings[i + 1].type != curveBinding.type);
        ...
        if (curveBinding.type == typeof(Animator) &&curveBinding.propertyName ==
            "m_Enabled")
          singleObjectBindings.Remove(curveBinding);

        if ((isLastItemOverall || isLastItemOnThisGroup) &&
            singleObjectBindings.Count > 0) {
          childNodes.Add(AddAnimatableObjectToHierarchy(
              singleObjectBindings.ToArray(), node, path));
          singleObjectBindings.Clear();
        }
      }
    }
    ...
    if (animator != null) {
      // If the Animator has a human avatar, we need to check if the avatar's
      // hierarchy matches that of the current GameObject. If they do not match,
      // disable the node.
    ...
    if (showEntireHierarchy) {
      ...
      for (int i = 0; i < gameObject.transform.childCount; i++) {
        ...
        TreeViewItem childNode = AddGameObjectToHierarchy(
            childTransform.gameObject, rootGameObject, animationClip, node);
        if (childNode != null)
  ...
  AddScriptableObjectToHierarchy(ScriptableObject scriptableObject,
                                 AnimationClip clip, TreeViewItem parent) {
    EditorCurveBinding[] allCurveBindings =
        AnimationUtility.GetAnimatableBindings(scriptableObject);
    EditorCurveBinding[] availableBindings =
        allCurveBindings
            .Where(c => !AnimationWindowUtility.IsCurveCreated(clip, c))
            .ToArray();
  ...
  static string GetClassName(EditorCurveBinding binding) {
    if (AddCurvesPopup.s_State.activeRootGameObject != null) {
      Object target = AnimationUtility.GetAnimatedObject(
          AddCurvesPopup.s_State.activeRootGameObject, binding);
      if (target)
  ...
  static Texture2D GetIcon(EditorCurveBinding binding) {
    if (AddCurvesPopup.s_State.activeRootGameObject != null) {
      return AssetPreview.GetMiniThumbnail(AnimationUtility.GetAnimatedObject(
          AddCurvesPopup.s_State.activeRootGameObject, binding));
    } else if (AddCurvesPopup.s_State.activeScriptableObject != null) {
      return AssetPreview.GetMiniThumbnail(
          AddCurvesPopup.s_State.activeScriptableObject);
    }
  ...
  private TreeViewItem
  AddAnimatableObjectToHierarchy(EditorCurveBinding[] curveBindings,
                                 TreeViewItem parentNode, string path) {
    TreeViewItem node = new AddCurvesPopupObjectNode(
        parentNode, path, GetClassName(curveBindings[0]));
    node.icon = GetIcon(curveBindings[0]);

    List<TreeViewItem> childNodes = new List<TreeViewItem>();
    List<EditorCurveBinding> singlePropertyBindings =
        new List<EditorCurveBinding>();
    ...
    for (int i = 0; i < curveBindings.Length; i++) {
      ...
      if (i == curveBindings.Length - 1 ||
          AnimationWindowUtility.GetPropertyGroupName(
              curveBindings[i + 1].propertyName) !=
              AnimationWindowUtility.GetPropertyGroupName(
                  curveBinding.propertyName)) {
        TreeViewItem newNode =
            CreateNode(singlePropertyBindings.ToArray(), node);
        if (newNode != null)
          childNodes.Add(newNode);
  ...
  private TreeViewItem CreateNode(EditorCurveBinding[] curveBindings,
  ...
  public void UpdateData() { m_TreeView.ReloadData(); }
}

internal class AddCurvesPopupGameObjectNode : TreeViewItem {
  internal bool propertyPathMismatchWithHumanAvatar = false;
  public AddCurvesPopupGameObjectNode(GameObject gameObject,
                                      TreeViewItem parent, string displayName)
      : base(gameObject.GetInstanceID(), parent != null ? parent.depth + 1
             : -1, parent, displayName) {}
}

internal class AddCurvesPopupObjectNode : TreeViewItem {
  public AddCurvesPopupObjectNode(TreeViewItem parent, string path,
                                  string className)
      : base((path + className).GetHashCode(), parent != null ? parent.depth + 1
             : -1, parent, className) {}
}
...
internal class AddCurvesPopupPropertyNode : TreeViewItem {
  ...
  public AddCurvesPopupPropertyNode(TreeViewItem parent,
                                    EditorCurveBinding[] curveBindings)
      : base(curveBindings [0]
                 .GetHashCode(),
             parent.depth + 1, parent,
             AnimationWindowUtility.NicifyPropertyGroupName(
                 curveBindings[0].type,
                 AnimationWindowUtility.GetPropertyGroupName(
  ...
  public override int CompareTo(TreeViewItem other) {
    ...
    if (otherNode != null) {
      if (displayName.Contains("Rotation") &&
          otherNode.displayName.Contains("Position"))
        return 1;
      if (displayName.Contains("Position") &&
          otherNode.displayName.Contains("Rotation"))
        return -1;
    }
