...
internal class AnimationRecording {
  ...
  static bool
  HasAnyRecordableModifications(GameObject root,
                                UndoPropertyModification[] modifications) {
    for (int i = 0; i < modifications.Length; i++) {
      EditorCurveBinding tempBinding;
      if (AnimationUtility.PropertyModificationToEditorCurveBinding(
              modifications[i].previousValue, root, out tempBinding) != null)
        return true;
    }
    return false;
  }

  static private UndoPropertyModification[] FilterModifications(
      IAnimationRecordingState state,
      ref UndoPropertyModification[] modifications) {
    GameObject rootGameObject = state.activeRootGameObject;

    List<UndoPropertyModification> discardedModifications =
        new List<UndoPropertyModification>();
    List<UndoPropertyModification> outModifications =
        new List<UndoPropertyModification>();
    for (int i = 0; i < modifications.Length; ++i) {
      ...
      if (AnimationUtility.PropertyModificationToEditorCurveBinding(
              prop, rootGameObject, out binding) != null)
  ...
  static private void CollectRotationModifications(
      IAnimationRecordingState state,
      ref UndoPropertyModification[] modifications,
      ref Dictionary<object, RotationModification> rotationModifications) {
    List<UndoPropertyModification> outModifs =
        new List<UndoPropertyModification>();
    foreach (var modification in modifications) {
      ...
      AnimationUtility.PropertyModificationToEditorCurveBinding(
          prop, state.activeRootGameObject, out binding);
      if (binding.propertyName.StartsWith(kLocalRotation)) {
        RotationModification rotationModification;

        if (!rotationModifications.TryGetValue(prop.target,
                                               out rotationModification)) {
  ...
  static private void DiscardRotationModification(
      RotationModification rotationModification,
      ref List<UndoPropertyModification> discardedModifications) {
  ...
  static private UndoPropertyModification[] FilterRotationModifications(
      IAnimationRecordingState state,
      ref Dictionary<object, RotationModification> rotationModifications) {
    GameObject rootGameObject = state.activeRootGameObject;
    ...
    List<UndoPropertyModification> discardedModifications =
        new List<UndoPropertyModification>();
    foreach (KeyValuePair<object, RotationModification> item in
                 rotationModifications) {
      ...
      EditorCurveBinding binding = new EditorCurveBinding();
      Type type = AnimationUtility.PropertyModificationToEditorCurveBinding(
          m.lastQuatModification.currentValue, rootGameObject, out binding);
      if (type == null) {
        DiscardRotationModification(m, ref discardedModifications);
        itemsToRemove.Add(item.Key);
      }
  ...
  static private void
  AddRotationPropertyModification(IAnimationRecordingState state,
    ...
    state.AddPropertyModification(binding, modification.previousValue,
                                  modification.keepPrefabOverride);
  }
  ...
  static private void ProcessRotationModifications(
    ...
    foreach (KeyValuePair<object, RotationModification> item in
      ...
      EditorCurveBinding binding = new EditorCurveBinding();
      Type type = AnimationUtility.PropertyModificationToEditorCurveBinding(
          m.lastQuatModification.currentValue, state.activeRootGameObject,
          out binding);
      if (type == null)
        continue;
      ...
      if (m.useEuler) {
        ...
        Vector3 previousEulerAngles =
            target.GetLocalEulerAngles(RotationOrder.OrderZXY);
        Vector3 currentEulerAngles = previousEulerAngles;

        object eulerX, eulerY, eulerZ;
        if (ValueFromPropertyModification(m.eulerX.previousValue, binding,
                                          out eulerX))
          previousEulerAngles.x = (float) eulerX;
        if (ValueFromPropertyModification(m.eulerY.previousValue, binding,
                                          out eulerY))
          previousEulerAngles.y = (float) eulerY;
        if (ValueFromPropertyModification(m.eulerZ.previousValue, binding,
                                          out eulerZ))
          previousEulerAngles.z = (float) eulerZ;

        if (ValueFromPropertyModification(m.eulerX.currentValue, binding,
                                          out eulerX))
          currentEulerAngles.x = (float) eulerX;
        if (ValueFromPropertyModification(m.eulerY.currentValue, binding,
                                          out eulerY))
          currentEulerAngles.y = (float) eulerY;
        if (ValueFromPropertyModification(m.eulerZ.currentValue, binding,
                                          out eulerZ))
          currentEulerAngles.z = (float) eulerZ;

        // Fallback to quaternion euler values if euler hint and quaternion are
        // out of sync.
        previousEulerAngles = AnimationUtility.GetClosestEuler(
            previousValue, previousEulerAngles, RotationOrder.OrderZXY);
        currentEulerAngles = AnimationUtility.GetClosestEuler(
            currentValue, currentEulerAngles, RotationOrder.OrderZXY);

        AddRotationKey(state, binding, type, previousEulerAngles,
                       currentEulerAngles);
      } else {
        Vector3 eulerAngles =
            target.GetLocalEulerAngles(RotationOrder.OrderZXY);

        Vector3 previousEulerAngles = AnimationUtility.GetClosestEuler(
            previousValue, eulerAngles, RotationOrder.OrderZXY);
        Vector3 currentEulerAngles = AnimationUtility.GetClosestEuler(
            currentValue, eulerAngles, RotationOrder.OrderZXY);
  ...
  static private void CollectVector3Modifications(
      IAnimationRecordingState state,
      ref UndoPropertyModification[] modifications,
      ref Dictionary<object, Vector3Modification> vector3Modifications,
      string propertyName) {
    List<UndoPropertyModification> outModifs =
        new List<UndoPropertyModification>();
    foreach (var modification in modifications) {
      ...
      if (binding.propertyName.StartsWith(propertyName)) {
        ...
        if (!vector3Modifications.TryGetValue(prop.target,
                                              out vector3Modification)) {
  ...
  static private void ProcessVector3Modification(
      IAnimationRecordingState state, EditorCurveBinding baseBinding,
      UndoPropertyModification modification, Transform target, string axis,
      float scale = 1.0f) {
    var binding = baseBinding;
    binding.propertyName =
        binding.propertyName.Remove(binding.propertyName.Length - 1, 1) + axis;
    ...
    if (previousModification == null) {
      ...
      previousModification.value =
          ((float) currentValue)
              .ToString(CultureInfo.InvariantCulture.NumberFormat);
    }

    object previousValue = currentValue;
    ValueFromPropertyModification(previousModification, binding,
                                  out previousValue);

    state.AddPropertyModification(binding, previousModification,
                                  modification.keepPrefabOverride);
  ...
  static public void ProcessVector3Modifications(
      IAnimationRecordingState state,
      ref Dictionary<object, Vector3Modification> vector3Modifications) {
    foreach (KeyValuePair<object, Vector3Modification> item in
                 vector3Modifications) {
      ...
      EditorCurveBinding binding = new EditorCurveBinding();
      Type type = AnimationUtility.PropertyModificationToEditorCurveBinding(
          m.last.currentValue, state.activeRootGameObject, out binding);
      if (type == null)
        continue;
  ...
  static public void
  ProcessModifications(IAnimationRecordingState state,
                       UndoPropertyModification[] modifications) {
    ...
    for (int i = 0; i < modifications.Length; i++) {
      ...
      Type type = AnimationUtility.PropertyModificationToEditorCurveBinding(
          prop, root, out binding);
      if (type != null) {
        object currentValue =
            CurveBindingUtility.GetCurrentValue(root, binding);

        object previousValue = null;
        if (!ValueFromPropertyModification(prop, binding, out previousValue))
          previousValue = currentValue;

        state.AddPropertyModification(binding, prop,
                                      modifications[i].keepPrefabOverride);
  ...
  static public UndoPropertyModification[] Process(
    ...
    Dictionary<object, RotationModification> rotationModifications =
        new Dictionary<object, RotationModification>();
    Dictionary<object, Vector3Modification> scaleModifications =
        new Dictionary<object, Vector3Modification>();
    Dictionary<object, Vector3Modification> positionModifications =
        new Dictionary<object, Vector3Modification>();

    // Strip out the rotation modifications from the rest
    CollectRotationModifications(state, ref modifications,
                                 ref rotationModifications);
    UndoPropertyModification[] discardedRotationModifications =
        FilterRotationModifications(state, ref rotationModifications);
    UndoPropertyModification[] discardedModifications =
        FilterModifications(state, ref modifications);
    CollectVector3Modifications(state, ref modifications,
                                ref positionModifications, kLocalPosition);
    CollectVector3Modifications(state, ref modifications,
                                ref scaleModifications, kLocalScale);

    // Process Animator Modifications.
    ProcessAnimatorModifications(state, ref positionModifications,
                                 ref rotationModifications,
                                 ref scaleModifications);
  ...
  static bool ValueFromPropertyModification(PropertyModification modification,
    ...
    if (modification == null) {
      ...
      float temp;
      if (float.TryParse(modification.value, NumberStyles.Float,
  ...
  static void AddKey(IAnimationRecordingState state, EditorCurveBinding binding,
                     Type type, object previousValue, object currentValue) {
      ...
      if (curve.length == 0) {
        if (state.currentFrame != 0)
          AnimationWindowUtility.AddKeyframeToCurve(
              curve, previousValue, type,
              AnimationKeyTime.Frame(0, clip.frameRate));
      }
    }
    ...
    AnimationWindowUtility.AddKeyframeToCurve(
        ...
        AnimationKeyTime.Frame(state.currentFrame, clip.frameRate));

    state.SaveCurve(curve);
  }

  static void AddRotationKey(IAnimationRecordingState state,
                             EditorCurveBinding binding, Type type,
                             Vector3 previousEulerAngles,
    ...
    EditorCurveBinding[] additionalBindings =
        RotationCurveInterpolation.RemapAnimationBindingForRotationAddKey(
            binding, clip);

    // Add key at current frame
    for (int i = 0; i < 3; i++) {
      AnimationWindowCurve curve =
          new AnimationWindowCurve(clip, additionalBindings[i], type);
      ...
      if (state.addZeroFrame) {
        ...
        if (curve.length == 0) {
          if (state.currentFrame != 0) {
            AnimationWindowUtility.AddKeyframeToCurve(
                curve, previousEulerAngles[i], type,
                AnimationKeyTime.Frame(0, clip.frameRate));
          }
        }
      }
      ...
      AnimationWindowUtility.AddKeyframeToCurve(
          curve, currentEulerAngles[i], type,
          AnimationKeyTime.Frame(state.currentFrame, clip.frameRate));
      state.SaveCurve(curve);
    }
  }
  ...
  static private void ProcessRootMotionModifications(
      IAnimationRecordingState state,
    ...
    foreach (KeyValuePair<object, RootMotionModification> item in
      ...
      if (m.lastP.previousValue != null) {
        ProcessRootMotionModification(state, animator, m.px, "MotionT.x",
                                      position.x, scale.x);
        ProcessRootMotionModification(state, animator, m.py, "MotionT.y",
                                      position.y, scale.y);
        ProcessRootMotionModification(state, animator, m.pz, "MotionT.z",
                                      position.z, scale.z);
      }

      if (m.lastR.previousValue != null) {
        ProcessRootMotionModification(state, animator, m.rx, "MotionQ.x",
                                      rotation.x, 1);
        ProcessRootMotionModification(state, animator, m.ry, "MotionQ.y",
                                      rotation.y, 1);
        ProcessRootMotionModification(state, animator, m.rz, "MotionQ.z",
                                      rotation.z, 1);
        ProcessRootMotionModification(state, animator, m.rw, "MotionQ.w",
                                      rotation.w, 1);
      }
    }
  }

  static private void ProcessAnimatorModifications(
      IAnimationRecordingState state,
    ...
    Dictionary<object, RootMotionModification> rootMotionModifications =
        new Dictionary<object, RootMotionModification>();
    ...
    foreach (KeyValuePair<object, Vector3Modification> item in
      ...
      bool isRootMotion = isRootTransform && applyRootMotion && hasRootCurves &&
                          (isHuman || hasRootMotion);
      bool isHumanBone =
          isHuman && !isRootTransform && animator.IsBoneTransform(target);
      ...
      if (isHumanBone) {
        ...
        if (!rootMotionModifications.TryGetValue(target,
                                                 out rootMotionModification)) {
        ...
        Vector3 scale =
            root.transform.localScale * (isHuman ? animator.humanScale : 1);
    ...
    foreach (KeyValuePair<object, Vector3Modification> item in
      ...
      bool isHumanBone =
          isHuman && !isRootTransform && animator.IsBoneTransform(target);
      if (isHumanBone) {
        Debug.LogWarning("Keyframing scale on humanoid rig is not supported!",
                         target as Transform);
  ...
  static void
  ProcessRootMotionModification(IAnimationRecordingState state,
                                Animator animator,
                                UndoPropertyModification modification,
                                string name, float value, float scale) {
    AnimationClip clip = state.activeAnimationClip;
    ...
    if (ValueFromPropertyModification(modification.currentValue,
                                      new EditorCurveBinding(), out oValue))
      value = (float) oValue;

    if (ValueFromPropertyModification(modification.previousValue,
                                      new EditorCurveBinding(), out oValue))
    ...
    AnimationWindowCurve curve =
        new AnimationWindowCurve(clip, binding, typeof(float));

    if (state.addZeroFrame && state.currentFrame != 0 && curve.length == 0) {
      AnimationWindowUtility.AddKeyframeToCurve(
          curve, prevValue, typeof(float),
          AnimationKeyTime.Frame(0, clip.frameRate));
    }

    AnimationWindowUtility.AddKeyframeToCurve(
        curve, value, typeof(float),
        AnimationKeyTime.Frame(state.currentFrame, clip.frameRate));
