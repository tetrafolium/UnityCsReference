...
internal class AnimationWindowEventInspector : Editor {
  ...
  public static GUIContent s_OverloadWarning = EditorGUIUtility.TrTextContent(
      "Some functions were overloaded in MonoBehaviour components and may not work as intended if used with Animation Events!");
  ...
  protected override void OnHeaderGUI() {
    string targetTitle = (targets.Length == 1)
                             ? "Animation Event"
                             : targets.Length + " Animation Events";
  ...
  public static void OnEditAnimationEvents(AnimationWindowEvent[] awEvents) {
    ...
    if (data.events == null || data.selectedEvents == null ||
        data.selectedEvents.Length == 0)
      return;

    AnimationEvent firstEvent = data.selectedEvents[0];

    bool singleFunctionName =
        Array.TrueForAll(data.selectedEvents,
                         evt => evt.functionName == firstEvent.functionName);

    GUI.changed = false;

    if (data.root != null) {
      List<AnimationWindowEventMethod> methods =
          new List<AnimationWindowEventMethod>();
      ...
      int selected =
          methods.FindIndex(method => method.name == firstEvent.functionName);
      if (selected == -1) {
      ...
      selected = EditorGUILayout.Popup("Function: ", selected,
                                       methodsFormatted.ToArray());
      if (wasSelected != selected && selected != -1 &&
      ...
      if (overloads.Count > 0) {
        EditorGUILayout.Space();
        EditorGUILayout.HelpBox(s_OverloadWarning.text, MessageType.Warning,
                                true);
      }
    } else {
      EditorGUI.showMixedValue = !singleFunctionName;
      ...
      string functionName =
          EditorGUILayout
              .TextField(EditorGUIUtility.TrTextContent("Function"),
                         oldFunctionName)
              .Replace(" ", "");
      ...
      if (singleFunctionName) {
        ...
        using(new EditorGUI.DisabledScope(true)) {
          ...
          DoEditRegularParameters(new AnimationEvent[]{dummyEvent},
  ...
  public static void OnDisabledAnimationEvent() {
    ...
    using(new EditorGUI.DisabledScope(true)) {
      dummyEvent.functionName = EditorGUILayout.TextField(
          EditorGUIUtility.TrTextContent("Function"), dummyEvent.functionName);
      DoEditRegularParameters(new AnimationEvent[]{dummyEvent},
                              typeof(AnimationEvent));
    }
  }
  ...
  public static void
  CollectSupportedMethods(GameObject gameObject,
                          List<AnimationWindowEventMethod> supportedMethods,
    ...
    foreach (MonoBehaviour behaviour in behaviours) {
      ...
      while (type != typeof(MonoBehaviour) &&type != null) {
        ...
        for (int i = 0; i < methods.Length; i++) {
          ...
          AnimationWindowEventMethod newMethod =
              new AnimationWindowEventMethod();
          newMethod.name = method.Name;
          newMethod.parameterType = parameterType;

          // Since AnimationEvents only stores method name, it can't handle
          // functions with multiple overloads. Only retrieve first found
          // function, but discard overloads.
          int existingMethodIndex =
              supportedMethods.FindIndex(m => m.name == name);
          if (existingMethodIndex != -1) {
            // The method is only ambiguous if it has a different signature to
            // the one we saw before
            if (supportedMethods[existingMethodIndex].parameterType !=
                parameterType) {
  ...
  public static string FormatEvent(GameObject root, AnimationEvent evt) {
    ...
    foreach (var behaviour in root.GetComponents<MonoBehaviour>()) {
      ...
      try {
        method = type.GetMethod(evt.functionName,
  ...
  private static void DoEditRegularParameters(AnimationEvent[] events,
                                              Type selectedParameter) {
    AnimationEvent firstEvent = events[0];

    if (selectedParameter == typeof(AnimationEvent) ||
        selectedParameter == typeof(float)) {
      bool singleParamValue = Array.TrueForAll(
          events, evt => evt.floatParameter == firstEvent.floatParameter);

      EditorGUI.BeginChangeCheck();
      EditorGUI.showMixedValue = !singleParamValue;
      float newValue =
          EditorGUILayout.FloatField("Float", firstEvent.floatParameter);
      EditorGUI.showMixedValue = false;
    ...
    if (selectedParameter == typeof(AnimationEvent) ||
        selectedParameter == typeof(int) || selectedParameter.IsEnum) {
      bool singleParamValue = Array.TrueForAll(
          events, evt => evt.intParameter == firstEvent.intParameter);
      ...
      if (selectedParameter.IsEnum)
        newValue =
            EnumPopup("Enum", selectedParameter, firstEvent.intParameter);
      else
    ...
    if (selectedParameter == typeof(AnimationEvent) ||
        selectedParameter == typeof(string)) {
      bool singleParamValue = Array.TrueForAll(
          events, evt => evt.stringParameter == firstEvent.stringParameter);

      EditorGUI.BeginChangeCheck();
      EditorGUI.showMixedValue = !singleParamValue;
      string newValue =
          EditorGUILayout.TextField("String", firstEvent.stringParameter);
      EditorGUI.showMixedValue = false;
    ...
    if (selectedParameter == typeof(AnimationEvent) ||
        selectedParameter.IsSubclassOf(typeof(UnityEngine.Object)) ||
        selectedParameter == typeof(UnityEngine.Object)) {
      bool singleParamValue =
          Array.TrueForAll(events, evt => evt.objectReferenceParameter ==
                                          firstEvent.objectReferenceParameter);
      ...
      Object newValue = EditorGUILayout.ObjectField(
          ...
          firstEvent.objectReferenceParameter, type, allowSceneObjects);
      EditorGUI.showMixedValue = false;
  ...
  private static bool IsSupportedMethodName(string name) {
    return name != "Main" && name != "Start" && name != "Awake" &&
           name != "Update";
  }

  private static string FormatEventArguments(IEnumerable<Type> paramTypes,
                                             AnimationEvent evt) {
    if (!paramTypes.Any())
      return " ( )";
    ...
    if (paramType.IsEnum)
      return " ( " + paramType.Name + "." +
             Enum.GetName(paramType, evt.intParameter) + " )";

    if (paramType == typeof(AnimationEvent))
      return " ( " + evt.floatParameter + " / " + evt.intParameter + " / \"" +
             evt.stringParameter + "\" / " +
             (evt.objectReferenceParameter == null ? "null"
              : evt.objectReferenceParameter.name) +
             " )";

    if (paramType.IsSubclassOf(typeof(UnityEngine.Object)) ||
        paramType == typeof(UnityEngine.Object))
      return " ( " +
             (evt.objectReferenceParameter == null ? "null"
              : evt.objectReferenceParameter.name) +
             " )";
  ...
  private static AnimationWindowEventData
  GetData(AnimationWindowEvent[] awEvents) {
  ...
  private static void SetData(AnimationWindowEvent[] awEvents,
                              AnimationWindowEventData data) {
    if (data.events == null)
      return;
    ...
    if (data.clip != null) {
      ...
      foreach (var awEvent in awEvents) {
        if (awEvent.eventIndex >= 0 && awEvent.eventIndex < data.events.Length)
          data.clipInfo.SetEvent(awEvent.eventIndex,
                                 data.events[awEvent.eventIndex]);
      }
    }
  }
