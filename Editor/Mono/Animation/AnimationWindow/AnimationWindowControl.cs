...
internal class AnimationWindowControl : IAnimationWindowControl,
  ...
  class CandidateRecordingState : IAnimationRecordingState {
    ...
    public CandidateRecordingState(AnimationWindowState state,
    ...
    public bool DiscardModification(PropertyModification modification) {
      return !AnimationMode.IsPropertyAnimated(modification.target,
    ...
    public void
    AddPropertyModification(EditorCurveBinding binding,
                            PropertyModification propertyModification,
                            bool keepPrefabOverride) {
      AnimationMode.AddCandidate(binding, propertyModification,
                                 keepPrefabOverride);
    }
  }

  enum RecordingStateMode { ManualKey, AutoKey }
  ...
  class RecordingState : IAnimationRecordingState {
    ...
    public GameObject activeGameObject {
      get { return m_State.activeGameObject; }
    }
    public GameObject activeRootGameObject {
      get { return m_State.activeRootGameObject; }
    }
    public AnimationClip activeAnimationClip {
      get { return m_State.activeAnimationClip; }
    }
    public int currentFrame {
      get { return m_State.currentFrame; }
    }
    ...
    AddPropertyModification(EditorCurveBinding binding,
                            PropertyModification propertyModification,
                            bool keepPrefabOverride) {
      AnimationMode.AddPropertyModification(binding, propertyModification,
                                            keepPrefabOverride);
    }
  }

  [Flags] enum ResampleFlags {
    None = 0,
  ...
  public AnimEditor animEditor {
    get { return state.animEditor; }
  }

  [SerializeField]
  private AnimationClip m_CandidateClip;
  [SerializeField]
  private AnimationClip m_DefaultPose;
  ...
  private static ProfilerMarker s_ResampleAnimationMarker =
      new ProfilerMarker("AnimationWindowControl.ResampleAnimation");
  ...
  public override void GoToTime(float time) { SetCurrentTime(time); }

  public override void GoToFrame(int frame) { SetCurrentFrame(frame); }

  public override void StartScrubTime() {
    // nothing to do...
  }

  public override void ScrubTime(float time) { SetCurrentTime(time); }

  public override void EndScrubTime() {
    // nothing to do...
  }

  public override void GoToPreviousFrame() { SetCurrentFrame(time.frame - 1); }

  public override void GoToNextFrame() { SetCurrentFrame(time.frame + 1); }

  public override void GoToPreviousKeyframe() {
    List<AnimationWindowCurve> curves =
        (state.showCurveEditor && state.activeCurves.Count > 0)
            ? state.activeCurves
            : state.allCurves;

    float newTime = AnimationWindowUtility.GetPreviousKeyframeTime(
        curves.ToArray(), time.time, state.clipFrameRate);
    SetCurrentTime(state.SnapToFrame(
        newTime, AnimationWindowState.SnapMode.SnapToClipFrame));
  }

  public void GoToPreviousKeyframe(PropertyModification[] modifications) {
    EditorCurveBinding[] bindings =
        AnimationWindowUtility.PropertyModificationsToEditorCurveBindings(
            modifications, state.activeRootGameObject,
            state.activeAnimationClip);
  ...
  public override void GoToNextKeyframe() {
    ...
    float newTime = AnimationWindowUtility.GetNextKeyframeTime(
        curves.ToArray(), time.time, state.clipFrameRate);
    SetCurrentTime(state.SnapToFrame(
        newTime, AnimationWindowState.SnapMode.SnapToClipFrame));
  }
  ...
  public override bool playing {
    get { return AnimationMode.InAnimationPlaybackMode() && previewing; }
  }
  ...
  public override bool PlaybackUpdate() {
    ...
    m_Time = AnimationKeyTime.Time(
        Mathf.Clamp(newTime, state.minTime, state.maxTime), state.frameRate);
  ...
  public override void StopPreview() {
    ...
    if (m_UsesPostProcessComponents) {
      IAnimationWindowPreview[] previewComponents =
  ...
  private void RebuildGraph(Animator animator) {
    ...
    m_ClipPlayable =
        AnimationClipPlayable.Create(m_Graph, state.activeAnimationClip);
    m_ClipPlayable.SetOverrideLoopTime(true);
    m_ClipPlayable.SetLoopTime(false);
    m_ClipPlayable.SetApplyFootIK(false);

    m_CandidateClipPlayable =
        AnimationClipPlayable.Create(m_Graph, m_CandidateClip);
    m_CandidateClipPlayable.SetApplyFootIK(false);

    IAnimationWindowPreview[] previewComponents = FetchPostProcessComponents();
    bool requiresDefaultPose =
        previewComponents != null && previewComponents.Length > 0;
    int nInputs = requiresDefaultPose ? 3 : 2;

    // Create a layer mixer if necessary, we'll connect playable nodes to it
    // after having populated AnimationStream.
    AnimationLayerMixerPlayable mixer =
        AnimationLayerMixerPlayable.Create(m_Graph, nInputs);
    ...
    if (requiresDefaultPose) {
      AnimationMode.RevertPropertyModificationsForGameObject(
          state.activeRootGameObject);

      EditorCurveBinding[] streamBindings =
          AnimationUtility.GetAnimationStreamBindings(
              state.activeRootGameObject);

      m_DefaultPose = new AnimationClip(){name = "DefaultPose"};

      AnimationWindowUtility.CreateDefaultCurves(state, m_DefaultPose,
                                                 streamBindings);

      m_DefaultPosePlayable =
          AnimationClipPlayable.Create(m_Graph, m_DefaultPose);
  ...
  private IAnimationWindowPreview[] FetchPostProcessComponents() {
    if (state.activeRootGameObject != null) {
      return state.activeRootGameObject
          .GetComponents<IAnimationWindowPreview>();
    }
    ...
    if (state.activeAnimationClip != null) {
      ...
      if (usePlayableGraph) {
        ...
        if (isValidGraph) {
          ...
          isValidGraph =
              playableOutput.GetTarget() == (Animator) animationPlayer;
        }
      ...
      if (usePlayableGraph) {
        ...
        if (!m_CandidateClip.empty)
          AnimationMode.AddCandidates(state.activeRootGameObject,
                                      m_CandidateClip);

        m_ClipPlayable.SetSampleRate(playing ? - 1
                                     : state.activeAnimationClip.frameRate);

        AnimationMode.SamplePlayableGraph(m_Graph, 0, time.time);

        // This will cover euler/quaternion matching in basic playable graphs
        // only (animation clip + candidate clip).
        AnimationUtility.SampleEulerHint(state.activeRootGameObject,
                                         state.activeAnimationClip, time.time,
                                         WrapMode.Clamp);
        if (!m_CandidateClip.empty)
          AnimationUtility.SampleEulerHint(state.activeRootGameObject,
                                           m_CandidateClip, time.time,
                                           WrapMode.Clamp);
      } else {
        AnimationMode.SampleAnimationClip(state.activeRootGameObject,
                                          state.activeAnimationClip, time.time);
        if (!m_CandidateClip.empty)
          AnimationMode.SampleCandidateClip(state.activeRootGameObject,
                                            m_CandidateClip, 0f);
      }
      ...
      if (HasFlag(flags, ResampleFlags.RefreshViews)) {
        ...
        // Particle editor needs to be manually repainted to refresh the
        // animated properties
  ...
  private UndoPropertyModification[] PostprocessAnimationRecordingModifications(
      UndoPropertyModification[] modifications) {
    // Fix for case 751009: The animationMode can be changed outside the
    // AnimationWindow, and this callback needs to be unregistered.
    ...
    // Only resample when playable graph has been customized with post process
    // nodes.
    if (m_UsesPostProcessComponents)
  ...
  private UndoPropertyModification[] ProcessAutoKey(
      UndoPropertyModification[] modifications) {
    BeginKeyModification();

    RecordingState recordingState =
        new RecordingState(state, RecordingStateMode.AutoKey);
    UndoPropertyModification[] discardedModifications =
        AnimationRecording.Process(recordingState, modifications);
  ...
  private UndoPropertyModification[] RegisterCandidates(
      UndoPropertyModification[] modifications) {
    ...
    CandidateRecordingState recordingState =
        new CandidateRecordingState(state, m_CandidateClip);
    UndoPropertyModification[] discardedModifications =
        AnimationRecording.Process(recordingState, modifications);

    // No modifications were added to the candidate clip, stop recording
    // candidates.
    if (!hasCandidates && discardedModifications.Length == modifications.Length)
      StopCandidateRecording();

    // Make sure inspector is repainted after adding new candidates to get
  ...
  private void RemoveFromCandidates(PropertyModification[] modifications) {
    EditorCurveBinding[] bindings =
        AnimationWindowUtility.PropertyModificationsToEditorCurveBindings(
            modifications, state.activeRootGameObject, m_CandidateClip);
    if (bindings.Length == 0)
      return;
    ...
    for (int i = 0; i < bindings.Length; ++i) {
      ...
      if (binding.isPPtrCurve)
        AnimationUtility.SetObjectReferenceCurve(m_CandidateClip, binding,
                                                 null);
      else
    ...
    if (AnimationUtility.GetCurveBindings(m_CandidateClip).Length == 0 &&
        AnimationUtility.GetObjectReferenceCurveBindings(m_CandidateClip)
                .Length == 0)
      ClearCandidates();
  }
  ...
  private void DestroyCandidateClip() { m_CandidateClip = null; }
  ...
  public override void ProcessCandidates() {
    ...
    EditorCurveBinding[] bindings =
        AnimationUtility.GetCurveBindings(m_CandidateClip);
    EditorCurveBinding[] objectCurveBindings =
        AnimationUtility.GetObjectReferenceCurveBindings(m_CandidateClip);
    ...
    for (int i = 0; i < state.allCurves.Count; ++i) {
      AnimationWindowCurve curve = state.allCurves[i];
      EditorCurveBinding remappedBinding =
          RotationCurveInterpolation.RemapAnimationBindingForRotationCurves(
              curve.binding, m_CandidateClip);
      if (Array.Exists(bindings, binding => remappedBinding.Equals(binding)) ||
          Array.Exists(objectCurveBindings,
                       binding => remappedBinding.Equals(binding)))
        curves.Add(curve);
    }
  ...
  private List<AnimationWindowKeyframe>
  GetKeys(PropertyModification[] modifications) {
  ...
  public bool IsAnimatable(PropertyModification[] modifications) {
    for (int i = 0; i < modifications.Length; ++i) {
      ...
      if (AnimationWindowUtility.PropertyIsAnimatable(
              modification.target, modification.propertyPath,
              state.activeRootGameObject))
        return true;
    }
  ...
  public bool IsEditable(Object targetObject) {
    ...
    if (gameObject != null) {
      Component animationPlayer =
          AnimationWindowUtility.GetClosestAnimationPlayerComponentInParents(
  ...
  public bool CandidateExists(PropertyModification[] modifications) {
    ...
    for (int i = 0; i < modifications.Length; ++i) {
      var modification = modifications[i];
      if (AnimationMode.IsPropertyCandidate(modification.target,
                                            modification.propertyPath))
        return true;
    }
  ...
  public bool CurveExists(PropertyModification[] modifications) {
    ...
    EditorCurveBinding[] clipBindings =
        AnimationUtility.GetCurveBindings(state.activeAnimationClip);
    if (clipBindings.Length == 0)
      return false;

    if (Array.Exists(bindings, binding => Array.Exists(
                                   clipBindings,
                                   clipBinding => clipBinding.Equals(binding))))
      return true;
    ...
    return Array.Exists(
        objectCurveBindings,
        binding => Array.Exists(clipBindings,
                                clipBinding => clipBinding.Equals(binding)));
  }

  public bool HasAnyCandidates() { return !m_CandidateClip.empty; }

  public bool HasAnyCurves() { return (state.allCurves.Count > 0); }

  public void AddKey(SerializedProperty property) {
    AddKey(AnimationWindowUtility.SerializedPropertyToPropertyModifications(
        property));
  }
  ...
  public void AddKey(PropertyModification[] modifications) {
    ...
    var recordingState =
        new RecordingState(state, RecordingStateMode.ManualKey);
  ...
  public void RemoveKey(SerializedProperty property) {
    RemoveKey(AnimationWindowUtility.SerializedPropertyToPropertyModifications(
        property));
  }
  ...
  public void RemoveCurve(PropertyModification[] modifications) {
    ...
    for (int i = 0; i < bindings.Length; ++i) {
      EditorCurveBinding binding = bindings[i];
      if (binding.isPPtrCurve)
        AnimationUtility.SetObjectReferenceCurve(state.activeAnimationClip,
                                                 binding, null);
      else
        AnimationUtility.SetEditorCurve(state.activeAnimationClip, binding,
                                        null);
    }
