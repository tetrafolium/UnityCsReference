...
public static Mode GetModeFromCurveData(EditorCurveBinding data) {
	if (AnimationWindowUtility.IsTransformType(data.type) &&
	    data.propertyName.StartsWith("localEulerAngles")) {
	...
	} else if (AnimationWindowUtility.IsTransformType(data.type) &&
	           data.propertyName.StartsWith("m_LocalRotation"))
...
public static State GetCurveState(AnimationClip clip,
                                  EditorCurveBinding[] selection) {
	State state;
...
internal static EditorCurveBinding
[] ConvertRotationPropertiesToDefaultInterpolation(
	AnimationClip clip, EditorCurveBinding[] selection) {
...
[] ConvertRotationPropertiesToInterpolationType(
	EditorCurveBinding[] selection, Mode newInterpolationMode) {
...
static EditorCurveBinding[] GenerateTransformCurveBindingArray(
	string path, string property, Type type, int count) {
	EditorCurveBinding[] bindings = new EditorCurveBinding[count];
	for (int i = 0; i < count; i++)
		bindings[i] =
			EditorCurveBinding.FloatCurve(path, type, property + kPostFix[i]);
	return bindings;
}

static public EditorCurveBinding[] RemapAnimationBindingForAddKey(
	EditorCurveBinding binding, AnimationClip clip) {
	if (!AnimationWindowUtility.IsTransformType(binding.type)) {
		return null;
	} else if (binding.propertyName.StartsWith("m_LocalPosition.")) {
		if (binding.type == typeof(Transform))
			return GenerateTransformCurveBindingArray(
				binding.path, "m_LocalPosition.", binding.type, 3);
		else
			return null;
	} else if (binding.propertyName.StartsWith("m_LocalScale."))
		return GenerateTransformCurveBindingArray(binding.path, "m_LocalScale.",
		                                          binding.type, 3);
...
static public EditorCurveBinding[] RemapAnimationBindingForRotationAddKey(
	EditorCurveBinding binding, AnimationClip clip) {
	if (!AnimationWindowUtility.IsTransformType(binding.type)) {
		return null;
	} else if (binding.propertyName.StartsWith("m_LocalRotation")) {
...
static private EditorCurveBinding[] SelectRotationBindingForAddKey(
	EditorCurveBinding binding, AnimationClip clip) {
	...
	if (AnimationUtility.GetEditorCurve(clip, testBinding) != null)
		return GenerateTransformCurveBindingArray(
			binding.path, "localEulerAnglesBaked.", binding.type, 3);
	else {
		testBinding.propertyName = "localEulerAngles.x";
		if (AnimationUtility.GetEditorCurve(clip, testBinding) != null) {
			return GenerateTransformCurveBindingArray(
				binding.path, "localEulerAngles.", binding.type, 3);
		} else {
			testBinding.propertyName = "localEulerAnglesRaw.x";
			if (clip.legacy &&
			    AnimationUtility.GetEditorCurve(clip, testBinding) == null)
				return GenerateTransformCurveBindingArray(
					binding.path, "localEulerAnglesBaked.", binding.type, 3);

			return GenerateTransformCurveBindingArray(
				binding.path, "localEulerAnglesRaw.", binding.type, 3);
		}
	}
}

static public EditorCurveBinding
RemapAnimationBindingForRotationCurves(EditorCurveBinding curveBinding,
                                       AnimationClip clip) {
	if (!AnimationWindowUtility.IsTransformType(curveBinding.type))
		return curveBinding;

	// Convert rotation binding to valid editor curve binding.
	// Only a single rotation binding (RawEuler, Baked or NonBaked) should be
	// allowed in one clip. RawQuaternions should be converted to appropriate
	// euler bindings if available.
	...
	if (mode != Mode.Undefined) {
		...
		if (mode != Mode.NonBaked) {
			newBinding.propertyName =
				GetPrefixForInterpolation(Mode.NonBaked) + "." + suffix;
			AnimationCurve curve =
				AnimationUtility.GetEditorCurve(clip, newBinding);
			if (curve != null)
				return newBinding;
		}
		...
		if (mode != Mode.Baked) {
			newBinding.propertyName =
				GetPrefixForInterpolation(Mode.Baked) + "." + suffix;
			AnimationCurve curve =
				AnimationUtility.GetEditorCurve(clip, newBinding);
			if (curve != null)
				return newBinding;
		...
		if (mode != Mode.RawEuler) {
			newBinding.propertyName =
				GetPrefixForInterpolation(Mode.RawEuler) + "." + suffix;
			AnimationCurve curve =
				AnimationUtility.GetEditorCurve(clip, newBinding);
			if (curve != null)
				return newBinding;
...
internal static void SetInterpolation(AnimationClip clip,
	...
	if (clip.legacy && newInterpolationMode == Mode.RawEuler) {
		Debug.LogWarning(
			"Warning, Euler Angles interpolation mode is not fully supported for Legacy animation clips. If you mix clips using Euler Angles interpolation with clips using other interpolation modes (using Animation.CrossFade, Animation.Blend or other methods), you will get erroneous results. Use with caution.",
			clip);
	}
	...
	foreach (EditorCurveBinding curveBinding in curveBindings) {
		...
		if (currentMode == Mode.RawQuaternions) {
			Debug.LogWarning("Can't convert quaternion curve: " +
		...
		string newPropertyPath =
			GetPrefixForInterpolation(newInterpolationMode) + '.' +
			ExtractComponentCharacter(curveBinding.propertyName);
	...
	foreach (EditorCurveBinding binding in newCurvesBindings)
		AnimationUtility.SetEditorCurve(
			clip, binding, newCurveDatas[newCurvesBindings.IndexOf(binding)]);
}
}
}
