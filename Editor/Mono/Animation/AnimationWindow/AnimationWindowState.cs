...
internal class AnimationWindowState : ScriptableObject,
  ...
  public enum SnapMode { Disabled = 0, SnapToFrame = 1, SnapToClipFrame = 2 }

  [SerializeField]
  public AnimationWindowHierarchyState
      hierarchyState; // Persistent state of treeview on the left side of window
  [SerializeField]
  public AnimEditor
      animEditor; // Reference to owner of this state. Used to trigger repaints.
  [SerializeField]
  public bool showCurveEditor; // Do we show dopesheet or curves
  [SerializeField]
  public bool linkedWithSequencer; // Toggle Sequencer selection mode.
  [SerializeField]
  private bool
      m_RippleTime; // Toggle ripple time option for curve editor and dopesheet.
  private bool m_RippleTimeClutch; // Toggle ripple time option for curve editor
                                   // and dopesheet.
  [SerializeField]
  private TimeArea m_TimeArea; // Either curveeditor or dopesheet depending on
                               // which is selected
  [SerializeField]
  private AnimationWindowSelectionItem m_EmptySelection;
  [SerializeField]
  private AnimationWindowSelectionItem m_Selection; // Internal selection
  [SerializeField]
  private AnimationWindowKeySelection
      m_KeySelection; // What is selected. Hashes persist cache reload, because
                      // they are from keyframe time+value
  [SerializeField]
  private int m_ActiveKeyframeHash; // Which keyframe is active (selected key
                                    // that user previously interacted with)
  [SerializeField]
  ...
  private static List<AnimationWindowKeyframe>
      s_KeyframeClipboard; // For copy-pasting keyframes
  ...
  // Hash of all the things that require animationWindow to refresh if they
  // change
  private int m_PreviousRefreshHash;

  // Changing m_Refresh means you are ordering a refresh at the next OnGUI().
  // CurvesOnly means that there is no need to refresh the hierarchy, since only
  // the keyframe data changed.
  ...
  public AnimationClip activeAnimationClip {
    get { return selection.animationClip; }
    set {
  ...
  // Previously or currently selected gameobject is considered as the active
  // gameobject
  public GameObject activeGameObject {
    get { return selection.gameObject; }
  }

  // Closes parent to activeGameObject that has Animator component
  public GameObject activeRootGameObject {
    get { return selection.rootGameObject; }
  }

  public Component activeAnimationPlayer {
    get { return selection.animationPlayer; }
  }
  ...
  public bool animatorIsOptimized {
    get { return selection.objectIsOptimized; }
  }
  ...
  public IAnimationWindowControl overrideControlInterface {
    get { return m_OverrideControlInterface; }
  ...
  public bool filterBySelection {
    get { return AnimationWindowOptions.filterBySelection; }
    set {
  ...
  public bool showReadOnly {
    get { return AnimationWindowOptions.showReadOnly; }
    set {
  ...
  public bool rippleTime {
    get { return m_RippleTime || m_RippleTimeClutch; }
    set { m_RippleTime = value; }
  }

  public bool rippleTimeClutch {
    get { return m_RippleTimeClutch; }
    set { m_RippleTimeClutch = value; }
  }

  public bool showFrameRate {
    get { return AnimationWindowOptions.showFrameRate; }
    set { AnimationWindowOptions.showFrameRate = value; }
  }
  ...
  private int GetRefreshHash() {
    return selection.GetRefreshHash() ^
           ...
           (hierarchyState !=
            null ? hierarchyState.GetTallInstancesCount() : 0) ^
           (showCurveEditor ? 1 : 0);
  }

  public void ForceRefresh() { refresh = RefreshType.Everything; }
  ...
  public void OnEnable() {
    ...
    if (m_ControlInterface == null)
      m_ControlInterface = CreateInstance(typeof(AnimationWindowControl))
          as AnimationWindowControl;
  ...
  public RefreshType refresh {
    ...
    // Make sure that if full refresh is already ordered, nobody gets to f***
    // with it
    set {
  ...
  // When curve is modified, we never trigger refresh right away. We order a
  // refresh at later time by setting refresh to appropriate value.
  void CurveWasModified(AnimationClip clip, EditorCurveBinding binding,
    ...
    if (type == AnimationUtility.CurveModifiedType.CurveModified) {
      ...
      if (didFind && !hadPhantom)
        ...
        // New curve was added, so let's save binding and make it active
        // selection when Refresh is called next time
    ...
    // Force repaint to display live animation curve changes from other editor
    // window (like timeline).
    Repaint();
  }
  ...
  public void SaveCurves(AnimationClip clip,
                         ICollection<AnimationWindowCurve> curves,
                         string undoLabel = kEditCurveUndoLabel) {
    if (curves.Count == 0)
      return;
  ...
  private void SaveSelectedKeys(string undoLabel) {
    ...
    foreach (LiveEditCurve snapshot in m_LiveEditSnapshot) {
      ...
      List<AnimationWindowKeyframe> toBeDeleted =
          new List<AnimationWindowKeyframe>();

      // If selected keys are dragged over non-selected keyframe at exact same
      // time, then delete the unselected ones underneath
      foreach (AnimationWindowKeyframe other in snapshot.curve.m_Keyframes) {
        // Keyframe is in selection, skip.
        if (snapshot.selectedKeys.Exists(liveEditKey =>
                                             liveEditKey.key == other))
          continue;

        // There is already a selected keyframe at that time, delete
        // non-selected keyframe.
        if (!snapshot.selectedKeys.Exists(
                liveEditKey =>
                    AnimationKeyTime.Time(liveEditKey.key.time, frameRate)
                        .frame ==
                    AnimationKeyTime.Time(other.time, frameRate).frame))
          continue;
  ...
  public bool previewing {
    get { return controlInterface.previewing; }
  }

  public bool canPreview {
    get { return controlInterface.canPreview; }
  }
  ...
  public void StopPreview() { controlInterface.StopPreview(); }

  public bool recording {
    get { return controlInterface.recording; }
  }

  public bool canRecord {
    get { return controlInterface.canRecord; }
  }
  ...
  public void StopRecording() { controlInterface.StopRecording(); }

  public bool playing {
    get { return controlInterface.playing; }
  }

  public void StartPlayback() { controlInterface.StartPlayback(); }

  public void StopPlayback() { controlInterface.StopPlayback(); }

  public void ResampleAnimation() { controlInterface.ResampleAnimation(); }
  ...
  private void UpdateSelectionFilter() {
    m_SelectionFilter =
        (filterBySelection) ? (int[]) Selection.instanceIDs.Clone() : null;
  }
      ...
      if (m_ActiveCurvesCache == null) {
        ...
        if (hierarchyState != null && hierarchyData != null) {
          foreach (int id in hierarchyState.selectedIDs) {
            ...
            AnimationWindowHierarchyNode hierarchyNode =
                node as AnimationWindowHierarchyNode;
      ...
      if (m_ActiveCurveWrappersCache == null || m_ActiveCurvesCache == null) {
        ...
        foreach (AnimationWindowCurve curve in activeCurves)
          if (!curve.isDiscreteCurve)
            activeCurveWrappers.Add(
                AnimationWindowUtility.GetCurveWrapper(curve, curve.clip));

        // If there are no active curves, we would end up with empty curve
        // editor so we just give all curves insteads
      ...
      if (m_dopelinesCache == null) {
        ...
        if (hierarchyData != null) {
          foreach (TreeViewItem node in hierarchyData.GetRows()) {
            ...
            if (hierarchyNode == null ||
                hierarchyNode is AnimationWindowHierarchyAddButtonNode)
              continue;
            ...
            dopeLine.hasChildren =
                !(hierarchyNode is AnimationWindowHierarchyPropertyNode);
            dopeLine.isMasterDopeline =
                node is AnimationWindowHierarchyMasterNode;
  ...
  public List<AnimationWindowHierarchyNode> selectedHierarchyNodes {
    get {
      List<AnimationWindowHierarchyNode> selectedHierarchyNodes =
          new List<AnimationWindowHierarchyNode>();

      if (activeAnimationClip != null && hierarchyData != null) {
        foreach (int id in hierarchyState.selectedIDs) {
          AnimationWindowHierarchyNode hierarchyNode =
              (AnimationWindowHierarchyNode) hierarchyData.FindItem(id);
  ...
  public void StartLiveEdit() {
    ...
    foreach (AnimationWindowKeyframe selectedKey in selectedKeys) {
      if (!m_LiveEditSnapshot.Exists(snapshot =>
  ...
  public bool InLiveEdit() { return m_LiveEditSnapshot != null; }
  ...
  public void MoveSelectedKeys(float deltaTime, bool snapToFrame) {
    ...
    foreach (LiveEditCurve snapshot in m_LiveEditSnapshot) {
      foreach (LiveEditKeyframe liveEditKey in snapshot.selectedKeys) {
        if (snapshot.curve.animationIsEditable) {
          liveEditKey.key.time =
              Mathf.Max(liveEditKey.keySnapshot.time + deltaTime, 0f);

          if (snapToFrame)
            liveEditKey.key.time = SnapToFrame(liveEditKey.key.time,
                                               snapshot.curve.clip.frameRate);
        }
  ...
  public void TransformSelectedKeys(Matrix4x4 matrix, bool flipX, bool flipY,
                                    bool snapToFrame) {
    bool inLiveEdit = InLiveEdit();
    if (!inLiveEdit)
      StartLiveEdit();
    ...
    foreach (LiveEditCurve snapshot in m_LiveEditSnapshot) {
      foreach (LiveEditKeyframe liveEditKey in snapshot.selectedKeys) {
        if (snapshot.curve.animationIsEditable) {
          ...
          Vector3 v = new Vector3(liveEditKey.keySnapshot.time,
                                  liveEditKey.keySnapshot.isPPtrCurve ? 0f
                                  : (float) liveEditKey.keySnapshot.value, 0f);
          v = matrix.MultiplyPoint3x4(v);

          liveEditKey.key.time = Mathf.Max(
              (snapToFrame) ? SnapToFrame(v.x, snapshot.curve.clip.frameRate)
                            : v.x,
              0f);

          if (flipX) {
            liveEditKey.key.inTangent =
                (liveEditKey.keySnapshot.outTangent != Mathf.Infinity)
                    ? -liveEditKey.keySnapshot.outTangent
                    : Mathf.Infinity;
            liveEditKey.key.outTangent =
                (liveEditKey.keySnapshot.inTangent != Mathf.Infinity)
                    ? -liveEditKey.keySnapshot.inTangent
                    : Mathf.Infinity;
            ...
            if (liveEditKey.keySnapshot.weightedMode == WeightedMode.In)
              ...
              liveEditKey.key.weightedMode =
                  liveEditKey.keySnapshot.weightedMode;
          ...
          if (!liveEditKey.key.isPPtrCurve) {
            ...
            if (flipY) {
              liveEditKey.key.inTangent =
                  (liveEditKey.key.inTangent != Mathf.Infinity)
                      ? -liveEditKey.key.inTangent
                      : Mathf.Infinity;
              liveEditKey.key.outTangent =
                  (liveEditKey.key.outTangent != Mathf.Infinity)
                      ? -liveEditKey.key.outTangent
                      : Mathf.Infinity;
            }
          }
  ...
  public void TransformRippleKeys(Matrix4x4 matrix, float t1, float t2,
                                  bool flipX, bool snapToFrame) {
    bool inLiveEdit = InLiveEdit();
    if (!inLiveEdit)
      StartLiveEdit();
      ...
      foreach (LiveEditKeyframe liveEditKey in snapshot.unselectedKeys) {
        if (liveEditKey.keySnapshot.time > t2) {
          ...
          if (dt > 0f) {
            ...
            liveEditKey.key.time = Mathf.Max(
                (snapToFrame)
                    ? SnapToFrame(newTime, snapshot.curve.clip.frameRate)
                    : newTime,
                0f);
          } else {
  ...
  public void PasteKeys() {
    ...
    List<AnimationWindowCurve> clipboardCurves =
        new List<AnimationWindowCurve>();
    foreach (AnimationWindowKeyframe keyframe in s_KeyframeClipboard)
      if (!clipboardCurves.Any() || clipboardCurves.Last() != keyframe.curve)
        clipboardCurves.Add(keyframe.curve);

    // If we have equal number of target and source curves, then match by index.
    // If not, then try to match with AnimationWindowUtility.BestMatchForPaste.
    ...
    foreach (AnimationWindowKeyframe keyframe in s_KeyframeClipboard) {
      ...
      AnimationWindowKeyframe newKeyframe =
          new AnimationWindowKeyframe(keyframe);

      if (matchCurveByIndex)
        newKeyframe.curve = activeCurves[targetIndex];
      else
        newKeyframe.curve = AnimationWindowUtility.BestMatchForPaste(
            newKeyframe.curve.binding, clipboardCurves, activeCurves);

      if (newKeyframe.curve == null) // Paste as new curve
      {
        // Curves are selected in the animation window hierarchy.  Since we
        // couldn't find a proper match, create a new curve in first selected
        // clip in active curves.
        if (activeCurves.Count > 0) {
          AnimationWindowCurve firstCurve = activeCurves[0];
          if (firstCurve.animationIsEditable) {
            newKeyframe.curve = new AnimationWindowCurve(
                firstCurve.clip, keyframe.curve.binding,
                keyframe.curve.valueType);
        ...
        // If nothing is selected, create a new curve in first selected clip.
          ...
          if (selection.animationIsEditable) {
            newKeyframe.curve = new AnimationWindowCurve(
                selection.animationClip, keyframe.curve.binding,
                keyframe.curve.valueType);
      ...
      newKeyframe.time = AnimationKeyTime
                             .Time(newKeyframe.time + currentTime,
                                   newKeyframe.curve.clip.frameRate)
                             .timeRound;

      //  Only allow pasting of key frame from numerical curves to numerical
      //  curves or from pptr curves to pptr curves.
      if ((newKeyframe.time >= 0.0f) && (newKeyframe.curve != null) &&
          (newKeyframe.curve.isPPtrCurve == keyframe.curve.isPPtrCurve)) {
        if (newKeyframe.curve.HasKeyframe(AnimationKeyTime.Time(
        ...
        SaveCurve(newKeyframe.curve.clip, newKeyframe.curve,
                  kEditCurveUndoLabel);
  ...
  private void ReloadModifiedDopelineCache() {
    ...
    for (int i = 0; i < m_dopelinesCache.Count; ++i) {
      ...
      for (int j = 0; j < curves.Length; ++j) {
        if (m_ModifiedCurves.Contains(curves [j]
                                          .GetHashCode())) {
  ...
  private void ReloadModifiedCurveWrapperCache() {
    ...
    Dictionary<int, AnimationWindowCurve> updateList =
        new Dictionary<int, AnimationWindowCurve>();
    ...
    for (int i = 0; i < m_ActiveCurveWrappersCache.Length; ++i) {
      ...
      if (m_ModifiedCurves.Contains(curveWrapper.id)) {
        AnimationWindowCurve curve =
            allCurves.Find(c => c.GetHashCode() == curveWrapper.id);
        if (curve != null) {
    ...
    for (int i = 0; i < updateList.Count; ++i) {
      ...
      m_ActiveCurveWrappersCache[entry.Key] =
          AnimationWindowUtility.GetCurveWrapper(entry.Value, entry.Value.clip);
    }
  }
  ...
  // This means that hierarchynodes and dopeline(s) for new curve are already
  // available.
  private void OnNewCurveAdded(EditorCurveBinding newCurve) {
    ...
    string groupPropertyName =
        AnimationWindowUtility.GetPropertyGroupName(newCurve.propertyName);
      ...
      foreach (AnimationWindowHierarchyNode node in hierarchyData.GetRows()) {
        if (node.path != newCurve.path ||
            ...
            (node.propertyName != propertyName &&
             node.propertyName != groupPropertyName))
          continue;
    ...
    //  Values do not change whenever a new curve is added, so we force an
    //  inspector update here.
    controlInterface.ResampleAnimation();
  ...
  public List<AnimationWindowKeyframe>
  GetAggregateKeys(AnimationWindowHierarchyNode hierarchyNode) {
    DopeLine dopeline =
        dopelines.FirstOrDefault(e => e.hierarchyNodeID == hierarchyNode.id);
    if (dopeline == null)
      return null;
  ...
  public void HandleHierarchySelectionChanged(int[] selectedInstanceIDs,
                                              bool triggerSceneSelectionSync) {
    m_ActiveCurvesCache = null;
  ...
  public void SelectHierarchyItem(DopeLine dopeline, bool additive,
                                  bool triggerSceneSelectionSync) {
    SelectHierarchyItem(dopeline.hierarchyNodeID, additive,
                        triggerSceneSelectionSync);
  }

  public void SelectHierarchyItem(int hierarchyNodeID, bool additive,
                                  bool triggerSceneSelectionSync) {
    if (!additive)
      ClearHierarchySelection();
    ...
    // We need to manually trigger this event, because injecting data to
    // m_SelectedInstanceIDs directly doesn't trigger one via TreeView
    HandleHierarchySelectionChanged(selectedInstanceIDs,
                                    triggerSceneSelectionSync);
  }

  public void SelectHierarchyItems(IEnumerable<int> hierarchyNodeIDs,
                                   bool additive,
                                   bool triggerSceneSelectionSync) {
    if (!additive)
      ClearHierarchySelection();
  ...
  public HashSet<int>
  GetAffectedHierarchyIDs(List<AnimationWindowKeyframe> keyframes) {
    HashSet<int> hierarchyIDs = new HashSet<int>();

    foreach (AnimationWindowKeyframe keyframe in keyframes) {
      var curve = keyframe.curve;

      int hierarchyID = AnimationWindowUtility.GetPropertyNodeID(
          0, curve.path, curve.type, curve.propertyName);
      if (hierarchyIDs.Add(hierarchyID)) {
        string propertyGroupName =
            AnimationWindowUtility.GetPropertyGroupName(curve.propertyName);
        hierarchyIDs.Add(AnimationWindowUtility.GetPropertyNodeID(
            0, curve.path, curve.type, propertyGroupName));
      }
    }
  ...
  public List<AnimationWindowCurve>
  GetAffectedCurves(List<AnimationWindowKeyframe> keyframes) {
    List<AnimationWindowCurve> affectedCurves =
        new List<AnimationWindowCurve>();
  ...
  private void SyncSceneSelection(int[] selectedNodeIDs) {
    ...
    foreach (var selectedNodeID in selectedNodeIDs) {
      ...
      AnimationWindowHierarchyNode node = hierarchyData.FindItem(selectedNodeID)
                                              as AnimationWindowHierarchyNode;
      ...
      // In the case of nested animation component, we don't want to sync the
      // scene selection (case 569506) When selection changes, animation window
      // will always pick nearest animator component in terms of hierarchy depth
      // Automatically syncinc scene selection in nested scenarios would cause
      // unintuitive clip & animation change for animation window so we check
      // for it and deny sync if necessary

      if (t != null && rootGameObject != null &&
          activeAnimationPlayer ==
              AnimationWindowUtility
                  .GetClosestAnimationPlayerComponentInParents(t))
  ...
  public float clipFrameRate {
    ...
    set {
      // @TODO: Changing the clip in AnimationWindowState.frame rate feels a bit
      // intrusive Should probably be done explicitly from the UI and not go
      // through AnimationWindowState...
      if (activeAnimationClip != null && value > 0 && value <= 10000) {
        // Clear selection and save empty selection snapshot for undo
        // consistency.
        ClearKeySelections();
        SaveKeySelection(kEditCurveUndoLabel);
        ...
        AnimationEvent[] events =
            AnimationUtility.GetAnimationEvents(activeAnimationClip);
  ...
  public AnimationKeyTime time {
    get { return controlInterface.time; }
  }
  public int currentFrame {
    get { return time.frame; }
    set { controlInterface.GoToFrame(value); }
  }
  public float currentTime {
    get { return time.time; }
    set { controlInterface.GoToTime(value); }
  }

  public TimeArea.TimeFormat timeFormat {
    get { return AnimationWindowOptions.timeFormat; }
  ...
  public float minVisibleTime {
    get { return m_TimeArea.shownArea.xMin; }
  }
  public float maxVisibleTime {
    get { return m_TimeArea.shownArea.xMax; }
  }
  public float visibleTimeSpan {
    get { return maxVisibleTime - minVisibleTime; }
  }
  public float minVisibleFrame {
    get { return minVisibleTime * frameRate; }
  }
  public float maxVisibleFrame {
    get { return maxVisibleTime * frameRate; }
  }
  public float visibleFrameSpan {
    get { return visibleTimeSpan * frameRate; }
  }
  ...
  public Vector2 timeRange {
    get {
      if (activeAnimationClip != null)
        return new Vector2(activeAnimationClip.startTime,
                           activeAnimationClip.stopTime);
  ...
  public string FormatFrame(int frame, int frameDigits) {
    return (frame / (int) frameRate) + ":" +
           (frame % frameRate).ToString().PadLeft(frameDigits, '0');
  }

  //@TODO: Remove. Replace with animationkeytime
  public float TimeToFrame(float time) { return time * frameRate; }

  //@TODO: Remove. Replace with animationkeytime
  public float FrameToTime(float frame) { return frame / frameRate; }
