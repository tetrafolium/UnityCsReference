...
internal class AnimEditorOverlay {
  ...
  public void Initialize() {
    if (m_PlayHeadCursor == null) {
      m_PlayHeadCursor =
          new TimeCursorManipulator(AnimationWindowStyles.playHead);

      m_PlayHeadCursor.onStartDrag +=
      ...
      m_PlayHeadCursor.onDrag += (AnimationWindowManipulator manipulator,
                                  Event evt) => { return OnDragPlayHead(evt); };
      m_PlayHeadCursor.onEndDrag +=
          (AnimationWindowManipulator manipulator, Event evt) => {
  ...
  public void OnGUI(Rect rect, Rect contentRect) {
    ...
    m_PlayHeadCursor.OnGUI(m_Rect,
                           m_Rect.xMin + TimeToPixel(state.currentTime));
  }
  ...
  public float MousePositionToTime(Event evt) {
    ...
    float time =
        Mathf.Max(((evt.mousePosition.x / width) * state.visibleTimeSpan +
                   state.minVisibleTime),
                  0);
