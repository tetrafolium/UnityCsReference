...
internal class TransitionPreview {
  ...
  public bool mustResample {
    set { m_MustResample = value; }
    get { return m_MustResample; }
  }
  ...
  class TransitionInfo {
    ...
    public bool IsEqual(TransitionInfo info) {
      return m_SrcState == info.m_SrcState && m_DstState == info.m_DstState &&
             Mathf.Approximately(m_TransitionDuration,
                                 info.m_TransitionDuration) &&
             Mathf.Approximately(m_TransitionOffset, info.m_TransitionOffset) &&
             Mathf.Approximately(m_ExitTime, info.m_ExitTime);
    }

    public TransitionInfo() { Init(); }
    ...
    public void Set(AnimatorStateTransition transition, AnimatorState srcState,
  ...
  private void CopyTransitionForPreview(AnimatorStateTransition src,
                                        ref AnimatorStateTransition dst) {
    if (src != null) {
  ...
  List<TimelineControl.PivotSample> m_SrcPivotList =
      new List<TimelineControl.PivotSample>();
  List<TimelineControl.PivotSample> m_DstPivotList =
      new List<TimelineControl.PivotSample>();

  private bool MustResample(TransitionInfo info) {
    bool isInPlayback =
        m_AvatarPreview != null && m_AvatarPreview.Animator != null &&
        m_AvatarPreview.Animator.recorderMode == AnimatorRecorderMode.Playback;
    ...
    if (m_Controller) {
      ...
      for (int i = 0; i < parameterCount; i++) {
        ...
        int parameterInfoIndex =
            FindParameterInfo(m_ParameterInfoList, parameterName);

        if (parameterInfoIndex != -1) {
          m_AvatarPreview.Animator.SetFloat(
              parameterName, m_ParameterInfoList[parameterInfoIndex].m_Value);
        }
      }
    }
  }

  private void ResampleTransition(AnimatorStateTransition transition,
                                  AvatarMask layerMask, TransitionInfo info,
                                  Animator previewObject) {
    m_IsResampling = true;
    ...
    Init(previewObject, sourceStateMotion != null ? sourceStateMotion
         : m_RefDstState.motion);
    ...
    // since transform might change during sampling, and could alter the default
    // valuesarray, and break recording
    ...
    float nextStateDuration =
        m_AvatarPreview.Animator.GetCurrentAnimatorStateInfo(m_LayerIndex)
            .length;
    ...
    float maxDuration =
        (currentStateDuration * m_RefTransition.exitTime) +
        (m_Transition.duration * (m_RefTransition.hasFixedDuration ? 1.0f
                                  : currentStateDuration)) +
        nextStateDuration;

    // case 546812 disable previewer if the duration is too big, otherwise it
    // hang Unity. 2000.0f is an arbitrary choice, it can be increase if needed.
    // in some case we got a m_Transition.duration == Infinity, bail out before
    // unity hang.
    if (maxDuration > 2000.0f) {
      Debug.LogWarning(
          "Transition duration is longer than 2000 second, Disabling previewer.");
      m_ValidTransition = false;
      m_IsResampling = false;
      return;
    }

    float effectiveCurrentStatetime =
        m_RefTransition.exitTime > 0
            ? currentStateDuration * m_RefTransition.exitTime
            : currentStateDuration;
    // We want 30 samples/sec, maxed at 300 sample for very long state, and very
    // short animation like 1 frame should at least get 5 sample
    float currentStateStepTime =
        effectiveCurrentStatetime > 0
            ? Mathf.Min(
                  Mathf.Max(effectiveCurrentStatetime / 300.0f, 1.0f / 30.0f),
                  effectiveCurrentStatetime / 5.0f)
            : 1.0f / 30.0f;
    float nextStateStepTime =
        nextStateDuration > 0
            ? Mathf.Min(Mathf.Max(nextStateDuration / 300.0f, 1.0f / 30.0f),
                        nextStateDuration / 5.0f)
            : 1.0f / 30.0f;

    currentStateStepTime =
        Mathf.Max(currentStateStepTime, maxDuration / 600.0f);
    ...
    // For transitions with exit time == 0, skip to end of clip so transition
    ...
    if (Mathf.Approximately(m_LeftStateWeightB, m_LeftStateWeightA) ||
        Mathf.Approximately(m_RightStateWeightB, m_RightStateWeightA)) {
      Debug.LogWarning(
    ...
    float leftDuration = (m_LeftStateTimeB - m_LeftStateTimeA) /
                         (m_LeftStateWeightB - m_LeftStateWeightA);
    float rightDuration = (m_RightStateTimeB - m_RightStateTimeA) /
                          (m_RightStateWeightB - m_RightStateWeightA);
    ...
    if (resetTimeSettings) {
      m_Timeline.Time = m_Timeline.StartTime =
  ...
  public void SetTransition(AnimatorStateTransition transition,
                            AnimatorState sourceState,
                            AnimatorState destinationState,
                            AnimatorControllerLayer srcLayer,
                            Animator previewObject) {
  ...
  void CreateController() {
    if (m_Controller == null && m_AvatarPreview != null &&
        m_AvatarPreview.Animator != null && m_RefTransition != null) {
      // controller
      m_LayerIndex = 0;
      ...
      if (m_LayerMask != null) {
        for (AvatarMaskBodyPart i = 0;
             i < AvatarMaskBodyPart.LastBodyPart && isDefaultMask; i++)
      ...
      if (m_SrcMotion && m_SrcMotion is BlendTree) {
        ...
        for (int i = 0; i < leftBlendTree.recursiveBlendParameterCount; i++) {
          ...
          if (m_Controller.IndexOfParameter(blendValueName) == -1) {
            m_Controller.AddParameter(blendValueName,
                                      AnimatorControllerParameterType.Float);
            m_ParameterMinMax.Add(
                new Vector2(leftBlendTree.GetRecursiveBlendParameterMin(i),
                            leftBlendTree.GetRecursiveBlendParameterMax(i)));
          }
        }
      }
        ...
        for (int i = 0; i < rightBlendTree.recursiveBlendParameterCount; i++) {
          ...
          if (parameterIndex == -1) {
            m_Controller.AddParameter(blendValueName,
                                      AnimatorControllerParameterType.Float);
            m_ParameterMinMax.Add(
                new Vector2(rightBlendTree.GetRecursiveBlendParameterMin(i),
                            rightBlendTree.GetRecursiveBlendParameterMax(i)));
          } else {
            m_ParameterMinMax[parameterIndex] = new Vector2(
                Mathf.Min(rightBlendTree.GetRecursiveBlendParameterMin(i),
                          m_ParameterMinMax [parameterIndex]
                          [0]),
                Mathf.Max(rightBlendTree.GetRecursiveBlendParameterMax(i),
                          m_ParameterMinMax [parameterIndex]
                          [1]));
          }
        }
  ...
  public void DoTransitionPreview() {
    ...
    if (parameters.Length > 0) {
      m_ShowBlendValue = EditorGUILayout.Foldout(m_ShowBlendValue,
                                                 "BlendTree Parameters", true);

      if (m_ShowBlendValue) {
        for (int i = 0; i < parameters.Length; i++) {
          AnimatorControllerParameter parameter = m_Controller.parameters[i];
          float value = m_ParameterInfoList[i].m_Value;
          float newValue = EditorGUILayout.Slider(parameter.name, value,
                                                  m_ParameterMinMax [i]
                                                  [0],
                                                  m_ParameterMinMax [i]
                                                  [1]);
  ...
  private void DoTimeline() {
    ...
    // get local durations
    float srcStateDuration = (m_LeftStateTimeB - m_LeftStateTimeA) /
                             (m_LeftStateWeightB - m_LeftStateWeightA);
    float dstStateDuration = (m_RightStateTimeB - m_RightStateTimeA) /
                             (m_RightStateWeightB - m_RightStateWeightA);
    ...
    m_Timeline.TransitionStartTime =
        m_RefTransition.exitTime * srcStateDuration;
    m_Timeline.TransitionStopTime =
        m_Timeline.TransitionStartTime + transitionDuration;

    m_Timeline.Time = m_AvatarPreview.timeControl.currentTime;

    m_Timeline.DstStartTime = m_Timeline.TransitionStartTime -
                              m_RefTransition.offset * dstStateDuration;
    ...
    if (m_Timeline.TransitionStopTime == Mathf.Infinity)
      m_Timeline.TransitionStopTime =
          Mathf.Min(m_Timeline.DstStopTime, m_Timeline.SrcStopTime);
    ...
    // Do the timeline
    Rect previewRect =
        EditorGUILayout.GetControlRect(false, 150, EditorStyles.label);
    ...
    if (EditorGUI.EndChangeCheck()) {
      if (changedData) {
        ...
        m_RefTransition.exitTime =
            m_Timeline.TransitionStartTime / m_Timeline.SrcDuration;
        m_RefTransition.duration = m_Timeline.TransitionDuration /
                                   (m_RefTransition.hasFixedDuration ? 1.0f
                                    : m_Timeline.SrcDuration);
        m_RefTransition.offset =
            (m_Timeline.TransitionStartTime - m_Timeline.DstStartTime) /
            m_Timeline.DstDuration;
      }

      m_AvatarPreview.timeControl.nextCurrentTime =
          Mathf.Clamp(m_Timeline.Time, 0, m_AvatarPreview.timeControl.stopTime);
    }
  }
  ...
  public void OnInteractivePreviewGUI(Rect r, GUIStyle background) {
    if (m_AvatarPreview != null && m_Controller != null) {
      if (m_LastEvalTime != m_AvatarPreview.timeControl.currentTime &&
          Event.current.type == EventType.Repaint) {
        m_AvatarPreview.Animator.playbackTime =
            m_AvatarPreview.timeControl.currentTime;
