...
namespace UnityEditor.Animations {
  ...
  public class GameObjectRecorder : Object {
    readonly static CurveFilterOptions k_DefaultCurveFilterOptions =
        new CurveFilterOptions(){
            unrollRotation = true, keyframeReduction = true,
            positionError = 0.5f,  rotationError = 0.5f,
            scaleError = 0.5f,     floatError = 0.5f};

    public GameObjectRecorder(GameObject root) { Internal_Create(this, root); }

    public void BindComponentsOfType<T>(GameObject target,
                                        bool recursive) where T : Component {
      BindComponentsOfType(target, typeof(T), recursive);
    }

    public void BindComponentsOfType(GameObject target, Type componentType,
                                     bool recursive) {
    ...
    Internal_Create([ Writable ] GameObjectRecorder self,
    ...
    [NativeMethod("Bind")] extern private void
    BindInternal(EditorCurveBinding binding);
    ...
    public void SaveToClip(AnimationClip clip) { SaveToClip(clip, 60.0f); }
    ...
    public void SaveToClip(AnimationClip clip, float fps) {
      ...
      AnimationUtility.onCurveWasModified?.Invoke(
    ...
    public void SaveToClip(AnimationClip clip, float fps,
      ...
      if (filterOptions.keyframeReduction) {
        if (filterOptions.positionError < 0 ||
            filterOptions.rotationError < 0 || filterOptions.scaleError < 0 ||
            filterOptions.floatError < 0)
    ...
    [NativeMethod("SaveToClip")] extern void
    SaveToClipInternal(AnimationClip clip, float fps,
                       CurveFilterOptions filterOptions);
    ...
    [Obsolete("The GameObjectRecorder constructor now takes a root GameObject",
              true)]
    ...
    [System.ComponentModel.EditorBrowsable(
        System.ComponentModel.EditorBrowsableState.Never)]
    [Obsolete(
        "BindComponent() using a System::Type is obsolete, use BindComponentsOfType() instead (UnityUpgradable) -> BindComponentsOfType(*)",
        true)]
    public void BindComponent(GameObject target, Type componentType,
                              bool recursive) {}

    [System.ComponentModel.EditorBrowsable(
        System.ComponentModel.EditorBrowsableState.Never)]
    [Obsolete(
        "\"BindComponent<T>() where T : Component\" is obsolete, use BindComponentsOfType<T>() instead (UnityUpgradable) -> BindComponentsOfType<T>(*)",
        true)]
    public void BindComponent<T>(GameObject target, bool recursive) where T
        : Component {}
  }
}
