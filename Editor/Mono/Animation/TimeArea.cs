...
internal const int kTickRulerDistMin =
	3; // min distance between ruler tick marks before they disappear
           // completely

internal const int kTickRulerDistFull =
	80; // distance between ruler tick marks where they gain full strength
internal const int kTickRulerDistLabel =
	40; // min distance between ruler tick mark labels
internal const float kTickRulerHeightMax =
	0.7f; // height of the ruler tick marks when they are highest

internal const float kTickRulerFatThreshold =
	0.5f; // size of ruler tick marks at which they begin getting fatter
...
public void SetTickMarkerRanges() {
	hTicks.SetRanges(shownArea.xMin, shownArea.xMax, drawRect.xMin,
	                 drawRect.xMax);
	vTicks.SetRanges(shownArea.yMin, shownArea.yMax, drawRect.yMin,
	                 drawRect.yMax);
}
...
public void TimeRuler(Rect position, float frameRate, bool labels,
                      bool useEntireHeight, float alpha,
			...
			for (int i = 0; i < m_TickCache.Count; i++) {
				...
				float height = useEntireHeight
				   ? position.height : position.height *
				               Mathf.Min(1, strength) * kTickRulerHeightMax;
				float x = FrameToPixel(frame, frameRate, position, cachedShowArea);

				// Draw line
				DrawVerticalLineFast(
					x, position.height - height + 0.5f, position.height - 0.5f,
					new Color(1, 1, 1, strength / kTickRulerFatThreshold) *
					baseColor);
			}
		}
	...
	if (labels) {
		...
		for (int i = 0; i < m_TickCache.Count; i++) {
			...
			GUI.Label(new Rect(labelpos + 3, -1, 40, 20), label,
...
public static void DrawPlayhead(float x, float yMin, float yMax,
	...
	Color lineColor =
		timeAreaStyles.playhead.normal.textColor.AlphaMultiplied(alpha);
	if (thickness > 1f) {
		Rect labelRect =
			Rect.MinMaxRect(x - halfThickness, yMin, x + halfThickness, yMax);
...
public static void DrawVerticalLine(float x, float minY, float maxY,
                                    Color color) {
	if (Event.current.type != EventType.Repaint)
		return;
...
public enum TimeRulerDragMode { None, Start, End, Dragging, Cancel }

static float s_OriginalTime;
static float s_PickOffset;

public TimeRulerDragMode BrowseRuler(Rect position, ref float time,
                                     float frameRate, bool pickAnywhere,
                                     GUIStyle thumbStyle) {
	int id = GUIUtility.GetControlID(3126789, FocusType.Passive);
	return BrowseRuler(position, id, ref time, frameRate, pickAnywhere,
	                   thumbStyle);
}

public TimeRulerDragMode BrowseRuler(Rect position, int id, ref float time,
                                     float frameRate, bool pickAnywhere,
                                     GUIStyle thumbStyle) {
	...
	if (time != -1) {
		pickRect.x =
			Mathf.Round(TimeToPixel(time, position)) - thumbStyle.overflow.left;
	...
	case EventType.Repaint:
		if (time != -1) {
			...
			thumbStyle.Draw(pickRect, id == GUIUtility.hotControl,
			                hover || id == GUIUtility.hotControl, false, false);
		}
		break;
	case EventType.MouseDown:
		if (pickRect.Contains(evt.mousePosition)) {
			...
			float newT = SnapTimeToWholeFPS(
				PixelToTime(evt.mousePosition.x, position), frameRate);
			s_OriginalTime = time;
			if (newT != time)
				GUI.changed = true;
			time = newT;
	...
	case EventType.MouseDrag:
		if (GUIUtility.hotControl == id) {
			float newT = SnapTimeToWholeFPS(
				PixelToTime(evt.mousePosition.x - s_PickOffset, position),
				frameRate);
			if (newT != time)
				GUI.changed = true;
			time = newT;
...
private float FrameToPixel(float i, float frameRate, Rect rect,
                           Rect theShownArea) {
	return (i - theShownArea.xMin * frameRate) * rect.width /
	       (theShownArea.width * frameRate);
}
...
public float TimeField(Rect rect, int id, float time, float frameRate,
                       TimeFormat timeFormat) {
	if (timeFormat == TimeFormat.None) {
		float newTime = EditorGUI.DoFloatField(
			EditorGUI.s_RecycledEditor, rect, new Rect(0, 0, 0, 0), id, time,
			EditorGUI.kFloatFieldFormatString, EditorStyles.numberField, false);
	...
	if (timeFormat == TimeFormat.Frame) {
		...
		int newFrame = EditorGUI.DoIntField(
			EditorGUI.s_RecycledEditor, rect, new Rect(0, 0, 0, 0), id, frame,
			EditorGUI.kIntFieldFormatString, EditorStyles.numberField, false, 0f);
	...
	} else // if (timeFormat == TimeFormat.TimeFrame)
	{
		...
		bool changed;
		str = EditorGUI.DoTextField(EditorGUI.s_RecycledEditor, id, rect, str,
		                            EditorStyles.numberField, allowedCharacters,
		...
		if (changed) {
			if (GUIUtility.keyboardControl == id) {
				...
				if (index >= 0) {
					...
					if (int.TryParse(timeStr, out timeValue) &&
					    int.TryParse(frameStr, out frameValue)) {
						float newTime =
							(float) timeValue + (float) frameValue / frameRate;
						return newTime;
					}
				}
...
public float ValueField(Rect rect, int id, float value) {
	float newValue = EditorGUI.DoFloatField(
		EditorGUI.s_RecycledEditor, rect, new Rect(0, 0, 0, 0), id, value,
		EditorGUI.kFloatFieldFormatString, EditorStyles.numberField, false);
...
public string FormatTime(float time, float frameRate, TimeFormat timeFormat) {
	if (timeFormat == TimeFormat.None) {
		...
		if (frameRate != 0)
			hDecimals =
				MathUtils.GetNumberOfDecimalsForMinimumDifference(1 / frameRate);
		else
			hDecimals = MathUtils.GetNumberOfDecimalsForMinimumDifference(
				shownArea.width / drawRect.width);
	...
	if (timeFormat == TimeFormat.TimeFrame) {
		int frameDigits =
			frameRate != 0 ? ((int) frameRate - 1).ToString().Length : 1;
...
public virtual string FormatTickTime(float time, float frameRate,
                                     TimeFormat timeFormat) {
	return FormatTime(time, frameRate, timeFormat);
}

public string FormatValue(float value) {
	int vDecimals = MathUtils.GetNumberOfDecimalsForMinimumDifference(
		shownArea.height / drawRect.height);
...
public void DrawTimeOnSlider(float time, Color c, float maxTime,
                             float leftSidePadding = 0,
                             float rightSidePadding = 0) {
	const float maxTimeFudgeFactor = 3;
	if (!hSlider)
		return;

	if (styles.horizontalScrollbar == null)
		styles.InitGUIStyles(false, allowSliderZoomHorizontal,
		                     allowSliderZoomVertical);

	var inMin = TimeToPixel(0, rect); // Assume 0 minTime
	var inMax = TimeToPixel(maxTime, rect);
	var outMin = TimeToPixel(shownAreaInsideMargins.xMin, rect) +
	             styles.horizontalScrollbarLeftButton.fixedWidth +
	             leftSidePadding;
	var outMax =
		TimeToPixel(shownAreaInsideMargins.xMax, rect) -
		(styles.horizontalScrollbarRightButton.fixedWidth + rightSidePadding);
	var x = (TimeToPixel(time, rect) - inMin) * (outMax - outMin) /
	        (inMax - inMin) +
	        outMin;
	if (x > rect.xMax - (styles.horizontalScrollbarLeftButton.fixedWidth +
	                     leftSidePadding + maxTimeFudgeFactor))
		return;
	var inset = styles.sliderWidth - styles.visualSliderWidth;
	var otherInset = (vSlider && hSlider) ? inset : 0;
	var hRangeSliderRect =
