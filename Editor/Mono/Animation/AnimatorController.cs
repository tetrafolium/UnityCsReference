...
OnInvalidateAnimatorController(AnimatorController controller) {
	if (controller.OnAnimatorControllerDirty != null)
		controller.OnAnimatorControllerDirty();
}

internal
AnimatorStateMachine FindEffectiveRootStateMachine(int layerIndex) {
...
private void
RemoveLayerInternal(int index, ref AnimatorControllerLayer[] layerVector) {
	if (layerVector[index].syncedLayerIndex == -1 &&
	    layerVector[index].stateMachine != null) {
		undoHandler.DoUndo(layerVector[index].stateMachine, "Layer removed");
		layerVector[index].stateMachine.Clear();
		if (MecanimUtilities.AreSameAsset(this,
		                                  layerVector[index].stateMachine))
...
public void AddParameter(string name,
	...
	AnimatorControllerParameter newParameter =
...
// We cannot call AddMotion from native code, because there are multiple
// signatures of that method, creating ambiguity.
[RequiredByNativeCode]
...
public AnimatorState AddMotion(Motion motion, int layerIndex) {
	AnimatorState state =
		layers[layerIndex].stateMachine.AddState(motion.name);
	state.motion = motion;
	return state;
}

public AnimatorState CreateBlendTreeInController(string name,
                                                 out BlendTree tree) {
...
public static AnimationClip AllocateAnimatorClip(string name) {
	var clip =
		UnityEditorInternal.AnimationWindowUtility.AllocateAndSetupClip(true);
	clip.name = name;
	return clip;
}

[RequiredByNativeCode]
internal static AnimatorController
CreateAnimatorControllerForClip(AnimationClip clip,
	...
	string name = System.Text.RegularExpressions.Regex.Replace(
		animatedObject.name, @"[\\\./]", "_");

	path = Path.Combine(FileUtil.DeleteLastPathNameComponent(path),
	                    name + "." + kControllerExtension);
...
public void
SetStateEffectiveBehaviours(AnimatorState state, int layerIndex,
...
public StateMachineBehaviour[] GetStateEffectiveBehaviours(
...
[System.Obsolete(
	 "parameterCount is obsolete. Use parameters.Length instead.", true)] int
parameterCount {
	get { return 0; }
}

[System.Obsolete("layerCount is obsolete. Use layers.Length instead.",
                 true)] int layerCount {
	get { return 0; }
}
}
