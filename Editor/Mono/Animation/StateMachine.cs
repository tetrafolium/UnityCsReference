...
namespace UnityEditor.Animations {
  internal struct PushUndoIfNeeded {
    ...
    private class PushUndoIfNeededImpl {
      public PushUndoIfNeededImpl(bool pushUndo) { m_PushUndo = pushUndo; }
  ...
  public partial class AnimatorTransitionBase : Object {
    ...
    internal bool pushUndo {
      set { undoHandler.pushUndo = value; }
    }

    public void AddCondition(AnimatorConditionMode mode, float threshold,
                             string parameter) {
  ...
  public partial class AnimatorState : Object {
    ...
    private AnimatorStateTransition CreateTransition(bool setDefaultExitTime) {
      ...
      if (AssetDatabase.GetAssetPath(this) != "")
        AssetDatabase.AddObjectToAsset(newTransition,
    ...
    private void
    SetDefaultTransitionExitTime(ref AnimatorStateTransition newTransition) {
      ...
      if (motion != null && motion.averageDuration > 0.0f) {
        ...
        float transitionDurationNormalized =
            transitionDefaultDuration / motion.averageDuration;
    ...
    AddTransition(AnimatorState destinationState) {
    ...
    public AnimatorStateTransition
    AddTransition(AnimatorStateMachine destinationStateMachine) {
      AnimatorStateTransition newTransition = CreateTransition(false);
    ...
    internal AnimatorStateMachine FindParent(AnimatorStateMachine root) {
      if (root.HasState(this, false))
        ...
        return root.stateMachinesRecursive
            .Find(sm => sm.stateMachine.HasState(this, false))
            .stateMachine;
    }

    internal AnimatorStateTransition
    FindTransition(AnimatorState destinationState) // pp todo return a list?
    {
      return (new List<AnimatorStateTransition>(transitions))
          .Find(t => t.destinationState == destinationState);
    }

    [System.Obsolete(
        "uniqueName does not exist anymore. Consider using .name instead.",
        true)]
  ...
  public partial class AnimatorStateMachine : Object {
    ...
    internal class StateMachineCache {
      static Dictionary<AnimatorStateMachine, ChildAnimatorStateMachine[]>
          m_ChildStateMachines;
      static bool m_Initialized;

      static void Init() {
        if (!m_Initialized) {
          m_ChildStateMachines = new Dictionary<AnimatorStateMachine,
      ...
      static public ChildAnimatorStateMachine[] GetChildStateMachines(
    ...
    internal List<ChildAnimatorStateMachine> stateMachinesRecursive {
      get {
        List<ChildAnimatorStateMachine> ret =
            new List<ChildAnimatorStateMachine>();
        var childStateMachines =
            AnimatorStateMachine.StateMachineCache.GetChildStateMachines(this);
        ret.AddRange(childStateMachines);

        for (int j = 0; j < childStateMachines.Length; j++) {
          ret.AddRange(
              childStateMachines[j].stateMachine.stateMachinesRecursive);
        }
        return ret;
      }
    }
    ...
    internal List<AnimatorStateTransition> anyStateTransitionsRecursive {
      get {
        List<AnimatorStateTransition> childTransitions =
        ...
        foreach (ChildAnimatorStateMachine stateMachine in stateMachines) {
          childTransitions.AddRange(
              stateMachine.stateMachine.anyStateTransitionsRecursive);
        }
    ...
    internal Vector3 GetStatePosition(AnimatorState state) {
      ...
      System.Diagnostics.Debug.Fail("Can't find state (" + state.name +
                                    ") in parent state machine (" + name +
                                    ").");
      return Vector3.zero;
    }
    ...
    internal
        Vector3 GetStateMachinePosition(AnimatorStateMachine stateMachine) {
      ...
      System.Diagnostics.Debug.Fail(
          "Can't find state machine (" + stateMachine.name +
          ") in parent state machine (" + name + ").");

      return Vector3.zero;
    }

    internal void SetStateMachinePosition(AnimatorStateMachine stateMachine,
                                          Vector3 position) {
      ChildAnimatorStateMachine[] childSM = stateMachines;
      for (int i = 0; i < childSM.Length; i++)
        if (stateMachine == childSM[i].stateMachine) {
    ...
    public AnimatorState AddState(string name) {
      return AddState(name, states.Length > 0
                                ? states[states.Length - 1].position +
                                      new Vector3(35, 65)
                                : new Vector3(200, 0, 0));
    }
    ...
    public void AddState(AnimatorState state, Vector3 position) {
      ChildAnimatorState[] childStates = states;
      if (System.Array.Exists(childStates,
                              childState => childState.state == state)) {
        Debug.LogWarning(System.String.Format(
            "State '{0}' already exists in state machine '{1}', discarding new state.",
            state.name, name));
        return;
      }
    ...
    public AnimatorStateMachine AddStateMachine(string name, Vector3 position) {
      ...
      if (AssetDatabase.GetAssetPath(this) != "")
        AssetDatabase.AddObjectToAsset(stateMachine,
                                       AssetDatabase.GetAssetPath(this));

      return stateMachine;
    }

    public void AddStateMachine(AnimatorStateMachine stateMachine,
                                Vector3 position) {
      ChildAnimatorStateMachine[] childStateMachines = stateMachines;
      if (System.Array.Exists(childStateMachines,
                              childStateMachine =>
                                  childStateMachine.stateMachine ==
                                  stateMachine)) {
        Debug.LogWarning(System.String.Format(
            "Sub state machine '{0}' already exists in state machine '{1}', discarding new state machine.",
            stateMachine.name, name));
        return;
      }

      undoHandler.DoUndo(this, "StateMachine " + stateMachine.name + " added");
      ChildAnimatorStateMachine newStateMachine =
    ...
    public AnimatorStateTransition
    AddAnyStateTransition(AnimatorState destinationState) {
    ...
    public AnimatorStateTransition
    AddAnyStateTransition(AnimatorStateMachine destinationStateMachine) {
      AnimatorStateTransition newTransition = AddAnyStateTransition();
      newTransition.destinationStateMachine = destinationStateMachine;
      return newTransition;
    }
    ...
    public bool RemoveAnyStateTransition(AnimatorStateTransition transition) {
      if ((new List<AnimatorStateTransition>(anyStateTransitions))
              .Any(t => t == transition)) {
    ...
    internal void
    RemoveAnyStateTransitionRecursive(AnimatorStateTransition transition) {
      if (RemoveAnyStateTransition(transition))
        return;

      List<ChildAnimatorStateMachine> childStateMachines =
          stateMachinesRecursive;
    ...
    AddStateMachineTransition(AnimatorStateMachine sourceStateMachine,
      ...
      AnimatorTransition[] transitionsVector =
          GetStateMachineTransitions(sourceStateMachine);
    ...
    AddStateMachineTransition(AnimatorStateMachine sourceStateMachine,
      ...
      AnimatorTransition newTransition =
          AddStateMachineTransition(sourceStateMachine);
      newTransition.destinationState = destinationState;
      return newTransition;
    }

    public AnimatorTransition
    AddStateMachineExitTransition(AnimatorStateMachine sourceStateMachine) {
      AnimatorTransition newTransition =
          AddStateMachineTransition(sourceStateMachine);
      newTransition.isExit = true;
      return newTransition;
    ...
    public bool
    RemoveStateMachineTransition(AnimatorStateMachine sourceStateMachine,
    ...
    public AnimatorTransition
    AddEntryTransition(AnimatorState destinationState) {
    ...
    public AnimatorTransition
    AddEntryTransition(AnimatorStateMachine destinationStateMachine) {
      AnimatorTransition newTransition = AddEntryTransition();
      newTransition.destinationStateMachine = destinationStateMachine;
      return newTransition;
    }
    ...
    public bool RemoveEntryTransition(AnimatorTransition transition) {
      if ((new List<AnimatorTransition>(entryTransitions))
              .Any(t => t == transition)) {
    ...
    internal ChildAnimatorState FindState(int nameHash) {
      return (new List<ChildAnimatorState>(states))
          .Find(s => s.state.nameHash == nameHash);
    }
    ...
    internal
        AnimatorStateMachine FindParent(AnimatorStateMachine stateMachine) {
      if (stateMachines.Any(childSM => childSM.stateMachine == stateMachine))
        return this;
      else
        return stateMachinesRecursive
            .Find(sm => sm.stateMachine.stateMachines.Any(
                      childSM => childSM.stateMachine == stateMachine))
            .stateMachine;
    }
    ...
    internal AnimatorStateMachine FindStateMachine(string path) {
      ...
      for (int i = 1; i < smNames.Length - 1 && currentSM != null; ++i) {
        var childStateMachines =
            AnimatorStateMachine.StateMachineCache.GetChildStateMachines(
                currentSM);
        int index = System.Array.FindIndex(
            childStateMachines, t => t.stateMachine.name == smNames[i]);
    ...
    internal AnimatorStateMachine FindStateMachine(AnimatorState state) {
      ...
      List<ChildAnimatorStateMachine> childStateMachines =
          stateMachinesRecursive;
      int index = childStateMachines.FindIndex(
          sm => sm.stateMachine.HasState(state, false));
      return index >= 0 ? childStateMachines[index].stateMachine : null;
    }

    internal
        AnimatorStateTransition FindTransition(AnimatorState destinationState) {
      return (new List<AnimatorStateTransition>(anyStateTransitions))
          .Find(t => t.destinationState == destinationState);
    }

    [System.Obsolete("stateCount is obsolete. Use .states.Length  instead.",
                     true)] int stateCount {
      get { return 0; }
    }

    [System.Obsolete(
        "stateMachineCount is obsolete. Use .stateMachines.Length instead.",
        true)] int stateMachineCount {
      get { return 0; }
    }

    [System.Obsolete(
        "GetTransitionsFromState is obsolete. Use AnimatorState.transitions instead.",
        true)]
    AnimatorState GetTransitionsFromState(AnimatorState state) {
      return null;
    }

    [System.Obsolete("uniqueNameHash does not exist anymore.", true)] int
        uniqueNameHash {
      get { return -1; }
    }
  }
