...
namespace UnityEditor.Animations {
  ...
  public partial class BlendTree : Motion {
    public void AddChild(Motion motion) { AddChild(motion, Vector2.zero, 0); }
    ...
    internal bool HasChild(BlendTree childTree, bool recursive) {
      foreach (ChildMotion child in children) {
        ...
        if (recursive && child.motion is BlendTree &&
            (child.motion as BlendTree).HasChild(childTree, true)) {
          return true;
        }
      }
