...
internal struct SavedGUIState {
  ...
  Internal_SetupSavedGUIState(out IntPtr state, out Vector2 screenManagerSize);

  static private extern void
  Internal_ApplySavedGUIState(IntPtr state, Vector2 screenManagerSize);
  ...
  static internal SavedGUIState Create() {
    ...
    if (Internal_GetGUIDepth() > 0) {
      ...
      Internal_SetupSavedGUIState(out state.guiState,
                                  out state.screenManagerSize);
    }
    return state;
  }
