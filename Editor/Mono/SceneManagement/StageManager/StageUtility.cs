...
namespace UnityEditor.SceneManagement {
  public static partial class StageUtility {
    internal enum ContextRenderMode { Normal, GreyedOut, Hidden }
    ...
    public static bool IsGameObjectRenderedByCamera(GameObject gameObject,
                                                    Camera camera) {
      return IsGameObjectRenderedByCameraInternal(gameObject, camera);
    }

    public static bool
    IsGameObjectRenderedByCameraAndPartOfEditableScene(GameObject gameObject,
                                                       Camera camera) {
    ...
    internal static void SetSceneToRenderInStage(Scene scene,
                                                 StageHandle stageHandle) {
      if (!stageHandle.IsValid())
        throw new System.ArgumentException("Stage is not valid.",
                                           nameof(stageHandle));
      if (stageHandle.isMainStage)
        SetSceneToRenderInMainStageInternal(scene.handle);
      else
        SetSceneToRenderInSameStageAsOtherSceneInternal(
            scene.handle, stageHandle.customScene.handle);
    }
    ...
    public static void GoToMainStage() {
      StageNavigationManager.instance.GoToMainStage(
          StageNavigationManager.Analytics.ChangeType.GoToMainViaUnknown);
    }

    public static void GoBackToPreviousStage() {
      StageNavigationManager.instance.NavigateBack(
          StageNavigationManager.Analytics.ChangeType.NavigateBackViaUnknown);
    }
    ...
    public static void GoToStage(Stage stage,
      ...
      StageNavigationManager.instance.SwitchToStage(
          ...
          StageNavigationManager.Analytics.ChangeType.Unknown);
    }
    ...
    internal static Hash128 CreateWindowAndStageIdentifier(string windowGUID,
                                                           Stage stage) {
    ...
    SetPrefabInstanceHiddenForInContextEditing(GameObject gameObject,
                                               bool hide) {
      SetPrefabInstanceHiddenForInContextEditingInternal(gameObject, hide);
    }

    internal static bool
    IsPrefabInstanceHiddenForInContextEditing(GameObject gameObject) {
      return IsPrefabInstanceHiddenForInContextEditingInternal(gameObject);
    }

    internal static void
    EnableHidingForInContextEditingInSceneView(bool enable) {
    ...
    internal static void
    SetFocusedSceneContextRenderMode(ContextRenderMode contextRenderMode) {
      SetFocusedSceneContextRenderModeInternal(contextRenderMode);
    }

    internal static void
    CallAwakeFromLoadOnSubHierarchy(GameObject prefabInstanceRoot) {
