...
[SerializeField] List<int> m_ExpandedSceneGameObjectInstanceIDs =
	new List<int>();
[SerializeField] int m_LastClickedInstanceID;
[SerializeField] string[] m_OpenSceneGUIDs = null;
...
public void SaveStateFromHierarchy(SceneHierarchyWindow hierarchy,
	...
	m_LastClickedInstanceID = lastClickedGameObject !=
	                          null? lastClickedGameObject.GetInstanceID() : 0;
...
void DebugLog(string prefix, Stage stage) {
	Debug.Log(
		...
		string.Format(
			"-main stage: {0}, -scrollY: {1}, -selection {2}, -setSelection {3}",
			DebugUtils.ListToString(m_ExpandedSceneGameObjectInstanceIDs),
			m_ScrollY, m_LastClickedInstanceID,
