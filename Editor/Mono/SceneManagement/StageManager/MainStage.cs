...
namespace UnityEditor.SceneManagement {
  public sealed class MainStage : Stage {
    static StateCache<MainStageHierarchyState> s_StateCache =
        new StateCache<MainStageHierarchyState>(
            "Library/StateCache/MainStageHierarchy/");
    ...
    protected override void OnCloseStage() {
      Debug.LogError(
          "The MainStage should not be destroyed. This is not supported.");
    }

    protected internal override GUIContent CreateHeaderContent() {
      return new GUIContent(
          "Scenes", EditorGUIUtility.IconContent("SceneAsset Icon").image);
    }
    ...
    internal override void SyncSceneViewToStage(SceneView sceneView) {
      ...
      // NOTE: We always set overrideSceneCullingMask to ensure the gizmo
      // handling in the Entities package works (in dots search for 'gizmo
      // hack'). This ensures normal picking works with the livelink since the
      // SceneCullingMasks.MainStageSceneViewObjects is part of the mask while
      // the gizmo bit is also set. When the gizmo hack is removed in dots we
      // can set 'sceneView.overrideSceneCullingMask = 0'.
    ...
    internal override void PlaceGameObjectInStage(GameObject rootGameObject) {
      if (rootGameObject.transform.parent != null)
        throw new ArgumentException(
            "GameObject has a transform parent, only root GameObjects are valid",
            "rootGameObject");

      if (StageUtility.GetStageHandle(rootGameObject) !=
          StageUtility.GetMainStageHandle())
        SceneManager.MoveGameObjectToScene(rootGameObject,
