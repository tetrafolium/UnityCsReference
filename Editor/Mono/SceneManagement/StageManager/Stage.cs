...
namespace UnityEditor.SceneManagement {
  public abstract class Stage : ScriptableObject {
    ...
    internal static ReadOnlyCollection<Stage> allStages {
      get { return s_AllStages.AsReadOnly(); }
    }
    ...
    // unload the contents of the stage. Only called if OnOpenStage was called.
    protected abstract void OnCloseStage();

    // Called when returning to a previous open stage (e.g by clicking a
    // non-current breadcrumb)
    ...
    internal virtual bool Save() {
      ...
      throw new System.NotImplementedException(
          "This Stage returns true for SupportsSaving() but has not implemented an override for the Save() method.");
    }
    ...
    internal virtual bool SaveAsNew() {
      if (!SupportsSaving())
        return true;
      throw new System.NotImplementedException(
          "This Stage returns true for SupportsSaving() but has not implemented an override for the SaveAsNew() method.");
    }

    // Transient state since it is set every time we switch stage
    internal bool setSelectionAndScrollWhenBecomingCurrentStage {
      get;
      set;
    }
    = true;

    internal virtual string GetErrorMessage() { return null; }
    ...
    internal virtual BreadcrumbBar.Item CreateBreadcrumbItem() {
      ...
      return new BreadcrumbBar.Item{
          content = CreateHeaderContent(), guistyle = style, userdata = this,
          separatorstyle = BreadcrumbBar.SeparatorStyle.Line};
    }

    internal virtual ulong GetSceneCullingMask() {
      return EditorSceneManager.DefaultSceneCullingMask;
    }
    ...
    internal virtual Color GetBackgroundColor() {
      return SceneView.kSceneViewBackground.Color;
    }
    ...
    // Called when new stage is created, after script reloads, and possibly at
    // other times by the stage itself.
    ...
    internal virtual void
    LoadHierarchyState(SceneHierarchyWindow hierarchyWindow) {}

    // Called after respective sync methods first time this stage is opened in
    // this window.
    protected internal virtual void
    OnFirstTimeOpenStageInSceneView(SceneView sceneView) {}
    internal virtual void OnFirstTimeOpenStageInSceneHierachyWindow(
    ...
    // Called on the the current stage when trying to switch to new stage.
    // Should return true if it OK to continue to switch away from current
    // stage. False if not ok to continue.
    ...
    // For stages that use the assetPath property, the hash is by default based
    // on the asset GUID.
    ...
    protected virtual void OnDisable() { s_AllStages.Remove(this); }
  ...
  public struct StageHandle : System.IEquatable<StageHandle> {
    ...
    internal bool isMainStage {
      get { return m_IsMainStage; }
    }
      ...
      if (isMainStage) {
        for (int i = 0; i < components.Length; i++) {
          ...
          if (!EditorUtility.IsPersistent(obj) &&
              !EditorSceneManager.IsPreviewScene(obj.gameObject.scene))
            return obj;
        }
      } else {
        for (int i = 0; i < components.Length; i++) {
    ...
    internal static StageHandle GetCurrentStageHandle() {
      if (StageNavigationManager.instance != null &&
          StageNavigationManager.instance.currentStage != null)
    ...
    public bool IsValid() { return m_IsMainStage ^ m_CustomScene.IsValid(); }
    ...
    public override bool Equals(object other) {
      ...
      return m_IsMainStage == rhs.m_IsMainStage &&
             m_CustomScene == rhs.m_CustomScene;
    }

    public bool Equals(StageHandle other) {
      return m_IsMainStage == other.m_IsMainStage &&
             m_CustomScene == other.m_CustomScene;
    }
