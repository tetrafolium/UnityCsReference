  ...
  class BreadcrumbBar {
    ...
    public static class DefaultStyles {
      ...
      static DefaultStyles() {
        ...
        labelMissing.normal.textColor = GameObjectTreeViewGUI.GameObjectStyles
                                            .brokenPrefabLabel.normal.textColor;
    ...
    public enum SeparatorStyle { None, Line, Arrow }
    ...
    public List<Item> breadcrumbs {
      get { return m_Breadcrumbs; }
    }

    float lastBreadcrumbWidth {
      get;
      set;
    }
    ...
    public void OnGUI() {
      ...
      for (int i = 0; i < m_Breadcrumbs.Count; ++i) {
        ...
        if (item.separatorstyle != SeparatorStyle.None) {
          var separatorGUIStyle =
              item.separatorstyle ==
              SeparatorStyle.Arrow ? DefaultStyles.separatorArrow
              : DefaultStyles.separatorLine;
          GUILayout.Label(GUIContent.none, separatorGUIStyle);
        }

        // Ensures last breadcrumb does not shrink
        var minWidth =
            (i == m_Breadcrumbs.Count - 1) ? lastBreadcrumbWidth : 32;

        if (GUILayout.Button(item.content, item.guistyle,
