...
[Shortcut("Stage/Enter Prefab Mode", KeyCode.P,
          displayName = "Stage/Edit Prefab in Context")]
...
static void
EnterPrefabModeFromCurrentSelection(PrefabStage.Mode preferredMode) {
	...
	if (PrefabUtility.IsPartOfAnyPrefab(activeGameObject)) {
		...
		if (!string.IsNullOrEmpty(prefabPath) &&
			...
			if (preferredMode == PrefabStage.Mode.InContext) {
				openedFromInstance = !EditorUtility.IsPersistent(activeGameObject)
				     ? activeGameObject
				     : null;
				prefabStageMode =
					openedFromInstance !=
					null ? PrefabStage.Mode.InContext : PrefabStage.Mode.InIsolation;
			}
...
internal static PrefabStage OpenPrefab(string prefabAssetPath,
                                       GameObject openedFromInstance) {
	var prefabStageMode =
		openedFromInstance != null ? PrefabStage.Mode.InContext
	  : PrefabStage.Mode.InIsolation;
	return OpenPrefab(
		prefabAssetPath, openedFromInstance, prefabStageMode,
		StageNavigationManager.Analytics.ChangeType.EnterViaUnknown);
}

internal static PrefabStage OpenPrefab(
	string prefabAssetPath, GameObject openedFromInstance,
	StageNavigationManager.Analytics.ChangeType changeTypeAnalytics) {
	var prefabStageMode =
		openedFromInstance != null ? PrefabStage.Mode.InContext
	  : PrefabStage.Mode.InIsolation;
	return OpenPrefab(prefabAssetPath, openedFromInstance, prefabStageMode,
	                  changeTypeAnalytics);
}
...
internal static PrefabStage OpenPrefab(
	...
	if (openedFromInstance != null &&
	    !PrefabUtility.IsPartOfPrefabInstance(openedFromInstance))
		throw new ArgumentException(
			      "GameObject must be part of a Prefab instance, or null.",
			      nameof(openedFromInstance));

	if (!prefabAssetPath.EndsWith(".prefab",
	                              StringComparison.OrdinalIgnoreCase))
		throw new ArgumentException("Incorrect file extension: " +
		                            prefabAssetPath + ". Must be '.prefab'",
		                            prefabAssetPath);

	if (AssetDatabase.LoadMainAssetAtPath(prefabAssetPath) == null)
		throw new ArgumentException(
			      "Prefab not found at path " + prefabAssetPath, prefabAssetPath);

	return OpenPrefabMode(prefabAssetPath, openedFromInstance,
	                      prefabStageMode, changeTypeAnalytics);
}

static PrefabStage
GetExistingPrefabStage(string prefabAssetPath,
                       GameObject openedFromInstance,
                       PrefabStage.Mode prefabStageMode) {
	...
	for (int i = 1; i < stageHistory.Count; i++) {
		...
		if (prefabStage != null && prefabStage.assetPath == prefabAssetPath) {
			// If PrefabStage.Mode did not match on existing PrefabStage we do not
			// reuse the stage so we create a new PrefabStage with the correct
			// mode
...
internal static PrefabStage OpenPrefabMode(
	string prefabAssetPath, GameObject openedFromInstance,
	PrefabStage.Mode prefabStageMode,
	StageNavigationManager.Analytics.ChangeType changeTypeAnalytics) {
	if (EditorApplication.isPlaying) {
		bool blockPrefabModeInPlaymode =
			CheckIfAnyComponentShouldBlockPrefabModeInPlayMode(prefabAssetPath);
		if (blockPrefabModeInPlaymode)
			return null;
	}

	PrefabStage prevPrefabStage = GetCurrentPrefabStage();
	bool setAsFirstItemAfterMainStage =
		prevPrefabStage == null ||
		!IsPartOfPrefabStage(openedFromInstance, prevPrefabStage);

	var previousSelection = Selection.activeGameObject;
	UInt64 previousFileID =
		(openedFromInstance != null)
	      ? GetFileIDForCorrespondingObjectFromSourceAtPath(
			previousSelection, prefabAssetPath)
	      : 0;

	// Ensure valid prefabStageMode (if no context then do not allow Prefab
	// Mode in Context)
	if (openedFromInstance == null &&
	...
	if (prefabStageMode == PrefabStage.Mode.InContext) {
		var stageHistory = StageNavigationManager.instance.stageHistory;
		contextStage = stageHistory [stageHistory.Count - 1]
		               .GetContextStage();
	}

	var prefabStage = GetExistingPrefabStage(
		prefabAssetPath, openedFromInstance, prefabStageMode);
	if (prefabStage == null)
		prefabStage = PrefabStage.CreatePrefabStage(
			prefabAssetPath, openedFromInstance, prefabStageMode, contextStage);
	if (StageNavigationManager.instance.SwitchToStage(
		    prefabStage, setAsFirstItemAfterMainStage, changeTypeAnalytics)) {
		// If selection did not change by switching stage by us or user (or if
		// current selection is not part of the opened prefab stage) then handle
		// automatic selection in new prefab mode.
		if (Selection.activeGameObject == previousSelection ||
		    !prefabStage.IsPartOfPrefabContents(Selection.activeGameObject)) {
			HandleSelectionWhenSwithingToNewPrefabMode(
				GetCurrentPrefabStage().prefabContentsRoot, previousFileID);
		}
...
static void
HandleSelectionWhenSwithingToNewPrefabMode(GameObject prefabContentsRoot,
                                           UInt64 previousFileID) {
	GameObject newSelection = null;

	if (previousFileID != 0)
		newSelection = FindFirstGameObjectThatMatchesFileID(
			prefabContentsRoot.transform, previousFileID, true);
	...
	// For Prefab Mode we restore the last expanded tree view state for the
	// opened Prefab. For usability if a child GameObject on the Prefab
	// Instance is selected when entering the Prefab Asset Mode we select the
	// corresponding child GameObject in the Asset. Here we ensure that
	// selction is revealed and framed in the Scene hierarchy.
...
static bool IsPartOfPrefabStage(GameObject gameObject,
                                PrefabStage prefabStage) {
	if (gameObject == null)
		return false;
	return FindGameObjectRecursive(prefabStage.prefabContentsRoot.transform,
	                               gameObject);
}

static bool FindGameObjectRecursive(Transform transform,
...
static UInt64
GetFileIDForCorrespondingObjectFromSourceAtPath(GameObject gameObject,
...
CheckIfAnyComponentShouldBlockPrefabModeInPlayMode(string prefabAssetPath) {
	var assetRoot =
		AssetDatabase.LoadAssetAtPath<GameObject>(prefabAssetPath);
	var monoBehaviors = assetRoot.GetComponentsInChildren<MonoBehaviour>(
		true); // also check the inactive since these can be made active while
	               // in play mode
	...
	if (warnList.Count > 0) {
		string blockingNames = string.Join(
			", ", warnList.Select(e => MonoScript.FromMonoBehaviour(e).name)
			.ToArray());
...
[UsedByNativeCode]
internal static bool
IsGameObjectThePrefabRootInAnyPrefabStage(GameObject gameObject) {
...
[UsedByNativeCode]
internal static bool
IsGameObjectInInvalidPrefabStage(GameObject gameObject) {
	PrefabStage prefabStage = GetCurrentPrefabStage();
...
static UInt64
GetPersistentPrefabOrVariantFileIdentifier(GameObject gameObject) {
	...
	return Unsupported.GetLocalIdentifierInFileForPersistentObject(
		gameObject);
}
...
FindFirstGameObjectThatMatchesFileID(Transform searchRoot, UInt64 fileID,
                                     bool generate) {
	GameObject result = null;
	var transformVisitor = new TransformVisitor();
	transformVisitor.VisitAndAllowEarlyOut(
		searchRoot, (transform, userdata) => {
				UInt64 id = generate ? Unsupported.GetOrGenerateFileIDHint(
...
static void RemoveBrokenPrefabRootsIfNeeded(
	string prefabAssetPath, GameObject[] environmentRoots,
	GameObject[] rootsAfterLoadingPrefab, UInt64 prefabAssetRootFileID) {
	var rootsLoadedFromFile =
		rootsAfterLoadingPrefab.Except(environmentRoots).ToList();

	// Filter out the subset of roots that were loaded from the Prefab file
	// (there can be dynamically created environment objects, created from
	// Awake and OnEnable calls from user land, if they use ExecuteAlways or
	// ExecuteInEditMode.
	rootsLoadedFromFile.RemoveAll(x => IsDynamicallyCreatedDuringLoad(x));

	if (rootsLoadedFromFile.Count >= 2) {
		Debug.LogError(string.Format(
				       "Prefab Mode: Broken Prefab with multiple roots detected ('{0}')",
				       prefabAssetPath));

		// First see if we can find a valid root at all. The root could have
		// been reparenting in User land in Awake. Keep only the same root as
		// the PrefabImporter if possible.
...
static GameObject FindPrefabRoot(string prefabAssetPath,
                                 GameObject[] environmentRoots,
                                 GameObject[] rootsAfterLoadingPrefab) {
	var assetRoot =
		AssetDatabase.LoadMainAssetAtPath(prefabAssetPath) as GameObject;
	if (assetRoot == null) {
		Debug.LogError(string.Format(
				       "Opening Prefab Mode failed: The Prefab at '{0}' is broken.",
				       prefabAssetPath));
		return null;
	}

	// Find the prefab root or variant root among the roots of the scene (or
	// as a child)
	UInt64 prefabAssetRootFileID =
		GetPersistentPrefabOrVariantFileIdentifier(assetRoot);

	// Check for broken prefabs with multiple roots
	RemoveBrokenPrefabRootsIfNeeded(prefabAssetPath, environmentRoots,
	                                rootsAfterLoadingPrefab,
	                                prefabAssetRootFileID);
	...
	foreach (var root in rootsAfterLoadingPrefab) {
		...
		var prefabRoot = FindFirstGameObjectThatMatchesFileID(
			root.transform, prefabAssetRootFileID, false);
		if (prefabRoot != null)
			return prefabRoot;
	}

	Debug.LogError(string.Format(
			       "Opening Prefab Mode failed: Could not detect a Prefab root after loading '{0}'.",
			       prefabAssetPath));
	return null;
}

internal static GameObject
LoadPrefabIntoPreviewScene(string prefabAssetPath, Scene previewScene) {
	...
	try {
		PrefabUtility.LoadPrefabContentsIntoPreviewScene(prefabAssetPath,
		                                                 previewScene);
	} catch (Exception e) {
	...
	var root = FindPrefabRoot(prefabAssetPath, environmentRoots,
	                          rootsAfterLoadingPrefab);
	if (root != null) {
		// We need to ensure root instance name is matching filename when
		// loading a prefab as a scene since the prefab file might contain an
		// old root name. The same name-matching is also ensured when importing
		// a prefab: the library prefab asset root gets the same name as the
		// filename of the prefab.
...
HandleReparentingIfNeeded(GameObject prefabInstanceRoot, bool isUIPrefab) {
...
static string GetEnvironmentScenePathForPrefab(bool isUIPrefab) {
	...
	if (isUIPrefab) {
		if (EditorSettings.prefabUIEnvironment != null)
			environmentEditingScenePath =
				AssetDatabase.GetAssetPath(EditorSettings.prefabUIEnvironment);
	}
	// Else try to use regular environment.
	// Note, if the prefab is a UI object we deliberately don't use the
	// regular environment as fallback, since our empty scene with
	// auto-generated Canvas is likely a better fallback than an environment
	// designed for non-UI objects.
	else {
		if (EditorSettings.prefabRegularEnvironment != null)
			environmentEditingScenePath = AssetDatabase.GetAssetPath(
...
static Scene LoadOrCreatePreviewScene(string environmentEditingScenePath) {
	...
	if (!string.IsNullOrEmpty(environmentEditingScenePath)) {
		previewScene =
			EditorSceneManager.OpenPreviewScene(environmentEditingScenePath);
...
static Scene CreateDefaultPreviewScene() {
	...
	UnityEngine.RenderSettings.customReflection =
		GetDefaultReflection(); // ensure chrome materials do not render black
...
internal static Scene GetEnvironmentSceneOrEmptyScene(bool isUIPrefab) {
	// Create the environment scene and move the prefab root to this scene to
	// ensure the correct rendersettings (skybox etc) are used in Prefab Mode.
	string environmentEditingScenePath =
		GetEnvironmentScenePathForPrefab(isUIPrefab);
...
internal static bool IsUIPrefab(string prefabAssetPath) {
	// We require a RectTransform and a CanvasRenderer to be considered a UI
	// prefab. E.g 3D TextMeshPro uses RectTransform but a MeshRenderer so
	// should not be considered a UI prefab This function needs to be
	// peformant since it is called every time a prefab is opened in a prefab
	// stage.
	var root =
		AssetDatabase.LoadMainAssetAtPath(prefabAssetPath) as GameObject;
	if (root == null)
	...
	// AND either a Canvas on the root or a CanvasRenderer somewhere in the
	// hierarchy.
	bool rectTransformOnRoot = root.GetComponent<RectTransform>() != null;
	bool uiSpecificComponentPresent =
		(root.GetComponent<Canvas>() != null ||
		 root.GetComponentInChildren<CanvasRenderer>(true) != null);
	return rectTransformOnRoot && uiSpecificComponentPresent;
}

static void HandleUIReparentingIfNeeded(GameObject instanceRoot) {
	// We need a Canvas in order to render UI so ensure the prefab instance is
	// under a Canvas
	...
	if (canvas != null) {
		...
		if (canvas.renderMode == RenderMode.WorldSpace) {
			...
			// We can't know if a World Space Canvas was a root or not in all
			// cases, but it's important to not make it a root if its
			// RectTransform has stretching (non-identical min and max anchors);
			// otherwise it will be previewed stretching to a single point (or
			// less!). The downsides of making a World Space Canvas that was a
			// root into a nested Canvas in Prefab Mode are less severe (a few
			// settings on the Canvas component will be hidden), so we make it
			// always nested as the lesser evil. Note that regardless, there is no
			// data loss, since World Space canvases don't drive their
			// RectTransform.
		} else {
			...
			if (rt.sizeDelta == Vector2.zero && rt.anchorMin == Vector2.zero &&
			    rt.anchorMax == Vector2.zero && rt.pivot == Vector2.zero)
				return; // Use as root.
		}
	}
...
static GameObject GetOrCreateCanvasGameObject(GameObject instanceRoot) {
	...
	GameObject root = EditorUtility.CreateGameObjectWithHideFlags(
		"Canvas (Environment)", HideFlags.DontSave);
...
internal static PrefabStage.Mode GetPrefabStageModeFromModifierKeys() {
	...
	return Event.current.alt ? PrefabStage.Mode.InIsolation
	  : PrefabStage.Mode.InContext;
}

static Dictionary<int, GUIContent> m_PrefabButtonContents =
