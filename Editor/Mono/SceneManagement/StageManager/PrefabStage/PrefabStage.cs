...
namespace UnityEditor.Experimental.SceneManagement {
  // For the initial Improved Prefabs release we didn't have time to generalize
  // the stage concept to also work for other things than main scenes and
  // Prefabs. But it makes sense to use also for e.g. Avatar Setup. Until we
  // have gone through the work of generalizing it by making e.g. Avatar Setup
  // use it too, we're not confident that the API is right, hence it's
  // experimental for now.

  [Serializable]
  public sealed partial class PrefabStage : PreviewSceneStage {
    static class Styles {
      public static GUIContent autoSaveGUIContent = EditorGUIUtility.TrTextContent(
          "Auto Save",
          "When Auto Save is enabled, every change you make is automatically saved to the Prefab Asset. Disable Auto Save if you experience long import times.");
      public static GUIContent saveButtonContent =
          EditorGUIUtility.TrTextContent("Save");
      public static GUIContent checkoutButtonContent =
          EditorGUIUtility.TrTextContent("Check Out");
      public static GUIContent autoSavingBadgeContent =
          EditorGUIUtility.TrTextContent("Auto Saving...");
      public static GUIContent immutablePrefabContent =
          EditorGUIUtility.TrTextContent("Immutable Prefab");
      ...
      public static GUIContent contextLabel =
          EditorGUIUtility.TrTextContent("Context:");
      public static GUIContent[] contextRenderModeTexts =
          new[]{EditorGUIUtility.TrTextContent("Normal"),
                EditorGUIUtility.TrTextContent("Gray"),
                EditorGUIUtility.TrTextContent("Hidden")};
      public static StageUtility.ContextRenderMode[] contextRenderModeOptions =
          new[]{StageUtility.ContextRenderMode.Normal,
                StageUtility.ContextRenderMode.GreyedOut,
                StageUtility.ContextRenderMode.Hidden};
      public static GUIContent showOverridesLabel = EditorGUIUtility.TrTextContent(
          "Show Overrides",
          "Visualize overrides from the Prefab instance on the Prefab Asset. Overrides on the root Transform are always visualized.");
    ...
    public enum Mode { InIsolation, InContext }
    ...
    internal static event Action<PrefabStage>
        prefabStageReloaded; // Used by tests.

    internal static List<PrefabStage> m_AllPrefabStages =
        new List<PrefabStage>();
    static StateCache<PrefabStageHierarchyState> s_StateCache =
        new StateCache<PrefabStageHierarchyState>(
            "Library/StateCache/PrefabStageHierarchy/");
    ...
    [System.Serializable] struct PatchedProperty {
    ...
    CreatePrefabStage(string prefabAssetPath,
                      GameObject openedFromInstanceObject,
                      PrefabStage.Mode prefabStageMode, Stage contextStage) {
      PrefabStage prefabStage = CreateInstance<PrefabStage>();
      prefabStage.Init(prefabAssetPath, openedFromInstanceObject,
                       prefabStageMode, contextStage);
      return prefabStage;
    }

    // Used for tests, if nothing else.
    internal static ReadOnlyCollection<PrefabStage> allPrefabStages {
      get { return m_AllPrefabStages.AsReadOnly(); }
    }
    ...
    void SetOpenedFromInstanceObject(GameObject go) {
      if (go != null) {
        ...
        m_OpenedFromInstanceRoot =
            PrefabUtility.GetNearestPrefabInstanceRoot(go);
        m_FileIdForOpenedFromInstanceObject =
            Unsupported.GetOrGenerateFileIDHint(go);
      } else {
    ...
    void Init(string prefabAssetPath, GameObject openedFromInstanceGameObject,
              PrefabStage.Mode prefabStageMode, Stage contextStage) {
    ...
    void ReconstructDataIfNeeded() {
      bool needsReconstruction = m_OpenedFromInstanceObject == null &&
                                 m_FileIdForOpenedFromInstanceObject != 0;
      if (!needsReconstruction)
        return;

      // The previous PrefabStage can have been reloaded which means we need to
      // update our reference to m_OpenedFromInstanceObject to the newly loaded
      // GameObject (the old GameObject was deleted as part of reloading the
      // PrefabStage).
      ...
      if (prevPrefabStage) {
        var go = PrefabStageUtility.FindFirstGameObjectThatMatchesFileID(
            prevPrefabStage.prefabContentsRoot.transform,
            m_FileIdForOpenedFromInstanceObject, true);
        if (go != null) {
          SetOpenedFromInstanceObject(go);
        } else {
          Debug.LogError("Could not find GameObject with fileID " +
                         m_FileIdForOpenedFromInstanceObject +
                         " in PrefabStage for: " + prevPrefabStage.assetPath);
        }
      }
    }

    internal bool analyticsDidUserModify {
      get { return m_AnalyticsDidUserModify; }
    }
    internal bool analyticsDidUserSave {
      get { return m_AnalyticsDidUserSave; }
    }

    static class Icons {
      public static Texture2D prefabVariantIcon =
          EditorGUIUtility.LoadIconRequired("PrefabVariant Icon");
      ...
      get {
        if (m_PrefabContentsRoot == null)
          throw new InvalidOperationException(
              "Requesting 'prefabContentsRoot' from Awake and OnEnable is not supported"); // The Prefab stage's m_PrefabContentsRoot is not yet set when we call Awake and OnEnable on user scripts when loading a Prefab
    ...
    public override ulong GetCombinedSceneCullingMaskForCamera() {
      if (m_Mode == Mode.InIsolation) {
        ...
        if (this == stageHistory.Last()) {
          ...
          // Remove the MainStagePrefabInstanceObjectsOpenInPrefabMode bit from
          // the camera's scenecullingmask. By removing that bit we ensure we
          // hide the MainStage Prefab instance when editing its Prefab Asset in
          // context. Since the MainStage Prefab instance GameObjects have the
          // MainStagePrefabInstanceObjectsOpenInPrefabMode set when as
          // override-scenecullingmask when entering Prefab Mode in Context for
          // MainStage instances.
          mask &=
              ~SceneCullingMasks.MainStagePrefabInstanceObjectsOpenInPrefabMode;
          return mask;
        } else {
          Debug.LogError(
              "We should only call GetOverrideSceneCullingMask() for the current stage");
          return 0;
        }
      } else {
    ...
    internal override Color GetBackgroundColor() {
      return SceneView.kSceneViewPrefabBackground.Color;
    }
    ...
    internal override bool hasUnsavedChanges {
      get { return scene.dirtyID != m_InitialSceneDirtyID; }
    }
    ...
    internal bool autoSave {
      get {
        return !m_TemporarilyDisableAutoSave &&
               StageNavigationManager.instance.autoSave;
      }
      set {
    ...
    internal override bool isValid {
      get {
        return m_PrefabContentsRoot != null &&
               m_PrefabContentsRoot.scene == scene;
      }
    }
    ...
    void OnPrefabInstanceUpdated(GameObject instance) {
      if (mode == Mode.InContext && m_OpenedFromInstanceRoot != null) {
        // We check against the outerMostInstanceRoot since that is what will be
        // updated when we change that or any nested prefabs. E.g having PrefabA
        // instance (that have nested PrefabB) in the Scene and we can enter
        // Prefab Mode in Context for prefabB from the scene. Then adding a
        // child; it will then be instanceA that will need to have its objects
        // visibility updated.
        var outerMostInstanceRoot =
            PrefabUtility.GetOutermostPrefabInstanceRoot(
                m_OpenedFromInstanceRoot);
        bool canHaveChangedCurrentlyHiddenObjects =
            outerMostInstanceRoot == instance;
        if (canHaveChangedCurrentlyHiddenObjects) {
          SetPrefabInstanceHiddenForInContextEditing(
              true); // if new GameObjects was added to the instance this will
                     // ensure to mark them hidden as well
        }
      }
    }

    void SetPrefabInstanceHiddenForInContextEditing(bool hide) {
      // In Playmode all instances have been unpacked so we need to check if the
      // PrefabInstanceHandle have been deleted
      if (m_OpenedFromInstanceRoot != null &&
          PrefabUtility.GetPrefabInstanceHandle(m_OpenedFromInstanceRoot) !=
              null)
        StageUtility.SetPrefabInstanceHiddenForInContextEditing(
            m_OpenedFromInstanceRoot, hide);
    }
    ...
    bool LoadStage() {
      ...
      if (!File.Exists(prefabPath)) {
        Debug.LogError(
            "LoadStage with an invalid path: Prefab file not found " +
            prefabPath);
        return false;
      }
      ...
      if (prefabStageMode == Mode.InContext &&
          openedFromInstanceObject == null) {
        Debug.LogError(
            "Invalid LoadStage state: InContext is specified but 'openedFromInstance' is null. This is invalid.");
        return false;
      }

      Init(prefabPath, openedFromInstanceObject, prefabStageMode, contextStage);

      // Ensure scene is set before calling LoadPrefabIntoPreviewScene() so the
      // user can request the current the PrefabStage in their OnEnable and
      // other callbacks (if they use ExecuteInEditMode or ExecuteAlways)
      ...
      m_PrefabContentsRoot =
          PrefabStageUtility.LoadPrefabIntoPreviewScene(prefabPath, scene);
      if (m_PrefabContentsRoot != null) {
        if (isUIPrefab && m_Mode == Mode.InIsolation)
          PrefabStageUtility.HandleReparentingIfNeeded(m_PrefabContentsRoot,
                                                       true);
        ...
        if (m_Mode == Mode.InContext) {
          ...
          if (instanceParent != null) {
            // Insert dummy parent which ensure Prefab contents is aligned same
            // as Prefab instance.
            GameObject dummyParent =
                new GameObject(PrefabUtility.kDummyPrefabStageRootObjectName);
            EditorSceneManager.MoveGameObjectToScene(dummyParent, scene);
            dummyParent.transform.SetParent(
                m_PrefabContentsRoot.transform.parent, false);

            // Make Prefab content parent match alignment of Prefab instance
            // parent. This code doesn't reliably handle cases where the parent
            // matrix is non-orthogonal.
            ...
            if (instanceRectParent != null) {
              ...
              Canvas instanceCanvas =
                  openedFromInstanceRoot.GetComponentInParent<Canvas>();
              if (instanceCanvas != null) {
                dummyCanvas.sortingOrder = instanceCanvas.sortingOrder;
              }
            }

            m_PrefabContentsRoot.transform.SetParent(dummyParent.transform,
                                                     false);
          }
        }
      } else {
    ...
    protected internal override bool OnOpenStage() {
      if (!isCurrentStage) {
        Debug.LogError(
            "Only opening the current PrefabStage is supported. Please report a bug");
        return false;
      }
      ...
      if (LoadStage()) {
        ...
        // Update environment scene objects after the 'prefabStageOpened' user
        // callback so we can ensure: correct hideflags and that our prefab root
        // is not under a prefab instance (which would mark it as an added
        // object). Note: The user can have reparented and created new
        // GameObjects in the environment scene during this callback.
        EnsureParentOfPrefabRootIsUnpacked();
        UpdateEnvironmentHideFlags();
        UpdateLastPrefabSourceFileHashIfNeeded();

        var sceneHierarchyWindows =
            SceneHierarchyWindow.GetAllSceneHierarchyWindows();
        foreach (SceneHierarchyWindow sceneHierarchyWindow in
                     sceneHierarchyWindows)
          sceneHierarchyWindow.FrameObject(prefabContentsRoot.GetInstanceID(),
                                           false);
    ...
    bool HasPatchedPropertyModificationsFor(UnityEngine.Object obj,
                                            string partialPropertyName) {
      if (m_PatchedProperties == null)
        return false;
      foreach (var patchedProperty in m_PatchedProperties) {
        if (patchedProperty.targetInContent == obj &&
            patchedProperty.modification.propertyPath.Contains(
                partialPropertyName))
          return true;
      }
      return false;
    ...
    internal bool
    ContainsTransformPrefabPropertyPatchingFor(GameObject[] gameObjects,
                                               string partialPropertyName) {
      if (gameObjects == null || gameObjects.Length == 0 ||
          m_PatchedProperties == null)
        return false;

      for (int i = 0; i < gameObjects.Length; i++)
        if (HasPatchedPropertyModificationsFor(gameObjects[i].transform,
                                               partialPropertyName))
          return true;
    ...
    void RecordPatchedPropertiesForContent() {
      ...
      Dictionary<ulong, UnityEngine.Object> contentObjectsFromFileID =
          new Dictionary<ulong, UnityEngine.Object>();
      Dictionary<ulong, Transform> instanceTransformsFromFileID =
      ...
      visitor.VisitAll(prefabContentsRoot.transform, (transform, dict) => {
        contentObjectsFromFileID[Unsupported.GetOrGenerateFileIDHint(
      ...
      visitor.VisitAndAllowEarlyOut(
          openedFromInstanceRoot.transform, (transform, dict) => {
            // If the GameObject is not hidden in the scene, it's not part of
            // the Prefab instance that we're hiding because we're opening its
            // Asset. That means it's not an object we should use as source for
            // the patching of properties on the Prefab Asset content.
            if (!StageUtility.IsPrefabInstanceHiddenForInContextEditing(
                    transform.gameObject))
              return false;

            Transform assetTransform =
                PrefabUtility.GetCorrespondingObjectFromSourceAtPath(transform,
                                                                     assetPath);
            // Added GameObjects have no corresponding asset object
            if (assetTransform != null)
              instanceTransformsFromFileID
                  [Unsupported.GetLocalIdentifierInFileForPersistentObject(
                      assetTransform)] = transform;
            return true;
          }, null);

      // Note: openedFromInstance is not necessarily a Prefab root, as it might
      // be a nested Prefab.
      List<GameObject> instanceAndCorrespondingObjectChain =
          new List<GameObject>();
      ...
      while (AssetDatabase.GetAssetPath(prefabObject) != assetPath) {
        ...
        prefabObject =
            PrefabUtility.GetCorrespondingObjectFromSource(prefabObject);
      }

      bool onlyPatchRootTransform = !s_PatchAllOverriddenProperties.value;

      // Run through same objects, but from innermost out so outer overrides are
      // applied last.
      for (int i = instanceAndCorrespondingObjectChain.Count - 1; i >= 0; i--) {
        ...
        PropertyModification[] mods =
            PrefabUtility.GetPropertyModifications(prefabObject);
        ...
        foreach (PropertyModification mod in mods) {
          ...
          // Optimization to avoid doing work to find matching object in Prefab
          // contents for a target we already saw. Performs best if
          // modifications are sorted by target.
          if (mod.target == lastTarget) {
            ...
            UnityEngine.Object targetInOpenAsset =
                PrefabUtility.GetCorrespondingObjectFromSourceAtPath(mod.target,
                                                                     assetPath);
            if (targetInOpenAsset != null) {
              ...
              Transform transform;
              if (instanceTransformsFromFileID.TryGetValue(fileID,
                                                           out transform)) {
          ...
          if (targetInContent != null) {
            // Don't patch root name, as Prefab Mode doesn't support changing
            // that.
            if (targetInContent == prefabContentsRoot &&
                mod.propertyPath == "m_Name")
              continue;

            if (onlyPatchRootTransform &&
                targetInContent != prefabContentsRoot.transform)
              continue;

            if (instanceTransformSO != null) {
              // If properties on a Prefab instance are driven, they are
              // serialized as 0, which means their Prefab override values will
              // be 0 as well. Patching over values of 0 for Prefab Mode in
              // Context is no good, so get the actual current value from the
              // instance instead.
              PropertyModification modFromValue =
                  instanceTransformSO.ExtractPropertyModification(
                      mod.propertyPath);
              if (modFromValue != null)
                mod.value = modFromValue.value;
            }

            DrivenPropertyManager.TryRegisterProperty(this, targetInContent,
                                                      mod.propertyPath);
            m_PatchedProperties.Add(new PatchedProperty(){
                modification = mod, targetInContent = targetInContent});
          }
        }
      }

      // Apply patched properties from outer stage where applicable.
      // This is so that changes that were patched when going in Prefab Mode in
      // Context don't suddenly change when digging in deeper into nested Prefab
      // or Variant bases.
      var stageHistory = StageNavigationManager.instance.stageHistory;
      int selfIndex = stageHistory.IndexOf(this);
      if (selfIndex >= 1) {
        PrefabStage previous = stageHistory[selfIndex - 1] as PrefabStage;
        if (previous != null && previous.mode == Mode.InContext &&
            previous.m_PatchedProperties != null) {
          List<PatchedProperty> previousPatchedProperties =
              previous.m_PatchedProperties;
          UnityEngine.Object lastTarget = null;
          UnityEngine.Object lastTargetInContent = null;
          for (int i = previousPatchedProperties.Count - 1; i >= 0; i--) {
            ...
            if (mod.target == lastTarget) {
              ...
              UnityEngine.Object targetInOpenAsset =
                  PrefabUtility.GetCorrespondingObjectFromSourceAtPath(
                      mod.target, assetPath);
              if (targetInOpenAsset != null) {
                ulong fileID =
                    Unsupported.GetLocalIdentifierInFileForPersistentObject(
                        targetInOpenAsset);
                contentObjectsFromFileID.TryGetValue(fileID,
                                                     out targetInContent);
              }
              lastTarget = mod.target;
              lastTargetInContent = targetInContent;
            ...
            if (targetInContent != null) {
              // Don't patch root name, as Prefab Mode doesn't support changing
              // that. Root can be a different object than it was in outer
              // Stage, så check again here.
              if (targetInContent == prefabContentsRoot &&
                  mod.propertyPath == "m_Name")
                continue;

              // Root can be a different object than it was in outer Stage, så
              // check again here.
              if (onlyPatchRootTransform &&
                  targetInContent != prefabContentsRoot.transform)
                continue;
    ...
    void ApplyPatchedPropertiesToContent() {
      ...
      for (int i = m_PatchedProperties.Count - 1; i >= 0; i--) {
        ...
        UnityEngine.Object targetInContent =
            m_PatchedProperties[i].targetInContent;
        mod.ApplyToObject(targetInContent);

        // If GameObject.active is an override, applying the value via
        // PropertyModification is not sufficient to actually change active
        // state of the object, nor is calling
        // AwakeFromLoad(kAnimationAwakeFromLoad) afterwards, so explicitly set
        // it here.
    ...
    void Cleanup() {
      ...
      if (scene.IsValid())
        PrefabStageUtility.DestroyPreviewScene(
            scene); // Automatically deletes all GameObjects in scene
    ...
    void ReloadStage() {
      ...
      if (!isCurrentStage) {
        Debug.LogError(
            "Only reloading the current PrefabStage is supported. Please report a bug");
        return;
      }
    ...
    internal override void SyncSceneViewToStage(SceneView sceneView) {
      // The reason we need to set customScene even though we also set
      // overrideSceneCullingMask is because the RenderSettings of the
      // customScene is used to override lighting settings for this SceneView.
      PrefabStage prefabStage = GetContextStage() as PrefabStage;
      sceneView.customScene =
          prefabStage == null ? default(Scene) : prefabStage.scene;
      ...
      case PrefabStage.Mode.InContext:
        sceneView.overrideSceneCullingMask =
    ...
    SyncSceneHierarchyToStage(SceneHierarchyWindow sceneHierarchyWindow) {
      var sceneHierarchy = sceneHierarchyWindow.sceneHierarchy;
      sceneHierarchy.customScenes = new[]{scene};
      sceneHierarchy.customParentForNewGameObjects =
    ...
    OnFirstTimeOpenStageInSceneView(SceneView sceneView) {
      ...
      // Default to not showing skybox if user did not specify a custom
      // environment scene.
      if (string.IsNullOrEmpty(scene.path))
        sceneView.sceneViewState.showSkybox = false;

      // For UI to frame properly we need to delay one full Update for the
      // layouting to have been processed
      EditorApplication.update += DelayedFraming;
    }

    internal override void OnFirstTimeOpenStageInSceneHierachyWindow(
        SceneHierarchyWindow sceneHierarchyWindow) {
      var expandedIDs = new List<int>();
      AddParentsBelowButIgnoreNestedPrefabsRecursive(
          prefabContentsRoot.transform, expandedIDs);
      expandedIDs.Sort();
      sceneHierarchyWindow.sceneHierarchy.treeViewState.expandedIDs =
          expandedIDs;
    }

    void AddParentsBelowButIgnoreNestedPrefabsRecursive(
        Transform transform, List<int> gameObjectInstanceIDs) {
      ...
      for (int i = 0; i < count; ++i) {
        ...
        if (child.childCount > 0 &&
          ...
          AddParentsBelowButIgnoreNestedPrefabsRecursive(child,
    ...
    static DragAndDropVisualMode PrefabModeDraggingHandler(
        GameObjectTreeViewItem parentItem, GameObjectTreeViewItem targetItem,
        TreeViewDragging.DropPosition dropPos, bool perform) {
      var prefabStage =
          StageNavigationManager.instance.currentStage as PrefabStage;
      if (prefabStage == null)
        throw new InvalidOperationException(
            "PrefabModeDraggingHandler should only be called in Prefab Mode");

      // Disallow dropping as sibling to the prefab instance root (In Prefab
      // Mode we only want to show one root).
      if (parentItem != null && parentItem.parent == null &&
          dropPos != TreeViewDragging.DropPosition.Upon)
        return DragAndDropVisualMode.Rejected;

      // Disallow dragging scenes into the hierarchy when it is in Prefab Mode
      // (we do not support multi-scenes for prefabs yet)
      foreach (var dragged in DragAndDrop.objectReferences) {
        if (dragged is SceneAsset)
          return DragAndDropVisualMode.Rejected;
      }

      // Check for cyclic nesting (only on perform since it is an expensive
      // operation)
      if (perform) {
        ...
        foreach (var dragged in DragAndDrop.objectReferences) {
          if (dragged is GameObject && EditorUtility.IsPersistent(dragged)) {
            ...
            if (PrefabUtility.CheckIfAddingPrefabWouldResultInCyclicNesting(
                    prefabAssetThatIsAddedTo, prefabAssetThatWillBeAdded)) {
    ...
    internal override string GetErrorMessage() {
      if (m_PrefabContentsRoot == null)
        return L10n.Tr(
                   "Error: The Prefab contents root has been deleted.\n\nPrefab: ") +
               m_PrefabAssetPath;

      if (m_PrefabContentsRoot.scene != scene)
        return L10n.Tr(
                   "Error: The root GameObject of the opened Prefab has been moved out of the Prefab Stage scene by a script.\n\nPrefab: ") +
               m_PrefabAssetPath;
    ...
    internal override BreadcrumbBar.Item CreateBreadcrumbItem() {
      ...
      var style =
          isLastCrumb ? BreadcrumbBar.DefaultStyles.labelBold
          : BreadcrumbBar.DefaultStyles.label;
      var separatorstyle =
          mode ==
          Mode.InIsolation ? BreadcrumbBar.SeparatorStyle.Line
          : BreadcrumbBar.SeparatorStyle.Arrow;
      if (isAssetMissing) {
        style = isLastCrumb ? BreadcrumbBar.DefaultStyles
                    .labelBoldMissing
            : BreadcrumbBar.DefaultStyles.labelMissing;
        content.tooltip = L10n.Tr("Prefab Asset has been deleted.");
      }

      return new BreadcrumbBar.Item{content = content, guistyle = style,
                                    userdata = this,
    ...
    void EnsureParentOfPrefabRootIsUnpacked() {
      ...
      if (parent != null) {
        if (PrefabUtility.IsPartOfPrefabInstance(parent)) {
          var outerMostPrefabInstance =
              PrefabUtility.GetOutermostPrefabInstanceRoot(parent);
          PrefabUtility.UnpackPrefabInstance(outerMostPrefabInstance,
    ...
    bool readyToAutoSave {
      get {
        return m_PrefabContentsRoot != null && hasUnsavedChanges &&
               GUIUtility.hotControl == 0 && !isTextFieldCaretShowing &&
      ...
      if (m_PrefabWasChangedOnDisk) {
        ...
        if (hasUnsavedChanges) {
          ...
          var message = string.Format(
              L10n.Tr(
                  "You have modifications to the Prefab '{0}' that was changed on disk while in Prefab Mode. Do you want to keep your changes or reload the Prefab and discard your changes?"),
              m_PrefabContentsRoot.name);
          bool keepChanges = EditorUtility.DisplayDialog(
              title, message, L10n.Tr("Keep Changes"),
              L10n.Tr("Discard Changes"));
          if (!keepChanges)
            ReloadStage();
        } else {
          ReloadStage();
    ...
    bool PromptIfMissingBasePrefabForVariant() {
      if (PrefabUtility.IsPrefabAssetMissing(m_PrefabContentsRoot)) {
        ...
        string message = L10n.Tr(
            "Can't save the Prefab Variant when its base Prefab is missing. You have to unpack the root GameObject or recover the missing base Prefab in order to save the Prefab Variant");
        if (autoSave)
    ...
    // Returns true if we can continue. Either the user saved changes or user
    // discarded changes. Returns false if the user cancelled switching stage.
    internal override bool AskUserToSaveModifiedStageBeforeSwitchingStage() {
      // Allow user to save any unsaved changes only after recompiling have
      // finished so any new scripts can be saved properly to the Prefab file
      // (but only if the stage is valid)
      if (isValid) {
        if (EditorApplication.isCompiling && hasUnsavedChanges) {
          SceneView.ShowNotification(
              "Compiling must finish before you can exit Prefab Mode");
          SceneView.RepaintAll();
          return false;
        }

        bool continueDestroyingScene =
            AskUserToSaveDirtySceneBeforeDestroyingScene();
    ...
    internal bool SavePrefab() {
      ...
      PrefabUtility.SaveAsPrefabAsset(m_PrefabContentsRoot, m_PrefabAssetPath,
                                      out savedSuccesfully);
      m_LastSavingDuration =
    ...
    internal override bool SaveAsNew() {
      Assert.IsTrue(
          m_PrefabContentsRoot != null,
          "We should have a valid m_PrefabContentsRoot when saving to prefab asset");

      string directoryOfCurrentPrefab =
          Path.GetDirectoryName(m_PrefabAssetPath);
      string nameOfCurrentPrefab =
          Path.GetFileNameWithoutExtension(m_PrefabAssetPath);
      string relativePath;
      while (true) {
        relativePath = EditorUtility.SaveFilePanelInProject(
            "Save Prefab", nameOfCurrentPrefab + " Copy", "prefab", "",
            directoryOfCurrentPrefab);
        ...
        if (relativePath == m_PrefabAssetPath) {
          if (EditorUtility.DisplayDialog(
                  L10n.Tr("Save Prefab has failed"),
                  L10n.Tr(
                      "Overwriting the same path as another open prefab is not allowed."),
                  L10n.Tr("Try Again"), L10n.Tr("Cancel")))
            continue;
    ...
    void AutoSave() {
      showingSavingLabel =
          m_LastSavingDuration > kDurationBeforeShowingSavingBadge;
    ...
    internal override bool Save() {
      ...
      if (!PrefabUtility.PromptAndCheckoutPrefabIfNeeded(
              m_PrefabAssetPath, PrefabUtility.SaveVerb.Save)) {
    ...
    bool CheckRenamedPrefabRootWhenSaving(bool showCancelButton) {
      ...
      if (m_PrefabContentsRoot.name != prefabFilename) {
        ...
        string newPath =
            Path.Combine(folder, m_PrefabContentsRoot.name + extension);
        string errorMsg =
            AssetDatabase.ValidateMoveAsset(m_PrefabAssetPath, newPath);

        if (!string.IsNullOrEmpty(errorMsg)) {
          var t = L10n.Tr("Rename Prefab File Not Possible");
          var m = string.Format(
              L10n.Tr(
                  "The Prefab file name must match the Prefab root GameObject name but there is already a Prefab asset with the file name '{0}' in the same folder. The root GameObject name will therefore be changed back to match the Prefab file name when saving."),
              m_PrefabContentsRoot.name);

          if (showCancelButton) {
            if (EditorUtility.DisplayDialog(t, m, L10n.Tr("OK"),
                                            L10n.Tr("Cancel Save"))) {
        ...
        var message = string.Format(
            L10n.Tr(
                "The Prefab file name must match the Prefab root GameObject name. Do you want to rename the file to '{0}' or use the old name '{1}' for both?"),
            m_PrefabContentsRoot.name, prefabFilename);

        if (showCancelButton) {
          int option = EditorUtility.DisplayDialogComplex(
              title, message, L10n.Tr("Rename File"), L10n.Tr("Use Old Name"),
              L10n.Tr("Cancel Save"));
          switch (option) {
          // Rename prefab file
          case 0:
          ...
          bool renameFile = EditorUtility.DisplayDialog(
              title, message, L10n.Tr("Rename File"), L10n.Tr("Use Old Name"));
          if (renameFile)
            RenamePrefabFileToMatchPrefabInstanceName();
          else
    ...
    internal void RenamePrefabFileToMatchPrefabInstanceName() {
      ...
      if (SceneHierarchy.s_DebugPrefabStage)
        Debug.LogFormat("RENAME Prefab Asset: '{0} to '{1}'", m_PrefabAssetPath,
                        newPrefabFileName);

      // If we don't ignore the next import event we will reload the current
      // prefab and hereby loosing the current selection (inspector goes blank)
      // Since we have made the change ourselves (rename) we don't need to
      // reload our instances.
      m_IgnoreNextAssetImportedEventForCurrentPrefab = true;

      string errorMsg =
          AssetDatabase.RenameAsset(m_PrefabAssetPath, newPrefabFileName);
      if (!string.IsNullOrEmpty(errorMsg))
    ...
    // (ok to continue to destroying scene)
    internal bool AskUserToSaveDirtySceneBeforeDestroyingScene() {
      ...
      if (IsPrefabInImmutableFolder()) {
        ...
        var message = L10n.Tr(
            "The Prefab was changed in Prefab Mode but is in a read-only folder so the changes cannot be saved.");
        var buttonOK = L10n.Tr("OK");
        var buttonCancel = L10n.Tr("Cancel");

        if (EditorUtility.DisplayDialog(header, message, buttonOK,
                                        buttonCancel))
          return true; // OK: continue to close stage
        return false;  // Cancel closing stage
      }

      // Rare condition. Prefab should have already been saved if auto-save is
      // enabled, but it's possible it hasn't, so save when exiting just in
      // case.
      if (autoSave)
        return Save();
      ...
      int dialogResult = EditorUtility.DisplayDialogComplex(
          ...
          "Do you want to save the changes you made in Prefab Mode? Your changes will be lost if you don't save them.",
      ...
      default:
        throw new InvalidOperationException("Unhandled dialog result " +
                                            dialogResult);
      }
    }

    internal void OnSavingPrefab(GameObject gameObject, string path) {
      // We care about 'irrelevant prefab paths' since the 'irrelevant' prefab
      // could be a nested prefab to the current prefab being edited (this would
      // result in our current prefab being reimported due to the dependency).
      // For nested prefabs the prefab merging code will take care of updating
      // the current GameObjects loaded so in this case do not reload the prefab
      // even though the path shows up in AssetsChangedOnHDD event.
    ...
    internal void OnAssetsChangedOnHDD(string[] importedAssets,
                                       string[] deletedAssets,
                                       string[] movedAssets,
                                       string[] movedFromAssetPaths) {
      if (SceneHierarchy.s_DebugPrefabStage)
        Debug.LogFormat(
            "AssetDatabase Event: Current Prefab: {0}, *ImportedAssets: {1}, *DeletedAssets: {2}, *MovedAssets: {3}, *MovedFromAssetPaths: {4}",
            m_PrefabAssetPath, DebugUtils.ListToString(importedAssets),
            DebugUtils.ListToString(deletedAssets),
            DebugUtils.ListToString(movedAssets),
            DebugUtils.ListToString(movedFromAssetPaths));

      // Prefab was moved (update cached path)
      for (int i = 0; i < movedFromAssetPaths.Length; ++i) {
        if (movedFromAssetPaths [i]
                .Equals(m_PrefabAssetPath,
      ...
      // Detect if our Prefab was modified on HDD outside Prefab Mode (in that
      // case we should ask the user if he wants to reload it)
      for (int i = 0; i < importedAssets.Length; ++i) {
        if (importedAssets[i] == m_PrefabAssetPath) {
          m_IsAssetMissing = false;
          if (UpdateLastPrefabSourceFileHashIfNeeded() &&
              !m_IgnoreNextAssetImportedEventForCurrentPrefab) {
            if (isCurrentStage) {
          ...
          // Reset the ignore flag when we finally have imported the saved
          // prefab (We set this flag when saving the Prefab from Prefab Mode)
          // Note we can get multiple OnAssetsChangedOnHDD events before the
          // Prefab imported event if e.g folders of the Prefab path needs to be
          // reimported first.
          m_IgnoreNextAssetImportedEventForCurrentPrefab = false;
          break;
        }
    ...
    // This method is not called from the SceneView if the SceneView does not
    // support stage handling
    internal override void OnPreSceneViewRender(SceneView sceneView) {
      ...
      StageUtility.SetFocusedSceneContextRenderMode(
          StageNavigationManager.instance.contextRenderMode);
      sceneView.SetSceneViewFilteringForStages(
          StageNavigationManager.instance.contextRenderMode ==
          StageUtility.ContextRenderMode.GreyedOut);
    }
    ...
    internal override void OnPostSceneViewRender(SceneView sceneView) {
      StaticOcclusionCullingVisualization.showOcclusionCulling =
          OcclusionCullingWindow.isVisible;
      ...
      StageUtility.SetFocusedSceneContextRenderMode(
          StageUtility.ContextRenderMode.Normal);
      sceneView.SetSceneViewFilteringForStages(false);
    }

    Texture2D DeterminePrefabFileIconFromInstanceRootGameObject() {
      bool partOfInstance =
          PrefabUtility.IsPartOfNonAssetPrefabInstance(prefabContentsRoot);
      bool disconnected =
          PrefabUtility.GetPrefabInstanceStatus(prefabContentsRoot) ==
    ...
    internal override void
    SaveHierarchyState(SceneHierarchyWindow hierarchyWindow) {
      if (!isValid)
        return;

      Hash128 key = StageUtility.CreateWindowAndStageIdentifier(
          hierarchyWindow.windowGUID, this);
    ...
    PrefabStageHierarchyState
    GetStoredHierarchyState(SceneHierarchyWindow hierarchyWindow) {
      Hash128 key = StageUtility.CreateWindowAndStageIdentifier(
          hierarchyWindow.windowGUID, this);
    ...
    void InitContextRenderModeSelector() {
      ...
      List<ExposablePopupMenu.ItemData> buttonData =
          new List<ExposablePopupMenu.ItemData>();
      ...
      for (int i = 0; i < Styles.contextRenderModeTexts.Length; ++i) {
        ...
        buttonData.Add(new ExposablePopupMenu.ItemData(
            Styles.contextRenderModeTexts[i], on ? onStyle
            : offStyle, on, true, Styles.contextRenderModeOptions[i]));
      }

      GUIContent popupButtonContent =
          Styles.contextRenderModeTexts[(int) currentState];

      ExposablePopupMenu.PopupButtonData popListData =
          new ExposablePopupMenu.PopupButtonData(popupButtonContent,
                                                 Styles.exposablePopup);
      s_ContextRenderModeSelector.Init(Styles.contextLabel, buttonData, 4f, 50,
                                       popListData,
    ...
    void
    ContextRenderModeClickedCallback(ExposablePopupMenu.ItemData itemClicked) {
      // Behave like radio buttons: a button that is on cannot be turned off
      if (!itemClicked.m_On) {
        var selectedMode =
            (StageUtility.ContextRenderMode) itemClicked.m_UserData;
    ...
    void InContextModeSelector(SceneView sceneView) {
      ...
      var rect = GUILayoutUtility.GetRect(
          s_ContextRenderModeSelector.widthOfPopupAndLabel,
          s_ContextRenderModeSelector.widthOfButtonsAndLabel, 0, 22);
    ...
    internal static SavedBool s_PatchAllOverriddenProperties =
        new SavedBool("InContextEditingPatchOverriddenProperties", false);

    void VisualizeOverridesToggle() {
      EditorGUI.BeginChangeCheck();
      bool patchAll = GUILayout.Toggle(s_PatchAllOverriddenProperties.value,
                                       Styles.showOverridesLabel);
      if (EditorGUI.EndChangeCheck()) {
    ...
    void CachePrefabFolderInfo() {
      bool isRootFolder;
      m_IsPrefabInValidAssetFolder = AssetDatabase.GetAssetFolderInfo(
          m_PrefabAssetPath, out isRootFolder, out m_IsPrefabInImmutableFolder);
    }
    ...
    void AutoSaveButtons(SceneView sceneView) {
      ...
      StatusQueryOptions opts =
          EditorUserSettings.allowAsyncStatusUpdate ? StatusQueryOptions
              .UseCachedAsync : StatusQueryOptions.UseCachedIfPossible;
      ...
      if (!autoSave) {
        using(new EditorGUI.DisabledScope(
            (!openForEdit || !hasUnsavedChanges) && !isAssetMissing)) {
...

      {
        ...
        EditorGUI.BeginChangeCheck();
        autoSaveForScene = GUILayout.Toggle(
            autoSaveForScene, Styles.autoSaveGUIContent, Styles.saveToggle);
        if (EditorGUI.EndChangeCheck())
    ...
    internal const HideFlags kVisibleEnvironmentObjectHideFlags =
        HideFlags.DontSave | HideFlags.NotEditable;
    internal const HideFlags kNotVisibleEnvironmentObjectHideFlags =
    ...
    class HideFlagUtility {
      ...
      public void UpdateEnvironmentHideFlags() {
        ...
        // We use hideflags to hide all environment objects (and make them
        // non-editable since the user cannot save them)
        ...
        foreach (GameObject go in m_Roots) {
          ...
          SetHideFlagsRecursively(go.transform,
                                  kNotVisibleEnvironmentObjectHideFlags);
        }

        if (rootOfPrefabInstance != m_PrefabInstanceRoot) {
          // Our prefab instance root might be a child of an environment object.
          // Here we set those environment objects hidden (leaving the root
          // prefab instance unchanged)
          SetHideFlagsRecursivelyWithIgnore(
              rootOfPrefabInstance.transform, m_PrefabInstanceRoot.transform,
              kNotVisibleEnvironmentObjectHideFlags);
      ...
      static void SetHideFlagsRecursively(Transform transform,
                                          HideFlags hideFlags) {
      ...
      static void
      SetHideFlagsRecursivelyWithIgnore(Transform transform,
                                        Transform ignoreThisTransform,
                                        HideFlags hideFlags) {
        ...
        if (transform.gameObject.hideFlags != hideFlags)
          transform.gameObject.hideFlags =
              hideFlags; // GameObject also sets all components so check before
                         // setting

        for (int i = 0; i < transform.childCount; i++)
          SetHideFlagsRecursivelyWithIgnore(transform.GetChild(i),
                                            ignoreThisTransform, hideFlags);
      }

      void ValidatePreviewSceneConsistency() {
        if (!ValidatePreviewSceneState.ValidatePreviewSceneObjectState(
                m_PrefabInstanceRoot)) {
    ...
    static class ValidatePreviewSceneState {
      ...
      static void ValidateGameObject(Transform transform, object userData) {
        ...
        if (!EditorSceneManager.IsPreviewSceneObject(go)) {
          m_Errors.Add(
              "   GameObject not correctly marked as PreviewScene object: " +
              go.name);
        }
        ...
        foreach (var c in components) {
          ...
          if (!EditorSceneManager.IsPreviewSceneObject(c))
            m_Errors.Add(string.Format(
                "   Component {0} not correctly marked as PreviewScene object: (GameObject: {1})",
                c.GetType().Name, go.name));
        }
      }
    }
    ...
    void OnPlayModeStateChanged(PlayModeStateChange playmodeState) {
      if (playmodeState == PlayModeStateChange.ExitingEditMode) {
        bool blockPrefabModeInPlaymode =
            PrefabStageUtility
                .CheckIfAnyComponentShouldBlockPrefabModeInPlayMode(assetPath);
        if (blockPrefabModeInPlaymode) {
          StageNavigationManager.instance.GoToMainStage(
              StageNavigationManager.Analytics.ChangeType
                  .GoToMainViaPlayModeBlocking);
        }
      }

      if (playmodeState == PlayModeStateChange.EnteredEditMode) {
        // When exiting play mode we reload scenes and if we are in in Prefab
        // Mode in Context we need to reconstruct the hidden state for the the
        // instance in the previous stage (this is setup when entering Prefab
        // Mode in Context)
        if (mode == Mode.InContext && m_OpenedFromInstanceRoot != null)
          SetPrefabInstanceHiddenForInContextEditing(true);
      }
    ...
    static bool OnOpenAsset(int instanceID, int line) {
      ...
      if (assetPath.EndsWith(".prefab", StringComparison.OrdinalIgnoreCase)) {
        // The 'line' parameter can be used for passing an instanceID of a
        // prefab instance
        GameObject instanceRoot =
            line == -1 ? null
                       : EditorUtility.InstanceIDToObject(line) as GameObject;
        var prefabStageMode =
            instanceRoot != null ? PrefabStage.Mode.InContext
            : PrefabStage.Mode.InIsolation;
        PrefabStageUtility.OpenPrefab(
            assetPath, instanceRoot, prefabStageMode,
            StageNavigationManager.Analytics.ChangeType.EnterViaAssetOpened);
        return true;
      }
      return false;
    }
