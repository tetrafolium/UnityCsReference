...
internal class StageNavigationManager
	: ScriptableSingleton<StageNavigationManager> {
StageNavigationHistory
        m_NavigationHistory; // Not marked with SerializeField since it should
                             // be reset on every restart of Unity
SavedBool m_AutoSave = new SavedBool("PrefabEditing.AutoSave", true);
SavedInt m_ContextRenderMode =
	new SavedInt("PrefabEditing.ContextRenderMode",
	             (int) StageUtility.ContextRenderMode.GreyedOut);
Analytics m_Analytics = new Analytics();
[NonSerialized] double m_NextUpdate;
[NonSerialized] bool m_DebugLogging = false;
...
internal Stage currentStage {
	...
	// No setter since invoking code should explicitly specify desired effect
	// on history.
}
...
void OnQuit() {
	// Ensure we destroy the prefab stages so all hidden editor objects
	// (environment objects are hidden) are removed before closing down.
	// Currently editor objects should be cleaned up if they have interest in
	// transform changes before shutting down.
...
internal Stage GetStage(Scene scene) {
	...
	var result = stageHistory.FirstOrDefault(stage => stage.stageHandle ==
	                                         inputStageHandle);
	return result;
}
...
void OnSceneOpened(Scene scene, OpenSceneMode mode) {
	if (!(currentStage is MainStage)) {
		GoToMainStage(
			Analytics.ChangeType
			.GoToMainViaSceneOpened); // Do not set previous selection as
		                                  // this would e.g remove the selection
		                                  // from the Project Browser when
		                                  // double clicking a scene asset
	}
}

void OnNewSceneCreated(Scene scene, NewSceneSetup setup,
                       NewSceneMode mode) {
	if (!(currentStage is MainStage)) {
		...
		if (stage == null) {
			if (showDialogIfInvalid)
				EditorUtility.DisplayDialog(
					"Stage Error",
					"A stage has been destroyed unexpectedly.\n\nReturning to the MainStage.",
					"OK");
...
internal void GoToMainStage(Analytics.ChangeType stageChangeAnalytics) {
	...
	SwitchToStage(m_NavigationHistory.GetMainStage(), false,
	              stageChangeAnalytics);
}
...
internal bool SwitchToStage(Stage stage, bool setAsFirstItemAfterMainStage,
                            Analytics.ChangeType changeTypeAnalytics) {
	if (stage == null) {
	...
	// Used by the Avatar Editor to exit its editing mode before opening new
	// stage
	stageChanging?.Invoke(previousStage, stage);

	// Set/Add stage in m_NavigationHistory (and detect what stages should be
	// removed)
	if (m_DebugLogging)
		Debug.Log("Set Navigation History (setAsFirstItemAfterMainStage " +
	...
	if (setAsFirstItemAfterMainStage) {
		...
		if (m_NavigationHistory.TrySetToIndexOfItem(stage))
			...
			deleteStages =
				m_NavigationHistory.ClearForwardHistoryAndAddItem(stage);
	}
	...
	// Activate stage after setting up the history above so objects loaded
	// during ActivateStage can query current stage
	bool success;
	try {
		if (!stage.opened) {
			stage.opened = true;
	...
	if (success) {
		...
		// A previous existing stage can have been requested to set as the
		// current so don't delete that
		deleteStages.Remove(stage);

		DeleteStagesInReverseOrder(deleteStages);

		// Here we update state that relies on old scenes having been destroyed
		// entirely.
...
struct PreviewSceneLeakDetection {
	...
	internal void LogErrorIfPreviewSceneWasNotDestroyed() {
		if (m_StageType != null && m_Scene.IsValid())
			Debug.LogError(
				$"Stage type '{m_StageType}' did not clean up properly: A PreviewScene was leaked. Ensure to call 'base.OnCloseStage()' from your implementation of OnCloseStage().");
	}
}
...
static void DeleteStagesInReverseOrder(List<Stage> stagesToDelete) {
	...
	// Remove in reverse order of added (simulates going back one stage at a
	// time)
...
void RecoverFromStageChangeError(Stage previousStage,
                                 List<Stage> deleteStages) {
	// Recover by going back to main stage
	if (m_NavigationHistory.TrySetToIndexOfItem(
		    m_NavigationHistory.GetMainStage())) {
		var lastStages =
			m_NavigationHistory.ClearForwardHistoryAfterCurrentStage();
		if (lastStages.Count > 0)
			deleteStages.InsertRange(0,
			                         lastStages); // Insert at start so we ensure
		                                              // the same order as in history

		try {
			// Here the Hierarchy and SceneView sync's up to the new stage
			stageChanged?.Invoke(previousStage,
			                     m_NavigationHistory.GetMainStage());
		} catch (Exception e) {
			Debug.LogError("Error while recovering from Stage change error: " +
			               e);
		}
...
public void ChangingStageStarted(Stage previousStage) {
	...
	m_EventData.existingBreadcrumbCount =
		StageNavigationManager.instance.stageHistory.Count;
