...
class PrefabStageHierarchyState {
  [SerializeField] float m_ScrollY;
  [SerializeField] List<UInt64> m_ExpandedPrefabGameObjectFileIDs =
      new List<UInt64>();
  [SerializeField] UInt64 m_LastClickedFileID;

  public void SaveStateFromHierarchy(SceneHierarchyWindow hierarchy,
                                     Stage stage) {
    var lastClickedGameObject = EditorUtility.InstanceIDToObject(
        hierarchy.sceneHierarchy.treeViewState.lastClickedID) as GameObject;
    m_LastClickedFileID =
        lastClickedGameObject !=
        null ? Unsupported.GetOrGenerateFileIDHint(lastClickedGameObject) : 0;
  ...
  public void LoadStateIntoHierarchy(SceneHierarchyWindow hierarchy,
                                     Stage stage) {
    // Restore expanded state always
    RestoreExandedStateFromFileIDs(hierarchy, stage as PrefabStage,
  ...
  static List<UInt64>
  GetExpandedGameObjectFileIDs(SceneHierarchyWindow hierarchy) {
  ...
  static void RestoreExandedStateFromFileIDs(
      SceneHierarchyWindow hierarchy, PrefabStage prefabStage,
      List<UInt64> expandedGameObjectFileIDs, UInt64 lastClickedFileID) {
    var searchRoot = prefabStage.prefabContentsRoot;
    var fileIDToInstanceIDMapper = new FileIDToInstanceIDMapper(
        searchRoot.transform, expandedGameObjectFileIDs, lastClickedFileID);
    hierarchy.sceneHierarchy.treeViewState.lastClickedID =
        fileIDToInstanceIDMapper.instanceID;
    hierarchy.sceneHierarchy.treeViewState.expandedIDs =
        fileIDToInstanceIDMapper.instanceIDs;
    hierarchy.sceneHierarchy.treeViewState.expandedIDs
  ...
  class FileIDToInstanceIDMapper {
    ...
    public FileIDToInstanceIDMapper(Transform searchRoot, List<UInt64> fileIDs,
                                    UInt64 fileID) {
      m_FileIDs = fileIDs;
      m_FileID = fileID;
  ...
  void DebugLog(string prefix, Stage stage) {
    Debug.Log(
        prefix + (stage.GetType().ToString()) +
        string.Format(
            "-prefab stage: {0}, -scrollY: {1}, -selection {2}, -setSelection {3}",
            DebugUtils.ListToString(m_ExpandedPrefabGameObjectFileIDs),
            m_ScrollY, m_LastClickedFileID,
