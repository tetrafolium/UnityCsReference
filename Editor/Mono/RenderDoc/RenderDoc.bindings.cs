...
public static partial class RenderDoc {
  ...
  static WindowAction RenderDocGlobalAction() {
    ...
    // content/panels
    var action = WindowAction.CreateWindowActionButton(
        "RenderDoc", CaptureRenderDocFullContent, null,
        ContainerWindow.kButtonWidth + 1, RenderDocCaptureButton);
  ...
  internal static bool RenderDocCaptureButton(EditorWindow view,
                                              WindowAction self, Rect r) {
    if (s_RenderDocContent == null)
      s_RenderDocContent = EditorGUIUtility.TrIconContent(
  ...
  private static void CaptureRenderDocFullContent(EditorWindow view,
                                                  WindowAction self) {
    view.m_Parent.CaptureRenderDocFullContent();
  }

  private static bool ShowRenderDocButton(EditorWindow view,
                                          WindowAction self) {
