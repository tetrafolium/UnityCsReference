...
public static bool CheckFileCollisions(BuildTarget buildTarget) {
	...
	if (ModuleManager.IsPlatformSupported(buildTarget))
		pluginImporterExtension =
			ModuleManager.GetPluginImporterExtension(buildTarget);
	if (pluginImporterExtension == null) {
		// Some platforms don't have platform specific settings for plugins, but
		// we still want to check that plugins don't collide, use default path in
		// this case
		if (BuildPipeline.GetBuildTargetGroup(buildTarget) ==
	...
	if (pluginImporterExtension.CheckFileCollisions(
