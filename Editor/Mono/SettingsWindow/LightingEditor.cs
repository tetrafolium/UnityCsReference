...
internal enum DefaultReflectionMode { FromSkybox = 0, Custom }
...
internal class LightingEditor : Editor {
  internal static class Styles {
    ...
    public static readonly GUIContent env_skybox_mat = EditorGUIUtility.TrTextContent(
        "Skybox Material",
        "Specifies the material that is used to simulate the sky or other distant background in the Scene.");
    public static readonly GUIContent env_skybox_sun = EditorGUIUtility.TrTextContent(
        "Sun Source",
    ...
    public static readonly GUIContent env_amb_top =
        EditorGUIUtility.TrTextContent("Environment Lighting");
    public static readonly GUIContent env_amb_src = EditorGUIUtility.TrTextContent(
        "Source",
    ...
    public static readonly GUIContent env_refl_src = EditorGUIUtility.TrTextContent(
        "Source",
        "Specifies whether to use the skybox or a custom cube map for reflection effects in the Scene.");
    public static readonly GUIContent env_refl_res = EditorGUIUtility.TrTextContent(
        "Resolution",
        "Controls the resolution for the cube map assigned to the skybox material for reflection effects in the Scene.");
    public static readonly GUIContent env_refl_cmp = EditorGUIUtility.TrTextContent(
        "Compression",
        "Controls how Unity compresses the reflection cube maps. Options are Auto, Compressed, and Uncompressed. Auto compresses the cube maps if the compression format is suitable.");
    public static readonly GUIContent env_refl_int = EditorGUIUtility.TrTextContent(
        "Intensity Multiplier",
    ...
    public static readonly GUIContent env_refl_bnc = EditorGUIUtility.TrTextContent(
        "Bounces",
    ...
    public static readonly GUIContent skyboxWarning =
        EditorGUIUtility.TrTextContent(
            "Shader of this material does not support skybox rendering.");
    public static readonly GUIContent createLight =
        EditorGUIUtility.TrTextContent("Create Light");
    ...
    public static readonly GUIContent ambient = EditorGUIUtility.TrTextContent(
        "Ambient Color",
    ...
    public static readonly GUIContent SubtractiveColor =
        EditorGUIUtility.TrTextContent(
    ...
    public static readonly int[] kFullAmbientSourceValues = {
        (int) AmbientMode.Skybox, (int) AmbientMode.Trilight,
  ...
  SerializedObject renderSettings {
    get {
      ...
      if (m_RenderSettings == null || m_RenderSettings.targetObject == null ||
        ...
        m_SubtractiveShadowColor =
            m_RenderSettings.FindProperty("m_SubtractiveShadowColor");
        ...
        m_AmbientEquatorColor =
            m_RenderSettings.FindProperty("m_AmbientEquatorColor");
        m_AmbientGroundColor =
            m_RenderSettings.FindProperty("m_AmbientGroundColor");
        m_AmbientIntensity =
            m_RenderSettings.FindProperty("m_AmbientIntensity");
        m_ReflectionIntensity =
            m_RenderSettings.FindProperty("m_ReflectionIntensity");
        m_ReflectionBounces =
            m_RenderSettings.FindProperty("m_ReflectionBounces");
        m_SkyboxMaterial = m_RenderSettings.FindProperty("m_SkyboxMaterial");
        m_DefaultReflectionMode =
            m_RenderSettings.FindProperty("m_DefaultReflectionMode");
        m_DefaultReflectionResolution =
            m_RenderSettings.FindProperty("m_DefaultReflectionResolution");
        m_CustomReflection =
            m_RenderSettings.FindProperty("m_CustomReflection");
      }
  ...
  SerializedObject lightmapSettings {
    get {
      ...
      if (m_LightmapSettings == null ||
          m_LightmapSettings.targetObject == null ||
        ...
        m_ReflectionCompression = m_LightmapSettings.FindProperty(
            "m_LightmapEditorSettings.m_ReflectionCompression");
      }
  ...
  private void DrawGUI() {
    ...
    m_bShowEnvironment = EditorGUILayout.FoldoutTitlebar(m_bShowEnvironment,
                                                         Styles.env_top, true);
    ...
    if (m_bShowEnvironment) {
      ...
      if (skyboxMaterial &&
          !EditorMaterialUtility.IsBackgroundMaterial(skyboxMaterial)) {
      ...
      EditorGUILayout.PropertyField(m_SubtractiveShadowColor,
                                    Styles.SubtractiveColor);
      EditorGUILayout.Space();

      EditorGUILayout.LabelField(Styles.env_amb_top);
      EditorGUI.indentLevel++;

      EditorGUILayout.IntPopup(m_AmbientSource, Styles.kFullAmbientSource,
                               Styles.kFullAmbientSourceValues,
                               Styles.env_amb_src);
      switch ((AmbientMode) m_AmbientSource.intValue) {
      case AmbientMode.Trilight: {
        EditorGUI.BeginChangeCheck();
        Color newValueUp = EditorGUILayout.ColorField(
            Styles.ambientUp, m_AmbientSkyColor.colorValue, true, false, true);
        Color newValueMid = EditorGUILayout.ColorField(
            Styles.ambientMid, m_AmbientEquatorColor.colorValue, true, false,
            true);
        Color newValueDown = EditorGUILayout.ColorField(
            Styles.ambientDown, m_AmbientGroundColor.colorValue, true, false,
            true);
      ...
      case AmbientMode.Flat: {
        ...
        Color newValue = EditorGUILayout.ColorField(
            Styles.ambient, m_AmbientSkyColor.colorValue, true, false, true);
        if (EditorGUI.EndChangeCheck())
      ...
      case AmbientMode.Skybox:
        if (skyboxMaterial == null) {
          ...
          EditorGUILayout.Slider(m_AmbientIntensity, 0.0F, 8.0F,
                                 Styles.env_amb_int);
        }
        break;
      }
      ...
      EditorGUILayout.PropertyField(m_DefaultReflectionMode,
                                    Styles.env_refl_src);

      DefaultReflectionMode defReflectionMode =
          (DefaultReflectionMode) m_DefaultReflectionMode.intValue;
      ...
      case DefaultReflectionMode.FromSkybox: {
        ...
        ReflectionProbeEditor.GetResolutionArray(
            ref reflectionResolutionValuesArray,
            ref reflectionResolutionTextArray);
        EditorGUILayout.IntPopup(m_DefaultReflectionResolution,
                                 reflectionResolutionTextArray,
                                 reflectionResolutionValuesArray,
                                 Styles.env_refl_res, GUILayout.MinWidth(40));
      } break;
      case DefaultReflectionMode.Custom:
        EditorGUILayout.PropertyField(m_CustomReflection,
                                      Styles.customReflection);
        break;
      }

      EditorGUILayout.PropertyField(m_ReflectionCompression,
                                    Styles.env_refl_cmp);
      EditorGUILayout.Slider(m_ReflectionIntensity, 0.0F, 1.0F,
                             Styles.env_refl_int);
