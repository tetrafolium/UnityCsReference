...
namespace UnityEditor.SceneManagement {
  public static class SceneHierarchyHooks {
    ...
    [UnityEngine.Bindings.NativeType(IntermediateScriptingStructName =
                                         "SceneHierarchyHooks_SubSceneInfo")]
    public struct SubSceneInfo {
      ...
      [UnityEngine.Bindings.Ignore]
      public string sceneName; // TODO: remove when when we have changed our
                               // examples to use 'provideSubSceneName'
      [UnityEngine.Bindings.Ignore]
    ...
    public static event Action<GenericMenu, GameObject>
        addItemsToGameObjectContextMenu;
    public static event Action<GenericMenu, Scene>
        addItemsToSceneHeaderContextMenu;
    public static event Action<GenericMenu, SubSceneInfo>
    ...
    public static bool CanSetNewParent(Transform transform,
                                       Transform newParent) {
      if (transform == null)
      ...
      RefreshSubSceneInfo();
      var parentIsChild =
          SubSceneGUI.IsChildOrSameAsOtherTransform(newParent, transform);
      return !parentIsChild;
    }

    public static bool CanMoveTransformToScene(Transform transform,
                                               Scene scene) {
      if (transform == null)
        throw new ArgumentNullException("transform");
    ...
    AddCustomGameObjectContextMenuItems(GenericMenu menu,
                                        GameObject gameObject) {
      addItemsToGameObjectContextMenu?.Invoke(menu, gameObject);
    }

    internal static void AddCustomSceneHeaderContextMenuItems(GenericMenu menu,
                                                              Scene scene) {
      addItemsToSceneHeaderContextMenu?.Invoke(menu, scene);
    }

    internal static void
    AddCustomSubSceneHeaderContextMenuItems(GenericMenu menu,
