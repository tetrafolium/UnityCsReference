...
// Lets you do ''move'', ''copy'', ''delete'' operations over files or
// directories
...
public partial class FileUtil {
  ...
  public static void CopyFileOrDirectory(string source, string dest) {
    CheckForValidSourceAndDestinationArgumentsAndRaiseAnExceptionWhenNullOrEmpty(
        source, dest);
    ...
    if (!CopyFileOrDirectoryInternal(source, dest)) {
      throw new System.IO.IOException(
          string.Format("Failed to Copy File / Directory from '{0}' to '{1}'.",
                        source, dest));
    }
  }

  [FreeFunction("CopyFileOrDirectory")]
  private static extern bool CopyFileOrDirectoryInternal(string source,
                                                         string dest);

  // Copies the file or directory following symbolic links.
  public static void CopyFileOrDirectoryFollowSymlinks(string source,
                                                       string dest) {
    CheckForValidSourceAndDestinationArgumentsAndRaiseAnExceptionWhenNullOrEmpty(
        source, dest);
  ...
  CopyFileOrDirectoryFollowSymlinksInternal(string source, string dest);
  ...
  public static void MoveFileOrDirectory(string source, string dest) {
    ...
    if (!MoveFileOrDirectoryInternal(source, dest)) {
      throw new System.IO.IOException(
          string.Format("Failed to Move File / Directory from '{0}' to '{1}'.",
                        source, dest));
    }
  }

  [FreeFunction("MoveFileOrDirectory")]
  private static extern bool MoveFileOrDirectoryInternal(string source,
                                                         string dest);
  ...
  [FreeFunction("PathToAbsolutePathFromScript")] extern internal static string
  PathToAbsolutePath(string path);
}
}
