...
static private bool
CompareComponentOrderAndTypes(List<Component> srcComponents,
                              List<Component> dstComponents) {
	if (srcComponents.Count != dstComponents.Count)
		return false;

	for (int i = 0; i != srcComponents.Count; i++) {
		if (srcComponents [i]
		    .GetType() != dstComponents [i]
		    .GetType())
			return false;
	}
...
public static void
DestroyComponentsMatching(GameObject dst,
                          IsDesiredComponent componentFilter) {
...
public static void
ReplaceComponentsIfDifferent(GameObject src, GameObject dst,
                             IsDesiredComponent componentFilter) {
	...
	if (!CompareComponentOrderAndTypes(srcComponents, dstComponents)) {
		...
		dstComponents.Clear();
		for (int i = 0; i != srcComponents.Count; i++) {
			Component com = dst.AddComponent(srcComponents [i]
			                                 .GetType());
	...
	for (int i = 0; i != srcComponents.Count; i++)
		UnityEditor.EditorUtility.CopySerializedIfDifferent(srcComponents[i],
		                                                    dstComponents[i]);
}
}
}
