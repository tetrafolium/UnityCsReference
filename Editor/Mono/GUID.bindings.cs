...
public partial struct GUID : IComparable,
                             IComparable<GUID>,
                             IEquatable<GUID> {
  ...
  public static bool operator ==(GUID x, GUID y) {
    return x.m_Value0 == y.m_Value0 && x.m_Value1 == y.m_Value1 &&
           x.m_Value2 == y.m_Value2 && x.m_Value3 == y.m_Value3;
  }

  public static bool operator !=(GUID x, GUID y) { return !(x == y); }
  ...
  public bool Equals(GUID obj) { return this == obj; }
  ...
  public static GUID Generate() { return GenerateGUIDInternal(); }

  public override string ToString() { return GUIDToHexInternal(ref this); }

  // Thread safety verified, only safe static methods are used.
  [NativeMethod(Name = "GUIDToString", IsFreeFunction = true,
                IsThreadSafe = true)] extern private static string
  GUIDToHexInternal(ref GUID value);

  // Thread safety verified, native method modified to not lazy init global
  // lookup table.
  [NativeMethod(Name = "StringToGUID", IsFreeFunction = true,
                IsThreadSafe = true)] extern private static GUID
  HexToGUIDInternal(string hex);

  [NativeMethod(Name = "GenerateGUID",
                IsFreeFunction = true)] extern private static GUID
  GenerateGUIDInternal();

  [NativeMethod(Name = "CreateGUIDFromSInt64",
                IsFreeFunction = true)] extern internal static GUID
  CreateGUIDFromSInt64(long value);
}
}
