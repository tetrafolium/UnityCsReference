...
using SerializableJsonDictionary =
...
public partial class EditorWindow : ScriptableObject {
  [HideInInspector] bool m_AutoRepaintOnSceneChange;

  [SerializeField]
  [HideInInspector] Vector2 m_MinSize = new Vector2(100, 100);

  [SerializeField]
  [HideInInspector] Vector2 m_MaxSize = new Vector2(4000, 4000);

  [SerializeField]
  [HideInInspector]
  internal GUIContent m_TitleContent;

  // We use a 24-bit depth/stencil buffer by default to support the UIRenderer
  // clipping feature
  [HideInInspector] int m_DepthBufferBits = 24;

  [HideInInspector] int m_AntiAliasing = 1;
  ...
  public VisualElement rootVisualElement =>
      m_UIRootElement ??(m_UIRootElement = CreateRoot());
  ...
  internal SerializableJsonDictionary viewDataDictionary {
    get {
      // If persistence is disabled, just don't even create the dictionary.
      // Return null.
      if (m_EnableViewDataPersistence && m_ViewDataDictionary == null) {
        ...
        m_ViewDataDictionary =
            UIElements.EditorWindowViewData.instance[editorPrefFileName];
      }
      return m_ViewDataDictionary;
    }
  }

  internal void SaveViewData() { m_RequestedViewDataSave = true; }

  private void SaveViewDataToDisk() {
    if (!m_EnableViewDataPersistence || m_ViewDataDictionary == null ||
        !m_RequestedViewDataSave)
      return;

    string editorPrefFileName = this.GetType().ToString();
    EditorWindowViewData.instance.Save(editorPrefFileName,
                                       m_ViewDataDictionary);
  ...
  // TODO: These should be made public when UIElements is no longer
  // experimental.
  ...
  public void EndWindows() { GUI.EndWindows(); }
  ...
  // Indicates that the editor window will only receive a layout pass before a
  // repaint event.
  public bool wantsLessLayoutEvents {
    get { return m_EventInterests.wantsLessLayoutEvents; }
    set {
  ...
  internal static GUIContent GetLocalizedTitleContentFromType(Type t) {
    ...
    if (attr != null) {
      ...
      if (!string.IsNullOrEmpty(iconName)) {
        // This should error msg if icon is not found since icon has been
  ...
  internal void DrawNotification() {
    ...
    float targetWidth =
        position.width - EditorStyles.notificationText.margin.horizontal;
    float targetHeight =
        position.height - EditorStyles.notificationText.margin.vertical - 20;
    ...
    if (targetWidth < m_NotificationSize.x) {
      ...
      scaledNotificationText.fontSize =
          Mathf.FloorToInt(scaledNotificationText.font.fontSize * scale);

      warningSize = scaledNotificationText.CalcSize(m_Notification);
    }

    warningSize.x += 1; // we'll give the text a little room to breathe to avoid
                        // word-wrapping issues with drop shadows

    if (warningSize.y > targetHeight)
      warningSize.y = targetHeight;

    Rect r = new Rect((position.width - warningSize.x) * .5f,
                      20 + (position.height - 20 - warningSize.y) * .7f,
                      warningSize.x, warningSize.y);

    double time = EditorApplication.timeSinceStartup;
    if (time > m_FadeoutTime)
      GUI.color = new Color(
          1, 1, 1, 1 - (float)((time - m_FadeoutTime) / kWarningFadeoutTime));
  ...
  public bool hasFocus {
    get { return m_Parent && m_Parent.actualView == this; }
  }

  // Is EditorWindow docked?
  public bool docked {
    get {
      return m_Parent != null && m_Parent.window != null &&
             !m_Parent.window.IsNotDocked();
    }
  }

  // This property can be used to stop OS events from being sent to the
  // EditorWindow
  ...
  internal void MakeParentsSettingsMatchMe() {
    ...
    m_Parent.antiAliasing = m_AntiAliasing;
    m_Parent.SetInternalGameViewDimensions(
        m_GameViewRect, m_GameViewClippedRect, m_GameViewTargetSize);
    m_Parent.eventInterests = m_EventInterests;
    m_Parent.disableInputEvents = m_DisableInputEvents;
    Vector2 parentBorderSizes =
        new Vector2(m_Parent.borderSize.left + m_Parent.borderSize.right,
                    m_Parent.borderSize.top + m_Parent.borderSize.bottom);
    m_Parent.SetMinMaxSizes(minSize + parentBorderSizes,
                            maxSize + parentBorderSizes);
  ...
  public void ShowUtility() { ShowWithMode(ShowMode.Utility); }

  internal void ShowTooltip() { ShowPopupWithMode(ShowMode.Tooltip, false); }

  public void ShowPopup() { ShowPopupWithMode(ShowMode.PopupMenu, true); }
  ...
  internal void ShowPopupWithMode(ShowMode mode, bool giveFocus) {
    if (m_Parent == null) {
      ...
      // Order is important here: first set rect of container, then assign main
      // view, then apply various settings, then show. Otherwise the rect won't
      // be set until first resize happens.
      cw.position = r;
      cw.rootView = host;
      MakeParentsSettingsMatchMe();
  ...
  internal void ShowWithMode(ShowMode mode) {
    if (m_Parent == null) {
      ...
      cw.rootView = host;
      MakeParentsSettingsMatchMe();
      cw.Show(mode, loadPosition
              : true, displayImmediately
              : false, setFocus
              : true);
      // set min/max size now that native window is not null so that it will
      // e.g., use proper styleMask on macOS
  ...
  // Show window with dropdown behaviour (e.g. window is closed when it loses
  // focus) and having
  public void ShowAsDropDown(Rect buttonRect, Vector2 windowSize) {
    ShowAsDropDown(buttonRect, windowSize, null);
  }

  internal void ShowAsDropDown(Rect buttonRect, Vector2 windowSize,
                               PopupLocation[] locationPriorityOrder) {
    ShowAsDropDown(buttonRect, windowSize, locationPriorityOrder,
                   ShowMode.PopupMenu);
  }
  ...
  // 'buttonRect' is used for displaying the dropdown below that rect if
  // possible otherwise above 'windowSize' is used for setting up initial size
  // 'locationPriorityOrder' is for manual popup direction, if null it uses
  // default order: down, up, left or right 'giveFocus' is for whether the
  // window should immediately be given focus (default true)
  internal void ShowAsDropDown(Rect buttonRect, Vector2 windowSize,
                               PopupLocation[] locationPriorityOrder,
                               ShowMode mode, bool giveFocus) {
    // Setup position before bringing window live (otherwise the dropshadow on
    // Windows will be placed in 0,0 first frame)
    position = ShowAsDropDownFitToScreen(buttonRect, windowSize,
                                         locationPriorityOrder);

    // ShowWithMode() always grabs window focus so we use ShowPopup() for popup
    // windows so PopupWindowWithoutFocus will work correctly (no focus when
    // opened).
    ...
    // aux windows are added after we got/lost- focus calls.
  ...
  internal
      Rect ShowAsDropDownFitToScreen(Rect buttonRect, Vector2 windowSize,
                                     PopupLocation[] locationPriorityOrder) {
    if (m_Parent == null)
      return new Rect(buttonRect.x, buttonRect.yMax, windowSize.x,
                      windowSize.y);

    return m_Parent.window.GetDropDownRect(buttonRect, windowSize, windowSize,
                                           locationPriorityOrder);
  }
  ...
  public void ShowAuxWindow() {
    ...
    // AuxWindowManager assumes that focus has been changed before a new window
    // is added.
    Focus();
  ...
  // Show modal editor window. Other windows will not be accessible until this
  // one is closed.
  public void ShowModal() {
    ...
    // the only way we can ensure that without major API changes in window
    // creation is to open them with this type.
  ...
  // screen.
  static EditorWindow GetWindowPrivate(System.Type t, bool utility,
                                       string title, bool focus) {
    ...
    if (!win) {
      ...
      win.Show(); // For some corner cases in saved layouts, the window can be
                  // in an unvisible state.
      // Since the caller asked for focus, it's fair to assume he wants it
      // always to be visible. (case 586743)
      win.Focus();
    }
  ...
  public static EditorWindow GetWindow(System.Type t,
                                       [ DefaultValue("false") ] bool utility,
                                       [ DefaultValue("null") ] string title,
                                       [ DefaultValue("true") ] bool focus) {
  ...
  public static EditorWindow
  GetWindowWithRect(System.Type t, Rect rect,
                    [ DefaultValue("false") ] bool utility,
                    [ DefaultValue("null") ] string title) {
  ...
  public static T GetWindow<T>(bool utility, string title) where T
      : EditorWindow {
  ...
  public static T GetWindow<T>(string title, bool focus) where T
      : EditorWindow {
  ...
  // Returns the first EditorWindow of type /T/ which is currently on the
  // screen.
  public static T GetWindow<T>(bool utility, string title, bool focus) where T
      : EditorWindow {
    return GetWindow(typeof(T), utility, title, focus) as T;
  }

  public static T GetWindow<T>(params System.Type[] desiredDockNextTo) where T
      : EditorWindow {
    return GetWindow<T>(null, true, desiredDockNextTo);
  }

  public static T GetWindow<T>(string title,
                               params System.Type[] desiredDockNextTo) where T
      : EditorWindow {
  ...
  public static T
  CreateWindow<T>(params System.Type[] desiredDockNextTo) where T
      : EditorWindow {
    return CreateWindow<T>(null, desiredDockNextTo);
  }

  public static T
  CreateWindow<T>(string title, params System.Type[] desiredDockNextTo) where T
      : EditorWindow {
  ...
  // Returns the first EditorWindow of type /t/ which is currently on the
  // screen.
  static EditorWindow GetWindowWithRectPrivate(System.Type t, Rect rect,
                                               bool utility, string title) {
    UnityEngine.Object[] wins = Resources.FindObjectsOfTypeAll(t);
    EditorWindow win = wins.Length > 0 ? (EditorWindow)(wins[0]) : null;
  ...
  public static T GetWindowWithRect<T>(Rect rect, bool utility, string title,
                                       bool focus) where T : EditorWindow {
  ...
  internal static T GetWindowDontShow<T>() where T : EditorWindow {
    UnityEngine.Object[] windows = Resources.FindObjectsOfTypeAll(typeof(T));
    return (windows.Length > 0) ? (T) windows[0]
  ...
  public virtual void SaveChanges() { hasUnsavedChanges = false; }
  ...
  [System.Obsolete(
      "Use titleContent instead (it supports setting a title icon as well).")]
  public string title {
    get { return titleContent.text; }
    set { titleContent = EditorGUIUtility.TextContent(value); }
  }
  ...
  public GUIContent titleContent {
    get {
      return m_TitleContent ??(
          m_TitleContent =
              new GUIContent()); // Ensure m_TitleContent is not null (so we can
                                 // prevent null checks)
    }
    set {
      m_TitleContent = value;
      if (m_TitleContent != null && m_Parent && m_Parent.window &&
          m_Parent.window.rootView == m_Parent)
  ...
  internal void SetParentGameViewDimensions(Rect rect, Rect clippedRect,
                                            Vector2 targetSize) {
    m_GameViewRect = rect;
  ...
  internal void SetPlayModeView(bool value) { m_IsPlayModeView = value; }
  ...
  public Rect position {
    ...
    set {
      ...
      // Only handle this is we have a parent. (unless we're just getting set
      // up)
      if (m_Parent) {
        // Figure out if we're the only window here. If we are not, we need to
        // undock us. If we are not, we should just move the ContainerWindow
        // that we're inside
        DockArea da = m_Parent as DockArea;
        if (!da) {
          m_Parent.window.position = value;
        } else if (da.parent && da.m_Panes.Count == 1 &&
                   !da.parent.parent) // We should have a DockArea, then a
                                      // splitView, then null
        {
  ...
  public bool SendEvent(Event e) { return m_Parent.SendEvent(e); }
  ...
  private void __internalAwake() {
    hideFlags = HideFlags.DontSave; // Can't be HideAndDontSave, as that would
                                    // make scriptable wizard GUI be disabled
  }

  private void OnEnableINTERNAL() {}

  private void OnDisableINTERNAL() { SaveViewDataToDisk(); }

  // Internal stuff:
  // Helper to show this EditorWindow
  internal static void CreateNewWindowForEditorWindow(EditorWindow window,
                                                      bool loadPosition,
    ...
    cw.Show(ShowMode.NormalWindow, loadPosition, showImmediately, setFocus
            : true);
    // Need this, as show my change the size of the window, due to screen
    // constraints
    cw.OnResize();
  ...
  internal static void UpdateWindowMenuListing() {
    ...
    s_UpdateWindowMenuListingOff =
        EditorApplication.CallDelayed(BuildWindowMenuListing);
  }
  ...
  internal static void BuildWindowMenuListing() {
    ...
    Menu.AddMenuItem(
        $"{k_RootMenuItemName}/Close all floating panels...", "", false,
        menuIdx++, () => {
          var windows = Resources.FindObjectsOfTypeAll<ContainerWindow>();
          foreach (var win in windows.Where(
                       w => !!w && w.showMode != ShowMode.MainWindow))
            win.Close();
        }, null);
    Menu.AddSeparator($"{k_RootMenuItemName}/", menuIdx++);

    int menuIndex = 1;
    foreach (var win in editorWindows.Where(e => !!e).OrderBy(
                 e => e.titleContent.text)) {
      var title = win.titleContent.text;
      if (!String.IsNullOrEmpty(win.titleContent.tooltip) &&
          win.titleContent.tooltip != title)
        title = win.titleContent.tooltip;
      title = title.Replace("/", "\\");
      Menu.AddMenuItem($"{k_RootMenuItemName}/{menuIndex++} {title}", "", false,
                       menuIdx++, () => win.Focus(), null);
    }
