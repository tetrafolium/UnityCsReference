...
internal class SerializedPropertyFilters {
  ...
  internal abstract class SerializableFilter : IFilter {
    ...
    Active(); // returns true if filter is active, false otherwise
    public abstract bool
    Filter(SerializedProperty prop);    // returns true if filtering passes
  ...
  internal class String : SerializableFilter {
    ...
    public override bool Filter(SerializedProperty prop) {
      return prop.stringValue.IndexOf(
    ...
    public override void OnGUI(Rect r) {
      ...
      buttonRect.width = 15;
      GUIStyle buttonStyle = empty ? Styles.searchFieldCancelButtonEmpty
          : Styles.searchFieldCancelButton;
      ...
      m_Text =
          EditorGUI.TextField(r, GUIContent.none, m_Text, Styles.searchField);
