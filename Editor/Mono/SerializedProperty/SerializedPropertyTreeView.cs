...
internal class SerializedPropertyDataStore {
  internal class Data {
    ...
    public string name {
      get { return m_Object != null ? m_Object.name : string.Empty; }
    }
    ...
    public int objectId {
      get {
        ...
        return comp != null ? comp.gameObject.GetInstanceID()
    ...
    public bool Update() {
      return m_Object != null ? m_SerializedObject.UpdateIfRequiredOrScript()
          : false;
    }
    public void Store() {
      if (m_Object != null) {
  ...
  public SerializedPropertyDataStore(string[] propNames,
                                     GatherDelegate gatherDel) {
  ...
  ~SerializedPropertyDataStore() { Clear(); }

  public bool Repopulate(out bool needsReload) {
    // this is done due to optimization reasons. we want to know if any major
    // changes has happened to the data without having to refetch it in another
    // place and loop through it there
    needsReload = false;

    Profiler.BeginSample(
        "SerializedPropertyDataStore.Repopulate.GatherDelegate");
    ...
    if (m_Objects != null) {
      ...
      if (objs.Length == m_Objects.Length &&
          ArrayUtility.ArrayReferenceEquals(objs, m_Objects)) {
        // The list of objects is the same, but since some should now
        // potentially be hidden, we need to request a reload
    ...
    for (int i = 0; i < objs.Length; i++) {
      ...
      if (objs[i] == null || objs[i].hideFlags == HideFlags.HideAndDontSave ||
  ...
  private static bool ActiveObjectsEquals(Object[] objects,
                                          bool[] activeObjects) {
    ...
    for (int i = 0; i < objects.Length; i++) {
      if (objects[i] != null && objects[i] is Component) {
        if (((Component) objects[i]).gameObject.activeInHierarchy !=
            activeObjects[i])
          return false;
      }
    }
...
internal class SerializedPropertyTreeView : TreeView {
  internal static class Styles {
    ...
    public static readonly string focusHelper =
        "SerializedPropertyTreeViewFocusHelper";
    ...
    public static readonly string serializeColumnHeaderState =
        "_ColumnHeaderState";
    public static readonly string serializeFilter = "_Filter_";
    public static readonly GUIContent filterSelection =
        EditorGUIUtility.TrTextContent(
            "Isolate Selection",
            "Limits the table contents to the active selection.");
    public static readonly GUIContent showInactiveObjects =
        EditorGUIUtility.TrTextContent(
            "Show Inactive Objects",
            "Show objects that are not active in the hierarchy but contains the component.");
  }

  // this gets stuffed into the view and displayed on screen. It is a visible
  // subset of the actual data
  internal class SerializedPropertyItem : TreeViewItem {
    SerializedPropertyDataStore.Data m_Data;

    public SerializedPropertyItem(int id, int depth,
                                  SerializedPropertyDataStore.Data ltd)
        : base(id, depth, ltd != null ? ltd.name
               : "root") {
      m_Data = ltd;
    }
  ...
  internal class Column : MultiColumnHeaderState.Column {
    public delegate void DrawEntry(Rect r, SerializedProperty prop,
                                   SerializedProperty[] dependencies);
    public delegate int CompareEntry(SerializedProperty lhs,
                                     SerializedProperty rhs);
    public delegate void CopyDelegate(SerializedProperty target,
    ...
    public static void
    OverwriteSerializedFields(MultiColumnHeaderState source,
      ...
      for (int i = 0; i < destination.columns.Length; ++i) {
        destination.columns[i].sortedAscending =
            source.columns[i].sortedAscending;
        ...
        if ((sourceColumn == null || destColumn == null) ||
            string.Equals(sourceColumn.propertyName, destColumn.propertyName)) {
  ...
  private struct ColumnInternal {
    public SerializedProperty[] dependencyProps; // helper array, must be the
                                                 // same size as
                                                 // dependencyIndices
  }

  internal class DefaultDelegates {
    public static readonly Column.DrawEntry DrawDefault =
        (Rect r, SerializedProperty prop,
         SerializedProperty[] dependencies) => {
          Profiler.BeginSample("PropDrawDefault");
          EditorGUI.PropertyField(r, prop, GUIContent.none);
          Profiler.EndSample();
        };
    public static readonly Column.DrawEntry DrawCheckbox =
        (Rect r, SerializedProperty prop,
         SerializedProperty[] dependencies) => {
    ...
    public static readonly Column.DrawEntry DrawName =
        (Rect r, SerializedProperty prop,
         SerializedProperty[] dependencies) => {};

    public static readonly Column.CompareEntry CompareFloat =
        (SerializedProperty lhs, SerializedProperty rhs) => {
          return lhs.floatValue.CompareTo(rhs.floatValue);
        };
    public static readonly Column.CompareEntry CompareCheckbox =
        (SerializedProperty lhs, SerializedProperty rhs) => {
          return lhs.boolValue.CompareTo(rhs.boolValue);
        };
    public static readonly Column.CompareEntry CompareEnum =
        (SerializedProperty lhs, SerializedProperty rhs) => {
          return lhs.enumValueIndex.CompareTo(rhs.enumValueIndex);
        };
    public static readonly Column.CompareEntry CompareInt =
        (SerializedProperty lhs, SerializedProperty rhs) => {
          return lhs.intValue.CompareTo(rhs.intValue);
        };
    public static readonly Column.CompareEntry CompareColor =
        (SerializedProperty lhs, SerializedProperty rhs) => {
    ...
    public static readonly Column.CompareEntry CompareName =
        (SerializedProperty lhs, SerializedProperty rhs) => { return 0; };
    public static readonly Column.CopyDelegate CopyDefault =
        (SerializedProperty target, SerializedProperty source) => {
  ...
  public SerializedPropertyTreeView(TreeViewState state,
    ...
    for (int i = 0; i < colcnt; i++) {
      ...
      if (c.propertyName != null)
        m_ColumnsInternal[i].dependencyProps =
            new SerializedProperty[c.propertyName.Length];
    }
  ...
  public void SerializeState(string uid) {
    SessionState.SetBool(uid + Styles.serializeFilterSelection,
                         m_bFilterSelection);
    SessionState.SetBool(uid + Styles.showInactiveObjects,
    ...
    EditorPrefs.SetString(uid + Styles.serializeColumnHeaderState,
  ...
  public void DeserializeState(string uid) {
    ...
    m_ShowInactiveObjects =
        SessionState.GetBool(uid + Styles.showInactiveObjects, false);

    MultiColumnHeaderState headerState =
        new MultiColumnHeaderState(multiColumnHeader.state.columns);
    string columnHeaderState =
        EditorPrefs.GetString(uid + Styles.serializeColumnHeaderState, "");
    ...
    if (MultiColumnHeaderState.CanOverwriteSerializedFields(
            headerState, multiColumnHeader.state))
    ...
    for (int i = 0; i < multiColumnHeader.state.columns.Length; i++) {
      ...
      string filterState =
          SessionState.GetString(uid + Styles.serializeFilter + i, null);
      if (string.IsNullOrEmpty(filterState))
        continue;
    ...
    string treeViewState =
        SessionState.GetString(uid + Styles.serializeTreeViewState, "");
  ...
  public void Update() {
    ...
    if (m_DataStore != null) {
      ...
      // The data can also be fine, but needs a reload without having to rebuild
      // it all (inactive/active objects changed)
    ...
    if (last != -1) {
      for (int i = first; i <= last; i++) {
        changed =
            changed || ((SerializedPropertyItem) rows[i]).GetData().Update();
      }
    }
  ...
  protected override IList<TreeViewItem> BuildRows(TreeViewItem root) {
    if (m_Items == null) {
      ...
      for (int i = 0; i < rawData.Length; i++) {
        var item =
            new SerializedPropertyItem(rawData[i].objectId, 0, rawData[i]);
        m_Items.Add(item);
      }
    }
    ...
    if (!m_ShowInactiveObjects)
      tmprows = m_Items.Where((TreeViewItem item) => {
        return ((SerializedPropertyItem) item).GetData().activeInHierarchy;
      });
    ...
    if (m_bFilterSelection) {
      ...
      tmprows = m_Items.Where((TreeViewItem item) => {
        return m_SelectionFilter.Contains(item.id);
      });
    } else
    ...
    // We still need to setup the child parent information for the rows since
    // this information is used by the TreeView internal logic (navigation,
    // dragging etc)
  ...
  void CellGUI(Rect cellRect, SerializedPropertyItem item, int columnIndex,
               ref RowGUIArgs args) {
    ...
    if (column.drawDelegate == DefaultDelegates.DrawName) {
      ...
      int depcnt =
          column.dependencyIndices != null ? column.dependencyIndices.Length : 0;

      for (int i = 0; i < depcnt; i++)
        m_ColumnsInternal[columnIndex].dependencyProps[i] =
            props[column.dependencyIndices[i]];

      // allow to capture tabs
      if (args.item.id == state.lastClickedID && HasFocus() &&
          columnIndex ==
              multiColumnHeader.state.visibleColumns
                  [multiColumnHeader.state.visibleColumns[0] == 0 ? 1 : 0])
      ...
      column.drawDelegate(cellRect, prop,
                          m_ColumnsInternal[columnIndex].dependencyProps);

      Profiler.EndSample();
      if (EditorGUI.EndChangeCheck()) {
        // if we changed a value in a filtered column we'll have to reload the
        // table
        m_ChangedId = ((column.filter != null) && column.filter.Active())
                          ? GUIUtility.keyboardControl
                          : m_ChangedId;
        // update all selected items if the current row was part of the
        // selection list
        ltd.Store();
        ...
        if (selIds.Contains(ltd.objectId)) {
          ...
          Undo.RecordObjects(rows.Select(r =>((SerializedPropertyItem) r)
                                                 .GetData()
  ...
  public void OnFilterGUI(Rect r) {
    ...
    r.x += toggleWidth;
    r.width = GUI.skin.label
                  .CalcSize(SerializedPropertyTreeView.Styles.filterSelection)
                  .x;
    ...
    r.width =
        GUI.skin.label
            .CalcSize(SerializedPropertyTreeView.Styles.showInactiveObjects)
            .x;
    EditorGUI.LabelField(r,
                         SerializedPropertyTreeView.Styles.showInactiveObjects);
    ...
    for (int i = 0; i < multiColumnHeader.state.columns.Length; i++) {
      ...
      if (c.filter != null &&
          c.filter.GetType().Equals(typeof(SerializedPropertyFilters.Name))) {
        c.filter.OnGUI(r);
      }
    }
  ...
  protected override void KeyEvent() {
    ...
    // We always focus as this callback only fires when the name column is
    // selected. Tabbing through the property columns never goes through here.
    // Also note that we're not capturing the KeyCode Tab. There are 2 tab
    // events, one for the keycode, and one for the character code. The keycode
    // comes first, so if we change the focus based on the keycode, we never get
    // the second character code event as the treeview loses focus and doesn't
    // fire the callback. The second tab event then gets handled as an ordinary
    // tab, leading to a double tab.
  ...
  void OnVisibleColumnChanged(MultiColumnHeader header) { Reload(); }
  ...
  void Sort(IList<TreeViewItem> rows, int sortIdx) {
    ...
    if (comp == null) {
      return;
               ...
               DefaultDelegates
                   .CompareName) // special case for sorting by the object name
    {
    ...
    for (int i = 0; i < cnt; i++) {
      ...
      if (c.filter != null) {
        ...
        if (c.filter.GetType().Equals(typeof(SerializedPropertyFilters.Name))) {
          ...
          tmp = tmp.Where((TreeViewItem item) => {
            return f.Filter(((SerializedPropertyItem) item).GetData().name);
          });
        } else {
          tmp = tmp.Where((TreeViewItem item) => {
            return c.filter.Filter(
                ((SerializedPropertyItem) item).GetData().properties[idx]);
          });
        }
      }
    }
