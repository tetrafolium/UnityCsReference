...
void OnGUI() {
	...
	m_DefaultScene =
		EditorGUILayout.TextField("Default scene:", m_DefaultScene);
	m_Version = EditorGUILayout.TextField("Version:", m_Version);

	if (GUILayout.Button("Save", GUILayout.Width(100))) {
		if (String.IsNullOrEmpty(m_Path)) {
			m_Path =
				EditorUtility.SaveFolderPanel("Save template to folder", "", "");
		}
		AssetDatabase.SaveAssets();
		EditorUtility.SaveProjectAsTemplate(m_Path, m_Name, m_DisplayName,
		                                    m_Description, m_DefaultScene,
		                                    m_Version);
	}
}
}
