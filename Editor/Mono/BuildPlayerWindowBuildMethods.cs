...
public partial class BuildPlayerWindow : EditorWindow {
  private static Func<BuildPlayerOptions, BuildPlayerOptions>
      getBuildPlayerOptionsHandler;
  ...
  /// <summary>
  /// Exception thrown when an abort or error condition is reached within a
  /// build method delegate.
  /// </summary>
  public class BuildMethodException : System.Exception {
    ...
    /// <summary>
    /// Constructor for aborting on error that will print the log the given
    /// message as an error.
    /// </summary>
  ...
  /// <summary>
  /// Register a delegate method to calculate BuildPlayerOptions that are passed
  /// to the build process.
  /// </summary>
  /// <param name="func">Delegate method</param>
  public static void RegisterGetBuildPlayerOptionsHandler(
      Func<BuildPlayerOptions, BuildPlayerOptions> func) {
    // Display a warning if user scripts try to register this delegate multiple
    // times
    if (func != null && getBuildPlayerOptionsHandler != null)
      Debug.LogWarning(
          "The get build player options handler in BuildPlayerWindow is being reassigned!");
  ...
  /// <param name="func">Delegate method</param>
  public static void
  RegisterBuildPlayerHandler(Action<BuildPlayerOptions> func) {
    // Display a warning if user scripts try to register this delegate multiple
    // times
    if (func != null && buildPlayerHandler != null)
      Debug.LogWarning(
          "The build player handler in BuildPlayerWindow is being reassigned!");

    buildPlayerHandler = func;
  }

  /// <summary>
  /// Method called by the UI when the "Build" or "Build and Run" buttons are
  /// pressed.
  /// </summary>
  /// <param name="defaultBuildOptions"></param>
  internal static void CallBuildMethods(bool askForBuildLocation,
    ...
    try {
      ...
      if (getBuildPlayerOptionsHandler != null)
        ...
        options = DefaultBuildMethods.GetBuildPlayerOptionsInternal(
            askForBuildLocation, options);
  ...
  public static class DefaultBuildMethods {
    ...
    public static void BuildPlayer(BuildPlayerOptions options) {
      ...
      if (!BuildPipeline.IsBuildTargetSupported(options.targetGroup,
                                                options.target))
        throw new BuildMethodException("Build target is not supported.");

      string module = ModuleManager.GetTargetStringFrom(
          EditorUserBuildSettings.selectedBuildTargetGroup, options.target);
      IBuildWindowExtension buildWindowExtension =
          ModuleManager.GetBuildWindowExtension(module);
      if (buildWindowExtension != null &&
          (options.options & BuildOptions.AutoRunPlayer) != 0 &&
          !buildWindowExtension.EnabledBuildAndRunButton())
        throw new BuildMethodException();
      ...
      if (Unsupported.IsBleedingEdgeBuild()) {
        ...
        sb.AppendLine(
            "We strongly recommend that you use a normal version of Unity instead.");

        if (EditorUtility.DisplayDialog("BleedingEdge Build", sb.ToString(),
                                        "Cancel", "OK"))
          throw new BuildMethodException();
      }

      // See if we need to switch platforms and delay the build.  We do this
      // whenever we're trying to build for a target different from the active
      // one so as to ensure that the compiled script code we have loaded is
      // built for the same platform we are building for.  As we can't reload
      // while our editor stuff is still executing, we need to defer to after
      // the next script reload then.
      ...
      if (EditorUserBuildSettings.activeBuildTarget != options.target ||
        ...
        if (!EditorUserBuildSettings.SwitchActiveBuildTargetAsync(
                options.targetGroup, options.target)) {
      ...
      bool locationPathExistedBeforeBuild =
          System.IO.Directory.Exists(options.locationPathName);
      // Trigger build.
      // Note: report will be null, if delayToAfterScriptReload = true
      var report = BuildPipeline.BuildPlayerInternalNoCheck(
          options.scenes, options.locationPathName, null, options.targetGroup,
          options.target, options.options, options.extraScriptingDefines,
          delayToAfterScriptReload);

      if (report != null) {
        var resultStr = String.Format(
            "Build completed with a result of '{0}' in {1} seconds ({2} ms)",
        ...
        case Build.Reporting.BuildResult.Failed:
          //  On some platforms the user creates the build folder, therefore
          //  they own the folder and
          // it should not be automatically deleted by the Unity Editor, even if
          // it is empty (case 1073851)
          if (options.target != BuildTarget.XboxOne &&
              !locationPathExistedBeforeBuild)
    ...
    /// <summary>
    /// Default implementation for calculating build options before building the
    /// player.
    /// </summary>
    /// <param name="defaultBuildPlayerOptions"></param>
    /// <returns></returns>
    public static BuildPlayerOptions
    ...
    internal static bool IsInstallInBuildFolderOption() {
      ...
      return EditorUserBuildSettings.installInBuildFolder &&
             PostprocessBuildPlayer.SupportsInstallInBuildFolder(
                 buildTargetGroup, buildTarget) &&
             (Unsupported.IsSourceBuild() || IsMetroPlayer(buildTarget));
    }

    internal static BuildPlayerOptions GetBuildPlayerOptionsInternal(
      ...
      // it if need be
      if (PostprocessBuildPlayer.SupportsLz4Compression(buildTargetGroup,
                                                        buildTarget)) {
        var compression =
            EditorUserBuildSettings.GetCompressionType(buildTargetGroup);
        if (compression < 0)
          compression = PostprocessBuildPlayer.GetDefaultCompression(
              buildTargetGroup, buildTarget);
      ...
      if (EditorUserBuildSettings.connectProfiler &&
          (developmentBuild || buildTarget == BuildTarget.WSAPlayer))
      ...
      if (IsInstallInBuildFolderOption()) {
        ...
        if (askForBuildLocation &&
            !PickBuildLocation(buildTargetGroup, buildTarget, options.options,
        ...
        if (newLocation.Length == 0) {
          throw new BuildMethodException("Build location for buildTarget " +
                                         buildTarget + "is not valid.");
        }

        if (!askForBuildLocation) {
          switch (UnityEditorInternal.InternalEditorUtility.BuildCanBeAppended(
              buildTarget, newLocation)) {
          ...
          case CanAppendBuild.No:
            if (!PickBuildLocation(buildTargetGroup, buildTarget,
                                   options.options, out updateExistingBuild))
              throw new BuildMethodException();
    ...
    private static bool PickBuildLocation(BuildTargetGroup targetGroup,
                                          BuildTarget target,
      ...
      if (previousPath == String.Empty) {
        defaultFolder =
            FileUtil.DeleteLastPathNameComponent(Application.dataPath);
        defaultName = "";
      } else {
      ...
      string extension = PostprocessBuildPlayer.GetExtensionForBuildTarget(
          targetGroup, target, options);
      // Invalidate default name, if extension mismatches the default file (for
      // ex., when switching between folder type export to file type export, see
      // Android)
      if (extension != Path.GetExtension(defaultName).Replace(".", ""))
        defaultName = string.Empty;

      // Hack: For Windows Standalone, we want the BuildPanel to choose a
      // folder, but we don't want BuildPlayer to take a folder path because
      // historically it took an .exe path and we would be breaking tons of
      // projects!
      ...
      string title =
          "Build " + BuildPlatforms.instance.GetBuildTargetDisplayName(
                         targetGroup, target);
      string path = EditorUtility.SaveBuildPanel(target, title, defaultFolder,
      ...
      if (isWindowsStandalone) {
        ...
        path = Path.Combine(
            path, Paths.MakeValidFileName(PlayerSettings.productName) + '.' +
                      extension);
      }
      ...
      if (extension != string.Empty &&
          FileUtil.GetPathExtension(path).ToLower() != extension)
        path += '.' + extension;

      // A path may not be empty initially, but it could contain, e.g., a drive
      // letter (as in Windows), so even appending an extension will work fine,
      // but in reality the name will be, for example, G:/
      // Debug.Log(path);
      ...
      string check_dir =
          extension != string.Empty ? FileUtil.DeleteLastPathNameComponent(path)
          : path;
      if (!Directory.Exists(check_dir))
        Directory.CreateDirectory(check_dir);

      // On OSX we've got replace/update dialog, for other platforms warn about
      // deleting files in target folder.
    ...
    private static string NormalizePath(string path) {
      ...
      fullPath = string.IsNullOrEmpty(fullPath) ? string.Empty
    ...
    internal static bool IsBuildPathValid(string path) {
      ...
      if (cleanedPath.Equals(string.Empty) && IsInstallInBuildFolderOption())
        return true;
      ...
      if (basePath.Contains(cleanedPath) || cleanedPath == assetsPath ||
          cleanedPath == settingsPath || cleanedPath == tempPath ||
          cleanedPath == libraryPath || cleanedPath == userSettingsPath) {
    ...
    private static bool UserWantsToDeleteFiles(string path) {
      string text =
          "WARNING: all files and folders located in target folder: '" + path +
          "' will be deleted by build process.";
      return EditorUtility.DisplayDialog("Deleting existing files", text, "OK",
                                         "Cancel");
    }
