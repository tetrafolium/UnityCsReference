...
internal partial class ObjectListArea {
  class AssetStoreGroup : Group {
    ...
    const int kMaxQueryItems =
        1000; // Max items allowed to query for by backend
    ...
    public override int ItemCount {
      get { return Math.Min(m_Assets.Count, ItemsWantedShown); }
    }
    ...
    public AssetStoreGroup(ObjectListArea owner, string groupTitle,
                           string groupName)
        : base(owner, groupTitle) {
      m_Assets = new List<AssetStoreAsset>();
      m_Name = groupName;
      m_ListMode = false;
      m_ShowMoreDims = EditorStyles.miniButton.CalcSize(
          EditorGUIUtility.TrTextContent("Show more"));
    ...
    protected override void DrawInternal(int itemIdx, int endItem,
                                         float yOffset) {
      ...
      if (ListMode) {
        // Draw row
        // position.width = Mathf.Max (position.width, 500); // For some reason
        // the icon is clipped if the rect cannot contain both the text and icon
        // so make sure its large
        for (; itemIdx < endItem && itemIdx < endContainerItem; itemIdx++) {
          ...
          if (isRepaintEvent) {
            bool selected =
                !AssetStoreAssetSelection.Empty &&
                AssetStoreAssetSelection.ContainsAsset(m_Assets[itemIdx].id);
        ...
        for (; itemIdx < endItem && itemIdx < endContainerItem; itemIdx++) {
          ...
          if (isRepaintEvent) {
            Rect position =
                new Rect(r.x, r.y, r.width,
                         r.height - s_Styles.resultsGridLabel.fixedHeight);
      ...
      r = new Rect(m_Owner.GetVisibleWidth() - m_ShowMoreDims.x - 6,
                   ...
                   m_ShowMoreDims.x, m_ShowMoreDims.y);

      if (ItemsAvailable > (m_Grid.rows * m_Grid.columns) &&
          ItemsAvailable >= Assets.Count && Assets.Count < kMaxQueryItems) {
        ...
        case EventType.MouseDown:
          if (evt.button == 0 && r.Contains(evt.mousePosition)) {
            if (ListMode) {
              ...
              // after the update. Partly filled rows can occur when resizing
              // the listarea or when we reached the limit of results from
              // server.
              int itemsNeededToFillLastRow =
                  m_Grid.columns - (ItemCount % m_Grid.columns);
              itemsNeededToFillLastRow =
                  itemsNeededToFillLastRow % m_Grid.columns;
              ItemsWantedShown +=
                  (kMoreRowsAdded * m_Grid.columns) + itemsNeededToFillLastRow;
            }
            if (NeedItems) // Check that we need to fetch it since we might
                           // already have it prefetched
    ...
    AssetStorePreviewManager.CachedAssetStoreImage
    GetIconForAssetStoreAsset(AssetStoreAsset assetStoreResource) {
      if (!string.IsNullOrEmpty(assetStoreResource.staticPreviewURL)) {
        ...
        AssetStorePreviewManager.CachedAssetStoreImage item =
            AssetStorePreviewManager.TextureFromUrl(
                assetStoreResource.staticPreviewURL, assetStoreResource.name,
                m_Owner.gridSize, s_Styles.resultsGridLabel, s_Styles.previewBg,
                false);
        return item;
      }
    ...
    void DrawIcon(Rect position, AssetStoreAsset assetStoreResource) {
      ...
      AssetStorePreviewManager.CachedAssetStoreImage icon =
          GetIconForAssetStoreAsset(assetStoreResource);

      if (icon == null) {
        // Draw static preview bg color as bg for icons that do not fill the
        // entire rect (mini icons and non-square textures)
        // s_Styles.previewBg.Draw (position, GUIContent.none, false, false,
        // false, false);

        // Use builtin type icon
        Texture2D iconImage =
            InternalEditorUtility.GetIconForFile(assetStoreResource.name);
        s_Styles.resultsGrid.Draw(position, iconImage, false, false, selected,
                                  selected);
      } else {
        // Fade in
        ...
        if (c.a != 1.0f)
          GUI.color = c;
        s_Styles.resultsGrid.Draw(position, m_Content, false, false, selected,
                                  selected);
        if (c.a != 1.0f) {
          GUI.color = oldColor;
    ...
    void DrawLabel(Rect position, AssetStoreAsset assetStoreResource,
                   bool selected) {
      if (ListMode) {
        position.width = Mathf.Max(
            position.width,
            500); // For some reason the icon is clipped if the rect cannot
                  // contain both the text and icon so make sure its large
        m_Content.text = assetStoreResource.displayName;

        // For now disable drawing preview icons in list mode since it looks
        // wrong because of the dark background on preview images.
        m_Content.image =
            InternalEditorUtility.GetIconForFile(assetStoreResource.name);

        s_Styles.resultsLabel.Draw(position, m_Content, false, false, selected,
                                   selected);
      } else {
        // We crop the text in Preview mode
        int pseudoInstanceID =
            assetStoreResource.id +
            10000000; // we add a large offset to the asset store id to ensure
                      // there is no overlap with instanceIDs
        var assetReference = new InternalEditorUtility.AssetReference(){
            instanceID = pseudoInstanceID};
        string labeltext = m_Owner.GetCroppedLabelText(
            assetReference, assetStoreResource.displayName, position.width);
        position.height -= s_Styles.resultsGridLabel.fixedHeight;
        // The -1 is to ensure the label has same width as the image and that it
        // aligns with the bottom of the image
        s_Styles.resultsGridLabel.Draw(
            new Rect(position.x, position.yMax + 1, position.width - 1,
                     s_Styles.resultsGridLabel.fixedHeight),
            labeltext, false, false, selected, m_Owner.HasFocus());
      }
    }
    ...
    public override void UpdateFilter(HierarchyType hierarchyType,
      ...
      ItemsWantedShown = ListMode ? kDefaultRowsShownListMode
          : (kDefaultRowsShown * m_Grid.columns);
      Assets.Clear();
    }
    ...
    protected int HandleMouse(Rect position) {
      ...
      case EventType.ContextClick:
        if (position.Contains(evt.mousePosition)) {
          // evt.Use(); // We do not use the event this should be handled by
          // client
          return 1;
        }
        break;
