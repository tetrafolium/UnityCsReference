...
[Flags] enum UnityTypeFlags {
  Abstract = 1 << 0,
  Sealed = 1 << 1,
  EditorOnly = 1 << 2
}

[System.Diagnostics.DebuggerDisplay("{module}:{name}")] sealed
    partial class UnityType {
  public string name {
    get;
    private set;
  }
  ...
  public bool isAbstract {
    get { return (flags & UnityTypeFlags.Abstract) != 0; }
  }
  public bool isSealed {
    get { return (flags & UnityTypeFlags.Sealed) != 0; }
  }
  public bool isEditorOnly {
    get { return (flags & UnityTypeFlags.EditorOnly) != 0; }
  }
  ...
  // NOTE : nativeNamespace == "" for types with no namespace so added this
  // helper for convenience in case the caller wasn't sure whether to compare
  // nativeNamespace with null or empty
  public bool hasNativeNamespace {
    get { return nativeNamespace.Length > 0; }
  }

  public bool IsDerivedFrom(UnityType baseClass) {
    // NOTE : Type indices are ordered so all derived classes are immediately
    // following the base class allowing us to test inheritance with only a
    // range check
    return (runtimeTypeIndex - baseClass.runtimeTypeIndex) <
           baseClass.descendantCount;
  }
  ...
  public static uint TypeCount {
    get { return (uint) ms_types.Length; }
  }
  ...
  public static UnityType FindTypeByNameCaseInsensitive(string name) {
    return ms_types.FirstOrDefault(
        t => string.Equals(name, t.name, StringComparison.OrdinalIgnoreCase));
  }
  ...
  static UnityType() {
    ...
    for (int i = 0; i < types.Length; ++i) {
      // Types are sorted so base < derived and null baseclass is passed from
      // native as 0xffffffff
      UnityType baseClass = null;
      if (types[i].baseClassIndex < types.Length)
        baseClass = ms_types[types[i].baseClassIndex];

      var newType = new UnityType{runtimeTypeIndex = types[i].runtimeTypeIndex,
