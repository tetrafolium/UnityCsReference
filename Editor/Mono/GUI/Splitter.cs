...
#pragma warning disable CS0649
  [SerializeField]
  [FormerlySerializedAs("realSizes")] int[] oldRealSizes;

  [SerializeField]
  [FormerlySerializedAs("minSizes")] int[] oldMinSizes;

  [SerializeField]
  [FormerlySerializedAs("maxSizes")] int[] oldMaxSizes;

  [SerializeField]
  [FormerlySerializedAs("splitSize")] int oldSplitSize;
#pragma warning restore CS0649

#region Old Constructors

  // In the past, pixelsPerPoint was always 1, so points could be stored as
  // ints. With the introduction of high-dpi and arbitrary scaling, the
  // implementation has been changed to store floats. For backwards
  // compatibility, the old constructors remain available but are NOT
  // recommended anymore, since they imply a conversion from int[] to float[].
  // Consider using the static functions FromAbsolute and FromRelative instead.
  ...
  public SplitterState(int[] realSizes, int[] minSizes, int[] maxSizes) {
    InitFromAbsolute(System.Array.ConvertAll(realSizes, s_ConverterDelegate),
                     minSizes == null ? null
                     : System.Array.ConvertAll(minSizes, s_ConverterDelegate),
                       maxSizes == null ? null
                     : System.Array.ConvertAll(maxSizes, s_ConverterDelegate));
  }

  public SplitterState(float[] relativeSizes, int[] minSizes, int[] maxSizes) {
    InitFromRelative(relativeSizes, minSizes == null ? null
                     : System.Array.ConvertAll(minSizes, s_ConverterDelegate),
                       maxSizes == null ? null
                     : System.Array.ConvertAll(maxSizes, s_ConverterDelegate),
                       0);
  ...
  SplitterState(bool useless) {} // A parameterless constructor would conflict
                                 // with the obsolete params constructor.

  public static SplitterState FromAbsolute(float[] realSizes, float[] minSizes,
                                           float[] maxSizes) {
  ...
  public static SplitterState FromRelative(float[] relativeSizes,
                                           float[] minSizes, float[] maxSizes) {
    var state = new SplitterState(false);
  ...
  public bool IsValid() {
    return realSizes != null && minSizes != null && maxSizes != null &&
           relativeSizes != null && realSizes.Length > 0 &&
           minSizes.Length > 0 && maxSizes.Length > 0 &&
           relativeSizes.Length > 0 && realSizes.Length == minSizes.Length &&
           minSizes.Length == maxSizes.Length &&
           maxSizes.Length == relativeSizes.Length;
  }
  ...
  void InitFromRelative(float[] relativeSizes, float[] minSizes,
                        float[] maxSizes, int splitSize) {
    this.relativeSizes = relativeSizes;
    this.minSizes =
        minSizes == null ? new float[relativeSizes.Length] : minSizes;
    this.maxSizes =
        maxSizes == null ? new float[relativeSizes.Length] : maxSizes;
    ...
    if (spaceToShare < 0) {
      for (k = 0; k < relativeSizes.Length; k++) {
        if (realSizes[k] > minSizes[k]) {
          ...
          float spaceToTake =
              -spaceToShare < spaceInThisOne ? - spaceToShare : spaceInThisOne;
  ...
  public void DoSplitter(int i1, int i2, float diff) {
    // TODO: This does not handle all cases properly. Theres a hope we will not
    // encounter those cases in the editor. Needs to be fixed once its passed to
    // users.
  ...
  public void OnBeforeSerialize() { m_Version = 1; }

  static void ConvertOldArray(int[] oldArray, ref float[] newArray) {
    if ((newArray == null || newArray.Length == 0) && oldArray != null &&
        oldArray.Length > 0)
...
class SplitterGUILayout {
    ...
    public override void SetHorizontal(float x, float width) {
      if (!isVertical) {
        ...
        foreach (GUILayoutEntry i in entries) {
          ...
          i.SetHorizontal(GUIUtility.RoundToPixelGrid(x),
                          GUIUtility.RoundToPixelGrid(thisSize));
          x += thisSize + spacing;
          k++;
        }
      } else {
      ...
      if (isVertical) {
        ...
        if (style != GUIStyle.none) {
          ...
          if (entries.Count != 0) {
            ...
            bottomMar = Mathf.Max(
                bottomMar,
                ((GUILayoutEntry) entries[entries.Count - 1]).marginBottom);
          }
          y += topMar;
        ...
        foreach (GUILayoutEntry i in entries) {
          ...
          i.SetVertical(GUIUtility.RoundToPixelGrid(y),
                        GUIUtility.RoundToPixelGrid(thisSize));
          y += thisSize + spacing;
          k++;
        }
      } else {
        // If we have a GUIStyle here, we need to respect the subelements'
        // margins
        if (style != GUIStyle.none) {
          foreach (GUILayoutEntry i in entries) {
          ...
          // If not, the subelements' margins have already been propagated
          // upwards to this group, so we can safely ignore them
  ...
  public static void BeginSplit(SplitterState state, GUIStyle style,
                                bool vertical,
                                params GUILayoutOption[] options) {
    float pos;
    var g = (GUISplitterGroup) GUILayoutUtility.BeginLayoutGroup(
        style, null, typeof(GUISplitterGroup));
    ...
    case EventType.MouseDown: {
      if ((Event.current.button == 0) && (Event.current.clickCount == 1)) {
        float cursor = GUIUtility.RoundToPixelGrid(g.isVertical ? g.rect.y
                                                   : g.rect.x);
        pos = GUIUtility.RoundToPixelGrid(
            g.isVertical ? Event.current.mousePosition.y
            : Event.current.mousePosition.x);

        for (int i = 0; i < state.relativeSizes.Length - 1; i++) {
          Rect splitterRect = g.isVertical ? new Rect(
              state.xOffset + g.rect.x,
              cursor + state.realSizes[i] - state.splitSize / 2, g.rect.width,
              state.splitSize)
              : new Rect(state.xOffset + cursor + state.realSizes[i] -
                             state.splitSize / 2,
                         g.rect.y, state.splitSize, g.rect.height);
    ...
    case EventType.MouseDrag: {
      if ((GUIUtility.hotControl == state.ID) &&
          (state.currentActiveSplitter >= 0)) {
        pos = g.isVertical ? Event.current.mousePosition.y
            : Event.current.mousePosition.x;
        ...
        if (diff != 0) {
          state.splitterInitialOffset = pos;
          state.DoSplitter(state.currentActiveSplitter,
                           state.currentActiveSplitter + 1, diff);
        }
    ...
    case EventType.Repaint: {
      ...
      for (var i = 0; i < state.relativeSizes.Length - 1; i++) {
        var splitterRect = g.isVertical ? new Rect(state.xOffset + g.rect.x,
                                                  cursor + state.realSizes[i] -
                                                      state.splitSize / 2,
                                                  g.rect.width, state.splitSize)
            : new Rect(state.xOffset + cursor + state.realSizes[i] -
                           state.splitSize / 2,
                       g.rect.y, state.splitSize, g.rect.height);

        EditorGUIUtility.AddCursorRect(
            splitterRect, g.isVertical ? MouseCursor.ResizeVertical
            : MouseCursor.SplitResizeLeftRight, state.ID);
  ...
  public static void BeginHorizontalSplit(SplitterState state,
                                          params GUILayoutOption[] options) {
  ...
  public static void BeginHorizontalSplit(SplitterState state, GUIStyle style,
                                          params GUILayoutOption[] options) {
    BeginSplit(state, style, false, options);
  }

  public static void BeginVerticalSplit(SplitterState state, GUIStyle style,
                                        params GUILayoutOption[] options) {
    BeginSplit(state, style, true, options);
  }

  public static void EndVerticalSplit() { GUILayoutUtility.EndLayoutGroup(); }

  public static void EndHorizontalSplit() { GUILayoutUtility.EndLayoutGroup(); }
}
}
