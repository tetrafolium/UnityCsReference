...
internal class ColumnView {
  ...
  public void OnGUI(List<ColumnViewElement> elements,
                    ObjectColumnFunction previewColumnFunction) {
    OnGUI(elements, previewColumnFunction, null, null, null);
  }

  public void OnGUI(List<ColumnViewElement> elements,
                    ObjectColumnFunction previewColumnFunction,
                    ObjectColumnFunction selectedSearchItemFunction,
                    ObjectColumnFunction selectedRegularItemFunction,
                    ObjectColumnGetDataFunction getDataForDraggingFunction) {
    InitStyles();
    ...
    do {
      ...
      selectedIndex =
          DoListColumn(listView, columnElements, listViewIndex, selectedIndex,
                       listViewIndex == 0 ? selectedSearchItemFunction : null,
                       selectedRegularItemFunction, getDataForDraggingFunction);

      if (Event.current.type == EventType.Layout &&
          m_ColumnToFocusKeyboard == listViewIndex) {
      ...
      if (listViewIndex == 0) {
        // pass some of the keys to the list view, even if something else is
        // active
        if (isSearching) {
      ...
      selectedObject =
          selectedIndex > -1 ? columnElements[selectedIndex].value : null;
  ...
  private static void
  DoItemSelectedEvent(ObjectColumnFunction selectedRegularItemFunction,
                      object value) {
  ...
  private void
  DoSearchItemSelectedEvent(ObjectColumnFunction selectedSearchItemFunction,
                            object value) {
  ...
  private void DoDummyColumn() {
    // TODO: Don't know why this width needs one more pixel to match the list
    // views. Bug in ScrollViews?
    GUILayout.Box(GUIContent.none, s_Styles.background,
                  GUILayout.Width(columnWidth + 1));
  }

  private static void
  DoPreviewColumn(object selectedObject,
                  ObjectColumnFunction previewColumnFunction) {
  ...
  private int
  DoListColumn(ListViewState listView,
               List<ColumnViewElement> columnViewElements, int columnIndex,
               int selectedIndex,
               ObjectColumnFunction selectedSearchItemFunction,
               ObjectColumnFunction selectedRegularItemFunction,
               ObjectColumnGetDataFunction getDataForDraggingFunction) {
    if (Event.current.type == EventType.KeyDown &&
      ...
      if (isSearching && selectedSearchItemFunction != null)
        DoSearchItemSelectedEvent(selectedSearchItemFunction,
                                  columnViewElements[selectedIndex].value);
      if (!isSearching && GUIUtility.keyboardControl == listView.ID &&
          selectedRegularItemFunction != null)
        DoItemSelectedEvent(selectedRegularItemFunction,
                            columnViewElements[selectedIndex].value);
    }

    if (GUIUtility.keyboardControl == listView.ID &&
        Event.current.type == EventType.KeyDown && !isSearching) {
    ...
    foreach (ListViewElement element in ListViewGUILayout.ListView(
                 listView, s_Styles.background, GUILayout.Width(columnWidth))) {
      var columnViewElement = columnViewElements[element.row];

      if (element.row == listView.row) {
        if (Event.current.type == EventType.Repaint) {
          // FIXME: One Pixel offset is required when using OL Box style. Use a
          // different style.
          var rect = element.position;
          rect.x++;
          rect.y++;

          s_Styles.selected.Draw(rect, false, true, true,
                                 GUIUtility.keyboardControl == listView.ID);
        }
      }
      ...
      DoDoubleClick(element, columnViewElement, selectedSearchItemFunction,
                    selectedRegularItemFunction);
      DoDragAndDrop(listView, element, columnViewElements,
  ...
  private static void
  DoDragAndDrop(ListViewState listView, ListViewElement element,
                List<ColumnViewElement> columnViewElements,
                ObjectColumnGetDataFunction getDataForDraggingFunction) {
    if (GUIUtility.hotControl == listView.ID &&
      ...
      var delay = (DragAndDropDelay) GUIUtility.GetStateObject(
          typeof(DragAndDropDelay), listView.ID);
    ...
    if (GUIUtility.hotControl == listView.ID &&
      ...
      if (delay.CanStartDrag()) {
        var data = getDataForDraggingFunction == null ? null
            : getDataForDraggingFunction(
                       columnViewElements[listView.row].value);
