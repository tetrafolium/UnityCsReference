...
// Description:
// EditorApplicationLayout handles the GUI when playmode changes (on a high
// level).

// When entering playmode the flow is as follows (also see
// Application::SetIsPlaying() for actual loading): 1) Calling
// InitPlaymodeLayout prepares the main gameView WITHOUT rendering it. Sets it
// up, maximizes it if needed and intitializes its size. 2) The current scene is
// loaded from Application.cp and initialized (Awake(), OnEnable(), Start() and
// first Update() is called (this takes time for large projects)) 3) Calling
// FinalizePlaymodeLayout finalizes and renders maximized window (if set).
