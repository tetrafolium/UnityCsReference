...
/*
 Note that content of PopupWindow do not survive assembly reloading because it
 derives from interface PopupWindowContent. E.g use it for short lived content
 where closing on lost focus is ok.
 */
...
public class PopupWindow : EditorWindow {
  ...
  internal static void Show(Rect activatorRect,
    ...
    Show(activatorRect, windowContent, locationPriorityOrder,
         ShowMode.PopupMenu);
  }
  ...
  internal static void Show(Rect activatorRect,
                            ...
                            PopupLocation[] locationPriorityOrder,
                            ShowMode showMode) {
    // If we already have a popup window showing this type of content, then just
    ...
    if (existingWindows != null && existingWindows.Length > 0) {
      ...
      if (existingPopup != null) {
        if (existingPopup.m_WindowContent.GetType() ==
            windowContent.GetType()) {
  ...
  internal static bool ShouldShowWindow(Rect activatorRect) {
    ...
    bool justClosed = (EditorApplication.timeSinceStartup - s_LastClosedTime) <
                      kJustClickedTime;
  ...
  internal void Init(Rect activatorRect, PopupWindowContent windowContent,
                     PopupLocation[] locationPriorityOrder, ShowMode showMode,
                     bool giveFocus) {
    ...
    ShowAsDropDown(m_ActivatorRect, m_WindowContent.GetWindowSize(),
                   locationPriorityOrder, showMode, giveFocus);
  }
  ...
  private void FitWindowToContent() {
    ...
    if (m_LastWantedSize != wantedSize) {
      ...
      Rect screenRect = m_Parent.window.GetDropDownRect(
          m_ActivatorRect, wantedSize, wantedSize, m_LocationPriorityOrder);
