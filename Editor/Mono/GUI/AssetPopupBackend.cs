...
internal class AssetPopupBackend {
  public static void AssetPopup<T>(SerializedProperty serializedProperty,
                                   GUIContent label, string fileExtension,
                                   string defaultFieldName) where T : Object,
                                                                      new () {
    ...
    if (serializedProperty.hasMultipleDifferentValues)
      ...
      buttonContent =
          GUIContent.Temp(serializedProperty.objectReferenceStringValue);
    else
      buttonContent = GUIContent.Temp(defaultFieldName);

    Rect buttonRect;
    if (AudioMixerEffectGUI.PopupButton(
            label, buttonContent, EditorStyles.popup,
            out buttonRect)) // move & rename PopupButton..
      ShowAssetsPopupMenu<T>(buttonRect, typeName, serializedProperty,
                             fileExtension, defaultFieldName);
  ...
  public static void AssetPopup<T>(SerializedProperty serializedProperty,
                                   GUIContent label,
                                   string fileExtension) where T : Object,
                                                                   new () {
  ...
  ShowAssetsPopupMenu<T>(Rect buttonRect, string typeName,
                         SerializedProperty serializedProperty,
                         string fileExtension, string defaultFieldName) where T
      : Object,
        new () {
    ...
    int selectedInstanceID =
        ...
        null ? serializedProperty.objectReferenceValue.GetInstanceID() : 0;
    ...
    if (classID > 0) {
      ...
      foreach (var resource in resourceList) {
        if (resource.m_Name == defaultFieldName) {
          gm.AddItem(new GUIContent(resource.m_Name),
                     resource.m_InstanceID == selectedInstanceID,
                     AssetPopupMenuCallback,
                     new object[]{resource.m_InstanceID, serializedProperty});
    ...
    foreach (var property in AssetDatabase.FindAllAssets(
                 new SearchFilter(){classNames = new[]{typeName}})) {
      gm.AddItem(new GUIContent(property.name),
                 property.instanceID == selectedInstanceID,
                 AssetPopupMenuCallback,
                 new object[]{property.instanceID, serializedProperty});
    }
    ...
    bool isPreset = target is Component ?(
                        (int)(target as Component).gameObject.hideFlags == 93)
        : !AssetDatabase.Contains(target);

    // the preset object is destroyed with the inspector, and nothing new can be
    // created that needs this link. Fix for case 1208437
    if (!isPreset) {
      // Create item
      gm.AddSeparator("");
      gm.AddItem(
          EditorGUIUtility.TrTextContent("Create New..."), false, delegate {
            ...
            ProjectWindowUtil.StartNameEditingIfProjectWindowExists(
                newAsset.GetInstanceID(), doCreate,
                "New " + typeName + "." + fileExtension,
                AssetPreview.GetMiniThumbnail(newAsset), null);
          });
    }
  ...
  static void ShowAssetsPopupMenu<T>(Rect buttonRect, string typeName,
                                     SerializedProperty serializedProperty,
                                     string fileExtension) where T : Object,
                                                                     new () {
    ShowAssetsPopupMenu<T>(buttonRect, typeName, serializedProperty,
