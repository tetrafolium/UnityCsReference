...
internal class AboutWindow : EditorWindow {
  static void ShowAboutWindow() {
    AboutWindow w = EditorWindow.GetWindowWithRect<AboutWindow>(
        new Rect(100, 100, 570, 360), true, "About Unity");
  ...
  private const string kSpecialThanksNames =
      "Thanks to Forest 'Yoggy' Johnson, Graham McAllister, David Janik-Jones, Raimund Schumacher, Alan J. Dickins and Emil 'Humus' Persson";

  static class Styles {
    public static GUIContent thanksContent =
        new GUIContent("Special thanks to our beta users");
    public static GUIStyle thanksStyle =
        EditorStyles.FromUSS("About-Thanks-Label");
    public static Uri thanksUri =
        new Uri($"https://unity.com/releases/{GetVersion()}/thanks");
  ...
  public void OnDisable() { EditorApplication.update -= UpdateScroll; }
  ...
  public void OnGUI() {
    ...
    if (m_ShowDetailedVersion) {
      ...
      EditorGUILayout.SelectableLabel(
          string.Format("Version: {0}{1}{2}\nRevision: {3} {4}\nBuilt: {5:r}",
                        InternalEditorUtility.GetUnityDisplayVersionVerbose(),
                        licenseTypeString, extensionVersion, branch,
                        InternalEditorUtility.GetUnityBuildHash(),
                        dt.AddSeconds(t)),
      ...
      GUILayout.Label(string.Format(
          "Version {0}{1}{2}", InternalEditorUtility.GetUnityDisplayVersion(),
          licenseTypeString, extensionVersion));
    }
    ...
    chunkOffset =
        DoCreditsNameChunk(kSpecialThanksNames, creditsWidth, chunkOffset);
    ...
    GUILayout.Label(
        ...
        "MiniLabel", GUILayout.Width(210));
    GUILayout.Label(s_AgeiaLogo);
    GUILayout.Label("Physics powered by PhysX.\n\n(c) 2019 NVIDIA Corporation.",
                    "MiniLabel", GUILayout.Width(200));
    GUILayout.EndHorizontal();
    GUILayout.FlexibleSpace();
    GUILayout.BeginHorizontal();
    GUILayout.Space(5);
    ...
    var specialThanksRect =
        GUILayoutUtility.GetRect(Styles.thanksContent, Styles.thanksStyle);
    if (GUI.Button(specialThanksRect, Styles.thanksContent,
                   Styles.thanksStyle)) {
    ...
    GUILayout.BeginVertical();
    GUILayout.FlexibleSpace();
    GUILayout.Label(InternalEditorUtility.GetLicenseInfo(),
  ...
  private static float DoCreditsNameChunk(string nameChunk, float creditsWidth,
                                          float creditsChunkYOffset) {
    float creditsNamesHeight = EditorStyles.wordWrappedLabel.CalcHeight(
        GUIContent.Temp(nameChunk), creditsWidth);
    Rect creditsNamesRect =
  ...
  private void ListenForSecretCodes() {
    ...
    if (SecretCodeHasBeenTyped("internal", ref m_InternalCodeProgress)) {
      ...
      ShowNotification(new GUIContent(
          string.Format(L10n.Tr("Developer Mode {0}"), (enabled ? L10n.Tr("On")
                                                        : L10n.Tr("Off")))));
  ...
  private bool SecretCodeHasBeenTyped(string code, ref int characterProgress) {
    if (characterProgress < 0 || characterProgress >= code.Length ||
        code[characterProgress] != Event.current.character)
      characterProgress = 0;

    // Don't use else here. Even if key was mismatch, it should still be
    // recognized as first key of sequence if it matches.
