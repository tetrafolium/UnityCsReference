...
// each mode preserves maximum componential integrity until editing using a
// different mode exposure and HDR interfaces assume that the supplied
// originalColor and all per-channel modifications are in linear space
[Serializable]
internal class ColorMutator {
// specifies the max byte value to use when decomposing a float color into
// bytes with exposure this is the value used by Photoshop
private const byte k_MaxByteForOverexposedColor = 191;

internal static void DecomposeHdrColor(Color linearColorHdr,
	...
	if (maxColorComponent == 0f ||
	    maxColorComponent <= 1f && maxColorComponent >= 1 / 255f) {
		exposure = 0f;
		...
		// maintain maximal integrity of byte values to prevent off-by-one errors
		// when scaling up a color one component at a time
		baseLinearColor.r =
			Math.Min(k_MaxByteForOverexposedColor,
...
public Color32 color {
	get {
		return new Color32(
			m_Color[(int) RgbaChannel.R], m_Color[(int) RgbaChannel.G],
			m_Color[(int) RgbaChannel.B], m_Color[(int) RgbaChannel.A]);
	}
}
[SerializeField]
...
public void SetColorChannel(RgbaChannel channel, byte value) {
	...
	Color.RGBToHSV(color, out m_Hsv[(int) HsvChannel.H],
...
public void SetColorChannel(RgbaChannel channel, float normalizedValue) {
	SetColorChannel(channel, (byte) Mathf.RoundToInt(
				Mathf.Clamp01(normalizedValue) * 255f));
}

public Color exposureAdjustedColor {
	get {
		return new Color(
			m_ColorHdr[(int) RgbaChannel.R], m_ColorHdr[(int) RgbaChannel.G],
			m_ColorHdr[(int) RgbaChannel.B], m_ColorHdr[(int) RgbaChannel.A]);
	}
}
[SerializeField]
...
public Vector3 colorHsv {
	get {
		return new Vector3(m_Hsv[(int) HsvChannel.H], m_Hsv[(int) HsvChannel.S],
...
public void SetColorChannel(HsvChannel channel, float value) {
	...
	var newColor =
		Color.HSVToRGB(m_Hsv[(int) HsvChannel.H], m_Hsv[(int) HsvChannel.S],
		               m_Hsv[(int) HsvChannel.V]);
...
void OnRgbaHdrChannelChanged(int channel) {
	m_Color[(int) RgbaChannel.A] = (byte) Mathf.RoundToInt(
		Mathf.Clamp01(m_ColorHdr[(int) RgbaChannel.A]) * 255f);
	if (channel == (int) RgbaChannel.A)
		return;

	Color32 baseColor;
	DecomposeHdrColor(exposureAdjustedColor, out baseColor,
	                  out m_ExposureValue);
