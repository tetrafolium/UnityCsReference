...
internal class ColorPicker : EditorWindow {
  ...
  // HDR and LDR have different slider mode pref keys because they have
  // different defaults for the sake of discoverability
  private const string k_SliderModePrefKey = "CPSliderMode";
  private const string k_SliderModeHDRPrefKey = "CPSliderModeHDR";

  // the default max amount of stops to use for the intensity slider; same as in
  // Photoshop/Affinity
  ...
  [SerializeField] bool m_HDR;

  [SerializeField]
  private ColorMutator m_Color;

#pragma warning disable 649
  [SerializeField] Texture2D m_ColorSlider;

  [SerializeField] Color[] m_Colors;
  const int kHueRes = 64;
  const int kColorBoxSize = 32;
  [SerializeField] Texture2D m_ColorBox;
  static int s_Slider2Dhash = "Slider2D".GetHashCode();
  [SerializeField] bool m_ShowPresets = true;

  [SerializeField] bool m_ShowAlpha = true;

  [SerializeField] Texture2D m_RTexture;
  float m_RTextureG = -1, m_RTextureB = -1;
  [SerializeField] Texture2D m_GTexture;
  float m_GTextureR = -1, m_GTextureB = -1;
  [SerializeField] Texture2D m_BTexture;
  float m_BTextureR = -1, m_BTextureG = -1;

  [SerializeField] Texture2D m_HueTexture;
  float m_HueTextureS = -1, m_HueTextureV = -1;
  [SerializeField] Texture2D m_SatTexture;
  float m_SatTextureH = -1, m_SatTextureV = -1;
  [SerializeField] Texture2D m_ValTexture;
  float m_ValTextureH = -1, m_ValTextureS = -1;

  [NonSerialized] int m_TextureColorBoxMode = -1;
  [SerializeField] float m_LastConstant = -1;

  [NonSerialized] bool m_ColorSpaceBoxDirty;

  enum ColorBoxMode { HSV, EyeDropper }

  [SerializeField] ColorBoxMode m_ColorBoxMode = ColorBoxMode.HSV;

  enum SliderMode { RGB, RGBFloat, HSV }

  [SerializeField] SliderMode m_SliderMode = SliderMode.HSV;

  [SerializeField] Texture2D m_AlphaTexture;
  float m_OldAlpha = -1;

  [SerializeField] GUIView m_DelegateView;

  private Action<Color> m_ColorChangedCallback;

  [SerializeField] int m_ModalUndoGroup = -1;
  ...
  // the exposure slider range can dynamically grow if needed per color picker
  // "session"
  ...
  Vector2 Slider2D(Rect rect, Vector2 value, Vector2 maxvalue, Vector2 minvalue,
                   GUIStyle thumbStyle) {
    ...
    case EventType.MouseDown: {
      if (rect.Contains(e.mousePosition)) // inside this control
      {
        ...
        value.x = (e.mousePosition.x - rect.x) / rect.width *
                  (maxvalue.x - minvalue.x);
        value.y = (e.mousePosition.y - rect.y) / rect.height *
    ...
    case EventType.MouseDrag: {
      ...
      value.x =
          (e.mousePosition.x - rect.x) / rect.width * (maxvalue.x - minvalue.x);
      value.y = (e.mousePosition.y - rect.y) / rect.height *
                (maxvalue.y - minvalue.y);
    ...
    case EventType.Repaint: {
      // thumb (set to black for light colors to prevent blending with
      // background)
      var oldColor = GUI.color;
      GUI.color = VisionUtility.ComputePerceivedLuminance(color) >
                  Styles.highLuminanceThreshold ? Styles
                      .highLuminanceContentColor
          : Styles.lowLuminanceContentColor;
  ...
  void RGBSliders() {
    ...
    m_RTexture = Update1DSlider(m_RTexture, kColorBoxSize, g, b,
                                ref m_RTextureG, ref m_RTextureB, 0, false);
    m_GTexture = Update1DSlider(m_GTexture, kColorBoxSize, r, b,
                                ref m_GTextureR, ref m_GTextureB, 1, false);
    m_BTexture = Update1DSlider(m_BTexture, kColorBoxSize, r, g,
                                ref m_BTextureR, ref m_BTextureG, 2, false);
  ...
  void RGBSlider(string label, RgbaChannel channel,
    ...
    case SliderMode.RGB:
      ...
      value = EditorGUILayout.SliderWithTexture(
          GUIContent.Temp(label), value, 0f, 255f,
          EditorGUI.kIntFieldFormatString, 0f, 255f, sliderBackground);
    ...
    case SliderMode.RGBFloat:
      ...
      var sliderMax =
          m_HDR && m_Color.exposureAdjustedColor.maxColorComponent > 1f
              ? m_Color.exposureAdjustedColor.maxColorComponent /
                    maxRgbNormalized
              : 1f;
      var textFieldMax = m_HDR ? float.MaxValue : 1f;
      EditorGUI.BeginChangeCheck();
      value = EditorGUILayout.SliderWithTexture(
          GUIContent.Temp(label), value, 0f,
          m_DraggingFloatSlider ? m_FloatSliderMaxOnMouseDown
          : sliderMax, EditorGUI.kFloatFieldFormatString, 0f, textFieldMax,
            sliderBackground);
      switch (evtType) {
      case EventType.MouseDown:
  ...
  Texture2D Update1DSlider(Texture2D tex, int xSize, float const1, float const2,
                           ref float oldConst1, ref float oldConst2, int idx,
                           bool hsvSpace) {
  ...
  void HSVSliders() {
    ...
    m_HueTexture =
        Update1DSlider(m_HueTexture, kHueRes, 1, 1, ref m_HueTextureS,
                       ref m_HueTextureV, 0, true);
    m_SatTexture =
        Update1DSlider(m_SatTexture, kColorBoxSize, h, Mathf.Max(v, .2f),
                       ref m_SatTextureH, ref m_SatTextureV, 1, true);
    m_ValTexture =
        Update1DSlider(m_ValTexture, kColorBoxSize, h, s, ref m_ValTextureH,
                       ref m_ValTextureS, 2, true);

    EditorGUI.BeginChangeCheck();
    h = EditorGUILayout.SliderWithTexture(GUIContent.Temp("H"), h * 360f, 0f,
                                          360f, EditorGUI.kIntFieldFormatString,
                                          m_HueTexture);
    if (EditorGUI.EndChangeCheck()) {
    ...
    EditorGUI.BeginChangeCheck();
    s = EditorGUILayout.SliderWithTexture(GUIContent.Temp("S"), s * 100f, 0f,
                                          100f, EditorGUI.kIntFieldFormatString,
                                          m_SatTexture);
    if (EditorGUI.EndChangeCheck()) {
    ...
    EditorGUI.BeginChangeCheck();
    v = EditorGUILayout.SliderWithTexture(GUIContent.Temp("V"), v * 100f, 0f,
                                          100f, EditorGUI.kIntFieldFormatString,
                                          m_ValTexture);
    if (EditorGUI.EndChangeCheck()) {
  ...
  static void FillArea(int xSize, int ySize, Color[] retval, Color topLeftColor,
                       Color rightGradient, Color downGradient,
                       bool convertToGamma) {
  ...
  public static Texture2D MakeTexture(int width, int height) {
    Texture2D tex = new Texture2D(width, height, TextureFormat.RGBA32, false,
                                  true){hideFlags = HideFlags.HideAndDontSave,
  ...
  void DrawColorSpaceBox(Rect colorBoxRect, float constantValue) {
    ...
    if ((int) m_ColorBoxMode != m_TextureColorBoxMode ||
        m_LastConstant != constantValue || m_ColorSpaceBoxDirty) {
      m_Colors = m_ColorBox.GetPixels(0);
      int xSize = m_ColorBox.width;
      int ySize = m_ColorBox.height;

      FillArea(xSize, ySize, m_Colors,
               new Color(m_Color.GetColorChannel(HsvChannel.H), 0, 0, 1),
               new Color(0, 1, 0, 0), new Color(0, 0, 1, 0), false);
    ...
    Graphics.DrawTexture(
        colorBoxRect, m_ColorBox,
        new Rect(.5f / m_ColorBox.width, .5f / m_ColorBox.height,
                 1 - 1f / m_ColorBox.width, 1 - 1f / m_ColorBox.height),
        0, 0, 0, 0, new Color(.5f, .5f, .5f, .5f));
  }
  ...
  static class Styles {
    ...
    public const float extraVerticalSpacing =
        8f - EditorGUI.kControlVerticalSpacingLegacy;
    ...
    public static readonly ScalableGUIContent currentColorSwatchFill =
        new ScalableGUIContent(string.Empty, "The new color.",
    ...
    public static readonly Texture2D alphaSliderCheckerBackground =
        EditorGUIUtility.LoadRequired("Previews/Textures/textureChecker.png")
            as Texture2D;

    public static readonly GUIContent[] sliderModeLabels =
        new[]{EditorGUIUtility.TrTextContent("RGB 0-255"),
  ...
  void InitializePresetsLibraryIfNeeded() {
    if (m_ColorLibraryEditorState == null) {
      m_ColorLibraryEditorState =
          new PresetLibraryEditorState(presetsEditorPrefID);
      m_ColorLibraryEditorState.TransferEditorPrefsState(true);
    }

    if (m_ColorLibraryEditor == null) {
      var saveLoadHelper =
          new ScriptableObjectSaveLoadHelper<ColorPresetLibrary>("colors",
                                                                 SaveType.Text);
      m_ColorLibraryEditor = new PresetLibraryEditor<ColorPresetLibrary>(
          saveLoadHelper, m_ColorLibraryEditorState, OnClickedPresetSwatch){
          previewAspect = 1f,
          minMaxPreviewHeight = new Vector2(ColorPresetLibrary.kSwatchSize,
      ...
      m_ColorLibraryEditor.InitializeGrid(
          Styles.fixedWindowWidth -
          (Styles.background.padding.left + Styles.background.padding.right));
    }
  }
  ...
  private Color GetGUIColor(Color color) { return m_HDR ? color.gamma : color; }
  ...
  void DoColorSwatchAndEyedropper() {
    ...
    Rect position = GUILayoutUtility.GetRect(Styles.eyeDropper, GUIStyle.none,
                                             GUILayout.Width(40),
    ...
    // For a direct manipulation device, the color picking is on a drag&drop, so
    // we just need to start the eyeDropper on mouseDown instead of mouseUp
    ...
    // current swatch and original swatch have the same size, so they can lay
    // out in the same row
    var rect = GUILayoutUtility.GetRect(Styles.currentColorSwatchFill,
                                        Styles.currentColorSwatch,
                                        GUILayout.ExpandWidth(true));

    var swatchSize =
        Styles.currentColorSwatch.CalcSize(Styles.currentColorSwatchFill);
    var swatchRect =
        new Rect{size = swatchSize, y = rect.y, x = rect.xMax - swatchSize.x};
    ...
    if (evt.type == EventType.Repaint) {
      GUI.backgroundColor =
          m_Color.exposureAdjustedColor.a == 1f ? Color.clear : Color.white;
      GUI.contentColor = GetGUIColor(m_Color.exposureAdjustedColor);
      Styles.currentColorSwatch.Draw(swatchRect, Styles.currentColorSwatchFill,
                                     id);
    }

    swatchRect.x -= swatchRect.width;
    GUI.backgroundColor =
        m_Color.originalColor.a == 1f ? Color.clear : Color.white;
    GUI.contentColor = GetGUIColor(m_Color.originalColor);
    if (GUI.Button(swatchRect, Styles.originalColorSwatchFill,
                   Styles.originalColorSwatch)) {
      m_Color.Reset();
      evt.Use();
      OnColorChanged();
    }
  ...
  void DoColorSpaceGUI() {
    var backgroundStyle =
        m_HDR ? Styles.hueDialBackgroundHDR : Styles.hueDialBackground;
    ...
    case ColorBoxMode.HSV:
      ...
      hue = EditorGUI.AngularDial(dialRect, GUIContent.none, hue * 360f,
                                  ((GUIContent) Styles.hueDialThumbFill).image,
                                  backgroundStyle, Styles.hueDialThumb);
      ...
      if ((size & 1) == 1)
        size += 1;
      var svRect =
          new Rect{size = Vector2.one * size, center = dialRect.center};
      ...
      var sv = new Vector2(m_Color.GetColorChannel(HsvChannel.S),
                           1f - m_Color.GetColorChannel(HsvChannel.V));
      sv =
          Slider2D(svRect, sv, Vector2.zero, Vector2.one, Styles.colorBoxThumb);
  ...
  void DoColorSliders(float availableWidth) {
    ...
    m_SliderMode = (SliderMode) EditorGUILayout.IntPopup(
        GUIContent.Temp(" "), (int) m_SliderMode, Styles.sliderModeLabels,
    ...
    if (m_ShowAlpha) {
      m_AlphaTexture = Update1DSlider(m_AlphaTexture, kColorBoxSize, 0, 0,
                                      ref m_OldAlpha, ref m_OldAlpha, 3, false);
      ...
      if (Event.current.type == EventType.Repaint) {
        ...
        backgroundRect.xMin += EditorGUIUtility.labelWidth +
                               Styles.sliderBackground.padding.horizontal;
        ...
        var uvLayout =
            new Rect{x = 0f, y = 0f,
                     width = backgroundRect.width /
                             backgroundRect.height, // texture aspect is 1:1
                     height = 1f};
        Graphics.DrawTexture(backgroundRect,
                             Styles.alphaSliderCheckerBackground, uvLayout, 0,
                             0, 0, 0);
      }

      EditorGUI.BeginChangeCheck();
      var alpha =
          m_Color.GetColorChannelNormalized(RgbaChannel.A) * displayScale;
      alpha = EditorGUI.SliderWithTexture(rect, GUIContent.Temp("A"), alpha, 0f,
                                          displayScale, formatString,
                                          m_AlphaTexture);
      if (EditorGUI.EndChangeCheck()) {
  ...
  void DoHexField(float availableWidth) {
    ...
    EditorGUI.BeginChangeCheck();
    var newColor = EditorGUILayout.HexColorTextField(Styles.hexLabel,
                                                     m_Color.color, false);
    if (EditorGUI.EndChangeCheck()) {
  ...
  void DoExposureSlider() {
    ...
    EditorGUI.BeginChangeCheck();
    var exposureValue =
        EditorGUI.Slider(sliderPosition, Styles.exposureValue,
                         m_Color.exposureValue, -m_ExposureSliderMax,
                         m_ExposureSliderMax, float.MinValue, float.MaxValue);
    if (EditorGUI.EndChangeCheck()) {
  ...
  void DoExposureSwatches() {
    var swatchesRect = GUILayoutUtility.GetRect(
        GUIContent.none, Styles.exposureSwatch, GUILayout.ExpandWidth(true));

    var numSwatches = 5;
    var swatchRect = new Rect{
        x = swatchesRect.x + (swatchesRect.width -
                              numSwatches * Styles.exposureSwatch.fixedWidth) *
                                 0.5f,
        y = swatchesRect.y, width = Styles.exposureSwatch.fixedWidth,
    ...
    for (int i = 0; i < numSwatches; ++i) {
      ...
      GUI.contentColor = VisionUtility.ComputePerceivedLuminance(col) <
                         Styles.highLuminanceThreshold ? Styles
                             .lowLuminanceContentColor
          : Styles.highLuminanceContentColor;

      if (GUI.Button(swatchRect,
                     GUIContent.Temp(
                         stop == 0 ? null
                                   : (stop < 0 ? stop.ToString()
                                               : string.Format("+{0}", stop))),
                     Styles.exposureSwatch)) {
        m_Color.exposureValue =
            Mathf.Clamp(m_Color.exposureValue + stop, -m_ExposureSliderMax,
                        m_ExposureSliderMax);
        OnColorChanged();
      }
      ...
      if (stop == 0 && Event.current.type == EventType.Repaint) {
        ...
        Styles.selectedExposureSwatchStroke.Draw(swatchRect, false, false,
                                                 false, false);
      }
  ...
  void DoPresetsGUI() {
    var foldoutRect =
        GUILayoutUtility.GetRect(Styles.presetsToggle, EditorStyles.foldout);
    foldoutRect.xMax -= 17f; // make room for presets settings menu button
    m_ShowPresets = EditorGUI.Foldout(foldoutRect, m_ShowPresets,
                                      Styles.presetsToggle, true);

    if (m_ShowPresets) {
      GUILayout.Space(-(
          EditorGUIUtility.singleLineHeight +
          EditorGUIUtility.standardVerticalSpacing)); // pull up to reuse space
      var presetsRect = GUILayoutUtility.GetRect(
          0, Mathf.Clamp(m_ColorLibraryEditor.contentHeight, 20f, 250f));
  ...
  void OnGUI() {
    ...
    float innerContentWidth =
        EditorGUILayout.GetControlRect(false, 1, EditorStyles.numberField)
            .width;
    EditorGUIUtility.labelWidth =
    ...
    if (Event.current.type == EventType.KeyDown) {
      ...
      case KeyCode.Escape:
        // eyedropper GUIView never gets keyboard focus from ColorPicker, so esc
        // to exit it must be handled here
  ...
  public static Texture2D GetGradientTextureWithAlpha1To0() {
    return s_LeftGradientTexture ??(
        s_LeftGradientTexture = CreateGradientTexture(
            "ColorPicker_1To0_Gradient", 4, 4, new Color(1, 1, 1, 1),
            new Color(1, 1, 1, 0)));
  }

  public static Texture2D GetGradientTextureWithAlpha0To1() {
    return s_RightGradientTexture ??(
        s_RightGradientTexture = CreateGradientTexture(
            "ColorPicker_0To1_Gradient", 4, 4, new Color(1, 1, 1, 0),
            new Color(1, 1, 1, 1)));
  }

  static Texture2D CreateGradientTexture(string name, int width, int height,
                                         Color leftColor, Color rightColor) {
    var texture =
        new Texture2D(width, height, TextureFormat.RGBA32, false,
                      true){name = name, hideFlags = HideFlags.HideAndDontSave};
    var pixels = new Color[width * height];

    for (int i = 0; i < width; i++) {
      Color columnColor =
          Color.Lerp(leftColor, rightColor, i / (float)(width - 1));
  ...
  void OnColorChanged(bool exitGUI = true) {
    ...
    if (m_DelegateView != null) {
      var e =
          EditorGUIUtility.CommandEvent(EventCommandNames.ColorPickerChanged);
      Repaint();
  ...
  public static void Show(Action<Color> colorChangedCallback, Color col,
                          bool showAlpha = true, bool hdr = false) {
    Show(null, colorChangedCallback, col, showAlpha, hdr);
  }

  static void Show(GUIView viewToUpdate, Action<Color> colorChangedCallback,
                   Color col, bool showAlpha, bool hdr) {
    var cp = instance;
    cp.m_HDR = hdr;
    ...
    cp.m_ExposureSliderMax =
        Mathf.Max(cp.m_ExposureSliderMax, cp.m_Color.exposureValue);
    originalKeyboardControl = GUIUtility.keyboardControl;

    if (cp.m_HDR) {
      cp.m_SliderMode = (SliderMode) EditorPrefs.GetInt(k_SliderModeHDRPrefKey,
                                                        (int) SliderMode.RGB);
    } else {
    ...
    cp.titleContent =
        hdr ? EditorGUIUtility.TrTextContent("HDR Color")
        : EditorGUIUtility.TrTextContent("Color");
    float height =
        EditorPrefs.GetInt(k_HeightPrefKey, (int) cp.position.height);
    cp.minSize = new Vector2(Styles.fixedWindowWidth, height);
    cp.maxSize = new Vector2(Styles.fixedWindowWidth, height);
    cp.InitializePresetsLibraryIfNeeded(); // Ensure the heavy lifting of
                                           // loading presets is done before
                                           // window is visible
    cp.ShowAuxWindow();
  }
  ...
  void OnEnable() {
    ...
    EditorGUIUtility.editingTextField =
        true; // To fix that color values is not directly editable when tabbing
              // (case 557510)

    m_SliderMode = (SliderMode) EditorPrefs.GetInt(k_SliderModePrefKey,
                                                   (int) SliderMode.RGB);
    m_ShowPresets = EditorPrefs.GetInt(k_ShowPresetsPrefKey, 1) != 0;
  }

  void OnDisable() {
    EditorPrefs.SetInt(m_HDR ? k_SliderModeHDRPrefKey
                       : k_SliderModePrefKey, (int) m_SliderMode);
...
internal class EyeDropper : GUIView {
  ...
  // Can't be larger right now since OSX Metal surfaces can't be larger than
  // 16384 pixels. This needs to be changed to a larger size since it will miss
  // mouse events when using multiple 4K monitors
  ...
  public static void Start(GUIView viewToUpdate) { Start(viewToUpdate, null); }
  ...
  static void Start(GUIView viewToUpdate, Action<Color> colorPickedCallback) {
    ...
    win.Show(ShowMode.PopupMenu, loadPosition
             : true, displayImmediately
             : true, setFocus
             : true);
    instance.AddToAuxWindowList();
    win.SetInvisible();
    instance.SetMinMaxSizes(new Vector2(0, 0),
                            new Vector2(kDummyWindowSize, kDummyWindowSize));
    win.position = new Rect(-kDummyWindowSize / 2f, -kDummyWindowSize / 2f,
                            kDummyWindowSize, kDummyWindowSize);
  ...
  public static Color GetPickedColor() {
    return InternalEditorUtility.ReadScreenPixelUnderCursor(s_PickCoordinates,
                                                            1, 1)[0];
  }

  public static Color GetLastPickedColor() { return s_LastPickedColor; }

  static class Styles {
    public static readonly GUIStyle eyeDropperHorizontalLine =
        "EyeDropperHorizontalLine";
    public static readonly GUIStyle eyeDropperVerticalLine =
        "EyeDropperVerticalLine";
    public static readonly GUIStyle eyeDropperPickedPixel =
  ...
  public static void DrawPreview(Rect position) {
    ...
    preview.SetPixels(
        InternalEditorUtility.ReadScreenPixelUnderCursor(p, width, height), 0);
    preview.Apply(true);
    ...
    Rect newR = new Rect((p.x - mPos.x) * xStep + position.x,
                         (p.y - mPos.y) * yStep + position.y, xStep, yStep);
    Styles.eyeDropperPickedPixel.Draw(newR, false, false, false, false);
  }

  protected override void OldOnGUI() {
    // On mouse move/click we remember screen coordinates where we are. Then
    // we'll use that in GetPickedColor to read. The reason is that because
    // GetPickedColor might be called from an event which is different, so the
    // coordinates would be already wrong. for direct manipulation devices, it
    // is on mouse drag/up
  ...
  private void StopEyeDropper() {
    if (Event.current.button == 0) {
      ...
      // the window covers whole desktop (see Show()) and is black with 0x01
      // alpha value. That might cause invalid picked color.
      window.Close();
