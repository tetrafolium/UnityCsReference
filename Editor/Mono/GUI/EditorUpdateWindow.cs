...
internal class EditorUpdateWindow : EditorWindow {
  ...
  static void ShowEditorUpdateWindow(string latestVersionString,
  ...
  private static EditorUpdateWindow ShowWindow() {
    return EditorWindow.GetWindowWithRect(typeof(EditorUpdateWindow),
                                          new Rect(100, 100, 570, 400), true,
                                          s_Title.text) as EditorUpdateWindow;
  }
  ...
  private static void LoadResources() {
    ...
    s_TextHasUpdate = EditorGUIUtility.TrTextContent(
        "There is a new version of the Unity Editor available for download.\n\nCurrently installed version is {0}\nNew version is {1}");
    s_TextUpToDate = EditorGUIUtility.TrTextContent(
        "The Unity Editor is up to date. Currently installed version is {0}");

    s_CheckForNewUpdatesText =
        EditorGUIUtility.TrTextContent("Check for Updates");
  }
  ...
  public void OnGUI() {
    ...
    GUILayout.Space(10);
    GUI.Box(new Rect(13, 8, s_UnityLogo.image.width, s_UnityLogo.image.height),
            s_UnityLogo, GUIStyle.none);
    GUILayout.Space(5);
    ...
    if (s_HasConnectionError) {
      ...
      GUILayout.Label(string.Format(s_TextHasUpdate.text,
                                    InternalEditorUtility.GetFullUnityVersion(),
                                    s_LatestVersionString),
                      "WordWrappedLabel", GUILayout.Width(300));

      GUILayout.Space(20);
      m_ScrollPos = EditorGUILayout.BeginScrollView(
          m_ScrollPos, GUILayout.Width(405), GUILayout.Height(200));
      ...
      if (GUILayout.Button("Skip new version", GUILayout.Width(200))) {
        EditorPrefs.SetString("EditorUpdateSkipVersionString",
                              s_LatestVersionString);
        Close();
      }
      ...
      GUILayout.Label(
          ...
          "WordWrappedLabel", GUILayout.Width(405));
    }
    ...
    GUI.changed = false;
    s_ShowAtStartup =
        GUILayout.Toggle(s_ShowAtStartup, s_CheckForNewUpdatesText);
    if (GUI.changed)
