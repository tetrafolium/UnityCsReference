...
public TypeSelection(string typeName, Object[] objects) {
	...
	label = new GUIContent(objects.Length + " " +
	                       ObjectNames.NicifyVariableName(typeName) +
	                       (objects.Length > 1 ? "s" : ""));
	if (objects[0] is GameObject)
		label.image = EditorUtility.IsPersistent(objects[0])
		        ? PrefabUtility.GameObjectStyles.prefabIcon
		        : PrefabUtility.GameObjectStyles.gameObjectIcon;
	else
