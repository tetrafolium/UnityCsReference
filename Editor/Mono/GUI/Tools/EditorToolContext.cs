...
namespace UnityEditor.EditorTools {
  sealed class EditorToolContext : ScriptableSingleton<EditorToolContext> {
    [SerializeField] List<EditorTool> m_ToolSingletons = new List<EditorTool>();

    [SerializeField] EditorTool m_ActiveTool;

    static ActiveEditorTracker m_Tracker;

    [SerializeField] List<EditorTool> m_ToolHistory = new List<EditorTool>();

    static bool s_ChangingActiveTool;

    // Mimic behavior of Tools.toolChanged for backwards compatibility until
    // existing tools are converted to the new apis.
    internal static event Action<EditorTool, EditorTool> activeToolChanged;

    // EditorTools that are created as custom editor tools. This list represents
    // only the shared tracker.
    List<EditorTool> m_CustomEditorTools = new List<EditorTool>();

    // This list represents any custom editor tools created by locked
    // inspectors. They are not shown in the scene or context menu.
    ...
    internal static EditorTool activeTool {
      ...
      set {
        if (s_ChangingActiveTool) {
          // pop the changing state so that we don't lock the active tool after
          // an exception is thrown.
          s_ChangingActiveTool = false;
          throw new InvalidOperationException(
              "Attempting to set the active tool from EditorTool.OnActivate or EditorTool.OnDeactivate. This is not allowed.");
        }
    ...
    [Serializable] struct CustomEditorToolContext
        : ISerializationCallbackReceiver {
      [SerializeField] string m_EditorToolType;

      [SerializeField] string m_EditorToolState;
      ...
      public static readonly CustomEditorToolContext Empty =
          new CustomEditorToolContext(null);
      ...
      public void OnBeforeSerialize() {
        m_EditorToolType =
            editorToolType != null ? editorToolType.AssemblyQualifiedName : null;
      }
    ...
    [SerializeField] CustomEditorToolContext m_PreviousCustomEditorToolContext;

    // EditorApplication.isPlayingOrWillEnterPlayMode doesn't handle exiting.
    [SerializeField] PlayModeStateChange m_PlayModeState;
    ...
    void PlayModeStateChanged(PlayModeStateChange state) {
      ...
      // TrackerRebuilt is called during the ExitingEditMode phase, but the
      // selection might not be valid yet.
    ...
    void SelectedObjectWasDestroyed(int id) {
      if (m_CustomEditorTools.Any(x => x == m_ActiveTool) &&
          m_ActiveTool.m_Targets.Any(x => x == null ||
                                          x.GetInstanceID() == id)) {
        m_PreviousCustomEditorToolContext =
            new CustomEditorToolContext(m_ActiveTool);
        RestorePreviousTool();
      }
    }

    void UndoRedoPerformed() { RestoreCustomEditorTool(); }

    void RestoreCustomEditorTool() {
      var restored = m_CustomEditorTools.FirstOrDefault(
          m_PreviousCustomEditorToolContext.IsEqual);

      // Check for existence in locked inspectors too, but only if the locked
      // inspector target is being inspected
      if (restored == null &&
          m_PreviousCustomEditorToolContext.targetObject != null &&
          Selection.objects.Any(
              x => x.Equals(m_PreviousCustomEditorToolContext.targetObject)))
        restored = m_LockedCustomEditorTools.FirstOrDefault(
            m_PreviousCustomEditorToolContext.IsEqual);

      if (restored != null) {
        var targets = restored.targets.ToArray();
        EditorJsonUtility.FromJsonOverwrite(
            m_PreviousCustomEditorToolContext.editorToolState, restored);
    ...
    internal static EditorTool GetSingleton(Type type) {
      ...
      var res =
          instance.m_ToolSingletons.FirstOrDefault(x => x.GetType() == type);
      if (res != null)
        return res;
    ...
    internal static void GetToolHistory(List<EditorTool> tools,
                                        bool customToolsOnly = false) {
      tools.Clear();
      instance.CleanupToolHistory();

      for (int i = instance.m_ToolHistory.Count - 1; i > -1; i--) {
        if (!customToolsOnly || EditorToolUtility.GetEnumWithEditorTool(
                                    instance.m_ToolHistory[i]) == Tool.Custom)
          tools.Add(instance.m_ToolHistory[i]);
      }
    }

    internal static EditorTool
    GetLastTool(Func<EditorTool, bool> predicate = null) {
      instance.CleanupToolHistory();
    ...
    internal static EditorTool GetLastCustomTool() {
      for (int i = instance.m_ToolHistory.Count - 1; i > -1; i--)
        if (EditorToolUtility.GetEnumWithEditorTool(
                instance.m_ToolHistory[i]) == Tool.Custom)
    ...
    static bool IsCustomEditorTool(EditorTool tool) {
      return EditorToolUtility.IsCustomEditorTool(tool != null ? tool.GetType()
                                                  : null);
    }

    static List<CustomEditorTool> s_CustomEditorTools =
        new List<CustomEditorTool>();

    void RebuildAvailableCustomEditorTools() {
      EditorApplication.delayCall -= RebuildAvailableCustomEditorTools;

      // Do not rebuild the cache since objects are serialized, destroyed,
      // deserialized during this phase
      ...
      // If the shared tracker is locked, use our own tracker instance so that
      // the current selection is always represented. Addresses case where a
      // single locked inspector is open.
      ...
      EditorToolUtility.GetEditorToolsForTracker(shared.isLocked ? tracker
                                                 : shared, s_CustomEditorTools);

      foreach (var customEditorTool in s_CustomEditorTools) {
        if (m_CustomEditorTools.Any(
                x => x.GetType() == customEditorTool.editorToolType &&
                     x.target == customEditorTool.owner.target))
          continue;
        EditorTool tool;
      ...
      foreach (var inspector in inspectors) {
        if (inspector.isLocked) {
          EditorToolUtility.GetEditorToolsForTracker(inspector.tracker,
                                                     s_CustomEditorTools);

          foreach (var customEditorTool in s_CustomEditorTools) {
            // Don't add duplicate tools to either another locked inspector with
            // the same target, or a shared tracker
            if (m_CustomEditorTools.Any(
                    x => x.GetType() == customEditorTool.editorToolType &&
                         x.target == customEditorTool.owner.target) ||
                m_LockedCustomEditorTools.Any(
                    x => x.GetType() == customEditorTool.editorToolType &&
                         x.target == customEditorTool.owner.target))
              continue;
            EditorTool tool;
            preservedActiveTool |=
                CreateOrRestoreTool(customEditorTool, out tool);
    ...
    bool CreateOrRestoreTool(CustomEditorTool customEditorTool,
                             out EditorTool customEditorToolInstance) {
      ...
      // The only case where a custom editor tool is serialized is when it is
      // the active tool. All other instances are discarded and rebuilt on any
      // tracker rebuild.
      if (activeIsCustomEditorTool &&
          CustomEditorToolIsMatch(toolOwner, toolType, m_ActiveTool)) {
        preservedActiveTool = true;

        m_ActiveTool.m_Targets = targets;
        m_ActiveTool.m_Target = target;

        // domain reload - the owning editor was destroyed and therefore we need
        // to reset the EditMode active
        if (m_ActiveTool is EditModeTool &&
            toolOwner.GetInstanceID() != UnityEditorInternal.EditMode.ownerID)
          UnityEditorInternal.EditMode.EditModeToolStateChanged(
    ...
    static bool CustomEditorToolIsMatch(Editor editor, Type toolType,
                                        EditorTool tool) {
      if (editor == null || toolType != tool.GetType())
        return false;

      // if it's an EditModeTool we need to be stricter about ownership for
      // backwards compatibility.
      var editModeTool = tool as EditModeTool;

      if (editModeTool != null)
        return editModeTool.owner == (IToolModeOwner) editor ||
               editModeTool.target == editor.target;
    ...
    internal static EditorTool
    GetCustomEditorToolOfType(Type type, bool searchLockedInspectors = true) {
    ...
    internal static EditorTool
    GetCustomEditorToolsForType(Type targetType, List<EditorTool> list,
                                bool searchLockedInspectors) {
      foreach (var customEditorTool in instance.m_CustomEditorTools)
        if (customEditorTool != null &&
            EditorToolUtility.GetCustomEditorToolTargetType(customEditorTool) ==
                targetType)
      ...
      if (searchLockedInspectors) {
        foreach (var customEditorTool in instance.m_LockedCustomEditorTools)
          if (customEditorTool != null &&
              EditorToolUtility.GetCustomEditorToolTargetType(
                  customEditorTool) == targetType)
            list.Add(customEditorTool);
      }
    ...
    GetCustomEditorTools(List<EditorTool> list,
                         bool includeLockedInspectorTools) {
      list.Clear();
    ...
    internal static void
    GetCustomEditorToolsForTarget(UnityObject target, List<EditorTool> list,
                                  bool searchLockedInspectors) {
      list.Clear();
    ...
    internal static EditorTool
    GetCustomEditorTool(Func<EditorTool, bool> predicate,
                        bool searchLockedInspectors) {
      foreach (var tool in instance.m_CustomEditorTools)
        if (predicate(tool))
          return tool;
    ...
    internal static ScriptableObject
    CreateInstance(Type type, Action<ScriptableObject> initialize) {
      if (!typeof(ScriptableObject).IsAssignableFrom(type))
        throw new ArgumentException("Type must inherit ScriptableObject.",
                                    "type");
