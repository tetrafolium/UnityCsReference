...
public enum ViewTool { None = -1, Orbit = 0, Pan = 1, Zoom = 2, FPS = 3 }

public enum PivotMode { Center = 0, Pivot = 1 }

public enum PivotRotation { Local = 0, Global = 1 }
...
#pragma warning disable 618
  [System.Obsolete(
      "Use EditorTools.activeToolDidChange or EditorTools.activeToolWillChange")]
...
#pragma warning restore 618
  ...
  public static Tool current {
    get {
      return EditorToolUtility.GetEnumWithEditorTool(
          EditorToolContext.GetActiveTool());
    }
    set {
      EditorToolContext.activeTool =
          EditorToolUtility.GetEditorToolWithEnum(value);
    }
  }

  internal static void SyncToolEnum() { RepaintAllToolViews(); }
  ...
  internal static bool viewToolActive {
    get {
      ...
      Event evt = Event.current;
      bool viewShortcut = evt.type != EventType.Used &&
                          (evt.alt || evt.button == 1 || evt.button == 2);
      return s_LockedViewTool != ViewTool.None || current == Tool.View ||
             viewShortcut;
    }
  }
      ...
      if (current == Tool.Rect)
        return handleRotation *
                   InternalEditorUtility
                       .CalculateSelectionBoundsInSpace(
                           Vector3.zero, handleRotation, rectBlueprintMode)
                       .center +
               totalOffset;
      else
        return InternalEditorUtility.CalculateSelectionBounds(true, false)
                   .center +
               totalOffset;
    }
    case PivotMode.Pivot: {
      if (current == Tool.Rect && rectBlueprintMode &&
          InternalEditorUtility.SupportsRectLayout(t))
        return t.parent.TransformPoint(
                   new Vector3(t.localPosition.x, t.localPosition.y, 0)) +
               totalOffset;
      else
  ...
  public static Rect handleRect {
    get {
      Bounds bounds = InternalEditorUtility.CalculateSelectionBoundsInSpace(
          handlePosition, handleRotation, rectBlueprintMode);
      int axis = GetRectAxisForViewDir(
          bounds, handleRotation,
  ...
  private static int GetRectAxisForViewDir(Bounds bounds, Quaternion rotation,
                                           Vector3 viewDir) {
    ...
    if (viewDir == Vector3.zero) {
      ...
      for (int normalAxis = 0; normalAxis < 3; normalAxis++) {
        ...
        float score =
            Vector3
                .Cross(Vector3.ProjectOnPlane(rotation * edge1, viewDir),
                       Vector3.ProjectOnPlane(rotation * edge2, viewDir))
                .magnitude;
        if (score > bestScore) {
          bestScore = score;
          axis = normalAxis;
  ...
  private static Rect GetRectFromBoundsForAxis(Bounds bounds, int axis) {
    switch (axis) {
    case 0:
      return new Rect(-bounds.max.z, bounds.min.y, bounds.size.z,
                      bounds.size.y);
    case 1:
      return new Rect(bounds.min.x, -bounds.max.z, bounds.size.x,
                      bounds.size.z);
    case 2:
    default:
  ...
  private static Quaternion GetRectRotationForAxis(Quaternion rotation,
                                                   int axis) {
    switch (axis) {
    case 0:
      return rotation * Quaternion.Euler(0, 90, 0);
    case 1:
      return rotation * Quaternion.Euler(-90, 0, 0);
    case 2:
    default:
      return rotation;
    }
  ...
  internal static void LockHandleRectRotation() {
    Bounds bounds = InternalEditorUtility.CalculateSelectionBoundsInSpace(
        handlePosition, handleRotation, rectBlueprintMode);
    s_LockHandleRectAxis = GetRectAxisForViewDir(
        bounds, handleRotation,
        SceneView.currentDrawingSceneView.camera.transform.forward);
  ...
  internal static int GetPivotMode() { return (int) pivotMode; }
  ...
  static StateCache<LayerSettings> s_LayersStateCache =
      new StateCache<LayerSettings>("Library/StateCache/LayerSettings/");
  ...
  void OnEnable() {
    ...
    var layerSettings = s_LayersStateCache.GetState(m_LayerSettingsKey,
                                                    new LayerSettings(-1, 0));
...
#pragma warning disable 618
      if (onToolChanged != null)
        onToolChanged(EditorToolUtility.GetEnumWithEditorTool(previous),
...
#pragma warning restore 618
  ...
  static void TogglePivotMode(ShortcutArguments args) {
    pivotMode =
        pivotMode == PivotMode.Center ? PivotMode.Pivot : PivotMode.Center;
    ResetGlobalHandleRotation();
    RepaintAllToolViews();
  }
  ...
  static void TogglePivotRotation(ShortcutArguments args) {
    pivotRotation =
        pivotRotation ==
        PivotRotation.Global ? PivotRotation.Local : PivotRotation.Global;
    ResetGlobalHandleRotation();
    RepaintAllToolViews();
  }
