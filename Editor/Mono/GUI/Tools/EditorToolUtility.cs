...
namespace UnityEditor.EditorTools {
  ...
  static class EditorToolUtility {
    static readonly Regex k_NewLine =
        new Regex(@"\r|\n", RegexOptions.Compiled | RegexOptions.Multiline);
    static readonly Regex k_TrailingForwardSlashOrWhiteSpace =
        new Regex(@"[/|\s]*\Z", RegexOptions.Compiled);
    ...
    static Dictionary<Type, List<Type>> s_CustomEditorToolsTypeAssociations =
        new Dictionary<Type, List<Type>>();
    static Dictionary<Type, GUIContent> s_EditorToolDefaulToolbarIcons =
    ...
    static CustomEditorToolAssociation[] customEditorTools {
      get {
        if (s_CustomEditorTools == null) {
          Type[] editorTools =
              TypeCache.GetTypesWithAttribute<EditorToolAttribute>()
          ...
          for (int i = 0; i < len; i++) {
            var customToolAttribute =
                (EditorToolAttribute) editorTools [i]
                    .GetCustomAttributes(typeof(EditorToolAttribute), false)
                    .FirstOrDefault();
    ...
    internal static string GetToolMenuPath(Type tool) {
      var attributes =
          tool.GetCustomAttributes(typeof(EditorToolAttribute), false);

      foreach (var attrib in attributes) {
        var menuAttrib = attrib as EditorToolAttribute;

        if (menuAttrib != null &&
            !string.IsNullOrEmpty(menuAttrib.displayName)) {
    ...
    internal static EditorToolAttribute GetEditorToolAttribute(Type type) {
      ...
      return (EditorToolAttribute)
          type.GetCustomAttributes(typeof(EditorToolAttribute), false)
              .FirstOrDefault();
    }
    ...
    GetEditorToolsForTracker(ActiveEditorTracker tracker,
                             List<CustomEditorTool> tools) {
      tools.Clear();
      ...
      for (int i = 0, c = editors.Length; i < c; i++) {
        ...
        // Some components can be added to a GameObject multiple times. Prevent
        // them from creating multiple tools.
    ...
    internal static EditorTool GetEditorToolWithEnum(Tool type) {
      ...
      if (type == Tool.Custom) {
        var tool = EditorToolContext.GetLastTool(
            x => GetEnumWithEditorTool(x) == Tool.Custom);
        if (tool != null)
          return tool;
      }
    ...
    static bool IsBuiltinTool(Type type) {
      return type == typeof(ViewModeTool) || type == typeof(TransformTool) ||
             type == typeof(MoveTool) || type == typeof(RotateTool) ||
             type == typeof(ScaleTool) || type == typeof(RectTool);
    }
    ...
    internal static GUIContent GetDefaultToolbarIcon(Type editorToolType) {
      ...
      res = new GUIContent(){tooltip = name};

      var attrib = GetEditorToolAttribute(editorToolType);

      if (attrib != null && attrib.targetType != null)
        res.image =
            AssetPreview.GetMiniTypeThumbnailFromType(attrib.targetType);
      else
