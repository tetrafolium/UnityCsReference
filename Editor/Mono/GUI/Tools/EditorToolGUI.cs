...
public sealed partial class EditorGUILayout {
  static readonly EditorToolGUI.ReusableArrayPool<GUIContent> s_ButtonArrays =
      new EditorToolGUI.ReusableArrayPool<GUIContent>();
  static readonly EditorToolGUI.ReusableArrayPool<bool> s_BoolArrays =
      new EditorToolGUI.ReusableArrayPool<bool>();
  ...
  public static void EditorToolbarForTarget(UObject target) {
    ...
    EditorToolContext.GetCustomEditorToolsForTarget(target, s_CustomEditorTools,
                                                    true);
    EditorToolbar<EditorTool>(s_CustomEditorTools);
    s_CustomEditorTools.Clear();
  }

  public static void EditorToolbarForTarget(GUIContent content,
                                            UObject target) {
    if (target == null)
      throw new ArgumentNullException("target");
  ...
  internal static bool EditorToolbar<T>(EditorTool selected, IList<T> tools,
                                        out EditorTool clicked) where T
      : EditorTool {
...
static class EditorToolGUI {
  ...
  static class Styles {
    public static GUIContent recentTools =
        EditorGUIUtility.TrTextContent("Recent");
    public static GUIContent selectionTools =
        EditorGUIUtility.TrTextContent("Selection");
    public static GUIContent availableTools =
        EditorGUIUtility.TrTextContent("Available");
    public static GUIContent noToolsAvailable =
        EditorGUIUtility.TrTextContent("No custom tools available");
  }
  ...
  public static readonly StyleRect s_ButtonRect =
      EditorResources.GetStyle("AppToolbar-Button")
          .GetRect(StyleCatalogKeyword.size, StyleRect.Size(22, 22));
  ...
  static internal void DoContextualToolbarOverlay(UnityEngine.Object target,
                                                  SceneView sceneView) {
    GUILayout.BeginHorizontal(GUIStyle.none, GUILayout.MinWidth(210),
                              GUILayout.Height(30));
    ...
    if (s_EditorToolModes.Count > 0) {
      ...
      GUILayout.Label(Styles.noToolsAvailable,
  ...
  internal static void DoToolContextMenu() {
    ...
    if (EditorToolContext.GetLastCustomTool() != null) {
      ...
      for (var i = 0; i < Math.Min(k_MaxToolHistory, s_ToolList.Count); i++) {
        ...
        if (tool.IsAvailable())
          toolHistoryMenu.AddItem(new GUIContent(name), false, () => {
      ...
      for (var i = 0; i < s_ToolList.Count; i++) {
        ...
        if (tool.IsAvailable())
          toolHistoryMenu.AddItem(
              path, false, () => { EditorToolContext.activeTool = tool; });
        else
    ...
    if (global.Any()) {
      ...
      foreach (var toolType in global) {
        toolHistoryMenu.AddItem(
            new GUIContent(EditorToolUtility.GetToolMenuPath(toolType)), false,
