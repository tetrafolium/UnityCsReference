...
GUIContent m_HandlePositionAndRotation =
	EditorGUIUtility.TrTextContent("Handle Position and Rotation");

public override void OnInspectorGUI() {
	var labelWidth = Mathf.Max(
		EditorGUIUtility.labelWidth,
		EditorStyles.label.CalcSize(m_HandlePositionAndRotation).x + k_Padding);

	Rect m_Rect = new Rect(k_Padding, k_Padding, labelWidth,
...
public override void OnToolGUI(EditorWindow window) {
	...
	if (!StageUtility.IsGameObjectRenderedByCameraAndPartOfEditableScene(
		    Selection.activeTransform.gameObject, Camera.current))
		return;
...
protected abstract void ToolGUI(SceneView view, Vector3 handlePosition,
                                bool isStatic);
protected virtual bool ShouldToolGUIBeDisabled(out GUIContent disabledLabel) {
	disabledLabel = Handles.s_StaticLabel;
	return (!Tools.s_Hidden && EditorApplication.isPlaying &&
	        GameObjectUtility.ContainsStatic(Selection.gameObjects));
}

protected bool
IsDisabledByPrefabPropertyPatching(string partialPropertyName,
                                   out GUIContent disabledLabel) {
	...
	return prefabStage.ContainsTransformPrefabPropertyPatchingFor(
		Selection.gameObjects, partialPropertyName);
}
}
...
public static void SetMinDragDifferenceForPos(Vector3 position) {
	minDragDifference =
		Vector3.one * (HandleUtility.GetHandleSize(position) / 80f);
}

public static void SetMinDragDifferenceForPos(Vector3 position,
                                              float multiplier) {
	minDragDifference = Vector3.one * (HandleUtility.GetHandleSize(position) *
	                                   multiplier / 80f);
}
...
public static void
DisableMinDragDifferenceBasedOnSnapping(Vector3 positionBeforeSnapping,
...
public static void DetectDraggingBasedOnMouseDownUp(string handleName,
                                                    EventType typeBefore) {
	if (typeBefore == EventType.MouseDrag &&
	    Event.current.type != EventType.MouseDrag)
		BeginDragging(handleName);
	else if (typeBefore == EventType.MouseUp &&
...
public override GUIContent toolbarIcon {
	get {
		return EditorGUIUtility.TrTextContentWithIcon(
			"Transform Tool", "Transform Tool", "TransformTool");
	}
}

protected override bool
ShouldToolGUIBeDisabled(out GUIContent disabledLabel) {
	if (IsDisabledByPrefabPropertyPatching("m_LocalPosition",
	                                       out disabledLabel))
		return true;

	if (IsDisabledByPrefabPropertyPatching("m_LocalRotation",
	                                       out disabledLabel))
		return true;
...
protected override void ToolGUI(SceneView view, Vector3 handlePosition,
                                bool isStatic) {
	...
	Handles.TransformHandle(ids, ref endPosition, ref endRotation, ref endScale,
	                        Handles.TransformHandleParam.Default);
	s_Scale = endScale;
	...
	if (EditorGUI.EndChangeCheck() && !isStatic) {
		...
		if (TransformManipulator.HandleHasMoved(endPosition)) {
			...
			TransformManipulator.SetPositionDelta(
				endPosition, TransformManipulator.mouseDownHandlePosition);
		}

		Quaternion deltaRotation =
			Quaternion.Inverse(startRotation) * endRotation;
		float angle;
		Vector3 axis;
		...
		if (!Mathf.Approximately(angle, 0)) {
			foreach (Transform t in Selection.transforms) {
				...
				// sync euler hints after a rotate tool update tyo fake continuous
				// rotation
				t.SetLocalEulerHint(t.GetLocalEulerAngles(t.rotationOrder));

				if (t.parent != null)
					t.SendTransformChangedScale(); // force scale update, needed if tr
				                                       // has non-uniformly scaled parent.
			}
...
public override GUIContent toolbarIcon {
	get {
		return EditorGUIUtility.TrTextContentWithIcon("Move Tool", "Move Tool",
		                                              "MoveTool");
	}
}
...
protected override void ToolGUI(SceneView view, Vector3 handlePosition,
	...
	Vector3 pos2 = Handles.PositionHandle(
		handlePosition, TransformManipulator.mouseDownHandleRotation);

	if (EditorGUI.EndChangeCheck() && !isStatic &&
	    TransformManipulator.HandleHasMoved(pos2)) {
		ManipulationToolUtility.SetMinDragDifferenceForPos(handlePosition);
		...
		TransformManipulator.SetPositionDelta(
			pos2, TransformManipulator.mouseDownHandlePosition);
	}
...
public override GUIContent toolbarIcon {
	get {
		return EditorGUIUtility.TrTextContentWithIcon(
			"Rotate Tool", "Rotate Tool", "RotateTool");
	}
}
...
protected override void ToolGUI(SceneView view, Vector3 handlePosition,
	...
	if (EditorGUI.EndChangeCheck() && !isStatic) {
		...
		foreach (Transform t in Selection.transforms) {
			...
			if (t.parent != null)
				t.SendTransformChangedScale(); // force scale update, needed if tr has
...
public override GUIContent toolbarIcon {
	get {
		return EditorGUIUtility.TrTextContentWithIcon("Scale Tool", "Scale Tool",
		                                              "ScaleTool");
	}
}
...
protected override void ToolGUI(SceneView view, Vector3 handlePosition,
                                bool isStatic) {
	// Allow global space scaling for multi-selection but not for a single
	// object
	Quaternion handleRotation = Selection.transforms.Length > 1
	                            ? Tools.handleRotation
	                            : Tools.handleLocalRotation;
	...
	s_CurrentScale =
		Handles.ScaleHandle(s_CurrentScale, handlePosition, handleRotation,
...
public override GUIContent toolbarIcon {
	get {
		return EditorGUIUtility.TrTextContentWithIcon("Rect Tool", "Rect Tool",
		                                              "RectTool");
	}
}
...
protected override void ToolGUI(SceneView view, Vector3 handlePosition,
	...
	if (Camera.current) {
		Vector3 viewDir =
			Camera.current.orthographic ? Camera.current.transform.forward
	  : (handlePosition + rectRotation * rect.center -
		...
		float visibleSize =
			Mathf.Sqrt(Vector3
			           .Cross(Vector3.ProjectOnPlane(rectRight, viewDir),
			                  Vector3.ProjectOnPlane(rectUp, viewDir))
			           .magnitude);
		visibleSize /= HandleUtility.GetHandleSize(handlePosition);
		faded =
			Mathf.Clamp01((visibleSize - kMinVisibleSize) / kMinVisibleSize * 2);
	...
	if (!Tools.vertexDragging) {
		...
		bool rectTransformPivot = !groupPivot &&
			...
			if (EditorGUI.EndChangeCheck() && !isStatic) {
				if (groupPivot) {
				  ...
				  : tr;
					...
					RectTransformEditor.SetPivotSmart(rectTransform, pivot.x, 0, true,
					                                  space != rectTransform.transform);
					RectTransformEditor.SetPivotSmart(rectTransform, pivot.y, 1, true,
					                                  space != rectTransform.transform);
				}
			}
		}
	...
	if (!Tools.vertexDragging) {
		...
		Vector3 scale =
			ResizeHandlesGUI(rect, handlePosition, rectRotation, out scalePivot);
		if (EditorGUI.EndChangeCheck() && !isStatic) {
		...
		if (enableRotation) {
			...
			EditorGUI.BeginChangeCheck();
			Quaternion after =
				RotationHandlesGUI(rect, handlePosition, rectRotation);
			if (EditorGUI.EndChangeCheck() && !isStatic) {
				...
				foreach (Transform t in Selection.transforms) {
					...
					if (t.parent != null)
						t.SendTransformChangedScale(); // force scale update, needed if
					                                       // transform has non-uniformly
					                                       // scaled parent.
				}
				Tools.handleRotation =
					Quaternion.AngleAxis(angle, axis) * Tools.handleRotation;
			}
		}
	}
	TransformManipulator.EndManipulationHandling();
	...
	if (EditorGUI.EndChangeCheck() && !isStatic) {
		...
		if (TransformManipulator.HandleHasMoved(newPos))
			TransformManipulator.SetPositionDelta(
				newPos, TransformManipulator.mouseDownHandlePosition);
	}
	TransformManipulator.EndManipulationHandling();
...
private static Vector3 GetRectPointInWorld(Rect rect, Vector3 pivot,
                                           Quaternion rotation, int xHandle,
                                           int yHandle) {
	Vector3 pos =
		new Vector2(pos.x = Mathf.Lerp(rect.xMin, rect.xMax, xHandle * 0.5f),
...
static Vector3 ResizeHandlesGUI(Rect rect, Vector3 pivot, Quaternion rotation,
	...
	for (int xHandle = 0; xHandle <= 2; xHandle++) {
		for (int yHandle = 0; yHandle <= 2; yHandle++) {
			...
			Vector3 origPos =
				GetRectPointInWorld(s_StartRect, pivot, rotation, xHandle, yHandle);
			Vector3 curPos =
				GetRectPointInWorld(rect, pivot, rotation, xHandle, yHandle);

			float size = 0.05f * HandleUtility.GetHandleSize(curPos);
			int id =
				GUIUtility.GetControlID(s_ResizeHandlesHash, FocusType.Passive);
			if (GUI.color.a > 0 || GUIUtility.hotControl == id) {
				...
				if (xHandle == 1 || yHandle == 1) {
					// Side resizer (1D)
					Vector3 sideDir =
						(xHandle == 1 ? rotation * Vector3.right * rect.width
			      : rotation * Vector3.up * rect.height);
					Vector3 slideDir = (xHandle == 1 ? rotation * Vector3.up
					     : rotation * Vector3.right);
					newPos = RectHandles.SideSlider(id, curPos, sideDir, slideDir, size,
					                                null, EditorSnapSettings.move);
				} else {
					// Corner handle (2D)
					Vector3 outwardsA = rotation * Vector3.right * (xHandle - 1);
					Vector3 outwardsB = rotation * Vector3.up * (yHandle - 1);
					newPos = RectHandles.CornerSlider(
						id, curPos, rotation * Vector3.forward, outwardsA, outwardsB,
						size, RectHandles.RectScalingHandleCap,
						EditorSnapSettings.move);
				}

				// Calculate snapping values if applicable
				bool supportsRectSnapping =
					Selection.transforms.Length == 1 &&
					UnityEditorInternal.InternalEditorUtility.SupportsRectLayout(
				...
				if (supportsRectSnapping) {
					...
					if (typeBefore == EventType.MouseDown &&
					    Event.current.type != EventType.MouseDown)
						RectTransformSnapping.CalculateOffsetSnapValues(
							transformParent, transform, rectTransformParent,
							rectTransform, xHandle, yHandle);
				}

				if (EditorGUI.EndChangeCheck()) {
					// Resize handles require more fine grained rounding of values than
					// other tools. With other tools, the slight rounding is not
					// notizable as long as it's just sub-pixel, because the manipulated
					// object is being moved at the same time. However, with resize
					// handles, when dragging one edge or corner, the opposite is
					// standing still, and even slight rounding can cause
					// shaking/vibration. At a fraction of the normal rounding, the
					// shaking is very unlikely to happen though.
					...
					if (supportsRectSnapping) {
						...
						Vector2 snapSize = Vector2.one *
						                   HandleUtility.GetHandleSize(newPos) *
						                   RectTransformSnapping.kSnapThreshold;
						snapSize.x /= (inverseRotation *
						               transformParent.TransformVector(Vector3.right))
						              .x;
						snapSize.y /= (inverseRotation *
						               transformParent.TransformVector(Vector3.up))
						              .y;

						Vector3 newPosInParent =
							transformParent.InverseTransformPoint(newPos) -
							(Vector3) rectParent.rect.min;
						Vector3 newPosInParentSnapped =
							(Vector3) RectTransformSnapping.SnapToGuides(newPosInParent,
							                                             snapSize) +
							Vector3.forward * newPosInParent.z;
						ManipulationToolUtility.DisableMinDragDifferenceBasedOnSnapping(
							newPosInParent, newPosInParentSnapped);
						newPos = transformParent.TransformPoint(
							newPosInParentSnapped + (Vector3) rectParent.rect.min);
					}
					...
					if (!scaleFromPivot)
						scalePivot = GetRectPointInWorld(s_StartRect, pivot, rotation,
						                                 2 - xHandle, 2 - yHandle);

					if (uniformScaling)
						newPos =
							Vector3.Project(newPos - scalePivot, origPos - scalePivot) +
							scalePivot;
				...
				if (xHandle == 0)
					ManipulationToolUtility.DetectDraggingBasedOnMouseDownUp(
						kChangingLeft, typeBefore);
				if (xHandle == 2)
					ManipulationToolUtility.DetectDraggingBasedOnMouseDownUp(
						kChangingRight, typeBefore);
				if (xHandle != 1)
					ManipulationToolUtility.DetectDraggingBasedOnMouseDownUp(
						kChangingWidth, typeBefore);
				if (yHandle == 0)
					ManipulationToolUtility.DetectDraggingBasedOnMouseDownUp(
						kChangingBottom, typeBefore);
				if (yHandle == 2)
					ManipulationToolUtility.DetectDraggingBasedOnMouseDownUp(
						kChangingTop, typeBefore);
				if (yHandle != 1)
					ManipulationToolUtility.DetectDraggingBasedOnMouseDownUp(
						kChangingHeight, typeBefore);
			}
		}
	}
...
static Vector3 MoveHandlesGUI(Rect rect, Vector3 pivot, Quaternion rotation) {
	...
	case EventType.MouseDown: {
		...
		if (Tools.vertexDragging) {
			...
			acceptClick =
				evt.button == 0 && evt.modifiers == 0 &&
				RectHandles.RaycastGUIPointToWorldHit(evt.mousePosition, guiPlane,
				                                      out s_StartMouseWorldPos) &&
				(SceneViewDistanceToRectangle(corners, evt.mousePosition) == 0f ||
				 (discOpacity > 0 &&
				  SceneViewDistanceToDisc(pivot, rotation * Vector3.forward,
				                          discSize, evt.mousePosition) == 0f));
		}
		...
		if (acceptClick) {
			...
			if (supportsRectSnapping) {
				...
				RectTransformSnapping.CalculatePositionSnapValues(
					transformParent, transform, rectTransformParent, rectTransform);
			}
		}
		break;
	}
	case EventType.MouseDrag: {
		if (GUIUtility.hotControl == id) {
			...
			if (!s_Moving && (s_CurrentMousePos - s_StartMousePos).magnitude > 3f) {
				...
				RectHandles.RaycastGUIPointToWorldHit(s_CurrentMousePos, guiPlane,
				                                      out s_StartMouseWorldPos);
			}
			if (s_Moving) {
				if (Tools.vertexDragging) {
					...
					Vector3 near;
					if (HandleUtility.FindNearestVertex(s_CurrentMousePos, null,
					                                    out near)) {
					...
					Vector3 pos;
					if (RectHandles.RaycastGUIPointToWorldHit(s_CurrentMousePos,
					                                          guiPlane, out pos)) {
						...
						if (evt.shift) {
							...
							if (s_LockAxis == -1)
								s_LockAxis =
									Mathf.Abs(offset.x) > Mathf.Abs(offset.y) ? 0 : 1;
						...
						if (supportsRectSnapping) {
							...
							Vector3 rectPosition =
								s_StartRectPosition +
								transformParent.InverseTransformVector(offset);
							rectPosition.z = 0;
							...
							Vector3 newRectPosition =
								RectTransformSnapping.SnapToGuides(rectPosition, snapSize);
							ManipulationToolUtility.DisableMinDragDifferenceBasedOnSnapping(
								rectPosition, newRectPosition);
							offset = transformParent.TransformVector(newRectPosition -
							                                         s_StartRectPosition);
						}
		...
		if (GUIUtility.hotControl == id) {
			if (!s_Moving)
				Selection.activeGameObject =
					SceneViewPicking.PickGameObject(evt.mousePosition);
	...
	case EventType.Repaint: {
		if (Tools.vertexDragging) {
			RectHandles.RectScalingHandleCap(id, pivot, rotation, 1,
			                                 EventType.Repaint);
		} else {
			Handles.color =
				Handles.secondaryColor * new Color(1, 1, 1, 1.5f * discOpacity);
			Handles.CircleHandleCap(id, pivot, rotation, discSize,
			                        EventType.Repaint);
			Handles.color =
				Handles.secondaryColor * new Color(1, 1, 1, 0.3f * discOpacity);
	...
	ManipulationToolUtility.DetectDraggingBasedOnMouseDownUp(kChangingPosX,
	                                                         eventType);
	ManipulationToolUtility.DetectDraggingBasedOnMouseDownUp(kChangingLeft,
	                                                         eventType);
	ManipulationToolUtility.DetectDraggingBasedOnMouseDownUp(kChangingRight,
	                                                         eventType);
	ManipulationToolUtility.DetectDraggingBasedOnMouseDownUp(kChangingPosY,
	                                                         eventType);
	ManipulationToolUtility.DetectDraggingBasedOnMouseDownUp(kChangingTop,
	                                                         eventType);
	ManipulationToolUtility.DetectDraggingBasedOnMouseDownUp(kChangingBottom,
	                                                         eventType);

	return newPos;
}

static float SceneViewDistanceToDisc(Vector3 center, Vector3 normal,
                                     float radius, Vector2 mousePos) {
...
// Determine the distance from the mouse position to the world rectangle
// specified by the 4 points.
static float SceneViewDistanceToRectangle(Vector3[] worldPoints,
                                          Vector2 mousePos) {
...
static float DistanceToRectangle(Vector2[] screenPoints, Vector2 mousePos) {
	...
	for (int i = 0; i < 5; i++) {
		...
		if ((v0.y > mousePos.y) != (v1.y > mousePos.y)) {
			if (mousePos.x <
			    (v1.x - v0.x) * (mousePos.y - v0.y) / (v1.y - v0.y) + v0.x) {
				oddNodes = !oddNodes;
			}
		}
		j = i;
...
static Quaternion RotationHandlesGUI(Rect rect, Vector3 pivot,
                                     Quaternion rotation) {
	...
	for (int xHandle = 0; xHandle <= 2; xHandle += 2) {
		for (int yHandle = 0; yHandle <= 2; yHandle += 2) {
			...
			float size = 0.05f * HandleUtility.GetHandleSize(curPos);
			int id =
				GUIUtility.GetControlID(s_RotationHandlesHash, FocusType.Passive);
			if (GUI.color.a > 0 || GUIUtility.hotControl == id) {
				EditorGUI.BeginChangeCheck();
				Vector3 outwardsA = rotation * Vector3.right * (xHandle - 1);
				Vector3 outwardsB = rotation * Vector3.up * (yHandle - 1);
				float angle = RectHandles.RotationSlider(
					id, curPos, euler.z, pivot, rotation * Vector3.forward, outwardsA,
					outwardsB, size, null, Vector2.zero);
				if (EditorGUI.EndChangeCheck()) {
...
static Vector3 PivotHandleGUI(Rect rect, Vector3 pivot, Quaternion rotation) {
	...
	if (GUI.color.a > 0 || GUIUtility.hotControl == id) {
		...
		Vector3 newPivot = Handles.Slider2D(
			id, pivot, rotation * Vector3.forward, rotation * Vector3.right,
			rotation * Vector3.up, HandleUtility.GetHandleSize(pivot) * 0.1f,
			RectHandles.PivotHandleCap, Vector2.zero);
		...
		if (EditorGUI.EndChangeCheck()) {
			...
			Vector2 pivotCoordBefore =
				new Vector2(-rect.x / rect.width, -rect.y / rect.height);
			Vector2 pivotCoordAfter = pivotCoordBefore + offset;

			Vector2 snapSize = HandleUtility.GetHandleSize(pivot) *
			                   RectTransformSnapping.kSnapThreshold *
			                   new Vector2(1 / rect.width, 1 / rect.height);
			pivotCoordAfter =
				RectTransformSnapping.SnapToGuides(pivotCoordAfter, snapSize);
	...
	ManipulationToolUtility.DetectDraggingBasedOnMouseDownUp(kChangingPivot,
	                                                         eventType);
