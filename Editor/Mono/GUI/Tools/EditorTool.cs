...
namespace UnityEditor.EditorTools {
  ...
  public interface IDrawSelectedHandles { void OnDrawHandles(); }
  ...
  public abstract class EditorTool : ScriptableObject {
    ...
    public IEnumerable<UnityObject> targets {
      get {
        // CustomEditor tools get m_Targets populated on instantiation.
        // Persistent tools always reflect the current selection, and therefore
        // do not have m_Targets set. When a tool is deserialized, the m_Targets
        // array is initialized but not populated, hence the `> 0` check (valid
        // because it is not possible to create a custom editor tool instance
        // with no targets)
