...
namespace UnityEditor.EditorTools {
  public static class EditorTools {
    ...
    public static void SetActiveTool(Type type) {
      if (!typeof(EditorTool).IsAssignableFrom(type) || type.IsAbstract)
        throw new ArgumentException(
            "Type must be assignable to EditorTool, and not abstract.");
      ...
      if (attrib?.targetType != null) {
        ...
        if (tool == null)
          throw new InvalidOperationException(
              "The current selection does not contain any objects editable by the CustomEditor tool \"" +
              type + ".\"");
    ...
    public static void RestorePreviousPersistentTool() {
      var last = EditorToolContext.GetLastTool(
          x => x && !EditorToolUtility.IsCustomEditorTool(x.GetType()));
