...
internal class TransformManipulator {
  private struct TransformData {
    ...
    private Quaternion GetRefAlignment(Quaternion targetRotation,
      ...
      for (int i = 0; i < s_Alignments.Length; i++) {
        float dot = Mathf.Min(
            Mathf.Abs(
                Vector3.Dot(targetRotation * Vector3.right,
                            ownRotation * s_Alignments[i] * Vector3.right)),
            Mathf.Abs(Vector3.Dot(targetRotation * Vector3.up,
                                  ownRotation * s_Alignments[i] * Vector3.up)),
            Mathf.Abs(
                Vector3.Dot(targetRotation * Vector3.forward,
                            ownRotation * s_Alignments[i] * Vector3.forward)));
        if (dot > biggestDot) {
          biggestDot = dot;
    ...
    private void SetScaleValue(Vector3 scale) { transform.localScale = scale; }

    public void SetScaleDelta(Vector3 scaleDelta, Vector3 scalePivot,
                              Quaternion scaleRotation, bool preferRectResize) {
      SetPosition(scaleRotation *
                      Vector3.Scale(Quaternion.Inverse(scaleRotation) *
                                        (position - scalePivot),
                                    scaleDelta) +
                  scalePivot);
      ...
      if (preferRectResize) {
        if (rectTransform != null) {
          Vector2 newSizeDelta =
              sizeDelta + Vector2.Scale(rect.size, scaleDelta) - rect.size;
          if (applySmartRounding) {
            newSizeDelta.x = MathUtils.RoundBasedOnMinimumDifference(
                newSizeDelta.x, minDifference.x);
            newSizeDelta.y = MathUtils.RoundBasedOnMinimumDifference(
                newSizeDelta.y, minDifference.y);
          }
      ...
      if (applySmartRounding) {
        scaleDelta.x = MathUtils.RoundBasedOnMinimumDifference(scaleDelta.x,
                                                               minDifference.x);
        scaleDelta.y = MathUtils.RoundBasedOnMinimumDifference(scaleDelta.y,
                                                               minDifference.y);
        scaleDelta.z = MathUtils.RoundBasedOnMinimumDifference(scaleDelta.z,
                                                               minDifference.z);
      }
    ...
    public void SetPositionDelta(Vector3 positionDelta,
                                 bool applySmartRounding) {
      Vector3 localPositionDelta = positionDelta;
      if (transform.parent != null) {
        localPositionDelta =
            transform.parent.InverseTransformVector(localPositionDelta);

        if (!applySmartRounding)
          applySmartRounding =
              !transform.parent.localRotation.Equals(Quaternion.identity);
      }

      // If we are snapping, disable the smart rounding. If not the case, the
      // transform will have the wrong snap value based on distance to screen.
      applySmartRounding &= !(EditorSnapSettings.incrementalSnapActive ||
                              EditorSnapSettings.gridSnapActive ||
                              EditorSnapSettings.vertexSnapActive);
      ...
      Vector3 minDifference = ManipulationToolUtility.minDragDifference;
      if (applySmartRounding) {
        // For zero delta, we don't want to change the value so we ignore
        // rounding
      ...
      if (rectTransform == null) {
        ...
        if (applySmartRounding) {
          newLocalPosition.x = zeroXDelta ? localPosition.x
              : MathUtils.RoundBasedOnMinimumDifference(newLocalPosition.x,
                                                        minDifference.x);
          newLocalPosition.y = zeroYDelta ? localPosition.y
              : MathUtils.RoundBasedOnMinimumDifference(newLocalPosition.y,
                                                        minDifference.y);
          newLocalPosition.z = zeroZDelta ? localPosition.z
              : MathUtils.RoundBasedOnMinimumDifference(newLocalPosition.z,
                                                        minDifference.z);
        }
        ...
        Vector2 newAnchoredPosition =
            anchoredPosition + (Vector2) localPositionDelta;

        if (applySmartRounding) {
          newAnchoredPosition.x = zeroXDelta ? anchoredPosition.x
              : MathUtils.RoundBasedOnMinimumDifference(newAnchoredPosition.x,
                                                        minDifference.x);
          newAnchoredPosition.y = zeroYDelta ? anchoredPosition.y
              : MathUtils.RoundBasedOnMinimumDifference(newAnchoredPosition.y,
                                                        minDifference.y);
        }
  ...
  public static Vector3 mouseDownHandlePosition {
    get { return s_StartHandlePosition; }
  }
  public static Quaternion mouseDownHandleRotation {
    get { return s_StartHandleRotation; }
    set { s_StartHandleRotation = value; }
  }
  ...
  public static bool active {
    get { return s_MouseDownState != null; }
  }
  ...
  private static void BeginEventCheck() {
    ...
    if (!active || (previousEvent != EventType.MouseDown &&
                    s_EventTypeBefore == EventType.MouseDown)) {
  ...
  public static EventType EndManipulationHandling() {
    ...
    } else if (s_MouseDownState != null &&
               (usedEvent == EventType.MouseUp ||
                GUIUtility.hotControl != s_HotControl)) {
      s_StartHandleRotation = Tools.handleRotation;
  ...
  private static void SetLocalHandleOffsetScaleDelta(Vector3 scaleDelta,
                                                     Quaternion pivotRotation) {
    Quaternion refAlignment =
        Quaternion.Inverse(Tools.handleRotation) * pivotRotation;
    Tools.localHandleOffset = Vector3.Scale(
        Vector3.Scale(s_StartLocalHandleOffset, refAlignment * scaleDelta),
        refAlignment * Vector3.one);
  }

  public static void SetScaleDelta(Vector3 scaleDelta,
                                   Quaternion pivotRotation) {
    ...
    for (int i = 0; i < s_MouseDownState.Length; i++) {
      ...
      s_MouseDownState [i]
          .SetScaleDelta(scaleDelta, point, pivotRotation, false);
    }
  }

  public static void SetResizeDelta(Vector3 scaleDelta, Vector3 pivotPosition,
                                    Quaternion pivotRotation) {
    if (s_MouseDownState == null)
      return;
    ...
    for (int i = 0; i < s_MouseDownState.Length; i++) {
      ...
      if (cur.rectTransform != null)
        ...
        if (sr != null && sr.drawMode != SpriteDrawMode.Simple)
          Undo.RecordObjects(new Object[]{(Object) sr, (Object) cur.transform},
                             "Resize");
        else
    ...
    for (int i = 0; i < s_MouseDownState.Length; i++) {
      s_MouseDownState [i]
          .SetScaleDelta(scaleDelta, pivotPosition, pivotRotation, true);
    }
  }

  public static void SetPositionDelta(Vector3 newPosition,
                                      Vector3 oldPosition) {
    if (s_MouseDownState == null)
      return;
    ...
    for (int i = 0; i < s_MouseDownState.Length; i++) {
      var cur = s_MouseDownState[i];
      undoObjects[i] = (cur.rectTransform != null ?(Object) cur.rectTransform
    ...
    if (s_MouseDownState.Length > 0) {
      s_MouseDownState [0]
          .SetPositionDelta(positionDelta, true);
      Vector3 firstDelta =
          s_MouseDownState[0].transform.position - s_MouseDownState[0].position;

      for (int i = 1; i < s_MouseDownState.Length; i++) {
        s_MouseDownState [i]
            .SetPositionDelta(firstDelta, false);
      }
    }
  }
