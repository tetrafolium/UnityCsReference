...
public sealed partial class EditorGUILayout {
  public static bool BeginFoldoutHeaderGroup(
      bool foldout, string content,
      [ DefaultValue("EditorStyles.foldoutHeader") ] GUIStyle style = null,
      Action<Rect> menuAction = null, GUIStyle menuIcon = null) {
    return BeginFoldoutHeaderGroup(foldout, GUIContent.Temp(content), style,
                                   menuAction, menuIcon);
  }

  public static bool BeginFoldoutHeaderGroup(
      bool foldout, GUIContent content,
      [ DefaultValue("EditorStyles.foldoutHeader") ] GUIStyle style = null,
      Action<Rect> menuAction = null, GUIStyle menuIcon = null) {
    if (style == null)
      style = EditorStyles.foldoutHeader;
    var backgroundRect = GUILayoutUtility.GetRect(content, style);
    return EditorGUI.BeginFoldoutHeaderGroup(backgroundRect, foldout, content,
                                             style, menuAction, menuIcon);
  }
...
public sealed partial class EditorGUI {
  ...
  private static readonly int s_FoldoutHeaderHash =
      "FoldoutHeader".GetHashCode();

  public static bool BeginFoldoutHeaderGroup(
      Rect position, bool foldout, string content,
      [ DefaultValue("EditorStyles.foldoutHeader") ] GUIStyle style = null,
      Action<Rect> menuAction = null, GUIStyle menuIcon = null) {
    return BeginFoldoutHeaderGroup(position, foldout, GUIContent.Temp(content),
                                   style, menuAction, menuIcon);
  }

  public static bool BeginFoldoutHeaderGroup(
      Rect position, bool foldout, GUIContent content,
      [ DefaultValue("EditorStyles.foldoutHeader") ] GUIStyle style = null,
      Action<Rect> menuAction = null, GUIStyle menuIcon = null) {
    ...
    Rect menuRect = new Rect{x = position.xMax - style.padding.right - iconSize,
                             y = position.y + style.padding.top,
                             size = Vector2.one * iconSize};

    bool menuIconHover = menuRect.Contains(Event.current.mousePosition);
    bool menuIconActive =
        (menuIconHover && Event.current.type == EventType.MouseDown &&
         Event.current.button == 0);
    if (menuAction != null && menuIconActive) {
      menuAction.Invoke(menuRect);
      Event.current.Use();
    }
    int id = GUIUtility.GetControlID(s_FoldoutHeaderHash, FocusType.Keyboard,
                                     position);
    ...
    if (Event.current.type == EventType.KeyDown &&
      ...
      foldout = EditorGUIInternal.DoToggleForward(position, id, foldout,
                                                  content, style);
    }
