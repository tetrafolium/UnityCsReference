...
internal static float AngularDial(GUIContent label, float angle,
                                  Texture thumbTexture, GUIStyle background,
                                  GUIStyle thumb,
	...
	var rect = GUILayoutUtility.GetRect(minWidth, maxWidth, height, height,
	                                    background, options);
	s_LastRect = rect;
	return EditorGUI.AngularDial(rect, label, angle, thumbTexture, background,
	                             thumb);
}
}

public sealed partial class EditorGUI {
internal static float AngularDial(Rect rect, GUIContent label, float angle,
                                  Texture thumbTexture, GUIStyle background,
                                  GUIStyle thumb) {
	...
	var thumbDimensions =
		thumb == null || thumb == GUIStyle.none
		              ? Vector2.zero
	...
	case EventType.Repaint:
		...
		if (thumb != null && thumb != GUIStyle.none) {
			...
			var thumbCenter = new Vector2(Mathf.Cos(radians), Mathf.Sin(radians)) *
			                  thumbCenterRadius +
			                  rect.center;
			...
			var thumbRect = new Rect {
				size = size, center = thumbCenter
			};
			thumbRect.center = thumbCenter;

			thumb.Draw(thumbRect, thumbTexture,
			           thumbRect.Contains(evt.mousePosition), active, false, false);
		}
		break;
	}
...
private static float UseAngularDialEventAndGetAngle(int id, Event evt,
                                                    Vector2 center,
                                                    float angle) {
	...
	// negate angle since gui space goes top to bottom
	var newAngle = -Mathf.Rad2Deg * Mathf.Acos(normalized.x) *
