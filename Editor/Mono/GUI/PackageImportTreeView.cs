...
internal class PackageImportTreeView {
  ...
  List<PackageImportTreeViewItem> m_Selection =
      new List<PackageImportTreeViewItem>();
  static readonly bool s_UseFoldouts = true;
  public enum EnabledState { NotSet = -1, None = 0, All = 1, Mixed = 2 }

  private PackageImport m_PackageImport;

  public ImportPackageItem[] packageItems {
    get { return m_PackageImport.packageItems; }
  }

  public PackageImportTreeView(PackageImport packageImport,
                               TreeViewState treeViewState, Rect startRect) {
  ...
  void RecursiveComputeEnabledStateForFolders(
      PackageImportTreeViewItem pitem,
      HashSet<PackageImportTreeViewItem> done) {
    if (pitem.item != null && !pitem.item.isFolder)
      return;

    // Depth first recursion to allow parent folders be dependant on child
    // folders

    // Recurse
    if (pitem.hasChildren) {
      foreach (var child in pitem.children) {
        RecursiveComputeEnabledStateForFolders(
            child as PackageImportTreeViewItem, done);
      }
    }
  ...
  void SelectionChanged(int[] selectedIDs) {
    ...
    if (m_Selection.Count == 1 && selectedItem != null &&
        !string.IsNullOrEmpty(selectedItem.previewPath)) {
  ...
  public void OnGUI(Rect rect) {
    ...
    if (Event.current.type == EventType.KeyDown &&
        Event.current.keyCode == KeyCode.Space && m_Selection != null &&
        m_Selection.Count > 0 &&
        GUIUtility.keyboardControl == keyboardControlID) {
      var pitem = m_Selection[0];
      if (pitem != null) {
        EnabledState newEnabled = (pitem.enableState == EnabledState.None)
                                      ? EnabledState.All
                                      : EnabledState.None;
  ...
  private class PackageImportTreeViewItem : TreeViewItem {
    ...
    public PackageImportTreeViewItem(ImportPackageItem itemIn, int id,
  ...
  private class PackageImportTreeViewGUI : TreeViewGUI {
    internal static class Constants {
      ...
      public static GUIContent badgeNew = EditorGUIUtility.TrIconContent(
          "PackageBadgeNew", "This is a new Asset");
      public static GUIContent badgeDelete = EditorGUIUtility.TrIconContent(
          "PackageBadgeDelete", "These files will be deleted!");
      public static GUIContent badgeWarn = EditorGUIUtility.TrIconContent(
          "console.warnicon",
          "Warning: File exists in project, but with different GUID. Will override existing asset which may be undesired.");
      public static GUIContent badgeChange = EditorGUIUtility.TrIconContent(
          "playLoopOff", "This file is new or has changed.");
    ...
    public PackageImportTreeViewGUI(TreeViewController treeView,
                                    PackageImportTreeView view)
        : base(treeView) {
    ...
    override public void OnRowGUI(Rect rowRect, TreeViewItem tvItem, int row,
      ...
      Rect toggleRect =
          new Rect(k_BaseIndent + tvItem.depth * indentWidth + k_FoldoutWidth,
                   rowRect.y, k_ToggleWidth, rowRect.height);

      if ((isFolder && !projectAsset) ||
          (validItem && !projectAsset && assetChanged))
        DoToggle(pitem, toggleRect);

      using(new EditorGUI.DisabledScope(!validItem || projectAsset)) {
        // 3. Icon & Text
        Rect contentRect =
            new Rect(toggleRect.xMax, rowRect.y, rowRect.width, rowRect.height);
        ...
        if (repainting && validItem && pathConflict) {
          Rect labelRect = new Rect(rowRect.xMax - 58, rowRect.y,
                                    rowRect.height, rowRect.height);
          EditorGUIUtility.SetIconSize(
              new Vector2(rowRect.height, rowRect.height));
        ...
        if (repainting && validItem && !(exists || pathConflict)) {
          ...
          Rect labelRect =
              new Rect(rowRect.xMax - badge.width - 6,
                       rowRect.y + (rowRect.height - badge.height) / 2,
        ...
        if (repainting && validItem && (exists || pathConflict) &&
          ...
          Rect labelRect = new Rect(rowRect.xMax - badge.width - 6, rowRect.y,
                                    rowRect.height, rowRect.height);
          GUI.Label(labelRect, Constants.badgeChange,
                    Constants.paddinglessStyle);
        }
      }
    }

    static void Toggle(ImportPackageItem[] items,
                       PackageImportTreeViewItem pitem, Rect toggleRect) {
    ...
    void DoToggle(PackageImportTreeViewItem pitem, Rect toggleRect) {
      ...
      if (EditorGUI.EndChangeCheck()) {
        // Only change selection if we already have single selection (Keep
        // multi-selection when toggling)
        if (m_TreeView.GetSelection().Length <= 1 ||
            !m_TreeView.GetSelection().Contains(pitem.id)) {
    ...
    void DoPreviewPopup(PackageImportTreeViewItem pitem, Rect rowRect) {
      ...
      if (item != null) {
        ...
        if (Event.current.type == EventType.MouseDown &&
            ...
            !PopupWindowWithoutFocus.IsVisible())
          showPreviewForID = pitem.id;

        // Show preview
        if (pitem.id == showPreviewForID &&
          ...
          if (!string.IsNullOrEmpty(item.previewPath)) {
            ...
            PopupWindowWithoutFocus.Show(buttonRect, new PreviewPopup(preview),
                                         new[]{PopupLocation.Right,
                                               PopupLocation.Left,
                                               PopupLocation.Below});
          }
        }
    ...
    void DoIconAndText(TreeViewItem item, Rect contentRect, bool selected,
      ...
      lineStyle.padding.left =
          0; // padding could have been set by other tree views
      contentRect.height +=
          5; // with the default row height, underscore and lower parts of
             // characters like g, p, etc. were not visible
      if (Event.current.type == EventType.Repaint)
        lineStyle.Draw(contentRect,
                       GUIContent.Temp(item.displayName, GetIconForItem(item)),
    ...
    protected override Texture GetIconForItem(TreeViewItem tvItem) {
      ...
      // We are using this TreeViewGUI when importing and exporting a package,
      // so handle both situations:

      // Exporting a package can use cached icons (icons we generate on import)
      Texture cachedIcon =
          AssetDatabase.GetCachedIcon(item.destinationAssetPath);
      if (cachedIcon != null)
        return cachedIcon;
  ...
  private class PackageImportTreeViewDataSource : TreeViewDataSource {
    ...
    public PackageImportTreeViewDataSource(TreeViewController treeView,
                                           PackageImportTreeView view)
        : base(treeView) {
      m_PackageImportView = view;
    ...
    public override void FetchData() {
      ...
      m_RootItem =
          new PackageImportTreeViewItem(null, "Assets".GetHashCode(), rootDepth,
                                        null, "InvisibleAssetsFolder");
      ...
      Dictionary<string, TreeViewItem> treeViewFolders =
          new Dictionary<string, TreeViewItem>();
      for (int i = 0; i < items.Length; i++) {
        var item = items[i];

        if (PackageImport.HasInvalidCharInFilePath(item.destinationAssetPath))
          continue; // Do not add invalid paths (we already warn the user with a
                    // dialog in PackageImport.cs)
        ...
        // Ensure folders. This is for when installed packages have been moved
        // to other folders.
        ...
        if (targetFolder == null) {
          targetFolder =
              EnsureFolderPath(folderPath, treeViewFolders, initExpandedState);
        }

        // Add file to folder
        if (targetFolder != null) {
          int id = item.destinationAssetPath.GetHashCode();
          var newItem = new PackageImportTreeViewItem(
              item, id, targetFolder.depth + 1, targetFolder, filename);
    ...
    TreeViewItem
    EnsureFolderPath(string folderPath,
                     Dictionary<string, TreeViewItem> treeViewFolders,
                     bool initExpandedState) {
      ...
      int folderDepth =
          -1; // Will be incremented to the right depth in the loop.
      ...
      for (int depth = 0; depth < splitPath.Length; ++depth) {
        ...
        // Only increment the folder depth if we are past the root "Assets"
        // folder.
        ++folderDepth;
        ...
        if (treeViewFolders.TryGetValue(currentPath, out foundItem)) {
          ...
          var folderItem = new PackageImportTreeViewItem(null, id, folderDepth,
                                                         currentItem, folder);
  ...
  class PreviewPopup : PopupWindowContent {
    ...
    public PreviewPopup(Texture2D preview) { m_Preview = preview; }

    public override void OnGUI(Rect rect) {
      PackageImport.DrawTexture(rect, m_Preview, false);
    }

    public override Vector2 GetWindowSize() { return kPreviewSize; }
  }
}
}
