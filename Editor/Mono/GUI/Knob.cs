...
public sealed partial class EditorGUILayout {
  public static float Knob(Vector2 knobSize, float value, float minValue,
                           float maxValue, string unit, Color backgroundColor,
                           Color activeColor, bool showValue,
                           params GUILayoutOption[] options) {
    Rect r = s_LastRect = GetControlRect(false, knobSize.y, options);
    return EditorGUI.Knob(
        r, knobSize, value, minValue, maxValue, unit, backgroundColor,
...
public sealed partial class EditorGUI {
  internal struct KnobContext {
    ...
    public KnobContext(Rect position, Vector2 knobSize, float currentValue,
                       float start, float end, string unit,
    ...
    private EventType CurrentEventType() { return CurrentEvent().type; }

    private bool IsEmptyKnob() { return start == end; }

    private Event CurrentEvent() { return Event.current; }

    private float Clamp(float value) {
      return Mathf.Clamp(value, MinValue(), MaxValue());
    }

    private float ClampedCurrentValue() { return Clamp(currentValue); }

    private float MaxValue() { return Mathf.Max(start, end); }

    private float MinValue() { return Mathf.Min(start, end); }
    ...
    private float OnMouseDown() {
      ...
      if (!position.Contains(CurrentEvent().mousePosition) ||
          KnobState().isEditing || IsEmptyKnob())
        return currentValue;
    ...
    private void PrintValue() {
      Rect textRect =
          new Rect(position.x + knobSize.x / 2 - 8,
                   position.y + knobSize.y / 2 - 8, position.width, 20);
      string value = currentValue.ToString(
          "0.##", CultureInfo.InvariantCulture
                      .NumberFormat); // FIXME: This needs to be done so it can
                                      // handle any range types.
    ...
    private float DoKeyboardInput() {
      ...
      Rect inputRect =
          new Rect(position.x + knobSize.x / 2 - 6,
                   position.y + knobSize.y / 2 - 7, position.width, 20);
    ...
    static void CreateKnobMaterial() {
      if (!knobMaterial) {
        ...
        Shader shader = AssetDatabase.GetBuiltinExtraResource(
            typeof(Shader), "Internal-GUITextureClip.shader") as Shader;
    ...
    Vector3 GetUVForPoint(Vector3 point) {
      Vector3 uv = new Vector3((point.x - position.x) / knobSize.x,
                               (point.y - position.y - knobSize.y) /
                                   -knobSize.y // we need to flip uv over Y
    ...
    private void DrawValueArc(float angle) {
      ...
      Vector3 center = new Vector3(position.x + knobSize.x / 2,
                                   position.y + knobSize.y / 2, 0);
      Vector3 centerRight =
          new Vector3(position.x + knobSize.x, position.y + knobSize.y / 2, 0);
      Vector3 bottomRight =
          new Vector3(position.x + knobSize.x, position.y + knobSize.y, 0);
      ...
      if (angle > 0.0f) {
        ...
        if (angle == (Mathf.PI * (3.0f / 2.0f))) {
          ...
          if (angle < Mathf.PI / 2.0f)
            projection =
                bottomRight +
                new Vector3((knobSize.y / 2 *
                             Mathf.Tan(Mathf.PI / 2.0f - adjustedAngle)) -
                                knobSize.x / 2,
                            0, 0);
          else if (angle < Mathf.PI)
            projection =
                bottomleft + new Vector3(0,
                                         (knobSize.x / 2 *
                                          Mathf.Tan(Mathf.PI - adjustedAngle)) -
                                             knobSize.y / 2,
                                         0);
          else
            projection =
                topLeft + new Vector3(-(knobSize.y / 2 *
                                        Mathf.Tan((Mathf.PI * 3.0f / 2.0f) -
                                                  adjustedAngle)) +
                                          knobSize.x / 2,
                                      0, 0);
  ...
  internal static float Knob(Rect position, Vector2 knobSize,
                             float currentValue, float start, float end,
                             string unit, Color backgroundColor,
                             Color activeColor, bool showValue, int id) {
    return new KnobContext(position, knobSize, currentValue, start, end, unit,
                           backgroundColor, activeColor, showValue, id)
        .Handle();
  }

  internal static float OffsetKnob(Rect position, float currentValue,
                                   float start, float end, float median,
                                   string unit, Color backgroundColor,
                                   Color activeColor, GUIStyle knob, int id) {
    ///@TODO Implement
    return 0f;
  }
}
