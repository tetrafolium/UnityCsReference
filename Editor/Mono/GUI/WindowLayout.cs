...
internal static class WindowLayout {
  ...
  // Backward compatibility: name of the old (non mode specific) per project
  // layout
  internal const string kOldCurrentLayoutPath = "Library/CurrentLayout.dwlt";
  internal static string layoutsPreferencesPath => FileUtil.CombinePaths(
      InternalEditorUtility.unityPreferencesFolder, "Layouts");
  internal static string layoutsModePreferencesPath =>
      FileUtil.CombinePaths(layoutsPreferencesPath, ModeService.currentId);
  internal static string layoutsDefaultModePreferencesPath =>
      FileUtil.CombinePaths(layoutsPreferencesPath, "default");
  internal static string layoutsProjectPath =>
      Directory.GetCurrentDirectory() + "/Library";
  // Backward compatibility: property for old global layout (for default mode
  // only)
  internal static string OldGlobalLayoutPath =>
      Path.Combine(layoutsPreferencesPath, "__Current__.dwlt");
  internal static string ProjectLayoutPath =>
      GetProjectLayoutPerMode(ModeService.currentId);
  internal static string LastLayoutPath =>
      Path.Combine(layoutsModePreferencesPath, kLastLayoutName);
  ...
  public static void LoadCurrentModeLayout(bool keepMainWindow) {
    ...
    var layoutData =
        ModeService.GetModeDataSection(ModeDescriptor.LayoutKey) as JSONObject;
    if (layoutData == null)
      LoadProjectLayout(keepMainWindow);
    else {
      var projectLayoutExists = File.Exists(ProjectLayoutPath);
      if ((projectLayoutExists &&
           Convert.ToBoolean(layoutData["restore_saved_layout"])) ||
          !LoadModeDynamicLayout(keepMainWindow, layoutData))
        LoadProjectLayout(keepMainWindow);
    }
  }

  private static bool LoadModeDynamicLayout(bool keepMainWindow,
                                            JSONObject layoutData) {
    ...
    LayoutViewInfo topViewInfo =
        new LayoutViewInfo(k_TopViewClassName, MainView.kToolbarHeight, true);
    LayoutViewInfo bottomViewInfo = new LayoutViewInfo(
        k_BottomViewClassName, MainView.kStatusbarHeight, true);
    LayoutViewInfo centerViewInfo =
        new LayoutViewInfo(k_CenterViewClassName, 0, true);

    var availableEditorWindowTypes =
        TypeCache.GetTypesDerivedFrom<EditorWindow>().ToArray();

    if (!GetLayoutViewInfo(layoutData, availableEditorWindowTypes,
                           ref centerViewInfo))
      return false;

    GetLayoutViewInfo(layoutData, availableEditorWindowTypes, ref topViewInfo);
    GetLayoutViewInfo(layoutData, availableEditorWindowTypes,
                      ref bottomViewInfo);

    var mainWindow =
        GenerateLayout(keepMainWindow, availableEditorWindowTypes,
                       centerViewInfo, topViewInfo, bottomViewInfo);
    if (mainWindow)
      mainWindow.m_DontSaveToLayout =
          !Convert.ToBoolean(layoutData["restore_saved_layout"]);

    return mainWindow != null;
  }

  private static View LoadLayoutView<T>(Type[] availableEditorWindowTypes,
                                        LayoutViewInfo viewInfo, float width,
                                        float height) where T : View {
    if (!viewInfo.used)
      return null;
    View view = null;
    if (viewInfo.isContainer) {
      bool useTabs = viewInfo.extendedData.Contains("tabs") &&
                     Convert.ToBoolean(viewInfo.extendedData["tabs"]);
      bool useSplitter = viewInfo.extendedData.Contains("vertical") ||
                         viewInfo.extendedData.Contains("horizontal");
      bool isVertical = viewInfo.extendedData.Contains("vertical") &&
                        Convert.ToBoolean(viewInfo.extendedData["vertical"]);
      ...
      foreach (var childData in childrenData) {
        var lvi = new LayoutViewInfo(
            childIndex, useTabs ? 1f : 1f / childrenData.Count, true);
        ...
        if (useTabs) {
          (view as DockArea)
              .AddTab((EditorWindow) ScriptableObject.CreateInstance(lvi.type));
        } else {
          view.AddChild(LoadLayoutView<HostView>(availableEditorWindowTypes,
                                                 lvi, cw, ch));
      ...
      if (viewInfo.type != null) {
        ...
        hostView.SetActualViewInternal(
            ScriptableObject.CreateInstance(viewInfo.type) as EditorWindow,
            true);
        view = hostView;
      } else
  ...
  private static ContainerWindow
  GenerateLayout(bool keepMainWindow, Type[] availableEditorWindowTypes,
                 LayoutViewInfo center, LayoutViewInfo top,
                 LayoutViewInfo bottom) {
    ...
    if (keepMainWindow && mainContainerWindow == null) {
      Debug.LogWarning(
          $"No main window to restore layout from while loading dynamic layout for mode {ModeService.currentId}");
      return null;
    }
    ...
    try {
      ...
      if (!mainContainerWindow)
        mainContainerWindow =
            ScriptableObject.CreateInstance<ContainerWindow>();
      ...
      // Create center view
      View centerView = LoadLayoutView<DockArea>(availableEditorWindowTypes,
                                                 center, width, height);
      var topView = LoadLayoutView<Toolbar>(availableEditorWindowTypes, top,
                                            width, height);
      var bottomView = LoadLayoutView<AppStatusBar>(availableEditorWindowTypes,
                                                    bottom, width, height);
      ...
      if (bottomView) {
        bottomView.position =
            new Rect(0, height - bottom.size, width, bottom.size);
  ...
  private static bool GetLayoutViewInfo(JSONObject layoutData,
                                        Type[] availableEditorWindowTypes,
                                        ref LayoutViewInfo viewInfo) {
  ...
  private static bool ParseViewData(Type[] availableEditorWindowTypes,
                                    object viewData,
                                    ref LayoutViewInfo viewInfo) {
    if (viewData is string) {
      ...
      if (viewExpandedData.Contains("children") ||
          viewExpandedData.Contains("vertical") ||
          viewExpandedData.Contains("horizontal") ||
          viewExpandedData.Contains("tabs")) {
    ...
    if (viewInfo.type == null) {
      Debug.LogWarning(
          $"Invalid layout view {viewInfo.key} with type {viewInfo.className} for mode {ModeService.currentId}");
      return false;
    }
  ...
  private static void LoadProjectLayout(bool keepMainWindow) {
    ...
    LoadWindowLayout(ProjectLayoutPath, !projectLayoutExists, false,
                     keepMainWindow);
  }
  ...
  internal static void SaveCurrentLayoutPerMode(string modeId) {
    ...
    SaveWindowLayout(FileUtil.CombinePaths(Directory.GetCurrentDirectory(),
                                           GetProjectLayoutPerMode(modeId)));

    // Save Global Last Layout
    SaveWindowLayout(
        FileUtil.CombinePaths(layoutsPreferencesPath, modeId, kLastLayoutName));
  }
  ...
  private static void InitializeLayoutPreferencesFolder() {
    ...
    string layoutResourcesPath = Path.Combine(
        EditorApplication.applicationContentsPath, "Resources/Layouts");
    ...
    if (!Directory.Exists(layoutsModePreferencesPath)) {
      // Make sure we have a valid default mode folder initialized with the
      // proper default layouts.
      if (layoutsDefaultModePreferencesPath == layoutsModePreferencesPath) {
        // Backward compatibility: if the default layout folder doesn't exists
        // but some layouts have been saved be sure to copy them to the "default
        // layout per mode folder".
        FileUtil.CopyFileOrDirectory(layoutResourcesPath,
                                     layoutsDefaultModePreferencesPath);
        var defaultModeUserLayouts =
            Directory.GetFiles(layoutsPreferencesPath, "*.wlt");
  ...
  internal static void ReloadWindowLayoutMenu() {
    ...
    if (Directory.Exists(layoutsModePreferencesPath)) {
      var layoutPaths = Directory.GetFiles(layoutsModePreferencesPath)
                            .Where(path => path.EndsWith(".wlt"))
                            .ToArray();
      foreach (var layoutPath in layoutPaths) {
        var name = Path.GetFileNameWithoutExtension(layoutPath);
        Menu.AddMenuItem("Window/Layouts/" + name, "", false,
                         layoutMenuItemPriority++,
                         () => LoadWindowLayout(layoutPath, false), null);
      }
    ...
    // Get mode layouts
    var modeLayoutPaths = ModeService.GetModeDataSection(
        ModeService.currentIndex, ModeDescriptor.LayoutsKey) as IList<object>;
    ...
    Menu.AddMenuItem("Window/Layouts/Save Layout...", "", false,
                     layoutMenuItemPriority++, SaveGUI, null);
    Menu.AddMenuItem("Window/Layouts/Delete Layout...", "", false,
                     layoutMenuItemPriority++, DeleteGUI, null);
    Menu.AddMenuItem("Window/Layouts/Revert Factory Settings...", "", false,
                     layoutMenuItemPriority++,
                     () => RevertFactorySettings(false), null);

    Menu.AddMenuItem("Window/Layouts/More/Save to disk...", "", false, 998,
                     () => SaveToFile(), null);
    Menu.AddMenuItem("Window/Layouts/More/Load from disk...", "", false, 999,
                     () => LoadFromFile(), null);
  }
  ...
  internal static EditorWindow TryGetLastFocusedWindowInSameDock() {
    // Get type of window that was docked together with game view and was
    // focused before play mode
    Type type = null;
    string windowTypeName =
        WindowFocusState.instance.m_LastWindowTypeInSameDock;
    if (windowTypeName != "")
      type = Type.GetType(windowTypeName);

    // Also get the PlayModeView Window
    var playModeView = PlayModeView.GetMainPlayModeView();
    if (type != null && playModeView && playModeView != null &&
        playModeView.m_Parent is DockArea) {
  ...
  SaveCurrentFocusedWindowInSameDock(EditorWindow windowToBeFocused) {
    if (windowToBeFocused.m_Parent != null &&
        windowToBeFocused.m_Parent is DockArea) {
  ...
  internal static EditorWindow
  TryFocusAppropriateWindow(bool enteringPlaymode) {
    if (enteringPlaymode) {
      var playModeView = PlayModeView.GetMainPlayModeView();
      if (playModeView) {
      ...
      // If we can retrieve what window type was active when we went into play
      // mode, go back to focus a window of that type.
  ...
  internal static EditorWindow GetMaximizedWindow() {
    UnityObject[] maximized =
        Resources.FindObjectsOfTypeAll(typeof(MaximizedHostView));
  ...
  internal static EditorWindow
  ShowAppropriateViewOnEnterExitPlaymode(bool entering) {
    // Prevent trying to go into the same state as we're already in, as it will
    // break things
    ...
    if (entering) {
      ...
      // just keep that maximized view, no matter if it's the game view or some
      // other. Trust that user has a good reason (desire by Ethan etc.)
      if (maximized != null)
        return maximized;
    } else {
      // If a view was already maximized before entering play mode,
      // then it was kept when switching to play mode, and can simply still be
      // kept when exiting
    ...
    if (entering) {
      // Try to create and focus a Game View tab docked together with the Scene
      // View tab
  ...
  public static void Unmaximize(EditorWindow win) {
    ...
    UnityObject[] newWindows =
        InternalEditorUtility.LoadSerializedFileAndForget(
            Path.Combine(layoutsProjectPath, kMaximizeRestoreFile));
    ...
    if (oldRoot == null) {
      Debug.Log(
          "Maximization failed because the root split view was not found");
      RevertFactorySettings();
      return;
    }
    ...
    if (parentWindow == null) {
      Debug.Log(
          "Maximization failed because the root split view has no container window");
      RevertFactorySettings();
      return;
    }
    ...
    try {
      ...
      if (maximizedHostView.parent) {
        ...
        // If parent window had to be resized, call this to make sure new size
        // gets propagated
    ...
    try {
      // Weird bug on AMD graphic cards under OSX Lion: Sometimes when
      // unmaximizing we get stray white rectangles. work around that by issuing
      // an extra repaint (case 438764)
      if (Application.platform == RuntimePlatform.OSXEditor &&
          SystemInfo.operatingSystem.Contains("10.7") &&
          SystemInfo.graphicsDeviceVendor.Contains("ATI")) {
  ...
  internal static void MaximizeGestureHandler() {
    if (Event.current.type != EditorGUIUtility.magnifyGestureEventType ||
  ...
  public static void AddSplitViewAndChildrenRecurse(View splitview,
                                                    ArrayList list) {
    list.Add(splitview);
  ...
  public static void SaveSplitViewAndChildren(View splitview, EditorWindow win,
                                              string path) {
  ...
  public static bool MaximizePrepare(EditorWindow win) {
    ...
    SaveSplitViewAndChildren(
        rootSplit, win, Path.Combine(layoutsProjectPath, kMaximizeRestoreFile));

    // Remove the window from the HostView now in order to invoke
    // OnBecameInvisible before OnBecameVisible
    dockArea.actualView = null;

    dockArea.m_Panes[oldDockIndex] = null;

    MaximizedHostView maximizedHostView =
        ScriptableObject.CreateInstance<MaximizedHostView>();
    ...
    maximizedHostView.position =
        p; // Must be set after actualView so that value is propagated
  ...
  public static bool LoadWindowLayout(string path,
                                      ...
                                      bool keepMainWindow) {
    Console.WriteLine(
        $"[LAYOUT] About to load {path}, keepMainWindow={keepMainWindow}");

    bool mainWindowMaximized = false;
    Rect mainWindowPosition = new Rect();
    UnityObject[] containers =
        Resources.FindObjectsOfTypeAll(typeof(ContainerWindow));
    foreach (ContainerWindow window in containers) {
      if (window.showMode == ShowMode.MainWindow) {
    ...
    try {
      ...
      UnityObject[] remainingContainers =
          Resources.FindObjectsOfTypeAll(typeof(ContainerWindow));
      ...
      // Load data
      UnityObject[] loadedWindows =
          InternalEditorUtility.LoadSerializedFileAndForget(path);

      if (loadedWindows == null || loadedWindows.Length == 0) {
        throw new ArgumentException("Window layout at '" + path +
                                    "' could not be loaded.");
      }

      List<UnityObject> newWindows = new List<UnityObject>();

      // At this point, unparented editor windows are neither desired nor
      // desirable. This can be caused by (legacy) serialization of
      // FallbackEditorWindows or other serialization hiccups (note that
      // unparented editor windows should not exist in theory). Same goes for
      // empty DockAreas (no panes).  Leave them behind.
      for (int i = 0; i < loadedWindows.Length; i++) {
        ...
        if (editorWin != null) {
          ...
          // Host views that do not hold any containers are not desirable at
          // this stage
      ...
      for (int i = 0; i < newWindows.Count; i++) {
        ...
        if (newProjectLayoutWasCreated) {
          MethodInfo method = o.GetType().GetMethod(
              "OnNewProjectLayoutWasCreated", BindingFlags.Public |
      ...
      mainWindow.Show(mainWindow.showMode, loadPosition
      ...
      // Make sure to restore the save to layout flag when loading a layout from
      // a file.
      if (keepMainWindow)
      ...
      for (int i = 0; i < newWindows.Count; i++) {
        ...
        if (win)
          win.minSize = win.minSize; // Causes minSize to be propagated upwards
                                     // to parents!

        ContainerWindow containerWindow = newWindows[i] as ContainerWindow;
        if (containerWindow && containerWindow != mainWindow)
          containerWindow.Show(containerWindow.showMode, loadPosition
                               : false, displayImmediately
                               : true, setFocus
                               : true);
      }
      ...
      UnityObject[] containerWindows =
          Resources.FindObjectsOfTypeAll(typeof(ContainerWindow));

      // Only show dialog if an actual window is present. If not, revert to
      // default immediately
  ...
  public static void CloseWindows() { CloseWindows(false); }
  ...
  private static void CloseWindows(bool keepMainWindow) {
    ...
    foreach (ContainerWindow window in containers) {
      try {
        if (window.showMode != ShowMode.MainWindow || !keepMainWindow ||
            mainWindow != null) {
          window.Close();
    ...
    UnityObject[] oldWindows =
        Resources.FindObjectsOfTypeAll(typeof(EditorWindow));
    if (oldWindows.Length != 0) {
      ...
      Debug.LogError("Failed to destroy editor windows: #" + oldWindows.Length +
                     output);
    }
    ...
    UnityObject[] toolbars =
        Resources.FindObjectsOfTypeAll(typeof(EditorToolbar));
  ...
  public static void SaveWindowLayout(string path) {
    ...
    foreach (EditorWindow w in windows) {
      ...
      if (w.m_Parent != null && w.m_Parent.window != null &&
          w.m_Parent.window.m_DontSaveToLayout)
        continue;
      all.Add(w);
    }
  ...
  public static void LoadFromFile() {
    var layoutFilePath = EditorUtility.OpenFilePanelWithFilters(
        "Load layout from disk...", "", new[]{"Layout", "wlt"});
  ...
  public static void SaveToFile() {
    var layoutFilePath = EditorUtility.SaveFilePanel("Save layout to disk...",
                                                     "", "layout", "wlt");
    if (String.IsNullOrEmpty(layoutFilePath))
      return;
...
internal class SaveWindowLayout : EditorWindow {
  ...
  static readonly ReadOnlyCollection<char> k_InvalidChars =
      new ReadOnlyCollection<char>(Path.GetInvalidFileNameChars());
  static StringBuilder s_CurrentInvalidChars =
      new StringBuilder(k_InvalidChars.Count);
  ...
  internal static SaveWindowLayout Show(Rect r) {
    SaveWindowLayout w = GetWindowWithRect<SaveWindowLayout>(
        new Rect(r.xMax - (k_Width - k_Offset), r.y + k_Offset, k_Width,
                 k_Height),
        true, L10n.Tr("Save Layout"));
  ...
  private static void UpdateCurrentInvalidChars() {
    ...
    // This approach will get the invalid characters in the layout name in they
    // order they appear. This approach would help locate invalid characters
    // faster (in theory) and makes more sense to display them this way if a few
    // unique characters were being typed in a row.
    ...
    for (int i = 0; i < s_LayoutName.Length; ++i) {
      ...
      // We loop through the invalid characters, trying to see if the current
      // character in the layout name is invalid.
      for (int j = 0; j < k_InvalidChars.Count && !isInvalidChr; ++j) {
        if (s_LayoutName[i] == k_InvalidChars[j]) {
          isInvalidChr = true;

          // We loop through the invalid characters to see if the current
          // invalid character was already added.
  ...
  void OnEnable() { titleContent = GetLocalizedTitleContent(); }
  ...
  void OnGUI() {
    ...
    if (s_CurrentInvalidChars.Length != 0) {
      EditorGUILayout.HelpBox(
          string.Format(s_InvalidCharsFormatString, s_CurrentInvalidChars),
          MessageType.Warning);
    ...
    bool canSaveLayout =
        s_LayoutName.Length > 0 && s_CurrentInvalidChars.Length == 0;
    ...
    if (GUILayout.Button("Save") || hitEnter && canSaveLayout) {
      ...
      string path = Path.Combine(WindowLayout.layoutsModePreferencesPath,
                                 s_LayoutName + ".wlt");
...
internal class DeleteWindowLayout : EditorWindow {
  ...
  internal static DeleteWindowLayout Show(Rect r) {
    DeleteWindowLayout w = GetWindowWithRect<DeleteWindowLayout>(
        new Rect(r.xMax - (k_Width - k_Offset), r.y + k_Offset, k_Width,
                 k_Height),
        true, L10n.Tr("Delete Layout"));
    w.m_Parent.window.m_DontSaveToLayout = true;
    return w;
  }
  ...
  private void InitializePaths() {
    string[] allPaths =
        Directory.GetFiles(WindowLayout.layoutsModePreferencesPath);
...
internal static class CreateBuiltinWindows {
  ...
  // Version Control is registered from native code
  // (EditorWindowController.cpp), for license check [MenuItem ("Window/Version
  // Control", false, 2010)]
  static void ShowVersionControl() { EditorWindow.GetWindow<WindowPending>(); }
...
internal class WindowFocusState : ScriptableObject {
  ...
  internal static WindowFocusState instance {
    get {
      if (m_Instance == null)
        m_Instance =
            FindObjectOfType(typeof(WindowFocusState)) as WindowFocusState;
      if (m_Instance == null)
