...
internal class PreviewResizer {
  // The raw preview size while dragging (not snapped to allowed values)
  // (shared)
  ...
  // The saved value is the size when expanded - when collapsed the value is
  // negative, so it can be restored when expanded again.
  [SerializeField]
  ...
  private int id {
    get {
      if (m_Id == 0)
        m_Id = EditorGUIUtility.GetControlID(m_ControlHash, FocusType.Passive,
                                             new Rect());
      return m_Id;
    }
  }

  public bool localFrame {
    get;
    set;
  }

  // Instances of this class should be serialized.
  // The Init function will only have effect if the serialized values are not
  // already set.
  public void Init(string prefName) {
    ...
    // This is the only place the pref is read. This means we can have e.g.
    // multiple Inspectors that can be controlled individually as long as
    // they're open.
    m_CachedPref = EditorPrefs.GetFloat(m_PrefName, 1);
  }

  public float ResizeHandle(Rect windowPosition, float minSize,
                            float minRemainingSize, float resizerHeight) {
    return ResizeHandle(windowPosition, minSize, minRemainingSize,
                        resizerHeight, new Rect());
  }

  public float ResizeHandle(Rect windowPosition, float minSize,
                            float minRemainingSize, float resizerHeight,
                            Rect dragRect) {
    // Sanity check the cached value. It can be positive or negative, but never
    // smaller than the minSize
    ...
    float previewSize = (dragging ? s_DraggedPreviewSize
                         : Mathf.Max(0, m_CachedPref));
    bool expanded = (m_CachedPref > 0);
    float lastSize = Mathf.Abs(m_CachedPref);

    Rect resizerRect =
        new Rect(0, windowPosition.height - previewSize - resizerHeight,
                 windowPosition.width, resizerHeight);
    if (dragRect.width != 0) {
    ...
    previewSize = -PixelPreciseCollapsibleSlider(id, resizerRect, -previewSize,
                                                 -maxPreviewSize, -0,
                                                 ref expanded, localFrame);
  ...
  public float SetExpanded(Rect windowPosition, float minSize,
                           float minRemainingSize, float resizerHeight,
                           Rect dragRect, bool isExpanded) {
    // Sanity check the cached value. It can be positive or negative, but never
    // smaller than the minSize
    if (Mathf.Abs(m_CachedPref) < minSize)
      m_CachedPref = minSize * Mathf.Sign(m_CachedPref);
  ...
  public bool GetExpandedBeforeDragging() { return (m_CachedPref > 0); }
  ...
  public void SetExpanded(bool expanded) {
    ...
    if (m_Container != null) {
      m_Container.style.minHeight = expanded ? containerMinimumHeightExpanded
          : m_ContainerMinimumHeightCollapsed;
    }
  }
  ...
  public void ToggleExpanded() {
    ...
    if (m_Container != null) {
      m_Container.style.minHeight = m_CachedPref > 0
                                        ? containerMinimumHeightExpanded
                                        : m_ContainerMinimumHeightCollapsed;
    }
  }

  // This is the slider behavior for resizing the preview area
  public static float PixelPreciseCollapsibleSlider(int id, Rect position,
                                                    float value, float min,
                                                    float max,
                                                    ref bool expanded) {
    return PixelPreciseCollapsibleSlider(id, position, value, min, max,
                                         ref expanded, false);
  }

  public static float
  PixelPreciseCollapsibleSlider(int id, Rect position, float value, float min,
                                float max, ref bool expanded, bool localFrame) {
    ...
    case EventType.MouseDrag:
      if (GUIUtility.hotControl == id) {
        value = Mathf.Clamp(mousePosition.y - s_MouseDownLocation +
                                s_MouseDownValue,
                            min, max - 1);
        GUI.changed = true;
        s_MouseDragged = true;
    ...
    case EventType.Repaint:
      ...
      if (GUIUtility.hotControl == 0) {
        EditorGUIUtility.AddCursorRect(
            new Rect(x, y, position.width, position.height),
      ...
      if (GUIUtility.hotControl == id) {
        ...
        EditorGUIUtility.AddCursorRect(new Rect(x, y - yMove, position.width,
                                                position.height + heightMove),
                                       MouseCursor.SplitResizeUpDown);
      }
      break;
    }
  ...
  internal void SetContainer(IMGUIContainer container,
                             float minimumHeightCollapsed) {
