...
public sealed class GenericMenu {
  ...
  // Add an item to the menu
  public void AddItem(GUIContent content, bool on, MenuFunction2 func,
                      object userData) {
  ...
  public int GetItemCount() { return menuItems.Count; }
  ...
  internal sealed class MenuItem {
    ...
    public MenuItem(GUIContent _content, bool _separator, bool _on,
                    MenuFunction _func) {
      content = _content;
    ...
    public MenuItem(GUIContent _content, bool _separator, bool _on,
                    MenuFunction2 _func, object _userData) {
      content = _content;
      separator = _separator;
      on = _on;
      func2 = _func;
  ...
  public void DropDown(Rect position) {
    ...
    EditorUtility.DisplayCustomMenuWithSeparators(
        position, titles, enabled, separator,
        (int[]) selected.ToArray(typeof(int)), CatchMenu, null, true,
        allowDuplicateNames);
  }

  // Display as a popup with /selectedIndex/. How this behaves depends on the
  // platform (on Mac, it'll try to scroll the menu to the right place)
  internal void Popup(Rect position, int selectedIndex) { DropDown(position); }
  ...
  // Show object context menu with builtin menu items plus the ones from this
  // GenericMenu.
  internal void ObjectContextDropDown(Rect position, Object[] context,
                                      int contextUserData) {
    string[] titles = new string[menuItems.Count];
    bool[] enabled = new bool[menuItems.Count];
    ArrayList selected = new ArrayList();
    ...
    EditorUtility.DisplayObjectContextPopupMenuWithExtraItems(
        position, context, contextUserData, titles, enabled, separator,
        (int[]) selected.ToArray(typeof(int)), CatchMenu, null, true);
  }
}
}
