...
internal delegate void TargetChoiceMenuFunction(SerializedProperty property,
                                                Object target);
...
internal static void ApplyPrefabPropertyOverride(object userData) {
	...
	if (!PrefabUtility.PromptAndCheckoutPrefabIfNeeded(
		    info.assetPath, PrefabUtility.SaveVerb.Apply))
		return;
	for (int i = 0; i < info.properties.Length; i++)
		PrefabUtility.ApplyPropertyOverride(info.properties[i], info.assetPath,
...
internal static void RevertPrefabPropertyOverride(object userData) {
	...
	for (int i = 0; i < properties.Length; i++)
		PrefabUtility.RevertPropertyOverride(properties[i],
		                                     InteractionMode.UserAction);
	EditorUtility.ForceReloadInspectors();
}

internal static void ApplyPrefabObjectOverride(object userData) {
	ObjectInstanceAndSourcePathInfo info =
		(ObjectInstanceAndSourcePathInfo) userData;
	if (!PrefabUtility.PromptAndCheckoutPrefabIfNeeded(
		    info.assetPath, PrefabUtility.SaveVerb.Apply))
		return;
	PrefabUtility.ApplyObjectOverride(info.instanceObject, info.assetPath,
	                                  InteractionMode.UserAction);
	EditorUtility.ForceReloadInspectors();
}
...
internal static void ApplyPrefabAddedComponent(object userData) {
	...
	if (!PrefabUtility.PromptAndCheckoutPrefabIfNeeded(
		    info.assetPath, PrefabUtility.SaveVerb.Apply))
		return;
	PrefabUtility.ApplyAddedComponent((Component) info.instanceObject,
	                                  info.assetPath,
	                                  InteractionMode.UserAction);
...
internal static void ApplyPrefabRemovedComponent(object userData) {
	...
	if (!PrefabUtility.PromptAndCheckoutPrefabIfNeeded(
		    path, PrefabUtility.SaveVerb.Apply))
		return;
	PrefabUtility.ApplyRemovedComponent((GameObject) info.instanceObject,
	                                    (Component)
	                                    info.correspondingObjectInSource,
	                                    InteractionMode.UserAction);
	EditorUtility.ForceRebuildInspectors();
}
...
internal static void RevertPrefabRemovedComponent(object userData) {
	ObjectInstanceAndSourceInfo info = (ObjectInstanceAndSourceInfo) userData;
	PrefabUtility.RevertRemovedComponent((GameObject) info.instanceObject,
	                                     (Component)
	                                     info.correspondingObjectInSource,
	                                     InteractionMode.UserAction);
	EditorUtility.ForceRebuildInspectors();
...
internal static void ApplyPrefabAddedGameObject(object userData) {
	...
	if (!PrefabUtility.PromptAndCheckoutPrefabIfNeeded(
		    info.assetPath, PrefabUtility.SaveVerb.Apply))
		return;
	PrefabUtility.ApplyAddedGameObject((GameObject) info.instanceObject,
	                                   info.assetPath,
	                                   InteractionMode.UserAction);
	EditorUtility.ForceRebuildInspectors();
}
...
internal static void SetToValueOfTarget(SerializedProperty property,
                                        Object target) {
...
AddSetToValueOfTargetMenuItems(GenericMenu menu, SerializedProperty property,
                               TargetChoiceMenuFunction func) {
	...
	foreach (Object target in targets) {
		...
		options.Add(option);
		menu.AddItem(
			EditorGUIUtility.TextContent(option), false,
			TargetChoiceForwardFunction,
			new PropertyAndTargetHandler(propertyWithPath, target, func));
	}
}
}

public sealed partial class EditorGUI {
/// Popup to choose which target to pick the value from. Use only when
/// hasMultipleDifferentValues is true.
internal static void TargetChoiceField(Rect position,
                                       SerializedProperty property,
                                       GUIContent label) {
	TargetChoiceField(position, property, label,
	                  TargetChoiceHandler.SetToValueOfTarget);
}

internal static void
TargetChoiceField(Rect position, SerializedProperty property,
                  GUIContent label,
                  TargetChoiceHandler.TargetChoiceMenuFunction func) {
...
/// Popup to choose which target to pick the value from. Use only when
/// hasMultipleDifferentValues is true.
internal static void TargetChoiceField(SerializedProperty property,
                                       GUIContent label,
                                       params GUILayoutOption[] options) {
	TargetChoiceField(property, label, TargetChoiceHandler.SetToValueOfTarget,
	                  options);
}

internal static void
TargetChoiceField(SerializedProperty property, GUIContent label,
                  TargetChoiceHandler.TargetChoiceMenuFunction func,
                  params GUILayoutOption[] options) {
...
public PropertyAndTargetHandler(
	SerializedProperty property, Object target,
	TargetChoiceHandler.TargetChoiceMenuFunction function) {
