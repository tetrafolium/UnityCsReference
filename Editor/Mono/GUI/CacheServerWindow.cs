...
internal class CacheServerWindow : PopupWindowContent {
  ...
  public CacheServerWindow() {
    m_StatusMessageDisabled =
        EditorGUIUtility.TrTextContent("No cache server connected");
    m_StatusMessageConnected = EditorGUIUtility.TrTextContent("Connected");
    m_StatusMessageError =
        EditorGUIUtility.TrTextContent("Attempting to reconnect");
    m_OpenProjectSettings =
        EditorGUIUtility.TrTextContent("Open Project Settings...");

    m_RefreshIcon =
        EditorGUIUtility.TrIconContent("Refresh", "Refresh connection");
  ...
  public override void OnGUI(Rect rect) {
    ...
    if (AssetDatabaseExperimental.IsCacheServerEnabled()) {
      ...
      iconPosition.x =
          rect.width - m_RefreshIcon.image.width - m_WindowStyle.padding.right;
      ...
      EditorGUILayout.LabelField(
          AssetDatabaseExperimental.GetCacheServerAddress(), style);
      GUILayout.EndHorizontal();
    }

    GUILayout.BeginHorizontal();
    var statusTextStyle = new GUIStyle(){normal = {textColor = Color.grey}};
  ...
  private void OpenPreferences() {
    var settings = SettingsWindow.Show(SettingsScope.User,
                                       "Preferences/Cache Server (global)");
    if (settings == null) {
      Debug.LogError(
          "Could not find Preferences for 'Preferences/Cache Server (global)'");
    }
  }
  ...
  public override Vector2 GetWindowSize() {
    ...
    int heightOfWindowPadding =
        m_WindowStyle.padding.top + m_WindowStyle.padding.bottom;
