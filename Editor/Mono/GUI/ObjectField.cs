...
public sealed partial class EditorGUI {
  static private GUIContent s_SceneMismatch = EditorGUIUtility.TrTextContent(
      "Scene mismatch (cross scene references not supported)");
  static private GUIContent s_TypeMismatch =
      EditorGUIUtility.TrTextContent("Type mismatch");
  ...
  internal static Object DoObjectField(Rect position, Rect dropRect, int id,
                                       Object obj, Object objBeingEdited,
                                       System.Type objType,
                                       ObjectFieldValidator validator,
                                       bool allowSceneObjects,
                                       GUIStyle style = null) {
    return DoObjectField(position, dropRect, id, obj, objBeingEdited, objType,
                         null, validator, allowSceneObjects,
                         style != null ? style
  ...
  DoObjectField(Rect position, Rect dropRect, int id, System.Type objType,
                SerializedProperty property, ObjectFieldValidator validator,
                bool allowSceneObjects, GUIStyle style = null) {
    return DoObjectField(position, dropRect, id, null, null, objType, property,
                         validator, allowSceneObjects, style != null ? style
                         : EditorStyles.objectField);
  }
  ...
  // if shift/control is pressed and object is a texture, pops up a large
  // texture preview window
  internal static void PingObjectOrShowPreviewOnClick(Object targetObject,
                                                      Rect position) {
  ...
  static Object AssignSelectedObject(SerializedProperty property,
                                     ObjectFieldValidator validator,
                                     System.Type objectType, Event evt) {
    Object[] references = {ObjectSelector.GetCurrentObject()};
    Object assigned = validator(references, objectType, property,
                                ObjectFieldValidatorOptions.None);
  ...
  static private Rect GetButtonRect(ObjectFieldVisualType visualType,
                                    Rect position) {
    switch (visualType) {
  ...
  static bool ValidDroppedObject(Object[] references, System.Type objType,
    ...
    if (obj is MonoBehaviour || obj is ScriptableObject) {
      if (!HasValidScript(obj)) {
        errorString =
            $"Type cannot be found: {reference.GetType()}. Containing file and class name must match.";
        return false;
      }
    }
    return true;
  ...
  // Timeline package is using this internal overload so can't remove until
  // that's fixed.
  internal static Object DoObjectField(Rect position, Rect dropRect, int id,
                                       Object obj, System.Type objType,
                                       SerializedProperty property,
                                       ObjectFieldValidator validator,
                                       bool allowSceneObjects, GUIStyle style) {
    return DoObjectField(position, dropRect, id, objType, property, validator,
                         allowSceneObjects);
  }

  // This method takes either object reference directly, or via
  // SerializedObject. Since it's not easy to know which parameters are mutually
  // exclusively used, this method is private and internal/public methods
  // instead EITHER take SerializedObject OR direct reference.
  static Object DoObjectField(Rect position, Rect dropRect, int id, Object obj,
                              Object objBeingEdited, System.Type objType,
                              SerializedProperty property,
                              ObjectFieldValidator validator,
                              bool allowSceneObjects, GUIStyle style) {
    if (validator == null)
    ...
    // special case test, so we continue to ping/select objects with the object
    // field disabled
    ...
    if (hasThumbnail && position.height <= kObjectFieldMiniThumbnailHeight &&
        position.width <= kObjectFieldMiniThumbnailWidth)
    ...
    if (visualType == ObjectFieldVisualType.IconAndText)
      EditorGUIUtility.SetIconSize(
          new Vector2(12, 12)); // Have to be this small to fit inside a single
                                // line height ObjectField
    ...
    case EventType.DragPerform:
      ...
      if (dropRect.Contains(Event.current.mousePosition) && GUI.enabled) {
        ...
        if (validatedObject != null) {
          // If scene objects are not allowed and object is a scene object then
          // clear
    ...
    case EventType.MouseDown:
      if (position.Contains(Event.current.mousePosition)) {
        if (Event.current.button == 1) {
          var actualObject =
              property != null ? property.objectReferenceValue : obj;
          var contextMenu = new GenericMenu();
          contextMenu.AddItem(
              GUIContent.Temp("Properties..."), false,
        ...
        if (buttonRect.Contains(Event.current.mousePosition)) {
          if (GUI.enabled) {
            ...
            if (property != null)
              ...
              ObjectSelector.get.Show(obj, objType, objBeingEdited,
                                      allowSceneObjects);
          ...
          // One click shows where the referenced object is, or pops up a
          // preview
          ...
          // Double click opens the asset in external app or changes selection
    ...
    case EventType.ExecuteCommand:
      ...
      if (commandName == ObjectSelector.ObjectSelectorUpdatedCommand &&
          ObjectSelector.get.objectSelectorID == id &&
          GUIUtility.keyboardControl == id &&
          (property == null || !property.isScript))
        return AssignSelectedObject(property, validator, objType, evt);
      else if (commandName == ObjectSelector.ObjectSelectorClosedCommand &&
               ObjectSelector.get.objectSelectorID == id &&
               GUIUtility.keyboardControl == id && property != null &&
               property.isScript) {
    ...
    case EventType.KeyDown:
      if (GUIUtility.keyboardControl == id) {
        ...
        // Apparently we have to check for the character being space instead of
        // the keyCode, otherwise the Inspector will maximize upon pressing
        // space.
    ...
    case EventType.Repaint:
      ...
      if (showMixedValue) {
        ...
        // correct type. But if not, EditorGUIUtility.ObjectContent is more
        // reliable. It can take a more specific object type specified as
        // argument into account, and it gets the icon at the same time.
        if (obj == null && objType == null && property != null) {
          temp =
              EditorGUIUtility.TempContent(property.objectReferenceStringValue);
        } else {
          // In order for ObjectContext to be able to distinguish between
          // None/Missing, we need to supply an instanceID. For some reason,
          // getting the instanceID from property.objectReferenceValue is not
          // reliable, so we have to explicitly check
          // property.objectReferenceInstanceIDValue if a property exists.
          if (property != null)
            temp = EditorGUIUtility.ObjectContent(
                obj, objType, property.objectReferenceInstanceIDValue);
          else
            ...
            if (EditorSceneManager.preventCrossSceneReferences &&
              ...
              if (!EditorApplication.isPlaying)
                ...
                temp.text =
                    temp.text +
                    string.Format(" ({0})",
                                  GetGameObjectFromObject(obj).scene.name);
            } else if (validator(references, objType, property,
                                 ObjectFieldValidatorOptions
                                     .ExactObjectTypeValidation) == null)
              temp = s_TypeMismatch;
          }
        }
      }
      ...
      case ObjectFieldVisualType.IconAndText:
        ...
        Rect buttonRect = EditorStyles.objectFieldButton.margin.Remove(
            GetButtonRect(visualType, position));
        EditorStyles.objectFieldButton.Draw(
            buttonRect, GUIContent.none, id, DragAndDrop.activeControlID == id,
  ...
  private static void DrawObjectFieldLargeThumb(Rect position, int id,
                                                Object obj,
                                                GUIContent content) {
    GUIStyle thumbStyle = EditorStyles.objectFieldThumb;
    thumbStyle.Draw(position, GUIContent.none, id,
                    DragAndDrop.activeControlID == id,
                    position.Contains(Event.current.mousePosition));
    ...
    if (obj != null && !showMixedValue) {
      ...
      if (isCubemap || isSprite) {
        ...
        if (t2d != null) {
          ...
          GUIStyle.none.Draw(thumbRect, content.image, false, false, false,
                             false);
  ...
  private static void DrawObjectFieldMiniThumb(Rect position, int id,
                                               Object obj, GUIContent content) {
    GUIStyle thumbStyle = EditorStyles.objectFieldMiniThumb;
    position.width = EditorGUI.kObjectFieldMiniThumbnailWidth;
    BeginHandleMixedValueContentColor();
    bool hover = obj != null; // we use hover texture for enhancing the border
    ...
    if (obj != null && !showMixedValue) {
      Rect thumbRect =
          new Rect(position.x + 1, position.y + 1, position.height - 2,
                   position.height - 2); // subtract 1 px border
      Texture2D t2d = content.image as Texture2D;
      if (t2d != null && t2d.alphaIsTransparency)
        DrawTextureTransparent(thumbRect, t2d);
      else
      ...
      if (thumbRect.Contains(Event.current.mousePosition))
        GUI.Label(thumbRect, GUIContent.Temp(string.Empty,
                                             "Ctrl + Click to show preview"));
    }
  }

  internal static Object DoDropField(Rect position, int id, System.Type objType,
                                     ObjectFieldValidator validator,
                                     bool allowSceneObjects, GUIStyle style) {
    if (validator == null)
      validator = ValidateObjectFieldAssignment;
    ...
    case EventType.DragPerform:
      ...
      break;
    case EventType.Repaint:
      style.Draw(position, GUIContent.none, id,
                 DragAndDrop.activeControlID == id);
      break;
    }
    return null;
...
internal class ObjectPreviewPopup : PopupWindowContent {
  ...
  public ObjectPreviewPopup(Object previewObject) {
    if (previewObject == null) {
      Debug.LogError(
          "ObjectPreviewPopup: Check object is not null, before trying to show it!");
      return;
    }
    m_ObjectName =
        new GUIContent(previewObject.name,
                       AssetDatabase.GetAssetPath(
                           previewObject)); // Show path as tooltip on label
  ...
  public override void OnGUI(Rect rect) {
    ...
    // Toolbar
    GUILayout.BeginArea(new Rect(rect.x, rect.y, rect.width, kToolbarHeight),
                        s_Styles.toolbar);
    ...
    GUI.Label(new Rect(rect.x + 5f, rect.y, rect.width - kMaxSettingsWidth,
                       kToolbarHeight),
              m_ObjectName, s_Styles.toolbarText);

    // Object preview
    Rect previewRect = new Rect(rect.x, rect.y + kToolbarHeight, rect.width,
