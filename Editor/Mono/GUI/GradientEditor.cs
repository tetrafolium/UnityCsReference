...
internal class GradientEditor {
  class Styles {
    ...
    static GUIStyle GetStyle(string name) {
      GUISkin s =
          (GUISkin) EditorGUIUtility.LoadRequired("GradientEditor.GUISkin");
  ...
  [System.NonSerialized] Swatch m_SelectedSwatch;
  Gradient m_Gradient;
  int m_NumSteps;
  bool m_HDR;
  ...
  void BuildArrays() {
    ...
    for (int i = 0; i < alphaKeys.Length; i++) {
      ...
      m_AlphaSwatches.Add(
          new Swatch(alphaKeys[i].time, new Color(a, a, a, 1), true));
    }
    m_GradientMode = m_Gradient.mode;
  }

  public static void DrawGradientWithBackground(Rect position,
                                                Gradient gradient) {
    Texture2D gradientTexture =
        UnityEditorInternal.GradientPreviewCache.GetGradientPreview(gradient);
    Rect r2 = new Rect(position.x + 1, position.y + 1, position.width - 2,
                       position.height - 2);

    // Background checkers
    Texture2D backgroundTexture = GetBackgroundTexture();
    Rect texCoordsRect = new Rect(0, 0, r2.width / backgroundTexture.width,
                                  r2.height / backgroundTexture.height);
  ...
  public void OnGUI(Rect position) {
    ...
    float gradientTextureHeight =
        position.height - 2 * swatchHeight - editSectionHeight - modeHeight;

    position.height = modeHeight;
    m_GradientMode = (GradientMode) EditorGUI.EnumPopup(
        position, s_Styles.modeText, m_GradientMode);
    ...
    // Alpha swatches (no idea why they're top, but that's what Adobe & Apple
    // seem to agree on)
    ...
    if (m_SelectedSwatch != null) {
      ...
      float totalLocationWidth =
          locationTextWidth + space + locationTextWidth + locationWidth;
      ...
      if (m_SelectedSwatch.m_IsAlpha) {
        ...
        float sliderValue =
            EditorGUI.IntSlider(rect, s_Styles.alphaText,
                                (int)(m_SelectedSwatch.m_Value.r * 255), 0,
                                255) /
            255f;
        if (EditorGUI.EndChangeCheck()) {
          sliderValue = Mathf.Clamp01(sliderValue);
          m_SelectedSwatch.m_Value.r = m_SelectedSwatch.m_Value.g =
              m_SelectedSwatch.m_Value.b = sliderValue;
          AssignBack();
          HandleUtility.Repaint();
        }
      } else {
        EditorGUI.BeginChangeCheck();
        m_SelectedSwatch.m_Value =
            EditorGUI.ColorField(rect, s_Styles.colorText,
                                 m_SelectedSwatch.m_Value, true, false, m_HDR);
        if (EditorGUI.EndChangeCheck()) {
          AssignBack();
          HandleUtility.Repaint();
        }
      ...
      EditorGUI.BeginChangeCheck();
      float newLocation =
          EditorGUI.FloatField(rect, s_Styles.locationText,
                               m_SelectedSwatch.m_Time * 100.0f) /
          100.0f;
      if (EditorGUI.EndChangeCheck()) {
  ...
  void ShowSwatchArray(Rect position, List<Swatch> swatches, bool isAlpha) {
    ...
    float mouseSwatchTime =
        GetTime((Event.current.mousePosition.x - position.x) / position.width);
    Vector2 fixedStepMousePosition =
        new Vector3(position.x + mouseSwatchTime * position.width,
    ...
    case EventType.MouseDown: {
      ...
      if (clickRect.Contains(evt.mousePosition)) {
        ...
        if (swatches.Contains(m_SelectedSwatch) &&
            !m_SelectedSwatch.m_IsAlpha &&
            CalcSwatchRect(position, m_SelectedSwatch)
        ...
        if (!found) {
          if (swatches.Count < k_MaxNumKeys) {
            ...
            if (isAlpha)
              currentColor =
                  new Color(currentColor.a, currentColor.a, currentColor.a, 1f);
            else
              currentColor.a = 1f;
            m_SelectedSwatch =
                new Swatch(mouseSwatchTime, currentColor, isAlpha);
            swatches.Add(m_SelectedSwatch);
            AssignBack();
          } else {
            Debug.LogWarning("Max " + k_MaxNumKeys + " color keys and " +
                             k_MaxNumKeys +
    ...
    case EventType.MouseDrag:
      ...
      if (GUIUtility.hotControl == id && m_SelectedSwatch != null) {
        ...
        // If user drags swatch outside in vertical direction, we'll remove the
        // swatch
        if ((evt.mousePosition.y + 5 < position.y ||
             evt.mousePosition.y - 5 > position.yMax)) {
          if (swatches.Count > 1) {
    ...
    case EventType.MouseUp:
      if (GUIUtility.hotControl == id) {
        ...
        // If the dragged swatch is NOT in the timeline, it means it was dragged
        // outside. We just forget about it and let GC get it later.
        if (!swatches.Contains(m_SelectedSwatch))
          m_SelectedSwatch = null;

        // Remove duplicate keys on mouse up so that we do not kill any keys
        // during the drag
  ...
  void DrawSwatch(Rect totalPos, Swatch s, bool upwards) {
    ...
    GUIStyle overlay =
        upwards ? s_Styles.upSwatchOverlay : s_Styles.downSwatchOverlay;
  ...
  Rect CalcSwatchRect(Rect totalRect, Swatch s) {
    float time = s.m_Time;
    return new Rect(totalRect.x + Mathf.Round(totalRect.width * time) - 5,
  ...
  void AssignBack() {
    ...
    for (int i = 0; i < m_AlphaSwatches.Count; i++) {
      alphaKeys[i].alpha =
          m_AlphaSwatches[i]
              .m_Value.r; // we use the red channel (see BuildArrays)
  ...
  // Kill any swatches that are at the same time (For example as the result of
  // dragging a swatch on top of another)
  void RemoveDuplicateOverlappingSwatches() {
    bool didRemoveAny = false;
    for (int i = 1; i < m_RGBSwatches.Count; i++) {
      if (Mathf.Approximately(m_RGBSwatches[i - 1].m_Time,
                              m_RGBSwatches[i].m_Time)) {
    ...
    for (int i = 1; i < m_AlphaSwatches.Count; i++) {
      if (Mathf.Approximately(m_AlphaSwatches[i - 1].m_Time,
                              m_AlphaSwatches[i].m_Time)) {
  ...
  public static Texture2D GetBackgroundTexture() {
    if (s_BackgroundTexture == null)
      s_BackgroundTexture = GradientEditor.CreateCheckerTexture(
          32, 4, 4, Color.white, new Color(0.7f, 0.7f, 0.7f));
    return s_BackgroundTexture;
  }

  public static Texture2D CreateCheckerTexture(int numCols, int numRows,
                                               int cellPixelWidth, Color col1,
                                               Color col2) {
      ...
      for (int j = 0; j < numCols; j++)
        for (int ci = 0; ci < cellPixelWidth; ci++)
          for (int cj = 0; cj < cellPixelWidth; cj++)
            pixels[(i * cellPixelWidth + ci) * width + j * cellPixelWidth +
                   cj] = ((i + j) % 2 == 0) ? col1 : col2;
  ...
  // GUI Helpers
  public static void DrawGradientSwatch(Rect position, Gradient gradient,
                                        Color bgColor) {
    DrawGradientSwatchInternal(position, gradient, null, bgColor);
  }

  public static void DrawGradientSwatch(Rect position,
                                        SerializedProperty property,
                                        Color bgColor) {
    DrawGradientSwatchInternal(position, null, property, bgColor);
  }

  private static void DrawGradientSwatchInternal(Rect position,
                                                 Gradient gradient,
                                                 SerializedProperty property,
                                                 Color bgColor) {
    if (Event.current.type != EventType.Repaint)
    ...
    if (EditorGUI.showMixedValue) {
      ...
      mgs.Draw(position, EditorGUI.mixedValueContent, false, false, false,
               false);
    ...
    if (backgroundTexture != null) {
      ...
      GUIStyle backgroundStyle =
          EditorGUIUtility.GetBasicTextureStyle(backgroundTexture);
    ...
    // HDR label
    if (maxColorComponent > 1.0f) {
      GUI.Label(new Rect(position.x, position.y - 1, position.width - 3,
                         position.height + 2),
                "HDR", EditorStyles.centeredGreyMiniLabel);
    }
  }
  ...
  private static float GetMaxColorComponent(Gradient gradient) {
    ...
    for (int i = 0; i < colorKeys.Length; i++) {
      maxColorComponent =
          Mathf.Max(maxColorComponent, colorKeys[i].color.maxColorComponent);
    }
