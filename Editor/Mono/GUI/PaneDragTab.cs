...
#pragma warning disable 169
  ...
  private static class Styles {
    ...
    public static readonly SVC<Color> backgroundColor =
        new SVC<Color>("--theme-background-color");
  }
  ...
  public void SetDropInfo(DropInfo di, Vector2 mouseScreenPos,
                          ContainerWindow inFrontOf) {
    if (m_Type != di.type ||
        (di.type == DropInfo.Type.Pane && di.rect != m_TargetRect)) {
      m_Type = di.type;
    ...
    switch (di.type) {
    case DropInfo.Type.Window:
      m_TargetRect = new Rect(mouseScreenPos.x - m_FullWindowSize.x / 2,
                              mouseScreenPos.y - m_FullWindowSize.y / 2,
    ...
    // On Windows, repainting without setting proper size first results in one
    // garbage frame... For some reason.
  ...
  public void Show(Rect pixelPos, GUIContent content, Vector2 viewSize,
    ...
    m_Window.Show(ShowMode.NoShadow, loadPosition
  ...
  void SetWindowPos(Rect screenPosition) { m_Window.position = screenPosition; }
  ...
  protected override void OldOnGUI() {
    ...
    if (m_Type == DropInfo.Type.Tab) {
      ...
      Styles.dragtab.CalcMinMaxWidth(m_Content, out minWidth,
                                     out expectedWidth);
      float tabWidth = Mathf.Max(Mathf.Min(expectedWidth, Styles.tabMaxWidth),
                                 Styles.tabMinWidth) +
      ...
      float roundedWidth =
          Mathf.Ceil(tabPositionRect.x + tabPositionRect.width) - roundedPosX;
      Rect tabContentRect = new Rect(roundedPosX, tabPositionRect.y,
                                     roundedWidth, tabPositionRect.height);
      Rect viewRect = new Rect(dragTabOffsetX, tabContentRect.yMax - 2f,
                               position.width - dragTabOffsetX * 2,
                               position.height - tabContentRect.yMax);
