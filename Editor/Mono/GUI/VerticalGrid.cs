...
internal class VerticalGrid {
  ...
  public float verticalSpacing {
    get; // spacing between each row of items
    set;
  }
  public float minHorizontalSpacing {
    get; // minimum spacing between each column in grid
    set;
  }
  ...
  public void InitNumRowsAndColumns(int itemCount, int maxNumRows) {
    if (useFixedHorizontalSpacing) {
      ...
      // Set height
      m_Height = m_Rows * (itemSize.y + verticalSpacing) - verticalSpacing +
    ...
    } else // center columns
    {
      ...
      // Set horizontal spacing
      m_HorizontalSpacing =
          Mathf.Max(0f, (fixedWidth -
                         (m_Columns * itemSize.x + leftMargin + rightMargin)) /
                            (m_Columns));
  ...
  public int CalcColumns() {
    float horizontalSpacing = useFixedHorizontalSpacing ? fixedHorizontalSpacing
        : minHorizontalSpacing;
    int cols = (int) Mathf.Floor((fixedWidth - leftMargin - rightMargin) /
                                 (itemSize.x + horizontalSpacing));
  ...
  public Rect CalcRect(int itemIdx, float yOffset) {
    ...
    if (useFixedHorizontalSpacing) {
      return new Rect(
          leftMargin + column * (itemSize.x + fixedHorizontalSpacing),
          row * (itemSize.y + verticalSpacing) + topMargin + yOffset,
          itemSize.x, itemSize.y);
    } else {
      return new Rect(leftMargin + horizontalSpacing * 0.5f +
                          column * (itemSize.x + horizontalSpacing),
  ...
  public int GetMaxVisibleItems(float height) {
    int visibleRows = (int) Mathf.Ceil((height - topMargin - bottomMargin) /
                                       (itemSize.y + verticalSpacing));
    return visibleRows * columns;
  }

  public bool IsVisibleInScrollView(float scrollViewHeight, float scrollPos,
                                    float gridStartY, int maxIndex,
    ...
    int startRow =
        Mathf.FloorToInt((scrollViewStart - offsetY) / itemHeightAndSpacing);
    startIndex = startRow * columns;
    startIndex = Mathf.Clamp(startIndex, 0, maxIndex);

    // endRow can be negative if grid is starting in the middle of the view
    int endRow =
        Mathf.FloorToInt((scrollViewEnd - offsetY) / itemHeightAndSpacing);
  ...
  public override string ToString() {
    return UnityString.Format(
        "VerticalGrid: rows {0}, columns {1}, fixedWidth {2}, itemSize {3}",
        rows, columns, fixedWidth, itemSize);
  }
}
...
internal class VerticalGridWithSplitter {
  ...
  public void InitNumRowsAndColumns(int itemCount, int maxNumRows) {
    // Set columns (with an excess of 1 minHorizontalSpacing)
    m_Columns = (int) Mathf.Floor((fixedWidth - leftMargin - rightMargin) /
                                  (itemSize.x + minHorizontalSpacing));
    ...
    if (m_Columns > 1)
      m_HorizontalSpacing =
          (fixedWidth - (m_Columns * itemSize.x + leftMargin + rightMargin)) /
          (m_Columns - 1);

    // Set rows
    m_Rows =
        Mathf.Min(maxNumRows, (int) Mathf.Ceil(itemCount / (float) m_Columns));
  ...
  public void OpenSplit(int splitAfterRowIndex, int numItems) {
    ...
    float splitHeight = numRows * (itemSize.y + verticalSpacing) -
                        verticalSpacing + topMargin + bottomMargin;
  ...
  public void CloseSplit() { m_TargetSplitHeight = 0f; }
  ...
  public bool UpdateSplitAnimationOnGUI() {
    if (m_SplitAfterRow != -1) {
      ...
      // Animate
      if (m_CurrentSplitHeight != m_TargetSplitHeight &&
          Event.current.type == EventType.Repaint) {
        m_CurrentSplitHeight =
            Mathf.MoveTowards(m_CurrentSplitHeight, m_TargetSplitHeight, 0.03f);
