...
internal class AssetSaveDialog : EditorWindow {
  class Styles {
    ...
    public GUIContent saveSelected =
        EditorGUIUtility.TrTextContent("Save Selected");
    ...
    public Styles() {
      ...
      buttonWidth = Mathf.Max(Mathf.Max(button.CalcSize(saveSelected).x,
                                        button.CalcSize(saveAll).x),
                              button.CalcSize(dontSave).x);
    }
  }
  ...
  public static void ShowWindow(string[] inAssets,
    ...
    assetsThatShouldBeSaved =
        new string[win.m_AssetsToSave.Count + numMetaFiles];
  ...
  public static GUIContent GetContentForAsset(string path) {
    ...
    if (path.StartsWith("Library/"))
      path = ObjectNames.NicifyVariableName(
          AssetDatabase.LoadMainAssetAtPath(path).name);
  ...
  void OnGUI() {
    ...
    int numSelected = 0;
    foreach (ListViewElement el in ListViewGUILayout.ListView(m_LV,
                                                              s_Styles.box)) {
      ...
      if (ListViewGUILayout.HasMouseUp(el.position)) {
        ...
        ListViewGUILayout.MultiSelection(prevSelectedRow, el.row,
                                         ref m_InitialSelectedItem,
                                         ref m_SelectedItems);
      }
    ...
    if (GUILayout.Button(s_Styles.close, s_Styles.button,
                         GUILayout.Width(s_Styles.buttonWidth))) {
      CloseWindow();
    }
    ...
    if (GUILayout.Button(s_Styles.dontSave, s_Styles.button,
                         GUILayout.Width(s_Styles.buttonWidth))) {
      IgnoreSelectedAssets();
    }

    if (GUILayout.Button(allSelected ? s_Styles.saveAll
                         : s_Styles.saveSelected, s_Styles.button,
                           GUILayout.Width(s_Styles.buttonWidth))) {
      SaveSelectedAssets();
    }
