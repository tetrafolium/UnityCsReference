...
class FlowLayout : GUILayoutGroup {
  ...
  public override void CalcWidth() {
    ...
    if (isVertical) {
      ...
      // Since we don't know what will wrap where we'll just take the Min margin
      // of all child elements
      if (!hasMinWidth) {
        minWidth = 0;
        foreach (GUILayoutEntry i in entries) {
          // Here we should probably include margins of the element, but that
          // does seem kind of annoying
  ...
  public override void SetHorizontal(float x, float width) {
    // Apply the base. Now everything is in one line (or column), and all we
    // need is to insert the linebreaks
  ...
  public override void CalcHeight() {
    ...
    if (isVertical) {
      ...
      foreach (GUILayoutEntry i in entries) {
        ...
        m_LineInfo[j].topBorder =
            Mathf.Min(i.marginTop, m_LineInfo[j].topBorder);
        m_LineInfo[j].bottomBorder =
            Mathf.Min(i.marginBottom, m_LineInfo[j].bottomBorder);
      }
      ...
      for (int i = 1; i < m_Lines; i++) {
        float space =
            Mathf.Max(m_LineInfo[i - 1].bottomBorder, m_LineInfo[i].topBorder);
    ...
    // Do the dance between children & parent for haggling over how many empty
    // pixels to have
  ...
  public override void SetVertical(float y, float height) {
    ...
    if (isVertical) {
      ...
      for (int i = 0; i < m_Lines; i++) {
        if (i > 0)
          lineY += Mathf.Max(m_LineInfo[i].topBorder,
                             m_LineInfo[i - 1].bottomBorder);
        m_LineInfo[i].start = lineY;
        m_LineInfo[i].size = Mathf.Lerp(m_LineInfo[i].minSize,
                                        m_LineInfo[i].maxSize, minMaxScale);
      ...
      foreach (GUILayoutEntry i in entries) {
        ...
        if (i.stretchHeight != 0)
          ...
          i.SetVertical(li.start + i.marginTop,
                        Mathf.Clamp(li.size - i.marginVertical, i.minHeight,
                                    i.maxHeight));
      }
    }
  }
...
internal class GUISlideGroup {
  ...
  public Rect BeginHorizontal(int id, params GUILayoutOption[] options) {
    SlideGroupInternal g =
        (SlideGroupInternal) GUILayoutUtility.BeginLayoutGroup(
            GUIStyle.none, options, typeof(SlideGroupInternal));
    g.SetID(this, id);
    g.isVertical = false;
    return g.m_FinalRect;
  }

  public void EndHorizontal() { GUILayoutUtility.EndLayoutGroup(); }
  ...
  Rect GetRect(int id, Rect r, out bool changed) {
    ...
    if (current.y != r.y || current.height != r.height || current.x != r.x ||
