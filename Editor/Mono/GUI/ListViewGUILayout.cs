...
static public ListViewShared.ListViewElementsEnumerator
ListView(ListViewState state, GUIStyle style,
         params GUILayoutOption[] options) {
	return ListView(state, 0, string.Empty, style, options);
}

static public ListViewShared.ListViewElementsEnumerator
ListView(ListViewState state, string dragTitle, GUIStyle style,
         params GUILayoutOption[] options) {
	return ListView(state, 0, dragTitle, style, options);
}

static public ListViewShared.ListViewElementsEnumerator
ListView(ListViewState state, ListViewOptions lvOptions, GUIStyle style,
         params GUILayoutOption[] options) {
	return ListView(state, lvOptions, string.Empty, style, options);
}

static public ListViewShared.ListViewElementsEnumerator
ListView(ListViewState state, ListViewOptions lvOptions, string dragTitle,
         GUIStyle style, params GUILayoutOption[] options) {
	lvState = state;

	GUILayout.BeginHorizontal(
		style,
		options); // no good reason for this here, except drawing LVs background
	...
	state.ilvState.wantsReordering =
		(lvOptions & ListViewOptions.wantsReordering) != 0;
	state.ilvState.wantsExternalFiles =
		(lvOptions & ListViewOptions.wantsExternalFiles) != 0;
	state.ilvState.wantsToStartCustomDrag =
		(lvOptions & ListViewOptions.wantsToStartCustomDrag) != 0;
	state.ilvState.wantsToAcceptCustomDrag =
		(lvOptions & ListViewOptions.wantsToAcceptCustomDrag) != 0;
	state.ilvState.wantsRowMultiSelection =
		(lvOptions & ListViewOptions.wantsRowMultiSelection) != 0;
...
static private ListViewShared.ListViewElementsEnumerator
DoListView(ListViewState state, int[] colWidths, string dragTitle) {
	...
	if (Event.current.type != EventType.Layout) {
		vRect = new Rect(0, state.scrollPos.y, GUIClip.visibleRect.width,
		...
		endRow = invisibleRows +
		         (int) System.Math.Ceiling(
			((vRect.yMin % state.rowHeight) + vRect.height) /
			state.rowHeight) -
		         1;
	...
	return new ListViewShared.ListViewElementsEnumerator(
		ilvState, colWidths, invisibleRows, endRow, dragTitle,
		new Rect(0, invisibleRows * state.rowHeight, vRect.width,
		         state.rowHeight));
}

private static void BeginLayoutedListview(ListViewState state, GUIStyle style,
                                          params GUILayoutOption[] options) {
	GUILayoutedListViewGroup g =
		(GUILayoutedListViewGroup) GUILayoutUtility.BeginLayoutGroup(
			style, null, typeof(GUILayoutedListViewGroup));
...
private void AddYRecursive(GUILayoutEntry e, float y) {
	// this looks kind of bad, but in 99% of cases it would only be one level
	// depth (e.g. few labels inside BeginHorizontal)
	e.rect.y += y;
...
public void AddY() {
	if (entries.Count > 0)
		AddYRecursive((GUILayoutEntry) entries[0],
		              ((GUILayoutEntry) entries[0]).minHeight);
}
...
static public bool MultiSelection(int prevSelected, int currSelected,
                                  ref int initialSelected,
                                  ref bool[] selectedItems) {
	return ListViewShared.MultiSelection(lvState.ilvState, prevSelected,
	                                     currSelected, ref initialSelected,
	                                     ref selectedItems);
}
