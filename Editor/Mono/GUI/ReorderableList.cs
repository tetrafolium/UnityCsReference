...
// TODO: make it not rely on GUILayout at all, so its safe to use under
// PropertyDrawers.
public class ReorderableList {
  public delegate void HeaderCallbackDelegate(Rect rect);
  public delegate void FooterCallbackDelegate(Rect rect);
  public delegate void ElementCallbackDelegate(Rect rect, int index,
  ...
  public delegate void ReorderCallbackDelegateWithDetails(ReorderableList list,
                                                          int oldIndex,
                                                          int newIndex);
  ...
  // layout callbacks
  // if supplying own element heights, try to cache the results as this may be
  // called frequently
  ...
  internal struct PropertyCacheEntry {
    ...
    public PropertyCacheEntry(SerializedProperty property, float height,
                              float offset) {
  ...
  // class for default rendering and behavior of reorderable list - stores
  // styles and is statically available as s_Defaults
  public class Defaults {
    public GUIContent iconToolbarPlus =
        EditorGUIUtility.TrIconContent("Toolbar Plus", "Add to list");
    public GUIContent iconToolbarPlusMore = EditorGUIUtility.TrIconContent(
        "Toolbar Plus More", "Choose to add to list");
    public GUIContent iconToolbarMinus = EditorGUIUtility.TrIconContent(
        "Toolbar Minus", "Remove selection or last element from list");
    ...
    private static GUIContent s_ListIsEmpty =
        EditorGUIUtility.TrTextContent("List is Empty");
    ...
    internal static readonly Rect infinityRect =
        new Rect(float.NegativeInfinity, float.NegativeInfinity,
    ...
    public void DrawFooter(Rect rect, ReorderableList list) {
      ...
      if (list.displayAdd) {
        using(new EditorGUI.DisabledScope(list.onCanAddCallback != null &&
                                          !list.onCanAddCallback(list))) {
          if (GUI.Button(addRect,
                         list.onAddDropdownCallback != null ? iconToolbarPlusMore
                         : iconToolbarPlus, preButton)) {
      ...
      if (list.displayRemove) {
        using(new EditorGUI.DisabledScope(list.index < 0 ||
                                          list.index >= list.count ||
                                          (list.onCanRemoveCallback != null &&
      ...
      if (list.serializedProperty != null) {
        if (list.serializedProperty.hasMultipleDifferentValues &&
            !EditorUtility.DisplayDialog(
                L10n.Tr(
                    "Changing size of the array will copy new value to all other selected objects."),
                L10n.Tr(
                    "Unique values in the different selected object array size properties will be lost"),
                L10n.Tr("OK"), L10n.Tr("Cancel")))
          return;

        SerializedProperty arraySize =
            list.m_Elements.FindPropertyRelative("Array.size");
        arraySize.intValue++;
        ...
        // this is ugly but there are a lot of cases like null types and default
        // constructors
        ...
        if (value != null)
          ...
          Debug.LogError(
              "Cannot add element. Type " + elementType +
          ...
          list.index = list.list.Add(Activator.CreateInstance(
              list.list.GetType().GetGenericArguments()[0]));
        else if (elementType != null)
    ...
    public void DoAddButton(ReorderableList list) { DoAddButton(list, null); }
      ...
      if (list.serializedProperty != null) {
        ...
        if (removeIndex < list.count - 1) {
          SerializedProperty currentProperty =
              list.serializedProperty.GetArrayElementAtIndex(removeIndex);
          for (int i = removeIndex + 1; i < list.count; i++) {
            SerializedProperty nextProperty =
                list.serializedProperty.GetArrayElementAtIndex(i);
    ...
    public void DrawHeaderBackground(Rect headerRect) {
      if (Event.current.type == EventType.Repaint) {
        // We assume that a height smaller than 5px means a header with no
        // content
    ...
    public void DrawHeader(Rect headerRect, SerializedObject serializedObject,
      ...
      EditorGUI.LabelField(
          headerRect, EditorGUIUtility.TempContent(
                          (element != null) ? "Serialized Property" : "IList"));
    }

    // draw the default element background
    public void DrawElementBackground(Rect rect, int index, bool selected,
    ...
    public void DrawElementDraggingHandle(Rect rect, int index, bool selected,
                                          bool focused, bool draggable) {
      if (Event.current.type == EventType.Repaint) {
        if (draggable)
          draggingHandle.Draw(new Rect(rect.x + 5, rect.y + 8, 10, 6), false,
                              false, false, false);
      }
    }

    // draw the default element
    public void DrawElement(Rect rect, SerializedProperty element,
                            System.Object listItem, bool selected, bool focused,
                            bool draggable) {
    ...
    public void DrawElement(Rect rect, SerializedProperty element,
      ...
      EditorGUI.LabelField(rect, EditorGUIUtility.TempContent(
  ...
  ClearExistingListCaches() => s_Instances.ForEach(list => list.ClearCache());
  ...
  public ReorderableList(IList elements, Type elementType, bool draggable,
                         bool displayHeader, bool displayAddButton,
                         bool displayRemoveButton) {
    InitList(null, null, elements, draggable, displayHeader, displayAddButton,
             displayRemoveButton);
  }
  ...
  public ReorderableList(SerializedObject serializedObject,
                         SerializedProperty elements, bool draggable,
                         bool displayHeader, bool displayAddButton,
                         bool displayRemoveButton) {
    InitList(serializedObject, elements, null, draggable, displayHeader,
             displayAddButton, displayRemoveButton);
  }

  private void InitList(SerializedObject serializedObject,
                        SerializedProperty elements, IList elementList,
  ...
  private float HeaderHeight {
    get { return Mathf.Max(headerHeight, Defaults.minHeaderHeight); }
  }

  private float listElementTopPadding =>
      headerHeight > 5 ? 4 : 1; // headerHeight is usually set to 3px when there
                                // is no header content. Therefore, we add a 1px
                                // top margin to match the 4px bottom margin
    ...
    while (m_Count > m_PropertyCache.Count) {
      ...
      if (m_Elements != null) {
        ...
        if (elementHeightCallback != null) {
          ...
          height = ScriptAttributeUtility.GetHandler(property).GetHeight(
              property, null, true);
        }
  ...
  internal void ClearCache() { m_PropertyCache.Clear(); }
  ...
  private Rect GetRowRect(int index, Rect listRect) {
    return new Rect(listRect.x, listRect.y + GetElementYOffset(index),
                    listRect.width, GetElementHeight(index));
  }
  ...
  public int count {
    get {
      if (m_Elements != null) {
        ...
        foreach (Object targetObject in m_Elements.serializedObject
                     .targetObjects) {
          using(SerializedObject serializedObject =
                    new SerializedObject(targetObject)) {
            SerializedProperty property =
                serializedObject.FindProperty(m_Elements.propertyPath);
  ...
  public void DoLayoutList() // TODO: better API?
  {
    ...
    Rect headerRect =
        GUILayoutUtility.GetRect(0, HeaderHeight, GUILayout.ExpandWidth(true));
    // Elements area
    Rect listRect = GUILayoutUtility.GetRect(10, GetListElementHeight(),
                                             GUILayout.ExpandWidth(true));
    // do the footer GUI
    Rect footerRect =
        GUILayoutUtility.GetRect(4, footerHeight, GUILayout.ExpandWidth(true));
  ...
  public void DoList(Rect rect, Rect visibleRect) // TODO: better API?
  {
    ...
    // Elements area
    Rect listRect = new Rect(rect.x, headerRect.y + headerRect.height,
                             rect.width, GetListElementHeight());
    // do the footer GUI
    Rect footerRect = new Rect(rect.x, listRect.y + listRect.height, rect.width,
                               footerHeight);
  ...
  private float GetListElementHeight() {
    float listElementPadding =
        kListElementBottomPadding + listElementTopPadding;
    ...
    CacheIfNeeded(true);
    return GetElementYOffset(m_Count - 1) + GetElementHeight(m_Count - 1) +
           listElementPadding;
  }
  ...
  private void DoListElements(Rect listRect, Rect visibleRect) {
    if ((drawElementCallback != null || m_HasPropertyDrawer) &&
        Event.current.type == EventType.Repaint && listRect != lastRect) {
      // Recalculate cache values in case their height changed due to window
      // resize
      lastRect = listRect;
      ClearCacheRecursive();
    }
    ...
    // How many elements? If none, make space for showing default line that
    // shows no elements are present
    m_Count = count;

    // draw the background in repaint
    if (showDefaultBackground && Event.current.type == EventType.Repaint)
      defaultBehaviours.boxBackground.Draw(listRect, false, false, false,
                                           false);

    // resize to the area that we want to draw our elements into
    listRect.yMin += listElementTopPadding;
    listRect.yMax -= kListElementBottomPadding;
    ...
    // the content rect is what we will actually draw into -- it doesn't include
    // the drag handle or padding
    var elementContentRect = elementRect;

    bool handlingInput = Event.current.type == EventType.MouseDown;

    if ((m_Elements != null && m_Elements.isArray || m_ElementList != null) &&
        m_Count > 0) {
      EditorGUI.BeginChangeCheck();
      // If there are elements, we need to draw them -- we will do this
      // differently depending on if we are dragging or not
      if (IsDragging() && Event.current.type == EventType.Repaint) {
        ...
        for (var i = 0; i < m_NonDragTargetIndices.Count; i++) {
          ...
          if (visibleRect.y + visibleRect.height <
              GetElementYOffset(i > 0 ? i - 1 : i))
            break;
          if (m_Elements != null)
            EnsureValidProperty(m_PropertyCache[i].property);
          ...
          if (nonDragTargetIndex != -1) {
            ...
            // update the position of the rect (based on element position and
            // accounting for sliding)
            elementRect.y = listRect.y + GetElementYOffset(nonDragTargetIndex,
                                                           m_ActiveElement);
            if (targetSeen) {
            ...
            if (drawElementBackgroundCallback == null)
              defaultBehaviours.DrawElementBackground(
                  elementRect, nonDragTargetIndex, false, false, m_Draggable);
            else
              drawElementBackgroundCallback(elementRect, nonDragTargetIndex,
                                            false, false);

            defaultBehaviours.DrawElementDraggingHandle(elementRect, i, false,
                                                        false, m_Draggable);

            elementContentRect = GetContentRect(elementRect);
            if (drawElementCallback == null) {
              if (m_Elements != null)
                s_Defaults.DrawElement(
                    elementContentRect,
                    m_PropertyCache[nonDragTargetIndex].property, null, false,
                    false, m_Draggable, m_IsEditable);
              else
                defaultBehaviours.DrawElement(
                    elementContentRect, null, m_ElementList[nonDragTargetIndex],
                    false, false, m_Draggable, m_IsEditable);
            } else {
              drawElementCallback(elementContentRect, nonDragTargetIndex, false,
                                  false);
            }
          } else {
            targetSeen = true;
        ...
        elementRect.y =
            GetClampedDragPosition(listRect) - m_DragOffset + listRect.y;
        elementRect.height = GetElementHeight(m_ActiveElement);

        // actually draw the element
        if (drawElementBackgroundCallback == null)
          defaultBehaviours.DrawElementBackground(elementRect, m_ActiveElement,
                                                  true, true, m_Draggable);
        else
          drawElementBackgroundCallback(elementRect, m_ActiveElement, true,
                                        true);

        defaultBehaviours.DrawElementDraggingHandle(
            elementRect, m_ActiveElement, true, true, m_Draggable);
        ...
        if (drawElementCallback == null) {
          if (m_Elements != null)
            s_Defaults.DrawElement(elementContentRect,
                                   m_PropertyCache[m_ActiveElement].property,
                                   null, true, true, m_Draggable, m_IsEditable);
          else
            defaultBehaviours.DrawElement(elementContentRect, null,
                                          m_ElementList[m_ActiveElement], true,
                                          true, m_Draggable, m_IsEditable);
        } else {
        ...
        for (int i = 0; i < m_Count; i++) {
          ...
          if (drawElementBackgroundCallback == null)
            defaultBehaviours.DrawElementBackground(
                elementRect, i, activeElement, focusedElement, m_Draggable);
          else
            drawElementBackgroundCallback(elementRect, i, activeElement,
                                          focusedElement);
          defaultBehaviours.DrawElementDraggingHandle(
              elementRect, i, activeElement, focusedElement, m_Draggable);
          ...
          if (drawElementCallback == null) {
            if (m_Elements != null) {
              s_Defaults.DrawElement(
                  elementContentRect, m_PropertyCache[i].property, null,
                  activeElement, focusedElement, m_Draggable, m_IsEditable);
            } else
              defaultBehaviours.DrawElement(
                  elementContentRect, null, m_ElementList[i], activeElement,
                  focusedElement, m_Draggable, m_IsEditable);
          } else {
            drawElementCallback(elementContentRect, i, activeElement,
                                focusedElement);
          }
      ...
      // draw the background
      if (drawElementBackgroundCallback == null)
        defaultBehaviours.DrawElementBackground(elementRect, -1, false, false,
                                                false);
      else
        drawElementBackgroundCallback(elementRect, -1, false, false);
      defaultBehaviours.DrawElementDraggingHandle(elementRect, -1, false, false,
                                                  false);
  ...
  private void DoListHeader(Rect headerRect) {
    ...
    if (drawHeaderCallback != null)
      ...
      defaultBehaviours.DrawHeader(headerRect, m_SerializedObject, m_Elements,
                                   m_ElementList);
  }
  ...
  private void DoListFooter(Rect footerRect) {
    ...
    if (drawFooterCallback != null)
      ...
      defaultBehaviours.DrawFooter(
          footerRect,
          this); // draw the footer if the add or remove buttons are required
  }
  ...
  private void DoDraggingAndSelection(Rect listRect) {
    ...
    case EventType.KeyDown:
      ...
      m_ActiveElement =
          Mathf.Clamp(m_ActiveElement, 0,
                      (m_Elements != null) ? m_Elements.arraySize - 1
    ...
    case EventType.MouseDown:
      ...
      if (m_Draggable && Event.current.button == 0) {
        // if we can drag, set the hot control and start dragging (storing the
        // offset)
      ...
      // Prevent consuming the right mouse event in order to enable the context
      // menu
    ...
    case EventType.MouseDrag:
      ...
      // Set m_Dragging state on first MouseDrag event after we got hotcontrol
      ...
      if (PropertyEditor.CurrentPropertyEditor != null) {
        Vector2 mousePoistion =
            new Vector2(0, Mathf.Clamp(evt.mousePosition.y, listRect.y,
                                       listRect.y + listRect.height));
        mousePoistion =
            GUIUtility.GUIToScreenPoint(mousePoistion) -
            new Vector2(0, PropertyEditor.CurrentPropertyEditor.m_Pos.y);
    ...
    case EventType.MouseUp:
      if (!m_Draggable) {
        // if mouse up was on the same index as mouse down we fire a mouse up
        // callback (useful if for beginning renaming on mouseup)
      ...
      try {
        ...
        if (m_ActiveElement != targetIndex) {
          ...
          if (m_SerializedObject != null && m_Elements != null) {
            Undo.RegisterCompleteObjectUndo(m_SerializedObject.targetObjects,
                                            Defaults.undoMove);

            // if we are working with Serialized Properties, we can handle it
            // for you
            m_Elements.MoveArrayElement(m_ActiveElement, targetIndex);
            m_SerializedObject.ApplyModifiedPropertiesWithoutUndo();
          } else if (m_ElementList != null) {
            // we are working with the IList, which is probably of a fixed
            // length
          ...
          if (m_SerializedObject != null && m_Elements != null) {
            SerializedProperty prop1 =
                m_Elements.GetArrayElementAtIndex(oldActiveElement);
            SerializedProperty prop2 =
                m_Elements.GetArrayElementAtIndex(newActiveElement);
          ...
          if (onReorderCallbackWithDetails != null)
            onReorderCallbackWithDetails(this, oldActiveElement,
                                         newActiveElement);
          else
        ...
        // (and thus an ExitGUIException thrown). We still need to cleanup
        // before we exitGUI proper.
        GUIUtility.hotControl = 0;
  ...
  bool IsMouseInsideActiveElement(Rect listRect) {
    ...
    return mouseRowIndex == m_ActiveElement &&
           GetRowRect(mouseRowIndex, listRect)
  ...
  private float GetClampedDragPosition(Rect listRect) {
    return Mathf.Clamp(m_DraggedY, m_DragOffset,
                       listRect.height - (GetElementHeight(m_ActiveElement)) +
                           m_DragOffset);
  }

  private int CalculateRowIndex(Rect listRect) {
    var rowIndex =
        GetRowIndex(GetClampedDragPosition(listRect) - m_DragOffset, true);

    var activeElementY = m_DraggedY - m_DragOffset;
    var position = activeElementY - GetElementYOffset(m_ActiveElement);

    if (position < 0) {
      while (rowIndex > 0 && GetElementYOffset(rowIndex - 1) +
                                     GetElementHeight(rowIndex - 1) / 2 >
                                 activeElementY)
        rowIndex--;
    }
  ...
  // or indexes of of elements that are not interacted with while drag is
  // happening
  private int GetRowIndex(float localY, bool skipActiveElement = false) {
    for (int i = 0; i < count; i++) {
      ...
      if (skipActiveElement) {
        if (i >= m_ActiveElement) {
          levelOffset +=
              -GetElementHeight(m_ActiveElement) + GetElementHeight(i) / 2;
  ...
  private bool IsDragging() { return m_Dragging; }
