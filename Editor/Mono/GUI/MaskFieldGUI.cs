...
internal static int DoMaskField(Rect position, int controlID, int mask,
	...
	int[] selectedOptions;
	GetMenuOptions(mask, flagNames, flagValues, out buttonText, out optionNames,
	               out optionMaskValues, out selectedOptions);

	Event evt = Event.current;
	if (evt.type == EventType.Repaint) {
		GUIContent buttonContent = EditorGUI.showMixedValue ? EditorGUI
		                           .mixedValueContent
	  : EditorGUIUtility.TempContent(buttonText);
	  ...
	  : selectedOptions,
			// optionMaskValues is from the pool so use a clone of the values
			// for the current control
			MaskCallbackInfo.m_Instance.SetMaskValueDelegate,
			optionMaskValues.Clone());
...
private static readonly HashSet<int> s_SelectedOptionsSet =
	new HashSet<int>();
...
internal static void GetMenuOptions(int mask, string[] flagNames,
                                    int[] flagValues, out string buttonText,
                                    out string[] optionNames,
                                    out int[] optionMaskValues,
                                    out int[] selectedOptions) {
	...
	var everythingName = (hasEverythingName ? flagNames[flagValues.Length - 1]
	                  : "Everything");

	var optionCount =
		flagNames.Length + (hasNothingName ? 0 : 1) + (hasEverythingName ? 0 : 1);
	var flagCount =
		flagNames.Length - (hasNothingName ? 1 : 0) - (hasEverythingName ? 1 : 0);
	...
	if (mask == 0)
		...
		for (var flagIndex = flagStartIndex; flagIndex < flagEndIndex;
		     flagIndex++) {
	...
	var intermediateMask =
		0; // Mask used to compute new mask value for each option
	...
	for (var flagIndex = flagStartIndex; flagIndex < flagEndIndex;
		...
		var newMask = (flagSet ? intermediateMask & ~flagValue
		     : intermediateMask | flagValue);

		// If all flag options are selected the mask becomes ~0 to be consistent
		// with the "Everything" option
		if (newMask == flagMask)
			newMask = ~0;
