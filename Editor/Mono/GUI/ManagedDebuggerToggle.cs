...
internal class ManagedDebuggerToggle {
  ...
  public ManagedDebuggerToggle() {
    m_DebuggerAttachedContent =
        EditorGUIUtility.TrIconContent("DebuggerAttached", "Debugger Attached");
    m_DebuggerDisabledContent =
        EditorGUIUtility.TrIconContent("DebuggerDisabled", "Debugger Disabled");
    m_DebuggerEnabledContent =
        EditorGUIUtility.TrIconContent("DebuggerEnabled", "Debugger Enabled");
  ...
  public void OnGUI() {
    using(new EditorGUI.DisabledScope(!ManagedDebugger.isEnabled)) {
      ...
      if (GUI.Button(rect, content, style)) {
        PopupWindow.Show(rect, new ManagedDebuggerWindow(codeOptimization),
                         m_PopupLocation);
        GUIUtility.ExitGUI();
      }
    }
  ...
  private GUIContent GetDebuggerContent(bool debuggerAttached,
                                        CodeOptimization optimization) {
    if (CodeOptimization.Debug == optimization) {
      return debuggerAttached ? m_DebuggerAttachedContent
          : m_DebuggerEnabledContent;
    }
