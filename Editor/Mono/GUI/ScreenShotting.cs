...
[MenuItem("Window/Internal/Screenshot/Set Window Size %&l", false, 1000,
          true)]
...
[MenuItem("Window/Internal/Screenshot/Set Window Size Small", false, 1000,
          true)]
...
[MenuItem("Window/Internal/Screenshot/Snap View Extended Right %&k", false,
          1000, true)]
...
public static void ScreenShotComponent(Rect contentRect, Object target) {
	...
	contentRect.yMax += 2;
	contentRect.xMin += 1;
	ScreenShots.SaveScreenShotWithBorder(contentRect, kWindowBorderColor,
	                                     target.GetType().Name + "Inspector");
}

[MenuItem("Window/Internal/Screenshot/Snap Game View Content", false, 1000,
          true)]
...
public static void SaveScreenShot(Rect r, string name) {
	SaveScreenShot((int) r.width, (int) r.height,
	               InternalEditorUtility.ReadScreenPixel(
			       new Vector2(r.x, r.y), (int) r.width, (int) r.height),
	               name);
}

// Adds a gray border around the screenshot
// Useful for e.g. toolbars because they don't have a nice border all the way
// round due to the tabs
public static string SaveScreenShotWithBorder(Rect r, Color borderColor,
                                              string name) {
	int w = (int) r.width;
	int h = (int) r.height;
	Color[] colors1 =
		InternalEditorUtility.ReadScreenPixel(new Vector2(r.x, r.y), w, h);
	...
	return SaveScreenShot((int)(r.width + 2), (int)(r.height + 2), colors2,
	                      name);
}

static string SaveScreenShot(int width, int height, Color[] pixels,
                             string name) {
...
static string GetUniquePathForName(string name) {
	string path =
		string.Format("{0}/../../{1}.png", Application.dataPath, name);
	int i = 0;
	while (System.IO.File.Exists(path)) {
		path = string.Format("{0}/../../{1}{2:000}.png", Application.dataPath,
		                     name, i);
		i++;
	}
	return path;
