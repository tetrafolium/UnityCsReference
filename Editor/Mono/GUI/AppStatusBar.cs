...
internal class AppStatusBar : GUIView {
  static readonly NumberFormatInfo percentageFormat =
      new CultureInfo("en-US", false).NumberFormat;

  internal static class Styles {
    public const int spacing = 4;

    public static readonly GUILayoutOption progressBarWidth =
        GUILayout.Width(180);
    public static readonly GUILayoutOption progressLabelMaxWidth =
        GUILayout.MaxWidth(230);
    public static readonly GUIStyle background = "AppToolbar";
    public static readonly GUIStyle statusLabel =
        EditorStyles.FromUSS(EditorStyles.label, "status-bar-label");
    public static readonly GUIStyle statusIcon = "StatusBarIcon";

    public static readonly GUIStyle progressBar = EditorStyles.FromUSS(
        EditorStyles.progressBarBar, "status-bar-progress");
    public static readonly GUIStyle progressBarAlmostDone =
        EditorStyles.FromUSS(progressBar, "status-bar-progress-almost-done");
    public static readonly GUIStyle progressBarUnresponsive =
        EditorStyles.FromUSS(progressBar, "status-bar-progress-unresponsive");
    public static readonly GUIStyle progressBarIndefinite =
        EditorStyles.FromUSS(progressBar, "status-bar-progress-indefinite");
    public static readonly GUIStyle progressBarBack = EditorStyles.FromUSS(
        EditorStyles.progressBarBack, "status-bar-progress-background");
    public static readonly GUIStyle progressBarText =
        EditorStyles.FromUSS(statusLabel, "status-bar-progress-text");

    public static readonly GUIContent[] statusWheel;
    public static readonly GUIContent assemblyLock = EditorGUIUtility.IconContent(
        "AssemblyLock",
        "|Assemblies are currently locked. Compilation will resume once they are unlocked");
    public static readonly GUIContent progressIcon =
        EditorGUIUtility.TrIconContent("Progress", "Show progress details");
    public static readonly GUIContent progressHideIcon =
        EditorGUIUtility.TrIconContent("Progress", "Hide progress details");
    public static readonly GUIContent autoGenerateLightOn =
        EditorGUIUtility.TrIconContent("AutoLightbakingOn",
                                       "Auto Generate Lighting On");
    public static readonly GUIContent autoGenerateLightOff =
        EditorGUIUtility.TrIconContent("AutoLightbakingOff",
    ...
    static Styles() {
      ...
      for (int i = 0; i < 12; i++)
        statusWheel[i] =
            EditorGUIUtility.IconContent("WaitSpin" + i.ToString("00"));
    }
  }
  ...
  protected override void OnEnable() {
    ...
    m_DrawExtraFeatures =
        ModeService.HasCapability(ModeCapability.StatusBarExtraFeatures, true);
  ...
  private void CheckProgressUnresponsive() {
    if (Progress.running) {
      var unresponsiveItem =
          Progress.EnumerateItems().FirstOrDefault(item => !item.responding);
  ...
  protected override void OldOnGUI() {
    ...
    if (Event.current.type == EventType.Repaint)
      Styles.background.Draw(new Rect(0, 0, position.width, position.height),
  ...
  private void DrawRefreshStatus() {
    ...
    if (compiling || showProgress) {
      ...
      if (GUILayout.Button(canHide ? Styles.progressHideIcon
                           : Styles.progressIcon, Styles.statusIcon)) {
        if (canHide)
  ...
  private void DrawProgressBar() {
    ...
    // Define the progress styles to be used based on the current progress
    // state.
    ...
    var progressRect = EditorGUILayout.GetControlRect(false, position.height,
                                                      Styles.progressBarBack,
                                                      Styles.progressBarWidth);
    if (EditorGUI.ProgressBar(progressRect, globalProgress, progressBarContent,
                              true, Styles.progressBarBack, progressBarStyle,
  ...
  private void DrawBakeMode() {
    ...
    if (GUILayout.Button(GetBakeModeIcon(m_autoLightBakingOn),
                         Styles.statusIcon)) {
      Event.current.Use();
  ...
  void DrawDebuggerToggle() { m_ManagedDebuggerToggle.OnGUI(); }

  void DrawCacheServerToggle() { m_CacheServerToggle.OnGUI(); }
  ...
  private void DrawStatusText() {
    ...
    GUILayout.Label(
        statusText, errorStyle,
        GetStatusTextLayoutOption((icon != null ? icon.width : 0) + 6));
  ...
  private void RefreshProgressBar(Progress.Item[] progressItems) {
    ...
    if (taskCount == 0) {
      ...
      if (!String.IsNullOrEmpty(currentItem.description))
        m_ProgressStatus.tooltip =
            currentItem.name + "\r\n" + currentItem.description;
      m_ProgressPercentageStatus.text =
          Progress.globalProgress.ToString("P", percentageFormat);

      var remainingTimeText = "";
      if (progressItems.Any(item =>
                                item.timeDisplayMode ==
                                Progress.TimeDisplayMode.ShowRemainingTime) &&
          progressItems.All(item => !item.indefinite) &&
          Progress.globalRemainingTime.TotalSeconds > 0) {
        remainingTimeText = $" [{Progress.globalRemainingTime:g}]";
      }

      if (taskCount > 1)
        m_ProgressStatus.text =
            $"Multiple tasks ({taskCount}){remainingTimeText}";
      else
  ...
  private void RepaintProgress(Progress.Item[] progressItems) {
    ...
    foreach (var item in progressItems) {
      if ((item.options & Progress.Options.Synchronous) ==
  ...
  private GUILayoutOption GetStatusTextLayoutOption(float consoleIconWidth) {
    int iconWidth = 25;
    float specialModeLabelWidth =
        Styles.statusLabel.CalcSize(new GUIContent(m_SpecialModeLabel)).x +
        k_SpaceBeforeProgress + 8;
    float statusRightReservedSpace = specialModeLabelWidth +
    ...
    return GUILayout.MaxWidth(position.width - statusRightReservedSpace -
                              k_SpaceBeforeProgress - 8 -
                              (float) Styles.progressBarWidth.value -
                              k_SpaceAfterProgress - consoleIconWidth);
  }
