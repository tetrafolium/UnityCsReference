...
internal class GradientPicker : EditorWindow {
  ...
  public static void Show(Gradient newGradient, bool hdr,
                          System.Action<Gradient> onGradientChanged) {
    PrepareShow(hdr);
  ...
  static void PrepareShow(bool hdr) {
    if (s_GradientPicker == null) {
      ...
      s_GradientPicker = (GradientPicker)
          GetWindow(typeof(GradientPicker), true, title, false);
      ...
      s_GradientPicker
          .ShowAuxWindow(); // Use this if auto close on lost focus is wanted.
    } else {
      s_GradientPicker
          .Repaint(); // Ensure we get a OnGUI so we refresh if new gradient
    }
  }
  ...
  public void OnDestroy() { m_GradientLibraryEditor.UnloadUsedLibraries(); }

  void OnPlayModeStateChanged() { Close(); }
    ...
    if (m_GradientLibraryEditor == null) {
      var saveLoadHelper =
          new ScriptableObjectSaveLoadHelper<GradientPresetLibrary>(
              "gradients", SaveType.Text);
      m_GradientLibraryEditor = new PresetLibraryEditor<GradientPresetLibrary>(
          saveLoadHelper, m_GradientLibraryEditorState, PresetClickedCallback);
  ...
  public void OnGUI() {
    // When we start play (using shortcut keys) we get two OnGui calls and
    // m_Gradient is null: so early out.
    if (m_Gradient == null)
      return;
    ...
    float presetLibraryHeight =
        position.height - gradientEditorHeight - distBetween;

    Rect gradientEditorRect =
        new Rect(10, 10, position.width - 20, gradientEditorHeight - 20);
    Rect gradientLibraryRect = new Rect(0, gradientEditorHeight + distBetween,
                                        position.width, presetLibraryHeight);

    // Separator
    EditorGUI.DrawRect(new Rect(gradientLibraryRect.x,
                                gradientLibraryRect.y - 1,
                                gradientLibraryRect.width, 1),
                       new Color(0, 0, 0, 0.3f));
    EditorGUI.DrawRect(new Rect(gradientLibraryRect.x, gradientLibraryRect.y,
                                gradientLibraryRect.width, 1),
