...
internal class CreateAssetUtility {
  [SerializeField] EndNameEditAction m_EndAction;
  [SerializeField] int m_InstanceID;
  [SerializeField] string m_Path = "";
  [SerializeField] Texture2D m_Icon;
  [SerializeField] string m_ResourceFile;
  ...
  public bool BeginNewAssetCreation(int instanceID,
                                    EndNameEditAction newAssetEndAction,
                                    string filePath, Texture2D icon,
                                    string newAssetResourceFile) {
    // Sanitize input
    string sanitizedFilePath =
        filePath != null ? filePath.ConvertSeparatorsToUnity() : filePath;
    string sanitizedNewAssetResourceFile =
        newAssetResourceFile !=
        null ? newAssetResourceFile.ConvertSeparatorsToUnity()
        : newAssetResourceFile;

    string uniquePath;
    if (!sanitizedFilePath.StartsWith(
            "assets/", System.StringComparison.CurrentCultureIgnoreCase)) {
      uniquePath =
          AssetDatabase.GetUniquePathNameAtSelectedPath(sanitizedFilePath);
    } else {
    ...
    if (!IsPathDataValid(uniquePath)) {
      Debug.LogErrorFormat(
          "Invalid generated unique path '{0}' (input path '{1}')", uniquePath,
          sanitizedFilePath);
      Clear();
      return false;
    }
  ...
  public void EndNewAssetCreation(string name) {
    ...
    Clear(); // Ensure clear if anything goes bad in EndNameEditAction and gui
             // is exited.

    ProjectWindowUtil.EndNameEditAction(endAction, instanceID, path,
                                        resourceFile, true);
  }

  public void EndNewAssetCreationCanceled(string name) {
    string path = folder + "/" + name + extension;
    ProjectWindowUtil.EndNameEditAction(m_EndAction, m_InstanceID, path,
                                        m_ResourceFile, false);
  }

  public bool IsCreatingNewAsset() { return !string.IsNullOrEmpty(m_Path); }
}
