...
public sealed partial class EditorGUI {
  internal class VUMeter {
    ...
    public static Texture2D horizontalVUTexture {
      get {
        if (s_HorizontalVUTexture == null)
          s_HorizontalVUTexture =
              EditorGUIUtility.LoadIcon("VUMeterTextureHorizontal");
        return s_HorizontalVUTexture;
      }
    }

    public static void HorizontalMeter(Rect position, float value, float peak,
      ...
      if (width < 2)
        width = 2;
      Rect newRect =
          new Rect(position.x + 1, position.y + 1, width, position.height - 2);
      ...
      if (peakpos < 2)
        peakpos = 2;
      newRect = new Rect(position.x + peakpos, position.y + 1, 1,
                         position.height - 2);
    ...
    public static void VerticalMeter(Rect position, float value, float peak,
                                     Texture2D foregroundTexture,
                                     Color peakColor) {
      if (Event.current.type != EventType.Repaint)
        return;
      ...
      if (height < 2)
        height = 2;
      Rect newRect =
          new Rect(position.x + 1, (position.y + position.height - 1) - height,
      ...
      if (peakpos < 2)
        peakpos = 2;
      newRect =
          new Rect(position.x + 1, (position.y + position.height - 1) - peakpos,
                   position.width - 2, 1);
      GUI.DrawTexture(newRect, EditorGUIUtility.whiteTexture,
                      ScaleMode.StretchToFill);
    ...
    public static void HorizontalMeter(Rect position, float value,
                                       ref SmoothingData data,
                                       Texture2D foregroundTexture,
                                       Color peakColor) {
      if (Event.current.type != EventType.Repaint)
        return;
      ...
      HorizontalMeter(position, renderValue, renderPeak, foregroundTexture,
                      peakColor);
    }

    // Auto smoothing version
    public static void VerticalMeter(Rect position, float value,
                                     ref SmoothingData data,
                                     Texture2D foregroundTexture,
                                     Color peakColor) {
      if (Event.current.type != EventType.Repaint)
      ...
      float renderValue, renderPeak;
      SmoothVUMeterData(ref value, ref data, out renderValue, out renderPeak);
      VerticalMeter(position, renderValue, renderPeak, foregroundTexture,
                    peakColor);
    }

    static void SmoothVUMeterData(ref float value, ref SmoothingData data,
                                  out float renderValue, out float renderPeak) {
