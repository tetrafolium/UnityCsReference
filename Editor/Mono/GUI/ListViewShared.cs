...
internal class ListViewShared {
  ...
  /// *undocumented*
  static bool DoLVPageUpDown(InternalListViewState ilvState,
                             ref int selectedRow, ref Vector2 scrollPos,
                             bool up) {
  ...
  /// *undocumented*
  static internal bool ListViewKeyboard(InternalListViewState ilvState,
                                        int totalCols) {
  ...
  internal static bool SendKey(InternalListViewState ilvState, KeyCode keyCode,
                               int totalCols) {
    var state = ilvState.state;

    int previousRow = state.row;

    // ilvState.state.row, ref ilvState.state.column, ref
    // ilvState.state.scrollPos
    switch (keyCode) {
    ...
    case KeyCode.PageDown: {
      if (!DoLVPageUpDown(ilvState, ref state.row, ref state.scrollPos,
                          false)) {
        Event.current.Use();
        return false;
      }
    ...
    case KeyCode.A: // must evade the return false and be handled by
                    // MultiSelection if needed
    ...
    if (ilvState.wantsRowMultiSelection)
      MultiSelection(ilvState,
                     previousRow); // scrollPos is set during MultiSelection
    else
      state.scrollPos =
          ListViewScrollToRow(ilvState, state.scrollPos, state.row);
    Event.current.Use();
    return true;
  }

  internal static void MultiSelection(InternalListViewState ilvState,
                                      int previousRow) {
    var state = ilvState.state;
    ...
    MultiSelection(ilvState, previousRow, state.row, ref state.initialRow,
                   ref state.selectedItems);
  }
  ...
  static internal bool MultiSelection(InternalListViewState ilvState,
                                      int prevSelected, int currSelected,
                                      ref int initialSelected,
                                      ref bool[] selectedItems) {
    ...
    // Select all : Ctrl + A
    bool selectAll = ctrlIsDown && Event.current.type == EventType.KeyDown &&
  ...
  static internal Vector2 ListViewScrollToRow(InternalListViewState ilvState,
                                              int row) {
    return ListViewScrollToRow(ilvState, ilvState.state.scrollPos, row);
  }

  static internal int ListViewScrollToRow(InternalListViewState ilvState,
                                          int currPosY, int row) {
    return (int) ListViewScrollToRow(ilvState, new Vector2(0, currPosY), row).y;
  }

  static internal Vector2 ListViewScrollToRow(InternalListViewState ilvState,
                                              Vector2 currPos, int row) {
    ...
    if (currPos.y < 0)
      currPos.y = 0;
    else if (currPos.y > ilvState.state.totalRows * ilvState.state.rowHeight -
                             ilvState.rectHeight)
      currPos.y = ilvState.state.totalRows * ilvState.state.rowHeight -
                  ilvState.rectHeight;
  ...
  internal class ListViewElementsEnumerator : IEnumerator<ListViewElement> {
    ...
    internal ListViewElementsEnumerator(InternalListViewState ilvState,
                                        int[] colWidths, int yFrom, int yTo,
                                        string dragTitle, Rect firstRect) {
    ...
    public bool MoveNext() {
      if (xPos > -1) {
        if (ListViewShared.HasMouseDown(ilvState, rect)) {
          ...
          ilvState.state.scrollPos = ListViewShared.ListViewScrollToRow(
              ilvState, yPos); // this is about clicking on a row that is
                               // partially visible
          ...
          if ((ilvState.wantsReordering || ilvState.wantsToStartCustomDrag) &&
            ...
            DragAndDropDelay delay =
                (DragAndDropDelay) GUIUtility.GetStateObject(
                    typeof(DragAndDropDelay), ilvState.state.ID);
        ...
        if (!ListViewShared.isDragging &&
          ...
          if (delay.CanStartDrag()) {
            ...
            DragAndDrop.objectReferences =
                new UnityEngine
                    .Object[]{}; // this IS required for dragging to work
            DragAndDrop.paths = null;

            if (ilvState.wantsReordering) {
              ilvState.state.dropHereRect =
                  new Rect(ilvState.rect.x, 0, ilvState.rect.width,
                           ilvState.state.rowHeight * 2);
      ...
      if (isLayouted && Event.current.type == EventType.Layout) {
        // this is just "on layout event enumerate just first row" (so we get
      ...
      if (quiting) {
        if (ilvState.state.drawDropHere &&
            Event.current.GetTypeForControl(ilvState.state.ID) ==
                EventType.Repaint) {
          GUIStyle insertion = Constants.insertion;
          insertion.Draw(insertion.margin.Remove(ilvState.state.dropHereRect),
        ...
        if (Event.current.GetTypeForControl(ilvState.state.ID) ==
            EventType.MouseUp) {
          GUIUtility.hotControl = 0;
        }
        ...
        if (ilvState.wantsReordering &&
          ...
          case EventType.DragUpdated: {
            DragAndDrop.visualMode =
                ilvState.rect.Contains(Event.current.mousePosition)
            ...
            if (DragAndDrop.visualMode != DragAndDropVisualMode.None) {
              lv.dropHereRect.y =
                  (Mathf.RoundToInt(Event.current.mousePosition.y /
                                    lv.rowHeight) -
                   1) *
                  lv.rowHeight;
          ...
          case EventType.DragPerform: {
            if (GUIClip.visibleRect.Contains(Event.current.mousePosition)) {
              ...
              ilvState.state.draggedTo = Mathf.RoundToInt(
                  Event.current.mousePosition.y / lv.rowHeight);
          ...
          case EventType.DragUpdated: {
            if ((GUIClip.visibleRect.Contains(Event.current.mousePosition)) &&
                (DragAndDrop.paths != null) &&
                (DragAndDrop.paths.Length !=
                 0)) // dragging files from somewhere
            {
              DragAndDrop.visualMode =
                  ilvState.rect.Contains(Event.current.mousePosition)
              ...
              if (DragAndDrop.visualMode != DragAndDropVisualMode.None) {
                ilvState.state.dropHereRect =
                    new Rect(ilvState.rect.x,
                             (Mathf.RoundToInt(Event.current.mousePosition.y /
                                               ilvState.state.rowHeight) -
                              1) *
                                 ilvState.state.rowHeight,
                             ilvState.rect.width, ilvState.state.rowHeight);

                if (ilvState.state.dropHereRect.y >=
                    ilvState.state.rowHeight * ilvState.state.totalRows)
                  ilvState.state.dropHereRect.y =
                      ilvState.state.rowHeight * (ilvState.state.totalRows - 1);
          ...
          case EventType.DragPerform: {
            if (GUIClip.visibleRect.Contains(Event.current.mousePosition)) {
              ...
              ilvState.state.drawDropHere = false;
              ilvState.state.draggedTo = Mathf.RoundToInt(
                  Event.current.mousePosition.y / ilvState.state.rowHeight);
              if (ilvState.state.draggedTo > ilvState.state.totalRows)
                ilvState.state.draggedTo = ilvState.state.totalRows;
              ilvState.state.row = ilvState.state.draggedTo;
            }
        ...
        } else if (ilvState.wantsToAcceptCustomDrag &&
                   (dragControlID != ilvState.state.ID)) {
          switch (Event.current.type) {
          case EventType.DragUpdated: {
        ...
        if (yPos != yFrom) {
          ...
          ilvStateL.group.AddY(ilvState.invisibleRows *
                               ilvState.state.rowHeight);
      }
