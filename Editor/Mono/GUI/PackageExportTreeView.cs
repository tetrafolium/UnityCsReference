...
internal class PackageExportTreeView {
  ...
  List<PackageExportTreeViewItem> m_Selection =
      new List<PackageExportTreeViewItem>();
  ...
  public ExportPackageItem[] items {
    get { return m_PackageExport.items; }
  }

  public PackageExportTreeView(PackageExport packageExport,
  ...
  void RecursiveComputeEnabledStateForFolders(
      PackageExportTreeViewItem pitem,
      HashSet<PackageExportTreeViewItem> done) {
    if (!pitem.isFolder)
      return;
    ...
    if (pitem.hasChildren) {
      foreach (var child in pitem.children) {
        RecursiveComputeEnabledStateForFolders(
            child as PackageExportTreeViewItem, done);
      }
    }
  ...
  EnabledState GetFolderChildrenEnabledState(PackageExportTreeViewItem folder) {
    ...
    if (amount == EnabledState.NotSet) {
      amount =
          initial == EnabledState.All ? EnabledState.All : EnabledState.None;
    }
  ...
  public void OnGUI(Rect rect) {
    ...
    if (Event.current.type == EventType.KeyDown &&
      ...
      EnabledState newEnabled =
          m_Selection[0].enabledState !=
          EnabledState.All ? EnabledState.All : EnabledState.None;
  ...
  private class PackageExportTreeViewItem : TreeViewItem {
    ...
    public EnabledState enabledState {
      get {
        return item != null ?(EnabledState) item.enabledStatus : m_EnabledState;
      }
      set {
        if (item != null)
    ...
    // We assume that items that don't have ExportPackageItem assigned, are
    // folders.
    public bool isFolder {
      get { return item != null ? item.isFolder : true; }
    }

    public PackageExportTreeViewItem(ExportPackageItem itemIn, int id,
  ...
  private class PackageExportTreeViewGUI : TreeViewGUI {
    ...
    public PackageExportTreeViewGUI(TreeViewController treeView,
                                    PackageExportTreeView view)
        : base(treeView) {
    ...
    static void Toggle(ExportPackageItem[] items,
                       PackageExportTreeViewItem pitem, Rect toggleRect) {
      bool enabled = pitem.enabledState > EnabledState.None;

      GUIStyle style = EditorStyles.toggle;
      bool setMixed =
          pitem.isFolder && (pitem.enabledState == EnabledState.Mixed);
      if (setMixed)
    ...
    void DoIconAndText(PackageExportTreeViewItem item, Rect contentRect,
                       bool selected, bool focused) {
  ...
  private class PackageExportTreeViewDataSource : TreeViewDataSource {
    ...
    public PackageExportTreeViewDataSource(TreeViewController treeView,
                                           PackageExportTreeView view)
        : base(treeView) {
      m_PackageExportView = view;
    ...
    public override void FetchData() {
      ...
      m_RootItem =
          new PackageExportTreeViewItem(null, "Assets".GetHashCode(), rootDepth,
      ...
      Dictionary<string, PackageExportTreeViewItem> treeViewFolders =
          new Dictionary<string, PackageExportTreeViewItem>();
      for (int i = 0; i < items.Length; i++) {
        ...
        string filename =
            Path.GetFileName(item.assetPath).ConvertSeparatorsToUnity();
        string folderPath =
            Path.GetDirectoryName(item.assetPath).ConvertSeparatorsToUnity();
        ...
        if (targetFolder != null) {
          ...
          var newItem = new PackageExportTreeViewItem(
    ...
    TreeViewItem EnsureFolderPath(
      ...
      for (int depth = 0; depth < splitPath.Length; ++depth) {
        ...
        if (treeViewFolders.TryGetValue(currentPath, out foundItem)) {
          ...
          var folderItem = new PackageExportTreeViewItem(null, id, folderDepth,
                                                         currentItem, folder);
