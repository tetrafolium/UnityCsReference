...
public ItemData(GUIContent content, GUIStyle style, bool on, bool enabled,
                object userData) {
...
public float widthOfButtonsAndLabel {
	get { return m_WidthOfButtons + labelAndSpacingWidth; }
}
public float widthOfPopupAndLabel {
	get { return m_PopupButtonSize.x + labelAndSpacingWidth; }
}
public bool rightAligned {
	get;
	set;
}
float labelAndSpacingWidth {
	get {
		return m_WidthOfLabel > 0 ? m_WidthOfLabel + m_SpacingLabelToButton : 0f;
	}
}
bool hasLabel {
	get { return m_Label != null && m_Label != GUIContent.none; }
}

public void Init(List<ItemData> items, float itemSpacing,
                 float minWidthOfPopup, PopupButtonData popupButtonData,
                 System.Action<ItemData> selectionChangedCallback) {
	Init(GUIContent.none, items, itemSpacing, minWidthOfPopup, popupButtonData,
	     selectionChangedCallback);
}

public void Init(GUIContent label, List<ItemData> items, float itemSpacing,
                 float minWidthOfPopup, PopupButtonData popupButtonData,
                 System.Action<ItemData> selectionChangedCallback) {
	m_Label = label;
	m_Items = items;
...
public float OnGUI(Rect rect) {
	// To ensure we allocate a consistent amount of controlIDs on every OnGUI we
	// preallocate before any logic
	if (m_Items.Count > 0 &&
	    (m_ItemControlIDs == null || m_ItemControlIDs.Length != m_Items.Count))
		m_ItemControlIDs = new int[m_Items.Count];
	for (int i = 0; i < m_Items.Count; ++i)
		m_ItemControlIDs[i] =
			GUIUtility.GetControlID(s_ItemHash, FocusType.Passive);
	m_DropDownButtonControlID =
		GUIUtility.GetControlID(m_DropDownButtonHash, FocusType.Passive);

	if (rect.width >= widthOfButtonsAndLabel &&
	    rect.width > m_MinWidthOfPopup) {
		// Show as buttons
		...
		for (int i = 0; i < m_Items.Count; ++i) {
			...
			using (new EditorGUI.DisabledScope(!item.m_Enabled)) {
				GUI.Toggle(buttonRect, m_ItemControlIDs[i], item.m_On,
				           item.m_GUIContent, item.m_Style);
			}

			if (EditorGUI.EndChangeCheck()) {
				SelectionChanged(item);
				GUIUtility.ExitGUI(); // To make sure we can survive if m_Buttons are
				                      // reallocated in the callback we exit gui
			}
		...
		if (hasLabel) {
			...
			GUI.Label(labelRect, m_Label, EditorStyles.boldLabel);
			dropDownRect.x =
				labelRect.x + (m_WidthOfLabel + m_SpacingLabelToButton);
		} else {
			if (rightAligned)
		...
		dropDownRect.width =
			Mathf.Clamp(dropDownRect.width, 0, m_PopupButtonSize.x);
		dropDownRect.height = m_PopupButtonSize.y;
		dropDownRect.y = rect.y + (rect.height - dropDownRect.height) / 2;

		if (EditorGUI.DropdownButton(m_DropDownButtonControlID, dropDownRect,
		                             m_PopupButtonData.m_GUIContent,
		                             m_PopupButtonData.m_Style))
...
void CalcWidths() {
	...
	// Popup
	m_PopupButtonSize =
		m_PopupButtonData.m_Style.CalcSize(m_PopupButtonData.m_GUIContent);
...
static internal void Show(Rect activatorRect, List<ItemData> buttonData,
                          ExposablePopupMenu caller) {
	m_Data = buttonData;
	m_Caller = caller;
