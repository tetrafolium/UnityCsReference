...
public sealed partial class EditorGUI {
  ...
  internal sealed class GUIContents {
    ...
    // We use a static constructor to lazily initialize all static properties.
    // This is useful because changed image files can then be picked up on
    // assembly reload.
    static GUIContents() {
      ...
      foreach (PropertyInfo property in propertyInfos) {
        IconName[] iconNames =
            (IconName[]) property.GetCustomAttributes(typeof(IconName), false);
    ...
    private class IconName : System.Attribute {
      ...
      public IconName(string name) { this.m_Name = name; }
