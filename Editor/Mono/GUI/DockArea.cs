...
internal class DockArea : HostView, IDropArea {
  private static class Styles {
    ...
    public static readonly float tabMinWidth =
        tab.GetFloat(StyleCatalogKeyword.minWidth, 50.0f);
    public static readonly float tabMaxWidth =
        tab.GetFloat(StyleCatalogKeyword.maxWidth, 150.0f);
    public static readonly float tabWidthPadding =
        tab.GetFloat(StyleCatalogKeyword.paddingRight);

    public static readonly float tabDragWidth =
        EditorResources.GetStyle("tab-drag")
            .GetFloat(StyleCatalogKeyword.width, 100.0f);

    public static readonly GUIStyle tabScrollerPrevButton =
        new GUIStyle("dragtab scroller prev");
    public static readonly GUIStyle tabScrollerNextButton =
        new GUIStyle("dragtab scroller next");

    public static SVC<float> genericMenuTopOffset =
        new SVC<float>("--window-generic-menu-top-offset", 20f);
    public static SVC<float> genericMenuFloatingTopOffset =
        new SVC<float>("--window-floating-generic-menu-top-offset", 20f);

    public static readonly GUIStyle tabLabel =
        new GUIStyle("dragtab"){name = "dragtab-label"};
    public static readonly GUIStyle dragTab = new GUIStyle("dragtab");
    public static readonly GUIStyle dragTabFirst =
        new GUIStyle("dragtab first");
    public static readonly GUIStyle dockTitleBarStyle =
        new GUIStyle("dockHeader");
  }
  ...
  // Mouse coords when we started the drag (used to figure out when we should
  // trigger a drag)
  static Vector2 s_StartDragPosition;
  // Are we dragging yet?
  static int s_DragMode;
  // A view that shouldn't be docked to (to make sure we don't attach to a
  // single view with only the tab that we're dragging)
  static internal View s_IgnoreDockingForView = null;

  private static DropInfo s_DropInfo = null;
  private static Dictionary<GUIContentKey, GUIContent> s_GUIContents =
      new Dictionary<GUIContentKey, GUIContent>();
  ...
  protected override void OnDestroy() {
    // Prevents double-destroy that may be indirectly caused if Close() is
    // called by OnLostFocus()
    ...
    // Since m_Panes can me modified indirectly by OnDestroy() callbacks, make a
    // copy of it for safe iteration
    var windows = new List<EditorWindow>(m_Panes);

    foreach (EditorWindow w in windows) {
      // Avoid destroying a window that has already being destroyed (case
      // 967778)
      if (w == null)
        continue;
  ...
  protected override void OnEnable() {
    if (m_Panes != null) {
      if (m_Panes.Count == 0)
        ...
        // before fix for case 840151, it is possible that out of range selected
        // index was serialized (e.g., case 846859)
  ...
  public void RemoveTab(EditorWindow pane) {
    RemoveTab(pane, killIfEmpty : true);
  }
  public void RemoveTab(EditorWindow pane, bool killIfEmpty,
    ...
    m_Selected =
        idx == m_Selected ? m_LastSelected : m_Panes.IndexOf(actualView);
  ...
  public DropInfo DragOver(EditorWindow window, Vector2 mouseScreenPosition) {
    ...
    DropInfo di = new DropInfo(this){
        ...
        rect = new Rect(pos.x - tabWidth * .25f + scr.x, scr.y + dragTopOffset,
                        tabWidth, kTabHeight)};
  ...
  public bool PerformDrop(EditorWindow w, DropInfo info, Vector2 screenPos) {
    ...
    s_OriginalDragSource.RemoveTab(w, killIfEmpty
                                   : s_OriginalDragSource != this, sendEvents
                                   : false);
    int tabInsertIndex =
        s_PlaceholderPos == -1 ||
        s_PlaceholderPos > m_Panes.Count ? m_Panes.Count : s_PlaceholderPos;
  ...
  protected bool isTopRightPane =>
      windowPosition.xMax >= Mathf.FloorToInt(window.position.width) && isTop;

  protected override void OldOnGUI() {
    var oldLabelWidth = EditorGUIUtility.labelWidth;
    EditorGUIUtility.ResetGUIState();

    // Exit if the window was destroyed after entering play mode or on
    // domain-reload.
    if (window == null)
      return;
    ...
    containerWindowPosition.width =
        GUIUtility.RoundToPixelGrid(containerWindowPosition.width);
    containerWindowPosition.height =
        GUIUtility.RoundToPixelGrid(containerWindowPosition.height);

    DrawDockAreaBackground(dockAreaRect);

    var viewRect = UpdateViewRect(dockAreaRect);
    var titleBarRect =
        new Rect(viewRect.x, dockAreaRect.y, viewRect.width, borderSize.top);
    float genericMenuLeftOffset =
        GetGenericMenuLeftOffset(floatingWindow && isTopRightPane);
    m_TabAreaRect = new Rect(
        titleBarRect.x, viewRect.y - kTabHeight,
        titleBarRect.width - (GetExtraButtonsWidth() + genericMenuLeftOffset),
        kTabHeight);
    ...
    if (!ContainerWindow.s_Modal) {
      ShowGenericMenu(position.width - genericMenuLeftOffset,
                      m_TabAreaRect.y + genericMenuTopOffset);
    }

    DrawTabs(m_TabAreaRect);
    HandleSplitView(); // fogbugz 1169963: in order to easily use the splitter
                       // in the gameView, it must be prioritized over
                       // DrawView(). Side effect for touch is that splitter
                       // picking zones might overlap other controls but the tabs
                       // still have higher priority so the user can undock the
                       // window in that case
  ...
  private void DrawTabs(Rect tabAreaRect) {
    ...
    // If the left scroll button is visible then clip the tabs a bit more
    // because of the top left border radius of the scroll left button
    ...
    using(new GUI.ClipScope(clipRect, new Vector2(-m_ScrollOffset - 1f, 0))) {
      ...
      var totalTabWidth =
          DragTab(tabAreaRect, m_ScrollOffset, tabStyle, Styles.dragTabFirst);
      if (totalTabWidth > 0f)
  ...
  private Rect UpdateViewRect(Rect dockAreaRect) {
    ...
    var viewRect =
        new Rect(dockAreaRect.x + border.left, dockAreaRect.y + border.top,
    ...
    if (selected >= 0 && selected < m_Panes.Count)
      m_Panes[selected].m_Pos =
          new Rect(GUIUtility.GUIToScreenPoint(Vector2.zero), viewRect.size);
  ...
  private void HandleTabScrolling(Rect tabAreaRect) {
    if (Event.current.type == EventType.MouseDown) {
      ...
      if (m_ScrollLeftRect.Contains(Event.current.mousePosition)) {
        ...
        SetupHoldScrollerUpdate(2f);
        m_ScrollOffset = Mathf.Min(m_ScrollOffset + scrollOffsetShift,
                                   m_TotalTabWidth - tabAreaRect.width);
        Event.current.Use();
      }
  ...
  private void DrawTabScroller(Rect scrollRect, GUIStyle tabScroller) {
    tabScroller.Draw(scrollRect,
                     scrollRect.Contains(Event.current.mousePosition), false,
                     false, false);
  }
  ...
  private void DrawTabScrollers(Rect tabAreaRect) {
    ...
    if (m_ScrollOffset > 0f) {
      m_ScrollLeftRect = new Rect(tabAreaRect.xMin, tabAreaRect.yMin, 16f,
                                  scrollerButtonHeight);
      DrawTabScroller(m_ScrollLeftRect, Styles.tabScrollerPrevButton);
    } else
      m_ScrollLeftRect.width = 0;

    if (m_TotalTabWidth > tabAreaRect.width &&
        m_ScrollOffset < (m_TotalTabWidth - tabAreaRect.width)) {
      m_ScrollRightRect = new Rect(tabAreaRect.xMax - 16f, tabAreaRect.yMin,
                                   16f, scrollerButtonHeight);
          ...
          if (sp.vertical) {
            if (idx != 0)
              EditorGUIUtility.AddCursorRect(
                  new Rect(0, 0, position.width, SplitView.kGrabDist),
                  MouseCursor.SplitResizeUpDown, id);
            if (idx != sp.children.Length - 1)
              EditorGUIUtility.AddCursorRect(
                  new Rect(0, position.height - SplitView.kGrabDist,
                           position.width, SplitView.kGrabDist),
                  MouseCursor.SplitResizeUpDown, id);
          } else // horizontal
          {
            if (idx != 0)
              EditorGUIUtility.AddCursorRect(
                  new Rect(0, 0, SplitView.kGrabDist, position.height),
                  MouseCursor.SplitResizeLeftRight, id);
            if (idx != sp.children.Length - 1)
              EditorGUIUtility.AddCursorRect(
                  new Rect(position.width - SplitView.kGrabDist, 0,
                           SplitView.kGrabDist, position.height),
                  MouseCursor.SplitResizeLeftRight, id);
          }
        }
  ...
  private void OnMouseDownHoldScroller() {
    ...
    m_ScrollOffset = Mathf.Max(
        0f, Mathf.Min(m_ScrollOffset + m_HoldScrollOffset * dt * 250.0f,
                      maxScrollOffset));
    Repaint();
  }

  private void RenderToHMDIfNecessary() {
    if (Event.current.type != EventType.Repaint || !XRSettings.isDeviceActive ||
  ...
  protected override void SetActualViewPosition(Rect newPos) {
    ...
    // In the case of DockArea i not the whole window, because we reserve some
    // space on the top for the window tabs. Because of that we want to report a
    // DockArea.position that does not contain the height of that bar. This is
    // exactly what we try to do here, remove the tab area from the rect passed
    // in. Note that this code path so far is only used when the HostView,
    // SplitView, or any "parent" view changes its dimensions and tries to
    // propagate that to its "children" There are other code paths that also set
    // the Dockarea.position, that won't go through here. But its important that
  ...
  private bool AllowTabAction() {
    ...
    ContainerWindow w = ContainerWindow.windows.FirstOrDefault(
        e => e.showMode == ShowMode.MainWindow);
    if (w != null) {
  ...
  protected override void AddDefaultItemsToMenu(GenericMenu menu,
                                                EditorWindow view) {
    base.AddDefaultItemsToMenu(menu, view);

    if (view) {
      if (parent.window.showMode == ShowMode.MainWindow)
        menu.AddItem(EditorGUIUtility.TrTextContent("Maximize"),
                     !(parent is SplitView), Maximize, view);
      else
        menu.AddDisabledItem(EditorGUIUtility.TrTextContent("Maximize"));
    }

    bool closeAllowed =
        (window.showMode != ShowMode.MainWindow || AllowTabAction());
    if (closeAllowed)
      menu.AddItem(EditorGUIUtility.TrTextContent("Close Tab"), false, Close,
                   view);
    else
    ...
    foreach (Type t in types) {
      ...
      GUIContent entry =
          new GUIContent(EditorWindow.GetLocalizedTitleContentFromType(
              t)); // make a copy since we modify the text below
  ...
  private float GetTabWidth(GUIStyle tabStyle, EditorWindow tabWindow) {
    ...
    tabStyle.CalcMinMaxWidth(tabWindow.titleContent, out minWidth,
                             out expectedWidth);
    return Mathf.Max(Mathf.Min(expectedWidth, Styles.tabMaxWidth),
                     Styles.tabMinWidth) +
           Styles.tabWidthPadding;
  }
  ...
  private int GetTabAtMousePos(GUIStyle tabStyle, Vector2 mousePos) {
    return GetTabAtMousePos(tabStyle, mousePos,
                            new Rect(0, 0, position.width, kTabHeight));
  }

  private int GetTabAtMousePos(GUIStyle tabStyle, Vector2 mousePos,
                               Rect tabAreaRect) {
    Rect tabRect = Rect.zero;
    return GetTabAtMousePos(tabStyle, mousePos + new Vector2(m_ScrollOffset, 0),
                            tabAreaRect, 0f, ref tabRect);
  }

  private int GetTabAtMousePos(GUIStyle tabStyle, Vector2 mousePos,
                               float scrollOffset, Rect tabAreaRect) {
    Rect tabRect = Rect.zero;
    return GetTabAtMousePos(tabStyle, mousePos, tabAreaRect, scrollOffset,
                            ref tabRect);
  }

  private int GetTabAtMousePos(GUIStyle tabStyle, Vector2 mousePos,
                               Rect tabAreaRect, float scrollOffset,
                               ref Rect tabRect) {
    ...
    for (int i = 0; i < m_Panes.Count; i++) {
      ...
      if (xPos <= mousePos.x && mousePos.x < xPos + tabWidth) {
        tabRect = new Rect(xPos, tabAreaRect.yMin + tabStyle.margin.top,
                           tabWidth, tabAreaRect.height);
        return i;
      }
  ...
  // Hack to get around Unity crashing when we have circular references in saved
  // stuff
  ...
  private float DragTab(Rect tabAreaRect, float scrollOffset, GUIStyle tabStyle,
    ...
    // Detect if hotcontrol was cleared while dragging (happens when pressing
    // Esc). We do not listen for the Escape keydown event because it is sent to
    // the dragged window (not this dockarea)
    ...
    case EventType.MouseDown:
      if (GUIUtility.hotControl == 0) {
        int sel = GetTabAtMousePos(tabStyle, evt.mousePosition, scrollOffset,
                                   tabAreaRect);
      ...
      if (GUIUtility.hotControl == 0) {
        ...
        if (sel != -1 && sel < m_Panes.Count && !ContainerWindow.s_Modal)
          PopupGenericMenu(m_Panes[sel], menuPos);
                 ...
                 m_Panes.Count == 0) // i.e. all panes have failed to load
    ...
    case EventType.MouseDrag:
      if (GUIUtility.hotControl == id) {
        ...
        if (s_DragMode == 0 && delta.sqrMagnitude > 99 && dragAllowed) {
          ...
          // If we're moving the only editorwindow in this dockarea, we'll be
          // destroyed - so it looks silly if we can attach as children of
          // ourselves
          ...
          PaneDragTab.get.Show(
              new Rect(tabAreaRect.x + screenRect.x + tabWidth * selected,
                       tabAreaRect.y + screenRect.y, tabWidth,
                       tabAreaRect.height - 1f),
              s_DragPane.titleContent, position.size,
      ...
      if (GUIUtility.hotControl == id) {
        ...
        if (s_DragMode != 0) {
          ...
          if (s_DropInfo?.dropArea != null) {
            ...
            s_DropInfo.dropArea.PerformDrop(s_DragPane, s_DropInfo,
                                            screenMousePos);
            ...
            // The active tab that we're moving to the new window stays focused
            // at all times. Do not remove focus from the tab being detached.
            ...
            // don't put windows top outside of the screen, on mac OS handles
            // this
            if (Application.platform == RuntimePlatform.WindowsEditor)
              wPos.y = Mathf.Max(InternalEditorUtility
                                     .GetBoundsOfDesktopAtPoint(screenMousePos)
                                     .y,
                                 wPos.y);

            // Don't call OnFocus on the tab when it is moved to the new window
            EditorWindow.CreateNewWindowForEditorWindow(w, loadPosition
                                                        : false, showImmediately
                                                        : false, setFocus
                                                        : false);

            w.position =
                w.m_Parent.window.FitWindowRectToScreen(wPos, true, true);
    ...
    case EventType.Repaint:
      ...
      if (actualView) {
        for (int i = 0, drawNum = 0; i < m_Panes.Count; i++) {
          // If we're dragging the tab we're about to draw, don't do that
          // (handled by some window)
          if (s_DragPane == m_Panes[i])
            continue;

          // If we need space for inserting a tab here, skip some horizontal
          if (s_DropInfo != null &&
              ReferenceEquals(s_DropInfo.dropArea, this) &&
              s_PlaceholderPos == drawNum)
        ...
        Rect r = new Rect(xPos, tabAreaRect.yMin, Styles.tabDragWidth,
                          tabAreaRect.height);
        float roundR = Mathf.Round(r.x);
        Rect r2 = new Rect(roundR, r.y, Mathf.Round(r.x + r.width) - roundR,
                           r.height);
  ...
  private GUIContent GetTruncatedTabContent(int tabIndex) {
    ...
    int cappedMaxChars =
        tabStyle.GetNumCharactersThatFitWithinWidth(text, Styles.tabMaxWidth);
    if (text.Length > cappedMaxChars) {
      // Save space for the '*'
      int maxLength =
          hasUnsavedChanges ? cappedMaxChars - 3 : cappedMaxChars - 2;
    ...
    if (text != tabContent.text)
      gc = new GUIContent(text, tabContent.image,
                          String.IsNullOrEmpty(tabContent.tooltip)
                              ? tabContent.text
                              : tabContent.tooltip);
  ...
  private float DrawTab(Rect tabRegionRect, GUIStyle tabStyle, int tabIndex,
                        float xPos) {
    float tabWidth = GetTabWidth(tabStyle, tabIndex);
    Rect tabPositionRect =
        new Rect(xPos, tabRegionRect.yMin + tabStyle.margin.top, tabWidth,
                 tabRegionRect.height);
    float roundedPosX = Mathf.Round(tabPositionRect.x);
    float roundedWidth =
        Mathf.Round(tabPositionRect.x + tabPositionRect.width) - roundedPosX;

    bool isActive = m_Panes[tabIndex] == EditorWindow.focusedWindow;

    if (isActive)
      UpdateWindowTitle(m_Panes[tabIndex]); // UnsavedChanges decoration already
                                            // taken care of.

    Rect tabContentRect = new Rect(roundedPosX, tabPositionRect.y, roundedWidth,
                                   tabPositionRect.height);
    tabStyle.Draw(tabContentRect,
                  tabContentRect.Contains(Event.current.mousePosition),
                  isActive, tabIndex == selected, false);
    GUI.Label(tabPositionRect, GetTruncatedTabContent(tabIndex),
              Styles.tabLabel);
  ...
  internal RectOffset GetBorderSizeInternal() {
    ...
    bool isBottomTab =
        Mathf.Abs(windowPosition.yMax - containerWindowPosition.height) < 0.02f;

    // Reset
    m_BorderSize.left = m_BorderSize.right = m_BorderSize.top =
        m_BorderSize.bottom = 0;

    Rect r = windowPosition;
    if (r.xMin != 0)
      m_BorderSize.left += (int) kSideBorders;
    if (Mathf.Abs(r.xMax - GUIUtility.RoundToPixelGrid(window.position.width)) >
        0.02f)
      m_BorderSize.right += (int) kSideBorders;

    m_BorderSize.top =
        (int) kTabHeight + (customBorder ? kFloatingWindowTopBorderWidth : 0);
...
internal class MaximizedHostView : HostView {
  ...
  protected override void OldOnGUI() {
    // Call reset GUI state as first thing so GUI.color is correct when drawing
    // window decoration.
    EditorGUIUtility.ResetGUIState();
    ...
    Rect backRect = new Rect(maximizedViewRect.x + 1, maximizedViewRect.y,
                             maximizedViewRect.width - 2, DockArea.kTabHeight);
    if (Event.current.type == EventType.Repaint) {
      Styles.background.Draw(maximizedViewRect, GUIContent.none, false, false,
                             false, false);
    ...
    if (Event.current.type == EventType.ContextClick &&
        ...
        !ContainerWindow.s_Modal)
      PopupGenericMenu(actualView,
                       new Rect(Event.current.mousePosition.x,
                                Event.current.mousePosition.y, 0, 0));

    // GetGenericMenuLeftOffset false because maximized window are not floating
    // windows
    ShowGenericMenu(position.width - GetGenericMenuLeftOffset(false),
                    backRect.yMin + Styles.genericMenuTopOffset);
  ...
  protected override void AddDefaultItemsToMenu(GenericMenu menu,
    ...
    menu.AddItem(EditorGUIUtility.TrTextContent("Maximize"),
                 !(parent is SplitView), Unmaximize, window);
    menu.AddDisabledItem(EditorGUIUtility.TrTextContent("Close Tab"));
    menu.AddSeparator("");
    IEnumerable<Type> types = GetPaneTypes();
