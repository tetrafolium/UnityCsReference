...
// - m_ContentDraw: Content render callback function  (is rendered on top of
// background ping style)
// - m_ContentRect: Size and position of content that is rendered in
// m_ContentDraw (is used for calculating the ping background and is passed to
// m_ContentDraw)
class PingData {
...
// How wide is the view where we are pinging. Needed for the pivot point trick
// where pinged content is only partly visible.
	...
	if (isPinging) {
		...
		if (t > 0.0f && t < totalTime) {
			...
			if (t < m_ZoomTime) {
				...
				float scale =
					(m_PeakScale - 1f) *
					(((m_ZoomTime - Mathf.Abs(peakTime - t)) / peakTime) - 1f) +
					1f;
				Matrix4x4 mat = GUI.matrix;

				// If the content is only partly visible, the zoom pivot point is
				// moved to right border. This avoids the nasty artefacts.
				Vector2 pivotPoint =
					m_ContentRect.xMax < m_AvailableWidth ? m_ContentRect.center
	      : new Vector2(m_AvailableWidth, m_ContentRect.center.y);
				Vector2 point = GUIClip.Unclip(pivotPoint);
				Matrix4x4 newMat =
					Matrix4x4.TRS(point, Quaternion.identity,
			...
			if (m_ContentDraw != null && Event.current.type == EventType.Repaint) {
				...
				m_PingStyle.Draw(backRect, GUIContent.none, false, false, false,
				                 false);
