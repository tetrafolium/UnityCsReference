...
internal class PragmaFixingWindow : EditorWindow {
  ...
  void OnGUI() {
    ...
    if (GUILayout.Button("Fix now", s_Styles.button)) {
      ...
      // bugfix (377429): do not call AssetDatabase.Refresh here as that screws
      // up project upgrading. When this script is invoked from
      // Application::InitializeProject, the assets will be refreshed anyway.
      GUIUtility.ExitGUI();
    }
