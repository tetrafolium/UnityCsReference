...
internal class PackageImport : EditorWindow {
  ...
  public ImportPackageItem[] packageItems {
    get { return m_ImportPackageItems; }
  }
  ...
  readonly static HashSet<char> s_InvalidPathChars =
  ...
  internal class Constants {
    ...
    public Constants() {
      lineColor = EditorGUIUtility.isProSkin ? new Color(0.1f, 0.1f, 0.1f)
  ...
  // Invoked from menu
  [UsedByNativeCode]
  public static void ShowImportPackage(string packagePath,
                                       ImportPackageItem[] items,
  ...
  public PackageImport() { minSize = new Vector2(350, 350); }

  void OnDisable() { DestroyCreatedIcons(); }
  ...
  void Init(string packagePath, ImportPackageItem[] items,
            string packageIconPath) {
    DestroyCreatedIcons();
  ...
  void TopArea() {
    ...
    if (hasPackageIcon) {
      Rect iconRect =
          new Rect(r.x + margin, r.y + margin, imageSize, imageSize);
      DrawTexture(iconRect, s_PackageIcon, true);
      titleRect = new Rect(iconRect.xMax + 10f, iconRect.yMin, r.width,
                           iconRect.height);
    } else {
  ...
  void BottomArea() {
    ...
    GUILayout.BeginHorizontal();
    GUILayout.Space(10);
    if (GUILayout.Button(EditorGUIUtility.TrTextContent("All"),
                         GUILayout.Width(50))) {
      m_Tree.SetAllEnabled(PackageImportTreeView.EnabledState.All);
    }

    if (GUILayout.Button(EditorGUIUtility.TrTextContent("None"),
                         GUILayout.Width(50))) {
    ...
    if (GUILayout.Button(EditorGUIUtility.TrTextContent("Cancel"))) {
      PackageUtility.ImportPackageAssetsCancelledFromGUI(m_PackageName,
    ...
    if (useDropshadow && Event.current.type == EventType.Repaint) {
      Rect borderPosition =
          new RectOffset(1, 1, 1, 1)
              .Remove(ms_Constants.textureIconDropShadow.border.Add(r));
      ms_Constants.textureIconDropShadow.Draw(borderPosition, GUIContent.none,
  ...
  static bool ValidateInput(ImportPackageItem[] items) {
    ...
    if (!IsAllFilePathsValid(items, out errorMessage)) {
      ...
      return EditorUtility.DisplayDialog("Invalid file path found",
                                         errorMessage, "Import",
                                         "Cancel importing");
    }
    return true;
  ...
  static bool IsAllFilePathsValid(ImportPackageItem[] assetItems,
    ...
    foreach (var item in assetItems) {
      ...
      if (HasInvalidCharInFilePath(item.destinationAssetPath, out invalidChar,
                                   out invalidCharIndex)) {
        errorMessage = string.Format(
            "Invalid character found in file path: '{0}'. Invalid ascii value: {1} (at character index {2}).",
            item.destinationAssetPath, (int) invalidChar, invalidCharIndex);
        return false;
      }
    }
  ...
  static bool HasInvalidCharInFilePath(string filePath, out char invalidChar,
                                       out int invalidCharIndex) {
  ...
  public static bool HasInvalidCharInFilePath(string filePath) {
    char invalidChar;
    int invalidCharIndex;
    return HasInvalidCharInFilePath(filePath, out invalidChar,
                                    out invalidCharIndex);
  }
}
}
