...
internal class ScalableGUIContent {
  [Serializable] struct TextureResource {
    public float pixelsPerPoint;
    public string resourcePath;
  }

  [SerializeField]
  private List<TextureResource> m_TextureResources =
      new List<TextureResource>(2);
  ...
  public ScalableGUIContent(string resourceName)
      : this(string.Empty, string.Empty, resourceName) {}
  ...
  public ScalableGUIContent(string text, string tooltip, string resourceName) {
    m_GuiContent = !string.IsNullOrEmpty(text) || !string.IsNullOrEmpty(tooltip)
                       ? EditorGUIUtility.TextContent(
                             string.Format("{0}|{1}", text, tooltip))
                       : new GUIContent();

    // TODO: make this more sophisticated when/if we have more granular support
    // for different DPI levels
    m_TextureResources.Add(
        new TextureResource{pixelsPerPoint = 1f, resourcePath = resourceName});
    m_TextureResources.Add(new TextureResource{
        pixelsPerPoint = 2f,
  ...
  private GUIContent asGUIContent() {
    ...
    if (resourcePath != m_CurrentResourcePath) {
      Texture2D loadedResource =
          EditorGUIUtility.LoadIconRequired(normalResourcePath);
