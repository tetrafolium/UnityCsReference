...
static readonly GUIContent s_ExposureIcon = EditorGUIUtility.TrIconContent(
	"Exposure",
	"Controls the number of stops to over or under expose the lightmap.");

static internal Rect GetTooltipRect() {
	return tooltipRect;
}
static internal string GetMouseTooltip() {
	return mouseTooltip;
}
internal static bool DoToggleForward(Rect position, int id, bool value,
	...
	bool returnValue = DoToggle(position, id, EditorGUI.showMixedValue ? false
	                        : value, content, style);
	if (nonLeftClick)
		evt.type = origType;
	else if (evt.type != origType)
		EditorGUIUtility.keyboardControl =
			id; // If control used event, give it keyboard focus.
	return returnValue;
}

internal static Vector2
DoBeginScrollViewForward(Rect position, Vector2 scrollPosition, Rect viewRect,
	...
	return DoBeginScrollView(
		position, scrollPosition, viewRect, alwaysShowHorizontal,
		alwaysShowVertical, horizontalScrollbar, verticalScrollbar, background);
}

internal static void BeginWindowsForward(int skinMode,
                                         int editorWindowInstanceID) {
	BeginWindows(skinMode, editorWindowInstanceID);
}

internal static void AssetPopup<T>(SerializedProperty serializedProperty,
                                   GUIContent content,
...
internal static void AssetPopup<T>(SerializedProperty serializedProperty,
                                   GUIContent content, string fileExtension,
...
internal static float ExposureSlider(float value, ref float maxValue,
                                     GUIStyle style) {
	float labelWidth = EditorGUIUtility.labelWidth;
	EditorGUIUtility.labelWidth = 20;
	value = EditorGUILayout.Slider(
		s_ExposureIcon, value, -maxValue, maxValue, -kExposureSliderAbsoluteMax,
		kExposureSliderAbsoluteMax, style, GUILayout.MaxWidth(64));
...
internal new static GUILayoutGroup BeginLayoutArea(GUIStyle style,
