...
// LazyTreeViewDataSource assumes that the Item tree only contains visible
// items, optimal for large data sets. Usage:
//    - Override FetchData () and build the tree with visible items with
//    m_RootItem as root  (and and populate the m_VisibleRows List)
//    - FetchData () is called every time the expanded state changes.
//    - Configure showRootItem and rootIsCollapsable as wanted
//
// Note: if dealing with small trees consider using TreeViewDataSource
// instead: it assumes that the tree contains all items.

internal abstract class LazyTreeViewDataSource : TreeViewDataSource {
static readonly List<TreeViewItem> s_ChildListForCollapsedParent =
...
public static List<TreeViewItem> CreateChildListForCollapsedParent() {
	...
	// The null element in the children list ensures we show the collapse
	// arrow. Reuse read-only list to prevent allocations.
	if (s_ChildListForCollapsedParent.Count != 1 ||
	    s_ChildListForCollapsedParent[0] != null) {
...
IsChildListForACollapsedParent(IList<TreeViewItem> childList) {
	return (childList != null && childList.Count == 1 &&
	        childList[0] == null); // see CreateChildListForCollapsedParent
}
...
override public void SetExpandedWithChildren(TreeViewItem item,
                                             bool expand) {
	SetExpandedWithChildren(item.id, expand);
}

// Override for special handling of recursion
// We cannot recurse normally to tree Item children because we have not
// loaded children of collapsed items therefore let client implement
// GetParentsBelow to fetch ids instead
override public void SetExpandedWithChildren(int id, bool expand) {
	...
	// Keep for debugging
	// Debug.Log ("New expanded state (bulk): " + DebugUtils.ListToString(new
...
// Get the flattened tree of visible items. Use GetFirstAndLastRowVisible to
