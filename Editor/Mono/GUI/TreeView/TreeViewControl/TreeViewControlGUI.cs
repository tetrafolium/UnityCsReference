...
namespace UnityEditor.IMGUI.Controls {
  public partial class TreeView {
    class TreeViewControlGUI : TreeViewGUI {
      ...
      const float k_BackgroundWidth =
          100000f; // The TreeView can have a horizontal scrollbar so ensure to
                   // fill out the entire width of the background
      ...
      public void RefreshRowRects(IList<TreeViewItem> rows) {
        ...
        for (int row = 0; row < rows.Count; ++row) {
          ...
          m_RowRects.Add(new Rect(
              0f, curY, 1f,
              height)); // row width is updatd to the actual visibleRect of the
                        // TreeView's scrollview on the fly
          curY += height;
        }
      }
      ...
      public int columnIndexForTreeFoldouts {
        get; // column 0 is the default column for the tree foldouts
        set;
      }

      public float totalHeight {
        get {
          return (useCustomRowRects ? customRowsTotalHeight
                  : base.GetTotalSize().y) +
                 (m_Owner.multiColumnHeader !=
                  null ? m_Owner.multiColumnHeader.height : 0f);
        }
      }

      public override Vector2 GetTotalSize() {
        Vector2 contentSize =
            useCustomRowRects ? new Vector2(1, customRowsTotalHeight)
            : base.GetTotalSize();

        // If we have multi column state we use the width of the columns so we
        // show the horizontal scrollbar if needed
        if (m_Owner.multiColumnHeader != null)
          contentSize.x = Mathf.Floor(
              m_Owner.multiColumnHeader.state.widthOfAllVisibleColumns);
        return contentSize;
      }
      ...
      float customRowsTotalHeight {
        get {
          return (m_RowRects.Count > 0 ? m_RowRects[m_RowRects.Count - 1].yMax
                                       : 0f) +
                 k_BottomRowMargin -
                 (m_TreeView.expansionAnimator.isAnimating ? m_TreeView
                      .expansionAnimator.deltaHeight : 0f);
        }
      }

      // We only override DrawIconAndLabel (and not OnRowGUI) so the user only
      // have to care about item content rendering; and not drag marker
      // rendering, renaming and foldout button
      protected override void OnContentGUI(Rect rect, int row,
                                           TreeViewItem item, string label,
        ...
        // Make sure the GUI contents for each row is starting with its own
        // controlID unique to its item. This prevents key-focus of a control
        // (e.g a float field) to jump from row to row when mouse scrolling (due
        // to culling of rows and the assigning of controlIDs in call-order)
        GUIUtility.GetControlID(TreeViewController.GetItemControlID(item),
                                FocusType.Passive);
        ...
        if (m_Owner.m_OverriddenMethods.hasRowGUI) {
          ...
          // For multi column header we call OnRowGUI for each cell with the
          // cell rect
          if (m_Owner.multiColumnHeader != null) {
            // args.columnInfo = m_ColumnInfo;
            var visibleColumns = m_Owner.multiColumnHeader.state.visibleColumns;
            if (m_CellRects == null ||
                m_CellRects.Length != visibleColumns.Length)
            ...
            for (int i = 0; i < visibleColumns.Length; ++i) {
              ...
              // Use cell margins for all columns except the column with the
              // tree view foldouts since that column assumes full cell rect for
              // rename overlay placement
            ...
            args.columnInfo = new RowGUIArgs.MultiColumnInfo(
                m_Owner.multiColumnHeader.state, m_CellRects);
          }
      ...
      internal void DefaultRowGUI(RowGUIArgs args) {
        base.OnContentGUI(args.rowRect, args.row, args.item, args.label,
                          args.selected, args.focused, false, false);
      }

      protected override Rect DoFoldout(Rect rowRect, TreeViewItem item,
                                        int row) {
        // For multicolumn setup we need to ensure foldouts are clipped so they
        // are not in the next column
        if (m_Owner.multiColumnHeader != null)
      ...
      Rect DoMultiColumnFoldout(Rect rowRect, TreeViewItem item, int row) {
        if (!m_Owner.multiColumnHeader.IsColumnVisible(
      ...
      public override Rect GetRenameRect(Rect rowRect, int row,
                                         TreeViewItem item) {
      ...
      public override void BeginRowGUI() {
        ...
        if (m_Owner.isDragging && m_Owner.multiColumnHeader != null &&
            columnIndexForTreeFoldouts > 0) {
          // Adjust insertion marker for multi column setups
          int visibleColumnIndex =
              m_Owner.multiColumnHeader.GetVisibleColumnIndex(
                  columnIndexForTreeFoldouts);
          extraInsertionMarkerIndent =
              m_Owner.multiColumnHeader.GetColumnRect(visibleColumnIndex).x;
        }
      ...
      protected override void RenameEnded() {
        ...
        var renameEndedArgs =
            new RenameEndedArgs{acceptedRename = renameState.userAcceptedRename,
        ...
        if (row < 0 || row >= m_RowRects.Count) {
          throw new ArgumentOutOfRangeException(
              ...
              string.Format(
                  ...
                  row, m_RowRects.Count, m_Owner.m_DataSource.rowCount));
        }
      ...
      // Should return the row number of the first and last row thats fits in
      // the pixel rect defined by top and height
      public override void GetFirstAndLastRowVisible(out int firstRowVisible,
                                                     out int lastRowVisible) {
        if (!useCustomRowRects) {
          base.GetFirstAndLastRowVisible(out firstRowVisible,
                                         out lastRowVisible);
          return;
        }
        ...
        for (int i = 0; i < m_RowRects.Count; ++i) {
          bool visible = ((m_RowRects[i].y > topPixel &&
                           (m_RowRects[i].y < topPixel + heightInPixels))) ||
                         ((m_RowRects[i].yMax > topPixel &&
                           (m_RowRects[i].yMax < topPixel + heightInPixels)));
      ...
      protected override void DrawItemBackground(Rect rect, int row,
                                                 TreeViewItem item,
                                                 bool selected, bool focused) {
        // To ensure backgrounds are also animated when animating expansion we
        // render the rows backgrounds individually here. When not animating
        // expansion the backgrounds are rendered in one go in
        // DrawAlternatingRowBackgrounds() which also handles background outside
        // rows
        if (m_Owner.showAlternatingRowBackgrounds &&
            m_TreeView.animatingExpansion) {
          rect.width = k_BackgroundWidth;
          var bgStyle = row % 2 == 0 ? DefaultStyles.backgroundEven
                                     : DefaultStyles.backgroundOdd;
      ...
      public void DrawAlternatingRowBackgrounds() {
        ...
        // Render entire bg
        float contentYMax =
            m_Owner.treeViewRect.height + m_Owner.state.scrollPos.y;

        DefaultStyles.backgroundOdd.Draw(
            new Rect(0, 0, k_BackgroundWidth, contentYMax), false, false, false,
            false);
        ...
        // Render every 2nd row for the entire background (we also show
        // alternating colors below the last item)
        ...
        for (int row = first; rowRect.yMax < contentYMax; row++) {
          ...
          DefaultStyles.backgroundEven.Draw(rowRect, false, false, false,
                                            false);
        }
      }

      protected override bool DoFoldoutButton(Rect foldoutRect,
                                              bool expandedState,
                                              GUIStyle foldoutStyle) {
        if (m_Owner.foldoutOverride != null)
          return m_Owner.foldoutOverride(foldoutRect, expandedState,
                                         foldoutStyle);
      ...
      public Rect DoBorder(Rect rect) {
        EditorGUI.DrawOutline(rect, borderWidth,
                              EditorGUI.kSplitLineSkinnedColor.color);
        return new Rect(rect.x + borderWidth, rect.y + borderWidth,
