...
namespace UnityEditor.IMGUI.Controls {
  // The TreeView is a wrapper for the internal treeview system (wraps the
  // TreeViewController and its 3 interfaces: ITreeViewDataSource, ITreeViewGUI,
  // ITreeViewDragging)
  // - It is intended for traditional treeviews and listviews (and supports
  // multiple columns)
  // - Usage:
  //   - Inherit from TreeView and implement the abtract methods.
  //   - Override the virtual methods for further customization (rendering,
  //   dragging, renaming etc)
  //   - Adjust layout properties

  public abstract partial class TreeView {
    public delegate bool DoFoldoutCallback(Rect position, bool expandedState,
                                           GUIStyle style);
    public delegate List<int> GetNewSelectionFunction(TreeViewItem clickedItem,
                                                      bool keepMultiSelection,
                                                      bool useActionKeyAsShift);
    ...
    protected DoFoldoutCallback foldoutOverride {
      get;
      set;
    }

    public TreeView(TreeViewState state) { Init(state); }
    ...
    // 1) Full tree: simplest conceptually and requires the least code. Just
    // create the full tree
    //    in BuildRoot and return the root of tree.
    // 2) For very large trees that often needs rebuilding of the tree structure
    // it is more
    //    performant to just create the expanded rows and omitting all items
    //    under collapsed items. This can be achieved by: a) Just create the
    //    root item in BuildRoot (no descendants) b) Override BuildRows to
    //    handle how the rows are generated: Create the rows list
    //       only visiting expanded items. Also override GetAncestors and
    //       GetDescendantsWithChildren to use the backend data to fetch this
    //       information (since it might not be available in the TreeView)
    protected abstract TreeViewItem BuildRoot();

    // Default implementation of BuildRows assumes full tree was built in
    // BuildRoot. With full tree we can also support search out of the box
    ...
    public void Repaint() { m_TreeView.Repaint(); }

    public TreeViewState state {
      get { return m_TreeView.state; }
    }
    public MultiColumnHeader multiColumnHeader {
      get { return m_MultiColumnHeader; }
      set { m_MultiColumnHeader = value; }
    }
        ...
        if (value < 0 || value >= multiColumnHeader.state.columns.Length)
          throw new ArgumentOutOfRangeException(
              ...
              string.Format(
                  ...
                  value, multiColumnHeader.state.columns.Length));
    ...
    protected Rect GetCellRectForTreeFoldouts(Rect rowRect) {
      ...
      int visibleColumnIndex =
          multiColumnHeader.GetVisibleColumnIndex(columnIndex);
    ...
    protected bool showingHorizontalScrollBar {
      get { return m_TreeView.showingHorizontalScrollBar; }
    }
    ...
    public float totalHeight {
      get {
        return m_GUI.totalHeight + (showBorder ? m_GUI.borderWidth * 2 : 0f);
      }
    }
    ...
    protected float rowHeight {
      ...
      set {
        m_GUI.k_LineHeight =
            Mathf.Max(value, EditorGUIUtility.singleLineHeight);
      }
    }
    ...
    protected void GetFirstAndLastVisibleRows(out int firstRowVisible,
                                              out int lastRowVisible) {
      m_GUI.GetFirstAndLastRowVisible(out firstRowVisible, out lastRowVisible);
    }

    // Expanded interface
    public void ExpandAll() { SetExpandedRecursive(rootItem.id, true); }

    public void CollapseAll() { SetExpanded(new int[0]); }
    ...
    public IList<int> GetExpanded() { return m_DataSource.GetExpandedIDs(); }

    public bool IsExpanded(int id) { return m_DataSource.IsExpanded(id); }
    ...
    // Selection interface
    public IList<int> GetSelection() { return m_TreeView.GetSelection(); }
    ...
    public void SetSelection(IList<int> selectedIDs,
      ...
      bool fireSelectionChanged =
          (options & TreeViewSelectionOptions.FireSelectionChanged) != 0;
      bool revealSelectionAndFrameLastSelected =
          (options & TreeViewSelectionOptions.RevealAndFrame) != 0;
      bool animatedFraming = false;

      m_TreeView.SetSelection(selectedIDs.ToArray(),
    ...
    public bool IsSelected(int id) { return m_TreeView.IsSelected(id); }

    public bool HasSelection() { return m_TreeView.HasSelection(); }

    public bool HasFocus() { return m_TreeView.HasFocus(); }
    ...
    // Rename interface
    public bool BeginRename(TreeViewItem item) { return BeginRename(item, 0f); }

    public bool BeginRename(TreeViewItem item, float delay) {
      return m_GUI.BeginRename(item, delay);
    }

    public void EndRename() { m_GUI.EndRename(); }
    ...
    public void SelectAllRows() {
      var rows = GetRows();
      var allowedSelection = (from treeViewItem in rows where CanMultiSelect(
                                  treeViewItem) select treeViewItem.id)
                                 .ToList();
      SetSelection(allowedSelection,
                   TreeViewSelectionOptions.FireSelectionChanged);
    }

    void TreeViewWithMultiColumnHeader(Rect rect) {
      Rect columnHeaderRect =
          new Rect(rect.x, rect.y, rect.width, m_MultiColumnHeader.height);
      Rect treeRect = new Rect(rect.x, columnHeaderRect.yMax, rect.width,
                               rect.height - columnHeaderRect.height);

      float scrollX =
          Mathf.Max(m_TreeView.state.scrollPos.x,
                    0f); // Ensure positive scroll values (work around an issue
                         // in the scroll view that when using mousescroll it
                         // returns invalid negative values)

      Event evt = Event.current;
      if (evt.type == EventType.MouseDown &&
          columnHeaderRect.Contains(evt.mousePosition))
        GUIUtility.keyboardControl = m_TreeViewKeyControlID;
    ...
    protected void AddExpandedRows(TreeViewItem root,
    ...
    void GetExpandedRowsRecursive(TreeViewItem item,
                                  IList<TreeViewItem> expandedRows) {
      if (item == null)
        Debug.LogError(
            "Found a TreeViewItem that is null. Invalid use of AddExpandedRows(): This method is only valid to call if you have built the full tree of TreeViewItems.");
    ...
    protected virtual bool CanRename(TreeViewItem item) {
      return false; // Default to false so user have to enable renaming if
                    // wanted
    }
    ...
    protected virtual DragAndDropVisualMode
    HandleDragAndDrop(DragAndDropArgs args) {
    ...
    protected virtual bool DoesItemMatchSearch(TreeViewItem item,
                                               string search) {
      return item.displayName.IndexOf(search,
    ...
    protected virtual Rect GetRenameRect(Rect rowRect, int row,
                                         TreeViewItem item) {
    ...
    SetupParentsAndChildrenFromDepths(TreeViewItem root,
                                      IList<TreeViewItem> rows) {
    ...
    IsChildListForACollapsedParent(IList<TreeViewItem> childList) {
    ...
    class OverriddenMethods {
      ...
      static bool IsOverridden(Type type, string methodName) {
        var methodInfo = type.GetMethod(methodName, BindingFlags.NonPublic |
                                                        BindingFlags.Instance);
        if (methodInfo != null)
          return methodInfo.GetBaseDefinition().DeclaringType !=
                 methodInfo.DeclaringType;

        Debug.LogError("IsOverridden: method name not found: " + methodName +
                       " (check spelling against method declaration)");
        return false;
      }
      ...
      void ValidateOverriddenMethods(TreeView treeView) {
        ...
        if (hasRenameEnded != hasCanRename) {
          if (hasCanRename)
            Debug.LogError(
                type.Name +
                ": If you are overriding CanRename you should also override RenameEnded (to handle the renaming).");

          if (hasRenameEnded)
            Debug.LogError(
                type.Name +
                ": If you are overriding RenameEnded you should also override CanRename (to allow renaming).");
        }
        ...
        if (hasCanStartDragAndDrop != hasSetupDragAndDrop) {
          if (hasCanStartDragAndDrop)
            Debug.LogError(
                type.Name +
                ": If you are overriding CanStartDrag you should also override SetupDragAndDrop (to setup the drag).");

          if (hasSetupDragAndDrop)
            Debug.LogError(
                type.Name +
                ": If you are overriding SetupDragAndDrop you should also override CanStartDrag (to allow dragging).");
        }
      }
    }
    ...
    protected struct RowGUIArgs {
      ...
      internal MultiColumnInfo columnInfo {
        get;
        set;
      }
      ...
      public int GetColumn(int visibleColumnIndex) {
        ...
        return columnInfo.multiColumnHeaderState
            .visibleColumns[visibleColumnIndex];
      }
      ...
      internal struct MultiColumnInfo {
        internal MultiColumnInfo(MultiColumnHeaderState multiColumnHeaderState,
                                 Rect[] cellRects) {
    ...
    protected struct SetupDragAndDropArgs { public IList<int> draggedItemIDs; }
    ...
    protected enum DragAndDropPosition { UponItem, BetweenItems, OutsideItems }
  }
