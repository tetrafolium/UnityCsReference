...
namespace UnityEditor.IMGUI.Controls {
  public partial class TreeView {
    internal class TreeViewControlDataSource : LazyTreeViewDataSource {
      ...
      public TreeViewControlDataSource(TreeViewController treeView,
                                       TreeView owner)
          : base(treeView) {
        m_Owner = owner;

        // The user should just create the visible rows, we create the hidden
        // root
        showRootItem = false;
      }

      public override void ReloadData() {
        // Clear root item to ensure client gets a call to BuildRoot every time
        // Reload is called
        m_RootItem = null;
        base.ReloadData();
      }
      ...
      void ValidateRootItem() {
        ...
        if (m_RootItem.depth != -1) {
          Debug.LogError(
              "BuildRoot should ensure the root item has a depth == -1. The visible items start at depth == 0.");
          m_RootItem.depth = -1;
        }
        if (m_RootItem.children == null &&
            !m_Owner.m_OverriddenMethods.hasBuildRows) {
      ...
      public override void FetchData() {
        // Set before BuildRoot and BuildRows so we can call GetRows in them
        // without recursion
      ...
      public void SearchFullTree(string search, List<TreeViewItem> result) {
        ...
        result.Sort((x, y) => EditorUtility.NaturalCompare(x.displayName,
                                                           y.displayName));
      }
