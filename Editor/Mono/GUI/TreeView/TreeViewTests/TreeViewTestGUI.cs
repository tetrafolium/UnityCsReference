...
namespace UnityEditor.TreeViewExamples {
  internal class FooTreeViewItem : TreeViewItem {
    ...
    public FooTreeViewItem(int id, int depth, TreeViewItem parent,
                           string displayName, BackendData.Foo foo)
  ...
  class TestGUI : TreeViewGUI {
    ...
    float[] columnWidths {
      get { return ((TreeViewStateWithColumns) m_TreeView.state).columnWidths; }
    }
    ...
    protected override void OnContentGUI(Rect rect, int row, TreeViewItem item,
      ...
      if (!m_StyleInitialized) {
        ...
        m_LabelStyleRightAlign.padding.right =
            m_LabelStyleRightAlign.padding.left = 6;
      ...
      for (int i = 0; i < columnWidths.Length; ++i) {
        ...
        if (i == 0)
          base.OnContentGUI(columnRect, row, item, label, selected, focused,
                            useBoldFont, isPinging);
        else
          GUI.Label(columnRect, "Zksdf SDFS DFASDF ",
                    (i % 2 == 0) ? lineStyle : m_LabelStyleRightAlign);
