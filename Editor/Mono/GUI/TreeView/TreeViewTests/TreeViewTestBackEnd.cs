...
namespace UnityEditor.TreeViewExamples {
  internal class BackendData {
    ...
    public void GenerateData(int maxNumItems) {
      ...
      for (int i = 0; i < 10; ++i)
        AddChildrenRecursive(
            m_Root, UnityEngine.Random.Range(k_MinChildren, k_MaxChildren),
            true);
    }

    public Foo Find(int id) { return FindRecursive(id, m_Root); }
    ...
    private void GetParentsBelowStackBased(Foo searchFromThis,
    ...
    private static void GetParentsBelowRecursive(Foo item,
    ...
    public void ReparentSelection(Foo parentItem, int insertionIndex,
                                  List<Foo> draggedItems) {
      ...
      if (insertionIndex > 0)
        insertionIndex -= parentItem.children.GetRange(0, insertionIndex)
                              .Count(draggedItems.Contains);

      // Remove draggedItems from their parents
      foreach (var draggedItem in draggedItems) {
        draggedItem.parent.children.Remove(
            draggedItem);                // remove from old parent
      ...
      // If insertionIndex is -1 then item was dropped upon the parent: client
      // have to decide where to place the dragged items. We add as the first.
      if (insertionIndex == -1)
        insertionIndex = 0;
    ...
    void AddChildrenRecursive(Foo foo, int numChildren, bool force) {
      ...
      foreach (var child in foo.children) {
        AddChildrenRecursive(
            child, UnityEngine.Random.Range(k_MinChildren, k_MaxChildren),
            false);
      }
    }
  ...
  internal class TreeViewColumnHeader {
    ...
    public void OnGUI(Rect rect) {
      ...
      for (int i = 0; i < columnWidths.Length; ++i) {
        Rect columnRect =
            new Rect(columnPos, rect.y, columnWidths[i], rect.height);
        columnPos += columnWidths[i];
        Rect dragRect = new Rect(columnPos - dragWidth / 2, rect.y,
                                 dragAreaWidth, rect.height);
