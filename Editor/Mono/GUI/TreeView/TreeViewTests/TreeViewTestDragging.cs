...
public override void StartDrag(TreeViewItem draggedItem,
	...
	DragAndDrop.SetGenericData(
		k_GenericDragID, new FooDragData(GetItemsFromIDs(draggedItemIDs)));
	string title = draggedItemIDs.Count + " Foo" +
	               (draggedItemIDs.Count > 1
	                  ? "s"
...
public override DragAndDropVisualMode DoDrag(TreeViewItem parentItem,
	...
	if (fooParent != null && dragData != null) {
		...
		if (perform && validDrag) {
			// Do reparenting here
			List<BackendData.Foo> draggedFoos =
				(from x in dragData
				 .m_DraggedItems where x is FooTreeViewItem select(
					 (FooTreeViewItem) x)
				 .foo)
				.ToList();
			var selectedIDs =
				...
				.ToArray();
			int insertionIndex =
				GetInsertionIndex(parentItem, targetItem, dropPos);
			m_BackendData.ReparentSelection(fooParent.foo, insertionIndex,
			                                draggedFoos);
...
GetItemsFromIDs(IEnumerable<int> draggedItemIDs) {
	// Note we only drag visible items here...
	return TreeViewUtility.FindItemsInList(draggedItemIDs,
