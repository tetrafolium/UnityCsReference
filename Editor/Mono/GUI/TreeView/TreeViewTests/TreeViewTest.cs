...
namespace UnityEditor.TreeViewExamples {
  ...
  internal class TreeViewTest {
    ...
    public int GetNumItemsInData() { return m_BackendData.IDCounter; }
    ...
    void OnColumnRenderer(int column, Rect rect) {
      ...
      string[] headers = new[]{"Name",   "Date Modified", "Size",   "Kind",
                               "Author", "Platform",      "Faster", "Slower"};
      GUI.Label(rect, headers[column],
                (column % 2 == 0) ? m_HeaderStyle : m_HeaderStyleRightAligned);
    }
    ...
    public void OnGUI(Rect rect) {
      ...
      Rect bottomRect = new Rect(rect.x, rect.yMax - kBottomHeight, rect.width,
                                 kBottomHeight);
      ...
      m_BackendData.m_RecursiveFindParentsBelow =
          GUILayout.Toggle(m_BackendData.m_RecursiveFindParentsBelow,
                           GUIContent.Temp("Recursive"));
    ...
    private string GetHeader() {
      return (m_Lazy ? "LAZY: "
              : "FULL: ") +
             "GUI items: " + GetNumItemsInTree() +
             "  (data items: " + GetNumItemsInData() + ")";
    }
  }
} // UnityEditor
