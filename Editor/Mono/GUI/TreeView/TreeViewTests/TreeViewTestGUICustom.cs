...
internal class Styles { public static GUIStyle foldout = "IN Foldout"; }
...
protected override Vector2 GetSizeOfRow(TreeViewItem item) {
	return new Vector2(m_TreeView.GetTotalRect().width,
	                   item.hasChildren ? 20 : 36f);
}
	...
	if (Event.current.type == EventType.Repaint) {
		...
		if (m_TreeView.dragging != null &&
			...
			m_DraggingInsertionMarkerRect =
				new Rect(rowRect.x + indent, rowRect.y, rowRect.width - indent,
				         rowRect.height);
	}

	// Draw foldout (after text content above to ensure drop down icon is
	// rendered above selection highlight)
	if (showFoldout) {
		tmpRect.x = indent;
		tmpRect.width = m_FoldoutWidth;
		EditorGUI.BeginChangeCheck();
		bool newExpandedValue =
			GUI.Toggle(tmpRect, m_TreeView.data.IsExpanded(item),
