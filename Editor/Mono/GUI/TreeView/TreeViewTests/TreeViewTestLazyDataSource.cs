...
public override void FetchData() {
	...
	itemCounter = 1;
	m_RootItem = new FooTreeViewItem(m_Backend.root.id, 0, null,
	                                 m_Backend.root.name, m_Backend.root);
...
void AddVisibleChildrenRecursive(BackendData.Foo source,
                                 TreeViewItem dest) {
	if (IsExpanded(source.id)) {
		if (source.children != null && source.children.Count > 0) {
			...
			for (int i = 0; i < source.children.Count; ++i) {
				...
				dest.children.Add(
					new FooTreeViewItem(s.id, dest.depth + 1, dest, s.name, s));
				++itemCounter;
		...
		if (source.hasChildren) {
			dest.children =
				CreateChildListForCollapsedParent(); // ensure we show the
			                                             // collapse arrow (because we
