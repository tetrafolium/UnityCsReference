...
namespace UnityEditor.IMGUI.Controls {
  internal static class TreeViewUtility {
    internal static void SetParentAndChildrenForItems(IList<TreeViewItem> rows,
                                                      TreeViewItem root) {
    ...
    internal static List<TreeViewItem>
    FindItemsInList(IEnumerable<int> itemIDs,
                    IList<TreeViewItem> treeViewItems) {
      return (from x in treeViewItems where itemIDs.Contains(x.id) select x)
          .ToList();
    }

    internal static TreeViewItem
    FindItemInList<T>(int id, IList<T> treeViewItems) where T : TreeViewItem {
      return treeViewItems.FirstOrDefault(t => t.id == id);
    }

    // Assumes full tree
    internal static TreeViewItem FindItem(int id,
    ...
    // Assumes full tree
    internal static void GetParentsAboveItem(TreeViewItem fromItem,
                                             HashSet<int> parentsAbove) {
      if (fromItem == null)
    ...
    // Assumes full tree
    internal static void GetParentsBelowItem(TreeViewItem fromItem,
                                             HashSet<int> parentsBelow) {
      if (fromItem == null)
        throw new ArgumentNullException("fromItem");
      ...
      while (stack.Count > 0) {
        ...
        if (current.hasChildren) {
          ...
          if (LazyTreeViewDataSource.IsChildListForACollapsedParent(
                  current.children))
    ...
    internal static void DebugPrintToEditorLogRecursive(TreeViewItem item) {
      if (item == null)
        return;
      System.Console.WriteLine(new System.String(' ', item.depth * 3) +
                               item.displayName);
    ...
    // Setup child and parent references based on the depth of the tree view
    // items in 'visibleItems'
    internal static void
    SetChildParentReferences(IList<TreeViewItem> visibleItems,
                             TreeViewItem root) {
    ...
    static void SetChildren(TreeViewItem item,
                            List<TreeViewItem> newChildList) {
      // Do not touch children if we have a LazyParent and did not find any
      // children == keep lazy children
      if (LazyTreeViewDataSource.IsChildListForACollapsedParent(
              item.children) &&
          newChildList == null)
        return;
    ...
    static void SetChildParentReferences(int parentIndex,
      ...
      // Count children based depth value, we are looking at children until it's
      // the same depth as this object
