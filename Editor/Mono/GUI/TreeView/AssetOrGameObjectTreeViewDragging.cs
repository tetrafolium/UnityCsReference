...
// Implements dragging behavior for HierarchyProperty based data: Assets or
// GameObjects
...
internal class AssetsTreeViewDragging : TreeViewDragging {
  ...
  public override bool CanStartDrag(TreeViewItem targetItem,
    ...
    foreach (var draggedItemID in draggedItemIDs) {
      ...
      if (AssetDatabase.IsValidFolder(path)) {
        ...
        var validPath = AssetDatabase.GetAssetFolderInfo(path, out rootFolder,
                                                         out readOnly);
  ...
  public override void StartDrag(TreeViewItem draggedItem,
    ...
    DragAndDrop.objectReferences =
        ProjectWindowUtil.GetDragAndDropObjects(draggedItem.id, draggedItemIDs);

    DragAndDrop.paths =
        ProjectWindowUtil.GetDragAndDropPaths(draggedItem.id, draggedItemIDs);
    if (DragAndDrop.objectReferences.Length > 1)
      DragAndDrop.StartDrag("<Multiple>");
    else {
      string title = ObjectNames.GetDragAndDropTitle(
          InternalEditorUtility.GetObjectFromInstanceID(draggedItem.id));
...
internal class GameObjectsTreeViewDragging : TreeViewDragging {
  ...
  public override void StartDrag(TreeViewItem draggedItem,
    ...
    if ((Event.current.control || Event.current.command) &&
        !draggedItemIDs.Contains(draggedItem.id)) {
      draggedItemIDs.Add(draggedItem.id);
    }

    // Ensure correct order for hierarchy items (to preserve visible order when
    // dropping at new location)
    ...
    Object[] draggedObjReferences =
        ProjectWindowUtil.GetDragAndDropObjects(draggedItem.id, draggedItemIDs);
    DragAndDrop.objectReferences = draggedObjReferences;

    // After introducing multi-scene, UnityEngine.Scene can be selected in
    // HierarchyWindow. UnityEngine.Scene is not a UnityEngine.Object. So
    // DragAndDrop.objectReferences can't cover this case.
  ...
  public override DragAndDropVisualMode DoDrag(TreeViewItem parentItem,
    ...
    if (m_CustomDragHandling != null) {
      DragAndDropVisualMode dragResult =
          m_CustomDragHandling(parentItem as GameObjectTreeViewItem,
                               hierarchyTargetItem, dropPos, perform);
    ...
    DragAndDropVisualMode dragSceneResult =
        DoDragScenes(parentItem as GameObjectTreeViewItem, hierarchyTargetItem,
                     perform, dropPos);
    ...
    if (parentItem == null || targetItem == null) {
      ...
      if (parentForDraggedObjectsOutsideItems != null) {
        ...
        return DragAndDropService.Drop(
            DragAndDropService.kHierarchyDropDstId, 0, option,
            parentForDraggedObjectsOutsideItems, perform);
      } else {
        // Simulate drag upon the last loaded scene in the hierarchy (adds as
        // last root sibling of the last scene)
        ...
        return DragAndDropService.Drop(DragAndDropService.kHierarchyDropDstId,
                                       lastScene.handle, option, null, perform);
      }
    }
    ...
    bool isDroppingBetweenParentAndFirstChild =
        parentItem != null && targetItem != parentItem &&
        dropPos == DropPosition.Above && parentItem.children[0] == targetItem;
    if (isDroppingBetweenParentAndFirstChild) {
      option |=
          InternalEditorUtility.HierarchyDropMode.kHierarchyDropAfterParent;
    }

    int gameObjectOrSceneInstanceID =
        GetDropTargetInstanceID(hierarchyTargetItem, dropPos);
    if (gameObjectOrSceneInstanceID == 0)
      return DragAndDropVisualMode.Rejected;

    if (perform && SubSceneGUI.IsUsingSubScenes() &&
        !IsValidSubSceneDropTarget(gameObjectOrSceneInstanceID, dropPos,
                                   DragAndDrop.objectReferences))
      return DragAndDropVisualMode.Rejected;

    return DragAndDropService.Drop(DragAndDropService.kHierarchyDropDstId,
                                   gameObjectOrSceneInstanceID, option, null,
                                   perform);
  }
  ...
  bool IsValidSubSceneDropTarget(int dropTargetGameObjectOrSceneInstanceID,
                                 DropPosition dropPosition,
                                 Object[] draggedObjects) {
    if (draggedObjects == null || draggedObjects.Length == 0)
      return false;

    Transform parentForDrop = GetTransformParentForDrop(
        dropTargetGameObjectOrSceneInstanceID, dropPosition);
    ...
    foreach (var obj in draggedObjects) {
      ...
      // Require all dragged objects to be valid (since native cannot filter out
      // invalid sub scene drags currently)
      if (SubSceneGUI.IsChildOrSameAsOtherTransform(parentForDrop,
  ...
  Transform GetTransformParentForDrop(int gameObjectOrSceneInstanceID,
                                      DropPosition dropPosition) {
    ...
    if (obj != null) {
      ...
      var scene =
          EditorSceneManager.GetSceneByHandle(gameObjectOrSceneInstanceID);
  ...
  private DragAndDropVisualMode DoDragScenes(GameObjectTreeViewItem parentItem,
                                             GameObjectTreeViewItem targetItem,
                                             bool perform,
                                             DropPosition dropPos) {
    // We allow dragging SceneAssets on any game object in the Hierarchy to make
    // it easy to drag in a Scene from the project browser. If dragging on a
    // game object (not a sceneheader) we place the dropped scene below the game
    // object's scene

    // Case: 1
    List<Scene> scenes =
        DragAndDrop.GetGenericData(kSceneHeaderDragString) as List<Scene>;
    ...
    if (!reorderExistingScenes && DragAndDrop.objectReferences.Length > 0) {
      ...
      insertNewScenes =
          (sceneAssetCount == DragAndDrop.objectReferences.Length);
    }
      ...
      if (insertNewScenes) {
          ...
          if (SceneHierarchy.IsSceneHeaderInHierarchyWindow(scene))
            ...
            if (unloaded)
              scene = EditorSceneManager.OpenScene(
                  scenePath, OpenSceneMode.AdditiveWithoutLoading);
            else
        ...
        if (insertedScenes.Count > 0) {
          Selection.instanceIDs =
              insertedScenes.Select(x => x.handle).ToArray();
          ...
          if (SceneHierarchy.IsSceneHeaderInHierarchyWindow(dstScene)) {
            ...
            if (dropPos == DropPosition.Above) {
              for (int i = 0; i < scenesToBeMoved.Count; i++)
                EditorSceneManager.MoveSceneBefore(scenesToBeMoved[i],
                                                   dstScene);
