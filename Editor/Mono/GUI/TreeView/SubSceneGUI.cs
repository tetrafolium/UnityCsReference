...
static class SubSceneGUI {
  static Dictionary<GameObject, SceneHierarchyHooks.SubSceneInfo>
      m_SubSceneHeadersMap =
          new Dictionary<GameObject, SceneHierarchyHooks.SubSceneInfo>();
  static Dictionary<Scene, SceneHierarchyHooks.SubSceneInfo>
      m_SceneToSubSceneMap =
          new Dictionary<Scene, SceneHierarchyHooks.SubSceneInfo>();
  static Dictionary<SceneAsset, SceneHierarchyHooks.SubSceneInfo>
      m_SceneAssetToSubSceneMap =
          new Dictionary<SceneAsset, SceneHierarchyHooks.SubSceneInfo>();
  ...
  static Transform GetParentCheckSubScenes(Transform transform) {
    if (transform.parent == null) {
      ...
      if (m_SceneToSubSceneMap.TryGetValue(transform.gameObject.scene,
                                           out subScene))
  ...
  internal static bool IsChildOrSameAsOtherTransform(Transform transform,
  ...
  CreateClosedSubSceneContextClick(GenericMenu menu,
                                   SceneHierarchyHooks.SubSceneInfo subScene) {
    var selectAssetContent =
        EditorGUIUtility.TrTextContent("Select Scene Asset");
    if (subScene.sceneAsset)
      menu.AddItem(selectAssetContent, false, SelectSceneAsset,
                   subScene.sceneAsset);
    else
  ...
  internal static SceneHierarchyHooks.SubSceneInfo
  GetSubSceneInfo(Scene scene) {
    SceneHierarchyHooks.SubSceneInfo subScene;
  ...
  internal static SceneHierarchyHooks.SubSceneInfo
  GetSubSceneInfo(SceneAsset sceneAsset) {
    SceneHierarchyHooks.SubSceneInfo subScene;
  ...
  internal static SceneHierarchyHooks.SubSceneInfo
  GetSubSceneInfo(GameObject gameObject) {
    SceneHierarchyHooks.SubSceneInfo subScene;
    if (gameObject != null &&
        m_SubSceneHeadersMap.TryGetValue(gameObject, out subScene))
      return subScene;
  ...
  internal static void DrawSubSceneHeaderBackground(Rect rect, float baseIndent,
                                                    float indentWidth,
                                                    GameObject gameObject) {
    float indent =
        CalcIndentOfSubSceneHeader(gameObject, baseIndent, indentWidth);
    if (indent < 0) {
      Debug.LogError(
          "Only call DrawSubSceneHeaderBackground if IsSubSceneHeader() is true");
      return;
    }
    ...
    GUI.color =
        GUI.color *
        new Color(1, 1, 1, 0.9f); // dimmed compared to main scene headers
    GUI.Label(headerRect, GUIContent.none,
              GameObjectTreeViewGUI.GameObjectStyles.sceneHeaderBg);
    GUI.color = oldColor;
  }

  static float CalcIndentOfSubSceneHeader(GameObject gameObject,
                                          float baseIndent, float indentWidth) {
    SceneHierarchyHooks.SubSceneInfo subSceneInfo;
    if (gameObject == null ||
        !m_SubSceneHeadersMap.TryGetValue(gameObject, out subSceneInfo))
      return -1; // Input is not a sub scene GameObject

    int hierarchyDepth = CalculateHierarchyDepthOfSubScene(subSceneInfo);
    if (hierarchyDepth > 0) {
      return baseIndent + s_SubSceneHeaderIndentAdjustment +
             (hierarchyDepth * indentWidth);
    }
    return -1f;
  }
  ...
  internal static Rect GetRectForVerticalLine(Rect rowRect, float baseIndent,
                                              float indentWidth, Scene scene) {
    ...
    float indent =
        CalcIndentOfVerticalLine(s_LastSubSceneInfo, baseIndent, indentWidth);
    if (indent < 0)
      return new Rect();
  ...
  internal static void DrawVerticalLine(Rect rowRect, float baseIndent,
                                        float indentWidth,
                                        GameObject gameObject) {
    if (gameObject == null)
      return;
    ...
    if (Event.current.type == EventType.Repaint) {
      ...
      Rect lineRect =
          GetRectForVerticalLine(rowRect, baseIndent, indentWidth, scene);
      if (lineRect.width > 0f) {
        Color color = GetColorForSubScene(scene);
        GUI.DrawTexture(lineRect, EditorGUIUtility.whiteTexture,
                        ScaleMode.StretchToFill, true, 1, color, color, color,
                        color, Vector4.zero, Vector4.zero, false);
      }
    }
  }

  static float
  CalcIndentOfVerticalLine(SceneHierarchyHooks.SubSceneInfo subSceneInfo,
                           float baseIndent, float indentWidth) {
    int hierarchyDepth = CalculateHierarchyDepthOfSubScene(subSceneInfo);
    if (hierarchyDepth > 0) {
  ...
  internal static int CalculateHierarchyDepthOfSubScene(
      SceneHierarchyHooks.SubSceneInfo subSceneInfo) {
    ...
    while (i++ < kMaxSubSceneIterations) {
      hierarchyDepth += FindTransformDepth(subSceneInfo.transform) +
                        1; // the +1 is for the SubScene header
      if (!m_SceneToSubSceneMap.TryGetValue(
              subSceneInfo.transform.gameObject.scene, out subSceneInfo))
        break;
    }
