...
internal class ToggleTreeView<T> : TreeView where T : ToggleTreeViewItem,
                                                      new () {
  static class Styles {
    public static readonly GUIContent toggleAll =
        EditorGUIUtility.TrTextContent("Toggle All");
    public static readonly GUIContent expandAll =
        EditorGUIUtility.TrTextContent("Expand All");
    public static readonly GUIContent collapseAll =
        EditorGUIUtility.TrTextContent("Collapse All");
    public static readonly GUIContent toggle = EditorGUIUtility.TrTextContent(
        "", "Maintain Alt/Option key to enable or disable all children");
    public static readonly GUIContent filterSelected =
        new GUIContent(EditorGUIUtility.FindTexture("FilterSelectedOnly"),
  ...
  public ToggleTreeView(TreeViewState state,
  ...
  public float totalHeightIncludingSearchBarAndBottomBar =>
      totalHeight + 18f + EditorGUI.kSingleLineHeight;
  ...
  void SearchFullTree(List<TreeViewItem> rows) {
    ...
    while (stack.Count > 0) {
      ...
      if (current.children != null) {
        foreach (var child in current.children) {
          if (child != null) {
            if (!searchEnabledState ||
                ((ToggleTreeViewItem) child).nodeState == searchedEnabledState)
    ...
    rows.Sort((x, y) =>
                  EditorUtility.NaturalCompare(x.displayName, y.displayName));
  }

  protected override bool DoesItemMatchSearch(TreeViewItem item,
                                              string search) {
    return string.IsNullOrEmpty(search) ||
           base.DoesItemMatchSearch(item, search);
  }
  ...
  void DrawSearchField() {
    using(new EditorGUILayout.HorizontalScope(
        ...
        GUILayout.MaxHeight(EditorGUI.kWindowToolbarHeight))) {
      ...
      using(new EditorGUI.DisabledScope(Regex.IsMatch(searchString, s_Regex))) {
        if (GUILayout.Button(Styles.filterSelected, EditorStyles.miniButton,
  ...
  protected virtual void BottomGUI() {
    using(new EditorGUILayout.HorizontalScope()) using(
        new EditorGUI.DisabledScope(!hasNodes)) {
  ...
  protected override TreeViewItem BuildRoot() {
    return m_RebuildRoot == null ? new T(){depth = -1, id = 0, displayName = "",
  ...
  protected override void RowGUI(RowGUIArgs args) {
    ...
    for (var i = 0; i < args.GetNumVisibleColumns(); ++i) {
      CellGUI(args.GetCellRect(i), (Column) args.GetColumn(i), (T) args.item,
              ref args);
    }
  }
  ...
  protected virtual void EnabledGUI(Rect cellRect, T node,
                                    ref RowGUIArgs args) {
  ...
  private static bool DoFoldoutButtonOverride(Rect foldoutRect,
                                              bool expandedState,
    ...
    // because we want it to ignore current indent level as it was already
    // calculated by the TreeView.
    var indent = EditorGUI.indentLevel;
    EditorGUI.indentLevel = 0;
    // Using EditorGUI.Foldout to keep the foldout active even when the GUI is
    // disabled.
    var newExpandedValue = EditorGUI.Foldout(foldoutRect, expandedState,
