...
namespace UnityEditor.IMGUI.Controls {
  ...
  internal abstract class TreeViewDragging : ITreeViewDragging {
    ...
    public enum DropPosition { Upon = 0, Below = 1, Above = 2 }
    ...
    static class Constants {
      ...
      public const string GetInsertionIndexNotFound =
          "Did not find targetItem,; should be a child of parentItem";
    ...
    public int GetRowMarkerControlID() { return m_DropData.rowMarkerControlID; }
    ...
    public virtual bool CanStartDrag(TreeViewItem targetItem,
    ...
    public abstract void StartDrag(TreeViewItem draggedItem,
                                   List<int> draggedItemIDs);

    // This method is called from within DragElement when it has determined what
    // is the parent item of the current targetItem (This depends on if
    // dropPosition is above, below or upon) Implemented by client code to
    // decide what should happen when the drag is e.g performed (e.g change the
    // backend state of the tree view) Notes on arguments: When hovering outside
    // any items: target and parent is null, dropPos is invalid If parentItem
    // and targetItem is the same then insert as first child of parent, dropPos
    // is invalid If parentItem and targetItem is different then use dropPos to
    // insert dragged items relative to targetItem parentItem can be null when
    // root is visible and hovering above or below the root
    ...
    public abstract DragAndDropVisualMode DoDrag(TreeViewItem parentItem,
                                                 TreeViewItem targetItem,
                                                 bool perform,
                                                 DropPosition dropPosition);

    protected float GetDropBetweenHalfHeight(TreeViewItem item, Rect itemRect) {
      return m_TreeView.data.CanBeParent(item)
                 ? m_TreeView.gui.halfDropBetweenHeight
                 : itemRect.height * 0.5f;
    }

    protected bool TryGetDropPosition(TreeViewItem item, Rect itemRect, int row,
                                      out DropPosition dropPosition) {
      ...
      if (itemRect.Contains(currentMousePos)) {
        ...
        if (nextRow < m_TreeView.data.rowCount) {
          ...
          bool nextCanBeParent =
              m_TreeView.data.CanBeParent(m_TreeView.data.GetItem(nextRow));
          if (nextCanBeParent)
    ...
    // 'targetItem' is null when not hovering over any target Item, if so the
    // rest of the arguments are invalid
    public virtual bool DragElement(TreeViewItem targetItem,
                                    Rect targetItemRect, int row) {
      ...
      if (targetItem == null) {
        ...
        DragAndDrop.visualMode =
            DoDrag(null, null, perform, DropPosition.Below);
      ...
      DropPosition dropPosition;
      if (!TryGetDropPosition(targetItem, targetItemRect, row,
                              out dropPosition))
        return false;
      ...
      case DropPosition.Upon: {
        // Client must decide what happens when dropping upon: e.g: insert last
        // or first in child list
        parentItem = targetItem;
      } break;
      case DropPosition.Below: {
        // When hovering between an expanded parent and its first child then
        // make sure we change state to match that
    ...
    void FinalizeDragPerformed(bool revertExpanded) {
      ...
      List<Object> objs = new List<Object>(
          DragAndDrop.objectReferences); // TODO, what about when dragging non
                                         // objects...

      bool draggedItemsFromOwnTreeView = true;
      if (objs.Count > 0 && objs[0] != null &&
          TreeViewUtility.FindItemInList(objs [0]
                                             .GetInstanceID(),
                                         m_TreeView.data.GetRows()) == null)
      ...
      for (int i = 0; i < objs.Count; ++i) {
        ...
        newSelection[i] = (objs [i]
                               .GetInstanceID());
      }
      m_TreeView.NotifyListenersThatDragEnded(newSelection,
                                              draggedItemsFromOwnTreeView);
      Undo.SetCurrentGroupName(Constants.UndoActionName);
    }

    protected virtual void HandleAutoExpansion(int itemControlID,
                                               TreeViewItem targetItem,
                                               Rect targetItemRect) {
      Vector2 currentMousePos = Event.current.mousePosition;
      ...
      float betweenHalfHeight =
          GetDropBetweenHalfHeight(targetItem, targetItemRect);
      Rect indentedContentRect =
          new Rect(targetItemRect.x + targetItemIndent,
                   targetItemRect.y + betweenHalfHeight,
                   targetItemRect.width - targetItemIndent,
                   targetItemRect.height - betweenHalfHeight * 2);
      bool hoveringOverIndentedContent =
          indentedContentRect.Contains(currentMousePos);

      if (itemControlID != m_DropData.lastControlID ||
      ...
      bool expandTimerExpired =
          Time.realtimeSinceStartup - m_DropData.expandItemBeginTimer >
          k_DropExpandTimeout;
      bool mayExpand = hoveringOverIndentedContent && expandTimerExpired;

      // Auto open folders we are about to drag into
      if (targetItem != null && mayExpand && targetItem.hasChildren &&
          !m_TreeView.data.IsExpanded(targetItem)) {
        // Store the expanded array prior to drag so we can revert it with a
        // delay later
    ...
    public List<int> GetCurrentExpanded() {
      ...
      List<int> expandedIDs =
          (from item in visibleItems where
               m_TreeView.data.IsExpanded(item) select item.id)
              .ToList();
      return expandedIDs;
    }
    ...
    internal static int GetInsertionIndex(TreeViewItem parentItem,
                                          TreeViewItem targetItem,
                                          DropPosition dropPosition) {
      if (parentItem == null)
        return -1;
