...
namespace UnityEditor.IMGUI.Controls {
  public partial class MultiColumnHeader {
    ...
    public MultiColumnHeader(MultiColumnHeaderState state) { m_State = state; }
    ...
    public void SetSortingColumns(int[] columnIndices, bool[] sortAscending) {
      ...
      if (columnIndices.Length > state.maximumNumberOfSortedColumns)
        throw new ArgumentException("The maximum number of sorted columns is " +
                                    state.maximumNumberOfSortedColumns +
                                    ". Trying to set " + columnIndices.Length +
                                    " columns.");

      if (columnIndices.Length != columnIndices.Distinct().Count())
        throw new ArgumentException("Duplicate column indices are not allowed",
                                    "columnIndices");
    ...
    public MultiColumnHeaderState.Column GetColumn(int columnIndex) {
      if (columnIndex < 0 || columnIndex >= state.columns.Length)
        throw new ArgumentOutOfRangeException(
            "columnIndex",
            string.Format(
                "columnIndex {0} is not valid when the current column count is {1}",
      ...
      set {
        if (value == null)
          throw new ArgumentNullException(
              "state", "MultiColumnHeader state is not allowed to be null");
        m_State = value;
      }
    }
    ...
    public int GetVisibleColumnIndex(int columnIndex) {
      ...
      string visibleIndices = string.Join(
          ", ", state.visibleColumns.Select(t => t.ToString()).ToArray());
      throw new ArgumentException(
          string.Format(
              "Invalid columnIndex: {0}. The index is not part of the current visible columns: {1}",
              columnIndex, visibleIndices),
          "columnIndex");
    }
      ...
      if (visibleColumnIndex < 0 || visibleColumnIndex >= m_ColumnRects.Length)
        throw new ArgumentException(
            string.Format(
                ...
                visibleColumnIndex, m_ColumnRects.Length),
            "visibleColumnIndex");
    ...
    void UpdateColumnHeaderRects(Rect totalHeaderRect) {
      if (m_ColumnRects == null ||
          m_ColumnRects.Length != state.visibleColumns.Length)
    ...
    public virtual void OnGUI(Rect rect, float xScroll) {
      ...
      if (m_ResizeToFit && evt.type == EventType.Repaint) {
        m_ResizeToFit = false;
        ResizeColumnsWidthsProportionally(
            rect.width - GUI.skin.verticalScrollbar.fixedWidth -
            state.widthOfAllVisibleColumns);
      }

      // We create a guiclip to let the header be able to scroll horizontally
      // according to the tree view's horizontal scroll
      GUIClip.Push(rect, new Vector2(-xScroll, 0f), Vector2.zero, false);
      {
        ...
        if (evt.type == EventType.ContextClick &&
            backgroundRect.Contains(evt.mousePosition)) {
          evt.Use();
          DoContextMenu();
        }

        // Update column rects (cached for clients to have fast access to column
        // rects by using GetCellRect)
        ...
        for (int v = 0; v < state.visibleColumns.Length; v++) {
          ...
          Rect dividerRect =
              new Rect(headerRect.xMax - 1, headerRect.y + limitHeightOfDivider,
                       1f, headerRect.height - 2 * limitHeightOfDivider);

          // Resize columns logic
          Rect dragRect =
              new Rect(dividerRect.x - m_DividerWidth * 0.5f, localRect.y,
                       m_DividerWidth, localRect.height);
          bool hasControl;
          column.width =
              EditorGUI.WidthResizer(dragRect, column.width, column.minWidth,
                                     column.maxWidth, out hasControl);
    ...
    DrawColumnResizing(Rect headerRect, MultiColumnHeaderState.Column column) {
    ...
    internal virtual void DrawDivider(Rect dividerRect,
                                      MultiColumnHeaderState.Column column) {
      EditorGUI.DrawRect(dividerRect, new Color(0.5f, 0.5f, 0.5f, 0.5f));
    }

    protected virtual void
    ColumnHeaderClicked(MultiColumnHeaderState.Column column, int columnIndex) {
      if (state.sortedColumnIndex == columnIndex)
    ...
    protected virtual void ColumnHeaderGUI(MultiColumnHeaderState.Column column,
                                           Rect headerRect, int columnIndex) {
      ...
      Rect labelRect = new Rect(headerRect.x, headerRect.yMax - labelHeight,
                                headerRect.width, labelHeight);
      GUI.Label(labelRect, column.headerContent, style);
    }

    protected void SortingButton(MultiColumnHeaderState.Column column,
                                 Rect headerRect, int columnIndex) {
      // Button logic
    ...
    internal virtual Rect GetArrowRect(MultiColumnHeaderState.Column column,
                                       Rect headerRect) {
      ...
      case TextAlignment.Center:
        arrowXPos =
            headerRect.x + headerRect.width * 0.5f - sortingArrowWidth * 0.5f;
        break;
      case TextAlignment.Right:
        arrowXPos = headerRect.xMax - DefaultStyles.columnHeader.padding.right -
                    sortingArrowWidth;
        break;
      default:
      ...
      Rect arrowRect =
          new Rect(Mathf.Round(arrowXPos), arrowYPos, sortingArrowWidth, 16f);
      return arrowRect;
    }
    ...
    protected virtual void AddColumnHeaderContextMenuItems(GenericMenu menu) {
      menu.AddItem(EditorGUIUtility.TrTextContent("Resize to Fit"), false,
                   ResizeToFit);
      ...
      for (int i = 0; i < state.columns.Length; ++i) {
        ...
        var menuText = !string.IsNullOrEmpty(column.contextMenuText)
                           ? column.contextMenuText
                           : column.headerContent.text;
        if (column.allowToggleVisibility)
          menu.AddItem(new GUIContent(menuText),
                       state.visibleColumns.Contains(i), ToggleVisibility, i);
        else
    ...
    void ToggleVisibility(object userData) { ToggleVisibility((int) userData); }
      ...
      if (Event.current.type == EventType.Repaint) {
        if (m_PreviousRect.width > 0f) {
          ...
          if (deltaWidth != 0f) {
            ...
            bool isColumnsVisible =
                rect.width - tep > state.widthOfAllVisibleColumns;
    ...
    void ResizeColumnsWidthsProportionally(float deltaWidth) {
      ...
      foreach (var column in autoResizeColumns) {
        ...
        column.width =
            Mathf.Clamp(column.width, column.minWidth, column.maxWidth);
      }
    }
  }
}
