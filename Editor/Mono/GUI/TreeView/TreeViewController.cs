...
namespace UnityEditor.IMGUI.Controls {
  /*
   ...
   The TreeViewController requires implementations from the following three
   interfaces: ITreeViewDataSource:    Should handle data fetching and data
   structure ITreeViewGUI:           Should handle visual representation of
   TreeView and mouse input on row controls ITreeViewDragging:      Should
   handle dragging, temp expansion of items, allow/disallow dropping The
   TreeViewController handles: Navigation, Item selection and initiates dragging

   Important concepts:
   1) The Item Tree: the DataSource should create the tree structure of items
   with parent and children references 2) Rows: the DataSource should be able to
   provide the visible items; a simple list that will become the rows we render.
   3) The root item might not be visible; its up to the data source to deliver a
   set of visible items from the tree
  ...
  public class TreeViewState {
    public List<int> selectedIDs {
      ...
      set { m_SelectedIDs = value; }
    }
    public int lastClickedID {
      get { return m_LastClickedID; }
      set { m_LastClickedID = value; }
    }
    public List<int> expandedIDs {
      get { return m_ExpandedIDs; }
      set { m_ExpandedIDs = value; }
    }
    internal RenameOverlay renameOverlay {
      get { return m_RenameOverlay; }
      set { m_RenameOverlay = value; }
    }
    ...
    [SerializeField]
    private int m_LastClickedID; // used for navigation
    ...
    internal virtual void OnAwake() {
      // Clear state that should not survive closing/starting Unity (If
      // TreeViewState is in EditorWindow that are serialized in a layout file)
  ...
  internal class TreeViewController {
    public System.Action<int[]> selectionChangedCallback {
      get; // ids
      set;
    }
    public System.Action<int> itemSingleClickedCallback {
      get; // id
      set;
    }
    public System.Action<int> itemDoubleClickedCallback {
      get; // id
      set;
    }
    public System.Action<int[], bool> dragEndedCallback {
      get; // dragged ids, if null then drag was not allowed, bool == true if
           // dragging tree view items from own treeview, false if drag was
           // started outside
      set;
    }
    public System.Action<int> contextClickItemCallback {
      get; // clicked item id
      set;
    }
    public System.Action contextClickOutsideItemsCallback {
      get;
      set;
    }
    public System.Action keyboardInputCallback {
      get;
      set;
    }
    public System.Action expandedStateChanged {
      get;
      set;
    }
    public System.Action<string> searchChanged {
      get;
      set;
    }
    public System.Action<Vector2> scrollChanged {
      get;
      set;
    }
    public System.Action<int, Rect> onGUIRowCallback {
      get; // <id, Rect of row>
      set;
    }

    internal System.Action<int, Rect> onFoldoutButton {
      get; // <id, Rect of row>
      set;
    }

    // Main state
    GUIView m_GUIView; // Containing view for this tree: used for checking if we
                       // have focus and for requesting repaints
    public ITreeViewDataSource data {
      get; // Data provider for this tree: handles data fetching
      set;
    }
    ...
    public ITreeViewGUI gui {
      get; // Handles GUI (input and rendering)
      set;
    }
    public TreeViewState state {
      get; // State that persists script reloads
      set;
    }
    ...
    public TreeViewItemExpansionAnimator expansionAnimator {
      get { return m_ExpansionAnimator; }
    }
    readonly TreeViewItemExpansionAnimator m_ExpansionAnimator =
        new TreeViewItemExpansionAnimator();
    ...
    public bool enableItemHovering {
      get;
      set;
    }
    ...
    internal const string kExpansionAnimationPrefKey =
        "TreeViewExpansionAnimation";
    bool m_UseExpansionAnimation =
        EditorPrefs.GetBool(kExpansionAnimationPrefKey, true);
    public bool useExpansionAnimation {
      get { return m_UseExpansionAnimation; }
      set { m_UseExpansionAnimation = value; }
    }
    ...
    Rect m_VisibleRect;
    Rect m_ContentRect;
    bool m_HadFocusLastEvent; // Cached from last event for keyboard focus
                              // changed event
    ...
    public TreeViewController(EditorWindow editorWindow,
                              TreeViewState treeViewState) {
      m_GUIView = editorWindow ? editorWindow.m_Parent : GUIView.current;
      state = treeViewState;
    }

    public void Init(Rect rect, ITreeViewDataSource data, ITreeViewGUI gui,
                     ITreeViewDragging dragging) {
      this.data = data;
      this.gui = gui;
      this.dragging = dragging;
      m_VisibleRect = m_TotalRect =
          rect; // We initialize the total rect because it might be needed for
                // framing selection when reloading data the first time.

      // Allow sub systems to set up delegates etc after treeview references
      // have been set up
      data.OnInitialize();
      gui.OnInitialize();
      if (dragging != null)
    ...
    void ExpandedStateHasChanged() { m_StopIteratingItems = true; }
    ...
    public bool showingVerticalScrollBar {
      get {
        return m_VisibleRect.height > 0 &&
               m_ContentRect.height > m_VisibleRect.height;
      }
    }

    public bool showingHorizontalScrollBar {
      get {
        return m_VisibleRect.width > 0 &&
               m_ContentRect.width > m_VisibleRect.width;
      }
    }

    public string searchString {
      get { return state.searchString; }
      set {
    ...
    public bool IsSelected(int id) { return state.selectedIDs.Contains(id); }

    public bool HasSelection() { return state.selectedIDs.Count > 0; }

    public int[] GetSelection() { return state.selectedIDs.ToArray(); }
    ...
    public void SetSelection(int[] selectedIDs,
      ...
      // Keep for debugging
      // Debug.Log ("SetSelection: new selection: " +
      // DebugUtils.ListToString(new List<int>(selectedIDs)));
      ...
      if (selectedIDs.Length > 0) {
        ...
        bool hasLastClicked =
            state.selectedIDs.IndexOf(state.lastClickedID) >= 0;
        if (!hasLastClicked) {
          // See if we can find a valid id, we check the last selected
          // (selectedids might contain invalid ids e.g scene objects in project
    ...
    public TreeViewItem FindItem(int id) { return data.FindItem(id); }

    [System.Obsolete(
        "SetUseScrollView has been deprecated. Use property useScrollView instead.")]
    ...
    public void ReloadData() {
      // Do not clear rename data here, we could be reloading due to assembly
      // reload and we want to let our rename session survive that
    ...
    public bool HasFocus() {
      bool hasKeyFocus = (m_GUIView != null)
                             ? m_GUIView.hasFocus
    ...
    public void HandleUnusedMouseEventsForItem(Rect rect, TreeViewItem item,
                                               int row) {
        ...
        if (rect.Contains(Event.current.mousePosition)) {
          ...
          if (Event.current.button == 0) {
            ...
            if (Event.current.clickCount == 2) {
              ...
              bool dragAbortedBySlowSelect =
                  (Time.realtimeSinceStartup - selectStartTime) >
                  kSlowSelectTimeout;

              bool canStartDrag =
                  !dragAbortedBySlowSelect && dragging != null &&
                  dragSelection.Count != 0 &&
                  dragging.CanStartDrag(item, dragSelection,
                                        Event.current.mousePosition);
              if (canStartDrag) {
                // Prepare drag and drop delay (we start the drag after a couple
                // of pixels mouse drag: See the case MouseDrag below)
                m_DragSelection = dragSelection;
                DragAndDropDelay delay =
                    (DragAndDropDelay) GUIUtility.GetStateObject(
                        typeof(DragAndDropDelay), GetItemControlID(item));
                delay.mouseDownPosition = Event.current.mousePosition;
              } else {
                // If dragging is not supported or not allowed for the drag
                // selection then handle selection on mouse down (when dragging
                // is handled we handle selection on mouse up to e.g allow to
                // drag to object fields in the inspector)
                m_DragSelection.Clear();
                if (m_AllowRenameOnMouseUp)
                  m_AllowRenameOnMouseUp =
                      (state.selectedIDs.Count == 1 &&
                       state.selectedIDs[0] ==
                           item.id); // If first time selection then prevent
                                     // starting a rename on the following mouse
                                     // up after this mouse down
      ...
      case EventType.MouseUp:
        if (GUIUtility.hotControl == itemControlID) {
          ...
          // On Mouse up either start name editing or change selection (if not
          // done on mouse down)
          if (rect.Contains(evt.mousePosition)) {
            Rect renameActivationRect = gui.GetRenameRect(rect, row, item);
            List<int> selected = state.selectedIDs;
            if (m_AllowRenameOnMouseUp && selected != null &&
                selected.Count == 1 && selected[0] == item.id &&
                renameActivationRect.Contains(evt.mousePosition) &&
      ...
      case EventType.ContextClick:
        if (rect.Contains(evt.mousePosition)) {
          // Do not use the event so the client can react to the context click
    ...
    public void GrabKeyboardFocus() { m_GrabKeyboardFocus = true; }
    ...
    public Vector2 GetContentSize() { return gui.GetTotalSize(); }

    public Rect GetTotalRect() { return m_TotalRect; }

    public void SetTotalRect(Rect rect) { m_TotalRect = rect; }

    public bool IsItemDragSelectedOrSelected(TreeViewItem item) {
      return m_DragSelection.Count > 0 ? m_DragSelection.Contains(item.id)
                                       : state.selectedIDs.Contains(item.id);
    }

    public bool animatingExpansion {
      get { return m_UseExpansionAnimation && m_ExpansionAnimator.isAnimating; }
    }

    void DoItemGUI(TreeViewItem item, int row, float rowWidth, bool hasFocus) {
      // Check valid row
      if (row < 0 || row >= data.rowCount) {
        Debug.LogError("Invalid. Org row: " + (row) + " Num rows " +
                       data.rowCount);
        return;
      }
      ...
      if (onGUIRowCallback != null) {
        ...
        Rect indentedRect = new Rect(rowRect.x + indent, rowRect.y,
                                     rowRect.width - indent, rowRect.height);
    ...
    public void OnGUI(Rect rect, int keyboardControlID) {
      ...
      if (m_GUIView != null && !m_GUIView.hasFocus &&
          state.renameOverlay.IsRenaming()) {
        EndNameEditing(true);
      }
      ...
      bool isMouseDownInTotalRect = evt.type == EventType.MouseDown &&
                                    m_TotalRect.Contains(evt.mousePosition);
      if (isMouseDownInTotalRect) {
        m_AllowRenameOnMouseUp = true; // reset value (can be changed later in
                                       // this event if the TreeView gets focus)
      }

      // Might change expanded state so call before InitIfNeeded (delayed
      // collapse until animation is done)
      if (animatingExpansion)
      ...
      if (m_UseScrollView) {
        state.scrollPos = GUI.BeginScrollView(
            m_TotalRect, state.scrollPos, m_ContentRect, false, false,
            horizontalScrollbarStyle != null ? horizontalScrollbarStyle
            : GUI.skin.horizontalScrollbar,
              verticalScrollbarStyle != null ? verticalScrollbarStyle
            : GUI.skin.verticalScrollbar,
              scrollViewStyle != null ? scrollViewStyle
            : EditorStyles.scrollViewAlt);
      } else
      ...
      if (evt.type == EventType.Repaint) {
        if (m_UseScrollView) {
          ...
          if (scrollView != null) {
            // Calculate the visible area of the TreeView inside of the
            // ScrollView taking into account that the TreeView may not be
            // contained within the whole ScrollView area.
            state.scrollPos = Vector2.Max(
                Vector2.zero, scrollView.scrollPosition - m_TotalRect.min -
                                  scrollView.position.min);
            m_VisibleRect = scrollView.visibleRect;
            m_VisibleRect.size =
                Vector2.Max(Vector2.zero,
                            Vector2.Min(m_VisibleRect.size,
                                        (m_TotalRect.size - state.scrollPos)));
          } else {
            m_VisibleRect = m_TotalRect;
          }
        }
      ...
      if (lastRow >= 0) {
        ...
        float rowWidth =
            Mathf.Max(GUIClip.visibleRect.width, m_ContentRect.width);

        IterateVisibleItems(firstRow, numVisibleRows, rowWidth, HasFocus());
      }

      // Call before gui.EndRowGUI() so stuff we render in EndRowGUI does not
      // end up in the the animation clip rect
      if (animatingExpansion)
      ...
      // Prevent controlID inconsistency for the controls following this tree
      // view: We use the hint parameter of GetControlID to fast forward to a
      // fixed entry in the id list so the following controls always start from
      // there regardless of the rows that have been culled.
    ...
    void HandleTreeViewGotFocus(bool isMouseDownInTotalRect) {
      ...
      // Detect if TreeView got keyboard focus (ignore layout event which gets
      // fired infront of mousedown)
      bool hasFocus = HasFocus();
      if (hasFocus != m_HadFocusLastEvent) {
        m_HadFocusLastEvent = hasFocus;

        if (hasFocus && isMouseDownInTotalRect) {
          // If we got focus this event by mouse down then we do not want to
          // begin renaming if clicking on an already selected item in the up
          // coming MouseUp event.
          m_AllowRenameOnMouseUp = false;
        }
      }
    }

    void IterateVisibleItems(int firstRow, int numVisibleRows, float rowWidth,
                             bool hasFocus) {
      // We stop iterating items if datasource state changes while iterating its
      // items. This can happen e.g when dragging items or items are
      ...
      for (int i = 0; i < numVisibleRows; ++i) {
        ...
        if (animatingExpansion) {
          ...
          // items after the expanding/collapsing items. When no more items can
          // get culled then keep adding the offset (to not handle already
          // handled items).
          ...
          float screenSpaceRowY =
              gui.GetRowRect(row, rowWidth).y - state.scrollPos.y;
        ...
        if (enableItemHovering) {
          Rect rowRect = gui.GetRowRect(
              row, showingVerticalScrollBar ? rowWidth - kSpaceForScrollBar
              : rowWidth);
        ...
        // Item GUI
        // Note that DoItemGUI() needs to be called right before checking
        // m_StopIteratingItems since UI in the current row can issue a reload
    ...
    private void ExpansionAnimationEnded(TreeViewAnimationInput setup) {
      // When collapsing we delay the actual collapse until the animation is
      // done
      if (!setup.expanding) {
    ...
    float GetAnimationDuration(float height) {
      ...
      // We have found from usability testing that for smaller height changes
      // (e.g 3-4 rows) we want a faster animation
      const float kThreshold = 60f;
      const float kMaxDuration = 0.07f;
      return (height > kThreshold) ? kMaxDuration
                                   : (height * kMaxDuration / kThreshold);
    }

    public void UserInputChangedExpandedState(TreeViewItem item, int row,
                                              bool expand) {
      var includeChildren = Event.current.alt;
      if (useExpansionAnimation) {
        // We need to expand prior to starting animation so we have the expanded
        // state ready
        if (expand)
    ...
    protected virtual Rect GetRectForRows(int startRow, int endRow,
                                          float rowWidth) {
      Rect startRect = gui.GetRowRect(startRow, rowWidth);
      Rect endRect = gui.GetRowRect(endRow, rowWidth);
      return new Rect(startRect.x, startRect.y, rowWidth,
                      endRect.yMax - startRect.yMin);
    }
    ...
    void HandleUnusedEvents() {
      ...
      case EventType.MouseDown:
        ...
        if (deselectOnUnhandledMouseDown && containsMouse &&
            Event.current.button == 0 && state.selectedIDs.Count > 0) {
    ...
    public void OnEvent() { state.renameOverlay.OnEvent(); }
    ...
    public bool BeginNameEditing(float delay) {
      ...
      foreach (int id in state.selectedIDs) {
        ...
        if (visibleAndSelectedItem == null)
          ...
          return false; // Don't allow rename if more than one item is both
                        // visible and selected
      }

      if (visibleAndSelectedItem != null &&
          data.IsRenamingItemAllowed(visibleAndSelectedItem))
    ...
    void HandleFastCollapse(TreeViewItem item, int row) {
      if (item.depth == 0) {
        // At depth 0 traverse upwards until a parent is found and select that
        // item
        ...
        // Traverse upwards until parent of item is found and select that parent
        // (users want this behavior)
        for (int i = row - 1; i >= 0; --i) {
      ...
      if (Event.current.type == EventType.KeyDown) {
        ...
        case KeyCode.PageUp: {
          ...
          if (lastClickedItem != null) {
            int numRowsPageUp = gui.GetNumRowsOnPageUpDown(
                lastClickedItem, true, m_TotalRect.height);
        ...
        case KeyCode.PageDown: {
          ...
          TreeViewItem lastClickedItem = data.FindItem(state.lastClickedID);
          if (lastClickedItem != null) {
            int numRowsPageDown = gui.GetNumRowsOnPageUpDown(
                lastClickedItem, true, m_TotalRect.height);
    ...
    public Func<TreeViewItem, bool, bool, List<int>> getNewSelectionOverride {
      private get;
      set;
    }

    // Returns list of selected ids
    List<int> GetNewSelection(TreeViewItem clickedItem, bool keepMultiSelection,
                              bool useShiftAsActionKey) {
      if (getNewSelectionOverride != null)
        return getNewSelectionOverride(clickedItem, keepMultiSelection,
                                       useShiftAsActionKey);

      var selectState = new TreeViewSelectState(){
          selectedIDs = state.selectedIDs, lastClickedID = state.lastClickedID,
    ...
    public void SelectionClick(TreeViewItem itemClicked,
                               bool keepMultiSelection) {
      var newSelection =
          GetNewSelection(itemClicked, keepMultiSelection, false);
      NewSelectionFromUserInteraction(newSelection,
                                      itemClicked != null ? itemClicked.id : 0);
    }
    ...
    float GetTopPixelOfRow(int row) { return gui.GetRowRect(row, 1).y; }
    ...
    void EnsureRowIsVisible(int row, bool animated) {
      ...
      if (row >= 0) {
        // Adjusting for when the horizontal scrollbar is being shown. Before
        // the TreeView has repainted once the m_VisibleRect is not valid, then
        // we use m_TotalRect which is passed when initialized
        float visibleHeight = m_VisibleRect.height > 0 ? m_VisibleRect.height
                                                       : m_TotalRect.height;
    ...
    public void Frame(int id, bool frame, bool ping, bool animated) {
      ...
      if (ping) {
        ...
        if (topPixelOfRow >= 0f && row >= 0 && row < data.rowCount) {
          ...
          float scrollBarOffset = GetContentSize().y >
                                  m_TotalRect.height ? - kSpaceForScrollBar : 0f;
          gui.BeginPingItem(item, topPixelOfRow,
                            m_TotalRect.width + scrollBarOffset);
        }
      }
    }

    public void EndPing() { gui.EndPingItem(); }

    // Item holding most basic data for TreeView event handling
    // Extend this class to hold your specific tree data and make your
    // DataSource create items of your type. During e.g OnGUI you can the cast
    // Item to your own type when needed.
    ...
    public List<int> SortIDsInVisiblityOrder(IList<int> ids) {
      ...
      // Some rows with selection are collapsed (not visible) so add those to
      // the end
      if (ids.Count != sorted.Count) {
        sorted.AddRange(ids.Except(sorted));
        if (ids.Count != sorted.Count)
          Debug.LogError("SortIDsInVisiblityOrder failed: " + ids.Count +
                         " != " + sorted.Count);
      }
