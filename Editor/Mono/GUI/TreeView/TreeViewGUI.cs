...
namespace UnityEditor.IMGUI.Controls {
  internal abstract class TreeViewGUI : ITreeViewGUI {
    ...
    public float iconTotalPadding {
      get { return iconLeftPadding + iconRightPadding; }
    }
    public System.Action<TreeViewItem, Rect> iconOverlayGUI {
      get; // Rect includes iconLeftPadding and iconRightPadding
      set;
    }
    public System.Action<TreeViewItem, Rect> labelOverlayGUI {
      get;
      set;
    }
    ...
    public float indentWidth {
      get { return k_IndentWidth + iconTotalPadding; }
    }
    ...
    public float halfDropBetweenHeight {
      get { return k_HalfDropBetweenHeight; }
    }

    public virtual float topRowMargin {
      get { return k_TopRowMargin; }
    }
    public virtual float bottomRowMargin {
      get { return k_BottomRowMargin; }
    }
    ...
    private GUIStyle renameStyle {
      get {
        ...
        renameStyle.fontSize =
            lineStyle.fontSize; // the textField should have the same text size
                                // as the line
    ...
    public TreeViewGUI(TreeViewController treeView) { m_TreeView = treeView; }
    ...
    // Calc correct width if horizontal scrollbar is wanted return new
    // Vector2(1, height)
    virtual public Vector2 GetTotalSize() {
      ...
      // Height
      float height = m_TreeView.data.rowCount * k_LineHeight + topRowMargin +
                     bottomRowMargin;

      if (m_AnimateScrollBarOnExpandCollapse &&
          m_TreeView.expansionAnimator.isAnimating) {
    ...
    protected float GetMaxWidth(IList<TreeViewItem> rows) {
      ...
      foreach (TreeViewItem item in rows) {
        ...
        lineStyle.CalcMinMaxWidth(GUIContent.Temp(item.displayName),
                                  out minNameWidth, out maxNameWidth);
        width += maxNameWidth;
    ...
    virtual public int GetNumRowsOnPageUpDown(TreeViewItem fromItem,
                                              bool pageUp,
                                              float heightOfTreeView) {
      return (int) Mathf.Floor(heightOfTreeView / k_LineHeight);
    }

    // Should return the row index of the first and last row thats fits in the
    // pixel rect defined by top and height
    virtual public void GetFirstAndLastRowVisible(out int firstRowVisible,
                                                  out int lastRowVisible) {
      if (m_TreeView.data.rowCount == 0 ||
          Mathf.Approximately(m_TreeView.visibleRect.height, 0.0f)) {
      ...
      firstRowVisible =
          (int) Mathf.Floor((topPixel - topRowMargin) / k_LineHeight);
      lastRowVisible =
          firstRowVisible + (int) Mathf.Ceil(heightInPixels / k_LineHeight);
      ...
      if (firstRowVisible >= m_TreeView.data.rowCount && firstRowVisible > 0) {
        // Reset scroll if it was invalid, this can be the case if scroll y
        // value was serialized and loading new tree data
    ...
    virtual public void BeginRowGUI() {
      ...
      SyncFakeItem(); // After domain reload we ensure to reconstruct new Item
                      // state

      // Input for rename overlay (repainted in EndRowGUI to ensure rendered on
      // top)
    ...
    virtual public void EndRowGUI() {
      ...
      if (m_DraggingInsertionMarkerRect.x >= 0 &&
        ...
        insertionStyle.Draw(m_DraggingInsertionMarkerRect, false, false, false,
                            false);
      }
    ...
    virtual public void OnRowGUI(Rect rowRect, TreeViewItem item, int row,
                                 bool selected, bool focused) {
      DoItemGUI(rowRect, row, item, selected, focused, false);
    }

    protected virtual void DrawItemBackground(Rect rect, int row,
                                              TreeViewItem item, bool selected,
                                              bool focused) {
      // override for custom rendering of background behind selection and drop
      // effect rendering
    }

    public virtual Rect GetRenameRect(Rect rowRect, int row,
                                      TreeViewItem item) {
      ...
      // By default we top align the rename rect to follow the label style,
      // foldout and controls alignment
      return new Rect(rowRect.x + offset, rowRect.y, rowRect.width - offset,
                      rowRect.height);
    }

    virtual protected void DoItemGUI(Rect rect, int row, TreeViewItem item,
                                     bool selected, bool focused,
                                     bool useBoldFont) {
      EditorGUIUtility.SetIconSize(new Vector2(
          k_IconWidth, k_IconWidth)); // If not set we see icons scaling down if
                                      // text is being cropped
      ...
      if (m_TreeView.dragging != null)
        isDropTarget =
            m_TreeView.dragging.GetDropTargetControlID() == itemControlID &&
            m_TreeView.data.CanBeParent(item);
      bool isRenamingThisItem = IsRenaming(item.id);
      bool showFoldout = m_TreeView.data.IsExpandable(item);

      // Adjust edit field if needed (on repaint since on layout rect.width is
      // invalid when using GUILayout)
      ...
      if (Event.current.type == EventType.Repaint) {
        ...
        // Draw drop marker
        if (isDropTarget) {
          Styles.lineStyle.Draw(GetDropTargetRect(rect), GUIContent.none, true,
                                true, false, false);
        }

        // Show insertion marker below this item (rendered end of rows)
        if (m_TreeView.dragging != null &&
            m_TreeView.dragging.GetRowMarkerControlID() == itemControlID) {
          float yPos = (m_TreeView.dragging.drawRowMarkerAbove ? rect.y
                        : rect.yMax) -
                       insertionStyle.fixedHeight * 0.5f;
          m_DraggingInsertionMarkerRect =
              new Rect(rect.x + indent + extraInsertionMarkerIndent +
                           foldoutStyleWidth + lineStyle.margin.left,
                       yPos, rect.width - indent, rect.height);
        }
      }
      ...
      OnContentGUI(rect, row, item, label, selected, focused, useBoldFont,
                   false);
    ...
    protected virtual Rect GetDropTargetRect(Rect rect) { return rect; }
    ...
    public virtual Rect GetRectForFraming(int row) {
      return GetRowRect(row, 1); // We ignore width by default when framing
                                 // (only y scroll is affected)
    }
    ...
    protected virtual Rect DoFoldout(Rect rect, TreeViewItem item, int row) {
      float indent = GetFoldoutIndent(item);
      Rect foldoutRect = new Rect(rect.x + indent, GetFoldoutYPosition(rect.y),
                                  foldoutStyleWidth, k_LineHeight);
      FoldoutButton(foldoutRect, item, row, foldoutStyle);
      return foldoutRect;
    }

    protected virtual void FoldoutButton(Rect foldoutRect, TreeViewItem item,
                                         int row, GUIStyle foldoutStyle) {
      var expansionAnimator = m_TreeView.expansionAnimator;

      bool newExpandedValue;
      EditorGUI.BeginChangeCheck();
      {
        bool expandedState = expansionAnimator.IsAnimating(item.id)
                                 ? expansionAnimator.isExpanding
                                 : m_TreeView.data.IsExpanded(item);
        newExpandedValue =
            DoFoldoutButton(foldoutRect, expandedState, foldoutStyle);
      }
    ...
    protected virtual bool DoFoldoutButton(Rect foldoutRect, bool expandedState,
                                           GUIStyle foldoutStyle) {
      return GUI.Toggle(foldoutRect, expandedState, GUIContent.none,
                        foldoutStyle);
    }

    protected virtual void OnAdditionalGUI(Rect rect, int row,
                                           TreeViewItem item, bool selected,
                                           bool focused) {}

    protected virtual void OnContentGUI(Rect rect, int row, TreeViewItem item,
    ...
    virtual public void BeginPingItem(TreeViewItem item, float topPixelOfRow,
      ...
      if (topPixelOfRow >= 0f) {
        ...
        m_Ping.m_ContentRect =
            new Rect(GetContentIndent(item) + extraSpaceBeforeIconAndLabel,
                     topPixelOfRow,
                     k_IconWidth + k_SpaceBetweenIconAndText + contentSize.x +
                         iconTotalPadding,
                     contentSize.y);
        ...
        m_Ping.m_ContentDraw = (Rect r) => {
          ...
          OnContentGUI(r, row, item, item.displayName, false, false,
                       useBoldFont, true);
        };
    ...
    virtual public void EndPingItem() { m_Ping.m_TimeStart = -1f; }
    ...
    virtual public void EndRename() {
      // We give keyboard focus back to our tree view because the rename utility
      // stole it (now we give it back)
      if (GetRenameOverlay().HasKeyboardFocus())
        m_TreeView.GrabKeyboardFocus();

      RenameEnded();
      ClearRenameAndNewItemState(); // Ensure clearing if RenameEnden is
                                    // overrided
    }
