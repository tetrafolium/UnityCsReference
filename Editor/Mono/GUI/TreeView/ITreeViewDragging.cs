...
namespace UnityEditor.IMGUI.Controls {
  ...
  //  ITreeViewGUI:           Should handle visual representation of TreeView
  //  and input handling ITreeViewDragging       Should handle dragging, temp
  ...
  internal interface ITreeViewDragging {
    void OnInitialize();
    bool CanStartDrag(TreeViewItem targetItem, List<int> draggedItemIDs,
                      Vector2 mouseDownPosition);
    void StartDrag(TreeViewItem draggedItem, List<int> draggedItemIDs);
    bool DragElement(
        TreeViewItem targetItem, Rect targetItemRect,
        int row); // 'targetItem' is null when not hovering over any target
                  // Item.  Returns true if drag was handled.
