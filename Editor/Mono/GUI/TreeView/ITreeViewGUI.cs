...
//  ITreeViewGUI:           Should handle visual representation of TreeView and
//  input handling ITreeViewDragging       Should handle dragging, temp
//  expansion of items, allow/disallow dropping
...
namespace UnityEditor.IMGUI.Controls {
  internal interface ITreeViewGUI {
    ...
    // Should return the row number of the first and last row thats fits between
    // top pixel and the height of the window If the treeview contains items
    // with varying heights then use the minium height for determining the
    // lastRowVisible this is needed when animating the treeview to ensure all
    // items are rendered while animating. Can use TreeView.GetTotalRect and
    // m_TreeView.state.scrollPos.y for calculating first and last values
    void GetFirstAndLastRowVisible(out int firstRowVisible,
                                   out int lastRowVisible);

    Rect GetRowRect(int row, float rowWidth);
    Rect GetRectForFraming(int row);

    int GetNumRowsOnPageUpDown(TreeViewItem fromItem, bool pageUp,
    ...
    void EndRowGUI();   // use for handling stuff after all rows have had their
                        // OnRowGUI

    // Ping Item interface (implement a rendering of a 'ping' for a Item).
    void BeginPingItem(TreeViewItem item, float topPixelOfRow,
