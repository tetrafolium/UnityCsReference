...
// AssetsTreeViewDataSource only fetches current visible items of the asset
// database tree, because we derive from LazyTreeViewDataSource Note: every time
// a Item's expanded state changes FetchData is called
...
public RootItem(int instanceID, string displayName, string path,
...
public AssetsTreeViewDataSource(TreeViewController treeView,
...
public override List<int> GetNewSelection(TreeViewItem clickedItem,
	...
	var result = InternalEditorUtility.GetNewSelection(
		ref clickedEntry, allIDs, allGuids, selectedIDs, lastClickedID,
		selectState.keepMultiSelection, selectState.useShiftAsActionKey,
		allowMultiselection);
...
private void BuildRoots() {
	...
	m_Roots.Add(new RootItem(ProjectBrowser.kPackagesFolderInstanceId,
	                         packagesMountPoint, packagesMountPoint, true));
	foreach (var package in packages) {
		...
		m_Roots.Add(new RootItem(packageFolderInstanceID, displayName,
		                         package.assetPath));
	}
}
...
public override void FetchData() {
	...
	if (multiRoot) {
		...
		var displayName =
			m_Roots[0].displayName ?? CreateDisplayName(rootInstanceID);
	...
	foreach (var root in m_Roots) {
		...
		if (multiRoot) {
			...
			if (subDepth > 0)
				rootItem = new FolderTreeItem(rootGuid, !property.hasChildren,
				                              rootInstanceID, rootDepth, parentItem,
				                              displayName);
			else
				rootItem = new RootTreeItem(rootInstanceID, rootDepth, parentItem,
				                            displayName);
		...
		bool shouldExpandIt =
			m_ExpandAtFirstTime && (rootItem.id == assetsInstanceIDs);
		if (IsExpanded(rootItem.id) &&
		    (rootItem == m_RootItem || IsExpanded(rootItem.parent.id)) ||
		    shouldExpandIt) {
			...
			while (property.NextWithDepthCheck(expandIDs, minDepth)) {
				if (!foldersOnly || property.isFolder) {
					...
					if (property.isFolder)
						item = new FolderTreeItem(property.guid, !property.hasChildren,
						                          property.instanceID, depth + subDepth,
						                          null, property.name);
					else
						item = new NonFolderTreeItem(
							property.guid, property.GetInstanceIDIfImported(),
							depth + subDepth, null, property.name);

					item.icon = property.icon;

					if (property.hasChildren) {
						item.AddChild(null); // add a dummy child in children list to
						                     // ensure we show the collapse arrow (because
						                     // we do not fetch data for collapsed items)
					}
					rows.Add(item);
				}
			}
	...
	bool frameLastSelected =
		false; // use false because we might just be expanding/collapsing a Item
	               // (which would prevent collapsing a Item with a selected child)
...
static void FoldersFirstRecursive(TreeViewItem item) {
	...
	// Parent child relation is untouched, we simply move child folders to the
	// beginning of the children array while keeping folders and files sorted.
	TreeViewItem[] children = item.children.ToArray();
	for (int nonFolderPos = 0; nonFolderPos < item.children.Count;
	     ++nonFolderPos) {
		if (children[nonFolderPos] == null)
			continue;

		if (children[nonFolderPos] is NonFolderTreeItem) {
			for (int folderPos = nonFolderPos + 1; folderPos < children.Length;
			     ++folderPos) {
				...
				System.Array.Copy(children, nonFolderPos, children, nonFolderPos + 1,
				                  length);
...
protected override void GetParentsBelow(int id, HashSet<int> parentsBelow) {
	...
	if (m_Roots.Count > 1) {
		...
		if (id != assetsInstanceIDs) {
			...
			var item = m_RootsTreeViewItem.Values.FirstOrDefault(
				tvi => tvi.id == id && tvi.depth == 0);
			if (item != null) {
				parentsBelow.Add(id);
...
override public void OnExpandedStateChanged() {
	if (k_HierarchyType == HierarchyType.Assets)
		InternalEditorUtility.expandedProjectWindowItems =
			expandedIDs.ToArray(); // Persist expanded state for ProjectBrowsers
...
override public void InsertFakeItem(int id, int parentID, string name,
                                    Texture2D icon) {
	bool isCreatingNewFolder =
		GetCreateAssetUtility().endAction is DoCreateFolder;
	...
	if (FindItem(parentID) != null) {
		...
		// Find pos under parent
		int insertAfterID = GetInsertAfterItemIDForNewItem(
			name, parentItem, isCreatingNewFolder, foldersFirst);
...
protected FolderTreeItemBase(int id, int depth, TreeViewItem parent,
                             string displayName)
...
public RootTreeItem(int id, int depth, TreeViewItem parent,
                    string displayName)
	: base(id, depth, parent, displayName) {
}
}
internal class PackageTreeItem : FolderTreeItemBase {
public PackageTreeItem(int id, int depth, TreeViewItem parent,
                       string displayName)
	: base(id, depth, parent, displayName) {
}
}
...
public FolderTreeItem(string guid, bool isEmpty, int id, int depth,
                      TreeViewItem parent, string displayName)
	: base(id, depth, parent, displayName) {
	Guid = guid;
	IsEmpty = isEmpty;
}
...
public NonFolderTreeItem(string guid, int id, int depth,
                         TreeViewItem parent, string displayName)
	: base(id, depth, parent, displayName) {
	Guid = guid;
}
