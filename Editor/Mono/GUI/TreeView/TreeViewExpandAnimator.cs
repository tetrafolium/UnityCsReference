...
// Setup animation, tracks animation, fires callback when done (fully
// expanded/collapsed)
//
...
public void BeginAnimating(TreeViewAnimationInput setup) {
	if (m_Setup != null) {
		if (m_Setup.item.id == setup.item.id &&
			...
			// If same item (changed expand/collapse while animating) then just
			// change direction, but skip the time that already passed
			...
			// Ensure current animation ends before starting a new (just finish it
			// immediately)
			SkipAnimating();
			m_Setup = setup;
		}
...
public void OnAfterAllRowsGUI() {
	...
	// Capture time at intervals to ensure that expansion value is consistent
	// across layout and repaint. This fixes that the scroll view showed its
	// scrollbars during expansion since using realtime would give higher
	// values on repaint than on layout event.
...
public float deltaHeight {
	get {
		return Mathf.Floor(m_Setup.rowsRect.height -
		                   m_Setup.rowsRect.height * expandedValueNormalized);
	}
}
...
bool printDebug {
	get {
		return s_Debug && (m_Setup != null) && (m_Setup.treeView != null) &&
...
public double elapsedTime {
	get { return timeCaptured - startTime; }

	set { startTime = timeCaptured - value; }
}
...
public Rect rowsRect {
	get; // the rect encapsulating startrow and endrow
	set;
}
...
public System.Action<TreeViewAnimationInput>
animationEnded;         // set to get a callback when animation ends
...
public override string ToString() {
	return "Input: startRow " + startRow + " endRow " + endRow +
	       " rowsRect " + rowsRect + " startTime " +
	       startTime.ToString(CultureInfo.InvariantCulture.NumberFormat) +
	       " anitmationDuration" + animationDuration + " " + expanding + " " +
	       item.displayName;
}
}
} // UnityEditor
