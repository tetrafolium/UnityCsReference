...
namespace UnityEditor.IMGUI.Controls {
  // TreeViewDataSource is a base abstract class for a data source for a
  // TreeView. Usage:
  ...
  //
  // Note: if dealing with very large trees use LazyTreeViewDataSource instead:
  // it assumes that tree only contains visible items.

  internal abstract class TreeViewDataSource : ITreeViewDataSource {
    protected readonly TreeViewController
        m_TreeView; // TreeView using this data source
    ...
    public bool alwaysAddFirstItemToSearchResult {
      get; // is only used in searches when showRootItem is false. It Doesn't
           // make sense for visible roots
      set;
    }
    ...
    virtual public void OnSearchChanged() { m_NeedRefreshRows = true; }

    //----------------------------
    // Visible Item section

    protected void GetVisibleItemsRecursive(TreeViewItem item,
    ...
    protected void SearchRecursive(TreeViewItem item, string search,
                                   IList<TreeViewItem> searchResult) {
    ...
    virtual protected List<TreeViewItem> Search(TreeViewItem root,
                                                string search) {
      var result = new List<TreeViewItem>();
    ...
    virtual public TreeViewItem GetItem(int row) { return GetRows()[row]; }
    ...
    virtual public int[] GetExpandedIDs() { return expandedIDs.ToArray(); }
    ...
    virtual public void SetExpandedWithChildren(TreeViewItem fromItem,
                                                bool expand) {
      if (fromItem == null) {
    ...
    virtual public bool CanBeMultiSelected(TreeViewItem item) { return true; }

    virtual public bool CanBeParent(TreeViewItem item) { return true; }

    virtual public List<int> GetNewSelection(TreeViewItem clickedItem,
      ...
      return InternalEditorUtility.GetNewSelection(
          clickedItem.id, allIDs, selectState.selectedIDs,
          selectState.lastClickedID, selectState.keepMultiSelection,
          selectState.useShiftAsActionKey, allowMultiselection);
    }
    ...
    // Fake Item should be inserted into the m_VisibleRows (not the tree
    // itself).
    virtual public void InsertFakeItem(int id, int parentID, string name,
                                       Texture2D icon) {
      Debug.LogError("InsertFakeItem missing implementation");
    }

    virtual public bool HasFakeItem() { return m_FakeItem != null; }
