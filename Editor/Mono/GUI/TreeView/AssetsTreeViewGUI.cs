...
internal class AssetsTreeViewGUI : TreeViewGUI {
  ...
  internal static ScalableGUIContent s_OpenFolderIcon =
      new ScalableGUIContent(null, null, EditorResources.openedFolderIconName);
  internal static ScalableGUIContent s_EmptyFolderIcon =
      new ScalableGUIContent(null, null, EditorResources.emptyFolderIconName);
  ...
  virtual protected bool IsCreatingNewAsset(int instanceID) {
    return GetCreateAssetUtility().IsCreatingNewAsset() &&
           IsRenaming(instanceID);
  }
  ...
  override protected void SyncFakeItem() {
    if (!m_TreeView.data.HasFakeItem() &&
      ...
      int parentInstanceID =
          AssetDatabase.GetMainAssetInstanceID(GetCreateAssetUtility().folder);
  ...
  virtual public void BeginCreateNewAsset(int instanceID,
  ...
  protected override void DoItemGUI(Rect rect, int row, TreeViewItem item,
  ...
  internal static void OnIconOverlayGUI(int instanceID, Rect overlayRect,
                                        bool addPadding,
    ...
    if (postAssetIconDrawCallback != null &&
        AssetDatabase.IsMainAsset(instanceID)) {
  ...
  private void OnLabelOverlayGUI(TreeViewItem item, Rect labelRect) {
    if (postAssetLabelDrawCallback != null &&
        AssetDatabase.IsMainAsset(item.id)) {
...
internal class TreeViewStateWithAssetUtility : TreeViewState {
  [SerializeField] CreateAssetUtility m_CreateAssetUtility =
      new CreateAssetUtility();

  internal CreateAssetUtility createAssetUtility {
    get { return m_CreateAssetUtility; }
    set { m_CreateAssetUtility = value; }
  }
