...
// Total size: 1) When changing: non changing rows + changing rows fraction, 2)
// When not changing sum of rows Size of changing rows fraction used for finding
// new endRow after last changing row
...
internal abstract class TreeViewGUIWithCustomItemsHeights : ITreeViewGUI {
  ...
  public Rect GetRectForFraming(int row) { return GetRowRect(row, 1); }

  public abstract void OnRowGUI(Rect rowRect, TreeViewItem item, int row,
                                bool selected, bool focused);

  protected virtual float AddSpaceBefore(TreeViewItem item) { return 0; }
  ...
  // Calc correct width if horizontal scrollbar is wanted return new Vector2(1,
  // height)
  ...
  public int GetNumRowsOnPageUpDown(TreeViewItem fromItem, bool pageUp,
  ...
  public void GetFirstAndLastRowVisible(out int firstRowVisible,
  ...
  public virtual void BeginPingItem(TreeViewItem item, float topPixelOfRow,
