...
internal class GameObjectTreeViewGUI : TreeViewGUI {
  ...
  internal static class GameObjectStyles {
    ...
    public static SVC<float> sceneHeaderWidth =
        new SVC<float>("SceneTopBarBg", "border-bottom-width", 1f);
    ...
    public static Color hoveredBackgroundColor =
        EditorResources.GetStyle("game-object-tree-view")
            .GetColor("-unity-object-tree-hovered-color");

    public static Texture2D sceneAssetIcon =
        EditorGUIUtility.FindTexture(typeof(SceneAsset));
  ...
  internal delegate float OnHeaderGUIDelegate(Rect availableRect,
                                              string scenePath);
  ...
  internal static void UpdateActiveParentObjectValuesForScene(string sceneGUID,
                                                              int instanceID) {
    if (instanceID == 0)
  ...
  internal void GetActiveParentObjectValuesFromSessionInfo() {
    for (int i = 0; i < SceneManager.sceneCount; i++) {
      ...
      var id = SceneHierarchy.GetDefaultParentForSession(
          SceneManager.GetSceneAt(i).guid);
      if (id != 0)
  ...
  static bool DetectSceneGuidMismatchInActiveParentState(
      KeyValuePair<string, int> activeParentObject) {
    var go = EditorUtility.InstanceIDToObject(activeParentObject.Value)
                 as GameObject;
  ...
  internal static void RemoveInvalidActiveParentObjects() {
    var itemsToRemove =
        s_ActiveParentObjectPerSceneGUID
            .Where(activeParent =>
                       DetectSceneGuidMismatchInActiveParentState(activeParent))
            .ToArray();
    foreach (var itemToRemove in itemsToRemove) {
      SceneHierarchy.UpdateSessionStateInfoAndActiveParentObjectValuesForScene(
          itemToRemove.Key, 0);
    }
  }
  ...
  public GameObjectTreeViewGUI(TreeViewController treeView,
  ...
  public override void OnInitialize() {
    SceneVisibilityManager.visibilityChanged +=
        SceneVisibilityManagerOnVisibilityChanged;
  ...
  bool DetectMouseDownInTreeViewRect() {
    ...
    if ((mouseEvent && m_TreeView.GetTotalRect().Contains(evt.mousePosition)) ||
        keyboardEvent)
      return true;
    return false;
  }

  bool showingStickyHeaders {
  ...
  void DoStickySceneHeaders() {
    ...
    if (firstRow >= 0 && lastRow >= 0) {
      ...
      if (firstRow == 0 && scrollY <= topRowMargin)
        return; // Do nothing when first row is 0 since we do not need any
                // sticky headers overlay

      var firstItem =
          (GameObjectTreeViewItem) m_TreeView.data.GetItem(firstRow);
      var nextItem =
          (GameObjectTreeViewItem) m_TreeView.data.GetItem(firstRow + 1);
      ...
      var sceneHeaderItem = dataSource.sceneHeaderItems.FirstOrDefault(
          p => p.scene == firstItem.scene);
      if (sceneHeaderItem != null) {
        bool selected =
            m_TreeView.IsItemDragSelectedOrSelected(sceneHeaderItem);
        ...
        if (GUI.Button(new Rect(rect.x, rect.y, rect.height, rect.height),
                       GUIContent.none, GUIStyle.none))
          m_TreeView.Frame(sceneHeaderItem.id, true, false);

        m_TreeView.HandleUnusedMouseEventsForItem(rect, sceneHeaderItem,
                                                  firstRow);
  ...
  void HandleStickyHeaderContextClick(Rect rect,
                                      GameObjectTreeViewItem sceneHeaderItem) {
    Event evt = Event.current;

    // On OSX manually handle context click for sticky headers here to prevent
    // items beneath the sticky header to also handle the event
    if (Application.platform == RuntimePlatform.OSXEditor) {
      bool showContextMenu =
          (evt.type == EventType.MouseDown && evt.button == 1) ||
          evt.type ==
              EventType
                  .ContextClick; // cmd+left click fires a context click event
      ...
      if (evt.type == EventType.MouseDown && evt.button == 1 &&
        ...
        // On Windows prevent right mouse down to propagate to items beneath
        // (which will select items below this item)
        evt.Use();
      }
    }
  }
  ...
  public override Rect GetRectForFraming(int row) {
    ...
    if (showingStickyHeaders && row < m_TreeView.data.rowCount) {
      ...
      if (item != null && !item.isSceneHeader) {
        // For game objects under a sceneheader we create a larger frame rect to
        // ensure row is shown beneath the sticky header (headers have same
        // lineheight as rows)
        rect.y -= k_LineHeight;
  ...
  private bool isDragging {
    get {
      return m_TreeView.isDragging ||
             (m_TreeView.dragging != null &&
              m_TreeView.dragging.GetDropTargetControlID() != -1);
    }
  }

  override protected void DrawItemBackground(Rect rect, int row,
                                             TreeViewItem item, bool selected,
                                             bool focused) {
    var goItem = (GameObjectTreeViewItem) item;
    if (goItem.isSceneHeader) {
      GUI.Label(rect, GUIContent.none, GameObjectStyles.sceneHeaderBg);
    } else {
      // Don't show indented sub scene header backgrounds when searching (as the
      // texts are not indented here)
      if (SubSceneGUI.IsUsingSubScenes() && !showingSearchResults) {
        ...
        if (gameObject != null && SubSceneGUI.IsSubSceneHeader(gameObject)) {
          SubSceneGUI.DrawSubSceneHeaderBackground(rect, k_BaseIndent,
                                                   k_IndentWidth, gameObject);
        }
      }
    }
    ...
    using(
        new GUI.BackgroundColorScope(GameObjectStyles.hoveredBackgroundColor)) {
      GUI.Label(rect, GUIContent.none,
                GameObjectStyles.hoveredItemBackgroundStyle);
    }
  }

  float m_ContentRectRight;

  override protected void DoItemGUI(Rect rect, int row, TreeViewItem item,
    ...
    SceneVisibilityHierarchyGUI.DoItemGUI(
        rect, goItem, selected && !IsRenaming(item.id),
        m_TreeView.hoveredItem == goItem, focused, isDragging);
  }

  protected override void OnAdditionalGUI(Rect rect, int row, TreeViewItem item,
                                          bool selected, bool focused) {
    GameObjectTreeViewItem goItem = item as GameObjectTreeViewItem;
    if (goItem == null)
      return;
    ...
    if (goItem.isSceneHeader) {
      ...
      if (SubSceneGUI.IsUsingSubScenes() && !showingSearchResults) {
        SubSceneGUI.DrawVerticalLine(rect, k_BaseIndent, k_IndentWidth,
                                     (GameObject) goItem.objectPPTR);
      }
    }

    if (SceneHierarchy.s_Debug)
      GUI.Label(new Rect(rect.xMax - 70, rect.y, 70, rect.height),
                "" + row + " (" + goItem.id + ")", EditorStyles.boldLabel);
  }
  ...
  protected float DoAdditionalSceneHeaderGUI(GameObjectTreeViewItem goItem,
                                             Rect rect) {
    // Options button
    ...
    Rect buttonRect =
        new Rect(rect.xMax - optionsButtonWidth - margin,
                 rect.y + (rect.height - optionsButtonHeight) * 0.5f,
                 optionsButtonWidth, rect.height);

    if (Event.current.type == EventType.Repaint)
      GameObjectStyles.optionsButtonStyle.Draw(buttonRect, false, false, false,
                                               false);
    ...
    if (EditorGUI.DropdownButton(buttonRect, GUIContent.none, FocusType.Passive,
                                 GUIStyle.none)) {
      // Ensure item is selected before using context menu (menu logic is based
      // on selection)
    ...
    if (null != OnPostHeaderGUI) {
      ...
      availableRectLeft = Math.Min(
          availableRectLeft, OnPostHeaderGUI(availableRect, goItem.scene.path));
    }
  ...
  void SetItemSelectedIcon(GameObjectTreeViewItem item) {
    if (item.icon != null) {
      item.selectedIcon =
          EditorUtility.GetIconInActiveState(item.icon) as Texture2D;
    }
  }
  ...
  void SetItemOverlayIcon(GameObjectTreeViewItem item) {
    ...
    if (!EditorApplication.isPlaying) {
      ...
      if (asset != null &&
          !m_HierarchyPrefabToAssetIDMap.ContainsKey(item.id)) {
  ...
  void SetPrefabModeButtonVisibility(GameObjectTreeViewItem item) {
    ...
    // Don't show button for disconnected prefab instances and if prefab asset
    // is missing
    if (PrefabUtility.GetPrefabInstanceStatus(go) !=
        PrefabInstanceStatus.Connected)
      return;

    // We can't simply check if the go is part of an immutable prefab, since
    // that would check the asset of the outermost prefab this go is part of.
    // Instead we have to check original source or variant root
  ...
  protected override void OnContentGUI(Rect rect, int row, TreeViewItem item,
    ...
    if (goItem.isSceneHeader) {
      ...
      using(new EditorGUI.DisabledScope(!goItem.scene.isLoaded)) {
        base.OnContentGUI(rect, row, item, label, selected, focused,
                          useBoldFont, isPinging);
      }
      return;
    }
    ...
    if (SubSceneGUI.IsUsingSubScenes())
      useBoldFont =
          SubSceneGUI.UseBoldFontForGameObject((GameObject) goItem.objectPPTR);
    ...
    if (useBoldFont) {
      ...
      if ((colorCode & 3) == (int) GameObjectColorType.Normal)
        lineStyle =
            (colorCode < 4) ? Styles.lineStyle : GameObjectStyles.disabledLabel;
      else if ((colorCode & 3) == (int) GameObjectColorType.Prefab)
        lineStyle = (colorCode < 4) ? GameObjectStyles.prefabLabel
                                    : GameObjectStyles.disabledPrefabLabel;
      else if ((colorCode & 3) == (int) GameObjectColorType.BrokenPrefab)
        lineStyle = (colorCode < 4)
                        ? GameObjectStyles.brokenPrefabLabel
                        : GameObjectStyles.disabledBrokenPrefabLabel;
    }
    ...
    if (icon != null) {
      ...
      Color col = GUI.color;
      if (renderDisabled ||
          (CutBoard.hasCutboardData && CutBoard.IsGameObjectPartOfCutAndPaste(
                                           (GameObject) goItem.objectPPTR)))
        col = new Color(1f, 1f, 1f, 0.5f);
      GUI.DrawTexture(iconRect, icon, ScaleMode.ScaleToFit, true, 0, col, 0, 0);

      if (goItem.overlayIcon != null)
        GUI.DrawTexture(iconRect, goItem.overlayIcon, ScaleMode.ScaleToFit,
                        true, 0, col, 0, 0);
  ...
  private Asset GetAsset(GameObjectTreeViewItem item) {
    ...
    Asset vcAsset =
        string.IsNullOrEmpty(guid) ? null : Provider.GetAssetByGUID(guid);
    return vcAsset;
  }

  public float PrefabModeButton(GameObjectTreeViewItem item,
                                Rect selectionRect) {
    float contentRectRight = selectionRect.xMax;
    if (item.showPrefabModeButton) {
      float yOffset =
          (selectionRect.height - GameObjectStyles.rightArrow.fixedWidth) / 2;
      Rect buttonRect = new Rect(
          selectionRect.xMax - GameObjectStyles.rightArrow.fixedWidth -
              GameObjectStyles.rightArrow.margin.right,
          selectionRect.y + yOffset, GameObjectStyles.rightArrow.fixedWidth,
          GameObjectStyles.rightArrow.fixedHeight);

      int instanceID = item.id;
      GUIContent content =
          buttonRect.Contains(Event.current.mousePosition)
              ? PrefabStageUtility.GetPrefabButtonContent(instanceID)
              : GUIContent.none;
