...
// GameObjectTreeViewDataSource only fetches current visible items of the scene
// tree, because we derive from LazyTreeViewDataSource Note: every time a Item's
// expanded state changes FetchData is called

internal class GameObjectTreeViewDataSource : LazyTreeViewDataSource {
  const double k_LongFetchTime =
      0.05; // How much time is considered to be a long fetch
  const double k_FetchDelta =
      0.1; // How much time between long fetches is acceptable.
  const int k_MaxDelayedFetch =
      5; // How many consecutive fetches can be delayed.
  ...
  int m_RowCount;
  List<TreeViewItem>
      m_ListOfRows; // We need the generic List type in this class for
                    // Add/RemoveRange and Sorting (m_Rows is IList)
  List<GameObjectTreeViewItem> m_StickySceneHeaderItems =
      new List<GameObjectTreeViewItem>();
  ...
  public List<GameObjectTreeViewItem> sceneHeaderItems {
    get { return m_StickySceneHeaderItems; }
  }
  ...
  public SearchableEditorWindow.SearchModeHierarchyWindow searchMode {
    get { return m_SearchMode; }
    set { m_SearchMode = value; }
  }
  public bool isFetchAIssue {
    get { return m_DelayedFetches >= k_MaxDelayedFetch; }
  }
  ...
  public GameObjectTreeViewDataSource(TreeViewController treeView,
                                      int rootInstanceID, bool showRoot,
                                      bool rootItemIsCollapsable)
      : base(treeView) {
  ...
  internal void SetupChildParentReferencesIfNeeded() {
    ...
    // We delay the calling SetChildParentReferences until needed to ensure fast
    // reloading of the game object hierarchy (in playmode this prevent hiccups
    // for large scenes)
  ...
  public override void RevealItem(int itemID) {
    // Optimization: Only spend time on revealing item if it is part of the
    // Hierarchy
  ...
  override public bool IsRevealed(int id) { return GetRow(id) != -1; }

  private bool IsValidHierarchyInstanceID(int instanceID) {
    bool isScene = SceneHierarchy.IsSceneHeaderInHierarchyWindow(
        EditorSceneManager.GetSceneByHandle(instanceID));
    bool isGameObject = InternalEditorUtility.GetTypeWithoutLoadingObject(
                            instanceID) == typeof(GameObject);
    return isScene || isGameObject;
  }

  HierarchyProperty FindHierarchyProperty(int instanceID) {
    // Optimization: Prevent search by early out if 'id' is not a game object
    // (or scene handle)
  ...
  override public TreeViewItem GetItem(int row) { return m_Rows[row]; }
  ...
  override public TreeViewItem FindItem(int id) {
    // Since this is a LazyTreeViewDataSource that only knows about expanded
    // items we need to reveal the item before searching for it (expand its
    // ancestors)
    RevealItem(id);

    // Since RevealItem can have reloaded data because items was revealed we
    // need to make sure that parent child references is set up
  ...
  bool ShouldShowSceneHeaders() {
    ...
    return (scenes == null || scenes.Length != 1 ||
            !EditorSceneManager.IsPreviewScene(scenes[0]));
  }
  ...
  void CreateRootItem(HierarchyProperty property) {
    ...
    if (property.isValid) {
      ...
      m_RootItem = new GameObjectTreeViewItem(m_RootInstanceID, rootDepth, null,
                                              property.name);
    } else {
      // All game objects
      m_RootItem = new GameObjectTreeViewItem(m_RootInstanceID, rootDepth, null,
  ...
  public override void FetchData() {
    ...
    if (m_RootInstanceID != 0) {
      ...
      if (!found) {
        Debug.LogError("Root gameobject with id " + m_RootInstanceID +
                       " not found!!");
        m_RootInstanceID = 0;
        property.Reset();
      }
    ...
    // Must be set before SetSelection (they calls GetRows which will fetch
    // again if m_NeedRefreshVisibleFolders = true)
    ...
    if (isSearching) {
      m_SearchSessionHandler.BeginSession(
          () => new SearchService.HierarchySearchContext{rootProperty =
                                                             property});
    }
    ...
    if (isSearching || subTreeWanted) {
      ...
      // We delay the setup of the full data and calling
      // SetChildParentReferences to ensure fast reloading of the game object
      // hierarchy
      InitializeMinimal();
    }
    ...
    m_TreeView.SetSelection(
        ...
        false); // use false because we might just be expanding/collapsing a
                // Item (which would prevent collapsing a Item with a selected
                // child)

    CreateSceneHeaderItems();

    if (SceneHierarchy.s_Debug)
      Debug.Log("Fetch time: " + fetchTotalTime * 1000.0 +
                " ms, alphaSort = " + IsUsingAlphaSort());

    Profiler.EndSample();
  }

  public override bool CanBeParent(TreeViewItem item) {
    // Ensure parent child-parent references are setup if requesting parent
    // information
    SetupChildParentReferencesIfNeeded();
  ...
  static void Resize(List<TreeViewItem> list, int count) {
    ...
    if (count < cur)
      ...
      if (count > list.Capacity) // this bit is purely an optimisation, to avoid
                                 // multiple automatic capacity changes.
        list.Capacity =
            count +
            20; // add some extra to prevent alloc'ing when adding to list
      list.AddRange(Enumerable.Repeat(default(TreeViewItem),
                                      count - cur)); // add range is nulls
    }
  }
  ...
  private void ResizeItemList(int count) {
    ...
    if (m_ListOfRows.Count != count) {
      ...
      Assert.AreEqual(m_ListOfRows.Count, count,
                      "List of rows count incorrect");
    }
  }

  private void AllocateBackingArrayIfNeeded() {
    if (m_Rows == null) {
      int startCapacity = m_RowCount > k_DefaultStartCapacity ? m_RowCount
          : k_DefaultStartCapacity;
  ...
  private void InitializeMinimal() {
    ...
    if (SceneHierarchy.s_Debug)
      Log("Init minimal (" + m_RowCount + ") num scenes " +
  ...
  void InitializeFull() {
    ...
    HierarchyProperty property = CreateHierarchyProperty();
    m_RowCount =
        property.CountRemaining(m_TreeView.state.expandedIDs.ToArray());
    ResizeItemList(m_RowCount);
    property.Reset();
  ...
  // Used for search results, sub tree view (of e.g. a prefab) and custom
  // sorting
  void InitializeProgressivly(HierarchyProperty property, bool subTreeWanted,
                              bool isSearching) {
    AllocateBackingArrayIfNeeded();
    ...
    if (!isSearching) {
      ...
      while (property.NextWithDepthCheck(expanded, minAllowedDepth)) {
        ...
        InitTreeViewItem(item, property, property.hasChildren,
                         property.depth - subtractDepth);
        row++;
      }
      m_RowCount = row;
    } else // Searching
  ...
  int InitializeSearchResults(HierarchyProperty property, int minAllowedDepth) {
    ...
    int row = 0;
    var searchFilter = SearchableEditorWindow.CreateFilter(
        searchString, (SearchableEditorWindow.SearchMode) m_SearchMode);
    var searchContext =
        (SearchService.HierarchySearchContext) m_SearchSessionHandler.context;
    ...
    while (property.NextWithDepthCheck(null, minAllowedDepth)) {
      if (!SearchService.Scene.Filter(m_SearchString, property,
                                      searchContext)) {
      ...
      var item = EnsureCreatedItem(row);
      // Add scene headers when encountering a new scene (and it's not a header
      // in itself)
      if (AddSceneHeaderToSearchIfNeeded(item, property,
                                         ref currentSceneHandle)) {
        row++;
        headerRows.Add(row);
    ...
    if (headerRows.Count > 0) {
      ...
      for (int i = 1; i < headerRows.Count; i++) {
        ...
        m_ListOfRows.Sort(currentSortStart, count,
                          new TreeViewItemAlphaNumericSort());
        currentSortStart = headerRows[i];
      }

      // last section
      m_ListOfRows.Sort(currentSortStart, numRows - currentSortStart,
                        new TreeViewItemAlphaNumericSort());
    }
  ...
  bool AddSceneHeaderToSearchIfNeeded(GameObjectTreeViewItem item,
                                      HierarchyProperty property,
                                      ref int currentSceneHandle) {
  ...
  private void InitTreeViewItem(GameObjectTreeViewItem item,
                                HierarchyProperty property,
                                bool itemHasChildren, int itemDepth) {
    InitTreeViewItem(item, property.instanceID, property.GetScene(),
                     IsSceneHeader(property), property.colorCode,
                     property.pptrValue, itemHasChildren, itemDepth);
  }

  private void InitTreeViewItem(GameObjectTreeViewItem item, int itemID,
                                Scene scene, bool isSceneHeader, int colorCode,
                                Object pptrObject, bool hasChildren,
                                int depth) {
    item.children = null;
    item.id = itemID;
    item.depth = depth;
    ...
    if (isSceneHeader)
      ...
      item.displayName =
          null; // For GameObject, name is empty as the name gets set from the
                // objectPPTR if the Item is used in GameObjectTreeViewGUI.
    ...
    if (hasChildren) {
      item.children =
          CreateChildListForCollapsedParent(); // add a dummy child in children
                                               // list to ensure we show the
                                               // collapse arrow (because we do
                                               // not fetch data for collapsed
                                               // items)
    }
  }
  ...
  override public bool IsRenamingItemAllowed(TreeViewItem item) {
    ...
    if (SubSceneGUI.IsUsingSubScenes() &&
        SubSceneGUI.IsSubSceneHeader((GameObject) goItem.objectPPTR))
      return false;
    ...
    if (SubSceneGUI.IsUsingSubScenes()) {
      for (int i = 0; i < numScenesInHierarchy; ++i) {
        ...
        if (subSceneInfo.isValid) {
          ...
          int depth =
              SubSceneGUI.CalculateHierarchyDepthOfSubScene(subSceneInfo);
          InitTreeViewItem(item, gameObject.GetInstanceID(), subSceneInfo.scene,
                           false, 0, gameObject, false, depth);
  ...
  public Scene GetLastScene() {
    ...
    for (int i = SceneManager.sceneCount - 1; i >= 0; i--)
      if (SceneManager.GetSceneAt(i).isLoaded &&
          !SceneManager.GetSceneAt(i).isSubScene)
