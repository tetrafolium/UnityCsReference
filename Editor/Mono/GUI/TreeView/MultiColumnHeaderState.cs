...
namespace UnityEditor.IMGUI.Controls {
  ...
  public class MultiColumnHeaderState {
    ...
    [Serializable]
    public class Column {
      // Only serialize the fields that can be changed by the user (the others
      // are reconstructed)
      [SerializeField]
    ...
    public static bool
    CanOverwriteSerializedFields(MultiColumnHeaderState source,
    ...
    OverwriteSerializedFields(MultiColumnHeaderState source,
      ...
      if (!CanOverwriteSerializedFields(source, destination)) {
        Debug.LogError(
            "MultiColumnHeaderState: Not able to overwrite serialized fields");
        return;
      }
      ...
      for (int i = 0; i < destination.m_Columns.Length; ++i) {
        ...
        destination.m_Columns[i].sortedAscending =
            source.m_Columns[i].sortedAscending;
      }
    }

    public MultiColumnHeaderState(Column[] columns) {
      if (columns == null)
        throw new ArgumentException("columns are no allowed to be null",
                                    "columns");
      if (columns.Length == 0)
    ...
    public int sortedColumnIndex {
      get { return m_SortedColumns.Count > 0 ? m_SortedColumns[0] : -1; }
      set {
    ...
    void RemoveInvalidSortingColumnsIndices() {
      ...
      if (m_SortedColumns.Count > m_MaxNumberOfSortedColumns)
        m_SortedColumns.RemoveRange(m_MaxNumberOfSortedColumns,
                                    m_SortedColumns.Count -
                                        m_MaxNumberOfSortedColumns);
    }
