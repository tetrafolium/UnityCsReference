...
internal class PackageExport : EditorWindow {
  ...
  public ExportPackageItem[] items {
    get { return m_ExportPackageItems; }
  }
  ...
  internal static class Styles {
    ...
    public static GUIContent includeDependenciesText =
        EditorGUIUtility.TrTextContent("Include dependencies");
    public static GUIContent header =
        EditorGUIUtility.TrTextContent("Items to Export");
  }
  ...
  internal static IEnumerable<ExportPackageItem>
  GetAssetItemsForExport(ICollection<string> guids, bool includeDependencies) {
    // if nothing is selected, export all
    if (0 == guids.Count) {
      string[] temp = new string[0]; // <--- I dont get this API
      guids = new HashSet<string>(AssetDatabase.CollectAllChildren(
          AssetDatabase.assetFolderGUID, temp));
    }

    ExportPackageItem[] assets =
        PackageUtility.BuildExportPackageItemsListWithPackageManagerWarning(
            guids.ToArray(), includeDependencies, true);

    // If any scripts are included, add all scripts with dependencies
    if (includeDependencies &&
        assets.Any(asset => UnityEditorInternal.InternalEditorUtility
                                .IsScriptOrAssembly(asset.assetPath))) {
      assets =
          PackageUtility.BuildExportPackageItemsListWithPackageManagerWarning(
              guids
                  .Union(UnityEditorInternal.InternalEditorUtility
                             .GetAllScriptGUIDs())
                  .ToArray(),
              includeDependencies, true);
    }

    // If the user exports the root Assets folder, we need to remove it from the
    // list explicitly, as it doesnt make sense
    assets = assets.Where(val => val.assetPath != "Assets").ToArray();

    return assets;
  }

  void RefreshAssetList() { m_ExportPackageItems = null; }

  bool HasValidAssetList() { return m_ExportPackageItems != null; }
  ...
  bool CheckAssetExportList() {
    if (m_ExportPackageItems.Length == 0) {
      ...
      GUILayout.Label("No assets to export were found in your project.",
                      "WordWrappedLabel");
  ...
  public void OnDestroy() { UnscheduleBuildAssetList(); }
  ...
  public void OnGUI() {
    ...
    using(new EditorGUI.DisabledScope(!HasValidAssetList())) { TopArea(); }

    TreeViewArea(!HasValidAssetList());

    using(new EditorGUI.DisabledScope(!HasValidAssetList())) { BottomArea(); }
  }
  ...
  void BottomArea() {
    ...
    m_IncludeDependencies =
        GUILayout.Toggle(m_IncludeDependencies, Styles.includeDependenciesText);
  ...
  private void Export() {
    string fileName = EditorUtility.SaveFilePanel("Export package ...", "", "",
                                                  "unitypackage");
    if (fileName != "") {
      // build guid list
  ...
  private void BuildAssetList() {
    ...
    m_ExportPackageItems =
        GetAssetItemsForExport(Selection.assetGUIDsDeepSelection,
                               m_IncludeDependencies)
            .ToArray();
