...
class SplitView : View, ICleanuppable, IDropArea {
  ...
  private Rect RectFromEdge(Rect rect, ViewEdge edge, float thickness,
                            float offset) {
    switch (edge) {
    case ViewEdge.Left:
      return new Rect(rect.x - offset, rect.y, thickness, rect.height);
    case ViewEdge.Right:
      return new Rect(rect.xMax - thickness + offset, rect.y, thickness,
                      rect.height);
    case ViewEdge.Top:
      return new Rect(rect.x, rect.y - offset, rect.width, thickness);
    case ViewEdge.Bottom:
      return new Rect(rect.x, rect.yMax - thickness + offset, rect.width,
                      thickness);
    default:
  ...
  void SetupSplitter() {
    ...
    for (int j = 0; j < children.Length; j++) {
      ...
      actualSizes[j] = GUIUtility.RoundToPixelGrid(vertical ? c.position.height
                                                   : c.position.width);
      minSizes[j] = GUIUtility.RoundToPixelGrid(vertical ? c.minSize.y
                                                : c.minSize.x);
    }
  ...
  void SetupRectsFromSplitter() {
    ...
    for (int i = 0; i < children.Length; i++)
      cursor += PlaceView(
          i, cursor,
          GUIUtility.RoundToPixelGrid(splitState.realSizes[i] * scale));
  ...
  internal override void Reflow() {
    ...
    splitState.RelativeToRealSizes(
        vertical ? GUIUtility.RoundToPixelGrid(position.height)
        : GUIUtility.RoundToPixelGrid(position.width));
  ...
  public void RemoveChildNice(View child) {
    if (children.Length != 1) {
      ...
      moveToPos = vertical ? Mathf
                      .Lerp(child.position.yMin, child.position.yMax, moveToPos)
  ...
  DropInfo RootViewDropZone(ViewEdge edge, Vector2 mousePos, Rect screenRect) {
    var offset = (edge & ViewEdge.FitsVertical) != 0 ? kRootDropZoneThickness
                                                     : kRootDropZoneOffset;
    if (!RectFromEdge(screenRect, edge, kRootDropZoneThickness, offset)
             .Contains(mousePos))
      return null;
    ...
    dropInfo.rect =
        RectFromEdge(screenRect, edge, kRootDropDestinationThickness, 0f);
    return dropInfo;
  }

  public DropInfo DragOverRootView(Vector2 mouseScreenPosition) {
    if (children.Length == 1 &&
        DockArea.s_IgnoreDockingForView == children[0]) {
      return null; // Prevent dragging the view from a single-view window into
                   // itself
    }
    return RootViewDropZone(ViewEdge.Bottom, mouseScreenPosition,
                            screenPosition)
        ...
        ?? RootViewDropZone(ViewEdge.Right, mouseScreenPosition,
                            screenPosition);
  }
  ...
  public DropInfo DragOver(EditorWindow w, Vector2 mouseScreenPosition) {
    for (var childIndex = 0; childIndex < children.Length; ++childIndex) {
      ...
      var childRectWithoutDock =
          RectFromEdge(childRect, ViewEdge.Bottom,
                       childRect.height - DockArea.kDockHeight, 0f);
      var borderWidth = Mathf.Min(Mathf.Round(childRectWithoutDock.width / 3),
                                  kMaxViewDropZoneThickness);
      var borderHeight = Mathf.Min(Mathf.Round(childRectWithoutDock.height / 3),
                                   kMaxViewDropZoneThickness);
      var leftDropZone =
          RectFromEdge(childRectWithoutDock, ViewEdge.Left, borderWidth, 0f);
      var rightDropZone =
          RectFromEdge(childRectWithoutDock, ViewEdge.Right, borderWidth, 0f);
      var bottomDropZone =
          RectFromEdge(childRectWithoutDock, ViewEdge.Bottom, borderHeight, 0f);
      var topDropZone =
          RectFromEdge(childRectWithoutDock, ViewEdge.Top, borderHeight, 0f);
      ...
      // If mouse is in more than one zone, it is in a corner. Find the corner
      ...
      case ViewEdge.BottomLeft:
        ...
        mouseToCorner =
            new Vector2(childRectWithoutDock.x, childRectWithoutDock.yMax) -
            mouseScreenPosition;
      ...
      case ViewEdge.BottomRight:
        ...
        mouseToCorner =
            new Vector2(childRectWithoutDock.xMax, childRectWithoutDock.yMax) -
            mouseScreenPosition;
        oppositeToCorner = new Vector2(borderWidth, borderHeight);
        break;
      case ViewEdge.TopLeft:
        ...
        cwEdge = ViewEdge.Top;
        mouseToCorner =
            new Vector2(childRectWithoutDock.x, childRectWithoutDock.y) -
            mouseScreenPosition;
        oppositeToCorner = new Vector2(-borderWidth, -borderHeight);
        break;
      case ViewEdge.TopRight:
        ...
        mouseToCorner =
            new Vector2(childRectWithoutDock.xMax, childRectWithoutDock.y) -
            mouseScreenPosition;
        oppositeToCorner = new Vector2(borderWidth, -borderHeight);
        break;
      }
      // ...then choose the edge based on the half the mouse is in
      mouseEdges = mouseToCorner.x * oppositeToCorner.y -
                               mouseToCorner.y * oppositeToCorner.x <
                           0
                       ? ccwEdge
                       : cwEdge;
      ...
      {
        var targetThickness = Mathf.Round(
            ((mouseEdges & ViewEdge.FitsHorizontal) != 0 ? childRect.width
                                                         : childRect.height) /
            3);
        targetThickness =
            Mathf.Max(targetThickness, kMinViewDropDestinationThickness);

        var dropInfo = new DropInfo(this);
        dropInfo.userData = new ExtraDropInfo(false, mouseEdges, childIndex);
        dropInfo.type = DropInfo.Type.Pane;
        dropInfo.rect =
            RectFromEdge(childRect, mouseEdges, targetThickness, 0f);
        return dropInfo;
      }
    }
    // Claim the drag if we are the root split view, so it doesn't fall through
    // to obscured windows
  ...
  public bool PerformDrop(EditorWindow dropWindow, DropInfo dropInfo,
                          Vector2 screenPos) {
    ...
    if (vertical == wantsVertical ||
        children.Length < 2) { // Current view can accommodate desired drop
      if (!beginning) {
        if (rootWindow)
      ...
      parentForDrop = this;
    } else if (rootWindow) { // Docking to a window: need to insert a parent
    ...
    } else { // Docking in a view: need to insert a child
      ...
      newChild.children[0].position =
          new Rect(Vector2.zero, newChild.position.size);
    ...
    DockArea.s_OriginalDragSource.RemoveTab(dropWindow, killIfEmpty
                                            : true, sendEvents
                                            : false);
  ...
  void CalcRoomForRect(Rect[] sources, Rect r) {
    ...
    for (splitPos = 0; splitPos < sources.Length; splitPos++) {
      float midPos =
          vertical ?(sources[splitPos].y + sources[splitPos].height * .5f)
  ...
  public void Cleanup() {
    ...
    if (children.Length == 1 && sp != null) {
      ...
      if (parent != null) {
        ...
        window.rootView = c;
        c.position =
            new Rect(0, 0, c.window.position.width, window.position.height);
        c.Reflow();
        if (!Unsupported.IsDestroyScriptableObject(this))
          DestroyImmediate(this);
        return;
    ...
    if (sp) {
      ...
      if (sp) {
        // If the parent has the same orientation as us, we can move our views
        // up and kill ourselves
        if (sp.vertical == vertical) {
          int idx = new List<View>(parent.children).IndexOf(this);
          foreach (View child in children) {
            sp.AddChild(child, idx++);
            child.position = new Rect(
                position.x + child.position.x, position.y + child.position.y,
    ...
    if (children.Length == 0) {
      if (parent == null && window != null) {
        ...
        if (parent is SplitView) {
          ...
          // We want to stay, but tell the parent (MainWindow) to Cleanup, so he
          // can reduce us to zero-size
          /*                  parent.RemoveChild (this);*/
        }
        ic.Cleanup();
      }
  ...
  public void SplitGUI(Event evt) {
    ...
    case EventType.MouseDown:
      ...
      {
        float cursor =
            vertical ? children[0].position.y : children[0].position.x;
        ...
        for (int i = 0; i < children.Length - 1; i++) {
          if (i >= splitState.realSizes.Length) {
            ...
            if (dock && dock.m_Selected < dock.m_Panes.Count &&
                dock.m_Panes[dock.m_Selected])
              name = dock.m_Panes [dock.m_Selected]
                         .GetType()
                         .ToString();

            if (Unsupported.IsDeveloperMode())
              Debug.LogError("Real sizes out of bounds for: " + name +
                             " index: " + i +
                             " RealSizes: " + splitState.realSizes.Length);

            SetupSplitter();
          }
          Rect splitterRect = vertical ? new Rect(
              children[0].position.x,
              cursor + splitState.realSizes[i] - splitState.splitSize / 2,
              children[0].position.width, splitState.splitSize)
              : new Rect(cursor + splitState.realSizes[i] -
                             splitState.splitSize / 2,
                         children[0].position.y, splitState.splitSize,
                         children[0].position.height);
    ...
    case EventType.MouseDrag:
      if (children.Length > 1 && (GUIUtility.hotControl == id) &&
          (splitState.currentActiveSplitter >= 0)) {
        float diff =
            GUIUtility.RoundToPixelGrid(pos) - splitState.splitterInitialOffset;
        if (Mathf.Abs(diff) > 0.01f) {
          splitState.splitterInitialOffset = GUIUtility.RoundToPixelGrid(pos);
          splitState.DoSplitter(splitState.currentActiveSplitter,
                                splitState.currentActiveSplitter + 1, diff);
        }
