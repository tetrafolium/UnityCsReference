...
namespace UnityEditor.AnimatedValues {
  public abstract class BaseAnimValueNonAlloc<T> : BaseAnimValue<T> where T
      : IEquatable<T> {
    ...
    protected override bool AreEqual(T a, T b) { return a.Equals(b); }
  }
  ...
  public abstract class BaseAnimValue<T> : ISerializationCallbackReceiver {
    ...
    // Don't have m_Animating survive script reloads, as it could cause the
    // AnimValue to get stuck. If m_Animating was true after reload but the
    // Update callback registration had been lost, The value would never change
    // and also never re-register to the Update callback.
    [NonSerialized] bool m_Animating;
    ...
    protected virtual bool AreEqual(T a, T b) { return a.Equals(b); }
    ...
    protected void StopAnim(T newValue) {
      // If the new value is different, or we might be in the middle of a fade,
      // we need to refresh. Checking GetValue is not reliable on its own, since
      // for e.g. bool it'll return the "closest" value, but that doesn't mean
      // the fade is done.
      bool invoke = (!AreEqual(newValue, GetValue()) || m_LerpPosition < 1) &&
                    valueChanged != null;
    ...
    internal void SkipFading() { StopAnim(target); }
    ...
    void ISerializationCallbackReceiver.OnAfterDeserialize() {
      // Ensure we resume animating after script reload if value differs from
      // target.
