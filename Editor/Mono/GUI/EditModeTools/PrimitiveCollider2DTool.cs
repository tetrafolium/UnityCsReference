...
abstract class PrimitiveCollider2DTool<T> : EditorTool where T : Collider2D {
  ...
  public override void OnToolGUI(EditorWindow window) {
    foreach (var obj in targets) {
      ...
      // collider matrix is 2d projection of center multiplied by transform's
      // matrix with custom postmultiplied lossy scale matrix only rotation of
      // transform about z-axis should be considered, if at all
      using(new Handles.DrawingScope(Matrix4x4.TRS(
          collider.transform.position, GetHandleRotation(), Vector3.one))) {
        Matrix4x4 colliderTransformMatrix =
            collider.transform.localToWorldMatrix;

        boundsHandle.center =
            ProjectOntoWorldPlane(Handles.inverseMatrix *
                                  (colliderTransformMatrix * collider.offset));
        ...
        if (EditorGUI.EndChangeCheck()) {
          ...
          if (CopyHandleSizeToCollider(collider))
            collider.offset =
                colliderTransformMatrix.inverse *
  ...
  // return specified world vector projected onto collider's x/y plane in world
  // space
  protected Vector3
  ProjectOntoColliderPlane(Vector3 worldVector,
                           Matrix4x4 colliderTransformMatrix) {
    Plane worldColliderPlane =
        new Plane(Vector3.Cross(colliderTransformMatrix * Vector3.right,
                                colliderTransformMatrix * Vector3.up),
                  Vector3.zero);
