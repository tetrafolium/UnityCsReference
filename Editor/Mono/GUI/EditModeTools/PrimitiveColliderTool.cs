...
abstract class PrimitiveColliderTool<T> : EditorTool where T : Collider {
  ...
  public override void OnToolGUI(EditorWindow window) {
    foreach (var obj in targets) {
      ...
      // collider matrix is center multiplied by transform's matrix with custom
      // postmultiplied lossy scale matrix
      using(new Handles.DrawingScope(Matrix4x4.TRS(collider.transform.position,
                                                   collider.transform.rotation,
                                                   Vector3.one))) {
        CopyColliderPropertiesToHandle(collider);

        boundsHandle.SetColor(collider.enabled ? Handles.s_ColliderHandleColor
  ...
  TransformColliderCenterToHandleSpace(Transform colliderTransform,
                                       Vector3 colliderCenter) {
    return Handles.inverseMatrix *
           (colliderTransform.localToWorldMatrix * colliderCenter);
  }

  protected static Vector3
  TransformHandleCenterToColliderSpace(Transform colliderTransform,
                                       Vector3 handleCenter) {
    return colliderTransform.localToWorldMatrix.inverse *
           (Handles.matrix * handleCenter);
  }
}
}
