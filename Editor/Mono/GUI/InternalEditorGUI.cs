...
public sealed partial class EditorGUI {
  ...
  // We need an EditorGUI.Button that only reacts to left mouse button
  // (GUI.Button reacts to all mouse buttons), so we can handle context click
  // events for button areas etc.
  internal static bool Button(Rect position, GUIContent content,
                              GUIStyle style) {
    ...
    case EventType.MouseUp:
      if (evt.button != 0)
        return false; // ignore all input from other buttons than the left mouse
                      // button
      break;
    }
  ...
  // and dragging the mouse cursor around to select different icons
  internal static bool IconButton(int id, Rect position, GUIContent content,
                                  GUIStyle style) {
  ...
  internal static float WidthResizer(Rect position, float width, float minWidth,
                                     float maxWidth) {
    bool hasControl;
  ...
  internal static float WidthResizer(Rect position, float width, float minWidth,
                                     float maxWidth, out bool hasControl) {
    return Resizer.Resize(position, width, minWidth, maxWidth, true,
                          out hasControl);
  }

  internal static float HeightResizer(Rect position, float height,
  ...
  static class Resizer {
    ...
    internal static float Resize(Rect position, float size, float minSize,
                                 float maxSize, bool horizontal,
                                 out bool hasControl) {
      int id = EditorGUIUtility.GetControlID(s_MouseDeltaReaderHash,
                                             FocusType.Passive, position);
      Event evt = Event.current;
      switch (evt.GetTypeForControl(id)) {
      case EventType.MouseDown:
        if (GUIUtility.hotControl == 0 &&
            position.Contains(evt.mousePosition) && evt.button == 0) {
          GUIUtility.hotControl = id;
          GUIUtility.keyboardControl = 0;
          s_MouseDeltaReaderStartPos = GUIClip.Unclip(
              evt.mousePosition); // We unclip to screenspace to prevent being
                                  // affected by scrollviews
          s_StartSize = size;
          evt.Use();
        }
        break;
      case EventType.MouseDrag:
        if (GUIUtility.hotControl == id) {
          evt.Use();
          Vector2 screenPos = GUIClip.Unclip(
              evt.mousePosition); // We unclip to screenspace to prevent being
                                  // affected by scrollviews
          float delta = horizontal ?(screenPos - s_MouseDeltaReaderStartPos)
                            .x : (screenPos - s_MouseDeltaReaderStartPos)
                            .y;
      ...
      case EventType.MouseUp:
        ...
        break;
      case EventType.Repaint:
        var cursor = horizontal ? MouseCursor.SplitResizeLeftRight
  ...
  internal static Vector2 MouseDeltaReader(Rect position, bool activated) {
    ...
    switch (evt.GetTypeForControl(id)) {
    case EventType.MouseDown:
      if (activated && GUIUtility.hotControl == 0 &&
          GUIUtility.HitTest(position, evt) && evt.button == 0) {
        GUIUtility.hotControl = id;
        GUIUtility.keyboardControl = 0;
        s_MouseDeltaReaderLastPos = GUIClip.Unclip(
            evt.mousePosition); // We unclip to screenspace to prevent being
                                // affected by scrollviews
        evt.Use();
      }
  ...
  // Shows an active button and a triangle button on the right, which expands
  // the dropdown list Returns true if button was activated, returns false if
  // the the dropdown button was activated or the button was not clicked.
  internal static bool
  ButtonWithDropdownList(string buttonName, string[] buttonNames,
  ...
  // the the dropdown button was activated or the button was not clicked.
  internal static bool
  ButtonWithDropdownList(GUIContent content, string[] buttonNames,
                         GenericMenu.MenuFunction2 callback,
                         params GUILayoutOption[] options) {
    var rect =
  ...
  internal static void GameViewSizePopup(Rect buttonRect,
                                         GameViewSizeGroupType groupType,
                                         int selectedIndex,
                                         IGameViewSizeMenuUser gameView,
                                         GUIStyle guiStyle) {
    ...
    if (EditorGUI.DropdownButton(buttonRect, GUIContent.Temp(text),
                                 FocusType.Passive, guiStyle)) {
      var menuData = new GameViewSizesMenuItemProvider(groupType);
      var flexibleMenu =
          new GameViewSizeMenu(menuData, selectedIndex,
                               new GameViewSizesMenuModifyItemUI(), gameView);
  ...
  internal static void DrawOutline(Rect rect, float size, Color color) {
    ...
    Color orgColor = GUI.color;
    GUI.color = GUI.color * color;
    GUI.DrawTexture(new Rect(rect.x, rect.y, rect.width, size),
                    EditorGUIUtility.whiteTexture);
    GUI.DrawTexture(new Rect(rect.x, rect.yMax - size, rect.width, size),
                    EditorGUIUtility.whiteTexture);
    GUI.DrawTexture(new Rect(rect.x, rect.y + 1, size, rect.height - 2 * size),
                    EditorGUIUtility.whiteTexture);
    GUI.DrawTexture(
...
internal struct PropertyGUIData {
  ...
  public Color color;
  public PropertyGUIData(SerializedProperty property, Rect totalPosition,
                         bool wasBoldDefaultFont, bool wasEnabled,
                         Color color) {
