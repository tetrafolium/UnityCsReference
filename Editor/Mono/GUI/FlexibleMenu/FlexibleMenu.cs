...
internal class FlexibleMenu : PopupWindowContent {
  class Styles {
    ...
    public GUIContent plusButtonText =
        EditorGUIUtility.TrTextContent("", "Add New Item");
  }
  ...
  int maxIndex {
    get {
      return m_ShowAddNewPresetItem ? m_ItemProvider.Count()
          : m_ItemProvider.Count() -
             1;
    }
  }
  ...
  protected float minTextWidth {
    get { return m_MinTextWidth; }
    set {
      m_MinTextWidth = value;
      ClearCachedWidth();
    }
  }

  // itemClickedCallback arguments is clicked index, clicked item object
  public FlexibleMenu(IFlexibleMenuItemProvider itemProvider,
                      int selectionIndex, FlexibleMenuModifyItemUI modifyItemUi,
  ...
  public override Vector2 GetWindowSize() { return CalcSize(); }

  bool IsDeleteModiferPressed() { return Event.current.alt; }

  bool AllowDeleteClick(int index) {
    return IsDeleteModiferPressed() &&
           m_ItemProvider.IsModificationAllowed(index) &&
    ...
    m_ScrollPosition = GUI.BeginScrollView(rect, m_ScrollPosition, contentRect);
    {
      ...
      for (int i = 0; i <= maxIndex; ++i) {
        ...
        case EventType.Repaint:
          ...
          s_Styles.menuItem.Draw(itemRect,
                                 GUIContent.Temp(m_ItemProvider.GetName(i)),
                                 hover, false, i == selectedIndex, false);

          if (addSeperator) {
            const float margin = 4f;
            Rect seperatorRect =
                new Rect(itemRect.x + margin,
                         itemRect.y + itemRect.height + seperatorHeight * 0.5f,
                         itemRect.width - 2 * margin, 1);
            DrawRect(seperatorRect,
        ...
        case EventType.ContextClick:
          if (itemRect.Contains(evt.mousePosition)) {
            ...
            if (m_ModifyItemUI != null &&
                m_ItemProvider.IsModificationAllowed(i))
  ...
  protected Vector2 CalcSize() {
    float height = (maxIndex + 1) * lineHeight +
                   m_SeperatorIndices.Length * seperatorHeight;
    if (m_CachedWidth < 0)
      m_CachedWidth = Math.Max(m_MinTextWidth, CalcWidth());
    return new Vector2(m_CachedWidth, height);
  }

  void ClearCachedWidth() { m_CachedWidth = -1f; }
  ...
  float CalcWidth() {
    ...
    for (int i = 0; i < m_ItemProvider.Count(); ++i) {
      float w =
          s_Styles.menuItem.CalcSize(GUIContent.Temp(m_ItemProvider.GetName(i)))
              .x;
  ...
  void CreateNewItemButton(Rect itemRect) {
    ...
    if (GUI.Button(plusRect, s_Styles.plusButtonText, "OL Plus")) {
      plusRect.y -= 15f;
      m_ModifyItemUI.Init(
          FlexibleMenuModifyItemUI.MenuType.Add, m_ItemProvider.Create(),
          delegate(object obj) {
            ClearCachedWidth();
            int newIndex = m_ItemProvider.Add(obj);
            SelectItem(newIndex);
            EditorApplication
                .RequestRepaintAllViews(); // We want to repaint the flexible
                                           // menu (currently in modifyItemUI)
          });
  ...
  void EditExistingItem(Rect itemRect, int index) {
    ...
    m_ModifyItemUI.Init(
        FlexibleMenuModifyItemUI.MenuType.Edit, m_ItemProvider.GetItem(index),
        delegate(object obj) {
          ClearCachedWidth();
          m_ItemProvider.Replace(index, obj);
          EditorApplication
              .RequestRepaintAllViews(); // We want to repaint the flexible menu
                                         // (currently in modifyItemUI)
        });
  ...
  void DeleteItem(int index) {
    ...
    selectedIndex = Mathf.Clamp(selectedIndex, 0,
                                m_ItemProvider.Count() - 1); // ensure valid
  }
  ...
  internal static class ItemContextMenu {
    ...
    static public void Show(int itemIndex, FlexibleMenu caller) {
      ...
      menu.AddItem(EditorGUIUtility.TrTextContent("Edit..."), false, Edit,
                   itemIndex);
