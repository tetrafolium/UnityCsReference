...
namespace UnityEditor.Modules {
  ...
  internal abstract class DefaultBuildPostprocessor : IBuildPostprocessor {
    ...
    public virtual void PostProcess(BuildPostProcessArgs args,
      ...
      // NOTE: For some reason, calling PostProcess seems like it can trigger
      // this object to be GC'd
      //  so create is just before returning
      outProperties = ScriptableObject.CreateInstance<DefaultBuildProperties>();
    }

    public virtual bool SupportsInstallInBuildFolder() { return false; }

    public virtual bool SupportsLz4Compression() { return false; }

    public virtual Compression GetDefaultCompression() {
      return Compression.None;
    }

    public virtual bool SupportsScriptsOnlyBuild() { return true; }

    public virtual string PrepareForBuild(BuildOptions options,
                                          BuildTarget target) {
      return null;
    }

    public virtual void UpdateBootConfig(BuildTarget target,
      ...
      if (String.IsNullOrEmpty(checkVREnabled) ||
          String.Compare("0", checkVREnabled, true) == 0) {
        bool isVrEnabled =
        ...
        if (isVrEnabled) {
          string[] vrDevices =
              UnityEditorInternal.VR.VREditor.GetVREnabledDevicesOnTarget(
                  target);
      ...
      config.Set("hdr-display-enabled", PlayerSettings.useHDRDisplay ? "1"
                 : "0");
    ...
    public virtual string GetExtension(BuildTarget target,
                                       BuildOptions options) {
      return string.Empty;
    }
  }
