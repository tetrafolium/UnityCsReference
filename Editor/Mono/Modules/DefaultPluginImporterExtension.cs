...
internal Property(string name, string key, object defaultValue,
                  string platformName)
	: this(new GUIContent(name), key, defaultValue, platformName) {
}

internal Property(GUIContent name, string key, object defaultValue,
                  string platformName) {
	this.name = name;
	this.key = key;
...
internal virtual void Reset(PluginImporterInspector inspector) {
	string valueString =
		inspector.importer.GetPlatformData(platformName, key);
	ParseStringValue(inspector, valueString);
}

protected void ParseStringValue(PluginImporterInspector inspector,
                                string valueString,
                                bool muteWarnings = false) {
	try {
		value =
			TypeDescriptor.GetConverter(type).ConvertFromString(valueString);
	} catch {
		value = defaultValue;

		if (!muteWarnings && !string.IsNullOrEmpty(valueString)) {
			// We mute warnings for properties that are on disabled platforms to
			// avoid unnecessary spam for deprecated values, case 909247
			if (inspector.importer.GetCompatibleWithPlatform(platformName))
				Debug.LogWarning(
					"Failed to parse value ('" + valueString + "') for " + key +
					", platform: " + platformName + ", type: " + type +
					". Default value will be set '" + defaultValue + "'");
		}
	}
}

internal virtual void Apply(PluginImporterInspector inspector) {
	string valueString = type == typeof(bool) ? value.ToString().ToLower()
	                                          : value.ToString();
...
RefreshProperties(PluginImporterInspector inspector) {
	foreach (var p in properties) {
		p.Reset(inspector);
	}
...
public virtual string CalculateFinalPluginPath(string platformName,
                                               PluginImporter imp) {
	string cpu = imp.GetPlatformData(platformName, "CPU");

	if (!string.IsNullOrEmpty(cpu) &&
	    (string.Compare(cpu, "AnyCPU", true) != 0) &&
	    (string.Compare(cpu, "None", true) != 0))
		return Path.Combine(cpu, Path.GetFileName(imp.assetPath));

	return Path.GetFileName(imp.assetPath);
}

protected Dictionary<string, List<PluginImporter> >
GetCompatiblePlugins(string buildTargetName) {
	...
	IEnumerable<PluginImporter> plugins =
		PluginImporter.GetAllImporters().Where(
			imp => imp.GetCompatibleWithPlatformOrAnyPlatformBuildTarget(
				buildTargetName) &&
			imp.IsCompatibleWithDefines(
				assemblyDefines.ContainsKey(imp.name)
			     ? assemblyDefines[imp.name]
			     : null));
	Dictionary<string, List<PluginImporter> > matchingPlugins =
		new Dictionary<string, List<PluginImporter> >();

	foreach (var plugin in plugins) {
		string finalPluginPath =
			CalculateFinalPluginPath(buildTargetName, plugin);
...
public virtual bool CheckFileCollisions(string buildTargetName) {
	Dictionary<string, List<PluginImporter> > matchingPlugins =
		GetCompatiblePlugins(buildTargetName);

	bool foundCollisions = false;

	StringBuilder errorMessage = new StringBuilder();
	foreach (KeyValuePair<string, List<PluginImporter> > pair in
	         matchingPlugins) {
		List<PluginImporter> plugins = pair.Value;
		// If we have only one plugin with specified final path, that means
		// everything is ok, and no overwrite will occur
		if (plugins.Count == 1)
			continue;
		...
		// If we have a single user project plugin, it will take precedence and
		// overwrite the others. Anything else should throw errors if there are
		// multiple.
		if (projectPluginCount == 1)
			continue;

		foundCollisions = true;
		// Two or more plugins are being copied to the same path, create an
		// error message
		errorMessage.AppendLine(
			string.Format("Plugin '{0}' is used from several locations:",
			              Path.GetFileName(pair.Key)));
		foreach (PluginImporter importer in plugins) {
			errorMessage.AppendLine(" " + importer.assetPath +
			                        " would be copied to <PluginPath>/" +
			                        pair.Key.Replace("\\", "/"));
		}
	}
	if (foundCollisions) {
