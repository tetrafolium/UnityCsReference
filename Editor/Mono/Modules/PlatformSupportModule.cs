...
/// Returns name identifying a target, for ex., Metro, note this name should
/// match prefix for extension module UnityEditor.Metro.Extensions.dll,
/// UnityEditor.Metro.Extensions.Native.dll
string TargetName {
	get;
}

/// Returns the filename of jam which should be executed when you're
/// recompiling extensions from Editor using CTRL + L shortcut, for ex.,
/// WP8EditorExtensions, MetroEditorExtensions, etc
string JamTarget {
	get;
}

/// Returns an array of native libraries that are required by the extension
/// and must be loaded by the editor.
///
/// NOTE: If two different platform extensions return a native library with
/// a same file name (regardless of the path), then only first one will be
/// loaded. This is due to the fact that some platforms may require same
/// native library, but we must ship a copy with both platforms, since our
/// modularization and platform installers don't support shared stuff.
string[] NativeLibraries {
	get;
}

/// Returns an array of assemblies that should be referenced by user's
/// scripts. These will be referenced by editor scripts, and game scripts
/// running in editor. Used to export additional platform specific editor
/// API.
string[] AssemblyReferencesForUserScripts {
	get;
}

// Returns an array of assemblies that should be included into C# project as
// references. This is different from AssemblyReferencesForUserScripts by
// that most assembly references are internal and not added to the C#
// project. On the other hand, certain assemblies contain public API, and
// thus should be added to C# project.
string[] AssemblyReferencesForEditorCsharpProject {
	get;
}

/// A human friendly version (eg. an incrementing number on each release) of
/// the platform extension. Null/Empty if none available
string ExtensionVersion {
	get;
}

// Names of displays to show in GameView and Camera inspector if the
// platform supports multiple displays. Return null if default names should
// be used.
GUIContent[] GetDisplayNames();

IBuildPostprocessor CreateBuildPostprocessor();

// Returns an instance of IDeploymentTargetsExtension or null if not
// supported
IDeploymentTargetsExtension CreateDeploymentTargetsExtension();

// Returns an instance of IScriptingImplementations or null if only one
// scripting backend is supported
...
// For ex., Metro specifc UnityEngine.Networking.dll which is different from
// the generic UnityEngine.Networking.dll
void RegisterAdditionalUnityExtensions();

// return valid object for this device. This ensures that API for certain
// operations is still available even if device was removed, for example
// stopping remote support.
IDevice CreateDevice(string id);

// Called when build target supplied by this module is activated in the
// editor.
//
// NOTE: Keep in mind that due domain reloads and the way unity builds,
// calls on OnActive
//     and OnDeactivate will be forced even if current build target isn't
//     being changed.
//
// PERFORMANCE: This method will be called each time user starts the game,
// so use this
...
//     * Scripts are recompiled and reloaded (due user's change, forced
//     reimport, etc).
//     * User clicks play in editor.
//     * ... and possibly more I'm not aware of.
void OnActivate();

// Called when build target supplied by this module is deactivated in the
// editor.
//
// NOTE: Keep in mind that due domain reloads and the way unity builds,
// calls on OnActive
...
//
// PERFORMANCE: This will be called for all available platform extensions
// during each
//     domain reload, including each time user starts the game, so use this
//     only for lightweight code.
void OnLoad();

// Called when extension is unloaded, when editor is exited or before domain
// reload.
//
// PERFORMANCE: This will be called for all available platform extensions
// during each
...
void PostProcess(BuildPostProcessArgs args,
                 out BuildProperties outProperties);
...
// This is the place to make sure platform has everything it needs for the
// build. Use EditorUtility.Display(Cancelable)ProgressBar when running long
// tasks (e.g. downloading SDK from internet). Return non-empty string
// indicating error message to stop the build.
string PrepareForBuild(BuildOptions options, BuildTarget target);

void UpdateBootConfig(BuildTarget target, BootConfigData config,
...
// If you are creating a new extension you should probably inherit from
// DefaultPlayerSettingsEditorExtension
...
// True is this extension contributes an external application/tool
// preference(s)
bool HasExternalApplications();

// Called from OnGui - this function should draw any contributing UI
// components
...
void GetBuildButtonTitles(out GUIContent buildButtonTitle,
                          out GUIContent buildAndRunButtonTitle);
...
// Enables a dialog "Wait For Managed debugger", which halts program
// execution until managed debugger is connected
...
// Start player connection support for this device. This only sets up
// ability to connect, like setting up TCP tunneling over USB, getting
// remote device's IP, but it doesn't actually make the connection. Only
// available if SupportsPlayerConnection is true. Otherwise throws
// NotSupportedException.
RemoteAddress StartPlayerConnectionSupport();

// Stop player connection support for this device. Only available if
// SupportsPlayerConnection is true. Otherwise throws NotSupportedException.
...
// Called before building the player, checks if plugins don't overwrite each
// other
...
// For performance reason, and to keep the Editor responsive, the validation
...
// Invoked by the Editor after the user assemblies have been rebuilt, to
// allow the validator to validate the generated assemblies.
//
// This method is invoked only in case rebuilding the scripts was
// successful. The method might be run in a separate thread in case
// canRunInBackground was true.
void Validate(string[] userAssemblies);

// Invoked by the Editor in case the validation has just been killed,
// usually due to script rebuilds being triggered when the Validate method
...
// Returns an array of windows metadata files (.winmd) that should be
// referenced when compiling scripts. Only WinRT based platforms need these
// references.
IEnumerable<string> GetWindowsMetadataReferences();

// Returns an array of managed assemblies that should be referenced when
// compiling scripts Currently, only .NET scripting backend uses it to
// include WinRTLegacy.dll into compilation
...
// Returns an array of C# source files that should be included into the
// assembly when compiling scripts
