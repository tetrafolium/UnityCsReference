...
namespace UnityEditor.Modules {
  internal static class ModuleManager {
    ...
    private const string k_IsCacheBuildKey =
        "ModuleManagerIsExtensionsRegistered";
    ...
    internal static Dictionary<string, IPlatformSupportModule>
        platformSupportModules {
      get {
    ...
    // Accesses the list of currently known platformSupportModules without
    // forcing platform support module registration. Use this API from all code
    // which may run during the initial domain reloading phase instead of
    // accessing platformSupportModules. Platform support modules will be
    // registered automatically when it is safe to do so.
    internal static Dictionary<string, IPlatformSupportModule>
        platformSupportModulesDontRegister {
      get {
        if (s_PlatformModules == null)
    ...
    static void OnActiveBuildTargetChanged(BuildTarget oldTarget,
                                           BuildTarget newTarget) {
    ...
    IsPlatformSupportLoadedByBuildTarget(BuildTarget target) {
    ...
    [NonSerialized]
    static ProfilerMarker s_InitializeModuleManagerMarker =
        new ProfilerMarker("ModuleManager.InitializeModuleManager");
    ...
    // entry point from native
    // Note that in order for this function to work properly, it must be called
    // between two domain reloads. The first domain reload is needed because
    // RegisterPlatformSupportModules() investigates the currently loaded set of
    // assemblies. The second reload is needed so that assemblies returned by
    // module.AssemblyReferencesForUserScripts are actually loaded in the
    ...
    internal static void InitializePlatformSupportModules() {
      ...
      foreach (var module in platformSupportModules.Values) {
        ...
        EditorUtility.LoadPlatformSupportModuleNativeDllInternal(
            module.TargetName);

        module.OnLoad();
      }

      // Setup active build target and call OnActivate() for current platform
      // module
      OnActiveBuildTargetChanged(BuildTarget.NoTarget,
    ...
    class IvyPackageFileData {
      ...
      public bool IsDll =>
          type.Equals("dll", StringComparison.InvariantCultureIgnoreCase);
      ...
      public static IvyPackageFileData CreateFromRegexMatch(Match match) {
        ...
        return new IvyPackageFileData(){filename = filename,
    ...
    static void LoadLegacyExtensionsFromIvyFiles() {
      // We can't use the cached native type scanner here since this is called
      // to early for that to be built up.

      HashSet<string> ivyFiles;
      try {
        ivyFiles = new HashSet<string>();

        string unityExtensionsFolder = FileUtil.CombinePaths(
            Directory.GetParent(EditorApplication.applicationPath).ToString(),
            "Data", "UnityExtensions");
        string playbackEngineFolders = FileUtil.CombinePaths(
            Directory.GetParent(EditorApplication.applicationPath).ToString(),
            "PlaybackEngines");

        foreach (var searchPath in new[]{
                     FileUtil.NiceWinPath(
          ...
          ivyFiles.UnionWith(Directory.GetFiles(searchPath, "ivy.xml",
                                                SearchOption.AllDirectories));
        }
      } catch (Exception ex) {
        Console.WriteLine("Error scanning for extension ivy.xml files: {0}",
                          ex);
        return;
      }
      ...
      var artifactRegex = new Regex(
          @"<artifact(\s+(name=" "(?<name>[^" "]*)" "|type=" "(?<type>[^" "]*)" "|ext=" "(?<ext>[^" "]*)" "|e:guid=" "(?<guid>[^" "]*)" "))+\s*/>",
      ...
      foreach (var ivyFile in ivyFiles) {
        try {
          ...
          var artifacts = artifactRegex.Matches(ivyFileContent)
                              .Cast<Match>()
                              .Select(IvyPackageFileData.CreateFromRegexMatch)
                              .ToList();
          ...
          Console.WriteLine(
              "Error reading extensions from ivy.xml file at {0}: {1}", ivyFile,
              ex);
        }
      }
      ...
      try {
        foreach (var packageInfo in packages) {
          ...
          foreach (var packageInfoFile in files) {
            string fullPath = Paths.NormalizePath(
                Path.Combine(packageInfo.Key, packageInfoFile.filename));

            if (!File.Exists(fullPath))
              Debug.LogWarningFormat(
                  "Missing assembly \t{0} listed in ivy file {1}. Extension support may be incomplete.",
                  fullPath, packageInfo.Key);

            if (!packageInfoFile.IsDll)
              continue;

            if (!string.IsNullOrEmpty(packageInfoFile.guid)) {
              InternalEditorUtility.RegisterExtensionDll(
                  fullPath.Replace('\\', '/'), packageInfoFile.guid);
            } else {
              InternalEditorUtility.RegisterPrecompiledAssembly(fullPath,
                                                                fullPath);
            }
          }
        }
    ...
    static bool TryParseBuildTarget(string targetString,
      ...
      try {
        ...
        Debug.LogWarning(
            string.Format("Couldn't find build target for {0}", targetString));
      }
      return false;
    }

    private static void RegisterPlatformSupportModules() {
      var allTypesWithInterface =
          TypeCache.GetTypesDerivedFrom<IPlatformSupportModule>();
      s_PlatformModules = new Dictionary<string, IPlatformSupportModule>(
          allTypesWithInterface.Count);
      ...
      foreach (var type in allTypesWithInterface) {
        ...
        var platformSupportModule =
            Activator.CreateInstance(type) as IPlatformSupportModule;
        s_PlatformModules.Add(platformSupportModule.TargetName,
    ...
    internal static IPlatformSupportModule
    FindPlatformSupportModule(string moduleName) {
      foreach (var module in platformSupportModules.Values)
    ...
    internal static IDevice GetDevice(string deviceId) {
      ...
      if (DevDeviceList.FindDevice(deviceId, out device)) {
        IPlatformSupportModule module =
            FindPlatformSupportModule(device.module);
        if (module != null)
          return module.CreateDevice(deviceId);
        else
          throw new ApplicationException("Couldn't find module for target: " +
                                         device.module);
      }

      throw new ApplicationException("Couldn't create device API for device: " +
                                     deviceId);
    }

    internal static IUserAssembliesValidator
    GetUserAssembliesValidator(string target) {
      if (target == null)
        return null;

      IPlatformSupportModule module;
      if (platformSupportModules.TryGetValue(target, out module)) {
        return platformSupportModules [target]
            .CreateUserAssembliesValidatorExtension();
      }
      return null;
    }
    ...
    internal static IBuildPostprocessor GetBuildPostProcessor(string target) {
      ...
      if (platformSupportModules.TryGetValue(target, out module)) {
        return platformSupportModules [target]
            .CreateBuildPostprocessor();
      }

      return null;
    }

    internal static IBuildPostprocessor
    GetBuildPostProcessor(BuildTargetGroup targetGroup, BuildTarget target) {
      return GetBuildPostProcessor(GetTargetStringFrom(targetGroup, target));
    }

    internal static IDeploymentTargetsExtension
    GetDeploymentTargetsExtension(string target) {
      if (target == null)
        return null;
      ...
      if (platformSupportModules.TryGetValue(target, out module)) {
        return platformSupportModules [target]
            .CreateDeploymentTargetsExtension();
      }
    ...
    internal static IDeploymentTargetsExtension
    GetDeploymentTargetsExtension(BuildTargetGroup targetGroup,
                                  BuildTarget target) {
      return GetDeploymentTargetsExtension(
          GetTargetStringFrom(targetGroup, target));
    }
    ...
    internal static ISettingEditorExtension
    GetEditorSettingsExtension(string target) {
    ...
    internal static ITextureImportSettingsExtension
    GetTextureImportSettingsExtension(BuildTarget target) {
      return GetTextureImportSettingsExtension(
          GetTargetStringFromBuildTarget(target));
    }

    internal static ITextureImportSettingsExtension
    GetTextureImportSettingsExtension(string targetName) {
      IPlatformSupportModule module;
      if (platformSupportModules.TryGetValue(targetName, out module)) {
        return platformSupportModules [targetName]
    ...
    internal static List<IPreferenceWindowExtension>
    GetPreferenceWindowExtensions() {
      List<IPreferenceWindowExtension> prefWindExtensions =
          new List<IPreferenceWindowExtension>();

      foreach (var module in platformSupportModules.Values) {
        IPreferenceWindowExtension prefWindowExtension =
            module.CreatePreferenceWindowExtension();
    ...
    GetBuildWindowExtension(string target) {
      if (string.IsNullOrEmpty(target))
        return null;
      ...
      if (platformSupportModules.TryGetValue(target, out module)) {
        return platformSupportModules [target]
            .CreateBuildWindowExtension();
      }

      return null;
    }

    internal static ICompilationExtension
    GetCompilationExtension(string target) {
      IPlatformSupportModule module;
      if (platformSupportModules.TryGetValue(target, out module)) {
        return platformSupportModules [target]
    ...
    private static IScriptingImplementations
    GetScriptingImplementations(string target) {
      if (string.IsNullOrEmpty(target))
        return null;
      ...
      if (platformSupportModules.TryGetValue(target, out module)) {
        return platformSupportModules [target]
            .CreateScriptingImplementations();
      }

      return null;
    }

    internal static IScriptingImplementations
    GetScriptingImplementations(BuildTargetGroup target) {
      // Standalone Windows, Linux and OS X share player settings between each
      // other, so they share scripting implementations too However, since we
      // can't pin BuildTargetGroup to any single platform support module, we
      // have to explicitly check for this case
      if (target == BuildTargetGroup.Standalone)
        return new DesktopStandalonePostProcessor.ScriptingImplementations();

      return GetScriptingImplementations(
          GetTargetStringFromBuildTargetGroup(target));
    }

    internal static IPluginImporterExtension
    GetPluginImporterExtension(string target) {
      if (target == null)
        return null;
    ...
    internal static IPluginImporterExtension
    GetPluginImporterExtension(BuildTarget target) {
      return GetPluginImporterExtension(GetTargetStringFromBuildTarget(target));
    }

    internal static IPluginImporterExtension
    GetPluginImporterExtension(BuildTargetGroup target) {
      return GetPluginImporterExtension(
          GetTargetStringFromBuildTargetGroup(target));
    }
    ...
    GetTargetStringFromBuildTargetGroup(BuildTargetGroup target) {
      return BuildTargetDiscovery.GetModuleNameForBuildTargetGroup(target);
    }

    // This function returns module name depending on the combination of
    // targetGroup x target
    internal static string GetTargetStringFrom(BuildTargetGroup targetGroup,
                                               BuildTarget target) {
    ...
    internal static bool ShouldShowMultiDisplayOption() {
      GUIContent[] platformDisplayNames = Modules.ModuleManager.GetDisplayNames(
          EditorUserBuildSettings.activeBuildTarget.ToString());
      BuildTargetGroup curPlatform = BuildPipeline.GetBuildTargetGroup(
          EditorUserBuildSettings.activeBuildTarget);
