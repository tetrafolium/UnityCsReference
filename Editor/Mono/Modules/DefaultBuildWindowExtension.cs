...
public GUIContent buildScriptsOnly = EditorGUIUtility.TrTextContent(
	"Scripts Only Build",
	"Scripts Only Build re-compiles only scripts in the current Project, it skips processing other assets. When you Build, it will produce a new Player build, based on a previous successful build.");
}
...
public virtual void
GetBuildButtonTitles(out GUIContent buildButtonTitle,
...
public virtual bool ShouldDrawWaitForManagedDebugger() {
	return false;
}
public virtual bool ShouldDisableManagedDebuggerCheckboxes() {
	return false;
}

public virtual void DoScriptsOnlyGUI() {
	EditorUserBuildSettings.buildScriptsOnly = EditorGUILayout.Toggle(
		styles.buildScriptsOnly, EditorUserBuildSettings.buildScriptsOnly);
}
}
}
