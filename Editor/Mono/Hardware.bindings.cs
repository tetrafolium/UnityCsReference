...
public struct UsbDevice {
	...
	public override string ToString() {
		return name + " (udid:" + udid + ", vid: " + vendorId.ToString("X4") +
		       ", pid: " + productId.ToString("X4") +
		       ", rev: " + revision.ToString("X4") + ")";
	}
}
...
public partial struct DevDevice {
	...
	public bool isConnected {
		get { return state == DevDeviceState.Connected; }
	}

	public static DevDevice none {
		get {
			return new DevDevice("None", "None", "none", "internal",
			                     DevDeviceState.Disconnected,
			                     DevDeviceFeatures.None);
		}
	}

	public override string ToString() {
		return name + " (id:" + id + ", type: " + type + ", module: " + module +
		       ", state: " + state + ", features: " + features + ")";
	}

	public DevDevice(string id, string name, string type, string module,
	                 DevDeviceState state, DevDeviceFeatures features) {
		this.id = id;
		this.name = name;
		this.type = type;
		this.module = module;
...
internal static extern void UpdateInternal(string target,
                                           DevDevice[] devices);
}
}
