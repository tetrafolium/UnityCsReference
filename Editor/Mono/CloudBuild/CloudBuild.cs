...
namespace UnityEditor.CloudBuild {
  ...
  internal class CloudBuild {
    public Dictionary<string, Dictionary<string, string>> GetScmCandidates() {
      Dictionary<string, Dictionary<string, string>> candidates =
    ...
    private Dictionary<string, string> DetectGit() {
      ...
      gitSettings.Add("branch",
                      RunCommand("git", "rev-parse --abbrev-ref HEAD"));
      gitSettings.Add("root", RemoveProjectDirectory(RunCommand(
                                  "git", "rev-parse --show-toplevel")));
      return gitSettings;
    }

    private Dictionary<string, string> DetectMercurial() {
      Dictionary<string, string> mercurialSettings =
      ...
      mercurialSettings.Add("root",
                            RemoveProjectDirectory(RunCommand("hg", "root")));
      return mercurialSettings;
    }

    private Dictionary<string, string> DetectSubversion() {
      Dictionary<string, string> subversionSettings =
          new Dictionary<string, string>();
      ...
      foreach (var s in lines) {
        ...
        if (parts.Length == 2) {
          if (parts [0]
                  .Equals("Repository Root")) {
            subversionSettings.Add("url", parts [1]
                                              .Trim());
          }

          if (parts [0]
                  .Equals("URL")) {
            subversionSettings.Add("branch", parts [1]
                                                 .Trim());
          }

          if (parts [0]
                  .Equals("Working Copy Root Path")) {
            subversionSettings.Add("root", RemoveProjectDirectory(parts [1]
                                                                      .Trim()));
          }
        }
      }
    ...
    private Dictionary<string, string> DetectPerforce() {
      Dictionary<string, string> perforceSettings =
          new Dictionary<string, string>();
    ...
    private String RunCommand(string command, string arguments) {
      try {
        ...
        // Problem executing, most likely Executable not found in path on
        // Windows systems
        return null;
      }
    }
