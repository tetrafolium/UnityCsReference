...
private GUIContent[] GetTextures(string baseName, string postFix,
                                 int startIndex, int count) {
	GUIContent[] textures = new GUIContent[count];
	for (int i = 0; i < count; ++i)
		textures[i] =
			EditorGUIUtility.IconContent(baseName + (startIndex + i) + postFix);
	return textures;
}
...
internal static bool ShowAtPosition(Object targetObj, Rect activatorRect,
                                    bool showLabelIcons) {
	return ShowAtPosition(new Object[] {targetObj}, activatorRect,
	                      showLabelIcons);
}

// Returns true if shown
internal static bool ShowAtPosition(Object[] targetObjects,
                                    Rect activatorRect, bool showLabelIcons) {
	int instanceHash = targetObjects.GetHashCode();
	// We could not use realtimeSinceStartUp since it is resetted when entering
...
SetMonoScriptIconChangedCallback(MonoScriptIconChangedCallback callback) {
	if (s_IconSelector != null)
...
void SetMultipleSelectedIcons() {
	...
	Texture2D firstIcon =
		EditorGUIUtility.GetIconForObject(m_TargetObjectList[0]);
...
void DoButton(GUIContent content, Texture2D selectedIcon, bool labelIcon) {
	int controlID =
		GUIUtility.GetControlID(s_HashIconSelector, FocusType.Keyboard);

	// Draw selection background if this is the selected icon
	if (content.image == selectedIcon) {
		// When placing our selection background we assume that icons rendered
		...
		GUI.Label(rect, GUIContent.none, labelIcon ? m_Styles.selectionLabel
		: m_Styles.selection);
	}
	...
	if (EditorGUILayout.IconButton(controlID, content, GUIStyle.none)) {
		// Map to new icon
		Texture2D largeIcon =
			ConvertSmallIconToLargeIcon((Texture2D) content.image, labelIcon);
...
internal void OnGUI() {
	...
	Texture2D selectedIcon = m_MultipleSelectedIcons ? null
	: EditorGUIUtility.GetIconForObject(m_TargetObject);
	bool isSelectionALabelIcon = false;
	if (Event.current.type == EventType.Repaint) {
		selectedIcon =
			ConvertLargeIconToSmallIcon(selectedIcon, ref isSelectionALabelIcon);
	}
