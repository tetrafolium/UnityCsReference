...
// Use the FilePathAttribute when you want to have your scriptable singleton to
// persist between unity sessions. Example:
// [FilePathAttribute("Library/SearchFilters.ssf",
// FilePathAttribute.Location.ProjectFolder)] Ensure to call Save() from client
// code (derived instance)
...
public class ScriptableSingleton<T> : ScriptableObject where T
    : ScriptableObject {
  static T s_Instance;
  ...
  // On domain reload ScriptableObject objects gets reconstructed from a backup.
  // We therefore set the s_Instance here
  ...
  protected virtual void Save(bool saveAsText) {
    ...
    if (!string.IsNullOrEmpty(filePath)) {
      ...
      InternalEditorUtility.SaveToSerializedFileAndForget(new[]{s_Instance},
                                                          filePath, saveAsText);
    }
  }
