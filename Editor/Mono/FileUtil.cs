...
internal static bool AppendTextAfter(string path, string find,
                                     string append) {
	bool res = false;
	path = NiceWinPath(path);
...
internal static void CopyDirectoryRecursive(string source, string target) {
	CopyDirectoryRecursive(source, target, overwrite
	                       : false, ignoreMeta
	                       : false);
}
...
internal static void CopyDirectoryRecursive(string source, string target,
                                            bool overwrite, bool ignoreMeta) {
	CopyDirectoryRecursiveFiltered(source, target, overwrite,
	                               ignoreMeta ? @"\.meta$"
	                           : null);
}

internal static void CopyDirectoryRecursiveForPostprocess(string source,
                                                          string target,
                                                          bool overwrite) {
	CopyDirectoryRecursiveFiltered(source, target, overwrite,
	                               @".*/\.+|\.meta$");
...
CopyDirectoryRecursiveFiltered(string source, string target, bool overwrite,
                               string regExExcludeFilter) {
	CopyDirectoryFiltered(source, target, overwrite, regExExcludeFilter, true);
}

internal static void CopyDirectoryFiltered(string source, string target,
                                           bool overwrite,
                                           string regExExcludeFilter,
                                           bool recursive) {
	Regex exclude = null;
	try {
		...
		Debug.Log(
			"CopyDirectoryRecursive: Pattern '" + regExExcludeFilter +
			"' is not a correct Regular Expression. Not excluding any files.");
		return;
	}

	Func<string, bool> includeCallback = file =>
	                                     (exclude == null || !exclude.IsMatch(file));

	CopyDirectoryFiltered(source, target, overwrite, includeCallback,
	                      recursive);
}

internal static void CopyDirectoryFiltered(string source, string target,
                                           bool overwrite,
                                           Func<string, bool> includeCallback,
                                           bool recursive) {
	// Check if the target directory exists, but dont create it yet until we
	// know there are files to copy.
	...
	foreach (string di in Directory.GetDirectories(source)) {
		...
		CopyDirectoryFiltered(Path.Combine(source, fname),
		                      Path.Combine(target, fname), overwrite,
		                      includeCallback, recursive);
	}
}
...
// set the System.IO.FileAttributes.Normal recursively on all files in
// target_dir
internal static void
UnityDirectoryRemoveReadonlyAttribute(string target_dir) {
...
internal static void CopyFileIfExists(string src, string dst,
                                      bool overwrite) {
	if (File.Exists(src)) {
...
{
	// this is because on Windows \\ means network path, in unity all \ are
	// converted to / network paths become // and Path class functions think
	// it's the same as /
...
internal static string RemovePathPrefix(string fullPath, string prefix) {
	...
	while (index < partsOfFull.Length && index < partsOfPrefix.Length &&
	...
	return string.Join(Path.DirectorySeparatorChar.ToString(), partsOfFull,
	                   index, partsOfFull.Length - index);
}
...
internal static List<string> GetAllFilesRecursive(string path) {
	...
	WalkFilesystemRecursively(path, (p) => { files.Add(p); },
	                          (p) => { return true; });
	return files;
}

internal static void
WalkFilesystemRecursively(string path, Action<string> fileCallback,
...
internal static bool IsReadOnly(UnityEngine.Object asset) {
	if (!AssetDatabase.IsOpenForEdit(
		    asset, StatusQueryOptions.UseCachedIfPossible)) {
		return false;
	}
