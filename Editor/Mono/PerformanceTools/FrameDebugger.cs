...
internal enum ShowAdditionalInfo { Preview, ShaderProperties }
...
internal class FrameDebuggerWindow : EditorWindow {
  // match enum FrameEventType on C++ side!
  static public readonly string[] s_FrameEventTypeNames =
      new[]{"Clear (nothing)",
            "Clear (color)",
            "Clear (Z)",
            "Clear (color+Z)",
  ...
  // Sometimes when disabling the frame debugger, the UI does not update
  // automatically - the repaint happens, but we still think there's zero events
  // present (on Mac at least). Haven't figured out why, so whenever changing
  // the enable/limit state, just repaint a couple of times. Yeah...
  const int kNeedToRepaintFrames = 4;

  [SerializeField] float m_ListWidth = kMinListWidth * 1.5f;
  ...
  [NonSerialized] FrameDebuggerTreeView m_Tree;
  [NonSerialized]
  ...
  [NonSerialized]
  private float m_RTWhiteLevel = 1.0f;
  ...
  private ShowAdditionalInfo m_AdditionalInfo =
      ShowAdditionalInfo.ShaderProperties;
  private GUIContent[] m_AdditionalInfoGuiContents =
      Enum.GetNames(typeof(ShowAdditionalInfo))
          .Select(m => new GUIContent(m))
          .ToArray();

  static List<FrameDebuggerWindow> s_FrameDebuggers =
      new List<FrameDebuggerWindow>();
  ...
  public FrameDebuggerWindow() {
    position =
        new Rect(50, 50, (kMinListWidth + kMinDetailsWidth) * 1.5f, 350f);
  ...
  internal void ChangeFrameEventLimit(int newLimit) {
    ...
    if (newLimit != FrameDebuggerUtility.limit && newLimit > 0) {
      GameObject go =
          FrameDebuggerUtility.GetFrameEventGameObject(newLimit - 1);
      if (go != null)
  ...
  static void DisableFrameDebugger() {
    ...
    FrameDebuggerUtility.SetEnabled(false,
                                    FrameDebuggerUtility.GetRemotePlayerGUID());
  }

  internal void OnDidOpenScene() { DisableFrameDebugger(); }

  void OnPauseStateChanged(PauseState state) { RepaintOnLimitChange(); }
  ...
  internal void OnEnable() {
    if (m_AttachToPlayerState == null)
      m_AttachToPlayerState =
          PlayerConnectionGUIUtility.GetConnectionState(this);
  ...
  public void EnableIfNeeded() {
    if (FrameDebuggerUtility.IsLocalEnabled() ||
        FrameDebuggerUtility.IsRemoteEnabled())
      return;
    m_RTChannel = 0;
    m_RTIndex = 0;
    m_RTBlackLevel = 0.0f;
  ...
  private void ClickEnableFrameDebugger() {
    bool isEnabled = FrameDebuggerUtility.IsLocalEnabled() ||
                     FrameDebuggerUtility.IsRemoteEnabled();

    bool enablingLocally =
        !isEnabled &&
        m_AttachToPlayerState.connectedToTarget == ConnectionTarget.Editor;
    ...
    if (!isEnabled)
      ...
      FrameDebuggerUtility.SetEnabled(
          false, FrameDebuggerUtility.GetRemotePlayerGUID());
  ...
  void BuildCurEventDataStrings() {
    // we will show that only if drawcall count is bigger than one so update
    // unconditionally
    m_CurEventDataStrings.drawCallCount =
        string.Format("{0}", m_CurEventData.drawCallCount);

    // shader name & subshader index
    m_CurEventDataStrings.shader =
        string.Format("{0}, SubShader #{1}", m_CurEventData.shaderName,
                      m_CurEventData.subShaderIndex.ToString());

    // pass name & LightMode tag
    string passName = string.IsNullOrEmpty(m_CurEventData.passName)
                          ? "#" + m_CurEventData.shaderPassIndex
                          : m_CurEventData.passName;
    string lightMode =
        string.IsNullOrEmpty(m_CurEventData.passLightMode)
            ? ""
            : string.Format(" ({0})", m_CurEventData.passLightMode);
    ...
    if (m_CurEventData.stencilState.stencilEnable) {
      ...
      if (m_CurEventData.stencilState.readMask != 255)
        m_CurEventDataStrings.stencilReadMask =
            m_CurEventData.stencilState.readMask.ToString();

      if (m_CurEventData.stencilState.writeMask != 255)
        m_CurEventDataStrings.stencilWriteMask =
            m_CurEventData.stencilState.writeMask.ToString();
      ...
      if (m_CurEventData.rasterState.cullMode == CullMode.Back) {
        m_CurEventDataStrings.stencilComp =
            m_CurEventData.stencilState.stencilFuncFront.ToString();
        m_CurEventDataStrings.stencilPass =
            m_CurEventData.stencilState.stencilPassOpFront.ToString();
        m_CurEventDataStrings.stencilFail =
            m_CurEventData.stencilState.stencilFailOpFront.ToString();
        m_CurEventDataStrings.stencilZFail =
            m_CurEventData.stencilState.stencilZFailOpFront.ToString();
      } else if (m_CurEventData.rasterState.cullMode == CullMode.Front) {
        m_CurEventDataStrings.stencilComp =
            m_CurEventData.stencilState.stencilFuncBack.ToString();
        m_CurEventDataStrings.stencilPass =
            m_CurEventData.stencilState.stencilPassOpBack.ToString();
        m_CurEventDataStrings.stencilFail =
            m_CurEventData.stencilState.stencilFailOpBack.ToString();
        m_CurEventDataStrings.stencilZFail =
            m_CurEventData.stencilState.stencilZFailOpBack.ToString();
      } else {
        m_CurEventDataStrings.stencilComp = string.Format(
            "{0} {1}", m_CurEventData.stencilState.stencilFuncFront.ToString(),
            m_CurEventData.stencilState.stencilFuncBack.ToString());

        m_CurEventDataStrings.stencilPass = string.Format(
            "{0} {1}",
            m_CurEventData.stencilState.stencilPassOpFront.ToString(),
            m_CurEventData.stencilState.stencilPassOpBack.ToString());

        m_CurEventDataStrings.stencilFail = string.Format(
            "{0} {1}",
            m_CurEventData.stencilState.stencilFailOpFront.ToString(),
            m_CurEventData.stencilState.stencilFailOpBack.ToString());

        m_CurEventDataStrings.stencilZFail = string.Format(
            "{0} {1}",
            m_CurEventData.stencilState.stencilZFailOpFront.ToString(),
            m_CurEventData.stencilState.stencilZFailOpBack.ToString());
      }
    ...
    ShaderTextureInfo[] textureInfoArray =
        m_CurEventData.shaderProperties.textures;
    m_CurEventDataStrings.texturePropertyTooltips =
        new string[textureInfoArray.Length];
    ...
    for (int i = 0; i < textureInfoArray.Length; ++i) {
      ...
      if (depth == 0)
        tooltip.AppendFormat("Size: {0} x {1}", texture.width.ToString(),
                             texture.height.ToString());
      else
        tooltip.AppendFormat("Size: {0} x {1} x {2}", texture.width.ToString(),
                             texture.height.ToString(), depth);
      ...
      if (texture is Texture2D)
        tooltip.AppendFormat(formatFormat,
                             (texture as Texture2D).format.ToString());
      else if (texture is Cubemap)
        tooltip.AppendFormat(formatFormat,
                             (texture as Cubemap).format.ToString());
      else if (texture is Texture2DArray) {
        tooltip.AppendFormat(formatFormat,
                             (texture as Texture2DArray).format.ToString());
        tooltip.AppendFormat(depthFormat,
                             (texture as Texture2DArray).depth.ToString());
      } else if (texture is Texture3D) {
        tooltip.AppendFormat(formatFormat,
                             (texture as Texture3D).format.ToString());
        tooltip.AppendFormat(depthFormat,
                             (texture as Texture3D).depth.ToString());
      } else if (texture is CubemapArray) {
        tooltip.AppendFormat(formatFormat,
                             (texture as CubemapArray).format.ToString());
        tooltip.AppendFormat("\nCubemap Count: {0}",
                             (texture as CubemapArray).cubemapCount.ToString());
      } else if (texture is RenderTexture) {
        tooltip.AppendFormat("\nRT Format: {0}",
                             (texture as RenderTexture).format.ToString());
      }
  ...
  private bool DrawToolbar(FrameDebuggerEvent[] descs) {
    ...
    bool isSupported =
        m_AttachToPlayerState.connectedToTarget != ConnectionTarget.Editor ||
        FrameDebuggerUtility.locallySupported;

    GUILayout.BeginHorizontal(EditorStyles.toolbar);
    // enable toggle
    EditorGUI.BeginChangeCheck();
    using(new EditorGUI.DisabledScope(!isSupported)) {
      GUILayout.Toggle(FrameDebuggerUtility.IsLocalEnabled() ||
                           FrameDebuggerUtility.IsRemoteEnabled(),
                       styles.recordButton, EditorStyles.toolbarButtonLeft,
    ...
    PlayerConnectionGUILayout.ConnectionTargetSelectionDropdown(
        m_AttachToPlayerState, EditorStyles.toolbarDropDown);

    bool isAnyEnabled = FrameDebuggerUtility.IsLocalEnabled() ||
                        FrameDebuggerUtility.IsRemoteEnabled();
    if (isAnyEnabled && ProfilerDriver.connectedProfiler !=
                            FrameDebuggerUtility.GetRemotePlayerGUID()) {
    ...
    using(new EditorGUI.DisabledScope(FrameDebuggerUtility.count <= 1)) {
      newLimit = EditorGUILayout.IntSlider(FrameDebuggerUtility.limit, 1,
                                           FrameDebuggerUtility.count, 1,
                                           EditorStyles.toolbarSlider);
    }
    if (EditorGUI.EndChangeCheck()) {
      ChangeFrameEventLimit(newLimit);
    }
    GUILayout.Label("of " + FrameDebuggerUtility.count,
    ...
    using(new EditorGUI.DisabledScope(newLimit >= FrameDebuggerUtility.count)) {
      ...
      // If we had last event selected, and something changed in the scene so
      // that number of events is different - then try to keep the last event
      // selected.
      if (m_PrevEventsLimit == m_PrevEventsCount) {
        if (FrameDebuggerUtility.count != m_PrevEventsCount &&
            FrameDebuggerUtility.limit == m_PrevEventsLimit) {
  ...
  private void DrawMeshPreview(Rect previewRect, Rect meshInfoRect, Mesh mesh,
                               int meshSubset) {
    if (m_PreviewUtility == null) {
    ...
    ModelInspector.RenderMeshPreview(mesh, m_PreviewUtility, m_Settings,
                                     meshSubset);
    ...
    string info = meshName + " subset " + meshSubset + "\n" +
                  m_CurEventData.vertexCount + " verts, " +
                  m_CurEventData.indexCount + " indices";
  ...
  private bool DrawEventMesh() {
    ...
    Rect previewRect =
        GUILayoutUtility.GetRect(10, 10, GUILayout.ExpandHeight(true));
    if (previewRect.width < kMinPreviewSize ||
        previewRect.height < kMinPreviewSize)
      return true;

    GameObject go = FrameDebuggerUtility.GetFrameEventGameObject(
        m_CurEventData.frameEventIndex);

    // Info display at bottom (and pings object when clicked)
    Rect meshInfoRect = previewRect;
    meshInfoRect.yMin =
        meshInfoRect.yMax - EditorGUIUtility.singleLineHeight * 2;
    ...
    m_Settings.previewDir =
        PreviewGUI.Drag2D(m_Settings.previewDir, previewRect);
  ...
  private void DrawRenderTargetControls() {
    ...
    var isDepthOnlyRT =
        GraphicsFormatUtility.IsDepthFormat((GraphicsFormat) cur.rtFormat);
    ...
    using(new EditorGUI.DisabledScope(showableRTCount <= 1)) {
      ...
      m_RTIndex = EditorGUILayout.Popup(m_RTIndex, rtNames,
                                        EditorStyles.toolbarPopupLeft,
                                        GUILayout.Width(70));
    }
    ...
    using(new EditorGUI.DisabledScope(isDepthOnlyRT)) {
      ...
      m_RTChannel = GUILayout.Toolbar(m_RTChannel, Styles.channelLabels,
    ...
    EditorGUILayout.MinMaxSlider(ref m_RTBlackLevel, ref m_RTWhiteLevel, 0.0f,
                                 1.0f, GUILayout.MaxWidth(200.0f));
    ...
    if (EditorGUI.EndChangeCheck() || rtIndexToSet != m_RTIndexLastSet) {
      ...
      FrameDebuggerUtility.SetRenderTargetDisplayOptions(
          rtIndexToSet, mask, m_RTBlackLevel, m_RTWhiteLevel);
    ...
    if (cur.rtLoadAction >= 0 || cur.rtStoreAction >= 0 ||
        cur.rtMemoryless != 0) {
      var sl = new List<string>();
      if (cur.rtLoadAction >= 0)
        sl.Add(String.Format("LoadAction: {0}",
                             (RenderBufferLoadAction) cur.rtLoadAction));
      if (cur.rtStoreAction >= 0)
        sl.Add(String.Format("StoreAction: {0}",
                             (RenderBufferStoreAction) cur.rtStoreAction));
    ...
    GUILayout.Label(string.Format("{0}x{1} {2} {3}", cur.rtWidth, cur.rtHeight,
    ...
    if (cur.rtLoadAction == (int) RenderBufferLoadAction.Clear) {
      ...
      bool isStencilFormat =
          GraphicsFormatUtility.IsStencilFormat((GraphicsFormat) cur.rtFormat);
      if (!isDepthFormat && !isStencilFormat) {
        GUILayout.Label(string.Format("Clear Color ({0}, {1}, {2}, {3})",
                                      cur.rtClearColorR, cur.rtClearColorG,
                                      cur.rtClearColorB, cur.rtClearColorA));
      } else {
        if (isDepthFormat)
          GUILayout.Label(string.Format("Clear Depth {0}", cur.rtClearDepth));
        if (isStencilFormat)
          GUILayout.Label(
              string.Format("Clear Stencil {0:X}", cur.rtClearStencil));
      }
    }
  ...
  private void DrawEventDrawCallInfo() {
    if (m_CurEventData.drawCallCount > 1)
      EditorGUILayout.LabelField("Draw Calls",
                                 m_CurEventDataStrings.drawCallCount);

    // shader, pass & keyword information
    EditorGUILayout.LabelField("Shader", m_CurEventDataStrings.shader);

    if (GUI.Button(GUILayoutUtility.GetLastRect(), Styles.selectShaderTooltip,
                   GUI.skin.label)) {
    ...
    if (!string.IsNullOrEmpty(m_CurEventData.shaderKeywords)) {
      ...
      if (GUI.Button(GUILayoutUtility.GetLastRect(),
                     Styles.copyToClipboardTooltip, GUI.skin.label))
        EditorGUIUtility.systemCopyBuffer = m_CurEventDataStrings.shader +
                                            System.Environment.NewLine +
                                            m_CurEventData.shaderKeywords;
    }

    DrawStates();

    // Show why this draw call can't batch with the previous one.
    if (m_CurEventData.batchBreakCause >
        1) // Valid batch break cause enum value on the C++ side starts at 2.
    {
      GUILayout.Space(10.0f);
      GUILayout.Label(Styles.causeOfNewDrawCallLabel, EditorStyles.boldLabel);
      GUILayout.Label(styles.batchBreakCauses[m_CurEventData.batchBreakCause],
    ...
    m_AdditionalInfo = (ShowAdditionalInfo) GUILayout.Toolbar(
        (int) m_AdditionalInfo, m_AdditionalInfoGuiContents, "LargeButton",
    ...
    case ShowAdditionalInfo.Preview:
      ...
      if (!DrawEventMesh()) {
        ...
        EditorGUILayout.LabelField("Vertices",
                                   m_CurEventData.vertexCount.ToString());
        EditorGUILayout.LabelField("Indices",
                                   m_CurEventData.indexCount.ToString());
      }
      break;
  ...
  private void DrawEventComputeDispatchInfo() {
    ...
    if (m_CurEventData.csThreadGroupsX != 0 ||
        m_CurEventData.csThreadGroupsY != 0 ||
        m_CurEventData.csThreadGroupsZ != 0)
      threadGroupsText = string.Format(
          "{0}x{1}x{2}", m_CurEventData.csThreadGroupsX,
          m_CurEventData.csThreadGroupsY, m_CurEventData.csThreadGroupsZ);
    else
  ...
  private void DrawCurrentEvent(Rect rect, FrameDebuggerEvent[] descs) {
    ...
    bool isFrameEventDataValid =
        curEventIndex == m_CurEventData.frameEventIndex;

    if (eventDataHash != 0 && m_CurEventDataHash != eventDataHash) {
      isFrameEventDataValid =
          FrameDebuggerUtility.GetFrameEventData(curEventIndex, m_CurEventData);
    ...
    GUILayout.Label(string.Format("Event #{0}: {1}", (curEventIndex + 1),
                                  s_FrameEventTypeNames[(int) cur.type]),
                    EditorStyles.boldLabel);

    if (FrameDebuggerUtility.IsRemoteEnabled() &&
        FrameDebuggerUtility.receivingRemoteFrameEventData) {
  ...
  void DrawShaderPropertyFlags(int flags) {
    // lowest bits of flags are set for each shader stage that property is used
    // in; matching ShaderType C++ enum
  ...
  void ShaderPropertyCopyValueMenu(Rect valueRect, System.Object value) {
    ...
    if (e.type == EventType.ContextClick &&
        valueRect.Contains(e.mousePosition)) {
      e.Use();
      var menu = new GenericMenu();
      menu.AddItem(
          EditorGUIUtility.TrTextContent("Copy value"), false, delegate {
            var str = string.Empty;
            if (value is Vector4)
  ...
  private void OnGUIShaderPropFloats(ShaderFloatInfo[] floats, int startIndex,
                                     int numValues) {
    ...
    if (numValues == 1) {
      GUILayout.Label(t.name, EditorStyles.miniLabel,
                      GUILayout.MinWidth(kNameFieldWidth));
      DrawShaderPropertyFlags(t.flags);
      GUILayout.Label(
          t.value.ToString(kFloatFormat,
                           CultureInfo.InvariantCulture.NumberFormat),
          EditorStyles.miniLabel, GUILayout.MinWidth(kValueFieldWidth));
      ShaderPropertyCopyValueMenu(GUILayoutUtility.GetLastRect(), t.value);
    } else {
      string arrayName = String.Format("{0} [{1}]", t.name, numValues);
      GUILayout.Label(arrayName, EditorStyles.miniLabel,
                      GUILayout.MinWidth(kNameFieldWidth));
      DrawShaderPropertyFlags(t.flags);

      Rect buttonRect = GUILayoutUtility.GetRect(
          Styles.arrayValuePopupButton, GUI.skin.button,
          GUILayout.MinWidth(kValueFieldWidth));
      ...
      if (GUI.Button(buttonRect, Styles.arrayValuePopupButton)) {
        ArrayValuePopup.GetValueStringDelegate getValueString =
            (int index, bool highPrecision) => floats[index].value.ToString(
                highPrecision ? kFloatDetailedFormat
                : kFloatFormat, CultureInfo.InvariantCulture.NumberFormat);

        PopupWindowWithoutFocus.Show(
            buttonRect,
            new ArrayValuePopup(startIndex, numValues, 100.0f, getValueString),
            new[]{PopupLocation.Left, PopupLocation.Below,
                  PopupLocation.Right});
      }
    }
    GUILayout.FlexibleSpace();
  ...
  private void OnGUIShaderPropVectors(ShaderVectorInfo[] vectors,
                                      int startIndex, int numValues) {
    GUILayout.BeginHorizontal();
    GUILayout.Space(kShaderPropertiesIndention);
    ...
    if (numValues == 1) {
      GUILayout.Label(t.name, EditorStyles.miniLabel,
                      GUILayout.MinWidth(kNameFieldWidth));
      DrawShaderPropertyFlags(t.flags);
      GUILayout.Label(t.value.ToString(kFloatDetailedFormat),
      ...
      if (GUI.Button(buttonRect, Styles.arrayValuePopupButton)) {
        ArrayValuePopup.GetValueStringDelegate getValueString =
            (int index, bool highPrecision) => vectors[index].value.ToString(
                highPrecision ? kFloatDetailedFormat
                : kFloatFormat);
  ...
  private void OnGUIShaderPropMatrices(ShaderMatrixInfo[] matrices,
                                       int startIndex, int numValues) {
    GUILayout.BeginHorizontal();
    GUILayout.Space(kShaderPropertiesIndention);
    ...
    if (numValues == 1) {
      GUILayout.Label(t.name, EditorStyles.miniLabel,
                      GUILayout.MinWidth(kNameFieldWidth));
      DrawShaderPropertyFlags(t.flags);
      GUILayout.Label(t.value.ToString(kFloatFormat), EditorStyles.miniLabel,
                      GUILayout.MinWidth(kValueFieldWidth));
      ShaderPropertyCopyValueMenu(GUILayoutUtility.GetLastRect(), t.value);
    } else {
      string arrayName = String.Format("{0} [{1}]", t.name, numValues);
  ...
  private void OnGUIShaderPropTexture(int idx, ShaderTextureInfo t) {
    ...
    Rect valueRect = GUILayoutUtility.GetRect(
        GUIContent.none, GUI.skin.label, GUILayout.MinWidth(kValueFieldWidth));

    // display texture similar to ObjectField, just without much of interaction
    // or styling:
    ...
    if (t.value != null && previewRect.Contains(evt.mousePosition))
      GUI.Label(
          previewRect,
          GUIContent.Temp(string.Empty,
                          m_CurEventDataStrings.texturePropertyTooltips[idx]));
    ...
    if (evt.type == EventType.Repaint) {
      ...
      if (t.value != null) {
        // for 2D textures, we want to display them directly as a preview (this
        // will make render textures display their contents); but for cube maps
        // and other non-2D types DrawPreview does not do anything useful right
        // now, so get their asset type icon at least
  ...
  void DrawShaderProperties(ShaderProperties props) {
    m_ScrollViewShaderProps =
        GUILayout.BeginScrollView(m_ScrollViewShaderProps);
    ...
    if (props.floats.Length > 0) {
      ...
      for (int i = 0; i < props.floats.Length;) {
        int arraySize =
            (props.floats[i].flags >> kShaderTypeBits) & kArraySizeBitMask;
    ...
    if (props.vectors.Length > 0) {
      ...
      for (int i = 0; i < props.vectors.Length;) {
        int arraySize =
            (props.vectors[i].flags >> kShaderTypeBits) & kArraySizeBitMask;
    ...
    if (props.matrices.Length > 0) {
      ...
      for (int i = 0; i < props.matrices.Length;) {
        int arraySize =
            (props.matrices[i].flags >> kShaderTypeBits) & kArraySizeBitMask;
  ...
  void DrawStates() {
    ...
    // blend state
    string blendText =
        string.Format("{0} {1}", blendState.srcBlend, blendState.dstBlend);
    // only add alpha blend mode if different from RGB one
    if (blendState.srcBlendAlpha != blendState.srcBlend ||
        blendState.dstBlendAlpha != blendState.dstBlend)
      blendText += string.Format(", {0} {1}", blendState.srcBlendAlpha,
                                 blendState.dstBlendAlpha);

    EditorGUILayout.LabelField("Blend", blendText);

    // only add blend op if non-Add
    if (blendState.blendOp != BlendOp.Add ||
        blendState.blendOpAlpha != BlendOp.Add) {
      string blendOpText;
      if (blendState.blendOp == blendState.blendOpAlpha)
        blendOpText = blendState.blendOp.ToString();
      else
        blendOpText = string.Format("{0}, {1}", blendState.blendOp,
    ...
    EditorGUILayout.LabelField("ZWrite",
                               depthState.depthWrite == 0 ? "Off" : "On");
    EditorGUILayout.LabelField("Cull", rasterState.cullMode.ToString());
    EditorGUILayout.LabelField("Conservative",
                               rasterState.conservative.ToString());

    // only add depth offset if non zero
    if (rasterState.slopeScaledDepthBias != 0 || rasterState.depthBias != 0) {
      string offsetText = UnityString.Format(
          "{0}, {1}", rasterState.slopeScaledDepthBias, rasterState.depthBias);
      EditorGUILayout.LabelField("Offset", offsetText);
    }

    // Stencil state
    if (m_CurEventData.stencilState.stencilEnable) {
      EditorGUILayout.LabelField("Stencil Ref",
                                 m_CurEventDataStrings.stencilRef);

      if (m_CurEventData.stencilState.readMask != 255)
        EditorGUILayout.LabelField("Stencil ReadMask",
                                   m_CurEventDataStrings.stencilReadMask);

      if (m_CurEventData.stencilState.writeMask != 255)
        EditorGUILayout.LabelField("Stencil WriteMask",
                                   m_CurEventDataStrings.stencilWriteMask);

      EditorGUILayout.LabelField("Stencil Comp",
                                 m_CurEventDataStrings.stencilComp);
      EditorGUILayout.LabelField("Stencil Pass",
                                 m_CurEventDataStrings.stencilPass);
      EditorGUILayout.LabelField("Stencil Fail",
                                 m_CurEventDataStrings.stencilFail);
      EditorGUILayout.LabelField("Stencil ZFail",
                                 m_CurEventDataStrings.stencilZFail);
    }
  }
  ...
  internal void OnGUI() {
    ...
    if (m_Tree == null) {
      m_Tree =
          new FrameDebuggerTreeView(descs, m_TreeViewState, this, new Rect());
    ...
    if (!FrameDebuggerUtility.IsLocalEnabled() &&
      ...
      if (!FrameDebuggerUtility.locallySupported) {
        EditorGUILayout.HelpBox(
            "Frame Debugger requires multi-threaded renderer. Usually Unity uses that; if it does not, try starting with -force-gfx-mt command line argument.",
            MessageType.Warning, true);
      }

      // info box
      EditorGUILayout.HelpBox(
          "Frame Debugger lets you step through draw calls and see how exactly frame is rendered. Click Enable!",
          MessageType.Info, true);
    } else {
      float toolbarHeight = EditorStyles.toolbar.fixedHeight;

      var dragRect = new Rect(m_ListWidth, toolbarHeight, kResizerWidth,
                              position.height - toolbarHeight);
      dragRect = EditorGUIUtility.HandleHorizontalSplitter(
          dragRect, position.width, kMinListWidth, kMinDetailsWidth);
      m_ListWidth = dragRect.x;

      var listRect = new Rect(0, toolbarHeight, m_ListWidth,
                              position.height - toolbarHeight);
      var currentEventRect =
          new Rect(m_ListWidth + kDetailsMargin, toolbarHeight + kDetailsMargin,
  ...
  private void RepaintAllNeededThings() {
    // indicate that editor needs a redraw (mostly to get offscreen cameras
    // rendered)
    EditorApplication.SetSceneRepaintDirty();
    // Note: do NOT add GameView.RepaintAll here; that would cause really
    // confusing behaviors when there are offscreen (rendering into RTs)
    // cameras.
  ...
  void DrawEventsTree(Rect rect) { m_Tree.OnGUI(rect); }
  ...
  internal class Styles {
    ...
    public GUIStyle toolbarLabelSliderGroup = new GUIStyle(
        EditorStyles.toolbarButton){margin = new RectOffset(4, 4, 0, 0),
                                    padding = new RectOffset(4, 4, 0, 0)};

    public GUIContent recordButton =
        new GUIContent(EditorGUIUtility.TrTextContent(
            "Record", "Record profiling information"));
    public GUIContent prevFrame = new GUIContent(EditorGUIUtility.TrIconContent(
        "Profiler.PrevFrame", "Go back one frame"));
    public GUIContent nextFrame = new GUIContent(EditorGUIUtility.TrIconContent(
        "Profiler.NextFrame", "Go one frame forwards"));

    public GUIContent[] headerContent;
    public readonly string[] batchBreakCauses;

    public static readonly string[] s_ColumnNames =
        new[]{"#", "Type", "Vertices", "Indices"};
    public static readonly GUIContent[] mrtLabels =
        new[]{EditorGUIUtility.TrTextContent("RT 0", "Show render target #0"),
    ...
    public static readonly GUIContent depthLabel =
        EditorGUIUtility.TrTextContent("Depth", "Show depth buffer");
    ...
    public static readonly GUIContent channelHeader =
        EditorGUIUtility.TrTextContent(
            "Channels", "Which render target color channels to show");
    public static readonly GUIContent levelsHeader =
        EditorGUIUtility.TrTextContent(
            "Levels", "Render target display black/white intensity levels");
    public static readonly GUIContent causeOfNewDrawCallLabel =
        EditorGUIUtility.TrTextContent(
            "Why this draw call can't be batched with the previous one");
    public static readonly GUIContent selectShaderTooltip =
        EditorGUIUtility.TrTextContent("", "Click to select shader");
    public static readonly GUIContent copyToClipboardTooltip =
        EditorGUIUtility.TrTextContent(
            "", "Click to copy shader and keywords text to clipboard.");
    public static readonly GUIContent arrayValuePopupButton =
        EditorGUIUtility.TrTextContent("...");
  ...
  private class ArrayValuePopup : PopupWindowContent {
    public delegate string GetValueStringDelegate(int index,
                                                  bool highPrecision);
    ...
    public ArrayValuePopup(int startIndex, int numValues, float windowWidth,
    ...
    public override Vector2 GetWindowSize() {
      float lineHeight =
          m_Style.lineHeight + m_Style.padding.vertical + m_Style.margin.top;
      return new Vector2(m_WindowWidth,
                         Math.Min(lineHeight * m_NumValues, 250.0f));
    }
    ...
    public override void OnGUI(Rect rect) {
      ...
      for (int i = 0; i < m_NumValues; ++i) {
        string text = String.Format("[{0}]\t{1}", i,
                                    GetValueString(m_StartIndex + i, false));
      ...
      if (e.type == EventType.ContextClick && rect.Contains(e.mousePosition)) {
        ...
        for (int i = 0; i < m_NumValues; ++i) {
          allText += String.Format("[{0}]\t{1}\n", i,
                                   GetValueString(m_StartIndex + i, true));
        }

        var menu = new GenericMenu();
        menu.AddItem(EditorGUIUtility.TrTextContent("Copy value"), false,
                     delegate { EditorGUIUtility.systemCopyBuffer = allText; });
        menu.ShowAsContext();
      }
    }
  }
