...
internal class FrameDebuggerTreeView {
  ...
  public FrameDebuggerTreeView(FrameDebuggerEvent[] frameEvents,
                               TreeViewState treeViewState,
                               FrameDebuggerWindow window, Rect startRect) {
  ...
  void SelectionChanged(int[] selectedIDs) {
    ...
    // IDs for hierarchy nodes are negative and need to stay consistently
    // ordered so that tree expanded state behaves well when something in the
    // scene changes.
    //
    // When selecting a hierarchy node, we want it's last child event to be set
    // as the limit, so that rendered state corresponds to "everything up to and
    // including this whole sub-tree".
    if (eventIndex <= 0) {
  ...
  public void SelectFrameEventIndex(int eventIndex) {
    ...
    // If the limit event is the same, then do not change the currently selected
    // item.
  ...
  // Item for TreeView
  // ID is different for leaf nodes (actual frame events) vs hierarchy nodes
  // (parent profiler nodes):
  // - leaf node IDs are frame event indices (always > 0).
  // - hierarchy node IDs are always negative; to get frame event index we want
  // we need to lookup the node and get it from m_EventIndex.
  private class FDTreeViewItem : TreeViewItem {
    ...
    public FDTreeViewItem(int id, int depth, FDTreeViewItem parent,
                          string displayName)
  ...
  private class FDTreeViewGUI : TreeViewGUI {
    ...
    protected override void OnContentGUI(Rect rect, int row,
                                         TreeViewItem itemRaw, string label,
      ...
      if (item.m_ChildEventCount > 0) {
        ...
        // reduce width of available space for the name, so that it does not
        // overlap event count
      ...
      if (item.id <= 0)
        ...
        text = FrameDebuggerWindow
                   .s_FrameEventTypeNames[(int) item.m_FrameEvent.type] +
               item.displayName; // leaf event
  ...
  internal class FDTreeViewDataSource : TreeViewDataSource {
    ...
    public FDTreeViewDataSource(TreeViewController treeView,
                                FrameDebuggerEvent[] frameEvents)
        : base(treeView) {
    ...
    public void SetEvents(FrameDebuggerEvent[] frameEvents) {
      ...
      // If we already had something in there, we want to preserve user's
      // expanded items.
      if (wasEmpty) {
    ...
    public override bool CanBeMultiSelected(TreeViewItem item) { return false; }

    // Used while building the tree data source; represents current tree
    // hierarchy level
    private class FDTreeHierarchyLevel {
      internal readonly FDTreeViewItem item;
      internal readonly List<TreeViewItem> children;
      internal FDTreeHierarchyLevel(int depth, int id, string name,
                                    FDTreeViewItem parent) {
        item = new FDTreeViewItem(id, depth, parent, name);
        children = new List<TreeViewItem>();
      }
    }
    private static void
    CloseLastHierarchyLevel(List<FDTreeHierarchyLevel> eventStack,
                            int prevFrameEventIndex) {
      ...
      if (eventStack[idx].item.parent != null)
        ((FDTreeViewItem) eventStack[idx].item.parent).m_ChildEventCount +=
            eventStack[idx].item.m_ChildEventCount;
    ...
    public override void FetchData() {
      ...
      for (var i = 0; i < m_FrameEvents.Length; ++i) {
        ...
        // that the TreeView always has.
                ...
                // close all the further levels from previous events in the
                // stack
                ...
                for (var j = level; j < names.Length; ++j) {
                  ...
                  var newLevel = new FDTreeHierarchyLevel(
                      eventStack.Count - 1, --hierarchyIDCounter, names[j],
                      parent.item);
                ...
                FDTreeHierarchyLevel parentEvent =
                    eventStack[eventStack.Count - 1];
                var leafEventID = i + 1;
                var item = new FDTreeViewItem(leafEventID, eventStack.Count - 1,
                                              parentEvent.item, displayName);
