...
internal class DefaultExternalCodeEditor : IExternalCodeEditor {
  static readonly GUIContent k_ResetArguments =
  ...
  string Arguments {
    get {
      // Starting in Unity 5.5, we support setting script editor arguments on
      // OSX and use then when opening the script editor. Before Unity 5.5, we
      // would still save the default script editor args in EditorPrefs, even
      // though we never used them. This means that the user potentially has
      // some script editor args saved and once he upgrades to 5.5, they will be
      // used when open the script editor. Which unintended and causes a
      // regression in behaviour. So on OSX we change the key for per
      // application for script editor args, to avoid reading the one from
      // previous versions. The year 2021: Delete mac hack.
      ...
      return EditorPrefs.GetString(k_ArgumentKey + Installation,
                                   k_DefaultArgument);
    }
    set {
      if (Application.platform == RuntimePlatform.OSXEditor) {
  ...
  string Installation {
    ...
    set { m_ChosenInstallation = value; }
  }

  public CodeEditor.Installation[] Installations { get; }
  public bool
  TryGetInstallationForPath(string editorPath,
                            out CodeEditor.Installation installation) {
    installation = new CodeEditor.Installation{
        Name = Path.GetFileNameWithoutExtension(editorPath), Path = editorPath};
    return true;
  }

  public void OnGUI() {
    Arguments =
        EditorGUILayout.TextField("External Script Editor Args", Arguments);
  ...
  public bool OpenProject(string path, int line, int column) {
    if (path != "" &&
        !SupportsExtension(
            path)) // Assets - Open C# Project passes empty path here
    {
      return false;
    }
