...
public enum PrefabType {
  ...
  // The object was an instance of a prefab, but the original prefab could not
  // be found.
  MissingPrefabInstance = 5,
  // The object is an instance of a user created prefab, but the connection is
  // broken.
  DisconnectedPrefabInstance = 6,
  // The object is an instance of an imported 3D model, but the connection is
  // broken.
...
[Flags]
[Obsolete(
    "This has turned into the more explicit APIs, SavePrefabAsset, SaveAsPrefabAsset, SaveAsPrefabAssetAndConnect")]
...
public sealed partial class PrefabUtility {
  internal static class GameObjectStyles {
    public static Texture2D gameObjectIcon =
        EditorGUIUtility.LoadIconRequired("UnityEngine/GameObject Icon");
    public static Texture2D prefabIcon =
        EditorGUIUtility.LoadIconRequired("Prefab Icon");
  }

  private const string kMaterialExtension = ".mat";
  internal const string kDummyPrefabStageRootObjectName =
  ...
  internal static void ExtractSelectedObjectsFromPrefab() {
    ...
    foreach (var selectedObj in Selection.objects) {
      ...
      // use the first selected element as the basis for the folder path where
      // all the materials will be extracted
      if (folder == null) {
        folder = EditorUtility.SaveFolderPanel(
            "Select Materials Folder",
            FileUtil.DeleteLastPathNameComponent(path), "");
      ...
      // TODO: [bogdanc 3/6/2017] if we want this function really generic, we
      // need to know what extension the new asset files should have
      var extension =
          selectedObj is Material ? kMaterialExtension : String.Empty;
      var newAssetPath =
  ...
  internal static void ExtractMaterialsFromAsset(Object[] targets,
    ...
    foreach (var t in targets) {
      ...
      foreach (var material in materials) {
        var newAssetPath =
            FileUtil.CombinePaths(destinationPath, material.name) +
            kMaterialExtension;
        newAssetPath = AssetDatabase.GenerateUniqueAssetPath(newAssetPath);
  ...
  GetObjectListFromHierarchy(HashSet<int> hierarchyInstanceIDs,
    ...
    foreach (var component in components) {
      if (component == null) {
        throw new Exception(String.Format(
            "Component on GameObject '{0}' is invalid", gameObject.name));
      }
    ...
    for (var i = 0; i < childCount; i++)
      GetObjectListFromHierarchy(hierarchyInstanceIDs,
                                 transform.GetChild(i).gameObject);
  }

  private static void CollectAddedObjects(GameObject gameObject,
                                          HashSet<int> hierarchyInstanceIDs,
                                          List<Object> danglingObjects) {
    Transform transform = null;
    List<Component> components = new List<Component>();
    ...
    if (hierarchyInstanceIDs.Contains(gameObject.GetInstanceID())) {
      ...
      int childCount = transform.childCount;
      for (var i = 0; i < childCount; i++)
        CollectAddedObjects(transform.GetChild(i).gameObject,
                            hierarchyInstanceIDs, danglingObjects);
    } else {
      danglingObjects.Add(gameObject);
    }
  }

  internal static void RegisterNewObjects(GameObject newHierarchy,
                                          HashSet<int> hierarchyInstanceIDs,
                                          string actionName) {
    ...
    // We need to ensure that dangling components are registered in an
    // acceptable order regarding dependencies. For example, if we're adding
    // RigidBody and ConfigurableJoint, the RigidBody will need to be added
    // first (as the ConfigurableJoint depends upon it existing)
    var addedTypes = new HashSet<Type>(){typeof(Transform)};
    ...
    while (danglingObjects.Count > 0 && !emptyPass) {
      ...
      for (var i = 0; i < danglingObjects.Count; i++) {
        ...
        var reqs = danglingObject.GetType().GetCustomAttributes(
            typeof(RequireComponent), inherit
            : true);
        var requiredComponentsExist = true;
        foreach (RequireComponent req in reqs) {
          if ((req.m_Type0 != null && !addedTypes.Contains(req.m_Type0)) ||
              (req.m_Type1 != null && !addedTypes.Contains(req.m_Type1)) ||
              (req.m_Type2 != null && !addedTypes.Contains(req.m_Type2))) {
    ...
    Debug.Assert(danglingObjects.Count == 0,
  ...
  static void
  ThrowExceptionIfNotValidPrefabInstanceObject(Object prefabInstanceObject,
                                               bool isApply) {
    if (!(prefabInstanceObject is GameObject ||
          prefabInstanceObject is Component))
      throw new ArgumentException(
          "Calling apply or revert methods on an object which is not a GameObject or Component is not supported.",
          nameof(prefabInstanceObject));
  ...
  static void
  ThrowExceptionIfInstanceIsPersistent(Object prefabInstanceObject) {
  ...
  public static void RevertPrefabInstance(GameObject instanceRoot,
    ...
    bool isDisconnected =
        PrefabUtility.IsDisconnectedFromPrefabAsset(instanceRoot);

    GameObject prefabInstanceRoot =
        GetOutermostPrefabInstanceRoot(instanceRoot);
    ...
    if (action == InteractionMode.UserAction) {
      ...
      if (isDisconnected) {
        Undo.RegisterCreatedObjectUndo(
            GetPrefabInstanceHandle(prefabInstanceRoot), actionName);
      }
    }
  }

  public static void ApplyPrefabInstance(GameObject instanceRoot,
                                         InteractionMode action) {
    ...
    using(new AtomicUndoScope()) {
      ...
      Object correspondingSourceObject =
          GetCorrespondingObjectFromSource(prefabInstanceRoot);

      HashSet<int> prefabHierarchy = null;
      if (action == InteractionMode.UserAction) {
        Undo.RegisterFullObjectHierarchyUndo(
            correspondingSourceObject,
            actionName); // handles changes to existing objects and object what
                         // will be deleted but not objects that are created
        Undo.RegisterFullObjectHierarchyUndo(prefabInstanceRoot, actionName);

        prefabHierarchy = new HashSet<int>();
        GetObjectListFromHierarchy(prefabHierarchy,
                                   correspondingSourceObject as GameObject);
      }

      PrefabUtility.ApplyPrefabInstance(prefabInstanceRoot);

      if (action == InteractionMode.UserAction) {
        RegisterNewObjects(correspondingSourceObject as GameObject,
                           prefabHierarchy,
                           actionName); // handles created objects
        if (isDisconnected) {
          var prefabInstanceHandle =
              GetPrefabInstanceHandle(prefabInstanceRoot);
    ...
    Analytics.SendApplyEvent(Analytics.ApplyScope.EntirePrefab, instanceRoot,
                             null, action, startTime, false);
  }

  private static void
  MapObjectReferencePropertyToSourceIfApplicable(SerializedProperty property,
                                                 string assetPath) {
    ...
    referencedObject =
        GetCorrespondingObjectFromSourceAtPath(referencedObject, assetPath);
    if (referencedObject != null) {
      property.objectReferenceValue = referencedObject;
    }
  }

  public static void ApplyPropertyOverride(SerializedProperty instanceProperty,
                                           string assetPath,
                                           InteractionMode action) {
    DateTime startTime = DateTime.UtcNow;

    Object prefabInstanceObject =
        instanceProperty.serializedObject.targetObject;
    ThrowExceptionIfNotValidPrefabInstanceObject(prefabInstanceObject, true);

    ApplyPropertyOverrides(prefabInstanceObject, instanceProperty, assetPath,
                           true, action);

    Analytics.SendApplyEvent(
        Analytics.ApplyScope.PropertyOverride, prefabInstanceObject, assetPath,
        action, startTime,
        IsPropertyOverrideDefaultOverrideComparedToAnySource(instanceProperty));
  }

  // This method is called both from ApplyPropertyOverride and
  // ApplyObjectOverride. In the former case, optionalSingleInstanceProperty is
  // passed along as is the only property that should be processed. In the
  // latter, all properties in the prefabInstanceObject are iterated. An
  // alternative approach was considered where the method takes an array of
  // SerializedProperties, but since there can be thousands of those in a
  // component, and they would each have to be copied from the iterator, it's
  // better to handle properties inline as the iterator iterates over them. This
  // does mean that we need to cache the SerializedObjects that end up being
  // touched. Those are that of the prefabInstanceObject itself, and the chain
  // of corresponding object all the way to the Prefab at the specified
  // assetPath. Since calling ApplyModifiedProperties on a SerializedObject can
  // trigger a whole chain of imports, we don't want to create and call
  // ApplyModifiedProperties more than once for each SerializedObject, hence the
  // caching. We also can't swap the inner and outer loop, iterating the chain
  // of corresponding objects in the outer loop and the SerializedProperties in
  // the inner loop, since we only process overridden properties, so it would
  // again require storing information about that for each property in some kind
  // of list, which we want to avoid.
  static void
  ApplyPropertyOverrides(Object prefabInstanceObject,
                         SerializedProperty optionalSingleInstanceProperty,
                         string assetPath, bool allowApplyDefaultOverride,
                         InteractionMode action) {
    Object prefabSourceObject =
        GetCorrespondingObjectFromSourceAtPath(prefabInstanceObject, assetPath);
    if (prefabSourceObject == null)
      return;

    SerializedObject prefabSourceSerializedObject =
        new SerializedObject(prefabSourceObject);
    ...
    if (optionalSingleInstanceProperty != null) {
      ...
      property =
          GetArrayPropertyIfGivenPropertyIsPartOfArrayElementInInstanceWhichDoesNotExistInAsset(
              optionalSingleInstanceProperty, prefabSourceSerializedObject,
              action, out cancel);
      ...
      if (property == null) {
        // We didn't find any mismatching array so just use the property the
        // user supplied.
      ...
      // component, since array sizes will always be applied before array
      // content.
    ...
    if (!property.hasVisibleChildren) {
      if (property.prefabOverride)
        ApplySingleProperty(property, prefabSourceSerializedObject, assetPath,
                            isObjectOnRootInAsset, true,
                            allowApplyDefaultOverride, serializedObjects,
                            action);
    } else {
      while (property.Next(property.hasVisibleChildren) &&
        ...
        // references, and if they reference non-asset objects, those references
        // will get lost, since ApplySingleProperty only patches up references
        // in the provided property; not its children. This could be fixed by
        // letting ApplySingleProperty patch up all its child properties as
        // well, but then calling ApplySingleProperty n times would result in
        // time complexity n*log(n). Instead we only call ApplySingleProperty
        // for visible leaf properties - the ones that actually contain the
        // data. Technically, leaf properties contain the data, but we're using
        // visible leafs here, which corresponds to leaf nodes as shown in the
        // Inspector. Note, this is not related to foldout expanded state; it's
        // related to the fact that some nodes can have flags that hide them.
        // Furthermore, special property types - like object references, which
        // is what we're particularly interested in - are hardcoded to have
        // their child nodes hidden. We need to call ApplySingleProperty on the
        // object reference property and not its hidden children, so we use
        // hasHiddenChildren, not hasChildren, to determine which properties to
        // call the method on. Applying all visible leaf properties applies all
        // data only once and ensures that when an object reference is applied,
        // it's via its own property and not a parent property.
        if (property.prefabOverride && !property.hasVisibleChildren)
          ApplySingleProperty(property, prefabSourceSerializedObject, assetPath,
                              isObjectOnRootInAsset, false,
                              allowApplyDefaultOverride, serializedObjects,
                              action);
      }
    }

    // Ensure importing of saved Prefab Assets only kicks in after all Prefab
    // Asset have been saved
    AssetDatabase.StartAssetEditing();

    try {
      // Write modified value to prefab source object.
      for (int i = 0; i < serializedObjects.Count; i++) {
        if (serializedObjects [i]
                .ApplyModifiedProperties())
          SaveChangesToPrefabFileIfPersistent(serializedObjects[i]);

        if (action == InteractionMode.UserAction)
          Undo.FlushUndoRecordObjects(); // flush'es ensure that SavePrefab() on
                                         // undo/redo on the source happens in
                                         // the right order
      }
    } finally {
  ...
  static void
  SaveChangesToPrefabFileIfPersistent(SerializedObject serializedObject) {
  ...
  // Returns null if property is not part of array where whole array needs to be
  // appled.
  static SerializedProperty
  GetArrayPropertyIfGivenPropertyIsPartOfArrayElementInInstanceWhichDoesNotExistInAsset(
      SerializedProperty optionalSingleInstanceProperty,
      SerializedObject prefabSourceSerializedObject, InteractionMode action,
      out bool cancel) {
    cancel = false;
    ...
    int fullPropertyPathLength =
        optionalSingleInstanceProperty.propertyPath.Length;
    ...
    while (startSearchIndex < fullPropertyPathLength) {
      int arrayPropertySplitIndex =
          propertyPath.IndexOf(arrayElementIndexPrefix, startSearchIndex);
      ...
      string arrayPropertyPath =
          propertyPath.Substring(0, arrayPropertySplitIndex);

      // Find array element index start and length up front since we need it
      // either way.
      int arrayElementIndexStart =
          arrayPropertySplitIndex + arrayElementIndexPrefix.Length;
      int arrayElementIndexLength =
      ...
      SerializedProperty arrayPropertyOnInstance =
          optionalSingleInstanceProperty.serializedObject.FindProperty(
              arrayPropertyPath);
      SerializedProperty arrayPropertyOnAsset =
          prefabSourceSerializedObject.FindProperty(arrayPropertyPath);
      if (arrayPropertyOnInstance.arraySize > arrayPropertyOnAsset.arraySize) {
        // Array size mismatches. Check if the property the user is attempting
        // to apply resides in an element which exceeds the array in the Prefab
        // Asset.
        string arrayElementIndexString = propertyPath.Substring(
            arrayElementIndexStart, arrayElementIndexLength);
        int indexInArray;
        if (!int.TryParse(arrayElementIndexString, out indexInArray)) {
          // This should not be able to happen, but fallback by using array.
          Debug.LogError("Misformed arrayElementIndexString " +
                         arrayElementIndexString);
        ...
        if (indexInArray >= arrayPropertyOnAsset.arraySize) {
          if (action == InteractionMode.UserAction &&
              !EditorUtility.DisplayDialog(
                  ...
                  string.Format(
                      "The property is part of an array element which does not exist in the source Prefab because the corresponding array there is shorter. Do you want to apply the entire array '{0}'?",
                      arrayPropertyOnInstance.displayName),
                  "Apply Array", "Cancel")) {
            cancel = true;
            return null;
          }
      ...
      // Array does not mismatch, so look if there are other arrays (deeper in
      // property path) that do.
      startSearchIndex = arrayPropertySplitIndex +
                         arrayElementIndexPrefix.Length +
                         arrayElementIndexLength + 1;
    }
    return null;
  }
  ...
  static void ApplySingleProperty(SerializedProperty instanceProperty,
                                  SerializedObject prefabSourceSerializedObject,
                                  string assetPath, bool isObjectOnRootInAsset,
                                  bool singlePropertyOnly,
                                  bool allowApplyDefaultOverride,
                                  List<SerializedObject> serializedObjects,
                                  InteractionMode action) {
    if (!allowApplyDefaultOverride && isObjectOnRootInAsset &&
        IsPropertyOverrideDefaultOverrideComparedToAnySource(
            instanceProperty)) {
      if (singlePropertyOnly) {
        // Neither of these will not happen from our own editor interface since
        // we don't display any menus to apply for default-override properties
        // in the first place.
        if (action == InteractionMode.AutomatedAction)
          Debug.LogWarning(
              "Cannot apply default-override property, since it is protected from being applied or reverted.");
        else
          EditorUtility.DisplayDialog(
    ...
    SerializedProperty sourceProperty =
        prefabSourceSerializedObject.FindProperty(
            instanceProperty.propertyPath);
    if (sourceProperty == null) {
      bool cancel;
      var instanceArrayProperty =
          GetArrayPropertyIfGivenPropertyIsPartOfArrayElementInInstanceWhichDoesNotExistInAsset(
              instanceProperty, prefabSourceSerializedObject,
              InteractionMode.AutomatedAction, out cancel);
      if (instanceArrayProperty != null) {
        prefabSourceSerializedObject.CopyFromSerializedProperty(
            instanceArrayProperty);
        sourceProperty = prefabSourceSerializedObject.FindProperty(
            instanceProperty.propertyPath);
        if (sourceProperty == null) {
          Debug.LogError(
              $"ApplySingleProperty full array copy error: SerializedProperty could not be found for {instanceProperty.propertyPath}. Please report a bug.");
          return;
        }
      } else {
        Debug.LogError(
            $"ApplySingleProperty copy state error: SerializedProperty could not be found for {instanceProperty.propertyPath}. Please report a bug.");
        return;
      }
    }
    ...
    if (sourceProperty.propertyType == SerializedPropertyType.ObjectReference) {
      ...
      if (sourceProperty.objectReferenceValue != null &&
          !EditorUtility.IsPersistent(sourceProperty.objectReferenceValue)) {
        // The property is a reference to a non-persistent object (scene object
        // which could not be mapped to the asset. It can not be applied.

        // Give a warning if the user tried to specifically apply this property.
        if (singlePropertyOnly) {
          if (action == InteractionMode.AutomatedAction)
            Debug.LogWarning(
                "Cannot apply reference to scene object that is not part of apply target prefab.");
          else
            EditorUtility.DisplayDialog(
    ...
    // Apply target SerializedObject should get ApplyModifiedProperties called
    // first.
    if (serializedObjects.Count == 0)
      serializedObjects.Add(prefabSourceSerializedObject);

    // Clear overrides for property in Prefab instance and outer Prefabs that
    // are using(nesting) the Prefab source. Otherwise applied modification
    // would appear to jump back to the value it had before applying.
    Object prefabInstanceObject =
        instanceProperty.serializedObject.targetObject;
    ...
    while (outerPrefabObject != prefabSourceObject) {
      ...
      // Case 1172835: When applying a new array size to an inner Prefab, this
      // change won't yet have propogated to the outer Prefabs. This means
      // properties inside the array may not yet exist here. To handle this, we
      // first copy the serialized value (which correctly handles array size
      // changes) before we clear the overrides below (by setting
      ...
      if (propertyType == SerializedPropertyType.ArraySize)
        outerPrefabSerializedObject.CopyFromSerializedProperty(
            instanceProperty);

      SerializedProperty outerPrefabProp =
          outerPrefabSerializedObject.FindProperty(
              instanceProperty.propertyPath);
      ...
      if (outerPrefabProp == null)
        Debug.LogError(
            $"ApplySingleProperty clear overrides error: SerializedProperty could not be found for {instanceProperty.propertyPath}. Please report a bug.");

      outerPrefabObject =
          PrefabUtility.GetCorrespondingObjectFromSource(outerPrefabObject);
      sourceIndex++;
    }
  }
  ...
  public static void RevertPropertyOverride(SerializedProperty instanceProperty,
    ...
    // Because prefabOverride changed ApplyModifiedProperties will do a prefab
    // merge causing the revert.
    if (action == InteractionMode.UserAction)
  ...
  public static void ApplyObjectOverride(Object instanceComponentOrGameObject,
                                         string assetPath,
                                         InteractionMode action) {
    DateTime startTime = DateTime.UtcNow;

    ThrowExceptionIfNotValidPrefabInstanceObject(instanceComponentOrGameObject,
                                                 true);

    ApplyPropertyOverrides(instanceComponentOrGameObject, null, assetPath,
                           false, action);

    Analytics.SendApplyEvent(
        Analytics.ApplyScope.ObjectOverride, instanceComponentOrGameObject,
        assetPath, action, startTime,
        IsObjectOverrideAllDefaultOverridesComparedToAnySource(
            instanceComponentOrGameObject));
  }

  public static void RevertObjectOverride(Object instanceComponentOrGameObject,
                                          InteractionMode action) {
    ThrowExceptionIfNotValidPrefabInstanceObject(instanceComponentOrGameObject,
                                                 false);

    if (action == InteractionMode.UserAction)
      Undo.RegisterCompleteObjectUndo(instanceComponentOrGameObject,
                                      "Revert component property overrides");
    PrefabUtility.RevertObjectOverride_Internal(instanceComponentOrGameObject);
  }

  public static void ApplyAddedComponent(Component component, string assetPath,
                                         InteractionMode action) {
    DateTime startTime = DateTime.UtcNow;
    ...
    try {
      GameObject prefabSourceGameObject =
          GetCorrespondingObjectFromSourceAtPath(component.gameObject,
                                                 assetPath);
      ...
      if (action == InteractionMode.UserAction) {
        string dependentComponents = string.Join(
            ", ",
            GetAddedComponentDependencies(component, OverrideOperation.Apply)
                .Select(e => ObjectNames.GetInspectorTitle(e))
                .ToArray());
        if (!string.IsNullOrEmpty(dependentComponents)) {
          string error = String.Format(
              L10n.Tr(
                  "Can't apply added component {0} because it depends on {1}."),
              ObjectNames.GetInspectorTitle(component), dependentComponents);
          EditorUtility.DisplayDialog(L10n.Tr("Can't apply added component"),
                                      error, L10n.Tr("OK"));
          return;
        }

        Undo.RegisterFullObjectHierarchyUndo(prefabSourceGameObject,
                                             actionName);
      ...
      if (action == InteractionMode.UserAction)
        Undo.RegisterCreatedObjectUndo(
            GetCorrespondingObjectFromOriginalSource(component), actionName);
    } catch (ArgumentException exception) {
      if (action == InteractionMode.UserAction) {
        EditorUtility.DisplayDialog(L10n.Tr("Can't add component"),
                                    exception.Message, L10n.Tr("OK"));
    ...
    Analytics.SendApplyEvent(Analytics.ApplyScope.AddedComponent, component,
                             assetPath, action, startTime, false);
  }

  public static void RevertAddedComponent(Component component,
                                          InteractionMode action) {
    if (component == null)
      throw new ArgumentNullException(
          nameof(component),
    ...
    if (action == InteractionMode.UserAction) {
      string dependentComponents = string.Join(
          ", ",
          GetAddedComponentDependencies(component, OverrideOperation.Revert)
              .Select(e => ObjectNames.GetInspectorTitle(e))
              .ToArray());
      if (!string.IsNullOrEmpty(dependentComponents)) {
        string error = String.Format(
            L10n.Tr(
                "Can't revert added component {0} because {1} depends on it."),
            ObjectNames.GetInspectorTitle(component), dependentComponents);
        EditorUtility.DisplayDialog(L10n.Tr("Can't revert added component"),
                                    error, L10n.Tr("OK"));
        return;
      }
  ...
  private static void
  RemoveRemovedComponentOverridesWhichAreNull(Object prefabInstanceObject) {
    var removedComponents =
        PrefabUtility.GetRemovedComponents(prefabInstanceObject);
    var filteredRemovedComponents =
        (from c in removedComponents where c != null select c).ToArray();
    PrefabUtility.SetRemovedComponents(prefabInstanceObject,
                                       filteredRemovedComponents);
  }

  // We can't use the same pattern of identifyiong the prefab asset via
  // assetPath only, since when the component is removed in the instance, the
  // only way to identify which component it is, is via the corresponding
  // component on the asset. Additionally supplying an assetPath would be
  // redundant.
  public static void ApplyRemovedComponent(GameObject instanceGameObject,
    ...
    if (assetComponent == null)
      throw new ArgumentNullException(nameof(assetComponent),
    ...
    if (action == InteractionMode.UserAction) {
      string dependentComponents = string.Join(
          ", ", GetRemovedComponentDependencies(
                    assetComponent, instanceGameObject, OverrideOperation.Apply)
                    .Select(e => ObjectNames.GetInspectorTitle(e))
                    .ToArray());
      if (!string.IsNullOrEmpty(dependentComponents)) {
        string error = String.Format(
            L10n.Tr(
                "Can't apply removed component {0} because {1} component in the Prefab Asset depends on it."),
            ObjectNames.GetInspectorTitle(assetComponent), dependentComponents);
        EditorUtility.DisplayDialog(L10n.Tr("Can't apply removed component"),
                                    error, L10n.Tr("OK"));
        return;
      }
    ...
    var prefabInstanceObject =
        PrefabUtility.GetPrefabInstanceHandle(instanceGameObject);
    ...
    Analytics.SendApplyEvent(
        Analytics.ApplyScope.RemovedComponent, instanceGameObject,
        AssetDatabase.GetAssetPath(assetComponent), action, startTime, false);
  }

  private static void RemoveRemovedComponentOverride(Object instanceObject,
                                                     Component assetComponent) {
    ...
    if (index != -1) {
      var filteredRemovedComponents = (from c in removedComponents where c !=
                                       removedComponents[index] select c)
                                          .ToArray();
      PrefabUtility.SetRemovedComponents(instanceObject,
                                         filteredRemovedComponents);
    }
  }

  public static void RevertRemovedComponent(GameObject instanceGameObject,
                                            Component assetComponent,
                                            InteractionMode action) {
    ...
    if (action == InteractionMode.UserAction) {
      string dependentComponents = string.Join(
          ", ",
          GetRemovedComponentDependencies(assetComponent, instanceGameObject,
                                          OverrideOperation.Revert)
              .Select(e => ObjectNames.GetInspectorTitle(e))
              .ToArray());
      if (!string.IsNullOrEmpty(dependentComponents)) {
        string error = String.Format(
            L10n.Tr(
                "Can't revert removed component {0} because it depends on {1}."),
            ObjectNames.GetInspectorTitle(assetComponent), dependentComponents);
        EditorUtility.DisplayDialog(L10n.Tr("Can't revert removed component"),
                                    error, L10n.Tr("OK"));
        return;
      }
  ...
  public static void ApplyAddedGameObject(GameObject gameObject,
                                          string assetPath,
                                          InteractionMode action) {
    DateTime startTime = DateTime.UtcNow;
    ...
    if (gameObject == null)
      throw new ArgumentNullException(
          nameof(gameObject),
          "Cannot apply added GameObject. GameObject is null.");

    if (!IsAddedGameObjectOverride(gameObject))
      throw new ArgumentException(
          "Cannot apply added GameObject. GameObject is not an added GameObject override on a Prefab instance.",
          nameof(gameObject));
    ...
    if (instanceParent == null)
      return;
    GameObject prefabSourceGameObjectParent =
        GetCorrespondingObjectFromSourceAtPath(instanceParent.gameObject,
                                               assetPath);
    ...
    PrefabUtility.AddGameObjectsToPrefabAndConnect(
        new GameObject[]{gameObject}, prefabSourceGameObjectParent);

    PrefabUtility.SavePrefabAsset(sourceRoot);

    if (action == InteractionMode.UserAction) {
      var createdAssetObject = GetCorrespondingObjectFromSourceInAsset(
          gameObject, prefabSourceGameObjectParent);
    ...
    Analytics.SendApplyEvent(Analytics.ApplyScope.AddedGameObject, instanceRoot,
                             assetPath, action, startTime, false);
  }

  public static void RevertAddedGameObject(GameObject gameObject,
                                           InteractionMode action) {
    if (gameObject == null)
      throw new ArgumentNullException(
          nameof(gameObject),
  ...
  GetObjectOverrides(GameObject prefabInstance,
    ...
    return PrefabOverridesUtility.GetObjectOverrides(prefabInstance,
  ...
  internal static void HandleApplyRevertMenuItems(
      string thingThatChanged, Object instanceObject,
    ...
    HandleApplyMenuItems(thingThatChanged, instanceObject,
                         addApplyMenuItemAction,
                         defaultOverrideComparedToSomeSources);
    HandleRevertMenuItem(thingThatChanged, addRevertMenuItemAction);
  }

  internal static void
  HandleApplyMenuItems(string thingThatChanged, Object instanceOrAssetObject,
                       Action<GUIContent, Object> addApplyMenuItemAction,
                       bool defaultOverrideComparedToSomeSources = false,
    ...
    List<Object> applyTargets = GetApplyTargets(
        instanceOrAssetObject, defaultOverrideComparedToSomeSources,
        includeSelfAsTarget);
    ...
    for (int i = 0; i < applyTargets.Count; i++) {
      ...
      GUIContent applyContent = new GUIContent(
          thingThatChanged + String.Format(translatedText, sourceRoot.name));
      addApplyMenuItemAction(applyContent, source);
    }
  }

  internal static void
  HandleRevertMenuItem(string thingThatChanged,
                       Action<GUIContent> addRevertMenuItemAction) {
    // If thingThatChanged word is empty, apply menu items directly into menu.
    // Otherwise, insert as sub-menu named after thingThatChanged.
    if (thingThatChanged == null)
    ...
    GUIContent revertContent =
        new GUIContent(thingThatChanged + L10n.Tr("Revert"));
  ...
  public static string GetPrefabAssetPathOfNearestInstanceRoot(
      Object instanceComponentOrGameObject) {
    return AssetDatabase.GetAssetPath(
        GetOriginalSourceOrVariantRoot(instanceComponentOrGameObject));
  }
  ...
  [Obsolete(
      "The concept of creating a completely empty Prefab has been discontinued. You can however use SaveAsPrefabAsset with an empty GameObject.")]
  ...
  public static GameObject SavePrefabAsset(GameObject asset,
    ...
    // Include model check even though models are also immutable, since we can
    // give a more clear exception message.
  ...
  internal static void ValidatePath(GameObject instanceRoot, string path) {
    ...
    // We allow relative paths outside the Assets folder so we do not throw if
    // isValidAssetFolder is false
    bool isRootFolder = false;
    bool isImmutableFolder = false;
    bool isValidAssetFolder = AssetDatabase.GetAssetFolderInfo(
        directory, out isRootFolder, out isImmutableFolder);
    ...
    if (isValidAssetFolder) {
      string projectRelativePath = Path.IsPathRooted(path)
                                       ? FileUtil.GetProjectRelativePath(path)
                                       : path;
      string prefabGUID = AssetDatabase.AssetPathToGUID(projectRelativePath);
      if (!VerifyNestingFromScript(
              new GameObject[]{instanceRoot}, prefabGUID,
              PrefabUtility.GetPrefabInstanceHandle(instanceRoot)))
  ...
  private static void SaveAsPrefabAssetArgumentCheck(GameObject instanceRoot,
    ...
    if (EditorUtility.IsPersistent(instanceRoot))
      throw new ArgumentException(
          "Can't save persistent object as a Prefab asset");

    if (IsPrefabAssetMissing(instanceRoot))
      throw new ArgumentException(
          "Can't save Prefab instance with missing asset as a Prefab. You may unpack the instance and save the unpacked GameObjects as a Prefab.");
  ...
  public static GameObject SaveAsPrefabAsset(GameObject instanceRoot,
                                             string assetPath,
                                             out bool success) {
  ...
  public static GameObject SaveAsPrefabAssetAndConnect(GameObject instanceRoot,
                                                       string assetPath,
                                                       InteractionMode action) {
    bool success;
    return SaveAsPrefabAssetAndConnect(instanceRoot, assetPath, action,
                                       out success);
  }
  ...
  public static GameObject SaveAsPrefabAssetAndConnect(GameObject instanceRoot,
    ...
    if (action == InteractionMode.UserAction) {
      Undo.RegisterCreatedObjectUndo(GetPrefabInstanceHandle(instanceRoot),
                                     actionName);
    }
  ...
  internal static void ApplyPrefabInstance(GameObject instance) {
    ...
    if (IsDisconnectedFromPrefabAsset(instance)) {
      // The concept of disconnecting are being deprecated. For now use
      // FindRootGameObjectWithSameParentPrefab to re-connect existing
      ...
      if (!ok &&
        ...
        throw new ArgumentException(
            "Can't save Prefab from an object that originates from a nested Prefab");
    } else {
  ...
  // Can't use UnityUpgradable since it doesn't currently support swapping
  // parameter order.
  ...
  public static GameObject CreatePrefab(string path, GameObject go,
    ...
    if ((options & ReplacePrefabOptions.ConnectToPrefab) != 0)
      return SaveAsPrefabAssetAndConnect(go, path,
  ...
  public static Object InstantiatePrefab(Object assetComponentOrGameObject) {
    return InstantiatePrefab_internal(
        assetComponentOrGameObject,
        EditorSceneManager.GetTargetSceneForNewGameObjects(), null);
  }

  // Instantiates the given prefab in a given scene
  public static Object InstantiatePrefab(Object assetComponentOrGameObject,
                                         Scene destinationScene) {
    return InstantiatePrefab_internal(assetComponentOrGameObject,
  ...
  public static Object InstantiatePrefab(Object assetComponentOrGameObject,
    ...
    return InstantiatePrefab_internal(
        assetComponentOrGameObject,
        EditorSceneManager.GetTargetSceneForNewGameObjects(), parent);
  }
  ...
  [Obsolete(
      "Use SaveAsPrefabAsset or SaveAsPrefabAssetAndConnect with a path instead.")]
  public static GameObject ReplacePrefab(GameObject go, Object targetPrefab,
    ...
    if (!EditorUtility.IsPersistent(targetPrefabObject)) {
      Debug.LogError(
          "The Prefab you are trying to replace is not a Prefab Asset but a Prefab instance. Please use PrefabUtility.GetCorrespondingObject().",
          targetPrefab);
      return null;
    }

    if (HideFlags.DontSaveInEditor ==
        (HideFlags.DontSaveInEditor & go.hideFlags)) {
      Debug.LogError(
          "The root GameObject of the Prefab source cannot be marked with DontSaveInEditor as it would create an empty Prefab.",
          go);
      return null;
    }
    ...
    bool success = false;
    return SavePrefab_Internal(
        go, assetPath,
        (replaceOptions & ReplacePrefabOptions.ConnectToPrefab) != 0,
        out success);
  }

  // Returns the corresponding object from its immediate source from a connected
  // Prefab, or null if it can't be found, or the Prefab instance is
  // disconnected.
  internal static TObject GetCorrespondingConnectedObjectFromSource<TObject>(
      TObject componentOrGameObject) where TObject : Object {
    if (IsDisconnectedFromPrefabAsset(GetGameObject(componentOrGameObject)))
      return null;
    return (TObject)
        GetCorrespondingObjectFromSource_internal(componentOrGameObject);
  }

  // Returns the corresponding object from its immediate source, or null if it
  // can't be found.
  ...
  // Returns the corresponding object from its original source, or null if it
  // can't be found.
  public static TObject GetCorrespondingObjectFromOriginalSource<TObject>(
      TObject componentOrGameObject) where TObject : Object {
    return (TObject) GetCorrespondingObjectFromOriginalSource_Internal(
        componentOrGameObject);
  }

  internal static TObject GetCorrespondingObjectFromSourceInAsset<TObject>(
      TObject instanceOrAsset, Object prefabAssetHandle) where TObject
      : Object {
    return (TObject) GetCorrespondingObjectFromSourceInAsset_internal(
        instanceOrAsset, prefabAssetHandle);
  }

  public static TObject GetCorrespondingObjectFromSourceAtPath<TObject>(
      TObject componentOrGameObject, string assetPath) where TObject : Object {
    return (TObject) GetCorrespondingObjectFromSourceAtPath_internal(
        componentOrGameObject, assetPath);
  }
  ...
  GetCorrespondingObjectFromOriginalSource_Internal(Object instanceOrAsset) {
    ...
    if (!EditorUtility.IsPersistent(sourceObjectInPrefabAsset)) {
      sourceObjectInPrefabAsset =
          GetCorrespondingObjectFromSource(sourceObjectInPrefabAsset);
  ...
  public static PrefabType GetPrefabType(Object target) {
    ...
    if (IsDisconnectedFromPrefabAsset(target)) {
      ...
      // Object was at some point connected to a prefab, but now it is not
      // attached to one anymore and the prefab no longer exists
  ...
  public static bool IsAddedGameObjectOverride(GameObject gameObject) {
    ...
    // Can't be added to a prefab instance if the parent is not part of a prefab
    // instance.
    GameObject parentAsset =
        (GameObject)
    ...
    // If the root of that prefab, then it can't be part of the parent prefab,
    // and must be added. This check works regardless of whether this prefab
    // instance is an instance of the same prefab asset as the parent is an
    // instance of (e.g. instance of A is added under instance of A), or not
    // (instance of B is added under instance of A).
    return (asset.transform.parent == null);
  }

  // Called before the prefab is saved to hdd (called after
  // AssetModificationProcessor.OnWillSaveAssets and before
  ...
  internal enum SaveVerb { Save, Apply }

  internal static bool PromptAndCheckoutPrefabIfNeeded(string assetPath,
                                                       SaveVerb saveVerb) {
    return PromptAndCheckoutPrefabIfNeeded(new string[]{assetPath}, saveVerb);
  }

  internal static bool PromptAndCheckoutPrefabIfNeeded(string[] assetPaths,
                                                       SaveVerb saveVerb) {
    ...
    string prefabNoun =
        assetPaths.Length > 1 ? L10n.Tr("Prefabs") : L10n.Tr("Prefab");
    bool result = AssetDatabase.MakeEditable(
        assetPaths,
        string.Format(
            saveVerb ==
                SaveVerb.Save ? L10n.Tr(
                    "The version control requires you to check out the {0} before saving changes.")
            : L10n.Tr(
                  "The version control requires you to check out the {0} before applying changes."),
              prefabNoun));

    if (!result)
      EditorUtility.DisplayDialog(
          String.Format(saveVerb == SaveVerb.Save ? L10n.Tr("Could not save {0}")
                        : L10n.Tr("Could not apply to {0}"), prefabNoun),
          String.Format(
              saveVerb ==
                  SaveVerb.Save ? L10n.Tr(
                      "It was not possible to check out the {0} so the save operation has been canceled.")
              : L10n.Tr(
                    "It was not possible to check out the {0} so the apply operation has been canceled."),
                prefabNoun),
          L10n.Tr("OK"));
  ...
  public static void UnpackPrefabInstance(GameObject instanceRoot,
    ...
    if (action == InteractionMode.UserAction) {
      ...
      var newInstanceRoots = UnpackPrefabInstanceAndReturnNewOutermostRoots(
          instanceRoot, unpackMode);
      foreach (var newInstanceRoot in newInstanceRoots) {
        var prefabInstance =
            PrefabUtility.GetPrefabInstanceHandle(newInstanceRoot);
        if (prefabInstance) {
  ...
  internal static bool HasInvalidComponent(Object gameObjectOrComponent) {
    ...
    transformVisitor.VisitAll(go.transform,
                              PrefabOverridesUtility.CheckForInvalidComponent,
                              GOsWithInvalidComponent);
    return GOsWithInvalidComponent.Count > 0;
  }

  public static bool
  IsPartOfPrefabThatCanBeAppliedTo(Object gameObjectOrComponent) {
    if (gameObjectOrComponent == null)
      return false;
    ...
    if (!EditorUtility.IsPersistent(gameObjectOrComponent))
      gameObjectOrComponent =
          GetCorrespondingObjectFromSource(gameObjectOrComponent);
  ...
  public static PrefabInstanceStatus
  GetPrefabInstanceStatus(Object componentOrGameObject) {
  ...
  public static PrefabAssetType
  GetPrefabAssetType(Object componentOrGameObject) {
  ...
  public static GameObject LoadPrefabContents(string assetPath) {
    ...
    if (!File.Exists(assetPath))
      throw new ArgumentException(
          string.Format("Path: {0}, does not exist", assetPath));

    if (Path.GetExtension(assetPath) != ".prefab")
      throw new ArgumentException(
          string.Format("Path: {0}, is not a prefab file", assetPath));
    ...
    if (roots.Length != 1) {
      ...
      throw new ArgumentException(string.Format(
          "Could not load Prefab contents at path {0}. Prefabs should have exactly 1 root GameObject, {1} was found.",
          assetPath, roots.Length));
    }
  ...
  public static void UnloadPrefabContents(GameObject contentsRoot) {
    if (!EditorSceneManager.IsPreviewSceneObject(contentsRoot)) {
      throw new ArgumentException(
          "Specified object is not part of Prefab contents");
    }
    var scene = contentsRoot.scene;
    EditorSceneManager.ClosePreviewScene(scene);
  }

  // Since an override can be applied to multiple different apply targets, what
  // is not a default override compared to e.g. the outermost apply target may
  // still be a default override compared to e.g. the innermost. For example, if
  // you have child Prefab B inside Prefab A in the scene and try want to apply
  // the position of B, the position is not a default override if applying to A,
  // but is if applying to B. This can be used to determine which apply targets
  // are valid to apply to.
  internal static bool IsPropertyOverrideDefaultOverrideComparedToAnySource(
    ...
    // Only Transform, RectTransform (derived from Transform) and GameObject can
    // have default overrides.
    if (!(componentOrGameObject is Transform ||
          componentOrGameObject is GameObject))
      return false;
    ...
    SerializedObject innermostInstanceSO =
        new SerializedObject(innermostInstance);
    return innermostInstanceSO.FindProperty(property.propertyPath)
        .isDefaultOverride;
  }

  // Same as IsPropertyOverrideDefaultOverrideComparedToAnySource, but checks if
  // it's the case for all overrides on the object.
  internal static bool IsObjectOverrideAllDefaultOverridesComparedToAnySource(
    ...
    while (property.Next(true)) {
      if (property.prefabOverride) {
        if (!innermostInstanceSO.FindProperty(property.propertyPath)
                 .isDefaultOverride)
          return false;
  ...
  internal static bool IsObjectOnRootInAsset(Object componentOrGameObject,
                                             string assetPath) {
    GameObject go = GetGameObject(componentOrGameObject);
    GameObject goInAsset =
        GetCorrespondingObjectFromSourceAtPath(go, assetPath);
    if (goInAsset == null)
      return false;
  ...
  GetApplyTargets(Object instanceOrAssetObject,
    ...
    while (source != null) {
      if (defaultOverrideComparedToSomeSources) {
        // If we're dealing with an override that's a default override compared
        // to some sources, then we need to check if the source object is or is
        // on the root GameObject in that Prefab. If it is, the overrides will
        // be default overrides compared to that Prefab, and so it shouldn't be
        // possible to apply to it. Note that for changed components that have
        // some overrides that can be default overrides, and some not, we do
        // allow applying, but only the properties that are not default
  ...
  internal enum OverrideOperation { Apply, Revert }

  // Applying/reverting multiple overrides at once is surprisingly tricky
  // because we have to handle components with dependencies on other components
  // in just the right order.
  internal static bool ProcessMultipleOverrides(
      GameObject prefabInstanceRoot, List<PrefabOverride> overrides,
      PrefabUtility.OverrideOperation operation, InteractionMode mode) {
    ...
    List<PrefabOverride> acceptedRemovedComponentOverrides =
    ...
    // Iterate over overrides. Immediately accept any overrides with no
    // dependencies. Otherwise save dependencies in dictionary for quick lookup.
    for (int i = 0; i < overrides.Count; i++) {
      ...
      if (removedComponent != null) {
        var deps = PrefabUtility.GetRemovedComponentDependencies(
            ...
            removedComponent.containingInstanceGameObject, operation);
        if (deps.Count > 0) {
      ...
      if (addedComponent != null) {
        var deps = PrefabUtility.GetAddedComponentDependencies(
            addedComponent.instanceComponent, operation);
        if (deps.Count > 0) {
          overrideDependencies[singleOverride] = deps;
          hasDependencies = true;
        }
    ...
    // Iteratively accept overrides whose dependencies are all in the accepted
    // overrides. Theoretically this algorithm is worst case n*n. However, long
    // dependency chains are uncommon, so in practise it's much closer to just
    // n.
    while (true) {
    ...
    if (remainingOverrides.Count > 0) {
      ...
      foreach (var singleOverride in remainingOverrides) {
        ...
        foreach (var dep in dependencies) {
          // The dependency direction is different for apply versus revert AND
          // for added versus removed components.
          bool dependsOnOther =
              (singleOverride is AddedComponent) ^
              (operation == PrefabUtility.OverrideOperation.Revert);
          dependenciesString +=
              "\n" + string.Format(dependsOnOther ? L10n.Tr("{0} depends on {1}")
                                   : L10n.Tr("{0} is depended on by {1}"),
                                     ObjectNames.GetInspectorTitle(
                                         singleOverride.GetObject()),
      ...
      if (operation == PrefabUtility.OverrideOperation.Apply) {
        ...
        error =
            L10n.Tr(
                "Can't apply selected overrides due to dependencies with non-selected overrides:") +
            dependenciesString;
      } else {
        dialogTitle = L10n.Tr("Can't revert selected overrides");
        error =
            L10n.Tr(
                "Can't revert selected overrides due to dependencies with non-selected overrides.") +
            dependenciesString;
      }
      ...
      if (mode == InteractionMode.UserAction) {
        ...
        string prefabAssetPath =
            PrefabUtility.GetPrefabAssetPathOfNearestInstanceRoot(
                prefabInstanceRoot);
        if (!PrefabUtility.PromptAndCheckoutPrefabIfNeeded(
                prefabAssetPath, PrefabUtility.SaveVerb.Apply))
          return false;
      }

      // Apply overrides *in the order* they were added, but all removed
      // components first, so removed and added component of same type can't
      // cause GameObject to temporarily have two at once.
      for (int i = 0; i < acceptedRemovedComponentOverrides.Count; i++)
        acceptedRemovedComponentOverrides [i]
            .Apply(mode);
      for (int i = 0; i < acceptedOverrides.Count; i++)
        acceptedOverrides [i]
            .Apply(mode);
    } else {
      // Revert overrides *in the order* they were added, but all removed
      // components last, so removed and added component of same type can't
      // cause GameObject to temporarily have two at once.
      for (int i = 0; i < acceptedOverrides.Count; i++)
        acceptedOverrides [i]
            .Revert(mode);
  ...
  internal static List<Component>
  GetAddedComponentDependencies(Component component, OverrideOperation op) {
    ...
    if (op == OverrideOperation.Apply)
      // We can't apply an added component if it depends on a different added
      // component. This would violate a dependency in the asset.
      return GetComponentsWhichThisDependsOn(component, addedComponentsOnGO);
    else
      // We can't revert an added component if another added component depends
      // on it. This would violate a dependency in the instance.
      return GetComponentsWhichDependOnThis(component, addedComponentsOnGO);
  }

  internal static List<Component>
  GetRemovedComponentDependencies(Component assetComponent,
    ...
    List<Component> removedComponentsOnAssetGO =
        GetRemovedComponents(instanceGameObject)
            ...
            .ToList();
    if (op == OverrideOperation.Apply)
      // We can't apply a removed component if another removed component depends
      // on it. This would violate a dependency in the asset.
      return GetComponentsWhichDependOnThis(assetComponent,
                                            removedComponentsOnAssetGO);
    else
      // We can't revert a removed component if it depends on another removed
      // component. This would violate a dependency in the instance.
      return GetComponentsWhichThisDependsOn(assetComponent,
                                             removedComponentsOnAssetGO);
  }

  static List<Component>
  GetComponentsWhichDependOnThis(Component component,
                                 List<Component> componentsToConsider) {
    ...
    for (int i = 0; i < componentsToConsider.Count; i++) {
      ...
      var requiredComps =
          comp.GetType().GetCustomAttributes(typeof(RequireComponent), inherit
                                             : true);
      foreach (RequireComponent reqComp in requiredComps) {
        if (reqComp.m_Type0 == componentType ||
            reqComp.m_Type1 == componentType ||
            reqComp.m_Type2 == componentType) {
          // We might get the same component type requirement from multiple
          // sources. Make sure we don't add the same component more than once.
  ...
  static List<Component>
  GetComponentsWhichThisDependsOn(Component component,
                                  List<Component> componentsToConsider) {
    var requiredComps = component.GetType().GetCustomAttributes(
        typeof(RequireComponent), inherit
        : true);
    List<Component> dependencies = new List<Component>();
  ...
  internal static class Analytics {
    ...
    public enum ApplyTarget { OnlyTarget, Outermost, Innermost, Middle }
    ...
    public static void
    SendApplyEvent(ApplyScope applyScope, Object instance,
                   string applyTargetPath, InteractionMode interactionMode,
                   DateTime startTime,
                   bool defaultOverrideComparedToSomeSources) {
      ...
      var applyTargets =
          GetApplyTargets(instance, defaultOverrideComparedToSomeSources);
      ...
      UsabilityAnalytics.SendEvent("prefabApply", startTime, duration, true,
                                   eventData);
    }
  }
  ...
  static void OnPrefabSavingEnded(long ticks) {
    ...
    UsabilityAnalytics.SendEvent("prefabSave", saveStartTime, duration, true,
                                 null);
  }
