...
class PrefabOverridesTreeView : TreeView {
  ...
  class PrefabOverridesTreeViewItem : TreeViewItem {
    public PrefabOverridesTreeViewItem(int id, int depth, string displayName)
        : base(id, depth, displayName) {}

    public PrefabOverride singleModification;
    public Texture overlayIcon;

    public ToggleValue included {
      get;
      set;
    }
    public ItemType type {
      ...
      set {
        ...
        if (m_Type == ItemType.ADDED_OBJECT)
          overlayIcon =
              EditorGUIUtility.IconContent("PrefabOverlayAdded Icon").image;
        else if (m_Type == ItemType.REMOVED_OBJECT)
          overlayIcon =
              EditorGUIUtility.IconContent("PrefabOverlayRemoved Icon").image;
      }
    }
    ItemType m_Type;
    ...
    public Object obj {
      ...
      set {
        m_Obj = value;
        icon = (m_Obj is GameObject)
                   ? PrefabUtility.GetIconForGameObject((GameObject) m_Obj)
                   : AssetPreview.GetMiniThumbnail(m_Obj);
      }
    }
    Object m_Obj;
  ...
  class PrefabOverrides {
    ...
    public List<ObjectOverride> objectOverrides {
      get { return m_ObjectOverrides; }
      set { m_ObjectOverrides = value; }
    }
    public List<AddedComponent> addedComponents {
      get { return m_AddedComponents; }
      set { m_AddedComponents = value; }
    }
    public List<RemovedComponent> removedComponents {
      get { return m_RemovedComponents; }
      set { m_RemovedComponents = value; }
    }
    public List<AddedGameObject> addedGameObjects {
      get { return m_AddedGameObjects; }
      set { m_AddedGameObjects = value; }
    }
  }

  static PrefabOverrides
  GetPrefabOverrides(GameObject prefabInstance,
    ...
    mods.objectOverrides = PrefabOverridesUtility.GetObjectOverrides(
        prefabInstance, includeDefaultOverrides);
    mods.addedComponents =
        PrefabOverridesUtility.GetAddedComponents(prefabInstance);
    mods.removedComponents =
        PrefabOverridesUtility.GetRemovedComponents(prefabInstance);
    mods.addedGameObjects =
        PrefabOverridesUtility.GetAddedGameObjects(prefabInstance);
    return mods;
  }
  ...
  public PrefabOverridesTreeView(GameObject selectedGameObject,
                                 TreeViewState state,
                                 PrefabOverridesWindow window)
      : base(state) {
  ...
  public void SetApplyTarget(GameObject prefabInstanceRoot,
  ...
  void BuildPrefabOverridesPerObject(
      out Dictionary<int, PrefabOverrides> instanceIDToPrefabOverridesMap) {
    ...
    foreach (var modifiedObject in m_AllModifications.objectOverrides) {
      ...
      if (modifiedObject.instanceObject is GameObject)
        ...
        instanceID = ((Component) modifiedObject.instanceObject)
                         .gameObject.GetInstanceID();

      if (instanceID != 0) {
        PrefabOverrides modificationsForObject = GetPrefabOverridesForObject(
            instanceID, instanceIDToPrefabOverridesMap);
    ...
    foreach (var addedComponent in m_AllModifications.addedComponents) {
      ...
      if (addedComponent.instanceComponent == null)
        continue;
      int instanceID =
          addedComponent.instanceComponent.gameObject.GetInstanceID();
      PrefabOverrides modificationsForObject = GetPrefabOverridesForObject(
          instanceID, instanceIDToPrefabOverridesMap);
    ...
    foreach (var removedComponent in m_AllModifications.removedComponents) {
      ...
      if (removedComponent.assetComponent == null)
        continue;
      int instanceID = removedComponent.containingInstanceGameObject.gameObject
                           .GetInstanceID();
      PrefabOverrides modificationsForObject = GetPrefabOverridesForObject(
          instanceID, instanceIDToPrefabOverridesMap);
  ...
  protected override TreeViewItem BuildRoot() {
    Debug.Assert(m_PrefabInstanceRoot != null,
                 "We should always have a valid apply target");
    ...
    var hiddenRoot =
        new TreeViewItem{id = 0, depth = -1, displayName = "Hidden Root"};
    var idSequence = new IdSequence();
    hasModifications =
        AddTreeViewItemRecursive(hiddenRoot, m_PrefabInstanceRoot,
                                 instanceIDToPrefabOverridesMap, idSequence);
    if (!hasModifications)
      hiddenRoot.AddChild(
          new TreeViewItem{id = 1, depth = 0, displayName = "No Overrides"});
  ...
  void AddDebugItems(TreeViewItem hiddenRoot, IdSequence idSequence) {
    var debugItem = new TreeViewItem(idSequence.get(), 0,
                                     "<Debug raw list of modifications>");
    foreach (var mod in m_AllModifications.addedGameObjects)
      debugItem.AddChild(new TreeViewItem(idSequence.get(), debugItem.depth + 1,
                                          mod.instanceGameObject.name +
                                              " (Added GameObject)"));
    foreach (var mod in m_AllModifications.addedComponents)
      debugItem.AddChild(new TreeViewItem(idSequence.get(), debugItem.depth + 1,
                                          mod.instanceComponent.GetType() +
                                              " (Added Component)"));
    foreach (var mod in m_AllModifications.removedComponents)
      debugItem.AddChild(new TreeViewItem(idSequence.get(), debugItem.depth + 1,
                                          mod.assetComponent.GetType() +
                                              " (Removed Component)"));
  ...
  // Returns true if input gameobject or any of its descendants have
  // modifications, otherwise returns false.
  bool
  AddTreeViewItemRecursive(TreeViewItem parentItem, GameObject gameObject,
                           Dictionary<int, PrefabOverrides> prefabOverrideMap,
                           IdSequence idSequence) {
    var gameObjectItem = new PrefabOverridesTreeViewItem(
        gameObject.GetInstanceID(), parentItem.depth + 1, gameObject.name);
    ...
    if (objectModifications != null) {
      ...
      if (addedGameObjectData != null) {
        ...
        // Modified GameObject
        ObjectOverride modifiedGameObjectData =
            objectModifications.objectOverrides.Find(x => x.instanceObject ==
                                                          gameObject);
      ...
      foreach (var component in gameObject.GetComponents(typeof(Component))) {
        // GetComponents will return Missing Script components as null, we will
        // skip them here to prevent NullReferenceExceptions. (case 1197599)
        if (component == null)
          continue;

        // Skip coupled components (they are merged into the display of their
        // owning component)
        if (component.IsCoupledComponent())
          continue;

        var componentItem = new PrefabOverridesTreeViewItem(
            component.GetInstanceID(), gameObjectItem.depth + 1,
            ObjectNames.GetInspectorTitle(component));
        componentItem.obj = component;

        AddedComponent addedComponentData =
            objectModifications.addedComponents.Find(x => x.instanceComponent ==
                                                          component);
        if (addedComponentData != null) {
          ...
          ObjectOverride modifiedObjectData =
              objectModifications.objectOverrides.Find(x => x.instanceObject ==
                                                            component);
          ObjectOverride modifiedCoupledObjectData =
              (coupledComponent != null)
                  ? objectModifications.objectOverrides.Find(
                        x => x.instanceObject == coupledComponent)
                  : null;
      ...
      foreach (var removedComponent in objectModifications.removedComponents) {
        ...
        var removedComponentItem = new PrefabOverridesTreeViewItem(
            idSequence.get(), gameObjectItem.depth + 1,
    ...
    foreach (Transform childTransform in gameObject.transform) {
      ...
      shouldAddGameObjectItemToParent |= AddTreeViewItemRecursive(
          gameObjectItem, childGameObject, prefabOverrideMap, idSequence);
    }
  ...
  static PrefabOverrides
  GetPrefabOverridesForObject(int instanceID,
                              Dictionary<int, PrefabOverrides> map) {
  ...
  static void UpdateChildrenIncludedState(PrefabOverridesTreeViewItem item,
                                          bool v) {
  ...
  void DoPreviewPopup() {
    ...
    var item =
        FindItem(state.selectedIDs[0], rootItem) as PrefabOverridesTreeViewItem;
    ...
    PopupWindowWithoutFocus.Show(
        buttonRect,
        new ComparisonViewPopup(source, instance, item.singleModification,
                                this),
  ...
  public float maxItemWidth {
    get {
      return maxDepthItem !=
             null ? GetContentIndent(maxDepthItem) + k_FixedContentWidth : 0;
    }
  }

  const float k_FixedContentWidth = 150f;
  TreeViewItem maxDepthItem {
    get;
    set;
  }
  ...
  class ComparisonViewPopup : PopupWindowContent {
    ...
    static class Styles {
      ...
      public static GUIContent sourceContent =
          EditorGUIUtility.TrTextContent("Prefab Source");
      public static GUIContent instanceContent =
          EditorGUIUtility.TrTextContent("Override");
      public static GUIContent removedContent =
          EditorGUIUtility.TrTextContent("Removed");
      public static GUIContent addedContent =
          EditorGUIUtility.TrTextContent("Added");
      public static GUIContent noModificationsContent =
          EditorGUIUtility.TrTextContent("No Overrides");
      public static GUIContent applyContent =
    ...
    public ComparisonViewPopup(Object source, Object instance,
                               PrefabOverride modification,
                               PrefabOverridesTreeView owner) {
      m_Owner = owner;
      m_Source = source;
      m_Instance = instance;
      m_Modification = modification;
      if (modification != null) {
        m_Unappliable = !PrefabUtility.IsPartOfPrefabThatCanBeAppliedTo(
            modification.GetAssetObject());
      } else {
        m_Unappliable = false;
      }
    ...
    UndoPropertyModification[] RecheckOverrideStatus(
    ...
    public override void OnGUI(Rect rect) {
      ...
      if (scroll)
        ...
        // We overdraw border by one pixel to the right, so subtract here to
        // account for that.
        rect.width -= 1;
      ...
      if (m_Modification == null) {
        DrawHeader(new Rect(rect.x, rect.y, rect.width, k_HeaderHeight),
      ...
      Rect scrollRectPosition =
          new Rect(rect.x, rect.y + k_HeaderHeight,
      ...
      if (m_Source != null && m_Instance != null) {
        Rect sourceHeaderRect =
            new Rect(rect.x, rect.y, middleCol, k_HeaderHeight);
        Rect instanceHeaderRect =
            new Rect(rect.x + middleCol, rect.y,
                     rect.xMax - middleCol + (scroll ? k_ScrollbarWidth : 0),
                     k_HeaderHeight);
        ...
        m_Scroll =
            GUI.BeginScrollView(scrollRectPosition, m_Scroll, viewPosition);
        {
          var leftColumnHeight = DrawEditor(
              new Rect(0, 0, middleCol, m_PreviewSize.y), m_SourceEditor, true);

          EditorGUIUtility.comparisonViewMode =
              EditorGUIUtility.ComparisonViewMode.Modified;
          var rightColumnHeight = DrawEditor(
              new Rect(middleCol, 0, rect.xMax - middleCol, m_PreviewSize.y),
        ...
        EditorGUIUtility.comparisonViewMode =
            EditorGUIUtility.ComparisonViewMode.Modified;
        float columnHeight = DrawEditor(
            new Rect(0, 0, rect.width, m_PreviewSize.y), editor, disable);
    ...
    void DrawHeader(Rect rect, GUIContent label) {
      ...
      // Don't overdraw down, since overlapping scroll view can make controls
      // overlap divider line.
      GUI.Label(new Rect(rect.x, rect.y, rect.width + 1, rect.height),
                GUIContent.none, Styles.borderStyle);
    }
    ...
    void DrawRevertApplyButtons(Rect rect) {
      ...
      if (GUILayout.Button(Styles.revertContent, EditorStyles.miniButton,
                           GUILayout.Width(55))) {
      ...
      using(new EditorGUI.DisabledScope(m_Unappliable)) {
        Rect applyRect = GUILayoutUtility.GetRect(
            GUIContent.none, "MiniPulldown", GUILayout.Width(55));
        if (EditorGUI.DropdownButton(applyRect, Styles.applyContent,
                                     FocusType.Passive)) {
    ...
    void Apply(object prefabAssetPathObject) {
      ...
      EditorUtility
          .ForceRebuildInspectors(); // handles applying RemovedComponents
    ...
    float DrawEditor(Rect rect, Editor editor, bool disabled) {
      ...
      Rect editorRect = EditorGUILayout.BeginVertical();
      {
        using(new EditorGUI.DisabledScope(disabled)) {
          if (editor == null) {
            GUI.enabled = true;
            GUILayout.Label("None - this should not happen.",
      ...
      GUI.Label(new Rect(rect.x - 1, -1, rect.width + 2, m_PreviewSize.y + 2),
                GUIContent.none, Styles.borderStyle);
    ...
    public override Vector2 GetWindowSize() {
      return new Vector2(m_PreviewSize.x,
                         m_PreviewSize.y + k_HeaderHeight + 1f);
    }
  }
}
}
