...
internal class PrefabOverridesWindow : PopupWindowContent {
  ...
  static class Styles {
    public static GUIContent revertAllContent =
        EditorGUIUtility.TrTextContent("Revert All", "Revert all overrides.");
    public static GUIContent revertSelectedContent =
        EditorGUIUtility.TrTextContent("Revert Selected",
                                       "Revert selected overrides.");
    public static GUIContent applyAllContent = EditorGUIUtility.TrTextContent(
        "Apply All", "Apply all overrides to Prefab source '{0}'.");
    public static GUIContent applySelectedContent =
        EditorGUIUtility.TrTextContent(
            "Apply Selected",
            "Apply selected overrides to Prefab source '{0}'.");
    public static GUIContent applyAllToBaseContent =
        EditorGUIUtility.TrTextContent(
            "Apply All to Base",
            "Apply all overrides to base Prefab source '{0}'.");
    public static GUIContent applySelectedToBaseContent =
        EditorGUIUtility.TrTextContent(
            "Apply Selected to Base",
            "Apply selected overrides to base Prefab source '{0}'.");
    public static GUIContent instanceLabel =
        EditorGUIUtility.TrTextContent("Overrides to");
    public static GUIContent contextLabel =
        EditorGUIUtility.TrTextContent("in");

    public static GUIContent infoMultiple = EditorGUIUtility.TrTextContent(
        "Multiple Prefabs selected. Cannot show overrides.");
    public static GUIContent infoMultipleNoApply = EditorGUIUtility.TrTextContent(
    ...
    public static GUIContent infoModel = EditorGUIUtility.TrTextContent(
        "Click on individual items to review and revert.\nApplying to a Model Prefab is not possible.");
    public static GUIContent infoDefault = EditorGUIUtility.TrTextContent(
        "Click on individual items to review, revert and apply.");
    public static GUIContent infoNoApply = EditorGUIUtility.TrTextContent(
        "Click on individual items to review and revert.");
    public static GUIContent warningDisconnected =
        EditorGUIUtility.TrTextContent("Disconnected. Cannot show overrides.");

    // Messages related to reasons for inability to apply.
    public static GUIContent warningInvalidAsset = EditorGUIUtility.TrTextContent(
        "The Prefab file contains an invalid script. Applying is not possible. Enter Prefab Mode and remove or recover the script.");
    public static GUIContent warningInvalidInstance = EditorGUIUtility.TrTextContent(
        "The Prefab instance contains an invalid script. Applying is not possible. Remove or recover the script.");
    public static GUIContent warningImmutable = EditorGUIUtility.TrTextContent(
  ...
  internal PrefabOverridesWindow(GameObject selectedGameObject) {
    ...
    m_TreeView =
        new PrefabOverridesTreeView(selectedGameObject, m_TreeViewState, this);

    GameObject prefabAssetRoot =
        PrefabUtility.GetCorrespondingObjectFromSource(selectedGameObject);

    m_TreeView.SetApplyTarget(selectedGameObject, prefabAssetRoot,
                              AssetDatabase.GetAssetPath(prefabAssetRoot));
  ...
  void OnUndoRedoPerformed() { RefreshStatus(); }
  ...
  internal void RefreshStatus() {
    ...
    if (m_SelectedGameObjects.Length == 1)
      UpdateTextSingle(PrefabUtility.GetCorrespondingObjectFromSource(
          m_SelectedGameObjects[0]));
    else
      UpdateTextMultiple();
    ...
    // There are a few cases where the Tree View reports no overrides even
    // though PrefabUtility.HasPrefabInstanceAnyOverrides says there are; for
    // example if a component has been removed on an instance, but also removed
    // on the Asset. In these cases we want to make the UI not show apply/revert
    // buttons, since it's confusing and inconsistent to have those when the
    // view says "No overrides". Case 1197800.
    if (m_TreeView != null && !m_TreeView.hasModifications)
      m_AnyOverrides = false;
  }

  void UpdateStatusChecks(GameObject prefabInstanceRoot) {
    // Can't reset values inside this method, since it's called consecutively
    // for each target.
    ...
    GameObject prefabAssetRoot =
        PrefabUtility.GetCorrespondingObjectFromSource(prefabInstanceRoot);
  ...
  bool IsShowingActionButton() { return m_AnyOverrides || m_Disconnected; }

  bool HasMultiSelection() { return m_SelectedGameObjects.Length > 1; }

  bool DisplayingTreeView() { return m_AnyOverrides && !HasMultiSelection(); }

  bool IsShowingApplyWarning() {
    return !HasMultiSelection() && (m_AnyOverrides || m_Disconnected) &&
           !m_ModelPrefab &&
  ...
  public override Vector2 GetWindowSize() {
    ...
    if (!IsShowingActionButton()) {
      ...
      if (DisplayingTreeView()) {
        height +=
            k_TreeViewPadding.top +
            Mathf.Min(k_MaxAllowedTreeViewHeight, m_TreeView.totalHeight) +
            k_TreeViewPadding.bottom;
        width = Mathf.Max(
            Mathf.Min(m_TreeView.maxItemWidth, k_MaxAllowedTreeViewWidth),
            width);
      }
    ...
    // Width should be no smaller than minimum width, but we could potentially
    // improve width handling by making it expand if needed based on tree view
    // content.
  ...
  Color headerBgColor {
    get {
      return EditorGUIUtility.isProSkin ? new Color(0.5f, 0.5f, 0.5f, 0.2f)
  ...
  public override void OnGUI(Rect rect) {
    ...
    Rect headerRect =
        GUILayoutUtility.GetRect(20, 10000, k_HeaderHeight, k_HeaderHeight);
    ...
    Rect labelRect = new Rect(headerRect.x + k_HeaderLeftMargin, headerRect.y,
                              labelSize, headerRect.height);
    ...
    // If we know there are no overrides and thus no meaningful actions we just
    // show that and nothing more.
    if (!IsShowingActionButton()) {
    ...
    if (HasMultiSelection()) {
      if (m_InvalidComponentOnAsset || m_InvalidComponentOnInstance ||
          m_ModelPrefab || m_Immutable)
        EditorGUILayout.HelpBox(Styles.infoMultipleNoApply.text,
                                MessageType.Info);
      else
        EditorGUILayout.HelpBox(Styles.infoMultiple.text, MessageType.Info);
    } else {
      if (m_Disconnected) {
        EditorGUILayout.HelpBox(Styles.warningDisconnected.text,
                                MessageType.Warning);
      } else if (m_AnyOverrides) {
        Rect treeViewRect = GUILayoutUtility.GetRect(100, 10000, 0, 10000);
        m_TreeView.OnGUI(treeViewRect);

        // Display info message telling user they can click on individual items
      ...
      if (IsShowingApplyWarning()) {
        ...
        // Model Prefabs are not an edge case and not needed to warn about so
        // it's not included here but rather combined into the info message
        // above.
        if (m_InvalidComponentOnAsset)
          EditorGUILayout.HelpBox(Styles.warningInvalidAsset.text,
                                  MessageType.Warning);
        else if (m_InvalidComponentOnInstance)
          EditorGUILayout.HelpBox(Styles.warningInvalidInstance.text,
                                  MessageType.Warning);
        else
          EditorGUILayout.HelpBox(Styles.warningImmutable.text,
                                  MessageType.Warning);
      }
    }
    ...
    using(new EditorGUI.DisabledScope(m_InvalidComponentOnAsset)) {
      if (m_TreeView != null && m_TreeView.GetSelection().Count > 1) {
        if (GUILayout.Button(m_RevertSelectedContent,
                             GUILayout.Width(m_ButtonWidth))) {
          if (OperateSelectedOverrides(
                  PrefabUtility.OverrideOperation.Revert)) {
            RefreshStatus();
            // We don't close the window even if there are no more overrides
            // left. We want to diplay explicit confirmation, since it's not a
            // given outcome when using Revert Selected. Only Revert All button
            // closes the window.
          }
        }

        using(new EditorGUI.DisabledScope(m_Immutable ||
                                          m_InvalidComponentOnInstance)) {
          if (GUILayout.Button(m_ApplySelectedContent,
                               GUILayout.Width(m_ButtonWidth))) {
            if (OperateSelectedOverrides(
                    PrefabUtility.OverrideOperation.Apply)) {
              RefreshStatus();
              // We don't close the window even if there are no more overrides
              // left. We want to diplay explicit confirmation, since it's not a
              // given outcome when using Apply Selected. Only Apply All button
              // closes the window.
            }
          }
        }
      } else {
        if (GUILayout.Button(m_RevertAllContent,
                             GUILayout.Width(m_ButtonWidth))) {
        ...
        using(new EditorGUI.DisabledScope(m_Immutable ||
                                          m_InvalidComponentOnInstance)) {
          if (GUILayout.Button(m_ApplyAllContent,
                               GUILayout.Width(m_ButtonWidth))) {
  ...
  bool ApplyAll() {
    // Collect Prefab Asset paths and also check if there's more than one of the
    // same.
    ...
    if (!PrefabUtility.PromptAndCheckoutPrefabIfNeeded(
            prefabAssetPaths.ToArray(), PrefabUtility.SaveVerb.Apply))
      return false;
    ...
    for (var i = 0; i < m_SelectedGameObjects.Length; i++) {
      ...
      PrefabUtility.GetObjectListFromHierarchy(us.prefabHierarchy,
                                               us.correspondingSourceObject);
      undoStructs.Add(us);
    }
    ...
    foreach (var t in undoStructs) {
      PrefabUtility.RegisterNewObjects(t.correspondingSourceObject,
                                       t.prefabHierarchy, actionName);
    }
  ...
  bool RevertAll() {
    for (int i = 0; i < m_SelectedGameObjects.Length; i++)
      PrefabUtility.RevertPrefabInstance(m_SelectedGameObjects[i],
  ...
  bool OperateSelectedOverrides(PrefabUtility.OverrideOperation operation) {
    ...
    // Get all overrides from selection. Immediately accept any overrides with
    // no dependencies.
    ...
    bool success = PrefabUtility.ProcessMultipleOverrides(
        m_SelectedGameObjects[0], overrides, operation,
        InteractionMode.UserAction);
    if (success)
      EditorUtility.ForceRebuildInspectors();
    return success;
  ...
  void UpdateTextSingle(GameObject prefabAsset) {
    Texture2D icon = (Texture2D) AssetDatabase.GetCachedIcon(
  ...
  void UpdateText(Texture assetIcon, string assetName) {
    ...
    if (stage is MainStage) {
      m_StageContent.image =
          EditorGUIUtility.IconContent("SceneAsset Icon").image;
      m_StageContent.text = "Scene";
    } else if (stage is PrefabStage) {
      m_StageContent.image =
          (Texture2D) AssetDatabase.GetCachedIcon(stage.assetPath);
      m_StageContent.text =
          System.IO.Path.GetFileNameWithoutExtension(stage.assetPath);
    }
    ...
    if (stage is PrefabStage &&
        PrefabUtility.IsPartOfVariantPrefab(
            AssetDatabase.LoadAssetAtPath<Object>(stage.assetPath))) {
    ...
    m_ApplyAllContent.tooltip =
        string.Format(applyAllContent.tooltip, assetName);
    m_ApplySelectedContent.text = applySelectedContent.text;
    m_ApplySelectedContent.tooltip =
        string.Format(applySelectedContent.tooltip, assetName);
  }
}
}
