...
internal class PrefabOverridesUtility
{
        ...
        foreach (var assetComponent in s_AssetComponentList)
        {
            ...
            if (!found)
            {
                ...
                removedComponents.Add(new RemovedComponent() {
                    assetComponent = assetComponent, containingInstanceGameObject = instanceGameObject
                });
            }
        }
        return true;
    ...
    static bool CheckForAddedGameObjectAndIfSoAddItAndReturnFalse(Transform transform, object userData)
    {
        ...
        if (IsAddedGameObject(addedGameObjectUserData.contextGameObject, transform.gameObject))
        {
            ...
            addedGameObjects.Add(new AddedGameObject() {
                instanceGameObject = transform.gameObject, siblingIndex = transform.GetSiblingIndex()
            });
            return false;
        }
        return true;
