...
namespace UnityEditor.SceneManagement {
  internal class PrefabOverridesUtility {
    ...
    static void
    ThrowExceptionIfNullOrNotPartOfPrefabInstance(GameObject prefabInstance) {
    ...
    GetObjectOverrides(GameObject prefabInstance,
      ...
      var prefabInstanceRoot =
          PrefabUtility.GetOutermostPrefabInstanceRoot(prefabInstance);

      // From root of instance traverse all child go and detect any GameObjects
      // or components that are not part of that source prefab objects component
      // list (these must be added)
      ...
      transformVisitor.VisitPrefabInstanceTransforms(
          prefabInstanceRoot.transform, checkMethod, modifiedObjects);
      return modifiedObjects;
    }

    static bool
    CheckForModifiedObjectsIncludingDefaultOverrides(Transform transform,
                                                     object userData) {
    ...
    static bool
    CheckForModifiedObjectsExcludingDefaultOverrides(Transform transform,
                                                     object userData) {
      var modifiedObjects = (List<ObjectOverride>) userData;
      return CheckForModifiedObjects(transform, modifiedObjects, false);
    }

    static bool CheckForModifiedObjects(Transform transform,
                                        List<ObjectOverride> modifiedObjects,
    ...
    GetAddedComponents(GameObject prefabInstance) {
      ...
      // From root of instance traverse all child go and detect any components
      // that are not part of that source prefab objects component list (these
      // must be added)
      TransformVisitor transformVisitor = new TransformVisitor();
      ...
      transformVisitor.VisitPrefabInstanceTransforms(
          prefabInstanceRoot.transform, CheckForAddedComponents,
          addedComponents);
      return addedComponents;
    }

    // Return value indicates if caller should traverse children of the the
    // input transform or not
    static bool CheckForAddedComponents(Transform transform, object userData) {
      ...
      var assetGameObject =
          PrefabUtility.GetCorrespondingObjectFromSource(transform.gameObject);
      if (assetGameObject == null)
        return false; // If this is an added normal GameObject then we do not
                      // record added components
      ...
      foreach (var component in s_ComponentList) {
        ...
        if (isAddedObject) {
          ...
          addedComponents.Add(
              new AddedComponent(){instanceComponent = component});
        }
      }
    ...
    GetRemovedComponents(GameObject prefabInstance) {
      ...
      // From root of asset traverse all children and detect any Components that
      // are not present on the instance object (these must be deleted)
      var removedComponents = new List<RemovedComponent>();
      if (PrefabUtility.IsDisconnectedFromPrefabAsset(prefabInstance))
        return removedComponents;
      TransformVisitor transformVisitor = new TransformVisitor();
      transformVisitor.VisitPrefabInstanceTransforms(
          prefabInstanceRoot.transform, CheckForRemovedComponents,
          removedComponents);
      return removedComponents;
    }

    public static List<RemovedComponent>
    GetRemovedComponentsForSingleGameObject(GameObject prefabInstance) {
      ThrowExceptionIfNullOrNotPartOfPrefabInstance(prefabInstance);
    ...
    // Return value indicates if caller should traverse children of the the
    // input transform or not
    static bool CheckForRemovedComponents(Transform transform,
                                          object userData) {
      GameObject instanceGameObject = transform.gameObject;
      GameObject assetGameObject =
          PrefabUtility.GetCorrespondingObjectFromSource(instanceGameObject);
      if (assetGameObject == null)
        return false; // skip added GameObjects (non of its components will be
                      // in the asset)
      ...
      foreach (var assetComponent in s_AssetComponentList) {
        ...
        foreach (var instanceComponent in s_ComponentList) {
          ...
          if (PrefabUtility.GetCorrespondingObjectFromSource(
                  instanceComponent) == assetComponent) {
            found = true;
            break;
          }
        }
        ...
        if (!found) {
          List<RemovedComponent> removedComponents =
              (List<RemovedComponent>) userData;
          removedComponents.Add(new RemovedComponent(){
              assetComponent = assetComponent,
              containingInstanceGameObject = instanceGameObject});
        }
      }
      return true;
    }
    ...
    GetAddedGameObjects(GameObject prefabInstance) {
      ...
      // From root instance traverse all children and detect any GameObjects
      // that are not a prefab gameobject (these must be added)
      TransformVisitor transformVisitor = new TransformVisitor();
      ...
      transformVisitor.VisitAndConditionallyEnterChildren(
          ...
          new AddedGameObjectUserData(){addedGameObjects = addedGameObjects,
                                        contextGameObject =
                                            prefabInstanceRoot});
      return addedGameObjects;
    }
    ...
    static bool
    CheckForAddedGameObjectAndIfSoAddItAndReturnFalse(Transform transform,
                                                      object userData) {
      // Don't list DontSave objects or their children as they won't get applied
      // or reverted.
      ...
      if (IsAddedGameObject(addedGameObjectUserData.contextGameObject,
                            transform.gameObject)) {
        var addedGameObjects = addedGameObjectUserData.addedGameObjects;
        addedGameObjects.Add(
            new AddedGameObject(){instanceGameObject = transform.gameObject,
    ...
    internal static bool IsAddedGameObject(GameObject contextGameObject,
                                           GameObject gameObject) {
      // The context GameObject itself can never have status as being added to
      // itself.
      ...
      // It is if its parent belong to the same Prefab as the context
      // GameObject.
      return (PrefabUtility
                  .GetCorrespondingObjectFromSource(gameObject.transform.parent)
                  .root ==
              PrefabUtility
                  .GetCorrespondingObjectFromSource(contextGameObject.transform)
                  .root);
    }

    internal static void CheckForInvalidComponent(Transform transform,
                                                  object userData) {
      GameObject instanceGameObject = transform.gameObject;
