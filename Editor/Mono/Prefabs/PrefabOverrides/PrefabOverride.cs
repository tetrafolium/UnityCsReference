...
namespace UnityEditor.SceneManagement {
  public abstract class PrefabOverride {
    ...
    public void Revert() { Revert(InteractionMode.UserAction); }
    ...
    protected UnityObject FindApplyTargetAssetObject(string prefabAssetPath) {
      ...
      while (assetObject != null) {
        ...
        if (assetPath == prefabAssetPath)
          return assetObject;
        assetObject =
            PrefabUtility.GetCorrespondingObjectFromSource(assetObject);
      }
      return null;
    }
    ...
    // For ObjectOverride, it's the object on the instance that has the
    // overrides. For AddedComponent, it's the added component on the instance.
    // For AddedGameObject, it's the added GameObject on the instance.
    // For RemovedComponent, it's the component on the Prefab Asset
    // corresponding to the removed component on the instance. For
    // RemovedGameObject, it's the GameObject on the Prefab Asset corresponding
    // to the removed GameObject on the instance.
    internal abstract UnityObject GetObject();

    internal void HandleApplyMenuItems(GenericMenu menu,
                                       GenericMenu.MenuFunction2 applyAction) {
      PrefabUtility.HandleApplyMenuItems(
          null, GetAssetObject(), (menuItemContent, sourceObject) => {
            ...
            if (!PrefabUtility.IsPartOfPrefabThatCanBeAppliedTo(rootObject))
              ...
              menu.AddItem(menuItemContent, false, applyAction,
                           prefabAssetPath);
          }, false, true);
    }
  }
  ...
  public class ObjectOverride : PrefabOverride {
    public UnityObject instanceObject {
      get;
      set;
    }
    public PrefabOverride coupledOverride {
      get;
      set;
    }

    public override void Apply(string prefabAssetPath, InteractionMode mode) {
      PrefabUtility.ApplyObjectOverride(instanceObject, prefabAssetPath, mode);
    }

    public override void Revert(InteractionMode mode) {
      PrefabUtility.RevertObjectOverride(instanceObject, mode);
    ...
    internal override UnityObject GetObject() { return instanceObject; }
  }
  ...
  public class AddedComponent : PrefabOverride {
    ...
    public override void Apply(string prefabAssetPath, InteractionMode mode) {
      PrefabUtility.ApplyAddedComponent(instanceComponent, prefabAssetPath,
                                        mode);

      var coupledComponent = instanceComponent.GetCoupledComponent();
      if (coupledComponent != null) {
        PrefabUtility.ApplyAddedComponent(coupledComponent, prefabAssetPath,
    ...
    public override void Revert(InteractionMode mode) {
      ...
      PrefabUtility.RevertAddedComponent(instanceComponent, mode);

      if (coupledComponent != null) {
        PrefabUtility.RevertAddedComponent(coupledComponent, mode);
      }
    }
    ...
    internal override UnityObject GetObject() { return instanceComponent; }
  }

  public class RemovedComponent : PrefabOverride {
    public GameObject containingInstanceGameObject {
      get;
      set;
    }
    ...
    public override void Apply(string prefabAssetPath, InteractionMode mode) {
      PrefabUtility.ApplyRemovedComponent(
          ...
          (Component) FindApplyTargetAssetObject(prefabAssetPath), mode);
    }

    public override void Revert(InteractionMode mode) {
      PrefabUtility.RevertRemovedComponent(containingInstanceGameObject,
                                           assetComponent, mode);

      var coupledComponent = assetComponent.GetCoupledComponent();
      if (coupledComponent != null) {
        PrefabUtility.RevertRemovedComponent(containingInstanceGameObject,
                                             coupledComponent, mode);
      }
    }

    public override UnityObject GetAssetObject() { return assetComponent; }

    internal override UnityObject GetObject() { return assetComponent; }
  }
  ...
  public class AddedGameObject : PrefabOverride {
    ...
    public override void Apply(string prefabAssetPath, InteractionMode mode) {
      PrefabUtility.ApplyAddedGameObject(instanceGameObject, prefabAssetPath,
                                         mode);
    }

    public override void Revert(InteractionMode mode) {
      PrefabUtility.RevertAddedGameObject(instanceGameObject, mode);
    }
    ...
    internal override UnityObject GetObject() { return instanceGameObject; }
  }
}
