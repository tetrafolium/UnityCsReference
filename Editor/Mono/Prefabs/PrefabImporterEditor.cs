...
internal class PrefabImporterEditor : AssetImporterEditor {
  static class Styles {
    public static GUIContent openContent =
        EditorGUIUtility.TrTextContent("Open Prefab");
    public static GUIContent openHelpText =
        EditorGUIUtility.TrTextContent("Open Prefab for full editing support.");
    public static GUIContent missingScriptsHelpText =
        EditorGUIUtility.TrTextContent(
            "Prefab has missing scripts. Open Prefab to fix the issue.");
    public static GUIContent multiSelectionMissingScriptsHelpText =
        EditorGUIUtility.TrTextContent(
            "Some of the selected Prefabs have missing scripts and needs to be fixed before editing them. Click to Open Prefab to fix the issue.");
    public static GUIContent savingFailedHelpText = EditorGUIUtility.TrTextContent(
        "Saving has failed. Check the Console window to get more insight into what needs to be fixed on the Prefab Asset.\n\nOpen Prefab to fix the issue.");
    public static GUIContent baseContent =
        EditorGUIUtility.TrTextContent("Base");
    public static string localizedTitleMultiplePrefabs =
        L10n.Tr("Prefab Assets");
  ...
  bool readyToAutoSave {
    get {
      return !m_SavingHasFailed && !hasMissingScripts &&
  ...
  protected override void Awake() {
    ...
    foreach (var prefabAssetRoot in assetTargets) {
      if (PrefabUtility.HasInvalidComponent(prefabAssetRoot)) {
        m_PrefabsWithMissingScript.Add(
  ...
  internal void SaveDirtyPrefabAssets(bool rebuildInspectors) {
    ...
    foreach (var asset in assetTargets) {
      // The asset could have been deleted when this method is called from
      // OnDestroy(). E.g delete the selected prefab asset from the Project
      // Browser.
      if (asset == null)
        continue;
      ...
      try {
        foreach (var rootGameObject in m_DirtyPrefabAssets) {
          var guid = AssetDatabase.AssetPathToGUID(
              AssetDatabase.GetAssetPath(rootGameObject));
          ...
          if (!savedSuccesfully) {
            ...
            string message = L10n.Tr(
                "Check the Console window to get more insight into what needs to be fixed on the Prefab Asset.\n\nYou can open Prefab Mode to fix any issues on child GameObjects");
          ...
          // Fix case 1239807: Prevent calling ForceRebuildInspectors() if the
          // the user is dirtying the prefab asset on every
          // CustomEditor::OnEnable() with a value that is the same as before
          // (so the artifact does not change). This fix avoids constant
          // rebuilding of the Inspector window.
          sourceFileChangedAfterSaving |=
              AssetDatabase.GetSourceAssetFileHash(guid) != hashBeforeSaving;
        }
      } finally {
        AssetDatabase.StopAssetEditing();

        // All inspectors needs to be rebuild to ensure property changes are
        // reflected after saving the Prefab shown. (Saving clears the
        // m_DirtyIndex of the target which is used for updating inspectors via
        // SerializedObject::UpdateIfRequiredOrScript() and thus the cached
        // dirty index in SerializedObject is not updated meaning the source
        // object is not reloaded even though it changed)
  ...
  bool IsDirty(GameObject prefabAssetRoot) {
    ...
    // For Prefab Variant Asset we need to also check if the instance handle is
    // dirty since this happens when the list of removed component changes
  ...
  void CacheHasMixedBaseVariants() {
    ...
    var firstBaseVarient =
        PrefabUtility.GetCorrespondingObjectFromSource(assetTarget);
  ...
  internal override string targetTitle {
    get {
      if (assetTargets == null || assetTargets.Length == 1 ||
          !m_AllowMultiObjectAccess)
        return assetTarget != null ? assetTarget.name + " (" +
                                  Styles.localizedTitleSinglePrefab +
                                  ")" : Styles.localizedTitleSinglePrefab;
      else
  ...
  public override void OnInspectorGUI() {
    ...
    // For things with explicit save operations (scenes, prefabs) we allow
    // editing and handle the potential version control conflict at the time
    // when the user saves.
    ...
    using(new EditorGUI.DisabledScope(assetTargets.Length > 1)) {
      ...
      if (GUILayout.Button(Styles.openContent, Styles.openButtonStyle)) {
        // We only support opening one prefab at a time (so do not use
        // 'targets')
        PrefabStageUtility.OpenPrefab(
            AssetDatabase.GetAssetPath(assetTarget), null,
      ...
      if (!hasMissingScripts && !m_SavingHasFailed) {
        EditorGUILayout.HelpBox(Styles.openHelpText.text, MessageType.Info,
                                true);
      }
    }
    ...
    if (hasMissingScripts) {
      if (assetTargets.Length > 1) {
        // List all assets that have missing scripts (but only if we have a
        // multi-selection)
        GUILayout.Space(5);
        EditorGUILayout.HelpBox(
            Styles.multiSelectionMissingScriptsHelpText.text,
        ...
        EditorGUILayout.HelpBox(Styles.missingScriptsHelpText.text,
                                MessageType.Warning, true);
      }
    } else if (m_SavingHasFailed) {
      EditorGUILayout.HelpBox(Styles.savingFailedHelpText.text,
                              MessageType.Warning, true);
    }
