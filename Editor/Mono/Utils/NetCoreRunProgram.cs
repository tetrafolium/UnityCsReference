...
namespace UnityEditor.Scripting {
  internal class NetCoreRunProgram : Program {
    ...
    static NetCoreRunProgram() {
      netcoreRunPath = Paths.Combine(EditorApplication.applicationContentsPath,
                                     "Tools", "netcorerun", "netcorerun");
    ...
    public NetCoreRunProgram(string executable, string arguments,
      ...
      if (!IsSupported()) {
        throw new NotSupportedException(
            "NetCoreRunProgram is not supported on this platform");
      }
    ...
    public static bool IsSupported() {
      ...
      else if (SystemInfo.operatingSystem.StartsWith(
                   "Mac OS X 10.", StringComparison.CurrentCulture)) {
