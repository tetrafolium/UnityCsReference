...
namespace UnityEditor.Utils {
  internal class PerformanceChecks {
    ...
    private static string FormattedTextContent(string localeString,
                                               params object[] args) {
    ...
    public static string CheckMaterial(Material mat, BuildTarget buildTarget) {
      ...
      bool hasMobileVariant =
          Array.Exists(kShadersWithMobileVariants, s => s == shaderName);
      bool isMobileTarget = BuildTargetDiscovery.PlatformHasFlag(
          buildTarget, TargetAttributes.HasIntegratedGPU);

      // Skip all performance-related checks if shader explicitly indicated that
      // via a PerformanceChecks=False tag.
      bool noPerfChecks =
          (mat.GetTag("PerformanceChecks", true).ToLower() == "false");
      if (!noPerfChecks) {
        ...
        if (hasMobileVariant) {
          ...
          if (isMobileTarget && shaderName.StartsWith("Particles/")) {
            return FormattedTextContent(
                "Consider using {0} shader on this platform for performance.",
                "Mobile/" + shaderName);
          }
        ...
        if (shaderLOD >= 300 && isMobileTarget &&
            !shaderName.StartsWith("Mobile/")) {
          return FormattedTextContent(
              "Shader might be expensive on this platform. Consider switching to a simpler shader; look under Mobile shaders.");
        }
        ...
        // normalmapped shader without a normalmap: recommend non-normal mapped
        // one
