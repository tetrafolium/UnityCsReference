...
internal class AssemblyReferenceChecker {
  ...
  private readonly HashSet<string> _userReferencedMethods =
      new HashSet<string>();
  private readonly HashSet<string> _definedMethods = new HashSet<string>();
  private HashSet<AssemblyDefinition> _assemblyDefinitions =
  ...
  public static AssemblyReferenceChecker
  AssemblyReferenceCheckerWithUpdateProgressAction(Action action) {
  ...
  private void CollectReferencesFromRootsRecursive(string dir,
                                                   IEnumerable<string> roots,
                                                   bool ignoreSystemDlls) {
    using(var resolver = AssemblyResolverFor(dir)) {
      foreach (var assemblyFileName in roots) {
        ...
        // It is possible to have references to missing assemblies, if the
        // compiler emitted an assembly reference which was not actually needed
        // (https://github.com/dotnet/roslyn/issues/31192), so that UnityLinker
        // would then delete such an assembly.
        if (!File.Exists(fileName))
          continue;

        var assemblyDefinition = AssemblyDefinition.ReadAssembly(
            fileName, new ReaderParameters{AssemblyResolver = resolver});
        ...
        foreach (var reference in assemblyDefinition.MainModule
                     .AssemblyReferences) {
          var refFileName = reference.Name + ".dll";
          if (_assemblyFileNames.Contains(refFileName))
            continue;
          CollectReferencesFromRootsRecursive(dir, new string[]{refFileName},
                                              ignoreSystemDlls);
        }
      }
    }
  ...
  // Follows actually referenced libraries only
  public void CollectReferencesFromRoots(string dir, IEnumerable<string> roots,
                                         bool collectMethods,
                                         float progressValue,
                                         bool ignoreSystemDlls) {
    ...
    _referencedTypes =
        MonoAOTRegistration.BuildReferencedTypeList(assemblyDefinitionsAsArray);

    if (collectMethods)
      CollectReferencedAndDefinedMethods(assemblyDefinitionsAsArray);
  }

  public void CollectReferences(string path, bool collectMethods,
    ...
    using(var resolver = AssemblyResolverFor(path)) {
      foreach (var filePath in filePaths) {
        ...
        var assembly = AssemblyDefinition.ReadAssembly(
            filePath, new ReaderParameters{AssemblyResolver = resolver});
  ...
  private bool MethodIsMouseEvent(MethodDefinition method) {
    var methodNameIsMouseEvent =
        method.Name == "OnMouseDown" || method.Name == "OnMouseDrag" ||
        method.Name == "OnMouseEnter" || method.Name == "OnMouseExit" ||
        method.Name == "OnMouseOver" || method.Name == "OnMouseUp" ||
    ...
    bool isInUnityEngineBehavior =
        InheritsFromMonoBehaviour(method.DeclaringType);
  ...
  private bool InheritsFromMonoBehaviour(TypeReference type) {
    // Case 833157: StagingArea\Data\Managed contains user and dependency
    // assemblies, but doesn't contain UnityEngine.dll. This applies for all
    // platforms
    //              Thus we wouldn't be able to load UnityEngine.dll when
    //              Resolve() is called. That's why we're delaying Resolve() as
    //              much as possible
    ...
    try {
      ...
      // We weren't able to resolve the base type - let's assume it's not a
      // monobehaviour
    }
  ...
  private void DisplayProgress() {
    ...
    var progressStrings = new[]{"Fetching assembly references",
                                "Building list of referenced assemblies..."};

    if (elapsedTime.TotalMilliseconds >= 100) {
      if (EditorUtility.DisplayCancelableProgressBar(
              progressStrings[0], progressStrings[1], _progressValue))
  ...
  public bool HasReferenceToMethod(string methodName, bool ignoreSystemDlls) {
    return !ignoreSystemDlls? _referencedMethods
                .Any(item => item.Contains(methodName))
        : _userReferencedMethods.Any(item => item.Contains(methodName));
  }
  ...
  public string WhoReferencesClass(string klass, bool ignoreSystemDlls) {
    foreach (var assembly in _assemblyDefinitions) {
      ...
      var types = MonoAOTRegistration.BuildReferencedTypeList(
          assemblyDefinitionsAsArray);
  ...
  public static bool IsIgnoredSystemDll(AssemblyDefinition assembly) {
    ...
    return name.StartsWith("System") || name.Equals("UnityEngine") ||
           name.Equals("UnityEngine.Networking") || name.Equals("Mono.Posix") ||
           name.Equals("Moq");
  }
