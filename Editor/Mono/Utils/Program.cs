...
namespace UnityEditor.Utils {
  internal class Program : IDisposable {
    ...
    protected Program() { _process = new Process(); }

    public Program(ProcessStartInfo si) : this() { _process.StartInfo = si; }
    ...
    public void Start(EventHandler exitCallback) {
      ...
      _process.Start();
      _stdout =
          new ProcessOutputStreamReader(_process, _process.StandardOutput);
      _stderr = new ProcessOutputStreamReader(_process, _process.StandardError);
      _stdin = _process.StandardInput.BaseStream;
    }

    public ProcessStartInfo GetProcessStartInfo() { return _process.StartInfo; }
      ...
      get {
        if (_process == null)
          throw new InvalidOperationException(
              "You cannot call HasExited before calling Start");
        try {
    ...
    public Stream GetStandardInput() { return _stdin; }
    ...
    public bool WaitForExit(int milliseconds) {
      ...
      if (Application.platform == RuntimePlatform.OSXEditor) {
        ...
        while (!_process.HasExited &&
               (DateTime.Now - start).Milliseconds < milliseconds) {
          // Don't consume 100% of CPU while waiting for process to exit
          Thread.Sleep(SleepTimeoutMiliseconds);
        }
        return _process.HasExited;
