...
private readonly static Operator[] s_Operators = {
	...
	new Operator('u', 4, 1,
	             Associativity.Left) // unary minus trick. For example we
	                                 // convert 2/-7+(-9*8)*2^-9-5 to
	                                 // 2/u7+(u9*8)*2^u9-5 before evaluation
};
...
private struct Operator {
	...
	public Operator(char character, int presedence, int inputs,
...
private static bool Evaluate<T>(string[] tokens, out T value) {
	...
	foreach (string token in tokens) {
		if (IsOperator(token)) {
			...
			while (stack.Count > 0 && !IsCommand(stack.Peek()) &&
			       values.Count < oper.inputs) {
				T newValue;
...
// Translate tokens from infix into RPN
...
// While there is an operator (topOfStack) at the top of the operators stack
// and either (newOperator) is left-associative and its precedence is less or
// equal to that of (topOfStack), or (newOperator) is right-associative and
// its precedence is less than (topOfStack)
private static bool NeedToPop(Stack<char> operatorStack,
                              Operator newOperator) {
	if (operatorStack.Count > 0) {
		Operator topOfStack = CharToOperator(operatorStack.Peek());

		if (IsOperator(topOfStack.character)) {
			if (newOperator.associativity == Associativity.Left &&
			    newOperator.presedence <= topOfStack.presedence ||
			    newOperator.associativity == Associativity.Right &&
			    newOperator.presedence < topOfStack.presedence) {
				return true;
			}
		}
...
private static string PreFormatExpression(string expression) {
	...
	// remove trailing operator for niceness (user is middle of typing, and we
	// don't want to evaluate to zero)
...
// Turn unary minus into an operator. For example: - ( 1 - 2 ) * - 3 becomes:
// u ( 1 - 2 ) * u 3
private static string[] FixUnaryOperators(string[] tokens) {
	...
	for (int i = 1; i < tokens.Length - 1; i++) {
		...
		if (token == "-" &&
		    (IsCommand(previousToken) || nextToken == "(" || nextToken == ")"))
			tokens[i] = "u";
	}
	return tokens;
}

// According to internetz, there are many bad ways to do arithmetics with
// generics. This is one of them.
private static T Evaluate<T>(T[] values, char oper) {
	if (typeof(T) == typeof(float)) {
		if (values.Length == 1) {
			...
			switch (oper) {
			case '+':
				return (T)(object)((float)(object) values[0] + (float)(object)
				                   values[1]);
			case '-':
				return (T)(object)((float)(object) values[0] - (float)(object)
				                   values[1]);
			...
			case '%':
				return (T)(object)((float)(object) values[0] % (float)(object)
				                   values[1]);
			case '^':
				return (T)(object) UnityEngine.Mathf.Pow((float)(object) values[0],
				                                         (float)(object) values[1]);
			}
		}
		...
		if (values.Length == 1) {
			...
			case '^':
				return (T)(object)(int)
				       Math.Pow((int)(object) values[0], (int)(object) values[1]);
			}
		}
	}
	if (typeof(T) == typeof(double)) {
		if (values.Length == 1) {
			...
			switch (oper) {
			case '+':
				return (T)(object)((double)(object) values[0] + (double)(object)
				                   values[1]);
			case '-':
				return (T)(object)((double)(object) values[0] - (double)(object)
				                   values[1]);
			...
			case '%':
				return (T)(object)((double)(object) values[0] % (double)(object)
				                   values[1]);
			case '^':
				return (T)(object) System.Math.Pow((double)(object) values[0],
				                                   (double)(object) values[1]);
			}
		}
		...
		if (values.Length == 1) {
			...
			switch (oper) {
			case '+':
				return (T)(object)((long)(object) values[0] + (long)(object)
				                   values[1]);
			case '-':
				return (T)(object)((long)(object) values[0] - (long)(object)
				                   values[1]);
			...
			case '%':
				return (T)(object)((long)(object) values[0] % (long)(object)
				                   values[1]);
			case '^':
				return (T)(object)(long) System.Math.Pow((long)(object) values[0],
				                                         (long)(object) values[1]);
			}
		}
	}
	return default(T);
...
private static bool TryParse<T>(string expression, out T result) {
	...
	if (typeof(T) == typeof(float)) {
		float temp = 0.0f;
		success =
			float.TryParse(expression, NumberStyles.Float,
			               CultureInfo.InvariantCulture.NumberFormat, out temp);
		result = (T)(object) temp;
	} else if (typeof(T) == typeof(int)) {
		int temp = 0;
		success =
			int.TryParse(expression, NumberStyles.Integer,
			             CultureInfo.InvariantCulture.NumberFormat, out temp);
		result = (T)(object) temp;
	} else if (typeof(T) == typeof(double)) {
		double temp = 0;
		success =
			double.TryParse(expression, NumberStyles.Float,
			                CultureInfo.InvariantCulture.NumberFormat, out temp);
		result = (T)(object) temp;
	} else if (typeof(T) == typeof(long)) {
		long temp = 0;
		success =
			long.TryParse(expression, NumberStyles.Integer,
			              CultureInfo.InvariantCulture.NumberFormat, out temp);
		result = (T)(object) temp;
	}
	return success;
