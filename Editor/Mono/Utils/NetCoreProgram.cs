...
public NetCoreProgram(string executable, string arguments,
	...
	if (!IsNetCoreAvailable()) {
		Debug.LogError(
			"Creating NetCoreProgram, but IsNetCoreAvailable() == false; fix the caller!");
		// let it happen anyway to preserve previous behaviour
	}

	var startInfo = CreateDotNetCoreStartInfoForArgs(
		CommandLineFormatter.PrepareFileName(executable) + " " + arguments);
...
CreateDotNetCoreStartInfoForArgs(string arguments) {
	...
	if (Application.platform == RuntimePlatform.OSXEditor) {
		// .NET Core needs to be able to find the newer openssl libraries that
		// it requires on OSX
		var nativeDepsPath = Path.Combine(
			Path.Combine(Path.Combine(GetNetCoreRoot(), "NativeDeps"), "osx"),
			"lib");

		if (startInfo.EnvironmentVariables.ContainsKey("DYLD_LIBRARY_PATH"))
			startInfo.EnvironmentVariables["DYLD_LIBRARY_PATH"] = string.Format(
				"{0}:{1}", nativeDepsPath,
				startInfo.EnvironmentVariables["DYLD_LIBRARY_PATH"]);
		else
			startInfo.EnvironmentVariables.Add("DYLD_LIBRARY_PATH",
			                                   nativeDepsPath);
	}
		...
		using (var getVersionProg = new Program(startInfo)) {
			try {
				...
				Debug.LogWarningFormat(
					"Disabling CoreCLR, got exception trying to run with --version: {0}",
					ex);
				return false;
			}
			...
			if (!getVersionProg.HasExited) {
				getVersionProg.Kill();
				Debug.LogWarning(
					"Disabling CoreCLR, timed out trying to run with --version");
				return false;
			}
			...
			if (getVersionProg.ExitCode != 0) {
				Debug.LogWarningFormat(
					...
					getVersionProg.ExitCode,
					getVersionProg.GetErrorOutputAsString());
				return false;
			}
		}
