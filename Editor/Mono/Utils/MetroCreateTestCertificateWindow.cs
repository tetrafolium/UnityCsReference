...
internal class MetroCreateTestCertificateWindow : EditorWindow {
  ...
  public static void Show(string publisher) {
    var windows =
        (MetroCreateTestCertificateWindow[]) Resources.FindObjectsOfTypeAll(
            typeof(MetroCreateTestCertificateWindow));
    var window =
        ((windows.Length > 0)
             ? windows[0]
             : ScriptableObject
                   .CreateInstance<MetroCreateTestCertificateWindow>());

    window.path = Path.Combine(Application.dataPath, "WSATestCertificate.pfx")
                      .Replace('\\', '/');
    window.publisher = publisher;
    window.password = string.Empty;
    window.confirm = window.password;
    window.message =
        (File.Exists(window.path) ? EditorGUIUtility.TrTextContent(
    ...
    if (windows.Length > 0) {
      ...
      window.titleContent = EditorGUIUtility.TrTextContent(
          "Create Test Certificate for Windows Store");
  ...
  public void OnGUI() {
    ...
    using(HorizontalLayout.DoLayout()) {
      ...
      using(VerticalLayout.DoLayout()) {
        ...
        using(HorizontalLayout.DoLayout()) {
          GUILayout.Label(
              EditorGUIUtility.TrTextContent("Confirm password",
                                             "Re-enter certificate password."),
              kLabelWidth);
        ...
        using(HorizontalLayout.DoLayout()) {
          ...
          if (GUILayout.Button(EditorGUIUtility.TrTextContent("Create"),
            ...
            if (string.IsNullOrEmpty(publisher)) {
              message = EditorGUIUtility.TrTextContent(
            ...
            /*else if (!IsValidPublisher(publisher))
            {
              ...
              if (string.IsNullOrEmpty(confirm)) {
                message =
                    EditorGUIUtility.TrTextContent("Confirm the password.");
                focus = kConfirmId;
              } else {
                message =
                    EditorGUIUtility.TrTextContent("Passwords do not match.");
              ...
              try {
                EditorUtility.WSACreateTestCertificate(path, publisher,
                                                       password, true);

                AssetDatabase.Refresh(ImportAssetOptions.ForceUpdate);

                if (!PlayerSettings.WSA.SetCertificate(
                        FileUtil.GetProjectRelativePath(path), password)) {
