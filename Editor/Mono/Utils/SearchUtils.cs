...
internal static bool MatchSearch(string searchContext, string content) {
	return content != null && searchContext != null &&
	       content.IndexOf(searchContext, StringComparison.OrdinalIgnoreCase) >=
	       0;
}
...
internal static bool MatchSearchGroups(string searchContext, string content) {
	int dummyStart;
	int dummyEnd;
	return MatchSearchGroups(searchContext, content, out dummyStart,
	                         out dummyEnd);
}
...
internal static bool MatchSearchGroups(string searchContext, string content,
	...
	foreach (var searchGroup in searchGroups) {
		...
		startSearchIndex =
			content.IndexOf(searchGroup, startSearchIndex,
...
internal static bool MatchNonConsecutive(string searchContext,
                                         string content) {
	int dummyStart;
	int dummyEnd;
	return MatchNonConsecutive(searchContext, content, out dummyStart,
	                           out dummyEnd);
}
...
internal static bool MatchNonConsecutive(string searchContext, string content,
	...
	startIndex = endIndex = -1;
	if (searchContext == null || content == null ||
	    searchContext.Length > content.Length)
		return false;
	...
	// Ensure we find all letters of searchContext but not necessarily
	// consecutively:
	var contentIndex = 0;
	var searchContextIndex = 0;
	for (; searchContextIndex < searchContext.Length &&
		...
		while (contentIndex < content.Length) {
			if (ch == Char.ToLowerInvariant(content[contentIndex++])) {
				...
				endIndex = searchContextIndex == searchContext.Length - 1
			 ? (contentIndex - 1)
			 : endIndex;
				break;
			}
...
public static bool EndsWith(string a, string b) {
	...
	int bp = b.Length - 1;
	while (ap >= 0 && bp >= 0 &&
	       char.ToUpperInvariant(a[ap]) == char.ToUpperInvariant(b[bp])) {
		ap--;
		bp--;
	}
