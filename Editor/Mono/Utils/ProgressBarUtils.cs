...
// Supports clamping to a sub range of a complete process (defined from 0.0f
// to 1.0f). When using a ProgressHandler with a sub range, processes can refer
// to their local completion rate
//  and the ProgressHandler will report back progress within that sub range.
internal class ProgressHandler {
  public delegate void ProgressCallback(string title, string message,
  ...
  public ProgressHandler(string title, ProgressCallback callback,
                         float progressRangeMin = 0.0f,
                         float progressRangeMax = 1.0f) {
    m_Title = title;
  ...
  private float CalcGlobalProcess(float localProcess) {
    return Mathf.Clamp(m_ProgressRangeMin * (1.0f - localProcess) +
                           m_ProgressRangeMax * localProcess,
                       0.0f, 1.0f);
  }
  ...
  public ProgressHandler SpawnFromLocalSubRange(float localRangeMin,
                                                float localRangeMax) {
    return new ProgressHandler(m_Title, m_ProgressCallback,
                               CalcGlobalProcess(localRangeMin),
                               CalcGlobalProcess(localRangeMax));
  }
}

// A helper class that attaches to a ProgressHandler
// Supports queueing tasks which will automatically notify the ProgressHandler
// when executed
internal class ProgressTaskManager {
  ...
  public ProgressTaskManager(ProgressHandler handler) { m_Handler = handler; }

  public void AddTask(Action task) { m_Tasks.Add(task); }
  ...
  public void UpdateProgress(string message) {
    if (m_Handler != null) {
      // Get some movement of the bar, even for unknown number of progress
      // updates
      float taskProgress =
          1.0f - Mathf.Pow(0.85f, m_ProgressUpdatesForCurrentTask);
      int totalTasks = m_Tasks.Count;
      if (totalTasks <= m_StartedTasks)
        totalTasks = m_StartedTasks;
      float taskStep = 1.0f / totalTasks;
      float runProgress =
          (m_StartedTasks - 1) * taskStep + taskProgress * taskStep;
