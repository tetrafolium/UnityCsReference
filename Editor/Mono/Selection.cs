...
public sealed partial class Selection {
  ...
  private static IEnumerable GetFilteredInternal(System.Type type,
                                                 SelectionMode mode) {
    if (typeof(Component).IsAssignableFrom(type) || type.IsInterface)
      return GetTransforms(mode)
      ...
      return GetObjectsMode(mode).Where(
          o => o != null && type.IsAssignableFrom(o.GetType()));
  }

  public static T[] GetFiltered<T>(
      SelectionMode mode) // no generic constraint because we also want to allow
                          // interfaces
  {
