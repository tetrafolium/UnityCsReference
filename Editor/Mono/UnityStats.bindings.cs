...
[StructLayout(LayoutKind.Sequential)] struct StateStats {
...
[NativeType(CodegenOptions.Force)] struct ClientStats {
	public int shadowCasters;
}

[NativeType(CodegenOptions.Force)] struct DrawStats {
	public int batches;
	public int calls;
...
[StructLayout(LayoutKind.Sequential)] struct MemoryStats {
...
//*undocumented*
// Undocumented, but left public. Some people want to figure out draw calls from
// editor scripts to do some performance checking optimizations.
...
internal extern static DrawStats drawStats {
	get;
}
internal extern static MemoryStats memoryStats {
	get;
}
internal extern static ClientStats clientStats {
	get;
}
internal extern static StateStats stateChanges {
	get;
}
...
public static int instancedBatchedDrawCalls =>
drawStats.instancedBatchedCalls;
...
// Temporary API for game view stats window, so it can display proper numbers
// for >2B cases. Profiling Counters API should happen in the future to solve
// this properly.
...
[NativeName("ClientFrameTime")]
public extern static float frameTime {
	get;
}
[NativeName("RenderFrameTime")]
public extern static float renderTime {
	get;
}

public extern static float audioLevel {
	[FreeFunction("GetAudioManager().GetMasterGroupLevel")] get;
}
public extern static float audioClippingAmount {
	[FreeFunction("GetAudioManager().GetMasterGroupClippingAmount")] get;
}
public extern static float audioDSPLoad {
	[FreeFunction("GetAudioManager().GetDSPLoad")] get;
}
public extern static float audioStreamLoad {
	[FreeFunction("GetAudioManager().GetStreamLoad")] get;
}

public extern static int renderTextureCount {
	[FreeFunction("RenderTexture::GetCreatedRenderTextureCount")] get;
}
public extern static int renderTextureBytes {
	[FreeFunction("RenderTexture::GetCreatedRenderTextureBytes")] get;
}
...
public extern static int vboTotal {
	[FreeFunction("GetGfxDevice().GetTotalBufferCount")] get;
}
public extern static int vboTotalBytes {
	[FreeFunction("GetGfxDevice().GetTotalBufferBytes")] get;
}
