...
internal class SavedFilter {
  ...
  public float m_PreviewSize =
      -1f; // if -1f then preview size is not applied when set
  public int m_ID;
  public SearchFilter m_Filter;

  public SavedFilter(string name, SearchFilter filter, int depth,
                     float previewSize) {
    m_Name = name;
    m_Depth = depth;
    m_Filter = filter;
...
[FilePathAttribute("SearchFilters",
...
internal class SavedSearchFilters : ScriptableSingleton<SavedSearchFilters> {
  ...
  [SerializeField] List<SavedFilter> m_SavedFilters;
  ...
  public static int AddSavedFilter(string displayName, SearchFilter filter,
                                   float previewSize) {
    int instanceID =
        instance.Add(displayName, filter, previewSize, GetRootInstanceID(),
                     true); // using 0 adds filter after the root
    return instanceID;
  }

  public static int AddSavedFilterAfterInstanceID(string displayName,
                                                  SearchFilter filter,
                                                  float previewSize,
                                                  int insertAfterID,
                                                  bool addAsChild) {
    int instanceID = instance.Add(displayName, filter, previewSize,
                                  insertAfterID, addAsChild);
    return instanceID;
  }
  ...
  public static int GetRootInstanceID() { return instance.GetRoot(); }
  ...
  public static int GetFilterInstanceID(string name, string searchFieldString) {
    if (instance.m_SavedFilters != null && instance.m_SavedFilters.Count > 0) {
      foreach (SavedFilter sf in instance.m_SavedFilters) {
        if ((string.IsNullOrEmpty(name) || sf.m_Name == name) &&
            sf.m_Filter.FilterToSearchFieldString() == searchFieldString) {
          return sf.m_ID;
        }
      }
    }
  ...
  public static string GetName(int instanceID) {
    ...
    Debug.LogError("Could not find saved filter " + instanceID + " " +
                   instance);
    return "";
  }
  ...
  public static void SetName(int instanceID, string name) {
    ...
    if (filter != null) {
      ...
      Debug.LogError("Could not set name of saved filter " + instanceID + " " +
                     instance);
  }

  public static void UpdateExistingSavedFilter(int instanceID,
                                               SearchFilter filter,
                                               float previewSize) {
  ...
  public static void RefreshSavedFilters() { instance.Changed(); }
  ...
  internal static void AddInitializedListener(System.Action callback) {
    instance.m_SavedFiltersInitialized -=
        callback; // ensures its not added twice
  ...
  public static void MoveSavedFilter(int instanceID, int parentInstanceID,
                                     int targetInstanceID, bool after) {
    instance.Move(instanceID, parentInstanceID, targetInstanceID, after);
  }

  public static bool CanMoveSavedFilter(int instanceID, int parentInstanceID,
                                        int targetInstanceID, bool after) {
    return instance.IsValidMove(instanceID, parentInstanceID, targetInstanceID,
                                after);
  }

  public static bool AllowsHierarchy() { return instance.m_AllowHierarchy; }

  // ------------
  // Impl

  bool IsValidMove(int instanceID, int parentInstanceID, int targetInstanceID,
                   bool after) {
    ...
    if (index < 0 || parentIndex < 0 || targetIndex < 0) {
      Debug.LogError("Move of a SavedFilter has invalid input: " + index + " " +
  ...
  void Move(int instanceID, int parentInstanceID, int targetInstanceID,
            bool after) {
  ...
  int GetNextAvailableID() {
    ...
    Debug.LogError("Could not assign valid ID to saved filter " +
                   DebugUtils.ListToString(allIDs) + " " + result);
    return ProjectWindowUtil.k_FavoritesStartInstanceID + 1000;
  }

  int Add(string displayName, SearchFilter filter, float previewSize,
          int insertAfterInstanceID, bool addAsChild) {
    ...
    SavedFilter savedFilter =
        new SavedFilter(displayName, filterCopy, depth, previewSize);
    savedFilter.m_ID = GetNextAvailableID();

    if (m_SavedFilters.Count == 0) {
      m_SavedFilters.Add(savedFilter);
    } else {
      m_SavedFilters.Insert(afterIndex + 1,
                            savedFilter); // insert after wanted index
    }
  ...
  void Init() {
    // Data is serialized to hdd so this code is only run first time (if we did
    // not provide a default set of saved filters)
    ...
    // Always set root filter data (is serialized so we need to set it here to
    // affect serialized data)
    ...
    // At init check if all have valid ids (for patching up ids old saved
    // filters)
    for (int i = 0; i < m_SavedFilters.Count; ++i)
  ...
  TreeViewItem BuildTreeView() {
    ...
    if (m_SavedFilters.Count == 0) {
      Debug.LogError(
          "BuildTreeView: No saved filters! We should at least have a root");
      return null;
    }
    ...
    for (int i = 0; i < m_SavedFilters.Count; ++i) {
      ...
      bool isFolder =
          savedFilter.m_Filter.GetState() == SearchFilter.State.FolderBrowsing;
      TreeViewItem item = new SearchFilterTreeItem(
          instanceID, depth, null, savedFilter.m_Name, isFolder);
      if (i == 0)
        root = item;
      else {
        if (Collab.instance.collabFilters.ContainsSearchFilter(
                savedFilter.m_Name,
                savedFilter.m_Filter.FilterToSearchFieldString())) {
          if (!Collab.instance.IsCollabEnabledForCurrentProject())
            continue;
        }

        if (SoftlockViewController.Instance.softLockFilters
                .ContainsSearchFilter(
                    savedFilter.m_Name,
                    savedFilter.m_Filter.FilterToSearchFieldString())) {
          if (CollabSettingsManager.IsAvailable(
                  CollabSettingType.InProgressEnabled)) {
            if (!Collab.instance.IsCollabEnabledForCurrentProject() ||
  ...
  public override string ToString() {
    ...
    for (int i = 0; i < m_SavedFilters.Count; ++i) {
      ...
      text += string.Format(": {0} ({1})({2})({3}) ", s.m_Name, instanceID,
                            s.m_Depth, s.m_PreviewSize);
    }
    return text;
  }
}
