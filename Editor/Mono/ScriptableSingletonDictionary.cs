...
// Use the LibraryFolderPathAttribute when you want to have your scriptable
// singleton dictionary to persist between unity sessions. Example:
// [LibraryFolderPathAttribute("EditorWindows")]
...
internal abstract class ScriptableSingletonDictionary<TDerived, TValue>
    : ScriptableObject where TDerived : ScriptableObject where TValue
    : ScriptableObject {
  ...
  public TValue this[string preferencesFileName] {
    get { return Load(preferencesFileName); }
  }
  ...
  public void Save(string preferencesFileName, TValue value) {
    ...
    Debug.Assert(preferencesFileName != null && value != null,
                 "Should always have valid key/values.");
    ...
    InternalEditorUtility.SaveToSerializedFileAndForget(
        new[]{value}, GetProjectRelativePath(file), saveAsText);
  }

  public void Clear(string preferencesFileName) {
    string fullPath = Application.dataPath + "/../" +
                      GetProjectRelativePath(preferencesFileName);
  ...
  private TValue Load(string preferencesFileName) {
    ...
    string file = preferencesFileName;
    if (!string.IsNullOrEmpty(file)) {
      var objects = InternalEditorUtility.LoadSerializedFileAndForget(
