...
internal class PlayerConnectionLogReceiver
	: ScriptableSingleton<PlayerConnectionLogReceiver> {
static Guid logMessageId {
	get { return new Guid("394ada03-8ba0-4f26-b001-1a6cdeb05a62"); }
}
static Guid cleanLogMessageId {
	get { return new Guid("3ded2dda-cdf2-46d8-a3f6-01741741e7a9"); }
}
const string prefsKey = "PlayerConnectionLoggingState";

internal enum ConnectionState { Disconnected, CleanLog, FullLog }

[SerializeField] ConnectionState state = ConnectionState.Disconnected;

void OnEnable() {
	State = (ConnectionState)
	        EditorPrefs.GetInt(prefsKey, (int) ConnectionState.CleanLog);
}
...
void LogMessage(MessageEventArgs messageEventArgs) {
	...
	string name =
		ProfilerDriver.GetConnectionIdentifier(messageEventArgs.playerId);
