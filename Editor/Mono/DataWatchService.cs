...
class DataWatchHandle : IDataWatchHandle {
  ...
  public bool disposed {
    get { return ReferenceEquals(watched, null); }
  }

  public void Dispose() {
    if (disposed) {
      throw new InvalidOperationException(
          "DataWatchHandle was already disposed of");
    }
...
internal class DataWatchService : IDataWatchService {
  ...
  private class Watchers {
    ...
    public bool IsEmpty() { return spyList == null || spyList.Count == 0; }
    ...
    public bool PollForChanges() {
      if (watchedObject == null) // object could have been deleted
      {
        ...
        // we need to poll the tracker first if it exists, since it needs to
        // update the object checksum
  ...
  private Dictionary<int, DataWatchHandle> m_Handles =
      new Dictionary<int, DataWatchHandle>();
  private Dictionary<Object, Watchers> m_Watched =
      new Dictionary<Object, Watchers>();

  public DataWatchService() {
    // TODO probably just do this when panel becomes active and stop when being
    // inactive
    Undo.postprocessModifications += PostProcessUndo;
  }

  ~DataWatchService() { Undo.postprocessModifications -= PostProcessUndo; }

  // callback to watch undo stack for changes
  // TODO: add the hook before animation strips some changes in record mode
  public UndoPropertyModification[] PostProcessUndo(
  ...
  public IDataWatchHandle AddWatch(Object watched,
                                   Action<Object> onDataChanged) {
    if (watched == null)
    ...
    if (!m_Watched.TryGetValue(watched, out watchers)) {
      ...
      watchers.scheduledItem =
          m_Scheduler.ScheduleUntil(watchers.OnTimerPoolForChanges, 0, 0,
                                    null); // we poll as often as possible
    }
