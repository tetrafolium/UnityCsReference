...
private HolographicStreamerConnectionState m_LastConnectionState =
...
private static GUIContent s_ConnectionStatusText =
	EditorGUIUtility.TrTextContent("Connection Status");
private static GUIContent s_EmulationModeText =
	EditorGUIUtility.TrTextContent("Emulation Mode");
private static GUIContent s_DeviceVersionText =
	EditorGUIUtility.TrTextContent("Device Version");
private static GUIContent s_RoomText =
	EditorGUIUtility.TrTextContent("Room");
private static GUIContent s_InputText =
	EditorGUIUtility.TrTextContent("Simulated Input");
private static GUIContent s_RemoteMachineText =
	EditorGUIUtility.TrTextContent("Remote Machine");
private static GUIContent s_EnableVideoText =
	EditorGUIUtility.TrTextContent("Enable Video");
private static GUIContent s_EnableAudioText =
	EditorGUIUtility.TrTextContent("Enable Audio");
private static GUIContent s_MaxBitrateText =
	EditorGUIUtility.TrTextContent("Max Bitrate (kbps)");
private static GUIContent s_ConnectionButtonConnectText =
	EditorGUIUtility.TrTextContent("Connect");
private static GUIContent s_ConnectionButtonDisconnectText =
	EditorGUIUtility.TrTextContent("Disconnect");
private static GUIContent s_ConnectionStateDisconnectedText =
	EditorGUIUtility.TrTextContent("Disconnected");
private static GUIContent s_ConnectionStateConnectingText =
	EditorGUIUtility.TrTextContent("Connecting");
private static GUIContent s_ConnectionStateConnectedText =
	EditorGUIUtility.TrTextContent("Connected");
...
private static GUIContent[] s_DeviceVersionStrings =
	new GUIContent[] {EditorGUIUtility.TrTextContent("HoloLens (1st Gen)"),
	                  EditorGUIUtility.TrTextContent("HoloLens 2")};

private static GUIContent[] s_RoomStrings =
	new GUIContent[] {EditorGUIUtility.TrTextContent("None"),
...
private bool RemoteMachineNameSpecified {
	get { return !String.IsNullOrEmpty(m_RemoteMachineAddress); }
}
...
private void OnEnable() {
	...
	m_RemoteMachineHistory =
		EditorPrefs.GetString("HolographicRemoting.RemoteMachineHistory")
		.Split(',');
}
...
private void LoadCurrentRoom() {
	...
	string roomPath =
		System.IO.Path.GetFullPath("Packages/com.unity.xr.windowsmr.metro") +
		"/Plugins/HolographicSimulation/Rooms/";
	HolographicAutomation.LoadRoom(roomPath +
	                               s_RoomStrings[m_RoomIndex].text + ".xef");
}
...
private void Disconnect() {
	if (PerceptionRemoting.GetConnectionState() !=
	    HolographicStreamerConnectionState.Disconnected)
		PerceptionRemoting.Disconnect();
}

private bool IsConnectedToRemoteDevice() {
	return PerceptionRemoting.GetConnectionState() ==
...
private void HandleButtonPress() {
	if (EditorApplication.isPlayingOrWillChangePlaymode) {
		Debug.LogError(
			"Unable to connect / disconnect remoting while playing.");
		return;
	}

	HolographicStreamerConnectionState connectionState =
		PerceptionRemoting.GetConnectionState();
...
private void UpdateRemoteMachineHistory() {
	...
	for (int i = 0; i < m_RemoteMachineHistory.Length; ++i) {
		if (m_RemoteMachineHistory [i]
		    .Equals(m_RemoteMachineAddress,
	...
	if (history.Count > s_MaxHistoryLength) {
		history.RemoveRange(s_MaxHistoryLength,
		                    history.Count - s_MaxHistoryLength);
	}

	m_RemoteMachineHistory = history.ToArray();
	EditorPrefs.SetString("HolographicRemoting.RemoteMachineHistory",
	                      String.Join(",", m_RemoteMachineHistory));
}

private void RemotingPreferencesOnGUI() {
	m_DeviceVersion = (RemoteDeviceVersion) EditorGUILayout.Popup(
		s_DeviceVersionText, (int) m_DeviceVersion, s_DeviceVersionStrings);

	EditorGUI.BeginChangeCheck();
	m_RemoteMachineAddress = EditorGUILayout.DelayedTextFieldDropDown(
		s_RemoteMachineText, m_RemoteMachineAddress, m_RemoteMachineHistory);
	...
	m_MaxBitrateKbps = EditorGUILayout.IntSlider(
		s_MaxBitrateText, m_MaxBitrateKbps, 1024, 99999);
}

void ConnectionStateGUI() {
	if (s_ConnectedTexture == null) {
		s_ConnectedTexture =
			EditorGUIUtility.LoadIconRequired("sv_icon_dot3_sml");
		s_ConnectingTexture =
			EditorGUIUtility.LoadIconRequired("sv_icon_dot4_sml");
		s_DisconnectedTexture =
			EditorGUIUtility.LoadIconRequired("sv_icon_dot6_sml");
	}
	...
	bool pressed = EditorGUILayout.DropdownButton(
		buttonContent, FocusType.Passive, EditorStyles.miniButton);
	...
	if (pressed) {
		...
		// we delay the call to let the RemoteMachineAddress control commit the
		// value
		EditorApplication.CallDelayed(() => { HandleButtonPress(); }, 0f);
	}
}
...
private void DrawRemotingMode() {
	...
	m_Mode = (EmulationMode) EditorGUILayout.Popup(
		s_EmulationModeText, (int) m_Mode, s_ModeStrings);
	if (EditorGUI.EndChangeCheck()) {
...
private bool CheckOperatingSystem() {
	if (!m_OperatingSystemChecked) {
		m_OperatingSystemValid =
			System.Environment.OSVersion.Version.Build >= 14318;
...
void OnGUI() {
	if (!CheckOperatingSystem()) {
		EditorGUILayout.HelpBox(
			"You must be running Windows build 14318 or later to use Holographic Simulation or Remoting.",
			MessageType.Warning);
		return;
	}
	...
	if (!IsWindowsMixedRealityCurrentTarget()) {
		EditorGUILayout.HelpBox(
			"You must enable Virtual Reality support in settings and add Windows Mixed Reality to the devices to use Holographic Emulation.",
			MessageType.Warning);
		return;
	}
	...
	case EmulationMode.Simulated:
		EditorGUI.BeginChangeCheck();
		m_RoomIndex =
			EditorGUILayout.Popup(s_RoomText, m_RoomIndex, s_RoomStrings);
		if (EditorGUI.EndChangeCheck() && m_InPlayMode)
			LoadCurrentRoom();

		EditorGUI.BeginChangeCheck();
		m_InputType = (PlaymodeInputType) EditorGUILayout.Popup(
			s_InputText, (int) m_InputType, s_InputStrings);
		if (EditorGUI.EndChangeCheck())
...
void Update() {
	...
	case EmulationMode.RemoteDevice:
		...
		var lastConnectionFailureReason =
			PerceptionRemoting.CheckForDisconnect();
		if (lastConnectionFailureReason ==
		    HolographicStreamerConnectionFailureReason.Unreachable ||
		    lastConnectionFailureReason ==
		    HolographicStreamerConnectionFailureReason.ConnectionLost) {
			Debug.LogWarning("Disconnected with failure reason " +
			                 lastConnectionFailureReason +
			                 ", attempting to reconnect.");
			Connect();
		} else if (lastConnectionFailureReason !=
		           HolographicStreamerConnectionFailureReason.None) {
			Debug.LogError("Disconnected with error " +
			               lastConnectionFailureReason);
		}
