...
// This is what we (not users) derive from to create various views. (Main
// Toolbar, etc.)
...
internal partial class GUIView : View,
  ...
  protected virtual void OnDisable() { windowBackend = null; }
  ...
  // Without leaving this in here for MonoBehaviour::DoGUI(),
  // GetMethod(MonoScriptCache::kGUI) will return null. In that case, commands
  // are not delegated (e.g., keyboard-based delete in Hierarchy/Project)
  ...
  public static void BeginOffsetArea(Rect screenRect, GUIContent content,
                                     GUIStyle style) {
    GUILayoutGroup g = EditorGUILayoutUtilityInternal.BeginLayoutArea(
        style, typeof(GUILayoutGroup));
  ...
  // for metal we need a bit more convoluted logic and we can push more things
  // to cs
  internal void CaptureMetalScene() {
    ...
    } else if (FrameCapture.IsDestinationSupported(
                   FrameCaptureDestination.GPUTraceDocument)) {
      string path = EditorUtility.SaveFilePanel(
          "Save Metal GPU Capture", "",
          PlayerSettings.productName + ".gputrace", "gputrace");
