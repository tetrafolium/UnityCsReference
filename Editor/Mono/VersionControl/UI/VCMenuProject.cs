...
// Top-level VCS menu; set up and invoked from native code when VCS
// integration is turned on
class ProjectContextMenu {
// Called from native class VCSAssetMenuHandler as "Assets/Version
// Control/Get Latest" menu handler
...
static void GetLatest(MenuCommand cmd) {
	AssetList selected = Provider.GetAssetListFromSelection();
	selected = Provider.ConsolidateAssetList(selected, CheckoutMode.Both);
	Provider.GetLatest(selected).SetCompletionAction(
		CompletionAction.UpdatePendingWindow);
}

// Called from native class VCSAssetMenuHandler as "Assets/Version
// Control/Submit..." menu handler
...
// Called from native class VCSAssetMenuHandler as "Assets/Version
// Control/Check Out" menu handler
static bool CheckOutTest(MenuCommand cmd) {
	// TODO: Retrieve CheckoutMode from settings (depends on asset type;
	// native vs. imported)
	AssetList selected = Provider.GetAssetListFromSelection();
	return Provider.enabled &&
	       Provider.CheckoutIsValid(selected, CheckoutMode.Both);
}
...
// Called from native class VCSAssetMenuHandler as "Assets/Version
// Control/Check Out (Other)/Only asset file" menu handler
static bool CheckOutAssetTest(MenuCommand cmd) {
	AssetList selected = Provider.GetAssetListFromSelection();
	return Provider.enabled &&
	       Provider.CheckoutIsValid(selected, CheckoutMode.Asset);
}
...
// Called from native class VCSAssetMenuHandler as "Assets/Version
// Control/Check Out (Other)/Only .meta file" menu handler
static bool CheckOutMetaTest(MenuCommand cmd) {
	AssetList selected = Provider.GetAssetListFromSelection();
	return Provider.enabled &&
	       Provider.CheckoutIsValid(selected, CheckoutMode.Meta);
}
...
/// Called from native class VCSAssetMenuHandler as "Assets/Version
/// Control/Revert..." menu handler
static bool RevertTest(MenuCommand cmd) {
	AssetList selected = Provider.GetAssetListFromSelection();
	selected = Provider.ConsolidateAssetList(selected, CheckoutMode.Both);
	return Provider.enabled &&
	       Provider.RevertIsValid(selected, RevertMode.Normal);
}
...
// Called from native class VCSAssetMenuHandler as "Assets/Version
// Control/Revert Unchanged" menu handler
static bool RevertUnchangedTest(MenuCommand cmd) {
	AssetList selected = Provider.GetAssetListFromSelection();
	selected = Provider.ConsolidateAssetList(selected, CheckoutMode.Both);
	return Provider.enabled &&
	       Provider.RevertIsValid(selected, RevertMode.Unchanged);
}
...
static void RevertUnchanged(MenuCommand cmd) {
	...
	AssetList selected = Provider.GetAssetListFromSelection();
	selected = Provider.ConsolidateAssetList(selected, CheckoutMode.Both);
	Provider.Revert(selected, RevertMode.Unchanged)
	.SetCompletionAction(CompletionAction.UpdatePendingWindow);
	Provider.Status(selected);
}

// Called from native class VCSAssetMenuHandler as "Assets/Version
// Control/Diff Against Head..." menu handler
...
// Called from native class VCSAssetMenuHandler as "Assets/Version
// Control/Lock" menu handler
static bool LockTest(MenuCommand cmd) {
	AssetList selected = Provider.GetAssetListFromSelection();
	selected = Provider.ConsolidateAssetList(selected, CheckoutMode.Both);
	return Provider.enabled && Provider.hasLockingSupport &&
	       Provider.LockIsValid(selected);
}
...
// Called from native class VCSAssetMenuHandler as "Assets/Version
// Control/Unlock" menu handler
static bool UnlockTest(MenuCommand cmd) {
	AssetList selected = Provider.GetAssetListFromSelection();
	selected = Provider.ConsolidateAssetList(selected, CheckoutMode.Both);
	return Provider.enabled && Provider.hasLockingSupport &&
	       Provider.UnlockIsValid(selected);
}
...
// Called from native class VCSAssetMenuHandler as "Assets/Version
// Control/Diff/Against Head with .meta..." menu handler
