...
// List control that manages VCAssets.  This is used in a number of places in
// the plugin to display and manipulate asset lists.
namespace UnityEditorInternal.VersionControl {
  ...
  public class ListControl {
    internal static class Styles {
      ...
      static Styles() {
        ...
        meta.active.textColor = meta.onActive.textColor =
            meta.focused.textColor = meta.onFocused.textColor = Color.white;
      }
    }

    public enum SelectDirection { Up, Down, Current }
    ...
    Dictionary<string, ListItem> pathSearch =
    ...
    Dictionary<string, ListItem> selectList =
        new Dictionary<string, ListItem>();
    ...
    const string c_changeKeyPrefix =
        "_chkeyprfx_"; // Workaround to make path mapping refresh work with
                       // changesets
    const string c_metaSuffix = ".meta";
    internal const string c_emptyChangeListMessage = "Empty change list";
    internal const string c_noSearchResultChangeListMessage =
        "No search results found in this change list";
    internal const string c_noChangeListsMessage = "No changelists found";

    // Handle unique id access to the list.  This was added to enable us to pass
    // the list as an integer.  If the unity pop up menu could handle passing
    // simple C# objects then we would not need this.  This is not thread safe
    // and feels like a bad workaround.
    [System.NonSerialized] int uniqueID = 0;
    static int s_uniqueIDCount = 1;
    static Dictionary<int, ListControl> s_uniqueIDList =
        new Dictionary<int, ListControl>();
    static public ListControl FromID(int id) {
      try {
      ...
      get {
        ...
        while (cur != null) {
          ...
          bool hasEmptyChangeNotice =
              c != null && cur.HasChildren && cur.FirstChild.Item == null &&
    ...
    ~ListControl() { s_uniqueIDList.Remove(uniqueID); }
    ...
    public void Sync() {
      ...
      foreach (UnityEngine.Object obj in Selection.objects) {
        if (AssetDatabase.IsMainAsset(obj)) {
          string projectPath = Application.dataPath.Substring(
              0, Application.dataPath.Length - 6);
    ...
    public bool OnGUI(Rect area, bool focus) {
      ...
      if (open > max) {
        Rect scrollRect =
            new Rect(area.x + area.width - 14, area.y, 14, area.height);
        area.width -= c_scrollWidth;
        float tmp = listState.Scroll;

        listState.Scroll =
            GUI.VerticalScrollbar(scrollRect, listState.Scroll, max, 0, open);
        listState.Scroll = Mathf.Clamp(listState.Scroll, 0, open - max);

        if (tmp != listState.Scroll)
          repaint = true;

        // Seems to be some sort of bug with focus when the scrollbar 1st
        // appears This is a work around for that so on the first update we
        // clear the hot control.
        if (!scrollVisible) {
    ...
    private bool HandleMouse(Rect area) {
      ...
      if (e.type == EventType.MouseDown && mouseInArea) {
        ...
        if (singleSelect != null && !singleSelect.Dummy) {
          ...
          if (e.button < 2) {
            ...
            if (e.mousePosition.x >= x && e.mousePosition.x < x + 16 &&
                singleSelect.CanExpand) {
      ...
      // Handle mouse up clicks
        ...
        if (singleSelect != null && !singleSelect.Dummy) {
          // right click menus - we pass the static index of the list so the
          // menu can find what is selected
          if (e.type == EventType.ContextClick) {
            ...
            if (!IsSelectedAsset() && !string.IsNullOrEmpty(menuFolder)) {
              ...
              EditorUtility.DisplayPopupMenu(
                  ...
                  menuFolder, new MenuCommand(null, uniqueID));
            } else if (!string.IsNullOrEmpty(menuDefault)) {
              s_uniqueIDList[uniqueID] = this;
              EditorUtility.DisplayPopupMenu(
                  new Rect(e.mousePosition.x, e.mousePosition.y, 0, 0),
                  menuDefault, new MenuCommand(null, uniqueID));
            }
          }
          // Left click up should set selection if singleSelect is in selection
          // since this is the case where the user has clicked and hold on a
          // selection (which may start a drag or not) and then released it.
          else if (e.type != EventType.ContextClick && e.button == 0 &&
      ...
      if (e.type == EventType.MouseDrag && mouseInArea) {
        ...
        if (dragCount > 2 && Selection.objects.Length > 0) {
          ...
          DragAndDrop.objectReferences =
              singleSelect?.Asset != null ? new UnityEngine
                                         .Object[]{singleSelect.Asset.Load()}
              : Selection.objects;
        ...
        if (dragTarget != null) {
          if (IsSelected(dragTarget)) {
            ...
            if (dragAcceptOnly) {
              ...
              bool canPrev = !dragTarget.CanAccept ||
                             dragTarget.PrevOpenVisible != dragTarget.Parent;
              bool canNext =
                  !dragTarget.CanAccept ||
                  dragTarget.NextOpenVisible != dragTarget.FirstChild;
              ...
              if (canPrev && e.mousePosition.y <= pos + size)
                dragAdjust = SelectDirection.Up;
                       ...
                       e.mousePosition.y >= pos + c_lineHeight - size)
      ...
      if (e.type == EventType.DragPerform) {
        if (dragTarget != null) {
          ListItem drag =
              dragAdjust == SelectDirection.Current ? dragTarget
              : dragTarget.Parent;
          if (dragDelegate != null && drag != null &&
              (drag.CanAccept || drag.Parent.Change != null)) {
            if (drag.Change == null)
    ...
    private void HandleSelectAll() {
      if (Event.current.type == EventType.ValidateCommand &&
          Event.current.commandName == EventCommandNames.SelectAll) {
        Event.current.Use();
    ...
    void HandleKeyInput(Event e) {
      ...
      if (e.keyCode == KeyCode.UpArrow || e.keyCode == KeyCode.DownArrow) {
        ...
        if (sel != null) {
          // Ensure that the item is in view - linear search but handles all the
          // corner cases
          if (!ScrollUpTo(sel))
            ScrollDownTo(sel);
    ...
    // Draw a list item.  A few too many magic numbers in here so will need to
    // tidy this a bit
    void DrawItem(ListItem item, Rect area, float x, float y, bool focus,
                  bool selected) {
      bool drag = (item == dragTarget);
      bool highlight = selected;

      if (selected && Event.current.type == EventType.Repaint) {
        Styles.evenBackground.Draw(
            new Rect(area.x, y, area.width, c_lineHeight), GUIContent.none,
        ...
        case SelectDirection.Up:
          if (item.PrevOpenVisible != item.Parent &&
            ...
            Styles.dragBackground.Draw(new Rect(x, y - 1, area.width, 2),
                                       GUIContent.none, false, false, true,
                                       false);
          }
          break;
        ...
        default:
          if (item.CanAccept || item.Parent.Change != null) {
            if (Event.current.type == EventType.Repaint) {
              Styles.dragBackground.Draw(
                  new Rect(area.x, y, area.width, c_lineHeight),
                  GUIContent.none, false, false, true, false);
            }
            highlight = true;
          }
          break;
        }
      } else if (dragTarget != null && item == dragTarget.Parent &&
                 dragAdjust != SelectDirection.Current) {
        if (Event.current.type == EventType.Repaint) {
          Styles.dragBackground.Draw(
              new Rect(area.x, y, area.width, c_lineHeight), GUIContent.none,
      ...
      if (item.CanExpand) {
        EditorGUI.Foldout(new Rect(x, y, 16, c_lineHeight), item.Expanded,
                          GUIContent.none);
      }
      ...
      if (!item.Dummy || item.Change != null) {
        ...
        if (item.Asset != null) {
          ...
          if (vcsType == ExternalVersionControl.Disabled ||
            ...
            drawOverlay =
                false; // no overlays for these version control systems
      ...
      Vector2 displayNameSize =
          label.CalcSize(EditorGUIUtility.TempContent(displayName));
      float labelOffsetX = x + 32;

      Rect textRect = new Rect(labelOffsetX, y, area.width - labelOffsetX,
                               c_lineHeight + 2);

      int startIndex = -1;
      if (!string.IsNullOrEmpty(filter)) {
        startIndex =
            displayName.IndexOf(filter, StringComparison.OrdinalIgnoreCase);
      }
      ...
      } else if (startIndex == -1 || item.Asset == null ||
                 Event.current.type != EventType.Repaint) {
        label.Draw(textRect, content, false, false, selected, selected);
      } else {
        int endIndex = startIndex + filter.Length;
        label.DrawWithTextSelection(textRect, content, selected, selected,
                                    startIndex, endIndex, false,
                                    GUI.skin.settings.selectionColor);
      }

      float spaceBefore =
          labelOffsetX + displayNameSize.x + (item.Change != null ? 10 : 2);
      Rect metaPosition =
          new Rect(spaceBefore, y, area.width - spaceBefore, c_lineHeight + 2);
      int visibleChildCount = item.VisibleChildCount;

      if (HasHiddenMetaFile(item)) {
        Styles.meta.Draw(metaPosition, GUIContent.Temp("+meta"), false, false,
                         selected, selected);
      }

      if (visibleChildCount > 0) {
        Styles.meta.Draw(metaPosition,
                         GUIContent.Temp($"{visibleChildCount} files"), false,
    ...
    void FilterItems() {
      for (ListItem item = root.NextOpen; item != null; item = item.NextOpen) {
        ...
        if (!string.IsNullOrEmpty(filter) && item.Change == null) {
          item.Hidden |=
              !DisplayName(item).ToLowerInvariant().Contains(filter) ||
              item.Dummy;
        }
      }
    }
    ...
    bool ScrollDownTo(ListItem item) {
      ...
      ListItem find =
          visibleList.Count > 0 ? visibleList[visibleList.Count - 1] : null;
      while (find != null && count >= 0) {
        if (find == item) {
          listState.Scroll = count;
          return true;
    ...
    // Parameter added for removing entries as this isnt necessary when updating
    // the full list
    ...
    ListItem PathSearchFind(string path) {
      try {
        return pathSearch[path.ToLower()];
      } catch {
        return null;
      }
    }
    ...
    internal bool IsSelected(ListItem item) {
      ...
      return item.Change != null &&
             selectList.ContainsKey(c_changeKeyPrefix + item.Change.id);
    }
    ...
    void SelectedRefresh() {
      Dictionary<string, ListItem> newSelect =
          new Dictionary<string, ListItem>();

      foreach (KeyValuePair<string, ListItem> de in selectList) {
        newSelect [de.Key]
        = PathSearchFind(de.Key);
      }
    ...
    ListItem GetTwinAsset(ListItem item) {
      ...
      if (item.Name.EndsWith(c_metaSuffix) && prev?.Asset != null &&
          AssetDatabase.GetTextMetaFilePathFromAssetPath(prev.Asset.path)
                  .ToLower() == item.Asset.path.ToLower())
        return prev;
      return null;
    }
    ...
    ListItem GetTwinMeta(ListItem item) {
      ...
      if (!item.Name.EndsWith(c_metaSuffix) && next?.Asset != null &&
          next.Asset.path.ToLower() ==
              AssetDatabase.GetTextMetaFilePathFromAssetPath(item.Asset.path)
                  .ToLower())
        return next;
      return null;
    }
    ...
    public void SelectedAdd(ListItem item) {
      ...
      int arrayLen = 0;
      if (selection != null)
        arrayLen = selection.Length;

      // UnityEngine.Object tmpObj = item.Asset.Load ();

      name = name.EndsWith(c_metaSuffix) ? name.Substring(0, name.Length - 5)
                                         : name;
    ...
    void SelectedRemove(ListItem item) {
      string name = item.Asset != null ? item.Asset.path.ToLower()
          : c_changeKeyPrefix + item.Change.id;
      // Remove item
      selectList.Remove(name);

      // Remove twin item of asset
      if (item.Asset != null)
        selectList.Remove(name.EndsWith(c_metaSuffix)
    ...
    // Selection flow.  Flows all selection towards the item.  Used for SHIFT
    // selection.
