...
namespace UnityEditor.VersionControl {
  // Window allowing you to review files who will be resolved.  As this may be
  // throwing changes away this window gives you the opportunty to see what will
  // change and if required the user can apply or cancel.
  internal class WindowResolve : EditorWindow {
    ListControl resolveList = new ListControl();
    AssetList assetList = new AssetList();
    bool cancelled = false;

    // This is required to preserve and be able to restore resolve list after
    // assembly reload that happens after resolving script conflicts
    ...
    static private WindowResolve GetWindow() {
      return EditorWindow.GetWindow<WindowResolve>(true,
                                                   "Version Control Resolve");
    }

    void DoOpen(AssetList resolve, bool selectAll = false) {
      if (assetList.Count == 0 && assetPaths.Count != 0) {
        assetPaths.ForEach(assetPath =>
                               assetList.Add(Provider.CacheStatus(assetPath)));
      }
    ...
    void OnGUI() {
      ...
      Rect r1 =
          new Rect(6, 40, position.width - 12, position.height - 112); // 82
      GUILayout.BeginArea(r1);
      GUILayout.Box("", GUILayout.ExpandWidth(true),
                    GUILayout.ExpandHeight(true));
      GUILayout.EndArea();

      bool repaint = resolveList.OnGUI(
      ...
      using(
          new EditorGUI.DisabledScope(resolveList.SelectedAssets.Count == 0)) {
        ...
        //                        We don't want to prompt the user about saving
        //                        the current scene when using the local or
        //                        incoming versions
        if (methodWasSelected &&
            ((resolveMethod != ResolveMethod.UseMerged) ||
             WindowChange.PromptUserToSaveDirtyScenes(assetList))) {
            ...
            if (t.success) {
              ...
              if (t.success) {
                ...
                EditorUtility.DisplayDialog(
                    "Error resolving",
                    "Error during resolve of files. Inspect log for details",
                    "Close");
              ...
              EditorUtility.DisplayDialog(
                  "Error merging",
                  "Error during merge of files. Inspect log for details",
                  "Close");
              AssetDatabase.Refresh();
            }
          } else {
