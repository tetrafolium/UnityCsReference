...
namespace UnityEditor.VersionControl {
  // Pending change list window which shows all the change lists for the version
  // control system.  This is agnostic of any version control system as long as
  // it supports the VCInterface interface.
  //
  // This window now works in an async manner but the method of update should
  // now be improved to hide the refreshing
  [EditorWindowTitle(title = "Version Control",
                     icon = "UnityEditor.VersionControl")]
  internal class WindowPending : EditorWindow {
    internal class Styles {
      ...
      public static readonly GUIContent connectingLabel =
          EditorGUIUtility.TrTextContent("CONNECTING...");
      public static readonly GUIContent offlineLabel =
          EditorGUIUtility.TrTextContent("OFFLINE");
      public static readonly GUIContent workOfflineLabel =
          EditorGUIUtility.TrTextContent(
              "WORK OFFLINE is enabled in Version Control Settings. Unity will behave as if version control is disabled.");
      public static readonly GUIContent disabledLabel =
          EditorGUIUtility.TrTextContent("Disabled");
      public static readonly GUIContent editorSettingsLabel =
          EditorGUIUtility.TrTextContent("Version Control Settings");
    }
    ...
    // Workaround to reload vcs info upon domain reload. TODO: Fix
    // VersionControl.ListControl to get rid of this
    ...
    static public void ExpandLatestChangeSet() {
      var wins = Resources.FindObjectsOfTypeAll(typeof(WindowPending))
                     as WindowPending[];
    ...
    void OnGotLatest(Task t) { UpdateWindow(); }

    static void OnVCTaskCompletedEvent(Task task,
    ...
    public static void OnStatusUpdated() { UpdateAllWindows(); }
    ...
    internal string FormatChangeSetDescription(ChangeSet changeSet) {
      ...
      string[] description = changeSet.description.Split(
          new char[]{'\n'}, StringSplitOptions.RemoveEmptyEntries);
      formattedDescription = description.Length > 1
    ...
    internal void PopulateListControl(ListControl list, Task task,
                                      Texture2D icon) {
      // We try to correct the existing list by removing/adding entries.
      // This way the entries will keep their children while the children are
      // updated and will prevent flicker
      ...
      foreach (ChangeSet change in css) {
        ...
        if (changeItem != null) {
          ...
          changeItem =
              list.Add(null, FormatChangeSetDescription(change), change);
        }
      ...
      // Refresh here will trigger the expand events to ensure the same change
      // lists are kept open. This will in turn trigger change list contents
      // update requests
      list.Refresh();
    ...
    void OnChangeContents(Task task) {
      ListItem pendingItem =
          pendingList.FindItemWithIdentifier(task.userIdentifier);
      ListItem item = pendingItem == null ? incomingList.FindItemWithIdentifier(
                                         task.userIdentifier)
          : pendingItem;
      ...
      if (assetList.Count == 0) {
        // Can only happen for pendingList since incoming lists are frozen by
        // design.
        ListItem empty =
            list.Add(item, ListControl.c_emptyChangeListMessage, (Asset) null);
        empty.Dummy = true;
      } else {
    ...
    private ChangeSets GetEmptyChangeSetsCandidates() {
      ...
      set.FindAll(item => item.id != ChangeSet.defaultID)
    ...
    private void DeleteEmptyPendingChangesets() {
      ChangeSets changeSets = GetEmptyChangeSetsCandidates();
      Provider.DeleteChangeSets(changeSets)
    ...
    private void SearchField(Event e, ListControl activeList) {
      ...
      Rect rect = GUILayoutUtility.GetRect(
          0, EditorGUILayout.kLabelFloatMaxW * 1.5f,
          EditorGUI.kSingleLineHeight, EditorGUI.kSingleLineHeight,
          EditorStyles.toolbarSearchField,
          GUILayout.MinWidth(k_MinSearchFieldWidth),
    ...
    void OnGUI() {
      ...
      int incomingChangesetCount =
          incomingList.Root == null ? 0 : incomingList.Root.ChildCount;
      bool switchToOutgoing = GUILayout.Toggle(!m_ShowIncoming, "Outgoing",
                                               EditorStyles.toolbarButton);

      GUIContent cont = GUIContent.Temp(
          "Incoming" + (incomingChangesetCount == 0
                            ? ""
                            : " (" + incomingChangesetCount + ")"));
      bool switchToIncoming =
          GUILayout.Toggle(m_ShowIncoming, cont, EditorStyles.toolbarButton);
      ...
      using(new EditorGUI.DisabledScope(Provider.activeTask != null)) {
        foreach (CustomCommand c in Provider.customCommands) {
          if (c.context == CommandContext.Global &&
              GUILayout.Button(c.label, EditorStyles.toolbarButton))
            c.StartTask();
        }
      }

      bool showDeleteEmptyChangesetsButton =
          Mathf.FloorToInt(position.width - s_ToolbarButtonsWidth -
                           k_MinSearchFieldWidth - s_SettingsButtonWidth -
                           s_DeleteChangesetsButtonWidth) > 0 &&
          HasEmptyPendingChangesets();
      if (showDeleteEmptyChangesetsButton &&
          GUILayout.Button("Delete Empty Changesets",
                           EditorStyles.toolbarButton)) {
      ...
      bool showSettingsButton =
          Mathf.FloorToInt(position.width - s_ToolbarButtonsWidth -
                           k_MinSearchFieldWidth - s_SettingsButtonWidth) > 0;

      if (showSettingsButton && GUILayout.Button(Styles.editorSettingsLabel,
                                                 EditorStyles.toolbarButton)) {
      ...
      bool refreshButtonClicked =
          GUILayout.Button(refreshIcon, EditorStyles.toolbarButton);
      ...
      Rect rect = new Rect(0, toolBarHeight, position.width,
                           position.height - toolBarHeight - k_BottomBarHeight);
      ...
      // Disabled Window view
        ...
        var content =
            EditorGUIUtility.TrTextContent("Apply All Incoming Changes");
        var buttonSize = EditorStyles.miniButton.CalcSize(content);
        Rect progressRect = new Rect(
            rect.x, rect.y - 2, rect.width - buttonSize.x - 5f, rect.height);
        ...
        if (m_ShowIncoming) {
          ...
          using(new EditorGUI.DisabledScope(
              incomingList?.Root?.VisibleChildCount == 0)) {
    ...
    [Shortcut("VersionControl/RefreshWindow", typeof(WindowPending), KeyCode.F5,
              displayName = "Version Control/Refresh Window")]
    ...
    {
        // We add a horizontal size controller to adjust incoming vs outgoing
    area float headersHeight = 26f + k_ResizerHeight +
    EditorStyles.toolbarButton.fixedHeight; Rect dragRect = new Rect (0f,
    m_IncomingAreaHeight + headersHeight, width, k_ResizerHeight);
    ...
    MouseCursor.ResizeVertical);
        ...
        m_IncomingAreaHeight = Mathf.Clamp (m_IncomingAreaHeight,
    k_MinIncomingAreaHeight, maxHeight);
    }
    */

    internal static bool ProgressGUI(Rect rect, Task activeTask,
                                     bool descriptionTextFirst) {
      if (activeTask != null &&
          (activeTask.progressPct != -1 || activeTask.secondsSpent != -1 ||
           activeTask.progressMessage.Length != 0)) {
        string msg = activeTask.progressMessage;
        Rect sr = rect;
        GUIContent cont =
            UnityEditorInternal.InternalEditorUtility.animatedProgressImage;
        sr.width = sr.height;
        sr.x += 4;
        sr.y += 4;
        GUI.Label(sr, cont);
    ...
    void CreateResources() {
      ...
      if (s_ToolbarButtonsWidth == 0f) {
        s_ToolbarButtonsWidth =
            EditorStyles.toolbarButton
                .CalcSize(EditorGUIUtility.TrTextContent("Incoming (xx)"))
                .x;
        s_ToolbarButtonsWidth +=
            EditorStyles.toolbarButton
                .CalcSize(EditorGUIUtility.TrTextContent("Outgoing"))
                .x;
        s_ToolbarButtonsWidth +=
            EditorStyles.toolbarButton.CalcSize(new GUIContent(refreshIcon)).x;
        s_SettingsButtonWidth =
            EditorStyles.toolbarButton.CalcSize(Styles.editorSettingsLabel).x;
        s_DeleteChangesetsButtonWidth =
            EditorStyles.toolbarButton
                .CalcSize(
                    EditorGUIUtility.TrTextContent("Delete Empty Changesets"))
                .x;
        minSize = new Vector2(s_ToolbarButtonsWidth + k_MinSearchFieldWidth,
                              k_MinWindowHeight);
      }
    }
