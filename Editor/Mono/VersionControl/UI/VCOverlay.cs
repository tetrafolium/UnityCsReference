...
internal static void DrawProjectOverlay(Asset asset, Asset metaAsset,
                                        Rect itemRect) {
	DrawOverlay(asset, metaAsset, itemRect, false, true, false);
}

internal static void DrawHierarchyOverlay(Asset asset, Asset metaAsset,
                                          Rect itemRect) {
	DrawOverlay(asset, metaAsset, itemRect, true, false, false);
}

internal static void DrawOtherOverlay(Asset asset, Asset metaAsset,
                                      Rect itemRect) {
	DrawOverlay(asset, metaAsset, itemRect, false, false, true);
}

public static void DrawOverlay(Asset asset, Asset metaAsset,
                               Rect itemRect) {
...
internal static void DrawOverlay(Asset asset, Asset metaAsset,
                                 Rect itemRect, bool isHierarchy = false,
                                 bool isProject = false,
                                 bool otherIcons = false) {
	if (asset == null || metaAsset == null)
		return;
	...
	DrawOverlays(asset, metaAsset, itemRect, isHierarchy, isProject,
	             otherIcons);
}
...
static void DrawOverlays(Asset asset, Asset metaAsset, Rect itemRect,
                         bool isHierarchy = false, bool isProject = false,
                         bool otherIcons = false) {
	...
	// slightly offset icons since their texture space is not filled with
	// content entirely
	float offsetX = 0;
	float offsetY = 2;

	Rect topLeft = new Rect(itemRect.x - offsetX, itemRect.y - offsetY,
	                        iconWidth, iconWidth);
	Rect topRight = new Rect(itemRect.xMax - iconWidth + offsetX,
	                         itemRect.y - offsetY, iconWidth, iconWidth);
	Rect bottomLeft =
		new Rect(itemRect.x - offsetX, itemRect.yMax - iconWidth + offsetY,
		         iconWidth, iconWidth);
	Rect bottomRight =
		new Rect(itemRect.xMax - iconWidth + offsetX,
		         itemRect.yMax - iconWidth + offsetY, iconWidth, iconWidth);

	Asset.States assetState = asset.state;
	Asset.States metaState =
		metaAsset != null ? metaAsset.state : Asset.States.None;

	Asset.States unmodifiedState =
		Asset.States.Local | Asset.States.MetaFile | Asset.States.ReadOnly |
		Asset.States.Synced;
	bool isMetaUnmodifiedState =
		metaAsset == null || (metaState & unmodifiedState) == unmodifiedState;

	Asset.States localMetaState =
		metaAsset == null ? Asset.States.None : metaState &
		(Asset.States.AddedLocal | Asset.States.CheckedOutLocal |
		 Asset.States.DeletedLocal | Asset.States.LockedLocal);
	Asset.States remoteMetaState =
		metaAsset == null ? Asset.States.None : metaState &
		(Asset.States.AddedRemote | Asset.States.CheckedOutRemote |
		 Asset.States.DeletedRemote | Asset.States.LockedRemote);

	bool keepFolderMetaParans =
		asset.isFolder && Provider.isVersioningFolders;
	...
	if (!isHierarchy) {
		...
		if (!providerActive) {
			...
			// Meta overlay if meta file is not added or already in repo and
			// unmodified.
			if (metaAsset != null &&
			    (localMetaState & Asset.States.AddedLocal) == 0 &&
			    !isMetaUnmodifiedState)
			...
			// Meta overlay if meta file is not deleted but asset is and meta file
			// is still present or missing (ie. should have been there)
			if (metaAsset != null &&
			    (localMetaState & Asset.States.DeletedLocal) == 0 &&
			    Asset.IsState(metaState,
			                  Asset.States.Local | Asset.States.Missing))
				DrawMetaOverlay(topLeft, false);
			...
			if (metaAsset != null &&
			    (localMetaState & Asset.States.LockedLocal) == 0 &&
			    !isMetaUnmodifiedState)
				DrawMetaOverlay(topLeft, false);
			...
			if (metaAsset != null &&
			    (localMetaState & Asset.States.CheckedOutLocal) == 0 &&
			    !isMetaUnmodifiedState)
				DrawMetaOverlay(topLeft, false);
			...
			if (metaAsset != null &&
			    (metaAsset.IsUnderVersionControl ||
			     !Asset.IsState(metaState, Asset.States.Local)))
				DrawMetaOverlay(bottomLeft, false);
		}
		// From here the local asset have no state that need a local state
		// overlay. We use the meta state if there is one instead.
		else if (metaAsset != null &&
		         Asset.IsState(metaState, Asset.States.AddedLocal)) {
			DrawOverlay(Asset.States.AddedLocal, topLeft);
			if (keepFolderMetaParans)
				DrawMetaOverlay(topLeft, false);
		} else if (metaAsset != null &&
		           Asset.IsState(metaState, Asset.States.DeletedLocal)) {
			DrawOverlay(Asset.States.DeletedLocal, topLeft);
			if (keepFolderMetaParans)
				DrawMetaOverlay(topLeft, false);
		} else if (metaAsset != null &&
		           Asset.IsState(metaState, Asset.States.LockedLocal)) {
			DrawOverlay(Asset.States.LockedLocal, topLeft);
			if (keepFolderMetaParans)
				DrawMetaOverlay(topLeft, false);
		} else if (metaAsset != null &&
		           Asset.IsState(metaState, Asset.States.CheckedOutLocal)) {
			DrawOverlay(Asset.States.CheckedOutLocal, topLeft);
			if (keepFolderMetaParans)
				DrawMetaOverlay(topLeft, false);
		} else if (metaAsset != null &&
		           ...
		           !(Asset.IsState(assetState, Asset.States.Conflicted) ||
		             (metaAsset != null &&
		              Asset.IsState(metaState, Asset.States.Conflicted)))) {
			DrawOverlay(Asset.States.Local, bottomLeft);
			if (keepFolderMetaParans)
				DrawMetaOverlay(bottomLeft, false);
		}
	...
	if (!providerActive) {
		...
		// unmodified.
		if (metaAsset != null &&
		    (remoteMetaState & Asset.States.AddedRemote) == 0)
		...
		// is still present or missing (ie. should have been there)
		if (metaAsset != null &&
		    (remoteMetaState & Asset.States.DeletedRemote) == 0)
			DrawMetaOverlay(topRight, true);
		...
		// Meta overlay if meta file is not locked or unmodified.
		if (metaAsset != null &&
		    (remoteMetaState & Asset.States.LockedRemote) == 0)
			DrawMetaOverlay(topRight, true);
		...
		// Meta overlay if meta file is not checked out or unmodified.
		if (metaAsset != null &&
		    (remoteMetaState & Asset.States.CheckedOutRemote) == 0)
			DrawMetaOverlay(topRight, true);
	}
	// From here the remote asset have no state that need a remote state
	// overlay. We use the meta state if there is one instead.
	else if (metaAsset != null &&
	         Asset.IsState(metaState, Asset.States.AddedRemote)) {
		DrawOverlay(Asset.States.AddedRemote, topRight);
		if (keepFolderMetaParans)
			DrawMetaOverlay(topRight, true);
	} else if (metaAsset != null &&
	           Asset.IsState(metaState, Asset.States.DeletedRemote)) {
		DrawOverlay(Asset.States.DeletedRemote, topRight);
		if (keepFolderMetaParans)
			DrawMetaOverlay(topRight, true);
	} else if (metaAsset != null &&
	           Asset.IsState(metaState, Asset.States.LockedRemote)) {
		DrawOverlay(Asset.States.LockedRemote, topRight);
		if (keepFolderMetaParans)
			DrawMetaOverlay(topRight, true);
	} else if (metaAsset != null &&
	           Asset.IsState(metaState, Asset.States.CheckedOutRemote)) {
		DrawOverlay(Asset.States.CheckedOutRemote, topRight);
		if (keepFolderMetaParans)
			DrawMetaOverlay(topRight, true);
	}
...
private static void CreateStaticResources() {
	if (s_BlueLeftParan == null) {
		s_BlueLeftParan =
			EditorGUIUtility.LoadIcon("VersionControl/P4_BlueLeftParenthesis");
		s_BlueLeftParan.hideFlags = HideFlags.HideAndDontSave;

		s_BlueRightParan =
			EditorGUIUtility.LoadIcon("VersionControl/P4_BlueRightParenthesis");
		s_BlueRightParan.hideFlags = HideFlags.HideAndDontSave;

		s_RedLeftParan =
			EditorGUIUtility.LoadIcon("VersionControl/P4_RedLeftParenthesis");
		s_RedLeftParan.hideFlags = HideFlags.HideAndDontSave;

		s_RedRightParan =
			EditorGUIUtility.LoadIcon("VersionControl/P4_RedRightParenthesis");
		s_RedRightParan.hideFlags = HideFlags.HideAndDontSave;

		s_DisconnectedIcon =
			EditorGUIUtility.LoadIcon("VersionControl/P4_Offline");
