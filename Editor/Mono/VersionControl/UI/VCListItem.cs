...
// This is a custom nested, double linked list.  This provides flexibility to
// search quickly up and down the list.  Particularly when parts of the list
// are expanded or not.
...
internal int VisibleChildCount {
	get {
		...
		while (listItem != null) {
			if (!listItem.Hidden && !listItem.Dummy)
				++count;
			listItem = listItem.m_Next;
		}
