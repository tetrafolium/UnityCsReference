...
namespace UnityEditor.VersionControl {
  // Change window used for either submitting a window, editing a change list or
  ...
  internal class WindowChange : EditorWindow {
    ...
    public void OnDisable() { m_TextAreaControlID = 0; }
    ...
    static public void Open(ChangeSet change, AssetList assets, bool submit) {
      ...
      WindowChange win = EditorWindow.GetWindow<WindowChange>(
          true, "Version Control Changeset");
      win.allowSubmit = submit;
      win.DoOpen(change, assets);
    }

    private string SanitizeDescription(string desc) {
      if (Provider.GetActivePlugin() != null &&
          Provider.GetActivePlugin().name != "Perforce")
        return desc;

      // The format of the desc is "327: on 2013/02/03 by foo@bar *pending* 'The
      // real description'" Extract the part after *pending*
      ...
      if (idx2 == -1)
        return desc;
      return desc.Substring(idx2 + 1, desc.Length - idx2 - 1)
          .TrimStart(' ')
          .Trim(' ', '\t');
    }

    // Open the change list window for one of 2 modes.  File list or exisiting
    // change list. If changeID is null then a list of files is assumed and the
    // user can sumbit them as a new change list.
    void DoOpen(ChangeSet change, AssetList assets) {
      taskSubmit = null;
      submitResultCode = kSubmitNotStartedResultCode;
      submitErrorMessage = null;
      changeSet = change;
      description = change == null ? c_defaultDescription
          : SanitizeDescription(change.description);
      assetList = null;
    ...
    internal static void OnSubmitted(Task task) {
      var winsChange = Resources.FindObjectsOfTypeAll(typeof(WindowChange))
                           as WindowChange[];
      if (winsChange.Length == 0)
        return; // user closed submit window before submit finished. Just ignore
                // the status callback.
      ...
      if ((task.resultCode &
           ((int) SubmitResult.OK | (int) SubmitResult.Error)) != 0) {
        WindowPending.UpdateAllWindows();
        bool isNewChangeSet = win.changeSet == null;
        if (isNewChangeSet) {
          // When change list becomes empty we open it to make "delete empty
          // changessets" button visible
          Task flushTask =
              Provider.Status(""); // Make a dummy task and wait for it since
                                   // that will guarantee up to date lists
          flushTask.Wait();
    ...
    internal static void OnAdded(Task task) {
      ...
      win.assetList = null;
      WindowPending
          .UpdateAllWindows(); // reflect newly added assets in pending window
    }
    ...
    void OnSubmitGUI() {
      ...
      if (taskStatus != null && taskStatus.resultCode != 0) {
        ...
        assetList = taskStatus.assetList
                        .Filter(
                            includeFolders, Asset.States.CheckedOutLocal,
...
Asset.States.Integrate*/);
        ...
        description =
            taskDesc.text.Length > 0 ? taskDesc.text : c_defaultDescription;
      ...
      Task progressTask =
          taskStatus != null && taskStatus.resultCode == 0
              ? taskStatus
              : taskDesc != null && taskDesc.resultCode == 0
                    ? taskDesc
                    : taskStat != null && taskStat.resultCode == 0 ? taskStat
                                                                   : taskSubmit;

      GUI.enabled = (taskDesc == null || taskDesc.resultCode != 0) &&
                    submitResultCode == kSubmitNotStartedResultCode;
      {
        description = EditorGUILayout
                          .TextArea(description, EditorStyles.textArea,
      ...
      if (submitResultCode == kSubmitNotStartedResultCode) {
        if (progressTask != null) {
          ...
          c.image = UnityEditorInternal.InternalEditorUtility
                        .animatedProgressImage.image;
          GUILayout.Label(c);
          c.image = null;
        }
        ...
        GUI.enabled =
            progressTask == null && !string.IsNullOrEmpty(description);

        bool keyboardShortcutActivated =
            evt.isKey && evt.shift && evt.keyCode == KeyCode.Return;
        bool saveByShortcut = keyboardShortcutActivated && !allowSubmit;

        if (Provider.hasChangelistSupport &&
            (GUILayout.Button("Save") || saveByShortcut))
          Save(false);

        if (allowSubmit) {
          bool origEnabled = GUI.enabled;
          GUI.enabled = assetList != null && assetList.Count > 0 &&
    ...
    void OnErrorGUI() {
      ...
      msg +=
          "See console for details. You can get more details by increasing log level in EditorSettings.";
      ...
      if (GUILayout.Button("Close")) {
        ...
        // to handle conflicts. After that has been handled and we close this
        // window we want to update our state.
        WindowPending.UpdateAllWindows();
      }
      GUILayout.EndHorizontal();
    }
    ...
    void OnUnaddedFilesGUI() {
      ...
      foreach (Asset a in assetList) {
        if (!a.IsState(Asset.States.OutOfSync) &&
            !a.IsState(Asset.States.Synced) &&
            !a.IsState(Asset.States.AddedLocal)) {
          files += a.prettyPath + "\n";
          toAdd.Add(a);
        }
      }
    ...
    void Save(bool submit) {
      if (description.Trim() == c_defaultDescription) {
        Debug.LogError(
            "Version control: Please enter a valid change description");
        return;
      }
      ...
      // Submit the change list. Last parameter is "save only" so invert the
      // submit flag

      taskSubmit = Provider.Submit(changeSet, assetList, description, !submit);
      submitResultCode = kSubmitRunningResultCode;
      submitErrorMessage = null;
      taskSubmit.SetCompletionAction(
          CompletionAction
              .OnSubmittedChangeWindow); // TODO: make it
                                         // CompletionAction.OnChangeSetsPendingWindow
    }

    [Shortcut("Version Control/Submit Changeset (from Changeset Window)",
              typeof(WindowChange), KeyCode.S,
