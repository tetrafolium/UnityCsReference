...
const double k_AnimatedProgressImageTimeout =
	0.5; // Stop animation repaints when the are no updates for 500 ms
static double s_LastInvalidArtifactHashTime = 0;
static readonly List<Action> s_ProgressRepainters = new List<Action>();
static readonly CallbackController s_CallbackController =
	new CallbackController(RepaintProgress,
	                       k_AnimatedProgressImageUpdateInterval);
...
static void HandleOndemandProgressOverlay(string guid, Rect drawRect,
                                          Action repaintAction) {
	var now = EditorApplication.timeSinceStartup;
	if (repaintAction != null) {
		GUID lookupGUID = new GUID(guid);
		var hash = AssetDatabaseExperimental.LookupArtifact(
		...
		if (!hash.isValid) {
			...
			if (yOffset < 0)
				yOffset = 0;
			var width =
				texture.width <= drawRect.width ? texture.width : drawRect.width;
			var height = texture.height <=
			             drawRect.height ? texture.height : drawRect.height;
			var rect = new Rect(drawRect.x + xOffset, drawRect.y + yOffset, width,
			                    height);
	...
	if (s_CallbackController.active &&
	    (s_LastInvalidArtifactHashTime + k_AnimatedProgressImageTimeout) <
	    now) {
