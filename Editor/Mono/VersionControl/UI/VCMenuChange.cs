...
namespace UnityEditorInternal.VersionControl {
  // Menu used when right clicking on change lists only.  As they are single
  // select they will not get mixed up with asset selections.
  class ChangeSetContextMenu {
    ...
    static void RevertUnchanged(int userData) {
      ChangeSets sets = ListControl.FromID(userData).SelectedChangeSets;
      Provider.RevertChangeSets(sets, RevertMode.Unchanged)
    ...
    static bool NewChangeSetTest(int userDatad) { return Provider.isActive; }
    ...
    static bool DeleteChangeSetTest(int userData) {
      ...
      bool hasAssets =
          item != null && item.HasChildren && item.FirstChild.Asset != null &&
          item.FirstChild.Name != ListControl.c_emptyChangeListMessage;
      if (!hasAssets) {
    ...
    static void DeleteChangeSet(int userData) {
      ChangeSets set = ListControl.FromID(userData).SelectedChangeSets;
      Provider.DeleteChangeSets(set).SetCompletionAction(
