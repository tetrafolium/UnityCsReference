...
namespace UnityEditor.VersionControl {
  ...
  [UsedByNativeCode]
  [StructLayout(LayoutKind.Sequential)] partial class ChangeSet {
    // The bindings generator will set the instance pointer in this field
    IntPtr m_Self;

    [FreeFunction("VersionControlBindings::ChangeSet::Create",
                  IsThreadSafe = true)]
    static extern IntPtr Create();

    [FreeFunction("VersionControlBindings::ChangeSet::CreateFromCopy",
                  IsThreadSafe = true)]
    static extern IntPtr CreateFromCopy(ChangeSet other);

    [FreeFunction("VersionControlBindings::ChangeSet::CreateFromString",
                  IsThreadSafe = true)]
    static extern IntPtr CreateFromString(string description);

    [FreeFunction("VersionControlBindings::ChangeSet::CreateFromStringString",
                  IsThreadSafe = true)]
    static extern IntPtr CreateFromStringString(string description,
                                                string changeSetID);

    [FreeFunction("VersionControlBindings::ChangeSet::Destroy",
                  IsThreadSafe = true)]
    static extern void Destroy(IntPtr changeSet);

    void InternalCreate() { m_Self = Create(); }
    ...
    void InternalCreateFromStringString(string description,
                                        string changeSetID) {
