...
namespace UnityEditor.VersionControl {
  ...
  public enum MergeMethod {
    ...
    MergeAll = 1,
    [ System.Obsolete(
        "This member is no longer supported (UnityUpgradable) -> MergeNone",
        true) ] MergeNonConflicting = 2
  }
  ...
  public partial class Provider {
    ...
    static private Task
    CheckAndCreateUserSuppliedChangeSet(string changesetID, string description,
                                        ref ChangeSet changeset) {
      // Options here:
      // ID is default and description is null - use default
      // ID is not default and description is null - use an existing changeset
      // if changesets are supported ID is default but description is not null -
      // create a new changeset if changesets are supported ID is not default
      // and description is not null - This *should* rename an existing
      // changeset to be consistent, but the current plugin doesn't implement
      // this. Throw back an error
      if (string.IsNullOrEmpty(description)) {
        ...
        if (changesetID == ChangeSet.defaultID) {
          // Description is not null but no changeset ID set - create a new
          // changeset unless the VCS provider does not support changesets
          ...
          var createChangeSetTask =
              Internal_Submit(null, null, description, true);
          ...
          foreach (var queriedChangeset in changesetsTask.changeSets) {
            ...
            if (System.Convert.ToInt64(queriedChangeset.id) >
                System.Convert.ToInt64(changeset.id))
              changeset = new ChangeSet(description, queriedChangeset.id);
          }
        } else
          // Description and changeset ID are set - this should rename but this
          // is not currently supported
          return Internal_ErrorTask(
              "User-created pre-checkout callback has set both a changeset ID and a changeset Description. This is not currently supported.");
      }
      return null;
    }

    static internal AssetList ConsolidateAssetList(AssetList assets,
                                                   CheckoutMode mode) {
      var consolidatedAssetArray =
          Internal_ConsolidateAssetList(assets.ToArray(), mode);
      var consolidatedAssetList = new AssetList();
      consolidatedAssetList.AddRange(consolidatedAssetArray);
      return consolidatedAssetList;
    }

    static private Task CheckCallbackAndCheckout(AssetList assets,
                                                 CheckoutMode mode,
                                                 ChangeSet changeset) {
      ...
      if (preCheckoutCallback != null) {
        ...
        try {
          if (preCheckoutCallback(consolidatedAssetList, ref changesetID,
                                  ref changesetDescription) == false)
            return Internal_WarningTask(
                "User-created pre-checkout callback has blocked this checkout.");
        } catch (System.Exception ex) {
          return Internal_ErrorTask(
              "User-created pre-checkout callback has raised an exception and this checkout will be blocked. Exception Message: " +
              ex.Message);
        }

        var changesetTask = CheckAndCreateUserSuppliedChangeSet(
            changesetID, changesetDescription, ref changeset);
        if (changesetTask != null)
          return changesetTask;
      }
      return Internal_Checkout(consolidatedAssetList.ToArray(), mode,
                               changeset);
    }
    ...
    internal static bool HandlePreCheckoutCallback(ref string[] paths,
                                                   ref ChangeSet changeSet) {
      if (preCheckoutCallback == null)
        return true;
      ...
      try {
        ...
        if (preCheckoutCallback(assetList, ref id, ref desc)) {
          var task =
              CheckAndCreateUserSuppliedChangeSet(id, desc, ref changeSet);
          if (task != null) {
            Debug.LogError(
        ...
        Debug.LogWarning(
            "User-created pre-checkout callback has thrown an exception: " +
            ex.Message);
        return false;
      }
    ...
    static internal Task RevertChangeSets(ChangeSets changesets,
                                          RevertMode mode) {
    ...
    static public bool SubmitIsValid(ChangeSet changeset, AssetList assets) {
      return Internal_SubmitIsValid(changeset, assets != null ? assets.ToArray()
                                    : null);
    }
    ...
    private static Task VerifyChangesetID(string changesetID) {
      ...
      if (changesetsTask.success == false) {
        Debug.LogError(string.Format(
            "Tried to validate user-supplied changeset ID {0} but Provider.ChangeSets task failed. See returned Task for details.",
            changesetID));
        return changesetsTask;
      }
      var idIsValid = false;
      ...
      if (idIsValid == false)
        return Internal_ErrorTask(string.Format(
            "The supplied changeset ID '{0}' did not match any known outgoing changesets. Aborting Task.",
            changesetID));

      return null;
    }

    static public Task Submit(ChangeSet changeset, AssetList list,
                              string description, bool saveOnly) {
      if (preSubmitCallback != null) {
        var changesetID = changeset == null ? ChangeSet.defaultID : changeset.id;
        try {
          if (preSubmitCallback(list, ref changesetID, ref description) ==
              false)
            return Internal_WarningTask(
                "User-created pre-submit callback has blocked this changeset submission.");
        } catch (System.Exception ex) {
          return Internal_ErrorTask(
              "User-created pre-submit callback has raised an exception and this submission will be blocked. Exception Message: " +
              ex.Message);
        }
      ...
      return Internal_Submit(changeset, list != null ? list.ToArray()
                             : null, description, saveOnly);
    }
    ...
    static internal bool NeedToCheckOutBoth(Object asset) {
      return AssetDatabase.IsNativeAsset(asset) || asset is SceneAsset ||
             asset is AssemblyDefinitionAsset || asset is GameObject;
    }
    ...
    static internal AssetList GetInspectorAssets(Object[] assets) {
      ...
      foreach (var asset in assets) {
        ...
        Asset metaAsset = CacheStatus(
            AssetDatabase.GetTextMetaFilePathFromAssetPath(assetPath));
        if (metaAsset != null)
    ...
    public delegate bool PreSubmitCallback(AssetList list,
                                           ref string changesetID,
                                           ref string changesetDescription);
    static public PreSubmitCallback preSubmitCallback;
    public delegate bool PreCheckoutCallback(AssetList list,
                                             ref string changesetID,
                                             ref string changesetDescription);
