...
// Monitor the behavior of assets.  This is where general unity asset
	...
	if (asset == null || asset.IsState(Asset.States.Updating)) {
		if (synchronous) {
			UnityEngine.Profiling.Profiler.BeginSample(
				"VersionControl.GetCachedStatus");
			// Fetch status
...
static AssetList GetStatusCachedIfPossible(List<string> fromPaths,
                                           bool synchronous) {
...
static AssetList GetStatusForceUpdate(List<string> fromPaths) {
	...
	result.AddRange(fromPaths.Select(
				path => taskResultList.SingleOrDefault(a => a.path == path)));
	return result;
}
...
public static AssetMoveResult OnWillMoveAsset(string from, string to) {
	...
	if (InternalEditorUtility.isHumanControllingUs &&
	    ...
	    !EditorUtility.DisplayDialog(
		    "Confirm version control operation",
		    L10n.Tr(
			    $"You are about to move or rename a folder that is under version control.\n\nFrom:\t{from}\nTo:\t{to}\n\nAre you sure you want to perform this action?"),
		    "Yes", "No")) {
	...
	Asset.States states = Asset.States.OutOfSync |
	                      Asset.States.DeletedRemote |
	                      Asset.States.CheckedOutRemote;

	bool userAllowedMove = false;
	if (Asset.IsState(assetState, states) || Asset.IsState(metaState, states))
		userAllowedMove =
			AllowUserOverrideMovingUnsyncedFiles(asset, metaAsset);

	if (Asset.IsState(assetState, Asset.States.OutOfSync) &&
	    !userAllowedMove) {
		Debug.LogError(
			"Cannot move version controlled file that is not up to date. Please get latest changes from server");
		return AssetMoveResult.FailedMove;
	} else if (Asset.IsState(assetState, Asset.States.DeletedRemote) &&
	           !userAllowedMove) {
		Debug.LogError(
			"Cannot move version controlled file that is deleted on server. Please get latest changes from server");
		return AssetMoveResult.FailedMove;
	} else if (Asset.IsState(assetState, Asset.States.CheckedOutRemote) &&
	           !userAllowedMove) {
		Debug.LogError(
			"Cannot move version controlled file that is checked out on server. Please get latest changes from server");
		return AssetMoveResult.FailedMove;
	} else if (Asset.IsState(assetState, Asset.States.LockedRemote)) {
		Debug.LogError(
			"Cannot move version controlled file that is locked on server. Please get latest changes from server");
		return AssetMoveResult.FailedMove;
	}
	...
	return task.success ? (AssetMoveResult) task.resultCode
...
static bool AllowUserOverrideMovingUnsyncedFiles(Asset asset,
                                                 Asset metaAsset) {
	...
	if (asset.IsState(Asset.States.OutOfSync) |
	    metaAsset.IsState(Asset.States.OutOfSync))
		AppendStateToString(ref state, "Out of Sync");
	if (asset.IsState(Asset.States.DeletedRemote) |
	    metaAsset.IsState(Asset.States.DeletedRemote))
		AppendStateToString(ref state, "Deleted remotely");
	if (asset.IsState(Asset.States.CheckedOutRemote) |
	    metaAsset.IsState(Asset.States.CheckedOutRemote))
		AppendStateToString(ref state, "Checked out remotely");

	string title = "Confirm move";
	string message =
		"The files you are trying to move or rename are " + state +
		". This may cause synchronization issues, would you like to proceed anyway?";
...
public static AssetDeleteResult
OnWillDeleteAsset(string assetPath, RemoveAssetOptions option) {
	if (!Provider.enabled || EditorUserSettings.WorkOffline)
	...
	if (task.success) {
		...
		return File.Exists(assetPath) ? AssetDeleteResult.DidNotDelete
		                      : AssetDeleteResult.DidDelete;
	}
	return AssetDeleteResult.FailedDelete;
}

// NOTE: this now assumes that version control is on and we are not working
// offline. Also all paths are expected to be versioned
public static void OnWillDeleteAssets(string[] assetPaths,
                                      AssetDeleteResult[] deletionResults,
                                      RemoveAssetOptions option) {
	Assert.IsTrue(deletionResults.Length == assetPaths.Length);

	// NOTE: we only submit assets for deletion in batches because PlasticSCM
	// will time out the
	// connection to the provider process with too many assets
	int deletionBatchSize = 1000;
	for (int batchStart = 0; batchStart < assetPaths.Length;
	     batchStart += deletionBatchSize) {
		var deleteAssetList = new AssetList();
		for (int i = batchStart;
		     i < batchStart + deletionBatchSize && i < assetPaths.Length; i++)
		...
		if (task.success) {
			for (int i = batchStart; i < batchStart + deleteAssetList.Count();
			     i++)
				deletionResults[i] = File.Exists(assetPaths[i])
				     ? AssetDeleteResult.DidNotDelete
				     : AssetDeleteResult.DidDelete;
		} else {
			// NOTE: we most likely don't know which assets failed to actually be
			// deleted
			for (int i = batchStart; i < batchStart + deleteAssetList.Count();
			     i++)
...
public static bool IsOpenForEdit(string assetPath, out string message,
	...
	Asset asset;
	if (statusOptions == StatusQueryOptions.UseCachedIfPossible ||
	    statusOptions == StatusQueryOptions.UseCachedAsync)
		asset = GetStatusCachedIfPossible(
			assetPath, statusOptions == StatusQueryOptions.UseCachedIfPossible);
	else
		asset = GetStatusForceUpdate(assetPath);

	if (asset == null) {
		if (Provider.onlineState == OnlineState.Offline &&
		    Provider.offlineReason != string.Empty)
...
internal static void IsOpenForEdit(List<string> assetPaths,
                                   List<string> outNotOpenPaths,
                                   StatusQueryOptions statusOptions) {
	if (!Provider.enabled || EditorUserSettings.WorkOffline ||
	    assetPaths == null || assetPaths.Count == 0)
		return; // everything is editable

	// paths that are empty/null are considered to be editable, so remove them
	// from consideration
	...
	if (statusOptions == StatusQueryOptions.UseCachedIfPossible ||
	    statusOptions == StatusQueryOptions.UseCachedAsync)
		assets = GetStatusCachedIfPossible(
			assetPaths,
