...
namespace UnityEditor.VersionControl {
  public partial class Asset {
    ...
    internal bool IsUnderVersionControl {
      get {
        return IsState(Asset.States.Synced) ||
               IsState(Asset.States.OutOfSync) ||
               IsState(Asset.States.AddedLocal);
      }
    }
    ...
    internal static string StateToString(States state) {
      ...
      if (IsState(state, States.CheckedOutLocal) &&
          !IsState(state, States.LockedLocal))
        return "Checked Out Local";

      if (IsState(state, States.CheckedOutRemote) &&
          !IsState(state, States.LockedRemote))
      ...
      if (IsState(state, States.Local) &&
          !(IsState(state, States.OutOfSync) || IsState(state, States.Synced)))
        return "Local";
