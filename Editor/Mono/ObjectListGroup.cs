...
internal partial class ObjectListArea {
  /* Abstract base class for each group of assets (e.g. Local, AssetStore) used
   * in the ObjectListArea
   */
  abstract class Group {
    readonly protected float kGroupSeparatorHeight =
    ...
    abstract public bool ListMode {
      get;
      set;
    }
    abstract public bool NeedsRepaint {
      get;
      protected set;
    }
    ...
    abstract protected void DrawInternal(int itemIdx, int endItem,
                                         float yOffset);

    // Called when the filter has changed
    abstract public void UpdateFilter(HierarchyType hierarchyType,
                                      SearchFilter searchFilter,
                                      bool showFoldersFirst);

    protected virtual float GetHeaderHeight() { return kGroupSeparatorHeight; }
    ...
    int FirstVisibleRow(float yOffset, Vector2 scrollPos) {
      ...
      if (yRelOffset > 0f) {
        ...
        invisibleRows =
            (int) Mathf.Max(0, Mathf.Floor(yRelOffset / itemHeight));
      }
    ...
    public void Draw(float yOffset, Vector2 scrollPos, ref int rowsInUse) {
      ...
      // We need to always draw the header as it uses controlIDs (and we cannot
      // cull gui elements using controlID)
      ...
      if (beginItem >= 0 && beginItem < totalItemCount) {
        ...
        // (plus an extra row to allow half a row in top and bottom at the same
        // time)
        ...
        float itemHeight = m_Grid.itemSize.y + m_Grid.verticalSpacing;
        int rowsInVisibleRect =
            (int) Math.Ceiling(m_Owner.m_VisibleRect.height / itemHeight);

        // When a row is hidden behind the header, it is still counted as
        // visible, therefore to avoid weird popping in and out for the icons, we
        // make sure that a new row will be rendered even if one is considered
        // visible, even though it cannot be seen in the window
        rowsInVisibleRect += 1;
        ...
        rowsInUse = Math.Min(
            rowsInVisibleRect,
            Mathf.CeilToInt((endItem - beginItem) / (float) m_Grid.columns));
      ...
      // Always handle drag events in the case where we have no items we still
      // want to be able to drag into the group.
    ...
    protected void DrawDropShadowOverlay(Rect position, bool selected,
                                         bool isDropTarget, bool isRenaming) {
      // Draw dropshadow overlay
      float fraction = position.width / 128f;
      Rect dropShadowRect =
          new Rect(position.x - 4 * fraction, position.y - 2 * fraction,
                   position.width + 8 * fraction,
                   position.height + 12 * fraction - 0.5f);
      s_Styles.iconDropShadow.Draw(dropShadowRect, GUIContent.none, false,
                                   false, selected || isDropTarget,
    ...
    protected void DrawHeaderBackground(Rect rect, bool firstHeader,
      ...
      (firstHeader ? s_Styles.groupHeaderTop
       : s_Styles.groupHeaderMiddle)
          ?.Draw(rect, GUIContent.none,
                 rect.Contains(Event.current.mousePosition), false, expanded,
                 false);
    }
    ...
    virtual protected void DrawHeader(float yOffset, bool collapsable) {
      ...
      Rect rect =
          new Rect(0, GetHeaderYPosInScrollArea(yOffset),
                   m_Owner.GetVisibleWidth(), kGroupSeparatorHeight - 1);
      ...
      if (collapsable) {
        ...
        Visible =
            GUI.Toggle(rect, Visible, GUIContent.none, s_Styles.groupFoldout);
    ...
    protected void DrawItemCount(Rect rect) {
      ...
      Vector2 labelDims =
          s_Styles.groupHeaderLabelCount.CalcSize(new GUIContent(label));
      if (labelDims.x < rect.width)
        rect.x = m_Owner.GetVisibleWidth() - labelDims.x -
                 rightMargin; // right align if room
