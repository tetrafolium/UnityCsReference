...
static InspectorTitles() {
	var addComponentMenuTypes =
		TypeCache.GetTypesWithAttribute<AddComponentMenu>();

	s_InspectorTitles =
		new Dictionary<Type, string>(addComponentMenuTypes.Count);

	foreach (var type in addComponentMenuTypes) {
		var attr = type.GetCustomAttributes(typeof(AddComponentMenu), false)
		           .FirstOrDefault() as AddComponentMenu;
		if (attr == null)
			continue;
...
private static string GetObjectTypeName([ NotNull ] Object o) {
	...
	// Importers don't have names. Just show the type without parenthesis (like
...
public static string GetInspectorTitle(Object obj) {
	if (obj == null && (object) obj != null &&
	    (obj is MonoBehaviour || obj is ScriptableObject))
...
// Like GetClassName but handles folders, scenes, GUISkins, and other default
// assets as separate types.
internal static string GetTypeName(Object obj) {
	// Return "Object" when null so we have a icon in the inspector(null does
	// not have an icon). case 707513.
	if (obj == null)
		return "Object";
