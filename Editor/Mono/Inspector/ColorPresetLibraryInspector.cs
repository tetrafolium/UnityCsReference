...
private GenericPresetLibraryInspector<ColorPresetLibrary>
m_GenericPresetLibraryInspector;

public void OnEnable() {
	m_GenericPresetLibraryInspector =
		new GenericPresetLibraryInspector<ColorPresetLibrary>(
			target, "Color Preset Library", OnEditButtonClicked);
	m_GenericPresetLibraryInspector.useOnePixelOverlappedGrid = true;
	m_GenericPresetLibraryInspector.maxShowNumPresets =
		2000; // does not use a preview cache so show many
}
...
public override void OnInspectorGUI() {
	...
	m_GenericPresetLibraryInspector.itemViewMode =
		PresetLibraryEditorState.GetItemViewMode(
			ColorPicker.presetsEditorPrefID); // ensure in-sync
