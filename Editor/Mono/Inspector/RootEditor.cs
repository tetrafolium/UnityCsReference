...
internal static class RootEditorUtils {
  ...
  private static readonly List<RootEditorDesc> kSRootEditor =
      new List<RootEditorDesc>();

  static RootEditorUtils() { Rebuild(); }

  internal static bool SupportsAddComponent(Editor[] editors) {
    if (editors.Length != 1)
      return true;

    return kSRootEditor.All(desc => desc.rootEditorType != editors [0]
                                                               .GetType());
  }
  ...
  internal static void Rebuild() {
    kSRootEditor.Clear();
    var rootEditorMethods =
        AttributeHelper.GetMethodsWithAttribute<RootEditorAttribute>(
            BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Static |
            BindingFlags.DeclaredOnly);
    foreach (var method in rootEditorMethods.methodsWithAttributes) {
      var callback =
          Delegate
              .CreateDelegate(typeof(RootEditorAttribute.RootEditorHandler),
                              method.info)
                  as RootEditorAttribute.RootEditorHandler;
      if (callback != null) {
        var attr = method.attribute as RootEditorAttribute;
        kSRootEditor.Add(new RootEditorDesc(){needsRootEditor = callback,
                                              supportsAddComponent =
                                                  attr.supportsAddComponent});
      }
    }
  }
}
