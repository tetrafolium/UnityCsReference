...
public override void OnEnable() {
	...
	m_ShowEdgeRadius.value =
		targets.Count(x =>(x as CompositeCollider2D).geometryType ==
		              CompositeCollider2D.GeometryType.Polygons) == 0;
	m_ShowEdgeRadius.valueChanged.AddListener(Repaint);
	m_ShowManualGenerationButton.value =
		targets.Count(x =>(x as CompositeCollider2D).generationType !=
		              CompositeCollider2D.GenerationType.Manual) == 0;
...
public override void OnInspectorGUI() {
	...
	m_ShowManualGenerationButton.target =
		targets.Count(x =>(x as CompositeCollider2D).generationType !=
		              CompositeCollider2D.GenerationType.Manual) == 0;
	if (EditorGUILayout.BeginFadeGroup(m_ShowManualGenerationButton.faded)) {
		if (GUILayout.Button("Regenerate Collider")) {
			foreach (var composite in targets) {
				...
				// Case 1189438: Set dirty for each composite to register a change in
				// target's scene
	...
	m_ShowEdgeRadius.target =
		targets.All(x =>(x as CompositeCollider2D).geometryType !=
	...
	if (targets.Count(
		    x =>(x as CompositeCollider2D).geometryType ==
		    CompositeCollider2D.GeometryType.Outlines &&
		    (x as CompositeCollider2D).attachedRigidbody != null &&
		    (x as CompositeCollider2D).attachedRigidbody.bodyType ==
		    RigidbodyType2D.Dynamic) > 0)
		EditorGUILayout.HelpBox(
			"Outline geometry is composed of edges and will not preserve the original collider's center-of-mass or rotational inertia.  The CompositeCollider2D is attached to a Dynamic Rigidbody2D so you may need to explicitly set these if they are required.",
			MessageType.Info);
