...
class LineRendererPositionsView : TreeView {
  ...
  public LineRendererPositionsView(SerializedProperty positions)
    ...
    MultiColumnHeaderState.Column[] columns =
        new MultiColumnHeaderState.Column[4];
  ...
  protected void CellGUI(Rect cellRect, TreeViewItem item, int col,
    ...
    if (col == 0) {
      ...
      var newVal = EditorGUI.DoFloatField(EditorGUI.s_RecycledEditor, cellRect,
                                          dragRect, id, prop.floatValue, "g7",
                                          EditorStyles.numberField, true);
  ...
  int GetArraySize() {
    ...
    if (m_Positions.serializedObject.isEditingMultipleObjects) {
      foreach (LineRenderer lineRend in m_Positions.serializedObject
                   .targetObjects) {
        arraySize = Mathf.Min(arraySize, lineRend.positionCount);
        m_Positions.serializedObject.maxArraySizeForMultiEditing =
            Mathf.Max(m_Positions.serializedObject.maxArraySizeForMultiEditing,
  ...
  protected override
    ...
    // Check if we can handle the current drag data (could be dragged in from
    // other areas/windows in the editor)
    ...
    if (args.performDrop) {
      if (m_Positions.hasMultipleDifferentValues) {
        if (!EditorUtility.DisplayDialog(
                L10n.Tr(
                    "Moving an array element will copy the complete array to all other selected objects."),
                L10n.Tr(
      ...
      for (int i = 0; i < arraySize; ++i) {
        if (i == args.insertAtIndex) {
          ...
          foreach (var viewItem in draggedRows) {
            newList.Add(
                m_Positions.GetArrayElementAtIndex(viewItem).vector3Value);
          }
        }
      ...
      SetSelection(
          Enumerable
              .Range(args.insertAtIndex -
                         draggedRows.Count(o => o < args.insertAtIndex),
                     draggedRows.Count)
              .ToList());
    }
