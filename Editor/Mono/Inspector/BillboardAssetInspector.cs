...
internal class BillboardAssetInspector : Editor {
  ...
  private class GUIStyles {
    public readonly GUIContent m_Shaded =
        EditorGUIUtility.TrTextContent("Shaded");
    public readonly GUIContent m_Geometry =
        EditorGUIUtility.TrTextContent("Geometry");
  ...
  public override void OnPreviewSettings() {
    ...
    var rect = GUILayoutUtility.GetRect(content, Styles.m_DropdownButton,
                                        GUILayout.Width(75));

    if (EditorGUI.DropdownButton(rect, content, FocusType.Passive,
                                 Styles.m_DropdownButton)) {
      GUIUtility.hotControl = 0;
      var dropDownMenu = new GenericMenu();
      dropDownMenu.AddItem(Styles.m_Shaded, m_PreviewShaded,
                           () => m_PreviewShaded = true);
      dropDownMenu.AddItem(Styles.m_Geometry, !m_PreviewShaded,
                           () => m_PreviewShaded = false);
  ...
  public override string GetInfoString() {
    string info =
        String.Format("{0} verts, {1} tris, {2} images", m_Vertices.arraySize,
                      m_Indices.arraySize / 3, m_Images.arraySize);
    return info;
  }

  // Make a mesh out of the BillboardAsset that is used for rendering by
  // SpeedTree billboard shader. Vertices are expanded by the shader therefore
  // the mesh vertices are not used (filled by zeroes).
  internal static void MakeRenderMesh(Mesh mesh, BillboardAsset billboard) {
    mesh.SetVertices(
        Enumerable.Repeat(Vector3.zero, billboard.vertexCount).ToList());
    mesh.SetColors(
        Enumerable.Repeat(Color.black, billboard.vertexCount).ToList());
    mesh.SetUVs(0, billboard.GetVertices().ToList());
    mesh.SetUVs(1, Enumerable
                       .Repeat(new Vector4(1.0f, 1.0f, 0.0f, 0.0f),
                               billboard.vertexCount)
                       .ToList());
    mesh.SetTriangles(billboard.GetIndices().Select(v =>(int) v).ToList(), 0);
  }

  // Make a mesh out of the BillboardAsset that is suitable for previewing the
  // geometry. The vertices are expanded and made double-face.
  private void MakePreviewMesh(Mesh mesh, BillboardAsset billboard) {
    ...
    mesh.SetVertices(
        Enumerable
            .Repeat(billboard.GetVertices().Select(
                        v => new Vector3((v.x - 0.5f) * width,
                                         v.y * height + bottom, 0)),
    ...
    // (0,0,1) for the front-facing vertices and (0,0,-1) for the back-facing
    // vertices
    mesh.SetNormals(
        Enumerable.Repeat(Vector3.forward, billboard.vertexCount)
            .Concat(Enumerable.Repeat(-Vector3.forward, billboard.vertexCount))
            .ToList());
  ...
  private void DoRenderPreview(bool shaded) {
    ...
    Bounds bounds = new Bounds(
        ...
        new Vector3(m_Width.floatValue, m_Height.floatValue,
                    m_Width.floatValue));
    ...
    var rotation =
        Quaternion.Euler(-m_Settings.previewDir.y, -m_Settings.previewDir.x, 0);
    ...
    m_PreviewUtility.lights[0].transform.rotation =
        rotation * Quaternion.Euler(40f, 40f, 0);
    ...
    if (shaded) {
      ...
      billboard.MakeMaterialProperties(m_ShadedMaterialProperties,
                                       m_PreviewUtility.camera);
      m_Settings.activeMaterial = billboard.material;
      ModelInspector.RenderMeshPreviewSkipCameraAndLighting(
          m_ShadedMesh, bounds, m_PreviewUtility, m_Settings,
          m_ShadedMaterialProperties, -1);
    } else {
      MakePreviewMesh(m_GeometryMesh, billboard);
      m_Settings.activeMaterial = m_GeometryMaterial;
      ModelInspector.RenderMeshPreviewSkipCameraAndLighting(
          m_GeometryMesh, bounds, m_PreviewUtility, m_Settings, null, -1);
    }
