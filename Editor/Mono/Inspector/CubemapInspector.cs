...
internal class CubemapInspector : TextureInspector {
  static private readonly string[] kSizes = {"16",  "32",  "64",   "128",
                                             "256", "512", "1024", "2048"};
  static private readonly int[] kSizesValues = {16,  32,  64,   128,
                                                256, 512, 1024, 2048};
    ...
    if (c != null) {
      ...
      for (int i = 0; i < m_Images.Length; ++i) {
        ...
        if (TextureUtil.GetSourceTexture(c, (CubemapFace) i)) {
          ...
          m_Images[i] = new Texture2D(kTextureSize, kTextureSize,
                                      TextureFormat.RGBA32, false);
          m_Images[i].hideFlags = HideFlags.HideAndDontSave;
          TextureUtil.CopyCubemapFaceIntoTexture(c, (CubemapFace) i,
                                                 m_Images[i]);
        }
      }
    }
  ...
  public override void OnInspectorGUI() {
    ...
    EditorGUILayout.HelpBox(
        "Lowering face size is a destructive operation, you might need to re-assign the textures later to fix resolution issues. It's preferable to use Cubemap texture import type instead of Legacy Cubemap assets.",
        MessageType.Warning);
    int faceSize = TextureUtil.GetGPUWidth(c);
    faceSize =
        EditorGUILayout.IntPopup("Face size", faceSize, kSizes, kSizesValues);
    ...
    if (useMipMap) {
      streamingMipmaps = EditorGUILayout.Toggle(
          EditorGUIUtility.TrTextContent(
              "Streaming Mipmaps",
              "Don't load image data immediately but wait till image data is requested from script."),
          streamingMipmaps);
    }
    ...
    if (EditorGUI.EndChangeCheck()) {
      ...
      if (TextureUtil.ReformatCubemap(c, faceSize, faceSize, c.format,
                                      useMipMap, linear))
        InitTexturesFromCubemap();
  ...
  private void ShowFace(string label, CubemapFace face) {
    ...
    var tex = (Texture2D)
        ObjectField(label, m_Images[iface], typeof(Texture2D), false);
    if (GUI.changed) {
      TextureUtil.CopyTextureIntoCubemapFace(tex, c, face);
      // enable this line in order to retain connections from cube faces to
      // their corresponding texture2D assets, this allows auto-update
      // functionality when editing the source texture images
  ...
  public static Object ObjectField(string label, Object obj,
                                   System.Type objType, bool allowSceneObjects,
                                   params GUILayoutOption[] options) {
    GUILayout.BeginHorizontal();
    Rect r = GUILayoutUtility.GetRect(
        EditorGUIUtility.labelWidth, EditorGUI.kSingleLineHeight * 2,
        EditorStyles.label, GUILayout.ExpandWidth(false));
    GUI.Label(r, label, EditorStyles.label);
    r = GUILayoutUtility.GetAspectRect(1, EditorStyles.objectField,
                                       GUILayout.Width(64));
