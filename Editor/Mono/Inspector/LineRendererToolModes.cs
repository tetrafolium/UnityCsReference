...
internal class LineRendererEditor : IEditablePoint, ICreatablePoint {
  public enum InputMode { MousePosition, PhysicsRaycast }
  ...
  public static float createPointSeparation {
    get {
      return EditorPrefs.GetFloat("LineRendererEditorCreatePointSeparation",
                                  1.0f);
    }
    set {
      EditorPrefs.SetFloat("LineRendererEditorCreatePointSeparation",
                           value < 0 ? 0 : value);
    }
  }
  ...
  public static float creationOffset {
    get {
      return EditorPrefs.GetFloat("LineRendererEditorCreationOffset", 5.0f);
    }
  ...
  public static InputMode inputMode {
    get {
      return (InputMode) EditorPrefs.GetInt("LineRendererEditorInputMode", 0);
    }
  ...
  public static bool showWireframe {
    get {
      return EditorPrefs.GetBool("LineRendererEditorShowWireFrame", false);
    }
  ...
  public LineRendererEditor(LineRenderer lineRenderer,
                            LineRendererInspector inspector) {
    m_LineRenderer = lineRenderer;
    m_Inspector = inspector;
  }

  public void Deselect() { m_Selection.Clear(); }
  ...
  public void HandleEditMenuHotKeyCommands() {
    ...
    if (Event.current.type == EventType.ValidateCommand ||
      ...
      case "Copy":
      case "Paste":
        // We need to capture these events to prevent being taken out of Edit
        // mode by accident. The copy paste would trigger the Inspector and
        // duplicate the GameObject instead.
        Event.current.Use();
        break;
      }
    }
  ...
  public void DestroySelected() {
    ...
    var newPositions =
        new Vector3[m_LineRenderer.positionCount - m_Selection.Count];
  ...
  void DrawWireframe() {
    ...
    Vector3[] positions =
        new Vector3 [m_LineRenderer.loop ? m_LineRenderer.positionCount +
            1:m_LineRenderer.positionCount];
    m_LineRenderer.GetPositions(positions);
  ...
  public void CreateSceneGUI() {
    PointCreator.CreatePoints(this, true, inputMode == InputMode.PhysicsRaycast,
                              raycastMask, creationOffset,
                              createPointSeparation);
    PointCreator.Draw();
  }

  public void EditSceneGUI() {
    Transform transform =
        m_LineRenderer.useWorldSpace ? null : m_LineRenderer.transform;
  ...
  public int Count {
    get { return m_LineRenderer.positionCount; }
  }
  ...
  public float GetPointScale() { return 5.0f * AnnotationUtility.iconSize; }
  ...
  private Bounds GetBounds(List<Vector3> positions) {
    ...
    if (positions.Count == 1) {
      return new Bounds(
          transform !=
              null ? m_LineRenderer.transform.TransformPoint(positions[0])
          : positions[0], new Vector3(1f, 1f, 1f));
    }

    return GeometryUtility.CalculateBounds(
        positions.ToArray(), transform != null ? transform.localToWorldMatrix
        : Matrix4x4.identity);
  }
  ...
  public void AddPositions(List<Vector3> newPositions) {
    ...
    var positions =
        new Vector3[m_LineRenderer.positionCount + newPositions.Count];
    ...
    for (; writePos < positions.Length; ++writePos, ++readPos) {
      var transformedPos =
          m_LineRenderer.useWorldSpace ? newPositions[readPos]
          : worldToLocal.MultiplyPoint(newPositions[readPos]);
