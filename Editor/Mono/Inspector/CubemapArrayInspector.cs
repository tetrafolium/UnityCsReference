...
internal class CubemapArrayInspector : TextureInspector {
  ...
  static class Styles {
    public static readonly GUIContent slice =
        EditorGUIUtility.TrTextContent("Slice", "Displayed array slice");
  ...
  public override void OnPreviewSettings() {
    ...
    m_Slice = EditorGUILayout.IntSlider(m_Slice, 0, t.cubemapCount - 1,
                                        GUILayout.Width(120));
    ...
    GUILayout.Box(EditorGUIUtility.IconContent("PreTextureMipMapLow"),
                  Styles.toolbarLabel);
    m_Mip = Mathf.RoundToInt(GUILayout.HorizontalSlider(
        m_Mip, m_MipCount - 1, 0, GUILayout.Width(64)));
    GUILayout.Box(EditorGUIUtility.IconContent("PreTextureMipMapHigh"),
                  Styles.toolbarLabel);
    EditorGUI.EndDisabledGroup();
  ...
  public override void OnPreviewGUI(Rect r, GUIStyle background) {
    if (!SystemInfo.supportsCubemapArrayTextures ||
        (m_Material != null && !m_Material.shader.isSupported)) {
      if (Event.current.type == EventType.Repaint)
        EditorGUI.DropShadowLabel(new Rect(r.x, r.y, r.width, 40),
                                  "Cubemap array preview is not supported");
      return;
    }
    ...
    Quaternion rot = Quaternion.Euler(m_PreviewDir.y, 0, 0) *
  ...
  public override Texture2D RenderStaticPreview(string assetPath,
                                                ...
                                                int height) {
    // It's not clear what a meaningful preview for a CubemapArray would be -
    // the first slice? Multiple slices composed? Until we have a clear idea
    // about the best way to do things, return null for now, to indicate no
    // preview.
    return null;
  }
}
