...
internal class BodyMaskEditor {
  static class Styles {
    public static GUIContent UnityDude =
        EditorGUIUtility.IconContent("AvatarInspector/BodySIlhouette");
    public static GUIContent PickingTexture =
        EditorGUIUtility.IconContent("AvatarInspector/BodyPartPicker");
  ...
  public static void Show(SerializedProperty bodyMask, int count) {
    if (Styles.UnityDude.image) {
      Rect rect = GUILayoutUtility.GetRect(
          Styles.UnityDude, GUIStyle.none,
          GUILayout.MaxWidth(Styles.UnityDude.image.width));
      rect.x += (GUIView.current.position.width - rect.width) / 2;

      Color oldColor = GUI.color;

      GUI.color = bodyMask.GetArrayElementAtIndex(0).intValue == 1 ? Color.green
                                                                   : Color.red;
      ...
      for (int i = 1; i < count; i++) {
        GUI.color = bodyMask.GetArrayElementAtIndex(i).intValue == 1
                        ? Color.green
                        : Color.red;
  ...
  protected static void DoPicking(Rect rect, SerializedProperty bodyMask,
                                  int count) {
    if (Styles.PickingTexture.image) {
      ...
      case EventType.MouseDown: {
        if (rect.Contains(evt.mousePosition)) {
          ...
          int left = Styles.UnityDude.image.height -
                     ((int) evt.mousePosition.y - (int) rect.y);
          ...
          for (int i = 0; i < count; i++) {
            if (m_MaskBodyPartPicker[i] == color) {
              ...
              bodyMask.GetArrayElementAtIndex(i).intValue =
                  bodyMask.GetArrayElementAtIndex(i).intValue == 1 ? 0 : 1;
...
internal class AvatarMaskInspector : Editor {
  private static class Styles {
    ...
    public static GUIContent MaskDefinition = EditorGUIUtility.TrTextContent(
        "Definition",
        "Choose between Create From This Model, Copy From Other Avatar. The first one creates a Mask for this file and the second one uses a Mask from another file to import animation.");
    public static GUIContent[] MaskDefinitionOpt = {
        EditorGUIUtility.TrTextContent(
            "Create From This Model",
            "Create a Mask based on the model from this file. For Humanoid rig all the human transforms are always imported and converted to muscle curve, thus they cannot be unchecked."),
        EditorGUIUtility.TrTextContent(
            "Copy From Other Mask",
            "Copy a Mask from another file to import animation clip."),
        EditorGUIUtility.TrTextContent("None ", " Import Everything")};
    public static GUIContent CopyFromOtherSource = EditorGUIUtility.TrTextContent(
        "Source",
        "Select from which AvatarMask the animation should take the mask information");
    public static GUIContent CreateMask = EditorGUIUtility.TrTextContent(
        "Create Mask", "Create a new mask from this model avatar.");

    // Avatar mask options
    public static GUIContent SelectAvatarReference = EditorGUIUtility.TrTextContent(
        "Use skeleton from",
        "The selected avatar is never linked here and only used to populate the list of transform.");
    public static GUIContent ImportAvatarReference =
        EditorGUIUtility.TrTextContent(
            "Import skeleton",
            "Generates new transform data based on the selected avatar skeleton");

    // Avatar mask foldouts
    public static GUIContent BodyMask = EditorGUIUtility.TrTextContent(
        "Humanoid",
        "Define which body part are active. Also define which animation curves will be imported for an Animation Clip.");
    public static GUIContent TransformMask = EditorGUIUtility.TrTextContent(
        "Transform",
        "Define which transform are active. Also define which animation curves will be imported for an Animation Clip.");

    // TreeView columns
    public static GUIContent TransformName =
        EditorGUIUtility.TrTextContent("Node Name");
    public static GUIContent EnableName = EditorGUIUtility.TrTextContent(
        "Use", "Maintain Alt/Option key to enable or disable all children");
  }
  ...
  // The TreeView is not serializable, so it should be reconstructed from the
  // tree data.
  ...
  private void OnEnable() {
    // The AvatarMaskInspector is instantiated in the ModelImporterClipEditor,
    // which does not derive from the Editor class anymore. All editors are
    // added to a list so that OnEnable() and other methods can be called on
    // them at certain times. When going in game while having settings pending
    // to be applied, at some point the target (ModelImporterClipEditor.m_Mask)
    // becomes null, since the parent becomes updated before this class.
    if (target == null) {
      return;
    }
  ...
  public void UpdateTransformInfos() { m_SimpleTreeView.Reload(); }
    ...
    get {
      if (animationType == ModelImporterAnimationType.Human &&
        ...
        if (m_HumanTransform == null) {
          ...
          m_HumanTransform = AvatarMaskUtility.GetAvatarHumanTransform(
              so, modelImporter.transformPaths);
        }
      } else
  ...
  void InspectorGUIWithClipInfo() {
    using(var change = new EditorGUI.ChangeCheckScope()) {
      ...
      EditorGUI.showMixedValue =
          clipInfo.maskTypeProperty.hasMultipleDifferentValues;
      maskType = EditorGUILayout.Popup(Styles.MaskDefinition, maskType,
                                       Styles.MaskDefinitionOpt);
    ...
    var showCopyFromOtherGUI =
        clipInfo.maskType == ClipAnimationMaskType.CopyFromOther;
  ...
  void CopyFromOtherGUI() {
    ...
    EditorGUILayout.PropertyField(clipInfo.maskSourceProperty,
                                  Styles.CopyFromOtherSource);
    AvatarMask maskSource =
        clipInfo.maskSourceProperty.objectReferenceValue as AvatarMask;
  ...
  public bool IsMaskEmpty() { return !m_SimpleTreeView.rootNode.hasChildren; }
  ...
  private void UpdateMask(ClipAnimationMaskType maskType) {
    ...
    if (maskType == ClipAnimationMaskType.CreateFromThisModel) {
      ...
      AvatarMaskUtility.UpdateTransformMask(
          m_TransformMask, modelImporter.transformPaths, humanTransforms);
      ...
      ModelImporter.UpdateTransformMask(emptyMask,
  ...
  public void OnBodyInspectorGUI() {
    if (m_ShowBodyMask) {
      ...
      m_BodyMaskFoldout =
          EditorGUILayout.Foldout(m_BodyMaskFoldout, Styles.BodyMask, true);
  ...
  public void OnTransformInspectorGUI() {
    // Don't make toggling foldout cause GUI.changed to be true (shouldn't cause
    // undoable action etc.)
    bool wasChanged = GUI.changed;
    m_TransformMaskFoldout = EditorGUILayout.Foldout(
        m_TransformMaskFoldout, Styles.TransformMask, true);
    GUI.changed = wasChanged;
    if (m_TransformMaskFoldout) {
      if (canImport)
        ImportAvatarReference();

      if (m_SimpleTreeView.rootNode == null ||
          m_TransformMask.arraySize != m_SimpleTreeView.rootNode.DeepCount)
        UpdateTransformInfos();
      ...
      if (IsMaskEmpty()) {
        ...
        if (animationType == ModelImporterAnimationType.Generic) {
          ...
          message =
              "No transform mask defined, only human curves will be imported";
        } else {
        ...
        if (!canImport &&
            clipInfo.maskType == ClipAnimationMaskType.CreateFromThisModel) {
  ...
  private void ImportAvatarReference() {
    EditorGUI.BeginChangeCheck();
    m_RefAvatar =
        EditorGUILayout.ObjectField(Styles.SelectAvatarReference, m_RefAvatar,
                                    typeof(Avatar), target) as Avatar;
    if (EditorGUI.EndChangeCheck())
      m_RefImporter = AssetImporter.GetAtPath(
          AssetDatabase.GetAssetPath(m_RefAvatar)) as ModelImporter;

    using(new EditorGUI.DisabledScope(m_RefImporter == null)) {
      if (GUILayout.Button(Styles.ImportAvatarReference))
        AvatarMaskUtility.UpdateTransformMask(
            m_TransformMask, m_RefImporter.transformPaths, null);
    }
  }
  ...
  private SerializedNodeInfo FillNodeInfos() {
    var rootNode =
        new SerializedNodeInfo(){depth = -1, displayName = "", id = 0,
                                 children = new List<TreeViewItem>(0)};
    ...
    for (int i = 1; i < nodesCount; i++) {
      ...
      while (!string.IsNullOrEmpty(currentPath) &&
        ...
        // we are in a new node, lets unstack until we reach the correct
        // hierarchy
      ...
      if (m_CanImport) {
        ...
        //  Enable only transforms that are not human. Human transforms in this
        //  case are handled by muscle curves and cannot be imported.
        if (ArrayUtility.FindIndex(humanTransforms, s => newPath == s) == -1) {
          if (m_TransformPaths != null &&
              ArrayUtility.FindIndex(m_TransformPaths, s => newPath == s) == -1)
      ...
      } else if (m_TransformPaths != null &&
                 ArrayUtility.FindIndex(m_TransformPaths, s => newPath == s) ==
                     -1) {
    ...
    TreeViewUtility.SetChildParentReferences(
        nodeInfos.Cast<TreeViewItem>().ToList(), rootNode);
    return rootNode;
  }
  ...
  private void InitTreeView() {
    ...
    var columns =
        new[]{new MultiColumnHeaderState.Column{
                  headerContent = Styles.EnableName,
                  headerTextAlignment = TextAlignment.Center, canSort = false,
    ...
    m_SimpleTreeView = new AvatarMaskTreeView(m_TreeViewState,
                                              multiColumnHeader, FillNodeInfos);
  ...
  private class AvatarMaskTreeView : ToggleTreeView<SerializedNodeInfo> {
    public AvatarMaskTreeView(TreeViewState state,
                              MultiColumnHeader multiColumnHeader,
                              Func<SerializedNodeInfo> rebuildRoot)
        : base(state, multiColumnHeader, rebuildRoot) {}

    protected override void EnabledGUI(Rect cellRect, SerializedNodeInfo node,
                                       ref RowGUIArgs args) {
      var serializedNode = node;
      EditorGUI.BeginDisabled(serializedNode.m_State ==
                              SerializedNodeInfo.State.Disabled);
      base.EnabledGUI(cellRect, node, ref args);
      EditorGUI.EndDisabled();
    }

    protected override void NameGUI(Rect position, SerializedNodeInfo node,
                                    ref RowGUIArgs args) {
      var serializedNode = node;
      var color = GUI.contentColor;
      color = serializedNode.m_State ==
              SerializedNodeInfo.State.Invalid ? new Color(1f, 0f, 0f, 0.66f)
          : color;
