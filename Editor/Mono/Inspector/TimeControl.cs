  ...
  public float normalizedTime {
    ...
    get {
      return (stopTime == startTime)
                 ? 0
                 : ((currentTime - startTime) / (stopTime - startTime));
    }
  ...
  public void DoTimeControl(Rect rect) {
    ...
    case EventType.MouseDown:
      ...
      if (scrubberRect.Contains(evt.mousePosition)) {
        ...
        nextCurrentTime =
            (m_MouseDrag * (stopTime - startTime) / scrubberRect.width +
             startTime);
    ...
    case EventType.MouseDrag:
      if (EditorGUIUtility.hotControl == id) {
        ...
        // We want to not wrap if we immediately drag to the beginning, but we
        // do want to wrap if we drag past the end.
        if (loop && ((m_MouseDrag < 0.0f && m_WrapForwardDrag) ||
                     (m_MouseDrag > scrubberRect.width))) {
          // scrubing out of range was generating a big deltaTime in wrong time
          // direction this new code prevent this and it is compliant with new
          // and more robust v5.0 root motion looping of animation clip
        ...
        nextCurrentTime = (Mathf.Clamp(m_MouseDrag, 0.0f, scrubberRect.width) *
                               (stopTime - startTime) / scrubberRect.width +
                           startTime);
        evt.Use();
      }
      break;
    ...
    // Play/Pause Button
    playing = GUI.Toggle(timelineRect, playing, playing ? s_Styles.pauseIcon
                         : s_Styles.playIcon, s_Styles.playButton);

    // Current time indicator
    float normalizedPosition =
        Mathf.Lerp(scrubberRect.x, scrubberRect.xMax, normalizedTime);
    TimeArea.DrawPlayhead(normalizedPosition, scrubberRect.yMin,
                          scrubberRect.yMax, 2f,
                          (EditorGUIUtility.keyboardControl == id) ? 1f : 0.5f);
  }

  public void OnDisable() { playing = false; }
  ...
  public void Update() {
    ...
    if (!m_DeltaTimeSet) {
      if (playing) {
        ...
        deltaTime =
            (float)(timeSinceStartup - m_LastFrameEditorTime) * playbackSpeed;
