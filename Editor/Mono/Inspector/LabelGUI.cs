...
internal class LabelGUI {
  ...
  public void OnLostFocus() { SaveLabels(); }
  ...
  public void AssetLabelsChangedForObject(Object asset) {
    if (!m_IgnoreNextAssetLabelsChangedCall && m_CurrentAssetsSet != null &&
        m_CurrentAssetsSet.Contains(asset)) {
      InvalidateLabels(); // someone else changed the labels for one of our
                          // selected assets, so invalidate cache
    }
  ...
  public void SaveLabels() {
    if (m_CurrentChanged && m_AssetLabels != null &&
      ...
      foreach (var currentAsset in m_CurrentAssetsSet) {
        bool currentAssetWasChanged =
            false; // when multi-editing, some assets might e.g. already have
                   // the label that was added to all
  ...
  public void InitLabelCache(Object[] assets) {
    ...
    if (m_CurrentAssetsSet == null ||
        !m_CurrentAssetsSet.SetEquals(newAssetSet)) {
      List<string> all;
      List<string> partial;
      GetLabelsForAssets(assets, out all, out partial);

      m_AssetLabels = new PopupList.InputData{
          m_CloseOnSelection = false, m_AllowCustom = true,
          m_OnSelectCallback = AssetLabelListCallback, m_MaxCount = 15,
      ...
      foreach (var pair in allLabels) {
        ...
        element.selected =
            all.Any(label => string.Equals(label, pair.Key,
                                           StringComparison.OrdinalIgnoreCase));
        element.partiallySelected = partial.Any(
            label => string.Equals(label, pair.Key,
                                   StringComparison.OrdinalIgnoreCase));
      }
    }
  ...
  public void OnLabelGUI(Object[] assets) {
    ...
    float buttonWidth =
        labelButton.margin.left + labelButton.fixedWidth + rightPadding;
    ...
    Rect r =
        GUILayoutUtility.GetRect(labelButton.fixedWidth, labelButton.fixedWidth,
                                 labelButton.fixedHeight + bottomPadding,
                                 labelButton.fixedHeight + bottomPadding);
    r.x = widthProbeRect.xMax + labelButton.margin.left;
    if (EditorGUI.DropdownButton(r, GUIContent.none, FocusType.Passive,
                                 labelButton)) {
  ...
  private void DrawLabelList(bool partiallySelected, float xMax) {
    GUIStyle labelStyle =
        partiallySelected ? EditorStyles.assetLabelPartial
        : EditorStyles.assetLabel;
    Event evt = Event.current;
    foreach (GUIContent content in (
                 from i in m_AssetLabels
                     .m_ListElements where(
                         partiallySelected ? i.partiallySelected
                         : i.selected) orderby i.text.ToLower()
                         select i.m_Content)
                 .Take(s_MaxShownLabels)) {
      ...
      if (rt.xMax <= xMax && evt.type == EventType.MouseDown &&
          rt.Contains(evt.mousePosition) && evt.button == 0 && GUI.enabled) {
        evt.Use();
        rt.x = xMax;
  ...
  private void GetLabelsForAssets(Object[] assets, out List<string> all,
                                  out List<string> partial) {
    all = new List<string>();
    ...
    foreach (Object asset in assets) {
      ...
      foreach (string label in currentLabels) {
        labelAssetCount[label] =
            labelAssetCount.ContainsKey(label) ? labelAssetCount[label] + 1 : 1;
      }
    }
