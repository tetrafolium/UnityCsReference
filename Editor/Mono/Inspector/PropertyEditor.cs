...
bool useUIElementsDefaultInspector {
	get;
}
HashSet<int> editorsWithImportedObjectLabel {
	get;
}
Editor lastInteractedEditor {
	get;
	set;
}
GUIView parent {
...
internal const string k_AssetPropertiesMenuItemName =
	"Assets/Properties... _&P";
protected const string s_MultiEditClassName =
	"unity-inspector-no-multi-edit-warning";
protected const string s_EditorListClassName = "unity-inspector-editors-list";
protected const string s_AddComponentClassName =
	"unity-inspector-add-component-button";
...
internal const string s_MainContainerClassName =
...
protected const float k_InspectorPreviewMinTotalHeight =
	k_InspectorPreviewMinHeight + kBottomToolbarHeight;
...
[SerializeField]
protected List<Object> m_ObjectsLockedBeforeSerialization =
	new List<Object>();
[SerializeField]
protected List<int> m_InstanceIDsLockedBeforeSerialization = new List<int>();
[SerializeField]
protected PreviewResizer m_PreviewResizer = new PreviewResizer();
[SerializeField]
protected LabelGUI m_LabelGUI = new LabelGUI();
[SerializeField]
protected int m_LastInspectedObjectInstanceID = -1;
[SerializeField]
protected float m_LastVerticalScrollValue = 0;
[SerializeField]
...
protected VisualElement editorsElement =>
m_EditorsElement ??(m_EditorsElement = FindVisualElementInTreeByClassName(
			    s_EditorListClassName));
protected VisualElement m_RemovedPrefabComponentsElement;
protected VisualElement m_PreviewAndLabelElement;
protected VisualElement previewAndLabelElement => m_PreviewAndLabelElement ??(
	m_PreviewAndLabelElement =
		FindVisualElementInTreeByClassName(s_FooterInfoClassName));
protected VisualElement m_VersionControlElement;
protected VisualElement versionControlElement => m_VersionControlElement ??(
	m_VersionControlElement =
		FindVisualElementInTreeByClassName(s_HeaderInfoClassName));
protected static Dictionary<Editor, VersionControlBarState>
m_VersionControlBarState =
	new Dictionary<Editor, VersionControlBarState>();
...
public HashSet<int> editorsWithImportedObjectLabel {
	get;
}
        = new HashSet<int>();
public EditorDragging editorDragging {
	get;
}
public bool useUIElementsDefaultInspector {
	...
	internal set;
        ...
        = false;
public Editor lastInteractedEditor {
	get;
	set;
}
internal static PropertyEditor CurrentPropertyEditor {
	get;
	private set;
}
...
protected Rect bottomAreaDropRectangle {
	get {
		...
		var worldRootRect =
			rootVisualElement.LocalToWorld(rootVisualElement.rect);
		return new Rect(worldEditorRect.x,
		                worldEditorRect.y + worldEditorRect.height,
		                worldEditorRect.width,
		                worldRootRect.y + worldRootRect.height -
		                worldEditorRect.height - worldEditorRect.y);
	}
}
...
public static readonly GUIContent preTitle =
	EditorGUIUtility.TrTextContent("Preview");
public static readonly GUIContent labelTitle =
	EditorGUIUtility.TrTextContent("Asset Labels");
public static readonly GUIContent addComponentLabel =
	EditorGUIUtility.TrTextContent("Add Component");
public static GUIStyle preBackground = "preBackground";
public static GUIStyle footer = "IN Footer";
public static GUIStyle preMargins =
	new GUIStyle(){
	margin = new RectOffset(0, 0, 0, 4)
};
public static GUIStyle preOptionsButton =
	new GUIStyle(EditorStyles.toolbarButtonRight){
	padding = new RectOffset(), contentOffset = new Vector2(1, 0)
};
...
public static readonly GUIContent menuIcon =
	EditorGUIUtility.TrIconContent("_Menu");
public static GUIStyle previewMiniLabel = EditorStyles.whiteMiniLabel;
public static GUIStyle typeSelection = "IN TypeSelection";

public static readonly GUIContent vcsCheckoutHint =
	EditorGUIUtility.TrTextContent(
		"Under Version Control\nCheck out this asset in order to make changes.",
		EditorGUIUtility.GetHelpIcon(MessageType.Info));
public static readonly GUIContent vcsNotConnected =
	EditorGUIUtility.TrTextContent("VCS ({0}) is not connected");
public static readonly GUIContent vcsOffline =
	EditorGUIUtility.TrTextContent("Work Offline option is active");
public static readonly GUIContent vcsSettings =
	EditorGUIUtility.TrTextContent("Settings");
public static readonly GUIContent vcsCheckout =
	EditorGUIUtility.TrTextContent("Check Out");
public static readonly GUIContent vcsCheckoutMeta =
	EditorGUIUtility.TrTextContent("Check Out Meta");
public static readonly GUIContent vcsAdd =
	EditorGUIUtility.TrTextContent("Add");
public static readonly GUIContent vcsLock =
	EditorGUIUtility.TrTextContent("Lock");
public static readonly GUIContent vcsUnlock =
	EditorGUIUtility.TrTextContent("Unlock");
public static readonly GUIContent vcsSubmit =
	EditorGUIUtility.TrTextContent("Submit");
public static readonly GUIContent vcsRevert =
	EditorGUIUtility.TrTextContent("Revert");
public static readonly GUIContent vcsRevertUnchanged =
	EditorGUIUtility.TrTextContent("Revert Unchanged");
public static readonly GUIContent[] vcsRevertMenuNames = {
	vcsRevertUnchanged
};
public static readonly GenericMenu.MenuFunction2[] vcsRevertMenuActions = {
	DoRevertUnchanged
};
public static readonly GUIStyle vcsButtonStyle = EditorStyles.miniButton;
public static GUIStyle vcsRevertStyle =
	new GUIStyle(EditorStyles.dropDownList);
public static readonly GUIStyle vcsBarStyleOneRow = EditorStyles.toolbar;
public static GUIStyle vcsBarStyleTwoRows =
...
public static readonly string
        objectDisabledModuleWithDependencyWarningFormat = L10n.Tr(
...
public static SVC<float> lineSeparatorOffset =
	new SVC<float>("AC-Button", "--separator-line-top-offset");
public static SVC<Color> lineSeparatorColor =
	new SVC<Color>("--theme-line-separator-color", Color.red);
...
public int GetButtonCount() {
	...
	if (settings)
		++c;
	if (revert)
		++c; // revertUnchanged is same button in a drop-down
	if (checkout)
		++c;
	if (add)
		++c;
...
public static VersionControlBarState
Calculate(Editor assetEditor, Asset asset, bool connected) {
	...
	res.assets.AddRange(assetEditor.targets.Select(
				    o => Provider.GetAssetByPath(AssetDatabase.GetAssetPath(o))));
	...
	res.revertUnchanged =
		Provider.RevertIsValid(res.assets, RevertMode.Unchanged);

	bool checkoutBoth = assetEditor.target == null ||
	                    Provider.NeedToCheckOutBoth(assetEditor.target);
	res.checkout = isFolder || Provider.CheckoutIsValid(
		res.assets, checkoutBoth ? CheckoutMode.Both
		                     : CheckoutMode.Meta);
	res.add = Provider.AddIsValid(res.assets);
	res.submit = Provider.SubmitIsValid(null, res.assets);
	res.@lock = Provider.hasLockingSupport && !isFolder &&
	            Provider.LockIsValid(res.assets);
	res.unlock = Provider.hasLockingSupport && !isFolder &&
	             Provider.UnlockIsValid(res.assets);
	return res;
}
}
...
protected virtual void OnDisable() {
	...
	m_LastInspectedObjectInstanceID =
		GetInspectedObject()?.GetInstanceID() ?? -1;
...
protected virtual void Update() {
	...
	foreach (var myEditor in s_Editors) {
		if (myEditor != null && myEditor.RequiresConstantRepaint() &&
		    !myEditor.hideInspector)
			wantsRepaint = true;
	}
...
protected void SetTitle(Object obj) {
	...
	if (!String.IsNullOrEmpty(m_AssetGUID))
		...
		if (go)
			...
			titleTooltip =
				$"{EditorUtility.GetHierarchyPath(c.gameObject)} ({objTitle})";
	}

	titleContent = EditorGUIUtility.TrTextContentWithIcon(
		obj.name, titleTooltip, "UnityEditor.InspectorWindow");
...
FindVisualElementInTreeByClassName(string elementClassName) {
...
protected void LoadVisualTreeFromUxml() {
	var tpl = EditorGUIUtility.Load("UXML/InspectorWindow/InspectorWindow.uxml")
	          as VisualTreeAsset;
	var fContainer =
		rootVisualElement.Query(null, s_MainContainerClassName).First();
	...
	multiContainer.Query<TextElement>().ForEach((label) => label.text =
							    L10n.Tr(label.text));
	...
	rootVisualElement.AddStyleSheetPath(
		"StyleSheets/InspectorWindow/InspectorWindow.uss");
}

private void OnGeometryChanged(GeometryChangedEvent e) {
	if (m_PreviewResizer.GetExpanded()) {
		if (previewAndLabelElement.layout.height > 0 &&
		    rootVisualElement.layout.height <=
		    k_MinimumRootVisualHeight +
		    m_PreviewResizer.containerMinimumHeightExpanded) {
...
public virtual void AddItemsToMenu(GenericMenu menu) {
	if (IsAnyComponentCollapsed())
		menu.AddItem(EditorGUIUtility.TrTextContent("Expand All Components"),
		             false, ExpandAllComponents);
	else
		menu.AddDisabledItem(
			EditorGUIUtility.TrTextContent("Expand All Components"));

	if (IsAnyComponentExpanded())
		menu.AddItem(EditorGUIUtility.TrTextContent("Collapse All Components"),
		             false, CollapseAllComponents);
	else
		menu.AddDisabledItem(
			EditorGUIUtility.TrTextContent("Collapse All Components"));
	...
	menu.AddSeparator("");
	menu.AddItem(EditorGUIUtility.TrTextContent("Ping"), false,
	             () => EditorGUIUtility.PingObject(GetInspectedObject()));
}
...
protected bool IsAnyComponentCollapsed() {
	if (Selection.activeGameObject == null)
		return false; // If the selection is not a game object then disable the
	                      // option.
...
protected bool LoadPersistedObject() {
	...
	if (!String.IsNullOrEmpty(m_AssetGUID)) {
		...
		m_InspectedObject = FindObjectFromInstanceID(m_InstanceID)
		                    ?? ForceLoadFromInstanceID(m_InstanceID);
	}

	if (m_InspectedObject) {
		SetTitle(m_InspectedObject);
		m_Tracker.SetObjectsLockedByThisTracker(
			new List<Object> {
					m_InspectedObject
				});
	} else {
...
private void OnTrackerRebuilt() {
	...
	// tracker gets rebuilt when selection or objects change; make sure to
	// recalc which VCS buttons are shown
	ClearVersionControlBarState();
}

private void ExtractPrefabComponents() {
	m_LastInitialEditorInstanceID = m_Tracker
	                                .activeEditors [0]
	                                .GetInstanceID();
	...
	GameObject sourceGo =
		PrefabUtility.GetCorrespondingConnectedObjectFromSource(go);
	if (sourceGo == null)
		return;

	m_ComponentsInPrefabSource = sourceGo.GetComponents<Component>();
	var removedComponentsList =
		PrefabOverridesUtility.GetRemovedComponentsForSingleGameObject(go);
...
public IMGUIContainer CreateIMGUIContainer(Action onGUIHandler,
                                           string name = null) {
...
internal virtual void RebuildContentsContainers() {
	...
	var addComponentButton = rootVisualElement.Q(className
	                                             : s_AddComponentClassName);
	...
	if (editors.Any() && versionControlElement != null) {
		versionControlElement.Add(CreateIMGUIContainer(
						  () => VersionControlBar(
							  InspectorWindowUtils.GetFirstNonImportInspectorEditor(editors))));
	}

	DrawEditors(editors);

	var labelMustBeAdded =
		editorsElement != null && m_MultiEditLabel.parent != editorsElement;

	// The PrefabImporterEditor can hide its imported objects if it detects
	// missing scripts. In this case do not add the multi editing warning
	...
	if (tracker.hasComponentsWhichCannotBeMultiEdited) {
		if (editors.Length == 0 && !tracker.isLocked &&
		    Selection.objects.Length > 0 && editorsElement != null) {
	...
	if (addComponentButton != null && editors.Any() &&
	    RootEditorUtils.SupportsAddComponent(editors)) {
	...
	if (m_PreviewResizer != null && editors.Any()) {
		var previewAndLabelsContainer =
			CreateIMGUIContainer(DrawPreviewAndLabels, "preview-container");
		m_PreviewResizer.SetContainer(previewAndLabelsContainer,
		                              kBottomToolbarHeight);
		if (previewAndLabelElement != null)
	...
	rootVisualElement.RegisterCallback<DragPerformEvent>(
		DragPerformInBottomArea);
	rootVisualElement.RegisterCallback<MouseEnterEvent>(
		(e) => { CurrentPropertyEditor = this; });
	rootVisualElement.RegisterCallback<MouseLeaveEvent>(
		(e) => { CurrentPropertyEditor = null; });
...
internal void AutoScroll(Vector2 mousePosition) {
	if (m_ScrollView != null) {
		...
		var localDragPosition =
			m_ScrollView.contentViewport.WorldToLocal(mousePosition);

		if (localDragPosition.y < k_AutoScrollZoneHeight)
			m_ScrollView.verticalScroller.ScrollPageUp();
		else if (localDragPosition.y > m_ScrollView.contentViewport.rect.height -
		         k_AutoScrollZoneHeight)
...
private void DragOverBottomArea(DragUpdatedEvent dragUpdatedEvent) {
	if (DragAndDrop.objectReferences.Any()) {
		if (editorsElement != null &&
		    editorsElement.ContainsPoint(
			    editorsElement.WorldToLocal(dragUpdatedEvent.mousePosition))) {
		...
		if (editorsElement != null) {
			...
			editorDragging.HandleDraggingInBottomArea(
				tracker.activeEditors, bottomAreaDropRectangle, lastChild.layout);
		}
	}
}
...
private void DragPerformInBottomArea(DragPerformEvent dragPerformedEvent) {
	...
	if (editorsElement.ContainsPoint(
		    editorsElement.WorldToLocal(dragPerformedEvent.mousePosition)))
		return;
	...
	editorDragging.HandleDragPerformInBottomArea(
		tracker.activeEditors, bottomAreaDropRectangle, lastChild.layout);
}
...
protected IPreviewable GetEditorThatControlsPreview(IPreviewable[] editors) {
	...
	// Find last interacted editor, if not found check if we had an editor of
	// similar type, if not found use first editor that can show a preview
	...
	foreach (IPreviewable e in editors) {
		...
		if (EditorUtility.IsPersistent(e.target) &&
		    AssetDatabase.GetAssetPath(e.target) !=
		    AssetDatabase.GetAssetPath(editors[0].target))
			continue;

		// If main editor is an asset importer editor and this is an editor of the
		// imported object, ignore.
...
protected IPreviewable[] GetEditorsWithPreviews(Editor[] editors) {
	...
	foreach (Editor e in editors) {
		...
		if (!EditorUtility.IsPersistent(editors[0].target) &&
		    EditorUtility.IsPersistent(e.target))
			continue;
...
private Object[] GetInspectedAssets() {
	// We use this technique to support locking of the inspector. An inspector
	// locks via an editor, so we need to use an editor to get the selection
	Editor assetEditor = InspectorWindowUtils.GetFirstNonImportInspectorEditor(
	...
	if (assetEditor != null && assetEditor.targets.Length == 1) {
		...
		if (HasLabel(assetEditor.target, assetPath) &&
		    !Directory.Exists(assetPath))
			return assetEditor.targets;
	}

	// This is used if more than one asset is selected
	// Ideally the tracker should be refactored to track not just editors but
	// also the selection that caused them, so we wouldn't need this
	return Selection.objects.Where(HasLabel).ToArray();
}

protected virtual bool BeginDrawPreviewAndLabels() {
	return true;
}
protected virtual void EndDrawPreviewAndLabels(Event evt, Rect rect,
                                               Rect dragRect) {
}
private void DrawPreviewAndLabels() {
	...
	IPreviewable[] editorsWithPreviews =
		GetEditorsWithPreviews(tracker.activeEditors);
	IPreviewable previewEditor =
		GetEditorThatControlsPreview(editorsWithPreviews);

	// Do we have a preview?
	m_HasPreview =
		previewEditor != null && previewEditor.HasPreviewGUI() && hasPreview;

	m_PreviewResizer.containerMinimumHeightExpanded =
		m_HasPreview ? k_InspectorPreviewMinTotalHeight : 0;

	Object[] assets = GetInspectedAssets();
	bool hasLabels = assets.Length > 0;
	bool hasBundleName =
		assets.Any(a => !(a is MonoScript) && AssetDatabase.IsMainAsset(a));
	...
	Rect rect =
		EditorGUILayout.BeginHorizontal(GUIContent.none, Styles.preToolbar,
		                                GUILayout.Height(kBottomToolbarHeight));
	{
		...
		dragIconRect.y =
			dragRect.y +
			(kBottomToolbarHeight - Styles.dragHandle.fixedHeight) / 2;
		...
		if (editorsWithPreviews.Length > 1) {
			...
			float maxFoldoutWidth =
				(dragIconRect.xMax - dragRect.xMin) - dragPadding - minDragWidth;
			float foldoutWidth = Mathf.Min(maxFoldoutWidth, foldoutSize.x);
			Rect foldoutRect =
				new Rect(dragRect.x, dragRect.y, foldoutWidth, foldoutSize.y);
			...
			for (int index = 0; index < editorsWithPreviews.Length; index++) {
				...
				GUIContent previewTitle =
					currentEditor.GetPreviewTitle() ?? Styles.preTitle;

				string fullTitle;
				if (previewTitle == Styles.preTitle) {
					string componentTitle =
						ObjectNames.GetTypeName(currentEditor.target);
					if (NativeClassExtensionUtilities.ExtendsANativeType(
						    currentEditor.target)) {
						componentTitle =
							MonoScript.FromScriptedObject(currentEditor.target)
							.GetClass()
							.Name;
					}
			...
			if (GUI.Button(foldoutRect, title, Styles.preDropDown)) {
				EditorUtility.DisplayCustomMenu(foldoutRect, panelOptions,
				                                selectedPreview, OnPreviewSelected,
				                                editorsWithPreviews);
			}
		} else {
			float maxLabelWidth =
				(dragIconRect.xMax - dragRect.xMin) - dragPadding - minDragWidth;
			float labelWidth =
				Mathf.Min(maxLabelWidth, Styles.preToolbar2.CalcSize(title).x);
			Rect labelRect =
				new Rect(dragRect.x, dragRect.y, labelWidth, dragRect.height);
		...
		if (m_HasPreview && Event.current.type == EventType.Repaint) {
			// workaround: To properly center the image because it already has a 1px
			// bottom padding
			dragIconRect.y += 1;
			Styles.dragHandle.Draw(dragIconRect, GUIContent.none, false, false,
			                       false, false);
		}
	...
	float previewSize;
	if (m_HasPreview) {
		// If we have a preview we'll use the ResizerControl which handles both
		// resizing and collapsing
		previewSize = m_PreviewResizer.ResizeHandle(
			position, k_InspectorPreviewMinTotalHeight, k_MinAreaAbovePreview,
			kBottomToolbarHeight, dragRect);
	} else {
		// If we don't have a preview, just toggle the collapsible state with a
		// button
	...
	GUILayout.BeginVertical(Styles.preBackground,
	                        GUILayout.Height(previewSize));
	{
		// Draw preview
		if (m_HasPreview) {
			var previewRect = GUILayoutUtility.GetRect(0, 10240, 64, 10240);
			if (!float.IsNaN(previewRect.height) &&
			    !float.IsNaN(previewRect.width)) {
		...
		if (hasLabels) {
			using (new EditorGUI.DisabledScope(
				       assets.Any(a => EditorUtility.IsPersistent(a) &&
				                  !Editor.IsAppropriateFileOpenForEdit(a)))) {
	...
	if (m_PreviousFooterHeight >= 0f &&
	    !FloatComparer.s_ComparerWithDefaultTolerance.Equals(
		    previewSize, m_PreviousFooterHeight)) {
		UIEventRegistration.MakeCurrentIMGUIContainerDirty();
	}
...
private void OnPreviewSelected(object userData, string[] options,
                               int selected) {
...
internal static void VersionControlBar(Editor assetEditor) {
	...
	if (vcsMode == ExternalVersionControl.Generic ||
	    vcsMode == ExternalVersionControl.Disabled ||
	    vcsMode == ExternalVersionControl.AutoDetect)
		return;
	...
	// checks if passed file ends with "meta" and says "here, take this asset
	// instead" if it exists -- so for files under project settings, it ends up
	// returning a valid entry for the non-existing meta file. So just don't do
	// it.
	...
	string currentMetaState =
		metaAsset == null ? String.Empty : metaAsset.StateToString();

	// If VCS is enabled but disconnected, the assets will have "Updating" state
	// most of the time, but what we want to displays is a note that VCS is not
	// connected.
	if (!connected) {
		if (EditorUserSettings.WorkOffline)
			currentState = Styles.vcsOffline.text;
		else
			currentState = string.Format(Styles.vcsNotConnected.text,
			                             Provider.GetActivePlugin().name);
	...
	// Based on that and the current inspector width, we might want to layout
	// the VCS bar in two rows to better fit status label & buttons.
	var approxSpaceForButtons = state.GetButtonCount() * 50;
	var useTwoRows =
		GUIView.current != null &&
		GUIView.current.position.width * 0.5f < approxSpaceForButtons;

	var lineHeight = Styles.vcsBarStyleOneRow.fixedHeight;
	var barStyle =
		useTwoRows ? Styles.vcsBarStyleTwoRows : Styles.vcsBarStyleOneRow;
	GUILayout.BeginHorizontal(GUIContent.none, barStyle);
	var barRect = GUILayoutUtility.GetRect(GUIContent.none, barStyle,
	...
	if (currentMetaState != currentState) {
		...
		else if (hasAssetState &&
		         metaAsset !=
		         null) // project settings don't even have .meta files; no
			       // point in adding "asset only" for them
	...
	var fullMetaState =
		metaAsset != null? Asset.AllStateToString(metaAsset.state)
			: string.Empty;
...
private static void VersionControlCheckoutHint(Editor assetEditor,
                                               bool connected) {
	if (!connected)
		return;
...
private static float VersionControlBarButtons(VersionControlBarState presence,
                                              Rect rect, bool connected) {
	...
	if (!connected) {
		if (presence.settings) {
			if (VersionControlActionButton(buttonRect, ref buttonX,
			                               Styles.vcsSettings))
	...
	if (presence.revert && !presence.revertUnchanged) {
		...
		if (VersionControlActionDropdownButton(
			    buttonRect, ref buttonX, Styles.vcsRevert,
			    Styles.vcsRevertMenuNames, Styles.vcsRevertMenuActions,
			    presence.assets)) {
			if (presence.revert) {
				WindowRevert.Open(presence.assets);
				GUIUtility.ExitGUI();
	...
	if (presence.checkout) {
		if (VersionControlActionButton(
			    buttonRect, ref buttonX,
			    Provider.NeedToCheckOutBoth(presence.editor.target)
		  ? Styles.vcsCheckout
		  : Styles.vcsCheckoutMeta))
...
private static bool
VersionControlActionDropdownButton(Rect buttonRect, ref float buttonX,
                                   GUIContent content, GUIContent[] menuNames,
                                   GenericMenu.MenuFunction2[] menuActions,
                                   object context) {
...
private static bool VersionControlActionButton(Rect buttonRect,
                                               ref float buttonX,
                                               GUIContent content) {
	var buttonStyle = Styles.vcsButtonStyle;
...
private void DrawEditors(Editor[] editors) {
	...
	if (m_DrawnSelection.SetEquals(selection)) {
		if (editorsElement.childCount > 0 &&
		    m_DrawnSelection.Any()) // do we already have a hierarchy
		{
	...
	if (editors.Length > 0 &&
	    editors [0]
	    .GetInstanceID() != m_LastInitialEditorInstanceID)
		OnTrackerRebuilt();
	if (m_RemovedComponents == null)
		ExtractPrefabComponents(); // needed after assembly reload (due to HashSet
	...
	for (int editorIndex = 0; editorIndex < editors.Length; editorIndex++) {
		...
		VisualElement prefabsComponentElement =
			new VisualElement(){
			name = "PrefabComponentElement"
		};
		if (checkForRemovedComponents && editorIndex > targetGameObjectIndex) {
			...
			while (prefabComponentIndex < m_ComponentsInPrefabSource.Length) {
				...
				if (target != null) {
					...
					Component nextInSource =
						m_ComponentsInPrefabSource[prefabComponentIndex];

					if (correspondingSource == nextInSource)
						break;
					AddRemovedPrefabComponentElement(targetGameObject, nextInSource,
					                                 prefabsComponentElement);
				}
		...
		if (editorTarget &&
		    (editorTarget?.hideFlags & HideFlags.HideInInspector) ==
		    HideFlags.HideInInspector)
			continue;

		try {
			if (mapping == null ||
			    !mapping.TryGetValue(editors[editorIndex].target.GetInstanceID(),
			                         out var editorContainer)) {
				string editorTitle = editorTarget == null ? "Nothing Selected"
	      : $"{editor.GetType().Name}_{editorTarget.GetType().Name}_{editorTarget.GetInstanceID()}";
				editorContainer = EditorUIService.instance.CreateEditorElement(
	...
	// Make sure to display any remaining removed components that come after the
	// last component on the GameObject.
	if (checkForRemovedComponents) {
		VisualElement prefabsComponentElement =
			new VisualElement(){
			name = "RemainingPrefabComponentElement"
		};
		while (prefabComponentIndex < m_ComponentsInPrefabSource.Length) {
			Component nextInSource =
				m_ComponentsInPrefabSource[prefabComponentIndex];
...
private void RestoreVerticalScrollIfNeeded() {
	if (m_LastInspectedObjectInstanceID == -1)
		return;
	var inspectedObjectInstanceID =
		GetInspectedObject()?.GetInstanceID() ?? -1;
	if (inspectedObjectInstanceID == m_LastInspectedObjectInstanceID &&
	    inspectedObjectInstanceID != -1)
		m_ScrollView.verticalScroller.value = m_LastVerticalScrollValue;
	m_LastInspectedObjectInstanceID =
		-1; // reset to make sure the restore occurs once
}

private void AddRemovedPrefabComponentElement(GameObject targetGameObject,
                                              Component nextInSource,
                                              VisualElement element) {
	if (ShouldDisplayRemovedComponent(targetGameObject, nextInSource)) {
		string missingComponentTitle =
			ObjectNames.GetInspectorTitle(nextInSource);
		var removedComponentElement = CreateIMGUIContainer(
...
public bool WasEditorVisible(Editor[] editors, int editorIndex,
	...
	if (wasVisibleState == -1) {
		// Some inspectors (MaterialEditor) needs to be told when they are the
		// main visible asset.
		if (editorIndex == 0 ||
		    (editorIndex == 1 && ShouldCullEditor(editors, 0))) {
...
public bool IsMultiEditingSupported(Editor editor, Object target) {
	...
	// If the current editor is a GenericInspector even though a custom editor
	// for it exists, then it's either a fallback for a custom editor that
	// doesn't support multi-object editing, or we're in debug mode.
	bool multiEditingSupported = true;
	if (editor is GenericInspector &&
	    CustomEditorAttributes.FindCustomEditorType(target, false) != null) {
...
internal static bool EditorHasLargeHeader(int editorIndex,
                                          Editor[] trackerActiveEditors) {
	return trackerActiveEditors[editorIndex].firstInspectedEditor ||
	       trackerActiveEditors [editorIndex]
	       .HasLargeHeader();
}
...
public bool ShouldCullEditor(Editor[] editors, int editorIndex) {
	...
	if (currentTarget != null &&
	    currentTarget.GetType() == typeof(AssetImporter))
		return true;
...
private void AddComponentButton(Editor[] editors) {
	// Don't show the Add Component button if we are not showing imported
	// objects for Asset Importers
	var assetImporter = GetAssetImporter(editors);
	if (assetImporter != null && !assetImporter.showImportedObject)
		return;
	...
	Editor editor =
		InspectorWindowUtils.GetFirstNonImportInspectorEditor(editors);
	if (editor != null && editor.target != null &&
	    editor.target is GameObject && editor.IsEnabled()) {
		EditorGUILayout.BeginHorizontal(
			GUIContent.none, GUIStyle.none,
			GUILayout.Height(kAddComponentButtonHeight));
		{
			GUILayout.FlexibleSpace();
			var content = Styles.addComponentLabel;
			Rect rect =
				GUILayoutUtility.GetRect(content, Styles.addComponentButtonStyle);

			// Visually separates the Add Component button from the existing
			// components
			if (Event.current.type == EventType.Repaint)
			...
			if (EditorGUI.DropdownButton(rect, content, FocusType.Passive,
			                             Styles.addComponentButtonStyle) ||
			    m_OpenAddComponentMenu && Event.current.type == EventType.Repaint) {
				m_OpenAddComponentMenu = false;
				if (AddComponentWindow.Show(
					    rect, editor.targets.Select(o =>(GameObject) o).ToArray())) {
					GUIUtility.ExitGUI();
				}
			}
...
private bool ReadyToRepaint() {
	if (AnimationMode.InAnimationPlaybackMode()) {
		...
		if (timeNow - m_LastUpdateWhilePlayingAnimation <
...
private void DrawSplitLine(float y) {
	Rect position =
		new Rect(0, y - Styles.lineSeparatorOffset, m_Pos.width + 1, 1);
	using (new GUI.ColorScope(Styles.lineSeparatorColor * GUI.color))
		GUI.DrawTexture(position, EditorGUIUtility.whiteTexture);
}

private Dictionary<int, IEditorElement>
ProcessEditorElementsToRebuild(Editor[] editors) {
	Dictionary<int, IEditorElement> editorToElementMap =
		new Dictionary<int, IEditorElement>();
	var currentElements =
		editorsElement.Children().OfType<IEditorElement>().ToList();
	if (editors.Length == 0) {
		return null;
	}
	...
	while (newEditorsIndex < editors.Length &&
	       previousEditorsIndex < currentElements.Count) {
		...
		// We won't have an EditorElement for editors that are normally culled so
		// we should skip this
	...
	// Remove any elements at the end of the PropertyEditor that don't have
	// matching Editors
...
internal static PropertyEditor OpenPropertyEditor(Object obj,
	...
	propertyEditor.tracker.SetObjectsLockedByThisTracker(new List<Object> {
				obj
			});
...
private static void ShowPropertyEditorWindow(PropertyEditor propertyEditor) {
	...
	var pos =
		s_LastPropertyEditor ? s_LastPropertyEditor.position
	: propertyEditor.m_Parent.screenPosition;
	propertyEditor.position =
		new Rect(pos.x + 30, pos.y + 30, propertyEditor.position.width,
		         propertyEditor.position.height);
	s_LastPropertyEditor = propertyEditor;
}

[ShortcutManagement.Shortcut("PropertyEditor/OpenMouseOver")]
static void
OpenHoveredItemPropertyEditor(ShortcutManagement.ShortcutArguments args) {
