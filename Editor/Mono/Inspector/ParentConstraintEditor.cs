...
internal override SerializedProperty isContraintActive {
	get { return m_IsContraintActive; }
}
...
GUIContent m_RestTranslation =
	EditorGUIUtility.TrTextContent("Position At Rest");
GUIContent m_TranslationOffset =
	EditorGUIUtility.TrTextContent("Position Offset");

GUIContent m_RestRotation =
	EditorGUIUtility.TrTextContent("Rotation At Rest");
GUIContent m_RotationOffset =
	EditorGUIUtility.TrTextContent("Rotation Offset");

GUIContent m_TranslationAxes =
	EditorGUIUtility.TrTextContent("Freeze Position Axes");
GUIContent m_RotationAxes =
	EditorGUIUtility.TrTextContent("Freeze Rotation Axes");

GUIContent m_DefaultSourceName = EditorGUIUtility.TrTextContent("None");

GUIContent m_SourceOffsets =
	EditorGUIUtility.TrTextContent("Source Offsets");
...
public GUIContent TranslationAtRest {
	get { return m_RestTranslation; }
}
public GUIContent RotationAtRest {
	get { return m_RestRotation; }
}
...
public GUIContent FreezeTranslationAxes {
	get { return m_TranslationAxes; }
}
public GUIContent FreezeRotationAxes {
	get { return m_RotationAxes; }
}
public GUIContent SourceOffsets {
	get { return m_SourceOffsets; }
}
public GUIContent DefaultSourceName {
	get { return m_DefaultSourceName; }
}
}
...
public void OnEnable() {
	...
	m_TranslationOffsets =
		serializedObject.FindProperty("m_TranslationOffsets");
	...
	m_OffsetList = new ReorderableList(serializedObject, sources, false, false,
	                                   false, false);
	m_OffsetList.drawHeaderCallback += rect =>
	                                   EditorGUI.LabelField(rect, s_Style.SourceOffsets);
...
private void DrawOffsetElementCallback(Rect rect, int index, bool isActive,
                                       bool isFocused) {
	...
	if (sourceElement.objectReferenceValue != null) {
		sourceName =
			EditorGUIUtility.TextContent(sourceElement.objectReferenceValue.name);
	}
	...
	EditorGUI.PropertyField(drawRect, translationOffset,
	                        s_Style.TranslationOffset);
	drawRect.y += EditorGUIUtility.wideMode ? EditorGUIUtility.singleLineHeight +
...
static readonly float kListItemHeight =
	3 * (EditorGUIUtility.singleLineHeight +
	     2 * EditorGUIUtility.standardVerticalSpacing);
static readonly float kListItemNarrowHeight =
...
internal override void OnValueAtRestChanged() {
	foreach (var t in targets) {
		(t as IConstraintInternal).transform.localPosition =
			m_TranslationAtRest.vector3Value;
		(t as IConstraintInternal)
		.transform.SetLocalEulerAngles(m_RotationAtRest.vector3Value,
		                               RotationOrder.OrderZXY);
	}
}

internal override void ShowFreezeAxesControl() {
	Rect drawRectT = EditorGUILayout.GetControlRect(
		true,
		EditorGUI.GetPropertyHeight(SerializedPropertyType.Vector3,
		                            s_Style.FreezeTranslationAxes),
		EditorStyles.toggle);
	EditorGUI.MultiPropertyField(
		drawRectT, s_Style.Axes,
		serializedObject.FindProperty("m_AffectTranslationX"),
		s_Style.FreezeTranslationAxes);

	Rect drawRectR = EditorGUILayout.GetControlRect(
		true,
		EditorGUI.GetPropertyHeight(SerializedPropertyType.Vector3,
		                            s_Style.FreezeRotationAxes),
		EditorStyles.toggle);
	EditorGUI.MultiPropertyField(
		drawRectR, s_Style.Axes,
		serializedObject.FindProperty("m_AffectRotationX"),
		s_Style.FreezeRotationAxes);
}

internal override void ShowValueAtRest(ConstraintStyleBase style) {
	EditorGUI.BeginChangeCheck();
	EditorGUILayout.PropertyField(m_TranslationAtRest,
	                              (style as Styles).TranslationAtRest);
	EditorGUILayout.PropertyField(m_RotationAtRest,
	                              (style as Styles).RotationAtRest);
