...
// TODO: We need to make the editMode state e.g a string so users are able to
// extend with new edit mode states and so we still can serialize it (to survive
// assembly reloads)
...
static EditMode() {
	...
	s_EditMode = (SceneViewEditMode)
	             SessionState.GetInt(kEditModeStringKey, (int) s_EditMode);
...
public delegate void OnEditModeStartFunc(Editor editor,
                                         SceneViewEditMode mode);

internal static event Action<IToolModeOwner> editModeEnded;
internal static event Action<IToolModeOwner, SceneViewEditMode>
editModeStarted;
...
static EditModeTool GetTool(IToolModeOwner owner, SceneViewEditMode mode) {
	...
	return EditorToolContext.GetCustomEditorTool(x => {
				...
				return tool != null && tool.editMode == mode && tool.owner == owner &&
				tool.editorType == editorType && tool.target == editor.target;
...
public static void OnSelectionChange() {
	IToolModeOwner owner = InternalEditorUtility.GetObjectFromInstanceID(
		ownerID) as IToolModeOwner;
...
public static void QuitEditMode() {
	if (ownerID != k_OwnerIdNone)
		ChangeEditMode(SceneViewEditMode.None,
		               new Bounds(Vector3.zero, Vector3.positiveInfinity), null);
}

[Obsolete(
	 "Obsolete msg (UnityUpgradable) -> UnityEditor.EditorTools.EditorTools.RestorePreviousTool()")]
...
public static void
DoEditModeInspectorModeButton(SceneViewEditMode mode, string label,
                              GUIContent icon, Bounds bounds, Editor caller) {
	DoEditModeInspectorModeButton(mode, label, icon, () => bounds,
	                              (IToolModeOwner) caller);
}

public static void DoEditModeInspectorModeButton(
	SceneViewEditMode mode, string label, GUIContent icon,
	Func<Bounds> getBoundsOfTargets, Editor caller) {
	DoEditModeInspectorModeButton(mode, label, icon, getBoundsOfTargets,
	                              (IToolModeOwner) caller);
}

internal static void DoEditModeInspectorModeButton(SceneViewEditMode mode,
                                                   string label,
                                                   GUIContent icon,
                                                   IToolModeOwner owner) {
	DoEditModeInspectorModeButton(mode, label, icon, null, owner);
}

private static void DoEditModeInspectorModeButton(
	SceneViewEditMode mode, string label, GUIContent icon,
	Func<Bounds> getBoundsOfTargets, IToolModeOwner owner) {
	Rect rect = EditorGUILayout.GetControlRect(true, k_EditColliderbuttonHeight,
	                                           Styles.singleButtonStyle);
	Rect buttonRect =
		new Rect(rect.xMin + EditorGUIUtility.labelWidth, rect.yMin,
		         k_EditColliderbuttonWidth, k_EditColliderbuttonHeight);

	GUIContent labelContent = new GUIContent(label);
	Vector2 labelSize = GUI.skin.label.CalcSize(labelContent);

	Rect labelRect = new Rect(buttonRect.xMax + k_SpaceBetweenLabelAndButton,
	                          rect.yMin + (rect.height - labelSize.y) * .5f,
	                          labelSize.x, rect.height);
	...
	using (new EditorGUI.DisabledScope(!owner.areToolModesAvailable)) {
		toggleEnabled =
			GUI.Toggle(buttonRect, modeEnabled, icon, Styles.singleButtonStyle);
		GUI.Label(labelRect, label);
	}

	if (EditorGUI.EndChangeCheck()) {
		ChangeEditMode(toggleEnabled ? mode
		     : SceneViewEditMode.None,
		               getBoundsOfTargets ==
		               null ? owner.GetWorldBoundsOfTargets()
		     : getBoundsOfTargets(), owner);
	}
}

[Obsolete("Use signature passing Func<Bounds> rather than Bounds.")]
public static void DoInspectorToolbar(SceneViewEditMode[] modes,
                                      GUIContent[] guiContents, Bounds bounds,
                                      Editor caller) {
	DoInspectorToolbar(modes, guiContents, () => bounds,
	                   (IToolModeOwner) caller);
}
...
public static void DoInspectorToolbar(SceneViewEditMode[] modes,
                                      ...
                                      Editor caller) {
	DoInspectorToolbar(modes, guiContents, getBoundsOfTargets,
	                   (IToolModeOwner) caller);
}

internal static void DoInspectorToolbar(SceneViewEditMode[] modes,
                                        GUIContent[] guiContents,
                                        IToolModeOwner owner) {
	DoInspectorToolbar(modes, guiContents, null, owner);
}

private static void DoInspectorToolbar(SceneViewEditMode[] modes,
                                       GUIContent[] guiContents,
                                       Func<Bounds> getBoundsOfTargets,
                                       IToolModeOwner owner) {
	...
	using (new EditorGUI.DisabledScope(!owner.areToolModesAvailable))
		newSelectedIndex = GUILayout.Toolbar(selectedIndex, guiContents,
		                                     Styles.multiButtonStyle);
	if (EditorGUI.EndChangeCheck()) {
		// Buttons can be toggled
		SceneViewEditMode newEditMode =
			newSelectedIndex == selectedIndex ? SceneViewEditMode.None
	  : modes[newSelectedIndex];
		ChangeEditMode(newEditMode,
		               getBoundsOfTargets == null ? owner.GetWorldBoundsOfTargets()
		     : getBoundsOfTargets(), owner);
	}
}
...
internal static void ChangeEditMode(SceneViewEditMode mode,
                                    IToolModeOwner owner) {
	ChangeEditMode(mode, owner.GetWorldBoundsOfTargets(), owner);
}

internal static void ChangeEditMode(SceneViewEditMode mode, Bounds bounds,
                                    IToolModeOwner owner) {
	if (mode == SceneViewEditMode.None) {
		var activeToolIsEditModeTool =
			(EditorToolContext.activeTool is EditModeTool ||
			 EditorToolContext.activeTool is NoneTool);
...
internal static void EditModeToolStateChanged(IToolModeOwner owner,
                                              SceneViewEditMode mode) {
	// In cases of domain reloads the EditorTool can be deserialized prior to
	// the target Inspector being created. The EditMode tools do not expect an
	// editModeStarted callback on reloads.
	if (owner == null && mode != SceneViewEditMode.None) {
		editMode = mode;
		return;
	}

	IToolModeOwner oldOwner = InternalEditorUtility.GetObjectFromInstanceID(
		ownerID) as IToolModeOwner;
	...
	ownerID =
		mode != SceneViewEditMode.None ? owner.GetInstanceID() : k_OwnerIdNone;
	...
	FocusEditModeToolTarget(
		(mode != SceneViewEditMode.None && owner != null)
...
// We make sure edited object is seen by the camera. We need object bounds to
// do that check.
static void FocusEditModeToolTarget(Bounds bounds) {
	// When entering the edit mode, check if the collider is seen by the scene
	// view camera. If not, then we frame it.
	if (editMode != SceneViewEditMode.None &&
...
private static bool PointSeenByCamera(Camera camera, Vector3 point) {
	...
	return viewPoint.x > 0.0f && viewPoint.x < 1.0f && viewPoint.y > 0.0f &&
	       viewPoint.y < 1.0f;
}
}
}
