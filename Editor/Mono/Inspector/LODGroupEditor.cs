...
internal class LODGroupEditor : Editor {
  ...
  void OnEnable() {
    ...
    m_LastLODIsBillboard =
        serializedObject.FindProperty("m_LastLODIsBillboard");
    m_LODs = serializedObject.FindProperty("m_LODs");

    m_ShowAnimateCrossFading.value =
        m_FadeMode.intValue != (int) LODFadeMode.None;
    ...
    // Calculate if the newly selected LOD group is a prefab... they require
    // special handling
  ...
  // Find the given sceen space recangular bounds from a list of vector 3
  // points.
  ...
  public static bool IsSceneGUIEnabled() {
    if (Event.current.type != EventType.Repaint || Camera.current == null ||
  ...
  public void OnSceneGUI() {
    ...
    var worldReferencePoint =
        LODUtility.CalculateWorldReferencePoint(m_LODGroup);
    ...
    Handles.color = info.activeLODLevel != -1
                        ? LODGroupGUI.kLODColors[info.activeLODLevel]
                        : LODGroupGUI.kCulledLODColor;

    Handles.SelectionFrame(0, worldReferencePoint, camera.transform.rotation,
                           size / 2);
    ...
    var rect = CalculateScreenRect(new[]{worldReferencePoint - sideways + up,
    ...
    rect = new Rect(midPoint - LODGroupGUI.kSceneLabelHalfWidth, rect.yMax,
                    LODGroupGUI.kSceneLabelHalfWidth * 2,
                    LODGroupGUI.kSceneLabelHeight);

    if (rect.yMax > Screen.height - LODGroupGUI.kSceneLabelHeight)
      rect.y = Screen.height - LODGroupGUI.kSceneLabelHeight -
               LODGroupGUI.kSceneHeaderOffset;

    Handles.BeginGUI();
    GUI.Label(rect, GUIContent.none, EditorStyles.notificationBackground);
    EditorGUI.DoDropShadowLabel(
        rect,
        GUIContent.Temp(info.activeLODLevel >= 0 ? "LOD " + info.activeLODLevel
                                                 : "Culled"),
        LODGroupGUI.Styles.m_LODLevelNotifyText, 0.3f);
    Handles.EndGUI();
  }
  ...
  private const string kPixelHeightDataPath =
      "m_LODs.Array.data[{0}].screenRelativeHeight";
  private const string kRenderRootPath = "m_LODs.Array.data[{0}].renderers";
  private const string kFadeTransitionWidthDataPath =
      "m_LODs.Array.data[{0}].fadeTransitionWidth";
  ...
  private ModelImporter GetImporter() {
    return AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(
        PrefabUtility.GetCorrespondingObjectFromSource(target)))
        as ModelImporter;
  }

  private bool IsLODUsingCrossFadeWidth(int lod) {
    if (m_FadeMode.intValue == (int) LODFadeMode.None ||
        m_AnimateCrossFading.boolValue)
      return false;
    ...
    if (m_NumberOfLODs > 1 && m_SelectedLOD == m_NumberOfLODs - 2) {
      ...
      var renderers = serializedObject.FindProperty(
          String.Format(kRenderRootPath, m_NumberOfLODs - 1));
      if (renderers.arraySize != 1)
        return false;
      var renderer = renderers.GetArrayElementAtIndex(0)
                         .FindPropertyRelative("renderer")
                         .objectReferenceValue;
      if (renderer is BillboardRenderer ||
          (renderer is MeshRenderer && m_LastLODIsBillboard.boolValue))
        return true;
    }
    return false;
  }
  ...
  public override void OnInspectorGUI() {
    ...
    m_ShowAnimateCrossFading.target =
        m_FadeMode.intValue != (int) LODFadeMode.None;
    ...
    // This could happen when you select a newly inserted LOD level and then
    // undo the insertion. It's valid for m_SelectedLOD to become -1, which
    ...
    if (m_NumberOfLODs > 0 && activeLOD >= 0) {
      var renderersProperty = serializedObject.FindProperty(
          string.Format(kRenderRootPath, activeLOD));
      for (var i = renderersProperty.arraySize - 1; i >= 0; i--) {
        var rendererRef =
            renderersProperty.GetArrayElementAtIndex(i).FindPropertyRelative(
                "renderer");
    ...
    var lods = LODGroupGUI.CreateLODInfos(
        ...
        i => serializedObject
                 .FindProperty(string.Format(kPixelHeightDataPath, i))
                 .floatValue);
    ...
    if (QualitySettings.lodBias != 1.0f)
      EditorGUILayout.HelpBox(
          string.Format(
              "Active LOD bias is {0:0.0#}. Distances are adjusted accordingly.",
    ...
    if (m_NumberOfLODs > 0 && activeLOD >= 0 && activeLOD < m_NumberOfLODs) {
      ...
      if (EditorGUILayout.BeginFadeGroup(m_ShowFadeTransitionWidth.faded))
        EditorGUILayout.Slider(serializedObject.FindProperty(string.Format(
                                   kFadeTransitionWidthDataPath, activeLOD)),
                               0, 1);
      EditorGUILayout.EndFadeGroup();
    ...
    bool needUpdateBounds =
        LODUtility.NeedUpdateLODGroupBoundingBox(m_LODGroup);
    using(new EditorGUI.DisabledScope(!needUpdateBounds)) {
      if (GUILayout.Button(
              needUpdateBounds ? LODGroupGUI.Styles.m_RecalculateBounds
              : LODGroupGUI.Styles.m_RecalculateBoundsDisabled,
    ...
    var importer =
        PrefabUtility.IsPartOfModelPrefab(target) ? GetImporter() : null;
    if (importer != null) {
      using(var importerRef = new SerializedObject(importer)) {
        var importerLODLevels =
            importerRef.FindProperty("m_LODScreenPercentages");
        var lodNumberOnImporterMatches =
            importerLODLevels.isArray &&
            importerLODLevels.arraySize == lods.Count;

        var guiState = GUI.enabled;
        if (!lodNumberOnImporterMatches)
          GUI.enabled = false;

        if (GUILayout.Button(lodNumberOnImporterMatches ? LODGroupGUI.Styles
                                 .m_UploadToImporter
          ...
          for (var i = 0; i < importerLODLevels.arraySize; i++)
            importerLODLevels.GetArrayElementAtIndex(i).floatValue =
                lods[i].RawScreenPercent;
  ...
  private void DrawRenderersInfo(float availableWidth) {
    var horizontalCount =
        Mathf.FloorToInt(availableWidth / LODGroupGUI.kRenderersButtonHeight);
    var titleArea =
        GUILayoutUtility.GetRect(LODGroupGUI.Styles.m_RendersTitle,
                                 LODGroupGUI.Styles.m_LODSliderTextSelected);
    if (Event.current.type == EventType.Repaint)
      EditorStyles.label.Draw(titleArea, LODGroupGUI.Styles.m_RendersTitle,
    ...
    var numberOfRows =
        Mathf.CeilToInt(numberOfButtons / (float) horizontalCount);

    var drawArea = GUILayoutUtility.GetRect(
        0, numberOfRows * LODGroupGUI.kRenderersButtonHeight,
    ...
    for (int i = 0; i < numberOfRows; i++) {
      for (int k = 0; k < horizontalCount &&
                      (i * horizontalCount + k) < renderersProperty.arraySize;
           k++) {
        var drawPos = new Rect(LODGroupGUI.kButtonPadding + rendererArea.x +
                                   k * buttonWidth,
                               LODGroupGUI.kButtonPadding + rendererArea.y +
                                   i * LODGroupGUI.kRenderersButtonHeight,
                               buttonWidth - LODGroupGUI.kButtonPadding * 2,
                               LODGroupGUI.kRenderersButtonHeight -
                                   LODGroupGUI.kButtonPadding * 2);
        buttons.Add(drawPos);
    ...
    HandleAddRenderer(
        new Rect(LODGroupGUI.kButtonPadding + rendererArea.x +
                     horizontalPos * buttonWidth,
                 LODGroupGUI.kButtonPadding + rendererArea.y +
                     verticalPos * LODGroupGUI.kRenderersButtonHeight,
                 buttonWidth - LODGroupGUI.kButtonPadding * 2,
                 LODGroupGUI.kRenderersButtonHeight -
                     LODGroupGUI.kButtonPadding * 2),
        buttons, drawArea);
  ...
  private void HandleAddRenderer(Rect position, IEnumerable<Rect> alreadyDrawn,
                                 Rect drawArea) {
    Event evt = Event.current;
    switch (evt.type) {
    case EventType.Repaint: {
      LODGroupGUI.Styles.m_LODStandardButton.Draw(position, GUIContent.none,
                                                  false, false, false, false);
      LODGroupGUI.Styles.m_LODRendererAddButton.Draw(
          new Rect(position.x - LODGroupGUI.kButtonPadding, position.y,
      ...
      if (DragAndDrop.objectReferences.Length > 0) {
        ...
        if (evt.type == EventType.DragPerform) {
          ...
          var selectedGameObjects =
              from go in DragAndDrop.objectReferences where go as GameObject !=
              null select go as GameObject;
    ...
    case EventType.ExecuteCommand: {
      ...
      if (commandName == ObjectSelector.ObjectSelectorClosedCommand &&
          ObjectSelector.get.objectSelectorID ==
              "LODGroupSelector".GetHashCode()) {
        var selectedObject = ObjectSelector.GetCurrentObject() as GameObject;
        if (selectedObject != null)
          AddGameObjectRenderers(
              GetRenderers(new List<GameObject>{selectedObject}, true), true);
        evt.Use();
        GUIUtility.ExitGUI();
      }
      break;
  ...
  private void DrawRendererButton(Rect position, int rendererIndex) {
    ...
    var deleteButton =
        new Rect(position.xMax - LODGroupGUI.kDeleteButtonSize,
                 position.yMax - LODGroupGUI.kDeleteButtonSize,
                 LODGroupGUI.kDeleteButtonSize, LODGroupGUI.kDeleteButtonSize);
    ...
    case EventType.Repaint: {
      if (renderer != null) {
        ...
        if (filter != null && filter.sharedMesh != null)
          content =
              new GUIContent(AssetPreview.GetAssetPreview(filter.sharedMesh),
                             renderer.gameObject.name);
        else if (renderer is SkinnedMeshRenderer)
          content =
              new GUIContent(AssetPreview.GetAssetPreview(
                                 (renderer as SkinnedMeshRenderer).sharedMesh),
                             renderer.gameObject.name);
        else if (renderer is BillboardRenderer)
          content =
              new GUIContent(AssetPreview.GetAssetPreview(
                                 (renderer as BillboardRenderer).billboard),
                             renderer.gameObject.name);
        else
          content = new GUIContent(
              ObjectNames.NicifyVariableName(renderer.GetType().Name),
              renderer.gameObject.name);

        LODGroupGUI.Styles.m_LODBlackBox.Draw(position, GUIContent.none, false,
                                              false, false, false);

        LODGroupGUI.Styles.m_LODRendererButton.Draw(
            new Rect(position.x + LODGroupGUI.kButtonPadding,
        ...
        LODGroupGUI.Styles.m_LODBlackBox.Draw(position, GUIContent.none, false,
                                              false, false, false);
        LODGroupGUI.Styles.m_LODRendererButton.Draw(position, "<Empty>", false,
                                                    false, false, false);
      }

      if (!m_IsPrefab) {
        LODGroupGUI.Styles.m_LODBlackBox.Draw(deleteButton, GUIContent.none,
                                              false, false, false, false);
        LODGroupGUI.Styles.m_LODRendererRemove.Draw(
            deleteButton, LODGroupGUI.Styles.m_IconRendererMinus, false, false,
            false, false);
      }
      break;
    }
  ...
  private IEnumerable<Renderer>
  GetRenderers(IEnumerable<GameObject> selectedGameObjects,
               bool searchChildren) {
    // Only allow renderers that are parented to this LODGroup
    if (EditorUtility.IsPersistent(m_LODGroup))
      return new List<Renderer>();

    var validSearchObjects =
        from go in selectedGameObjects where go.transform.IsChildOf(
            m_LODGroup.transform) select go;
    ...
    if (nonChildObjects.Count() > 0) {
      ...
      if (EditorUtility.DisplayDialog("Reparent GameObjects", kReparent,
                                      "Yes, Reparent",
    ...
    // Then try renderers
    var selectedRenderers =
        from go in DragAndDrop.objectReferences where go as Renderer !=
  ...
  private void AddGameObjectRenderers(IEnumerable<Renderer> toAdd, bool add) {
    ...
    foreach (var renderer in toAdd) {
      ...
      renderersProperty.GetArrayElementAtIndex(renderersProperty.arraySize - 1)
          .FindPropertyRelative("renderer")
  ...
  private class LODAction {
    ...
    public LODAction(List<LODGroupGUI.LODInfo> lods, float percentage,
                     Vector2 clickedPosition, SerializedProperty propLODs,
                     Callback callback) {
      m_LODs = lods;
    ...
    public void InsertLOD() {
      ...
      var renderers =
          m_ObjectRef.FindProperty(string.Format(kRenderRootPath, insertIndex));
      renderers.arraySize = 0;

      var newLOD = m_LODsProperty.GetArrayElementAtIndex(insertIndex);
      newLOD.FindPropertyRelative("screenRelativeHeight").floatValue =
          m_Percentage;
      if (m_Callback != null)
        m_Callback();
    ...
    public void DeleteLOD() {
      ...
      foreach (var lod in m_LODs) {
        var numberOfRenderers =
            m_ObjectRef
                .FindProperty(string.Format(kRenderRootPath, lod.LODLevel))
                .arraySize;
        if (lod.m_RangePosition.Contains(m_ClickedPosition) &&
            (numberOfRenderers == 0 ||
             EditorUtility.DisplayDialog(
                 "Delete LOD", "Are you sure you wish to delete this LOD?",
                 "Yes", "No"))) {
          var lodData = m_ObjectRef.FindProperty(
              string.Format(kLODDataPath, lod.LODLevel));
  ...
  private void DeletedLOD() { m_SelectedLOD--; }

  // Set the camera distance so that the current LOD group covers the desired
  ...
  private static void UpdateCamera(float desiredPercentage, LODGroup group) {
    ...
    var percentage =
        Mathf.Max(desiredPercentage / QualitySettings.lodBias, 0.000001f);
    ...
    SceneView.lastActiveSceneView.LookAtDirect(
        worldReferencePoint, sceneCamera.transform.rotation, size);
  }

  private void
  UpdateSelectedLODFromCamera(IEnumerable<LODGroupGUI.LODInfo> lods,
                              float cameraPercent) {
  ...
  private void DrawLODLevelSlider(Rect sliderPosition,
                                  List<LODGroupGUI.LODInfo> lods) {
    ...
    case EventType.MouseDown: {
      ...
      if (evt.button == 1 && sliderPosition.Contains(evt.mousePosition)) {
        var cameraPercent =
            LODGroupGUI.GetCameraPercent(evt.mousePosition, sliderPosition);
        ...
        if (lods.Count >= 8) {
          ...
          pm.AddItem(EditorGUIUtility.TrTextContent("Insert Before"), false,
                     new LODAction(lods, cameraPercent, evt.mousePosition,
                                   m_LODs, null)
                         .InsertLOD);
        }
        ...
        if (lods.Count > 0 &&
            lods[lods.Count - 1].RawScreenPercent < cameraPercent)
      ...
      if (barPosition.Contains(evt.mousePosition)) {
        ...
        // case:464019 have to re-sort the LOD array for these buttons to get
        // the overlaps in the right order...
        var lodsLeft = lods.Where(lod => lod.ScreenPercent > 0.5f)
                           .OrderByDescending(x => x.LODLevel);
        var lodsRight = lods.Where(lod => lod.ScreenPercent <= 0.5f)
                            .OrderBy(x => x.LODLevel);
    ...
    case EventType.MouseDrag: {
      if (GUIUtility.hotControl == sliderId && m_SelectedLODSlider >= 0 &&
        ...
        var cameraPercent =
            LODGroupGUI.GetCameraPercent(evt.mousePosition, sliderPosition);
        // Bias by 0.1% so that there is no skipping when sliding
        LODGroupGUI.SetSelectedLODLevelPercentage(cameraPercent - 0.001f,
                                                  m_SelectedLODSlider, lods);
        var percentageProperty = serializedObject.FindProperty(string.Format(
            kPixelHeightDataPath, lods[m_SelectedLODSlider].LODLevel));
        percentageProperty.floatValue =
            lods[m_SelectedLODSlider].RawScreenPercent;
    ...
    case EventType.DragPerform: {
      ...
      if (lodLevel == -2) {
        var culledRange = LODGroupGUI.GetCulledBox(
            sliderPosition,
      ...
      if (lodLevel >= -1) {
        ...
        if (DragAndDrop.objectReferences.Length > 0) {
          DragAndDrop.visualMode =
              m_IsPrefab ? DragAndDropVisualMode.None
              : DragAndDropVisualMode.Copy;

          if (evt.type == EventType.DragPerform) {
            // First try gameobjects...
            var selectedGameObjects =
                from go in DragAndDrop.objectReferences where go
                    as GameObject != null select go as GameObject;
            var renderers = GetRenderers(selectedGameObjects, true);

            if (lodLevel == -1) {
              m_LODs.arraySize++;
              var pixelHeightNew = serializedObject.FindProperty(
                  string.Format(kPixelHeightDataPath, lods.Count));

              if (lods.Count == 0)
                pixelHeightNew.floatValue = 0.5f;
              else {
                var pixelHeightPrevious = serializedObject.FindProperty(
                    string.Format(kPixelHeightDataPath, lods.Count - 1));
                pixelHeightNew.floatValue =
                    pixelHeightPrevious.floatValue / 2.0f;
              }
    ...
    if (SceneView.lastActiveSceneView != null &&
        SceneView.lastActiveSceneView.camera != null && !m_IsPrefab) {
      var camera = SceneView.lastActiveSceneView.camera;

      var info = LODUtility.CalculateVisualizationData(camera, m_LODGroup, -1);
      var linearHeight = info.activeRelativeScreenSize;
      var relativeHeight =
          LODGroupGUI.DelinearizeScreenPercentage(linearHeight);
      ...
      var cameraRect = LODGroupGUI.CalcLODButton(sliderPosition,
                                                 Mathf.Clamp01(relativeHeight));
      var cameraIconRect =
          new Rect(cameraRect.center.x - 15, cameraRect.y - 25, 32, 32);
      var cameraLineRect =
          new Rect(cameraRect.center.x - 1, cameraRect.y, 2, cameraRect.height);
      var cameraPercentRect =
          new Rect(cameraIconRect.center.x - 5, cameraLineRect.yMax, 35, 20);
      ...
      case EventType.Repaint: {
        ...
        GUI.backgroundColor =
            new Color(colorCache.r, colorCache.g, colorCache.b, 0.8f);
        LODGroupGUI.Styles.m_LODCameraLine.Draw(cameraLineRect, false, false,
                                                false, false);
        GUI.backgroundColor = colorCache;
        GUI.Label(cameraIconRect, LODGroupGUI.Styles.m_CameraIcon,
                  GUIStyle.none);
        LODGroupGUI.Styles.m_LODSliderText.Draw(
            cameraPercentRect,
            String.Format("{0:0}%", Mathf.Clamp01(linearHeight) * 100.0f),
            false, false, false, false);
        break;
      }
      case EventType.MouseDown: {
        if (cameraIconRect.Contains(evt.mousePosition)) {
          ...
          // Update the selected LOD to be where the camera is if we click the
          // camera
      ...
      case EventType.MouseDrag: {
        if (GUIUtility.hotControl == camerId) {
          ...
          // Change the active LOD level if the camera moves into a new LOD
          // level
          UpdateSelectedLODFromCamera(lods, cameraPercent);
          UpdateLODDrag(cameraPercent, m_LODGroup);
        }
  ...
  private void BeginLODDrag(float desiredPercentage, LODGroup group) {
    ...
    HierarchyProperty.FilterSingleSceneObject(group.gameObject.GetInstanceID(),
                                              false);
  ...
  private void EndLODDrag() {
    ...
    SceneView.lastActiveSceneView.ClearSearchFilter();
    // Clearing the search filter of a SceneView will not actually reset the
    // visibility values of the GameObjects in the scene so we have to
    // explicitly do that  (case 770915).
  ...
  private void SendPercentagesToLightmapScale() {
    ...
    for (var i = 0; i < m_NumberOfLODs; i++) {
      var renderersProperty =
          serializedObject.FindProperty(string.Format(kRenderRootPath, i));
      var renderersAtLOD = new List<SerializedProperty>();

      for (var k = 0; k < renderersProperty.arraySize; k++) {
        var rendererRef =
            renderersProperty.GetArrayElementAtIndex(k).FindPropertyRelative(
                "renderer");
      ...
      var pixelHeight =
          i == 0 ? 1.0f
                 : serializedObject
                       .FindProperty(string.Format(kPixelHeightDataPath, i - 1))
                       .floatValue;
      lodRenderers.Add(new LODLightmapScale(pixelHeight, renderersAtLOD));
    }

    // set from least detailed to most detailed, as renderers can be in multiple
    // layers
  ...
  private static void SetLODLightmapScale(LODLightmapScale lodRenderer) {
    foreach (var renderer in lodRenderer.m_Renderers) {
      ...
      using(var so = new SerializedObject(renderer.objectReferenceValue)) {
        ...
        lightmapScaleProp.floatValue = Mathf.Max(
            0.0f, lodRenderer.m_Scale *
                      (1.0f / LightmapVisualization.GetLightmapLODLevelScale(
                                  (Renderer) renderer.objectReferenceValue)));
  ...
  protected void DoRenderPreview() {
    if (m_PreviewUtility.renderTexture.width <= 0 ||
        m_PreviewUtility.renderTexture.height <= 0 || m_NumberOfLODs <= 0 ||
        activeLOD < 0)
      return;
    ...
    for (int i = 0; i < renderers.arraySize; i++) {
      var lodRenderRef =
          renderers.GetArrayElementAtIndex(i).FindPropertyRelative("renderer");
      var renderer = lodRenderRef.objectReferenceValue as Renderer;
      ...
      if (meshFilter != null && meshFilter.sharedMesh != null &&
          meshFilter.sharedMesh.subMeshCount > 0) {
    ...
    m_PreviewUtility.lights[0].transform.rotation =
    ...
    foreach (var meshFilter in meshsToRender) {
      for (int k = 0; k < meshFilter.sharedMesh.subMeshCount; k++) {
        if (k < meshFilter.GetComponent<Renderer>().sharedMaterials.Length) {
          var matrix = Matrix4x4.TRS(meshFilter.transform.position,
                                     meshFilter.transform.rotation,
                                     meshFilter.transform.localScale);
          m_PreviewUtility.DrawMesh(
              meshFilter.sharedMesh, matrix,
              meshFilter.GetComponent<Renderer>().sharedMaterials[k], k);
        }
      }
    }
    ...
    foreach (var billboard in billboards) {
      ...
      BillboardAssetInspector.MakeRenderMesh(m_BillboardPreviewMesh,
                                             billboard.billboard);
      billboard.billboard.MakeMaterialProperties(m_BillboardPreviewProperties,
                                                 m_PreviewUtility.camera);
      var matrix = Matrix4x4.TRS(billboard.transform.position,
                                 billboard.transform.rotation,
                                 billboard.transform.localScale);
      m_PreviewUtility.DrawMesh(m_BillboardPreviewMesh, matrix,
                                billboard.sharedMaterial, 0,
                                m_BillboardPreviewProperties);
    }
  ...
  override public string GetInfoString() {
    if (SceneView.lastActiveSceneView == null ||
        SceneView.lastActiveSceneView.camera == null || m_NumberOfLODs <= 0 ||
        activeLOD < 0)
      return "";
    ...
    var info =
        LODUtility.CalculateVisualizationData(camera, m_LODGroup, activeLOD);
    return activeLOD != -1
               ? string.Format(
                     "{0} Renderer(s)\n{1} Triangle(s)\n{2} Material(s)",
                     renderers.arraySize, info.triangleCount,
