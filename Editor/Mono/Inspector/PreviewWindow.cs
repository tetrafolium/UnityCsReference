...
internal class PreviewWindow : InspectorWindow {
  ...
  VisualElement previewElement => m_previewElement ??(
      m_previewElement = rootVisualElement.Q(className
                                             : "unity-inspector-preview"));

  public void SetParentInspector(InspectorWindow inspector) {
    m_ParentInspectorWindow = inspector;

    // Create tracker after parent inspector window has been set (case 829182,
    // 846156)
    CreateTracker();
  }
  ...
  // shared tracker. This makes sense when we are an InspectorWindow about to
  // die, but it does not make sense when we are a PreviewWindow sharing this
  // tracker with a perfectly not dead InspectorWindow. Killing the tracker used
  // by a still-alive InspectorWindow cause many problems. case 1119612
  ...
  protected override void OnEnable() {
    ...
    var tpl = EditorGUIUtility.Load("UXML/InspectorWindow/PreviewWindow.uxml")
                  as VisualTreeAsset;
    ...
    rootVisualElement.AddStyleSheetPath(
        "StyleSheets/InspectorWindow/PreviewWindow.uss");
  ...
  protected void DrawPreview() {
    ...
    if (evt.type == EventType.MouseUp && evt.button == 1 &&
        toolbarRect.Contains(evt.mousePosition)) {
      evt.Use();
      Close();
  ...
  public override void AddItemsToMenu(GenericMenu menu) {
    menu.AddItem(EditorGUIUtility.TrTextContent("Dock Preview to Inspector"),
                 false, Close);
  }
