...
protected override void CopyColliderPropertiesToHandle(BoxCollider collider) {
	...
	m_BoundsHandle.size =
		Vector3.Scale(collider.size, collider.transform.lossyScale);
}
...
protected override void CopyHandlePropertiesToCollider(BoxCollider collider) {
	...
	Vector3 size = Vector3.Scale(
		m_BoundsHandle.size, InvertScaleVector(collider.transform.lossyScale));
...
protected GUIContent centerContent = EditorGUIUtility.TrTextContent(
	"Center", "The position of the Collider in the object's local space.");
protected GUIContent sizeContent = EditorGUIUtility.TrTextContent(
