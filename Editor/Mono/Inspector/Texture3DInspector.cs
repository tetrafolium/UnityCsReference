...
public static Material SDF {
	get {
		if (s_SDF == null)
			s_SDF = Instantiate(EditorGUIUtility.LoadRequired(
						    "Previews/Preview3DSDF.mat")) as Material;
		return s_SDF;
	}
}
...
public static Material Slice {
	get {
		if (s_Slice == null)
			s_Slice = Instantiate(EditorGUIUtility.LoadRequired(
						      "Previews/Preview3DSliced.mat")) as Material;
		return s_Slice;
	}
}
...
public static Material Volume {
	get {
		if (s_Volume == null)
			s_Volume = Instantiate(EditorGUIUtility.LoadRequired(
						       "Previews/Preview3DVolume.mat")) as Material;
		return s_Volume;
	}
}
}
...
public static readonly int invChannels =
	Shader.PropertyToID("_InvChannels");
public static readonly int invScale = Shader.PropertyToID("_InvScale");
public static readonly int globalScale =
	Shader.PropertyToID("_GlobalScale");
public static readonly int invResolution =
	Shader.PropertyToID("_InvResolution");
...
public static readonly int isNormalMap =
	Shader.PropertyToID("_IsNormalMap");
}

static class Styles {
public static readonly GUIContent ramp = EditorGUIUtility.TrTextContent(
	"Ramp", "Use gradient color ramp visualization");
public static readonly GUIContent quality = EditorGUIUtility.TrTextContent(
	"Quality", "Sample per texture pixel modifier");
public static readonly GUIContent alpha = EditorGUIUtility.TrTextContent(
	"Alpha", "Opacity of the texture visualization");
...
public static readonly GUIContent scale = EditorGUIUtility.TrTextContent(
	"Scale", "SDF value scale (how many texels SDF value of 1 represents)");
public static readonly GUIContent offset = EditorGUIUtility.TrTextContent(
	"Offset", "SDF surface is at this value");
public static readonly GUIContent volume = EditorGUIUtility.TrTextContent(
	"Volume", "Volumetric rendering display");
public static readonly GUIContent slice = EditorGUIUtility.TrTextContent(
	"Slice", "Display three planar slices of the texture");
public static readonly GUIContent sdf = EditorGUIUtility.TrTextContent(
	"SDF", "Display texture as a Signed Distance Field surface");
}
...
static Color TurboColorEvaluation(float t) {
	Vector4 kRedVec4 =
		new Vector4(0.13572138f, 4.61539260f, -42.66032258f, 132.13108234f);
	Vector4 kGreenVec4 =
		new Vector4(0.09140261f, 2.19418839f, 4.84296658f, -14.18503333f);
	Vector4 kBlueVec4 =
		new Vector4(0.10667330f, 12.64194608f, -60.58204836f, 110.36276771f);
	...
	get {
		if (s_TurboColorRamp == null) {
			...
			for (int i = 0; i < pixels.Length; i++) {
				pixels [i]
				        = TurboColorEvaluation(i * inversePixelCount);
			}
...
void InitPreviewUtility() {
	if (m_PreviewUtility == null) {
		...
		Vector3 scale =
			VoxelSize(GetTextureResolution(m_Texture), out inverseResolution);
		m_ViewDistance =
			Mathf.Lerp(1, 3, Mathf.Clamp01(scale.magnitude - 0.414f));
		m_PreviewUtility.camera.farClipPlane = s_MaxViewDistance * 2 + 1;
		m_PreviewUtility.camera.nearClipPlane = 0.1f;

		m_QualityModifier =
			Mathf.Clamp(8 - Mathf.Log(1.0f / inverseResolution, 2), 1, 8);
		m_Slice = Vector3.Scale(
			Vector3.one / 2,
			new Vector3(m_Texture.width, m_Texture.height, m_Texture.depth));
	}
}
...
float PreviewFloatField(GUIContent content, float value, float width) {
	...
	Rect controlRect =
		EditorGUILayout.GetControlRect(GUILayout.Width(labelWidth + width));
	int controlId = GUIUtility.GetControlID(FocusType.Keyboard);
	Rect labelRect = new Rect(controlRect.position,
...
public override void OnPreviewSettings() {
	...
	case Preview3DMode.Volume:
		m_Ramp =
			GUILayout.Toggle(m_Ramp, Styles.ramp, EditorStyles.toolbarButton);
		m_QualityModifier =
			PreviewSettingsSlider(Styles.quality, m_QualityModifier, 0.5f, 8,
			                      s_SliderWidth, s_FloatWidth, isInteger
			                      : false);
		m_MaxAlpha = PreviewSettingsSlider(Styles.alpha, m_MaxAlpha, 0.01f, 1,
		                                   s_SliderWidth, s_FloatWidth, isInteger
		                                   : false);
		break;
	...
	case Preview3DMode.Slice:
		m_Ramp =
			GUILayout.Toggle(m_Ramp, Styles.ramp, EditorStyles.toolbarButton);
		m_Slice.x =
			Mathf.Clamp(PreviewFloatField(Styles.x, m_Slice.x, s_FloatWidth), 0,
			            m_Texture.width);
		m_Slice.y =
			Mathf.Clamp(PreviewFloatField(Styles.y, m_Slice.y, s_FloatWidth), 0,
			            m_Texture.height);
		m_Slice.z =
			Mathf.Clamp(PreviewFloatField(Styles.z, m_Slice.z, s_FloatWidth), 0,
			            m_Texture.depth);
		break;
	...
	case Preview3DMode.SDF:
		...
		m_SurfaceOffset =
			PreviewFloatField(Styles.offset, m_SurfaceOffset, s_FloatWidth);
		break;

	default:
		throw new ArgumentException(
			      $"Unexpected \"Preview3DMode\" value: \"{m_Preview3DMode}\"");
	}

	if (GUILayout.Toggle(m_Preview3DMode == Preview3DMode.Volume, Styles.volume,
	                     EditorStyles.toolbarButton))
		m_Preview3DMode = Preview3DMode.Volume;
	if (GUILayout.Toggle(m_Preview3DMode == Preview3DMode.Slice, Styles.slice,
	                     EditorStyles.toolbarButton))
		m_Preview3DMode = Preview3DMode.Slice;
	if (GUILayout.Toggle(m_Preview3DMode == Preview3DMode.SDF, Styles.sdf,
	                     EditorStyles.toolbarButton))
...
static Vector3 GetTextureResolution(Texture texture) {
	if (texture.dimension != UnityEngine.Rendering.TextureDimension.Tex3D) {
		throw new ArgumentException(
			      $"Texture dimension must be {UnityEngine.Rendering.TextureDimension.Tex3D}, was {texture.dimension}");
	}
	...
	if (texture3D != null) {
		textureResolution =
			new Vector3(texture3D.width, texture3D.height, texture3D.depth);
	} else if (renderTexture != null) {
		textureResolution = new Vector3(renderTexture.width, renderTexture.height,
		                                renderTexture.volumeDepth);
	} else {
		throw new ArgumentException(
			      $"Could not cast texture to {typeof(Texture3D)} or {typeof(RenderTexture)}");
	}

	return textureResolution;
}

static Vector3 VoxelSize(Vector3 textureResolution,
                         out float inverseResolution) {
	inverseResolution =
		1.0f / Mathf.Max(textureResolution.x, textureResolution.y,
		                 textureResolution.z);
	return new Vector3(textureResolution.x * inverseResolution,
	                   textureResolution.y * inverseResolution,
	                   textureResolution.z * inverseResolution);
}

static Vector3 PrepareGeneralPreview(Material material, Texture texture,
                                     out Vector3 inverseScale,
                                     out float inverseResolution,
                                     Gradient customColorRamp = null) {
	Vector3 voxelSize =
		VoxelSize(GetTextureResolution(texture), out inverseResolution);
	inverseScale =
		new Vector3(1.0f / voxelSize.x, 1.0f / voxelSize.y, 1.0f / voxelSize.z);
	...
	if (customColorRamp != null) {
		material.SetTexture(
			MaterialProps.colorRamp,
			GradientPreviewCache.GetGradientPreview(customColorRamp));
	} else {
...
internal static void PrepareSDFPreview(Material material, Texture texture,
                                       Vector3 scale, float stepScale = 1,
                                       float surfaceOffset = 0,
                                       Gradient customColorRamp = null) {
	float inverseResolution;
	Vector3 inverseScale;
	PrepareGeneralPreview(material, texture, out inverseScale,
	                      out inverseResolution, customColorRamp);
...
internal static void PrepareSlicePreview(Material material, Texture texture,
                                         Vector3 slice, FilterMode filterMode,
	...
	PrepareGeneralPreview(material, texture, out inverseScale,
	                      out inverseResolution, customColorRamp);
	uint colorChannelCount =
		GraphicsFormatUtility.GetColorComponentCount(texture.graphicsFormat);

	Vector3 voxelSize = material.GetVector(MaterialProps.voxelSize);
	Vector3 textureResolution = GetTextureResolution(texture);
	Vector3 positions = new Vector3(slice.x / textureResolution.x,
	                                slice.y / textureResolution.y,
	                                slice.z / textureResolution.z);
	...
	material.SetVector(MaterialProps.invChannels,
	                   new Vector3(1.0f / colorChannelCount,
	                               1.0f / colorChannelCount,
	                               1.0f / colorChannelCount));
...
internal static int
PrepareVolumePreview(Material material, Texture texture, Vector3 scale,
                     float opacity, FilterMode filterMode, bool colorRamp,
                     Gradient customColorRamp, Camera camera, Matrix4x4 trs,
	...
	material.SetVector(MaterialProps.invChannels,
	                   new Vector3(1.0f / colorChannelCount,
	                               1.0f / colorChannelCount,
	                               1.0f / colorChannelCount));
	material.SetFloat(MaterialProps.alpha,
	                  Mathf.Pow(Mathf.Clamp01(opacity), 3));
	material.SetFloat(MaterialProps.filterMode, Convert.ToSingle(filterMode));
...
void DrawPreview() {
	...
	if (texture == null)
		return;
	if (!SystemInfo.supportsCompressed3DTextures &&
	    GraphicsFormatUtility.IsCompressedTextureFormat(texture.format))
		return;

	Quaternion rotation =
		Quaternion.Euler(-m_PreviewDir.y + s_InitialRotation.x,
		                 -m_PreviewDir.x + s_InitialRotation.y, 0);

	m_PreviewUtility.camera.transform.position =
		rotation * Vector3.back * m_ViewDistance;
	m_PreviewUtility.camera.transform.rotation = rotation;

	Vector3 scale = Vector3.one;
	switch (m_Preview3DMode) {
	case Preview3DMode.Volume:
		int sampleCount = PrepareVolumePreview(
			Materials.Volume, texture, scale, m_MaxAlpha, texture.filterMode,
			m_Ramp, null, m_PreviewUtility.camera, Matrix4x4.identity,
			m_QualityModifier);
		GL.PushMatrix();
		...
		return;
	case Preview3DMode.Slice:
		PrepareSlicePreview(Materials.Slice, texture, m_Slice, texture.filterMode,
		                    m_Ramp);
		m_PreviewUtility.DrawMesh(
			Handles.cubeMesh,
			Matrix4x4.Scale(scale + new Vector3(0.0001f, 0.0001f, 0.0001f)),
			Materials.Slice, 0);
		break;
	case Preview3DMode.SDF:
		PrepareSDFPreview(Materials.SDF, texture, scale, m_StepScale,
		                  m_SurfaceOffset);
		m_PreviewUtility.DrawMesh(Handles.cubeMesh, Matrix4x4.Scale(scale),
		                          Materials.SDF, 0);
		break;
	}
...
public override void OnPreviewGUI(Rect r, GUIStyle background) {
	if (!ShaderUtil.hardwareSupportsRectRenderTexture ||
		...
		if (Event.current.type == EventType.Repaint)
			EditorGUI.DropShadowLabel(new Rect(r.x, r.y, r.width, 40),
			                          "3D texture preview not supported");
		return;
	}
	Texture3D texture = target as Texture3D;
	if (texture == null)
	...
	if (!SystemInfo.supportsCompressed3DTextures &&
		...
		if (Event.current.type == EventType.Repaint)
			EditorGUI.DropShadowLabel(
				...
				"Compressed 3D texture preview is not supported");
		return;
	}
	...
	if (e.type == EventType.ScrollWheel) {
		m_ViewDistance =
			Mathf.Clamp(m_ViewDistance +
			            e.delta.y * (0.01f + Mathf.Sqrt(m_ViewDistance) / 20),
			            s_MinViewDistance, s_MaxViewDistance);
		e.Use();
		Repaint();
	}
