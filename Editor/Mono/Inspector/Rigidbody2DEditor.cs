...
internal class Rigidbody2DEditor : Editor {
  ...
  static readonly GUIContent m_FreezePositionLabel =
      EditorGUIUtility.TrTextContent("Freeze Position");
  static readonly GUIContent m_FreezeRotationLabel =
      EditorGUIUtility.TrTextContent("Freeze Rotation");
  ...
  public void OnEnable() {
    ...
    m_CollisionDetection =
        serializedObject.FindProperty("m_CollisionDetection");
  ...
  public override void OnInspectorGUI() {
    ...
    if (m_BodyType.hasMultipleDifferentValues) {
      EditorGUILayout.HelpBox(
          "Cannot edit properties that are body type specific when the selection contains different body types.",
          MessageType.Info);
    } else {
      // Non-static options.
      ...
      if (EditorGUILayout.BeginFadeGroup(m_ShowIsStatic.faded)) {
        ...
        if (EditorGUILayout.BeginFadeGroup(m_ShowIsKinematic.faded)) {
          ...
          // Only show mass property if selected objects have the same
          // useAutoMass value.
          if (!m_UseAutoMass.hasMultipleDifferentValues) {
            // If we're using auto-mass but either the object is part of a
            // prefab parent or is not active then we cannot show the calculated
            // mass value.
            if (m_UseAutoMass.boolValue &&
                targets.Any(
                    x => PrefabUtility.IsPartOfPrefabAsset(x) ||
                         !(x as Rigidbody2D).gameObject.activeInHierarchy)) {
              EditorGUILayout.HelpBox(
                  "The auto mass value cannot be displayed for a prefab or if the object is not active.  The value will be calculated for a prefab instance and when the object is active.",
                  MessageType.Info);
            } else {
        ...
        if (targets.Any(x =>(x as Rigidbody2D).interpolation !=
                            RigidbodyInterpolation2D.None)) {
          if (Physics2D.simulationMode == SimulationMode2D.Update)
            EditorGUILayout.HelpBox(
                "The physics simulation mode is set to run per-frame. Any interpolation mode will be ignored and can be set to 'None'.",
                MessageType.Info);

          if (Physics2D.simulationMode == SimulationMode2D.Script)
            EditorGUILayout.HelpBox(
                "The physics simulation mode is set to run manually in the scripts. Some or all selected Rigidbody2D are using an interpolation mode other than 'None' which will be executed per-frame. If the manual simulation is being run per-frame then the interpolation mode should be set to 'None'.",
                MessageType.Info);
        }

        GUILayout.BeginHorizontal();
        m_Constraints.isExpanded = EditorGUILayout.Foldout(
            m_Constraints.isExpanded, "Constraints", true);
  ...
  private void ShowBodyInfoProperties() {
    ...
    if (EditorGUILayout.BeginFadeGroup(m_ShowInfo.faded)) {
      if (targets.Length == 1) {
        ...
        EditorGUILayout.Vector2Field("World Center of Mass",
  ...
  void ShowContacts(Rigidbody2D body) {
    EditorGUI.indentLevel++;
    m_ShowContacts.target =
        EditorGUILayout.Foldout(m_ShowContacts.target, "Contacts", true);
    if (EditorGUILayout.BeginFadeGroup(m_ShowContacts.faded)) {
      var contactCount = body.GetContacts(m_Contacts);
      if (contactCount > 0) {
        m_ContactScrollPosition = EditorGUILayout.BeginScrollView(
            m_ContactScrollPosition, GUILayout.Height(180));
        EditorGUI.BeginDisabledGroup(true);
        for (var i = 0; i < contactCount; ++i) {
          var contact = m_Contacts[i];
          EditorGUILayout.HelpBox(string.Format("Contact#{0}", i),
                                  MessageType.None);
          EditorGUI.indentLevel++;
          EditorGUILayout.Vector2Field("Point", contact.point);
          EditorGUILayout.Vector2Field("Normal", contact.normal);
          EditorGUILayout.Vector2Field("Relative Velocity",
          ...
          EditorGUILayout.ObjectField("Collider", contact.collider,
                                      typeof(Collider2D), true);
          EditorGUILayout.ObjectField("Rigidbody", contact.rigidbody,
                                      typeof(Rigidbody2D), false);
          EditorGUILayout.ObjectField("OtherCollider", contact.otherCollider,
                                      typeof(Collider2D), false);
          EditorGUI.indentLevel--;
  ...
  void ConstraintToggle(Rect r, string label, RigidbodyConstraints2D value,
                        int bit) {
    var toggle = ((int) value & (1 << bit)) != 0;
    EditorGUI.showMixedValue =
        (m_Constraints.hasMultipleDifferentValuesBitwise & (1 << bit)) != 0;
  ...
  void ToggleFreezePosition(RigidbodyConstraints2D constraints,
                            GUIContent label, int x, int y) {
    GUILayout.BeginHorizontal();
  ...
  void ToggleFreezeRotation(RigidbodyConstraints2D constraints,
                            GUIContent label, int z) {
    GUILayout.BeginHorizontal();
    var r = GUILayoutUtility.GetRect(
        EditorGUIUtility.fieldWidth, EditorGUILayout.kLabelFloatMaxW,
        EditorGUI.kSingleLineHeight, EditorGUI.kSingleLineHeight,
