...
internal class Physics2DSettingsInspector : ProjectSettingsBaseEditor {
  private static class Styles {
    public static readonly GUIContent kJobOptionsLabel =
        EditorGUIUtility.TrTextContent(
            "Job Options (Experimental)",
            "Allows the configuration of multi-threaded physics using the job system.");
  }
  ...
  public void OnEnable() {
    ...
    m_AlwaysShowColliders =
        serializedObject.FindProperty("m_AlwaysShowColliders");
    m_ShowColliderSleep = serializedObject.FindProperty("m_ShowColliderSleep");
    m_ShowColliderContacts =
        serializedObject.FindProperty("m_ShowColliderContacts");
    ...
    m_ColliderAwakeColor =
        serializedObject.FindProperty("m_ColliderAwakeColor");
    m_ColliderAsleepColor =
        serializedObject.FindProperty("m_ColliderAsleepColor");
    m_ColliderContactColor =
        serializedObject.FindProperty("m_ColliderContactColor");
  ...
  public override void OnInspectorGUI() {
    ...
    m_GizmoSettingsFade.value = m_GizmoSettingsFade.target =
        EditorGUILayout.Foldout(m_GizmoSettingsFade.target, "Gizmos", true);
    ...
    if (m_GizmoSettingsFade.value) {
      if (EditorGUILayout.BeginFadeGroup(m_GizmoSettingsFade.faded)) {
        ...
        EditorGUILayout.Slider(m_ContactArrowScale, 0.1f, 1.0f,
                               m_ContactArrowScale.displayName);
    ...
    LayerMatrixGUI.DoGUI("Layer Collision Matrix",
                         ref m_ShowLayerCollisionMatrix, GetValue, SetValue);
  }
