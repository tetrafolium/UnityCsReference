...
internal class AudioMixerGroupEditor : Editor {
  ...
  internal override Rect DrawHeaderHelpAndSettingsGUI(Rect r) {
    ...
    GUI.Label(rect, GUIContent.Temp(group.controller.name),
              EditorStyles.miniLabel);

    return helpAndSettingsRect; // Return the helpAndSettings part, not the
                                // label under
  }

  // Add item to the context menu of the AudioMixerGroupController inspector
  // header
  [MenuItem(
  ...
  static void CopyAllEffectToSnapshots(MenuCommand command) {
    AudioMixerGroupController group =
        command.context as AudioMixerGroupController;
