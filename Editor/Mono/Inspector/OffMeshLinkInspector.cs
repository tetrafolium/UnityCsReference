...
internal class OffMeshLinkInspector : Editor {
  ...
  void OnEnable() {
    ...
    m_AutoUpdatePositions =
        serializedObject.FindProperty("m_AutoUpdatePositions");
  }
  ...
  private void SelectNavMeshArea() {
    ...
    for (var i = 0; i < areaNames.Length; i++) {
      if (GameObjectUtility.GetNavMeshAreaFromName(areaNames[i]) ==
    ...
    if (EditorGUI.EndChangeCheck()) {
      var newAreaIndex =
          GameObjectUtility.GetNavMeshAreaFromName(areaNames[area]);
