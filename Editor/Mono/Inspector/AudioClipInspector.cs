...
internal class AudioClipInspector : Editor {
  ...
  private bool playing => s_PlayingInstance == this && m_Clip != null &&
                          AudioUtil.IsPreviewClipPlaying();
  ...
  static void Init() {
    ...
    s_AutoPlayIcon = EditorGUIUtility.TrIconContent("preAudioAutoPlayOff",
                                                    "Turn Auto Play on/off");
    s_PlayIcon = EditorGUIUtility.TrIconContent("PlayButton", "Play");
    s_LoopIcon =
        EditorGUIUtility.TrIconContent("preAudioLoopOff", "Loop on/off");
  ...
  public void OnEnable() {
    ...
    m_HandleLinesMaterial =
        EditorGUIUtility.LoadRequired("SceneView/HandleLines.mat") as Material;
  }
  ...
  public override Texture2D RenderStaticPreview(string assetPath,
    ...
    // We're drawing into an offscreen here which will have a resolution defined
    // by EditorGUIUtility.pixelsPerPoint. This is different from the
    // DoRenderPreview call below where we draw directly to the screen, so we
    // need to take the higher resolution into account when drawing into the
    // offscreen, otherwise only the upper-left quarter of the preview texture
    // will be drawn.
    DoRenderPreview(false, clip, audioImporter,
                    new Rect(0.05f * width * EditorGUIUtility.pixelsPerPoint,
  ...
  public override bool HasPreviewGUI() { return (targets != null); }
  ...
  public override void OnPreviewSettings() {
...

    {
      using(new EditorGUI.DisabledScope(m_MultiEditing && !playing)) {
        bool newPlaying =
            GUILayout.Toggle(playing, s_PlayIcon, EditorStyles.toolbarButton);
      ...
      using(new EditorGUI.DisabledScope(m_MultiEditing)) {
        ...
        s_AutoPlay = GUILayout.Toggle(s_AutoPlay, s_AutoPlayIcon,
  ...
  // Passing in clip and importer separately as we're not completely done with
  // the asset setup at the time we're asked to generate the preview.
  private void DoRenderPreview(bool setMaterial, AudioClip clip,
                               AudioImporter audioImporter, Rect wantedRect,
                               float scaleFactor) {
    scaleFactor *= 0.95f; // Reduce amplitude slightly to make highly compressed
                          // signals fit.
    float[] minMaxData =
        (audioImporter == null) ? null : AudioUtil.GetMinMaxData(audioImporter);
    int numChannels = clip.channels;
    int numSamples =
        (minMaxData == null) ? 0 : (minMaxData.Length / (2 * numChannels));
    float h = (float) wantedRect.height / (float) numChannels;
    for (int channel = 0; channel < numChannels; channel++) {
      Rect channelRect = new Rect(wantedRect.x, wantedRect.y + h * channel,
                                  wantedRect.width, h);
      ...
      AudioCurveRendering.AudioMinMaxCurveAndColorEvaluator dlg = delegate(
        ...
        if (numSamples <= 0) {
          ...
          minValue =
              Mathf.Min(minMaxData[offset1 + 1], minMaxData[offset2 + 1]) *
              scaleFactor;
          maxValue =
              Mathf.Max(minMaxData[offset1 + 0], minMaxData[offset2 + 0]) *
              scaleFactor;
  ...
  public override void OnPreviewGUI(Rect r, GUIStyle background) {
    ...
    if (evt.type != EventType.Repaint && evt.type != EventType.Layout &&
      ...
      case EventType.MouseDown: {
        if (r.Contains(evt.mousePosition)) {
          var startSample =
              (int)(evt.mousePosition.x *
                    (AudioUtil.GetSampleCount(clip) / (int) r.width));
    ...
    bool previewAble =
        AudioUtil.HasPreview(clip) || !(AudioUtil.IsTrackerFile(clip));
    if (!previewAble) {
      float labelY = (r.height > 150) ? r.y + (r.height / 2) - 10
                                      : r.y + (r.height / 2) - 25;
      if (r.width > 64) {
        if (AudioUtil.IsTrackerFile(clip)) {
          EditorGUI.DropShadowLabel(
              new Rect(r.x, labelY, r.width, 20),
              string.Format("Module file with " +
                            AudioUtil.GetMusicChannelCount(clip) +
                            " channels."));
        } else
          EditorGUI.DropShadowLabel(new Rect(r.x, labelY, r.width, 20),
      ...
      if (m_Clip == clip) {
        ...
        System.TimeSpan ts =
            new System.TimeSpan(0, 0, 0, 0, (int)(t * 1000.0f));

        EditorGUI.DropShadowLabel(
            new Rect(s_WantedRect.x, s_WantedRect.y, s_WantedRect.width, 20),
            string.Format("Playing - {0:00}:{1:00}.{2:000}", ts.Minutes,
      ...
      if (Event.current.type == EventType.Repaint) {
        DoRenderPreview(true, clip, AudioUtil.GetImporterFromClip(clip),
                        s_WantedRect, 1.0f);
      }
      ...
      for (int i = 0; i < c; ++i) {
        if (c > 1 && r.width > 64) {
          var labelRect =
              new Rect(s_WantedRect.x + 5,
                       s_WantedRect.y + (s_WantedRect.height / c) * i, 30, 20);
      ...
      if (m_Clip == clip) {
        ...
        GUI.DrawTexture(new Rect(s_WantedRect.x + (int)(sec2px * t),
                                 s_WantedRect.y, 2, s_WantedRect.height),
                        EditorGUIUtility.whiteTexture);
        if (r.width > 64)
          EditorGUI.DropShadowLabel(
              new Rect(s_WantedRect.x, s_WantedRect.y, s_WantedRect.width, 20),
              string.Format("{0:00}:{1:00}.{2:000}", ts.Minutes, ts.Seconds,
                            ts.Milliseconds));
        else
          EditorGUI.DropShadowLabel(
              new Rect(s_WantedRect.x, s_WantedRect.y, s_WantedRect.width, 20),
  ...
  public override string GetInfoString() {
    ...
    AudioCompressionFormat platformFormat =
        AudioUtil.GetTargetPlatformSoundCompressionFormat(clip);
    AudioCompressionFormat editorFormat =
        AudioUtil.GetSoundCompressionFormat(clip);
    ...
    System.TimeSpan ts =
        new System.TimeSpan(0, 0, 0, 0, (int) AudioUtil.GetDuration(clip));

    if ((uint) AudioUtil.GetDuration(clip) == 0xffffffff)
      s += "Unlimited";
    else
      s += UnityString.Format("{0:00}:{1:00}.{2:000}", ts.Minutes, ts.Seconds,
                              ts.Milliseconds);
