...
// Provides a default appearance for a generic reorderable list that is
// typically used in inspector to draw arrays
internal class ReorderableListWrapper {
  ...
  internal SerializedProperty Property {
    ...
    set {
      ...
      if (m_ReorderableList != null) {
        ...
        int targetHash = GetTargetHash(
            m_ReorderableList.serializedProperty.serializedObject);
        if (m_lastArraySize != m_ArraySize.intValue ||
  ...
  GetPropertyIdentifier(SerializedProperty serializedProperty) {
    return serializedProperty.propertyPath +
           serializedProperty.serializedObject.targetObject.GetInstanceID();
  }
  ...
  void Init(bool reorderable) {
    ...
    m_ReorderableList =
        new ReorderableList(Property.serializedObject, Property.Copy(),
                            m_Reorderable, false, true, true);
    m_ReorderableList.headerHeight = ReorderableList.Defaults.minHeaderHeight;
    m_ReorderableList.m_IsEditable = true;
    // Check to see if the list has any elements, and use one to find out if
    // serialized property type has children
    m_ReorderableList.m_HasPropertyDrawer =
        (childProperty != null) ? childProperty.hasChildren : false;

    m_ReorderableList.onCanAddCallback +=
        (list) => { return m_IsNotInPrefabContextModeWithOverrides; };

    m_ReorderableList.onCanRemoveCallback +=
        (list) => { return m_IsNotInPrefabContextModeWithOverrides; };
  }

  internal void ClearCache() { m_ReorderableList.ClearCache(); }

  public float GetHeight() {
    return m_HeaderHeight +
           (m_Foldout &&
            m_ReorderableList != null ? Constants.kHeaderPadding +
                                     m_ReorderableList.GetHeight() : 0.0f);
  }

  public void Draw(Rect r) { Draw(r, ReorderableList.Defaults.infinityRect); }

  public void Draw(Rect r, Rect visibleArea) {
    var prefabStage = PrefabStageUtility.GetCurrentPrefabStage();
    m_IsNotInPrefabContextModeWithOverrides =
        prefabStage == null || prefabStage.mode != PrefabStage.Mode.InContext ||
        !PrefabStage.s_PatchAllOverriddenProperties ||
        Selection.objects.All(
            obj =>
                PrefabUtility.IsPartOfAnyPrefab(obj) &&
                !AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(obj))
                     .Equals(
                         AssetDatabase.AssetPathToGUID(prefabStage.assetPath)));
    m_ReorderableList.draggable =
        m_Reorderable && m_IsNotInPrefabContextModeWithOverrides;

    Rect headerRect = new Rect(r.x, r.y, r.width, m_HeaderHeight);
    Rect sizeRect =
        new Rect(headerRect.xMax - Constants.kArraySizeWidth, headerRect.y,
    ...
    m_Foldout =
        EditorGUI.BeginFoldoutHeaderGroup(headerRect, m_Foldout, m_Header);
    ...
    if (headerRect.Contains(Event.current.mousePosition)) {
      if (Event.current.type == EventType.DragUpdated ||
        ...
        foreach (var o in objReferences) {
          if (EditorGUI.ValidateObjectFieldAssignment(
                  new[]{o}, typeof(Object),
                  m_ReorderableList.serializedProperty,
