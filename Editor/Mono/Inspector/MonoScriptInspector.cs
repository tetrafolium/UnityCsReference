...
internal class MonoScriptImporterInspector : AssetImporterEditor {
  ...
  internal override void OnHeaderIconGUI(Rect iconRect) {
    if (assetTargets != null) {
      ...
      dropDownRect.size =
          EditorGUI.GetObjectIconDropDownSize(iconRect.width, iconRect.height);
      EditorGUI.ObjectIconDropDown(dropDownRect, assetTargets, true, null,
                                   m_Icon);
    } else {
  ...
  [MenuItem("CONTEXT/MonoImporter/Reset", isValidateFunction
            : true)]
  ...
  void ShowFieldInfo(Type type, MonoImporter importer, List<string> names,
                     List<UnityEngine.Object> objects, ref bool didModify) {
    // Only show default properties for types that support it (so far only
    ...
    foreach (FieldInfo field in infos) {
      ...
      if (field.FieldType.IsSubclassOf(typeof(UnityEngine.Object)) ||
        ...
        UnityEngine.Object newTarget = EditorGUILayout.ObjectField(
            ObjectNames.NicifyVariableName(field.Name), oldTarget,
  ...
  public override void OnInspectorGUI() {
    ...
    if (script) {
      ...
      using(new EditorGUIUtility.IconSizeScope(
          new Vector2(m_RowHeight, m_RowHeight))) {
...
internal class TextAssetInspector : Editor {
  ...
  public virtual void OnEnable() { alwaysAllowExpansion = true; }
  ...
  public override void OnInspectorGUI() {
    ...
    if (textAsset != null) {
      ...
      if (targets.Length > 1) {
        text = targetTitle;
      } else if (Path.GetExtension(AssetDatabase.GetAssetPath(textAsset)) !=
                 ".bytes") {
        text = textAsset.text;
        if (text.Length > kMaxChars)
          text = text.Substring(0, kMaxChars) + "...\n\n<...etc...>";
      }
      Rect rect = GUILayoutUtility.GetRect(EditorGUIUtility.TempContent(text),
                                           m_TextStyle);
      rect.x = 0;
      rect.y -= 3;
      ...
      if (assemblyName != null) {
        ...
        var assemblyDefinitionFile =
            Compilation.CompilationPipeline
                .GetAssemblyDefinitionFilePathFromScriptPath(assetPath);

        if (assemblyDefinitionFile != null) {
          var assemblyDefintionFileAsset =
              AssetDatabase.LoadAssetAtPath<TextAsset>(assemblyDefinitionFile);

          using(new EditorGUI.DisabledScope(true)) {
            EditorGUILayout.ObjectField("Definition File",
                                        assemblyDefintionFileAsset,
