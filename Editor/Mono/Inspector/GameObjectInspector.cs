...
static class Styles {
public static GUIContent typelessIcon =
	EditorGUIUtility.IconContent("Prefab Icon");
public static GUIContent overridesContent =
	EditorGUIUtility.TrTextContent("Overrides");
public static GUIContent staticContent = EditorGUIUtility.TrTextContent(
	"Static",
	"Enable the checkbox to mark this GameObject as static for all systems.\n\nDisable the checkbox to mark this GameObject as not static for all systems.\n\nUse the drop-down menu to mark as this GameObject as static or not static for individual systems.");
public static GUIContent layerContent = EditorGUIUtility.TrTextContent(
	"Layer",
	"The layer that this GameObject is in.\n\nChoose Add Layer... to edit the list of available layers.");
public static GUIContent tagContent = EditorGUIUtility.TrTextContent(
	"Tag",
	"The tag that this GameObject has.\n\nChoose Untagged to remove the current tag.\n\nChoose Add Tag... to edit the list of available tags.");

public static float tagFieldWidth => EditorGUI.CalcPrefixLabelWidth(
	Styles.tagContent, EditorStyles.boldLabel);
public static float layerFieldWidth => EditorGUI.CalcPrefixLabelWidth(
	Styles.layerContent, EditorStyles.boldLabel);
...
public static GUIStyle prefabButtonsHorizontalLayout = new GUIStyle {
	fixedHeight = 17, margin = new RectOffset {
		top = 1, bottom = 1
	}
};

public static GUIContent goTypeLabelMultiple =
	EditorGUIUtility.TrTextContent("Multiple");
private static GUIContent regularPrefab =
	EditorGUIUtility.TrTextContent("Prefab");
private static GUIContent disconnectedPrefab = EditorGUIUtility.TrTextContent(
	"Prefab",
	"You have broken the prefab connection. Changes to the prefab will not be applied to this object before you Apply or Revert.");
private static GUIContent modelPrefab =
	EditorGUIUtility.TrTextContent("Model");
private static GUIContent disconnectedModelPrefab =
	EditorGUIUtility.TrTextContent(
		"Model",
		"You have broken the prefab connection. Changes to the model will not be applied to this object before you Revert.");
private static GUIContent variantPrefab =
	EditorGUIUtility.TrTextContent("Variant");
private static GUIContent disconnectedVariantPrefab =
	EditorGUIUtility.TrTextContent(
		"Variant",
		"You have broken the prefab connection. Changes to the prefab will not be applied to this object before you Apply or Revert.");
private static GUIContent missingPrefabAsset =
	EditorGUIUtility.TrTextContent(
		"Missing", "The source Prefab or Model has been deleted.");
public static GUIContent openModel =
	EditorGUIUtility.TrTextContent("Open", "Open Model in external tool.");
public static GUIContent openPrefab = EditorGUIUtility.TrTextContent(
	"Open",
	"Open Prefab Asset '{0}'\nPress modifier key [Alt] to open in isolation.");
public static string selectString = L10n.Tr("Select");

// Matrix based on two enums:
// Columns correspond to PrefabTypeUtility.PrefabAssetType (see comments
// above rows). Rows correspond to PrefabTypeUtility.PrefabInstanceStatus
// (None, Connected, Disconnected, Missing). If missing, both enums will be
// "Missing".
...
static Styles() {
	// Seems to be a bug in the way controls with margin internal to layout
	// groups with padding calculate position. We'll work around it here.
...
Dictionary<int, PreviewData> m_PreviewInstances =
	new Dictionary<int, PreviewData>();
...
void CalculatePrefabStatus() {
	...
	PrefabInstanceStatus firstStatus =
		PrefabUtility.GetPrefabInstanceStatus(targets[0]);
	...
	foreach (var o in targets) {
		...
		if (!PrefabUtility.IsAnyPrefabInstanceRoot(go))
			m_IsPrefabInstanceAnyRoot =
				false; // Conservative is false if any is false
		if (!m_IsPrefabInstanceAnyRoot ||
		    !PrefabUtility.IsOutermostPrefabInstanceRoot(go))
			m_IsPrefabInstanceOutermostRoot =
				false; // Conservative is false if any is false
		...
		GameObject originalSourceOrVariant =
			PrefabUtility.GetOriginalSourceOrVariantRoot(go);
		if (originalSourceOrVariant != null &&
		    PrefabUtility.IsPartOfImmutablePrefab(originalSourceOrVariant))
...
internal bool DrawInspector() {
	...
	// Can't do this in OnEnable since it will cause Styles static initializer
	// to be called and access properties on EditorStyles static class before
	// that one has been initialized.
	if (m_OpenPrefabContent == null) {
		if (targets.Length == 1) {
			GameObject originalSourceOrVariant =
				PrefabUtility.GetOriginalSourceOrVariantRoot((GameObject) target);
			if (originalSourceOrVariant != null)
				m_OpenPrefabContent =
					new GUIContent(Styles.openPrefab.text,
					               string.Format(Styles.openPrefab.tooltip,
					                             originalSourceOrVariant.name));
		}
		if (m_OpenPrefabContent == null)
	...
	Vector2 dropDownSize =
		EditorGUI.GetObjectIconDropDownSize(kIconSize, kIconSize);
	EditorGUI.ObjectIconDropDown(
		GUILayoutUtility.GetRect(dropDownSize.x, dropDownSize.y,
		                         GUILayout.ExpandWidth(false)),
		targets, true, icon, m_Icon);
	DrawPostIconContent();
	...
	using (new EditorGUI.DisabledScope(m_ImmutableSelf)) {
		EditorGUILayout.BeginVertical();
		{
			...
			{
				EditorGUILayout.BeginHorizontal(
					GUILayout.Width(Styles.tagFieldWidth));
				{
					GUILayout.FlexibleSpace();

					// IsActive
					EditorGUI.PropertyField(
						GUILayoutUtility.GetRect(EditorStyles.toggle.padding.left,
						                         EditorGUIUtility.singleLineHeight,
						                         EditorStyles.toggle,
						                         GUILayout.ExpandWidth(false)),
						m_IsActive, GUIContent.none);
				}
...
void DoPrefixLabel(GUIContent label, GUIStyle style) {
	var rect =
		GUILayoutUtility.GetRect(label, style, GUILayout.ExpandWidth(false));
...
private void DoPrefabButtons() {
	...
	using (new EditorGUI.DisabledScope(m_PlayModeObjects)) {
		...
		// Prefab information
		PrefabAssetType singlePrefabType =
			PrefabUtility.GetPrefabAssetType(target);
		PrefabInstanceStatus singleInstanceStatus =
			PrefabUtility.GetPrefabInstanceStatus(target);
		GUIContent prefixLabel;
		if (targets.Length > 1) {
			prefixLabel = Styles.goTypeLabelMultiple;
		} else {
			prefixLabel = Styles.goTypeLabel[(int) singlePrefabType,
			                                 (int) singleInstanceStatus];
		}

		if (prefixLabel != null) {
			EditorGUILayout.BeginHorizontal(
				GUILayout.Width(kIconSize + Styles.tagFieldWidth));
			GUILayout.FlexibleSpace();
			if (m_IsDisconnected || m_IsMissing) {
				GUI.contentColor =
					GUI.skin.GetStyle("CN StatusWarn").normal.textColor;
		...
		if (!m_IsMissing) {
			using (new EditorGUI.DisabledScope(targets.Length > 1)) {
				if (singlePrefabType == PrefabAssetType.Model) {
					// Open Model Prefab
					if (GUILayout.Button(Styles.openModel,
					                     EditorStyles.miniButtonLeft)) {
						GameObject asset =
							PrefabUtility.GetOriginalSourceOrVariantRoot(target);
					...
					using (new EditorGUI.DisabledScope(m_ImmutableSourceAsset)) {
						if (GUILayout.Button(m_OpenPrefabContent,
						                     EditorStyles.miniButtonLeft)) {
							GameObject asset =
								PrefabUtility.GetOriginalSourceOrVariantRoot(target);
							var prefabStageMode =
								PrefabStageUtility.GetPrefabStageModeFromModifierKeys();
							PrefabStageUtility.OpenPrefab(
								AssetDatabase.GetAssetPath(asset), (GameObject) target,
								prefabStageMode,
			...
			// Select prefab
			if (GUILayout.Button(Styles.selectString,
			                     EditorStyles.miniButtonRight)) {
				HashSet<GameObject> selectedAssets = new HashSet<GameObject>();
				for (int i = 0; i < targets.Length; i++) {
					GameObject prefabGo =
						PrefabUtility.GetOriginalSourceOrVariantRoot(targets[i]);

					// Because of legacy prefab references we have to have this extra
					// step to make sure we ping the prefab asset correctly. Reason is
					// that scene files created prior to making prefabs CopyAssets will
					// reference prefabs as if they are serialized assets. Those
					// references works fine but we are not able to ping objects loaded
					// directly from the asset file, so we have to make sure we ping the
					// metadata version of the prefab.
			...
			// Should be EditorGUILayout.Space, except it does not have ExpandWidth
			// set to false. Maybe we can change that?
			GUILayoutUtility.GetRect(6, 6, GUILayout.ExpandWidth(false));

			// Reserve space regardless of whether the button is there or not to
			// avoid jumps in button sizes.
			Rect rect = GUILayoutUtility.GetRect(Styles.overridesContent,
			                                     Styles.overridesDropdown);
			if (m_IsPrefabInstanceOutermostRoot) {
				if (EditorGUI.DropdownButton(rect, Styles.overridesContent,
				                             FocusType.Passive)) {
					if (targets.Length > 1)
						PopupWindow.Show(
							rect, new PrefabOverridesWindow(
								targets.Select(e =>(GameObject) e).ToArray()));
					else
						PopupWindow.Show(rect,
						                 new PrefabOverridesWindow((GameObject) target));
					GUIUtility.ExitGUI();
				}
			}
		}
...
private void DoLayerField(GameObject go) {
	...
	Rect layerRect =
		GUILayoutUtility.GetRect(GUIContent.none, Styles.layerPopup);
	EditorGUI.BeginProperty(layerRect, GUIContent.none, m_Layer);
	EditorGUI.BeginChangeCheck();
	int layer = EditorGUI.LayerField(layerRect, Styles.layerContent, go.layer,
	                                 Styles.layerPopup);
	if (EditorGUI.EndChangeCheck()) {
		GameObjectUtility.ShouldIncludeChildren
		        includeChildren = GameObjectUtility.DisplayUpdateChildrenDialogIfNeeded(
			targets.OfType<GameObject>(), L10n.Tr("Change Layer"),
			string.Format(
				L10n.Tr(
					"Do you want to set layer to {0} for all child objects as well?"),
		...
		// Displaying the dialog to ask the user whether to update children nukes
		// the gui state
...
private void DoStaticFlagsDropDown(GameObject go) {
	...
	int changedFlags;
	bool changedToValue;
	var rect = GUILayoutUtility.GetRect(GUIContent.none, Styles.staticDropdown,
	                                    GUILayout.ExpandWidth(false));

	rect.height = Math.Max(EditorGUIUtility.singleLineHeight, rect.height);
	EditorGUI.EnumFlagsField(
		rect, GUIContent.none, GameObjectUtility.GetStaticEditorFlags(go),
		false, out changedFlags, out changedToValue, Styles.staticDropdown);
...
private void DoStaticToggleField(GameObject go) {
	var staticRect =
		GUILayoutUtility.GetRect(Styles.staticContent, EditorStyles.toggle,
		                         GUILayout.ExpandWidth(false));

	staticRect.height =
		Math.Max(EditorGUIUtility.singleLineHeight, staticRect.height);
	EditorGUI.BeginProperty(staticRect, GUIContent.none, m_StaticEditorFlags);
	EditorGUI.BeginChangeCheck();
	var toggleRect = staticRect;
	EditorGUI.showMixedValue |= ShowMixedStaticEditorFlags(
		(StaticEditorFlags) m_StaticEditorFlags.intValue);
	// Ignore mouse clicks that are not with the primary (left) mouse button so
	// those can be grabbed by other things later.
	...
	var toggled =
		EditorGUI.ToggleLeft(toggleRect, Styles.staticContent, go.isStatic);
	if (nonLeftClick)
		evt.type = origType;
	EditorGUI.showMixedValue = false;
	if (EditorGUI.EndChangeCheck()) {
...
PreviewData GetPreviewData() {
	...
	if (!m_PreviewInstances.TryGetValue(referenceTargetIndex,
	                                    out previewData)) {
...
public static void GetRenderableBoundsRecurse(ref Bounds bounds,
                                              GameObject go) {
	// Do we have a mesh?
	...
	if (renderer && filter && filter.sharedMesh) {
		...
		// with renderer.bounds. This ensures correct bounds for meshes with origo
		// outside the mesh.
...
private static float GetRenderableCenterRecurse(ref Vector3 center,
	...
	if (depth > minDepth) {
		...
		if (renderer == null && filter == null && skin == null &&
		    sprite == null && billboard == null) {
			ret = 1;
			center = center + go.transform.position;
		} else if (renderer != null && filter != null) {
			// case 542145, epsilon is too small. Accept up to 1 centimeter before
			// discarding this model.
			if (Vector3.Distance(renderer.bounds.center, go.transform.position) <
			    0.01F) {
				ret = 1;
				center = center + go.transform.position;
			}
			...
			// case 542145, epsilon is too small. Accept up to 1 centimeter before
			// discarding this model.
			if (Vector3.Distance(skin.bounds.center, go.transform.position) <
			    0.01F) {
				ret = 1;
				center = center + go.transform.position;
			}
		} else if (sprite != null) {
			if (Vector3.Distance(sprite.bounds.center, go.transform.position) <
			    0.01F) {
				ret = 1;
				center = center + go.transform.position;
			}
		} else if (billboard != null) {
			if (Vector3.Distance(billboard.bounds.center, go.transform.position) <
			    0.01F) {
				ret = 1;
				center = center + go.transform.position;
			}
	...
	// Recurse into children
	foreach (Transform t in go.transform) {
		ret += GetRenderableCenterRecurse(ref center, t.gameObject, depth,
		                                  minDepth, maxDepth);
	}
...
public static Vector3 GetRenderableCenterRecurse(GameObject go, int minDepth,
                                                 int maxDepth) {
	Vector3 center = Vector3.zero;

	float sum =
		GetRenderableCenterRecurse(ref center, go, 0, minDepth, maxDepth);
...
private void DoRenderPreview() {
	...
	Bounds bounds =
		new Bounds(previewData.gameObject.transform.position, Vector3.zero);
	...
	previewData.renderUtility.lights[0].transform.rotation =
		rot * Quaternion.Euler(40f, 40f, 0);
	previewData.renderUtility.lights[1].intensity = .7f;
	previewData.renderUtility.lights[1].transform.rotation =
		rot * Quaternion.Euler(340, 218, 177);
...
internal void OnSceneDragInternal(SceneView sceneView, int index,
                                  EventType type, Vector2 mousePosition,
                                  bool alt) {
	GameObject go = target as GameObject;
	if (!PrefabUtility.IsPartOfPrefabAsset(go))
		return;
	...
	case EventType.DragUpdated:
		Scene destinationScene = sceneView.customScene.IsValid()
		                   ? sceneView.customScene
		                   : SceneManager.GetActiveScene();
		if (m_DragObject == null) {
			// While dragging the instantiated prefab we do not want to record undo
			// for this object this will cause a remerge of the instance since
			// changes are undone while dragging. The DrivenRectTransformTracker by
			// default records Undo when used when driving UI components. This
			// breaks our hideflag setup below due to a remerge of the dragged
			// instance. StartRecordingUndo() is called on DragExited. Fixes case
			// 1223793.
			DrivenRectTransformTracker.StopRecordingUndo();

			if (!EditorApplication.isPlaying ||
			    EditorSceneManager.IsPreviewScene(destinationScene)) {
				m_DragObject = (GameObject) PrefabUtility.InstantiatePrefab(
					prefabAssetRoot, destinationScene);
			...
			if (HandleUtility.ignoreRaySnapObjects == null)
				HandleUtility.ignoreRaySnapObjects =
					m_DragObject.GetComponentsInChildren<Transform>();
			else
				HandleUtility.ignoreRaySnapObjects =
					HandleUtility.ignoreRaySnapObjects
					.Union(m_DragObject.GetComponentsInChildren<Transform>())
					.ToArray();

			PrefabStage prefabStage = PrefabStageUtility.GetCurrentPrefabStage();
			if (prefabStage != null) {
				GameObject prefab = AssetDatabase.LoadMainAssetAtPath(
					prefabStage.assetPath) as GameObject;
		...
		if (index == 0) {
			s_PlaceObject = HandleUtility.PlaceObject(
				mousePosition, out s_PlaceObjectPoint, out s_PlaceObjectNormal);
		}
		...
		if (s_PlaceObject) {
			if (Tools.pivotMode == PivotMode.Center) {
				float geomOffset = HandleUtility.CalcRayPlaceOffset(
					m_DragObject.GetComponentsInChildren<Transform>(), normal);
				if (geomOffset != Mathf.Infinity)
					offset = Vector3.Dot(m_DragObject.transform.position, normal) -
					         geomOffset;
			}
			m_DragObject.transform.position =
				Matrix4x4.identity.MultiplyPoint(point + (normal * offset));
		} else
			m_DragObject.transform.position =
		...
		if (alt) {
			...
			m_DragObject.transform.position +=
				prefabAssetRoot.transform.localPosition;
		}
		...
		// Schedule selection clearing for when we start performing the actual
		// drag action
	...
	case EventType.DragExited:
		// DragExited is always fired after DragPerform so we do no need to call
		// StartRecordingUndo in DragPerform
...
internal static void DragPerform(SceneView sceneView,
                                 GameObject draggedObject, GameObject go) {
	...
	string uniqueName =
		GameObjectUtility.GetUniqueNameForSibling(parent, draggedObject.name);
	if (parent != null)
		draggedObject.transform.parent = parent;
	draggedObject.hideFlags = 0;
	Undo.RegisterCreatedObjectUndo(draggedObject,
	                               "Place " + draggedObject.name);
	...
	if (s_ShouldClearSelection) {
		...
		// Since this inspector code executes for each dragged GameObject we
		// should retain selection to all of them by joining them to the previous
		// selection list
		Selection.objects =
			Selection.gameObjects.Union(new[] {draggedObject}).ToArray();
	}
	HandleUtility.ignoreRaySnapObjects = null;
