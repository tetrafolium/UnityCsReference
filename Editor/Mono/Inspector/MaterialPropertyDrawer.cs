...
internal class MaterialPropertyHandler {
  ...
  public MaterialPropertyDrawer propertyDrawer {
    get { return m_PropertyDrawer; }
  }

  public bool IsEmpty() {
    return m_PropertyDrawer == null &&
           (m_DecoratorDrawers == null || m_DecoratorDrawers.Count == 0);
  }

  public void OnGUI(ref Rect position, MaterialProperty prop, GUIContent label,
  ...
  private static Dictionary<string, MaterialPropertyHandler>
      s_PropertyHandlers = new Dictionary<string, MaterialPropertyHandler>();
  ...
  private static MaterialPropertyDrawer CreatePropertyDrawer(Type klass,
                                                             string argsText) {
    ...
    for (var i = 0; i < argStrings.Length; ++i) {
      ...
      if (float.TryParse(
              arg, System.Globalization.NumberStyles.Float,
  ...
  private static MaterialPropertyDrawer
  GetShaderPropertyDrawer(string attrib, out bool isDecorator) {
    isDecorator = false;
    ...
    foreach (var klass in TypeCache
                 .GetTypesDerivedFrom<MaterialPropertyDrawer>()) {
      ...
      //@TODO: namespaces?
      if (klass.Name == className || klass.Name == className + "Drawer" ||
        ...
        try {
          ...
          Debug.LogWarningFormat(
              "Failed to create material drawer {0} with arguments '{1}'",
              className, args);
          return null;
        }
      }
  ...
  private static MaterialPropertyHandler GetShaderPropertyHandler(Shader shader,
                                                                  string name) {
    ...
    foreach (var attr in attribs) {
      ...
      MaterialPropertyDrawer drawer =
          GetShaderPropertyDrawer(attr, out isDecorator);
      if (drawer != null) {
        if (isDecorator) {
          ...
          if (handler.m_PropertyDrawer != null) {
            Debug.LogWarning(
                string.Format(
                    "Shader property {0} already has a property drawer", name),
                shader);
          }
  ...
  internal static MaterialPropertyHandler GetHandler(Shader shader,
                                                     string name) {
    if (shader == null)
      return null;
...
public abstract class MaterialPropertyDrawer {
  public virtual void OnGUI(Rect position, MaterialProperty prop,
                            GUIContent label, MaterialEditor editor) {
    OnGUI(position, prop, label.text, editor);
  }

  public virtual void OnGUI(Rect position, MaterialProperty prop, string label,
                            MaterialEditor editor) {
    EditorGUI.LabelField(position, new GUIContent(label),
                         EditorGUIUtility.TempContent("No GUI Implemented"));
  }

  public virtual float GetPropertyHeight(MaterialProperty prop, string label,
                                         MaterialEditor editor) {
...
// They aren't directly used by the code, but can be used by writing
// attribute-like syntax in shaders, e.g. [Toggle] in front of a shader property
// will end up using MaterialToggleDrawer to display it as a toggle.
...
internal class MaterialToggleUIDrawer : MaterialPropertyDrawer {
  ...
  public override void OnGUI(Rect position, MaterialProperty prop,
                             GUIContent label, MaterialEditor editor) {
    if (!IsPropertyTypeSuitable(prop)) {
      GUIContent c = EditorGUIUtility.TempContent(
          "Toggle used on a non-float property: " + prop.name,
...
internal class MaterialToggleDrawer : MaterialToggleUIDrawer {
  ...
  public MaterialToggleDrawer(string keyword) { this.keyword = keyword; }
  ...
  protected void SetKeywordInternal(MaterialProperty prop, bool on,
                                    string defaultKeywordSuffix) {
    // if no keyword is provided, use <uppercase property name> +
    // defaultKeywordSuffix
    string kw = string.IsNullOrEmpty(keyword)
                    ? prop.name.ToUpperInvariant() + defaultKeywordSuffix
                    : keyword;
...
// This is useful when adding Toggles to existing shaders while maintaining
...
internal class MaterialPowerSliderDrawer : MaterialPropertyDrawer {
  ...
  public MaterialPowerSliderDrawer(float power) { this.power = power; }
  ...
  public override void OnGUI(Rect position, MaterialProperty prop,
    ...
    if (prop.type != MaterialProperty.PropType.Range) {
      GUIContent c = EditorGUIUtility.TempContent(
          "PowerSlider used on a non-range property: " + prop.name,
          EditorGUIUtility.GetHelpIcon(MessageType.Warning));
      EditorGUI.LabelField(position, c, EditorStyles.helpBox);
      return;
    }
...
internal class MaterialIntRangeDrawer : MaterialPropertyDrawer {
  public override void OnGUI(Rect position, MaterialProperty prop,
    ...
    if (prop.type != MaterialProperty.PropType.Range) {
      GUIContent c = EditorGUIUtility.TempContent(
          "IntRange used on a non-range property: " + prop.name,
          EditorGUIUtility.GetHelpIcon(MessageType.Warning));
      EditorGUI.LabelField(position, c, EditorStyles.helpBox);
      return;
    }
...
internal class MaterialKeywordEnumDrawer : MaterialPropertyDrawer {
  ...
  public MaterialKeywordEnumDrawer(string kw1, string kw2, string kw3)
      : this(new[]{kw1, kw2, kw3}) {}
  public MaterialKeywordEnumDrawer(string kw1, string kw2, string kw3,
                                   string kw4)
      : this(new[]{kw1, kw2, kw3, kw4}) {}
  public MaterialKeywordEnumDrawer(string kw1, string kw2, string kw3,
                                   string kw4, string kw5)
      : this(new[]{kw1, kw2, kw3, kw4, kw5}) {}
  public MaterialKeywordEnumDrawer(string kw1, string kw2, string kw3,
                                   string kw4, string kw5, string kw6)
      : this(new[]{kw1, kw2, kw3, kw4, kw5, kw6}) {}
  public MaterialKeywordEnumDrawer(string kw1, string kw2, string kw3,
                                   string kw4, string kw5, string kw6,
                                   string kw7)
      : this(new[]{kw1, kw2, kw3, kw4, kw5, kw6, kw7}) {}
  public MaterialKeywordEnumDrawer(string kw1, string kw2, string kw3,
                                   string kw4, string kw5, string kw6,
                                   string kw7, string kw8)
      : this(new[]{kw1, kw2, kw3, kw4, kw5, kw6, kw7, kw8}) {}
  public MaterialKeywordEnumDrawer(string kw1, string kw2, string kw3,
                                   string kw4, string kw5, string kw6,
                                   string kw7, string kw8, string kw9)
      : this(new[]{kw1, kw2, kw3, kw4, kw5, kw6, kw7, kw8, kw9}) {}
  ...
  public override void OnGUI(Rect position, MaterialProperty prop,
    ...
    if (!IsPropertyTypeSuitable(prop)) {
      GUIContent c = EditorGUIUtility.TempContent(
          "KeywordEnum used on a non-float property: " + prop.name,
          EditorGUIUtility.GetHelpIcon(MessageType.Warning));
      EditorGUI.LabelField(position, c, EditorStyles.helpBox);
      return;
    }
...
internal class MaterialEnumDrawer : MaterialPropertyDrawer {
  ...
  // Single argument: enum type name; entry names & values fetched via
  // reflection
  public MaterialEnumDrawer(string enumName) {
    var loadedTypes =
        AppDomain.CurrentDomain.GetAssemblies()
            .SelectMany(x => AssemblyHelper.GetTypesFromAssembly(x))
            .ToArray();
    try {
      ...
      Debug.LogWarningFormat(
          "Failed to create MaterialEnum, enum {0} not found", enumName);
      throw;
    }
  }
  ...
  public MaterialEnumDrawer(string n1, float v1, string n2, float v2)
      : this(new[]{n1, n2}, new[]{v1, v2}) {}
  public MaterialEnumDrawer(string n1, float v1, string n2, float v2, string n3,
                            float v3)
      : this(new[]{n1, n2, n3}, new[]{v1, v2, v3}) {}
  public MaterialEnumDrawer(string n1, float v1, string n2, float v2, string n3,
                            float v3, string n4, float v4)
      : this(new[]{n1, n2, n3, n4}, new[]{v1, v2, v3, v4}) {}
  public MaterialEnumDrawer(string n1, float v1, string n2, float v2, string n3,
                            float v3, string n4, float v4, string n5, float v5)
      : this(new[]{n1, n2, n3, n4, n5}, new[]{v1, v2, v3, v4, v5}) {}
  public MaterialEnumDrawer(string n1, float v1, string n2, float v2, string n3,
                            float v3, string n4, float v4, string n5, float v5,
                            string n6, float v6)
      : this(new[]{n1, n2, n3, n4, n5, n6}, new[]{v1, v2, v3, v4, v5, v6}) {}
  public MaterialEnumDrawer(string n1, float v1, string n2, float v2, string n3,
                            float v3, string n4, float v4, string n5, float v5,
                            string n6, float v6, string n7, float v7)
      : this(new[]{n1, n2, n3, n4, n5, n6, n7},
             new[]{v1, v2, v3, v4, v5, v6, v7}) {}
  ...
  public override void OnGUI(Rect position, MaterialProperty prop,
    ...
    if (prop.type != MaterialProperty.PropType.Float &&
        prop.type != MaterialProperty.PropType.Range) {
      GUIContent c = EditorGUIUtility.TempContent(
          "Enum used on a non-float property: " + prop.name,
          EditorGUIUtility.GetHelpIcon(MessageType.Warning));
      EditorGUI.LabelField(position, c, EditorStyles.helpBox);
      return;
    }
...
// [Space] or [Space(height)] decorator creates a vertical space before shader
// property.
internal class MaterialSpaceDecorator : MaterialPropertyDrawer {
  private readonly float height;

  public MaterialSpaceDecorator() { height = 6f; }

  public MaterialSpaceDecorator(float height) { this.height = height; }
  ...
  public override void OnGUI(Rect position, MaterialProperty prop, string label,
                             MaterialEditor editor) {}
}
...
internal class MaterialHeaderDecorator : MaterialPropertyDrawer {
  ...
  public MaterialHeaderDecorator(string header) { this.header = header; }
