...
internal class TimelineControl {
  ...
  private List<TimelineControl.PivotSample> m_SrcPivotList =
      new List<TimelineControl.PivotSample>();
  private List<TimelineControl.PivotSample> m_DstPivotList =
      new List<TimelineControl.PivotSample>();

  public List<TimelineControl.PivotSample> SrcPivotList {
    get { return m_SrcPivotList; }
    set {
      m_SrcPivotList = value;
      m_SrcPivotVectors = null;
    }
  }
  ...
  public List<TimelineControl.PivotSample> DstPivotList {
    ...
    set {
      m_DstPivotList = value;
      m_DstPivotVectors = null;
    }
  }
  ...
  private Vector3 CalculatePoint(float t, Vector3 p0, Vector3 p1, Vector3 p2,
                                 Vector3 p3) {
    float u = 1 - t;
    float tt = t * t;
    float uu = u * u;
  ...
  private Color[] GetPivotColors(Vector3[] vectors, float motionStart,
                                 float motionStop, Color fromColor,
                                 Color toColor, Color loopColor, float offset) {
    Color[] colors = new Color[vectors.Length];

    float transStart = m_TimeArea.TimeToPixel(m_TransitionStartTime, m_Rect) +
                       m_LeftThumbOffset;
    float transStop = m_TimeArea.TimeToPixel(m_TransitionStopTime, m_Rect) +
                      m_RightThumbOffset;
    float transWidth = transStop - transStart;

    for (int i = 0; i < colors.Length; i++) {
      if (vectors[i].x >= transStart && vectors[i].x <= transStop)
        colors[i] = Color.Lerp(fromColor, toColor,
                               (vectors[i].x - transStart) / transWidth);
      else if (vectors[i].x < transStart &&
               vectors[i].x >= motionStart + offset)
        colors[i] = fromColor;
  ...
  private Vector3[] GetPivotVectors(PivotSample[] samples, float width,
                                    Rect rect, float height, bool loop) {
    if (samples.Length == 0 ||
        width < 0.33f) // not pivots for super small anims
  ...
  private void DoPivotCurves() {
    ...
    if (m_SrcPivotVectors == null)
      m_SrcPivotVectors =
          GetPivotVectors(m_SrcPivotList.ToArray(), srcStop - srcStart, rect,
                          rect.height, srcLoop);
    if (m_DstPivotVectors == null)
      m_DstPivotVectors =
          GetPivotVectors(m_DstPivotList.ToArray(), dstStop - dstStart, rect,
                          rect.height, dstLoop);

    m_DstPivotVectors = OffsetPivotVectors(
        m_DstPivotVectors, m_DstDragOffset + dstStart - srcStart);

    Color[] srcColors = GetPivotColors(m_SrcPivotVectors, srcStart, srcStop,
                                       srcFull, srcNone, srcLoopColor, 0f);
    Color[] dstColors =
        GetPivotColors(m_DstPivotVectors, dstStart, dstStop, dstNone, dstFull,
                       dstLoopColor, m_DstDragOffset);
  ...
  private void EnforceConstraints() {
    ...
    if (m_DragState == DragStates.LeftSelection) {
      float minLimit = m_TimeArea.TimeToPixel(SrcStartTime, r) -
                       m_TimeArea.TimeToPixel(TransitionStartTime, r);
      float maxLimit = m_TimeArea.TimeToPixel(TransitionStopTime, r) -
                       m_TimeArea.TimeToPixel(TransitionStartTime, r);
    ...
    if (m_DragState == DragStates.RightSelection) {
      float minLimit = m_TimeArea.TimeToPixel(TransitionStartTime, r) -
                       m_TimeArea.TimeToPixel(TransitionStopTime, r);
  ...
  private bool WasDraggingData() {
    return m_DstDragOffset != 0 || m_LeftThumbOffset != 0 ||
  ...
  public bool DoTimeline(Rect timeRect) {
    ...
    GUI.BeginGroup(timeRect);
    {
      ...
      float dstStart =
          m_TimeArea.TimeToPixel(DstStartTime, r) + m_DstDragOffset;
      float dstStop = m_TimeArea.TimeToPixel(DstStopTime, r) + m_DstDragOffset;
      float transStart =
          m_TimeArea.TimeToPixel(TransitionStartTime, r) + m_LeftThumbOffset;
      float transStop =
          m_TimeArea.TimeToPixel(TransitionStopTime, r) + m_RightThumbOffset;
      ...
      Rect transHeaderRect =
          new Rect(transStart, 0, transStop - transStart, 18);
      Rect transRect =
          new Rect(transStart, 18, transStop - transStart, r.height - 18);
      ...
      if (evt.type == EventType.MouseDrag) {
        if (EditorGUIUtility.hotControl == id) {
          ...
          case DragStates.RightSelection:
            // clamp the delta when off range
            if (evt.delta.x > 0 && evt.mousePosition.x > transStart ||
                evt.delta.x < 0)
          ...
          case DragStates.Playhead:
            if ((evt.delta.x > 0 && evt.mousePosition.x > srcStart) ||
                (evt.delta.x < 0 &&
                 evt.mousePosition.x <=
                     m_TimeArea.TimeToPixel(SampleStopTime, r)))
      ...
      // handle mouse up both when it happens over the control area and outside
      // control area (case 834214)
      ...
      // draw src Loop
      int srcLoopCount = srcLoop ?(1 + (int)((transStop - srcRect.xMin) /
                                            (srcRect.xMax - srcRect.xMin))) : 1;
      Rect loopRect = srcRect;
      if (srcRect.width < 10) // if smaller than 10 pixel, group
      {
        loopRect = new Rect(srcRect.x, srcRect.y,
                            (srcRect.xMax - srcRect.xMin) * srcLoopCount,
                            srcRect.height);
        srcLoopCount = 1;
      }
      ...
      for (int loopSrcIt = 0; loopSrcIt < srcLoopCount; loopSrcIt++) {
        ...
        float widthAfter =
            (loopRect.xMax - loopRect.xMin) - (widthBefore + widthDuring);
        if (widthBefore > 0)
          GUI.Box(new Rect(0, 0, widthBefore, srcRect.height), GUIContent.none,
                  styles.onLeft);
        if (widthDuring > 0)
          GUI.Box(new Rect(widthBefore, 0, widthDuring, srcRect.height),
                  GUIContent.none, styles.onOff);
        if (widthAfter > 0)
          GUI.Box(new Rect(widthBefore + widthDuring, 0, widthAfter,
                           srcRect.height),
                  GUIContent.none, styles.offRight);
        ...
        if (srcPercentLeft < srcLabelRight && srcPercentRight > 0f &&
            m_DragState == DragStates.LeftSelection)
          srcAlphaTarget = 0f;
        GUI.EndGroup();

        float srcAlpha = styles.leftBlock.normal.textColor.a;
        if (!Mathf.Approximately(srcAlpha, srcAlphaTarget) &&
            Event.current.type == EventType.Repaint) {
          srcAlpha = Mathf.Lerp(srcAlpha, srcAlphaTarget, 0.1f);
          styles.leftBlock.normal.textColor =
              new Color(styles.leftBlock.normal.textColor.r,
                        styles.leftBlock.normal.textColor.g,
                        styles.leftBlock.normal.textColor.b, srcAlpha);
          HandleUtility.Repaint();
        }
        GUI.Box(loopRect, srcContent, styles.leftBlock);
        loopRect =
            new Rect(loopRect.xMax, 85, loopRect.xMax - loopRect.xMin, 32);
      }

      GUIContent dstContent = EditorGUIUtility.TempContent(DstName);
      int dstLoopCount = dstLoop  ?(1 + (int)((transStop - dstRect.xMin) /
                                            (dstRect.xMax - dstRect.xMin))) : 1;
      loopRect = dstRect;
      if (dstRect.width < 10) // if smaller than 10 pixel, group
      {
        loopRect = new Rect(dstRect.x, dstRect.y,
                            (dstRect.xMax - dstRect.xMin) * dstLoopCount,
                            dstRect.height);
        dstLoopCount = 1;
      }
      ...
      for (int loopDstIt = 0; loopDstIt < dstLoopCount; loopDstIt++) {
        ...
        float widthAfter =
            (loopRect.xMax - loopRect.xMin) - (widthBefore + widthDuring);
        if (widthBefore > 0)
          GUI.Box(new Rect(0, 0, widthBefore, dstRect.height), GUIContent.none,
                  styles.offLeft);
        if (widthDuring > 0)
          GUI.Box(new Rect(widthBefore, 0, widthDuring, dstRect.height),
                  GUIContent.none, styles.offOn);
        if (widthAfter > 0)
          GUI.Box(new Rect(widthBefore + widthDuring, 0, widthAfter,
                           dstRect.height),
                  GUIContent.none, styles.onRight);
        ...
        if (dstPercentLeft < dstLabelRight && dstPercentRight > 0f &&
            (m_DragState == DragStates.LeftSelection ||
             m_DragState == DragStates.Destination))
          dstAlphaTarget = 0f;
        GUI.EndGroup();
        float dstAlpha = styles.rightBlock.normal.textColor.a;
        if (!Mathf.Approximately(dstAlpha, dstAlphaTarget) &&
            Event.current.type == EventType.Repaint) {
          dstAlpha = Mathf.Lerp(dstAlpha, dstAlphaTarget, 0.1f);
          styles.rightBlock.normal.textColor =
              new Color(styles.rightBlock.normal.textColor.r,
                        styles.rightBlock.normal.textColor.g,
                        styles.rightBlock.normal.textColor.b, dstAlpha);
          HandleUtility.Repaint();
        }
        GUI.Box(loopRect, dstContent, styles.rightBlock);

        loopRect = new Rect(loopRect.xMax, loopRect.yMin,
                            loopRect.xMax - loopRect.xMin, 32);
      }
      ...
      // draw the thumbs
      GUI.Box(leftThumbRect, GUIContent.none,
              (m_HasExitTime) ? styles.handLeft : styles.handLeftPrev);
      ...
      Handles.DrawLine(new Vector3(playPoint, 19, 0),
                       new Vector3(playPoint, r.height, 0));
      ...
      if (m_DragState == DragStates.Destination && !oneFrameDst) {
        Rect transLabelRect =
            new Rect(transStart - 50, dstRect.y, 45, dstRect.height);
        string transLabel = UnityString.Format(
            "{0:0%}", (transStart - dstStart) / (dstStop - dstStart));
        GUI.Box(transLabelRect, EditorGUIUtility.TempContent(transLabel),
                styles.timeBlockRight);
      }

      // show normalized time label on left side when moving left edge of
      // transition
      if (m_DragState == DragStates.LeftSelection) {
        if (!oneFrameSrc) {
          Rect srcLabelRect =
              new Rect(transStart - 50, srcRect.y, 45, srcRect.height);
          string srcLabel = UnityString.Format(
              "{0:0%}", (transStart - srcStart) / (srcStop - srcStart));
          GUI.Box(srcLabelRect, EditorGUIUtility.TempContent(srcLabel),
                  styles.timeBlockRight);
        }
        ...
        if (!oneFrameDst) {
          Rect dstLabelRect =
              new Rect(transStart - 50, dstRect.y, 45, dstRect.height);
          string dstLabel = UnityString.Format(
              "{0:0%}", (transStart - dstStart) / (dstStop - dstStart));
          GUI.Box(dstLabelRect, EditorGUIUtility.TempContent(dstLabel),
                  styles.timeBlockRight);
        }
      }

      // show normalized time label on right side when moving right edge of
      // transition
      if (m_DragState == DragStates.RightSelection) {
        if (!oneFrameSrc) {
          Rect srcLabelRect =
              new Rect(transStop + 5, srcRect.y, 45, srcRect.height);
          string srcLabel = UnityString.Format(
              "{0:0%}", (transStop - srcStart) / (srcStop - srcStart));
          GUI.Box(srcLabelRect, EditorGUIUtility.TempContent(srcLabel),
                  styles.timeBlockLeft);
        }

        if (!oneFrameDst) {
          Rect dstLabelRect =
              new Rect(transStop + 5, dstRect.y, 45, dstRect.height);
          string dstLabel = UnityString.Format(
              "{0:0%}", (transStop - dstStart) / (dstStop - dstStart));
          GUI.Box(dstLabelRect, EditorGUIUtility.TempContent(dstLabel),
                  styles.timeBlockLeft);
        }
      }
