...
public void HandleDraggingToEditor(Editor[] editors, int editorIndex,
                                   Rect dragRect, Rect contentRect) {
...
public void HandleDraggingInBottomArea(Editor[] editors, Rect bottomRect,
                                       Rect contentRect) {
	...
	if (m_LastIndex >= 0 && m_LastIndex < editors.Length) {
		...
		HandleEditorDragging(editors, m_LastIndex, bottomRect, contentRect.yMax,
		                     true);
	} else {
...
internal void HandleDragPerformInBottomArea(Editor[] editors, Rect bottomRect,
                                            Rect targetRect) {
	HandleNativeDragDropInBottomArea(editors, bottomRect);

	if (m_LastIndex >= 0 && m_LastIndex < editors.Length) {
		HandleEditorDragging(editors, m_LastIndex, GetTargetRect(targetRect),
		                     targetRect.yMax, true);
	}
...
void HandleNativeDragDropInBottomArea(Editor[] editors, Rect rect) {
	...
	DragAndDrop.visualMode = DragAndDropService.Drop(
		DragAndDropService.kInspectorDropDstId, editor.targets,
...
void HandleEditorDragging(Editor[] editors, int editorIndex, Rect targetRect,
                          float markerY, bool bottomTarget) {
	...
	case EventType.DragUpdated:
		if (targetRect.Contains(evt.mousePosition)) {
			var draggingMode =
				DragAndDrop.GetGenericData(k_DraggingModeKey) as DraggingMode ?;
			...
			if (draggingMode.Value != DraggingMode.NotApplicable) {
				if (bottomTarget) {
					...
					m_TargetAbove =
						evt.mousePosition.y < targetRect.y + targetRect.height / 2f;
					m_TargetIndex = editorIndex;

					if (m_TargetAbove) {
						m_TargetIndex++;
						while (m_TargetIndex < editors.Length &&
						       m_InspectorWindow.ShouldCullEditor(editors, m_TargetIndex))
							m_TargetIndex++;
				...
				if (m_TargetAbove &&
				    InspectorWindow.EditorHasLargeHeader(m_TargetIndex, editors)) {
					m_TargetIndex--;
					while (m_TargetIndex >= 0 &&
					       m_InspectorWindow.ShouldCullEditor(editors, m_TargetIndex))
						m_TargetIndex--;
				...
				if (draggingMode.Value == DraggingMode.Script) {
					...
					// Always allow script dragging, instead fail during DragPerform
					// with a dialog box
					...
					if (editors[m_TargetIndex].targets.All(t => t is Component)) {
						var targetComponents =
							editors[m_TargetIndex].targets.Cast<Component>().ToArray();
						var sourceComponents =
							DragAndDrop.objectReferences.Cast<Component>().ToArray();
						valid = MoveOrCopyComponents(
							sourceComponents, targetComponents,
							EditorUtility.EventHasDragCopyModifierPressed(evt), true);
					}

					if (valid)
						DragAndDrop.visualMode =
							EditorUtility.EventHasDragCopyModifierPressed(evt)
	...
	case EventType.Repaint:
		if (m_TargetIndex != -1 && editorIndex == m_TargetIndex &&
			...
			Styles.insertionMarker.Draw(
				GetMarkerRect(targetRect, markerY, m_TargetAbove), false, false,
				false, false);
		}
		break;
	}
...
void HandleDragPerformEvent(Editor[] editors, Event evt,
                            ref int targetIndex) {
	if (targetIndex != -1) {
		var draggingMode =
			DragAndDrop.GetGenericData(k_DraggingModeKey) as DraggingMode ?;
		if (!draggingMode.HasValue ||
		    draggingMode.Value == DraggingMode.NotApplicable) {
			targetIndex = -1;
			return;
		}
		...
		var targetComponents =
			editors[targetIndex].targets.Cast<Component>().ToArray();
		...
		if (draggingMode.Value == DraggingMode.Script) {
			...
			foreach (var targetComponent in targetComponents) {
				...
				if (scripts.Any(s => !ComponentUtility.WarnCanAddScriptComponent(
							gameObject, s))) {
					valid = false;
					break;
				}
			...
			if (valid) {
				...
				var index = 0;
				var addedComponents =
					new Component[targetComponents.Length * scripts.Count()];
				for (int i = 0; i < targetComponents.Length; i++) {
					var targetComponent = targetComponents[i];
					var gameObject = targetComponent.gameObject;
					bool targetIsTransform = targetComponent is Transform;
					foreach (var script in scripts)
						addedComponents[index++] =
							ObjectFactory.AddComponent(gameObject, script.GetClass());

					// If the target is a Transform, the AddComponent might have
					// replaced it with a RectTransform. Handle this possibility by
					// updating the target component.
					if (targetIsTransform)
						targetComponents[i] = gameObject.transform;
				}

				// Move added components relative to target components
				if (!ComponentUtility.MoveComponentsRelativeToComponents(
					    addedComponents, targetComponents, m_TargetAbove)) {
					// Ensure we have the same selection after calling
					// RevertAllDownToGroup below (MoveComponentsRelativeToComponents
					// can have opened a Prefab in Prefab Mode and changed selection to
					// that root)
					var wantedSelectedGameObject = Selection.activeGameObject;

					// Revert added components if move operation fails (e.g. user has
					// been shown the dialog with 'prefab instance restructuring is not
					// posssible' or object is not editable)
			...
			MoveOrCopyComponents(sourceComponents, targetComponents,
			                     EditorUtility.EventHasDragCopyModifierPressed(evt),
			                     false);
		}
...
private static Rect GetTargetRect(Rect contentRect) {
	...
	return new Rect(contentRect.x,
	                yPos + uiDragTargetHeight / (targetHeight * .25f),
	                contentRect.width, uiDragTargetHeight);
}

private static Rect GetMarkerRect(Rect targetRect, float markerY,
                                  bool targetAbove) {
...
bool MoveOrCopyComponents(Component[] sourceComponents,
                          Component[] targetComponents, bool copy,
                          bool validateOnly) {
	...
	if (sourceComponents.Length == 1 && targetComponents.Length == 1) {
		...
		return ComponentUtility.MoveComponentRelativeToComponent(
			sourceComponents[0], targetComponents[0], m_TargetAbove,
			validateOnly);
	} else
		return ComponentUtility.MoveComponentsRelativeToComponents(
			sourceComponents, targetComponents, m_TargetAbove, validateOnly);
}
}
}
