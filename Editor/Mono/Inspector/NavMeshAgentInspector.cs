...
public readonly GUIContent m_AgentSteeringHeader =
	EditorGUIUtility.TrTextContent("Steering");
public readonly GUIContent m_AgentAvoidanceHeader =
	EditorGUIUtility.TrTextContent("Obstacle Avoidance");
public readonly GUIContent m_AgentPathFindingHeader =
	EditorGUIUtility.TrTextContent("Path Finding");
}
...
void OnEnable() {
	...
	m_AutoTraverseOffMeshLink =
		serializedObject.FindProperty("m_AutoTraverseOffMeshLink");
	...
	m_ObstacleAvoidanceType =
		serializedObject.FindProperty("m_ObstacleAvoidanceType");
...
public override void OnInspectorGUI() {
	...
	EditorGUILayout.LabelField(s_Styles.m_AgentSteeringHeader,
	...
	EditorGUILayout.Space();
	EditorGUILayout.LabelField(s_Styles.m_AgentAvoidanceHeader,
	                           EditorStyles.boldLabel);
	EditorGUILayout.PropertyField(m_Radius);
	EditorGUILayout.PropertyField(m_Height);
	EditorGUILayout.PropertyField(m_ObstacleAvoidanceType,
	                              GUIContent.Temp("Quality"));
	EditorGUILayout.PropertyField(m_AvoidancePriority,
	                              GUIContent.Temp("Priority"));

	EditorGUILayout.Space();
	EditorGUILayout.LabelField(s_Styles.m_AgentPathFindingHeader,
	                           EditorStyles.boldLabel);
	...
	var position = GUILayoutUtility.GetRect(EditorGUILayout.kLabelFloatMinW,
	                                        EditorGUILayout.kLabelFloatMaxW, kH,
	                                        kH, EditorStyles.layerMaskField);

	EditorGUI.BeginChangeCheck();
	EditorGUI.showMixedValue = m_WalkableMask.hasMultipleDifferentValues;
	var areaMask = EditorGUI.MaskField(position, "Area Mask", compressedMask,
	                                   areaNames, EditorStyles.layerMaskField);
	...
	if (EditorGUI.EndChangeCheck()) {
		if (areaMask == ~0) {
			...
			for (var i = 0; i < areaNames.Length; i++) {
				...
				if (((areaMask >> i) & 1) != 0) {
					// Find out the 'real' layer from the name, then set it in the new
					// mask
					newMask =
						newMask | (uint)(1 << GameObjectUtility.GetNavMeshAreaFromName(
									 areaNames[i]));
				}
			}
...
private static void AgentTypePopupInternal(string labelName,
