...
static private readonly GUIContent kAssetBundleName =
	EditorGUIUtility.TrTextContent("AssetBundle");
static private readonly int kAssetBundleNameFieldIdHash =
	"AssetBundleNameFieldHash".GetHashCode();

static private readonly int kAssetBundleVariantFieldIdHash =
	"AssetBundleVariantFieldHash".GetHashCode();
...
public void OnAssetBundleNameGUI(IEnumerable<Object> assets) {
	...
	int id = GUIUtility.GetControlID(kAssetBundleNameFieldIdHash,
	                                 FocusType.Passive, bundleRect);

	bundleRect =
		EditorGUI.PrefixLabel(bundleRect, id, kAssetBundleName, Styles.label);
	...
	id = GUIUtility.GetControlID(kAssetBundleVariantFieldIdHash,
	                             FocusType.Passive, variantRect);
...
private void AssetBundleTextField(Rect rect, int id,
                                  IEnumerable<Object> assets,
                                  bool isVariant) {
	EditorGUI.BeginChangeCheck();
	string temp = EditorGUI.DelayedTextFieldInternal(
		rect, id, GUIContent.none, "", null, Styles.textField);
	...
	if (EditorGUI.IsEditingTextField() == false &&
...
private void AssetBundlePopup(Rect rect, int id, IEnumerable<Object> assets,
                              bool isVariant) {
	...
	// assetBundle name by script, the UI needs to detect this kind of change.
	bool mixedValue;
	IEnumerable<string> assetBundleFromAssets =
		GetAssetBundlesFromAssets(assets, isVariant, out mixedValue);

	string[] assetBundles =
		isVariant ? AssetDatabase.GetAllAssetBundleVariants()
	...
	selectedIndex = EditorGUI.DoPopup(
		rect, id, selectedIndex,
		EditorGUIUtility.TempContent(displayedOptions.ToArray()), Styles.popup);
	...
	if (EditorGUI.EndChangeCheck()) {
		if (selectedIndex == 0) // None
			...
			SetAssetBundleForAssets(assets, displayedOptions[selectedIndex],
			                        isVariant);
	}
}
...
private void FilterSelected(IEnumerable<string> assetBundleNames) {
	...
	searchFilter.assetBundleNames =
		assetBundleNames.Where(name => !String.IsNullOrEmpty(name)).ToArray();
...
GetAssetBundlesFromAssets(IEnumerable<Object> assets, bool isVariant,
                          out bool isMixed) {
	...
	foreach (Object obj in assets) {
		...
		AssetImporter importer =
			AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(obj));
		if (importer == null)
			continue;

		string currentAssetBundle =
			isVariant ? importer.assetBundleVariant : importer.assetBundleName;
...
private void SetAssetBundleForAssets(IEnumerable<Object> assets, string name,
                                     bool isVariant) {
