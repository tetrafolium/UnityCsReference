...
public static readonly GUIContent s_ColliderEditDisableHelp =
	EditorGUIUtility.TrTextContent(
		"Collider cannot be edited because it is driven by SpriteRenderer's tiling properties.");
public static readonly GUIContent s_AutoTilingLabel =
	EditorGUIUtility.TrTextContent(
		"Auto Tiling ",
		" When enabled, the collider's shape will update automatically based on the SpriteRenderer's tiling properties");
}
...
public override void OnInspectorGUI() {
	...
	// Only show 'Used By Composite' if all targets are capable of being
	// composited.
...
public void FinalizeInspectorGUI() {
	...
	// If used-by-composite is enabled but there is not composite then show a
	// warning.
	if (targets.Length == 1) {
		var collider = target as Collider2D;
		if (collider.isActiveAndEnabled && collider.composite == null &&
		    m_UsedByComposite.boolValue)
			EditorGUILayout.HelpBox(
				"This collider will not function with a composite until there is a CompositeCollider2D on the GameObject that the attached Rigidbody2D is on.",
				MessageType.Warning);
	}
...
private void ShowColliderInfoProperties() {
	...
	if (EditorGUILayout.BeginFadeGroup(m_ShowInfo.faded)) {
		if (targets.Length == 1) {
			...
			EditorGUILayout.ObjectField("Attached Body", collider.attachedRigidbody,
			...
			EditorGUILayout.HelpBox(
				"Cannot show Info properties when multiple colliders are selected.",
				MessageType.Info);
		}
	}
	EditorGUILayout.EndFadeGroup();
...
bool ShouldShowDensity() {
	if (targets.Select(x =>(x as Collider2D).attachedRigidbody)
	    .Distinct()
	    .Count() > 1)
		return false;

	var rigidbody = (target as Collider2D).attachedRigidbody;
	return rigidbody && rigidbody.useAutoMass &&
		...
		if (contactCount > 0) {
			...
			for (var i = 0; i < contactCount; ++i) {
				...
				EditorGUILayout.ObjectField("Rigidbody", contact.rigidbody,
				                            typeof(Rigidbody2D), false);
				EditorGUILayout.ObjectField("OtherRigidbody", contact.otherRigidbody,
				                            typeof(Rigidbody2D), false);
				EditorGUI.indentLevel--;
...
internal override void OnForceReloadInspector() {
	...
	// Whenever inspector get reloaded (reset, move up/down), quit the edit mode
	// if was in editing mode. Not sure why this pattern is used here but not
	// for any other editors that implement edit mode button
	if (editingCollider)
...
protected void CheckColliderErrorState() {
	...
	case ColliderErrorState2D.RemovedShapes:
		// Show warning.
		EditorGUILayout.HelpBox(
			"The collider created collision shape(s) but some were removed as they failed verification.  This could be because they were deemed too small or the vertices were too close.  Vertices can also become close under certain rotations or very small scaling.",
			MessageType.Warning);
		break;
	}
}
...
protected void BeginColliderInspector() {
	...
	using (new EditorGUI.DisabledScope(targets.Length > 1)) {
		EditorGUILayout.EditorToolbarForTarget(
			EditorGUIUtility.TrTempContent("Edit Collider"), target);
	}
}
...
protected bool CanEditCollider() {
	var e = targets.FirstOrDefault((x) => {
				...
				return (sr != null && sr.drawMode != SpriteDrawMode.Simple &&
				        m_AutoTiling.boolValue == true);
			});
	return e == false;
}
}
