...
internal class AvatarPreview {
  ...
  private class ObjectSelectorOperation : ObjectSelectorReceiver {
    public static void Start(AvatarPreview owner) {
      var operation =
          ScriptableObject.CreateInstance<ObjectSelectorOperation>();
    ...
    bool IsValid() { return m_Owner != null; }
  ...
  public Animator Animator {
    get {
      return m_PreviewInstance != null ? m_PreviewInstance.GetComponent(
                                      typeof(Animator)) as Animator : null;
    }
  }
  ...
  public Vector3 bodyPosition {
    get {
      ...
      if (m_PreviewInstance != null)
        return GameObjectInspector.GetRenderableCenterRecurse(m_PreviewInstance,
                                                              1, 8);
  ...
  public Vector3 rootPosition {
    get {
      return m_PreviewInstance ? m_PreviewInstance.transform.position
          : Vector3.zero;
    }
  }
  ...
  // 60 is the default framerate for animations created inside Unity so as good
  // a default as any.
  public int fps = 60;
  ...
  private class Styles {
    public GUIContent pivot = EditorGUIUtility.TrIconContent(
        "AvatarPivot", "Displays avatar's pivot and mass center");
    public GUIContent ik =
        EditorGUIUtility.TrTextContent("IK", "Toggles feet IK preview");
    public GUIContent is2D = EditorGUIUtility.TrIconContent(
        "SceneView2D", "Toggles 2D preview mode");
    public GUIContent avatarIcon = EditorGUIUtility.TrIconContent(
        "AvatarSelector", "Changes the model to use for previewing.");
  ...
  void SetPreviewCharacterEnabled(bool enabled, bool showReference) {
    ...
    PreviewRenderUtility.SetEnabledRecursive(m_ReferenceInstance,
                                             showReference && enabled);
    PreviewRenderUtility.SetEnabledRecursive(m_DirectionInstance,
                                             showReference && enabled);
    PreviewRenderUtility.SetEnabledRecursive(m_PivotInstance,
                                             showReference && enabled);
    PreviewRenderUtility.SetEnabledRecursive(m_RootInstance,
                                             showReference && enabled);
  }
  ...
  static public bool
  IsValidPreviewGameObject(GameObject target,
                           ModelImporterAnimationType requiredClipType) {
  ...
  static public GameObject FindBestFittingRenderableGameObjectFromModelAsset(
      Object asset, ModelImporterAnimationType animationType) {
    if (asset == null)
      return null;

    ModelImporter importer = AssetImporter.GetAtPath(
        AssetDatabase.GetAssetPath(asset)) as ModelImporter;
    if (importer == null)
      return null;
    ...
    // We should also check for isHumanClip matching the animationclip
    // requiremenets...
  ...
  static GameObject
  CalculatePreviewGameObject(Animator selectedAnimator, Motion motion,
    ...
    if (selectedAnimator != null &&
        IsValidPreviewGameObject(selectedAnimator.gameObject, animationType))
      return selectedAnimator.gameObject;

    // Find the best fitting preview game object for the asset we are viewing
    // (Handles @ convention, will pick base path for you)
    selected = FindBestFittingRenderableGameObjectFromModelAsset(sourceClip,
                                                                 animationType);
    if (selected != null)
      return selected;
  ...
  public void ResetPreviewInstance() {
    ...
    GameObject go = CalculatePreviewGameObject(
        m_SourceScenePreviewAnimator, m_SourcePreviewMotion, animationClipType);
    SetupBounds(go);
  }
  ...
  void SetupBounds(GameObject go) {
    ...
    if (go != null) {
      ...
      Bounds bounds =
          new Bounds(m_PreviewInstance.transform.position, Vector3.zero);
      GameObjectInspector.GetRenderableBoundsRecurse(ref bounds,
                                                     m_PreviewInstance);
  ...
  void InitInstance(Animator scenePreviewObject, Motion motion) {
    ...
    if (m_PreviewInstance == null) {
      GameObject go = CalculatePreviewGameObject(scenePreviewObject, motion,
                                                 animationClipType);
      SetupBounds(go);
    }
    ...
    if (m_ReferenceInstance == null) {
      GameObject referenceGO =
          (GameObject) EditorGUIUtility.Load("Avatar/dial_flat.prefab");
      m_ReferenceInstance = (GameObject) Object.Instantiate(
          referenceGO, Vector3.zero, Quaternion.identity);
    ...
    if (m_DirectionInstance == null) {
      GameObject directionGO =
          (GameObject) EditorGUIUtility.Load("Avatar/arrow.fbx");
      m_DirectionInstance = (GameObject) Object.Instantiate(
          directionGO, Vector3.zero, Quaternion.identity);
    ...
    if (m_PivotInstance == null) {
      GameObject pivotGO =
          (GameObject) EditorGUIUtility.Load("Avatar/root.fbx");
      m_PivotInstance = (GameObject) Object.Instantiate(pivotGO, Vector3.zero,
                                                        Quaternion.identity);
    ...
    if (m_RootInstance == null) {
      ...
      m_RootInstance = (GameObject) Object.Instantiate(rootGO, Vector3.zero,
                                                       Quaternion.identity);
    ...
    is2D = EditorPrefs.GetBool(k2DPref, EditorSettings.defaultBehaviorMode ==
  ...
  private void Init() {
    ...
    if (m_FloorPlane == null) {
      m_FloorPlane =
          Resources.GetBuiltinResource(typeof(Mesh), "New-Plane.fbx") as Mesh;
    }

    if (m_FloorTexture == null) {
      m_FloorTexture =
          (Texture2D) EditorGUIUtility.Load("Avatar/Textures/AvatarFloor.png");
    }

    if (m_FloorMaterial == null) {
      Shader shader = EditorGUIUtility.LoadRequired(
          "Previews/PreviewPlaneWithShadow.shader") as Shader;
      m_FloorMaterial = new Material(shader);
      m_FloorMaterial.mainTexture = m_FloorTexture;
      m_FloorMaterial.mainTextureScale =
          Vector2.one * kFloorScale * kFloorTextureScale;
      m_FloorMaterial.SetVector(
          "_Alphas", new Vector4(kFloorAlpha, kFloorShadowAlpha, 0, 0));
      m_FloorMaterial.hideFlags = HideFlags.HideAndDontSave;

      m_FloorMaterialSmall = new Material(m_FloorMaterial);
      m_FloorMaterialSmall.mainTextureScale =
          Vector2.one * kFloorScaleSmall * kFloorTextureScale;
      m_FloorMaterialSmall.hideFlags = HideFlags.HideAndDontSave;
    }

    if (m_ShadowMaskMaterial == null) {
      Shader shader = EditorGUIUtility.LoadRequired(
          "Previews/PreviewShadowMask.shader") as Shader;
    ...
    if (m_ShadowPlaneMaterial == null) {
      Shader shader = EditorGUIUtility.LoadRequired(
          "Previews/PreviewShadowPlaneClip.shader") as Shader;
  ...
  public void DoSelectionChange() { m_OnAvatarChangeFunc(); }

  public AvatarPreview(Animator previewObjectInScene,
                       Motion objectOnSameAsset) {
  ...
  float PreviewSlider(Rect rect, float val, float snapThreshold) {
    val = GUI.HorizontalSlider(
        rect, val, 0.1f, 2.0f, s_Styles.preSlider,
  ...
  public void DoPreviewSettings() {
    ...
    if (m_ShowIKOnFeetButton) {
      ...
      m_IKOnFeet =
          GUILayout.Toggle(m_IKOnFeet, s_Styles.ik, s_Styles.preButton);
    ...
    EditorGUI.BeginChangeCheck();
    m_ShowReference =
        GUILayout.Toggle(m_ShowReference, s_Styles.pivot, s_Styles.preButton);
    if (EditorGUI.EndChangeCheck())
      EditorPrefs.SetBool(kReferencePref, m_ShowReference);

    if (EditorGUILayout.DropdownButton(s_Styles.avatarIcon, FocusType.Passive,
                                       EditorStyles.toolbarDropDownRight)) {
      GenericMenu menu = new GenericMenu();
      menu.AddItem(EditorGUIUtility.TrTextContent("Auto"), false,
                   SetPreviewAvatarOption, PreviewPopupOptions.Auto);
      menu.AddItem(EditorGUIUtility.TrTextContent("Unity Model"), false,
                   SetPreviewAvatarOption, PreviewPopupOptions.DefaultModel);
      menu.AddItem(EditorGUIUtility.TrTextContent("Other..."), false,
                   SetPreviewAvatarOption, PreviewPopupOptions.Other);
      menu.ShowAsContext();
    }
  }

  private RenderTexture RenderPreviewShadowmap(Light light, float scale,
                                               Vector3 center, Vector3 floorPos,
    ...
    cam.transform.rotation =
        is2D ? Quaternion.identity : light.transform.rotation;
    ...
    Matrix4x4 texMatrix =
        Matrix4x4.TRS(new Vector3(0.5f, 0.5f, 0.5f), Quaternion.identity,
                      new Vector3(0.5f, 0.5f, 0.5f));
    outShadowMatrix =
        texMatrix * cam.projectionMatrix * cam.worldToCameraMatrix;
  ...
  public void DoRenderPreview(Rect previewRect, GUIStyle background) {
    ...
    PositionPreviewObjects(pivotRot, pivotPos, bodyRot, bodyPosition,
                           directionRot, rootRot, rootPos, directionPos,
                           m_AvatarScale);

    bool dynamicFloorHeight =
        is2D ? false : Mathf.Abs(m_NextFloorHeight - m_PrevFloorHeight) >
        m_ZoomFactor * 0.01f;

    // Calculate floor height and alpha
    float mainFloorHeight, mainFloorAlpha;
    if (dynamicFloorHeight) {
      float fadeMoment = m_NextFloorHeight <
                         m_PrevFloorHeight ? kFloorFadeDuration
          : (1 - kFloorFadeDuration);
      mainFloorHeight = timeControl.normalizedTime <
                        fadeMoment ? m_PrevFloorHeight : m_NextFloorHeight;
      mainFloorAlpha =
          Mathf.Clamp01(Mathf.Abs(timeControl.normalizedTime - fadeMoment) /
                        kFloorFadeDuration);
    } else {
    ...
    Quaternion floorRot =
        is2D ? Quaternion.Euler(-90, 0, 0) : Quaternion.identity;
    ...
    RenderTexture shadowMap = RenderPreviewShadowmap(
        previewUtility.lights[0], m_BoundingVolumeScale / 2, bodyPosition,
        floorPos, out shadowMatrix);
    ...
    // Add panning offset
    Vector3 camPos = camRot * (Vector3.forward * -5.5f * tempZoomFactor) +
                     bodyPos + m_PivotPositionOffset;
    ...
    Vector2 textureOffset = -new Vector2(floorPos.x, is2D ? floorPos.y
                                         : floorPos.z);
    ...
    // Render main floor
    {
      ...
      Matrix4x4 matrix = Matrix4x4.TRS(
          floorPos, floorRot, Vector3.one * kFloorScale * m_AvatarScale);

      mat.mainTextureOffset =
          textureOffset * kFloorScale * 0.08f * (1.0f / m_AvatarScale);
      mat.SetTexture("_ShadowTexture", shadowMap);
      mat.SetMatrix("_ShadowTextureMatrix", shadowMatrix);
      mat.SetVector("_Alphas",
                    new Vector4(kFloorAlpha * mainFloorAlpha,
                                kFloorShadowAlpha * mainFloorAlpha, 0, 0));
      mat.renderQueue = (int) UnityEngine.Rendering.RenderQueue.Background;

      Graphics.DrawMesh(m_FloorPlane, matrix, mat, Camera.PreviewCullingLayer,
    ...
    if (dynamicFloorHeight) {
      ...
      float floorAlpha =
          (floorHeight == mainFloorHeight ? 1 - mainFloorAlpha : 1) *
      ...
      Matrix4x4 matrix = Matrix4x4.TRS(
          floorPos, floorRot, Vector3.one * kFloorScaleSmall * m_AvatarScale);
      Graphics.DrawMesh(m_FloorPlane, matrix, mat, Camera.PreviewCullingLayer,
                        previewUtility.camera, 0);
    }
  ...
  private void PositionPreviewObjects(Quaternion pivotRot, Vector3 pivotPos,
                                      Quaternion bodyRot, Vector3 bodyPos,
                                      Quaternion directionRot,
                                      Quaternion rootRot, Vector3 rootPos,
                                      Vector3 directionPos, float scale) {
    ...
    if (Animator) {
      ...
      float normalizedDelta = timeControl.deltaTime /
                              (timeControl.stopTime - timeControl.startTime);

      // Always set last height to next height after wrapping the time.
      if (normalizedTime - normalizedDelta < 0 ||
          normalizedTime - normalizedDelta >= 1)
        m_PrevFloorHeight = m_NextFloorHeight;

      // Check that AvatarPreview is getting reliable info about time and
      // deltaTime.
      if (m_LastNormalizedTime != -1000 &&
          timeControl.startTime == m_LastStartTime &&
          timeControl.stopTime == m_LastStopTime) {
        float difference =
            normalizedTime - normalizedDelta - m_LastNormalizedTime;
        if (difference > 0.5f)
          difference -= 1;
  ...
  public void AvatarTimeControlGUI(Rect rect) {
    ...
    Rect labelRect = new Rect(new Vector2(rect.x, rect.y),
                              EditorStyles.toolbarLabel.CalcSize(
                                  EditorGUIUtility.TrTempContent("xxxxxx")));
    ;
    ...
    EditorGUI.BeginChangeCheck();
    timeControl.playbackSpeed =
        PreviewSlider(sliderControlRect, timeControl.playbackSpeed, 0.03f);
    if (EditorGUI.EndChangeCheck())
    ...
    EditorGUI.DropShadowLabel(
        ...
        UnityString.Format("{0,2}:{1:00} ({2:000.0%}) Frame {3}", (int) time,
                           Repeat(Mathf.FloorToInt(time * fps), fps),
    ...
    get {
      ...
      if (m_ViewTool == ViewTool.None) {
        bool controlKeyOnMac =
            (evt.control && Application.platform == RuntimePlatform.OSXEditor);
        ...
        if ((evt.button <= 0 && noModifiers) ||
            (evt.button <= 0 && actionKey) || evt.button == 2)
          m_ViewTool = ViewTool.Pan;
        else if ((evt.button <= 0 && controlKeyOnMac) ||
                 (evt.button == 1 && evt.alt))
  ...
  protected void HandleMouseDrag(Event evt, int id, Rect previewRect) {
    ...
    if (GUIUtility.hotControl == id) {
      ...
      case ViewTool.Zoom:
        DoAvatarPreviewZoom(evt, -HandleUtility.niceMouseDeltaZoom *
                                     (evt.shift ? 2.0f : 0.5f));
        break;
      default:
  ...
  protected void HandleViewTool(Event evt, EventType eventType, int id,
                                Rect previewRect) {
    switch (eventType) {
  ...
  public void DoAvatarPreviewOrbit(Event evt, Rect previewRect) {
    ...
    m_PreviewDir -= evt.delta * (evt.shift ? 3 : 1) /
                    Mathf.Min(previewRect.width, previewRect.height) * 140.0f;
  ...
  public void DoAvatarPreviewPan(Event evt) {
    ...
    Vector3 screenPos =
        cam.WorldToScreenPoint(bodyPosition + m_PivotPositionOffset);
    Vector3 delta = new Vector3(-evt.delta.x, evt.delta.y, 0);
    // delta panning is scale with the zoom factor to allow fine tuning when
    // user is zooming closely.
    screenPos += delta * Mathf.Lerp(0.25f, 2.0f, m_ZoomFactor * 0.5f);
    Vector3 worldDelta = cam.ScreenToWorldPoint(screenPos) -
                         (bodyPosition + m_PivotPositionOffset);
  ...
  public void DoAvatarPreviewFrame(Event evt, EventType type,
                                   Rect previewRect) {
    ...
    if (type == EventType.KeyDown && Event.current.keyCode == KeyCode.G) {
      m_PivotPositionOffset =
          GetCurrentMouseWorldPosition(evt, previewRect) - bodyPosition;
      evt.Use();
    }
  }
  ...
  protected Vector3 GetCurrentMouseWorldPosition(Event evt, Rect previewRect) {
    ...
    float scaleFactor =
        previewUtility.GetScaleFactor(previewRect.width, previewRect.height);
    Vector3 mouseLocal = new Vector3(
        (evt.mousePosition.x - previewRect.x) * scaleFactor,
        (previewRect.height - (evt.mousePosition.y - previewRect.y)) *
            scaleFactor,
        0);
  ...
  public void DoAvatarPreview(Rect rect, GUIStyle background) {
    ...
    if (EditorGUI.DropdownButton(choserRect, GUIContent.none, FocusType.Passive,
                                 GUIStyle.none)) {
      GenericMenu menu = new GenericMenu();
      menu.AddItem(EditorGUIUtility.TrTextContent("Auto"), false,
                   SetPreviewAvatarOption, PreviewPopupOptions.Auto);
    ...
    int previewID =
        GUIUtility.GetControlID(m_PreviewHint, FocusType.Passive, previewRect);
    Event evt = Event.current;
    ...
    int previewSceneID =
        GUIUtility.GetControlID(m_PreviewSceneHint, FocusType.Passive);
