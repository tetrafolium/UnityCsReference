...
internal class WebCamTextureInspector : Editor {
  ...
  public override void OnInspectorGUI() {
    ...
    EditorGUILayout.LabelField(
        "Requested FPS",
        t.requestedFPS.ToString(CultureInfo.InvariantCulture.NumberFormat));
    EditorGUILayout.LabelField(
        "Requested Width",
        t.requestedWidth.ToString(CultureInfo.InvariantCulture.NumberFormat));
    EditorGUILayout.LabelField(
        "Requested Height",
        t.requestedHeight.ToString(CultureInfo.InvariantCulture.NumberFormat));
  ...
  public override void OnPreviewSettings() {
    ...
    WebCamTexture t = target as WebCamTexture;
    bool isPlaying =
        PreviewGUI.CycleButton(t.isPlaying ? 1 : 0, s_PlayIcons) != 0;
  ...
  public override void OnPreviewGUI(Rect r, GUIStyle background) {
    ...
    float zoomLevel =
        Mathf.Min(Mathf.Min(r.width / t.width, r.height / t.height), 1);
    Rect wantedRect =
        new Rect(r.x, r.y, t.width * zoomLevel, t.height * zoomLevel);
    ...
    if (Application.isPlaying) {
      if (t.isPlaying)
        EditorGUI.DropShadowLabel(new Rect(r.x, r.y + 10, r.width, 20),
                                  "Can't pause preview when in play mode");
      else
        EditorGUI.DropShadowLabel(new Rect(r.x, r.y + 10, r.width, 20),
                                  "Can't start preview when in play mode");
    }
  }
