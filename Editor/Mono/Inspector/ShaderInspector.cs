...
private static readonly string[] kTextureTypes = {
	"No Texture?: ", "Any texture: ", "2D: ",       "3D: ",
	"Cube: ",        "2DArray: ",     "CubeArray: "
};
...
public static Texture2D errorIcon =
	EditorGUIUtility.LoadIcon("console.erroricon.sml");
public static Texture2D warningIcon =
	EditorGUIUtility.LoadIcon("console.warnicon.sml");

public static GUIContent togglePreprocess = EditorGUIUtility.TrTextContent(
	"Preprocess only",
	"Show preprocessor output instead of compiled shader code");
public static GUIContent showSurface = EditorGUIUtility.TrTextContent(
	"Show generated code", "Show generated code of a surface shader");
public static GUIContent showFF = EditorGUIUtility.TrTextContent(
	"Show generated code",
	"Show generated code of a fixed function shader");
public static GUIContent showCurrent = EditorGUIUtility.TrTextContent(
	"Compile and show code \u007C \u25BE"); // vertical bar & dropdow arrow
                                                // - due to lacking editor style
                                                // of "mini button with a
                                                // dropdown"
...
public static GUIContent builtinShader =
	EditorGUIUtility.TrTextContent("Built-in shader");
...
public override void OnInspectorGUI() {
	...
	if (s.isSupported) {
		EditorGUILayout.LabelField(
			"Cast shadows", (ShaderUtil.HasShadowCasterPass(s)) ? "yes" : "no");
		EditorGUILayout.LabelField(
			"Render queue",
			ShaderUtil.GetRenderQueue(s).ToString(CultureInfo.InvariantCulture));
		EditorGUILayout.LabelField(
			"LOD", ShaderUtil.GetLOD(s).ToString(CultureInfo.InvariantCulture));
		EditorGUILayout.LabelField(
			"Ignore projector", ShaderUtil.DoesIgnoreProjector(s) ? "yes" : "no");
		...
		if (RenderPipelineManager.currentPipeline != null) {
			// NOTE: Force the shader compilation to ensure
			// GetSRPBatcherCompatibilityCode will be up to date
			srpCompatibilityCheckMaterial.SetPass(0);
			int subShader = ShaderUtil.GetShaderActiveSubshaderIndex(s);
			int SRPErrCode =
				ShaderUtil.GetSRPBatcherCompatibilityCode(s, subShader);
			string result = (0 == SRPErrCode) ? "compatible" : "not compatible";
			EditorGUILayout.LabelField("SRP Batcher", result);
			if (SRPErrCode != 0) {
				EditorGUILayout.HelpBox(
					ShaderUtil.GetSRPBatcherCompatibilityIssueReason(s, subShader,
					                                                 SRPErrCode),
					MessageType.Info);
			}
		}
...
private void ShowKeywords(Shader s) {
	...
	Rect buttonRect =
		GUILayoutUtility.GetRect(Styles.arrayValuePopupButton, GUI.skin.button,
	...
	if (GUI.Button(buttonRect, Styles.arrayValuePopupButton,
		...
		PopupWindowWithoutFocus.Show(
			buttonRect, new KeywordsPopup(globalKeywords, localKeywords, 150.0f),
...
internal static void ShaderErrorListUI(Object shader,
	...
	int errorListID =
		GUIUtility.GetControlID(kErrorViewHash, FocusType.Passive);
	float height = Mathf.Min(n * 20f + 40f, 150f);
	scrollPosition = GUILayout.BeginScrollView(
		scrollPosition, GUISkin.current.box, GUILayout.MinHeight(height));

	EditorGUIUtility.SetIconSize(new Vector2(16.0f, 16.0f));
	float lineHeight = Styles.messageStyle.CalcHeight(
		EditorGUIUtility.TempContent(Styles.errorIcon), 100);
	...
	for (int i = 0; i < n; ++i) {
		...
		if (e.type == EventType.MouseDown && e.button == 0 &&
			...
			if (e.clickCount == 2) {
				...
				Object asset = string.IsNullOrEmpty(filePath)
			     ? null
			     : AssetDatabase.LoadMainAssetAtPath(filePath);

				// if we don't have an asset and the filePath is an absolute path,
				// it's an error in a system cginc - open that instead
		...
		if (e.type == EventType.ContextClick && r.Contains(e.mousePosition)) {
			...
			var errorIndex = i;
			menu.AddItem(
				EditorGUIUtility.TrTextContent("Copy error text"), false, delegate {
		...
		if (line > 0) {
			...
			if (string.IsNullOrEmpty(fileName))
				...
				gc = EditorGUIUtility.TempContent(
					fileName + ":" + line.ToString(CultureInfo.InvariantCulture));
		...
		if (plat.Length > 0) {
			...
			// draw platform in dimmer color; it's often not very important
			// information
		...
		GUI.Label(msgRect,
		          EditorGUIUtility.TempContent(err, warn ? Styles.warningIcon
		                             : Styles.errorIcon),
		          Styles.messageStyle);
	}
...
private void ShowCompiledCodeButton(Shader s) {
	...
	ShaderImporter importer = AssetImporter.GetAtPath(
		AssetDatabase.GetAssetPath(s.GetInstanceID())) as ShaderImporter;
	bool enablePreprocessOnly = EditorSettings.cachingShaderPreprocessor;
	if (importer && importer.preprocessorOverride ==
	    PreprocessorOverride.ForceCachingPreprocessor)
		enablePreprocessOnly = true;
	using (new EditorGUI.DisabledScope(!enablePreprocessOnly)) s_PreprocessOnly =
	...
	var hasCode = ShaderUtil.HasShaderSnippets(s) ||
	              ShaderUtil.HasSurfaceShaders(s) ||
	              ShaderUtil.HasFixedFunctionShaders(s);
	if (hasCode) {
		// button with a drop-down part on the right
		var modeContent = Styles.showCurrent;
		var modeRect = GUILayoutUtility.GetRect(
			modeContent, EditorStyles.miniButton, GUILayout.ExpandWidth(false));
		var modeDropRect =
			new Rect(modeRect.xMax - 16, modeRect.y, 16, modeRect.height);
		if (EditorGUI.DropdownButton(modeDropRect, GUIContent.none,
		...
		if (GUI.Button(modeRect, modeContent, EditorStyles.miniButton)) {
			ShaderUtil.OpenCompiledShader(
				s, ShaderInspectorPlatformsPopup.currentMode,
				ShaderInspectorPlatformsPopup.currentPlatformMask,
				ShaderInspectorPlatformsPopup.currentVariantStripping == 0,
				enablePreprocessOnly && s_PreprocessOnly);
			GUIUtility.ExitGUI();
		}
	} else {
		// Note: PrefixLabel is sometimes buggy if followed by a non-control (like
		// Label). We just want to show a label here, but have to pretend it's a
		// button so it is treated like a control.
...
private static void ShowSurfaceShaderButton(Shader s) {
	...
	if (hasSurface) {
		...
		var builtinShader =
			(AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(s)) == null);
		if (!builtinShader) {
			if (GUILayout.Button(Styles.showSurface, EditorStyles.miniButton,
...
private static void ShowFixedFunctionShaderButton(Shader s) {
	...
	if (hasFF) {
		...
		var builtinShader =
			(AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(s)) == null);
		if (!builtinShader) {
			if (GUILayout.Button(Styles.showFF, EditorStyles.miniButton,
			                     GUILayout.ExpandWidth(false))) {
...
public KeywordsPopup(string[] globalKeywords, string[] localKeywords,
                     float windowWidth) {
...
public override void OnGUI(Rect rect) {
	...
	m_GlobalKeywordsExpended = KeywordsFoldout(
		m_GlobalKeywordsExpended, "Global Keywords", m_GlobalKeywords);
	m_LocalKeywordsExpended = KeywordsFoldout(
		m_LocalKeywordsExpended, "Local Keywords", m_LocalKeywords);
...
static internal readonly string[] s_PlatformModes = {
	"Current graphics device", "Current build platform", "All platforms",
	"Custom:"
};
...
public static int currentPlatformMask {
	get {
		if (s_CurrentPlatformMask < 0) {
			int defaultMask =
				(1
				        << Enum.GetNames(typeof(Rendering.ShaderCompilerPlatform)).Length -
				        1);
			s_CurrentPlatformMask =
				EditorPrefs.GetInt("ShaderInspectorPlatformMask", defaultMask);
		}
	...
	get {
		if (s_CurrentVariantStripping < 0)
			s_CurrentVariantStripping =
				EditorPrefs.GetInt("ShaderInspectorVariantStripping", 1);
...
public override Vector2 GetWindowSize() {
	...
	var windowHeight =
		rowCount * EditorGUI.kSingleLineHeight + kSeparatorHeight * 3;
...
private void DrawSeparator(ref Rect rect) {
	GUI.Label(new Rect(rect.x + 5, rect.y + 3, rect.width - 10, 3),
	          GUIContent.none, Styles.separator);
...
void DoOneMode(Rect rect, int index) {
	...
	GUI.Toggle(rect, currentMode == index,
	           EditorGUIUtility.TempContent(s_PlatformModes[index]),
	           Styles.menuItem);
...
void DoCustomPlatformBit(Rect rect, int index) {
	...
	on = GUI.Toggle(rect, on,
	                EditorGUIUtility.TempContent(s_ShaderPlatformNames[index]),
	                Styles.menuItem);
	if (EditorGUI.EndChangeCheck()) {
		if (on)
...
static string FormatCount(ulong count) {
	if (count > 1000 * 1000 * 1000)
		return ((double) count / 1000000000.0)
...
void DoShaderVariants(EditorWindow caller, ref Rect drawPos) {
	...
	EditorGUI.BeginChangeCheck();
	bool strip =
		GUI.Toggle(drawPos, currentVariantStripping == 1,
		           EditorGUIUtility.TempContent("Skip unused shader_features"),
		           Styles.menuItem);
	drawPos.y += EditorGUI.kSingleLineHeight;
	if (EditorGUI.EndChangeCheck())
	...
	var variantText = FormatCount(variantCount) + (strip ? " variants included"
	                                           : " variants total");
