...
internal class SnapGuideCollection {
  ...
  private Dictionary<float, List<SnapGuide>> guides =
      new Dictionary<float, List<SnapGuide>>();

  public void Clear() { guides.Clear(); }
  ...
  public float SnapToGuides(float value, float snapDistance) {
    ...
    KeyValuePair<float, List<SnapGuide>> snapPair =
        new KeyValuePair<float, List<SnapGuide>>();
...
internal class SnapGuide {
  ...
  public void Draw() {
    Handles.color = safe ? new Color(0.0f, 0.5f, 1.0f, 1.0f)
...
internal class RectTransformSnapping {
  ...
  private static SnapGuideCollection[] s_SnapGuides = new SnapGuideCollection[]{
      new SnapGuideCollection(), new SnapGuideCollection()};
  ...
  static Vector3 GetInterpolatedCorner(Vector3[] corners, int mainAxis,
                                       float alongMainAxis,
                                       float alongOtherAxis) {
    if (mainAxis != 0) {
  ...
  internal static void CalculatePivotSnapValues(Rect rect, Vector3 pivot,
                                                Quaternion rotation) {
    for (int axis = 0; axis < 2; axis++) {
      ...
      for (int i = 0; i < kSidesAndMiddle.Length; i++)
        s_SnapGuides [axis]
            .AddGuide(
                new SnapGuide(kSidesAndMiddle [i]
                              ,
                              GetGuideLineForRect(rect, pivot, rotation, axis,
                                                  kSidesAndMiddle[i])));
    }
  }

  internal static void CalculateAnchorSnapValues(Transform parentSpace,
                                                 Transform self,
                                                 RectTransform gui, int minmaxX,
                                                 int minmaxY) {
    for (int axis = 0; axis < 2; axis++) {
      s_SnapGuides [axis]
          .Clear();
      ...
      for (int i = 0; i < kSidesAndMiddle.Length; i++) {
        ...
        s_SnapGuides [axis]
            .AddGuide(new SnapGuide(
                val, GetInterpolatedCorner(s_Corners, axis, val, 0),
      ...
      foreach (Transform tr in parentSpace) {
        ...
        if (sibling) {
          s_SnapGuides [axis]
              .AddGuide(new SnapGuide(sibling.anchorMin[axis]));
          s_SnapGuides [axis]
              .AddGuide(new SnapGuide(sibling.anchorMax[axis]));
        }
      }
      ...
      if (minmax == 0)
        s_SnapGuides [axis]
            .AddGuide(new SnapGuide(gui.anchorMax[axis]));
      if (minmax == 1)
        s_SnapGuides [axis]
            .AddGuide(new SnapGuide(gui.anchorMin[axis]));
    }
  }

  // handle values are: 0 = min, 1 = middle, 2 = max
  internal static void CalculateOffsetSnapValues(Transform parentSpace,
                                                 Transform self,
  ...
  internal static void CalculatePositionSnapValues(Transform parentSpace,
                                                   Transform self,
                                                   RectTransform parentRect,
                                                   RectTransform rect) {
    for (int axis = 0; axis < 2; axis++)
    ...
    for (int axis = 0; axis < 2; axis++) {
      for (int side = 0; side < kSidesAndMiddle.Length; side++) {
        ...
        foreach (SnapGuide guide in guides) {
          guide.value = GetGuideValueForRect(rect, guide.value, axis,
                                             kSidesAndMiddle[side]);
          s_SnapGuides [axis]
              .AddGuide(guide);
        }
      }
  ...
  GetSnapGuides(Transform parentSpace, Transform self, RectTransform parentRect,
                RectTransform rect, int axis, int side) {
    List<SnapGuide> guides = new List<SnapGuide>();

    // For snapping we have a concept of safe and unsafe snapping.
    //  - Safe snapping means the objects will remain aligned when the parent
    //  rect changes size.
    //  - Unsafe snapping means that the objects are aligned right now, but will
    ...
    if (parentRect != null) {
      ...
      // Snap min side to min anchor, center to center point, max side to max
      // anchor - this is always safe
      float normalized =
          Mathf.Lerp(rect.anchorMin[axis], rect.anchorMax[axis], val);
      guides.Add(
          new SnapGuide(normalized * parentRect.rect.size [axis]
                        ,
                        GetGuideLineForRect(parentRect, axis, normalized)));

      // Snap to sides and center of the parent rect (if not the same as the
      // anchor). This snapping is not safe.
      float anchorVal =
          Mathf.Lerp(rect.anchorMin[axis], rect.anchorMax[axis], val);
      if (val != anchorVal) {
    ...
    foreach (Transform tr in parentSpace) {
      ...
      if (sibling) {
        if (side == 0) {
          ...
          guides.Add(new SnapGuide(sibling.GetRectInParentSpace().min[axis],
                                   safe,
                                   GetGuideLineForRect(sibling, axis, 0)));

          // Snap min to max side
          // This is safe if min anchor of this object is equal to max anchor of
          // the sibling.
          safe = (sibling.anchorMax[axis] == rect.anchorMin[axis]);
          guides.Add(new SnapGuide(sibling.GetRectInParentSpace().max[axis],
                                   safe,
        ...
        if (side == 2) {
          ...
          // This is safe if max anchor of this object is equal to min anchor of
          // the sibling.
        ...
        if (side == 1) {
          ...
          // This is safe if the difference between the min anchors is the
          // opposite of the difference between the max anchors. This also
          // includes the case where both objects have the same min and max
          // anchors.
          safe = (sibling.anchorMin[axis] - rect.anchorMin[axis] ==
                  -(sibling.anchorMax[axis] - rect.anchorMax[axis]));
          guides.Add(new SnapGuide(sibling.GetRectInParentSpace().center[axis],
                                   safe,
  ...
  private static Vector3[] GetGuideLineForRect(RectTransform rect, int axis,
                                               float side) {
    Vector3[] points = new Vector3[2];

    // Set end points of line on other axis
    points [0]
    [1 - axis] = rect.rect.min[1 - axis];
    points [1]
    [1 - axis] = rect.rect.max[1 - axis];

    // Set position of line on axis
    points [0]
    [axis] = Mathf.Lerp(rect.rect.min[axis], rect.rect.max[axis], side);
    points [1]
    [axis] = points [0]
    [axis];
  ...
  private static Vector3[] GetGuideLineForRect(Rect rect, Vector3 pivot,
                                               Quaternion rotation, int axis,
                                               float side) {
    Vector3[] points = new Vector3[2];

    // Set end points of line on other axis
    points [0]
    [1 - axis] = rect.min[1 - axis];
    points [1]
    [1 - axis] = rect.max[1 - axis];

    // Set position of line on axis
    points [0]
    [axis] = Mathf.Lerp(rect.min[axis], rect.max[axis], side);
    points [1]
    [axis] = points [0]
    [axis];
  ...
  private static float GetGuideValueForRect(RectTransform rect, float value,
                                            int axis, float side) {
    RectTransform parentRect =
        rect.transform.parent.GetComponent<RectTransform>();
    float parentSize = parentRect ? parentRect.rect.size[axis] : 0;
    float anchorReference = Mathf.Lerp(rect.anchorMin[axis],
                                       rect.anchorMax[axis], rect.pivot[axis]) *
                            parentSize;
    float positionRelativeToSide =
        rect.rect.size[axis] * (rect.pivot[axis] - side);
  ...
  internal static Vector2 SnapToGuides(Vector2 value, Vector2 snapDistance) {
    return new Vector2(SnapToGuides(value.x, snapDistance.x, 0),
                       SnapToGuides(value.y, snapDistance.y, 1));
  }

  internal static float SnapToGuides(float value, float snapDistance,
                                     int axis) {
    if (EditorGUI.actionKey)
      return value;

    SnapGuideCollection guides =
        (axis == 0 ? s_SnapGuides[0] : s_SnapGuides[1]);
