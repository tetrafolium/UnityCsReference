...
public struct LayoutGroupChecker : IDisposable {
	// cache the layout group we expect to have at the end of drawing this
	// editor
	GUILayoutGroup m_ExpectedGroup;
	GUILayoutGroup expectedGroup =>
	m_ExpectedGroup ??(m_ExpectedGroup = GUILayoutUtility.current.topLevel);
...
public static void
GetPreviewableTypes(out Dictionary<Type, List<Type> > previewableTypes) {
	...
	foreach (var type in TypeCache.GetTypesDerivedFrom<IPreviewable>()) {
		...
		var attrs = type.GetCustomAttributes(typeof(CustomPreviewAttribute),
		                                     false) as CustomPreviewAttribute[];
...
internal static bool IsExcludedClass(Object target) {
	return ModuleMetadata.GetModuleIncludeSettingForObject(target) ==
	       ModuleIncludeSetting.ForceExclude;
}
...
public static void DisplayDeprecationMessageIfNecessary(Editor editor) {
	...
	var obsoleteAttribute = (ObsoleteAttribute) Attribute.GetCustomAttribute(
		editor.target.GetType(), typeof(ObsoleteAttribute));
	if (obsoleteAttribute == null) {
		return;
	}

	string message = String.IsNullOrEmpty(obsoleteAttribute.Message)
	                 ? "This component has been marked as obsolete."
	                 : obsoleteAttribute.Message;
	EditorGUILayout.HelpBox(message,
	                        obsoleteAttribute.IsError ? MessageType.Error
	                    : MessageType.Warning);
}

public static void DrawAddedComponentBackground(Rect position,
                                                Object[] targets,
                                                float adjust = 0) {
	if (Event.current.type == EventType.Repaint && targets.Length == 1) {
		...
		if (comp != null &&
			...
			// Ensure colored margin here for component body doesn't overlap colored
			// margin from InspectorTitlebar, and extends down to exactly touch the
			// separator line between/after components.
			EditorGUI.DrawOverrideBackground(
				new Rect(position.x, position.y + 3 + adjust, position.width,
				         position.height - 2));
		}
	}
}
