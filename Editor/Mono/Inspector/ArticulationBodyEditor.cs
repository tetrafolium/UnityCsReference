...
internal class ArticulationBodyEditor : Editor {
  ...
  internal enum NonLockedMotion { Free = 2, Limited = 1 }

  internal enum LinearDof { X, Y, Z }

  public void OnEnable() {
    m_Mass = serializedObject.FindProperty("m_Mass");
    m_ParentAnchorPosition =
        serializedObject.FindProperty("m_ParentAnchorPosition");
    m_ParentAnchorRotation =
        serializedObject.FindProperty("m_ParentAnchorRotation");
    ...
    m_ComputeParentAnchor =
        serializedObject.FindProperty("m_ComputeParentAnchor");
    m_ArticulationJointType =
        serializedObject.FindProperty("m_ArticulationJointType");
  ...
  public void OnDisable() { m_ShowInfo.valueChanged.RemoveListener(Repaint); }

  private void QuaternionAsEulerAnglesPropertyField(
      string tag, SerializedProperty quaternionProperty, Quaternion rotation) {
    quaternionProperty.quaternionValue = Quaternion.Euler(
        EditorGUILayout.Vector3Field(tag, rotation.eulerAngles));
  }
  ...
  public override void OnInspectorGUI() {
    ...
    if (body.isRoot) {
      ...
      EditorGUILayout.HelpBox("This is the root body of the articulation.",
                              MessageType.Info);
    } else {
      EditorGUILayout.PropertyField(m_UseGravity);
      ...
      // The reason we have change checks here is because in AwakeFromLoad we
      // won't overwrite anchors If we were to do that, simulation would drift
      // caused by anchors reset relative to current poses
      EditorGUI.BeginChangeCheck();
      EditorGUILayout.PropertyField(m_AnchorPosition);
      QuaternionAsEulerAnglesPropertyField("Anchor Rotation", m_AnchorRotation,
                                           body.anchorRotation);
      if (EditorGUI.EndChangeCheck()) {
        ...
        if (m_ComputeParentAnchor.boolValue) {
          ...
          // lets fetch them back otherwise ApplyModifiedProperties overwrites
          // them
      ...
      if (!m_ComputeParentAnchor.boolValue) {
        ...
        QuaternionAsEulerAnglesPropertyField("Parent Anchor Rotation",
                                             m_ParentAnchorRotation,
      ...
      if (GUILayout.Button("Snap anchor to closest contact")) {
        ...
        Undo.RecordObject(
            body,
            "Changing anchor position/rotation to match closest contact.");
        Vector3 com = parentBody.worldCenterOfMass;
        Vector3 closestOnSurface = body.GetClosestPoint(com);
        body.anchorPosition =
            body.transform.InverseTransformPoint(closestOnSurface);
        body.anchorRotation = Quaternion.FromToRotation(
            Vector3.right,
            body.transform.InverseTransformDirection(com - closestOnSurface)
                .normalized);
      }
      ...
      case ArticulationJointType.PrismaticJoint:
        ...
        NonLockedMotion motion =
            (dofLockSetting == ArticulationDofLock.FreeMotion)
        ...
        if (EditorGUI.EndChangeCheck() || overrideDof) {
          m_LinearX.enumValueIndex =
              (linearDof == LinearDof.X) ? (int) motion : 0;
          m_LinearY.enumValueIndex =
              (linearDof == LinearDof.Y) ? (int) motion : 0;
          m_LinearZ.enumValueIndex =
              (linearDof == LinearDof.Z) ? (int) motion : 0;
        }

        EditorGUILayout.Space();

        DoDriveInspector(m_YDrive,
                         (ArticulationDofLock) m_LinearY.enumValueIndex);
        DoDriveInspector(m_ZDrive,
                         (ArticulationDofLock) m_LinearZ.enumValueIndex);
        DoDriveInspector(m_XDrive,
                         (ArticulationDofLock) m_LinearX.enumValueIndex);

        break;

      case ArticulationJointType.RevoluteJoint:
        ArticulationDofLock serialisedDofLock =
            (ArticulationDofLock) m_Twist.enumValueIndex;

        NonLockedMotion revoluteMotion =
            serialisedDofLock ==
            ArticulationDofLock.LimitedMotion
                                                 ? NonLockedMotion
                .Limited : NonLockedMotion.Free;

        EditorGUILayout.Space();
        motion = (NonLockedMotion)
                     EditorGUILayout.EnumPopup("Motion", revoluteMotion);
      ...
      case ArticulationJointType.SphericalJoint:
        EditorGUILayout.Space();
        // here we just need to make sure we disable getting into an invalid
        // configuration
  ...
  private void SphericalJointMotionSetting(SerializedProperty axis) {
    EditorGUI.BeginChangeCheck();
    int oldSetting =
        axis.enumValueIndex; // assume it was correct before changing
    EditorGUILayout.PropertyField(axis);
    if (EditorGUI.EndChangeCheck()) {
      if (m_SwingY.enumValueIndex == 0 && m_SwingZ.enumValueIndex == 0 &&
          m_Twist.enumValueIndex == 0) {
  ...
  protected virtual void OnSceneGUI() {
...

    {
      ...
      if (EditorGUI.EndChangeCheck()) {
        Undo.RecordObject(
            target, "Changing Articulation body anchor position/rotation");
    ...
    if (!m_ComputeParentAnchor.boolValue) {
      ...
      if (EditorGUI.EndChangeCheck()) {
        Undo.RecordObject(
            target,
            "Changing Articulation body parent anchor position/rotation");
  ...
  private void DisplayJointLimits(ArticulationBody body) {
    ...
    Matrix4x4 parentAnchorSpace = Matrix4x4.TRS(
        parentBody.transform.TransformPoint(body.parentAnchorPosition),
        parentBody.transform.rotation * body.parentAnchorRotation, Vector3.one);
  ...
  private void ShowPrismaticLimits(ArticulationBody body,
                                   ArticulationBody parentBody,
                                   Matrix4x4 parentAnchorSpace) {
    // if prismatic and unlocked - nothing to visualise
    if (body.linearLockX == ArticulationDofLock.FreeMotion ||
    ...
    using(new Handles.DrawingScope(parentAnchorSpace)) {
      ...
      Handles.CylinderHandleCap(0, lowerPoint, orientation, CapScale,
                                EventType.Repaint);

      Handles.color = Color.green;
      Handles.CylinderHandleCap(0, upperPoint, orientation, CapScale,
                                EventType.Repaint);
  ...
  private void ShowAngularLimitSpan(bool freeMotion, ArticulationBody body,
                                    ArticulationBody parentBody, Color color,
                                    Vector3 axis, Matrix4x4 space,
    ...
    Quaternion zeroPose = Quaternion.FromToRotation(
        Vector3.forward, Vector3.Cross(axis, zeroDirection));
    ...
    using(new Handles.DrawingScope(space)) {
      color.a = 0.3f;
      Handles.color = color;
      Handles.DrawSolidArc(Vector3.zero, axis, from, totalAngle,
                           GizmoLinearSize);

      Handles.color = Color.red;
      Handles.CylinderHandleCap(0, from * GizmoLinearSize,
                                lowerRotation * zeroPose, CapScale,
                                EventType.Repaint);

      Handles.color = Color.green;
      Handles.CylinderHandleCap(0, to * GizmoLinearSize,
                                upperRotation * zeroPose, CapScale,
                                EventType.Repaint);
    }
  }

  private void ShowRevoluteLimits(ArticulationBody body,
                                  ArticulationBody parentBody,
                                  Matrix4x4 parentAnchorSpace) {
    bool free = (body.twistLock == ArticulationDofLock.FreeMotion);

    ShowAngularLimitSpan(free, body, parentBody, Color.red, Vector3.right,
                         parentAnchorSpace, body.xDrive, Vector3.forward);
  }

  // this is a variant that draws cross
  private void ShowSphericalLimits(ArticulationBody body,
                                   ArticulationBody parentBody,
                                   Matrix4x4 parentAnchorSpace) {
    // swing z
    if (body.swingZLock != ArticulationDofLock.LockedMotion) {
      bool free = (body.swingZLock == ArticulationDofLock.FreeMotion);
      ShowAngularLimitSpan(free, body, parentBody, Color.blue, Vector3.forward,
                           parentAnchorSpace, body.zDrive, Vector3.right);
    }

    // swing y
    if (body.swingYLock != ArticulationDofLock.LockedMotion) {
      bool free = (body.swingYLock == ArticulationDofLock.FreeMotion);
      ShowAngularLimitSpan(free, body, parentBody, Color.green, Vector3.up,
                           parentAnchorSpace, body.yDrive, Vector3.right);
    }
  ...
  private void DisplayProperAnchorHandle(ArticulationBody body,
                                         ref Vector3 anchorPos,
                                         ref Quaternion anchorRot) {
    float handleScaling = 0.5f;

    var bodySpace =
        Matrix4x4.TRS(body.transform.position, body.transform.rotation,
                      Vector3.one * handleScaling);

    // Need to pre-scale in body space because our handle matrix is scaled, we
    // will remove scaling after reading back
  ...
  private void DoDriveInspector(SerializedProperty drive,
                                ArticulationDofLock dofLock) {
  ...
  private void ShowBodyInfoProperties() {
    ...
    m_ShowInfoFoldout.value = m_ShowInfo.target =
        EditorGUILayout.Foldout(m_ShowInfo.target, "Info", true);
    if (EditorGUILayout.BeginFadeGroup(m_ShowInfo.faded)) {
      if (targets.Length == 1) {
        ...
        EditorGUILayout.Vector3Field("Inertia Tensor Rotation",
                                     body.inertiaTensorRotation.eulerAngles);
        EditorGUILayout.Vector3Field("Local Center of Mass", body.centerOfMass);
        EditorGUILayout.Vector3Field("World Center of Mass",
                                     body.worldCenterOfMass);
        EditorGUILayout.LabelField("Sleep State",
                                   body.IsSleeping() ? "Asleep" : "Awake");
        EditorGUI.EndDisabledGroup();

        // We need to repaint as some of the above properties can change without
        // causing a repaint.
        if (EditorApplication.isPlaying)
          m_RequiresConstantRepaint = true;
      } else {
        EditorGUILayout.HelpBox(
            "Cannot show Info properties when multiple bodies are selected.",
            MessageType.Info);
      }
    }
