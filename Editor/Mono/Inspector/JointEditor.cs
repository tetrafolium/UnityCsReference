    ...
    if (editor.targets.Length == 1) {
      ...
      if (joint.connectedBody != null) {
        ...
        if (bodyScene.IsValid() && connectedBodyScene.IsValid()) {
          if (bodyScene.GetPhysicsScene() !=
              connectedBodyScene.GetPhysicsScene()) {
            EditorGUILayout.HelpBox(
                "This joint will not function because it is connected to a Rigidbody in a different physics scene. This is not supported.",
                MessageType.Warning);
          }
        }
      }
...
abstract class JointEditor<T> : Editor where T : Joint {
  ...
  protected void DoInspectorEditButtons() {
    ...
    EditorGUILayout.EditorToolbarForTarget(
        EditorGUIUtility.TrTempContent("Edit Angular Limits"), target);
  ...
  internal override Bounds GetWorldBoundsOfTarget(UnityObject targetObject) {
    ...
    if (jointTool != null)
      bounds.Encapsulate(jointTool.GetAngularLimitHandleMatrix((T) targetObject)
...
abstract class JointTool<T> : EditorTool where T : Joint {
  protected static class Styles {
    public static readonly string editAngularLimitsUndoMessage =
        L10n.Tr("Change Joint Angular Limits");
  }
  ...
  protected JointAngularLimitHandle angularLimitHandle {
    get { return m_AngularLimitHandle; }
  }
  ...
  public override void OnToolGUI(EditorWindow window) {
    foreach (var obj in targets) {
      ...
      // wake up rigidbodies in case current orientation is out of bounds of new
      // limits
  ...
  protected virtual void GetActors(T joint, out Rigidbody dynamicActor,
    ...
    if (dynamicActor.isKinematic && connectedActor != null &&
        !connectedActor.isKinematic) {
  ...
  internal Matrix4x4 GetAngularLimitHandleMatrix(T joint) {
    ...
    GetActors(joint, out dynamicActor, out connectedActor,
              out jointFrameActorIndex, out rightHandedLimit);

    var connectedBodyRotation =
        connectedActor == null ? Quaternion.identity
        : connectedActor.transform.rotation;

    // to enhance usability, orient the limit region so the dynamic body is
    // within it, assuming bodies were bound on opposite sides of the anchor
    ...
    var jointFrameOrientation = Quaternion.LookRotation(
        ...
        jointFrame.MultiplyVector(rightHandedLimit ? Vector3.down
                                  : Vector3.up));

    // point of rotation is about the anchor of the joint body, which is not
    // necessarily aligned to the anchor on the connected body
    var jointAnchorPosition = joint.anchor;
    if (dynamicActor != null)
      jointAnchorPosition =
          dynamicActor.transform.TransformPoint(jointAnchorPosition);

    return Matrix4x4.TRS(jointAnchorPosition,
                         connectedBodyRotation * jointFrameOrientation,
                         Vector3.one);
  }
