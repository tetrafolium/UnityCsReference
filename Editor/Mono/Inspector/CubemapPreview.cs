...
internal class CubemapPreview {
  static readonly int s_ShaderCubemapRotation =
      Shader.PropertyToID("_CubemapRotation");
  ...
  private enum PreviewType { RGB = 0, Alpha = 1 }
  // Preview settings
  [SerializeField]
  private PreviewType m_PreviewType = PreviewType.RGB;
  [SerializeField] float m_MipLevel = 0.0F;
  ...
  public void OnPreviewSettings(Object[] targets) {
    ...
    if (needsExposureControl) {
      m_ExposureSliderValue = EditorGUIInternal.ExposureSlider(
          m_ExposureSliderValue, ref m_ExposureSliderMax, Styles.preSlider);
    }
    ...
    m_MipLevel = Mathf.Round(GUILayout.HorizontalSlider(
        m_MipLevel, mipCount - 1, 0, Styles.preSlider, Styles.preSliderThumb,
  ...
  public Texture2D RenderStaticPreview(Texture t, int width, int height) {
    ...
    // When rendering the cubemap preview we don't need lighting so we provide a
    // custom list with no lights. If we don't do this and we are generating the
    // preview for a point light cookie, if a light uses this cookie it will try
    // to bind it which result in internal assert in AssetDatabase due to using
    // the texture while building it.
  ...
  private void RenderCubemap(Texture t, Vector2 previewDir,
                             float previewDistance) {
    m_PreviewUtility.camera.transform.position =
        -Vector3.forward * previewDistance;
    m_PreviewUtility.camera.transform.rotation = Quaternion.identity;
    Quaternion rot = Quaternion.Euler(previewDir.y, 0, 0) *
                     Quaternion.Euler(0, previewDir.x, 0);

    var mat = EditorGUIUtility.LoadRequired(
        "Previews/PreviewCubemapMaterial.mat") as Material;
    mat.mainTexture = t;

    mat.SetMatrix(s_ShaderCubemapRotation,
                  Matrix4x4.TRS(Vector3.zero, rot, Vector3.one));

    // -1 indicates "use regular sampling"; mips 0 and larger sample only that
    // mip level for preview
    float mipLevel = GetMipLevelForRendering(t);
    mat.SetFloat(s_ShaderMip, mipLevel);
    mat.SetFloat(s_ShaderAlpha,
                 (m_PreviewType == PreviewType.Alpha) ? 1.0f : 0.0f);
