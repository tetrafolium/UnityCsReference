...
internal class ShaderImporterInspector : AssetImporterEditor {
  ...
  private class ShaderProperties : ScriptableObject {
    ...
    public void CleanUp() { m_Properties.Clear(); }
  }
  ...
  protected override void InitializeExtraDataInstance(Object extraTarget,
    ...
    for (var i = 0; i < propertyCount; i++) {
      ...
      var modifiable = (shader.GetPropertyFlags(i) &
      ...
      var temp =
          new TextureProp{propertyName = propertyName, texture = tex,
                          dimension = shader.GetPropertyTextureDimension(i),
                          displayName = displayName, modifiable = modifiable};
  ...
  private void DrawTextureField(SerializedProperty prop) {
    ...
    Type textureType = MaterialEditor.GetTextureTypeFromDimension(
        (TextureDimension) prop.FindPropertyRelative("dimension").intValue);
    ...
    if (textureType != null) {
      ...
      var text =
          string.IsNullOrEmpty(displayName)
              ...
              : displayName;
      newTexture = EditorGUILayout.MiniThumbnailObjectField(
          GUIContent.Temp(text), oldTexture, textureType) as Texture;
    }
  ...
  protected override void Apply() {
    ...
    var defaultNames = properties.m_Properties.Where(x => x.modifiable)
                           .Select(x => x.propertyName)
                           .ToArray();
    var defaultTextures = properties.m_Properties.Where(x => x.modifiable)
                              .Select(x => x.texture)
                              .ToArray();
    importer.SetDefaultTextures(defaultNames, defaultTextures);

    var nonModNames = properties.m_Properties.Where(x => !x.modifiable)
                          .Select(x => x.propertyName)
                          .ToArray();
    var nonModTextures = properties.m_Properties.Where(x => !x.modifiable)
                             .Select(x => x.texture)
                             .ToArray();
