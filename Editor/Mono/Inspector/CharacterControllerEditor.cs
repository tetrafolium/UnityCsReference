...
internal class CharacterControllerEditor : Editor {
  ...
  public void OnSceneGUI() {
    ...
    float radius = cc.radius * Mathf.Max(cc.transform.lossyScale.x,
                                         cc.transform.lossyScale.z);
    height = Mathf.Max(height, radius * 2);

    Matrix4x4 matrix = Matrix4x4.TRS(cc.transform.TransformPoint(cc.center),
    ...
    if (!GUI.changed)
      adjusted =
          SizeHandle(-Vector3.left * radius, -Vector3.left, matrix, true);
    if (!GUI.changed)
      adjusted =
          SizeHandle(Vector3.forward * radius, Vector3.forward, matrix, true);
    if (!GUI.changed)
      adjusted =
          SizeHandle(-Vector3.forward * radius, -Vector3.forward, matrix, true);
    if (GUI.changed) {
  ...
  private static float SizeHandle(Vector3 localPos, Vector3 localPullDir,
                                  Matrix4x4 matrix, bool isEdgeHandle) {
    ...
    if (Camera.current.orthographic)
      ...
      cosV = Vector3.Dot(
          (Camera.current.transform.position - worldPos).normalized, worldDir);
    if (cosV < -displayThreshold)
      Handles.color =
          new Color(Handles.color.r, Handles.color.g, Handles.color.b,
                    Handles.color.a * Handles.backfaceAlphaMultiplier);

    // Now do handle
    Vector3 newWorldPos = Handles.Slider(worldPos, worldDir, handleSize * 0.03f,
                                         Handles.DotHandleCap, 0f);
    float adjust = 0.0f;
    if (GUI.changed) {
      // Project newWorldPos to worldDir  (the sign of the return value
      // indicates if we growing or shrinking)
      adjust =
          HandleUtility.PointOnLineParameter(newWorldPos, worldPos, worldDir);
    }
