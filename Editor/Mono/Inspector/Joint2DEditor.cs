...
internal class Joint2DEditor : Editor {
  ...
  public override void OnInspectorGUI() {
    ...
    if (targets.Length == 1) {
      ...
      if (joint.connectedBody != null && joint.gameObject.activeInHierarchy &&
          joint.connectedBody.gameObject.activeInHierarchy &&
          joint.gameObject.scene.GetPhysicsScene2D() !=
              joint.connectedBody.gameObject.scene.GetPhysicsScene2D()) {
        EditorGUILayout.HelpBox(
            "This joint will not function because it is connected to a Rigidbody2D in a different physics scene. This is not supported.",
            MessageType.Warning);
      }
    }
    ...
    // Distance/Spring/Target joints produce no reaction torque so they're not
    // supported.
    var targetType = target.GetType();
    if (targetType != typeof(DistanceJoint2D) &&targetType !=
        typeof(TargetJoint2D) &&targetType != typeof(SpringJoint2D))
  ...
  protected bool HandleAnchor(ref Vector3 position, bool isConnectedAnchor) {
    ...
    var drawCapFunction = isConnectedAnchor ?(Handles.CapFunction)
        ConnectedAnchorHandleCap : (Handles.CapFunction) AnchorHandleCap;

    int id = target.GetInstanceID() + (isConnectedAnchor ? 1 : 0);

    EditorGUI.BeginChangeCheck();
    position = Handles.Slider2D(id, position, Vector3.back, Vector3.right,
                                Vector3.up, 0, drawCapFunction, Vector2.zero);
    return EditorGUI.EndChangeCheck();
  }

  public static void AnchorHandleCap(int controlID, Vector3 position,
  ...
  public static void ConnectedAnchorHandleCap(int controlID, Vector3 position,
                                              Quaternion rotation, float size,
                                              EventType eventType) {
    if (controlID == GUIUtility.keyboardControl)
  ...
  static void HandleCap(int controlID, Vector3 position, GUIStyle guiStyle,
                        EventType eventType) {
    switch (eventType) {
    case EventType.Layout:
    case EventType.MouseMove:
      HandleUtility.AddControl(
          controlID, HandleUtility.DistanceToRectangleInternal(
                         position, Quaternion.identity, Vector2.zero));
      break;
  ...
  public static void DrawDistanceGizmo(Vector3 anchor, Vector3 connectedAnchor,
                                       float distance) {
  ...
  protected static Vector3 TransformPoint(Transform transform,
                                          Vector3 position) {
    // Local to World
    return GetAnchorSpaceMatrix(transform).MultiplyPoint(position);
  }

  protected static Vector3 InverseTransformPoint(Transform transform,
                                                 Vector3 position) {
    // World to Local
    return GetAnchorSpaceMatrix(transform).inverse.MultiplyPoint(position);
  }

  protected static Vector3 SnapToSprite(SpriteRenderer spriteRenderer,
                                        Vector3 position, float snapDistance) {
    ...
    var anchors =
        new[]{new Vector2(-x, -y), new Vector2(0, -y), new Vector2(x, -y),
  ...
  protected static Vector3 SnapToPoint(Vector3 position, Vector3 snapPosition,
                                       float snapDistance) {
    snapDistance = HandleUtility.GetHandleSize(position) * snapDistance;
    return Vector3.Distance(position, snapPosition) <=
           snapDistance ? snapPosition : position;
  }
