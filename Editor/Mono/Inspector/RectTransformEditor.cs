...
internal class RectTransformEditor : Editor {
  // Consts
  // (Some are technically statics, but still have k prefix because they're not
  // meant to be changed.)

  private const string kShowAnchorPropsPrefName =
      "RectTransformEditor.showAnchorProperties";
  ...
  class Styles {
    ...
    public GUIContent anchorsContent =
        EditorGUIUtility.TrTextContent("Anchors");
    public GUIContent anchorMinContent = EditorGUIUtility.TrTextContent(
        "Min",
        "The normalized position in the parent rectangle that the lower left corner is anchored to.");
    public GUIContent anchorMaxContent = EditorGUIUtility.TrTextContent(
        "Max",
        "The normalized position in the parent rectangle that the upper right corner is anchored to.");
    public GUIContent pivotContent = EditorGUIUtility.TrTextContent(
        "Pivot",
        "The pivot point specified in normalized values between 0 and 1. The pivot point is the origin of this rectangle. Rotation and scaling are around this point.");
    public GUIContent transformScaleContent = EditorGUIUtility.TrTextContent(
        "Scale",
        "The local scaling of this Game Object relative to the parent. This scales everything including image borders and text.");
    public GUIContent rawEditContent;
    public GUIContent blueprintContent;

    public Styles() {
      rawEditContent = EditorGUIUtility.IconContent(
          @"RectTransformRaw",
          "|Raw edit mode. When enabled, editing pivot and anchor values will not counter-adjust the position and size of the rectangle in order to make it stay in place.");
      blueprintContent = EditorGUIUtility.IconContent(
          @"RectTransformBlueprint",
          "|Blueprint mode. Edit RectTransforms as if they were not rotated and scaled. This enables snapping too.");
    }
  }
  ...
  private static int s_ParentRectPreviewHandlesHash =
      "ParentRectPreviewDragHandles".GetHashCode();
  private static GUIContent[] s_XYLabels = {new GUIContent("X"),
                                            new GUIContent("Y")};
  private static GUIContent[] s_XYZLabels = {
  ...
  private Dictionary<int, AnimBool> m_KeyboardControlIDs =
      new Dictionary<int, AnimBool>();
  private AnimatedValues.AnimBool m_ChangingAnchors =
      new AnimatedValues.AnimBool();
  private AnimatedValues.AnimBool m_ChangingPivot =
      new AnimatedValues.AnimBool();
  private AnimatedValues.AnimBool m_ChangingWidth =
      new AnimatedValues.AnimBool();
  private AnimatedValues.AnimBool m_ChangingHeight =
      new AnimatedValues.AnimBool();
  private AnimatedValues.AnimBool m_ChangingPosX =
      new AnimatedValues.AnimBool();
  private AnimatedValues.AnimBool m_ChangingPosY =
      new AnimatedValues.AnimBool();
  private AnimatedValues.AnimBool m_ChangingLeft =
      new AnimatedValues.AnimBool();
  private AnimatedValues.AnimBool m_ChangingRight =
      new AnimatedValues.AnimBool();
  private AnimatedValues.AnimBool m_ChangingTop = new AnimatedValues.AnimBool();
  private AnimatedValues.AnimBool m_ChangingBottom =
      new AnimatedValues.AnimBool();
  ...
  void OnEnable() {
    ...
    m_RotationGUI.OnEnable(serializedObject.FindProperty("m_LocalRotation"),
  ...
  void HandleDragChange(string handleName, bool dragging) {
    ...
    switch (handleName) {
    case RectTool.kChangingLeft:
      animBool = m_ChangingLeft;
      break;
    case RectTool.kChangingRight:
      animBool = m_ChangingRight;
      break;
    case RectTool.kChangingPosY:
      animBool = m_ChangingPosY;
      break;
    case RectTool.kChangingWidth:
      animBool = m_ChangingWidth;
      break;
    case RectTool.kChangingBottom:
      animBool = m_ChangingBottom;
      break;
    case RectTool.kChangingTop:
      animBool = m_ChangingTop;
      break;
    case RectTool.kChangingPosX:
      animBool = m_ChangingPosX;
      break;
    case RectTool.kChangingHeight:
      animBool = m_ChangingHeight;
      break;
    case RectTool.kChangingPivot:
      animBool = m_ChangingPivot;
      break;
    default:
      animBool = null;
      break;
  ...
  void SetFadingBasedOnMouseDownUp(ref AnimatedValues.AnimBool animBool,
                                   Event eventBefore) {
    if (eventBefore.type == EventType.MouseDrag &&
        Event.current.type != EventType.MouseDrag)
      animBool.value = true;
    else if (eventBefore.type == EventType.MouseUp &&
             Event.current.type != EventType.MouseUp)
      animBool.target = false;
  }

  void SetFadingBasedOnControlID(ref AnimatedValues.AnimBool animBool, int id) {
    GUIView focusedView = (EditorWindow.focusedWindow == null ? null
    ...
    if (GUIUtility.keyboardControl == id && GUIView.current == focusedView) {
      ...
      m_KeyboardControlIDs[id] = animBool;
                ...
                GUIView.current != focusedView) &&
               m_KeyboardControlIDs.ContainsKey(id)) {
  ...
  void RepaintScene() { SceneView.RepaintAll(); }
  ...
  public override void OnInspectorGUI() {
    ...
    foreach (RectTransform gui in targets) {
      if (gui.drivenByObject != null) {
        anyDriven = true;
        if ((gui.drivenProperties &
             (DrivenTransformProperties.AnchoredPositionX |
              DrivenTransformProperties.SizeDeltaX)) != 0)
          anyDrivenXPositionOrSize = true;
        if ((gui.drivenProperties &
             (DrivenTransformProperties.AnchoredPositionY |
              DrivenTransformProperties.SizeDeltaY)) != 0)
          anyDrivenYPositionOrSize = true;
      }

      if ((gui.transform.parent == null ||
           gui.transform.parent.GetComponent<RectTransform>() == null) &&
    ...
    if (anyDriven) {
      if (targets.Length == 1)
        EditorGUILayout.HelpBox(
            "Some values driven by " +
                (target as RectTransform).drivenByObject.GetType().Name + ".",
            MessageType.None);
      else
        EditorGUILayout.HelpBox(
            "Some values in some or all objects are driven.", MessageType.None);
    }
    ...
    SmartPositionAndSizeFields(anyWithoutParent, anyDrivenXPositionOrSize,
                               anyDrivenYPositionOrSize);
    ...
    m_RotationGUI.RotationField(
        targets.Any(x =>((x as RectTransform).drivenProperties &
                         DrivenTransformProperties.Rotation) != 0));

    // Scale
    s_ScaleDisabledMask[0] =
        targets.Any(x =>((x as RectTransform).drivenProperties &
                         DrivenTransformProperties.ScaleX) != 0);
    s_ScaleDisabledMask[1] =
        targets.Any(x =>((x as RectTransform).drivenProperties &
                         DrivenTransformProperties.ScaleY) != 0);
    s_ScaleDisabledMask[2] =
        targets.Any(x =>((x as RectTransform).drivenProperties &
                         DrivenTransformProperties.ScaleZ) != 0);
    Vector3FieldWithDisabledMash(EditorGUILayout.GetControlRect(), m_LocalScale,
                                 styles.transformScaleContent,
                                 s_ScaleDisabledMask);
  ...
  static void Vector3FieldWithDisabledMash(Rect position,
    ...
    EditorGUI.MultiPropertyFieldInternal(
        position, s_XYZLabels, cur, EditorGUI.PropertyVisibility.OnlyVisible,
        disabledMask);
  ...
  void LayoutDropdownButton(bool anyWithoutParent) {
    ...
    using(new EditorGUI.DisabledScope(anyWithoutParent)) {
      ...
      if (EditorGUI.DropdownButton(dropdownPosition, GUIContent.none,
                                   FocusType.Passive, "box")) {
    ...
    if (!anyWithoutParent) {
      LayoutDropdownWindow.DrawLayoutMode(
          new RectOffset(7, 7, 7, 7).Remove(dropdownPosition), m_AnchorMin,
          m_AnchorMax, m_AnchoredPosition, m_SizeDelta);
      LayoutDropdownWindow.DrawLayoutModeHeadersOutsideRect(
          dropdownPosition, m_AnchorMin, m_AnchorMax, m_AnchoredPosition,
          m_SizeDelta);
    }
  }

  void SmartPositionAndSizeFields(bool anyWithoutParent, bool anyDrivenX,
                                  bool anyDrivenY) {
    Rect rect = EditorGUILayout.GetControlRect(
        true, EditorGUIUtility.singleLineHeight * 4);

    rect.height = EditorGUIUtility.singleLineHeight * 2;
    Rect rect2;

    bool anyStretchX = targets.Any(x =>(x as RectTransform).anchorMin.x !=
                                       (x as RectTransform).anchorMax.x);
    bool anyStretchY = targets.Any(x =>(x as RectTransform).anchorMin.y !=
                                       (x as RectTransform).anchorMax.y);
    bool anyNonStretchX = targets.Any(x =>(x as RectTransform).anchorMin.x ==
                                          (x as RectTransform).anchorMax.x);
    bool anyNonStretchY = targets.Any(x =>(x as RectTransform).anchorMin.y ==
                                          (x as RectTransform).anchorMax.y);

    rect2 = GetColumnRect(rect, 0);
    if (anyNonStretchX || anyWithoutParent || anyDrivenX) {
      EditorGUI.BeginProperty(rect2, null,
                              m_AnchoredPosition.FindPropertyRelative("x"));
      FloatFieldLabelAbove(
          rect2, rectTransform => rectTransform.anchoredPosition.x,
          (rectTransform, val) => rectTransform.anchoredPosition =
              new Vector2(val, rectTransform.anchoredPosition.y),
          DrivenTransformProperties.AnchoredPositionX,
          EditorGUIUtility.TrTextContent("Pos X"));
      SetFadingBasedOnControlID(ref m_ChangingPosX,
                                EditorGUIUtility.s_LastControlID);
      EditorGUI.EndProperty();
    } else {
      // Affected by both anchored position and size delta so do property
      // handling for both. (E.g. showing animated value, prefab override etc.)
      EditorGUI.BeginProperty(rect2, null,
                              m_AnchoredPosition.FindPropertyRelative("x"));
      EditorGUI.BeginProperty(rect2, null,
                              m_SizeDelta.FindPropertyRelative("x"));
      FloatFieldLabelAbove(rect2, rectTransform => rectTransform.offsetMin.x,
                           (rectTransform, val) => rectTransform.offsetMin =
                               new Vector2(val, rectTransform.offsetMin.y),
                           DrivenTransformProperties.None,
                           EditorGUIUtility.TrTextContent("Left"));
      SetFadingBasedOnControlID(ref m_ChangingLeft,
                                EditorGUIUtility.s_LastControlID);
      EditorGUI.EndProperty();
      EditorGUI.EndProperty();
    }
    ...
    if (anyNonStretchY || anyWithoutParent || anyDrivenY) {
      EditorGUI.BeginProperty(rect2, null,
                              m_AnchoredPosition.FindPropertyRelative("y"));
      FloatFieldLabelAbove(
          rect2, rectTransform => rectTransform.anchoredPosition.y,
          (rectTransform, val) => rectTransform.anchoredPosition =
              new Vector2(rectTransform.anchoredPosition.x, val),
          DrivenTransformProperties.AnchoredPositionY,
          EditorGUIUtility.TrTextContent("Pos Y"));
      SetFadingBasedOnControlID(ref m_ChangingPosY,
                                EditorGUIUtility.s_LastControlID);
      EditorGUI.EndProperty();
    } else {
      // Affected by both anchored position and size delta so do property
      ...
      EditorGUI.BeginProperty(rect2, null,
                              m_AnchoredPosition.FindPropertyRelative("y"));
      EditorGUI.BeginProperty(rect2, null,
                              m_SizeDelta.FindPropertyRelative("y"));
      FloatFieldLabelAbove(rect2, rectTransform => -rectTransform.offsetMax.y,
                           (rectTransform, val) => rectTransform.offsetMax =
                               new Vector2(rectTransform.offsetMax.x, -val),
                           DrivenTransformProperties.None,
                           EditorGUIUtility.TrTextContent("Top"));
      SetFadingBasedOnControlID(ref m_ChangingTop,
                                EditorGUIUtility.s_LastControlID);
      EditorGUI.EndProperty();
      EditorGUI.EndProperty();
    }
    ...
    FloatFieldLabelAbove(
        rect2, rectTransform => rectTransform.transform.localPosition.z,
        (rectTransform, val) => rectTransform.transform.localPosition =
            new Vector3(rectTransform.transform.localPosition.x,
                        rectTransform.transform.localPosition.y, val),
    ...
    if (anyNonStretchX || anyWithoutParent || anyDrivenX) {
      EditorGUI.BeginProperty(rect2, null,
                              m_SizeDelta.FindPropertyRelative("x"));
      FloatFieldLabelAbove(
          rect2, rectTransform => rectTransform.sizeDelta.x,
          (rectTransform, val) => rectTransform.sizeDelta =
              new Vector2(val, rectTransform.sizeDelta.y),
          DrivenTransformProperties.SizeDeltaX,
          anyStretchX ? EditorGUIUtility.TrTextContent("W Delta")
          : EditorGUIUtility.TrTextContent("Width"));
      SetFadingBasedOnControlID(ref m_ChangingWidth,
                                EditorGUIUtility.s_LastControlID);
      EditorGUI.EndProperty();
    } else {
      // Affected by both anchored position and size delta so do property
      ...
      EditorGUI.BeginProperty(rect2, null,
                              m_SizeDelta.FindPropertyRelative("x"));
      FloatFieldLabelAbove(rect2, rectTransform => -rectTransform.offsetMax.x,
                           (rectTransform, val) => rectTransform.offsetMax =
                               new Vector2(-val, rectTransform.offsetMax.y),
                           DrivenTransformProperties.None,
                           EditorGUIUtility.TrTextContent("Right"));
      SetFadingBasedOnControlID(ref m_ChangingRight,
                                EditorGUIUtility.s_LastControlID);
      EditorGUI.EndProperty();
      EditorGUI.EndProperty();
    }
    ...
    if (anyNonStretchY || anyWithoutParent || anyDrivenY) {
      EditorGUI.BeginProperty(rect2, null,
                              m_SizeDelta.FindPropertyRelative("y"));
      FloatFieldLabelAbove(
          rect2, rectTransform => rectTransform.sizeDelta.y,
          (rectTransform, val) => rectTransform.sizeDelta =
              new Vector2(rectTransform.sizeDelta.x, val),
          DrivenTransformProperties.SizeDeltaY,
          anyStretchY ? EditorGUIUtility.TrTextContent("H Delta")
          : EditorGUIUtility.TrTextContent("Height"));
      SetFadingBasedOnControlID(ref m_ChangingHeight,
                                EditorGUIUtility.s_LastControlID);
      EditorGUI.EndProperty();
    } else {
      // Affected by both anchored position and size delta so do property
      ...
      EditorGUI.BeginProperty(rect2, null,
                              m_SizeDelta.FindPropertyRelative("y"));
      FloatFieldLabelAbove(rect2, rectTransform => rectTransform.offsetMin.y,
                           (rectTransform, val) => rectTransform.offsetMin =
                               new Vector2(rectTransform.offsetMin.x, val),
                           DrivenTransformProperties.None,
                           EditorGUIUtility.TrTextContent("Bottom"));
      SetFadingBasedOnControlID(ref m_ChangingBottom,
                                EditorGUIUtility.s_LastControlID);
      EditorGUI.EndProperty();
      EditorGUI.EndProperty();
    }
  ...
  void SmartAnchorFields() {
    Rect anchorRect = EditorGUILayout.GetControlRect(
        true, EditorGUIUtility.singleLineHeight * (m_ShowLayoutOptions ? 3 : 1));
    ...
    m_ShowLayoutOptions = EditorGUI.Foldout(anchorRect, m_ShowLayoutOptions,
                                            styles.anchorsContent, true);
    EditorGUI.EndProperty();
    EditorGUI.EndProperty();
    ...
    Vector2Field(anchorRect, rectTransform => rectTransform.anchorMin.x,
                 (rectTransform, val) => SetAnchorSmart(
                     rectTransform, val, 0, false, !m_RawEditMode, true),
                 rectTransform => rectTransform.anchorMin.y,
                 (rectTransform, val) => SetAnchorSmart(
                     rectTransform, val, 1, false, !m_RawEditMode, true),
                 DrivenTransformProperties.AnchorMinX,
                 DrivenTransformProperties.AnchorMinY, m_AnchorMin,
                 styles.anchorMinContent);

    EditorGUILayout.Space(EditorGUI.kVerticalSpacingMultiField);

    anchorRect.y += EditorGUIUtility.singleLineHeight +
                    EditorGUI.kVerticalSpacingMultiField;
    Vector2Field(anchorRect, rectTransform => rectTransform.anchorMax.x,
                 (rectTransform, val) => SetAnchorSmart(
                     rectTransform, val, 0, true, !m_RawEditMode, true),
                 rectTransform => rectTransform.anchorMax.y,
                 (rectTransform, val) => SetAnchorSmart(
                     rectTransform, val, 1, true, !m_RawEditMode, true),
                 DrivenTransformProperties.AnchorMaxX,
                 DrivenTransformProperties.AnchorMaxY, m_AnchorMax,
  ...
  void SmartPivotField() {
    Vector2Field(
        ...
        (rectTransform, val) =>
            SetPivotSmart(rectTransform, val, 0, !m_RawEditMode, false),
        rectTransform => rectTransform.pivot.y,
        (rectTransform, val) =>
            SetPivotSmart(rectTransform, val, 1, !m_RawEditMode, false),
        DrivenTransformProperties.PivotX, DrivenTransformProperties.PivotY,
        m_Pivot, styles.pivotContent);
  }

  void RawButton(Rect position) {
    EditorGUI.BeginChangeCheck();
    m_RawEditMode = GUI.Toggle(position, m_RawEditMode, styles.rawEditContent,
                               "ButtonRight");
    if (EditorGUI.EndChangeCheck())
  ...
  void BlueprintButton(Rect position) {
    EditorGUI.BeginChangeCheck();
    bool newValue = GUI.Toggle(position, Tools.rectBlueprintMode,
                               styles.blueprintContent, "ButtonLeft");
    if (EditorGUI.EndChangeCheck()) {
  ...
  void FloatFieldLabelAbove(Rect position, FloatGetter getter,
                            FloatSetter setter,
                            DrivenTransformProperties driven,
                            GUIContent label) {
    using(new EditorGUI.DisabledScope(targets.Any(
        x =>((x as RectTransform).drivenProperties & driven) != 0))) {
      float value = getter(target as RectTransform);
      EditorGUI.showMixedValue =
          targets.Select(x => getter(x as RectTransform)).Distinct().Count() >=
          2;

      EditorGUI.BeginChangeCheck();

      int id = GUIUtility.GetControlID(s_FloatFieldHash, FocusType.Keyboard,
                                       position);
      Rect positionLabel = new Rect(position.x, position.y, position.width,
      ...
      float newValue = EditorGUI.DoFloatField(
          EditorGUI.s_RecycledEditor, positionField, positionLabel, id, value,
          EditorGUI.kFloatFieldFormatString, EditorStyles.textField, true);
      if (EditorGUI.EndChangeCheck()) {
  ...
  void Vector2Field(Rect position, FloatGetter xGetter, FloatSetter xSetter,
                    FloatGetter yGetter, FloatSetter ySetter,
                    DrivenTransformProperties xDriven,
                    DrivenTransformProperties yDriven,
  ...
  void FloatField(Rect position, FloatGetter getter, FloatSetter setter,
                  DrivenTransformProperties driven, GUIContent label) {
    using(new EditorGUI.DisabledScope(targets.Any(
        x =>((x as RectTransform).drivenProperties & driven) != 0))) {
      float value = getter(target as RectTransform);
      EditorGUI.showMixedValue =
  ...
  void DrawRect(Rect rect, Transform space, bool dotted) {
    ...
    if (!dotted) {
      ...
      RectHandles.DrawDottedLineWithShadow(kShadowColor, kShadowOffset, p0, p1,
                                           kDottedLineSize);
      RectHandles.DrawDottedLineWithShadow(kShadowColor, kShadowOffset, p1, p2,
                                           kDottedLineSize);
      RectHandles.DrawDottedLineWithShadow(kShadowColor, kShadowOffset, p2, p3,
                                           kDottedLineSize);
      RectHandles.DrawDottedLineWithShadow(kShadowColor, kShadowOffset, p3, p0,
                                           kDottedLineSize);
    }
  }
  ...
  void OnSceneGUI() {
    ...
    // Also show when moving or resizing rect if anchors are scaling on either
    // axis
    if (gui.anchorMin != gui.anchorMax) {
      alpha = Mathf.Max(alpha, m_ChangingPosX.faded, m_ChangingPosY.faded,
                        m_ChangingLeft.faded, m_ChangingRight.faded,
                        m_ChangingTop.faded, m_ChangingBottom.faded);
    }
    ...
    if (m_TargetCount == 1) {
      ...
      DrawSizes(rectInUserSpace, userSpace, rectInParentSpace, parentSpace, gui,
                guiParent);
    ...
    for (int xHandle = 0; xHandle <= 2; xHandle++) {
      for (int yHandle = 0; yHandle <= 2; yHandle++) {
        ...
        for (int axis = 0; axis < 2; axis++)
          curPos[axis] = Mathf.Lerp(rect.min[axis], rect.max[axis],
                                    (axis == 0 ? xHandle : yHandle) * 0.5f);
        curPos = space.TransformPoint(curPos);

        int id = GUIUtility.GetControlID(s_ParentRectPreviewHandlesHash,
                                         FocusType.Passive);

        Vector3 sideDir =
            (xHandle == 1 ? space.right * rect.width : space.up * rect.height);
        ...
        EditorGUI.BeginChangeCheck();
        Vector3 newPos = RectHandles.SideSlider(id, curPos, sideDir, slideDir,
                                                size, null, Vector2.zero, -3);
        if (EditorGUI.EndChangeCheck()) {
        ...
        if (EditorGUIUtility.hotControl == id) {
          Handles.BeginGUI();
          EditorGUI.DropShadowLabel(new Rect(Event.current.mousePosition.x,
                                             Event.current.mousePosition.y, 60,
                                             16),
                                    "Preview");
          Handles.EndGUI();
        }
  ...
  void UpdateTemporaryRect() {
    ...
    if (lerp > 0) {
      Rect r = new Rect();
      r.position = Vector2.Lerp(s_ParentDragOrigRect.position,
                                s_ParentDragPreviewRect.position, lerp);
      r.size = Vector2.Lerp(s_ParentDragOrigRect.size,
                            s_ParentDragPreviewRect.size, lerp);
      InternalEditorUtility.SetRectTransformTemporaryRect(
          s_ParentDragRectTransform, r);
    } else {
      InternalEditorUtility.SetRectTransformTemporaryRect(
          s_ParentDragRectTransform, new Rect());
      EditorApplication.update -= UpdateTemporaryRect;
  ...
  void AllAnchorsSceneGUI(RectTransform gui, RectTransform guiParent,
                          Transform parentSpace, Transform transform) {
  ...
  static Vector2 NormalizedToPointUnclamped(Rect rectangle,
                                            Vector2 normalizedRectCoordinates) {
    return new Vector2(Mathf.LerpUnclamped(rectangle.x, rectangle.xMax,
                                           normalizedRectCoordinates.x),
                       Mathf.LerpUnclamped(rectangle.y, rectangle.yMax,
                                           normalizedRectCoordinates.y));
  }

  static bool AnchorAllowedOutsideParent(int axis, int minmax) {
    // Allow dragging outside if action key is held down (same key that disables
    // snapping). Also allow when not dragging at all - for e.g. typing values
    // into the Inspector.
    if (EditorGUI.actionKey || EditorGUIUtility.hotControl == 0)
      return true;
    // Also allow if drag started outside of range to begin with.
    float value =
        (minmax == 0 ? s_StartDragAnchorMin[axis] : s_StartDragAnchorMax[axis]);
    return (value < -0.001f || value > 1.001f);
  }

  void AnchorsSceneGUI(RectTransform gui, RectTransform guiParent,
                       Transform parentSpace, bool interactive) {
    ...
    AnchorSceneGUI(gui, guiParent, parentSpace, interactive, 0, 0,
                   GUIUtility.GetControlID(FocusType.Passive));
    AnchorSceneGUI(gui, guiParent, parentSpace, interactive, 0, 1,
                   GUIUtility.GetControlID(FocusType.Passive));
    AnchorSceneGUI(gui, guiParent, parentSpace, interactive, 1, 0,
                   GUIUtility.GetControlID(FocusType.Passive));
    AnchorSceneGUI(gui, guiParent, parentSpace, interactive, 1, 1,
                   GUIUtility.GetControlID(FocusType.Passive));
    ...
    // These are not drawn, so don't need to be done for non-interactive
    // handles.
  ...
  void AnchorSceneGUI(RectTransform gui, RectTransform guiParent,
                      Transform parentSpace, bool interactive, int minmaxX,
                      int minmaxY, int id) {
    ...
    if (minmaxX < 2 && minmaxY < 2)
      DrawAnchor(curPos, parentSpace.right * size * 2 * (minmaxX * 2 - 1),
                 parentSpace.up * size * 2 * (minmaxY * 2 - 1));
    ...
    EditorGUI.BeginChangeCheck();
    Vector3 newPos = Handles.Slider2D(id, curPos, parentSpace.forward,
                                      parentSpace.right, parentSpace.up, size,
                                      (Handles.CapFunction) null, Vector2.zero);
    ...
    if (evtCopy.type == EventType.MouseDown && GUIUtility.hotControl == id) {
      ...
      RectTransformSnapping.CalculateAnchorSnapValues(
          parentSpace, gui.transform, gui, minmaxX, minmaxY);
    }
    ...
    if (EditorGUI.EndChangeCheck()) {
      ...
      for (int axis = 0; axis <= 1; axis++) {
        ...
        float snapSize = HandleUtility.GetHandleSize(newPos) *
                         RectTransformSnapping.kSnapThreshold /
                         guiParent.rect.size[axis];
        snapSize *= parentSpace.InverseTransformVector(
            axis == 0 ? Vector3.right : Vector3.up)[axis];
        snappedValue =
            RectTransformSnapping.SnapToGuides(snappedValue, snapSize, axis);

        bool snap = snappedValue != newValue;
        newValue = snappedValue;

        if (minmaxForAxis == 2) {
          SetAnchorSmart(gui, newValue, axis, false, !evtCopy.shift, snap,
                         false, s_DragAnchorsTogether);
          SetAnchorSmart(gui, newValue, axis, true, !evtCopy.shift, snap, false,
                         s_DragAnchorsTogether);
        } else {
          SetAnchorSmart(gui, newValue, axis, isMax, !evtCopy.shift, snap, true,
                         s_DragAnchorsTogether);
        }
  ...
  void DrawSizes(Rect rectInUserSpace, Transform userSpace,
                 Rect rectInParentSpace, Transform parentSpace,
                 RectTransform gui, RectTransform guiParent) {
    ...
    alpha = Mathf.Max(m_ChangingPosX.faded, m_ChangingLeft.faded,
                      m_ChangingRight.faded, m_ChangingAnchors.faded);
    DrawAnchorRect(parentSpace, gui, guiParent, 0, alpha);

    alpha = Mathf.Max(m_ChangingPosY.faded, m_ChangingTop.faded,
                      m_ChangingBottom.faded, m_ChangingAnchors.faded);
    DrawAnchorRect(parentSpace, gui, guiParent, 1, alpha);

    DrawAnchorDistances(parentSpace, gui, guiParent, size,
                        m_ChangingAnchors.faded);

    if (stretchW) {
      DrawPositionDistances(userSpace, rectInParentSpace, parentSpace, gui,
                            guiParent, size, 0, 1, m_ChangingLeft.faded);
      DrawPositionDistances(userSpace, rectInParentSpace, parentSpace, gui,
                            guiParent, size, 0, 2, m_ChangingRight.faded);
    } else {
      DrawPositionDistances(userSpace, rectInParentSpace, parentSpace, gui,
                            guiParent, size, 0, 0, m_ChangingPosX.faded);
      DrawSizeDistances(userSpace, rectInParentSpace, parentSpace, gui,
                        guiParent, size, 0, m_ChangingWidth.faded);
    }

    if (stretchH) {
      DrawPositionDistances(userSpace, rectInParentSpace, parentSpace, gui,
                            guiParent, size, 1, 1, m_ChangingBottom.faded);
      DrawPositionDistances(userSpace, rectInParentSpace, parentSpace, gui,
                            guiParent, size, 1, 2, m_ChangingTop.faded);
    } else {
      DrawPositionDistances(userSpace, rectInParentSpace, parentSpace, gui,
                            guiParent, size, 1, 0, m_ChangingPosY.faded);
      DrawSizeDistances(userSpace, rectInParentSpace, parentSpace, gui,
                        guiParent, size, 1, m_ChangingHeight.faded);
    }
  }

  void DrawSizeDistances(Transform userSpace, Rect rectInParentSpace,
                         Transform parentSpace, RectTransform gui,
                         RectTransform guiParent, float size, int axis,
                         float alpha) {
    if (alpha <= 0)
      return;
    ...
    DrawLabelBetweenPoints(s_Corners[0] + dir,
                           s_Corners[axis == 0 ? 3 : 1] + dir, label);
  }

  void DrawPositionDistances(Transform userSpace, Rect rectInParentSpace,
                             Transform parentSpace, RectTransform gui,
                             RectTransform guiParent, float size, int axis,
                             int side, float alpha) {
    ...
    if (side == 0) {
      ...
      posB = pivot;
      posA[axis] =
          Mathf.LerpUnclamped(guiParent.rect.min[axis],
                              guiParent.rect.max[axis], gui.anchorMin[axis]);
      ...
      if (side == 1) {
        ...
        posA[axis] =
            Mathf.LerpUnclamped(guiParent.rect.min[axis],
                                guiParent.rect.max[axis], gui.anchorMax[axis]);
    ...
    RectHandles.DrawDottedLineWithShadow(kShadowColor, kShadowOffset, posA,
                                         posB, kDottedLineSize);
    GUIContent label = new GUIContent(
        value.ToString(CultureInfo.InvariantCulture.NumberFormat));
    DrawLabelBetweenPoints(posA, posB, label);
  }

  void DrawAnchorDistances(Transform parentSpace, RectTransform gui,
                           RectTransform guiParent, float size, float alpha) {
    if (guiParent == null || alpha <= 0)
      return;
    ...
    for (int axis = 0; axis < 2; axis++) {
      ...
      int midValue =
          RoundToInt((gui.anchorMax[1 - axis] - gui.anchorMin[1 - axis]) * 100);
      int endValue = RoundToInt((1 - gui.anchorMax[1 - axis]) * 100);
      if (startValue > 0)
        DrawLabelBetweenPoints(points[axis, 0] - dir, points[axis, 1] - dir,
                               GUIContent.Temp(startValue + "%"));
      if (midValue > 0)
        DrawLabelBetweenPoints(points[axis, 1] - dir, points[axis, 2] - dir,
                               GUIContent.Temp(midValue + "%"));
      if (endValue > 0)
        DrawLabelBetweenPoints(points[axis, 2] - dir, points[axis, 3] - dir,
                               GUIContent.Temp(endValue + "%"));
    }
  }

  void DrawAnchorRect(Transform parentSpace, RectTransform gui,
                      RectTransform guiParent, int axis, float alpha) {
    if (guiParent == null || alpha <= 0)
      return;
    ...
    for (int side = 0; side < 2; side++) {
      ...
      points [side, 0]
      [1 - axis] = Mathf.Min(0, gui.anchorMin[1 - axis]);
      points [side, 1]
      [1 - axis] = Mathf.Max(1, gui.anchorMax[1 - axis]);
      for (int endpoint = 0; endpoint < 2; endpoint++) {
        points [side, endpoint]
        [axis] = (side == 0 ? gui.anchorMin[axis] : gui.anchorMax[axis]);
        points[side, endpoint] = parentSpace.TransformPoint(
            GetAnchorLocal(guiParent, points[side, endpoint]));
      }
      RectHandles.DrawDottedLineWithShadow(kShadowColor, kShadowOffset,
                                           points[side, 0], points[side, 1],
                                           kDottedLineSize);
    }
  }
  ...
  void DrawLabelBetweenPoints(Vector3 pA, Vector3 pB, GUIContent label) {
    ...
    EditorGUI.DropShadowLabel(new Rect(center.x - 50, center.y - 9, 100, 16),
                              label, style);
  ...
  void DrawAnchor(Vector3 pos, Vector3 right, Vector3 up) {
    ...
    up *= 1.4f;
    right *= 1.4f;
    RectHandles.DrawPolyLineWithShadow(kShadowColor, kShadowOffset, pos,
                                       pos + up + right * 0.5f,
                                       pos + right + up * 0.5f, pos);
  }

  public static void SetPivotSmart(RectTransform rect, float value, int axis,
                                   bool smart, bool parentSpace) {
  ...
  public static void SetAnchorSmart(RectTransform rect, float value, int axis,
                                    bool isMax, bool smart) {
  ...
  public static void SetAnchorSmart(RectTransform rect, float value, int axis,
    ...
    SetAnchorSmart(rect, value, axis, isMax, smart, enforceExactValue, false,
                   false);
  }
  ...
  public static void SetAnchorSmart(RectTransform rect, float value, int axis,
    ...
    if (smart) {
      ...
      // Note: In this particular instance we want to use Mathf.Round (which
      // rounds towards nearest even number) instead of Round from this class
      // which always rounds down. This makes the position of rect more stable
      // when their anchors are changed.
      ...
      if (moveTogether)
        ...
        offsetPositionPixels = (isMax ? offsetSizePixels * rect.pivot[axis]
                                : (offsetSizePixels * (1 - rect.pivot[axis])));
    }
    ...
    if (isMax) {
      ...
      if (moveTogether)
        other[axis] = s_StartDragAnchorMin[axis] + rectAnchorMax[axis] -
                      s_StartDragAnchorMax[axis];
      ...
      if (moveTogether)
        other[axis] = s_StartDragAnchorMax[axis] + rectAnchorMin[axis] -
                      s_StartDragAnchorMin[axis];
