...
public string extension {
	get { return m_SaveLoadHelper.fileExtensionWithoutDot; }
}
...
public GenericPresetLibraryInspector(UnityEngine.Object target, string header,
                                     Action<string> editButtonClicked) {
	m_Target = target;
	m_Header = header;
	...
	m_SaveLoadHelper =
		new ScriptableObjectSaveLoadHelper<T>(extension, SaveType.Text);
	m_Grid = new VerticalGrid();

	// Default configuration
	maxShowNumPresets =
		49; // We clear some preview caches when they reach 50 (See
	            // AnimationCurvePreviewCache and GradientPreviewCache)
...
public void OnInspectorGUI() {
	...
	if (isInAnEditorFolder && m_EditButtonClickedCallback != null &&
	    GUILayout.Button("Edit...", s_EditButtonStyle)) {
	...
	if (!isInAnEditorFolder) {
		GUIContent c = EditorGUIUtility.TrTextContent(
			"Preset libraries should be placed in an 'Editor' folder.",
			EditorGUIUtility.warningIcon);
...
private void DrawPresets(string libraryPath) {
	...
	PresetLibrary lib = PresetLibraryManager.instance.GetLibrary(
		m_SaveLoadHelper, libraryPath) as PresetLibrary;
	if (lib == null) {
	...
	float contentHeight = m_Grid.CalcRect(showNumPresets - 1, 0f).yMax +
	                      (hiddenNumPresets > 0 ? 20f : 0f);
	...
	for (int index = 0; index < showNumPresets; ++index) {
		...
		if (itemViewMode == PresetLibraryEditorState.ItemViewMode.List) {
			Rect nameRect = new Rect(r.x + spaceBetweenPresetAndText, r.y,
			                         r.width - spaceBetweenPresetAndText, r.height);
			string name = lib.GetName(index);
			GUI.Label(nameRect, name);
		}
	}
	if (hiddenNumPresets > 0) {
		Rect textRect = new Rect(m_Grid.CalcRect(0, 0).x,
		                         reservedRect.y + contentHeight - 20f,
		                         reservedRect.width, 20f);
		GUI.Label(textRect, string.Format("+ {0} more...", hiddenNumPresets));
	}
}

void SetupGrid(float availableWidth, int itemCount,
               PresetLibraryEditorState.ItemViewMode presetsViewMode) {
	...
	case PresetLibraryEditorState.ItemViewMode.List:
		...
		m_Grid.itemSize =
			new Vector2(availableWidth - m_Grid.leftMargin, presetSize.y);
		m_Grid.InitNumRowsAndColumns(itemCount, int.MaxValue);
		break;
	}
}
