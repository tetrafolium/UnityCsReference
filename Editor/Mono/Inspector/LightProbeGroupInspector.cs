...
internal class LightProbeGroupEditor : IEditablePoint {
  ...
  public SavedBool drawTetrahedra {
    get;
    set;
  }
  public bool deringProbes {
    get { return m_Group.dering; }
    set { m_Group.dering = value; }
  }
  ...
  public LightProbeGroupEditor(LightProbeGroup group) {
    ...
    m_SerializedSelectedProbes =
        ScriptableObject.CreateInstance<LightProbeGroupSelection>();
    m_SerializedSelectedProbes.hideFlags = HideFlags.HideAndDontSave;
  }

  public void SetEditing(bool editing) { m_Editing = editing; }

  public void AddProbe(Vector3 position) {
    Undo.RegisterCompleteObjectUndo(
        new Object[]{m_Group, m_SerializedSelectedProbes}, "Add Probe");
  ...
  public void DuplicateSelectedProbes() {
    ...
    Undo.RegisterCompleteObjectUndo(
        new Object[]{m_Group, m_SerializedSelectedProbes}, "Duplicate Probes");
  ...
  private void CopySelectedProbes() {
    ...
    serializer.Serialize(
        writer,
        localPositions.Select(pos => m_Group.transform.TransformPoint(pos))
            .ToArray());
    writer.Close();
  ...
  private bool PasteProbes() {
    ...
    try {
      ...
      if (pastedProbes.Length == 0)
        return false;

      Undo.RegisterCompleteObjectUndo(
          new Object[]{m_Group, m_SerializedSelectedProbes}, "Paste Probes");

      var oldLength = m_SourcePositions.Count;

      // Need to convert into local space...
      foreach (var position in pastedProbes) {
        m_SourcePositions.Add(
            m_Group.transform.InverseTransformPoint(position));
      }
  ...
  public void RemoveSelectedProbes() {
    ...
    Undo.RegisterCompleteObjectUndo(
        new Object[]{m_Group, m_SerializedSelectedProbes}, "Delete Probes");
  ...
  private void DrawTetrahedra() {
    ...
    if (SceneView.lastActiveSceneView) {
      LightProbeVisualization.DrawTetrahedra(
          m_ShouldRecalculateTetrahedra,
  ...
  public static void TetrahedralizeSceneProbes(out Vector3[] positions,
                                               out int[] indices) {
    var probeGroups =
        Object.FindObjectsOfType(typeof(LightProbeGroup)) as LightProbeGroup[];
    ...
    Lightmapping.Tetrahedralize(probePositions.ToArray(), out indices,
                                out positions);
  }
  ...
  public bool OnSceneGUI(Transform transform) {
    ...
    if (m_Editing) {
      if (PointEditor.SelectPoints(this, transform, ref m_Selection)) {
        Undo.RegisterCompleteObjectUndo(
            new Object[]{m_Group, m_SerializedSelectedProbes}, "Select Probes");
      }
    }

    // Special handling for paste (want to be able to paste when not in edit
    // mode!)
    ...
    if (m_Editing && PointEditor.MovePoints(this, transform, m_Selection)) {
      Undo.RegisterCompleteObjectUndo(
          new Object[]{m_Group, m_SerializedSelectedProbes}, "Move Probes");
      if (LightProbeVisualization.dynamicUpdateLightProbes)
        MarkSourcePositionsDirty();
    }

    if (m_Editing && mouseUpEvent &&
        !LightProbeVisualization.dynamicUpdateLightProbes) {
      MarkSourcePositionsDirty();
    }
  ...
  private Bounds GetBounds(List<Vector3> positions) {
    ...
    if (positions.Count == 1)
      return new Bounds(m_Group.transform.TransformPoint(positions[0]),
                        new Vector3(1f, 1f, 1f));

    return GeometryUtility.CalculateBounds(
        positions.ToArray(), m_Group.transform.localToWorldMatrix);
  }

  /// Get the world-space position of a specific point
  public Vector3 GetPosition(int idx) { return m_SourcePositions[idx]; }
  ...
  private static readonly Color kCloudColor =
      new Color(200f / 255f, 200f / 255f, 20f / 255f, 0.85f);
  private static readonly Color kSelectedCloudColor = new Color(.3f, .6f, 1, 1);

  public Color GetDefaultColor() { return kCloudColor; }

  public Color GetSelectedColor() { return kSelectedCloudColor; }
  ...
  public IEnumerable<Vector3> GetPositions() { return m_SourcePositions; }
  ...
  public int Count {
    get { return m_SourcePositions.Count; }
  }
...
class LightProbeGroupInspector : Editor {
  private static class Styles {
    public static readonly GUIContent showWireframe = EditorGUIUtility.TrTextContent(
        "Show Wireframe",
        "Show the tetrahedron wireframe visualizing the blending between probes.");
    public static readonly GUIContent selectedProbePosition =
        EditorGUIUtility.TrTextContent(
            "Selected Probe Position",
            "The local position of this probe relative to the parent group.");
    public static readonly GUIContent addProbe =
        EditorGUIUtility.TrTextContent("Add Probe");
    public static readonly GUIContent deleteSelected =
        EditorGUIUtility.TrTextContent("Delete Selected");
    ...
    public static readonly GUIContent duplicateSelected =
        EditorGUIUtility.TrTextContent("Duplicate Selected");
    public static readonly GUIContent performDeringing =
        EditorGUIUtility.TrTextContent(
            "Remove Ringing",
  ...
  public void OnEnable() {
    ...
    m_Editor.drawTetrahedra =
        new SavedBool($"{target.GetType()}.drawTetrahedra", true);
  ...
  private void OnEditModeStarted(IToolModeOwner owner,
                                 EditMode.SceneViewEditMode mode) {
    if (owner == (IToolModeOwner) this &&
        mode == EditMode.SceneViewEditMode.LightProbeGroup) {
      StartEditProbes();
    }
  }

  public void StartEditMode() {
    EditMode.ChangeEditMode(EditMode.SceneViewEditMode.LightProbeGroup,
                            m_Editor.bounds, this);
  }
  ...
  private void UndoRedoPerformed() {
    // Update the cached probe positions from the ones just restored in the
    // LightProbeGroup
    m_Editor.PullProbePositions();
  ...
  public override void OnInspectorGUI() {
    ...
    EditMode.DoEditModeInspectorModeButton(
        EditMode.SceneViewEditMode.LightProbeGroup, "Edit Light Probes",
        Styles.editModeButton, this);

    GUILayout.Space(3);
    EditorGUI.BeginDisabledGroup(EditMode.editMode !=
                                 EditMode.SceneViewEditMode.LightProbeGroup);

    m_Editor.drawTetrahedra.value = EditorGUILayout.Toggle(
        Styles.showWireframe, m_Editor.drawTetrahedra.value);
    m_Editor.deringProbes =
        EditorGUILayout.Toggle(Styles.performDeringing, m_Editor.deringProbes);

    EditorGUI.BeginDisabledGroup(m_Editor.SelectedCount == 0);

    EditorGUI.BeginChangeCheck();
    Vector3 pos = m_Editor.SelectedCount > 0
                      ? m_Editor.GetSelectedPositions()[0]
                      : Vector3.zero;
    Vector3 newPosition =
        EditorGUILayout.Vector3Field(Styles.selectedProbePosition, pos);
  ...
  public bool HasFrameBounds() { return m_Editor.SelectedCount > 0; }

  public Bounds OnGetFrameBounds() { return m_Editor.selectedProbeBounds; }
}
}
