...
public enum BlendMode {
	Opaque,
	Cutout,
	Fade, // Old school alpha-blending mode, fresnel does not affect amount of
	      // transparency
	Transparent, // Physically plausible transparency mode, implemented as alpha
	             // pre-multiply
	Additive,
	Subtractive,
	Modulate
...
public enum FlipbookMode { Simple, Blended }
...
public static GUIContent smoothnessText =
	EditorGUIUtility.TrTextContent("Smoothness", "Smoothness value.");
public static GUIContent smoothnessScaleText =
	EditorGUIUtility.TrTextContent("Smoothness",
	                               "Smoothness scale factor.");
public static GUIContent normalMapText =
	EditorGUIUtility.TrTextContent("Normal Map", "Normal Map.");
public static GUIContent emissionText =
	EditorGUIUtility.TrTextContent("Color", "Emission (RGB).");

public static GUIContent renderingMode = EditorGUIUtility.TrTextContent(
	"Rendering Mode",
	"Determines the transparency and blending method for drawing the object to the screen.");
public static GUIContent[] blendNames = Array.ConvertAll(
	Enum.GetNames(typeof(BlendMode)), item => new GUIContent(item));

public static GUIContent colorMode = EditorGUIUtility.TrTextContent(
	"Color Mode",
	"Determines the blending mode between the particle color and the texture albedo.");
public static GUIContent[] colorNames = Array.ConvertAll(
	Enum.GetNames(typeof(ColorMode)), item => new GUIContent(item));

public static GUIContent flipbookBlending = EditorGUIUtility.TrTextContent(
	"Flip-Book Frame Blending",
	"Enables blending between the frames of animated texture sheets.");
public static GUIContent twoSidedEnabled = EditorGUIUtility.TrTextContent(
	"Two Sided",
	"Render both front and back faces of the particle geometry.");

public static GUIContent distortionEnabled = EditorGUIUtility.TrTextContent(
	"Distortion", "Use a grab pass and normal map to simulate refraction.");
public static GUIContent distortionStrengthText =
	EditorGUIUtility.TrTextContent("Strength", "Distortion Strength.");
public static GUIContent distortionBlendText =
	EditorGUIUtility.TrTextContent(
		"Blend", "Weighting between albedo and grab pass.");

public static GUIContent softParticlesEnabled = EditorGUIUtility.TrTextContent(
	"Soft Particles",
...
public static GUIContent softParticlesNearFadeDistanceText =
	EditorGUIUtility.TrTextContent("Near fade",
	                               "Soft Particles near fade distance.");
public static GUIContent softParticlesFarFadeDistanceText =
	EditorGUIUtility.TrTextContent("Far fade",
	                               "Soft Particles far fade distance.");
...
public static GUIContent cameraNearFadeDistanceText =
	EditorGUIUtility.TrTextContent("Near fade",
	                               "Camera near fade distance.");
public static GUIContent cameraFarFadeDistanceText =
	EditorGUIUtility.TrTextContent("Far fade", "Camera far fade distance.");

public static GUIContent emissionEnabled =
	EditorGUIUtility.TrTextContent("Emission");

public static GUIContent blendingOptionsText =
	EditorGUIUtility.TrTextContent("Blending Options");
public static GUIContent mainOptionsText =
	EditorGUIUtility.TrTextContent("Main Options");
public static GUIContent mapsOptionsText =
	EditorGUIUtility.TrTextContent("Maps");
public static GUIContent requiredVertexStreamsText =
	EditorGUIUtility.TrTextContent("Required Vertex Streams");

public static GUIContent streamPositionText =
	EditorGUIUtility.TrTextContent("Position (POSITION.xyz)");
public static GUIContent streamNormalText =
	EditorGUIUtility.TrTextContent("Normal (NORMAL.xyz)");
public static GUIContent streamColorText =
	EditorGUIUtility.TrTextContent("Color (COLOR.xyzw)");
public static GUIContent streamColorInstancedText =
	EditorGUIUtility.TrTextContent("Color (INSTANCED0.xyzw)");
public static GUIContent streamUVText =
	EditorGUIUtility.TrTextContent("UV (TEXCOORD0.xy)");
public static GUIContent streamUV2Text =
	EditorGUIUtility.TrTextContent("UV2 (TEXCOORD0.zw)");
public static GUIContent streamAnimBlendText =
	EditorGUIUtility.TrTextContent("AnimBlend (TEXCOORD1.x)");
public static GUIContent streamAnimFrameText =
	EditorGUIUtility.TrTextContent("AnimFrame (INSTANCED1.x)");
public static GUIContent streamTangentText =
	EditorGUIUtility.TrTextContent("Tangent (TANGENT.xyzw)");
...
public static string undoApplyCustomVertexStreams =
	L10n.Tr("Apply custom vertex streams from material");
}
...
List<ParticleSystemRenderer> m_RenderersUsingThisMaterial =
...
public void FindProperties(MaterialProperty[] props) {
	...
	softParticlesNearFadeDistance =
		FindProperty("_SoftParticlesNearFadeDistance", props);
	softParticlesFarFadeDistance =
		FindProperty("_SoftParticlesFarFadeDistance", props);
...
public override void OnGUI(MaterialEditor materialEditor,
	...
	FindProperties(props); // MaterialProperties can be animated so we do not
	                       // cache them but fetch them every event to ensure
	...
	// switching some existing material to a standard shader. Do this before any
	// GUI code has been issued to prevent layout issues in subsequent GUILayout
	// statements (case 780071)
	if (m_FirstTimeApply) {
...
public void ShaderPropertiesGUI(Material material) {
	...
	EditorGUI.BeginChangeCheck();
	{
		...
		if (!flipbookMode.hasMixedValue &&
		    (FlipbookMode) flipbookMode.floatValue != FlipbookMode.Blended) {
			EditorGUI.BeginChangeCheck();
			m_MaterialEditor.TextureScaleOffsetProperty(albedoMap);
			if (EditorGUI.EndChangeCheck())
				emissionMap.textureScaleAndOffset =
					albedoMap
					.textureScaleAndOffset; // Apply the main texture scale and
			                                        // offset to the emission texture as
			                                        // well, for Enlighten's sake
		}
	}
	if (EditorGUI.EndChangeCheck()) {
...
public override void AssignNewShaderToMaterial(Material material,
                                               Shader oldShader,
                                               Shader newShader) {
	...
	// _Emission property is lost after assigning Standard shader to the
	// material thus transfer it before assigning the new shader
...
void BlendModePopup() {
	...
	EditorGUI.BeginChangeCheck();
	mode = (BlendMode) EditorGUILayout.Popup(Styles.renderingMode, (int) mode,
	                                         Styles.blendNames);
	if (EditorGUI.EndChangeCheck()) {
...
void ColorModePopup() {
	if (colorMode != null) {
		...
		EditorGUI.BeginChangeCheck();
		mode = (ColorMode) EditorGUILayout.Popup(Styles.colorMode, (int) mode,
		                                         Styles.colorNames);
		if (EditorGUI.EndChangeCheck()) {
...
void FlipbookBlendingPopup() {
	...
	if (EditorGUI.EndChangeCheck()) {
		...
		flipbookMode.floatValue =
			enabled ? (float) FlipbookMode.Blended : (float) FlipbookMode.Simple;
	}
...
void FadingPopup(Material material) {
	...
	if (!hasZWrite) {
		// Soft Particles
		{
			...
			EditorGUI.BeginChangeCheck();
			enabled =
				EditorGUILayout.Toggle(Styles.softParticlesEnabled, enabled != 0.0f)
		? 1.0f
		: 0.0f;
			if (EditorGUI.EndChangeCheck()) {
		...
		// Camera Fading
		{
			...
			EditorGUI.BeginChangeCheck();
			enabled =
				EditorGUILayout.Toggle(Styles.cameraFadingEnabled, enabled != 0.0f)
		? 1.0f
		: 0.0f;
			if (EditorGUI.EndChangeCheck()) {
			...
			if (enabled != 0.0f) {
				int indentation = 2;
				m_MaterialEditor.ShaderProperty(cameraNearFadeDistance,
				                                Styles.cameraNearFadeDistanceText,
				                                indentation);
				m_MaterialEditor.ShaderProperty(cameraFarFadeDistance,
...
void DistortionPopup(Material material) {
	...
	if (!hasZWrite) {
		...
		if (enabled) {
			int indentation = 2;
			m_MaterialEditor.ShaderProperty(
				distortionStrength, Styles.distortionStrengthText, indentation);
			m_MaterialEditor.ShaderProperty(
				distortionBlend, Styles.distortionBlendText, indentation);
		}
...
void DoAlbedoArea(Material material) {
	m_MaterialEditor.TexturePropertyWithHDRColor(Styles.albedoText, albedoMap,
	                                             albedoColor, true);
	if (((BlendMode) material.GetFloat("_Mode") == BlendMode.Cutout)) {
		m_MaterialEditor.ShaderProperty(
			alphaCutoff, Styles.alphaCutoffText,
...
void DoEmissionArea(Material material) {
	...
	if (enabled) {
		...
		m_MaterialEditor.TexturePropertyWithHDRColor(
			Styles.emissionText, emissionMap, emissionColorForRendering, false);

		// If texture was assigned and color was black set color to white
		float brightness = emissionColorForRendering.colorValue.maxColorComponent;
		if (emissionMap.textureValue != null && !hadEmissionTexture &&
		    brightness <= 0f)
...
void DoSpecularMetallicArea(Material material) {
	...
	if (useLighting) {
		...
		m_MaterialEditor.TexturePropertySingleLine(Styles.metallicMapText,
		                                           metallicMap, hasGlossMap ? null
		                                 : metallic);

		int indentation = 2; // align with labels of texture properties
		bool showSmoothnessScale = hasGlossMap;
		m_MaterialEditor.ShaderProperty(
			smoothness, showSmoothnessScale ? Styles.smoothnessScaleText
	  : Styles.smoothnessText, indentation);
	}
}
...
void DoNormalMapArea(Material material) {
	...
	bool useLighting = (material.GetFloat("_LightingEnabled") > 0.0f);
	bool useDistortion =
		(material.GetFloat("_DistortionEnabled") > 0.0f) && !hasZWrite;
	if (useLighting || useDistortion) {
		m_MaterialEditor.TexturePropertySingleLine(
			Styles.normalMapText, bumpMap, bumpMap.textureValue != null ? bumpScale
	  : null);
	}
}
...
void DoVertexStreamsArea(Material material) {
	...
	if (useGPUInstancing && m_RenderersUsingThisMaterial.Count > 0) {
		if (!m_RenderersUsingThisMaterial[0].enableGPUInstancing ||
		    m_RenderersUsingThisMaterial[0].renderMode !=
	...
	GUILayout.Label(useGPUInstancing ? Styles.streamColorInstancedText
	            : Styles.streamColorText, EditorStyles.label);
	...
	List<ParticleSystemVertexStream> instancedStreams =
		new List<ParticleSystemVertexStream>(streams);
	...
	if (GUILayout.Button(Styles.streamApplyToAllSystemsText,
	                     EditorStyles.miniButton,
	                     GUILayout.ExpandWidth(false))) {
		Undo.RecordObjects(
			m_RenderersUsingThisMaterial.Where(r => r != null).ToArray(),
			Styles.undoApplyCustomVertexStreams);

		foreach (ParticleSystemRenderer renderer in
		         m_RenderersUsingThisMaterial) {
			if (renderer != null) {
				if (useGPUInstancing &&
	...
	string Warnings = "";
	List<ParticleSystemVertexStream> rendererStreams =
		new List<ParticleSystemVertexStream>();
	...
	if (Warnings != "") {
		EditorGUILayout.HelpBox(
			...
			Warnings + "Use the Apply to Systems button to fix this",
...
public static void SetupMaterialWithBlendMode(Material material,
	...
	case BlendMode.Modulate:
		...
		material.SetInt("_BlendOp", (int) UnityEngine.Rendering.BlendOp.Add);
		material.SetInt("_SrcBlend",
		                (int) UnityEngine.Rendering.BlendMode.DstColor);
		material.SetInt("_DstBlend",
		                (int) UnityEngine.Rendering.BlendMode.OneMinusSrcAlpha);
		material.SetInt("_ZWrite", 0);
		material.DisableKeyword("_ALPHATEST_ON");
...
public static void SetupMaterialWithColorMode(Material material,
	...
	case ColorMode.Difference:
		...
		material.SetVector("_ColorAddSubDiff",
...
void SetMaterialKeywords(Material material) {
	...
	// due to multi-edit & material animation (MaterialProperty value might come
	// from renderer material property block)
	bool useDistortion =
		!hasZWrite && (material.GetFloat("_DistortionEnabled") > 0.0f);
	SetKeyword(material, "_NORMALMAP",
	           (useLighting || useDistortion) &&
	           material.GetTexture("_BumpMap"));
	SetKeyword(material, "_METALLICGLOSSMAP",
	           useLighting && material.GetTexture("_MetallicGlossMap"));

	material.globalIlluminationFlags = MaterialGlobalIlluminationFlags.None;
	SetKeyword(material, "_EMISSION",
	           material.GetFloat("_EmissionEnabled") > 0.0f);
	...
	float softParticlesNearFadeDistance =
		material.GetFloat("_SoftParticlesNearFadeDistance");
	float softParticlesFarFadeDistance =
		material.GetFloat("_SoftParticlesFarFadeDistance");
	...
	if (useSoftParticles)
		material.SetVector("_SoftParticleFadeParams",
		                   new Vector4(softParticlesNearFadeDistance,
		                               1.0f / (softParticlesFarFadeDistance -
		                                       softParticlesNearFadeDistance),
		                               0.0f, 0.0f));
	else
		material.SetVector("_SoftParticleFadeParams",
	...
	if (useCameraFading)
		material.SetVector(
			"_CameraFadeParams",
			new Vector4(cameraNearFadeDistance,
			            1.0f / (cameraFarFadeDistance - cameraNearFadeDistance),
			            0.0f, 0.0f));
	else
		material.SetVector("_CameraFadeParams",
		                   new Vector4(0.0f, Mathf.Infinity, 0.0f, 0.0f));
	...
	if (useDistortion)
		material.SetFloat("_DistortionStrengthScaled",
		                  ...
		                  0.1f); // more friendly number scale than 1 unit per
	                                 // size of the screen
}
...
void MaterialChanged(Material material) {
	...
	if (colorMode != null)
		SetupMaterialWithColorMode(material,
		                           (ColorMode) material.GetFloat("_ColorMode"));
...
void CacheRenderersUsingThisMaterial(Material material) {
	...
	ParticleSystemRenderer[] renderers = Resources.FindObjectsOfTypeAll(
		typeof(ParticleSystemRenderer)) as ParticleSystemRenderer[];
