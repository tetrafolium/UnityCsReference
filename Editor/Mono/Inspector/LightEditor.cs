...
public void DrawInnerAndOuterSpotAngle() {
	...
	EditorGUI.BeginProperty(new Rect(rect){
					width = rect.width * 0.5f
				},
	                        Styles.InnerOuterSpotAngle, innerSpotAngle);
	EditorGUI.BeginProperty(new Rect(rect){
					xMin = rect.x + rect.width * 0.5f
				},
	...
	var maxRect = new Rect(rect){
		xMin = rect.xMax - textFieldWidth
	};
	var sliderRect = new Rect(rect){
		xMin = minRect.xMax + EditorGUI.kSpacing,
