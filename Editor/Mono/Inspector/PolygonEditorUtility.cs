...
internal class PolygonEditorUtility {
  ...
  public void OnSceneGUI() {
    ...
    // Handles.Slider2D will render active point as yellow if there is
    // keyboardControl set. We don't want that happening.
    GUIUtility.keyboardControl = 0;

    HandleUtility.s_CustomPickDistance = k_HandlePickDistance;

    // Find mouse positions in local and world space
    Plane plane = new Plane(-transform.forward,
    ...
    if (evt.type == EventType.MouseMove || m_FirstOnSceneGUIAfterReset) {
      ...
      float distance;
      if (PolygonEditor.GetNearestPoint(mouseLocalPos - colliderOffset,
                                        out pathIndex, out pointIndex0,
                                        out distance)) {
      ...
      if (PolygonEditor.GetNearestEdge(
              mouseLocalPos - colliderOffset, out pathIndex, out pointIndex0,
              out pointIndex1, out distance, m_LoopingCollider)) {
    ...
    // TODO: there probably isn't a case when selectedPath is valid and
    // selectedEdge is invalid. This needs a refactor.
        ...
        m_HandleEdge =
            (HandleUtility.WorldToGUIPoint(worldPos) -
             Event.current.mousePosition)
                .sqrMagnitude > k_HandlePointSnap * k_HandlePointSnap;
    ...
    // If this is a polygon collider being used by a composite then draw the
    // polygon paths.
    if ((m_ActiveCollider is PolygonCollider2D) &&
        m_ActiveCollider.usedByComposite)
    ...
    if (m_HandleEdge && !m_DeleteMode) {
      ...
      Vector3 newPoint = GetNearestPointOnEdge(
          transform.TransformPoint(mouseLocalPos), worldPosV0, worldPosV1);
      ...
      newPoint = Handles.Slider2D(newPoint, new Vector3(0, 0, 1),
                                  new Vector3(1, 0, 0), new Vector3(0, 1, 0),
                                  guiSize, Handles.DotHandleCap, Vector3.zero);
      Handles.color = Color.white;
      if (EditorGUI.EndChangeCheck()) {
        PolygonEditor.InsertPoint(m_SelectedEdgePath, m_SelectedEdgeVertex1,
                                  ((p0 + p1) / 2) - colliderOffset);
    ...
    if (m_HandlePoint) {
      ...
      if (m_DeleteMode && evt.type == EventType.MouseDown &&
              Vector2.Distance(screenPos, Event.current.mousePosition) <
                  k_HandlePickDistance ||
      ...
      Vector3 newWorldPos = Handles.Slider2D(
          worldPos, new Vector3(0, 0, 1), new Vector3(1, 0, 0),
          new Vector3(0, 1, 0), guiSize, Handles.DotHandleCap, Vector3.zero);
      Handles.color = Color.white;
      if (EditorGUI.EndChangeCheck() && !m_DeleteMode) {
        point = transform.InverseTransformPoint(newWorldPos);
        point -= colliderOffset;
        PolygonEditor.TestPointMove(m_SelectedPath, m_SelectedVertex, point,
                                    out m_LeftIntersect, out m_RightIntersect,
                                    m_LoopingCollider);
      ...
      if (!applyToCollider)
        DrawEdgesForSelectedPoint(newWorldPos, transform, m_LeftIntersect,
                                  m_RightIntersect, m_LoopingCollider);
    }
    ...
    if (DeleteCommandEvent(evt))
      Event.current.Use(); // If we don't use the delete event in all cases, it
                           // sceneview might delete the entire object

    m_FirstOnSceneGUIAfterReset = false;
  }

  private bool DeleteCommandEvent(Event evt) {
    return (evt.type == EventType.ExecuteCommand ||
  ...
  private void DrawEdgesForSelectedPoint(Vector3 worldPos, Transform transform,
                                         bool leftIntersect,
                                         bool rightIntersect, bool loop) {
    bool drawLeft = true;
    bool drawRight = true;

    int pathPointCount =
        UnityEditor.PolygonEditor.GetPointCount(m_SelectedPath);
