...
internal class ReflectionProbeEditor : Editor {
  ...
  static HashSet<ReflectionProbe> s_CurrentlyEditedProbes =
      new HashSet<ReflectionProbe>();
  ...
  public static void GetResolutionArray(ref int[] resolutionList,
                                        ref GUIContent[] resolutionStringList) {
    if (Styles.reflectionResolutionValuesArray == null &&
        Styles.reflectionResolutionTextArray == null) {
      int cubemapResolution =
          Mathf.Max(1, ReflectionProbe.minBakedCubemapResolution);
      ...
      do {
        ...
        envReflectionResolutionText.Add(
            new GUIContent(cubemapResolution.ToString()));
        cubemapResolution *= 2;
      } while (cubemapResolution <= ReflectionProbe.maxBakedCubemapResolution);

      Styles.reflectionResolutionValuesArray =
          envReflectionResolutionValues.ToArray();
      Styles.reflectionResolutionTextArray =
          envReflectionResolutionText.ToArray();
    }
  ...
  static internal class Styles {
    static Styles() { richTextMiniLabel.richText = true; }
    ...
    public static GUIContent bakeCustomButtonText = EditorGUIUtility.TrTextContent(
        "Bake",
        "Bakes Reflection Probe's cubemap, overwriting the existing cubemap texture asset (if any).");
    public static GUIContent runtimeSettingsHeader = EditorGUIUtility.TrTextContent(
        "Runtime Settings",
        "These settings are used by objects when they render with the cubemap of this probe");
    public static GUIContent backgroundColorText =
        EditorGUIUtility.TrTextContent(
            "Background",
            "Camera clears the screen to this color before rendering.");
    public static GUIContent clearFlagsText =
        EditorGUIUtility.TrTextContent("Clear Flags");
    public static GUIContent intensityText =
        EditorGUIUtility.TrTextContent("Intensity");
    public static GUIContent resolutionText =
        EditorGUIUtility.TrTextContent("Resolution");
    public static GUIContent captureCubemapHeader =
        EditorGUIUtility.TrTextContent("Cubemap Capture Settings");
    public static GUIContent boxProjectionText = EditorGUIUtility.TrTextContent(
        "Box Projection",
    ...
    public static GUIContent blendDistanceText = EditorGUIUtility.TrTextContent(
        "Blend Distance",
        "Area around the probe where it is blended with other probes. Only used in deferred probes.");
    public static GUIContent sizeText = EditorGUIUtility.TrTextContent(
        "Box Size",
        "The size of the box in which the reflections will be applied to objects. The value is not affected by the Transform of the Game Object.");
    public static GUIContent centerText = EditorGUIUtility.TrTextContent(
        "Box Offset",
        "The center of the box in which the reflections will be applied to objects. The value is relative to the position of the Game Object.");
    public static GUIContent customCubemapText =
        EditorGUIUtility.TrTextContent("Cubemap");
    public static GUIContent importanceText =
        EditorGUIUtility.TrTextContent("Importance");
    public static GUIContent renderDynamicObjects =
        EditorGUIUtility.TrTextContent(
            "Dynamic Objects",
            "If enabled dynamic objects are also rendered into the cubemap");
    public static GUIContent timeSlicing = EditorGUIUtility.TrTextContent(
        "Time Slicing",
        "If enabled this probe will update over several frames, to help reduce the impact on the frame rate");
    public static GUIContent refreshMode = EditorGUIUtility.TrTextContent(
        "Refresh Mode", "Controls how this probe refreshes in the Player");

    public static GUIContent typeText = EditorGUIUtility.TrTextContent(
        "Type",
        "'Baked Cubemap' uses the 'Auto Baking' mode from the Lighting window. If it is enabled, then baking is automatic otherwise manual bake is needed (use the bake button below). \n'Custom' can be used if a custom cubemap is wanted. \n'Realtime' can be used to dynamically re-render the cubemap during runtime (via scripting).");
    ...
    public static int[] reflectionProbeModeValues = {
        (int) ReflectionProbeMode.Baked, (int) ReflectionProbeMode.Custom,
  ...
  internal static Color kGizmoReflectionProbe =
      new Color(0xFF / 255f, 0xE5 / 255f, 0x94 / 255f, 0x80 / 255f);
  internal static Color kGizmoReflectionProbeDisabled =
      new Color(0x99 / 255f, 0x89 / 255f, 0x59 / 255f, 0x60 / 255f);
  internal static Color kGizmoHandleReflectionProbe =
      new Color(0xFF / 255f, 0xE5 / 255f, 0xAA / 255f, 0xFF / 255f);

  private SavedBool m_ShowRuntimeSettings;
  private SavedBool m_ShowCubemapCaptureSettings;

  readonly AnimBool m_ShowProbeModeRealtimeOptions =
      new AnimBool(); // p.mode == ReflectionProbeMode.Realtime; Will be brought
                      // back in 5.1
  ...
  public void OnEnable() {
    ...
    m_NearAndFarProperties = new[]{serializedObject.FindProperty("m_NearClip"),
    ...
    m_IntensityMultiplier =
        serializedObject.FindProperty("m_IntensityMultiplier");
    m_BlendDistance = serializedObject.FindProperty("m_BlendDistance");
    m_CustomBakedTexture =
        serializedObject.FindProperty("m_CustomBakedTexture");
    m_RenderDynamicObjects =
        serializedObject.FindProperty("m_RenderDynamicObjects");
    m_UseOcclusionCulling =
        serializedObject.FindProperty("m_UseOcclusionCulling");
    ...
    m_ShowProbeModeRealtimeOptions.value =
    ...
    m_ShowRuntimeSettings =
        new SavedBool("ReflectionProbeEditor.ShowRuntimeSettings", true);
    m_ShowCubemapCaptureSettings =
        new SavedBool("ReflectionProbeEditor.ShowCubemapCaptureSettings", true);
  ...
  private bool IsCollidingWithOtherProbes(string targetPath,
                                          ReflectionProbe targetProbe,
                                          out ReflectionProbe collidingProbe) {
  ...
  private void BakeCustomReflectionProbe(ReflectionProbe probe,
    ...
    if (string.IsNullOrEmpty(path) ||
        Path.GetExtension(path) != "." + targetExtension) {
      // We use the path of the active scene as the target path
      string targetPath =
          FileUtil.GetPathWithoutExtension(SceneManager.GetActiveScene().path);
      if (string.IsNullOrEmpty(targetPath))
        targetPath = "Assets";
      else if (Directory.Exists(targetPath) == false)
        Directory.CreateDirectory(targetPath);

      string fileName = probe.name +
                        (probe.hdr ? "-reflectionHDR"
                         : "-reflection") +
                        "." + targetExtension;
      ...
      path = EditorUtility.SaveFilePanelInProject(
          "Save reflection probe's cubemap.", fileName, targetExtension, "",
          targetPath);
      if (string.IsNullOrEmpty(path))
        return;
      ...
      if (IsCollidingWithOtherProbes(path, probe, out collidingProbe)) {
        if (!EditorUtility.DisplayDialog(
                "Cubemap is used by other reflection probe",
                string.Format(
                    "'{0}' path is used by the game object '{1}', do you really want to overwrite it?",
                    path, collidingProbe.name),
                "Yes", "No")) {
          return;
        }
      }
    ...
    EditorUtility.DisplayProgressBar("Reflection Probes", "Baking " + path,
                                     0.5f);
  ...
  void DoBakeButton() {
    if (reflectionProbeTarget.mode == ReflectionProbeMode.Realtime) {
      EditorGUILayout.HelpBox(
          "Baking of this reflection probe should be initiated from the scripting API because the type is 'Realtime'",
          MessageType.Info);
    ...
    if (reflectionProbeTarget.mode == ReflectionProbeMode.Baked &&
        Lightmapping.GetLightingSettingsOrDefaultsFallback().autoGenerate) {
      EditorGUILayout.HelpBox(
          "Baking of this reflection probe is automatic because this probe's type is 'Baked' and the Lighting window is using 'Auto Baking'. The cubemap created is stored in the GI cache.",
          MessageType.Info);
      return;
    }
    ...
    case ReflectionProbeMode.Custom:
      if (EditorGUI.ButtonWithDropdownList(Styles.bakeCustomButtonText,
                                           Styles.bakeCustomOptionText,
                                           OnBakeCustomButton)) {
  ...
  void DoToolbar() {
    ...
    EditMode.DoInspectorToolbar(Styles.sceneViewEditModes, Styles.toolContents,
                                this);
    ...
    if (sceneViewEditing) {
      int index =
          ArrayUtility.IndexOf(Styles.sceneViewEditModes, EditMode.editMode);
      if (index >= 0)
  ...
  public override void OnInspectorGUI() {
    ...
    m_ShowProbeModeRealtimeOptions.target =
        reflectionProbeMode == ReflectionProbeMode.Realtime;
    m_ShowProbeModeCustomOptions.target =
        reflectionProbeMode == ReflectionProbeMode.Custom;

    // Bake/custom/realtime type
    EditorGUILayout.IntPopup(m_Mode, Styles.reflectionProbeMode,
                             Styles.reflectionProbeModeValues, Styles.typeText);
    ...
    if (!m_Mode.hasMultipleDifferentValues) {
      EditorGUI.indentLevel++;
      {
        ...
        if (EditorGUILayout.BeginFadeGroup(
                m_ShowProbeModeCustomOptions.faded)) {
          EditorGUILayout.PropertyField(m_RenderDynamicObjects,
                                        Styles.renderDynamicObjects);

          EditorGUI.BeginChangeCheck();
          EditorGUI.showMixedValue =
              m_CustomBakedTexture.hasMultipleDifferentValues;
          var newCubemap = EditorGUILayout.ObjectField(
              Styles.customCubemapText,
              m_CustomBakedTexture.objectReferenceValue, typeof(Texture),
              false);
        ...
        // Realtime UI
        if (EditorGUILayout.BeginFadeGroup(
                m_ShowProbeModeRealtimeOptions.faded)) {
    ...
    m_ShowRuntimeSettings.value = EditorGUILayout.BeginFoldoutHeaderGroup(
        m_ShowRuntimeSettings.value, Styles.runtimeSettingsHeader);

    if (m_ShowRuntimeSettings.value) {
      EditorGUI.indentLevel++;

      EditorGUILayout.PropertyField(m_Importance, Styles.importanceText);
      EditorGUILayout.PropertyField(m_IntensityMultiplier,
                                    Styles.intensityText);

      if (Rendering.EditorGraphicsSettings.GetCurrentTierSettings()
              .reflectionProbeBoxProjection == false) {
        using(new EditorGUI.DisabledScope(true)) {
          EditorGUILayout.Toggle(Styles.boxProjectionText, false);
        }
      } else {
        EditorGUILayout.PropertyField(m_BoxProjection,
                                      Styles.boxProjectionText);
      }

      bool isDeferredRenderingPath = SceneView.IsUsingDeferredRenderingPath();
      bool isDeferredReflections =
          isDeferredRenderingPath &&
          (UnityEngine.Rendering.GraphicsSettings.GetShaderMode(
      ...
      using(new EditorGUI.DisabledScope(!isDeferredReflections)) {
        EditorGUILayout.PropertyField(m_BlendDistance,
                                      Styles.blendDistanceText);
      }

      // Bounds editing (box projection bounds + the bounds that objects use to
      // check if they should be affected by this reflection probe)
    ...
    m_ShowCubemapCaptureSettings.value =
        EditorGUILayout.BeginFoldoutHeaderGroup(
            m_ShowCubemapCaptureSettings.value, Styles.captureCubemapHeader);
    ...
    if (m_ShowCubemapCaptureSettings.value) {
      ...
      GetResolutionArray(ref reflectionResolutionValuesArray,
                         ref reflectionResolutionTextArray);

      EditorGUILayout.IntPopup(m_Resolution, reflectionResolutionTextArray,
                               reflectionResolutionValuesArray,
                               Styles.resolutionText, GUILayout.MinWidth(40));
      EditorGUILayout.PropertyField(m_HDR);
      EditorGUILayout.PropertyField(m_ShadowDistance);
      EditorGUILayout.IntPopup(m_ClearFlags, Styles.clearFlags,
                               Styles.clearFlagsValues, Styles.clearFlagsText);
      ...
      EditorGUILayout.PropertiesField(
          EditorGUI.s_ClipingPlanesLabel, m_NearAndFarProperties,
    ...
    if (targets.Length == 1) {
      ...
      if (probe.mode == ReflectionProbeMode.Custom &&
          probe.customBakedTexture != null) {
        Cubemap cubemap = probe.customBakedTexture as Cubemap;
        if (cubemap && cubemap.mipmapCount == 1)
          EditorGUILayout.HelpBox(
              "No mipmaps in the cubemap, Smoothness value in Standard shader will be ignored.",
              MessageType.Warning);
      }
    }
  ...
  public override bool HasPreviewGUI() {
    ...
    if (ValidPreviewSetup()) {
      ...
      Editor.CreateCachedEditor(((ReflectionProbe) target).texture, null,
                                ref editor);
      m_CubemapEditor = editor as TextureInspector;
    }

    // If having one probe selected we always want preview (to prevent preview
    // window from popping)
    return true;
  }
  ...
  public override void OnPreviewSettings() {
    ...
    // Need to repaint, because mipmap value changes affect reflection probe
  ...
  public override void OnPreviewGUI(Rect position, GUIStyle style) {
    // Fix for case 939947 where we didn't get the Layout event if the texture
    // was null when changing color
    if (!ValidPreviewSetup() &&
    ...
    if (m_CubemapEditor != null) {
      m_CubemapEditor.SetCubemapIntensity(
          GetProbeIntensity((ReflectionProbe) target));
  ...
  private static Mesh sphereMesh {
    get {
      return s_SphereMesh ??(s_SphereMesh = Resources.GetBuiltinResource(
                                typeof(Mesh), "New-Sphere.fbx") as Mesh);
    }
  }
  ...
  private Material reflectiveMaterial {
    get {
      if (m_ReflectiveMaterial == null) {
        m_ReflectiveMaterial = (Material) Instantiate(
            EditorGUIUtility.Load("Previews/PreviewCubemapMaterial.mat"));
  ...
  private void OnPreSceneGUICallback(SceneView sceneView) {
    ...
    foreach (var t in targets) {
      ...
      if (!StageUtility.IsGameObjectRenderedByCameraAndPartOfEditableScene(
              p.gameObject, Camera.current))
        return;

      Matrix4x4 m = new Matrix4x4();

      // @TODO: use MaterialPropertyBlock instead - once it actually works!
      // Tried to use MaterialPropertyBlock in 5.4.0b2, but would get
      // incorrectly set parameters when using with Graphics.DrawMesh
        ...
        return;
      {
        ...
        // draw a preview sphere that scales with overall GO scale, but always
        // uniformly
        var scale = p.transform.lossyScale.magnitude * 0.5f;
        m.SetTRS(p.transform.position, Quaternion.identity,
                 new Vector3(scale, scale, scale));
        Graphics.DrawMesh(sphereMesh, m, mat, 0,
                          SceneView.currentDrawingSceneView.camera, 0);
      }
    }
  }
  ...
  private bool ValidateAABB(ref Vector3 center, ref Vector3 size) {
    ...
    Vector3 localTransformPosition =
        localSpace.inverse.MultiplyPoint3x4(p.transform.position);
  ...
  static void RenderBoxGizmo(ReflectionProbe reflectionProbe,
                             GizmoType gizmoType) {
    if (s_LastInteractedEditor == null)
      return;

    if (s_LastInteractedEditor.sceneViewEditing &&
        EditMode.editMode == EditMode.SceneViewEditMode.ReflectionProbeBox) {
  ...
  static void RenderBoxOutline(ReflectionProbe reflectionProbe,
                               GizmoType gizmoType) {
    if (!s_CurrentlyEditedProbes.Contains(reflectionProbe))
      return;

    Color oldColor = Gizmos.color;
    Gizmos.color = reflectionProbe.isActiveAndEnabled ? kGizmoReflectionProbe
        : kGizmoReflectionProbeDisabled;
  ...
  void DoOriginEditing() {
    ...
    EditorGUI.BeginChangeCheck();
    Vector3 newPostion =
        Handles.PositionHandle(transformPosition, GetLocalSpaceRotation(p));

    bool changed = EditorGUI.EndChangeCheck();

    if (changed || m_OldLocalSpace != GetLocalSpace((ReflectionProbe) target)) {
      Vector3 localNewPosition =
          m_OldLocalSpace.inverse.MultiplyPoint3x4(newPostion);
      ...
      p.center = GetLocalSpace(p).inverse.MultiplyPoint3x4(
          m_OldLocalSpace.MultiplyPoint3x4(p.center));
  ...
  static Quaternion GetLocalSpaceRotation(ReflectionProbe probe) {
    bool supportsRotation =
        (SupportedRenderingFeatures.active.reflectionProbeModes &
         SupportedRenderingFeatures.ReflectionProbeModes.Rotation) != 0;
    if (supportsRotation)
