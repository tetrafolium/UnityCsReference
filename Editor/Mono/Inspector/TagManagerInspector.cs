...
internal class TagManagerInspector : ProjectSettingsBaseEditor {
  ...
  private static InitialExpansionState s_InitialExpansionState =
  ...
  internal class Styles {
    ...
    public static GUIContent sortingLayers =
        EditorGUIUtility.TrTextContent("Sorting Layers");
  ...
  public virtual void OnEnable() {
    ...
    if (m_TagsList == null) {
      m_TagsList = new ReorderableList(serializedObject, m_Tags, false, false,
                                       true, true);
    ...
    if (m_SortLayersList == null) {
      m_SortLayersList = new ReorderableList(serializedObject, m_SortingLayers,
    ...
    if (m_LayersList == null) {
      m_LayersList = new ReorderableList(serializedObject, m_Layers, false,
                                         false, false, false);
  ...
  ShowWithInitialExpansion(InitialExpansionState initialExpansionState) {
  ...
  class EnterNamePopup : PopupWindowContent {
    ...
    public EnterNamePopup(SerializedProperty tags, EnterDelegate cb) {
      ...
      m_NewTagName =
          ObjectNames.GetUniqueName(existingTagNames.ToArray(), m_NewTagName);
    }

    public override Vector2 GetWindowSize() {
      return new Vector2(400, EditorGUI.kSingleLineHeight * 2 +
                                  EditorGUI.kControlVerticalSpacing + 14);
    }
  ...
  private void RemoveFromTagsList(ReorderableList list) {
    ...
    if (go != null) {
      EditorUtility.DisplayDialog(
          "Error",
          "Can't remove this tag because it is being used by " + go.name, "OK");
    } else {
  ...
  private void DrawTagListElement(Rect rect, int index, bool selected,
                                  bool focused) {
  ...
  private void DrawSortLayerListElement(Rect rect, int index, bool selected,
                                        bool focused) {
    // nicer looking with selected list row and a text field in it
    rect.yMin += 1;
    rect.yMax -= 1;
  ...
  public override void OnInspectorGUI() {
    ...
    m_IsEditable =
        AssetDatabase.IsOpenForEdit("ProjectSettings/TagManager.asset",
    ...
    // Tags
    m_Tags.isExpanded =
        EditorGUILayout.Foldout(m_Tags.isExpanded, Styles.tags, true);
    if (m_Tags.isExpanded) {
      ...
      if (m_HaveRemovedTag)
        EditorGUILayout.HelpBox(
            "There are removed tags. They will be removed from this list the next time the project is loaded.",
            MessageType.Info, true);
    }

    // Sorting layers
    m_SortingLayers.isExpanded = EditorGUILayout.Foldout(
        m_SortingLayers.isExpanded, Styles.sortingLayers, true);
    ...
    // Layers
    m_Layers.isExpanded =
        EditorGUILayout.Foldout(m_Layers.isExpanded, Styles.layers, true);
