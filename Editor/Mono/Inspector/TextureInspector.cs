...
internal class PreviewHelpers {
  ...
  internal static void AdjustWidthAndHeightForStaticPreview(int textureWidth,
    ...
    if (textureWidth <= width && textureHeight <= height) {
      // For textures smaller than our wanted width and height we use the
      // textures size to prevent excessive magnification artifacts (as seen in
      // the Asset Store).
      width = textureWidth;
      height = textureHeight;
    } else {
      // For textures larger than our wanted width and height we ensure to
      // keep aspect ratio of the texture and fit it to best match our wanted
      // width and height.
...
internal class TextureInspector : Editor {
  class Styles {
    ...
    public GUIStyle toolbarButton, previewSlider, previewSliderThumb,
        previewLabel;
    ...
    public readonly GUIContent wrapModeLabel =
    ...
    public readonly int[] wrapModeValues = {
        (int) TextureWrapMode.Repeat, (int) TextureWrapMode.Clamp,
        (int) TextureWrapMode.Mirror, (int) TextureWrapMode.MirrorOnce, -1};
  ...
  [SerializeField] float m_MipLevel = 0;

  [SerializeField]
  protected float m_ExposureSliderValue = 0.0f;

  protected float m_ExposureSliderMax =
      16f; // this value can be altered by the user
  ...
  // VT textures can be very large and aren't in GPU memory yet. To avoid
  // unnecessary streaming and cache use, we limit the default shown mip
  // resolution.
    ...
    foreach (var t in targets) {
      ...
      if (EditorGUI.UseVTMaterial(tex)) {
        ...
        if (mips > numMipsFor1K) {
          mipLevel = Mathf.Max(mipLevel,
                               mips - numMipsFor1K); // set to 1024x1024 or less
        }
      }
    }
  }

  public override void ReloadPreviewInstances() { SetMipLevelDefaultForVT(); }
  ...
  public override bool RequiresConstantRepaint() {
    // Keep repainting if the texture is rendered with a virtual texturing
    // material because we don't know when all texture tiles will be streamed in
    ...
    foreach (TextureMipLevels textureInfo in m_TextureMipLevels) {
      ...
      if (textureInfo.texture.loadedMipmapLevel !=
          textureInfo.lastMipmapLevel) {
  ...
  public float GetMipLevelForRendering() {
    ...
    if (IsCubemap())
      ...
      return Mathf.Min(m_MipLevel,
                       TextureUtil.GetMipmapCount(target as Texture) - 1);
  }
  ...
  // Note: Even though this is a custom editor for Texture2D, the target may not
  // be a Texture2D, since other editors inherit from this one, such as
  ...
  // but in case of textures we need an extra step to ApplySettings (so
  // rendering uses new values) alas we cant have good things: it will be PITA
  // to make sure we always call that after applying changes to serialized
  // object meaning that we need to work without relying on it, hence we do
  // similar to TextureImporter:
  //   use TextureUtil methods to update texture settings from current values of
  //   serialized property
  // another possibility would be to do it separately for wrap/filter/aniso
  //   alas for wrapmode i dont see how it can be done clearly and leaving it
  //   out seems a bit weird

  protected void ApplySettingsToTextures() {
    bool anisoDiffer = m_Aniso.hasMultipleDifferentValues,
         filterDiffer = m_FilterMode.hasMultipleDifferentValues;
    bool wrapDiffer = m_WrapU.hasMultipleDifferentValues ||
    ...
    foreach (Texture tex in targets) {
      ...
      // NB i am not sure if it is *possible* to have -1 in there and if it make
      // sense to pass -1 to SetWrapModeNoDirty NB but this is how
      // TextureImporter checks things and who am i to argue
      if ((m_WrapU.intValue != -1 || m_WrapV.intValue != -1 ||
           m_WrapW.intValue != -1) &&
          !wrapDiffer)
        TextureUtil.SetWrapModeNoDirty(tex, (TextureWrapMode) m_WrapU.intValue,
                                       (TextureWrapMode) m_WrapV.intValue,
                                       (TextureWrapMode) m_WrapW.intValue);
    }
  }

  static void WrapModeAxisPopup(GUIContent label,
                                SerializedProperty wrapProperty) {
    // In texture importer settings, serialized properties for wrap modes can
    // contain -1, which means "use default".
  ...
  private static bool
  IsAnyTextureObjectUsingPerAxisWrapMode(Object[] objects,
  ...
  // showPerAxisWrapModes is state of whether "Per-Axis" mode should be active
  // in the main dropdown. It is set automatically if wrap modes in UVW are
  // different, or if user explicitly picks "Per-Axis" option -- when that one
  // is picked, then it should stay true even if UVW wrap modes will initially
  // be the same. enforcePerAxis is used to always show all axis option. In some
  // cases (Preset), we want to show each property separately to make sure users
  // can act on one or another with the context menu. Note: W wrapping mode is
  // only shown when isVolumeTexture is true.
  internal static void
  WrapModePopup(SerializedProperty wrapU, SerializedProperty wrapV,
                SerializedProperty wrapW, bool isVolumeTexture,
                ref bool showPerAxisWrapModes, bool enforcePerAxis) {
    if (s_Styles == null)
      s_Styles = new Styles();

    // In texture importer settings, serialized properties for things like wrap
    // modes can contain -1; that seems to indicate "use defaults, user has not
    // changed them to anything" but not totally sure. Show them as Repeat wrap
    // modes in the popups.
    ...
    // It's not possible to determine whether any single texture in the whole
    // selection is using per-axis wrap modes just from SerializedProperty
    // values. They can only tell if "some values in whole selection are
    // different" (e.g. wrap value on U axis is not the same among all
    // textures), and can return value of "some" object in the selection
    // (typically based on object loading order). So in order for more intuitive
    // behavior with multi-selection, we go over the actual objects when there's
    // >1 object selected and some wrap modes are different.
    if (!showPerAxisWrapModes) {
      if (wrapU.hasMultipleDifferentValues ||
          wrapV.hasMultipleDifferentValues ||
          (isVolumeTexture && wrapW.hasMultipleDifferentValues)) {
        if (IsAnyTextureObjectUsingPerAxisWrapMode(
                wrapU.serializedObject.targetObjects, isVolumeTexture)) {
          showPerAxisWrapModes = true;
        }
      }
    }
    ...
    if (enforcePerAxis) {
      ...
      EditorGUI.showMixedValue =
          !showPerAxisWrapModes &&
          (wrapU.hasMultipleDifferentValues ||
           wrapV.hasMultipleDifferentValues ||
           (isVolumeTexture && wrapW.hasMultipleDifferentValues));
      value = EditorGUILayout.IntPopup(s_Styles.wrapModeLabel, value,
                                       s_Styles.wrapModeContents,
                                       s_Styles.wrapModeValues);
  ...
  protected void DoWrapModePopup() {
    WrapModePopup(m_WrapU, m_WrapV, m_WrapW, IsVolume(),
                  ref m_ShowPerAxisWrapModes, false);
  }
  ...
  protected void DoFilterModePopup() {
    ...
    filter = (FilterMode) EditorGUILayout.EnumPopup(
        EditorGUIUtility.TempContent("Filter Mode"), filter);
    EditorGUI.showMixedValue = false;
    if (EditorGUI.EndChangeCheck())
    ...
    if (anisoLevel > 1) {
      if (QualitySettings.anisotropicFiltering == AnisotropicFiltering.Disable)
        EditorGUILayout.HelpBox(
            "Anisotropic filtering is disabled for all textures in Quality Settings.",
            MessageType.Info);
      else if (QualitySettings.anisotropicFiltering ==
               AnisotropicFiltering.ForceEnable)
        EditorGUILayout.HelpBox(
            "Anisotropic filtering is enabled for all textures in Quality Settings.",
            MessageType.Info);
    }
  }
  ...
  bool IsCubemap() {
    ...
    return t != null &&
           t.dimension == UnityEngine.Rendering.TextureDimension.Cube;
  }

  bool IsCubemapArray() {
    var t = target as Texture;
    return t != null &&
           t.dimension == UnityEngine.Rendering.TextureDimension.CubeArray;
  }
  ...
  bool IsTexture2DArray() {
    ...
    return t != null &&
           t.dimension == UnityEngine.Rendering.TextureDimension.Tex2DArray;
  }
  ...
  public override void OnPreviewSettings() {
    ...
    foreach (Texture t in targets) {
      if (t == null) // texture might have disappeared while we're showing this
                     // in a preview popup
        continue;
      ...
      if (t is Texture2D) {
        ...
        if (TextureUtil.HasAlphaTextureFormat(format)) {
          if (mode == TextureUsageMode.Default) // all other texture usage modes
                                                // don't displayable alpha
            hasAlpha = true;
        }
    ...
    if (previewCandidates.Count > 1 && tex != null && !IsNormalMap(tex)) {
      ...
      if (previewCandidates.Contains(PreviewMode.RGB))
        m_PreviewMode = GUILayout.Toggle(m_PreviewMode == PreviewMode.RGB,
                                         s_Styles.previewButtonContents[0],
                                         s_Styles.toolbarButton)
                            ? PreviewMode.RGB
                            : m_PreviewMode;
      if (previewCandidates.Contains(PreviewMode.R))
        m_PreviewMode = GUILayout.Toggle(m_PreviewMode == PreviewMode.R,
                                         s_Styles.previewButtonContents[1],
                                         s_Styles.toolbarButton)
                            ? PreviewMode.R
                            : m_PreviewMode;
      if (previewCandidates.Contains(PreviewMode.G))
        m_PreviewMode = GUILayout.Toggle(m_PreviewMode == PreviewMode.G,
                                         s_Styles.previewButtonContents[2],
                                         s_Styles.toolbarButton)
                            ? PreviewMode.G
                            : m_PreviewMode;
      if (previewCandidates.Contains(PreviewMode.B))
        m_PreviewMode = GUILayout.Toggle(m_PreviewMode == PreviewMode.B,
                                         s_Styles.previewButtonContents[3],
                                         s_Styles.toolbarButton)
                            ? PreviewMode.B
                            : m_PreviewMode;
      if (previewCandidates.Contains(PreviewMode.A))
        m_PreviewMode = GUILayout.Toggle(m_PreviewMode == PreviewMode.A,
                                         s_Styles.previewButtonContents[4],
                                         s_Styles.toolbarButton)
                            ? PreviewMode.A
                            : m_PreviewMode;
    }
    ...
    if (mipCount > 1) {
      ...
      m_MipLevel = Mathf.Round(GUILayout.HorizontalSlider(
          m_MipLevel, mipCount - 1, 0, s_Styles.previewSlider,
          s_Styles.previewSliderThumb, GUILayout.MaxWidth(64)));

      // For now, we don't have mipmaps smaller than the tile size when using
      // VT.
      if (EditorGUI.UseVTMaterial(tex)) {
        int numMipsOfTile =
            (int) Mathf.Log(VirtualTexturing.EditorHelpers.tileSize, 2) + 1;
        m_MipLevel =
            Mathf.Min(m_MipLevel, Mathf.Max(mipCount - numMipsOfTile, 0));
      }
  ...
  public override bool HasPreviewGUI() { return (target != null); }
  ...
  public override void OnPreviewGUI(Rect r, GUIStyle background) {
    ...
    if (t == null) // texture might be gone by now, in case this code is used
                   // for floating texture preview
      return;
    ...
    float zoomLevel =
        Mathf.Min(Mathf.Min(r.width / texWidth, r.height / texHeight), 1);
    Rect wantedRect =
        new Rect(r.x, r.y, texWidth * zoomLevel, texHeight * zoomLevel);
    ...
    if (m_PreviewMode == PreviewMode.A) {
      ...
      if (t2d != null && t2d.alphaIsTransparency)
        EditorGUI.DrawTextureTransparent(wantedRect, t, ScaleMode.StretchToFill,
                                         0, mipLevel, colorWriteMask,
                                         GetExposureValueForTexture(t));
      else
        EditorGUI.DrawPreviewTexture(
            wantedRect, t, null, ScaleMode.StretchToFill, 0, mipLevel,
            colorWriteMask, GetExposureValueForTexture(t));
    }

    // TODO: Less hacky way to prevent sprite rects to not appear in smaller
    // previews like icons.
    if ((wantedRect.width > 32 && wantedRect.height > 32) &&
      ...
      SpriteMetaData[] spritesheet =
          textureImporter != null ? textureImporter.spritesheet : null;

      if (spritesheet != null &&
          textureImporter.spriteImportMode == SpriteImportMode.Multiple) {
        Rect screenRect = new Rect();
        Rect sourceRect = new Rect();
        GUI.CalculateScaledTextureRects(wantedRect, ScaleMode.StretchToFill,
        ...
        foreach (SpriteMetaData sprite in spritesheet) {
          ...
          spriteScreenRect.xMin =
              screenRect.xMin +
              screenRect.width * (spriteRect.xMin / t.width * definitionScale);
          spriteScreenRect.xMax =
              screenRect.xMin +
              screenRect.width * (spriteRect.xMax / t.width * definitionScale);
          spriteScreenRect.yMin =
              screenRect.yMin +
              screenRect.height *
                  (1f - spriteRect.yMin / t.height * definitionScale);
          spriteScreenRect.yMax =
              screenRect.yMin +
              screenRect.height *
                  (1f - spriteRect.yMax / t.height * definitionScale);
    ...
    if (mipLevel != 0)
      EditorGUI.DropShadowLabel(new Rect(r.x, r.y, r.width, 20),
                                "Mip " + mipLevel);
  }
  ...
  public override Texture2D RenderStaticPreview(string assetPath,
    ...
    if (textureImporter != null &&
      ...
      // If the texture importer is a Sprite of primitive, use the sprite
      // inspector for generating preview/icon.
      if (subAssets.Length > 0) {
        Sprite sprite = subAssets[0] as Sprite;
        if (sprite)
          return SpriteInspector.BuildPreviewTexture(width, height, sprite,
                                                     null, true);
      } else
        return null;
    }

    PreviewHelpers.AdjustWidthAndHeightForStaticPreview(
        texture.width, texture.height, ref width, ref height);

    RenderTexture savedRT = RenderTexture.active;
    Rect savedViewport = ShaderUtil.rawViewportRect;

    RenderTexture tmp = RenderTexture.GetTemporary(
        width, height, 0, SystemInfo.GetGraphicsFormat(DefaultFormat.LDR));
    Material mat = EditorGUI.GetMaterialForSpecialTexture(
        texture, null, QualitySettings.activeColorSpace == ColorSpace.Linear,
        false);
    if (mat != null)
  ...
  public override string GetInfoString() {
    ...
    TextureImporter textureImporter = AssetImporter.GetAtPath(
        AssetDatabase.GetAssetPath(t)) as TextureImporter;
    string info = t.width + "x" + t.height;

    bool showSize = true;
    bool isPackedSprite =
        textureImporter && textureImporter.qualifiesForSpritePacking;
    bool isNormalmap = IsNormalMap(t);
    bool stillNeedsCompression = TextureUtil.DoesTextureStillNeedToBeCompressed(
    ...
    if (stillNeedsCompression)
      ...
      if (isNormalmap) {
        ...
        textureImporter.ReadTextureImportInstructions(
            EditorUserBuildSettings.activeBuildTarget, out desiredFormat,
            out dummyColorSpace, out dummyComressionQuality);

        info += "\n " + GraphicsFormatUtility.GetFormatString(format) +
                "(Original) " +
                TextureUtil.GetTextureFormatString(desiredFormat) + "(Atlas)";
      } else
        info += "  " + GraphicsFormatUtility.GetFormatString(format);
    }
    ...
    if (showSize)
      info += "\n" + EditorUtility.FormatBytes(
                         TextureUtil.GetStorageMemorySizeLong(t));
  ...
  internal static float PreviewSettingsSlider(GUIContent content, float value,
                                              float min, float max,
                                              float sliderWidth,
    ...
    var controlRect = EditorGUILayout.GetControlRect(
        GUILayout.Width(labelWidth + sliderWidth + floatFieldWidth));
    var controlId = GUIUtility.GetControlID(FocusType.Keyboard);

    var labelRect = new Rect(controlRect.position,
                             new Vector2(labelWidth, controlRect.height));
    controlRect.x += labelRect.width;
    controlRect.width -= labelRect.width + 2;
    GUI.Label(labelRect, content);

    var sliderRect = new Rect(controlRect.position,
                              new Vector2(sliderWidth, controlRect.height));
    controlRect.x += sliderRect.width + 2;
    controlRect.width -= sliderRect.width;
    value =
        GUI.Slider(sliderRect, value, 0, min, max, GUI.skin.horizontalSlider,
                   GUI.skin.horizontalSliderThumb, true, 0);
    if (isInteger)
      value = Mathf.Round(EditorGUI.DoIntField(
          EditorGUI.s_RecycledEditor, controlRect, labelRect, controlId,
          Mathf.RoundToInt(value), EditorGUI.kIntFieldFormatString,
          EditorStyles.numberField, false, 0));
    else
      value = EditorGUI.DoFloatField(
          EditorGUI.s_RecycledEditor, controlRect, labelRect, controlId, value,
...
class PreviewGUI {
  ...
  internal static void BeginScrollView(Rect position, Vector2 scrollPosition,
                                       Rect viewRect,
    ...
    GUIClip.Push(
        ...
        new Vector2(Mathf.Round(-scrollPosition.x - viewRect.x -
                                (viewRect.width - position.width) * .5f),
                    Mathf.Round(-scrollPosition.y - viewRect.y -
                                (viewRect.height - position.height) * .5f)),
        Vector2.zero, false);
  }

  internal class Styles {
    public static GUIStyle preButton;
    public static void Init() { preButton = "toolbarbutton"; }
  }
  ...
  public static Vector2 EndScrollView() {
    ...
    default:
      bool needsVerticalScrollbar =
          ((int) viewRect.width > (int) clipRect.width);
      bool needsHorizontalScrollbar =
          ((int) viewRect.height > (int) clipRect.height);
      ...
      if (needsHorizontalScrollbar) {
        ...
        scrollPosition.x = GUI.Slider(
            new Rect(
                position.x, position.yMax - horizontalScrollbar.fixedHeight,
                clipRect.width - (needsVerticalScrollbar ? horizontalScrollbar
                                      .fixedHeight : 0),
        ...
        // Get the same number of Control IDs so the ID generation for children
        // don't depend on number of things above
        scrollPosition.x = 0;
      }
      ...
      if (needsVerticalScrollbar) {
        ...
        scrollPosition.y = GUI.Slider(
            new Rect(clipRect.xMax - verticalScrollbar.fixedWidth, clipRect.y,
                     verticalScrollbar.fixedWidth, clipRect.height),
            scrollPosition.y, clipRect.height + offset, -offset,
            viewRect.height, verticalScrollbar, verticalScrollbarThumb, false,
            id);
      } else {
        scrollPosition.y = 0;
      }
  ...
  public static Vector2 Drag2D(Vector2 scrollPosition, Rect position) {
    ...
    case EventType.MouseDrag:
      if (GUIUtility.hotControl == id) {
        scrollPosition -= evt.delta * (evt.shift ? 3 : 1) /
                          Mathf.Min(position.width, position.height) * 140.0f;
        evt.Use();
        GUI.changed = true;
      }
      break;
