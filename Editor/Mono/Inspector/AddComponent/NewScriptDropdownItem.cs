...
private readonly char[] kInvalidPathChars =
	new char[] {'<', '>', ':', '"', '|', '?', '*', (char) 0};
private readonly char[] kPathSepChars = new char[] {'/', '\\'};
private readonly string kResourcesTemplatePath =
...
internal bool CanCreate() {
	return m_ClassName.Length > 0 && !ClassNameIsInvalid() &&
	       !File.Exists(TargetPath()) && !ClassAlreadyExists() &&
	       !InvalidTargetPath();
}
...
public string GetError() {
	...
	if (m_ClassName != string.Empty) {
		if (ClassNameIsInvalid())
			...
			blockReason = "A script called \"" + m_ClassName +
			              "\" already exists at that path.";
		else if (ClassAlreadyExists())
			blockReason =
				"A class called \"" + m_ClassName + "\" already exists.";
...
internal void Create(GameObject[] gameObjects, string searchString) {
	...
	foreach (var go in gameObjects) {
		var script = AssetDatabase.LoadAssetAtPath(
			TargetPath(), typeof(MonoScript)) as MonoScript;
...
private bool InvalidTargetPath() {
	if (m_Directory.IndexOfAny(kInvalidPathChars) >= 0)
		return true;
	if (TargetDir()
	    .Split(kPathSepChars, StringSplitOptions.None)
...
private void CreateScript() {
	var basePath = Path.Combine(EditorApplication.applicationContentsPath,
	                            kResourcesTemplatePath);
	var templatePath =
		Path.Combine(basePath, "81-C# Script-NewBehaviourScript.cs.txt");
	ProjectWindowUtil.CreateScriptAssetFromTemplate(TargetPath(),
	                                                templatePath);
