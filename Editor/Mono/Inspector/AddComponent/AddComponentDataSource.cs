    ...
    protected AdvancedDropdownItem RebuildTree() {
      ...
      for (var i = 0; i < menuDictionary.Count; i++) {
        ...
        for (var j = 0; j < paths.Length; j++) {
          var path = paths[j];
          if (j == paths.Length - 1) {
            var element = new ComponentDropdownItem(path, L10n.Tr(path),
                                                    menuPath, menu.Value);
            parent.AddChild(element);
            m_SearchableElements.Add(element);
            continue;
          }
          var group = (ComponentDropdownItem)
      ...
      var newScript =
          new ComponentDropdownItem("New script", L10n.Tr("New script"));
