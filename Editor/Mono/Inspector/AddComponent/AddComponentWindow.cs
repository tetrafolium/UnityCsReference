...
namespace UnityEditor.AddComponent {
  ...
  internal class AddComponentWindow : AdvancedDropdownWindow {
    ...
    protected override bool setInitialSelectionPosition { get; }
    = false;
    ...
    internal static bool Show(Rect rect, GameObject[] gos) {
      ...
      window.gui =
          new AddComponentGUI(window.dataSource, window.OnCreateNewScript);
    ...
    private void OnItemSelected(AdvancedDropdownItem item) {
      if (item is ComponentDropdownItem) {
        SendUsabilityAnalyticsEvent(new AnalyticsEventData{
            name = name, filter = searchString, isNewScript = false});

        var gos = m_GameObjects;
        EditorApplication.ExecuteMenuItemOnGameObjects(
            ((ComponentDropdownItem) item).menuPath, gos);
      }
    }
    ...
    protected override bool SpecialKeyboardHandling(Event evt) {
      ...
      if (createScriptMenu is NewScriptDropdownItem) {
        // When creating new script name we want to dedicate both left/right
        // arrow and backspace to editing the script name so they can't be used
        // for navigating the menus. The only way to get back using the keyboard
        // is pressing Esc.
    ...
    void OnCreateNewScript(NewScriptDropdownItem item) {
      ...
      SendUsabilityAnalyticsEvent(new AnalyticsEventData{
          name = item.className, filter = searchString, isNewScript = true});
      Close();
    }

    internal void SendUsabilityAnalyticsEvent(AnalyticsEventData eventData) {
      var openTime = m_ComponentOpenTime;
      UsabilityAnalytics.SendEvent("executeAddComponentWindow", openTime,
                                   DateTime.UtcNow - openTime, false,
                                   eventData);
    }
