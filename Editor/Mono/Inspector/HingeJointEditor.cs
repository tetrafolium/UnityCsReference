...
class HingeJointEditor : JointEditor<HingeJoint> {
  static readonly GUIContent s_WarningMessage = EditorGUIUtility.TrTextContent(
      "Min and max limits must be within the range [-180, 180].");
...
class HingeJointTool : JointTool<HingeJoint> {
  void OnEnable() {
    ...
    angularLimitHandle.xRange = new Vector2(-Physics.k_MaxFloatMinusEpsilon,
                                            Physics.k_MaxFloatMinusEpsilon);
  }

  protected override void GetActors(HingeJoint joint,
                                    out Rigidbody dynamicActor,
                                    out Rigidbody connectedActor,
                                    out int jointFrameActorIndex,
                                    out bool rightHandedLimit) {
    base.GetActors(joint, out dynamicActor, out connectedActor,
  ...
  protected override void DoAngularLimitHandles(HingeJoint joint) {
    ...
    angularLimitHandle.xMotion = joint.useLimits ? ConfigurableJointMotion
