...
internal class AnimationClipOverrideComparer
    : IComparer<KeyValuePair<AnimationClip, AnimationClip>> {
  public int Compare(KeyValuePair<AnimationClip, AnimationClip> x,
                     KeyValuePair<AnimationClip, AnimationClip> y) {
    return string.Compare(x.Key.name, y.Key.name,
...
internal class AnimatorOverrideControllerInspector : Editor {
  ...
  void OnEnable() {
    AnimatorOverrideController animatorOverrideController =
        target as AnimatorOverrideController;
    ...
    if (m_ClipList == null) {
      ...
      m_ClipList = new ReorderableList(
          m_Clips, typeof(KeyValuePair<AnimationClip, AnimationClip>), false,
          true, false, false);
  ...
  public override void OnInspectorGUI() {
    ...
    RuntimeAnimatorController runtimeAnimatorController =
        m_Controller
            .hasMultipleDifferentValues ? null
        : animatorOverrideController.runtimeAnimatorController;

    EditorGUI.BeginChangeCheck();
    runtimeAnimatorController =
        EditorGUILayout.ObjectField("Controller", runtimeAnimatorController,
                                    typeof(Animations.AnimatorController),
                                    false) as RuntimeAnimatorController;
    if (EditorGUI.EndChangeCheck()) {
      for (int i = 0; i < targets.Length; i++) {
        AnimatorOverrideController controller =
            targets[i] as AnimatorOverrideController;
...

    {
      ...
      if (Event.current.type == EventType.KeyDown &&
          ...
          GUI.GetNameOfFocusedControl() == "OverridesSearch")
        m_Search = "";
    ...
    using(new EditorGUI.DisabledScope(
        m_Controller == null ||
        (isEditingMultipleObjects && m_Controller.hasMultipleDifferentValues) ||
  ...
  private void FilterOverrides() {
    ...
    // first priority.
    List<KeyValuePair<AnimationClip, AnimationClip>> matchesStart =
        new List<KeyValuePair<AnimationClip, AnimationClip>>();
    List<KeyValuePair<AnimationClip, AnimationClip>> matchesWithin =
        new List<KeyValuePair<AnimationClip, AnimationClip>>();
    foreach (KeyValuePair<AnimationClip, AnimationClip> kvp in m_Clips) {
      ...
      for (int w = 0; w < searchWords.Length; w++) {
        ...
        if (name.Contains(search)) {
          // If the start of the item matches the first search word, make a note
          // of that.
  ...
  private void DrawClipElement(Rect rect, int index, bool selected,
    ...
    EditorGUI.BeginChangeCheck();
    overrideClip = EditorGUI.ObjectField(
        rect, "", overrideClip, typeof(AnimationClip), false) as AnimationClip;
    if (EditorGUI.EndChangeCheck())
      m_Clips[index] = new KeyValuePair<AnimationClip, AnimationClip>(
          originalClip, overrideClip);
  }
