...
class Styles {
public static readonly GUIContent flipLabel =
	EditorGUIUtility.TrTextContent("Flip", "Sprite flipping");
public static readonly GUIContent flipXLabel =
	EditorGUIUtility.TrTextContent("X", "Sprite horizontal flipping");
public static readonly GUIContent flipYLabel =
	EditorGUIUtility.TrTextContent("Y", "Sprite vertical flipping");
public static readonly int flipToggleHash = "FlipToggleHash".GetHashCode();

public static readonly GUIContent fullTileLabel =
	EditorGUIUtility.TrTextContent("Tile Mode",
	                               "Specify the 9-slice tiling behaviour");
public static readonly GUIContent fullTileThresholdLabel =
	EditorGUIUtility.TrTextContent(
		"Stretch Value",
		"This value defines how much the center portion will stretch before it tiles.");
public static readonly GUIContent drawModeLabel =
	EditorGUIUtility.TrTextContent("Draw Mode",
	                               "Specify the draw mode for the sprite");
public static readonly GUIContent widthLabel =
	EditorGUIUtility.TrTextContent(
		"Width", "The width dimension value for the sprite");
public static readonly GUIContent heightLabel =
	EditorGUIUtility.TrTextContent(
		"Height", "The height dimension value for the sprite");
public static readonly GUIContent sizeLabel =
	EditorGUIUtility.TrTextContent(
		"Size", "The rendering dimension for the sprite");
public static readonly GUIContent notFullRectWarningLabel =
	EditorGUIUtility.TrTextContent(
		"Sprite Tiling might not appear correctly because the Sprite used is not generated with Full Rect or Sprite Mode is set to Polygon mode. To fix this, change the Mesh Type in the Sprite's import setting to Full Rect and Sprite Mode is either Single or Multiple");
public static readonly GUIContent notFullRectMultiEditWarningLabel =
	EditorGUIUtility.TrTextContent(
		"Sprite Tiling might not appear correctly because some of the Sprites used are not generated with Full Rect. To fix this, change the Mesh Type in the Sprite's import setting to Full Rect");
public static readonly int sizeFieldHash =
	"SpriteRendererSizeField".GetHashCode();
public static readonly GUIContent materialLabel =
	EditorGUIUtility.TrTextContent("Material",
	                               "Material to be used by SpriteRenderer");
public static readonly GUIContent spriteLabel =
	EditorGUIUtility.TrTextContent("Sprite", "The Sprite to render");
public static readonly GUIContent colorLabel =
	EditorGUIUtility.TrTextContent(
		"Color", "Rendering color for the Sprite graphic");
public static readonly GUIContent maskInteractionLabel =
	EditorGUIUtility.TrTextContent(
		"Mask Interaction",
		"SpriteRenderer's interaction with a Sprite Mask");
public static readonly GUIContent spriteSortPointLabel =
	EditorGUIUtility.TrTextContent(
		"Sprite Sort Point",
		"Determines which position of the Sprite which is used for sorting");
public static readonly Texture2D warningIcon =
	EditorGUIUtility.LoadIcon("console.warnicon");
}
...
public override void OnEnable() {
	...
	m_Material = serializedObject.FindProperty(
		"m_Materials.Array"); // Only allow to edit one material
	...
	m_AdaptiveModeThreshold =
		serializedObject.FindProperty("m_AdaptiveModeThreshold");
...
public override void OnInspectorGUI() {
	...
	using (new EditorGUI.DisabledGroupScope(IsTextureless())) {
		...
		if (EditorGUILayout.BeginFadeGroup(m_ShowDrawMode.faded)) {
			...
			FloatFieldLabelAbove(Styles.widthLabel,
			                     m_Size.FindPropertyRelative("x"));
			FloatFieldLabelAbove(Styles.heightLabel,
			...
			if (EditorGUILayout.BeginFadeGroup(m_ShowTileMode.faded)) {
				...
				if (EditorGUILayout.BeginFadeGroup(m_ShowAdaptiveThreshold.faded)) {
					EditorGUI.indentLevel++;
					EditorGUILayout.Slider(m_AdaptiveModeThreshold, 0.0f, 1.0f,
					                       Styles.fullTileThresholdLabel);
					EditorGUI.indentLevel--;
				}
	...
	EditorGUILayout.PropertyField(m_SpriteSortPoint,
	                              Styles.spriteSortPointLabel);
	EditorGUILayout.PropertyField(m_Material.GetArrayElementAtIndex(0),
	                              Styles.materialLabel, true);
...
void FloatFieldLabelAbove(GUIContent contentLabel, SerializedProperty sp) {
	...
	int id = GUIUtility.GetControlID(Styles.sizeFieldHash, FocusType.Keyboard,
	                                 rtLabel);
	...
	float value = EditorGUI.DoFloatField(
		EditorGUI.s_RecycledEditor, rt, rtLabel, id, sp.floatValue,
...
string GetSpriteNotFullRectWarning() {
	foreach (var t in targets) {
		if (!(t as SpriteRenderer).shouldSupportTiling)
		   ...
		   ? Styles.notFullRectWarningLabel.text
		   : Styles.notFullRectMultiEditWarningLabel.text;
	}
	return null;
}
...
bool ShouldShowDrawMode() {
	return m_DrawMode.intValue != (int) SpriteDrawMode.Simple &&
	       !m_DrawMode.hasMultipleDifferentValues;
}

bool ShouldShowAdaptiveThreshold() {
	return m_SpriteTileMode.intValue == (int) SpriteTileMode.Adaptive &&
	       !m_SpriteTileMode.hasMultipleDifferentValues;
}

bool ShouldShowTileMode() {
	return m_DrawMode.intValue == (int) SpriteDrawMode.Tiled &&
	       !m_DrawMode.hasMultipleDifferentValues;
}
...
void FlipToggles() {
	...
	Rect r = GUILayoutUtility.GetRect(
		EditorGUIUtility.fieldWidth, EditorGUILayout.kLabelFloatMaxW,
		EditorGUI.kSingleLineHeight, EditorGUI.kSingleLineHeight,
		EditorStyles.numberField);
	int id =
		GUIUtility.GetControlID(Styles.flipToggleHash, FocusType.Keyboard, r);
...
private void ShowMaterialError() {
	if (IsMaterialTextureAtlasConflict())
		ShowError(
			"Material has CanUseSpriteAtlas=False tag. Sprite texture has atlasHint set. Rendering artifacts possible.");
...
private bool IsMaterialTextureAtlasConflict() {
	...
	if (tag.ToLower() == "false") {
		...
		TextureImporter ti = AssetImporter.GetAtPath(
			AssetDatabase.GetAssetPath(frame)) as TextureImporter;
		if (ti != null && ti.spritePackingTag != null &&
		    ti.spritePackingTag.Length > 0) {
			return true;
		}
	}
