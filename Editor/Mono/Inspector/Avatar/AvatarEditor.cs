...
internal class AvatarSubEditor : ScriptableObject {
  ...
  ActiveEditorTracker.MakeCustomEditor(modelImporter) as ModelImporterEditor;

          SerializedProperty humanDescription =
  inspector.serializedObject.FindProperty("m_HumanDescription");

          Avatar avatar =
  AssetDatabase.LoadAssetAtPath("Assets/1_Characters/Dude/Dude.fbx",
  typeof(UnityEngine.Avatar)) as Avatar; if (avatar == null) Debug.Log("Could
  not find avatar when importing : " + modelImporter.assetPath);
  ...
  */
  ...
  Resources.FindObjectsOfTypeAll(typeof(UnityEngine.Object)); foreach
  (UnityEngine.Object asset in objects)
      {
  ...
  protected GameObject gameObject {
    get { return m_Inspector.m_GameObject; }
  }
  protected GameObject prefab {
    get { return m_Inspector.prefab; }
  }
  protected Dictionary<Transform, bool> modelBones {
    get { return m_Inspector.m_ModelBones; }
  }
  protected Transform root {
    get { return gameObject == null ? null : gameObject.transform; }
  }
  protected SerializedObject serializedObject {
    get { return m_Inspector.serializedAssetImporter; }
  }
  protected Avatar avatarAsset {
    get { return m_Inspector.avatar; }
  }
  ...
  public virtual void OnDestroy() {
    if (HasModified()) {
      AssetImporter importer =
          AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(avatarAsset));
      if (importer) {
  ...
  protected virtual void ResetValues() { serializedObject.Update(); }
      ...
      using(new EditorGUI.DisabledScope(!HasModified())) {
        ...
        if (GUILayout.Button("Revert")) {
          ...
          System.Diagnostics.Debug.Assert(
              !HasModified(),
              "Avatar settings are marked as modified after calling Reset.");
        }
...
internal class AvatarEditor : Editor {
  private class Styles {
    ...
    public GUIContent editCharacter =
        EditorGUIUtility.TrTextContent("Configure Avatar");
  ...
  enum EditMode { NotEditing, Starting, Editing, Stopping }
  ...
  // only the FBXImporter part and not mess up with the Editor's
  // serializedObject.
  internal SerializedObject m_SerializedAssetImporter = null;
  public SerializedObject serializedAssetImporter {
    get {
      // TODO find a better sync for that and be in better control of the
      // lifetime
  ...
  // We'd prefer to have just one AvatarEditor member but due to Serialization
  // issues, we need to keep one of each type. It should still be treated as a
  // single one though. I.e. only one should be used at a time; the rest should
  // be null.
  protected AvatarSubEditor editor {
    get {
      switch (m_TabIndex) {
      case sMuscleTab:
        return m_MuscleEditor;
      default:
      case sMappingTab:
        return m_MappingEditor;
      }
    }
    set {
      switch (m_TabIndex) {
      case sMuscleTab:
        m_MuscleEditor = value as AvatarMuscleEditor;
        break;
      default:
      case sMappingTab:
        m_MappingEditor = value as AvatarMappingEditor;
        break;
      }
    }
  }
  ...
  static private SerializedObject
  CreateSerializedImporterForTarget(UnityEngine.Object target) {
    SerializedObject so = null;

    AssetImporter importer =
        AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(target));
    if (importer != null) {
  ...
  void OnEnable() {
    ...
    if (m_EditMode == EditMode.Editing) {
      m_ModelBones =
          AvatarSetupTool.GetModelBones(m_GameObject.transform, false, null);
      editor.Enable(this);
  ...
  void OnDestroy() {
    ...
    if (StageNavigationManager.instance.currentStage is
            AvatarConfigurationStage)
  ...
  protected void CreateEditor() {
    switch (m_TabIndex) {
    case sMuscleTab:
      editor = ScriptableObject.CreateInstance<AvatarMuscleEditor>();
      break;
    default:
    case sMappingTab:
      editor = ScriptableObject.CreateInstance<AvatarMappingEditor>();
      break;
    }
  ...
  (AssetDatabase.GetAssetPath (command.context)); SerializedObject
  serializedObject = new SerializedObject (importer);
      ...
      if (importer && serializedObject != null)
      {
          ...
          SerializedProperty skeleton = serializedObject.FindProperty
  (sSkeleton); if (skeleton != null && skeleton.isArray) skeleton.ClearArray ();
  ...
  public override void OnInspectorGUI() {
    ...
    using(new EditorGUILayout.VerticalScope(
      ...
      if (m_EditMode == EditMode.Editing) {
        ...
        if (m_EditMode == EditMode.NotEditing &&
  ...
  internal void SwitchToEditMode() {
    ...
    AvatarConfigurationStage stage =
        AvatarConfigurationStage.CreateStage(assetPath, this);
    ...
    SerializedProperty humanBoneArray =
        serializedAssetImporter.FindProperty("m_HumanDescription.m_Human");

    // First get all available modelBones
    Dictionary<Transform, bool> modelBones =
        AvatarSetupTool.GetModelBones(m_GameObject.transform, true, null);
    AvatarSetupTool.BoneWrapper[] humanBones =
        AvatarSetupTool.GetHumanBones(humanBoneArray, modelBones);

    m_ModelBones = AvatarSetupTool.GetModelBones(m_GameObject.transform, false,
                                                 humanBones);

    // Unfold all nodes in hierarchy
    // TODO@MECANIM: Only expand actual bones
    foreach (SceneHierarchyWindow shw in Resources.FindObjectsOfTypeAll(
  ...
  internal void SwitchToAssetMode() { StageUtility.GoToMainStage(); }

  internal void CleanupEditor() {
    bool selectAvatarAsset = StageUtility.GetCurrentStageHandle() ==
    ...
    CleanUpOnDestroy = () => {
      // Make sure that we restore the "original" selection if we exit Avatar
      // Editing mode from the avatar tooling itself (e.g by clicking done).
    ...
    // Reset back the Edit Mode specific states (they probably should be better
    // encapsulated)
    m_GameObject = null;
    m_ModelBones = null;
  }
  ...
  public Bounds OnGetFrameBounds() {
    ...
    if (bone.parent)
      bounds.Encapsulate(bone.parent.position);
