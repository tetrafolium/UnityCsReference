...
internal static class AvatarSetupTool {
  ...
  internal class BoneWrapper {
    ...
    public string messageName {
      get {
        return ObjectNames.NicifyVariableName(m_HumanBoneName) +
               " Transform '" +
               (bone ? bone.name
                : "None") +
               "'";
    ...
    public void Reset(SerializedProperty humanBoneArray,
                      Dictionary<Transform, bool> bones) {
      bone = null;
      SerializedProperty property =
          GetSerializedProperty(humanBoneArray, false);
      if (property != null) {
    ...
    public void Serialize(SerializedProperty humanBoneArray) {
      if (bone == null) {
        ...
        SerializedProperty property =
            GetSerializedProperty(humanBoneArray, true);
        if (property != null)
    ...
    protected void DeleteSerializedProperty(SerializedProperty humanBoneArray) {
      ...
      for (int i = 0; i < humanBoneArray.arraySize; i++) {
        SerializedProperty humanNameP =
            humanBoneArray.GetArrayElementAtIndex(i).FindPropertyRelative(
                sHumanName);
    ...
    GetSerializedProperty(SerializedProperty humanBoneArray,
      ...
      if (createIfMissing) {
        ...
        SerializedProperty bone =
            humanBoneArray.GetArrayElementAtIndex(humanBoneArray.arraySize - 1);
        if (bone != null) {
    ...
    public void BoneDotGUI(Rect rect, Rect selectRect, int boneIndex,
                           bool doClickSelect, bool doDragDrop,
                           bool doDeleteKey, SerializedObject serializedObject,
                           AvatarMappingEditor editor) {
    ...
    public void HandleClickSelection(int keyboardID, Rect selectRect,
                                     int boneIndex) {
      Event evt = Event.current;
      if (evt.type == EventType.MouseDown &&
          selectRect.Contains(evt.mousePosition)) {
    ...
    public void HandleDeleteSelection(int keyboardID,
      ...
      if (evt.type == EventType.KeyDown) {
        if (GUIUtility.keyboardControl == keyboardID) {
          if ((evt.keyCode == KeyCode.Backspace) ||
            ...
            SerializedProperty humanBoneArray =
                serializedObject.FindProperty(sHuman);
    ...
    private void HandleDragDrop(Rect dropRect, int boneIndex, int id,
                                SerializedObject serializedObject,
                                AvatarMappingEditor editor) {
      ...
      case EventType.DragPerform:
        if (dropRect.Contains(Event.current.mousePosition) && GUI.enabled) {
          ...
          if (validatedObject != null) {
            if (!(validatedObject is Transform ||
                  validatedObject is GameObject) ||
                EditorUtility.IsPersistent(validatedObject))
              validatedObject = null;
          }
          if (validatedObject != null) {
            DragAndDrop.visualMode = DragAndDropVisualMode.Generic;
            if (eventType == EventType.DragPerform) {
              Undo.RegisterCompleteObjectUndo(editor,
                                              "Avatar mapping modified");
  ...
  private class BonePoseData {
    ...
    public BonePoseData(Vector3 dir, bool globalSpace, float maxAngleDiff,
                        Vector3 planeNormal, int[] children)
  ...
  private static BonePoseData[] sBonePoses = new BonePoseData[]{
      ...
      new BonePoseData(new Vector3(0.05f, -1, -0.15f), true,
                       20), // RightLowerLeg,
      new BonePoseData(new Vector3(-0.05f, 0, 1), true, 20, Vector3.up,
                       null), // LeftFoot,
      new BonePoseData(new Vector3(0.05f, 0, 1), true, 20, Vector3.up,
                       null), // RightFoot,
      new BonePoseData(Vector3.up, true, 30,
                       new int[]{(int) HumanBodyBones.Chest,
                                 (int) HumanBodyBones.UpperChest,
                                 (int) HumanBodyBones.Neck,
                                 (int) HumanBodyBones.Head}), // Spine,
      new BonePoseData(Vector3.up, true, 30,
                       new int[]{(int) HumanBodyBones.UpperChest,
                                 (int) HumanBodyBones.Neck,
                                 (int) HumanBodyBones.Head}), // Chest,
      ...
      new BonePoseData(
          -Vector3.right, false, 10, Vector3.forward,
          new int[]{(int) HumanBodyBones.LeftMiddleProximal}), // LeftHand,
      new BonePoseData(
          Vector3.right, false, 10, Vector3.forward,
          new int[]{(int) HumanBodyBones.RightMiddleProximal}), // RightHand,
      null,                                                     // LeftToes,
      null,                                                     // RightToes,
      null,                                                     // LeftEye,
      null,                                                     // RightEye,
      ...
      new BonePoseData(Vector3.right, false, 05),
      new BonePoseData(Vector3.up, true, 30,
                       new int[]{(int) HumanBodyBones.Neck,
                                 (int) HumanBodyBones.Head}), // UpperChest,
  };

  public static Dictionary<Transform, bool>
  GetModelBones(Transform root, bool includeAll, BoneWrapper[] humanBones) {
    if (root == null)
      return null;

    // Find out which transforms are actual bones and which are parents of
    // actual bones
    ...
    // If not enough bones were found, fallback to treating all transforms as
    // bones
  ...
  private static bool DetermineIsActualBone(Transform tr,
                                            Dictionary<Transform, bool> bones,
      ...
      if (components.Length > 1) {
        foreach (Component comp in components) {
          if ((comp is Renderer) && !(comp is SkinnedMeshRenderer)) {
            ...
            if (tr.childCount == 0 && tr.parent &&
                bounds.Contains(tr.parent.position)) {
  ...
  public static int GetFirstHumanBoneAncestor(BoneWrapper[] bones,
                                              int boneIndex) {
  ...
  public static BoneWrapper[] GetHumanBones(
      SerializedProperty humanBoneArray,
      Dictionary<Transform, bool> actualBones) {
    // cache the (human bone name, bone name) mapping,
    // since accessing a dictionary is much faster than iterating over the
  ...
  public static void ClearAll(SerializedProperty humanBoneArray,
                              SerializedProperty humanSkeletonArray) {
  ...
  public static bool
  IsPoseValidOnInstance(GameObject modelPrefab,
                        SerializedObject modelImporterSerializedObject) {
    GameObject instance = GameObject.Instantiate(modelPrefab) as GameObject;
    instance.hideFlags = HideFlags.HideAndDontSave;

    Dictionary<Transform, bool> modelBones =
        GetModelBones(instance.transform, false, null);
    SerializedProperty humanBoneArray =
        modelImporterSerializedObject.FindProperty(sHuman);
    BoneWrapper[] humanBones = GetHumanBones(humanBoneArray, modelBones);

    TransferDescriptionToPose(modelImporterSerializedObject,
                              instance.transform);
  ...
  ///
  /// We need to call this method when importing humanoids, in order to have a
  /// valid avatar during the import process.
  /// </summary>
  [RequiredByNativeCode]
  private static void SetupHumanSkeleton(GameObject modelPrefab,
    ...
    Dictionary<Transform, bool> modelBones =
        GetModelBones(modelInstance.transform, false, null);
    SimpleProfiler.End();
    ...
    if (humanBoneMappingArray != null && humanBoneMappingArray.Length > 0) {
      ...
      for (var i = 0; i < humanBoneMappingArray.Length; ++i)
        existingMappings[humanBoneMappingArray[i].humanName] =
            humanBoneMappingArray[i].boneName;
    }
    ...
    for (int i = 0; i < humanBones.Length; ++i) {
      ...
      if (mapping.ContainsKey(i)) {
        humanBoneMappings.Add(new HumanBone(){
            boneName = mapping[i].name, humanName = bone.humanBoneName,
    ...
    if (!isBiped) {
      ...
      // If the bind pose was better, sample the bind pose again to use that as
      // a starting point for the T-pose. Otherwise use the reset pose as
      // starting point.
  ...
  public static bool TestAndValidateAutoSetup(GameObject modelAsset) {
    ...
    if (!(PrefabUtility.IsPartOfModelPrefab(modelAsset) &&
          PrefabUtility.IsPartOfPrefabAsset(modelAsset))) {
      Debug.LogError(modelAsset.name + ": GameObject is not a ModelPrefab",
                     modelAsset);
      return false;
    }
    if (modelAsset.transform.parent != null) {
      Debug.LogError(modelAsset.name + ": GameObject is not the root",
                     modelAsset);
      return false;
    }
    ...
    if (importer == null) {
      Debug.LogError(modelAsset.name +
                         ": Could not load ModelImporter (path:" + path + ")",
                     modelAsset);
      return false;
    }
    ...
    using(var serializedObject = new SerializedObject(importer)) {
      ...
      SerializedProperty humanSkeletonArray =
          serializedObject.FindProperty(sSkeleton);
      SerializedProperty animationType =
          serializedObject.FindProperty("m_AnimationType");
      if (animationType == null) {
        Debug.LogError(
            modelAsset.name +
                ": Could not find property m_AnimationType on ModelImporter",
            modelAsset);
        return false;
      }
      ...
      Avatar avatar =
          AssetDatabase.LoadAssetAtPath(path, typeof(Avatar)) as Avatar;
      if (avatar == null) {
        Debug.LogError(
            modelAsset.name +
                ": Did not find Avatar after reimport with CreateAvatar enabled",
            modelAsset);
        return false;
      }
      if (!avatar.isHuman) {
        Debug.LogError(modelAsset.name + ": Avatar is not valid after reimport",
                       modelAsset);
        return false;
      }
      if (!IsPoseValidOnInstance(modelAsset, serializedObject)) {
        Debug.LogError(modelAsset.name +
                           ": Avatar has invalid pose after reimport",
                       modelAsset);
        return false;
      }

      // Check that mapping matches reference mapping
      string pathWithoutExtension = path.Substring(
          0, path.Length - System.IO.Path.GetExtension(path).Length);

      // Get reference template
      string templatePath = pathWithoutExtension + ".ht";
      HumanTemplate humanTemplate =
          AssetDatabase.LoadMainAssetAtPath(templatePath) as HumanTemplate;
      if (humanTemplate == null)
        Debug.LogWarning(modelAsset.name + ": Didn't find template at path " +
                         templatePath);
      else {
        ...
        if (System.IO.File.Exists(ignorePath))
          ignoreBones =
              new List<string>(System.IO.File.ReadAllLines(ignorePath));
        ...
        for (int i = 0; i < humanBones.Length; i++) {
          ...
          if (ignoreBones != null &&
              ignoreBones.Contains(humanBones[i].humanBoneName))
            continue;

          string referenceBoneName =
              humanTemplate.Find(humanBones[i].humanBoneName);
          string actualBoneName = (humanBones[i].bone == null ? ""
                                   : humanBones[i].bone.name);
          if (!AvatarMappingEditor.MatchName(actualBoneName,
                                             referenceBoneName)) {
            mismatch = true;
            Debug.LogError(modelAsset.name + ": Avatar has bone " +
                               humanBones[i].humanBoneName + " mapped to \"" +
                               actualBoneName + "\" but expected \"" +
                               referenceBoneName + "\"",
                           modelAsset);
          }
        }
  ...
  public static void DebugTransformTree(Transform tr,
                                        Dictionary<Transform, bool> bones,
                                        int level) {
    string pre = "  ";
    if (bones.ContainsKey(tr))
  ...
  FindSkeletonBone(SerializedObject serializedObject, Transform t,
                   bool createMissing, bool isRoot) {
    SerializedProperty skeletonBoneArray =
        serializedObject.FindProperty(sSkeleton);
  ...
  public static SerializedProperty
  FindSkeletonBone(SerializedProperty skeletonBoneArray, Transform t,
                   bool createMissing, bool isRoot) {
    ...
    if (createMissing) {
      ...
      SerializedProperty bone = skeletonBoneArray.GetArrayElementAtIndex(
          skeletonBoneArray.arraySize - 1);
      if (bone != null) {
        bone.FindPropertyRelative(sName).stringValue = t.name;
        bone.FindPropertyRelative(sParentName).stringValue =
            isRoot ? "" : t.parent.name;
  ...
  public static void GetBindPoseBonePositionRotation(Matrix4x4 skinMatrix,
    ...
    Vector3 mX = new Vector3(bindMatrixGlobal.m00, bindMatrixGlobal.m10,
                             bindMatrixGlobal.m20);
    Vector3 mY = new Vector3(bindMatrixGlobal.m01, bindMatrixGlobal.m11,
                             bindMatrixGlobal.m21);
    Vector3 mZ = new Vector3(bindMatrixGlobal.m02, bindMatrixGlobal.m12,
                             bindMatrixGlobal.m22);
    Vector3 mP = new Vector3(bindMatrixGlobal.m03, bindMatrixGlobal.m13,
                             bindMatrixGlobal.m23);

    // Set position
    // Adjust scale of matrix to compensate for difference in binding scale and
    // model scale
  ...
  public static void SampleBindPose(GameObject go) {
    ...
    foreach (SkinnedMeshRenderer skin in skins) {
      ...
      // Set all parents to be null to be able to set global alignments of bones
      // without affecting their children.
      Dictionary<Transform, Transform> parents =
      ...
      for (int i = 0; i < bones.Count; i++) {
        Vector3 position;
        Quaternion rotation;
        GetBindPoseBonePositionRotation(goMatrix, skin.sharedMesh.bindposes[i],
                                        bones[i], out position, out rotation);
  ...
  public static void ShowBindPose(SkinnedMeshRenderer skin) {
    ...
    for (int i = 0; i < skin.bones.Length; i++) {
      ...
      Quaternion rotation;
      GetBindPoseBonePositionRotation(goMatrix, skin.sharedMesh.bindposes[i],
                                      skin.bones[i], out position,
                                      out rotation);
      float size = HandleUtility.GetHandleSize(position);
      Handles.color = Handles.xAxisColor;
      Handles.DrawLine(position,
                       position + rotation * Vector3.right * 0.3f * size);
      Handles.color = Handles.yAxisColor;
      Handles.DrawLine(position,
                       position + rotation * Vector3.up * 0.3f * size);
      Handles.color = Handles.zAxisColor;
      Handles.DrawLine(position,
                       position + rotation * Vector3.forward * 0.3f * size);
    }
  }
  ...
  public static void
  TransferPoseToDescription(SerializedProperty skeletonBoneArray,
                            Transform root) {
  ...
  private static void
  TransferPoseToDescription(Transform transform, bool isRoot,
  ...
  public static void
  TransferDescriptionToPose(SerializedObject serializedObject, Transform root) {
    if (root != null)
      TransferDescriptionToPose(serializedObject, root, true);
  }

  private static void
  TransferDescriptionToPose(SerializedObject serializedObject,
                            Transform transform, bool isRoot) {
    SerializedProperty bone =
        FindSkeletonBone(serializedObject, transform, false, isRoot);
    if (bone != null) {
  ...
  public static void MakePoseValid(BoneWrapper[] bones) {
    ...
    for (int i = 0; i < sBonePoses.Length; i++) {
      ...
      // Recalculate orientation after handling hips since they may have changed
      // it
  ...
  public static float GetBoneAlignmentError(BoneWrapper[] bones,
    ...
    if (boneIndex == (int) HumanBodyBones.Hips) {
      float angleX =
          Vector3.Angle(avatarOrientation * Vector3.right, Vector3.right);
      float angleY = Vector3.Angle(avatarOrientation * Vector3.up, Vector3.up);
      float angleZ =
          Vector3.Angle(avatarOrientation * Vector3.forward, Vector3.forward);
      return Mathf.Max(0, Mathf.Max(angleX, angleY, angleZ) - pose.maxAngle);
    }

    Vector3 dir =
        GetBoneAlignmentDirection(bones, avatarOrientation, boneIndex);
    if (dir == Vector3.zero)
      return 0;
  ...
  public static void MakeBoneAlignmentValid(BoneWrapper[] bones,
                                            Quaternion avatarOrientation,
                                            int boneIndex) {
    if (boneIndex < 0 || boneIndex >= sBonePoses.Length ||
    ...
    if (boneIndex == (int) HumanBodyBones.Hips) {
      ...
      float angleZ =
          Vector3.Angle(avatarOrientation * Vector3.forward, Vector3.forward);
      if (angleX > pose.maxAngle || angleY > pose.maxAngle ||
          angleZ > pose.maxAngle)
        bone.bone.rotation =
            Quaternion.Inverse(avatarOrientation) * bone.bone.rotation;
      return;
    }
    ...
    if (deltaAngle > pose.maxAngle * 0.99f) {
      ...
      // If this bone is hip or knee, remember global foor rotation and apply it
      // after this adjustment
      Transform footBone = null;
      Quaternion footRot = Quaternion.identity;
      if (boneIndex == (int) HumanBodyBones.LeftUpperLeg ||
          boneIndex == (int) HumanBodyBones.LeftLowerLeg)
        footBone = bones[(int) HumanBodyBones.LeftFoot].bone;
      if (boneIndex == (int) HumanBodyBones.RightUpperLeg ||
          boneIndex == (int) HumanBodyBones.RightLowerLeg)
  ...
  private static Quaternion GetRotationSpace(BoneWrapper[] bones,
                                             Quaternion avatarOrientation,
                                             int boneIndex) {
    ...
    if (!pose.compareInGlobalSpace) {
      ...
      if (parentIndex > 0) {
        ...
        if (bones[parentIndex].bone != null && parentPose != null) {
          Vector3 parentDir =
              GetBoneAlignmentDirection(bones, avatarOrientation, parentIndex);
  ...
  private static Vector3 GetBoneAlignmentDirection(BoneWrapper[] bones,
                                                   Quaternion avatarOrientation,
                                                   int boneIndex) {
    ...
    if (childBoneIndex >= 0 && bones[childBoneIndex] != null &&
        bones[childBoneIndex].bone != null) {
      ...
      // TODO@MECANIM Something si wrong with the indexes
      // if (boneIndex == (int)HumanBodyBones.LeftHand)
      //  Debug.Log (" - "+childBone.humanBoneName + " - "
      //  +childBone.bone.name);
    } else {
  ...
  public static Quaternion AvatarComputeOrientation(BoneWrapper[] bones) {
    ...
    if (leftUpLeg != null && rightUpLeg != null && leftArm != null &&
        rightArm != null)
      return AvatarComputeOrientation(leftUpLeg.position, rightUpLeg.position,
                                      leftArm.position, rightArm.position);
    else
      return Quaternion.identity;
  }

  public static Quaternion AvatarComputeOrientation(Vector3 leftUpLeg,
                                                    Vector3 rightUpLeg,
                                                    Vector3 leftArm,
                                                    Vector3 rightArm) {
    ...
    // If the orientation is sensible, assume character up vector is aligned
    // along x, y, or z axis, so fix it to closest axis
  ...
  private static Vector3 GetCharacterPositionAdjustVector(BoneWrapper[] bones,
                                                          out float error) {
    error = 0;
    ...
    // But the feet might be at a perfect height from the start if the bind pose
    // is good. So only do it if the feet look like they're not at a good
    // position from the beginning. Check if feet are already at height that
    // looks about right.
    if (avgFootPos.y < 0 ||
        avgFootPos.y > hipsHeight * (usingToes ? 0.1f : 0.3f)) {
      // Current height is not good, so adjust it using best guess based on
      // human anatomy.
      float estimatedFootBottomHeight =
          avgHipPos.y - hipsHeight * (usingToes ? 1.03f : 1.13f);
      adjustVector.y = -estimatedFootBottomHeight;
    }

    // Move the avg hip pos to the center on the left-right axis if it's not
    // already there.
    if (Mathf.Abs(avgHipPos.x) > 0.01f * hipsHeight)
      adjustVector.x = -avgHipPos.x;

    // Move the avg hip pos to the center on the front-back axis if it's not
    // already approximately there.
