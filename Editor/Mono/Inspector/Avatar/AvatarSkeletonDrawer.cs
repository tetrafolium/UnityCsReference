...
internal class AvatarSkeletonDrawer {
  ...
  private static Color kSkeletonColor =
      new Color(103.0f / 255.0f, 103.0f / 255.0f, 103.0f / 255.0f, 0.5f);
  private static Color kDummyColor =
      new Color(60.0f / 255.0f, 60.0f / 255.0f, 60.0f / 255.0f, 0.5f);
  private static Color kHumanColor =
      new Color(0, 210.0f / 255.0f, 74.0f / 255.0f, 0.5f);
  private static Color kErrorColor = new Color(1, 0, 0, 0.5f);
  private static Color kErrorMessageColor = new Color(1, 0, 0, 0.75f);
  private static Color kSelectedColor =
      new Color(128.0f / 255.0f, 192.0f / 255.0f, 255.0f / 255.0f, 0.30f);

  public static void DrawSkeleton(Transform reference,
                                  Dictionary<Transform, bool> actualBones,
                                  Handles.BoneRenderer boneRenderer) {
  ...
  public static void DrawSkeleton(Transform reference,
                                  ...
                                  Handles.BoneRenderer boneRenderer) {
    // it can happen when the avatar tool is in edit mode and the user exit the
    // tool in an unsual way
    //  new scene
    //  delete GO
    //  press play
    ...
    if (sPoseError && camera != null) {
      ...
      GUIContent content =
          EditorGUIUtility.TrTextContent("Character is not in T pose");
  ...
  private static bool
  DrawSkeletonSubTree(Dictionary<Transform, bool> actualBones,
                      AvatarSetupTool.BoneWrapper[] bones,
                      Quaternion orientation, Transform tr, Bounds bounds,
                      Handles.BoneRenderer boneRenderer) {
    ...
    // There is no need to check for a pose error if the avatar is not yet valid
    // or tools is not the active one
    bool poseError =
        AvatarSetupTool.GetBoneAlignmentError(bones, orientation, index) > 0;
