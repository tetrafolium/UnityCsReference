...
public GUIContent armTwist =
	EditorGUIUtility.TrTextContent("Upper Arm Twist");
public GUIContent foreArmTwist =
	EditorGUIUtility.TrTextContent("Lower Arm Twist");
public GUIContent upperLegTwist =
	EditorGUIUtility.TrTextContent("Upper Leg Twist");
public GUIContent legTwist =
	EditorGUIUtility.TrTextContent("Lower Leg Twist");
public GUIContent armStretch =
	EditorGUIUtility.TrTextContent("Arm Stretch");
public GUIContent legStretch =
	EditorGUIUtility.TrTextContent("Leg Stretch");
public GUIContent feetSpacing =
	EditorGUIUtility.TrTextContent("Feet Spacing");
public GUIContent hasTranslationDoF =
	EditorGUIUtility.TrTextContent("Translation DoF");
...
protected int[][] m_MasterMuscle = {
	...
	// In Out
	new int[] {(int) Dof.LeftLegDofStart + (int) LegDof.UpperLegInOut,
		   (int) Dof.LeftLegDofStart + (int) LegDof.FootInOut,
		   (int) Dof.RightLegDofStart + (int) LegDof.UpperLegInOut,
		   (int) Dof.RightLegDofStart + (int) LegDof.FootInOut,
		   (int) Dof.LeftArmDofStart + (int) ArmDof.ShoulderFrontBack,
	...
	// Roll In Out
	new int[] {(int) Dof.LeftLegDofStart + (int) LegDof.UpperLegRollInOut,
		   (int) Dof.LeftLegDofStart + (int) LegDof.LegRollInOut,
		   (int) Dof.RightLegDofStart + (int) LegDof.UpperLegRollInOut,
		   (int) Dof.RightLegDofStart + (int) LegDof.LegRollInOut,
		   (int) Dof.LeftArmDofStart + (int) ArmDof.ArmRollInOut,
...
[SerializeField] float[] m_MuscleValue = null;

[SerializeField] float[] m_MuscleMasterValue = null;
...
[SerializeField] float[] m_MuscleMinEdit = null;

[SerializeField] float[] m_MuscleMaxEdit = null;
...
internal void ResetValuesFromProperties() {
	...
	for (int i = 0; i < m_Bones.Length; i++) {
		if (m_Modified[i] != null) {
			...
			if (dx != -1) {
				m_MuscleMinEdit[dx] =
					modified ? m_MuscleMin[dx].floatValue
	      : HumanTrait.GetMuscleDefaultMin(dx);
				m_MuscleMaxEdit[dx] =
					modified ? m_MuscleMax[dx].floatValue
	      : HumanTrait.GetMuscleDefaultMax(dx);
			}

			if (dy != -1) {
				m_MuscleMinEdit[dy] =
					modified ? m_MuscleMin[dy].floatValue
	      : HumanTrait.GetMuscleDefaultMin(dy);
				m_MuscleMaxEdit[dy] =
					modified ? m_MuscleMax[dy].floatValue
	      : HumanTrait.GetMuscleDefaultMax(dy);
			}

			if (dz != -1) {
				m_MuscleMinEdit[dz] =
					modified ? m_MuscleMin[dz].floatValue
	      : HumanTrait.GetMuscleDefaultMin(dz);
				m_MuscleMaxEdit[dz] =
					modified ? m_MuscleMax[dz].floatValue
	      : HumanTrait.GetMuscleDefaultMax(dz);
			}
		}
	}
}
...
internal void InitializeProperties() {
	...
	m_HasTranslationDoFProperty =
		serializedObject.FindProperty(sHasTranslationDoF);
	m_HumanBoneArrayProperty = serializedObject.FindProperty(sHumanBoneArray);

	for (int i = 0; i < m_Bones.Length; i++) {
		SerializedProperty bone =
			m_Bones [i]
			.GetSerializedProperty(m_HumanBoneArrayProperty, false);
		if (bone != null) {
...
internal void Initialize() {
	...
	if (m_Bones == null)
		m_Bones =
			AvatarSetupTool.GetHumanBones(m_HumanBoneArrayProperty, modelBones);
...
protected void MuscleGroupGUI() {
	...
	GUILayout.BeginVertical(styles.box);
	{
		...
		for (int i = 0; i < m_MasterMuscle.Length; i++) {
			...
			m_MuscleMasterValue [i]
			        = PreviewSlider(r, m_MuscleMasterValue[i]);
			if (m_MuscleMasterValue[i] != oldValue) {
				...
				for (int j = 0; j < m_MasterMuscle[i].Length; j++) {
					if (m_MasterMuscle [i]
					    [j] != -1) {
						m_MuscleValue [m_MasterMuscle [i]
						               [j]
						]
						        = m_MuscleMasterValue[i];
					}
			...
			recomputePose |=
				m_MuscleMasterValue[i] != oldValue && gameObject != null;
...
protected void MuscleGUI() {
	...
	GUILayout.BeginVertical(styles.box);
	{
		...
		for (int i = 0; i < m_MuscleBodyGroupToggle.Length; i++) {
			...
			m_MuscleBodyGroupToggle[i] =
				GUI.Toggle(settingsRect, m_MuscleBodyGroupToggle[i],
				           styles.muscleBodyGroup[i], EditorStyles.foldout);
			if (m_MuscleBodyGroupToggle[i]) {
				for (int j = 0; j < m_Muscles[i].Length; j++) {
					int muscleId = m_Muscles [i]
					               [j];
					// Some muscle can be optionnal like Toes, if this bone is not
					// characterized you can't edit this muscle
					if (muscleId != -1 && m_MuscleMin[muscleId] != null &&
					    m_MuscleMax[muscleId] != null) {
						bool expanded = m_MuscleToggle[muscleId];

						r = GUILayoutUtility.GetRect(10, expanded ? kLineHeight * 2
					   : kLineHeight);
						settingsRect = GetSettingsRect(r);
						...
						m_MuscleToggle[muscleId] =
							GUI.Toggle(settingsRect, m_MuscleToggle[muscleId],
							           m_MuscleName[muscleId], EditorStyles.foldout);
						...
						if (expanded) {
							...
							if (settingsRect.width > 160) {
								...
								m_MuscleMinEdit[muscleId] = EditorGUI.FloatField(
									numberRect, m_MuscleMinEdit[muscleId]);
								muscleChanged |= EditorGUI.EndChangeCheck();

								numberRect.x = settingsRect.xMax - kNumberWidth;

								EditorGUI.BeginChangeCheck();
								m_MuscleMaxEdit[muscleId] = EditorGUI.FloatField(
									numberRect, m_MuscleMaxEdit[muscleId]);
								muscleChanged |= EditorGUI.EndChangeCheck();
							...
							EditorGUI.MinMaxSlider(
								sliderRect, ref m_MuscleMinEdit[muscleId],
								ref m_MuscleMaxEdit[muscleId], sMuscleMin, sMuscleMax);
							muscleChanged |= EditorGUI.EndChangeCheck();

							if (muscleChanged) {
								m_MuscleMinEdit[muscleId] =
									Mathf.Clamp(m_MuscleMinEdit[muscleId], sMuscleMin, 0);
								m_MuscleMaxEdit[muscleId] =
									Mathf.Clamp(m_MuscleMaxEdit[muscleId], 0, sMuscleMax);
								recomputePose |=
									UpdateMuscle(muscleId, m_MuscleMinEdit[muscleId],
									             m_MuscleMaxEdit[muscleId]);
							}
						}
					}
...
protected void PropertiesGUI() {
	...
	{
		m_ArmTwistFactor =
			EditorGUI.Slider(GetSettingsRect(), styles.armTwist, m_ArmTwistFactor,
			                 0, 1, EditorStyles.miniTextField);
		...
		m_ForeArmTwistFactor = EditorGUI.Slider(
			GetSettingsRect(), styles.foreArmTwist, m_ForeArmTwistFactor, 0, 1,
		...
		if (m_ForeArmTwistProperty.floatValue != m_ForeArmTwistFactor) {
			...
			UpdateAvatarParameter(HumanParameter.LowerArmTwist,
			                      m_ForeArmTwistFactor);
			recomputePose = true;
		}

		m_UpperLegTwistFactor = EditorGUI.Slider(
			GetSettingsRect(), styles.upperLegTwist, m_UpperLegTwistFactor, 0, 1,
			EditorStyles.miniTextField);
		if (m_UpperLegTwistProperty.floatValue != m_UpperLegTwistFactor) {
			...
			UpdateAvatarParameter(HumanParameter.UpperLegTwist,
			                      m_UpperLegTwistFactor);
			recomputePose = true;
		}

		m_LegTwistFactor =
			EditorGUI.Slider(GetSettingsRect(), styles.legTwist, m_LegTwistFactor,
			                 0, 1, EditorStyles.miniTextField);
		...
		m_ArmStretchFactor = EditorGUI.Slider(
			GetSettingsRect(), styles.armStretch, m_ArmStretchFactor, 0, 1,
			EditorStyles.miniTextField);
		...
		m_LegStretchFactor = EditorGUI.Slider(
			GetSettingsRect(), styles.legStretch, m_LegStretchFactor, 0, 1,
			EditorStyles.miniTextField);
		...
		m_FeetSpacingFactor = EditorGUI.Slider(
			GetSettingsRect(), styles.feetSpacing, m_FeetSpacingFactor, 0, 1,
			EditorStyles.miniTextField);
		...
		m_HasTranslationDoF = EditorGUI.Toggle(
			GetSettingsRect(), styles.hasTranslationDoF, m_HasTranslationDoF);
...
public void DrawMuscleHandle(Transform t, int humanId) {
	...
	if (mx != -1) {
		Quaternion zyPostQ =
			avatar.GetZYPostQ(humanId, t.parent.rotation, t.rotation);
	...
	if (my != -1) {
		...
		Handles.DrawSolidArc(t.position, normal, from, maxy - miny,
		                     axisLen * 0.25f);
	}
	if (mz != -1) {
		...
		Handles.DrawSolidArc(t.position, normal, from, maxz - minz,
		                     axisLen * 0.25f);
	}
}
