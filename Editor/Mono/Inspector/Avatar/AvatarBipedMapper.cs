...
internal class AvatarBipedMapper {
  ...
  private static bool MapBipedBones(Transform root,
                                    ref Transform[] humanToTransform,
                                    List<string> report) {
    for (int bipedBoneIter = 0; bipedBoneIter < s_BipedBones.Length;
         bipedBoneIter++) {
      ...
      bool parentRequired =
          parentIndex != -1 ? HumanTrait.RequiredBone(parentIndex) : true;

      Transform parentTransform =
          parentIndex != -1 ? humanToTransform[parentIndex] : root;
      ...
      if (parentTransform == null && !parentRequired) {
        ...
        parentRequired =
            parentIndex != -1 ? HumanTrait.RequiredBone(parentIndex) : true;
        parentTransform =
            parentIndex != -1 ? humanToTransform[parentIndex] : null;
      ...
      humanToTransform [boneIndex]
      = MapBipedBone(bipedBoneIter, parentTransform, parentTransform, report);
  ...
  private static Transform MapBipedBone(int bipedBoneIndex, Transform transform,
                                        Transform parentTransform,
                                        List<string> report) {
    Transform ret = null;
    ...
    if (transform != null) {
      ...
      for (int childIter = 0; ret == null && childIter < childCount;
           childIter++) {
        ...
        if (transform.GetChild(childIter).name.EndsWith(boneName)) {
          ...
          if (ret != null && report != null &&
              boneIndex != (int) HumanBodyBones.Hips &&
              transform != parentTransform) {
            string current = "- Invalid parent for " + ret.name +
                             ". Expected " + parentTransform.name +
                             ", but found " + transform.name + ".";

            if (boneIndex == (int) HumanBodyBones.LeftUpperLeg ||
                boneIndex == (int) HumanBodyBones.RightUpperLeg) {
              current += " Disable Triangle Pelvis";
            } else if (boneIndex == (int) HumanBodyBones.LeftShoulder ||
                       boneIndex == (int) HumanBodyBones.RightShoulder) {
      ...
      for (int childIter = 0; ret == null && childIter < childCount;
           childIter++) {
        ret = MapBipedBone(bipedBoneIndex, transform.GetChild(childIter),
                           parentTransform, report);
      }
    }
  ...
  internal static void BipedPose(GameObject go,
