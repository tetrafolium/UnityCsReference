...
public GUIContent RequiredBone =
	EditorGUIUtility.TrTextContent("Optional Bones");
...
public GUIContent sampleBindPose =
	EditorGUIUtility.TrTextContent("Sample Bind-Pose");
public GUIContent enforceTPose =
	EditorGUIUtility.TrTextContent("Enforce T-Pose");
public GUIContent bipedPose = EditorGUIUtility.TrTextContent("Biped Pose");

public GUIContent ShowError =
	EditorGUIUtility.TrTextContent("Show Error(s)...");
public GUIContent CloseError =
	EditorGUIUtility.TrTextContent("Close Error(s)");

public GUIContent dotFill =
	EditorGUIUtility.IconContent("AvatarInspector/DotFill");
public GUIContent dotFrame =
	EditorGUIUtility.IconContent("AvatarInspector/DotFrame");
public GUIContent dotFrameDotted =
	EditorGUIUtility.IconContent("AvatarInspector/DotFrameDotted");
public GUIContent dotSelection =
	EditorGUIUtility.IconContent("AvatarInspector/DotSelection");
...
private readonly Handles.BoneRenderer m_BoneRenderer =
	new Handles.BoneRenderer();

// This list containt the mecanim's human bones id for each body part
protected int[][] m_BodyPartHumanBone = {
	new int[] {-1},
	new int[] {(int) HumanBodyBones.Hips, (int) HumanBodyBones.Spine,
		   (int) HumanBodyBones.Chest, (int) HumanBodyBones.UpperChest},
	new int[] {(int) HumanBodyBones.Neck, (int) HumanBodyBones.Head,
		   (int) HumanBodyBones.LeftEye, (int) HumanBodyBones.RightEye,
		   (int) HumanBodyBones.Jaw},
	new int[] {
		(int) HumanBodyBones.LeftShoulder, (int) HumanBodyBones.LeftUpperArm,
		(int) HumanBodyBones.LeftLowerArm, (int) HumanBodyBones.LeftHand
	},
	new int[] {(int) HumanBodyBones.LeftThumbProximal,
	...
	new int[] {(int) HumanBodyBones.LeftUpperLeg,
		   (int) HumanBodyBones.LeftLowerLeg,
		   (int) HumanBodyBones.LeftFoot, (int) HumanBodyBones.LeftToes},
	new int[] {(int) HumanBodyBones.RightUpperLeg,
...
protected void Init() {
	...
	m_HumanBoneArray =
		serializedObject.FindProperty("m_HumanDescription.m_Human");
	m_Skeleton = serializedObject.FindProperty("m_HumanDescription.m_Skeleton");
	m_AutoGenerateAvatarMappingIfUnspecified = serializedObject.FindProperty(
	...
	m_HasSkinnedMesh =
		(gameObject.GetComponentInChildren<SkinnedMeshRenderer>() != null);
...
protected void ResetBones() {
	for (int i = 0; i < m_Bones.Length; i++)
		m_Bones [i]
		.Reset(m_HumanBoneArray, modelBones);
}
...
private void HandleUndoPerformed() {
	if (Event.current.type == EventType.ValidateCommand &&
	    Event.current.commandName == EventCommandNames.UndoRedoPerformed) {
		SetupInternalProperties();

		AvatarSetupTool.TransferPoseToDescription(m_Skeleton, root);
		for (int i = 0; i < m_Bones.Length; i++)
			m_Bones [i]
			.Serialize(m_HumanBoneArray);
	}
}
...
public override void OnInspectorGUI() {
	...
	GUILayout.BeginVertical();
	{
		...
		GUILayout.BeginVertical("", "TE NodeBackground");
		{
			m_BodyView = AvatarControl.ShowBoneMapping(
				m_BodyView,
				new AvatarControl.BodyPartFeedback(this.IsValidBodyPart), m_Bones,
				serializedObject, this);
			HandleBodyView(m_BodyView);
		}
		GUILayout.EndVertical();

		m_FoldoutScroll = GUILayout.BeginScrollView(
			m_FoldoutScroll, styles.box, GUILayout.MinHeight(80),
			GUILayout.MaxHeight(510), GUILayout.ExpandHeight(true));
		{
			DisplayFoldout();
	...
	if (Selection.activeTransform != null) {
		...
		if (m_CurrentTransformEditor != null &&
		    m_CurrentTransformEditor.target != Selection.activeTransform)
			DestroyImmediate(m_CurrentTransformEditor);

		if (m_CurrentTransformEditor == null)
			m_CurrentTransformEditor =
				Editor.CreateEditor(Selection.activeTransform);

		EditorGUILayout.Space();
		m_CurrentTransformEditorFoldout = EditorGUILayout.InspectorTitlebar(
			m_CurrentTransformEditorFoldout, Selection.activeTransform, true);
...
protected void AutoMapping() {
	Dictionary<int, Transform> mapping =
		AvatarAutoMapper.MapBones(gameObject.transform, modelBones);
...
protected void BipedMapping() {
	Dictionary<int, Transform> mapping =
		AvatarBipedMapper.MapBones(gameObject.transform);
	foreach (KeyValuePair<int, Transform> kvp in mapping) {
		AvatarSetupTool.BoneWrapper bone = m_Bones[kvp.Key];
		bone.bone = kvp.Value;
		bone.Serialize(m_HumanBoneArray);
...
private void UpdateSelectedBone() {
	...
	if (s_SelectedBoneIndex < 0 || s_SelectedBoneIndex >= m_Bones.Length ||
	    m_Bones[s_SelectedBoneIndex].bone != Selection.activeTransform) {
	...
	if (s_SelectedBoneIndex != oldSelectedBoneIndex) {
		// If selected bone changed, change visual to show image that contains
		// that bone.
		List<int> views =
			AvatarControl.GetViewsThatContainBone(s_SelectedBoneIndex);
		...
		if (m_BodyPartToggle[i]) {
			...
			if ((s_DirtySelection == true) && (m_BodyPartFoldout[i] == false)) {
				for (int j = 0; j < m_BodyPartHumanBone[i].Length; j++) {
					int boneIndex = m_BodyPartHumanBone [i]
					                [j];
			...
			m_BodyPartFoldout[i] =
				GUILayout.Toggle(m_BodyPartFoldout[i], styles.BodyPartMapping[i],
			...
			if (m_BodyPartFoldout[i]) {
				for (int j = 0; j < m_BodyPartHumanBone[i].Length; j++) {
					...
					if ((BodyPart) i == BodyPart.RightArm ||
					    (BodyPart) i == BodyPart.RightFingers ||
					    (BodyPart) i == BodyPart.RightLeg)
						displayBoneName = displayBoneName.Replace("Right", "");
					if ((BodyPart) i == BodyPart.LeftArm ||
					    (BodyPart) i == BodyPart.LeftFingers ||
					    (BodyPart) i == BodyPart.LeftLeg)
					...
					Rect rect = new Rect(r.x + EditorGUI.indent, r.y - 1,
					                     AvatarSetupTool.BoneWrapper.kIconSize,
					                     AvatarSetupTool.BoneWrapper.kIconSize);

					bone.BoneDotGUI(rect, selectRect, boneIndex, true, false, true,
					                serializedObject, this);
					r.xMin += AvatarSetupTool.BoneWrapper.kIconSize;

					Transform newBoneTransform = EditorGUI.ObjectField(
						r, new GUIContent(displayBoneName), bone.bone,
					...
					if (!string.IsNullOrEmpty(bone.error)) {
						...
						GUILayout.Space(EditorGUI.indent +
						                AvatarSetupTool.BoneWrapper.kIconSize + 4);
...
bool TransformChanged(Transform tr) {
	SerializedProperty bone =
		AvatarSetupTool.FindSkeletonBone(serializedObject, tr, false, false);
	if (bone != null) {
		SerializedProperty positionP =
			bone.FindPropertyRelative(AvatarSetupTool.sPosition);
		if (positionP != null && positionP.vector3Value != tr.localPosition)
			return true;

		SerializedProperty rotationP =
			bone.FindPropertyRelative(AvatarSetupTool.sRotation);
		if (rotationP != null && rotationP.quaternionValue != tr.localRotation)
			return true;

		SerializedProperty scaleP =
			bone.FindPropertyRelative(AvatarSetupTool.sScale);
...
protected BoneState GetBoneState(int i, out string error) {
	...
	AvatarSetupTool.BoneWrapper ancestor =
		m_Bones[ancestorIndex > 0 ? ancestorIndex : 0];
	...
	if (ancestor.bone != null && !bone.bone.IsChildOf(ancestor.bone)) {
		error =
			bone.messageName + " is not a child of " + ancestor.messageName + ".";
		return BoneState.InvalidHierarchy;
	}
	...
	if (i != (int) HumanBodyBones.Jaw && ancestor.bone != null &&
	    ancestor.bone != bone.bone &&
	    (bone.bone.position - ancestor.bone.position).sqrMagnitude <
	    Mathf.Epsilon) {
	...
	IEnumerable<AvatarSetupTool.BoneWrapper> match =
		m_Bones.Where(f => f.bone == bone.bone);
...
protected AvatarControl.BodyPartColor IsValidBodyPart(BodyPart bodyPart) {
	...
	if (bodyPart != BodyPart.LeftFingers && bodyPart != BodyPart.RightFingers) {
		...
		for (i = 0; i < m_BodyPartHumanBone[idx].Length; i++) {
			if (m_BodyPartHumanBone [idx]
			    [i] != -1) {
				BoneState state = m_Bones [m_BodyPartHumanBone [idx]
				                           [i]
				                  ]
				                  .state;
				hasAnyBone |= state == BoneState.Valid;

				// if it a required bone and no bone is set this body part is not
				// valid
				if (HumanTrait.RequiredBone(m_BodyPartHumanBone [idx]
				                            [i]) == true &&
		...
		for (i = 0; i < m_BodyPartHumanBone[idx].Length / phalangeCount; i++) {
			...
			for (j = phalangeCount - 1; j >= 0; j--) {
				bool hasBone = m_Bones [m_BodyPartHumanBone [idx]
				                        [fingerIndex + j]
				               ]
				...
				if (hasFinger) {
					if (!hasBone)
						return AvatarControl.BodyPartColor.Red |
						       AvatarControl.BodyPartColor.IKRed;
				} else
					hasAnyBone |= hasFinger = !hasFinger && hasBone;
			}
		}

		ik = hasAllFinger ? AvatarControl.BodyPartColor.IKGreen
	  : AvatarControl.BodyPartColor.IKRed;
	}

	if (!hasAnyBone)
		return AvatarControl.BodyPartColor.Off |
		       AvatarControl.BodyPartColor.IKRed;
...
private HumanTemplate OpenHumanTemplate() {
	...
	string templatePath = EditorUtility.OpenFilePanel("Open Human Template",
	                                                  assetsDirectory, "ht");
	...
	HumanTemplate template = AssetDatabase.LoadMainAssetAtPath(
		relativeTemplatePath) as HumanTemplate;

	// Asset not found in project, try to import it into the project
	if (template == null) {
		if (EditorUtility.DisplayDialog("Human Template not found in project",
		                                "Import asset \'" + templatePath +
		                                "\' into project",
		                                "Yes", "No")) {
			string newFileName =
				assetsDirectory + FileUtil.GetLastPathNameComponent(templatePath);
			...
			template =
				AssetDatabase.LoadMainAssetAtPath(newFileName) as HumanTemplate;

			if (template == null) {
				Debug.Log("Failed importing file \'" + templatePath + "\' to \'" +
				          newFileName + "\'");
			}
		}
	}
...
static public bool MatchName(string transformName, string boneName) {
	...
	return transformName == boneName ||
	       (transformSplit.Length > 1 &&
	        transformSplit[1] == boneName) || // transform does have a prefix
	       (boneSplit.Length > 1 &&
	        transformName == boneSplit[1]) || // boneName does have a prefix
	       (transformSplit.Length > 1 && boneSplit.Length > 1 &&
	        transformSplit[1] == boneSplit[1]); // both have a different prefix
}
...
protected void ApplyTemplate() {
	...
	for (int i = 0; i < m_Bones.Length; i++) {
		...
		if (boneName.Length > 0) {
			Transform transform = modelBones.Keys.FirstOrDefault(
				f => AvatarMappingEditor.MatchName(f.name, boneName));
...
private void SaveHumanTemplate() {
	// Go forward with presenting user a save clip dialog
	string newTemplatePath = EditorUtility.SaveFilePanelInProject(
		"Create New Human Template", "New Human Template", "ht",
		"Create a new human template");
...
public override void OnSceneGUI() {
	// It seems styles can't safely be created from inside OnSceneGUI so not
	// much we can do here. :(
	if (s_Styles == null)
		return;
