...
private string[] speedTreeGeometryTypeString = {
	"GEOM_TYPE_BRANCH", "GEOM_TYPE_BRANCH_DETAIL", "GEOM_TYPE_FROND",
	"GEOM_TYPE_LEAF", "GEOM_TYPE_MESH"
};
...
public override void OnInspectorGUI() {
	...
	if (!isVisible || theShader.hasMultipleDifferentValues ||
	    theShader.objectReferenceValue == null)
		return;

	List<MaterialProperty> props =
		new List<MaterialProperty>(GetMaterialProperties(targets));
	...
	for (int i = 0; i < targets.Length; ++i) {
		...
		for (int j = 0; j < speedTreeGeometryTypeString.Length; ++j) {
			if (((Material) targets[i])
			    .shaderKeywords.Contains(speedTreeGeometryTypeString[j])) {
	...
	var setGeomType = (SpeedTreeGeometryType) EditorGUILayout.EnumPopup(
		"Geometry Type", geomTypes[0]);
	if (EditorGUI.EndChangeCheck()) {
		bool shouldEnableAlphaTest = ShouldEnableAlphaTest(setGeomType);
		UnityEngine.Rendering.CullMode cullMode =
			shouldEnableAlphaTest ? UnityEngine.Rendering.CullMode
	...
	if (bumpMap != null) {
		...
		var enableBump = targets.Select(
			t =>((Material) t).shaderKeywords.Contains("EFFECT_BUMP"));
		bool? enable = ToggleShaderProperty(bumpMap, enableBump.First(),
		                                    enableBump.Distinct().Count() > 1);
		if (enable != null) {
			foreach (var m in targets.Cast<Material>()) {
				if (enable.Value)
	...
	//---------------------------------------------------------------
	var enableHueVariation = targets.Select(
		t =>((Material) t).shaderKeywords.Contains("EFFECT_HUE_VARIATION"));
	...
	if (enableHueVariation != null && hueVariation != null) {
		...
		bool? enable = ToggleShaderProperty(
			hueVariation, enableHueVariation.First(),
			enableHueVariation.Distinct().Count() > 1);
		if (enable != null) {
			foreach (var m in targets.Cast<Material>()) {
				if (enable.Value)
...
private bool? ToggleShaderProperty(MaterialProperty prop, bool enable,
                                   bool hasMixedEnable) {
	EditorGUI.BeginChangeCheck();
	EditorGUI.showMixedValue = hasMixedEnable;
	enable = EditorGUI.ToggleLeft(
		EditorGUILayout.GetControlRect(false, GUILayout.ExpandWidth(false)),
