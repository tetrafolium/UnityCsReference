...
internal class BlendTreeInspector : Editor {
  class Styles {
    ...
    public GUIContent speedIcon =
        new GUIContent(EditorGUIUtility.IconContent("SpeedScale"));
    public GUIContent mirrorIcon =
        new GUIContent(EditorGUIUtility.IconContent("Mirror"));
    public Texture2D pointIcon = EditorGUIUtility.LoadIcon("blendKey");
    public Texture2D pointIconSelected =
        EditorGUIUtility.LoadIcon("blendKeySelected");
    public Texture2D pointIconOverlay =
        EditorGUIUtility.LoadIcon("blendKeyOverlay");
    ...
    public Styles() {
      ...
      visBgColor = !EditorGUIUtility.isProSkin ? new Color(0.95f, 0.95f, 1.00f)
          : new Color(0.20f, 0.20f, 0.20f);
      visWeightColor =
          !EditorGUIUtility.isProSkin ? new Color(0.50f, 0.60f, 0.90f, 0.80f)
          : new Color(0.65f, 0.75f, 1.00f, 0.65f);
      visWeightShapeColor =
          !EditorGUIUtility.isProSkin ? new Color(0.40f, 0.65f, 1.00f, 0.15f)
          : new Color(0.40f, 0.65f, 1.00f, 0.12f);
      visWeightLineColor =
          !EditorGUIUtility.isProSkin ? new Color(0, 0, 0, 0.30f)
          : new Color(1, 1, 1, 0.60f);
      visPointColor = new Color(0.50f, 0.70f, 1.00f);
      visPointEmptyColor =
          !EditorGUIUtility.isProSkin ? new Color(0.80f, 0.80f, 0.80f)
          : new Color(0.60f, 0.60f, 0.60f);
      visPointOverlayColor =
          !EditorGUIUtility.isProSkin ? new Color(0, 0, 0, 0.20f)
          : new Color(1, 1, 1, 0.40f);
  ...
  private readonly int m_BlendAnimationID =
      "BlendAnimationIDHash".GetHashCode();
  private readonly int m_ClickDragFloatID =
      "ClickDragFloatIDHash".GetHashCode();
  ...
  private int ParameterCount {
    get {
      return m_BlendType.intValue >
             (int) BlendTreeType.Simple1D ?(
                 m_BlendType.intValue < (int) BlendTreeType.Direct ? 2 : 0) : 1;
    }
  }

  static internal void SetParameterValue(Animator animator, BlendTree blendTree,
                                         BlendTree parentBlendTree,
                                         string parameterName,
                                         float parameterValue) {
    bool liveLink = EditorApplication.isPlaying && animator != null &&
  ...
  static internal float GetParameterValue(Animator animator,
                                          BlendTree blendTree,
                                          string parameterName) {
    bool liveLink = EditorApplication.isPlaying && animator != null &&
                    animator.enabled && animator.gameObject.activeInHierarchy;
  ...
  public void OnEnable() {
    ...
    m_UseAutomaticThresholds =
        serializedObject.FindProperty("m_UseAutomaticThresholds");
    m_NormalizedBlendValues =
        serializedObject.FindProperty("m_NormalizedBlendValues");
  ...
  void Init() {
    ...
    if (m_Childs == null) {
      ...
      m_ReorderableList =
          new UnityEditorInternal.ReorderableList(serializedObject, m_Childs);
      ...
      if (m_BlendType.intValue == (int) BlendTreeType.Simple1D)
        SortByThreshold();
      m_ShowGraphValue = m_BlendType.intValue ==
                         (int) BlendTreeType.Direct ? m_Childs.arraySize >= 1
          : m_Childs.arraySize >= 2;
    ...
    if (m_VisInstance == null) {
      ...
      m_VisInstance =
          (GameObject) EditorUtility.InstantiateForAnimatorPreview(go);

      foreach (Renderer renderer in m_VisInstance
  ...
  internal override void OnHeaderTitleGUI(Rect titleRect, string header) {
    ...
    string newName = EditorGUI.DelayedTextField(
        textFieldRect, m_Name.stringValue, EditorStyles.textField);
  ...
  private void ParameterGUI() {
    ...
    EditorGUI.BeginChangeCheck();
    currentParameter = EditorGUILayout.DelayedTextFieldDropDown(
        currentParameter, parameters.ToArray());
    ...
    if (ParameterCount > 1) {
      ...
      EditorGUI.BeginChangeCheck();
      currentParameterY = EditorGUILayout.TextFieldDropDown(
          currentParameterY, parameters.ToArray());
      if (EditorGUI.EndChangeCheck()) {
  ...
  public override void OnInspectorGUI() {
    ...
    if (EditorGUILayout.BeginFadeGroup(m_ShowGraph.faded)) {
      if (m_BlendType.intValue == (int) BlendTreeType.Simple1D) {
        BlendGraph(
            EditorGUILayout.GetControlRect(false, 40, styles.background));
        ThresholdValues();
        ...
        for (int i = 0; i < m_BlendTree.recursiveBlendParameterCount; i++) {
          ...
          EditorGUI.BeginChangeCheck();
          float eventValue = EditorGUILayout.Slider(
              eventName,
              GetParameterValue(currentAnimator, m_BlendTree, eventName),
              eventMin, eventMax);
          if (EditorGUI.EndChangeCheck())
            SetParameterValue(currentAnimator, m_BlendTree, parentBlendTree,
      ...
      } else // 2D blend tree types
      {
        ...
        Rect graphRect =
            GUILayoutUtility.GetAspectRect(1, GUILayout.MaxWidth(235));
        GUI.Label(new Rect(graphRect.x - 1, graphRect.y - 1,
                           graphRect.width + 2, graphRect.height + 2),
    ...
    if (m_BlendType.intValue == (int) BlendTreeType.Direct) {
      EditorGUILayout.PropertyField(
          m_NormalizedBlendValues,
          EditorGUIUtility.TempContent("Normalized Blend Values"));
    }
  ...
  private void ThresholdValues() {
    ...
    if (EditorGUI.EndChangeCheck()) {
      ...
      if (m_Childs.arraySize >= 2) {
        ...
        SerializedProperty lastChild =
            m_Childs.GetArrayElementAtIndex(m_Childs.arraySize - 1);
        SerializedProperty firstThreshold =
            firstChild.FindPropertyRelative("m_Threshold");
        SerializedProperty lastThreshold =
            lastChild.FindPropertyRelative("m_Threshold");
        ...
        if (!m_UseAutomaticThresholds.boolValue) {
          // Since this isn't being automatically calculated, we need to scale
          // the values.
          int arraySize = m_Childs.arraySize;
          for (int i = 1; i < arraySize - 1; ++i) {
            SerializedProperty child = m_Childs.GetArrayElementAtIndex(i);
            SerializedProperty threshold =
                child.FindPropertyRelative("m_Threshold");
            float ratio = Mathf.InverseLerp(previousMin, previousMax,
        ...
        float blendValue = GetParameterValue(currentAnimator, m_BlendTree,
                                             m_BlendTree.blendParameter);
        blendValue = Mathf.Clamp(blendValue, newMin, newMax);
        SetParameterValue(currentAnimator, m_BlendTree, parentBlendTree,
                          m_BlendTree.blendParameter, blendValue);
      }
  ...
  public float ClickDragFloat(Rect position, float value, bool alignRight) {
    ...
    int id = EditorGUIUtility.GetControlID(m_ClickDragFloatID,
    ...
    GUIStyle style =
        (GUIUtility.keyboardControl == id && EditorGUIUtility.editingTextField)
            ? (alignRight ? styles.clickDragFloatFieldRight
               : styles.clickDragFloatFieldLeft)
            : (alignRight ? styles.clickDragFloatLabelRight
               : styles.clickDragFloatLabelLeft);
    if (GUIUtility.keyboardControl == id) {
      if (!EditorGUI.s_RecycledEditor.IsEditingControl(id)) {
        str = EditorGUI.s_RecycledCurrentEditingString =
            value.ToString("g7", CultureInfo.InvariantCulture.NumberFormat);
      } else {
        str = EditorGUI.s_RecycledCurrentEditingString;
      ...
      str = EditorGUI.DoTextField(EditorGUI.s_RecycledEditor, id, position, str,
                                  style, allowedCharacters, out changed, false,
                                  false, false);
      if (changed) {
        GUI.changed = true;
  ...
  private void BlendGraph(Rect area) {
    ...
    // (This is normally not needed anymore, but this style has some overdraw
    // that needs to be compensated.)
    area.xMin += 1;
    area.xMax -= 1;

    int sliderId =
        GUIUtility.GetControlID(m_BlendAnimationID, FocusType.Passive);
    ...
    for (int i = 0; i < points.Length; i++) {
      points[i] =
          area.x + (Mathf.InverseLerp(min, max, points[i]) * area.width);
    }
    ...
    float blendBar =
        area.x +
        (Mathf.InverseLerp(min, max,
                           GetParameterValue(currentAnimator, m_BlendTree,
                                             currentParameter)) *
         area.width);
    ...
    case EventType.Repaint:
      ...
      if (m_Childs.arraySize >= 2) {
        ...
        styles.blendPosition.Draw(barRect, GUIContent.none, false, false, false,
                                  false);
      } else {
        GUI.Label(area,
                  EditorGUIUtility.TempContent(
                      "Please Add Motion Fields or Blend Trees"),
                  styles.errorStyle);
      }
      break;
    case EventType.MouseDown:
      ...
      if (barRect.Contains(evt.mousePosition)) {
        ...
        curBlendValue =
            GetParameterValue(currentAnimator, m_BlendTree, currentParameter);
        ...
        if (m_ReorderableList.index != -1) {
          SerializedProperty child =
              m_Childs.GetArrayElementAtIndex(m_ReorderableList.index);
          SerializedProperty threshold =
              child.FindPropertyRelative("m_Threshold");
    ...
    case EventType.MouseDrag:
      ...
      newMouseBlendValue =
          Mathf.LerpUnclamped(m_OriginMin, m_OriginMax, newMouseBlendValue);
      ...
      if (m_ReorderableList.index == -1) {
        ...
        SetParameterValue(currentAnimator, m_BlendTree, parentBlendTree,
                          currentParameter, newBlendValue);
      } else {
        ...
        SerializedProperty lastChild =
            (m_ReorderableList.index <= 0)
                ? child
                : m_Childs.GetArrayElementAtIndex(m_ReorderableList.index - 1);
        SerializedProperty nextChild =
            (m_ReorderableList.index == m_Childs.arraySize - 1)
                ? child
                : m_Childs.GetArrayElementAtIndex(m_ReorderableList.index + 1);
        SerializedProperty lastThreshold =
            lastChild.FindPropertyRelative("m_Threshold");
        SerializedProperty nextThreshold =
            nextChild.FindPropertyRelative("m_Threshold");

        // change threshold value
        threshold.floatValue = newBlendValue;

        // reorder if dragged beyond range
        if (threshold.floatValue < lastThreshold.floatValue &&
            m_ReorderableList.index != 0) {
          m_Childs.MoveArrayElement(m_ReorderableList.index,
                                    m_ReorderableList.index - 1);
          m_ReorderableList.index -= 1;
        }
        if (threshold.floatValue > nextThreshold.floatValue &&
            m_ReorderableList.index < m_Childs.arraySize - 1) {
          m_Childs.MoveArrayElement(m_ReorderableList.index,
                                    m_ReorderableList.index + 1);
          m_ReorderableList.index += 1;
        ...
        } else if (nextThreshold.floatValue - threshold.floatValue <=
                   snapThreshold) {
  ...
  private void UpdateBlendVisualization() {
    ...
    if (m_BlendTex == null) {
      m_BlendTex = new Texture2D(kVisResolution, kVisResolution,
    ...
    while (m_WeightTexs.Count < points.Length) {
      Texture2D tex = new Texture2D(kVisResolution, kVisResolution,
                                    TextureFormat.RGBA32, false);
    ...
    UnityEditorInternal.BlendTreePreviewUtility.CalculateBlendTexture(
        m_VisBlendTree.animator, 0,
        m_VisBlendTree.animator.GetCurrentAnimatorStateInfo(0).fullPathHash,
        m_BlendTex, textures, m_BlendRect);
    watch.Stop();

    // Debug.Log ("CalculateBlendTexture took "+watch.ElapsedMilliseconds+"
    // ms");
  }
  ...
  private Rect Get2DBlendRect(Vector2[] points) {
    ...
    if (maxDist == 0)
      maxDist = 1;
    maxDist *= 1.35f;
    return new Rect(center.x - maxDist, center.y - maxDist, maxDist * 2,
                    maxDist * 2);
  }
  ...
  private float ConvertFloat(float input, float fromMin, float fromMax,
  ...
  private void BlendGraph2D(Rect area) {
    ...
    for (int i = 0; i < points.Length; i++) {
      points[i].x =
          ConvertFloat(points[i].x, min.x, max.x, area.xMin, area.xMax);
      points[i].y =
          ConvertFloat(points[i].y, min.y, max.y, area.yMax, area.yMin);
    }
    ...
    string currentParameterY = m_BlendTree.blendParameterY;
    float inputX =
        GetParameterValue(currentAnimator, m_BlendTree, currentParameterX);
    float inputY =
        GetParameterValue(currentAnimator, m_BlendTree, currentParameterY);
    ...
    UnityEditorInternal.BlendTreePreviewUtility
        .CalculateRootBlendTreeChildWeights(
            m_VisBlendTree.animator, 0,
            m_VisBlendTree.animator.GetCurrentAnimatorStateInfo(0).fullPathHash,
            m_Weights, inputX, inputY);

    // Move input into GUI space
    inputX = area.x + Mathf.InverseLerp(min.x, max.x, inputX) * area.width;
    inputY =
        area.y + (1 - Mathf.InverseLerp(min.y, max.y, inputY)) * area.height;
    ...
    case EventType.Repaint:
      ...
      if (m_ReorderableList.index < 0 ||
          m_ReorderableList.index >= presences.Length) {
      ...
      for (int i = 0; i < points.Length; i++) {
        ...
        GUI.DrawTexture(pointRect, drawSelected ? styles.pointIconSelected
                        : styles.pointIcon);
    ...
    case EventType.MouseDrag:
      ...
      if (m_SelectedPoint == -1) {
        ...
        mousePosition.x = ConvertFloat(evt.mousePosition.x, area.xMin,
                                       area.xMax, min.x, max.x);
        mousePosition.y = ConvertFloat(evt.mousePosition.y, area.yMax,
                                       area.yMin, min.y, max.y);

        // Set blend values
        SetParameterValue(currentAnimator, m_BlendTree, parentBlendTree,
                          currentParameterX, mousePosition.x);
        SetParameterValue(currentAnimator, m_BlendTree, parentBlendTree,
                          currentParameterY, mousePosition.y);
        ...
        for (int i = 0; i < points.Length; i++) {
          if (m_SelectedPoint == i) {
            ...
            mousePosition.x = MathUtils.RoundBasedOnMinimumDifference(
                mousePosition.x, minDiff);
            mousePosition.y = MathUtils.RoundBasedOnMinimumDifference(
                mousePosition.y, minDiff);
            mousePosition.x = Mathf.Clamp(mousePosition.x, -10000, 10000);
            mousePosition.y = Mathf.Clamp(mousePosition.y, -10000, 10000);

            SerializedProperty child = m_Childs.GetArrayElementAtIndex(i);
            SerializedProperty position =
                child.FindPropertyRelative("m_Position");
    ...
    if (m_ReorderableList.index >= 0 &&
        m_ReorderableList.index < presences.Length &&
        presences[m_ReorderableList.index] < 0)
      ShowHelp(area, EditorGUIUtility.TempContent(
                         "The selected child has no Motion assigned."));
  ...
  private void ShowHelp(Rect area, GUIContent content) {
    ...
    GUI.Label(new Rect(area.x, area.y, area.width, height), content,
  ...
  private void ValidatePositions() {
    ...
    for (int i = 0; i < points.Length; i++) {
      for (int j = 0; j < i && !duplicatePositions; j++) {
        if (((points[i] - points[j]) / m_BlendRect.height).sqrMagnitude <
            0.0001f) {
    ...
    if (duplicatePositions) {
      m_WarningMessage =
          "Two or more of the positions are too close to each other.";
      return;
    }
    ...
    if (m_BlendType.intValue == (int) BlendTreeType.SimpleDirectional2D) {
      List<float> angles = points.Where(e => e != Vector2.zero)
                               .Select(e => Mathf.Atan2(e.y, e.x))
                               .OrderBy(e => e)
                               .ToList();
      float maxAngle = 0;
      float minAngle = 180;
      ...
      if (maxAngle * Mathf.Rad2Deg >= 180)
        m_WarningMessage =
            "Simple Directional blend should have motions with directions less than 180 degrees apart.";
      else if (minAngle * Mathf.Rad2Deg < 2)
        m_WarningMessage =
            "Simple Directional blend should not have multiple motions in almost the same direction.";
    } else if (m_BlendType.intValue ==
               (int) BlendTreeType.FreeformDirectional2D) {
      ...
      if (!hasCenter)
        m_WarningMessage =
            "Freeform Directional blend should have one motion at position (0,0) to avoid discontinuities.";
    }
  }
  ...
  private void DrawWeightShape(Vector2 point, float weight, int pass) {
    ...
    for (int i = 0; i < kNumCirclePoints; i++) {
      ...
      points[i + 1] = new Vector3(point.x + 0.5f, point.y + 0.5f, 0) +
                      new Vector3(Mathf.Sin(v * 2 * Mathf.PI),
                                  Mathf.Cos(v * 2 * Mathf.PI), 0) *
                          radius;
    }
    // First and last point have to meet each other in a straight line;
    // otherwise we'll get a gap
    points[0] = points[kNumCirclePoints + 1] =
        (points[1] + points[kNumCirclePoints]) * 0.5f;

    if (pass == 0) {
      // Draw disc
      Handles.color = styles.visWeightShapeColor;
      Handles.DrawSolidDisc(point + new Vector2(0.5f, 0.5f), -Vector3.forward,
                            radius);
    } else {
      // Draw outline
  ...
  private void DrawAnimation(float val, float min, float max, bool selected,
                             Rect area) {
    float top = area.y;
    ...
    Color oldColor = Handles.color;
    Color newColor =
        selected ? new Color(1f, 1f, 1f, 0.6f) : new Color(1f, 1f, 1f, 0.4f);
    Handles.color = newColor;
    if (selected)
      Handles.DrawLine(new Vector3(val, top, 0),
                       new Vector3(val, top + area.height, 0));
    Vector3[] points = new Vector3[2]{new Vector3(min, top + area.height, 0f),
                                      new Vector3(val, top, 0f)};
    Handles.DrawAAPolyLine(points);
    points = new Vector3[2]{new Vector3(val, top, 0f),
                            new Vector3(max, top + area.height, 0f)};
    Handles.DrawAAPolyLine(points);
    Handles.color = oldColor;
  }
  ...
  private void DrawHeader(Rect headerRect) {
    headerRect.xMin += 14; // Ignore width used by drag-handles while
    ...
    rects[col].xMin =
        rects[col].xMin -
        14; // Make first column extend into space of drag-handles.
    GUI.Label(rects[col], EditorGUIUtility.TempContent("Motion"),
              EditorStyles.label);
    col++;
    if (m_Childs.arraySize >= 1) {
      if (m_BlendType.intValue == (int) BlendTreeType.Simple1D) {
        GUI.Label(rects[col], EditorGUIUtility.TempContent("Threshold"),
                  EditorStyles.label);
        col++;
      } else if (m_BlendType.intValue == (int) BlendTreeType.Direct) {
        GUI.Label(rects[col], EditorGUIUtility.TempContent("Parameter"),
                  EditorStyles.label);
        col++;
      } else {
        GUI.Label(rects[col], EditorGUIUtility.TempContent("Pos X"),
                  EditorStyles.label);
        col++;
        GUI.Label(rects[col], EditorGUIUtility.TempContent("Pos Y"),
                  EditorStyles.label);
        col++;
      }
  ...
  public void AddButton(Rect rect, UnityEditorInternal.ReorderableList list) {
    ...
    menu.AddItem(EditorGUIUtility.TrTextContent("Add Motion Field"), false,
                 AddChildAnimation);
    menu.AddItem(EditorGUIUtility.TempContent("New Blend Tree"), false,
  ...
  private Rect[] GetRowRects(Rect r, int blendType) {
    int rowCount = blendType > (int) BlendTreeType.Simple1D &&
                   blendType < (int) BlendTreeType.Direct ? 2 : 1;
  ...
  public void DrawChild(Rect r, int index, bool isActive, bool isFocused) {
    ...
    col++;
    if (EditorGUI.EndChangeCheck()) {
      // [case 1028113] Delete previous Motion only if it serialized in the same
      // asset file
      if (prevMotion is BlendTree &&
          prevMotion != (motion.objectReferenceValue as Motion) &&
          MecanimUtilities.AreSameAsset(m_BlendTree, prevMotion)) {
        if (EditorUtility.DisplayDialog(
                "Changing BlendTree will delete previous BlendTree",
    ...
    if (m_BlendType.intValue == (int) BlendTreeType.Simple1D) {
      ...
      using(new EditorGUI.DisabledScope(m_UseAutomaticThresholds.boolValue)) {
        ...
        EditorGUI.BeginChangeCheck();
        string floatStr = EditorGUI.DelayedTextFieldInternal(
            rects[col], nr.ToString(CultureInfo.InvariantCulture.NumberFormat),
            "inftynaeINFTYNAE0123456789.,-", EditorStyles.textField);
        col++;
        if (EditorGUI.EndChangeCheck()) {
          if (float.TryParse(floatStr, NumberStyles.Float,
      ...
      EditorGUI.BeginChangeCheck();
      directParam = EditorGUI.TextFieldDropDown(rects[col], directParam,
                                                parameters.ToArray());
      col++;
      ...
      for (int i = 0; i < 2; i++) {
        EditorGUI.BeginChangeCheck();
        string valStr = EditorGUI.DelayedTextFieldInternal(
            rects[col],
            pos [i]
                .ToString(),
            "inftynaeINFTYNAE0123456789.,-", EditorStyles.textField);
    ...
    if (motion.objectReferenceValue is AnimationClip &&
        (motion.objectReferenceValue as AnimationClip).isHumanMotion) {
  ...
  private bool AllMotions() {
    ...
    for (int i = 0; i < m_Childs.arraySize && allClips; i++) {
      SerializedProperty motion =
          m_Childs.GetArrayElementAtIndex(i).FindPropertyRelative("m_Motion");
  ...
  private void AutoCompute() {
    if (m_BlendType.intValue == (int) BlendTreeType.Simple1D) {
      EditorGUILayout.PropertyField(
          m_UseAutomaticThresholds,
          EditorGUIUtility.TempContent("Automate Thresholds"));
    ...
    if (EditorGUILayout.BeginFadeGroup(m_ShowCompute.faded)) {
      ...
      if (EditorGUI.DropdownButton(controlRect,
                                   EditorGUIUtility.TempContent("Select"),
                                   FocusType.Passive, EditorStyles.popup)) {
        GenericMenu menu = new GenericMenu();
        if (ParameterCount == 1) {
          AddComputeMenuItems(menu, string.Empty,
                              ChildPropertyToCompute.Threshold);
        } else {
          menu.AddItem(EditorGUIUtility.TrTextContent("Velocity XZ"), false,
                       ComputePositionsFromVelocity);
          menu.AddItem(
              EditorGUIUtility.TrTextContent("Speed And Angular Speed"), false,
              ComputePositionsFromSpeedAndAngularSpeed);
          AddComputeMenuItems(menu, "X Position From/",
                              ChildPropertyToCompute.PositionX);
          AddComputeMenuItems(menu, "Y Position From/",
                              ChildPropertyToCompute.PositionY);
        }
    ...
    if (EditorGUILayout.BeginFadeGroup(m_ShowAdjust.faded)) {
      Rect controlRect = EditorGUILayout.GetControlRect();
      controlRect = EditorGUI.PrefixLabel(
          controlRect, 0, EditorGUIUtility.TempContent("Adjust Time Scale"));

      if (EditorGUI.DropdownButton(controlRect,
                                   EditorGUIUtility.TempContent("Select"),
                                   FocusType.Passive, EditorStyles.popup)) {
        GenericMenu menu = new GenericMenu();
        menu.AddItem(EditorGUIUtility.TrTextContent("Homogeneous Speed"), false,
                     ComputeTimeScaleFromSpeed);
        menu.AddItem(EditorGUIUtility.TrTextContent("Reset Time Scale"), false,
                     ResetTimeScale);
        menu.DropDown(controlRect);
      }
    }
  ...
  enum ChildPropertyToCompute { Threshold, PositionX, PositionY }

  delegate float GetFloatFromMotion(Motion motion, float mirrorMultiplier);

  void AddComputeMenuItems(GenericMenu menu, string menuItemPrefix,
                           ChildPropertyToCompute prop) {
    menu.AddItem(new GUIContent(menuItemPrefix + "Speed"), false,
                 ComputeFromSpeed, prop);
    menu.AddItem(new GUIContent(menuItemPrefix + "Velocity X"), false,
                 ComputeFromVelocityX, prop);
    menu.AddItem(new GUIContent(menuItemPrefix + "Velocity Y"), false,
                 ComputeFromVelocityY, prop);
    menu.AddItem(new GUIContent(menuItemPrefix + "Velocity Z"), false,
                 ComputeFromVelocityZ, prop);
    menu.AddItem(new GUIContent(menuItemPrefix + "Angular Speed (Rad)"), false,
                 ComputeFromAngularSpeedRadians, prop);
    menu.AddItem(new GUIContent(menuItemPrefix + "Angular Speed (Deg)"), false,
                 ComputeFromAngularSpeedDegrees, prop);
  }

  private void ComputeFromSpeed(object obj) {
    ChildPropertyToCompute prop = (ChildPropertyToCompute) obj;
    ComputeProperty((Motion m, float mirrorMultiplier) => m.apparentSpeed,
                    prop);
  }

  private void ComputeFromVelocityX(object obj) {
    ChildPropertyToCompute prop = (ChildPropertyToCompute) obj;
    ComputeProperty((Motion m, float mirrorMultiplier) =>
                        m.averageSpeed.x * mirrorMultiplier,
                    prop);
  }

  private void ComputeFromVelocityY(object obj) {
    ChildPropertyToCompute prop = (ChildPropertyToCompute) obj;
    ComputeProperty((Motion m, float mirrorMultiplier) => m.averageSpeed.y,
                    prop);
  }

  private void ComputeFromVelocityZ(object obj) {
    ChildPropertyToCompute prop = (ChildPropertyToCompute) obj;
    ComputeProperty((Motion m, float mirrorMultiplier) => m.averageSpeed.z,
                    prop);
  }
  ...
  private void ComputeFromAngularSpeedDegrees(object obj) {
    ChildPropertyToCompute prop = (ChildPropertyToCompute) obj;
    ComputeProperty((Motion m, float mirrorMultiplier) =>
                        m.averageAngularSpeed * 180.0f / Mathf.PI *
                        mirrorMultiplier,
                    prop);
  }
  ...
  private void ComputeFromAngularSpeedRadians(object obj) {
    ChildPropertyToCompute prop = (ChildPropertyToCompute) obj;
    ComputeProperty((Motion m, float mirrorMultiplier) =>
                        m.averageAngularSpeed * mirrorMultiplier,
                    prop);
  }

  private void ComputeProperty(GetFloatFromMotion func,
                               ChildPropertyToCompute prop) {
    float mean = 0.0f;
    ...
    for (int i = 0; i < m_Childs.arraySize; i++) {
      ...
      SerializedProperty mirror =
          m_Childs.GetArrayElementAtIndex(i).FindPropertyRelative("m_Mirror");
      ...
      if (motionObj != null) {
        ...
        if (prop == ChildPropertyToCompute.Threshold) {
          SerializedProperty threshold =
              m_Childs.GetArrayElementAtIndex(i).FindPropertyRelative(
                  "m_Threshold");
    ...
    if (variance < Mathf.Epsilon) {
      Debug.LogWarning("Could not compute threshold for '" + m_BlendTree.name +
      ...
      if (clip != null) {
        if (!clip.legacy) {
          if (clip.apparentSpeed < Mathf.Epsilon) {
            Debug.LogWarning("Could not adjust time scale for " + clip.name +
          ...
          Debug.LogWarning("Could not adjust time scale for " + clip.name +
                           " because it is not a muscle clip");
        }
      }
    }
    m_SerializedObject.ApplyModifiedProperties();
  ...
  private void SortByThreshold() {
    ...
    for (int i = 0; i < m_Childs.arraySize; i++) {
      ...
      for (int j = i; j < m_Childs.arraySize; j++) {
        ...
        float testThreshold =
            testElement.FindPropertyRelative("m_Threshold").floatValue;
  ...
  private void SetMinMaxThreshold() {
    m_SerializedObject.Update();
    SerializedProperty minThreshold =
        m_Childs.GetArrayElementAtIndex(0).FindPropertyRelative("m_Threshold");
    SerializedProperty maxThreshold =
        m_Childs.GetArrayElementAtIndex(m_Childs.arraySize - 1)
            .FindPropertyRelative("m_Threshold");
    m_MinThreshold.floatValue =
        Mathf.Min(minThreshold.floatValue, maxThreshold.floatValue);
    m_MaxThreshold.floatValue =
        Mathf.Max(minThreshold.floatValue, maxThreshold.floatValue);
    m_SerializedObject.ApplyModifiedProperties();
  }
  ...
  void AddChildAnimation() {
    ...
    if (currentController != null) {
      m_BlendTree.SetDirectBlendTreeParameter(
          numChildren - 1, currentController.GetDefaultBlendTreeParameter());
    }
  ...
  void SetNewThresholdAndPosition(int index) {
    ...
    if (!m_UseAutomaticThresholds.boolValue) {
      ...
      if (m_Childs.arraySize >= 3 && index == m_Childs.arraySize - 1) {
        float threshold1 = m_Childs.GetArrayElementAtIndex(index - 2)
                               .FindPropertyRelative("m_Threshold")
                               .floatValue;
        float threshold2 = m_Childs.GetArrayElementAtIndex(index - 1)
                               .FindPropertyRelative("m_Threshold")
                               .floatValue;
        ...
        if (m_Childs.arraySize == 1)
          ...
          newThreshold = m_Childs.GetArrayElementAtIndex(m_Childs.arraySize - 1)
                             .FindPropertyRelative("m_Threshold")
                             .floatValue +
                         1;
      }
      SerializedProperty addedThreshold =
          m_Childs.GetArrayElementAtIndex(index).FindPropertyRelative(
              "m_Threshold");
    ...
    if (m_Childs.arraySize >= 1) {
      ...
      for (int iter = 0; iter < 24; iter++) {
        ...
        for (int i = 0; i < points.Length && satisfied; i++)
          if (i != index &&
              Vector2.Distance(points[i], newPosition) < goodMinDist)
            satisfied = false;
        ...
        newPosition =
            center + new Vector2(-Mathf.Cos(radians), Mathf.Sin(radians)) *
                         0.37f * m_BlendRect.width;
        newPosition.x = MathUtils.RoundBasedOnMinimumDifference(
            newPosition.x, m_BlendRect.width * 0.005f);
        newPosition.y = MathUtils.RoundBasedOnMinimumDifference(
            newPosition.y, m_BlendRect.width * 0.005f);
      }
    }
    SerializedProperty addedPosition =
        m_Childs.GetArrayElementAtIndex(index).FindPropertyRelative(
            "m_Position");
...
class VisualizationBlendTree {
  ...
  public bool controllerDirty {
    get { return m_ControllerIsDirty; }
  }
  protected virtual void ControllerDirty() { m_ControllerIsDirty = true; }

  private void CreateParameters() {
    for (int i = 0; i < m_BlendTree.recursiveBlendParameterCount; i++)
      m_Controller.AddParameter(m_BlendTree.GetRecursiveBlendParameter(i),
  ...
  public void Destroy() { ClearStateMachine(); }
  ...
  public void Update() {
    ...
    for (int i = 0; i < count; i++) {
      ...
      float value = BlendTreeInspector.GetParameterValue(animator, m_BlendTree,
                                                         blendParameter);
...
class PreviewBlendTree {
  ...
  public Animator PreviewAnimator {
    get { return m_AvatarPreview.Animator; }
  }
  ...
  private void OnPreviewAvatarChanged() { ResetStateMachine(); }
  ...
  private void UpdateAvatarState() {
    ...
    if (m_AvatarPreview.Animator) {
      if (m_PrevIKOnFeet != m_AvatarPreview.IKOnFeet) {
        ...
        ResetStateMachine();
        m_AvatarPreview.Animator.Update(
      ...
      if (m_AvatarPreview.Animator) {
        for (int i = 0; i < m_BlendTree.recursiveBlendParameterCount; i++) {
          string blendParameter = m_BlendTree.GetRecursiveBlendParameter(i);
          float value = BlendTreeInspector.GetParameterValue(
              m_AvatarPreview.Animator, m_BlendTree, blendParameter);
      ...
      if (m_AvatarPreview.Animator.layerCount > 0) {
        AnimatorStateInfo stateInfo =
            m_AvatarPreview.Animator.GetCurrentAnimatorStateInfo(0);
  ...
  public void TestForReset() {
    if ((m_State != null && m_AvatarPreview != null &&
         m_State.iKOnFeet != m_AvatarPreview.IKOnFeet)) {
      ResetStateMachine();
    }
  }
  ...
  public void OnPreviewSettings() { m_AvatarPreview.DoPreviewSettings(); }
