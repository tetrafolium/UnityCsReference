...
class OcclusionAreaEditor : Editor {
  ...
  void OnSceneGUI() {
    ...
    // Yes, it's weird to use lossyScale here, but that's what the occlusion
    // volumes do
    ...
    if (GUI.changed) {
      // Occlusion volumes can't be rotated but the center is still affected by
      // rotation, so we need to rotate the offset (and apply the inverse
      // lossyScale AFTER that)
      m_Center.vector3Value =
          m_Center.vector3Value +
          Vector3.Scale(Quaternion.Inverse(area.transform.rotation) *
                            (max - min) * 0.5f,
                        inverseScale);
  ...
  float SizeSlider(Vector3 p, Vector3 d, float r) {
    ...
    GUI.changed = false;
    position =
        Handles.Slider(position, d, size * 0.1f, Handles.CylinderHandleCap, 0f);
    if (GUI.changed)
