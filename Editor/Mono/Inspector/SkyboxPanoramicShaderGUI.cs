...
internal class SkyboxPanoramicShaderGUI : ShaderGUI {
  ...
  public override void OnGUI(MaterialEditor materialEditor,
    ...
    // Allow the default implementation to set widths for consistency for common
    // properties.
    ...
    m_ShowLatLongLayout.target =
        ShowProp(materialEditor, FindProperty("_Mapping", props)) == 1;
    if (EditorGUILayout.BeginFadeGroup(m_ShowLatLongLayout.faded)) {
      m_ShowMirrorOnBack.target =
          ShowProp(materialEditor, FindProperty("_ImageType", props)) == 1;
    ...
    // Let the default implementation add the extra shader properties at the
    // bottom.
