...
internal class RayTracingShaderInspector : Editor {
  ...
  private class Styles {
    public GUIContent s_MaxRecursionDepthText = EditorGUIUtility.TrTextContent(
        "Max. Recursion Depth",
        "Limit on ray recursion for the Ray Tracing pipeline. This is defined in the shader by using max_recursion_depth pragma(e.g. \"#pragma max_recursion_depth 5\"). Applications should pick a limit that is as low as absolutely necessary. A value of 1 means that only primary rays can be cast.");
    public GUIContent s_PlatformList =
        EditorGUIUtility.TrTextContent("Platforms:");
    public GUIContent s_NotSupported = EditorGUIUtility.TrTextContent(
        "Ray Tracing Shader not supported! No graphics APIs with Ray Tracing support found in the Graphics APIs list.");
    ...
    public GUIContent s_PayloadSize =
        EditorGUIUtility.TrTextContent("Payload Size (Bytes)");
    public GUIContent s_ParamSize =
        EditorGUIUtility.TrTextContent("Param. Size (Bytes)");
    public GUIContent s_RayGenShaderNames = EditorGUIUtility.TrTextContent(
        "Ray Generation Shaders",
        "The list of all ray generation shaders in the shader file. Only one ray generation shader can be executed at a time.");
    public GUIContent s_MissShaderNames = EditorGUIUtility.TrTextContent(
        "Miss Shaders",
        "The list of all miss shaders in the shader file. The index of the miss shader to execute is specified when calling TraceRay HLSL function.");
    public GUIContent s_CallableShaderNames = EditorGUIUtility.TrTextContent(
        "Callable Shaders",
        "The list of all callable shaders in the shader file. The index of the callable shader to execute is specified when calling CallShader HLSL function.");
    public GUIStyle s_LabelStyle = new GUIStyle(EditorStyles.boldLabel);
    public Styles() { s_LabelStyle.richText = true; }
  }
  ...
  void ShowMissShaderList(string[] missShaderNames, int[] rayPayloadSize) {
    ...
    float lineHeight = messageStyle.CalcHeight(
        EditorGUIUtility.TempContent("ShaderName"), 100);
    ...
    Vector2 payloadColumnSize =
        EditorStyles.boldLabel.CalcSize(styles.s_PayloadSize);
  ...
  void ShowCallableShaderList(string[] callableShaderNames, int[] paramSize) {
    ...
    Vector2 paramColumnSize =
        EditorStyles.boldLabel.CalcSize(styles.s_ParamSize);
  ...
  public override void OnInspectorGUI() {
    ...
    if (ShowPlatformListSection(rts)) {
      ...
      EditorGUILayout.PropertyField(m_MaxRecursionDepth,
                                    styles.s_MaxRecursionDepthText);
      ...
      if (missShaderCount > 0) {
        ...
        for (int i = 0; i < missShaderCount; i++) {
          ...
          missShaderPayloadSize[i] =
              ShaderUtil.GetMissShaderRayPayloadSize(rts, i);
        }
      ...
      if (callableShaderCount > 0) {
        ...
        for (int i = 0; i < callableShaderCount; i++) {
          ...
          callableShaderParamsSize[i] =
              ShaderUtil.GetCallableShaderParamSize(rts, i);
        }
  ...
  private void ShowShaderErrors(RayTracingShader s) {
    ...
    if (m_ShaderMessages == null)
      return;
    ShaderInspector.ShaderErrorListUI(s, m_ShaderMessages,
