...
class Texture2DArrayInspector : TextureInspector {
  ...
  public override string GetInfoString() {
    ...
    var info =
        $"{tex.width}x{tex.height} {tex.depth} slice{(tex.depth != 1 ? "s " : " ")} {TextureUtil.GetTextureFormatString(tex.format)} {EditorUtility.FormatBytes(TextureUtil.GetRuntimeMemorySizeLong(tex))}";
    return info;
  }
  ...
  public override void OnPreviewSettings() {
    ...
    if (t.depth > 1) {
      m_Slice = (int) PreviewSettingsSlider(GUIContent.none, m_Slice, 0,
                                            t.depth - 1, 60, 30, isInteger
                                            : true);
  ...
  public override void OnPreviewGUI(Rect r, GUIStyle background) {
    ...
    // If multiple objects are selected, we might be using a slice level before
    // the maximum
    ...
    EditorGUI.DrawPreviewTexture(wantedRect, t, m_Material,
                                 ScaleMode.StretchToFill, 0, effectiveMipLevel);
  ...
  public override Texture2D RenderStaticPreview(string assetPath,
    ...
    var subRect = new Rect(0, 0,
                           previewUtility.renderTexture.width -
                               sliceDistance * (elementCount - 1),
                           previewUtility.renderTexture.height -
                               sliceDistance * (elementCount - 1));
    for (var el = elementCount - 1; el >= 0; --el) {
      m_Material.SetInt(s_ShaderSliceIndex, el);

      subRect.x = sliceDistance * el;
      subRect.y = previewUtility.renderTexture.height - subRect.height -
                  sliceDistance * el;

      var aspect = texture.width / (float) texture.height;
      GUI.CalculateScaledTextureRects(subRect, ScaleMode.ScaleToFit, aspect,
                                      ref screenRect, ref sourceRect);
      Graphics.DrawTexture(screenRect, texture, sourceRect, 0, 0, 0, 0,
                           Color.white, m_Material);
    }
