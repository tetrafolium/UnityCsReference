...
class CapsuleCollider2DTool : PrimitiveCollider2DTool<CapsuleCollider2D> {
  ...
  protected override void CopyColliderSizeToHandle(CapsuleCollider2D collider) {
    ...
    GetHandleVectorsInWorldSpace(collider, out handleHeightAxis,
                                 out handleRadiusAxis);
  ...
  protected override bool CopyHandleSizeToCollider(CapsuleCollider2D collider) {
    ...
    Vector3 worldHeight =
        Handles.matrix * (localHeightAxis * m_BoundsHandle.height);
    Vector3 worldDiameter =
        Handles.matrix * (localDiameterAxis * m_BoundsHandle.radius * 2f);

    // project collider's diameter and height axes onto world x/y plane and
    // scale by handle values
    Matrix4x4 colliderTransformMatrix = collider.transform.localToWorldMatrix;
    Vector3 projectedDiameter =
        ProjectOntoWorldPlane(colliderTransformMatrix * localDiameterAxis)
            .normalized *
        worldDiameter.magnitude;
    Vector3 projectedHeight =
        ProjectOntoWorldPlane(colliderTransformMatrix * localHeightAxis)
            .normalized *
        worldHeight.magnitude;

    // project results back in collider's local space
    projectedDiameter =
        ProjectOntoColliderPlane(projectedDiameter, colliderTransformMatrix);
    projectedHeight =
        ProjectOntoColliderPlane(projectedHeight, colliderTransformMatrix);

    Vector2 oldSize = collider.size;

    collider.size =
        colliderTransformMatrix.inverse * (projectedDiameter + projectedHeight);

    // test for size change after using property setter in case input data was
    // sanitized
  ...
  protected override Quaternion GetHandleRotation() {
    ...
    GetHandleVectorsInWorldSpace(target as CapsuleCollider2D, out heightVector,
                                 out diameterVector);
    return Quaternion.LookRotation(Vector3.forward, heightVector);
  }

  void GetHandleVectorsInWorldSpace(CapsuleCollider2D collider,
                                    out Vector3 handleHeightVector,
                                    out Vector3 handleDiameterVector) {
    Matrix4x4 colliderTransformMatrix = collider.transform.localToWorldMatrix;
    Vector3 x = ProjectOntoWorldPlane(colliderTransformMatrix *
                                      (Vector3.right * collider.size.x));
    Vector3 y = ProjectOntoWorldPlane(colliderTransformMatrix *
