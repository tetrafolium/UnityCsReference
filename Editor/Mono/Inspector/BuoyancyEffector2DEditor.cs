...
internal class BuoyancyEffector2DEditor : Effector2DEditor {
  ...
  public override void OnInspectorGUI() {
    ...
    // Drag.
    m_ShowDampingRollout.target =
        EditorGUILayout.Foldout(m_ShowDampingRollout.target, "Damping", true);
    ...
    // Flow.
    m_ShowFlowRollout.target =
        EditorGUILayout.Foldout(m_ShowFlowRollout.target, "Flow", true);
  ...
  public void OnSceneGUI() {
    ...
    var surfaceY = effectorPosition.y +
                   (effector.transform.lossyScale.y * effector.surfaceLevel);
    ...
    foreach (var c in effector.gameObject.GetComponents<Collider2D>().Where(
                 c => c.enabled && c.usedByEffector)) {
      var b = c.bounds;
      var left = b.min.x;
      var right = b.max.x;
    ...
    Handles.DrawAAPolyLine(
        new Vector3[]{new Vector3(farLeft, surfaceY, 0.0f),
                      new Vector3(farRight, surfaceY, 0.0f)});
