...
internal class ScriptExecutionOrderInspector : Editor {
  ...
  */
  ...
  public class SortMonoScriptExecutionOrder : SortMonoScriptNameOrder {
    ...
    public SortMonoScriptExecutionOrder(
        ScriptExecutionOrderInspector inspector) {
  ...
  private static readonly List<ScriptExecutionOrderInspector> m_Instances =
      new List<ScriptExecutionOrderInspector>();
  ...
  public class Content {
    public static GUIContent helpText = EditorGUIUtility.TrTextContent(
    ...
    public static GUIContent iconToolbarPlus = EditorGUIUtility.TrIconContent(
        "Toolbar Plus", "Add script to custom order");
    public static GUIContent iconToolbarMinus = EditorGUIUtility.TrIconContent(
        "Toolbar Minus", "Remove script from custom order");
    public static GUIContent defaultTimeContent = EditorGUIUtility.TrTextContent(
        "Default Time",
        "All scripts not in the custom order are executed at the default time.");
  }
  ...
  public static class Styles {
    ...
    public static readonly GUIStyle textfield = new GUIStyle(
        EditorStyles.textField){padding = new RectOffset(2, 2, 1, 0),
                                contentOffset = new Vector2(0, -0.5f),
                                alignment = TextAnchor.UpperLeft};

    public static readonly GUIStyle label = new GUIStyle(EditorStyles.label){
        padding = new RectOffset(1, 1, 2, 1), alignment = TextAnchor.UpperLeft};
  }
  ...
  private void AskApplyRevertIfNecessary() {
    ...
    if (EditorUtility.DisplayDialog("Unapplied execution order",
                                    "Unapplied script execution order", "Apply",
                                    "Revert"))
      Apply();
  }
  ...
  static Object
  MonoScriptValidatorCallback(Object[] references, System.Type objType,
  ...
  static bool IsValidScript(MonoScript script) {
    ...
    // The user can only define the order of scripts that contains valid classes
    // (see case 579536)
    ...
    if (!AssetDatabase.GetAssetFolderInfo(AssetDatabase.GetAssetPath(script),
                                          out isRootFolder, out isReadOnly)) {
      return false;
    }
  ...
  private void Apply() {
    ...
    foreach (var script in changedScripts) {
      ...
      if (string.IsNullOrEmpty(assetPath)) // Script might be outside of the
                                           // project (e.g. in a package).
        continue;
    ...
    if (!AssetDatabase.MakeEditable(changedPaths.ToArray())) {
      Debug.LogError(
          "Could not make scrips editable for changing script execution order.");
      return;
    }
  ...
  private void Revert() { PopulateScriptArray(); }
  ...
  private void AddScriptToCustomOrder(MonoScript script) {
    ...
    int orderOfAddedScript =
        RoundByAmount(GetExecutionOrderAtIndex(m_CustomTimeScripts.Count - 1) +
                          kPreferredSpacing,
                      kPreferredSpacing);
  ...
  private void ShowScriptPopup(Rect r) {
    ...
    if (Provider.enabled) {
      ...
      for (int i = 0; i < length; i++) {
        var metaPath = AssetDatabase.GetTextMetaFilePathFromAssetPath(
            AssetDatabase.GetAssetPath(m_DefaultTimeScripts[i]));
      ...
      const Asset.States kExclusiveLockedRemote =
          Asset.States.Exclusive | Asset.States.LockedRemote;
    ...
    for (int c = 0; c < length; c++) {
      names[c] = m_DefaultTimeScripts [c]
                     .GetClass()
                     .FullName; // TODO: localization with a proper database.
      enabled[c] = !lockedScripts.Contains(
          m_DefaultTimeScripts[c]); // List item is disabled when asset is
                                    // locked
    }
    EditorUtility.DisplayCustomMenu(r, names, enabled, null, MenuSelection,
                                    null);
  }

  private int RoundBasedOnContext(int val, int lowerBound, int upperBound) {
    // Make the bounds a bit closer to avoid the value ending up right next to
    // one of the bounds
  ...
  private void
  SetExecutionOrderAtIndexAccordingToNeighbors(int indexOfChangedItem,
                                               int pushDirection) {
    // Ignore invalid index
    if (indexOfChangedItem < 0 ||
        indexOfChangedItem >= m_CustomTimeScripts.Count)
      return;
    ...
    if (indexOfChangedItem == 0) {
      SetExecutionOrderAtIndex(
          indexOfChangedItem,
          RoundByAmount(GetExecutionOrderAtIndex(indexOfChangedItem + 1) -
                            kPreferredSpacing,
                        kPreferredSpacing));
      return;
    }
    ...
    // Make nr average of prev and next script nr, but rounded to a nice round
    // number
    int prevOrder = GetExecutionOrderAtIndex(indexOfChangedItem - 1);
    int nextOrder = GetExecutionOrderAtIndex(indexOfChangedItem + 1);
    int newAverageExecutionOrder =
        RoundBasedOnContext(GetAverageRoundedAwayFromZero(prevOrder, nextOrder),
                            prevOrder, nextOrder);

    if (newAverageExecutionOrder != 0) {
      if (pushDirection == 0)
        pushDirection =
            GetBestPushDirectionForOrderValue(newAverageExecutionOrder);

      // Ensure new value is at least one higher/lower than the neighbor
      if (pushDirection > 0)
        newAverageExecutionOrder =
            Mathf.Max(newAverageExecutionOrder, prevOrder + 1);
      else
        newAverageExecutionOrder =
            Mathf.Min(newAverageExecutionOrder, nextOrder - 1);
    }
  ...
  private void UpdateOrder(MonoScript changedScript) {
    ...
    // If script order was set to zero, script gets removed from list, so no
    // reordering needed
    ...
    // See if any other scripts have an order that conflicts with the one just
    // moved
  ...
  private void PushAwayToAvoidConflicts(int startIndex, int pushDirection) {
    ...
    while (curIndex >= 0 && curIndex < m_CustomTimeScripts.Count) {
      // Check if there's any conflict between the order at this index and the
      // previous one. If not, stop here.
      if ((GetExecutionOrderAtIndex(curIndex) -
           GetExecutionOrderAtIndex(curIndex - pushDirection)) *
              pushDirection >=
          1)
        break;
  ...
  private int GetBestPushDirectionForOrderValue(int order) {
    // If there's any conflicts we want to push neighboring scripts until the
    // conflict has been resolved. However, we can't push the order of any
    // script past 0 (from either direction) or past the minimum or maximum
    // allowed order values. So we push away from zero if the new order is close
    // to zero and towards zero if it's closer to the min or max. This should
    // only be able to fail if the user has more than 16000 scripts (a quarter
    // of the total range).
  ...
  public override void OnInspectorGUI() {
    ...
    if (Provider.enabled && !Provider.isActive &&
        !EditorUserSettings.WorkOffline) {
      GUI.enabled = false;
    }
    ...
    EditorGUILayout.BeginVertical(EditorStyles.inspectorFullWidthMargins);
    {
      ...
      {
        int dropFieldId = EditorGUIUtility.GetControlID(
            s_DropFieldHash, FocusType.Passive, listRect);
        MonoScript dropped = EditorGUI.DoDropField(
            listRect, dropFieldId, typeof(MonoScript),
            MonoScriptValidatorCallback, false, Styles.dropField) as MonoScript;
        if (dropped)
        ...
        EditorGUILayout.BeginVertical(Styles.boxBackground);
        {
          ...
          m_Scroll = EditorGUILayout.BeginVerticalScrollView(m_Scroll);
          {
            ...
            Rect r = GUILayoutUtility.GetRect(
                10, kListElementHeight * m_CustomTimeScripts.Count,
                GUILayout.ExpandWidth(true));
            int changed = DragReorderGUI.DragReorder(
                r, kListElementHeight, m_CustomTimeScripts, DrawElement);
            if (changed >= 0) {
              ...
              // Neighbors may have been moved so there's more space around
              // dragged item, so set order again to get possible rounding
              // benefits
              SetExecutionOrderAtIndexAccordingToNeighbors(changed, 0);
            }
          }
        ...
        GUILayout.BeginHorizontal(Styles.toolbar);
        {
          ...
          if (EditorGUI.DropdownButton(r2, content, FocusType.Passive,
    ...
    if (Provider.enabled && !Provider.isActive &&
        !EditorUserSettings.WorkOffline) {
      EditorGUILayout.HelpBox("Version control is disconnected",
                              MessageType.Warning);
    }
  ...
  private Rect GetButtonLabelRect(Rect r) {
    return new Rect(r.x + 20, r.y,
                    r.width - GetMinusButtonSize().x - 10 - 20 -
                        (kIntFieldWidth + 5),
                    r.height);
  }
  ...
  private Rect GetAddRemoveButtonRect(Rect r) {
    ...
    return new Rect(r.xMax - buttonSize.x - 5, r.y + 1, buttonSize.x,
                    buttonSize.y);
  }

  private Rect GetFieldRect(Rect r) {
    return new Rect(r.xMax - kIntFieldWidth - GetMinusButtonSize().x - 10,
                    r.y + 2, kIntFieldWidth, r.height - 5);
  }
  ...
  public void DrawElement(Rect r, object obj, bool dragging) {
    ...
    if (Event.current.type == EventType.Repaint) {
      ...
      Styles.draggingHandle.Draw(GetDraggingHandleRect(r), false, false, false,
                                 false);
    }
    ...
    // associate control id with script so that removing an element when its
    // text field is active will not potentially cause subsequent element to
    // inherit value when list is reordered
    int id = GUIUtility.GetControlID(script.GetHashCode(), FocusType.Keyboard,
                                     position);
    string intStr = EditorGUI.DelayedTextFieldInternal(
        position, id, GUIContent.none, oldNr.ToString(), "0123456789-",
        Styles.textfield);
    int newNr = oldNr;
    ...
    if (GUI.Button(GetAddRemoveButtonRect(r), Content.iconToolbarMinus,
                   Styles.removeButton)) {
  ...
  class DragReorderGUI {
    ...
    public static int DragReorder(Rect position, int elementHeight,
                                  List<MonoScript> elements,
                                  DrawElementDelegate drawElementDelegate) {
      ...
      // If we're dragging, draw elements based on their animated reordering
      // positions, but only for repainting. Control event handling does't like
      // to suddenly change order, so things will screw up if we HANDLE the
      // controls in a different order when dragging.
      if (GUIUtility.hotControl == id &&
        ...
        defRect =
            new Rect(elementRect.x,
                     position.y + s_ReorderingPositions[defPos] * elementHeight,
                     elementRect.width,
                     (s_ReorderingPositions[defPos + 1] -
                      s_ReorderingPositions[defPos] + 1) *
                         elementHeight);
      }
      ...
      // draw controls based on their positions in the array.
        ...
        defRect = new Rect(elementRect.x, position.y + defPos * elementHeight,
                           elementRect.width, elementHeight * 2);
      }
      ...
      if (GUIUtility.hotControl == id) {
        ...
        elementRect.y =
            position.y +
            s_ReorderingPositions[s_ReorderingDraggedElement] * elementHeight;
        drawElementDelegate(elementRect, elements[s_ReorderingDraggedElement],
                            true);
      ...
      case EventType.MouseDown:
        if (position.Contains(Event.current.mousePosition)) {
          ...
          s_ReorderingDraggedElement = Mathf.FloorToInt(
              (Event.current.mousePosition.y - position.y) / elementHeight);
      ...
      case EventType.MouseDrag:
        ...
        s_ReorderingPositions[s_ReorderingDraggedElement] =
            (Event.current.mousePosition.y - position.y) / elementHeight - 0.5f;
        // Clamp to range of list
        s_ReorderingPositions[s_ReorderingDraggedElement] =
            Mathf.Clamp(s_ReorderingPositions[s_ReorderingDraggedElement], 0,
                        elements.Count - 1);
        // Set draggedToPosition based on dragged position
        int draggedToPosition =
            Mathf.RoundToInt(s_ReorderingPositions[s_ReorderingDraggedElement]);
        ...
        if (draggedToPosition !=
          ...
          int direction =
              (draggedToPosition > s_ReorderingDraggedElement ? - 1 : 1);

          // Move goals of elements to make room for the dragged one
          for (int i = s_ReorderingDraggedElement; i != draggedToPosition;
               i -= direction)
      ...
      case EventType.MouseUp:
        ...
        if (s_ReorderingGoals[s_ReorderingDraggedElement] !=
      ...
      case EventType.Repaint:
        ...
        if (GUIUtility.hotControl == id) {
          for (int i = 0; i < elements.Count; i++)
            if (i != s_ReorderingDraggedElement)
              s_ReorderingPositions[i] = Mathf.MoveTowards(
                  s_ReorderingPositions[i], s_ReorderingGoals[i], 0.075f);
  ...
  internal static SettingsProvider CreateProjectSettingsProvider() {
    var provider = new AssetSettingsProvider(
        ...
        () => Editor.CreateEditor(
            Resources.FindObjectsOfTypeAll(typeof(MonoManager)).First()),
