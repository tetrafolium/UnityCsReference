...
internal class AssemblyDefinitionReferenceImporterInspector
	: AssetImporterEditor {
internal class Styles {
public static readonly GUIContent assemblyDefinition =
	EditorGUIUtility.TrTextContent("Assembly Definition");
public static readonly GUIContent loadError =
	EditorGUIUtility.TrTextContent("Load error");
public static readonly GUIContent useGUID = EditorGUIUtility.TrTextContent(
	"Use GUID",
	"Use the Assembly Definition asset GUID instead of name for referencing. Allows the referenced assembly to be renamed without having to update references.");
}
...
public AssemblyDefinitionImporterInspector
...
public override void OnInspectorGUI() {
	...
	var obj = EditorGUILayout.ObjectField(
		Styles.assemblyDefinition, m_ReferenceAsset.objectReferenceValue,
	...
	if (EditorGUI.EndChangeCheck()) {
		...
		if (m_ReferenceAsset.objectReferenceValue != null) {
			var data = CustomScriptAssemblyData.FromJson(
				((AssemblyDefinitionAsset) obj).text);
...
protected override void Apply() {
	base.Apply();
	SaveAndUpdateAssemblyDefinitionReferenceStates(
		extraDataTargets.Cast<AssemblyDefinitionReferenceState>().ToArray());
}
...
protected override Type extraDataType =>
typeof(AssemblyDefinitionReferenceState);

protected override void
InitializeExtraDataInstance(UnityEngine.Object extraData, int targetIndex) {
	try {
		LoadAssemblyDefinitionReferenceState(
			(AssemblyDefinitionReferenceState) extraData,
...
static void
LoadAssemblyDefinitionReferenceState(AssemblyDefinitionReferenceState state,
                                     string path) {
	var asset =
		AssetDatabase.LoadAssetAtPath<AssemblyDefinitionReferenceAsset>(path);
	if (asset == null)
		return;
	...
	state.useGUIDs =
		...
		CompilationPipeline.GetAssemblyDefinitionReferenceType(
			data.reference) == AssemblyDefinitionReferenceType.Guid;
	state.reference =
		new AssemblyDefinitionImporterInspector.AssemblyDefinitionReference {
		name = data.reference, serializedReference = data.reference
	};
	state.reference.Load(data.reference, state.useGUIDs);
}

static void SaveAndUpdateAssemblyDefinitionReferenceStates(
	AssemblyDefinitionReferenceState[] states) {
	foreach (var state in states) {
		SaveAndUpdateAssemblyDefinitionReferenceState(state);
	}
}

static void SaveAndUpdateAssemblyDefinitionReferenceState(
	AssemblyDefinitionReferenceState state) {
	CustomScriptAssemblyReferenceData data =
		new CustomScriptAssemblyReferenceData();
	if (state.asset != null) {
		data.reference =
			state.useGUIDs ? CompilationPipeline
			.GUIDToAssemblyDefinitionReferenceGUID(
				AssetDatabase.AssetPathToGUID(
					AssetDatabase.GetAssetPath(state.reference.asset)))
