...
public static void DrawAgentDiagram(Rect rect, float agentRadius,
                                    float agentHeight, float agentClimb,
                                    float agentSlope) {
	...
	scale = Mathf.Min(viewSize /
	                  (cylinderHeight + cylinderRadius * 2.0f * squash),
	...
	for (int i = 0; i < kDivs; i++) {
		...
		cylinderOutline[i] = new Vector3(
			baseX + dx * cylinderRadius,
			baseY - cylinderHeight - dy * cylinderRadius * squash, 0);
		cylinderOutline[i + kDivs] =
			new Vector3(baseX - dx * cylinderRadius,
			            baseY + dy * cylinderRadius * squash, 0);

		topRim[i] = new Vector3(
			baseX - dx * cylinderRadius,
			baseY - cylinderHeight + dy * cylinderRadius * squash, 0);
		climbRim[i] =
			new Vector3(baseX - dx * cylinderRadius,
			            baseY - stepHeight + dy * cylinderRadius * squash, 0);
	}
	...
	Handles.color = EditorGUIUtility.isProSkin ? new Color(0, 0, 0, 0.5f)
	  : new Color(1, 1, 1, 0.5f);
	Handles.DrawAAPolyLine(2.0f, groundReference);

	// Draw ground
	Handles.color = EditorGUIUtility.isProSkin ? new Color(1, 1, 1, 0.5f)
	  : new Color(0, 0, 0, 0.5f);
	Handles.DrawAAPolyLine(3.0f, ground);

	// Draw cylinder background
	Handles.color = Color.Lerp(new Color(0.0f, 0.75f, 1.0f, 1.0f),
	                           new Color(0.5f, 0.5f, 0.5f, 0.5f), 0.2f);
	...
	radiusLine[1] =
		new Vector3(baseX + cylinderRadius, baseY - cylinderHeight, 0);
	Handles.color = new Color(0, 0, 0, 0.5f);
	Handles.DrawAAPolyLine(2.0f, radiusLine);

	// Labels
	GUI.Label(new Rect(baseX + cylinderRadius + 5,
	                   baseY - cylinderHeight * 0.5f - 10, 150, 20),
	          UnityString.Format("H = {0}", agentHeight));
	GUI.Label(new Rect(baseX,
	                   baseY - cylinderHeight - cylinderRadius * squash - 15,
	                   150, 20),
	          UnityString.Format("R = {0}", agentRadius));

	GUI.Label(new Rect((startX + stepX) * 0.5f - 20, startY - 15, 150, 20),
	          UnityString.Format("{0}", agentClimb));
	GUI.Label(new Rect(slopeStartX + 20, slopeStartY - 15, 150, 20),
	          UnityString.Format("{0}\u00b0", agentSlope));
