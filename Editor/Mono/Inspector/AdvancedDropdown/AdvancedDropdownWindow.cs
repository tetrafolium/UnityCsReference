...
namespace UnityEditor.IMGUI.Controls {
  internal class AdvancedDropdownWindow : EditorWindow {
    ...
    private Stack<AdvancedDropdownItem> m_ViewsStack =
        new Stack<AdvancedDropdownItem>();
    ...
    protected virtual bool setInitialSelectionPosition { get; }
    = true;
    ...
    protected virtual void OnEnable() { m_DirtyList = true; }

    protected virtual void OnDisable() {}

    protected virtual void OnDestroy() {
      // This window sets 'editingTextField = true' continuously, through
      // EditorGUI.FocusTextInControl(), for the searchfield in its
      // AdvancedDropdownGUI so here we ensure to clean up. This fixes the issue
      // that EditorGUI.IsEditingTextField() was returning true after e.g the
      // Add Component Menu closes
      EditorGUIUtility.editingTextField = false;
    }

    public static T CreateAndInit<T>(Rect rect,
                                     AdvancedDropdownState state) where T
    ...
    public void Init(Rect buttonRect) {
      ...
      m_CurrentlyRenderedTree =
          hasSearch ? m_DataSource.searchTree : m_DataSource.mainTree;
      ShowAsDropDown(buttonRect, CalculateWindowSize(buttonRect),
                     GetLocationPriority());
      if (setInitialSelectionPosition) {
        m_InitialSelectionPosition =
            m_Gui.GetSelectionHeight(m_DataSource, buttonRect);
      }
      wantsMouseMove = true;
    ...
    protected virtual Vector2 CalculateWindowSize(Rect buttonRect) {
      ...
      var fitRect = ContainerWindow.FitRectToScreen(
          new Rect(buttonRect.x, buttonRect.y, size.x, size.y), true, true);
      // If the scrollbar is visible, we want to add extra space to compensate
      // it
    ...
    internal void OnGUI() {
      GUI.Label(new Rect(0, 0, position.width, position.height),
                GUIContent.none, Styles.background);
    ...
    private void OnGUISearch() {
      m_Gui.DrawSearchField(isSearchFieldDisabled, m_Search,
                            (newSearch) => { searchString = newSearch; });
    }
      ...
      if (evt.type == EventType.KeyDown) {
          ...
          if (selected != null) {
            if (selected.children.Any()) {
              ...
              if (selectionChanged != null) {
                selectionChanged(
                    m_State.GetSelectedChild(m_CurrentlyRenderedTree));
              }
              if (closeOnSelection) {
                CloseWindow();
              }
        ...
        if (!hasSearch) {
          ...
          if (evt.keyCode == KeyCode.RightArrow) {
            ...
            if (idx > -1 && m_CurrentlyRenderedTree.children.ElementAt(idx)
                                .children.Any()) {
              GoToChild();
            }
            evt.Use();
    ...
    protected virtual bool SpecialKeyboardHandling(Event evt) { return false; }
    ...
    private void DrawList(AdvancedDropdownItem item) {
      ...
      m_State.SetScrollState(
          item,
          GUILayout.BeginScrollView(m_State.GetScrollState(item), GUIStyle.none,
      ...
      for (var i = 0; i < item.children.Count(); i++) {
        ...
        if (child.IsSeparator()) {
          ...
          m_Gui.DrawItem(child, child.displayName, child.icon, child.enabled,
                         child.children.Any(), selected, hasSearch);
        }
      ...
      if (m_ScrollToSelected && m_InitialSelectionPosition != 0) {
        ...
        m_State.SetScrollState(item,
                               new Vector2(0, m_InitialSelectionPosition -
                                                  diffOfPopupAboveTheButton));
      ...
      // Scroll to show selected
        ...
        if (selectedRect.yMax - scrollRect.height >
            m_State.GetScrollState(item).y) {
          m_State.SetScrollState(
              item, new Vector2(0, selectedRect.yMax - scrollRect.height));
          Repaint();
        }
