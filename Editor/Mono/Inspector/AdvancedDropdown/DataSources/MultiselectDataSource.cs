...
public MultiselectDataSource(Enum enumValue) {
	...
	MaskFieldGUI.GetMenuOptions(
		m_Mask, m_DisplayNames, m_FlagValues, out buttonText,
		out m_OptionNames, out m_OptionMaskValues, out m_SelectedOptions);
}

public MultiselectDataSource(int mask, string[] displayedOptions,
                             int[] flagValues) {
	m_DisplayNames = displayedOptions;
	m_FlagValues = flagValues;
	string buttonText;
	MaskFieldGUI.GetMenuOptions(
		mask, displayedOptions, flagValues, out buttonText, out m_OptionNames,
...
public void UpdateSelectedId(AdvancedDropdownItem item) {
	...
	if (enumFlags != null)
		m_EnumFlag =
			EnumDataUtility.IntToEnumFlags(enumFlags.GetType(), m_Mask);
