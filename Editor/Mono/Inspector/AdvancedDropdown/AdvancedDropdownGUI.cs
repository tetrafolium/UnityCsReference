...
namespace UnityEditor.IMGUI.Controls {
  internal class AdvancedDropdownGUI {
    private static class Styles {
      ...
      public static GUIStyle searchFieldStyle = new GUIStyle(
          EditorStyles.toolbarSearchField){margin = new RectOffset(5, 4, 4, 5)};
      public static SVC<Color> searchBackgroundColor =
          new SVC<Color>("--theme-toolbar-background-color", Color.black);
    ...
    internal virtual void DrawItem(AdvancedDropdownItem item, string name,
                                   Texture2D icon, bool enabled, bool drawArrow,
                                   bool selected, bool hasSearch) {
      ...
      var rect = GUILayoutUtility.GetRect(content, lineStyle,
      ...
      if (m_DataSource.selectedIDs.Any() &&
          m_DataSource.selectedIDs.Contains(item.id)) {
        var checkMarkRect = new Rect(rect);
        checkMarkRect.width = iconSize.x + 1;
        Styles.checkMark.Draw(checkMarkRect, Styles.checkMarkContent, false,
      ...
      if (drawArrow) {
        var yOffset =
            (lineStyle.fixedHeight - Styles.rightArrow.fixedHeight) / 2;
        Rect arrowRect =
            new Rect(rect.xMax - Styles.rightArrow.fixedWidth -
                         Styles.rightArrow.margin.right,
                     rect.y + yOffset, Styles.rightArrow.fixedWidth,
    ...
    internal virtual void DrawLineSeparator() {
      ...
      Color tintColor = (EditorGUIUtility.isProSkin)
    ...
    internal void DrawHeader(AdvancedDropdownItem group,
                             Action backButtonPressed, bool hasParent) {
      var content = GUIContent.Temp(group.displayName, group.icon);
      m_HeaderRect = GUILayoutUtility.GetRect(content, Styles.header,
                                              GUILayout.ExpandWidth(true));
      ...
      if (hasParent) {
        ...
        var arrowRect =
            new Rect(m_HeaderRect.x + Styles.leftArrow.margin.left,
                     m_HeaderRect.y + yOffset, Styles.leftArrow.fixedWidth,
    ...
    internal virtual string DrawSearchFieldControl(string searchString) {
      ...
      var newSearch =
          EditorGUI.ToolbarSearchField(controlRect, searchString, false);
    ...
    internal
        Vector2 CalculateContentSize(AdvancedDropdownDataSource dataSource) {
      float maxWidth = 0;
      float maxHeight = 0;
      ...
      foreach (var child in dataSource.mainTree.children) {
        ...
        if (child.IsSeparator()) {
          maxHeight += Styles.lineSeparator.CalcHeight(content, maxWidth) +
                       Styles.lineSeparator.margin.vertical;
        } else {
    ...
    internal float GetSelectionHeight(AdvancedDropdownDataSource dataSource,
                                      Rect buttonRect) {
      ...
      for (int i = 0; i < dataSource.mainTree.children.Count(); i++) {
        ...
        if (state.GetSelectedIndex(dataSource.mainTree) == i) {
          var diff =
              (lineStyle.CalcHeight(content, 0) - buttonRect.height) / 2f;
          return heigth + diff;
        }
        if (child.IsSeparator()) {
          heigth += Styles.lineSeparator.CalcHeight(content, 0) +
                    Styles.lineSeparator.margin.vertical;
        } else {
