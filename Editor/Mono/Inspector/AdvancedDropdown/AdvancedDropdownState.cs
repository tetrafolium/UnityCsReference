...
namespace UnityEditor.IMGUI.Controls {
  ...
  public class AdvancedDropdownState {
    ...
    [SerializeField]
    private AdvancedDropdownItemState[] states =
        new AdvancedDropdownItemState[0];
    private AdvancedDropdownItemState m_LastSelectedState;

    private AdvancedDropdownItemState
    GetStateForItem(AdvancedDropdownItem item) {
    ...
    internal void MoveDownSelection(AdvancedDropdownItem item) {
      ...
      do {
        ++selectedIndex;
      } while (selectedIndex < item.children.Count() &&
               item.children.ElementAt(selectedIndex).IsSeparator());
    ...
    internal void SetSelectionOnItem(AdvancedDropdownItem item,
                                     int selectedIndex) {
      var state = GetStateForItem(item);
    ...
    internal void SetScrollState(AdvancedDropdownItem item,
                                 Vector2 scrollState) {
