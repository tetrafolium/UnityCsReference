...
internal static partial class StatelessAdvancedDropdown {
  ...
  private static void InitSearchableWindow(Rect rect, string label,
                                           int selectedIndex,
  ...
  private static void InitPopupWindow(Rect rect, int selectedIndex,
  ...
  private static void
  InitMultiselectPopupWindow(Rect rect, MultiselectDataSource dataSource) {
  ...
  internal static int DoSearchablePopup(Rect rect, int selectedIndex,
                                        string[] displayedOptions,
                                        GUIStyle style) {
    string contentLabel = "";
    if (selectedIndex >= 0) {
    ...
    int id = EditorGUIUtility.GetControlID("AdvancedDropdown".GetHashCode(),
  ...
  internal static int
  DoLazySearchablePopup(Rect rect, string selectedOption, int selectedIndex,
    ...
    if (EditorGUI.DropdownButton(id, rect, content, style)) {
      ...
      InitSearchableWindow(rect, content.text, displayData.Item1,
                           displayData.Item2);
  ...
  public static int DoPopup(Rect rect, int selectedIndex,
                            GUIContent[] displayedOptions) {
  ...
  public static Enum DoEnumMaskPopup(Rect rect, Enum options, GUIStyle style) {
    ...
    int[] selectedOptions;
    MaskFieldGUI.GetMenuOptions(
        optionValue, enumData.displayNames, enumData.flagValues, out buttonText,
        out optionNames, out optionMaskValues, out selectedOptions);

    var id = EditorGUIUtility.GetControlID("AdvancedDropdown".GetHashCode(),
                                           FocusType.Keyboard, rect);

    if (EditorGUI.DropdownButton(id, rect, GUIContent.Temp(buttonText),
                                 EditorStyles.popup)) {
      s_CurrentControl = id;
      ResetAndCreateWindow();
      ...
      s_Instance.selectionChanged += i => { m_ShouldReturnValue = true; };
      s_Instance.windowClosed += w => { m_WindowClosed = true; };
    }
  ...
  private static Enum DoEnumPopup(Rect rect, Enum selected, GUIStyle style,
    ...
    using(new LocalizationGroup(enumType)) {
      i = DoPopup(rect, i,
                  EditorGUIUtility.TrTempContent(enumData.displayNames,
                                                 enumData.tooltip));
    }
  ...
  private static int DoIntPopup(Rect rect, int selectedValue,
                                string[] displayedOptions, int[] optionValues) {
    var idx = Array.IndexOf(optionValues, selectedValue);
    var returnedValue =
        DoPopup(rect, idx, EditorGUIUtility.TempContent(displayedOptions));
    return returnedValue >= 0 ? optionValues[returnedValue] : returnedValue;
  }

  static int DoMaskField(Rect rect, int mask, string[] displayedOptions,
                         GUIStyle popup) {
    ...
    int[] optionMaskValues;
    int[] selectedOptions;
    MaskFieldGUI.GetMenuOptions(mask, displayedOptions, flagValues,
                                out buttonText, out optionNames,
    ...
    if (EditorGUI.DropdownButton(id, rect, GUIContent.Temp(buttonText),
      ...
      ResetAndCreateWindow();
      var dataSource =
          new MultiselectDataSource(mask, displayedOptions, flagValues);
      InitMultiselectPopupWindow(rect, dataSource);
      s_Instance.selectionChanged += dataSource.UpdateSelectedId;
      s_Instance.selectionChanged += i => { m_ShouldReturnValue = true; };
      s_Instance.windowClosed += w => { m_WindowClosed = true; };
