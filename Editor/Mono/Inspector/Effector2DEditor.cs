...
/// <summary>
/// Prompts the end-user to add 2D colliders if non exist for 2D effector to
/// work with.
/// </summary>
...
internal class Effector2DEditor : Editor {
  ...
  public override void OnInspectorGUI() {
    ...
    if (effector.GetComponents<Collider2D>().Any(
            collider => collider.enabled && collider.usedByEffector))
      return;
    ...
    if (effector.requiresCollider)
      ...
      EditorGUILayout.HelpBox(
          "This effector can optionally work without a 2D collider.",
          MessageType.Info);
  }
  ...
  public static void CheckEffectorWarnings(Collider2D collider) {
    ...
    if (effector == null || !effector.enabled) {
      // Show warning.
      EditorGUILayout.HelpBox(
          "This collider will not function with an effector until there is at least one enabled 2D effector on this GameObject.",
          MessageType.Warning);
    ...
    if (effector.designedForNonTrigger && collider.isTrigger) {
      // Show warning.
      EditorGUILayout.HelpBox(
          "This collider has 'Is Trigger' checked but this should be unchecked when used with the '" +
              effector.GetType().Name +
              "' component which is designed to work with collisions.",
          MessageType.Warning);
    } else if (effector.designedForTrigger && !collider.isTrigger) {
      // Show warning.
      EditorGUILayout.HelpBox(
          "This collider has 'Is Trigger' unchecked but this should be checked when used with the '" +
              effector.GetType().Name +
              "' component which is designed to work with triggers.",
          MessageType.Warning);
    }
  }
}
