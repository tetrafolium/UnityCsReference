...
internal class AvatarMaskUtility {
  ...
  static public string[] GetAvatarHumanTransform(SerializedObject so,
                                                 string[] refTransformsPath) {
    ...
    for (int i = 0; i < humanBoneArray.arraySize; i++) {
      SerializedProperty transformNameP =
          humanBoneArray.GetArrayElementAtIndex(i).FindPropertyRelative(
              sBoneName);
      humanTransforms.Add(transformNameP.stringValue);
    }

    return TokeniseHumanTransformsPath(refTransformsPath,
                                       humanTransforms.ToArray());
  }

  static public string[] GetAvatarHumanAndActiveExtraTransforms(
      SerializedObject so, SerializedProperty transformMaskProperty,
      string[] refTransformsPath) {
    SerializedProperty humanBoneArray = so.FindProperty(sHuman);
    if (humanBoneArray == null || !humanBoneArray.isArray)
      return null;
    ...
    List<string> values = new List<string>(TokeniseHumanTransformsPath(
        refTransformsPath, humanTransforms.ToArray()));

    for (int i = 0; i < transformMaskProperty.arraySize; i++) {
      float weight = transformMaskProperty.GetArrayElementAtIndex(i)
                         .FindPropertyRelative("m_Weight")
                         .floatValue;
      string transformName = transformMaskProperty.GetArrayElementAtIndex(i)
  ...
  static public string[] GetAvatarInactiveTransformMaskPaths(
      SerializedProperty transformMaskProperty) {
    ...
    for (int i = 0; i < transformMaskProperty.arraySize; i++) {
      SerializedProperty weight =
          transformMaskProperty.GetArrayElementAtIndex(i).FindPropertyRelative(
              "m_Weight");
      if (weight.floatValue < 0.5f) {
        SerializedProperty transformNameP =
            transformMaskProperty.GetArrayElementAtIndex(i)
                .FindPropertyRelative("m_Path");
  ...
  static public void UpdateTransformMask(AvatarMask mask,
                                         string[] refTransformsPath,
    ...
    for (int i = 0; i < refTransformsPath.Length; i++) {
      ...
      bool isActiveTransform = humanTransforms ==
                               null
                                     ? true
          : ArrayUtility.FindIndex(humanTransforms,
                                   s => refTransformsPath[i] == s) != -1;
      mask.SetTransformActive(i, isActiveTransform);
    }
  }

  static public void UpdateTransformMask(SerializedProperty transformMask,
                                         string[] refTransformsPath,
                                         string[] currentPaths,
                                         bool areActivePaths = true) {
    // if areActivePaths=true, currentPaths is treated as the list of active
    // transform paths else, currentPaths is treated as the list of inactive
    // transform paths
    ...
    for (int i = 0; i < refTransformsPath.Length; i++) {
      ...
      if (currentPaths == null)
        ...
        isActiveTransform =
            ArrayUtility.FindIndex(currentPaths,
                                   s => refTransformsPath[i] == s) != -1;
      else
        isActiveTransform =
            ArrayUtility.FindIndex(currentPaths,
  ...
  static public void SetActiveHumanTransforms(AvatarMask mask,
                                              string[] humanTransforms) {
  ...
  static private string[] TokeniseHumanTransformsPath(
      string[] refTransformsPath, string[] humanTransforms) {
    ...
    for (int i = 0; i < humanTransforms.Length; i++) {
      int index1 = ArrayUtility.FindIndex(
          refTransformsPath,
          s => humanTransforms[i] == FileUtil.GetLastPathNameComponent(s));
      if (index1 != -1) {
        ...
        while (path.Length > 0) {
          int index2 =
              ArrayUtility.FindIndex(tokeniseTransformsPath, s => path == s);
          if (index2 == -1)
