...
static readonly Color k_ColorThemeCameraGizmo =
	new Color(233f / 255f, 233f / 255f, 233f / 255f, 128f / 255f);
static readonly Color k_ColorThemeCameraSensorGizmo =
	new Color(180f / 255f, 180f / 255f, 180f / 255f, 128f / 255f);

public static float GameViewAspectRatio =>
CameraEditor.GetGameViewAspectRatio();
...
public static void HandleFrustum(Camera c, int cameraEditorTargetIndex) {
	...
	// manage our own gui changed state, so we can use it for individual slider
	// changes
	bool guiChanged = GUI.changed;

	Vector3 farMid = Vector3.Lerp(leftBottomFar, rightTopFar, 0.5f);
	if (s_MovingHandleId != 0) {
		if (!s_FrustumHandleIds.Contains(GUIUtility.hotControl -
		                                 cameraEditorTargetIndex))
			s_MovingHandleId = GUIUtility.hotControl;
		else
			farMid = s_InitialFarMid;
	} else if (s_FrustumHandleIds.Contains(GUIUtility.hotControl -
	                                       cameraEditorTargetIndex)) {
		s_MovingHandleId = GUIUtility.hotControl;
	...
	Vector3 changedPosition =
		MidPointPositionSlider(s_FrustumHandleIds[0] + cameraEditorTargetIndex,
		                       leftTopFar, rightTopFar, c.transform.up);
	if (!GUI.changed)
		changedPosition = MidPointPositionSlider(
			s_FrustumHandleIds[1] + cameraEditorTargetIndex, leftBottomFar,
			rightBottomFar, -c.transform.up);
	if (GUI.changed)
	...
	GUI.changed = false;
	changedPosition =
		MidPointPositionSlider(s_FrustumHandleIds[2] + cameraEditorTargetIndex,
		                       rightBottomFar, rightTopFar, c.transform.right);
	if (!GUI.changed)
		changedPosition = MidPointPositionSlider(
			s_FrustumHandleIds[3] + cameraEditorTargetIndex, leftBottomFar,
			leftTopFar, -c.transform.right);
	if (GUI.changed)
...
// Returns near- and far-corners in this order: leftBottom, leftTop, rightTop,
// rightBottom Assumes input arrays are of length 4 (if allocated)
public static bool TryGetSensorGateFrustum(Camera camera, Vector3[] near,
                                           Vector3[] far,
                                           out float frustumAspect) {
	...
	if (far != null) {
		Vector2 planeSize;
		planeSize.y = camera.farClipPlane *
		              Mathf.Tan(Mathf.Deg2Rad * camera.fieldOfView * 0.5f);
		...
		Vector3 planePosition =
			camera.cameraToWorldMatrix.MultiplyPoint(localAim);
	...
	if (near != null) {
		Vector2 planeSize;
		planeSize.y = 2.0f * camera.nearClipPlane *
		              Mathf.Tan(Mathf.Deg2Rad * camera.fieldOfView * 0.5f);
		planeSize.x = planeSize.y * camera.sensorSize.x / camera.sensorSize.y;
...
// Returns near- and far-corners in this order: leftBottom, leftTop, rightTop,
// rightBottom Assumes input arrays are of length 4 (if allocated)
public static bool TryGetFrustum(Camera camera, Vector3[] near, Vector3[] far,
                                 out float frustumAspect) {
	frustumAspect = GetFrustumAspectRatio(camera);
	if (frustumAspect < 0)
		return false;
	...
	if (far != null) {
		if (camera.projectionMatrixMode ==
		    (int) Camera.ProjectionMatrixMode.Explicit) {
...
private static void CalculateFrustumPlaneAt(Camera camera, float distance,
                                            Vector3[] plane) {
...
public static bool IsViewportRectValidToRender(Rect normalizedViewPortRect) {
	if (normalizedViewPortRect.width <= 0f ||
	    normalizedViewPortRect.height <= 0f)
		return false;
...
public static float GetFrustumAspectRatio(Camera camera) {
	...
	return camera.usePhysicalProperties ? camera.sensorSize.x /
	       camera.sensorSize.y : GameViewAspectRatio *
	       normalizedViewPortRect.width / normalizedViewPortRect.height;
}

public static Vector3 PerspectiveClipToWorld(Matrix4x4 clipToWorld,
...
public static void GetFrustumPlaneAt(Matrix4x4 clipToWorld,
                                     Vector3 viewPosition, float distance,
                                     Vector3[] points) {
	...
	for (var i = 0; i < 4; ++i)
		points [i]
		        = PerspectiveClipToWorld(clipToWorld, viewPosition, points[i]);
}

static Vector3 MidPointPositionSlider(int controlID, Vector3 position1,
                                      Vector3 position2, Vector3 direction) {
	Vector3 midPoint = Vector3.Lerp(position1, position2, 0.5f);
	return Handles.Slider(controlID, midPoint, direction,
	                      HandleUtility.GetHandleSize(midPoint) * 0.03f,
