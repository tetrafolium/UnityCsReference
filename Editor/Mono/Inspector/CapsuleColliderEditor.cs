...
class CapsuleColliderTool : PrimitiveColliderTool<CapsuleCollider> {
  ...
  CopyColliderPropertiesToHandle(CapsuleCollider collider) {
    m_BoundsHandle.center = TransformColliderCenterToHandleSpace(
        collider.transform, collider.center);

    float radiusScaleFactor;
    Vector3 sizeScale = GetCapsuleColliderHandleScale(
        collider.transform.lossyScale, collider.direction,
        out radiusScaleFactor);

    m_BoundsHandle.height = m_BoundsHandle.radius = 0f;
    m_BoundsHandle.height =
        collider.height * Mathf.Abs(sizeScale[collider.direction]);
  ...
  protected override void
  CopyHandlePropertiesToCollider(CapsuleCollider collider) {
    collider.center = TransformHandleCenterToColliderSpace(
        collider.transform, m_BoundsHandle.center);
    ...
    // only apply changes to collider radius/height if scale factor from
    ...
    if (sizeScale[collider.direction] != 0f)
      collider.height =
          m_BoundsHandle.height * Mathf.Abs(sizeScale[collider.direction]);
  }

  public override void OnToolGUI(EditorWindow window) {
    // prevent possibility that user increases height if radius scale is zero
    // and user drags (non-moving) radius handles to exceed height extents
    CapsuleCollider collider = (CapsuleCollider) target;
    float radiusScaleFactor;
    GetCapsuleColliderHandleScale(collider.transform.lossyScale,
                                  collider.direction, out radiusScaleFactor);
  ...
  static Vector3 GetCapsuleColliderHandleScale(Vector3 lossyScale,
                                               int capsuleDirection,
                                               out float radiusScaleFactor) {
    radiusScaleFactor = 0f;

    for (int axis = 0; axis < 3; ++axis) {
      if (axis != capsuleDirection)
        radiusScaleFactor =
            Mathf.Max(radiusScaleFactor, Mathf.Abs(lossyScale[axis]));
    }
