...
internal class SkinnedMeshRendererEditor : RendererEditorBase {
  class Styles {
    ...
    public static readonly GUIContent meshNotSupportingSkinningInfo =
        EditorGUIUtility.TrTextContent(
    ...
    public static readonly GUIContent bounds =
        EditorGUIUtility.TrTextContent("Bounds");
    public static readonly GUIContent quality = EditorGUIUtility.TrTextContent(
        "Quality", "Number of bones to use per vertex during skinning.");
    public static readonly GUIContent updateWhenOffscreen =
        EditorGUIUtility.TrTextContent(
            "Update When Offscreen",
            "If an accurate bounding volume representation should be calculated every frame. ");
    public static readonly GUIContent mesh =
        EditorGUIUtility.TrTextContent("Mesh");
    public static readonly GUIContent rootBone =
        EditorGUIUtility.TrTextContent("Root Bone");
  }
  ...
  public override void OnEnable() {
    ...
    m_UpdateWhenOffscreen =
        serializedObject.FindProperty("m_UpdateWhenOffscreen");
  ...
  public override void OnInspectorGUI() {
    ...
    EditMode.DoEditModeInspectorModeButton(
        EditMode.SceneViewEditMode.Collider, "Edit Bounds",
        PrimitiveBoundsHandle.editModeButton, this);
    ...
    EditorGUILayout.PropertyField(m_UpdateWhenOffscreen,
                                  Styles.updateWhenOffscreen);
  ...
  public void OnMeshUI() {
    ...
    if (renderer.sharedMesh != null) {
      bool haveClothComponent =
          renderer.gameObject.GetComponent<Cloth>() != null;

      if (!haveClothComponent && renderer.sharedMesh.blendShapeCount == 0 &&
          (renderer.sharedMesh.boneWeights.Length == 0 ||
           renderer.sharedMesh.bindposes.Length == 0)) {
        EditorGUILayout.HelpBox(Styles.meshNotSupportingSkinningInfo.text,
                                MessageType.Error);
      }
    }
  }
  ...
  public void OnBlendShapeUI() {
    SkinnedMeshRenderer renderer = (SkinnedMeshRenderer) target;
    int blendShapeCount =
        renderer.sharedMesh == null ? 0 : renderer.sharedMesh.blendShapeCount;
    ...
    if (PlayerSettings.legacyClampBlendShapeWeights)
      EditorGUILayout.HelpBox(Styles.legacyClampBlendShapeWeightsInfo.text,
                              MessageType.Info);
    ...
    for (int i = 0; i < blendShapeCount; i++) {
      ...
      // Calculate the min and max values for the slider from the frame
      // blendshape weights
      ...
      // The SkinnedMeshRenderer blendshape weights array size can be out of
      // sync with the size defined in the mesh (default values in that case are
      // 0) The desired behaviour is to resize the blendshape array on edit.

      // Default path when the blend shape array size is big enough.
      if (i < arraySize)
        EditorGUILayout.Slider(m_BlendShapeWeights.GetArrayElementAtIndex(i),
                               sliderMin, sliderMax, float.MinValue,
                               float.MaxValue, content);
      // Fall back to 0 based editing &
      else {
        EditorGUI.BeginChangeCheck();

        float value = EditorGUILayout.Slider(content, 0f, sliderMin, sliderMax,
  ...
  public void OnSceneGUI() {
    ...
    if (renderer.updateWhenOffscreen) {
      ...
      using(new Handles.DrawingScope(
          renderer.actualRootBone.localToWorldMatrix)) {
        ...
        m_BoundsHandle.handleColor =
            EditMode.editMode == EditMode.SceneViewEditMode.Collider &&
                    EditMode.IsOwner(this)
                ? m_BoundsHandle.wireframeColor
                : Color.clear;
        ...
        if (EditorGUI.EndChangeCheck()) {
          ...
          renderer.localBounds =
              new Bounds(m_BoundsHandle.center, m_BoundsHandle.size);
        }
      }
    }
  }
