...
internal class LayoutDropdownWindow : PopupWindowContent {
  ...
  static float[] kPivotsForModes = new float[]{
      0, 0.5f, 1, 0.5f, 0.5f}; // Only for actual modes, not for Undefined.
  static string[] kHLabels =
      new string[]{"custom", "left", "center", "right", "stretch", "%"};
  static string[] kVLabels =
      new string[]{"custom", "top", "middle", "bottom", "stretch", "%"};
  ...
  public override void OnGUI(Rect rect) {
    ...
    GUI.Label(new Rect(rect.x + 5, rect.y + 3, rect.width - 10, 16),
              EditorGUIUtility.TrTextContent("Anchor Presets"),
              EditorStyles.boldLabel);
    GUI.Label(new Rect(rect.x + 5, rect.y + 3 + 16, rect.width - 10, 16),
    ...
    GUI.DrawTexture(new Rect(0, kTopPartHeight - 1, 400, 1),
                    EditorGUIUtility.whiteTexture);
    GUI.color = oldColor;

    GUI.BeginGroup(new Rect(rect.x, rect.y + kTopPartHeight, rect.width,
                            rect.height - kTopPartHeight));
    TableGUI(rect);
    GUI.EndGroup();
  }
  ...
  internal static void DrawLayoutModeHeadersOutsideRect(
      Rect rect, SerializedProperty anchorMin, SerializedProperty anchorMax,
      SerializedProperty position, SerializedProperty sizeDelta) {
  ...
  internal static void DrawLayoutModeHeaderOutsideRect(Rect position, int axis,
                                                       LayoutMode mode) {
    ...
    GUI.Label(headerRect, axis == 0 ? kHLabels[index] : kVLabels[index],
              s_Styles.label);
  ...
  void TableGUI(Rect rect) {
    ...
    GUI.DrawTexture(new Rect(0, 0, 400, headerW),
                    EditorGUIUtility.whiteTexture);
    GUI.DrawTexture(new Rect(0, 0, headerW, 400),
                    EditorGUIUtility.whiteTexture);
    GUI.color = s_Styles.tableLineColor * oldColor;
    GUI.DrawTexture(new Rect(0, headerW, 400, 1),
                    EditorGUIUtility.whiteTexture);
    GUI.DrawTexture(new Rect(headerW, 0, 1, 400),
                    EditorGUIUtility.whiteTexture);
    GUI.color = oldColor;
    ...
    for (int i = 0; i < number; i++) {
      ...
      for (int j = 0; j < number; j++) {
        ...
        Rect position =
            new Rect(i * (size + spacing) + groupings[i],
                     j * (size + spacing) + groupings[j], size, size);
        ...
        bool selectedHeader =
            (i == 0 && cellVMode == vMode) || (j == 0 && cellHMode == hMode);
        ...
        DrawLayoutMode(new Rect(position.x + padding, position.y + padding,
                                position.width - padding * 2,
                                position.height - padding * 2),
                       cellHMode, cellVMode, doPivot, doPosition);

        int clickCount = Event.current.clickCount;
        if (GUI.Button(position, GUIContent.none, GUIStyle.none)) {
          SetLayoutModeForAxis(m_AnchorMin, 0, cellHMode, doPivot, doPosition,
                               m_InitValues);
          SetLayoutModeForAxis(m_AnchorMin, 1, SwappedVMode(cellVMode), doPivot,
                               doPosition, m_InitValues);
          if (clickCount == 2)
            editorWindow.Close();
          else
  ...
  static LayoutMode GetLayoutModeForAxis(SerializedProperty anchorMin,
                                         SerializedProperty anchorMax,
                                         int axis) {
    if (anchorMin.vector2Value[axis] == 0 && anchorMax.vector2Value[axis] == 0)
      return LayoutMode.Min;
    if (anchorMin.vector2Value[axis] == 0.5f &&
        anchorMax.vector2Value[axis] == 0.5f)
  ...
  static void SetLayoutModeForAxis(SerializedProperty anchorMin, int axis,
                                   LayoutMode layoutMode, bool doPivot,
                                   bool doPosition, Vector2[, ] defaultValues) {
    ...
    for (int i = 0; i < targetObjects.Length; i++) {
      ...
      if (doPosition) {
        if (defaultValues != null && defaultValues.Length > i) {
          ...
          temp = gui.anchorMin;
          temp[axis] = defaultValues [i, 0]
          [axis];
          gui.anchorMin = temp;

          temp = gui.anchorMax;
          temp[axis] = defaultValues [i, 1]
          [axis];
          gui.anchorMax = temp;

          temp = gui.anchoredPosition;
          temp[axis] = defaultValues [i, 2]
          [axis];
          gui.anchoredPosition = temp;

          temp = gui.sizeDelta;
          temp[axis] = defaultValues [i, 3]
          [axis];
          gui.sizeDelta = temp;
        }
      }
      ...
      if (doPivot && layoutMode != LayoutMode.Undefined) {
        RectTransformEditor.SetPivotSmart(
            gui, kPivotsForModes[(int) layoutMode], axis, true, true);
      }
  ...
  internal static void DrawLayoutMode(Rect rect, SerializedProperty anchorMin,
                                      SerializedProperty anchorMax,
  ...
  internal static void DrawLayoutMode(Rect position, LayoutMode hMode,
                                      LayoutMode vMode) {
    DrawLayoutMode(position, hMode, vMode, false, false);
  }

  internal static void DrawLayoutMode(Rect position, LayoutMode hMode,
                                      LayoutMode vMode, bool doPivot) {
  ...
  internal static void DrawLayoutMode(Rect position, LayoutMode hMode,
    ...
    // Make parent size the largest possible square, but enforce it's an uneven
    // number.
    ...
    Rect outer = new Rect(position.x + padding.x, position.y + padding.y,
                          parentSize.x, parentSize.y);
    Rect inner = new Rect(position.x + padding2.x, position.y + padding2.y,
                          selfSize.x, selfSize.y);
    if (doPosition) {
    ...
    if (hMode != LayoutMode.Undefined && hMode != LayoutMode.Stretch) {
      ...
      GUI.DrawTexture(
          new Rect(anchor.xMin - 0.5f, outer.y + 1, 1, outer.height - 2),
          EditorGUIUtility.whiteTexture);
      GUI.DrawTexture(
          new Rect(anchor.xMax - 0.5f, outer.y + 1, 1, outer.height - 2),
          EditorGUIUtility.whiteTexture);
    }
    if (vMode != LayoutMode.Undefined && vMode != LayoutMode.Stretch) {
      GUI.color = s_Styles.simpleAnchorColor * oldColor;
      GUI.DrawTexture(
          new Rect(outer.x + 1, anchor.yMin - 0.5f, outer.width - 2, 1),
          EditorGUIUtility.whiteTexture);
      GUI.DrawTexture(
          new Rect(outer.x + 1, anchor.yMax - 0.5f, outer.width - 2, 1),
          EditorGUIUtility.whiteTexture);
    }
    ...
    if (hMode == LayoutMode.Stretch) {
      ...
      DrawArrow(
          new Rect(inner.x + 1, inner.center.y - 0.5f, inner.width - 2, 1));
    }
    if (vMode == LayoutMode.Stretch) {
      GUI.color = s_Styles.stretchAnchorColor * oldColor;
      DrawArrow(
          new Rect(inner.center.x - 0.5f, inner.y + 1, 1, inner.height - 2));
    }
    ...
    // Draw pivot
    if (doPivot && hMode != LayoutMode.Undefined &&
        vMode != LayoutMode.Undefined) {
      Vector2 pivot =
          new Vector2(Mathf.Lerp(inner.xMin + 0.5f, inner.xMax - 0.5f,
                                 kPivotsForModes[(int) hMode]),
                      Mathf.Lerp(inner.yMin + 0.5f, inner.yMax - 0.5f,
                                 kPivotsForModes[(int) vMode]));

      GUI.color = s_Styles.pivotColor * oldColor;
      GUI.DrawTexture(new Rect(pivot.x - 2.5f, pivot.y - 1.5f, 5, 3),
                      EditorGUIUtility.whiteTexture);
      GUI.DrawTexture(new Rect(pivot.x - 1.5f, pivot.y - 2.5f, 3, 5),
                      EditorGUIUtility.whiteTexture);
    }
    ...
    if (hMode != LayoutMode.Undefined && vMode != LayoutMode.Undefined) {
      ...
      GUI.DrawTexture(new Rect(anchor.xMin - 1.5f, anchor.yMin - 1.5f, 2, 2),
                      EditorGUIUtility.whiteTexture);
      GUI.DrawTexture(new Rect(anchor.xMax - 0.5f, anchor.yMin - 1.5f, 2, 2),
                      EditorGUIUtility.whiteTexture);
      GUI.DrawTexture(new Rect(anchor.xMin - 1.5f, anchor.yMax - 0.5f, 2, 2),
                      EditorGUIUtility.whiteTexture);
      GUI.DrawTexture(new Rect(anchor.xMax - 0.5f, anchor.yMax - 0.5f, 2, 2),
                      EditorGUIUtility.whiteTexture);
    }
  ...
  static void DrawArrow(Rect lineRect) {
    ...
    if (lineRect.width == 1) {
      GUI.DrawTexture(new Rect(lineRect.x - 1, lineRect.y + 1, 3, 1),
                      EditorGUIUtility.whiteTexture);
      GUI.DrawTexture(new Rect(lineRect.x - 2, lineRect.y + 2, 5, 1),
                      EditorGUIUtility.whiteTexture);
      GUI.DrawTexture(new Rect(lineRect.x - 1, lineRect.yMax - 2, 3, 1),
                      EditorGUIUtility.whiteTexture);
      GUI.DrawTexture(new Rect(lineRect.x - 2, lineRect.yMax - 3, 5, 1),
                      EditorGUIUtility.whiteTexture);
    } else {
      GUI.DrawTexture(new Rect(lineRect.x + 1, lineRect.y - 1, 1, 3),
                      EditorGUIUtility.whiteTexture);
      GUI.DrawTexture(new Rect(lineRect.x + 2, lineRect.y - 2, 1, 5),
                      EditorGUIUtility.whiteTexture);
      GUI.DrawTexture(new Rect(lineRect.xMax - 2, lineRect.y - 1, 1, 3),
                      EditorGUIUtility.whiteTexture);
      GUI.DrawTexture(new Rect(lineRect.xMax - 3, lineRect.y - 2, 1, 5),
                      EditorGUIUtility.whiteTexture);
    }
  }
}
