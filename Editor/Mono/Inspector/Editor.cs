...
public class ObjectPreview : IPreviewable {
  ...
  public virtual void ResetTarget() { m_ReferenceTargetIndex = 0; }

  public virtual UnityObject target {
    get { return m_Targets[m_ReferenceTargetIndex]; }
  }
  ...
  public virtual GUIContent GetPreviewTitle() {
    ...
    if (m_Targets.Length == 1)
      ...
      if (NativeClassExtensionUtilities.ExtendsANativeType(target))
        guiContent.text +=
            MonoScript.FromScriptedObject(target).GetClass().Name;
      else
        guiContent.text +=
            ObjectNames.NicifyVariableName(ObjectNames.GetClassName(target));
  ...
  public virtual string GetInfoString() { return ""; }
  ...
  internal static void DrawPreview(IPreviewable defaultPreview,
                                   Rect previewArea, UnityObject[] targets) {
    string text = string.Empty;
    Event evt = Event.current;

    // If multiple targets, draw a grid of previews
    if (targets.Length > 1) {
      // Draw the previews inside the region of the background that's solid
      // colored
      Rect previewPositionInner =
          new RectOffset(16, 16, 20, 25).Remove(previewArea);

      // Number of previews to aim at
      int maxRows = Mathf.Max(
          1, Mathf.FloorToInt(
                 (previewPositionInner.height + kGridSpacing) /
                 (kPreviewMinSize + kGridSpacing + kPreviewLabelHeight)));
      int maxCols = Mathf.Max(
          1, Mathf.FloorToInt((previewPositionInner.width + kGridSpacing) /
                              (kPreviewMinSize + kGridSpacing)));
      ...
      if (neededCount < countWithMinimumSize) {
        division = GetGridDivision(previewPositionInner, neededCount,
                                   kPreviewLabelHeight);
        fixedSize = false;
      }

      // The available cells in the grid may be slightly higher than what was
      // aimed at. If the number of targets is also higher, we might as well
      // fill in the remaining cells.
      int count = Mathf.Min(division[0] * division[1], targets.Length);

      // Calculations become simpler if we add one spacing to the width and
      // height, so there is the same number of spaces and previews.
      previewPositionInner.width += kGridSpacing;
      previewPositionInner.height += kGridSpacing;

      Vector2 cellSize = new Vector2(
          Mathf.FloorToInt(previewPositionInner.width / division[0] -
                           kGridSpacing),
          Mathf.FloorToInt(previewPositionInner.height / division[1] -
                           kGridSpacing));
      float previewSize =
          Mathf.Min(cellSize.x, cellSize.y - kPreviewLabelHeight);
      if (fixedSize)
        previewSize = Mathf.Min(previewSize, kPreviewMinSize);

      bool selectingOne =
          (evt.type == EventType.MouseDown && evt.button == 0 &&
           evt.clickCount == 2 && previewArea.Contains(evt.mousePosition));

      defaultPreview.ResetTarget();
      for (int i = 0; i < count; i++) {
        Rect r = new Rect(
            previewPositionInner.x +
                (i % division[0]) * previewPositionInner.width / division[0],
            previewPositionInner.y +
                (i / division[0]) * previewPositionInner.height / division[1],
        ...
        if (selectingOne && r.Contains(Event.current.mousePosition)) {
          if (defaultPreview.target is AssetImporter)
            ...
            Selection.objects =
                new[]{AssetDatabase.LoadAssetAtPath<UnityObject>(
                    ((AssetImporter) defaultPreview.target).assetPath)};
          else
        ...
        // Make preview square
        Rect rSquare = new Rect(r.x + (r.width - previewSize) * 0.5f,
                                r.y + (r.height - previewSize) * 0.5f,
        ...
        defaultPreview.OnInteractivePreviewGUI(
            new Rect(0, 0, previewSize, previewSize),
            Styles.preBackgroundSolid);
      ...
      defaultPreview
          .ResetTarget(); // Remember to reset referenceTargetIndex to prevent
                          // following calls to 'editor.target' will return a
                          // different target which breaks all sorts of places.
                          // Fix for case 600235

      if (Event.current.type == EventType.Repaint)
        text = string.Format("Previewing {0} of {1} Objects", count,
                             targets.Length);
    }
    // If only a single target, just draw that one
      ...
      if (Event.current.type == EventType.Repaint) {
        // TODO: This should probably be calculated during import and stored
        // together with the asset somehow. Or maybe not. Not sure, really...
    ...
    if (Event.current.type == EventType.Repaint && text != string.Empty) {
      var textHeight = Styles.dropShadowLabelStyle.CalcHeight(
          GUIContent.Temp(text), previewArea.width);
      EditorGUI.DropShadowLabel(
          new Rect(previewArea.x,
                   previewArea.yMax - textHeight - kPreviewLabelPadding,
                   previewArea.width, textHeight),
          text);
    }
  }

  // Get the number or columns and rows for a grid with a certain minimum number
  // of cells such that the cells are as close to square as possible.
  private static int[] GetGridDivision(Rect rect, int minimumNr,
                                       int labelHeight) {
    ...
    while (xCount * yCount < minimumNr) {
      float ratioIfXInc =
          AbsRatioDiff((xCount + 1) / rect.width,
                       yCount / (rect.height - yCount * labelHeight));
      float ratioIfYInc = AbsRatioDiff(
          xCount / rect.width,
          (yCount + 1) / (rect.height - (yCount + 1) * labelHeight));
...
public class CustomEditorForRenderPipelineAttribute : CustomEditor {
  ...
  public CustomEditorForRenderPipelineAttribute(Type inspectedType,
                                                Type renderPipeline)
      : base(inspectedType) {
...
internal sealed class CachePropertyAttribute : System.Attribute {
  ...
  public CachePropertyAttribute() { propertyPath = null; }
...
public partial class Editor : ScriptableObject,
  ...
  // Last time this struct could be found at:
  // Editor\src\Utility\CreateEditor.cpp
  ...
  // Note that m_Dirty is not only set through 'isInspectorDirty' but also from
  // C++ in 'SetCustomEditorIsDirty (MonoBehaviour* inspector, bool dirty)'
  int m_IsDirty;
  ...
  internal bool firstInspectedEditor {
    get;
    set;
  }
  ...
  internal virtual bool HasLargeHeader() {
    return AssetDatabase.IsMainAsset(target) ||
  ...
  internal bool canEditMultipleObjects {
    get {
      return GetType()
                 .GetCustomAttributes(typeof(CanEditMultipleObjects), false)
                 .Length > 0;
    }
  }
  ...
  static class BaseStyles {
    public static readonly GUIContent open =
        EditorGUIUtility.TrTextContent("Open");
    public static readonly GUIStyle inspectorBig =
        new GUIStyle(EditorStyles.inspectorBig);
  ...
  public UnityObject target {
    get { return m_Targets[referenceTargetIndex]; }
    set {
  ...
  internal virtual int referenceTargetIndex {
    ...
    set {
      m_ReferenceTargetIndex =
          (Math.Abs(value * m_Targets.Length) + value) % m_Targets.Length;
    }
  }
    ...
    get {
      if (m_Targets.Length == 1 || !m_AllowMultiObjectAccess)
        ...
        return m_Targets.Length + " " +
               ObjectNames.NicifyVariableName(ObjectNames.GetTypeName(target)) +
               "s";
    }
  }
  ...
  CreateEditorWithContext(UnityObject[] targetObjects, UnityObject context,
                          [ DefaultValue("null") ] Type editorType) {
  ...
  [ExcludeFromDocs]
  public static Editor CreateEditorWithContext(UnityObject[] targetObjects,
                                               UnityObject context) {
    Type editorType = null;
    return CreateEditorWithContext(targetObjects, context, editorType);
  }

  public static void CreateCachedEditorWithContext(UnityObject targetObject,
                                                   UnityObject context,
                                                   Type editorType,
                                                   ref Editor previousEditor) {
    CreateCachedEditorWithContext(new[]{targetObject}, context, editorType,
                                  ref previousEditor);
  }

  public static void CreateCachedEditorWithContext(UnityObject[] targetObjects,
                                                   UnityObject context,
                                                   Type editorType,
                                                   ref Editor previousEditor) {
    if (previousEditor != null &&
        ArrayUtility.ArrayEquals(previousEditor.m_Targets, targetObjects) &&
        previousEditor.m_Context == context)
      return;

    if (previousEditor != null)
      DestroyImmediate(previousEditor);
    previousEditor =
        CreateEditorWithContext(targetObjects, context, editorType);
  }

  public static void CreateCachedEditor(UnityObject targetObject,
                                        Type editorType,
                                        ref Editor previousEditor) {
    CreateCachedEditorWithContext(new[]{targetObject}, null, editorType,
                                  ref previousEditor);
  }

  public static void CreateCachedEditor(UnityObject[] targetObjects,
                                        Type editorType,
                                        ref Editor previousEditor) {
    CreateCachedEditorWithContext(targetObjects, null, editorType,
                                  ref previousEditor);
  }
  ...
  private void OnDisableINTERNAL() { CleanupPropertyEditor(); }
  ...
  AssignCachedProperties<T>(T self, SerializedProperty root) where T : class {
    ...
    foreach (var fieldInfo in fields) {
      var attribute =
          (CachePropertyAttribute) fieldInfo
              .GetCustomAttributes(typeof(CachePropertyAttribute), false)
              .First();
      var propertyName = string.IsNullOrEmpty(attribute.propertyPath)
                             ? fieldInfo.Name
    ...
    if (exitCount > 0) {
      Debug.LogWarning(
          "The following properties registered with CacheProperty where not found during the inspector creation: " +
          string.Join(", ", properties.Keys.ToArray()));
    }
  }
  ...
  internal virtual Bounds GetWorldBoundsOfTarget(UnityObject targetObject) {
    return targetObject is Component ?((Component) targetObject)
        .gameObject.CalculateBounds()
        : new Bounds();
  }
  ...
  internal virtual void OnForceReloadInspector() {
    if (m_SerializedObject != null) {
      ...
      // Need to make sure internal target list PPtr have been updated from a
      // native memory When assets are reloaded they are destroyed and recreated
      // and the managed list does not get updated The m_SerializedObject is a
      // native object thus its targetObjects is a native memory PPtr list which
      // have the new PPtr ids.
      InternalSetTargets(m_SerializedObject.targetObjects);
    }
  }

  internal virtual bool GetOptimizedGUIBlock(bool isDirty, bool isVisible,
                                             out float height) {
    height = -1;
    return false;
  }
  ...
  DrawPropertiesExcluding(SerializedObject obj,
                          params string[] propertyToExclude) {
    // Loop through properties and create one field (including children) for
  ...
  public bool DrawDefaultInspector() { return DoDrawDefaultInspector(); }
  ...
  internal bool DoDrawDefaultInspector() {
    ...
    using(new LocalizationGroup(target)) {
      ...
      if (behaviour == null || !AudioUtil.HasAudioCallback(behaviour) ||
          AudioUtil.GetCustomFilterChannelCount(behaviour) <= 0)
        return res;
  ...
  // Implement this function if you want your editor constantly repaint (every
  // 33ms)
  public virtual bool RequiresConstantRepaint() { return false; }

  public static event Action<Editor> finishedDefaultHeaderGUI = null;

  // This is the method that should be called from externally e.g.
  // myEditor.DrawHeader (); Do not make this method virtual - override
  // OnHeaderGUI instead.
  public void DrawHeader() {
    // If we call DrawHeader from inside an an editor's OnInspectorGUI call, we
    // have to do special handling. (See DrawHeaderFromInsideHierarchy for
    // details.) We know we're inside the OnInspectorGUI block (or a similar
    // vertical block) if hierarchyMode is set to true.
    ...
    if (finishedDefaultHeaderGUI != null) {
      ...
      GUILayout.Space(-1f // move up to cover up bottom pixel of header box
                      - BaseStyles.inspectorBig.margin.bottom -
                      BaseStyles.inspectorBig.padding.bottom -
                      BaseStyles.inspectorBig.overflow
                          .bottom // move up to bottom of content area in header
      );

      // align with controls in the Inspector
      // see InspectorWindow.DrawEditor() before calls to
      // OnOptimizedInspectorGUI()/OnInspectorGUI()
      EditorGUIUtility.hierarchyMode = true;
      EditorGUIUtility.wideMode =
          EditorGUIUtility.contextWidth > k_WideModeMinWidth;
      EditorGUILayout.BeginVertical(BaseStyles.postLargeHeaderBackground,
      ...
      if (hierarchyMode) {
        ...
        EditorGUILayout.BeginVertical(UseDefaultMargins()
  ...
  protected virtual void OnHeaderGUI() { DrawHeaderGUI(this, targetTitle); }
  ...
  internal virtual void OnHeaderControlsGUI() {
    ...
    // only show open button for the main object of an asset and for
    // AssetImportInProgressProxy (asset not yet imported)
    ...
    if (showOpenButton && !ShouldHideOpenButton()) {
      var assets =
          importerEditor != null ? importerEditor.assetTargets : targets;
      var disabled =
          importerEditor != null && importerEditor.assetTarget == null;
      ShowOpenButton(assets, !disabled);
    }
  }

  internal void ShowOpenButton(UnityObject[] assets,
  ...
  internal virtual void OnHeaderIconGUI(Rect iconRect) {
    ...
    //  Fetch isLoadingAssetPreview to ensure that there is no situation where a
    //  preview needs a repaint because it hasn't finished loading yet.
  ...
  internal virtual Rect DrawHeaderHelpAndSettingsGUI(Rect r) {
    // Help
    var settingsSize = EditorStyles.iconButton.CalcSize(
        EditorGUI.GUIContents.titleSettingsIcon);

    float currentOffset = settingsSize.x;

    const int kTopMargin = 5;
    // Settings; process event even for disabled UI
    Rect settingsRect = new Rect(r.xMax - currentOffset, r.y + kTopMargin,
                                 settingsSize.x, settingsSize.y);
    var wasEnabled = GUI.enabled;
    GUI.enabled = true;
    var showMenu = EditorGUI.DropdownButton(settingsRect, GUIContent.none,
                                            FocusType.Passive,
    ...
    return EditorGUIUtility.DrawEditorHeaderItems(
        new Rect(r.xMax - currentOffset, r.y + kTopMargin, settingsSize.x,
                 settingsSize.y),
        targets);
  }

  // If we call DrawHeaderGUI from inside an an editor's OnInspectorGUI call, we
  // have to do special handling. Since OnInspectorGUI is wrapped inside a
  // BeginVertical/EndVertical block that adds padding, and we don't want this
  // padding for headers, we have to stop the vertical block, draw the header,
  // and then start a new vertical block with the same style.
  ...
  internal static Rect DrawHeaderGUI(Editor editor, string header,
    ...
    // Content rect
    Rect r = new Rect(fullRect.x + leftMargin, fullRect.y,
                      fullRect.width - leftMargin, fullRect.height);
    ...
    if (editor)
      ...
      GUI.Label(iconRect,
                AssetPreview.GetMiniTypeThumbnail(typeof(UnityObject)),
                BaseStyles.centerStyle);
    ...
    if (editor) {
      ...
      titleRect = new Rect(rectX, r.y + 6, (helpAndSettingsRect.x - rectX) - 4,
                           titleHeight);
    } else
      titleRect = new Rect(r.x + kImageSectionWidth, r.y + 6,
                           r.width - kImageSectionWidth, titleHeight);
    ...
    GUI.enabled = true;
    Event evt = Event.current;
    var showMenu = editor != null && evt.type == EventType.MouseDown &&
                   evt.button == 1 && r.Contains(evt.mousePosition);
    GUI.enabled = wasEnabled;
  ...
  internal void DrawPostIconContent(Rect iconRect) {
    if (OnPostIconGUI != null) {
      ...
      drawRect.x = (drawRect.xMax - k_Size) +
                   4; // Move slightly outside bounds for overlap effect.
  ...
  public static void DrawFoldoutInspector(UnityObject target,
                                          ref Editor editor) {
    ...
    Rect titleRect =
        Editor.DrawHeaderGUI(editor, editor.targetTitle, kSpaceForFoldoutArrow);
    ...
    bool oldVisible =
        UnityEditorInternal.InternalEditorUtility.GetIsInspectorExpanded(
            target);
    bool newVisible = EditorGUI.DoObjectFoldout(oldVisible, titleRect,
                                                renderRect, editor.targets, id);
  ...
  public virtual bool HasPreviewGUI() { return preview.HasPreviewGUI(); }
  ...
  public virtual Texture2D RenderStaticPreview(string assetPath,
                                               UnityObject[] subAssets,
                                               int width, int height) {
    return null;
  }

  // Implement to create your own custom preview. Custom previews are used in
  // the preview area of the inspector, primary editor headers, and the object
  // selector.
  public virtual void OnPreviewGUI(Rect r, GUIStyle background) {
  ...
  // Override this method if you want to show custom controls in the preview
  // header.
  public virtual void OnPreviewSettings() { preview.OnPreviewSettings(); }

  // Implement this method to show asset information on top of the asset
  // preview.
  public virtual string GetInfoString() { return preview.GetInfoString(); }
  ...
  // Some custom editors manually display SerializedObjects with only private
  // properties Setting this to true allows them to properly toggling the
  // visibility via the standard header foldout
  internal bool alwaysAllowExpansion {
    get;
    set;
  }

  // Auxiliary method that determines whether this editor has a set of public
  // properties and, as thus, can be expanded via a foldout. This is used in
  // order to determine whether a foldout needs to be rendered on top of the
  // inspector title bar or not. Some examples of editors that don't require a
  // foldout are GUI Layer and Audio Listener.
  ...
  static internal bool IsAppropriateFileOpenForEdit(UnityObject assetObject) {
    // The native object for a ScriptableObject with an invalid script will be
    // considered not alive. In order to allow editing of the m_Script property
    // of a ScriptableObject with an invalid script we use the instance ID
    // instead of the UnityEngine.Object reference to check if the asset is open
    // for edit.
  ...
  internal virtual bool IsEnabled() {
    ...
    foreach (UnityObject target in targets) {
      ...
      if (EditorUtility.IsPersistent(target) &&
          !IsAppropriateFileOpenForEdit(target))
        return false;
    }
  ...
  public virtual bool UseDefaultMargins() { return true; }
  ...
  public void ResetTarget() { referenceTargetIndex = 0; }

  // Implement this method to show a limited inspector for showing tweakable
  // parameters in an Asset Store preview.
