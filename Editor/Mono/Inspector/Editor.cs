...
public partial class Editor : ScriptableObject, IPreviewable, IToolModeOwner
{
    ...
    {
        get {
            return Mathf.Clamp(m_ReferenceTargetIndex, 0, m_Targets.Length - 1);
        }
        // Modulus that works for negative numbers as well
        set {
            m_ReferenceTargetIndex = (Math.Abs(value * m_Targets.Length) + value) % m_Targets.Length;
        }
    }
