...
internal class VersionControlSettingsProvider : ProjectSettingsBaseEditor {
  class Styles {
    ...
    public static GUIContent automaticAdd = new GUIContent(
        "Automatic Add",
    ...
    public static GUIContent workOffline = new GUIContent(
        "Work Offline",
    ...
    public static GUIContent allowAsyncUpdate = new GUIContent(
        "Async Status",
        "Enable asynchronous file status queries (useful with slow server connections).");
    public static GUIContent showFailedCheckouts =
        new GUIContent("Show Failed Checkouts",
                       "Show dialogs for failed 'Check Out' operations.");
    public static GUIContent overwriteFailedCheckoutAssets = new GUIContent(
        "Overwrite Failed Checkout Assets",
        "When on, assets that can not be checked out will get saved anyway.");
    public static GUIContent overlayIcons = new GUIContent(
        "Overlay Icons", "Should version control status icons be shown.");
    public static GUIContent projectOverlayIcons = new GUIContent(
        "Project Window",
        "Should version control status icons be shown in the Project window.");
    public static GUIContent hierarchyOverlayIcons = new GUIContent(
        "Hierarchy Window",
        "Should version control status icons be shown in the Hierarchy window.");
    public static GUIContent otherOverlayIcons = new GUIContent(
        "Other Windows",
  ...
  Dictionary<string, string[]> m_VCConfigFieldsRecentValues =
  ...
  public struct ExternalVersionControl {
    ...
    public ExternalVersionControl(string value) { m_Value = value; }
  ...
  private string[] logLevelPopupList = {"Verbose", "Info", "Notice", "Fatal"};

  private string[] semanticMergePopupList = {"Off", "Premerge", "Ask"};
  ...
  void DrawOverlayDescription(Asset.States state) {
    ...
    GUILayout.Label("    " + Asset.StateToString(state),
  ...
  private void CreatePopupMenuVersionControl(
      string title, EditorSettingsInspector.PopupElement[] elements,
      string selectedValue, GenericMenu.MenuFunction2 func) {
    var selectedIndex = System.Array.FindIndex(
        elements, (EditorSettingsInspector.PopupElement typeElem) =>
                      (typeElem.id == selectedValue));
    var content = new GUIContent(elements[selectedIndex].content);
    EditorSettingsInspector.CreatePopupMenu(null, title, content, elements,
                                            selectedIndex, func);
  }
  ...
  private bool VersionControlSystemHasGUI() {
    ...
    if (!collabEnabled) {
      ...
      return system != ExternalVersionControl.Disabled &&
  ...
  public void OnEnable() {
    ...
    List<EditorSettingsInspector.PopupElement> popupArray =
        new List<EditorSettingsInspector.PopupElement>(vcDefaultPopupList);
  ...
  public override void OnInspectorGUI() {
    ...
    using(new EditorGUI.DisabledScope(!collabEnabled)) {
      ...
      CreatePopupMenuVersionControl(Styles.mode.text, vcPopupList, selvc,
    ...
    if (collabEnabled) {
      EditorGUILayout.HelpBox(
          "Version Control not available when using Collaboration feature.",
          MessageType.Warning);
    }
    ...
    if (VersionControlSystemHasGUI()) {
      ...
      if (VersionControlSettings.mode == ExternalVersionControl.Generic ||
        ...
        foreach (ConfigField field in configFields) {
          ...
          if (field.isPassword) {
            newVal = EditorGUILayout.PasswordField(
                GUIContent.Temp(field.label, field.description), oldVal);
            if (newVal != oldVal)
              EditorUserSettings.SetPrivateConfigValue(field.name, newVal);
          } else {
            var recentValues = GetVCConfigFieldRecentValues(field.name);
            newVal = EditorGUILayout.TextFieldDropDown(
                GUIContent.Temp(field.label, field.description), oldVal,
                recentValues);
            if (newVal != oldVal)
      ...
      int idx = System.Array.FindIndex(logLevelPopupList,
                                       (item) => item.ToLower() == logLevel);
      if (idx == -1) {
        logLevel = "notice";
        idx = System.Array.FindIndex(logLevelPopupList,
                                     (item) => item.ToLower() == logLevel);
        if (idx == -1) {
          idx = 0;
        }
      ...
      int newIdx =
          EditorGUILayout.Popup(Styles.logLevel, idx, logLevelPopupList);
      if (newIdx != idx) {
        EditorUserSettings.SetConfigValue("vcSharedLogLevel", logLevelPopupList
                                                              [newIdx]
                                                                  .ToLower());
      }
      ...
      GUI.enabled =
          hasRequiredFields && Provider.onlineState != OnlineState.Updating;
      if (GUILayout.Button(Provider.onlineState !=
                               OnlineState.Offline ? Styles.vcsReconnect
                           : Styles.vcsConnect, EditorStyles.miniButton)) {
      ...
      if (m_NeedToSaveValuesOnConnect &&
          Provider.onlineState == OnlineState.Online) {
        // save connection field settings if we got online with them
        // successfully
      ...
      if (Provider.requiresNetwork) {
        bool workOfflineNew = EditorGUILayout.Toggle(
            Styles.workOffline,
            EditorUserSettings
                .WorkOffline); // Enabled has a slightly different behaviour
        if (workOfflineNew != EditorUserSettings.WorkOffline) {
          // On toggling on show a warning
          if (workOfflineNew &&
              !EditorUtility.DisplayDialog(
                  "Confirm working offline",
      ...
      EditorUserSettings.AutomaticAdd = EditorGUILayout.Toggle(
          Styles.automaticAdd, EditorUserSettings.AutomaticAdd);

      if (Provider.requiresNetwork)
        EditorUserSettings.allowAsyncStatusUpdate = EditorGUILayout.Toggle(
            Styles.allowAsyncUpdate, EditorUserSettings.allowAsyncStatusUpdate);

      if (Provider.hasCheckoutSupport) {
        EditorUserSettings.showFailedCheckout = EditorGUILayout.Toggle(
            Styles.showFailedCheckouts, EditorUserSettings.showFailedCheckout);
        EditorUserSettings.overwriteFailedCheckoutAssets =
            EditorGUILayout.Toggle(
                Styles.overwriteFailedCheckoutAssets,
                EditorUserSettings.overwriteFailedCheckoutAssets);
      }

      EditorUserSettings.semanticMergeMode =
          (SemanticMergeMode) EditorGUILayout.Popup(
              Styles.smartMerge, (int) EditorUserSettings.semanticMergeMode,
              semanticMergePopupList);

      GUILayout.Space(10);
      GUILayout.Label(Styles.overlayIcons);

      EditorGUI.indentLevel++;
      var newProjectOverlayIcons = EditorGUILayout.Toggle(
          Styles.projectOverlayIcons, EditorUserSettings.overlayIcons);
      if (newProjectOverlayIcons != EditorUserSettings.overlayIcons) {
        EditorUserSettings.overlayIcons = newProjectOverlayIcons;
        EditorApplication.RequestRepaintAllViews();
      }

      var newHierarchyOverlayIcons =
          EditorGUILayout.Toggle(Styles.hierarchyOverlayIcons,
                                 EditorUserSettings.hierarchyOverlayIcons);
      if (newHierarchyOverlayIcons !=
          EditorUserSettings.hierarchyOverlayIcons) {
        EditorUserSettings.hierarchyOverlayIcons = newHierarchyOverlayIcons;
        EditorApplication.RequestRepaintAllViews();
      }

      var newOtherOverlayIcons = EditorGUILayout.Toggle(
          Styles.otherOverlayIcons, EditorUserSettings.otherOverlayIcons);
      ...
      GUI.enabled = true;
      if (newProjectOverlayIcons || newHierarchyOverlayIcons ||
          newOtherOverlayIcons)
