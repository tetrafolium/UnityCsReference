...
internal class LineRendererCurveEditor {
  private class Styles {
    public static GUIContent widthMultiplier = EditorGUIUtility.TrTextContent(
        "Width",
  ...
  public void OnEnable(SerializedObject serializedObject) {
    m_WidthMultiplier =
        serializedObject.FindProperty("m_Parameters.widthMultiplier");
  ...
  private void UndoRedoPerformed() { m_Refresh = true; }
  ...
  public void OnInspectorGUI() {
    ...
    // Draw Curve Editor
    if (m_Refresh) {
      m_Editor.animationCurves =
          new CurveWrapper[]{GetCurveWrapper(m_WidthCurve.animationCurveValue)};
      m_Refresh = false;
    }
    ...
    var labelRect =
        new Rect(r.x + 4, r.y, r.width, EditorGUI.kSingleLineHeight);
    EditorGUI.LabelField(labelRect, Styles.widthMultiplier);

    // Apply curve changes
    if ((m_Editor.GetCurveWrapperFromID(0) != null) &&
        (m_Editor.GetCurveWrapperFromID(0).changed)) {
