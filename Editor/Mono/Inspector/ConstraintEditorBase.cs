...
internal abstract class ConstraintStyleBase {
  GUIContent m_Activate = EditorGUIUtility.TrTextContent(
      "Activate",
      "Activate the constraint at the current offset from the sources.");
  GUIContent m_Zero = EditorGUIUtility.TrTextContent(
      "Zero", "Activate the constraint at zero offset from the sources.");

  GUIContent m_Sources = EditorGUIUtility.TrTextContent("Sources");

  GUIContent m_Weight = EditorGUIUtility.TrTextContent("Weight");

  GUIContent m_IsActive = EditorGUIUtility.TrTextContent(
      "Is Active", "When set, the constraint is being evaluated.");
  GUIContent m_IsLocked = EditorGUIUtility.TrTextContent(
      "Lock",
      "When set, evaluate with the current offset. When not set, update the offset based on the current transform.");

  GUIContent[] m_Axes = {EditorGUIUtility.TextContent("X"),
                         EditorGUIUtility.TextContent("Y"),
                         EditorGUIUtility.TextContent("Z")};

  GUIContent m_ConstraintSettings =
      EditorGUIUtility.TrTextContent("Constraint Settings");
  ...
  public virtual GUIContent ConstraintSettings {
    get { return m_ConstraintSettings; }
  }
}
...
internal abstract class ConstraintEditorBase : Editor {
  ...
  protected int selectedSourceIndex {
    get { return m_SelectedSourceIdx; }
    set { m_SelectedSourceIdx = value; }
  }
  ...
  public void OnEnable(ConstraintStyleBase style) {
    ...
    m_SourceList =
        new ReorderableList(serializedObject, sources, sources.editable, true,
                            sources.editable, sources.editable);
    m_SourceList.drawElementCallback += DrawElementCallback;
    m_SourceList.onAddCallback += OnAddCallback;
    m_SourceList.drawHeaderCallback += rect =>
  ...
  public void OnDisable() { Undo.undoRedoPerformed -= OnUndoRedoPerformed; }

  internal void OnUndoRedoPerformed() {
    // must call UserUpdateOffset to allow the offsets to be updated by the Undo
    // system, otherwise the constraint can override them
  ...
  private void DrawElementCallback(Rect rect, int index, bool isActive,
                                   bool isFocused) {
    ...
    EditorGUI.PropertyField(new Rect(rect.x, rect.y,
                                     rect.width - kSourceWeightWidth,
                                     EditorGUIUtility.singleLineHeight),
                            source, GUIContent.none);
    EditorGUI.PropertyField(new Rect(rect.x + rect.width - kSourceWeightWidth,
                                     rect.y, kSourceWeightWidth,
                                     EditorGUIUtility.singleLineHeight),
  ...
  internal void ShowConstraintEditor<T>(ConstraintStyleBase style) where T
      : class
  , IConstraintInternal {
    if (m_SelectedSourceIdx == -1 ||
        m_SelectedSourceIdx >= m_SourceList.serializedProperty.arraySize) {
      SelectSource(0);
    }
    ...
    using(new EditorGUI.DisabledScope(Application.isPlaying)) {
      using(new EditorGUILayout.HorizontalScope()) {
        if (GUILayout.Button(style.Activate)) {
          ...
          Undo.RegisterCompleteObjectUndo(recordObjects.ToArray(),
    ...
    m_ShowConstraintSettings = EditorGUILayout.Foldout(
        m_ShowConstraintSettings, style.ConstraintSettings, true);
  ...
  internal virtual void ShowOffset<T>(ConstraintStyleBase style) where T : class
  , IConstraintInternal {
    EditorGUI.BeginChangeCheck();
