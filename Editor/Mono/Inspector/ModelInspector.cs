...
internal class ModelInspector : Editor {
  internal static class Styles {
    public static readonly GUIContent wireframeToggle =
        EditorGUIUtility.TrTextContent("Wireframe", "Show wireframe");
    public static GUIContent displayModeDropdown =
        EditorGUIUtility.TrTextContent("", "Change display mode");
    public static GUIContent uvChannelDropdown =
        EditorGUIUtility.TrTextContent("", "Change active UV channel");
  ...
  private static string[] m_DisplayModes = {
      "Shaded",  "UV Checker", "UV Layout",  "Vertex Color",
      "Normals", "Tangents",   "Blendshapes"};

  private static string[] m_UVChannels = {"Channel 0", "Channel 1", "Channel 2",
  ...
  static Material CreateMeshMultiPreviewMaterial() {
    var shader = EditorGUIUtility.LoadRequired(
        "Previews/MeshPreviewShader.shader") as Shader;
    if (!shader) {
  ...
  void Init() {
    ...
    m_CheckeredTexture = EditorGUIUtility.LoadRequired(
  ...
  void ResetView() {
    ...
    m_Settings.meshMultiPreviewMaterial.SetInt("_UVChannel",
                                               m_Settings.activeUVChannel);
  ...
  private void DoPopup(Rect popupRect, string[] elements, int selectedIndex,
                       GenericMenu.MenuFunction2 func, bool[] disabledItems) {
    GenericMenu menu = new GenericMenu();
    for (int i = 0; i < elements.Length; i++) {
      var element = elements[i];
      if (element == m_DisplayModes[(int) DisplayMode.Blendshapes] &&
          Selection.count > 1)
        continue;
  ...
  private void SetUVChannel(object data) {
    ...
    if (m_Settings.displayMode == DisplayMode.UVLayout ||
        m_Settings.displayMode == DisplayMode.UVChecker)
  ...
  private void SetDisplayMode(object data) {
    ...
    case DisplayMode.UVChecker:
      ...
      m_Settings.meshMultiPreviewMaterial.SetTexture("_MainTex",
                                                     m_CheckeredTexture);
      m_Settings.meshMultiPreviewMaterial.mainTextureScale =
          new Vector2(m_Settings.checkerTextureMultiplier,
                      m_Settings.checkerTextureMultiplier);
      break;
  ...
  internal static void RenderMeshPreview(Mesh mesh,
    ...
    Transform renderCamTransform =
        previewUtility.camera.GetComponent<Transform>();
    ...
    Vector3 camPosition =
        camRotation * Vector3.forward * (-distance * settings.zoomFactor) +
        settings.pivotPositionOffset;
    ...
    previewUtility.lights[0].transform.rotation =
        Quaternion.Euler(-settings.lightDir.y, -settings.lightDir.x, 0);
    ...
    RenderMeshPreviewSkipCameraAndLighting(mesh, bounds, previewUtility,
                                           settings, null, meshSubset);
  }
  ...
  internal static void RenderMeshPreviewSkipCameraAndLighting(
      Mesh mesh, Bounds bounds, PreviewRenderUtility previewUtility,
      PreviewSettings settings, MaterialPropertyBlock customProperties,
      int meshSubset) // -1 for whole mesh
  {
    if (mesh == null || previewUtility == null)
      return;

    Quaternion rot = Quaternion.Euler(settings.previewDir.y, 0, 0) *
                     Quaternion.Euler(0, settings.previewDir.x, 0);
    ...
    var colorPropID = 0;
    if (submeshes > 1 && settings.displayMode == DisplayMode.Shaded &&
        customProperties == null & meshSubset == -1) {
      tintSubmeshes = true;
    ...
    if (settings.activeMaterial != null) {
      ...
      if (meshSubset < 0 || meshSubset >= submeshes) {
        for (int i = 0; i < submeshes; ++i) {
          ...
          previewUtility.DrawMesh(mesh, pos, rot, settings.activeMaterial, i,
                                  customProperties);
        }
      } else
        previewUtility.DrawMesh(mesh, pos, rot, settings.activeMaterial,
    ...
    if (settings.wireMaterial != null && settings.drawWire) {
      ...
      if (meshSubset < 0 || meshSubset >= submeshes) {
        for (int i = 0; i < submeshes; ++i) {
          // lines/points already are wire-like; it does not make sense to
          // overdraw them again with dark wireframe color
          var topology = mesh.GetTopology(i);
          if (topology == MeshTopology.Lines ||
              topology == MeshTopology.LineStrip ||
              topology == MeshTopology.Points)
            continue;
          previewUtility.DrawMesh(mesh, pos, rot, settings.wireMaterial, i,
                                  customProperties);
        }
      } else
        previewUtility.DrawMesh(mesh, pos, rot, settings.wireMaterial,
                                meshSubset, customProperties);
      previewUtility.Render();
  ...
  void BakeSkinnedMesh() {
    ...
    var baseGameObjectForSkinnedMeshRenderer =
        new GameObject{hideFlags = HideFlags.HideAndDontSave};
    SkinnedMeshRenderer skinnedMeshRenderer =
        baseGameObjectForSkinnedMeshRenderer
  ...
  public override Texture2D RenderStaticPreview(string assetPath,
    ...
    if (!ShaderUtil.hardwareSupportsRectRenderTexture) {
      // Debug.Log("Could not generate static preview. Render texture not
  ...
  void DrawMeshPreviewToolbar() {
    if (m_Settings.displayMode == DisplayMode.UVChecker) {
      ...
      float sliderWidth =
          EditorStyles.label.CalcSize(new GUIContent("--------")).x;
      Rect sliderRect =
          EditorGUILayout.GetControlRect(GUILayout.Width(sliderWidth));
      sliderRect.x += 3;

      m_Settings.checkerTextureMultiplier = (int) GUI.HorizontalSlider(
          sliderRect, m_Settings.checkerTextureMultiplier, 30, 1,
          Styles.preSlider, Styles.preSliderThumb);
      if (oldVal != m_Settings.checkerTextureMultiplier)
        m_Settings.activeMaterial.mainTextureScale =
            new Vector2(m_Settings.checkerTextureMultiplier,
                        m_Settings.checkerTextureMultiplier);
    }
    ...
    if (m_Settings.displayMode == DisplayMode.UVLayout ||
        m_Settings.displayMode == DisplayMode.UVChecker) {
      float channelDropDownWidth =
          EditorStyles.toolbarDropDown.CalcSize(new GUIContent("Channel 6")).x;
      Rect channelDropdownRect =
          EditorGUILayout.GetControlRect(GUILayout.Width(channelDropDownWidth));
      channelDropdownRect.y -= 1;
      channelDropdownRect.x += 5;
      GUIContent channel =
          new GUIContent("Channel " + m_Settings.activeUVChannel,
                         Styles.uvChannelDropdown.tooltip);

      if (EditorGUI.DropdownButton(channelDropdownRect, channel,
                                   FocusType.Passive,
                                   EditorStyles.toolbarDropDown))
        DoPopup(channelDropdownRect, m_UVChannels, m_Settings.activeUVChannel,
                SetUVChannel, m_Settings.availableUVChannels);
    }

    if (m_Settings.displayMode == DisplayMode.Blendshapes) {
      float blendshapesDropDownWidth =
          EditorStyles.toolbarDropDown.CalcSize(new GUIContent("Blendshapes"))
              .x;
      Rect blendshapesDropdownRect = EditorGUILayout.GetControlRect(
          GUILayout.Width(blendshapesDropDownWidth));
      blendshapesDropdownRect.y -= 1;
      blendshapesDropdownRect.x += 5;
      GUIContent blendshape =
          new GUIContent(m_BlendShapes[m_Settings.activeBlendshape],
                         Styles.uvChannelDropdown.tooltip);

      if (EditorGUI.DropdownButton(blendshapesDropdownRect, blendshape,
                                   FocusType.Passive,
                                   EditorStyles.toolbarDropDown))
    ...
    float displayModeDropDownWidth =
        EditorStyles.toolbarDropDown
            .CalcSize(
                new GUIContent(m_DisplayModes[(int) DisplayMode.VertexColor]))
            .x;
    Rect displayModeDropdownRect = EditorGUILayout.GetControlRect(
        GUILayout.Width(displayModeDropDownWidth));
    displayModeDropdownRect.y -= 1;
    displayModeDropdownRect.x += 2;
    GUIContent displayModeDropdownContent =
        new GUIContent(m_DisplayModes[(int) m_Settings.displayMode],
                       Styles.displayModeDropdown.tooltip);

    if (EditorGUI.DropdownButton(displayModeDropdownRect,
                                 displayModeDropdownContent, FocusType.Passive,
                                 EditorStyles.toolbarDropDown))
      DoPopup(displayModeDropdownRect, m_DisplayModes,
              (int) m_Settings.displayMode, SetDisplayMode,
              m_Settings.availableDisplayModes);

    using(new EditorGUI.DisabledScope(m_Settings.displayMode ==
                                      DisplayMode.UVLayout)) {
      m_Settings.drawWire =
          GUILayout.Toggle(m_Settings.drawWire, Styles.wireframeToggle,
  ...
  public override void OnPreviewGUI(Rect r, GUIStyle background) {
    ...
    if (evt.type == EventType.MouseDrag &&
        (m_Settings.displayMode == DisplayMode.UVLayout || evt.button == 2))
  ...
  void MeshPreviewZoom(Rect rect, Event evt) {
    ...
    var mouseViewPos = new Vector2(evt.mousePosition.x / rect.width,
                                   1 - evt.mousePosition.y / rect.height);
    var mouseWorldPos =
        m_PreviewUtility.camera.ViewportToWorldPoint(mouseViewPos);
    var mouseToCamPos = m_Settings.orthoPosition - mouseWorldPos;
    var newCamPos =
        mouseWorldPos + mouseToCamPos * (newZoom / m_Settings.zoomFactor);

    if (m_Settings.displayMode != DisplayMode.UVLayout) {
      m_PreviewUtility.camera.transform.position =
          new Vector3(newCamPos.x, newCamPos.y, newCamPos.z);
    } else {
  ...
  void MeshPreviewPan(Rect rect, Event evt) {
    ...
    var delta = new Vector3(-evt.delta.x * cam.pixelWidth / rect.width,
                            evt.delta.y * cam.pixelHeight / rect.height, 0);
    ...
    if (m_Settings.displayMode == DisplayMode.UVLayout) {
      ...
      screenPos += delta;
      worldPos =
          cam.ScreenToWorldPoint(screenPos) - m_Settings.pivotPositionOffset;
  ...
  static void DrawColorRect(Rect rect, Color color) {
    ...
    EditorGUI.DrawRect(
        new Rect(rect.x + rect.width - 1, rect.y, 1, rect.height), dimmed);
    EditorGUI.DrawRect(new Rect(rect.x, rect.y, rect.width, 1), dimmed);
    EditorGUI.DrawRect(
        new Rect(rect.x, rect.y + rect.height - 1, rect.width, 1), dimmed);
  }
  ...
  public override void OnInspectorGUI() {
    ...
    if (targets.Length > 1) {
      ...
      EditorGUILayout.LabelField(
          $"{targets.Length} meshes selected, {totalVertices} total vertices, {totalIndices} total indices");
      return;
    }
  ...
  static void ShowOtherInfo(Mesh mesh) {
    ...
    EditorGUILayout.LabelField("Bounds Center",
                               mesh.bounds.center.ToString("g4"));
  ...
  static void ShowBlendShapeInfo(Mesh mesh) {
    ...
    EditorGUILayout.Space();
    EditorGUILayout.LabelField($"Blend Shapes: {blendShapeCount}",
                               EditorStyles.boldLabel);

    var showScroll = blendShapeCount > 10;
    if (showScroll)
      m_ScrollPos = EditorGUILayout.BeginScrollView(
          m_ScrollPos,
          GUILayout.Height(10 * EditorGUIUtility.singleLineHeight));
    EditorGUI.indentLevel++;
    for (int i = 0; i < blendShapeCount; ++i) {
      EditorGUILayout.LabelField(
          $"#{i}: {mesh.GetBlendShapeName(i)} ({mesh.GetBlendShapeFrameCount(i)} frames)");
    }
    EditorGUI.indentLevel--;
    if (showScroll)
  ...
  static void ShowSkinInfo(Mesh mesh, VertexAttributeDescriptor[] attributes) {
    ...
    EditorGUILayout.Space();
    EditorGUILayout.LabelField($"Skin: {boneCount} bones",
                               EditorStyles.boldLabel);
    EditorGUI.indentLevel++;
    foreach (var a in attributes) {
      // only list skin related attributes
      if (a.attribute == VertexAttribute.BlendIndices ||
          a.attribute == VertexAttribute.BlendWeight)
        EditorGUILayout.LabelField(a.attribute.ToString(),
                                   GetAttributeString(a));
    }
    EditorGUI.indentLevel--;
  }
  ...
  static void ShowIndexInfo(Mesh mesh) {
    ...
    EditorGUILayout.Space();
    EditorGUILayout.LabelField(
        $"Indices: {indexCount}, {mesh.indexFormat} format ({bufferSizeStr})",
        EditorStyles.boldLabel);
    EditorGUI.indentLevel++;
    ...
    for (int i = 0; i < mesh.subMeshCount; i++) {
      ...
      string baseVertex =
          subMesh.baseVertex == 0 ? "" : ", base vertex " + subMesh.baseVertex;
      ...
      case MeshTopology.LineStrip:
        divisor = 2;
        break; // technically not correct, but eh
      }
      ...
      EditorGUILayout.LabelField(
          $"#{i}: {primCount} {topology} ({subMesh.indexCount} indices starting from {subMesh.indexStart}){baseVertex}");
      if (subMeshCount > 1) {
  ...
  static void ShowVertexInfo(Mesh mesh,
    ...
    var vertexSize = attributes.Sum(attr => ConvertFormatToSize(attr.format) *
                                            attr.dimension);
    var bufferSizeStr =
        EditorUtility.FormatBytes(mesh.vertexCount * vertexSize);
    EditorGUILayout.LabelField(
        $"Vertices: {mesh.vertexCount} ({bufferSizeStr})",
        EditorStyles.boldLabel);
