...
internal class AnimatorInspector : Editor {
  ...
  private bool IsWarningMessageEmpty {
    get {
      return m_WarningMessage != null &&
             m_WarningMessage.stringValue.Length > 0;
    }
  }
  private string WarningMessage {
    get { return m_WarningMessage != null ? m_WarningMessage.stringValue : ""; }
  }

  class Styles {
    public GUIContent applyRootMotion =
        new GUIContent(EditorGUIUtility.TrTextContent("Apply Root Motion"));
    public GUIContent updateMode =
        new GUIContent(EditorGUIUtility.TrTextContent("Update Mode"));
    public GUIContent cullingMode =
        new GUIContent(EditorGUIUtility.TrTextContent("Culling Mode"));

    public Styles() {
      applyRootMotion.tooltip =
          "Automatically move the object using the root motion from the animations";
      updateMode.tooltip =
          "Controls when and how often the Animator is updated";
      cullingMode.tooltip =
          "Controls what is updated when the object has been culled";
    }
  }
  ...
  public override void OnInspectorGUI() {
    ...
    var controller =
        m_Controller.objectReferenceValue as RuntimeAnimatorController;
    ...
    if (firstAnimator.supportsOnAnimatorMove && !isEditingMultipleObjects) {
      ...
      // This might change between layout & repaint so we have local cached
      // value to only update on layout
      if (Event.current.type == EventType.Layout)
        m_IsRootPositionOrRotationControlledByCurves =
            firstAnimator.isRootPositionOrRotationControlledByCurves;

      if (!m_ApplyRootMotion.boolValue &&
          m_IsRootPositionOrRotationControlledByCurves) {
