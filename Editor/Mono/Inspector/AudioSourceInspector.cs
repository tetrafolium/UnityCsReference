...
public AudioCurveWrapper(AudioCurveType type, string legend, int id,
                         Color color, SerializedProperty curveProp,
...
internal static Color kRolloffCurveColor =
	new Color(0.90f, 0.30f, 0.20f, 1.0f);
internal static Color kSpatialCurveColor =
	new Color(0.25f, 0.70f, 0.20f, 1.0f);
internal static Color kSpreadCurveColor =
	new Color(0.25f, 0.55f, 0.95f, 1.0f);
internal static Color kLowPassCurveColor =
	new Color(0.80f, 0.25f, 0.90f, 1.0f);
internal static Color kReverbZoneMixCurveColor =
	new Color(0.70f, 0.70f, 0.20f, 1.0f);
...
public static GUIContent rolloffLabel = EditorGUIUtility.TrTextContent(
	"Volume Rolloff", "Which type of rolloff curve to use");
public static string controlledByCurveLabel = "Controlled by curve";
public static GUIContent audioClipLabel = EditorGUIUtility.TrTextContent(
	"AudioClip",
	"The AudioClip asset played by the AudioSource. Can be undefined if the AudioSource is generating a live stream of audio via OnAudioFilterRead.");
public static GUIContent panStereoLabel = EditorGUIUtility.TrTextContent(
	"Stereo Pan",
	"Only valid for Mono and Stereo AudioClips. Mono sounds will be panned at constant power left and right. Stereo sounds will have each left/right value faded up and down according to the specified pan value.");
public static GUIContent spatialBlendLabel = EditorGUIUtility.TrTextContent(
	"Spatial Blend",
	"Sets how much this AudioSource is treated as a 3D source. 3D sources are affected by spatial position and spread. If 3D Pan Level is 0, all spatial attenuation is ignored.");
public static GUIContent reverbZoneMixLabel = EditorGUIUtility.TrTextContent(
	"Reverb Zone Mix",
	"Sets how much of the signal this AudioSource is mixing into the global reverb associated with the zones. [0, 1] is a linear range (like volume) while [1, 1.1] lets you boost the reverb mix by 10 dB.");
public static GUIContent dopplerLevelLabel = EditorGUIUtility.TrTextContent(
	"Doppler Level",
	"Specifies how much the pitch is changed based on the relative velocity between AudioListener and AudioSource.");
public static GUIContent spreadLabel = EditorGUIUtility.TrTextContent(
	"Spread", "Sets the spread of a 3d sound in speaker space");
public static GUIContent outputMixerGroupLabel = EditorGUIUtility.TrTextContent(
	"Output",
	"Set whether the sound should play through an Audio Mixer first or directly to the Audio Listener");
public static GUIContent volumeLabel = EditorGUIUtility.TrTextContent(
	"Volume", "Sets the overall volume of the sound.");
public static GUIContent pitchLabel = EditorGUIUtility.TrTextContent(
	"Pitch",
	"Sets the frequency of the sound. Use this to slow down or speed up the sound.");
public static GUIContent priorityLabel = EditorGUIUtility.TrTextContent(
	"Priority",
	"Sets the priority of the source. Note that a sound with a larger priority value will more likely be stolen by sounds with smaller priority values.");
public static GUIContent spatializeLabel = EditorGUIUtility.TrTextContent(
	"Spatialize",
	"Enables or disables custom spatialization for the AudioSource.");
public static GUIContent spatializePostEffectsLabel =
	EditorGUIUtility.TrTextContent(
		"Spatialize Post Effects",
		"Determines if the custom spatializer is applied before or after the effect filters attached to the AudioSource. This flag only has an effect if the spatialize flag is enabled on the AudioSource.");
public static GUIContent priorityLeftLabel =
	EditorGUIUtility.TrTextContent("High");
public static GUIContent priorityRightLabel =
	EditorGUIUtility.TrTextContent("Low");
public static GUIContent spatialLeftLabel =
	EditorGUIUtility.TrTextContent("2D");
public static GUIContent spatialRightLabel =
	EditorGUIUtility.TrTextContent("3D");
public static GUIContent panLeftLabel =
	EditorGUIUtility.TrTextContent("Left");
public static GUIContent panRightLabel =
...
void OnEnable() {
	...
	m_SpatializePostEffects =
		serializedObject.FindProperty("SpatializePostEffects");
	...
	m_BypassListenerEffects =
		serializedObject.FindProperty("BypassListenerEffects");
	m_BypassReverbZones = serializedObject.FindProperty("BypassReverbZones");
	m_OutputAudioMixerGroup =
		serializedObject.FindProperty("OutputAudioMixerGroup");

	m_AudioCurves = new AudioCurveWrapper[] {
		new AudioCurveWrapper(
			AudioCurveType.Volume, "Volume", kRolloffCurveID,
			kRolloffCurveColor,
			serializedObject.FindProperty("rolloffCustomCurve"), 0, 1),
		new AudioCurveWrapper(
			AudioCurveType.SpatialBlend, "Spatial Blend", kSpatialBlendCurveID,
			kSpatialCurveColor,
			serializedObject.FindProperty("panLevelCustomCurve"), 0, 1),
		new AudioCurveWrapper(
			AudioCurveType.Spread, "Spread", kSpreadCurveID, kSpreadCurveColor,
			serializedObject.FindProperty("spreadCustomCurve"), 0, 1),
		new AudioCurveWrapper(AudioCurveType.Lowpass, "Low-Pass",
		                      kLowPassCurveID, kLowPassCurveColor, null, 0, 1),
		new AudioCurveWrapper(
			AudioCurveType.ReverbZoneMix, "Reverb Zone Mix",
			kReverbZoneMixCurveID, kReverbZoneMixCurveColor,
			serializedObject.FindProperty("reverbZoneMixCustomCurve"), 0,
			1.1f)
	};
	...
	m_CurveEditor =
		new CurveEditor(new Rect(0, 0, 1000, 100), new CurveWrapper[0], false);
	...
	foreach (AudioCurveWrapper audioCurve in m_AudioCurves) {
		...
		if (audioCurve.type == AudioCurveType.Volume) {
			...
			if (m_RolloffMode.hasMultipleDifferentValues) {
				...
				includeCurve = !m_MinDistance.hasMultipleDifferentValues &&
				               !m_MaxDistance.hasMultipleDifferentValues;
				if (mode == AudioRolloffMode.Linear)
					curve = AnimationCurve.Linear(m_MinDistance.floatValue /
					                              m_MaxDistance.floatValue,
					                              1.0f, 1.0f, 0.0f);
				else if (mode == AudioRolloffMode.Logarithmic)
					curve =
						Logarithmic(m_MinDistance.floatValue / m_MaxDistance.floatValue,
						            1.0f, 1.0f);
			}
		}
		// All other curves
...
private CurveWrapper GetCurveWrapper(AnimationCurve curve,
                                     AudioCurveWrapper audioCurve) {
	float colorMultiplier = !EditorGUIUtility.isProSkin ? 0.9f : 1.0f;
	Color colorMult =
		new Color(colorMultiplier, colorMultiplier, colorMultiplier, 1);
...
private static float LogarithmicValue(float distance, float minDistance,
                                      float rolloffScale) {
...
/// A logarithmic curve starting at /timeStart/, /valueStart/ and ending at
/// /timeEnd/, /valueEnd/
private static AnimationCurve Logarithmic(float timeStart, float timeEnd,
                                          float logBase) {
	float value, slope, s;
	...
	for (float d = timeStart; d < timeEnd; d *= step) {
		...
		s = d / 50.0f;
		slope = (LogarithmicValue(d + s, timeStart, logBase) -
		         LogarithmicValue(d - s, timeStart, logBase)) /
		        (s * 2);
	...
	s = timeEnd / 50.0f;
	slope = (LogarithmicValue(timeEnd + s, timeStart, logBase) -
	         LogarithmicValue(timeEnd - s, timeStart, logBase)) /
	        (s * 2);
...
private void Update() {
	...
	if ((m_LastSourcePosition - sourcePos).sqrMagnitude > EPSILON ||
	    (m_LastListenerPosition - listenerPos).sqrMagnitude > EPSILON) {
...
private void HandleLowPassFilter() {
	...
	for (int i = 0; i < targets.Length; i++) {
		filterArray[i] =
			((AudioSource) targets[i]).GetComponent<AudioLowPassFilter>();
	...
	// If we don't have the corresponding SerializedObject and
	// SerializedProperties, create them.
	if (audioCurve.curveProp == null) {
		m_LowpassObject = new SerializedObject(filterArray);
		audioCurve.curveProp =
			m_LowpassObject.FindProperty("lowpassLevelCustomCurve");
	}
}

public override void OnInspectorGUI() {
	// Bug fix: 1018456 Moved the HandleLowPassFilter method before updating the
	// serializedObjects
	HandleLowPassFilter();
	...
	EditorGUILayout.PropertyField(m_OutputAudioMixerGroup,
	                              Styles.outputMixerGroupLabel);
	...
	if (AudioUtil.canUseSpatializerEffect) {
		...
		using (new EditorGUI.DisabledScope(!m_Spatialize.boolValue)) {
			EditorGUILayout.PropertyField(m_SpatializePostEffects,
			                              Styles.spatializePostEffectsLabel);
		}
	}
	...
	EditorGUILayout.Space();
	EditorGUIUtility.sliderLabels.SetLabels(Styles.priorityLeftLabel,
	                                        Styles.priorityRightLabel);
	...
	EditorGUIUtility.sliderLabels.SetLabels(Styles.panLeftLabel,
	                                        Styles.panRightLabel);
	EditorGUILayout.Slider(m_Pan2D, -1f, 1f, Styles.panStereoLabel);
	EditorGUIUtility.sliderLabels.SetLabels(null, null);
	EditorGUILayout.Space();

	// 3D Level control
	EditorGUIUtility.sliderLabels.SetLabels(Styles.spatialLeftLabel,
	                                        Styles.spatialRightLabel);
	AnimProp(Styles.spatialBlendLabel,
	         m_AudioCurves[kSpatialBlendCurveID].curveProp, 0.0f, 1.0f, false);
	EditorGUIUtility.sliderLabels.SetLabels(null, null);
	EditorGUILayout.Space();

	// 3D Level control
	AnimProp(Styles.reverbZoneMixLabel,
	         m_AudioCurves[kReverbZoneMixCurveID].curveProp, 0.0f, 1.1f, false);
	EditorGUILayout.Space();

	m_Expanded3D =
		EditorGUILayout.Foldout(m_Expanded3D, "3D Sound Settings", true);
	if (m_Expanded3D) {
...
private static void SetRolloffToTarget(SerializedProperty property,
                                       Object target) {
	property.SetToValueOfTarget(target);
	property.serializedObject.FindProperty("rolloffMode")
	.SetToValueOfTarget(target);
...
private void Audio3DGUI() {
	EditorGUILayout.Slider(m_DopplerLevel, 0.0f, 5.0f,
	                       Styles.dopplerLevelLabel);

	// Spread control
	AnimProp(Styles.spreadLabel, m_AudioCurves[kSpreadCurveID].curveProp, 0.0f,
	         360.0f, true);

	// Rolloff mode
	if (m_RolloffMode.hasMultipleDifferentValues ||
	    (m_RolloffMode.enumValueIndex == (int) AudioRolloffMode.Custom &&
	     m_AudioCurves[kRolloffCurveID].curveProp.hasMultipleDifferentValues)) {
		EditorGUILayout.TargetChoiceField(
			m_AudioCurves[kRolloffCurveID].curveProp, Styles.rolloffLabel,
			SetRolloffToTarget);
	} else {
		EditorGUILayout.PropertyField(m_RolloffMode, Styles.rolloffLabel);

		if ((AudioRolloffMode)
		    m_RolloffMode.enumValueIndex != AudioRolloffMode.Custom) {
			EditorGUI.BeginChangeCheck();
			EditorGUILayout.PropertyField(m_MinDistance);
			if (EditorGUI.EndChangeCheck()) {
				m_MinDistance.floatValue = Mathf.Clamp(
					m_MinDistance.floatValue, 0, m_MaxDistance.floatValue / 1.01f);
			}
		} else {
			using (new EditorGUI.DisabledScope(true)) {
				EditorGUILayout.LabelField(m_MinDistance.displayName,
				                           Styles.controlledByCurveLabel);
			}
		}
	}
	...
	if (EditorGUI.EndChangeCheck())
		m_MaxDistance.floatValue =
			Mathf.Min(Mathf.Max(Mathf.Max(m_MaxDistance.floatValue, 0.01f),
			                    m_MinDistance.floatValue * 1.01f),
			          1000000.0f);
	...
	if (targets.Length == 1) {
		...
		AudioListener audioListener =
			(AudioListener) FindObjectOfType(typeof(AudioListener));
		if (audioListener != null) {
			float distToListener =
				(AudioUtil.GetListenerPos() - t.transform.position).magnitude;
	...
	if (!m_CurveEditor.InLiveEdit()) {
		...
		foreach (AudioCurveWrapper audioCurve in m_AudioCurves) {
			if ((m_CurveEditor.GetCurveWrapperFromID(audioCurve.id) != null) &&
			    (m_CurveEditor.GetCurveWrapperFromID(audioCurve.id).changed)) {
				AnimationCurve changedCurve =
					m_CurveEditor.GetCurveWrapperFromID(audioCurve.id).curve;
...
void DrawLegend() {
	...
	Rect legendRect =
		GUILayoutUtility.GetRect(10, 40 * EditorGUIUtility.pixelsPerPoint);
	...
	int width =
		Mathf.Min(75, Mathf.FloorToInt(legendRect.width / curves.Count));
	for (int i = 0; i < curves.Count; i++) {
		legendRects.Add(new Rect(legendRect.x + width * i, legendRect.y, width,
		                         legendRect.height));
	}
	...
	if (EditorGUIExt.DragSelection(legendRects.ToArray(), ref m_SelectedCurves,
	                               GUIStyle.none) ||
	    resetSelections) {
	...
	for (int i = 0; i < curves.Count; i++) {
		EditorGUI.DrawLegend(legendRects[i], curves[i].color,
		                     curves[i].legend.text, m_SelectedCurves[i]);
		if (curves[i].curveProp.hasMultipleDifferentValues) {
			GUI.Button(new Rect(legendRects[i].x, legendRects[i].y + 20,
			                    legendRects[i].width, 20),
			           "Different");
		}
	}
}
...
private List<AudioCurveWrapper> GetShownAudioCurves() {
	return m_AudioCurves
	       .Where(f => m_CurveEditor.GetCurveWrapperFromID(f.id) != null)
	       .ToList();
}
...
private void SyncShownCurvesToLegend(List<AudioCurveWrapper> curves) {
	...
	for (int i = 0; i < curves.Count; i++)
		m_CurveEditor.GetCurveWrapperFromID(curves[i].id).hidden =
			!m_SelectedCurves[i];
...
void DrawLabel(string label, float value, Rect r) {
	...
	size.x += 2;
	Vector2 posA = m_CurveEditor.DrawingToViewTransformPoint(
		new Vector2(value / m_MaxDistance.floatValue, 0));
	Vector2 posB = m_CurveEditor.DrawingToViewTransformPoint(
		new Vector2(value / m_MaxDistance.floatValue, 1));
	GUI.BeginGroup(r);
	Color temp = Handles.color;
	Handles.color = new Color(1, 0, 0, 0.3f);
	Handles.DrawLine(new Vector3(posA.x, posA.y, 0),
	                 new Vector3(posB.x, posB.y, 0));
	Handles.DrawLine(new Vector3(posA.x + 1, posA.y, 0),
	                 new Vector3(posB.x + 1, posB.y, 0));
	Handles.color = temp;
	GUI.Label(new Rect(Mathf.Floor(posB.x - size.x / 2), 2, size.x, 15), label,
	          Styles.labelStyle);
	GUI.EndGroup();
}

internal static void AnimProp(GUIContent label, SerializedProperty prop,
                              float min, float max, bool useNormalizedValue) {
	...
	if (curve.length != 1) {
		...
		float f =
			useNormalizedValue ? Mathf.Lerp(min, max, curve.keys[0].value)
	  : curve.keys[0].value;
...
void OnSceneGUI() {
	...
	EditorGUI.BeginChangeCheck();
	float minDistance = Handles.RadiusHandle(Quaternion.identity, position,
	                                         source.minDistance, true);
	float maxDistance = Handles.RadiusHandle(Quaternion.identity, position,
	                                         source.maxDistance, true);
	if (EditorGUI.EndChangeCheck()) {
