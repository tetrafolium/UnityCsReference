...
internal class ShaderVariantCollectionInspector : Editor {
  private class Styles {
    public static readonly GUIContent iconAdd =
        EditorGUIUtility.TrIconContent("Toolbar Plus", "Add variant");
    public static readonly GUIContent iconRemove =
        EditorGUIUtility.TrIconContent("Toolbar Minus", "Remove entry");
  ...
  void DisplayAddVariantsWindow(Shader shader,
  ...
  void DrawShaderEntry(int shaderIndex) {
    ...
    Shader shader =
        (Shader) entryProp.FindPropertyRelative("first").objectReferenceValue;

    // Shader name and button to remove it
    var variantsProp = entryProp.FindPropertyRelative("second.variants");
    using(new GUILayout.HorizontalScope()) {
      Rect rowRect =
          GUILayoutUtility.GetRect(GUIContent.none, EditorStyles.boldLabel);
      Rect minusRect = GetAddRemoveButtonRect(rowRect);
      rowRect.xMax = minusRect.x;

      GUI.Label(rowRect, shader == null ? "<MISSING SHADER>"
                : shader.name, EditorStyles.boldLabel);
    ...
    for (var i = 0; i < variantsProp.arraySize; ++i) {
      ...
      var passType = (UnityEngine.Rendering.PassType)
                         prop.FindPropertyRelative("passType")
                             .intValue;
      Rect rowRect =
    ...
    if (shader != null &&
        GUI.Button(plusRect, Styles.iconAdd, Styles.invisibleButton)) {
  ...
  public override void OnInspectorGUI() {
    ...
    if (GUILayout.Button("Add shader")) {
      ...
      ObjectSelector.get.objectSelectorID =
          "ShaderVariantSelector".GetHashCode();
      GUIUtility.ExitGUI();
    }
    if (Event.current.type == EventType.ExecuteCommand) {
      ...
      if (Event.current.commandName ==
        ...
        if (newShader != null) {
          ShaderUtil.AddNewShaderToCollection(
              newShader, target as ShaderVariantCollection);
        }
        Event.current.Use();
        GUIUtility.ExitGUI();
      }
...
// (e.g. about 30000), and approaches like "display a popup menu with all of
// them" don't work.
internal class AddShaderVariantWindow : EditorWindow {
  ...
  static readonly float kMiscUIHeight = 5 * EditorGUI.kSingleLineHeight +
                                        kSeparatorHeight * 2 +
                                        kSpaceHeight * 5 + kMargin * 2;

  private static readonly float kMinWindowHeight =
      9 * EditorGUI.kSingleLineHeight + kMiscUIHeight;

  PopupData m_Data;
  List<string> m_SelectedKeywords;
  List<string> m_AvailableKeywords;
  List<int>
      m_SelectedVariants; // Indices of variants currently selected for adding
  ...
  public AddShaderVariantWindow() {
    position =
        new Rect(100, 100, kMinWindowWidth * 1.5f, kMinWindowHeight * 1.5f);
  ...
  public static void ShowAddVariantWindow(PopupData data) {
    var w = EditorWindow.GetWindow<AddShaderVariantWindow>(
        true, "Add shader " + data.shader.name + " variants to collection");
    w.Initialize(data);
    w.m_Parent.window.m_DontSaveToLayout = true;
  }

  void ApplyKeywordFilter() {
    m_MaxVisibleVariants =
        (int)(CalcVerticalSpaceForVariants() / EditorGUI.kSingleLineHeight);
    ...
    ShaderUtil.GetShaderVariantEntriesFiltered(
        ...
        m_SelectedKeywords.ToArray(), m_Data.collection,
        out m_FilteredVariantTypes, out keywordLists, out remainingKeywords);
    ...
    for (var i = 0; i < m_NumFilteredVariants; ++i) {
      m_FilteredVariantKeywords[i] = keywordLists [i]
                                         .Split(' ');
    }
  ...
  private bool KeywordButton(Rect buttonRect, string k, Vector2 areaSize) {
    // If we can't fit all buttons (shader has *a lot* of keywords) and would
    // start clipping, do display the partially clipped ones with some
    // transparency.
    var oldColor = GUI.color;
    if (buttonRect.yMax > areaSize.y)
      GUI.color = new Color(1, 1, 1, 0.4f);

    var result = GUI.Button(buttonRect, EditorGUIUtility.TempContent(k),
  ...
  void DrawKeywordsList(ref Rect rect, List<string> keywords,
                        bool clickingAddsToSelected) {
    ...
    var layoutRects = EditorGUIUtility.GetFlowLayoutedRects(
        indentRect, EditorStyles.miniButton, 2, 2, displayKeywords);
  ...
  private void Draw(Rect windowRect) {
    var rect = new Rect(kMargin, kMargin, windowRect.width - kMargin * 2,
                        EditorGUI.kSingleLineHeight);

    DrawSectionHeader(
        ref rect, "Pick shader keywords to narrow down variant list:", false);
    ...
    DrawSectionHeader(
        ref rect,
        "Shader variants with these keywords (click to select):", true);

    if (m_NumFilteredVariants > 0) {
      int maxFilteredLength =
          (int)(CalcVerticalSpaceForVariants() / EditorGUI.kSingleLineHeight);

      if (maxFilteredLength >
          m_MaxVisibleVariants) // Query data again if we have bigger window
                                // than at last query
        ApplyKeywordFilter();

      // Display first N variants (don't want to display thousands of them if
      // filter is not narrow)
      for (var i = 0; i < Mathf.Min(m_NumFilteredVariants, maxFilteredLength);
           ++i) {
        var passType =
            (UnityEngine.Rendering.PassType) m_FilteredVariantTypes[i];
        var wasSelected = m_SelectedVariants.Contains(i);
        var keywordString =
            string.IsNullOrEmpty(m_FilteredVariantKeywords [i]
                                 [0])
                ? "<no keywords>"
                : string.Join(" ", m_FilteredVariantKeywords[i]);
        var displayString = passType + " " + keywordString.ToLowerInvariant();
        var isSelected =
            GUI.Toggle(rect, wasSelected, displayString, Styles.sMenuItem);
        rect.y += rect.height;
      ...
      if (m_NumFilteredVariants > maxFilteredLength) {
        GUI.Label(
            rect,
            "List of variants was cropped. Pick further keywords to narrow the selection.",
    ...
    using(new EditorGUI.DisabledScope(m_SelectedVariants.Count == 0)) {
      if (GUI.Button(rect, string.Format("Add {0} selected variants",
                                         m_SelectedVariants.Count))) {
        ...
        for (var i = 0; i < m_SelectedVariants.Count; ++i) {
          ...
          var variant = new ShaderVariantCollection.ShaderVariant(
              m_Data.shader,
              (UnityEngine.Rendering.PassType) m_FilteredVariantTypes[index],
              m_FilteredVariantKeywords[index]);
