...
public static readonly GUIContent nameLabel =
	EditorGUIUtility.TrTextContent("Name", "The name for the Sprite.");
public static readonly GUIContent spriteAlignmentLabel =
	EditorGUIUtility.TrTextContent(
		"Pivot",
		"The value is normalized to the Sprite's size where (0, 0) is the lower left and (1, 1) is the upper right. May be used for syncing animation frames of different sizes.");
public static readonly GUIContent spriteAlignmentText =
	EditorGUIUtility.TrTextContent("X:{0:0.##}, Y:{1:0.##}");
public static readonly GUIContent borderLabel = EditorGUIUtility.TrTextContent(
	"Border",
	"Border values for the Sprite set in Sprite Editor window. May be useful for 9-Slicing Sprites.");
public static readonly GUIContent borderText =
	EditorGUIUtility.TrTextContent(
		"L:{0:0.##} B:{1:0.##} R:{2:0.##} T:{3:0.##}");
public static readonly GUIContent multiValueText =
	EditorGUIUtility.TrTextContent("-");
}
...
void OnEnable() {
	...
	m_SpriteAlignmentContent = new GUIContent(
		UnityString.Format(Styles.spriteAlignmentText.text,
		                   m_Pivot.vector2Value.x, m_Pivot.vector2Value.y));
	m_SpriteBorderContent = new GUIContent(
		UnityString.Format(Styles.borderText.text, m_BorderValue.x,
		                   m_BorderValue.y, m_BorderValue.z, m_BorderValue.w));
}
...
public override void OnInspectorGUI() {
	...
	if (!m_Pivot.hasMultipleDifferentValues)
		EditorGUILayout.LabelField(Styles.spriteAlignmentLabel,
		                           m_SpriteAlignmentContent);
	else
		EditorGUILayout.LabelField(Styles.spriteAlignmentLabel,
		                           Styles.multiValueText);
...
public static Texture2D BuildPreviewTexture(int width, int height,
                                            Sprite sprite,
                                            Material spriteRendererMaterial,
                                            bool isPolygon) {
	...
	Texture2D texture =
		UnityEditor.Sprites.SpriteUtility.GetSpriteTexture(sprite, false);

	// only adjust the preview texture size if the sprite is not in polygon
	// mode. In polygon mode, we are looking at a 4x4 texture will detailed
	// mesh. It's better to maximize the display of it.
	if (!isPolygon) {
		PreviewHelpers.AdjustWidthAndHeightForStaticPreview(
			(int) spriteWidth, (int) spriteHeight, ref width, ref height);
	}
	...
	if (spriteRendererMaterial != null) {
		...
		_matHasTexelSize =
			spriteRendererMaterial.HasProperty("_MainTex_TexelSize");
	}
	...
	if (spriteRendererMaterial != null) {
		...
		if (_matHasTexelSize) {
			...
			spriteRendererMaterial.SetVector(
				"_MainTex_TexelSize", TextureUtil.GetTexelSizeVector(texture));
		}
	...
	for (int i = 0; i < triangles.Length; ++i) {
		...
		GL.Vertex3((vertex.x * pixelsToUnits + pivot.x) / spriteWidth,
		           (vertex.y * pixelsToUnits + pivot.y) / spriteHeight, 0);
	}
	GL.End();
	GL.PopMatrix();
	...
	var tmp2 = RenderTexture.GetTemporary(width, height, 0,
...
public static void DrawPreview(Rect r, Sprite frame,
                               Material spriteRendererMaterial,
                               bool isPolygon) {
	if (frame == null)
		return;
	...
	float zoomLevel =
		Mathf.Min(r.width / frame.rect.width, r.height / frame.rect.height);
	Rect wantedRect = new Rect(r.x, r.y, frame.rect.width * zoomLevel,
	                           frame.rect.height * zoomLevel);
	wantedRect.center = r.center;

	Texture2D previewTexture =
		BuildPreviewTexture((int) wantedRect.width, (int) wantedRect.height,
		                    frame, spriteRendererMaterial, isPolygon);
	EditorGUI.DrawTextureTransparent(wantedRect, previewTexture,
...
public override string GetInfoString() {
	...
	return UnityString.Format("({0}x{1})", (int) sprite.rect.width,
