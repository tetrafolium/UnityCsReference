...
public readonly GUIContent size = EditorGUIUtility.TrTextContent(
	"Size", "Size of the render texture in pixels.");
public readonly GUIContent cross = EditorGUIUtility.TextContent("x");
public readonly GUIContent antiAliasing = EditorGUIUtility.TrTextContent(
	"Anti-aliasing", "Number of anti-aliasing samples.");
public readonly GUIContent colorFormat = EditorGUIUtility.TrTextContent(
	"Color Format", "Format of the color buffer.");
public readonly GUIContent depthBuffer = EditorGUIUtility.TrTextContent(
	"Depth Buffer", "Format of the depth buffer.");
public readonly GUIContent enableCompatibleFormat =
	EditorGUIUtility.TrTextContent(
		"Enable Compatible Color Format",
		"Lets the color format be changed to compatible and supported formats for the target platform automatically, if the target platform doesn't support the input format.");
public readonly GUIContent dimension = EditorGUIUtility.TrTextContent(
	"Dimension", "Is the texture 2D, Cube or 3D?");
public readonly GUIContent enableMipmaps = EditorGUIUtility.TrTextContent(
	"Enable Mip Maps", "This render texture will have Mip Maps.");
public readonly GUIContent bindMS = EditorGUIUtility.TrTextContent(
	"Bind multisampled",
	"If enabled, the texture will not go through an AA resolve if bound to a shader.");
public readonly GUIContent autoGeneratesMipmaps =
	EditorGUIUtility.TrTextContent(
		"Auto generate Mip Maps",
		"This render texture automatically generates its Mip Maps.");
public readonly GUIContent sRGBTexture = EditorGUIUtility.TrTextContent(
	"sRGB (Color RenderTexture)",
	"RenderTexture content is stored in gamma space. Non-HDR color textures should enable this flag.");
public readonly GUIContent useDynamicScale = EditorGUIUtility.TrTextContent(
	"Dynamic Scaling",
	"Allow the texture to be automatically resized by ScalableBufferManager, to support dynamic resolution.");
...
public readonly GUIContent[] dimensionStrings = {
	EditorGUIUtility.TextContent("2D"),
	EditorGUIUtility.TextContent("2D Array"),
	EditorGUIUtility.TrTextContent("Cube"),
	EditorGUIUtility.TrTextContent("3D")
};
public readonly int[] dimensionValues = {
	(int) UnityEngine.Rendering.TextureDimension.Tex2D,
	(int) UnityEngine.Rendering.TextureDimension.Tex2DArray,
	(int) UnityEngine.Rendering.TextureDimension.Cube,
	(int) UnityEngine.Rendering.TextureDimension.Tex3D
};
}
...
const GUIElements s_AllGUIElements =
	GUIElements.RenderTargetDepthGUI | GUIElements.RenderTargetAAGUI;
...
protected override void OnEnable() {
	...
	m_EnableCompatibleFormat =
		serializedObject.FindProperty("m_EnableCompatibleFormat");
...
protected void OnRenderTextureGUI(GUIElements guiElements) {
	...
	bool isTexture3D = (m_Dimension.intValue ==
	                    (int) UnityEngine.Rendering.TextureDimension.Tex3D);

	EditorGUILayout.IntPopup(m_Dimension, styles.dimensionStrings,
	                         styles.dimensionValues, styles.dimension);

	GUILayout.BeginHorizontal();
	EditorGUILayout.PrefixLabel(styles.size, EditorStyles.popup);
	EditorGUILayout.DelayedIntField(m_Width, GUIContent.none,
	                                GUILayout.MinWidth(40));
	GUILayout.Label(styles.cross);
	EditorGUILayout.DelayedIntField(m_Height, GUIContent.none,
	                                GUILayout.MinWidth(40));
	if (isTexture3D) {
		GUILayout.Label(styles.cross);
		EditorGUILayout.DelayedIntField(m_Depth, GUIContent.none,
		                                GUILayout.MinWidth(40));
	}
	...
	if ((guiElements & GUIElements.RenderTargetAAGUI) != 0)
		EditorGUILayout.IntPopup(m_AntiAliasing, styles.renderTextureAntiAliasing,
		                         styles.renderTextureAntiAliasingValues,
		                         styles.antiAliasing);

	GraphicsFormat format = (GraphicsFormat) m_ColorFormat.intValue;
	GraphicsFormat compatibleFormat =
		SystemInfo.GetCompatibleFormat(format, FormatUsage.Render);

	using (new EditorGUI.DisabledScope(compatibleFormat == GraphicsFormat.None))
		EditorGUILayout.PropertyField(m_EnableCompatibleFormat,
		                              styles.enableCompatibleFormat);

	EditorGUILayout.PropertyField(m_ColorFormat, styles.colorFormat);

	if (compatibleFormat != format) {
		string text = string.Format(
			"Format {0} is not supported on this platform. ", format.ToString());
		if (compatibleFormat != GraphicsFormat.None) {
			if (m_EnableCompatibleFormat.boolValue)
				text += string.Format("Using {0} as a compatible format.",
				                      compatibleFormat.ToString());
			else
				text += string.Format(
					"You may enable Compatible Color Format to fallback automatically to a platform specific compatible format, {0} on this device.",
					compatibleFormat.ToString());
		}
		EditorGUILayout.HelpBox(
			text, m_EnableCompatibleFormat.boolValue &&
			compatibleFormat != GraphicsFormat.None ? MessageType.Warning
	  : MessageType.Error);
	}
	...
	using (new EditorGUI.DisabledScope(isTexture3D)) {
		...
		using (new EditorGUI.DisabledScope(!m_EnableMipmaps.boolValue))
			EditorGUILayout.PropertyField(m_AutoGeneratesMipmaps,
			                              styles.autoGeneratesMipmaps);
	}

	if (isTexture3D) {
		// Mip map generation is not supported yet for 3D textures.
		EditorGUILayout.HelpBox("3D RenderTextures do not support Mip Maps.",
		                        MessageType.Info);
	}
	...
	using (new EditorGUI.DisabledScope(
		       RenderTextureHasDepth())) // Render Textures with depth are forced to 0
	                                         // Aniso Level
	{
		DoAnisoLevelSlider();
	}
	if (RenderTextureHasDepth()) {
		// RenderTextures don't enforce this nicely. RenderTexture only forces
		// Aniso to 0 if the gfx card supports depth, rather than forcing Aniso to
		// zero depending on what the user asks of the RT. If the user requests
		// any kind of depth then we will force aniso to zero here.
		m_Aniso.intValue = 0;
		EditorGUILayout.HelpBox(
...
public override void OnPreviewSettings() {
	if (m_Dimension.intValue ==
		...
		if (rt.volumeDepth > 1) {
			m_Slice = EditorGUILayout.IntSlider(m_Slice, 0, rt.volumeDepth - 1,
			                                    GUILayout.Width(150));
...
void InitPreview() {
	if (m_Material == null) {
		m_Material = (Material) EditorGUIUtility.LoadRequired(
			"Previews/Preview2DTextureArrayMaterial.mat");
	}
}
...
public override void OnPreviewGUI(Rect r, GUIStyle background) {
	if (m_Dimension.intValue ==
		...
		if (!SystemInfo.supports2DArrayTextures) {
			if (Event.current.type == EventType.Repaint)
				EditorGUI.DropShadowLabel(new Rect(r.x, r.y, r.width, 40),
				                          "2D texture array preview not supported");
			return;
		}
		...
		if (Event.current.type == EventType.Repaint) {
			...
			// If multiple objects are selected, we might be using a slice level
			// before the maximum
			int effectiveSlice = Mathf.Clamp(m_Slice, 0, rt.volumeDepth - 1);

			m_Material.SetInt(s_ShaderSliceIndex, effectiveSlice);
			m_Material.SetInt(s_ShaderToSrgb, QualitySettings.activeColorSpace ==
			...
			PreviewGUI.BeginScrollView(r, m_Pos, wantedRect,
			...
			EditorGUI.DrawPreviewTexture(wantedRect, rt, m_Material,
			                             ScaleMode.StretchToFill, 0, mipLevel);
...
override public string GetInfoString() {
	...
	if (QualitySettings.desiredColorSpace == ColorSpace.Linear) {
		bool formatIsHDR =
			GraphicsFormatUtility.IsIEEE754Format(t.graphicsFormat);
	...
	info += "  " +
	        EditorUtility.FormatBytes(TextureUtil.GetRuntimeMemorySizeLong(t));
