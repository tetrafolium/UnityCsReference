...
public class CameraEditor : Editor {
  private static class Styles {
    public static GUIContent iconRemove = EditorGUIUtility.TrIconContent(
        "Toolbar Minus", "Remove command buffer");
    public static GUIContent clearFlags = EditorGUIUtility.TrTextContent(
        "Clear Flags",
        "What to display in empty areas of this Camera's view.\n\nChoose Skybox to display a skybox in empty areas, defaulting to a background color if no skybox is found.\n\nChoose Solid Color to display a background color in empty areas.\n\nChoose Depth Only to display nothing in empty areas.\n\nChoose Don't Clear to display whatever was displayed in the previous frame in empty areas.");
    public static GUIContent background = EditorGUIUtility.TrTextContent(
        "Background",
        "The Camera clears the screen to this color before rendering.");
    public static GUIContent projection = EditorGUIUtility.TrTextContent(
        "Projection",
        "How the Camera renders perspective.\n\nChoose Perspective to render objects with perspective.\n\nChoose Orthographic to render objects uniformly, with no sense of perspective.");
    public static GUIContent size = EditorGUIUtility.TrTextContent(
        "Size", "The vertical size of the camera view.");
    public static GUIContent fieldOfView = EditorGUIUtility.TrTextContent(
        "Field of View",
        "The camera's view angle measured in degrees along the selected axis.");
    public static GUIContent viewportRect = EditorGUIUtility.TrTextContent(
        "Viewport Rect",
        "Four values that indicate where on the screen this camera view will be drawn. Measured in Viewport Coordinates (values 0-1).");
    public static GUIContent sensorSize = EditorGUIUtility.TrTextContent(
        "Sensor Size", "The size of the camera sensor in millimeters.");
    public static GUIContent lensShift = EditorGUIUtility.TrTextContent(
        "Lens Shift",
        "Offset from the camera sensor. Use these properties to simulate a shift lens. Measured as a multiple of the sensor size.");
    public static GUIContent physicalCamera = EditorGUIUtility.TrTextContent(
        "Physical Camera",
        "Enables Physical camera mode. When checked, the field of view is calculated from properties for simulating physical attributes (focal length, sensor size, and lens shift)");
    public static GUIContent cameraType = EditorGUIUtility.TrTextContent(
        "Sensor Type",
        "Common sensor sizes. Choose an item to set Sensor Size, or edit Sensor Size for your custom settings.");
    public static GUIContent renderingPath = EditorGUIUtility.TrTextContent(
        "Rendering Path",
        "Choose a rendering method for this camera.\n\nUse Graphics Settings to use the rendering path specified in Player settings.\n\nUse Forward to render all objects with one pass per material.\n\nUse Deferred to draw all objects once without lighting and then draw the lighting of all objects at the end of the render queue.\n\nUse Legacy Vertex Lit to render all lights in a single pass, calculated in vertices.\n\nLegacy Deferred has been deprecated.");
    public static GUIContent focalLength = EditorGUIUtility.TrTextContent(
        "Focal Length",
        "The simulated distance between the lens and the sensor of the physical camera. Larger values give a narrower field of view.");
    public static GUIContent allowOcclusionCulling = EditorGUIUtility.TrTextContent(
        "Occlusion Culling",
    ...
    public static GUIContent allowHDR = EditorGUIUtility.TrTextContent(
        "HDR",
        "High Dynamic Range gives you a wider range of light intensities, so your lighting looks more realistic. With it, you can still see details and experience less saturation even with bright light.");
    public static GUIContent allowMSAA = EditorGUIUtility.TrTextContent(
        "MSAA", "Use Multi Sample Anti-aliasing to reduce aliasing.");
    public static GUIContent gateFit = EditorGUIUtility.TrTextContent(
        "Gate Fit",
        "Determines how the rendered area (resolution gate) fits into the sensor area (film gate).");
    public static GUIContent allowDynamicResolution = EditorGUIUtility.TrTextContent(
        "Allow Dynamic Resolution",
        "Scales render textures to support dynamic resolution if the target platform/graphics API supports it.");
    public static GUIContent FOVAxisMode =
        EditorGUIUtility.TrTextContent("FOV Axis", "Field of view axis.");
    public static GUIStyle invisibleButton = "InvisibleButton";
    public static GUIContent[] displayedOptions =
        new[]{new GUIContent("Off"), new GUIContent("Use Graphics Settings")};
  ...
  public sealed class Settings {
    ...
    public static IEnumerable<string> ApertureFormatNames =>
        k_ApertureFormatNames;

    private static readonly string[] k_ApertureFormatNames = {
        "8mm",         "Super 8mm",    "16mm",     "Super 16mm",
        "35mm 2-perf", "35mm Academy", "Super-35", "65mm ALEXA",
        "70mm",        "70mm IMAX",    "Custom"};

    public static IEnumerable<Vector2> ApertureFormatValues =>
        k_ApertureFormatValues;
    ...
    private static readonly int[] kCameraRenderPathValues = {
        (int) RenderingPath.UsePlayerSettings, (int) RenderingPath.Forward,
        (int) RenderingPath.DeferredShading, (int) RenderingPath.VertexLit,
    ...
    internal SerializedProperty projectionMatrixMode {
      get;
      private set;
    }
    public SerializedProperty sensorSize {
      get;
      private set;
    }
    ...
    public SerializedProperty verticalFOV {
      get;
      private set;
    }
    ...
    public SerializedProperty fovAxisMode {
      get;
      private set;
    }
    ...
    public void OnEnable() {
      ...
      normalizedViewPortRect =
          m_SerializedObject.FindProperty("m_NormalizedViewPortRect");
      ...
      projectionMatrixMode =
          m_SerializedObject.FindProperty("m_projectionMatrixMode");
      ...
      allowDynamicResolution =
          m_SerializedObject.FindProperty("m_AllowDynamicResolution");

      stereoConvergence =
          m_SerializedObject.FindProperty("m_StereoConvergence");
    ...
    public void DrawProjection() {
      ProjectionType projectionType =
          orthographic.boolValue ? ProjectionType.Orthographic
          : ProjectionType.Perspective;
      EditorGUI.BeginChangeCheck();
      EditorGUI.showMixedValue = orthographic.hasMultipleDifferentValues;
      projectionType = (ProjectionType) EditorGUILayout.EnumPopup(
          Styles.projection, projectionType);
      EditorGUI.showMixedValue = false;
      if (EditorGUI.EndChangeCheck())
        orthographic.boolValue =
      ...
      if (!orthographic.hasMultipleDifferentValues) {
        if (projectionType == ProjectionType.Orthographic)
          ...
          bool isPhysicalCamera =
              projectionMatrixMode.intValue ==
              (int) Camera.ProjectionMatrixMode.PhysicalPropertiesBased;

          var rect = EditorGUILayout.GetControlRect();
          var guiContent =
              EditorGUI.BeginProperty(rect, Styles.FOVAxisMode, fovAxisMode);
          EditorGUI.showMixedValue = fovAxisMode.hasMultipleDifferentValues;

          EditorGUI.BeginChangeCheck();
          var fovAxisNewVal = (int)(Camera.FieldOfViewAxis) EditorGUI.EnumPopup(
              rect, guiContent, (Camera.FieldOfViewAxis) fovAxisMode.intValue);
          if (EditorGUI.EndChangeCheck())
          ...
          if (!fovAxisVertical && !fovAxisMode.hasMultipleDifferentValues) {
            ...
            float aspectRatio = isPhysicalCamera ? sensorSize.vector2Value.x /
                                sensorSize.vector2Value.y : camera0.aspect;
            // camera.aspect is not serialized so we have to check all targets.
            fovCurrentValue = Camera.VerticalToHorizontalFieldOfView(
                camera0.fieldOfView, aspectRatio);
          ...
          var content =
              EditorGUI.BeginProperty(EditorGUILayout.BeginHorizontal(),
                                      Styles.fieldOfView, verticalFOV);
          EditorGUI.BeginDisabled(
              projectionMatrixMode.hasMultipleDifferentValues ||
              isPhysicalCamera && (sensorSize.hasMultipleDifferentValues ||
                                   fovAxisMode.hasMultipleDifferentValues));
          EditorGUI.BeginChangeCheck();
          var fovNewValue =
              EditorGUILayout.Slider(content, fovCurrentValue, 0.00001f, 179f);
          ...
          if (EditorGUI.EndChangeCheck())
            projectionMatrixMode.intValue = isPhysicalCamera ?(int)
                                                Camera.ProjectionMatrixMode
                                                    .PhysicalPropertiesBased
                : (int) Camera.ProjectionMatrixMode.Implicit;
          EditorGUILayout.EndHorizontal();
          EditorGUI.EndProperty();
            ...
            using(new EditorGUI.IndentLevelScope()) {
              using(var horizontal = new EditorGUILayout.HorizontalScope()) using(
                  new EditorGUI.PropertyScope(
                      horizontal.rect, Styles.focalLength,
                      focalLength)) using(var checkScope =
                                              new EditorGUI
                                                  .ChangeCheckScope()) {
                EditorGUI.showMixedValue =
                    focalLength.hasMultipleDifferentValues;
                float sensorLength =
                    fovAxisVertical ? sensorSize.vector2Value.y
                    : sensorSize.vector2Value.x;
                float focalLengthVal =
                    fovChanged ? Camera
                        .FieldOfViewToFocalLength(fovNewValue, sensorLength)
                    : focalLength.floatValue;
                focalLengthVal = EditorGUILayout.FloatField(Styles.focalLength,
                                                            focalLengthVal);
              ...
              EditorGUI.BeginChangeCheck();
              int filmGateIndex = Array.IndexOf(
                  k_ApertureFormatValues,
                  new Vector2((float) Math.Round(sensorSize.vector2Value.x, 3),
                              (float)
                                  Math.Round(sensorSize.vector2Value.y, 3)));
              if (filmGateIndex == -1)
                filmGateIndex = EditorGUILayout.Popup(
                    Styles.cameraType, k_ApertureFormatNames.Length - 1,
                    k_ApertureFormatNames);
              else
                filmGateIndex = EditorGUILayout.Popup(
                    Styles.cameraType, filmGateIndex, k_ApertureFormatNames);
              EditorGUI.showMixedValue = false;
              if (EditorGUI.EndChangeCheck() &&
                  filmGateIndex < k_ApertureFormatValues.Length) {
              ...
              using(var horizontal = new EditorGUILayout.HorizontalScope()) using(
                  var propertyScope = new EditorGUI.PropertyScope(
                      horizontal.rect, Styles.gateFit,
                      gateFit)) using(var checkScope =
                                          new EditorGUI.ChangeCheckScope()) {
                int gateValue =
                    (int)(Camera.GateFitMode) EditorGUILayout.EnumPopup(
                        propertyScope.content,
                        (Camera.GateFitMode) gateFit.intValue);
                if (checkScope.changed)
                  gateFit.intValue = gateValue;
              }
            }
          } else if (fovChanged) {
            verticalFOV.floatValue = fovAxisVertical ? fovNewValue
                : Camera.HorizontalToVerticalFieldOfView(
                      fovNewValue,
                      (m_SerializedObject.targetObjects[0] as Camera).aspect);
          }
    ...
    public void DrawClippingPlanes() {
      EditorGUILayout.PropertiesField(
          EditorGUI.s_ClipingPlanesLabel,
          new[]{nearClippingPlane, farClippingPlane},
          EditorGUI.s_NearAndFarLabels, EditorGUI.kNearFarLabelsWidth);
    }
    ...
    public void DrawDepth() { EditorGUILayout.PropertyField(depth); }

    public void DrawRenderingPath() {
      EditorGUILayout.IntPopup(renderingPath, kCameraRenderPaths,
                               kCameraRenderPathValues, Styles.renderingPath);
    }
    ...
    public void DrawTargetTexture(bool deferred) {
      ...
      // only do this if the m_TargetTexture has the same values across all
      // target cameras
      if (!targetTexture.hasMultipleDifferentValues) {
        var targetTexture =
            this.targetTexture.objectReferenceValue as RenderTexture;
        if (targetTexture && targetTexture.antiAliasing > 1 && deferred) {
          EditorGUILayout.HelpBox(
              "Manual MSAA target set with deferred rendering. This will lead to undefined behavior.",
    ...
    public void DrawHDR() {
      ...
      value = EditorGUI.IntPopup(rect, Styles.allowHDR, value,
                                 Styles.displayedOptions, Styles.optionValues);
    ...
    public void DrawMSAA() {
      ...
      value = EditorGUI.IntPopup(rect, Styles.allowMSAA, value,
                                 Styles.displayedOptions, Styles.optionValues);
    ...
    public void DrawDynamicResolution() {
      EditorGUILayout.PropertyField(allowDynamicResolution,
                                    Styles.allowDynamicResolution);
      if ((allowDynamicResolution.boolValue ||
           allowDynamicResolution.hasMultipleDifferentValues) &&
          !PlayerSettings.enableFrameTimingStats)
        EditorGUILayout.HelpBox(
            "It is recommended to enable Frame Timing Statistics under Rendering Player Settings when using dynamic resolution cameras.",
            MessageType.Warning, true);
    }
    ...
    public void DrawMultiDisplay() {
      if (ModuleManager.ShouldShowMultiDisplayOption()) {
        ...
        EditorGUILayout.IntPopup(
            ...
            DisplayUtility.GetDisplayIndices(),
            EditorGUIUtility.TempContent("Target Display"));
    ...
    public void DrawTargetEye() {
      EditorGUILayout.IntPopup(targetEye, kTargetEyes, kTargetEyeValues,
                               EditorGUIUtility.TempContent("Target Eye"));
    }

    public static void DrawCameraWarnings(Camera camera) {
      string[] warnings = camera.GetCameraBufferWarnings();
      if (warnings.Length > 0)
        EditorGUILayout.HelpBox(String.Join("\n\n", warnings), MessageType.Info,
                                true);
    }
  }
  ...
  protected Camera previewCamera {
    get {
      if (m_PreviewCamera == null)
        m_PreviewCamera = EditorUtility
                              .CreateGameObjectWithHideFlags(
                                  "Preview Camera", HideFlags.HideAndDontSave,
  ...
  static List<XRDisplaySubsystemDescriptor> displayDescriptors =
  ...
  Dictionary<Camera, OverlayWindow> m_OverlayWindows =
      new Dictionary<Camera, OverlayWindow>();

  public void OnEnable() {
    settings.OnEnable();

    var c = (Camera) target;
    m_ShowBGColorOptions.value = !clearFlagsHasMultipleValues &&
                                 (c.clearFlags == CameraClearFlags.SolidColor ||
                                  c.clearFlags == CameraClearFlags.Skybox);
    m_ShowOrthoOptions.value = c.orthographic;
    m_ShowTargetEyeOption.value =
        targetEyeValue != (int) StereoTargetEyeMask.Both ||
    ...
    foreach (var camera in targets) {
      m_OverlayWindows[(Camera) camera] = new OverlayWindow(
          new GUIContent(camera.name), OnOverlayGUI,
  ...
  private void DepthTextureModeGUI() {
    // Camera's depth texture mode is not serialized data, so can't get to it
    // through serialized property (hence no multi-edit).
    if (targets.Length != 1)
      return;
  ...
  private void CommandBufferGUI() {
    ...
    foreach (CameraEvent ce in (CameraEvent[])
                 System.Enum.GetValues(typeof(CameraEvent))) {
      ...
      foreach (CommandBuffer cb in cbs) {
        using(new GUILayout.HorizontalScope()) {
          ...
          GUI.Label(rowRect,
                    string.Format("{0}: {1} ({2})", ce, cb.name,
                                  EditorUtility.FormatBytes(cb.sizeInBytes)),
                    EditorStyles.miniLabel);
          // and a button to remove it
          if (GUI.Button(minusRect, Styles.iconRemove,
  ...
  public override void OnInspectorGUI() {
    ...
    var c = (Camera) target;
    m_ShowBGColorOptions.target =
        !clearFlagsHasMultipleValues &&
        (c.clearFlags == CameraClearFlags.SolidColor ||
         c.clearFlags == CameraClearFlags.Skybox);
    m_ShowOrthoOptions.target =
        !orthographicHasMultipleValues && c.orthographic;

    bool displaySubsystemPresent = displayDescriptors.Count > 0;
    m_ShowTargetEyeOption.target =
        targetEyeValue != (int) StereoTargetEyeMask.Both ||
        PlayerSettings.virtualRealitySupported || displaySubsystemPresent;
    ...
    if (m_ShowOrthoOptions.target && wantDeferredRendering)
      EditorGUILayout.HelpBox(
          "Deferred rendering does not work with Orthographic camera, will use Forward.",
          MessageType.Warning, true);
  ...
  public virtual void OnOverlayGUI(Object target, SceneView sceneView) {
    ...
    // Do not render the Camera Preview overlay if the target camera GameObject
    // is not part of the objects the SceneView is rendering
    if (!sceneView.IsGameObjectInThisSceneView(c.gameObject))
      return;

    Vector2 previewSize = c.targetTexture ? new Vector2(c.targetTexture.width,
                                                       c.targetTexture.height)
    ...
    if (Event.current.type == EventType.Repaint) {
      Graphics.DrawTexture(cameraRect, Texture2D.whiteTexture,
                           new Rect(0, 0, 1, 1), 0, 0, 0, 0, Color.black);
    }
    ...
    if (Event.current.type == EventType.Repaint) {
      ...
      // make sure the preview camera is rendering the same stage as the
      // SceneView is
      ...
      var previewTexture = GetPreviewTextureWithSize((int) cameraRect.width,
                                                     (int) cameraRect.height);
      previewTexture.antiAliasing = Mathf.Max(1, QualitySettings.antiAliasing);
      previewCamera.targetTexture = previewTexture;
      previewCamera.pixelRect =
          new Rect(0, 0, cameraRect.width, cameraRect.height);

      Handles.EmitGUIGeometryForCamera(c, previewCamera);

      if (c.usePhysicalProperties) {
        // when sensor size is reduced, the previous frame is still visible
        // behing so we need to clear the texture before rendering.
      ...
      Graphics.DrawTexture(cameraRect, previewTexture, new Rect(0, 0, 1, 1), 0,
                           0, 0, 0, GUI.color,
  ...
  private RenderTexture GetPreviewTextureWithSize(int width, int height) {
    if (m_PreviewTexture == null || m_PreviewTexture.width != width ||
        m_PreviewTexture.height != height ||
        m_QualitySettingsAntiAliasing != QualitySettings.antiAliasing) {
      m_PreviewTexture = new RenderTexture(
          width, height, 24, SystemInfo.GetGraphicsFormat(DefaultFormat.LDR));
      m_QualitySettingsAntiAliasing = QualitySettings.antiAliasing;
    }
  ...
  public virtual void OnSceneGUI() {
    ...
    Vector2 currentMainPlayModeViewTargetSize =
        PlayModeView.GetMainPlayModeViewTargetSize();
    if (s_PreviousMainPlayModeViewTargetSize !=
        currentMainPlayModeViewTargetSize) {
      // a gameView size change can affect horizontal FOV, refresh the inspector
      // when that happens.
      Repaint();
