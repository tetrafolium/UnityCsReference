...
internal class WheelColliderEditor : Editor {
  ...
  public void OnEnable() {
    // Wheel Collider does not serialize Collider properties, so we don't use
    // base OnEnable like other collider types
    m_Center = serializedObject.FindProperty("m_Center");
    m_Radius = serializedObject.FindProperty("m_Radius");
    m_SuspensionDistance =
        serializedObject.FindProperty("m_SuspensionDistance");
    ...
    m_ForceAppPointDistance =
        serializedObject.FindProperty("m_ForceAppPointDistance");
