...
internal class AimConstraintEditor : ConstraintEditorBase {
  ...
  private class Styles : ConstraintStyleBase {
    GUIContent m_RotationAtRest = EditorGUIUtility.TrTextContent(
        "Rotation At Rest",
        "The orientation of the constrained object when the weights of the sources add up to zero or when all the rotation axes are disabled.");
    GUIContent m_RotationOffset = EditorGUIUtility.TrTextContent(
        "Rotation Offset", "The offset from the constrained orientation.");

    GUIContent m_RotationAxes = EditorGUIUtility.TrTextContent(
        "Freeze Rotation Axes",
        "The axes along which the constraint is applied.");

    GUIContent m_AimVector = EditorGUIUtility.TrTextContent(
        "Aim Vector",
        "Specifies which axis of the constrained object should aim at the target.");
    GUIContent m_UpVector = EditorGUIUtility.TrTextContent(
        "Up Vector",
        "Specifies the direction of the up vector in local space.");
    GUIContent m_WorldUpVector = EditorGUIUtility.TrTextContent(
        "World Up Vector", "Specifies the direction of the global up vector.");
    GUIContent m_WorldUpObject = EditorGUIUtility.TrTextContent(
        "World Up Object",
        "The reference object when the World Up Type is either Object Up or Object Rotation Up.");
    GUIContent m_WorldUpType = EditorGUIUtility.TrTextContent(
        "World Up Type",
        "Specifies how the world up vector should be computed.");
    GUIContent[] m_WorldUpTypes = {
        EditorGUIUtility.TrTextContent(
            "Scene Up", "Use the Y axis as the world up vector."),
        EditorGUIUtility.TrTextContent(
            "Object Up",
            "Use a vector that points to the reference object as the world up vector."),
        EditorGUIUtility.TrTextContent(
            "Object Rotation Up",
            "Use a vector defined in the reference object's local space as the world up vector."),
        EditorGUIUtility.TrTextContent("Vector",
                                       "The world up vector is user defined."),
        EditorGUIUtility.TrTextContent("None",
                                       "The world up vector is ignored.")};
  ...
  internal override void OnValueAtRestChanged() {
    foreach (var t in targets)
      (t as AimConstraint)
          .transform.SetLocalEulerAngles(atRest.vector3Value,
  ...
  internal override void ShowCustomProperties() {
    ...
    EditorGUILayout.Popup(m_WorldUpType, s_Style.WorldUpTypes,
                          s_Style.WorldUpType);

    var worldUpType = (AimConstraint.WorldUpType) m_WorldUpType.intValue;
    using(new EditorGUI.DisabledGroupScope(
        worldUpType != AimConstraint.WorldUpType.ObjectRotationUp &&
        worldUpType != AimConstraint.WorldUpType.Vector)) {
      EditorGUILayout.PropertyField(m_WorldUpVector, s_Style.WorldUpVector);
    }

    using(new EditorGUI.DisabledGroupScope(
        worldUpType != AimConstraint.WorldUpType.ObjectUp &&
        worldUpType != AimConstraint.WorldUpType.ObjectRotationUp)) {
  ...
  internal override void ShowFreezeAxesControl() {
    ...
    EditorGUI.MultiPropertyField(
        drawRect, s_Style.Axes,
        serializedObject.FindProperty("m_AffectRotationX"), s_Style.FreezeAxes);
  }
