...
private void BuildTemplateList() {
	...
	for (int i = 0; i < s_TemplateGUIThumbnails.Length; i++) {
		s_TemplateGUIThumbnails[i] = s_Templates [i]
		                             .ToGUIContent(defaultIcon);
	}
}
...
private WebTemplate Load(string path) {
	...
	if (splitPath.Length > 3 && splitPath [splitPath.Length - 3]
	    .Equals("Assets")) {
...
public void SelectionUI(SerializedProperty templateProp) {
	...
	if (TemplateGUIThumbnails.Length < 1) {
		...
		int numCols = Mathf.Min(
			(int) Mathf.Max((Screen.width - kWebTemplateGridPadding * 2.0f) /
			                kThumbnailSize,
			                1.0f),
			TemplateGUIThumbnails.Length);
		int numRows =
			Mathf.Max((int) Mathf.Ceil((float) TemplateGUIThumbnails.Length /
			                           (float) numCols),
			          1);

		bool wasChanged = GUI.changed;

		templateProp.stringValue =
			Templates
			[ThumbnailList(GUILayoutUtility.GetRect(
					       numCols * kThumbnailSize,
					       numRows * (kThumbnailSize + kThumbnailLabelHeight),
					       GUILayout.ExpandWidth(false)),
			               GetTemplateIndex(templateProp.stringValue),
			               TemplateGUIThumbnails, numCols)]
			.ToString();
		...
		if (templateChanged) {
			...
			PlayerSettings.templateCustomKeys =
				Templates[GetTemplateIndex(templateProp.stringValue)].CustomKeys;
			templateProp.serializedObject.Update();
		}
	}
}

static int ThumbnailList(Rect rect, int selection, GUIContent[] thumbnails,
                         int maxRowItems) {
	for (int y = 0, i = 0; i < thumbnails.Length; y++) {
		for (int x = 0; x < maxRowItems && i < thumbnails.Length; x++, i++) {
			if (ThumbnailListItem(
				    new Rect(rect.x + x * kThumbnailSize,
				             rect.y + y * (kThumbnailSize + kThumbnailLabelHeight),
				             kThumbnailSize,
				             (kThumbnailSize + kThumbnailLabelHeight)),
...
static bool ThumbnailListItem(Rect rect, bool selected, GUIContent content) {
	...
	case EventType.Repaint:
		Rect thumbRect = new Rect(
			rect.x + kThumbnailPadding, rect.y + kThumbnailPadding,
			rect.width - kThumbnailPadding * 2.0f,
			rect.height - kThumbnailLabelHeight - kThumbnailPadding * 2.0f);
		s_Styles.thumbnail.Draw(thumbRect, content.image, false, false, selected,
		                        selected);
		s_Styles.thumbnailLabel.Draw(
			new Rect(rect.x, rect.y + rect.height - kThumbnailLabelHeight,
			         rect.width, kThumbnailLabelHeight),
			content.text, false, false, selected, selected);
		break;
	}
	return selected;
}
