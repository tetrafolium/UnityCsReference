...
// If the user changes an asset(delete, re-import etc) then we should cancel
// the splash screen to avoid using invalid data. (case 857060)
...
static AssetDeleteResult OnWillDeleteAsset(string asset,
...
public GUIContent backgroundColor = EditorGUIUtility.TrTextContent(
	"Background Color",
	"Background color when no background image is used.");
public GUIContent backgroundImage = EditorGUIUtility.TrTextContent(
	"Background Image",
	"Image to be used in landscape and portrait (when portrait image is not set).");
public GUIContent backgroundPortraitImage = EditorGUIUtility.TrTextContent(
	"Alternate Portrait Image*",
	"Optional image to be used in portrait mode.");
public GUIContent backgroundTitle =
	EditorGUIUtility.TrTextContent("Background*");
public GUIContent backgroundZoom =
	EditorGUIUtility.TrTextContent("Background Zoom");
public GUIContent blurBackground =
	EditorGUIUtility.TrTextContent("Blur Background Image");
public GUIContent cancelPreviewSplash =
	EditorGUIUtility.TrTextContent("Cancel Preview");
public GUIContent configDialogBanner =
	EditorGUIUtility.TrTextContent("Application Config Dialog Banner");
public GUIContent configDialogBannerDeprecationWarning =
	EditorGUIUtility.TrTextContent(
		"Application Config Dialog Banner is deprecated and will be removed in future versions.");
public GUIContent drawMode = EditorGUIUtility.TrTextContent("Draw Mode");
public GUIContent logoDuration = EditorGUIUtility.TrTextContent(
	"Logo Duration", "The time the logo will be shown for.");
...
public GUIContent overlayOpacity = EditorGUIUtility.TrTextContent(
	"Overlay Opacity",
	"Overlay strength applied to improve logo visibility.");
public GUIContent previewSplash = EditorGUIUtility.TrTextContent(
	"Preview", "Preview the splash screen in the game view.");
public GUIContent showLogo =
	EditorGUIUtility.TrTextContent("Show Unity Logo");
public GUIContent showSplash =
	EditorGUIUtility.TrTextContent("Show Splash Screen");
public GUIContent splashStyle =
	EditorGUIUtility.TrTextContent("Splash Style");
public GUIContent splashTitle =
	EditorGUIUtility.TrTextContent("Splash Screen");
public GUIContent title = EditorGUIUtility.TrTextContent("Splash Image");
public GUIContent vrSplashScreen =
	EditorGUIUtility.TrTextContent("Virtual Reality Splash Image");
}
...
public void OnEnable() {
	...
	m_ShowUnitySplashScreen =
		m_Owner.FindPropertyAssert("m_ShowUnitySplashScreen");
	m_SplashScreenAnimation =
		m_Owner.FindPropertyAssert("m_SplashScreenAnimation");
	m_SplashScreenBackgroundAnimationZoom =
		m_Owner.FindPropertyAssert("m_SplashScreenBackgroundAnimationZoom");
	m_SplashScreenBackgroundColor =
		m_Owner.FindPropertyAssert("m_SplashScreenBackgroundColor");
	m_SplashScreenBackgroundLandscape =
		m_Owner.FindPropertyAssert("splashScreenBackgroundSourceLandscape");
	m_SplashScreenBackgroundPortrait =
		m_Owner.FindPropertyAssert("splashScreenBackgroundSourcePortrait");
	m_SplashScreenBlurBackground =
		m_Owner.FindPropertyAssert("blurSplashScreenBackground");
	m_SplashScreenDrawMode =
		m_Owner.FindPropertyAssert("m_SplashScreenDrawMode");
	m_SplashScreenLogoAnimationZoom =
		m_Owner.FindPropertyAssert("m_SplashScreenLogoAnimationZoom");
	m_SplashScreenLogos = m_Owner.FindPropertyAssert("m_SplashScreenLogos");
	m_SplashScreenLogoStyle =
		m_Owner.FindPropertyAssert("m_SplashScreenLogoStyle");
	m_SplashScreenOverlayOpacity =
		m_Owner.FindPropertyAssert("m_SplashScreenOverlayOpacity");
	m_VirtualRealitySplashScreen =
		m_Owner.FindPropertyAssert("m_VirtualRealitySplashScreen");

	m_LogoList = new ReorderableList(
		m_Owner.serializedObject, m_SplashScreenLogos, true, true, true, true);
	...
	// Set up animations
	m_ShowAnimationControlsAnimator.value =
		m_SplashScreenAnimation.intValue ==
		(int) PlayerSettings.SplashScreen.AnimationMode.Custom;
	m_ShowBackgroundColorAnimator.value =
		m_SplashScreenBackgroundLandscape.objectReferenceValue == null;
	m_ShowLogoControlsAnimator.value = m_ShowUnitySplashLogo.boolValue;
	SetValueChangeListeners(m_Owner.Repaint);

	if (s_UnityLogo == null)
		s_UnityLogo =
			Resources.GetBuiltinResource<Sprite>("UnitySplash-cube.png");
}
...
private void DrawElementUnityLogo(Rect rect, int index, bool isActive,
                                  bool isFocused) {
	...
	// Unity logo
	float logoWidth =
		Mathf.Clamp(rect.width - k_LogoListPropertyMinWidth,
		            k_LogoListUnityLogoMinWidth, k_LogoListUnityLogoMaxWidth);
	var logoRect = new Rect(rect.x, rect.y, logoWidth, rect.height);
	var oldCol = GUI.color;
	GUI.color = (m_SplashScreenLogoStyle.intValue ==
	             (int) PlayerSettings.SplashScreen.UnityLogoStyle
	             .DarkOnLight ? Color.black
	         : Color.white);
	...
	var propertyRect =
		new Rect(rect.x + logoWidth,
		         rect.y + EditorGUIUtility.standardVerticalSpacing +
		         EditorGUIUtility.singleLineHeight,
		         rect.width - logoWidth, EditorGUIUtility.singleLineHeight);
	EditorGUI.BeginChangeCheck();
	var durationLabel =
		EditorGUI.BeginProperty(propertyRect, k_Texts.logoDuration, duration);
	var newDurationVal =
		EditorGUI.Slider(propertyRect, durationLabel, duration.floatValue,
		                 k_MinLogoTime, k_MaxLogoTime);
...
private void DrawLogoListElementCallback(Rect rect, int index, bool isActive,
                                         bool isFocused) {
	...
	// Logo field
	float unityLogoWidth =
		Mathf.Clamp(rect.width - k_LogoListPropertyMinWidth,
		            k_LogoListUnityLogoMinWidth, k_LogoListUnityLogoMaxWidth);
	var logoRect = new Rect(
		rect.x, rect.y + (rect.height - k_LogoListLogoFieldHeight) / 2.0f,
		k_LogoListUnityLogoMinWidth, k_LogoListLogoFieldHeight);
	EditorGUI.BeginChangeCheck();
	var value = EditorGUI.ObjectField(logoRect, GUIContent.none,
	                                  (Sprite) logo.objectReferenceValue,
	                                  typeof(Sprite), false);
	if (EditorGUI.EndChangeCheck())
	...
	var propertyRect = new Rect(
		rect.x + unityLogoWidth,
		rect.y + EditorGUIUtility.standardVerticalSpacing,
		rect.width - unityLogoWidth, EditorGUIUtility.singleLineHeight);
	var duration = element.FindPropertyRelative("duration");
	...
	EditorGUIUtility.labelWidth = k_LogoListPropertyLabelWidth;
	var newDurationVal =
		EditorGUI.Slider(propertyRect, k_Texts.logoDuration,
		                 duration.floatValue, k_MinLogoTime, k_MaxLogoTime);
	EditorGUIUtility.labelWidth = oldLabelWidth;
	if (EditorGUI.EndChangeCheck())
		duration.floatValue = newDurationVal;
...
private void DrawLogoListFooterCallback(Rect rect) {
	...
	EditorGUI.LabelField(rect, "Splash Screen Duration: " + totalDuration,
...
private void OnLogoListAddCallback(ReorderableList list) {
	...
	m_SplashScreenLogos.InsertArrayElementAtIndex(
		m_SplashScreenLogos.arraySize);
	var element = m_SplashScreenLogos.GetArrayElementAtIndex(index);
...
private static bool OnLogoListCanRemoveCallback(ReorderableList list) {
	...
	var logo =
		(Sprite) element.FindPropertyRelative("logo").objectReferenceValue;
...
private static void
ObjectReferencePropertyField<T>(SerializedProperty property,
                                GUIContent label) where T
: UnityEngine.Object {
	EditorGUI.BeginChangeCheck();
	Rect r =
		EditorGUILayout.GetControlRect(true, 64, EditorStyles.objectFieldThumb);
	label = EditorGUI.BeginProperty(r, label, property);
	var value = EditorGUI.ObjectField(
		r, label, (T) property.objectReferenceValue, typeof(T), false);
	if (EditorGUI.EndChangeCheck()) {
...
public void SplashSectionGUI(BuildPlatform platform,
                             BuildTargetGroup targetGroup,
                             ISettingEditorExtension settingsExtension,
                             int sectionIndex = 2) {
	if (m_Owner.BeginSettingsBox(sectionIndex, k_Texts.title)) {
		ObjectReferencePropertyField<Texture2D>(m_VirtualRealitySplashScreen,
		                                        k_Texts.vrSplashScreen);

		if (TargetSupportsOptionalBuiltinSplashScreen(targetGroup,
		                                              settingsExtension))
...
private void BuiltinCustomSplashScreenGUI() {
	...
	using (new EditorGUI.DisabledScope(!licenseAllowsDisabling)) {
		EditorGUILayout.PropertyField(m_ShowUnitySplashScreen,
		                              k_Texts.showSplash);
		if (!m_ShowUnitySplashScreen.boolValue)
			return;
	}
	...
	GUIContent buttonLabel =
		SplashScreen.isFinished ? k_Texts.previewSplash
	: k_Texts.cancelPreviewSplash;
	Rect previewButtonRect = GUILayoutUtility.GetRect(buttonLabel, "button");
	previewButtonRect =
		EditorGUI.PrefixLabel(previewButtonRect, new GUIContent(" "));
	...
	m_ShowAnimationControlsAnimator.target =
		m_SplashScreenAnimation.intValue ==
		(int) PlayerSettings.SplashScreen.AnimationMode.Custom;

	if (EditorGUILayout.BeginFadeGroup(m_ShowAnimationControlsAnimator.faded)) {
		EditorGUI.indentLevel++;
		EditorGUILayout.Slider(m_SplashScreenLogoAnimationZoom, 0.0f, 1.0f,
		                       k_Texts.logoZoom);
		EditorGUILayout.Slider(m_SplashScreenBackgroundAnimationZoom, 0.0f, 1.0f,
		                       k_Texts.backgroundZoom);
		EditorGUI.indentLevel--;
	}
	...
	using (new EditorGUI.DisabledScope(!Application.HasProLicense())) {
		...
		if (EditorGUI.EndChangeCheck()) {
			...
			else if (m_SplashScreenDrawMode.intValue ==
			         (int) PlayerSettings.SplashScreen.DrawMode.AllSequential)
	...
	if (EditorGUILayout.BeginFadeGroup(m_ShowLogoControlsAnimator.faded)) {
		...
		if (oldDrawmode != m_SplashScreenDrawMode.intValue) {
			if (m_SplashScreenDrawMode.intValue ==
			    (int) PlayerSettings.SplashScreen.DrawMode.UnityLogoBelow)
				RemoveUnityLogoFromLogosList();
			else
				AddUnityLogoToLogosList();
		}
	...
	using (var vertical = new EditorGUILayout.VerticalScope()) using (
			new EditorGUI.PropertyScope(vertical.rect, GUIContent.none,
			                            m_SplashScreenLogos)) {
	...
	EditorGUILayout.Slider(m_SplashScreenOverlayOpacity,
	                       Application.HasProLicense()
	                       ? k_MinProEditionOverlayOpacity
	                       : k_MinPersonalEditionOverlayOpacity,
	                       1.0f, k_Texts.overlayOpacity);
	m_ShowBackgroundColorAnimator.target =
		m_SplashScreenBackgroundLandscape.objectReferenceValue == null;
	if (EditorGUILayout.BeginFadeGroup(m_ShowBackgroundColorAnimator.faded))
		EditorGUILayout.PropertyField(m_SplashScreenBackgroundColor,
		                              k_Texts.backgroundColor);
	EditorGUILayout.EndFadeGroup();

	EditorGUILayout.PropertyField(m_SplashScreenBlurBackground,
	                              k_Texts.blurBackground);
	EditorGUI.BeginChangeCheck();
	ObjectReferencePropertyField<Sprite>(m_SplashScreenBackgroundLandscape,
	                                     k_Texts.backgroundImage);
	if (EditorGUI.EndChangeCheck() &&
	    m_SplashScreenBackgroundLandscape.objectReferenceValue == null)
	...
	using (new EditorGUI.DisabledScope(
		       m_SplashScreenBackgroundLandscape.objectReferenceValue == null)) {
		ObjectReferencePropertyField<Sprite>(m_SplashScreenBackgroundPortrait,
...
void PollSplashState() {
	...
	// When the splash screen is playing we need to keep track so that we can
	// update the preview button when it has finished.
	if (SplashScreen.isFinished) {
