...
public override void OnInspectorGUI() {
	...
	// One-Way.
	m_ShowOneWayRollout.target =
		EditorGUILayout.Foldout(m_ShowOneWayRollout.target, "One Way", true);
	...
	// Sides.
	m_ShowSidesRollout.target =
		EditorGUILayout.Foldout(m_ShowSidesRollout.target, "Sides", true);
...
private static void DrawSurfaceArc(PlatformEffector2D effector) {
	...
	var localUp = effector.transform
	              .TransformVector(new Vector3(Mathf.Sin(rotation),
	                                           Mathf.Cos(rotation), 0.0f))
	              .normalized;

	// If the transform has created a degenerate local then we cannot draw the
	// gizmo!
	...
	var fromAngle =
		new Vector3(Mathf.Sin(surfaceAngle - halfSurfaceArcRadians),
		            Mathf.Cos(surfaceAngle - halfSurfaceArcRadians), 0.0f);
	var toAngle =
		new Vector3(Mathf.Sin(surfaceAngle + halfSurfaceArcRadians),
		            Mathf.Cos(surfaceAngle + halfSurfaceArcRadians), 0.0f);

	// Fetch all the effector-collider bounds.
	foreach (var collider in effector.gameObject.GetComponents<Collider2D>()
	         .Where(collider =>
	                collider.enabled && collider.usedByEffector)) {
		var center = collider.bounds.center;
		var arcRadius = HandleUtility.GetHandleSize(center);

		// arc background
		Handles.color = new Color(0, 1, 1, 0.07f);
		Handles.DrawSolidArc(center, Vector3.back, fromAngle, surfaceArc,
		                     arcRadius);

		// arc frame
		Handles.color = new Color(0, 1, 1, 0.7f);
		Handles.DrawWireArc(center, Vector3.back, fromAngle, surfaceArc,
		                    arcRadius);
...
private static void DrawSideArc(PlatformEffector2D effector) {
	...
	var localSide = effector.transform
	                .TransformVector(new Vector3(Mathf.Sin(rotation),
	                                             Mathf.Cos(rotation), 0.0f))
	                .normalized;
	...
	var fromAngleLeft =
		new Vector3(Mathf.Sin(sideAngleLeft - halfSideArcRadians),
		            Mathf.Cos(sideAngleLeft - halfSideArcRadians), 0.0f);
	var toAngleLeft =
		new Vector3(Mathf.Sin(sideAngleLeft + halfSideArcRadians),
		            Mathf.Cos(sideAngleLeft + halfSideArcRadians), 0.0f);
	var fromAngleRight =
		new Vector3(Mathf.Sin(sideAngleRight - halfSideArcRadians),
		            Mathf.Cos(sideAngleRight - halfSideArcRadians), 0.0f);
	var toAngleRight =
		new Vector3(Mathf.Sin(sideAngleRight + halfSideArcRadians),
		            Mathf.Cos(sideAngleRight + halfSideArcRadians), 0.0f);
	...
	foreach (var collider in effector.gameObject.GetComponents<Collider2D>()
		...
		Handles.DrawSolidArc(center, Vector3.back, fromAngleLeft, sideArc,
		                     arcRadius);
		Handles.DrawSolidArc(center, Vector3.back, fromAngleRight, sideArc,
		                     arcRadius);

		// arc frame
		Handles.color = new Color(0, 1, 0.7f, 0.7f);
		Handles.DrawWireArc(center, Vector3.back, fromAngleLeft, sideArc,
		                    arcRadius);
		Handles.DrawWireArc(center, Vector3.back, fromAngleRight, sideArc,
		                    arcRadius);
