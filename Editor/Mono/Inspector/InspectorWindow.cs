...
internal class InspectorWindow : PropertyEditor,
                                 IPropertyView,
                                 IHasCustomMenu {
  static readonly List<InspectorWindow> m_AllInspectors =
      new List<InspectorWindow>();
  static bool s_AllOptimizedGUIBlocksNeedsRebuild;

  [SerializeField] EditorGUIUtility
      .EditorLockTrackerWithActiveEditorTracker m_LockTracker =
      new EditorGUIUtility.EditorLockTrackerWithActiveEditorTracker();
  ...
  public bool isLocked {
    get {
      // this makes sure the getter for InspectorWindow.tracker gets called and
      // creates an ActiveEditorTracker if needed
  ...
  internal void Awake() { AddInspectorWindow(this); }

  protected override void OnDestroy() {
    if (m_PreviewWindow != null)
      m_PreviewWindow.Close();
    if (m_Tracker != null &&
        !m_Tracker.Equals(ActiveEditorTracker.sharedTracker))
      m_Tracker.Destroy();
  ...
  private void OnProjectWasLoaded() {
    // EditorApplication.projectWasLoaded, which calls this, fires after
    // OnEnabled therefore the logic in OnEnabled already tried to de-serialize
    // the locked objects, including those it only had InstanceIDs of This needs
    // to get fixed here as the InstanceIDs have been reshuffled with the new
    // session and could resolving to random Objects
    if (m_InstanceIDsLockedBeforeSerialization.Count > 0) {
      // Game objects will have new instanceIDs in a new Unity session, so take
      // out all objects that where reconstructed from InstanceIDs
      for (int i = m_InstanceIDsLockedBeforeSerialization.Count - 1; i >= 0;
           i--) {
        for (int j = m_ObjectsLockedBeforeSerialization.Count - 1; j >= 0;
             j--) {
          if (m_ObjectsLockedBeforeSerialization[j] == null ||
              m_ObjectsLockedBeforeSerialization [j]
                      .GetInstanceID() ==
                  m_InstanceIDsLockedBeforeSerialization[i]) {
  ...
  public override void AddItemsToMenu(GenericMenu menu) {
    menu.AddItem(EditorGUIUtility.TrTextContent("Normal"),
                 m_InspectorMode == InspectorMode.Normal, SetNormal);
    menu.AddItem(EditorGUIUtility.TrTextContent("Debug"),
                 m_InspectorMode == InspectorMode.Debug, SetDebug);

    if (Unsupported.IsDeveloperMode()) {
      menu.AddItem(EditorGUIUtility.TrTextContent("Debug-Internal"),
                   m_InspectorMode == InspectorMode.DebugInternal,
                   SetDebugInternal);
      menu.AddItem(
          EditorGUIUtility.TrTextContent("Use UI Toolkit Default Inspector"),
          useUIElementsDefaultInspector, SetUseUIEDefaultInspector);
    }
  ...
  protected override void RefreshTitle() {
    ...
    if (m_InspectorMode == InspectorMode.Normal)
      titleContent =
          EditorGUIUtility.TrTextContentWithIcon("Inspector", iconName);
    else
  ...
  private void SetDebug() { inspectorMode = InspectorMode.Debug; }

  private void SetNormal() { inspectorMode = InspectorMode.Normal; }
  ...
  protected override void CreateTracker() {
    ...
    m_Tracker = sharedTrackerInUse ? new ActiveEditorTracker()
  ...
  bool sharedTrackerInUse {
    get {
      return m_AllInspectors.Any(
          i => i.m_Tracker != null &&
               i.m_Tracker.Equals(ActiveEditorTracker.sharedTracker));
    }
  }
  ...
  protected override bool CloseIfEmpty() { return false; }
  ...
  protected override void BeginRebuildContentContainers() {
    ...
    if (m_TrackerResetter == null) {
      ...
      m_TrackerResetter =
          CreateIMGUIContainer(() => {}, "activeEditorTrackerResetter");
  ...
  protected override void EndDrawPreviewAndLabels(Event evt, Rect rect,
                                                  Rect dragRect) {
    if (m_HasPreview || m_PreviewWindow != null) {
      if (EditorGUILayout.DropdownButton(Styles.menuIcon, FocusType.Passive,
                                         Styles.preOptionsButton)) {
        ...
        menu.AddItem(
            EditorGUIUtility.TrTextContent(m_PreviewResizer.GetExpanded()
                                               ...
                                               : "Restore in Inspector"),
            false, () => {
              m_PreviewResizer.SetExpanded(
                  position, k_InspectorPreviewMinTotalHeight,
    ...
    if (evt.type == EventType.MouseUp && evt.button == 1 &&
        rect.Contains(evt.mousePosition) && m_PreviewWindow == null)
      DetachPreview();
  }
  ...
  private void PrepareLockedObjectsForSerialization() {
    ...
    if (m_Tracker != null && m_Tracker.isLocked) {
      m_Tracker.GetObjectsLockedByThisTracker(
          m_ObjectsLockedBeforeSerialization);

      // take out non persistent and track them in a list of instance IDs,
      // because they wouldn't survive serialization as Objects
      for (int i = m_ObjectsLockedBeforeSerialization.Count - 1; i >= 0; i--) {
        if (!EditorUtility.IsPersistent(
                m_ObjectsLockedBeforeSerialization[i])) {
          if (m_ObjectsLockedBeforeSerialization[i] != null)
            m_InstanceIDsLockedBeforeSerialization.Add(
                m_ObjectsLockedBeforeSerialization [i]
                    .GetInstanceID());
  ...
  private void RestoreLockStateFromSerializedData() {
    ...
    if (m_InstanceIDsLockedBeforeSerialization.Count > 0) {
      for (int i = 0; i < m_InstanceIDsLockedBeforeSerialization.Count; i++) {
        Object instance = EditorUtility.InstanceIDToObject(
            m_InstanceIDsLockedBeforeSerialization[i]);
    ...
    // set the tracker to the serialized list. if it contains nulls or is empty,
    // the tracker won't lock this fixes case 775007
    m_Tracker.SetObjectsLockedByThisTracker(m_ObjectsLockedBeforeSerialization);
    // since this method likely got called during OnEnable, and rebuilding the
    // tracker could call OnDisable on all Editors, some of which might not have
    // gotten their enable yet, the rebuilding needs to happen delayed in
  ...
  internal override Object GetInspectedObject() {
    Editor editor = InspectorWindowUtils.GetFirstNonImportInspectorEditor(
