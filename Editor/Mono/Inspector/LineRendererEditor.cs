...
internal class LineRendererInspector : RendererEditorBase {
  class Styles {
    public static readonly GUIContent alignment = EditorGUIUtility.TrTextContent(
        "Alignment",
        "Lines can rotate to face their transform component or the camera. When using Local mode, lines face the XY plane of the Transform.");
    public static readonly GUIContent colorGradient =
        EditorGUIUtility.TrTextContent(
            "Color", "The gradient describing the color along the line.");
    public static readonly string disabledEditMessage = L10n.Tr(
        "Editing is only available when editing a single LineRenderer in a scene.");
    public static readonly GUIContent inputMode = EditorGUIUtility.TrTextContent(
        "Input",
        "Use mouse position or physics raycast to determine where to create points.");
    public static readonly GUIContent layerMask =
        EditorGUIUtility.TrTextContent(
            "Layer Mask", "The layer mask to use when performing raycasts.");
    public static readonly GUIContent normalOffset = EditorGUIUtility.TrTextContent(
        "Offset",
        "The offset applied to created points either from the scene camera or raycast normal, when using physics.");
    public static readonly GUIContent numCapVertices =
        EditorGUIUtility.TrTextContent("End Cap Vertices",
                                       "How many vertices to add at each end.");
    public static readonly GUIContent numCornerVertices =
        EditorGUIUtility.TrTextContent(
            "Corner Vertices", "How many vertices to add for each corner.");
    public static readonly GUIContent pointSeparation =
        EditorGUIUtility.TrTextContent(
            "Min Vertex Distance",
            "When dragging the mouse, a new point will be created after the distance has been exceeded.");
    public static readonly GUIContent positions =
        EditorGUIUtility.TrTextContent("Positions");
    public static readonly GUIContent propertyMenuContent =
        EditorGUIUtility.TrTextContent("Delete Selected Array Elements");
    public static readonly GUIContent showWireframe =
        EditorGUIUtility.TrTextContent(
            "Show Wireframe", "Show the wireframe visualizing the line.");
    public static readonly GUIContent simplify = EditorGUIUtility.TrTextContent(
        "Simplify",
        "Generates a simplified version of the original line by removing points that fall within the specified tolerance.");
    public static readonly GUIContent simplifyPreview =
        EditorGUIUtility.TrTextContent(
            "Simplify Preview",
            "Show a preview of the simplified version of the line.");
    public static readonly GUIContent subdivide =
        EditorGUIUtility.TrTextContent(
            "Subdivide Selected",
            "Inserts a new point in between selected adjacent points.");
    public static readonly GUIContent textureMode =
        EditorGUIUtility.TrTextContent(
            "Texture Mode", "Should the U coordinate be stretched or tiled?");
    public static readonly GUIContent tolerance = EditorGUIUtility.TrTextContent(
        "Tolerance",
        "Used to evaluate which points should be removed from the line. A higher value results in a simpler line (fewer points). A value of 0 results in the exact same line with little to no reduction.");
    public static readonly GUIStyle richTextMiniLabel =
    ...
    public static readonly GUIContent generateLightingData =
        EditorGUIUtility.TrTextContent(
            "Generate Lighting Data",
            "Toggle generation of normal and tangent data, for use in lit shaders.");

    public static readonly GUIContent[] toolContents = {
        EditorGUIUtility.IconContent("EditCollider",
                                     "|Edit Points in Scene View"),
        EditorGUIUtility.IconContent("Toolbar Plus",
                                     "|Create Points in Scene View.")};

    public static readonly EditMode.SceneViewEditMode[] sceneViewEditModes =
        new[]{EditMode.SceneViewEditMode.LineRendererEdit,
              EditMode.SceneViewEditMode.LineRendererCreate};

    public static readonly string baseSceneEditingToolText =
        "<color=grey>Line Renderer Scene Editing Mode:</color> ";
  ...
  public static float simplifyTolerance {
    get {
      return EditorPrefs.GetFloat("LineRendererInspectorSimplifyTolerance",
                                  1.0f);
    }
    set {
      EditorPrefs.SetFloat("LineRendererInspectorSimplifyTolerance",
                           value < 0 ? 0 : value);
    }
  }
  ...
  public static bool showSimplifyPreview {
    get {
      return EditorPrefs.GetBool("LineRendererEditorShowSimplifyPreview",
                                 false);
    }
  ...
  private static bool
  IsLineRendererEditMode(EditMode.SceneViewEditMode editMode) {
    return editMode == EditMode.SceneViewEditMode.LineRendererEdit ||
           editMode == EditMode.SceneViewEditMode.LineRendererCreate;
  }

  private bool sceneViewEditing {
    get {
      return IsLineRendererEditMode(EditMode.editMode) &&
  ...
  public override void OnEnable() {
    ...
    m_ColorGradient =
        serializedObject.FindProperty("m_Parameters.colorGradient");
    m_NumCornerVertices =
        serializedObject.FindProperty("m_Parameters.numCornerVertices");
    m_NumCapVertices =
        serializedObject.FindProperty("m_Parameters.numCapVertices");
    ...
    m_GenerateLightingData =
        serializedObject.FindProperty("m_Parameters.generateLightingData");
    ...
    m_ShowPositionsAnimation =
        new AnimBool(false, Repaint){value = m_Positions.isExpanded};
    EditorApplication.contextualPropertyMenu += OnPropertyContextMenu;

    // We cannot access isEditingMultipleObjects when drawing the SceneView so
    // we need to cache it here for later use.
    m_IsMultiEditing = serializedObject.isEditingMultipleObjects;
    m_IsGameObjectEditable =
        (lineRenderer.gameObject.hideFlags & HideFlags.NotEditable) == 0;
  }
  ...
  void OnPropertyContextMenu(GenericMenu menu, SerializedProperty property) {
    ...
    if (property.propertyPath.Contains("m_Positions") &&
        m_PositionsView.GetSelection().Count > 1) {
  ...
  private void DrawEditPointTools() {
    LineRendererEditor.showWireframe = GUILayout.Toggle(
        LineRendererEditor.showWireframe, Styles.showWireframe);
  ...
  private static void CreatePointTools() {
    LineRendererEditor.inputMode =
        (LineRendererEditor.InputMode) EditorGUILayout.EnumPopup(
            Styles.inputMode, LineRendererEditor.inputMode);
    if (LineRendererEditor.inputMode ==
        LineRendererEditor.InputMode.PhysicsRaycast) {
      LineRendererEditor.raycastMask = EditorGUILayout.LayerMaskField(
          LineRendererEditor.raycastMask, Styles.layerMask);
    }

    LineRendererEditor.createPointSeparation = EditorGUILayout.FloatField(
        Styles.pointSeparation, LineRendererEditor.createPointSeparation);
    LineRendererEditor.creationOffset = EditorGUILayout.FloatField(
        Styles.normalOffset, LineRendererEditor.creationOffset);
  }

  Bounds GetBounds() {
    var lineRenderer = (target as LineRenderer);
    return m_Positions.arraySize > 0
               ? lineRenderer.bounds
               : new Bounds(
                     lineRenderer.useWorldSpace ? lineRenderer.transform.position
  ...
  private void DrawToolbar() {
    ...
    if (!sceneViewEditing) {
      ...
      showSimplifyPreview =
          EditorGUILayout.Toggle(Styles.simplifyPreview, showSimplifyPreview);
      EditorGUILayout.BeginHorizontal();
      simplifyTolerance = Mathf.Max(
          0, EditorGUILayout.FloatField(Styles.tolerance, simplifyTolerance));
  ...
  private void SubdivideSelected() {
    ...
    int numInserted = 0; // As we insert new nodes, the selected indexes will
                         // become offset so we need to keep track of this.
    for (int i = 0; i < selection.Count - 1; ++i) {
      if (selection[i + 1] == selection[i] + 1) {
  ...
  private void ResetSimplifyPreview() { m_PreviewPoints = null; }
  ...
  private void DrawSimplifyPreview() {
    ...
    if (m_PreviewPoints == null && lineRenderer.positionCount > 2) {
      ...
      LineUtility.Simplify(m_PreviewPoints.ToList(), simplifyTolerance,
                           simplePoints);
  ...
  public override void OnInspectorGUI() {
    ...
    m_ShowPositionsAnimation.target = m_Positions.isExpanded =
        EditorGUILayout.Foldout(m_Positions.isExpanded, Styles.positions, true);
    if (m_ShowPositionsAnimation.faded > 0) {
      ...
      m_PositionsView.OnGUI(EditorGUILayout.GetControlRect(
          false,
          Mathf.Lerp(kPositionsViewMinHeight, m_PositionsView.totalHeight,
                     m_ShowPositionsAnimation.faded)));
    ...
    EditorGUILayout.PropertyField(m_NumCornerVertices,
                                  Styles.numCornerVertices);
    ...
    EditorGUILayout.PropertyField(m_GenerateLightingData,
                                  Styles.generateLightingData);
  ...
  private void InternalOnSceneView() {
    ...
    case EditMode.SceneViewEditMode.LineRendererEdit:
      ...
      // We need to wait for m_Positions to be updated next frame or we risk
      // calling SetSelection with invalid indexes.
