...
private OptimizedBlockState m_OptimizedBlockState =
	OptimizedBlockState.CheckOptimizedBlock;

internal override bool GetOptimizedGUIBlock(bool isDirty, bool isVisible,
                                            out float height) {
	height = -1;
	...
	while (property.NextVisible(childrenAreExpanded)) {
		...
		height += handler.GetHeight(property, null, hasPropertyDrawer) +
		          EditorGUI.kControlVerticalSpacing;
		childrenAreExpanded = !hasPropertyDrawer && property.isExpanded &&
		                      EditorGUI.HasVisibleChildFields(property);
	}
...
internal override bool OnOptimizedInspectorGUI(Rect contentRect) {
	...
	// Release keyboard focus before scrolling so that the virtual scrolling
	// focus wrong control.
	...
	using (new LocalizationGroup(behaviour)) {
		while (property.NextVisible(childrenAreExpanded)) {
			...
			childrenAreExpanded = !hasPropertyDrawer && property.isExpanded &&
			                      EditorGUI.HasVisibleChildFields(property);
			contentRect.height =
				handler.GetHeight(property, null, hasPropertyDrawer);

			if (contentRect.Overlaps(visibleRect)) {
				EditorGUI.indentLevel = property.depth;
				using (new EditorGUI.DisabledScope(
					       isInspectorModeNormal && "m_Script" == property.propertyPath))
					childrenAreExpanded &=
						handler.OnGUI(contentRect, property, null, false, visibleRect);
			}
...
private static bool CheckIfScriptLoaded(SerializedProperty scriptProperty) {
	...
	return targetScript != null &&
	       targetScript.GetScriptTypeWasJustCreatedFromComponentMenu();
}
...
ShowScriptNotLoadedWarning(SerializedProperty scriptProperty) {
	bool scriptLoaded = CheckIfScriptLoaded(scriptProperty);
	if (!scriptLoaded) {
		ShowScriptNotLoadedWarning();
	}
...
private void
ResetOptimizedBlock(OptimizedBlockState resetState =
                    OptimizedBlockState.CheckOptimizedBlock) {
	m_LastHeight = -1;
...
internal static bool ObjectIsMonoBehaviourOrScriptableObject(Object obj) {
	...
	{
		return obj.GetType() == typeof(MonoBehaviour) ||
		       obj.GetType() == typeof(ScriptableObject);
	}
