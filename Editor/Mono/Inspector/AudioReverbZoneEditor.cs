...
SerializedProperty
        m_DecayHFRatio; //  high-frequency to mid-frequency decay time ratio
SerializedProperty
        m_Reflections; // early reflections level relative to room effect
SerializedProperty m_ReflectionsDelay;   //  initial reflection delay time
SerializedProperty
        m_Reverb; //  late reverberation level relative to room effect
SerializedProperty m_ReverbDelay;   //  late reverberation delay time relative
                                    //  to initial reflection
SerializedProperty m_HFReference;   // reference high frequency (hz)
SerializedProperty m_LFReference;   // reference low frequency (hz)
SerializedProperty m_Diffusion;     //  Value that controls the echo density in
                                    //  the late reverberation decay
SerializedProperty m_Density;   // Value that controls the modal density in the
...
void OnEnable() {
	...
	m_Room = serializedObject.FindProperty(
		"m_Room"); // room effect level (at mid frequencies)
	m_RoomHF = serializedObject.FindProperty(
		"m_RoomHF"); // relative room effect level at high frequencies
	m_RoomLF = serializedObject.FindProperty(
		"m_RoomLF"); // relative room effect level at low frequencies
	m_DecayTime = serializedObject.FindProperty(
		"m_DecayTime"); // reverberation decay time at mid frequencies
	m_DecayHFRatio = serializedObject.FindProperty(
		"m_DecayHFRatio"); //  high-frequency to mid-frequency decay time ratio
	m_Reflections = serializedObject.FindProperty(
		"m_Reflections"); // early reflections level relative to room effect
	m_ReflectionsDelay = serializedObject.FindProperty(
		"m_ReflectionsDelay"); //  initial reflection delay time
	m_Reverb = serializedObject.FindProperty(
		"m_Reverb"); //  late reverberation level relative to room effect
	m_ReverbDelay = serializedObject.FindProperty(
		"m_ReverbDelay"); //  late reverberation delay time relative to initial
	                          //  reflection
	m_HFReference = serializedObject.FindProperty(
		"m_HFReference"); // reference high frequency (hz)
	m_LFReference = serializedObject.FindProperty(
		"m_LFReference"); // reference low frequency (hz)
	m_Diffusion = serializedObject.FindProperty(
		"m_Diffusion"); //  Value that controls the echo density in the late
	                        //  reverberation decay
	m_Density = serializedObject.FindProperty(
		"m_Density"); // Value that controls the modal density in the late
	                      // reverberation decay
}
...
public override void OnInspectorGUI() {
	...
	// Changing the preset changes all the other properties as well, so we need
	// to do a full refresh afterwards
	if (EditorGUI.EndChangeCheck())
		serializedObject.SetIsDifferentCacheDirty();

	using (new EditorGUI.DisabledScope(
		       m_ReverbPreset.enumValueIndex != 27 ||
		       m_ReverbPreset.hasMultipleDifferentValues)) {
