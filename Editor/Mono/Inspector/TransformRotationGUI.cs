...
internal class TransformRotationGUI {
  private GUIContent rotationContent = EditorGUIUtility.TrTextContent(
      "Rotation",
  ...
  public void RotationField() { RotationField(false); }
  ...
  public void RotationField(bool disabled) {
    ...
    if (m_OldEulerAngles.x != localEuler.x ||
    ...
    for (int i = 1; i < targets.Length; i++) {
      ...
      if (!differentRotation) {
        Vector3 otherLocalEuler =
            otherTransform.GetLocalEulerAngles(otherTransform.rotationOrder);
        differentRotation = (otherLocalEuler.x != localEuler.x ||
                             otherLocalEuler.y != localEuler.y ||
                             otherLocalEuler.z != localEuler.z);
      }

      differentRotationOrder |=
          otherTransform.rotationOrder != targetRotationOrder;
    }
    ...
    if (AnimationMode.InAnimationMode() &&
        t.rotationOrder != RotationOrder.OrderZXY) {
    ...
    if (EditorGUI.EndChangeCheck()) {
      m_EulerAngles =
          new Vector3(m_EulerFloats[0], m_EulerFloats[1], m_EulerFloats[2]);
      Undo.RecordObjects(targets,
                         "Inspector"); // Generic undo title to be consistent
                                       // with Position and Scale changes.
      foreach (Transform tr in targets) {
        tr.SetLocalEulerAngles(m_EulerAngles, tr.rotationOrder);
        if (tr.parent != null)
          tr.SendTransformChangedScale(); // force scale update, needed if tr
    ...
    if (differentRotationOrder) {
      EditorGUILayout.HelpBox(
          "Transforms have different rotation orders, keyframes saved will have the same value but not the same local rotation",
          MessageType.Warning);
    }
