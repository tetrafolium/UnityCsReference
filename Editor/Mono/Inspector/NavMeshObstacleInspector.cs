...
internal class NavMeshObstacleInspector : Editor {
  ...
  void OnEnable() {
    ...
    m_CarveOnlyStationary =
        serializedObject.FindProperty("m_CarveOnlyStationary");
  }
  ...
  public override void OnInspectorGUI() {
    ...
    if (m_Shape.enumValueIndex == 0) {
      ...
      EditorGUI.BeginChangeCheck();
      float radius =
          EditorGUILayout.FloatField("Radius", m_Extents.vector3Value.x);
      float height =
          EditorGUILayout.FloatField("Height", m_Extents.vector3Value.y * 2.0f);
      if (EditorGUI.EndChangeCheck()) {
