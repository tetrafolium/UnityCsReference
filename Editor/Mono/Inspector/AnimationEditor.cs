...
internal class AnimationEditor : Editor {
  ...
  public void OnEnable() { m_PrePreviewAnimationArraySize = -1; }
  ...
  public override void OnInspectorGUI() {
    ...
    SerializedProperty arrProperty =
        serializedObject.FindProperty("m_Animations");
    ...
    // Make sure the array is the original array size + 1 at max (+1 for the
    // ObjectSelector preview slot)
    if (m_PrePreviewAnimationArraySize != -1) {
      // Always resize if the last anim element is not the current animation
      int lastAnimID = arrSize > 0
                           ? arrProperty.GetArrayElementAtIndex(arrSize - 1)
