...
internal class CustomRenderTextureEditor : RenderTextureEditor {
  private class Styles {
    public readonly GUIContent shaderPass = EditorGUIUtility.TrTextContent(
        "Shader Pass", "Shader Pass used to update the Custom Render Texture.");
    public readonly GUIContent needSwap = EditorGUIUtility.TrTextContent(
        "Swap (Double Buffer)",
        "If ticked, and if the texture is double buffered, a request is made to swap the buffers before the next update. If this is not ticked, the buffers will not be swapped.");
    public readonly GUIContent updateMode = EditorGUIUtility.TrTextContent(
        "Update Mode", "Specify how the texture should be updated.");
    public readonly GUIContent updatePeriod = EditorGUIUtility.TrTextContent(
        "Period",
        "Period in seconds at which real-time textures are updated (0.0 will update every frame).");
    public readonly GUIContent doubleBuffered = EditorGUIUtility.TrTextContent(
        "Double Buffered",
        "If ticked, the Custom Render Texture is double buffered so that you can access it during its own update. If unticked, the Custom Render Texture will be not be double buffered.");
    public readonly GUIContent initializationMode =
        EditorGUIUtility.TrTextContent(
            "Initialization Mode",
            "Specify how the texture should be initialized.");
    public readonly GUIContent initSource = EditorGUIUtility.TrTextContent(
        "Source",
        "Specify if the texture is initialized by a Material or by a Texture and a Color.");
    public readonly GUIContent initColor = EditorGUIUtility.TrTextContent(
        "Color", "Color with which the Custom Render Texture is initialized.");
    public readonly GUIContent initTexture = EditorGUIUtility.TrTextContent(
        "Texture",
        "Texture with which the Custom Render Texture is initialized (multiplied by the initialization color).");
    public readonly GUIContent initMaterial = EditorGUIUtility.TrTextContent(
        "Material",
        "Material with which the Custom Render Texture is initialized.");
    public readonly GUIContent updateZoneSpace = EditorGUIUtility.TrTextContent(
        "Update Zone Space",
        "Space in which the update zones are expressed (Normalized or Pixel space).");
    public readonly GUIContent updateZoneList = EditorGUIUtility.TrTextContent(
        "Update Zones", "List of partial update zones.");
    public readonly GUIContent cubemapFacesLabel =
        EditorGUIUtility.TrTextContent(
            "Cubemap Faces",
            "Enable or disable rendering on each face of the cubemap.");
    public readonly GUIContent updateZoneCenter =
        EditorGUIUtility.TrTextContent("Center",
                                       "Center of the partial update zone.");
    public readonly GUIContent updateZoneSize = EditorGUIUtility.TrTextContent(
        "Size", "Size of the partial update zone.");
    public readonly GUIContent updateZoneRotation =
        EditorGUIUtility.TrTextContent("Rotation",
                                       "Rotation of the update zone.");
    public readonly GUIContent wrapUpdateZones = EditorGUIUtility.TrTextContent(
        "Wrap Update Zones",
        "If ticked, Update zones will wrap around the border of the Custom Render Texture. If unticked, Update zones will be clamped at the border of the Custom Render Texture.");

    public readonly GUIContent[] updateModeStrings = {
        EditorGUIUtility.TextContent("OnLoad"),
        EditorGUIUtility.TrTextContent("Realtime"),
        EditorGUIUtility.TrTextContent("OnDemand")};
    public readonly int[] updateModeValues = {
        (int) CustomRenderTextureUpdateMode.OnLoad,
        (int) CustomRenderTextureUpdateMode.Realtime,
        (int) CustomRenderTextureUpdateMode.OnDemand};

    public readonly GUIContent[] initSourceStrings = {
        EditorGUIUtility.TrTextContent("Texture and Color"),
        EditorGUIUtility.TrTextContent("Material")};
    public readonly int[] initSourceValues = {
        (int) CustomRenderTextureInitializationSource.TextureAndColor,
        (int) CustomRenderTextureInitializationSource.Material};

    public readonly GUIContent[] updateZoneSpaceStrings = {
        EditorGUIUtility.TrTextContent("Normalized"),
        EditorGUIUtility.TrTextContent("Pixel")};
    public readonly int[] updateZoneSpaceValues = {
        (int) CustomRenderTextureUpdateZoneSpace.Normalized,
        (int) CustomRenderTextureUpdateZoneSpace.Pixel};
  ...
  void UpdateZoneVec3PropertyField(Rect rect, SerializedProperty prop,
                                   GUIContent label, bool as2D) {
    EditorGUI.BeginProperty(rect, label, prop);
    if (!as2D) {
      prop.vector3Value =
          EditorGUI.Vector3Field(rect, label, prop.vector3Value);
    } else {
      Vector2 newValue = EditorGUI.Vector2Field(
          rect, label, new Vector2(prop.vector3Value.x, prop.vector3Value.y));
      prop.vector3Value =
          new Vector3(newValue.x, newValue.y, prop.vector3Value.z);
    }
    EditorGUI.EndProperty();
  }

  private void OnDrawElement(Rect rect, int index, bool isActive,
                             bool isFocused) {
    CustomRenderTexture customRenderTexture = target as CustomRenderTexture;
    bool is3DTexture = customRenderTexture.dimension ==
    ...
    SerializedProperty centerProp =
        element.FindPropertyRelative("updateZoneCenter");
    UpdateZoneVec3PropertyField(rect, centerProp, styles.updateZoneCenter,
                                !is3DTexture);

    rect.y += lineHeight + EditorGUIUtility.standardVerticalSpacing;
    SerializedProperty sizeProp =
        element.FindPropertyRelative("updateZoneSize");
    UpdateZoneVec3PropertyField(rect, sizeProp, styles.updateZoneSize,
                                !is3DTexture);

    if (!is3DTexture) {
      rect.y += (EditorGUIUtility.standardVerticalSpacing + lineHeight);
      EditorGUI.PropertyField(rect, element.FindPropertyRelative("rotation"),
                              styles.updateZoneRotation);
    }
    ...
    using(new EditorGUI.DisabledScope(shaderPassNames.Count == 0)) {
      SerializedProperty passIndexProperty =
          element.FindPropertyRelative("passIndex");
      rect.y += (EditorGUIUtility.standardVerticalSpacing + lineHeight);
      EditorGUI.IntPopup(rect, passIndexProperty, shaderPassNames.ToArray(),
                         shaderPassValues.ToArray(), styles.shaderPass);
    }

    if (isDoubleBuffer) {
      rect.y += (EditorGUIUtility.standardVerticalSpacing + lineHeight);
      EditorGUI.PropertyField(rect, element.FindPropertyRelative("needSwap"),
                              styles.needSwap);
    }
  }
  ...
  private void OnAdd(ReorderableList l) {
    ...
    if (customRenderTexture.updateZoneSpace ==
        CustomRenderTextureUpdateZoneSpace.Pixel) {
      Vector3 size = new Vector3((float) customRenderTexture.width,
                                 (float) customRenderTexture.height,
                                 (float) customRenderTexture.volumeDepth);
      defaultCenter.Scale(size);
      defaultSize.Scale(size);
    }
    element.FindPropertyRelative("updateZoneCenter").vector3Value =
        defaultCenter;
  ...
  private float OnElementHeight(int index) {
    ...
    if (!is3DTexture)  // We don't support rotation for 3D textures so we don't
                       // show it.
      lineCount++;
  ...
  protected override void OnEnable() {
    ...
    m_InitializationMode =
        serializedObject.FindProperty("m_InitializationMode");
    ...
    m_RectList = new UnityEditorInternal.ReorderableList(serializedObject,
                                                         m_UpdateZones);
    ...
    m_ShowInitSourceAsMaterial.value =
        !m_InitSource.hasMultipleDifferentValues &&
        (m_InitSource.intValue ==
         (int) CustomRenderTextureInitializationSource.Material);
  ...
  void DisplayRenderTextureGUI() {
    OnRenderTextureGUI(GUIElements.RenderTargetDepthGUI |
                       GUIElements.RenderTargetAAGUI);
    GUILayout.Space(10.0f);
  }

  void BuildShaderPassPopup(Material material, List<GUIContent> names,
                            List<int> values, bool addDefaultPass) {
    names.Clear();
    values.Clear();
    ...
    if (addDefaultPass) {
      ...
      GUIContent defaultName = EditorGUIUtility.TextContent(string.Format(
          "Default ({0})", names[customRenderTexture.shaderPass].text));
  ...
  void DisplayMaterialGUI() {
    ...
    using(new EditorGUI.DisabledScope(
        shaderPassNames.Count == 0 ||
        m_Material
            .hasMultipleDifferentValues)) // Different materials can have widely
                                          // different passes, so there's no
                                          // point trying to edit that when
                                          // multiple editing.
    {
      if (material != null)
        EditorGUILayout.IntPopup(m_ShaderPass, shaderPassNames.ToArray(),
                                 shaderPassValues.ToArray(), styles.shaderPass);
    }
  ...
  void DisplayInitializationGUI() {
    m_ShowInitSourceAsMaterial.target =
        !m_InitSource.hasMultipleDifferentValues &&
        (m_InitSource.intValue ==
         (int) CustomRenderTextureInitializationSource.Material);

    EditorGUILayout.IntPopup(m_InitializationMode, styles.updateModeStrings,
                             styles.updateModeValues,
                             styles.initializationMode);
    EditorGUI.indentLevel++;

    EditorGUILayout.IntPopup(m_InitSource, styles.initSourceStrings,
                             styles.initSourceValues, styles.initSource);
    if (!m_InitSource.hasMultipleDifferentValues) {
      ...
      if (EditorGUILayout.BeginFadeGroup(1 -
                                         m_ShowInitSourceAsMaterial.faded)) {
  ...
  void DisplayUpdateGUI() {
    EditorGUILayout.IntPopup(m_UpdateMode, styles.updateModeStrings,
                             styles.updateModeValues, styles.updateMode);
    ...
    if (isCubemap) {
      ...
      var AllRects = GUILayoutUtility.GetRect(
          0, EditorGUIUtility.singleLineHeight * 3 +
      ...
      int faceIndex = 0;
      {
        ...
        for (int i = 0; i < 3; ++i) {
          ...
          {
            for (int j = 0; j < 2; ++j) {
              bool value = EditorGUI.ToggleLeft(
                  toggleRect, styles.cubemapFaces[faceIndex],
                  (currentFaceMask & (1 << faceIndex)) != 0);
              if (value)
    ...
    EditorGUILayout.IntPopup(m_UpdateZoneSpace, styles.updateZoneSpaceStrings,
                             styles.updateZoneSpaceValues,
                             styles.updateZoneSpace);

    if (!multipleEditing) {
      Rect listRect = GUILayoutUtility.GetRect(
          0.0f, m_RectList.GetHeight() + kRListAddButtonOffset,
          GUILayout.ExpandWidth(
              true)); // kRListAddButtonOffset because reorderable list does not
                      // take the  +/- button at the bottom when computing its
                      // Rects making it half occulted by other GUI elements.
      ...
      EditorGUILayout.HelpBox(
          "Update Zones cannot be changed while editing multiple Custom Textures.",
          MessageType.Info);
    }
  ...
  void DisplayCustomRenderTextureGUI() {
    ...
    if (customRenderTexture.updateMode !=
            CustomRenderTextureUpdateMode.Realtime &&
        customRenderTexture.initializationMode ==
            CustomRenderTextureUpdateMode.Realtime)
      EditorGUILayout.HelpBox(
          "Initialization Mode is set to Realtime but Update Mode is not. This will result in update never being visible.",
          MessageType.Warning);
  }
  ...
  static void SaveToDisk(MenuCommand command) {
    ...
    // This has its TextureFormat helper equivalent in C++ but since we are
    // going to try to refactor TextureFormat/RenderTextureFormat into a single
    // type so let's not bloat Scripting APIs with stuff that will get useless
    // soon(tm).
    bool isFormatHDR =
        GraphicsFormatUtility.IsIEEE754Format(texture.graphicsFormat);
    bool isFloatFormat =
        GraphicsFormatUtility.IsFloatFormat(texture.graphicsFormat);

    TextureFormat format =
        isFormatHDR ? TextureFormat.RGBAFloat : TextureFormat.RGBA32;
    ...
    if (isFormatHDR)
      bytes = tex.EncodeToEXR(
          ...
          (isFloatFormat ? Texture2D.EXRFlags.OutputAsFloat : 0));
    else
    ...
    string assetPath = EditorUtility.SaveFilePanel(
        "Save Custom Render Texture", directory, texture.name, extension);
  ...
  override public string GetInfoString() { return base.GetInfoString(); }
}
}
