...
protected virtual void OnSceneGUI() {
	...
	// this.serializedObject will not work within OnSceneGUI if multiple targets
	// are selected
...
internal override Bounds GetWorldBoundsOfTarget(Object targetObject) {
	using (var so = new SerializedObject(targetObject)) {
		...
		Matrix4x4 portalTransformMatrix =
			((OcclusionPortal) targetObject).transform.localToWorldMatrix;
		var worldBounds = new Bounds(portalTransformMatrix.MultiplyPoint3x4(
						     new Vector3(max.x, max.y, max.z)),
		                             Vector3.zero);
		worldBounds.Encapsulate(portalTransformMatrix.MultiplyPoint3x4(
						new Vector3(max.x, max.y, max.z)));
		worldBounds.Encapsulate(portalTransformMatrix.MultiplyPoint3x4(
