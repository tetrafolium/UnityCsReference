...
public void OnSceneGUI() {
	...
	Vector3 worldAnchor =
		TransformPoint(anchorJoint2D.transform, anchorJoint2D.anchor);
	Vector3 worldConnectedAnchor = anchorJoint2D.connectedAnchor;
	if (anchorJoint2D.connectedBody)
		worldConnectedAnchor = TransformPoint(
			anchorJoint2D.connectedBody.transform, worldConnectedAnchor);

	// Draw line between anchors
	Vector3 startPoint =
		worldAnchor + (worldConnectedAnchor - worldAnchor).normalized *
		HandleUtility.GetHandleSize(worldAnchor) * 0.1f;
	...
	if (HandleAnchor(ref worldConnectedAnchor, true)) {
		...
		worldConnectedAnchor =
			SnapToPoint(worldConnectedAnchor, worldAnchor, k_SnapDistance);

		if (anchorJoint2D.connectedBody)
			worldConnectedAnchor = InverseTransformPoint(
				anchorJoint2D.connectedBody.transform, worldConnectedAnchor);
	...
	if (HandleAnchor(ref worldAnchor, false)) {
		...
		worldAnchor =
			SnapToPoint(worldAnchor, worldConnectedAnchor, k_SnapDistance);

		Undo.RecordObject(anchorJoint2D, "Move Anchor");
		anchorJoint2D.anchor =
			InverseTransformPoint(anchorJoint2D.transform, worldAnchor);
	}
}

Vector3 SnapToSprites(Vector3 position) {
	SpriteRenderer spriteRenderer =
		anchorJoint2D.GetComponent<SpriteRenderer>();
	position = SnapToSprite(spriteRenderer, position, k_SnapDistance);

	if (anchorJoint2D.connectedBody) {
		spriteRenderer =
			anchorJoint2D.connectedBody.GetComponent<SpriteRenderer>();
		position = SnapToSprite(spriteRenderer, position, k_SnapDistance);
	}
