...
public ScriptableRenderPipelineExtensionAttribute(
	Type renderPipelineAsset) {
	if (!(renderPipelineAsset?.IsSubclassOf(typeof(RenderPipelineAsset)) ?? false))
		throw new ArgumentException(
			      "Given renderPipelineAsset must derive from RenderPipelineAsset");
...
FetchFirstCompatibleTypeUsingScriptableRenderPipelineExtension<
	...
	foreach (Type extensionType in extensionTypes) {
		ScriptableRenderPipelineExtensionAttribute attribute =
			Attribute.GetCustomAttribute(
				extensionType,
				typeof(ScriptableRenderPipelineExtensionAttribute))
			as ScriptableRenderPipelineExtensionAttribute;
