...
public static readonly string editAngularLimitsUndoMessage =
	EditorGUIUtility.TrTextContent("Change Joint Angular Limits").text;
public static readonly Color handleColor = new Color(0f, 1f, 0f, 0.7f);
public static readonly float handleRadius = 0.8f;

static Styles() {
	editAngularLimitsButton.tooltip =
		EditorGUIUtility.TrTextContent("Edit joint angular limits.").text;
}
}

private JointAngularLimitHandle m_AngularLimitHandle =
	new JointAngularLimitHandle();
...
new public void OnEnable() {
	...
	// +/-PHYSICS_2D_LARGE_RANGE_CLAMP, which is currently used in
	// JointAngleLimits2D.CheckConsistency()
...
public override void OnInspectorGUI() {
	EditMode.DoEditModeInspectorModeButton(
		...
		"Edit Joint Angular Limits", Styles.editAngularLimitsButton, this);
...
new public void OnSceneGUI() {
	...
	m_AngularLimitHandle.xMotion =
		hingeJoint2D.useLimits ? ConfigurableJointMotion
		.Limited : ConfigurableJointMotion.Free;
	...
	var editMode =
		EditMode.editMode == EditMode.SceneViewEditMode.JointAngularLimits &&
		EditMode.IsOwner(this);
	if (editMode)
		m_AngularLimitHandle.angleHandleDrawFunction =
			ArcHandle.DefaultAngleHandleDrawFunction;
	else
		m_AngularLimitHandle.angleHandleDrawFunction = null;

	// to enhance usability, orient the manipulator to best illustrate its
	// affects on the dynamic body in the system
	...
	var handleOrientation =
		(connectedBody == null ? Quaternion.identity
	 : Quaternion.LookRotation(
			 Vector3.forward, connectedBody.transform.rotation * Vector3.up)) *
		handleOrientationOffset;
	var dynamicActorReferencePosition =
		handlePosition +
		Quaternion.LookRotation(Vector3.forward,
		                        dynamicBody.transform.rotation * Vector3.up) *
		dynamicBodyLocalReferencePosition;

	var handleMatrix =
		Matrix4x4.TRS(handlePosition, handleOrientation, Vector3.one);

	EditorGUI.BeginChangeCheck();

	using (new Handles.DrawingScope(Styles.handleColor, handleMatrix)) {
		var radius =
			HandleUtility.GetHandleSize(Vector3.zero) * Styles.handleRadius;
		m_AngularLimitHandle.radius = radius;

		// reference line within arc to illustrate affected local axis
		Handles.DrawLine(
			Vector3.zero,
			handleMatrix.inverse.MultiplyPoint3x4(dynamicActorReferencePosition)
			.normalized *
			radius);
