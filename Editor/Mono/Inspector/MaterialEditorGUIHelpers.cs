...
public partial class MaterialEditor {
  ...
  public void RenderQueueField(Rect r) {
    ...
    int curDisplayQueue =
        mat.renderQueue; // this gets final queue value used for rendering,
                         // taking shader's queue into account

    // Figure out if we're using one of common queues, or a custom one
    GUIContent[] queueNames = null;
    int[] queueValues = null;
    float labelWidth;
    // If we use queue value that is not available, lets switch to the custom
    // one
    bool useCustomQueue = Array.IndexOf(Styles.queueValues, curRawQueue) < 0;
    if (useCustomQueue) {
      // It is a big chance that we already have this custom queue value
      // available
      bool updateNewCustomQueueValue =
          Array.IndexOf(Styles.customQueueNames, curRawQueue) < 0;
      if (updateNewCustomQueueValue) {
        ...
        int targetQueueValueOverflow =
            curRawQueue - Styles.queueValues[targetQueueIndex];

        string newQueueName =
            string.Format(targetQueueValueOverflow > 0 ? "{0}+{1}" : "{0}{1}",
                          targetQueueName, targetQueueValueOverflow);
    ...
    // We want the custom queue number field to line up with thumbnails & other
    // value fields (on the right side), and common queues popup to be on the
    // left of that.
    ...
    int newPopupValue = EditorGUI.IntPopup(
        popupRect, Styles.queueLabel, curRawQueue, queueNames, queueValues);

    // Custom queue field
    int newDisplayQueue =
        EditorGUI.DelayedIntField(numberRect, curDisplayQueue);
  ...
  public void EnableInstancingField(Rect r) {
    EditorGUI.PropertyField(r, m_EnableInstancing,
  ...
  public Rect TexturePropertySingleLine(GUIContent label,
  ...
  // Mini texture slot, with two extra controls on the same line (allocates rect
  // in GUILayout) Have up to 3 controls on one line
  public Rect TexturePropertySingleLine(GUIContent label,
                                        MaterialProperty textureProp,
                                        MaterialProperty extraProperty1,
    ...
    // Temporarily reset the indent level as it was already used earlier to
    // compute the positions of the layout items. See issue 946082.
    ...
    } else // Two extra properties
    {
      if (extraProperty1.type == MaterialProperty.PropType.Color) {
        ExtraPropertyAfterTexture(GetFlexibleRectBetweenFieldAndRightEdge(r),
                                  extraProperty2);
        ...
        ExtraPropertyAfterTexture(GetFlexibleRectBetweenLabelAndField(r),
                                  extraProperty1);
      }
    }
  ...
  [Obsolete(
      "Use TexturePropertyWithHDRColor(GUIContent label, MaterialProperty textureProp, MaterialProperty colorProperty, bool showAlpha)")]
  public Rect
  TexturePropertyWithHDRColor(GUIContent label, MaterialProperty textureProp,
                              MaterialProperty colorProperty,
                              ColorPickerHDRConfig hdrConfig, bool showAlpha) {
    return TexturePropertyWithHDRColor(label, textureProp, colorProperty,
                                       showAlpha);
  }

  public Rect TexturePropertyWithHDRColor(GUIContent label,
                                          MaterialProperty textureProp,
                                          MaterialProperty colorProperty,
                                          bool showAlpha) {
    Rect r = GetControlRectForSingleLine();
    ...
    if (colorProperty.type != MaterialProperty.PropType.Color) {
      Debug.LogError("Assuming MaterialProperty.PropType.Color (was " +
                     colorProperty.type + ")");
      return r;
    }
    ...
    Color newValue =
        EditorGUI.ColorField(leftRect, GUIContent.none,
                             colorProperty.colorValue, true, showAlpha, true);
    EditorGUI.showMixedValue = false;
  ...
  public Rect TexturePropertyTwoLines(GUIContent label,
                                      MaterialProperty textureProp,
                                      MaterialProperty extraProperty1,
                                      GUIContent label2,
                                      MaterialProperty extraProperty2) {
    // If not using the second extra property then use the single line version
    // as the first extra property is always inlined with the the texture slot
    ...
    ShaderProperty(r2, extraProperty2, label2.text,
  ...
  Rect GetControlRectForSingleLine() {
    const float extraSpacing =
        2f; // The shader properties needs a little more vertical spacing due to
            // the mini texture field (looks cramped without)
    return EditorGUILayout.GetControlRect(
        true, EditorGUI.kSingleLineHeight + extraSpacing,
  ...
  void ExtraPropertyAfterTexture(Rect r, MaterialProperty property) {
    if ((property.type == MaterialProperty.PropType.Float ||
      ...
      // We want color fields and float fields to have same width as
      // EditorGUIUtility.fieldWidth so controls aligns vertically. This also
      // makes us able to have a draggable area in front of the float fields. We
      // therefore ensures the property has a label (here we use a whitespace)
      // and adjust label width.
      float oldLabelWidth = EditorGUIUtility.labelWidth;
  ...
  static public Rect GetRightAlignedFieldRect(Rect r) {
    return new Rect(r.xMax - EditorGUIUtility.fieldWidth, r.y,
                    EditorGUIUtility.fieldWidth,
                    EditorGUIUtility.singleLineHeight);
  }

  static public Rect GetLeftAlignedFieldRect(Rect r) {
    return new Rect(r.x + EditorGUIUtility.labelWidth, r.y,
                    EditorGUIUtility.fieldWidth,
                    EditorGUIUtility.singleLineHeight);
  }
  ...
  static public Rect GetFlexibleRectBetweenLabelAndField(Rect r) {
    return new Rect(r.x + EditorGUIUtility.labelWidth, r.y,
                    r.width - EditorGUIUtility.labelWidth -
                        EditorGUIUtility.fieldWidth -
                        kSpaceBetweenFlexibleAreaAndField,
                    EditorGUIUtility.singleLineHeight);
  }
  ...
  static internal System.Type
  GetTextureTypeFromDimension(TextureDimension dim) {
    switch (dim) {
    case TextureDimension.Tex2D:
      return typeof(Texture); // common use case is RenderTextures too, so
                              // return base class
