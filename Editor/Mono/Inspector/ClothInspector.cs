...
class ClothInspectorState : ScriptableSingleton<ClothInspectorState> {
  ...
  [SerializeField]
  public bool SetMaxDistance = false;
  [SerializeField]
  public bool SetCollisionSphereDistance = false;
  [SerializeField]
  public ClothInspector.ToolMode ToolMode = ClothInspector.ToolMode.Paint;
  [SerializeField]
  public ClothInspector.CollToolMode CollToolMode =
      ClothInspector.CollToolMode.Select;
  [SerializeField]
  public float BrushRadius = 0.075f;
  [SerializeField]
  public bool SetSelfAndInterCollision = false;
  [SerializeField]
  public ClothInspector
      .CollisionVisualizationMode VisualizeSelfOrInterCollision =
      ClothInspector.CollisionVisualizationMode.SelfCollision;
  [SerializeField]
  ...
  [SerializeField]
  public float GradientStartValue = 0.0f;
  [SerializeField]
  public float GradientEndValue = 1.0f;
}
...
class ClothInspector : Editor {
  ...
  public static PrefColor s_BrushColor =
      new PrefColor("Cloth/Brush Color 2", 0.0f / 255.0f, 0.0f / 255.0f,
                    0.0f / 255.0f, 51.0f / 255.0f);
  public static PrefColor s_SelfAndInterCollisionParticleColor =
      new PrefColor("Cloth/Self or Inter Collision Particle Color 2",
                    145.0f / 255.0f, 244.0f / 255.0f, 139.0f / 255.0f, 0.5f);
  public static PrefColor s_UnselectedSelfAndInterCollisionParticleColor =
      new PrefColor("Cloth/Unselected Self or Inter Collision Particle Color 2",
                    0.1f, 0.1f, 0.1f, 0.5f);
  public static PrefColor s_SelectedParticleColor =
      new PrefColor("Cloth/Selected Self or Inter Collision Particle Color 2",
                    64.0f / 255.0f, 160.0f / 255.0f, 255.0f / 255.0f, 0.5f);

  public static ToolMode[] s_ToolMode = {ToolMode.Paint, ToolMode.Select,
  ...
  private static class Styles {
    public static readonly GUIContent editConstraintsLabel =
        EditorGUIUtility.TrTextContent("Edit Constraints");
    public static readonly GUIContent editSelfInterCollisionLabel =
        EditorGUIUtility.TrTextContent("Edit Collision Particles");
    public static readonly GUIContent selfInterCollisionParticleColor =
        EditorGUIUtility.TrTextContent("Visualization Color");
    public static readonly GUIContent selfInterCollisionBrushColor =
        EditorGUIUtility.TrTextContent("Brush Color");
    ...
    public static readonly GUIContent paintCollisionParticles =
        EditorGUIUtility.TrTextContent("Paint Collision Particles");
    public static readonly GUIContent selectCollisionParticles =
        EditorGUIUtility.TrTextContent("Select Collision Particles");
    public static readonly GUIContent brushRadiusString =
        EditorGUIUtility.TrTextContent("Brush Radius");
    public static readonly GUIContent constraintSizeString =
        EditorGUIUtility.TrTextContent("Constraint Size");
    public static readonly GUIContent gradientStartString =
        EditorGUIUtility.TrTextContent("Gradient Start");
    public static readonly GUIContent gradientEndString =
        EditorGUIUtility.TrTextContent("Gradient End");
    public static readonly GUIContent setMaxDistanceString =
        EditorGUIUtility.TrTextContent("Max Distance");
    public static readonly GUIContent setCollisionSphereDistanceString =
        EditorGUIUtility.TrTextContent("Surface Penetration");
    public static readonly GUIContent selfAndInterCollisionMode =
        EditorGUIUtility.TrTextContent("Paint or Select Particles");
    public static readonly GUIContent backFaceManipulationMode =
        EditorGUIUtility.TrTextContent("Back Face Manipulation");
    public static readonly GUIContent manipulateBackFaceString =
        EditorGUIUtility.TrTextContent("Manipulate Backfaces");
    ...
    public static readonly GUIContent setSelfAndInterCollisionString =
        EditorGUIUtility.TrTextContent("Self-Collision and Inter-Collision");
    ...
    public static GUIContent paintIcon = EditorGUIUtility.TrIconContent(
        "ClothInspector.PaintValue",
        "Change this vertex coefficient value by painting in the scene view.");

    public static EditMode.SceneViewEditMode[] sceneViewEditModes =
        new[]{EditMode.SceneViewEditMode.ClothConstraints,
              EditMode.SceneViewEditMode.ClothSelfAndInterCollisionParticles};

    public static GUIContent selfCollisionDistanceGUIContent =
        EditorGUIUtility.TrTextContent("Self-Collision Distance");
    public static GUIContent selfCollisionStiffnessGUIContent =
        EditorGUIUtility.TrTextContent("Self-Collision Stiffness");

    static Styles() {
      toolContents[0].tooltip =
          EditorGUIUtility.TrTextContent("Edit cloth constraints").text;
      toolContents[1].tooltip =
          EditorGUIUtility.TrTextContent("Edit cloth self/inter-collision")
              .text;

      toolIcons[0].tooltip =
          EditorGUIUtility
              .TrTextContent(
                  "Select cloth particles for use in self/inter-collision")
              .text;
      toolIcons[1].tooltip =
          EditorGUIUtility
              .TrTextContent(
                  "Paint cloth particles for use in self/inter-collision")
              .text;

      collToolModeIcons[0].tooltip =
          EditorGUIUtility.TrTextContent("Select cloth particles.").text;
      collToolModeIcons[1].tooltip =
          EditorGUIUtility.TrTextContent("Paint cloth particles.").text;
      collToolModeIcons[2].tooltip =
          EditorGUIUtility.TrTextContent("Erase cloth particles.").text;
    }
  }
  ...
  public bool editingConstraints =>
      EditMode.editMode == EditMode.SceneViewEditMode.ClothConstraints &&
  ...
  Texture2D GenerateColorTexture(int width) {
    ...
    for (int i = 0; i < width; i++)
      colors [i]
      = GetGradientColor(i / (float)(width - 1));
  ...
  public override void OnInspectorGUI() {
    ...
    if (targets.Length < 2) {
      ...
      EditMode.DoInspectorToolbar(Styles.sceneViewEditModes,
                                  Styles.toolContents, this);
      GUILayout.FlexibleSpace();
      GUILayout.EndHorizontal();
    }

    if (editingSelfAndInterCollisionParticles) {
      if ((state.SetSelfAndInterCollision) ||
          ((state.CollToolMode == CollToolMode.Paint) ||
           (state.CollToolMode == CollToolMode.Erase))) {
        ...
        Rect rect = GUILayoutUtility.GetRect(new GUIContent(), GUIStyle.none,
                                             GUILayout.ExpandWidth(true),
                                             GUILayout.Height(17));
        EditorGUI.LabelField(rect, Styles.selfCollisionString,
                             EditorStyles.boldLabel);
        Rect rectDist = GUILayoutUtility.GetRect(
            new GUIContent(), GUIStyle.none, GUILayout.ExpandWidth(true),
            GUILayout.Height(17));
        EditorGUI.PropertyField(rectDist, m_SelfCollisionDistance,
                                Styles.selfCollisionDistanceGUIContent);
        Rect rectStiff = GUILayoutUtility.GetRect(
            new GUIContent(), GUIStyle.none, GUILayout.ExpandWidth(true),
            GUILayout.Height(17));
        EditorGUI.PropertyField(rectStiff, m_SelfCollisionStiffness,
                                Styles.selfCollisionStiffnessGUIContent);
        GUILayout.Space(10);
      }
    ...
    DrawPropertiesExcluding(serializedObject, "m_SelfAndInterCollisionIndices",
    ...
    if (meshRenderer != null)
      Debug.LogWarning(
          "MeshRenderer will not work with a cloth component! Use only SkinnedMeshRenderer. Any MeshRenderer's attached to a cloth component will be deleted at runtime.");
  }
  ...
  internal override Bounds GetWorldBoundsOfTarget(UnityObject targetObject) {
    ...
    return skin == null ? base.GetWorldBoundsOfTarget(targetObject)
        : skin.bounds;
  }
  ...
  bool SelectionMeshDirty() {
    if (m_LastVertices != null) {
      ...
      for (int i = 0; i < numLastVertices; i++) {
        // we use !(==) here instead of !=, since Vector3 != is incorrectly
        // written to return false if one component is NaN.
  ...
  void DrawSelfAndInterCollisionParticles() {
    ...
    if (state.VisualizeSelfOrInterCollision ==
        CollisionVisualizationMode.SelfCollision) {
      size = state.SelfCollisionDistance;
    } else if (state.VisualizeSelfOrInterCollision ==
               CollisionVisualizationMode.InterCollision) {
    ...
    for (int i = 0; i < length; i++) {
      ...
      bool forwardFacing = Vector3.Dot(m_ClothNormalsInWorldSpace[i],
                                       Camera.current.transform.forward) <= 0;
      if (forwardFacing || state.ManipulateBackfaces) {
        ...
        } else if (!m_SelfAndInterCollisionSelection[i] &&
                   !m_ParticleSelection[i]) {
          Handles.color = s_UnselectedSelfAndInterCollisionParticleColor;
        }

        if ((m_ParticleSelection[i] == true) && (m_NumSelection > 0) &&
            (state.CollToolMode == CollToolMode.Select)) {
          Handles.color = s_SelectedParticleColor;
        }

        if ((distanceBetween.magnitude < state.BrushRadius) && forwardFacing &&
            ((state.CollToolMode == CollToolMode.Paint) ||
             (state.CollToolMode == CollToolMode.Erase))) {
        ...
        Handles.SphereHandleCap(id, m_ClothParticlesInWorldSpace[i],
                                m_TransformOverride.rotation, size,
                                EventType.Repaint);
      }
    }
  }
  ...
  void OnEnable() {
    ...
    m_SelfCollisionDistance =
        serializedObject.FindProperty("m_SelfCollisionDistance");
    m_SelfCollisionStiffness =
        serializedObject.FindProperty("m_SelfCollisionStiffness");

    SceneView.beforeSceneGui += OnPreSceneGUICallback;

    m_ConstraintEditingOverlayWindow = new OverlayWindow(
        EditorGUIUtility.TrTextContent("Cloth Constraints"), ConstraintEditing,
        (int) SceneViewOverlay.Ordering.ClothConstraints, null,
        SceneViewOverlay.WindowDisplayOption.OneWindowPerTarget);
    m_SelfAndInterCollisionEditingOverlayWindow = new OverlayWindow(
        Styles.clothSelfCollisionAndInterCollision,
        SelfAndInterCollisionEditing,
        (int) SceneViewOverlay.Ordering.ClothSelfAndInterCollision, null,
        SceneViewOverlay.WindowDisplayOption.OneWindowPerTarget);
  }

  public void OnDestroy() { SceneView.beforeSceneGui -= OnPreSceneGUICallback; }
  ...
  void OnDisable() { SceneView.beforeSceneGui -= OnPreSceneGUICallback; }

  float CoefficientField(float value, float useValue, bool enabled,
                         DrawMode mode) {
  ...
  float PaintField(float value, ref bool enabled, DrawMode mode) {
    var label = GetDrawModeString(mode);
    GUILayout.BeginHorizontal();
    enabled = GUILayout.Toggle(enabled, Styles.paintIcon, "MiniButton",
    ...
    using(new EditorGUI.DisabledScope(!enabled)) {
      EditorGUI.BeginChangeCheck();
      useValue =
          EditorGUILayout.Toggle(GUIContent.none, value < kDisabledValue);
      if (EditorGUI.EndChangeCheck()) {
        if (useValue)
          value = 0;
        else
      ...
      using(new EditorGUI.DisabledScope(!useValue)) {
        ...
        if (EditorGUI.EndChangeCheck() ||
            oldHotControl != GUIUtility.keyboardControl)
          // if we got focus or changed values, set draw mode.
          drawMode = mode;
      }
    }
  ...
  void SelectionGUI() {
    ...
    for (int i = 0; i < numParticleSelection; i++) {
      if (m_ParticleSelection[i]) {
        if (firstVertex) {
          ...
          useCollisionSphereDistance =
              (collisionSphereDistance < kDisabledValue) ? 1 : 0;
          firstVertex = false;
        }
        ...
        if (useMaxDistance !=
            ((coefficients[i].maxDistance < kDisabledValue) ? 1 : 0))
          useMaxDistance = -1;
        if (useCollisionSphereDistance !=
            ((coefficients[i].collisionSphereDistance < kDisabledValue) ? 1
                                                                        : 0))
    ...
    float maxDistanceNew = CoefficientField(
        maxDistance, useMaxDistance, numSelection > 0, DrawMode.MaxDistance);
    ...
    float collisionSphereDistanceNew =
        CoefficientField(collisionSphereDistance, useCollisionSphereDistance,
                         numSelection > 0, DrawMode.CollisionSphereDistance);
  ...
  void GradientToolGUI() {
    ...
    using(new EditorGUI.DisabledScope(numSelection == 0)) {
      ...
      EditorGUI.BeginChangeCheck();
      bool setMaxDistance =
          EditorGUILayout.Toggle(GUIContent.none, state.SetMaxDistance);
      if (EditorGUI.EndChangeCheck()) {
        state.SetMaxDistance = setMaxDistance;
        int numCoefficients = coefficients.Length;
        for (int i = 0; i < numCoefficients; i++) {
          if (m_ParticleSelection[i]) {
            Vector3 pointOnLine = HandleUtility.ProjectPointLine(
                m_ClothParticlesInWorldSpace[i], m_GradientStartPoint,
                m_GradientEndPoint);
            Vector3 gradientStartToPoint = pointOnLine - m_GradientStartPoint;
            float lerpParameter =
                gradientStartToPoint.magnitude / gradientDirection.magnitude;
            float maxDistanceNew =
                Mathf.Lerp(state.GradientStartValue, state.GradientEndValue,
                           lerpParameter);
            coefficients[i].maxDistance = maxDistanceNew;
          }
        }
    ...
    using(new EditorGUI.DisabledScope(numSelection == 0)) {
      EditorGUILayout.BeginHorizontal();
      EditorGUI.BeginChangeCheck();
      bool setCollisionSphereDistance = EditorGUILayout.Toggle(
          GUIContent.none, state.SetCollisionSphereDistance);
      if (EditorGUI.EndChangeCheck()) {
  ...
  void CollSelectionGUI() {
    ...
    for (int i = 0; i < lengthSelection; i++) {
      if (m_ParticleRectSelection[i]) {
        if (firstFound == false) {
          ...
          if (state.SetSelfAndInterCollision !=
              m_SelfAndInterCollisionSelection[i]) {
            mixedValue = true;
    ...
    using(new EditorGUI.DisabledScope(m_NumSelection == 0)) {
      ...
      EditorGUI.BeginChangeCheck();
      bool setSelfAndInterCollision = EditorGUILayout.Toggle(
          GUIContent.none, state.SetSelfAndInterCollision);
      if (EditorGUI.EndChangeCheck()) {
        state.SetSelfAndInterCollision = setSelfAndInterCollision;
        for (int i = 0; i < lengthSelection; i++) {
          if (m_ParticleRectSelection[i]) {
            m_SelfAndInterCollisionSelection[i] =
                state.SetSelfAndInterCollision;
          }
        }

        Undo.RegisterCompleteObjectUndo(
            target,
            "Change Cloth Particles Selected for self or inter collision");
      }
  ...
  void EditBrushSize() {
    EditorGUI.BeginChangeCheck();
    float fieldValue =
        EditorGUILayout.FloatField(Styles.brushRadiusString, state.BrushRadius);
    bool changed = EditorGUI.EndChangeCheck();
    if (changed) {
  ...
  void EditConstraintSize() {
    EditorGUI.BeginChangeCheck();
    float fieldValue = EditorGUILayout.FloatField(Styles.constraintSizeString,
                                                  state.ConstraintSize);
    bool changed = EditorGUI.EndChangeCheck();
    if (changed) {
  ...
  void EditGradientStart() {
    EditorGUI.BeginChangeCheck();
    float fieldValue = EditorGUILayout.FloatField(Styles.gradientStartString,
                                                  state.GradientStartValue);
    bool changed = EditorGUI.EndChangeCheck();
    if (changed) {
  ...
  void EditGradientEnd() {
    EditorGUI.BeginChangeCheck();
    float fieldValue = EditorGUILayout.FloatField(Styles.gradientEndString,
                                                  state.GradientEndValue);
    bool changed = EditorGUI.EndChangeCheck();
    if (changed) {
  ...
  void PaintGUI() {
    state.PaintMaxDistance =
        PaintField(state.PaintMaxDistance, ref state.PaintMaxDistanceEnabled,
                   DrawMode.MaxDistance);

    state.PaintCollisionSphereDistance =
        PaintField(state.PaintCollisionSphereDistance,
                   ref state.PaintCollisionSphereDistanceEnabled,
                   DrawMode.CollisionSphereDistance);

    // Automatically switch visualization mode if we are only painting a
    // coefficient we are not visualizing
    if (state.PaintMaxDistanceEnabled &&
        !state.PaintCollisionSphereDistanceEnabled)
      drawMode = DrawMode.MaxDistance;
    else if (!state.PaintMaxDistanceEnabled &&
             state.PaintCollisionSphereDistanceEnabled)
  ...
  int GetMouseVertex(Event e) {
    ...
    for (int i = 0; i < m_NumVerts; i++) {
      ...
      bool forwardFacing =
          Vector3.Dot(
              m_ClothNormalsInWorldSpace[i],
              SceneView.GetLastActiveSceneViewCamera().transform.forward) <= 0;
      if ((forwardFacing || state.ManipulateBackfaces) &&
          sqrDistance < minDistance && sqrDistance < 0.05f * 0.05f) {
  ...
  void DrawConstraints() {
    ...
    for (int i = 0; i < length; i++) {
      ...
      if (forwardFacing || state.ManipulateBackfaces) {
        ...
        Vector3 distanceBetween = m_ClothParticlesInWorldSpace[i] - m_BrushPos;
        if ((m_ParticleSelection[i] == true) &&
            (state.CollToolMode == CollToolMode.Select)) {
          Handles.color = s_SelectedParticleColor;
        }

        if ((distanceBetween.magnitude < state.BrushRadius) && forwardFacing &&
            (state.ToolMode == ToolMode.Paint)) {
          if (m_BrushFace > -1) // check mouse pointer is over a valid mesh face
          {
            Handles.color = s_SelectedParticleColor;
          }
        ...
        Handles.SphereHandleCap(id, m_ClothParticlesInWorldSpace[i],
                                m_TransformOverride.rotation,
                                state.ConstraintSize, EventType.Repaint);
      }
    }
  }
  ...
  bool UpdateRectParticleSelection() {
    ...
    Plane top = new Plane(topRight.origin + topRight.direction,
                          topLeft.origin + topLeft.direction, topLeft.origin);
    Plane bottom =
        new Plane(botLeft.origin + botLeft.direction,
                  botRight.origin + botRight.direction, botRight.origin);
    Plane left = new Plane(topLeft.origin + topLeft.direction,
                           botLeft.origin + botLeft.direction, botLeft.origin);
    Plane right =
        new Plane(botRight.origin + botRight.direction,
                  topRight.origin + topRight.direction, topRight.origin);
    ...
    for (int i = 0; i < length; i++) {
      ...
      bool forwardFacing =
          Vector3.Dot(
              m_ClothNormalsInWorldSpace[i],
              SceneView.GetLastActiveSceneViewCamera().transform.forward) <= 0;
      bool selected = top.GetSide(v) && bottom.GetSide(v) && left.GetSide(v) &&
                      right.GetSide(v);
  ...
  internal void SendCommandsOnModifierKeys() {
    SceneView.lastActiveSceneView.SendEvent(
        EditorGUIUtility.CommandEvent(EventCommandNames.ModifierKeysChanged));
  }
  ...
  void SelectionPreSceneGUI(int id) {
    ...
    case EventType.MouseDrag:
      if (GUIUtility.hotControl == id) {
        if (!m_RectSelecting &&
            (e.mousePosition - m_SelectStartPoint).magnitude > 2f) {
        ...
        if (m_RectSelecting) {
          m_SelectMousePoint = new Vector2(Mathf.Max(e.mousePosition.x, 0),
                                           Mathf.Max(e.mousePosition.y, 0));
          RectSelectionModeFromEvent();
    ...
    case EventType.ExecuteCommand:
      if (m_RectSelecting &&
          e.commandName == EventCommandNames.ModifierKeysChanged) {
        RectSelectionModeFromEvent();
        UpdateRectParticleSelection();
      }
      break;
    ...
    case EventType.MouseUp:
      if (GUIUtility.hotControl == id && e.button == 0) {
        ...
        // Disable text focus when selection changes, otherwise we cannot update
        // inspector fields if text is currently selected.
    ...
    for (int i = 0; i < m_NumVerts; i++) {
      ...
      bool forwardFacing =
          Vector3.Dot(
              m_ClothNormalsInWorldSpace[i],
              SceneView.GetLastActiveSceneViewCamera().transform.forward) <= 0;
      if ((distanceBetween.magnitude < state.BrushRadius) &&
          (forwardFacing || state.ManipulateBackfaces)) {
        bool changed = false;
        if (state.PaintMaxDistanceEnabled &&
  ...
  void GetBrushedParticles(Event e) {
    ...
    for (int i = 0; i < m_NumVerts; i++) {
      ...
      if ((distanceBetween.magnitude < state.BrushRadius) &&
        ...
        Handles.color = s_SelectedParticleColor;
        Handles.SphereHandleCap(id, m_ClothParticlesInWorldSpace[i], rotation,
  ...
  void PaintPreSceneGUI(int id) {
    ...
    if (type == EventType.MouseDown || type == EventType.MouseDrag) {
      if (GUIUtility.hotControl != id &&
          (e.alt || e.control || e.command || e.button != 0))
        return;
  ...
  void OnPreSceneGUI() {
    ...
    if ((m_ParticleSelection == null) ||
        (m_LastVertices.Length != cloth.vertices.Length)) {
      GenerateSelectionMesh();
    } else {
      ClothSkinningCoefficient[] coefficients = cloth.coefficients;
  ...
  void OnSceneEditConstraintsGUI() {
    ...
    if (m_RectSelecting &&
        (state.ToolMode == ToolMode.Select ||
         state.ToolMode == ToolMode.GradientTool) &&
        Event.current.type == EventType.Repaint)
      EditorStyles.selectionRect.Draw(
          EditorGUIExt.FromToRect(m_SelectStartPoint, m_SelectMousePoint),
  ...
  void OnSceneEditSelfAndInterCollisionParticlesGUI() {
    ...
    Handles.BeginGUI();
    if (m_RectSelecting && state.CollToolMode == CollToolMode.Select &&
        Event.current.type == EventType.Repaint)
      EditorStyles.selectionRect.Draw(
          EditorGUIExt.FromToRect(m_SelectStartPoint, m_SelectMousePoint),
          GUIContent.none, false, false, false, false);
  ...
  public void VisualizationMenuSelfCollision() {
    state.VisualizeSelfOrInterCollision =
        CollisionVisualizationMode.SelfCollision;
  }
  ...
  public void DrawColorBox(Texture gradientTex, Color col) {
    ...
    GUILayout.Space(5);
    Rect r = GUILayoutUtility.GetRect(new GUIContent(), GUIStyle.none,
                                      GUILayout.ExpandWidth(true),
                                      GUILayout.Height(10));
  ...
  void ConstraintEditing(UnityObject unused, SceneView sceneView) {
    ...
    GUILayout.BeginVertical();
    if (EditorGUILayout.DropdownButton(GetDrawModeString(drawMode),
                                       FocusType.Passive,
                                       EditorStyles.toolbarDropDown)) {
      Rect buttonRect = GUILayoutUtility.topLevel.GetLast();
      GenericMenu menu = new GenericMenu();
      menu.AddItem(GetDrawModeString(DrawMode.MaxDistance),
                   drawMode == DrawMode.MaxDistance,
                   VisualizationMenuSetMaxDistanceMode);
      menu.AddItem(GetDrawModeString(DrawMode.CollisionSphereDistance),
                   drawMode == DrawMode.CollisionSphereDistance,
                   VisualizationMenuSetCollisionSphereMode);
      menu.AddSeparator("");
      menu.AddItem(EditorGUIUtility.TrTextContent("Manipulate Backfaces"),
                   state.ManipulateBackfaces,
                   VisualizationMenuToggleManipulateBackfaces);
      menu.DropDown(buttonRect);
    }

    GUILayout.BeginHorizontal();
    GUILayout.Label(m_MinVisualizedValue [(int) drawMode]
                        .ToString(),
                    GUILayout.ExpandWidth(false));
    DrawColorBox(s_ColorTexture, Color.clear);
    GUILayout.Label(m_MaxVisualizedValue [(int) drawMode]
                        .ToString(),
                    GUILayout.ExpandWidth(false));
    ...
    GUILayout.FlexibleSpace();
    state.ToolMode =
        (ToolMode) GUILayout.Toolbar((int) state.ToolMode, Styles.toolIcons);
    using(var check = new EditorGUI.ChangeCheckScope()) {
      if (check.changed) {
        // delselect text, so we don't end up having a text field highlighted in
        // the new tab
        GUIUtility.keyboardControl = 0;
        SceneView.RepaintAll();
      }
    ...
    if (m_CachedMesh.Target == null) {
      EditorGUILayout.HelpBox(
          "No mesh has been selected to use with cloth, please select a mesh for the skinned mesh renderer.",
          MessageType.Info);
    } else if (!IsConstrained()) {
      EditorGUILayout.HelpBox(
          "No constraints have been set up, so the cloth will move freely. Set up vertex constraints here to restrict it.",
          MessageType.Info);
    }
  ...
  void SelectManipulateBackFaces() {
    GUILayout.BeginHorizontal();
    EditorGUI.BeginChangeCheck();
    bool manipulateBackfaces =
        EditorGUILayout.Toggle(GUIContent.none, state.ManipulateBackfaces);
    if (EditorGUI.EndChangeCheck()) {
  ...
  void SelfAndInterCollisionEditing(UnityObject unused, SceneView sceneView) {
    ...
    if (EditorGUILayout.DropdownButton(
            GetCollVisModeString(state.VisualizeSelfOrInterCollision),
            FocusType.Passive, EditorStyles.toolbarDropDown)) {
      Rect buttonRect = GUILayoutUtility.topLevel.GetLast();
      GenericMenu menu = new GenericMenu();
      menu.AddItem(
          GetCollVisModeString(CollisionVisualizationMode.SelfCollision),
          state.VisualizeSelfOrInterCollision ==
              CollisionVisualizationMode.SelfCollision,
          VisualizationMenuSelfCollision);
      menu.AddItem(
          GetCollVisModeString(CollisionVisualizationMode.InterCollision),
          state.VisualizeSelfOrInterCollision ==
              CollisionVisualizationMode.InterCollision,
          VisualizationMenuInterCollision);
      menu.DropDown(buttonRect);
    }
    GUILayout.EndVertical();
    GUILayout.EndHorizontal();
    ...
    state.CollToolMode = (CollToolMode) GUILayout.Toolbar(
        (int) state.CollToolMode, Styles.collToolModeIcons);
    ...
    if (countIndices > 0) {
      List<UInt32> selfAndInterCollisionIndices =
          new List<UInt32>(countIndices);
      selfAndInterCollisionIndices.Clear();
