...
internal class PreviewScene : IDisposable {
  ...
  public PreviewScene(string sceneName) {
    ...
    var camGO = EditorUtility.CreateGameObjectWithHideFlags(
        "Preview Scene Camera", HideFlags.HideAndDontSave, typeof(Camera));
    AddGameObject(camGO);
    ...
    // (deferred fails when generating some static previews at editor launch;
    // and we never want vertex lit previews if that is chosen in the player
    // settings)
...
public class PreviewRenderUtility {
  ...
  public PreviewRenderUtility() {
    ...
    defaultBackgroundColor =
        new Color(49.0f / 255.0f, 49.0f / 255.0f, 49.0f / 255.0f, 1.0f);
    colorSpace = QualitySettings.activeColorSpace;
    camera.backgroundColor =
        colorSpace ==
        ColorSpace.Gamma ? defaultBackgroundColor
  ...
  ~PreviewRenderUtility() {
    if (m_Type != null) {
      Debug.LogErrorFormat(
          "{0} created a PreviewRenderUtility but didn't call its Cleanup() during OnDisable. This is leaking the Preview scene in the Editor and should be fixed.",
          m_Type);
    } else {
      Debug.LogError(
          "A PreviewRenderUtility was not clean up properly before assembly reloading which lead to leaking this scene in the Editor. " +
  ...
  [Obsolete("Use the property camera instead (UnityUpgradable) -> camera",
            false)]
  ...
  [Obsolete(
      "Use the property cameraFieldOfView (UnityUpgradable) -> cameraFieldOfView",
      false)]
  ...
  public void BeginPreview(Rect r, GUIStyle previewBackground) {
    ...
    if (previewBackground == null || previewBackground == GUIStyle.none ||
        previewBackground.normal.background == null)
      return;

    Graphics.DrawTexture(
        previewBackground.overflow.Add(
            new Rect(0, 0, m_RenderTexture.width, m_RenderTexture.height)),
        previewBackground.normal.background, new Rect(0, 0, 1, 1),
        previewBackground.border.left, previewBackground.border.right,
        previewBackground.border.top, previewBackground.border.bottom,
  ...
  public void BeginStaticPreview(Rect r) {
    ...
    if (!EditorApplication.isUpdating) {
      ...
      Texture defaultEnvTexture = ReflectionProbe.defaultTexture;
      if (Unsupported.SetOverrideLightingSettings(previewScene.scene)) {
        // Most preview windows just want the light probe from the main scene so
        // by default we copy it here. It can then be overridden if user wants.
  ...
  private void InitPreview(Rect r) {
    ...
    // about colour space, otherwise flip to the background colour to the
    // correct one
    if (colorSpace != QualitySettings.activeColorSpace &&
        (camera.backgroundColor == defaultBackgroundColor ||
         camera.backgroundColor.linear == defaultBackgroundColor.linear)) {
      camera.backgroundColor = QualitySettings.activeColorSpace ==
                               ColorSpace.Linear
                                     ? defaultBackgroundColor.linear
          : defaultBackgroundColor;
    }
    ...
    if (!m_RenderTexture || m_RenderTexture.width != rtWidth ||
      ...
      // Do not use GetTemporary to manage render textures. Temporary RTs are
      // only garbage collected each N frames, and in the editor we might be
      // wildly resizing the inspector, thus using up tons of memory.
      GraphicsFormat format =
          camera.allowHDR ? GraphicsFormat.R16G16B16A16_SFloat
    ...
    ShaderUtil.rawViewportRect =
        new Rect(0, 0, m_RenderTexture.width, m_RenderTexture.height);
    ShaderUtil.rawScissorRect =
        new Rect(0, 0, m_RenderTexture.width, m_RenderTexture.height);
  ...
  public float GetScaleFactor(float width, float height) {
    ...
    float result =
        Mathf.Min(scaleFacX, scaleFacY) * EditorGUIUtility.pixelsPerPoint;
    if (m_PixelPerfect)
  ...
  [Obsolete(
      "This method has been marked obsolete, use BeginStaticPreview() instead (UnityUpgradable) -> BeginStaticPreview(*)",
      false)]
  ...
  [Obsolete(
      "This method has been marked obsolete, use BeginPreview() instead (UnityUpgradable) -> BeginPreview(*)",
      false)]
  ...
  public Texture2D EndStaticPreview() {
    ...
    var copy =
        new Texture2D((int) m_TargetRect.width, (int) m_TargetRect.height,
                      TextureFormat.RGB24, false, false);
    copy.ReadPixels(new Rect(0, 0, m_TargetRect.width, m_TargetRect.height), 0,
                    0);
    copy.Apply();
  ...
  [Obsolete(
      "AddSingleGO(GameObject go, bool instantiateAtZero) has been deprecated, use AddSingleGo(GameObject go) instead. instantiateAtZero has no effect and is not supported.")]
  public void AddSingleGO(GameObject go, bool instantiateAtZero) {
    AddSingleGO(go);
  }

  public void AddSingleGO(GameObject go) { previewScene.AddGameObject(go); }

  public GameObject InstantiatePrefabInScene(GameObject prefab) {
    var instance = (GameObject) PrefabUtility.InstantiatePrefab(
        prefab, m_PreviewScene.scene);
    return instance;
  }

  internal void AddManagedGO(GameObject go) { m_PreviewScene.AddManagedGO(go); }

  public void DrawMesh(Mesh mesh, Matrix4x4 matrix, Material mat,
                       int subMeshIndex) {
  ...
  public void DrawMesh(Mesh mesh, Matrix4x4 m, Material mat, int subMeshIndex,
                       MaterialPropertyBlock customProperties,
                       Transform probeAnchor, bool useLightProbe) {
    var quat = Quaternion.LookRotation(m.GetColumn(2), m.GetColumn(1));
    var pos = m.GetColumn(3);
    var scale = new Vector3(m.GetColumn(0).magnitude, m.GetColumn(1).magnitude,
                            m.GetColumn(2).magnitude);

    DrawMesh(mesh, pos, scale, quat, mat, subMeshIndex, customProperties,
             probeAnchor, useLightProbe);
  }

  public void DrawMesh(Mesh mesh, Vector3 pos, Quaternion rot, Material mat,
                       int subMeshIndex) {
  ...
  public void DrawMesh(Mesh mesh, Vector3 pos, Quaternion rot, Material mat,
                       int subMeshIndex, MaterialPropertyBlock customProperties,
                       Transform probeAnchor) {
    DrawMesh(mesh, pos, rot, mat, subMeshIndex, customProperties, probeAnchor,
             false);
  }
  ...
  public void DrawMesh(Mesh mesh, Vector3 pos, Quaternion rot, Material mat,
                       int subMeshIndex, MaterialPropertyBlock customProperties,
                       Transform probeAnchor, bool useLightProbe) {
    DrawMesh(mesh, pos, Vector3.one, rot, mat, subMeshIndex, customProperties,
             probeAnchor, useLightProbe);
  }

  public void DrawMesh(Mesh mesh, Vector3 pos, Vector3 scale, Quaternion rot,
                       Material mat, int subMeshIndex,
                       MaterialPropertyBlock customProperties,
                       Transform probeAnchor, bool useLightProbe) {
    Graphics.DrawMesh(mesh, Matrix4x4.TRS(pos, rot, scale), mat, 1, camera,
                      subMeshIndex, customProperties, ShadowCastingMode.Off,
                      false, probeAnchor, useLightProbe);
  }
  ...
  protected static GameObject CreateLight() {
    GameObject lightGO = EditorUtility.CreateGameObjectWithHideFlags(
        "PreRenderLight", HideFlags.HideAndDontSave, typeof(Light));
  ...
  public void Render(bool allowScriptableRenderPipeline = false,
    ...
    if (updatefov) {
      // Calculate a view multiplier to avoid clipping when the preview width is
      // smaller than the height.
      float viewMultiplier =
          (m_RenderTexture.width <= 0
               ? 1.0f
               : Mathf.Max(1.0f, (float) m_RenderTexture.height /
                                     m_RenderTexture.width));
      // Multiply the viewing area by the viewMultiplier - it requires some
      // conversions since the camera view is expressed as an angle.
      camera.fieldOfView =
          Mathf.Atan(viewMultiplier *
