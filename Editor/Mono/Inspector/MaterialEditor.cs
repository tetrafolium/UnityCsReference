...
public partial class MaterialEditor : Editor {
  private static class Styles {
    ...
    public static readonly GUIContent reflectionProbePickerIcon =
        EditorGUIUtility.TrIconContent("ReflectionProbeSelector");
    public static readonly GUIContent lightmapEmissiveLabel =
        EditorGUIUtility.TrTextContent(
            "Global Illumination",
            "Controls if the emission is baked or realtime.\n\nBaked only has effect in scenes where baked global illumination is enabled.\n\nRealtime uses realtime global illumination if enabled in the scene. Otherwise the emission won't light up other objects.");
    ...
    public static int[] lightmapEmissiveValues = {
        (int) MaterialGlobalIlluminationFlags.RealtimeEmissive,
        (int) MaterialGlobalIlluminationFlags.BakedEmissive,
        (int) MaterialGlobalIlluminationFlags.None};
    ...
    public static readonly GUIContent queueLabel =
        EditorGUIUtility.TrTextContent("Render Queue");
    ...
    public static readonly GUIContent enableInstancingLabel =
        EditorGUIUtility.TrTextContent("Enable GPU Instancing");
    public static readonly GUIContent doubleSidedGILabel =
        EditorGUIUtility.TrTextContent(
            "Double Sided Global Illumination",
            "When enabled, the lightmapper accounts for both sides of the geometry when calculating Global Illumination. Backfaces are not rendered or added to lightmaps, but get treated as valid when seen from other objects. When using the Progressive Lightmapper backfaces bounce light using the same emission and albedo as frontfaces.");
    public static readonly GUIContent emissionLabel =
        EditorGUIUtility.TrTextContent("Emission");
  ...
  private static readonly List<MaterialEditor> s_MaterialEditors =
  ...
  private enum PreviewType { Mesh = 0, Plane = 1, Skybox = 2 }
  ...
  private static readonly GUIContent s_TilingText =
      EditorGUIUtility.TrTextContent("Tiling");
  private static readonly GUIContent s_OffsetText =
  ...
  public ShaderGUI customShaderGUI {
    get { return m_CustomShaderGUI; }
  }
  ...
  private struct AnimatedCheckData {
    ...
    public Color color;
    public AnimatedCheckData(MaterialProperty property, Rect totalPosition,
                             Color color) {
  ...
  private static Stack<AnimatedCheckData> s_AnimatedCheckStack =
      new Stack<AnimatedCheckData>();

  internal delegate void
  MaterialPropertyCallbackFunction(GenericMenu menu, MaterialProperty property,
                                   Renderer[] renderers);
  ...
  internal class ReflectionProbePicker : PopupWindowContent {
    ...
    public Transform Target {
      get {
        return m_SelectedReflectionProbe !=
               null ? m_SelectedReflectionProbe.transform : null;
      }
    }
    ...
    public void OnEnable() {
      m_SelectedReflectionProbe = EditorUtility.InstanceIDToObject(
          SessionState.GetInt("PreviewReflectionProbe", 0)) as ReflectionProbe;
    }

    public void OnDisable() {
      SessionState.SetInt("PreviewReflectionProbe",
                          m_SelectedReflectionProbe ? m_SelectedReflectionProbe
                              .GetInstanceID() : 0);
    }

    public override void OnGUI(Rect rc) {
      EditorGUILayout.LabelField("Select Reflection Probe",
                                 EditorStyles.boldLabel);
      EditorGUILayout.Space();
      m_SelectedReflectionProbe = EditorGUILayout.ObjectField(
          "", m_SelectedReflectionProbe, typeof(ReflectionProbe), true)
                                      as ReflectionProbe;
    }
  }
  ...
  public void SetShader(Shader shader) { SetShader(shader, true); }
  ...
  public void SetShader(Shader newShader, bool registerUndo) {
    ...
    foreach (Material material in targets) {
      ...
      if (m_CustomShaderGUI != null) {
        m_CustomShaderGUI.AssignNewShaderToMaterial(material, oldShader,
                                                    newShader);
      } else {
  ...
  private void ShaderPopup(GUIStyle style) {
    ...
    position = EditorGUI.PrefixLabel(position, 47385,
                                     EditorGUIUtility.TempContent("Shader"));
    EditorGUI.showMixedValue = HasMultipleMixedShaderValues();

    var buttonContent =
        EditorGUIUtility.TempContent(m_Shader != null ? m_Shader.name
                                     : "No Shader Selected");
    if (EditorGUI.DropdownButton(position, buttonContent, FocusType.Keyboard,
                                 style)) {
      var dropdown =
          new ShaderSelectionDropdown(m_Shader, OnSelectedShaderPopup);
  ...
  private class ShaderSelectionDropdown : AdvancedDropdown {
    ...
    public ShaderSelectionDropdown(Shader shader,
                                   Action<object> onSelectedShaderPopup)
    ...
    protected override AdvancedDropdownItem BuildRoot() {
      ...
      foreach (var shader in shaders) {
        if (shader.name.StartsWith("Deprecated") ||
      ...
      if (legacyList.Any() || notSupportedList.Any() ||
          failedCompilationList.Any())
        root.AddSeparator();
      legacyList.ForEach(s => AddShaderToMenu("", root, s, s));
      notSupportedList.ForEach(s => AddShaderToMenu("Not supported/", root, s,
                                                    "Not supported/" + s));
      failedCompilationList.ForEach(
          s => AddShaderToMenu("Failed to compile/", root, s,
                               "Failed to compile/" + s));
    ...
    private void AddShaderToMenu(string prefix, AdvancedDropdownItem parent,
                                 string fullShaderName, string shaderName) {
      var shaderNameParts = shaderName.Split('/');
      if (shaderNameParts.Length > 1) {
        AddShaderToMenu(prefix, FindOrCreateChild(parent, shaderName),
                        fullShaderName,
    ...
    private AdvancedDropdownItem FindOrCreateChild(AdvancedDropdownItem parent,
                                                   string path) {
    ...
    private class ShaderDropdownItem : AdvancedDropdownItem {
      ...
      public ShaderDropdownItem(string prefix, string fullName,
                                string shaderName)
          : base(shaderName) {
        m_FullName = fullName;
        m_Prefix = prefix;
    ...
    private class MaterialDropdownGUI : AdvancedDropdownGUI {
      ...
      internal override void DrawItem(AdvancedDropdownItem item, string name,
        ...
        if (hasSearch && newScriptItem != null) {
          name = string.Format("{0} ({1})", newScriptItem.name,
                               newScriptItem.prefix + newScriptItem.fullName);
        }
  ...
  private void DetectShaderEditorNeedsUpdate() {
    ...
    bool customEditorChanged =
        material && material.shader &&
        m_CustomEditorClassName != material.shader.customEditor;
  ...
  private void DetectTextureStackValidationIssues() {
    if (PlayerSettings.GetVirtualTexturingSupportEnabled()) {
      if (isVisible && m_Shader != null && !HasMultipleMixedShaderValues()) {
        // We want additional spacing, but only when the material properties are
        // visible
      ...
      var material = target as Material;
      StackValidationResult[] stackValidationResults =
          VirtualTexturing.EditorHelpers.ValidateMaterialTextureStacks(
              material);
      if (stackValidationResults.Length == 0)
        return;

      foreach (StackValidationResult validationResult in
                   stackValidationResults) {
  ...
  void CheckSetup() {
    ...
    if (m_CustomShaderGUI == null && !IsMaterialEditor(m_Shader.customEditor)) {
      Debug.LogWarningFormat(
          "Could not create a custom UI for the shader '{0}'. The shader has the following: 'CustomEditor = {1}'. Does the custom editor specified include its namespace? And does the class either derive from ShaderGUI or MaterialEditor?",
          m_Shader.name, m_Shader.customEditor);
    }
  }

  // A minimal list of settings to be shown in the Asset Store preview inspector
  internal override void OnAssetStoreInspectorGUI() { OnInspectorGUI(); }

  public void PropertiesChanged() {
    // @TODO: Show performance warnings for multi-selections too?
    m_InfoMessage = null;
    if (targets.Length == 1)
      m_InfoMessage = Utils.PerformanceChecks.CheckMaterial(
          target as Material, EditorUserBuildSettings.activeBuildTarget);
  }
  ...
  protected override void OnHeaderGUI() {
    ...
    Rect titleRect = DrawHeaderGUI(this, targetTitle, firstInspectedEditor ? 0
                                   : spaceForFoldoutArrow);
    int id = GUIUtility.GetControlID(45678, FocusType.Passive);

    if (!firstInspectedEditor) {
      Rect renderRect =
          EditorGUI.GetInspectorTitleBarObjectFoldoutRenderRect(titleRect);
      renderRect.y = titleRect.yMax - 17f; // align with bottom
      bool oldVisible = InternalEditorUtility.GetIsInspectorExpanded(target);
      bool newVisible = EditorGUI.DoObjectFoldout(oldVisible, titleRect,
                                                  renderRect, targets, id);
  ...
  internal override void OnHeaderControlsGUI() {
    ...
    using(new EditorGUI.DisabledScope(!IsEnabled())) {
      ...
      if (m_Shader != null && !HasMultipleMixedShaderValues() &&
          (m_Shader.hideFlags & HideFlags.DontSave) == 0) {
  ...
  public Color GetColor(string propertyName, out bool hasMixedValue) {
    ...
    for (int i = 1; i < targets.Length; i++)
      if (((Material) targets[i]).GetColor(propertyName) != f) {
        hasMixedValue = true;
        break;
      }
    return f;
  ...
  public Vector4 GetVector(string propertyName, out bool hasMixedValue) {
    ...
    for (int i = 1; i < targets.Length; i++)
      if (((Material) targets[i]).GetVector(propertyName) != f) {
        hasMixedValue = true;
        break;
      }
    return f;
  ...
  public Texture GetTexture(string propertyName, out bool hasMixedValue) {
    ...
    for (int i = 1; i < targets.Length; i++)
      if (((Material) targets[i]).GetTexture(propertyName) != f) {
        hasMixedValue = true;
        break;
      }
    return f;
  ...
  [System.Obsolete("Use MaterialProperty instead.")]
  public Vector2 GetTextureScale(string propertyName, out bool hasMixedValueX,
                                 out bool hasMixedValueY) {
  ...
  [System.Obsolete("Use MaterialProperty instead.")]
  public Vector2 GetTextureOffset(string propertyName, out bool hasMixedValueX,
                                  out bool hasMixedValueY) {
    hasMixedValueX = false;
    hasMixedValueY = false;
  ...
  // The 'Property' methods that accept GUIContent are internal with different
  // name to avoid breaking backwards compatibility caused by adding overloads:
  // 'RangeProperty(prop, null);' would cause a compile error because of
  // ambigious overloads.
  ...
  internal float RangePropertyInternal(MaterialProperty prop,
                                       GUIContent label) {
    Rect r = GetPropertyRect(prop, label, true);
    return RangePropertyInternal(r, prop, label);
  }

  public float RangeProperty(Rect position, MaterialProperty prop,
                             string label) {
    return RangePropertyInternal(position, prop, new GUIContent(label));
  }

  internal float RangePropertyInternal(Rect position, MaterialProperty prop,
                                       GUIContent label) {
    return DoPowerRangeProperty(position, prop, label, 1f);
  }

  internal static float DoPowerRangeProperty(Rect position,
                                             MaterialProperty prop,
                                             GUIContent label, float power) {
    EditorGUI.BeginChangeCheck();
    EditorGUI.showMixedValue = prop.hasMixedValue;

    // For range properties we want to show the slider so we adjust label width
    // to use default width (setting it to 0) See SetDefaultGUIWidths where we
    // set: EditorGUIUtility.labelWidth = GUIClip.visibleRect.width -
    // EditorGUIUtility.fieldWidth - 17;
    float oldLabelWidth = EditorGUIUtility.labelWidth;
    EditorGUIUtility.labelWidth = 0f;

    // fix for case 1245429 where we sometimes get a rounding issue when
    // converting between gamma and linear, which causes us to break the slider
    float value =
        Mathf.Clamp(prop.floatValue, prop.rangeLimits.x, prop.rangeLimits.y);

    float newValue = EditorGUI.PowerSlider(
        position, label, value, prop.rangeLimits.x, prop.rangeLimits.y, power);
    EditorGUI.showMixedValue = false;
  ...
  internal static int DoIntRangeProperty(Rect position, MaterialProperty prop,
                                         GUIContent label) {
    EditorGUI.BeginChangeCheck();
    EditorGUI.showMixedValue = prop.hasMixedValue;
    ...
    int newValue =
        EditorGUI.IntSlider(position, label, (int) prop.floatValue,
  ...
  internal float FloatPropertyInternal(MaterialProperty prop,
                                       GUIContent label) {
    Rect r = GetPropertyRect(prop, label, true);
    return FloatPropertyInternal(r, prop, label);
  }

  public float FloatProperty(Rect position, MaterialProperty prop,
                             string label) {
    return FloatPropertyInternal(position, prop, new GUIContent(label));
  }

  internal float FloatPropertyInternal(Rect position, MaterialProperty prop,
                                       GUIContent label) {
    EditorGUI.BeginChangeCheck();
    EditorGUI.showMixedValue = prop.hasMixedValue;
  ...
  internal Color ColorPropertyInternal(MaterialProperty prop,
                                       GUIContent label) {
    Rect r = GetPropertyRect(prop, label, true);
    return ColorPropertyInternal(r, prop, label);
  }

  public Color ColorProperty(Rect position, MaterialProperty prop,
                             string label) {
    return ColorPropertyInternal(position, prop, new GUIContent(label));
  }

  internal Color ColorPropertyInternal(Rect position, MaterialProperty prop,
                                       GUIContent label) {
    EditorGUI.BeginChangeCheck();
    EditorGUI.showMixedValue = prop.hasMixedValue;
  ...
  public Vector4 VectorProperty(Rect position, MaterialProperty prop,
                                string label) {
    EditorGUI.BeginChangeCheck();
    EditorGUI.showMixedValue = prop.hasMixedValue;

    // We want to make room for the field in case it's drawn on the same line as
    // the label Set label width to default width (zero) temporarily
    var oldLabelWidth = EditorGUIUtility.labelWidth;
    EditorGUIUtility.labelWidth = 0f;

    Vector4 newValue =
        EditorGUI.Vector4Field(position, label, prop.vectorValue);
  ...
  public void TextureScaleOffsetProperty(MaterialProperty property) {
    Rect rect = EditorGUILayout.GetControlRect(
        true, 2 * (kLineHeight + EditorGUI.kVerticalSpacingMultiField),
        EditorStyles.layerMaskField);
    TextureScaleOffsetProperty(rect, property, false);
  }

  // Returns height used
  public float TextureScaleOffsetProperty(Rect position,
  ...
  public float TextureScaleOffsetProperty(Rect position,
    ...
    EditorGUI.BeginChangeCheck();
    // Mixed value mask is 4 bits for the uv offset & scale (First bit is for
    // the texture itself)
    int mixedValuemask = property.mixedValueMask >> 1;
    Vector4 scaleAndOffset = TextureScaleOffsetProperty(
        position, property.textureScaleAndOffset, mixedValuemask,
  ...
  private Texture TexturePropertyBody(Rect position, MaterialProperty prop) {
    if (prop.type != MaterialProperty.PropType.Texture) {
      throw new ArgumentException(string.Format(
          "The MaterialProperty '{0}' should be of type 'Texture' (its type is '{1})'",
          prop.name, prop.type));
    }
    ...
    // If it's because object references can't be changed, shouldn't it be done
    // in ObjectField instead?
    bool wasEnabled = GUI.enabled;
    ...
    EditorGUI.showMixedValue = prop.hasMixedValue;
    int controlID =
        GUIUtility.GetControlID(12354, FocusType.Keyboard, position);
    var newValue = EditorGUI.DoObjectField(position, position, controlID,
                                           prop.textureValue, target, t,
                                           TextureValidator, false) as Texture;
    EditorGUI.showMixedValue = false;
    if (EditorGUI.EndChangeCheck())
  ...
  public Texture TextureProperty(MaterialProperty prop, string label) {
    bool scaleOffset =
        ((prop.flags & MaterialProperty.PropFlags.NoScaleOffset) == 0);
  ...
  public bool HelpBoxWithButton(GUIContent messageContent,
    ...
    Rect buttonRect = new Rect(contentRect.xMax - kButtonWidth - 4f,
                               contentRect.yMax - kButtonHeight - 4f,
  ...
  public void TextureCompatibilityWarning(MaterialProperty prop) {
    if (InternalEditorUtility.BumpMapTextureNeedsFixing(prop)) {
      if (HelpBoxWithButton(EditorGUIUtility.TrTextContent(
  ...
  public Texture TexturePropertyMiniThumbnail(Rect position,
                                              MaterialProperty prop,
                                              string label, string tooltip) {
    BeginAnimatedCheck(position, prop);
    Rect thumbRect, labelRect;
    EditorGUI.GetRectsForMiniThumbnailField(position, out thumbRect,
                                            out labelRect);
    EditorGUI.HandlePrefixLabel(position, labelRect,
                                new GUIContent(label, tooltip), 0,
                                EditorStyles.label);
    EndAnimatedCheck();
  ...
  public Texture TextureProperty(Rect position, MaterialProperty prop,
                                 string label) {
    bool scaleOffset =
        ((prop.flags & MaterialProperty.PropFlags.NoScaleOffset) == 0);
  ...
  public static Vector4 TextureScaleOffsetProperty(Rect position,
                                                   Vector4 scaleOffset) {
    return TextureScaleOffsetProperty(position, scaleOffset, 0, false);
  }

  public static Vector4
  TextureScaleOffsetProperty(Rect position, Vector4 scaleOffset,
                             bool partOfTexturePropertyControl) {
    return TextureScaleOffsetProperty(position, scaleOffset, 0,
                                      partOfTexturePropertyControl);
  }

  internal static Vector4
  TextureScaleOffsetProperty(Rect position, Vector4 scaleOffset,
                             int mixedValueMask,
                             bool partOfTexturePropertyControl) {
    ...
    // Temporarily reset the indent level as it was already used above to
    // compute the positions of the label and control. See issue 946082.
    ...
    if (partOfTexturePropertyControl) {
      ...
      position.y = position.yMax -
                   2 * kLineHeight; // align with large texture thumb bottom
    }

    // Tiling
    Rect labelRect = new Rect(labelStartX, position.y, labelWidth, kLineHeight);
    Rect valueRect = new Rect(controlStartX, position.y,
                              position.width - labelWidth, kLineHeight);
  ...
  public float GetPropertyHeight(MaterialProperty prop, string label) {
    ...
    MaterialPropertyHandler handler = MaterialPropertyHandler.GetHandler(
        ((Material) target).shader, prop.name);
    if (handler != null) {
      handlerHeight =
          handler.GetPropertyHeight(prop, label ?? prop.displayName, this);
      // if we have a property drawer (and not just decorators), exit now and
      // don't add default height
  ...
  private Rect GetPropertyRect(MaterialProperty prop, GUIContent label,
                               bool ignoreDrawer) {
    return GetPropertyRect(prop, label.text, ignoreDrawer);
  }

  private Rect GetPropertyRect(MaterialProperty prop, string label,
                               bool ignoreDrawer) {
    float handlerHeight = 0f;
    if (!ignoreDrawer) {
      MaterialPropertyHandler handler = MaterialPropertyHandler.GetHandler(
      ...
      if (handler != null) {
        ...
        // don't add default height
        if (handler.propertyDrawer != null)
          return EditorGUILayout.GetControlRect(true, handlerHeight,
                                                EditorStyles.layerMaskField);
      }
    }

    return EditorGUILayout.GetControlRect(
        true, handlerHeight + GetDefaultPropertyHeight(prop),
        EditorStyles.layerMaskField);
  }
  ...
  public void BeginAnimatedCheck(Rect totalPosition, MaterialProperty prop) {
    ...
    s_AnimatedCheckStack.Push(
        new AnimatedCheckData(prop, totalPosition, GUI.backgroundColor));

    Color overrideColor;
    if (MaterialAnimationUtility.OverridePropertyColor(
            prop, rendererForAnimationMode, out overrideColor))
  ...
  public void ShaderProperty(Rect position, MaterialProperty prop,
                             string label) {
  ...
  public void ShaderProperty(Rect position, MaterialProperty prop,
                             GUIContent label, int labelIndent) {
    BeginAnimatedCheck(position, prop);
    EditorGUI.indentLevel += labelIndent;

    // [PerRendererData] material properties are read-only as they are meant to
    // be set in code on a per-renderer basis.
    using(new EditorGUI.DisabledScope(
        (prop.flags & MaterialProperty.PropFlags.PerRendererData) != 0)) {
  ...
  public void LightmapEmissionProperty() { LightmapEmissionProperty(0); }

  public void LightmapEmissionProperty(int labelIndent) {
    Rect r = EditorGUILayout.GetControlRect(true, EditorGUI.kSingleLineHeight,
                                            EditorStyles.layerMaskField);
    LightmapEmissionProperty(r, labelIndent);
  }

  static MaterialGlobalIlluminationFlags
  GetGlobalIlluminationFlags(MaterialGlobalIlluminationFlags flags) {
    MaterialGlobalIlluminationFlags newFlags =
  ...
  public void LightmapEmissionProperty(Rect position, int labelIndent) {
    ...
    MaterialGlobalIlluminationFlags giFlags =
        GetGlobalIlluminationFlags(firstMaterial.globalIlluminationFlags);
    bool isMixed = false;
    for (int i = 1; i < materials.Length; i++) {
      Material material = (Material) materials[i];

      if (GetGlobalIlluminationFlags(material.globalIlluminationFlags) !=
          giFlags)
        isMixed = true;
    }
    ...
    giFlags = (MaterialGlobalIlluminationFlags) EditorGUI.IntPopup(
        position, Styles.lightmapEmissiveLabel, (int) giFlags,
        Styles.lightmapEmissiveStrings, Styles.lightmapEmissiveValues);
    EditorGUI.showMixedValue = false;

    // Apply flags. But only the part that this tool modifies (RealtimeEmissive,
    // BakedEmissive, None)
    if (EditorGUI.EndChangeCheck()) {
  ...
  public bool EmissionEnabledProperty() {
    Material[] materials =
        Array.ConvertAll(targets, (Object o) => { return (Material) o; });

    var settings = Lightmapping.GetLightingSettingsOrDefaultsFallback();

    MaterialGlobalIlluminationFlags defaultEnabled =
        settings.realtimeGI ? MaterialGlobalIlluminationFlags.RealtimeEmissive
        : (settings.bakedGI ? MaterialGlobalIlluminationFlags.BakedEmissive
           : MaterialGlobalIlluminationFlags.None);

    // Calculate isMixed
    bool enabled = materials[0].globalIlluminationFlags !=
                   MaterialGlobalIlluminationFlags.EmissiveIsBlack;
    bool isMixed = false;
    for (int i = 1; i < materials.Length; i++) {
      if ((materials[i].globalIlluminationFlags !=
           MaterialGlobalIlluminationFlags.EmissiveIsBlack) != enabled) {
        isMixed = true;
        break;
      }
    }
    ...
    if (EditorGUI.EndChangeCheck()) {
      foreach (Material mat in materials) {
        mat.globalIlluminationFlags = enabled ? defaultEnabled
            : MaterialGlobalIlluminationFlags.EmissiveIsBlack;
      }
      return enabled;
    }
  ...
  public static void FixupEmissiveFlag(Material mat) {
    ...
    mat.globalIlluminationFlags = FixupEmissiveFlag(
        mat.GetColor("_EmissionColor"), mat.globalIlluminationFlags);
  }

  public static MaterialGlobalIlluminationFlags
  FixupEmissiveFlag(Color col, MaterialGlobalIlluminationFlags flags) {
    if ((flags & MaterialGlobalIlluminationFlags.BakedEmissive) != 0 &&
        col.maxColorComponent == 0.0f) // flag black baked
  ...
  public void LightmapEmissionFlagsProperty(int indent, bool enabled,
    ...
    // Calculate isMixed
    MaterialGlobalIlluminationFlags any_em =
        MaterialGlobalIlluminationFlags.AnyEmissive;
    MaterialGlobalIlluminationFlags giFlags =
        materials[0].globalIlluminationFlags & any_em;
    bool isMixed = false;
    for (int i = 1; i < materials.Length; i++) {
      isMixed =
          isMixed || (materials[i].globalIlluminationFlags & any_em) != giFlags;
    }
    ...
    giFlags = (MaterialGlobalIlluminationFlags) EditorGUILayout.IntPopup(
        Styles.lightmapEmissiveLabel, (int) giFlags,
        Styles.lightmapEmissiveStrings, Styles.lightmapEmissiveValues);
    EditorGUI.indentLevel -= indent;
    EditorGUI.showMixedValue = false;
  ...
  void ShaderPropertyInternal(Rect position, MaterialProperty prop,
                              GUIContent label) {
    // Use custom property drawers if needed
    MaterialPropertyHandler handler = MaterialPropertyHandler.GetHandler(
        ((Material) target).shader, prop.name);
    if (handler != null) {
      handler.OnGUI(ref position, prop, label.text != null ? label
                    : new GUIContent(prop.displayName), this);
      // If we had a property drawer (and not just decorators), exit now and
      // don't draw default UI.
      if (handler.propertyDrawer != null)
        return;
    }
  ...
  internal void DefaultShaderPropertyInternal(MaterialProperty prop,
                                              GUIContent label) {
    Rect r = GetPropertyRect(prop, label, true);
    DefaultShaderPropertyInternal(r, prop, label);
  }

  public void DefaultShaderProperty(Rect position, MaterialProperty prop,
                                    string label) {
    DefaultShaderPropertyInternal(position, prop, new GUIContent(label));
  }

  internal void DefaultShaderPropertyInternal(Rect position,
                                              MaterialProperty prop,
                                              GUIContent label) {
    switch (prop.type) {
    ...
    default:
      GUI.Label(position,
                "Unknown property type: " + prop.name + ": " + (int) prop.type);
      break;
    }
  }
  ...
  public float RangeProperty(string propertyName, string label, float v2,
                             float v3) {
  ...
  public Texture TextureProperty(string propertyName, string label,
  ...
  [System.Obsolete(
      "Use ShaderProperty that takes MaterialProperty parameter instead.")]
  ...
  public static MaterialProperty GetMaterialProperty(Object[] mats,
                                                     string name) {
    if (mats == null)
      throw new ArgumentNullException("mats");
    if (Array.IndexOf(mats, null) >= 0)
  ...
  class ForwardApplyMaterialModification {
    ...
    public ForwardApplyMaterialModification(Renderer[] r,
                                            bool inIsMaterialEditable) {
      renderers = r;
      isMaterialEditable = inIsMaterialEditable;
    }

    public bool DidModifyAnimationModeMaterialProperty(
        MaterialProperty property, int changedMask, object previousValue) {
      bool didModify = false;
      foreach (Renderer renderer in renderers) {
        didModify =
            didModify | MaterialAnimationUtility
                            .ApplyMaterialModificationToAnimationRecording(
                                property, changedMask, renderer, previousValue);
      }
      ...
      // then we explicitly make sure that things that these properties arecan
      // not be recorded are not going to be applied to the material
  ...
  static Renderer[] GetAssociatedRenderersFromInspector() {
    ...
    if (imguicontainer != null) {
      var editorElement =
          imguicontainer.GetFirstAncestorOfType<IEditorElement>();
  ...
  internal static Renderer[] GetAssociatedRenderersFromEditors(
  ...
  PrepareMaterialPropertiesForAnimationMode(MaterialProperty[] properties,
                                            bool isMaterialEditable) {
    Renderer[] renderers = PrepareMaterialPropertiesForAnimationMode(
        properties, GetAssociatedRenderersFromInspector(), isMaterialEditable);
  ...
  PrepareMaterialPropertiesForAnimationMode(MaterialProperty[] properties,
                                            ...
                                            bool isMaterialEditable) {
    Renderer[] renderers = PrepareMaterialPropertiesForAnimationMode(
        properties, new Renderer[]{renderer}, isMaterialEditable);
    return (renderers != null && renderers.Length > 0) ? renderers[0] : null;
  }

  internal static Renderer[] PrepareMaterialPropertiesForAnimationMode(
      MaterialProperty[] properties, Renderer[] renderers,
      bool isMaterialEditable) {
    bool isInAnimationMode = AnimationMode.InAnimationMode();

    if (renderers != null && renderers.Length > 0) {
      var callback =
          new ForwardApplyMaterialModification(renderers, isMaterialEditable);
      ...
      foreach (MaterialProperty prop in properties) {
        ...
        if (isInAnimationMode)
          prop.applyPropertyCallback =
              callback.DidModifyAnimationModeMaterialProperty;
      }
    }
  ...
  public void SetDefaultGUIWidths() {
    ...
    EditorGUIUtility.labelWidth =
        GUIClip.visibleRect.width - EditorGUIUtility.fieldWidth - 25;
  }

  private bool IsMaterialEditor(string customEditorName) {
    string unityEditorFullName =
        "UnityEditor." +
        customEditorName; // for convenience: adding UnityEditor namespace is
                          // not needed in the shader
  ...
  void CreateCustomShaderEditorIfNeeded(Shader shader) {
    ...
    m_CustomShaderGUI =
        ShaderGUIUtility.CreateShaderGUI(m_CustomEditorClassName);
    // We need to delay checking setup because we need all loaded editor
    // assemblies which is not ready during package import. During package
    // import we create an Editor to generate a asset preview. (case 707328)
    m_CheckSetup = true;
  }

  public bool PropertiesGUI() {
    // OnInspectorGUI is wrapped inside a BeginVertical/EndVertical block that
    // adds padding, which we don't want here so we could have the VC bar span
    // the entire Material Editor width we stop the vertical block, draw the VC
    // bar, and then start a new vertical block with the same style.
    var style = GUILayoutUtility.topLevel.style;
    EditorGUILayout.EndVertical();

    // setting the GUI to enabled where the VC status bar is drawn because it
    // gets disabled by the parent inspector for non-checked out materials, and
    // we need the version control status bar to be always active
    bool wasGUIEnabled = GUI.enabled;
    GUI.enabled = true;

    // Material Editor is the first inspected editor when accessed through the
    // Project panel and this is the scenario where we do not want to redraw the
    // VC status bar since InspectorWindow already takes care of that.
    // Otherwise, the Material Editor is not the first inspected editor (e.g.
    // when it's a part of a GO Inspector) thus we draw the VC status bar
    if (!firstInspectedEditor) {
    ...
    // In animation mode we are actually animating the Renderer instead of the
    // material. Thus all properties are editable even if the material is not
    // editable.
    m_RenderersForAnimationMode = PrepareMaterialPropertiesForAnimationMode(
        props, GetAssociatedRenderersFromInspector(), GUI.enabled);
    bool wasEnabled = GUI.enabled;
    if (m_RenderersForAnimationMode != null)
      GUI.enabled = true;
    ...
    // properties gui in try/catch to catch the ExitGUIException thrown by
    // ExitGUI() to ensure our m_InsidePropertiesGUI flag is reset
    try {
      if (m_CustomShaderGUI != null)
      ...
      if (renderers != null && renderers.Length > 0) {
        if (Event.current.type == EventType.Layout) {
          renderers [0]
              .GetPropertyBlock(m_PropertyBlock);
        }
  ...
  public void PropertiesDefaultGUI(MaterialProperty[] props) {
    ...
    if (m_InfoMessage != null)
      ...
      // Hack to make sure that control IDs stay the same when the help box is
      // there or is not there. Otherwise, open color picker windows will not
      // keep synched to the same properties when the help box shows up or
      // disappears (case 566958)
      GUIUtility.GetControlID(s_ControlHash, FocusType.Passive,
                              new Rect(0, 0, 0, 0));
    ...
    for (var i = 0; i < props.Length; i++) {
      ...
      Rect r =
          EditorGUILayout.GetControlRect(true, h, EditorStyles.layerMaskField);
    ...
    if (UnityEngine.Rendering.SupportedRenderingFeatures.active
  ...
  ApplyMaterialPropertyDrawersFromNative(Material material) {
    var objs = new Object[]{material};
    ApplyMaterialPropertyDrawers(objs);
  }
    ...
    if (!EditorMaterialUtility.disableApplyMaterialPropertyDrawers) {
      ...
      for (var i = 0; i < props.Length; i++) {
        MaterialPropertyHandler handler =
            MaterialPropertyHandler.GetHandler(shader, props[i].name);
  ...
  private Object
  TextureValidator(Object[] references, System.Type objType,
                   SerializedProperty property,
                   EditorGUI.ObjectFieldValidatorOptions options) {
    foreach (Object i in references) {
      var t = i as Texture;
      if (t) {
        if (t.dimension == m_DesiredTexdim ||
            m_DesiredTexdim == UnityEngine.Rendering.TextureDimension.Any)
          return t;
      }
    }
    return null;
  ...
  private static readonly GUIContent[] s_MeshIcons = {null, null, null, null,
                                                      null};
  ...
  private void Init() {
    if (s_Meshes[0] == null) {
      var handleGo = (GameObject) EditorGUIUtility.LoadRequired(
          "Previews/PreviewMaterials.fbx");
      ...
      Mesh quadMesh =
          Resources.GetBuiltinResource(typeof(Mesh), "Quad.fbx") as Mesh;
      s_Meshes[4] = quadMesh;
  ...
  private bool DoReflectionProbePicker(out Rect buttonRect) {
    buttonRect = GUILayoutUtility.GetRect(Styles.reflectionProbePickerIcon,
                                          EditorStyles.toolbarDropDownRight);

    if (EditorGUI.DropdownButton(buttonRect, Styles.reflectionProbePickerIcon,
                                 FocusType.Passive,
                                 EditorStyles.toolbarDropDownRight))
      return true;
  ...
  public sealed override Texture2D RenderStaticPreview(string assetPath,
                                                       Object[] subAssets,
    ...
    if (PlayerSettings.GetVirtualTexturingSupportEnabled()) {
      foreach (var t in targets) {
          ...
          if (shader.GetPropertyType(i) ==
            ...
            if (shader.FindTextureStack(i, out stackName, out dummy)) {
              ...
              if (!stackTextures.ContainsKey(stackId)) {
                // Get the dimension of the texture stack. This can be different
                // from the texture dimensions.
                try {
                  int width, height;
                  VirtualTexturing.System.GetTextureStackSize(
                      mat, stackId, out width, out height);
        ...
        if (stackTextures.Count != 0) {
          //@TODO Poor mans prefetching. Remove once we request the mips
          //synchronously and are guaranteed that they are in the cache. Now we
          // need to update the VT system. We sleep to make sure any VT threads
          // (transcoder?) can pick up the work.

          const int numberOfVTUpdates =
              3; // We assume the texture data will be in the texture tile cache
                 // after this number of updates
          // Streaming texture mips for all the stacks so we have texture data
          // to render with
          for (int i = 0; i < numberOfVTUpdates; i++) {
            foreach (var item in stackTextures) {
              ...
              // Requesting the 256x256 mip and 128x128 mip so that their is
              // content in the cache to render with
              const int mipResolutionToRequest = 256;
              int mipToRequest = 0;

              if (maxDimension > mipResolutionToRequest) {
                float factor =
                    (float) maxDimension / (float) mipResolutionToRequest;
              ...
              VirtualTexturing.System.RequestRegion(
                  mat, stackId, new Rect(0, 0, 1, 1), mipToRequest, 2);
            }

            // 2 system updates per sleep to make sure we flush the VT system
            // while limiting sleeping.
  ...
  private void DoRenderPreview(PreviewRenderUtility previewRenderUtility,
                               bool overridePreviewMesh = false) {
    if (previewRenderUtility.renderTexture.width <= 0 ||
        previewRenderUtility.renderTexture.height <= 0)
      return;
    ...
    if (m_LightMode == 0) {
      ...
      previewRenderUtility.lights[0].transform.rotation =
          Quaternion.Euler(30f, 30f, 0);
      previewRenderUtility.lights[1].intensity = 0;
    } else {
      previewRenderUtility.lights[0].intensity = 1.0f;
      previewRenderUtility.lights[0].transform.rotation =
          Quaternion.Euler(50f, 50f, 0);
    ...
    if (DoesPreviewAllowRotation(viewType))
      rot = Quaternion.Euler(m_PreviewDir.y, 0, 0) *
            Quaternion.Euler(0, m_PreviewDir.x, 0);
    ...
    case PreviewType.Mesh:
      // We need to rotate camera, so we can see different reflections from
      // different angles If we would only rotate object, the reflections would
      // stay the same
    ...
    if (mesh != null) {
      previewRenderUtility.DrawMesh(mesh, Vector3.zero, rot, mat, 0, null,
                                    m_ReflectionProbePicker.Target, false);
    }

    previewRenderUtility.Render(true);
    if (viewType == PreviewType.Skybox) {
      InternalEditorUtility.DrawSkyboxMaterial(mat,
                                               previewRenderUtility.camera);
    }
  }

  public sealed override bool HasPreviewGUI() { return true; }

  public override bool RequiresConstantRepaint() { return m_TimeUpdate == 1; }
  ...
  private static PreviewRenderUtility GetPreviewRendererUtility() {
    if (s_PreviewRenderUtility == null) {
      ...
      EditorUtility.SetCameraAnimateMaterials(s_PreviewRenderUtility.camera,
                                              true);
    }
  ...
  public virtual void OnEnable() {
    ...
    m_EnableInstancing =
        serializedObject.FindProperty("m_EnableInstancingVariants");
  ...
  public virtual void UndoRedoPerformed() {
    // Undo could've restored old shader which might lead to change in custom
    // editor class therefore we need to rebuild inspector
    UpdateAllOpenMaterialEditors();
  ...
  internal void OnSceneDrag(SceneView sceneView, int index) {
    ...
    if (EditorMaterialUtility.IsBackgroundMaterial((target as Material))) {
      ...
      HandleRenderer(go.GetComponent<Renderer>(), materialIndex,
                     target as Material, evt.type, evt.alt);
    else
      ClearDragMaterialRendering();
  }
    ...
    if (s_previousDraggedUponRenderer != null) {
      ...
      if (!s_previousAlreadyHadPrefabModification &&
          PrefabUtility.GetPrefabInstanceStatus(
              s_previousDraggedUponRenderer) ==
              PrefabInstanceStatus.Connected) {
        var materialRendererSerializedObject =
            new SerializedObject(s_previousDraggedUponRenderer)
                .FindProperty("m_Materials");
        PrefabUtility.RevertPropertyOverride(materialRendererSerializedObject,
  ...
  internal void HandleSkybox(GameObject go, Event evt) {
    ...
    if (applyAndConsumeEvent) {
      if (s_OriginalMaterial == null) {
        Undo.RecordObject(FindObjectOfType<RenderSettings>(),
                          Styles.undoAssignSkyboxMaterial);
  ...
  internal static void HandleRenderer(Renderer r, int materialIndex,
                                      Material dragMaterial,
                                      EventType eventType, bool alt) {
    if (r.GetType()
            .GetCustomAttributes(typeof(RejectDragAndDropMaterial), true)
            .Length > 0)
      return;
    ...
    if (applyMaterial) {
      if (eventType != EventType.DragPerform) {
        ...
        if (PrefabUtility.GetPrefabInstanceStatus(
                s_previousDraggedUponRenderer) ==
            PrefabInstanceStatus.Connected) {
          var materialRendererSerializedObject =
              new SerializedObject(s_previousDraggedUponRenderer)
          ...
          s_previousAlreadyHadPrefabModification =
              materialRendererSerializedObject.prefabOverride;
        }
      }
      ...
      bool isValidMaterialIndex =
          (materialIndex >= 0 && materialIndex < r.sharedMaterials.Length);
      ...
      // Since we can handle multiple objects being dragged, we cannot use the
      // event here. This will fall under respective view message processing
      // responsibilities.
    }
  }

  internal override bool HasLargeHeader() { return true; }
