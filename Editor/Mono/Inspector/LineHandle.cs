...
class LineHandle {
  ...
  public LineHandle(IList<Vector2> points, bool loop,
                    LineIntersectionHighlight intersectionHighlight =
                        LineIntersectionHighlight.None) {
  ...
  public LineHandle(IList<Vector3> points, bool loop,
                    LineIntersectionHighlight intersectionHighlight =
                        LineIntersectionHighlight.None) {
    int count = points.Count;
    m_Points = new Vector3[count];
  ...
  public void OnGUI(Vector3 handleDir, Vector3 slideDir1, Vector3 slideDir2) {
    Do(GUIUtility.GetControlID(s_LineControlHash, FocusType.Passive),
       ref m_Points, handleDir, slideDir1, slideDir2, k_DotHandleSize,
       Handles.DotHandleCap, EditorSnapSettings.move, false,
       m_IntersectionHighlight, m_Loop);

    for (int i = 0, c = m_Points.Length; i < c; i++) {
      float guiSize =
          HandleUtility.GetHandleSize(m_Points[i]) * k_ProximityDotHandleSize;
      m_Points[i] = Handles.Slider2D(
          m_Points[i], Vector3.forward, Vector3.right, Vector3.up, guiSize,
          ProximityDotHandleCap, EditorSnapSettings.move);
    }

    HandleUtility.s_CustomPickDistance = HandleUtility.kPickDistance;
  }

  static void ProximityDotHandleCap(int controlID, Vector3 position,
                                    Quaternion rotation, float size,
                                    EventType eventType) {
    // Draw the handle cap only if it is the hotcontrol or is the nearest
    // control with no hotcontrol active.
  ...
  public static void Do(ref Vector3[] line, Vector3 handleDir,
                        Vector3 slideDir1, Vector3 slideDir2, float handleSize,
                        Handles.CapFunction capFunction,
                        LineIntersectionHighlight intersectionHighlight,
                        bool loop = false) {
    int id = GUIUtility.GetControlID(s_InsertPointHash, FocusType.Passive);
    Do(id, ref line, handleDir, slideDir1, slideDir2, handleSize, capFunction,
       EditorSnapSettings.move, false, intersectionHighlight, loop);
  }

  public static void Do(int id, ref Vector3[] line, Vector3 handleDir,
                        Vector3 slideDir1, Vector3 slideDir2, float handleSize,
                        Handles.CapFunction capFunction, Vector2 snap,
                        bool drawHelper,
                        LineIntersectionHighlight intersectionHighlight,
                        bool loop) {
    ...
    case EventType.MouseMove: {
      ...
      if (line != null && line.Length > 1)
        HandleUtility.AddControl(
            id, HandleUtility.DistanceToPolyLine(line, loop, out _));
      break;
    }
    ...
    case EventType.Repaint: {
      ...
      for (int n = 0, lineIterCount = loop ? line.Length : line.Length - 1;
           n < lineIterCount; n++) {
        int a = n, b = Wrap(n + 1, lineCount);

        // Inserting a point has special handling to draw the inserted lines
        // thicker, and optionally draw a red warning line if intersections are
        // detected
        if (activeIndex > -1 &&
            RangeContains(n, lineCount, activeIndex - 1, 2)) {
          Handles.color =
              intersectionHighlight != LineIntersectionHighlight.None &&
                      LineIntersectsPath(line, line[a], line[b], slideDir1,
                                         slideDir2, loop, a - 1, 3)
                  ? Color.red
                  : color;
          ...
          Handles.DrawAAPolyLine(k_ActiveLineSegmentWidth, s_AAPolyLinePoints);
            ...
            // Draw the hovered line segment with a thicker line. If this is a
            // remove event, we also handle drawing the next segment.
            if (GUIUtility.hotControl == 0 &&
                HandleUtility.nearestControl == id &&
                (a == hoveredEdgeIndex ||
                 (IsRemoveEvent(evt) && a == hoveredEdgeIndex + 1))) {
          Handles.color =
              intersectionHighlight == LineIntersectionHighlight.Always &&
                      LineIntersectsPath(line, line[a], line[b], slideDir1,
                                         slideDir2, loop, a - 1, 3)
                  ? Color.red
                  : color;

          // If we're in 'delete' mode, both the hovered edge and the following
          // edge are highlighted as removal candidates
          if (IsRemoveEvent(evt)) {
            Handles.color = Color.red;
            Handles.DrawAAPolyLine(k_ActiveLineSegmentWidth,
                                   line[hoveredEdgeIndex],
                                   line[Wrap(hoveredEdgeIndex + 1, lineCount)]);
            Handles.DrawAAPolyLine(
                k_ActiveLineSegmentWidth,
                line[Wrap(hoveredEdgeIndex + 1, lineCount)],
                line [loop ? Wrap(hoveredEdgeIndex + 2, lineCount
                                 ):Mathf.Clamp(hoveredEdgeIndex + 2, 0,
                                               lineCount - 1)]);
          } else {
      ...
      // If no hotControl is active and we are the nearest control, show a
      // preview of the insertion point
      if (GUIUtility.hotControl != 0 || HandleUtility.nearestControl != id ||
          capFunction == null)
        break;

      var ls = Handles.matrix.MultiplyPoint3x4(
          line[Wrap(hoveredEdgeIndex, lineCount)]);
      var le = Handles.matrix.MultiplyPoint3x4(
          line[Wrap(hoveredEdgeIndex + 1, lineCount)]);

      var direction = le - ls;
      var constraint = Vector3.Normalize(direction);

      if (HandleUtility.CalcParamOnConstraint(Camera.current, evt.mousePosition,
                                              ls, constraint,
                                              out float param)) {
        var handlePosition =
            ls + constraint * Mathf.Clamp(param, 0f, direction.magnitude);
        var localSpace = Handles.inverseMatrix.MultiplyPoint3x4(handlePosition);
        capFunction(id, localSpace, Quaternion.identity,
    ...
    case EventType.MouseDown: {
      if (HandleUtility.nearestControl == id && evt.button == 0 &&
          GUIUtility.hotControl == 0 && !evt.alt) {
        ...
        var le = Handles.matrix.MultiplyPoint3x4(
            line[(nearestEdgeIndex + 1) % count]);
        activeIndex = nearestEdgeIndex;

        var direction = le - ls;
        var constraint = Vector3.Normalize(direction);

        if (!HandleUtility.CalcParamOnConstraint(Camera.current,
                                                 evt.mousePosition, ls,
                                                 constraint, out float param))
          break;

        var handlePosition =
            ls + constraint * Mathf.Clamp(param, 0f, direction.magnitude);
        s_InsertedPointPosition =
            Handles.inverseMatrix.MultiplyPoint(handlePosition);

        if (activeIndex != -1) {
          s_InsertedIndex = (activeIndex + 1) % line.Length;
          ArrayUtility.Insert(ref line, s_InsertedIndex,
                              s_InsertedPointPosition);
          GUI.changed = true;
        }
    ...
    default: {
      s_InsertedPointPosition = Slider2D.Do(
          id, s_InsertedPointPosition, Vector3.zero, handleDir, slideDir1,
          slideDir2,
  ...
  static int Wrap(int i, int c) { return ((i % c) + c) % c; }

  static bool RangeContains(int index, int count, int rangeStart,
                            int rangeCount) {
    return (index >= rangeStart && index < rangeStart + rangeCount) ||
           rangeStart + rangeCount >= count &&
               index < Wrap(rangeStart + rangeCount, count);
  }

  internal static bool LineIntersectsPath(Vector3[] path, Vector3 lineA,
                                          Vector3 lineB, Vector3 slideDir1,
                                          Vector3 slideDir2, bool loop,
                                          int ignoreIndex, int ignoreCount) {
    ...
    for (int i = 1, c = path.Length; i < (loop ? c + 1 : c); i++) {
      ...
      if (!RangeContains(edge, c, ignoreIndex, ignoreCount) &&
          GetLineSegmentIntersect(a, b, prev, next))
        return true;
      prev = next;
    }
  ...
  internal static bool GetLineSegmentIntersect(Vector2 p0, Vector2 p1,
