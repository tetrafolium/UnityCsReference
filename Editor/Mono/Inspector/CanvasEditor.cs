...
public static GUIContent eventCamera = EditorGUIUtility.TrTextContent(
	"Event Camera",
	"The Camera which the events are triggered through. This is used to determine clicking and hover positions if the Canvas is in World Space render mode.");
public static GUIContent renderCamera = EditorGUIUtility.TrTextContent(
	"Render Camera",
	"The Camera which will render the canvas. This is also the camera used to send events.");
public static GUIContent sortingOrder = EditorGUIUtility.TrTextContent(
	"Sort Order",
	"The order in which Screen Space - Overlay canvas will render");
public static string s_RootAndNestedMessage =
	"Cannot multi-edit root Canvas together with nested Canvas.";
public static GUIContent m_SortingLayerStyle =
	EditorGUIUtility.TrTextContent("Sorting Layer",
	                               "Name of the Renderer's sorting layer");
public static GUIContent targetDisplay = EditorGUIUtility.TrTextContent(
	"Target Display",
	"Display on which to render the canvas when in overlay mode");
public static GUIContent m_SortingOrderStyle =
	EditorGUIUtility.TrTextContent(
		"Order in Layer", "Renderer's order within a sorting layer");
public static GUIContent m_ShaderChannel =
	EditorGUIUtility.TrTextContent("Additional Shader Channels");
}
...
private string[] shaderChannelOptions = {"TexCoord1", "TexCoord2",
	                                 "TexCoord3", "Normal", "Tangent"};

enum PixelPerfect { Inherit, On, Off }
...
void OnEnable() {
	...
	m_PixelPerfectOverride =
		serializedObject.FindProperty("m_OverridePixelPerfect");
	m_ShaderChannels =
		serializedObject.FindProperty("m_AdditionalShaderChannelsFlag");
	...
	if (m_PixelPerfectOverride.boolValue)
		pixelPerfect =
			m_PixelPerfect.boolValue ? PixelPerfect.On : PixelPerfect.Off;
	else
...
private void AllRootCanvases() {
	if (PlayerSettings.virtualRealitySupported &&
		...
		EditorGUILayout.HelpBox(
			"Using a render mode of ScreenSpaceOverlay while VR is enabled will cause the Canvas to continue to incur a rendering cost, even though the Canvas will not be visible in VR.",
			MessageType.Warning);
	}
	...
	if (EditorGUILayout.BeginFadeGroup(m_OverlayMode.faded)) {
		...
		EditorGUILayout.IntPopup(m_TargetDisplay, displayNames,
	...
	if (EditorGUILayout.BeginFadeGroup(m_CameraMode.faded)) {
		...
		if (m_Camera.objectReferenceValue != null)
			EditorGUILayout.SortingLayerField(Styles.m_SortingLayerStyle,
			                                  m_SortingLayerID, EditorStyles.popup,
			                                  EditorStyles.label);
...
private void AllNestedCanvases() {
	...
	pixelPerfect =
		(PixelPerfect) EditorGUILayout.EnumPopup("Pixel Perfect", pixelPerfect);
...
public override void OnInspectorGUI() {
	...
	if (m_AllRoot || m_AllNested) {
		...
		EditorGUI.BeginChangeCheck();
		newShaderChannelValue = EditorGUILayout.MaskField(
			Styles.m_ShaderChannel, m_ShaderChannels.intValue,
			shaderChannelOptions);
		...
		{
			if (((newShaderChannelValue &
			      (int) AdditionalCanvasShaderChannels.Normal) |
			     (newShaderChannelValue &
			      (int) AdditionalCanvasShaderChannels.Tangent)) != 0)
				EditorGUILayout.HelpBox(
