...
public class UnityEventDrawer : PropertyDrawer {
  ...
  internal const string kObjectArgumentAssemblyTypeName =
  ...
  static string GetEventParams(UnityEventBase evt) {
    var methodInfo = evt.FindMethod("Invoke", evt.GetType(),
  ...
  private State GetState(SerializedProperty prop) {
    ...
    if (state == null ||
        state.m_ReorderableList.serializedProperty.serializedObject !=
            prop.serializedObject) {
      if (state == null)
        state = new State();

      SerializedProperty listenersArray =
          prop.FindPropertyRelative("m_PersistentCalls.m_Calls");
      state.m_ReorderableList = new ReorderableList(
          prop.serializedObject, listenersArray, false, true, true,
          true){drawHeaderCallback = DrawEventHeader,
  ...
  public override float GetPropertyHeight(SerializedProperty property,
                                          GUIContent label) {
    // TODO: Also we need to have a constructor or initializer called for this
    // property Drawer, before OnGUI or GetPropertyHeight otherwise, we get
    // Restore the State twice, once here and again in OnGUI. Maybe we should
    // only do it here?
  ...
  protected virtual void SetupReorderableList(ReorderableList list) {
    // Two standard lines with standard spacing between and extra spacing below
    // to better separate items visually.
    list.elementHeight = EditorGUI.kSingleLineHeight * 2 +
                         EditorGUI.kControlVerticalSpacing + kExtraSpacing;
  }

  protected virtual void DrawEventHeader(Rect headerRect) {
    headerRect.height = EditorGUI.kSingleLineHeight;
    string text = (string.IsNullOrEmpty(m_Text) ? "Event" : m_Text) +
                  GetEventParams(m_DummyEvent);
  ...
  protected virtual void DrawEvent(Rect rect, int index, bool isActive,
                                   bool isFocused) {
    ...
    var desiredArgTypeName =
        arguments.FindPropertyRelative(kObjectArgumentAssemblyTypeName)
            .stringValue;
    ...
    if (modeEnum == PersistentListenerMode.Object) {
      EditorGUI.BeginChangeCheck();
      var result = EditorGUI.ObjectField(argRect, GUIContent.none,
                                         argument.objectReferenceValue,
                                         desiredType, true);
      if (EditorGUI.EndChangeCheck())
        argument.objectReferenceValue = result;
    } else if (modeEnum != PersistentListenerMode.Void &&
               modeEnum != PersistentListenerMode.EventDefined)
      EditorGUI.PropertyField(argRect, argument, GUIContent.none);

    using(new EditorGUI.DisabledScope(listenerTarget.objectReferenceValue ==
                                      null)) {
      EditorGUI.BeginProperty(functionRect, GUIContent.none, methodName);
      {
        ...
        if (EditorGUI.showMixedValue) {
          ...
          if (listenerTarget.objectReferenceValue == null ||
          ...
          } else if (!IsPersistantListenerValid(
                         m_DummyEvent, methodName.stringValue,
                         listenerTarget.objectReferenceValue, GetMode(mode),
                         desiredType)) {
            ...
            buttonLabel.Append(string.Format(
                "<Missing {0}.{1}>", instanceString, methodName.stringValue));
          } else {
            buttonLabel.Append(
                listenerTarget.objectReferenceValue.GetType().Name);
        ...
        if (GUI.Button(functionRect, buttonContent, EditorStyles.popup))
          BuildPopupList(listenerTarget.objectReferenceValue, m_DummyEvent,
                         pListener)
  ...
  Rect[] GetRowRects(Rect rect) {
    ...
    goRect.y +=
        EditorGUIUtility.singleLineHeight + EditorGUI.kControlVerticalSpacing;
  ...
  protected virtual void OnAddEvent(ReorderableList list) {
    if (m_ListenersArray.hasMultipleDifferentValues) {
      ...
      // The Serialization system applies the Serialized data of one object, to
      // all other objects in the selection. We handle this case here, by
      // creating a SerializedObject for each object. Case 639025.
      foreach (var targetObject in m_ListenersArray.serializedObject
                   .targetObjects) {
        using(var temSerialziedObject = new SerializedObject(targetObject)) {
          var listenerArrayProperty =
              temSerialziedObject.FindProperty(m_ListenersArray.propertyPath);
  ...
  static UnityEventBase GetDummyEvent(SerializedProperty prop) {
    // Use the SerializedProperty path to iterate through the fields of the
    // inspected targetObject
    ...
    do {
      ...
      // no need to look for public members again since the base type covered
      // that
      bindflags = BindingFlags.Instance | BindingFlags.NonPublic;
      ft = ft.BaseType;
    } while (ret == null && ft != null);
    // go up the class hierarchy if it exists and the property is not found on
    // the child
    return (ret == null) ? new UnityEvent() : ret;
  }

  private static UnityEventBase GetDummyEventHelper(string propPath,
                                                    Type targetObjectType,
                                                    BindingFlags flags) {
    if (targetObjectType == null)
      return null;
    while (propPath.Length != 0) {
      // we could have a leftover '.' if the previous iteration handled an array
      // element
  ...
  static IEnumerable<ValidMethodMap> CalculateMethodMap(Object target, Type[] t,
    ...
    var componentMethods =
        componentType.GetMethods().Where(x => !x.IsSpecialName).ToList();

    var wantedProperties = componentType.GetProperties().AsEnumerable();
    wantedProperties = wantedProperties.Where(
        x => x.GetCustomAttributes(typeof(ObsoleteAttribute), true).Length ==
                 0 &&
             x.GetSetMethod() != null);
    ...
    foreach (var componentMethod in componentMethods) {
      ...
      if (componentMethod.GetCustomAttributes(typeof(ObsoleteAttribute), true)
              .Length > 0)
        continue;
      ...
      for (int i = 0; i < t.Length; i++) {
        ...
        if (allowSubclasses &&
            t [i]
                .IsAssignableFrom(componentParamaters[i].ParameterType))
      ...
      if (paramatersMatch) {
        var vmm =
            new ValidMethodMap{target = target, methodInfo = componentMethod};
  ...
  public static bool IsPersistantListenerValid(UnityEventBase dummyEvent,
    ...
    return dummyEvent.FindMethod(methodName, uObject.GetType(), modeEnum,
                                 argumentType) != null;
  }

  static GenericMenu BuildPopupList(Object target, UnityEventBase dummyEvent,
                                    SerializedProperty listener) {
    // special case for components... we want all the game objects targets
    // there!
    ...
    menu.AddItem(new GUIContent(kNoFunctionString),
                 string.IsNullOrEmpty(methodName.stringValue),
                 ClearEventFunction,
                 new UnityEventFunction(listener, null, null,
    ...
    // The property at this stage points to the 'container' and has the field
    // name
    Type delegateType = dummyEvent.GetType();

    // check out the signature of invoke as this is the callback!
    MethodInfo delegateMethod = delegateType.GetMethod("Invoke");
    var delegateArgumentsTypes =
        delegateMethod.GetParameters().Select(x => x.ParameterType).ToArray();

    GeneratePopUpForType(menu, targetToUse, false, listener,
                         delegateArgumentsTypes);
    if (targetToUse is GameObject) {
      Component[] comps =
          (targetToUse as GameObject).GetComponents<Component>();
      ...
      foreach (Component comp in comps) {
        ...
        GeneratePopUpForType(menu, comp,
                             duplicateNames.Contains(comp.GetType().Name),
                             listener, delegateArgumentsTypes);
      }
    }
  ...
  private static void GeneratePopUpForType(GenericMenu menu, Object target,
                                           bool useFullTargetName,
                                           SerializedProperty listener,
                                           Type[] delegateArgumentsTypes) {
    var methods = new List<ValidMethodMap>();
    string targetName =
        useFullTargetName ? target.GetType().FullName : target.GetType().Name;
    ...
    if (delegateArgumentsTypes.Length != 0) {
      ...
      if (methods.Count > 0) {
        menu.AddDisabledItem(new GUIContent(
            ...
            string.Join(
                ", ",
                delegateArgumentsTypes.Select(e => GetTypeName(e)).ToArray())));
    ...
    methods.Clear();
    GetMethodsForTargetAndMode(target, new[]{typeof(float)}, methods,
                               PersistentListenerMode.Float);
    GetMethodsForTargetAndMode(target, new[]{typeof(int)}, methods,
                               PersistentListenerMode.Int);
    GetMethodsForTargetAndMode(target, new[]{typeof(string)}, methods,
                               PersistentListenerMode.String);
    GetMethodsForTargetAndMode(target, new[]{typeof(bool)}, methods,
                               PersistentListenerMode.Bool);
    GetMethodsForTargetAndMode(target, new[]{typeof(Object)}, methods,
                               PersistentListenerMode.Object);
    GetMethodsForTargetAndMode(target, new Type[]{}, methods,
                               PersistentListenerMode.Void);
    if (methods.Count > 0) {
      if (didAddDynamic)
        // AddSeperator doesn't seem to work for sub-menus, so we have to use
        // this workaround instead of a proper separator for now.
  ...
  private static void AddMethodsToMenu(GenericMenu menu,
                                       SerializedProperty listener,
                                       List<ValidMethodMap> methods,
                                       string targetName) {
    // Note: sorting by a bool in OrderBy doesn't seem to work for some reason,
    // so using numbers explicitly.
    IEnumerable<ValidMethodMap> orderedMethods =
        methods.OrderBy(e => e.methodInfo.Name.StartsWith("set_") ? 0 : 1)
  ...
  private static void GetMethodsForTargetAndMode(Object target,
                                                 Type[] delegateArgumentsTypes,
                                                 List<ValidMethodMap> methods,
                                                 PersistentListenerMode mode) {
    IEnumerable<ValidMethodMap> newMethods = CalculateMethodMap(
        target, delegateArgumentsTypes, mode == PersistentListenerMode.Object);
  ...
  static void AddFunctionsForScript(GenericMenu menu,
                                    SerializedProperty listener,
                                    ValidMethodMap method, string targetName) {
    PersistentListenerMode mode = method.mode;

    // find the current event target...
    var listenerTarget =
        listener.FindPropertyRelative(kInstancePath).objectReferenceValue;
    ...
    var typeName = listener.FindPropertyRelative(kArgumentsPath)
                       .FindPropertyRelative(kObjectArgumentAssemblyTypeName);
    ...
    if (isCurrentlySet && mode == PersistentListenerMode.Object &&
        method.methodInfo.GetParameters().Length == 1) {
      isCurrentlySet &=
          (method.methodInfo.GetParameters()[0]
               .ParameterType.AssemblyQualifiedName == typeName.stringValue);
    }

    string path = GetFormattedMethodName(
        targetName, method.methodInfo.Name, args.ToString(),
        mode == PersistentListenerMode.EventDefined);
    menu.AddItem(new GUIContent(path), isCurrentlySet, SetEventFunction,
                 new UnityEventFunction(listener, method.target,
                                        method.methodInfo, mode));
  }
  ...
  static string GetFormattedMethodName(string targetName, string methodName,
                                       string args, bool dynamic) {
    if (dynamic) {
      ...
      if (methodName.StartsWith("set_"))
        return string.Format("{0}/{2} {1}", targetName, methodName.Substring(4),
                             args);
      else
  ...
  struct UnityEventFunction {
    ...
    public UnityEventFunction(SerializedProperty listener, Object target,
                              MethodInfo method, PersistentListenerMode mode) {
      m_Listener = listener;
      m_Target = target;
      m_Method = method;
      m_Mode = mode;
    ...
    public void Assign() {
      ...
      var listenerTargetType =
          m_Listener.FindPropertyRelative(kInstanceTypePath);
      ...
      listenerTargetType.stringValue =
          m_Method.DeclaringType.AssemblyQualifiedName;
      methodName.stringValue = m_Method.Name;
      mode.enumValueIndex = (int) m_Mode;

      if (m_Mode == PersistentListenerMode.Object) {
        var fullArgumentType =
            arguments.FindPropertyRelative(kObjectArgumentAssemblyTypeName);
        var argParams = m_Method.GetParameters();
        if (argParams.Length == 1 &&
            typeof(Object).IsAssignableFrom(argParams[0].ParameterType))
          fullArgumentType.stringValue =
              argParams[0].ParameterType.AssemblyQualifiedName;
        else
    ...
    private void ValidateObjectParamater(SerializedProperty arguments,
                                         PersistentListenerMode mode) {
      var fullArgumentType =
          arguments.FindPropertyRelative(kObjectArgumentAssemblyTypeName);
      ...
      if (!typeof(Object).IsAssignableFrom(t) ||
          !t.IsInstanceOfType(argumentObj))
        argument.objectReferenceValue = null;
    }
