...
private enum SunDiskMode { None, Simple, HighQuality }
...
public override void OnGUI(MaterialEditor materialEditor,
	...
	for (var i = 0; i < props.Length; i++) {
		...
		if ((props[i].name == "_SunSizeConvergence") &&
		    (sunDiskMode != SunDiskMode.HighQuality))
			continue;

		float h =
			materialEditor.GetPropertyHeight(props[i], props[i].displayName);
		Rect r =
