...
internal abstract class TabbedEditor : Editor {
  ...
  internal virtual void OnEnable() {
    m_ActiveEditorIndex =
        EditorPrefs.GetInt(GetType().Name + "ActiveEditorIndex", 0);
    if (m_ActiveEditor == null)
      m_ActiveEditor =
          CreateEditor(targets, m_SubEditorTypes[m_ActiveEditorIndex]);
  }

  void OnDestroy() { DestroyImmediate(activeEditor); }

  public override void OnInspectorGUI() {
    using(new GUILayout.HorizontalScope()) {
      GUILayout.FlexibleSpace();
      using(var check = new EditorGUI.ChangeCheckScope()) {
        m_ActiveEditorIndex = GUILayout.Toolbar(
            m_ActiveEditorIndex, m_SubEditorNames, "LargeButton",
  ...
  public override void OnPreviewSettings() { activeEditor.OnPreviewSettings(); }

  public override void OnInteractivePreviewGUI(Rect r, GUIStyle background) {
    activeEditor.OnInteractivePreviewGUI(r, background);
  }

  public override bool HasPreviewGUI() { return activeEditor.HasPreviewGUI(); }
}
}
