...
static class Styles {
static public readonly GUIContent defaultPlatform =
	EditorGUIUtility.TrTextContent("Default");
static public readonly GUIContent overrideFor =
	EditorGUIUtility.TrTextContent("Override For {0}");
}

public abstract bool textureTypeHasMultipleDifferentValues {
	get;
}
public abstract TextureImporterType textureType {
	get;
}
...
public abstract TextureImportPlatformSettingsData model {
	get;
}
public abstract bool DoesSourceTextureHaveAlpha(int i);
public abstract TextureImporterPlatformSettings
GetPlatformTextureSettings(int i, string name);
public abstract BaseTextureImportPlatformSettings GetDefaultImportSettings();
public abstract void
SetPlatformTextureSettings(int i,
...
protected void Init() {
	for (int i = 0; i < GetTargetCount(); i++) {
		TextureImporterPlatformSettings curPlatformSettings =
			GetPlatformTextureSettings(i, model.platformTextureSettings.name);
		if (i == 0) {
			model.platformTextureSettings = curPlatformSettings;
		} else {
			if (curPlatformSettings.overridden !=
			    model.platformTextureSettings.overridden)
			...
			if (curPlatformSettings.maxTextureSize !=
			    model.platformTextureSettings.maxTextureSize)
				model.maxTextureSizeIsDifferent = true;
			if (curPlatformSettings.resizeAlgorithm !=
			    model.platformTextureSettings.resizeAlgorithm)
				model.resizeAlgorithmIsDifferent = true;
			if (curPlatformSettings.textureCompression !=
			    model.platformTextureSettings.textureCompression)
				model.textureCompressionIsDifferent = true;
			if (curPlatformSettings.compressionQuality !=
			    model.platformTextureSettings.compressionQuality)
				model.compressionQualityIsDifferent = true;
			if (curPlatformSettings.crunchedCompression !=
			    model.platformTextureSettings.crunchedCompression)
				model.crunchedCompressionIsDifferent = true;
			if (curPlatformSettings.allowsAlphaSplitting !=
			    model.platformTextureSettings.allowsAlphaSplitting)
...
public void Sync() {
	...
	if (!model.isDefault && (!model.platformTextureSettings.overridden ||
	                         model.overriddenIsDifferent)) {
		BaseTextureImportPlatformSettings defaultSettings =
			GetDefaultImportSettings();
		model.platformTextureSettings.maxTextureSize =
			defaultSettings.model.platformTextureSettings.maxTextureSize;
		model.maxTextureSizeIsDifferent =
			defaultSettings.model.maxTextureSizeIsDifferent;
		model.platformTextureSettings.resizeAlgorithm =
			defaultSettings.model.platformTextureSettings.resizeAlgorithm;
		model.resizeAlgorithmIsDifferent =
			defaultSettings.model.resizeAlgorithmIsDifferent;
		model.platformTextureSettings.textureCompression =
			defaultSettings.model.platformTextureSettings.textureCompression;
		model.textureCompressionIsDifferent =
			defaultSettings.model.textureCompressionIsDifferent;
		model.platformTextureSettings.format =
			defaultSettings.model.platformTextureSettings.format;
		model.textureFormatIsDifferent =
			defaultSettings.model.textureFormatIsDifferent;
		model.platformTextureSettings.compressionQuality =
			defaultSettings.model.platformTextureSettings.compressionQuality;
		model.compressionQualityIsDifferent =
			defaultSettings.model.compressionQualityIsDifferent;
		model.platformTextureSettings.crunchedCompression =
			defaultSettings.model.platformTextureSettings.crunchedCompression;
		model.crunchedCompressionIsDifferent =
			defaultSettings.model.crunchedCompressionIsDifferent;
		model.platformTextureSettings.allowsAlphaSplitting =
			defaultSettings.model.platformTextureSettings.allowsAlphaSplitting;
		model.allowsAlphaSplitIsDifferent =
			defaultSettings.model.allowsAlphaSplitIsDifferent;
		model.androidETC2FallbackOverrideIsDifferent =
			defaultSettings.model.androidETC2FallbackOverrideIsDifferent;
	}

	if ((model.platformTextureSettings.overridden ||
	     model.overriddenIsDifferent) &&
	    model.platformTextureSettings.format < 0) {
		var showSettingsForPreset = ShowPresetSettings();

		model.platformTextureSettings.format =
			TextureImporter.DefaultFormatFromTextureParameters(
				GetImporterSettings(0), model.platformTextureSettings,
		...
		for (int i = 1; i < GetTargetCount(); i++) {
			...
			TextureImporterFormat format =
				TextureImporter.DefaultFormatFromTextureParameters(
					settings, model.platformTextureSettings,
...
public void Apply() {
	for (int i = 0; i < GetTargetCount(); i++) {
		...
		if (!model.resizeAlgorithmIsDifferent)
			platformSettings.resizeAlgorithm =
				model.platformTextureSettings.resizeAlgorithm;
		if (!model.textureCompressionIsDifferent)
			platformSettings.textureCompression =
				model.platformTextureSettings.textureCompression;
		if (!model.compressionQualityIsDifferent)
			platformSettings.compressionQuality =
				model.platformTextureSettings.compressionQuality;
		if (!model.crunchedCompressionIsDifferent)
			platformSettings.crunchedCompression =
				model.platformTextureSettings.crunchedCompression;
		if (!model.allowsAlphaSplitIsDifferent)
			platformSettings.allowsAlphaSplitting =
				model.platformTextureSettings.allowsAlphaSplitting;
		if (!model.androidETC2FallbackOverrideIsDifferent)
			platformSettings.androidETC2FallbackOverride =
				model.platformTextureSettings.androidETC2FallbackOverride;

		platformSettings.forceMaximumCompressionQuality_BC6H_BC7 =
			model.platformTextureSettings.forceMaximumCompressionQuality_BC6H_BC7;
...
internal static void ShowPlatformSpecificSettings(
	List<BaseTextureImportPlatformSettings> platformSettings) {
	...
	int shownTextureFormatPage = EditorGUILayout.BeginPlatformGrouping(
		validPlatforms, Styles.defaultPlatform);
	BaseTextureImportPlatformSettings realPS =
		platformSettings[shownTextureFormatPage + 1];
	...
	if (!realPS.model.isDefault) {
		...
		string title =
			string.Format(Styles.overrideFor.text,
			              validPlatforms[shownTextureFormatPage].title.text);
		bool newOverride = EditorGUILayout.ToggleLeft(
			title, realPS.model.platformTextureSettings.overridden);
	...
	bool notAllOverriddenForThisPlatform =
		(!realPS.model.isDefault && !realPS.model.allAreOverridden);
	using (new EditorGUI.DisabledScope(notAllOverriddenForThisPlatform)) {
		// acquire the platform support module for this platform, and present the
		// appropriate UI
		ITextureImportSettingsExtension textureSettingsExtension =
			ModuleManager.GetTextureImportSettingsExtension(
				realPS.model.buildTarget);
...
internal static void SyncPlatformSettings(
	List<BaseTextureImportPlatformSettings> platformSettings) {
	foreach (BaseTextureImportPlatformSettings ps in platformSettings)
		ps.Sync();
}

internal static void ApplyPlatformSettings(
	List<BaseTextureImportPlatformSettings> platformSettings) {
	SyncPlatformSettings(platformSettings);
	foreach (BaseTextureImportPlatformSettings ps in platformSettings)
		ps.Apply();
...
public bool allAreOverridden {
	get {
		return isDefault ||
		       (platformTextureSettings.overridden && !overriddenIsDifferent);
	}
}
public bool isDefault {
	get {
		return platformTextureSettings.name ==
...
public void
SetAndroidETC2FallbackOverrideForAll(AndroidETC2FallbackOverride value) {
	Debug.Assert(
		allAreOverridden,
...
public void
SetTextureCompressionForAll(TextureImporterCompression textureCompression) {
	Debug.Assert(
		allAreOverridden,
...
internal bool forceMaximumCompressionQuality_BC6H_BC7 {
	get {
		return platformTextureSettings.forceMaximumCompressionQuality_BC6H_BC7 !=
		       0;
	}
	set {
		platformTextureSettings.forceMaximumCompressionQuality_BC6H_BC7 =
			value ? 1 : 0;
	}
}
...
public void GetValidTextureFormatsAndStrings(TextureImporterType textureType,
                                             out int[] formatValues,
                                             out string[] formatStrings) {
	if (isDefault)
		TextureImportValidFormats.GetDefaultTextureFormatValuesAndStrings(
			textureType, out formatValues, out formatStrings);
	else
		TextureImportValidFormats.GetPlatformTextureFormatValuesAndStrings(
			textureType, buildTarget, out formatValues, out formatStrings);
}
...
[SerializeField] TextureImportPlatformSettingsData m_Data =
...
public override bool textureTypeHasMultipleDifferentValues {
	get { return m_Inspector.textureTypeHasMultipleDifferentValues; }
}

public override TextureImporterType textureType {
	get { return m_Inspector.textureType; }
}

public override SpriteImportMode spriteImportMode {
	get { return m_Inspector.spriteImportMode; }
}

public TextureImportPlatformSettings(string name, BuildTarget target,
                                     TextureImporterInspector inspector)
	: base(name, target) {
...
public override TextureImporterPlatformSettings
GetPlatformTextureSettings(int i, string name) {
	return m_Importers [i]
	       .GetPlatformTextureSettings(name);
}
...
SetPlatformTextureSettings(int i,
                           TextureImporterPlatformSettings platformSettings) {
	m_Importers [i]
	.SetPlatformTextureSettings(platformSettings);
}
