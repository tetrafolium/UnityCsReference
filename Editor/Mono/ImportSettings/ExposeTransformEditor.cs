...
internal class ExposeTransformEditor {
  ...
  public void OnEnable(string[] transformPaths,
                       SerializedObject serializedObject) {
    m_TransformPaths = transformPaths;
    m_ExtraExposedTransformPaths =
        serializedObject.FindProperty("m_ExtraExposedTransformPaths");
  ...
  void SetupOptimizeGameObjectTreeView() {
    ...
    m_ExposeTransformEditor = new OptimizeGameObjectTreeView(
        m_TreeViewState, multiColumnHeader, FillNodeInfos);
  ...
  private void SetNodeState(string nodePath, bool state) {
    if (GetNodeState(nodePath) != state) {
      if (state) {
        m_ExtraExposedTransformPaths.InsertArrayElementAtIndex(
            m_ExtraExposedTransformPaths.arraySize);
        m_ExtraExposedTransformPaths
            .GetArrayElementAtIndex(m_ExtraExposedTransformPaths.arraySize - 1)
            .stringValue = nodePath;
...
internal class OptimizeGameObjectTreeView : ToggleTreeView<SerializedNodeInfo> {
  public OptimizeGameObjectTreeView(TreeViewState state,
...
internal class SerializedNodeInfo : ToggleTreeViewItem {
  ...
  public override bool nodeState {
    ...
    set { setNodeState?.Invoke(path, value); }
  }
}
}
