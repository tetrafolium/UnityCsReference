...
internal class SpeedTreeImporterInspector : AssetImporterTabbedEditor {
  private static class Styles {
    ...
    public static GUIContent RegenerateRemapped = EditorGUIUtility.TrTextContent(
        "Regenerate Materials",
        "Regenerate the remapped materials from the current import settings.");
    public static GUIContent ApplyAndGenerateRemapped =
        EditorGUIUtility.TrTextContent(
            "Apply & Generate Materials",
            "Apply current importer settings and regenerate the remapped materials with new settings.");
  }
  ...
  public override void OnEnable() {
    ...
    if (tabs == null) {
      tabs = new BaseAssetImporterTabUI[]{
          new SpeedTreeImporterModelEditor(this),
          new SpeedTreeImporterMaterialEditor(this)};
  ...
  public override bool showImportedObject {
    get { return activeTab is SpeedTreeImporterModelEditor; }
  }
  ...
  internal GUIContent GetGenButtonText(bool modified, bool upgrade) {
    if (modified || upgrade) {
      if (m_MaterialLocation.intValue ==
          (int) SpeedTreeImporter.MaterialLocation.External)
  ...
  protected override bool OnApplyRevertGUI() {
    ...
    bool doMatsHaveDifferentShaders = (tabs[0] as SpeedTreeImporterModelEditor)
    ...
    if (HasRemappedMaterials() || doMatsHaveDifferentShaders) {
      ...
      if (GUILayout.Button(GetGenButtonText(hasModified, upgrade))) {
        // Apply the changes and generate the materials before importing so that
        // asset previews are up-to-date.
        if (hasModified)
          Apply();
  ...
  private void GenerateMaterials() {
    var matFolders =
        importers
            .Where(im => im.materialLocation ==
                         SpeedTreeImporter.MaterialLocation.External)
            .Select(im => im.materialFolderPath)
            .ToList();
    var guids = AssetDatabase.FindAssets("t:Material", matFolders.ToArray());
    var paths =
        guids.Select(guid => AssetDatabase.GUIDToAssetPath(guid)).ToList();

    var importersWithEmbeddedMaterials =
        importers.Where(im => im.materialLocation ==
    ...
    foreach (var importer in importersWithEmbeddedMaterials) {
      ...
      var materials =
          remappedAssets.Where(kv => kv.Value is Material && kv.Value != null)
              .Select(kv => kv.Value);
    ...
    if (paths.Count > 0)
      doGenerate = AssetDatabase.MakeEditable(
          paths.ToArray(), $"Materials will be checked out in:\n{string.Join("\n
          ", matFolders.ToArray())}");
  ...
  private bool HasRemappedMaterials() {
    ...
    foreach (var importer in importers) {
      ...
      m_HasRemappedMaterials =
          m_HasRemappedMaterials && remappedMaterialCount != 0;
