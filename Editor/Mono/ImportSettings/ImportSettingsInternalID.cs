...
internal class ImportSettingInternalID {
  static public void RegisterInternalID(SerializedObject serializedObject,
                                        UnityType type, long id, string name) {
    SerializedProperty internalIDMap =
        serializedObject.FindProperty("m_InternalIDToNameTable");
    internalIDMap.arraySize++;
    SerializedProperty newEntry =
        internalIDMap.GetArrayElementAtIndex(internalIDMap.arraySize - 1);
  ...
  static public bool
  RemoveEntryFromInternalIDTable(SerializedObject serializedObject,
                                 UnityType type, long id, string name) {
  ...
  static public long MakeInternalID(SerializedObject serializedObject,
                                    UnityType type, string name) {
    long id =
        ImportSettingInternalID.FindInternalID(serializedObject, type, name);
    if (id == 0L) {
      id =
          AssetImporter.MakeLocalFileIDWithHash(type.persistentTypeID, name, 0);
      RegisterInternalID(serializedObject, type, id, name);
    }
    return id;
  }

  static public void Rename(SerializedObject serializedObject, UnityType type,
                            string oldName, string newName) {
    RenameMultiple(serializedObject, type, new string[]{oldName},
                   new string[]{newName});
  }

  // Rename multiple entries at once to avoid situations where swapping names of
  // two entries would break references
  static public void RenameMultiple(SerializedObject serializedObject,
                                    UnityType type, string[] oldNames,
                                    string[] newNames) {
    int classID = type.persistentTypeID;

    int left = oldNames.Length;

    SerializedProperty recycleMap =
        serializedObject.FindProperty("m_InternalIDToNameTable");
