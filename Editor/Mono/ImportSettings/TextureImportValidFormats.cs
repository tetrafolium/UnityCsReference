...
internal static class TextureImportValidFormats {
  private struct TextureTypeAndBuildTarget {
    ...
    public TextureTypeAndBuildTarget(TextureImporterType textureType,
                                     BuildTarget target) {
  ...
  private static Dictionary<TextureTypeAndBuildTarget, Value>
      s_ValidTextureFormats;
  private static Dictionary<TextureImporterType, Value>
      s_ValidDefaultTextureFormats;
  ...
  private static void BuildValidFormats() {
    ...
    foreach (var textureTypeName in Enum.GetNames(
                 typeof(TextureImporterType))) {
      var textureTypeField =
          typeof(TextureImporterType).GetField(textureTypeName);
      if (textureTypeField.IsDefined(typeof(ObsoleteAttribute), false))
        continue;

      var textureType = (TextureImporterType) Enum.Parse(
          typeof(TextureImporterType), textureTypeName);
      ...
      foreach (var targetName in Enum.GetNames(typeof(BuildTarget))) {
        ...
        var validFormats = Array.ConvertAll(
            TextureImporter.RecommendedFormatsFromTextureTypeAndPlatform(
                textureType, target),
            value =>(int) value);
        Array.Sort(validFormats, SortTextureFormats);
        s_ValidTextureFormats.Add(
            Key, new Value(validFormats, BuildTextureStrings(validFormats)));

        defaultFormats = defaultFormats?.Intersect(validFormats).ToList()
            ?? validFormats.ToList();
      }
      ...
      s_ValidDefaultTextureFormats[textureType] = new Value(
          defaultFormatsArray, BuildTextureStrings(defaultFormatsArray));
    }
  }
  ...
  static uint GetSortCodeForFormat(TextureFormat fmt) {
    ...
    if (GraphicsFormatUtility.IsNormFormat(f))
      type = 3;
    else if (GraphicsFormatUtility.IsHalfFormat(f) ||
             GraphicsFormatUtility.IsFloatFormat(f))
      type = 2;
    ...
    // then compression: first compressed regular, then compressed Crunch, then
    // uncompressed
    uint compression = 0;
  ...
  public static void GetPlatformTextureFormatValuesAndStrings(
      TextureImporterType textureType, BuildTarget target,
      out int[] formatValues, out string[] formatStrings) {
    BuildValidFormats();
  ...
  public static void
  GetDefaultTextureFormatValuesAndStrings(TextureImporterType textureType,
