...
public static readonly GUIContent streamingMipmapsPriority =
	EditorGUIUtility.TrTextContent(
		"Mip Map Priority",
...
public static readonly int[] filterModeValues = {
	(int) FilterMode.Point, (int) FilterMode.Bilinear,
...
// alas it looks impossible to share code so we copy paste from
// TextureImporterInspector.TextureSettingsGUI()
...
public void TextureSettingsGUI() {
	...
	// true for isVolume based on m_TextureShape Also, always consider we may
	// have a volume if we don't know the target.
	...
	FilterMode filter = m_FilterMode.intValue == -1
	                    ? FilterMode.Bilinear
	                    : (FilterMode) m_FilterMode.intValue;
	filter = (FilterMode) EditorGUI.IntPopup(
		rect, Styles.filterMode, (int) filter, Styles.filterModeOptions,
		Styles.filterModeValues);
...
public override void OnInspectorGUI() {
		...
		foreach (AssetImporter importer in targets) {
			...
			if (tex != null) {
				if (m_FilterMode.intValue != -1)
					TextureUtil.SetFilterModeNoDirty(tex, (FilterMode)
					                                 m_FilterMode.intValue);
	...
	if (m_StreamingMipmaps.boolValue &&
		...
		EditorGUILayout.PropertyField(m_StreamingMipmapsPriority,
		                              Styles.streamingMipmapsPriority);
		EditorGUI.indentLevel--;
	}
