...
internal class SpeedTreeImporterMaterialEditor : BaseSpeedTreeImporterTabUI {
  private static class Styles {
    ...
    public static GUIContent[] MaterialLocationOpt = {
        EditorGUIUtility.TrTextContent(
            ...
            "Use external materials if found in the project."),
        EditorGUIUtility.TrTextContent(
            "Use Embedded Materials",
            "Embed the material inside the imported asset.")};
    ...
    public static GUIContent ExternalMaterialSearchHelp =
        EditorGUIUtility.TrTextContent(
            "Remap the imported materials to materials from the Unity project.");

    public static GUIContent SelectMaterialFolder =
        EditorGUIUtility.TrTextContent("Select Materials Folder");
  }
  ...
  private bool HasEmbeddedMaterials() {
    ...
    // if the m_ExternalObjecs map has any unapplied changes, keep the state of
    // the button as is
    ...
    foreach (var t in m_ExternalObjects.serializedObject.targetObjects) {
      ...
      foreach (var entry in externalObjectMap) {
        if (entry.Key.type == typeof(Material) &&Array.Exists(
                                  materialsList, x => x.name == entry.Key.name))
          ++remappedMaterialCount;
      }

      m_HasEmbeddedMaterials = m_HasEmbeddedMaterials &&
                               remappedMaterialCount != materialsList.Length;
    }
    return m_HasEmbeddedMaterials;
  }
  ...
  public override void OnInspectorGUI() { ShowMaterialGUI(); }
  ...
  private void ShowMaterialGUI() {
    ...
    if (targets.Length == 1 && m_Materials.arraySize > 0 &&
      ...
      // objects can change based on user actions. For each material name, map
      // the external object associated with it. The complexity comes from the
      // fact that we may not have an external object in the map, so we can't
      // make a property out of it
      for (int materialIdx = 0; materialIdx < m_Materials.arraySize;
        ...
        for (int externalObjectIdx = 0, count = m_ExternalObjects.arraySize;
          ...
          var pair =
              m_ExternalObjects.GetArrayElementAtIndex(externalObjectIdx);
          var externalName =
              pair.FindPropertyRelative("first.name").stringValue;
          var externalType =
              pair.FindPropertyRelative("first.type").stringValue;

          if (externalName == name && externalType == type) {
            materialProp = pair.FindPropertyRelative("second");
            material =
                materialProp !=
                null ? materialProp.objectReferenceValue as Material : null;
        ...
        if (materialProp != null) {
          ...
          EditorGUILayout.ObjectField(materialProp, typeof(Material),
                                      nameLabel);
          ...
          material = EditorGUILayout.ObjectField(
              nameLabel, material, typeof(Material), false) as Material;
          if (EditorGUI.EndChangeCheck()) {
            if (material != null) {
              ...
              pair.FindPropertyRelative("first.assembly").stringValue =
                  assembly;
              pair.FindPropertyRelative("second").objectReferenceValue =
                  material;
            }
          }
        }
      ...
      using(new EditorGUI.DisabledScope(!HasEmbeddedMaterials())) {
        if (GUILayout.Button(Styles.ExtractEmbeddedMaterials)) {
          ...
          destinationPath = EditorUtility.SaveFolderPanel(
              Styles.SelectMaterialFolder.text,
  ...
  private bool MaterialRemapOptons() {
    m_ShowMaterialRemapOptions = EditorGUILayout.Foldout(
        m_ShowMaterialRemapOptions, Styles.RemapOptions);
    if (m_ShowMaterialRemapOptions) {
      EditorGUI.indentLevel++;

      EditorGUILayout.HelpBox(Styles.ExternalMaterialSearchHelp.text,
                              MessageType.Info);
          ...
          if (GUILayout.Button(Styles.RemapMaterialsInProject)) {
            try {
              ...
              foreach (var t in targets) {
                ...
                folderToSearch = EditorUtility.OpenFolderPanel(
                    Styles.SelectMaterialFolder.text,
                    importer.materialFolderPath, "");
                importer.SearchAndRemapMaterials(
                    FileUtil.GetProjectRelativePath(folderToSearch));
