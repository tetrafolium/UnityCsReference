...
internal class TextureImporterInspector : AssetImporterEditor {
  ...
  private struct TextureInspectorTypeGUIProperties {
    ...
    public TextureInspectorTypeGUIProperties(
        TextureInspectorGUIElement _commonElements,
        TextureInspectorGUIElement _advancedElements,
        TextureImporterShape _shapeCaps) {
  ...
  private Dictionary<TextureInspectorGUIElement, GUIMethod>
      m_GUIElementMethods =
          new Dictionary<TextureInspectorGUIElement, GUIMethod>();
  ...
  public static bool
  IsCompressedDXTTextureFormat(TextureImporterFormat format) {
  ...
  internal static readonly
      TextureImporterFormat[] kFormatsWithCompressionSettings = {
  ...
  enum CookieMode { Spot = 0, Directional = 1, Point = 2 }
  ...
  readonly int[] m_FilterModeOptions =
      (int[])(Enum.GetValues(typeof(FilterMode)));
  ...
  internal class Styles {
    public readonly GUIContent textureTypeTitle =
        EditorGUIUtility.TrTextContent("Texture Type",
                                       "What will this texture be used for?");
    public readonly GUIContent[] textureTypeOptions = {
        EditorGUIUtility.TrTextContent(
            "Default",
            "Texture is a normal image such as a diffuse texture or other."),
        EditorGUIUtility.TrTextContent("Normal map",
                                       "Texture is a bump or normal map."),
        EditorGUIUtility.TrTextContent("Editor GUI and Legacy GUI",
                                       "Texture is used for a GUI element."),
        EditorGUIUtility.TrTextContent("Sprite (2D and UI)",
                                       "Texture is used for a sprite."),
        EditorGUIUtility.TrTextContent("Cursor",
                                       "Texture is used for a cursor."),
        EditorGUIUtility.TrTextContent(
            "Cookie", "Texture is a cookie you put on a light."),
    ...
    public readonly GUIContent textureShape = EditorGUIUtility.TrTextContent(
        "Texture Shape", "What shape is this texture?");
    readonly GUIContent textureShape2D =
        EditorGUIUtility.TrTextContent("2D", "Texture is 2D.");
    readonly GUIContent textureShapeCube =
        EditorGUIUtility.TrTextContent("Cube", "Texture is a Cubemap.");
    readonly GUIContent textureShape2DArray =
        EditorGUIUtility.TrTextContent("2D Array", "Texture is a 2D Array.");
    readonly GUIContent textureShape3D =
        EditorGUIUtility.TrTextContent("3D", "Texture is 3D.");
    public readonly Dictionary<TextureImporterShape, GUIContent[]>
        textureShapeOptionsDictionnary =
            new Dictionary<TextureImporterShape, GUIContent[]>();
    public readonly Dictionary<TextureImporterShape, int[]>
        textureShapeValuesDictionnary =
            new Dictionary<TextureImporterShape, int[]>();

    public readonly GUIContent filterMode =
    ...
    public readonly GUIContent cookieType =
        EditorGUIUtility.TrTextContent("Light Type");
    ...
    public readonly GUIContent generateFromBump = EditorGUIUtility.TrTextContent(
        "Create from Grayscale",
        "The grayscale of the image is used as a heightmap for generating the normal map.");
    public readonly GUIContent bumpiness =
        EditorGUIUtility.TrTextContent("Bumpiness");
    public readonly GUIContent bumpFiltering =
        EditorGUIUtility.TrTextContent("Filtering");
    public readonly GUIContent[] bumpFilteringOptions = {
        EditorGUIUtility.TrTextContent("Sharp"),
        EditorGUIUtility.TrTextContent("Smooth"),
    };
    public readonly GUIContent cubemap =
    ...
    public readonly GUIContent[] cubemapOptions = {
        ...
        EditorGUIUtility.TrTextContent(
            ...
            "Texture contains 6 images arranged in one of the standard cubemap layouts - cross or sequence (+x, -x, +y, -y, +z, -z). Texture can be in vertical or horizontal orientation."),
        EditorGUIUtility.TrTextContent(
            "Latitude-Longitude Layout (Cylindrical)",
            "Texture contains an image of a ball unwrapped such that latitude and longitude are mapped to horizontal and vertical dimensions (as on a globe)."),
        EditorGUIUtility.TrTextContent(
    ...
    public readonly GUIContent cubemapConvolution =
        EditorGUIUtility.TrTextContent("Convolution Type");
    ...
    public readonly GUIContent seamlessCubemap = EditorGUIUtility.TrTextContent(
        "Fixup Edge Seams",
        "Enable if this texture is used for glossy reflections.");
    public readonly GUIContent textureFormat =
        EditorGUIUtility.TrTextContent("Format");

    public readonly GUIContent mipmapFadeOutToggle =
        EditorGUIUtility.TrTextContent("Fadeout Mip Maps");
    public readonly GUIContent mipmapFadeOut =
        EditorGUIUtility.TrTextContent("Fade Range");
    public readonly GUIContent readWrite = EditorGUIUtility.TrTextContent(
        "Read/Write Enabled",
        "Enable to be able to access the raw pixel data from code.");
    public readonly GUIContent streamingMipmaps = EditorGUIUtility.TrTextContent(
        "Streaming Mipmaps",
    ...
    public readonly GUIContent streamingMipmapsPriority =
        EditorGUIUtility.TrTextContent(
            "Mip Map Priority",
            "Mip map streaming priority when there's contention for resources. Positive numbers represent higher priority. Valid range is -128 to 127.");

    public readonly GUIContent alphaSource = EditorGUIUtility.TrTextContent(
        "Alpha Source", "How is the alpha generated for the imported texture.");
    ...
    public readonly GUIContent singleChannelComponent =
        EditorGUIUtility.TrTextContent(
            "Channel",
            "As which color/alpha component the single channel texture is treated.");
    public readonly GUIContent[] singleChannelComponentOptions = {
        EditorGUIUtility.TrTextContent(
            "Alpha", "Use the alpha channel (compression not supported)."),
    ...
    public readonly GUIContent generateMipMaps =
        EditorGUIUtility.TrTextContent("Generate Mip Maps");
    public readonly GUIContent sRGBTexture = EditorGUIUtility.TrTextContent(
        "sRGB (Color Texture)",
    ...
    public readonly GUIContent borderMipMaps =
        EditorGUIUtility.TrTextContent("Border Mip Maps");
    public readonly GUIContent mipMapsPreserveCoverage =
        EditorGUIUtility.TrTextContent(
            "Mip Maps Preserve Coverage",
            "The alpha channel of generated Mip Maps will preserve coverage during the alpha test.");
    public readonly GUIContent alphaTestReferenceValue =
        EditorGUIUtility.TrTextContent(
            "Alpha Cutoff Value",
            "The reference value used during the alpha test. Controls Mip Map coverage.");
    public readonly GUIContent mipMapFilter =
        EditorGUIUtility.TrTextContent("Mip Map Filtering");
    public readonly GUIContent[] mipMapFilterOptions = {
        EditorGUIUtility.TrTextContent("Box"),
        EditorGUIUtility.TrTextContent("Kaiser"),
    };
    public readonly GUIContent npot = EditorGUIUtility.TrTextContent(
        "Non-Power of 2",
        "How non-power-of-two textures are scaled on import.");
    public readonly GUIContent generateCubemap =
        EditorGUIUtility.TrTextContent("Generate Cubemap");

    public readonly GUIContent spriteMode =
        EditorGUIUtility.TrTextContent("Sprite Mode");
    ...
    public readonly GUIContent spritePackingTag =
        EditorGUIUtility.TrTextContent("Packing Tag",
                                       "Tag for the Sprite Packing system.");
    public readonly GUIContent spritePixelsPerUnit =
        EditorGUIUtility.TrTextContent(
            "Pixels Per Unit",
            "How many pixels in the sprite correspond to one unit in the world.");
    public readonly GUIContent spriteExtrude = EditorGUIUtility.TrTextContent(
        "Extrude Edges",
        "How much empty area to leave around the sprite in the generated mesh.");
    public readonly GUIContent spriteMeshType = EditorGUIUtility.TrTextContent(
        "Mesh Type", "Type of sprite mesh to generate.");
    public readonly GUIContent spriteAlignment = EditorGUIUtility.TrTextContent(
        "Pivot",
        "Sprite pivot point in its localspace. May be used for syncing animation frames of different sizes.");
    ...
    public readonly GUIContent spriteGenerateFallbackPhysicsShape =
        EditorGUIUtility.TrTextContent(
            "Generate Physics Shape",
            "Generates a default physics shape from the outline of the Sprite/s when a physics shape has not been set in the Sprite Editor.");

    public readonly GUIContent alphaIsTransparency = EditorGUIUtility.TrTextContent(
        "Alpha Is Transparency",
        "If the provided alpha channel is transparency, enable this to pre-filter the color to avoid texture filtering artifacts. This is not supported for HDR textures.");

    public readonly GUIContent showAdvanced =
        EditorGUIUtility.TrTextContent("Advanced", "Show advanced settings.");

    public readonly GUIContent psdRemoveMatte = EditorGUIUtility.TrTextContent(
        "Remove Matte (PSD)",
        "Enable special processing for PSD that has transparency, as color pixels will be tweaked (blended with white color).");
    public readonly GUIContent psdRemoveMatteInfo = EditorGUIUtility.TrTextContent(
        "If you have PSD with transparency, colors will be tweaked by blending them with white color. Matte removal refers to our attempts to undo that.");
    public readonly GUIContent psdRemoveMatteURLButton =
        EditorGUIUtility.TrTextContent("How to handle PSD with alpha");
    public readonly string psdRemoveMatteURL =
        "https://docs.unity3d.com/Manual/HOWTO-alphamaps.html";

    public readonly GUIContent ignorePngGamma = EditorGUIUtility.TrTextContent(
        "Ignore PNG file gamma",
        "Ignore the Gamma value attribute in PNG files, this setting has no effect on other file formats.");
    public readonly GUIContent readWriteWarning = EditorGUIUtility.TrTextContent(
        "Textures larger than 8192 can not be Read/Write enabled. Value will be ignored.");

    public readonly GUIContent flipbookColumns = EditorGUIUtility.TrTextContent(
        "Columns", "Source image is divided into this amount of columns.");
    public readonly GUIContent flipbookRows = EditorGUIUtility.TrTextContent(
        "Rows", "Source image is divided into this amount of rows.");

    public Styles() {
      // This is far from ideal, but it's better than having tons of logic in
      // the GUI code itself. The combination should not grow too much.
      ...
      GUIContent[] sAll_Options = {textureShape2D, textureShapeCube,
                                   textureShape2DArray, textureShape3D};
      textureShapeOptionsDictionnary.Add(TextureImporterShape.Texture2D,
                                         s2D_Options);
      textureShapeOptionsDictionnary.Add(TextureImporterShape.TextureCube,
                                         sCube_Options);
      textureShapeOptionsDictionnary.Add(TextureImporterShape.Texture2D |
                                             TextureImporterShape.TextureCube,
                                         s2D_Cube_Options);
      textureShapeOptionsDictionnary.Add(
      ...
      int[] s2D_Cube_Values = {(int) TextureImporterShape.Texture2D,
                               (int) TextureImporterShape.TextureCube};
      int[] sAll_Values = {(int) TextureImporterShape.Texture2D,
                           (int) TextureImporterShape.TextureCube,
                           (int) TextureImporterShape.Texture2DArray,
                           (int) TextureImporterShape.Texture3D};
      textureShapeValuesDictionnary.Add(TextureImporterShape.Texture2D,
                                        s2D_Values);
      textureShapeValuesDictionnary.Add(TextureImporterShape.TextureCube,
                                        sCube_Values);
      textureShapeValuesDictionnary.Add(TextureImporterShape.Texture2D |
                                            TextureImporterShape.TextureCube,
                                        s2D_Cube_Values);
      textureShapeValuesDictionnary.Add(
          TextureImporterShape.Texture2D | TextureImporterShape.TextureCube |
              TextureImporterShape.Texture2DArray |
              TextureImporterShape.Texture3D,
          sAll_Values);
  ...
  TextureInspectorTypeGUIProperties[] m_TextureTypeGUIElements =
      new TextureInspectorTypeGUIProperties
          [Enum.GetValues(typeof(TextureImporterType)).Length];
  List<TextureInspectorGUIElement> m_GUIElementsDisplayOrder =
      new List<TextureInspectorGUIElement>();
  ...
  void EnumPopup(SerializedProperty property, System.Type type,
                 GUIContent label) {
    EditorGUILayout.IntPopup(
        property, EditorGUIUtility.TempContent(System.Enum.GetNames(type)),
        System.Enum.GetValues(type) as int[], label);
  }
  ...
  void CacheSerializedProperties() {
    ...
    m_ConvertToNormalMap =
        serializedObject.FindProperty("m_ConvertToNormalMap");
    ...
    m_MipMapsPreserveCoverage =
        serializedObject.FindProperty("m_MipMapsPreserveCoverage");
    m_AlphaTestReferenceValue =
        serializedObject.FindProperty("m_AlphaTestReferenceValue");
    ...
    m_StreamingMipmapsPriority =
        serializedObject.FindProperty("m_StreamingMipmapsPriority");
    ...
    m_MipMapFadeDistanceStart =
        serializedObject.FindProperty("m_MipMapFadeDistanceStart");
    m_MipMapFadeDistanceEnd =
        serializedObject.FindProperty("m_MipMapFadeDistanceEnd");

    m_Aniso = serializedObject.FindProperty("m_TextureSettings.m_Aniso");
    m_FilterMode =
        serializedObject.FindProperty("m_TextureSettings.m_FilterMode");
    ...
    m_CubemapConvolution =
        serializedObject.FindProperty("m_CubemapConvolution");
    ...
    m_SpritePixelsToUnits =
        serializedObject.FindProperty("m_SpritePixelsToUnits");
    ...
    m_SpriteGenerateFallbackPhysicsShape =
        serializedObject.FindProperty("m_SpriteGenerateFallbackPhysicsShape");
    ...
    m_SingleChannelComponent =
        serializedObject.FindProperty("m_SingleChannelComponent");
  ...
  void InitializeGUI() {
    // This is where we decide what GUI elements are displayed depending on the
    // texture type.
    // TODO: Maybe complement the bitfield with a list to add a concept of order
    // in the display. Not sure if necessary...
    TextureImporterShape shapeCapsAll =
        TextureImporterShape.Texture2D | TextureImporterShape.TextureCube |
        TextureImporterShape.Texture2DArray | TextureImporterShape.Texture3D;

    m_TextureTypeGUIElements[(int) TextureImporterType.Default] =
        new TextureInspectorTypeGUIProperties(
            TextureInspectorGUIElement.ColorSpace |
                TextureInspectorGUIElement.AlphaHandling |
                TextureInspectorGUIElement.PngGamma |
                TextureInspectorGUIElement.CubeMapConvolution |
                TextureInspectorGUIElement.CubeMapping |
                TextureInspectorGUIElement.ElementsAtlas,
            TextureInspectorGUIElement.PowerOfTwo |
                TextureInspectorGUIElement.Readable |
                TextureInspectorGUIElement.MipMaps |
                TextureInspectorGUIElement.StreamingMipmaps,
            shapeCapsAll);
    m_TextureTypeGUIElements[(int) TextureImporterType.NormalMap] =
        new TextureInspectorTypeGUIProperties(
            TextureInspectorGUIElement.NormalMap |
                TextureInspectorGUIElement.PngGamma |
                TextureInspectorGUIElement.CubeMapping |
                TextureInspectorGUIElement.ElementsAtlas,
            TextureInspectorGUIElement.PowerOfTwo |
                ...
                TextureInspectorGUIElement.StreamingMipmaps,
            shapeCapsAll);
    m_TextureTypeGUIElements[(int) TextureImporterType.Sprite] =
        new TextureInspectorTypeGUIProperties(
    ...
    m_TextureTypeGUIElements[(int) TextureImporterType.Cookie] =
        new TextureInspectorTypeGUIProperties(
            TextureInspectorGUIElement.Cookie |
                TextureInspectorGUIElement.AlphaHandling |
                TextureInspectorGUIElement.PngGamma |
                TextureInspectorGUIElement.CubeMapping,
            TextureInspectorGUIElement.PowerOfTwo |
                ...
                TextureInspectorGUIElement.MipMaps,
            TextureImporterShape.Texture2D | TextureImporterShape.TextureCube);
    m_TextureTypeGUIElements[(int) TextureImporterType.SingleChannel] =
        new TextureInspectorTypeGUIProperties(
            TextureInspectorGUIElement.AlphaHandling |
                TextureInspectorGUIElement.PngGamma |
                TextureInspectorGUIElement.SingleChannelComponent |
                TextureInspectorGUIElement.CubeMapping |
                TextureInspectorGUIElement.ElementsAtlas,
            TextureInspectorGUIElement.PowerOfTwo |
                TextureInspectorGUIElement.Readable |
                ...
                TextureInspectorGUIElement.StreamingMipmaps,
            shapeCapsAll);
    m_TextureTypeGUIElements[(int) TextureImporterType.GUI] =
        new TextureInspectorTypeGUIProperties(
            0,
            TextureInspectorGUIElement.AlphaHandling |
                TextureInspectorGUIElement.PngGamma |
                ...
                TextureInspectorGUIElement.MipMaps,
            TextureImporterShape.Texture2D);
    m_TextureTypeGUIElements[(int) TextureImporterType.Cursor] =
        new TextureInspectorTypeGUIProperties(
            0,
            TextureInspectorGUIElement.AlphaHandling |
                TextureInspectorGUIElement.PngGamma |
                ...
                TextureInspectorGUIElement.MipMaps,
            TextureImporterShape.Texture2D);
    m_TextureTypeGUIElements[(int) TextureImporterType.Lightmap] =
        new TextureInspectorTypeGUIProperties(
            0,
            TextureInspectorGUIElement.PowerOfTwo |
                TextureInspectorGUIElement.Readable |
    ...
    m_GUIElementMethods.Add(TextureInspectorGUIElement.PowerOfTwo,
                            this.POTScaleGUI);
    m_GUIElementMethods.Add(TextureInspectorGUIElement.Readable,
                            this.ReadableGUI);
    m_GUIElementMethods.Add(TextureInspectorGUIElement.StreamingMipmaps,
                            this.StreamingMipmapsGUI);
    m_GUIElementMethods.Add(TextureInspectorGUIElement.ColorSpace,
                            this.ColorSpaceGUI);
    m_GUIElementMethods.Add(TextureInspectorGUIElement.AlphaHandling,
                            this.AlphaHandlingGUI);
    m_GUIElementMethods.Add(TextureInspectorGUIElement.PngGamma,
                            this.PngGammaGUI);
    ...
    m_GUIElementMethods.Add(TextureInspectorGUIElement.CubeMapping,
                            this.CubemapMappingGUI);
    m_GUIElementMethods.Add(TextureInspectorGUIElement.ElementsAtlas,
                            this.ElementsAtlasGui);

    // This list dictates the order in which the GUI Elements are displayed.
    // It could be different for each TextureImporterType but let's keep it
    // simple for now.
    ...
    UnityEngine.Debug.Assert(
        m_GUIElementsDisplayOrder.Count ==
            (Enum.GetValues(typeof(TextureInspectorGUIElement)).Length - 1),
        "Some GUIElement are not present in the list."); // -1 because
                                                         // TextureInspectorGUIElement.None
  }

  public override void OnEnable() {
    base.OnEnable();

    s_DefaultPlatformName =
        TextureImporter.defaultPlatformName; // Can't be called everywhere so we
                                             // save it here for later use.

    m_ShowAdvanced =
        EditorPrefs.GetBool("TextureImporterShowAdvanced", m_ShowAdvanced);
    ...
    m_ShowCubeMapSettings.value =
        (TextureImporterShape)
            m_TextureShape.intValue == TextureImporterShape.TextureCube;
    m_ShowElementsAtlasSettings.valueChanged.AddListener(Repaint);
    m_ShowElementsAtlasSettings.value =
        (TextureImporterShape) m_TextureShape.intValue ==
            TextureImporterShape.Texture2DArray ||
        (TextureImporterShape)
                m_TextureShape.intValue == TextureImporterShape.Texture3D;
  ...
  void SetSerializedPropertySettings(TextureImporterSettings settings) {
    ...
    m_MipMapsPreserveCoverage.intValue =
        settings.mipMapsPreserveCoverage ? 1 : 0;
    ...
    m_SpriteGenerateFallbackPhysicsShape.intValue =
        settings.spriteGenerateFallbackPhysicsShape ? 1 : 0;
  ...
  internal TextureImporterSettings
  GetSerializedPropertySettings(TextureImporterSettings settings) {
    if (!m_AlphaSource.hasMultipleDifferentValues)
      settings.alphaSource =
          (TextureImporterAlphaSource) m_AlphaSource.intValue;
    ...
    if (!m_NormalMapFilter.hasMultipleDifferentValues)
      settings.normalMapFilter =
          (TextureImporterNormalFilter) m_NormalMapFilter.intValue;

    if (!m_GenerateCubemap.hasMultipleDifferentValues)
      settings.generateCubemap =
          (TextureImporterGenerateCubemap) m_GenerateCubemap.intValue;

    if (!m_CubemapConvolution.hasMultipleDifferentValues)
      settings.cubemapConvolution =
          (TextureImporterCubemapConvolution) m_CubemapConvolution.intValue;
    ...
    if (!m_SpriteGenerateFallbackPhysicsShape.hasMultipleDifferentValues)
      settings.spriteGenerateFallbackPhysicsShape =
          m_SpriteGenerateFallbackPhysicsShape.intValue > 0;
    ...
    if (!m_SingleChannelComponent.hasMultipleDifferentValues)
      settings.singleChannelComponent = (TextureImporterSingleChannelComponent)
                                            m_SingleChannelComponent.intValue;
  ...
  void CookieGUI(TextureInspectorGUIElement guiElements) {
    ...
    cm = (CookieMode) EditorGUILayout.Popup(s_Styles.cookieType, (int) cm,
                                            s_Styles.cookieOptions);
    if (EditorGUI.EndChangeCheck())
      SetCookieMode(cm);
  ...
  void CubemapMappingGUI(TextureInspectorGUIElement guiElements) {
    m_ShowCubeMapSettings.target =
        (TextureImporterShape)
            m_TextureShape.intValue == TextureImporterShape.TextureCube;
    if (EditorGUILayout.BeginFadeGroup(m_ShowCubeMapSettings.faded)) {
      if ((TextureImporterShape)
              m_TextureShape.intValue == TextureImporterShape.TextureCube) {
        using(new EditorGUI.DisabledScope(
            !m_IsPOT &&
            m_NPOTScale.intValue == (int) TextureImporterNPOTScale.None)) {
          EditorGUI.showMixedValue =
              m_GenerateCubemap.hasMultipleDifferentValues ||
              m_SeamlessCubemap.hasMultipleDifferentValues;

          EditorGUI.BeginChangeCheck();

          int value = EditorGUILayout.IntPopup(
              s_Styles.cubemap, m_GenerateCubemap.intValue,
              s_Styles.cubemapOptions, s_Styles.cubemapValues2);
          if (EditorGUI.EndChangeCheck())
          ...
          // Convolution
          if (ShouldDisplayGUIElement(
                  guiElements, TextureInspectorGUIElement.CubeMapConvolution)) {
            EditorGUILayout.IntPopup(
                m_CubemapConvolution, s_Styles.cubemapConvolutionOptions,
                s_Styles.cubemapConvolutionValues, s_Styles.cubemapConvolution);
          }
  ...
  void ElementsAtlasGui(TextureInspectorGUIElement guiElements) {
    ...
    var isLayerShape = shape == TextureImporterShape.Texture2DArray ||
                       shape == TextureImporterShape.Texture3D;
    m_ShowElementsAtlasSettings.target = isLayerShape;
    if (EditorGUILayout.BeginFadeGroup(m_ShowElementsAtlasSettings.faded) &&
        isLayerShape) {
      EditorGUI.indentLevel++;
      EditorGUI.BeginChangeCheck();
      EditorGUILayout.PropertyField(m_FlipbookColumns,
                                    s_Styles.flipbookColumns);
      if (EditorGUI.EndChangeCheck()) {
      ...
      if (m_TextureWidth % m_FlipbookColumns.intValue != 0)
        EditorGUILayout.HelpBox(
            $"Image width {m_TextureWidth} does not divide into {m_FlipbookColumns.intValue} columns exactly",
      ...
      if (m_TextureHeight % m_FlipbookRows.intValue != 0)
        EditorGUILayout.HelpBox(
            $"Image height {m_TextureHeight} does not divide into {m_FlipbookRows.intValue} rows exactly",
            MessageType.Warning, true);
      EditorGUI.indentLevel--;
    }
    EditorGUILayout.EndFadeGroup();
  ...
  void ReadableGUI(TextureInspectorGUIElement guiElements) {
    ...
    using(new EditorGUI.DisabledScope(!enabled)) {
      ...
      if (!enabled && m_IsReadable.intValue > 0) {
        EditorGUILayout.HelpBox(s_Styles.readWriteWarning.text,
                                MessageType.Warning, true);
      }
    }
  }
  ...
  void StreamingMipmapsGUI(TextureInspectorGUIElement guiElements) {
    ...
    var shape = (TextureImporterShape) m_TextureShape.intValue;
    var shapeHasStreaming = shape == TextureImporterShape.Texture2D ||
                            shape == TextureImporterShape.TextureCube;
    if (!shapeHasStreaming)
      return;

    ToggleFromInt(m_StreamingMipmaps, s_Styles.streamingMipmaps);
    if (m_StreamingMipmaps.boolValue &&
        !m_StreamingMipmaps.hasMultipleDifferentValues) {
      EditorGUI.indentLevel++;
      EditorGUILayout.PropertyField(m_StreamingMipmapsPriority,
                                    s_Styles.streamingMipmapsPriority);
      EditorGUI.indentLevel--;
    }
  }
  ...
  void AlphaHandlingGUI(TextureInspectorGUIElement guiElements) {
    ...
    if (ShouldDisplayGUIElement(
            guiElements, TextureInspectorGUIElement.SingleChannelComponent)) {
      EditorGUI.showMixedValue =
          m_SingleChannelComponent.hasMultipleDifferentValues;
      EditorGUI.BeginChangeCheck();
      int newSingleChannelComponent = EditorGUILayout.IntPopup(
          s_Styles.singleChannelComponent, m_SingleChannelComponent.intValue,
          s_Styles.singleChannelComponentOptions,
          s_Styles.singleChannelComponentValues);
      ...
      showAlphaSource = (m_SingleChannelComponent.intValue ==
                         (int) TextureImporterSingleChannelComponent.Alpha);
    }
    ...
    if (showAlphaSource) {
      ...
      EditorGUI.BeginChangeCheck();
      int newAlphaUsage = EditorGUILayout.IntPopup(
          s_Styles.alphaSource, m_AlphaSource.intValue,
          s_Styles.alphaSourceOptions, s_Styles.alphaSourceValues);
      ...
      bool showAlphaIsTransparency =
          success &&
          (TextureImporterAlphaSource)
                  m_AlphaSource.intValue != TextureImporterAlphaSource.None &&
          countHDR == 0; // AlphaIsTransparency is not properly implemented for
                         // HDR texture yet.
      using(new EditorGUI.DisabledScope(assetTarget != null &&
                                        !showAlphaIsTransparency)) {
        ToggleFromInt(m_AlphaIsTransparency, s_Styles.alphaIsTransparency);
      }
    }

    // This is pure backward compatibility codepath. It can be removed when we
    ...
    if (importer.ShouldShowRemoveMatteOption()) {
      ...
      if (m_PSDRemoveMatte.boolValue) {
        ...
        EditorGUILayout.HelpBox(s_Styles.psdRemoveMatteInfo.text,
  ...
  private bool ShouldShowSpriteMeshTypeOption() {
    return m_SpriteMode.intValue != (int) SpriteImportMode.Polygon &&
           !m_SpriteMode.hasMultipleDifferentValues;
  }
  ...
  private void SpriteGUI(TextureInspectorGUIElement guiElements) {
    ...
    EditorGUILayout.IntPopup(m_SpriteMode, s_Styles.spriteModeOptions,
                             new[]{1, 2, 3}, s_Styles.spriteMode);

    // Ensure that PropertyField focus will be cleared when we change
    // spriteMode.
    if (EditorGUI.EndChangeCheck()) {
    ...
    if (EditorGUILayout.BeginFadeGroup(m_ShowGenericSpriteSettings.faded)) {
      using(new EditorGUI.DisabledScope(true)) EditorGUILayout.PropertyField(
          m_SpritePackingTag, s_Styles.spritePackingTag);

      EditorGUILayout.PropertyField(m_SpritePixelsToUnits,
                                    s_Styles.spritePixelsPerUnit);
      ...
      if (EditorGUILayout.BeginFadeGroup(m_ShowSpriteMeshTypeOption.faded)) {
        EditorGUILayout.IntPopup(m_SpriteMeshType,
                                 s_Styles.spriteMeshTypeOptions, new[]{0, 1},
      ...
      if (m_SpriteMode.intValue == (int) SpriteImportMode.Single) {
        EditorGUILayout.Popup(m_Alignment, s_Styles.spriteAlignmentOptions,
                              s_Styles.spriteAlignment);
      ...
      if (m_SpriteMode.intValue != (int) SpriteImportMode.Polygon)
        ToggleFromInt(m_SpriteGenerateFallbackPhysicsShape,
                      s_Styles.spriteGenerateFallbackPhysicsShape);
      ...
      using(new EditorGUI.DisabledScope(targets.Length != 1)) {
        ...
        if (GUILayout.Button("Sprite Editor")) {
          if (HasModified()) {
            // To ensure Sprite Editor Window to have the latest texture import
            // setting, We must applied those modified values first.
            string dialogText = "Unapplied import settings for \'" +
                                ((TextureImporter) target).assetPath + "\'.\n";
            dialogText += "Apply and continue to sprite editor or cancel.";
            if (EditorUtility.DisplayDialog("Unapplied import settings",
                                            dialogText, "Apply", "Cancel")) {
              ApplyAndImport();
              SpriteUtilityWindow.ShowSpriteEditorWindow(this.assetTarget);

              // We reimported the asset which destroyed the editor, so we can't
              // keep running the UI here.
              GUIUtility.ExitGUI();
            }
          } else {
            SpriteUtilityWindow.ShowSpriteEditorWindow(this.assetTarget);
  ...
  void MipMapGUI(TextureInspectorGUIElement guiElements) {
    ...
    if (EditorGUILayout.BeginFadeGroup(m_ShowMipMapSettings.faded)) {
      ...
      EditorGUILayout.Popup(m_MipMapMode, s_Styles.mipMapFilterOptions,
                            s_Styles.mipMapFilter);

      ToggleFromInt(m_MipMapsPreserveCoverage,
                    s_Styles.mipMapsPreserveCoverage);
      if (m_MipMapsPreserveCoverage.intValue != 0 &&
          !m_MipMapsPreserveCoverage.hasMultipleDifferentValues) {
        EditorGUI.indentLevel++;
        EditorGUILayout.PropertyField(m_AlphaTestReferenceValue,
                                      s_Styles.alphaTestReferenceValue);
        EditorGUI.indentLevel--;
      }
      ...
      if (m_FadeOut.intValue > 0) {
        ...
        EditorGUILayout.MinMaxSlider(s_Styles.mipmapFadeOut, ref min, ref max,
                                     0, 10);
        if (EditorGUI.EndChangeCheck()) {
  ...
  void BumpGUI(TextureInspectorGUIElement guiElements) {
    ...
    if (EditorGUILayout.BeginFadeGroup(m_ShowBumpGenerationSettings.faded)) {
      ...
      EditorGUILayout.Popup(m_NormalMapFilter, s_Styles.bumpFilteringOptions,
                            s_Styles.bumpFiltering);
      EditorGUI.indentLevel--;
    }
    EditorGUILayout.EndFadeGroup();

    if (EditorGUI.EndChangeCheck())
      BaseTextureImportPlatformSettings.SyncPlatformSettings(
          m_PlatformSettings.ConvertAll<BaseTextureImportPlatformSettings>(
              x => x as BaseTextureImportPlatformSettings));
  }
  ...
  void TextureSettingsGUI() {
    ...
    // Wrap mode
    // NOTE: once we get ability to have 3D/Volume texture shapes, should pass
    // true for isVolume based on m_TextureShape
    bool isVolume = false;
    TextureInspector.WrapModePopup(m_WrapU, m_WrapV, m_WrapW, isVolume,
                                   ref m_ShowPerAxisWrapModes,
                                   assetTarget == null);
    ...
    if (m_NPOTScale.intValue == (int) TextureImporterNPOTScale.None &&
        (m_WrapU.intValue == (int) TextureWrapMode.Repeat ||
         m_WrapV.intValue == (int) TextureWrapMode.Repeat) &&
    ...
    filter = (FilterMode) EditorGUILayout.IntPopup(
        s_Styles.filterMode, (int) filter, s_Styles.filterModeOptions,
        m_FilterModeOptions);
    EditorGUI.showMixedValue = false;
    if (EditorGUI.EndChangeCheck())
  ...
  public override void OnInspectorGUI() {
    ...
    int newTextureType = EditorGUILayout.IntPopup(
        s_Styles.textureTypeTitle, m_TextureType.intValue,
        s_Styles.textureTypeOptions, s_Styles.textureTypeValues);
    EditorGUI.showMixedValue = false;
    // (case 857001) EndChangeCheck will return true even if the same value is
    // selected. Consequently the sprite will be reset to Single mode and looks
    // very confusing to the user.
    if (EditorGUI.EndChangeCheck() &&
        (m_TextureType.intValue != newTextureType)) {
      // please note that in GetSerializedPropertySettings() we will init
      // TextureImporterSettings from current state
      //   and at this point m_TextureType still has *old* value
      // meaning that we still need to change TextureImporterSettings
      // textureType manually NB we do these weird things partly because
      // ApplyTextureType has early out NB hence we want settings to have *old*
      // textureType when calling it
    ...
    // Texture Shape
    int[] shapeArray =
        s_Styles.textureShapeValuesDictionnary
            [m_TextureTypeGUIElements[(int) newTextureType].shapeCaps];
    using(new EditorGUI.DisabledScope(
        shapeArray.Length == 1 ||
        m_TextureType.intValue ==
            (int) TextureImporterType
                .Cookie)) // Cookie is a special case because the cookie type
                          // drives the shape of the texture
    {
      EditorGUI.BeginChangeCheck();
      EditorGUI.showMixedValue = m_TextureShape.hasMultipleDifferentValues;
      int newTextureShape = EditorGUILayout.IntPopup(
          s_Styles.textureShape, m_TextureShape.intValue,
          s_Styles.textureShapeOptionsDictionnary
              [m_TextureTypeGUIElements[(int) newTextureType].shapeCaps],
          s_Styles.textureShapeValuesDictionnary
              [m_TextureTypeGUIElements[(int) newTextureType].shapeCaps]);
      EditorGUI.showMixedValue = false;
    ...
    // Show advanced settings for texture types that have the same subset of
    // advanced settings (rather than just those that are exactly the same type)
    bool showAdvanced = false;
    if (m_TextureType.hasMultipleDifferentValues) {
      showAdvanced = true;
      int iteratedTextureType = (int) m_TextureTypes[0];
      TextureInspectorGUIElement firstAdvancedElements =
          m_TextureTypeGUIElements[iteratedTextureType].advancedElements;
      for (int selectionIndex = 1; selectionIndex < m_TextureTypes.Count;
           selectionIndex++) {
        iteratedTextureType = (int) m_TextureTypes[selectionIndex];
        if (firstAdvancedElements !=
            m_TextureTypeGUIElements[iteratedTextureType].advancedElements) {
          showAdvanced = false;
          break;
        }
    ...
    if (showAdvanced) {
      DoGUIElements(m_TextureTypeGUIElements[newTextureType].commonElements,
                    m_GUIElementsDisplayOrder);
      if (m_TextureTypeGUIElements[newTextureType].advancedElements != 0) {
        EditorGUILayout.Space();

        m_ShowAdvanced = EditorGUILayout.Foldout(m_ShowAdvanced,
                                                 s_Styles.showAdvanced, true);
        if (m_ShowAdvanced) {
          EditorGUI.indentLevel++;
          DoGUIElements(
              m_TextureTypeGUIElements[newTextureType].advancedElements,
              m_GUIElementsDisplayOrder);
          EditorGUI.indentLevel--;
        }
      }
    ...
    BaseTextureImportPlatformSettings.ShowPlatformSpecificSettings(
        m_PlatformSettings.ConvertAll<BaseTextureImportPlatformSettings>(
            x => x as BaseTextureImportPlatformSettings));
    ...
    // screw this - after lots of retries i have no idea how to poll it only
    // when we change related stuff
    UpdateImportWarning();
  ...
  bool ShouldDisplayGUIElement(TextureInspectorGUIElement guiElements,
                               TextureInspectorGUIElement guiElement) {
    return ((guiElements & guiElement) == guiElement);
  }

  private void
  DoGUIElements(TextureInspectorGUIElement guiElements,
                List<TextureInspectorGUIElement> guiElementsDisplayOrder) {
    foreach (TextureInspectorGUIElement guiElement in guiElementsDisplayOrder) {
      if (ShouldDisplayGUIElement(guiElements, guiElement) &&
          m_GUIElementMethods.ContainsKey(guiElement)) {
  ...
  void ApplySettingsToTexture() {
    foreach (AssetImporter importer in targets) {
      Texture tex =
          AssetDatabase.LoadMainAssetAtPath(importer.assetPath) as Texture;
      if (tex != null) // This can happen if the texture fails to import (for
                       // example, cube texture with non-PoT input).
      {
        if (m_Aniso.intValue != -1)
          TextureUtil.SetAnisoLevelNoDirty(tex, m_Aniso.intValue);
        if (m_FilterMode.intValue != -1)
          TextureUtil.SetFilterModeNoDirty(tex,
                                           (FilterMode) m_FilterMode.intValue);
        if ((m_WrapU.intValue != -1 || m_WrapV.intValue != -1 ||
             m_WrapW.intValue != -1) &&
            !m_WrapU.hasMultipleDifferentValues &&
            !m_WrapV.hasMultipleDifferentValues &&
            !m_WrapW.hasMultipleDifferentValues) {
          TextureUtil.SetWrapModeNoDirty(tex,
                                         (TextureWrapMode) m_WrapU.intValue,
                                         (TextureWrapMode) m_WrapV.intValue,
                                         (TextureWrapMode) m_WrapW.intValue);
        }
      }
    }
  ...
  static bool CountImportersWithAlpha(Object[] importers, out int count) {
    // DoesSourceTextureHaveAlpha will throw exception on importer reset
    // (593478).
    try {
      count = 0;
  ...
  void SetCookieMode(CookieMode cm) {
    ...
    case CookieMode.Spot:
      ...
      m_WrapU.intValue = m_WrapV.intValue = m_WrapW.intValue =
          (int) TextureWrapMode.Clamp;
      m_GenerateCubemap.intValue =
          (int) TextureImporterGenerateCubemap.AutoCubemap;
      m_TextureShape.intValue = (int) TextureImporterShape.Texture2D;
      break;
  ...
  internal static string[] BuildTextureStrings(int[] texFormatValues) {
    ...
    for (int i = 0; i < texFormatValues.Length; i++) {
      ...
      retval[i] =
          " " + (val < 0
                     ? "Auto"
                     : TextureUtil.GetTextureFormatString((TextureFormat) val));
    }
    return retval;
  }

  internal static bool
  IsFormatRequireCompressionSetting(TextureImporterFormat format) {
    return ArrayUtility.Contains<TextureImporterFormat>(
        TextureImporterInspector.kFormatsWithCompressionSettings, format);
  }

  private static bool IsPowerOfTwo(int f) { return ((f & (f - 1)) == 0); }

  bool CanReadWrite() {
    foreach (TextureImportPlatformSettings ps in m_PlatformSettings) {
      if (ps.model.platformTextureSettings.maxTextureSize >
          TextureImporter.MaxTextureSizeAllowedForReadable)
        return false;
    }
    return true;
  }
  ...
  public virtual void BuildTargetList() {
    ...
    m_PlatformSettings.Add(new TextureImportPlatformSettings(
        s_DefaultPlatformName, BuildTarget.StandaloneWindows, this));

    foreach (BuildPlatform bp in validPlatforms)
      m_PlatformSettings.Add(
          new TextureImportPlatformSettings(bp.name, bp.defaultTarget, this));
  }
  ...
  protected override void ResetValues() {
    ...
    System.Diagnostics.Debug.Assert(
        !HasModified(),
        "TextureImporter settings are marked as modified after calling Reset.");
    ApplySettingsToTexture();
  }

  protected override void Apply() {
    base.Apply();
    BaseTextureImportPlatformSettings.ApplyPlatformSettings(
        m_PlatformSettings.ConvertAll<BaseTextureImportPlatformSettings>(
            x => x as BaseTextureImportPlatformSettings));
  }
  ...
  public override void DrawPreview(Rect previewArea) {
    ...
    // Drawing texture previewers for VT only textures will have generated
    // texture tile request. We need to update the VT system to actually stream
    // these tiles into VRAM and render the textures correctly.
