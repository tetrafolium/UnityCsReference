...
internal partial class View : ScriptableObject {
  ...
  [SerializeField] MonoReloadableIntPtr m_ViewPtr;
  [SerializeField] View[] m_Children = new View[0];
  [System.NonSerialized] View m_Parent;
  [System.NonSerialized] ContainerWindow m_Window;
  ...
  protected virtual void SetPosition(Rect newPos) { m_Position = newPos; }

  // Only set the position.
  internal void SetPositionOnly(Rect newPos) { m_Position = newPos; }
  ...
  public Rect windowPosition {
    get {
      ...
      return new Rect(p.x + position.x, p.y + position.y, position.width,
                      position.height);
    }
  }
  ...
  //  Which window we're inside. don't set this directly, but change use
  //  AddChild, RemoveChild instead.
  ...
  // please don't modify this array directly. use AddChild or set the child's
  // parent view
  ...
  public void AddChild(View child) { AddChild(child, m_Children.Length); }
  public virtual void AddChild(View child, int idx) {
    ...
    if (idx != m_Children.Length - 1)
      System.Array.Copy(m_Children, idx, m_Children, idx + 1,
                        m_Children.Length - idx - 1);
  ...
  public virtual void RemoveChild(int idx) {
    ...
    System.Array.Copy(m_Children, idx + 1, m_Children, idx,
                      m_Children.Length - idx - 1);
    System.Array.Resize(ref m_Children, m_Children.Length - 1);
    ChildrenMinMaxChanged();
  }

  internal virtual void SetWindow(ContainerWindow win) { m_Window = win; }
  ...
  virtual protected bool OnFocus() { return true; }
}
} // namespace
