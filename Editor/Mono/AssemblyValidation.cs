...
static class AssemblyValidation {
  ...
  public struct Error {
    ...
    public void ClearFlags(ErrorFlags clearFlags) { flags &= ~clearFlags; }
  }
  ...
  public static Error[] ValidateAssemblies(string[] assemblyPaths,
                                           bool enableLogging) {
    var searchPaths = AssemblyHelper.GetDefaultAssemblySearchPaths();

    var assemblyDefinitions =
        LoadAssemblyDefinitions(assemblyPaths, searchPaths);
    ...
    var errors = ValidateAssemblyDefinitions(assemblyPaths, assemblyDefinitions,
  ...
  public static Error[] ValidateAssemblyDefinitionFiles() {
    var customScriptAssemblies =
        EditorCompilationInterface.Instance.GetCustomScriptAssemblies();
    ...
    foreach (var pluginImporter in pluginImporters) {
      ...
      var lowerPluginFilename = AssetPath.GetFileName(pluginAssetPath)
    ...
    foreach (var customScriptAssembly in customScriptAssemblies) {
      var lowerAsmdefFilename =
          $"{customScriptAssembly.Name.ToLower(CultureInfo.InvariantCulture)}.dll";

      string pluginPath;

      if (pluginFilenameToAssetPath.TryGetValue(lowerAsmdefFilename,
                                                out pluginPath)) {
        var error = new Error(){
            message =
                $"Plugin '{pluginPath}' has the same filename as Assembly Definition File '{customScriptAssembly.FilePath}'. Rename the assemblies to avoid hard to diagnose issues and crashes.",
            flags = ErrorFlags.AsmdefPluginConflict, assemblyPath = pluginPath};
  ...
  public static void
  PrintAssemblyDefinitions(AssemblyDefinition[] assemblyDefinitions) {
    foreach (var assemblyDefinition in assemblyDefinitions) {
      Console.WriteLine("[AssemblyValidation] Assembly: " +
  ...
  public static Error[] ValidateAssembliesInternal(
  ...
  public static Error[] ValidateAssemblyDefinitions(
      string[] assemblyPaths, AssemblyDefinition[] assemblyDefinitions,
      Func<string, bool> compatibleWithEditor) {
  ...
  public static AssemblyDefinition[] LoadAssemblyDefinitions(
      string[] assemblyPaths, string[] searchPaths) {
    ...
    for (int i = 0; i < assemblyPaths.Length; ++i) {
      assemblyDefinitions[i] =
          AssemblyDefinition.ReadAssembly(assemblyPaths[i], readerParameters);
      // Cecil tries to resolve references by filename, since Unity force loads
      // assemblies, then assembly reference will resolve even if the assembly
      // name does not match the assembly filename. So we register all
      // assemblies in in the resolver.
  ...
  public static void
  CheckAssemblyReferences(string[] assemblyPaths, Error[] errors,
    ...
    var assembliesAndReferencesArray =
        new AssemblyAndReferences[assemblyPaths.Length];
    ...
    for (int i = 0; i < assemblyDefinitions.Length; ++i) {
      ...
      assembliesAndReferencesArray[i] = new AssemblyAndReferences{
          assemblyIndex = i, referenceIndicies = new int[0]};
    }

    for (int i = 0; i < assemblyPaths.Length; ++i) {
      if (errors [i]
              .HasFlag(ErrorFlags.IncompatibleWithEditor))
        continue;
      ...
      ResolveAndSetupReferences(i, errors, assemblyDefinitions,
    ...
    do {
      ...
      foreach (var assemblyAndReferences in assembliesAndReferencesArray) {
        ...
        foreach (var referenceIndex in assemblyAndReferences
                     .referenceIndicies) {
          ...
          if (errors[assemblyIndex].flags == ErrorFlags.None &&
            ...
            if (referenceError.HasFlag(ErrorFlags.IncompatibleWithEditor)) {
              errors [assemblyIndex]
                  .Add(ErrorFlags.ReferenceHasErrors |
                           ErrorFlags.IncompatibleWithEditor,
                       string.Format(
                           "Reference '{0}' is incompatible with the editor.",
                           assemblyDefinitions[referenceIndex].Name.Name));
            } else {
              errors [assemblyIndex]
                  .Add(ErrorFlags.ReferenceHasErrors | referenceError.flags,
                       string.Format(
                           "Reference has errors '{0}'.",
                           assemblyDefinitions[referenceIndex].Name.Name));
            }
  ...
  public static void
  SetupEditorCompatibility(string[] assemblyPaths, Error[] errors,
                           Func<string, bool> compatibleWithEditor) {
    for (int i = 0; i < assemblyPaths.Length; ++i) {
      var assemblyPath = assemblyPaths[i];
      ...
      if (!compatibleWithEditor(assemblyPath)) {
        errors [i]
            .Add(ErrorFlags.IncompatibleWithEditor,
                 "Assembly is incompatible with the editor");
      }
    }
  }

  public static void ResolveAndSetupReferences(
      int index, Error[] errors, AssemblyDefinition[] assemblyDefinitions,
    ...
    var referenceIndieces = new List<int>{Capacity = assemblyReferences.Length};
    ...
    foreach (var reference in assemblyReferences) {
      try {
        ...
        if (assemblyDefinitionNameToIndex.TryGetValue(
                referenceAssemblyDefinition.Name.Name,
                out referenceAssemblyDefinitionIndex)) {
          bool isSigned = IsSigned(reference);
          if (isSigned) {
            var definition =
                assemblyDefinitions[referenceAssemblyDefinitionIndex];

            if (definition.Name.Version.ToString() !=
                    reference.Version.ToString() &&
                !IsInSameFolder(assemblyDefinition,
                                referenceAssemblyDefinition)) {
              errors [index]
                  .Add(
                      ErrorFlags.UnresolvableReference,
        ...
        errors [index]
            .Add(
                ErrorFlags.UnresolvableReference,
                string.Format(
                    "Unable to resolve reference '{0}'. Is the assembly missing or incompatible with the current platform?\nReference validation can be disabled in the Plugin Inspector.",
                    reference.Name));
      }
    }

    assemblyAndReferences[index].referenceIndicies =
        referenceIndieces.ToArray();
  }

  private static bool IsInSameFolder(AssemblyDefinition first,
                                     AssemblyDefinition second) {
    ...
    return firstAssemblyPath.Equals(secondAssemblyPath,
  ...
  public static AssemblyNameReference[] GetAssemblyNameReferences(
      AssemblyDefinition assemblyDefinition) {
    List<AssemblyNameReference> result =
        new List<AssemblyNameReference>{Capacity = 16};
