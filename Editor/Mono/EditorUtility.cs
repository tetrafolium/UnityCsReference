...
// Compression Quality. Corresponds to the settings in a
// [[wiki:class-Texture2D|texture inspector]].
...
public partial class EditorUtility {
  static class Content {
    ...
    static readonly string k_DialogOptOutForThisMachine =
        L10n.Tr("Do not show me this message again on this machine.");
    static readonly string k_DialogOptOutForThisSession =
        L10n.Tr("Do not show me this message again for this session.");
    public static string
    GetDialogOptOutMessage(DialogOptOutDecisionType dialogOptOutType) {
  ...
  private static readonly Stack<StringBuilder> _SbPool =
      new Stack<StringBuilder>();

  public delegate void SelectMenuItemFunction(object userData, string[] options,
                                              int selected);
  ...
  public static void CompressTexture(Texture2D texture, TextureFormat format,
  ...
  internal static Texture GetIconInActiveState(Texture icon) {
    ...
    if (!s_ActiveIconPathLUT.TryGetValue(iconInstanceId,
                                         out selectedIconPath)) {
  ...
  public static string SaveFilePanelInProject(string title, string defaultName,
                                              string extension,
                                              string message) {
    return Internal_SaveFilePanelInProject(title, defaultName, extension,
                                           message, "Assets");
  }
  ...
  public static void UnloadUnusedAssetsImmediate() { UnloadUnusedAssets(true); }

  public static void
  UnloadUnusedAssetsImmediate(bool includeMonoReferencesAsRoots) {
  ...
  public static bool
  GetDialogOptOutDecision(DialogOptOutDecisionType dialogOptOutDecisionType,
                          string dialogOptOutDecisionStorageKey) {
  ...
  public static void
  SetDialogOptOutDecision(DialogOptOutDecisionType dialogOptOutDecisionType,
                          string dialogOptOutDecisionStorageKey,
                          bool optOutDecision) {
    switch (dialogOptOutDecisionType) {
    case DialogOptOutDecisionType.ForThisMachine:
  ...
  public static bool
  DisplayDialog(string title, string message, string ok,
                DialogOptOutDecisionType dialogOptOutDecisionType,
                string dialogOptOutDecisionStorageKey) {
  ...
  DisplayDialog(string title, string message, string ok,
                ...
                DialogOptOutDecisionType dialogOptOutDecisionType,
                string dialogOptOutDecisionStorageKey) {
    if (GetDialogOptOutDecision(dialogOptOutDecisionType,
                                dialogOptOutDecisionStorageKey)) {
      return true;
    } else {
      bool optOutDecision;
      bool dialogDecision = DisplayDialog(
          title, message, ok, cancel,
          Content.GetDialogOptOutMessage(dialogOptOutDecisionType),
          out optOutDecision);
      // Cancel means the user pressed ESC as the Cancel button was grayed out.
      // Don't store the opt-out decision on cancel. Also, only store it if the
      // user opted out since it defaults to opt-in.
      if (dialogDecision && optOutDecision)
        SetDialogOptOutDecision(dialogOptOutDecisionType,
                                dialogOptOutDecisionStorageKey, optOutDecision);
      return dialogDecision;
    }
  }

  // TODO: This is an MVP solution. The OptOut option should be a check-box in
  // the dialog. To achieve that, this API will need to move to bindings and get
  // platform specific implementations.
  static bool DisplayDialog(string title, string message, string ok,
                            string cancel, string optOutText,
                            out bool optOutDecision) {
    if (string.IsNullOrEmpty(cancel)) {
      // we can't allow empty cancel buttons in this MVP workaround. Only the
      // two button dialog would be possible to use and it can't differentiate
      // between pressing a cancel button (labeled with OptOut text) and
      // pressing X or ESC.
      cancel = Content.Cancel;
    }
    int result = DisplayDialogComplex(
        title, message, ok, cancel, string.Format("{0} - {1}", ok, optOutText));
    // result 0 -> OK, 1 -> Cancel, 2 -> Ok & opt out
    optOutDecision = result == 2;
    return result != 1;
  }

  public static void DisplayPopupMenu(Rect position, string menuItemPath,
                                      MenuCommand command) {
    // Validate input. Fixes case 406024: 'Custom context menu in a custom
    // window crashes Unity'
    if (menuItemPath == "CONTEXT" || menuItemPath == "CONTEXT/" ||
        menuItemPath == "CONTEXT\\") {
      bool error = command == null || command.context == null;
      if (error) {
        Debug.LogError(
            "DisplayPopupMenu: invalid arguments: using CONTEXT requires a valid MenuCommand object. If you want a custom context menu then try using the GenericMenu.");
        return;
      }
    }

    Vector2 temp =
        GUIUtility.GUIToScreenPoint(new Vector2(position.x, position.y));
    position.x = temp.x;
    position.y = temp.y;
    Internal_DisplayPopupMenu(position, menuItemPath, command?.context,
                              command?.userData ?? 0);
    ResetMouseDown();
  }

  public static void DisplayCustomMenu(Rect position, GUIContent[] options,
                                       int selected,
                                       SelectMenuItemFunction callback,
                                       object userData) {
    DisplayCustomMenu(position, options, null, selected, callback, userData,
                      false);
  }
  ...
  public static void DisplayCustomMenu(Rect position, GUIContent[] options,
    ...
    DisplayCustomMenu(position, strings, enabled, selectedArray, callback,
                      userData, showHotkey);
  }
  ...
  public static void SetSelectedWireframeHidden(Renderer renderer,
                                                bool enabled) {
    SetSelectedRenderState(renderer, enabled
  ...
  CreateGameObjectWithHideFlags(string name, HideFlags flags,
  ...
  public static string[] CompileCSharp(string[] sources, string[] references,
                                       string[] defines, string outputFile) {
    return Scripting.Compilers.MicrosoftCSharpCompiler.Compile(
        sources, references, defines, outputFile,
  ...
  [Obsolete(
      "Use PrefabUtility.SaveAsPrefabAsset or PrefabUtility.SaveAsPrefabAssetAndConnect with a path instead.",
      false)]
  public static GameObject ReplacePrefab(GameObject go, Object targetPrefab) {
    return PrefabUtility.ReplacePrefab(go, targetPrefab,
                                       ReplacePrefabOptions.Default);
  }

  [Obsolete(
      "The concept of creating a completely empty Prefab has been discontinued. You can however use PrefabUtility.SaveAsPrefabAsset with an empty GameObject.",
      false)]
  ...
  [Obsolete(
      "Use PrefabUtility.GetPrefabAssetType and PrefabUtility.GetPrefabInstanceStatus to get the full picture about Prefab types.",
      false)]
  ...
  [Obsolete(
      "Use PrefabUtility.GetOutermostPrefabInstanceRoot if source is a Prefab instance or source.transform.root.gameObject if source is a Prefab Asset object.",
      false)]
  ...
  internal static void DisplayCustomMenu(Rect position, string[] options,
                                         int[] selected,
                                         SelectMenuItemFunction callback,
                                         object userData) {
  ...
  internal static void DisplayCustomMenu(Rect position, string[] options,
    ...
    DisplayCustomMenuWithSeparators(position, options, separator, selected,
                                    callback, userData, showHotkey);
  }

  internal static void DisplayCustomMenuWithSeparators(
      Rect position, string[] options, bool[] separator, int[] selected,
      SelectMenuItemFunction callback, object userData) {
    DisplayCustomMenuWithSeparators(position, options, separator, selected,
  ...
  internal static void DisplayCustomMenuWithSeparators(
      Rect position, string[] options, bool[] separator, int[] selected,
      SelectMenuItemFunction callback, object userData, bool showHotkey) {
    Vector2 temp =
        GUIUtility.GUIToScreenPoint(new Vector2(position.x, position.y));
    position.x = temp.x;
    position.y = temp.y;
    ...
    Internal_DisplayCustomMenu(position, options, enabled, separator, selected,
                               callback, userData, showHotkey);
    ResetMouseDown();
  }

  internal static void DisplayCustomMenu(Rect position, string[] options,
                                         bool[] enabled, int[] selected,
                                         SelectMenuItemFunction callback,
                                         object userData) {
    DisplayCustomMenu(position, options, enabled, selected, callback, userData,
                      false);
  }
  ...
  internal static void DisplayCustomMenu(Rect position, string[] options,
                                         ...
                                         object userData, bool showHotkey) {
    bool[] separator = new bool[options.Length];
    DisplayCustomMenuWithSeparators(position, options, enabled, separator,
                                    selected, callback, userData, showHotkey);
  }

  public static void DisplayCustomMenuWithSeparators(
      Rect position, string[] options, bool[] enabled, bool[] separator,
      int[] selected, SelectMenuItemFunction callback, object userData) {
    DisplayCustomMenuWithSeparators(position, options, enabled, separator,
                                    selected, callback, userData, false);
  }
  ...
  internal static void DisplayCustomMenuWithSeparators(
      Rect position, string[] options, bool[] enabled, bool[] separator,
      int[] selected, SelectMenuItemFunction callback, object userData,
      bool showHotkey, bool allowDisplayNames) {
    Vector2 temp =
        GUIUtility.GUIToScreenPoint(new Vector2(position.x, position.y));
    position.x = temp.x;
    position.y = temp.y;
  ...
  internal static void DisplayObjectContextMenu(Rect position, Object context,
                                                int contextUserData) {
  ...
  internal static void DisplayObjectContextMenu(Rect position, Object[] context,
    ...
    if (context != null && context.Length == 1 && context[0] is Component) {
      ...
      if (PrefabUtility.GetCorrespondingConnectedObjectFromSource(
              targetComponent.gameObject) == null) {
      }
      // Handle added component.
      else if (PrefabUtility.GetCorrespondingObjectFromSource(targetObject) ==
                   null &&
               targetComponent != null) {
        GameObject instanceGo = targetComponent.gameObject;
        PrefabUtility.HandleApplyRevertMenuItems(
            "Added Component", instanceGo,
            (menuItemContent, sourceGo) => {
              ...
              if (!PrefabUtility.IsPartOfPrefabThatCanBeAppliedTo(rootObject) ||
                ...
                pm.AddItem(menuItemContent, false,
                           TargetChoiceHandler.ApplyPrefabAddedComponent, info);
            },
            (menuItemContent) => {
              pm.AddItem(menuItemContent, false,
                         TargetChoiceHandler.RevertPrefabAddedComponent,
                         targetComponent);
            });
      } else {
        ...
        if (hasPrefabOverride) {
          ...
          PrefabUtility.HandleApplyRevertMenuItems(
              "Modified Component", targetObject,
              (menuItemContent, sourceObject) => {
                ...
                if (!PrefabUtility.IsPartOfPrefabThatCanBeAppliedTo(
                  ...
                  pm.AddItem(menuItemContent, false,
                             TargetChoiceHandler.ApplyPrefabObjectOverride,
                             info);
              },
              (menuItemContent) => {
                pm.AddItem(menuItemContent, false,
                           TargetChoiceHandler.RevertPrefabObjectOverride,
                           targetObject);
              },
              defaultOverrides);
        }
  ...
  internal static void Internal_DisplayPopupMenu(Rect position,
                                                 string menuItemPath,
                                                 Object context,
                                                 int contextUserData) {
  ...
  internal static GameObject InstantiateForAnimatorPreview(Object original) {
    ...
    GameObject go = InstantiateRemoveAllNonAnimationComponents(
        original, Vector3.zero, Quaternion.identity) as GameObject;
    go.name = go.name + "AnimatorPreview"; // To avoid FindGameObject picking up
                                           // our dummy object
    go.tag = "Untagged";
  ...
  InstantiateRemoveAllNonAnimationComponents(Object original, Vector3 position,
                                             Quaternion rotation) {
    if (original == null)
      throw new ArgumentException(
          "The Prefab you want to instantiate is null.");

    return Internal_InstantiateRemoveAllNonAnimationComponentsSingle(
        original, position, rotation);
  }

  private static void
  Internal_DisplayCustomMenu(Rect screenPosition, string[] options,
                             bool[] enabled, bool[] separator, int[] selected,
                             SelectMenuItemFunction callback, object userData,
                             bool showHotkey, bool allowDisplayNames = false) {
    DisplayCustomContextPopupMenu(screenPosition, options, enabled, separator,
                                  selected, callback, userData, showHotkey,
                                  allowDisplayNames);
  }
