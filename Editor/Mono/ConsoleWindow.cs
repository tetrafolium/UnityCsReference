...
internal class ConsoleWindow : EditorWindow,
  ...
  internal class Constants {
    ...
    public static readonly GUIContent Clear =
        EditorGUIUtility.TrTextContent("Clear", "Clear console entries");
    public static readonly GUIContent ClearOnPlay =
        EditorGUIUtility.TrTextContent("Clear on Play");
    public static readonly GUIContent ClearOnBuild =
        EditorGUIUtility.TrTextContent("Clear on Build");
    public static readonly GUIContent Collapse = EditorGUIUtility.TrTextContent(
        "Collapse", "Collapse identical entries");
    public static readonly GUIContent ErrorPause =
        EditorGUIUtility.TrTextContent("Error Pause",
                                       "Pause Play Mode on error");
    public static readonly GUIContent StopForAssert =
        EditorGUIUtility.TrTextContent("Stop for Assert");
    public static readonly GUIContent StopForError =
        EditorGUIUtility.TrTextContent("Stop for Error");
    ...
    public static void Init() {
      ...
      // If the console window isn't open OnEnable() won't trigger so it will
      // end up with 0 lines, so we always make sure we read it up when we
      // initialize here.
      LogStyleLineCount = EditorPrefs.GetInt("ConsoleWindowLogLineCount", 2);
    }

    private static void UpdateLogStyleFixedHeights() {
      // Whenever we change the line height count or the styles are set we need
      // to update the fixed height of the following GuiStyles so the entries do
      // not get cropped incorrectly.
      ErrorStyle.fixedHeight =
          (LogStyleLineCount * ErrorStyle.lineHeight) + ErrorStyle.border.top;
      WarningStyle.fixedHeight = (LogStyleLineCount * WarningStyle.lineHeight) +
                                 WarningStyle.border.top;
      LogStyle.fixedHeight =
          (LogStyleLineCount * LogStyle.lineHeight) + LogStyle.border.top;
    }
  }
  ...
  SplitterState spl = SplitterState.FromRelative(new float[]{70, 30},
                                                 new float[]{32, 32}, null);
  ...
  class ConsoleAttachToPlayerState : GeneralConnectionState {
    static class Content {
      public static GUIContent PlayerLogging =
          EditorGUIUtility.TrTextContent("Player Logging");
      public static GUIContent FullLog =
          EditorGUIUtility.TrTextContent("Full Log (Developer Mode Only)");
    }

    public ConsoleAttachToPlayerState(EditorWindow parentWindow,
                                      Action<string> connectedCallback = null)
        : base(parentWindow, connectedCallback) {
      // This is needed to force initialize the instance and the state so that
      // messages from players are received and printed to the console (if that
      // is the serialized state) on creation of the ConsoleWindow UI instead of
      // when the uer first clicks on the dropdown, and triggers AddItemsToMenu.
      PlayerConnectionLogReceiver.instance.State =
          PlayerConnectionLogReceiver.instance.State;
    }

    bool IsConnected() {
      return PlayerConnectionLogReceiver.instance.State !=
             PlayerConnectionLogReceiver.ConnectionState.Disconnected;
    }
    ...
    public override void AddItemsToMenu(GenericMenu menu, Rect position) {
      ...
      menu.AddItem(Content.PlayerLogging, IsConnected(),
                   PlayerLoggingOptionSelected);
      if (IsConnected()) {
        // All other options but the first are only available if logging is
        // enabled
        menu.AddItem(Content.FullLog, IsLoggingFullLog(),
                     FullLogOptionSelected);
        menu.AddSeparator("");
  ...
  public void DoLogChanged() { ms_ConsoleWindow.Repaint(); }
  ...
  internal void OnEnable() {
    ...
    // Update the filter on enable for DomainReload(keep current filter) and
    // window opening(reset filter because m_searchText is null)
    ...
    Constants.LogStyleLineCount =
        EditorPrefs.GetInt("ConsoleWindowLogLineCount", 2);
  }
  ...
  private int RowHeight =>
      (Constants.LogStyleLineCount > 1
           ? Mathf.Max(32, (Constants.LogStyleLineCount * m_LineHeight))
           : m_LineHeight) +
      m_BorderHeight;
  ...
  static internal Texture2D GetIconForErrorMode(int mode, bool large) {
    // Errors
    if (HasMode(mode, Mode.Fatal | Mode.Assert | Mode.Error |
                          Mode.ScriptingError | Mode.AssetImportError |
                          Mode.ScriptCompileError | Mode.GraphCompileError |
                          Mode.ScriptingAssertion))
      return large ? iconError : iconErrorSmall;
    // Warnings
    if (HasMode(mode, Mode.ScriptCompileWarning | Mode.ScriptingWarning |
                          Mode.AssetImportWarning))
  ...
  static internal GUIStyle GetStyleForErrorMode(int mode, bool isIcon,
                                                bool isSmall) {
    // Errors
    if (HasMode(mode, Mode.Fatal | Mode.Assert | Mode.Error |
                          Mode.ScriptingError | Mode.AssetImportError |
                          Mode.ScriptCompileError | Mode.GraphCompileError |
                          Mode.ScriptingAssertion | Mode.ScriptingException)) {
      if (isIcon) {
        if (isSmall) {
  ...
  [UsedImplicitly] // Used implicitly with CallStaticMonoMethod("ConsoleWindow",
                   // "ShowConsoleRow", param);
  ...
  void UpdateListView() {
    ...
    // We reset the scroll list to auto scrolling whenever the log entry count
    // is modified
  ...
  bool HasSpaceForExtraButtons() { return position.width > 420; }
  ...
  internal void OnGUI() {
    ...
    if (!m_HasUpdatedGuiStyles) {
      ...
      m_BorderHeight =
          Constants.ErrorStyle.border.top + Constants.ErrorStyle.border.bottom;
      UpdateListView();
    }
    ...
    if (EditorGUILayout.DropDownToggle(ref clearClicked, Constants.Clear,
      ...
      menu.AddItem(Constants.ClearOnPlay, clearOnPlay,
                   () => { SetFlag(ConsoleFlags.ClearOnPlay, !clearOnPlay); });
      menu.AddItem(Constants.ClearOnBuild, clearOnBuild, () => {
        SetFlag(ConsoleFlags.ClearOnBuild, !clearOnBuild);
      });
    ...
    if (m_ListView.totalRows != currCount) {
      ...
      if (m_ListView.scrollPos.y >=
          m_ListView.rowHeight * m_ListView.totalRows - ms_LVHeight) {
        m_ListView.scrollPos.y = currCount * RowHeight - ms_LVHeight;
      }
    }

    bool wasCollapsed = HasFlag(ConsoleFlags.Collapse);
    SetFlag(ConsoleFlags.Collapse,
            GUILayout.Toggle(wasCollapsed, Constants.Collapse,
                             Constants.MiniButton));
    ...
    if (HasSpaceForExtraButtons()) {
      SetFlag(ConsoleFlags.ErrorPause,
              GUILayout.Toggle(HasFlag(ConsoleFlags.ErrorPause),
                               Constants.ErrorPause, Constants.MiniButton));
      PlayerConnectionGUILayout.ConnectionTargetSelectionDropdown(
          m_ConsoleAttachToPlayerState, EditorStyles.toolbarDropDown);
    }
    ...
    bool setLogFlag = GUILayout.Toggle(
        HasFlag(ConsoleFlags.LogLevelLog),
        new GUIContent((logCount <= 999 ? logCount.ToString() : "999+"),
                       logCount > 0 ? iconInfoSmall : iconInfoMono),
        Constants.MiniButton);
    bool setWarningFlag = GUILayout.Toggle(
        HasFlag(ConsoleFlags.LogLevelWarning),
        new GUIContent((warningCount <= 999 ? warningCount.ToString() : "999+"),
                       warningCount > 0 ? iconWarnSmall : iconWarnMono),
        Constants.MiniButton);
    bool setErrorFlag = GUILayout.Toggle(
        HasFlag(ConsoleFlags.LogLevelError),
        new GUIContent((errorCount <= 999 ? errorCount.ToString() : "999+"),
                       errorCount > 0 ? iconErrorSmall : iconErrorMono),
        Constants.MiniButtonRight);
    ...
    using(new GettingLogEntriesScope(m_ListView)) {
      ...
      foreach (ListViewElement el in ListViewGUI.ListView(
                   m_ListView, ListViewOptions.wantsRowMultiSelection,
                   Constants.Box)) {
        if (e.type == EventType.MouseDown && e.button == 0 &&
            el.position.Contains(e.mousePosition)) {
          ...
          int mode = 0;
          string text = null;
          LogEntries.GetLinesAndModeFromEntryInternal(
              el.row, Constants.LogStyleLineCount, ref mode, ref text);
          ...
          // Draw the background
          GUIStyle s = el.row % 2 == 0 ? Constants.OddBackground
                                       : Constants.EvenBackground;
          s.Draw(el.position, false, false, entryIsSelected, false);

          // Draw the icon
          GUIStyle iconStyle = GetStyleForErrorMode(
              mode, true, Constants.LogStyleLineCount == 1);
          ...
          GUIStyle errorModeStyle = GetStyleForErrorMode(
              mode, false, Constants.LogStyleLineCount == 1);
          var textRect = el.position;
          textRect.x += offset;

          if (string.IsNullOrEmpty(m_SearchText))
            errorModeStyle.Draw(textRect, tempContent, id,
                                m_ListView.row == el.row);
          else {
            // the whole text contains the searchtext, we have to know where it
            // is
            int startIndex =
                text.IndexOf(m_SearchText, StringComparison.OrdinalIgnoreCase);
            if (startIndex == -1) // the searchtext is not in the visible text,
                                  // we don't show the selection
              errorModeStyle.Draw(textRect, tempContent, id,
                                  m_ListView.row == el.row);
            else // the searchtext is visible, we show the selection
            {
              ...
              const bool hasKeyboardFocus =
                  true; // This ensure we draw the selection text over the
                        // label.
              const bool drawAsComposition = false;
              Color selectionColor = GUI.skin.settings.selectionColor;

              errorModeStyle.DrawWithTextSelection(
                  textRect, tempContent, isActive, hasKeyboardFocus, startIndex,
                  endIndex, drawAsComposition, selectionColor);
            }
          }

          if (collapsed) {
            Rect badgeRect = el.position;
            tempContent.text = LogEntries.GetEntryCount(el.row).ToString(
            ...
            badgeRect.yMin +=
                ((badgeRect.yMax - badgeRect.yMin) - badgeSize.y) * 0.5f;
            badgeRect.x -= 5f;
      ...
      if (selectedRow != -1) {
        if (m_ListView.scrollPos.y >=
            m_ListView.rowHeight * m_ListView.totalRows - ms_LVHeight)
          m_ListView.scrollPos.y =
              m_ListView.rowHeight * m_ListView.totalRows - ms_LVHeight - 1;
      }

      // Make sure the selected entry is up to date
      if (m_ListView.totalRows == 0 || m_ListView.row >= m_ListView.totalRows ||
          m_ListView.row < 0) {
        ...
        if (m_ListView.selectionChanged ||
            !m_ActiveText.Equals(entry.message)) {
          SetActiveEntry(entry);
        }

        // If copy, get the messages from selected rows
        if (e.type == EventType.ExecuteCommand &&
            e.commandName == EventCommandNames.Copy &&
            m_ListView.selectedItems != null) {
          m_CopyString.Clear();
          for (int rowIndex = 0; rowIndex < m_ListView.selectedItems.Length;
               rowIndex++) {
      ...
      if ((GUIUtility.keyboardControl == m_ListView.ID) &&
    ...
    // Prevent dead locking in EditorMonoConsole by delaying callbacks (which
    // can log to the console) until after LogEntries.EndGettingEntries() has
    // been called (this releases the mutex in EditorMonoConsole so logging
    // again is allowed). Fix for case 1081060.
    ...
    float height = Constants.MessageStyle.CalcHeight(
        GUIContent.Temp(stackWithHyperlinks), position.width);
    EditorGUILayout.SelectableLabel(stackWithHyperlinks, Constants.MessageStyle,
                                    GUILayout.ExpandWidth(true),
                                    GUILayout.ExpandHeight(true),
                                    GUILayout.MinHeight(height + 10));
    ...
    if ((e.type == EventType.ValidateCommand ||
        ...
        e.commandName == EventCommandNames.Copy && m_CopyString != null) {
      if (e.type == EventType.ExecuteCommand)
  ...
  private void SearchField(Event e) {
    ...
    Rect rect = GUILayoutUtility.GetRect(
        ...
        GUILayout.MinWidth(60), GUILayout.MaxWidth(300));
  ...
  internal static string StacktraceWithHyperlinks(string stacktraceText) {
    ...
    var lines =
        stacktraceText.Split(new string[]{"\n"}, StringSplitOptions.None);
    for (int i = 0; i < lines.Length; ++i) {
      string textBeforeFilePath = ") (at ";
      int filePathIndex =
          lines [i]
              .IndexOf(textBeforeFilePath, StringComparison.Ordinal);
      if (filePathIndex > 0) {
        filePathIndex += textBeforeFilePath.Length;
        if (lines [i]
            [filePathIndex] != '<') // sometimes no url is given, just an id
                                    // between <>, we can't do an hyperlink
        {
          string filePathPart = lines [i]
                                    .Substring(filePathIndex);
          int lineIndex = filePathPart.LastIndexOf(
              ":", StringComparison.Ordinal); // LastIndex because the url can
                                              // contain ':' ex:"C:"
          if (lineIndex > 0) {
            int endLineIndex = filePathPart.LastIndexOf(
                ")",
                StringComparison.Ordinal); // LastIndex because files or folder
                                           // in the url can contain ')'
            if (endLineIndex > 0) {
              string lineString = filePathPart.Substring(
                  lineIndex + 1, (endLineIndex) - (lineIndex + 1));
              string filePath = filePathPart.Substring(0, lineIndex);

              textWithHyperlinks.Append(lines [i]
                                            .Substring(0, filePathIndex));
              textWithHyperlinks.Append("<a href=\"" + filePath + "\"" +
                                        " line=\"" + lineString + "\">");
    ...
    // Remove the last \n
    if (textWithHyperlinks.Length >
        0) // textWithHyperlinks always ends with \n if it is not empty
  ...
  public void ToggleLogStackTracesForAll(object userData) {
    foreach (LogType logType in Enum.GetValues(typeof(LogType)))
      PlayerSettings.SetStackTraceLogType(logType,
                                          (StackTraceLogType) userData);
  }
  ...
  public void AddItemsToMenu(GenericMenu menu) {
    ...
    menu.AddItem(EditorGUIUtility.TrTextContent("Show Timestamp"),
                 HasFlag(ConsoleFlags.ShowTimestamp), SetTimestamp);

    if (m_DevBuild) {
      menu.AddItem(Constants.StopForAssert, HasFlag(ConsoleFlags.StopForAssert),
                   () => {
                     SetFlag(ConsoleFlags.StopForAssert,
                             !HasFlag(ConsoleFlags.StopForAssert));
                   });
      menu.AddItem(Constants.StopForError, HasFlag(ConsoleFlags.StopForError),
                   () => {
                     SetFlag(ConsoleFlags.StopForError,
                             !HasFlag(ConsoleFlags.StopForError));
                   });
    }
    ...
    for (int i = 1; i <= 10; ++i) {
      ...
      menu.AddItem(
          new GUIContent(string.Format("Log Entry/{0} {1}", i, lineString)),
          i == Constants.LogStyleLineCount, SetLogLineCount, i);
    }
  ...
  private void AddStackTraceLoggingMenu(GenericMenu menu) {
    // TODO: Maybe remove this, because it basically duplicates UI in
    // PlayerSettings
    foreach (LogType logType in Enum.GetValues(typeof(LogType))) {
      foreach (StackTraceLogType stackTraceLogType in Enum.GetValues(
        ...
        menu.AddItem(
            EditorGUIUtility.TrTextContent("Stack Trace Logging/" + logType +
                                           "/" + stackTraceLogType),
    ...
    int stackTraceLogTypeForAll =
        (int) PlayerSettings.GetStackTraceLogType(LogType.Log);
    foreach (LogType logType in Enum.GetValues(typeof(LogType))) {
      if (PlayerSettings.GetStackTraceLogType(logType) !=
          (StackTraceLogType) stackTraceLogTypeForAll) {
    ...
    foreach (StackTraceLogType stackTraceLogType in Enum.GetValues(
                 typeof(StackTraceLogType))) {
      menu.AddItem(EditorGUIUtility.TrTextContent("Stack Trace Logging/All/" +
                                                  stackTraceLogType),
                   (StackTraceLogType)
                           stackTraceLogTypeForAll == stackTraceLogType,
  ...
  private void SetFilter(string filteringText) {
    if (filteringText == null) {
      ...
      LogEntries.SetFilteringText(
          filteringText); // Reset the active entry when we change the filtering
                          // text
    }
    SetActiveEntry(null);
  }
  ...
  [UsedImplicitly]
  private static event EntryDoubleClickedDelegate
  ...
  [UsedImplicitly] // This method is used by the Visual Scripting project.
                   // Please do not delete. Contact @husseink for more
                   // information.
      internal void AddMessageWithDoubleClickCallback(string message,
                                                      string file, int mode,
                                                      int instanceID) {
    var outputEntry = new LogEntry{message = message, file = file, mode = mode,
