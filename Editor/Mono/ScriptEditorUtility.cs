...
static readonly List<Func<Installation[]> > k_PathCallbacks =
	new List<Func<Installation[]> >();
...
[Obsolete(
	 "This functionality is going to be removed. See IExternalCodeEditor for more information",
	 false)]
public static ScriptEditor GetScriptEditorFromPath(string path) {
	...
	if (lowerCasePath.Contains("monodevelop") ||
	    lowerCasePath.Contains("xamarinstudio") ||
	    lowerCasePath.Contains("xamarin studio"))
	...
	if (lowerCasePath.EndsWith("vcsexpress.exe") ||
	    lowerCasePath.EndsWith("wdexpress.exe"))
...
internal static bool IsVisualStudioForMac(string path) {
	...
	var filename =
		Path.GetFileName(Paths.UnifyDirectorySeparator(lowerCasePath))
		.Replace(" ", "");
	return filename.StartsWith("visualstudio") && !filename.Contains("code") &&
...
[Obsolete("This method has been moved to CodeEditor.SetExternalScriptEditor",
          false)]
...
[Obsolete(
	 "This functionality has been moved to the IExternalCodeEditor packages",
	 true)]
static string GetScriptEditorArgsKey(string path) {
	// Starting in Unity 5.5, we support setting script editor arguments on OSX
	// and use then when opening the script editor. Before Unity 5.5, we would
	// still save the default script editor args in EditorPrefs, even though we
	// never used them. This means that the user potentially has some script
	// editor args saved and once he upgrades to 5.5, they will be used when
	// open the script editor. Which unintended and causes a regression in
	// behaviour. So on OSX we change the key for per application for script
	// editor args, to avoid reading the one from previous versions.
...
static string GetDefaultStringEditorArgs() {
	...
	// If the script editor args are set, we only launch the script editor with
	// args specified and do not use the built-in mechanism for opening script
	// files.
	if (Application.platform == RuntimePlatform.OSXEditor)
		return "";
...
[Obsolete(
	 "This method is being internalized, please use UnityEditorInternal.CodeEditorUtility.GetFoundScriptEditorPaths",
	 false)]
public static Dictionary<string, string>
GetFoundScriptEditorPaths(RuntimePlatform platform) {
	return CodeEditor.Editor.GetFoundScriptEditorPaths();
}

static void AddIfDirectoryExists(string name, string path,
