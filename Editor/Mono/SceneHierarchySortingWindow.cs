...
internal static bool ShowAtPosition(Vector2 pos, List<InputData> data,
	...
	if (!justClosed) {
		...
		if (s_SceneHierarchySortingWindow == null)
			s_SceneHierarchySortingWindow =
				CreateInstance<SceneHierarchySortingWindow>();
...
private void Init(Vector2 pos, List<InputData> data,
                  OnSelectCallback callback) {
	// Has to be done before calling Show / ShowWithMode
	// pos = GUIUtility.GUIToScreenPoint(pos);

	Rect buttonRect = new Rect(pos.x, pos.y - 16, 16,
	                           16); // fake a button: we know we are showing it
	                                // below the bottonRect if possible
	buttonRect = GUIUtility.GUIToScreenRect(buttonRect);
	data.Sort(delegate(InputData lhs, InputData rhs) {
...
void DrawListElement(Rect rect, InputData data) {
	...
	GUI.Toggle(rect, data.m_Selected, EditorGUIUtility.TempContent(data.m_Name),
	           s_Styles.menuItem);
