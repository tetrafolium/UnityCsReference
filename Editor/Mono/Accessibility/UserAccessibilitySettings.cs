...
namespace UnityEditor.Accessibility {
  ...
  // NOTE: The preferences in this class are currently only exposed via a
  // context menu in the ProfilerWindow these toggles need to instead be moved
  // to e.g., the Preferences menu before they are used elsewhere
  internal static class UserAccessiblitySettings {
    static UserAccessiblitySettings() {
      s_ColorBlindCondition = (ColorBlindCondition) EditorPrefs.GetInt(
          k_ColorBlindConditionPrefKey, (int) ColorBlindCondition.Default);
    }

    private const string k_ColorBlindConditionPrefKey =
        "AccessibilityColorBlindCondition";
