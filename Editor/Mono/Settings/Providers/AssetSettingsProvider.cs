...
public class AssetSettingsProvider : SettingsProvider {
  static class Styles {
    ...
    public static StyleBlock settingsBtn =>
        EditorResources.GetStyle("sb-settings-icon-btn");
  }
  ...
  public AssetSettingsProvider(string settingsWindowPath,
  ...
  public AssetSettingsProvider(string settingsWindowPath,
                               Func<UnityEngine.Object> settingsGetter)
      : this(settingsWindowPath,
             () => CreateEditorFromSettingsObject(settingsGetter())) {}

  public static AssetSettingsProvider
  ...
  public static AssetSettingsProvider
  CreateProviderFromObject(string settingsWindowPath,
                           UnityEngine.Object settingsObj,
                           IEnumerable<string> keywords = null) {
    return new AssetSettingsProvider(
        settingsWindowPath, () => CreateEditorFromSettingsObject(settingsObj),
        keywords);
  }
  ...
  public override void OnGUI(string searchContext) {
    if (settingsEditor != null) {
      using(new EditorGUI.DisabledScope(!settingsEditor.IsEnabled())) {
        using(new SettingsWindow.GUIScope()) settingsEditor.OnInspectorGUI();

        // Emulate the Inspector by handling DnD at the native level.
        var remainingRect = GUILayoutUtility.GetRect(
            GUIContent.none, GUIStyle.none, GUILayout.ExpandHeight(true));
        if ((Event.current.type == EventType.DragUpdated ||
             Event.current.type == EventType.DragPerform) &&
            remainingRect.Contains(Event.current.mousePosition)) {
          DragAndDrop.visualMode = DragAndDropService.Drop(
              DragAndDropService.kInspectorDropDstId,
              new[]{settingsEditor.target},
              Event.current.type == EventType.DragPerform);
  ...
  public override void OnTitleBarGUI() {
    if (settingsEditor != null) {
      using(new EditorGUI.DisabledScope(!settingsEditor.IsEnabled())) {
        ...
        var btnMargin =
            Styles.settingsBtn.GetFloat(StyleCatalogKeyword.marginTop);
        ...
        GUI.enabled = true;
        var showMenu =
            GUI.Button(settingsRect, EditorGUI.GUIContents.titleSettingsIcon,
  ...
  CreateEditorFromSettingsObject(UnityEngine.Object settingsObj) {
    if (settingsObj != null) {
      return Editor.CreateEditor(settingsObj);
    }
    return null;
