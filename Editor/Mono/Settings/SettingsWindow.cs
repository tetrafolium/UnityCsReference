...
internal class SettingsWindow : EditorWindow, IHasCustomMenu {
  [SerializeField]
  private Vector2 m_PosLeft;
  [SerializeField]
  private Vector2 m_PosRight;

  [SerializeField]
  private SettingsScope m_Scope;
  [SerializeField]
  public float m_SplitterFlex = 0.2f;
  ...
  private static class Styles {
    public static StyleBlock window =>
        EditorResources.GetStyle("sb-settings-window");
    public static StyleBlock settingsPanel =>
        EditorResources.GetStyle("sb-settings-panel-client-area");
    public static StyleBlock header =>
        EditorResources.GetStyle("sb-settings-header");
  }

  public static float s_DefaultLabelWidth =>
      Styles.window.GetFloat("-unity-label-width");
  public static float s_DefaultLayoutMaxWidth =>
      Styles.window.GetFloat("-unity-max-layout-width");

  public SettingsWindow() : this(SettingsScope.Project) {}

  public SettingsWindow(SettingsScope scope) {
    m_Scope = scope;
    titleContent.text =
        scope == SettingsScope.Project ? "Project Settings" : "Preferences";
  }

  internal SettingsProvider[] GetProviders() { return m_Providers; }
  ...
  public void AddItemsToMenu(GenericMenu menu) {
    if (Unsupported.IsDeveloperMode()) {
      menu.AddItem(EditorGUIUtility.TrTextContent("Print Provider Keywords"),
                   false, PrintProviderKeywords);
      menu.AddItem(EditorGUIUtility.TrTextContent("Refresh providers"), false,
  ...
  internal int GetVisibleProviderCount() { return m_TreeView.GetRows().Count; }
  ...
  internal void OnDisable() {
    ...
    if (m_TreeView != null && m_TreeView.currentProvider != null)
      EditorPrefs.SetString(
          GetPrefKeyName(titleContent.text + "_current_provider"),
          m_TreeView.currentProvider.settingsPath);
  ...
  private void RestoreSelection() {
    var lastSelectedProvider = EditorPrefs.GetString(
        GetPrefKeyName(titleContent.text + "_current_provider"), "");
    if (!string.IsNullOrEmpty(lastSelectedProvider) &&
        Array.Find(m_Providers, provider => provider.settingsPath ==
  ...
  private void Init() {
    m_Providers = SettingsService.FetchSettingsProviders()
                      .Where(p => p.scope == m_Scope)
                      .ToArray();

    WarnAgainstDuplicates();

    m_SplitterFlex = EditorPrefs.GetFloat(GetPrefKeyName(nameof(m_Splitter)),
                                          m_SplitterFlex);
    ...
    foreach (var provider in m_Providers) {
      ...
      if (!provider.icon) {
        provider.icon =
            EditorGUIUtility.FindTexture("UnityEditor/EditorSettings Icon");
      }
    }
  ...
  private void WarnAgainstDuplicates() {
    ...
    foreach (var g in m_Providers.GroupBy(x => x.settingsPath)) {
      if (g.Count() > 1)
        Debug.LogWarning(
            $"There are {g.Count()} settings providers with the same name {g.Key}.");
    }
  }

  private void ProviderChanged(SettingsProvider lastSelectedProvider,
                               SettingsProvider newlySelectedProvider) {
    ...
    if (newlySelectedProvider != null) {
      ...
      EditorPrefs.SetString(
          GetPrefKeyName(titleContent.text + "_current_provider"),
          newlySelectedProvider.settingsPath);
    }
  ...
  private void SetupWindowPosition() {
    ...
    Vector2 initialSize = new Vector2(Mathf.Min(initialWidth, Screen.width),
                                      Mathf.Min(initialHeight, Screen.height));
    foreach (ContainerWindow window in containers) {
      if (window.showMode == ShowMode.MainWindow) {
        position =
            new Rect(window.position.center - (initialSize / 2), initialSize);
        break;
      }
    }
  }
  ...
  private void SetupUI() {
    ...
    root.AddStyleSheetPath(
        $"StyleSheets/SettingsWindow{(EditorGUIUtility.isProSkin ? " Dark
        " : " Light ")}.uss");
    ...
    m_TreeViewContainer = new IMGUIContainer(DrawTreeView){
        style = {flexGrow = m_SplitterFlex, flexBasis = 0f},
    ...
    m_SettingsPanel = new VisualElement(){
        style = {flexGrow = 1.0f - m_SplitterFlex, flexBasis = 0f}};
  ...
  private void DrawSettingsPanel() {
    ...
    using(var scrollViewScope = new EditorGUILayout.ScrollViewScope(
              m_PosRight, GUILayout.ExpandWidth(true))) {
  ...
  private void DrawControls() {
    using(new EditorGUI.LabelHighlightScope(
        m_SearchText,
        Styles.settingsPanel.GetColor(
            "-unity-search-highlight-selection-color"),
        Styles.settingsPanel.GetColor("-unity-search-highlight-color"))) {
      ...
      // If that's the case we'll set wideMode to true to avoid computing too
      // tall an inspector on the first layout calculation
  ...
  private void DrawTitleBar() {
    ...
    GUILayout.Space(
        Styles.settingsPanel.GetFloat(StyleCatalogKeyword.marginLeft));
    var headerContent = new GUIContent(m_TreeView.currentProvider.label,
                                       Styles.header.GetBool("-unity-show-icon")
                                           ? m_TreeView.currentProvider.icon
                                           : null);
    GUILayout.Label(headerContent, ImguiStyles.header,
                    GUILayout.MaxHeight(Styles.header.GetFloat("max-height")));
    GUILayout.FlexibleSpace();
    m_TreeView.currentProvider.OnTitleBarGUI();
    GUILayout.EndHorizontal();
  }

  private void DrawFooterBar() { m_TreeView.currentProvider.OnFooterBarGUI(); }

  private void DrawTreeView() {
    var splitterRect =
        m_Splitter.GetSplitterRect(m_Splitter.Children().First());
    ...
    using(var scrollViewScope = new GUILayout.ScrollViewScope(
              m_PosLeft, GUILayout.Width(splitterPos),
              GUILayout.MaxWidth(splitterPos),
              GUILayout.MinWidth(splitterPos))) {
      m_PosLeft = scrollViewScope.scrollPosition;
      m_TreeView.OnGUI(
          new Rect(0, Styles.window.GetFloat("margin-top"), treeWidth,
                   position.height - ImguiStyles.searchFieldHeight -
                       Styles.window.GetFloat("margin-top")));
    }
  }
  ...
  internal static SettingsWindow Show(SettingsScope scopes,
  ...
  internal static SettingsWindow FindWindowByScope(SettingsScope scopes) {
    var settingsWindows = Resources.FindObjectsOfTypeAll(typeof(SettingsWindow))
                              .Cast<SettingsWindow>();
    return settingsWindows.FirstOrDefault(settingsWindow =>
                                              settingsWindow.m_Scope == scopes);
  }
  ...
  internal class GUIScope : GUI.Scope {
    ...
    public GUIScope(float layoutMaxWidth) {
      ...
      GUILayout.Space(
          Styles.settingsPanel.GetFloat(StyleCatalogKeyword.marginTop));
    }
...
internal class ProjectSettingsWindow : SettingsWindow {
  public ProjectSettingsWindow() : base(SettingsScope.Project) {}
}

internal class PreferenceSettingsWindow : SettingsWindow {
  public PreferenceSettingsWindow() : base(SettingsScope.User) {}
}
}
