...
public static class SettingsService {
  ...
  internal static SettingsProvider[] FetchSettingsProviders() {
    return FetchSettingProviderFromAttribute()
  ...
  private static void CheckProjectSettings() {
    ...
    if (deprecatedMenuItems.Length > 0) {
      ...
      sb.Append(
          "There are menu items registered under Edit/Project Settings: ");
      sb.Append(string.Join(
          ", ", deprecatedMenuItems
                    .Select(item => item.Replace(k_ProjectSettings + "/", ""))
                    .ToArray()));
      sb.Append("\n");
      sb.AppendLine(
...
#pragma warning disable CS0618
    var methods = AttributeHelper.GetMethodsWithAttribute<PreferenceItem>(
        BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Static |
        BindingFlags.DeclaredOnly);
#pragma warning restore CS0618
    return methods.methodsWithAttributes.Select(method => {
      var callback =
          Delegate.CreateDelegate(typeof(Action), method.info) as Action;
      if (callback != null) {
#pragma warning disable CS0618
...
#pragma warning restore CS0618
        try {
          ...
          Debug.LogError("Cannot create preference wrapper for: " +
                         attributeName);
        }
      }
  ...
  private static IEnumerable<SettingsProvider>
  FetchSettingProviderFromAttribute() {
    var methods =
        AttributeHelper.GetMethodsWithAttribute<SettingsProviderAttribute>();
    return methods.methodsWithAttributes.Select(method => {
      try {
        var callback =
            Delegate.CreateDelegate(typeof(Func<SettingsProvider>), method.info)
                as Func<SettingsProvider>;
        return callback?.Invoke();
      } catch (Exception) {
        Debug.LogError("Cannot create Settings Provider for: " +
                       method.info.Name);
      }
      return null;
    });
  ...
  private static IEnumerable<SettingsProvider>
  FetchSettingProvidersFromAttribute() {
    var methods =
        AttributeHelper
            .GetMethodsWithAttribute<SettingsProviderGroupAttribute>();
    return methods.methodsWithAttributes.SelectMany(method => {
      try {
        var callback =
            Delegate.CreateDelegate(typeof(Func<SettingsProvider[]>),
                                    method.info) as Func<SettingsProvider[]>;
        var providers = callback?.Invoke();
        if (providers != null) {
          return providers;
        }
      } catch (Exception) {
        Debug.LogError("Cannot create Settings Providers for: " +
                       method.info.Name);
      }
