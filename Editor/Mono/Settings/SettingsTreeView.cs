...
internal class SettingsTreeView : TreeView {
  ...
  private static class Styles {
    public static StyleBlock tree =>
        EditorResources.GetStyle("sb-settings-tree");
  ...
  public SettingsProvider currentProvider {
    get;
    private set;
  }
  ...
  ProviderChangedHandler(SettingsProvider lastSelectedProvider,
  ...
  public List<SettingsProvider> GetChildrenAtPath(string path) {
    ...
    if (pathItem != null)
      children = pathItem.children.Select(item => FindProviderById(item.id))
                     .Where(p => p != null)
                     .ToList();
  ...
  protected override void SelectionChanged(IList<int> selectedIds) {
    SettingsProvider selectedProvider =
        GetFirstValidProvider(selectedIds.Count > 0 ? selectedIds.First() : -1);
  ...
  protected override void RowGUI(RowGUIArgs args) {
    ...
    if (Event.current.type == EventType.Repaint) {
      ...
      elementStyle.Draw(labelRect, args.item.displayName,
                        args.rowRect.Contains(Event.current.mousePosition),
                        HasFocus(), args.selected, false);
    }
  }
  ...
  protected override void SearchChanged(string newSearch) {
    ...
    if (rows.Count == 1 ||
        (rows.Count > 1 &&
         !GetSelection().Any(selectedId => rows.Any(r => r.id == selectedId))))
      SetSelection(new[]{rows[0].id},
                   TreeViewSelectionOptions.FireSelectionChanged);
  }

  protected override TreeViewItem BuildRoot() {
    SettingsNode rootNode =
        new SettingsNode(){children = new Dictionary<string, SettingsNode>()};
  ...
  private void BuildSettingsNodeTree(SettingsNode rootNode) {
    ...
    foreach (var provider in providers) {
      ...
      for (var tokenIndex = allChildrenUnderSameRoot ? 1 : 0;
           tokenIndex < provider.pathTokens.Length; ++tokenIndex) {
        var token = provider.pathTokens[tokenIndex];
        if (nodePath.Length > 0)
          nodePath += "/";
        nodePath += token;
        if (!current.children.ContainsKey(token)) {
          current.children[token] = new SettingsNode(){
              path = nodePath,
  ...
  private void AppendSettingsNode(SettingsNode node, string rootPath, int depth,
                                  ICollection<TreeViewItem> items) {
    ...
    foreach (var nodeName in sortedChildNames) {
      ...
      var childNodePath =
          rootPath.Length == 0 ? nodeName : rootPath + "/" + nodeName;
      var id = childNode.path.GetHashCode();
      var provider = FindProviderById(id);
      items.Add(new TreeViewItem{
        id = id,
        depth = depth,
        displayName = provider != null ? provider.label : nodeName,
        icon = provider?.icon
      });
