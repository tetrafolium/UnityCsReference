...
// Set icons rendered as part of [[GUIContent]] to be rendered at a specific
// size.
public static extern void SetIconSize(Vector2 size);

// Get a white texture.
public static extern Texture2D whiteTexture {
	[NativeMethod("GetWhiteTexture")] get;
}
...
// Iterates over cameras and counts the ones that would render to a specified
// screen (doesn't involve culling)
...
[Obsolete(
	 ...
	 true)]
public static void RenderGameViewCameras(RenderTexture target,
                                         int targetDisplay, Rect screenRect,
                                         Vector2 mousePosition, bool gizmos) {
}
...
SetCurrentViewCursor(Texture2D texture, Vector2 hotspot, MouseCursor type);
...
// Duplicate of SetDefaultFont in UnityEngine. We need to call it from editor
// code as well, while keeping both internal.
internal static extern void SetDefaultFont(Font font);

// Remember to call CopyMonoScriptIconToImporters when data should be copied
// to mono importer and mono assembly importer.
internal static extern Texture2D GetIconForObject(Object obj);

// Render all ingame cameras bound to a specific Display.
internal static extern void
RenderPlayModeViewCamerasInternal(RenderTexture target, int targetDisplay,
                                  Vector2 mousePosition, bool gizmos,
                                  bool renderIMGUI);
...
private static extern void Internal_AddCursorRect(Rect r, MouseCursor m,
                                                  int controlID);
}

[NativeHeader("Editor/Src/InspectorExpandedState.h"),
 StaticAccessor("GetInspectorExpandedState().GetSessionState()",
