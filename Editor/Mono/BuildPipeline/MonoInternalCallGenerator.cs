...
internal class MonoAOTRegistration {
  static void ExtractNativeMethodsFromTypes(ICollection<TypeDefinition> types,
                                            ArrayList res) {
    foreach (TypeDefinition typ in types) {
      foreach (MethodDefinition method in typ.Methods) {
        if (method.IsStatic && method.IsPInvokeImpl &&
            method.PInvokeInfo.Module.Name.Equals("__Internal")) {
          if (res.Contains(method.Name))
            throw new SystemException(
                "Duplicate native method found : " + method.Name +
                ". Please check your source carefully.");
  ...
  // Builds list of extern methods that are marked with DllImport("__Internal")
  // attribute
  ...
  static public HashSet<string>
  BuildReferencedTypeList(AssemblyDefinition[] assemblies) {
    HashSet<string> res = new HashSet<string>();

    foreach (AssemblyDefinition ass in assemblies) {
      // string assname = ass.Name.Name;
      if (!ass.Name.Name.StartsWith("System") &&
          !ass.Name.Name.Equals("UnityEngine")) {
  ...
  static public void WriteCPlusPlusFileForStaticAOTModuleRegistration(
      BuildTarget buildTarget, string file,
      CrossCompileOptions crossCompileOptions, bool advancedLic,
      string targetDevice, bool stripping,
      RuntimeClassRegistry usedClassRegistry, AssemblyReferenceChecker checker,
      string stagingAreaDataManaged) {
    WriteCPlusPlusFileForStaticAOTModuleRegistration(
        buildTarget, file, crossCompileOptions, advancedLic, targetDevice,
        stripping, usedClassRegistry, checker, stagingAreaDataManaged, null);
  }
  ...
  static public void WriteCPlusPlusFileForStaticAOTModuleRegistration(
      ...
      string targetDevice, bool stripping,
      RuntimeClassRegistry usedClassRegistry, AssemblyReferenceChecker checker,
      string stagingAreaDataManaged, IIl2CppPlatformProvider platformProvider) {
    // generate the Interal Call Summary file
    var icallSummaryPath =
        Path.Combine(stagingAreaDataManaged, "ICallSummary.txt");
    var dlls =
        Directory.GetFiles(stagingAreaDataManaged, "UnityEngine.*Module.dll")
            .Concat(
                new[]{Path.Combine(stagingAreaDataManaged, "UnityEngine.dll")});
    var exe = Path.Combine(
        MonoInstallationFinder.GetFrameWorksFolder(),
        "Tools/InternalCallRegistrationWriter/InternalCallRegistrationWriter.exe");
    var args =
        string.Format("-assembly=\"{0}\" -summary=\"{1}\"",
    ...
    CodeStrippingUtils.GenerateDependencies(
        stagingAreaDataManaged, icallSummaryPath, usedClassRegistry, stripping,
        out nativeClasses, out nativeModules, platformProvider);
    ...
    using(TextWriter w = new StreamWriter(file)) {
      ...
      bool fastICall =
          (crossCompileOptions & CrossCompileOptions.FastICall) != 0;
      ...
      if (buildTarget == BuildTarget.iOS) {
        w.WriteLine(
            "    const char*         UnityIPhoneRuntimeVersion = \"{0}\";",
      ...
      w.WriteLine(
          "#define DLL_EXPORT __declspec(dllexport)"); // ps4 needs dllexport.
      w.WriteLine("#else");
      ...
      for (int q = 0; q < fileNames.Length; ++q) {
        ...
        w.WriteLine("    extern gpointer*    mono_aot_module_{0}_info; // {1}",
      ...
      foreach (AssemblyDefinition definition in assemblies) {
        ...
        assemblyName = assemblyName.Replace("-", "_");
        assemblyName = assemblyName.Replace(" ", "_");
        w.WriteLine("    mono_aot_register_module(mono_aot_module_{0}_info);",
                    assemblyName);
      }
  ...
  public static void GenerateRegisterModules(HashSet<UnityType> nativeClasses,
                                             HashSet<string> nativeModules,
                                             TextWriter output,
  ...
  public static void GenerateRegisterClassesForStripping(
      HashSet<UnityType> nativeClassesAndBaseClasses, TextWriter output) {
    ...
    foreach (var type in UnityType.GetTypes()) {
      ...
      if (!type.hasNativeNamespace) {
        ...
        output.WriteLine("namespace {0} {{ class {1}; }}", type.nativeNamespace,
                         type.name);
      }
    ...
    foreach (var type in UnityType.GetTypes()) {
      if (type.baseClass == null || type.isEditorOnly ||
          !nativeClassesAndBaseClasses.Contains(type))
        continue;
    ...
    // UNITY_EDITOR in them module definition file (for example WorldAnchor only
    // being there if ENABLE_HOLOLENS_MODULE_API). Doing this would require
    // either some non trivial changes to the module registration macros or a
    // way for these conditionals to be included in the RTTI so we can emit them
    // when generating the code, so we disabling the registration of stripped
    // classes for now and will get back to this when we have landed the
    // remaining changes to the type system.
  ...
  public static void GenerateRegisterClasses(HashSet<UnityType> allClasses,
                                             TextWriter output) {
  ...
  //
  public static void
  GenerateRegisterInternalCalls(AssemblyDefinition[] assemblies,
                                TextWriter output) {
    output.Write("void RegisterAllStrippedInternalCalls ()\n{\n");

    foreach (AssemblyDefinition definition in assemblies)
      GenerateRegisterInternalCallsForTypes(definition.MainModule.Types,
                                            output);
  ...
  private static void
  GenerateRegisterInternalCallsForTypes(IEnumerable<TypeDefinition> types,
                                        TextWriter output) {
  ...
  static void GenerateInternalCallMethod(TypeDefinition typeDefinition,
  ...
  AssemblyFactory.GetAssembly("/Users/joe/Sources/unity-trunk/build/iPhonePlayer/Managed/UnityEngine.dll")};
