...
internal class MonoCrossCompile {
  ...
  private class JobCompileAOT {
    public JobCompileAOT(BuildTarget target, string crossCompilerAbsolutePath,
                         string assembliesAbsoluteDirectory,
                         CrossCompileOptions crossCompileOptions, string input,
                         string output, string additionalOptions) {
      m_target = target;
    ...
    public void ThreadPoolCallback(System.Object threadContext) {
      try {
        MonoCrossCompile.CrossCompileAOT(m_target, m_crossCompilerAbsolutePath,
                                         m_assembliesAbsoluteDirectory,
                                         m_crossCompileOptions, m_input,
                                         m_output, m_additionalOptions);
      } catch (Exception ex) {
        m_Exception = ex;
      }
      m_doneEvent.Set();
  ...
  static public void CrossCompileAOTDirectory(
      BuildTarget buildTarget, CrossCompileOptions crossCompileOptions,
      string sourceAssembliesFolder, string targetCrossCompiledASMFolder,
      string additionalOptions) {
    CrossCompileAOTDirectory(
        buildTarget, crossCompileOptions, sourceAssembliesFolder,
  ...
  static public void CrossCompileAOTDirectory(
    ...
    string crossCompilerPath =
        BuildPipeline.GetBuildToolsDirectory(buildTarget);
    if (Application.platform == RuntimePlatform.OSXEditor)
      crossCompilerPath = Path.Combine(
          Path.Combine(crossCompilerPath, pathExtension), "mono-xcompiler");
    else
      crossCompilerPath = Path.Combine(
          Path.Combine(crossCompilerPath, pathExtension), "mono-xcompiler.exe");

    sourceAssembliesFolder =
        Path.Combine(Directory.GetCurrentDirectory(), sourceAssembliesFolder);
    targetCrossCompiledASMFolder = Path.Combine(Directory.GetCurrentDirectory(),
                                                targetCrossCompiledASMFolder);
    ...
    foreach (string fileName in Directory.GetFiles(sourceAssembliesFolder)) {
      ...
      string outputPath =
          Path.Combine(targetCrossCompiledASMFolder, inputPath + ".s");

      if (EditorUtility.DisplayCancelableProgressBar(
              "Building Player", "AOT cross compile " + inputPath, 0.95F))
  ...
  static public bool CrossCompileAOTDirectoryParallel(
      BuildTarget buildTarget, CrossCompileOptions crossCompileOptions,
      string sourceAssembliesFolder, string targetCrossCompiledASMFolder,
      string additionalOptions) {
  ...
  static public bool CrossCompileAOTDirectoryParallel(
    ...
    return CrossCompileAOTDirectoryParallel(
        ...
        sourceAssembliesFolder, targetCrossCompiledASMFolder,
        additionalOptions);
  }

  static private bool WaitForBuildOfFile(List<ManualResetEvent> events,
                                         ref long timeout) {
  ...
  static public bool CrossCompileAOTDirectoryParallel(
    ...
    var dllList =
        new List<string>(Directory.GetFiles(sourceAssembliesFolder)
    ...
    long timeout = System.Math.Min(
        30 * 1000 * 60,
        (numFiles + 3) * 1000 *
            30); // 30 minute limit in case of FOOBARs, 30 seconds for each file
    ...
    foreach (string fileName in dllList) {
      ...
      JobCompileAOT job = new JobCompileAOT(
          buildTarget, crossCompilerPath, sourceAssembliesFolder,
  ...
  static void CrossCompileAOT(BuildTarget target,
                              string crossCompilerAbsolutePath,
                              string assembliesAbsoluteDirectory,
    ...
    // We don't want debugging for non-script assemblies (anyway source code is
    // not available for the end users)
    ...
    bool debugging =
        ((crossCompileOptions & CrossCompileOptions.Debugging) != 0);
    bool loadSymbols =
        ((crossCompileOptions & CrossCompileOptions.LoadSymbols) != 0);
    ...
    // So we temporarily write into the assembliesAbsoluteDirectory and move it
    // away afterwards
    string outputTempPath =
        Path.Combine(assembliesAbsoluteDirectory, outputFileName);
    ...
    process.StartInfo.EnvironmentVariables["GAC_PATH"] =
        assembliesAbsoluteDirectory;
    ...
    if ((crossCompileOptions & CrossCompileOptions.ExplicitNullChecks) != 0)
      process.StartInfo.EnvironmentVariables["MONO_DEBUG"] =
    ...
    if (ArtifactsPath != null) {
      ...
      File.AppendAllText(ArtifactsPath + "output.txt",
      ...
      File.AppendAllText(ArtifactsPath + "houtput.txt",
                         outputFileName + "\n\n");
      File.Copy(assembliesAbsoluteDirectory + "\\" + input,
                ArtifactsPath + "\\" + input, true);
    }

    process.StartInfo.WorkingDirectory = assembliesAbsoluteDirectory;
    MonoProcessUtility.RunMonoProcess(process, "AOT cross compiler",
                                      outputTempPath);
    // For some reason we can't pass a full path to outfile, so we move the .s
    // file after compilation instead
    File.Move(outputTempPath, output);

    // handle .def files if present for dlls
    if ((crossCompileOptions & CrossCompileOptions.Static) == 0) {
      string defOutputTempPath =
          Path.Combine(assembliesAbsoluteDirectory, outputFileName + ".def");
