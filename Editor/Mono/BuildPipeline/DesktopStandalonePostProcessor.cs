...
internal abstract class DesktopStandalonePostProcessor
  ...
  public override string PrepareForBuild(BuildOptions options,
                                         BuildTarget target) {
  ...
  public override void PostProcess(BuildPostProcessArgs args) {
    try {
      ...
      if (EditorUtility.DisplayCancelableProgressBar(
              "Building Player", "Copying files to final destination", 0.1f))
  ...
  public override bool SupportsLz4Compression() { return true; }

  public override void UpdateBootConfig(BuildTarget target,
                                        BootConfigData config,
    ...
    if (IL2CPPUtils.UseIl2CppCodegenWithMonoBackend(
  ...
  private void CopyNativePlugins(BuildPostProcessArgs args,
                                 ...
                                 out List<string> cppPlugins) {
    string buildTargetName = BuildPipeline.GetBuildTargetName(buildTarget);
    IPluginImporterExtension pluginImpExtension =
    ...
    foreach (PluginImporter imp in PluginImporter.GetImporters(buildTarget)) {
      // Skip .cpp files. They get copied to il2cpp output folder just before
      // code compilation
      ...
      if (string.IsNullOrEmpty(imp.assetPath)) {
        UnityEngine.Debug.LogWarning("Got empty plugin importer path for " +
                                     buildTarget);
        continue;
      }
      ...
      string destinationPath =
          pluginImpExtension.CalculateFinalPluginPath(buildTargetName, imp);
      if (string.IsNullOrEmpty(destinationPath))
        continue;

      string finalDestinationPath =
          Path.Combine(pluginsFolder, destinationPath);
      ...
      if (isDirectory) {
        // Since we may be copying from Assets make sure to not include .meta
        // files to the build
    ...
    // TODO: Move all plugins using GetExtensionPlugins to GetImporters and
    // remove GetExtensionPlugins
    foreach (UnityEditorInternal.PluginDesc pluginDesc in PluginImporter
                 .GetExtensionPlugins(buildTarget)) {
      if (!haveCreatedPluginsFolder) {
        Directory.CreateDirectory(pluginsFolder);
        haveCreatedPluginsFolder = true;
      }

      string pluginCopyPath =
          Path.Combine(pluginsFolder, Path.GetFileName(pluginDesc.pluginPath));
  ...
  private void CopyCppPlugins(BuildPostProcessArgs args, string cppOutputDir,
                              IEnumerable<string> cppPlugins) {
    if (GetCreateSolution(args))
      return;

    foreach (var plugin in cppPlugins) {
      FileUtil.CopyFileOrDirectory(
          plugin, Path.Combine(cppOutputDir, Path.GetFileName(plugin)));
    }
  }
  ...
  private void SetupStagingArea(BuildPostProcessArgs args,
    ...
    if (GetCreateSolution(args) &&
      ...
      // For Windows Standalone player solution build, we want to copy plugins
      // for all architectures as the ultimate CPU architecture choice can be
    ...
    PostprocessBuildPlayer.InstallStreamingAssets(args.stagingAreaData,
                                                  args.report);
    ...
    if (GetInstallingIntoBuildsFolder(args)) {
      ...
      if (UseIl2Cpp) {
        ...
        IL2CPPUtils.RunIl2Cpp(
            args.stagingAreaData, il2cppPlatformProvider,
            (cppOutputDir) => CopyCppPlugins(args, cppOutputDir, cppPlugins),
            args.usedClassRegistry);

        if (GetCreateSolution(args)) {
          ProcessIl2CppOutputForSolution(args, il2cppPlatformProvider,
                                         cppPlugins);
        } else {
        ...
        AssemblyStripper.StripForMonoBackend(
            args.target, args.usedClassRegistry, managedStrippingLevel,
            args.report);
      }
  ...
  private void ProcessIl2CppOutputForBinary(BuildPostProcessArgs args) {
    ...
    var il2cppOutputNativeDirectory =
        Path.Combine(args.stagingAreaData, "Native");
    ...
    foreach (var file in Directory.GetFiles(il2cppOutputNativeDirectory)) {
      ...
      if (fileName.StartsWith("."))
        continue; // Skip files starting with ., as they weren't output by our
                  // tools and potentially belong to the OS (like .DS_Store on
                  // macOS)

      FileUtil.MoveFileOrDirectory(
          file, Path.Combine(gameAssemblyDirectory, fileName));
    }

    // Move il2cpp data directory one directory up
    FileUtil.MoveFileOrDirectory(
        Path.Combine(il2cppOutputNativeDirectory, "Data"),
        Path.Combine(args.stagingAreaData, "il2cpp_data"));

    // Native directory is supposed to be empty at this point
    FileUtil.DeleteFileOrDirectory(il2cppOutputNativeDirectory);

    var dataBackupFolder =
        Path.Combine(args.stagingArea, GetIl2CppDataBackupFolderName(args));
    FileUtil.CreateOrCleanDirectory(dataBackupFolder);

    var il2cppOutputFolder = Path.Combine(args.stagingAreaData, "il2cppOutput");

    // Delete duplicate il2cpp_data that was created in il2cppOutput directory
    // (case 1198179)
    FileUtil.DeleteFileOrDirectory(Path.Combine(il2cppOutputFolder, "Data"));

    // Move generated C++ code out of Data directory
    FileUtil.MoveFileOrDirectory(
        il2cppOutputFolder, Path.Combine(dataBackupFolder, "il2cppOutput"));

    if (IL2CPPUtils.UseIl2CppCodegenWithMonoBackend(
            BuildPipeline.GetBuildTargetGroup(args.target))) {
      // Are we using IL2CPP code generation with the Mono runtime? If so, strip
      // the assemblies so we can use them for metadata.
      StripAssembliesToLeaveOnlyMetadata(args.target,
                                         args.stagingAreaDataManaged);
    } else {
      // Otherwise, move them to temp data directory as il2cpp does not need
      // managed assemblies to run
      FileUtil.MoveFileOrDirectory(args.stagingAreaDataManaged,
                                   Path.Combine(dataBackupFolder, "Managed"));
    }
  ...
  private void
  ProcessIl2CppOutputForSolution(BuildPostProcessArgs args,
                                 IIl2CppPlatformProvider il2cppPlatformProvider,
                                 IEnumerable<string> cppPlugins) {
    if (EditorUtility.DisplayCancelableProgressBar(
            "Building Player", "Copying IL2CPP related files", 0.1f))
      throw new OperationCanceledException();
    ...
    FileUtil.MoveFileOrDirectory(
        args.stagingAreaDataManaged,
        Paths.Combine(args.stagingArea, projectName, "Managed"));
    ...
    var il2cppOutputProjectDirectory =
        Path.Combine(args.stagingArea, "Il2CppOutputProject");
    var sourceFolder = Path.Combine(il2cppOutputProjectDirectory, "Source");
    Directory.CreateDirectory(sourceFolder);
    FileUtil.MoveFileOrDirectory(il2cppOutputPath,
                                 Path.Combine(sourceFolder, "il2cppOutput"));
    ...
    if (cppPlugins.Any()) {
      ...
      foreach (var cppPlugin in cppPlugins)
        FileUtil.CopyFileOrDirectory(
            cppPlugin,
            Path.Combine(cppPluginsDirectory, Path.GetFileName(cppPlugin)));
    }

    // Copy IL2CPP
    var il2cppSourceFolder = IL2CPPUtils.GetIl2CppFolder();
    var il2cppTargetFolder =
        Paths.Combine(il2cppOutputProjectDirectory, "IL2CPP");
    Directory.CreateDirectory(il2cppTargetFolder);

    FileUtil.CopyFileOrDirectory(Path.Combine(il2cppSourceFolder, "build"),
                                 Path.Combine(il2cppTargetFolder, "build"));
    FileUtil.CopyFileOrDirectory(Path.Combine(il2cppSourceFolder, "external"),
                                 Path.Combine(il2cppTargetFolder, "external"));
    FileUtil.CopyFileOrDirectory(Path.Combine(il2cppSourceFolder, "libil2cpp"),
                                 Path.Combine(il2cppTargetFolder, "libil2cpp"));

    if (IL2CPPUtils.EnableIL2CPPDebugger(il2cppPlatformProvider,
                                         BuildTargetGroup.Standalone)) {
      FileUtil.CopyFileOrDirectory(Path.Combine(il2cppSourceFolder, "libmono"),
                                   Path.Combine(il2cppTargetFolder, "libmono"));
    }

    FileUtil.CopyFileOrDirectory(
        Path.GetDirectoryName(IL2CPPBuilder.GetMapFileParserPath()),
        Path.Combine(il2cppTargetFolder, "MapFileParser"));
    FileUtil.CopyFileOrDirectory(
        Path.Combine(il2cppSourceFolder, "il2cpp_root"),
        Path.Combine(il2cppTargetFolder, "il2cpp_root"));

    WriteIl2CppOutputProject(args, il2cppOutputProjectDirectory,
                             il2cppPlatformProvider);
  }

  protected virtual void
  WriteIl2CppOutputProject(BuildPostProcessArgs args,
                           string il2cppOutputProjectDirectory,
                           IIl2CppPlatformProvider il2cppPlatformProvider) {
    throw new NotSupportedException(
        "CreateSolution is not supported on " +
        BuildPipeline.GetBuildTargetName(args.target));
  }

  private static void
  StripAssembliesToLeaveOnlyMetadata(BuildTarget target,
                                     string stagingAreaDataManaged) {
    var checker = new AssemblyReferenceChecker();
    checker.CollectReferences(stagingAreaDataManaged, true, 0.0f, false);

    EditorUtility.DisplayProgressBar(
        "Removing bytecode from assemblies",
        "Stripping assemblies so that only metadata remains", 0.95F);
    MonoAssemblyStripping.MonoCilStrip(target, stagingAreaDataManaged,
                                       checker.GetAssemblyFileNames());
  }

  // Creates app.info which is used by Standalone player (when run in Low
  // Integrity mode) when creating log file path at program start. Log file path
  // is created very early in the program execution, when none of the Unity
  // managers are even created, that's why we can't get it from PlayerSettings
  // Note: In low integrity mode, we can only write to %USER
  // PROFILE%\AppData\LocalLow on Windows
  private static void CreateApplicationData(BuildPostProcessArgs args) {
    File.WriteAllText(
        Path.Combine(args.stagingAreaData, "app.info"),
        string.Join("\n", new[]{args.companyName, args.productName}));
    args.report.RecordFileAdded(Path.Combine(args.stagingAreaData, "app.info"),
                                CommonRoles.appInfo);
  }
  ...
  protected bool CopyPlayerFilter(string path, BuildPostProcessArgs args) {
    ...
    for (int i = 0; i < args.report.files.Length; ++i)
      if (Path.GetFileName(args.report.files[i].path) == fileName &&
          args.report.files[i].isOverridingUnityAssembly)
        return false;

    // Don't copy UnityEngine mdb/pdb files
    return (Path.GetExtension(path) != ".mdb" &&
            Path.GetExtension(path) != ".pdb") ||
           !Path.GetFileName(path).StartsWith("UnityEngine.");
  }

  private static uint StringToFourCC(string literal) {
    if (literal.Length > 4)
      throw new NotSupportedException(
          "FourCC can consist of maximum 4 characters");
  ...
  CopyVariationFolderIntoStagingArea(BuildPostProcessArgs args,
                                     HashSet<string> filesToNotOverwrite);

  protected static void RecordCommonFiles(BuildPostProcessArgs args,
                                          string variationSourceFolder,
                                          string monoFolderRoot) {
    // Mark the default resources file
    args.report.RecordFileAdded(
        Path.Combine(args.stagingArea,
                     "Data/Resources/unity default resources"),
        CommonRoles.builtInResources);

    // Mark up each mono runtime
    args.report.RecordFilesAddedRecursive(
        Path.Combine(monoFolderRoot, k_MonoDirectoryName + "/EmbedRuntime"),
        CommonRoles.monoRuntime);
    args.report.RecordFilesAddedRecursive(
        Path.Combine(monoFolderRoot, k_MonoDirectoryName + "/etc"),
  ...
  private void CopyStagingAreaIntoBuildsFolder(BuildPostProcessArgs args) {
    ...
    if (!Directory.Exists(Path.GetDirectoryName(dst))) {
      throw new Exception(
          "Installing in builds folder failed because the player has not been built (You most likely want to enable 'Development build').");
    }

    FileUtil.CopyDirectoryRecursive(args.stagingAreaData, dst, overwrite
                                    : true);
  }

  private void
  CopyStagingAreaIntoDestination(BuildPostProcessArgs args,
                                 HashSet<string> filesToNotOverwrite) {
    DeleteDestination(args);

    // Copy entire stagingarea over
    CopyFilesToDestination(args.stagingArea, GetDestinationFolder(args),
                           filesToNotOverwrite);
    args.report.RecordFilesMoved(args.stagingArea, GetDestinationFolder(args));
  }

  private static void
  CopyFilesToDestination(string source, string target,
                         HashSet<string> filesToNotOverwrite) {
    ...
    foreach (var directory in Directory.GetDirectories(source))
      CopyFilesToDestination(directory,
                             Path.Combine(target, Path.GetFileName(directory)),
                             filesToNotOverwrite);
  }

  protected abstract
      string GetStagingAreaPluginsFolder(BuildPostProcessArgs args);

  protected abstract void DeleteDestination(BuildPostProcessArgs args);

  protected static bool UseMono =>
      !UseIl2Cpp ||
      IL2CPPUtils.UseIl2CppCodegenWithMonoBackend(BuildTargetGroup.Standalone);

  protected static void DeleteUnusedMono(string dataFolder,
                                         BuildReport report) {
  ...
  GetDestinationFolderForInstallingIntoBuildsFolder(BuildPostProcessArgs args);

  protected abstract void CopyDataForBuildsFolder(BuildPostProcessArgs args);

  protected static bool
  GetInstallingIntoBuildsFolder(BuildPostProcessArgs args) {
  ...
  protected virtual string GetVariationName(BuildPostProcessArgs args) {
    ...
    if (UseIl2Cpp && !IL2CPPUtils.UseIl2CppCodegenWithMonoBackend(
                         BuildTargetGroup.Standalone))
  ...
  protected abstract
      IIl2CppPlatformProvider GetPlatformProvider(BuildPostProcessArgs args);

  protected virtual void
  ProcessPlatformSpecificIL2CPPOutput(BuildPostProcessArgs args) {}

  protected virtual void ProcessSymbolFiles(BuildPostProcessArgs args) {}

  protected static string
  GetIl2CppDataBackupFolderName(BuildPostProcessArgs args) {
    return Path.GetFileNameWithoutExtension(args.installPath) +
           "_BackUpThisFolder_ButDontShipItWithYourGame";
  }

  protected virtual string
  GetDirectoryForGameAssembly(BuildPostProcessArgs args) {
