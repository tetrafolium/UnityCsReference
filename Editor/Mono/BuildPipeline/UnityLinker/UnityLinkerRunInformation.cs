...
class UnityLinkerRunInformation {
  ...
  public UnityLinkerRunInformation(
    ...
    pipelineData =
        new UnityLinkerBuildPipelineData(target, managedAssemblyFolderPath);

    buildTargetGroup = BuildPipeline.GetBuildTargetGroup(buildTarget);
    argumentProvider = new UnityLinkerArgumentValueProvider(this);
    isMonoBackend = PlayerSettings.GetScriptingBackend(buildTargetGroup) ==
                    ScriptingImplementation.Mono2x;
    engineStrippingSupported =
        platformProvider.supportsEngineStripping && !isMonoBackend;
    performEngineStripping = rcr != null && PlayerSettings.stripEngineCode &&
                             engineStrippingSupported;
  }

  public string ModulesAssetFilePath => Path.Combine(
      platformProvider.moduleStrippingInformationFolder, "../modules.asset");

  public IEnumerable<string> GetModuleBlacklistFiles() {
    return Directory.GetFiles(platformProvider.moduleStrippingInformationFolder,
                              "*.xml");
  }

  public BuildReport BuildReport =>
      il2CppPlatformProvider == null ? null : il2CppPlatformProvider.buildReport;

  public StrippingInfo BuildReportData =>
      BuildReport == null ? null : StrippingInfo.GetBuildReportData(BuildReport);

  public List<string> GetUserAssemblies() {
    return rcr.GetUserAssemblies()
        .Where(s => rcr.IsDLLUsed(s))
        .Select(s => Path.Combine(managedAssemblyFolderPath, s))
        .ToList();
  }

  public List<string> AssembliesToProcess() {
    var userAssemblies = GetUserAssemblies();
    userAssemblies.AddRange(Directory.GetFiles(
        managedAssemblyFolderPath, "I18N*.dll", SearchOption.TopDirectoryOnly));
    return userAssemblies;
  }

  public string EditorToLinkerDataPath =>
      Path.Combine(managedAssemblyFolderPath, "EditorToUnityLinkerData.json");
