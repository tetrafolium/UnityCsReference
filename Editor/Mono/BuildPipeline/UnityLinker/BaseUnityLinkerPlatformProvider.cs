...
public virtual bool AllowOutputToBeMadeArchitectureDependent {
	get {
		// For now we are not leveraging this capability but I don't want to
		// remove the plumbing to use it in case we ever want to take advantage of
		// it
		return false;
	}
}
...
public virtual bool supportsEngineStripping {
	get {
		return BuildPipeline.IsFeatureSupported("ENABLE_ENGINE_CODE_STRIPPING",
		                                        m_Target);
	}
}
...
public virtual string moduleStrippingInformationFolder {
	get {
		return Path.Combine(BuildPipeline.GetPlaybackEngineDirectory(
