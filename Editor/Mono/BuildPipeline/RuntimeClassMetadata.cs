...
/// <summary>
///  Registry for Unity native-managed class dependencies. Aimed to make native
///  and managed code stripping possible. Note: only UnityEngine.dll content is
///  covered there.
/// </summary>
internal class RuntimeClassRegistry {
  ...
  protected Dictionary<string, string[]> m_UsedTypesPerUserAssembly =
      new Dictionary<string, string[]>();
  protected Dictionary<int, List<string>> classScenes =
  ...
  // This is a step in the refactor of native code stripping.
  // RuntimeClassRegistry should only be carrying this information to post
  // process scripts rather than doing so much.
  protected Dictionary<int, string> allNativeClasses =
  ...
  public void AddNativeClassID(int ID) {
    ...
    ////System.Console.WriteLine("Looking for ID {0} name {1} --> is manager?
    ///{2}", ID, className, functionalityGroups.ContainsValue(className));
  ...
  public void SetUsedTypesInUserAssembly(string[] typeNames,
                                         string assemblyName) {
    ...
    if (Array.IndexOf(CodeStrippingUtils.UserAssemblies, dll) != -1) {
      ...
      var asmdefPath =
          CompilationPipeline.GetAssemblyDefinitionFilePathFromAssemblyName(
              dll);
      if (asmdefPath == null ||
          !EditorCompilationInterface.Instance.IsPathInPackageDirectory(
              asmdefPath))
        return true;
    }
  ...
  public void AddNativeClassFromName(string className) {
    ...
    ////System.Console.WriteLine("Looking for name {1}  ID {0}", classID,
    ///className);
  ...
  public Dictionary<string, string[]> GetAllManagedTypesInScenes() {
    ...
    // Duplicates of UnityEngine.Object will happen because native types without
    // a managed type will come back as UnityEngine.Object
  ...
  internal List<MethodDescription> m_MethodsToPreserve =
      new List<MethodDescription>();

  // invoked by native code
  internal void AddMethodToPreserve(string assembly, string @namespace,
                                    string klassName, string methodName) {
    m_MethodsToPreserve.Add(new MethodDescription(){
        assembly = assembly,
        fullTypeName =
            @namespace + (@namespace.Length > 0 ? "." : "") + klassName,
  ...
  internal string[] GetUserAssemblies() { return m_UserAssemblies.ToArray(); }

  protected void InitRuntimeClassRegistry() {
    BuildTargetGroup group =
        UnityEditor.BuildPipeline.GetBuildTargetGroup(buildTarget);
    ...
    string[] runtimeLoadDontStripClassNames =
        RuntimeInitializeOnLoadManager.dontStripClassNames;
