...
public abstract void CompileDynamicLibrary(string outFile,
...
protected void ExecuteCommand(string command, params string[] arguments) {
	var startInfo = new ProcessStartInfo(
		command, arguments.Aggregate((buff, s) => buff + " " + s));
...
private void RunProgram(ProcessStartInfo startInfo) {
	using (var program = new UnityEditor.Utils.Program(startInfo)) {
		...
		if (standardOutput.Length > 0)
			output =
				standardOutput.Aggregate((buf, s) => buf + Environment.NewLine + s);

		var errorOutput = program.GetErrorOutput();
		if (errorOutput.Length > 0)
			output +=
				errorOutput.Aggregate((buf, s) => buf + Environment.NewLine + s);

		if (program.ExitCode != 0) {
			UnityEngine.Debug.LogError("Failed running " + startInfo.FileName +
			                           " " + startInfo.Arguments + "\n\n" + output);
...
protected static string Aggregate(IEnumerable<string> items, string prefix,
                                  string suffix) {
	return items.Aggregate("",
	                       (current, additionalFile) =>
	                       current + (prefix + additionalFile + suffix));
}
...
protected internal static IEnumerable<string>
AllSourceFilesIn(string directory) {
	return Directory.GetFiles(directory, "*.cpp", SearchOption.AllDirectories)
	       .Concat(
		Directory.GetFiles(directory, "*.c", SearchOption.AllDirectories));
}
