...
internal static class FileMirroring {
  ...
  public static void MirrorFolder(string from, string to,
    ...
    var toFileEntries =
        Directory.GetFileSystemEntries(to).Select(s => StripPrefix(s, to));
    var fromFileEntries =
        Directory.GetFileSystemEntries(from).Select(s => StripPrefix(s, from));
  ...
  enum FileEntryType { File, Directory, NotExisting }
  ...
  static bool AreFilesIdentical(string filePath1, string filePath2) {
    using(var file = File.OpenRead(filePath1)) using(
