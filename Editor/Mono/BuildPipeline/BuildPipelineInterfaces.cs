...
void OnProcessShader(Shader shader, ShaderSnippetData snippet,
                     IList<ShaderCompilerData> data);
}

public interface IUnityLinkerProcessor : IOrderedCallback {
string GenerateAdditionalLinkXmlFile(
	BuildReport report, UnityLinker.UnityLinkerBuildPipelineData data);

void OnBeforeRun(BuildReport report,
                 UnityLinker.UnityLinkerBuildPipelineData data);

void OnAfterRun(BuildReport report,
                UnityLinker.UnityLinkerBuildPipelineData data);
}

public interface IIl2CppProcessor : IOrderedCallback {
void OnBeforeConvertRun(BuildReport report,
                        Il2Cpp.Il2CppBuildPipelineData data);
}
...
internal static int CompareICallbackOrder(IOrderedCallback a,
                                          IOrderedCallback b) {
...
static void AddToListIfTypeImplementsInterface<T>(Type t, ref object o,
                                                  ref List<T> list) where T
: class {
	if (!ValidateType<T>(t))
		return;
...
public AttributeCallbackWrapper(MethodInfo m) {
	m_callbackOrder =
		((CallbackOrderAttribute) Attribute.GetCustomAttribute(
			 m, typeof(CallbackOrderAttribute)))
		.callbackOrder;
	m_method = m;
}
...
public void OnPostprocessBuild(BuildReport report) {
	m_method.Invoke(null, new object[] {report.summary.platform,
...
// this variable is reinitialized on domain reload so any calls to Init
// after a domain reload will set things up correctly
...
internal static void InitializeBuildCallbacks(BuildCallbacks findFlags) {
	...
	bool findBuildProcessors = (findFlags & BuildCallbacks.BuildProcessors) ==
	                           BuildCallbacks.BuildProcessors;
	bool findSceneProcessors = (findFlags & BuildCallbacks.SceneProcessors) ==
	                           BuildCallbacks.SceneProcessors;
	bool findTargetProcessors =
		(findFlags & BuildCallbacks.BuildTargetProcessors) ==
		BuildCallbacks.BuildTargetProcessors;
	bool findFilterProcessors =
		(findFlags & BuildCallbacks.FilterAssembliesProcessors) ==
		BuildCallbacks.FilterAssembliesProcessors;
	bool findShaderProcessors =
		(findFlags & BuildCallbacks.ShaderProcessors) ==
		BuildCallbacks.ShaderProcessors;
	bool findBuildPlayerScriptDLLsProcessors =
		(findFlags & BuildCallbacks.BuildPlayerScriptDLLProcessors) ==
		BuildCallbacks.BuildPlayerScriptDLLProcessors;
	bool findUnityLinkerProcessors =
		(findFlags & BuildCallbacks.UnityLinkerProcessors) ==
		BuildCallbacks.UnityLinkerProcessors;
	bool findIl2CppProcessors =
		(findFlags & BuildCallbacks.I2CppProcessors) ==
		BuildCallbacks.I2CppProcessors;

	var postProcessBuildAttributeParams =
		new Type[] {typeof(BuildTarget), typeof(string)};
	foreach (var t in TypeCache.GetTypesDerivedFrom<IOrderedCallback>()) {
		if (t.IsAbstract || t.IsInterface)
			continue;

		// Defer creating the instance until we actually add it to one of the
		// lists
		object instance = null;

		if (findBuildProcessors) {
			AddToListIfTypeImplementsInterface(t, ref instance,
			                                   ref processors.buildPreprocessors);
			AddToListIfTypeImplementsInterface(
				t, ref instance, ref processors.buildPreprocessorsWithReport);
			AddToListIfTypeImplementsInterface(
				t, ref instance, ref processors.buildPostprocessors);
			AddToListIfTypeImplementsInterface(
				t, ref instance, ref processors.buildPostprocessorsWithReport);
		}

		if (findSceneProcessors) {
			AddToListIfTypeImplementsInterface(t, ref instance,
			                                   ref processors.sceneProcessors);
			AddToListIfTypeImplementsInterface(
				t, ref instance, ref processors.sceneProcessorsWithReport);
		}

		if (findTargetProcessors) {
			AddToListIfTypeImplementsInterface(
				t, ref instance, ref processors.buildTargetProcessors);
		}

		if (findFilterProcessors) {
			AddToListIfTypeImplementsInterface(
				t, ref instance, ref processors.filterBuildAssembliesProcessor);
		}

		if (findUnityLinkerProcessors) {
			AddToListIfTypeImplementsInterface(
				t, ref instance, ref processors.unityLinkerProcessors);
		}

		if (findIl2CppProcessors) {
			AddToListIfTypeImplementsInterface(t, ref instance,
			                                   ref processors.il2cppProcessors);
		}

		if (findShaderProcessors) {
			AddToListIfTypeImplementsInterface(t, ref instance,
			                                   ref processors.shaderProcessors);
		}

		if (findBuildPlayerScriptDLLsProcessors) {
			AddToListIfTypeImplementsInterface(
				t, ref instance, ref processors.buildPlayerScriptDLLProcessors);
		}
	}

	if (findBuildProcessors) {
		foreach (var m in EditorAssemblies.GetAllMethodsWithAttribute<
				 Callbacks.PostProcessBuildAttribute>())
			if (ValidateMethod<Callbacks.PostProcessBuildAttribute>(
				    m, postProcessBuildAttributeParams))
				AddToList(new AttributeCallbackWrapper(m),
				          ref processors.buildPostprocessorsWithReport);
	}

	if (findSceneProcessors) {
		foreach (var m in EditorAssemblies.GetAllMethodsWithAttribute<
				 Callbacks.PostProcessSceneAttribute>())
			if (ValidateMethod<Callbacks.PostProcessSceneAttribute>(
				    m, Type.EmptyTypes))
				AddToList(new AttributeCallbackWrapper(m),
				          ref processors.sceneProcessorsWithReport);
	}
...
internal static bool ValidateType<T>(Type t) {
	return (typeof(T).IsAssignableFrom(t) &&
	        t != typeof(AttributeCallbackWrapper));
}
...
static bool ValidateMethod<T>(MethodInfo method, Type[] expectedArguments) {
	...
	if (method.IsDefined(attribute, false)) {
		...
		if (!method.IsStatic) {
			...
			Debug.LogErrorFormat("Method {0} with {1} attribute must be static.",
			                     method.Name, atributeName);
			return false;
		}
		...
		if (method.IsGenericMethod || method.IsGenericMethodDefinition) {
			...
			Debug.LogErrorFormat(
				"Method {0} with {1} attribute cannot be generic.", method.Name,
				atributeName);
			return false;
		}
		...
		if (!signatureCorrect) {
			...
			Debug.LogErrorFormat(
				"Method {0} with {1} attribute does not have the correct signature, expected: {2}.",
				method.Name, atributeName, expectedArgumentsString);
			return false;
		}
		return true;
	}
...
private static bool InvokeCallbackInterfacesPair<T1, T2>(
	List<T1> oneInterfaces, Action<T1> invocationOne,
	List<T2> twoInterfaces, Action<T2> invocationTwo,
	bool exitOnFailure) where T1 : IOrderedCallback where T2
: IOrderedCallback {
	if (oneInterfaces == null && twoInterfaces == null)
		return true;

	// We want to walk both interface lists and invoke the callbacks, but if
	// we just did the whole of list 1 followed by the whole of list 2, the
	// ordering would be wrong. So, we have to walk both lists simultaneously,
	// calling whichever callback has the lower ordering value
	IEnumerator<T1> e1 = (oneInterfaces != null)
	                       ? (IEnumerator<T1>)oneInterfaces.GetEnumerator()
	                       : null;
	IEnumerator<T2> e2 = (twoInterfaces != null)
	                       ? (IEnumerator<T2>)twoInterfaces.GetEnumerator()
	                       : null;
	...
	while (e1 != null || e2 != null) {
		try {
			if (e1 != null && (e2 == null || e1.Current.callbackOrder <
			                   e2.Current.callbackOrder)) {
...
#pragma warning disable 618
	InvokeCallbackInterfacesPair(
		processors.buildPreprocessors,
		bpp => bpp.OnPreprocessBuild(report.summary.platform,
		                             report.summary.outputPath),
		processors.buildPreprocessorsWithReport,
		bpp => bpp.OnPreprocessBuild(report),
		(report.summary.options & BuildOptions.StrictMode) != 0 ||
		(report.summary.assetBundleOptions &
		 BuildAssetBundleOptions.StrictMode) != 0);
#pragma warning restore 618
}

[RequiredByNativeCode]
internal static void OnSceneProcess(UnityEngine.SceneManagement.Scene scene,
                                    BuildReport report) {
#pragma warning disable 618
	InvokeCallbackInterfacesPair(
		processors.sceneProcessors, spp => spp.OnProcessScene(scene),
		processors.sceneProcessorsWithReport,
		spp => spp.OnProcessScene(scene, report),
		report && ((report.summary.options & BuildOptions.StrictMode) != 0 ||
		           (report.summary.assetBundleOptions &
		            BuildAssetBundleOptions.StrictMode) != 0));
#pragma warning restore 618
}
...
#pragma warning disable 618
	InvokeCallbackInterfacesPair(
		processors.buildPostprocessors,
		bpp => bpp.OnPostprocessBuild(report.summary.platform,
		                              report.summary.outputPath),
		processors.buildPostprocessorsWithReport,
		bpp => bpp.OnPostprocessBuild(report),
		(report.summary.options & BuildOptions.StrictMode) != 0 ||
		(report.summary.assetBundleOptions &
		 BuildAssetBundleOptions.StrictMode) != 0);
#pragma warning restore 618
...
OnActiveBuildTargetChanged(BuildTarget previousPlatform,
                           BuildTarget newPlatform) {
	if (processors.buildTargetProcessors != null) {
		foreach (IActiveBuildTargetChanged abtc in processors
...
[RequiredByNativeCode]
internal static ShaderCompilerData[] OnPreprocessShaders(
	...
	if (processors.buildPlayerScriptDLLProcessors != null) {
		foreach (var step in processors.buildPlayerScriptDLLProcessors) {
			try {
				...
				Debug.LogException(e);
				if ((report.summary.options & BuildOptions.StrictMode) != 0 ||
				    (report.summary.assetBundleOptions &
				     BuildAssetBundleOptions.StrictMode) != 0)
					return;
			}
...
[RequiredByNativeCode]
internal static string[] FilterAssembliesIncludedInBuild(
	BuildOptions buildOptions, string[] assemblies) {
	if (processors.filterBuildAssembliesProcessor == null) {
		return assemblies;
	}
	...
	foreach (var filteredAssembly in processors
	         .filterBuildAssembliesProcessor) {
		int assemblyCount = filteredAssemblies.Length;
		filteredAssemblies = filteredAssembly.OnFilterAssemblies(
			buildOptions, filteredAssemblies);
