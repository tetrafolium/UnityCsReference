...
protected BuildVerifier() {
	...
	var configPath = Path.Combine(
		Path.Combine(EditorApplication.applicationContentsPath, "Resources"),
		"BuildVerification.xml");
	...
	while (it.MoveNext()) {
		...
		if (string.IsNullOrEmpty(name))
			throw new ApplicationException(string.Format(
							       "Failed to load {0}, <assembly> name attribute is empty",
							       configPath));
		...
		m_UnsupportedAssemblies [platform]
		.Add(name);
	}
}

protected void VerifyBuildInternal(BuildTarget target,
                                   string managedDllFolder) {
		...
		if (file.EndsWith(".dll")) {
			...
			if (!VerifyAssembly(target, fname))
				Debug.LogWarningFormat(
					...
					" on {1} platform. Various failures might follow.",
...
protected bool VerifyAssembly(BuildTarget target, string assembly) {
	if (m_UnsupportedAssemblies.ContainsKey("*") && m_UnsupportedAssemblies
	    ["*"]
	    .Contains(assembly) ||
	    m_UnsupportedAssemblies.ContainsKey(target.ToString()) &&
	    m_UnsupportedAssemblies [target.ToString()]
	    .Contains(assembly))
		return false;
