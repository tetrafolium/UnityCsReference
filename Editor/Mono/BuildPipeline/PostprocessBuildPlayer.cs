...
internal static class PostprocessBuildPlayer {
  ...
  internal static bool GetProjectBootConfigKeyValue(string key,
                                                    out string value) {
  ...
  private static Dictionary<string, string> projectBootConfigEntries =
      new Dictionary<string, string>();

  // Seems to be used only by
  // PlatformDependent\AndroidPlayer\Editor\Managed\PostProcessAndroidPlayer.cs
  internal static bool InstallPluginsByExtension(string pluginSourceFolder,
                                                 string extension,
                                                 string debugExtension,
                                                 string destPluginFolder,
                                                 bool copyDirectories) {
    ...
    foreach (string path in contents) {
      ...
      bool filenameMatch =
          fileExtension.Equals(extension, StringComparison.OrdinalIgnoreCase) ||
          fileName.Equals(extension, StringComparison.OrdinalIgnoreCase);
      bool debugMatch =
          !string.IsNullOrEmpty(debugExtension) &&
          (fileExtension.Equals(debugExtension,
  ...
  internal static void InstallStreamingAssets(string stagingAreaDataPath,
    ...
    if (Directory.Exists(StreamingAssets)) {
      ...
      FileUtil.CopyDirectoryRecursiveForPostprocess(StreamingAssets, outputPath,
                                                    true);
      if (report != null) {
        report.RecordFilesAddedRecursive(outputPath,
                                         CommonRoles.streamingAsset);
      }
    }
  }

  static public string PrepareForBuild(BuildOptions options,
                                       BuildTargetGroup targetGroup,
                                       BuildTarget target) {
    var postprocessor =
        ModuleManager.GetBuildPostProcessor(targetGroup, target);
    if (postprocessor == null)
      return null;
    return postprocessor.PrepareForBuild(options, target);
  }

  static public bool SupportsScriptsOnlyBuild(BuildTargetGroup targetGroup,
                                              BuildTarget target) {
  ...
  static public string GetExtensionForBuildTarget(BuildTargetGroup targetGroup,
                                                  BuildTarget target,
  ...
  static public bool SupportsInstallInBuildFolder(BuildTargetGroup targetGroup,
                                                  BuildTarget target) {
    IBuildPostprocessor postprocessor =
        ModuleManager.GetBuildPostProcessor(targetGroup, target);
    if (postprocessor != null) {
  ...
  static public bool SupportsLz4Compression(BuildTargetGroup targetGroup,
                                            BuildTarget target) {
    IBuildPostprocessor postprocessor =
        ModuleManager.GetBuildPostProcessor(targetGroup, target);
    if (postprocessor != null)
  ...
  static public Compression GetDefaultCompression(BuildTargetGroup targetGroup,
                                                  BuildTarget target) {
    IBuildPostprocessor postprocessor =
        ModuleManager.GetBuildPostProcessor(targetGroup, target);
    if (postprocessor != null)
  ...
  static public void Launch(BuildTargetGroup targetGroup,
                            BuildTarget buildTarget, string path,
                            string productName, BuildOptions options,
                            BuildReport buildReport) {
    IBuildPostprocessor postprocessor =
        ModuleManager.GetBuildPostProcessor(targetGroup, buildTarget);
    if (postprocessor != null) {
      BuildLaunchPlayerArgs args;
      args.target = buildTarget;
      args.playerPackage =
          BuildPipeline.GetPlaybackEngineDirectory(buildTarget, options);
  ...
  static public void LaunchOnTargets(BuildTargetGroup targetGroup,
                                     BuildTarget buildTarget,
                                     Build.Reporting.BuildReport buildReport,
                                     List<DeploymentTargetId> launchTargets) {
    try {
      ...
      ProgressHandler progressHandler = new ProgressHandler(
          ...
          delegate(string title, string message, float globalProgress) {
            if (EditorUtility.DisplayCancelableProgressBar(title, message,
                                                           globalProgress))
      ...
      taskManager.AddTask(() => {
        ...
        foreach (var target in launchTargets) {
          try {
            var manager = DeploymentTargetManager.CreateInstance(
                targetGroup, buildReport.summary.platform);
            var buildProperties =
                BuildProperties.GetFromBuildReport(buildReport);
            manager.LaunchBuildOnTarget(
                buildProperties, target,
                taskManager.SpawnProgressHandlerFromCurrentTask());
            successfulLaunches++;
      ...
      throw new UnityException(
          string.Format("Could not find any valid targets to launch on for {0}",
                        buildTarget));
    }
  }

  [RequiredByNativeCode]
  static public void UpdateBootConfig(BuildTargetGroup targetGroup,
                                      BuildTarget target, BootConfigData config,
                                      BuildOptions options) {
    IBuildPostprocessor postprocessor =
        ModuleManager.GetBuildPostProcessor(targetGroup, target);
    if (postprocessor != null)
  ...
  static public void Postprocess(BuildTargetGroup targetGroup,
                                 BuildTarget target, string installPath,
    ...
    string playerPackage =
        BuildPipeline.GetPlaybackEngineDirectory(target, options);

    // Disallow providing an empty string as the installPath
    bool willInstallInBuildFolder =
        (options & BuildOptions.InstallInBuildFolder) != 0 &&
        SupportsInstallInBuildFolder(targetGroup, target);
    ...
    if (postprocessor != null) {
      ...
      try {
        ...
        // Rethrow exceptions during build postprocessing as
        // BuildFailedException, so we don't pretend the build was fine.
    ...
    throw new UnityException(
        string.Format("Build target '{0}' not supported", target));
  }

  internal static string ExecuteSystemProcess(string command, string args,
                                              string workingdir) {
    var psi = new ProcessStartInfo(){FileName = command, Arguments = args,
