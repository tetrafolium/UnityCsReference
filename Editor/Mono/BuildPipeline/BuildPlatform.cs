...
public BuildPlatform(string locTitle, string iconId,
                     BuildTargetGroup targetGroup,
                     BuildTarget defaultTarget, bool forceShowTarget)
	: this(locTitle, "", iconId, targetGroup, defaultTarget,
	       forceShowTarget) {
}

public BuildPlatform(string locTitle, string tooltip, string iconId,
                     BuildTargetGroup targetGroup,
                     BuildTarget defaultTarget, bool forceShowTarget) {
	this.targetGroup = targetGroup;
	name = targetGroup !=
	       BuildTargetGroup.Unknown ? BuildPipeline.GetBuildTargetGroupName(
		defaultTarget)
	  : "";
...
internal BuildPlatforms() {
	...
	DiscoveredTargetInfo[] buildTargets =
		BuildTargetDiscovery.GetBuildTargetInfoList();

	// Standalone needs to be first
	// Before we had BuildTarget.StandaloneWindows for
	// BuildPlatform.defaultTarget But that doesn't make a lot of sense, as
	// editor use it in places, so it should agree with editor platform
	// TODO: should we poke module manager for target support? i think we can
	// assume support for standalone for editor platform
	// TODO: even then - picking windows standalone unconditionally wasn't
	// much better
	...
	buildPlatformsList.Add(new BuildPlatform(
				       ...
				       "BuildSettings.Standalone", BuildTargetGroup.Standalone,
				       standaloneTarget, true));

	foreach (var target in buildTargets) {
		if (!target.HasFlag(TargetAttributes.IsStandalonePlatform)) {
			BuildTargetGroup btg =
				BuildPipeline.GetBuildTargetGroup(target.buildTargetPlatformVal);
			buildPlatformsList.Add(new BuildPlatform(
						       BuildPipeline.GetBuildTargetGroupDisplayName(btg),
						       target.iconName, btg, target.buildTargetPlatformVal,
...
public string GetBuildTargetDisplayName(BuildTargetGroup group,
                                        BuildTarget target) {
...
public string GetModuleDisplayName(BuildTargetGroup buildTargetGroup,
...
public List<BuildPlatform> GetValidPlatforms(bool includeMetaPlatforms) {
	...
	foreach (BuildPlatform bp in buildPlatforms)
		if (bp.targetGroup == BuildTargetGroup.Standalone ||
		    BuildPipeline.IsBuildTargetSupported(bp.targetGroup,
		                                         bp.defaultTarget))
			platforms.Add(bp);
