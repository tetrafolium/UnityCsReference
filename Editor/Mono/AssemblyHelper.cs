...
internal partial class AssemblyHelper {
  ...
  static public void CheckForAssemblyFileNameMismatch(string assemblyPath) {
    ...
    if (fileName != assemblyName) {
      Debug.LogWarning("Assembly '" + assemblyName +
  ...
  GetAssemblyDefinitionCached(string path,
  ...
  static private bool IgnoreAssembly(string assemblyPath, BuildTarget target) {
    if (target == BuildTarget.WSAPlayer) {
      ...
      var profile =
          PlayerSettings.GetApiCompatibilityLevel(BuildTargetGroup.XboxOne);
      if (profile == ApiCompatibilityLevel.NET_4_6 ||
          profile == ApiCompatibilityLevel.NET_Standard_2_0) {
        if (CouldBelongToDotNetOrWindowsRuntime(assemblyPath))
          return true;
      }
    }
  ...
  static private void AddReferencedAssembliesRecurse(
      string assemblyPath, List<string> alreadyFoundAssemblies,
      string[] allAssemblyPaths, string[] foldersToSearch,
      Dictionary<string, AssemblyDefinition> cache, BuildTarget target) {
    ...
    AssemblyDefinition assembly =
        GetAssemblyDefinitionCached(assemblyPath, cache);
    if (assembly == null)
      throw new System.ArgumentException("Referenced Assembly " +
                                         Path.GetFileName(assemblyPath) +
                                         " could not be found!");
    ...
    var architectureSpecificPlugins =
        PluginImporter.GetImporters(target)
            .Where(i => {
            ...
            .Select(i => Path.GetFileName(i.assetPath))
            .Distinct();

    // Go through all referenced assemblies
    foreach (AssemblyNameReference referencedAssembly in assembly.MainModule
                 .AssemblyReferences) {
      ...
      string foundPath =
          FindAssemblyName(referencedAssembly.FullName, referencedAssembly.Name,
                           allAssemblyPaths, foldersToSearch, cache);

      if (foundPath == "") {
        // Ignore architecture specific plugin references
        var found = false;
        foreach (var extension in new[]{".dll", ".winmd"}) {
          if (architectureSpecificPlugins.Any(
                  p => string.Equals(
                      p, referencedAssembly.Name + extension,
                      StringComparison.InvariantCultureIgnoreCase))) {
            found = true;
            break;
          }
        ...
        throw new System.ArgumentException(string.Format(
            "The Assembly {0} is referenced by {1} ('{2}'). But the dll is not allowed to be included or could not be found.",
            referencedAssembly.Name, assembly.MainModule.Assembly.Name.Name,
            assemblyPath));
      }

      AddReferencedAssembliesRecurse(foundPath, alreadyFoundAssemblies,
                                     allAssemblyPaths, foldersToSearch, cache,
                                     target);
    }
  }

  static string FindAssemblyName(string fullName, string name,
                                 string[] allAssemblyPaths,
    ...
    for (int i = 0; i < allAssemblyPaths.Length; i++) {
      ...
      AssemblyDefinition definition =
          GetAssemblyDefinitionCached(allAssemblyPaths[i], cache);
  ...
  static public string[] FindAssembliesReferencedBy(string[] paths,
                                                    string[] foldersToSearch,
                                                    BuildTarget target) {
    ...
    for (int i = 0; i < paths.Length; i++)
      AddReferencedAssembliesRecurse(paths[i], unique, allAssemblyPaths,
                                     foldersToSearch, cache, target);
  ...
  static public string[] FindAssembliesReferencedBy(string path,
                                                    string[] foldersToSearch,
                                                    BuildTarget target) {
  ...
  static public bool IsUnityEngineModule(AssemblyDefinition assembly) {
    return assembly.CustomAttributes.Any(
        a => a.AttributeType.FullName ==
             typeof(UnityEngineModuleAssembly).FullName);
  }

  static public bool IsUnityEngineModule(Assembly assembly) {
    return assembly
               .GetCustomAttributes(typeof(UnityEngineModuleAssembly), false)
               .Length > 0;
  }
  ...
  public static string[] GetDefaultAssemblySearchPaths() {
    ...
    var precompiledAssemblies =
        InternalEditorUtility.GetPrecompiledAssemblies(true, group, target);
    ...
    precompiledAssemblies =
        InternalEditorUtility.GetUnityAssemblies(true, group, target);
    foreach (var asm in precompiledAssemblies)
      searchPaths.Add(Path.GetDirectoryName(asm.Path));
  ...
  public static void ExtractAllClassesThatAreUserExtendedScripts(
      string path, out string[] classNamesArray,
      out string[] classNameSpacesArray,
      out string[] originalClassNameSpacesArray) {
    List<string> classNames = new List<string>();
    List<string> nameSpaces = new List<string>();
    List<string> originalNamespaces = new List<string>();
    var readerParameters = new ReaderParameters();

    // this will resolve any types in assemblies within the same directory as
    // the type's assembly or any folder which contains a currently available
    // precompiled dll
    ...
    AssemblyDefinition assembly =
        AssemblyDefinition.ReadAssembly(path, readerParameters);
      ...
      foreach (TypeDefinition type in module.Types) {
        ...
        try {
          if (IsTypeAUserExtendedScript(baseType)) {
            ...
            var attribute = type.CustomAttributes.SingleOrDefault(
                a =>
                    a.AttributeType.FullName ==
                    typeof(UnityEngine.Scripting.APIUpdating.MovedFromAttribute)
                        .FullName);
          ...
          Debug.LogError("Failed to extract " + type.FullName +
                         " class of base type " + baseType.FullName +
  ...
  /// Extract information about all types in the specified assembly, searchDirs
  /// might be used to resolve dependencies.
  static public AssemblyTypeInfoGenerator.ClassInfo[] ExtractAssemblyTypeInfo(
      BuildTarget targetPlatform, bool isEditor, string assemblyPathName,
      string[] searchDirs) {
    try {
      AssemblyTypeInfoGenerator gen =
          new AssemblyTypeInfoGenerator(assemblyPathName, searchDirs);
      return gen.GatherClassInfo();
    } catch (System.Exception ex) {
      throw new Exception("ExtractAssemblyTypeInfo: Failed to process " +
                          assemblyPathName + ", " + ex);
    }
  }
  ...
  static void FindRuntimeInitializeOnLoadMethodAttributes(
      TypeDefinition type, string assemblyName,
      ref List<RuntimeInitializeClassInfo> classInfoList, ref int methodCount) {
    if (!type.HasMethods)
      return;
      ...
      foreach (var attribute in method.CustomAttributes) {
        if (attribute.AttributeType.FullName ==
          ...
          if (attribute.ConstructorArguments != null &&
            ...
            loadType = (RuntimeInitializeLoadType)
                           attribute.ConstructorArguments[0]
                               .Value;

          RuntimeInitializeClassInfo classInfo =
              new RuntimeInitializeClassInfo();
  ...
  public static RuntimeInitializeOnLoadMethodsData
  ExtractPlayerRuntimeInitializeOnLoadMethods(BuildTarget targetPlatform,
    ...
    foreach (var assemblyPath in assemblyPaths) {
      try {
        ...
        var assembly = AssemblyDefinition.ReadAssembly(
            assemblyPath,
            new ReaderParameters{AssemblyResolver =
                                     assemblyResolverData.Resolver});
        ...
        foreach (var module in assembly.Modules) {
          foreach (var type in module.Types) {
            FindRuntimeInitializeOnLoadMethodAttributes(
                type, assemblyName, ref classInfoList, ref methodCount);
          }
        }
      } catch (Exception ex) {
        throw new Exception(
            "ExtractPlayerRuntimeInitializeOnLoadMethods: Failed to process " +
            assemblyPath + ", " + ex);
      }
    }
  ...
  public static bool IsInternalAssembly(string file) {
    return ModuleUtils.GetAdditionalReferencesForUserScripts().Any(
        p => p.Equals(file));
  }
  ...
  internal static ICollection<string> FindAssemblies(string basePath,
    ...
    try {
      ...
      foreach (DirectoryInfo subdirectory in directory.GetDirectories())
        assemblies.AddRange(
            FindAssemblies(subdirectory.FullName, maxDepth - 1));
    } catch (Exception) {
