...
public static class ObjectFactory {
  ...
  [FreeFunction(ThrowsException = true)]
  static extern Component AddDefaultComponent([ NotNull ] GameObject gameObject,
                                              [ NotNull ] Type type);
  ...
  static void CheckTypeValidity(Type type) {
    if (type.IsAbstract) {
      throw new ArgumentException(
          "Abstract types can't be used in the ObjectFactory : " +
          type.FullName);
    }
    if (Attribute.GetCustomAttribute(
            type, typeof(ExcludeFromObjectFactoryAttribute)) != null) {
      throw new ArgumentException("The type " + type.FullName +
                                  " is not supported by the ObjectFactory.");
    }
  }
  ...
  public static Object CreateInstance(Type type) {
    ...
    if (type == typeof(GameObject)) {
      throw new ArgumentException(
          "GameObject type must be created using ObjectFactory.CreateGameObject instead : " +
          type.FullName);
    }
    if (type.IsSubclassOf(typeof(Component))) {
      throw new ArgumentException(
          "Component type must be created using ObjectFactory.AddComponent instead : " +
          type.FullName);
    }
    if (type.GetConstructor(BindingFlags.NonPublic | BindingFlags.Instance,
      ...
      throw new ArgumentException(
          type.FullName +
          " constructor is not accessible which prevent this type from being used in ObjectFactory.");
    }
  ...
  public static Component AddComponent(GameObject gameObject, Type type) {
    ...
    if (!type.IsSubclassOf(typeof(Component))) {
      throw new ArgumentException(
          "Non-Component type must use ObjectFactory.CreateInstance instead : " +
          type.FullName);
    }
    return AddDefaultComponent(gameObject, type);
  }

  public static GameObject CreateGameObject(Scene scene, HideFlags hideFlags,
                                            string name, params Type[] types) {
    if (!scene.IsValid()) {
      throw new ArgumentException(
  ...
  public static GameObject CreatePrimitive(PrimitiveType type) {
    var go = CreateGameObject(type.ToString(), typeof(MeshFilter),
                              typeof(MeshRenderer));
    go.SetActive(false);
    switch (type) {
    ...
    case PrimitiveType.Plane:
      go.GetComponent<MeshFilter>().sharedMesh =
          Resources.GetBuiltinResource<Mesh>("New-Plane.fbx");
