...
namespace UnityEditor.VisualStudioIntegration {
  ...
  class SolutionSynchronizer {
    enum Mode { UnityScriptAsUnityProj, UnityScriptAsPrecompiledAssembly }

    public static readonly ISolutionSynchronizationSettings
    ...
    static internal readonly
        Dictionary<string, ScriptingLanguage> BuiltinSupportedExtensions =
    ...
    private static readonly string[] reimportSyncExtensions =
        new[]{".dll", ".asmdef"};
    ...
    /// </summary>
    static readonly Dictionary<ScriptingLanguage, string> ProjectExtensions =
        new Dictionary<ScriptingLanguage, string>{
    ...
    public SolutionSynchronizer(string projectDirectory,
                                ISolutionSynchronizationSettings settings,
                                IAssemblyNameProvider assemblyNameProvider,
                                IFileIO fileIO, IGUIDGenerator guidGenerator) {
    ...
    public SolutionSynchronizer(string projectDirectory,
                                ISolutionSynchronizationSettings settings)
        : this(projectDirectory, settings, new AssemblyNameProvider(),
               new FileIOProvider(), new GUIDProvider()) {}

    public SolutionSynchronizer(string projectDirectory)
        : this(projectDirectory, DefaultSynchronizationSettings) {}

    private void SetupProjectSupportedExtensions() {
      ProjectSupportedExtensions =
          m_assemblyNameProvider.ProjectSupportedExtensions;
    }
    ...
    private static ScriptingLanguage
    ScriptingLanguageFor(Compilation.Assembly assembly) {
      return ScriptingLanguageFor(
    ...
    private static ScriptingLanguage ScriptingLanguageFor(string extension) {
      ...
      if (BuiltinSupportedExtensions.TryGetValue(extension.TrimStart('.'),
                                                 out result))
        return result;
    ...
    static string GetExtensionOfSourceFiles(string[] files) {
      // Files are sorted by the compilation pipeline, ensuring cs files are the
      // first in the list.
    ...
    private static void DumpAssembly(Compilation.Assembly assembly) {
      Console.WriteLine("{0} ({1})", assembly.outputPath,
    ...
    public void Sync() {
      ...
      // Do not sync solution until all Unity extensions are registered and
      // initialized. Otherwise Unity might emit errors when VSTU tries to
      // generate the solution and get all managed extensions, which not yet
      // initialized.
      ...
      bool externalCodeAlreadyGeneratedProjects =
          AssetPostprocessingInternal.OnPreGeneratingCSProjectFiles();
...
#pragma warning disable 618
    ...
    internal void GenerateAndWriteSolutionAndProjects(
        ScriptEditorUtility.ScriptEditor scriptEditor) {
      Profiler.BeginSample("GenerateAndWriteSolutionAndProjects");

      Profiler.BeginSample("SolutionSynchronizer.GetIslands");
      // Only synchronize islands that have associated source files and ones
      // that we actually want in the project. This also filters out DLLs coming
      // from .asmdef files in packages.
      List<Compilation.Assembly> assemblies =
          m_assemblyNameProvider.GetAssemblies(ShouldFileBePartOfSolution)
              .ToList();
      ...
      var allProjectAssemblies =
          RelevantAssembliesForMode(assemblies, ModeForCurrentExternalEditor())
              .ToList();

      foreach (Compilation.Assembly assembly in allProjectAssemblies) {
        Profiler.BeginSample("SyncProject");
        SyncProject(assembly, allAssetProjectParts,
                    ParseResponseFileData(assembly), allProjectAssemblies);
        Profiler.EndSample();
      }
      Profiler.EndSample();
    }
    ...
    ParseResponseFileData(Compilation.Assembly assembly) {
      ...
      Dictionary<string, ResponseFileData> responseFilesWithErrors =
          responseFilesData.Where(x => x.Value.Errors.Any())
              .ToDictionary(x => x.Key, x => x.Value);

      if (responseFilesWithErrors.Any()) {
        foreach (var error in responseFilesWithErrors)
          foreach (var valueError in error.Value.Errors) {
            UnityEngine.Debug.LogErrorFormat("{0} Parse Error : {1}", error.Key,
                                             valueError);
          }
      }
    ...
    Dictionary<string, string> GenerateAllAssetProjectParts() {
      Dictionary<string, StringBuilder> stringBuilders =
      ...
      foreach (string asset in m_assemblyNameProvider.GetAllAssetPaths()) {
        ...
        if (IsSupportedExtension(extension) &&
          ...
          // Find assembly the asset belongs to by adding script extension and
          // using compilation pipeline.
          var assemblyName =
              m_assemblyNameProvider.GetAssemblyNameFromScriptPath(asset +
                                                                   ".cs");
          assemblyName = assemblyName ?? m_assemblyNameProvider
                             .GetAssemblyNameFromScriptPath(asset + ".js");
          assemblyName = assemblyName ?? m_assemblyNameProvider
                             .GetAssemblyNameFromScriptPath(asset + ".boo");
          ...
          projectBuilder.Append("     <None Include=\"")
              .Append(EscapedRelativePathFor(asset))
              .Append("\" />")
    ...
    void SyncProject(Compilation.Assembly assembly,
      ...
      SyncProjectFileIfNotChanged(
          ...
          ProjectText(assembly, ModeForCurrentExternalEditor(),
                      allAssetsProjectParts, responseFilesData,
                      allProjectAssemblies));
    }

    void SyncProjectFileIfNotChanged(string path, string newContents) {
      if (Path.GetExtension(path) == ".csproj") {
        newContents = AssetPostprocessingInternal.CallOnGeneratedCSProject(
            path, newContents);
      }
    ...
    void SyncSolutionFileIfNotChanged(string path, string newContents) {
      newContents = AssetPostprocessingInternal.CallOnGeneratedSlnSolution(
          path, newContents);

      SyncFileIfNotChanged(path, newContents);
    }

    static void LogDifference(string path, string currentContents,
                              string newContents) {
      Console.WriteLine("[C# Project] Writing {0} because it has changed",
                        path);
      ...
      do {
        ...
        if (currentLine != null && newLine != null && currentLine != newLine) {
          Console.WriteLine("[C# Project] First difference on line {0}",
                            lineNumber);

          Console.WriteLine("\n[C# Project] Current {0}:", path);

          for (int i = 0; i < 5 && currentLine != null;
               i++, currentLine = currentReader.ReadLine()) {
            Console.WriteLine("[C# Project]   {0:D3}: {1}", lineNumber + i,
                              currentLine);
          }

          Console.WriteLine("\n[C# Project] New {0}:", path);

          for (int i = 0; i < 5 && newLine != null;
    ...
    public static readonly Regex scriptReferenceExpression =
        new Regex(@"^Library.ScriptAssemblies.(?<dllname>(?<project>.*)\.dll$)",
                  RegexOptions.Compiled | RegexOptions.IgnoreCase);

    static bool
    IsAdditionalInternalAssemblyReference(bool isBuildingEditorProject,
                                          string reference) {
      if (isBuildingEditorProject)
        return Modules.ModuleUtils
            .GetAdditionalReferencesForEditorCsharpProject()
            .Contains(reference);
      return false;
    }

    string ProjectText(Compilation.Assembly assembly, Mode mode,
                       Dictionary<string, string> allAssetsProjectParts,
                       IEnumerable<ResponseFileData> responseFilesData,
                       List<Compilation.Assembly> allProjectAssemblies) {
      var projectBuilder =
          new StringBuilder(ProjectHeader(assembly, responseFilesData));
      ...
      foreach (string file in assembly.sourceFiles) {
        ...
        if (".dll" != extension) {
          projectBuilder.Append("     <Compile Include=\"")
              .Append(fullFile)
              .Append("\" />")
              .Append(Environment.NewLine);
        } else {
      ...
      // Append additional non-script files that should be included in project
      // generation.
      string additionalAssetsForProject;
      if (allAssetsProjectParts.TryGetValue(assembly.name,
                                            out additionalAssetsForProject))
        projectBuilder.Append(additionalAssetsForProject);

      var responseRefs = responseFilesData.SelectMany(
          x => x.FullPathReferences.Select(r => r));
      var internalAssemblyReferences =
          assembly.assemblyReferences
              .Where(i => !i.sourceFiles.Any(ShouldFileBePartOfSolution))
              .Select(i => i.outputPath);
      var allReferences =
          assembly.compiledAssemblyReferences.Union(responseRefs)
              .Union(references)
      ...
      foreach (var reference in allReferences) {
        string fullReference = Path.IsPathRooted(reference)
                                   ? reference
                                   : Path.Combine(_projectDirectory, reference);
      ...
      if (0 < assembly.assemblyReferences.Length) {
        ...
        projectBuilder.Append("  <ItemGroup>").Append(Environment.NewLine);
        foreach (Assembly reference in assembly.assemblyReferences.Where(
                     i => i.sourceFiles.Any(ShouldFileBePartOfSolution))) {
          var referencedProject = reference.outputPath;

          projectBuilder.Append("    <ProjectReference Include=\"")
    ...
    static void AppendReference(string fullReference,
      ...
      projectBuilder.Append(" <Reference Include=\"")
          .Append(Utility.FileNameWithoutExtension(escapedFullPath))
          .Append("\">")
          .Append(Environment.NewLine);
    ...
    public string ProjectFile(Compilation.Assembly assembly) {
      ...
      return Path.Combine(
          _projectDirectory,
          string.Format("{0}{1}", assembly.name, ProjectExtensions[language]));
    }

    internal string SolutionFile() {
      return Path.Combine(_projectDirectory,
                          string.Format("{0}.sln", _projectName));
    }

    private string
    ProjectHeader(Compilation.Assembly assembly,
                  IEnumerable<ResponseFileData> responseFilesData) {
      ...
      cscToolPath = Paths.Combine(EditorApplication.applicationContentsPath,
                                  "Tools", "RoslynScripts");
      cscToolPath = Paths.UnifyDirectorySeparator(cscToolPath);
      string cscToolExe = Application.platform == RuntimePlatform.WindowsEditor ?
                                                  "unity_csc.bat"
          : "unity_csc.sh";
      ...
      var arguments = new object[]{
          ...
          assembly.compilerOptions.AllowUnsafeCode |
              responseFilesData.Any(x => x.Unsafe),
          cscToolPath,
          cscToolExe,
      };
      ...
      try {
        ScriptingLanguage language = ScriptingLanguageFor(assembly);
        return string.Format(_settings.GetProjectHeaderTemplate(language),
                             arguments);
      } catch (Exception) {
    ...
    private void SyncSolution(List<Compilation.Assembly> assemblies) {
      SyncSolutionFileIfNotChanged(
          SolutionFile(),
          SolutionText(assemblies, ModeForCurrentExternalEditor()));
    }
...
#pragma warning disable 618
      ...
      return EditorPrefs.GetBool("kExternalEditorSupportsUnityProj", false)
                 ? Mode.UnityScriptAsUnityProj
                 : Mode.UnityScriptAsPrecompiledAssembly;
    }

    private string SolutionText(List<Compilation.Assembly> assemblies,
                                Mode mode) {
      ...
      string projectConfigurations = string.Join(
          ...
          relevantAssemblies
              ...
              .ToArray());
      return string.Format(_settings.SolutionTemplate, fileversion, vsversion,
                           projectEntries, projectConfigurations);
    }

    private static IEnumerable<Compilation.Assembly>
    RelevantAssembliesForMode(List<Compilation.Assembly> assemblies,
                              Mode mode) {
      return assemblies.Where(i => ScriptingLanguage.CSharp ==
                                   ScriptingLanguageFor(i));
    }

    /// <summary>
    /// Get a Project("{guid}") = "MyProject", "MyProject.unityproj",
    /// "{projectguid}" entry for each relevant language
    /// </summary>
    internal
        string GetProjectEntries(IEnumerable<Compilation.Assembly> assemblies) {
    ...
    string EscapedRelativePathFor(string file) {
      ...
      var packageInfo = m_assemblyNameProvider.FindForAssetPath(
          path.ConvertSeparatorsToUnity());
      if (packageInfo != null) {
        // We have to normalize the path, because the PackageManagerRemapper
        // assumes dir seperators will be os specific.
        var absolutePath =
            Path.GetFullPath(NormalizePath(path)).ConvertSeparatorsToWindows();
    ...
    string SolutionGuid(Assembly assembly) {
      return m_GUIDProvider.SolutionGuid(
          _projectName, GetExtensionOfSourceFiles(assembly.sourceFiles));
    }
