...
string[] ProjectSupportedExtensions {
	get;
}
string ProjectGenerationRootNamespace {
	get;
}
ProjectGenerationFlag ProjectGenerationFlag {
	get;
}
...
ResponseFileData ParseResponseFile(string responseFilePath,
...
[Flags] enum ProjectGenerationFlag {
	None = 0,
	Embedded = 1,
	Local = 2,
	Registry = 4,
...
ProjectGenerationFlag m_ProjectGenerationFlag =
	...
	EditorPrefs.GetInt("unity_project_generation_flag", 3);
...
public IEnumerable<Assembly>
GetAssemblies(Func<string, bool> shouldFileBePartOfSolution) {
	foreach (var assembly in CompilationPipeline.GetAssemblies()) {
		if (assembly.sourceFiles.Any(shouldFileBePartOfSolution)) {
			yield return new Assembly(
				assembly.name, assembly.outputPath, assembly.sourceFiles,
				new[] {"DEBUG", "TRACE"}
	...
	if (HasFlag(ProjectGenerationFlag.PlayerAssemblies)) {
		foreach (var assembly in CompilationPipeline
			...
			yield return new Assembly(
				assembly.name + ".Player", assembly.outputPath,
...
public string GetCompileOutputPath(string assemblyName) {
	return assemblyName.EndsWith(".Player", StringComparison.Ordinal)
	  ? @"Temp\Bin\Debug\Player\" : @" Temp\Bin\Debug\";
}
...
public UnityEditor.PackageManager.PackageInfo
FindForAssetPath(string assetPath) {
...
ParseResponseFile(string responseFilePath, string projectDirectory,
                  string[] systemReferenceDirectories) {
	return CompilationPipeline.ParseResponseFile(
		responseFilePath, projectDirectory, systemReferenceDirectories);
}

public IEnumerable<string> GetRoslynAnalyzerPaths() {
	return PluginImporter.GetAllImporters()
	       .Where(i => !i.isNativePlugin &&
	              AssetDatabase.GetLabels(i).SingleOrDefault(
			      l => l == "RoslynAnalyzer") != null)
