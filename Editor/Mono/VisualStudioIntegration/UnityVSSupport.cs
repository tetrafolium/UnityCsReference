...
public static void Initialize(string editorPath) {
	var externalEditor =
		editorPath ?? ScriptEditorUtility.GetExternalScriptEditor();
...
private static void InitializeVSForMac(string externalEditor) {
	...
	if (string.IsNullOrEmpty(bridgeFile) || !File.Exists(bridgeFile)) {
		Console.WriteLine(
			"Unable to find Tools for Unity bridge dll for Visual Studio for Mac " +
			externalEditor);
		return;
	}

	s_UnityVSBridgeToLoad = bridgeFile;
	InternalEditorUtility.RegisterPrecompiledAssembly(
		Path.GetFileNameWithoutExtension(bridgeFile), bridgeFile);
}

private static bool IsVSForMac(string externalEditor,
                               out Version vsfmVersion) {
	vsfmVersion = null;
	...
	try {
		...
		Console.WriteLine(
			"Failed to read Visual Studio for Mac information: {0}", e);
		return false;
	}
}

private static bool GetVSForMacVersion(string externalEditor,
                                       out Version vsfmVersion) {
	vsfmVersion = null;

	// Read the full VS for Mac version from the plist, it will look like
	// this:
	//
	...
	const string versionStringRegex =
		@"\<key\>CFBundleShortVersionString\</key\>\s+\<string\>(?<version>\d+\.\d+\.\d+\.\d+?)\</string\>";
...
private static string GetVSForMacBridgeAssembly(string externalEditor,
	...
	var localAddins = Path.Combine(
		...
		"Library/Application Support/VisualStudio/" + vsfmVersion.Major +
		".0" + "/LocalInstall/Addins");

	// In the user addins repository, the addins are suffixed by their
	// versions, like `MonoDevelop.Unity.1.0` When installing another local
	// user addin, MD will remove files inside the folder So we browse all
	// VSTUM addins, and return the one with a bridge, which is the one MD
	// will load
	if (Directory.Exists(localAddins)) {
		foreach (var folder in Directory.GetDirectories(
				 localAddins, addinName + "*",
	...
	bridge = Path.Combine(externalEditor,
	                      "Contents/Resources/lib/monodevelop/AddIns/" +
	                      addinName + "/" + addinBridge);
	if (File.Exists(bridge))
		return bridge;
...
private static void InitializeVisualStudio(string externalEditor) {
	...
	if (bridgeFile == null) {
		Console.WriteLine(
			"Unable to find bridge dll in registry for Microsoft Visual Studio Tools for Unity for " +
			externalEditor);
		return;
	}
	if (!File.Exists(bridgeFile)) {
		Console.WriteLine(
			"Unable to find bridge dll on disk for Microsoft Visual Studio Tools for Unity for " +
			bridgeFile);
		return;
	}
	s_UnityVSBridgeToLoad = bridgeFile;
...
static bool IsVisualStudio(string externalEditor,
                           out VisualStudioVersion vsVersion) {
	...
	var matches =
		SyncVS.InstalledVisualStudios
		.Where(kvp => kvp.Value.Any(v => UnityEditor.Utils.Paths.AreEqual(
						    v.Path, externalEditor, true)))
		.ToArray();
	if (matches.Length > 0) {
	...
	if (externalEditor.EndsWith("devenv.exe",
...
private static bool
TryGetVisualStudioVersion(string externalEditor,
                          out VisualStudioVersion vsVersion) {
...
private static Version ProductVersion(string externalEditor) {
	try {
		return new Version(
			System.Diagnostics.FileVersionInfo.GetVersionInfo(externalEditor)
			.ProductVersion);
	} catch (Exception) {
...
static public bool IsUnityVSEnabled() {
	...
	if (!s_IsUnityVSEnabled.HasValue)
		s_IsUnityVSEnabled = AppDomain.CurrentDomain.GetAssemblies().Any(
			a => GetAssemblyLocation(a) == s_UnityVSBridgeToLoad);
...
private static string GetVstuBridgeAssembly(VisualStudioVersion version) {
	try {
		...
		// search first for the current user with a fallback to machine wide
		// setting
...
private static string GetVstuBridgePathFromRegistry(string vsVersion,
                                                    bool currentUser) {
	var registryKey = string.Format(
		@"{0}\Software\Microsoft\Microsoft Visual Studio {1} Tools for Unity",
		currentUser ? "HKEY_CURRENT_USER"
	  : "HKEY_LOCAL_MACHINE", vsVersion);

	return (string)
	       Registry.GetValue(registryKey, "UnityExtensionPath", null);
}
...
public static void ScriptEditorChanged(string editorPath) {
	...
	// We reload the domain because selecting a different editor requires
