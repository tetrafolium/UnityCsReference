...
internal class ProjectBrowser : EditorWindow,
                                IHasCustomMenu {
  ...
  private static List<ProjectBrowser> s_ProjectBrowsers =
      new List<ProjectBrowser>();
  ...
  enum ViewMode { OneColumn, TwoColumns }
  ...
  class Styles {
    ...
    public GUIContent m_FilterByLabel =
        EditorGUIUtility.TrIconContent("FilterByLabel", "Search by Label");
    public GUIContent m_FilterByType =
        EditorGUIUtility.TrIconContent("FilterByType", "Search by Type");
    public GUIContent m_CreateDropdownContent =
        EditorGUIUtility.IconContent("CreateAddNew");
    public GUIContent m_SaveFilterContent =
        EditorGUIUtility.TrIconContent("Favorite", "Save search");
    public GUIContent m_PackagesVisibilityContent = EditorGUIUtility.TrIconContent(
        "SceneViewVisibility",
        "Number of hidden packages, click to toggle hidden packages visibility");
    public GUIContent m_EmptyFolderText =
        EditorGUIUtility.TrTextContent("This folder is empty");
    public GUIContent m_SearchIn = EditorGUIUtility.TrTextContent("Search:");

    public Styles() { selectedPathLabel.alignment = TextAnchor.MiddleLeft; }
  ...
  [NonSerialized] string m_SearchFieldText = "";

  // Display state
  [SerializeField] ViewMode m_ViewMode = ViewMode.TwoColumns;
  [SerializeField] int m_StartGridSize = 16;
  [SerializeField] string[] m_LastFolders = new string[0];
  [SerializeField] float m_LastFoldersGridSize = -1f;
  [SerializeField] string m_LastProjectPath;
  [SerializeField] EditorGUIUtility.EditorLockTracker m_LockTracker =
      new EditorGUIUtility.EditorLockTracker();
  ...
  bool m_InternalSelectionChange =
      false; // to know when selection change originated in project view itself
  SearchFilter.SearchArea m_LastLocalAssetsSearchArea =
      SearchFilter.SearchArea.InAssetsOnly;
  ...
  // Folder TreeView
  [SerializeField] TreeViewStateWithAssetUtility m_FolderTreeState;
  TreeViewController m_FolderTree;
  int m_TreeViewKeyboardControlID;

  // Full Asset TreeViewState
  [SerializeField] TreeViewStateWithAssetUtility m_AssetTreeState;
  TreeViewController m_AssetTree;

  // Icon/List area
  [SerializeField] ObjectListAreaState
      m_ListAreaState; // state that survives assembly reloads
  ...
  List<KeyValuePair<GUIContent, string>> m_BreadCrumbs =
      new List<KeyValuePair<GUIContent, string>>();
  bool m_BreadCrumbLastFolderHasSubFolders = false;
  ExposablePopupMenu m_SearchAreaMenu;

  // Keep/Skip Hidden Packages
  [SerializeField] bool m_SkipHiddenPackages;
  ...
  [SerializeField] float m_DirectoriesAreaWidth = k_MinWidthTwoColumns / 2;
  const float k_ResizerWidth = 5f;
  const float k_SliderWidth = 55f;
  [NonSerialized] float m_SearchAreaMenuOffset = -1f;
  [NonSerialized] Rect m_ListAreaRect;
  [NonSerialized] Rect m_TreeViewRect;
  [NonSerialized] Rect m_BottomBarRect;
  [NonSerialized] Rect m_ListHeaderRect;
  [NonSerialized]
  ...
  [NonSerialized]
  public GUIContent m_SearchInPackagesOnly =
      EditorGUIUtility.TrTextContent("In Packages");
  [NonSerialized]
  public GUIContent m_SearchInAssetsOnly =
      EditorGUIUtility.TrTextContent("In Assets");
  [NonSerialized]
  public GUIContent m_SearchInFolders =
      new GUIContent(""); // updated when needed
  [NonSerialized]
  public GUIContent m_SearchAssetStore =
      EditorGUIUtility.TrTextContent("Asset Store"); // updated when needed
  ...
  void OnEnable() {
    ...
    // TestProjectItemOverlayCallback;
  }

  void OnAfterAssemblyReload() {
    // Repaint to ensure ProjectBrowser UI is initialized. This fixes the issue
    // where a new script is created from the application menu bar while the
    // project browser ui was not initialized.
    Repaint();
  }
  ...
  void OnPauseStateChanged(PauseState state) { EndRenaming(); }

  void OnPlayModeStateChanged(PlayModeStateChange state) { EndRenaming(); }
  ...
  void OnAssetLabelsChanged() {
    if (Initialized()) // Otherwise Init will handle this
    {
      ...
      if (m_SearchFilter.IsSearching())
        InitListArea(); // Ensure to refresh search results: we could have
                        // results based on previous label settings
    }
  }
  ...
  void Awake() {
    ...
    if (m_FolderTreeState != null) {
      ...
      m_FolderTreeState.expandedIDs =
          new List<int>(InternalEditorUtility.expandedProjectWindowItems);
    }

    if (m_AssetTreeState != null) {
      m_AssetTreeState.OnAwake();
      m_AssetTreeState.expandedIDs =
          new List<int>(InternalEditorUtility.expandedProjectWindowItems);
    }
  ...
  // Return the logical path that user has active. Used when you need to create
  // new assets from outside the project browser.
  internal string GetActiveFolderPath() {
    if (m_ViewMode == ViewMode.TwoColumns &&
        m_SearchFilter.GetState() == SearchFilter.State.FolderBrowsing &&
        m_SearchFilter.folders.Length > 0)
  ...
  void EnsureValidFolders() {
    ...
    foreach (string folder in m_SearchFilter.folders) {
      ...
      if (AssetDatabase.IsValidFolder(folder)) {
        ...
        // The folder does not exist (could have been deleted) now find first
        // valid parent folder
  ...
  private void ResetViews() {
    if (m_AssetTree != null) {
      ...
      SetSearchFoldersFromCurrentSelection(); // We could have moved, deleted or
                                              // renamed a folder so ensure we
                                              // get folder paths by instanceID
    }

    if (m_FolderTree != null) {
      m_FolderTree.ReloadData();
      SetSearchFolderFromFolderTreeSelection(); // We could have moved, deleted
                                                // or renamed a folder so ensure
                                                // we get folders paths by
                                                // instanceID
    }
  ...
  private void OnProjectChanged() { ResetViews(); }

  public bool Initialized() { return m_ListArea != null; }
  ...
  public void Init() {
    ...
    CalculateRects(); // setup m_TreeViewRect and m_ListAreaRect before calling
                      // InitListArea and TreeViews
    RefreshSelectedPath();
    SetupDroplists();
    ...
    m_StartGridSize = Mathf.Clamp(m_StartGridSize, m_ListArea.minGridSize,
                                  m_ListArea.maxGridSize);
    m_LastFoldersGridSize =
        Mathf.Min(m_LastFoldersGridSize, m_ListArea.maxGridSize);

    m_SearchAreaMenu = new ExposablePopupMenu();

    // m_FolderTreeState is our serialized state (ensure not to reassign it if
    // our serialization system has created it for us)
    if (m_FolderTreeState == null)
  ...
  SearchViewState GetSearchViewState() {
    ...
    case SearchFilter.State.SearchingInAssetsOnly:
      return SearchViewState.InAssetsOnly;
    case SearchFilter.State.SearchingInPackagesOnly:
      return SearchViewState.InPackagesOnly;
  ...
  void SearchButtonClickedCallback(ExposablePopupMenu.ItemData itemClicked) {
    if (!itemClicked.m_On) // Behave like radio buttons: a button that is on
  ...
  void InitSearchMenu() {
    ...
    buttonData.Add(new ExposablePopupMenu.ItemData(
        m_SearchAllAssets, on ? onStyle
        : offStyle, on, true, (int) SearchViewState.AllAssets));
    on = state == SearchViewState.InPackagesOnly;
    buttonData.Add(new ExposablePopupMenu.ItemData(
        m_SearchInPackagesOnly, on ? onStyle
        : offStyle, on, true, (int) SearchViewState.InPackagesOnly));
    on = state == SearchViewState.InAssetsOnly;
    buttonData.Add(new ExposablePopupMenu.ItemData(
        m_SearchInAssetsOnly, on ? onStyle
        : offStyle, on, true, (int) SearchViewState.InAssetsOnly));
    on = state == SearchViewState.SubFolders;
    buttonData.Add(new ExposablePopupMenu.ItemData(
        m_SearchInFolders, on ? onStyle
        : offStyle, on, hasFolderSelected, (int) SearchViewState.SubFolders));
    on = state == SearchViewState.AssetStore;
    buttonData.Add(new ExposablePopupMenu.ItemData(
        m_SearchAssetStore, on ? onStyle
        : offStyle, on, true, (int) SearchViewState.AssetStore));
    ...
    ExposablePopupMenu.PopupButtonData popListData =
        new ExposablePopupMenu.PopupButtonData(popupButtonContent,
                                               s_Styles.exposablePopup);
    m_SearchAreaMenu.Init(buttonData, 10f, 450f, popListData,
                          SearchButtonClickedCallback);
  }

  void AssetStoreSearchEndedCallback() { InitSearchMenu(); }
  ...
  void RefreshSearchText() {
    if (m_SearchFilter.folders.Length > 0) {
      string[] baseFolders =
          ProjectWindowUtil.GetBaseFolders(m_SearchFilter.folders);
      ...
      for (int i = 0; i < baseFolders.Length && i < maxShow; ++i) {
        ...
        var packageInfo =
            PackageManager.PackageInfo.FindForAssetPath(baseFolder);
        if (packageInfo != null &&
            !string.IsNullOrEmpty(packageInfo.displayName))
          baseFolder = Regex.Replace(baseFolder, @"^" + packageInfo.assetPath,
                                     PackageManager.Folders.GetPackagesPath() +
        ...
        if (i == 0 && folderName != "Assets" &&
            folderName != PackageManager.Folders.GetPackagesPath())
          toolTip = baseFolder;
      }
  ...
  void EnsureValidSetup() {
    if (m_LastProjectPath != Directory.GetCurrentDirectory()) {
      ...
      if (Selection.activeInstanceID != 0)
        FrameObjectPrivate(Selection.activeInstanceID, !m_LockTracker.isLocked,
                           false);

      // If we did not frame anything above ensure to show Assets folder in two
      // column mode
    ...
    // In two column do not allow empty search filter, at least show the Assets
    // folders
    if (m_ViewMode == ViewMode.TwoColumns &&
        m_SearchFilter.GetState() == SearchFilter.State.EmptySearchFilter) {
      SelectAssetsFolder();
    }
  }
  ...
  private void InitOneColumnView() {
    ...
    m_AssetTree.contextClickOutsideItemsCallback +=
        AssetTreeViewContextClickOutsideItems;
    ...
    m_AssetTree.Init(m_TreeViewRect, data, new AssetsTreeViewGUI(m_AssetTree),
  ...
  void InitViewMode(ViewMode viewMode) {
    ...
    float minWidth = (m_ViewMode == ViewMode.OneColumn) ? k_MinWidthOneColumn
                                                        : k_MinWidthTwoColumns;
    // if (position.width < minWidth)
    //  Debug.LogError ("ProjectBrowser: Ensure that mode cannot be changed
    //  resulting in an invalid min width: " + minWidth + " (cur width " +
    //  position.width + ")");
  ...
  void SetViewMode(ViewMode newViewMode) {
    ...
    EndRenaming();
    InitViewMode(m_ViewMode == ViewMode.OneColumn ? ViewMode.TwoColumns
                 : ViewMode.OneColumn);
  ...
  string[] GetTypesDisplayNames() {
    return new[]{
        "AnimationClip", "AudioClip",      "AudioMixer", "ComputeShader",
        "Font",          "GUISkin",        "Material",   "Mesh",
        "Model",         "PhysicMaterial", "Prefab",     "Scene",
        "Script",        "Shader",         "Sprite",     "Texture",
        "VideoClip",
  ...
  public void TypeListCallback(PopupList.ListElement element) {
    ...
    string[] selectedDisplayNames =
        (from item in m_ObjectTypes.m_ListElements where
             item.selected select item.text)
            .ToArray();
  ...
  public void AssetLabelListCallback(PopupList.ListElement element) {
    ...
    // Toggle clicked element
    element.selected = !element.selected;
    m_SearchFilter.assetLabels =
        (from item in m_AssetLabels.m_ListElements where
             item.selected select item.text)
            .ToArray();
  ...
  void SetupDroplists() {
    ...
    for (int i = 0; i < types.Length; ++i) {
      PopupList.ListElement element =
          m_ObjectTypes.NewOrMatchingElement(types[i]);
      if (i == 0)
  ...
  void SetupAssetLabelList() {
    // List of predefined asset labels (and in the future also user defined
    // ones)
    ...
    foreach (var pair in tags) {
      PopupList.ListElement element =
          m_AssetLabels.NewOrMatchingElement(pair.Key);
  ...
  void ShowFolderContents(int folderInstanceID,
                          bool revealAndFrameInFolderTree) {
    if (m_ViewMode != ViewMode.TwoColumns)
      Debug.LogError(
          "ShowFolderContents should only be called in two column mode");
    ...
    if (!m_SkipHiddenPackages ||
        PackageManagerUtilityInternal.IsPathInVisiblePackage(folderPath)) {
  ...
  bool IsShowingFolderContents() { return m_SearchFilter.folders.Length > 0; }
  ...
  void ListAreaKeyboardCallback() {
    if (Event.current.type == EventType.KeyDown) {
      ...
      case KeyCode.UpArrow:
        if (Application.platform == RuntimePlatform.OSXEditor &&
            Event.current.command && m_ViewMode == ViewMode.TwoColumns) {
      ...
      case KeyCode.Backspace:
        if (Application.platform != RuntimePlatform.OSXEditor &&
            m_ViewMode == ViewMode.TwoColumns) {
          ShowParentFolderOfCurrentlySelected();
          Event.current.Use();
        }
        break;
    ...
    if (IsShowingFolderContents()) {
      ...
      if (selectedFolderInstanceIDs.Length == 1) {
        ...
        if (item != null && item.parent != null &&
            item.id != AssetDatabase.GetMainAssetOrInProgressProxyInstanceID(
                           "Assets")) {
  ...
  void OpenListAreaSelection() {
    ...
    if (selectionCount > 0) {
      ...
      if (allFolders) {
        OpenSelectedFolders();
        GUIUtility
            .ExitGUI(); // Exit because if we are mouse clicking to open we are
                        // in the middle of iterating list area items..
      } else {
  ...
  void RefreshSelectedPath() {
    if (Selection.activeObject != null) {
      ...
      if (!string.IsNullOrEmpty(m_SelectedPath) &&
          IsInsideHiddenPackage(m_SelectedPath)) {
        m_SelectedPath = string.Empty;
        Selection.activeObject = null;
      }
    }

    // By clearing we auto refresh it when needed (in an OnGUI code path because
    // we need Styles)
  ...
  void ListAreaItemSelectedCallback(bool doubleClicked) {
    ...
    if (instanceIDs.Length > 0) {
      ...
      m_SearchFilter.searchArea =
          m_LastLocalAssetsSearchArea; // local asset was selected
  ...
  void OnSelectionChange() {
    // We do not want to init our UI on OnSelectionChange because EditorStyles
    // might not be allocated yet (at play/stop)
    if (m_ListArea == null)
      return;

    // Keep for debugging
    // Debug.Log ("OnSelectionChange (ProjectBrowser): " +
    // DebugUtils.ListToString(new List<int>(Selection.instanceIDs)));

    // The list area selection state is based on the main selection (both in
    // search mode and folderbrowsing)
    m_ListArea.InitSelection(Selection.instanceIDs);

    int instanceID =
        Selection.instanceIDs.Length > 0
            ? Selection.instanceIDs[Selection.instanceIDs.Length - 1]
            : 0;
    if (m_ViewMode == ViewMode.OneColumn) {
      // If searching we are not showing the asset tree but we set selection
      // anyways to ensure its setup when clearing search
      bool revealSelectionAndFrameLast = !m_LockTracker.isLocked &&
                                         ShouldFrameAsset(instanceID) &&
                                         Selection.instanceIDs.Length <= 1;
      m_AssetTree.SetSelection(Selection.instanceIDs,
                               revealSelectionAndFrameLast);
    } else if (m_ViewMode == ViewMode.TwoColumns) {
      if (!m_InternalSelectionChange) {
        bool frame = !m_LockTracker.isLocked &&
                     Selection.instanceIDs.Length > 0 &&
                     ShouldFrameAsset(instanceID);
        if (frame) {
          // If searching we keep the search when framing. If folder browsing we
          // change folder and frame current selection in its folder
          if (m_SearchFilter.IsSearching()) {
    ...
    if (Selection.activeObject != null &&
        Selection.activeObject.GetType() != typeof(AssetStoreAssetInspector)) {
  ...
  internal void SetFolderSelection(int[] selectedInstanceIDs,
                                   bool revealSelectionAndFrameLastSelected) {
    m_FolderTree.SetSelection(selectedInstanceIDs,
  ...
  void SetSearchFoldersFromCurrentSelection() {
    ...
    // Set them as folders in search filter (so search in folder works
    // correctly)
    m_SearchFilter.folders =
        ProjectWindowUtil.GetBaseFolders(folders.ToArray());

    // Keep for debugging
    // Debug.Log ("Search folders: " + DebugUtils.ListToString(new
    // List<string>(m_SearchFilter.folders)));
  }

  void SetSearchFolderFromFolderTreeSelection() {
    if (m_FolderTree != null)
      m_SearchFilter.folders =
          GetFolderPathsFromInstanceIDs(m_FolderTree.GetSelection());
  }
  ...
  void FolderTreeSelectionCallback(int[] selectedTreeViewInstanceIDs) {
    ...
    if (firstTreeViewInstanceID != 0) {
      ...
      if (type == ItemType.SavedFilter) {
        SearchFilter filter =
            SavedSearchFilters.GetFilter(firstTreeViewInstanceID);

        // Check if the filter is valid (the root of filters are not an actual
        // filter)
        if (ValidateFilter(firstTreeViewInstanceID, filter)) {
          m_SearchFilter = filter;
          EnsureValidFolders();
          float previewSize =
              filter.GetState() == SearchFilter.State
                                       .FolderBrowsing ? m_LastFoldersGridSize
              : SavedSearchFilters.GetPreviewSize(firstTreeViewInstanceID);
          if (previewSize > 0f)
            m_ListArea.gridSize =
                Mathf.Clamp((int) previewSize, m_ListArea.minGridSize,
                            m_ListArea.maxGridSize);
          SyncFilterGUI();
        }
      }
  ...
  bool ValidateFilter(int savedFilterID, SearchFilter filter) {
    ...
    if (state == SearchFilter.State.FolderBrowsing ||
        state == SearchFilter.State.SearchingInFolders) {
        ...
        if (instanceID == 0) {
          if (EditorUtility.DisplayDialog(
                  ...
                  "The folder '" + folderPath +
                      "' might have been deleted or belong to another project. Do you want to delete the favorite?",
                  "Delete", "Cancel")) {
            SavedSearchFilters.RemoveSavedFilter(savedFilterID);
            GUIUtility.ExitGUI(); // exit gui since we are iterating items we
                                  // just reloaded
          }
    ...
    if (m_ViewMode == ViewMode.TwoColumns && m_FolderTree != null) {
      ...
      if (selection.Length > 0) {
        isSavedFilterSelected =
            GetItemType(selection[0]) == ItemType.SavedFilter;
      }
      ...
      case SearchViewState.NotSearching: {
        if (!isSavedFilterSelected)
          m_FolderTree.SetSelection(
              GetFolderInstanceIDs(m_SearchFilter.folders), true);
      } break;
      }
    }
  }
  ...
  static internal int[] DuplicateFolders(int[] instanceIDs) {
    ...
    bool failed = false;
    int asssetsFolderInstanceID =
        AssetDatabase.GetMainAssetOrInProgressProxyInstanceID("Assets");
    ...
    foreach (int instanceID in instanceIDs) {
      ...
      string assetPath = AssetDatabase.GetAssetPath(
          InternalEditorUtility.GetObjectFromInstanceID(instanceID));
    ...
    for (int i = 0; i < copiedPaths.Count; i++) {
      copiedAssets[i] =
          AssetDatabase.LoadMainAssetAtPath(copiedPaths[i]).GetInstanceID();
    }
  ...
  static void DeleteFilter(int filterInstanceID) {
    if (SavedSearchFilters.GetRootInstanceID() == filterInstanceID) {
      ...
      EditorUtility.DisplayDialog(
          title, "Deleting the 'Filters' root is not allowed", "Ok");
    } else {
      string title = "Delete selected favorite?";
      if (EditorUtility.DisplayDialog(title, "You cannot undo this action.",
                                      "Delete", "Cancel")) {
  ...
  // Also called from C++ (used for AssetsMenu check if selection is Packages
  // folder)
  [UsedByNativeCode]
  internal static bool SelectionIsPackagesRootFolder() {
    ...
    if (pb.m_ViewMode == ViewMode.OneColumn && pb.m_AssetTree != null &&
        pb.m_AssetTree.IsSelected(kPackagesFolderInstanceId)) {
      return true;
    }

    if (pb.m_ViewMode == ViewMode.TwoColumns &&
        (pb.useTreeViewSelectionInsteadOfMainSelection ||
  ...
  void HandleCommandEventsForTreeView() {
    ...
    if (eventType == EventType.ExecuteCommand ||
      ...
      // Only one type can be selected at a time (and savedfilters can only be
      // single-selected)
      ...
      if (Event.current.commandName == EventCommandNames.Delete ||
          Event.current.commandName == EventCommandNames.SoftDelete) {
        Event.current.Use();
        if (execute) {
          if (itemType == ItemType.SavedFilter) {
            System.Diagnostics.Debug.Assert(
                instanceIDs.Length ==
                1); // We do not support multiselection for filters
            ...
            if (askIfSure)
              Focus(); // Workaround that we do not get focus back when dialog
                       // is closed
          }
        }
        GUIUtility.ExitGUI();
    ...
    if (eventType == EventType.ExecuteCommand ||
      ...
      if (Event.current.commandName == EventCommandNames.Delete ||
        ...
        if (execute) {
          ...
          DeleteSelectedAssets(askIfSure);
          if (askIfSure)
            Focus(); // Workaround that we do not get focus back when dialog is
                     // closed
        }
        GUIUtility.ExitGUI();
      } else if (Event.current.commandName == EventCommandNames.Duplicate) {
  ...
  void RefreshSplittedSelectedPath() {
    ...
    if (String.IsNullOrEmpty(m_SelectedPath)) {
      ...
      if (m_SearchFilter.GetState() == SearchFilter.State.FolderBrowsing) {
        m_SelectedPathSplitted.Add(new GUIContent(
            displayPath, AssetDatabase.GetCachedIcon(m_SelectedPath)));
      } else {
        float availableWidth =
            position.width - m_DirectoriesAreaWidth - k_SliderWidth - 16f;
        Vector2 stringSize =
            s_Styles.selectedPathLabel.CalcSize(GUIContent.Temp(displayPath));
  ...
  float GetListHeaderHeight() {
    return m_SearchFilter.GetState() == SearchFilter.State.EmptySearchFilter ? 0f
        : k_ToolbarHeight;
  }
  ...
  void CalculateRects() {
    ...
    if (m_ViewMode == ViewMode.OneColumn) {
      m_ListAreaRect = new Rect(
          0, EditorGUI.kWindowToolbarHeight + listHeaderHeight, position.width,
          position.height - k_ToolbarHeight - listHeaderHeight -
              bottomBarHeight);
      m_TreeViewRect =
          new Rect(0, EditorGUI.kWindowToolbarHeight, position.width,
                   position.height - k_ToolbarHeight - bottomBarHeight);
      m_BottomBarRect = new Rect(0, position.height - bottomBarHeight,
                                 position.width, bottomBarHeight);
      m_ListHeaderRect = new Rect(0, EditorGUI.kWindowToolbarHeight,
                                  position.width, listHeaderHeight);
    } else // if (m_ViewMode == ViewMode.TwoColumns)
    {
      float listWidth = position.width - m_DirectoriesAreaWidth;

      m_ListAreaRect =
          new Rect(m_DirectoriesAreaWidth,
                   EditorGUI.kWindowToolbarHeight + listHeaderHeight, listWidth,
                   position.height - k_ToolbarHeight - listHeaderHeight -
                       bottomBarHeight);
      m_TreeViewRect =
          new Rect(0, EditorGUI.kWindowToolbarHeight, m_DirectoriesAreaWidth,
                   position.height - k_ToolbarHeight);
      m_BottomBarRect =
          new Rect(m_DirectoriesAreaWidth, position.height - bottomBarHeight,
                   listWidth, bottomBarHeight);
      m_ListHeaderRect =
          new Rect(m_ListAreaRect.x, EditorGUI.kWindowToolbarHeight,
                   m_ListAreaRect.width, listHeaderHeight);
    }
  }
  ...
  void OnGUI() {
    ...
    // We grab keyboard control ids early to ensure consistency (for focus
    // rendering)
    ...
    if (evt.type == EventType.MouseDown &&
        ProjectBrowserRect.Contains(evt.mousePosition)) {
      EndPing();
      SetAsLastInteractedProjectBrowser();
    }
      ...
      if (m_SearchFilter.IsSearching()) {
        ...
        if (GUIUtility.keyboardControl == m_TreeViewKeyboardControlID)
          GUIUtility.keyboardControl =
              m_ListKeyboardControlID; // AssetTree is not shown so we can steal
                                       // keyboard control
        m_ListArea.OnGUI(m_ListAreaRect, m_ListKeyboardControlID);
      } else {
        if (GUIUtility.keyboardControl == m_ListKeyboardControlID)
          GUIUtility.keyboardControl =
              m_TreeViewKeyboardControlID; // List is not shown so we can steal
                                           // keyboard control
    ...
    } else // ViewMode.TwoColumns
    {
      ...
      // Vertical splitter line between folders and content (drawn before
      // listarea so listarea ping is drawn on top of line)
      EditorGUIUtility.DrawHorizontalSplitter(
          new Rect(m_ListAreaRect.x + 1f, EditorGUI.kWindowToolbarHeight, 1,
                   m_TreeViewRect.height));

      if (m_SearchFilter.GetState() == SearchFilter.State.FolderBrowsing &&
          m_ListArea.numItemsDisplayed == 0) {
        Vector2 size = EditorStyles.label.CalcSize(s_Styles.m_EmptyFolderText);
        Rect textRect =
            new Rect(m_ListAreaRect.x + 2f +
                         Mathf.Max(0, (m_ListAreaRect.width - size.x) * 0.5f),
                     m_ListAreaRect.y + 10f, size.x, 20f);
    ...
    if (m_ViewMode == ViewMode.TwoColumns)
      useTreeViewSelectionInsteadOfMainSelection =
          GUIUtility.keyboardControl == m_TreeViewKeyboardControlID;

    // Handle command events AFTER tree and list view since commands events
    // should be handled by text fields first (rename overlay + search field)
    // Let folder/filters tree view try to handle command events first if it has
    // keyboard focus
    if (m_ViewMode == ViewMode.TwoColumns &&
        GUIUtility.keyboardControl == m_TreeViewKeyboardControlID)
  ...
  void HandleContextClickInListArea(Rect listRect) {
    ...
    case EventType.MouseDown:
      // This section handles selecting the folders showing their content, if
      // right clicked outside items. We do this to ensure our assets context
      // menu is operating on the active folder(s)
      if (m_ViewMode == ViewMode.TwoColumns &&
          m_SearchFilter.GetState() == SearchFilter.State.FolderBrowsing &&
          evt.button == 1 && !m_ItemSelectedByRightClickThisEvent) {
        if (m_SearchFilter.folders.Length > 0 &&
    ...
    case EventType.ContextClick:
      if (listRect.Contains(evt.mousePosition)) {
        ...
        // Context click in list area (can be an asset store asset or a local
        // asset)
  ...
  void AssetTreeViewContextClick(int clickedItemID) {
    ...
    EditorUtility.DisplayPopupMenu(
        new Rect(evt.mousePosition.x, evt.mousePosition.y, 0, 0), "Assets/",
        null);
  ...
  static bool IsFolderTreeViewContextClick() {
    ...
    if (projectBrowser == null) {
      return true; // return true to ignore Context menu's 'Open' option in
                   // folder tree in the ProjectBrowser
  ...
  void AssetTreeDragEnded(int[] draggedInstanceIds,
                          bool draggedItemsFromOwnTreeView) {
    // We only change selection if 'draggedItemsFromOwnTreeView' == true.
    // This ensures that we do not override the selection that might have been
    // set before calling this callback when dragging e.g a gameobject to the
    // projectbrowser (case 628939)
    if (draggedInstanceIds != null && draggedItemsFromOwnTreeView) {
      m_AssetTree.SetSelection(draggedInstanceIds, true);
      m_AssetTree
          .NotifyListenersThatSelectionChanged(); // behave as if selection was
                                                  // performed in treeview
      Repaint();
      GUIUtility.ExitGUI();
    }
  }

  void FolderTreeDragEnded(int[] draggedInstanceIds,
                           bool draggedItemsFromOwnTreeView) {
    // In the folder tree we do not want to change selection as we do in the
    // full asset tree when dragging is performed. We do not want to change
    // folder when dragging assets to folders (convention of both OSX and Win)
  }
  ...
  void TopToolbar() {
    GUILayout.BeginHorizontal(EditorStyles.toolbar);
    {
      ...
      bool compactMode =
          listWidth < 500; // We need quite some space for filtering text
      if (!compactMode) {
        spaceBetween = 10f;
      }
  ...
  internal bool IsTwoColumns() { return m_ViewMode == ViewMode.TwoColumns; }

  void OpenTreeViewTestWindow() { GetWindow<TreeViewTestWindow>(); }

  void ToggleExpansionAnimationPreference() {
    bool oldValue = EditorPrefs.GetBool(
        TreeViewController.kExpansionAnimationPrefKey, false);
    EditorPrefs.SetBool(TreeViewController.kExpansionAnimationPrefKey,
                        !oldValue);
  ...
  public virtual void AddItemsToMenu(GenericMenu menu) {
    if (m_EnableOldAssetTree) {
      GUIContent assetTreeText =
          EditorGUIUtility.TrTextContent("One Column Layout");
      GUIContent assetBrowserText =
          EditorGUIUtility.TrTextContent("Two Column Layout");

      menu.AddItem(assetTreeText, m_ViewMode == ViewMode.OneColumn,
                   SetOneColumn);
      if (position.width >= k_MinWidthTwoColumns)
        menu.AddItem(assetBrowserText, m_ViewMode == ViewMode.TwoColumns,
                     SetTwoColumns);
      else
      ...
      if (Unsupported.IsDeveloperMode()) {
        menu.AddItem(EditorGUIUtility.TrTextContent(
                         "DEVELOPER/Open TreeView Test Window..."),
                     false, OpenTreeViewTestWindow);
        menu.AddItem(EditorGUIUtility.TrTextContent(
                         "DEVELOPER/Use TreeView Expansion Animation"),
                     EditorPrefs.GetBool(
                         TreeViewController.kExpansionAnimationPrefKey, false),
                     ToggleExpansionAnimationPreference);
      }
    }
  }

  float DrawLocalAssetHeader(Rect r) { return 0; }
  ...
  void ResizeHandling(float width, float height) {
    ...
    Rect dragRect =
        new Rect(m_DirectoriesAreaWidth, EditorGUI.kWindowToolbarHeight,
                 k_ResizerWidth, height);
    dragRect = EditorGUIUtility.HandleHorizontalSplitter(
        dragRect, position.width, k_MinDirectoriesAreaWidth,
        k_MinWidthTwoColumns - k_MinDirectoriesAreaWidth);
  ...
  void ButtonSaveFilter() {
    ...
    using(new EditorGUI.DisabledScope(!m_SearchFilter.IsSearching())) {
      if (GUILayout.Button(s_Styles.m_SaveFilterContent,
                           EditorStyles.toolbarButtonRight)) {
        ProjectBrowserColumnOneTreeViewGUI ProjectBrowserTreeViewGUI =
            m_FolderTree.gui as ProjectBrowserColumnOneTreeViewGUI;
        if (ProjectBrowserTreeViewGUI != null) {
          ...
          if (treeViewSelection.Length == 1) {
            ...
            bool isRootFilter =
                SavedSearchFilters.GetRootInstanceID() == instanceID;
            ...
            if (SavedSearchFilters.IsSavedFilter(instanceID) && !isRootFilter) {
              ...
              string text = "Do you want to overwrite '" +
                            SavedSearchFilters.GetName(instanceID) +
                            "' or create a new filter?";
              int result = 2; // cancel
              result = EditorUtility.DisplayDialogComplex(
                  title, text, "Overwrite", "Create", "Cancel");
              if (result == 0)
                SavedSearchFilters.UpdateExistingSavedFilter(
                    instanceID, m_SearchFilter, listAreaGridSize);
              else if (result == 1)
          ...
          if (createNewFilter) {
            // User wants to create new filter. We re-focus to ensure rename
            // overlay gets input (dialog stole our focus and we might not get
            // it back)
            Focus();
  ...
  void CreateDropdown() {
    ...
    Rect r = GUILayoutUtility.GetRect(s_Styles.m_CreateDropdownContent,
                                      EditorStyles.toolbarCreateAddNewDropDown);
    if (EditorGUI.DropdownButton(r, s_Styles.m_CreateDropdownContent,
                                 FocusType.Passive,
                                 EditorStyles.toolbarCreateAddNewDropDown)) {
      GUIUtility.hotControl = 0;
  ...
  void AssetLabelsDropDown() {
    // Labels button
    Rect r = GUILayoutUtility.GetRect(s_Styles.m_FilterByLabel,
                                      EditorStyles.toolbarButton);
    if (EditorGUI.DropdownButton(r, s_Styles.m_FilterByLabel, FocusType.Passive,
                                 EditorStyles.toolbarButton)) {
  ...
  void TypeDropDown() {
    // Object type button
    Rect r = GUILayoutUtility.GetRect(s_Styles.m_FilterByType,
                                      EditorStyles.toolbarButton);
    if (EditorGUI.DropdownButton(r, s_Styles.m_FilterByType, FocusType.Passive,
                                 EditorStyles.toolbarButton)) {
  ...
  private void ToggleHiddenPackagesVisibility() {
    s_Styles.m_PackagesVisibilityContent.text =
        PackageManagerUtilityInternal.HiddenPackagesCount.ToString();
    var skipHiddenPackage = GUILayout.Toggle(
        m_SkipHiddenPackages, s_Styles.m_PackagesVisibilityContent,
        EditorStyles.toolbarButtonRight);
    if (skipHiddenPackage != m_SkipHiddenPackages) {
      ...
      if (m_FolderTree != null) {
        var dataSource =
            m_FolderTree.data as ProjectBrowserColumnOneTreeViewDataSource;
        dataSource.skipHiddenPackages = m_SkipHiddenPackages;
      }
  ...
  void SearchField() {
    Rect rect = GUILayoutUtility.GetRect(
        0, EditorGUILayout.kLabelFloatMaxW * 1.5f, EditorGUI.kSingleLineHeight,
        EditorGUI.kSingleLineHeight, EditorStyles.toolbarSearchField,
        GUILayout.MinWidth(65), GUILayout.MaxWidth(300));
    int searchFieldControlID = EditorGUIUtility.GetControlID(
        s_HashForSearchField, FocusType.Passive,
        rect); // We use 'Passive' to ensure we only tab between folder tree and
               // list area. Focus search field by using Ctrl+F.
    ...
    if (evt.type == EventType.KeyDown &&
        (evt.keyCode == KeyCode.DownArrow || evt.keyCode == KeyCode.UpArrow)) {
    ...
    m_lastSearchFilter = EditorGUI.ToolbarSearchField(
        searchFieldControlID, rect, m_SearchFieldText, false);

    if (m_lastSearchFilter != m_SearchFieldText || m_FocusSearchField) {
      // Update filter with string
      m_SearchFieldText = m_lastSearchFilter;

      m_NextSearch = EditorApplication.timeSinceStartup +
                     SearchableEditorWindow.k_SearchTimerDelaySecs;
    }
  }
    ...
    if (!m_SearchFilter.IsSearching()) {
      if (m_DidSelectSearchResult) {
        ...
        if (GUIUtility.keyboardControl == 0) {
          // Ensure item has focus for visual feedback and instant key
          // navigation
          if (m_ViewMode == ViewMode.OneColumn)
            GUIUtility.keyboardControl = m_TreeViewKeyboardControlID;
          else if (m_ViewMode == ViewMode.TwoColumns)
        ...
        if (GUIUtility.keyboardControl == 0 && m_LastFolders != null &&
            m_LastFolders.Length > 0) {
  ...
  internal static int GetFolderInstanceID(string folderPath) {
    return folderPath == PackageManager.Folders.GetPackagesPath()
               ...
               : AssetDatabase.GetMainAssetOrInProgressProxyInstanceID(
                     folderPath);
  }
  ...
  void FolderTreeSelectionChanged(bool folderWasSelected) {
    if (folderWasSelected) {
      SearchViewState state = GetSearchViewState();
      if (state == SearchViewState.AllAssets ||
          state == SearchViewState.InAssetsOnly ||
          state == SearchViewState.InPackagesOnly ||
          state == SearchViewState.AssetStore) {
        // Clear all except folders if folder is set
        string[] folders = m_SearchFilter.folders;
        ClearSearch();
        m_SearchFilter.folders = folders;

        // Ensures that when selecting folders we start local asset search next
        // time we search
  ...
  void IconSizeSlider(Rect r) {
    ...
    int newGridSize = (int) GUI.HorizontalSlider(
        r, m_ListArea.gridSize, m_ListArea.minGridSize, m_ListArea.maxGridSize);
  ...
  void BreadCrumbBar() {
    ...
    if (evt.type == EventType.MouseDown &&
        m_ListHeaderRect.Contains(evt.mousePosition)) {
      GUIUtility.keyboardControl = m_ListKeyboardControlID;
      Repaint();
    }
      ...
      if (IsInsideHiddenPackage(path)) {
        ...
        if (packageInfo != null) {
          ...
          if (path != packageInfo.assetPath) {
            var subpaths =
                Regex.Replace(path, @"^" + packageInfo.assetPath + "/", "")
                    .Split('/');
        ...
        foreach (var folderName in folderNames) {
          ...
          m_BreadCrumbs.Add(new KeyValuePair<GUIContent, string>(
              new GUIContent(folderDisplayNames[i++]), folderPath));
        }

        if (path == packagesMountPoint) {
          m_BreadCrumbLastFolderHasSubFolders =
              PackageManagerUtilityInternal
                  .GetAllVisiblePackages(m_SkipHiddenPackages)
                  .Length > 0;
        }
      }
    }
    ...
    if (m_SearchFilter.folders.Length == 1) {
      for (int i = 0; i < m_BreadCrumbs.Count; ++i) {
        ...
        GUIStyle style = lastElement ? EditorStyles.boldLabel
            : EditorStyles.label; // EditorStyles.miniBoldLabel :
        ...
        if (!lastElement || m_BreadCrumbLastFolderHasSubFolders) {
          Rect buttonRect = new Rect(
              rect.x,
              rect.y + (rect.height - s_Styles.separator.fixedHeight) / 2,
              s_Styles.separator.fixedWidth, s_Styles.separator.fixedHeight);
          if (EditorGUI.DropdownButton(buttonRect, GUIContent.none,
                                       FocusType.Passive, s_Styles.separator)) {
            string currentSubFolder = "";
            if (!lastElement)
              currentSubFolder = m_BreadCrumbs[i + 1].Value;
            BreadCrumbListMenu.Show(folderPath, currentSubFolder, buttonRect,
                                    this);
          }
        }
      ...
      GUI.Label(rect, GUIContent.Temp("Showing multiple folders..."),
                EditorStyles.miniLabel);
    }
  }
  ...
  void BottomBar() {
    ...
    Rect sliderRect =
        new Rect(rect.x + rect.width - k_SliderWidth - 16f,
                 rect.y + rect.height -
                     (m_BottomBarRect.height + EditorGUI.kSingleLineHeight) / 2,
                 k_SliderWidth, m_BottomBarRect.height);
    IconSizeSlider(sliderRect);

    // File path
    EditorGUIUtility.SetIconSize(new Vector2(
        16,
        16)); // If not set we see icons scaling down if text is being cropped
  ...
  public void SelectAssetsFolder() {
    ShowFolderContents(
        AssetDatabase.GetMainAssetOrInProgressProxyInstanceID("Assets"), true);
  }

  string ValidateCreateNewAssetPath(string pathName) {
    // Normally we create assets relative to current selected asset. If no asset
    // is selected we normally create the asset in the root folder (Assets) but
    // in two column mode we want to create the asset in the currently shown
    // folder if no asset is selected. (fix case 550484)
    if (m_ViewMode == ViewMode.TwoColumns &&
        m_SearchFilter.GetState() == SearchFilter.State.FolderBrowsing &&
        m_SearchFilter.folders.Length > 0) {
      ...
      if (!pathName.StartsWith("assets/",
        ...
        if (Selection.GetFiltered(typeof(Object), SelectionMode.Assets)
                .Length == 0) {
  ...
  internal void BeginPreimportedNameEditing(int instanceID,
                                            EndNameEditAction endAction,
                                            string pathName, Texture2D icon,
                                            string resourceFile) {
    if (!Initialized())
      Init();

    // End any rename that might be active, this will also end any asset
    ...
    if (m_ViewMode == ViewMode.TwoColumns) {
      if (m_SearchFilter.GetState() != SearchFilter.State.FolderBrowsing) {
        // Force select the Assets folder as default when no folders are
        // selected
        SelectAssetsFolder();
      }
      ...
      if (m_ListAreaState.m_CreateAssetUtility.BeginNewAssetCreation(
              instanceID, endAction, pathName, icon, resourceFile)) {
        ShowFolderContents(
            AssetDatabase.GetMainAssetOrInProgressProxyInstanceID(
                m_ListAreaState.m_CreateAssetUtility.folder),
            true);
        m_ListArea.BeginNamingNewAsset(
            m_ListAreaState.m_CreateAssetUtility.originalName, instanceID,
            isCreatingNewFolder);
      }
    } else if (m_ViewMode == ViewMode.OneColumn) {
      // If search is active put new asset under selected folder otherwise in
      // Assets
      if (m_SearchFilter.IsSearching()) {
        ClearSearch();
      }
      ...
      // Create in tree
      AssetsTreeViewGUI defaultTreeViewGUI =
          m_AssetTree.gui as AssetsTreeViewGUI;
      if (defaultTreeViewGUI != null)
        defaultTreeViewGUI.BeginCreateNewAsset(instanceID, endAction, pathName,
                                               icon, resourceFile);
      else
  ...
  public void FrameObject(int instanceID, bool ping) {
    bool frame =
        !m_LockTracker.isLocked && (ping || ShouldFrameAsset(instanceID));
  ...
  private void FrameObjectInTwoColumnMode(int instanceID, bool frame,
                                          bool ping) {
    ...
    if (instanceID == kPackagesFolderInstanceId)
      ...
      if (!String.IsNullOrEmpty(assetPath)) {
        string containingFolder =
            ProjectWindowUtil.GetContainingFolder(assetPath);
  ...
  internal static int[] GetTreeViewFolderSelection(
      bool forceUseTreeViewSelection = false) {
    // Since we can delete entire folder hierarchies with the returned selection
    // we need to be very careful. We therefore require the following:
    // - The folder/favorite tree view must have keyboard focus
    // Note we cannot require window focus (focusedWindow) since on OSX window
    // focus is lost to popup window when right clicking
    ProjectBrowser ob = s_LastInteractedProjectBrowser;

    if (ob != null &&
        (ob.useTreeViewSelectionInsteadOfMainSelection ||
         forceUseTreeViewSelection) &&
        ob.m_FolderTree != null) {
  ...
  internal static bool CanDeleteSelectedAssets() {
    ...
    var instanceIDs = treeViewSelection.Length > 0
                          ? new List<int>(treeViewSelection)
                          : new List<int>(Selection.instanceIDs);

    var objectsToDelete = new HashSet<int>();
    foreach (var instanceID in instanceIDs) {
      if (instanceID ==
              AssetDatabase.GetMainAssetOrInProgressProxyInstanceID("Assets") ||
          instanceID == kPackagesFolderInstanceId) {
        return false;
      }
      ...
      if (AssetDatabase.IsMainAsset(instanceID)) {
        ...
        if (string.IsNullOrEmpty(path) ||
            !AssetDatabase.GetAssetFolderInfo(path, out isRootFolder,
                                              out isImmutable) ||
            isImmutable) {
          return false;
        }
  ...
  internal static void DeleteSelectedAssets(bool askIfSure) {
    ...
    if (ProjectWindowUtil.DeleteAssets(instanceIDs, askIfSure)) {
      // Ensure selection is cleared since StopAssetEditing() will restore
      // selection from a backup saved in StartAssetEditing.
  ...
  internal static void RenameSelectedAssets() {
    ProjectBrowser ob = s_LastInteractedProjectBrowser;
    if (ob != null) {
      if (ob.useTreeViewSelectionInsteadOfMainSelection &&
          ob.m_FolderTree != null) {
        ob.m_FolderTree.BeginNameEditing(0f);
      } else if (ob.m_ViewMode == ViewMode.OneColumn &&
                 !ob.m_SearchFilter.IsSearching() && ob.m_AssetTree != null) {
  ...
  internal
      IHierarchyProperty GetHierarchyPropertyUsingFilter(string textFilter) {
    FilteredHierarchy filteredHierarchy =
        new FilteredHierarchy(HierarchyType.Assets);
    filteredHierarchy.searchFilter =
        SearchFilter.CreateSearchFilterFromString(textFilter);
    IHierarchyProperty property =
        FilteredHierarchyProperty.CreateHierarchyPropertyForFilter(
            filteredHierarchy);
    return property;
  }
  ...
  internal void ShowObjectsInList(int[] instanceIDs) {
    ...
    if (m_ViewMode == ViewMode.TwoColumns) {
      ...
      m_FolderTree.SetSelection(
          k_EmptySelection, false); // Remove selection from folder tree since
                                    // we show custom list (press F to focus)
    } else if (m_ViewMode == ViewMode.OneColumn) {
      foreach (int instanceID in Selection.instanceIDs)
  ...
  static void ShowSelectedObjectsInLastInteractedProjectBrowser() {
    // Only one ProjectBrowser can have focus at a time so if we find one just
    // return that one
    if (s_LastInteractedProjectBrowser != null) {
  ...
  internal class SavedFiltersContextMenu {
    ...
    static internal void Show(int savedFilterInstanceID) {
      ...
      menu.AddItem(delete, false,
                   new SavedFiltersContextMenu(savedFilterInstanceID).Delete);
    ...
    private void Delete() { DeleteFilter(m_SavedFilterInstanceID); }
  }
  ...
  internal class BreadCrumbListMenu {
    ...
    static internal void Show(string folder, string currentSubFolder,
                              Rect activatorRect, ProjectBrowser caller) {
      m_Caller = caller;
      ...
      if (folder == Folders.GetPackagesPath()) {
        foreach (var package in PackageManagerUtilityInternal
                     .GetAllVisiblePackages(caller.m_SkipHiddenPackages)) {
          subFolders.Add(package.assetPath);
          var displayName = !string.IsNullOrEmpty(package.displayName)
                                ? package.displayName
                                : package.name;
      ...
      if (subFolders.Count > 0) {
        var i = 0;
        foreach (var subFolderPath in subFolders) {
          menu.AddItem(new GUIContent(subFolderDisplayNames[i++]),
                       subFolderPath == currentSubFolder,
                       new BreadCrumbListMenu(subFolderPath).SelectSubFolder);
          menu.ShowAsContext();
        }
      } else {
        menu.AddDisabledItem(
            EditorGUIUtility.TrTextContent("No sub folders..."));
      }

      menu.DropDown(activatorRect);
    }

    private BreadCrumbListMenu(string subFolder) { m_SubFolder = subFolder; }

    private void SelectSubFolder() {
      int folderInstanceID =
          AssetDatabase.GetMainAssetOrInProgressProxyInstanceID(m_SubFolder);
      if (folderInstanceID != 0)
  ...
  internal class AssetStoreItemContextMenu {
    static internal void Show() {
      ...
      GUIContent assetStoreWindow =
          EditorGUIUtility.TrTextContent("Show in Asset Store window");
      AssetStoreAsset activeAsset = AssetStoreAssetSelection.GetFirstAsset();
      if (activeAsset != null && activeAsset.id != 0)
        menu.AddItem(assetStoreWindow, false,
                     new AssetStoreItemContextMenu().OpenAssetStoreWindow);
      else
