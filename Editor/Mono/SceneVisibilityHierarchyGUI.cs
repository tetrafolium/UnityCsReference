...
public static readonly IconState iconNormal = new IconState {
	...
	pickingEnabledAll =
		EditorGUIUtility.TrIconContent("scenepicking_pickable"),
	pickingEnabledMixed =
		EditorGUIUtility.TrIconContent("scenepicking_pickable-mixed"),
	pickingDisabledAll =
		EditorGUIUtility.TrIconContent("scenepicking_notpickable"),
	pickingDisabledMixed =
		EditorGUIUtility.TrIconContent("scenepicking_notpickable-mixed"),
};
...
public static readonly IconState iconHovered = new IconState {
	...
	hiddenMixed =
		EditorGUIUtility.TrIconContent("scenevis_hidden-mixed_hover"),
	pickingEnabledAll =
		EditorGUIUtility.TrIconContent("scenepicking_pickable_hover"),
	pickingEnabledMixed =
		EditorGUIUtility.TrIconContent("scenepicking_pickable-mixed_hover"),
	pickingDisabledAll =
		EditorGUIUtility.TrIconContent("scenepicking_notpickable_hover"),
	pickingDisabledMixed = EditorGUIUtility.TrIconContent(
...
public static readonly Color selectedBackgroundColor =
	EditorResources.GetStyle("game-object-tree-view-scene-visibility")
	.GetColor("-unity-object-selected-color");

public static readonly Color selectedNoFocusBackgroundColor =
	EditorResources.GetStyle("game-object-tree-view-scene-visibility")
	.GetColor("-unity-object-selected-no-focus-color");

public static readonly GUIStyle sceneVisibilityStyle = "SceneVisibility";

public static Color GetItemBackgroundColor(bool isHovered, bool isSelected,
                                           bool isFocused) {
	if (isSelected) {
		if (isFocused)
...
private static float k_sceneHeaderOverflow =>
GameObjectTreeViewGUI.GameObjectStyles.sceneHeaderBg.fixedHeight +
2 * GameObjectTreeViewGUI.GameObjectStyles.sceneHeaderWidth -
EditorGUIUtility.singleLineHeight;
private static bool m_PrevItemWasScene;

public const float utilityBarWidth =
	k_VisibilityIconPadding * 3 + k_IconWidth * 2;
...
public static void DoItemGUI(Rect rect, GameObjectTreeViewItem goItem,
                             bool isSelected, bool isHovered, bool isFocused,
                             bool isDragging) {
	...
	bool isIconHovered =
		!isDragging && iconRect.Contains(Event.current.mousePosition);

	Rect icon2Rect = rect;
	icon2Rect.xMin += 2 * k_VisibilityIconPadding + k_IconWidth;
	icon2Rect.width = k_IconWidth;
	bool isIcon2Hovered =
		!isDragging && icon2Rect.Contains(Event.current.mousePosition);
	...
	if (gameObject) {
		// The scene header overlaps it's next item by some pixels. Displace the
		// background so it doesn't draw on top of the scene header. Don't
		// displace when selected or hovered (They already show on top of the
		// header)
		...
		DrawGameObjectItemVisibility(iconRect, gameObject, isHovered,
		                             isIconHovered);
		DrawGameObjectItemPicking(icon2Rect, gameObject, isHovered,
		                          isIcon2Hovered);
...
private static void DrawItemBackground(Rect rect, bool isScene,
	...
	if (isScene) {
		if (isSelected) {
			TreeViewGUI.Styles.selectionStyle.Draw(rect, false, false, true,
			                                       isFocused);
		}
	} else {
		using (new GUI.BackgroundColorScope(
			       Styles.GetItemBackgroundColor(isHovered, isSelected, isFocused))) {
			GUI.Label(
				rect, GUIContent.none,
				GameObjectTreeViewGUI.GameObjectStyles.hoveredItemBackgroundStyle);
		}
...
private static void DrawGameObjectItemVisibility(Rect rect,
                                                 GameObject gameObject,
                                                 bool isItemHovered,
                                                 bool isIconHovered) {
	var isHidden = SceneVisibilityManager.instance.IsHidden(gameObject);
	bool shouldDisplayIcon = isItemHovered || isHidden;
	Styles.IconState iconState =
		isIconHovered ? Styles.iconHovered : Styles.iconNormal;

	GUIContent icon;
	if (isHidden) {
		icon = gameObject.transform.childCount == 0 ||
		       SceneVisibilityManager.instance.AreAllDescendantsHidden(
			gameObject)
		 ? iconState.hiddenAll
		 : iconState.hiddenMixed;
	} else if (!SceneVisibilityManager.instance.AreAllDescendantsVisible(
			   gameObject)) {
	...
	if (shouldDisplayIcon &&
	    GUI.Button(rect, icon, Styles.sceneVisibilityStyle)) {
		SceneVisibilityManager.instance.ToggleVisibility(gameObject,
		                                                 !Event.current.alt);
	}
}

private static void DrawGameObjectItemPicking(Rect rect,
                                              GameObject gameObject,
                                              bool isItemHovered,
                                              bool isIconHovered) {
	var isPickingDisabled =
		SceneVisibilityManager.instance.IsPickingDisabled(gameObject);
	...
	if (isPickingDisabled) {
		icon = gameObject.transform.childCount == 0 ||
		 ...
		 ? iconState.pickingDisabledAll
		 : iconState.pickingDisabledMixed;
	} else if (!SceneVisibilityManager.instance
	...
	if (shouldDisplayIcon &&
	    GUI.Button(rect, icon, Styles.sceneVisibilityStyle)) {
		SceneVisibilityManager.instance.TogglePicking(gameObject,
		                                              !Event.current.alt);
	}
}

private static void DrawSceneItemVisibility(Rect rect, Scene scene,
                                            bool isItemHovered,
                                            bool isIconHovered) {
...
private static void DrawSceneItemPicking(Rect rect, Scene scene,
                                         bool isItemHovered,
                                         bool isIconHovered) {
