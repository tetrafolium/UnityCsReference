...
namespace UnityEditor.Rendering {
  ...
  public struct ShaderCompilerData {
    ...
    public PlatformKeywordSet
        platformKeywordSet; // C++ side is PlatformCapKeywords
  ...
  public enum ShaderCompilerPlatform {
    ...
    D3D = 4,  // Direct3D 11 (FL10.0 and up) and Direct3D 12, compiled with MS
              // D3DCompiler
    GLES20 =
        5, // OpenGL ES 2.0 / WebGL 1.0, compiled with MS D3DCompiler + HLSLcc
    GLES3x =
        9, // OpenGL ES 3.0+ / WebGL 2.0, compiled with MS D3DCompiler + HLSLcc
    PS4 = 11,          // Sony PS4
  ...
  public enum ShaderCompilerMessageSeverity { Error, Warning }
  ...
  public enum ShaderRequirements {
    ...
    MRT4 = (1 << 3), // Multiple render targets, at least 4 (ability for
                     // fragment shader to output up to 4 colors)
    MRT8 = (1 << 4), // Multiple render targets, at least 8 (ability for
                     // fragment shader to output up to 8 colors)
    Derivatives =
        (1 << 5), // Derivative (ddx/ddy) instructions in the fragment shader
    SampleLOD = (1 << 6), // Ability to sample textures in fragment shader with
                          // explicit LOD level
    FragCoord = (1 << 7), // Pixel position (VPOS/SV_Position/gl_FragCoord)
                          // input in fragment shader
    //                          = (1 << 8), // unused
    Interpolators15Integers =
        (1 << 9), // Integers + Interpolators15. We bundle them together since
                  // extremely unlikely a GPU/API will ever exist that only has
                  // part of that.
    ...
    TessellationShaders = (1 << 17), // Tessellation shaders, i.e. DX11 style
                                     // (hull/domain shader stages)
    SparseTexelResident = (1 << 18), // Sparse textures with sampling
                                     // instructions that return residency info
    FramebufferFetch = (1 << 19),  // Framebuffer fetch (ability to have in+out
                                   // fragment shader color params)
    MSAATextureSamples = (1 << 20) // Access to MSAA'd texture samples in
                                   // shaders (e.g. HLSL Texture2DMS)
  }
