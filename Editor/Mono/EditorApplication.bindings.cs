...
public sealed partial class EditorApplication {
  ...
  private static extern void OpenProjectInternal(string projectPath,
                                                 string[] args);

  // Saves all serializable assets that have not yet been written to disk (eg.
  // Materials)
  [System.Obsolete(
      "Use AssetDatabase.SaveAssets instead (UnityUpgradable) -> AssetDatabase.SaveAssets()",
      true)]
  ...
  public static void EnterPlaymode() { isPlaying = true; }

  public static void ExitPlaymode() { isPlaying = false; }
  ...
  public static extern bool isPlayingOrWillChangePlaymode {
    [NativeMethod("IsPlayingOrWillEnterExitPlaymode")] get;
  }

  // Perform a single frame step.
  [StaticAccessor("GetApplication().GetPlayerLoopController()",
  ...
  public static extern bool isPaused {
    [NativeMethod("IsPaused")] get;
    [NativeMethod("SetPaused")] set;
  }
  ...
  public static extern bool isUpdating { get; }

  internal static extern bool isPreviewPackageInUse {
    [StaticAccessor("GetApplication()", StaticAccessorType.Dot)] get;
  }

  // Is remote connected to a client app?
  public static extern bool isRemoteConnected { get; }
  ...
  internal static extern bool useLibmonoBackendForIl2cpp {
    [NativeName("UseLibmonoBackendForIl2cpp")] get;
  }
  ...
  //  Must be called after LockReloadAssemblies, to reenable loading of
  //  assemblies.
  [StaticAccessor("GetApplication()", StaticAccessorType.Dot)]
  ...
  // Like ExecuteMenuItem, but applies action to specified GameObjects if the
  // menu action supports it.
  internal static extern bool
  ExecuteMenuItemOnGameObjects(string menuItemPath, GameObject[] objects);

  // Like ExecuteMenuItem, but applies action to specified GameObjects if the
  // menu action supports it.
  internal static extern bool
  ExecuteMenuItemWithTemporaryContext(string menuItemPath, Object[] objects);

  // Path to the Unity editor contents folder (RO)
  public static extern string applicationContentsPath {
    [FreeFunction("GetApplicationContentsPath", IsThreadSafe = true)] get;
  }
  ...
  internal static extern string userJavascriptPackagesPath { get; }

  public static extern bool isTemporaryProject {
    [FreeFunction("IsTemporaryProject")] get;
  }
  ...
  internal static extern Object tagManager { [FreeFunction] get; }

  internal static extern Object renderSettings { [FreeFunction] get; }

  // The time since the editor was started (RO)
  public static extern double timeSinceStartup { [FreeFunction] get; }
