...
public sealed partial class MaterialProperty {
  public delegate bool ApplyPropertyCallback(MaterialProperty prop,
                                             int changeMask,
  ...
  public void WriteToMaterialPropertyBlock(MaterialPropertyBlock materialblock,
                                           int changedPropertyMask) {
    ShaderUtil.ApplyMaterialPropertyToMaterialPropertyBlock(
        this, changedPropertyMask, materialblock);
  }
  ...
  public float floatValue {
    get {
      if (m_Type == ShaderPropertyType.Float ||
          m_Type == ShaderPropertyType.Range)
  ...
  private void ApplyProperty(object previousValue, int changedPropertyMask) {
    ...
    if (mats.Length == 1)
      ...
      targetTitle =
          mats.Length + " " +
          ObjectNames.NicifyVariableName(ObjectNames.GetClassName(mats[0])) +
          "s";
    ...
    if (m_ApplyPropertyCallback != null)
      didApply =
          m_ApplyPropertyCallback(this, changedPropertyMask, previousValue);

    if (!didApply)
      ShaderUtil.ApplyProperty(this, changedPropertyMask,
                               "Modify " + displayName + " of " + targetTitle);
  }
}
