...
class StyleDrawInspectView : BaseInspectView {
  ...
  [NonSerialized] List<IMGUIDrawInstruction> m_Instructions =
  ...
  public override void ShowOverlay() {
    if (m_CachedInstructionInfo != null)
      debuggerWindow.HighlightInstruction(debuggerWindow.inspected,
                                          m_Instruction.rect,
                                          m_Instruction.usedGUIStyle);
  }

  protected override int GetInstructionCount() { return m_Instructions.Count; }
  ...
  protected override void DoDrawInstruction(ListViewElement el, int id) {
    ...
    GUIViewDebuggerWindow.Styles.listItemBackground.Draw(
        el.position, false, false, listViewState.row == el.row, false);

    GUIViewDebuggerWindow.Styles.listItem.Draw(el.position, tempContent, id,
  ...
  protected override bool isInstructionSelected {
    get { return m_CachedInstructionInfo != null; }
  }
  ...
  DoDrawSelectedInstructionDetails(int selectedInstructionIndex) {
    ...
    DoSelectableInstructionDataField("VisibleRect",
                                     m_Instruction.visibleRect.ToString());
    EditorGUILayout.Space();

    EditorGUI.BeginChangeCheck();
    EditorGUILayout.PropertyField(
        m_CachedInstructionInfo.styleSerializedProperty,
        GUIContent.Temp("Style"), true);
    if (EditorGUI.EndChangeCheck()) {
      m_CachedInstructionInfo.styleContainerSerializedObject
    ...
    using(new EditorGUI.IndentLevelScope()) {
      DoSelectableInstructionDataField("Text",
                                       m_Instruction.usedGUIContent.text);
      DoSelectableInstructionDataField("Tooltip",
                                       m_Instruction.usedGUIContent.tooltip);
      using(new EditorGUI.DisabledScope(true)) EditorGUILayout.ObjectField(
          "Icon", m_Instruction.usedGUIContent.image, typeof(Texture2D), false);
    }
  }

  internal override string GetInstructionListName(int index) {
    // This means we will resolve the stack trace for all instructions.
    // TODO: make sure only visible items do this. Also, cache so we don't have
    // to do everyframe.
  ...
  internal override void OnSelectedInstructionChanged(int index) {
    ...
    if (listViewState.row >= 0 && index < m_Instructions.Count) {
      ...
      m_CachedInstructionInfo.styleContainer.inspectedStyle =
          m_Instruction.usedGUIStyle;
      ...
      GetSelectedStyleProperty(
          out m_CachedInstructionInfo.styleContainerSerializedObject,
          out m_CachedInstructionInfo.styleSerializedProperty);
  ...
  int GetInterestingFrameIndex(StackFrame[] stacktrace) {
    ...
    // If there is no frame inside the user project, we will return the first
    // frame outside any class starting with: UnityEngine.GUI or
    // UnityEditor.EditorGUI, this should include:
  ...
  void GetSelectedStyleProperty(out SerializedObject serializedObject,
                                out SerializedProperty styleProperty) {
    ...
    if (guiskin != null) {
      ...
      Debug.Log(string.Format(
          "Showing editable Style from GUISkin: {0}, IsPersistant: {1}",
          guiskin.name, EditorUtility.IsPersistent(guiskin)));
    }

    // Could not find the styles in the GUISkin.
    serializedObject =
        new SerializedObject(m_CachedInstructionInfo.styleContainer);
  ...
  void ShowInstructionInExternalEditor(StackFrame[] frames) {
    ...
    InternalEditorUtility.OpenFileAtLineExternal(frame.sourceFile,
                                                 (int) frame.lineNumber, -1);
  }
}
}
