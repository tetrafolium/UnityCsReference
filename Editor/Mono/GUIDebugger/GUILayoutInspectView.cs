...
class GUILayoutInspectView : BaseInspectView {
  ...
  readonly List<IMGUILayoutInstruction> m_LayoutInstructions =
  ...
  public override void ShowOverlay() {
    ...
    IMGUILayoutInstruction instruction =
        m_LayoutInstructions[listViewState.row];
    ...
    debuggerWindow.HighlightInstruction(debuggerWindow.inspected, rect,
                                        m_FakeMarginStyleForOverlay);
  }
  ...
  DoDrawSelectedInstructionDetails(int selectedInstructionIndex) {
    IMGUILayoutInstruction instruction =
        m_LayoutInstructions[selectedInstructionIndex];

    using(new EditorGUI.DisabledScope(true))
        DrawInspectedRect(instruction.unclippedRect);

    DoSelectableInstructionDataField("margin.left",
                                     instruction.marginLeft.ToString());
    DoSelectableInstructionDataField("margin.top",
                                     instruction.marginTop.ToString());
    DoSelectableInstructionDataField("margin.right",
                                     instruction.marginRight.ToString());
    DoSelectableInstructionDataField("margin.bottom",
                                     instruction.marginBottom.ToString());
    ...
    DoSelectableInstructionDataField("IsVertical",
                                     (instruction.isVertical == 1).ToString());
  }
  ...
  int GetInterestingFrameIndex(StackFrame[] stacktrace) {
    ...
    // frame outside any class starting with:
