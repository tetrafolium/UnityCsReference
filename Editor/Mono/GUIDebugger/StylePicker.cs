...
internal class StylePicker {
  ...
  public GUIStyle ExploredStyle {
    get;
    set;
  }
  public int ExploredDrawInstructionIndex {
    get;
    private set;
  }
  ...
  public Func<GUIView, bool> CanInspectView {
    get;
    set;
  }

  public StylePicker(EditorWindow window,
  ...
  public void OnGUI() {
    ...
    // NOTE: we can only StopDebugging and StartDebugging OUTSIDE of OnGUI or
    // else we crash (popclip/pushclip mismatch) Only keep the data necessary to
    // Start/Stop Debug in next Update tick.
    m_MouseCursorScreenPos =
  ...
  private void UpdateExploredGUIStyle(Vector2 screenPos) {
    ...
    if (mouseUnderView != m_ExploredView) {
      ...
      if (m_ExploredView) {
        ...
        // Since we have attached the debugger, this view hasn't logged its
        // repaint steps yet.
    ...
    if (m_ExploredView) {
      ...
      var localPosition =
          new Vector2(screenPos.x - mouseUnderView.screenPosition.x,
      ...
      /** Note: no perfect way to find the style under cursor:
          ...
          - by starting with the end, we hope to follow the "Last drawn
         instruction is the one on top"
          - Some styles are "transparent" and drawn last (TabWindowBackground
         and such)
      ...
      */
      ...
      for (var i = drawInstructions.Count - 1; i >= 0; --i) {
        ...
        if (instr.rect.Contains(localPosition) &&
          ...
          // Debug.Log(GetViewName(m_ExploredView) + " - Found Style: " +
          // instr.usedGUIStyle.name);
        }
      }

      if (Highlighter != null && mouseUnderStyle != null) {
        var visualElement =
            m_ExploredView.windowBackend.visualTree as VisualElement;
        if (visualElement != null) {
          // Debug.Log(GetViewName(m_ExploredView) + " - Highlight Style: " +
          // mouseUnderStyle.name);
          Highlighter.HighlightElement(visualElement, styleRect,
                                       mouseUnderStyle);
        }
      }
  ...
  private void FindStyleUnderMouse() {
    // Keep sending Repaint to the master view so it can update the mouse
    // position
    m_BoundWindow.Repaint();
  ...
  internal static GUIView GetViewUnderMouse(Vector2 mouseScreenPos,
                                            List<GUIView> views) {
