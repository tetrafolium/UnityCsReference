...
internal abstract class BaseInspectView : IBaseInspectView {
  protected static class Styles {
    public static readonly GUIContent instructionsLabel =
        EditorGUIUtility.TrTextContent("Instructions");
    public static readonly GUIContent emptyViewLabel =
        EditorGUIUtility.TrTextContent(
            "Select an Instruction on the left to see details");
  ...
  [NonSerialized] readonly ListViewState m_ListViewState = new ListViewState();
  ...
  readonly SplitterState m_InstructionDetailStacktraceSplitter =
      SplitterState.FromRelative(new float[]{80, 20}, new float[]{100, 100},
                                 null);
  ...
  public virtual void DrawInstructionList() {
    ...
    EditorGUILayout.BeginVertical(
        GUIViewDebuggerWindow.Styles.listBackgroundStyle);
    GUILayout.Label(Styles.instructionsLabel);

    int id = GUIUtility.GetControlID(FocusType.Keyboard);
    foreach (var element in ListViewGUI.ListView(
                 m_ListViewState,
                 GUIViewDebuggerWindow.Styles.listBackgroundStyle)) {
      ListViewElement listViewElement = (ListViewElement) element;
      if (evt.type == EventType.MouseDown && evt.button == 0 &&
          listViewElement.position.Contains(evt.mousePosition)) {
        if (evt.clickCount == 2) {
      ...
      if (evt.type == EventType.Repaint &&
          listViewElement.row < GetInstructionCount()) {
  ...
  public virtual void DrawSelectedInstructionDetails(float availableWidth) {
    ...
    m_InstructionDetailsScrollPos = EditorGUILayout.BeginScrollView(
        m_InstructionDetailsScrollPos, GUIViewDebuggerWindow.Styles.boxStyle);
  ...
  public virtual void SelectRow(int index) {
    if (index >= 0 && index < m_ListViewState.totalRows) {
      ...
      m_ListViewState.scrollPos = new Vector2(
          m_ListViewState.scrollPos.x,
          index * GUIViewDebuggerWindow.Styles.listItem.fixedHeight);
    }
  }
  ...
  protected virtual bool isInstructionSelected => m_ListViewState.row >= 0 &&
                                                  m_ListViewState.row <
                                                      GetInstructionCount();
  ...
  protected void DrawStackFrameList(StackFrame[] stackframes,
    ...
    if (stackframes != null) {
      ...
      foreach (var stackframe in stackframes) {
        ...
        var signature = stackframe.signature.Substring(
            0, cpos != -1 ? cpos : stackframe.signature.Length);

        callstack += string.Format(
            "{0} [<a href=\"{1}\" line=\"{2}\">{1}:{2}</a>]\n", signature,
            stackframe.sourceFile, stackframe.lineNumber);
      }

      float height = GUIViewDebuggerWindow.Styles.messageStyle.CalcHeight(
          GUIContent.Temp(callstack), availableWidth);
      EditorGUILayout.SelectableLabel(
          callstack, GUIViewDebuggerWindow.Styles.messageStyle,
  ...
  protected void DrawInspectedRect(Rect instructionRect) {
    ...
    var reserveTopFieldHeight = Mathf.CeilToInt(
        EditorGUI.kSingleLineHeight * 2 + EditorGUI.kControlVerticalSpacing);
    var reserveBottomFieldHeight = Mathf.CeilToInt(EditorGUI.kSingleLineHeight);
    var reserveFieldWidth = 100;
    var fieldsArea = new RectOffset(
        50, reserveFieldWidth, reserveTopFieldHeight, reserveBottomFieldHeight);
    ...
    GUI.CalculateScaledTextureRects(visualRect, ScaleMode.ScaleToFit,
                                    aspectRatio, ref aspectedRect, ref dummy);
    visualRect = aspectedRect;
    visualRect.width = Mathf.Max(80, visualRect.width);
    visualRect.height =
        Mathf.Max(EditorGUI.kSingleLineHeight + 10, visualRect.height);

    var startPointFieldRect = new Rect{height = EditorGUI.kSingleLineHeight,
                                       width = fieldsArea.left * 2,
                                       y = visualRect.y - fieldsArea.top};
    startPointFieldRect.x = visualRect.x - startPointFieldRect.width / 2f;

    var endPointFieldRect =
        new Rect{height = EditorGUI.kSingleLineHeight,
                 width = fieldsArea.right * 2, y = visualRect.yMax};
    ...
    var widthMarkersArea = new Rect{
        ...
        width = visualRect.width, height = EditorGUI.kSingleLineHeight};

    var widthFieldRect = widthMarkersArea;
    widthFieldRect.width = widthMarkersArea.width / 3;
    widthFieldRect.x = widthMarkersArea.x +
                       (widthMarkersArea.width - widthFieldRect.width) / 2f;
    ...
    heightFieldRect.y = heightFieldRect.y +
                        (heightMarkerArea.height - heightFieldRect.height) / 2f;

    // Draw TopLeft point
    GUI.Label(
        startPointFieldRect,
        UnityString.Format("({0},{1})", instructionRect.x, instructionRect.y),
    ...
    GUI.Label(widthFieldRect,
              instructionRect.width.ToString(CultureInfo.InvariantCulture),
              Styles.centeredLabel);
    ...
    GUI.Label(heightFieldRect,
              instructionRect.height.ToString(CultureInfo.InvariantCulture));

    GUI.Label(endPointFieldRect,
              UnityString.Format("({0},{1})", instructionRect.xMax,
                                 instructionRect.yMax),
              Styles.centeredLabel);
  ...
  protected void DoSelectableInstructionDataField(string label,
  ...
  void DoDrawNothingSelected() {
    ...
    GUILayout.Label(Styles.emptyViewLabel,
                    GUIViewDebuggerWindow.Styles.centeredText);
    GUILayout.FlexibleSpace();
    EditorGUILayout.EndVertical();
  }
}
