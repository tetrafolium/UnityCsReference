...
class UnifiedInspectView : BaseInspectView {
  ...
  public UnifiedInspectView(GUIViewDebuggerWindow guiViewDebuggerWindow)
    ...
    m_InstructionPropertyView =
        new GUIPropertyInspectView(guiViewDebuggerWindow);
    m_InstructionNamedControlView =
        new GUINamedControlInspectView(guiViewDebuggerWindow);
  }
  ...
  public override void UpdateInstructions() {
    ...
    /*
    This is an expensive operation, it will resolve the callstacks for all
    instructions. Currently we marshall all instructions for every GUI Event.
  ...
  protected override void DoDrawInstruction(ListViewElement el, int controlId) {
    ...
    GUIViewDebuggerWindow.Styles.listItem.Draw(rect, tempContent, controlId,
  ...
  internal override void OnSelectedInstructionChanged(int index) {
    ...
    if (listViewState.row >= -0) {
      ...
      var viewForType = GetInspectViewForType(instruction.type);
      viewForType.OnSelectedInstructionChanged(
          instruction.typeInstructionIndex);
