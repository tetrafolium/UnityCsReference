...
class GUIViewDebuggerWindow : EditorWindow {
  ...
  internal static class Styles {
    ...
    public static readonly GUIContent inspectedWindowLabel =
        EditorGUIUtility.TrTextContent("Inspected View: ");
    public static readonly GUIContent pickStyleLabel =
        EditorGUIUtility.TrTextContent("Pick Style");
    public static readonly GUIContent pickingStyleLabel =
        EditorGUIUtility.TrTextContent("Picking   ");
    ...
    public static readonly Color contentHighlighterColor =
        new Color(0.62f, 0.77f, 0.90f, 0.5f);
    public static readonly Color paddingHighlighterColor =
        new Color(0.76f, 0.87f, 0.71f, 0.5f);
  }
  ...
  public GUIView inspected {
    get {
      if (m_Inspected != null || m_InspectedEditorWindow == null)
        return m_Inspected;
      // continue inspecting the same window if its dock area is destroyed by
      // e.g., docking or undocking it
    ...
    private set {
      if (m_Inspected != value) {
        ...
        m_Inspected = value;
        if (m_Inspected != null) {
          m_InspectedEditorWindow = (m_Inspected is HostView)
                                        ? ((HostView) m_Inspected).actualView
                                        : null;
  ...
  [SerializeField] GUIView m_Inspected;
  ...
  public void HighlightInstruction(GUIView view, Rect instructionRect,
                                   GUIStyle style) {
  ...
  [SerializeField] InstructionType m_InstructionType = InstructionType.Draw;

  [SerializeField] bool m_ShowHighlighter = true;

  [SerializeField]
  private bool m_InspectOptimizedGUIBlocks = false;

  // TODO: figure out proper minimum values and make sure the window also has
  // compatible minimum size
  readonly SplitterState m_InstructionListDetailSplitter =
      SplitterState.FromRelative(new float[]{30, 70}, new float[]{32, 32},
                                 null);

  // Internal Tool for now. Uncomment it to enable it.
  [MenuItem("Window/Analysis/IMGUI Debugger &5", false, 102,
            false)] // TODO: DEBUG ONLY - REVERT ME
      static void Init() {
    // Get existing open window or if none, make a new one:
    if (s_ActiveInspector == null) {
      GUIViewDebuggerWindow window =
          (GUIViewDebuggerWindow) GetWindow(typeof(GUIViewDebuggerWindow));
  ...
  void OnBecameVisible() { OnShowOverlayChanged(); }

  void OnBecameInvisible() { ClearInstructionHighlighter(); }
  ...
  void HandleStylePicking() {
    if (m_StylePicker != null && m_StylePicker.IsPicking) {
      ...
      if (m_StylePicker.ExploredView != null &&
  ...
  void DoWindowPopup() {
    string selectedName = inspected == null ? Styles.defaultWindowPopupText
        : GetViewName(inspected);

    GUILayout.Label(Styles.inspectedWindowLabel, GUILayout.ExpandWidth(false));

    Rect popupPosition = GUILayoutUtility.GetRect(GUIContent.Temp(selectedName),
                                                  EditorStyles.toolbarDropDown,
                                                  GUILayout.ExpandWidth(true));
    if (GUI.Button(popupPosition, GUIContent.Temp(selectedName),
                   EditorStyles.toolbarDropDown)) {
      ...
      for (int i = 0; i < views.Count; ++i) {
        ...
        GUIContent label = new GUIContent(
            string.Format("{0}. {1}", options.Count, GetViewName(view)));
        options.Add(label);
      ...
      // TODO: convert this to a Unity Window style popup. This way we could
      // highlight the window on hover ;)
      EditorUtility.DisplayCustomMenu(popupPosition, options.ToArray(),
                                      selectedIndex, OnWindowSelected,
                                      selectableViews);
    }
  }
  ...
  void DoInspectTypePopup() {
    ...
    var newInstructionType = (InstructionType) EditorGUILayout.EnumPopup(
        m_InstructionType, EditorStyles.toolbarDropDown);
  ...
  void DoInstructionOverlayToggle() {
    EditorGUI.BeginChangeCheck();
    m_ShowHighlighter =
        GUILayout.Toggle(m_ShowHighlighter, GUIContent.Temp("Show Overlay"),
  ...
  void DoOptimizedGUIBlockToggle() {
    EditorGUI.BeginChangeCheck();
    m_InspectOptimizedGUIBlocks =
        GUILayout.Toggle(m_InspectOptimizedGUIBlocks,
                         GUIContent.Temp("Force Inspect Optimized GUI Blocks"),
                         EditorStyles.toolbarButton);
    if (EditorGUI.EndChangeCheck())
      OnInspectedViewChanged();
  }
  ...
  void DoStylePicker() {
    var pickerRect = GUILayoutUtility.GetRect(Styles.pickStyleLabel,
                                              EditorStyles.toolbarButtonRight);
    if (!m_StylePicker.IsPicking && Event.current.isMouse &&
        Event.current.type == EventType.MouseDown &&
        pickerRect.Contains(Event.current.mousePosition)) {
      m_StylePicker.StartExploreStyle();
    }
    GUI.Toggle(pickerRect, m_StylePicker.IsPicking,
               m_StylePicker.IsPicking ? Styles.pickingStyleLabel
               : Styles.pickStyleLabel, EditorStyles.toolbarButtonRight);
  }
  ...
  void RefreshData() { instructionModeView.UpdateInstructions(); }
  ...
  void ShowDrawInstructions() {
    ...
    EditorGUILayout.BeginVertical();
    {
      if (m_StylePicker.IsPicking &&
        ...
        m_InstructionModeView.SelectRow(
            m_StylePicker.ExploredDrawInstructionIndex);
      }
      instructionModeView.DrawSelectedInstructionDetails(
          position.width - m_InstructionListDetailSplitter.realSizes[0]);
    }
    ...
    EditorGUIUtility.DrawHorizontalSplitter(
        new Rect(m_InstructionListDetailSplitter.realSizes[0] + 1,
                 EditorGUI.kWindowToolbarHeight, 1, position.height));
  }
}

// This needs to match ManagedStackFrameToMono in native
[StructLayout(LayoutKind.Sequential)] struct StackFrame {
  public uint lineNumber;
  public string sourceFile;
  public string methodName;
  public string signature;
  public string moduleName; // TODO: we only get "Mono JIT Code" or "wrapper
                            // something", can we get the actual assembly name?
}
...
internal static partial class GUIViewDebuggerHelper {
  ...
  [RequiredByNativeCode]
  private static void CallDebuggingViewChanged(GUIView view,
