...
public void LogWarning(string warning,
                       [ DefaultValue("null") ] Object context) {
...
public AssetImporter assetImporter {
	get { return AssetImporter.GetAtPath(assetPath); }
}

[System.ComponentModel.EditorBrowsable(
	 System.ComponentModel.EditorBrowsableState.Never)]
[Obsolete(
	 "To set or get the preview, call EditorUtility.SetAssetPreview or AssetPreview.GetAssetPreview instead",
	 true)]
...
public List<AssetPostProcessorMethodCallAnalyticsData> postProcessorCalls =
	new List<AssetPostProcessorMethodCallAnalyticsData>();
}

[Serializable] struct AssetPostProcessorMethodCallAnalyticsData {
...
static void LogPostProcessorMissingDefaultConstructor(Type type) {
	Debug.LogErrorFormat(
		"{0} requires a default constructor to be used as an asset post processor",
		type);
}
...
static void
PostprocessAllAssets(string[] importedAssets, string[] addedAssets,
                     string[] deletedAssets, string[] movedAssets,
                     string[] movedFromPathAssets) {
	object[] args = {importedAssets, deletedAssets, movedAssets,
		         movedFromPathAssets};
	foreach (var assetPostprocessorClass in
	         GetCachedAssetPostprocessorClasses()) {
		MethodInfo method = assetPostprocessorClass.GetMethod(
...
#pragma warning disable 618
	if (ScriptEditorUtility.GetScriptEditorFromPath(
		...
		CodeEditorProjectSync.PostprocessSyncProject(importedAssets, addedAssets,
		                                             deletedAssets, movedAssets,
		                                             movedFromPathAssets);
	} else {
		///@TODO: we need addedAssets for SyncVS. Make this into a proper API and
		///write tests
		SyncVS.PostprocessSyncProject(importedAssets, addedAssets, deletedAssets,
		                              movedAssets, movedFromPathAssets);
	}
	Profiler.EndSample();
}
...
// This is undocumented, and a "safeguard" for when visualstudio gets a new
// release that is incompatible with ours, so that users can postprocess our
// csproj to fix it. (or just completely replace them). Hopefully we'll never
//need this.
static internal void CallOnGeneratedCSProjectFiles() {
	object[] args = {};
	foreach (var method in AllPostProcessorMethodsNamed(
...
static internal string CallOnGeneratedSlnSolution(string path,
                                                  string content) {
	foreach (var method in AllPostProcessorMethodsNamed(
...
AllPostProcessorMethodsNamed(string callbackName) {
	return GetCachedAssetPostprocessorClasses()
	       .Select(assetPostprocessorClass => assetPostprocessorClass.GetMethod(
			       callbackName, BindingFlags.Public | BindingFlags.NonPublic |
			       BindingFlags.Static))
...
static Type[] GetCachedAssetPostprocessorClasses() {
	if (m_PostprocessorClasses == null)
		m_PostprocessorClasses =
			TypeCache.GetTypesDerivedFrom<AssetPostprocessor>().ToArray();
...
static void InitPostprocessors(AssetImportContext context, string pathName) {
	...
	analyticsEvent.importActionId =
		((int) Math.Floor(
			 AssetImporter.GetAtPath(pathName).GetImportStartTime() * 1000))
		.ToString();
	s_AnalyticsEventsStack.Push(analyticsEvent);

	// @TODO: This is just a temporary workaround for the import settings.
	// We should add importers to the asset, persist them and show an inspector
	// for them.
	foreach (Type assetPostprocessorClass in
	         GetCachedAssetPostprocessorClasses()) {
		try {
			var assetPostprocessor = Activator.CreateInstance(
				assetPostprocessorClass) as AssetPostprocessor;
	...
	if (m_PostprocessStack != null) {
		...
		if (m_PostprocessStack.Count != 0) {
			PostprocessStack postStack =
				(PostprocessStack) m_PostprocessStack[m_PostprocessStack.Count - 1];
			m_ImportProcessors = postStack.m_ImportProcessors;
		}
	}

	if (s_AnalyticsEventsStack.Peek().postProcessorCalls.Count != 0)
		EditorAnalytics.SendAssetPostprocessorsUsage(
			s_AnalyticsEventsStack.Peek());
...
static bool ImplementsAnyOfTheses(Type type, string[] methods) {
	foreach (var method in methods) {
		if (type.GetMethod(method, BindingFlags.Public | BindingFlags.NonPublic |
...
static string GetMeshProcessorsHashString() {
	...
	foreach (var assetPostprocessorClass in
	         GetCachedAssetPostprocessorClasses()) {
		try {
			var inst = Activator.CreateInstance(assetPostprocessorClass)
			           as AssetPostprocessor;
			var type = inst.GetType();
			bool hasAnyPostprocessMethod = ImplementsAnyOfTheses(
				type, new[] {"OnPreprocessModel", "OnPostprocessMeshHierarchy",
				             "OnPostprocessModel", "OnPreprocessAnimation",
				             "OnPostprocessAnimation",
				             "OnPostprocessGameObjectWithAnimatedUserProperties",
				             "OnPostprocessGameObjectWithUserProperties",
				             "OnPostprocessMaterial", "OnAssignMaterialModel",
...
[RequiredByNativeCode]
static Material ProcessMeshAssignMaterial(Renderer renderer,
                                          Material material) {
	object[] args = {material, renderer};
	Material assignedMaterial;
	CallPostProcessMethodsUntilReturnedObjectIsValid(
		"OnAssignMaterialModel", args, out assignedMaterial);
...
static bool ProcessMeshHasAssignMaterial() {
	foreach (AssetPostprocessor inst in m_ImportProcessors) {
		if (inst.GetType().GetMethod("OnAssignMaterialModel",
		                             BindingFlags.Public |
...
[RequiredByNativeCode]
static void PreprocessMaterialDescription(MaterialDescription description,
                                          Material material,
...
[RequiredByNativeCode]
static void PostprocessGameObjectWithUserProperties(GameObject go,
                                                    string[] prop_names,
                                                    object[] prop_values) {
...
[RequiredByNativeCode]
static EditorCurveBinding[] PostprocessGameObjectWithAnimatedUserProperties(
	GameObject go, EditorCurveBinding[] bindings) {
	object[] args = {go, bindings};
	CallPostProcessMethods("OnPostprocessGameObjectWithAnimatedUserProperties",
	                       args);
	return bindings;
}
	...
	foreach (var assetPostprocessorClass in
		...
		try {
			var inst = Activator.CreateInstance(assetPostprocessorClass)
			           as AssetPostprocessor;
			var type = inst.GetType();
			bool hasPreProcessMethod =
				type.GetMethod("OnPreprocessTexture",
				               BindingFlags.Public | BindingFlags.NonPublic |
				               BindingFlags.Instance) != null;
			bool hasPostProcessMethod =
				(type.GetMethod("OnPostprocessTexture",
				                BindingFlags.Public | BindingFlags.NonPublic |
				                BindingFlags.Instance) != null) ||
				(type.GetMethod("OnPostprocessCubemap",
				                BindingFlags.Public | BindingFlags.NonPublic |
				                BindingFlags.Instance) != null);
			uint version = inst.GetVersion();
...
[RequiredByNativeCode]
static void PostprocessSprites(Texture2D tex, string pathName,
                               Sprite[] sprites) {
...
static string GetAudioProcessorsHashString() {
	...
	foreach (var assetPostprocessorClass in
		...
		try {
			...
			var type = inst.GetType();
			bool hasPreProcessMethod =
				type.GetMethod("OnPreprocessAudio",
				               BindingFlags.Public | BindingFlags.NonPublic |
				               BindingFlags.Instance) != null;
			bool hasPostProcessMethod =
				type.GetMethod("OnPostprocessAudio",
				               BindingFlags.Public | BindingFlags.NonPublic |
				               BindingFlags.Instance) != null;
			uint version = inst.GetVersion();
			if (hasPreProcessMethod || hasPostProcessMethod) {
...
static Stack<AssetPostProcessorAnalyticsData> s_AnalyticsEventsStack =
	new Stack<AssetPostProcessorAnalyticsData>();
...
static string GetPrefabProcessorsHashString() {
	...
	foreach (var assetPostprocessorClass in
		...
		try {
			...
			var type = inst.GetType();
			bool hasPostProcessMethod =
				type.GetMethod("OnPostprocessPrefab",
				               BindingFlags.Public | BindingFlags.NonPublic |
				               BindingFlags.Instance) != null;
			uint version = inst.GetVersion();
...
[RequiredByNativeCode]
static void PostprocessAssetbundleNameChanged(string assetPath,
                                              string prevoiusAssetBundleName,
                                              string newAssetBundleName) {
	object[] args = {assetPath, prevoiusAssetBundleName, newAssetBundleName};

	foreach (var assetPostprocessorClass in
	         GetCachedAssetPostprocessorClasses()) {
		var assetPostprocessor = Activator.CreateInstance(assetPostprocessorClass)
		                         as AssetPostprocessor;
		InvokeMethodIfAvailable(assetPostprocessor,
		                        "OnPostprocessAssetbundleNameChanged", args);
	}
}
...
static string GetSpeedTreeProcessorsHashString() {
	...
	foreach (var assetPostprocessorClass in
		...
		try {
			...
			var type = inst.GetType();
			bool hasPreProcessMethod =
				type.GetMethod("OnPreprocessSpeedTree",
				               BindingFlags.Public | BindingFlags.NonPublic |
				               BindingFlags.Instance) != null;
			bool hasPostProcessMethod =
				type.GetMethod("OnPostprocessSpeedTree",
				               BindingFlags.Public | BindingFlags.NonPublic |
				               BindingFlags.Instance) != null;
			uint version = inst.GetVersion();
			if (hasPreProcessMethod || hasPostProcessMethod) {
...
static void CallPostProcessMethodsUntilReturnedObjectIsValid<T>(
	string methodName, object[] args, out T returnedObject) where T : class {
...
static void CallPostProcessMethods(string methodName, object[] args) {
	if (IsAssetPostprocessorAnalyticsEnabled()) {
		...
		if (invocationCount > 0) {
			var methodCallAnalytics =
				new AssetPostProcessorMethodCallAnalyticsData();
			methodCallAnalytics.invocationCount = invocationCount;
			methodCallAnalytics.methodName = methodName;
			methodCallAnalytics.duration_sec =
				Time.realtimeSinceStartup - startTime;
			s_AnalyticsEventsStack.Peek().postProcessorCalls.Add(
				methodCallAnalytics);
		}
	} else {
		foreach (AssetPostprocessor inst in m_ImportProcessors) {
...
static object InvokeMethod(MethodInfo method, object[] args) {
	object res = null;
	using (new EditorPerformanceTracker(method.DeclaringType.Name + "." +
	                                    method.Name)) {
...
static bool InvokeMethodIfAvailable(object target, string methodName,
                                    object[] args) {
	MethodInfo method = target.GetType().GetMethod(
		methodName,
		BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
	if (method != null) {
		using (new EditorPerformanceTracker(target.GetType().Name + "." +
		                                    methodName)) {
...
static bool InvokeMethodIfAvailable<T>(object target, string methodName,
                                       object[] args,
                                       ref T returnedObject) where T : class {
	MethodInfo method = target.GetType().GetMethod(
		methodName,
		BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
	if (method != null) {
