...
public class PlatformIcon {
  ...
  public int maxLayerCount {
    get { return m_MaxLayerCount; }
    private set { m_MaxLayerCount = value; }
  }
  public int minLayerCount {
    get { return m_MinLayerCount; }
    private set { m_MinLayerCount = value; }
  }
  ...
  internal string iconSubKind {
    get { return m_iconSubKind; }
    private set { m_iconSubKind = value; }
  }
  public int width {
    get { return m_Width; }
  ...
  internal bool IsEmpty() { return m_Textures.Count(t => t != null) == 0; }

  internal static PlatformIcon[] GetRequiredPlatformIconsByType(
      IPlatformIconProvider platformIcons, PlatformIconKind kind) {
    Dictionary<PlatformIconKind, PlatformIcon[]> requiredIcons =
        platformIcons.GetRequiredPlatformIcons();
  ...
  internal PlatformIcon(int width, int height, int minLayerCount,
                        int maxLayerCount, string iconSubKind,
                        string description, PlatformIconKind kind,
                        bool draggable = true) {
    this.width = width;
    this.height = height;
  ...
  public Texture2D GetTexture(int layer = 0) {
    if (layer < 0 || layer >= m_MaxLayerCount)
      throw new ArgumentOutOfRangeException(string.Format(
          "Attempting to retrieve icon layer {0}, while the icon only contains {1} layers!",
          layer, layerCount));
  ...
  public Texture2D[] GetTextures() { return m_Textures.ToArray(); }

  internal Texture2D[] GetPreviewTextures() {
    Texture2D[] previewTextures = new Texture2D[maxLayerCount];

    for (int i = 0; i < maxLayerCount; i++) {
      previewTextures[i] = PlayerSettings.GetPlatformIconAtSize(
          m_Kind.platform, m_Width, m_Height, m_Kind.kind, m_iconSubKind, i);
    }
  ...
  public void SetTexture(Texture2D texture, int layer = 0) {
    if (layer < 0 || layer >= maxLayerCount) {
      throw new ArgumentOutOfRangeException(string.Format(
          "Attempting to set icon layer {0}, while icon only supports {1} layers!",
          layer, maxLayerCount));
  ...
  public void SetTextures(params Texture2D[] textures) {
    if (textures == null || textures.Length == 0 ||
        textures.Count(t => t != null) == 0) {
      m_Textures.Clear();
      return;
    } else if (textures.Length > maxLayerCount ||
               textures.Length < minLayerCount) {
      throw new InvalidOperationException(string.Format(
          "Attempting to assign an incorrect amount of layers to an PlatformIcon, trying to assign {0} textures while the Icon requires atleast {1} but no more than {2} layers",
          textures.Length, minLayerCount, maxLayerCount));
    }
  ...
  public override string ToString() {
    return UnityString.Format("({0}x{1}) {2}", width, height, description);
  }
}

public class PlatformIconKind {
  internal static readonly PlatformIconKind Any =
      new PlatformIconKind(-1, "Any", "", BuildTargetGroup.Unknown);
  ...
  internal PlatformIconKind(int kind, string kindString, string description,
  ...
  public override int GetHashCode() { return kind.GetHashCode(); }
...
public partial class PlayerSettings : UnityEngine.Object {
  internal static Dictionary<BuildTargetGroup, IPlatformIconProvider>
      platformIconProviders =
          new Dictionary<BuildTargetGroup, IPlatformIconProvider>();

  internal static IPlatformIconProvider
  GetPlatformIconProvider(BuildTargetGroup platform) {
  ...
  RegisterPlatformIconProvider(BuildTargetGroup platform,
                               IPlatformIconProvider platformIconProvider) {
    if (platformIconProviders.ContainsKey(platform))
      return;
  ...
  // serialized.
  public static PlatformIcon[] GetPlatformIcons(BuildTargetGroup platform,
                                                PlatformIconKind kind) {
    IPlatformIconProvider platformIconProvider =
        GetPlatformIconProvider(platform);
    if (platformIconProvider == null)
      return new PlatformIcon[]{};

    string platformName = PlayerSettings.GetPlatformName(platform);

    PlatformIconStruct[] serializedIcons =
        GetPlatformIconsInternal(platformName, kind.kind);
    PlatformIcon[] icons =
        PlatformIcon.GetRequiredPlatformIconsByType(platformIconProvider, kind);
    ...
    if (serializedIcons.Length <= 0) {
      ...
      foreach (PlatformIcon icon in icons) {
        foreach (PlatformIconStruct serializedIcon in serializedIcons) {
          int requiredKind = kind.Equals(PlatformIconKind.Any)
                                 ? serializedIcon.m_Kind
                                 : kind.kind;
          if (icon.kind.kind == requiredKind &&
              icon.iconSubKind == serializedIcon.m_SubKind) {
            if (icon.width == serializedIcon.m_Width &&
                icon.height == serializedIcon.m_Height) {
              Texture2D[] serializedTextures =
                  serializedIcon.m_Textures.Take(icon.maxLayerCount).ToArray();
              Texture2D[] textures =
                  new Texture2D [serializedTextures.Length >
                                 icon.minLayerCount
                                                                 ? serializedTextures
                                     .Length:icon.minLayerCount];
  ...
  public static void SetPlatformIcons(BuildTargetGroup platform,
                                      PlatformIconKind kind,
                                      PlatformIcon[] icons) {
    ...
    if (icons == null)
      ...
      throw new InvalidOperationException(string.Format(
          "Attempting to set an incorrect number of icons for {0} {1} kind, it requires {2} icons but trying to assign {3}.",
          platform.ToString(), kind, requiredIconCount, icons.Length));
    } else {
      iconStructs = icons.Select(i => i.GetPlatformIconStruct())
  ...
  public static PlatformIconKind[] GetSupportedIconKindsForPlatform(
      BuildTargetGroup platform) {
    IPlatformIconProvider platformIconProvider =
        GetPlatformIconProvider(platform);
  ...
  internal static int GetValidPlatformIconCount(PlatformIcon[] icons) {
    return icons.Count(i => i.GetTextures().Count(t => t != null) >=
                                i.minLayerCount &&
                            i.layerCount <= i.maxLayerCount);
  }

  [StaticAccessor("GetPlayerSettings()", StaticAccessorType.Dot)]
  [NativeMethod(Name = "SetIconsForPlatform")] extern internal static void
  SetPlatformIconsInternal(string platform, PlatformIconStruct[] icons,
                           int kind);

  [StaticAccessor("GetPlayerSettings()", StaticAccessorType.Dot)]
  [NativeMethod(
      Name = "GetIconsForPlatform")] extern internal static PlatformIconStruct
      [] GetPlatformIconsInternal(string platform, int kind);

  // Get the texture that will be used as the display icon at a specified size
  // for the specified platform.
  [StaticAccessor("GetPlayerSettings()",
                  StaticAccessorType.Dot)] extern internal static Texture2D
  GetPlatformIconAtSize(string platform, int width, int height, int kind,
                        string subKind = "", int layer = 0);

  internal static void ClearSetIconsForPlatform(BuildTargetGroup target) {
    SetPlatformIcons(target, PlatformIconKind.Any, null);
  }

  // Old API methods, will be made obsolete when the new API is implemented for
  // all platforms, currently it functions as a wrapper for the new API for all
  // platforms that support it (iOS, Android & tvOS).
  public static void SetIconsForTargetGroup(BuildTargetGroup platform,
                                            Texture2D[] icons, IconKind kind) {
    if (platform == BuildTargetGroup.iOS || platform == BuildTargetGroup.tvOS ||
      ...
      PlatformIconKind platformIconKind =
          iconProvider.GetPlatformIconKindFromEnumValue(kind);

      PlatformIcon[] platformIcons =
          GetPlatformIcons(platform, platformIconKind);

      for (var i = 0; i < icons.Length; i++)
        platformIcons [i]
            .SetTexture(icons[i], 0);
  ...
  // Returns the list of assigned icons for the specified platform of a specific
  // kind.
  public static Texture2D[] GetIconsForTargetGroup(BuildTargetGroup platform,
    ...
    if (platform == BuildTargetGroup.iOS || platform == BuildTargetGroup.tvOS ||
      ...
      return GetPlatformIcons(platform, platformIconKind)
          .Select(t => t.GetTexture(0))
          .ToArray();
    } else {
  ...
  internal static void ImportLegacyIcons(string platform, PlatformIconKind kind,
                                         PlatformIcon[] platformIcons) {
    ...
    for (var i = 0; i < legacyIcons.Length; i++) {
      ...
      foreach (var icon in platformIcons) {
        if (icon.width == legacyIconWidths[i] &&
            icon.height == legacyIconHeights[i]) {
  ...
  internal static void ImportLegacyIcons(BuildTargetGroup platform,
                                         PlatformIconKind kind,
                                         PlatformIcon[] platformIcons) {
