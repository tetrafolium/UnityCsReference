...
namespace UnityEditor.PlatformSupport {
  internal class PlatformIconFieldGroup {
    internal class IconFieldGroupInfo {
      ...
      public override int GetHashCode() { return this.m_Label.GetHashCode(); }
    }
    ...
    internal Dictionary<
        IconFieldGroupInfo,
        Dictionary<IconFieldGroupInfo, PlatformIconField[]>> m_IconsFields =
        new Dictionary<IconFieldGroupInfo,
                       Dictionary<IconFieldGroupInfo, PlatformIconField[]>>();

    internal Dictionary<PlatformIconKind, PlatformIcon[]>
        m_PlatformIconsByKind =
            new Dictionary<PlatformIconKind, PlatformIcon[]>();
    ...
    public bool IsEmpty() { return !(m_IconsFields.Count > 0); }
    ...
    public void AddPlatformIcons(PlatformIcon[] icons, PlatformIconKind kind) {
      ...
      if (!m_IconsFields.ContainsKey(kindKey)) {
        ...
        kindDictionary =
            new Dictionary<IconFieldGroupInfo, PlatformIconField[]>();
      } else {
      ...
      foreach (var subKindGroup in groupedBySubKind) {
        var subKindIcons = subKindGroup.ToArray()
                               .Select(i => CreatePlatformIconField(i))
                               .ToArray();
        ...
        subKindKey.m_SetIconSlots =
            PlayerSettings.GetNonEmptyPlatformIconCount(subKindGroup.ToArray());
  ...
  abstract class PlatformIconField {
    ...
    internal ReorderableList.Defaults s_Defaults =
        new ReorderableList.Defaults();

    internal PlatformIconField(PlatformIcon platformIcon,
                               BuildTargetGroup targetGroup) {
      this.platformIcon = platformIcon;
      m_TargetGroup = targetGroup;
      m_HeaderString = this.platformIcon.description;
      m_SizeLabel =
          string.Format("{0}x{1}px", platformIcon.width, platformIcon.height);
    }

    public static Rect GetContentRect(Rect rect, float paddingVertical = 0,
  ...
  class PlatformIconFieldSingleLayer : PlatformIconField {
    ...
    internal PlatformIconFieldSingleLayer(PlatformIcon platformIcon,
                                          BuildTargetGroup targetGroup)
        : base(platformIcon, targetGroup) {
    ...
    void DrawHeader(Rect headerRect) {
      ...
      m_ShowSizeLabel = headerRect.width > (EditorGUIUtility.labelWidth +
                                            kSlotSize + kIconSpacing + 24);
      ...
      GUI.Label(headerRect,
                LocalizationDatabase.GetLocalizedString(sizeLabelString),
                EditorStyles.label);
    }

    void DrawElement(Rect elementRect) {
      int slotWidth = kMaxPreviewSize;
      int slotHeight = (int)((float) platformIcon.height / platformIcon.height *
                             slotWidth); // take into account the aspect ratio
      int previewWidth = Mathf.Min(slotWidth, platformIcon.width);
      int previewHeight =
          (int)((float) platformIcon.height * previewWidth /
                platformIcon.width); // take into account the aspect ratio

      if (Event.current.type == EventType.Repaint)
        s_Defaults.boxBackground.Draw(elementRect, false, false, false, false);

      float width = Mathf.Min(elementRect.width,
                              EditorGUIUtility.labelWidth + 4 + kSlotSize +
                                  kIconSpacing + kMaxPreviewSize);
      Rect elementContentRect = GetContentRect(elementRect, 6f, 12f);
      Rect textureRect =
          new Rect(elementContentRect.x + elementContentRect.width -
                       kMaxPreviewSize - slotWidth - kIconSpacing,
                   elementContentRect.y, slotWidth, slotHeight);

      Texture2D texture = (Texture2D) EditorGUI.ObjectField(
          textureRect, platformIcon.GetTexture(0), typeof(Texture2D), false);

      platformIcon.SetTexture(texture, 0);

      // Preview
      Rect previewRect = new Rect(
          elementContentRect.x + elementContentRect.width - kMaxPreviewSize,
          elementContentRect.y, previewWidth, previewHeight);

      GUI.Box(previewRect, "");
      Texture2D closestIcon = PlayerSettings.GetPlatformIconAtSize(
          m_PlatformName, platformIcon.width, platformIcon.height,
          platformIcon.kind.kind, platformIcon.iconSubKind);
      if (closestIcon != null)
      ...
      if (m_ShowSizeLabel) {
        GUI.Label(new Rect(elementContentRect.x, elementContentRect.y,
                           width - kSlotSize - kIconSpacing, 20),
                  m_SizeLabel);
      }
    }

    internal override void DrawAt() {
      Rect rect = GUILayoutUtility.GetRect(0, kMaxElementHeight + kIconSpacing,
                                           GUILayout.ExpandWidth(true));

      Rect headerRect = new Rect(rect.x, rect.y, rect.width, kHeaderHeight);
      Rect elementRect = new Rect(rect.x, rect.y + kHeaderHeight, rect.width,
                                  rect.height - kHeaderHeight - kIconSpacing);
  ...
  class PlatformIconFieldMultiLayer : PlatformIconField {
    ...
    internal PlatformIconFieldMultiLayer(PlatformIcon platformIcon,
                                         BuildTargetGroup targetGroup)
        : base(platformIcon, targetGroup) {
      bool showControls =
          this.platformIcon.minLayerCount != this.platformIcon.maxLayerCount;

      m_IconLayers =
          new ReorderableIconLayerList(platformIcon.draggable, showControls);
      m_IconLayers.headerString = string.Format(
          "{0} ({1})", this.platformIcon.description, m_SizeLabel);
      ...
      int slotWidth = kMaxPreviewSize;
      int slotHeight =
          (int)((float) platformIcon.height / platformIcon.height * slotWidth);
    ...
    internal override void DrawAt() {
      ...
      // Don't show warning if no layers are set in that case the default
      // placeholder icon will be used.
      if (validLayerCount > 0 &&
          validLayerCount < this.platformIcon.minLayerCount) {
        var errorMsg = string.Format(
            "{0} icons require at least {1} layers to be set with valid textures, however currently only {2} {3} set.",
            this.platformIcon.kind.ToString(), this.platformIcon.minLayerCount,
            validLayerCount, validLayerCount == 1 ? "layer is" : "layers are");
