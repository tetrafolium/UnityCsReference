...
public ReorderableIconLayerList(bool draggable = true,
                                bool showControls = true) {
	m_List = new UnityEditorInternal.ReorderableList(
		new List<Texture2D>(), typeof(Texture2D), draggable, true,
...
void OnElementDraw(Rect rect, int index, bool isActive, bool isFocused) {
	...
	float width = Mathf.Min(rect.width, EditorGUIUtility.labelWidth + 4 +
	                        kSlotSize + kIconSpacing);
	GUI.Label(new Rect(rect.x, rect.y, width - kSlotSize - kIconSpacing, 20),
	          label);

	// Texture slot
	int slotWidth = kSlotSize;
	int slotHeight = (int)((float) m_ImageHeight / m_ImageWidth *
	                       kSlotSize); // take into account the aspect ratio
	var textureRect =
		new Rect(rect.x + rect.width - slotWidth - slotWidth - kIconSpacing,
		         rect.y, slotWidth, slotHeight);

	EditorGUI.BeginChangeCheck();
	textures[index] = (Texture2D) EditorGUI.ObjectField(
		textureRect, textures[index], typeof(Texture2D), false);
	if (EditorGUI.EndChangeCheck())
		OnChange(m_List);

	// Preview
	Rect previewRect = new Rect(rect.x + rect.width - slotWidth, rect.y,
	...
	if (closestIcon != null)
		GUI.DrawTexture(PlatformIconField.GetContentRect(previewRect, 1, 1),
		                previewTextures[index]);
}

void OnHeaderDraw(Rect rect) {
	GUI.Label(rect, LocalizationDatabase.GetLocalizedString(headerString),
	          EditorStyles.label);
}
