...
public sealed partial class HandleUtility {
  ...
  public static float CalcLineTranslation(Vector2 src, Vector2 dest,
                                          Vector3 srcPosition,
    ...
    // The constrained direction is facing towards the camera, THATS BAD when
    // the handle is close to the camera The srcPosition  goes through to the
    // other side of the camera
    float invert = 1.0F;
    Vector3 cameraForward = Camera.current == null ? Vector3.forward
    ...
    // we then parametrise the perpendicular position of dest into the line
    // (p1-p2)
    Vector3 cd = constraintDir;
    cd.y = -cd.y;
    Camera cam = Camera.current;
    // if camera is null, then we are drawing in OnGUI, where y-coordinate goes
    // top-to-bottom
    Vector2 p1 = cam == null
                     ? Vector2
                            .Scale(srcPosition, new Vector2(1f, -1f))
        : EditorGUIUtility.PixelsToPoints(cam.WorldToScreenPoint(srcPosition));
    Vector2 p2 = cam == null
                     ? Vector2
                            .Scale(srcPosition + constraintDir * invert,
                                   new Vector2(1f, -1f))
        : EditorGUIUtility.PixelsToPoints(
              cam.WorldToScreenPoint(srcPosition + constraintDir * invert));
    Vector2 p3 = dest;
    Vector2 p4 = src;
  ...
  // This limits the "shoot off into infinity" factor when the cursor ray and
  // constraint are near parallel. Increase this value to more conservatively
  // restrict movement, lower to allow more extreme values. Ex, with a camera
  // roughly 30 degrees to the handle a value of .1 restricts translation to
  // ~1500m, whereas a value of .01 will allow closer to 50000 units of
  // movement.
  const float k_MinRayConstraintDot = .05f;

  // constraintOrigin and constraintDir are expected to be in Handle space (ie,
  // origin and direction are pre-multiplied by the Handles.matrix)
  internal static bool CalcPositionOnConstraint(Camera camera,
                                                Vector2 guiPosition,
                                                Vector3 constraintOrigin,
                                                Vector3 constraintDir,
                                                out Vector3 position) {
    if (CalcParamOnConstraint(camera, guiPosition, constraintOrigin,
                              constraintDir, out float pointOnLineParam)) {
  ...
  internal static bool CalcParamOnConstraint(Camera camera, Vector2 guiPosition,
                                             Vector3 constraintOrigin,
                                             Vector3 constraintDir,
                                             out float parameterization) {
    Vector3 constraintToCameraTangent = Vector3.Cross(
        constraintDir, camera.transform.position - constraintOrigin);
    Vector3 constraintPlaneNormal =
        Vector3.Cross(constraintDir, constraintToCameraTangent);
    Plane plane = new Plane(constraintPlaneNormal, constraintOrigin);
    var ray = GUIPointToWorldRay(guiPosition);

    if (Vector3.Dot(ray.direction, plane.normal) > k_MinRayConstraintDot &&
        plane.Raycast(ray, out float distance)) {
      var pointOnPlane = ray.GetPoint(distance);
      parameterization =
          PointOnLineParameter(pointOnPlane, constraintOrigin, constraintDir);
  ...
  public static float PointOnLineParameter(Vector3 point, Vector3 linePoint,
                                           Vector3 lineDirection) {
    return (Vector3.Dot(lineDirection, (point - linePoint))) /
           lineDirection.sqrMagnitude;
  }

  // Project /point/ onto a line.
  public static Vector3 ProjectPointLine(Vector3 point, Vector3 lineStart,
                                         Vector3 lineEnd) {
  ...
  public static float DistancePointLine(Vector3 point, Vector3 lineStart,
                                        Vector3 lineEnd) {
    return Vector3.Magnitude(ProjectPointLine(point, lineStart, lineEnd) -
                             point);
  }
  ...
  public static float acceleration {
    get {
      return NumericFieldDraggerUtility.Acceleration(Event.current.shift,
                                                     Event.current.alt);
    }
  }
  ...
  public static float niceMouseDelta {
    get {
      return NumericFieldDraggerUtility.NiceDelta(Event.current.delta,
                                                  acceleration);
    }
  }
  ...
  public static float niceMouseDeltaZoom {
    get {
      ...
      // Problem is that when the user zooms horizontal and vertical, it can
      // jitter back and forth. So we only update from which axis we pick the
      // sign if x and y movement is not very close to each other
      if (Mathf.Abs(Mathf.Abs(d.x) - Mathf.Abs(d.y)) /
              Mathf.Max(Mathf.Abs(d.x), Mathf.Abs(d.y)) >
          .1f) {
  ...
  internal static float DistanceToLineInternal(Vector3 point, Vector3 p1,
                                               Vector3 p2) {
  ...
  static ProfilerMarker s_DistanceToConeMarker =
      new ProfilerMarker("Handles.DistanceToCone");
  static readonly Vector3[] s_DistanceToConePoints = new Vector3[7];
  public static float DistanceToCone(Vector3 position, Quaternion rotation,
                                     float size) {
    using(s_DistanceToConeMarker.Auto()) {
      ...
      s_DistanceToConePoints[1] =
          mat.MultiplyPoint(new Vector3(+baseR, 0, baseZ));
      s_DistanceToConePoints[2] =
          mat.MultiplyPoint(new Vector3(-baseR, 0, baseZ));
      s_DistanceToConePoints[3] =
          mat.MultiplyPoint(new Vector3(+baseR60x, +baseR60y, baseZ));
      s_DistanceToConePoints[4] =
          mat.MultiplyPoint(new Vector3(-baseR60x, +baseR60y, baseZ));
      s_DistanceToConePoints[5] =
          mat.MultiplyPoint(new Vector3(+baseR60x, -baseR60y, baseZ));
      s_DistanceToConePoints[6] =
          mat.MultiplyPoint(new Vector3(-baseR60x, -baseR60y, baseZ));
  ...
  static ProfilerMarker s_DistanceToCubeMarker =
      new ProfilerMarker("Handles.DistanceToCube");
  static readonly Vector3[] s_DistanceToCubePoints = new Vector3[8];
  public static float DistanceToCube(Vector3 position, Quaternion rotation,
                                     float size) {
  ...
  static Vector3[] s_Points = {Vector3.zero, Vector3.zero, Vector3.zero,
                               Vector3.zero, Vector3.zero};
  public static float DistanceToRectangle(Vector3 position, Quaternion rotation,
                                          float size) {
    return DistanceToRectangleInternal(position, rotation,
                                       new Vector2(size, size));
  }

  // Pixel distance from mouse pointer to a rectangle on screen.
  // The method is stable in pixel space but fails when one or more corners of
  // the rectangle is behind the camera.
  internal static float DistanceToRectangleInternal(Vector3 position,
                                                    Quaternion rotation,
                                                    Vector2 size) {
    Vector3 sideways = rotation * new Vector3(size.x, 0, 0);
    Vector3 up = rotation * new Vector3(0, size.y, 0);
    s_Points [0]
    = WorldToGUIPoint(position + sideways + up);
    s_Points [1]
    = WorldToGUIPoint(position + sideways - up);
    s_Points [2]
    = WorldToGUIPoint(position - sideways - up);
    ...
    for (int i = 0; i < 5; i++) {
      if ((s_Points[i].y > pos.y) != (s_Points[j].y > pos.y)) {
        if (pos.x < (s_Points[j].x - s_Points[i].x) * (pos.y - s_Points[i].y) /
                            (s_Points[j].y - s_Points[i].y) +
                        s_Points[i].x) {
          oddNodes = !oddNodes;
        }
      }
  ...
  // then the distance between nearest point on the rectangle and mouse position
  // calculated in pixel space. This method is more stable than
  // DistanceToRectangleInternal for cases when one or more corners of the
  // rectangle is behind the camera. But at the same time it is less stable than
  // DistanceToRectangleInternal in pixel space when the rectangle plane is
  // parallel to cameras forward direction.
  internal static float
  DistanceToRectangleInternalWorldSpace(Vector3 position, Quaternion rotation,
                                        Vector2 size) {
    ...
    if (plane.Raycast(ray, out enter)) {
      ...
      Vector3 d = new Vector3(Mathf.Max(Mathf.Abs(hitPoint.x) - size.x, 0.0f) *
                                  Mathf.Sign(hitPoint.x),
                              Mathf.Max(Mathf.Abs(hitPoint.y) - size.y, 0.0f) *
                                  Mathf.Sign(hitPoint.y),
                              0.0f);
      ...
      return Vector2.Distance(WorldToGUIPoint(hitPoint),
                              WorldToGUIPoint(nearestPoint));
    }

    return float.PositiveInfinity;
  }

  internal static float DistanceToDiamond(Vector3 position, Quaternion rotation,
                                          float size) {
    return DistanceToDiamondInternal(position, rotation, size,
                                     Event.current.mousePosition);
  }

  internal static float DistanceToDiamondInternal(Vector3 position,
                                                  Quaternion rotation,
                                                  float size,
    ...
    s_Points [0]
    = WorldToGUIPoint(position + sideways);
    s_Points [1]
    = WorldToGUIPoint(position - up);
    s_Points [2]
    = WorldToGUIPoint(position - sideways);
    s_Points [3]
    = WorldToGUIPoint(position + up);
  ...
  // Distance from a point /p/ in 2d to a line defined by two s_Points /a/ and
  // /b/
  ...
  // Distance from a point /p/ in 2d to a line segment defined by two s_Points
  // /a/ and /b/
  public static float DistancePointToLineSegment(Vector2 p, Vector2 a,
                                                 Vector2 b) {
  ...
  public static float DistanceToDisc(Vector3 center, Vector3 normal,
                                     float radius) {
  ...
  public static Vector3 ClosestPointToDisc(Vector3 center, Vector3 normal,
                                           float radius) {
    Vector3 tangent = Vector3.Cross(normal, Vector3.up);
    if (tangent.sqrMagnitude < .001f)
      tangent = Vector3.Cross(normal, Vector3.right);
  ...
  public static float DistanceToArc(Vector3 center, Vector3 normal,
                                    Vector3 from, float angle, float radius) {
    Handles.SetDiscSectionPoints(m_ArcPointsBuffer, center, normal, from, angle,
                                 radius);
    return DistanceToPolyLineOnPlane(m_ArcPointsBuffer, center, normal);
  }

  // Get the nearest 3D point.
  public static Vector3 ClosestPointToArc(Vector3 center, Vector3 normal,
                                          Vector3 from, float angle,
                                          float radius) {
    Handles.SetDiscSectionPoints(m_ArcPointsBuffer, center, normal, from, angle,
                                 radius);
  ...
  public static float DistanceToPolyLine(params Vector3[] points) {
    ...
    CameraProjectionCache cam =
        new CameraProjectionCache(Camera.current, Screen.height);
  ...
  // Pixel distance from mouse pointer to a polyline.
  internal static float DistanceToPolyLine(Vector3[] points, bool loop,
                                           out int index) {
    Matrix4x4 handleMatrix = Handles.matrix;
    CameraProjectionCache cam =
        new CameraProjectionCache(Camera.current, Screen.height);
  ...
  internal static float
  DistanceToPolyLineOnPlane(Vector3[] points, Vector3 center, Vector3 normal) {
    Matrix4x4 handleMatrix = Handles.matrix;
    ...
    Vector3 intersect =
        handleMatrix.inverse.MultiplyPoint3x4(r.GetPoint(enter));
    Vector3 p1 = points[0];
    Vector3 p2 = points[1];
    ...
    return DistanceToLineInternal(point, WorldToGUIPoint(s1),
                                  WorldToGUIPoint(s2));
  }
  ...
  public static Vector3 ClosestPointToPolyLine(params Vector3[] vertices) {
    ...
    Vector2 relativePoint =
        Event.current.mousePosition - WorldToGUIPoint(lineStart);
    Vector2 lineDirection =
        WorldToGUIPoint(lineEnd) - WorldToGUIPoint(lineStart);
    float length = lineDirection.magnitude;
  ...
  static void RemoveInsidePoints(int countLimit, Vector2 pt,
                                 List<Vector2> hull) {
    while (hull.Count >= countLimit &&
           CalcPointSide(hull[hull.Count - 2], hull[hull.Count - 1], pt) <= 0)
  ...
  static void CalcPointCloudConvexHull(Vector3[] points,
                                       List<Vector2> outHull) {
    outHull.Clear();
    if (points == null || points.Length == 0)
      return;
  ...
  internal static void EndHandles() {
    if (s_PreviousNearestControl != s_NearestControl && s_NearestControl != 0) {
  ...
  public static float GetHandleSize(Vector3 position) {
    ...
    if (cam) {
      ...
      float distance = Vector3.Dot(position - camPos,
                                   tr.TransformDirection(new Vector3(0, 0, 1)));
      Vector3 screenPos = cam.WorldToScreenPoint(
          camPos + tr.TransformDirection(new Vector3(0, 0, distance)));
      Vector3 screenPos2 = cam.WorldToScreenPoint(
          camPos + tr.TransformDirection(new Vector3(1, 0, distance)));
  ...
  public static Vector3 WorldToGUIPointWithDepth(Vector3 world) {
    return WorldToGUIPointWithDepth(world, Camera.current, Handles.matrix,
                                    Screen.height);
  }

  // Convert world space point to a 2D GUI position.
  // Use this version in critical loops.
  internal static Vector3 WorldToGUIPointWithDepth(Vector3 world, Camera camera,
                                                   Matrix4x4 matrixHandles,
                                                   float screenHeight) {
  ...
  private static Ray
  GUIPointToWorldRayPrecise(Vector2 position,
    ...
    if (camera.orthographic) {
      ...
      // The camera/projection matrices follow OpenGL convention: positive Z is
      // towards the viewer. So negate it to get into Unity convention.
      ...
      Vector3 originOffsetWorldSpace =
          rayDirectionWorldSpace * -startZ / rayDirectionCameraSpace.z;
  ...
  public static Rect WorldPointToSizedRect(Vector3 position, GUIContent content,
                                           GUIStyle style) {
  ...
  internal static bool FindNearestVertex(Vector2 guiPoint,
                                         Transform[] objectsToSearch,
                                         out Vector3 vertex) {
    Camera cam = Camera.current;
    var screenPoint = EditorGUIUtility.PointsToPixels(guiPoint);
    screenPoint.y = cam.pixelRect.yMax - screenPoint.y;
    return Internal_FindNearestVertex(cam, screenPoint, objectsToSearch,
                                      ignoreRaySnapObjects, out vertex);
  }

#pragma warning disable 618
  [Obsolete("Use PickGameObjectCallback")]
  internal delegate GameObject PickClosestGameObjectFunc(Camera cam, int layers,
                                                         Vector2 position,
                                                         GameObject[] ignore,
                                                         GameObject[] filter,
...
#pragma warning restore 618
  ...
  public static GameObject PickGameObject(Vector2 position, GameObject[] ignore,
                                          out int materialIndex) {
    return PickGameObjectDelegated(position, ignore, null, out materialIndex);
  }

  public delegate GameObject PickGameObjectCallback(Camera cam, int layers,
                                                    Vector2 position,
                                                    GameObject[] ignore,
                                                    GameObject[] filter,
                                                    out int materialIndex);
  ...
  internal static GameObject PickGameObjectDelegated(Vector2 position,
                                                     GameObject[] ignore,
                                                     GameObject[] filter,
                                                     out int materialIndex) {
    Camera cam = Camera.current;
...
#pragma warning disable 618
    if (pickClosestGameObjectDelegate != null)
      picked = pickClosestGameObjectDelegate(cam, layers, position, ignore,
                                             filter, out materialIndex);
#pragma warning restore 618

    if (picked == null)
      picked = Internal_PickClosestGO(cam, layers, position, ignore, filter,
                                      out materialIndex);

    if (picked == null && pickGameObjectCustomPasses != null) {
      foreach (var method in pickGameObjectCustomPasses.GetInvocationList()) {
        picked = ((PickGameObjectCallback) method)(
            cam, layers, position, ignore, filter, out materialIndex);
        // don't trust this method to respect the ignore or filter argument,
        // because in the event that it does not it will break pick cycling in
        // SceneViewPicking.GetAllOverlapping.
        if (picked != null && (ignore == null || !ignore.Contains(picked)) &&
            (filter == null || filter.Contains(picked)))
          break;
        picked = null;
      }
    }
  ...
  public static GameObject PickGameObject(Vector2 position,
                                          bool selectPrefabRoot) {
    return PickGameObject(position, selectPrefabRoot, null);
  }

  public static GameObject
  PickGameObject(Vector2 position, bool selectPrefabRoot, GameObject[] ignore) {
    return PickGameObject(position, selectPrefabRoot, ignore, null);
  }

  internal static GameObject PickGameObject(Vector2 position,
                                            bool selectPrefabRoot,
                                            GameObject[] ignore,
                                            GameObject[] filter) {
    int dummyMaterialIndex;
    GameObject picked = PickGameObjectDelegated(position, ignore, filter,
                                                out dummyMaterialIndex);
  ...
  // Get the selection base object, taking into account user enabled picking
  // filter
  internal static GameObject FindSelectionBaseForPicking(GameObject go) {
    ...
    // Walk up the hierarchy to find the outermost prefab instance root that is
    // not marked as non-pickable, or alternatively a GameObject with the
    // SelectionBaseAttribute assigned.
    ...
    while (tr != null) {
      if (!SceneVisibilityState.IsGameObjectPickingDisabled(tr.gameObject)) {
        ...
        // If prefabBase is not pickable, we want to select the nearest pickable
        // root to the base
        ...
        // If a SelectionBaseAttribute is found, select the nearest to the
        // picked GameObject
        if (AttributeHelper.GameObjectContainsAttribute<SelectionBaseAttribute>(
                tr.gameObject))
          return tr.gameObject;
      }
  ...
  public static Material handleMaterial {
    get {
      if (!s_HandleMaterial) {
        s_HandleMaterial =
            (Material) EditorGUIUtility.Load("SceneView/Handles.mat");
      }
  ...
  static void CleanupHandleMaterials() {
    ...
    // This is enough for all of them to get re-fetched in next call to
    // InitHandleMaterials()
  ...
  static internal GraphicsBuffer GetArcIndexBuffer(int segments, int sides) {
    ...
    s_ArcIndexBuffer =
        new GraphicsBuffer(GraphicsBuffer.Target.Index, indexCount, 2);
  ...
  static void InitHandleMaterials() {
    if (!s_HandleWireMaterial) {
      ...
      s_HandleWireMaterial =
          (Material) EditorGUIUtility.LoadRequired("SceneView/HandleLines.mat");
      s_HandleWireMaterial2D = (Material) EditorGUIUtility.LoadRequired(
          "SceneView/2DHandleLines.mat");
      s_HandleWireTextureIndex = ShaderUtil.GetTextureBindingIndex(
          s_HandleWireMaterial.shader, Shader.PropertyToID("_MainTex"));
      s_HandleWireTextureIndex2D = ShaderUtil.GetTextureBindingIndex(
          s_HandleWireMaterial2D.shader, Shader.PropertyToID("_MainTex"));
      s_HandleWireTextureSamplerIndex =
          ShaderUtil.GetTextureSamplerBindingIndex(
              s_HandleWireMaterial.shader, Shader.PropertyToID("_MainTex"));
      s_HandleWireTextureSamplerIndex2D =
          ShaderUtil.GetTextureSamplerBindingIndex(
              s_HandleWireMaterial2D.shader, Shader.PropertyToID("_MainTex"));

      s_HandleDottedWireMaterial = (Material) EditorGUIUtility.LoadRequired(
          "SceneView/HandleDottedLines.mat");
      s_HandleDottedWireMaterial2D = (Material) EditorGUIUtility.LoadRequired(
          "SceneView/2DHandleDottedLines.mat");
      s_HandleDottedWireTextureIndex = ShaderUtil.GetTextureBindingIndex(
          s_HandleDottedWireMaterial.shader, Shader.PropertyToID("_MainTex"));
      s_HandleDottedWireTextureIndex2D = ShaderUtil.GetTextureBindingIndex(
          s_HandleDottedWireMaterial2D.shader, Shader.PropertyToID("_MainTex"));
      s_HandleDottedWireTextureSamplerIndex =
          ShaderUtil.GetTextureSamplerBindingIndex(
              s_HandleDottedWireMaterial.shader,
              Shader.PropertyToID("_MainTex"));
      s_HandleDottedWireTextureSamplerIndex2D =
          ShaderUtil.GetTextureSamplerBindingIndex(
              s_HandleDottedWireMaterial2D.shader,
              Shader.PropertyToID("_MainTex"));

      s_HandleArcMaterial =
          (Material) EditorGUIUtility.LoadRequired("SceneView/CircularArc.mat");
    }
  }

  // Material used to draw "lines" and other wireframe-like stuff.
  // Private for a reason; always use ApplyWireMaterial to set it up for
  // rendering! It needs to remember texture binding index so that DrawAA*
  // functions can properly set the anti-aliasing texture.
  ...
  static Material handleDottedWireMaterial {
    get {
      InitHandleMaterials();
      return Camera.current ? s_HandleDottedWireMaterial
          : s_HandleDottedWireMaterial2D;
    }
  }
  ...
  internal static void ApplyWireMaterial([
    DefaultValue("UnityEngine.Rendering.CompareFunction.Always")
  ] CompareFunction zTest) {
    Material mat = handleWireMaterial;
    // Note: important to call this from C# side, so that it tracks "scripting
    // channels" for any later GL.Begin calls.
    mat.SetInt("_HandleZTest", (int) zTest);
    mat.SetPass(0);
    int textureIndex =
        Camera.current ? s_HandleWireTextureIndex : s_HandleWireTextureIndex2D;
    int samplerIndex = Camera.current ? s_HandleWireTextureSamplerIndex
        : s_HandleWireTextureSamplerIndex2D;
  ...
  internal static void ApplyDottedWireMaterial([
    ...
    mat.SetInt("_HandleZTest", (int) zTest);
    mat.SetPass(0);
    int textureIndex = Camera.current ? s_HandleDottedWireTextureIndex
        : s_HandleDottedWireTextureIndex2D;
    int samplerIndex = Camera.current ? s_HandleDottedWireTextureSamplerIndex
        : s_HandleDottedWireTextureSamplerIndex2D;
    Internal_SetHandleWireTextureIndex(textureIndex, samplerIndex);
  }
  ...
  // Objects to ignore when raysnapping (typically the objects being dragged by
  // the handles)
  ...
  public static object RaySnap(Ray ray) {
    ...
    if (sceneCullingMask == SceneCullingMasks.MainStageSceneViewObjects) {
      // Default code path for Scene view that is just displaying the Main
      // Stage. Note that even if Prefab Mode is open, special Scene views can
      // still show the Main Stage! We only check against default physics scene
      // here, and shouldn't ignore Prefab instances that are opened in Prefab
      // Mode in Context.
      hitAny |= GetNearestHitFromPhysicsScene(ray, Physics.defaultPhysicsScene,
                                              layerCullingMask, false,
                                              ref raycastHit);
    } else {
      ...
      // including each stage as long as they are displayed as context. Prefab
      // instances that are hidden due to being opened in Prefab Mode in Context
      // should be ignored.
      ...
      for (int i = stageHistory.Count - 1; i >= 0; i--) {
        ...
        PhysicsScene physics =
            previewSceneStage !=
            null ? previewSceneStage.scene.GetPhysicsScene()
            : Physics.defaultPhysicsScene;
        hitAny |= GetNearestHitFromPhysicsScene(ray, physics, layerCullingMask,
                                                true, ref raycastHit);
  ...
  static bool GetNearestHitFromPhysicsScene(Ray ray, PhysicsScene physicsScene,
                                            int cullingMask,
                                            bool ignorePrefabInstance,
                                            ref RaycastHit raycastHit) {
    float maxDist = raycastHit.distance;
    int numHits =
        physicsScene.Raycast(ray.origin, ray.direction, s_RaySnapHits, maxDist,
                             cullingMask, QueryTriggerInteraction.Ignore);

    // We are not sure at this point if the hits returned from RaycastAll are
    // sorted or not, so go through them all
    ...
    if (ignoreRaySnapObjects != null) {
      for (int i = 0; i < numHits; i++) {
        if (s_RaySnapHits[i].distance < nearestHitDist) {
          ...
          if (ignorePrefabInstance &&
              GameObjectUtility.IsPrefabInstanceHiddenForInContextEditing(
                  tr.gameObject))
            continue;
  ...
  public delegate bool PlaceObjectDelegate(Vector2 guiPosition,
                                           out Vector3 position,
                                           out Vector3 normal);
  public static event PlaceObjectDelegate placeObjectCustomPasses;

  public static bool PlaceObject(Vector2 guiPosition, out Vector3 position,
                                 out Vector3 normal) {
    Ray ray = GUIPointToWorldRay(guiPosition);
    object hit = RaySnap(ray);
    bool objectIntersected = hit != null;
    float bestDistance =
        objectIntersected ?((RaycastHit) hit).distance : Mathf.Infinity;
    position =
        objectIntersected ? ray.GetPoint(((RaycastHit) hit).distance)
        : Vector3.zero;
  ...
  public static void Repaint() { Internal_Repaint(); }
}
}
