...
[NativeHeader(
    "Runtime/Network/PlayerCommunicator/GeneralConnection.h")] enum GeneralConnectionSendMode {
  NoBlocking,
  AllowBlocking
}
...
[NativeHeader(
    "Runtime/Network/PlayerCommunicator/ManagedProxy/EditorConnectionManaged.h")]
internal class EditorConnectionInternal : IPlayerEditorConnectionNative {
  void IPlayerEditorConnectionNative.SendMessage(Guid messageId, byte[] data,
                                                 int playerId) {
  ...
  bool IPlayerEditorConnectionNative.TrySendMessage(Guid messageId, byte[] data,
                                                    int playerId) {
    if (messageId == Guid.Empty) {
      throw new ArgumentException(nameof(messageId) + " must not be empty");
    }
  ...
  void IPlayerEditorConnectionNative.Poll() { PollInternal(); }
  ...
  void IPlayerEditorConnectionNative.Initialize() { Initialize(); }

  void IPlayerEditorConnectionNative.DisconnectAll() { DisconnectAll(); }
  ...
  public static void RegisterInternal(string messageId) {
    RegisterInternal(new GUID(messageId));
  }
  ...
  SendMessageInternal(int playerId, GUID messageId, byte *data, int dataSize,
                      GeneralConnectionSendMode sendMode);

  // playerId 0 is ANY_PLAYERCONNECTION
  public static void SendMessage(string messageId, byte[] data, int playerId) {
    unsafe {
      fixed(byte *dataBytes = data) SendMessageInternal(
          playerId, new GUID(messageId), dataBytes, data?.Length ?? 0,
          GeneralConnectionSendMode.AllowBlocking);
    }
  }

  [NativeConditional("ENABLE_PLAYERCONNECTION && UNITY_EDITOR")]
  [NativeName("TrySendMessage")]
  private extern unsafe static bool
  TrySendMessageInternal(int playerId, GUID messageId, byte *data,
                         int dataSize);

  // playerId 0 is ANY_PLAYERCONNECTION
  public static bool TrySendMessage(string messageId, byte[] data,
                                    int playerId) {
    unsafe {
      fixed(byte *dataBytes = data) return TrySendMessageInternal(
          playerId, new GUID(messageId), dataBytes, data?.Length ?? 0);
    }
  }
  ...
  [NativeConditional(
      "ENABLE_PLAYERCONNECTION && UNITY_EDITOR")] extern public static string
  GetMulticastAddress();

  [NativeConditional(
      "ENABLE_PLAYERCONNECTION && UNITY_EDITOR")] extern public static UInt32
  GetMulticastPort();

  [NativeConditional("ENABLE_PLAYERCONNECTION && UNITY_EDITOR")]
  [NativeName("BuildServerIDString")] extern public static string
  BuildServerIdentificationFormat(string localIP, UInt32 listenPort, int flags,
                                  UInt32 localGuid, UInt32 editorGuid,
                                  string idString, bool allowDebugging,
                                  string packageString, string projectName);
}
}
