...
internal class AssetStoreWindow : EditorWindow {
  ...
  // Index at 1499 because "Package Manager" is 1500, pairing tools for user to
  ...
  private static void OpenAssetStoreInBrowser() {
    string assetStoreUrl = UnityConnect.instance.GetConfigurationURL(
        CloudConfigUrl.CloudAssetStoreUrl);
    if (UnityEditor.Connect.UnityConnect.instance.loggedIn)
      UnityEditor.Connect.UnityConnect.instance.OpenAuthorizedURLInWebBrowser(
          assetStoreUrl);
    else
  ...
  public void OnEnable() {
    ...
    var windowResource = EditorGUIUtility.Load(
        "UXML/AssetStore/AssetStoreWindow.uxml") as VisualTreeAsset;
    if (windowResource != null) {
      var root = windowResource.CloneTree();

      var lightStyleSheet = EditorGUIUtility.Load(
          EditorUIService.instance
              .GetUIToolkitDefaultCommonLightStyleSheetPath()) as StyleSheet;
      var assetStoreStyleSheet = EditorGUIUtility.Load(
          "StyleSheets/AssetStore/AssetStoreWindow.uss") as StyleSheet;
      ...
      launchPackageManagerButton.clickable.clicked +=
          OnLaunchPackageManagerButtonClicked;

      alwaysOpenInBrowserToggle.SetValueWithoutNotify(
          EditorPrefs.GetBool("AlwaysOpenAssetStoreInBrowser", false));

      alwaysOpenInBrowserToggle.RegisterValueChangedCallback(changeEvent => {
        EditorPrefs.SetBool("AlwaysOpenAssetStoreInBrowser",
  ...
  public static void OpenURL(string url) {
    string assetStoreUrl =
        $"{UnityConnect.instance.GetConfigurationURL(CloudConfigUrl.CloudAssetStoreUrl)}/packages/{url}";
    if (UnityEditor.Connect.UnityConnect.instance.loggedIn)
      UnityEditor.Connect.UnityConnect.instance.OpenAuthorizedURLInWebBrowser(
          assetStoreUrl);
    else
  ...
  private void OnVisitWebsiteButtonClicked() { OpenAssetStoreInBrowser(); }
  ...
  private Button visitWebsiteButton {
    get { return rootVisualElement.Q<Button>("visitWebsiteButton"); }
  }
  private Button launchPackageManagerButton {
    get { return rootVisualElement.Q<Button>("launchPackageManagerButton"); }
  }
  private Toggle alwaysOpenInBrowserToggle {
