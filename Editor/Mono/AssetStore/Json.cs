...
internal struct JSONValue {
  public JSONValue(object o) { data = o; }
  ...
  public static implicit operator JSONValue(bool s) { return new JSONValue(s); }

  public static implicit operator JSONValue(int s) {
    return new JSONValue((float) s);
  }

  public object AsObject() { return data; }
  ...
  public float AsFloat() { return AsFloat(false); }
  ...
  public List<JSONValue> AsList(bool nothrow) {
    ...
    if (!nothrow)
      throw new JSONTypeException("Tried to read " + data.GetType().Name +
                                  " json value as list");
    return null;
  }

  public List<JSONValue> AsList() { return AsList(false); }
  ...
  public Dictionary<string, JSONValue> AsDict() { return AsDict(false); }

  public static JSONValue NewString(string val) { return new JSONValue(val); }

  public static JSONValue NewFloat(float val) { return new JSONValue(val); }
  ...
  public static JSONValue NewBool(bool val) { return new JSONValue(val); }
...
class JSONParser {
  ...
  private void SkipWs() {
    string ws = " \n\t\r";
    while (ws.IndexOf(cur) != -1)
      Next();
  }

  private string PosMsg() { return "line " + line + ", column " + linechar; }
  ...
  private JSONValue ParseValue() {
    ...
    default:
      throw new JSONParseException("Cannot parse json value starting with '" +
                                   json.Substring(idx, 5) + "' at " + PosMsg());
    }
  }
  ...
  private JSONValue ParseDict() {
    ...
    while (cur != '}') {
      ...
      SkipWs();
      if (cur != ':')
        throw new JSONParseException("Missing dict entry delimiter ':' at " +
                                     PosMsg());
      Next();
  ...
  private JSONValue ParseString() {
    ...
    while (idx < len) {
      ...
      if (endidx < 0)
        throw new JSONParseException("missing '\"' to end string at " +
                                     PosMsg());
      ...
      if (endidx >= len)
        throw new JSONParseException(
            "End of json while parsing while parsing string at " + PosMsg());
      ...
      case 'u':
        ...
        digit += json[endidx + 4];
        try {
          int d = System.Int32.Parse(
              digit, System.Globalization.NumberStyles.AllowHexSpecifier);
          res += (char) d;
        } catch (FormatException) {
          throw new JSONParseException("Invalid unicode escape char near " +
                                       PosMsg());
        }
        endidx += 4;
        break;
      default:
        throw new JSONParseException("Invalid escape char '" + ncur +
                                     "' near " + PosMsg());
      }
      idx = endidx + 1;
    }
    if (idx >= len)
      throw new JSONParseException(
          "End of json while parsing while parsing string near " + PosMsg());
  ...
  private JSONValue ParseNumber() {
    ...
    if (cur == 'e' || cur == 'E') {
      ...
      Next();
      if (cur != '-' && cur != '+') {
        // throw new JSONParseException("Missing - or + in 'e' potent specifier
        // at " + PosMsg());
        resstr += cur;
        Next();
      }
    ...
    try {
      ...
      throw new JSONParseException("Cannot convert string to float : '" +
                                   resstr + "' at " + PosMsg());
    }
  }
