...
internal partial class AssetStoreContext {
  static AssetStoreContext() { AssetStoreContext.GetInstance(); }
  ...
  public string GetString(string key) { return EditorPrefs.GetString(key); }

  public int GetInt(string key) { return EditorPrefs.GetInt(key); }

  public float GetFloat(string key) { return EditorPrefs.GetFloat(key); }
  ...
  public int GetSkinIndex() { return EditorGUIUtility.skinIndex; }

  public bool OpenPackage(string id) { return OpenPackage(id, "default"); }
  ...
  public static bool OpenPackageInternal(string id) {
    ...
    if (match.Success &&
        File.Exists(match.Groups[1].Value)) // If id looks like a path name,
                                            // just try to open that
    {
      ...
      foreach (PackageInfo package in PackageInfo.GetPackageList()) {
        if (package.jsonInfo != "") {
          ...
          string itemID = item.Get("id").IsNull() ? null
                                                  : item ["id"]
                                                        .AsString(true);
  ...
  public void OpenBrowser(string url) { Application.OpenURL(url); }
  ...
  public void Download(Package package, DownloadInfo downloadInfo) {
    Download(downloadInfo.id, downloadInfo.url, downloadInfo.key, package.title,
             package.publisher.label, package.category.label, null);
  }

  public static void
  Download(string package_id, string url, string key, string package_name,
           string publisher_name, string category_name,
           AssetStoreUtils.DownloadDoneCallback doneCallback) {
    string[] dest = PackageStorePath(publisher_name, category_name,
                                     package_name, package_id, url);

    JSONValue existing = JSONParser.SimpleParse(
        AssetStoreUtils.CheckDownload(package_id, url, dest, key));

    // If the package is actively being downloaded right now just return
    if (existing.Get("in_progress").AsBool(true)) {
      Debug.Log("Will not download " + package_name +
    ...
    AssetStoreUtils.Download(package_id, url, dest, key, parameters.ToString(),
                             resumeOK, doneCallback);
    EditorAnalytics.SendAssetDownloadEvent(new DownloadAssetInfo(){
        package_id = package_id, package_name = package_name,
        publisher_name = publisher_name, category_name = category_name});
  }
  ...
  public PackageList GetPackageList() {
    ...
    foreach (PackageInfo info in packageInfos) {
      ...
      if (info.jsonInfo == "") {
        package.title =
            System.IO.Path.GetFileNameWithoutExtension(info.packagePath);
        package.id = info.packagePath;

        if (IsBuiltinStandardAsset(info.packagePath)) {
          package.publisher =
              new LabelAndId{label = "Unity Technologies", id = "1"};
          package.category =
              new LabelAndId{label = "Prefab Packages", id = "4"};
      ...
      // If no package with the same ID is in the dictionary yet or if the
      // current package is newer than what we currently have in the dictionary,
      ...
      if (!packages.ContainsKey(package.id) ||
          ...
          (package.version_id != null && package.version_id != "-1" &&
           Int32.Parse(packages[package.id].version_id) <=
               Int32.Parse(package.version_id))) {
  ...
  private static Regex s_StandardPackageRegExp = new Regex(
      @"/Standard Packages/(Character\ Controller|Glass\ Refraction\ \(Pro\ Only\)|Image\ Effects\ \(Pro\ Only\)|Light\ Cookies|Light\ Flares|Particles|Physic\ Materials|Projectors|Scripts|Standard\ Assets\ \(Mobile\)|Skyboxes|Terrain\ Assets|Toon\ Shading|Tree\ Creator|Water\ \(Basic\)|Water\ \(Pro\ Only\))\.unitypackage$",
  ...
  public class Package {
    ...
    public override string ToString() {
      return string.Format(
          ...
          title, id, publisher, category, version, version_id, local_icon,
          local_path, pubdate, description, link);
    }
  }

  public class PackageList { public Package[] results; }
}
}
