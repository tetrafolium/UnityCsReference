...
public sealed class AssetStoreAsset {
  ...
  public int id;
  public string name;
  public string displayName; // name without extension, we cache it to prevent
                             // string operations when rendering
  ...
  public AssetStoreAsset() { disposed = false; }
  ...
  internal string DebugString {
    get {
      string r = string.Format(
          "id: {0}\nname: {1}\nstaticPreviewURL: {2}\ndynamicPreviewURL: {3}\n" +
              "className: {4}\nprice: {5}\npackageID: {6}",
          id, name ?? "N/A", staticPreviewURL ?? "N/A", dynamicPreviewURL ?? "N/A",
          className ?? "N/A", price, packageID);
      if (previewInfo != null) {
        r += string.Format(
            "previewInfo {{\n" + "    packageName: {0}\n" +
                "    packageShortUrl: {1}\n" + "    packageSize: {2}\n" +
            ...
            previewInfo.packageShortUrl ?? "N/A", previewInfo.packageSize,
            previewInfo.packageVersion ?? "N/A", previewInfo.packageRating,
            previewInfo.packageAssetCount, previewInfo.isPurchased,
            previewInfo.isDownloadable, previewInfo.publisherName ?? "N/A",
            previewInfo.encryptionKey ?? "N/A", previewInfo.packageUrl ?? "N/A",
            previewInfo.buildProgress, previewInfo.downloadProgress,
 ...
 *  asset at a time. This class is somewhat prepared for multiple asset store
 * asset selections though.
 */
internal static class AssetStoreAssetSelection {
  ...
  public static void AddAsset(AssetStoreAsset searchResult,
                              Texture2D placeholderPreviewImage) {
    ...
    if (!string.IsNullOrEmpty(searchResult.dynamicPreviewURL) &&
        searchResult.previewBundle == null) {
      // Debug.Log("dyn url " + searchResult.disposed.ToString() + " " +
      // searchResult.dynamicPreviewURL);
      searchResult.disposed = false;
      // searchResult.previewBundle =
      // AssetBundle.CreateFromFile("/users/jonasd/test.unity3d");
      ...
      AsyncHTTPClient client =
          new AsyncHTTPClient(searchResult.dynamicPreviewURL);
      client.doneCallback = delegate(IAsyncHTTPClient c) {
        if (!client.IsSuccess()) {
          System.Console.WriteLine("Error downloading dynamic preview: " +
                                   client.text);
        ...
        if (searchResult.disposed || sel == null || searchResult.id != sel.id) {
          // Debug.Log("dyn disposed " + searchResult.disposed.ToString() + " "
          // + (sel == null ? "null" : sel.id.ToString()) + " " +
          // searchResult.id.ToString());
          return;
        }

        // Go create the asset bundle in memory from the binary blob
        // asynchronously
        try {
          AssetBundleCreateRequest cr =
              AssetBundle.LoadFromMemoryAsync(c.bytes);

          // Workaround: Don't subject the bundle to the usual compatibility
          // checks.  We want to stay compatible with previews created in prior
          // versions of Unity and with the stuff we put into previews, we
          // should generally be able to still load the content in the editor.
          cr.DisableCompatibilityChecks();

          searchResult.previewBundleRequest = cr;
          EditorApplication.CallbackFunction callback = null;

          // The callback will be called each tick and check if the asset bundle
          // is ready
          ...
          callback = () => {
            ...
            if (!cr.isDone) {
              ...
              if (nowTime - startTime > 10.0) {
                ...
                System.Console.WriteLine(
                    "Timed out fetch live preview bundle " +
            ...
            if (searchResult.disposed || sel2 == null ||
                searchResult.id != sel2.id) {
              // No problem. Just ignore.
              // Debug.Log("dyn late disposed " +
              // searchResult.disposed.ToString() + " " + (sel2 == null ? "null"
              // : sel2.id.ToString()) + " " + searchResult.id.ToString());
            } else {
...
#pragma warning disable 618
              if (cr.assetBundle == null || cr.assetBundle.mainAsset == null) {
                ...
                searchResult.previewAsset =
                    searchResult.previewBundle.mainAsset;
#pragma warning restore 618
  ...
  static void DownloadStaticPreview(AssetStoreAsset searchResult) {
    ...
    client.doneCallback = delegate(IAsyncHTTPClient c) {
      if (!client.IsSuccess()) {
        System.Console.WriteLine("Error downloading static preview: " +
                                 client.text);
      ...
      Texture2D tex = new Texture2D(srcTex.width, srcTex.height,
                                    TextureFormat.RGB24, false, true);
      AssetStorePreviewManager.ScaleImage(tex.width, tex.height, srcTex, tex,
                                          null);
      // tex.Compress(true);
  ...
  public static void RefreshFromServer(AssetsRefreshed callback) {
    ...
    AssetStoreClient.AssetsInfo(queryAssets, delegate(
                                                 AssetStoreAssetsInfo results) {
      AssetStoreAssetInspector.paymentAvailability =
          AssetStoreAssetInspector.PaymentAvailability.ServiceDisabled;
      if (!string.IsNullOrEmpty(results.error)) {
        System.Console.WriteLine("Error performing Asset Store Info search: " +
                                 results.error);
        AssetStoreAssetInspector.OfflineNoticeEnabled = true;
        // Debug.LogError("Error performing Asset Store Info search: " +
        // results.error);
        if (callback != null)
          callback();
        return;
      ...
      if (results.status == AssetStoreAssetsInfo.Status.Ok)
        AssetStoreAssetInspector.paymentAvailability =
            AssetStoreAssetInspector.PaymentAvailability.Ok;
      else if (results.status == AssetStoreAssetsInfo.Status.BasketNotEmpty)
        AssetStoreAssetInspector.paymentAvailability =
            AssetStoreAssetInspector.PaymentAvailability.BasketNotEmpty;
      else if (results.status == AssetStoreAssetsInfo.Status.AnonymousUser)
        AssetStoreAssetInspector.paymentAvailability =
            AssetStoreAssetInspector.PaymentAvailability.AnonymousUser;
...
internal class AssetStoreAssetInspector : Editor {
    ...
    get {
      if (s_SharedAssetStoreAssetInspector == null) {
        s_SharedAssetStoreAssetInspector =
            ScriptableObject.CreateInstance<AssetStoreAssetInspector>();
  ...
  class Styles {
    ...
    public Styles() { link.normal.textColor = new Color(.26f, .51f, .75f, 1f); }
  }
  ...
  public void OnDownloadProgress(string id, string message, int bytes,
                                 int total) {
  ...
  public void Update() {
    ...
    bool hasProgress = a != null && a.previewInfo != null &&
                       (a.previewInfo.buildProgress >= 0f ||
                        a.previewInfo.downloadProgress >= 0f);
    if ((a == null && lastAssetID != 0) || (a != null && lastAssetID != a.id) ||
        hasProgress) {
      lastAssetID = a == null ? 0 : a.id;
      Repaint();
    }

    // Repaint when the main asset of a possibly downloaded bundle is ready for
    // preview
  ...
  public override void OnInspectorGUI() {
    ...
    if (activeAsset != null) {
      string typeName = activeAsset.className == null ? ""
          : activeAsset.className.Split(new char[]{' '}, 2)[0];
      ...
      if (isPackage) {
        ...
        packageInfoShown =
            EditorGUILayout.Foldout(packageInfoShown, "Part of package", true);
      }
      if (packageInfoShown) {
        EditorGUILayout.LabelField("Name", info == null ? "-"
                                   : info.packageName);
        EditorGUILayout.LabelField("Version", info == null ? "-"
                                   : info.packageVersion);
        string price = info == null ? "-"
            : (!string.IsNullOrEmpty(activeAsset.price) ? activeAsset.price
                                                        : "free");
        EditorGUILayout.LabelField("Price", price);
        string rating = info != null && info.packageRating >= 0
                            ? info.packageRating + " of 5"
                            : "-";
        EditorGUILayout.LabelField("Rating", rating);
        EditorGUILayout.LabelField("Size", info == null ? "-"
                                   : intToSizeString(info.packageSize));
        string assetCount = info != null && info.packageAssetCount >= 0
                                ? info.packageAssetCount.ToString()
                                : "-";
        EditorGUILayout.LabelField("Asset count", assetCount);
        GUILayout.BeginHorizontal();
        EditorGUILayout.PrefixLabel("Web page");
        bool hasPageUrl = info != null && info.packageShortUrl != null &&
                          info.packageShortUrl != "";
        bool guiBefore = GUI.enabled;
        GUI.enabled = hasPageUrl;

        if (GUILayout.Button(hasPageUrl ? new GUIContent(info.packageShortUrl,
                                                        "View in browser")
        ...
        EditorGUILayout.LabelField("Publisher", info == null ? "-"
                                   : info.publisherName);
      }
      ...
      if (activeAsset.id != 0) {
        ...
        if (GUILayout.Button("Open Asset Store", GUILayout.Height(40),
                             GUILayout.Width(120))) {
  ...
  public static void OpenItemInAssetStore(AssetStoreAsset activeAsset) {
    if (activeAsset.id != 0) {
      AssetStore.Open(string.Format("content/{0}?assetID={1}",
                                    activeAsset.packageID, activeAsset.id));
    }
  }
  ...
  private EditorWrapper previewEditor {
    get {
      ...
      if (preview != m_PreviewObject) {
        ...
        m_PreviewEditor =
            EditorWrapper.Make(m_PreviewObject, EditorFeatures.PreviewGUI);
      }
  ...
  public override void OnPreviewGUI(Rect r, GUIStyle background) {
    ...
    if (editor != null && m_PreviewObject is AnimationClip)
      editor.OnPreviewGUI(
          r, background); // currently renders nothing for animation clips
    else
  ...
  public override void OnInteractivePreviewGUI(Rect r, GUIStyle background) {
    ...
    AssetStoreAsset a = AssetStoreAssetSelection.GetFirstAsset();
    if (a != null && !a.HasLivePreview &&
        !string.IsNullOrEmpty(a.dynamicPreviewURL)) {
