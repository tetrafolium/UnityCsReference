  ...
  public void Parse(AssetStoreResponse response) {
    if (response.job != null && response.job.IsSuccess()) {
      if (response.job.responseCode >= 300)
        error =
            string.Format("HTTP status code {0}", response.job.responseCode);
      ...
      string url =
          response.job == null ? "nulljob" : (response.job.url == null ? "null"
                                             : response.job.url);
      error = "Error receiving response from server on url '" + url +
              "': " + (response.job.text ?? "n/a");
    }
...
class AssetStoreSearchResults : AssetStoreResultBase<AssetStoreSearchResults> {
  ...
  protected override void Parse(Dictionary<string, JSONValue> dict) {
    foreach (JSONValue v in dict ["groups"]
                 .AsList(true)) {
      Group group = Group.Create();
      ParseList(v, ref group);
      groups.Add(group);
    }
    JSONValue offsets = dict ["query"]
    ["offsets"];
    List<JSONValue> limits = dict ["query"]
                             ["limits"]
                                 .AsList(true);
    int idx = 0;
  ...
  private void ParseList(JSONValue matches, ref Group group) {
    ...
    if (matches.ContainsKey("name"))
      group.name = matches ["name"]
                       .AsString(true);
    if (matches.ContainsKey("label"))
      group.label = matches ["label"]
                        .AsString(true);
    group.label = group.label ?? group.name; // fallback
    if (matches.ContainsKey("total_found"))
      group.totalFound = (int) matches ["total_found"]
                             .AsFloat(true);

    if (matches.ContainsKey("matches")) {
      foreach (JSONValue asset in matches ["matches"]
                   .AsList(true)) {
        AssetStoreAsset res = new AssetStoreAsset();
        if (!asset.ContainsKey("id") || !asset.ContainsKey("name") ||
            !asset.ContainsKey("package_id"))
          continue;
        ...
        res.packageID = (int) asset ["package_id"]
                            .AsFloat();

        if (asset.ContainsKey("static_preview_url"))
          res.staticPreviewURL = asset ["static_preview_url"]
                                     .AsString();
        if (asset.ContainsKey("dynamic_preview_url"))
          res.dynamicPreviewURL = asset ["dynamic_preview_url"]
                                      .AsString();

        res.className = asset.ContainsKey("class_name") ? asset ["class_name"]
                                                              .AsString()
                                                        : "";
...
class AssetStoreAssetsInfo : AssetStoreResultBase<AssetStoreAssetsInfo> {
  internal enum Status { BasketNotEmpty, ServiceDisabled, AnonymousUser, Ok }
  internal Status status;

  internal Dictionary<int, AssetStoreAsset> assets =
      new Dictionary<int, AssetStoreAsset>();
  ...
  internal AssetStoreAssetsInfo(Callback c, List<AssetStoreAsset> assets)
      : base(c) {
  ...
  protected override void Parse(Dictionary<string, JSONValue> dict) {
    Dictionary<string, JSONValue> purchaseInfo = dict ["purchase_info"]
                                                     .AsDict(true);
    string statusStr = purchaseInfo ["status"]
                           .AsString(true);
    ...
    paymentTokenAvailable = purchaseInfo ["payment_token_available"]
                                .AsBool();
    if (purchaseInfo.ContainsKey("payment_method_card"))
      paymentMethodCard = purchaseInfo ["payment_method_card"]
                              .AsString(true);
    if (purchaseInfo.ContainsKey("payment_method_expire"))
      paymentMethodExpire = purchaseInfo ["payment_method_expire"]
                                .AsString(true);
    price = purchaseInfo ["price"]
                .AsFloat(true);
    vat = purchaseInfo ["vat"]
              .AsFloat(true);
    priceText = purchaseInfo ["price_text"]
                    .AsString(true);
    vatText = purchaseInfo ["vat_text"]
                  .AsString(true);
    currency = purchaseInfo ["currency"]
                   .AsString(true);
    message = purchaseInfo.ContainsKey("message") ? purchaseInfo ["message"]
                                                        .AsString(true)
                                                  : null;

    List<JSONValue> assetsIn = dict ["results"]
                                   .AsList(true);
    ...
    foreach (JSONValue val in assetsIn) {
      ...
      asset.className = val ["class_names"]
                            .AsString(true)
                            .Trim();
      a.packageName = val ["package_name"]
                          .AsString(true)
                          .Trim();
      a.packageShortUrl = val ["short_url"]
                              .AsString(true)
                              .Trim();
      // a.packagePrice = val.ContainsKey("price_text") ?
      // val["price_text"].AsString(true).Trim() : null;
      asset.price = val.ContainsKey("price_text") ? val ["price_text"]
                                                        .AsString(true)
                                                        .Trim()
                                                  : null;
      a.packageSize =
          int.Parse(val.Get("package_size").IsNull() ? "-1"
                                                     : val ["package_size"]
                                                           .AsString(true));
      asset.packageID = int.Parse(val ["package_id"]
                                      .AsString());
      a.packageVersion = val ["package_version"]
                             .AsString();
      a.packageRating =
          int.Parse(val.Get("rating").IsNull() || val ["rating"]
                                                          .AsString(true)
                                                          .Length == 0
                        ? "-1"
                        : val ["rating"]
                              .AsString(true));
      a.packageAssetCount = int.Parse(val ["package_asset_count"]
                                              .IsNull()
                                          ? "-1"
                                          : val ["package_asset_count"]
                                                .AsString(true));
      a.isPurchased = val.ContainsKey("purchased") ? val ["purchased"]
                                                         .AsBool(true)
                                                   : false;
      a.isDownloadable = a.isPurchased || asset.price == null;
      a.publisherName = val ["publisher_name"]
                            .AsString(true)
                            .Trim();
      // a.previewBundle = (!val.ContainsKey("preview_bundle") ||
      // val["preview_bundle"].IsNull()) ? "" :
      // val["preview_bundle"].AsString(true);
      a.packageUrl = val.Get("package_url").IsNull() ? ""
                                                     : val ["package_url"]
                                                           .AsString(true);
      a.encryptionKey = val.Get("encryption_key").IsNull()
                            ? ""
                            : val ["encryption_key"]
                                  .AsString(true);
      a.categoryName = val.Get("category_name").IsNull() ? ""
                                                         : val ["category_name"]
                                                               .AsString(true);
      a.buildProgress = -1f;
...
class PurchaseResult : AssetStoreResultBase<PurchaseResult> {
  ...
  protected override void Parse(Dictionary<string, JSONValue> dict) {
    packageID = int.Parse(dict ["package_id"]
                              .AsString());
    message = dict.ContainsKey("message") ? dict ["message"]
                                                .AsString(true)
                                          : null;

    string statusStr = dict ["status"]
                           .AsString(true);
    if (statusStr == "basket-not-empty")
      status = Status.BasketNotEmpty;
    else if (statusStr == "service-disabled")
...
class BuildPackageResult : AssetStoreResultBase<BuildPackageResult> {
  ...
  protected override void Parse(Dictionary<string, JSONValue> dict) {
    ...
    packageID = int.Parse(dict ["id"]
                              .AsString());
    if (packageID != asset.packageID) {
      Debug.LogError(
          "Got asset store server build result from mismatching package");
      return;
    }
    asset.previewInfo.packageUrl = dict.ContainsKey("url") ? dict ["url"]
                                                                 .AsString(true)
                                                           : "";
    asset.previewInfo.encryptionKey = dict.ContainsKey("key")
                                          ? dict ["key"]
                                                .AsString(true)
                                          : "";

    // The asset store is weird. Of pct is 0 it returns a 0.0 float, if it is >
    // 0 it returns a string with the pct ie "23".
    asset.previewInfo.buildProgress =
        (dict ["progress"]
                 .IsFloat()
             ? dict ["progress"]
                   .AsFloat(true)
             : float.Parse(dict ["progress"]
                               .AsString(true),
...
class AssetStoreClient {
  ...
  internal enum LoginState { LOGGED_OUT, IN_PROGRESS, LOGGED_IN, LOGIN_ERROR }
  ...
  static string VersionParams {
    get {
      return "unityversion=" +
             System.Uri.EscapeDataString(Application.unityVersion) +
             "&skip_terms=1";
    }
  }
  ...
  // The base server API url. This is used for all the normal request towards
  // the server API.
  static string APIUrl(string path) {
    return String.Format("{0}/api{2}.json?{1}", AssetStoreUrl, VersionParams,
                         path);
  }

  static string APISearchUrl(string path) {
    return String.Format("{0}/public-api{2}.json?{1}", AssetStoreSearchUrl,
                         VersionParams, path);
  }
  ...
  static string GetToken() { return InternalEditorUtility.GetAuthToken(); }
  ...
  internal static void LoginWithCredentials(string username, string password,
                                            bool rememberMe,
                                            DoneLoginCallback callback) {
    if (sLoginState == LoginState.IN_PROGRESS) {
      Debug.LogError(
          "Tried to login with credentials while already in progress of logging in");
      return;
    }
    ...
    AsyncHTTPClient client =
        new AsyncHTTPClient(url.Replace("http://", "https://"));
  ...
  internal static void LoginWithRememberedSession(DoneLoginCallback callback) {
    if (sLoginState == LoginState.IN_PROGRESS) {
      Debug.LogError(
          "Tried to login with remembered session while already in progress of logging in");
      return;
    }
    sLoginState = LoginState.IN_PROGRESS;
    ...
    string url =
        AssetStoreUrl + "/login?skip_terms=1&reuse_session=" + SavedSessionID;
    AssetStoreClient.sLoginErrorMessage = null;
  ...
  static AsyncHTTPClient.DoneCallback
  WrapLoginCallback(DoneLoginCallback callback) {
    return delegate(IAsyncHTTPClient job) {
      // We're logging in
      string msg = job.text;
      if (!job.IsSuccess()) {
        AssetStoreClient.sLoginState = LoginState.LOGIN_ERROR;
        AssetStoreClient.sLoginErrorMessage =
            job.responseCode >= 200 && job.responseCode < 300
                ? msg
                : "Failed to login - please retry";
      } else if (msg.StartsWith("<!DOCTYPE")) // TODO: Expose status line in job
      {
        ...
        if (msg.Contains(
                "@")) // login with reused session id returns the user email
  ...
  static AsyncHTTPClient CreateJSONRequestPost(string url,
  ...
  */
   ...
   */
  private static AsyncHTTPClient.DoneCallback
  WrapJsonCallback(DoneCallback callback) {
    return delegate(IAsyncHTTPClient job) {
      if (job.IsDone()) {
        try {
  ...
  /*
   * Parse the HTTP response as a JSON string and into a AssetStoreResponse
   * object.
   */
   ...
   */
  static Dictionary<string, JSONValue>
  ParseJSON(string content, out string status, out string message) {
    ...
    try {
      ...
      // Old backend encapsulated all replies in a "result" dict. Just unwrap
      // that here.
      if (dict.ContainsKey("result") && dict ["result"]
                                            .IsDict()) {
        dict = dict ["result"]
                   .AsDict(true);
      }
   ...
   */
  internal static AsyncHTTPClient
  SearchAssets(string searchString, string[] requiredClassNames,
               string[] assetLabels, List<SearchCount> counts,
    ...
    // If one af the class names is "MonoScript" then also include "Script"
    // since that is what asset store expects
    if (Array.Exists(requiredClassNames, (string a) => {
          return a.Equals("MonoScript", StringComparison.OrdinalIgnoreCase);
        })) {
    ...
    string url = string.Format(
        ...
        System.Uri.EscapeDataString(string.Join(",", assetLabels)), offsets,
        limits, groupNames);
    ...
    return CreateJSONRequest(url,
                             delegate(AssetStoreResponse ar) { r.Parse(ar); });
  }

  /*
   * Looks up assets by ID in the asset store and passes the results to
   * DoneCallback
   */
  internal static AsyncHTTPClient
  AssetsInfo(List<AssetStoreAsset> assets,
             AssetStoreAssetsInfo.Callback callback) {
   ...
   */
  internal static AsyncHTTPClient
  DirectPurchase(int packageID, string password,
                 PurchaseResult.Callback callback) {
    string url =
        APIUrl(string.Format("/purchase/direct/{0}", packageID.ToString()));
    // Debug.Log(url);
    PurchaseResult r = new PurchaseResult(callback);
    Dictionary<string, string> d = new Dictionary<string, string>();
    d["password"] = password;
    return CreateJSONRequestPost(
        url, d, delegate(AssetStoreResponse ar) { r.Parse(ar); });
  }

  internal static AsyncHTTPClient
  BuildPackage(AssetStoreAsset asset, BuildPackageResult.Callback callback) {
