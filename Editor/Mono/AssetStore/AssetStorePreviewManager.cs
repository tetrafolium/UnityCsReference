...
public string label;           //
internal AsyncHTTPClient
        client; // null if not in progress of fetching preview
public Color color {
	get {
		return Color.Lerp(
			new Color(1, 1, 1, 0), new Color(1, 1, 1, 1),
			Mathf.Min(
				1f, (float)((EditorApplication.timeSinceStartup - lastFetched) /
				            kFadeTime)));
	}
}
}
...
//        not have the jumpy behaviour. Just put the in a queue an handle them
//        from there.
//
...
static Dictionary<string, CachedAssetStoreImage> CachedAssetStoreImages {
	get {
		if (Instance.m_CachedAssetStoreImages == null) {
			Instance.m_CachedAssetStoreImages =
				new Dictionary<string, CachedAssetStoreImage>();
		}
...
public static int Downloading {
	get {
		int c = 0;
		foreach (KeyValuePair<string, CachedAssetStoreImage> kv in
		         CachedAssetStoreImages)
...
public static string StatsString() {
	return string.Format(
		"Reqs: {0}, Ok: {1}, Abort: {2}, CacheDel: {3}, Cache: {4}/{5}, CacheHit: {6}",
		Instance.Requested, Instance.m_Success, Instance.m_Aborted,
		Instance.m_CacheRemove,
		AssetStorePreviewManager.CachedAssetStoreImages.Count,
		Instance.m_MaxCachedAssetStoreImages, Instance.CacheHit);
}

/**
 * Return a texture from a url that points to an image resource
 *
 * This method does not block but queues a request to fetch the image and
 * return null. When the image has been fetched this method will return the
 * image texture downloaded.
 */
public static CachedAssetStoreImage
TextureFromUrl(string url, string label, int textureSize, GUIStyle labelStyle,
               GUIStyle iconStyle, bool onlyCached) {
	...
	if (CachedAssetStoreImages.TryGetValue(url, out cached)) {
		...
		// Refetch the image if the size has changed and is not in the progress of
		// being fetched
		bool refetchInitiated = cached.requestedWidth == textureSize;
		bool correctSize =
			cached.image != null && cached.image.width == textureSize;

		bool cacheRequestAborted = cached.requestedWidth == -1;

		if ((correctSize || refetchInitiated || onlyCached) &&
		    !cacheRequestAborted) {
			Instance.CacheHit++;

			// Use cached image (that may be null) if we're in progress of fetching
			// the image or if we have rendered the images correctly
			// return cached;
			bool fetchingImage = cached.client != null;
			bool labelDrawn = cached.label == null;
			bool valid = fetchingImage || labelDrawn;
			bool convPerTickExceeded =
				Instance.m_ConvertedThisTick > kMaxConvertionsPerTick;
			s_NeedsRepaint = s_NeedsRepaint || convPerTickExceeded;
			return (valid || convPerTickExceeded)
		   ? cached
		   : RenderEntry(cached, labelStyle, iconStyle);
		}
		// Debug.Log(string.Format("Found {0} {1} {2} {3}", correctSize,
		// refetchInitiated, onlyCached, cacheRequestAborted));
		newentry = false;
...
private static AsyncHTTPClient
SetupTextureDownload(CachedAssetStoreImage cached, string url, string tag) {
	...
	client.doneCallback = delegate(IAsyncHTTPClient c) {
		...
		if (!client.IsSuccess()) {
			if (client.state != AsyncHTTPClient.State.ABORTED) {
				string err =
					"error " + client.text + " " + client.state + " '" + url + "'";
		...
		// In the case of refetch because of resize first destroy the current
		// image
		if (cached.image != null)
...
/*
 * When the new GUI system is is place this should render the label to the
 * cached icon to speed up rendering. For now we just scale the incoming image
 * and render the label separately
 */
private static CachedAssetStoreImage RenderEntry(CachedAssetStoreImage cached,
                                                 GUIStyle labelStyle,
                                                 GUIStyle iconStyle) {
	if (cached.label == null || cached.image == null)
		return cached;
	Texture2D tmp = cached.image;
	cached.image = new Texture2D(cached.requestedWidth, cached.requestedWidth,
	                             TextureFormat.RGB24, false, true);
	ScaleImage(cached.requestedWidth, cached.requestedWidth, tmp, cached.image,
	           iconStyle);
 ...
 */
internal static void ScaleImage(int w, int h, Texture2D inimage,
                                Texture2D outimage, GUIStyle bgStyle) {
	SavedRenderTargetState saved = new SavedRenderTargetState();

	if (s_RenderTexture != null &&
	    (s_RenderTexture.width != w || s_RenderTexture.height != h)) {
		Object.DestroyImmediate(s_RenderTexture);
		s_RenderTexture = null;
	}

	if (s_RenderTexture == null) {
		s_RenderTexture = RenderTexture.GetTemporary(
			w, h, 16, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
	...
	if (inimage.width > inimage.height) {
		float newHeight = (float) blitRect.height *
		                  ((float) inimage.height / (float) inimage.width);
		blitRect.height = (int) newHeight;
		blitRect.y += (int)(newHeight * 0.5f);
	} else if (inimage.width < inimage.height) {
		float newWidth = (float) blitRect.width *
		                 ((float) inimage.width / (float) inimage.height);
		blitRect.width = (int) newWidth;
		blitRect.x += (int)(newWidth * 0.5f);
	}
	if (bgStyle != null && bgStyle.normal != null &&
...
public static void AbortSize(int size) {
	...
	foreach (KeyValuePair<string, CachedAssetStoreImage> kv in
	         AssetStorePreviewManager.CachedAssetStoreImages) {
...
public static void AbortOlderThan(double timestamp) {
	...
	// @TODO: Currently we know that AbortOlderThan is called exactly once each
	// repaint.
	//        Therefore this counter can be reset here. Should probably be moved
