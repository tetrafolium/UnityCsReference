...
private static readonly Dictionary<Type, EnumData> s_NonObsoleteEnumData =
	new Dictionary<Type, EnumData>();
private static readonly Dictionary<Type, EnumData> s_EnumData =
	new Dictionary<Type, EnumData>();
...
internal static EnumData GetCachedEnumData(Type enumType,
	...
	EnumData enumData;
	if (excludeObsolete &&
	    s_NonObsoleteEnumData.TryGetValue(enumType, out enumData))
		return enumData;
	...
	enumData.unsigned = enumData.underlyingType == typeof(byte) ||
	...
	// We can't order the enum from its MetadataToken if its Assembly Dynamic
	// because of a bug in .NET
	try {
	...
	enumData.displayNames =
		enumfieldlist.Select(f => EnumNameFromEnumField(f)).ToArray();
	if (enumData.displayNames.Distinct().Count() !=
	    enumData.displayNames.Length) {
		Debug.LogWarning(
			$"Enum {enumType.Name} has multiple entries with the same display name, this prevents selection in EnumPopup.");
	}

	enumData.tooltip =
		enumfieldlist.Select(f => EnumTooltipFromEnumField(f)).ToArray();
	enumData.values =
		enumfieldlist.Select(f =>(Enum) f.GetValue(null)).ToArray();
	enumData.flagValues =
		enumData.unsigned
	    ? enumData.values.Select(v => unchecked ((int) Convert.ToUInt64(v)))
		.ToArray()
	    : enumData.values.Select(v => unchecked ((int) Convert.ToInt64(v)))
		.ToArray();
	...
	enumData.serializable = enumData.underlyingType !=
	                        typeof(long) &&enumData.underlyingType !=
	                        typeof(ulong);
...
internal static int EnumFlagsToInt(EnumData enumData, Enum enumValue) {
	if (enumData.unsigned) {
		...
		// ensure unsigned 16- and 8-bit variants will display using "Everything"
		// label
		if (enumData.underlyingType == typeof(ushort)) {
...
internal static Enum IntToEnumFlags(Type enumType, int value) {
	...
	// parsing a string seems to be the only way to go from a flags int to an
	// enum value
	if (enumData.unsigned) {
		if (enumData.underlyingType == typeof(uint)) {
...
private static bool CheckObsoleteAddition(FieldInfo field,
