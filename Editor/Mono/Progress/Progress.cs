...
public static partial class Progress {
  ...
  public class Item {
    struct CachedValue<T> {
      ...
      public void Dirty() { m_Updated = false; }

      public bool IsDirty() { return !m_Updated; }
    }
    ...
    CachedValue<DateTime> m_StartTime =
        new CachedValue<DateTime>(id => MSecToDateTime(GetStartDateTime(id)));
    CachedValue<DateTime> m_UpdateTime =
        new CachedValue<DateTime>(id => MSecToDateTime(GetUpdateDateTime(id)));
    ...
    CachedValue<TimeDisplayMode> m_TimeDisplayMode =
        new CachedValue<TimeDisplayMode>(GetTimeDisplayMode);
    CachedValue<TimeSpan> m_RemainingTime =
        new CachedValue<TimeSpan>(id => SecToTimeSpan(GetRemainingTime(id)));
    ...
    public TimeSpan remainingTime {
      get {
        ...
        return m_RemainingTime.GetValue(id) -
               new TimeSpan(duration.Days, duration.Hours, duration.Minutes,
                            duration.Seconds);
      }
    }
    ...
    public bool responding =>
        !running || (DateTime.Now.ToUniversalTime() - updateTime) <=
                        TimeSpan.FromSeconds(5f);
    public bool cancellable => IsCancellable(id);
    public bool indefinite =>
        running && (progress == -1f ||
                    (options & Options.Indefinite) == Options.Indefinite);
    public float elapsedTime =>(float)(updateTime - startTime).TotalSeconds;
    public bool exists => Exists(id);

    internal Item(int id) { this.id = id; }

    public void Report(float newProgress) { Progress.Report(id, newProgress); }
    ...
    public int Remove() { return Progress.Remove(id); }
    ...
    public void ClearRemainingTime() { Progress.ClearRemainingTime(id); }
  ...
  public class TaskError : TaskReport {
    public TaskError(string error) : base(0f) { this.error = error; }
  }
  ...
  class Task {
    ...
    public Func<int, object, IEnumerator> handler {
      get;
      internal set;
    }
    ...
    public Stack<IEnumerator> iterators {
      get;
      internal set;
    }
  }
