...
public static partial class Progress {
  ...
  [NativeMethod(IsFreeFunction = true, IsThreadSafe = true,
                Name = "Editor::Progress::Start")]
  private static extern int Internal_Start(string name, string description,
                                           Options options, int parentId);

  public static int Start(string name, string description = null,
                          Options options = Options.None, int parentId = -1) {
    // Automatically set C# progress as managed so they get canceled when a
  ...
  [NativeMethod(IsFreeFunction = true, IsThreadSafe = true,
                Name = "Editor::Progress::Report")]
  private static extern void Internal_Report(int id, float progress,
  ...
  RegisterCancelCallbackFromScript(int id, Func<bool> callback);
  ...
  [ThreadSafe]
  public static extern void SetTimeDisplayMode(int id,
  ...
  public static int RunTask(string name, string description,
                            Func<int, object, IEnumerator> taskHandler,
                            Options options = Options.None, int parentId = -1,
                            object userData = null) {
    var progressId = Start(name, description, options, parentId);
    s_Tasks.Add(new Task{id = progressId, handler = taskHandler,
                         userData = userData,
                         iterators = new Stack<IEnumerator>()});
    ...
    for (var taskIndex = s_Tasks.Count - 1; taskIndex >= 0; --taskIndex) {
      ...
      try {
        ...
        if (report?.error != null) {
          ...
          } else if ((GetOptions(task.id) & Options.Indefinite) ==
                     Options.Indefinite) {
            Report(task.id, -1f);
          }
        }
  ...
  public static IEnumerable<Item> EnumerateItems() { return s_ProgressItems; }
  ...
  [NativeMethod(IsFreeFunction = true, IsThreadSafe = false,
                Name = "Editor::Progress::GetGlobalProgress")]
  ...
  [NativeMethod(IsFreeFunction = true, IsThreadSafe = false,
                Name = "Editor::Progress::GetGlobalRemainingTime")]
  ...
  public static TimeSpan globalRemainingTime {
    get {
      ...
      return s_RemainingTime - new TimeSpan(duration.Days, duration.Hours,
