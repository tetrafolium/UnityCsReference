...
internal class CacheServerPreferences {
  internal class Properties {
    ...
    public static readonly GUIContent maxCacheSize = EditorGUIUtility.TrTextContent(
        ...
        "The size of the local asset cache server folder will be kept below this maximum value.");
    public static readonly GUIContent customCacheLocation =
        EditorGUIUtility.TrTextContent(
            "Custom cache location",
            "Specify the local asset cache server folder location.");
    public static readonly GUIContent cacheFolderLocation =
        EditorGUIUtility.TrTextContent(
            "Cache Folder Location",
            "The local asset cache server folder is shared between all projects.");
    public static readonly GUIContent cleanCache =
        EditorGUIUtility.TrTextContent("Clean Cache");
    public static readonly GUIContent enumerateCache =
        EditorGUIUtility.TrTextContent(
            "Check Cache Size",
            "Check the size of the local asset cache server - can take a while");
    public static readonly GUIContent browseCacheLocation =
        EditorGUIUtility.TrTextContent(
            "Browse for local asset cache server location");
    public static readonly GUIContent cacheServerDefaultMode = new GUIContent(
        "Cache Server Default Mode",
        "Specifies if cache server should be enabled or disabled by default. This can be overridden per project in editor settings.");
    public static readonly GUIContent cacheServerIPLabel = new GUIContent(
        "Default IP address",
        "This IP address is used for the cache server if not overridden in the editor settings per project.");
  }

  internal static class Styles {
    public static GUIStyle cacheFolderLocation = new GUIStyle(GUI.skin.label);
    static Styles() { cacheFolderLocation.wordWrap = true; }
  }

  const string kAssetPipelineVersionForNewProjects =
      "AssetPipelineVersionForNewProjects";
  ...
  public static bool IsCacheServerV2Enabled {
    get {
      ...
      return AssetDatabase.IsV2Enabled() &&
             s_CacheServer2Mode == CacheServer2Mode.Enabled;
    }
  }
  ...
  public static void ReadPreferences() {
    s_AssetPipelineVersionForNewProjects =
        (AssetPipelineVersion)
            EditorPrefs.GetInt(kAssetPipelineVersionForNewProjects,
                               (int) AssetPipelineVersion.Version2);
    s_CacheServer2IPAddress =
        EditorPrefs.GetString(kIPAddress2Key, s_CacheServer2IPAddress);
    s_CacheServer2Mode = (CacheServer2Mode) EditorPrefs.GetInt(
        kMode2Key, (int) CacheServer2Mode.Disabled);
    s_CacheServerIPAddress =
        EditorPrefs.GetString(kIPAddressKey, s_CacheServerIPAddress);
    s_CacheServerMode = (CacheServerMode) EditorPrefs.GetInt(
        kModeKey, (int)(EditorPrefs.GetBool(kDeprecatedEnabledKey)
                            ? CacheServerMode.Remote
  ...
  public static void WritePreferences() {
    ...
    if (s_EnableCustomPath && oldPath != s_CachePath)
      changedDir = true;
    if (s_EnableCustomPath != oldCustomPath &&
        s_CachePath != LocalCacheServer.GetCacheLocation() && s_CachePath != "")
      changedDir = true;
    if (changedDir) {
      var message = s_CacheServerMode ==
                    CacheServerMode.Local ?
                    "You have changed the location of the local cache storage."
          : "You have disabled the local cache.";
      message += " Do you want to delete the old locally cached data at " +
                 LocalCacheServer.GetCacheLocation() + "?";
      if (EditorUtility.DisplayDialog("Delete old Cache", message, "Delete",
                                      "Don't Delete")) {
        LocalCacheServer.Clear();
      }
    }

    EditorPrefs.SetInt(kAssetPipelineVersionForNewProjects,
                       (int) s_AssetPipelineVersionForNewProjects);
  ...
  internal static SettingsProvider CreateSettingsProvider() {
    return new SettingsProvider("Preferences/Cache Server (global)",
                                SettingsScope.User){
        guiHandler = searchContext => {using(new SettingsWindow.GUIScope())
                                           OnGUI(searchContext);
  }
};
}
...
public static bool GetEnvironmentAssetPipelineOverride() {
  return !string.IsNullOrEmpty(System.Environment.GetEnvironmentVariable(
      "UNITY_ASSETS_V2_KATANA_TESTS"));
}
...

  {
    ...
    if (GetEnvironmentAssetPipelineOverride())
      EditorGUILayout.HelpBox(
          "Asset pipeline currently forced environment variable UNITY_ASSETS_V2_KATANA_TESTS",
          MessageType.Info, true);
    else if (GetCommandLineAssetPipelineOverride() != 0)
      EditorGUILayout.HelpBox(
          "Asset pipeline currently forced by command line argument",
          MessageType.Info, true);
    else if (GetMagicFileAssetPipelineOverride())
      EditorGUILayout.HelpBox(
          "Asset pipeline currently forced by magic adb2.txt file",
          MessageType.Info, true);
    ...
    if (!allowCacheServerChanges)
      EditorGUILayout.HelpBox(
          "Cache Server preferences currently forced via command line argument to " +
              overrideAddress +
              " and any changes here will not take effect until starting Unity without that command line argument.",
          MessageType.Info, true);

    if (EditorGUI.EndChangeCheck())
      s_HasPendingChanges = true;

    // Only commit changes when we don't have an active hot control, to avoid
    // restarting the cache server all the time while the slider is dragged,
    // slowing down the UI.
...
static void CacheServerVersion2GUI(bool allowCacheServerChanges,
                                   string overrideAddress) {
  GUILayout.Space(5);

  bool changeStateBeforeControls = GUI.changed;

  s_CacheServer2Mode = (CacheServer2Mode) EditorGUILayout.EnumPopup(
      Properties.cacheServerDefaultMode, s_CacheServer2Mode);

  s_CacheServer2IPAddress = EditorGUILayout.TextField(
      Properties.cacheServerIPLabel, s_CacheServer2IPAddress);
  ...
  var s =
      AssetDatabase.IsV2Enabled() ? s_ConnectionState : ConnectionState.Unknown;
...
private static void OnPreferencesReadGUI() {
  bool shouldTryConnect =
      s_ConnectionState == ConnectionState.Unknown &&
      (AssetDatabase.IsV1Enabled()
           ? s_CacheServerMode != CacheServerMode.Disabled
