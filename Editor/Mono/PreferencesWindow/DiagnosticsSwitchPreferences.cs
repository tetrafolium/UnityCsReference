...
internal class DiagnosticSwitchPreferences : SettingsProvider {
  private static class Styles {
    ...
    public static GUIContent restartNeededWarning =
        EditorGUIUtility.TrTextContent(
            "Some settings will not take effect until you restart Unity.");

    static Styles() {
      smallWarningIcon =
          EditorGUIUtility.LoadIconRequired("console.warnicon.sml");
    }
  }
  ...
  public override void OnFooterBarGUI() {
    var helpBox =
        GUILayoutUtility.GetRect(Styles.restartNeededWarning,
                                 EditorStyles.helpBox, GUILayout.MinHeight(40));
    if (m_HasAnyUnappliedSwitches)
      EditorGUI.HelpBox(helpBox, Styles.restartNeededWarning.text,
                        MessageType.Warning);
  }

  private static bool PassesFilter(DiagnosticSwitch diagnosticSwitch,
                                   string filterString) {
  ...
  private bool DisplaySwitch(DiagnosticSwitch diagnosticSwitch) {
    var labelText =
        new GUIContent(diagnosticSwitch.name, diagnosticSwitch.name + "\n\n" +
    ...
    var warningRect =
        new Rect(rowRect.x, rowRect.y, Styles.smallWarningIcon.width,
                 Styles.smallWarningIcon.height);
    if (m_HasAnyUnappliedSwitches) {
    ...
    if (diagnosticSwitch.value is bool) {
      diagnosticSwitch.persistentValue = EditorGUI.Toggle(
          rowRect, labelText, (bool) diagnosticSwitch.persistentValue);
    } else if (diagnosticSwitch.enumInfo != null) {
      if (diagnosticSwitch.enumInfo.isFlags) {
        // MaskField's "Everything" entry will set the value to 0xffffffff, but
        // that might mean that it has set bits that are not actually valid in
        // the enum. Correct for it by masking the value against the bit
        // patterns that are actually valid.
        int validMask = 0;
        foreach (int value in diagnosticSwitch.enumInfo.values)
          validMask |= value;

        // Also, many enums will provide a 'Nothing' entry at the bottom. If so
        // we need to chop it off, because MaskField can't cope with there being
        // two 'Nothing' entries.
        ...
        if (diagnosticSwitch.enumInfo.values[0] == 0) {
          ...
          Array.Copy(diagnosticSwitch.enumInfo.names, 1, names, 0,
                     names.Length);
          Array.Copy(diagnosticSwitch.enumInfo.values, 1, values, 0,
                     values.Length);
        }

        diagnosticSwitch.persistentValue =
            EditorGUI.MaskFieldInternal(rowRect, labelText,
                                        (int) diagnosticSwitch.persistentValue,
                                        names, values, EditorStyles.popup) &
            validMask;
      } else {
        var guiNames = new GUIContent[diagnosticSwitch.enumInfo.names.Length];
        for (int i = 0; i < diagnosticSwitch.enumInfo.names.Length; ++i)
          guiNames[i] =
              new GUIContent(diagnosticSwitch.enumInfo.names[i],
                             diagnosticSwitch.enumInfo.annotations[i]);
        diagnosticSwitch.persistentValue = EditorGUI.IntPopup(
            rowRect, labelText, (int) diagnosticSwitch.persistentValue,
            guiNames, diagnosticSwitch.enumInfo.values);
      }
      ...
      if ((maxValue - minValue <= kMaxRangeForSlider) &&
        ...
        diagnosticSwitch.persistentValue = (UInt32) EditorGUI.IntSlider(
            rowRect, labelText, (int)(UInt32) diagnosticSwitch.persistentValue,
            (int) minValue, (int) maxValue);
      } else {
        diagnosticSwitch.persistentValue = (UInt32) EditorGUI.IntField(
            rowRect, labelText, (int)(UInt32) diagnosticSwitch.persistentValue);
      }
      ...
      if ((maxValue - minValue <= kMaxRangeForSlider) &&
          (maxValue - minValue > 0) &&
          (minValue < int.MaxValue &&maxValue < int.MaxValue)) {
        diagnosticSwitch.persistentValue = EditorGUI.IntSlider(
            rowRect, labelText, (int) diagnosticSwitch.persistentValue,
            minValue, maxValue);
      } else {
        diagnosticSwitch.persistentValue = EditorGUI.IntField(
            rowRect, labelText, (int) diagnosticSwitch.persistentValue);
      }
    } else if (diagnosticSwitch.value is string) {
      diagnosticSwitch.persistentValue = EditorGUI.TextField(
          rowRect, labelText, (string) diagnosticSwitch.persistentValue);
    } else {
      var redStyle = new GUIStyle();
      redStyle.normal.textColor = Color.red;
      EditorGUI.LabelField(
          rowRect, labelText,
          EditorGUIUtility.TrTextContent("Unsupported type: " +
                                         diagnosticSwitch.value.GetType().Name),
          redStyle);
    }

    if (EditorGUI.EndChangeCheck())
      Debug.SetDiagnosticSwitch(diagnosticSwitch.name,
                                diagnosticSwitch.persistentValue, true);
