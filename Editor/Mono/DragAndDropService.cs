...
internal static class DragAndDropService {
  ...
  public delegate DragAndDropVisualMode
  SceneDropHandler(UnityEngine.Object dropUpon, Vector3 worldPosition,
  ...
  public delegate DragAndDropVisualMode
  InspectorDropHandler(UnityEngine.Object[] targets, bool perform);
  ...
  public static Action AddDropHandler(int dropDstId, Delegate handler) {
    if (HasHandler(dropDstId, handler)) {
      throw new Exception("Delegate already registered for dropDestinationId:" +
                          dropDstId);
    }
    ...
    Action off = () => { RemoveDropHandler(dropDstId, handler); };
    return off;
  }
  ...
  public static DragAndDropVisualMode Drop(int dropDstId,
                                           params object[] args) {
    List<Delegate> handlers;
...
#region Implementation
  ...
  static DragAndDropService() { Init(); }
  ...
  internal static DragAndDropVisualMode
  DefaultProjectBrowserDropHandler(int dragUponInstanceId, string dummy,
                                   bool perform) {
  ...
  internal static SceneDropHandler DefaultSceneDropHandler =
      InternalEditorUtility.SceneViewDrag;
  internal static InspectorDropHandler DefaultInspectorDropHandler =
      InternalEditorUtility.InspectorWindowDrag;
  internal static HierarchyDropHandler DefaultHierarchyDropHandler =
      InternalEditorUtility.HierarchyWindowDragByID;
