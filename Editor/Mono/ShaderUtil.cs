...
internal Shader SourceShader {
	get { return m_Data.SourceShader; }
}

public int PassCount {
	get {
		return ShaderUtil.GetShaderTotalPassCount(m_Data.SourceShader,
		                                          m_SubshaderIndex);
	}
}
...
public Pass GetPass(int passIndex) {
	if (passIndex < 0 || passIndex >= PassCount) {
		Debug.LogErrorFormat(
			"Pass index is incorrect: {0}, shader {1} has {2} passes.",
			passIndex, SourceShader, PassCount);
		return null;
	}
...
internal Shader SourceShader {
	get { return m_Subshader.m_Data.SourceShader; }
}
internal int SubshaderIndex {
	get { return m_Subshader.m_SubshaderIndex; }
}

public string SourceCode {
	get {
		return ShaderUtil.GetShaderPassSourceCode(SourceShader, SubshaderIndex,
		                                          m_PassIndex);
	}
}
public string Name {
	get {
		return ShaderUtil.GetShaderPassName(SourceShader, SubshaderIndex,
		                                    m_PassIndex);
	}
}
}
...
public int ActiveSubshaderIndex {
	get { return ShaderUtil.GetShaderActiveSubshaderIndex(SourceShader); }
}
public int SubshaderCount {
	get { return ShaderUtil.GetShaderSubshaderCount(SourceShader); }
}
...
public Subshader GetSubshader(int index) {
	if (index < 0 || index >= SubshaderCount) {
		Debug.LogErrorFormat(
			"Subshader index is incorrect: {0}, shader {1} has {2} passes.",
			index, SourceShader, SubshaderCount);
		return null;
	}
...
MaterialsUseInstancingShader(SerializedProperty materialsArray) {
	if (materialsArray.hasMultipleDifferentValues)
		return false;
	for (int i = 0; i < materialsArray.arraySize; ++i) {
		var material = materialsArray.GetArrayElementAtIndex(i)
		               .objectReferenceValue as Material;
		if (material != null && material.enableInstancing &&
		    material.shader != null && HasInstancing(material.shader))
			return true;
	}
	return false;
}
