...
private static SequencePoint
GetMethodFirstSequencePoint(MethodDefinition methodDefinition) {
...
private static MethodDefinition GetMethodByName(TypeDefinition typeDefinition,
                                                string methodName) {
	if (typeDefinition == null) {
		Debug.Log(
			"TypeDefinition cannot be null. Check whether the type exists in assembly.");
		return null;
	}

	if (!typeDefinition.HasMethods) {
		Debug.Log(string.Concat("TypeDefinition ", typeDefinition.Name,
...
FindTypeByFullName(AssemblyDefinition assemblyDefinition,
                   string typeFullName) {
	if (assemblyDefinition == null) {
		Debug.Log(
			"AssemblyDefinition cannot be null. Check if it's read correctly.");
		return null;
	}
...
AggregateAllTypeDefinitions(IEnumerable<TypeDefinition> types) {
	var typeDefs = types.ToList();
	foreach (var typeDefinition in types) {
		if (typeDefinition.HasNestedTypes)
			typeDefs.AddRange(
				AggregateAllTypeDefinitions(typeDefinition.NestedTypes));
	}
	return typeDefs;
}

private static SequencePoint GetSequencePointForMethod(string assemblyPath,
                                                       string typeFullName,
                                                       string methodName) {
...
public IFileOpenInfo TryGetCecilFileOpenInfo(Type type,
                                             MethodInfo methodInfo) {
	var assemblyPath = type.Assembly.Location;
	var methodName = methodInfo.Name;

	// Nested types are appended to the type name in reflection, but not in
	// Cecil
	var typeName = type.FullName ?? string.Empty;
	typeName = typeName.Contains("+") ? typeName.Split('+').First() : typeName;

	var sequencePoint =
		GetSequencePointForMethod(assemblyPath, typeName, methodName);

	var fileOpenInfo = new FileOpenInfo();
	if (sequencePoint !=
	    null) // Can be null in case of yield return in target method
	{
