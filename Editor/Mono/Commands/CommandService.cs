...
[ExcludeFromDocs, AttributeUsage(AttributeTargets.Method, Inherited = false,
                                 AllowMultiple = true)]
public class CommandHandlerAttribute : Attribute {
  ...
  public CommandHandlerAttribute(string id, string label)
      : this(id, label, CommandHint.Any) {}

  [UsedImplicitly, RequiredSignature]
  static void RequiredSignature(CommandExecuteContext context) {}
}
...
public static class CommandService {
  ...
  public static string GetCommandLabel(string commandId) {
    if (!Exists(commandId))
      throw new ArgumentException($"No command with id {commandId} exists",
                                  nameof(commandId));

    return s_Commands[commandId].label;
  }

  public static void RegisterCommand(string id, string label,
                                     CommandHandler handler,
                                     CommandHint hint = CommandHint.Any) {
    if (Exists(id))
      throw new ArgumentException($"A command with id {id} already exists",
                                  nameof(id));

    s_Commands[id] = new Command{id = id, label = label ?? id, hint = hint,
                                 handler = handler, managed = false};
  }

  public static void RegisterCommand(string id, CommandHandler handler,
                                     CommandHint hint = CommandHint.Any) {
  ...
  public static bool UnregisterCommand(string id) {
    ...
    if (s_Commands[id].managed)
      Debug.LogWarning(
          $"The command {id} was not explicitly registered by the user and should not be unregistered.");

    return s_Commands.Remove(id);
  }

  public static bool Exists(string id) { return s_Commands.ContainsKey(id); }
  ...
  private static IEnumerable<Command> ScanAttributes() {
    ...
    var commandAttributes =
        AttributeHelper.GetMethodsWithAttribute<CommandHandlerAttribute>();
    foreach (var method in commandAttributes.methodsWithAttributes) {
      var callback = Delegate.CreateDelegate(typeof(CommandHandler),
                                             method.info) as CommandHandler;
      if (callback == null)
        continue;
      ...
      if (commands.Any(c => c.id == attr.id)) {
        Debug.LogWarning(
            $"There is already a command with the ID {attr.id}. " +
            "Commands need to have a unique ID, i.e. \"Unity/Category/Command_42\".");
        continue;
      }

      commands.Add(new Command{id = attr.id, label = attr.label ?? attr.id,
                               hint = attr.hint, handler = callback,
                               managed = true});
    }
  ...
  private static object ExecuteCommand(string id, CommandHint hint,
                                       object[] args) {
    if (!Exists(id))
    ...
    var context =
        new CommandExecuteContext{hint = hint, args = args, result = null};
