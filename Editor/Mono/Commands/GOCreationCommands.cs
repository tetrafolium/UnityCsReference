...
internal static SavedBool s_PlaceObjectsAtWorldOrigin =
	new SavedBool("Create3DObject.PlaceAtWorldOrigin", false);

// This is here because we can't pass Scenes around with the MenuCommand
// context. SceneHierarchy toggles this flag when add object context menu
// items are invoked from a context click on Scene headers. If you make use of
// this, be sure to return it's value to 'false' as soon as your operation is
// out of scope.
internal static bool forcePlaceObjectsAtWorldOrigin;

static bool placeObjectsAtWorldOrigin {
	get {
		return s_PlaceObjectsAtWorldOrigin.value ||
		       forcePlaceObjectsAtWorldOrigin;
	}
}

private static void SetGameObjectParent(GameObject go,
...
internal static void Place(GameObject go, GameObject parent) {
	...
	if (parent != null) {
		...
		// When creating a 3D object without a parent, this option puts it at the
		// world origin instead of scene pivot.
	...
	// Only at this point do we know the actual parent of the object and can
	// modify its name accordingly.
...
static void CreateEmptyChild(MenuCommand menuCommand) {
	...
	// If selected GameObject is a Sub Scene header, place GameObject in active
	// scene similar to what happens when other scene headers are selected.
...
// Avoiding executing this method per-object, by adding menu item manually in
// SceneHierarchy
...
internal static void CreateEmptyParent() {
	...
	if (selected.Length > 0) {
		for (int i = 0; i < selected.Length; i++) {
			if (PrefabUtility.GetPrefabAssetType(selected[i].gameObject) !=
			    PrefabAssetType.NotAPrefab) {
				selected[i] =
					PrefabUtility
					.GetOutermostPrefabInstanceRoot(selected[i].gameObject)
					.transform;
			}
		}
	}

	// Selection.transform does not provide correct list order, so we have to do
	// it manually
	selected = selected.ToList().OrderBy(g => g.GetSiblingIndex()).ToArray();

	GameObject parent = Selection.activeTransform !=
	                    null ? Selection.activeTransform.gameObject : null;
	Transform sibling = null;

	if (parent != null) {
		sibling = parent.transform;
		parent = parent.transform.parent !=
		         null ? parent.transform.parent.gameObject : null;
	}
	...
	if (selected.Length > 0) {
		foreach (var gameObject in selected) {
			if (gameObject != null) {
				Undo.SetTransformParent(gameObject.transform, go.transform,
				                        "Reparenting");
				gameObject.transform.SetAsLastSibling();
			}
		}

		SceneHierarchyWindow.lastInteractedHierarchyWindow.SetExpanded(
			go.GetInstanceID(), true);
	}
}
...
internal static bool ValidateCreateEmptyParent() {
	...
	if (selected.Length == 0) {
		...
		// Check if selected objects are under the same parent and in the same
		// scene
		...
		// Check if we are not trying to create parent object for root object if
		// we are in prefab stage
		if (StageNavigationManager.instance.currentStage is PrefabStage) {
			GameObject rootGameObject = PrefabStageUtility.GetCurrentPrefabStage()
...
static void CreateCube(MenuCommand menuCommand) {
	CreateAndPlacePrimitive(PrimitiveType.Cube,
...
static void CreateSprite(MenuCommand menuCommand) {
	var parent = menuCommand.context as GameObject;
	var go =
		ObjectFactory.CreateGameObject("New Sprite", typeof(SpriteRenderer));
...
static void CreateDirectionalLight(MenuCommand menuCommand) {
	...
	go.GetComponent<Transform>().SetLocalEulerAngles(new Vector3(50, -30, 0),
...
static void CreateSpotLight(MenuCommand menuCommand) {
	...
	go.GetComponent<Transform>().SetLocalEulerAngles(new Vector3(90, 0, 0),
	                                                 RotationOrder.OrderZXY);
...
static void CreateReflectionProbe(MenuCommand menuCommand) {
	var parent = menuCommand.context as GameObject;
	Place(ObjectFactory.CreateGameObject("Reflection Probe",
...
static void CreateLightProbeGroup(MenuCommand menuCommand) {
	var parent = menuCommand.context as GameObject;
	Place(ObjectFactory.CreateGameObject("Light Probe Group",
...
static void CreateAudioSource(MenuCommand menuCommand) {
	var parent = menuCommand.context as GameObject;
	Place(ObjectFactory.CreateGameObject("Audio Source", typeof(AudioSource)),
	      parent);
}
...
static void CreateAudioReverbZone(MenuCommand menuCommand) {
	var parent = menuCommand.context as GameObject;
	Place(ObjectFactory.CreateGameObject("Audio Reverb Zone",
...
static void CreateVideoPlayer(MenuCommand menuCommand) {
	var parent = menuCommand.context as GameObject;
	Place(ObjectFactory.CreateGameObject("Video Player", typeof(VideoPlayer)),
	      parent);
}
...
static void CreateParticleSystemForceField(MenuCommand menuCommand) {
	var parent = menuCommand.context as GameObject;
	Place(ObjectFactory.CreateGameObject("Particle System Force Field",
...
static void CreateTrail(MenuCommand menuCommand) {
	...
	go.GetComponent<TrailRenderer>().material =
...
static void CreateCamera(MenuCommand menuCommand) {
	var parent = menuCommand.context as GameObject;
	Place(ObjectFactory.CreateGameObject("Camera", typeof(Camera),
