...
internal partial class ObjectListArea {
  ...
  internal delegate void OnAssetIconDrawDelegate(Rect iconRect, string guid,
                                                 bool isListMode);
  internal static event OnAssetIconDrawDelegate postAssetIconDrawCallback =
      null;
  ...
  class LocalGroup : Group {
    ...
    List<int> m_DragSelection = new List<int>(); // Temp instanceID state while
                                                 // dragging (not serialized)
    ...
    public SearchFilter searchFilter {
      get { return m_FilteredHierarchy.searchFilter; }
    }
    ...
    public bool HasBuiltinResources {
      get { return m_CurrentBuiltinResources.Length > 0; }
    }
    ...
    public override void UpdateAssets() {
      ...
      if (m_FilteredHierarchy.hierarchyType == HierarchyType.Assets)
        ...
        m_ActiveBuiltinList =
            new BuiltinResource[0]; // The Scene tab does not display builtin
                                    // resources

      ItemsAvailable =
          m_FilteredHierarchy.results.Length + m_ActiveBuiltinList.Length;
    }

    protected override float GetHeaderHeight() { return 0f; }

    override protected void DrawHeader(float yOffset, bool collapsable) {
      if (GetHeaderHeight() > 0f) {
        Rect rect = new Rect(0, GetHeaderYPosInScrollArea(yOffset),
                             m_Owner.GetVisibleWidth(), kGroupSeparatorHeight);
        ...
        if (collapsable) {
          ...
          Visible = GUI.Toggle(new Rect(rect.x, rect.y, 14, rect.height),
                               Visible, GUIContent.none, s_Styles.groupFoldout);
        ...
        if (m_Owner.drawLocalAssetHeader != null)
          usedWidth = m_Owner.drawLocalAssetHeader(rect) +
                      10f; // add space between arrow and count
    ...
    protected override void DrawInternal(int beginIndex, int endIndex,
                                         float yOffset) {
      ...
      if (!ListMode && isFolderBrowsing)
        DrawSubAssetBackground(beginIndex, endIndex,
                               yOffset); // only show sub asset bg when not
                                         // searching i.e. folder browsing
      ...
      while (true) {
        ...
        if (IsCreatingAtThisIndex(itemIndex)) {
          ...
          DrawItem(m_Grid.CalcRect(itemIndex, yOffset), null, newAsset,
                   isFolderBrowsing);
      ...
      if (m_ActiveBuiltinList.Length > 0) {
        ...
        for (int builtinIdx = builtinStartIdx;
             builtinIdx < m_ActiveBuiltinList.Length && itemIndex <= endIndex;
             itemIndex++, builtinIdx++) {
          DrawItem(m_Grid.CalcRect(itemIndex, yOffset), null,
                   m_ActiveBuiltinList[builtinIdx], isFolderBrowsing);
        }
      }

      // Repaint again if we are in preview icon mode and previews are being
      // loaded
      if (!ListMode && AssetPreview.IsLoadingAssetPreviews(
                           m_Owner.GetAssetPreviewManagerID()))
        m_Owner.Repaint();
    }
    ...
    void AddDirtyStateFor(int instanceID) {
      ...
      m_LastRenderedAssetDirtyCounts.Add(
          EditorUtility.GetDirtyCount(instanceID));
    }

    public bool IsAnyLastRenderedAssetsDirty() {
      for (int i = 0; i < m_LastRenderedAssetInstanceIDs.Count; ++i) {
        int dirtyCount =
            EditorUtility.GetDirtyCount(m_LastRenderedAssetInstanceIDs[i]);
    ...
    protected override void HandleUnusedDragEvents(float yOffset) {
      ...
      case EventType.DragPerform:
        Rect localGroupRect = new Rect(0, yOffset, m_Owner.m_TotalRect.width,
                                       m_Owner.m_TotalRect.height >
                                           Height ? m_Owner.m_TotalRect.height
                                       : Height);
        if (localGroupRect.Contains(evt.mousePosition)) {
          DragAndDropVisualMode mode;
          bool isFolderBrowsing =
              (m_FilteredHierarchy.searchFilter.GetState() ==
               SearchFilter.State.FolderBrowsing);
          if (isFolderBrowsing &&
              m_FilteredHierarchy.searchFilter.folders.Length == 1) {
            ...
            // Disallow drop: more than one folder or search is active, since
            // dropping would be ambiguous.
    ...
    void HandleMouseWithDragging(ref AssetReference assetReference,
                                 int controlID, Rect rect) {
      ...
      case EventType.MouseDown:
        if (Event.current.button == 0 &&
          ...
          if (evt.clickCount == 2) {
            // Double clicked
            var newSelection =
                GetNewSelection(ref assetReference, false, false);
            ...
            if (controlID == oldItemControlID) {
              ...
              DragAndDropDelay delay =
                  (DragAndDropDelay) GUIUtility.GetStateObject(
                      typeof(DragAndDropDelay), controlID);
              delay.mouseDownPosition = Event.current.mousePosition;
              m_Owner.ScrollToPosition(
                  ObjectListArea.AdjustRectForFraming(rect));
            } else {
          ...
          evt.Use();
                   ...
                   rect.Contains(Event.current.mousePosition)) {
          // Right mouse down selection (do NOT use event since we need
          // ContextClick event, which is not fired if right click is used)
          m_Owner.SetSelection(
              GetNewSelection(ref assetReference, true, false).ToArray(),
              false);
        }
        break;
      ...
      case EventType.DragPerform: {
        ...
        if (rect.Contains(evt.mousePosition)) {
          DragAndDropVisualMode mode =
              DoDrag(assetReference.instanceID, perform);
        ...
        if (GUIUtility.hotControl == controlID) {
          if (rect.Contains(evt.mousePosition)) {
            ...
            if (ListMode) {
              ...
              rect.y =
                  rect.y + rect.height - s_Styles.resultsGridLabel.fixedHeight;
              rect.height = s_Styles.resultsGridLabel.fixedHeight;
              clickedOnText = rect.Contains(evt.mousePosition);
            }

            List<int> selected = m_Owner.m_State.m_SelectedInstanceIDs;
            if (clickedOnText && m_Owner.allowRenaming &&
                m_Owner.m_AllowRenameOnMouseUp && selected.Count == 1 &&
                selected[0] == assetReference.instanceID &&
                !EditorGUIUtility.HasHolddownKeyModifiers(evt)) {
      ...
      case EventType.ContextClick:
        ...
        if (overlayPos.width != rect.width && Provider.isActive &&
            overlayPos.Contains(evt.mousePosition)) {
          EditorUtility.DisplayPopupMenu(
              new Rect(evt.mousePosition.x, evt.mousePosition.y, 0, 0),
              "Assets/Version Control", new MenuCommand(null, 0));
          evt.Use();
        }
        break;
      }
    ...
    void HandleMouseWithoutDragging(ref AssetReference assetReference,
                                    int controlID, Rect position) {
      Event evt = Event.current;
      ...
      case EventType.MouseDown:
        if (evt.button == 0 && position.Contains(evt.mousePosition)) {
          ...
          if (evt.clickCount == 1) {
            m_Owner.ScrollToPosition(
                ObjectListArea.AdjustRectForFraming(position));
          }
      ...
      case EventType.ContextClick:
        if (position.Contains(evt.mousePosition)) {
          ...
          overlayPos.x += 2;
          overlayPos = ProjectHooks.GetOverlayRect(overlayPos);
          if (overlayPos.width != position.width && Provider.isActive &&
              overlayPos.Contains(evt.mousePosition)) {
            EditorUtility.DisplayPopupMenu(
                new Rect(evt.mousePosition.x, evt.mousePosition.y, 0, 0),
                "Assets/Version Control", new MenuCommand(null, 0));
    ...
    public void ChangeExpandedState(int instanceID, bool expanded) {
      ...
      m_FilteredHierarchy.RefreshVisibleItems(
          m_Owner.m_State.m_ExpandedInstanceIDs);
    }
    ...
    bool IsRenaming(int instanceID) {
      ...
      return renameOverlay.IsRenaming() &&
             renameOverlay.userData == instanceID &&
             !renameOverlay.isWaitingForDelay;
    }

    protected void DrawSubAssetRowBg(int startSubAssetIndex,
                                     int endSubAssetIndex, bool continued,
                                     float yOffset) {
      ...
      float adjustStart =
          startIsOnFirstColumn ? 18f * fraction : m_Grid.horizontalSpacing +
          fraction * 10f;
      Rect rect =
          new Rect(startRect.x - adjustStart, startRect.y + shrinkHeight,
                   texWidth * fraction,
                   startRect.width - shrinkHeight * 2 + overflowHeight - 1);
      rect.y = Mathf.Round(rect.y);
      rect.height = Mathf.Ceil(rect.height);
      s_Styles.subAssetBg.Draw(rect, GUIContent.none, false, false, false,
                               false);

      // End
      float scaledWidth = texWidth * fraction;
      bool endIsOnLastColumn =
          (endSubAssetIndex % m_Grid.columns) == (m_Grid.columns - 1);
      float extendEnd =
          (continued || endIsOnLastColumn) ? 16 * fraction : 8 * fraction;
      Rect rect2 = new Rect(endRect.xMax - scaledWidth + extendEnd,
                            endRect.y + shrinkHeight, scaledWidth, rect.height);
      rect2.y = Mathf.Round(rect2.y);
      rect2.height = Mathf.Ceil(rect2.height);
      GUIStyle endStyle = continued ? s_Styles.subAssetBgOpenEnded
          : s_Styles.subAssetBgCloseEnded;
      ...
      rect.y = Mathf.Round(rect.y);
      rect.height = Mathf.Ceil(rect.height);
      s_Styles.subAssetBgMiddle.Draw(rect, GUIContent.none, false, false, false,
                                     false);
    }
        ...
        if (startSubAssetIndex != -1) {
          ...
          if (y < rows - 1) {
            ...
            if (indexFirstColumnNextRow < results.Length)
              continued =
                  !results[indexFirstColumnNextRow].isMainRepresentation;
          }
    ...
    void DrawItem(Rect position, FilteredHierarchy.FilterResult filterItem,
                  BuiltinResource builtinResource, bool isFolderBrowsing) {
      System.Diagnostics.Debug.Assert(
          (filterItem != null && builtinResource == null) ||
          (builtinResource != null &&
           filterItem == null)); // only one should be valid
      ...
      if (filterItem != null) {
        ...
        showFoldout =
            filterItem.hasChildren && !filterItem.isFolder &&
            isFolderBrowsing; // we do not want to be able to expand folders
      ...
      bool selected;
      if (m_Owner.allowDragging)
        selected = m_DragSelection.Count > 0
                       ? m_DragSelection.Contains(assetReference.instanceID)
                       : m_Owner.IsSelected(assetReference.instanceID);
      else
        selected = m_Owner.IsSelected(assetReference.instanceID);

      if (selected &&
          assetReference.instanceID == m_Owner.m_State.m_LastClickedInstanceID)
        m_LastClickedDrawTime = EditorApplication.timeSinceStartup;

      Rect foldoutRect =
          new Rect(position.x + s_Styles.groupFoldout.margin.left, position.y,
                   s_Styles.groupFoldout.padding.left,
                   position.height); // ListMode foldout
      if (showFoldout && !ListMode) {
        ...
        foldoutRect = new Rect(
            position.xMax - buttonWidth * 0.5f,
            position.y +
                (position.height - s_Styles.resultsGridLabel.fixedHeight) *
                    0.5f -
                buttonWidth * 0.5f,
            buttonWidth, buttonHeight);
        // foldoutRect = new Rect(position.xMax - 16, position.yMax - 16 -
        // s_Styles.resultsGridLabel.fixedHeight, 16, 16);   // bottom right
        // corner
      }

      bool toggleState = false;
      if (selected && evt.type == EventType.KeyDown &&
          m_Owner.HasFocus()) // We need to ensure we have keyboard focus
                              // because rename overlay might have it - and need
                              // the key events)
      {
        switch (evt.keyCode) {
        // Fold in
      ...
      // Foldout mouse button logic (rendering the item itself can be found
      // below)
      if (showFoldout && evt.type == EventType.MouseDown && evt.button == 0 &&
      ...
      if (toggleState) {
        ...
        if (expanded)
          m_ItemFader.Start(m_FilteredHierarchy.GetSubAssetInstanceIDs(
              assetReference.instanceID));
      ...
      if (!ListMode)
        labelRect =
            new Rect(position.x,
                     position.yMax + 1 - s_Styles.resultsGridLabel.fixedHeight,
                     position.width - 1, s_Styles.resultsGridLabel.fixedHeight);

      int vcPadding = Provider.isActive &&
                      ListMode ? k_ListModeVersionControlOverlayPadding : 0;
      ...
      // Draw section
      if (Event.current.type == EventType.Repaint) {
        if (m_DropTargetControlID == controlID &&
            !position.Contains(evt.mousePosition))
          m_DropTargetControlID = 0;
        bool isDropTarget =
            controlID == m_DropTargetControlID &&
            m_DragSelection.IndexOf(m_DropTargetControlID) == -1;

        string labeltext =
            filterItem != null ? filterItem.name : builtinResource.m_Name;
        if (ListMode) {
          if (isRenaming) {
            selected = false;
            labeltext = "";
          ...
          if (string.IsNullOrEmpty(assetReference.guid) &&
              m_Owner.GetCreateAssetUtility().instanceID ==
                  assetReference.instanceID &&
              m_Owner.GetCreateAssetUtility().icon != null) {
            ...
            if (icon == null) {
              if (assetReference.instanceID != 0)
                icon = AssetPreview.GetAssetPreview(
                    assetReference.instanceID,
                    m_Owner.GetAssetPreviewManagerID());
              else if (!string.IsNullOrEmpty(assetReference.guid))
                icon = AssetPreview.GetAssetPreviewFromGUID(
                    assetReference.guid, m_Owner.GetAssetPreviewManagerID());
            }
          }

          if (selected)
            s_Styles.resultsLabel.Draw(position, GUIContent.none, false, false,
                                       selected, m_Owner.HasFocus());

          if (isDropTarget)
            s_Styles.resultsLabel.Draw(position, GUIContent.none, true, true,
                                       false, false);

          DrawIconAndLabel(
              new Rect(contentStartX, position.y,
                       position.width - contentStartX, position.height),
              filterItem, labeltext, icon, selected, m_Owner.HasFocus());

          // Foldout
          if (showFoldout)
            s_Styles.groupFoldout.Draw(foldoutRect, !ListMode, !ListMode,
                                       IsExpanded(assetReference.instanceID),
                                       false);
        } else // Icon grid
        {
          // Get icon
          ...
          if (string.IsNullOrEmpty(assetReference.guid) &&
            ...
            Texture image = null;
            if (assetReference.instanceID != 0)
              image = AssetPreview.GetAssetPreview(
                  assetReference.instanceID,
                  m_Owner.GetAssetPreviewManagerID());
            else if (!string.IsNullOrEmpty(assetReference.guid))
              image = AssetPreview.GetAssetPreviewFromGUID(
                  assetReference.guid, m_Owner.GetAssetPreviewManagerID());
            ...
            if (filterItem != null) {
              ...
              // When folder browsing sub assets are shown on a background slate
              // and do not need rounded corner overlay
          ...
          float padding =
              (drawDropShadow)
                  ? 2.0f
                  : 0.0f; // the padding compensates for the drop shadow (so it
                          // doesn't get too close to the label text)
          position.height -=
              s_Styles.resultsGridLabel.fixedHeight +
              2 * padding; // get icon rect (remove label height which is
                           // included in the position rect)
          position.y += padding;

          Rect actualImageDrawPosition =
              (m_Content.image == null)
                  ? new Rect()
                  : ActualImageDrawPosition(position, m_Content.image.width,
                                            m_Content.image.height);
          m_Content.text = null;
          float alpha = 1f;
          if (filterItem != null) {
            ...
            // Draw static preview bg color as bg for small textures and
            // non-square textures
            if (drawDropShadow &&
                filterItem.iconDrawStyle == IconDrawStyle.NonTexture)
              s_Styles.previewBg.Draw(actualImageDrawPosition, GUIContent.none,
          ...
          if (m_Content.image != null) {
            ...
            if (alpha < 1f)
              GUI.color =
                  new Color(GUI.color.r, GUI.color.g, GUI.color.b, alpha);

            s_Styles.resultsGrid.Draw(actualImageDrawPosition, m_Content, false,
                                      false, selected, m_Owner.HasFocus());
          ...
          if (drawDropShadow) {
            Rect borderPosition =
                new RectOffset(1, 1, 1, 1)
                    .Remove(s_Styles.textureIconDropShadow.border.Add(
                        actualImageDrawPosition));
            s_Styles.textureIconDropShadow.Draw(
                borderPosition, GUIContent.none, false, false,
                selected || isDropTarget,
                m_Owner.HasFocus() || isRenaming || isDropTarget);
          }

          // Draw label
          if (!isRenaming) {
            if (isDropTarget)
              s_Styles.resultsLabel.Draw(
                  new Rect(labelRect.x - 10, labelRect.y, labelRect.width + 20,
                           labelRect.height),
                  GUIContent.none, true, true, false, false);

            labeltext = m_Owner.GetCroppedLabelText(assetReference, labeltext,
                                                    position.width);
            var labelNewRect =
                s_Styles.resultsGridLabel.CalcSizeWithConstraints(
                    GUIContent.Temp(labeltext), position.size);
            labelRect.x = position.x + (position.width - labelNewRect.x) / 2.0f;
            labelRect.width = labelNewRect.x;
            s_Styles.resultsGridLabel.Draw(labelRect, labeltext, false, false,
                                           selected, m_Owner.HasFocus());
          }
          ...
          if (showFoldout) {
            ...
            style.Draw(foldoutRect, !ListMode, !ListMode,
                       IsExpanded(assetReference.instanceID), false);
          }
      ...
      if (isRenaming) {
        if (ListMode) {
          float iconOffset = vcPadding + k_IconWidth +
                             k_SpaceBetweenIconAndText +
                             s_Styles.resultsLabel.margin.left;
      ...
      // User hook for rendering stuff on top of items (notice it being called
      // after rendering but before mouse handling to make user able to react on
      // mouse events)
      if (EditorApplication.projectWindowItemOnGUI != null &&
          filterItem != null && m_Owner.allowUserRenderingHook)
        EditorApplication.projectWindowItemOnGUI(filterItem.guid, itemRect);

      // Mouse handling (must be after rename overlay to ensure overlay get
      // mouseevents)
      if (m_Owner.allowDragging)
    ...
    private static Rect ActualImageDrawPosition(Rect position, float imageWidth,
                                                float imageHeight) {
      if (imageWidth > position.width || imageHeight > position.height) {
        Rect screenRect = new Rect();
        Rect sourceRect = new Rect();
        float imageAspect = imageWidth / imageHeight;
        GUI.CalculateScaledTextureRects(position, ScaleMode.ScaleToFit,
                                        imageAspect, ref screenRect,
                                        ref sourceRect);
        return screenRect;
      } else {
        float x =
            position.x + Mathf.Round((position.width - imageWidth) / 2.0f);
        float y =
            position.y + Mathf.Round((position.height - imageHeight) / 2.0f);
    ...
    public List<KeyValuePair<string, int>> GetVisibleNameAndInstanceIDs() {
      ...
      if (m_NoneList.Length > 0)
        result.Add(new KeyValuePair<string, int>(
            m_NoneList[0].m_Name, m_NoneList[0].m_InstanceID)); // 0
      ...
      for (int i = 0; i < m_ActiveBuiltinList.Length; ++i)
        result.Add(
            new KeyValuePair<string, int>(m_ActiveBuiltinList[i].m_Name,
                                          m_ActiveBuiltinList[i].m_InstanceID));
    ...
    public void GetAssetReferences(out List<int> instanceIDs,
                                   out List<string> guids) {
      ...
      // 2. Project Assets
      foreach (FilteredHierarchy.FilterResult r in m_FilteredHierarchy
                   .results) {
    ...
    public List<int> GetNewSelection(ref AssetReference clickedAssetReference,
                                     bool beginOfDrag,
      ...
      return InternalEditorUtility.GetNewSelection(
          ref clickedAssetReference, instanceIDs, guids, selectedInstanceIDs,
          lastClickedInstanceID, beginOfDrag, useShiftAsActionKey,
          allowMultiselection);
    }

    public override void UpdateFilter(HierarchyType hierarchyType,
    ...
    private void RefreshHierarchy(HierarchyType hierarchyType,
                                  SearchFilter searchFilter,
                                  bool foldersFirst) {
    ...
    void RefreshBuiltinResourceList(SearchFilter searchFilter) {
      ...
      if (!m_Owner.allowBuiltinResources ||
          (searchFilter.GetState() == SearchFilter.State.FolderBrowsing) ||
          (searchFilter.GetState() == SearchFilter.State.EmptySearchFilter)) {
        m_CurrentBuiltinResources = new BuiltinResource[0];
        return;
      }

      List<BuiltinResource> currentBuiltinResources =
          new List<BuiltinResource>();

      // Filter by assets labels (Builtins have no asset labels currently)
      if (searchFilter.assetLabels != null &&
          searchFilter.assetLabels.Length > 0) {
      ...
      if (requiredClassIDs.Count > 0) {
        foreach (KeyValuePair<string, BuiltinResource[]> kvp in
                     m_BuiltinResourceMap) {
          UnityType classType = UnityType.FindTypeByName(kvp.Key);
          if (classType == null)
            continue;

          foreach (int requiredClassID in requiredClassIDs) {
            if (classType.IsDerivedFrom(
                    UnityType.FindTypeByPersistentTypeID(requiredClassID)))
      ...
      if (builtinList.Length > 0 &&
          !string.IsNullOrEmpty(searchFilter.nameFilter)) {
        List<BuiltinResource> filtered =
            new List<BuiltinResource>(); // allocated here to prevent from
    ...
    private void InitBuiltinAssetType(System.Type type) {
      ...
      if (unityType == null) {
        Debug.LogWarning("ObjectSelector::InitBuiltinAssetType: class '" +
                         typeName + "' not found");
        return;
      }

      BuiltinResource[] resourceList =
          EditorGUIUtility.GetBuiltinResourceList(unityType.persistentTypeID);
    ...
    // Can return an index 1 past end of existing items (if newText is last in
    // sort)
    public int IndexOfNewText(string newText, bool isCreatingNewFolder,
                              bool foldersFirst) {
      int idx = 0;
      if (m_ShowNoneItem)
        idx++;
      ...
      for (; idx < m_FilteredHierarchy.results.Length; ++idx) {
        ...
        // When inserting a folder in folders first list break when we reach
        // normal assets
        if (foldersFirst && !r.isFolder && isCreatingNewFolder)
          break;

        // Use same name compare as when we sort in the backend: See
        // AssetDatabase.cpp: SortChildren
        string propertyPath = AssetDatabase.GetAssetPath(r.instanceID);
        if (EditorUtility.NaturalCompare(
                System.IO.Path.GetFileNameWithoutExtension(propertyPath),
                newText) > 0) {
          return idx;
        }
      }
    ...
    public int IndexOf(int instanceID) {
      ...
      foreach (FilteredHierarchy.FilterResult r in m_FilteredHierarchy
                   .results) {
        // When creating new asset we jump over that item (assuming we do not
        // search for that new asset)
    ...
    // Returns true if index was valid. Note that instance can be 0 if 'None'
    // item was found at index
    public bool AssetReferenceAtIndex(int index,
    ...
    public virtual void StartDrag(int draggedInstanceID,
    ...
    public DragAndDropVisualMode DoDrag(int dragToInstanceID, bool perform) {
      return DragAndDropService.Drop(
          DragAndDropService.kProjectBrowserDropDstId, dragToInstanceID,
          AssetDatabase.GetAssetPath(dragToInstanceID), perform);
    }
    ...
    public bool DoCharacterOffsetSelection() {
      if (Event.current.type == EventType.KeyDown && Event.current.shift &&
        ...
        System.StringComparison ignoreCase =
        ...
        // If same c is same start char as current selected then find current
        // selected index
        int startIndex = 0;
    ...
    public void DrawIconAndLabel(Rect rect,
                                 FilteredHierarchy.FilterResult filterItem,
                                 string label, Texture2D icon, bool selected,
                                 bool focus) {
      float vcPadding =
      ...
      float delegateDrawWidth =
          (k_ListModeExternalIconPadding * 2) + k_IconWidth;
      Rect delegateDrawRect = new Rect(rect.xMax - delegateDrawWidth, rect.y,
                                       delegateDrawWidth, rect.height);
      Rect labelRect = new Rect(rect);
      if (DrawExternalPostLabelInList(delegateDrawRect, filterItem)) {
        labelRect.width = (rect.width - delegateDrawWidth);
      }

      s_Styles.resultsLabel.padding.left =
          (int)(vcPadding + k_IconWidth + k_SpaceBetweenIconAndText);
      s_Styles.resultsLabel.Draw(labelRect, label, false, false, selected,
                                 focus);
      ...
      if (filterItem != null && filterItem.guid != null &&
          filterItem.isMainRepresentation) {
    ...
    private static bool
    DrawExternalPostLabelInList(Rect drawRect,
                                FilteredHierarchy.FilterResult filterItem) {
      bool didDraw = false;
      if (filterItem != null && filterItem.guid != null &&
          filterItem.isMainRepresentation) {
    ...
    class ItemFader {
      ...
      public void Start(List<int> instanceIDs) {
        ...
        if (m_InstanceIDs.Count > 1)
          m_TimeBetweenEachItem =
              m_FirstToLastDuration / (m_InstanceIDs.Count - 1);
      }
      ...
      public float GetAlpha(int instanceID) {
        ...
        if (EditorApplication.timeSinceStartup >
            m_FadeStartTime + m_FadeDuration + m_FirstToLastDuration) {
        ...
        if (index >= 0) {
          ...
          if (itemStartTime < elapsed) {
            alpha = Mathf.Clamp(
                (float)((elapsed - itemStartTime) / m_FadeDuration), 0f, 1f);
          }
          return alpha;
        }
        return 1f;
