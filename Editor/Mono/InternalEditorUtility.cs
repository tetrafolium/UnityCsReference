...
partial class InternalEditorUtility {
  public static Texture2D FindIconForFile(string fileName) {
    ...
    switch (extension) {
    // Most .asset files use their scriptable object defined icon instead of a
    // default one.
    case "asset":
      return AssetDatabase
          .GetCachedIcon(fileName)
              as Texture2D ?? EditorGUIUtility.FindTexture("GameManager Icon");
    ...
    case "mixer":
      return EditorGUIUtility.FindTexture(
          typeof(UnityEditor.Audio.AudioMixerController));
    case "uxml":
      return EditorGUIUtility.FindTexture(
    ...
    case "lighting":
      return EditorGUIUtility.FindTexture(typeof(UnityEngine.LightingSettings));
    ...
    case "xvid":
      return AssetDatabase
          .GetCachedIcon(fileName)
              as Texture2D ?? EditorGUIUtility.FindTexture(typeof(VideoClip));
  ...
  public static Texture2D GetIconForFile(string fileName) {
    return FindIconForFile(fileName)
        ?? EditorGUIUtility.FindTexture(typeof(DefaultAsset));
  }
  ...
  public static void SaveEditorSettingsList(string prefix, string[] aList,
                                            int count) {
    int i;
  ...
  public static string TextAreaForDocBrowser(Rect position, string text,
                                             GUIStyle style) {
    int id = EditorGUIUtility.GetControlID(
        "TextAreaWithTabHandling".GetHashCode(), FocusType.Keyboard, position);
    ...
    bool dummy;
    text =
        EditorGUI.DoTextField(editor, id, EditorGUI.IndentedRect(position),
                              text, style, null, out dummy, false, true, false);
    return text;
  }
  ...
  internal struct AssetReference {
    // The guid is always valid. Assets not yet present will not have an
    // instanceID yet. Could be because it is on-demand imported.
    public string guid;
    public int instanceID; // instanceID of an object in an asset if the asset
                           // is available in imported form. Else 0.

    public sealed class GuidThenInstanceIDEqualityComparer
        : IEqualityComparer<AssetReference> {
      ...
      public int GetHashCode(AssetReference assetReference) {
        return (assetReference.instanceID * 397) ^
               (assetReference.guid !=
                null ? assetReference.guid.GetHashCode() : 0);
      }
    }
  }

  public static List<int>
  GetNewSelection(int clickedInstanceID, List<int> allInstanceIDs,
                  List<int> selectedInstanceIDs, int lastClickedInstanceID,
                  bool keepMultiSelection, bool useShiftAsActionKey,
                  bool allowMultiSelection) {
    return GetNewSelection(
        clickedInstanceID, allInstanceIDs, selectedInstanceIDs,
        lastClickedInstanceID, keepMultiSelection, useShiftAsActionKey,
        allowMultiSelection, Event.current.shift, EditorGUI.actionKey);
  }
  ...
  GetNewSelection(int clickedInstanceID, List<int> allInstanceIDs,
                  List<int> selectedInstanceIDs, int lastClickedInstanceID,
                  bool keepMultiSelection, bool useShiftAsActionKey,
                  bool allowMultiSelection, bool shiftKeyIsDown,
                  bool actionKeyIsDown) {
    List<string> allGuids = null;
    var clicked =
        new AssetReference(){guid = null, instanceID = clickedInstanceID};

    return GetNewSelection(
        ref clicked, allInstanceIDs, allGuids, selectedInstanceIDs,
        lastClickedInstanceID, keepMultiSelection, useShiftAsActionKey,
        allowMultiSelection, shiftKeyIsDown, actionKeyIsDown);
  }
  ...
  internal static bool TrySetInstanceId(ref AssetReference entry) {
    ...
    var hash =
        UnityEditor.Experimental.AssetDatabaseExperimental.ProduceArtifactAsync(
            new ArtifactKey(lookupGUID));
    if (!hash.isValid)
      return false;
  ...
  internal static List<int> TryGetInstanceIds(List<int> entryInstanceIds,
                                              List<string> entryInstanceGuids,
                                              int from, int to) {
    ...
    if (guids.Count == 0) {
      ...
      var hashes = UnityEditor.Experimental.AssetDatabaseExperimental
                       .ProduceArtifactsAsync(guids.ToArray());
  ...
  internal static List<int>
  GetNewSelection(ref AssetReference clickedEntry,
                  List<int> allEntryInstanceIDs, List<string> allEntryGuids,
                  List<int> selectedInstanceIDs, int lastClickedInstanceID,
                  bool keepMultiSelection, bool useShiftAsActionKey,
                  bool allowMultiSelection) {
    return GetNewSelection(ref clickedEntry, allEntryInstanceIDs, allEntryGuids,
                           selectedInstanceIDs, lastClickedInstanceID,
                           keepMultiSelection, useShiftAsActionKey,
                           allowMultiSelection, Event.current.shift,
                           EditorGUI.actionKey);
  }
  ...
  GetNewSelection(ref AssetReference clickedEntry,
    ...
    // Select everything between the first selected object and the selected
      ...
      int firstIndex;
      int lastIndex;
      if (!GetFirstAndLastSelected(allEntryInstanceIDs, selectedInstanceIDs,
                                   out firstIndex, out lastIndex)) {
        // We had no selection
      ...
      System.Diagnostics.Debug.Assert(
          newIndex != -1); // new item should be part of visible folder set
      int dir = 0;
      if (prevIndex != -1)
      ...
      // Outcomment to debug
      // Debug.Log (clickedEntry + ",   firstIndex " + firstIndex + ", lastIndex
      // " + lastIndex + ",    newIndex " + newIndex + " " + ", lastClickedIndex
      // " + prevIndex + ",     from " + from + ", to " + to);
      if (allEntryGuids == null)
        return allEntryInstanceIDs.GetRange(from, to - from + 1);

      var foundInstanceIDs =
          TryGetInstanceIds(allEntryInstanceIDs, allEntryGuids, from, to);
    ...
    // Just set the selection to the clicked object
      ...
      if (keepMultiSelection) {
        ...
        // This is for dragging in case with multiple items selected or right
        // click (mouse down should not unselect the rest).
  ...
  static bool GetFirstAndLastSelected(List<int> allEntries,
  ...
  GetApplicationExtensionForRuntimePlatform(RuntimePlatform platform) {
    switch (platform) {
    ...
    if (hasInvalidChar && logIfInvalidChars) {
      ...
      if (invalid.Length > 0)
        Debug.LogWarningFormat(
            "A filename cannot contain the following character{0}:  {1}",
            invalid.Length > 1 ? "s" : "", invalid);
    }
  ...
  GetDisplayStringOfInvalidCharsOfFileName(string filename) {
    if (string.IsNullOrEmpty(filename))
      return "";
  ...
  internal static IEnumerable<string> GetAllScriptGUIDs() {
    return AssetDatabase.GetAllAssetPaths()
        .Where(asset =>(IsScriptOrAssembly(asset) &&
                        !UnityEditor.PackageManager.Folders.IsPackagedAssetPath(
                            asset)))
  ...
  internal static UnityEditor.Scripting.MonoIsland[] GetMonoIslandsForPlayer() {
    ...
    PrecompiledAssembly[] unityAssemblies =
        GetUnityAssemblies(false, group, target);
    ...
    return EditorCompilationInterface.Instance.GetAllMonoIslands(
        unityAssemblies, allPrecompiledAssemblies,
        EditorScriptCompilationOptions.BuildingEmpty |
            EditorScriptCompilationOptions.BuildingIncludingTestAssemblies);
  }

  // Do not remove. Called through reflection by Visual Studio Tools for Unity.
  // Use EditorCompilationInterface.GetAllMonoIslands internally instead of this
  // method.
  ...
  internal static string[] GetCompilationDefinesForPlayer() {
    var group = EditorUserBuildSettings.activeBuildTargetGroup;
    var target = EditorUserBuildSettings.activeBuildTarget;
    return GetCompilationDefines(EditorScriptCompilationOptions.BuildingEmpty,
                                 group, target);
  }

  internal static string GetMonolithicEngineAssemblyPath() {
    // We still build a monolithic UnityEngine.dll as a compilation target for
    // user projects. It lives next to the editor dll.
  ...
  internal static string[] GetCompilationDefines(
      EditorScriptCompilationOptions options, BuildTargetGroup targetGroup,
      BuildTarget target) {
    return GetCompilationDefines(
        options, targetGroup, target,
