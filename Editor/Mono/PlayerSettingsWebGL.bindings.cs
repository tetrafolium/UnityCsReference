...
public enum WebGLLinkerTarget {
	...
	Wasm,
	[ Obsolete(
		  "WebGLLinkerTarget.Both mode is no longer supported. Instead you can create separate asm.js and WebAssembly builds and download the appropriate one depending on the browser capabilities.",
		  true) ] Both
}

public enum WebGLWasmArithmeticExceptions { Throw, Ignore }
...
public static bool useWasm {
	...
	set {
		linkerTarget = value ? WebGLLinkerTarget.Both : WebGLLinkerTarget.Asm;
	}
}
...
[Obsolete(
	 "wasmStreaming Property deprecated. WebAssembly streaming will be automatically used when decompressionFallback is disabled and vice versa.",
	 true)]
...
public extern static WebGLWasmArithmeticExceptions
