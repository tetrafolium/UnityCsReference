...
public class SceneVisibilityManager
    : ScriptableSingleton<SceneVisibilityManager> {
  ...
  private readonly static List<GameObject> m_RootBuffer =
      new List<GameObject>();
  ...
  private static void Initialize() {
    ...
    EditorApplication.playModeStateChanged +=
        EditorApplicationPlayModeStateChanged;
    StageNavigationManager.instance.afterSuccessfullySwitchedToStage +=
        StageNavigationManagerAfterSuccessfullySwitchedToStage;
    SceneVisibilityState.internalStructureChanged += InternalStructureChanged;
    PreviewSceneStage stage =
        StageNavigationManager.instance.currentStage as PreviewSceneStage;
  ...
  private static void EditorSceneManagerOnSceneOpened(Scene scene,
                                                      OpenSceneMode mode) {
    if (mode == OpenSceneMode.Single) {
      SceneVisibilityState.isolation = false;
    }
    if (mode == OpenSceneMode.Additive) {
      // make sure added scenes are isolated when opened if main stage is
      // isolated
  ...
  StageNavigationManagerAfterSuccessfullySwitchedToStage(Stage newStage) {
  ...
  private static void
  EditorApplicationPlayModeStateChanged(PlayModeStateChange state) {
  ...
  private static void EditorSceneManagerOnSceneSaving(Scene scene,
                                                      string path) {
    SceneVisibilityState.OnSceneSaving(scene, path);
  }

  private static void EditorSceneManagerOnSceneOpening(string path,
                                                       OpenSceneMode mode) {
    RevertIsolationCurrentStage();
    if (mode == OpenSceneMode.Single)
      SceneVisibilityState.GeneratePersistentDataForAllLoadedScenes();
  ...
  private static void EditorSceneManagerOnSceneClosing(Scene scene,
                                                       bool removingScene) {
    SceneVisibilityState.GeneratePersistentDataForLoadedScene(scene);
  }

  private static void EditorSceneManagerOnNewSceneCreated(Scene scene,
                                                          NewSceneSetup setup,
                                                          NewSceneMode mode) {
  ...
  public void DisableAllPicking() {
    Undo.RecordObject(SceneVisibilityState.GetInstance(),
                      "Disable All Picking");
  ...
  public void Show(GameObject gameObject, bool includeDescendants) {
    ...
    SceneVisibilityState.SetGameObjectHidden(gameObject, false,
                                             includeDescendants);
  }
  ...
  public void Hide(GameObject gameObject, bool includeDescendants) {
    ...
    SceneVisibilityState.SetGameObjectHidden(gameObject, true,
                                             includeDescendants);
  }

  public void DisablePicking(GameObject gameObject, bool includeDescendants) {
    Undo.RecordObject(SceneVisibilityState.GetInstance(),
                      "Disable Picking GameObject");
    SceneVisibilityState.SetGameObjectPickingDisabled(gameObject, true,
                                                      includeDescendants);
  }

  public void EnablePicking(GameObject gameObject, bool includeDescendants) {
    Undo.RecordObject(SceneVisibilityState.GetInstance(),
                      "Enable Picking GameObject");
    SceneVisibilityState.SetGameObjectPickingDisabled(gameObject, false,
                                                      includeDescendants);
  }
  ...
  private void HideNoUndo(Scene scene) {
    ...
    SceneVisibilityState.SetGameObjectsHidden(m_RootBuffer.ToArray(), true,
                                              true);
  }
  ...
  internal void DisablePickingNoUndo(Scene scene) {
    ...
    SceneVisibilityState.EnablePicking(scene);
    scene.GetRootGameObjects(m_RootBuffer);
    SceneVisibilityState.SetGameObjectsPickingDisabled(m_RootBuffer.ToArray(),
                                                       true, true);
  }
  ...
  internal enum SceneVisState { AllHidden, AllVisible, Mixed }
  ...
  public void Show(GameObject[] gameObjects, bool includeDescendants) {
    ...
    SceneVisibilityState.SetGameObjectsHidden(gameObjects, false,
                                              includeDescendants);
  }
  ...
  public void Hide(GameObject[] gameObjects, bool includeDescendants) {
    ...
    SceneVisibilityState.SetGameObjectsHidden(gameObjects, true,
                                              includeDescendants);
  }

  public void DisablePicking(GameObject[] gameObjects,
                             bool includeDescendants) {
    Undo.RecordObject(SceneVisibilityState.GetInstance(),
                      "Disable Picking GameObjects");
    SceneVisibilityState.SetGameObjectsPickingDisabled(gameObjects, true,
                                                       includeDescendants);
  }

  public void EnablePicking(GameObject[] gameObjects, bool includeDescendants) {
    Undo.RecordObject(SceneVisibilityState.GetInstance(),
                      "Enable Picking GameObjects");
    SceneVisibilityState.SetGameObjectsPickingDisabled(gameObjects, false,
                                                       includeDescendants);
  }
  ...
  private void VisibilityChanged() { visibilityChanged?.Invoke(); }

  private void PickableContentChanged() { pickingChanged?.Invoke(); }
  ...
  public void ToggleVisibility(GameObject gameObject, bool includeDescendants) {
    ...
    SceneVisibilityState.SetGameObjectHidden(
        gameObject, !SceneVisibilityState.IsGameObjectHidden(gameObject),
        includeDescendants);
  }
  ...
  public void TogglePicking(GameObject gameObject, bool includeDescendants) {
    ...
    SceneVisibilityState.SetGameObjectPickingDisabled(
        gameObject,
        !SceneVisibilityState.IsGameObjectPickingDisabled(gameObject),
        includeDescendants);
  }
  ...
  private static void ToggleSelectionVisibility() {
    ...
    if (shouldHide) {
      SceneHierarchyWindow.lastInteractedHierarchyWindow.GetSelectedScenes(
          instance.m_SelectedScenes);
    ...
    SceneVisibilityState.SetGameObjectsHidden(Selection.gameObjects, shouldHide,
                                              false);
  ...
  [Shortcut("Scene Visibility/Toggle Selection And Descendants Visibility",
            typeof(ShortcutContext), KeyCode.H)]
  private static void ToggleSelectionAndDescendantsVisibility() {
    ...
    Undo.RecordObject(SceneVisibilityState.GetInstance(),
                      "Toggle Selection And Descendants Visibility");
    SceneVisibilityState.SetGameObjectsHidden(Selection.gameObjects, shouldHide,
                                              true);
  ...
  [Shortcut("Scene Picking/Toggle Picking On Selection And Descendants",
            typeof(ShortcutContext), KeyCode.L)]
  private static void ToggleSelectionAndDescendantsPicking() {
    ...
    Undo.RecordObject(SceneVisibilityState.GetInstance(),
                      "Toggle Selection And Descendants Picking");
    SceneVisibilityState.SetGameObjectsPickingDisabled(
  ...
  internal static void ToggleSelectionPickable() {
    ...
    SceneVisibilityState.SetGameObjectsPickingDisabled(
        Selection.gameObjects, shouldDisablePicking, false);
  ...
  [Shortcut("Scene Visibility/Toggle Isolation On Selection And Descendants",
            typeof(ShortcutContext), KeyCode.H, ShortcutModifiers.Shift)]
  ...
  internal void ToggleIsolateSelectionAndDescendants() {
    Undo.RecordObject(SceneVisibilityState.GetInstance(),
                      "Toggle Isolation on Selection And Children");
    ...
    if (!IsCurrentStageIsolated()) {
      ...
      if (Selection.gameObjects.Length > 0) {
        SceneVisibilityState.SetGameObjectsHidden(Selection.gameObjects, false,
                                                  true);
      }
  ...
  internal void ToggleScene(Scene scene, SceneVisState visibilityState) {
    if (visibilityState == SceneVisState.AllVisible ||
        visibilityState == SceneVisState.Mixed) {
      Hide(scene);
    } else {
      Show(scene);
    }
