...
public struct UndoPropertyModification {
  ...
  [NativeName("keepPrefabOverride")] int m_KeepPrefabOverride;

  public bool keepPrefabOverride {
    get { return m_KeepPrefabOverride != 0; }
    set { m_KeepPrefabOverride = value ? 1 : 0; }
  }
}
...
internal class AtomicUndoScope : IDisposable {
  ...
  public AtomicUndoScope() { Undo.BeginAtomicUndoGroup(); }
...
// Lets you register undo operations on specific objects you are about to
// perform changes on.
...
public partial class Undo {
  ...
  private static extern void GetRecordsInternal(object undoRecords,
                                                object redoRecords);

  internal static void GetRecords(List<string> undoRecords,
                                  List<string> redoRecords) {
  ...
  public static void RegisterCompleteObjectUndo(Object[] objectsToUndo,
    ...
    if (objectsToUndo.Length > 0)
      RegisterCompleteObjectUndoMultiple(objectsToUndo[0], objectsToUndo, name,
                                         0);
  }
  ...
  RegisterCompleteObjectUndoMultiple([ NotNull ] Object identifier,
                                     Object[] objectsToUndo, string name,
                                     int namePriority);
  ...
  [NativeThrows]
  [StaticAccessor("UndoBindings", StaticAccessorType.DoubleColon)]
  public static extern void MoveGameObjectToScene([ NotNull ] GameObject go,
                                                  Scene scene, string name);

  // Register the state of a Unity Object so the user can later undo back to
  // that state.
  [FreeFunction]
  public static extern void
  RegisterCreatedObjectUndo([ NotNull ] Object objectToUndo, string name);
  ...
  DestroyObjectUndoable([ NotNull ] Object objectToUndo, string name);

  [NativeThrows]
  [StaticAccessor("UndoBindings", StaticAccessorType.DoubleColon)]
  public static extern Component AddComponent([ NotNull ] GameObject gameObject,
                                              Type type);
  ...
  public static extern void
  RegisterFullObjectHierarchyUndo([ NotNull ] Object objectToUndo, string name);

  [Obsolete("Use Undo.RegisterFullObjectHierarchyUndo(Object, string) instead")]
  public static void
  RegisterFullObjectHierarchyUndo(UnityEngine.Object objectToUndo) {
  ...
  [NativeThrows]
  [StaticAccessor("UndoBindings", StaticAccessorType.DoubleColon)]
  private static extern void RecordObjectsInternal(Object[] objectToUndo,
  ...
  internal static UndoPropertyModification[] InvokePostprocessModifications(
    ...
    for (int i = 0, n = delegates.Length; i < n; ++i) {
      remainingModifications = ((Undo.PostprocessModifications) delegates[i])
                                   .Invoke(remainingModifications);
    }
  ...
  internal static extern void
  ClearUndoSceneHandle(UnityEngine.SceneManagement.Scene scene);
  ...
  [Obsolete(
      "Use DestroyObjectImmediate, RegisterCreatedObjectUndo or RegisterUndo instead.")]
