...
internal class UndoWindow : EditorWindow {
  ...
  private void OnGUI() {
    GUILayout.Label("(Available only in Developer builds)",
    ...
    undosScroll = GUILayout.BeginScrollView(
        undosScroll, EditorStyles.helpBox,
        new GUILayoutOption[]{GUILayout.MinHeight(height),
    ...
    redosScroll = GUILayout.BeginScrollView(
        redosScroll, EditorStyles.helpBox,
        new GUILayoutOption[]{GUILayout.MinHeight(height),
                              GUILayout.MinWidth(width)});
