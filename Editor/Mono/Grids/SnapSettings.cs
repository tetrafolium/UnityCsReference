...
[SerializeField] Vector3 m_SnapValue =
	new Vector3(k_DefaultSnapValue, k_DefaultSnapValue, k_DefaultSnapValue);

[SerializeField] Vector3Int m_SnapMultiplier =
	new Vector3Int(k_DefaultSnapMultiplier, k_DefaultSnapMultiplier,
	               k_DefaultSnapMultiplier);

[SerializeField] float m_Rotation = k_DefaultRotation;

[SerializeField] float m_Scale = k_DefaultScale;
...
Vector3 SnapMultiplierFrac() {
	...
	return new Vector3(m_SnapMultiplier.x * val, m_SnapMultiplier.y * val,
	                   m_SnapMultiplier.z * val);
}

Vector3 SnapValueInUnityUnits() {
	Vector3 frac = SnapMultiplierFrac();
	return new Vector3(m_SnapValue.x * frac.x, m_SnapValue.y * frac.y,
	                   m_SnapValue.z * frac.z);
}
