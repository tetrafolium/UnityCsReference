...
[FilePath("Library/EditorSnapSettings.asset",
          FilePathAttribute.Location.ProjectFolder)]
class EditorSnapSettingsData : ScriptableSingleton<EditorSnapSettingsData> {
[SerializeField] bool m_SnapEnabled;

[SerializeField] SnapSettings m_SnapSettings = new SnapSettings();
...
// Most of EditorSnapSettings is private for now because we are just hard-coding
// grid snap and incremental snap. in the near future, we will be extending
// support for multiple snapping modes and this API will change. ex, snap modes
// will likely be implemented as objects that can be registered to this class,
// making user-implemented snapping behaviours possible.
...
internal static bool activeToolSupportsGridSnap {
	get {
		return (EditorTools.EditorTools.activeToolType == typeof(MoveTool) ||
		        EditorTools.EditorTools.activeToolType ==
		        typeof(TransformTool)) &&
...
internal static bool gridSnapActive {
	get {
		return !incrementalSnapActive && activeToolSupportsGridSnap &&
