...
namespace UnityEditor.Snap {
  sealed class GridSettingsWindow : PopupWindowContent {
    static class Contents {
      public static readonly GUIContent axisHeader =
          EditorGUIUtility.TrTextContent("Grid Axis");
      public static readonly GUIContent axisX =
          EditorGUIUtility.TrTextContent("X");
      public static readonly GUIContent axisY =
          EditorGUIUtility.TrTextContent("Y");
      public static readonly GUIContent axisZ =
          EditorGUIUtility.TrTextContent("Z");

      public static readonly GUIContent settingsHeader =
          EditorGUIUtility.TrTextContent("Grid Settings");
      public static readonly GUIContent opacitySlider =
          EditorGUIUtility.TrTextContent("Opacity");

      public static readonly GUIContent reset =
          EditorGUIUtility.TrTextContent("Reset");
      public static readonly GUIContent editGridAndSnapSettings =
          EditorGUIUtility.TrTextContent("Edit Grid and Snap Settings...");
    }
    ...
    static class Styles {
      ...
      internal static void Init() {
        ...
        settingsArea =
            new GUIStyle(){padding = new RectOffset(
                               k_SettingsAreaBorder, k_SettingsAreaBorder,
                               k_SettingsAreaBorder, k_SettingsAreaBorder)};
      }
    }
    ...
    const float k_WindowHeight =
        EditorGUI.kSingleLineHeight * 6 + k_SettingsAreaBorder * 2;
    readonly SceneView m_SceneView;

    public GridSettingsWindow(SceneView sceneView) { m_SceneView = sceneView; }
    ...
    void DoGridAxes() {
      ...
      if (GUILayout.Button(GUIContent.none, Styles.options)) {
        ...
        menu.AddItem(Contents.editGridAndSnapSettings, false, () => {
          EditorWindow.GetWindow<SnapSettingsWindow>(
              false, SnapSettingsWindow.k_WindowTitle, true);
        });
        menu.ShowAsContext();
      }
      ...
      if (DrawListElement(Contents.axisX,
                          axis == SceneViewGrid.GridRenderAxis.X))
        m_SceneView.sceneViewGrids.gridAxis = SceneViewGrid.GridRenderAxis.X;

      if (DrawListElement(Contents.axisY,
                          axis == SceneViewGrid.GridRenderAxis.Y))
        m_SceneView.sceneViewGrids.gridAxis = SceneViewGrid.GridRenderAxis.Y;

      if (DrawListElement(Contents.axisZ,
                          axis == SceneViewGrid.GridRenderAxis.Z))
    ...
    void DoGridSettings() {
      ...
      EditorGUIUtility.labelWidth = EditorGUI.CalcPrefixLabelWidth(
          Contents.opacitySlider, EditorStyles.label);
      m_SceneView.sceneViewGrids.gridOpacity = EditorGUILayout.Slider(
          Contents.opacitySlider, m_SceneView.sceneViewGrids.gridOpacity, 0, 1);
