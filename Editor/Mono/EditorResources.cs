...
namespace UnityEditor.Experimental {
  internal class FontDef {
    ...
    private Dictionary<Style, FontData> m_Fonts =
        new Dictionary<Style, FontData>();
    ...
    public FontDef(string name) { this.name = name; }
    ...
    public static FontDef CreateFromResources(string fontName,
                                              Dictionary<Style, string> fonts) {
  ...
  public partial class EditorResources {
    ...
    private static bool CanEnableExtendedStyles() {
      var rootBlock =
          styleCatalog.GetStyle(StyleCatalogKeyword.root, StyleState.root);
      var styleExtensionDisabled =
          rootBlock.GetBool("-unity-disable-style-extensions");
    ...
    private static bool IsEditorStyleSheet(string path) {
      if (!SearchUtils.EndsWith(path, ".uss"))
        return false;
      return path.IndexOf("/stylesheets/extensions/",
                          StringComparison.OrdinalIgnoreCase) != -1 &&
             (SearchUtils.EndsWith(path, "common.uss") ||
              SearchUtils.EndsWith(path, Constants.isDarkTheme ? "dark.uss"
                                   : "light.uss"));
    }

    internal static string GetDefaultFont() {
      // In languages other than english, we use 'lucida grande' because the
      // localization settings are currently mapped to 'Lucida Grande'. see
      // Editor/Resources/Windows/fontsettings.text for instance
      if (LocalizationDatabase.currentEditorLanguage ==
    ...
    internal static IEnumerable<string> supportedFontNames =>
        EditorResources.supportedFonts.Keys;
      ...
      get {
        if (s_CurrentFontName == null) {
          if (LocalizationDatabase.currentEditorLanguage ==
              SystemLanguage.English) {
            s_CurrentFontName =
                EditorPrefs.GetString(k_PrefsUserFontKey, GetDefaultFont());
    ...
    static void AddLucidaGrande() {
      s_SupportedFonts[FontDef.k_LucidaGrande] = FontDef.CreateFromResources(
          FontDef.k_LucidaGrande,
    ...
    internal static Dictionary<string, FontDef> supportedFonts {
      get {
        if (s_SupportedFonts == null) {
          ...
          // In languages other than english, we use 'lucida grande' because the
          // localization settings are currently are mapped to 'lucida grande'.
          // see Editor/Resources/Windows/fontsettings.text for instance
          if (LocalizationDatabase.currentEditorLanguage !=
              SystemLanguage.English) {
            AddLucidaGrande();
          } else {
            s_SupportedFonts[FontDef.k_Inter] = FontDef.CreateFromResources(
                FontDef.k_Inter,
                new Dictionary<FontDef.Style, string>{
                    ...
                    {FontDef.Style.BoldAndItalic,
                     "Fonts/Inter/Inter-SemiBoldItalic.ttf"}});

            if (Application.platform == RuntimePlatform.WindowsEditor) {
              s_SupportedFonts[FontDef.k_Verdana] =
                  FontDef.CreateSystemFont(FontDef.k_Verdana);
            } else {
              AddLucidaGrande();
            }
          }
    ...
    private static List<string> GetDefaultStyleCatalogPaths() {
      ...
      if (LocalizationDatabase.currentEditorLanguage ==
          SystemLanguage.English) {
        string currentFontStyleSheet = "StyleSheets/Extensions/fonts/" +
                                       currentFontName.ToLower() + ".uss";

        catalogFiles.Add(currentFontStyleSheet);
      }

      catalogFiles.Add(useDarkTheme ? "StyleSheets/Extensions/base/dark.uss"
    ...
    static void SaveCatalogToDisk(StyleCatalog catalog, string hash,
                                  string savePath) {
      using(var stream = new FileStream(
                savePath, FileMode.Create, FileAccess.Write,
                FileShare.None)) using(BinaryWriter writer =
      ...
      using(new EditorPerformanceTracker(nameof(BuildCatalog))) {
        ...
        if (!EditorApplication.isBuildingAnyResources) {
          ...
          if (!forceRebuild && File.Exists(k_GlobalStyleCatalogCacheFilePath)) {
            using(var cacheCatalogStream = new FileStream(
                      k_GlobalStyleCatalogCacheFilePath, FileMode.Open,
        ...
        if (rebuildCatalog) {
          Console.WriteLine(
              $"Loading style catalogs ({paths.Count})\r\n\t{String.Join("\r\n\t
              ", paths.ToArray())}");

          styleCatalog.Load(paths);

          if (paths.Count != 0) {
            SaveCatalogToDisk(styleCatalog, catalogHash,
                              k_GlobalStyleCatalogCacheFilePath);
          }
        }
      }
    }
    ...
    internal static void RefreshSkin() {
      using(new EditorPerformanceTracker(nameof(RefreshSkin))) {
        ...
        if (skin != null) {
          ...
          if (Path.GetFileName(Path.GetDirectoryName(Application.dataPath)) ==
              "editor_resources")
            ConverterUtils.ResetSkinToPristine(
                skin, Constants.isDarkTheme ? SkinTarget.Dark
                : SkinTarget.Light);
    ...
    private static void UpdateGUIStyleProperties(string name, GUIStyle style) {
      ...
      try {
        ...
        Debug.LogWarning(
            $"Failed to parse extended style properties for {sname}\n{ex.Message}");
      }
    }
    ...
    public static T Load<T>(string assetPath, bool isRequired = true) where T
        : UnityEngine.Object {
      var obj = Load(assetPath, typeof(T));
      if (!obj && isRequired)
        throw new FileNotFoundException("Could not find editor resource " +
                                        assetPath);
      return obj as T;
    }
    ...
    internal static StyleBlock GetStyle(string selectorName,
      ...
      return styleCatalog.GetStyle(selectorName, states);
    }
    internal static StyleBlock GetStyle(int selectorKey,
                                        params StyleState[] states) {
      return styleCatalog.GetStyle(selectorKey, states);
    }
    ...
    private class StyleCatalogPostProcessor : AssetPostprocessor {
      ...
      static void OnPostprocessAllAssets(string[] importedAssets,
        ...
        if (importedAssets.Concat(deletedAssets)
                .Any(path => IsEditorStyleSheet(path)))
