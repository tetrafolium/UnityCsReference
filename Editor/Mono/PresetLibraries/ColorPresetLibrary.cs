...
class ColorPresetLibrary : PresetLibrary {
  [SerializeField] List<ColorPreset> m_Presets = new List<ColorPreset>();
  ...
  public override int Count() { return m_Presets.Count; }

  public override object GetPreset(int index) { return m_Presets[index].color; }
  ...
  public override void Remove(int index) { m_Presets.RemoveAt(index); }

  public override void Move(int index, int destIndex,
                            bool insertAfterDestIndex) {
    PresetLibraryHelpers.MoveListItem(m_Presets, index, destIndex,
                                      insertAfterDestIndex);
  }
  ...
  private void Init() {
    if (m_ColorSwatch == null)
      m_ColorSwatch =
          CreateColorSwatchWithBorder(kSwatchSize, kSwatchSize, false);

    if (m_ColorSwatchTriangular == null)
      m_ColorSwatchTriangular =
          CreateColorSwatchWithBorder(kSwatchSize, kSwatchSize, true);

    if (m_MiniColorSwatchTriangular == null)
      m_MiniColorSwatchTriangular =
          CreateColorSwatchWithBorder(kMiniSwatchSize, kMiniSwatchSize, true);

    if (m_CheckerBoard == null)
      m_CheckerBoard = GradientEditor.CreateCheckerTexture(
          2, 2, 3, new Color(0.8f, 0.8f, 0.8f), new Color(0.5f, 0.5f, 0.5f));
  }
  ...
  private void DrawInternal(Rect rect, Color preset) {
    ...
    // Normalize color if HDR to give some color hint (otherwise it likely is
    // just white)
    if (isHDR)
  ...
  private void RenderSwatchWithAlpha(Rect rect, Color preset,
                                     Texture2D swatchTexture) {
    Rect r2 = new Rect(rect.x + 1, rect.y + 1, rect.width - 2, rect.height - 2);

    // Background checkers
    GUI.color = Color.white;
    Rect texCoordsRect = new Rect(0, 0, r2.width / m_CheckerBoard.width,
                                  r2.height / m_CheckerBoard.height);
  ...
  public override string GetName(int index) { return m_Presets[index].name; }
  ...
  public void CreateDebugColors() {
    for (int i = 0; i < 2000; ++i)
      m_Presets.Add(new ColorPreset(new Color(Random.Range(0.2f, 1f),
                                              Random.Range(0.2f, 1f),
                                              Random.Range(0.2f, 1f), 1f),
                                    "Preset Color " + i));
  }

  private static Texture2D CreateColorSwatchWithBorder(int width, int height,
                                                       bool triangular) {
    Texture2D texture =
