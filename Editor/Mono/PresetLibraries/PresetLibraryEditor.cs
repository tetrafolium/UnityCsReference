...
// Client code should allocate and ensure its being serialized (e.g as a field
// in an EditorWindow)
[System.Serializable]
internal class PresetLibraryEditorState {
  ...
  public string m_CurrrentLibrary =
      PresetLibraryLocations.defaultPresetLibraryPath;
  ...
  static public ItemViewMode GetItemViewMode(string prefix) {
    return (ItemViewMode)
        EditorPrefs.GetInt(prefix + "ViewMode", (int) ItemViewMode.Grid);
  }

  public PresetLibraryEditorState(string prefix) { m_Prefix = prefix; }
  ...
  public ItemViewMode itemViewMode {
    ...
    set {
      if (m_ItemViewMode != value) {
        ...
        InspectorWindow
            .RepaintAllInspectors(); // if inspector is showing a preset library
                                     // we want it to follow the state
  ...
  public void TransferEditorPrefsState(bool load) {
    if (load) {
      m_ItemViewMode = (ItemViewMode) EditorPrefs.GetInt(m_Prefix + "ViewMode",
                                                         (int) m_ItemViewMode);
      m_PreviewHeight =
          EditorPrefs.GetFloat(m_Prefix + "ItemHeight", m_PreviewHeight);
      m_ScrollPosition.y =
          EditorPrefs.GetFloat(m_Prefix + "Scroll", m_ScrollPosition.y);
      m_CurrrentLibrary =
          EditorPrefs.GetString(m_Prefix + "CurrentLib", m_CurrrentLibrary);
    } else // save
    {
      EditorPrefs.SetInt(m_Prefix + "ViewMode", (int) m_ItemViewMode);
...
internal partial class PresetLibraryEditor<T> where T : PresetLibrary {
  class Styles {
    ...
    public GUIContent plusButtonText =
        EditorGUIUtility.TrTextContent("", "Add new preset");
    public GUIContent plusButtonTextNotCheckedOut =
        EditorGUIUtility.TrTextContent(
            "",
            "To add presets you need to press the 'Check out' button below");
  ...
  readonly System.Action<int, object>
      m_ItemClickedCallback; // <click count, clicked preset>
  ...
  float versionControlAreaHeight {
    get { return 20f; }
  }
  float gridWidth {
    get;
  ...
  public PresetLibraryEditor(ScriptableObjectSaveLoadHelper<T> helper,
    ...
    m_PresetLibraryFileLocation =
        PresetLibraryLocations.GetFileLocationFromPath(
  ...
  public void InitializeGrid(float availableWidth) {
    ...
    if (lib != null) {
      ...
      Debug.LogError("Could not load preset library " +
                     currentLibraryWithoutExtension);
  }
  ...
  public string currentLibraryWithoutExtension {
    get { return m_State.m_CurrrentLibrary; }
    set {
      m_State.m_CurrrentLibrary =
          Path.ChangeExtension(value, null); // ensure no extension
      m_PresetLibraryFileLocation =
          PresetLibraryLocations.GetFileLocationFromPath(
              m_State.m_CurrrentLibrary);
      OnLayoutChanged();
  ...
  public float previewHeight {
    ...
    set {
      m_State.m_PreviewHeight =
          Mathf.Clamp(value, minMaxPreviewHeight.x, minMaxPreviewHeight.y);
    }
  }
  ...
  bool drawLabels {
    get {
      return m_State.itemViewMode == PresetLibraryEditorState.ItemViewMode.List;
    }
  }
  ...
  string CreateNewLibraryCallback(string libraryName,
    ...
    string defaultPath =
        PresetLibraryLocations.GetDefaultFilePathForFileLocation(fileLocation);
  ...
  void SetupGrid(float width, int itemCount) {
    ...
    case PresetLibraryEditorState.ItemViewMode.Grid:
      ...
      m_Grid.itemSize = new Vector2(m_State.m_PreviewHeight * m_PreviewAspect,
                                    m_State.m_PreviewHeight); // no text
    ...
    case PresetLibraryEditorState.ItemViewMode.List:
      ...
      m_Grid.itemSize =
          new Vector2(width - m_Grid.leftMargin, m_State.m_PreviewHeight);
      m_Grid.InitNumRowsAndColumns(itemCount, int.MaxValue);
      break;
    }

    float gridHeight =
        m_Grid.CalcRect(itemCount - 1, 0f).yMax + m_Grid.bottomMargin;
    contentHeight = topAreaHeight + gridHeight +
                    (m_IsOpenForEdit ? 0
                     : versionControlAreaHeight);
  }

  public void OnGUI(Rect rect, object presetObject) {
    // If removing this early out grid setup needs to be ignored for layout and
    // used events
    if (rect.width < 2f)
      return;
    ...
    Rect presetRect =
        new Rect(rect.x, topArea.yMax, rect.width, rect.height - topAreaHeight);
  ...
  void TopArea(Rect rect) {
    ...
    {
      if (showHeader)
        GUI.Label(new Rect(10, 0, rect.width - 20, rect.height),
                  s_Styles.header);

      const float optionsButtonWidth = 16f;
      const float optionsButtonHeight = 16f;
      Rect buttonRect =
          new Rect(rect.width - optionsButtonWidth - settingsMenuRightMargin,
                   (rect.height - optionsButtonHeight) * 0.5f,
                   optionsButtonWidth, rect.height);
      ...
      if (wantsToCreateLibrary) {
        ...
        PopupWindow.Show(buttonRect, new PopupWindowContentForNewLibrary(
  ...
  Rect GetDragRect(Rect itemRect) {
    ...
    return new RectOffset(extraHorz, extraHorz, extraVert, extraVert)
        .Add(itemRect);
  }
  ...
  private string pathWithExtension {
    get {
      return currentLibraryWithoutExtension + "." +
             m_SaveLoadHelper.fileExtensionWithoutDot;
    }
  }
  ...
  void VersionControlArea(Rect rect) {
    if (rect.width > kCheckoutButtonMaxWidth)
      rect =
          new Rect(rect.xMax - kCheckoutButtonMaxWidth - kCheckoutButtonMargin,
                   rect.y + kCheckoutButtonMargin, kCheckoutButtonMaxWidth,
                   rect.height - kCheckoutButtonMargin * 2);
  ...
  void ListArea(Rect rect, PresetLibrary lib, object newPresetObject) {
    ...
    if (m_PresetLibraryFileLocation == PresetFileLocation.ProjectFolder &&
        evt.type == EventType.Repaint)
      m_IsOpenForEdit = AssetDatabase.IsOpenForEdit(pathWithExtension);
    else if (m_PresetLibraryFileLocation ==
             PresetFileLocation.PreferencesFolder)
      m_IsOpenForEdit = true;

    if (!m_IsOpenForEdit) {
      Rect versionControlRect =
          new Rect(rect.x, rect.yMax - versionControlAreaHeight, rect.width,
                   versionControlAreaHeight);
      VersionControlArea(versionControlRect);
      rect.height -= versionControlAreaHeight;
    }

    // To ensure we setup grid to visible rect we need to run once to check if
    // scrollbar is taking up screen estate. To optimize the first width is
    // based on the last frame and we therefore most likely will only run once.
    for (int i = 0; i < 2; i++) {
    ...
    if ((m_ShowedScrollBarLastFrame || alwaysShowScrollAreaHorizontalLines) &&
    ...
    m_State.m_ScrollPosition =
        GUI.BeginScrollView(rect, m_State.m_ScrollPosition, contentRect);
    {
      int startIndex, endIndex;
      float yOffset = 0f;
      int maxIndex = m_ShowAddNewPresetItem ? lib.Count() : lib.Count() - 1;
      bool isGridVisible = m_Grid.IsVisibleInScrollView(
          rect.height, m_State.m_ScrollPosition.y, yOffset, maxIndex,
          out startIndex, out endIndex);
      bool drawDragInsertionMarker = false;
      if (isGridVisible) {
        // Handle renaming overlay before item handling because its needs mouse
        // input first to end renaming if clicked outside
        if (GetRenameOverlay().IsRenaming() &&
            !GetRenameOverlay().isWaitingForDelay) {
        ...
        for (int i = startIndex; i <= endIndex; ++i) {
          ...
          case PresetLibraryEditorState.ItemViewMode.List:
            ...
            labelRect.y =
                itemRect.yMin + (itemRect.height - kGridLabelHeight) * 0.5f;
            break;
          ...
          if (m_ShowAddNewPresetItem && i == lib.Count()) {
            CreateNewPresetButton(previewRect, newPresetObject, lib,
                                  m_IsOpenForEdit);
            continue;
          }
          ...
          if (isRenamingThisItem) {
            ...
            renameRect.y -= 1f;
            renameRect.x -= 1f; // adjustment to fit perfectly
          ...
          case EventType.Repaint:
            ...
            if (m_DragState.draggingIndex == i ||
            ...
            if (m_DragState.dragUponIndex == i &&
                m_DragState.draggingIndex != m_DragState.dragUponIndex)
          ...
          case EventType.DragPerform: {
            ...
            if (dragRect.Contains(evt.mousePosition)) {
              ...
              if (m_State.itemViewMode ==
                  PresetLibraryEditorState.ItemViewMode.List)
                m_DragState.insertAfterIndex =
                    ((evt.mousePosition.y - dragRect.y) / dragRect.height) >
                    0.5f;
              else
                m_DragState.insertAfterIndex =
                    ((evt.mousePosition.x - dragRect.x) / dragRect.width) >
                    0.5f;

              bool perform = evt.type == EventType.DragPerform;
              if (perform) {
                if (m_DragState.draggingIndex >= 0) {
                  MovePreset(m_DragState.draggingIndex,
                             m_DragState.dragUponIndex,
                             m_DragState.insertAfterIndex);
  ...
  void DrawDragInsertionMarker() {
    ...
    if (m_State.itemViewMode == PresetLibraryEditorState.ItemViewMode.List) {
      if (m_DragState.insertAfterIndex)
        insertRect = new Rect(dragRect.xMin, dragRect.yMax - halfInserter,
                              dragRect.width, inserterThickness);
      else
        insertRect = new Rect(dragRect.xMin, dragRect.yMin - halfInserter,
                              dragRect.width, inserterThickness);
    } else // grid
    {
      if (m_DragState.insertAfterIndex)
        insertRect = new Rect(dragRect.xMax - halfInserter, dragRect.yMin,
                              inserterThickness, dragRect.height);
      else
        insertRect = new Rect(dragRect.xMin - halfInserter, dragRect.yMin,
                              inserterThickness, dragRect.height);
    }
    EditorGUI.DrawRect(insertRect, new Color(0.3f, 0.3f, 1.0f));
  }

  void CreateNewPresetButton(Rect buttonRect, object newPresetObject,
                             PresetLibrary lib, bool isOpenForEdit) {
    using(new EditorGUI.DisabledScope(!isOpenForEdit)) {
      if (GUI.Button(buttonRect, isOpenForEdit ? s_Styles.plusButtonText
                     : s_Styles.plusButtonTextNotCheckedOut)) {
        int newIndex = CreateNewPreset(newPresetObject, "");
        if (drawLabels)
          BeginRenaming("", newIndex, 0f);
        InspectorWindow
            .RepaintAllInspectors(); // If inspecting a preset libarary we want
                                     // to show the new preset there as well
      }
      ...
      if (Event.current.type == EventType.Repaint) {
        ...
        if (buttonRect.width > 30) {
          LabelWithOutline(buttonRect, s_Styles.newPreset,
                           new Color(0.1f, 0.1f, 0.1f),
                           s_Styles.newPresetStyle);
        } else {
  ...
  static void LabelWithOutline(Rect rect, GUIContent content,
                               Color outlineColor, GUIStyle style) {
  ...
  bool IsRenaming(int itemID) {
    return GetRenameOverlay().IsRenaming() &&
           GetRenameOverlay().userData == itemID &&
           !GetRenameOverlay().isWaitingForDelay;
  }

  RenameOverlay GetRenameOverlay() { return m_State.m_RenameOverlay; }
  ...
  public T GetCurrentLib() {
    T lib = PresetLibraryManager.instance.GetLibrary<T>(
        m_SaveLoadHelper, currentLibraryWithoutExtension);
    if (lib == null) {
      // If current library not found then get the default library (or create
      // the default library if not present)
      lib = PresetLibraryManager.instance.GetLibrary<T>(
          m_SaveLoadHelper, PresetLibraryLocations.defaultPresetLibraryPath);
      if (lib == null) {
        ...
        if (lib != null) {
          ...
          if (addDefaultPresets != null) {
            ...
            PresetLibraryManager.instance.SaveLibrary(
                m_SaveLoadHelper, lib,
                PresetLibraryLocations.defaultPresetLibraryPath);
          }
        } else {
          Debug.LogError("Could not create Default preset library " +
                         PresetLibraryManager.instance.GetLastError());
        }
      }
  ...
  public void MovePreset(int presetIndex, int destPresetIndex,
                         bool insertAfterDestIndex) {
    T lib = GetCurrentLib();
    if (lib == null)
      return;
  ...
  public void SaveCurrentLib() {
    ...
    PresetLibraryManager.instance.SaveLibrary(m_SaveLoadHelper, lib,
                                              currentLibraryWithoutExtension);
    InternalEditorUtility.RepaintAllViews();
  }

  public T CreateNewLibrary(string presetLibraryPathWithoutExtension) {
    T newLib = PresetLibraryManager.instance.CreateLibrary<T>(
        m_SaveLoadHelper, presetLibraryPathWithoutExtension);
    if (newLib != null) {
      PresetLibraryManager.instance.SaveLibrary(
          m_SaveLoadHelper, newLib, presetLibraryPathWithoutExtension);
      InternalEditorUtility.RepaintAllViews(); // Needed because we call this
                                               // function from another window
    }
    return newLib;
  }
  ...
  public void RevealCurrentLibrary() {
    if (m_PresetLibraryFileLocation == PresetFileLocation.PreferencesFolder)
      ...
      EditorGUIUtility.PingObject(
          AssetDatabase.GetMainAssetInstanceID(pathWithExtension));
  }
  ...
  internal class PresetContextMenu {
    ...
    static internal void Show(bool isOpenForEdit, int presetIndex,
                              object newPresetObject,
                              PresetLibraryEditor<T> caller) {
      s_Caller = caller;
      ...
      if (isOpenForEdit) {
        menu.AddItem(replaceText, false, new PresetContextMenu().Replace,
                     newPresetObject);
