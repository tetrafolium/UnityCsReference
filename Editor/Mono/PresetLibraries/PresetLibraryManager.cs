...
internal enum PresetFileLocation {
  PreferencesFolder,
  ProjectFolder
} // ProjectFolder: We look in all Editor folders in Assets

internal static class PresetLibraryLocations {
  public static string defaultLibraryLocation {
    get {
      return GetDefaultFilePathForFileLocation(
  ...
  GetAvailableFilesWithExtensionOnTheHDD(PresetFileLocation fileLocation,
                                         string fileExtensionWithoutDot) {
    List<string> folderPaths = GetDirectoryPaths(fileLocation);
    List<string> files =
        GetFilesWithExentionFromFolders(folderPaths, fileExtensionWithoutDot);
    for (int i = 0; i < files.Count; ++i)
      files [i]
      = ConvertToUnitySeperators(files[i]);
    return files;
  }

  public static string
  GetDefaultFilePathForFileLocation(PresetFileLocation fileLocation) {
    switch (fileLocation) {
  ...
  static List<string> GetDirectoryPaths(PresetFileLocation fileLocation) {
    ...
    switch (fileLocation) {
    case PresetFileLocation.PreferencesFolder:
      folderPaths.Add(GetDefaultFilePathForFileLocation(
          PresetFileLocation.PreferencesFolder));
      break;

    case PresetFileLocation.ProjectFolder:
      string[] editorFolders = Directory.GetDirectories(
          "Assets/", "Editor", SearchOption.AllDirectories);
  ...
  static List<string>
  GetFilesWithExentionFromFolders(List<string> folderPaths,
                                  string fileExtensionWithoutDot) {
    // First get all potential files
    var files = new List<string>();
    foreach (string editorFolder in folderPaths) {
      string[] filePaths =
          Directory.GetFiles(editorFolder, "*." + fileExtensionWithoutDot);
  ...
  static public string GetParticleCurveLibraryExtension(bool singleCurve,
                                                        bool signedRange) {
...
internal class PresetLibraryManager
    : ScriptableSingleton<PresetLibraryManager> {
  ...
  private HideFlags libraryHideFlag {
    get {
      return HideFlags.DontSave; // Use of DontSave prevents library from being
                                 // nulled when going out of playmode
    }
  }

  // Returns lists of filepaths for libraries with a given extension found on
  // the HDD
  public void GetAvailableLibraries<T>(ScriptableObjectSaveLoadHelper<T> helper,
                                       out List<string> preferencesLibs,
                                       out List<string> projectLibs) where T
      : ScriptableObject {
    preferencesLibs =
        PresetLibraryLocations.GetAvailableFilesWithExtensionOnTheHDD(
            PresetFileLocation.PreferencesFolder,
            helper.fileExtensionWithoutDot);
    projectLibs = PresetLibraryLocations.GetAvailableFilesWithExtensionOnTheHDD(
        PresetFileLocation.ProjectFolder, helper.fileExtensionWithoutDot);
  }
  ...
  public T CreateLibrary<T>(ScriptableObjectSaveLoadHelper<T> helper,
                            string presetLibraryPathWithoutExtension) where T
      : ScriptableObject {
    string libraryName =
        GetLibaryNameFromPath(presetLibraryPathWithoutExtension);
    if (!InternalEditorUtility.IsValidFileName(libraryName)) {
      string invalid =
          InternalEditorUtility.GetDisplayStringOfInvalidCharsOfFileName(
              libraryName);
      if (invalid.Length > 0)
        s_LastError = string.Format(
            "A library filename cannot contain the following character{0}:  {1}",
    ...
    if (GetLibrary(helper, presetLibraryPathWithoutExtension) != null) {
      s_LastError =
          "Library '" + libraryName + "' already exists! Ensure a unique name.";
      return null;
    }
  ...
  public T GetLibrary<T>(ScriptableObjectSaveLoadHelper<T> helper,
                         string presetLibraryPathWithoutExtension) where T
      : ScriptableObject {
    LibraryCache set = GetPresetLibraryCache(helper.fileExtensionWithoutDot);
    ...
    for (int i = 0; i < set.loadedLibraryIDs.Count; ++i) {
      if (set.loadedLibraryIDs[i] == presetLibraryPathWithoutExtension) {
        if (set.loadedLibraries[i] != null)
          ...
          // The library has been destroyed. Remove it from the lists so it can
          // be reloaded
          set.loadedLibraries.RemoveAt(i);
          set.loadedLibraryIDs.RemoveAt(i);
          Debug.LogError("Invalid library detected: Reload " +
                         set.loadedLibraryIDs[i] + " from HDD");
          break;
        }
      }
    }
    ...
    if (library != null) {
      library.hideFlags =
          libraryHideFlag; // ensure correct hideflag with pre 4.3 versions
      set.loadedLibraries.Add(library);
      set.loadedLibraryIDs.Add(presetLibraryPathWithoutExtension);
      return library;
    }
  ...
  public void SaveLibrary<T>(ScriptableObjectSaveLoadHelper<T> helper,
                             T library,
                             string presetLibraryPathWithoutExtension) where T
      : ScriptableObject {
    bool fileExistedBeforeSaving =
        File.Exists(presetLibraryPathWithoutExtension + "." +
                    helper.fileExtensionWithoutDot);
  ...
  private class LibraryCache {
    string m_Identifier; // Identifier for a group of libraries. For now its the
                         // file extension

    // Should have been a Dictonary but we cannot serialize those properly
    // yet...
    List<ScriptableObject> m_LoadedLibraries =
        new List<ScriptableObject>(); // 1:1 with m_LoadedLibraryIDs
    List<string> m_LoadedLibraryIDs =
        new List<string>(); // 1:1 with m_LoadedLibraries
    ...
    public List<ScriptableObject> loadedLibraries {
      get { return m_LoadedLibraries; }
    }
    public List<string> loadedLibraryIDs {
      get {
        return m_LoadedLibraryIDs; // List of paths without extension
      }
    }
    ...
    public LibraryCache(string identifier) { m_Identifier = identifier; }
  }
}
}
