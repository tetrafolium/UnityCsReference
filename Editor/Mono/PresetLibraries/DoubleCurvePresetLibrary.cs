...
class DoubleCurve {
  ...
  [SerializeField] bool m_SignedRange;

  public DoubleCurve(AnimationCurve minCurve, AnimationCurve maxCurve,
                     bool signedRange) {
    ...
    if (maxCurve != null) {
      ...
      Debug.LogError(
          "Ensure that maxCurve is not null when creating a double curve. The minCurve can be null for single curves");
    }
...
class DoubleCurvePresetLibrary : PresetLibrary {
  [SerializeField] List<DoubleCurvePreset> m_Presets =
      new List<DoubleCurvePreset>();
  ...
  readonly Rect kDefaultRange = new Rect(
      0, 0, -1, -1); // Default range triggers a range to be calculated.
  ...
  public override void Add(object presetObject, string presetName) {
    ...
    if (doubleCurve == null) {
      Debug.LogError(
          "Wrong type used in DoubleCurvePresetLibrary: Should be a DoubleCurve");
      return;
    }
  ...
  private void DrawInternal(Rect rect, DoubleCurve doubleCurve) {
    ...
    if (m_UseRanges)
      EditorGUIUtility.DrawRegionSwatch(
          rect, doubleCurve.maxCurve, doubleCurve.minCurve,
          new Color(0.8f, 0.8f, 0.8f, 1.0f), EditorGUI.kCurveBGColor,
          doubleCurve.signedRange ? kSignedRange
          : kUnsignedRange);
    else
      EditorGUIUtility.DrawRegionSwatch(rect, doubleCurve.maxCurve,
                                        doubleCurve.minCurve,
                                        new Color(0.8f, 0.8f, 0.8f, 1.0f),
                                        EditorGUI.kCurveBGColor, kDefaultRange);
  }
  ...
  class DoubleCurvePreset {
    ...
    [SerializeField] DoubleCurve m_DoubleCurve;
