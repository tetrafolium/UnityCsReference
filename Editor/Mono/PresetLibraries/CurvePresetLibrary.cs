...
[AssetFileNameExtension("curves", "curvesNormalized", "particleCurves",
                        "particleCurvesSigned", "particleDoubleCurves",
                        "particleDoubleCurvesSigned")]
[ExcludeFromPreset]
class CurvePresetLibrary : PresetLibrary {
  [SerializeField] List<CurvePreset> m_Presets = new List<CurvePreset>();

  public override int Count() { return m_Presets.Count; }

  public override object GetPreset(int index) { return m_Presets[index].curve; }
  ...
  private void DrawInternal(Rect rect, AnimationCurve animCurve) {
    if (animCurve == null)
      return;
    EditorGUIUtility.DrawCurveSwatch(rect, animCurve, null,
                                     new Color(0.8f, 0.8f, 0.8f, 1.0f),
                                     EditorGUI.kCurveBGColor);
  }
  ...
  class CurvePreset {
    ...
    public CurvePreset(AnimationCurve preset, AnimationCurve preset2,
                       string presetName) {
      curve = preset;
      name = presetName;
    }
