...
internal enum CurveLibraryType { Unbounded, NormalizedZeroToOne }
...
internal class CurvePresetsContentsForPopupWindow : PopupWindowContent {
  ...
  public CurvePresetsContentsForPopupWindow(
      AnimationCurve animCurve, CurveLibraryType curveLibraryType,
      System.Action<AnimationCurve> presetSelectedCallback) {
  ...
  public void InitIfNeeded() {
    if (m_CurveLibraryEditorState == null) {
      m_CurveLibraryEditorState =
          new PresetLibraryEditorState(GetBasePrefText(m_CurveLibraryType));
      m_CurveLibraryEditorState.TransferEditorPrefsState(true);
    }

    if (m_CurveLibraryEditor == null) {
      var saveLoadHelper =
          new ScriptableObjectSaveLoadHelper<CurvePresetLibrary>(
              GetExtension(m_CurveLibraryType), SaveType.Text);
      m_CurveLibraryEditor = new PresetLibraryEditor<CurvePresetLibrary>(
  ...
  void OnPresetsWasReordered() { InternalEditorUtility.RepaintAllViews(); }
  ...
  void AddDefaultPresetsToLibrary(PresetLibrary presetLibrary) {
    ...
    if (curveDefaultLib == null) {
      Debug.Log(
          "Incorrect preset library, should be a CurvePresetLibrary but was a " +
