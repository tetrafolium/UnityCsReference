...
internal partial class ContainerWindow : ScriptableObject {
  ...
  [SerializeField, UnityEngine.Serialization.FormerlySerializedAs(
                       "m_MainView")] View m_RootView;
  ...
  static internal bool macEditor =>
      Application.platform == RuntimePlatform.OSXEditor;
  static internal bool linuxEditor =>
  ...
  private static class Styles {
    ...
    public static GUIStyle buttonClose =
        macEditor ? "WinBtnCloseMac" : "WinBtnClose";
    public static GUIStyle buttonMax = macEditor ? "WinBtnMaxMac" : "WinBtnMax";
    public static GUIStyle buttonRestore =
        macEditor ? "WinBtnRestoreMac" : "WinBtnRestore";

    public static float borderSize => macEditor ? osxBorderSize : winBorderSize;
    public static float buttonMargin =>
        macEditor ? osxBorderMargin : winBorderMargin;

    public static SVC<float> buttonTop =
        new SVC<float>("--container-window-button-top-margin");

    private static SVC<float> winBorderSize =
        new SVC<float>("--container-window-buttons-right-margin-win");
    private static SVC<float> osxBorderSize =
        new SVC<float>("--container-window-buttons-right-margin-osx");
    private static SVC<float> winBorderMargin =
        new SVC<float>("--container-window-button-left-right-margin-win");
    private static SVC<float> osxBorderMargin =
        new SVC<float>("--container-window-button-left-right-margin-osx");
  }

  private const float kButtonCountOSX = 0;
  private const float kButtonCountWin = 2;
  static internal float buttonHorizontalSpace =>
      (kButtonWidth + Styles.buttonMargin * 2f);
  static internal float buttonStackWidth =>
      buttonHorizontalSpace * (macEditor || linuxEditor ? kButtonCountOSX
                               : kButtonCountWin) +
      Styles.borderSize;
  ...
  internal void __internalAwake() { hideFlags = HideFlags.DontSave; }
  ...
  internal void ShowPopup() { ShowPopupWithMode(ShowMode.PopupMenu, true); }
  ...
  internal void ShowPopupWithMode(ShowMode mode, bool giveFocus) {
    ...
    rootView.position =
        new Rect(0, 0, GUIUtility.RoundToPixelGrid(m_PixelRect.width),
                 GUIUtility.RoundToPixelGrid(m_PixelRect.height));
    rootView.Reflow();
  }

  private static readonly Color lightSkinColor =
      new Color(0.541f, 0.541f, 0.541f, 1.0f);
  private static readonly Color darkSkinColor =
      new Color(0.098f, 0.098f, 0.098f, 1.0f);
  static Color skinBackgroundColor =>
      EditorGUIUtility.isProSkin ? darkSkinColor : lightSkinColor;

  // Show the editor window.
  public void Show(ShowMode showMode, bool loadPosition,
                   bool displayImmediately, bool setFocus) {
    bool useMousePos = showMode == ShowMode.AuxWindow;
    if (showMode == ShowMode.AuxWindow)
      showMode = ShowMode.Utility;

    if (showMode == ShowMode.Utility || showMode == ShowMode.ModalUtility ||
        IsPopup(showMode))
    ...
    // Tell the main view its now in this window (quick hack to get
    // platform-specific code to move its views to the right window)
    if (m_RootView)
      m_RootView.SetWindowRecurse(this);
    Internal_SetTitle(m_Title);
    ...
    // Restore the initial maximized state since Internal_BringLiveAfterCreation
    // might not be reflected right away and Save() might alter it.
  ...
  private bool PrivateRequestClose(List<EditorWindow> allUnsaved) {
    ...
    if (allUnsaved.Count == 1) {
      option = EditorUtility.DisplayDialogComplex(
          ...
          allUnsaved.First().saveChangesMessage, L10n.Tr("Save"),
          L10n.Tr("Cancel"), L10n.Tr("Discard"));
    } else {
      string unsavedChangesMessage = string.Join(
          "\n", allUnsaved.Select(v => v.saveChangesMessage).ToArray());

      option = EditorUtility.DisplayDialogComplex(
          L10n.Tr("Unsaved Changes Detected"), unsavedChangesMessage,
          L10n.Tr("Save All"), L10n.Tr("Cancel"), L10n.Tr("Discard All"));
    }
  ...
  internal bool IsNotDocked() {
    return ( // hallelujah
        ...
        (m_ShowMode == (int) ShowMode.Utility ||
         m_ShowMode == (int) ShowMode.AuxWindow) ||
        (m_ShowMode == (int) ShowMode.MainWindow && rootView is HostView) ||
        (rootView is SplitView && rootView.children.Length == 1 &&
  ...
  internal string GetWindowID() {
    ...
    if (rootView.children.Length > 0) {
      var dockArea =
          rootView.children.FirstOrDefault(c => c is DockArea) as DockArea;
      if (dockArea && dockArea.m_Panes.Count > 0) {
        return (m_ShowMode == (int) ShowMode.Utility ||
                m_ShowMode == (int) ShowMode.AuxWindow)
                   ? v.actualView.GetType().ToString()
                   : dockArea
                         .m_Panes [0]
                         .GetType()
                         .ToString();
  ...
  public bool IsMainWindow() {
    if (UnityEditor.MPE.ProcessService.level ==
        UnityEditor.MPE.ProcessLevel.Master)
      return m_ShowMode == (int) ShowMode.MainWindow &&
             m_DontSaveToLayout == false;
    return false;
  }
  ...
  internal void OnResize() {
    ...
    // Depending on the context, GUIUtility.pixelsPerPoint isn't reliable at
    // this point, so we must not round. Anyway the backend is responsible of
    // providing a window size that is aligned with the pixel grid, so it
    // shouldn't be necessary. In the past position.width and position.height
    // were rounded. When moving a window from a monitor that had a given
    // scaling to another monitor with a different scaling, this could cause the
    // original pixelsPerPoint to be used. As a result, black borders could
    // appear for the first frame.
  ...
  internal void AddToWindowList() { s_AllWindows.Add(this); }
  ...
  public View rootView {
    ...
    set {
      m_RootView = value;
      m_RootView.SetWindowRecurse(this);
      m_RootView.position = new Rect(0, 0, Mathf.Ceil(position.width),
                                     Mathf.Ceil(position.height));
  ...
  public SplitView rootSplitView {
    get {
      if (m_ShowMode == (int) ShowMode.MainWindow && rootView &&
          rootView.children.Length == 3)
        return rootView.children[1] as SplitView;
      if (m_ShowMode == (int) ShowMode.MainWindow && rootView &&
          rootView.children.Length == 2)
  ...
  internal string DebugHierarchy() { return rootView.DebugHierarchy(0); }

  internal Rect GetDropDownRect(Rect buttonRect, Vector2 minSize,
                                Vector2 maxSize,
                                PopupLocation[] locationPriorityOrder) {
    return PopupLocationHelper.GetDropDownRect(buttonRect, minSize, maxSize,
                                               this, locationPriorityOrder);
  }
  ...
  public void HandleWindowDecorationStart(Rect windowPosition) {
    if (!macEditor && !linuxEditor) {
      bool hasTitleBar =
          (windowPosition.y == 0 &&
           (showMode != ShowMode.Utility && showMode != ShowMode.MainWindow) &&
           !isPopup);

      if (!hasTitleBar)
        return;

      bool hasWindowButtons =
          Mathf.Abs(windowPosition.xMax - position.width) < 2;
      if (hasWindowButtons) {
        GUIStyle min = Styles.buttonMin;
        GUIStyle close = Styles.buttonClose;
        GUIStyle maxOrRestore =
            maximized ? Styles.buttonRestore : Styles.buttonMax;
        ...
        var canMaximize = m_MaxSize.x == 0 || m_MaxSize.y == 0 ||
                          m_MaxSize.x >= Screen.currentResolution.width ||
                          m_MaxSize.y >= Screen.currentResolution.height;
  ...
  private bool TitleBarButton(GUIStyle style) {
    var buttonRect = new Rect(m_TitleBarWidth - Styles.borderSize -
                                  (buttonHorizontalSpace * ++m_ButtonCount),
                              Styles.buttonTop, kButtonWidth, kButtonHeight);
    var guiView = rootView as GUIView;
    if (guiView == null) {
      var splitView = rootView as SplitView;
      if (splitView != null)
        guiView = splitView.children.Length > 0
                      ? splitView.children[0] as GUIView
                      : null;
    }
    if (guiView != null)
  ...
  private void DragTitleBar(Rect titleBarRect) {
    ...
    case EventType.MouseDown:
      // If the mouse is inside the title bar rect, we say that we're the hot
      // control
      if (titleBarRect.Contains(evt.mousePosition) &&
          GUIUtility.hotControl == 0 && evt.button == 0) {
        Event.current.Use();
    ...
    case EventType.MouseUp:
      ...
      if (GUIUtility.hotControl == id) {
        ...
        Event.current.Use();
        Unsupported.SetAllowCursorLock(
            true, Unsupported.DisallowCursorLockReasons.SizeMove);
      }
      break;
    case EventType.MouseDrag:
      ...
      if (GUIUtility.hotControl == id) {
        ...
        if (startDragDpi != GUIUtility.pixelsPerPoint) {
          // We ignore this mouse event when changing screens in multi monitor
          // setups with different dpi scalings as funky things might/will
          // happen
          ...
          if (Mathf.Abs(movement.x) >= minimumDelta ||
              Mathf.Abs(movement.y) >= minimumDelta) {
