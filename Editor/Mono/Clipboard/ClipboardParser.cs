...
internal static class ClipboardParser {
  ...
  static float[] ParseFloats(string text, string prefix, int count) {
    ...
    for (var i = 0; i < count; ++i) {
      if (float.TryParse(match.Groups[i + 1].Value, NumberStyles.Float,
  ...
  public static string WriteVector3(Vector3 value) {
    return string.Format(CultureInfo.InvariantCulture,
                         "Vector3({0:g9},{1:g9},{2:g9})", value.x, value.y,
                         value.z);
  }
  ...
  public static string WriteVector2(Vector2 value) {
    return string.Format(CultureInfo.InvariantCulture, "Vector2({0:g9},{1:g9})",
                         value.x, value.y);
  }
  ...
  public static string WriteVector4(Vector4 value) {
    return string.Format(CultureInfo.InvariantCulture,
                         "Vector4({0:g9},{1:g9},{2:g9},{3:g9})", value.x,
  ...
  public static string WriteRect(Rect value) {
    return string.Format(CultureInfo.InvariantCulture,
                         "Rect({0:g9},{1:g9},{2:g9},{3:g9})", value.x, value.y,
  ...
  public static bool ParseBounds(string text, out Bounds res) {
    ...
    if (v == null)
      return false;
    res = new Bounds(new Vector3(v[0], v[1], v[2]),
                     new Vector3(v[3] * 2, v[4] * 2, v[5] * 2));
    return true;
  }
  ...
  public static string WriteQuaternion(Quaternion value) {
    return string.Format(CultureInfo.InvariantCulture,
                         "Quaternion({0:g9},{1:g9},{2:g9},{3:g9})", value.x,
                         value.y, value.z, value.w);
  }
  ...
  public static string WriteColor(Color val) {
    ...
    return string.Format(CultureInfo.InvariantCulture,
                         "Color({0:g9},{1:g9},{2:g9},{3:g9})", val.r, val.g,
                         val.b, val.a);
  }

  internal const string kEnumPrefix = "Enum:";

  public static int ParseEnumPropertyIndex(string text,
  ...
  static string CustomPrefix<T>() { return typeof(T).FullName + "JSON:"; }

  // Given a serialized property, produce an object structure for JSON encoding
  // of it.
  internal static Dictionary<string, object>
  WriteGenericSerializedProperty(SerializedProperty p) {
    var res = new Dictionary<string, object>{["name"] = p.name,
    ...
    case SerializedPropertyType.Float:
      res["val"] = p.floatValue;
      break;
    case SerializedPropertyType.String:
      res["val"] = p.stringValue;
      break;
    case SerializedPropertyType.ObjectReference:
      res ["val"]
      = WriteCustom(new ObjectWrapper(p.objectReferenceValue));
      break;
    ...
    case SerializedPropertyType.AnimationCurve:
      res ["val"]
      = WriteCustom(new AnimationCurveWrapper(p.animationCurveValue));
      break;
    ...
    case SerializedPropertyType.Bounds:
      res ["val"]
      = WriteBounds(p.boundsValue);
      break;
    case SerializedPropertyType.Gradient:
      res ["val"]
      = WriteCustom(new GradientWrapper(p.gradientValue));
      break;
    case SerializedPropertyType.Quaternion:
      res ["val"]
      = WriteQuaternion(p.quaternionValue);
      break;
    case SerializedPropertyType.Vector2Int:
      res ["val"]
      = WriteVector2(p.vector2IntValue);
      break;
    case SerializedPropertyType.Vector3Int:
      res ["val"]
      = WriteVector3(p.vector3IntValue);
      break;
    case SerializedPropertyType.RectInt:
      var ri = p.rectIntValue;
      res ["val"]
      = WriteRect(new Rect(ri.x, ri.y, ri.width, ri.height));
      break;
    case SerializedPropertyType.BoundsInt:
      var bi = p.boundsIntValue;
      res ["val"]
      = WriteBounds(new Bounds(bi.center, bi.size));
      break;

    // Copy/Paste of these for generic serialized properties is not implemented
    // yet.
  ...
  // Given an object structure from JSON encoding, apply that to a serialized
  // property.
  internal static void
  ParseGenericSerializedProperty(SerializedProperty prop,
    ...
    try {
      ...
      case SerializedPropertyType.String:
        ...
        break;
      case SerializedPropertyType.ObjectReference:
        if (ParseCustom<ObjectWrapper>(Convert.ToString(oval),
                                       out var objectWrapper))
      ...
      case SerializedPropertyType.ArraySize:
        ...
        break;
      case SerializedPropertyType.AnimationCurve:
        if (ParseCustom<AnimationCurveWrapper>(Convert.ToString(oval),
                                               out var animWrapper))
      ...
      case SerializedPropertyType.Bounds:
        ...
        break;
      case SerializedPropertyType.Gradient:
        if (ParseCustom<GradientWrapper>(Convert.ToString(oval),
                                         out var gradientWrapper)) {
      ...
      case SerializedPropertyType.Vector2Int:
        if (ParseVector2(Convert.ToString(oval), out var v2Value))
          prop.vector2IntValue =
              new Vector2Int((int) v2Value.x, (int) v2Value.y);
        break;
      case SerializedPropertyType.Vector3Int:
        if (ParseVector3(Convert.ToString(oval), out var v3Value))
          prop.vector3IntValue =
              new Vector3Int((int) v3Value.x, (int) v3Value.y, (int) v3Value.z);
        break;
      case SerializedPropertyType.RectInt:
        if (ParseRect(Convert.ToString(oval), out var rectValue))
          prop.rectIntValue =
              new RectInt((int) rectValue.x, (int) rectValue.y,
                          (int) rectValue.width, (int) rectValue.height);
        break;
      case SerializedPropertyType.BoundsInt:
        if (ParseBounds(Convert.ToString(oval), out var biValue))
          prop.boundsIntValue = new BoundsInt(
              new Vector3Int((int) biValue.center.x, (int) biValue.center.y,
                             (int) biValue.center.z),
              new Vector3Int((int) biValue.size.x, (int) biValue.size.y,
                             (int) biValue.size.z));
        break;

      // Copy/Paste of these for generic serialized properties is not
      // implemented yet.
      case SerializedPropertyType.FixedBufferSize:
        break;
      case SerializedPropertyType.ExposedReference:
...
internal class ObjectWrapper {
  ...
  public Object ToObject() {
    var id =
        new ObjectIdentifier{guid = new GUID(guid),
                             localIdentifierInFile = localId, fileType = type};
    if (!id.guid.Empty()) {
  ...
  // instanceIDs are for in-scene references, not stable across sessions but
  // fine for local copy/paste
