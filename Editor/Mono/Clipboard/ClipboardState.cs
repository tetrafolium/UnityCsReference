...
// Doing queries or gets into the state (e.g. query for presence of color in the
// clipboard) is cached and figured out only once per state lifetime.
internal class ClipboardState {
  ...
  internal void FetchLayerMask() {
    if (!m_HasLayerMask.HasValue)
      m_HasLayerMask =
          ClipboardParser.ParseLayerMask(m_RawContents, out m_ValueLayerMask);
  }
  ...
  internal void FetchVector3() {
    if (!m_HasVector3.HasValue)
      m_HasVector3 =
          ClipboardParser.ParseVector3(m_RawContents, out m_ValueVector3);
  }
  ...
  internal void FetchVector2() {
    if (!m_HasVector2.HasValue)
      m_HasVector2 =
          ClipboardParser.ParseVector2(m_RawContents, out m_ValueVector2);
  }
  ...
  internal void FetchVector4() {
    if (!m_HasVector4.HasValue)
      m_HasVector4 =
          ClipboardParser.ParseVector4(m_RawContents, out m_ValueVector4);
  }
  ...
  internal void FetchQuaternion() {
    if (!m_HasQuaternion.HasValue)
      m_HasQuaternion =
          ClipboardParser.ParseQuaternion(m_RawContents, out m_ValueQuaternion);
  }
  ...
  internal void FetchBounds() {
    if (!m_HasBounds.HasValue)
      m_HasBounds =
          ClipboardParser.ParseBounds(m_RawContents, out m_ValueBounds);
  }
  ...
  internal void FetchObject() {
    if (!m_HasObject.HasValue) {
      m_HasObject = ClipboardParser.ParseCustom<ObjectWrapper>(m_RawContents,
                                                               out var wrapper);
  ...
  internal void FetchGradient() {
    if (!m_HasGradient.HasValue) {
      m_HasGradient = ClipboardParser.ParseCustom<GradientWrapper>(
          m_RawContents, out var wrapper);
  ...
  internal void FetchAnimationCurve() {
    if (!m_HasAnimationCurve.HasValue) {
      m_HasAnimationCurve = ClipboardParser.ParseCustom<AnimationCurveWrapper>(
          m_RawContents, out var wrapper);
      m_ValueAnimationCurve = wrapper.curve;
    }
  }

  readonly Dictionary<Type, object> m_ValuesCustom =
