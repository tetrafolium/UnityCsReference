...
// ReSharper disable once CheckNamespace - we explicitly want UnityEditor
// namespace
...
internal static class ClipboardContextMenu {
  ...
  internal static void SetupPropertyCopyPaste(SerializedProperty property,
                                              GenericMenu menu, Event evt) {
    var type = property.propertyType;
    // ReSharper disable once SwitchStatementMissingSomeEnumCasesNoDefault -- we
    // don't support all the types
    switch (type) {
    ...
    case SerializedPropertyType.Boolean:
      SetupAction(property, menu, evt, p => Clipboard.boolValue = p.boolValue,
                  p => Clipboard.hasBool,
                  p => p.boolValue = Clipboard.boolValue);
      break;
    case SerializedPropertyType.Quaternion:
      SetupQuaternion(property, menu, evt);
      break;
    case SerializedPropertyType.ObjectReference:
      SetupObjectReference(property, menu, evt);
      break;
    case SerializedPropertyType.Color:
      SetupAction(property, menu, evt, p => Clipboard.colorValue = p.colorValue,
    ...
    case SerializedPropertyType.Gradient:
      SetupAction(property, menu, evt,
                  ...
                  p => Clipboard.hasGradient, delegate(SerializedProperty p) {
    ...
    case SerializedPropertyType.Vector2Int:
      SetupAction(property, menu, evt,
                  ...
                  p => Clipboard.hasVector2, delegate(SerializedProperty p) {
    ...
    case SerializedPropertyType.Vector3Int:
      SetupAction(property, menu, evt,
                  ...
                  p => Clipboard.hasVector3, delegate(SerializedProperty p) {
                    var vi = Clipboard.vector3Value;
                    p.vector3IntValue =
                        new Vector3Int((int) vi.x, (int) vi.y, (int) vi.z);
                  });
      break;
    case SerializedPropertyType.Rect:
      SetupAction(property, menu, evt, p => Clipboard.rectValue = p.rectValue,
                  p => Clipboard.hasRect,
    ...
    case SerializedPropertyType.RectInt:
      SetupAction(property, menu, evt,
                  delegate(SerializedProperty p) {
                    ...
                    Clipboard.rectValue =
                        new Rect(ri.x, ri.y, ri.width, ri.height);
                  },
                  p => Clipboard.hasRect,
                  delegate(SerializedProperty p) {
                    var r = Clipboard.rectValue;
                    p.rectIntValue = new RectInt((int) r.x, (int) r.y,
                                                 (int) r.width, (int) r.height);
                  });
      break;
    case SerializedPropertyType.Bounds:
      SetupAction(
          property, menu, evt, p => Clipboard.boundsValue = p.boundsValue,
          p => Clipboard.hasBounds, p => p.boundsValue = Clipboard.boundsValue);
      break;
    ...
    case SerializedPropertyType.Enum:
      SetupAction(property, menu, evt, Clipboard.SetEnumProperty,
                  Clipboard.HasEnumProperty, Clipboard.GetEnumProperty);
      break;
    case SerializedPropertyType.Generic:
      SetupAction(property, menu, evt, Clipboard.SetSerializedProperty,
  ...
  static readonly GUIContent kCopyContent =
      EditorGUIUtility.TrTextContent("Copy");
  static readonly GUIContent kPasteContent =
      EditorGUIUtility.TrTextContent("Paste");
  static readonly GUIContent kCopyEulerContent =
      EditorGUIUtility.TrTextContent("Copy Euler Angles");
  static readonly GUIContent kCopyQuatContent =
      EditorGUIUtility.TrTextContent("Copy Quaternion");
  static readonly GUIContent kCopyPathContent =
      EditorGUIUtility.TrTextContent("Copy Path");
  static readonly GUIContent kCopyGuidContent =
      EditorGUIUtility.TrTextContent("Copy GUID");
  ...
  static void SetupAction(SerializedProperty property, GenericMenu menu,
                          Event evt, Action<SerializedProperty> copyFunc,
    ...
    if (menu != null) {
      ...
      if (canCopy)
        menu.AddItem(kCopyContent, false, o => copyFunc((SerializedProperty) o),
                     property);
      else
        menu.AddDisabledItem(kCopyContent);
      if (canPaste)
        menu.AddItem(kPasteContent, false, delegate(object o) {
  ...
  static void PasteQuaternion(SerializedProperty p) {
    p.quaternionValue =
        Clipboard.hasQuaternion ? Clipboard.quaternionValue
        : Quaternion.Euler(Clipboard.vector3Value);
    p.serializedObject.ApplyModifiedProperties();
  }

  static void SetupQuaternion(SerializedProperty property, GenericMenu menu,
                              Event evt) {
    var canCopy = !property.hasMultipleDifferentValues;
    var canPaste =
        GUI.enabled && (Clipboard.hasQuaternion || Clipboard.hasVector3);

    if (menu != null) {
      AddSeparator(menu);
      if (canCopy) {
        menu.AddItem(kCopyEulerContent, false,
                     o => Clipboard.vector3Value =
                         ((SerializedProperty) o).quaternionValue.eulerAngles,
                     property);
        menu.AddItem(kCopyQuatContent, false,
                     o => Clipboard.quaternionValue =
                         ((SerializedProperty) o).quaternionValue,
                     property);
      } else {
      ...
      if (canPaste)
        menu.AddItem(kPasteContent, false,
                     o => PasteQuaternion((SerializedProperty) o), property);
      else
        menu.AddDisabledItem(kPasteContent);
    }
    if (evt != null) {
  ...
  static void PasteObjectReference(SerializedProperty p) {
    if (Clipboard.hasObject)
      ...
      p.objectReferenceValue =
          AssetDatabase.LoadMainAssetAtGUID(Clipboard.guidValue);
    else if (!new GUID(AssetDatabase.AssetPathToGUID(Clipboard.stringValue))
                  .Empty())
      p.objectReferenceValue =
          AssetDatabase.LoadMainAssetAtPath(Clipboard.stringValue);
    p.serializedObject.ApplyModifiedProperties();
  }

  static void SetupObjectReference(SerializedProperty property,
                                   GenericMenu menu, Event evt) {
    var hasPasteObject = Clipboard.hasObject;
    var hasPasteGuid = Clipboard.hasGuid;
    var hasPastePath =
        !new GUID(AssetDatabase.AssetPathToGUID(Clipboard.stringValue)).Empty();
    var obj = property.objectReferenceValue;

    var canCopy = !property.hasMultipleDifferentValues && obj != null;
    var canPaste =
        GUI.enabled && (hasPasteGuid || hasPastePath || hasPasteObject);

    if (menu != null) {
      AddSeparator(menu);
      var hasAsset = canCopy && AssetDatabase.Contains(obj);
      if (canCopy)
        menu.AddItem(kCopyContent, false,
                     o => Clipboard.objectValue = (UnityEngine.Object) o, obj);
      else
        menu.AddDisabledItem(kCopyContent);
      if (hasAsset) {
        menu.AddItem(
            kCopyPathContent, false,
            o => Clipboard.stringValue =
                AssetDatabase.GetAssetPath((UnityEngine.Object) o).ToString(),
            obj);
        menu.AddItem(
            kCopyGuidContent, false,
            o => Clipboard.guidValue = new GUID(AssetDatabase.AssetPathToGUID(
                AssetDatabase.GetAssetPath((UnityEngine.Object) o))),
            obj);
      } else {
      ...
      if (canPaste)
        menu.AddItem(kPasteContent, false,
                     o => PasteObjectReference((SerializedProperty) o),
                     property);
      else
        menu.AddDisabledItem(kPasteContent);
    }
  ...
  [MenuItem("CONTEXT/Transform/Copy Position")] // ReSharper disable once
                                                // UnusedMember.Local
  ...
  [MenuItem("CONTEXT/Transform/Paste Position",
            true)] // ReSharper disable once UnusedMember.Local
  ...
  [MenuItem("CONTEXT/Transform/Paste Position")] // ReSharper disable once
                                                 // UnusedMember.Local
  ...
  [MenuItem("CONTEXT/Transform/Copy Rotation")] // ReSharper disable once
                                                // UnusedMember.Local
  ...
  [MenuItem("CONTEXT/Transform/Paste Rotation",
    ...
    var tr = command.context as Transform;
    return tr != null && (Clipboard.hasVector3 || Clipboard.hasQuaternion) &&
           IsUserModifiable(tr);
  }

  [MenuItem("CONTEXT/Transform/Paste Rotation")] // ReSharper disable once
                                                 // UnusedMember.Local
  ...
  [MenuItem("CONTEXT/Transform/Copy Scale")] // ReSharper disable once
                                             // UnusedMember.Local
  ...
  [MenuItem("CONTEXT/Transform/Paste Scale")] // ReSharper disable once
                                              // UnusedMember.Local
  ...
  [MenuItem("CONTEXT/Transform/Copy World Placement")] // ReSharper disable once
                                                       // UnusedMember.Local
  ...
  [MenuItem("CONTEXT/Transform/Paste World Placement",
            true)] // ReSharper disable once UnusedMember.Local
      static bool
      PasteTransformWorldPlacementMenuValidate(MenuCommand command) {
    var tr = command.context as Transform;
    return tr != null && IsUserModifiable(tr) &&
           Clipboard.HasCustomValue<TransformWorldPlacement>();
  }

  [MenuItem("CONTEXT/Transform/Paste World Placement")] // ReSharper disable
                                                        // once
                                                        // UnusedMember.Local
