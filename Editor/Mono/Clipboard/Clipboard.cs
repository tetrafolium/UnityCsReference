...
public static Vector3 vector3Value {
	...
	set => EditorGUIUtility.systemCopyBuffer =
		ClipboardParser.WriteVector3(value);
}
...
public static Vector2 vector2Value {
	...
	set => EditorGUIUtility.systemCopyBuffer =
		ClipboardParser.WriteVector2(value);
}
...
public static Vector4 vector4Value {
	...
	set => EditorGUIUtility.systemCopyBuffer =
		ClipboardParser.WriteVector4(value);
}
...
public static Quaternion quaternionValue {
	...
	set => EditorGUIUtility.systemCopyBuffer =
		ClipboardParser.WriteQuaternion(value);
}
...
public static Bounds boundsValue {
	...
	set => EditorGUIUtility.systemCopyBuffer =
		ClipboardParser.WriteBounds(value);
}
...
public static Color colorValue {
	...
	set => EditorGUIUtility.systemCopyBuffer =
		ClipboardParser.WriteColor(value);
}
...
public static Object objectValue {
	...
	set => EditorGUIUtility.systemCopyBuffer =
		ClipboardParser.WriteCustom(new ObjectWrapper(value));
}
...
public static Gradient gradientValue {
	...
	set => EditorGUIUtility.systemCopyBuffer =
		ClipboardParser.WriteCustom(new GradientWrapper(value));
}
...
public static AnimationCurve animationCurveValue {
	...
	set => EditorGUIUtility.systemCopyBuffer =
		ClipboardParser.WriteCustom(new AnimationCurveWrapper(value));
}
...
public static void GetSerializedProperty(SerializedProperty dst) {
	...
	if (!clip.StartsWith(kGenericPrefix))
		return;
	if (!(Json.Deserialize(clip.Substring(kGenericPrefix.Length))
	      is Dictionary<string, object> jsonObjects))
		return;
...
static void FetchState() {
	...
	if (systemClipboard == m_State.m_RawContents)
		return;
	m_State = new ClipboardState {
		m_RawContents = systemClipboard
	};
}
}
}
