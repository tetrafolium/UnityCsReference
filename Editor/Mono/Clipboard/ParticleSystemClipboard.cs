...
internal class ParticleSystemClipboard {
  ...
  public static void CopyAnimationCurves(AnimationCurve animCurve,
  ...
  static void ClampCurve(SerializedProperty animCurveProperty,
                         Rect curveRanges) {
    AnimationCurve clampedCurve = animCurveProperty.animationCurveValue;
    Keyframe[] keys = clampedCurve.keys;
    for (int i = 0; i < keys.Length; ++i) {
      keys[i].time =
          Mathf.Clamp(keys[i].time, curveRanges.xMin, curveRanges.xMax);
      keys[i].value =
          Mathf.Clamp(keys[i].value, curveRanges.yMin, curveRanges.yMax);
    }
  ...
  public static void
  PasteAnimationCurves(SerializedProperty animCurveProperty,
                       SerializedProperty animCurveProperty2,
                       SerializedProperty scalarProperty, Rect curveRanges,
...
internal class GradientContextMenu {
  ...
  GradientContextMenu(SerializedProperty prop) { m_Property = prop; }

  void Copy() { Clipboard.gradientValue = m_Property?.gradientValue; }
...
internal class AnimationCurveContextMenu {
  ...
  static internal void Show(Rect position, SerializedProperty property,
                            SerializedProperty property2,
                            SerializedProperty scalar, Rect curveRanges,
                            ParticleSystemCurveEditor curveEditor) {
    // Curve context menu
    GUIContent copy = EditorGUIUtility.TrTextContent("Copy");
    GUIContent paste = EditorGUIUtility.TrTextContent("Paste");
    ...
    bool validPaste =
        (isRegion && ParticleSystemClipboard.HasDoubleAnimationCurve()) ||
        (!isRegion && ParticleSystemClipboard.HasSingleAnimationCurve());

    AnimationCurveContextMenu obj = new AnimationCurveContextMenu(
        property, property2, scalar, curveRanges, curveEditor);
  ...
  private AnimationCurveContextMenu(SerializedProperty prop1,
                                    SerializedProperty prop2,
                                    SerializedProperty scalar, Rect curveRanges,
                                    ParticleSystemCurveEditor owner) {
    m_Prop1 = prop1;
    m_Prop2 = prop2;
    m_Scalar = scalar;
  ...
  private void Copy() {
    AnimationCurve animCurve1 =
        m_Prop1 != null ? m_Prop1.animationCurveValue : null;
    AnimationCurve animCurve2 =
        m_Prop2 != null ? m_Prop2.animationCurveValue : null;
  ...
  private void Paste() {
    ParticleSystemClipboard.PasteAnimationCurves(
