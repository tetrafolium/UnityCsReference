...
using SelectorHandlerType =
    System.Func<UnityEditor.SearchService.ObjectSelectorTargetInfo,
                UnityEngine.Object[],
                UnityEditor.SearchService.ObjectSelectorSearchContext, bool>;
...
internal class ObjectSelector : EditorWindow {
  ...
  static class Styles {
    ...
    public static GUIStyle previewBackground =
        "PopupCurveSwatchBackground"; // TODO: Make dedicated style
    public static GUIStyle previewTextureBackground =
        "ObjectPickerPreviewBackground"; // TODO: Make dedicated style

    public static GUIContent assetsTabLabel =
        EditorGUIUtility.TrTextContent("Assets");
    public static GUIContent sceneTabLabel =
        EditorGUIUtility.TrTextContent("Scene");

    public static readonly GUIContent packagesVisibilityContent =
        EditorGUIUtility.TrIconContent(
            "SceneViewVisibility",
            "Number of hidden packages, click to toggle packages visibility");
  }
  ...
  Rect listPosition {
    get {
      return new Rect(0, kTopAreaHeight, position.width,
                      Mathf.Max(0f, m_TopSize - kTopAreaHeight));
    }
  }
  ...
  public static ObjectSelector get {
    get {
      if (s_SharedObjectSelector == null) {
        UnityObject[] objs =
            Resources.FindObjectsOfTypeAll(typeof(ObjectSelector));
        if (objs != null && objs.Length > 0)
          s_SharedObjectSelector = (ObjectSelector) objs[0];
        if (s_SharedObjectSelector == null)
          s_SharedObjectSelector =
              ScriptableObject.CreateInstance<ObjectSelector>();
      }
  ...
  public static bool isVisible {
    get { return s_SharedObjectSelector != null; }
  }

  bool IsUsingTreeView() { return m_ObjectTreeWithSearch.IsInitialized(); }

  int GetInternalSelectedInstanceID() {
    if (m_ListArea == null)
      InitIfNeeded();
    int[] selection = IsUsingTreeView() ? m_ObjectTreeWithSearch.GetSelection()
                                        : m_ListArea.GetSelection();
    if (selection.Length >= 1)
      return selection[0];
    return 0;
  }

  int GetSelectedInstanceID() { return m_LastSelectedInstanceId; }
  ...
  internal string searchFilter {
    ...
    set {
      if (SearchService.ObjectSelector.HasEngineOverride()) {
        SearchService.ObjectSelector.SetSearchFilter(
            value, (SearchService.ObjectSelectorSearchContext)
                       m_SearchSessionHandler.context);
        return;
      }
  ...
  void FilterSettingsChanged() {
    ...
    var hierarchyType =
        m_IsShowingAssets ? HierarchyType.Assets : HierarchyType.GameObjects;

    if (hierarchyType == HierarchyType.GameObjects) {
      if (m_ObjectBeingEdited != null) {
        var scene = GetSceneFromObject(m_ObjectBeingEdited);
        if (scene.IsValid()) {
          // We do not support cross scene references so ensure we only show
          // game objects from the same scene as the object being edited is part
          // of. Also don't allow references to other scenes if object being
          // edited is in a preview scene.
        ...
        // If we don't know which object is being edited, assume it's one in
        // current stage.
        PreviewSceneStage previewSceneStage =
            StageUtility.GetCurrentStage() as PreviewSceneStage;
    ...
    if (hierarchyType == HierarchyType.Assets) {
      // When AssemblyDefinitionAsset is the required type, don't skip hidden
      // packages
      if (!string.IsNullOrEmpty(m_RequiredType) &&
          m_RequiredType == typeof(AssemblyDefinitionAsset).Name) {
    ...
    var requiredType =
        TypeCache.GetTypesDerivedFrom<UnityEngine.Object>().FirstOrDefault(
            t => t.Name == m_RequiredType)
        ?? typeof(UnityObject);
  ...
  internal void Show(Type requiredType, SerializedProperty property,
                     bool allowSceneObjects,
                     List<int> allowedInstanceIDs = null,
                     Action<UnityObject> onObjectSelectorClosed = null,
                     Action<UnityObject> onObjectSelectedUpdated = null) {
    if (property == null)
    ...
    if (requiredType == null) {
      ScriptAttributeUtility.GetFieldInfoFromProperty(property,
                                                      out requiredType);
      // case 951876: built-in types do not actually have reflectable fields, so
      // their object types must be extracted from the type string this works
      // because built-in types will only ever have serialized references to
      // other built-in types, which this window's filter expects as unqualified
      // names
      if (requiredType == null)
        m_RequiredType =
            s_MatchPPtrTypeName.Match(property.type).Groups[1].Value;
    }

    // Don't select anything on multi selection
    UnityObject obj = property.hasMultipleDifferentValues ? null
        : property.objectReferenceValue;

    objectBeingEdited = property.serializedObject.targetObject;
    m_EditedProperty = property;

    Show(obj, requiredType, objectBeingEdited, allowSceneObjects,
         allowedInstanceIDs, onObjectSelectorClosed, onObjectSelectedUpdated);
  }

  internal void Show(UnityObject obj, Type requiredType,
                     UnityObject objectBeingEdited, bool allowSceneObjects,
                     List<int> allowedInstanceIDs = null,
                     Action<UnityObject> onObjectSelectorClosed = null,
                     Action<UnityObject> onObjectSelectedUpdated = null) {
    ...
    // Do not allow to show scene objects if the object being edited is
    // persistent
    if (m_ObjectBeingEdited != null &&
        EditorUtility.IsPersistent(m_ObjectBeingEdited))
    ...
    if (m_AllowSceneObjects) {
      if (obj != null) {
        ...
        m_IsShowingAssets =
            (requiredType !=
             typeof(GameObject) &&!typeof(Component).IsAssignableFrom(
                 requiredType));
      }
    } else {
      m_IsShowingAssets = true;
    ...
    // type filter requires unqualified names for built-in types, but will
    // prioritize them over user types, so ensure user types are
    // namespace-qualified
    if (requiredType != null)
      m_RequiredType =
    ...
    if (SearchService.ObjectSelector.HasEngineOverride()) {
      ...
      var searchContext = (SearchService.ObjectSelectorSearchContext)
                              m_SearchSessionHandler.context;
      Action<UnityObject> onSelectionChanged = selectedObj => {
        m_LastSelectedInstanceId =
            selectedObj == null ? 0 : selectedObj.GetInstanceID();
      ...
      if (SearchService.ObjectSelector.SelectObject(
              searchContext, onSelectorClosed, onSelectionChanged))
        return;
    }
    ...
    titleContent = EditorGUIUtility.TrTextContent(
        "Select " + (requiredType == null ? m_RequiredType
                     : requiredType.Name));
    ...
    position = p;
    minSize = new Vector2(kMinWidth, kMinTopSize + kPreviewExpandedAreaHeight +
                                         2 * kPreviewMargin);
    ...
    // Add after unfreezing display because AuxWindowManager.cpp assumes that
    // aux windows are added after we get 'got/lost'- focus calls.
    if (m_Parent != null)
    ...
    if (initialSelection != 0) {
      ...
      if (m_SkipHiddenPackages &&
          !PackageManagerUtilityInternal.IsPathInVisiblePackage(assetPath))
        m_SkipHiddenPackages = false;
    }

    if (ShouldTreeViewBeUsed(m_RequiredType)) {
      m_ObjectTreeWithSearch.Init(position, this, CreateAndSetTreeView,
                                  TreeViewSelection, ItemWasDoubleClicked,
                                  initialSelection, 0);
    } else {
      // To frame the selected item we need to wait to initialize the search
      // until our window has been setup
      InitIfNeeded();
  ...
  public static UnityObject GetCurrentObject() {
    return EditorUtility.InstanceIDToObject(
        ObjectSelector.get.GetSelectedInstanceID());
  }
  ...
  void SearchArea() {
    GUI.Label(new Rect(0, 0, position.width, kToolbarHeight), GUIContent.none,
              EditorStyles.toolbar);

    // ESC clears search field and removes it's focus. But if we get an esc
    // event we only want to clear search field. So we need special handling
    // afterwards.
    bool wasEscape = Event.current.type == EventType.KeyDown &&
                     Event.current.keyCode == KeyCode.Escape;

    GUI.SetNextControlName("SearchFilter");
    string searchFilter = EditorGUI.ToolbarSearchField(
        new Rect(2, 2, position.width - 2, 16), m_SearchFilter, false);
    ...
    GUI.Label(new Rect(0, kToolbarHeight, position.width, kToolbarHeight),
              GUIContent.none, EditorStyles.toolbar);

    // TAB BAR
    GUILayout.BeginArea(
        new Rect(4, kToolbarHeight, position.width - 4, kToolbarHeight));
    GUILayout.BeginHorizontal();

    // Asset Tab
    bool showAssets =
        GUILayout.Toggle(m_IsShowingAssets, Styles.assetsTabLabel, Styles.tab);
    ...
    showingSceneTab =
        GUILayout.Toggle(showingSceneTab, Styles.sceneTabLabel, Styles.tab);
    ...
    if (m_IsShowingAssets) {
      Styles.packagesVisibilityContent.text =
          PackageManagerUtilityInternal.HiddenPackagesCount.ToString();
      size =
          EditorStyles.toolbarButton.CalcSize(Styles.packagesVisibilityContent);
    }

    if (m_ListArea.CanShowThumbnails()) {
      EditorGUI.BeginChangeCheck();
      var newGridSize = (int) GUI.HorizontalSlider(
          new Rect(position.width - (60 + size.x),
                   kToolbarHeight + GUI.skin.horizontalSlider.margin.top, 55,
                   EditorGUI.kSingleLineHeight),
          m_ListArea.gridSize, m_ListArea.minGridSize, m_ListArea.maxGridSize);
    ...
    if (m_IsShowingAssets) {
      EditorGUI.BeginChangeCheck();
      var skipHiddenPackages =
          GUI.Toggle(new Rect(position.width - size.x, kToolbarHeight, size.x,
                              EditorStyles.toolbarButton.fixedHeight),
                     m_SkipHiddenPackages, Styles.packagesVisibilityContent,
  ...
  void OnInspectorUpdate() {
    if (m_ListArea != null && AssetPreview.HasAnyNewPreviewTexturesAvailable(
                                  m_ListArea.GetAssetPreviewManagerID()))
      Repaint();
  }

  // This is the preview area at the bottom of the screen
  void PreviewArea() {
    GUI.Box(new Rect(0, m_TopSize, position.width, m_PreviewSize), "",
    ...
    if (m_PreviewSize < kPreviewExpandedAreaHeight) {
      ...
      if (selectedObject != null) {
        ...
        string typeName =
            ObjectNames.NicifyVariableName(selectedObject.GetType().Name);
        if (p != null)
          s = p.name + " (" + typeName + ")";
        else
  ...
  void WidePreview(float actualSize, string s, UnityObject o, EditorWrapper p) {
    ...
    Rect previewRect =
        new Rect(margin, m_TopSize + margin, actualSize - margin * 2,
                 actualSize - margin * 2);

    Rect labelRect = new Rect(
        m_PreviewSize + 3,
        m_TopSize + (m_PreviewSize - kPreviewExpandedAreaHeight) * 0.5f,
        m_Parent.window.position.width - m_PreviewSize - 3 - margin,
        kPreviewExpandedAreaHeight);
  ...
  void OverlapPreview(float actualSize, string s, UnityObject o,
                      EditorWrapper p) {
    float margin = kPreviewMargin;
    Rect previewRect =
        new Rect(margin, m_TopSize + margin, position.width - margin * 2,
                 actualSize - margin * 2);
    ...
    if (EditorGUIUtility.isProSkin)
      ...
      EditorGUI.DoDropShadowLabel(previewRect, EditorGUIUtility.TempContent(s),
                                  Styles.largeStatus, .3f);
  }

  void LinePreview(string s, UnityObject o, EditorWrapper p) {
    if (m_ListArea.m_SelectedObjectIcon != null)
      GUI.DrawTexture(new Rect(2, (int)(m_TopSize + 2), 16, 16),
                      m_ListArea.m_SelectedObjectIcon, ScaleMode.StretchToFill);
  ...
  void DrawObjectIcon(Rect position, Texture icon) {
    ...
    GUI.DrawTexture(new Rect(position.x + ((int) position.width - size) / 2,
                             position.y + ((int) position.height - size) / 2,
                             size, size),
  ...
  void ResizeBottomPartOfWindow() {
    ...
    // Handle preview size
    m_PreviewSize =
        m_PreviewResizer.ResizeHandle(
            position,
            kPreviewExpandedAreaHeight + kPreviewMargin * 2 - kResizerHeight,
            kMinTopSize + kResizerHeight, kResizerHeight) +
        kResizerHeight;
  ...
  bool PreviewIsOpen() { return m_PreviewSize >= 32 + kPreviewMargin; }

  bool PreviewIsWide() {
    return position.width - m_PreviewSize - kPreviewMargin >
           Mathf.Min(m_PreviewSize * 2 - 20, 256);
  }
  ...
  void OnGUI() {
    ...
    // Must be after gui so search field can use the Escape event if it has
    // focus
    if (Event.current.type == EventType.KeyDown &&
        Event.current.keyCode == KeyCode.Escape)
      Cancel();
  ...
  void OnObjectGridGUI() {
    ...
    GUI.Label(new Rect(position.width * .5f - 16,
                       position.height - m_PreviewSize + 2, 32,
                       Styles.bottomResize.fixedHeight),
              GUIContent.none, Styles.bottomResize);
  }
  ...
  SelectorHandlerType
  GetSelectorHandlerFromProperty(SerializedProperty property) {
    var fieldInfo =
        ScriptAttributeUtility.GetFieldInfoFromProperty(property, out _);
    if (fieldInfo == null)
      return null;
  ...
  static SelectorHandlerType
  GetSelectorHandlerFromFieldInfo(FieldInfo fieldInfo) {
    var customHandlerAttributes =
        fieldInfo.GetCustomAttributes(typeof(Attribute), false)
            .Where(attr =>
                       !(attr is ObjectSelectorHandlerWithLabelsAttribute) &&
                       !(attr is ObjectSelectorHandlerWithTagsAttribute))
            .Select(attr =>
                        GetCustomSelectorHandlerForAttribute(attr as Attribute))
            .Where(attr => attr != null)
            .ToList();
    var handlersWithLabelsAttribute =
        fieldInfo
            .GetCustomAttributes(
                typeof(ObjectSelectorHandlerWithLabelsAttribute), false)
            .OfType<ObjectSelectorHandlerWithLabelsAttribute>()
            .ToList();
    var handlersWithTagsAttribute =
        fieldInfo
            .GetCustomAttributes(typeof(ObjectSelectorHandlerWithTagsAttribute),
                                 false)
            .OfType<ObjectSelectorHandlerWithTagsAttribute>()
            .ToList();

    var totalCount = customHandlerAttributes.Count +
                     handlersWithLabelsAttribute.Count +
                     handlersWithTagsAttribute.Count;
    if (totalCount == 0)
      return null;

    if (totalCount > 1) {
      Debug.LogWarning(
          $"Multiple {GetSelectorHandlerAttributeName()} attributes detected on member \"{fieldInfo.Name}\" of \"{fieldInfo.DeclaringType}\". Keeping only the first found.");
    }
    ...
    if (handlersWithLabelsAttribute.Count > 0) {
      ...
      return CreateSelectorHandlerWithLabelsFunction(
          handlerWithLabelsAttribute.labels,
          handlerWithLabelsAttribute.matchAll);
    }

    if (handlersWithTagsAttribute.Count > 0) {
      var handlerWithTagsAttribute = handlersWithTagsAttribute[0];
      return CreateSelectorHandlerWithTagsFunction(
          handlerWithTagsAttribute.tags);
    }
  ...
  static MethodInfo GetCustomSelectorHandlerForAttribute(Attribute attribute) {
    var methodsWithHandler = TypeCache.GetMethodsWithAttribute(
        typeof(ObjectSelectorHandlerAttribute));
    foreach (var methodWithHandler in methodsWithHandler) {
      var selectorHandlerAttributes =
          methodWithHandler
              .GetCustomAttributes(typeof(ObjectSelectorHandlerAttribute),
                                   false)
              .OfType<ObjectSelectorHandlerAttribute>();
  ...
  static bool ValidateSelectorHandlerSignature(MethodInfo mi) {
    if (mi.ReturnType != typeof(bool)) {
      Debug.LogWarning(
          $"{GetSelectorHandlerAttributeName()} \"{mi.Name}\" must return a boolean.");
      return false;
    }

    var paramTypes =
        new[]{typeof(ObjectSelectorTargetInfo), typeof(UnityObject[]),
              typeof(SearchService.ObjectSelectorSearchContext)};
    var parameters = mi.GetParameters();
    if (parameters.Length != paramTypes.Length) {
      Debug.LogWarning(
          $"{GetSelectorHandlerAttributeName()} \"{mi.Name}\" must have {paramTypes.Length} parameters.");
      return false;
    }
    ...
    for (var i = 0; i < paramTypes.Length; ++i) {
      ...
      if (currentParam.ParameterType != currentType) {
        Debug.LogWarning(
            $"Parameter \"{currentParam.Name}\" of {GetSelectorHandlerAttributeName()} \"{mi.Name}\" must be of type \"{currentType}\".");
        return false;
      }
    }
  ...
  static SelectorHandlerType
  CreateSelectorHandlerFunction(MethodInfo methodInfo) {
    return Delegate.CreateDelegate(typeof(SelectorHandlerType), methodInfo)
        as SelectorHandlerType;
  }

  static SelectorHandlerType
  CreateSelectorHandlerWithLabelsFunction(string[] requiredLabels,
                                          bool matchAll) {
    if (matchAll) {
  ...
  static SelectorHandlerType
  CreateSelectorHandlerWithTagsFunction(string[] requiredTags) {
    return (target, editedObjects, context) => {
      string tag = null;
