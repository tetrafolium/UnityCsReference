...
// Handles picking/selection in the scene view (both "click" type and
// "drag-rect" type)
internal class RectSelection {
  ...
  public RectSelection(EditorWindow window) { m_Window = window; }
  ...
  public void OnGUI() {
    ...
    case EventType.MouseDrag:
      if (GUIUtility.hotControl == id) {
        if (!m_RectSelecting &&
            (mousePos - m_SelectStartPoint).magnitude > 6f) {
        ...
        if (m_RectSelecting) {
          m_SelectMousePoint =
              new Vector2(Mathf.Max(mousePos.x, 0), Mathf.Max(mousePos.y, 0));
          GameObject[] rectObjs = HandleUtility.PickRectObjects(
          ...
          if (!setIt) {
            Dictionary<GameObject, bool> set =
                new Dictionary<GameObject, bool>(rectObjs.Length);
          ...
          if (setIt) {
            ...
            if (evt.shift)
              UpdateSelection(m_SelectionStart, rectObjs,
                              SelectionType.Additive, m_RectSelecting);
            else if (EditorGUI.actionKey)
              UpdateSelection(m_SelectionStart, rectObjs,
                              SelectionType.Subtractive, m_RectSelecting);
            else
              UpdateSelection(m_SelectionStart, rectObjs, SelectionType.Normal,
                              m_RectSelecting);
          }
        }
        evt.Use();
      ...
      if (GUIUtility.hotControl == id && evt.button == 0) {
        ...
        if (m_RectSelecting) {
          ...
          if (evt.shift || EditorGUI.actionKey) {
            // For shift, we check if EXACTLY the active GO is hovered by mouse
            // and then subtract. Otherwise additive. For control/cmd, we check
            // if ANY of the selected GO is hovered by mouse and then subtract.
            // Otherwise additive. Control/cmd takes priority over shift.
            GameObject hovered =
                HandleUtility.PickGameObject(evt.mousePosition, false);

            var handledIt = false;
            // shift-click deselects only if the active GO is exactly what we
            // clicked on
            if (!EditorGUI.actionKey && Selection.activeGameObject == hovered) {
              UpdateSelection(m_SelectionStart, hovered,
                              SelectionType.Subtractive, m_RectSelecting);
              handledIt = true;
            }

            // ctrl-click deselects everything up to prefab root, that is
            // already selected
            if (!handledIt && EditorGUI.actionKey) {
              var selectedGos = Selection.gameObjects;
              var hoveredRoot =
                  HandleUtility.FindSelectionBaseForPicking(hovered);
              ...
              if (deselectList.Any()) {
                UpdateSelection(m_SelectionStart, deselectList.ToArray(),
                                SelectionType.Subtractive, m_RectSelecting);
                handledIt = true;
              }
            }

            // we did not deselect anything, so add the new thing into selection
            // instead
            if (!handledIt) {
              UpdateSelection(
                  m_SelectionStart,
                  HandleUtility.PickGameObject(evt.mousePosition, true),
                  SelectionType.Additive, m_RectSelecting);
            }
          } else // With no modifier keys, we do the "cycle through overlapped"
                 // picking logic in SceneViewPicking.cs
          {
            GameObject picked =
                SceneViewPicking.PickGameObject(evt.mousePosition);
            UpdateSelection(m_SelectionStart, picked, SelectionType.Normal,
                            m_RectSelecting);
          }
    ...
    case EventType.ExecuteCommand:
      if (id == GUIUtility.hotControl &&
          evt.commandName == EventCommandNames.ModifierKeysChanged) {
        if (evt.shift)
  ...
  static void UpdateSelection(Object[] existingSelection, Object newObject,
                              SelectionType type, bool isRectSelection) {
    Object[] objs;
    if (newObject == null) {
      objs = new Object[0];
    } else {
  ...
  static void UpdateSelection(Object[] existingSelection, Object[] newObjects,
                              SelectionType type, bool isRectSelection) {
    Object[] newSelection;
    switch (type) {
    case SelectionType.Additive:
      if (newObjects.Length > 0) {
        ...
        System.Array.Copy(existingSelection, newSelection,
    ...
    case SelectionType.Subtractive:
      Dictionary<Object, bool> set =
          new Dictionary<Object, bool>(existingSelection.Length);
  ...
  // When rect selecting, we update the selected objects based on which modifier
  // keys are currently held down, so the window needs to repaint.
