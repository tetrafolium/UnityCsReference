...
public class SceneView : SearchableEditorWindow,
  ...
  public struct CameraMode {
    ...
    public static bool operator ==(CameraMode a, CameraMode z) {
      return a.drawMode == z.drawMode && a.name == z.name &&
             a.section == z.section;
    }
  ...
  public static SceneView currentDrawingSceneView {
    get { return s_CurrentDrawingSceneView; }
  }

  internal static readonly PrefColor kSceneViewBackground =
      new PrefColor("Scene/Background", 0.278431f, 0.278431f, 0.278431f, 0);
  internal static readonly PrefColor kSceneViewPrefabBackground =
      new PrefColor("Scene/Background for Prefabs", 0.132f, 0.231f, 0.330f, 0);
  static readonly PrefColor kSceneViewWire =
      new PrefColor("Scene/Wireframe", 0.0f, 0.0f, 0.0f, 0.5f);
  static readonly PrefColor kSceneViewWireOverlay =
      new PrefColor("Scene/Wireframe Overlay", 0.0f, 0.0f, 0.0f, 0.25f);
  static readonly PrefColor kSceneViewSelectedOutline =
      new PrefColor("Scene/Selected Outline", 255.0f / 255.0f, 102.0f / 255.0f,
                    0.0f / 255.0f, 0.0f / 255.0f);
  static readonly PrefColor kSceneViewSelectedChildrenOutline =
      new PrefColor("Scene/Selected Children Outline", 94.0f / 255.0f,
                    119.0f / 255.0f, 155.0f / 255.0f, 0.0f / 255.0f);
  static readonly PrefColor kSceneViewSelectedWire =
      new PrefColor("Scene/Wireframe Selected", 94.0f / 255.0f, 119.0f / 255.0f,
                    155.0f / 255.0f, 64.0f / 255.0f);

  static readonly PrefColor kSceneViewMaterialValidateLow =
      new PrefColor("Scene/Material Validator Value Too Low", 255.0f / 255.0f,
                    0.0f, 0.0f, 1.0f);
  static readonly PrefColor kSceneViewMaterialValidateHigh =
      new PrefColor("Scene/Material Validator Value Too High", 0.0f, 0.0f,
                    255.0f / 255.0f, 1.0f);
  static readonly PrefColor kSceneViewMaterialValidatePureMetal =
      new PrefColor("Scene/Material Validator Pure Metal", 255.0f / 255.0f,
                    255.0f / 255.0f, 0.0f, 1.0f);

  static readonly PrefColor kSceneViewMaterialNoContributeGI =
      new PrefColor("Scene/Contribute GI Off", 229.0f / 255.0f, 203.0f / 255.0f,
                    132.0f / 255.0f, 1.0f);
  static readonly PrefColor kSceneViewMaterialReceiveGILightmaps =
      new PrefColor("Scene/Contribute GI / Receive GI Lightmaps",
                    89.0f / 255.0f, 148.0f / 255.0f, 161.0f / 255.0f, 1.0f);
  static readonly PrefColor kSceneViewMaterialReceiveGILightProbes =
      new PrefColor("Scene/Contribute GI / Receive GI Light Probes",
                    221.0f / 255.0f, 115.0f / 255.0f, 91.0f / 255.0f, 1.0f);

  internal static Color kSceneViewFrontLight =
      new Color(0.769f, 0.769f, 0.769f, 1);
  internal static Color kSceneViewUpLight =
      new Color(0.212f, 0.227f, 0.259f, 1);
  internal static Color kSceneViewMidLight =
      new Color(0.114f, 0.125f, 0.133f, 1);
  internal static Color kSceneViewDownLight =
      new Color(0.047f, 0.043f, 0.035f, 1);

  public static Color selectedOutlineColor => kSceneViewSelectedOutline.Color;

  [SerializeField] bool m_ShowContextualTools;

  internal static SavedBool s_PreferenceEnableFilteringWhileSearching =
      new SavedBool("SceneView.enableFilteringWhileSearching", true);
  internal static SavedBool s_PreferenceEnableFilteringWhileLodGroupEditing =
      new SavedBool("SceneView.enableFilteringWhileLodGroupEditing", true);
  ...
  internal static Transform GetDefaultParentObjectIfSet() {
    ...
    string activeSceneGUID =
        prefabStage !=
        null ? prefabStage.scene.guid : EditorSceneManager.GetActiveScene().guid;
    int id = SceneHierarchy.GetDefaultParentForSession(activeSceneGUID);
    if (id != 0) {
      var objectFromInstanceID =
  ...
  static void OnEditorTrackerRebuilt() { s_ActiveEditorsDirty = true; }
  ...
  internal void SkipFading() { m_SkipFadingPending = true; }

  [SerializeField] string m_WindowGUID;
  ...
  float toolbarHeight {
    get {
      var _toolbarHeight =
          showToolbar ? EditorGUI.kWindowToolbarHeight.value : 0;
  ...
  // Returns the calculated rect where we render the camera in the SceneView (in
  ...
  [NonSerialized]
  static readonly Quaternion kDefaultRotation =
      Quaternion.LookRotation(new Vector3(-1, -.7f, -1));
  ...
  // Don't allow scene view zoom/size to go to crazy high values, or otherwise
  // various operations will start going to infinities etc.
  internal const float k_MaxSceneViewSize = 3.2e34f;
  // Limit the max draw distance to Sqrt(float.MaxValue) because transparent
  // sorting function uses dist^2, and Asserts that values are finite.
  ...
  [SerializeField] bool m_SceneIsLit = true;

  [Obsolete(
      "m_SceneLighting has been deprecated. Use sceneLighting instead (UnityUpgradable) -> UnityEditor.SceneView.sceneLighting",
      true)]
  ...
  public class SceneViewState {
    [SerializeField, FormerlySerializedAs("showMaterialUpdate")] bool
        m_AlwaysRefresh;
    ...
    public bool visualEffectGraphsEnabled =>
        fxEnabled && showVisualEffectGraphs;
    ...
    [Obsolete(
        "IsAllOn() has been deprecated. Use allEnabled instead (UnityUpgradable) -> allEnabled")]
    public bool IsAllOn() {
      return allEnabled;
    }
    ...
    public bool allEnabled {
      get {
        bool all = showFog && alwaysRefresh && showSkybox && showFlares &&
                   showImageEffects && showParticleSystems;
    ...
    [Obsolete(
        "Toggle() has been deprecated. Use SetAllEnabled() instead (UnityUpgradable) -> SetAllEnabled(*)")]
  ...
  [SerializeField] bool m_isRotationLocked = false;
  public bool isRotationLocked {
    get { return m_isRotationLocked; }
    set { m_isRotationLocked = value; }
  }

  internal static List<CameraMode> userDefinedModes { get; }
  = new List<CameraMode>();

  [SerializeField] bool m_PlayAudio = false;
  ...
  [SerializeField] AnimVector3 m_Position = new AnimVector3(kDefaultPivot);
...
#pragma warning disable 618
  ...
  [Obsolete(
      "onSceneGUIDelegate has been deprecated. Use duringSceneGui instead.")]
  [System.ComponentModel.EditorBrowsable(
      System.ComponentModel.EditorBrowsableState.Never)]
...
#pragma warning restore 618
  ...
  internal static event Func<SceneView, VisualElement>
      addCustomVisualElementToSceneView;
  ...
  public CameraMode cameraMode {
    get {
      // fix for case 969889 where the toolbar is empty when we haven't fully
      // initialized the value
      if (string.IsNullOrEmpty(m_CameraMode.name)) {
        m_CameraMode =
            SceneRenderModeWindow.GetBuiltinCameraMode(m_CameraMode.drawMode);
      }
      return m_CameraMode;
    }
    set {
      if (!IsValidCameraMode(value)) {
        throw new ArgumentException(string.Format(
            "The provided camera mode {0} is not registered!", value));
      }
      m_CameraMode = value;
  ...
  [Obsolete(
      "m_ValidateTrueMetals has been deprecated. Use validateTrueMetals instead (UnityUpgradable) -> validateTrueMetals",
      true)]
  public bool m_ValidateTrueMetals = false;

  [SerializeField] bool m_DoValidateTrueMetals = false;
  ...
  [SerializeField] float m_ExposureSliderValue = 0.0f;
  ...
  internal bool showExposureSettings {
    get {
      return (this.cameraMode.drawMode == DrawCameraMode.BakedEmissive ||
              this.cameraMode.drawMode == DrawCameraMode.BakedLightmap ||
              this.cameraMode.drawMode == DrawCameraMode.RealtimeEmissive ||
              this.cameraMode.drawMode == DrawCameraMode.RealtimeIndirect);
    }
  }

  [SerializeField] SceneViewState m_SceneViewState;
  ...
  [SerializeField] SceneViewGrid m_Grid;
  ...
  [SerializeField] AnimFloat m_Size = new AnimFloat(kDefaultViewSize);
  ...
  static readonly string s_CameraRectVisualElementName =
      "unity-scene-view-camera-rect";
  ...
  public class CameraSettings {
    ...
    [SerializeField] float m_SpeedNormalized;
    [SerializeField] float m_SpeedMin;
    [SerializeField] float m_SpeedMax;
    [SerializeField] bool m_EasingEnabled;
    [SerializeField] float m_EasingDuration;
    [SerializeField] bool m_AccelerationEnabled;

    [SerializeField] float
        m_FieldOfViewHorizontalOrVertical; // either horizontal or vertical
                                           // depending on aspect ratio
    [SerializeField] float m_NearClip;
    [SerializeField] float m_FarClip;
    [SerializeField] bool m_DynamicClip;
    [SerializeField] bool m_OcclusionCulling;
    ...
    // Easing is applied when starting and stopping movement. When enabled, the
    // camera will lerp from it's current speed to the target speed over the
    // course of `CameraSettings.easingDuration` seconds.
    public bool easingEnabled {
      get { return m_EasingEnabled; }
      set { m_EasingEnabled = value; }
    }

    // How many seconds should the camera take to go from stand-still to initial
    // full speed. When setting an animated value speed, use `1 / duration`.
    public float easingDuration {
      get { return m_EasingDuration; }
      set {
        // Clamp and round to 1 decimal point
        m_EasingDuration =
            (float) Math.Round(Mathf.Clamp(value, kAbsoluteEasingDurationMin,
                                           kAbsoluteEasingDurationMax),
                               1);
      }
    }

    // When acceleration is enabled, camera speed is continuously increased
    // while in motion. When acceleration is disabled, speed is a constant value
    // defined by `CameraSettings.speed`
    public bool accelerationEnabled {
      get { return m_AccelerationEnabled; }
      set { m_AccelerationEnabled = value; }
    }

    // this ensures that the resolution the slider snaps to is sufficient given
    // the minimum speed, and the range of appropriate values
    internal float RoundSpeedToNearestSignificantDecimal(float value) {
      ...
      int min_rnd =
          speedMin < .01f ? 3 : speedMin < .1f ? 2 : speedMin < 1f ? 1 : 0;
    ...
    internal void SetSpeedMinMax(float[] floatValues) {
      ...
      float min = Mathf.Clamp(floatValues[0], kAbsoluteSpeedMin,
                              kAbsoluteSpeedMax - kMinSpeedMinMaxRange);
      float minRange = min < .1f ? .01f : min < 1f ? .1f : 1f;
      float max =
          Mathf.Clamp(floatValues[1], min + minRange, kAbsoluteSpeedMax);
  ...
  public void ResetCameraSettings() { m_CameraSettings = new CameraSettings(); }
  ...
  private static readonly List<CursorRect> s_MouseRects =
      new List<CursorRect>();
  ...
  [System.NonSerialized] Scene m_CustomLightsScene;
  [System.NonSerialized] Light[] m_Light = new Light[3];
  ...
  internal static class Styles {
    public static GUIContent toolsContent = EditorGUIUtility.TrIconContent(
        "SceneViewTools",
        "Hide or show the Component Editor Tools panel in the Scene view.");
    public static GUIContent lighting = EditorGUIUtility.TrIconContent(
        "SceneviewLighting",
        "When toggled on, the Scene lighting is used. When toggled off, a light attached to the Scene view camera is used.");
    public static GUIContent fx = EditorGUIUtility.TrIconContent(
        "SceneviewFx", "Toggle skybox, fog, and various other effects.");
    public static GUIContent audioPlayContent = EditorGUIUtility.TrIconContent(
        "SceneviewAudio", "Toggle audio on or off.");
    public static GUIContent gizmosContent = EditorGUIUtility.TrTextContent(
        "Gizmos", "Toggle visibility of all Gizmos in the Scene view");
    public static GUIContent gizmosDropDownContent =
        EditorGUIUtility.TrTextContent(
            "", "Toggle the visibility of different Gizmos in the Scene view.");
    public static GUIContent mode2DContent = EditorGUIUtility.TrIconContent(
        "SceneView2D",
        "When toggled on, the Scene is in 2D view. When toggled off, the Scene is in 3D view.");
    public static GUIContent gridXToolbarContent =
        EditorGUIUtility.TrIconContent("GridAxisX",
                                       "Toggle the visibility of the grid");
    public static GUIContent gridYToolbarContent =
        EditorGUIUtility.TrIconContent("GridAxisY",
                                       "Toggle the visibility of the grid");
    public static GUIContent gridZToolbarContent =
        EditorGUIUtility.TrIconContent("GridAxisZ",
                                       "Toggle the visibility of the grid");
    public static GUIContent isolationModeExitButton =
        EditorGUIUtility.TrTextContent("Exit", "Exit isolation mode");
    public static GUIContent renderDocContent;
    public static GUIContent metalFrameCaptureContent =
        EditorGUIUtility.TrIconContent(
            "FrameCapture",
            "Capture the current view and open in Xcode frame debugger");
    public static GUIContent sceneVisToolbarButtonContent =
        EditorGUIUtility.TrIconContent(
            "SceneViewVisibility",
            "Number of hidden objects, click to toggle scene visibility");
    public static GUIStyle gizmoButtonStyle;
    public static GUIContent sceneViewCameraContent =
        EditorGUIUtility.TrIconContent("SceneViewCamera",
                                       "Settings for the Scene view camera.");
    public static GUIContent contributeGIOff =
        EditorGUIUtility.TrTextContent("Contribute GI Off");
    public static GUIContent receiveGILightmaps =
        EditorGUIUtility.TrTextContent("Contribute GI / Receive GI Lightmaps");
    public static GUIContent receiveGILightProbes =
        EditorGUIUtility.TrTextContent(
            "Contribute GI / Receive GI Light Probes");

    static Styles() {
      gizmoButtonStyle = "GV Gizmo DropDown";
      renderDocContent = EditorGUIUtility.TrIconContent(
          "FrameCapture", UnityEditor.RenderDocUtil.openInRenderDocLabel);
    }
  }
  ...
  // While Locking the view to object, we have different behaviour for different
  // scenarios: Smooth camera behaviour: User dragging the handles Instant
  // camera behaviour: Position changed externally (via inspector, physics or
  // scripts etc.)
  internal enum DraggingLockedState {
    NotDragging, // Default state. Scene view camera is snapped to selected
                 // object instantly
    Dragging, // User is dragging from handles. Scene view camera holds still.
    LookAt    // Temporary state after dragging or selection change, where we
              // return scene view camera smoothly to selected object
  }
  DraggingLockedState m_DraggingLockedState;
  internal DraggingLockedState draggingLocked {
    set { m_DraggingLockedState = value; }
    get { return m_DraggingLockedState; }
  }

  [SerializeField]
  private Object m_LastLockedObject;

  [SerializeField] bool m_ViewIsLockedToObject;
  ...
  public static bool FrameLastActiveSceneView() {
    if (lastActiveSceneView == null)
      return false;
    return lastActiveSceneView.SendEvent(
        EditorGUIUtility.CommandEvent(EventCommandNames.FrameSelected));
  }
  ...
  public static bool FrameLastActiveSceneViewWithLock() {
    if (lastActiveSceneView == null)
      return false;
    return lastActiveSceneView.SendEvent(
        EditorGUIUtility.CommandEvent(EventCommandNames.FrameSelectedWithLock));
  }
  ...
  internal override void SetSearchFilter(string searchFilter, SearchMode mode,
                                         bool setAll, bool delayed) {
  ...
  public override void OnEnable() {
    ...
    m_SceneVisOverlayWindow = new OverlayWindow(
        EditorGUIUtility.TrTextContent("Isolation View", ""), OverlayWindowGUI,
        (int) SceneViewOverlay.Ordering.ParticleEffect + 100, null,
        SceneViewOverlay.WindowDisplayOption.OneWindowPerTarget);
    m_PBRSettingsOverlayWindow = new OverlayWindow(
        EditorGUIUtility.TrTextContent("PBR Validation Settings"),
        DrawPBRSettings, (int) SceneViewOverlay.Ordering.PhysicsDebug, this,
        SceneViewOverlay.WindowDisplayOption.OneWindowPerTarget);
    m_LightingExposureSettingsOverlayWindow = new OverlayWindow(
        EditorGUIUtility.TrTextContent("Lighting Exposure"),
        DrawLightingExposureSettings,
        (int) SceneViewOverlay.Ordering.PhysicsDebug, this,
        SceneViewOverlay.WindowDisplayOption.OneWindowPerTarget);
    m_GIContributorsReceiversOverlayWindow = new OverlayWindow(
        EditorGUIUtility.TrTextContent("GI Contributors / Receivers"),
        DrawGIContributorsReceiversSettings,
        (int) SceneViewOverlay.Ordering.PhysicsDebug, this,
        SceneViewOverlay.WindowDisplayOption.OneWindowPerTarget);
    m_EditorToolsOverlayWindow = new OverlayWindow(
        EditorGUIUtility.TrTextContent("Tools"),
        EditorToolGUI.DoContextualToolbarOverlay, int.MaxValue, null,
    ...
    EditorApplication.modifierKeysChanged +=
        RepaintAll; // Because we show handles on shift
    ...
    if (m_CameraMode.drawMode == DrawCameraMode.UserDefined &&
        !userDefinedModes.Contains(m_CameraMode))
  ...
  VisualElement CreateCameraRectVisualElement() {
    var root = new VisualElement(){
        ...
        pickingMode =
            PickingMode.Ignore, // do not eat events so IMGUI gets them
        viewDataKey = name, renderHints = RenderHints.ClipWithScissors};
    ...
    if (addCustomVisualElementToSceneView != null) {
      foreach (var del in addCustomVisualElementToSceneView
                   .GetInvocationList()) {
  ...
  void OverlayWindowGUI(Object target, SceneView view) {
    if (GUILayout.Button(Styles.isolationModeExitButton,
  ...
  protected virtual bool SupportsStageHandling() { return true; }
  ...
  public SceneView() {
    ...
    // Note: Rendering for Scene view picking depends on the depth buffer of the
    // window
    depthBufferBits = 32;
  }
  ...
  // Thus, we can't just register to the
  // StageNavigationManager.instance.stageChanged event since that would not
  // guarantee the order dependency.
  ...
  void ToolbarDisplayStateGUI() {
    ...
    Rect modeRect = GUILayoutUtility.GetRect(
        modeContent, EditorStyles.toolbarDropDownLeft, GUILayout.Width(120));
    if (EditorGUI.DropdownButton(modeRect, modeContent, FocusType.Passive,
                                 EditorStyles.toolbarDropDownLeft)) {
    ...
    in2DMode = GUILayout.Toggle(in2DMode, Styles.mode2DContent,
                                EditorStyles.toolbarButton);

    EditorGUILayout.Space();

    m_SceneIsLit = GUILayout.Toggle(m_SceneIsLit, Styles.lighting,
                                    EditorStyles.toolbarButton) ||
                   cameraMode.drawMode == DrawCameraMode.ShadowCascades;

    using(new EditorGUI.DisabledScope(Application.isPlaying)) {
      audioPlay = GUILayout.Toggle(audioPlay, Styles.audioPlayContent,
                                   EditorStyles.toolbarButton);
    }

    var enabled = sceneViewState.fxEnabled;
    if (EditorGUILayout.DropDownToggle(ref enabled, Styles.fx,
                                       EditorStyles.toolbarDropDownToggle)) {
      Rect rect = GUILayoutUtility.topLevel.GetLast();
  ...
  void ToolbarGridDropdownGUI() {
    ...
    if (EditorGUILayout.DropDownToggle(ref toggled, gridIcon,
                                       EditorStyles.toolbarDropDownToggle)) {
      Rect rect = GUILayoutUtility.topLevel.GetLast();
  ...
  void ToolbarGizmosDropdownGUI() {
    ...
    if (EditorGUILayout.DropDownToggle(ref toggled, Styles.gizmosContent,
                                       EditorStyles.toolbarDropDownToggle)) {
      Rect rect = GUILayoutUtility.topLevel.GetLast();
  ...
  void ToolbarRenderDocGUI() {
    if (RenderDoc.IsLoaded()) {
      using(new EditorGUI.DisabledScope(!RenderDoc.IsSupported())) {
        if (GUILayout.Button(Styles.renderDocContent,
                             EditorStyles.toolbarButton)) {
  ...
  void ToolbarMetalCaptureGUI() {
    bool canCapture =
        FrameCapture.IsDestinationSupported(FrameCaptureDestination.DevTools) ||
        FrameCapture.IsDestinationSupported(
  ...
  private void ToolbarSceneVisibilityGUI() {
    ...
    m_SceneVisActive =
        GUILayout.Toggle(m_SceneVisActive, Styles.sceneVisToolbarButtonContent,
                         EditorStyles.toolbarButton);
  }
  ...
  void ToolbarSceneToolsGUI() {
    displayToolModes = GUILayout.Toggle(displayToolModes, Styles.toolsContent,
                                        EditorStyles.toolbarButton);
  }

  void ToolbarSceneCameraGUI() {
    Rect sceneCameraRect = GUILayoutUtility.GetRect(
        Styles.sceneViewCameraContent, EditorStyles.toolbarDropDown);

    var evt = Event.current;
    if (evt.type == EventType.ContextClick &&
        sceneCameraRect.Contains(evt.mousePosition))
      SceneViewCameraWindow.ShowContextMenu(this);

    if (EditorGUI.DropdownButton(sceneCameraRect, Styles.sceneViewCameraContent,
                                 FocusType.Passive,
  ...
  void RefreshAudioPlay() {
    ...
    // We have to find all loaded AudioSources, not just the ones in main
    // scenes.
    var sources =
        (AudioSource[]) Resources.FindObjectsOfTypeAll(typeof(AudioSource));
    foreach (AudioSource source in sources) {
      if (EditorUtility.IsPersistent(source))
        continue;

      if (source.playOnAwake) {
        if (!m_PlayAudio || !StageUtility.IsGameObjectRenderedByCamera(
                                source.gameObject, m_Camera)) {
          source.Stop();
        } else {
          if (!source.isPlaying)
            source.Play();
    ...
    var zones = (AudioReverbZone[])
                    Resources.FindObjectsOfTypeAll(typeof(AudioReverbZone));
    foreach (AudioReverbZone zone in zones) {
      if (EditorUtility.IsPersistent(zone))
        continue;

      zone.active = m_PlayAudio && StageUtility.IsGameObjectRenderedByCamera(
                                       zone.gameObject, m_Camera);
    }
  ...
  public virtual void AddItemsToMenu(GenericMenu menu) {
    if (RenderDoc.IsInstalled() && !RenderDoc.IsLoaded()) {
      menu.AddItem(EditorGUIUtility.TrTextContent(
                       UnityEditor.RenderDocUtil.loadRenderDocLabel),
                   false, LoadRenderDoc);
    }
  }
  ...
  internal static bool ValidateMenuMoveToFront() {
    if (Selection.activeTransform != null) {
      ...
      return (parent != null &&
              parent.GetChild(0) != Selection.activeTransform);
    }
    return false;
  }
  ...
  internal static bool ValidateMenuMoveToBack() {
    if (Selection.activeTransform != null) {
      Transform parent = Selection.activeTransform.parent;
      return (parent != null && parent.GetChild(parent.childCount - 1) !=
  ...
  bool forceSceneViewFilteringForLodGroupEditing =>
      m_ForceSceneViewFilteringForLodGroupEditing &&
      s_PreferenceEnableFilteringWhileLodGroupEditing;

  bool UseSceneFiltering() {
    return (!string.IsNullOrEmpty(m_SearchFilter) &&
            s_PreferenceEnableFilteringWhileSearching) ||
           forceSceneViewFilteringForLodGroupEditing ||
           m_ForceSceneViewFilteringForStageHandling ||
           m_ForceSceneViewFiltering;
  }
  ...
  private void SetupFogAndShadowDistance(out bool oldFog,
                                         out float oldShadowDistance) {
    ...
    if (Event.current.type == EventType.Repaint) {
      ...
      if (m_Camera.orthographic)
        Unsupported.SetQualitySettingsShadowDistanceTemporarily(
            QualitySettings.shadowDistance + 0.5f * cameraDistance);
    }
  }

  private void RestoreFogAndShadowDistance(bool oldFog,
  ...
  private void CreateCameraTargetTexture(Rect cameraRect, bool hdr) {
    ...
    GraphicsFormat format =
        (hdr && SystemInfo.IsFormatSupported(GraphicsFormat.R16G16B16A16_SFloat,
                                             FormatUsage.Render))
    ...
    if (m_SceneTargetTexture == null) {
      m_SceneTargetTexture = new RenderTexture(0, 0, 24, format){
          name = "SceneView RT", antiAliasing = 1,
          hideFlags = HideFlags.HideAndDontSave};
    }
    if (m_SceneTargetTexture.width != width ||
  ...
  public bool IsCameraDrawModeEnabled(CameraMode mode) {
    ...
    return (onValidateCameraMode == null ||
            onValidateCameraMode.GetInvocationList().All(
                validate =>((Func<CameraMode, bool>) validate)(mode)));
  }

  internal bool IsSceneCameraDeferred() {
    bool usingScriptableRenderPipeline =
        (GraphicsSettings.currentRenderPipeline != null);
    if (m_Camera == null || usingScriptableRenderPipeline)
      return false;
    if (m_Camera.actualRenderingPath == RenderingPath.DeferredLighting ||
        m_Camera.actualRenderingPath == RenderingPath.DeferredShading)
      return true;
    return false;
  }

  internal static bool DoesCameraDrawModeSupportDeferred(DrawCameraMode mode) {
    // many of special visualization modes don't support deferred
    // shading/lighting overdraw/mipmaps visualizations need special forward
    // shader various lightmaps/visualization modes, don't use deferred for
    // safety (previous code also did not use deferred)
    return mode == DrawCameraMode.Normal || mode == DrawCameraMode.Textured ||
  ...
  internal static bool DoesCameraDrawModeSupportHDR(DrawCameraMode mode) {
    // HDR/Tonemap only supported on regular views, and not on any special
    // visualizations
    return mode == DrawCameraMode.Textured ||
           mode == DrawCameraMode.TexturedWire;
  }
  ...
  private void PrepareCameraTargetTexture(Rect cameraRect) {
    ...
    // Do not use deferred rendering when using search filtering or
    // wireframe/overdraw/mipmaps rendering modes.
    if (UseSceneFiltering() ||
        !DoesCameraDrawModeSupportDeferred(m_CameraMode.drawMode)) {
  ...
  private void PrepareCameraReplacementShader() {
    ...
    Handles.SetSceneViewColors(
        kSceneViewWire, kSceneViewWireOverlay, kSceneViewSelectedOutline,
        kSceneViewSelectedChildrenOutline, kSceneViewSelectedWire);
    ...
    if (m_CameraMode.drawMode == DrawCameraMode.Overdraw) {
      ...
      if (!s_ShowOverdrawShader)
        s_ShowOverdrawShader = EditorGUIUtility.LoadRequired(
            "SceneView/SceneViewShowOverdraw.shader") as Shader;
      ...
      // show mip levels
      if (!s_ShowMipsShader)
        s_ShowMipsShader = EditorGUIUtility.LoadRequired(
            "SceneView/SceneViewShowMips.shader") as Shader;
      ...
      if (!s_ShowTextureStreamingShader)
        s_ShowTextureStreamingShader = EditorGUIUtility.LoadRequired(
            "SceneView/SceneViewShowTextureStreaming.shader") as Shader;
      if (s_ShowTextureStreamingShader != null &&
          s_ShowTextureStreamingShader.isSupported) {
        m_Camera.SetReplacementShader(s_ShowTextureStreamingShader,
                                      "RenderType");
      } else {
        m_Camera.SetReplacementShader(m_ReplacementShader, m_ReplacementString);
      }
  ...
  bool SceneCameraRendersIntoRT() { return m_Camera.targetTexture != null; }

  private void DoDrawCamera(Rect windowSpaceCameraRect,
                            Rect groupSpaceCameraRect, out bool pushedGUIClip) {
    pushedGUIClip = false;
    ...
    DrawGridParameters gridParam = sceneViewGrids.PrepareGridRender(
        camera, pivot, m_Rotation.target, size, m_Ortho.target);
    if (m_SkipFadingPending)
      sceneViewGrids
          .SkipFading(); // Called AFTER fade target values have been updated.
    ...
    if (UseSceneFiltering()) {
      ...
      if (evt.type == EventType.Repaint)
        Graphics.DrawTexture(groupSpaceCameraRect, m_SceneTargetTexture,
                             new Rect(0, 0, 1, 1), 0, 0, 0, 0, GUI.color,
                             GUI.blitMaterial);
      Handles.SetCamera(groupSpaceCameraRect, m_Camera);
    } else {
      // If the camera is rendering into a Render Texture we need to reset the
      // offsets of the GUIClip stack otherwise all GUI drawing after here will
      // get offset incorrectly.
      if (SceneCameraRendersIntoRT()) {
        GUIClip.Push(new Rect(0f, 0f, position.width, position.height),
                     Vector2.zero, Vector2.zero, true);
        pushedGUIClip = true;
      }
      Handles.DrawCameraStep1(groupSpaceCameraRect, m_Camera,
                              m_CameraMode.drawMode, gridParam, drawGizmos);
  ...
  void RenderFilteredScene(Rect groupSpaceCameraRect) {
    ...
    // First pass: Draw the scene normally in destination render texture, save
    // color buffer for later
    ...
    float fade =
        UseSceneFiltering()
            ? 1f
            : Mathf.Clamp01((float)(EditorApplication.timeSinceStartup -
                                    m_StartSearchFilterTime));
    if (!s_FadeMaterial)
      s_FadeMaterial = EditorGUIUtility.LoadRequired(
          "SceneView/SceneViewGrayscaleEffectFade.mat") as Material;
    s_FadeMaterial.SetFloat("_Fade", fade);
    Graphics.Blit(colorRT, m_SceneTargetTexture, s_FadeMaterial);

    // Third pass: Draw aura for objects which meet the search filter, but are
    // occluded. Save color buffer for later.
    m_Camera.renderingPath = RenderingPath.Forward;
    if (!s_AuraShader)
      s_AuraShader = EditorGUIUtility.LoadRequired(
          "SceneView/SceneViewAura.shader") as Shader;
    m_Camera.SetReplacementShader(s_AuraShader, "");
    Handles.SetCameraFilterMode(m_Camera,
                                Handles.CameraFilterMode.ShowFiltered);
    Handles.DrawCamera(groupSpaceCameraRect, m_Camera, m_CameraMode.drawMode,
                       drawGizmos);
    ...
    if (!s_ApplyFilterMaterial)
      s_ApplyFilterMaterial = EditorGUIUtility.LoadRequired(
          "SceneView/SceneViewApplyFilter.mat") as Material;
  ...
  void DoClearCamera(Rect cameraRect) {
    // Clear (color/skybox)
    // We do funky FOV interpolation when switching between ortho and
    // perspective. However, for the skybox we always want to use the same FOV.
  ...
  // Give editors a chance to kick in. Disable in search mode, editors rendering
  // to the scene
  ...
  private static bool ComponentHasImageEffectAttribute(Component c) {
    if (c == null)
      return false;
    return Attribute.IsDefined(c.GetType(),
  ...
  void UpdateImageEffects(bool enable) {
    ...
    if (!enable || mainCam == null) {
      ComponentUtility.DestroyComponentsMatching(
          m_Camera.gameObject, ComponentHasImageEffectAttribute);
      return;
    }

    ComponentUtility.ReplaceComponentsIfDifferent(
        mainCam.gameObject, m_Camera.gameObject,
        ComponentHasImageEffectAttribute);
  }
  ...
  void UpdateSwatchTexture(Texture2D t, Color c, bool colorCorrect = false) {
    // Some colors were created in gamma space and should be viewed as such when
    // in linear
    if (PlayerSettings.colorSpace == ColorSpace.Linear && colorCorrect) {
      c = c.gamma; // offset linear to gamma correction that happens in IMGUI,
                   // by doing the inverse beforehand
    }
    c.a = 1.0f;
    t.SetPixel(0, 0, c);
    t.Apply();
  ...
  String CreateSwatchDescriptionForName(float minLum, float maxLum) {
    return "Luminance (" +
           minLum.ToString("F2", CultureInfo.InvariantCulture.NumberFormat) +
           " - " +
           maxLum.ToString("F2", CultureInfo.InvariantCulture.NumberFormat) +
           ")";
  }

  void CreateAlbedoSwatchData() {
    AlbedoSwatchInfo[] graphicsSettingsSwatches =
    ...
    if (graphicsSettingsSwatches.Length != 0) {
      ...
      m_AlbedoSwatchInfos = new AlbedoSwatchInfo[]{
          ...
          new AlbedoSwatchInfo(){
              ...
              minLuminance = 0.03f, maxLuminance = 0.07f},
          new AlbedoSwatchInfo(){
              name = "Dark Soil",
              color = new Color(85f / 255f, 61f / 255f, 49f / 255f),
              minLuminance = 0.05f, maxLuminance = 0.14f},

          new AlbedoSwatchInfo(){
              name = "Worn Asphalt",
              color = new Color(91f / 255f, 91f / 255f, 91f / 255f),
              minLuminance = 0.10f, maxLuminance = 0.15f},
          new AlbedoSwatchInfo(){
              name = "Dry Clay Soil",
              color = new Color(137f / 255f, 120f / 255f, 102f / 255f),
              minLuminance = 0.15f, maxLuminance = 0.35f},
          new AlbedoSwatchInfo(){
              name = "Green Grass",
              color = new Color(123f / 255f, 131f / 255f, 74f / 255f),
              minLuminance = 0.16f, maxLuminance = 0.26f},
          new AlbedoSwatchInfo(){
              name = "Old Concrete",
              color = new Color(135f / 255f, 136f / 255f, 131f / 255f),
              minLuminance = 0.17f, maxLuminance = 0.30f},
          new AlbedoSwatchInfo(){
              name = "Red Clay Tile",
              color = new Color(197f / 255f, 125f / 255f, 100f / 255f),
              minLuminance = 0.23f, maxLuminance = 0.33f},
          new AlbedoSwatchInfo(){
              name = "Dry Sand",
              color = new Color(177f / 255f, 167f / 255f, 132f / 255f),
              minLuminance = 0.20f, maxLuminance = 0.45f},
          new AlbedoSwatchInfo(){
              name = "New Concrete",
              color = new Color(185f / 255f, 182f / 255f, 175f / 255f),
              minLuminance = 0.32f, maxLuminance = 0.55f},
          new AlbedoSwatchInfo(){
              name = "White Acrylic Paint",
              color = new Color(227f / 255f, 227f / 255f, 227f / 255f),
              minLuminance = 0.75f, maxLuminance = 0.85f},
          new AlbedoSwatchInfo(){
              name = "Fresh Snow",
              color = new Color(243f / 255f, 243f / 255f, 243f / 255f),
              minLuminance = 0.85f, maxLuminance = 0.95f},
          new AlbedoSwatchInfo(){
              name = "Blue Sky",
              color = new Color(93f / 255f, 123f / 255f, 157f / 255f),
              minLuminance = new Color(93f / 255f, 123f / 255f, 157f / 255f)
                                 .linear.maxColorComponent -
                             0.05f,
              maxLuminance = new Color(93f / 255f, 123f / 255f, 157f / 255f)
                                 .linear.maxColorComponent +
                             0.05f},
          new AlbedoSwatchInfo(){
              name = "Foliage",
              color = new Color(91f / 255f, 108f / 255f, 65f / 255f),
              minLuminance = new Color(91f / 255f, 108f / 255f, 65f / 255f)
                                 .linear.maxColorComponent -
                             0.05f,
              maxLuminance = new Color(91f / 255f, 108f / 255f, 65f / 255f)
                                 .linear.maxColorComponent +
                             0.05f},
      };
    }
  ...
  void UpdateAlbedoSwatchGUI() {
    ...
    m_AlbedoSwatchColorStyles [0]
    = CreateSwatchStyleForColor(Color.gray, true);
    m_AlbedoSwatchDescriptions[0] = "Default Luminance";
    m_AlbedoSwatchGUIContent[0] = new GUIContent(m_AlbedoSwatchDescriptions[0]);
    m_AlbedoSwatchLuminanceStrings [0]
    = CreateSwatchDescriptionForName(0.012f, 0.9f);
    for (int i = 1; i < m_AlbedoSwatchInfos.Length + 1; i++) {
      m_AlbedoSwatchColorStyles [i]
      = CreateSwatchStyleForColor(m_AlbedoSwatchInfos[i - 1].color, true);
      m_AlbedoSwatchDescriptions[i] = m_AlbedoSwatchInfos[i - 1].name;
      m_AlbedoSwatchGUIContent[i] =
          new GUIContent(m_AlbedoSwatchDescriptions[i]);
      m_AlbedoSwatchLuminanceStrings [i]
      = CreateSwatchDescriptionForName(m_AlbedoSwatchInfos[i - 1].minLuminance,
                                       m_AlbedoSwatchInfos[i - 1].maxLuminance);
    }
  }

  void UpdatePBRColorLegend() {
    if (m_TooLowColorStyle == null ||
        m_TooLowColorStyle.normal.background == null) {
      m_TooLowColorStyle =
          CreateSwatchStyleForColor(kSceneViewMaterialValidateLow.Color);
      m_TooHighColorStyle =
          CreateSwatchStyleForColor(kSceneViewMaterialValidateHigh.Color);
      m_PureMetalColorStyle =
          CreateSwatchStyleForColor(kSceneViewMaterialValidatePureMetal.Color);
    } else {
      UpdateSwatchTexture(m_TooLowColorStyle.normal.background,
                          kSceneViewMaterialValidateLow.Color);
      UpdateSwatchTexture(m_TooHighColorStyle.normal.background,
                          kSceneViewMaterialValidateHigh.Color);
      UpdateSwatchTexture(m_PureMetalColorStyle.normal.background,
                          kSceneViewMaterialValidatePureMetal.Color);
    }

    if (PlayerSettings.colorSpace == ColorSpace.Linear) {
      Shader.SetGlobalColor("unity_MaterialValidateLowColor",
                            kSceneViewMaterialValidateLow.Color.linear);
      Shader.SetGlobalColor("unity_MaterialValidateHighColor",
                            kSceneViewMaterialValidateHigh.Color.linear);
      Shader.SetGlobalColor("unity_MaterialValidatePureMetalColor",
                            kSceneViewMaterialValidatePureMetal.Color.linear);
    } else {
      Shader.SetGlobalColor("unity_MaterialValidateLowColor",
                            kSceneViewMaterialValidateLow.Color);
      Shader.SetGlobalColor("unity_MaterialValidateHighColor",
  ...
  void UpdateAlbedoSwatch() {
    ...
    if (m_SelectedAlbedoSwatchIndex != 0) {
      ...
      Shader.SetGlobalFloat(
          "_AlbedoMinLuminance",
          m_AlbedoSwatchInfos[m_SelectedAlbedoSwatchIndex - 1].minLuminance);
      Shader.SetGlobalFloat(
          "_AlbedoMaxLuminance",
          m_AlbedoSwatchInfos[m_SelectedAlbedoSwatchIndex - 1].maxLuminance);
      Shader.SetGlobalFloat("_AlbedoHueTolerance", m_AlbedoSwatchHueTolerance);
      Shader.SetGlobalFloat("_AlbedoSaturationTolerance",
                            m_AlbedoSwatchSaturationTolerance);
    }
    Shader.SetGlobalColor("_AlbedoCompareColor", color.linear);
    Shader.SetGlobalInt("_CheckAlbedo",
                        (m_SelectedAlbedoSwatchIndex != 0) ? 1 : 0);
    Shader.SetGlobalInt("_CheckPureMetal", m_DoValidateTrueMetals ? 1 : 0);
  }

  internal void DrawTrueMetalCheckbox() {
    EditorGUI.BeginChangeCheck();
    m_DoValidateTrueMetals = EditorGUILayout.ToggleLeft(
        EditorGUIUtility.TrTextContent(
            "Check Pure Metals",
            "Check if albedo is black for materials with an average specular color above 0.45"),
    ...
    if (m_CameraMode.drawMode == DrawCameraMode.ValidateAlbedo) {
      if (PlayerSettings.colorSpace == ColorSpace.Gamma) {
        EditorGUILayout.HelpBox(
            "Albedo Validation doesn't work when Color Space is set to gamma space",
            MessageType.Warning);
      }

      EditorGUIUtility.labelWidth = 140;

      m_SelectedAlbedoSwatchIndex = EditorGUILayout.Popup(
          EditorGUIUtility.TrTextContent(
              "Luminance Validation:",
              "Select default luminance validation or validate against a configured albedo swatch"),
          m_SelectedAlbedoSwatchIndex, m_AlbedoSwatchGUIContent);
      EditorGUI.indentLevel++;
      using(new EditorGUILayout.HorizontalScope()) {
        EditorGUIUtility.labelWidth = 5;
        EditorGUILayout.LabelField(
            " ", m_AlbedoSwatchColorStyles[m_SelectedAlbedoSwatchIndex]);
        EditorGUIUtility.labelWidth = 140;
        EditorGUILayout.LabelField(
            m_AlbedoSwatchLuminanceStrings[m_SelectedAlbedoSwatchIndex]);
      }
      ...
      using(new EditorGUI.DisabledScope(m_SelectedAlbedoSwatchIndex == 0)) {
        EditorGUI.BeginChangeCheck();
        using(new EditorGUI.DisabledScope(m_SelectedAlbedoSwatchIndex == 0)) {
          m_AlbedoSwatchHueTolerance = EditorGUILayout.Slider(
              EditorGUIUtility.TrTextContent(
                  "Hue Tolerance:",
                  "Check that the hue of the albedo value of a material is within the tolerance of the hue of the albedo swatch being validated against"),
              m_AlbedoSwatchHueTolerance, 0f, 0.5f);

          m_AlbedoSwatchSaturationTolerance = EditorGUILayout.Slider(
              EditorGUIUtility.TrTextContent(
                  "Saturation Tolerance:",
                  "Check that the saturation of the albedo value of a material is within the tolerance of the saturation of the albedo swatch being validated against"),
              m_AlbedoSwatchSaturationTolerance, 0f, 0.5f);
        }
  ...
  internal void DrawLightingExposureSlider() {
    m_ExposureSliderValue = EditorGUIInternal.ExposureSlider(
        m_ExposureSliderValue, ref m_ExposureSliderMax,
  ...
  void UpdateGIContributorsReceiversColors() {
    if (m_NoContributeGIStyle == null ||
        m_NoContributeGIStyle.normal.background == null) {
      m_NoContributeGIStyle =
          CreateSwatchStyleForColor(kSceneViewMaterialNoContributeGI.Color);
      m_ReceiveGILightmapsStyle =
          CreateSwatchStyleForColor(kSceneViewMaterialReceiveGILightmaps.Color);
      m_ReceiveGILightProbesStyle = CreateSwatchStyleForColor(
          kSceneViewMaterialReceiveGILightProbes.Color);
    } else {
      UpdateSwatchTexture(m_NoContributeGIStyle.normal.background,
                          kSceneViewMaterialNoContributeGI.Color);
      UpdateSwatchTexture(m_ReceiveGILightmapsStyle.normal.background,
                          kSceneViewMaterialReceiveGILightmaps.Color);
      UpdateSwatchTexture(m_ReceiveGILightProbesStyle.normal.background,
                          kSceneViewMaterialReceiveGILightProbes.Color);
    }
  ...
  static void DrawGIContributorsReceiversSettings(Object target,
                                                  SceneView sceneView) {
    sceneView.DrawGIContributorsReceiversSettings();
  }

  void DrawSceneViewSwatch() {
    if (this.cameraMode.drawMode == DrawCameraMode.ValidateAlbedo ||
        this.cameraMode.drawMode == DrawCameraMode.ValidateMetalSpecular) {
      PrepareValidationUI();
  ...
  void UpdateGizmoExposure() {
    if (this.showExposureSettings) {
      if (m_ExposureTexture == null)
        m_ExposureTexture = new Texture2D(1, 1, GraphicsFormat.R32G32_SFloat,
                                          TextureCreationFlags.None);

      m_ExposureTexture.SetPixel(
          0, 0, new Color(Mathf.Pow(2.0f, m_ExposureSliderValue), 0.0f, 0.0f));
      m_ExposureTexture.Apply();

      Gizmos.exposure = m_ExposureTexture;
    } else {
      if (m_EmptyExposureTexture == null) {
        m_EmptyExposureTexture = new Texture2D(
            1, 1, GraphicsFormat.R32G32_SFloat, TextureCreationFlags.None);
  ...
  protected virtual void OnGUI() {
    ...
    if (evt.type == EventType.Repaint) {
      s_MouseRects.Clear();
      Tools.InvalidateHandlePosition(); // Some cases that should invalidate the
                                        // cached position are not handled
                                        // correctly yet so we refresh it once
                                        // per frame
    }
    ...
    if (m_CustomScene.IsValid())
      restoreOverrideRenderSettings =
          Unsupported.SetOverrideLightingSettings(m_CustomScene);
    ...
    Rect groupSpaceCameraRect = new Rect(0, 0, windowSpaceCameraRect.width,
                                         windowSpaceCameraRect.height);
    Rect groupSpaceCameraRectInPixels =
        EditorGUIUtility.PointsToPixels(groupSpaceCameraRect);
    ...
    // Unfocus search field on mouse clicks into content, so that key presses
    // work to navigate.
    ...
    if (evt.GetTypeForControl(m_MainViewControlID) == EventType.MouseDown &&
        groupSpaceCameraRect.Contains(evt.mousePosition))
      GUIUtility.keyboardControl = m_MainViewControlID;

    // Draw camera
    bool pushedGUIClip;
    DoDrawCamera(windowSpaceCameraRect, groupSpaceCameraRect,
                 out pushedGUIClip);
    ...
    // messed up (think HDR target with value of 50 + alpha blend gizmo... gonna
    // be white!)

    bool hdrDisplayActive =
        (m_Parent != null && m_Parent.actualView == this && m_Parent.hdrActive);
    if (!UseSceneFiltering() && evt.type == EventType.Repaint &&
        GraphicsFormatUtility.IsIEEE754Format(
            m_SceneTargetTexture.graphicsFormat) &&
        !hdrDisplayActive) {
      ...
      Graphics.SetRenderTarget(m_SceneTargetTexture.colorBuffer,
                               currentDepthBuffer);
    ...
    if (!UseSceneFiltering()) {
      ...
      if (evt.type == EventType.Repaint) {
        Graphics.DrawTexture(groupSpaceCameraRect, m_SceneTargetTexture,
                             new Rect(0, 0, 1, 1), 0, 0, 0, 0, GUI.color,
                             EditorGUIUtility.GUITextureBlit2SRGBMaterial);
        Profiler.EndSample();
      }
    }

    // By this time the 3D scene is done being drawn, and we're left with
    // gizmos, handles and SceneViewGUI stuff. Reusing the same 3D scene render
    // target, we draw those things and blit them on the back buffer without
    // doing sRGB conversions on them since they were always intended to draw
    // without sRGB conversions.
    GUIClip.Push(new Rect(0f, 0f, m_SceneTargetTexture.width,
                          m_SceneTargetTexture.height),
                 Vector2.zero, Vector2.zero, true);

    if (evt.type == EventType.Repaint) {
      Graphics.SetRenderTarget(m_SceneTargetTexture);
      GL.Clear(false, true,
               new Color(0, 0, 0, 0)); // Only clear color. Keep depth intact.
    }
    ...
    // Calling OnSceneGUI before DefaultHandles, so users can use events before
    // the Default Handles
    ...
    // Handle scene view motion when this scene view is active (always after
    // duringSceneGui and Tools, so that user tools can access RMB and alt keys
    // if they want to override the event) Do not pass the camera transform to
    // the SceneViewMotion calculations. The camera transform is calculation
    // *output* not *input*. Avoiding using it as input too avoids errors
    // accumulating.
    SceneViewMotion.DoViewTool(this);

    Handles.SetCameraFilterMode(Camera.current,
                                UseSceneFiltering()
                                    ? Handles.CameraFilterMode.ShowFiltered
                                    : Handles.CameraFilterMode.Off);

    // Handle scene commands after EditorTool.OnSceneGUI so that tools can
    // handle commands
    if (evt.type == EventType.ExecuteCommand ||
        evt.type == EventType.ValidateCommand || evt.keyCode == KeyCode.Escape)
      CommandsGUI();
    ...
    if (evt.type == EventType.Repaint) {
      // Blit the results with a pre-multiplied alpha shader to compose them
      // correctly on top of the 3D scene on the back buffer
      Graphics.DrawTexture(groupSpaceCameraRect, m_SceneTargetTexture,
                           new Rect(0, 0, 1, 1), 0, 0, 0, 0, GUI.color,
                           EditorGUIUtility.GUITextureBlitSceneGUIMaterial);
    }
  ...
  void HandleMouseCursor() {
    ...
    var checkMouseRects =
        evt.type == EventType.MouseMove || evt.type == EventType.Repaint;
  ...
  void DrawRenderModeOverlay(Rect cameraRect) {
    ...
    if (m_CameraMode.drawMode == DrawCameraMode.AlphaChannel) {
      if (!s_AlphaOverlayMaterial)
        s_AlphaOverlayMaterial = EditorGUIUtility.LoadRequired(
            "SceneView/SceneViewAlphaMaterial.mat") as Material;
      Handles.BeginGUI();
      if (Event.current.type == EventType.Repaint)
        Graphics.DrawTexture(cameraRect, EditorGUIUtility.whiteTexture,
                             s_AlphaOverlayMaterial);
      Handles.EndGUI();
    }
    ...
    if (m_CameraMode.drawMode == DrawCameraMode.DeferredDiffuse ||
      ...
      if (!s_DeferredOverlayMaterial)
        s_DeferredOverlayMaterial = EditorGUIUtility.LoadRequired(
            "SceneView/SceneViewDeferredMaterial.mat") as Material;
      Handles.BeginGUI();
      if (Event.current.type == EventType.Repaint) {
        s_DeferredOverlayMaterial.SetInt(
            "_DisplayMode",
            (int) m_CameraMode.drawMode - (int) DrawCameraMode.DeferredDiffuse);
        Graphics.DrawTexture(cameraRect, EditorGUIUtility.whiteTexture,
                             s_DeferredOverlayMaterial);
      }
      Handles.EndGUI();
    }
  }
  ...
  // Center point of the scene view. Modify it to move the sceneview
  // immediately, or use LookAt to animate it nicely.
  ...
  public bool orthographic {
    ...
    set {
      m_Ortho.value = value;
      svRot.UpdateGizmoLabel(this, m_Rotation.target * Vector3.forward,
                             m_Ortho.target);
    }
  }
  ...
  void ResetIfNaN() {
    // If you zoom out enough, m_Position would get corrupted with no way to
    // reset it, even after restarting Unity. Crude hack to at least get the
    // scene view working again!
    if (Single.IsInfinity(m_Position.value.x) ||
        Single.IsNaN(m_Position.value.x))
      m_Position.value = Vector3.zero;
    if (Single.IsInfinity(m_Rotation.value.x) ||
        Single.IsNaN(m_Rotation.value.x))
      m_Rotation.value = Quaternion.identity;
  }
  ...
  // In order to check things like "is using deferred", use
  // IsUsingDeferredRenderingPath
  ...
  internal static bool IsUsingDeferredRenderingPath() {
    ...
    return (renderingPath == RenderingPath.DeferredShading) ||
           (renderingPath == RenderingPath.UsePlayerSettings &&
            Rendering.EditorGraphicsSettings.GetCurrentTierSettings()
  ...
  private void SetSceneCameraHDRAndDepthModes() {
    ...
    m_Camera.clearStencilAfterLightingPass =
        mainCamera.clearStencilAfterLightingPass;
  }
  ...
  void SetupCamera() {
    if (m_CameraMode.drawMode == DrawCameraMode.Overdraw) {
      ...
      if (m_StageHandling != null)
        m_Camera.backgroundColor =
            StageNavigationManager.instance.currentStage.GetBackgroundColor();
      else
        m_Camera.backgroundColor = kSceneViewBackground;
    }

    if (Event.current.type == EventType.Repaint) {
      bool enableImageEffects =
          m_CameraMode.drawMode == DrawCameraMode.Textured &&
          sceneViewState.imageEffectsEnabled;
      UpdateImageEffects(enableImageEffects);
    }

    EditorUtility.SetCameraAnimateMaterials(
        m_Camera, sceneViewState.alwaysRefreshEnabled);
    ParticleSystemEditorUtils.renderInSceneView =
        m_SceneViewState.particleSystemsEnabled;
    UnityEngine.VFX.VFXManager.renderInSceneView =
        m_SceneViewState.visualEffectGraphsEnabled;
    ...
    m_Camera.transform.position =
        m_Position.value +
        m_Camera.transform.rotation * new Vector3(0, 0, -cameraDistance);
    ...
    if (m_ViewIsLockedToObject && Selection.gameObjects.Length > 0) {
      var bounds = InternalEditorUtility.CalculateSelectionBounds(
          false, Tools.pivotMode == PivotMode.Pivot);
      ...
      case (DraggingLockedState.NotDragging):
        // Once framed, we only need to lock position rather than all the
        // parameters Frame() sets
  ...
  void UpdateAnimatedMaterials() {
    if (sceneViewState.alwaysRefreshEnabled &&
        m_lastRenderedTime + 0.033f < EditorApplication.timeSinceStartup) {
  ...
  internal Vector3 cameraTargetPosition =>
      m_Position.target +
      m_Rotation.target * new Vector3(0, 0, -cameraDistance);
  ...
  // Look directionally at a specific point from a given direction with a given
  // zoom level.
  ...
  // Look at a specific point from a given direction with a given zoom level,
  // enabling and disabling perspective
  public void LookAt(Vector3 point, Quaternion direction, float newSize,
                     bool ortho) {
  ...
  void DefaultHandles() {
    // Note event state.
    EditorGUI.BeginChangeCheck();
    bool IsDragEvent = Event.current.GetTypeForControl(GUIUtility.hotControl) ==
                       EventType.MouseDrag;
    bool IsMouseUpEvent = Event.current.GetTypeForControl(
                              GUIUtility.hotControl) == EventType.MouseUp;

    EditorToolContext.OnToolGUI(this);

    // If we are actually dragging the object(s) then disable 2D physics
    // movement.
    if (EditorGUI.EndChangeCheck() && EditorApplication.isPlaying &&
        IsDragEvent)
      Physics2D.SetEditorDragMovement(true, Selection.gameObjects);

    // If we have finished dragging the object(s) then enable 2D physics
    // movement.
  ...
  bool CallEditorDragFunctions(IList<Object> dragAndDropObjects) {
    ...
    if (evt.type == EventType.Used || dragAndDropObjects.Count == 0)
      return true;
    ...
    // We iterate through dragged items backwards to preserve the alphabetical
    // order of GameObjects when they are created in hierarchy once drag is
    // performed
  ...
  internal static bool CanDoDrag(ICollection<Object> objects) {
    ...
    // Only allow dragging multiple GameObjects, or multiple non-GameObjects,
    // but not mixed sets. For example when dragging GameObjects and Materials
    // would sometimes apply material to already existing scene object, and
    // other times to the object being spawned. It depends on the order in which
    // the user selects those assets. We decided it was not an intuitive
    // behavior and it should just not be allowed.
    // Also we don't want multiple materials be dropped into scene because there
    // is no case where we can handle it in a way that benefit the user. For
    // example multiple skybox materials doesn't make sense and dropping
    // multiple materials onto geometry will only drop the first material on the
    // hovered material entry.
    foreach (Object obj in objects) {
      ...
      if (gameObjectCount > 0 && assetCount > 0 || materialCount > 1)
        return false;
    }
  ...
  void HandleDragging() {
    ...
    case EventType.DragUpdated:
      if (evt.type == EventType.DragPerform &&
          GameObjectInspector.s_CyclicNestingDetected) {
      ...
      if (DragAndDrop.visualMode != DragAndDropVisualMode.Copy) {
        ...
        var parent = defaultParentObject != null ? defaultParentObject
        ...
        DragAndDrop.visualMode = DragAndDropService.Drop(
            DragAndDropService.kSceneDropDstId, go, pivot,
            Event.current.mousePosition, parent, isPerform);
      }
  ...
  void CommandsGUI() {
    ...
    case EventCommandNames.SelectChildren:
      if (execute) {
        List<GameObject> gameObjects =
            new List<GameObject>(Selection.gameObjects);
    ...
    case EventCommandNames.SelectPrefabRoot:
      if (execute) {
        List<GameObject> gameObjects =
            new List<GameObject>(Selection.gameObjects.Length);
        foreach (var gameObject in Selection.gameObjects) {
  ...
  public void AlignWithView() {
    ...
    Quaternion delta = Quaternion.Inverse(Selection.activeTransform.rotation) *
  ...
  public void MoveToView(Transform target) { target.position = pivot; }
  ...
  public bool FrameSelected() { return FrameSelected(false); }
  ...
  public virtual bool FrameSelected(bool lockView, bool instant) {
    ...
    Bounds bounds;
    if (!m_WasFocused) {
      bounds = InternalEditorUtility.CalculateSelectionBounds(
          false, Tools.pivotMode == PivotMode.Pivot, true);
    } else {
    ...
    foreach (Editor editor in activeEditors) {
      MethodInfo hasBoundsMethod = editor.GetType().GetMethod(
          "HasFrameBounds", BindingFlags.Public | BindingFlags.NonPublic |
      ...
      if (hasBoundsMethod != null) {
        ...
        if (hasBounds is bool && (bool) hasBounds) {
          MethodInfo getBoundsMethod = editor.GetType().GetMethod(
              "OnGetFrameBounds", BindingFlags.Public | BindingFlags.NonPublic |
                                      BindingFlags.Instance |
                                      BindingFlags.FlattenHierarchy);
  ...
  public bool Frame(Bounds bounds, bool instant = true) {
    ...
    // We snap instantly into target on playmode, because things might be moving
    // fast and lerping lags behind
  ...
  void CreateSceneCameraAndLights() {
    GameObject cameraGO = EditorUtility.CreateGameObjectWithHideFlags(
        "SceneCamera", HideFlags.HideAndDontSave, typeof(Camera));
    ...
    for (int i = 0; i < 3; i++) {
      GameObject lightGO = EditorUtility.CreateGameObjectWithHideFlags(
          "SceneLight", HideFlags.HideAndDontSave, typeof(Light));
    ...
    HandleUtility.handleMaterial.SetColor("_SkyColor",
                                          kSceneViewUpLight * 1.5f);
    HandleUtility.handleMaterial.SetColor("_GroundColor",
                                          kSceneViewDownLight * 1.5f);
    HandleUtility.handleMaterial.SetColor("_Color",
                                          kSceneViewFrontLight * 1.5f);
  }

  void CallOnSceneGUI() {
    foreach (Editor editor in activeEditors) {
      if (!drawGizmos ||
          !EditorGUIUtility.IsGizmosAllowedForObject(editor.target))
        continue;
      ...
      if (method != null) {
        ...
        bool enabled = (methodEnabled != null)
                           ? (bool) methodEnabled.Invoke(null, null)
                           : true;
        if (enabled) {
          using(new EditorPerformanceTracker(
              $"Editor.{editor.GetType().Name}.OnSceneGUI")) {
            ...
            for (int n = 0; n < editor.targets.Length; n++) {
              ...
              EditorGUI.BeginChangeCheck();
              // Ironically, only allow multi object access inside OnSceneGUI if
              // editor does NOT support multi-object editing. since there's no
              // harm in going through the serializedObject there if there's
        ...
        // This would mean that OnSceneGUI has changed the scene and it is not
        // drawn
...
#pragma warning restore 618
  ...
  void CallOnPreSceneGUI() {
    foreach (Editor editor in activeEditors) {
      ...
      // Don't call function for editors whose target's GameObject is not
      // active.
      ...
      MethodInfo method = editor.GetType().GetMethod(
          "OnPreSceneGUI", BindingFlags.Public | BindingFlags.NonPublic |
                               BindingFlags.Instance |
                               BindingFlags.FlattenHierarchy);
      ...
      if (method != null) {
        ...
        for (int n = 0; n < editor.targets.Length; n++) {
          editor.referenceTargetIndex = n;
          // Ironically, only allow multi object access inside OnPreSceneGUI if
          // editor does NOT support multi-object editing. since there's no harm
          // in going through the serializedObject there if there's always only
          // one target.
          Editor.m_AllowMultiObjectAccess = !editor.canEditMultipleObjects;
          method.Invoke(editor, null);
          Editor.m_AllowMultiObjectAccess = true;
  ...
  internal static void ShowSceneViewPlayModeSaveWarning() {
    // In this case, we want to explicitly try the GameView before passing it on
    // to whatever notificationView we have
    var playModeView = (PlayModeView) WindowLayout.FindEditorWindowOfType(
        typeof(PlayModeView));
    if (playModeView != null && playModeView.hasFocus)
      playModeView.ShowNotification(EditorGUIUtility.TrTextContent(
  ...
  public static CameraMode AddCameraMode(string name, string section) {
    ...
    if (userDefinedModes.Contains(newMode))
      throw new InvalidOperationException(string.Format(
          "A mode named {0} already exists in section {1}", name, section));
  ...
  private static bool IsValidCameraMode(CameraMode cameraMode) {
    foreach (var mode in Enum.GetValues(typeof(DrawCameraMode))) {
      if (SceneRenderModeWindow.DrawCameraModeExists((DrawCameraMode) mode) &&
          cameraMode == GetBuiltinCameraMode((DrawCameraMode) mode)) {
        return true;
      }
    }
  ...
  public static void ClearUserDefinedCameraModes() { userDefinedModes.Clear(); }
