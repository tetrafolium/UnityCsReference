...
class SceneViewStageHandling {
  ...
  StateCache<SceneViewCameraState> m_StateCache =
      new StateCache<SceneViewCameraState>("Library/StateCache/SceneView/");
  ...
  public float breadcrumbHeight {
    get { return BreadcrumbBar.DefaultStyles.background.fixedHeight; }
  }
  ...
  public void OnEnable() {
    ...
    // We need to sync to the current stage to ensure we have the correct
    // SceneView settings: We could have closed Unity while a prefab scene was
    // open this means that the SceneView settings for that prefab is saved to
    // the window layout. Opening Unity always opens the main scenes so here we
    // ensure we have e.g the corrct sky box and other settings.
    currentStage.SyncSceneViewToStage(m_SceneView);
    LoadCameraState(m_SceneView, currentStage);
  }

  public void OnDisable() {
    // Ensure saving current stage settings so we can reconstruct them on
    // OnEnable
  ...
  public void StartOnGUI() { currentStage.OnPreSceneViewRender(m_SceneView); }

  public void EndOnGUI() { currentStage.OnPostSceneViewRender(m_SceneView); }

  internal void RebuildBreadcrumbBar() { m_BreadcrumbInitialized = false; }

  void AssetsChangedOnHDD(string[] importedAssets, string[] deletedAssets,
  ...
  void SaveCameraState(SceneView sceneView, Stage stage) {
    ...
    // Useful for Prefab Mode in Context where we want to save the context scene
    // state.
    ...
    Hash128 key = StageUtility.CreateWindowAndStageIdentifier(
        sceneView.windowGUID, contextStage);
  ...
  public void BreadcrumbGUI() {
    ...
    float verticalOffset =
        Mathf.Floor((BreadcrumbBar.DefaultStyles.background.fixedHeight -
                     breadcrumbsHeight) /
                    2f);
    ...
    using(new GUILayout.VerticalScope(BreadcrumbBar.DefaultStyles.background)) {
      ...
      using(new EditorGUILayout.HorizontalScope(
          GUILayout.Height(breadcrumbsHeight))) {
  ...
  void SwitchStageOnNextUpdate() {
    ...
    if (stageClicked != null && stageClicked.isValid)
      StageNavigationManager.instance.SwitchToStage(
          stageClicked, false,
          StageNavigationManager.Analytics.ChangeType.NavigateViaBreadcrumb);
  }
  ...
  void BreadCrumbItemClicked(BreadcrumbBar.Item item) {
    ...
    if (StageNavigationManager.instance.currentStage == stageClicked) {
      EditorGUIUtility.PingObject(
          AssetDatabase.LoadMainAssetAtPath(stageClicked.assetPath));
    } else {
      // The BreadCrumbItemClicked event is called during the SceneView's OnGUI
      // and if we switch stage directly we might delete the target of the
      // SceneView's activeEditors which the rest of the SceneView's OnGUI is
      // not expecting. E.g it will cause null ref exceptions in OnSceneGUI().
      // Instead we delay switching until the next Update.
