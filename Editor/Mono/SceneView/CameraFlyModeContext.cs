...
public struct InputSamplingScope : IDisposable {
	...
	active && !Mathf.Approximately(currentInputVector.sqrMagnitude, 0f);

	public Vector3 currentInputVector =>
	active ? s_CurrentInputVector : Vector3.zero;

	public bool inputVectorChanged =>
	active &&
	!Mathf.Approximately(
		(s_CurrentInputVector - m_Context.m_PreviousVector).sqrMagnitude,
		0f);
	...
	// controlID will get hotControl if using arrow keys while shortcut context
	// is not active passing a value of zero disables the arrow keys
	public InputSamplingScope(CameraFlyModeContext context,
	                          ViewTool currentViewTool, int controlID,
	                          EditorWindow window, bool orthographic = false) {
		...
		if (currentlyMoving &&
		    Mathf.Approximately(m_Context.m_PreviousVector.sqrMagnitude, 0f))
			s_Timer.Begin();
	}
	...
	static readonly Dictionary<KeyCode, Action<bool, ShortcutArguments> >
	s_ArrowKeyBindings =
		new Dictionary<KeyCode, Action<bool, ShortcutArguments> > {
		{KeyCode.UpArrow,
		 (orthographic, args) => {if (orthographic) WalkUp(args);
			                  else WalkForward(args); }}
		, {KeyCode.DownArrow, (orthographic, args) => {if (orthographic) WalkDown(args);
			                                       else WalkBackward(args); }}
	...
	void ForceArrowKeysUp(bool orthographic) {
		foreach (KeyCode key in s_ArrowKeysDown) {
			...
			action(orthographic, new ShortcutArguments {
...
[ClutchShortcut("3D Viewport/Fly Mode Forward", typeof(CameraFlyModeContext),
                KeyCode.W)]
[FormerlyPrefKeyAs("View/FPS Forward", "w")]
static void WalkForward(ShortcutArguments args) {
	s_CurrentInputVector.z =
		args.stage == ShortcutStage.Begin ? 1f : s_CurrentInputVector.z > 0f
	  ? 0f
	  : s_CurrentInputVector.z;
...
[ClutchShortcut("3D Viewport/Fly Mode Backward", typeof(CameraFlyModeContext),
                KeyCode.S)]
...
[ClutchShortcut("3D Viewport/Fly Mode Left", typeof(CameraFlyModeContext),
                KeyCode.A)]
[FormerlyPrefKeyAs("View/FPS Strafe Left", "a")]
static void WalkLeft(ShortcutArguments args) {
	s_CurrentInputVector.x =
		args.stage == ShortcutStage.Begin ? -1f : s_CurrentInputVector.x < 0f
	  ? 0f
	  : s_CurrentInputVector.x;
	var context = (CameraFlyModeContext) args.context;
	context.m_Window.Repaint();
...
[ClutchShortcut("3D Viewport/Fly Mode Right", typeof(CameraFlyModeContext),
                KeyCode.D)]
...
[ClutchShortcut("3D Viewport/Fly Mode Up", typeof(CameraFlyModeContext),
                KeyCode.E)]
[FormerlyPrefKeyAs("View/FPS Strafe Up", "e")]
static void WalkUp(ShortcutArguments args) {
	s_CurrentInputVector.y =
		args.stage == ShortcutStage.Begin ? 1f : s_CurrentInputVector.y > 0f
	  ? 0f
	  : s_CurrentInputVector.y;
	var context = (CameraFlyModeContext) args.context;
	context.m_Window.Repaint();
...
[ClutchShortcut("3D Viewport/Fly Mode Down", typeof(CameraFlyModeContext),
                KeyCode.Q)]
