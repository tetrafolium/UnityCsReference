...
static SavedFloat s_GridSizeX =
	new SavedFloat("GridSizeX", k_DefaultGridSize);
static SavedFloat s_GridSizeY =
	new SavedFloat("GridSizeY", k_DefaultGridSize);
static SavedFloat s_GridSizeZ =
	new SavedFloat("GridSizeZ", k_DefaultGridSize);
static SavedInt s_GridMultiplier =
	new SavedInt("GridMultiplier", k_DefaultMultiplier);
...
public static Vector3 size {
	...
	set {
		...
		ResetSizeMultiplier();
		s_GridSizeX.value =
			Mathf.Min(k_GridSizeMax, Mathf.Max(k_GridSizeMin, value.x));
		s_GridSizeY.value =
			Mathf.Min(k_GridSizeMax, Mathf.Max(k_GridSizeMin, value.y));
		s_GridSizeZ.value =
			Mathf.Min(k_GridSizeMax, Mathf.Max(k_GridSizeMin, value.z));
		sizeChanged(size);
	}
}
...
[System.Serializable]
internal class Grid {
[SerializeField] AnimBool m_Fade = new AnimBool();

[SerializeField] Color m_Color;

[SerializeField] Vector3 m_Pivot;
...
internal static PrefColor kViewGridColor =
	new PrefColor("Scene/Grid", .5f, .5f, .5f, .4f);
static float k_AngleThresholdForOrthographicGrid = 0.15f;

[SerializeField] Grid xGrid = new Grid();

[SerializeField] Grid yGrid = new Grid();

[SerializeField] Grid zGrid = new Grid();

[SerializeField] bool m_ShowGrid = k_DefaultShowGrid;

[SerializeField] GridRenderAxis m_GridAxis = k_DefaultRenderAxis;

[SerializeField] float m_gridOpacity = k_DefaultGridOpacity;
...
void GridSizeChanged(Vector3 size) {
	SetPivot(GridRenderAxis.X,
	         Snapping.Snap(GetPivot(GridRenderAxis.X), GridSettings.size));
	SetPivot(GridRenderAxis.Y,
	         Snapping.Snap(GetPivot(GridRenderAxis.Y), GridSettings.size));
	SetPivot(GridRenderAxis.Z,
	         Snapping.Snap(GetPivot(GridRenderAxis.Z), GridSettings.size));
}
...
internal void UpdateGridsVisibility(Quaternion rotation, bool orthoMode) {
	...
	if (showGrid) {
		...
		// In ortho, fallback on this path if camera is not aligned with X, Y or Z
		// axis.
		if (!showX && !showY && !showZ) {
			showX =
				(gridAxis == GridRenderAxis.X || gridAxis == GridRenderAxis.All);
			showY =
				(gridAxis == GridRenderAxis.Y || gridAxis == GridRenderAxis.All);
			showZ =
				(gridAxis == GridRenderAxis.Z || gridAxis == GridRenderAxis.All);
		}
	}
...
DrawGridParameters PrepareGridRenderPerspectiveMode(Camera camera,
                                                    Vector3 pivot,
                                                    Quaternion rotation,
                                                    float size) {
	DrawGridParameters parameters = default(DrawGridParameters);
...
DrawGridParameters PrepareGridRenderOrthogonalMode(Camera camera,
                                                   Vector3 pivot,
                                                   Quaternion rotation,
                                                   float size) {
	Vector3 direction =
		camera.transform.TransformDirection(new Vector3(0, 0, 1));
	...
	if (xGrid.fade.target &&
		...
		parameters = xGrid.PrepareGridRender(0, gridOpacity);
	else if (yGrid.fade.target &&
	         Mathf.Abs(direction.y) >= k_AngleThresholdForOrthographicGrid)
		parameters = yGrid.PrepareGridRender(1, gridOpacity);
	else if (zGrid.fade.target &&
	         Mathf.Abs(direction.z) >= k_AngleThresholdForOrthographicGrid)
		parameters = zGrid.PrepareGridRender(2, gridOpacity);
