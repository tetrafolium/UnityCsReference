...
enum MotionState { kInactive, kActive, kDragging }

static MotionState s_CurrentState;

static int s_ViewToolID = GUIUtility.GetPermanentControlID();

static readonly CameraFlyModeContext s_CameraFlyModeContext =
	new CameraFlyModeContext();
...
public static void DoViewTool(SceneView view) {
	...
	// If a SceneView is currently taking input, don't let other views accept
	// input
	...
	// In FPS mode we update the pivot for Orbit mode (see below and inside
	// HandleMouseDrag)
	...
	using (var inputSamplingScope = new CameraFlyModeContext.InputSamplingScope(
		       s_CameraFlyModeContext, Tools.s_LockedViewTool, id, view,
		       view.orthographic)) {
	...
	case EventType.ScrollWheel:
		...
		break; // Default to zooming to mouse position in 2D mode without alt
	case EventType.MouseDown:
		HandleMouseDown(view, id, evt.button);
		break;
	case EventType.MouseUp:
		HandleMouseUp(view, id, evt.button, evt.clickCount);
		break;
...
static Vector3 GetMovementDirection() {
	...
	if (s_Moving) {
		if (s_CurrentSceneView.cameraSettings.accelerationEnabled)
			s_FlySpeedTarget = s_FlySpeedTarget < Mathf.Epsilon ? k_FlySpeed
	    : s_FlySpeedTarget * Mathf.Pow(k_FlySpeedAcceleration, deltaTime);
		else
	...
	if (s_CurrentSceneView.cameraSettings.easingEnabled) {
		...
		s_FlySpeed.target =
			s_Motion.normalized * s_FlySpeedTarget * speedModifier;
	} else {
...
private static void HandleMouseDown(SceneView view, int id, int button) {
	...
	if (Tools.viewToolActive) {
		...
		if (Tools.s_LockedViewTool != wantedViewTool) {
			...
			// Set the hotcontrol and then lock the viewTool (important to set the
			// hotControl AFTER the Tools.viewTool has been evaluated)
			...
			// we're not dragging yet, but enter this state so we can cleanup
			// correctly
			s_CurrentState = MotionState.kActive;

			// This prevents camera from moving back on mousedown... find out why...
			// and write a comment
			GUIUtility.ExitGUI();
		}
	}
...
private static void HandleMouseUp(SceneView view, int id, int button,
                                  int clickCount) {
	if (GUIUtility.hotControl == id) {
		GUIUtility.hotControl = 0;
		...
		if (button == 2 && s_CurrentState != MotionState.kDragging) {
			...
			if (RaycastWorld(Event.current.mousePosition, out hit)) {
				Vector3 currentPosition = view.pivot - view.rotation *
				                          Vector3.forward *
				                          view.cameraDistance;
				float targetSize = view.size;
				if (!view.orthographic)
					targetSize = view.size *
					             Vector3.Dot(hit.point - currentPosition,
					                         view.rotation * Vector3.forward) /
					             view.cameraDistance;
...
static bool RaycastWorld(Vector2 position, out RaycastHit hit) {
	...
	// Loop through all meshes and find the RaycastHit closest to the ray
	// origin.
	...
	foreach (MeshFilter mf in meshFil) {
		...
		if (!mesh)
			continue;
		RaycastHit localHit;
		if (HandleUtility.IntersectRayMesh(
			    mouseRay, mesh, mf.transform.localToWorldMatrix, out localHit)) {
	...
	if (minT == Mathf.Infinity) {
		// If we didn't hit any mesh or collider surface, then use the transform
		// position projected onto the ray.
		hit.point = Vector3.Project(picked.transform.position - mouseRay.origin,
		                            mouseRay.direction) +
		            mouseRay.origin;
	}
	return true;
...
private static void OrbitCameraBehavior(SceneView view) {
	...
	rotation = Quaternion.AngleAxis(evt.delta.y * .003f * Mathf.Rad2Deg,
	                                rotation * Vector3.right) *
	           rotation;
	rotation =
		Quaternion.AngleAxis(evt.delta.x * .003f * Mathf.Rad2Deg, Vector3.up) *
		rotation;
	if (view.size < 0) {
	...
	if (GUIUtility.hotControl == id) {
		...
		case ViewTool.Orbit: {
			if (!view.in2DMode && !view.isRotationLocked) {
				...
				view.svRot.UpdateGizmoLabel(view, view.rotation * Vector3.forward,
				                            view.m_Ortho.target);
			}
		} break;
		case ViewTool.FPS: {
			if (!view.in2DMode && !view.isRotationLocked) {
				if (!view.orthographic) {
					...
					// The reason we calculate the camera position from the pivot,
					// rotation and distance, rather than just getting it from the
					// camera transform is that the camera transform is the *output* of
					// camera motion calculations. It shouldn't be input and output at
					// the same time, otherwise we easily get accumulated error. We did
					// get accumulated error before when we did this - the camera would
					// continuously move slightly in FPS mode even when not holding down
					// any arrow/ASDW keys or moving the mouse.
					Vector3 camPos = view.pivot - view.rotation * Vector3.forward *
					                 view.cameraDistance;
					...
					rotation = Quaternion.AngleAxis(evt.delta.y * .003f * Mathf.Rad2Deg,
					                                rotation * Vector3.right) *
					           rotation;
					rotation = Quaternion.AngleAxis(evt.delta.x * .003f * Mathf.Rad2Deg,
					                                Vector3.up) *
					           rotation;
					view.rotation = rotation;

					view.pivot =
						camPos + rotation * Vector3.forward * view.cameraDistance;
				} else {
		...
		case ViewTool.Pan: {
			...
			Vector3 worldDelta = ScreenToWorldDistance(
				view, new Vector2(-screenDelta.x, screenDelta.y));
...
internal static Vector3 ScreenToWorldDistance(SceneView view, Vector2 delta) {
	// Ensures that the camera matrix doesn't end up with gigantic or minuscule
	// values in the clip to world matrix
	...
	Vector3 pivot =
		camera.transform.rotation * new Vector3(0f, 0f, view.cameraDistance);
...
private static void HandleScrollWheel(SceneView view,
                                      bool zoomTowardsCenter) {
	if (Tools.s_LockedViewTool == ViewTool.FPS) {
		float scrollWheelDelta =
			Event.current.delta.y * s_FPSScrollWheelMultiplier;
		...
		string cameraSpeedDisplayValue = cameraSettingsSpeed.ToString(
			cameraSettingsSpeed < 0.01f
	      ? "F3"
	      : cameraSettingsSpeed < 0.1f
		    ? "F2"
		    : cameraSettingsSpeed < 10f ? "F1" : "F0");

		if (cameraSettingsSpeed < 0.1f)
			cameraSpeedDisplayValue =
				cameraSpeedDisplayValue.TrimStart(new Char[] {'0'});

		GUIContent cameraSpeedContent =
			EditorGUIUtility.TempContent(string.Format(
							     "{0}{1}", cameraSpeedDisplayValue,
		...
		if (!zoomTowardsCenter && Mathf.Abs(view.cameraDistance) < 1.0e7f) {
			...
			var mousePivot =
				mouseRay.origin + mouseRay.direction * view.cameraDistance;
...
internal static string[] kMenuDirNames = {"Free",  "Right", "Top",
	                                  "Front", "Left",  "Bottom",
	                                  "Back",  "",      "Perspective"};
...
int currentDir = 7;
internal AnimBool[] dirVisible = {new AnimBool(true), new AnimBool(true),
	                          new AnimBool(true)};
...
float fadedRotationLock {
	get {
		return Mathf.Lerp(kRotationLockedAlpha, 1.0f, m_RotationLocked.faded);
	}
}
...
public Styles() {
	...
	unlockedRotationIcon = EditorGUIUtility.TrIconContent(
		"LockIcon", "Click to lock the rotation in the current direction.");
	lockedRotationIcon = EditorGUIUtility.TrIconContent(
		"LockIcon-On", "Click to unlock the rotation.");
...
public void Register(SceneView view) {
	...
	int labelIndex = GetLabelIndexForView(view, view.rotation * Vector3.forward,
	                                      view.orthographic);
	for (int i = 0; i < dirNameVisible.Length; i++)
...
private void AxisSelectors(SceneView view, Camera cam, float size, float sgn,
                           GUIStyle viewAxisLabelStyle) {
	for (int h = kDirectionRotations.Length - 1; h >= 0; h--) {
		...
		if (sgn > 0 && Handles.Button(m_ViewDirectionControlIDs[h],
		                              q1 * Vector3.forward * size * -1.2f, q1,
...
internal void HandleContextClick(SceneView view) {
	if (!view.in2DMode && !view.isRotationLocked) {
		...
		if (evt.type == EventType.MouseDown && evt.button == 1) {
			...
			Rect rotationRect =
				new Rect(cameraGUIRect.width - kRotationSize + kRotationMenuInset,
				         kRotationMenuInset, kRotationSize - kRotationMenuInset * 2,
				         kRotationSize - kRotationMenuInset * 2);

			if (rotationRect.Contains(evt.mousePosition)) {
				DisplayContextMenu(
					new Rect(evt.mousePosition.x, evt.mousePosition.y, 0, 0), view);
				evt.Use();
			}
		}
	}
...
private void DisplayContextMenu(Rect buttonOrCursorRect, SceneView view) {
	...
	if (!view.orthographic)
		selectedItems[1] = 8;
	EditorUtility.DisplayCustomMenu(buttonOrCursorRect, kMenuDirNames,
	                                selectedItems, ContextMenuDelegate, view);
	GUIUtility.ExitGUI();
}

private void ContextMenuDelegate(object userData, string[] options,
                                 int selected) {
...
private void DrawIsoStatusSymbol(Vector3 center, SceneView view,
                                 float alpha) {
	...
	Handles.DrawAAPolyLine(pos + up * (1 - persp),
	                       pos + right + up * (1 + persp * 0.5f));
...
void DrawRotationLock(SceneView view) {
	...
	Rect lockRect =
		new Rect(lockCenterX - (clickWidth / 2),
		         lockCenterY - (clickHeight / 2), clickWidth, clickHeight);
	Color c = Handles.centerColor;
	c.a *= m_Visible.faded;
	if (c.a > 0.0f) {
		var prevColor = GUI.color;
		GUI.color = c;
		var content = (view.isRotationLocked) ? styles.lockedRotationIcon
		                            : styles.unlockedRotationIcon;
		if (GUI.Button(lockRect, m_RotationLockControlID, content,
		               styles.lockStyle) &&
		    !view.in2DMode) {
...
void DrawLabels(SceneView view) {
	Rect cameraGUIRect = view.cameraRect;
	Rect labelRect = new Rect(cameraGUIRect.width - kRotationSize + 17,
	                          kRotationSize - 8, kRotationSize - 17 * 2, 16);

	// Button (overlayed over the labels) to toggle between iso and perspective
	if (!view.in2DMode && !view.isRotationLocked) {
		if (GUI.Button(labelRect, m_PerspectiveIsoControlID, GUIContent.none,
		               styles.viewLabelStyleLeftAligned)) {
	...
	if (Event.current.type == EventType.Repaint) {
		...
		// Calculate the weighted average width of the labels so we can do smart
		// centering of the labels.
		Rect slidingLabelRect = labelRect;
		float width = 0;
		float weightSum = 0;
		for (int i = 0; i < kDirNames.Length; i++) {
			if (i ==
			    index2D) // Future proof even if we add more labels after the 2D one
				continue;
			weightSum += dirNameVisible[i].faded;
			if (dirNameVisible[i].faded > 0)
				width += styles.viewLabelStyleLeftAligned
				         .CalcSize(EditorGUIUtility.TempContent(kDirNames[i]))
				         .x *
		...
		// First draw the regular ones - all except the 2D label. They use the
		// slidingLabelRect.
		for (int i = 0; i < dirNameVisible.Length && i < kDirNames.Length; i++) {
			...
			if (c.a > 0.0f) {
				GUI.color = c;
				GUI.Label(slidingLabelRect, kDirNames[i],
				          styles.viewLabelStyleLeftAligned);
			}
		}
		// Then draw just the label for 2D. It uses the original labelRect, and
		// with a style where the text is horizontally centered.
		{
			Color c = Handles.centerColor;
			c.a *= faded2Dgray * fadedVisibility;
			if (c.a > 0.0f) {
				GUI.color = c;
				GUI.Label(labelRect, kDirNames[index2D],
				          styles.viewLabelStyleCentered);
			}
		}
		...
		if (faded2Dgray < 1) {
			DrawIsoStatusSymbol(
				new Vector3(slidingLabelRect.x - 8, slidingLabelRect.y + 8.5f, 0),
				view, (1 - faded2Dgray) * fadedRotationLock);
		}
	}
}

// Ensure all controlIDs used in this OnGUI are permanent controlIDs so this
// method can be called in different places for handling input early and
// rendering late.
internal void OnGUI(SceneView view) {
	...
	SceneView.AddCursorRect(
		new Rect(cameraGUIRect.width - kRotationSize + kRotationMenuInset,
		         kRotationMenuInset, kRotationSize - 2 * kRotationMenuInset,
		         kRotationSize + 24 - kRotationMenuInset),
		MouseCursor.Arrow);

	Handles.SetCamera(new Rect(cameraGUIRect.width - kRotationSize, 0,
	                           kRotationSize, kRotationSize),
	                  cam);
	...
	for (int i = 0; i < 3; ++i) {
		...
		dirVisible[i].target =
			Mathf.Abs(Vector3.Dot(cam.transform.forward, direction)) < 0.9f;
	}
	...
	if (Handles.Button(m_CenterButtonControlID, Vector3.zero,
	                   Quaternion.identity, size * 0.8f, size,
	                   Handles.CubeHandleCap) &&
	    !view.in2DMode && !view.isRotationLocked) {
		if (Event.current.clickCount == 2)
			view.FrameSelected();
		else {
			// If middle-click or shift-click, choose a perspective view from a nice
			// angle, similar to in Unity 3.5.
	...
	if (!view.in2DMode && !view.isRotationLocked) {
		...
		if (Event.current.type == EditorGUIUtility.swipeGestureEventType) {
			...
			// This is consistent with how we do orbiting, where moving the mouse
			// down sees the object more from above etc. Also, make swipe dir point
			// almost 45 degrees in towards the camera. This means the swipe will
			// pick the closest axis in the swiped direction, instead of picking the
			// one closest to being 90 degrees away.
			Vector3 goalVector = -swipeDir - Vector3.forward * 0.9f;

			// Transform swipe dir by camera transform, so up is camera's local up,
			// etc.
			...
			int dir = 0;
			for (int i = 0; i < 6; i++) {
				// Note that kDirectionRotations are not the forward direction of each
				// dir; it's the back direction *towards* the camera.
				float dotProduct = Vector3.Dot(
					kDirectionRotations[i] * -Vector3.forward, goalVector);
...
internal void ViewAxisDirection(SceneView view, int dir) {
	// If holding shift or clicking with middle mouse button, orthographic is
	// enforced, otherwise not altered. Note: This function can also be called
	// from a context menu where Event.current is null.
	bool ortho = view.orthographic ||
	             Event.current != null &&
	             (Event.current.shift || Event.current.button == 2);
...
internal void ViewSetUnityDefault(SceneView view) {
	...
	view.LookAt(view.pivot, Quaternion.LookRotation(new Vector3(-1, -.7f, -1)),
	            view.size, view.orthographic);
}
...
internal void UpdateGizmoLabel(SceneView view, Vector3 direction,
                               bool ortho) {
	SwitchDirNameVisible(GetLabelIndexForView(view, direction, ortho));
}

internal int GetLabelIndexForView(SceneView view, Vector3 direction,
                                  bool ortho) {
	if (!view.in2DMode) {
		// If the view is axis aligned, find the correct axis.
		if (IsAxisAligned(direction))
			for (int i = 0; i < 6; i++)
				if (Vector3.Dot(kDirectionRotations[i] * Vector3.forward, direction) >
				    0.9f)
					return i;
...
internal void ViewFromNiceAngle(SceneView view, bool forcePerspective) {
	// Use dir that's the same as the current one in the x-z plane, but placed a
	// bit above middle vertically. (Same as old dir except it had the x-z dir
	// fixed.)
	Vector3 dir = view.rotation * Vector3.forward;
	dir.y = 0;
	if (dir == Vector3.zero)
		// When the view is top or bottom, the closest nice view is to look
		// approx. towards z.
		dir = Vector3.forward;
	else
...
private bool IsAxisAligned(Vector3 v) {
	return (Mathf.Abs(v.x * v.y) < 0.0001f && Mathf.Abs(v.y * v.z) < 0.0001f &&
	        Mathf.Abs(v.z * v.x) < 0.0001f);
}
