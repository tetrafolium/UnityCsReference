...
// 3. All subsequent clicks cycle through the stack of overlapping objects from
// top to bottom, regardless of their SelectionBase status
// 4. When we hit the bottom, we goto 1

// Example: Scene from back to front (visually): Panel(base), Label, Image,
// Button(base), Image2, Label2 Selection order: Button, Label2, Image2, Image,
// Label, Panel, goto start
...
internal class SceneViewPicking {
  ...
  static SceneViewPicking() { Selection.selectionChanged += ResetHashes; }
  ...
  public static GameObject PickGameObject(Vector2 mousePosition) {
    ...
    if (topmostHash != s_PreviousTopmostHash) {
      ...
      // Return selection base if exists and is not already selected, otherwise
      // topmost game object
      s_PreviousTopmostHash = topmostHash;
      s_PreviousPrefixHash = prefixHash;
    ...
    var picked = HandleUtility.PickGameObject(
        mousePosition, false, null,
        new GameObject[]{Selection.activeGameObject});
  ...
  // Use picking system to get us ordered list of all visually overlapping
  // gameobjects in screen position from top to bottom
  private static IEnumerable<GameObject> GetAllOverlapping(Vector2 position) {
    ...
    while (true) {
      var go =
          HandleUtility.PickGameObject(position, false, ignoreList.ToArray());
      if (go == null)
        break;
      ...
      // Prevent infinite loop if game object cannot be ignored when picking
      // (This needs to fixed so print an error)
      if (allOverlapping.Count > 0 && go == allOverlapping.Last()) {
        Debug.LogError(
            "GetAllOverlapping failed, could not ignore game object '" +
            go.name + "' when picking");
        break;
      }
