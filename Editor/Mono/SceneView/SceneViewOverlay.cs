...
public void End() {
	...
	if (s_Windows.Count > 0) {
		...
		var windowOverlayRect =
			new Rect(sceneViewGUIRect.x, 0f, sceneViewGUIRect.width,
			         m_SceneView.position.height);
		GUILayout.Window("SceneViewOverlay".GetHashCode(), windowOverlayRect,
		                 WindowTrampoline, "",
		                 Styles.sceneViewOverlayTransparentBackground);
	}
...
static void EatMouseInput(Rect position) {
	...
	var id = GUIUtility.GetControlID("SceneViewOverlay".GetHashCode(),
...
public static void Window(GUIContent title, WindowFunction sceneViewFunc,
                          int order, WindowDisplayOption option) {
	Window(title, sceneViewFunc, order, null, option);
}

// pass window parameter to render in sceneviews that are not the active view.
public static void Window(GUIContent title, WindowFunction sceneViewFunc,
                          int order, Object target,
	...
	foreach (var overlayWindow in s_Windows) {
		if (option == WindowDisplayOption.OneWindowPerTarget &&
		    overlayWindow.target == target && target != null)
			return;

		if (option == WindowDisplayOption.OneWindowPerTitle &&
		    (overlayWindow.title == title ||
		     overlayWindow.title.text == title.text))
			return;
	}

	var newWindow =
		new OverlayWindow(title, sceneViewFunc, order, target, option){
		secondaryOrder = s_Windows.Count, canCollapse = false
	};
...
public static void ShowWindow(OverlayWindow window) {
	...
	foreach (var overlayWindow in s_Windows) {
		if (window.option == WindowDisplayOption.OneWindowPerTarget &&
		    overlayWindow.target == window.target && window.target != null)
			return;

		if (window.option == WindowDisplayOption.OneWindowPerTitle &&
		    (overlayWindow.title == window.title ||
		     overlayWindow.title.text == window.title.text))
			return;
	}
...
public OverlayWindow(GUIContent title,
                     SceneViewOverlay.WindowFunction guiFunction,
                     int primaryOrder, Object target,
...
public SceneViewOverlay.WindowFunction sceneViewFunc {
	get;
}
...
public SceneViewOverlay.WindowDisplayOption option {
	get;
}
