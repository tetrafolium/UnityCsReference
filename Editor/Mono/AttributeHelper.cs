...
[StructLayout(LayoutKind.Sequential)] struct MonoGizmoMethod {
...
static MonoGizmoMethod[] ExtractGizmos(Assembly assembly) {
	...
	foreach (var mi in EditorAssemblies
	         .GetAllMethodsWithAttribute<DrawGizmo>(BindingFlags.Static)
	         .Where(m => m.DeclaringType.Assembly == assembly)) {
		var attrs =
			mi.GetCustomAttributes(typeof(DrawGizmo), false).Cast<DrawGizmo>();
		foreach (var gizmoAttr in attrs) {
			...
			if (parameters[1].ParameterType !=
			    typeof(GizmoType) &&parameters[1].ParameterType != typeof(int)) {
				Debug.LogWarningFormat(
...
[StructLayout(LayoutKind.Sequential)] struct MonoCreateAssetItem {
...
static MonoCreateAssetItem[] ExtractCreateAssetMenuItems(Assembly assembly) {
	...
	foreach (var type in TypeCache
		...
		var attr =
			type.GetCustomAttributes(typeof(CreateAssetMenuAttribute), false)
		...
		if (!type.IsSubclassOf(typeof(ScriptableObject))) {
			Debug.LogWarningFormat(
				"CreateAssetMenu attribute on {0} will be ignored as {0} is not derived from ScriptableObject.",
				type.FullName);
			continue;
		}

		string menuItemName = (string.IsNullOrEmpty(attr.menuName))
		                ? ObjectNames.NicifyVariableName(type.Name)
		                : attr.menuName;
		string fileName =
			(string.IsNullOrEmpty(attr.fileName))
	      ? ("New " + ObjectNames.NicifyVariableName(type.Name) + ".asset")
	      : attr.fileName;
		if (!System.IO.Path.HasExtension(fileName))
			fileName = fileName + ".asset";

		var item =
			new MonoCreateAssetItem {
			menuItem = menuItemName, fileName = fileName,
			order = attr.order, type = type
		};
		result.Add(item);
	}
...
internal static bool GameObjectContainsAttribute<T>(GameObject go) where T
: Attribute {
...
internal static IEnumerable<TReturnValue>
CallMethodsWithAttribute<TReturnValue, TAttr>(
	params object[] arguments) where TAttr : Attribute {
	foreach (var method in EditorAssemblies.GetAllMethodsWithAttribute<TAttr>(
...
MethodMatchesAnyRequiredSignatureOfAttribute(MethodInfo method,
                                             Type attributeType) {
	List<MethodInfo> validSignatures = new List<MethodInfo>();
	foreach (var signature in attributeType.GetMethods(
			 BindingFlags.NonPublic | BindingFlags.Static |
			 BindingFlags.Instance | BindingFlags.DeclaredOnly)) {
		var requiredSignatureAttributes = signature.GetCustomAttributes(
			typeof(RequiredSignatureAttribute), false);
	...
	if (validSignatures.Count == 0)
		Debug.LogError(MethodToString(method) + " has an invalid attribute : " +
		               attributeType + ". " + attributeType +
		               " must have at least one required signature declaration");
	else if (validSignatures.Count == 1)
		Debug.LogError(MethodToString(method) + " does not match " +
		               attributeType + " expected signature.\n Use " +
		               MethodToString(validSignatures[0]));
	else
		Debug.LogError(
			MethodToString(method) + " does not match any of " + attributeType +
			" expected signatures.\n Valid signatures are: " +
			string.Join(
				" , ",
				validSignatures.Select((a) => MethodToString(a)).ToArray()));
	return false;
}
...
public IEnumerable<MethodInfo> FilterAndSortOnAttribute<T>(
	Func<T, bool> filter, Func<T, IComparable> sorter) where T : Attribute {
	return methodsWithAttributes.Where(a => filter((T) a.attribute))
	       .OrderBy(c => sorter((T) c.attribute))
	       .Select(o => o.info);
}

public IEnumerable<MethodWithAttribute> methodsWithAttributes {
	get;
}
}

static Dictionary<Type, MethodInfoSorter> s_DecoratedMethodsByAttrTypeCache =
	new Dictionary<Type, MethodInfoSorter>();
private const BindingFlags kAllStatic =
	BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic;

internal static MethodInfoSorter
GetMethodsWithAttribute<T>(BindingFlags bindingFlags = kAllStatic) where T
: Attribute {
	...
	if (!s_DecoratedMethodsByAttrTypeCache.TryGetValue(typeof(T), out result)) {
		...
		foreach (var method in EditorAssemblies.GetAllMethodsWithAttribute<T>(
				 bindingFlags)) {
			if (method.IsGenericMethod) {
				Debug.LogErrorFormat(
					"{0} is a generic method. {1} cannot be applied to it.",
					MethodToString(method), typeof(T));
			} else {
				foreach (var attr in method.GetCustomAttributes(typeof(T), false)) {
					if (MethodMatchesAnyRequiredSignatureOfAttribute(method,
					                                                 typeof(T))) {
						var methodWithAttribute =
							new MethodWithAttribute {
