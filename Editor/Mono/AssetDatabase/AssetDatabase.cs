...
// Delegate to be called from [[AssetDatabase.ImportPackage]] callbacks in the
// event of failure
public delegate void ImportPackageFailedCallback(string packageName,
                                                 string errorMessage);
...
[RequiredByNativeCode]
private static void
Internal_CallOnImportPackageItemsCompleted(string[] items) {
...
[RequiredByNativeCode]
private static void Internal_CallImportPackageFailed(string packageName,
                                                     string errorMessage) {
...
IsOpenForEdit(string[] assetOrMetaFilePaths, List<string> outNotEditablePaths,
	...
	AssetModificationProcessorInternal.IsOpenForEdit(
		assetOrMetaFilePaths, outNotEditablePaths, statusQueryOptions);
...
[RequiredByNativeCode]
private static bool
Internal_MakeEditable2(string[] paths, string prompt = null,
                       List<string> outNotEditablePaths = null) {
	return MakeEditable(paths, prompt, outNotEditablePaths);
}

public static bool MakeEditable(string[] paths, string prompt = null,
                                List<string> outNotEditablePaths = null) {
	if (paths == null)
	...
	return Provider.MakeEditableImpl(paths, prompt, changeSet,
	                                 outNotEditablePaths);
}
}
}
