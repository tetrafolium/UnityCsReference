...
internal static extern Texture2D GetAssetPreview(int instanceID,
                                                 int clientID);
...
internal static extern Texture2D GetAssetPreviewFromGUID(string guid,
                                                         int clientID);
...
internal static extern bool IsLoadingAssetPreview(int instanceID,
                                                  int clientID);
...
internal static extern void SetPreviewTextureCacheSize(int size,
                                                       int clientID);
...
public static Texture2D GetMiniTypeThumbnail(Type type) {
	...
	if (typeof(MonoBehaviour).IsAssignableFrom(type))
		tex =
			EditorGUIUtility.LoadIcon(type.FullName.Replace('.', '/') + " Icon");
	else
...
GetMiniTypeThumbnailFromType(Type managedType);
}
}
