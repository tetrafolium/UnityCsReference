...
public sealed partial class AssetDatabase {
  ...
  public static string[] FindAssets(string filter, string[] searchInFolders) {
    var searchFilter =
        new SearchFilter{searchArea = SearchFilter.SearchArea.AllAssets};
  ...
  internal static string[] FindAssets(SearchFilter searchFilter) {
    return FindAllAssets(searchFilter)
        .Select(property => property.guid)
        .ToArray();
  }

  internal static IEnumerable<HierarchyProperty>
  FindAllAssets(SearchFilter searchFilter) {
    var enumerator = EnumerateAllAssets(searchFilter);
    while (enumerator.MoveNext())
      yield return enumerator.Current;
  }

  internal static IEnumerator<HierarchyProperty>
  EnumerateAllAssets(SearchFilter searchFilter) {
  ...
  private static IEnumerator<T>
  FindInFolders<T>(SearchFilter searchFilter,
    ...
    foreach (var folderPath in folders) {
      ...
      if (packageInfo != null) {
        ...
        if (searchFilter.skipHidden &&
            !PackageManagerUtilityInternal.IsPathInVisiblePackage(rootPath))
          continue;
      }
      ...
      if (property.Find(folderInstanceID, null)) {
        ...
        Debug.LogWarning("AssetDatabase.FindAssets: Folder not found: '" +
                         folderPath + "'");
      }
    }
  }
  ...
  private static IEnumerator<T>
  FindEverywhere<T>(SearchFilter searchFilter,
                    Func<HierarchyProperty, T> selector) {
