...
// Will generate per-triangle uv (3 uv pairs for each triangle) with default
// settings
...
// Will generate per-triangle uv (3 uv pairs for each triangle) with provided
// settings
public static Vector2[] GeneratePerTriangleUV(Mesh src,
...
[NativeThrows]
internal static extern Vector2[] GeneratePerTriangleUVImpl(
	Mesh src, UnwrapParam settings);

// Will auto generate uv2 with default settings for provided mesh, and fill
// them in
public static void GenerateSecondaryUVSet(Mesh src) {
	MeshUtility.SetPerTriangleUV2(src, GeneratePerTriangleUV(src));
}

// Will auto generate uv2 with provided settings for provided mesh, and fill
// them in
