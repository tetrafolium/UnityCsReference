...
internal class LayerVisibilityWindow : EditorWindow {
  private class Styles {
    ...
    public readonly Texture2D visibleOn =
        EditorGUIUtility.LoadIcon("animationvisibilitytoggleon");
    public readonly Texture2D visibleOff =
        EditorGUIUtility.LoadIcon("animationvisibilitytoggleoff");
    public readonly GUIContent editLayers =
        EditorGUIUtility.TrTextContent("Edit Layers...");
    public Styles() {
  ...
  private void Init(Rect buttonRect) {
    ...
    var windowHeight =
        rowCount * EditorGUI.kSingleLineHeight + kSeparatorHeight;
  ...
  internal void OnGUI() {
    ...
    var scrollViewRect =
        new Rect(kFrameWidth, kFrameWidth, position.width - 2 * kFrameWidth,
                 position.height - 2 * kFrameWidth);
  ...
  private void DrawSeparator(ref Rect rect, bool even) {
    DrawListBackground(
        new Rect(rect.x + 1, rect.y, rect.width - 2, kSeparatorHeight), even);
    GUI.Label(new Rect(rect.x + 5, rect.y + 3, rect.width - 10, 3),
              GUIContent.none, s_Styles.separator);
  ...
  void DoSpecialLayer(Rect rect, bool all, ref bool even) {
    ...
    DoLayerEntry(rect, all ? "Everything"
                 : "Nothing", even, true, false, visible, false,
                   out visibleChanged, out lockedChanged);
    if (visibleChanged) {
  ...
  void DoOneLayer(Rect rect, int index, ref bool even) {
    ...
    bool visibleChanged, lockedChanged;
    DoLayerEntry(rect, s_LayerNames[index], even, true, true, visible, locked,
                 out visibleChanged, out lockedChanged);
    if (visibleChanged) {
  ...
  void DoOneSortingLayer(Rect rect, int index, ref bool even) {
    ...
    bool visibleChanged, lockedChanged;
    DoLayerEntry(rect, InternalEditorUtility.GetSortingLayerName(index), even,
                 false, true, true, locked, out visibleChanged,
                 out lockedChanged);
    if (lockedChanged) {
  ...
  private void DoLayerEntry(Rect rect, string layerName, bool even,
                            bool showVisible, bool showLock, bool visible,
                            bool locked, out bool visibleChanged,
                            out bool lockedChanged) {
    DrawListBackground(rect, even);
    ...
    visible =
        GUI.Toggle(textRect, visible, EditorGUIUtility.TempContent(layerName),
                   s_Styles.listTextStyle);

    // visible checkbox
    var toggleRect = new Rect(rect.width - kToggleSize * 2,
                              rect.y + (rect.height - kToggleSize) * 0.5f,
                              kToggleSize, kToggleSize);
    visibleChanged = false;
    if (showVisible) {
      var iconRect = toggleRect;
      iconRect.y += 3;
      var gc = new GUIContent(string.Empty, visible ? s_Styles.visibleOn
                              : s_Styles.visibleOff, kLayerVisible);
    ...
    if (showLock) {
      ...
      EditorGUI.BeginChangeCheck();
      GUI.Toggle(toggleRect, locked, new GUIContent(string.Empty, kLayerLocked),
                 s_Styles.lockButton);
