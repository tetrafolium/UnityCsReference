...
public class SceneViewCameraWindow : PopupWindowContent {
  static class Styles {
    public static readonly GUIContent copyPlacementLabel =
        EditorGUIUtility.TrTextContent("Copy Placement");
    public static readonly GUIContent pastePlacementLabel =
        EditorGUIUtility.TrTextContent("Paste Placement");
    public static readonly GUIContent copySettingsLabel =
        EditorGUIUtility.TrTextContent("Copy Settings");
    public static readonly GUIContent pasteSettingsLabel =
        EditorGUIUtility.TrTextContent("Paste Settings");
    public static readonly GUIContent resetSettingsLabel =
        EditorGUIUtility.TrTextContent("Reset Settings");
  ...
  GUIContent m_SceneCameraLabel =
      EditorGUIUtility.TrTextContent("Scene Camera");
  ...
  const int kWindowHeight =
      ((int) EditorGUI.kSingleLineHeight) * kFieldCount + kContentPadding * 2;
  const float kPrefixLabelWidth = 120f;
  const float kNearClipMin = .01f;
  const int k_HeaderSpacing = 2;
  // FOV values chosen to be the smallest and largest before extreme visual
  // corruption
  ...
  public override Vector2 GetWindowSize() { return m_WindowSize; }

  public SceneViewCameraWindow(SceneView sceneView) {
    m_SceneView = sceneView;

    m_CameraSpeedSliderContent = EditorGUIUtility.TrTextContent(
        "Camera Speed", "The current speed of the camera in the Scene view.");
    m_AccelerationEnabled = EditorGUIUtility.TrTextContent(
        "Camera Acceleration",
        "Check this to enable acceleration when moving the camera. When enabled, camera speed is evaluated as a modifier. With acceleration disabled, the camera is accelerated to the Camera Speed.");
    m_FieldOfView = EditorGUIUtility.TrTextContent(
        "Field of View",
        "The height of the camera's view angle. Measured in degrees vertically, or along the local Y axis.");
    m_DynamicClip = EditorGUIUtility.TrTextContent(
        "Dynamic Clipping",
        "Check this to enable camera's near and far clipping planes to be calculated relative to the viewport size of the Scene.");
    m_OcclusionCulling = EditorGUIUtility.TrTextContent(
        "Occlusion Culling",
        "Check this to enable occlusion culling in the Scene view. Occlusion culling disables rendering of objects when they\'re not currently seen by the camera because they\'re hidden (occluded) by other objects.");
    m_EasingEnabled = EditorGUIUtility.TrTextContent(
        "Camera Easing",
        "Check this to enable camera movement easing. This makes the camera ease in when it starts moving and ease out when it stops.");
    m_WindowSize = new Vector2(kWindowWidth, kWindowHeight);
    m_MinMaxContent = new[]{
        EditorGUIUtility.TrTextContent(
            "Min",
            "The minimum speed of the camera in the Scene view. Valid values are between [0.001, 98]."),
        EditorGUIUtility.TrTextContent(
            "Max",
            "The maximum speed of the camera in the Scene view. Valid values are between [0.002, 99].")};
  }
  ...
  void Draw() {
    ...
    if (GUILayout.Button(EditorGUI.GUIContents.titleSettingsIcon,
    ...
    // fov isn't applicable in orthographic mode, and orthographic size is
    // controlled by the user zoom
    using(new EditorGUI.DisabledScope(m_SceneView.orthographic)) {
      settings.fieldOfView =
          EditorGUILayout.Slider(m_FieldOfView, settings.fieldOfView,
                                 k_MinFieldOfView, k_MaxFieldOfView);
    }

    settings.dynamicClip =
        EditorGUILayout.Toggle(m_DynamicClip, settings.dynamicClip);

    using(new EditorGUI.DisabledScope(settings.dynamicClip)) {
      float near = settings.nearClip, far = settings.farClip;
      DrawClipPlanesField(EditorGUI.s_ClipingPlanesLabel, ref near, ref far,
                          EditorGUI.kNearFarLabelsWidth);
      settings.SetClipPlanes(near, far);
    }

    settings.occlusionCulling =
        EditorGUILayout.Toggle(m_OcclusionCulling, settings.occlusionCulling);
    ...
    settings.easingEnabled =
        EditorGUILayout.Toggle(m_EasingEnabled, settings.easingEnabled);
    settings.accelerationEnabled = EditorGUILayout.Toggle(
        m_AccelerationEnabled, settings.accelerationEnabled);

    EditorGUI.BeginChangeCheck();
    float min = settings.speedMin, max = settings.speedMax,
          speed =
              settings.RoundSpeedToNearestSignificantDecimal(settings.speed);
    ...
    if (Event.current.type == EventType.Repaint)
      m_WindowSize.y =
          Math.Min(GUILayoutUtility.GetLastRect().yMax + kContentPadding,
                   kWindowHeight * 3);
  ...
  void DrawClipPlanesField(GUIContent label, ref float near, ref float far,
                           float propertyLabelsWidth,
                           params GUILayoutOption[] options) {
    bool hasLabel = EditorGUI.LabelHasContent(label);
    float height =
        EditorGUI.kSingleLineHeight * 2 + EditorGUI.kVerticalSpacingMultiField;
    Rect r = EditorGUILayout.GetControlRect(hasLabel, height,
    ...
    near = EditorGUI.FloatField(fieldPosition, EditorGUI.s_NearAndFarLabels[0],
                                near);
    fieldPosition.y +=
        EditorGUI.kSingleLineHeight + EditorGUI.kVerticalSpacingMultiField;
    far = EditorGUI.FloatField(fieldPosition, EditorGUI.s_NearAndFarLabels[1],
                               far);
  ...
  internal static void ShowContextMenu(SceneView view) {
    ...
    if (CanPastePlacement())
      menu.AddItem(Styles.pastePlacementLabel, false,
                   () => PastePlacement(view));
    else
  ...
  static void CopyPlacementShortcut() {
    ...
    if (playView != null && (EditorWindow.focusedWindow == playView ||
                             EditorWindow.mouseOverWindow == playView)) {
      var cam = Camera.main;
      if (cam != null)
  ...
  static void CopyPlacement(SceneView view) {
    Clipboard.SetCustomValue(
        new TransformWorldPlacement(view.camera.transform));
  }
  ...
  static void PastePlacement(SceneView view) {
    ...
    // Similar to what AlignViewToObject does, except we need to do that
    // instantly in case the shortcut key was pressed while FPS camera controls
    // (right click drag) were active.
    view.size = 10;
    view.LookAt(tr.position + tr.forward * view.cameraDistance, tr.rotation,
                view.size, view.orthographic, true);
