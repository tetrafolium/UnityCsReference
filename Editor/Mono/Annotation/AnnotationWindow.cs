...
internal class AnnotationWindow : EditorWindow {
  class Styles {
    ...
    public GUIStyle listEvenBg =
        "ObjectPickerResultsOdd"; //"ObjectPickerResultsEven";//
  ...
  private enum EnabledState { NotSet = -1, None = 0, All = 1, Mixed = 2 }
  ...
  GUIContent iconToggleContent =
      EditorGUIUtility.TrTextContent("", "Show/Hide Icon");
  GUIContent iconSelectContent =
      EditorGUIUtility.TrTextContent("", "Select Icon");
  GUIContent icon3dGizmoContent = EditorGUIUtility.TrTextContent("3D Icons");
  GUIContent showOutlineContent =
      EditorGUIUtility.TrTextContent("Selection Outline");
  GUIContent showWireframeContent =
      EditorGUIUtility.TrTextContent("Selection Wire");
  ...
  static float Convert01ToTexelWorldSize(float value01) {
    ...
    return Mathf.Pow(
        ...
        exponentStart +
            exponentRange *
                value01); // texel size is between 10e-2 (0.01) and 10e2 (100)
                          // worldunits. (texel in the icon texture)
  }
  ...
  static string ConvertTexelWorldSizeToString(float texelWorldSize) {
    ...
    float displaySize =
        texelWorldSize * 32.0f; // The 32 is default icon size, so we show
                                // worldsize for 32 pixel icons.
    int numDecimals =
        MathUtils.GetNumberOfDecimalsForMinimumDifference(displaySize * 0.1f);
  ...
  float GetTopSectionHeight() {
    ...
    return EditorGUI.kSingleLineHeight * numberOfControls +
           EditorGUI.kControlVerticalSpacing * (numberOfControls - 1) +
           EditorStyles.inspectorBig.padding.bottom;
  }
  ...
  void OnDisable() {
    AssemblyReloadEvents.beforeAssemblyReload -= Close;
    // When window closes we copy all changes to monoimporters (reimport
    // monoScripts)
  ...
  internal static bool ShowAtPosition(Rect buttonRect, bool isGameView) {
    // We could not use realtimeSinceStartUp since it is set to 0 when
    // entering/exitting playmode, we assume an increasing time when comparing
    // time.
    ...
    if (!justClosed) {
      ...
      if (s_AnnotationWindow == null)
        s_AnnotationWindow =
            ScriptableObject.CreateInstance<AnnotationWindow>();
      else {
        // We are treating AnnotationWindow like a PopupWindow which has logic
        // to reclose it when opened, AuxWindows derived from EditorWindow
        // reset/reopen when repeatedly clicking the open button by design. Call
        // Cancel() here if it is already open.
  ...
  void Init(Rect buttonRect, bool isGameView) {
    ...
    float windowHeight = 2 * frameWidth + GetTopSectionHeight() +
                         DrawNormalList(false, 100, 0, 10000);
  ...
  AInfo GetAInfo(int classID, string scriptClass) {
    if (scriptClass != "")
      return m_ScriptAnnotations.Find(
          delegate(AInfo o) { return o.m_ScriptClass == scriptClass; });

    return m_BuiltinAnnotations.Find(
        delegate(AInfo o) { return o.m_ClassID == classID; });
  }
  ...
  private void SyncToState() {
    ...
    for (int i = 0; i < a.Length; ++i) {
      if (s_Debug)
        debuginfo += "   same as below: icon " + a[i].iconEnabled + " gizmo " +
                     a[i].gizmoEnabled + "\n";

      bool ge = (a[i].gizmoEnabled == 1);
      bool ie = (a[i].iconEnabled == 1);
      AInfo anno =
          new AInfo(ge, ie, a[i].flags, a[i].classID, a[i].scriptClass);
      ...
      if (anno.m_ScriptClass == "") {
        ...
        if (s_Debug)
          debuginfo +=
              "   " +
              UnityType.FindTypeByPersistentTypeID(anno.m_ClassID).name +
              ": icon " + anno.m_IconEnabled + " gizmo " + anno.m_GizmoEnabled +
              "\n";
      } else {
        m_ScriptAnnotations.Add(anno);
        if (s_Debug)
          debuginfo += "   " + a[i].scriptClass + ": icon " +
                       anno.m_IconEnabled + " gizmo " + anno.m_GizmoEnabled +
                       "\n";
      }
    }
  ...
  internal void OnGUI() {
    ...
    if (Event.current.type == EventType.Repaint)
      m_Styles.background.Draw(new Rect(0, 0, position.width, position.height),
  ...
  void DrawTopSection(float topSectionHeight) {
    // Bg
    GUI.Label(new Rect(frameWidth, 0, position.width - 2 * frameWidth,
                       topSectionHeight),
              "", EditorStyles.inspectorBig);

    float topmargin = 7;
    float margin = 11;
    float curY = topmargin;

    float labelWidth = m_Styles.listHeaderStyle.CalcSize(showOutlineContent).x +
                       GUI.skin.toggle.CalcSize(GUIContent.none).x + 1;
    float rowHeight = 18;

    // Toggle 3D gizmos
    Rect toggleRect = new Rect(margin, curY, labelWidth, rowHeight);
    AnnotationUtility.use3dGizmos = GUI.Toggle(
        toggleRect, AnnotationUtility.use3dGizmos, icon3dGizmoContent);

    // Texel world size
    float texelWorldSize = AnnotationUtility.iconSize;
    if (s_Debug) {
      Rect texelSizeRect =
          new Rect(0, curY + 10, position.width - margin, rowHeight);
      GUI.Label(texelSizeRect, ConvertTexelWorldSizeToString(texelWorldSize),
                m_Styles.texelWorldSizeStyle);
    }
    ...
    using(new EditorGUI.DisabledScope(!AnnotationUtility.use3dGizmos)) {
      ...
      Rect sliderRect =
          new Rect(labelWidth + margin, curY, sliderWidth - margin, rowHeight);
    ...
    using(new EditorGUI.DisabledScope(m_IsGameView)) {
      ...
      AnnotationUtility.showSelectionOutline =
          GUI.Toggle(toggleRect, AnnotationUtility.showSelectionOutline,
                     showOutlineContent);

      toggleRect.y += rowHeight;
      AnnotationUtility.showSelectionWire =
          GUI.Toggle(toggleRect, AnnotationUtility.showSelectionWire,
                     showWireframeContent);
    }
  }
  ...
  void DrawAnnotationList(float startY, float height) {
    ...
    Rect scrollViewRect = new Rect(0, startY + barHeight, position.width - 4,
    ...
    m_ScrollPosition = GUI.BeginScrollView(
        scrollViewRect, m_ScrollPosition, contentRect, false, false,
        GUI.skin.horizontalScrollbar, GUI.skin.verticalScrollbar,
        EditorStyles.scrollViewAlt);
    {
      DrawNormalList(true, listElementWidth,
                     m_ScrollPosition.y - listElementHeight,
                     m_ScrollPosition.y + totalContentHeight);
    }
    GUI.EndScrollView();
  }

  void Flip(ref bool even) { even = !even; }

  float DrawNormalList(bool doDraw, float listElementWidth, float startY,
                       float endY) {
    bool even = true;
    float curY = 0;
    bool headerDrawn = false;

    curY = DrawListSection(curY, L10n.Tr("Recently Changed"),
                           m_RecentAnnotations, doDraw, listElementWidth,
                           startY, endY, ref even, true, ref headerDrawn);
    curY = DrawListSection(curY, L10n.Tr("Scripts"), m_ScriptAnnotations,
                           doDraw, listElementWidth, startY, endY, ref even,
                           false, ref headerDrawn);
    curY = DrawListSection(curY, L10n.Tr("Built-in Components"),
                           m_BuiltinAnnotations, doDraw, listElementWidth,
                           startY, endY, ref even, false, ref headerDrawn);

    return curY;
  }

  float DrawListSection(float y, string sectionHeader, List<AInfo> listElements,
                        bool doDraw, float listElementWidth, float startY,
                        float endY, ref bool even, bool useSeperator,
                        ref bool headerDrawn) {
    float curY = y;
    ...
    if (listElements.Count > 0) {
      if (doDraw) {
        // Header background
        Rect rect =
            new Rect(1, curY, listElementWidth - 2, extraHeader + headerHeight);
        Flip(ref even);
        GUIStyle backgroundStyle = even ? m_Styles.listEvenBg
            : m_Styles.listOddBg; // m_Styles.listSectionHeaderBg;//
        GUI.Label(rect, GUIContent.Temp(""), backgroundStyle);
      }
      curY += extraHeader;
      if (doDraw) {
        // Header text
        DrawListHeader(sectionHeader, listElements,
                       new Rect(3, curY, listElementWidth, headerHeight),
                       ref headerDrawn);
      }
      curY += headerHeight;
      ...
      for (int i = 0; i < listElements.Count; ++i) {
        ...
        if (curY > startY && curY < endY) {
          Rect rect =
              new Rect(1, curY, listElementWidth - 2, listElementHeight);
          if (doDraw)
      ...
      if (useSeperator) {
        ...
        if (doDraw) {
          GUIStyle backgroundStyle =
              even ? m_Styles.listEvenBg : m_Styles.listOddBg;
          GUI.Label(new Rect(1, curY, listElementWidth - 2, height),
                    GUIContent.Temp(""), backgroundStyle);
          GUI.Label(new Rect(10, curY + 3, listElementWidth - 15, 3),
                    GUIContent.Temp(""), m_Styles.seperator);
        }
        curY += height;
      }
    }
  ...
  void DrawListHeader(string header, List<AInfo> elements, Rect rect,
                      ref bool headerDrawn) {
    ...
    for (int i = 0; i < elements.Count; i++) {
      ...
      if (element.HasGizmo()) {
        if (enabledState == EnabledState.NotSet) {
          enabledState =
              element.m_GizmoEnabled ? EnabledState.All : EnabledState.None;
        } else if ((enabledState == EnabledState.All) !=
                   element.m_GizmoEnabled) {
    ...
    var gizmoText = "gizmo";
    var gizmoTextSize =
        m_Styles.columnHeaderStyle.CalcSize(new GUIContent(gizmoText));
    gizmoTextRightAlign = gizmoTextSize.x;
    gizmoRightAlign = gizmoTextRightAlign -
                      (gizmoTextSize.x * 0.5f -
                       m_Styles.toggle.CalcSize(GUIContent.none).x * 0.5f);
    var iconText = "icon";
    var iconTextSize =
        m_Styles.columnHeaderStyle.CalcSize(new GUIContent(iconText));
    iconTextRightAlign = iconTextSize.x + gizmoTextRightAlign + 10;
    iconRightAlign =
        iconTextRightAlign - (iconTextSize.x * 0.5f - iconSize * 0.5f);
    ...
    Rect toggleRect = new Rect(rect.width - gizmoRightAlign,
                               rect.y + (rect.height - toggleSize) * 0.5f,
  ...
  void DrawListElement(Rect rect, bool even, AInfo ainfo) {
    ...
    Rect textRect = rect;
    // textRect.x += 22;
    textRect.width = rect.width - iconRightAlign -
                     22; // ensure text doesnt flow behind toggles
    GUI.Label(textRect, ainfo.m_DisplayText, m_Styles.listTextStyle);

    // Icon toggle
    Rect iconRect =
        new Rect(rect.width - iconRightAlign + 2,
                 rect.y + (rect.height - iconSize) * 0.5f, iconSize,
                 iconSize); // +2 because the given rect is shortened by 2px
    ...
    if (ainfo.m_ScriptClass != "") {
      ...
      GUI.DrawTexture(div, EditorGUIUtility.whiteTexture,
        ...
        if (GUI.Button(arrowRect, iconSelectContent, m_Styles.iconDropDown)) {
          ...
          if (script != null) {
            ...
            if (IconSelector.ShowAtPosition(script, arrowRect, true)) {
              IconSelector.SetMonoScriptIconChangedCallback(
                  MonoScriptIconChanged);
              GUIUtility.ExitGUI();
            }
          }
        }
    ...
    if (ainfo.Thumb != null) {
      if (!ainfo.m_IconEnabled) {
        GUI.color =
            new Color(GUI.color.r, GUI.color.g, GUI.color.b, disabledAlpha);
        tooltip = "";
      }
    ...
    if (ainfo.HasGizmo()) {
      ...
      Rect togglerRect =
          new Rect(rect.width - gizmoRightAlign + 2,
                   rect.y + (rect.height - togglerSize) * 0.5f, togglerSize,
                   togglerSize); // +2 because the given rect is shortened by
                                 // 2px before this method call
      ainfo.m_GizmoEnabled =
          GUI.Toggle(togglerRect, ainfo.m_GizmoEnabled,
                     new GUIContent("", tooltip), m_Styles.toggle);
      if (GUI.changed) {
        SetGizmoState(ainfo);
      }
    }
  ...
  void SetIconState(AInfo ainfo) {
    AnnotationUtility.SetIconEnabled(ainfo.m_ClassID, ainfo.m_ScriptClass,
                                     ainfo.m_IconEnabled ? 1 : 0);
    SceneView.RepaintAll();
  }

  void SetGizmoState(AInfo ainfo, bool addToMostRecentChanged = true) {
    AnnotationUtility.SetGizmoEnabled(ainfo.m_ClassID, ainfo.m_ScriptClass,
                                      ainfo.m_GizmoEnabled ? 1 : 0,
                                      addToMostRecentChanged);
    SceneView.RepaintAll();
  }
}
...
internal class AInfo : System.IComparable, System.IEquatable<AInfo> {
  ...
  public AInfo(bool gizmoEnabled, bool iconEnabled, int flags, int classID,
               string scriptClass) {
  ...
  public bool HasGizmo() { return (m_Flags & (int) Flags.kHasGizmo) > 0; }

  public bool HasIcon() { return (m_Flags & (int) Flags.kHasIcon) > 0; }

  public bool IsDisabled() { return (m_Flags & (int) Flags.kIsDisabled) > 0; }
