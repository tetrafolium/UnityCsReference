...
internal class BuildPlayerSceneTreeViewItem : TreeViewItem {
  ...
  public BuildPlayerSceneTreeViewItem(int id, int depth, GUID g, bool state)
      : base(id, depth) {
    active = state;
...
internal class BuildPlayerSceneTreeView : TreeView {
  ...
  private void HandleExternalSceneListChange() { Reload(); }
  ...
  protected override TreeViewItem BuildRoot() {
    ...
    foreach (var sc in scenes) {
      var item = new BuildPlayerSceneTreeViewItem(sc.guid.GetHashCode(), 0,
                                                  sc.guid, sc.enabled);
      root.AddChild(item);
    }
    return root;
  ...
  protected override bool CanBeParent(TreeViewItem item) { return false; }
  ...
  protected override void BeforeRowsGUI() {
    ...
    foreach (var item in rootItem.children) {
      ...
      // Need to set counter here because RowGUI is only called on items that
      // are visible.
      if (bpst.active) {
    ...
    if (sceneItem != null) {
      ...
      using(new EditorGUI.DisabledScope(!sceneExists)) {
        ...
        if (!sceneExists)
          newState = false;
        newState = GUI.Toggle(
            new Rect(args.rowRect.x, args.rowRect.y, 16f, 16f), newState, "");
        ...
        if (sceneItem.counter != BuildPlayerSceneTreeViewItem.kInvalidCounter) {
          TreeView.DefaultGUI.LabelRightAligned(args.rowRect,
                                                "" + sceneItem.counter,
                                                args.selected, args.focused);
        } else if (sceneItem.displayName == string.Empty || !sceneExists) {
          TreeView.DefaultGUI.LabelRightAligned(args.rowRect, "Deleted",
                                                args.selected, args.focused);
        }
      }
    } else
  ...
  protected override
      DragAndDropVisualMode HandleDragAndDrop(DragAndDropArgs args) {
    DragAndDropVisualMode visualMode = DragAndDropVisualMode.None;

    var draggedIDs =
        DragAndDrop.GetGenericData("BuildPlayerSceneTreeViewItem") as List<int>;
  ...
  protected override void DoubleClickedItem(int id) {
    BuildPlayerSceneTreeViewItem item =
        FindItem(id, rootItem) as BuildPlayerSceneTreeViewItem;
    int instanceID =
        AssetDatabase.GetMainAssetOrInProgressProxyInstanceID(item.fullName);
  ...
  protected int FindDropAtIndex(DragAndDropArgs args) {
    ...
    // covers if(args.dragAndDropPosition == DragAndDropPosition.OutsideItems)
    // and a safety check.
  ...
  protected override void SetupDragAndDrop(SetupDragAndDropArgs args) {
    ...
    DragAndDrop.SetGenericData("BuildPlayerSceneTreeViewItem",
                               new List<int>(args.draggedItemIDs));
  ...
  protected override void ContextClicked() {
    if (GetSelection().Count > 0) {
      ...
      menu.AddItem(EditorGUIUtility.TrTextContent("Remove Selection"), false,
                   RemoveSelection);
      menu.ShowAsContext();
    }
  }
  ...
  public EditorBuildSettingsScene[] GetSceneList() {
    ...
    for (int index = 0; index < rootItem.children.Count; index++) {
      ...
      sceneList[index] =
          new EditorBuildSettingsScene(sceneItem.fullName, sceneItem.active);
    }
    return sceneList;
  }
}
