...
[assembly:InternalsVisibleTo("VideoTesting")] namespace UnityEditor.Media {
  ...
  public struct MediaRational {
    ...
    public bool isZero {
      get { return isValid && numerator == 0; }
    }
    ...
    Internal_MediaRational_Reduce(ref int numerator, ref int denominator);
  }

  public struct MediaTime {
    public static readonly MediaTime Invalid =
        new MediaTime{count = 0, rate = MediaRational.Invalid};

    public MediaTime(long seconds) : this(seconds, 1) {}

    public MediaTime(long count, uint rateNumerator, uint rateDenominator = 1) {
      this.count = count;
      m_Rate = new MediaRational(Convert.ToInt32(rateNumerator),
                                 Convert.ToInt32(rateDenominator));
    }
  ...
  public struct AudioTrackAttributes {
    ...
    // Future work:
    // public string      format;   // E.g.: "Stereo", "5.1", "Ambisonic 1st
    // order", ... public string      layout[]; // E.g.: ["Left", "Right",
    // "Center"]
  }
  ...
  public class MediaEncoder : IDisposable {
    ...
    [Obsolete("Was made public by mistake. Not meant to be used by user code.",
              true)]
    public IntPtr m_Ptr;
    ...
    unsafe public bool AddFrame(int width, int height, int rowBytes,
                                TextureFormat format, NativeArray<byte> data) {
      return Internal_AddFrameRaw(m_ThisPtr, width, height, rowBytes, format,
                                  data.GetUnsafeReadOnlyPtr(), data.Length,
                                  MediaTime.Invalid);
    }
    ...
    unsafe public bool AddSamples(ushort trackIndex,
                                  NativeArray<float> interleavedSamples) {
      return Internal_AddSamples(m_ThisPtr, trackIndex,
                                 interleavedSamples.GetUnsafeReadOnlyPtr(),
    ...
    private IntPtr Create(string filePath, VideoTrackAttributes[] videoAttrs,
                          AudioTrackAttributes[] audioAttrs) {
      foreach (var v in videoAttrs) {
        var r = v.frameRate;
        if (r.isNegative)
          throw new ArgumentException(
              $"Negative frame rate not supported: {r.numerator}/{r.denominator}");
      }
      ...
      foreach (var a in audioAttrs) {
        ...
        if (!r.isValid)
          throw new ArgumentException(
              $"Invalid sample rate: {r.numerator}/{r.denominator}");
        if (r.isZero)
          throw new ArgumentException(
              $"Zero sample rate not supported: {r.numerator}/{r.denominator}");
        if (r.isNegative)
          throw new ArgumentException(
              $"Negative sample rate not supported: {r.numerator}/{r.denominator}");
      }
    ...
    [FreeFunction] extern private static IntPtr
    Internal_Create(string filePath, VideoTrackAttributes[] videoAttrs,
                    AudioTrackAttributes[] audioAttrs);

    [FreeFunction] extern private static void Internal_Release(IntPtr encoder);

    [FreeFunction] extern private static bool
    Internal_AddFrame(IntPtr encoder, Texture2D texture, MediaTime time);

    [FreeFunction] unsafe extern private static bool
    Internal_AddFrameRaw(IntPtr encoder, int width, int height, int rowBytes,
                         TextureFormat format, void *buffer, int byteCount,
                         MediaTime time);

    [FreeFunction] unsafe extern private static bool
    Internal_AddSamples(IntPtr encoder, ushort trackIndex, void *buffer,
                        int sampleCount);
  }
}
