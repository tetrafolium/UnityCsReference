...
// 'l:assetlabel' syntax (e.g 'l:architecture' will show assets with
// AssetLabel 'architecture') 'ref[:id]:path' syntax (e.g 'ref:1234' will show
// objects that references the object with instanceID 1234) 'v:versionState'
// syntax (e.g 'v:modified' will show objects that are modified locally)
// 's:softLockState' syntax (e.g 's:inprogress' will show objects that are
// modified by anyone (except you)) 'a:area' syntax (e.g 'a:all' will s search
// in all assets, 'a:assets' will s search in assets folder only and
// 'a:packages' will s search in packages folder only) 'glob:path' syntax (e.g
// 'glob:Assets/**/*.{png|PNG}' will show objects in any subfolder with name
// ending by .png or .PNG)
internal static bool ParseSearchString(string searchText,
                                       SearchFilter filter) {
	...
	while (pos < searchString.Length) {
		...
		if (endpos > pos) {
			...
			if (CheckForKeyWords(token, filter, q1 - pos, q2 - pos))
				...
				filter.nameFilter +=
					(string.IsNullOrEmpty(filter.nameFilter) ? "" : " ") +
					token; // force single space between name tokens
		}
		pos = endpos + 1;
	}
	return parsed;
...
internal static bool CheckForKeyWords(string searchString,
                                      SearchFilter filter, int quote1,
                                      int quote2) {
	bool parsed = false;
	...
	// Support: 'l:assetlabel' syntax (e.g 'l:architecture' will show assets
	// with AssetLabel 'architecture')
	...
	// Support: 'b:assetBundleName' syntax (e.g 'b:materialAssetBundle' will
	// show assets within assetBundle 'materialAssetBundle')
	...
	// Support: 'ref[:id]:path' syntax (e.g 'ref:1234' will show objects that
	// references the object with instanceID 1234)
	...
	if (index == 0) {
		...
		if (secondColon >= 0) {
			// Instead of resolving a passed-in pathname to an instance-id, use a
			// supplied one. The pathname is effectively just a UI hint of whose
			// references we're filtering out.
			string refString = searchString.Substring(firstColon + 1,
			                                          secondColon - firstColon - 1);
			int id;
			if (System.Int32.TryParse(refString, out id))
				instanceID = id;
			// else
			//  Debug.Log ("Not valid refString to case to Integer " + refString);
			...
			if (quote1 != -1 && quote2 != -1) {
				...
				// Strip filepath from quotes, don't prefix with Assets/, we need to
				// support Packages/ (https://fogbugz.unity3d.com/f/cases/1161019/)
				assetPath = searchString.Substring(startIndex, count);
			} else
				// Otherwise use string from colon to end, don't prefix with Assets/,
				// we need to support Packages/
				// (https://fogbugz.unity3d.com/f/cases/1161019/)
			...
			// else
			//  Debug.Log ("Not valid assetPath " + assetPath); // outcomment for
			//  debugging
		}
	...
	// Support: 'glob:path' syntax (e.g 'glob:Assets/**/*.{png|PNG}' will show
	// objects in any subfolder with name ending by .png or .PNG)
