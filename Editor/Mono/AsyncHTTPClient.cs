...
internal interface IAsyncHTTPClient {
  ...
  AsyncHTTPClient.DoneCallback doneCallback {
    get;
    set;
  }
...
internal partial class AsyncHTTPClient : IAsyncHTTPClient {
  ...
  public delegate void StatusCallback(State status, int bytesDone,
                                      int bytesTotal);
  ...
  public byte[] bytes {
    get { return GetBytesByHandle(m_Handle); }
  }
  public Texture2D texture {
    get { return GetTextureByHandle(m_Handle); }
  }
  ...
  /*
   * POST request for uploading url application/x-www-form-urlencoded
   * dictionary. The encoding normally allows for duplicate keys, but this
   * method is restricted to unique keys.
   */
  public Dictionary<string, string> postDictionary {
    set {
      postData = string.Join(
          "&",
          value.Select(kv => EscapeLong(kv.Key) + "=" + EscapeLong(kv.Value))
              .ToArray());
    }
  }
  ...
  public bool IsAborted() { return state == State.ABORTED; }

  public bool IsDone() { return IsDone(state); }
  ...
  public bool IsSuccess() { return state == State.DONE_OK; }

  public static bool IsSuccess(State state) { return state == State.DONE_OK; }
  ...
  public void Begin() {
    ...
    string[] headerFlattened =
        header.Select(kv => string.Format("{0}: {1}", kv.Key, kv.Value))
            .ToArray();

    m_Handle = SubmitClientRequest(tag, m_ToUrl, headerFlattened, m_Method,
                                   m_FromData, Done, Progress);
  }
  ...
  private void Done(State status, int i_ResponseCode) {
    ...
    m_Handle = (IntPtr) 0; // The CurlRequestMessage will be deallocated after
  ...
  /*
   * The normal escape function does not support strings longer than 32766
   * characters
   */
  private string EscapeLong(string v) {
    ...
    for (int i = 0; i < v.Length; i += c_ChunkLength) {
      q.Append(System.Uri.EscapeDataString(
          v.Substring(i, v.Length - i > c_ChunkLength ? c_ChunkLength
                      : v.Length - i)));
    }
    return q.ToString();
  }
