...
internal static class PreviewEditorWindow {
  internal static void RepaintAll() { PlayModeView.RepaintAll(); }
}
...
internal abstract class PlayModeView : EditorWindow {
  ...
  private readonly string m_ViewsCache = Path.GetFullPath(
      Directory.GetCurrentDirectory() + "/Library/PlayModeViewStates/");

  [SerializeField]
  private List<string> m_SerializedViewNames = new List<string>();
  [SerializeField]
  private List<string> m_SerializedViewValues = new List<string>();
  ...
  protected RenderTexture RenderView(Vector2 mousePosition, bool clearTexture) {
    using(var renderingView = new RenderingView(this)) {
      ...
      bool hdr = (m_Parent != null && m_Parent.actualView == this &&
                  m_Parent.hdrActive);
      ConfigureTargetTexture((int) targetSize.x, (int) targetSize.y,
                             clearTexture, playModeViewName, hdr);
      ...
      if (m_TargetTexture.IsCreated())
        EditorGUIUtility.RenderPlayModeViewCamerasInternal(
            m_TargetTexture, currentTargetDisplay, mousePosition, showGizmos,
            renderIMGUI);
  ...
  protected string GetWindowTitle(Type type) {
    var attributes =
        type.GetCustomAttributes(typeof(EditorWindowTitleAttribute), true);
    return attributes.Length > 0
               ? ((EditorWindowTitleAttribute) attributes[0]).title
               : type.Name;
  }

  protected Dictionary<Type, string> GetAvailableWindowTypes() {
    return m_AvailableWindowTypes ??(
        m_AvailableWindowTypes =
            TypeCache.GetTypesDerivedFrom(typeof(PlayModeView))
                .OrderBy(GetWindowTitle)
                .ToDictionary(t => t, GetWindowTitle));
  }
  ...
  private string GetTypeName() { return GetType().ToString(); }

  private Dictionary<string, string> ListsToDictionary(List<string> keys,
                                                       List<string> values) {
    var dict = keys.Select((key, val) => new {key, val = values[val]})
  ...
  protected void SwapMainWindow(Type type) {
    if (type.BaseType != typeof(PlayModeView))
      throw new ArgumentException("Type should derive from " +
                                  typeof(PlayModeView).Name);
    if (type.Name != GetType().Name) {
      var serializedViews =
          ListsToDictionary(m_SerializedViewNames, m_SerializedViewValues);
      ...
      var serializedViewPath =
          Path.GetFullPath(Path.Combine(m_ViewsCache, guid.ToString()));
      if (!Directory.Exists(m_ViewsCache))
        Directory.CreateDirectory(m_ViewsCache);

      InternalEditorUtility.SaveToSerializedFileAndForget(
          new[]{this}, serializedViewPath, true);
      ...
      if (serializedViews.ContainsKey(type.ToString())) {
        ...
        if (File.Exists(path)) {
          window = InternalEditorUtility.LoadSerializedFileAndForget(
              path)[0] as PlayModeView;
          File.Delete(path);
        }
      }
  ...
  private void ConfigureTargetTexture(int width, int height, bool clearTexture,
                                      string name, bool hdr) {
    ...
    if (m_TargetTexture &&
        (m_CurrentColorSpace != QualitySettings.activeColorSpace ||
         m_TargetTexture.useMipMap != m_UseMipMap ||
         m_TargetTexture.graphicsFormat != format)) {
  ...
  internal static PlayModeView GetRenderingView() { return s_RenderingView; }
  ...
  internal Vector2 GetPlayModeViewSize() { return targetSize; }
  ...
  public bool IsShowingGizmos() { return showGizmos; }
  ...
  protected void SetVSync(bool enable) { m_Parent.EnableVSync(enable); }
