...
internal class PopupList : PopupWindowContent {
  ...
  static EditorGUI.RecycledTextEditor s_RecycledEditor =
      new EditorGUI.RecycledTextEditor();
  ...
  public class ListElement {
    ...
    public float filterScore {
      get { return m_WasSelected ? float.MaxValue : m_FilterScore; }
      set {
        m_FilterScore = value;
        ResetScore();
      }
  ...
  public class InputData {
    ...
    public InputData() { m_ListElements = new List<ListElement>(); }
    ...
    public virtual IEnumerable<ListElement> BuildQuery(string prefix) {
      if (prefix == "")
        ...
        return m_ListElements.Where(
            element => element.m_Content.text.StartsWith(
    ...
    public IEnumerable<ListElement> GetFilteredList(string prefix) {
      ...
      if (m_MaxCount > 0)
        res = res.OrderByDescending(element => element.filterScore)
                  .Take(m_MaxCount);
  ...
  private class Styles {
    ...
    public Styles() {
      ...
      customTextFieldCancelButton =
          new GUIStyle(EditorStyles.toolbarSearchFieldCancelButton);
      customTextFieldCancelButtonEmpty =
          new GUIStyle(EditorStyles.toolbarSearchFieldCancelButtonEmpty);
    }
  }
  ...
  public virtual float GetWindowHeight() {
    int count = (m_Data.m_MaxCount == 0)
                    ? m_Data.GetFilteredCount(m_EnteredText)
                    : m_Data.m_MaxCount;
    return count * k_LineHeight + 2 * k_Margin +
           (m_Data.m_AllowCustom ? k_TextFieldHeight : 0);
  }

  public virtual float GetWindowWidth() { return 150f; }
  ...
  public override void OnGUI(Rect windowRect) {
    ...
    if (evt.type == EventType.Repaint)
      s_Styles.background.Draw(new Rect(windowRect.x, windowRect.y,
  ...
  private void DrawCustomTextField(EditorWindow editorWindow, Rect windowRect) {
    ...
    if (evt.type == EventType.KeyDown) {
      ...
      case KeyCode.Return:
        if (textBeforeEdit != "") {
          ...
          if (m_Data.m_OnSelectCallback != null)
            m_Data.m_OnSelectCallback(
                m_Data.NewOrMatchingElement(textBeforeEdit));

          if (evt.keyCode == KeyCode.Tab || evt.keyCode == KeyCode.Comma)
            clearText = true; // to ease multiple entries (it is unlikely that
                              // the same filter is used more than once)
      ...
      case KeyCode.Backspace:
        ...
        // Don't use the event yet, so the textfield below can get it and delete
        // the selection
        break;
    ...
    // Draw textfield
    {
      bool dummy = false;
      Rect pos = new Rect(
          windowRect.x + k_Margin / 2,
          windowRect.y +
              (m_Gravity == Gravity.Top ?(k_Margin / 2)
               : (windowRect.height - k_TextFieldHeight - k_Margin / 2)),
          windowRect.width - k_Margin - 14, k_TextFieldHeight);

      GUI.SetNextControlName(s_TextFieldName);
      EditorGUI.FocusTextInControl(s_TextFieldName);
      int id = EditorGUIUtility.GetControlID(s_TextFieldHash,
                                             FocusType.Keyboard, pos);

      if (useEventBeforeTextField)
        evt.Use(); // We have to delay this until after we get the control id,
                   // otherwise the id we get is just -1

      if (GUIUtility.keyboardControl == 0)
        GUIUtility.keyboardControl = id;

      textFieldText = EditorGUI.DoTextField(
          s_RecycledEditor, id, pos, textBeforeEdit, s_Styles.customTextField,
          null, out dummy, false, false, false);
      Rect buttonRect = pos;
      ...
      if ((GUI.Button(buttonRect, GUIContent.none,
                      textFieldText != ""
                          ? s_Styles.customTextFieldCancelButton
    ...
    if (textBeforeEdit != textFieldText) {
      m_EnteredText =
          (0 <= s_RecycledEditor.cursorIndex &&
           s_RecycledEditor.cursorIndex < textFieldText.Length)
              ? textFieldText.Substring(0, s_RecycledEditor.cursorIndex)
              : textFieldText;
  ...
  private string CurrentDisplayedText() {
    return m_EnteredTextCompletion != "" ? m_EnteredTextCompletion
                                         : m_EnteredText;
  }
  ...
  private void UpdateCompletion() {
    ...
    IEnumerable<string> query =
        m_Data.GetFilteredList(m_EnteredText).Select(element => element.text);

    if (m_EnteredTextCompletion != "" &&
        m_EnteredTextCompletion.StartsWith(
            m_EnteredText, System.StringComparison.OrdinalIgnoreCase)) {
      m_SelectedCompletionIndex =
          query.TakeWhile(element => element != m_EnteredTextCompletion)
              .Count();
      ...
      m_EnteredTextCompletion = query.Skip(m_SelectedCompletionIndex)
                                    .DefaultIfEmpty("")
                                    .FirstOrDefault();
    }
  ...
  private void ChangeSelectedCompletion(int change) {
    ...
    if (m_SelectedCompletionIndex == -1 &&
        change < 0) // specal case for initial selection
      m_SelectedCompletionIndex = count;

    int index =
        count > 0 ? (m_SelectedCompletionIndex + change + count) % count : 0;
  ...
  private void AdjustRecycledEditorSelectionToCompletion() {
    if (m_EnteredTextCompletion != "") {
      ...
      s_RecycledEditor.selectIndex =
          m_EnteredTextCompletion
              .Length; // the selection goes from s_RecycledEditor.cursorIndex
                       // (already set by DoTextField) to
                       // s_RecycledEditor.selectIndex
    }
  }
  ...
  private void DrawList(EditorWindow editorWindow, Rect windowRect) {
    ...
    foreach (var element in m_Data.GetFilteredList(m_EnteredText)) {
      i++;
      Rect rect = new Rect(windowRect.x,
                           windowRect.y + k_Margin + i * k_LineHeight +
                               (m_Gravity == Gravity.Top &&
                                m_Data.m_AllowCustom ? k_TextFieldHeight : 0),
                           windowRect.width, k_LineHeight);

      switch (evt.type) {
      case EventType.Repaint: {
        GUIStyle style =
            element.partiallySelected ? s_Styles.menuItemMixed
            : s_Styles.menuItem;
      ...
      case EventType.MouseDown: {
        if (Event.current.button == 0 &&
            rect.Contains(Event.current.mousePosition) && element.enabled) {
          // Toggle state
          if (m_Data.m_OnSelectCallback != null)
