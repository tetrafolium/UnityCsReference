...
namespace UnityEditor.SceneManagement {
  ...
  public sealed partial class EditorSceneManager : SceneManager {
    ...
    internal static bool CreateSceneAsset(string scenePath,
                                          bool createDefaultGameObjects) {
      if (!Utils.Paths.CheckValidAssetPathAndThatDirectoryExists(scenePath,
                                                                 ".unity"))
        return false;
    ...
    public static bool
    SaveScene(Scene scene, [ uei.DefaultValue("\"\"") ] string dstScenePath,
              [ uei.DefaultValue("false") ] bool saveAsCopy) {
      if (!string.IsNullOrEmpty(dstScenePath))
        if (!Utils.Paths.CheckValidAssetPathAndThatDirectoryExists(dstScenePath,
                                                                   ".unity"))
          return false;
    ...
    private static void Internal_NewSceneWasCreated(Scene scene,
                                                    NewSceneMode mode) {
      if (sceneWasCreated != null)
        sceneWasCreated(scene, mode);
    }

    private static void Internal_SceneWasOpened(Scene scene,
                                                OpenSceneMode mode) {
    ...
    private static void
    Internal_ActiveSceneChangedInEditor(Scene previousActiveScene,
    ...
    public static Scene LoadSceneInPlayMode(string path,
    ...
    LoadSceneAsyncInPlayMode(string path, LoadSceneParameters parameters) {
