...
[UsedImplicitly, ExcludeFromPreset,
 ScriptedImporter(version : 1, ext
                  : "mode")]
class ModeDescriptorImporter : ScriptedImporter {
  internal static bool allowExplicitModeRefresh {
    get;
    set;
  }
...
public static class ModeService {
  ...
  public static string currentId =>
      currentIndex == -1 ? k_DefaultModeId : modes[currentIndex].id;
  public static int currentIndex {
    get;
    private set;
  }
  private static ModeEntry[] modes {
    get;
    set;
  }
  = new ModeEntry[0];
  internal static bool hasSwitchableModes {
    get;
    private set;
  }
  ...
  internal static bool HasCapability(ModeCapability capability,
                                     bool defaultValue = false) {
    return HasCapability(currentIndex, capability, defaultValue);
  }

  internal static bool HasCapability(string capabilityName,
                                     bool defaultValue = false) {
    return HasCapability(currentIndex, capabilityName, defaultValue);
  }

  internal static bool HasCapability(int modeIndex, ModeCapability capability,
                                     bool defaultValue = false) {
    return HasCapability(modeIndex, capability.ToString().ToSnakeCase(),
                         defaultValue);
  }

  internal static bool HasCapability(int modeIndex, string capabilityName,
                                     bool defaultValue = false) {
    var lcCapabilityName = capabilityName.ToLower();
    var capabilities =
        GetModeDataSection(modeIndex, ModeDescriptor.CapabilitiesKey)
            as JSONObject;
    if (capabilities == null)
      return defaultValue;

    if (!capabilities.Contains(lcCapabilityName) ||
        capabilities [lcCapabilityName]
                .GetType() != typeof(bool))
      return defaultValue;
  ...
  internal static bool Execute(ModeAction builtinAction, CommandHint hint,
                               params object[] args) {
  ...
  internal static bool Execute(string actionName, CommandHint hint,
                               params object[] args) {
    // Call some command/shortcut actions to execute the current action
    actionName = actionName.ToLower();
    var executeHandlers =
        GetModeDataSection(currentIndex, ModeDescriptor.ExecuteHandlersKey)
            as JSONObject;
  ...
  internal static IEnumerable<T> GetModeDataSectionList<T>(int modeIndex,
                                                           string sectionName) {
  ...
  internal static void RaiseModeChanged(int prevIndex, int nextIndex) {
    modeChanged?.Invoke(
        new ModeChangedArgs{prevIndex = prevIndex, nextIndex = nextIndex});
  ...
  internal static string GetDefaultModeLayout(string modeId = null) {
    var layouts = GetModeDataSection(currentIndex, ModeDescriptor.LayoutsKey)
        as IList<object>;
    if (layouts != null && layouts.Count > 0) {
      ...
      if (layoutPath != null) {
        if (File.Exists(layoutPath)) {
          ...
          Debug.LogWarning("Default Mode Layout: " + layoutPath +
                           " doesn't exists.");
        }
      }
    }
  ...
  private static void LoadModes(bool checkStartupMode = false) {
    ...
    if (checkStartupMode && HasStartupMode()) {
      ...
      if (modeIndex != -1) {
        ...
        Console.WriteLine(
            $"[MODES] Loading editor mode {modeNames[currentModeIndex]} ({currentModeIndex}) from command line.");
      }
    }
  ...
  private static void FillModeData(string path,
                                   Dictionary<string, object> modesData) {
    try {
      ...
      foreach (var rawModeId in json.Keys) {
        ...
        if (IsValidModeId(modeId)) {
          ...
          if (modesData.TryGetValue(modeId, out modeData))
            modesData[modeId] = JsonUtils.DeepMerge(modeData as JSONObject,
                                                    json[modeId] as JSONObject);
          else
            modesData[modeId] = json[modeId];
        } else {
          Debug.LogWarning(
              $"Invalid Mode Id: {modeId} contains non alphanumeric characters.");
        }
      }
    } catch (Exception ex) {
      Debug.LogError(
          $"[ModeService] Error while parsing mode file {path}.\n{ex}");
    }
  }
  ...
  internal static void ScanModes() {
    var modesData = new Dictionary<string, object>{
            [k_DefaultModeId] =
                new Dictionary<string, object>{[ModeDescriptor.LabelKey] =
                                                   "Default"}};

    var builtinModeFile = Path.Combine(
        EditorApplication.applicationContentsPath, "Resources/default.mode");
    ...
    var modeDescriptors = AssetDatabase.EnumerateAllAssets(new SearchFilter{
        ...
        classNames = new[]{nameof(ModeDescriptor)}, showAllHits = true});
    ...
    modes [0]
    = CreateEntry(k_DefaultModeId, (JSONObject) modesData[k_DefaultModeId]);
    var modeIndex = 1;
  ...
  private static void SetModeIndex(int modeIndex) {
    ...
    var capabilities =
        GetModeDataSection(currentIndex, ModeDescriptor.CapabilitiesKey)
            as IDictionary;
    if (capabilities != null) {
  ...
  private static int LoadProjectPrefModeIndex() {
    ...
    Console.WriteLine(
        $"[MODES] Loading mode {modeNames[loadModeIndex]} ({loadModeIndex}) for {modePreyKeyName}");
  ...
  private static void SaveProjectPrefModeIndex(int modeIndex) {
    var modePreyKeyName = GetProjectPrefKeyName(k_ModeCurrentIdKeyName);
    Console.WriteLine(
        $"[MODES] Saving user mode to {modeNames[modeIndex]} ({modeIndex}) for {modePreyKeyName}");
  ...
  private static void LoadMenu(IList menus, string prefix = "",
                               int priority = 100) {
    ...
    foreach (var menuData in menus) {
      if (menuData != null) {
        ...
        if (!String.IsNullOrEmpty(platform) &&
            !Application.platform.ToString().ToLowerInvariant().StartsWith(
        ...
        if (menu.Contains(k_MenuKeyChildren)) {
          if (menu[k_MenuKeyChildren] is IList)
            LoadMenu(menu[k_MenuKeyChildren] as IList, fullMenuName + "/",
                     priority);
          else if (menu[k_MenuKeyChildren] is string &&
                   (string) menu[k_MenuKeyChildren] == "*") {
            var whitelistedItems = Menu.ExtractSubmenus(fullMenuName);
            var renamedTo = prefix + JsonUtils.JsonReadString(
                                         menu, k_MenuKeyRename, menuName);
            foreach (var wi in whitelistedItems)
              Menu.AddExistingMenuItem(wi.Replace(fullMenuName, renamedTo), wi,
                                       hasExplicitPriority ? priority
                                       : -1);
          }
        } else {
          ...
          if (String.IsNullOrEmpty(commandId)) {
            ...
            var menuItemId =
                JsonUtils.JsonReadString(menu, k_MenuKeyItemId, fullMenuName);
            ...
            var validateCommandId =
                JsonUtils.JsonReadString(menu, k_MenuKeyValidateCommandId);
            if (!String.IsNullOrEmpty(validateCommandId))
              validateHandler = () =>(bool) CommandService.Execute(
                  validateCommandId, CommandHint.Menu | CommandHint.Validate);

            Menu.AddMenuItem(
                fullMenuName, shortcut, @checked, priority,
                () => CommandService.Execute(commandId, CommandHint.Menu),
                validateHandler);
          }
        }
      } else {
  ...
  private static void OnModeChangeLayouts(ModeChangedArgs args) {
    // Prevent double loading the default/last layout already done by the
    // WindowLayout system.
    if (args.prevIndex == -1)
      return;
    ...
    if (HasCapability(ModeCapability.LayoutSwitching, true)) {
      ...
      try {
        if (args.nextIndex != 0 || args.prevIndex == -1 ||
            HasCapability(args.prevIndex, ModeCapability.LayoutSwitching,
                          true)) {
  ...
  static class JsonUtils {
    public static string JsonReadString(JSONObject data, string fieldName,
                                        string defaultValue = "") {
    ...
    public static int JsonReadInt(JSONObject data, string fieldName,
                                  int defaultValue = 0) {
      if (!data.Contains(fieldName))
        return defaultValue;
    ...
    public static bool JsonReadBoolean(JSONObject data, string fieldName,
                                       bool defaultValue = false) {
      if (!data.Contains(fieldName))
        return defaultValue;
    ...
    public static Dictionary<string, object> DeepMerge(JSONObject first,
                                                       JSONObject second) {
      if (first == null)
    ...
    private static void DeepMergeInto(JSONObject destination,
                                      JSONObject source) {
      if (destination == null)
      ...
      foreach (var secondItem in secondArray) {
        ...
        if (secondItemObject != null) {
          string secondItemId = secondItemObject.Contains("id")
                                    ...
                                    : (secondItemObject.Contains("name")
                                           ? secondItemObject["name"] as string
                                           : null);
          if (!String.IsNullOrEmpty(secondItemId)) {
            ...
            foreach (var firstItem in firstArray) {
              ...
              string firstItemId =
                  firstItemObject.Contains("id")
                      ? firstItemObject["id"] as string
                      : (firstItemObject.Contains("name")
                             ? firstItemObject["name"] as string
                             : null);

              if (String.Equals(firstItemId, secondItemId,
