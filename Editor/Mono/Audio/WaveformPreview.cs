...
class WaveformPreview : IDisposable {
  ...
  public enum ChannelMode { MonoSum, Separate, SpecificChannel }
  ...
  protected WaveformPreview(UnityEngine.Object presentedObject,
                            int samplesAndWidth, int channels) {
  ...
  protected internal WaveformPreview(UnityEngine.Object presentedObject,
                                     int samplesAndWidth, int channels,
                                     bool deferTextureCreation) {
    this.presentedObject = presentedObject;
    optimized = true;
    m_Samples = samplesAndWidth;
    m_Channels = channels;
  ...
  public void Render(Rect rect) {
    if (s_Material == null) {
      s_Material = EditorGUIUtility.LoadRequired(
          "Previews/PreviewAudioClipWaveform.mat") as Material;
    }
  ...
  public bool ApplyModifications() {
    if (m_Dirty || m_Texture == null) {
      m_Flags |= UpdateTexture((int) m_Size.x, m_Channels)
                     ? MessageFlags.TextureChanged
                     : MessageFlags.None;
  ...
  public void SetChannelMode(ChannelMode mode) { SetChannelMode(mode, 0); }
  ...
  bool UpdateTexture(int width, int channels) {
    ...
    Action<bool> updateTexture = clear => {
      if (m_Texture == null) {
        m_Texture = new Texture2D(s_BaseTextureWidth, textureHeight,
