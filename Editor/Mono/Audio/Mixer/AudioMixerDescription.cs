...
namespace UnityEditor.Audio {
  internal class MixerEffectDefinition {
    public MixerEffectDefinition(string name,
                                 MixerParameterDefinition[] parameters) {
  ...
  static class MixerEffectDefinitionReloader {
    // We use this class with InitializeOnLoad attribute for ensuring
    // MixerEffectDefinitions are refreshed when needed: 1) At startup, 2) after
    // script recompile and 3) when project changes (new effects can have been
    // added)
    ...
    static void OnProjectChanged() { MixerEffectDefinitions.Refresh(); }
  }
  ...
  internal sealed partial class MixerEffectDefinitions {
    public static void Refresh() {
      ...
      duckVolDef[1] = new MixerParameterDefinition{
          ...
          defaultValue = 2.0f,
          description =
              "Ratio of compression applied when side-chain signal exceeds threshold"};
      duckVolDef[2] = new MixerParameterDefinition{
          name = "Attack Time",   units = "ms",
          displayScale = 1000.0f, displayExponent = 3.0f,
          minRange = 0.0f,        maxRange = 10.0f,
          defaultValue = 0.1f,    description = "Level detector attack time"};
      duckVolDef[3] = new MixerParameterDefinition{
          name = "Release Time",  units = "ms",
          displayScale = 1000.0f, displayExponent = 3.0f,
          minRange = 0.0f,        maxRange = 10.0f,
          defaultValue = 0.1f,    description = "Level detector release time"};
      duckVolDef[4] = new MixerParameterDefinition{
          name = "Make-up Gain", units = "dB",
          displayScale = 1.0f,   displayExponent = 1.0f,
          minRange = -80.0f,     maxRange = 40.0f,
          defaultValue = 0.0f,   description = "Make-up gain"};
      ...
      duckVolDef[6] = new MixerParameterDefinition{
          ...
          defaultValue = 1.0f,
          description =
              "Sidechain/source mix. If set to 100% the compressor detects level entirely from sidechain signal."};
      ...
      foreach (var effectName in effectNames) {
        MixerParameterDefinition[] paramDesc =
            GetAudioEffectParameterDesc(effectName);
    ...
    public static MixerParameterDefinition[] GetEffectParameters(
        string effect) {
      foreach (MixerEffectDefinition definition in s_MixerEffectDefinitions) {
    ...
    RegisterAudioMixerEffect(string name,
                             MixerParameterDefinition[] definitions) {
      foreach (MixerEffectDefinition definition in s_MixerEffectDefinitions) {
        if (definition.name == name) {
      ...
      MixerEffectDefinition newDefinition =
          new MixerEffectDefinition(name, definitions);
      s_MixerEffectDefinitions.Add(newDefinition);

      // Wasteful - Clears the runtime representation each time a new effect is
      // added and rebuilds all runtime representations.
      ClearDefinitionsRuntime();
      foreach (MixerEffectDefinition definition in s_MixerEffectDefinitions) {
    ...
    private static readonly List<MixerEffectDefinition>
        s_MixerEffectDefinitions = new List<MixerEffectDefinition>();
  }
}
