...
internal class AudioMixerItem : TreeViewItem {
  ...
  bool lastSuspendedState {
    get;
    set;
  }
  const string kSuspendedText = " - Inactive";

  public AudioMixerItem(int id, int depth, TreeViewItem parent,
                        string displayName, AudioMixerController mixer,
                        string infoText)
...
internal class AudioMixerTreeViewDragging : TreeViewDragging {
  ...
  Action<List<AudioMixerController>, AudioMixerController>
  ...
  public AudioMixerTreeViewDragging(
      TreeViewController treeView,
      Action<List<AudioMixerController>, AudioMixerController>
          mixerDroppedOnMixerCallback)
      : base(treeView) {
    m_MixersDroppedOnMixerCallback = mixerDroppedOnMixerCallback;
  }

  public override void StartDrag(TreeViewItem draggedNode,
                                 List<int> draggedNodes) {
    ...
    DragAndDrop.SetGenericData(k_AudioMixerDraggingID,
                               new DragData(draggedItems));
    string title = draggedNodes.Count + " AudioMixer" +
                   (draggedNodes.Count > 1
                        ? "s"
                        : ""); // title is only shown on OSX (at the cursor)
    DragAndDrop.StartDrag(title);
  }

  public override bool DragElement(TreeViewItem targetItem, Rect targetItemRect,
                                   int row) {
    // First ensure we are dragging AudioMixers (and not some other objects)
    var dragData =
        DragAndDrop.GetGenericData(k_AudioMixerDraggingID) as DragData;
    if (dragData == null) {
      DragAndDrop.visualMode = DragAndDropVisualMode.None;
      return false;
    }

    // Handle the case where we hover inside the treeview rect but not over any
    // items
    bool isDraggingOverEmptyArea = targetItem == null;
    if (isDraggingOverEmptyArea &&
        m_TreeView.GetTotalRect().Contains(Event.current.mousePosition)) {
      if (m_DropData != null) {
  ...
  public override DragAndDropVisualMode DoDrag(TreeViewItem parentNode,
                                               TreeViewItem targetNode,
                                               bool perform,
                                               DropPosition dragPos) {
    var dragData =
        DragAndDrop.GetGenericData(k_AudioMixerDraggingID) as DragData;
    if (dragData == null)
    ...
    if (parentMixerItem != null && dragData != null) {
      List<AudioMixerGroupController> draggedMasterGroups =
          (from i in draggedItems select i.mixer.masterGroup).ToList();
      var allGroups = parentMixerItem.mixer.GetAllAudioGroupsSlow();
      bool causesFeedback =
          AudioMixerController.WillModificationOfTopologyCauseFeedback(
              allGroups, draggedMasterGroups, parentMixerItem.mixer.masterGroup,
              null);
      bool validDrag = ValidDrag(parentNode, draggedItems) && !causesFeedback;
      if (perform && validDrag) {
        if (m_MixersDroppedOnMixerCallback != null)
          m_MixersDroppedOnMixerCallback(GetAudioMixersFromItems(draggedItems),
                                         parentMixerItem.mixer);
      }
      return validDrag ? DragAndDropVisualMode.Move
  ...
  private List<AudioMixerItem>
  GetAudioMixerItemsFromIDs(List<int> draggedMixers) {
    var found = TreeViewUtility.FindItemsInList(draggedMixers,
                                                m_TreeView.data.GetRows());
    return found.OfType<AudioMixerItem>().ToList();
  }

  private List<AudioMixerController>
  GetAudioMixersFromItems(List<AudioMixerItem> draggedItems) {
...
internal class AudioMixersDataSource : TreeViewDataSource {
  ...
  public AudioMixersDataSource(
      TreeViewController treeView,
      Func<List<AudioMixerController>> getAllControllersCallback)
      : base(treeView) {
    showRootItem = false;
  ...
  public override void FetchData() {
    int depth = -1;
    bool expandAll =
        m_TreeView.state.expandedIDs.Count ==
        0; // State is persisted so only do this once per new AudioMixerWindow
    ...
    if (m_Mixers.Count > 0) {
      ...
      var roots = m_Mixers
                      .Select(mixer => new AudioMixerItem(
                                  mixer.GetInstanceID(), 0, m_RootItem,
                                  mixer.name, mixer, GetInfoText(mixer)))
                      .ToList();
      ...
      SetItemDepthRecursive(m_RootItem,
                            -1); // -1 because the root item is hidden
  ...
  static string GetInfoText(AudioMixerController controller) {
    ...
    if (controller.outputAudioMixerGroup != null)
      s = string.Format("({0} of {1})", controller.outputAudioMixerGroup.name,
                        controller.outputAudioMixerGroup.audioMixer.name);
    else
      s = "(Audio Listener)";
    return s;
  }
  ...
  void SetChildParentOfMixerItem(AudioMixerItem item,
                                 List<AudioMixerItem> items) {
    if (item.mixer.outputAudioMixerGroup != null) {
      var parentMixer = item.mixer.outputAudioMixerGroup.audioMixer;
      var parentItem = TreeViewUtility.FindItemInList(
          parentMixer.GetInstanceID(), items) as AudioMixerItem;
      if (parentItem != null) {
  ...
  public override bool IsRenamingItemAllowed(TreeViewItem item) { return true; }

  public int GetInsertAfterItemIDForNewItem(string newName,
    ...
    for (int idx = 0; idx < parentItem.children.Count; ++idx) {
      ...
      if (EditorUtility.NaturalCompare(
              Path.GetFileNameWithoutExtension(propertyPath), newName) > 0)
        break;
  ...
  override public void InsertFakeItem(int id, int parentID, string name,
                                      Texture2D icon) {
    TreeViewItem checkItem = FindItem(id);
    if (checkItem != null) {
      Debug.LogError("Cannot insert fake Item because id is not unique " + id +
                     " Item already there: " + checkItem.displayName);
      return;
    }
...
internal class AudioMixersTreeViewGUI : TreeViewGUI {
  ...
  protected override void OnContentGUI(Rect rect, int row, TreeViewItem item,
    ...
    // Draw text
    lineStyle.padding.left =
        (int)(k_IconWidth + iconTotalPadding + k_SpaceBetweenIconAndText);
    ...
    if (mixerItem.labelWidth <= 0)
      mixerItem.labelWidth =
          lineStyle.CalcSize(GUIContent.Temp(label)).x; // only calc once
    Rect infoRect = rect;
  ...
  protected CreateAssetUtility GetCreateAssetUtility() {
    return ((TreeViewStateWithAssetUtility) m_TreeView.state)
        .createAssetUtility;
  }
  ...
  AudioMixerItem GetSelectedItem() {
    return m_TreeView.FindItem(m_TreeView.GetSelection().FirstOrDefault())
        as AudioMixerItem;
  }

  override protected void SyncFakeItem() {
    if (!m_TreeView.data.HasFakeItem() &&
        GetCreateAssetUtility().IsCreatingNewAsset()) {
      // Get selection from tree
      int parentInstanceID = m_TreeView.data.root.id;
      var selectedItem = GetSelectedItem();
      if (selectedItem != null)
        parentInstanceID = selectedItem.parent.id;
      m_TreeView.data.InsertFakeItem(
          GetCreateAssetUtility().instanceID, parentInstanceID,
          GetCreateAssetUtility().originalName, GetCreateAssetUtility().icon);
    }
  ...
  public void BeginCreateNewMixer() {
    // int instanceID, EndNameEditAction endAction, string pathName, Texture2D
    // icon, string resourceFile
    ClearRenameAndNewItemState();

    // Use resouce file to store instanceID for output group so we can use that
    // for the newly created audiomixer
    string resourceFileData = string.Empty;
    var selectedItem = GetSelectedItem();
    if (selectedItem != null &&
        selectedItem.mixer.outputAudioMixerGroup != null)
      resourceFileData =
          selectedItem.mixer.outputAudioMixerGroup.GetInstanceID().ToString();

    int instanceID = 0;

    if (GetCreateAssetUtility().BeginNewAssetCreation(
            instanceID, ScriptableObject.CreateInstance<DoCreateAudioMixer>(),
            "NewAudioMixer.mixer", null, resourceFileData)) {
      SyncFakeItem();

      // Start naming the asset
      bool renameStarted = GetRenameOverlay().BeginRename(
          GetCreateAssetUtility().originalName, instanceID, 0f);
      if (!renameStarted)
...
internal class AudioMixersTreeView {
  ...
  class Styles {
    public GUIContent header = EditorGUIUtility.TrTextContent(
        "Mixers",
        "All mixers in the project are shown here. By default, a mixer outputs to the AudioListener but mixers can also route their output to other mixers. Each mixer shows where it outputs (in parenthesis). To reroute a mixer simply drag the mixer upon another mixer and select a group from the popup.");
    public GUIContent addButton = EditorGUIUtility.TrIconContent(
        "CreateAddNew",
        "Add mixer asset. The asset will be saved in the same folder as the current selected mixer or, if none is selected, saved in the Assets folder.");
    public Texture2D audioMixerIcon =
        EditorGUIUtility.FindTexture(typeof(AudioMixerController));
  }
  static Styles s_Styles;

  public AudioMixersTreeView(
      AudioMixerWindow mixerWindow, TreeViewState treeState,
      Func<List<AudioMixerController>> getAllControllersCallback) {
    ...
    var treeViewDataSource =
        new AudioMixersDataSource(m_TreeView, getAllControllersCallback);
    var treeViewDragging = new AudioMixerTreeViewDragging(
        m_TreeView, OnMixersDroppedOnMixerCallback);
    m_TreeView.Init(mixerWindow.position, treeViewDataSource, treeViewGUI,
                    treeViewDragging);
    m_TreeView.ReloadData();
  }
  ...
  public void DeleteAudioMixerCallback(object obj) {
    ...
    if (controller != null) {
      ProjectWindowUtil.DeleteAssets(new[]{controller.GetInstanceID()}.ToList(),
                                     true);
    }
  }
  ...
  public void OnTreeViewContextClick(int index) {
    ...
    if (node != null) {
      ...
      pm.AddItem(EditorGUIUtility.TrTextContent("Delete AudioMixer"), false,
                 DeleteAudioMixerCallback, node.mixer);
      pm.ShowAsContext();
    }
  }
  ...
  public float GetTotalHeight() {
    ...
    return AudioMixerDrawUtils.kSectionHeaderHeight +
           Mathf.Max(minHeight, m_TreeView.gui.GetTotalSize().y);
  }
  ...
  public void OnGUI(Rect rect) {
    ...
    AudioMixerDrawUtils.HeaderLabel(headerRect, s_Styles.header,
                                    s_Styles.audioMixerIcon);

    if (GUI.Button(new Rect(headerRect.xMax - 17f, headerRect.y + 3f, 16f, 16f),
                   s_Styles.addButton, EditorStyles.iconButton)) {
    ...
    if (m_TreeView.data.rowCount == 0) {
      using(new EditorGUI.DisabledScope(true)) {
        GUI.Label(new RectOffset(-20, 0, -2, 0).Add(contentRect),
                  "No mixers found");
      }
    }

    AudioMixerDrawUtils.DrawScrollDropShadow(contentRect,
                                             m_TreeView.state.scrollPos.y,
                                             m_TreeView.gui.GetTotalSize().y);
  ...
  void HandleCommandEvents(int treeViewKeyboardControlID) {
    ...
    if (eventType == EventType.ExecuteCommand ||
      ...
      if (Event.current.commandName == EventCommandNames.Delete ||
        ...
        Event.current.Use();
        if (execute)
          ProjectWindowUtil.DeleteAssets(m_TreeView.GetSelection().ToList(),
                                         true);
  ...
  public void EndRenaming() { m_TreeView.EndNameEditing(true); }

  public void OnUndoRedoPerformed() { ReloadTree(); }

  void OnMixersDroppedOnMixerCallback(List<AudioMixerController> draggedMixers,
                                      AudioMixerController droppedUponMixer) {
    // Set Unity selection when drag ended (when dragging non selected items we
    // just render them as selected while dragging)
    int[] draggedIDs =
        (from i in draggedMixers select i.GetInstanceID()).ToArray();
    ...
    if (droppedUponMixer == null) {
      ...
      Undo.RecordObjects(draggedMixers.ToArray(),
                         "Set output group for mixer" +
                             (draggedMixers.Count > 1 ? "s" : ""));
      ...
      Object startSelection = draggedMixers.Count == 1
                                  ? draggedMixers[0].outputAudioMixerGroup
                                  : null;
      ObjectSelector.get.Show(
          startSelection, typeof(AudioMixerGroup), null, false,
          new List<int>(){droppedUponMixer.GetInstanceID()});
      ObjectSelector.get.objectSelectorID = kObjectSelectorID;
      ObjectSelector.get.titleContent =
          EditorGUIUtility.TrTextContent("Select Output Audio Mixer Group");
      GUIUtility.ExitGUI();
    }
  }
  ...
  void HandleObjectSelectorResult() {
    ...
    if (evt.type == EventType.ExecuteCommand) {
      ...
      if (commandName == ObjectSelector.ObjectSelectorUpdatedCommand &&
          ObjectSelector.get.objectSelectorID == kObjectSelectorID) {
        ...
        AudioMixerGroup selectedGroup =
            selected != null ? selected as AudioMixerGroup : null;
        Undo.RecordObjects(m_DraggedMixers.ToArray(),
                           "Set output group for mixer" +
                               (m_DraggedMixers.Count > 1 ? "s" : ""));
        ...
        int[] selectedIDs =
            (from i in m_DraggedMixers select i.GetInstanceID()).ToArray();
