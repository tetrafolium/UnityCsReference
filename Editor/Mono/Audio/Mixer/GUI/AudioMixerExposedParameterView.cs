...
class AudioMixerExposedParameterView {
  ...
  private float height {
    get { return m_ReorderableListWithRenameAndScrollView.list.GetHeight(); }
  }

  public AudioMixerExposedParameterView(
  ...
  public void OnMixerControllerChanged(AudioMixerController controller) {
    ...
    if (m_Controller)
      m_Controller.ChangedExposedParameter +=
          new ChangedExposedParameterHandler(RecreateListControl);
  ...
  public void RecreateListControl() {
    if (m_Controller != null) {
      ...
      var exposedParams =
          m_ControllerSerialized.FindProperty("m_ExposedParameters");
      System.Diagnostics.Debug.Assert(exposedParams != null);

      ReorderableList reorderableList = new ReorderableList(
          m_ControllerSerialized, exposedParams, false, false, false, false);
  ...
  public void OnContextClick(int itemIndex) {
    ...
    pm.AddItem(EditorGUIUtility.TrTextContent("Unexpose"), false,
               delegate(object data) { Delete((int) data); }, itemIndex);

    pm.AddItem(
        EditorGUIUtility.TrTextContent("Rename"), false, delegate(object data) {
          m_ReorderableListWithRenameAndScrollView.BeginRename((int) data, 0f);
        }, itemIndex);
  ...
  void DrawElement(Rect rect, int index, bool isActive, bool isFocused) {
    ...
    // The left side of the text is drawn by the reorderable list, now draw the
    // additional right aligned text
    using(new EditorGUI.DisabledScope(true)) {
      m_ReorderableListWithRenameAndScrollView.elementStyleRightAligned.Draw(
          rect, GetInfoString(index), false, false, false, false);
    }
  }

  public Vector2 CalcSize() {
    if (m_ReorderableListWithRenameAndScrollView.list.count !=
        m_Controller.exposedParameters.Length) {
      RecreateListControl();
    }
    float maxWidth = 0;
    for (int index = 0;
         index < m_ReorderableListWithRenameAndScrollView.list.count; index++) {
      var width = WidthOfRow(
          index, m_ReorderableListWithRenameAndScrollView.elementStyle,
          m_ReorderableListWithRenameAndScrollView.elementStyleRightAligned);
      if (width > maxWidth)
        maxWidth = width;
    }
  ...
  public void NameChanged(int index, string newName) {
    ...
    if (newName.Length > kMaxNameLength) {
      ...
      Debug.LogWarning("Maximum name length of an exposed parameter is " +
                       kMaxNameLength + " characters. Name truncated to '" +
                       newName + "'");
    }
  ...
  void Delete(int index) {
    if (m_Controller != null) {
      ...
      ExposedAudioParameter exposedParam =
          m_Controller.exposedParameters[index];
  ...
  public void OnEvent() { m_ReorderableListWithRenameAndScrollView.OnEvent(); }
}
}
