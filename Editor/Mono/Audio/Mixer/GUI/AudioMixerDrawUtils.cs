...
internal class AudioMixerDrawUtils {
  // OpenGL and D3D<11 use different ways of specifying the rasterization grid,
  // so in order to draw exactly we need to account for this.
  ...
  private static void DetectVertexOffset() {
    ...
    // vertexOffset = 0.5f; //(driver.StartsWith("OpenGL") ||
    // driver.StartsWith("Direct3D 11")) ? 0.0f : 0.5f; vertexOffset /=
    // EditorGUIUtility.pixelsPerPoint;
    vertexOffset = 0.0f;
    // Debug.Log("Detected vertex offset as " + vertexOffset + " driver=" +
    // driver);
  }

  static readonly Color kAttenuationColor = AudioCurveRendering.kAudioOrange;
  static readonly Color kEffectColor =
      new Color(103 / 255f, 160 / 255f, 0 / 255f);
  static readonly Color kSendColor =
      new Color(0 / 255f, 147 / 255f, 170 / 255f);
  ...
  public class Styles {
    ...
    public GUIStyle channelStripAreaBackground =
        GetStyle("CurveEditorBackground"); //"flow background";
    public GUIStyle channelStripBg = GetStyle("ChannelStripBg");
    public GUIStyle duckingMarker = GetStyle("ChannelStripDuckingMarker");
    public GUIStyle channelStripAttenuationMarkerSquare =
        GetStyle("ChannelStripAttenuationMarkerSquare");
    public GUIStyle channelStripHeaderStyle =
        GetStyle("AM ChannelStripHeaderStyle");
    ...
    public Texture2D scrollShadowTexture =
        EditorGUIUtility.FindTexture("ScrollShadow");
    public Texture2D leftToRightShadowTexture =
        EditorGUIUtility.FindTexture("LeftToRightShadow");
    public GUIContent soloGUIContent = EditorGUIUtility.TrTextContent(
        "", "Adds this group to set of soloed groups");
    public GUIContent muteGUIContent =
        EditorGUIUtility.TrTextContent("", "Mutes this group");
    public GUIContent bypassGUIContent = EditorGUIUtility.TrTextContent(
        "", "Bypasses the effects on this group");
    public GUIContent effectSlotGUIContent = EditorGUIUtility.TrTextContent(
        "",
        "Drag horizontally to change wet mix levels or vertically to change order of effects. Note: Enable wet mixing in the context menu.");
    public GUIContent attenuationSlotGUIContent = EditorGUIUtility.TrTextContent(
        "",
        "Place the attenuation slot in the effect stack where attenuation should take effect");
    public GUIContent emptySendSlotGUIContent = EditorGUIUtility.TrTextContent(
        "", "Connect to a Receive in the context menu or in the inspector");
    public GUIContent returnSlotGUIContent =
        EditorGUIUtility.TrTextContent("", "Connect a Send to this Receive");
    public GUIContent duckVolumeSlotGUIContent = EditorGUIUtility.TrTextContent(
        "", "Connect a Send to this Duck Volume");
    public GUIContent duckingFaderGUIContent =
        EditorGUIUtility.TrTextContent("", "Ducking Fader");
    public GUIContent attenuationFader =
        EditorGUIUtility.TrTextContent("", "Attenuation fader");
    public GUIContent vuMeterGUIContent = EditorGUIUtility.TrTextContent(
        "",
        "The VU meter shows the current level of the mix of all sounds and subgroups.");
    public GUIContent referencedGroups = EditorGUIUtility.TrTextContent(
        "Referenced groups",
        "Mixer groups that are hidden but are referenced by the visible mixer groups are shown here for convenience");
  ...
  public static float GetAlpha() { return GUI.enabled ? 1.0f : 0.7f; }

  public static void DrawSplitter() {
    Rect r = GUILayoutUtility.GetRect(1, 1);
    if (Event.current.type == EventType.Repaint) {
      Color color =
          (EditorGUIUtility.isProSkin ? new Color(0.12f, 0.12f, 0.12f, 1.333f)
  ...
  // todo: 339 draw calls in our test case. Reduce it to fewer calls into this
  // function.
  ...
  public static void DrawRegionBg(Rect rect, out Rect headerRect,
                                  out Rect contentRect) {
    const float headerIndent = 2f;
    headerRect = new Rect(rect.x + headerIndent, rect.y,
                          rect.width - headerIndent, kSectionHeaderHeight);
    contentRect = new Rect(rect.x, headerRect.yMax, rect.width,
                           rect.height - kSectionHeaderHeight);
    GUI.Label(new RectOffset(1, 1, 1, 1).Add(contentRect), GUIContent.none,
  ...
  public static GUIStyle BuildGUIStyleForLabel(Color color, int fontSize,
                                               bool wrapText,
                                               FontStyle fontstyle,
                                               TextAnchor anchor) {
  ...
  public static void DrawConnection(Color col, float mixLevel, float srcX,
                                    float srcY, float dstX, float dstY,
                                    float width) {
    if (Event.current.type != EventType.Repaint)
      return;
    ...
    arrowPoints[1] =
        new Vector3(dstX - a * dirX + b * dirY, dstY - a * dirY - b * dirX);
    arrowPoints[2] =
        new Vector3(dstX - a * dirX - b * dirY, dstY - a * dirY + b * dirX);
    ...
    Handles.DrawAAPolyLine(width, new[]{triColor, triColor, triColor, triColor},
                           arrowPoints);

    Handles.DrawAAPolyLine(
        width,
        new[]{new Color(col.r, col.g, col.b, mixLevel),
  ...
  public static void DrawVerticalShow(Rect rect, bool fadeToTheRight) {
    Rect textureCoordRect = fadeToTheRight ? new Rect(0, 0, 1, 1)
        : new Rect(1, 1, -1, -1); // flip over X
    GUI.DrawTextureWithTexCoords(rect, styles.leftToRightShadowTexture,
                                 textureCoordRect);
  }

  public static void DrawScrollDropShadow(Rect scrollViewRect, float scrollY,
                                          float contentHeight) {
    if (Event.current.type == EventType.Repaint) {
      ...
      if (alpha > 0f)
        GUI.DrawTexture(new Rect(scrollViewRect.x, scrollViewRect.y,
                                 scrollViewRect.width, shadowHeightTop),
                        styles.scrollShadowTexture);
      if (alpha < 1f)
        GUI.color = orgColor;

      // Bottom shadow is shown if the scroll bar is visible
      float scrollAvailable =
          Mathf.Max(contentHeight - scrollViewRect.height, 0f);
      float alphaBottom = (scrollAvailable - scrollY) >
                          fadeDist ? 1f : (scrollAvailable - scrollY) / fadeDist;
      if (alphaBottom < 1f)
        GUI.color = new Color(1, 1, 1, alphaBottom);
      if (alphaBottom > 0f)
        GUI.DrawTextureWithTexCoords(
            new Rect(scrollViewRect.x, scrollViewRect.yMax - shadowHeightBottom,
                     scrollViewRect.width, shadowHeightBottom),
            styles.scrollShadowTexture, new Rect(1, 1, -1, -1)); // flip over Y
      if (alphaBottom < 1f)
        GUI.color = orgColor;
    }
  }
  ...
  public static void DrawRect(Rect rect, Color color) {
    Color orgColor = GUI.color;
    GUI.color = color;
    GUI.Label(rect, GUIContent.Temp(string.Empty),
              EditorGUIUtility
                  .whiteTextureStyle); // Using style rendering so that disabled
                                       // state is reflected in rendering
    GUI.color = orgColor;
  }
}
}
