...
internal class AudioMixerTreeViewNode : TreeViewItem {
  ...
  public AudioMixerTreeViewNode(int instanceID, int depth, TreeViewItem parent,
                                string displayName,
...
// Dragging
// We want dragging to work from the mixer window to the inspector like the
// project browser, but also want custom dragging behavior (reparent the group
// sub assets) so we derive from AssetOrGameObjectTreeViewDragging and override
// DoDrag.

internal class AudioGroupTreeViewDragging : AssetsTreeViewDragging {
  private AudioMixerGroupTreeView m_owner;

  public AudioGroupTreeViewDragging(TreeViewController treeView,
                                    AudioMixerGroupTreeView owner)
      : base(treeView) {
    m_owner = owner;
  }

  public override void StartDrag(TreeViewItem draggedItem,
                                 List<int> draggedItemIDs) {
  ...
  public override DragAndDropVisualMode DoDrag(TreeViewItem parentNode,
    ...
    var draggedGroups = new List<Object>(DragAndDrop.objectReferences)
                            .OfType<AudioMixerGroupController>()
                            .ToList();
    if (parentGroupNode != null && draggedGroups.Count > 0) {
      var draggedIDs =
          (from i in draggedGroups select i.GetInstanceID()).ToList();
      bool validDrag =
          ValidDrag(parentNode, draggedIDs) &&
          !AudioMixerController.WillModificationOfTopologyCauseFeedback(
              m_owner.Controller.GetAllAudioGroupsSlow(), draggedGroups,
              parentGroupNode.group, null);
      if (perform && validDrag) {
        ...
        m_owner.Controller.ReparentSelection(parentGroup, insertionIndex,
                                             draggedGroups);
        m_owner.ReloadTree();
        m_TreeView.SetSelection(
            draggedIDs.ToArray(),
            true); // Ensure dropped item(s) are selected and revealed (fixes
                   // selection if click dragging a single item that is not
                   // selected when drag was started)
      }
...
internal class AudioGroupDataSource : TreeViewDataSource {
  public AudioGroupDataSource(TreeViewController treeView,
                              AudioMixerController controller)
      : base(treeView) {
    m_Controller = controller;
  }

  private void AddNodesRecursively(AudioMixerGroupController group,
                                   TreeViewItem parent, int depth) {
    var children = new List<TreeViewItem>();
    for (int i = 0; i < group.children.Length; ++i) {
      int uniqueNodeID = GetUniqueNodeID(group.children[i]);
      var node =
          new AudioMixerTreeViewNode(uniqueNodeID, depth, parent,
                                     group.children[i].name, group.children[i]);
      node.parent = parent;
      children.Add(node);
  ...
  public override void FetchData() {
    ...
    m_RootItem = new AudioMixerTreeViewNode(uniqueNodeID, 0, null,
                                            m_Controller.masterGroup.name,
                                            m_Controller.masterGroup);
...
internal class AudioGroupTreeViewGUI : TreeViewGUI {
  ...
  readonly GUIContent k_VisibleON =
      EditorGUIUtility.TrIconContent("animationvisibilitytoggleon");
  readonly GUIContent k_VisibleOFF =
      EditorGUIUtility.TrIconContent("animationvisibilitytoggleoff");
  ...
  void OpenGroupContextMenu(AudioMixerTreeViewNode audioNode, bool visible) {
    ...
    if (NodeWasToggled != null) {
      menu.AddItem(new GUIContent(visible ? "Hide group"
                                  : "Show Group"),
                   false, () => NodeWasToggled(audioNode, !visible));
    }
  ...
  override public void OnRowGUI(Rect rowRect, TreeViewItem node, int row,
    ...
    if (audioNode != null) {
      bool oldSelected =
          m_Controller.CurrentViewContainsGroup(audioNode.group.groupID);
      const float kIconSize = 16f;

      float xMargin = 3f;
      Rect iconRect =
          new Rect(rowRect.x + xMargin, rowRect.y, kIconSize, kIconSize);
      Rect iconBgRect = new Rect(iconRect.x + 1, iconRect.y + 1,
                                 iconRect.width - 2, iconRect.height - 2);

      int colorIndex = audioNode.group.userColorIndex;
      if (colorIndex > 0)
        EditorGUI.DrawRect(
            new Rect(rowRect.x, iconBgRect.y, 2, iconBgRect.height),
            AudioMixerColorCodes.GetColor(colorIndex));
...
internal class AudioMixerGroupPopupContext {
  public AudioMixerGroupPopupContext(AudioMixerController controller,
                                     AudioMixerGroupController group) {
...
internal class AudioMixerGroupTreeView {
  ...
  class Styles {
    public GUIContent header = EditorGUIUtility.TrTextContent(
        "Groups",
        "An Audio Mixer Group is used by e.g Audio Sources to modify the audio output before it reaches the Audio Listener. An Audio Mixer Group will route its output to another Audio Mixer Group if it is made a child of that group. The Master Group will route its output to the Audio Listener if it doesn't route its output into another Mixer.");
    public GUIContent addButton =
        EditorGUIUtility.TrIconContent("CreateAddNew", "Add child group");
    public Texture2D audioMixerGroupIcon =
        EditorGUIUtility.FindTexture(typeof(UnityEngine.Audio.AudioMixerGroup));
  }
  static Styles s_Styles;

  public AudioMixerGroupTreeView(AudioMixerWindow mixerWindow,
                                 TreeViewState treeState) {
    m_AudioGroupTreeState = treeState;

    m_AudioGroupTree =
        new TreeViewController(mixerWindow, m_AudioGroupTreeState);
    ...
    m_AudioGroupTreeDataSource =
        new AudioGroupDataSource(m_AudioGroupTree, m_Controller);
    m_AudioGroupTree.Init(
        mixerWindow.position, m_AudioGroupTreeDataSource, m_TreeViewGUI,
  ...
  public void ReloadTreeData() { m_AudioGroupTree.ReloadData(); }
  ...
  public void AddSiblingGroupPopupCallback(object obj) {
    AudioMixerGroupPopupContext context = (AudioMixerGroupPopupContext) obj;
    if (context.groups != null && context.groups.Length > 0) {
      var item = m_AudioGroupTree.FindItem(context
                                               .groups [0]
                                               .GetInstanceID())
                     as AudioMixerTreeViewNode;
      if (item != null) {
  ...
  public void AddAudioMixerGroup(AudioMixerGroupController parent) {
    ...
    Undo.RecordObjects(new UnityEngine.Object[]{m_Controller, parent},
                       "Add Child Group");
  ...
  static string PluralIfNeeded(int count) { return count > 1 ? "s" : ""; }
  ...
  public void DeleteGroups(List<AudioMixerGroupController> groups,
    ...
    foreach (AudioMixerGroupController group in groups) {
      if (group.HasDependentMixers()) {
        if (!EditorUtility.DisplayDialog(
                "Referenced Group",
                "Deleted group is referenced by another AudioMixer, are you sure?",
                "Delete", "Cancel"))
          return;
        break;
      }
    ...
    if (recordUndo)
      Undo.RegisterCompleteObjectUndo(
          m_Controller, "Delete Group" + PluralIfNeeded(groups.Count));

    m_Controller.DeleteGroups(groups.ToArray());
    ReloadTree();
  }

  public void DuplicateGroups(List<AudioMixerGroupController> groups,
                              bool recordUndo) {
    if (recordUndo) {
      Undo.RecordObject(m_Controller,
                        "Duplicate group" + PluralIfNeeded(groups.Count));
      Undo.RecordObject(m_Controller.masterGroup, "");
    }

    var duplicatedRoots =
        m_Controller.DuplicateGroups(groups.ToArray(), recordUndo);
    if (duplicatedRoots.Count > 0) {
      ReloadTree();
      var instanceIDs =
          duplicatedRoots
              .Select(audioMixerGroup => audioMixerGroup.GetInstanceID())
              .ToArray();
  ...
  void DuplicateGroupPopupCallback(object obj) {
    var audioMixerGroupTreeView = (AudioMixerGroupTreeView) obj;
    audioMixerGroupTreeView.DuplicateGroups(
        GetGroupSelectionWithoutMasterGroup(), true);
  }
  ...
  public void OnTreeViewContextClick(int index) {
    ...
    if (node != null) {
      ...
      if (mixerNode != null && mixerNode.group != null) {
        ...
        if (!EditorApplication.isPlaying) {
          pm.AddItem(
              EditorGUIUtility.TrTextContent("Add child group"), false,
              AddChildGroupPopupCallback,
              new AudioMixerGroupPopupContext(m_Controller, mixerNode.group));
          if (mixerNode.group != m_Controller.masterGroup) {
            pm.AddItem(
                EditorGUIUtility.TrTextContent("Add sibling group"), false,
                AddSiblingGroupPopupCallback,
                new AudioMixerGroupPopupContext(m_Controller, mixerNode.group));
            pm.AddSeparator("");
            pm.AddItem(EditorGUIUtility.TrTextContent("Rename"), false,
                       RenameGroupCallback, node);

            // Mastergroup cannot be deleted nor duplicated
            var selection = GetGroupSelectionWithoutMasterGroup().ToArray();
            pm.AddItem(new GUIContent((selection.Length > 1)
                                          ? "Duplicate groups (and children)"
                                          : "Duplicate group (and children)"),
                       false, DuplicateGroupPopupCallback, this);
            pm.AddItem(new GUIContent((selection.Length > 1)
                                          ? "Remove groups (and children)"
                                          : "Remove group (and children)"),
                       false, DeleteGroupsPopupCallback, this);
          }
        } else {
          pm.AddDisabledItem(EditorGUIUtility.TrTextContent(
              "Modifying group topology in play mode is not allowed"));
        }
  ...
  void OnNodeToggled(AudioMixerTreeViewNode node, bool nodeWasEnabled) {
    ...
    var treeSelection =
        GetAudioMixerGroupsFromNodeIDs(m_AudioGroupTree.GetSelection());
  ...
  List<AudioMixerGroupController>
  GetAudioMixerGroupsFromNodeIDs(int[] instanceIDs) {
    List<AudioMixerGroupController> newSelectedGroups =
  ...
  public void InitSelection(bool revealSelectionAndFrameLastSelected) {
    ...
    m_AudioGroupTree.SetSelection(
        (from x in groups select x.GetInstanceID()).ToArray(),
  ...
  public float GetTotalHeight() {
    if (m_Controller == null)
      return 0f;
    return m_AudioGroupTree.gui.GetTotalSize().y +
  ...
  public void OnGUI(Rect rect) {
    ...
    using(new EditorGUI.DisabledScope(m_Controller == null)) {
      ...
      AudioMixerDrawUtils.HeaderLabel(headerRect, s_Styles.header,
                                      s_Styles.audioMixerGroupIcon);
    }

    if (m_Controller != null) {
      AudioMixerGroupController parent =
          (m_Controller.CachedSelection.Count == 1)
              ? m_Controller.CachedSelection[0]
              : m_Controller.masterGroup;
      using(new EditorGUI.DisabledScope(EditorApplication.isPlaying)) {
        if (GUI.Button(
                new Rect(headerRect.xMax - 17f, headerRect.y + 3f, 16f, 16f),
      ...
      AudioMixerDrawUtils.DrawScrollDropShadow(
          contentRect, m_AudioGroupTree.state.scrollPos.y,
          m_AudioGroupTree.gui.GetTotalSize().y);
    ...
    if (eventType == EventType.ExecuteCommand ||
      ...
      if (Event.current.commandName == EventCommandNames.Delete ||
        ...
        if (execute) {
          ...
          GUIUtility.ExitGUI(); // Cached groups might have been deleted to so
                                // early out of event
        }
  ...
  void HandleKeyboardEvents(int treeViewKeyboardControlID) {
    ...
    if (evt.keyCode == KeyCode.Space && evt.type == EventType.KeyDown) {
      ...
      if (selection.Length > 0) {
        AudioMixerTreeViewNode node =
            m_AudioGroupTree.FindItem(selection[0]) as AudioMixerTreeViewNode;
  ...
  public void OnMixerControllerChanged(AudioMixerController controller) {
    if (m_Controller != controller) {
      ...
      if (controller != null) {
        ...
        InitSelection(false);
        LoadExpandedState();
        m_AudioGroupTree.data.SetExpandedWithChildren(
            m_AudioGroupTree.data.root, true);
      }
    }
  }
  ...
  void SaveExpandedState() {
    SessionState.SetIntArray(GetUniqueAudioMixerName(m_Controller),
                             m_AudioGroupTreeState.expandedIDs.ToArray());
  }

  void LoadExpandedState() {
    int[] cachedExpandedState =
        SessionState.GetIntArray(GetUniqueAudioMixerName(m_Controller), null);
    if (cachedExpandedState != null) {
      m_AudioGroupTreeState.expandedIDs = new List<int>(cachedExpandedState);
    } else {
      // Expand whole tree. If no cached data then its the first time tree was
      // loaded in this session
      m_AudioGroupTree.state.expandedIDs = new List<int>();
      m_AudioGroupTree.data.SetExpandedWithChildren(m_AudioGroupTree.data.root,
                                                    true);
    }
  }

  public void EndRenaming() { m_AudioGroupTree.EndNameEditing(true); }
