...
internal static class TreeViewForAudioMixerGroup {
  public static void
  CreateAndSetTreeView(ObjectTreeForSelector.TreeSelectorData data) {
    var ignoreController = InternalEditorUtility.GetObjectFromInstanceID(
        data.userData) as AudioMixerController;

    // Create treeview
    var treeView = new TreeViewController(data.editorWindow, data.state);
    var treeGui = new GroupTreeViewGUI(treeView);
    var dataSource =
        new TreeViewDataSourceForMixers(treeView, ignoreController);
  ...
  class GroupTreeViewGUI : TreeViewGUI {
    private readonly Texture2D k_AudioGroupIcon =
        EditorGUIUtility.FindTexture(typeof(AudioMixerGroup));
    private readonly Texture2D k_AudioListenerIcon =
        EditorGUIUtility.FindTexture(typeof(AudioListener));
    ...
    public override void OnRowGUI(Rect rowRect, TreeViewItem item, int row,
      ...
      if (isRootItem && !isNoneItem) {
        AudioMixerController controller =
            ((MixerTreeViewItem) item).group.controller;
        GUI.Label(new Rect(rowRect.x + 2f, rowRect.y - 18f, rowRect.width, 18f),
                  GUIContent.Temp(controller.name), EditorStyles.boldLabel);
      }
    }
    ...
    public override int GetNumRowsOnPageUpDown(TreeViewItem fromItem,
                                               bool pageUp,
      ...
      return (int) Mathf.Floor(
          heightOfTreeView /
          k_LineHeight); // good enough though not exact due to k_SpaceBetween
    }
  ...
  class MixerTreeViewItem : TreeViewItem {
    public MixerTreeViewItem(int id, int depth, TreeViewItem parent,
                             string displayName,
                             AudioMixerGroupController groupController)
  ...
  class TreeViewDataSourceForMixers : TreeViewDataSource {
    ...
    public TreeViewDataSourceForMixers(TreeViewController treeView,
    ...
    bool ShouldShowController(AudioMixerController controller,
                              List<int> allowedInstanceIDs) {
      if (!controller)
        return false;
    ...
    public override void FetchData() {
      ...
      // If we only have one controller then just expand that entirely to the
      // user doesnt have to. (If we have more than one root keep them collapsed
    ...
    private TreeViewItem BuildSubTree(AudioMixerController controller) {
      ...
      var masterItem =
          new MixerTreeViewItem(masterGroup.GetInstanceID(), 0, m_RootItem,
                                masterGroup.name, masterGroup);
      AddChildrenRecursive(masterGroup, masterItem);
      return masterItem;
    }

    private void AddChildrenRecursive(AudioMixerGroupController group,
                                      TreeViewItem item) {
      item.children = new List<TreeViewItem>(group.children.Length);
      for (int i = 0; i < group.children.Length; ++i) {
        item.children.Add(new MixerTreeViewItem(
            group
                .children [i]
                .GetInstanceID(),
            item.depth + 1, item, group.children[i].name, group.children[i]));
