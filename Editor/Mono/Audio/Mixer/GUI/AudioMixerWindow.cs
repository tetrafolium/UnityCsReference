...
internal class AudioMixerWindow : EditorWindow,
                                  IHasCustomMenu {
  ...
  enum SectionType { MixerTree, GroupTree, ViewList, SnapshotList }

  public enum LayoutMode { Uninitialized = -1, Horizontal = 0, Vertical = 1 }
  ...
  [SerializeField] Layout m_LayoutStripsOnTop;
  [SerializeField] Layout m_LayoutStripsOnRight;
  [SerializeField] SectionType[] m_SectionOrder = {
      SectionType.MixerTree, SectionType.SnapshotList, SectionType.GroupTree,
      SectionType.ViewList}; // default order

  [SerializeField] LayoutMode m_LayoutMode = LayoutMode.Uninitialized;
  [SerializeField] bool m_SortGroupsAlphabetically = false;
  [SerializeField] bool m_ShowReferencedBuses = true;
  [SerializeField] bool m_ShowBusConnections = false;
  [SerializeField] bool m_ShowBusConnectionsOfSelection = false;
  ...
  [NonSerialized] bool m_ShowDeveloperOverlays = false;
  ...
  LayoutMode layoutMode {
    ...
    set {
      m_LayoutMode = value;
      m_RepaintCounter = 2;
      EditorPrefs.SetBool(kAudioMixerUseHorzLayout,
  ...
  class GUIContents {
    ...
    public GUIContents() {
      editSnapShots = EditorGUIUtility.TrTextContent(
          "Edit in Play Mode",
          "Edit in playmode and your changes are automatically saved. Note when editing is disabled, live values are shown.",
          EditorGUIUtility
              .TrIconContent(
                  "Animation.Record",
  ...
  public static void CreateAudioMixerWindow() {
    var win = GetWindow<AudioMixerWindow>(typeof(
        ProjectBrowser)); // From usability tests we decided to auto dock
                          // together with project browser to prevent the mixer
                          // window keep going behind the main window on OSX

    if (win.m_Pos.width < 400f)
      win.m_Pos = new Rect(win.m_Pos.x, win.m_Pos.y, 800,
                           450f); // Set default size if window is small
  }
  ...
  void Init() {
    ...
    if (m_LayoutStripsOnTop.m_VerticalSplitter == null ||
        m_LayoutStripsOnTop.m_VerticalSplitter.realSizes.Length != 2) {
      m_LayoutStripsOnTop.m_VerticalSplitter = SplitterState.FromAbsolute(
          new float[]{65, 35}, new float[]{85, 105}, null);
    }

    if (m_LayoutStripsOnTop.m_HorizontalSplitter == null ||
        m_LayoutStripsOnTop.m_HorizontalSplitter.realSizes.Length != 4)
      m_LayoutStripsOnTop.m_HorizontalSplitter = SplitterState.FromAbsolute(
          new float[]{60, 60, 60, 60}, new float[]{85, 85, 85, 85}, null);

    if (m_LayoutStripsOnRight == null)
      m_LayoutStripsOnRight = new Layout();

    if (m_LayoutStripsOnRight.m_HorizontalSplitter == null ||
        m_LayoutStripsOnRight.m_HorizontalSplitter.realSizes.Length != 2)
      m_LayoutStripsOnRight.m_HorizontalSplitter = SplitterState.FromAbsolute(
          new float[]{30, 70}, new float[]{160, 160}, null);

    if (m_LayoutStripsOnRight.m_VerticalSplitter == null ||
        m_LayoutStripsOnRight.m_VerticalSplitter.realSizes.Length != 4)
      m_LayoutStripsOnRight.m_VerticalSplitter = SplitterState.FromAbsolute(
          new float[]{60, 60, 60, 60}, new float[]{100, 85, 85, 85}, null);
    ...
    m_MixersTree =
        new AudioMixersTreeView(this, m_MixersTreeState, GetAllControllers);
    ...
    m_ChannelStripView =
        new AudioMixerChannelStripView(m_ChannelStripViewState);
  ...
  List<AudioMixerController> GetAllControllers() { return m_AllControllers; }
  ...
  public void Awake() {
    ...
    if (m_MixersTreeState != null) {
      // Clear state that are serialized in the layout file that should not
      // survive closing/starting Unity
  ...
  public void OnEnable() {
    if (m_LayoutMode == LayoutMode.Uninitialized) {
      ...
      m_LayoutMode = EditorPrefs.GetBool(kAudioMixerUseHorzLayout)
  ...
  public void Update() {
    if (m_Ticker.DoTick()) {
      if (EditorApplication.isPlaying ||
          (m_ChannelStripView != null && m_ChannelStripView.requiresRepaint))
        Repaint();
    }
  }
  ...
  Dictionary<AudioMixerEffectController, AudioMixerGroupController>
  GetEffectMap(List<AudioMixerGroupController> allGroups) {
    var effectMap =
        new Dictionary<AudioMixerEffectController, AudioMixerGroupController>();
  ...
  void DoToolbar() {
    EditorGUILayout.BeginHorizontal(
        EditorStyles.toolbar, GUILayout.Height(EditorGUI.kWindowToolbarHeight));
    ...
    if (m_Controller != null) {
      if (Application.isPlaying) {
        ...
        AudioSettings.editingInPlaymode = GUILayout.Toggle(
            AudioSettings.editingInPlaymode, s_GuiContents.editSnapShots,
      ...
      AudioMixerExposedParametersPopup.Popup(m_Controller,
  ...
  void LayoutWithStripsOnRightSideOneScrollBar(
      List<AudioMixerGroupController> allGroups,
      Dictionary<AudioMixerEffectController, AudioMixerGroupController>
          effectMap) {
    // Do layouting
    SplitterState horizontalState = m_LayoutStripsOnRight.m_HorizontalSplitter;
    SplitterGUILayout.BeginHorizontalSplit(horizontalState,
    ...
    Rect column1Rect =
        new Rect(0, EditorGUI.kWindowToolbarHeight, column1Width,
                 position.height - EditorGUI.kWindowToolbarHeight);
    Rect column2Rect = new Rect(column1Width, EditorGUI.kWindowToolbarHeight,
                                column2Width, column1Rect.height);

    // Column1

    // Background color for mixertree and views (if needed)
    if (EditorGUIUtility.isProSkin)
      EditorGUI.DrawRect(column1Rect, EditorGUIUtility.isProSkin ? new Color(
                                          0.19f, 0.19f, 0.19f)
    ...
    for (int i = 0; i < m_SectionOrder.Length; i++) {
      ...
      sectionRects[i] = new Rect(xPos, yPos, column1Rect.width,
                                 GetHeightOfSection(m_SectionOrder[i]));
    ...
    // Scroll view
    m_SectionsScrollPosition =
        GUI.BeginScrollView(column1Rect, m_SectionsScrollPosition, contentRect);
    DoSections(column1Rect, sectionRects, m_SectionOrder);
    GUI.EndScrollView();

    // Column2
    m_ChannelStripView.OnGUI(column2Rect, m_ShowReferencedBuses,
                             m_ShowBusConnections,
                             m_ShowBusConnectionsOfSelection, allGroups,
                             effectMap, m_SortGroupsAlphabetically,
                             m_ShowDeveloperOverlays, m_GroupTree.ScrollToItem);

    // Vertical line (split)
    EditorGUI.DrawRect(
        new Rect(column1Rect.xMax - 1, EditorGUI.kWindowToolbarHeight, 1,
                 position.height - EditorGUI.kWindowToolbarHeight),
        EditorGUIUtility.isProSkin ? new Color(0.15f, 0.15f, 0.15f)
        : new Color(0.6f, 0.6f, 0.6f));
  }

  void LayoutWithStripsOnTop(List<AudioMixerGroupController> allGroups,
    ...
    SplitterGUILayout.BeginVerticalSplit(verticalState,
                                         GUILayout.ExpandWidth(true),
                                         GUILayout.ExpandHeight(true));
    ...
    float channelStripYPos = m_GroupsRenderedAboveSections ? EditorGUI
                                 .kWindowToolbarHeight
        : EditorGUI.kWindowToolbarHeight +
                             verticalState.realSizes[0];
    float channelStripHeight = m_GroupsRenderedAboveSections ? verticalState
                                   .realSizes[0] : verticalState.realSizes[1];

    float sectionsYPos = !m_GroupsRenderedAboveSections ? EditorGUI
                              .kWindowToolbarHeight
        : EditorGUI.kWindowToolbarHeight +
                         verticalState.realSizes[0];
    float sectionsHeight = !m_GroupsRenderedAboveSections ? verticalState
                                .realSizes[0] : verticalState.realSizes[1];

    Rect channelStripViewRect =
        new Rect(0, channelStripYPos, position.width, channelStripHeight);
    Rect totalRectOfSections =
        new Rect(0, channelStripViewRect.yMax, position.width,
                 position.height - channelStripViewRect.height);
    ...
    for (int i = 0; i < sectionRects.Length; i++) {
      ...
      sectionRects[i] =
          new Rect(xPos, sectionsYPos, horizontalState.realSizes[i],
                   sectionsHeight - spaceFromBottom);
    }
    ...
    m_ChannelStripView.OnGUI(channelStripViewRect, m_ShowReferencedBuses,
                             m_ShowBusConnections,
                             m_ShowBusConnectionsOfSelection, allGroups,
                             effectMap, m_SortGroupsAlphabetically,
                             m_ShowDeveloperOverlays, m_GroupTree.ScrollToItem);
    ...
    EditorGUI.DrawRect(new Rect(0,
                                EditorGUI.kWindowToolbarHeight +
                                    verticalState.realSizes[0] - 1,
                                position.width, 1),
  ...
  float GetHeightOfSection(SectionType sectionType) {
    ...
    case SectionType.MixerTree:
      return m_MixersTree.GetTotalHeight();
    case SectionType.SnapshotList:
      return m_SnapshotListView.GetTotalHeight();
    case SectionType.GroupTree:
      return m_GroupTree.GetTotalHeight();
    case SectionType.ViewList:
      return m_GroupViews.GetTotalHeight();
    default:
  ...
  void DoSections(Rect totalRectOfSections, Rect[] sectionRects,
                  SectionType[] sectionOrder) {
    Event evt = Event.current;
    bool enabledGUI =
        m_Controller == null || AudioMixerController.EditingTargetSnapshot();
    ...
    for (int i = 0; i < sectionOrder.Length; ++i) {
      ...
      if (evt.type == EventType.ContextClick) {
        Rect sectionHeaderRect =
            new Rect(sectionRect.x, sectionRect.y, sectionRect.width - 15f,
                     AudioMixerDrawUtils.kSectionHeaderHeight);
  ...
  void ReorderContextMenu(Rect rect, int sectionIndex) {
    ...
    if (Event.current.type == EventType.ContextClick &&
      ...
      GUIContent moveUp =
          new GUIContent(m_LayoutMode == LayoutMode.Horizontal ? "Move Left"
                         : "Move Up");
      GUIContent moveDown =
          new GUIContent(m_LayoutMode == LayoutMode.Horizontal ? "Move Right"
                         : "Move Down");

      GenericMenu menu = new GenericMenu();
      if (sectionIndex > 1)
        menu.AddItem(moveUp, false, ChangeSectionOrder,
                     new Vector2(sectionIndex, -1));
      else
        menu.AddDisabledItem(moveUp);
      if (sectionIndex > 0 && sectionIndex < m_SectionOrder.Length - 1)
        menu.AddItem(moveDown, false, ChangeSectionOrder,
                     new Vector2(sectionIndex, 1));
      else
  ...
  void ChangeSectionOrder(object userData) {
    ...
    int newSectionIndex =
        Mathf.Clamp(sectionIndex + direction, 0, m_SectionOrder.Length - 1);
  ...
  public MixerParameterDefinition ParamDef(string name, string desc,
                                           string units, float displayScale,
  ...
  // Add items to the context menu for the AudioMixerWindow (the tree horizontal
  // lines, upper right corner)
  public virtual void AddItemsToMenu(GenericMenu menu) {
    menu.AddItem(EditorGUIUtility.TrTextContent("Sort groups alphabetically"),
                 m_SortGroupsAlphabetically, delegate {
                   m_SortGroupsAlphabetically = !m_SortGroupsAlphabetically;
                 });
    menu.AddItem(EditorGUIUtility.TrTextContent("Show referenced groups"),
                 m_ShowReferencedBuses,
                 delegate { m_ShowReferencedBuses = !m_ShowReferencedBuses; });
    menu.AddItem(EditorGUIUtility.TrTextContent("Show group connections"),
                 m_ShowBusConnections,
                 delegate { m_ShowBusConnections = !m_ShowBusConnections; });
    if (m_ShowBusConnections)
      menu.AddItem(EditorGUIUtility.TrTextContent(
                       "Only highlight selected group connections"),
                   m_ShowBusConnectionsOfSelection, delegate {
                     m_ShowBusConnectionsOfSelection =
                         !m_ShowBusConnectionsOfSelection;
                   });
    menu.AddSeparator("");
    menu.AddItem(EditorGUIUtility.TrTextContent("Vertical layout"),
                 layoutMode == LayoutMode.Vertical,
                 delegate { layoutMode = LayoutMode.Vertical; });
    menu.AddItem(EditorGUIUtility.TrTextContent("Horizontal layout"),
                 layoutMode == LayoutMode.Horizontal,
                 delegate { layoutMode = LayoutMode.Horizontal; });
    menu.AddSeparator("");
    menu.AddItem(
        EditorGUIUtility.TrTextContent("Use RMS metering for display"),
        EditorPrefs.GetBool(kAudioMixerUseRMSMetering, true),
        delegate { EditorPrefs.SetBool(kAudioMixerUseRMSMetering, true); });
    menu.AddItem(
        EditorGUIUtility.TrTextContent("Use peak metering for display"),
        !EditorPrefs.GetBool(kAudioMixerUseRMSMetering, true),
        delegate { EditorPrefs.SetBool(kAudioMixerUseRMSMetering, false); });
    if (Unsupported.IsDeveloperMode()) {
      menu.AddSeparator("");
      menu.AddItem(
          EditorGUIUtility.TrTextContent("DEVELOPER/Groups Rendered Above"),
          m_GroupsRenderedAboveSections, delegate {
            m_GroupsRenderedAboveSections = !m_GroupsRenderedAboveSections;
          });
      menu.AddItem(EditorGUIUtility.TrTextContent("DEVELOPER/Build 10 groups"),
                   false, delegate { m_Controller.BuildTestSetup(0, 7, 10); });
      menu.AddItem(EditorGUIUtility.TrTextContent("DEVELOPER/Build 20 groups"),
                   false, delegate { m_Controller.BuildTestSetup(0, 7, 20); });
      menu.AddItem(EditorGUIUtility.TrTextContent("DEVELOPER/Build 40 groups"),
                   false, delegate { m_Controller.BuildTestSetup(0, 7, 40); });
      menu.AddItem(EditorGUIUtility.TrTextContent("DEVELOPER/Build 80 groups"),
                   false, delegate { m_Controller.BuildTestSetup(0, 7, 80); });
      menu.AddItem(EditorGUIUtility.TrTextContent("DEVELOPER/Build 160 groups"),
                   false, delegate { m_Controller.BuildTestSetup(0, 7, 160); });
      menu.AddItem(
          EditorGUIUtility.TrTextContent("DEVELOPER/Build chain of 10 groups"),
          false, delegate { m_Controller.BuildTestSetup(1, 1, 10); });
      menu.AddItem(
          EditorGUIUtility.TrTextContent("DEVELOPER/Build chain of 20 groups "),
          false, delegate { m_Controller.BuildTestSetup(1, 1, 20); });
      menu.AddItem(
          EditorGUIUtility.TrTextContent("DEVELOPER/Build chain of 40 groups"),
          false, delegate { m_Controller.BuildTestSetup(1, 1, 40); });
      menu.AddItem(
          EditorGUIUtility.TrTextContent("DEVELOPER/Build chain of 80 groups"),
          false, delegate { m_Controller.BuildTestSetup(1, 1, 80); });
      menu.AddItem(EditorGUIUtility.TrTextContent("DEVELOPER/Show overlays"),
                   m_ShowDeveloperOverlays, delegate {
                     m_ShowDeveloperOverlays = !m_ShowDeveloperOverlays;
                   });
    }
  }
}
...
internal class AssetSelectionPopupMenu {
  static public void Show(Rect buttonRect, string[] classNames,
                          int initialSelectedInstanceID) {
    var menu = new GenericMenu();

    var objs =
        AssetDatabase.FindAllAssets(new SearchFilter(){classNames = classNames})
            .Select(property => property.pptrValue)
            .ToList();
    if (objs.Any()) {
      objs.Sort((result1, result2) =>
                    EditorUtility.NaturalCompare(result1.name, result2.name));
      ...
      menu.AddDisabledItem(EditorGUIUtility.TrTextContent(
          "No Audio Mixers found in this project"));
    }
