...
internal class ReorderableListWithRenameAndScrollView {
  ...
  public GUIStyle listElementStyle =
      null; // if null then a default style is used
  public GUIStyle renameOverlayStyle =
      null; // if null then the default rename style is used
  ...
  public ReorderableListWithRenameAndScrollView(ReorderableList list,
                                                State state) {
    m_State = state;
  ...
  void EnsureRowIsVisible(int index, float scrollGUIHeight) {
    ...
    float scrollBottom =
        topPixelOfRow - scrollGUIHeight + m_ReorderableList.elementHeight + 3;
    m_State.m_ScrollPos.y =
        Mathf.Clamp(m_State.m_ScrollPos.y, scrollBottom, topPixelOfRow);
  }
  ...
  public bool IsRenamingIndex(int index) {
    return GetRenameOverlay().IsRenaming() &&
           GetRenameOverlay().userData == index &&
  ...
  public void DrawElement(Rect r, int index, bool isActive, bool isFocused) {
    if (IsRenamingIndex(index)) {
      // We do not want to set rect when they can be invalid, this can happen
      // for layout or used events
      ...
      if (onCustomDrawElement != null)
        ...
        DrawElementText(r, index, isActive, index == m_ReorderableList.index,
                        isFocused);
    }
  }

  public void DrawElementText(Rect r, int index, bool isActive, bool isSelected,
                              bool isFocused) {
    if (Event.current.type == EventType.Repaint && onGetNameAtIndex != null) {
      elementStyle.Draw(r, onGetNameAtIndex(index), false, false, isSelected,
                        isFocused);
    }
  }
  ...
  void RenameEnded() {
    ...
    // We give keyboard focus back to our reorderable list because the rename
    // utility stole it (now we give it back)
  ...
  public void FrameItem(int index) { m_FrameIndex = index; }
