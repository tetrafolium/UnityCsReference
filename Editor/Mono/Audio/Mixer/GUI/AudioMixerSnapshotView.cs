...
internal class AudioMixerSnapshotListView {
  private ReorderableListWithRenameAndScrollView
  ...
  class Styles {
    public GUIContent starIcon =
        EditorGUIUtility.TrIconContent("Favorite", "Start snapshot");
    public GUIContent header = EditorGUIUtility.TrTextContent(
        "Snapshots",
        "A snapshot is a set of values for all parameters in the mixer. When using the mixer, you modify parameters in the selected snapshot. Blend between multiple snapshots at runtime.");
    public GUIContent addButton =
        EditorGUIUtility.TrIconContent("CreateAddNew");
    public Texture2D snapshotsIcon = EditorGUIUtility.FindTexture(
        typeof(UnityEngine.Audio.AudioMixerSnapshot));
  }
  static Styles s_Styles;

  public AudioMixerSnapshotListView(
      ReorderableListWithRenameAndScrollView.State state) {
    m_State = state;
  }
  ...
  void RecreateListControl() {
    ...
    m_Snapshots =
        new List<AudioMixerSnapshotController>(m_Controller.snapshots);

    ReorderableList reorderableList =
        new ReorderableList(m_Snapshots, typeof(AudioMixerSnapshotController),
    ...
    m_ReorderableListWithRenameAndScrollView =
        new ReorderableListWithRenameAndScrollView(reorderableList, m_State);
    m_ReorderableListWithRenameAndScrollView.onSelectionChanged +=
        SelectionChanged;
    m_ReorderableListWithRenameAndScrollView.onNameChangedAtIndex +=
        NameChanged;
    m_ReorderableListWithRenameAndScrollView.onDeleteItemAtIndex += Delete;
    m_ReorderableListWithRenameAndScrollView.onGetNameAtIndex +=
        GetNameOfElement;
    m_ReorderableListWithRenameAndScrollView.onCustomDrawElement +=
        CustomDrawElement;
  }
  ...
  public void CustomDrawElement(Rect r, int index, bool isActive,
                                bool isFocused) {
    ...
    bool isSelected =
        (index == m_ReorderableListWithRenameAndScrollView.list.index) &&
        !m_ReorderableListWithRenameAndScrollView.IsRenamingIndex(index);

    // Text
    r.width -= iconSize + spacing;
    m_ReorderableListWithRenameAndScrollView.DrawElementText(
        r, index, isActive, isSelected, isFocused);
  ...
  public float GetTotalHeight() {
    if (m_Controller == null)
      return 0f;
    return m_ReorderableListWithRenameAndScrollView.list.GetHeight() +
  ...
  public void OnGUI(Rect rect) {
    ...
    using(new EditorGUI.DisabledScope(m_Controller == null)) {
      ...
      AudioMixerDrawUtils.HeaderLabel(headerRect, s_Styles.header,
                                      s_Styles.snapshotsIcon);
    }

    if (m_Controller != null) {
      // Ensure gui is in-sync with backend (TargetSnapShotIndex can be changed
      // anytime from the backend)
  ...
  public void SelectionChanged(int index) {
    // For some UI reason, selecting some region just below the last element of
    // the re-orderable list will cause a selection index greater than is
    // available.
  ...
  string GetNameOfElement(int index) { return m_Snapshots[index].name; }
  ...
  void DeleteSnapshot(AudioMixerSnapshotController snapshot) {
    ...
    LoadFromBackend();
    m_ReorderableListWithRenameAndScrollView.list.index =
        GetSnapshotIndex(m_Controller.TargetSnapshot);
    UpdateViews();
  }

  void Delete(int index) { DeleteSnapshot(m_Snapshots[index]); }

  public void EndDragChild(ReorderableList list) {
    m_Snapshots = m_ReorderableListWithRenameAndScrollView.list
                      .list as List<AudioMixerSnapshotController>;
    SaveToBackend();
  }

  private void UpdateViews() {
    AudioMixerWindow mixerWindow =
        (AudioMixerWindow)
            WindowLayout.FindEditorWindowOfType(typeof(AudioMixerWindow));
    if (mixerWindow != null)
      mixerWindow.Repaint();
  ...
  void Rename(AudioMixerSnapshotController snapshot) {
    m_ReorderableListWithRenameAndScrollView.BeginRename(
        GetSnapshotIndex(snapshot), 0f);
  }
  ...
  internal class SnapshotMenu {
    ...
    static public void Show(Rect buttonRect,
                            AudioMixerSnapshotController snapshot,
                            AudioMixerSnapshotListView list) {
      var menu = new GenericMenu();
      data input = new data(){snapshot = snapshot, list = list};
      menu.AddItem(EditorGUIUtility.TrTextContent("Set as start Snapshot"),
                   false, SetAsStartupSnapshot, input);
      menu.AddSeparator("");
      menu.AddItem(EditorGUIUtility.TrTextContent("Rename"), false, Rename,
                   input);
  ...
  public void OnUndoRedoPerformed() { LoadFromBackend(); }
}
}
