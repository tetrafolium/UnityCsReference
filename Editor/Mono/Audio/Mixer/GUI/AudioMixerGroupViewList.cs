...
internal class AudioMixerGroupViewList {
  ...
  class Styles {
    public GUIContent header = EditorGUIUtility.TrTextContent(
        "Views",
        "A view is the saved visibility state of the current Mixer Groups. Use views to setup often used combinations of Mixer Groups.");
    public GUIContent addButton =
        EditorGUIUtility.TrIconContent("CreateAddNew");
    public Texture2D viewsIcon =
        EditorGUIUtility.FindTexture("AudioMixerView Icon");
  }
  static Styles s_Styles;

  public AudioMixerGroupViewList(
  ...
  public void OnUndoRedoPerformed() { RecreateListControl(); }
  ...
  public void RecreateListControl() {
    ...
    if (m_Views.Count == 0) {
      ...
      view.guids = m_Controller.GetAllAudioGroupsSlow()
    ...
    var reorderableList = new ReorderableList(m_Views, typeof(MixerGroupView),
    ...
    if (m_Controller.currentViewIndex >= reorderableList.count)
      Debug.LogError(
          "State mismatch, currentViewIndex: " + m_Controller.currentViewIndex +
          ", num items: " + reorderableList.count);
  ...
  public void OnGUI(Rect rect) {
    ...
    if (m_Controller != null) {
      // Ensure in-sync
      if (m_ReorderableListWithRenameAndScrollView.list.index !=
          m_Controller.currentViewIndex) {
        m_ReorderableListWithRenameAndScrollView.list.index =
            m_Controller.currentViewIndex;
        m_ReorderableListWithRenameAndScrollView.FrameItem(
            m_Controller.currentViewIndex);
      }
  ...
  void SaveToBackend() { m_Controller.views = m_Views.ToArray(); }
  ...
  string GetNameOfElement(int index) { return m_Views[index].name; }
  ...
  public void EndDragChild(ReorderableList list) {
    m_Views = m_ReorderableListWithRenameAndScrollView.list
                  .list as List<MixerGroupView>;
    SaveToBackend();
  }
  ...
  internal class ViewsContexttMenu {
    ...
    static public void Show(Rect buttonRect, int viewIndex,
                            AudioMixerGroupViewList list) {
