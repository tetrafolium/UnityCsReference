...
internal class AudioMixerChannelStripView {
  ...
  public static float kEffectScaleMouseDrag =
      0.3f; // higher precision for effect slot editing

  private static Color kMoveColorHighlight = new Color(0.3f, 0.6f, 1.0f, 0.4f);
  private static Color kMoveSlotColHiAllowed = new Color(
      1f, 1f, 1f, 0.7f); // new Color(59 / 255f, 162 / 255f, 216 / 255f, 0.7f);
  ...
  private static Color kMoveSlotColHiDisallowed =
      new Color(1.0f, 0.0f, 0.0f, 0.7f);
  private static Color kMoveSlotColLoDisallowed =
      new Color(0.8f, 0.0f, 0.0f, 0.0f);
  private static Color kMoveSlotColBorderDisallowed =
  ...
  private MixerGroupControllerCompareByName m_GroupComparer =
      new MixerGroupControllerCompareByName();
  ...
  static Texture2D CreateTilableGridTexture(int width, int height,
  ...
  Texture2D gridTextureTilable {
    get {
      if (m_GridTexture == null)
        m_GridTexture =
            CreateTilableGridTexture((int) kGridTileWidth, (int) kGridTileWidth,
                                     new Color(0, 0, 0, 0), gridColor);
  ...
  void DrawAreaBackground(Rect rect) {
    if (Event.current.type == EventType.Repaint) {
      ...
      AudioMixerDrawUtils.styles.channelStripAreaBackground.Draw(
          rect, false, false, false, false);
      GUI.color = prevColor;
      // Draw grid
      // GUI.DrawTextureWithTexCoords (rect, gridTextureTilable, new Rect(0, 0,
      // rect.width / gridTextureTilable.width, rect.height /
      // gridTextureTilable.height), true);
    }
  }
  ...
  public class EffectContext {
    public EffectContext(AudioMixerController controller,
                         AudioMixerGroupController[] groups, int index,
                         string name) {
  ...
  public static void InsertEffectPopupCallback(object obj) {
    ...
    foreach (AudioMixerGroupController group in context.groups) {
      ...
      AudioMixerEffectController newEffect =
          new AudioMixerEffectController(context.name);

      int index = (context.index == -1 || context.index > group.effects.Length)
  ...
  public class ConnectSendContext {
    public ConnectSendContext(AudioMixerEffectController sendEffect,
                              AudioMixerEffectController targetEffect) {
  ...
  private bool ClipRect(Rect r, Rect clipRect, ref Rect overlap) {
    ...
    overlap.width =
        Mathf.Min(r.x + r.width, clipRect.x + clipRect.width) - overlap.x;
    overlap.height =
        Mathf.Min(r.y + r.height, clipRect.y + clipRect.height) - overlap.y;
    return overlap.width > 0.0f && overlap.height > 0.0f;
  }

  public float VerticalFader(Rect r, float value, int direction,
                             float dragScale, bool drawScaleValues,
                             bool drawMarkerValue, string tooltip,
                             float maxValue, GUIStyle style) {
    Event evt = Event.current;
    int handleHeight = (int) style.fixedHeight;
    int faderScreenRange = (int) r.height - handleHeight;
    float valueScreenPos = AudioMixerController.VolumeToScreenMapping(
        Mathf.Clamp(value, AudioMixerController.kMinVolume, maxValue),
        faderScreenRange, true);
    Rect handleRect =
        new Rect(r.x, r.y + (int) valueScreenPos, r.width, handleHeight);

    int controlID =
        GUIUtility.GetControlID(kVerticalFaderHash, FocusType.Passive);
    ...
    case EventType.MouseDrag:
      if (GUIUtility.hotControl == controlID) {
        valueScreenPos = Mathf.Clamp(
            m_MouseDragStartValue +
                dragScale * (evt.mousePosition.y - m_MouseDragStartY),
            0.0f, faderScreenRange);
        value = Mathf.Clamp(AudioMixerController.VolumeToScreenMapping(
                                valueScreenPos, faderScreenRange, false),
                            AudioMixerController.kMinVolume, maxValue);
        evt.Use();
      }
      break;
    ...
    case EventType.Repaint:
      ...
      if (drawScaleValues) {
        ...
        using(new EditorGUI.DisabledScope(true)) {
          while (level >= AudioMixerController.kMinVolume) {
            float y = AudioMixerController.VolumeToScreenMapping(
                level, faderScreenRange, true);

            if (level / 10 % 2 == 0)
              GUI.Label(new Rect(r.x, tickStartY + y - 7, r.width, 14),
                        GUIContent.Temp(Mathf.RoundToInt(level).ToString()),
                        styles.vuValue);
            EditorGUI.DrawRect(new Rect(r.x, tickStartY + y - 1, 5f, 1),
      ...
      // Drag handle
      if (drawMarkerValue)
        style.Draw(handleRect,
                   GUIContent.Temp(Mathf.RoundToInt(value).ToString()), false,
                   false, false, false);
      else
  ...
  public float HorizontalFader(Rect r, float value, float minValue,
                               float maxValue, int direction, float dragScale) {
    m_IndexCounter++;
    Rect r2 = new Rect(r);
    float handleWidth = r.width * 0.2f,
          faderScreenRange = r2.width - handleWidth;
      ...
      if (evt.type == EventType.MouseDrag) {
        value = m_MouseDragStartValue +
                (dragScale * (maxValue - minValue) *
                 (evt.mousePosition.x - m_MouseDragStartX) / faderScreenRange);
        Event.current.Use();
    ...
    r2.width = r.width;
    r2.x =
        r.x + faderScreenRange * ((value - minValue) / (maxValue - minValue));
  ...
  GUIContent bypassButtonContent =
      EditorGUIUtility.TrTextContent("", "Toggle bypass on this effect");
  void
  EffectSlot(Rect effectRect, AudioMixerSnapshotController snapshot,
             AudioMixerEffectController effect, int effectIndex,
             ref int highlightEffectIndex, ChannelStripParams p,
             ref Dictionary<AudioMixerEffectController, PatchSlot> patchslots) {
    if (effect == null)
      return;
    ...
    if (evt.type == EventType.Repaint && patchslots != null &&
        (effect.IsSend() ||
         MixerEffectDefinitions.EffectCanBeSidechainTarget(effect))) {
      var c = new PatchSlot();
      c.group = p.group;
    ...
    Rect bypassRect = r;
    bypassRect.width = bypassWidth;
    r.xMin += bypassWidth;
    if (isBypassable) {
      // Handle mouse input is first, rendering is further down (to render on
      // top).
    ...
    float level =
        (effect != null)
            ? Mathf.Clamp(effect.GetValueForMixLevel(m_Controller, snapshot),
                          AudioMixerController.kMinVolume,
                          AudioMixerController.kMaxEffect)
            : AudioMixerController.kMinVolume;
    bool showLevel =
        (effect != null) &&
        ((effect.IsSend() && effect.sendTarget != null) || effect.enableWetMix);
    if (evt.type == EventType.Repaint) {
      GUIStyle style = GetEffectBarStyle(effect);

      float drawLevel = (showLevel)
                            ? ((level - AudioMixerController.kMinVolume) /
                               (AudioMixerController.kMaxEffect -
                                AudioMixerController.kMinVolume))
                            : 1.0f;
      bool enabled =
          (!p.group.bypassEffects && (effect == null || !effect.bypass)) ||
          (effect != null && effect.DisallowsBypass());
      Color col1 = (effect != null) ? AudioMixerDrawUtils.GetEffectColor(effect)
      ...
      if (enabled) {
        if (drawLevel < 1.0f) {
          // Bar forground (we need special for small widths because style
          // rendering with borders have a min width to work)
          ...
          if (forgroundWidth < minimumWidthForBorderSetup) {
            // Forground less than minimumWidthForBorderSetup (Always show
            // marker to indicate the slot is draggable)
            ...
            GUI.DrawTextureWithTexCoords(
                new Rect(r.x, r.y, forgroundWidth, r.height),
                style.focused.background, new Rect(frac, 0f, 1f - frac, 1f));
            GUI.color = orgColor;
          } else {
            // Forground larger than minimumWidthForBorderSetup (uses border
            // setup)
            style.Draw(new Rect(r.x, r.y, forgroundWidth, r.height), false,
                       false, false, true);
          }

          // Bar background
          GUI.DrawTexture(new Rect(r.x + forgroundWidth, r.y,
                                   r.width - forgroundWidth, r.height),
                          style.onFocused.background, ScaleMode.StretchToFill);
        } else {
          // GUI.DrawTexture(r, showLevel ? style.focused.background :
          // style.hover.background, ScaleMode.StretchToFill);
      ...
      // Bypass toggle
      if (isBypassable)
        styles.circularToggle.Draw(new Rect(bypassRect.x + 2, bypassRect.y + 5f,
                                            bypassRect.width - 2,
                                            bypassRect.width - 2f),
                                   false, false, !effect.bypass, false);
      ...
      using(new EditorGUI.DisabledScope(!enabled)) {
        ...
        GUI.Label(new Rect(r.x, r.y, r.width - bypassWidth, r.height),
                  GUIContent.Temp(name, tooltip), styles.effectName);
      }
    } else {
      EffectSlotDragging(effectRect, snapshot, effect, showLevel, level,
                         effectIndex, ref highlightEffectIndex, p);
    }
  }

  string GetEffectSlotName(AudioMixerEffectController effect, bool showLevel,
                           AudioMixerSnapshotController snapshot,
                           ChannelStripParams p) {
    if (m_ChangingWetMixIndex == m_IndexCounter && showLevel) {
      return UnityString.Format(
          "{0:F1} dB", effect.GetValueForMixLevel(m_Controller, snapshot));
    }
  ...
  string GetEffectSlotTooltip(AudioMixerEffectController effect,
                              Rect effectRect, ChannelStripParams p) {
    ...
    if (effect.IsSend()) {
      if (effect.sendTarget != null) {
        ...
        return "Send to: " +
               sendTarget; // We add the tooltip here because we rarely
      } else {
  ...
  private void EffectSlotDragging(Rect r, AudioMixerSnapshotController snapshot,
                                  AudioMixerEffectController effect,
                                  bool showLevel, float level, int effectIndex,
                                  ref int highlightEffectIndex,
                                  ChannelStripParams p) {
    Event evt = Event.current;
    ...
    case EventType.MouseUp:
      if (GUIUtility.hotControl == m_EffectInteractionControlID &&
          evt.button == 0 && p.stripRect.Contains(evt.mousePosition)) {
        if (m_MovingEffectDstIndex != -1 && m_MovingEffectAllowed) {
          if (IsDuplicateKeyPressed() && CanDuplicateDraggedEffect()) {
            // Duplicate effect
            AudioMixerEffectController sourceEffect =
                m_MovingSrcGroup.effects[m_MovingEffectSrcIndex];
            AudioMixerEffectController copiedEffect =
                m_MovingSrcGroup.controller.CopyEffect(sourceEffect);
            var targetEffects = m_MovingDstGroup.effects.ToList();
            if (AudioMixerController.InsertEffect(
                    copiedEffect, ref targetEffects, m_MovingEffectDstIndex)) {
            ...
            if (m_MovingSrcGroup == m_MovingDstGroup) {
              ...
              if (AudioMixerController.MoveEffect(
                      ref effects, m_MovingEffectSrcIndex, ref effects,
                      m_MovingEffectDstIndex)) {
                m_MovingSrcGroup.effects = effects.ToArray();
              }
            } else if (!m_MovingSrcGroup
                            .effects [m_MovingEffectSrcIndex]
                            .IsAttenuation()) {
              var sourceEffects = m_MovingSrcGroup.effects.ToList();
              var targetEffects = m_MovingDstGroup.effects.ToList();
              if (AudioMixerController.MoveEffect(
                      ref sourceEffects, m_MovingEffectSrcIndex,
                      ref targetEffects, m_MovingEffectDstIndex)) {
        ...
        GUIUtility
            .ExitGUI(); // We changed order of effects so stop iterating effects
      }
      break;
    ...
    case EventType.MouseDrag:
      ...
      if (GUIUtility.hotControl == m_EffectInteractionControlID) {
        // Detect direction of drag to decide if we want to adjust value or move
        // effect
        ...
        if (IsMovingEffect() && p.stripRect.Contains(evt.mousePosition)) {
          ...
          float max = effectIndex == p.group.effects.Length - 1 ? r.height + h2
                                                                : r.height;
          float dy = evt.mousePosition.y - r.y;
          if (dy >= min && dy <= max && effectIndex < p.group.effects.Length) {
            int newMovingEffectDstIndex =
                (dy < h2) ? effectIndex : (effectIndex + 1);
            if (newMovingEffectDstIndex != m_MovingEffectDstIndex ||
                m_MovingDstGroup != p.group) {
              ...
              m_MovingEffectAllowed =
                  !(m_MovingSrcGroup
                        .effects [m_MovingEffectSrcIndex]
                        .IsAttenuation() &&
                    m_MovingSrcGroup != m_MovingDstGroup) &&
                  !AudioMixerController.WillMovingEffectCauseFeedback(
                      p.allGroups, m_MovingSrcGroup, m_MovingEffectSrcIndex,
                      m_MovingDstGroup, newMovingEffectDstIndex, null) &&
                  (!IsDuplicateKeyPressed() || CanDuplicateDraggedEffect());
            }
            evt.Use(); // We use when having valid pos so because if drag event
                       // is not used after all strips we clear drag destination
                       // to remove insert hightlight for invalid positions
          }
        }
        ...
        if (IsAdjustingWetMix() && HasFocus()) {
          if (showLevel) {
            ...
            float tmp =
                kEffectScaleMouseDrag * HandleUtility.niceMouseDelta + level;
            float deltaLevel = Mathf.Clamp(tmp, AudioMixerController.kMinVolume,
                                           AudioMixerController.kMaxEffect) -
                               level;
            if (deltaLevel != 0.0f) {
              Undo.RecordObject(m_Controller.TargetSnapshot,
                                "Change effect level");
              if (effect.IsSend() && m_Controller.CachedSelection.Count > 1 &&
                  m_Controller.CachedSelection.Contains(p.group)) {
                List<AudioMixerEffectController> changeEffects =
                    new List<AudioMixerEffectController>();
                foreach (var g in m_Controller.CachedSelection)
                  foreach (var e in g.effects)
                    if (e.effectName == effect.effectName &&
                        e.sendTarget == effect.sendTarget)
                      changeEffects.Add(e);
                foreach (var e in changeEffects)
                  if (!e.IsSend() || e.sendTarget != null)
                    e.SetValueForMixLevel(
                        m_Controller, snapshot,
                        Mathf.Clamp(
                            e.GetValueForMixLevel(m_Controller, snapshot) +
                                deltaLevel,
                            AudioMixerController.kMinVolume,
                            AudioMixerController.kMaxEffect));
              } else {
    ...
    case EventType.Repaint:
      ...
      if (IsMovingEffect()) {
        ...
        if (evt.type == EventType.Repaint) {
          ...
          var cursor = (IsDuplicateKeyPressed() && m_MovingEffectAllowed)
                           ? MouseCursor.ArrowPlus
                           : MouseCursor.ResizeVertical;
          EditorGUIUtility.AddCursorRect(new Rect(evt.mousePosition.x - 10,
                                                  evt.mousePosition.y - 10, 20,
                                                  20),
                                         cursor, m_EffectInteractionControlID);
        }
      }

      if (m_MovingEffectDstIndex != -1 &&
          m_MovingDstRect.y >=
              0.0f) // && m_MovingEffectDstIndex != m_MovingEffectSrcIndex &&
                    // m_MovingEffectDstIndex != m_MovingEffectSrcIndex +1)
      {
        float kMoveRange = 2;
        Color moveSlotColLo = (m_MovingEffectAllowed)
                                  ? kMoveSlotColLoAllowed
                                  : kMoveSlotColLoDisallowed;
        Color moveSlotColHi = (m_MovingEffectAllowed)
                                  ? kMoveSlotColHiAllowed
                                  : kMoveSlotColHiDisallowed;
        Color moveSlotColBorder = (m_MovingEffectAllowed)
                                      ? kMoveSlotColBorderAllowed
                                      : kMoveSlotColBorderDisallowed;
        AudioMixerDrawUtils.DrawGradientRect(
            new Rect(m_MovingDstRect.x, m_MovingDstRect.y - kMoveRange,
                     m_MovingDstRect.width, kMoveRange),
            moveSlotColLo, moveSlotColHi);
        AudioMixerDrawUtils.DrawGradientRect(
            new Rect(m_MovingDstRect.x, m_MovingDstRect.y,
                     m_MovingDstRect.width, kMoveRange),
            moveSlotColHi, moveSlotColLo);
        AudioMixerDrawUtils.DrawGradientRect(
            new Rect(m_MovingDstRect.x, m_MovingDstRect.y - 1,
                     m_MovingDstRect.width, 1),
            moveSlotColBorder, moveSlotColBorder);
      }
  ...
  bool IsDuplicateKeyPressed() { return Event.current.alt; }

  bool CanDuplicateDraggedEffect() {
    if (IsMovingEffect() && m_MovingSrcGroup != null)
      return !m_MovingSrcGroup
                  .effects [m_MovingEffectSrcIndex]
                  .IsAttenuation();
    return false;
  }
  ...
  private class BusConnection {
    public BusConnection(float srcX, float srcY,
                         AudioMixerEffectController targetEffect,
                         float mixLevel, Color col, bool isSend,
                         bool isSelected) {
      this.srcX = srcX;
      this.srcY = srcY;
      this.targetEffect = targetEffect;
  ...
  private void
  RecordSelectedGroupUndoState(List<AudioMixerGroupController> selection,
                               string text) {
  ...
  private bool DoSoloButton(Rect r, AudioMixerGroupController group,
    ...
    if (evt.type == EventType.MouseUp && evt.button == 1 &&
        r.Contains(evt.mousePosition) && allGroups.Any(g => g.solo)) {
    ...
    bool newState = GUI.Toggle(r, group.solo, styles.soloGUIContent,
                               AudioMixerDrawUtils.styles.soloToggle);
  ...
  private bool DoMuteButton(Rect r, AudioMixerGroupController group,
                            List<AudioMixerGroupController> allGroups,
                            bool anySoloActive,
                            List<AudioMixerGroupController> selection) {
    Event evt = Event.current;
    // Right click toggle
    if (evt.type == EventType.MouseUp && evt.button == 1 &&
        r.Contains(evt.mousePosition) && allGroups.Any(g => g.mute)) {
    ...
    bool newState = GUI.Toggle(r, group.mute, styles.muteGUIContent,
                               AudioMixerDrawUtils.styles.muteToggle);
    if (dimColor)
      GUI.color = orgColor;
  ...
  private bool
  DoBypassEffectsButton(Rect r, AudioMixerGroupController group,
                        List<AudioMixerGroupController> allGroups,
                        List<AudioMixerGroupController> selection) {
    Event evt = Event.current;
    // Right click toggle
    if (evt.type == EventType.MouseUp && evt.button == 1 &&
        r.Contains(evt.mousePosition) && allGroups.Any(g => g.bypassEffects)) {
    ...
    bool newState = GUI.Toggle(r, group.bypassEffects, styles.bypassGUIContent,
                               AudioMixerDrawUtils.styles.bypassToggle);
  ...
  void GroupClicked(AudioMixerGroupController clickedGroup,
                    ChannelStripParams p, bool clickedControlInGroup) {
    // Get ids from items
    ...
    List<int> newSelection = InternalEditorUtility.GetNewSelection(
        clickedGroup.GetInstanceID(), allIDs, selectedIDs, lastClickedID,
        keepMultiSelection, useShiftAsActionKey, allowMultiselection);
    List<AudioMixerGroupController> groups =
        (from x in p.allGroups where newSelection.Contains(x.GetInstanceID())
             select x)
            .ToList();
  ...
  private void DoAttenuationFader(Rect r, AudioMixerGroupController group,
                                  List<AudioMixerGroupController> selection,
                                  GUIStyle style) {
    float volume = Mathf.Clamp(
        group.GetValueForVolume(m_Controller, m_Controller.TargetSnapshot),
        AudioMixerController.kMinVolume, AudioMixerController.GetMaxVolume());
    float newVolume = VerticalFader(r, volume, 1, kVolumeScaleMouseDrag, true,
                                    true, styles.attenuationFader.tooltip,
                                    AudioMixerController.GetMaxVolume(), style);
    if (volume != newVolume) {
      float deltaVolume = newVolume - volume;
      Undo.RecordObject(m_Controller.TargetSnapshot, "Change volume fader");
      foreach (var g in selection) {
        float vol = Mathf.Clamp(
            g.GetValueForVolume(m_Controller, m_Controller.TargetSnapshot),
            AudioMixerController.kMinVolume,
            AudioMixerController.GetMaxVolume());
        g.SetValueForVolume(m_Controller, m_Controller.TargetSnapshot,
                            Mathf.Clamp(vol + deltaVolume,
                                        AudioMixerController.kMinVolume,
                                        AudioMixerController.GetMaxVolume()));
      }
      InspectorWindow.RepaintAllInspectors();
  ...
  static internal void AddEffectItemsToMenu(AudioMixerController controller,
                                            AudioMixerGroupController[] groups,
                                            int insertIndex, string prefix,
                                            GenericMenu pm) {
    string[] effectNames = MixerEffectDefinitions.GetEffectList();
    for (int t = 0; t < effectNames.Length; t++) {
      if (effectNames[t] != "Attenuation")
        pm.AddItem(
            new GUIContent(prefix + AudioMixerController.FixNameForPopupMenu(
                                        effectNames[t])),
            false, InsertEffectPopupCallback,
            new EffectContext(controller, groups, insertIndex, effectNames[t]));
    }
  }

  private void DoEffectSlotInsertEffectPopup(
      Rect buttonRect, AudioMixerGroupController group,
      List<AudioMixerGroupController> allGroups, int effectSlotIndex,
      ref Dictionary<AudioMixerEffectController, AudioMixerGroupController>
          effectMap) {
    ...
    if (effectSlotIndex < group.effects.Length) {
      ...
      if (!effect.IsAttenuation() && !effect.IsSend() && !effect.IsReceive() &&
          !effect.IsDuckVolume()) {
        pm.AddItem(EditorGUIUtility.TrTextContent(
                       "Allow Wet Mixing (causes higher memory usage)"),
                   effect.enableWetMix,
                   delegate { effect.enableWetMix = !effect.enableWetMix; });
        pm.AddItem(EditorGUIUtility.TrTextContent("Bypass"), effect.bypass,
                   delegate {
                     effect.bypass = !effect.bypass;
                     m_Controller.UpdateBypass();
                     InspectorWindow.RepaintAllInspectors();
      ...
      AddEffectItemsToMenu(group.controller, groups, effectSlotIndex,
                           "Add effect before/", pm);
      AddEffectItemsToMenu(group.controller, groups, effectSlotIndex + 1,
                           "Add effect after/", pm);
    } else {
    ...
    if (effectSlotIndex < group.effects.Length) {
      ...
      if (!effect.IsAttenuation()) {
        pm.AddSeparator("");
        pm.AddItem(
            EditorGUIUtility.TrTextContent("Remove"), false,
            RemoveEffectPopupCallback,
            new EffectContext(m_Controller, groups, effectSlotIndex, ""));
        ...
        if (effect.IsSend()) {
          if (effect.sendTarget != null) {
            ...
            pm.AddItem(EditorGUIUtility.TrTextContent(
                           "Disconnect from '" +
                           effect.GetSendTargetDisplayString(effectMap) + "'"),
                       false, ConnectSendPopupCallback,
                       new ConnectSendContext(effect, null));
          }

          if (!insertedSeparator)
            AddSeperatorIfAnyReturns(pm, allGroups);
          AddMenuItemsForReturns(pm, "Connect to ", effectSlotIndex, group,
                                 allGroups, effectMap, effect, false);
        }
      }
    }
  ...
  void AddSeperatorIfAnyReturns(GenericMenu pm,
                                List<AudioMixerGroupController> allGroups) {
    foreach (var g in allGroups) {
  ...
  public static void
  AddMenuItemsForReturns(GenericMenu pm, string prefix, int effectIndex,
                         AudioMixerGroupController group,
                         List<AudioMixerGroupController> allGroups,
                         Dictionary<AudioMixerEffectController,
                                    AudioMixerGroupController> effectMap,
                         AudioMixerEffectController effect, bool showCurrent) {
    foreach (var g in allGroups) {
      foreach (var ge in g.effects) {
        if (MixerEffectDefinitions.EffectCanBeSidechainTarget(ge)) {
          var identifiedLoop = new List<AudioMixerController.ConnectionNode>();

          // TODO: In the future, consider allowing feedback loops, as the send
          // code now allows for this, though give a warning to the user
          // that they may be doing something stupid
          if (!AudioMixerController.WillChangeOfEffectTargetCauseFeedback(
                  allGroups, group, effectIndex, ge, identifiedLoop)) {
            if (showCurrent || effect.sendTarget != ge)
              pm.AddItem(new GUIContent(prefix + "'" +
                                        ge.GetDisplayString(effectMap) + "'"),
                         effect.sendTarget == ge, ConnectSendPopupCallback,
                         new ConnectSendContext(effect, ge));
          } else {
            string baseString = "A connection to '" +
                                AudioMixerController.FixNameForPopupMenu(
                                    ge.GetDisplayString(effectMap)) +
                                "' would result in a feedback loop/";
            ...
            pm.AddDisabledItem(
                new GUIContent(baseString + loopIndex + ": ..."));
          }
        }
      }
    }
  ...
  public void VUMeter(AudioMixerGroupController group, Rect r, float level,
                      float peak) {
    EditorGUI.VUMeter.VerticalMeter(
        r, level, peak, EditorGUI.VUMeter.verticalVUTexture, Color.grey);
  }
  ...
  class ChannelStripParams {
    ...
    public List<AudioMixerGroupController> rectSelectionGroups =
        new List<AudioMixerGroupController>();
    ...
    public Dictionary<AudioMixerEffectController, AudioMixerGroupController>
        effectMap;
    ...
    public void Init(AudioMixerController controller, Rect channelStripRect,
                     int maxNumEffects) {
      numChannels = controller.GetGroupVUInfo(group.groupID, false,
                                              vuinfo_level, vuinfo_peak);
    ...
    List<Rect> GetBackgroundRects(Rect r, AudioMixerGroupController group,
                                  int maxNumGroups) {
      ...
      int maxNumEffectSlots =
          maxNumGroups; // maxNumGroups includes an empty slot used as button
                        // for adding new effect
  ...
  void DrawBackgrounds(ChannelStripParams p, bool selected) {
    if (Event.current.type == EventType.Repaint) {
      ...
      Color lineColor =
          EditorGUIUtility.isProSkin ? new Color(darkColor, darkColor, darkColor)
          : new Color(lightColor, lightColor, lightColor);
      ...
      // vuNumbersRect.x = vuNumbersRect.xMax -
      // AudioMixerDrawUtils.styles.channelStripVUMeterBg.normal.background.width;
      // // right align
      // AudioMixerDrawUtils.styles.channelStripVUMeterBg.Draw(vuNumbersRect,
      // false, false, false, false); // 0 -80 dB numbers overlay
    }
  ...
  void OpenGroupContextMenu(AudioMixerGroupController[] groups) {
    ...
    AddEffectItemsToMenu(groups[0].controller, groups, 0, "Add effect at top/",
                         pm);
    AddEffectItemsToMenu(groups[0].controller, groups, -1,
                         "Add effect at bottom/", pm);
  ...
  private void DrawChannelStrip(
      ChannelStripParams p, ref int highlightEffectIndex,
      ref Dictionary<AudioMixerEffectController, PatchSlot> patchslots,
      bool showBusConnectionsOfSelection) {
    Event evt = Event.current;

    bool mouseDownInsideStrip = evt.type == EventType.MouseDown &&
                                p.stripRect.Contains(evt.mousePosition);
    ...
    // Header area
    headerGUIContent.text = headerGUIContent.tooltip =
        p.group.GetDisplayString();
    GUI.Label(p.bgRects[p.kHeaderIndex], headerGUIContent,
              AudioMixerDrawUtils.styles.channelStripHeaderStyle);

    // VU Meter and attenuation area
    Rect innerRect =
        new RectOffset(-6, 0, 0, -4).Add(p.bgRects[p.kVUMeterFaderIndex]);

    float spaceBetweenColumns = 1f;
    float column1Width = 54f;
    float column2Width = innerRect.width - column1Width - spaceBetweenColumns;
    Rect column1Rect =
        new Rect(innerRect.x, innerRect.y, column1Width, innerRect.height);
    Rect column2Rect = new Rect(column1Rect.xMax + spaceBetweenColumns,
                                innerRect.y, column2Width, innerRect.height);

    float faderWidth = 29f;
    Rect faderRect =
        new Rect(column2Rect.x, column2Rect.y, faderWidth, column2Rect.height);
    Rect tripleButtonRect = p.bgRects[p.kSoloMuteBypassIndex];
    GUIStyle attenuationMarkerStyle =
        AudioMixerDrawUtils.styles.channelStripAttenuationMarkerSquare;
    using(new EditorGUI.DisabledScope(
        !AudioMixerController.EditingTargetSnapshot())) {
      DoVUMeters(column1Rect, attenuationMarkerStyle.fixedHeight, p);
      DoAttenuationFader(faderRect, p.group, m_Controller.CachedSelection,
                         attenuationMarkerStyle);
      DoTotaldB(p);
      DoEffectList(p, selected, ref highlightEffectIndex, ref patchslots,
                   showBusConnectionsOfSelection);
    }

    // We want to be able to set mute, solo and bypass even when
    // EditingTargetSnapshot is false for testing during gameplay
    DoSoloMuteBypassButtons(tripleButtonRect, p.group, p.allGroups,
                            m_Controller.CachedSelection, p.anySoloActive);

    // Handle group click after all controls so we can detect if a control was
    // clicked by checking if the event was used here
    if (mouseDownInsideStrip && evt.button == 0)
      GroupClicked(p.group, p, evt.type == EventType.Used);

    // Check context click after all UI controls (only show if we did not
    // interact with any controls)
    if (evt.type == EventType.ContextClick &&
        p.stripRect.Contains(evt.mousePosition)) {
      evt.Use();

      // If its part of an existing selection, then pass all, otherwise pretend
      // its a new selection
      if (selected)
  ...
  void DoTotaldB(ChannelStripParams p) {
    ...
    float textWidth = 50f;
    styles.totalVULevel.padding.right =
        (int)((p.stripRect.width - textWidth) * 0.5f);
  ...
  void
  DoEffectList(ChannelStripParams p, bool selected,
               ref int highlightEffectIndex,
               ref Dictionary<AudioMixerEffectController, PatchSlot> patchslots,
               bool showBusConnectionsOfSelection) {
    Event evt = Event.current;
    for (int i = 0; i < p.maxEffects; i++) {
      ...
      if (i < p.group.effects.Length) {
        ...
        if (p.visible) {
          ...
          if (evt.type == EventType.ContextClick &&
              effectRect.Contains(Event.current.mousePosition)) {
            ClearFocus();
            DoEffectSlotInsertEffectPopup(effectRect, p.group, p.allGroups, i,
                                          ref p.effectMap);
            evt.Use();
          }

          // Then do effect button
          EffectSlot(effectRect, m_Controller.TargetSnapshot, effect, i,
                     ref highlightEffectIndex, p, ref patchslots);
        }
      }
    }
    ...
    if (p.visible) {
      ...
      if (evt.type == EventType.Repaint) {
        GUI.DrawTextureWithTexCoords(
            new Rect(effectRect.x, effectRect.y, effectRect.width,
                     effectRect.height - 1),
            styles.effectBar.hover.background, new Rect(0, 0.5f, 0.1f, 0.1f));
      ...
      if (evt.type == EventType.MouseDown &&
        ...
        DoEffectSlotInsertEffectPopup(effectRect, p.group, p.allGroups,
                                      insertEffectAtIndex, ref p.effectMap);
        evt.Use();
      }
    }
  }

  float DoVUMeters(Rect vuRect, float attenuationMarkerHeight,
                   ChannelStripParams p) {
    float spaceBetweenMeters = 1f;

    // When the audiomixer has been rebuild we might have some frames where
    // numChannels is not initialized. To prevent flickering we use last valid
    // num channels used at current index
    ...
    // This can happen when simulating player builds, i.e. not calculating
    // VU-info for most of the channels.
    if (numChannels == 0)
      return vuRect.x;
    ...
    float vuWidth = Mathf.Round(
        (vuRect.width - numChannels * spaceBetweenMeters) / numChannels);
    Rect vuSubRect =
        new Rect(vuRect.xMax - vuWidth, vuRect.y, vuWidth, vuRect.height);
    // Draw from right to left to ensure perfect right side pixel position (we
    // are rounding widths above)
    for (int i = numChannels - 1; i >= 0; i--) {
      ...
      float warpedLevel =
          1f -
          AudioMixerController.VolumeToScreenMapping(
              Mathf.Clamp(p.vuinfo_level[i], AudioMixerController.kMinVolume,
                          AudioMixerController.GetMaxVolume()),
              1f, true);
      float warpedPeak =
          1f -
          AudioMixerController.VolumeToScreenMapping(
              Mathf.Clamp(p.vuinfo_peak[i], AudioMixerController.kMinVolume,
                          AudioMixerController.GetMaxVolume()),
              1f, true);
      VUMeter(p.group, vuSubRect, warpedLevel, warpedPeak);
    }
    AudioMixerDrawUtils.AddTooltipOverlay(vuRect,
                                          styles.vuMeterGUIContent.tooltip);

    return vuSubRect.x;
  }

  void DoSoloMuteBypassButtons(Rect rect, AudioMixerGroupController group,
                               List<AudioMixerGroupController> allGroups,
                               List<AudioMixerGroupController> selection,
                               bool anySoloActive) {
    ...
    float startX =
        rect.x + (rect.width - (buttonSize * 3 + spaceBetween * 2)) * 0.5f;
    ...
    needMixerUpdate |=
        DoMuteButton(buttonRect, group, allGroups, anySoloActive, selection);
    if (needMixerUpdate)
  ...
  [System.NonSerialized]
  private GUIStyle developerInfoStyle =
      AudioMixerDrawUtils.BuildGUIStyleForLabel(
          new Color(1.0f, 0.0f, 0.0f, 0.5f), 20, false, FontStyle.Bold,
          TextAnchor.MiddleLeft);

  public void ShowDeveloperOverlays(Rect rect, Event evt, bool show) {
    if (show && Unsupported.IsDeveloperMode() &&
        evt.type == EventType.Repaint) {
      AudioMixerDrawUtils.ReadOnlyLabel(
          new Rect(rect.x + 5, rect.y + 5, rect.width - 10, 20),
          "Current snapshot: " + m_Controller.TargetSnapshot.name,
          developerInfoStyle);
      AudioMixerDrawUtils.ReadOnlyLabel(
          new Rect(rect.x + 5, rect.y + 25, rect.width - 10, 20),
          "Frame count: " + FrameCounter++, developerInfoStyle);
    }
  }
  ...
  public static void GetCableVertex(float x1, float y1, float x2, float y2,
                                    float x3, float y3, float t, out float x,
                                    out float y) {
  ...
  public void OnGUI(Rect rect, bool showReferencedBuses,
                    bool showBusConnections, bool showBusConnectionsOfSelection,
                    List<AudioMixerGroupController> allGroups,
                    Dictionary<AudioMixerEffectController,
                               AudioMixerGroupController> effectMap,
                    bool sortGroupsAlphabetically, bool showDeveloperOverlays,
                    AudioMixerGroupController scrollToItem) {
    ...
    m_RectSelectionControlID =
        GUIUtility.GetControlID(kRectSelectionHashCode, FocusType.Passive);
    m_EffectInteractionControlID =
        GUIUtility.GetControlID(kEffectDraggingHashCode, FocusType.Passive);
    ...
    Rect baseChannelStripRect =
        new Rect(channelStripsOffset.x, channelStripsOffset.y,
                 channelStripBaseWidth, 300);
    if (scrollToItem != null) {
      int index = sortedGroups.IndexOf(scrollToItem);
      if (index >= 0) {
        float x = (baseChannelStripRect.width + channelStripSpacing) * index -
                  m_State.m_ScrollPos.x;
    ...
    foreach (var c in sortedGroups) {
      foreach (var e in c.effects) {
        if (e.sendTarget != null) {
          ...
          if (!unsortedBuses.Contains(targetGroup) &&
              !sortedGroups.Contains(targetGroup))
    ...
    Rect contentRect =
        GetContentRect(sortedGroups, isShowingReferencedGroups, maxEffects);
    m_State.m_ScrollPos =
        GUI.BeginScrollView(rect, m_State.m_ScrollPos, contentRect);
    DrawAreaBackground(new Rect(0, 0, 10000, 10000));

    // Setup shared AudioGroup data
    var channelStripParams = new ChannelStripParams{
        effectMap = effectMap, allGroups = allGroups,
        shownGroups = sortedGroups, anySoloActive = allGroups.Any(g => g.solo),
        visibleRect = new Rect(m_State.m_ScrollPos.x, m_State.m_ScrollPos.y,
                               rect.width, rect.height)};

    var patchslots =
        showBusConnections ? new Dictionary<AudioMixerEffectController,
                                           PatchSlot>()
        : null;
    for (int i = 0; i < sortedGroups.Count; ++i) {
      ...
      channelStripParams.visible =
          RectOverlaps(channelStripParams.visibleRect, baseChannelStripRect);
      channelStripParams.Init(m_Controller, baseChannelStripRect, maxEffects);

      DrawChannelStrip(channelStripParams,
                       ref m_Controller.m_HighlightEffectIndex, ref patchslots,
                       showBusConnectionsOfSelection);

      // If we click inside the the strip rect we use the event to ensure the
      // mousedown is not caught by rectangle selection
      if (evt.type == EventType.MouseDown && evt.button == 0 &&
          channelStripParams.stripRect.Contains(evt.mousePosition))
        evt.Use();
      // If we are dragging effects we reset destination index when dragging
      // outside effects
      if (IsMovingEffect() && evt.type == EventType.MouseDrag &&
          channelStripParams.stripRect.Contains(evt.mousePosition) &&
          GUIUtility.hotControl == m_EffectInteractionControlID) {
        m_MovingEffectDstIndex = -1;
        evt.Use();
      }
      baseChannelStripRect.x +=
          channelStripParams.stripRect.width + channelStripSpacing;

      // Add extra space to referenced groups
      if (showReferencedBuses && i == numMainGroups - 1 &&
          sortedGroups.Count > numMainGroups) {
        baseChannelStripRect.x += 50f;

        using(new EditorGUI.DisabledScope(true)) {
          GUI.Label(new Rect(baseChannelStripRect.x,
                             channelStripParams.stripRect.yMax, 130, 22),
                    styles.referencedGroups, styles.channelStripHeaderStyle);
        }
      }
    }
    ...
    if (evt.type == EventType.Repaint && patchslots != null) {
      foreach (var c in patchslots) {
        var slot = c.Value;
        bool active = !showBusConnectionsOfSelection ||
                      m_Controller.CachedSelection.Contains(slot.group);
        if (active)
          styles.circularToggle.Draw(new Rect(slot.x - 3, slot.y - 3, 6, 6),
                                     false, false, active, false);
      }
      float moveamp = Mathf.Exp(-0.03f * Time.time * Time.time) * 0.1f;
      var trCol1 = new Color(
          0.0f, 0.0f, 0.0f,
          AudioMixerController.EditingTargetSnapshot() ? 0.1f : 0.05f);
      var trCol2 =
      ...
      foreach (var c in patchslots) {
        ...
        float mx1 = Mathf.Abs(targetSlot.x - sourceSlot.x) *
                        Mathf.Sin(Time.time * 5.0f + phase) * moveamp +
                    (sourceSlot.x + targetSlot.x) * 0.5f;
        float my1 =
            Mathf.Abs(targetSlot.y - sourceSlot.y) *
                Mathf.Cos(Time.time * 5.0f + phase) * moveamp +
            Math.Max(sourceSlot.y, targetSlot.y) +
            Mathf.Max(
                Mathf.Min(0.5f * Math.Abs(targetSlot.y - sourceSlot.y), 50.0f),
                50.0f);
        for (int i = 0; i < cablepoints.Length; i++)
          GetCableVertex(sourceSlot.x, sourceSlot.y, mx1, my1, targetSlot.x,
                         targetSlot.y,
                         (float) i / (float)(cablepoints.Length - 1),
                         out cablepoints[i].x, out cablepoints[i].y);
        bool skip = showBusConnectionsOfSelection &&
                    !m_Controller.CachedSelection.Contains(sourceSlot.group) &&
                    !m_Controller.CachedSelection.Contains(targetSlot.group);
        ...
        Handles.color = new Color(
            (color & 3) * 0.15f + 0.55f, ((color >> 2) & 3) * 0.15f + 0.55f,
            ((color >> 4) & 3) * 0.15f + 0.55f,
            AudioMixerController.EditingTargetSnapshot() ? 1.0f : 0.5f);
        Handles.DrawAAPolyLine(4.0f, cablepoints.Length, cablepoints);
        Handles.color = new Color(
            1.0f, 1.0f, 1.0f,
            AudioMixerController.EditingTargetSnapshot() ? 0.5f : 0.25f);
    ...
    AudioMixerDrawUtils.DrawScrollDropShadow(rect, m_State.m_ScrollPos.y,
                                             contentRect.height);
  ...
  bool IsMovingEffect() { return m_MovingEffectSrcIndex != -1; }

  bool IsAdjustingWetMix() { return m_ChangingWetMixIndex != -1; }
  ...
  void RectSelection(ChannelStripParams channelStripParams) {
    ...
    if (evt.type == EventType.MouseDown && evt.button == 0 &&
      ...
      m_RectSelectionRect =
          new Rect(m_RectSelectionStartPos.x, m_RectSelectionStartPos.y, 0, 0);
      Event.current.Use();
      InspectorWindow.RepaintAllInspectors();
    }
    ...
    if (evt.type == EventType.MouseDrag) {
      if (IsRectSelectionActive()) {
        m_RectSelectionRect.x =
            Mathf.Min(m_RectSelectionStartPos.x, evt.mousePosition.x);
        m_RectSelectionRect.y =
            Mathf.Min(m_RectSelectionStartPos.y, evt.mousePosition.y);
        m_RectSelectionRect.width =
            Mathf.Max(m_RectSelectionStartPos.x, evt.mousePosition.x) -
            m_RectSelectionRect.x;
        m_RectSelectionRect.height =
            Mathf.Max(m_RectSelectionStartPos.y, evt.mousePosition.y) -
            m_RectSelectionRect.y;
        Event.current.Use();
      }
    ...
    if (IsRectSelectionActive() &&
        evt.GetTypeForControl(m_RectSelectionControlID) == EventType.MouseUp) {
      var newChannelStripSelection =
          (evt.shift) ? m_Controller.CachedSelection
                      : new List<AudioMixerGroupController>();
    ...
    if (evt.type == EventType.Repaint) {
      if (IsRectSelectionActive()) {
        ...
        AudioMixerDrawUtils.DrawGradientRectHorizontal(m_RectSelectionRect,
                                                       rectSelCol, rectSelCol);
      }
    }
  }

  void WarningOverlay(List<AudioMixerGroupController> allGroups, Rect rect,
                      Rect contentRect) {
    ...
    foreach (var g in allGroups) {
      ...
      if (g.bypassEffects)
        numBypassEffects +=
            g.effects.Length - 1; // one of the effects is "Attenuation"
      else
    ...
    // Warning overlay
    if (evt.type == EventType.Repaint && numSoloed > 0 || numMuted > 0 ||
        numBypassEffects > 0) {
      string warningStr =
          "Warning: " +
          ((numSoloed > 0)
               ? (numSoloed + (numSoloed > 1 ? " groups" : " group") +
                  " currently soloed")
               : (numMuted > 0)
                     ? (numMuted + (numMuted > 1 ? " groups" : " group") +
                        " currently muted")
                     : (numBypassEffects +
                        (numBypassEffects > 1 ? " effects" : " effect") +
                        " currently bypassed"));
      ...
      float textwidth =
          styles.warningOverlay.CalcSize(GUIContent.Temp(warningStr)).x;
      Rect warningRect = new Rect(
          rect.x + margin +
              Mathf.Max((rect.width - 2 * margin - textwidth) * 0.5f, 0f),
          rect.yMax - styles.warningOverlay.fixedHeight - margin -
              (horizontalScrollbar ? 17f : 0f),
          textwidth, styles.warningOverlay.fixedHeight);

      GUI.Label(warningRect, GUIContent.Temp(warningStr),
                styles.warningOverlay);
    }
  }

  Rect GetContentRect(List<AudioMixerGroupController> sortedGroups,
                      bool isShowingReferencedGroups, int maxEffects) {
    float fixedHeight =
        headerHeight + vuHeight + dbHeight + soloMuteBypassHeight;
    float maxHeight = channelStripsOffset.y + fixedHeight +
                      (maxEffects * effectHeight) + 10 + effectHeight + 10;

    float maxWidth =
        channelStripsOffset.x * 2 +
        (channelStripBaseWidth + channelStripSpacing) * sortedGroups.Count +
        (isShowingReferencedGroups ? spaceBetweenMainGroupsAndReferenced : 0f);
