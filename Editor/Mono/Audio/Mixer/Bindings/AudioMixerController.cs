...
namespace UnityEditor.Audio {
  ...
  internal class AudioGroupParameterPath : AudioParameterPath {
    ...
    public AudioGroupParameterPath(AudioMixerGroupController group,
                                   GUID parameter) {
      this.group = group;
  ...
  internal sealed class AudioEffectParameterPath : AudioGroupParameterPath {
    ...
    public AudioEffectParameterPath(AudioMixerGroupController group,
    ...
    public override string ResolveStringPath(bool getOnlyBasePath) {
      ...
      if (effect.GetGUIDForMixLevel() == parameter) {
        return "Mix Level" +
               GetBasePath(group.GetDisplayString(), effect.effectName);
      } else {
        MixerParameterDefinition[] paramDefs =
            MixerEffectDefinitions.GetEffectParameters(effect.effectName);
        for (int i = 0; i < paramDefs.Length; i++) {
          GUID guid = effect.GetGUIDForParameter(paramDefs[i].name);
          if (guid == parameter) {
            return paramDefs[i].name +
                   GetBasePath(group.GetDisplayString(), effect.effectName);
          }
        }
  ...
  internal sealed partial class AudioMixerController : AudioMixer {
    public static float kMinVolume =
        -80.0f; // The minimum volume is the level at which sends and effects
                // can be bypassed
    ...
    public static string s_GroupEffectDisplaySeperator =
        "\\"; // Use backslash instead of forward slash to prevent OS menus from
              // splitting path and creating submenus
    ...
    private string FindUniqueParameterName(string template,
                                           ExposedAudioParameter[] parameters) {
    ...
    int SortFuncForExposedParameters(ExposedAudioParameter p1,
                                     ExposedAudioParameter p2) {
      return string.CompareOrdinal(ResolveExposedParameterPath(p1.guid, true),
                                   ResolveExposedParameterPath(p2.guid, true));
    }

    public void AddExposedParameter(AudioParameterPath path) {
      if (!ContainsExposedParameter(path.parameter)) {
        List<ExposedAudioParameter> parameters =
            new List<ExposedAudioParameter>(exposedParameters);
        ExposedAudioParameter newParam = new ExposedAudioParameter();
        newParam.name =
            FindUniqueParameterName("MyExposedParam", exposedParameters);
    ...
    public bool ContainsExposedParameter(GUID parameter) {
      return exposedParameters.Where(val => val.guid == parameter)
                 .ToArray()
                 .Length > 0;
    }
    ...
    public void RemoveExposedParameter(GUID parameter) {
      exposedParameters =
          exposedParameters.Where(val => val.guid != parameter).ToArray();
      OnChangedExposedParameter();
    ...
    public string ResolveExposedParameterPath(GUID parameter,
                                              bool getOnlyBasePath) {
      ...
      foreach (AudioMixerGroupController group in groups) {
        if (group.GetGUIDForVolume() == parameter ||
          ...
          AudioGroupParameterPath newPath =
              new AudioGroupParameterPath(group, parameter);
          ...
          for (int i = 0; i < group.effects.Length; i++) {
            ...
            for (int j = 0; j < paramDefs.Length; j++) {
              ...
              if (guid == parameter) {
                AudioEffectParameterPath newPath =
                    new AudioEffectParameterPath(group, effect, parameter);
                exposedParamCache[parameter] = newPath;
                return newPath.ResolveStringPath(getOnlyBasePath);
              }
            }
    ...
    public static AudioMixerController
    CreateMixerControllerAtPath(string path) {
    ...
    public void CreateDefaultAsset(string path) {
      ...
      AudioMixerSnapshotController snapshot =
          new AudioMixerSnapshotController(this);
    ...
    private void BuildTestSetup(System.Random r,
                                AudioMixerGroupController parent, int minSpan,
                                int maxSpan, int maxGroups, string prefix,
                                ref int numGroups) {
      ...
      for (int i = 0; i < span; i++) {
        ...
        if (++numGroups >= maxGroups)
          return;
        BuildTestSetup(r, g, minSpan,
                       (maxSpan > minSpan) ? (maxSpan - 1) : minSpan, maxGroups,
                       name, ref numGroups);
      }
    }
    ...
    public void BuildTestSetup(int minSpan, int maxSpan, int maxGroups) {
      ...
      BuildTestSetup(new System.Random(), masterGroup, minSpan, maxSpan,
                     maxGroups, "G", ref numGroups);
    }
    ...
    private void
    GetAllAudioGroupsSlowRecurse(AudioMixerGroupController g,
                                 ref List<AudioMixerGroupController> groups) {
      groups.Add(g);
    ...
    private bool IsChildOf(AudioMixerGroupController child,
                           List<AudioMixerGroupController> groups) {
      while (child != null) {
    ...
    // Checks whether the groups in the list are in some kind of child-parent
    // relationship. They do of course have common ancestors, but there
    // shouldn't be any ancestor path from items in the list to any other items
    // in the list.
    public bool AreAnyOfTheGroupsInTheListAncestors(
        List<AudioMixerGroupController> groups) {
      return groups.Any(g => IsChildOf(g, groups));
    }

    // Before duplicating or removing a selection, this function is called to
    // make sure that the selection does not contain any items that are in a
    // child-parent relationship. By working on a cleaned list,
    // duplication/removal is simplified a lot and just becomes a matter or
    // recursively traversing each of the items in the selection without the
    // need to watch out for sub-items that already got deleted during the
    // recursive traversal.
    ...
    private void
    DestroyExposedParametersContainedInGroup(AudioMixerGroupController group) {
      Undo.RecordObject(this, "Remove Exposed Parameter");
      // Clean up the exposed parameters that were in the group.
      var exposedParams = exposedParameters;
      foreach (var param in exposedParams) {
        if (group.GetGUIDForVolume() == param.guid ||
            group.GetGUIDForPitch() == param.guid ||
            group.GetGUIDForSend() == param.guid)
          RemoveExposedParameter(param.guid);
      }
    }
    ...
    public void RemoveEffect(AudioMixerEffectController effect,
                             AudioMixerGroupController group) {
      Undo.RecordObject(group, "Delete Effect");

      var modifiedEffectsList =
          new List<AudioMixerEffectController>(group.effects);
    ...
    public AudioMixerGroupController CreateNewGroup(string name,
                                                    bool storeUndoState) {
    ...
    public void AddChildToParent(AudioMixerGroupController child,
                                 AudioMixerGroupController parent) {
      RemoveGroupsFromParent(new AudioMixerGroupController[]{child}, false);
      var modifiedChildList =
          new List<AudioMixerGroupController>(parent.children);
    ...
    public void RemoveGroupsFromParent(AudioMixerGroupController[] groups,
                                       bool storeUndoState) {
      List<AudioMixerGroupController> filteredGroups = groups.ToList();
      RemoveAncestorGroups(filteredGroups);
      ...
      foreach (var g in filteredGroups) {
        ...
        foreach (var parent in allGroups) {
          List<AudioMixerGroupController> modifiedChildList =
              new List<AudioMixerGroupController>(parent.children);
    ...
    public AudioMixerGroupController
    FindParentGroup(AudioMixerGroupController node,
                    AudioMixerGroupController group) {
    ...
    CopyEffect(AudioMixerEffectController sourceEffect) {
      var copiedEffect =
          new AudioMixerEffectController(sourceEffect.effectName);
      copiedEffect.name = sourceEffect.name;
      copiedEffect.PreallocateGUIDs();
      MixerParameterDefinition[] paramDefs =
          MixerEffectDefinitions.GetEffectParameters(sourceEffect.effectName);
      float value;
    ...
    private AudioMixerGroupController
    DuplicateGroupRecurse(AudioMixerGroupController sourceGroup,
                          bool recordUndo) {
    ...
    // Returns duplicated root groups (traverse group.children to get all groups
    // duplicated)
    public List<AudioMixerGroupController>
    DuplicateGroups(AudioMixerGroupController[] sourceGroups, bool recordUndo) {
      ...
      foreach (var source in filteredGroups) {
        ...
        if (parent != null && source != null) {
          ...
          AudioMixerGroupController copy =
              DuplicateGroupRecurse(source, recordUndo);
    ...
    public void CopyEffectSettingsToAllSnapshots(
        AudioMixerGroupController group, int effectIndex,
        AudioMixerSnapshotController snapshot, bool includeWetParam) {
    ...
    public void
    CopyAllSettingsToAllSnapshots(AudioMixerGroupController group,
                                  AudioMixerSnapshotController snapshot) {
      ...
      for (int n = 0; n < snaps.Length; n++) {
        if (snaps[n] == snapshot)
          continue;
        var dst = snaps[n];
        group.SetValueForVolume(this, dst,
                                group.GetValueForVolume(this, snapshot));
        group.SetValueForPitch(this, dst,
                               group.GetValueForPitch(this, snapshot));
      }
    }

    public void
    CopyAttenuationToAllSnapshots(AudioMixerGroupController group,
                                  AudioMixerSnapshotController snapshot) {
      AudioMixerSnapshotController[] snaps = snapshots;
      for (int n = 0; n < snaps.Length; n++) {
        if (snaps[n] == snapshot)
    ...
    public void ReparentSelection(AudioMixerGroupController newParent,
      ...
      if (insertionIndex >= 0)
        insertionIndex -= newParent.children.ToList()
                              .GetRange(0, insertionIndex)
                              .Count(selection.Contains);

      Undo.RecordObject(newParent, "Change Audio Mixer Group Parent");
      List<AudioMixerGroupController> groups = GetAllAudioGroupsSlow();
      foreach (var g in groups) {
        // Check if any groups in the selection is part of current groups child
        // list
        if (g.children.Intersect(selection).Any()) {
          Undo.RecordObject(
              g, string.Empty); // empty string will use undo name above
          var modifiedChildList =
              new List<AudioMixerGroupController>(g.children);
      ...
      var newChildList =
          new List<AudioMixerGroupController>(newParent.children);
      newChildList.InsertRange(insertionIndex, selection);
      newParent.children = newChildList.ToArray();
    }

    public static bool
    InsertEffect(AudioMixerEffectController effect,
                 ref List<AudioMixerEffectController> targetEffects,
                 int targetIndex) {
      if (targetIndex < 0 || targetIndex > targetEffects.Count) {
        Debug.LogError("Inserting effect failed! size: " + targetEffects.Count +
    ...
    public static bool MoveEffect(
        ref List<AudioMixerEffectController> sourceEffects, int sourceIndex,
        ref List<AudioMixerEffectController> targetEffects, int targetIndex) {
    ...
    // Todo: Remove this function (we should ensure that effect names are valid
    // for popup menus)
    static public string FixNameForPopupMenu(string s) { return s; }
    ...
    public class ConnectionNode {
      ...
      public string GetDisplayString() {
        ...
        if (effect != null)
          s += s_GroupEffectDisplaySeperator +
               FixNameForPopupMenu(effect.effectName);
        return s;
      }
    }

    // Builds a graph over all connections going out of a group (the parent
    // counts as one). While building the map, the existing routing of
    // effectSlotUnderTest are ignored and pretended it is connected to
    // targetToTest instead. Then checks are performed whether this setup would
    // cause any loops in the graph.
    static private Dictionary<object, ConnectionNode> BuildTemporaryGraph(
        List<AudioMixerGroupController> allGroups,
        AudioMixerGroupController groupWhoseEffectIsChanged,
        AudioMixerEffectController effectWhoseTargetIsChanged,
        AudioMixerEffectController targetToTest,
        AudioMixerGroupController modifiedGroup1,
        List<AudioMixerEffectController> modifiedGroupEffects1,
        AudioMixerGroupController modifiedGroup2,
        List<AudioMixerEffectController> modifiedGroupEffects2) {
      ...
      foreach (var group in allGroups) {
        ...
        var reorderedEffects = (group == modifiedGroup1)
                                   ? modifiedGroupEffects1
                                   : (group == modifiedGroup2)
                                         ? modifiedGroupEffects2
                                         : group.effects.ToList();
        foreach (var effect in reorderedEffects) {
          ...
          AudioMixerEffectController target =
              (group == groupWhoseEffectIsChanged &&
               effectWhoseTargetIsChanged == effect)
                  ? targetToTest
                  : effect.sendTarget;
          if (target != null) {
    ...
    static private void
    ListTemporaryGraph(Dictionary<object, ConnectionNode> graph) {
      Debug.Log("Listing temporary graph:");
      int nodeIndex = 0;
      foreach (var node in graph) {
        Debug.Log(string.Format("Node {0}: {1}", nodeIndex++,
                                node.Value.GetDisplayString()));
        int targetIndex = 0;
        foreach (var t in node.Value.targets)
          Debug.Log(string.Format("  Target {0}: {1}", targetIndex++,
                                  graph [t]
                                      .GetDisplayString()));
      }
    }

    static private bool CheckForCycle(object curr,
                                      Dictionary<object, ConnectionNode> graph,
                                      List<ConnectionNode> identifiedLoop) {
      var node = graph[curr];
      if (node.visited) {
    ...
    static public bool DoesTheTemporaryGraphHaveAnyCycles(
        List<AudioMixerGroupController> allGroups,
        List<ConnectionNode> identifiedLoop,
        Dictionary<object, ConnectionNode> graph) {
      foreach (var g in allGroups) {
        if (CheckForCycle(g, graph, identifiedLoop)) {
          // Clean up identified loop so that we only show the looping part and
          // not what lead to it
          if (identifiedLoop != null) {
    ...
    static public bool WillChangeOfEffectTargetCauseFeedback(
        List<AudioMixerGroupController> allGroups,
        AudioMixerGroupController groupWhoseEffectIsChanged,
        int effectWhoseTargetIsChanged, AudioMixerEffectController targetToTest,
        List<ConnectionNode> identifiedLoop) {
      var graph = BuildTemporaryGraph(
          allGroups, groupWhoseEffectIsChanged,
          groupWhoseEffectIsChanged.effects[effectWhoseTargetIsChanged],
          targetToTest, null, null, null, null);
      ...
      return DoesTheTemporaryGraphHaveAnyCycles(allGroups, identifiedLoop,
                                                graph);
    }

    static public bool WillModificationOfTopologyCauseFeedback(
        List<AudioMixerGroupController> allGroups,
        List<AudioMixerGroupController> groupsToBeMoved,
        AudioMixerGroupController newParentForMovedGroups,
        List<ConnectionNode> identifiedLoop) {
      var graph = BuildTemporaryGraph(allGroups, null, null, null, null, null,
                                      null, null);

      // Connect the chains up using the group hierarchy, pretending that the
      // groups in the list groupsToBeMoved belong to newParentForMovedGroups
      foreach (var group in allGroups) {
        foreach (var childGroup in group.children) {
          var parentGroup = (groupsToBeMoved.Contains(childGroup))
    ...
    static public bool WillMovingEffectCauseFeedback(
        List<AudioMixerGroupController> allGroups,
        AudioMixerGroupController sourceGroup, int sourceIndex,
        AudioMixerGroupController targetGroup, int targetIndex,
        List<ConnectionNode> identifiedLoop) {
      Dictionary<object, ConnectionNode> graph;

      if (sourceGroup == targetGroup) {
        var modifiedEffects = sourceGroup.effects.ToList();
        if (!MoveEffect(ref modifiedEffects, sourceIndex, ref modifiedEffects,
                        targetIndex))
          return false;
        graph = BuildTemporaryGraph(allGroups, null, null, null, sourceGroup,
                                    modifiedEffects, null, null);
      } else {
        var modifiedSourceEffects = sourceGroup.effects.ToList();
        var modifiedTargetEffects = targetGroup.effects.ToList();
        if (!MoveEffect(ref modifiedSourceEffects, sourceIndex,
                        ref modifiedTargetEffects, targetIndex))
          return false;
        graph = BuildTemporaryGraph(allGroups, null, null, null, sourceGroup,
                                    modifiedSourceEffects, targetGroup,
                                    modifiedTargetEffects);
      }
    ...
    static public float DbToLin(float x) {
      // This check needs to be kept in sync with the runtime. It serves as a
      // cutoff point at which connections can be cut or effects bypassed.
      if (x < kMinVolume)
        return 0.0f;
    ...
    public void SanitizeGroupViews() {
      ...
      for (int i = 0; i < viewList.Length; i++) {
        viewList[i].guids =
            (from x in viewList[i].guids from y in allGroups where y.groupID ==
             x select x)
                .ToArray();
      }
    ...
    public AudioMixerGroupController[] GetCurrentViewGroupList() {
      ...
      return (from g in allGroups where view.guids.Contains(g.groupID) select g)
          .ToArray();
    }

    public static float VolumeToScreenMapping(float value, float screenRange,
                                              bool forward) {
      ...
      if (forward)
        return (value > 0.0f)
                   ? (screenRange1 -
                      Mathf.Pow(value / GetMaxVolume(), 1.0f / kVolumeWarp) *
                          screenRange1)
                   : ((Mathf.Pow(value / kMinVolume, 1.0f / kVolumeWarp) *
                       screenRange2) +
                      screenRange1);
      else
        return (value < screenRange1)
                   ? (Mathf.Pow(1.0f - (value / screenRange1), kVolumeWarp) *
                      GetMaxVolume())
                   : (Mathf.Pow((value - screenRange1) / screenRange2,
                                kVolumeWarp) *
                      kMinVolume);
    }
    ...
    public void OnUnitySelectionChanged() {
      ...
      var selected = Selection.GetFiltered(typeof(AudioMixerGroupController),
                                           SelectionMode.Deep);
      m_CachedSelection =
          allGroups
              .Intersect(selected.Select(g =>(AudioMixerGroupController) g))
              .ToList();
    }
  }
}
