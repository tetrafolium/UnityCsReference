...
static Dictionary<AudioClip, float[]> s_Data =
...
struct ClipPreviewDetails {
	...
	public ClipPreviewDetails(AudioClip clip, bool isLooping, int size,
	                          double localStart, double localLength) {
		if (size < 2)
			throw new ArgumentException("Size has to be larger than 1");

		if (localLength <= 0)
			throw new ArgumentException("length has to be longer than zero",
			                            "localLength");

		if (localStart < 0)
			throw new ArgumentException("localStart has to be positive",
			                            "localStart");
		...
		if (preview == null)
			throw new ArgumentException("Clip " + clip +
			                            "'s overview preview is null");
		...
		if (looping) {
			...
			previewPixelsToRender = (int) Math.Min(
				size, size * Math.Max(0, clampedLength / localLength));
		}
	...
	public bool IsCandidateForStreaming() {
		// shortcut, no need to start the stream if the start extends beyond the
		// first clip region and the clip is "hold"
...
Dictionary<WaveformStreamer, StreamingContext> m_Contexts =
	new Dictionary<WaveformStreamer, StreamingContext>();
...
protected override void OnModifications(MessageFlags cFlags) {
	...
	if (HasFlag(cFlags, MessageFlags.TextureChanged) ||
	    HasFlag(cFlags, MessageFlags.Size) ||
	    HasFlag(cFlags, MessageFlags.Length) ||
	    HasFlag(cFlags, MessageFlags.Looping)) {
		KillAndClearStreamers();
		...
		var details =
			new ClipPreviewDetails(m_Clip, looping, (int) Size.x, start, length);
	...
	} else if (HasFlag(cFlags, MessageFlags.Optimization) &&
	           !restartStreaming) {
		// optimization toggled on, need to query whether we should start
		// streaming
		var details =
			new ClipPreviewDetails(m_Clip, looping, (int) Size.x, start, length);
...
Segment[] CalculateAndStartStreamers(double localStart, double localLength) {
	...
	// we don't care about the global position, only the locally visible offset
	// into the clip
	...
	if (!looping) {
		...
		var previewPixelsToRender = (int) Math.Min(
			Size.x, Size.x * Math.Max(0, clampedLength / localLength));
		...
		segments[0].streamer =
			new WaveformStreamer(m_Clip, originalStart, clampedLength,
			                     previewPixelsToRender, OnNewWaveformData);
	...
	// epsilon added to discriminate between invisible floating point rounding
	// errors and actual loops (i.e. more than a single pixel larger than then
	// length)
	if (localStart + localLength - secondsPerPixel > m_Clip.length) {
		var secondsToPixels = Size.x / localLength;

		// special case, the first part is clipped but at least one full length of
		// the clip is available we can then use one streamer to fill in all
		// visible segments
		if (localLength >= m_Clip.length) {
			var numberOfLoops = localLength / m_Clip.length;
			var streamer = new WaveformStreamer(m_Clip, 0, m_Clip.length,
			                                    (int)(Size.x / numberOfLoops),
			                                    OnNewWaveformData);
			...
			segments = new Segment[Mathf.CeilToInt(
						       (float)((localStart + localLength) / m_Clip.length))];

			for (int i = 0; i < segments.Length; ++i) {
				var cappedLength =
					Math.Min(currentClipSegmentPart + localPosition, localLength) -
					localPosition;
				...
				segments[i].streamingIndexOffset =
					(int)((m_Clip.length - currentClipSegmentPart) * secondsToPixels);

				localPosition += currentClipSegmentPart;
				currentClipSegmentPart = m_Clip.length;
			}
		} else {
			// two disjoint regions, since streaming is time-continuous we have to
			// split it up in two portions
			var firstPart = m_Clip.length - localStart;
			var secondPart = localLength - firstPart;

			segments = new Segment[2];

			segments[0].streamer = new WaveformStreamer(
				m_Clip, localStart, firstPart, (int)(firstPart * secondsToPixels),
				OnNewWaveformData);
			segments[0].segmentLength = (int)(firstPart * secondsToPixels);
			segments[0].textureOffset = 0;
			segments[0].streamingIndexOffset = 0;

			segments[1].streamer = new WaveformStreamer(
				m_Clip, 0, secondPart, (int)(secondPart * secondsToPixels),
				OnNewWaveformData);
		...
		segments[0].streamer = new WaveformStreamer(
			m_Clip, localStart, localLength, (int) Size.x, OnNewWaveformData);
		segments[0].segmentLength = (int) Size.x;
		segments[0].textureOffset = 0;
		segments[0].streamingIndexOffset = 0;
	}
...
void ResamplePreviewConfined(ClipPreviewDetails details,
                             float[] resampledPreview) {
	...
	if (delta > 0.5) {
		...
		// for each step, there's more than one sample so we do min max on the min
		// max data to avoid aliasing issues
		for (int i = 0; i < details.previewPixelsToRender; ++i) {
			for (int c = 0; c < channels; ++c) {
				...
				while (++x < floorPosition) {
					// yes, the data contained in the min max audio util overview is
					// actually swapped (maxmin data)
		...
		for (int i = 0; i < details.previewPixelsToRender; ++i) {
			...
			for (int c = 0; c < channels; ++c) {
				...
				resampledPreview[2 * i * channels + c * 2] =
					fraction * maxNext + (1 - fraction) * maxCurrent;
				resampledPreview[2 * i * channels + c * 2 + 1] =
					fraction * minNext + (1 - fraction) * minCurrent;
			}
...
void ResamplePreviewLooped(ClipPreviewDetails details,
                           float[] resampledPreview) {
...
bool OnNewWaveformData(WaveformStreamer streamer, float[] data,
                       int remaining) {
	...
	for (var i = 0; i < m_StreamedSegments.Length; i++) {
		if (m_StreamedSegments[i].streamer == streamer &&
		    pixelPos >= m_StreamedSegments[i].streamingIndexOffset &&
		    m_StreamedSegments[i].segmentLength >
		    (pixelPos - m_StreamedSegments[i].streamingIndexOffset)) {
			SetMMWaveData((m_StreamedSegments[i].textureOffset -
			               m_StreamedSegments[i].streamingIndexOffset) *
			              m_Clip.channels +
			              c.index,
			              data);
		}
