...
public class AudioCurveRendering {
  // This slight adjustment is needed to make sure that numerical imprecision
  // doesn't suddenly snap to the wrong pixel (causing vertical gaps and
  // overdrawn lines)
  ...
  public delegate void AudioMinMaxCurveAndColorEvaluator(float x, out Color col,
                                                         out float minValue,
                                                         out float maxValue);
  public static readonly Color kAudioOrange =
      new Color(255 / 255f, 168 / 255f, 7 / 255f);
  ...
  public static void EndCurveFrame() { GUI.EndGroup(); }
  ...
  // -------------
  // Curve rendering -- do not remove any of these, as they may be used by
  // custom GUIs of native audio plugins
  // -------------

  public static void DrawFilledCurve(Rect r, AudioCurveEvaluator eval,
                                     Color curveColor) {
    DrawFilledCurve(r, delegate(float x, out Color color) {
      color = curveColor;
      return eval(x);
    });
  }
  ...
  public static void DrawFilledCurve(Rect r, AudioCurveAndColorEvaluator eval) {
    ...
    // Adjust by a half pixel to each side so that the transition covers a full
    // pixel. This is needed for very slowly rising edges.
  ...
  public static void
  DrawMinMaxFilledCurve(Rect r, AudioMinMaxCurveAndColorEvaluator eval) {
    HandleUtility.ApplyWireMaterial();
    DrawMinMaxFilledCurveInternal(r, eval);
  }

  internal static void
  DrawMinMaxFilledCurveInternal(Rect r,
                                AudioMinMaxCurveAndColorEvaluator eval) {
    GL.Begin(GL.LINES);
    ...
    Color color;
    eval(0.0001f, out color, out minValue, out maxValue);
  ...
  public static void
  DrawSymmetricFilledCurve(Rect r, AudioCurveAndColorEvaluator eval) {
    if (Event.current.type != EventType.Repaint)
      return;
    ...
    for (int x = 0; x < pixelWidth; x++) {
      ...
      float e1 = Mathf.Max(Mathf.Min(ny, py) - pixelHalfSize,
                           0.0f); // Avoid self-intersection
  ...
  public static void DrawCurve(Rect r, AudioCurveEvaluator eval,
                               Color curveColor) {
    if (Event.current.type != EventType.Repaint)
      return;
  ...
  public static void DrawGradientRect(Rect r, Color c1, Color c2, float blend,
                                      bool horizontal) {
    if (Event.current.type != EventType.Repaint)
      return;
    HandleUtility.ApplyWireMaterial();
