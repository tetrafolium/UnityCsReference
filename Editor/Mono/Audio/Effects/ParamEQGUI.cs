...
internal class ParamEqGUI : IAudioEffectPluginGUI {
  ...
  public static GUIStyle textStyle10 = BuildGUIStyleForLabel(
      Color.grey, 10, false, FontStyle.Normal, TextAnchor.MiddleCenter);

  private static void DrawFrequencyTickMarks(Rect r, float samplerate,
                                             bool logScale, Color col) {
    textStyle10.normal.textColor = col;
    float px = r.x, w = 60.0f;
    for (float normFreq = 0; normFreq < 1.0f; normFreq += 0.01f) {
      float f =
          (float) MapNormalizedFrequency(normFreq, samplerate, logScale, true);
      float x = r.x + normFreq * r.width;
      if (x - px > w) {
        EditorGUI.DrawRect(new Rect(x, r.yMax - 5f, 1f, 5f), col);
        GUI.Label(new Rect(x, r.yMax - 22f, 1, 15f),
                  (f < 1000.0f)
  ...
  private static double MapNormalizedFrequency(double f, double sr,
                                               bool useLogScale, bool forward) {
  ...
  static bool ParamEqualizerCurveEditor(IAudioEffectPlugin plugin, Rect r,
                                        ref float centerFreq,
                                        ref float bandwidth, ref float gain,
                                        float blend) {
    ...
    plugin.GetFloatParameterInfo(kCenterFreqName, out minCenterFreq,
                                 out maxCenterFreq, out defCenterFreq);
    float minOctaveRange, maxOctaveRange, defOctaveRange;
    plugin.GetFloatParameterInfo(kOctaveRangeName, out minOctaveRange,
                                 out maxOctaveRange, out defOctaveRange);
    float minGain, maxGain, defGain;
    plugin.GetFloatParameterInfo(kFrequencyGainName, out minGain, out maxGain,
                                 out defGain);
    ...
    case EventType.MouseDrag:
      if (GUIUtility.hotControl == controlID) {
        ...
        centerFreq = Mathf.Clamp(
            (float) MapNormalizedFrequency(
                MapNormalizedFrequency(centerFreq, plugin.GetSampleRate(),
                                       useLogScale, false) +
                    evt.delta.x / r.width,
                plugin.GetSampleRate(), useLogScale, true),
            minCenterFreq, maxCenterFreq);
        if (Event.current.shift)
          bandwidth =
              Mathf.Clamp(bandwidth - evt.delta.y * 0.02f * dragAcceleration,
                          minOctaveRange, maxOctaveRange);
        else
          gain = Mathf.Clamp(gain - evt.delta.y * 0.01f * dragAcceleration,
                             minGain, maxGain);
        modifiedValue = true;
        evt.Use();
      }
    ...
    if (Event.current.type == EventType.Repaint) {
      ...
      float c = (float) MapNormalizedFrequency(
          centerFreq, plugin.GetSampleRate(), useLogScale, false);
      EditorGUI.DrawRect(new Rect(c * r.width + r.x, r.y, 1f, r.height),
                         GUIUtility.hotControl ==
                             controlID ? new Color(0.6f, 0.6f, 0.6f)
      ...
      AudioCurveRendering.DrawCurve(r, delegate(float x) {
        double f = MapNormalizedFrequency((double) x, plugin.GetSampleRate(),
                                          useLogScale, true);
    ...
    DrawFrequencyTickMarks(r, plugin.GetSampleRate(), useLogScale,
  ...
  public override bool OnGUI(IAudioEffectPlugin plugin) {
    ...
    if (ParamEqualizerCurveEditor(plugin, r, ref centerFreq, ref octaveRange,
                                  ref frequencyGain, blend)) {
