...
public static GUIStyle textStyle10 = BuildGUIStyleForLabel(
	Color.grey, 10, false, FontStyle.Normal, TextAnchor.MiddleLeft);

public static void DrawText(float x, float y, string text) {
	GUI.Label(new Rect(x, y - 5, 200, 10), new GUIContent(text, ""),
	          textStyle10);
}
...
protected static void DrawVU(Rect r, float level, float blend, bool topdown) {
	...
	var r2 = new Rect(r.x + border,
	                  r.y + border +
	                  (topdown ? 0.0f
	                   : level * r.height),
	                  r.width - 2 * border,
	                  r.y - 2 * border +
	                  (topdown ? level * r.height
	                   : r.height - level * r.height));
...
static float EvaluateDuckingVolume(float x, float ratio, float threshold,
                                   float makeupGain, float knee,
                                   float dbRange, float dbMin) {
	float duckGradient = 1.0f / ratio;
	float duckThreshold = threshold;
	float duckMakeupGain = makeupGain;
	float duckKnee = knee;
	float duckKneeC1 =
		(knee > 0.0f) ? ((duckGradient - 1.0f) / (4.0f * knee)) : 0.0f;
...
static bool CurveDisplay(IAudioEffectPlugin plugin, Rect r0,
                         ref float threshold, ref float ratio,
                         ref float makeupGain, ref float attackTime,
                         ref float releaseTime, ref float knee,
                         float sidechainLevel, float outputLevel,
                         float blend) {
	...
	plugin.GetFloatParameterInfo(kThresholdName, out minThreshold,
	                             out maxThreshold, out defThreshold);
	float minRatio, maxRatio, defRatio;
	plugin.GetFloatParameterInfo(kRatioName, out minRatio, out maxRatio,
	                             out defRatio);
	float minMakeupGain, maxMakeupGain, defMakeupGain;
	plugin.GetFloatParameterInfo(kMakeupGainName, out minMakeupGain,
	                             out maxMakeupGain, out defMakeupGain);
	float minKnee, maxKnee, defKnee;
	plugin.GetFloatParameterInfo(kKneeName, out minKnee, out maxKnee,
	                             out defKnee);
	...
	case EventType.MouseDown:
		if (r.Contains(Event.current.mousePosition) && evt.button == 0) {
			...
			// Ensure visible state change on mousedown to make it clear that
			// interaction is possible
			if ((Mathf.Abs(r.x + thresholdPosX - evt.mousePosition.x) >=
			     thresholdActiveWidth))
				dragtype = (evt.mousePosition.x < r.x + thresholdPosX)
			 ? DragType.MakeupGain
			 : DragType.Ratio;
			else
	...
	case EventType.MouseDrag:
		if (GUIUtility.hotControl == controlID) {
			...
			if (dragtype == DragType.ThresholdAndKnee) {
				...
				if (dragKnee)
					knee = Mathf.Clamp(knee + evt.delta.y * 0.5f * dragAcceleration,
					                   minKnee, maxKnee);
				else
					threshold =
						Mathf.Clamp(threshold + evt.delta.x * 0.1f * dragAcceleration,
						            minThreshold, maxThreshold);
			} else if (dragtype == DragType.Ratio)
				ratio = Mathf.Clamp(ratio + evt.delta.y *
				                    (ratio > 1.0f ? 0.05f : 0.003f) *
				                    dragAcceleration,
				                    minRatio, maxRatio);
			else if (dragtype == DragType.MakeupGain)
				makeupGain =
					Mathf.Clamp(makeupGain - evt.delta.y * 0.5f * dragAcceleration,
					            minMakeupGain, maxMakeupGain);
			else {
	...
	if (evt.type == EventType.Repaint) {
		...
		float thresholdPosY =
			r.height * (1.0f - ((threshold - dbMin + makeupGain) / dbRange));
		...
		AudioCurveRendering.DrawFilledCurve(r, delegate(float x, out Color col) {
					...
					float gain = level;
					float t = level - duckThreshold;
					col = ScaleAlpha(duckSidechainLevel >
					                 level ? AudioCurveRendering.kAudioOrange
			 : Color.grey, blend);
					if (t > -duckKnee && t < duckKnee) {
					        t += duckKnee;
					        gain = t * (duckKneeC1 * t + 1.0f) + duckKneeC2;
		...
		EditorGUI.DrawRect(new Rect(r.x + thresholdPosX, r.y, 1, r.height),
		                   textStyle10.normal.textColor);
		DrawText(r.x + thresholdPosX + 4, r.y + 6,
		         string.Format(CultureInfo.InvariantCulture.NumberFormat,
		                       "Threshold: {0:F1} dB", threshold));

		// Sidechain text and line
		textStyle10.normal.textColor = ScaleAlpha(sidechainColor, blend);
		DrawText(r.x + 4, r.y + r.height - 10,
		         sidechainLevel < -80
		   ? "Input: None"
		   : string.Format(CultureInfo.InvariantCulture.NumberFormat,
		                   "Input: {0:F1} dB", sidechainLevel));

		if (dragtype == DragType.Ratio) {
			float aspect = (float) r.height / (float) r.width;
			Handles.DrawAAPolyLine(
				2.0f, new Color[] {Color.black, Color.black},
				new Vector3[] {
						new Vector3(r.x + thresholdPosX + r.width,
						            r.y + thresholdPosY - aspect * r.width, 0.0f),
						new Vector3(r.x + thresholdPosX - r.width,
						            r.y + thresholdPosY + aspect * r.width, 0.0f)
					});
			Handles.DrawAAPolyLine(
				3.0f, new Color[] {Color.white, Color.white},
				new Vector3[] {new Vector3(r.x + thresholdPosX + r.width,
				                           r.y + thresholdPosY -
				                           aspect * duckGradient * r.width,
				                           0.0f),
				               new Vector3(r.x + thresholdPosX - r.width,
				                           r.y + thresholdPosY +
				                           aspect * duckGradient * r.width,
			...
			float y1 = EvaluateDuckingVolume(normalizedKnee1, ratio, threshold,
			                                 makeupGain, knee, dbRange, dbMin);
			float y2 = EvaluateDuckingVolume(normalizedKnee2, ratio, threshold,
			                                 makeupGain, knee, dbRange, dbMin);
			float knee1PosY = r.yMax - (y1 + 1f) * 0.5f * r.height;
			float knee2PosY = r.yMax - (y2 + 1f) * 0.5f * r.height;
			EditorGUI.DrawRect(new Rect(r.x + normalizedKnee1 * r.width, knee1PosY,
			                            1, r.height - knee1PosY),
			                   new Color(0, 0, 0, 0.5f));
			EditorGUI.DrawRect(new Rect(r.x + normalizedKnee2 * r.width - 1,
			                            knee2PosY, 1, r.height - knee2PosY),
			                   new Color(0, 0, 0, 0.5f));

			// Enhanced threshold
			EditorGUI.DrawRect(new Rect(r.x + thresholdPosX - 1, r.y, 3, r.height),
			                   Color.white);
		}

		outputLevel =
			(Mathf.Clamp(outputLevel - makeupGain, dbMin, dbMin + dbRange) -
			 dbMin) /
			dbRange;
		if (EditorApplication.isPlaying) {
			const int margin = 2;
			Rect vuRect = new Rect(r.x + r.width - vuWidth + margin, r.y + margin,
			                       vuWidth - 2 * margin, r.height - 2 * margin);
...
public override bool OnGUI(IAudioEffectPlugin plugin) {
	...
	if (CurveDisplay(plugin, r, ref threshold, ref ratio, ref makeupGain,
	                 ref attackTime, ref releaseTime, ref knee, metering[0],
	                 metering[1], blend)) {
