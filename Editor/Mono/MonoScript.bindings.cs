...
public class MonoScript : TextAsset {
  ...
  FromScriptableObject(ScriptableObject scriptableObject) {
  ...
  // *undocumented*
  // Pass CreateOptions.None to TextAsset constructor so it does not create a
  // native TextAsset object. We create MonoScript native object instead.
  public MonoScript() : base(TextAsset.CreateOptions.None, null) {
    Init_Internal(this);
  }

  internal void Init(string scriptContents, string className, string nameSpace,
                     string assemblyName, bool isEditorScript) {
    Init(this, scriptContents, className, nameSpace, assemblyName,
         isEditorScript);
  }
  ...
  private static extern void Init(MonoScript self, string scriptContents,
