...
internal class ScriptAttributeUtility {
  ...
  internal static Stack<PropertyDrawer> s_DrawerStack =
      new Stack<PropertyDrawer>();
  private static Dictionary<Type, DrawerKeySet> s_DrawerTypeForType = null;
  private static Dictionary<string, List<PropertyAttribute>>
      s_BuiltinAttributes = null;
  ...
  private static PropertyHandlerCache s_GlobalCache =
      new PropertyHandlerCache();
  private static PropertyHandlerCache s_CurrentCache = null;

  internal static PropertyHandlerCache propertyHandlerCache {
    get { return s_CurrentCache ?? s_GlobalCache; }
    set { s_CurrentCache = value; }
  }

  internal static void ClearGlobalCache() { s_GlobalCache.Clear(); }
  ...
  private static void PopulateBuiltinAttributes() {
    ...
    // Example: Make Orthographic Size in Camera component be in range between 0
    // and 1000
    // AddBuiltinAttribute ("Camera", "m_OrthographicSize", new RangeAttribute
    // (0, 1000));
  }

  private static void AddBuiltinAttribute(string componentTypeName,
                                          string propertyPath,
    ...
    s_BuiltinAttributes [key]
        .Add(attr);
  }

  private static List<PropertyAttribute>
  GetBuiltinAttributes(SerializedProperty property) {
  ...
  private static void BuildDrawerTypeForTypeDictionary() {
    ...
    foreach (var type in TypeCache.GetTypesDerivedFrom<GUIDrawer>()) {
      ...
      object[] attrs =
          type.GetCustomAttributes(typeof(CustomPropertyDrawer), true);
      foreach (CustomPropertyDrawer editor in attrs) {
        // Debug.Log("Base type: " + editor.type);
        s_DrawerTypeForType[editor.m_Type] =
            new DrawerKeySet(){drawer = type, type = editor.m_Type};
        ...
        foreach (var candidateType in candidateTypes) {
          ...
          if (s_DrawerTypeForType.ContainsKey(candidateType) &&
              (editor.m_Type.IsAssignableFrom(
                  s_DrawerTypeForType[candidateType].type))) {
            //  Debug.Log("skipping");
            continue;
          }

          // Debug.Log("Setting");
          s_DrawerTypeForType[candidateType] =
              new DrawerKeySet(){drawer = type, type = editor.m_Type};
        }
      }
    }
  ...
  internal static Type GetDrawerTypeForType(Type type) {
    ...
    if (type.IsGenericType)
      s_DrawerTypeForType.TryGetValue(type.GetGenericTypeDefinition(),
                                      out drawerType);
  ...
  /// <summary>
  /// Does the same thing as 'GetDrawerTypeForType' (with the same side effect
  /// of building the cache) but also plays well with Managed References. If the
  /// property that is used as a reference for the drawer query is of a managed
  /// reference type, the class parents are also looked up as fallbacks.
  /// </summary>
  /// <param name="property"></param>
  /// <param name="type"></param>
  /// <returns>The custom property drawer type or 'null' if not found.</returns>
  internal static Type
  GetDrawerTypeForPropertyAndType(SerializedProperty property, Type type) {
    ...
    // The custom property drawers for those are defined with
    // 'useForChildren=false' (otherwise the dynamic type is not taking into
    // account in the custom property drawer resolution) so even if
    // 's_DrawerTypeForType' is built (based on static types) we have to check
    // base types for custom property drawers manually. Managed references with
    // no drawers should properly try to fallback
    if (property.propertyType == SerializedPropertyType.ManagedReference) {
      DrawerKeySet drawerTypes;

      FieldInfo foundField = null;
      for (Type currentType = type; foundField == null && currentType != null;
  ...
  private static List<PropertyAttribute> GetFieldAttributes(FieldInfo field) {
    ...
    if (attrs != null && attrs.Length > 0)
      return new List<PropertyAttribute>(
          attrs.Select(e => e as PropertyAttribute).OrderBy(e => -e.order));

    return null;
  }

  /// <summary>
  /// Returns the field info and field type for the property. The types are
  /// based on the static field definition.
  /// </summary>
  /// <param name="property"></param>
  /// <param name="type"></param>
  /// <returns></returns>
  internal static FieldInfo
  GetFieldInfoAndStaticTypeFromProperty(SerializedProperty property,
                                        out Type type) {
    ...
    if (property.isReferencingAManagedReferenceField) {
      // When the field we are trying to access is a dynamic instance, things
      // are a bit more tricky since we cannot "statically" (looking only at the
      // parent class field types) know the actual "classType" of the parent
      // class.

      // The issue also is that at this point our only view on the object is the
      // very limited SerializedProperty.

      // So we have to:
      // 1. try to get the FQN from for the current managed type from the
      // serialized data,
      ...
      var objectTypename =
          property.GetFullyQualifiedTypenameForCurrentTypeTreeInternal();
  ...
  private static Type
  GetManagedReferenceTypeHostForField(SerializedProperty property) {
    if (!property.isReferencingAManagedReferenceField)
      throw new ArgumentException(
          "Property not referencing an element of a polymorphic instance");
  ...
  /// <summary>
  /// Returns the field info and type for the property. Contrary to
  /// GetFieldInfoAndStaticTypeFromProperty, when confronted with a managed
  /// reference the dynamic instance type is returned.
  /// </summary>
  /// <param name="property"></param>
  /// <param name="type"></param>
  /// <returns></returns>
  internal static FieldInfo
  GetFieldInfoFromProperty(SerializedProperty property, out Type type) {
    var fieldInfo = GetFieldInfoAndStaticTypeFromProperty(property, out type);
    if (fieldInfo == null)
      return fieldInfo;

    // Managed references are a special case, we need to override the static
    // type returned by 'GetFieldInfoFromPropertyPath' for custom property
    // handler matching by the dynamic type of the instance.
    if (property.propertyType == SerializedPropertyType.ManagedReference) {
      Type managedReferenceInstanceType;

      // Try to get a Type instance for the managed reference
      if (GetTypeFromManagedReferenceFullTypeName(
              property.managedReferenceFullTypename,
              out managedReferenceInstanceType)) {
        type = managedReferenceInstanceType;
      }

      // We keep the fallback to the field type returned by
      // 'GetFieldInfoFromPropertyPath'.
    }
  ...
  /// <summary>
  /// Create a Type instance from the managed reference full type name
  /// description. The expected format for the typename string is the one
  /// returned by SerializedProperty.managedReferenceFullTypename.
  /// </summary>
  /// <param name="managedReferenceFullTypename"></param>
  /// <param name="managedReferenceInstanceType"></param>
  /// <returns></returns>
  internal static bool GetTypeFromManagedReferenceFullTypeName(
    ...
    if (parts.Length == 2) {
      ...
      managedReferenceInstanceType =
          Type.GetType($"{nsClassnamePart}, {assemblyPart}");
    }
  ...
  private static Type GetScriptTypeFromProperty(SerializedProperty property) {
    ...
    // Fallback in case the targetObject has been destroyed but the property is
    // still valid.
  ...
  struct Cache : IEquatable<Cache> {
    ...
    public override int GetHashCode() {
      unchecked {
        return ((host != null ? host.GetHashCode() : 0) * 397) ^
  ...
  static Dictionary<Cache, FieldInfoCache> s_FieldInfoFromPropertyPathCache =
      new Dictionary<Cache, FieldInfoCache>();
  ...
  private static FieldInfo GetFieldInfoFromPropertyPath(Type host, string path,
    ...
    if (s_FieldInfoFromPropertyPathCache.TryGetValue(cache,
                                                     out fieldInfoCache)) {
    ...
    // we are looking for array element only when the path ends with
    // Array.data[x]
    var lookingForArrayElement = Regex.IsMatch(path, arrayData + "$");
    // remove any Array.data[x] from the path because it is prevents cache
    // searching.
    ...
    for (int i = 0; i < parts.Length; i++) {
      ...
      // Private fields are relevant because they can still be shown in the
      // Inspector, and that applies to private fields in base classes too.
      FieldInfo foundField = null;
      for (Type currentType = type; foundField == null && currentType != null;
           currentType = currentType.BaseType)
        foundField = currentType.GetField(member, BindingFlags.Instance |
                                                      BindingFlags.Public |
    ...
    fieldInfoCache = new FieldInfoCache{type = type, fieldInfo = fieldInfo};
  ...
  internal static PropertyHandler GetHandler(SerializedProperty property) {
    ...
    if (NativeClassExtensionUtilities.ExtendsANativeType(target)) {
      // For scripts, use reflection to get FieldInfo for the member the
      // property represents
    ...
    // Field has no CustomPropertyDrawer attribute with matching drawer so look
    // for default drawer for field type
    if (!handler.hasPropertyDrawer && propertyType != null)
      handler.HandleDrawnType(property, propertyType, propertyType, field,
                              null);
  ...
  internal static List<FieldInfo> GetAutoLoadProperties(Type type) {
    ...
    if (!s_AutoLoadProperties.TryGetValue(type, out list)) {
      list = type.GetFields(BindingFlags.NonPublic | BindingFlags.Public |
                            BindingFlags.Instance)
                 .Where(f => f.FieldType ==
                             typeof(SerializedProperty) &&f.IsDefined(
                                 typeof(CachePropertyAttribute), false))
                 .ToList();
