...
public bool hasPropertyDrawer {
	get { return propertyDrawer != null; }
}
internal PropertyDrawer propertyDrawer {
	get { return isCurrentlyNested ? null : m_PropertyDrawer; }
}

internal static Dictionary<string, ReorderableListWrapper>
s_reorderableLists = new Dictionary<string, ReorderableListWrapper>();
...
public static void ClearListCacheIncludingChildren(string propertyPath) {
	foreach (var listEntry in s_reorderableLists) {
		if (listEntry.Key.Contains(propertyPath))
			listEntry.Value.ClearCache();
	}
}
...
public bool empty {
	get {
		return m_DecoratorDrawers == null && tooltip == null &&
		       propertyDrawer == null && contextMenuItems == null;
	}
}

public void HandleAttribute(SerializedProperty property,
                            PropertyAttribute attribute, FieldInfo field,
                            Type propertyType) {
	...
	HandleDrawnType(property, attribute.GetType(), propertyType, field,
	                attribute);
}

public void HandleDrawnType(SerializedProperty property, Type drawnType,
                            Type propertyType, FieldInfo field,
                            PropertyAttribute attribute) {
	Type drawerType = ScriptAttributeUtility.GetDrawerTypeForPropertyAndType(
		property, drawnType);

	// If we found a drawer type, instantiate the drawer, cache it, and return
	// it.
	if (drawerType != null) {
		if (typeof(PropertyDrawer).IsAssignableFrom(drawerType)) {
			// Use PropertyDrawer on array elements, not on array itself.
			// If there's a PropertyAttribute on an array, we want to apply it to
			// the individual array elements instead. This is the only convenient
			// way we can let the user apply PropertyDrawer attributes to elements
			// inside an array.
			if (propertyType != null && propertyType.IsArrayOrList())
				return;

			m_PropertyDrawer =
				(PropertyDrawer) System.Activator.CreateInstance(drawerType);
			...
			if (field != null && field.FieldType.IsArrayOrList() &&
			    !propertyType.IsArrayOrList())
				return;

			DecoratorDrawer decorator =
				(DecoratorDrawer) System.Activator.CreateInstance(drawerType);
...
public bool OnGUI(Rect position, SerializedProperty property,
...
internal bool OnGUI(Rect position, SerializedProperty property,
                    GUIContent label, bool includeChildren,
                    Rect visibleArea) {
	TestInvalidateCache();
	...
	if (propertyDrawer != null) {
		...
		propertyDrawer.OnGUISafe(
			position, property.Copy(),
			label ?? EditorGUIUtility.TempContent(property.localizedDisplayName));
		// Restore widths
		EditorGUIUtility.labelWidth = oldLabelWidth;
		EditorGUIUtility.fieldWidth = oldFieldWidth;
		...
		bool childrenAreExpanded =
			EditorGUI.DefaultPropertyField(position, prop, label) &&
			EditorGUI.HasVisibleChildFields(prop);
		...
		if (childrenAreExpanded) {
			...
			while (prop.NextVisible(childrenAreExpanded) &&
			       !SerializedProperty.EqualContents(prop, endProperty)) {
				...
				if (position.Overlaps(visibleArea)) {
					...
					childrenAreExpanded = handler.OnGUI(position, prop, null, false) &&
					                      EditorGUI.HasVisibleChildFields(prop);
					// Changing child properties (like array size) may invalidate the
					// iterator, so stop now, or we may get errors.
...
public bool OnGUILayout(SerializedProperty property, GUIContent label,
	...
	if (property.propertyType == SerializedPropertyType.Boolean &&
		...
		r = EditorGUILayout.GetControlRect(
			...
			GetHeight(property, label, includeChildren), options);
...
public float GetHeight(SerializedProperty property, GUIContent label,
	...
	if (IsNonStringArray(property)) {
		...
		// If collection doesn't have a ReorderableList assigned to it, create one
		// and assign it
		if (!s_reorderableLists.TryGetValue(key, out reorderableList)) {
			reorderableList =
				new ReorderableListWrapper(property, IsArrayReorderable(property));
			s_reorderableLists[key] = reorderableList;
		}

		reorderableList.Property = property;
		height = s_reorderableLists [key]
		         .GetHeight();
		return height;
	}
	...
	if (propertyDrawer != null) {
		height += propertyDrawer.GetPropertyHeightSafe(
			property.Copy(),
		...
		height += EditorGUI.GetSinglePropertyHeight(property, label);
		bool childrenAreExpanded =
			property.isExpanded && EditorGUI.HasVisibleChildFields(property);
...
public void AddMenuItems(SerializedProperty property, GenericMenu menu) {
	...
	foreach (ContextMenuItemAttribute attribute in contextMenuItems) {
		...
		menu.AddItem(new GUIContent(attribute.name), false,
		             () => CallMenuCallback(
				     property.serializedObject.targetObjects, method));
	}
}
...
internal static bool IsNonStringArray(SerializedProperty property) {
	// Strings should not be represented with ReorderableList, they will use
	// custom drawer therefore we don't treat them as other arrays
...
static bool IsArrayReorderable(SerializedProperty property) {
	...
	Queue<string> propertyName = new Queue<string>(property.propertyPath.Split(
							       ".".ToCharArray(), StringSplitOptions.RemoveEmptyEntries));
	listInfo = property.serializedObject.targetObject.GetType().GetField(
		propertyName.Dequeue());

	if (listInfo == null)
		return false;

	// If we have a nested property we need to find it via reflection in order
	// to verify if it has a non-reorderable attribute
	...
	return TypeCache.GetFieldsWithAttribute(typeof(ReorderableAttribute))
	       .Any(f => f.Equals(listInfo)) ||
	       property.IsReorderable();
}
}
}
