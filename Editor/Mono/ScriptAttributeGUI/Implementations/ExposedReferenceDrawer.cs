...
private static Color kMissingOverrideColor =
	new Color(1.0f, 0.11f, 0.11f, 1.0f);

protected readonly GUIContent ExposePropertyContent =
	EditorGUIUtility.TrTextContent("Expose Property");
protected readonly GUIContent UnexposePropertyContent =
	EditorGUIUtility.TrTextContent("Unexpose Property");
protected readonly GUIContent NotFoundOn =
	EditorGUIUtility.TrTextContent("not found on");
protected readonly GUIContent OverridenByContent =
	EditorGUIUtility.TrTextContent("Overridden by ");

private GUIContent m_ModifiedLabel = new GUIContent();

protected enum ExposedPropertyMode { DefaultValue, Named, NamedGUID }

protected enum OverrideState { DefaultValue, MissingOverride, Overridden }
...
protected IExposedPropertyTable
GetExposedPropertyTable(SerializedProperty property) {
	var t = property.serializedObject.context;
	return t as IExposedPropertyTable;
}

protected abstract void
OnRenderProperty(Rect position, PropertyName exposedPropertyNameString,
...
public override void OnGUI(Rect position, UnityEditor.SerializedProperty prop,
                           GUIContent label) {
	...
	propertyFieldPosition.xMax =
		propertyFieldPosition.xMax -
		ExposedReferencePropertyDrawer.kDriveWidgetWidth;
	...
	var currentReferenceValue =
		Resolve(propertyName, exposedPropertyTable,
	...
	var valuePosition = DrawLabel(showContextMenu, currentOverrideState, label,
	                              position, exposedPropertyTable,
	                              exposedNameStr, exposedName, defaultValue);

	if (propertyMode == ExposedPropertyMode.DefaultValue ||
	    propertyMode == ExposedPropertyMode.NamedGUID) {
		OnRenderProperty(valuePosition, propertyName, currentReferenceValue,
		                 defaultValue, exposedName, propertyMode,
		...
		OnRenderProperty(valuePosition, new PropertyName(exposedNameStr),
		                 currentReferenceValue, defaultValue, exposedName,
		                 propertyMode, exposedPropertyTable);
	}
	...
	if (showContextMenu &&
	    GUI.Button(driveFieldPosition, GUIContent.none, kDropDownStyle)) {
		GenericMenu menu = new GenericMenu();
		PopulateContextMenu(menu, currentOverrideState, exposedPropertyTable,
		                    exposedName, defaultValue);
		menu.ShowAsContext();
		Event.current.Use();
	}
}
...
Rect DrawLabel(bool showContextMenu, OverrideState currentOverrideState,
               GUIContent label, Rect position,
               IExposedPropertyTable exposedPropertyTable,
               string exposedNameStr, SerializedProperty exposedName,
               SerializedProperty defaultValue) {
	if (showContextMenu) {
		position.xMax =
			position.xMax - ExposedReferencePropertyDrawer.kDriveWidgetWidth;
	}

	EditorGUIUtility.SetBoldDefaultFont(currentOverrideState !=
	                                    OverrideState.DefaultValue);
	...
	} else if (currentOverrideState == OverrideState.Overridden &&
	...
	if (exposedPropertyTable != null &&
		...
		if (position.Contains(Event.current.mousePosition)) {
			...
			PopulateContextMenu(menu,
			                    string.IsNullOrEmpty(exposedNameStr)
			        ? OverrideState.DefaultValue
...
protected Object Resolve(PropertyName exposedPropertyName,
                         IExposedPropertyTable exposedPropertyTable,
                         Object defaultValue,
                         out OverrideState currentOverrideState) {
	...
	if (exposedPropertyTable != null) {
		objReference = exposedPropertyTable.GetReferenceValue(
			exposedPropertyName, out propertyFoundInTable);
	...
	return currentOverrideState == OverrideState.Overridden ? objReference
	: defaultValue;
}

protected abstract void
PopulateContextMenu(GenericMenu menu, OverrideState overrideState,
                    IExposedPropertyTable exposedPropertyTable,
                    SerializedProperty exposedName,
                    SerializedProperty defaultValue);
}
...
OnRenderProperty(Rect position, PropertyName exposedPropertyNameString,
	...
	EditorGUI.BeginChangeCheck();
	var newValue = EditorGUI.ObjectField(position, currentReferenceValue,
	                                     typeOfExposedReference,
	                                     exposedPropertyTable != null);
	...
	if (EditorGUI.EndChangeCheck()) {
		if (mode == ExposedPropertyMode.DefaultValue) {
			...
			if (!EditorUtility.IsPersistent(
				    exposedPropertyDefault.serializedObject.targetObject) ||
				...
				Undo.RecordObject(exposedPropertyTable as UnityEngine.Object,
				                  "Set Exposed Property");
				exposedPropertyTable.SetReferenceValue(exposedPropertyNameString,
				                                       newValue);
			}
		} else {
			Undo.RecordObject(exposedPropertyTable as UnityEngine.Object,
...
PopulateContextMenu(GenericMenu menu, OverrideState overrideState,
	...
	UnityEngine.Object currentValue =
		Resolve(new PropertyName(exposedName.stringValue), exposedPropertyTable,
		        defaultValue.objectReferenceValue, out currentOverrideState);

	if (overrideState == OverrideState.DefaultValue) {
		menu.AddItem(
			new GUIContent(ExposePropertyContent.text), false, (userData) => {
				var guid = UnityEditor.GUID.Generate();
				...
				Undo.RecordObject(exposedPropertyTable as Object,
				                  "Set Exposed Property");
				exposedPropertyTable.SetReferenceValue(newPropertyName,
				                                       currentValue);
			}, null);
	} else {
