	...
	if (EditorGUI.EndChangeCheck()) {
		...
		if (property.propertyType == SerializedPropertyType.Float) {
			...
			property.vector2Value =
				new Vector2(Mathf.Max(minAttribute.min, value.x),
				            Mathf.Max(minAttribute.min, value.y));
		} else if (property.propertyType == SerializedPropertyType.Vector2Int) {
			var value = property.vector2IntValue;
			property.vector2IntValue =
				new Vector2Int(Mathf.Max((int) minAttribute.min, value.x),
				               Mathf.Max((int) minAttribute.min, value.y));
		} else if (property.propertyType == SerializedPropertyType.Vector3) {
			var value = property.vector3Value;
			property.vector3Value =
				new Vector3(Mathf.Max(minAttribute.min, value.x),
				            Mathf.Max(minAttribute.min, value.y),
				            Mathf.Max(minAttribute.min, value.z));
		} else if (property.propertyType == SerializedPropertyType.Vector3Int) {
			var value = property.vector3IntValue;
			property.vector3IntValue =
				new Vector3Int(Mathf.Max((int) minAttribute.min, value.x),
				               Mathf.Max((int) minAttribute.min, value.y),
				               Mathf.Max((int) minAttribute.min, value.z));
		} else if (property.propertyType == SerializedPropertyType.Vector4) {
			var value = property.vector4Value;
			property.vector4Value =
				new Vector4(Mathf.Max(minAttribute.min, value.x),
				            Mathf.Max(minAttribute.min, value.y),
				            Mathf.Max(minAttribute.min, value.z),
				            Mathf.Max(minAttribute.min, value.w));
		} else {
			EditorGUI.LabelField(position, label.text,
			                     "Use Min with float, int or Vector.");
		}
	}
}
}
...
public override void OnGUI(Rect position, SerializedProperty property,
	...
	if (property.propertyType == SerializedPropertyType.String) {
		...
		EditorGUI.indentLevel = 0; // The MultiFieldPrefixLabel already applied
		                           // indent, so avoid indent of TextArea itself.
...
public override float GetPropertyHeight(SerializedProperty property,
                                        GUIContent label) {
	return (EditorGUIUtility.wideMode ? 0
	    : (int) EditorGUI.kSingleLineHeight) // header
	       + EditorGUI.kSingleLineHeight     // first line
	       + (((MultilineAttribute) attribute).lines - 1) *
	       kLineHeight;   // remaining lines
}
}
...
public override void OnGUI(Rect position, SerializedProperty property,
	...
	if (property.propertyType == SerializedPropertyType.String) {
		...
		EditorGUI.BeginChangeCheck();
		string newValue = EditorGUI.ScrollableTextAreaInternal(
			position, property.stringValue, ref m_ScrollPosition,
		...
		EditorGUI.LabelField(position, label.text,
		                     "Use TextAreaDrawer with string.");
}
...
public override float GetPropertyHeight(SerializedProperty property,
	...
	float fullTextHeight = EditorStyles.textArea.CalcHeight(
		GUIContent.Temp(text), EditorGUIUtility.contextWidth);
	int lines = Mathf.CeilToInt(fullTextHeight / kLineHeight);

	lines = Mathf.Clamp(lines, textAreaAttribute.minLines,
...
public override void OnGUI(Rect position, SerializedProperty property,
	...
	if (property.propertyType == SerializedPropertyType.Color) {
		...
		EditorGUI.BeginChangeCheck();
		Color newColor =
			EditorGUI.ColorField(position, label, property.colorValue, true,
			                     colorUsage.showAlpha, colorUsage.hdr);
		if (EditorGUI.EndChangeCheck()) {
...
public override void OnGUI(Rect position, SerializedProperty property,
	...
	EditorGUI.BeginChangeCheck();
	Gradient newGradient = EditorGUI.GradientField(
		position, label, property.gradientValue, colorUsage.hdr);
	if (EditorGUI.EndChangeCheck()) {
...
public override void OnGUI(Rect position, SerializedProperty property,
	...
	if (property.propertyType == SerializedPropertyType.Float)
		...
		EditorGUI.LabelField(position, label.text,
		                     "Use Delayed with float, int, or string.");
}
}
}
