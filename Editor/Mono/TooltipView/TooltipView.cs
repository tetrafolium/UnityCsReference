...
protected override void OldOnGUI() {
	if (window != null) {
		...
		GUI.Box(new Rect(0, 0, m_optimalSize.x, m_optimalSize.y), m_tooltip,
		        m_Style);
		GUI.color = prevColor;
	}
}
...
void Setup(string tooltip, Rect rect, GUIView hostView) {
	...
	var popupPosition =
		new Rect(Mathf.Floor(m_hoverRect.x + (m_hoverRect.width / 2) -
		                     (m_optimalSize.x / 2)),
	...
	var fittedToScreen =
		ContainerWindow.FitRectToScreen(popupPosition, true, true);
...
public static void Show(string tooltip, Rect rect, GUIView hostView = null) {
	...
	if (s_guiView.window == null) {
		...
		newWindow.SetMinMaxSizes(new Vector2(10.0f, 10.0f),
