...
public static extern void SetStaticEditorFlags(GameObject go,
                                               StaticEditorFlags flags);
public static extern bool AreStaticEditorFlagsSet(GameObject go,
                                                  StaticEditorFlags flags);
...
GetFirstItemPathAfterGameObjectCreationMenuItems();

public static extern string GetUniqueNameForSibling(Transform parent,
                                                    string name);
...
internal static ShouldIncludeChildren
DisplayUpdateChildrenDialogIfNeeded(IEnumerable<GameObject> gameObjects,
	...
	return (ShouldIncludeChildren) EditorUtility.DisplayDialogComplex(
		title, message, L10n.Tr("Yes, change children"),
		L10n.Tr("No, this object only"), L10n.Tr("Cancel"));
}
...
GetMonoBehavioursWithMissingScriptCount(GameObject go);

[FreeFunction]
public static extern int RemoveMonoBehavioursWithMissingScript(GameObject go);

public static ulong
ModifyMaskIfGameObjectIsHiddenForPrefabModeInContext(ulong sceneCullingMask,
...
IsPrefabInstanceHiddenForInContextEditing(GameObject go);
