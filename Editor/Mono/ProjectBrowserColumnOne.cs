...
class SearchFilterTreeItem : TreeViewItem {
  bool m_IsFolder;
  public SearchFilterTreeItem(int id, int depth, TreeViewItem parent,
                              string displayName, bool isFolder)
...
internal class ProjectBrowserColumnOneTreeViewGUI : AssetsTreeViewGUI {
  ...
  Texture2D k_FavoriteFolderIcon =
  ...
  override public int GetNumRowsOnPageUpDown(TreeViewItem fromItem, bool pageUp,
                                             float heightOfTreeView) {
    return (int) Mathf.Floor(heightOfTreeView / k_LineHeight) -
           1; // -1 is fast fix for space between roots
  }

  // Should return the row number of the first and last row thats fits in the
  // pixel rect defined by top and height
  override public void GetFirstAndLastRowVisible(out int firstRowVisible,
    ...
    firstRowVisible -= (int) Mathf.Ceil(
        2 * rowsPerSpaceBetween); // for now we just add extra rows to ensure
  ...
  override public void OnRowGUI(Rect rowRect, TreeViewItem item, int row,
  ...
  bool IsVisibleRootNode(TreeViewItem item) {
    return (m_TreeView.data as ProjectBrowserColumnOneTreeViewDataSource)
        .IsVisibleRootNode(item);
  }
  ...
  public static float GetListAreaGridSize() {
    ...
    if (ProjectBrowser.s_LastInteractedProjectBrowser != null)
      previewSize =
          ProjectBrowser.s_LastInteractedProjectBrowser.listAreaGridSize;
    return previewSize;
  }
  ...
  virtual internal void BeginCreateSavedFilter(SearchFilter filter) {
    ...
    int instanceID = SavedSearchFilters.AddSavedFilter(savedFilterName, filter,
                                                       GetListAreaGridSize());
  ...
  override protected void RenameEnded() {
    ...
    if (m_IsCreatingSavedFilter) {
      ...
      // Ensure to sync filter to new folder name (so we still show the contents
      // of the folder)
      if (GetRenameOverlay().userAcceptedRename)
...
internal class ProjectBrowserColumnOneTreeViewDataSource
  ...
  public bool skipHiddenPackages {
    get;
    set;
  }
  ...
  public override bool CanBeMultiSelected(TreeViewItem item) {
    return ProjectBrowser.GetItemType(item.id) !=
           ProjectBrowser.ItemType.SavedFilter;
  }

  public override bool CanBeParent(TreeViewItem item) {
    return !(item is SearchFilterTreeItem) ||
           SavedSearchFilters.AllowsHierarchy();
  }
  ...
  public override void FetchData() {
    ...
    // Assets root
    int assetsFolderInstanceID =
        AssetDatabase.GetMainAssetOrInProgressProxyInstanceID("Assets");
    int depth = 0;
    string displayName = "Assets";
    AssetsTreeViewDataSource.RootTreeItem assetRootItem =
        new AssetsTreeViewDataSource.RootTreeItem(assetsFolderInstanceID, depth,
                                                  m_RootItem, displayName);
    assetRootItem.icon = s_FolderIcon;
    visibleRoots.Add(assetRootItem);

    // Packages root
    displayName = PackageManager.Folders.GetPackagesPath();
    AssetsTreeViewDataSource.RootTreeItem packagesRootItem =
        new AssetsTreeViewDataSource.RootTreeItem(
            ProjectBrowser.kPackagesFolderInstanceId, depth, m_RootItem,
            displayName);
    ...
    // Set global expanded state for roots from EditorPrefs (must be before
    // building the rows)
    if (firstInitialize) {
      foreach (TreeViewItem item in m_RootItem.children) {
        ...
        if (item.id == ProjectBrowser.kPackagesFolderInstanceId)
          continue;
        bool expanded =
            EditorPrefs.GetBool(kProjectBrowserString + item.displayName, true);
    ...
    var packages =
        PackageManagerUtilityInternal.GetAllVisiblePackages(skipHiddenPackages);
    if (IsExpanded(packagesRootItem)) {
      ...
      foreach (var package in packages) {
        ...
        displayName = !string.IsNullOrEmpty(package.displayName)
                          ? package.displayName
                          : package.name;
        AssetsTreeViewDataSource.PackageTreeItem packageItem =
            new AssetsTreeViewDataSource.PackageTreeItem(
                packageFolderInstanceId, depth, packagesRootItem, displayName);
  ...
  private void ReadAssetDatabase(string assetFolderRootPath,
                                 TreeViewItem parent, int baseDepth,
                                 IList<TreeViewItem> allRows) {
    ...
    while (property.Next(expandedIDs)) {
      if (property.isFolder) {
        AssetsTreeViewDataSource.FolderTreeItem folderItem =
            new AssetsTreeViewDataSource.FolderTreeItem(
                property.guid, !property.hasChildren,
                property.GetInstanceIDIfImported(), baseDepth + property.depth,
                null, property.name);
        ...
        } else // expanded status does not get updated when deleting/moving
               // folders. We need to check if the expanded folder still has
               // subFolders when reading the AssetDatabase
        {
          if (!HasSubFolders(property))
...
internal class ProjectBrowserColumnOneTreeViewDragging
  ...
  public override DragAndDropVisualMode DoDrag(TreeViewItem parentItem,
    ...
    object savedFilterData = DragAndDrop.GetGenericData(
        ProjectWindowUtil.k_DraggingFavoriteGenericData);
    ...
    if (savedFilterData != null) {
      ...
      if (targetItem is SearchFilterTreeItem &&
          parentItem is
              SearchFilterTreeItem) // && targetItem.id != draggedInstanceID &&
                                    // parentItem.id != draggedInstanceID)
      {
        bool validMove = SavedSearchFilters.CanMoveSavedFilter(
            instanceID, parentItem.id, targetItem.id,
            dropPos == DropPosition.Below);
        if (validMove && perform) {
          SavedSearchFilters.MoveSavedFilter(instanceID, parentItem.id,
                                             targetItem.id,
                                             dropPos == DropPosition.Below);
          m_TreeView.SetSelection(new[]{instanceID}, false);
          m_TreeView.NotifyListenersThatSelectionChanged();
        }
        return validMove ? DragAndDropVisualMode.Copy
    ...
    // Dragging of folders into filters
      ...
      if (targetItem is SearchFilterTreeItem) {
        string genericData = DragAndDrop.GetGenericData(
            ProjectWindowUtil.k_IsFolderGenericData) as string;
        if (genericData == "isFolder") {
          if (perform) {
            ...
            if (objs.Length > 0) {
              ...
              if (!string.IsNullOrEmpty(path)) {
                // TODO: Fix with new AssetDatabase API when it is ready
                // (GetName)
                ...
                float previewSize =
                    ProjectBrowserColumnOneTreeViewGUI.GetListAreaGridSize();
                int instanceID =
                    SavedSearchFilters.AddSavedFilterAfterInstanceID(
                        folderName, searchFilter, previewSize, targetItem.id,
                        addAsChild);
                m_TreeView.SetSelection(new[]{instanceID}, false);
                m_TreeView.NotifyListenersThatSelectionChanged();
              } else {
                Debug.Log("Could not get asset path from id " +
                          objs [0]
                              .GetInstanceID());
              }
            }
          ...
          return DragAndDropVisualMode
              .Copy; // Allow dragging folders to filters
        }
        return DragAndDropVisualMode
            .None; // Assets that are not folders are not allowed to be dragged
                   // to filters
      }
    }
