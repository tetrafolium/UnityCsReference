...
internal class CustomEditorAttributes {
  private static readonly Dictionary<Type, List<MonoEditorType>>
      kSCustomEditors = new Dictionary<Type, List<MonoEditorType>>();
  private static readonly Dictionary<Type, List<MonoEditorType>>
      kSCustomMultiEditors = new Dictionary<Type, List<MonoEditorType>>();
  ...
  internal static Type FindCustomEditorType(UnityEngine.Object o,
                                            bool multiEdit) {
    return FindCustomEditorTypeByType(o.GetType(), multiEdit);
  }

  private static List<MonoEditorType> s_SearchCache =
      new List<MonoEditorType>();
  internal static Type FindCustomEditorTypeByType(Type type, bool multiEdit) {
    ...
    for (int pass = 0; pass < 2; ++pass) {
      for (Type inspected = type; inspected != null;
        ...
        foreach (var result in foundEditors) {
          if (!IsAppropriateEditor(result, inspected, type != inspected,
                                   pass == 1))
            continue;
  ...
  private static bool IsAppropriateEditor(MonoEditorType editor,
                                          Type parentClass, bool isChildClass,
                                          bool isFallback) {
    if (isChildClass && !editor.m_EditorForChildClasses)
      // skip if it's a child class and this editor doesn't want to match on
      // children
      return false;
    ...
    return parentClass == editor.m_InspectedType ||
           (parentClass.IsGenericType &&
            parentClass.GetGenericTypeDefinition() == editor.m_InspectedType);
  }
    ...
    foreach (var type in types) {
      ...
      foreach (CustomEditor inspectAttr in attrs) {
        ...
        if (inspectAttr.m_InspectedType == null)
          Debug.Log("Can't load custom inspector " + type.Name +
                    " because the inspected type is null.");
        else if (!type.IsSubclassOf(typeof(Editor))) {
          // Suppress a warning on TweakMode, we did this bad in the default
          // project folder and it's going to be too hard for customers to
          // figure out how to fix it and also quite pointless.
          ...
          if (!kSCustomEditors.TryGetValue(inspectAttr.m_InspectedType,
                                           out editors)) {
          ...
          if (type.GetCustomAttributes(typeof(CanEditMultipleObjects), false)
                  .Length > 0) {
            List<MonoEditorType> multiEditors;
            if (!kSCustomMultiEditors.TryGetValue(inspectAttr.m_InspectedType,
                                                  out multiEditors)) {
