...
public class SearchableEditorWindow : EditorWindow {
  ...
  internal static SearchFilter CreateFilter(string searchString,
  ...
  static List<SearchableEditorWindow> searchableWindows =
      new List<SearchableEditorWindow>();
  private static int s_SearchableEditorWindowSearchField =
      "SearchableEditorWindowSearchField".GetHashCode();
  ...
  virtual public void OnEnable() { searchableWindows.Add(this); }

  virtual public void OnDisable() { searchableWindows.Remove(this); }
  ...
  internal static void SearchForReferencesToInstanceID(int instanceID) {
    ...
    // Don't remove "Assets" prefix, we need to support Packages as well
  ...
  internal void FocusSearchField() { m_FocusSearchField = true; }

  internal void ClearSearchFilter() {
    SetSearchFilter("", m_SearchMode, true, false);
    // Reset current editor. This is needed, so if the user types into a search
    // field, and then a new object is selected, which is not in the filter,
    // with the editor still having keyboard focus, the search field gets
    // properly cleared.
  ...
  internal virtual void SetSearchFilter(string searchFilter, SearchMode mode,
    ...
    if (setAll) {
      foreach (SearchableEditorWindow sw in searchableWindows) {
        if (sw != this && sw.m_HierarchyType == m_HierarchyType &&
            m_SearchGroup == sw.m_SearchGroup &&
    ...
    if (delayed) {
      m_NextSearch =
          EditorApplication.timeSinceStartup + k_SearchTimerDelaySecs;
    } else {
      Repaint();
      EditorApplication.Internal_CallSearchHasChanged();
    }
  ...
  internal void SearchFieldGUI(float maxWidth) {
    Rect rect = GUILayoutUtility.GetRect(EditorGUILayout.kLabelFloatMaxW * 0.2f,
                                         maxWidth, EditorGUI.kSingleLineHeight,
    ...
    string[] enumStrings = System.Enum.GetNames(
        m_HierarchyType ==
            HierarchyType.GameObjects ? typeof(SearchModeHierarchyWindow)
        : typeof(SearchMode));
    int searchFieldControlId = GUIUtility.GetControlID(
        s_SearchableEditorWindowSearchField, FocusType.Keyboard, rect);

    EditorGUI.BeginChangeCheck();
    string searchFilter =
        EditorGUI.ToolbarSearchField(searchFieldControlId, rect, enumStrings,
                                     ref searchMode, m_SearchFilter);
    ...
    if (Event.current.type == EventType.KeyDown &&
        Event.current.keyCode == KeyCode.Escape && m_SearchFilter != "" &&
