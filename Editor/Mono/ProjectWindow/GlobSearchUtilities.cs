...
static readonly Regex k_BasicSymbolsRegex = new Regex(
	@"(?<range>\\\[..+\])|(?<dstarfold>\\\*\\\*/)|(?<dstar>\\\*\\\*)|(?<star>\\\*)|(?<single>\\\?)");
static readonly Regex k_ComplexSymbolsRegex =
	new Regex(@"(?<or>\\\(.+(?:\\\|.+)+\\\))");
...
static string GlobToRegex(string glob) {
	...
	// Complex patterns are replaced in a second pass because they may contain
	// basic symbols that we want to replace first.
	...
	// Searches like "Editor" should match the same as "**Editor" and looks only
	// for Editor folders or file in any subfolder This is why we are always
	// adding optional folder path at the beginning and option folder ending
...
static bool IsRegexValid(string regex) {
	try {
		...
		// we use this to make sure the generated regex can be used during a
		// search.
		Regex.Match("", regex);
