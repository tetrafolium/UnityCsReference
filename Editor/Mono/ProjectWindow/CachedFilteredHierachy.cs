...
internal partial class FilteredHierarchy {
      ...
      get {
        if (m_Icon == null) {
          if (type == HierarchyType.Assets) {
            // Note: Do not set m_Icon as GetCachedIcon uses its own cache that
            // is cleared on reaching a max limit. This is because when having
            // e.g very large projects (1000s of textures with unique icons) we
            // do not want all icons loaded at the same time so don't keep a
            // reference in m_Icon here
            string path =
                instanceID == 0 ? null : AssetDatabase.GetAssetPath(instanceID);

            if (path != null)
              // Finding icon based on only file extension fails in several
              // ways, and a different approach have to be found. Using
              ...
              //  types, such as prefab variants.
              // Support for specific file types based on file extensiom have to
              // be supported inside AssetDatabase.GetCachedIcon itself to work
              // correctly and universally. for e.g. uxml files from within
              // GetCachedIcon without relying on FindIconForFile.
              return AssetDatabase.GetCachedIcon(path) as Texture2D;

            path = string.IsNullOrEmpty(m_Guid)
                       ? null
                       : AssetDatabase.GUIDToAssetPath(m_Guid);
            if (path != null)
              return UnityEditorInternal.InternalEditorUtility.FindIconForFile(
                  path);
          } else if (type == HierarchyType.GameObjects) {
            // GameObject thumbnail can be set to m_Icon since its an actual
            // icon which means we have a limited set of them
  ...
  FilterResult[] m_Results = new FilterResult[0]; // When filtering of folder we
                                                  // have all sub assets here
  FilterResult[] m_VisibleItems =
      new FilterResult[0]; // Subset of m_Results used for showing/hiding sub
                           // assets

  SearchService.SearchSessionHandler m_SearchSessionHandler =
  ...
  public void SetResults(int[] instanceIDs) {
    ...
    if (m_HierarchyType == HierarchyType.Assets) {
      ...
      HierarchyProperty property =
          new HierarchyProperty(m_HierarchyType, false);
      property.Reset();
  ...
  void SetAssetsResults(HashSet<int> instanceIdsSet,
                        Dictionary<string, int> idsUnderEachRoot) {
  ...
  void CopyPropertyData(ref FilterResult result, HierarchyProperty property) {
    ...
    // If this is not the main representation, cache the icon, as we don't have
    // an API to access it later. Otherwise, don't - as this may cause Textures
    // to load unintendedly (e.g if we have 3000 search results we do not want
    // to load icons before needed when rendering)
    if (!property.isMainRepresentation)
      result.icon = property.icon;
    else if (property.isFolder && !property.hasChildren)
      result.icon =
          EditorGUIUtility.FindTexture(EditorResources.emptyFolderIconName);
    else
      result.icon = null;
    ...
    if (m_HierarchyType == HierarchyType.Assets) {
      ...
      m_SearchSessionHandler.BeginSession(() => {
        return new SearchService.HierarchySearchContext{
            filter = m_SearchFilter, rootProperty = property,
  ...
  void FolderBrowsing() {
    // We are not concerned with assets being added multiple times as we only
    // show the contents of each selected folder. This is an issue when
    // searching recursively into child folders.
    List<FilterResult> list = new List<FilterResult>();
    HierarchyProperty property;
      ...
      if (folderPath == PackageManager.Folders.GetPackagesPath()) {
        ...
        foreach (var package in packages) {
          ...
          if (property.Find(packageFolderInstanceId, null)) {
            FilterResult result = new FilterResult();
            CopyPropertyData(ref result, property);
            result.name = !string.IsNullOrEmpty(package.displayName)
                              ? package.displayName
                              : package.name;
            list.Add(result);
          }
  ...
  void AddResults() {
    ...
    case SearchFilter.State.SearchingInAllAssets:
      SearchAllAssets(SearchFilter.SearchArea.AllAssets);
      break;
    case SearchFilter.State.SearchingInAssetsOnly:
      SearchAllAssets(SearchFilter.SearchArea.InAssetsOnly);
      break;
    case SearchFilter.State.SearchingInPackagesOnly:
      SearchAllAssets(SearchFilter.SearchArea.InPackagesOnly);
      break;
    case SearchFilter.State.SearchingInFolders:
      SearchInFolders();
      break;
    case SearchFilter.State.SearchingInAssetStore: /*do nothing*/
      break;
    case SearchFilter.State.EmptySearchFilter: /*do nothing*/
      break;
    default:
  ...
  public void ResultsChanged() {
    ...
    if (m_SearchFilter.GetState() != SearchFilter.State.EmptySearchFilter) {
      ...
      // When filtering on folder we use the order we get from
      // BaseHiearchyProperty.cpp (to keep indented children under parent)
      // otherwise we sort
      if (m_SearchFilter.IsSearching()) {
        System.Array.Sort(m_Results,
                          (result1, result2) => EditorUtility.NaturalCompare(
                              result1.name, result2.name));
      }

      if (foldersFirst) {
        for (int nonFolderPos = 0; nonFolderPos < m_Results.Length;
             ++nonFolderPos) {
          if (m_Results[nonFolderPos].isFolder)
            continue;

          for (int folderPos = nonFolderPos + 1; folderPos < m_Results.Length;
               ++folderPos) {
            ...
            System.Array.Copy(m_Results, nonFolderPos, m_Results,
                              nonFolderPos + 1, length);
      ...
      // Reset visible flags if filter string is empty (see
      // BaseHiearchyProperty::SetSearchFilter)
      if (m_HierarchyType == HierarchyType.GameObjects) {
        HierarchyProperty gameObjects =
            new HierarchyProperty(HierarchyType.GameObjects, false);
  ...
  public void RefreshVisibleItems(List<int> expandedInstanceIDs) {
    ...
    for (int i = 0; i < m_Results.Length; ++i) {
      ...
      if (m_Results[i].isMainRepresentation && m_Results[i].hasChildren &&
          !m_Results[i].isFolder) {
        bool isParentExpanded =
            expandedInstanceIDs.IndexOf(m_Results[i].instanceID) >= 0;
        bool addSubItems = isParentExpanded || isSearching;
        int numSubItems =
            AddSubItemsOfMainRepresentation(i, addSubItems ? visibleItems
                                            : null);
        i += numSubItems;
      }
    }
  ...
  public List<int> GetSubAssetInstanceIDs(int mainAssetInstanceID) {
    for (int i = 0; i < m_Results.Length; ++i) {
      if (m_Results[i].instanceID == mainAssetInstanceID) {
        ...
        while (index < m_Results.Length &&
               !m_Results[index].isMainRepresentation) {
  ...
  public int AddSubItemsOfMainRepresentation(int mainRepresentionIndex,
                                             List<FilterResult> visibleItems) {
    int count = 0;
    int index =
        mainRepresentionIndex + 1; // Start after the main representation
...
internal class FilteredHierarchyProperty : IHierarchyProperty {
  ...
  CreateHierarchyPropertyForFilter(FilteredHierarchy filteredHierarchy) {
    if (filteredHierarchy.searchFilter.GetState() !=
        SearchFilter.State.EmptySearchFilter)
  ...
  public int instanceID {
    get {
      ...
      if (id == 0)
        m_Hierarchy.results[m_Position].instanceID =
            AssetDatabase.GetMainAssetInstanceID(guid);
  ...
  public bool IsExpanded(int[] expanded) { return false; }
  ...
  public bool isValid {
    get {
      return m_Hierarchy.results != null &&
             m_Position < m_Hierarchy.results.Length && m_Position >= 0;
    }
  }
  ...
  public bool NextWithDepthCheck(int[] expanded, int minDepth) {
    // Depth check does not make sense for filtered properties as tree info is
    // lost
  ...
  public bool Parent() { return false; }
  ...
  public int[] FindAllAncestors(int[] instanceIDs) { return new int[0]; }
