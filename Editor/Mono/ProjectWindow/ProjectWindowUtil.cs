...
public abstract class EndNameEditAction : ScriptableObject {
  ...
  public abstract void Action(int instanceId, string pathName,
                              string resourceFile);
  public virtual void Cancelled(int instanceId, string pathName,
                                string resourceFile) {}

  public virtual void CleanUp() { DestroyImmediate(this); }
}
...
internal class DoCreateFolder : EndNameEditAction {
  public override void Action(int instanceId, string pathName,
                              string resourceFile) {
    string guid = AssetDatabase.CreateFolder(Path.GetDirectoryName(pathName),
                                             Path.GetFileName(pathName));
    Object o = AssetDatabase.LoadAssetAtPath(
        AssetDatabase.GUIDToAssetPath(guid), typeof(Object));
...
internal class DoCreateScene : EndNameEditAction {
  public override void Action(int instanceId, string pathName,
    ...
    if (EditorSceneManager.CreateSceneAsset(pathName,
                                            createDefaultGameObjects)) {
      Object sceneAsset =
          AssetDatabase.LoadAssetAtPath(pathName, typeof(SceneAsset));
...
internal class DoCreateFolderWithTemplates : EndNameEditAction {
  ...
  public override void Action(int instanceId, string pathName,
    ...
    foreach (var template in templates ?? Enumerable.Empty<string>()) {
      ...
      ProjectWindowUtil.CreateScriptAssetFromTemplate(
          Path.Combine(pathName, fileName + templateExtension),
          Path.Combine(basePath, template));
    }
...
internal class DoCreateAssetWithContent : EndNameEditAction {
  ...
  public override void Action(int instanceId, string pathName,
                              string resourceFile) {
    Object o =
        ProjectWindowUtil.CreateScriptAssetWithContent(pathName, filecontent);
    ProjectWindowUtil.ShowCreatedAsset(o);
  }
}
...
internal class DoCreateAnimatorController : EndNameEditAction {
  public override void Action(int instanceId, string pathName,
    ...
    Animations.AnimatorController controller =
        Animations.AnimatorController.CreateAnimatorControllerAtPath(pathName);
...
internal class DoCreateAudioMixer : EndNameEditAction {
  public override void Action(int instanceId, string pathName,
    ...
    AudioMixerController controller =
        AudioMixerController.CreateMixerControllerAtPath(pathName);

    // Check if the output group should be initialized (instanceID is stored in
    // the resource file) TODO: rename 'resourceFile' to 'userData' so it's more
    // obvious that it can be used by all EndNameEditActions
    if (!string.IsNullOrEmpty(resourceFile)) {
      int outputInstanceID;
      if (System.Int32.TryParse(resourceFile, out outputInstanceID)) {
        var outputGroup = InternalEditorUtility.GetObjectFromInstanceID(
            outputInstanceID) as AudioMixerGroupController;
        if (outputGroup != null)
...
internal class DoCreateSpriteAtlas : EndNameEditAction {
  ...
  public override void Action(int instanceId, string pathName,
    ...
    InternalEditorUtility.SaveToSerializedFileAndForget(
        new Object[]{spriteAtlasAsset}, pathName, true);
...
public class ProjectWindowUtil {
  ...
  public static void CreateNewGUISkin() {
    ...
    GUISkin original = Resources.GetBuiltinResource(
        typeof(GUISkin), "GameSkin/GameSkin.guiskin") as GUISkin;
    if (original)
  ...
  // Returns the path of currently selected folder. If multiple are selected,
  ...
  internal static void EndNameEditAction(EndNameEditAction action,
                                         int instanceId, string pathName,
                                         string resourceFile, bool accepted) {
  ...
  public static void CreateAsset(Object asset, string pathName) {
    StartNameEditingIfProjectWindowExists(
        asset.GetInstanceID(),
        ScriptableObject.CreateInstance<DoCreateNewAsset>(), pathName,
  ...
  public static void CreateFolder() {
    StartNameEditingIfProjectWindowExists(
        0, ScriptableObject.CreateInstance<DoCreateFolder>(), "New Folder",
        EditorGUIUtility.IconContent(EditorResources.emptyFolderIconName)
            .image as Texture2D,
        null);
  }

  internal static void CreateFolderWithTemplates(string defaultName,
    ...
    var endNameEditAction =
        ScriptableObject.CreateInstance<DoCreateFolderWithTemplates>();
    endNameEditAction.templates = templates;
    StartNameEditingIfProjectWindowExists(
        0, endNameEditAction, defaultName,
        EditorGUIUtility.IconContent(folderIcon).image as Texture2D, null);
  }

  internal static void CreateFolderWithTemplatesWithCustomResourcesPath(
      string defaultName, string customResPath, params string[] templates) {
    var folderIcon = templates != null && templates.Length > 0
                         ? EditorResources.folderIconName
                         : EditorResources.emptyFolderIconName;
  ...
  public static void CreateScene() {
    StartNameEditingIfProjectWindowExists(
        0, ScriptableObject.CreateInstance<DoCreateScene>(), "New Scene.unity",
        EditorGUIUtility.FindTexture(typeof(SceneAsset)), null);
  }
  ...
  static void CreatePrefabVariant() {
    ...
    string sourceDir =
        Path.GetDirectoryName(sourcePath).ConvertSeparatorsToUnity();
    string variantPath =
        string.Format("{0}/{1} Variant.prefab", sourceDir, go.name);

    StartNameEditingIfProjectWindowExists(
        0, ScriptableObject.CreateInstance<DoCreatePrefabVariant>(),
        variantPath,
  ...
  public static void
  CreateScriptAssetFromTemplateFile(string templatePath,
    ...
    case ".shader":
      ...
      break;
    case ".asmdef":
      icon = EditorGUIUtility.IconContent<AssemblyDefinitionAsset>()
                 .image as Texture2D;
      break;
    case ".asmref":
      icon = EditorGUIUtility.IconContent<AssemblyDefinitionReferenceAsset>()
                 .image as Texture2D;
      break;
    default:
    ...
    StartNameEditingIfProjectWindowExists(
        0, ScriptableObject.CreateInstance<DoCreateScriptAsset>(),
        defaultNewFileName, icon, templatePath);
  }
  ...
  static private void CreateAnimatorController() {
    var icon = EditorGUIUtility.IconContent<Animations.AnimatorController>()
                   .image as Texture2D;
    StartNameEditingIfProjectWindowExists(
        0, ScriptableObject.CreateInstance<DoCreateAnimatorController>(),
        "New Animator Controller.controller", icon, null);
  }

  static private void CreateAudioMixer() {
    var icon =
        EditorGUIUtility.IconContent<AudioMixerController>().image as Texture2D;
    StartNameEditingIfProjectWindowExists(
        0, ScriptableObject.CreateInstance<DoCreateAudioMixer>(),
        "NewAudioMixer.mixer", icon, null);
  }

  static private void CreateSpriteAtlas() {
    var icon =
        EditorGUIUtility.IconContent<SpriteAtlasAsset>().image as Texture2D;
    DoCreateSpriteAtlas action =
        ScriptableObject.CreateInstance<DoCreateSpriteAtlas>();
    StartNameEditingIfProjectWindowExists(
        0, action, "New Sprite Atlas.spriteatlasv2", icon, null);
  }
  ...
  static private void CreateSpritePolygon(int sides) {
    ...
    DoCreateSpritePolygon action =
        ScriptableObject.CreateInstance<DoCreateSpritePolygon>();
    action.sides = sides;
    StartNameEditingIfProjectWindowExists(0, action, assetName + ".png", icon,
                                          null);
  }

  internal static string SetLineEndings(string content,
                                        LineEndingsMode lineEndingsMode) {
  ...
  internal static Object CreateScriptAssetWithContent(string pathName,
                                                      string templateContent) {
    templateContent = SetLineEndings(templateContent,
                                     EditorSettings.lineEndingsForNewScripts);

    string fullPath = Path.GetFullPath(pathName);

    // utf8-bom encoding was added for case 510374 in 2012. i think this was the
    // wrong solution. BOM's are problematic for diff tools, naive readers and
    // writers (of which we have many!), and generally not something most people
    // think about. you wouldn't believe how many unity source files have BOM's
    // embedded in the middle of them for no reason. copy paste problem? bad
    // tool? unity should instead have been fixed to read all files that have no
    // BOM as utf8 by default, and then we just strip them all, always, from
    // files we control. perhaps we'll do this one day and this next line can be
    // removed. -scobi
    var encoding =
  ...
  internal static string RemoveOrInsertNamespace(string content,
                                                 string rootNamespace) {
    var rootNamespaceBeginTag = "#ROOTNAMESPACEBEGIN#";
    var rootNamespaceEndTag = "#ROOTNAMESPACEEND#";

    if (!content.Contains(rootNamespaceBeginTag) ||
        !content.Contains(rootNamespaceEndTag))
      return content;
    ...
    // Use first found newline character as newline for entire file after
    // replace.
    ...
    for (; i < contentLines.Count; ++i) {
      if (contentLines [i]
              .Contains(rootNamespaceBeginTag))
        break;
    }

    var beginTagLine = contentLines[i];

    // Use the whitespace between beginning of line and #ROOTNAMESPACEBEGIN# as
    // identation.
    var indentationString =
        beginTagLine.Substring(0, beginTagLine.IndexOf("#"));
  ...
  internal static Object CreateScriptAssetFromTemplate(string pathName,
    ...
    // #NOTRIM# is a special marker that is used to mark the end of a line where
    // we want to leave whitespace. prevent editors auto-stripping it by
    // accident.
    ...
    // if the script name begins with an uppercase character we support a
    // lowercase substitution variant
    if (char.IsUpper(baseFileNoSpaces, 0)) {
      baseFileNoSpaces =
          char.ToLower(baseFileNoSpaces[0]) + baseFileNoSpaces.Substring(1);
      content = content.Replace("#SCRIPTNAME_LOWER#", baseFileNoSpaces);
    } else {
      // still allow the variant, but change the first character to upper and
      // prefix with "my"
      baseFileNoSpaces = "my" + char.ToUpper(baseFileNoSpaces[0]) +
                         baseFileNoSpaces.Substring(1);
      content = content.Replace("#SCRIPTNAME_LOWER#", baseFileNoSpaces);
    }
  ...
  public static void StartNameEditingIfProjectWindowExists(
      int instanceID, EndNameEditAction endAction, string pathName,
      Texture2D icon, string resourceFile) {
    ProjectBrowser pb = GetProjectBrowserIfExists();
    if (pb) {
      pb.Focus();
      pb.BeginPreimportedNameEditing(instanceID, endAction, pathName, icon,
                                     resourceFile);
      pb.Repaint();
    } else {
  ...
  internal static bool IsFavoritesItem(int instanceID) {
    return instanceID >= k_FavoritesStartInstanceID &&
           instanceID != ProjectBrowser.kPackagesFolderInstanceId;
  }

  internal static void StartDrag(int draggedInstanceID,
    ...
    if (IsFavoritesItem(draggedInstanceID)) {
      DragAndDrop.SetGenericData(k_DraggingFavoriteGenericData,
                                 draggedInstanceID);
    } else {
      // Normal assets dragging
      bool isFolder = IsFolder(draggedInstanceID);
      DragAndDrop.objectReferences =
          GetDragAndDropObjects(draggedInstanceID, selectedInstanceIDs);
      DragAndDrop.SetGenericData(k_IsFolderGenericData, isFolder ? "isFolder"
                                 : "");
      string[] paths =
          GetDragAndDropPaths(draggedInstanceID, selectedInstanceIDs);
      if (paths.Length > 0)
      ...
      if (DragAndDrop.objectReferences.Length > 1)
        ...
        title = ObjectNames.GetDragAndDropTitle(
  ...
  internal static Object[] GetDragAndDropObjects(
      int draggedInstanceID, List<int> selectedInstanceIDs) {
    List<Object> outList = new List<Object>(selectedInstanceIDs.Count);
    if ((Event.current.control || Event.current.command) &&
        !selectedInstanceIDs.Contains(draggedInstanceID)) {
    ...
    if (selectedInstanceIDs.Contains(draggedInstanceID)) {
      for (int i = 0; i < selectedInstanceIDs.Count; ++i) {
        Object obj = InternalEditorUtility.GetObjectFromInstanceID(
  ...
  internal static string[] GetDragAndDropPaths(int draggedInstanceID,
                                               List<int> selectedInstanceIDs) {
    // Assets
  ...
  // Returns instanceID of folders (and main asset if input is a subasset) up
  // until and including the Assets folder
  ...
  internal static void GetAncestors(int instanceID, HashSet<int> ancestors) {
    ...
    string currentFolderPath =
        GetContainingFolder(AssetDatabase.GetAssetPath(mainAssetInstanceID));
    while (!string.IsNullOrEmpty(currentFolderPath)) {
      int currentInstanceID =
          ProjectBrowser.GetFolderInstanceID(currentFolderPath);
      ancestors.Add(currentInstanceID);
      currentFolderPath =
          GetContainingFolder(AssetDatabase.GetAssetPath(currentInstanceID));
    }
  }
  ...
  public static string[] GetBaseFolders(string[] folders) {
    ...
    // Remove forward slashes before sorting otherwise will "Assets 1/" come
    // before "Assets/" which we do not want in the find base folders section
    // below
    ...
    // Ensure folder paths are ending with '/' so e.g: "assets/" is not found in
    // "assets 1/". If we did not end with '/' then "assets" could be found in
    // "assets 1" which is not what we want when finding base folders
    for (int i = 0; i < sortedFolders.Count; ++i)
      if (!sortedFolders [i]
               .EndsWith("/"))
        sortedFolders[i] = sortedFolders[i] + "/";
    ...
    // Find base folders
    // We assume sortedFolders is sorted with less first. E.g: {assets/,
    // assets/icons/}
    string curPath = sortedFolders[0];
    result.Add(curPath);
    for (int i = 1; i < sortedFolders.Count; ++i) {
      // Ensure path matches from start of curPath (to ensure "assets/monkey"
      // and "npc/assets/monkey" both are returned as base folders)
      bool startOfPathMatches =
          sortedFolders [i]
              .IndexOf(curPath, StringComparison.Ordinal) == 0;
      if (!startOfPathMatches) {
        // Add tested path if not part of current path and use tested path as
        // new base
  ...
  // Deletes the assets of the instance IDs, with an optional user confirmation
  // dialog. Returns true if the delete operation was successfully performed on
  // all assets. Note: Zero input assets always returns true. Also note that the
  // operation cannot be undone even if some operations failed.
  internal static bool DeleteAssets(List<int> instanceIDs, bool askIfSure) {
    if (instanceIDs.Count == 0)
      return true;

    bool foundAssetsFolder =
        instanceIDs.IndexOf(
            AssetDatabase.GetMainAssetOrInProgressProxyInstanceID("Assets")) >=
        0;
    if (foundAssetsFolder) {
      EditorUtility.DisplayDialog(
          L10n.Tr("Cannot Delete"),
          L10n.Tr("Deleting the 'Assets' folder is not allowed"),
          L10n.Tr("Ok"));
      return false;
    }
    ...
    if (!success) {
      string message =
          (Provider.enabled && Provider.onlineState == OnlineState.Offline)
              ? L10n.Tr(
      ...
      EditorUtility.DisplayDialog(L10n.Tr("Cannot Delete"), message,
                                  L10n.Tr("Ok"));
    }
    return success;
  }
  ...
  DuplicateAssets(IEnumerable<Object> assets) {
    ...
    foreach (var asset in assets) {
      ...
      // if duplicating a sub-asset, then create a copy next to the main asset
      // file
      if (!String.IsNullOrEmpty(assetPath) && AssetDatabase.IsSubAsset(asset)) {
        if (asset is ISubAssetNotDuplicatable) {
          firstDuplicatedObjectToFail =
              firstDuplicatedObjectToFail ? firstDuplicatedObjectToFail : asset;
          continue;
        }
        ...
        // since the assertdb will do it for us and has a whole tailored logic
        // for that.

        // It feels wrong that the asset name (that can apparently contain any
        // char) is conflated with the orthogonal notion of filename. From the
        // user's POV it will force an asset dup but with mangled names if the
        // original name contained "invalid chars" for filenames. Path.Combine
        // is not used here to avoid blocking asset names that might contain
        // chars not allowed in filenames.
        if ((new HashSet<char>(Path.GetInvalidFileNameChars()))
                .Intersect(asset.name)
                .Count() != 0) {
          Debug.LogWarning(string.Format(
              "Duplicated asset name '{0}' contains invalid characters. Those will be replaced in the duplicated asset name.",
              asset.name));
        }

        var newPath = AssetDatabase.GenerateUniqueAssetPath(
            string.Format("{0}{1}{2}.{3}", Path.GetDirectoryName(assetPath),
                          Path.DirectorySeparatorChar, asset.name, extension));
  ...
  internal static IEnumerable<Object>
  DuplicateAssets(IEnumerable<int> instanceIDs) {
    return DuplicateAssets(
        instanceIDs.Select(id => EditorUtility.InstanceIDToObject(id)));
  }

  internal static IEnumerable<string>
  GetMainPathsOfAssets(IEnumerable<int> instanceIDs) {
