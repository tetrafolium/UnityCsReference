...
[RequiredByNativeCode]
[System.Obsolete(
	 "PostProcessAttribute has been renamed to CallbackOrderAttribute.")]
public abstract partial class PostProcessAttribute : CallbackOrderAttribute {
[System.Obsolete(
	 "PostProcessAttribute has been renamed. Use m_CallbackOrder of CallbackOrderAttribute.")]
protected int m_PostprocessOrder;

[System.Obsolete(
	 "PostProcessAttribute has been renamed. Use callbackOrder of CallbackOrderAttribute.")]
internal int GetPostprocessOrder {
	get { return m_PostprocessOrder; }
}
}

namespace Callbacks {
internal sealed partial class RegisterPluginsAttribute
	: CallbackOrderAttribute {
public RegisterPluginsAttribute() {
	m_CallbackOrder = 1;
}
public RegisterPluginsAttribute(int callbackOrder) {
	m_CallbackOrder = callbackOrder;
}

[RequiredSignature]
static IEnumerable<PluginDesc> Signature(BuildTarget target) {
...
[RequiredSignature]
static void Signature(BuildTarget target, string pathToBuiltProject) {
	throw new InvalidOperationException();
}
}
...
// Add this attribute to a static method to get a callback for opening an asset
// inside Unity before trying to open it with an external tool
[RequiredByNativeCode]
...
[RequiredSignature]
static bool SignatureLine(int instanceID, int line) {
	throw new InvalidOperationException();
}

[RequiredSignature]
static bool SignatureLineColumn(int instanceID, int line, int column) {
	throw new InvalidOperationException();
}
}
}
