...
internal enum ShowMode {
  ...
  Utility = 2,
  // Window has no shadow or decorations. Used internally for dragging stuff
  // around.
  NoShadow = 3,
  // The Unity main window. On mac, this is the same as NormalWindow, except
  // window doesn't have a close button.
  MainWindow = 4,
...
internal partial class ContainerWindow {
  ...
  public extern Rect position {
    [FreeFunction(k_ScriptingPrefix + "GetPosition",
                  HasExplicitThis = true)] get;
    [FreeFunction(k_ScriptingPrefix + "SetPosition",
                  HasExplicitThis = true)] set;
  }

  public extern bool maximized {
    [FreeFunction(k_ScriptingPrefix + "IsWindowMaximized",
                  HasExplicitThis = true)] get;
  }
  ...
  [FreeFunction(k_ScriptingPrefix + "FitWindowRectToScreen",
                HasExplicitThis = true)]
  public extern Rect FitWindowRectToScreen(Rect r, bool forceCompletelyVisible,
                                           bool useMouseScreen);
  ...
  [FreeFunction(k_ScriptingPrefix + "Internal_SetMinMaxSizes",
                HasExplicitThis = true)]
  ...
  private extern void Internal_Show(Rect r, int showMode, Vector2 minSize,
                                    Vector2 maxSize);

  [FreeFunction(k_ScriptingPrefix + "Internal_BringLiveAfterCreation",
                HasExplicitThis = true)]
  private extern void Internal_BringLiveAfterCreation(bool displayImmediately,
                                                      bool setFocus,
                                                      bool showMaximized);

  [FreeFunction(k_ScriptingPrefix + "Internal_SetTitle",
                HasExplicitThis = true)]
  private extern void Internal_SetTitle(string title);

  [FreeFunction(k_ScriptingPrefix + "Internal_SetHasUnsavedChanges",
                HasExplicitThis = true)]
  private extern void Internal_SetHasUnsavedChanges(bool hasUnsavedChanges);

  [FreeFunction(k_ScriptingPrefix + "SetBackgroundColor",
                HasExplicitThis = true)]
  private extern void SetBackgroundColor(Color color);

  [FreeFunction(k_ScriptingPrefix + "Internal_GetTopleftScreenPosition",
                HasExplicitThis = true)]
  ...
  internal static extern Rect FitRectToScreen(Rect defaultRect,
