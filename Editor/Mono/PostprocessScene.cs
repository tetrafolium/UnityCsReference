...
internal class UnityBuildPostprocessor : IProcessSceneWithReport {
  ...
  public void OnProcessScene(UnityEngine.SceneManagement.Scene scene,
                             Build.Reporting.BuildReport report) {
    int staticBatching, dynamicBatching;
    PlayerSettings.GetBatchingForPlatform(
        EditorUserBuildSettings.activeBuildTarget, out staticBatching,
        out dynamicBatching);
    if (staticBatching != 0) {
      InternalStaticBatchingUtility.Combine(
          null, true, true, new EditorStaticBatcherGOSorter(scene));
    }
  }

  internal class EditorStaticBatcherGOSorter
      : InternalStaticBatchingUtility.StaticBatcherGOSorter {
    Scene scene;

    public EditorStaticBatcherGOSorter(Scene scene) { this.scene = scene; }

    private static long GetStableHash(UnityEngine.Object instance,
                                      string guid) {
