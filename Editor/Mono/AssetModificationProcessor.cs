...
internal class AssetModificationProcessorInternal {
  enum FileMode { Binary, Text }

  static bool CheckArgumentTypes(Type[] types, MethodInfo method) {
    ParameterInfo[] parameters = method.GetParameters();

    if (types.Length != parameters.Length) {
      Debug.LogWarning("Parameter count did not match. Expected: " +
                       types.Length + " Got: " + parameters.Length + " in " +
    ...
    foreach (Type type in types) {
      ...
      if (type != pInfo.ParameterType) {
        Debug.LogWarning("Parameter type mismatch at parameter " + i +
                         ". Expected: " + type +
                         " Got: " + pInfo.ParameterType + " in " +
                         method.DeclaringType + "." + method.Name);
        return false;
      }
      ++i;
  ...
  static bool CheckArgumentTypesAndReturnType(Type[] types, MethodInfo method,
                                              System.Type returnType) {
    if (returnType != method.ReturnType) {
      Debug.LogWarning("Return type mismatch. Expected: " + returnType +
                       " Got: " + method.ReturnType + " in " +
                       method.DeclaringType + "." + method.Name);
      return false;
    }
  ...
  static bool CheckArgumentsAndReturnType(object[] args, MethodInfo method,
                                          System.Type returnType) {
    Type[] types = new Type[args.Length];
...
#pragma warning disable 0618
  static System.Collections.Generic
      .IEnumerable<System.Type> assetModificationProcessors = null;
  static System.Collections.Generic
      .IEnumerable<System.Type> AssetModificationProcessors {
    get {
      if (assetModificationProcessors == null) {
        List<Type> processors = new List<Type>();
        processors.AddRange(
            TypeCache.GetTypesDerivedFrom<AssetModificationProcessor>());
        processors.AddRange(
            TypeCache
                .GetTypesDerivedFrom<global::AssetModificationProcessor>());
...
#pragma warning restore 0618
  ...
  static void OnWillCreateAsset(string path) {
    foreach (var assetModificationProcessorClass in
                 AssetModificationProcessors) {
      MethodInfo method = assetModificationProcessorClass.GetMethod(
          "OnWillCreateAsset",
  ...
  static void FileModeChanged(string[] assets,
                              UnityEditor.VersionControl.FileMode mode) {
    if (!Provider.enabled)
      return;

    // if we happen to be disconnected or work offline, there's not much we can
    // do; just ignore the file mode and hope that VCS client/project is setup
    // to handle appropriate file types correctly
    if (!Provider.isActive)
      return;

    // we'll want to re-serialize these assets in different (text vs binary)
    // mode; make sure they are editable first
  ...
  // ReSharper disable once UnusedMember.Local - invoked from native code
  static void OnWillSaveAssets(string[] assets,
                               out string[] assetsThatShouldBeSaved,
                               out string[] assetsThatShouldBeReverted,
                               bool explicitlySaveAsset) {
    assetsThatShouldBeReverted = new string[0];
    assetsThatShouldBeSaved = assets;

    bool showSaveDialog = assets.Length > 0 &&
                          EditorPrefs.GetBool("VerifySavingAssets", false) &&
                          InternalEditorUtility.isHumanControllingUs;

    // If we are only saving a single scene or prefab and the user explicitly
    // said we should, skip the dialog. We don't need to verify this twice.
    if (explicitlySaveAsset && assets.Length == 1 &&
        (assets [0]
             .EndsWith(".unity") ||
         assets [0]
             .EndsWith(".prefab")))
    ...
    AssetDatabase.IsOpenForEdit(assetsThatShouldBeSaved, assetsNotOpened,
    ...
    if (assets.Length != 0 &&
        !AssetDatabase.MakeEditable(assets, null, notEditableAssets)) {
      // only save assets that can be made editable (not locked by someone else,
      // etc.), unless we are in the behavior mode that just overwrites
      // everything anyway
      if (!EditorUserSettings.overwriteFailedCheckoutAssets) {
        ...
        assetsThatShouldBeSaved =
            assetsThatShouldBeSaved.Except(assetsThatShouldBeReverted)
                .ToArray();
      }
    }
  }

  static AssetMoveResult OnWillMoveAsset(string fromPath, string toPath,
                                         string[] newPaths,
                                         string[] NewMetaPaths) {
  ...
  static void OnWillDeleteAssets(string[] assetPaths,
                                 AssetDeleteResult[] outPathDeletionResults,
                                 RemoveAssetOptions options) {
    ...
    foreach (var assetModificationProcessorClass in
      ...
      for (int i = 0; i < assetPaths.Length; i++) {
        ...
        if (!CheckArgumentsAndReturnType(args, method,
                                         typeof(AssetDeleteResult)))
          continue;

        AssetDeleteResult callbackResult =
            (AssetDeleteResult) method.Invoke(null, args);
    ...
    if (nonDeletedPaths.Count > 0) {
      ...
      var nonDeletedPathDeletionResults =
          new AssetDeleteResult[nonDeletedPaths.Count];

      AssetModificationHook.OnWillDeleteAssets(
          nonDeletedPaths.ToArray(), nonDeletedPathDeletionResults, options);

      for (int i = 0; i < nonDeletedPathIndices.Count; i++) {
        outPathDeletionResults[nonDeletedPathIndices[i]] =
            nonDeletedPathDeletionResults[i];
      }
    }
  }
    ...
    if (isOpenForEditMethods == null) {
      ...
      foreach (var assetModificationProcessorClass in
        ...
        if (method != null) {
          ...
          if (!CheckArgumentTypesAndReturnType(types, method,
                                               bool_dummy.GetType()))
            continue;
  ...
  enum Editability { Never, Always, Maybe }

  static Editability GetPathEditability(string assetPath) {
    // read-only asset locations (e.g. shared packages) are considered not
    // editable
    bool rootFolder, readOnly;
    bool validPath = AssetDatabase.GetAssetFolderInfo(assetPath, out rootFolder,
                                                      out readOnly);
    if (validPath && readOnly)
      return Editability.Never;

    // other paths that are not know to asset database, and not versioned, are
    // considered always editable
  ...
  static bool IsOpenForEditViaScriptCallbacks(string assetPath,
                                              ref string message) {
  ...
  internal static bool IsOpenForEdit(string assetPath, out string message,
    ...
    if (string.IsNullOrEmpty(assetPath))
      return true; // treat empty/null paths as editable (might be under Library
                   // folders etc.)
    ...
    if (editability == Editability.Never)
      return false;
    if (!AssetModificationHook.IsOpenForEdit(assetPath, out message,
                                             statusOptions))
      return false;
  ...
  IsOpenForEdit(string[] assetOrMetaFilePaths, List<string> outNotEditablePaths,
    ...
    foreach (var path in assetOrMetaFilePaths) {
      if (string.IsNullOrEmpty(path))
        continue; // treat empty/null paths as editable (might be under Library
                  // folders etc.)
    ...
    // check with VCS
    AssetModificationHook.IsOpenForEdit(queryList, outNotEditablePaths,
                                        statusQueryOptions);
    ...
    if (scriptCallbacks != null && scriptCallbacks.Length > 0) {
      var stillEditable = assetOrMetaFilePaths.Except(outNotEditablePaths)
