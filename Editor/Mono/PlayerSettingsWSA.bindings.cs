...
public enum WSAImageScale {
	...
	[ Obsolete("Image scale 80 is not used in Universal Windows Platform",
	           false) ] _80 = 80,
	[ Obsolete("Image scale 140 is not used in Universal Windows Platform",
	           false) ] _140 = 140,
	[ Obsolete("Image scale 180 is not used in Universal Windows Platform",
	           false) ] _180 = 180,
	[ Obsolete("Image scale 240 is not used in Universal Windows Platform",
	           false) ] _240 = 240,
}
...
[StaticAccessor("PlayerSettingsBindings::WSA",
...
private static extern string GetWSAImage(WSAImageType type,
                                         WSAImageScale scale);

private static extern void SetWSAImage(string image, WSAImageType type,
                                       WSAImageScale scale);
...
[Obsolete("PlayerSettings.WSA.commandLineArgsFile is deprecated", error
          : true)]
public static string commandLineArgsFile {
...
private static extern string internalCertificateSubject {
	get;
}
...
private static extern string internalCertificateIssuer {
	get;
}
...
[Obsolete(
	 "PlayerSettings.WSA.enableIndependentInputSource is deprecated. Use PlayerSettings.WSA.inputSource.",
	 false)]
public static bool enableIndependentInputSource {
	get { return inputSource == WSAInputSource.IndependentInputSource; }
	set {
		inputSource =
			value ? WSAInputSource.IndependentInputSource
	    : WSAInputSource.CoreWindow;
	}
}

[StaticAccessor("GetPlayerSettings()", StaticAccessorType.Dot)]
[NativeProperty("metroInputSource", TargetType.Field)]
public static extern WSAInputSource inputSource {
	get;
	set;
}
...
private static extern void InternalSetTargetDeviceFamily(string name,
                                                         string value);

private static extern string InternalGetTargetDeviceFamily(string name);

// Workaround for Case 756100 - properties (and probably functions) in
// nested+nested class are not correctly bound, causing
// MissingMethodException when calling them from C#
...
internal static WSAFileTypeAssociations internalFileTypeAssociations {
	get {
		return new WSAFileTypeAssociations {
			       name = metroFTAName, supportedFileTypes = metroFTAFileTypes
		};
	}
...
private static extern string metroFTAName {
	get;
	set;
}
...
public static string protocolName {
	get { return PlayerSettings.WSA.internalProtocolName; }
	set { PlayerSettings.WSA.internalProtocolName = value; }
}

public static WSAFileTypeAssociations fileTypeAssociations {
	get { return PlayerSettings.WSA.internalFileTypeAssociations; }
	set { PlayerSettings.WSA.internalFileTypeAssociations = value; }
}
}
}
}
