...
internal class SceneHierarchyWindow : SearchableEditorWindow,
                                      IHasCustomMenu,
                                      IPropertySourceOpener {
  public static SceneHierarchyWindow lastInteractedHierarchyWindow {
    get { return s_LastInteractedHierarchy; }
  }
  static SceneHierarchyWindow s_LastInteractedHierarchy;
  public static List<SceneHierarchyWindow> GetAllSceneHierarchyWindows() {
    return s_SceneHierarchyWindows;
  }
  static List<SceneHierarchyWindow> s_SceneHierarchyWindows =
      new List<SceneHierarchyWindow>();

  internal static SavedBool s_EnterRenameModeForNewGO =
      new SavedBool("SceneHierarchyWindow.RenameNewObjects", true);

  static class Styles { public const float kStageHeaderHeight = 25f; }

  [SerializeField] SceneHierarchy m_SceneHierarchy;
  SceneHierarchyStageHandling m_StageHandling;

  [SerializeField] string m_WindowGUID;
  public string windowGUID {
    get { return m_WindowGUID; }
  }

  public SceneHierarchy sceneHierarchy {
    get { return m_SceneHierarchy; }
  }

  bool showingStageHeader {
    get { return !(StageNavigationManager.instance.currentStage is MainStage); }
  }
  ...
  public override void OnEnable() {
    ...
    m_SceneHierarchy.position =
        treeViewRect; // ensure SceneHierarchy has a valid rect before
                      // initializing in OnEnable
  ...
  void OnBecameVisible() { m_SceneHierarchy.OnBecameVisible(); }

  void OnLostFocus() { m_SceneHierarchy.OnLostFocus(); }

  void OnSelectionChange() { m_SceneHierarchy.OnSelectionChange(); }

  void OnHierarchyChange() { m_SceneHierarchy.OnHierarchyChange(); }

  void OnProjectWasLoaded() { m_SceneHierarchy.OnProjectWasLoaded(); }

  void OnQuit() { m_SceneHierarchy.OnQuit(); }

  void OnGUI() {
    Rect sceneHierarchyRect = new Rect(0, 0, position.width, position.height);
    Event evt = Event.current;
    if (evt.type == EventType.MouseDown &&
        sceneHierarchyRect.Contains(evt.mousePosition)) {
      s_LastInteractedHierarchy = this;
    }
  ...
  public Object hoveredObject => sceneHierarchy.treeView.hoveredItem !=
                                 null ? Object.FindObjectFromInstanceID(
                                     sceneHierarchy.treeView.hoveredItem.id)
      : null;

  public void ReloadData() { m_SceneHierarchy.ReloadData(); }
  ...
  Rect stageHeaderRect {
    get {
      return new Rect(0, EditorGUI.kWindowToolbarHeight, position.width,
                      Styles.kStageHeaderHeight);
    }
  }

  Rect treeViewRect {
    get {
      float startY = EditorGUI.kWindowToolbarHeight +
                     (showingStageHeader ? Styles.kStageHeaderHeight : 0);
  ...
  void DoToolbarLayout() {
    ...
    // When searchfield has focus give keyboard focus to the tree view on
    // Down/UpArrow
    if (hasSearchFilterFocus && evt.type == EventType.KeyDown &&
  ...
  SetSearchFilter(string searchFilter,
                  SearchableEditorWindow.SearchMode searchMode, bool setAll,
                  bool delayed = false) {
    base.SetSearchFilter(searchFilter, searchMode, setAll, delayed);
    m_SceneHierarchy.SetSearchFilter(
        searchFilter,
  ...
  internal int[] GetExpandedIDs() { return m_SceneHierarchy.GetExpandedIDs(); }
  ...
  internal void SelectPrevious() { m_SceneHierarchy.SelectPrevious(); }

  internal void SelectNext() { m_SceneHierarchy.SelectNext(); }
  ...
  void OnSceneSelectionStateChanged(Scene selectedScene) {
    m_SceneHierarchy.customScenes =
        selectedScene.IsValid() ? new[]{selectedScene} : null;
    Repaint();
  }
...
internal class TransformSorting : HierarchySorting {
  readonly ScalableGUIContent m_Content =
      new ScalableGUIContent(null, "Transform Child Order", "DefaultSorting");
...
internal class AlphabeticalSorting : HierarchySorting {
  readonly ScalableGUIContent m_Content =
      new ScalableGUIContent(null, "Alphabetical Order", "AlphabeticalSorting");
  public override GUIContent content {
    get { return m_Content; }
  }
}
