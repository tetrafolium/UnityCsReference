...
public sealed class MenuItem : Attribute {
  ...
  public MenuItem(string itemName, bool isValidateFunction)
      : this(itemName, isValidateFunction,
             itemName.StartsWith("GameObject/Create Other") ? 10 : 1000) {}
  // The special treatment of "GameObject/Other" is to ensure that legacy
  // scripts that don't set a priority don't create a "Create Other" menu at the
  // very bottom of the GameObject menu (thus preventing the items from being
  // propagated to the scene hierarchy dropdown and context menu).
  ...
  private static string NormalizeMenuItemName(string rawName) {
    return string.Join(
        kMenuItemSeparators[0],
        rawName.Split(kMenuItemSeparators, StringSplitOptions.None)
