...
internal class ObjectListAreaState {
  ...
  public bool
      m_HadKeyboardFocusLastEvent; // Needs to survive domain reloads to prevent
                                   // setting selection on got keyboard focus
...
internal partial class ObjectListArea {
  class Styles {
    ...
    public GUIStyle groupHeaderMiddle =
        GetStyle("ProjectBrowserHeaderBgMiddle");
    ...
    public GUIStyle textureIconDropShadow =
        GetStyle("ProjectBrowserTextureIconDropShadow");
    ...
    public GUIStyle subAssetBgOpenEnded =
        GetStyle("ProjectBrowserSubAssetBgOpenEnded");
    public GUIStyle subAssetBgCloseEnded =
        GetStyle("ProjectBrowserSubAssetBgCloseEnded");
    public GUIStyle subAssetBgMiddle =
        GetStyle("ProjectBrowserSubAssetBgMiddle");
    public GUIStyle subAssetExpandButton =
        GetStyle("ProjectBrowserSubAssetExpandBtn");
    public GUIStyle subAssetExpandButtonMedium =
        GetStyle("ProjectBrowserSubAssetExpandBtnMedium");
    public GUIStyle subAssetExpandButtonSmall =
        GetStyle("ProjectBrowserSubAssetExpandBtnSmall");

    public GUIContent m_AssetStoreNotAvailableText =
        EditorGUIUtility.TrTextContent("The Asset Store is not available");
  ...
  Dictionary<AssetReference, string> m_AssetReferenceToCroppedNameMap =
      new Dictionary<AssetReference, string>(
          new AssetReference.GuidThenInstanceIDEqualityComparer());
  ...
  readonly SearchService.SearchSessionHandler m_SearchSessionHandler =
      new SearchService.SearchSessionHandler(
  ...
  public ObjectListArea(ObjectListAreaState state, EditorWindow owner,
                        bool showNoneItem) {
    m_State = state;
    m_Owner = owner;

    AssetStorePreviewManager.MaxCachedImages =
        72; // Magic number. Will be reset on first layout.
  ...
  public void Init(Rect rect, HierarchyType hierarchyType,
                   SearchFilter searchFilter, bool checkThumbnails) {
    // Keep for debugging
    ...
    bool isFolderBrowsing =
        (searchFilter.GetState() == SearchFilter.State.FolderBrowsing);
    if (isFolderBrowsing) {
      ...
      m_LastAssetStoreQuerySearchFilter =
          searchFilter.nameFilter == null ? "" : searchFilter.nameFilter;
      bool disableClassConstraint =
          searchFilter.classNames == null ||
          System.Array.IndexOf(searchFilter.classNames, "Object") >= 0;
      m_LastAssetStoreQueryClassName =
          disableClassConstraint ? new string[0] : searchFilter.classNames;
      m_LastAssetStoreQueryLabels =
          searchFilter.assetLabels == null ? new string[0]
          : searchFilter.assetLabels;
    }

    m_LastAssetStoreQueryChangeTime = EditorApplication.timeSinceStartup;
    m_RequeryAssetStore = true;

    m_ShowLocalAssetsOnly =
        isFolderBrowsing ||
        (searchFilter.GetState() != SearchFilter.State.SearchingInAssetStore);
    m_AssetStoreError = "";
  ...
  internal void InitForSearch(Rect rect, HierarchyType hierarchyType,
                              SearchFilter searchFilter, bool checkThumbnails,
    ...
    // Override Asset search here. For GameObjects, it is done in
    // CachedFilteredHierarchy.cs
    ...
    if (searchFilter.IsSearching()) {
      m_SearchSessionHandler.BeginSession(() => {
        return new SearchService.ProjectSearchContext{
            ...
            requiredTypes = searchFilter.classNames.Select(
                name => TypeCache.GetTypesDerivedFrom<Object>().FirstOrDefault(
                    t => name == t.FullName || name == t.Name))};
      });
      m_SearchSessionHandler.BeginSearch(searchQuery);
      var searchContext =
          (SearchService.ProjectSearchContext) m_SearchSessionHandler.context;
      // Asynchronous searches return new results. Accumulate those results when
      // using ShowObjectsInList.
      var results = SearchService.Project.Search(
          searchQuery, searchContext, newResults => {
            if (newResults == null || !searchFilter.IsSearching())
              return;
            allResults.AddRange(newResults);
            InitListAreaWithItems(rect, hierarchyType, searchFilter,
                                  checkThumbnails, allResults,
                                  assetToInstanceId);
          });
      InitListAreaWithItems(rect, hierarchyType, searchFilter, checkThumbnails,
                            results, assetToInstanceId);
      if (results != null)
  ...
  void InitListAreaWithItems(Rect rect, HierarchyType hierarchyType,
                             SearchFilter searchFilter, bool checkThumbnails,
                             IEnumerable<string> items,
                             Func<string, int> assetToInstanceId) {
    // When items is null, we fallback to default implementation. Current
    // default search engine returns null.
    Init(rect, hierarchyType, items == null ? searchFilter
         : new SearchFilter(), checkThumbnails);
      ...
      foreach (var path in items) {
        ...
        if (reformattedPath.StartsWith("Assets"))
          ...
          var secondSlashIndex =
              reformattedPath.IndexOf('/', "Packages".Length + 1);
          rootPath = secondSlashIndex == -1
                         ? reformattedPath
                         : reformattedPath.Substring(0, secondSlashIndex);
        } else
          continue;
  ...
  bool HasFocus() {
    if (!allowFocusRendering)
      return true;
    return m_KeyboardControlID == GUIUtility.keyboardControl &&
           m_Owner.m_Parent.hasFocus;
  }
  ...
  void QueryAssetStore() {
    ...
    // We disable Asset Store searching here. Note that we still query to get
    // hits when searching local assets
    if (m_ShowLocalAssetsOnly &&
        !ShowAssetStoreHitsWhileSearchingLocalAssets()) {
      return;
    }

    bool hasValidFilter = m_LastAssetStoreQuerySearchFilter != "" ||
                          m_LastAssetStoreQueryClassName.Length != 0 ||
                          m_LastAssetStoreQueryLabels.Length != 0;
    ...
    // In order not to query prematurely we delay the query a bit to let the
    // user type more characters in the search input box if needed.
    if ((m_LastAssetStoreQueryChangeTime + kQueryDelay) >
    ...
    // Remember the filter to check for changes after we have received the reply
    // from asset store
    string queryFilter = m_LastAssetStoreQuerySearchFilter +
                         m_LastAssetStoreQueryClassName +
                         m_LastAssetStoreQueryLabels;

    AssetStoreSearchResults.Callback dg =
        delegate(AssetStoreSearchResults results) {
      m_QueryInProgress = false;

      // If filter changed while fetching the result then requery using the new
      // filter.
      if (queryFilter != m_LastAssetStoreQuerySearchFilter +
                             m_LastAssetStoreQueryClassName +
                             m_LastAssetStoreQueryLabels)
        m_RequeryAssetStore = true;
      ...
      if (!string.IsNullOrEmpty(results.error)) {
        if (s_Debug)
          Debug.LogError("Error performing Asset Store search: " +
                         results.error);
        else
          System.Console.Write("Error performing Asset Store search: " +
                               results.error);
    ...
    List<AssetStoreClient.SearchCount> groupsQuery =
        new List<AssetStoreClient.SearchCount>();
    ...
    AssetStoreClient.SearchAssets(m_LastAssetStoreQuerySearchFilter,
                                  m_LastAssetStoreQueryClassName,
                                  m_LastAssetStoreQueryLabels, groupsQuery, dg);
  }
  ...
  void RequeryAssetStore() { m_RequeryAssetStore = true; }
  ...
  public string GetAssetStoreButtonText() {
    ...
    if (ShowAssetStoreHitsWhileSearchingLocalAssets()) {
      for (int i = 0; i < m_StoreAssets.Count; ++i) {
        ...
        buttonText +=
            (group.ItemsAvailable > 999 ? "999+"
                                        : group.ItemsAvailable.ToString());
      }
    }
    return buttonText;
  }
  ...
  bool ShowAssetStoreHitsWhileSearchingLocalAssets() {
    return EditorPrefs.GetBool("ShowAssetStoreSearchHits",
                               true); // See PreferencesWindow
  }
  ...
  internal float GetVisibleWidth() { return m_VisibleRect.width; }
  ...
  public void OnGUI(Rect position, int keyboardControlID) {
    ...
    // For keyboard focus handling (for Tab support and rendering of keyboard
    // focus state)
    ...
    if (hasKeyboardFocus != m_State.m_HadKeyboardFocusLastEvent) {
      ...
      if (hasKeyboardFocus) {
        if (evt.type == EventType.MouseDown)
          m_AllowRenameOnMouseUp =
              false; // If we got focus by mouse down then we do not want to
                     // begin renaming if clicking on an already selected item
    ...
    // For key navigation: Auto set selection to first element if selection is
    // not shown currently when tabbing
    if (evt.keyCode == KeyCode.Tab && evt.type == EventType.KeyDown &&
        !hasKeyboardFocus && !IsShowingAny(GetSelection())) {
    ...
    DoOffsetSelection();
    HandleUnusedEvents();
    // GUI.Label(position, new
    // GUIContent(AssetStorePreviewManager.StatsString()));
  }
  ...
  void FrameLastClickedItemIfWanted() {
    if (m_FrameLastClickedItem && Event.current.type == EventType.Repaint) {
      ...
      double timeSinceLastDraw = EditorApplication.timeSinceStartup -
                                 m_LocalAssets.m_LastClickedDrawTime;
  ...
  void HandleUnusedEvents() {
    if (allowDeselection && Event.current.type == EventType.MouseDown &&
  ...
  public bool CanShowThumbnails() {
    //
    //      return m_AllowThumbnails || m_StoreAssets.Find( g =>
    //      g.ItemsAvailable > 0) != null;
    // #else
  ...
  bool ObjectsHaveThumbnails(HierarchyType type, SearchFilter searchFilter) {
    // Check if we have any built-ins, if so we have thumbs since all builtins
    // have thumbs
    ...
    IHierarchyProperty assetProperty =
        FilteredHierarchyProperty.CreateHierarchyPropertyForFilter(hierarchy);
    int[] empty = new int[0];
    if (assetProperty.CountRemaining(empty) == 0)
      return true; // allow thumbnails: we prefer asset-store results as icons
                   // over list
  ...
  public void OnEvent() { GetRenameOverlay().OnEvent(); }

  CreateAssetUtility GetCreateAssetUtility() {
    return m_State.m_CreateAssetUtility;
  }

  internal RenameOverlay GetRenameOverlay() { return m_State.m_RenameOverlay; }

  internal void BeginNamingNewAsset(string newAssetName, int instanceID,
                                    bool isCreatingNewFolder) {
    m_State.m_NewAssetIndexInList = m_LocalAssets.IndexOfNewText(
        newAssetName, isCreatingNewFolder, foldersFirst);
  ...
  void RenameEnded() {
    // We are done renaming (user accepted/rejected, we lost focus etc, other
    // grabbed renameOverlay etc.)
    string name = string.IsNullOrEmpty(GetRenameOverlay().name)
                      ? GetRenameOverlay().originalName
                      : GetRenameOverlay().name;
    int instanceID =
        GetRenameOverlay().userData; // we passed in an instanceID as userData
    ...
    if (GetRenameOverlay().userAcceptedRename) {
      Frame(instanceID, true,
            false); // frames existing assets (new ones could have instanceID 0)
    }
  ...
  internal void HandleRenameOverlay() {
    if (GetRenameOverlay().IsRenaming()) {
      ...
      if (!GetRenameOverlay().OnGUI(renameStyle)) {
        RenameEnded();
        GUIUtility.ExitGUI(); // We exit gui because we are iterating items and
                              // when we end naming a new asset this will change
                              // the order of items we are iterating.
      }
    }
  }
  ...
  public bool IsLastClickedItemVisible() { return GetSelectedAssetIdx() >= 0; }
  ...
  public void SelectAll() {
    ...
    if (instanceIDs.Count != 0) {
      var selectedInstanceIDs = InternalEditorUtility.TryGetInstanceIds(
          instanceIDs, guids, 0, instanceIDs.Count - 1);
      if (selectedInstanceIDs == null) {
        Debug.Log(
            "Cannot select all because some assets being selected are in progress of being imported");
        return;
      }
    }
  ...
  public void InitSelection(int[] selectedInstanceIDs) {
    ...
    // Keep for debugging
    // Debug.Log ("InitSelection (ObjectListArea): new selection " +
    // DebugUtils.ListToString(m_State.m_SelectedInstanceIDs));

    if (m_State.m_SelectedInstanceIDs.Count > 0) {
      // Only init last clicked instance if it is NOT part of our selection (we
      // need it for navigation)
      if (!m_State.m_SelectedInstanceIDs.Contains(
              m_State.m_LastClickedInstanceID))
        m_State.m_LastClickedInstanceID =
            m_State
                .m_SelectedInstanceIDs[m_State.m_SelectedInstanceIDs.Count - 1];
    } else {
  ...
  void SetSelection(AssetStoreAsset assetStoreResult, bool doubleClicked) {
    ...
    AssetStoreAssetSelection.Clear();
    AssetStorePreviewManager.CachedAssetStoreImage item =
        AssetStorePreviewManager.TextureFromUrl(
            assetStoreResult.staticPreviewURL, assetStoreResult.name, gridSize,
            s_Styles.resultsGridLabel, s_Styles.resultsGrid, true);
  ...
  bool IsPreviewIconExpansionModifierPressed() { return Event.current.alt; }

  bool AllowLeftRightArrowNavigation() {
    bool gridMode =
        !m_LocalAssets.ListMode && !IsPreviewIconExpansionModifierPressed();
    bool validItemCount =
        !m_ShowLocalAssetsOnly || (m_LocalAssets.ItemCount > 1);
    ...
    if (Event.current.type == EventType.KeyDown) {
      ...
      if (IsLastClickedItemVisible()) {
        ...
        case KeyCode.UpArrow:
          offset = -m_LocalAssets.m_Grid.columns; // we assume that all groups
      ...
      if (offset != 0) {
        // If nothing is selected then select first object and ignore the offset
        // (when showing none GetSelectedAssetIdx return -1)
  ...
  void DoOffsetSelectionSpecialKeys(int idx, int maxIndex) {
    float itemHeight =
        m_LocalAssets.m_Grid.itemSize.y + m_LocalAssets.m_Grid.verticalSpacing;
    ...
    case kPageUp:
      ...
      if (Application.platform == RuntimePlatform.OSXEditor) {
        ...
        m_SelectionOffset =
            -Mathf.RoundToInt(m_TotalRect.height / itemHeight) * columns;
        // we want it to go to the very top row, but stay on same column
        m_SelectionOffset =
            Mathf.Max(-Mathf.FloorToInt(idx / (float) columns) * columns,
                      m_SelectionOffset);
      }
      break;
    case kPageDown:
      ...
      if (Application.platform == RuntimePlatform.OSXEditor) {
        ...
        m_SelectionOffset = Mathf.Min(
            Mathf.FloorToInt(remainingItems / (float) columns) * columns,
            m_SelectionOffset);
      }
      break;
    case kHome:
  ...
  void DoOffsetSelection() {
    ...
    selectedAssetIdx =
        selectedAssetIdx < 0 ? 0 : selectedAssetIdx; // default to first item
    ...
    // We ignore the offset if newIdx is less than 0 or clamp to item list
    // length. This ensures that we stay on the same column at top or jump to
    // the last item when navigating with keys.
    if (newGridIdx < 0)
      newGridIdx = selectedAssetIdx;
    else
      newGridIdx = Mathf.Min(newGridIdx, maxGridIndex);

    // If newIdx is on one of the two bottom rows we scroll to the bottom row
    // because we might jump to another row when navigating (because the last
    // row is half empty). This is a usability decision and can be removed.
  ...
  public void SelectFirst() {
    int startIndex = 0;
    if (m_ShowLocalAssetsOnly && m_LocalAssets.ShowNone &&
        m_LocalAssets.ItemCount > 1)
      startIndex = 1;
  ...
  void SetSelectedAssetByIdx(int selectedIdx) {
    ...
    if (m_LocalAssets.AssetReferenceAtIndex(selectedIdx, out assetReference)) {
      ...
      if (IsLocalAssetsCurrentlySelected())
        newSelection =
            m_LocalAssets.GetNewSelection(ref assetReference, false, true)
                .ToArray(); // Handle multi selection
      else
        newSelection =
            m_LocalAssets.GetNewSelection(ref assetReference, false, false)
                .ToArray(); // If current selection is asset store asset do not
  ...
  void Reveal(int instanceID) {
    ...
    int mainAssetInstanceID = AssetDatabase.GetMainAssetInstanceID(
  ...
  bool IsLocalAssetsCurrentlySelected() {
    int currentSelectedInstanceID =
        m_State.m_SelectedInstanceIDs.FirstOrDefault();
  ...
  void HandleListArea() {
    ...
    // Requery the Asset Store to get more assets if content area has changed
    // size so that more assets fits in there.
    ...
    if (!IsObjectSelector() && !m_QueryInProgress) {
      ...
      if (needItems || m_RequeryAssetStore) {
        QueryAssetStore(); // need more data to fill required rows with asset
                           // store assets
      }
    }
    ...
    foreach (Group g in m_Groups) {
      ...
      // ShowNone is only used in object select window and we don't want asset
      // store asset there
    ...
    m_State.m_ScrollPosition =
        GUI.BeginScrollView(scrollRect, m_State.m_ScrollPosition, contentRect);
    {
      Vector2 scrollPos = m_State.m_ScrollPosition; // Copy scroll pos since the
                                                    // draw calls may change it
    ...
    // draw in order to let the cache have the correct lastUsed timestamps on
    // the cached images. This is important when the cache is decreased in order
    // not to dispose the wrong cache entries
    if (m_ResizePreviewCacheTo > 0 &&
    ...
    if (!m_ShowLocalAssetsOnly && !string.IsNullOrEmpty(m_AssetStoreError)) {
      Vector2 size =
          EditorStyles.label.CalcSize(s_Styles.m_AssetStoreNotAvailableText);
      Rect textRect =
          new Rect(m_TotalRect.x + 2f +
                       Mathf.Max(0, (m_TotalRect.width - size.x) * 0.5f),
                   m_TotalRect.y + 10f, size.x, 20f);
      using(new EditorGUI.DisabledScope(true)) {
        GUI.Label(textRect, s_Styles.m_AssetStoreNotAvailableText,
                  EditorStyles.label);
      }
    }
  }
  ...
  bool IsListMode() {
    if (allowMultiSelect)
      return (gridSize ==
              kListLineHeight); // ProjectBrowser (should auto change layout on
                                // content but entirely user controlled)
    else
      return (gridSize == kListLineHeight) ||
             !CanShowThumbnails(); // ObjectSelector
  }
  ...
  void Reflow() {
    ...
    if (IsListMode()) {
      foreach (Group g in m_Groups) {
        ...
        // ShowNone is only used in object select window and we don't want aiy
        // sset store asset there
        if (m_LocalAssets.ShowNone)
          break;
      }

      m_ResizePreviewCacheTo =
          Mathf.CeilToInt((float) m_TotalRect.height / kListLineHeight) + 10;
    }
    // we're in thumbnail mode
      ...
      if (scrollbarVisible) {
        ...
        foreach (Group g in m_Groups) {
          ...
          g.m_Grid.InitNumRowsAndColumns(g.ItemCount,
                                         g.m_Grid.CalcRows(g.ItemsWantedShown));
          g.UpdateHeight();

          // ShowNone is only used in object select window and we don't want
          // asset store asset there
          if (m_LocalAssets.ShowNone)
            break;
        }
      }
      ...
      AssetPreview.SetPreviewTextureCacheSize(maxVisibleItems * 2 + 30,
                                              GetAssetPreviewManagerID());
    }
  }
  ...
  public void OnInspectorUpdate() {
    if (EditorApplication.timeSinceStartup > m_NextDirtyCheck &&
        m_LocalAssets.IsAnyLastRenderedAssetsDirty()) {
      // If an asset is dirty we ensure to get a updated preview by clearing
      // cache of temporary previews
  ...
  void ClearCroppedLabelCache() { m_AssetReferenceToCroppedNameMap.Clear(); }

  protected string GetCroppedLabelText(AssetReference assetReference,
                                       string fullText, float cropWidth) {
    // Clear when width changes
    if (m_WidthUsedForCroppingName != (int) cropWidth)
      ClearCroppedLabelCache();

    string croppedText;
    if (!m_AssetReferenceToCroppedNameMap.TryGetValue(assetReference,
                                                      out croppedText)) {
      // Ensure to clean up once in a while
      if (m_AssetReferenceToCroppedNameMap.Count >
          GetMaxNumVisibleItems() * 2 + 30)
        ClearCroppedLabelCache();

      // Check if we need to crop
      int characterCountVisible =
          s_Styles.resultsGridLabel.GetNumCharactersThatFitWithinWidth(
              fullText, cropWidth);
      ...
      if (characterCountVisible > 1 && characterCountVisible != fullText.Length)
        croppedText = fullText.Substring(0, characterCountVisible - 1) +
                      ("\u2026"); // 'horizontal ellipsis' (U+2026) is: ...
      else
  ...
  internal int GetAssetPreviewManagerID() { return m_Owner.GetInstanceID(); }
  ...
  public void BeginPing(int instanceID) {
    ...
    if (index != -1) {
      ...
      HierarchyProperty hierarchyProperty =
          new HierarchyProperty(HierarchyType.Assets);
      ...
      float vcPadding =
          s_VCEnabled ? k_ListModeVersionControlOverlayPadding : 0f;
      var assetReference = new AssetReference(){instanceID = instanceID};
      GUIContent cont =
          new GUIContent(m_LocalAssets.ListMode ? name
                         : GetCroppedLabelText(assetReference, name,
                                               m_WidthUsedForCroppingName));
      ...
      if (m_LocalAssets.ListMode) {
        ...
        m_LeftPaddingForPinging =
            hierarchyProperty.isMainRepresentation ? LocalGroup
                .k_ListModeLeftPadding
            : LocalGroup.k_ListModeLeftPaddingForSubAssets;
        FilteredHierarchy.FilterResult res =
            m_LocalAssets.LookupByInstanceID(instanceID);
        ...
        m_Ping.m_ContentRect.height = pingLabelSize.y;
        m_Ping.m_ContentDraw = (Rect r) => {
          // We need to temporary adjust style to render into content rect (org
          // anchor is middle-centered)
  ...
  public void EndPing() { m_Ping.m_TimeStart = -1f; }

  void HandlePing() {
    // We need to update m_Ping.m_ContentTopLeft in icon mode. The position
    // might change if user resizes the window while pinging
  ...
  Vector2 CalculatePingPosition() {
    ...
    if (m_LocalAssets.ListMode) {
      ...
      return new Vector2(
          gridRect.center.x - width / 2f + m_Ping.m_PingStyle.padding.left,
          gridRect.yMax - s_Styles.resultsGridLabel.fixedHeight + 3);
    }
  }
}
} // namespace UnityEditor
