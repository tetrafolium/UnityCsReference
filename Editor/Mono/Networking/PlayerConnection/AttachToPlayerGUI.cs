...
GUIContent notificationMessage {
	get;
}
bool deepProfilingSupported {
	get;
}
...
public static IConnectionState
GetConnectionState(EditorWindow parentWindow,
                   Action<string> connectedCallback = null) {
	return new GeneralConnectionState(parentWindow, connectedCallback);
}

internal static IConnectionState GetConnectionState(
	EditorWindow parentWindow,
	Action<EditorConnectionTarget> editorModeTargetSwitchedCallback,
	Func<EditorConnectionTarget, bool> editorModeTargetConnectionStatus,
	Action<string> connectedCallback = null) {
	return new GeneralConnectionState(parentWindow, connectedCallback,
	                                  editorModeTargetSwitchedCallback,
	                                  editorModeTargetConnectionStatus);
}
}

static class Styles {
public static readonly GUIStyle defaultDropdown = "MiniPullDown";
public static readonly GUIContent dropdownButton =
	UnityEditor.EditorGUIUtility.TrTextContent(
		"", "Target Selection: Choose the target to connect to.");
}

public static class PlayerConnectionGUI {
public static void
ConnectionTargetSelectionDropdown(Rect rect, IConnectionState state,
                                  GUIStyle style = null) {
	...
	if (internalState?.parentWindow) {
		if (internalState.notificationMessage != null)
			internalState.parentWindow.ShowNotification(
				internalState.notificationMessage);
		else
	...
	if (!UnityEditor.EditorGUI.DropdownButton(rect, Styles.dropdownButton,
	                                          FocusType.Passive, style))
		return;
...
public static void
ConnectionTargetSelectionDropdown(IConnectionState state,
                                  GUIStyle style = null) {
	if (style == null)
		style = Styles.defaultDropdown;
	Styles.dropdownButton.text = state.connectionName;
	...
	PlayerConnectionGUI.ConnectionTargetSelectionDropdown(connectRect, state,
	                                                      style);
}
}
...
public static readonly GUIContent Playmode =
	UnityEditor.EditorGUIUtility.TrTextContent("Playmode");
public static readonly GUIContent Editmode =
	UnityEditor.EditorGUIUtility.TrTextContent("Editor");
public static readonly GUIContent EnterIPText =
	UnityEditor.EditorGUIUtility.TrTextContent("<Enter IP>");
public static readonly GUIContent AutoconnectedPlayer =
	UnityEditor.EditorGUIUtility.TrTextContent("(Autoconnected Player)");
public static readonly GUIContent ConnectingToPlayerMessage =
	UnityEditor.EditorGUIUtility.TrTextContent(
		"Connecting to player... (this can take a while)");

public static readonly string LocalHostProhibited =
	L10n.Tr(" (Localhost prohibited)");
public static readonly string VersionMismatch =
	L10n.Tr(" (Version mismatch)");
}
static GUIContent s_NotificationMessage;

public GUIContent notificationMessage => s_NotificationMessage;

// keep this constant in sync with PLAYER_DIRECT_IP_CONNECT_GUID in
// GeneralConnection.h
const int PLAYER_DIRECT_IP_CONNECT_GUID = 0xFEED;
// keep this constant in sync with PLAYER_DIRECT_URL_CONNECT_GUID in
// GeneralConnection.h
...
ProfilerDriver.IsConnectionEditor() ? ConnectionTarget.Editor
                                            : ConnectionTarget.Player;

public string connectionName {
	get {
		string name = ProfilerDriver.GetConnectionIdentifier(
			ProfilerDriver.connectedProfiler);
		if (m_EditorModeTargetState.HasValue &&
		    name.Contains(k_EditorConnectionName)) {
			if (m_EditorModeTargetConnectionStatus(
...
public bool deepProfilingSupported =>
ProfilerDriver.IsDeepProfilingSupported(
	ProfilerDriver.connectedProfiler);
...
static List<WeakReference> s_AllGeneralAttachToPlayerStates =
	new List<WeakReference>();

public GeneralConnectionState(
	EditorWindow parentWindow, Action<string> connectedCallback = null,
	Action<EditorConnectionTarget> editorModeTargetSwitchedCallback = null,
	Func<EditorConnectionTarget, bool> editorModeTargetConnectionStatus =
	null) {
	...
	if (editorModeTargetSwitchedCallback != null) {
		Debug.Assert(
			editorModeTargetConnectionStatus != null,
			$"{nameof(editorModeTargetConnectionStatus)} can't be null when a {nameof(editorModeTargetSwitchedCallback)} is provided.");
...
private static void SuccessfullyConnectedToPlayer(
	string player, EditorConnectionTarget? editorConnectionTarget = null) {
	for (int i = s_AllGeneralAttachToPlayerStates.Count - 1; i >= 0; i--) {
		if (s_AllGeneralAttachToPlayerStates[i] == null ||
		    !s_AllGeneralAttachToPlayerStates[i].IsAlive) {
			s_AllGeneralAttachToPlayerStates.RemoveAt(i);
		}
		var generalConnectionState = (s_AllGeneralAttachToPlayerStates[i]
		                              .Target as GeneralConnectionState);
		generalConnectionState.connected?.Invoke(player);
		if (editorConnectionTarget.HasValue) {
			generalConnectionState.switchedEditorModeTarget?.Invoke(
				editorConnectionTarget.Value);
		} else {
			if (player.Contains(k_EditorConnectionName)) {
				// if e.g. the console or the memory profiler connects to the
				// Editor, the profiler should switch to PlayMode profiling, not to
				// Editmode profiling especially since falling back onto the Editor
				// is the default.
				generalConnectionState.switchedEditorModeTarget?.Invoke(
					EditorConnectionTarget.MainEditorProcessPlaymode);
			} else {
				generalConnectionState.switchedEditorModeTarget?.Invoke(
					EditorConnectionTarget.None);
			}
		}
	}
}
...
public virtual void AddItemsToMenu(GenericMenu menu, Rect position) {
	...
	// Case 810030: Check if player is connected using AutoConnect Profiler
	// feature via 'connect <ip> string in PlayerConnectionConfigFile In that
	// case ProfilerDriver.GetAvailableProfilers() won't return the connected
	// player But we still want to show that it's connected, because the data
	// is incoming
...
internal static void DirectIPConnect(string ip) {
	// Profiler.DirectIPConnect is a blocking call, so a notification message
	// is used to show the progress
...
internal static void DirectURLConnect(string url) {
	// Profiler.DirectURLConnect is a blocking call, so a notification message
	// is used to show the progress
	s_NotificationMessage = Content.ConnectingToPlayerMessage;
...
void AddLastConnectedIP(GenericMenu menuOptions,
                        ref bool hasOpenConnection) {
	string lastIP = AttachToPlayerPlayerIPWindow.GetLastIPString();
	if (string.IsNullOrEmpty(lastIP))
		return;

	bool isConnected =
		ProfilerDriver.connectedProfiler == PLAYER_DIRECT_IP_CONNECT_GUID;
	hasOpenConnection |= isConnected;
	menuOptions.AddItem(new GUIContent(lastIP), isConnected,
	                    () => DirectIPConnect(lastIP));
}

void AddAvailablePlayerConnections(GenericMenu menuOptions,
                                   ref bool hasOpenConnection) {
	...
	for (int index = 0; index < connectionGuids.Length; index++) {
		...
		bool isProhibited =
			ProfilerDriver.IsIdentifierOnLocalhost(guid) &&
			(name.Contains("MetroPlayerX") || name.Contains("UWPPlayerX"));
		bool enabled =
			!isProhibited && ProfilerDriver.IsIdentifierConnectable(guid);
		...
		if (enabled) {
			if (m_EditorModeTargetState.HasValue &&
				...
				menuOptions.AddItem(
					Content.Playmode,
					isConnected &&
					!m_EditorModeTargetConnectionStatus(
						EditorConnectionTarget.MainEditorProcessPlaymode),
					() => {
							ProfilerDriver.connectedProfiler = guid;
							SuccessfullyConnectedToPlayer(
								connectionName,
								EditorConnectionTarget.MainEditorProcessPlaymode);
						});
				menuOptions.AddItem(
...
void AddAvailableDeviceConnections(GenericMenu menuOptions,
                                   ref bool hasOpenConnection) {
	foreach (var device in DevDeviceList.GetDevices()) {
		bool supportsPlayerConnection =
			(device.features & DevDeviceFeatures.PlayerConnection) != 0;
		if (!device.isConnected || !supportsPlayerConnection)
			continue;

		var url = "device://" + device.id;
		bool isConnected = ProfilerDriver.connectedProfiler ==
		                   PLAYER_DIRECT_URL_CONNECT_GUID &&
		                   ProfilerDriver.directConnectionUrl == url;
		hasOpenConnection |= isConnected;
		menuOptions.AddItem(new GUIContent(device.name), isConnected,
		                    () => DirectURLConnect(url));
	}
}

void AddConnectionViaEnterIPWindow(GenericMenu menuOptions,
                                   Rect buttonScreenRect) {
	menuOptions.AddItem(
		Content.EnterIPText, false,
...
~GeneralConnectionState() {
	if (!disposed)
		// Referring to the interface here, because the user only knows about
		// the public IConnectionState interfaces and nothing about the internal
		// GeneralConnectionState (except for the fact that it's about to show
		// up in this error's callstack)
		Debug.LogError(
...
public void Dispose() {
	if (!disposed) {
		if (s_AllGeneralAttachToPlayerStates != null) {
			for (int i = s_AllGeneralAttachToPlayerStates.Count - 1; i >= 0;
			     i--) {
				if (!s_AllGeneralAttachToPlayerStates[i].IsAlive ||
				    s_AllGeneralAttachToPlayerStates[i].Target == this) {
					s_AllGeneralAttachToPlayerStates.RemoveAt(i);
				}
			}
		}
...
static class Content {
public static readonly GUIContent ConnectButtonContent =
	UnityEditor.EditorGUIUtility.TrTextContent("Connect");
public static readonly string EnterPlayerIPWindowName =
	L10n.Tr("Enter Player IP");
}
...
public static void Show(Rect buttonScreenRect) {
	...
	AttachToPlayerPlayerIPWindow w =
		EditorWindow.GetWindowWithRect<AttachToPlayerPlayerIPWindow>(
			rect, true, Content.EnterPlayerIPWindowName);
	w.position = rect;
