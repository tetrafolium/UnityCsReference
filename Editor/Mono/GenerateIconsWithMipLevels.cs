 ...
 *
 *      InstanceID to Image
 *          gCachedThumbnails (AssetImporter::GetThumbnailForInstanceID) : where
   image is retrieved from Texture from gNamedTextures
 *          Used for setting icon for hierarchyProperty
 *
 *      InstanceID to Texture2D
 *          ms_HackedCache (AssetDatabaseProperty::GetCachedAssetDatabaseIcon) :
   where tetxure2D icon is generated from the image from gCachedThumbnails
 *          Cache max size: 200
 ...
 *
    ...
    - Texture2DNamed handles reading local files instead of editor resources
   bundle

    Icon loading in other projects
    - When reimport of asset (cpp): AssetDatabase::ImportAsset ->
   MonoImporter::GenerateAssetData -> ImageForMonoScript -> GetImageNames ->
   ObjectImages::Texture2DNamed
    - IconContent -> LoadIconRequired -> LoadIcon -> LoadIconForSkin -> Load
   (EditorResources.iconsPath + name) as Texture2D;
    - AssetDatabase.GetCachedIcon (assetpath) -> CPP ->
   AssetDatabaseProperty::GetCachedAssetDatabaseIcon
    - InternalEditorUtility.GetIconForFile (filename) ->
   EditorGUIUtility.FindTexture("boo Script Icon") -> CPP ->
   ObjectImages::Texture2DNamed(cpp)
...
private const string k_IconTargetFolder =
	"Assets/Editor Default Resources/Icons/Processed";
...
public readonly List<string> generatedFileNames =
	new List<string>(); // for internal use
}
...
private static InputData GetInputData() {
	return new InputData {
		       ...
		       mipIdentifier = k_IconMipIdentifier, mipFileExtension = "png"
	};
}
...
public static void GenerateAllIconsWithMipLevels() {
	...
	Debug.Log(string.Format(
			  "Generated {0} icons with mip levels in {1} seconds",
			  data.generatedFileNames.Count, Time.realtimeSinceStartup - startTime));

	RemoveUnusedFiles(data.generatedFileNames);
	AssetDatabase
	.Refresh(); // For some reason we creash if we dont refresh twice...
...
public static bool VerifyIconPath(string assetPath, bool logError) {
	if (string.IsNullOrEmpty(assetPath)) {
		return false;
	} else if (assetPath.IndexOf(k_IconSourceFolder, StringComparison.Ordinal) <
	           0) {
		if (logError)
			Debug.Log(
				"Selection is not a valid mip texture, it should be located in: " +
				k_IconSourceFolder);
		return false;
	} else if (assetPath.IndexOf(k_IconMipIdentifier,
	                             StringComparison.Ordinal) < 0) {
		if (logError)
			Debug.Log("Selection does not have a valid mip identifier " +
			          assetPath + "  " + k_IconMipIdentifier);
		return false;
	}
	return true;
}
...
public static void GenerateSelectedIconsWithMips() {
	...
	if (Selection.activeInstanceID == 0) {
		Debug.Log("Ensure to select a mip texture..." +
...
public static void
GenerateIconWithMipLevels(string assetPath,
                          Dictionary<int, Texture2D> mipTextures,
                          FileInfo fileInfo) {
	...
	baseName = baseName.Substring(
		0, baseName.LastIndexOf(data.mipIdentifier, StringComparison.Ordinal));

	string cwd = new DirectoryInfo(data.sourceFolder).FullName;
	List<string> assetPaths = GetIconAssetPaths(
		cwd, data.sourceFolder, data.mipIdentifier, data.mipFileExtension);

	EnsureFolderIsCreated(data.targetFolder);
	if (GenerateIcon(data, baseName, assetPaths, mipTextures, fileInfo))
		Debug.Log(
			string.Format("Generated {0} icon with mip levels in {1} seconds",
			              baseName, Time.realtimeSinceStartup - startTime));
	InternalEditorUtility.RepaintAllViews();
}
...
public static int MipLevelForAssetPath(string assetPath, string separator) {
	...
	if (separatorIndex == -1) {
		Debug.LogError("\"" + separator +
		               "\" could not be found in asset path: " + assetPath);
		return -1;
	}
	...
	if (endIndex == -1) {
		Debug.LogError("Could not find path extension in asset path: " +
		               assetPath);
		return -1;
	}
...
private static void GenerateIconsWithMips(InputData inputData) {
	...
	List<string> files =
		GetIconAssetPaths(cwd, inputData.sourceFolder, inputData.mipIdentifier,
		                  inputData.mipFileExtension);

	if (files.Count == 0) {
		Debug.LogWarning(
			"No mip files found for generating icons! Searching in: " +
			inputData.sourceFolder +
			", for files with extension: " + inputData.mipFileExtension);
	}
...
private static bool GenerateIcon(InputData inputData, string baseName,
                                 List<string> assetPathsOfAllIcons,
                                 Dictionary<int, Texture2D> mipTextures,
                                 FileInfo sourceFileInfo) {
	// We need to create the folder before we Create the Asset.
	// AssetDatabase.CreateFolder will trigger a Asset Garbage Collection and
	// stack items are not preserved.
	string generatedPath =
		inputData.targetFolder + "/" + baseName + " Icon" + ".asset";
	...
	Texture2D iconWithMips = CreateIconWithMipLevels(
		inputData, baseName, assetPathsOfAllIcons, mipTextures);
	...
	iconWithMips.name =
		baseName + " Icon" +
		".png"; // asset name must have .png as extension (used when loading the
	                // icon, search for LoadIconForSkin)
...
CreateIconWithMipLevels(InputData inputData, string baseName,
                        List<string> assetPathsOfAllIcons,
                        Dictionary<int, Texture2D> mipTextures) {
	List<string> allMipPaths = assetPathsOfAllIcons.FindAll(
		o => o.IndexOf('/' + baseName + inputData.mipIdentifier,
		               StringComparison.Ordinal) >= 0);
	...
	// Create our icon
	Texture2D iconWithMips = new Texture2D(maxResolution, maxResolution,
	                                       GraphicsFormat.R8G8B8A8_SRGB, -1,
	...
	// Keep for debugging
	// Debug.Log ("Processing max mip file: " + inputData.GetMipFileName
...
private static bool BlitMip(Texture2D iconWithMips,
                            List<Texture2D> sortedTextures, int mipLevel) {
...
static List<string> GetIconAssetPaths(string absolute, string folderPath,
                                      string mustHaveIdentifier,
                                      string extension) {
	Uri absoluteUri = new Uri(absolute);
	List<string> files = new List<string>(Directory.GetFiles(
						      absolute, "*." + extension, SearchOption.AllDirectories));
	files.RemoveAll(
		o =>
		o.IndexOf(mustHaveIdentifier, StringComparison.Ordinal) <
		0); // // Remove all files that do not have the 'mustHaveIdentifier'
...
private static void EnsureFolderIsCreatedRecursively(string targetFolder) {
	if (AssetDatabase.GetMainAssetInstanceID(targetFolder) == 0) {
		...
		AssetDatabase.CreateFolder(Path.GetDirectoryName(targetFolder),
...
// Get rid of old icons in the Icons folder (with same filename as a generated
// icon)
...
private static string[] GetBaseNames(InputData inputData,
                                     List<string> files) {
	...
	for (int i = 0; i < files.Count; ++i) {
		baseNames[i] =
			files [i]
			.Substring(startPos, files [i]
			           .IndexOf(inputData.mipIdentifier,
