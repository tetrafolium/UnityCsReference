...
[Obsolete(
	 "ClearShaderErrors has been deprecated. Use ClearShaderMessages instead (UnityUpgradable) -> ClearShaderMessages(*)")]
[NativeName("ClearShaderMessages")] extern public static void
ClearShaderErrors([ NotNull ] Shader s);
...
// We can't deprecate them yet (Sept 2019): Some integration tests expect no
// compile warning/error from built-in packages but TextMeshPro uses these
// APIs and emits deprecation warnings. We'll land the new APIs without
// deprecation first and then see if we can upgrade TextMeshPro afterwards.
...
//[Obsolete("Use Shader.GetPropertyRangeLimits and Shader.GetDefaultValue
//instead.", false)]
public static float GetRangeLimits(Shader s, int propertyIdx, int defminmax) {
	...
	return defminmax > 0 ? s.GetPropertyRangeLimits(propertyIdx)[defminmax - 1]
...
//[Obsolete("Use Shader.GetPropertyFlags and test against
//ShaderPropertyFlags.HideInInspector instead.", false)]
public static bool IsShaderPropertyHidden(Shader s, int propertyIdx) {
	if (s == null)
		throw new ArgumentNullException("s");
	return (s.GetPropertyFlags(propertyIdx) &
	        ShaderPropertyFlags.HideInInspector) != 0;
}

//[Obsolete("Use Shader.GetPropertyFlags and test against
//ShaderPropertyFlags.NonModifiableTextureData instead.", false)]
public static bool
IsShaderPropertyNonModifiableTexureProperty(Shader s, int propertyIdx) {
	if (s == null)
		throw new ArgumentNullException("s");
	return (s.GetPropertyFlags(propertyIdx) &
