...
#pragma warning disable 649
  ...
  internal struct ProjectInfo {
    public bool valid {
      get { return m_Valid != 0; }
    }
    public bool buildAllowed {
      get { return m_BuildAllowed != 0; }
    }
    public bool projectBound {
      get { return m_ProjectBound != 0; }
    }
    ...
    public string organizationForeignKey {
      get { return m_OrganizationForeignKey; }
    }
    ...
    public bool coppaLock {
      get { return m_COPPALock != 0; }
    }
    public bool moveLock {
      get { return m_MoveLock != 0; }
    }
  ...
  internal partial class UserInfo {
    ...
    public string primaryOrg {
      get { return m_PrimaryOrg; }
    }
    public bool whitelisted {
      get { return m_Whitelisted != 0; }
    }
    public string organizationForeignKeys {
      get { return m_OrganizationForeignKeys; }
    }
    public string accessToken {
      get { return m_AccessToken; }
    }
    public string[] organizationNames {
      get { return m_OrganizationNames; }
    }

    [NativeName("valid")] bool m_Valid;
    [NativeName("id")] string m_UserId;
    [NativeName("name")] string m_UserName;
    [NativeName("displayName")] string m_DisplayName;
    [NativeName("primaryOrg")] string m_PrimaryOrg;
    [NativeName("organizationForeignKeys")] string m_OrganizationForeignKeys;
    [NativeName("organizationNames")] string[] m_OrganizationNames;
    [NativeName("accessToken")] string m_AccessToken;

    [Ignore] int m_Whitelisted = 1;
  }
  ...
  internal struct ConnectInfo {
    public bool initialized {
      get { return m_Initialized != 0; }
    }
    public bool ready {
      get { return m_Ready != 0; }
    }
    public bool online {
      get { return m_Online != 0; }
    }
    public bool loggedIn {
      get { return m_LoggedIn != 0; }
    }
    public bool workOffline {
      get { return m_WorkOffline != 0; }
    }
    public bool showLoginWindow {
      get { return m_ShowLoginWindow != 0; }
    }
    public bool error {
      get { return m_Error != 0; }
    }
    public string lastErrorMsg {
      get { return m_LastErrorMsg; }
    }
    public bool maintenance {
      get { return m_Maintenance != 0; }
    }
...
#pragma warning restore 1635
  ...
  public static class UnityOAuth {
    ...
    GetAuthorizationCodeAsync(string clientId,
                              Action<AuthCodeResponse> callback) {
      ...
      if (string.IsNullOrEmpty(UnityConnect.instance.GetAccessToken())) {
        throw new InvalidOperationException(
            "User is not logged in or user status invalid.");
      }

      string url = string.Format("{0}/v1/oauth2/authorize",
                                 UnityConnect.instance.GetConfigurationURL(
                                     CloudConfigUrl.CloudIdentity));

      AsyncHTTPClient client = new AsyncHTTPClient(url);
      client.postData = string.Format(
          "client_id={0}&response_type=code&format=json&access_token={1}&prompt=none",
          clientId, UnityConnect.instance.GetAccessToken());
      client.doneCallback = delegate(IAsyncHTTPClient c) {
        AuthCodeResponse response = new AuthCodeResponse();
        if (!c.IsSuccess()) {
          response.Exception = new InvalidOperationException(
              "Failed to call Unity ID to get auth code.");
        } else {
          try {
            var json = new JSONParser(c.text).Parse();
            if (json.ContainsKey("code") && !json ["code"]
                                                 .IsNull()) {
              response.AuthCode = json ["code"]
                                      .AsString();
            } else if (json.ContainsKey("message")) {
              response.Exception = new InvalidOperationException(
                  string.Format("Error from server: {0}", json ["message"]
                                                              .AsString()));
            } else {
              response.Exception = new InvalidOperationException(
            ...
            response.Exception = new InvalidOperationException(
                "Unexpected response from server: Failed to parse JSON.");
          }
        }
  ...
  internal partial class UnityConnect {
    ...
    public void GoToHub(string page) {
      // Old CEF approach to go to the hub was removed. Keeping method for
      // legacy.
    }
    ...
    public bool DisplayDialog(string title, string message, string okBtn,
                              string cancelBtn) {
    ...
    static UnityConnect() { s_Instance = new UnityConnect(); }
    ...
    public string GetProjectGUID() { return GetProjectGUID_Internal(); }

    [NativeMethod("GetProjectName")]
    private static extern string GetProjectName_Internal();
    public string GetProjectName() { return GetProjectName_Internal(); }

    [NativeMethod("GetOrganizationId")]
    private static extern string GetOrganizationId_Internal();
    public string GetOrganizationId() { return GetOrganizationId_Internal(); }
    ...
    public void RefreshProject() { RefreshProject_Internal(); }

    [NativeMethod("ClearCache")]
    private static extern void ClearCache_Internal();
    public void ClearCache() { ClearCache_Internal(); }
    ...
    public void ShowLogin() { ShowLogin_Internal(); }

    [NativeMethod("OpenAuthorizedURLInWebBrowser")]
    private static extern void
    OpenAuthorizedURLInWebBrowser_Internal(string url);
    public void OpenAuthorizedURLInWebBrowser(string url) {
      OpenAuthorizedURLInWebBrowser_Internal(url);
    }

    [NativeMethod("BindProject")]
    private static extern void BindProject_Internal(string projectGUID,
                                                    string projectName,
                                                    string organizationId);
    public void BindProject(string projectGUID, string projectName,
                            string organizationId) {
    ...
    private void UnbindCloudProject() { UnbindProject_Internal(); }

    [NativeMethod("SetCOPPACompliance")]
    private static extern void
    SetCOPPACompliance_Internal(COPPACompliance compliance);
    ...
    public void SetError(int errorCode) { SetError_Internal(errorCode); }

    [NativeMethod("ClearError")]
    private static extern void ClearError_Internal(int errorCode);
    public void ClearError(int errorCode) { ClearError_Internal(errorCode); }

    [NativeMethod("ClearErrors")]
    private static extern void ClearErrors_Internal();
    public void ClearErrors() { ClearErrors_Internal(); }

    [NativeMethod("UnhandledError")]
    private static extern void
    UnhandledError_Internal(string request, int responseCode, string response);
    public void UnhandledError(string request, int responseCode,
                               string response) {
      UnhandledError_Internal(request, responseCode, response);
    }

    private static extern void InvokeComputerGoesToSleep();
    public void ComputerGoesToSleep() { InvokeComputerGoesToSleep(); }

    private static extern void InvokeComputerDidWakeUp();
    public void ComputerDidWakeUp() { InvokeComputerDidWakeUp(); }
