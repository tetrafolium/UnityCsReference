...
public partial class Highlighter {
  ...
  private const float kPopupDuration =
      0.33f; // How long time in seconds the popup takes
  private const int kExpansionMovementSize =
      5; // How many pixels the rect expands out in the pulsing movement
  ...
  public static bool Highlight(string windowTitle, string text,
    ...
    if (s_RecursionLock || searching) {
      ...
      Debug.LogWarning(
          "You are calling Highlighter.Highlight() inorrectly.  Avoid highlighting during layout and repaint events.");
    } else {
      ...
      if (!SetWindow(windowTitle)) {
        ...
        if (success) {
          ...
          Debug.LogWarning("Item " + text + " not found in window " +
                           windowTitle + ".");
          Stop();
        }
  ...
  internal static bool IsSearchingForIdentifier() {
    return searchMode == HighlightSearchMode.Identifier ||
  ...
  private static void Update() {
    ...
    // Keep elapsed time explicitly rather than measuring time since highlight
    // began. This way all views use the same elapsed time even if some realtime
    // elapsed between redraws of the different views.
    if (activeVisible)
    ...
    Rect r = activeRect;
    if (prevRect.width > 0) {
      // If the highlight rect moved, make the repaint region include both the
      // old and new rect.
    ...
    r = new RectOffset(kExpansionMovementSize + 2, kExpansionMovementSize + 2,
                       kExpansionMovementSize + 2, kExpansionMovementSize + 2)
            .Add(r);
    if (s_HighlightElapsedTime < kPopupDuration + 0.1f)
      r = new RectOffset((int) r.width / 2, (int) r.width / 2,
                         (int) r.height / 2, (int) r.height / 2)
              .Add(r);
    s_RepaintRegion = r;
    ...
    foreach (GUIView view in Resources.FindObjectsOfTypeAll(typeof(GUIView))) {
      // Only handle highlight in views in the same window as the view with the
      // highlight is in.
  ...
  private static bool SetWindow(string windowTitle) {
    ...
    foreach (GUIView currentView in views) {
      if (currentView is HostView) {
        if ((currentView as HostView).actualView.titleContent.text ==
            windowTitle) {
          view = currentView;
          break;
        }
      } else if (currentView.window &&
                 currentView.GetType().Name == windowTitle) {
        view = currentView;
        break;
      }
    }
  ...
  internal static void ControlHighlightGUI(GUIView self) {
    ...
    if (Event.current.type == EventType.ExecuteCommand &&
        Event.current.commandName == "HandleControlHighlight") {
    ...
    GUI.color = new Color(1, 1, 1,
                          Mathf.Clamp01(0.8f * popupScale - 0.3f * pulseValue));
