...
[NativeHeader("Runtime/Shaders/ShaderImpl/ShaderUtilities.h")] partial class
    EditorUtility {
  public static extern string OpenFilePanel(string title, string directory,
                                            string extension);

  private static extern string
  Internal_OpenFilePanelWithFilters(string title, string directory,
                                    string[] filters);
  public static string OpenFilePanelWithFilters(string title, string directory,
                                                string[] filters) {
    if (filters.Length % 2 != 0)
      throw new Exception(
          "Filters must be declared in pairs { \"FileType\", \"FileExtension\" }; for instance { \"CSharp\", \"cs\" }. ");
  ...
  public static extern int DisplayDialogComplex(string title, string message,
                                                string ok, string cancel,
                                                string alt);

  [FreeFunction("RunOpenFolderPanel")]
  public static extern string OpenFolderPanel(string title, string folder,
                                              string defaultName);

  [FreeFunction("RunSaveFolderPanel")]
  public static extern string SaveFolderPanel(string title, string folder,
                                              string defaultName);

  [FreeFunction("WarnPrefab")]
  public static extern bool WarnPrefab(Object target, string title,
                                       string warning, string okButton);

  public static extern bool IsPersistent(Object target);
  public static extern string SaveFilePanel(string title, string directory,
                                            string defaultName,
                                            string extension);
  public static extern int NaturalCompare(string a, string b);
  public static extern Object InstanceIDToObject(int instanceID);
  public static extern void CompressTexture([ NotNull ] Texture2D texture,
                                            TextureFormat format, int quality);
  public static extern void CompressCubemapTexture([ NotNull ] Cubemap texture,
                                                   TextureFormat format,
                                                   int quality);

  private extern static int[] RemapInstanceIds(UnityEngine.Object[] objects,
                                               int[] srcIds, int[] dstIds);

  internal static int[] RemapInstanceIds(UnityEngine.Object[] objects,
                                         Dictionary<int, int> idMap) {
    return RemapInstanceIds(objects, idMap.Keys.ToArray(),
                            idMap.Values.ToArray());
  }

  private extern static void RemapAssetReferences(UnityEngine.Object[] objects,
                                                  string[] sourceAssetPaths,
                                                  string[] dstAssetPaths,
                                                  int[] srcIds, int[] dstIds);

  internal static void
  RemapAssetReferences(UnityEngine.Object[] objects,
                       Dictionary<string, string> assetPathMap,
                       Dictionary<int, int> idMap = null) {
    RemapAssetReferences(objects, assetPathMap.Keys.ToArray(),
                         assetPathMap.Values.ToArray(),
                         idMap == null ? new int[0]
                         : idMap.Keys.ToArray(), idMap == null ? new int[0]
                         : idMap.Values.ToArray());
  }
  ...
  InvokeDiffTool(string leftTitle, string leftFile, string rightTitle,
                 string rightFile, string ancestorTitle, string ancestorFile);
  ...
  CopySerializedManagedFieldsOnly([ NotNull ] System.Object source,
                                  [ NotNull ] System.Object dest);

  [FreeFunction("CopySerializedIfDifferent")]
  private static extern void InternalCopySerializedIfDifferent(Object source,
                                                               Object dest);
  ...
  private static extern Object
  Internal_InstantiateRemoveAllNonAnimationComponentsSingle(Object data,
                                                            Vector3 pos,
                                                            Quaternion rot);
  ...
  Private_DisplayPopupMenu(Rect position, string menuItemPath, Object context,
                           int contextUserData);

  [FreeFunction("UpdateMenuTitleForLanguage")]
  internal static extern void
  Internal_UpdateMenuTitleForLanguage(SystemLanguage newloc);

  internal static extern void Internal_UpdateAllMenus();

  [FreeFunction("DisplayObjectContextPopupMenu")]
  internal static extern void
  DisplayObjectContextPopupMenu(Rect position, Object[] context,
                                int contextUserData);

  [FreeFunction("DisplayCustomContextPopupMenu")]
  private static extern void DisplayCustomContextPopupMenu(
      Rect screenPosition, string[] options, bool[] enabled, bool[] separator,
  ...
  internal static extern void DisplayObjectContextPopupMenuWithExtraItems(
      Rect position, Object[] context, int contextUserData, string[] options,
  ...
  public static extern void DisplayProgressBar(string title, string info,
                                               float progress);

  public static extern bool
  DisplayCancelableProgressBar(string title, string info, float progress);
  ...
  public static extern void
  SetSelectedRenderState(Renderer renderer,
  ...
  internal static extern GameObject
  Internal_CreateGameObjectWithHideFlags(string name, HideFlags flags);
  ...
  WSACreateTestCertificate(string path, string publisher, string password,
                           bool overwrite);
  internal static extern bool
  WSAGetCertificateExpirationDate(string path, string password,
                                  out long expirationDate);

  internal static extern bool IsWindows10OrGreater();

  public static extern void SetCameraAnimateMaterials([ NotNull ] Camera camera,
                                                      bool animate);
  public static extern void
  SetCameraAnimateMaterialsTime([ NotNull ] Camera camera, float time);
  ...
  public static extern bool scriptCompilationFailed {
    [FreeFunction("HaveEditorCompileErrors")] get;
  }
  ...
  internal static extern void
  SaveProjectAsTemplate(string targetPath, string name, string displayName,
                        string description, string defaultScene,
                        string version);
  ...
  internal static extern void
  LoadPlatformSupportModuleNativeDllInternal(string target);

  [FreeFunction("LoadPlatformSupportNativeLibrary")]
  internal static extern void
  LoadPlatformSupportNativeLibrary(string nativeLibrary);
  ...
  public static int GetDirtyCount(Object target) {
    return target != null ? GetDirtyCount(target.GetInstanceID()) : 0;
  }
  public static extern bool IsDirty(int instanceID);
  public static bool IsDirty(Object target) {
    return target != null ? IsDirty(target.GetInstanceID()) : false;
  }
  internal static extern string SaveBuildPanel(BuildTarget target, string title,
                                               string directory,
                                               string defaultName,
                                               string extension,
  ...
  private static extern string
  Internal_SaveFilePanelInProject(string title, string defaultName,
  ...
  public static void FocusProjectWindow() {
    ...
    if (prjBrowser != null) {
      prjBrowser.Focus(); // This line is to circumvent a limitation where a
                          // tabbed window can't be directly targeted by a
                          // command: only the focused tab can.
      var commandEvent = EditorGUIUtility.CommandEvent("FocusProjectWindow");
      prjBrowser.SendEvent(commandEvent);
    }
  }

  [StaticAccessor("GetApplication()",
