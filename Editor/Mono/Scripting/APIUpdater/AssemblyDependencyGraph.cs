...
namespace UnityEditor.Scripting.APIUpdater {
  ...
  internal class AssemblyDependencyGraph {
    public AssemblyDependencyGraph() { m_Graph = new List<DependencyEntry>(); }
    ...
    InternalAddDependencies(string root, params string[] dependencies) {
      var existingDependency = FindAssembly(root);
      if (existingDependency == null) {
        existingDependency =
            FindInDependents(root) ?? new DependencyEntry(root);
        m_Graph.Add(existingDependency);
      }

      foreach (var dependency in dependencies) {
        var dep =
            FindAssembly(dependency) ?? InternalAddDependencies(dependency);
    ...
    private DependencyEntry FindInDependents(string root) {
      foreach (var entry in m_Graph) {
        var found = entry.m_Dependencies.FirstOrDefault(
            dependency => dependency.m_Name == root);
        if (found != null)
          return found;
      }
    ...
    public IList<string> GetDependentsOf(string source) {
      ...
      foreach (var dependencyEntry in m_Graph) {
        if (dependencyEntry.m_Dependencies.Any(candidate =>
                                                   candidate.m_Name == source))
    ...
    public void RemoveDependencies(string dependent,
      ...
      entry.m_Dependencies.RemoveAll(
          candidate => dependencies.Contains(candidate.m_Name));
    }
    ...
    public void RemoveRoot(string tbr, bool updateDependents = false) {
      ...
      if (updateDependents) {
        foreach (var entry in m_Graph) {
          var danglingReference =
              entry.m_Dependencies.Find(e => e == toBeRemoved);
    ...
    private bool HasDirectOrIndirectDependency(DependencyEntry lhs,
                                               DependencyEntry rhs) {
    ...
    bool HasDirectOrIndirectDependencyRecursive(
        DependencyEntry toBeLookedUp, IList<DependencyEntry> dependencies) {
      foreach (var entry in dependencies) {
        if (entry == toBeLookedUp)
          return true;

        if (m_Processed.Contains(entry.Name)) {
          // We've found a cycle in the assemblies (which has already been
          // logged)
          return false;
        }
        ...
        try {
          if (HasDirectOrIndirectDependencyRecursive(toBeLookedUp,
    ...
    static void LogCycles(IEnumerable<DependencyEntry> entries,
                          HashSet<string> seen) {
      foreach (var entry in entries) {
        if ((entry.Status & AssemblyStatus.NoCyclesDetected) ==
            AssemblyStatus.NoCyclesDetected)
          continue;

        if (seen.Contains(entry.Name)) {
          Console.WriteLine(
              $"[APIUpdater] Warning: Cycle detected in assembly references: {string.Join("
                  ->", seen.ToArray())}->{entry.Name}. This is not supported and AssemblyUpdater may not work as expected.");
          continue;
        }
    ...
    public void SaveTo(Stream stream) {
      ...
      stream.Write(hash, 0,
                   hash.Length); // and reserve space of the "payload" hash.
      ...
      stream.Position =
          hash.Length +
          h.Length; // Position the stream in the first byte of the serialized
                    // data (i.e, skip *hash length* and *hash*
      hash = hasher.ComputeHash(stream);

      stream.Position = h.Length; // position the stream past the *hash length*
                                  // (i.e, *hash first byte*)
