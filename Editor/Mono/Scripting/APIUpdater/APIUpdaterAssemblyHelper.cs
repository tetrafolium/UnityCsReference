...
namespace UnityEditor.Scripting {
  internal class APIUpdaterAssemblyHelper {
    ...
    internal static int Run(string arguments, string workingDir,
                            out string stdOut, out string stdErr) {
      var monodistribution =
          MonoInstallationFinder.GetMonoInstallation("MonoBleedingEdge");

      var monoexe = Path.Combine(monodistribution, "bin/mono");
      if (Application.platform == RuntimePlatform.WindowsEditor) {
        monoexe = Application.platform ==
                  RuntimePlatform.WindowsEditor
                      ? CommandLineFormatter
                      .EscapeCharsWindows(monoexe + ".exe")
            : CommandLineFormatter.EscapeCharsQuote(monoexe + ".exe");
      }

      var startInfo = new ProcessStartInfo{Arguments = AssemblyUpdaterPath() +
                                                       " " + arguments,
                                           CreateNoWindow = true,
                                           FileName = monoexe,
    ...
    static string AssemblyUpdaterPath() {
      var unescapedAssemblyUpdaterPath =
          EditorApplication.applicationContentsPath +
          "/Tools/ScriptUpdater/AssemblyUpdater.exe";
      return Application.platform ==
             RuntimePlatform.WindowsEditor
               ? CommandLineFormatter
                 .EscapeCharsWindows(unescapedAssemblyUpdaterPath)
          : CommandLineFormatter.EscapeCharsQuote(unescapedAssemblyUpdaterPath);
    }

    internal static string
    ArgumentsForUpdateAssembly(string assemblyPath, string tempOutputPath,
                               IEnumerable<string> updateConfigSourcePaths) {
      var assemblyFullPath = ResolveAssemblyPath(assemblyPath);
      return "update -a " + assemblyFullPath + " --output " +
             CommandLineFormatter.PrepareFileName(tempOutputPath) +
             APIVersionArgument() +
             AssemblySearchPathArgument(
                 updateConfigSourcePaths.Select(Path.GetDirectoryName)
                     .Distinct()) +
             ConfigurationProviderAssembliesPathArgument(
                 updateConfigSourcePaths);
    }

    internal static string
    ArgumentsForCheckingForUpdaterConfigsOn(string assemblyPath) {
      var assemblyFullPath = ResolveAssemblyPath(assemblyPath);
      return "checkupdaterconfigs -a " + assemblyFullPath +
             TimeStampArgument() + AssemblySearchPathArgument();
    }
    ...
    internal static bool IsUnknown(int exitCode) {
      // See AssemblyUpdater/Program.cs
      return exitCode != 0 &&
             (exitCode & FirstErrorStatus) ==
                 0 // It is not an error code returned from AssemblyUpdater.exe
             && (exitCode & FirstSuccessStatus) ==
                    0; // It is not an success code returned from
                       // AssemblyUpdater.exe
    }
    ...
    private static string AssemblySearchPathArgument(
        IEnumerable<string> configurationSourceDirectories = null) {
      var searchPath =
          Path.Combine(MonoInstallationFinder.GetFrameWorksFolder(),
                       "Managed") +
          "," + "+" + Application.dataPath;

      if (configurationSourceDirectories != null) {
        var searchPathFromConfigSources =
            configurationSourceDirectories.Aggregate("", (acc, curr) =>
                                                             acc + ",+" + curr);
    ...
    private static string ConfigurationProviderAssembliesPathArgument(
      ...
      foreach (var configSourcePath in updateConfigSourcePaths) {
        paths.AppendFormat(
            " {0}", CommandLineFormatter.PrepareFileName(configSourcePath));
      }
