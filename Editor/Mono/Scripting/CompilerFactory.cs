...
public ScriptCompilerBase Create(ScriptAssembly scriptAssembly,
                                 string tempOutputDirectory) {
	if (scriptAssembly.Files.Length == 0) {
		throw new ArgumentException(
			      $"Cannot compile ScriptAssembly {scriptAssembly.Filename} with no files");
	}

	ScriptCompilerBase scriptCompilerBase;
	if (m_FactoryHelper.HasExternalCompiler()) {
		scriptCompilerBase =
			new ExternalCSharpCompiler(scriptAssembly, tempOutputDirectory);
	} else {
		scriptCompilerBase =
			new MicrosoftCSharpCompiler(scriptAssembly, tempOutputDirectory);
	}
...
public bool CompilerChanged() {
	...
	try {
		...
		Console.WriteLine(
			$"Exception getting compiler name: {exception.Message} {Environment.NewLine} {exception.StackTrace}");
		return false;
	}
...
public void UpdateCompilerUsed() {
	try {
		...
		Console.WriteLine(
			$"Exception for updating last used compiler: {exception.Message} {Environment.NewLine} {exception.StackTrace}");
	}
}
}
}
