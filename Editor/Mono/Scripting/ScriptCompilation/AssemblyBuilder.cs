...
namespace UnityEditor.Compilation {
  ...
  public class AssemblyBuilder {
    ...
    public ReferencesOptions referencesOptions {
      get;
      set;
    }
    ...
    public string[] defaultReferences {
      get { return GetDefaultReferences(EditorCompilationInterface.Instance); }
    }

    public string[] defaultDefines {
      get { return GetDefaultDefines(EditorCompilationInterface.Instance); }
    }
    ...
    public bool Build() { return Build(EditorCompilationInterface.Instance); }
    ...
    internal bool Build(EditorCompilation editorCompilation) {
      ...
      compilationTask = new CompilationTask(
          new[]{scriptAssembly}, null, scriptAssembly.OutputDirectory, this,
          EditorScriptCompilationOptions.BuildingEmpty,
          CompilationTaskOptions.StopOnFirstError, 1,
          editorCompilation.ILPostProcessing,
          new CompilerFactory(new CompilerFactoryHelper()));

      compilationTask.OnCompilationTaskStarted +=
          (context) => { editorCompilation.InvokeCompilationStarted(context); };
      ...
      compilationTask.OnCompilationFinished += (assembly, messages) => {
        editorCompilation.InvokeAssemblyCompilationFinished(
            assembly, messages, EditorScriptCompilationOptions.BuildingEmpty);
    ...
    internal
        string[] GetDefaultReferences(EditorCompilation editorCompilation) {
    ...
    public AssemblyBuilderStatus status {
      get {
        ...
        if (compilationTask.IsCompiling)
          return compilationTask.Poll() ? AssemblyBuilderStatus.Finished
                                        : AssemblyBuilderStatus.IsCompiling;
    ...
    void OnCompilationFinished(
        ...
        List<UnityEditor.Scripting.Compilers.CompilerMessage> messages) {
      if (buildFinished == null)
        return;

      var convertedMessages =
          EditorCompilation.ConvertCompilerMessages(messages);
