...
public static bool IsCodeGen(string assemblyName) {
	...
	if (name.StartsWith(k_CodeGenPrefix,
	                    StringComparison.OrdinalIgnoreCase) &&
	    name.EndsWith(k_CompilerSuffix, StringComparison.OrdinalIgnoreCase))
		return true;

	if (name.StartsWith(k_CodeGenPrefix,
	                    StringComparison.OrdinalIgnoreCase) &&
	    name.EndsWith(k_CodeGenSuffix, StringComparison.OrdinalIgnoreCase))
		return true;
...
public static bool IsCodeGenTest(string assemblyName) {
	...
	if (name.StartsWith(k_CodeGenPrefix,
	                    StringComparison.OrdinalIgnoreCase) &&
	    name.EndsWith(k_CompilerTestsSuffix,
	                  StringComparison.OrdinalIgnoreCase))
		return true;

	if (name.StartsWith(k_CodeGenPrefix,
	                    StringComparison.OrdinalIgnoreCase) &&
	    name.EndsWith(k_CodeGenTestsSuffix,
	                  StringComparison.OrdinalIgnoreCase))
		return true;
...
public static ScriptCodeGenAssemblies
ToScriptCodeGenAssemblies(ScriptAssembly[] scriptAssemblies) {
	var result = new ScriptCodeGenAssemblies();

	result.ScriptAssemblies =
		new List<ScriptAssembly>(scriptAssemblies.Length);
	result.CodeGenAssemblies =
		new List<ScriptAssembly>(scriptAssemblies.Length);
