...
namespace UnityEditor.Scripting.ScriptCompilation {
  internal struct SemVersionRanges {
    private readonly Dictionary<ExpressionTypeKey, ExpressionTypeValue>
        m_ExpressionTypes;
    ...
    public SemVersionRanges(
        Dictionary<ExpressionTypeKey, ExpressionTypeValue> expressionTypes) {
    ...
    public VersionDefineExpression GetExpression(string expression) {
      ...
      if (m_ExpressionTypes.ContainsKey(
              parsedExpressionData.GenerateExpressionTypeKey)) {
        ExpressionTypeValue expressionTypeValue =
            m_ExpressionTypes[parsedExpressionData.GenerateExpressionTypeKey];

        return new VersionDefineExpression(
            expressionTypeValue.IsValid, parsedExpressionData.leftSemVersion,
            parsedExpressionData.rightSemVersion){
    ...
    private static bool IsCharDigit(char c) { return (c >= '0' && c <= '9'); }
    ...
    private ExpressionParsedData ParseExpression(string expression) {
      ...
      if (!IsCharDigit(expression[0])) {
        ...
        if (!Contains(k_LeftValidSymbols, leftSymbol)) {
          throw new ExpressionNotValidException(
              $"Invalid character {leftSymbol}", expression);
        }
      ...
      if (!IsCharDigit(lastChar) && !IsCharLetter(lastChar)) {
        ...
        if (!Contains(k_RightValidSymbols, rightSymbol)) {
          throw new ExpressionNotValidException(
              $"Invalid character {rightSymbol}", expression);
        }
      ...
      int nextSemVer;
      string leftSemVerString =
          PopSemanticVersion(expression, begin, end, out nextSemVer);
      var hasLeftSemVer = !string.IsNullOrEmpty(leftSemVerString);
      if (hasLeftSemVer) {
        expressionParsedData.leftSemVersion =
            SemVersionParser.Parse(leftSemVerString);
      }

      int notNeeded;
      string rightSemVerString =
          PopSemanticVersion(expression, nextSemVer, end, out notNeeded);
      var hasRightSemVer = !string.IsNullOrEmpty(rightSemVerString);
      if (hasRightSemVer) {
        expressionParsedData.rightSemVersion =
            SemVersionParser.Parse(rightSemVerString);
      }
      expressionParsedData.GenerateExpressionTypeKey =
          new ExpressionTypeKey(leftSymbol
                                : leftSymbol, rightSymbol
                                : rightSymbol, hasSeparator
                                : hasSeperator, hasLeftSemVer
                                : hasLeftSemVer, hasRightSemVer
                                : hasRightSemVer);
      return expressionParsedData;
    }

    private static string PopSemanticVersion(string expression, int begin,
                                             int end, out int newBegin) {
      newBegin = begin;
      if (begin >= end) {
        return null;
      }
      ...
      while (newBegin <= end) {
        ...
        if (!IsCharDigit(value) && value != '.' && value != '-' &&
            !IsCharLetter(value) && value != '*') {
          throw new ExpressionNotValidException(
