...
using DiscoveredTargetInfo =
	      UnityEditor.BuildTargetDiscovery.DiscoveredTargetInfo;
...
static Dictionary<string, string> renamedReferences =
	new Dictionary<string, string>(StringComparer.Ordinal);

static CustomScriptAssemblyData() {
	renamedReferences["Unity.RenderPipelines.Lightweight.Editor"] =
		"Unity.RenderPipelines.Universal.Editor";
	renamedReferences["Unity.RenderPipelines.Lightweight.Runtime"] =
...
public static CustomScriptAssemblyData
FromJsonNoFieldValidation(string json) {
	...
	if (assemblyData == null)
		throw new System.Exception(
			      "Json file does not contain an assembly definition");
...
static void UpdateRenamedReferences(CustomScriptAssemblyData data) {
	...
	if (additionalReferences != null && additionalReferences.Count > 0) {
		...
		if (additionalReferences.Count > 0)
			data.references =
				data.references.Concat(additionalReferences).ToArray();
	}
}

[Serializable]
private class CustomScriptAssemblyWithLegacyData
	: CustomScriptAssemblyData {
...
public void UpdateLegacyData() {
	if (optionalUnityReferences != null && optionalUnityReferences.Any()) {
		...
		AddTo(ref references, "UnityEngine.TestRunner",
...
struct CustomScriptAssemblyPlatform {
	...
	public CustomScriptAssemblyPlatform(string name, string displayName,
...
public AssemblyFlags AssemblyFlags {
	get {
		...
		if (IncludePlatforms != null && IncludePlatforms.Length == 1 &&
		    IncludePlatforms[0].BuildTarget == BuildTarget.NoTarget)
		...
		bool imported = AssetDatabase.GetAssetFolderInfo(
			PathPrefix, out rootFolder, out immutable);
...
public static CustomScriptAssemblyPlatform[] DeprecatedPlatforms {
	get;
	private set;
}

static CustomScriptAssembly() {
	// When removing a platform from Platforms, please add it to
	// DeprecatedPlatforms.
	DiscoveredTargetInfo[] buildTargetList =
		BuildTargetDiscovery.GetBuildTargetInfoList();

	// Need extra slot for Editor which is not included in the build target
	// list
	Platforms = new CustomScriptAssemblyPlatform[buildTargetList.Length + 1];
	Platforms[0] =
		new CustomScriptAssemblyPlatform("Editor", BuildTarget.NoTarget);
...
#pragma warning disable 0618
	DeprecatedPlatforms = new CustomScriptAssemblyPlatform[] {
		...
		new CustomScriptAssemblyPlatform("LinuxStandalone32",
		                                 BuildTarget.StandaloneLinux),
		new CustomScriptAssemblyPlatform(
			"LinuxStandaloneUniversal", BuildTarget.StandaloneLinuxUniversal),
	};
...
public bool IsCompatibleWith(BuildTarget buildTarget,
	...
	var isBuildingWithTestAssemblies =
		(options &
		 EditorScriptCompilationOptions.BuildingIncludingTestAssemblies) ==
		EditorScriptCompilationOptions.BuildingIncludingTestAssemblies;
	var isTestAssembly =
		DefineConstraints != null &&
		DefineConstraints.Any(x => x == "UNITY_INCLUDE_TESTS");
	if (!buildingForEditor && isTestAssembly &&
	    !isBuildingWithTestAssemblies) {
		return false;
	}
	...
	if (DefineConstraints != null) {
		for (var i = 0; i < DefineConstraints.Length; ++i) {
			if (!DefineConstraintsHelper.IsDefineConstraintValid(
				    DefineConstraints[i])) {
	...
	if (!DefineConstraintsHelper.IsDefineConstraintsCompatible(
		    defines, DefineConstraints)) {
		return false;
	}

	if (isTestAssembly && AssetPathMetaData != null &&
	    !AssetPathMetaData.IsTestable) {
		return false;
	}
...
public static CustomScriptAssembly FromCustomScriptAssemblyData(
	string path, string guid,
	CustomScriptAssemblyData customScriptAssemblyData) {
	...
	customScriptAssembly.RootNamespace =
		customScriptAssemblyData.rootNamespace;
	...
	customScriptAssembly.AutoReferenced =
		customScriptAssemblyData.autoReferenced;
	customScriptAssembly.OverrideReferences =
		customScriptAssemblyData.overrideReferences;
	customScriptAssembly.NoEngineReferences =
		customScriptAssemblyData.noEngineReferences;
	customScriptAssembly.PrecompiledReferences =
		customScriptAssemblyData.precompiledReferences ?? new string[0];
	customScriptAssembly.DefineConstraints =
		customScriptAssemblyData.defineConstraints ?? new string[0];
	customScriptAssembly.VersionDefines =
		(customScriptAssemblyData.versionDefines ?? new VersionDefine[0]);

	if (customScriptAssemblyData.includePlatforms != null &&
	    customScriptAssemblyData.includePlatforms.Length > 0)
		customScriptAssembly.IncludePlatforms =
			GetPlatformsFromNames(customScriptAssemblyData.includePlatforms);

	if (customScriptAssemblyData.excludePlatforms != null &&
	    customScriptAssemblyData.excludePlatforms.Length > 0)
		customScriptAssembly.ExcludePlatforms =
			GetPlatformsFromNames(customScriptAssemblyData.excludePlatforms);

	var compilerOptions = new ScriptCompilerOptions();

	compilerOptions.AllowUnsafeCode =
		customScriptAssemblyData.allowUnsafeCode;
...
public static CustomScriptAssemblyPlatform[] GetPlatformsFromNames(
	string[] names) {
...
public static CustomScriptAssemblyPlatform
GetPlatformFromName(string name) {
	...
	var platformNames =
		Platforms.Select(p => string.Format("\"{0}\"", p.Name)).ToArray();
...
public static CustomScriptAssemblyPlatform
GetPlatformFromBuildTarget(BuildTarget buildTarget) {
	foreach (var platform in Platforms)
		if (platform.BuildTarget == buildTarget)
			return platform;

	throw new System.ArgumentException(string.Format(
						   "No CustomScriptAssemblyPlatform setup for BuildTarget '{0}'",
						   buildTarget));
}
}
}
