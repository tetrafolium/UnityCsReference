...
static HashSet<string> runtimeAssemblyNames = new HashSet<string>(
	new[] {"UnityEngine.UI.dll"}, StringComparer.Ordinal);

static HashSet<string> editorAssemblyNames = new HashSet<string>(
	new[] {"UnityEditor.UI.dll"}, StringComparer.Ordinal);

// Do not add automatic package references to these assemblies,
// as they also add themselves to all .asmdefs
static HashSet<string> ignoreAssemblies = new HashSet<string>(
	new[] {"UnityEngine.TestRunner.dll", "UnityEditor.TestRunner.dll"},
...
public static void
AddReferences(Dictionary<string, TargetAssembly> customTargetAssemblies,
              EditorScriptCompilationOptions options,
              Func<TargetAssembly, bool> shouldAdd) {
	...
	var autoReferencedAssemblies =
		buildingForEditor ? editorAssemblyNames : runtimeAssemblyNames;
	...
	foreach (var assemblyName in autoReferencedAssemblies) {
		...
		if (customTargetAssemblies.TryGetValue(assemblyName,
		                                       out targetAssembly)) {
	...
	foreach (var entry in customTargetAssemblies) {
		...
		var newReferences = assembly.References.Concat(additionalReferences)
		                    .Distinct()
		                    .ToList();
