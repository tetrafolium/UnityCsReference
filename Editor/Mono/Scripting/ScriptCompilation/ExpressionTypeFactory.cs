...
namespace UnityEditor.Scripting.ScriptCompilation {
  internal struct ExpressionTypeValue {
    public Func<SemVersion, SemVersion, SemVersion, bool> IsValid {
      get;
      set;
    }
  ...
  internal class ExpressionTypeFactory {
    public static Dictionary<ExpressionTypeKey, ExpressionTypeValue> Create() {
      return new Dictionary<ExpressionTypeKey, ExpressionTypeValue>{
          {new ExpressionTypeKey(hasLeftSemVer
                                 : true),
          ...
          {new ExpressionTypeKey(leftSymbol
                                 ...
                                 : true),
           new ExpressionTypeValue{
               IsValid = SemVersionRangesEvaluators
                             .MixedExclusiveMinimumAndInclusiveMaximumVersion,
               AppliedRule = "{0} < x <= {1}",
           }},
          {new ExpressionTypeKey(leftSymbol
                                 : '(', rightSymbol
          ...
          {new ExpressionTypeKey(leftSymbol
           ...
           new ExpressionTypeValue{
               IsValid = SemVersionRangesEvaluators
                             .MixedInclusiveMinimumAndExclusiveMaximumVersion,
               AppliedRule = "{0} <= x < {1}",
           }},
          {new ExpressionTypeKey(leftSymbol
                                 : '(', rightSymbol
                                 ...
                                 : true),
           new ExpressionTypeValue{
               IsValid = (left, right, version) =>
                   SemVersionRangesEvaluators.Invalid($"({version})"),
               AppliedRule = "Invalid",
           }},
      };
    }
