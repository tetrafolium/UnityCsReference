...
public static EditorCompilation Instance {
	get {
		if (editorCompilation == null) {
			...
			CompilationPipeline
			.ClearEditorCompilationErrors(); // Clear all errors on domain
			                                 // reload.
		}
...
static void
ClearErrors(EditorCompilation.CompilationSetupErrorFlags flags) {
	if (flags == EditorCompilation.CompilationSetupErrorFlags.none)
		CompilationPipeline.ClearEditorCompilationErrors();
}

static void LogException(Exception exception) {
	var assemblyDefinitionException =
		exception as AssemblyDefinitionException;
	var precompiledAssemblyException =
		exception as PrecompiledAssemblyException;

	if (assemblyDefinitionException != null &&
	    assemblyDefinitionException.filePaths.Length > 0) {
		...
		foreach (var filePath in precompiledAssemblyException.filePaths) {
			...
			var loadAssetAtPath =
				AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(filePath);

			if (loadAssetAtPath != null) {
				CompilationPipeline.LogEditorCompilationError(
					message, loadAssetAtPath.GetInstanceID());
			} else {
...
DirtyPredefinedAssemblyScripts(EditorScriptCompilationOptions options,
	...
	EmitExceptionAsError(() => Instance.DirtyPredefinedAssemblyScripts(
				     options, platformGroup, platform));
}
...
[RequiredByNativeCode]
public static void
SkipCustomScriptAssemblyGraphValidation(bool skipValidation) {
...
[RequiredByNativeCode]
public static void
SetAllUnityAssemblies(PrecompiledAssembly[] unityAssemblies) {
...
[RequiredByNativeCode]
public static void SetAssembliesOutputDirectories(string directory,
...
public static string GetCompileScriptsOutputDirectory() {
	return EmitExceptionAsError(
		() => Instance.GetCompileScriptsOutputDirectory(), string.Empty);
}

[RequiredByNativeCode]
public static void
SetAllCustomScriptAssemblyJsons(string[] allAssemblyJsons, string[] guids) {
	EmitExceptionsAsErrors(
		Instance.SetAllCustomScriptAssemblyJsons(allAssemblyJsons, guids));
}

[RequiredByNativeCode]
public static void SetAllCustomScriptAssemblyReferenceJsons(
	string[] allAssemblyReferenceJsons,
	string[] allAssemblyReferenceJsonContents) {
	EmitExceptionsAsErrors(
		Instance.SetAllCustomScriptAssemblyReferenceJsonsContents(
			allAssemblyReferenceJsons, allAssemblyReferenceJsonContents));
}

[RequiredByNativeCode]
public static void
SetAllCustomScriptAssemblyJsonContents(string[] allAssemblyJsonPaths,
                                       string[] allAssemblyJsonContents,
                                       string[] guids) {
	EmitExceptionsAsErrors(Instance.SetAllCustomScriptAssemblyJsonContents(
				       allAssemblyJsonPaths, allAssemblyJsonContents, guids));
}

[RequiredByNativeCode]
public static EditorCompilation
.TargetAssemblyInfo[] GetAllCompiledAndResolvedCustomTargetAssemblies(
	EditorScriptCompilationOptions options, BuildTarget buildTarget) {
	EditorCompilation
	.CustomScriptAssemblyAndReference[] assembliesWithMissingReference =
		null;

	var result = EmitExceptionAsError(
		() => Instance.GetAllCompiledAndResolvedCustomTargetAssemblies(
			options, buildTarget, out assembliesWithMissingReference),
		new EditorCompilation.TargetAssemblyInfo[0]);

	if (assembliesWithMissingReference.Length > 0) {
		foreach (var assemblyAndReference in assembliesWithMissingReference) {
			LogWarning(
				string.Format(
					"The assembly for Assembly Definition File '{0}' will not be loaded. Because the assembly for its reference '{1}'' does not exist on the file system. " +
	...
	// Check we do not have any assembly definition references (asmref)
	// without matching assembly definitions (asmdef)
	List<CustomScriptAssemblyReference> referencesWithMissingAssemblies;
	Instance.GetAssemblyDefinitionReferencesWithMissingAssemblies(
		out referencesWithMissingAssemblies);
	if (referencesWithMissingAssemblies.Count > 0) {
		foreach (var asmref in referencesWithMissingAssemblies) {
			var warning =
				$"The Assembly Definition Reference file '{asmref.FilePath}' will not be used. ";
			if (string.IsNullOrEmpty(asmref.Reference))
				warning +=
					"It does not contain a reference to an Assembly Definition File.";
			else
				warning +=
					$"The reference to the Assembly Definition File with the name '{asmref.Reference}' could not be found.";
...
public static string[] GetCompiledAssemblyGraph(string assemblyName) {
	return EmitExceptionAsError(
		() => Instance.GetCompiledAssemblyGraph(assemblyName), new string[0]);
}

[RequiredByNativeCode]
public static EditorCompilation
.TargetAssemblyInfo[] GetTargetAssembliesWithScripts() {
	var options = GetAdditionalEditorScriptCompilationOptions();
	return EmitExceptionAsError(
		() => Instance.GetTargetAssembliesWithScripts(options),
		new EditorCompilation.TargetAssemblyInfo[0]);
}
...
[RequiredByNativeCode]
public static EditorCompilation.CompileStatus
CompileScripts(EditorScriptCompilationOptions definesOptions,
               BuildTargetGroup platformGroup, BuildTarget platform,
               string[] extraScriptingDefines) {
	return EmitExceptionAsError(
		() => Instance.CompileScripts(definesOptions, platformGroup, platform,
...
[RequiredByNativeCode]
public static bool
CompileCustomScriptAssemblies(EditorScriptCompilationOptions definesOptions,
                              BuildTargetGroup platformGroup,
                              BuildTarget platform) {
	return EmitExceptionAsError(() => Instance.CompileCustomScriptAssemblies(
					    definesOptions, platformGroup, platform),
	                            false);
}
...
[RequiredByNativeCode]
public static EditorCompilation
.AssemblyCompilerMessages[] GetCompileMessages() {
...
public static EditorCompilation.CompileStatus TickCompilationPipeline(
	...
	try {
		return Instance.TickCompilationPipeline(
...
public static EditorCompilation.CompileStatus PollCompilation() {
	return EmitExceptionAsError(() => Instance.PollCompilation(),
	                            EditorCompilation.CompileStatus.Idle);
}
...
[RequiredByNativeCode]
public static EditorCompilation.TargetAssemblyInfo
GetTargetAssembly(string scriptPath) {
...
public static EditorScriptCompilationOptions
GetAdditionalEditorScriptCompilationOptions() {
	...
	if (PlayerSettings.useReferenceAssemblies)
		options |=
			EditorScriptCompilationOptions.BuildingUseReferenceAssemblies;
...
public static ScriptAssembly[] GetAllScriptAssembliesForLanguage<T>()
    where T : SupportedLanguage {
...
GetScriptAssemblyForLanguage<T>(string assemblyNameOrPath) where T
: SupportedLanguage {
	var additionalOptions = GetAdditionalEditorScriptCompilationOptions();

	return Instance.GetScriptAssemblyForLanguage<T>(assemblyNameOrPath,
	                                                additionalOptions);
}
}
}
