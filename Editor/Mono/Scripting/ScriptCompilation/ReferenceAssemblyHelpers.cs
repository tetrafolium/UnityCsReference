...
public static bool IsReferenceAssemblyUnchanged(ScriptAssembly assembly,
                                                string outputDirectory) {
	var referenceAssemblyPath = AssetPath.Combine(
		outputDirectory, assembly.ReferenceAssemblyFilename);
	var assemblyFileHashPath =
		AssetPath.Combine(outputDirectory, $"{assembly.Filename}.hash");
	...
	unsafe {
		fixed(byte *bytePointer = bytes) {
			HashUnsafeUtilities.ComputeHash128(
				bytePointer, (ulong) bytes.LongLength, &hash1, &hash2);
		}
	}
