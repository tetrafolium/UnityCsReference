...
namespace UnityEditor.Scripting.ScriptCompilation {
  internal struct SemVersion : IComparable<SemVersion>, IComparable {
    ...
    public int CompareTo(object obj) { return CompareTo((SemVersion) obj); }
    ...
    private static int CompareExtension(string current, string other,
                                        bool lower = false) {
      ...
      for (var i = 0; i < Math.Min(currentParts.Length, otherParts.Length);
           i++) {
  ...
  internal static class SemVersionParser {
    private static string GetStringUntil(string value, ref int cursor,
                                         Func<char, bool> isEnd) {
      int length = 0;
    ...
    public static SemVersion Parse(string version, bool strict = false) {
      ...
      try {
        major = int.Parse(
            GetStringUntil(version, ref cursor, x => !char.IsDigit(x)));
