...
public Func<ScriptAssemblySettings, string[], bool> IsCompatibleFunc {
	get;
	private set;
}
public List<TargetAssembly> References {
	get;
	set;
}
public List<string> ExplicitPrecompiledReferences {
	get;
	set;
}
public TargetAssemblyType Type {
	get;
	private set;
}
...
public TargetAssembly(
	string name, SupportedLanguage language, AssemblyFlags flags,
	TargetAssemblyType type, string pathPrefix, string[] additionalPrefixes,
	...
	if (AdditionalPrefixes != null)
		MaxPathLength = UnityEngine.Mathf.Max(
			MaxPathLength, AdditionalPrefixes.Max(am => am.Length));
}
...
public EditorCompatibility editorCompatibility {
	get {
		bool isCompatibleWithEditor =
			IsCompatibleFunc == null ||
			IsCompatibleFunc(
				new ScriptAssemblySettings {
		...
		return isCompatibleWithEditor
		   ? EditorCompatibility
		       .CompatibleWithEditor : EditorCompatibility.NotCompatibleWithEditor;
	}
}
...
protected bool Equals(TargetAssembly other) {
	return string.Equals(Filename, other.Filename) && Flags == other.Flags &&
	       Type == other.Type;
}
