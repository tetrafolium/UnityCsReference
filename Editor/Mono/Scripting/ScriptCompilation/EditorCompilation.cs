...
namespace UnityEditor.Scripting.ScriptCompilation {
  class EditorCompilation {
    ...
    class UnsafeErrorProcessor : UnitySpecificCompilerMessageProcessor {
      ...
      public UnsafeErrorProcessor(ScriptAssembly assembly,
                                  EditorCompilation editorCompilation) {
        var assemblyName =
            AssetPath.GetAssemblyNameWithoutExtension(assembly.Filename);

        try {
          var customScriptAssembly =
              editorCompilation.FindCustomScriptAssemblyFromAssemblyName(
                  assemblyName);
          unityUnsafeMessage = string.Format(
              "Enable \"Allow 'unsafe' code\" in the inspector for '{0}' to fix this error.",
              customScriptAssembly.FilePath);
        } catch {
          unityUnsafeMessage =
      ...
      public override bool IsInterestedInMessage(CompilerMessage m) {
        return m.type == CompilerMessageType.Error &&
               m.message.Contains("CS0227");
      }
    ...
    internal class DeterministicAssemblyVersionErrorProcessor
        : UnitySpecificCompilerMessageProcessor {
      public override bool IsInterestedInMessage(CompilerMessage message) {
        if (message.type != CompilerMessageType.Error) {
          return false;
        }

        if (message.message.IndexOf("error CS8357", StringComparison.Ordinal) >=
            0) {
          return true;
        }
    ...
    class ModuleReferenceErrorProcessor
        : UnitySpecificCompilerMessageProcessor {
      Regex messageRegex;
      ...
      public override bool IsInterestedInMessage(CompilerMessage m) {
        return m.type == CompilerMessageType.Error &&
               (m.message.Contains("CS1069") || m.message.Contains("CS1070"));
      }
      ...
      public override void PostprocessMessage(ref CompilerMessage message) {
        ...
        if (match.Success) {
          ...
          message.message += string.Format(
              "Enable the built in package '{0}' in the Package Manager window to fix this error.",
              moduleName);
        }
      }
    }

    public PrecompiledAssemblyProviderBase PrecompiledAssemblyProvider {
      get;
      set;
    }
    = new PrecompiledAssemblyProvider();

    Dictionary<object, Stopwatch> stopWatchDict =
        new Dictionary<object, Stopwatch>();
    ...
    HashSet<TargetAssembly> dirtyTargetAssemblies =
        new HashSet<TargetAssembly>();
    ...
    List<CustomScriptAssemblyReference> customScriptAssemblyReferences =
        new List<CustomScriptAssemblyReference>();
    Dictionary<string, TargetAssembly> customTargetAssemblies =
        new Dictionary<string, TargetAssembly>(); // TargetAssemblies for
                                                  // customScriptAssemblies.
    PrecompiledAssembly[] unityAssemblies;
    CompilationTask compilationTask;
    string outputDirectory;
    string outputDirectoryEditor;
    CompilationSetupErrorFlags setupErrorFlags =
        CompilationSetupErrorFlags.none;
    bool skipCustomScriptAssemblyGraphValidation = false;
    List<AssemblyBuilder> assemblyBuilders =
        new List<Compilation.AssemblyBuilder>();
    ...
    public event
        Action<ScriptAssembly, UnityEditor.Compilation.CompilerMessage[],
               EditorScriptCompilationOptions> assemblyCompilationFinished;

    public Dictionary<string, TargetAssembly> CustomTargetAssemblies {
      get { return customTargetAssemblies; }
    }
    ...
    internal void
    SetAssetPathsMetaData(AssetPathMetaData[] assetPathMetaDatas) {
      m_AssetPathsMetaData = assetPathMetaDatas;

      var assetPathVersionMetaDataComparer =
          new AssetPathVersionMetaDataComparer();

      m_AllDistinctVersionMetaDatas =
          assetPathMetaDatas ?
              .SelectMany(
                  x => x.VersionMetaDatas ?? new AssetPathVersionMetaData[0])
              .Distinct(assetPathVersionMetaDataComparer)
              .ToDictionary(x => x.Name, x => x.Version);
      UpdateCustomTargetAssembliesAssetPathsMetaData(
          customScriptAssemblies, assetPathMetaDatas, forceUpdate
          : true);
    }
    ...
    public string[] GetExtensionsSupportedByCompiler() {
      var languages = ScriptCompilers.SupportedLanguages;
      return languages.Select(language => language.GetExtensionICanCompile())
          .ToArray();
    }

    public void
    DirtyPredefinedAssemblyScripts(EditorScriptCompilationOptions options,
                                   BuildTargetGroup platformGroup,
                                   BuildTarget platform) {
      var scriptAssemblySettings =
          CreateScriptAssemblySettings(platformGroup, platform, options);
      var scriptAssemblies = GetAllScriptAssembliesOfType(
          scriptAssemblySettings, TargetAssemblyType.Predefined);
    ...
    public void DirtyMovedScript(string oldPath, string newPath) {
      ...
      var targetAssembly = EditorBuildRules.GetTargetAssemblyLinearSearch(
          oldPath, projectDirectory, customTargetAssemblies);
    ...
    public void DirtyRemovedScript(string path) {
      ...
      var targetAssembly = EditorBuildRules.GetTargetAssemblyLinearSearch(
          path, projectDirectory, customTargetAssemblies);
      ...
      if (targetAssembly == null) {
        ...
        // Add to changedAssemblies in case we delete the last script of an
        // assembly and then do not get OnCompilationFinished callback
    ...
    private void DirtyPrecompiledAssembly(string path) {
      ...
      var precompiledAssemblies =
          PrecompiledAssemblyProvider.GetPrecompiledAssemblies(
              true, EditorUserBuildSettings.activeBuildTargetGroup,
      ...
      var explicitlyReferenced = (precompiledAssembly.Value.Flags &
                                  AssemblyFlags.ExplicitlyReferenced) ==
                                 AssemblyFlags.ExplicitlyReferenced;
    ...
    public void SetAssembliesOutputDirectories(string directory,
    ...
    public string GetEditorAssembliesOutputDirectory() {
      if (string.IsNullOrEmpty(outputDirectoryEditor))
        throw new Exception(
            "Must set output directories through SetAssembliesOutputDirectories before compiling");
      return outputDirectoryEditor;
    }

    public void
    SetCompilationSetupErrorFlags(CompilationSetupErrorFlags flags) {
    ...
    public void
    ClearCompilationSetupErrorFlags(CompilationSetupErrorFlags flags) {
      var newFlags = setupErrorFlags & ~flags;
    ...
    public void GetAssemblyDefinitionReferencesWithMissingAssemblies(
        out List<CustomScriptAssemblyReference>
            referencesWithMissingAssemblies) {
    ...
    public TargetAssemblyInfo[] GetAllCompiledAndResolvedCustomTargetAssemblies(
      ...
      if (customTargetAssemblies == null) {
        assembliesWithMissingReference =
            new CustomScriptAssemblyAndReference[0];
        return new TargetAssemblyInfo[0];
      }

      var customTargetAssemblyCompiledPaths =
          new Dictionary<TargetAssembly, string>();
      ...
      var assembliesWithScripts =
          GetTargetAssembliesWithScriptsHashSet(options);
        ...
        if (customTargetAssemblyCompiledPaths.Count > 0) {
          foreach (var entry in customTargetAssemblies) {
            ...
            foreach (var reference in assembly.References) {
              // Don't check references that are not compatible with the current
              // build target, as those assemblies have not been compiled.
              if (!EditorBuildRules.IsCompatibleWithPlatformAndDefines(
                      reference, buildTarget, options))
                continue;
              ...
              if (!customTargetAssemblyCompiledPaths.ContainsKey(reference)) {
                ...
                var customScriptAssembly =
                    FindCustomTargetAssemblyFromTargetAssembly(assembly);
                var customScriptAssemblyReference =
                    FindCustomTargetAssemblyFromTargetAssembly(reference);

                removedAssemblies.Add(new CustomScriptAssemblyAndReference{
                    Assembly = customScriptAssembly,
                    Reference = customScriptAssemblyReference});
                removed = true;
                break;
              }
            }
      ...
      foreach (var entry in customTargetAssemblyCompiledPaths) {
        ...
        targetAssemblies [index++]
        = ToTargetAssemblyInfo(assembly);
      }
    ...
    public string[] GetCompiledAssemblyGraph(string assemblyName) {
      ...
      if (compilationTask == null)
        throw new InvalidOperationException(
            "Cannot call GetCompiledAssemblyGraph without having an active CompilationTask");

      var compiledScriptAssemblies =
          compilationTask.CodeGenAssemblies
              .Concat(compilationTask.ScriptAssemblies)
              .ToArray();

      assemblyName = AssetPath.GetAssemblyNameWithoutExtension(assemblyName);
      ScriptAssembly scriptAssembly = compiledScriptAssemblies.SingleOrDefault(
          a => AssetPath.GetAssemblyNameWithoutExtension(a.Filename) ==
               assemblyName);

      if (scriptAssembly == null)
        throw new ArgumentException(
            $"Could not find assembly name '{assemblyName}' in GetCompiledAssemblyGraph.");

      // Build a dictionary with the set of compiled referencing assemblies for
      // each compiled assembly.
      var referencingAssemblies =
          new Dictionary<ScriptAssembly, HashSet<ScriptAssembly>>();

      foreach (var compiledScriptAssembly in compiledScriptAssemblies) {
        foreach (var referenceAssembly in compiledScriptAssembly
                     .ScriptAssemblyReferences) {
          HashSet<ScriptAssembly> referencingScriptAssemblies;

          if (!referencingAssemblies.TryGetValue(
                  referenceAssembly, out referencingScriptAssemblies)) {
            referencingScriptAssemblies = new HashSet<ScriptAssembly>();
            referencingAssemblies[referenceAssembly] =
                referencingScriptAssemblies;
          }
      ...
      while (visit.Count > 0) {
        ...
        if (!referencingAssemblies.TryGetValue(visitAssembly,
                                               out referencing)) {
          continue;
        }
    ...
    LoadCustomScriptAssemblyFromJsonPath(string path, string guid) {
      var json = Utility.ReadTextAsset(path);

      try {
        var customScriptAssemblyData = CustomScriptAssemblyData.FromJson(json);
        return CustomScriptAssembly.FromCustomScriptAssemblyData(
            path, guid, customScriptAssemblyData);
      } catch (Exception e) {
    ...
    static CustomScriptAssembly
    LoadCustomScriptAssemblyFromJson(string path, string json, string guid) {
      try {
        var customScriptAssemblyData = CustomScriptAssemblyData.FromJson(json);
        return CustomScriptAssembly.FromCustomScriptAssemblyData(
            path, guid, customScriptAssemblyData);
    ...
    static CustomScriptAssemblyReference
    LoadCustomScriptAssemblyReferenceFromJsonPath(string path) {
      var json = Utility.ReadTextAsset(path);
      return LoadCustomScriptAssemblyReferenceFromJson(path, json);
    }

    static CustomScriptAssemblyReference
    LoadCustomScriptAssemblyReferenceFromJson(string path, string json) {
      try {
        var customScriptAssemblyRefData =
            CustomScriptAssemblyReferenceData.FromJson(json);
        return CustomScriptAssemblyReference
            .FromCustomScriptAssemblyReferenceData(path,
                                                   customScriptAssemblyRefData);
      } catch (Exception e) {
        throw new Compilation.AssemblyDefinitionException(e.Message, path);
      }
    }

    string[] CustomTargetAssembliesToFilePaths(
        IEnumerable<TargetAssembly> targetAssemblies) {
      var customAssemblies = targetAssemblies.Select(
          a => FindCustomTargetAssemblyFromTargetAssembly(a));
    ...
    string CustomTargetAssemblyToFilePath(TargetAssembly targetAssembly) {
      return FindCustomTargetAssemblyFromTargetAssembly(targetAssembly)
          .FilePath;
    }
    ...
    static void CheckCyclicAssemblyReferencesDFS(
        TargetAssembly visitAssembly, HashSet<TargetAssembly> visited,
      ...
      foreach (var reference in visitAssembly.References) {
        if (reference.Filename == visitAssembly.Filename) {
          throw new Compilation.AssemblyDefinitionException(
              "Assembly contains a references to itself",
        ...
        if (!visited.Contains(reference)) {
          CheckCyclicAssemblyReferencesDFS(reference, visited, recursion,
                                           functions);
        }
      }
    ...
    public static void CheckCyclicAssemblyReferences(
      ...
      foreach (var entry in customTargetAssemblies) {
        ...
        if (!visited.Contains(assembly)) {
          ...
          CheckCyclicAssemblyReferencesDFS(assembly, visited, recursion,
                                           functions);
        }
      }
    }
    ...
    void CheckCyclicAssemblyReferences() {
      try {
        ...
        SetCompilationSetupErrorFlags(
            CompilationSetupErrorFlags.cyclicReferences);
        throw e;
      }
    }

    public static Exception[] UpdateCustomScriptAssemblies(
        CustomScriptAssembly[] customScriptAssemblies,
        List<CustomScriptAssemblyReference> customScriptAssemblyReferences,
        AssetPathMetaData[] assetPathsMetaData) {
      var asmrefLookup =
          customScriptAssemblyReferences.ToLookup(x => x.Reference);

      // Add AdditionalPrefixes
      foreach (var assembly in customScriptAssemblies) {
        var foundAsmRefs = asmrefLookup[assembly.Name];

        // Assign the found references or null. We need to assign null so as not
        // to hold onto references that may have been removed/changed.
        assembly.AdditionalPrefixes =
            foundAsmRefs.Any()
                ? foundAsmRefs.Select(ar => ar.PathPrefix).ToArray()
                : null;
      }

      var exceptions = UpdateCustomTargetAssembliesAssetPathsMetaData(
          customScriptAssemblies, assetPathsMetaData);
    ...
    static Exception[] UpdateCustomTargetAssembliesAssetPathsMetaData(
      ...
      for (int i = 0; i < assetPathsMetaData.Length; ++i) {
        ...
        assetMetaDataPaths[i] = AssetPath.ReplaceSeparators(
            assetPathMetaData.DirectoryPath + AssetPath.Separator);
    ...
    Exception[] UpdateCustomTargetAssemblies(
        bool forceUpdateAssetMetadata = false) {
      var exceptions = UpdateCustomScriptAssemblies(
          customScriptAssemblies, customScriptAssemblyReferences,
      ...
      customTargetAssemblies =
          EditorBuildRules.CreateTargetAssemblies(customScriptAssemblies);

      ClearCompilationSetupErrorFlags(
          CompilationSetupErrorFlags.cyclicReferences);
    ...
    void UpdateDirtyTargetAssemblies() {
      ...
      if (customTargetAssemblies != null) {
        foreach (var entry in customTargetAssemblies) {
          var customTargetAssembly = entry.Value;
          if (dirtyTargetAssemblyFilenames.Contains(
      ...
      var predefinedTargetAssemblies =
    ...
    public Exception[] SetAllCustomScriptAssemblyReferenceJsons(
        string[] paths) {
      return SetAllCustomScriptAssemblyReferenceJsonsContents(paths, null);
    }

    public Exception[] SetAllCustomScriptAssemblyReferenceJsonsContents(
        string[] paths, string[] contents) {
      var assemblyRefs = new List<CustomScriptAssemblyReference>();
      var exceptions = new List<Exception>();

      // We only construct this lookup if it is required, which is when we are
      // using guids instead of assembly names.
      Dictionary<string, CustomScriptAssembly> guidsToAssemblies = null;

      // To check if a path prefix is already being used we use a Dictionary
      // where the key is the prefix and the value is the file path.
      var prefixToFilePathLookup = skipCustomScriptAssemblyGraphValidation ? null
          : customScriptAssemblies.GroupBy(x => x.PathPrefix)
                .ToDictionary(x => x.First().PathPrefix,
                              x => new List<string>(){x.First().FilePath},
                              StringComparer.OrdinalIgnoreCase);

      for (var i = 0; i < paths.Length; ++i) {
        var path = paths[i];

        CustomScriptAssemblyReference loadedCustomScriptAssemblyReference =
            null;

        try {
          var fullPath = AssetPath.IsPathRooted(path)
                             ? AssetPath.GetFullPath(path)
                             : AssetPath.Combine(projectDirectory, path);

          if (contents != null) {
            var jsonContents = contents[i];
            loadedCustomScriptAssemblyReference =
                LoadCustomScriptAssemblyReferenceFromJson(fullPath,
                                                          jsonContents);
          } else {
            loadedCustomScriptAssemblyReference =
                LoadCustomScriptAssemblyReferenceFromJsonPath(fullPath);
          }

          if (!skipCustomScriptAssemblyGraphValidation) {
            // Check both asmdef and asmref files.
            List<string> duplicateFilePaths;
            if (prefixToFilePathLookup.TryGetValue(
                    loadedCustomScriptAssemblyReference.PathPrefix,
                    out duplicateFilePaths)) {
              ...
              throw new Compilation.AssemblyDefinitionException(
                  string.Format(
                      "Folder '{0}' contains multiple assembly definition files",
                      loadedCustomScriptAssemblyReference.PathPrefix),
                  filePaths.ToArray());
            }
          }
          ...
          if (GUIDReference.IsGUIDReference(
                  loadedCustomScriptAssemblyReference.Reference)) {
            // Generate the guid to assembly lookup?
            if (guidsToAssemblies == null)
              guidsToAssemblies =
                  customScriptAssemblies.ToDictionary(x => x.GUID);

            var guid = Utility.FastToLower(GUIDReference.GUIDReferenceToGUID(
                loadedCustomScriptAssemblyReference.Reference));
        ...
        if (loadedCustomScriptAssemblyReference != null) {
          ...
          if (!skipCustomScriptAssemblyGraphValidation) {
            ...
            if (!prefixToFilePathLookup.TryGetValue(
              ...
              prefixToFilePathLookup[loadedCustomScriptAssemblyReference
                                         .PathPrefix] = duplicateFilePaths;
            }

            duplicateFilePaths.Add(
      ...
      var updateCustomTargetAssembliesExceptions =
    ...
    public Exception[] SetAllCustomScriptAssemblyJsonContents(string[] paths,
                                                              string[] contents,
                                                              string[] guids) {
      var assemblies = new List<CustomScriptAssembly>();
      var assemblyLowercaseNamesLookup =
          new Dictionary<string, CustomScriptAssembly>();
      var exceptions = new List<Exception>();
      ...
      // To check if a path prefix is already being used we use a Dictionary
      // where the key is the prefix and the value is the file path.
      var prefixToFilePathLookup = customScriptAssemblyReferences.ToDictionary(
          x => x.PathPrefix, x => new List<string>(){x.FilePath},
          StringComparer.OrdinalIgnoreCase);

      ClearCompilationSetupErrorFlags(CompilationSetupErrorFlags.loadError);

      // Load first to setup guidsToAssemblies dictionary and convert guids to
      // assembly names before checking for assembly reference errors, so errors
      // emit assembly names instead of guids.
      for (var i = 0; i < paths.Length; ++i) {
        var path = paths[i];
        var guid = guids[i];
        ...
        try {
          ...
          if (contents != null) {
            ...
            loadedCustomScriptAssembly =
                LoadCustomScriptAssemblyFromJson(fullPath, jsonContents, guid);
          } else {
            loadedCustomScriptAssembly =
          ...
          guidsToAssemblies[Utility.FastToLower(guid)] =
              loadedCustomScriptAssembly;

          if (!skipCustomScriptAssemblyGraphValidation) {
            if (predefinedAssemblyNames == null) {
              predefinedAssemblyNames = new HashSet<string>(
                  EditorBuildRules.PredefinedTargetAssemblyNames);
              var net46 = MonoLibraryHelpers
                              .GetSystemLibraryReferences(
                                  ApiCompatibilityLevel.NET_4_6,
                                  ScriptCompilers.CSharpSupportedLanguage)
            ...
            if (predefinedAssemblyNames.Contains(
            ...
            if (assemblyLowercaseNamesLookup.TryGetValue(lowerCaseName,
                                                         out duplicate)) {
              var filePaths = new string[]{loadedCustomScriptAssembly.FilePath,
                                           duplicate.FilePath};
              var errorMsg =
                  string.Format("Assembly with name '{0}' already exists",
                                loadedCustomScriptAssembly.Name);
              loadedCustomScriptAssembly =
                  null; // Set to null to prevent it being added.
              throw new Compilation.AssemblyDefinitionException(errorMsg,
                                                                filePaths);
            }
            ...
            List<string> duplicateFilePaths;
            if (prefixToFilePathLookup.TryGetValue(
                    loadedCustomScriptAssembly.PathPrefix,
                    out duplicateFilePaths)) {
              var filePaths = new List<string>();
        ...
        if (loadedCustomScriptAssembly != null) {
          ...
          if (!skipCustomScriptAssemblyGraphValidation ||
              !assemblyLowercaseNamesLookup.ContainsKey(lowerCaseName)) {
            assemblyLowercaseNamesLookup[lowerCaseName] =
                loadedCustomScriptAssembly;
            ...
            if (!prefixToFilePathLookup.TryGetValue(
                    ...
                    out duplicateFilePaths)) {
              duplicateFilePaths = new List<string>();
              prefixToFilePathLookup[loadedCustomScriptAssembly.PathPrefix] =
                  duplicateFilePaths;
            }
      ...
      foreach (var assembly in assemblies) {
        for (int i = 0; i < assembly.References.Length; ++i) {
          ...
          var guid =
              Utility.FastToLower(GUIDReference.GUIDReferenceToGUID(reference));
      ...
      // Check loaded assemblies for assembly reference errors after all GUID
      // references have been converted to names.
      if (!skipCustomScriptAssemblyGraphValidation) {
        foreach (var loadedCustomScriptAssembly in assemblies) {
          try {
            var references = loadedCustomScriptAssembly.References.Where(
                r => !string.IsNullOrEmpty(r));

            if (references.Count() != references.Distinct().Count()) {
              var duplicateRefs = references.GroupBy(r => r)
                                      .Where(g => g.Count() > 1)
                                      .Select(g => g.Key)
                                      .ToArray();
              ...
              throw new Compilation.AssemblyDefinitionException(
                  string.Format("Assembly has duplicate references: {0}",
                                duplicateRefsString),
                  loadedCustomScriptAssembly.FilePath);
            }
          } catch (Exception e) {
    ...
    public bool IsPathInPackageDirectory(string path) {
      if (m_AssetPathsMetaData == null)
        return false;
      return m_AssetPathsMetaData.Any(
          p => path.StartsWith(p.DirectoryPath,
                               StringComparison.OrdinalIgnoreCase));
    }

    public void DeleteUnusedAssemblies() {
      ScriptAssemblySettings settings = CreateEditorScriptAssemblySettings(
    ...
    public void DeleteUnusedAssemblies(ScriptAssemblySettings settings) {
      string fullEditorAssemblyPath = AssetPath.Combine(
          projectDirectory, GetCompileScriptsOutputDirectory());

      if (!Directory.Exists(fullEditorAssemblyPath)) {
        // This is called in GetTargetAssembliesWithScripts and is required for
        // compilation to be set up correctly. Since we early out here, we need
        // to call this here.
        UpdateAllTargetAssemblyDefines(
            customTargetAssemblies,
            EditorBuildRules.GetPredefinedTargetAssemblies(),
            m_AllDistinctVersionMetaDatas, settings);
        return;
      }

      var deleteFiles = Directory.GetFiles(fullEditorAssemblyPath)
                            .Select(f => AssetPath.ReplaceSeparators(f))
                            .ToList();
      string timestampPath =
          GetAssemblyTimestampPath(GetCompileScriptsOutputDirectory());
    ...
    DeleteFile(string path,
               DeleteFileOptions fileOptions = DeleteFileOptions.LogError) {
      try {
        File.Delete(path);
      } catch (Exception) {
    ...
    static bool MoveOrReplaceFile(string sourcePath, string destinationPath) {
      ...
      if (!fileMoved) {
        ...
        DeleteFile(
            backupFile,
            DeleteFileOptions.NoLogError); // Delete any previous backup files.
    ...
    static string MDBPath(string dllPath) { return dllPath + ".mdb"; }
    ...
    public CustomScriptAssembly
    FindCustomScriptAssemblyFromAssemblyName(string assemblyName) {
      assemblyName = AssetPath.GetAssemblyNameWithoutExtension(assemblyName);

      if (customScriptAssemblies != null) {
        var result =
            customScriptAssemblies.FirstOrDefault(a => a.Name == assemblyName);
        if (result != null)
          return result;
      }

      var exceptionMessage =
          "Cannot find CustomScriptAssembly with name '" + assemblyName + "'.";

      if (customScriptAssemblies == null) {
        exceptionMessage += " customScriptAssemblies is null.";
      } else {
        var assemblyNames =
            customScriptAssemblies.Select(a => a.Name).ToArray();
    ...
    internal CustomScriptAssembly
    FindCustomScriptAssemblyFromScriptPath(string scriptPath) {
      var customTargetAssembly = EditorBuildRules.GetCustomTargetAssembly(
          scriptPath, projectDirectory, customTargetAssemblies);
      var customScriptAssembly = customTargetAssembly !=
                                 null ? FindCustomScriptAssemblyFromAssemblyName(
                                     customTargetAssembly.Filename)
          : null;
    ...
    internal CustomScriptAssembly
    FindCustomTargetAssemblyFromTargetAssembly(TargetAssembly assembly) {
      var assemblyName =
          AssetPath.GetAssemblyNameWithoutExtension(assembly.Filename);
    ...
    public CustomScriptAssembly
    FindCustomScriptAssemblyFromAssemblyReference(string reference) {
      if (!GUIDReference.IsGUIDReference(reference)) {
      ...
      if (customScriptAssemblies != null) {
        ...
        var result = customScriptAssemblies.FirstOrDefault(
            a => string.Equals(a.GUID, guid,
                               StringComparison.OrdinalIgnoreCase));
      ...
      throw new InvalidOperationException(
          $"Cannot find CustomScriptAssembly with reference '{reference}'");
    }

    public CompileStatus CompileScripts(EditorScriptCompilationOptions options,
                                        BuildTargetGroup platformGroup,
                                        BuildTarget platform,
                                        string[] extraScriptingDefines) {
      var scriptAssemblySettings = CreateScriptAssemblySettings(
          platformGroup, platform, options, extraScriptingDefines);

      TargetAssembly[] notCompiledTargetAssemblies = null;
      string[] notCompiledScripts = null;

      var result = CompileScripts(scriptAssemblySettings, EditorTempPath,
                                  options, ref notCompiledTargetAssemblies,
                                  ref notCompiledScripts);

      if (notCompiledTargetAssemblies != null) {
        foreach (var targetAssembly in notCompiledTargetAssemblies) {
          var customScriptAssembly = customScriptAssemblies.Single(
              a => a.Name == AssetPath.GetAssemblyNameWithoutExtension(
                                 targetAssembly.Filename));
          ...
          UnityEngine.Debug.LogWarning(string.Format(
              "Script assembly '{0}' has not been compiled. Folder containing assembly definition file '{1}' contains script files for different script languages. Folder must only contain script files for one script language.",
              targetAssembly.Filename, filePath));
        }
      }
      ...
      if (notCompiledScripts != null) {
        ...
        foreach (var script in notCompiledScripts) {
          UnityEngine.Debug.LogWarning(string.Format(
              "Script '{0}' will not be compiled because it exists outside the Assets folder and does not to belong to any assembly definition file.",
              script));
        }
      }
    ...
    private static TargetAssembly[] GetPredefinedAssemblyReferences(
        IDictionary<string, TargetAssembly> targetAssemblies) {
      var targetAssembliesResult =
          (targetAssemblies.Values ?? Enumerable.Empty<TargetAssembly>())
              .Where(x =>(x.Flags & AssemblyFlags.ExplicitlyReferenced) ==
                         AssemblyFlags.None)
              .ToArray();
      return targetAssembliesResult;
    }

    internal CompileStatus CompileScripts(
        ScriptAssemblySettings scriptAssemblySettings,
        string tempBuildDirectory, EditorScriptCompilationOptions options,
      ...
      var assemblies = new EditorBuildRules.CompilationAssemblies{
          ...
          PrecompiledAssemblies =
              PrecompiledAssemblyProvider.GetPrecompiledAssembliesDictionary(
                  ...
                  scriptAssemblySettings.BuildTargetGroup,
                  scriptAssemblySettings.BuildTarget),
          CustomTargetAssemblies = customTargetAssemblies,
          PredefinedAssembliesCustomTargetReferences =
              GetPredefinedAssemblyReferences(customTargetAssemblies),
          EditorAssemblyReferences =
              ModuleUtils.GetAdditionalReferencesForUserScripts(),
      };

      var allDirtyScripts =
          (areAllScriptsDirty || areAllPrecompiledAssembliesDirty)
              ? allScripts
              : dirtyScripts;
      ...
      ScriptAssembly[] scriptAssemblies =
          EditorBuildRules.GenerateChangedScriptAssemblies(args);

      foreach (var customTargetAssembly in args
                   .NoScriptsCustomTargetAssemblies) {
        var customScriptAssembly =
            FindCustomTargetAssemblyFromTargetAssembly(customTargetAssembly);
      ...
      // If only the last script of an assembly is removed, then
      // scriptAssemblies will be empty and dirtyTargetAssemblies will be not
      // empty. Or if only an explicitly referenced precompiled assembly without
      // any references is marked as dirty, then scriptAssemblies will be empty
      // and dirtyPrecompiledAssemblies will not be dirty. Then we should delete
      // unused assemblies and clear compilationTask and retun
      // CompileStatus.CompilationComplete to indicate to native that
      // compilation was successful and that the assemblies should be reloaded.
      // If the last script of assembly was removed or precompiled assemblies
      // are dirty along with modifying other scripts, then scriptAssemblies
      // will not be empty and we will recompile scripts and then reload
      // assemblies if compilation is successful.
      bool returnCompilationComplete =
          scriptAssemblies.Length == 0 &&
      ...
      bool compiling = CompileScriptAssemblies(
          scriptAssemblies, scriptAssemblySettings, tempBuildDirectory, options,
    ...
    internal bool
    CompileCustomScriptAssemblies(EditorScriptCompilationOptions options,
                                  BuildTargetGroup platformGroup,
                                  BuildTarget platform) {
      var scriptAssemblySettings =
          CreateScriptAssemblySettings(platformGroup, platform, options);
      return CompileCustomScriptAssemblies(scriptAssemblySettings,
                                           EditorTempPath, options,
                                           platformGroup, platform);
    }

    internal bool CompileCustomScriptAssemblies(
        ScriptAssemblySettings scriptAssemblySettings,
        string tempBuildDirectory, EditorScriptCompilationOptions options,
        BuildTargetGroup platformGroup, BuildTarget platform) {
      DeleteUnusedAssemblies();
      var scriptAssemblies = GetAllScriptAssembliesOfType(
          scriptAssemblySettings, TargetAssemblyType.Custom);

      return CompileScriptAssemblies(
          scriptAssemblies, scriptAssemblySettings, tempBuildDirectory, options,
          CompilationTaskOptions.None,
          CompileScriptAssembliesOptions.skipSetupChecks);
    }

    internal bool CompileScriptAssemblies(
        ScriptAssembly[] scriptAssemblies,
        ScriptAssemblySettings scriptAssemblySettings,
        string tempBuildDirectory, EditorScriptCompilationOptions options,
      ...
      bool skipSetupChecks = (compileScriptAssembliesOptions &
                              CompileScriptAssembliesOptions.skipSetupChecks) ==
                             CompileScriptAssembliesOptions.skipSetupChecks;

      // Skip setup checks when compiling custom script assemblies on startup,
      // as we only load the ones that been compiled and have all their
      ...
      var scriptCodegenAssemblies =
          UnityCodeGenHelpers.ToScriptCodeGenAssemblies(scriptAssemblies);
      ...
      if (scriptAssembliesCodegen.Any()) {
        scriptAssembliesCodegen =
            scriptAssembliesCodegen
                .Where(a => a.DirtySource != DirtySource.DirtyReference)
                .ToList();
      }
      ...
      // 3. Fix compile error in A, also clears compile errors for B due to
      // dependency.
      ...
      compilationTask = new CompilationTask(
          ...
          scriptAssembliesCodegen.ToArray(), tempBuildDirectory,
          "Editor Compilation", options, compilationTaskOptions,
          maxConcurrentCompilers, ILPostProcessing, compilerFactory);
      ...
      compilationTask.OnCompilationTaskFinished += (context) => {
        ...
        Console.WriteLine(
            $"- Finished script compilation in {elapsed.TotalSeconds:0.######} seconds");
      };
      ...
      compilationTask.OnCompilationStarted += (assembly, phase) => {
        var stopwatch = new Stopwatch();
        stopwatch.Start();
        var assemblyOutputPath = AssetPath.Combine(
            scriptAssemblySettings.OutputDirectory, assembly.Filename);
      ...
      compilationTask.OnCompilationFinished += (assembly, messages) => {
        ...
        if (!CopyAssembly(
                AssetPath.Combine(tempBuildDirectory, assembly.Filename),
                assembly.FullPath)) {
          messages.Add(new CompilerMessage{
              message = string.Format(
                  "Copying assembly from '{0}' to '{1}' failed",
                  AssetPath.Combine(tempBuildDirectory, assembly.Filename),
                  assembly.FullPath),
              type = CompilerMessageType.Error, file = assembly.FullPath,
              line = -1, column = -1});
          StopCompilationTask();
          InvokeAssemblyCompilationFinished(assembly, messages, options);
          return;
        ...
        try {
          ...
          var elapsed = stopwatch.Elapsed;
          Console.WriteLine(
              $"- Finished compile {assemblyOutputPath} in {elapsed.TotalSeconds:0.######} seconds");
    ...
    static void RunScriptUpdater(ScriptAssembly assembly,
                                 string tempBuildDirectory,
                                 EditorScriptCompilationOptions options) {
      assembly.GeneratedResponseFile =
          MicrosoftCSharpCompiler.GenerateResponseFile(assembly,
                                                       tempBuildDirectory);

      APIUpdaterHelper.UpdateScripts(assembly.GeneratedResponseFile, "cs",
                                     assembly.Files);
    }

    static void TransferHasCompileErrors(ScriptAssembly[] sourceAssemblies,
                                         ScriptAssembly[] destAssemblies) {
      var scriptAssemblyHasCompileErrors =
          new Dictionary<string, bool>(sourceAssemblies.Length);

      foreach (var lastScriptAssembly in sourceAssemblies) {
        scriptAssemblyHasCompileErrors[lastScriptAssembly.Filename] =
            lastScriptAssembly.HasCompileErrors;
      }

      foreach (var currentScriptAssembly in destAssemblies) {
        bool hasCompileErrors = false;

        scriptAssemblyHasCompileErrors.TryGetValue(
            currentScriptAssembly.Filename, out hasCompileErrors);

        currentScriptAssembly.HasCompileErrors = hasCompileErrors;
      }
    }

    void AddUnitySpecificErrorMessages(ScriptAssembly assembly,
    ...
    public void InvokeAssemblyCompilationFinished(
        ScriptAssembly assembly, List<CompilerMessage> messages,
        EditorScriptCompilationOptions scriptCompilationOptions) {
      if (assemblyCompilationFinished != null) {
        var convertedMessages = ConvertCompilerMessages(messages);
        assemblyCompilationFinished(assembly, convertedMessages,
    ...
    public bool AreAllScriptsDirty() { return areAllScriptsDirty; }

    public bool ArePrecompiledAssembliesDirty() {
      return areAllPrecompiledAssembliesDirty ||
             dirtyPrecompiledAssemblies.Count > 0;
    }
    ...
    public bool DoesProjectHaveAnyCustomScriptAssemblies() {
      foreach (var entry in allScripts) {
        ...
        var targetAssembly = EditorBuildRules.GetTargetAssembly(
            script, assemblyFilename, projectDirectory, customTargetAssemblies);
    ...
    CreateScriptAssemblySettings(BuildTargetGroup buildTargetGroup,
                                 BuildTarget buildTarget,
                                 EditorScriptCompilationOptions options) {
      return CreateScriptAssemblySettings(buildTargetGroup, buildTarget,
                                          options, new string[]{});
    }
    ...
    CreateScriptAssemblySettings(BuildTargetGroup buildTargetGroup,
      ...
      if ((options & EditorScriptCompilationOptions
                         .BuildingPredefinedAssembliesAllowUnsafeCode) ==
          EditorScriptCompilationOptions
              .BuildingPredefinedAssembliesAllowUnsafeCode)
        predefinedAssembliesCompilerOptions.AllowUnsafeCode = true;

      if ((options & EditorScriptCompilationOptions
                         .BuildingUseDeterministicCompilation) ==
      ...
      predefinedAssembliesCompilerOptions.ApiCompatibilityLevel =
      ...
      if ((options & EditorScriptCompilationOptions.BuildingForEditor) == 0) {
        compilationExtension = ModuleManager.FindPlatformSupportModule(
            ModuleManager.GetTargetStringFromBuildTarget(buildTarget))
            ?.CreateCompilationExtension();
      }
    ...
    ScriptAssemblySettings
    CreateEditorScriptAssemblySettings(EditorScriptCompilationOptions options) {
      return CreateScriptAssemblySettings(
    ...
    public AssemblyCompilerMessages[] GetCompileMessages() {
      ...
      var result =
          new AssemblyCompilerMessages[compilationTask.CompilerMessages.Count];
      ...
      foreach (var entry in compilationTask.CompilerMessages) {
        ...
        result[index++] = new AssemblyCompilerMessages{
            assemblyFilename = assembly.Filename, messages = messages};
      }
    ...
    public bool IsCompilationPending() {
      // If there were any errors in setting up the compilation, then return
      // false.
      if (setupErrorFlags != CompilationSetupErrorFlags.none)
        return false;

      // If we have dirty scripts or script updater has marked assemblies for
      // updated, then compilation will trigger on next TickCompilationPipeline.
    ...
    public bool IsCompiling() {
      // Native code expects IsCompiling to be true after marking scripts as
      // dirty, therefore return true if the compilation is pending
      return IsCompilationTaskCompiling() || IsCompilationPending() ||
             IsAnyAssemblyBuilderCompiling();
    }
    ...
    public CompileStatus TickCompilationPipeline(
        EditorScriptCompilationOptions options, BuildTargetGroup platformGroup,
        BuildTarget platform, string[] extraScriptingDefines) {
      ...
      // If we are not currently compiling and there are dirty scripts, start
      // compilation.
      if (!IsCompilationTaskCompiling() && IsCompilationPending()) {
        Profiler.BeginSample("CompilationPipeline.CompileScripts");
        CompileStatus compileStatus = CompileScripts(
            options, platformGroup, platform, extraScriptingDefines);
        Profiler.EndSample();
    ...
    public CompileStatus PollCompilation() {
      if (IsCompilationTaskCompiling()) {
        if (compilationTask.Poll()) // Returns true when compilation finished.
          return (compilationTask == null || compilationTask.CompileErrors)
    ...
    public TargetAssemblyInfo[] GetTargetAssemblies() {
      TargetAssembly[] predefindTargetAssemblies =
          EditorBuildRules.GetPredefinedTargetAssemblies();

      TargetAssemblyInfo[] targetAssemblyInfo =
          new TargetAssemblyInfo[predefindTargetAssemblies.Length +
                                 (customTargetAssemblies !=
                                  null ? customTargetAssemblies.Count : 0)];

      for (int i = 0; i < predefindTargetAssemblies.Length; ++i)
        targetAssemblyInfo [i]
        = ToTargetAssemblyInfo(predefindTargetAssemblies[i]);
      ...
      if (customTargetAssemblies != null) {
        ...
        foreach (var entry in customTargetAssemblies) {
          var customTargetAssembly = entry.Value;
          targetAssemblyInfo [i]
          = ToTargetAssemblyInfo(customTargetAssembly);
          i++;
        }
      }
    ...
    public TargetAssemblyInfo[] GetTargetAssembliesWithScripts(
        EditorScriptCompilationOptions options) {
      ScriptAssemblySettings settings = CreateEditorScriptAssemblySettings(
          EditorScriptCompilationOptions.BuildingForEditor | options);
    ...
    public TargetAssemblyInfo[] GetTargetAssembliesWithScripts(
      ...
      var targetAssemblies = EditorBuildRules.GetTargetAssembliesWithScripts(
          allScripts, projectDirectory, customTargetAssemblies, settings);

      var targetAssemblyInfos = new TargetAssemblyInfo[targetAssemblies.Length];

      for (int i = 0; i < targetAssemblies.Length; ++i)
        targetAssemblyInfos [i]
        = ToTargetAssemblyInfo(targetAssemblies[i]);

      return targetAssemblyInfos;
    }

    public HashSet<TargetAssembly> GetTargetAssembliesWithScriptsHashSet(
        EditorScriptCompilationOptions options) {
      ScriptAssemblySettings settings = CreateEditorScriptAssemblySettings(
          EditorScriptCompilationOptions.BuildingForEditor | options);
    ...
    public ScriptAssembly[] GetAllScriptAssembliesForLanguage<T>(
        EditorScriptCompilationOptions additionalOptions) where T
        : SupportedLanguage {
      var assemblies =
          GetAllScriptAssemblies(
              EditorScriptCompilationOptions.BuildingForEditor, null)
              .Where(a => a.Language.GetType() == typeof(T))
              .ToArray();
      return assemblies;
    }

    public ScriptAssembly GetScriptAssemblyForLanguage<T>(
        string assemblyNameOrPath,
        EditorScriptCompilationOptions additionalOptions) where T
        : SupportedLanguage {
      var assemblyName =
          AssetPath.GetAssemblyNameWithoutExtension(assemblyNameOrPath);
      var scriptAssemblies =
          GetAllScriptAssembliesForLanguage<T>(additionalOptions);
      return scriptAssemblies.SingleOrDefault(
          a => String.Compare(
                   assemblyName,
                   AssetPath.GetAssemblyNameWithoutExtension(a.Filename),
    ...
    public TargetAssemblyInfo GetTargetAssembly(string scriptPath) {
      TargetAssembly targetAssembly =
          EditorBuildRules.GetTargetAssemblyLinearSearch(
              scriptPath, projectDirectory, customTargetAssemblies);

      TargetAssemblyInfo targetAssemblyInfo =
          ToTargetAssemblyInfo(targetAssembly);
      return targetAssemblyInfo;
    }

    public TargetAssembly GetTargetAssemblyDetails(string scriptPath) {
      return EditorBuildRules.GetTargetAssemblyLinearSearch(
          scriptPath, projectDirectory, customTargetAssemblies);
    }

    public ScriptAssembly[] GetAllEditorScriptAssemblies(
        EditorScriptCompilationOptions additionalOptions) {
      return GetAllScriptAssemblies(
          EditorScriptCompilationOptions.BuildingForEditor |
              EditorScriptCompilationOptions.BuildingIncludingTestAssemblies |
              additionalOptions,
          null);
    }

    public ScriptAssembly[] GetAllEditorScriptAssemblies(
        EditorScriptCompilationOptions additionalOptions, string[] defines) {
      return GetAllScriptAssemblies(
          EditorScriptCompilationOptions.BuildingForEditor |
              EditorScriptCompilationOptions.BuildingIncludingTestAssemblies |
              additionalOptions,
    ...
    public ScriptAssembly[] GetAllScriptAssemblies(
        EditorScriptCompilationOptions options, string[] defines) {
      var isForEditor =
          (options & EditorScriptCompilationOptions.BuildingForEditor) ==
          EditorScriptCompilationOptions.BuildingForEditor;
      var precompiledAssemblies =
          PrecompiledAssemblyProvider.GetPrecompiledAssembliesDictionary(
              isForEditor, EditorUserBuildSettings.activeBuildTargetGroup,
              EditorUserBuildSettings.activeBuildTarget);
      return GetAllScriptAssemblies(options, unityAssemblies,
                                    precompiledAssemblies, defines);
    }
    ...
    public ScriptAssembly[] GetAllScriptAssemblies(
      ...
      return GetAllScriptAssemblies(settings, unityAssembliesArg,
                                    precompiledAssembliesArg, defines);
    }
    ...
    public ScriptAssembly[] GetAllScriptAssemblies(
      ...
      return EditorBuildRules.GetAllScriptAssemblies(
          allScripts, projectDirectory, settings, assemblies,
          runScriptUpdaterAssemblies);
    }

    // TODO: Get rid of calls to this method and ensure that the defines are
    // always setup correctly at all times.
    private static void UpdateAllTargetAssemblyDefines(
        IDictionary<string, TargetAssembly> customScriptAssemblies,
        TargetAssembly[] predefinedTargetAssemblies,
        Dictionary<string, string> assetPathVersionMetaDatas,
        ScriptAssemblySettings settings) {
      var allTargetAssemblies = customScriptAssemblies.Values.ToArray().Concat(
          predefinedTargetAssemblies ?? new TargetAssembly[0]);

      var semVersionRangesFactory = new SemVersionRangesFactory();

      string[] editorOnlyCompatibleDefines = null;

      editorOnlyCompatibleDefines = InternalEditorUtility.GetCompilationDefines(
          settings.CompilationOptions, settings.BuildTargetGroup,
          settings.BuildTarget, ApiCompatibilityLevel.NET_4_6);

      var playerAssembliesDefines = InternalEditorUtility.GetCompilationDefines(
          settings.CompilationOptions, settings.BuildTargetGroup,
          settings.BuildTarget,
          settings.PredefinedAssembliesCompilerOptions.ApiCompatibilityLevel);

      foreach (var targetAssembly in allTargetAssemblies) {
        SetTargetAssemblyDefines(
            targetAssembly, semVersionRangesFactory, assetPathVersionMetaDatas,
            editorOnlyCompatibleDefines, playerAssembliesDefines, settings);
      }
    }

    private static void SetTargetAssemblyDefines(
        TargetAssembly targetAssembly,
        SemVersionRangesFactory semVersionRangesFactory,
        Dictionary<string, string> assetPathVersionMetaDatas,
        string[] editorOnlyCompatibleDefines, string[] playerAssembliesDefines,
        ScriptAssemblySettings settings) {
      ...
      if ((targetAssembly.Flags & AssemblyFlags.EditorOnly) ==
      ...
      string[] defines = new string[compilationDefines.Length +
                                    targetAssembly.VersionDefines.Count +
                                    settingsExtraGeneralDefines.Length];

      Array.Copy(settingsExtraGeneralDefines, defines,
                 settingsExtraGeneralDefines.Length);
      populatedVersionDefinesCount += settingsExtraGeneralDefines.Length;
      Array.Copy(compilationDefines, 0, defines, populatedVersionDefinesCount,
                 compilationDefines.Length);
      ...
      for (int i = 0; i < targetAssemblyVersionDefines.Count; i++) {
        ...
        if (!assetPathVersionMetaDatas.ContainsKey(
                targetAssemblyVersionDefine.name)) {
          continue;
        }
        ...
        try {
          var versionDefineExpression = semVersionRangesFactory.GetExpression(
              targetAssemblyVersionDefine.expression);
          var assetPathVersionMetaData =
              assetPathVersionMetaDatas[targetAssemblyVersionDefine.name];
          var semVersion = SemVersionParser.Parse(assetPathVersionMetaData);
          if (versionDefineExpression.IsValid(semVersion)) {
            defines[populatedVersionDefinesCount] =
                targetAssemblyVersionDefine.define;
            ++populatedVersionDefinesCount;
          }
        } catch (Exception e) {
          var asset = AssetDatabase.LoadAssetAtPath<AssemblyDefinitionAsset>(
              EditorCompilationInterface.Instance
                  .FindCustomTargetAssemblyFromTargetAssembly(targetAssembly)
                  .FilePath);
    ...
    ScriptAssembly[] GetAllScriptAssembliesOfType(
        ScriptAssemblySettings settings, TargetAssemblyType type) {
      var precompiledAssemblies =
          PrecompiledAssemblyProvider.GetPrecompiledAssembliesDictionary(
              settings.BuildingForEditor, settings.BuildTargetGroup,
              settings.BuildTarget);
      ...
      return EditorBuildRules.GetAllScriptAssemblies(
          allScripts, projectDirectory, settings, assemblies,
          runScriptUpdaterAssemblies, type);
    }

    public MonoIsland[] GetAllMonoIslands(
        EditorScriptCompilationOptions additionalOptions) {
      bool isEditor = (additionalOptions &
                       EditorScriptCompilationOptions.BuildingForEditor) ==
                      EditorScriptCompilationOptions.BuildingForEditor;
      var precompiledAssemblies =
          PrecompiledAssemblyProvider.GetPrecompiledAssembliesDictionary(
              isEditor, EditorUserBuildSettings.activeBuildTargetGroup,
              EditorUserBuildSettings.activeBuildTarget);
      return GetAllMonoIslands(
          unityAssemblies, precompiledAssemblies,
          EditorScriptCompilationOptions.BuildingForEditor |
              EditorScriptCompilationOptions.BuildingIncludingTestAssemblies |
              additionalOptions);
    }
    ...
    public MonoIsland[] GetAllMonoIslands(
        ...
        Dictionary<string, PrecompiledAssembly> precompiledAssembliesArg,
        EditorScriptCompilationOptions options) {
      var scriptAssemblies = GetAllScriptAssemblies(
          options, unityAssembliesArg, precompiledAssembliesArg, null);
      var monoIslands = new MonoIsland[scriptAssemblies.Length];

      for (int i = 0; i < scriptAssemblies.Length; ++i)
        monoIslands[i] =
            scriptAssemblies [i]
                .ToMonoIsland(EditorScriptCompilationOptions.BuildingForEditor,
                              EditorTempPath);
    ...
    public bool IsRuntimeScriptAssembly(string assemblyNameOrPath) {
      ...
      var predefinedAssemblyTargets =
          EditorBuildRules.GetPredefinedTargetAssemblies();

      if (predefinedAssemblyTargets.Any(
              a =>((a.Flags & AssemblyFlags.EditorOnly) !=
                   AssemblyFlags.EditorOnly) &&
                  a.Filename == assemblyFilename))
        return true;

      if (customTargetAssemblies != null &&
          customTargetAssemblies.Any(
              a =>((a.Value.Flags & AssemblyFlags.EditorOnly) !=
                   AssemblyFlags.EditorOnly) &&
                  a.Value.Filename == assemblyFilename))
        return true;
    ...
    static EditorScriptCompilationOptions
    ToEditorScriptCompilationOptions(Compilation.AssemblyBuilderFlags flags) {
      EditorScriptCompilationOptions options =
          EditorScriptCompilationOptions.BuildingEmpty;

      if ((flags & Compilation.AssemblyBuilderFlags.DevelopmentBuild) ==
          Compilation.AssemblyBuilderFlags.DevelopmentBuild)
        options |= EditorScriptCompilationOptions.BuildingDevelopmentBuild;

      if ((flags & Compilation.AssemblyBuilderFlags.EditorAssembly) ==
    ...
    static AssemblyFlags
    ToAssemblyFlags(Compilation.AssemblyBuilderFlags assemblyBuilderFlags) {
      AssemblyFlags assemblyFlags = AssemblyFlags.None;

      if ((assemblyBuilderFlags &
           Compilation.AssemblyBuilderFlags.EditorAssembly) ==
          Compilation.AssemblyBuilderFlags.EditorAssembly)
    ...
    static EditorBuildRules.UnityReferencesOptions
    ToUnityReferencesOptions(ReferencesOptions options) {
      var result = EditorBuildRules.UnityReferencesOptions.ExcludeModules;

      if ((options & ReferencesOptions.UseEngineModules) ==
    ...
    ScriptAssembly InitializeScriptAssemblyWithoutReferencesAndDefines(
        Compilation.AssemblyBuilder assemblyBuilder) {
      var scriptFiles = assemblyBuilder.scriptPaths
                            .Select(p => AssetPath.Combine(projectDirectory, p))
                            .ToArray();
      var assemblyPath =
          AssetPath.Combine(projectDirectory, assemblyBuilder.assemblyPath);
      ...
      scriptAssembly.Language = ScriptCompilers.GetLanguageFromExtension(
          ScriptCompilers.GetExtensionOfSourceFile(
              assemblyBuilder.scriptPaths[0]));
      ...
      scriptAssembly.CompilerOptions.ApiCompatibilityLevel =
          PlayerSettings.GetApiCompatibilityLevel(
              assemblyBuilder.buildTargetGroup);
    ...
    CreateScriptAssembly(Compilation.AssemblyBuilder assemblyBuilder) {
      var scriptAssembly =
          InitializeScriptAssemblyWithoutReferencesAndDefines(assemblyBuilder);

      var options = ToEditorScriptCompilationOptions(assemblyBuilder.flags);
      var referencesOptions =
          ToUnityReferencesOptions(assemblyBuilder.referencesOptions);

      var references = GetAssemblyBuilderDefaultReferences(
          scriptAssembly, options, referencesOptions);

      if (assemblyBuilder.additionalReferences != null &&
          assemblyBuilder.additionalReferences.Length > 0)
        references =
            references.Concat(assemblyBuilder.additionalReferences).ToArray();

      if (assemblyBuilder.excludeReferences != null &&
          assemblyBuilder.excludeReferences.Length > 0)
        references =
            references
                .Where(r => !assemblyBuilder.excludeReferences.Contains(r))
                .ToArray();
    ...
    string[] GetAssemblyBuilderDefaultReferences(
        ScriptAssembly scriptAssembly, EditorScriptCompilationOptions options,
        EditorBuildRules.UnityReferencesOptions unityReferencesOptions) {
      bool buildingForEditor =
          (scriptAssembly.Flags & AssemblyFlags.EditorOnly) ==
          AssemblyFlags.EditorOnly;

      var monolithicEngineAssemblyPath =
          InternalEditorUtility.GetMonolithicEngineAssemblyPath();

      var unityReferences = EditorBuildRules.GetUnityReferences(
          scriptAssembly, unityAssemblies, options, unityReferencesOptions);

      var customReferences =
          EditorBuildRules.GetCompiledCustomAssembliesReferences(
              scriptAssembly, customTargetAssemblies,
              GetCompileScriptsOutputDirectory());

      var precompiledAssemblies =
          PrecompiledAssemblyProvider.GetPrecompiledAssemblies(
              buildingForEditor, EditorUserBuildSettings.activeBuildTargetGroup,
              EditorUserBuildSettings.activeBuildTarget);
      var precompiledReferences = EditorBuildRules.GetPrecompiledReferences(
          scriptAssembly, TargetAssemblyType.Custom, options,
          EditorCompatibility.CompatibleWithEditor, precompiledAssemblies);
      var additionalReferences = MonoLibraryHelpers.GetSystemLibraryReferences(
          scriptAssembly.CompilerOptions.ApiCompatibilityLevel,
          scriptAssembly.Language);
      string[] editorReferences =
          buildingForEditor ? ModuleUtils.GetAdditionalReferencesForUserScripts()
          : new string[0];

      var references = new List<string>();

      if (unityReferencesOptions ==
          EditorBuildRules.UnityReferencesOptions.ExcludeModules)
        references.Add(monolithicEngineAssemblyPath);

      references.AddRange(unityReferences.Concat(customReferences)
                              .Concat(precompiledReferences)
                              .Concat(editorReferences)
                              .Concat(additionalReferences));

      return references.ToArray();
    }

    public string[] GetAssemblyBuilderDefaultReferences(
    ...
    public string[] GetAssemblyBuilderDefaultDefines(
        AssemblyBuilder assemblyBuilder) {
      var options = ToEditorScriptCompilationOptions(assemblyBuilder.flags);
      var defines = InternalEditorUtility.GetCompilationDefines(
          options, assemblyBuilder.buildTargetGroup,
          assemblyBuilder.buildTarget);
      return defines;
    }

    public void AddAssemblyBuilder(
        UnityEditor.Compilation.AssemblyBuilder assemblyBuilder) {
      assemblyBuilders.Add(assemblyBuilder);
    }

    public static UnityEditor.Compilation
        .CompilerMessage[] ConvertCompilerMessages(
            List<CompilerMessage> messages) {
      var newMessages =
          new UnityEditor.Compilation.CompilerMessage[messages.Count];
