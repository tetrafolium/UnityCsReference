...
[Flags] enum CompilationTaskOptions { None = 0, StopOnFirstError = (1 << 0) }

// CompilationTask represents one complete rebuild of all the ScriptAssembly's
// that are passed in the constructor. The ScriptAssembly's are built in
// correct order according to their ScriptAssembly dependencies.
class CompilationTask : IDisposable {
enum CompilionTaskState { Started = 0, Running = 1, Finished = 2 }

List<ScriptAssembly> pendingAssemblies;
HashSet<ScriptAssembly> codeGenAssemblies;
HashSet<ScriptAssembly> compiledCodeGenAssemblies =
	new HashSet<ScriptAssembly>();
HashSet<ScriptAssembly> notCompiledCodeGenAssemblies =
	new HashSet<ScriptAssembly>();
Dictionary<ScriptAssembly, CompilerMessage[]> compiledAssemblies =
	new Dictionary<ScriptAssembly, CompilerMessage[]>();
Dictionary<ScriptAssembly, CompilerMessage[]> processedAssemblies =
	new Dictionary<ScriptAssembly, CompilerMessage[]>();
Dictionary<ScriptAssembly, ScriptCompilerBase> compilerTasks =
	new Dictionary<ScriptAssembly, ScriptCompilerBase>();
HashSet<ScriptAssembly> unchangedCompiledAssemblies =
	new HashSet<ScriptAssembly>();
List<PostProcessorTask> postProcessorTasks = new List<PostProcessorTask>();
List<PostProcessorTask> pendingPostProcessorTasks =
	new List<PostProcessorTask>();
...
public event Action<ScriptAssembly, List<CompilerMessage> >
...
bool BuildingForEditor {
	get {
		return (options & EditorScriptCompilationOptions.BuildingForEditor) ==
...
bool UseReferenceAssemblies {
	get {
		return (options & EditorScriptCompilationOptions
		        .BuildingUseReferenceAssemblies) ==
...
public CompilationTask(ScriptAssembly[] scriptAssemblies,
                       ...
                       string buildOutputDirectory, object context,
                       EditorScriptCompilationOptions options,
                       CompilationTaskOptions compilationTaskOptions,
                       int maxConcurrentCompilers,
                       IILPostProcessing ilPostProcessing,
...
public ScriptAssembly[] ScriptAssemblies {
	get { return scriptAssemblies; }
}

public HashSet<ScriptAssembly> CodeGenAssemblies {
	get { return codeGenAssemblies; }
}

public bool AreAllCodegenAssembliesCompiled {
	get { return codeGenAssemblies.Count == compiledCodeGenAssemblies.Count; }
}
...
public bool Poll() {
	...
	Dictionary<ScriptAssembly, ScriptCompilerBase> finishedCompilerTasks =
		null;
	...
	// Save compiler messages from finished compiler processes and check for
	// compile errors.
	if (finishedCompilerTasks != null)
		foreach (var task in finishedCompilerTasks) {
			...
			assembly.HasCompileErrors =
				messagesList.Any(m => m.type == CompilerMessageType.Error);
			compiledAssemblies.Add(assembly, messagesList.ToArray());

			bool havePostProcessors =
				ilPostProcessing != null && ilPostProcessing.HasPostProcessors;
			...
			if (havePostProcessors && notCompiledCodeGenAssemblies.Count == 0 &&
			    !hasCompileErrors && !isCodeGenAssembly) {
				var assemblySourcePath =
					AssetPath.Combine(buildOutputDirectory, assembly.Filename);
				var pdbSourcePath =
					AssetPath.Combine(buildOutputDirectory, assembly.PdbFilename);
				...
				try {
					...
					var postProcessorTask =
						new PostProcessorTask(assembly, messagesList,
						                      buildOutputDirectory, ilPostProcessing);
					pendingPostProcessorTasks.Add(postProcessorTask);
				} catch (IOException e) {
					UnityEngine.Debug.LogError(
						$"Fail to copy {assemblySourcePath} or {pdbSourcePath} to {AssetPath.GetDirectoryName(assembly.FullPath)} before post processing the assembly. Skipping post processing.\n{e}");
					// OnCompilationFinished callbacks might add more compiler
					// messages
			...
			try {
				var referenceAssemblyPath = AssetPath.Combine(
					buildOutputDirectory, assembly.ReferenceAssemblyFilename);

				// If a reference assembly is built, check if it is unchanged
				// since the previous compilation.
				if (UseReferenceAssemblies && BuildingForEditor &&
				    File.Exists(referenceAssemblyPath)) {
					Profiler.BeginSample(
						"ReferenceAssemblyHelpers.IsReferenceAssemblyUnchanged");
					bool isUnchanged =
						ReferenceAssemblyHelpers.IsReferenceAssemblyUnchanged(
							assembly, buildOutputDirectory);
					Profiler.EndSample();
	...
	// If StopOnFirstError is set, do not queue assemblies for compilation in
	// case of compile errors.
	bool stopOnFirstError =
		(compilationTaskOptions & CompilationTaskOptions.StopOnFirstError) ==
	...
	// Queue pending assemblies for compilation if we have no running
	// compilers or if compilers have finished.
	if (compilerTasks.Count == 0 ||
	    (finishedCompilerTasks != null && finishedCompilerTasks.Count > 0))
...
void PollPostProcessors() {
	...
	foreach (var postProcessorTask in pendingPostProcessorTasks) {
		...
		LogStartInfo($"# Starting IL post processing on {assembly.Filename}",
		             postProcessorTask.GetProcessStartInfo());
	...
	foreach (var postProcessorTask in postProcessorTasks) {
		...
		processedAssemblies.Add(postProcessorTask.Assembly,
...
bool RunningMaxConcurrentProcesses {
	get {
		return (compilerTasks.Count + postProcessorTasks.Count) >=
...
void QueuePendingAssemblies() {
	...
	foreach (var pendingAssembly in pendingAssemblies) {
		...
		foreach (var reference in pendingAssembly.ScriptAssemblyReferences) {
			...
			if (!compiledAssemblies.TryGetValue(reference, out messages)) {
				...
				if (!compilerTasks.ContainsKey(reference) &&
				    !pendingAssemblies.Contains(reference)) {
			...
			// If reference has compile errors, do not compile the pending
			// assembly.
			bool compileErrors =
				messages.Any(m => m.type == CompilerMessageType.Error);
		...
		if (UseReferenceAssemblies && BuildingForEditor &&
		    ...
		    unchangedAssemblyReferencesCount ==
		    pendingAssembly.ScriptAssemblyReferences.Length &&
		    File.Exists(pendingAssembly.FullPath)) {
			var assemblyOutputPath = AssetPath.Combine(
				pendingAssembly.OutputDirectory, pendingAssembly.Filename);

			Console.WriteLine(
				$"- Skipping compile {assemblyOutputPath} because all references are unchanged");
	...
	// No assemblies to compile, need to wait for more references to finish
	// compiling.
	if (assemblyCompileQueue == null) {
		if (compilerTasks.Count == 0)
			throw new Exception(
				      "No pending assemblies queued for compilation and no compilers running. Compilation will never finish.");
		return;
	}
	...
	foreach (var assembly in assemblyCompileQueue) {
		...
		if (assembly.CallOnBeforeCompilationStarted &&
		    OnBeforeCompilationStarted != null)
		...
		LogStartInfo($"# Starting compiling {assembly.Filename}",
		             compiler.GetProcessStartInfo());
