...
internal struct MonoIsland {
	...
	public MonoIsland(BuildTarget target,
	                  ApiCompatibilityLevel api_compatibility_level,
	                  bool allowUnsafeCode, string[] files, string[] references,
	                  string[] defines, string output) {
		_target = target;
	...
	public MonoIsland(BuildTarget target, bool editor, bool development_player,
	                  bool allowUnsafeCode,
	                  ApiCompatibilityLevel api_compatibility_level,
	                  string[] files, string[] references, string[] defines,
	                  string output) {
		_target = target;
	...
	public MonoIsland(BuildTarget target, bool editor, bool development_player,
	                  ...
	                  ApiCompatibilityLevel api_compatibility_level,
	                  string[] files, string[] references, string[] defines,
	                  string output, string[] responseFiles)
		: this(target, editor, development_player, allowUnsafeCode,
		       api_compatibility_level, files, references, defines, output) {
		_responseFiles = responseFiles;
	}

	public string GetExtensionOfSourceFiles() {
		return _files.Length > 0
		 ? ScriptCompilers.GetExtensionOfSourceFile(_files[0])
		 : "NA";
	}
}
...
static ScriptCompilers() {
	...
	CSharpSupportedLanguage =
		SupportedLanguages.Single(l => l.GetType() == typeof(CSharpLanguage));
}

internal static SupportedLanguageStruct[] GetSupportedLanguageStructs() {
	// we communicate with the runtime by xforming our SupportedLaanguage
	// class to a struct, because that's just a lot easier to marshall between
	// native and managed code.
	return SupportedLanguages
	       .Select(lang => new SupportedLanguageStruct {
...
internal static SupportedLanguage GetLanguageFromName(string name) {
	foreach (var lang in SupportedLanguages) {
		if (String.Equals(name, lang.GetLanguageName(),
		                  StringComparison.OrdinalIgnoreCase))
			return lang;
	}

	throw new ApplicationException(
		      string.Format("Script language '{0}' is not supported", name));
}

internal static SupportedLanguage
GetLanguageFromExtension(string extension) {
	foreach (var lang in SupportedLanguages) {
		if (String.Equals(extension, lang.GetExtensionICanCompile(),
		                  StringComparison.OrdinalIgnoreCase))
			return lang;
	}

	throw new ApplicationException(string.Format(
					       "Script file extension '{0}' is not supported", extension));
}
