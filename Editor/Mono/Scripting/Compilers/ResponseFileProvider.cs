...
public List<string> Get(string folderToLookForResponseFilesIn) {
	if (!string.IsNullOrEmpty(folderToLookForResponseFilesIn) &&
	    !Path.IsPathRooted(folderToLookForResponseFilesIn)) {
		folderToLookForResponseFilesIn =
			AssetPath.Combine(ProjectPath, folderToLookForResponseFilesIn);
	}

	var result = new List<string>();

	var folderResponseFile =
		GetCompilerSpecific(folderToLookForResponseFilesIn);
...
protected string GetDefaultResponseFiles() {
	var rootResponseFilePath =
		AssetPath.Combine(ProjectPath, k_AssetsFolder, ResponseFileName);
	...
	foreach (var obsoleteResponseFileName in ObsoleteResponseFileNames) {
		var obsoleteResponseFilePath = AssetPath.Combine(
			ProjectPath, k_AssetsFolder, obsoleteResponseFileName);
