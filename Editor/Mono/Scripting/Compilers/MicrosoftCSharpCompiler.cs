...
namespace UnityEditor.Scripting.Compilers {
  internal class MicrosoftCSharpCompiler : ScriptCompilerBase {
    ...
    public MicrosoftCSharpCompiler(ScriptAssembly assembly,
                                   string tempOutputDirectory)
        : base(assembly, tempOutputDirectory) {}

    static void FillCompilerOptions(List<string> arguments,
      ...
      // Case 755238: Always use english for outputing errors, the same way as
      // Mono compilers do
      arguments.Add("/preferreduilang:en-US");
      arguments.Add("/langversion:latest");
    }

    internal static string GenerateResponseFile(ScriptAssembly assembly,
                                                string tempBuildDirectory) {
      var assemblyOutputPath = PrepareFileName(
          AssetPath.Combine(tempBuildDirectory, assembly.Filename));

      var arguments = new List<string>{"/target:library", "/nowarn:0169",
                                       "/out:" + assemblyOutputPath};

      if (assembly.CompilerOptions.EmitReferenceAssembly) {
        var referenceAssemblyOutputPath = PrepareFileName(AssetPath.Combine(
            tempBuildDirectory, assembly.ReferenceAssemblyFilename));
      ...
      if (assembly.CompilerOptions.CodeOptimization ==
      ...
      string[] scriptAssemblyReferences =
          new string[assembly.ScriptAssemblyReferences.Length];
      for (var index = 0; index < assembly.ScriptAssemblyReferences.Length;
           index++) {
        var reference = assembly.ScriptAssemblyReferences[index];
        scriptAssemblyReferences[index] =
            "/reference:" + PrepareFileName(AssetPath.Combine(
                                assembly.OutputDirectory, reference.Filename));
      }
      ...
      var responseFileProvider =
          assembly.Language?.CreateResponseFileProvider();
      var responseFilesList =
          responseFileProvider?.Get(assembly.OriginPath) ?? new List<string>();
      ...
      string obsoleteResponseFile = responseFiles.SingleOrDefault(
          x => CompilerSpecificResponseFiles.MicrosoftCSharpCompilerObsolete
                   .Contains(AssetPath.GetFileName(x)));

      if (!string.IsNullOrEmpty(obsoleteResponseFile)) {
        Debug.LogWarning(
            $"Using obsolete custom response file '{AssetPath.GetFileName(obsoleteResponseFile)}'. Please use '{CompilerSpecificResponseFiles.MicrosoftCSharpCompiler}' instead.");
      }

      foreach (var file in responseFiles) {
        AddResponseFileToArguments(
            arguments, file, assembly.CompilerOptions.ApiCompatibilityLevel);
      }
    ...
    private static void ThrowCompilerNotFoundException(string path) {
      throw new Exception(string.Format(
          "'{0}' not found. Is your Unity installation corrupted?", path));
    }
    ...
    public override void BeginCompiling() {
      ...
      var csc = Paths.Combine(EditorApplication.applicationContentsPath,
                              "Tools", "RoslynScripts", "unity_csc");
      ...
      if (assembly.GeneratedResponseFile == null) {
        assembly.GeneratedResponseFile =
            GenerateResponseFile(assembly, tempOutputDirectory);
      }

      var psi = new ProcessStartInfo(){
          Arguments = "/noconfig @" + assembly.GeneratedResponseFile,
          FileName = csc, CreateNoWindow = true};
    ...
    protected string[] GetErrorOutput() { return process.GetErrorOutput(); }
    ...
    public override void WaitForCompilationToFinish() { process.WaitForExit(); }
    ...
    public override CompilerMessage[] GetCompilerMessages() {
      ...
      return CreateOutputParser()
          .Parse(GetStreamContainingCompilerMessages(), CompilationHadFailure(),
                 assembly.Filename)
          .ToArray();
    }
    ...
    private bool CompilationHadFailure() { return (process.ExitCode != 0); }

    public static string[] Compile(string[] sources, string[] references,
      ...
      using(var c = new MicrosoftCSharpCompiler(assembly,
