...
namespace UnityEditor.Scripting.Compilers {
  internal class ExternalCSharpCompiler : ScriptCompilerBase {
    ...
    public ExternalCSharpCompiler(ScriptAssembly assembly,
    ...
    public static string ExternalCompilerName() {
      ...
      var creationTime =
          File.GetCreationTime(typesDerivedFrom.Assembly.Location);
    ...
    private static CompilerBase CreateExternalCompilerBase() {
      ...
      if (typesDerivedFrom.Count > 1) {
        var compilerBaseImplementations =
            typesDerivedFrom.Select(x => x.FullName).ToArray();
        throw new Exception(
            $"More than 1 external C# compiler found: {string.Join(",
            ", compilerBaseImplementations)}");
      }

      var externalCompilerType = typesDerivedFrom[0];
      Console.WriteLine(
          $"External compiler found: {externalCompilerType.FullName}");
    ...
    ScriptAssemblyToAssemblyInfo(ScriptAssembly scriptAssembly,
                                 string outputDir) {
    ...
    public override void Dispose() { m_ExternalCompiler.Dispose(); }
    ...
    public override void BeginCompiling() {
      var assemblyInfo =
          ScriptAssemblyToAssemblyInfo(m_Assembly, m_TempOutputDirectory);
      var systemReferenceDirectories =
          MonoLibraryHelpers.GetSystemReferenceDirectories(
              m_Assembly.CompilerOptions.ApiCompatibilityLevel);
      m_ExternalCompiler.BeginCompiling(
          assemblyInfo, m_Assembly.CompilerOptions.ResponseFiles,
          SystemInfo.operatingSystemFamily, systemReferenceDirectories);
    }

    public override bool Poll() { return m_ExternalCompiler.Poll(); }
    ...
    public override CompilerMessage[] GetCompilerMessages() {
      return m_ExternalCompiler.GetCompilerMessages()
          .Select(x => new CompilerMessage(){
            ...
            message = x.message,
            type = x.type ==
                   Compilation.CompilerMessageType.Error ? CompilerMessageType
                       .Error : CompilerMessageType.Warning,
            assemblyName = m_Assembly.Filename,
          })
          .ToArray();
    }

    public override ProcessStartInfo GetProcessStartInfo() { return null; }
  }
}
