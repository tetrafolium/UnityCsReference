...
namespace UnityEditor.Scripting.Compilers {
  internal static class MicrosoftResponseFileParser {
    ...
    ParseResponseFileFromFile(string responseFilePath, string projectDirectory,
      ...
      var relativeResponseFilePath =
          GetRelativePath(responseFilePath, projectDirectory);
      var responseFile =
          AssetDatabase.LoadAssetAtPath<TextAsset>(relativeResponseFilePath);

      if (!responseFile && File.Exists(responseFilePath)) {
        var responseFileText = File.ReadAllText(responseFilePath);
        return ParseResponseFileText(responseFileText, responseFilePath,
                                     projectDirectory,
      ...
      return ParseResponseFileText(responseFile.text, responseFile.name,
                                   projectDirectory,
                                   systemReferenceDirectories);
    }

    static string GetRelativePath(string responseFilePath,
                                  string projectDirectory) {
      if (Path.IsPathRooted(responseFilePath) &&
          responseFilePath.Contains(projectDirectory)) {
        responseFilePath =
            responseFilePath.Substring(projectDirectory.Length + 1);
      }

      return responseFilePath;
    }

    static ResponseFileData
    ParseResponseFileText(string fileContent, string fileName,
                          string projectDirectory,
                          string[] systemReferenceDirectories) {
      ...
      foreach (var line in responseFileStrings) {
        ...
        compilerOptions.Add(
            new ScriptCompilerBase.CompilerOption{Arg = arg, Value = value});
      }
      ...
      foreach (var option in compilerOptions) {
        ...
        case "/reference": {
          ...
          var responseReference =
              index > -1 ? reference.Substring(index + 1) : reference;
          ...
          if (!isRooted) {
            foreach (var directory in systemReferenceDirectories) {
              var systemReferencePath =
                  Paths.Combine(directory, responseReference);
