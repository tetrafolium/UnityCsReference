...
public UWPSDK(Version version, Version minVSVersion,
              IEnumerable<PreviousUWPSDK> previousSDKs) {
	Version = version;
...
public UWPExtension(string manifest, string windowsKitsFolder,
                    string sdkVersion) {
	...
	if (fileListElement.Attribute("TargetPlatform").Value != "UAP")
		throw new Exception(string.Format(
					    "Invalid extension manifest at \"{0}\".", manifest));
	Name = fileListElement.Attribute("DisplayName").Value;
	var containedApiContractsElement =
		fileListElement.Element("ContainedApiContracts");
	References = GetReferences(windowsKitsFolder, sdkVersion,
	                           containedApiContractsElement);
}
}

private static readonly Version kMinimumSupportedUWPVersion =
	new Version(10, 0, 10240, 0);
private static readonly PreviousUWPSDK kMinimumSupportedPreviousUWPSDK =
	new PreviousUWPSDK(kMinimumSupportedUWPVersion, true);
private static readonly UWPSDK kMinimumSupportedUWPSDK =
	new UWPSDK(kMinimumSupportedUWPVersion, new Version(14, 0),
	           new[] {kMinimumSupportedPreviousUWPSDK});

public static UWPSDK MinimumSupportedUWPSDK {
	get { return kMinimumSupportedUWPSDK; }
}
...
public static string[] GetReferences(UWPSDK sdk) {
	...
	var windowsWinMd = CombinePaths(folder, "UnionMetadata", version,
	                                "Facade", "Windows.winmd");
	if (!File.Exists(windowsWinMd))
		windowsWinMd =
			CombinePaths(folder, "UnionMetadata", "Facade", "Windows.winmd");
...
public static IEnumerable<UWPExtensionSDK> GetExtensionSDKs(UWPSDK sdk) {
	...
	return GetExtensionSDKs(windowsKit10Directory,
	                        SdkVersionToString(sdk.Version));
}
...
public static IEnumerable<UWPSDK> GetInstalledSDKs() {
	...
	var platformsUAP =
		CombinePaths(windowsKit10Directory, "Platforms", "UAP");
	if (!Directory.Exists(platformsUAP))
		return Enumerable.Empty<UWPSDK>();

	var allSDKs = new List<UWPSDK>();

	var filesUnderPlatformsUAP =
		Directory.GetFiles(platformsUAP, "*", SearchOption.AllDirectories);
	var allPlatformXmlFiles = filesUnderPlatformsUAP.Where(
		f => string.Equals("Platform.xml", Path.GetFileName(f),
	...
	foreach (var platformXmlFile in allPlatformXmlFiles) {
		...
		foreach (var platformElement in xDocument.Elements(
				 "ApplicationPlatform")) {
			Version version;
			if (FindVersionInNode(platformElement, out version)) {
				...
				var minVSVersionString =
					platformElement.Elements("MinimumVisualStudioVersion")
					.Select(e => e.Value)
					.FirstOrDefault();

				// Get supported previous versionss
				var previousVersionPath =
					Path.Combine(Path.GetDirectoryName(platformXmlFile),
					             "PreviousPlatforms.xml");
				var previousVersions = new List<PreviousUWPSDK>();

				if (File.Exists(previousVersionPath)) {
					XNamespace xn =
						"http://microsoft.com/schemas/Windows/SDK/PreviousPlatforms";
					...
					if (previousPlatformsDocument != null) {
						var previousPlatformsElement =
							previousPlatformsDocument.Element(xn + "PreviousPlatforms");
						if (previousPlatformsElement != null) {
							foreach (XElement previousPlatformElement in
							         previousPlatformsElement.Elements(
									 xn + "ApplicationPlatform")) {
								var versionAttribute =
									previousPlatformElement.Attribute("version");
								if (versionAttribute != null) {
									var previousVersionString = versionAttribute.Value;
									bool isDefault = false;

									var isDefaultFallbackAttribute =
										previousPlatformElement.Attribute(
											"IsDefaultFallback");
									if (isDefaultFallbackAttribute != null)
										bool.TryParse(isDefaultFallbackAttribute.Value,
										              out isDefault);

									var previousVersion =
										TryParseVersion(previousVersionString);
									if (previousVersion != null &&
									    previousVersion >= kMinimumSupportedUWPVersion)
										previousVersions.Add(
											new PreviousUWPSDK(previousVersion, isDefault));
								}
							}
						}
					}
				...
				if (previousVersions.Count == 0) {
					// For previous versions, only support the current version and our
					// minimum supported version if no PreviousVersions.xml was found.
					previousVersions.Add(new PreviousUWPSDK(version, true));

					if (version > kMinimumSupportedUWPVersion)
						previousVersions.Add(
							new PreviousUWPSDK(kMinimumSupportedUWPVersion, false));
				}
...
private static bool FindVersionInNode(XElement node, out Version version) {
	for (var attribute = node.FirstAttribute; attribute != null;
	     attribute = attribute.NextAttribute) {
		if (string.Equals(attribute.Name.LocalName, "version",
		                  StringComparison.OrdinalIgnoreCase)) {
...
private static string[] GetPlatform(string folder, string version) {
	var platformXml =
		CombinePaths(folder, @"Platforms\UAP", version, "Platform.xml");
	...
	if (applicationPlatformElement.Attribute("name").Value != "UAP")
		throw new Exception(string.Format(
					    "Invalid platform manifest at \"{0}\".", platformXml));
	var containedApiContractsElement =
		applicationPlatformElement.Element("ContainedApiContracts");
...
private static IEnumerable<UWPExtensionSDK>
GetExtensionSDKs(string sdkFolder, string sdkVersion) {
	...
	foreach (var extensionFolder in Directory.GetDirectories(
			 extensionsFolder)) {
		var manifest =
			CombinePaths(extensionFolder, sdkVersion, "SDKManifest.xml");
		var extensionName = Path.GetFileName(extensionFolder);

		if (File.Exists(manifest)) {
			extensions.Add(
				new UWPExtensionSDK(extensionName, sdkVersion, manifest));
			continue;
		}

		if (extensionName == "XboxLive") {
			// Workaround for XboxLive SDK bug: currently, its version is
			// always 1.0. Microsoft said they'll fix it in the future.
...
private static UWPExtension[] GetExtensions(string windowsKitsFolder,
                                            string version) {
	var extensions = new List<UWPExtension>();

	foreach (var extensionSDK in GetExtensionSDKs(windowsKitsFolder,
	                                              version)) {
		try {
			var extension = new UWPExtension(extensionSDK.ManifestPath,
			                                 windowsKitsFolder, version);
...
private static string[] GetReferences(
	...
	foreach (var apiContractElement in containedApiContractsElement.Elements(
			 "ApiContract")) {
		var name = apiContractElement.Attribute("name").Value;
		var version = apiContractElement.Attribute("version").Value;
		var reference =
			CombinePaths(windowsKitsFolder, "References", sdkVersion, name,
			             version, name + ".winmd");
		if (!File.Exists(reference)) {
			reference = CombinePaths(windowsKitsFolder, "References", name,
			                         version, name + ".winmd");
...
private static string GetWindowsKit10() {
	...
	try
	{
		...
		folder = UnityEditorInternal.RegistryUtil.GetRegistryStringValue(
			keyName, "InstallationFolder", folder,
			UnityEditorInternal.RegistryView._32);
	}
	catch
	{
		// ignore exceptions
