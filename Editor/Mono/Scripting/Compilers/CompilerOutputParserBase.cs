...
namespace UnityEditor.Scripting.Compilers {
  internal abstract class CompilerOutputParserBase {
    static protected CompilerMessage
    ...
    internal static protected CompilerMessage
    CreateCompilerMessageFromMatchedRegex(string line, Match m,
      ...
      message.type = (m.Groups["type"].Value == erroridentifier)
    ...
    public virtual IEnumerable<CompilerMessage>
    Parse(string[] errorOutput, bool compilationHadFailure,
          string assemblyName) {
    ...
    public virtual IEnumerable<CompilerMessage>
    Parse(string[] errorOutput, string[] standardOutput,
          bool compilationHadFailure, string assemblyName) {
      var hasErrors = false;
      ...
      foreach (var line in errorOutput) {
        // Jamplus can fail with enormous lines in the stdout, parsing of which
        // can take 30! seconds.
        ...
        CompilerMessage message = CreateCompilerMessageFromMatchedRegex(
            line, m, GetErrorIdentifier());
    ...
    protected virtual NormalizedCompilerStatus
    NormalizedStatusFor(Match match) {
    ...
    protected static NormalizedCompilerStatus TryNormalizeCompilerStatus(
        Match match, string idToCheck, Regex messageParser,
        Func<Match, Regex, NormalizedCompilerStatus> normalizer) {
    ...
    protected static NormalizedCompilerStatus
    NormalizeMemberNotFoundError(Match outputMatch, Regex messageParser) {
      ...
      ret.details =
          dm.Groups["type_name"].Value + "%" + dm.Groups["member_name"].Value;

      return ret;
    }

    protected static NormalizedCompilerStatus
    NormalizeSimpleUnknownTypeOfNamespaceError(Match outputMatch,
                                               Regex messageParser) {
      NormalizedCompilerStatus ret;
    ...
    protected static NormalizedCompilerStatus
    NormalizeUnknownTypeMemberOfNamespaceError(Match outputMatch,
                                               Regex messageParser) {
      NormalizedCompilerStatus ret;
      ret.code = NormalizedCompilerStatusCode.UnknownTypeOrNamespace;

      var dm = messageParser.Match(outputMatch.Groups["message"].Value);
      ret.details = "EntityName=" + dm.Groups["namespace"].Value + "." +
                    dm.Groups["type_name"].Value + "\n" +
                    "Script=" + outputMatch.Groups["filename"].Value + "\n" +
                    "Line=" + outputMatch.Groups["line"].Value + "\n" +
                    "Column=" + outputMatch.Groups["column"].Value;
