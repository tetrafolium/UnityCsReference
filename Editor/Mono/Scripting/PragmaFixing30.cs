...
namespace UnityEditor.Scripting {
  internal class PragmaFixing30 {
    ...
    static bool CheckOrFixPragmas(string fileName, bool onlyCheck) {
      ...
      if (!onlyCheck)
        DoFixPragmasInFile(fileName, oldText,
                           strictMatch.Index + strictMatch.Length, hasDowncast,
                           hasImplicit);

      return true;
    }

    static void DoFixPragmasInFile(string fileName, string oldText, int fixPos,
                                   bool hasDowncast, bool hasImplicit) {
    ...
    static void LooseComments(StringBuilder sb) {
      // TODO: better comment ignoring? this one sort of does the job, it
      // handles // and if it's in multiline comments, our added lines will end
      // up commented as well
      Regex r = new Regex("//");
    ...
    static Match PragmaMatch(StringBuilder sb, string pragma) {
      // unity java script, like regex, treats new line as space character as
      // well
    ...
    static string[] CollectBadFiles() {
      ...
      foreach (string f in SearchRecursive(
