...
internal class HostView : GUIView, IEditorWindowModel {
  static class Styles {
    ...
    static Styles() {
      // Fix annoying GUILayout issue: When using GUILayout in Utility windows
      // there was always padded 10 px at the top! Todo: Fix this in
      // EditorResources
  ...
  internal static PrefColor kPlayModeDarken =
      new PrefColor(kPlayModeDarkenKey, .8f, .8f, .8f, 1);
  ...
  private void CreateDelegates() {
    ...
    if (methodInfo != null)
      m_ShowButton = (EditorWindowShowButtonDelegate) Delegate.CreateDelegate(
          typeof(EditorWindowShowButtonDelegate), m_ActualView, methodInfo);
  }
  ...
  Action IEditorWindowModel.onSplitterGUIHandler {
    get;
    set;
  }
  ...
  protected override void OnDisable() {
    ...
    base.OnDisable();
    DeregisterSelectedPane(clearActualView : false, sendEvents : true);
    // Host views are destroyed in the middle of an OnGUI loop, so we need to
    // ensure that we're not invoking OnGUI on destroyed instances.
    m_OnGUI = null;
    m_Update = null;
  }
  ...
  private static readonly Type[] k_PaneTypes = {
      typeof(SceneView),       typeof(GameView),
      typeof(InspectorWindow), typeof(SceneHierarchyWindow),
      typeof(ProjectBrowser),  typeof(ProfilerWindow),
      typeof(AnimationWindow)};

  private static IEnumerable<Type>
  GetCurrentModePaneTypes(string modePaneTypeSectionName) {
    var modePaneTypes = ModeService.GetModeDataSectionList<string>(
        ModeService.currentIndex, modePaneTypeSectionName);
    var editorWindowTypes = TypeCache.GetTypesDerivedFrom<EditorWindow>();
    foreach (var paneTypeName in modePaneTypes) {
      var paneType =
          editorWindowTypes.FirstOrDefault(t => t.Name.EndsWith(paneTypeName));
      if (paneType != null)
        yield return paneType;
      else
        Debug.LogWarning(
            $"Cannot find editor window pane type {paneTypeName} for editor mode {ModeService.currentId}.");
    }
  }

  private static IEnumerable<Type> GetDefaultPaneTypes() {
    const string k_PaneTypesSectionName = "pane_types";
    if (!ModeService.HasSection(ModeService.currentIndex,
                                k_PaneTypesSectionName))
      return k_PaneTypes;
  ...
  // The implementation is not very good, but oh well... it gets the message
  // across.
  internal void OnMainWindowMove() { Invoke("OnMainWindowMove"); }

  internal void OnProjectChange() { m_OnProjectChange?.Invoke(); }

  internal void OnSelectionChange() { m_OnSelectionChange?.Invoke(); }

  internal void OnDidOpenScene() { m_OnDidOpenScene?.Invoke(); }

  internal void OnInspectorUpdate() { m_OnInspectorUpdate?.Invoke(); }

  internal void OnHierarchyChange() { m_OnHierarchyChange?.Invoke(); }

  EditorWindowDelegate CreateDelegate(string methodName) {
    var methodInfo = GetPaneMethod(methodName);
    if (methodInfo != null)
      return (EditorWindowDelegate) Delegate.CreateDelegate(
          typeof(EditorWindowDelegate), m_ActualView, methodInfo);
    return null;
  }
  ...
  private string GetActualViewName() {
    ...
    m_ActualViewName =
        actualView != null ? actualView.GetType().Name : GetType().Name;
    return m_ActualViewName;
  }
  ...
  public void InvokeOnGUI(Rect onGUIPosition, Rect viewRect) {
    ...
    try {
      using(new EditorPerformanceTracker(
          $"{GetActualViewName()}.OnGUI.{Event.current.type}")) {
        m_OnGUI?.Invoke();
      }
  ...
  protected void Invoke(string methodName) { Invoke(methodName, m_ActualView); }
  ...
  protected void RegisterSelectedPane(bool sendEvents) {
    ...
    if (sendEvents) {
      try {
        ...
        // We need to catch these so the window initialization doesn't get
        // screwed
        if (ex.InnerException != null)
          Debug.LogError(ex.InnerException.GetType().Name + ":" +
  ...
  void SendUpdate() { m_Update?.Invoke(); }

  void SendModKeysChanged() { m_ModifierKeysChanged?.Invoke(); }
  ...
  public static SVC<float> genericMenuLeftOffset =
      new SVC<float>("--window-generic-menu-left-offset", 20f);
  public static SVC<float> genericMenuFloatingLeftOffset =
      new SVC<float>("--window-floating-generic-menu-left-offset", 20f);
  internal static float
  GetGenericMenuLeftOffset(bool addFloatingWindowButtonsTopRight) {
  ...
  internal float GetExtraButtonsWidth() {
    ...
    foreach (var item in windowActions) {
      if (item != null &&
          (item.validateHandler == null ||
           item.validateHandler(actualView, item)) &&
          item.width.HasValue)
  ...
  protected void ShowGenericMenu(float leftOffset, float topOffset) {
    Rect paneMenu =
        new Rect(leftOffset, topOffset, Styles.paneOptions.fixedWidth,
                 Styles.paneOptions.fixedHeight);
    if (EditorGUI.DropdownButton(paneMenu, GUIContent.none, FocusType.Passive,
                                 Styles.paneOptions))
      PopupGenericMenu(m_ActualView, paneMenu);

    // Give panes an option of showing a small button next to the generic menu
    // (used for inspector lock icon
    if (m_ShowButton != null) {
      leftOffset -= ContainerWindow.kButtonWidth + k_iconMargin;
      m_ShowButton.Invoke(new Rect(leftOffset, topOffset,
                                   ContainerWindow.kButtonWidth,
                                   ContainerWindow.kButtonHeight));
    }
    ...
    foreach (var item in windowActions) {
      if (item != null &&
        ...
        Rect itemRect = new Rect(leftOffset, topOffset, item.width.Value,
                                 ContainerWindow.kButtonHeight);
        if (item.drawHandler != null) {
          ...
          if (EditorGUI.Button(
                  itemRect,
                  EditorGUIUtility.TrIconContent(item.icon, item.menuPath),
  ...
  private static WindowAction[] FetchWindowActionFromAttribute() {
    var methods =
        AttributeHelper.GetMethodsWithAttribute<WindowActionAttribute>();
    return methods.methodsWithAttributes
        .Select(method => {
          try {
            var callback = Delegate.CreateDelegate(
                typeof(Func<WindowAction>), method.info) as Func<WindowAction>;
            return callback?.Invoke();
          } catch (Exception) {
            Debug.LogError("Cannot create Window Action for: " +
                           method.info.Name);
          }
          return null;
        })
  ...
  internal static void AddWindowActionMenu(GenericMenu menu,
                                           EditorWindow view) {
    bool itemAdded = false;
    int previousItemPriority = 0;
    foreach (var item in windowActions) {
      if (item != null &&
          (item.validateHandler == null || item.validateHandler(view, item)) &&
          !string.IsNullOrEmpty(item.menuPath)) {
        if (!itemAdded) {
          menu.AddSeparator("");
          itemAdded = true;
        ...
        menu.AddItem(new GUIContent(item.menuPath, item.icon), false,
                     () => item.executeHandler(view, item));
  ...
  protected virtual void AddDefaultItemsToMenu(GenericMenu menu,
                                               EditorWindow window) {
    if (menu.GetItemCount() != 0)
      menu.AddSeparator("");

    if (window && Unsupported.IsDeveloperMode()) {
      menu.AddItem(EditorGUIUtility.TrTextContent("Inspect Window"), false,
                   Inspect, window);
      menu.AddItem(EditorGUIUtility.TrTextContent("Inspect View"), false,
                   Inspect, window.m_Parent);
      menu.AddItem(EditorGUIUtility.TrTextContent("Reload Window _f5"), false,
                   Reload, window);
