...
internal class SceneHierarchy {
  static class Styles {
    ...
    const string kWarningMessage =
        "The current sorting method is taking a lot of time. Consider using 'Transform Sort' in playmode for better performance.";

    public static GUIContent defaultSortingContent =
        EditorGUIUtility.TrIconContent(kCustomSorting);
    public static GUIContent createContent =
        EditorGUIUtility.IconContent("CreateAddNew");
    public static GUIContent fetchWarning = new GUIContent(
        "", EditorGUIUtility.FindTexture(kWarningSymbol), kWarningMessage);

    public static GUIStyle lockButton = "IN LockButton";

    public static GUIContent renamingEnabledContent =
        EditorGUIUtility.TrTextContent("Rename New Objects");
    public static GUIContent setOriginLabel =
        new GUIContent("Set as Default Parent");
    public static GUIContent clearOriginLabel =
        new GUIContent("Clear Default Parent");
  }
  ...
  public Transform customParentForNewGameObjects {
    set {
      ...
      if (m_TreeView != null && m_TreeView.dragging != null)
        ((GameObjectsTreeViewDragging) m_TreeView.dragging)
            .parentForDraggedObjectsOutsideItems =
            m_CustomParentForNewGameObjects;
    }
  }

  GameObjectsTreeViewDragging.CustomDraggingDelegate m_CustomDragHandler;
  public void SetCustomDragHandler(
      GameObjectsTreeViewDragging.CustomDraggingDelegate handler) {
    m_CustomDragHandler = handler;
    if (m_TreeView != null && m_TreeView.dragging != null)
      ((GameObjectsTreeViewDragging) m_TreeView.dragging)
          .SetCustomDragHandler(handler);
  }
  ...
  [SerializeField] List<string> m_ExpandedScenes =
      new List<string>(); // saved in layout so we can expand on next Unity
                          // session (expanded state is saved per window)
  ...
  [NonSerialized] List<string> m_ParentNamesForSelectedSearchResult =
      new List<string>();

  string m_SearchFilter;
  SearchableEditorWindow.SearchModeHierarchyWindow m_SearchMode =
      SearchableEditorWindow.SearchModeHierarchyWindow.All;

  [NonSerialized]
  private int m_LastFramedID = -1;
  [NonSerialized] bool m_TreeViewReloadNeeded;
  [NonSerialized] bool m_SelectionSyncNeeded;
  [NonSerialized] bool m_FrameOnSelectionSync;
  [NonSerialized] bool m_DidSelectSearchResult;
  [NonSerialized] double m_LastUserInteractionTime;
  ...
  [SerializeField] string m_CurrentSortingName = ""; // serialize as string
  ...
  public bool hasSortMethods {
    get { return m_SortingObjects != null && m_SortingObjects.Count > 1; }
  }

  public int treeViewKeyboardControlID {
    get { return m_TreeViewKeyboardControlID; }
  }
  ...
  void Repaint() {
    // m_EditorWindow is null before Awake is called on startup (we have events
    // like OnBecameVisible and OnFocus which is called before Awake)
  ...
  void Init() {
    ...
    var dataSource =
        new GameObjectTreeViewDataSource(m_TreeView, m_CurrenRootInstanceID,
                                         showRootItem, rootItemIsCollapsable);
    ...
    dataSource.sortingState = m_SortingObjects[m_CurrentSortingName];
    dragging.parentForDraggedObjectsOutsideItems =
        m_CustomParentForNewGameObjects;
  ...
  void SetUpSortMethodLists() {
    ...
    m_SortingObjects.Add(GetNameForType(transformSorting.GetType()),
                         transformSorting);

    // The user have to activate AlphabeticalSorting in the preferences (we
    // ensure AlphabeticalSorting is added for integration tests)
    if (m_AllowAlphaNumericalSort ||
        !InternalEditorUtility.isHumanControllingUs) {
      var alphabeticalSorting = new AlphabeticalSorting();
      m_SortingObjects.Add(GetNameForType(alphabeticalSorting.GetType()),
                           alphabeticalSorting);
    }
  ...
  string GetNameForType(Type type) { return type.Name; }
  ...
  public void SetCurrentRootInstanceID(int instanceID) {
    ...
    Init();
    EditorGUIUtility
        .ExitGUI(); // exit gui since this can be called while iterating items
  }
  ...
  internal void SelectPrevious() { m_TreeView.OffsetSelection(-1); }

  internal void SelectNext() { m_TreeView.OffsetSelection(1); }
  ...
  public virtual void OnBecameVisible() {
    // We need to ensure Hierarchy window is reloaded when becoming visible
    // because while it is hidden it does not receive OnHierarchyChanged
    // callbacks (case 611409) During assembly reload editor windows are
    // recreated, at that point scene count is 0 so ignore that event
  ...
  public virtual void OnEnable() {
    Assert.IsTrue(m_EditorWindow != null,
                  "Editor Window is null. It should survive assembly reload");

    EditorApplication.projectChanged +=
        ReloadData; // Required to know if a prefab gets deleted. Better way of
                    // doing this?
    ...
    m_AllowAlphaNumericalSort =
        ...
        !InternalEditorUtility
             .isHumanControllingUs; // Always allow alphasorting when running
                                    // automated tests so we can test alpha
                                    // sorting
    SetUpSortMethodLists();
  ...
  internal void OnProjectWasLoaded() {
    ...
    // the expanded state on project start up (we still need this even though we
    // also clear in OnQuit to be sure if loading a old layout)
  ...
  public virtual void OnQuit() {
    ...
    // Game objects will have new instanceIDs in the next Unity session, so
    // clear all state before serializing to layout to prevent saving redundant
    // data
    m_TreeViewState = new TreeViewState();
  }
  ...
  void SceneViewRectSelectionStarting() { m_RectSelectInProgress = true; }
  ...
  void SyncIfNeeded() {
    ...
    if (selectionSyncNeeded) {
      ...
      bool userJustInteracted = (EditorApplication.timeSinceStartup -
                                 m_LastUserInteractionTime) < 0.2;
      bool frame =
          (!isLocked || m_FrameOnSelectionSync || userJustInteracted) &&
          !m_RectSelectInProgress;
  ...
  void OnRowGUICallback(int instanceID, Rect rect) {
    if (EditorApplication.hierarchyWindowItemOnGUI != null) {
      // Adjust rect for the right aligned column for the prefab isolation
      // button
      rect.xMax -=
  ...
  void OnDragEndedCallback(int[] draggedInstanceIds,
    ...
    // set before calling this callback when dragging e.g a prefab to the
    // hierarchy (case 628939)
  ...
  GameObjectTreeViewDataSource dataSource {
    get { return (GameObjectTreeViewDataSource) treeView.data; }
  }

  public void
  SetSearchFilter(string searchString,
                  SearchableEditorWindow.SearchModeHierarchyWindow searchMode) {
    ...
    // If the user clears the search we frame the last selection he made during
    // the search
  ...
  void TreeViewSelectionChanged(int[] ids) {
    // Last selected should be the active selected object to reflect the
    // behavior of the scene view selection
    if (ids.Length > 0)
  ...
  public void OnSelectionChange() {
    if (!IsTreeViewSelectionInSyncWithBackend()) {
      ...
      if (s_Debug)
        Debug.Log(
            "OnSelectionChange: Selection is already in sync so no framing will happen");
    }
  }

  public void OnHierarchyChange() {
    // Avoid end renaming once if gameObject is newly created and is set to
    // enter rename mode
  ...
  void OnEvent() { treeView.OnEvent(); }

  // Tree view item handle their own scene visibility background.
  // So the background doesn't stop if we don't have enough item to fill the
  // entire tree view rect, we draw the background in the unused space.
  private void DoSceneVisibilityBackgroundOverflow(float reservedFooterSpace) {
    ...
    // If the tree view already covers the entire rect, we don't need to fill
    // the overflow
  ...
  /*
   * NOTE: AddCreateGameObjectItemsToMenu() cooks existing menu, so that make
   * sure menu entries are added to localization entry.
   * @Localization("Create Empty", "MenuItem")
   * @Localization("Create Empty Child", "MenuItem")
   */
  void AddCreateGameObjectItemsToMenu(GenericMenu menu,
    ...
    foreach (string path in menus) {
      ...
      if (!includeCreateEmptyChild &&
          path.ToLower() == "GameObject/Create Empty Child".ToLower())
        continue;

      if (!useCreateEmptyParentMenuItem &&
          path.ToLower() == "GameObject/Create Empty Parent".ToLower()) {
        if (GOCreationCommands.ValidateCreateEmptyParent())
          menu.AddItem(EditorGUIUtility.TrTextContent("Create Empty Parent"),
                       false, GOCreationCommands.CreateEmptyParent);
        continue;
      }

      // The first item after the GameObject creation menu items
      if (path.ToLower() ==
          GameObjectUtility.GetFirstItemPathAfterGameObjectCreationMenuItems()
              .ToLower())
        return;
      ...
      MenuUtils.ExtractMenuItemWithPath(
          path, menu, menupath, tempContext, targetSceneHandle,
          BeforeCreateGameObjectMenuItemWasExecuted,
          AfterCreateGameObjectMenuItemWasExecuted, origin);
    }
  }

  void BeforeCreateGameObjectMenuItemWasExecuted(
      string menuPath, UnityEngine.Object[] contextObjects,
      MenuUtils.ContextMenuOrigin origin, int userData) {
    int sceneHandle = userData;
    if (origin == MenuUtils.ContextMenuOrigin.Scene ||
        origin == MenuUtils.ContextMenuOrigin.Subscene)
      GOCreationCommands.forcePlaceObjectsAtWorldOrigin = true;
    EditorSceneManager.SetTargetSceneForNewGameObjects(sceneHandle);
  }

  void AfterCreateGameObjectMenuItemWasExecuted(
      string menuPath, UnityEngine.Object[] contextObjects,
      MenuUtils.ContextMenuOrigin origin, int userData) {
  ...
  public void GameObjectCreateDropdownButton() {
    Rect rect = GUILayoutUtility.GetRect(
        Styles.createContent, EditorStyles.toolbarCreateAddNewDropDown, null);
    bool mouseOver = rect.Contains(Event.current.mousePosition);
    if (Event.current.type == EventType.Repaint)
      EditorStyles.toolbarCreateAddNewDropDown.Draw(
          rect, Styles.createContent, mouseOver, false, false, false);

    if (Event.current.type == EventType.MouseDown && mouseOver) {
      GUIUtility.hotControl = 0;

      GenericMenu menu = new GenericMenu();
      var targetSceneHandle =
          m_CustomParentForNewGameObjects !=
          null ? m_CustomParentForNewGameObjects.gameObject.scene.handle
          : kInvalidSceneHandle;
      // The context should be null, just like it is in the main menu. Case
      // 1185434.
      AddCreateGameObjectItemsToMenu(menu, null, true, true, false,
                                     targetSceneHandle,
                                     MenuUtils.ContextMenuOrigin.Toolbar);
      menu.DropDown(rect);
  ...
  public void SortMethodsDropDownButton() {
    if (hasSortMethods) {
      ...
      Rect r =
          GUILayoutUtility.GetRect(content, EditorStyles.toolbarButtonRight);
      if (EditorGUI.DropdownButton(r, content, FocusType.Passive,
                                   EditorStyles.toolbarButtonRight)) {
        // Build list items
        var sortFunctionItems =
            new List<SceneHierarchySortingWindow.InputData>();
        foreach (var entry in m_SortingObjects) {
          var data = new SceneHierarchySortingWindow.InputData();
          data.m_TypeName = entry.Key;
          data.m_Name = ObjectNames.NicifyVariableName(
              entry.Key); // entry.Key == class name;
          data.m_Selected = entry.Key == m_CurrentSortingName;
          sortFunctionItems.Add(data);
        }

        // Show popup
        if (SceneHierarchySortingWindow.ShowAtPosition(
                new Vector2(r.x, r.y + r.height), sortFunctionItems,
                SortFunctionCallback)) {
          GUIUtility.ExitGUI();
        }
      }
    }
  ...
  void ExecuteCommands() {
    ...
    if (evt.commandName == EventCommandNames.Delete ||
      ...
      if (execute && !CutCopyPasteUtility.GetIsCustomParentSelected(
                         m_CustomParentForNewGameObjects))
        DeleteGO();
      evt.Use();
      GUIUtility.ExitGUI();
  ...
  void CreateSubSceneGameObjectContextClick(GenericMenu menu,
                                            int contextClickedItemID) {
    // For Sub Scenes GameObjects, have menu items for cut, paste and delete.
    // Not copy or duplicate, since multiple of the same Sub Scene is not
    // supported anyway.

    menu.AddItem(EditorGUIUtility.TrTextContent("Cut"), false,
                 CutCopyPasteUtility.CutGO);
    if (CutBoard.CanGameObjectsBePasted() ||
        Unsupported.CanPasteGameObjectsFromPasteboard())
    ...
    if (CutCopyPasteUtility.GetIsCustomParentSelected(
            m_CustomParentForNewGameObjects))
      menu.AddDisabledItem(EditorGUIUtility.TrTextContent("Delete GameObject"));
    else
      menu.AddItem(EditorGUIUtility.TrTextContent("Delete GameObject"), false,
                   DeleteGO);
  }

  void CreateGameObjectContextClick(GenericMenu menu,
                                    int contextClickedItemID) {
    menu.AddItem(EditorGUIUtility.TrTextContent("Cut"), false,
                 CutCopyPasteUtility.CutGO);
    menu.AddItem(EditorGUIUtility.TrTextContent("Copy"), false,
                 CutCopyPasteUtility.CopyGO);
    if (CutBoard.CanGameObjectsBePasted() ||
        Unsupported.CanPasteGameObjectsFromPasteboard())
      menu.AddItem(EditorGUIUtility.TrTextContent("Paste"), false, PasteGO);
    ...
    menu.AddItem(EditorGUIUtility.TrTextContent("Duplicate"), false,
                 DuplicateGO);
    ...
    if (IsSelectChildrenAvailable())
      menu.AddItem(EditorGUIUtility.TrTextContent("Select Children"), false,
                   SelectChildren);
    else
    ...
    if (Selection.objects.Length > 0)
      selectedObject =
          Selection.objects[Selection.objects.Length - 1] as GameObject;

    if (Selection.count == 0 && treeView.hoveredItem == null ||
        selectedObject &&
            (selectedObject.name ==
                 PrefabUtility.kDummyPrefabStageRootObjectName ||
             PrefabStageUtility.IsGameObjectThePrefabRootInAnyPrefabStage(
                 selectedObject))) {
      menu.AddDisabledItem(Styles.setOriginLabel);
    } else if (selectedObject &&
               selectedObject.GetInstanceID() !=
                   GetDefaultParentForSession(selectedObject.scene.guid)) {
      menu.AddItem(Styles.setOriginLabel, false,
                   () => { SetDefaultParentObject(false); });
    } else {
      menu.AddItem(Styles.clearOriginLabel, false,
    ...
    if (m_TreeViewState.selectedIDs.Count == 1) {
      GameObjectTreeViewItem item = treeView.FindItem(
          m_TreeViewState.selectedIDs[0]) as GameObjectTreeViewItem;
    ...
    if (!string.IsNullOrEmpty(assetPath)) {
      ...
      if (PrefabUtility.IsPartOfModelPrefab(prefabAsset)) {
        ...
        menu.AddItem(
            EditorGUIUtility.TrTextContent("Prefab/Open Asset in Context"),
            false, () => {
              PrefabStageUtility.OpenPrefab(
                  assetPath, go, PrefabStage.Mode.InContext,
                  StageNavigationManager.Analytics.ChangeType
                      .EnterViaInstanceHierarchyContextMenu);
            });
        menu.AddItem(
            EditorGUIUtility.TrTextContent("Prefab/Open Asset in Isolation"),
            false, () => {
              PrefabStageUtility.OpenPrefab(
                  assetPath, go, PrefabStage.Mode.InIsolation,
                  StageNavigationManager.Analytics.ChangeType
                      .EnterViaInstanceHierarchyContextMenu);
            });
      }
    ...
    if (IsSelectPrefabRootAvailable()) {
      menu.AddItem(EditorGUIUtility.TrTextContent("Prefab/Select Root"), false,
                   SelectPrefabRoot);
    }
    ...
    if (go != null && PrefabUtility.IsAddedGameObjectOverride(go)) {
      ...
      PrefabUtility.HandleApplyRevertMenuItems(
          "Added GameObject", parentTransform.gameObject,
          (menuItemContent, sourceGo) => {
            TargetChoiceHandler.ObjectInstanceAndSourcePathInfo info =
                new TargetChoiceHandler.ObjectInstanceAndSourcePathInfo();
            ...
            if (!PrefabUtility.IsPartOfPrefabThatCanBeAppliedTo(rootGo) ||
                EditorUtility.IsPersistent(parentTransform))
              menu.AddDisabledItem(menuItemContent);
            else
              menu.AddItem(menuItemContent, false,
                           TargetChoiceHandler.ApplyPrefabAddedGameObject,
                           info);
          },
          (menuItemContent) => {
            menu.AddItem(menuItemContent, false,
                         TargetChoiceHandler.RevertPrefabAddedGameObject, go);
          });
    }

    if (AnyOutermostPrefabRoots()) {
      menu.AddItem(EditorGUIUtility.TrTextContent("Prefab/Unpack"), false,
                   UnpackPrefab);
      menu.AddItem(EditorGUIUtility.TrTextContent("Prefab/Unpack Completely"),
                   false, UnpackPrefabCompletely);
    }
    ...
    // All Create GameObject menu items
    {
      ...
      // Set the context of each MenuItem to the current selection, so the
      // created gameobjects will be added as children Sets
      // includeCreateEmptyChild to false, since that item is superfluous here
      // (the normal "Create Empty" is added as a child anyway)
      AddCreateGameObjectItemsToMenu(
          menu, selectedGameObjects, false, false, false,
          targetSceneForCreation,
          contextClickedItemID == 0 ? MenuUtils.ContextMenuOrigin.None
                                    : MenuUtils.ContextMenuOrigin.GameObject);
    }

    SceneHierarchyHooks.AddCustomGameObjectContextMenuItems(
        menu, contextClickedItemID == 0
                  ? null
                  : (GameObject)
                        EditorUtility.InstanceIDToObject(contextClickedItemID));

    if (selectedGameObjects.Length > 0) {
      menu.AddSeparator("");
      menu.AddItem(new GUIContent("Properties..."), false,
                   () => PropertyEditor.OpenPropertyEditorOnSelection());
    }

    menu.ShowAsContext();
  }

  protected void AddCreateGameObjectItemsToSceneMenu(GenericMenu menu,
                                                     Scene scene) {
    AddCreateGameObjectItemsToMenu(
        menu, Selection.transforms.Select(t => t.gameObject).ToArray(), false,
        false, true, scene.handle, MenuUtils.ContextMenuOrigin.Scene);
  }

  protected void AddCreateGameObjectItemsToSubSceneMenu(GenericMenu menu,
                                                        Scene scene) {
    AddCreateGameObjectItemsToMenu(menu, new UnityEngine.Object[0], false, true,
                                   true, scene.handle,
  ...
  internal void CreateSceneHeaderContextClick(GenericMenu menu, Scene scene) {
    ...
    if (scene.isLoaded) {
      if (!EditorApplication.isPlaying) {
        menu.AddItem(EditorGUIUtility.TrTextContent("Save Scene"), false,
                     SaveSelectedScenes, scene);
        if (!scene.isSubScene)
          menu.AddItem(EditorGUIUtility.TrTextContent("Save Scene As"), false,
                       SaveSceneAs, scene);
        if (hasMultipleScenes)
          menu.AddItem(EditorGUIUtility.TrTextContent("Save All"), false,
                       SaveAllScenes, scene);
        else
    ...
    bool isUnloadOrRemoveValid =
        EditorSceneManager.loadedSceneCount != GetNumLoadedScenesInSelection();

    if (!scene.isSubScene) {
      if (scene.isLoaded) {
        // Unload
        var content = EditorGUIUtility.TrTextContent("Unload Scene");
        bool canUnloadScenes = isUnloadOrRemoveValid &&
      ...
      bool selectedAllScenes =
          GetSelectedScenes().Count == EditorSceneManager.sceneCount;
      bool canRemoveScenes = isUnloadOrRemoveValid && !selectedAllScenes &&
                             !EditorApplication.isPlaying;
      if (canRemoveScenes)
    ...
    if (scene.isLoaded) {
      ...
      bool canDiscardChanges =
          !EditorApplication.isPlaying && modifiedScenes.Length > 0;
    ...
    // Ping Scene Asset
    menu.AddSeparator("");
    var selectAssetContent =
        EditorGUIUtility.TrTextContent("Select Scene Asset");
    ...
    // Set the context of each MenuItem to the current selection, so the created
    // gameobjects will be added as children Sets includeCreateEmptyChild to
    // false, since that item is superfluous here (the normal "Create Empty" is
    // added as a child anyway)
    if (scene.isLoaded) {
      menu.AddSeparator("");
      if (scene.isSubScene)
  ...
  List<int> GetSelectedScenes() {
    ...
    foreach (int id in instanceIDs) {
      if (IsSceneHeaderInHierarchyWindow(
              EditorSceneManager.GetSceneByHandle(id))) {
  ...
  void ContextClickOutsideItems() {
    ...
    // added to the last loaded scene in the Hierarchy (otherwise it would be
    // parented to current selcection)
  ...
  void ItemContextClick(int contextClickedItemID) {
    ...
    if (clickedSceneHeader) {
      ...
      GameObject gameObject =
          EditorUtility.InstanceIDToObject(contextClickedItemID) as GameObject;
      var subSceneInfo = SubSceneGUI.GetSubSceneInfo(gameObject);
      if (subSceneInfo.isValid) {
        ...
        if (subSceneInfo.scene.IsValid()) {
          // Sub scenes where the scene object exists can reuse menu for regular
          // scenes.
          CreateSceneHeaderContextClick(menu, subSceneInfo.scene);
        } else {
          // Sub scenes where only the info exists, but not the scene object,
  ...
  void RenameGO() { treeView.BeginNameEditing(0f); }
  ...
  void DeleteGO() { Unsupported.DeleteGameObjectSelection(); }
  ...
  internal static void SetDefaultParentForSession(string sceneGUID,
                                                  int instanceID) {
    SessionState.SetInt(GetDefaultParentKeyForScene(sceneGUID), instanceID);
  }

  internal static void
  UpdateSessionStateInfoAndActiveParentObjectValuesForScene(string sceneGUID,
                                                            int id) {
  ...
  bool UserAllowedDiscardingChanges(Scene[] modifiedScenes) {
    ...
    string message = LocalizationDatabase.GetLocalizedString(
        "Are you sure you want to discard the changes in the following scenes:\n\n   {0}\n\nYour changes will be lost.");
    ...
    return EditorUtility.DisplayDialog(
        title, message, LocalizationDatabase.GetLocalizedString("OK"),
  ...
  void DiscardChangesInSelectedScenes(object userData) {
    ...
    var selectedSceneHandles = GetSelectedScenes();
    var modifiedScenes = GetModifiedScenes(selectedSceneHandles);
    var modifiedScenesWithSavePath =
        modifiedScenes.Where(scene => !string.IsNullOrEmpty(scene.path))
            .ToArray();
    ...
    if (modifiedScenesWithSavePath.Length != modifiedScenes.Length)
      Debug.LogWarning(
          "Discarding changes in a scene that have not yet been saved is not supported. Save the scene first or create a new scene.");

    foreach (var scene in modifiedScenesWithSavePath) {
      EditorSceneManager.ReloadScene(scene);
    }

    // When reloading a single scene it will be given a new scene handle which
    // will collapse it in the Hierarchy. Here we ensure same scene are expanded
    if (SceneManager.sceneCount == 1)
  ...
  Scene[] GetModifiedScenes(List<int> handles) {
    return handles.Select(handle => EditorSceneManager.GetSceneByHandle(handle))
  ...
  void CloseSelectedScenes(bool removeScenes) {
    ...
    foreach (var id in selectedSceneHandles)
      EditorSceneManager.CloseScene(EditorSceneManager.GetSceneByHandle(id),
                                    removeScenes);
  ...
  void AddNewScene(object userData) {
    ...
    if (untitledScene.IsValid()) {
      ...
      var subTitle =
          EditorGUIUtility
              .TrTextContent(
                  "Existing Untitled scene needs to be saved before creating a new scene. Only one untitled scene is supported at a time.")
              .text;
  ...
  void InvertSelection() {
    int[] instanceIDs =
        treeView.GetRowIDs().Except(treeView.GetSelection()).ToArray();
    ...
    foreach (var id in treeView.GetSelection()) {
      ...
      if (IsSceneHeaderInHierarchyWindow(scene) && scene.isLoaded) {
        foreach (var rootGameObject in scene.GetRootGameObjects()) {
          if (rootGameObject.transform.GetComponentsInChildren<Transform>(true)
                  .Length > 1)
            return true;
        }
      } else {
        var go =
            InternalEditorUtility.GetObjectFromInstanceID(id) as GameObject;
        if (go != null) {
      ...
      if (IsSceneHeaderInHierarchyWindow(scene)) {
        foreach (var rootGameObject in scene.GetRootGameObjects()) {
          ...
          instanceIDs.AddRange(
              rootGameObject.transform.GetComponentsInChildren<Transform>(true)
                  .Select(t => t.gameObject.GetInstanceID()));
        }
      } else {
        var go =
            InternalEditorUtility.GetObjectFromInstanceID(id) as GameObject;
        if (go != null)
          instanceIDs.AddRange(
              go.transform.GetComponentsInChildren<Transform>(true).Select(
                  t => t.gameObject.GetInstanceID()));
      }
    }
  ...
  public virtual void AddItemsToWindowMenu(GenericMenu menu) {
    ...
    menu.AddItem(Styles.renamingEnabledContent,
                 SceneHierarchyWindow.s_EnterRenameModeForNewGO,
                 SceneHierarchyWindow.SwitchEnterRenameModeForNewGO);

    if (Unsupported.IsDeveloperMode()) {
      menu.AddItem(new GUIContent("DEVELOPER/Debug Mode - Hierarchy "), s_Debug,
                   () => s_Debug = !s_Debug);
      menu.AddItem(new GUIContent("DEVELOPER/Debug Mode - Prefab Scene"),
                   s_DebugPrefabStage,
                   () => s_DebugPrefabStage = !s_DebugPrefabStage);
      menu.AddItem(
          new GUIContent("DEVELOPER/Debug Mode - Expanded State Persistence"),
  ...
  private void FrameObjectPrivate(int instanceID, bool frame, bool ping,
    ...
    FrameObjectPrivate(
        InternalEditorUtility.GetGameObjectInstanceIDFromComponent(instanceID),
        frame, ping, animatedFraming);
  }
  ...
  float DoSearchResultPathGUI() {
    ...
    rowRect.y = treeViewRect.yMax - height +
                (rowHeight - EditorGUI.kSingleLineHeight) / 2;
  ...
  static class TransformPath {
    public static void AddGameObjectNames(int gameObjectInstanceID,
                                          List<string> list) {
      var gameObject = InternalEditorUtility.GetObjectFromInstanceID(
          gameObjectInstanceID) as GameObject;
      if (gameObject == null)
        return;
    ...
    public static void AddGameObjectNames(GameObject target,
                                          List<string> list) {
