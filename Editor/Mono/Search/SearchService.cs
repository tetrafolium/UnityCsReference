...
public enum SearchEngineScope { Scene, Project, ObjectSelector }

public interface ISearchContext {
Guid guid {
	get;
}
SearchEngineScope engineScope {
	get;
}
...
IEnumerable<T> Search(ISearchContext context, string query,
                      Action<IEnumerable<T> > asyncItemsReceived);
}
...
bool SelectObject(ISearchContext context,
                  Action<Object, bool> onObjectSelectorClosed,
                  Action<Object> onObjectSelectedUpdated);
...
void Init() {
	var index = SearchService.searchApis.FindIndex(api => api.engineScope ==
	                                               m_EngineScope);
	if (index >= 0) {
	...
	if (m_Api == null) {
		...
		if (m_Api == null)
			throw new NullReferenceException(
				      "SearchService Apis were not initialized properly.");
	}
...
IEnumerable<ISearchEngineBase> engines {
	get;
}
string activeSearchEngineName {
	get;
}
...
public const string activeSearchEnginesPrefKey =
	keyPrefix + ".activeengines.";
public static List<ISearchApi> searchApis = new List<ISearchApi>();
}

class SearchApiBaseImp<TAttribute, TEngine> : ISearchApi where TAttribute
	: Attribute where TEngine : class,
	                                      ISearchEngineBase {
int m_SearchPerformanceTrackerHandle;

public SearchEngineScope engineScope {
	get;
}
...
List<TEngine> engines {
	get;
}
        = new List<TEngine>();

IEnumerable<ISearchEngineBase> ISearchApi.engines =>
engines.Cast<ISearchEngineBase>();

public string activeSearchEngineName {
	get;
	private set;
}
...
public TEngine GetActiveSearchEngine() {
	if (engines.Count == 0)
		return null;
	var actualActiveEngineName = string.IsNullOrEmpty(activeSearchEngineName)
	                               ? GetDefaultEngine().name
	                               : activeSearchEngineName;
	var activeEngineIndex =
		engines.FindIndex(engine => engine.name == actualActiveEngineName);
...
public TEngine GetDefaultEngine() {
	...
	var index =
		engines.FindIndex(engine => engine is DefaultSearchEngineBase);
	if (index < 0)
		return null;
...
public void SetActiveSearchEngine(string searchEngineName) {
	...
	EditorPrefs.SetString(SearchService.activeSearchEnginesPrefKey +
	                      engineScope,
	                      searchEngineName);
...
public void BeginSearch(string query, ISearchContext context) {
	m_SearchPerformanceTrackerHandle = EditorPerformanceTracker.StartTracker(
		$"Search Engine {activeSearchEngineName} ({displayName})");
	try {
...
public void EndSearch(ISearchContext context) {
	...
	if (EditorPerformanceTracker.IsTrackerActive(
		    m_SearchPerformanceTrackerHandle))
		EditorPerformanceTracker.StopTracker(m_SearchPerformanceTrackerHandle);
}

void LoadActiveSearchEngine() {
	activeSearchEngineName = EditorPrefs.GetString(
		SearchService.activeSearchEnginesPrefKey + engineScope,
		GetDefaultEngine().name);
}
...
public void HandleUserException(Exception ex) {
	...
	Debug.LogFormat(
		LogType.Error, LogOption.None, null,
		$"Exception caught with search engine ({displayName}){activeSearchEngine.name}:\n{ex}");
	Debug.LogFormat(LogType.Error, LogOption.None, null,
	                $"Setting {GetDefaultEngine().name} engine as active.");
