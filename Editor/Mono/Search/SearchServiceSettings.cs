...
static SettingsProvider CreateSearchSettings() {
	var settings =
		new SettingsProvider(settingsPreferencesKey, SettingsScope.User){
		keywords = new[] {"search"},
		guiHandler = searchContext => {EditorGUIUtility.labelWidth = 500;
	...
	foreach (var api in orderedApis) {
		...
		using (new EditorGUILayout.HorizontalScope()) {
			try {
				var items =
					searchEngines
					.Select(se => new GUIContent(
							se.name,
							searchEngines.Count == 1
				? $"Search engine for {searchContextName}"
				: $"Set search engine for {searchContextName}"))
					.ToArray();
				var activeEngine = api.GetActiveSearchEngine();
				var activeEngineIndex =
					Math.Max(searchEngines.FindIndex(engine => engine.name ==
					                                 activeEngine?.name),
					         0);

				GUILayout.Space(20);
				GUILayout.Label(new GUIContent(searchContextName),
				                GUILayout.Width(175));
				GUILayout.Space(20);

				using (var scope = new EditorGUI.ChangeCheckScope()) {
					var newSearchEngine = EditorGUILayout.Popup(
						activeEngineIndex, items, GUILayout.ExpandWidth(true));
					if (scope.changed) {
...
static List<ISearchEngineBase>
OrderSearchEngines(IEnumerable<ISearchEngineBase> engines) {
	var defaultEngine =
		engines.First(engine => engine is DefaultSearchEngineBase);
	var overrides = engines.Where(engine => !(engine is DefaultSearchEngineBase));
	var orderedSearchEngines = new List<ISearchEngineBase> {
		defaultEngine
	};
