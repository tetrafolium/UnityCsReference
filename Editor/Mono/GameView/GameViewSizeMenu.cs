...
// Resolution/Aspect ratio menu for the GameView, with an optional toggle for
// low-resolution aspect ratios
internal class GameViewSizeMenu : FlexibleMenu {
static class Styles {
public static GUIContent vSyncToggleContent =
...
float frameHeight {
	get {
		return kTopMargin * 2 +
		       EditorGUI.kSingleLineHeight * (IsVSyncToggleVisible() ? 2 : 1);
	}
}
float contentOffset {
	get { return frameHeight + EditorGUI.kControlVerticalSpacing; }
}

public GameViewSizeMenu(IFlexibleMenuItemProvider itemProvider,
                        int selectionIndex,
                        FlexibleMenuModifyItemUI modifyItemUi,
                        IGameViewSizeMenuUser gameView)
	: base(itemProvider, selectionIndex, modifyItemUi,
	       gameView.SizeSelectionCallback) {
	m_GameView = gameView;
}

public override Vector2 GetWindowSize() {
	var lowAspectRatiosContentSize = EditorStyles.toggle.CalcSize(
		GameView.Styles.lowResAspectRatiosContextMenuContent);
	var size = CalcSize();
...
private void DoVSyncToggle(Rect rect) {
	if (!IsVSyncToggleVisible())
		return;
	var toggleRect = new Rect(rect.xMin, rect.yMax + 2, rect.width,
	                          EditorGUI.kSingleLineHeight);
	m_GameView.vSyncEnabled = GUI.Toggle(toggleRect, m_GameView.vSyncEnabled,
	                                     Styles.vSyncToggleContent);
}
...
public override void OnGUI(Rect rect) {
	...
	var toggleRect =
		new Rect(kMargin, kTopMargin, rect.width, EditorGUI.kSingleLineHeight);
	m_GameView.lowResolutionForAspectRatios =
		GUI.Toggle(toggleRect,
		           m_GameView.forceLowResolutionAspectRatios ||
		           m_GameView.lowResolutionForAspectRatios,
		           GameView.Styles.lowResAspectRatiosContextMenuContent);
