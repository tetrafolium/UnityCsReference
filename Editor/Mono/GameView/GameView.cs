...
internal class GameView : PlayModeView, IHasCustomMenu, IGameViewSizeMenuUser
{
    ...
    {
        get {
            return Mathf.Max(kMaxScale * EditorGUIUtility.pixelsPerPoint, ScaleThatFitsTargetInView(targetRenderSize, viewInWindow.size));
        }
    }
    ...
    Vector2 gameMouseOffset {
        get {
            return -viewInWindow.position - targetInView.position;
        }
    }

    float gameMouseScale {
        get {
            return EditorGUIUtility.pixelsPerPoint / m_ZoomArea.scale.y;
        }
    }
