...
internal class GameView : PlayModeView,
                          IHasCustomMenu,
                          IGameViewSizeMenuUser {
  ...
  // This will save the previous state which will be useful in case of platform
  // switch
  ...
  float minScale {
    get {
      var clampedMinScale =
          Mathf.Min(kMinScale, ScaleThatFitsTargetInView(targetRenderSize,
                                                         viewInWindow.size));
      if (m_LowResolutionForAspectRatios[(int) currentSizeGroupType] &&
          currentGameViewSize.sizeType == GameViewSizeType.AspectRatio)
        clampedMinScale = Mathf.Max(
            clampedMinScale, Mathf.Floor(EditorGUIUtility.pixelsPerPoint));
  ...
  float maxScale {
    get {
      return Mathf.Max(
          kMaxScale * EditorGUIUtility.pixelsPerPoint,
          ScaleThatFitsTargetInView(targetRenderSize, viewInWindow.size));
    }
  }
  ...
  [SerializeField] int[] m_SelectedSizes =
      new int[0]; // We have a selection for each game view size group (e.g
                  // standalone, android etc)
  ...
  internal static class Styles {
    public static GUIContent gizmosContent =
        EditorGUIUtility.TrTextContent("Gizmos");
    public static GUIContent zoomSliderContent = EditorGUIUtility.TrTextContent(
        "Scale", "Size of the game view on the screen.");
    public static GUIContent vsyncContent =
        EditorGUIUtility.TrTextContent("VSync");
    public static GUIContent maximizeOnPlayContent =
        EditorGUIUtility.TrTextContent("Maximize On Play");
    public static GUIContent muteContent =
        EditorGUIUtility.TrTextContent("Mute Audio");
    public static GUIContent statsContent =
        EditorGUIUtility.TrTextContent("Stats");
    public static GUIContent frameDebuggerOnContent =
        EditorGUIUtility.TrTextContent("Frame Debugger On");
    public static GUIContent loadRenderDocContent =
        EditorGUIUtility.TrTextContent(
            UnityEditor.RenderDocUtil.loadRenderDocLabel);
    public static GUIContent noCameraWarningContextMenuContent =
        EditorGUIUtility.TrTextContent("Warn if No Cameras Rendering");
    public static GUIContent clearEveryFrameContextMenuContent =
        EditorGUIUtility.TrTextContent("Clear Every Frame in Edit Mode");
    public static GUIContent lowResAspectRatiosContextMenuContent =
        EditorGUIUtility.TrTextContent("Low Resolution Aspect Ratios");
    ...
    // The ordering here must correspond with ordering in
    // UnityEngine.XR.GameViewRenderMode
    public static GUIContent[] xrRenderingModes = {
        EditorGUIUtility.TextContent(
            "Left Eye|Left eye is displayed in play mode."),
        EditorGUIUtility.TextContent(
            "Right Eye|Right eye is displayed in play mode."),
        EditorGUIUtility.TextContent(
            "Both Eyes|Both eyes are displayed in play mode."),
        EditorGUIUtility.TextContent(
            "Occlusion Mesh|Both eyes are displayed in play mode along with the occlusion mesh.")};
  ...
  static GameViewSizeGroupType currentSizeGroupType =>
      GameViewSizes.instance.currentGroupType;

  GameViewSize currentGameViewSize =>
      GameViewSizes.instance.currentGroup.GetGameViewSize(selectedSizeIndex);

  // The area of the window that the rendered game view is limited to
  Rect viewInWindow {
    get {
      if (showToolbar)
        return new Rect(0, EditorGUI.kWindowToolbarHeight, position.width,
  ...
  {
    get {
      var viewPixelRect = lowResolutionForAspectRatios ? viewInWindow
          : EditorGUIUtility.PointsToPixels(viewInWindow);
      return GameViewSizes.GetRenderTargetSize(
          viewPixelRect, currentSizeGroupType, selectedSizeIndex,
          out m_TargetClamped);
    }
  }

  // Area of the render target in zoom content space (it is centered in content
  // space)
  Rect targetInContent {
    get {
      var targetSizeCached = targetRenderSize;
      return EditorGUIUtility.PixelsToPoints(
          new Rect(-0.5f * targetSizeCached, targetSizeCached));
    }
  }
  ...
  Rect targetInView // Area of the render target in zoom view space
  {
    get {
      ...
      return new Rect(
          m_ZoomArea.DrawingToViewTransformPoint(
              targetInContentCached.position),
          m_ZoomArea.DrawingToViewTransformVector(targetInContentCached.size));
    }
  }

  // The final image needs to be flipped upside-down if we use coordinates with
  // zero at the top
  ...
  Rect viewInParent // The view area in parent view space
  {
    get {
      ...
      // Note: DockArea has extra space on top of the tab area which is
      // accounted for strangely. It is stored in borderSize.bottom but it
      // should be added to the top instead
      // @TODO: Make DockArea.borderSize more sensible (requires fixing all
      // usages)
  ...
  Rect targetInParent // Area of the render target in parent view space
  {
    get {
      ...
      return new Rect(targetInViewCached.position + viewInParent.position,
                      targetInViewCached.size);
    }
  }

  // Area for warnings such as no cameras rendering
  Rect warningPosition {
    get {
      return new Rect((viewInWindow.size - kWarningSize) * 0.5f, kWarningSize);
    }
  }

  Vector2 gameMouseOffset {
    get { return -viewInWindow.position - targetInView.position; }
  }

  float gameMouseScale {
    get { return EditorGUIUtility.pixelsPerPoint / m_ZoomArea.scale.y; }
  }
  ...
  public void OnEnable() {
    ...
    UpdateZoomAreaAndParent();
    showToolbar =
        ModeService.HasCapability(ModeCapability.GameViewToolbar, true);
  ...
  [UsedImplicitly] // This is here because NGUI uses it via reflection (noted in
                   // https://confluence.hq.unity3d.com/display/DEV/Game+View+Bucket)
      internal static Vector2 GetSizeOfMainGameView() {
    return GetMainPlayModeViewTargetSize();
  }

  private void UpdateZoomAreaAndParent() {
    // Configure ZoomableArea for new resolution so that old resolution doesn't
    // restrict scale
    bool oldScaleWasDefault =
        Mathf.Approximately(m_ZoomArea.scale.y, m_defaultScale);
    ConfigureZoomArea();
    m_defaultScale =
        DefaultScaleForTargetInView(targetRenderSize, viewInWindow.size);
  ...
  protected void AllowCursorLockAndHide(bool enable) {
    Unsupported.SetAllowCursorLock(enable,
                                   Unsupported.DisallowCursorLockReasons.Other);
  ...
  private void OnLostFocus() {
    // We unlock the cursor when the game view loses focus to allow the user to
    // regain cursor control. Fix for 389362: Ensure that we do not unlock
    // cursor during init of play mode. Because we could be maximizing game
    // view, which causes a lostfocus on the original game view, in these
    // situations we do not unlock cursor.
  ...
  internal override void OnResized() { targetSize = targetRenderSize; }

  // Call when number of available aspects can have changed (after
  // deserialization or gui change)
  private void EnsureSelectedSizeAreValid() {
    ...
    foreach (GameViewSizeGroupType groupType in sizeGroupTypes) {
      ...
      m_SelectedSizes[index] =
          Mathf.Clamp(m_SelectedSizes[index], 0, gvsg.GetTotalCount() - 1);
    }

    // Resize low resolution array as necessary, and fill in new indices with
    // defaults
    ...
    if (m_LowResolutionForAspectRatios.Length != sizeGroupTypes.Length)
      System.Array.Resize(ref m_LowResolutionForAspectRatios,
                          sizeGroupTypes.Length);
    for (var groupIndex = lowResolutionSettingsLength;
         groupIndex < sizeGroupTypes.Length; ++groupIndex)
      m_LowResolutionForAspectRatios[groupIndex] =
          GameViewSizes.DefaultLowResolutionSettingForSizeGroupType(
  ...
  public virtual void AddItemsToMenu(GenericMenu menu) {
    ...
    menu.AddItem(Styles.noCameraWarningContextMenuContent, m_NoCameraWarning,
                 ToggleNoCameraWarning);
    menu.AddItem(Styles.clearEveryFrameContextMenuContent, m_ClearInEditMode,
                 ToggleClearInEditMode);
  }
  ...
  void SnapZoomDelayed() {
    if (EditorApplication.timeSinceStartup >
        s_LastScrollTime + kScrollZoomSnapDelay) {
  ...
  void SnapZoom(float newZoom) {
    ...
    if (logScale > logMin && logScale < logMax) {
      for (var i = 1; i <= maxScale; ++i) {
        // Snap distance is defined in points, so convert difference back to UI
        // space
        var sliderDistanceToI = kScaleSliderMaxWidth *
                                Mathf.Abs(logScale - Mathf.Log10(i)) /
                                (logMax - logMin);
        if (sliderDistanceToI < kScaleSliderSnapThreshold &&
            sliderDistanceToI < shortestSliderDistance) {
          newZoom = i;
  ...
  void DoZoomSlider() {
    ...
    var logMin = Mathf.Log10(minScale);
    var logMax = Mathf.Log10(maxScale);
    logScale = GUILayout.HorizontalSlider(
        logScale, logMin, logMax, GUILayout.MaxWidth(kScaleSliderMaxWidth),
        GUILayout.MinWidth(kScaleSliderMinWidth));
    if (EditorGUI.EndChangeCheck()) {
      var newZoom = Mathf.Pow(10f, logScale);
      SnapZoom(newZoom);
    }
    var scaleContent = EditorGUIUtility.TempContent(UnityString.Format(
        "{0}x",
        (m_ZoomArea.scale.y)
            .ToString("G2", CultureInfo.InvariantCulture.NumberFormat)));
  ...
  private bool ShouldShowMetalFrameCaptureGUI() {
    return FrameCapture.IsDestinationSupported(
  ...
  private void DoToolbarGUI() {
    ...
    GUILayout.BeginHorizontal(EditorStyles.toolbar);
    {
      ...
      if (availableTypes.Count > 1) {
        ...
        int viewIndex = EditorGUILayout.Popup(
            typeNames.IndexOf(titleContent.text), typeNames.ToArray(),
      ...
      if (ModuleManager.ShouldShowMultiDisplayOption()) {
        int display = EditorGUILayout.Popup(
            targetDisplay, DisplayUtility.GetDisplayNames(),
            EditorStyles.toolbarPopupLeft, GUILayout.Width(80));
        if (display != targetDisplay) {
          targetDisplay = display;
          UpdateZoomAreaAndParent();
        }
      }
      EditorGUILayout.GameViewSizePopup(currentSizeGroupType, selectedSizeIndex,
                                        this, EditorStyles.toolbarPopup,
      ...
      if (FrameDebuggerUtility.IsLocalEnabled()) {
        ...
        // This has nothing to do with animation recording.  Can we replace this
        // color with something else?
        GUI.color *= AnimationMode.recordedPropertyColor;
        GUILayout.Label(Styles.frameDebuggerOnContent,
                        EditorStyles.toolbarLabel);
        GUI.color = oldCol;
        // Make frame debugger windows repaint after each time game view
        // repaints. We want them to always display the latest & greatest game
      ...
      // Allow the user to select how the XR device will be rendered during
      // "Play In Editor"
      if (PlayerSettings.virtualRealitySupported ||
          (m_DisplaySubsystems.Count != 0 &&
           !m_DisplaySubsystems[0].disableLegacyRenderer)) {
        EditorGUI.BeginChangeCheck();
        GameViewRenderMode currentGameViewRenderMode =
            UnityEngine.XR.XRSettings.gameViewRenderMode;
        int selectedRenderMode = EditorGUILayout.Popup(
            Mathf.Clamp(((int) currentGameViewRenderMode) - 1, 0,
                        Styles.xrRenderingModes.Length - 1),
            Styles.xrRenderingModes, EditorStyles.toolbarPopup,
            GUILayout.Width(80));
        if (EditorGUI.EndChangeCheck() &&
            currentGameViewRenderMode != GameViewRenderMode.None) {
          SetXRRenderMode(selectedRenderMode);
        }
      }
      // Handles the case where XRSDK is being used without the shim layer
      else if (m_DisplaySubsystems.Count != 0 &&
               m_DisplaySubsystems[0].disableLegacyRenderer) {
        EditorGUI.BeginChangeCheck();
        int currentMirrorViewBlitMode = m_DisplaySubsystems [0]
                                            .GetPreferredMirrorBlitMode();
        int currentRenderMode = XRTranslateMirrorViewBlitModeToRenderMode(
            currentMirrorViewBlitMode);
        int selectedRenderMode = EditorGUILayout.Popup(
            Mathf.Clamp(currentRenderMode, 0,
                        Styles.xrRenderingModes.Length - 1),
            Styles.xrRenderingModes, EditorStyles.toolbarPopup,
            GUILayout.Width(80));
        int selectedMirrorViewBlitMode =
            XRTranslateRenderModeToMirrorViewBlitMode(selectedRenderMode);
        if (EditorGUI.EndChangeCheck() || currentMirrorViewBlitMode == 0) {
          m_DisplaySubsystems [0]
              .SetPreferredMirrorBlitMode(selectedMirrorViewBlitMode);
      ...
      maximizeOnPlay =
          GUILayout.Toggle(maximizeOnPlay, Styles.maximizeOnPlayContent,
                           EditorStyles.toolbarButton);

      EditorUtility.audioMasterMute =
          GUILayout.Toggle(EditorUtility.audioMasterMute, Styles.muteContent,
                           EditorStyles.toolbarButton);

      m_Stats = GUILayout.Toggle(m_Stats, Styles.statsContent,
                                 EditorStyles.toolbarButton);

      if (EditorGUILayout.DropDownToggle(
              ref m_Gizmos, Styles.gizmosContent,
              EditorStyles.toolbarDropDownToggleRight)) {
  ...
  private int
  XRTranslateMirrorViewBlitModeToRenderMode(int mirrorViewBlitMode) {
  ...
  private void SetXRRenderMode(int mode) {
    ...
    default: // or 0
      UnityEngine.XR.XRSettings.gameViewRenderMode =
          UnityEngine.XR.GameViewRenderMode.LeftEye;
      break;
    case 1:
      UnityEngine.XR.XRSettings.gameViewRenderMode =
          UnityEngine.XR.GameViewRenderMode.RightEye;
      break;
    case 2:
      UnityEngine.XR.XRSettings.gameViewRenderMode =
          UnityEngine.XR.GameViewRenderMode.BothEyes;
      break;
    case 3:
      UnityEngine.XR.XRSettings.gameViewRenderMode =
          UnityEngine.XR.GameViewRenderMode.OcclusionMesh;
      break;
    }
  ...
  private float ScaleThatFitsTargetInView(Vector2 targetInPixels,
                                          Vector2 viewInPoints) {
    var targetInPoints = EditorGUIUtility.PixelsToPoints(targetInPixels);
    var viewToTargetRatio = new Vector2(viewInPoints.x / targetInPoints.x,
                                        viewInPoints.y / targetInPoints.y);
    return Mathf.Min(viewToTargetRatio.x, viewToTargetRatio.y);
  }

  private float DefaultScaleForTargetInView(Vector2 targetToFit,
                                            Vector2 viewSize) {
    var scale = ScaleThatFitsTargetInView(targetToFit, viewSize);
    if (scale > 1f) {
      scale = Mathf.Min(maxScale * EditorGUIUtility.pixelsPerPoint,
                        Mathf.FloorToInt(scale));
    }
    return scale;
  }
  ...
  private void EnforceZoomAreaConstraints() {
    ...
    {
      shownArea.x = Mathf.Clamp(shownArea.x, targetInContentCached.xMin,
                                targetInContentCached.xMax - shownArea.width);
    }
    // Horizontal and vertical are separated because otherwise we get weird
    // behaviour when only one is zoomed out
    if (shownArea.height > targetInContent.height) {
      shownArea.y = -0.5f * shownArea.height;
    } else {
      shownArea.y = Mathf.Clamp(shownArea.y, targetInContentCached.yMin,
                                targetInContentCached.yMax - shownArea.height);
    }
  ...
  private void OnPlayModeStateChanged(PlayModeStateChange state) {
    if (state == PlayModeStateChange.EnteredPlayMode) {
      // Enable vsync in play mode to get as much as possible frame rate
      // consistency
  ...
  private void OnGUI() {
    if (position.size * EditorGUIUtility.pixelsPerPoint !=
        m_LastWindowPixelSize) // pixelsPerPoint only reliable in OnGUI()
    {
      UpdateZoomAreaAndParent();
    }
    ...
    // This isn't ideal. Custom Cursors set by editor extensions for other
    // windows can leak into the game view. To fix this we should probably stop
    // using the global custom cursor (intended for runtime) for custom editor
    // cursors. This has been noted for Cursors tech debt.
    ...
    if (type == EventType.MouseDown &&
        viewInWindow.Contains(Event.current.mousePosition)) {
      AllowCursorLockAndHide(true);
    }
    ...
    // We hide sliders when playing, and also when we are zoomed out beyond
    // canvas edges
    var playing = EditorApplication.isPlaying && !EditorApplication.isPaused;
    var targetInContentCached = targetInContent;
    m_ZoomArea.hSlider =
        !playing && m_ZoomArea.shownArea.width < targetInContentCached.width;
    m_ZoomArea.vSlider =
        !playing && m_ZoomArea.shownArea.height < targetInContentCached.height;
    m_ZoomArea.enableMouseInput = !playing;
    ConfigureZoomArea();

    // We don't want controls inside the GameView (e.g. the toolbar) to have
    // keyboard focus while playing. The game should get the keyboard events.
    if (playing)
    ...
    if (m_Parent) {
      var zoomedTarget = new Rect(
          targetInView.position + gameViewTarget.position, targetInView.size);
      SetParentGameViewDimensions(zoomedTarget, gameViewTarget,
                                  targetRenderSize);
    }

    var editorMousePosition = Event.current.mousePosition;
    var gameMousePosition =
        (editorMousePosition + gameMouseOffset) * gameMouseScale;

    if (type == EventType.Repaint) {
      GUI.Box(m_ZoomArea.drawRect, GUIContent.none,
              Styles.gameViewBackgroundStyle);
      ...
      if (ModuleManager.ShouldShowMultiDisplayOption()) {
        ...
        System.Diagnostics.Debug.Assert(targetDisplay < 8,
                                        "Display Target is Out of Range");
      ...
      if (!EditorApplication.isPlaying ||
          (EditorApplication.isPlaying &&
           Time.frameCount % OnDemandRendering.renderFrameInterval == 0))
        m_RenderTexture = RenderView(gameMousePosition, clearTexture);

      if (m_TargetClamped)
        Debug.LogWarningFormat(
            "GameView reduced to a reasonable size for this system ({0}x{1})",
            targetSize.x, targetSize.y);
      EditorGUIUtility.SetupWindowSpaceAndVSyncInternal(
      ...
      if (m_RenderTexture != null && m_RenderTexture.IsCreated()) {
        ...
        Graphics.DrawTexture(drawRect, m_RenderTexture, new Rect(0, 0, 1, 1), 0,
                             0, 0, 0, GUI.color, GUI.blitMaterial);
        GUI.EndGroup();
      }
      ...
      bool mousePosInGameViewRect =
          viewInWindow.Contains(Event.current.mousePosition);

      // MouseDown events outside game view rect are not send to scripts but
      // MouseUp events are (see below)
      if (Event.current.rawType == EventType.MouseDown &&
          !mousePosInGameViewRect)
        return;
      ...
      // Do not use mouse UP event if mousepos is outside game view rect (fix
      // for case 380995: Gameview tab's context menu is not appearing on right
      // click) Placed after event queueing above to ensure scripts can react on
      // mouse up events.
      bool useEvent = !(Event.current.rawType == EventType.MouseUp &&
                        !mousePosInGameViewRect);
    ...
    if (originalEventType == EventType.ScrollWheel &&
    ...
    if (m_NoCameraWarning &&
        !EditorGUIUtility.IsDisplayReferencedByCameras(targetDisplay)) {
      GUI.Label(warningPosition, GUIContent.none,
                EditorStyles.notificationBackground);
      var displayName =
          ModuleManager.ShouldShowMultiDisplayOption()
              ? DisplayUtility.GetDisplayNames()[targetDisplay].text
              : string.Empty;
      var cameraWarning =
          string.Format("{0}\nNo cameras rendering", displayName);
      EditorGUI.DoDropShadowLabel(warningPosition,
                                  EditorGUIUtility.TempContent(cameraWarning),
                                  EditorStyles.notificationText, .3f);
    }
