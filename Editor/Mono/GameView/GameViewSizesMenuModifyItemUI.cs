...
internal class GameViewSizesMenuModifyItemUI : FlexibleMenuModifyItemUI {
  private static class Styles {
    ...
    public static GUIContent headerEdit =
        EditorGUIUtility.TrTextContent("Edit");
    public static GUIContent typeName = EditorGUIUtility.TrTextContent("Type");
    public static GUIContent widthHeightText =
        EditorGUIUtility.TrTextContent("Width & Height");
    public static GUIContent optionalText =
        EditorGUIUtility.TrTextContent("Label");
    ...
    public static GUIContent[] typeNames =
        new[]{EditorGUIUtility.TrTextContent("Aspect Ratio"),
              EditorGUIUtility.TrTextContent("Fixed Resolution")};

    public static SVC<float> windowWidth = new SVC<float>(
        "GameView", "--sizes-menu-modify-item-window-width", 230f);
    public static SVC<float> windowHeight = new SVC<float>(
        "GameView", "--sizes-menu-modify-item-window-height", 140f);
    public static SVC<float> windowBottomPadding =
        new SVC<float>("GameView", "--window-bottom-padding");
    public static SVC<float> spaceBetweenOkCancelButtons =
        new SVC<float>("GameView", "--space-between-ok-cancel-buttons", 10f);
  }
  ...
  override public void OnGUI(Rect rect) {
    ...
    // We use a local gameviewsize to ensure we do not edit the original state
    // (if user presses cancel state is not changed)
    ...
    GUILayout.Space(3);
    GUILayout.Label(m_MenuType == MenuType.Add ? Styles.headerAdd
                    : Styles.headerEdit, EditorStyles.boldLabel);

    Rect seperatorRect = GUILayoutUtility.GetRect(1, 1);
    FlexibleMenu.DrawRect(
        seperatorRect,
    ...
    m_GameViewSize.baseText =
        EditorGUILayout.TextField(m_GameViewSize.baseText);
    ...
    GUILayout.Space(kSpacing);
    m_GameViewSize.sizeType = (GameViewSizeType) EditorGUILayout.Popup(
        (int) m_GameViewSize.sizeType, Styles.typeNames);
    GUILayout.EndHorizontal();
    ...
    using(new EditorGUI.DisabledScope(string.IsNullOrEmpty(displayText))) {
      if (string.IsNullOrEmpty(displayText))
        ...
        displayText =
            GetCroppedText(displayText, cropWidth, EditorStyles.label);
  ...
  string GetCroppedText(string fullText, float cropWidth, GUIStyle style) {
    ...
    int characterCountVisible =
        style.GetNumCharactersThatFitWithinWidth(fullText, cropWidth);
    if (characterCountVisible == -1) {
      return fullText;
    }

    if (characterCountVisible > 1 && characterCountVisible != fullText.Length)
      return fullText.Substring(0, characterCountVisible - 1) +
