...
internal enum GameViewSizeType { AspectRatio, FixedResolution }
...
public GameViewSize(GameViewSizeType type, int width, int height,
                    string baseText) {
	this.sizeType = type;
	this.width = width;
	this.height = height;
...
public string displayText {
	get {
		return m_CachedDisplayText ??(m_CachedDisplayText =
						      ComposeDisplayString());
	}
}
...
private string ComposeDisplayString() {
	...
	if (sizeType == GameViewSizeType.FixedResolution) {
		if (baseText.Contains($"{width}x{height}") ||
		    baseText.Contains($"{height}x{width}"))
			return baseText;
	}
	if (sizeType == GameViewSizeType.AspectRatio) {
		if (baseText.Contains($"{width}:{height}") ||
		    baseText.Contains($"{height}:{width}"))
			return baseText;
	}
