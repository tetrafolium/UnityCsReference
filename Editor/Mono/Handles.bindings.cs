...
public static extern Matrix4x4 inverseMatrix {
	get;
}
...
internal static extern bool IsCameraDrawModeEnabled(Camera camera,
                                                    DrawCameraMode drawMode);

[FreeFunction]
static extern void
Internal_DrawCameraWithGrid(Camera cam, DrawCameraMode renderMode,
...
[FreeFunction]
static extern void
Internal_FinishDrawingCamera(Camera cam,
                             [ DefaultValue("true") ] bool drawGizmos);
static void Internal_FinishDrawingCamera(Camera cam) {
	Internal_FinishDrawingCamera(cam, true);
}
...
SetSceneViewColors(Color wire, Color wireOverlay, Color selectedOutline,
                   Color selectedChildrenOutline, Color selectedWire);

[FreeFunction("Internal_SetSceneViewModeGIContributorsReceiversColors")]
internal static extern void SetSceneViewModeGIContributorsReceiversColors(
	Color noContributeGI, Color receiveGILightmaps,
	Color receiveGILightProbesColor);
...
[FreeFunction]
static extern void
Internal_DrawAAPolyLine(Color[] colors, Vector3[] points, Color defaultColor,
                        int actualNumberOfPoints, Texture2D texture,
                        float width, Matrix4x4 toWorld);

[FreeFunction]
static extern void
Internal_DrawAAConvexPolygon(Vector3[] points, Color defaultColor,
                             int actualNumberOfPoints, Matrix4x4 toWorld);

[FreeFunction]
static extern void
Internal_DrawBezier(Vector3 startPosition, Vector3 endPosition,
                    Vector3 startTangent, Vector3 endTangent, Color color,
                    Texture2D texture, float width, Matrix4x4 toWorld);

[FreeFunction("Internal_SetDiscSectionPoints")]
internal static extern void
SetDiscSectionPoints(Vector3[] dest, Vector3 center, Vector3 normal,
                     Vector3 from, float angle, float radius);

[FreeFunction("Internal_EmitGUIGeometryForCamera")]
internal static extern void EmitGUIGeometryForCamera(Camera source,
                                                     Camera dest);

[FreeFunction("Internal_SetCameraFilterMode")]
internal static extern void SetCameraFilterMode(Camera camera,
...
[FreeFunction]
static extern Vector3[] Internal_MakeBezierPoints(Vector3 startPosition,
                                                  Vector3 endPosition,
