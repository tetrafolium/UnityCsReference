...
public sealed partial class Handles {
  ...
  // Description
  // 'radius' is near-radius at start of cone frustrum (far-radius is derived
  // from 'angle' and near-radius) 'angle' is cone angle from near disc of
  // conefrustrum (if near-radius is 0 then angle is the half angle of view
  // frustrum) 'range' is height of conefrustrum
  internal static Vector3 DoConeFrustrumHandle(Quaternion rotation,
                                               Vector3 position,
                                               Vector3 radiusAngleRange,
                                               ConeHandles showHandles) {
    ...
    float farRadius =
        Mathf.Min(1000f, Mathf.Abs(range * Mathf.Tan(Mathf.Deg2Rad * angle)) +
                             nearRadius);
    if ((showHandles & ConeHandles.Angle) != 0) {
      ...
      if (GUI.changed)
        angle =
            Mathf.Clamp((Mathf.Rad2Deg * Mathf.Atan((farRadius - nearRadius) /
                                                    Mathf.Abs(range))),
                        0.0F, 90f);
      GUI.changed |= temp;
    }
    ...
    DrawLine(position + up * nearRadius,
             (position + forward * range) + up * farRadius);
    DrawLine(position - up * nearRadius,
             (position + forward * range) - up * farRadius);
    DrawLine(position + right * nearRadius,
             (position + forward * range) + right * farRadius);
    DrawLine(position - right * nearRadius,
             (position + forward * range) - right * farRadius);
