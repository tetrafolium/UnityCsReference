...
public sealed partial class Handles {
  internal static float DoSimpleEdgeHandle(Quaternion rotation,
  ...
  internal static float DoSimpleRadiusHandle(Quaternion rotation,
                                             Vector3 position, float radius,
                                             bool hemisphere,
                                             float arc = 360.0f,
                                             bool editable = true) {
    ...
    if (radius > 0) {
      ...
      for (int quarter = 0; quarter < 4; quarter++) {
        if (arc >= (90.0f * quarter)) {
          Vector3 normal =
              Matrix4x4.Rotate(Quaternion.Euler(0.0f, 0.0f, 90.0f * quarter)) *
              up;
      ...
      Vector3 capNormal =
          Matrix4x4.Rotate(Quaternion.Euler(0.0f, 0.0f, arc)) * up;
  ...
  //  {
  //      Vector3 planeNormal = position - Camera.current.transform.position; //
  //      vector from camera to center float sqrDist = planeNormal.sqrMagnitude;
  //      // squared distance from camera to center float sqrRadius = radius *
  //      radius; // squared radius float sqrOffset = sqrRadius * sqrRadius /
  //      sqrDist; // squared distance from actual center to drawn disc center
  //      float insideAmount = sqrOffset / sqrRadius;
  //
  //      // If we are not inside the sphere, calculate where to draw the
  //      periphery if (insideAmount < 1)
  //      {
  //          float drawnRadius = Mathf.Sqrt(sqrRadius - sqrOffset); // the
  ...
  //          DrawWireArc(position - sqrRadius * planeNormal / sqrDist,
  //          planeNormal, tangent, -180, drawnRadius);
  //      }
  //  }
}
}
