...
public sealed partial class Handles {
  internal struct PositionHandleIds {
    ...
    public bool Has(int id) {
      return x == id || y == id || z == id || xy == id || yz == id ||
             xz == id || xyz == id;
    }

    public PositionHandleIds(int x, int y, int z, int xy, int xz, int yz,
                             int xyz) {
      this.x = x;
      this.y = y;
      this.z = z;
    ...
    public override int GetHashCode() { return x ^ y ^ z ^ xy ^ xz ^ yz ^ xyz; }
    ...
    public override bool Equals(object obj) {
      ...
      return o.x == x && o.y == y && o.z == z && o.xy == xy && o.xz == xz &&
             o.yz == yz && o.xyz == xyz;
    }
  }
  ...
  internal struct PositionHandleParam {
    ...
    public static PositionHandleParam DefaultFreeMoveHandle =
        new PositionHandleParam(Handle.X | Handle.Y | Handle.Z | Handle.XYZ,
                                Vector3.zero, Vector3.one, Vector3.zero,
                                Vector3.one * 0.25f, Orientation.Signed,
                                Orientation.Signed);
    ...
    public enum Orientation { Signed, Camera }
    ...
    public bool ShouldShow(Handle handle) { return (handles & handle) != 0; }

    public PositionHandleParam(Handle handles, Vector3 axisOffset,
                               Vector3 axisSize, Vector3 planeOffset,
                               Vector3 planeSize, Orientation axesOrientation,
  ...
  static Vector3[] verts = {Vector3.zero, Vector3.zero, Vector3.zero,
                            Vector3.zero};

  const float kFreeMoveHandleSizeFactor = 0.15f;

  // While the user has Free Move mode turned on by holding 'shift' or 'V' (for
  // Vertex Snapping), this variable will be set to True.
  ...
  // and will disallow toggling Free Move mode on or off, or changing the octant
  // of the planar handles.
  static bool currentlyDragging {
    get { return EditorGUIUtility.hotControl != 0; }
  }
  ...
  internal static Vector3 DoPositionHandle(PositionHandleIds ids,
                                           Vector3 position,
                                           Quaternion rotation) {
    Event evt = Event.current;
    switch (evt.type) {
    case EventType.KeyDown:
      // Holding 'V' turns on the FreeMove transform gizmo and enables vertex
      // snapping.
    ...
    case EventType.KeyUp:
      // If the user has released the 'V' key, then rendering the transform
      // gizmo one last time with Free Move mode off is technically incorrect
      // since it can add one additional frame of input with FreeMove enabled,
      // but the implementation is a fair bit simpler this way. Basic idea:
      // Leave this call above the 'if' statement.
      position = DoPositionHandle_Internal(ids, position, rotation,
                                           PositionHandleParam.DefaultHandle);
    ...
    var param = s_FreeMoveMode ? PositionHandleParam
                    .DefaultFreeMoveHandle : PositionHandleParam.DefaultHandle;
  ...
  static string[] s_DoPositionHandle_Internal_AxisNames = {"xAxis", "yAxis",
                                                           "zAxis"};
  ...
  static Vector3 DoPositionHandle_Internal(PositionHandleIds ids,
                                           Vector3 position,
                                           Quaternion rotation,
                                           PositionHandleParam param) {
    Color temp = color;
    ...
    var viewVectorDrawSpace =
        GetCameraViewFrom(handlePosition, invDrawToWorldMatrix);

    var size = HandleUtility.GetHandleSize(position);

    // Calculate per axis camera lerp
    for (var i = 0; i < 3; ++i)
      s_DoPositionHandle_Internal_CameraViewLerp[i] =
          ids[i] == GUIUtility.hotControl ? 0
          : GetCameraViewLerpForWorldAxis(viewVectorDrawSpace,
                                          GetAxisVector(i));
    // Calculate per plane camera lerp (xy, yz, xz)
    for (var i = 0; i < 3; ++i)
      s_DoPositionHandle_Internal_CameraViewLerp[3 + i] =
          Mathf.Max(s_DoPositionHandle_Internal_CameraViewLerp[i],
                    s_DoPositionHandle_Internal_CameraViewLerp[(i + 1) % 3]);
    ...
    var planeSize =
        isHot ? param.planeSize + param.planeOffset : param.planeSize;
    for (var i = 0; i < 3; ++i) {
      if (!param.ShouldShow(3 + i) ||
          isHot && ids[3 + i] != GUIUtility.hotControl)
        continue;

      var cameraLerp =
          isHot ? 0 : s_DoPositionHandle_Internal_CameraViewLerp[3 + i];
      if (cameraLerp <= kCameraViewThreshold) {
        var offset = planeOffset * size;
        offset[s_DoPositionHandle_Internal_PrevIndex[i]] = 0;
        var planarSize = Mathf.Max(
            planeSize[i], planeSize[s_DoPositionHandle_Internal_NextIndex[i]]);
        position = DoPlanarHandle(ids[3 + i], i, position, offset, rotation,
                                  size * planarSize, cameraLerp,
                                  viewVectorDrawSpace, param.planeOrientation);
      }
    }

    // Draw axis sliders last to have priority over the planes
    CalcDrawOrder(viewVectorDrawSpace,
                  s_DoPositionHandle_Internal_AxisDrawOrder);
    for (var ii = 0; ii < 3; ++ii) {
      ...
      if (!currentlyDragging) {
        ...
        case PositionHandleParam.Orientation.Camera:
          s_DoPositionHandle_AxisHandlesOctant[i] =
              viewVectorDrawSpace[i] > 0.01f ? -1 : 1;
          break;
      ...
      color = isDisabled ? Color.Lerp(axisColor, staticColor, staticBlend)
          : axisColor;
      GUI.SetNextControlName(s_DoPositionHandle_Internal_AxisNames[i]);

      // if we are hot here, the hot handle must be opaque
      var cameraLerp =
          isThisAxisHot ? 0 : s_DoPositionHandle_Internal_CameraViewLerp[i];
      ...
      if (cameraLerp <= kCameraViewThreshold) {
        ...
        if (isHot && (ids[s_DoPositionHandle_Internal_PrevPlaneIndex[i]] ==
        ...
        s_DoPositionHandle_ArrowCapConeOffset =
            isHot
                                                        ? rotation *
            Vector3
                .Scale(Vector3.Scale(axisVector, param.axisOffset),
                       s_DoPositionHandle_AxisHandlesOctant)
            : Vector3.zero;

        position =
            Slider(ids[i], position, offset, dir, size * param.axisSize[i],
                   DoPositionHandle_ArrowCap, GridSnapping.active ? 0f
                   : EditorSnapSettings.move[i]);
      }
    }

    VertexSnapping.HandleMouseMove(ids.xyz);
    if (param.ShouldShow(PositionHandleParam.Handle.XYZ) &&
        (isHot && ids.xyz == GUIUtility.hotControl || !isHot)) {
      color = ToActiveColorSpace(centerColor);
      GUI.SetNextControlName("FreeMoveAxis");
      position = FreeMoveHandle(ids.xyz, position, rotation,
                                size * kFreeMoveHandleSizeFactor,
                                GridSnapping.active ? Vector3.zero
                                : EditorSnapSettings.move, RectangleHandleCap);
    }
  ...
  static Vector3 DoPlanarHandle(int id, int planePrimaryAxis, Vector3 position,
                                Vector3 offset, Quaternion rotation,
                                float handleSize, float cameraLerp,
    ...
    // NOTE: The planar transform handles always face toward the camera so they
    // won't obscure each other (unlike the X, Y, and Z axis handles which
    // always face in the positive axis directions). Whenever the octant that
    // the camera is in (relative to to the transform tool) changes, we need to
    // move the planar transform handle positions to the correct octant.

    // Comments below assume axis1 is X and axis2 is Z to make it easier to
    // visualize things.

    // Shift the planar transform handle in the positive direction by half its
    // handleSize so that it doesn't overlap in the center of the transform
    // gizmo, and also move the handle origin into the octant that the camera is
    // in. Don't update the actant while dragging to avoid too much distraction.
    if (!currentlyDragging) {
      switch (orientation) {
      case PositionHandleParam.Orientation.Camera:
        // Offset the X position of the handle in negative direction if camera
        // is in the -X octants; otherwise positive. Test against -0.01 instead
        // of 0 to give a little bias to the positive quadrants. This looks
        // better in axis views.
        s_PlanarHandlesOctant[axis1index] =
            (viewVectorDrawSpace[axis1index] > 0.01f ? -1 : 1);
        // Likewise with the other axis.
        s_PlanarHandlesOctant[axis2index] =
            (viewVectorDrawSpace[axis2index] > 0.01f ? -1 : 1);
        break;
      case PositionHandleParam.Orientation.Signed:
    ...
    verts[0] = position + positionOffset + handleOffset +
               (axis1 + axis2) * handleSize * 0.5f;
    verts[1] = position + positionOffset + handleOffset +
               (-axis1 + axis2) * handleSize * 0.5f;
    verts[2] = position + positionOffset + handleOffset +
               (-axis1 - axis2) * handleSize * 0.5f;
    verts[3] = position + positionOffset + handleOffset +
               (axis1 - axis2) * handleSize * 0.5f;
    Color faceColor =
        new Color(color.r, color.g, color.b, color.a * faceOpacity);
    ...
    position = Slider2D(id, position, handleOffset + positionOffset, axisNormal,
                        axis1, axis2, handleSize * 0.5f, RectangleHandleCap,
                        GridSnapping.active ? Vector2.zero
                        : new Vector2(EditorSnapSettings.move[axis1index],
                                      EditorSnapSettings.move[axis2index]),
                          false);
  ...
  static void DoPositionHandle_ArrowCap(int controlId, Vector3 position,
