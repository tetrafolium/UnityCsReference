...
public sealed partial class Handles {
  internal sealed class BoneRenderer {
    ...
    public enum SubMeshType { BoneFaces, BoneWire, BoneLeafWire, Count }
    ...
    public void AddBoneLeafInstance(Vector3 position, Quaternion rotation,
                                    float radius, Color color) {
      radius *= k_BoneLeafScale;
      if (radius < k_Epsilon)
        return;

      m_BoneLeafMatrices.Add(Matrix4x4.TRS(
          position, rotation, new Vector3(radius, radius, radius)));
      ...
      get {
        if (!s_Mesh) {
          ...
          List<Vector3> vertices = new List<Vector3>(
              k_BoneVertexCount + k_BoneLeafDiscResolution * 3);
          ...
          Handles.SetDiscSectionPoints(tmp, Vector3.zero, Vector3.up,
                                       Vector3.right, 360f, 1f);
          vertices.AddRange(tmp);
          Handles.SetDiscSectionPoints(tmp, Vector3.zero, Vector3.right,
                                       Vector3.up, 360f, 1f);
          vertices.AddRange(tmp);
          Handles.SetDiscSectionPoints(tmp, Vector3.zero, Vector3.forward,
                                       Vector3.up, 360f, 1f);
          vertices.AddRange(tmp);
          s_Mesh.vertices = vertices.ToArray();

          // Build indices for different sub meshes
          int[] boneFaceIndices = new int[]{0, 2, 1, 0, 1, 3, 0, 3, 2, 1, 2, 3};
          s_Mesh.SetIndices(boneFaceIndices, MeshTopology.Triangles,
                            (int) SubMeshType.BoneFaces);

          int[] boneWireIndices = new int[]{0, 1, 0, 2, 0, 3, 1, 2, 2, 3, 3, 1};
          s_Mesh.SetIndices(boneWireIndices, MeshTopology.Lines,
                            (int) SubMeshType.BoneWire);

          int counter = 0;
          int[] boneLeafWireIndices =
              new int[(vertices.Count - k_BoneVertexCount - 3) * 2];
          ...
          s_Mesh.SetIndices(boneLeafWireIndices, MeshTopology.Lines,
                            (int) SubMeshType.BoneLeafWire);
        }
    ...
    private static Matrix4x4 ComputeBoneMatrix(Vector3 start, Vector3 end,
                                               float length) {
      ...
      return new Matrix4x4(new Vector4(tangent.x * scale, tangent.y * scale,
                                       tangent.z * scale, 0f),
                           new Vector4(direction.x * length,
                                       direction.y * length,
                                       direction.z * length, 0f),
                           new Vector4(bitangent.x * scale, bitangent.y * scale,
                                       bitangent.z * scale, 0f),
                           new Vector4(start.x, start.y, start.z, 1f));
    }

    private static int RenderChunkCount(int totalCount) {
      return Mathf.CeilToInt(
          (totalCount / (float) Graphics.kMaxDrawMeshInstanceCount));
    }

    private static T[] GetRenderChunk<T>(List<T> array, int chunkIndex) {
      int rangeCount =
          (chunkIndex < (RenderChunkCount(array.Count) - 1))
              ? Graphics.kMaxDrawMeshInstanceCount
              : array.Count - (chunkIndex * Graphics.kMaxDrawMeshInstanceCount);

      return array
          .GetRange(chunkIndex * Graphics.kMaxDrawMeshInstanceCount, rangeCount)
          .ToArray();
    }
    ...
    public void Render() {
      ...
      for (int i = 0; i < chunkCount; ++i) {
        ...
        cb.DrawMeshInstanced(mesh, (int) SubMeshType.BoneFaces, material, 0,
                             matrices, matrices.Length, propertyBlock);
        Graphics.ExecuteCommandBuffer(cb);

        cb.Clear();
        material.EnableKeyword("WIRE_ON");
        cb.DrawMeshInstanced(mesh, (int) SubMeshType.BoneWire, material, 0,
                             matrices, matrices.Length, propertyBlock);
        Graphics.ExecuteCommandBuffer(cb);
      }
      ...
      for (int i = 0; i < chunkCount; ++i) {
        ...
        propertyBlock.SetVectorArray("_Color",
                                     GetRenderChunk(m_BoneLeafColors, i));
        cb.DrawMeshInstanced(mesh, (int) SubMeshType.BoneLeafWire, material, 0,
                             matrices, matrices.Length, propertyBlock);
      }
      Graphics.ExecuteCommandBuffer(cb);
    }
  ...
  internal static void DoBoneHandle(Transform target,
    ...
    if (!hasValidChildBones && target.parent != null) {
      endPoints.Add(target.position +
                    (target.position - target.parent.position) * 0.4f);
    } else {
      foreach (Transform child in target) {
    ...
    for (int i = 0; i < endPoints.Count; i++) {
      ...
      case EventType.MouseMove: {
        // TODO : This is slow and should be revisited prior to exposing bone
        // handles
        Vector3[] vertices =
            BoneRenderer.GetBoneWireVertices(basePoint, endPoint);
        if (vertices != null)
      ...
      case EventType.MouseDrag: {
        if (!evt.alt && GUIUtility.hotControl == id) {
          ...
          // having a hot control set during drag makes the control eat the drag
          // events and dragging of bones no longer works over the avatar
          // configure window see case 912016
      ...
      case EventType.Repaint: {
        color = GUIUtility.hotControl == 0 &&
                HandleUtility.nearestControl == id ? Handles.preselectionColor
            : color;
        if (hasValidChildBones)
          renderer.AddBoneInstance(basePoint, endPoint, color);
        else
          renderer.AddBoneLeafInstance(basePoint, target.rotation,
                                       (endPoint - basePoint).magnitude, color);
      } break;
      }
    }
  }
