...
private static readonly Matrix4x4 s_XHandleOffset = Matrix4x4.TRS(
	Vector3.zero, Quaternion.AngleAxis(90f, Vector3.forward), Vector3.one);
private static readonly Matrix4x4 s_ZHandleOffset = Matrix4x4.TRS(
	Vector3.zero, Quaternion.AngleAxis(90f, Vector3.left), Vector3.one);
...
private static float GetSortingDistance(ArcHandle handle) {
	Vector3 worldPosition = Handles.matrix.MultiplyPoint3x4(
		Quaternion.AngleAxis(handle.angle, Vector3.up) * Vector3.forward *
		handle.radius);
	...
	if (Camera.current == null || Camera.current.orthographic) {
		Vector3 lookVector = Camera.current == null ? Vector3.forward
...
private static int
CompareHandleFunctionsByDistance(KeyValuePair<Action, float> func1,
                                 KeyValuePair<Action, float> func2) {
...
// when primary axis is limited, secondary and tertiary manipulators are
// reoriented about its average
...
public Color xHandleColor {
	get {
		// Handles.xAxisColor uses EditorPrefs so it cannot be called from a
		// field initializer
...
public Color yHandleColor {
	get {
		// Handles.yAxisColor uses EditorPrefs so it cannot be called from a
		// field initializer
...
public Color zHandleColor {
	get {
		// Handles.zAxisColor uses EditorPrefs so it cannot be called from a
		// field initializer
...
public void DrawHandle() {
	...
	case ConfigurableJointMotion.Limited:
		drawX = true;
		m_SecondaryAxesMatrix *= Matrix4x4.TRS(
			Vector3.zero,
			Quaternion.AngleAxis((xMin + xMax) * 0.5f, Vector3.left),
			Vector3.one);
		if (yMotion == ConfigurableJointMotion.Limited) {
			...
			using (new Handles.DrawingScope(Handles.matrix * s_XHandleOffset))
				DrawArc(m_XMinHandle, m_XMaxHandle, xHandleColor * fillScalar,
				        ArcType.Solid);
		}
		break;
	case ConfigurableJointMotion.Locked:
		using (new Handles.DrawingScope(
			       Handles.color *
			       Color.Lerp(xHandleColor, s_LockedColor, s_LockedColorAmount)))
			Handles.DrawWireDisc(Vector3.zero, Vector3.right, radius);
		break;
	}
	using (new Handles.DrawingScope(m_SecondaryAxesMatrix)) {
		...
		case ConfigurableJointMotion.Limited:
			...
			if (xMotion != ConfigurableJointMotion.Limited)
				DrawArc(m_YMinHandle, m_YMaxHandle, yHandleColor * fillScalar,
				        ArcType.Solid);
			break;
		case ConfigurableJointMotion.Locked:
			using (new Handles.DrawingScope(
				       Handles.color *
				       Color.Lerp(yHandleColor, s_LockedColor, s_LockedColorAmount)))
				Handles.DrawWireDisc(Vector3.zero, Vector3.up, radius);
			break;
		}
		switch (zMotion) {
		...
		case ConfigurableJointMotion.Limited:
			using (new Handles.DrawingScope(Handles.matrix * s_ZHandleOffset))
				DrawArc(m_ZMinHandle, m_ZMaxHandle, zHandleColor * fillScalar,
				        ArcType.Solid);
			drawZ = true;
			break;
		case ConfigurableJointMotion.Locked:
			using (new Handles.DrawingScope(
				       Handles.color *
				       Color.Lerp(zHandleColor, s_LockedColor, s_LockedColorAmount)))
				Handles.DrawWireDisc(Vector3.zero, Vector3.forward, radius);
			break;
		}
	}
	...
	if (drawX) {
		using (new Handles.DrawingScope(Handles.matrix * s_XHandleOffset)) {
			...
			m_HandleFunctionDistances.Add(new KeyValuePair<Action, float>(
							      DrawXMinHandle, GetSortingDistance(m_XMinHandle)));
			m_HandleFunctionDistances.Add(new KeyValuePair<Action, float>(
							      DrawXMaxHandle, GetSortingDistance(m_XMaxHandle)));
		}
	}
	using (new Handles.DrawingScope(m_SecondaryAxesMatrix)) {
		if (drawY) {
			...
			m_HandleFunctionDistances.Add(new KeyValuePair<Action, float>(
							      DrawYMinHandle, GetSortingDistance(m_YMinHandle)));
			m_HandleFunctionDistances.Add(new KeyValuePair<Action, float>(
							      DrawYMaxHandle, GetSortingDistance(m_YMaxHandle)));
		}
		if (drawZ) {
			using (new Handles.DrawingScope(Handles.matrix * s_ZHandleOffset)) {
				...
				m_HandleFunctionDistances.Add(new KeyValuePair<Action, float>(
								      DrawZMinHandle, GetSortingDistance(m_ZMinHandle)));
				m_HandleFunctionDistances.Add(new KeyValuePair<Action, float>(
								      DrawZMaxHandle, GetSortingDistance(m_ZMaxHandle)));
			}
		}
	}
...
private void DrawArc(ArcHandle minHandle, ArcHandle maxHandle, Color color,
                     ArcType arcType) {
	float angle = maxHandle.angle - minHandle.angle;
	Vector3 forward =
		Quaternion.AngleAxis(minHandle.angle, Vector3.up) * Vector3.forward;
	using (new Handles.DrawingScope(Handles.color * color)) {
		if (arcType == ArcType.Solid) {
			for (int i = 0, revolutions = (int) Mathf.Abs(angle) / 360;
			     i < revolutions; ++i)
				Handles.DrawSolidArc(Vector3.zero, Vector3.up, Vector3.forward,
				                     360f, radius);
			Handles.DrawSolidArc(Vector3.zero, Vector3.up, forward, angle % 360f,
			                     radius);
		} else {
			for (int i = 0, revolutions = (int) Mathf.Abs(angle) / 360;
			     i < revolutions; ++i)
				Handles.DrawWireArc(Vector3.zero, Vector3.up, Vector3.forward, 360f,
				                    radius);
			Handles.DrawWireArc(Vector3.zero, Vector3.up, forward, angle % 360f,
			                    radius);
		}
	}
}
...
private void DrawMultiaxialFillShape() {
	...
	using (new Handles.DrawingScope(Handles.color *
	                                (yHandleColor * fillAlphaScalar))) {
		float yAngle = yMax - yMin;

		start = toXMax * toYMax * Vector3.forward;
		Handles.DrawSolidArc(Vector3.zero, toXMax * Vector3.down, start, yAngle,
		                     radius);

		start = toXMin * toYMax * Vector3.forward;
		Handles.DrawSolidArc(Vector3.zero, toXMin * Vector3.down, start, yAngle,
		                     radius);
	}
	using (new Handles.DrawingScope(Handles.color *
	                                (xHandleColor * fillAlphaScalar))) {
		float xAngle = xMax - xMin;

		start = toXMax * toYMax * Vector3.forward;
		Handles.DrawSolidArc(Vector3.zero, Vector3.right, start, xAngle,
		                     radius);
...
private void DrawXMinHandle() {
	using (new Handles.DrawingScope(Handles.matrix * s_XHandleOffset)) {
		...
		m_XMinHandle.angle =
			Mathf.Clamp(m_XMinHandle.angle, xRange.x, m_XMaxHandle.angle);
	}
}
...
private void DrawXMaxHandle() {
	using (new Handles.DrawingScope(Handles.matrix * s_XHandleOffset)) {
		...
		m_XMaxHandle.angle =
			Mathf.Clamp(m_XMaxHandle.angle, m_XMinHandle.angle, xRange.y);
	}
}
...
private void DrawYMinHandle() {
	using (new Handles.DrawingScope(m_SecondaryAxesMatrix)) {
		...
		m_YMinHandle.angle =
			Mathf.Clamp(m_YMinHandle.angle, yRange.x, m_YMaxHandle.angle);
	}
}
...
private void DrawYMaxHandle() {
	using (new Handles.DrawingScope(m_SecondaryAxesMatrix)) {
		...
		m_YMaxHandle.angle =
			Mathf.Clamp(m_YMaxHandle.angle, m_YMinHandle.angle, yRange.y);
	}
}
...
private void DrawZMinHandle() {
	using (new Handles.DrawingScope(m_SecondaryAxesMatrix * s_ZHandleOffset)) {
		...
		m_ZMinHandle.angle =
			Mathf.Clamp(m_ZMinHandle.angle, zRange.x, m_ZMaxHandle.angle);
	}
}
...
private void DrawZMaxHandle() {
	using (new Handles.DrawingScope(m_SecondaryAxesMatrix * s_ZHandleOffset)) {
		...
		m_ZMaxHandle.angle =
			Mathf.Clamp(m_ZMaxHandle.angle, m_ZMinHandle.angle, zRange.y);
	}
}
}
}
