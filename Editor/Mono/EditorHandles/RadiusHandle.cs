...
internal static float DoRadiusHandle(Quaternion rotation, Vector3 position,
                                     float radius, bool handlesOnly) {
	Vector3 planeNormal;
	float frontAngle = 90;
	Vector3[] dirs =
		new[] {rotation * Vector3.right,   rotation * Vector3.up,
		       rotation * Vector3.forward, rotation * -Vector3.right,
	...
	if (Camera.current.orthographic) {
		...
		planeNormal =
			position - invMatrix.MultiplyPoint(
				Camera.current.transform
				.position); // vector from camera to center
		float sqrDist =
			planeNormal.sqrMagnitude; // squared distance from camera to center
		float sqrRadius = radius * radius; // squared radius
		float sqrOffset =
			sqrRadius * sqrRadius /
			sqrDist; // squared distance from actual center to drawn disc center
		float insideAmount = sqrOffset / sqrRadius;

		// If we are not inside the sphere, calculate where to draw the periphery
		if (insideAmount < 1) {
			float drawnRadius =
				Mathf.Sqrt(sqrRadius - sqrOffset); // the radius of the drawn disc
			frontAngle =
				Mathf.Atan2(drawnRadius, Mathf.Sqrt(sqrOffset)) * Mathf.Rad2Deg;

			// Draw periphery circle
			if (!handlesOnly)
				DrawWireDisc(position - sqrRadius * planeNormal / sqrDist,
				             planeNormal, drawnRadius);
		} else {
			frontAngle = -1000;
		}
			...
			for (int i = 0; i < 3; i++) {
				if (insideAmount < 1) {
					...
					if (g < 1) {
						...
						DrawTwoShadedWireDisc(position, dirs[i], from, (90 - e) * 2,
						                      radius);
					} else {
	...
	for (int i = 0; i < 6; i++) {
		...
		// in order to not corrupt the id order when slider handles pop in and
		// out.
		...
		// Don't make the slider if the axis is pointing almost towards or away
		// from the camera, but always draw it if the user is already dragging it.
