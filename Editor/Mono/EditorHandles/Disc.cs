...
public static Quaternion Do(int id, Quaternion rotation, Vector3 position,
                            Vector3 axis, float size, bool cutoffPlane,
                            float snap) {
	return Do(id, rotation, position, axis, size, cutoffPlane, snap, true, true,
	          Handles.secondaryColor);
}

public static Quaternion Do(int id, Quaternion rotation, Vector3 position,
                            Vector3 axis, float size, bool cutoffPlane,
                            float snap, bool enableRayDrag, bool showHotArc,
                            Color fillColor) {
	var cam = Handles.inverseMatrix.MultiplyVector(
		Camera.current != null ? Camera.current.transform.forward
	: Vector3.forward);
	...
	case EventType.MouseMove: {
		...
		if (cutoffPlane) {
			...
			d = HandleUtility.DistanceToArc(position, axis, from, 180, size) *
			    k_GrabZoneScale;
		} else {
			d = HandleUtility.DistanceToDisc(position, axis, size) *
			    k_GrabZoneScale;
		}
	...
	case EventType.MouseDown:
		...
		if (HandleUtility.nearestControl == id && evt.button == 0 && !evt.alt) {
			...
			if (cutoffPlane) {
				Vector3 from = Vector3.Cross(axis, cam).normalized;
				s_StartPosition =
					HandleUtility.ClosestPointToArc(position, axis, from, 180, size);
			} else {
				s_StartPosition =
					HandleUtility.ClosestPointToDisc(position, axis, size);
			}
			s_RotationDist = 0;
	...
	case EventType.MouseDrag:
		if (GUIUtility.hotControl == id) {
			...
			if (rayDrag) {
				...
				if (hit != null && Vector3.Dot(axis.normalized,
				                               rotation * Vector3.forward) < 0.999) {
					RaycastHit rh = (RaycastHit) hit;
					Vector3 lookPoint = rh.point - position;
					Vector3 lookPointProjected =
						lookPoint -
						Vector3.Dot(lookPoint, axis.normalized) * axis.normalized;
					rotation = Quaternion.LookRotation(lookPointProjected,
					                                   rotation * Vector3.up);
				}
			} else {
				Vector3 direction =
					Vector3.Cross(axis, position - s_StartPosition).normalized;
				s_CurrentMousePosition += evt.delta;
				s_RotationDist = HandleUtility.CalcLineTranslation(
					s_StartMousePosition, s_CurrentMousePosition,
					s_StartPosition, direction) /
				                 size * 30;
				s_RotationDist = Handles.SnapValue(s_RotationDist, snap);
				rotation = Quaternion.AngleAxis(s_RotationDist * -1, s_StartAxis) *
				           s_StartRotation;
			}
	...
	case EventType.KeyDown:
		if (evt.keyCode == KeyCode.Escape && GUIUtility.hotControl == id) {
			// We do not use the event nor clear hotcontrol to ensure auto revert
			// value kicks in from native side
			Tools.UnlockHandlePosition();
			EditorGUIUtility.SetWantsMouseJumping(0);
		}
		break;
	...
	case EventType.Repaint:
		...
		if (GUIUtility.hotControl == id) {
			...
			var d = -Mathf.Sign(s_RotationDist) *
			        Mathf.Repeat(Mathf.Abs(s_RotationDist), 360);
			...
			for (int i = 0, revolutions =
				     (int) Mathf.Abs(s_RotationDist * 0.002777777778f);
			     i < revolutions; ++i)
			...
			if (EditorSnapSettings.incrementalSnapActive && snap > 0) {
				DrawRotationUnitSnapMarkers(
					position, axis, size, k_RotationUnitSnapMarkerSize, snap, @from);
				DrawRotationUnitSnapMarkers(position, axis, size,
				                            k_RotationUnitSnapMajorMarkerSize,
				                            k_RotationUnitSnapMajorMarkerStep, @from);
			}
			Handles.color = t;
		}

		if (showHotArc && GUIUtility.hotControl == id ||
		    GUIUtility.hotControl != id && !cutoffPlane)
...
static void DrawRotationUnitSnapMarkers(Vector3 position, Vector3 axis,
                                        float handleSize, float markerSize,
                                        float snap, Vector3 @from) {
	...
	for (var i = -count; i < count; ++i) {
		...
		if (performFading) {
			var alpha =
				...
				Mathf.SmoothStep(
					0, 1,
					Mathf.Abs(i / ((float) limitedIterationCount - 1) - 0.5f) * 2);
			Handles.color =
				new Color(Handles.color.r, Handles.color.g, Handles.color.b, alpha);
		}
