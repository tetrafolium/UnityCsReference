...
internal static Vector2 DoConeHandle(Quaternion rotation, Vector3 position,
	...
	float lightDisc =
		actualRange * Mathf.Tan(Mathf.Deg2Rad * spotAngle / 2.0f) * angleScale;
	...
	if (GUI.changed)
		spotAngle =
			Mathf.Clamp((Mathf.Rad2Deg *
			             Mathf.Atan(lightDisc / (actualRange * angleScale)) * 2),
			            0.0F, 179F);
	GUI.changed |= temp;
	...
	if (!handlesOnly) {
		Handles.DrawLine(position,
		                 (position + forward * actualRange) + up * lightDisc);
		Handles.DrawLine(position,
		                 (position + forward * actualRange) - up * lightDisc);
		Handles.DrawLine(position,
		                 (position + forward * actualRange) + right * lightDisc);
		Handles.DrawLine(position,
		                 (position + forward * actualRange) - right * lightDisc);
...
static internal float SizeSlider(Vector3 p, Vector3 d, float r) {
	...
	GUI.changed = false;
	position =
		Handles.Slider(position, d, size * 0.03f, Handles.DotHandleCap, 0f);
	if (GUI.changed)
