...
private static readonly Vector3[] s_HeightAxes =
...
public float height {
	get {
		...
		return !IsAxisEnabled(m_HeightAxis)
		   ? 0f
		   : Mathf.Max(GetSize()[m_HeightAxis], 2f * radius);
	}
	set {
...
public float radius {
	...
	set {
		...
		for (int axis = 0; axis < 3; ++axis)
			size[axis] =
				axis == m_HeightAxis? Mathf.Max(size[axis], diameter) : diameter;
		SetSize(size);
	}
}
...
protected override void DrawWireframe() {
	...
	// draw caps and connecting lines for each enabled axis if height axis is
	// enabled
	if (doHeightAxis) {
		if (doRadiusAxis2) {
...
protected override Bounds OnHandleChanged(HandleDirection handle,
	...
	// ensure radius changes uniformly and enlarges the height if necessary
		...
		upperBound[m_HeightAxis] = boundsOnClick.center[m_HeightAxis] +
		                           0.5f * boundsOnClick.size[m_HeightAxis];
		lowerBound[m_HeightAxis] = boundsOnClick.center[m_HeightAxis] -
		                           0.5f * boundsOnClick.size[m_HeightAxis];

		float rad = 0.5f * (upperBound[changedAxis] - lowerBound[changedAxis]);
		float halfCurrentHeight =
			0.5f * (upperBound[m_HeightAxis] - lowerBound[m_HeightAxis]);
		for (int axis = 0; axis < 3; ++axis) {
			if (axis == changedAxis)
				continue;
			float amt =
				axis == m_HeightAxis? Mathf.Max(halfCurrentHeight, rad) : rad;
			lowerBound[axis] = center[axis] - amt;
			upperBound[axis] = center[axis] + amt;
		}
	}
	return new Bounds((upperBound + lowerBound) * 0.5f,
