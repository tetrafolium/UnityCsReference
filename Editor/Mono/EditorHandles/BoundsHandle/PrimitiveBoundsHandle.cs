...
namespace UnityEditor.IMGUI.Controls {
  public abstract class PrimitiveBoundsHandle {
    ...
    public void DrawHandle() {
      ...
      // unless holding alt to pin center, exit before drawing control handles
      // when holding alt, since alt-click will rotate scene view
      ...
      bool isCameraInsideBox =
          Camera.current != null &&
          m_Bounds.Contains(Handles.inverseMatrix.MultiplyPoint(
      ...
      if (prevHotControl != GUIUtility.hotControl &&
      ...
      if (changed) {
        ...
        for (int i = 0, count = m_ControlIDs.Length; i < count; ++i) {
          if (GUIUtility.hotControl == m_ControlIDs[i])
            m_Bounds =
                OnHandleChanged((HandleDirection) i, m_BoundsOnClick, m_Bounds);
        }
        ...
        if (Event.current.shift) {
          ...
          int scaleAxis = 0;
          if (hotControl == m_ControlIDs[(int) HandleDirection.PositiveY] ||
              hotControl == m_ControlIDs[(int) HandleDirection.NegativeY]) {
            scaleAxis = 1;
          }
          if (hotControl == m_ControlIDs[(int) HandleDirection.PositiveZ] ||
              hotControl == m_ControlIDs[(int) HandleDirection.NegativeZ]) {
            scaleAxis = 2;
          }
    ...
    protected virtual Bounds OnHandleChanged(HandleDirection handle,
    ...
    protected bool IsAxisEnabled(Axes axis) { return (axes & axis) == axis; }
    ...
    protected bool IsAxisEnabled(int vector3Axis) {
      ...
      default:
        throw new ArgumentOutOfRangeException("vector3Axis",
                                              "Must be 0, 1, or 2");
      }
    }
    ...
    private void MidpointHandles(ref Vector3 minPos, ref Vector3 maxPos,
      ...
      if (IsAxisEnabled(Axes.X)) {
        ...
        newPos = MidpointHandle(m_ControlIDs[(int) HandleDirection.PositiveX],
                                localPos, yAxis, zAxis, isCameraInsideBox);
        maxPos.x = Mathf.Max(newPos.x, minPos.x);

        // -X
        localPos = new Vector3(minPos.x, middle.y, middle.z);
        newPos = MidpointHandle(m_ControlIDs[(int) HandleDirection.NegativeX],
                                localPos, yAxis, -zAxis, isCameraInsideBox);
      ...
      if (IsAxisEnabled(Axes.Y)) {
        ...
        newPos = MidpointHandle(m_ControlIDs[(int) HandleDirection.PositiveY],
                                localPos, xAxis, -zAxis, isCameraInsideBox);
        maxPos.y = Mathf.Max(newPos.y, minPos.y);

        // -Y
        localPos = new Vector3(middle.x, minPos.y, middle.z);
        newPos = MidpointHandle(m_ControlIDs[(int) HandleDirection.NegativeY],
                                localPos, xAxis, zAxis, isCameraInsideBox);
      ...
      if (IsAxisEnabled(Axes.Z)) {
        ...
        newPos = MidpointHandle(m_ControlIDs[(int) HandleDirection.PositiveZ],
                                localPos, yAxis, -xAxis, isCameraInsideBox);
        maxPos.z = Mathf.Max(newPos.z, minPos.z);

        // -Z
        localPos = new Vector3(middle.x, middle.y, minPos.z);
        newPos = MidpointHandle(m_ControlIDs[(int) HandleDirection.NegativeZ],
                                localPos, yAxis, xAxis, isCameraInsideBox);
        minPos.z = Mathf.Min(newPos.z, maxPos.z);
      }
    }

    private Vector3 MidpointHandle(int id, Vector3 localPos,
                                   Vector3 localTangent, Vector3 localBinormal,
                                   bool isCameraInsideBox) {
      Color oldColor = Handles.color;

      AdjustMidpointHandleColor(localPos, localTangent, localBinormal,
                                isCameraInsideBox);

      if (Handles.color.a > 0f && midpointHandleDrawFunction != null) {
        Vector3 localDir =
            Vector3.Cross(localTangent, localBinormal).normalized;

        var size = midpointHandleSizeFunction == null ? 0f
            : midpointHandleSizeFunction(localPos);

        localPos = UnityEditorInternal.Slider1D.Do(id, localPos, localDir, size,
    ...
    private void AdjustMidpointHandleColor(Vector3 localPos,
                                           Vector3 localTangent,
                                           Vector3 localBinormal,
                                           bool isCameraInsideBox) {
      float alphaMultiplier = 1f;

      // if inside the box then ignore back facing alpha multiplier (otherwise
      // all handles will look disabled)
      if (!isCameraInsideBox && axes == (Axes.X | Axes.Y | Axes.Z)) {
        // use tangent and binormal to calculate normal in case handle matrix is
        // skewed
        ...
        Vector3 worldDir =
            Vector3.Cross(worldTangent, worldBinormal).normalized;
        ...
        if (Camera.current.orthographic)
          ...
          cosV = Vector3.Dot((Camera.current.transform.position -
                              Handles.matrix.MultiplyPoint(localPos))
                                 .normalized,
                             worldDir);
