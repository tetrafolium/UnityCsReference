...
internal class VertexSnapping {
  ...
  [Shortcut("Scene View/Toggle Vertex Snapping", typeof(SceneView), KeyCode.V,
  ...
  private static void
  ToggleVertexSnappingViaClutchShortcut(ShortcutArguments arguments) {
    int id = GUIUtility.hotControl;
  ...
  //          Vector3 near;
  //          if (HandleUtility.FindNearestVertex (evt.mousePosition, null, out
  //          near))
  //          {
  ...
  public static void HandleMouseMove(int id) {
    ...
    if (evt.GetTypeForControl(id) == EventType.MouseMove &&
        Tools.vertexDragging) {
      EnableVertexSnapping(id);
      evt.Use();
    }
  }
  ...
  // Iterates over selected objects, finds nearest vertex or pivot and sets
  // Tools.handleOffset accordingly
  private static void UpdateVertexSnappingOffset() {
    ...
    // Make sure we're not ignoring any objects, that other handles may have set
    // to ignore and forgot to reset.
    HandleUtility.ignoreRaySnapObjects = null;

    Vector3 nearestPivot = FindNearestPivot(selection, evt.mousePosition);
    bool foundVertex = HandleUtility.FindNearestVertex(
        evt.mousePosition, selection, out nearestVertex);

    Vector3 near;

    // Is nearest vertex closer than nearest pivot?
    float distanceToNearestVertex =
        (HandleUtility.WorldToGUIPoint(nearestVertex) - evt.mousePosition)
            .magnitude;
    float distanceToNearestPivot =
        (HandleUtility.WorldToGUIPoint(nearestPivot) - evt.mousePosition)
            .magnitude;
  ...
  private static Vector3 FindNearestPivot(Transform[] transforms,
    ...
    foreach (Transform transform in transforms) {
      ...
      if (!foundPivot || (pivot - worldPosition).magnitude >
                             (transform.position - worldPosition).magnitude) {
