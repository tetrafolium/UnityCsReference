...
namespace UnityEditor.IMGUI.Controls {
  public class ArcHandle {
    ...
    static readonly Dictionary<int, Quaternion>
        s_MostRecentValidAngleHandleOrientations =
            new Dictionary<int, Quaternion>();

    // only used inside of DefaultAngleHandleDrawFunction to look up and store
    // the most recent valid orientation per handle done so that
    // DefaultAngleHandleDrawFunction can be a static method, which makes
    // angleHandleDrawFunction more usable by sub-classes
    static int s_CurrentlyDrawingAngleHandleHash;

    public static void DefaultAngleHandleDrawFunction(int controlID,
                                                      Vector3 position,
                                                      Quaternion rotation,
                                                      float size,
                                                      EventType eventType) {
      ...
      // draw a cylindrical "hammer head" to indicate the direction the handle
      // will move
      Vector3 worldPosition = Handles.matrix.MultiplyPoint3x4(position);
      Vector3 normal =
          worldPosition - Handles.matrix.MultiplyPoint3x4(Vector3.zero);
      Vector3 tangent = Handles.matrix.MultiplyVector(
          Quaternion.AngleAxis(90f, Vector3.up) * position);
      Quaternion mostRecentValidOrientation;
      if (!s_MostRecentValidAngleHandleOrientations.TryGetValue(
              s_CurrentlyDrawingAngleHandleHash,
              out mostRecentValidOrientation))
        mostRecentValidOrientation = Quaternion.identity;
      rotation = Mathf.Approximately(tangent.sqrMagnitude, 0f)
                     ? mostRecentValidOrientation
                     : Quaternion.LookRotation(tangent, normal);
      s_MostRecentValidAngleHandleOrientations
          [s_CurrentlyDrawingAngleHandleHash] = rotation;
      Matrix4x4 matrix = Matrix4x4.TRS(
          worldPosition, rotation,
          (Vector3.one + Vector3.forward * s_DefaultAngleHandleSizeRatio));
      using(new Handles.DrawingScope(matrix)) Handles.CylinderHandleCap(
          controlID, Vector3.zero, Quaternion.identity, size, eventType);
    ...
    public void DrawHandle() {
      ...
      Vector3 scale = Handles.matrix.MultiplyPoint3x4(Vector3.one) -
                      Handles.matrix.MultiplyPoint3x4(Vector3.zero);
      if (scale.x == 0f && scale.z == 0f)
        return;

      Vector3 angleHandlePosition =
      ...
      using(new Handles.DrawingScope(Handles.color * fillColor)) {
        if (Handles.color.a > 0f) {
          for (int i = 0, revolutions = (int) absAngle / 360; i < revolutions;
               ++i)
      ...
      using(new Handles.DrawingScope(Handles.color * wireframeColor)) {
        if (Handles.color.a > 0f) {
          Handles.DrawWireArc(Vector3.zero, Vector3.up, Vector3.forward,
                              absAngle >= 360f ? 360f : excessAngle, radius);
        }
      }

      // unless holding alt while already manipulating a control, exit before
      // drawing control handles when holding alt, since alt-click will rotate
      // scene view
      ...
      using(new Handles.DrawingScope(Handles.color * radiusHandleColor)) {
        if (Handles.color.a > 0f) {
          // draw one radius handle in each cardinal direction encompassed by
          // the arc (i.e. every 90 degrees)
          float direction = Mathf.Sign(angle);
          int numRadiusHandles = Mathf.Min(
              1 + (int)(Mathf.Min(360f, absAngle) * 0.01111111111f), 4);
          for (int i = 0; i < numRadiusHandles; ++i) {
            Quaternion handleOrientation =
                Quaternion.AngleAxis(i * 90f * direction, Vector3.up);
            using(new Handles.DrawingScope(
                Handles.matrix *
                Matrix4x4.TRS(Vector3.zero, handleOrientation, Vector3.one))) {
              Vector3 radiusHandlePosition = Vector3.forward * radius;
              Vector3 newPosition;
              EditorGUI.BeginChangeCheck();
              {
                var size = radiusHandleSizeFunction == null ? 0f
                    : radiusHandleSizeFunction(radiusHandlePosition);
                newPosition = Handles.Slider(
                    m_RadiusHandleControlIDs[i], radiusHandlePosition,
                    Vector3.forward, size, radiusHandleDrawFunction,
      ...
      // draw angle handle last so it will always take precedence when
      // overlapping a radius handle
      using(new Handles.DrawingScope(Handles.color * angleHandleColor)) {
        if (Handles.color.a > 0f && angleHandleDrawFunction != null) {
          EditorGUI.BeginChangeCheck();
          {
            var size = angleHandleSizeFunction == null ? 0f
                : angleHandleSizeFunction(angleHandlePosition);
            s_CurrentlyDrawingAngleHandleHash = GetHashCode();
            angleHandlePosition =
                Handles.Slider2D(m_AngleHandleControlID, angleHandlePosition,
                                 Vector3.up, Vector3.forward, Vector3.right,
                                 size, angleHandleDrawFunction, Vector2.zero);
          }
          if (EditorGUI.EndChangeCheck()) {
            float newAngle =
                Vector3.Angle(Vector3.forward, angleHandlePosition) *
