...
public sealed partial class Handles {
  internal struct RotationHandleIds {
    public static RotationHandleIds @default {
      get {
        return new RotationHandleIds(
            ...
            GUIUtility.GetControlID(s_cameraAxisRotateHandleHash,
                                    FocusType.Passive),
    ...
    public bool Has(int id) {
      return x == id || y == id || z == id || cameraAxis == id || xyz == id;
    }
    ...
    public override int GetHashCode() { return x ^ y ^ z ^ cameraAxis ^ xyz; }
    ...
    public override bool Equals(object obj) {
      ...
      return o.x == x && o.y == y && o.z == z && o.cameraAxis == cameraAxis &&
             o.xyz == xyz;
    }
  }
  ...
  internal struct RotationHandleParam {
    ...
    static RotationHandleParam s_Default = new RotationHandleParam(
        (Handle)(-1), Vector3.one, 1f, 1.1f, true, true);
    ...
    public RotationHandleParam(Handle handles, Vector3 axisSize, float xyzSize,
                               float cameraAxisSize, bool enableRayDrag,
  ...
  static readonly Color k_RotationPieColor =
      new Color(246f / 255, 242f / 255, 50f / 255, .89f);

  public static Quaternion DoRotationHandle(Quaternion rotation,
                                            Vector3 position) {
    return DoRotationHandle(RotationHandleIds.@default, rotation, position,
                            RotationHandleParam.Default);
  }

  internal static Quaternion DoRotationHandle(RotationHandleIds ids,
                                              Quaternion rotation,
                                              Vector3 position,
                                              RotationHandleParam param) {
    var evt = Event.current;
    var camForward = Handles.inverseMatrix.MultiplyVector(
    ...
    if (!isDisabled && param.ShouldShow(RotationHandleParam.Handle.XYZ) &&
        (isHot && ids.xyz == GUIUtility.hotControl || !isHot)) {
      color = new Color(0, 0, 0, 0.3f);
      rotation = UnityEditorInternal.FreeRotate.Do(ids.xyz, rotation, position,
                                                   size * param.xyzSize,
                                                   param.displayXYZCircle);
    }
    ...
    for (var i = 0; i < 3; ++i) {
      ...
      rotation = UnityEditorInternal.Disc.Do(
          ids[i], rotation, position, rotation * axisDir, radius, true,
          EditorSnapSettings.rotate, param.enableRayDrag, true,
          k_RotationPieColor);
    }
    ...
    if (!isDisabled &&
      ...
      rotation = UnityEditorInternal.Disc.Do(
          ids.cameraAxis, rotation, position, camForward,
          size * param.cameraAxisSize, false, 0, param.enableRayDrag, true,
          k_RotationPieColor);
    }
