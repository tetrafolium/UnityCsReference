...
public sealed partial class Handles {
  internal struct ScaleHandleIds {
    ...
    public override int GetHashCode() { return x ^ y ^ z ^ xyz; }
    ...
    public override bool Equals(object obj) {
      ...
      return o.x == x && o.y == y && o.z == z && o.xyz == xyz;
    }
  }
  ...
  internal struct ScaleHandleParam {
    ...
    static ScaleHandleParam s_Default =
        new ScaleHandleParam((Handle)(-1), Vector3.zero, Vector3.one,
                             Vector3.one, 1, Orientation.Signed);
    public static ScaleHandleParam Default {
    ...
    public ScaleHandleParam(Handle handles, Vector3 axisOffset,
                            Vector3 axisSize, Vector3 axisLineScale,
                            float xyzSize, Orientation orientation) {
  ...
  public static Vector3 DoScaleHandle(Vector3 scale, Vector3 position,
                                      Quaternion rotation, float size) {
    return DoScaleHandle(ScaleHandleIds.@default, scale, position, rotation,
                         size, ScaleHandleParam.Default);
  }

  internal static Vector3 DoScaleHandle(ScaleHandleIds ids, Vector3 scale,
                                        Vector3 position, Quaternion rotation,
                                        float handleSize,
                                        ScaleHandleParam param) {
    ...
    for (var ii = 0; ii < 3; ++ii) {
      ...
      if (!currentlyDragging) {
        ...
        case ScaleHandleParam.Orientation.Camera:
          s_DoScaleHandle_AxisHandlesOctant[i] =
      ...
      var cameraLerp = id == GUIUtility.hotControl ? 0
          : GetCameraViewLerpForWorldAxis(viewVectorDrawSpace, axisDir);
      ...
      if (cameraLerp <= kCameraViewThreshold) {
        ...
        scale[i] = UnityEditorInternal.SliderScale.DoAxis(
            id, scale[i], position, rotation * axisDir, rotation,
            handleSize * param.axisSize[i], EditorSnapSettings.scale, offset,
            axisLineScale[i]);
      }
    }

    if (param.ShouldShow(ScaleHandleParam.Handle.XYZ) &&
        (isHot && ids.xyz == GUIUtility.hotControl || !isHot)) {
      color = ToActiveColorSpace(centerColor);
      EditorGUI.BeginChangeCheck();
      var s = ScaleValueHandle(ids.xyz, scale.x, position, rotation,
                               handleSize * param.xyzSize, CubeHandleCap,
