...
internal static Quaternion Do(int id, Quaternion rotation, Vector3 position,
                              float size, bool drawCircle) {
	...
	case EventType.MouseMove:
		...
		HandleUtility.AddControl(
			id, HandleUtility.DistanceToCircle(worldPosition, size) +
			...
			if (rayDrag) {
				...
				if (hit != null) {
					...
					Quaternion newRotation =
						Quaternion.LookRotation(rh.point - position);
				...
				Vector3 rotDir = Camera.current.transform.TransformDirection(
					new Vector3(-evt.delta.y, -evt.delta.x, 0));
				rotation =
					Quaternion.AngleAxis(evt.delta.magnitude, rotDir.normalized) *
					rotation;
			}
			GUI.changed = true;
			evt.Use();
	...
	case EventType.Repaint:
		...
		Handles.matrix = Matrix4x4.identity;
		if (drawCircle)
			Handles.DrawWireDisc(worldPosition, Camera.current.transform.forward,
			                     size, thickness);
		if (isHover || isHot) {
			Handles.color = s_DimmingColor;
			Handles.DrawSolidDisc(worldPosition, Camera.current.transform.forward,
			                      size);
		}
		Handles.matrix = origMatrix;
