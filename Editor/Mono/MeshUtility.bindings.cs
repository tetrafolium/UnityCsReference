...
public class MeshUtility {
  ...
  OptimizeReorderVertexBuffer(Mesh mesh);
  ...
  SetMeshCompression(Mesh mesh, ModelImporterMeshCompression compression);
  extern public static ModelImporterMeshCompression
  GetMeshCompression(Mesh mesh);

  [NativeName("SetPerTriangleUV2")] extern private static void
  SetPerTriangleUV2NoCheck(Mesh src, Vector2[] triUV);
  public static void SetPerTriangleUV2(Mesh src, Vector2[] triUV) {
    ...
    int triCount = InternalMeshUtil.CalcTriangleCount(src),
        uvCount = triUV.Length;
    if (uvCount != 3 * triCount) {
      Debug.LogError("mesh contains " + triCount + " triangles but " + uvCount +
                     " uvs are provided");
      return;
    }
  ...
  extern internal static Vector2[] ComputeTextureBoundingHull(Texture texture,
                                                              int vertexCount);
}
}
