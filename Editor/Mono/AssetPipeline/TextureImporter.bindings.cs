...
// Texture importer lets you modify [[Texture2D]] import settings from editor
// scripts.
[NativeHeader("Editor/Src/AssetPipeline/TextureImporting/TextureImporter.h")]
[NativeHeader(
	 "Editor/Src/AssetPipeline/TextureImporting/TextureImporter.deprecated.h")]
[NativeHeader(
	 "Editor/Src/AssetPipeline/TextureImporting/TextureImporterUtils.h")]
...
public extern TextureImporterFormat textureFormat {
	[FreeFunction("GetTextureFormat", HasExplicitThis = true)] get;
	[FreeFunction("SetTextureFormat", HasExplicitThis = true)] set;
}

[NativeProperty("TextureImporter::s_DefaultPlatformName", true,
                TargetType.Field)]
...
internal extern bool forceMaximumCompressionQuality_BC6H_BC7 {
	get;
	set;
}
...
[Obsolete(
	 "Use UnityEditor.TextureImporter.GetPlatformTextureSettings() instead.")]
[NativeMethod(HasExplicitThis = true)]
public extern bool GetAllowsAlphaSplitting();

[Obsolete(
	 "Use UnityEditor.TextureImporter.SetPlatformTextureSettings() instead.")]
[NativeMethod(HasExplicitThis = true)]
public extern void SetAllowsAlphaSplitting(bool flag);

public bool
GetPlatformTextureSettings(string platform, out int maxTextureSize,
                           out TextureImporterFormat textureFormat,
                           out int compressionQuality,
                           out bool etc1AlphaSplitEnabled) {
...
GetPlatformTextureSettings(string platform, out int maxTextureSize,
	...
	return GetPlatformTextureSettings(platform, out maxTextureSize,
	                                  out textureFormat, out compressionQuality,
	                                  out etc1AlphaSplitEnabled);
}
...
// C++ implementation will return default platform if the requested platform
// does not have any settings yet.
[NativeName("GetPlatformTextureSettings")]
private extern TextureImporterPlatformSettings
GetPlatformTextureSetting_Internal(string platform);

// Read texture settings for specified platform into
// [[TextureImporterPlatformSettings]] class. public API will always return a
// valid TextureImporterPlatformSettings, creating it based on the default one
// if it did not exist.
public TextureImporterPlatformSettings
GetPlatformTextureSettings(string platform) {
	// make sure we are converting the settings to use the proper
	// BuildTargetGroupName to get them (the way it works on other importers)
	platform = GetFixedPlatformName(platform);

	TextureImporterPlatformSettings dest =
		GetPlatformTextureSetting_Internal(platform);
...
public TextureImporterPlatformSettings GetDefaultPlatformTextureSettings() {
	return GetPlatformTextureSettings(
...
public TextureImporterFormat GetAutomaticFormat(string platform) {
	...
	foreach (BuildPlatform bp in validPlatforms) {
		if (bp.name == platform) {
			return DefaultFormatFromTextureParameters(
				settings, platformSettings, DoesSourceTextureHaveAlpha(),
				IsSourceTextureHDR(), bp.defaultTarget);
		}
	}
...
[Obsolete(
	 "Use UnityEditor.TextureImporter.SetPlatformTextureSettings(TextureImporterPlatformSettings) instead.")]
public void SetPlatformTextureSettings(string platform, int maxTextureSize,
                                       TextureImporterFormat textureFormat,
                                       int compressionQuality,
                                       bool allowsAlphaSplit) {
...
[Obsolete(
	 "Use UnityEditor.TextureImporter.SetPlatformTextureSettings(TextureImporterPlatformSettings) instead.")]
public void
SetPlatformTextureSettings(string platform, int maxTextureSize,
                           TextureImporterFormat textureFormat,
                           [ DefaultValue(false) ] bool allowsAlphaSplit) {
	TextureImporterPlatformSettings dest = GetPlatformTextureSettings(platform);
	dest.overridden = true;
	dest.maxTextureSize = maxTextureSize;
	dest.format = textureFormat;
...
private extern void SetPlatformTextureSettings_Internal(
	TextureImporterPlatformSettings platformSettings);

// Set specific target platform settings
public void
SetPlatformTextureSettings(TextureImporterPlatformSettings platformSettings) {
	// we need to fix the name in case the user changed it to some mismatching
	// value
...
[FreeFunction]
internal static extern TextureImporterFormat
DefaultFormatFromTextureParameters(
	[ NotNull ] TextureImporterSettings settings,
	TextureImporterPlatformSettings platformSettings,
	bool doesTextureContainAlpha, bool sourceWasHDR,
	BuildTarget destinationPlatform);

[FreeFunction]
internal static extern TextureImporterFormat
[] RecommendedFormatsFromTextureTypeAndPlatform(
	TextureImporterType textureType, BuildTarget destinationPlatform);

[RequiredByNativeCode]
public static bool
IsPlatformTextureFormatValid(TextureImporterType textureType,
                             BuildTarget target,
                             TextureImporterFormat currentFormat) {
	if (currentFormat != TextureImporterFormat.Automatic) {
		int[] formatValues;
		string[] formatStrings;
		TextureImportValidFormats.GetPlatformTextureFormatValuesAndStrings(
			textureType, target, out formatValues, out formatStrings);
...
[RequiredByNativeCode]
public static bool
IsDefaultPlatformTextureFormatValid(TextureImporterType textureType,
                                    TextureImporterFormat currentFormat) {
	if (currentFormat != TextureImporterFormat.Automatic) {
		int[] formatValues;
		string[] formatStrings;
...
// When in linear rendering should this texture be sampled with hardware gamma
// correction (sRGB) or without (linear)?
...
[Obsolete(
	 "generateMipsInLinearSpace Property deprecated. Mipmaps are always generated in linear space.")]
...
[Obsolete(
	 "correctGamma Property deprecated. Mipmaps are always generated in linear space.")]
...
[Obsolete(
	 "normalmap Property deprecated. Check [[TextureImporterSettings.textureType]] instead. Getter will work as expected. Setter will set textureType to NormalMap if true, nothing otherwise.")]
...
[Obsolete(
	 "lightmap Property deprecated. Check [[TextureImporterSettings.textureType]] instead. Getter will work as expected. Setter will set textureType to Lightmap if true, nothing otherwise.")]
...
public extern TextureWrapMode wrapMode {
	[NativeName("GetWrapU")] get;
	[NativeName("SetWrapUVW")] set;
}
...
// Use alpha channel as transparency. Removes white borders from transparent
// textures
...
public extern SecondarySpriteTexture[] secondarySpriteTextures {
	get;
	set;
}
...
// The number of pixels in one unit. Note: The C++ side still uses the name
// pixelsToUnits which is misleading, but has not been changed yet to minimize
// merge conflicts.
...
public extern Vector2 spritePivot {
	get;
	set;
}
public extern Vector4 spriteBorder {
	get;
	set;
}
...
private extern TextureImporterSettings settings {
	get;
	set;
}
...
public extern void ReadTextureImportInstructions(
	BuildTarget target, out TextureFormat desiredFormat,
	out ColorSpace colorSpace, out int compressionQuality);
...
internal extern bool removeMatte {
	get;
	set;
}

public extern bool ignorePngGamma {
	get;
	set;
}
internal static readonly int MaxTextureSizeAllowedForReadable =
	8192; // keep in sync with TextureImporter.h
}
}
