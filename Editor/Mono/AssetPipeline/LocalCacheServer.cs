...
internal class LocalCacheServer : ScriptableSingleton<LocalCacheServer> {
  ...
  void Create(int _port, ulong _size) {
    var nodeExecutable = Paths.Combine(
        EditorApplication.applicationContentsPath, "Tools", "nodejs");
    ...
    var cacheServerJs = Paths.Combine(EditorApplication.applicationContentsPath,
                                      "Tools", "CacheServer", "main.js");
    var processStartInfo = new ProcessStartInfo(nodeExecutable){
        Arguments = "\"" + cacheServerJs + "\"" + " --port " + _port +
                    " --path \"" + path + "\" --nolegacy" +
                    " --monitor-parent-process " +
                    Process.GetCurrentProcess().Id
                    // node.js has issues running on windows with stdout not
                    // redirected. so we silence logging to avoid that. And also
                    // to avoid CacheServer spamming the editor logs on OS X.
                    + " --silent" + " --size " + _size,
  ...
  public static bool WaitForServerToComeAlive(int port) {
    ...
    while (DateTime.Now < maximum) {
      if (PingHost("localhost", port, 10)) {
        System.Console.WriteLine("Server Came alive after {0} ms",
  ...
  public static void CreateIfNeeded() {
    ...
    // Check if this process is really the one we used (and not another one
    // reusing the PID).
    if (p != null && p.StartTime.ToString() == instance.time) {
      if (instance.size == size && instance.path == GetCacheLocation()) {
        // We have a server running for this setup, which we can reuse, but make
        // sure that the cache server directory exists in case it was cleaned
        // earlier
        CreateCacheDirectory();
        return;
      } else {
        // This server does not match our setup. Kill it, so we can start a new
        // one.
        Kill();
      }
    }
  ...
  public static void Setup() {
    var mode = (CacheServerPreferences.CacheServerMode)
                   EditorPrefs.GetInt("CacheServerMode");
