...
/// Universal structure that holds all the data relevant to importing an
/// asset, including temporary data that needs to be shared across stages that
/// make on any given importer's pipeline.
///
/// Breaking up legacy importers into peaces and re-arranging them as
/// pipelines that use those pieces so that the pieces can become building
/// blocks that other importers can re-use implies that the pieces is not
/// coupled with any given importer. For this decoupling and maximizing reuse,
/// we need something that can hold the information describing what is being
/// imported but also the data generated by the various parts that make up an
/// importers pipeline. This container simply transports information from one
/// "stage" to the other. Each stage is free to add/delete/alter the content
/// of the container
...
extern void LogMessage(string msg, string file, int line,
                       UnityEngine.Object obj, bool isAnError);

[NativeThrows]
public extern void SetMainObject(Object obj);
public extern Object mainObject {
	get;
}
...
[FreeFunction("AssetImportContextBindings::GetObjects",
              HasExplicitThis = true)]
public extern void GetObjects([ NotNull ] List<Object> objects);

[NativeThrows]
public extern void AddObjectToAsset(string identifier, Object obj,
                                    Texture2D thumbnail);

// Create a dependency against the contents of the source asset at the
// provided path
...
public void DependsOnSourceAsset(string path) {
	if (string.IsNullOrEmpty(path)) {
		throw new ArgumentNullException(
			      "path", "Cannot add dependency on invalid path.");
	}
...
internal void LogImportError(string msg, string file, int line,
...
internal void LogImportWarning(string msg, string file, int line,
                               UnityEngine.Object obj = null) {
...
void AddToLog(string msg, string file, int line, bool isAnError,
