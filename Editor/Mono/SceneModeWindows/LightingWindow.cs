...
internal class LightingWindow : EditorWindow {
  static class Styles {
    ...
    public static readonly GUIContent buildLabel = EditorGUIUtility.TrTextContent(
        "Generate Lighting",
        "Generates the lightmap data for the current master scene.  This lightmap data (for realtime and baked global illumination) is stored in the GI Cache. For GI Cache settings see the Preferences panel.");

    public static string[] BakeModeStrings = {"Bake Reflection Probes",
                                              "Clear Baked Data"};

    public static readonly float ButtonWidth = 90;
  }

  enum BakeMode { BakeReflectionProbes = 0, Clear = 1 }
  ...
  const string kGlobalIlluminationUnityManualPage =
      "file:///unity/Manual/GlobalIllumination.html";
  ...
  SerializedObject lightingSettings {
    get {
      ...
      if (m_LightingSettings == null ||
          m_LightingSettings.targetObject == null ||
          m_LightingSettings.targetObject !=
              Lightmapping.lightingSettingsInternal) {
  ...
  internal void SetSelectedTabIndex(int index) { m_SelectedModeIndex = index; }
  ...
  void OnEnable() {
    ...
    m_RealtimeLightmapsTab =
        new LightingWindowLightmapPreviewTab(LightmapType.DynamicLightmap);
    m_BakedLightmapsTab =
        new LightingWindowLightmapPreviewTab(LightmapType.StaticLightmap);
  ...
  void OnBecameVisible() { RepaintSceneAndGameViews(); }

  void OnBecameInvisible() { RepaintSceneAndGameViews(); }
  ...
  void SetupModes() {
    ...
    bool isBakedSupported =
        SupportedRenderingFeatures.IsLightmapBakeTypeSupported(
            LightmapBakeType.Baked);
    bool isEnvironmentSupported =
        !(SupportedRenderingFeatures.active.overridesEnvironmentLighting &&
          SupportedRenderingFeatures.active.overridesFog &&
          SupportedRenderingFeatures.active.overridesOtherLightingSettings);

    if (m_IsRealtimeSupported != isRealtimeSupported ||
  ...
  void DrawHelpGUI() {
    var iconSize =
        EditorStyles.iconButton.CalcSize(EditorGUI.GUIContents.helpIcon);
    var rect = GUILayoutUtility.GetRect(iconSize.x, iconSize.y);

    if (GUI.Button(rect, EditorGUI.GUIContents.helpIcon,
  ...
  void DrawSettingsGUI(Mode mode) {
    if (mode == Mode.LightingSettings || mode == Mode.EnvironmentSettings) {
      var iconSize = EditorStyles.iconButton.CalcSize(
          EditorGUI.GUIContents.titleSettingsIcon);
      var rect = GUILayoutUtility.GetRect(iconSize.x, iconSize.y);

      if (EditorGUI.DropdownButton(
              rect, EditorGUI.GUIContents.titleSettingsIcon, FocusType.Passive,
              EditorStyles.iconButton)) {
        if (mode == Mode.LightingSettings)
          EditorUtility.DisplayCustomMenu(
              rect, new[]{EditorGUIUtility.TrTextContent("Reset")}, -1,
              ResetLightingSettings, null);
        else if (mode == Mode.EnvironmentSettings)
          EditorUtility.DisplayCustomMenu(
              rect, new[]{EditorGUIUtility.TrTextContent("Reset")}, -1,
              ResetEnvironmentSettings, null);
      }
    }
  }
  ...
  void ResetLightingSettings(object userData, string[] options, int selected) {
    if (Lightmapping.lightingSettingsInternal != null) {
      Undo.RecordObjects(new[]{Lightmapping.lightingSettingsInternal},
                         "Reset Lighting Settings");
      Unsupported.SmartReset(Lightmapping.lightingSettingsInternal);
    }
  }

  void ResetEnvironmentSettings(object userData, string[] options,
                                int selected) {
    Undo.RecordObjects(new[]{RenderSettings.GetRenderSettings()},
  ...
  void DrawTopBarGUI(Mode selectedMode) {
    ...
    if (selectedMode == Mode.LightingSettings ||
        selectedMode == Mode.EnvironmentSettings)
      GUILayout.Space(
          EditorStyles.iconButton.CalcSize(EditorGUI.GUIContents.helpIcon).x);

    GUILayout.FlexibleSpace();

    if (m_Modes.Count > 1) {
      m_SelectedModeIndex = GUILayout.Toolbar(
          m_SelectedModeIndex, m_ModeStrings, Styles.buttonStyle,
  ...
  void Buttons(bool showAutoToggle) {
    using(new EditorGUI.DisabledScope(
      ...
      if (Lightmapping.lightingDataAsset &&
          !Lightmapping.lightingDataAsset.isValid) {
        EditorGUILayout.HelpBox(
            Lightmapping.lightingDataAsset.validityErrorMessage,
            MessageType.Warning);
      }
      ...
      bool iterative = m_WorkflowMode.intValue ==
                       (int) Lightmapping.GIWorkflowMode.Iterative;
      Rect rect =
          GUILayoutUtility.GetRect(Styles.continuousBakeLabel, GUIStyle.none);
      ...
      using(new EditorGUI.DisabledScope(iterative)) {
        ...
        if (showBakeButton) {
          if (EditorGUI.ButtonWithDropdownList(
                  Styles.buildLabel, Styles.BakeModeStrings,
                  BakeDropDownCallback, GUILayout.Width(170))) {
            DoBake();

            // DoBake could've spawned a save scene dialog. This breaks GUI on
            // mac (Case 490388). We work around this with an ExitGUI here.
            GUIUtility.ExitGUI();
          }
        }
        // Cancel button if we are currently baking
          ...
          if (settings.lightmapper != LightingSettings.Lightmapper.Enlighten &&
              settings.bakedGI &&
              GUILayout.Button("Force Stop",
  ...
  void Summary() {
    ...
    foreach (LightmapData ld in LightmapSettings.lightmaps) {
      ...
      Vector2 texSize =
          new Vector2(ld.lightmapColor.width, ld.lightmapColor.height);
    ...
    sizesString.Append((directionalLightmapsMode ? " Directional"
                        : " Non-Directional"));
    ...
    foreach (var s in sizes) {
      ...
      sizesString.Append(
          s.Key.x.ToString(CultureInfo.InvariantCulture.NumberFormat));
      sizesString.Append("x");
      sizesString.Append(
          s.Key.y.ToString(CultureInfo.InvariantCulture.NumberFormat));
    ...
    GUILayout.BeginVertical();
    GUILayout.Label(EditorUtility.FormatBytes(totalMemorySize),
                    Styles.labelStyle);
    GUILayout.Label((lightmapCount == 0 ? "No Lightmaps" : ""),
                    Styles.labelStyle);
    GUILayout.EndVertical();
    ...
    if (Lightmapping.GetLightingSettingsOrDefaultsFallback().lightmapper !=
      ...
      GUILayout.BeginVertical();
      GUILayout.Label("Occupied Texels: " +
                          InternalEditorUtility.CountToString(
                              Lightmapping.occupiedTexelCount),
                      Styles.labelStyle);
      if (Lightmapping.isRunning) {
        ...
        if (Lightmapping.atlasCount > 0) {
          int convergedMaps =
              numConvergedLightmapsInView + numConvergedLightmapsNotInView;
          GUILayout.Label("Lightmap convergence: (" + convergedMaps + "/" +
                              Lightmapping.atlasCount + ")",
                          Styles.labelStyle);
        }
        EditorGUILayout.LabelField("Lightmaps in view: " + numLightmapsInView,
                                   Styles.labelStyle);
        EditorGUI.indentLevel += 1;
        EditorGUILayout.LabelField("Converged: " + numConvergedLightmapsInView,
                                   Styles.labelStyle);
        EditorGUILayout.LabelField("Not Converged: " +
                                       numNotConvergedLightmapsInView,
                                   Styles.labelStyle);
        EditorGUI.indentLevel -= 1;
        EditorGUILayout.LabelField("Lightmaps not in view: " +
                                       numLightmapsNotInView,
                                   Styles.labelStyle);
        EditorGUI.indentLevel += 1;
        EditorGUILayout.LabelField(
            "Converged: " + numConvergedLightmapsNotInView, Styles.labelStyle);
        EditorGUILayout.LabelField("Not Converged: " +
                                       numNotConvergedLightmapsNotInView,
                                   Styles.labelStyle);
        EditorGUI.indentLevel -= 1;
        ...
        if (lpc.IsValid() && lpc.probeSetCount > 0)
          GUILayout.Label("Light Probes convergence: (" +
                              lpc.convergedProbeSetCount + "/" +
                              lpc.probeSetCount + ")",
                          Styles.labelStyle);
      }
      ...
      if (!Lightmapping.isRunning) {
        ...
        if (bakeTime >= 0.0) {
          ...
          GUILayout.Label("Total Bake Time: " + timeH.ToString("0") + ":" +
                              timeM.ToString("00") + ":" + timeS.ToString("00"),
                          Styles.labelStyle);
          if (Unsupported.IsDeveloperBuild())
            GUILayout.Label("(Raw Bake Time: " + timeRawH.ToString("0") + ":" +
                                timeRawM.ToString("00") + ":" +
                                timeRawS.ToString("00") +
                                ", Overhead: " + oHeadTimeH.ToString("0") +
                                ":" + oHeadTimeM.ToString("00") + ":" +
                                oHeadTimeS.ToString("00") + ")",
                            Styles.labelStyle);
        }
      }
