...
public class LightingExplorerExtensionAttribute
...
internal class LightingExplorerWindow : EditorWindow {
  ...
  static void CreateLightingExplorerWindow() {
    LightingExplorerWindow window =
        EditorWindow.GetWindow<LightingExplorerWindow>();
  ...
  void OnInspectorUpdate() {
    if (m_TableTabs != null && (int) m_SelectedTab >= 0 &&
        (int) m_SelectedTab < m_TableTabs.Length) {
      m_TableTabs [(int) m_SelectedTab]
          .OnInspectorUpdate();
    }
  }
          ...
          int[] selectedIds =
              UnityEngine.Object.FindObjectsOfType<MeshRenderer>()
                  .Where((MeshRenderer mr) => {
                    return Selection.instanceIDs.Contains(
                        mr.gameObject.GetInstanceID());
                  })
  ...
  void OnGUI() {
    ...
    if (m_TabTitles != null)
      m_SelectedTab =
          GUILayout.Toolbar(m_SelectedTab, m_TabTitles, "LargeButton",
  ...
  private void UpdateTabs() {
    ...
    if (m_CurrentLightingExplorerExtension == null ||
      ...
      if (m_CurrentLightingExplorerExtension.GetContentTabs() == null ||
          m_CurrentLightingExplorerExtension.GetContentTabs().Length == 0) {
      ...
      m_TabTitles =
          m_TableTabs !=
          null ? m_TableTabs.Select(item => item.title).ToArray() : null;
    }
  }

  private ILightingExplorerExtension GetDefaultLightingExplorerExtension() {
    if (s_DefaultLightingExplorerExtension == null) {
      s_DefaultLightingExplorerExtension =
          new DefaultLightingExplorerExtension();
    }
    return s_DefaultLightingExplorerExtension;
  }

  private ILightingExplorerExtension
  GetLightExplorerExtension(System.Type currentSRPType) {
    if (currentSRPType == null)
      return GetDefaultLightingExplorerExtension();

    Type extensionType =
        RenderPipelineEditorUtility
            .FetchFirstCompatibleTypeUsingScriptableRenderPipelineExtension<
                ILightingExplorerExtension>();
    if (extensionType != null) {
      ILightingExplorerExtension extension =
          (ILightingExplorerExtension)
              System.Activator.CreateInstance(extensionType);
      return extension;
    }
