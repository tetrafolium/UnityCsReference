...
internal class LightingWindowLightmapPreviewTab {
  ...
  static class Styles {
    public static readonly GUIStyle SelectedLightmapHighlight =
        "LightmapEditorSelectedHighlight";
    public static readonly GUIContent LightingDataAsset =
        EditorGUIUtility.TrTextContent(
            "Lighting Data Asset",
            "A different LightingData.asset can be assigned here. These assets are generated by baking a scene in the OnDemand mode.");
  ...
  private bool isRealtimeLightmap {
    get { return m_LightmapType == LightmapType.DynamicLightmap; }
  }
  ...
  public void UpdateActiveGameObjectSelection() {
    ...
    // if the active object in the selection is a renderer or a terrain, we're
    // interested in it's lightmapIndex
    if (Selection.activeGameObject == null ||
        ((renderer = Selection.activeGameObject.GetComponent<MeshRenderer>()) ==
             null &&
  ...
  public void OnGUI(Rect position) {
    ...
    VisualisationGITexture[] realtimeLightmaps =
        LightmapVisualizationUtility.GetRealtimeGITextures(
  ...
  private void InitSettings() {
    if (m_LightmapSettings == null || m_LightmapSettings.targetObject == null ||
      ...
      m_LightingDataAsset =
          m_LightmapSettings.FindProperty("m_LightingDataAsset");
    }
  }

  private void LightmapListGUI(LightmapData[] lightmaps,
                               VisualisationGITexture[] realtimeLightmaps) {
    m_ScrollPosition = GUILayout.BeginScrollView(m_ScrollPosition);

    if (!Lightmapping.GetLightingSettingsOrDefaultsFallback().autoGenerate &&
        !isRealtimeLightmap) {
      EditorGUILayout.PropertyField(m_LightingDataAsset,
                                    Styles.LightingDataAsset);
    ...
    int length =
        isRealtimeLightmap ? realtimeLightmaps.Length : lightmaps.Length;

    if (m_SelectedLightmapIndex >= length)
      m_SelectedLightmapIndex = 0;

    for (int i = 0; i < length; i++) {
      Texture2D texture = isRealtimeLightmap ? realtimeLightmaps[i]
                              .texture : lightmaps[i]
                              .lightmapColor;
      ...
      if (texture != null) {
        ...
        Hash128 hash =
            isRealtimeLightmap ? realtimeLightmaps[i].hash : new Hash128();
        ...
        GUILayout.Label("Size: " + texture.width + "x" + texture.height,
                        EditorStyles.miniBoldLabel);
        GUILayout.Label("Format: " + texture.format,
                        EditorStyles.miniBoldLabel);
  ...
  private void LightmapField(Texture2D lightmap, int index, Hash128 hash) {
    ...
    if (EditorGUI.Toggle(rect, index == m_SelectedLightmapIndex,
      ...
      if ((buttonRect.Contains(Event.current.mousePosition) &&
        ...
        if (isRealtimeLightmap)
          LightmapPreviewWindow.CreateLightmapPreviewWindow(
              m_SelectedLightmapIndex, true, true);
        else
          LightmapPreviewWindow.CreateLightmapPreviewWindow(
              m_SelectedLightmapIndex, false, true);
        ...
        EditorGUI.PingObjectOrShowPreviewOnClick(
            actualTargetObject, GUILayoutUtility.GetLastRect());
      }
    }
    ...
    if (Event.current.type == EventType.Repaint) {
      ...
      if ((!isRealtimeLightmap && index == m_ActiveGameObjectLightmapIndex) ||
          (isRealtimeLightmap && hash == m_ActiveGameObjectTextureHash)) {
  ...
  private void DebugInfoSection(LightmapData[] lightmaps) {
...

    {
      ...
      if (gpuMemory > 0.0f) {
        string foldoutGPUMemoryGPU =
            String.Format("Total GPU memory ({0})", SizeString(gpuMemory));
        EditorGUILayout.FoldoutTitlebar(
            false, new GUIContent(foldoutGPUMemoryGPU), true);
        if (GUILayout.Button("Dump detail to Editor.log",
    ...
    {
      Dictionary<Hash128, SortedList<int, int>> gbufferHashToLightmapIndices =
          new Dictionary<Hash128, SortedList<int, int>>();
      for (int i = 0; i < lightmaps.Length; i++) {
        Hash128 gbufferHash;
        if (Lightmapping.GetGBufferHash(i, out gbufferHash)) {
          if (!gbufferHashToLightmapIndices.ContainsKey(gbufferHash))
            gbufferHashToLightmapIndices.Add(gbufferHash,
                                             new SortedList<int, int>());

          gbufferHashToLightmapIndices [gbufferHash]
              .Add(i, i);
        }
      }
      ...
      foreach (var entry in gbufferHashToLightmapIndices) {
        ...
        foreach (var i in lightmapIndices) {
          LightmapMemory lightmapMemory =
              Lightmapping.GetLightmapMemory(i.Value);
      ...
      string foldoutNameFull = String.Format(
          "G-buffers ({0}) | Lightmaps ({1})", SizeString(totalGBuffersSize),
          SizeString(totalLightmapsSize));

      if (lightmaps.Length > 0)
        EditorGUILayout.FoldoutTitlebar(false, new GUIContent(foldoutNameFull),
                                        true);
    }
...

    {
      ...
      ShowObjectNamesSizesAndCounts("Light Probes", kEditorPrefsLightProbes,
                                    labels.labels, labels.sizes, dummyCounts);
    }

    {
      MemLabels labels = Lightmapping.GetTransmissionTexturesMemLabels();
      ShowObjectNamesSizesAndCounts("Transmission textures",
                                    kEditorPrefsTransmissionTextures,
                                    labels.labels, labels.sizes, dummyCounts);
    }

    {
      MemLabels labels = Lightmapping.GetMaterialTexturesMemLabels();
      ShowObjectNamesSizesAndCounts("Albedo/emissive textures",
                                    kEditorPrefsMaterialTextures, labels.labels,
                                    labels.sizes, dummyCounts);
    }

    {
      GeoMemLabels labels = Lightmapping.GetGeometryMemory();
      ShowObjectNamesSizesAndCounts("Geometry data", kEditorPrefsGeometryData,
                                    labels.labels, labels.sizes,
                                    labels.triCounts);
    }

    {
      // Note: this needs to go last.
      // It simply shows all the memory labels that were not explicitly queried
      // after the Lightmapping.ResetExplicitlyShownMemLabels() call.
      MemLabels labels = Lightmapping.GetNotShownMemLabels();
      string remainingEntriesFoldoutName =
          Lightmapping.isProgressiveLightmapperDone ? "Leaks" : "In-flight";
      ShowObjectNamesSizesAndCounts(remainingEntriesFoldoutName,
                                    kEditorPrefsInFlight, labels.labels,
                                    labels.sizes, dummyCounts);
    }
  ...
  private void LightmapDebugInfo(int index) {
    ...
    if (lc.IsValid()) {
      GUILayout.Label("Occupied: " + InternalEditorUtility.CountToString(
                                         (ulong) lc.occupiedTexelCount),
                      EditorStyles.miniLabel);

      GUIContent direct = EditorGUIUtility.TrTextContent(
          "Direct: " + lc.minDirectSamples + " / " + lc.maxDirectSamples +
              " / " + lc.avgDirectSamples + "",
          "min / max / avg samples per texel");
      GUILayout.Label(direct, EditorStyles.miniLabel);

      GUIContent gi = EditorGUIUtility.TrTextContent(
          "GI: " + lc.minGISamples + " / " + lc.maxGISamples + " / " +
              lc.avgGISamples + "",
          "min / max / avg samples per texel");
      GUILayout.Label(gi, EditorStyles.miniLabel);

      GUIContent env = EditorGUIUtility.TrTextContent(
          "Environment: " + lc.minEnvSamples + " / " + lc.maxEnvSamples +
              " / " + lc.avgEnvSamples + "",
          "min / max / avg samples per texel");
    ...
    if (mraysPerSec >= 0.0)
      GUILayout.Label(mraysPerSec.ToString(
    ...
    GUILayout.Label("Lightmap data: " +
                        SizeString(lightmapMemory.lightmapDataSizeCPU),
                    EditorStyles.miniLabel);

    GUIContent lightmapTexturesSizeContent = null;
    if (lightmapMemory.lightmapTexturesSize > 0.0f)
      lightmapTexturesSizeContent = EditorGUIUtility.TrTextContent(
          "Lightmap textures: " +
          SizeString(lightmapMemory.lightmapTexturesSize));
    else
      lightmapTexturesSizeContent = EditorGUIUtility.TrTextContent(
          "Lightmap textures: N/A",
          "This lightmap has converged and is not owned by the Progressive Lightmapper anymore.");
    ...
    if (lightmapMemory.lightmapDataSizeGPU > 0.0f)
      GPUSizeContent = EditorGUIUtility.TrTextContent(
          "GPU memory: " + SizeString(lightmapMemory.lightmapDataSizeGPU));
    else
  ...
  private void ShowObjectNamesSizesAndCounts(string foldoutName,
                                             string editorPrefsName,
                                             string[] objectNames,
                                             float[] sizes,
    ...
    string countString =
        counts.Length > 0 ? (SumCounts(counts) + " tris, ") : "";
    string sizeString = SizeString(SumSizes(sizes));
    string foldoutNameFull =
        foldoutName + " (" + countString + sizeString + ")";
    bool showDetailsOld = EditorPrefs.GetBool(editorPrefsName, false);

    bool showDetails = EditorGUILayout.FoldoutTitlebar(
        showDetailsOld, new GUIContent(foldoutNameFull), true);
    ...
    GUILayout.BeginHorizontal();
    {
      ...
      for (int i = 0; i < objectNames.Length; ++i) {
        ...
        string[] result = fullName.Split(stringSeparators,
        ...
        GUILayout.Label(new GUIContent(objectName, tooltip),
                        EditorStyles.miniLabel);
      }
      GUILayout.EndVertical();
