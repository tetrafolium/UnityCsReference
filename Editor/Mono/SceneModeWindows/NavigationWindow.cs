...
internal class NavMeshEditorWindow : EditorWindow,
                                     IHasCustomMenu {
  ...
  private class Styles {
    public readonly GUIContent m_AgentRadiusContent =
        EditorGUIUtility.TrTextContent(
            "Agent Radius", "How close to the walls navigation mesh exist.");
    public readonly GUIContent m_AgentHeightContent =
        EditorGUIUtility.TrTextContent(
            "Agent Height", "How much vertical clearance space must exist.");
    public readonly GUIContent m_AgentSlopeContent =
        EditorGUIUtility.TrTextContent("Max Slope",
                                       "Maximum slope the agent can walk up.");
    public readonly GUIContent m_AgentDropContent =
        EditorGUIUtility.TrTextContent("Drop Height",
                                       "Maximum agent drop height.");
    public readonly GUIContent m_AgentClimbContent = EditorGUIUtility.TrTextContent(
        "Step Height",
        "The height of discontinuities in the level the agent can climb over (i.e. steps and stairs).");
    public readonly GUIContent m_AgentJumpContent =
        EditorGUIUtility.TrTextContent("Jump Distance",
                                       "Maximum agent jump distance.");
    public readonly GUIContent m_AgentPlacementContent =
        EditorGUIUtility.TrTextContent(
            "Height Mesh",
            "Generate an accurate height mesh for precise agent placement (slower).");
    public readonly GUIContent m_MinRegionAreaContent =
        EditorGUIUtility.TrTextContent(
            "Min Region Area", "Minimum area that a navmesh region can be.");
    public readonly GUIContent m_ManualCellSizeContent =
        EditorGUIUtility.TrTextContent("Manual Voxel Size",
                                       "Enable to set voxel size manually.");
    public readonly GUIContent m_CellSizeContent = EditorGUIUtility.TrTextContent(
        "Voxel Size",
        "Specifies at the voxelization resolution at which the NavMesh is build.");
    public readonly GUIContent m_LearnAboutComponent = EditorGUIUtility.TrTextContent(
        "Learn instead about the component workflow.",
        "Components available for building and using navmesh data for different agent types.");

    public readonly GUIContent m_AgentSizeHeader =
        EditorGUIUtility.TrTextContent("Baked Agent Size");
    public readonly GUIContent m_OffmeshHeader =
        EditorGUIUtility.TrTextContent("Generated Off Mesh Links");
    public readonly GUIContent m_AdvancedHeader =
        EditorGUIUtility.TrTextContent("Advanced");
    public readonly GUIContent m_AgentTypesHeader =
        EditorGUIUtility.TrTextContent("Agent Types");

    public readonly GUIContent m_NameLabel =
        EditorGUIUtility.TrTextContent("Name");
    public readonly GUIContent m_CostLabel =
        EditorGUIUtility.TrTextContent("Cost");
    ...
    public readonly GUIContent[] m_ModeToggles = {
        ...
        EditorGUIUtility.TrTextContent(
            "Object", "Bake settings for the currently selected object."),
    };
  }
  ...
  public static void OpenAgentSettings(int agentTypeID) {
    ...
    for (int i = 0; i < s_NavMeshEditorWindow.m_Agents.arraySize; i++) {
      SerializedProperty agent =
          s_NavMeshEditorWindow.m_Agents.GetArrayElementAtIndex(i);
  ...
  public void OnEnable() {
    ...
    m_DisplayControlsOverlayWindow = new OverlayWindow(
        EditorGUIUtility.TrTextContent("Navmesh Display"), DisplayControls,
        (int) SceneViewOverlay.Ordering.NavMesh, null,
        SceneViewOverlay.WindowDisplayOption.OneWindowPerTarget);
    m_DisplayAgentControlsOverlayWindow = new OverlayWindow(
        EditorGUIUtility.TrTextContent("Agent Display"), DisplayAgentControls,
        (int) SceneViewOverlay.Ordering.NavMesh, null,
        SceneViewOverlay.WindowDisplayOption.OneWindowPerTarget);
    m_DisplayObstacleControlsOverlayWindow = new OverlayWindow(
        EditorGUIUtility.TrTextContent("Obstacle Display"),
        DisplayObstacleControls, (int) SceneViewOverlay.Ordering.NavMesh, null,
        SceneViewOverlay.WindowDisplayOption.OneWindowPerTarget);
  }

  private void InitProjectSettings() {
    if (m_NavMeshProjectSettingsObject == null) {
      Object obj = Unsupported.GetSerializedAssetInterfaceSingleton(
  ...
  private void InitSceneBakeSettings() {
    ...
    m_SettingsObject =
        new SerializedObject(EditorNavMeshBuilder.navMeshSettingsObject);
    ...
    m_LedgeDropHeight =
        m_SettingsObject.FindProperty(kRootPath + "ledgeDropHeight");
    m_AgentClimb = m_SettingsObject.FindProperty(kRootPath + "agentClimb");
    m_MaxJumpAcrossDistance =
        m_SettingsObject.FindProperty(kRootPath + "maxJumpAcrossDistance");

    // Advanced Settings
    m_MinRegionArea =
        m_SettingsObject.FindProperty(kRootPath + "minRegionArea");
    m_ManualCellSize =
        m_SettingsObject.FindProperty(kRootPath + "manualCellSize");
    m_CellSize = m_SettingsObject.FindProperty(kRootPath + "cellSize");
    m_AccuratePlacement =
        m_SettingsObject.FindProperty(kRootPath + "accuratePlacement");
  }
  ...
  private void InitAreas() {
    ...
    if (m_AreasList == null) {
      m_AreasList = new ReorderableList(m_NavMeshProjectSettingsObject, m_Areas,
                                        false, false, false, false);
      m_AreasList.drawElementCallback = DrawAreaListElement;
      m_AreasList.drawHeaderCallback = DrawAreaListHeader;
      m_AreasList.elementHeight = EditorGUIUtility.singleLineHeight +
  ...
  private void InitAgents() {
    if (m_Agents == null) {
      ...
      m_SettingNames =
          m_NavMeshProjectSettingsObject.FindProperty("m_SettingNames");
    }
    if (m_AgentsList == null) {
      m_AgentsList = new ReorderableList(m_NavMeshProjectSettingsObject,
                                         m_Agents, false, false, true, true);
      ...
      m_AgentsList.elementHeight = EditorGUIUtility.singleLineHeight +
                                   EditorGUIUtility.standardVerticalSpacing;
    }
  }

  // This code is replicated from the navmesh debug draw code.
  private int Bit(int a, int b) { return (a & (1 << b)) >> b; }
  ...
  void UpdateSelectedAgentAndObstacleState() {
    Object[] selectedAgents = Selection.GetFiltered(
        typeof(NavMeshAgent),
        SelectionMode.ExcludePrefab | SelectionMode.Editable);
    Object[] selectedObstacles = Selection.GetFiltered(
  ...
  void ModeToggle() {
    ...
    m_Mode = (Mode) GUILayout.Toolbar((int) m_Mode, s_Styles.m_ModeToggles,
                                      "LargeButton",
  ...
  private void GetAreaListRects(Rect rect, out Rect stripeRect,
                                out Rect labelRect, out Rect nameRect,
                                out Rect costRect) {
  ...
  private void DrawAreaListHeader(Rect rect) {
    ...
    GetAreaListRects(rect, out stripeRect, out labelRect, out nameRect,
                     out costRect);
    GUI.Label(nameRect, s_Styles.m_NameLabel);
    GUI.Label(costRect, s_Styles.m_CostLabel);
  }

  private void DrawAreaListElement(Rect rect, int index, bool selected,
                                   bool focused) {
    ...
    rect.height -=
        2; // nicer looking with selected list row and a text field in it
    ...
    Color dimmed =
        new Color(color.r * 0.1f, color.g * 0.1f, color.b * 0.1f, 0.6f);
    EditorGUI.DrawRect(stripeRect, color);

    EditorGUI.DrawRect(
        new Rect(stripeRect.x, stripeRect.y, 1, stripeRect.height), dimmed);
    EditorGUI.DrawRect(new Rect(stripeRect.x + stripeRect.width - 1,
                                stripeRect.y, 1, stripeRect.height),
                       dimmed);
    EditorGUI.DrawRect(
        new Rect(stripeRect.x + 1, stripeRect.y, stripeRect.width - 2, 1),
        dimmed);
    EditorGUI.DrawRect(new Rect(stripeRect.x + 1,
                                stripeRect.y + stripeRect.height - 1,
                                stripeRect.width - 2, 1),
                       dimmed);
  ...
  static void DisplayControls(Object target, SceneView sceneView) {
    ...
    if (showNavMesh !=
        EditorGUILayout.Toggle(EditorGUIUtility.TrTextContent("Show NavMesh"),
                               showNavMesh)) {
    ...
    using(new EditorGUI.DisabledScope(
        !NavMeshVisualizationSettings.hasHeightMesh)) {
    ...
    if (Unsupported.IsDeveloperMode()) {
      ...
      if (showNavMeshLinks !=
          EditorGUILayout.Toggle(
              EditorGUIUtility.TrTextContent("Show NavMesh Links"),
              showNavMeshLinks)) {
  ...
  void OnInspectorUpdate() {
    ...
    if (m_SelectedNavMeshAgentCount > 0) {
      if (m_HasPendingAgentDebugInfo !=
          NavMeshVisualizationSettings.hasPendingAgentDebugInfo) {
  ...
  static void DisplayAgentControls(Object target, SceneView sceneView) {
    ...
    // NavMeshVisualizationSettings.hasPendingAgentDebugInfo can change between
    // events, capture the value on Layout event.
    if (Event.current.type == EventType.Layout) {
      s_NavMeshEditorWindow.m_HasPendingAgentDebugInfo =
          NavMeshVisualizationSettings.hasPendingAgentDebugInfo;
    }
    ...
    if (showAgentPath !=
        EditorGUILayout.Toggle(
            EditorGUIUtility.TrTextContent(
                "Show Path Polygons", "Shows the polygons leading to goal."),
            showAgentPath)) {
    ...
    if (showAgentWalls !=
        EditorGUILayout.Toggle(
            EditorGUIUtility.TrTextContent(
                "Show Walls",
                "Shows the wall segments handled during simulation."),
            showAgentWalls)) {
    ...
    if (showAgentAvoidance) {
      if (s_NavMeshEditorWindow.m_HasPendingAgentDebugInfo) {
        ...
        EditorGUILayout.HelpBox(
            "Avoidance display is not valid until after next game update.",
            MessageType.Warning);
        EditorGUILayout.EndVertical();
      }
      // Should match NavMeshManager::kDebugAgentCount
      const int kDebugAgentCount = 10;
      if (s_NavMeshEditorWindow.m_SelectedNavMeshAgentCount >
          kDebugAgentCount) {
        EditorGUILayout.BeginVertical(GUILayout.MaxWidth(165));
        EditorGUILayout.HelpBox(
            string.Format(
                "Avoidance visualization can be drawn for {0} agents ({1} selected).",
                kDebugAgentCount,
  ...
  static void DisplayObstacleControls(Object target, SceneView sceneView) {
    ...
    var showObstacleCarveHull =
        NavMeshVisualizationSettings.showObstacleCarveHull;
    if (showObstacleCarveHull !=
        EditorGUILayout.Toggle(
            EditorGUIUtility.TrTextContent(
                "Show Carve Hull",
                "Shows the hull used to carve the obstacle from navmesh."),
            showObstacleCarveHull)) {
      NavMeshVisualizationSettings.showObstacleCarveHull =
          !showObstacleCarveHull;
      bRepaint = true;
    }
  ...
  public virtual void AddItemsToMenu(GenericMenu menu) {
    menu.AddItem(EditorGUIUtility.TrTextContent("Reset Legacy Bake Settings"),
                 false, ResetBakeSettings);
  }
  ...
  private static void ObjectSettings() {
    ...
    MeshRenderer[] renderers =
        SceneModeUtility.GetSelectedObjectsOfType<MeshRenderer>(out gos);
    if (gos.Length > 0) {
    ...
    Terrain[] terrains =
        SceneModeUtility.GetSelectedObjectsOfType<Terrain>(out gos);
    if (gos.Length > 0) {
      emptySelection = false;
    ...
    if (emptySelection)
      GUILayout.Label("Select a MeshRenderer or a Terrain from the scene.",
  ...
  private static void ObjectSettings(Object[] components, GameObject[] gos) {
    ...
    using(var so = new SerializedObject(gos)) {
      using(new EditorGUI.DisabledScope(!SceneModeUtility.StaticFlagField(
          "Navigation Static", so.FindProperty("m_StaticEditorFlags"),
          (int) StaticEditorFlags.NavigationStatic))) {
        SceneModeUtility.StaticFlagField(
            "Generate OffMeshLinks", so.FindProperty("m_StaticEditorFlags"),
        ...
        // Need to find the index as the list of names will compress out empty
        // area
        for (var i = 0; i < navAreaNames.Length; i++) {
          if (GameObjectUtility.GetNavMeshAreaFromName(navAreaNames[i]) ==
        ...
        var navMeshArea = EditorGUILayout.Popup("Navigation Area", navAreaIndex,
                                                navAreaNames);
        EditorGUI.showMixedValue = false;

        if (EditorGUI.EndChangeCheck()) {
          // Convert the selected index into absolute index...
          var newNavAreaIndex = GameObjectUtility.GetNavMeshAreaFromName(
              navAreaNames[navMeshArea]);

          GameObjectUtility.ShouldIncludeChildren includeChildren =
              GameObjectUtility.DisplayUpdateChildrenDialogIfNeeded(
                  Selection.gameObjects, "Change Navigation Area",
                  "Do you want change the navigation area to " +
                      navAreaNames[navMeshArea] +
                      " for all the child objects as well?");
          if (includeChildren !=
              GameObjectUtility.ShouldIncludeChildren.Cancel) {
  ...
  private void SceneBakeSettings() {
    ...
    EditorGUILayout.LabelField(s_Styles.m_AgentSizeHeader,
                               EditorStyles.boldLabel);

    // Draw image
    const float kDiagramHeight = 120.0f;
    Rect agentDiagramRect =
        EditorGUILayout.GetControlRect(false, kDiagramHeight);
    NavMeshEditorHelpers.DrawAgentDiagram(
        agentDiagramRect, m_AgentRadius.floatValue, m_AgentHeight.floatValue,
        m_AgentClimb.floatValue, m_AgentSlope.floatValue);

    // Agent Settings
    var radius = EditorGUILayout.FloatField(s_Styles.m_AgentRadiusContent,
                                            m_AgentRadius.floatValue);
    if (radius >= 0.001f &&
        !Mathf.Approximately(radius - m_AgentRadius.floatValue, 0.0f)) {
    ...
    // If radius is really small warn the user about it and instruct common use
    // case for small radius.
    if (m_AgentRadius.floatValue < 0.05f &&
        m_ManualCellSize.boolValue == false) {
      EditorGUILayout.HelpBox(
          "The agent radius you've set is really small, this can slow down the build.\nIf you intended to allow the agent to move close to the borders and walls, please adjust voxel size in advaced settings to ensure correct bake.",
          MessageType.Warning);
    }

    var height = EditorGUILayout.FloatField(s_Styles.m_AgentHeightContent,
                                            m_AgentHeight.floatValue);
    if (height >= 0.001f &&
        !Mathf.Approximately(height - m_AgentHeight.floatValue, 0.0f))
      m_AgentHeight.floatValue = height;

    const float kMaxSlopeAngle = 60.0f;
    EditorGUILayout.Slider(m_AgentSlope, 0.0f, kMaxSlopeAngle,
                           s_Styles.m_AgentSlopeContent);
    if (m_AgentSlope.floatValue > kMaxSlopeAngle) {
      EditorGUILayout.HelpBox(
          "The maximum slope should be set to less than " + kMaxSlopeAngle +
              " degrees to prevent NavMesh build artifacts on slopes. ",
          MessageType.Warning);
    }

    // Step height
    var newClimb = EditorGUILayout.FloatField(s_Styles.m_AgentClimbContent,
                                              m_AgentClimb.floatValue);
    if (newClimb >= 0.0f &&
        !Mathf.Approximately(m_AgentClimb.floatValue - newClimb, 0.0f))
    ...
    if (m_AgentClimb.floatValue > m_AgentHeight.floatValue) {
      ...
      EditorGUILayout.HelpBox(
          "Step height should be less than agent height.\nClamping step height to " +
              m_AgentHeight.floatValue + " internally when baking.",
          MessageType.Warning);
    }
    ...
    // Recast treats voxels whose neighbours min/max height difference is more
    // than step height.
    float slopeHeightPerVoxel =
        Mathf.Tan(m_AgentSlope.floatValue / 180.0f * Mathf.PI) * cs;
    ...
    if (slopeVx > walkableClimbVx) {
      ...
      EditorGUILayout.HelpBox(
          "Step Height conflicts with Max Slope. This makes some slopes unwalkable.\nConsider decreasing Max Slope to < " +
              betterSlopeAngle.ToString(
                  "0.0", CultureInfo.InvariantCulture.NumberFormat) +
              " degrees.\nOr, increase Step Height to > " +
              betterStep.ToString("0.00") + ".",
          MessageType.Warning);
    }

    EditorGUILayout.Space();

    EditorGUILayout.LabelField(s_Styles.m_OffmeshHeader,
                               EditorStyles.boldLabel);

    // Drop height
    var newDropHeight = EditorGUILayout.FloatField(
        s_Styles.m_AgentDropContent, m_LedgeDropHeight.floatValue);
    if (newDropHeight >= 0.0f &&
        !Mathf.Approximately(newDropHeight - m_LedgeDropHeight.floatValue,
                             0.0f))
      m_LedgeDropHeight.floatValue = newDropHeight;

    // Jump distance
    var newJumpDistance = EditorGUILayout.FloatField(
        s_Styles.m_AgentJumpContent, m_MaxJumpAcrossDistance.floatValue);
    if (newJumpDistance >= 0.0f &&
        !Mathf.Approximately(
            newJumpDistance - m_MaxJumpAcrossDistance.floatValue, 0.0f))
      m_MaxJumpAcrossDistance.floatValue = newJumpDistance;

    EditorGUILayout.Space();

    // Advanced Settings
    m_Advanced = GUILayout.Toggle(m_Advanced, s_Styles.m_AdvancedHeader,
    ...
    if (m_Advanced) {
      ...
      // Cell size
      var manualCellSize = EditorGUILayout.Toggle(
          s_Styles.m_ManualCellSizeContent, m_ManualCellSize.boolValue);
      ...
      using(new EditorGUI.DisabledScope(!m_ManualCellSize.boolValue)) {
        var cellSize = EditorGUILayout.FloatField(s_Styles.m_CellSizeContent,
                                                  m_CellSize.floatValue);
        if (cellSize > 0.0f &&
            !Mathf.Approximately(cellSize - m_CellSize.floatValue, 0.0f)) {
          m_CellSize.floatValue = Math.Max(0.01f, cellSize);
        }
        if (cellSize < 0.01f) {
          EditorGUILayout.HelpBox("The voxel size should be larger than 0.01.",
                                  MessageType.Warning);
        }

        float voxelsPerRadius =
            m_CellSize.floatValue > 0
                ? (m_AgentRadius.floatValue / m_CellSize.floatValue)
                : 0.0f;
        EditorGUILayout.LabelField(
            " ",
            voxelsPerRadius.ToString(
                "0.00", CultureInfo.InvariantCulture.NumberFormat) +
                " voxels per agent radius",
            EditorStyles.miniLabel);

        if (m_ManualCellSize.boolValue) {
          // Make sure these calculations match the ones in NavMeshBuilder.cpp
          // ConfigureConfig()
          const float kCellSizeToHeightRatio = 0.5f;
          float cellheight = m_CellSize.floatValue * kCellSizeToHeightRatio;
          // Some places inside Recast store height as a byte, make sure the
          // ratio between the agent height and cell height does not exceed this
          // limit.
          if ((int) Mathf.Floor(m_AgentHeight.floatValue / cellheight) > 250) {
            float goodValue =
                (m_AgentHeight.floatValue / 250.0f) / kCellSizeToHeightRatio;
            EditorGUILayout.HelpBox(
                "The number of voxels per agent height is too high. This will reduce the accuracy of the navmesh. Consider using voxel size of at least " +
                    goodValue.ToString(
          ...
          if (voxelsPerRadius < 1.0f) {
            ...
            EditorGUILayout.HelpBox(
                "The number of voxels per agent radius is too small. The agent may not avoid walls and ledges properly. Consider using a voxel size less than " +
                    goodValue.ToString(
                        "0.000", CultureInfo.InvariantCulture.NumberFormat) +
                    " (2 voxels per agent radius).",
                MessageType.Warning);
          } else if (voxelsPerRadius > 8.0f) {
            float goodValue = m_AgentRadius.floatValue / 8.0f;
            EditorGUILayout.HelpBox(
                "The number of voxels per agent radius is too high. It can cause excessive build times. Consider using voxel size closer to " +
                    goodValue.ToString(
                        "0.000", CultureInfo.InvariantCulture.NumberFormat) +
                    " (8 voxels per radius).",
                MessageType.Warning);
      ...
      // Min region area
      var minRegionArea = EditorGUILayout.FloatField(
          s_Styles.m_MinRegionAreaContent, m_MinRegionArea.floatValue);
      ...
      // Height mesh
      var accurate = EditorGUILayout.Toggle(s_Styles.m_AgentPlacementContent,
                                            m_AccuratePlacement.boolValue);
  ...
  private void AgentSettings() {
    ...
    if (m_AgentsList.index >= 0 && m_AgentsList.index < m_Agents.arraySize) {
      SerializedProperty nameProp =
          m_SettingNames.GetArrayElementAtIndex(m_AgentsList.index);
      SerializedProperty selectedAgent =
          m_Agents.GetArrayElementAtIndex(m_AgentsList.index);

      SerializedProperty radiusProp =
          selectedAgent.FindPropertyRelative("agentRadius");
      SerializedProperty heightProp =
          selectedAgent.FindPropertyRelative("agentHeight");
      SerializedProperty stepHeightProp =
          selectedAgent.FindPropertyRelative("agentClimb");
      SerializedProperty maxSlopeProp =
          selectedAgent.FindPropertyRelative("agentSlope");

      const float kDiagramHeight = 120.0f;
      Rect agentDiagramRect =
          EditorGUILayout.GetControlRect(false, kDiagramHeight);
      NavMeshEditorHelpers.DrawAgentDiagram(
          agentDiagramRect, radiusProp.floatValue, heightProp.floatValue,
          stepHeightProp.floatValue, maxSlopeProp.floatValue);

      EditorGUILayout.PropertyField(nameProp,
                                    EditorGUIUtility.TempContent("Name"));
      EditorGUILayout.PropertyField(radiusProp,
                                    EditorGUIUtility.TempContent("Radius"));
      EditorGUILayout.PropertyField(heightProp,
                                    EditorGUIUtility.TempContent("Height"));
      EditorGUILayout.PropertyField(
          stepHeightProp, EditorGUIUtility.TempContent("Step Height"));

      const float kMaxSlopeAngle = 60.0f;
      EditorGUILayout.Slider(maxSlopeProp, 0.0f, kMaxSlopeAngle,
                             EditorGUIUtility.TempContent("Max Slope"));
    }
