...
public static class SceneModeUtility {
  ...
  public static T[] GetSelectedObjectsOfType<T>(out GameObject[] gameObjects,
                                                params Type[] types) where T
      : Object {
    if (types.Length == 0)
  ...
  public static void SearchForType(Type type) {
    Object[] wins =
        Resources.FindObjectsOfTypeAll(typeof(SceneHierarchyWindow));
    SceneHierarchyWindow win =
        wins.Length > 0 ? (wins[0] as SceneHierarchyWindow) : null;
  ...
  public static Type SearchBar(params Type[] types) {
    ...
    if (s_FocusType != null &&
        (s_HierarchyWindow == null ||
         s_HierarchyWindow.m_SearchFilter != "t:" + s_FocusType.Name))
      s_FocusType = null;
...

    {
      GUIContent label = EditorGUIUtility.TempContent(
          "All", AssetPreview.GetMiniTypeThumbnail(typeof(GameObject)));
  ...
  public static bool StaticFlagField(string label, SerializedProperty property,
                                     int flag) {
    bool oldToggle = (property.intValue & flag) != 0;
    bool oldDifferent =
        (property.hasMultipleDifferentValuesBitwise & flag) != 0;
    EditorGUI.showMixedValue = oldDifferent;
    EditorGUI.BeginChangeCheck();
    var toggle = EditorGUILayout.Toggle(label, oldToggle);
    if (EditorGUI.EndChangeCheck()) {
      if (!SetStaticFlags(property.serializedObject.targetObjects, flag,
                          toggle))
  ...
  public static bool SetStaticFlags(Object[] targetObjects, int changedFlags,
                                    bool flagValue) {
    ...
    StaticEditorFlags flag = allFlagsAreChanged ?(StaticEditorFlags) 0
        : (StaticEditorFlags)
              Enum.Parse(typeof(StaticEditorFlags), msgChangedFlags.ToString());
    ...
    GameObjectUtility.ShouldIncludeChildren includeChildren =
        GameObjectUtility.DisplayUpdateChildrenDialogIfNeeded(
            targetObjects.OfType<GameObject>(), "Change Static Flags",
            allFlagsAreChanged ? "Do you want to " +
                (flagValue ? "enable"
                 : "disable") +
                " the static flags for all the child objects as well?"
            : "Do you want to " +
                  (flagValue ? "enable"
                   : "disable") +
                  " the " + ObjectNames.NicifyVariableName(flag.ToString()) +
    ...
    var objects =
        GetObjects(targetObjects,
                   includeChildren ==
                       GameObjectUtility.ShouldIncludeChildren.IncludeChildren);
    Undo.RecordObjects(objects, "Change Static Flags");

    // Calculate new flags value separately for each object so other flags are
    // not affected.
  ...
  public static GameObject[] GetObjects(Object[] gameObjects,
