...
internal class OcclusionCullingWindow : EditorWindow {
  ...
  class Styles {
    ...
    public GUIContent emptyAreaSelection = EditorGUIUtility.TrTextContent(
        "Select a Mesh Renderer or an Occlusion Area from the scene.");
    public GUIContent emptyCameraSelection =
        EditorGUIUtility.TrTextContent("Select a Camera from the scene.");
    public GUIContent visualizationNote = EditorGUIUtility.TrTextContent(
        "The visualization may not correspond to current bake settings and Occlusion Area placements if they have been changed since last bake.");
    public GUIContent seeVisualizationInScene = EditorGUIUtility.TrTextContent(
        "See the occlusion culling visualization in the Scene View based on the selected Camera.");
    public GUIContent noOcclusionData =
        EditorGUIUtility.TrTextContent("No occlusion data has been baked.");
    public GUIContent smallestHole = EditorGUIUtility.TrTextContent(
        "Smallest Hole",
        "Smallest hole in the geometry through which the camera is supposed to see. The single float value of the parameter represents the diameter of the imaginary smallest hole, i.e. the maximum extent of a 3D object that fits through the hole.");
    public GUIContent backfaceThreshold = EditorGUIUtility.TrTextContent(
        "Backface Threshold",
    ...
    public GUIContent farClipPlane = EditorGUIUtility.TrTextContent(
        "Far Clip Plane",
        "Far Clip Plane used during baking. This should match the largest far clip plane used by any camera in the scene. A value of 0.0 sets the far plane to Infinity.");
    public GUIContent smallestOccluder = EditorGUIUtility.TrTextContent(
        "Smallest Occluder",
    ...
    public GUIContent defaultParameterText = EditorGUIUtility.TrTextContent(
        "Default Parameters",
  ...
  enum Mode { AreaSettings = 0, BakeSettings = 1, Visualization = 2 }
  ...
  void OnEnable() {
    ...
    m_OverlayWindow = new OverlayWindow(
        EditorGUIUtility.TrTextContent("Occlusion Culling"), DisplayControls,
        (int) SceneViewOverlay.Ordering.OcclusionCulling, null,
  ...
  void SummaryGUI() {
    ...
    if (StaticOcclusionCulling.umbraDataSize == 0) {
      ...
      GUILayout.BeginVertical();
      GUILayout.Label(
          EditorUtility.FormatBytes(StaticOcclusionCulling.umbraDataSize),
          s_Styles.labelStyle);
      GUILayout.EndVertical();
  ...
  void AreaSelectionGUI() {
    ...
    GameObject[] gos;
    Type focusType =
        SceneModeUtility.SearchBar(typeof(Renderer), typeof(OcclusionArea));
    EditorGUILayout.Space();

    // Occlusion Areas
    OcclusionArea[] oas =
        SceneModeUtility.GetSelectedObjectsOfType<OcclusionArea>(out gos);
    if (gos.Length > 0) {
    ...
    // Renderers
    Renderer[] renderers = SceneModeUtility.GetSelectedObjectsOfType<Renderer>(
        out gos, typeof(MeshRenderer), typeof(SkinnedMeshRenderer));
    if (gos.Length > 0) {
      emptySelection = false;
      ...
      using(var goso = new SerializedObject(gos)) {
        SceneModeUtility.StaticFlagField(
            "Occluder Static", goso.FindProperty("m_StaticEditorFlags"),
            (int) StaticEditorFlags.OccluderStatic);
        SceneModeUtility.StaticFlagField(
            "Occludee Static", goso.FindProperty("m_StaticEditorFlags"),
    ...
    if (emptySelection) {
      ...
      if (focusType == typeof(OcclusionArea)) {
        ...
        if (GUILayout.Button("Occlusion Area", EditorStyles.miniButton,
  ...
  void CameraSelectionGUI() {
    ...
    if (cam) {
      ...
      EditorGUILayout.HelpBox(s_Styles.seeVisualizationInScene.text,
                              MessageType.Info);
    } else {
  ...
  void BakeSettings() {
    ...
    if (GUILayout.Button("Set default parameters",
      ...
      Undo.RegisterCompleteObjectUndo(
          StaticOcclusionCulling.occlusionCullingSettings,
          "Set Default Parameters");
      GUIUtility.keyboardControl =
          0; // Force focus out from potentially selected field for default
             // parameters setting
      StaticOcclusionCulling.SetDefaultOcclusionBakeSettings();
    }

    // Label for default parameter setting
    GUILayout.Label(s_Styles.defaultParameterText.tooltip,
                    EditorStyles.helpBox);

    // Edit Smallest Occluder
    EditorGUI.BeginChangeCheck();
    float smallestOccluder = EditorGUILayout.FloatField(
        s_Styles.smallestOccluder, StaticOcclusionCulling.smallestOccluder);
    if (EditorGUI.EndChangeCheck()) {
      Undo.RegisterCompleteObjectUndo(
          StaticOcclusionCulling.occlusionCullingSettings,
          "Change Smallest Occluder");
    ...
    float smallestHole = EditorGUILayout.FloatField(
        s_Styles.smallestHole, StaticOcclusionCulling.smallestHole);
    if (EditorGUI.EndChangeCheck()) {
      Undo.RegisterCompleteObjectUndo(
          StaticOcclusionCulling.occlusionCullingSettings,
          "Change Smallest Hole");
    ...
    float backfaceThreshold = EditorGUILayout.Slider(
        s_Styles.backfaceThreshold, StaticOcclusionCulling.backfaceThreshold,
        5.0F, 100.0F);
    if (EditorGUI.EndChangeCheck()) {
      Undo.RegisterCompleteObjectUndo(
          StaticOcclusionCulling.occlusionCullingSettings,
          "Change Backface Threshold");
  ...
  private bool CustomDropdownButton(string name, float width,
                                    params GUILayoutOption[] options) {
    var content = EditorGUIUtility.TrTextContent(name);
    var rect =
        GUILayoutUtility.GetRect(content, EditorStyles.dropDownList, options);
    ...
    if (Event.current.type == EventType.MouseDown &&
      ...
      for (int i = 0; i != names.Length; i++)
        menu.AddItem(new GUIContent(names[i]), values[i],
                     CustomDropdownCallback, i);
  ...
  void ModeToggle() {
    ...
    using(var check = new EditorGUI.ChangeCheckScope()) {
      m_Mode = (Mode) GUILayout.Toolbar((int) m_Mode, s_Styles.ModeToggles,
                                        "LargeButton",
  ...
  bool ShowModePopup(Rect popupRect) {
    ...
    // We can only change the preVis state during layout mode. However, the Tome
    // data could be emptied at anytime, which will immediately disable preVis.
    ...
    if (EditorGUI.DropdownButton(popupRect, new GUIContent(options[selected]),
                                 FocusType.Passive, EditorStyles.popup)) {
      GenericMenu menu = new GenericMenu();
      menu.AddItem(new GUIContent(options[0]), selected == 0,
                   SetShowVolumePreVis);
      if (tomeSize > 0)
        menu.AddItem(new GUIContent(options[1]), selected == 1,
                     SetShowVolumeCulling);
      else
  ...
  void DisplayControls(Object target, SceneView sceneView) {
    ...
    // If we don't have any Tome data, act as if pvs is true, but don't actually
    // set it to true
    // - that way it will act like it switches to false (the default value) by
    // itself as soon as pvs data has been build, which better leads to
    // discovery of this feature.

    bool preVis = ShowModePopup(
        GUILayoutUtility.GetRect(170, EditorGUIUtility.singleLineHeight));
    ...
    if (Event.current.type == EventType.Layout) {
      ...
      if (!preVis) {
        ...
        else if (!StaticOcclusionCullingVisualization
    ...
    if (!string.IsNullOrEmpty(m_Warning)) {
      ...
      Rect viewLegendRect = new Rect(legendRect.x, legendRect.y,
                                     legendRect.width, legendRect.height);

      if (preVis)
        EditorGUI.DrawLegend(
            viewLegendRect, Color.white, "View Volumes",
            StaticOcclusionCullingVisualization.showViewVolumes);
      else
        EditorGUI.DrawLegend(
            viewLegendRect, Color.white, "Camera Volumes",
            StaticOcclusionCullingVisualization.showViewVolumes);

      temp = GUI.Toggle(viewLegendRect,
                        StaticOcclusionCullingVisualization.showViewVolumes, "",
                        GUIStyle.none);
      ...
      if (!preVis) {
        ...
        legendRect.x += EditorGUI.indent;
        legendRect.width -= EditorGUI.indent;
        viewLegendRect = new Rect(legendRect.x, legendRect.y, legendRect.width,
                                  legendRect.height);
        EditorGUI.DrawLegend(
            viewLegendRect, Color.green, "Visibility Lines",
            StaticOcclusionCullingVisualization.showVisibilityLines);

        temp =
            GUI.Toggle(viewLegendRect,
                       StaticOcclusionCullingVisualization.showVisibilityLines,
                       "", GUIStyle.none);
        ...
        viewLegendRect = new Rect(legendRect.x, legendRect.y, legendRect.width,
                                  legendRect.height);
        EditorGUI.DrawLegend(viewLegendRect, Color.grey, "Portals",
