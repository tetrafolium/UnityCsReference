...
internal class LightmapPreviewWindow : EditorWindow {
  // UI selections etc
  [SerializeField] ZoomableArea m_ZoomablePreview = null;
  [SerializeField] bool m_ShowUVOverlay = true;
  [SerializeField] int m_SelectedPreviewTextureOptionIndex = 0;
  [SerializeField] float m_ExposureSliderValue = 0.0f;

  float m_ExposureSliderMax = 16f; // this value can be altered by the user

  // Lightmap specifiers
  [SerializeField] int m_LightmapIndex = -1;
  [SerializeField] int m_InstanceID = -1;
  [SerializeField] bool m_IsRealtimeLightmap = false;

  Hash128 m_RealtimeTextureHash = new Hash128();
  VisualisationGITexture m_CachedTexture;
  GameObject[] m_CachedTextureObjects;

  int m_ActiveGameObjectLightmapIndex =
      -1; // the object the user selects in the scene
  int m_ActiveGameObjectInstanceId =
      -1; // for instance based non-atlas textures such as baked emissive for
          // Progressive
  Hash128 m_ActiveGameObjectTextureHash =
      new Hash128(); // the object the user selects in the scene

  GITextureType[] kRealtimePreviewTextureTypes = {
      GITextureType.Irradiance, GITextureType.Directionality,
      GITextureType.Albedo, GITextureType.Emissive, GITextureType.Charting};
  ...
  static class Styles {
    ...
    public static readonly GUIStyle PreviewLabel =
        new GUIStyle(EditorStyles.whiteLabel);

    public static readonly GUIContent TextureNotAvailableRealtime =
        EditorGUIUtility.TrTextContent(
            "The texture is not available at the moment.");
    public static readonly GUIContent TextureNotAvailableBaked =
        EditorGUIUtility.TrTextContent(
    ...
    public static readonly GUIContent TextureNotAvailableBakedShadowmask =
        EditorGUIUtility.TrTextContent(
            "The texture is not available at the moment.\nPlease make sure that Mixed Lights affect this GameObject and that it is set to 'Contribute Global Illumination'.");
    public static readonly GUIContent TextureNotAvailableBakedAlbedoEmissive =
        EditorGUIUtility.TrTextContent(
            "The texture is not an index-based texture and is not available when using Progressive.\nPlease go to the instance you wish to debug, and select the lightmap on the Mesh Renderer.");
    public static readonly GUIContent TextureLoading =
        EditorGUIUtility.TrTextContent("Loading...");
    public static readonly GUIContent UVOverlayIcon = EditorGUIUtility.TrIconContent(
        "ToggleUVOverlay",
        "Toggles the UV Overlay for all the objects in the lightmap. The currently selected object will be highlighted. ");
  }
  ...
  private string lightmapTitle {
    get {
      if (isIndexBased)
        return ((isRealtimeLightmap ? "Realtime Lightmap Index "
                 : "Lightmap Index ") +
                m_LightmapIndex);
      ...
      if (obj)
        return (isRealtimeLightmap ? "Realtime"
                : "") +
               " Lightmap for '" + obj.name + "'";
  ...
  private float exposure {
    get {
      return SelectedTextureTypeNeedsExposureControl() ? m_ExposureSliderValue
                                                       : 0.0f;
    }
  }

  public static void CreateLightmapPreviewWindow(int lightmapId,
                                                 bool realtimeLightmap,
                                                 bool indexBased) {
    LightmapPreviewWindow window =
        EditorWindow.CreateInstance<LightmapPreviewWindow>();
  ...
  void OnDisable() { Lightmapping.lightingDataUpdated -= Repaint; }
  ...
  void OnGUI() {
    EditorGUILayout.BeginHorizontal(
        ...
        GUILayout.Height(EditorGUI.kWindowToolbarHeight));
    ...
    {
      var previewRect = new Rect(0, EditorGUI.kWindowToolbarHeight,
                                 position.width, position.height);

      GUI.BeginGroup(previewRect);
      DrawPreview(
          new Rect(0, -1, position.width,
                   position.height - EditorGUI.kWindowToolbarHeight - 1));
      GUI.EndGroup();
    }
  ...
  private void DrawPreviewSettings() {
    using(new EditorGUI.DisabledScope(
        !SelectedTextureTypeNeedsExposureControl())) {
    ...
    m_ShowUVOverlay = GUILayout.Toggle(m_ShowUVOverlay, Styles.UVOverlayIcon,
                                       EditorStyles.toolbarButton);

    Rect dropRect =
        GUILayoutUtility.GetRect(14, 160, EditorGUI.kWindowToolbarHeight,
                                 EditorGUI.kWindowToolbarHeight);
    GUIContent[] options =
        isRealtimeLightmap ? Styles.RealtimePreviewTextureOptions
        : Styles.BakedPreviewTextureOptions;
    GITextureType[] types = isRealtimeLightmap ? kRealtimePreviewTextureTypes
        : kBakedPreviewTextureTypes;

    if ((m_SelectedPreviewTextureOptionIndex < 0 ||
         m_SelectedPreviewTextureOptionIndex >= options.Length) ||
        !LightmapVisualizationUtility.IsTextureTypeEnabled(
            types[m_SelectedPreviewTextureOptionIndex])) {
      m_SelectedPreviewTextureOptionIndex = 0;
    }

    if (EditorGUI.DropdownButton(
            dropRect, options[m_SelectedPreviewTextureOptionIndex],
            FocusType.Passive, EditorStyles.toolbarDropDownRight)) {
      GenericMenu menu = new GenericMenu();

      for (int i = 0; i < options.Length; i++) {
        if (LightmapVisualizationUtility.IsTextureTypeEnabled(types[i]))
          menu.AddItem(options[i], m_SelectedPreviewTextureOptionIndex == i,
                       SelectPreviewTextureIndex, options.ElementAt(i));
        else
  ...
  private void DrawPreview(Rect r) {
    ...
    if (m_CachedTexture.textureAvailability ==
            GITextureAvailability.GITextureNotAvailable ||
        m_CachedTexture.textureAvailability ==
            GITextureAvailability.GITextureUnknown) {
      if (!isRealtimeLightmap) {
        if (!LightmapVisualizationUtility.IsAtlasTextureType(textureType) &&
            isIndexBased)
          GUI.Label(drawableArea, Styles.TextureNotAvailableBakedAlbedoEmissive,
                    Styles.PreviewLabel);
    ...
    if (m_CachedTexture.textureAvailability ==
            GITextureAvailability.GITextureLoading &&
        m_CachedTexture.texture == null) {
      GUI.Label(drawableArea, Styles.TextureLoading, Styles.PreviewLabel);
      return;
    }

    LightmapType lightmapType =
        LightmapVisualizationUtility.GetLightmapType(textureType);
    ...
    case EventType.ExecuteCommand:
      ...
      if (Event.current.commandName == EventCommandNames.FrameSelected &&
          IsActiveGameObjectInLightmap(textureType)) {
        // There are instance based baked textures where we don't get any STs
        // and can't do the framing
        if (!isRealtimeLightmap &&
            !LightmapVisualizationUtility.IsAtlasTextureType(textureType))
          break;

        Vector4 lightmapTilingOffset =
            LightmapVisualizationUtility.GetLightmapTilingOffset(lightmapType);

        Vector2 min =
            new Vector2(lightmapTilingOffset.z, lightmapTilingOffset.w);
        Vector2 max =
            min + new Vector2(lightmapTilingOffset.x, lightmapTilingOffset.y);
        ...
        if (textureRect.height == drawableArea.height)
          offsetX =
              (drawableArea.width - textureRect.width) / drawableArea.width;
        else
          offsetY =
              (drawableArea.height - textureRect.height) / drawableArea.height;
    ...
    case EventType.Repaint: {
      ...
      if (texture) {
        ...
        // Texture shouldn't be filtered since it will make previewing really
        // blurry
        FilterMode prevMode = texture.filterMode;
        texture.filterMode = FilterMode.Point;

        LightmapVisualizationUtility.DrawTextureWithUVOverlay(
            texture,
            (m_ShowUVOverlay && IsActiveGameObjectInLightmap(textureType))
                ? Selection.activeGameObject
                : null,
            m_ShowUVOverlay ? m_CachedTextureObjects
            : new GameObject[]{}, drawableArea, textureRect, textureType,
              exposure);
  ...
  private bool SelectedTextureTypeNeedsExposureControl() {
    ...
    // it only make sense to allow the user to adjust the exposure on these
    // textures
    return textureType == GITextureType.BakedEmissive ||
           textureType == GITextureType.Baked ||
           textureType == GITextureType.Emissive ||
           textureType == GITextureType.Irradiance;
  }
  ...
  private void UpdateActiveGameObjectSelection() {
    ...
    // if the selected active object (also active in the hierarchy) is a
    // renderer or a terrain, we check its index etc. otherwise bail
    ...
    if (isRealtimeLightmap) {
      ...
      m_ActiveGameObjectLightmapIndex =
          renderer != null ? renderer.lightmapIndex : terrain.lightmapIndex;
  ...
  private void UpdateCachedTexture(GITextureType textureType) {
    if (isIndexBased) {
      if (isRealtimeLightmap) {
        ...
        if (!m_RealtimeTextureHash.isValid ||
            !mainHashes.Contains(m_RealtimeTextureHash)) {
          m_RealtimeTextureHash =
              mainHashes.ElementAtOrDefault(m_LightmapIndex);
        }
      }
    } else // instance based
    {
    ...
    Hash128 contentHash =
        isRealtimeLightmap ? LightmapVisualizationUtility
            .GetRealtimeGITextureHash(m_RealtimeTextureHash, textureType)
        : LightmapVisualizationUtility.GetBakedGITextureHash(
              m_LightmapIndex, m_InstanceID, textureType);

    // if we need to fetch a new texture
    if (m_CachedTexture.texture == null ||
        m_CachedTexture.type != textureType ||
        m_CachedTexture.contentHash != contentHash ||
        m_CachedTexture.contentHash == new Hash128()) {
      m_CachedTexture =
          isRealtimeLightmap ? LightmapVisualizationUtility
              .GetRealtimeGITexture(m_RealtimeTextureHash, textureType)
          : LightmapVisualizationUtility.GetBakedGITexture(
                m_LightmapIndex, m_InstanceID, textureType);
    }
    ...
    if (isRealtimeLightmap)
      m_CachedTextureObjects =
          LightmapVisualizationUtility.GetRealtimeGITextureRenderers(
              m_RealtimeTextureHash);
    else if (LightmapVisualizationUtility.IsAtlasTextureType(textureType))
      m_CachedTextureObjects =
          LightmapVisualizationUtility.GetBakedGITextureRenderers(
              m_LightmapIndex);
    else // if it's an instance based baked lightmap, we only have 1 object in
         // it
  ...
  private Rect ScaleRectByZoomableArea(Rect rect, ZoomableArea zoomableArea) {
    float x = -(zoomableArea.shownArea.x / zoomableArea.shownArea.width) *
              (rect.x + zoomableArea.rect.width);
    float y =
        ((zoomableArea.shownArea.y - (1f - zoomableArea.shownArea.height)) /
         zoomableArea.shownArea.height) *
        zoomableArea.rect.height;
