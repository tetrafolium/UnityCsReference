...
namespace UnityEditor.Collaboration {
  internal abstract class AbstractFilters {
    ...
    public void ShowInFavoriteSearchFilters() {
      ...
      foreach (var filter in filters) {
        int instanceID =
            SavedSearchFilters.GetFilterInstanceID(filter[0], filter[1]);
        if (instanceID == 0) {
          SearchFilter searchFilter =
              SearchFilter.CreateSearchFilterFromString(filter[1]);
          if (prevInstanceID == 0)
            prevInstanceID =
                SavedSearchFilters.AddSavedFilter(filter[0], searchFilter, 64);
          else
            prevInstanceID = SavedSearchFilters.AddSavedFilterAfterInstanceID(
                filter[0], searchFilter, 64, prevInstanceID, false);
        }
      }
  ...
  internal class CollabFilters : AbstractFilters {
    ...
    public CollabFilters() { InitializeFilters(); }
        ...
        if (browser.IsTwoColumns()) {
          foreach (var filter in filters) {
            if (filterSearchString == filter[1]) {
              int instanceID = SavedSearchFilters.GetFilterInstanceID(
                  filter[0], filterSearchString);
