...
namespace UnityEditor.Collaboration {
  ...
  internal static class SoftLockUIData {
    private static Dictionary<string, Texture> s_ImageCache =
        new Dictionary<string, Texture>();
    private static Dictionary<SectionEnum, string> s_ImageNameCache =
        new Dictionary<SectionEnum, string>();
    private const string kIconMipSuffix = " Icon";

    public enum SectionEnum { None, Inspector, Scene, ProjectBrowser }
...
#region Scene
    ...
    // For each iteration, returns the pair (scene name : list of other users'
    // names).
    public static IEnumerable<KeyValuePair<string, List<string>>>
    GetLockNamesOnOpenScenes() {
      if (Collab.instance.IsCollabEnabledForCurrentProject()) {
        for (int sceneIndex = 0; sceneIndex < EditorSceneManager.sceneCount;
             sceneIndex++) {
          ...
          KeyValuePair<string, List<string>> sceneData =
              new KeyValuePair<string, List<string>>(sceneName, names);
    ...
    public static int CountOfLocksOnOpenScenes() {
      ...
      foreach (KeyValuePair<string, List<string>> sceneData in
                   GetLockNamesOnOpenScenes()) {
...
#region Game Object
    ...
    GetLockNamesOnObject(UnityEngine.Object objectWithGUID) {
