...
private void UnregisterDrawDelegates() {
	ObjectListArea.postAssetIconDrawCallback -=
		Instance.DrawProjectBrowserGridUI;
	ObjectListArea.postAssetLabelDrawCallback -=
		Instance.DrawProjectBrowserListUI;
...
private void RegisterDrawDelegates() {
	...
	ObjectListArea.postAssetLabelDrawCallback +=
		Instance.DrawProjectBrowserListUI;
	AssetsTreeViewGUI.postAssetLabelDrawCallback +=
		Instance.DrawSingleColumnProjectBrowserUI;
	Editor.OnPostIconGUI += Instance.DrawInspectorUI;
	GameObjectTreeViewGUI.OnPostHeaderGUI += Instance.DrawSceneUI;
}
...
private bool HasSoftlockSupport(Editor editor) {
	if (!Collab.instance.IsCollabEnabledForCurrentProject() || editor == null ||
	...
	// Support Scene and Game object Inspector headers, not others like
	// MaterialEditor.
	bool hasSupport = true;
	Type editorType = editor.GetType();

	if (editorType != typeof(GameObjectInspector) &&editorType !=
	    typeof(GenericInspector)) {
		hasSupport = false;
	}
...
private bool HasSoftlocks(string assetGUID) {
	...
	bool hasSoftLocks;
	bool isValid = (SoftLockData.TryHasSoftLocks(assetGUID, out hasSoftLocks) &&
	                hasSoftLocks);
	return isValid;
}
...
private void RepaintSceneHierarchy() {
	List<SceneHierarchyWindow> sceneUIs =
...
public float DrawSceneUI(Rect availableRect, string scenePath) {
	...
	content.image =
		SoftLockUIData.GetIconForSection(SoftLockUIData.SectionEnum.Scene);
...
// Draws the Scene Inspector (Editor.cs) as well as the Game Object Inspector
...
private void DrawInspectorUI(Editor editor, Rect drawRect) {
	...
	Texture icon = SoftLockUIData.GetIconForSection(
		SoftLockUIData.SectionEnum.ProjectBrowser);
	if (icon != null) {
...
private void DrawProjectBrowserGridUI(Rect iconRect, string assetGUID,
                                      bool isListMode) {
	...
	Texture icon = SoftLockUIData.GetIconForSection(
		SoftLockUIData.SectionEnum.ProjectBrowser);
	if (icon != null) {
		drawRect = Overlay.GetRectForBottomRight(
			iconRect, Overlay.k_OverlaySizeOnLargeIcon);
		DrawIconWithTooltips(drawRect, icon, assetGUID);
	}
}

// Should draw only in listMode and expects 'drawRect' to be the designed
// space for the icon, and not the entire row.
private bool DrawProjectBrowserListUI(Rect drawRect, string assetGUID,
                                      bool isListMode) {
	if (!isListMode || !HasSoftlocks(assetGUID)) {
		return false;
	}
...
private bool DrawSingleColumnProjectBrowserUI(Rect drawRect,
                                              string assetGUID) {
	if (ProjectBrowser.s_LastInteractedProjectBrowser.IsTwoColumns() ||
	    !HasSoftlocks(assetGUID)) {
		return false;
	}
	...
	iconRect.x =
		(float) Math.Round(drawRect.xMax - iconRect.width - spacingFromEnd);
	return DrawInProjectBrowserListMode(iconRect, assetGUID);
}
...
private void DrawIconWithTooltips(Rect iconRect, Texture icon,
                                  string assetGUID) {
...
private string GetTooltip(string assetGUID) {
	...
	if (!m_Cache.TryGetTooltipForGUID(assetGUID, out formattedText)) {
		List<string> softLockNames =
			SoftLockUIData.GetLocksNamesOnAsset(assetGUID);
		string tooltipHeaderText =
			(SoftLockData.IsPrefab(assetGUID) ? k_TooltipPrefabHeader
			                    : k_TooltipHeader);
...
private string FitTextToWidth(string text, float width, GUIStyle style) {
	int characterCountVisible =
		style.GetNumCharactersThatFitWithinWidth(text, width);
	if (characterCountVisible > 1 && characterCountVisible != text.Length) {
		string ellipsedText;
		int characterLength = (characterCountVisible - 1);
		if (!Instance.m_Cache.TryGetEllipsedNames(text, characterLength,
		                                          out ellipsedText)) {
			ellipsedText = text.Substring(0, characterLength) +
			               (" \u2026"); // 'horizontal ellipsis' (U+2026) is: ...
			Instance.m_Cache.StoreEllipsedNames(text, ellipsedText,
			                                    characterLength);
		}
		return ellipsedText;
	}
...
private List<WeakReference> m_CachedWeakReferences =
	new List<WeakReference>();
private static Dictionary<int, string> s_CachedStringCount =
	new Dictionary<int, string>();
private Dictionary<string, string> m_AssetGUIDToTooltip =
	new Dictionary<string, string>();
private Dictionary<string, Dictionary<int, string> >
m_NamesListToEllipsedNames =
...
public bool TryGetEllipsedNames(string allNames, int characterLength,
                                out string ellipsedNames) {
	Dictionary<int, string> ellipsedVersions;
	if (m_NamesListToEllipsedNames.TryGetValue(allNames,
	                                           out ellipsedVersions)) {
...
// 'allNames' and 'characterLength' will be the keys to access the cached
// 'ellipsedNames' see TryGetEllipsedNames() for retrieval.
public void StoreEllipsedNames(string allNames, string ellipsedNames,
                               int characterLength) {
	Dictionary<int, string> ellipsedVersions;
	if (!m_NamesListToEllipsedNames.TryGetValue(allNames,
	                                            out ellipsedVersions)) {
...
public void StoreEditor(Editor editor) {
	...
	for (int index = 0; canAdd && (index < m_EditorReferences.Count);
	     index++) {
		WeakReference reference = m_EditorReferences[index];
