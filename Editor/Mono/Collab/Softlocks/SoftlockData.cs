...
public static bool TryHasSoftLocks(UnityEngine.Object objectWithGUID,
                                   out bool hasSoftLocks) {
...
public static bool TryHasSoftLocks(string assetGuid,
                                   out bool hasSoftLocks) {
	hasSoftLocks = false;
	bool success = false;
	int count = 0;
...
// Failure: assigns 0 to count, return false.
// Success: assigns a value in [0, n] to count, returns true.
public static bool TryGetSoftlockCount(UnityEngine.Object objectWithGUID,
                                       out int count) {
	string assetGUID = null;
	AssetAccess.TryGetAssetGUIDFromObject(objectWithGUID, out assetGUID);
...
// Success: assigns the retrieved list to 'softLocks', return true. (May be
// empty).
public static bool TryGetLocksOnAssetGUID(string assetGuid,
                                          out List<SoftLock> softLocks) {
	if (assetGuid == null) {
