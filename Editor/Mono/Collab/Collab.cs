...
namespace UnityEditor.Collaboration {
  ...
  internal delegate void RevisionChangedDelegate(CollabInfo info, string rev,
                                                 string action);
  ...
  internal delegate void ChangesChangedDelegate(Change[] changes,
                                                bool isFiltered);
...
#pragma warning disable 414 // The private field is assigned but its value is
                            // never used
    ...
    public enum CollabStates : uint {
      ...
      kAnyLocalChanged = (kCollabAddedLocal | kCollabCheckedOutLocal |
                          kCollabDeletedLocal | kCollabMovedLocal),
      kAnyLocalEdited =
          (kCollabAddedLocal | kCollabCheckedOutLocal | kCollabMovedLocal),
      kCollabAny = 0xFFFFFFFF
    }

    internal enum CollabStateID { None, Uninitialized, Initialized }

    public static string[] clientType = {"Cloud Server", "Mock Server"};

    internal static string editorPrefCollabClientType = "CollabConfig_Client";

    public static string GetProjectClientType() {
      var cvalue =
          EditorUserSettings.GetConfigValue(editorPrefCollabClientType);
    ...
    internal static CollabStates GetAssetState(string assetGuid,
                                               string assetPath) {
      if (s_VersionControlInstance != null) {
    ...
    static Collab() {
      ...
      ObjectListArea.postAssetIconDrawCallback +=
          CollabProjectHook.OnProjectWindowIconOverlay;
      AssetsTreeViewGUI.postAssetIconDrawCallback +=
          CollabProjectHook.OnProjectBrowserNavPanelIconOverlay;
      InitializeSoftlocksViewController();
      CollabSettingsManager
          .statusNotifier[CollabSettingType.InProgressEnabled] +=
          OnSettingStatusChanged;
      CollabSettingsManager
          .statusNotifier[CollabSettingType.InProgressEnabled] +=
          SoftlockViewController.Instance.softLockFilters
              .OnSettingStatusChanged;
    }

    public static void OnSettingStatusChanged(CollabSettingType type,
    ...
    public bool GetError(UnityConnect.UnityErrorFilter errorFilter,
    ...
    public CollabInfo GetCollabInfo() { return collabInfo; }
    ...
    public PublishInfo GetChangesToPublish() {
      ...
      return new PublishInfo(){changes = changes, filter = isFiltered};
    }

    public PublishInfo_V2 GetChangesToPublish_V2() {
      ChangeItem[] changes = GetChangeItemsToPublishInternal_V2();

      return new PublishInfo_V2(){changes = changes, filter = false};
    }
    ...
    [RequiredByNativeCode]
    private static void OnChangeItemsChanged(ChangeItem[] changes,
                                             bool isFiltered) {
    ...
    [RequiredByNativeCode]
    private static void OnSelectedChangeItemsChanged(ChangeItem[] changeItems,
                                                     bool isFiltered) {
    ...
    [RequiredByNativeCode]
    private static void SetCollabError(int code, int priority, int behavior,
                                       string msg, string shortmsg,
                                       string codeStr) {
      ...
      if (handler_v2 != null) {
        handler_v2(new UnityErrorInfo(){
            code = code, priority = priority, behaviour = behavior, msg = msg,
            shortMsg = shortmsg, codeStr = codeStr});
      }
    }
    ...
    public static void
    OnProgressEnabledSettingStatusChanged(CollabSettingType type,
                                          CollabSettingStatus status) {
      if (type == CollabSettingType.InProgressEnabled &&
        ...
        CollabSettingsManager
            .statusNotifier[CollabSettingType.InProgressEnabled] -=
            OnProgressEnabledSettingStatusChanged;
      }
    }
    ...
    static void OnCollabEnabledForCurrentProject(bool enabled) {
      if (enabled) {
        ...
        if (ProjectBrowser.s_LastInteractedProjectBrowser != null) {
          if (ProjectBrowser.s_LastInteractedProjectBrowser.Initialized() &&
              ProjectBrowser.s_LastInteractedProjectBrowser.IsTwoColumns()) {
            int instanceID = AssetDatabase.GetMainAssetInstanceID("assets");
            ProjectBrowser.s_LastInteractedProjectBrowser.SetFolderSelection(
                new int[]{instanceID}, true);
          }
