...
namespace UnityEditor.Collaboration {
  // Display hooks for the main project window.  Icons are overlayed to show the
  // version control state.
  internal class CollabProjectHook {
    // GUI callback for each item visible in the project window/object list area
    public static void OnProjectWindowIconOverlay(Rect iconRect, string guid,
                                                  bool isListMode) {
    ...
    public static void OnProjectBrowserNavPanelIconOverlay(Rect iconRect,
                                                           string guid) {
      DrawProjectBrowserIconOverlay(iconRect, guid, true);
    }

    private static void
    DrawProjectBrowserIconOverlay(Rect iconRect, string guid, bool isListMode) {
    ...
    public static Collab.CollabStates GetAssetState(String assetGuid) {
      ...
      Collab.CollabStates assetState = Collab.GetAssetState(
  ...
  internal class Overlay {
    ...
    private static readonly Dictionary<Collab.CollabStates, GUIContent>
        s_Overlays = new Dictionary<Collab.CollabStates, GUIContent>();

    protected static void LoadOverlays() {
      // Order of priority must match GetLocalStatus (CollabClient.h)
      s_Overlays.Clear();
      s_Overlays.Add(Collab.CollabStates.kCollabIgnored,
                     EditorGUIUtility.IconContent("CollabExclude Icon"));
      s_Overlays.Add(Collab.CollabStates.kCollabConflicted,
                     EditorGUIUtility.IconContent("CollabConflict Icon"));
      s_Overlays.Add(Collab.CollabStates.kCollabPendingMerge,
                     EditorGUIUtility.IconContent("CollabConflict Icon"));
      s_Overlays.Add(Collab.CollabStates.kCollabMovedLocal,
                     EditorGUIUtility.IconContent("CollabMoved Icon"));
      s_Overlays.Add(Collab.CollabStates.kCollabCheckedOutLocal |
                         Collab.CollabStates.kCollabMovedLocal,
                     EditorGUIUtility.IconContent("CollabMoved Icon"));
      s_Overlays.Add(Collab.CollabStates.kCollabCheckedOutLocal,
                     EditorGUIUtility.IconContent("CollabEdit Icon"));
      s_Overlays.Add(Collab.CollabStates.kCollabAddedLocal,
                     EditorGUIUtility.IconContent("CollabCreate Icon"));
      s_Overlays.Add(Collab.CollabStates.kCollabDeletedLocal,
                     EditorGUIUtility.IconContent("CollabDeleted Icon"));

      // The folder overlay should take precedence on the folder content's
      // status.
      s_Overlays.Add(
          Collab.CollabStates.KCollabContentConflicted,
          EditorGUIUtility.IconContent("CollabChangesConflict Icon"));
      s_Overlays.Add(Collab.CollabStates.KCollabContentChanged,
                     EditorGUIUtility.IconContent("CollabChanges Icon"));
      s_Overlays.Add(Collab.CollabStates.KCollabContentDeleted,
    ...
    protected static Collab.CollabStates
    GetOverlayStateForAsset(Collab.CollabStates assetStates) {
    ...
    protected static void DrawOverlayElement(Collab.CollabStates singleState,
                                             Rect itemRect) {
      GUIContent content;
    ...
    protected static bool HasState(Collab.CollabStates assetStates,
                                   Collab.CollabStates includesState) {
      return ((assetStates & includesState) == includesState);
    }

    public static void DrawOverlays(Collab.CollabStates assetState,
                                    Rect itemRect, bool isListMode) {
      ...
      DrawOverlayElement(
          state, GetRectForTopRight(itemRect, GetScale(itemRect, isListMode)));
    }

    // Return a new Rect with its width and height scaled, and converted to the
    // ceiling.
    ...
    public static Rect GetRectForTopRight(Rect projectBrowserDrawRect,
                                          double scale) {
    ...
    public static Rect GetRectForBottomRight(Rect projectBrowserDrawRect,
                                             double scale) {
      Rect scaledRect = ScaleRect(projectBrowserDrawRect, scale);
      scaledRect.x += (projectBrowserDrawRect.width - scaledRect.width);
  ...
  internal static class TextureUtility {
    public static Texture2D LoadTextureFromApplicationContents(string path) {
      ...
      string resourcesPath = Path.Combine(
          Path.Combine(Path.Combine(EditorApplication.applicationContentsPath,
                                    "Resources"),
                       "Collab"),
          "overlays");
