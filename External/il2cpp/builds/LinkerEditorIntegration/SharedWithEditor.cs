...
// linker or editor may not make use of all fields so suppress unused fields
// warn
...
public TypeInSceneData(string managedAssemblyName,
                       string fullManagedTypeName, string nativeClass,
                       string moduleName, string[] usedInScenes) {
...
public override string ToString() {
	var managedPart = string.IsNullOrEmpty(fullManagedTypeName)
	                    ? "None"
	                    : $"{managedAssemblyName}::{fullManagedTypeName}";
	var nativeName = string.IsNullOrEmpty(nativeClass)
	                   ? "None"
	                   : $"{moduleName}::{nativeClass}";
...
public enum DependencyType { ManagedType, NativeType, Module, Custom }
}
}
}
