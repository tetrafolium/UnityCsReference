...
namespace UnityEngine.UIElements {
  ...
  //
  // For example: In the case of Event 1 Button F getting clicked, the following
  // handlers will be called if registered: result ==> Phase TrickleDown [ root,
  // D, E ], Phase Target [F], Phase BubbleUp [ E, D, root ]
  //
  // For example 2: A keydown with Textfocus in TextField C
  // result ==> Phase TrickleDown [ root, A], Phase Target [C], Phase BubbleUp [
  // A, root ]
  ...
  /// </remarks>
  /// <remarks>
  /// When the gate is instantiated, it closes automatically, causing the
  /// dispatcher to store the events it receives in a queue. At the end of the
  /// <c>using</c> block, <see cref="Dispose"/> is called, which triggers
  /// opening the gate. When all gates on a dispatcher open, the events stored
  /// in the queue are processed. Closing a gate while the event queue is
  /// processed does not stop it from being processed. Instead, a new queue is
  /// created to store new events.
  /// </remarks>
  /// <example>
  /// Here is an example of using a gate:
  /// <code>
  ...
  /// </code>
  /// </example>
  public struct EventDispatcherGate : IDisposable,
                                      IEquatable<EventDispatcherGate> {
    ...
    /// <summary>
    /// Implementation of IDisposable.Dispose. Opens the gate. If all gates are
    /// open, events in the queue are processed.
    /// </summary>
    public void Dispose() { m_Dispatcher.OpenGate(); }
    ...
    public static bool operator ==(EventDispatcherGate left,
                                   EventDispatcherGate right) {
  ...
  public sealed class EventDispatcher {
    ...
    static readonly ObjectPool<Queue<EventRecord>> k_EventQueuePool =
        new ObjectPool<Queue<EventRecord>>();
    ...
    internal EventDispatcher() {
      ...
      m_DebuggerEventDispatchingStrategy =
          new DebuggerEventDispatchingStrategy();
    ...
    internal void Dispatch(EventBase evt, IPanel panel,
    ...
    internal void PushDispatcherContext() {
      // Drain the event queue before pushing a new context.  This allows some
      // important events (such as AttachToPanel events) to be processed before
      // showing a modal window. (Fixes case 1215148).
      ProcessEventQueue();

      m_DispatchContexts.Push(
          new DispatchContext(){m_GateCount = m_GateCount, m_Queue = m_Queue});
      m_GateCount = 0;
      m_Queue = k_EventQueuePool.Get();
    }
    ...
    internal void PopDispatcherContext() {
      Debug.Assert(
          m_GateCount == 0,
          "All gates should have been opened before popping dispatch context.");
      Debug.Assert(m_Queue.Count == 0,
                   "Queue should be empty when popping dispatch context.");
    ...
    internal void CloseGate() { m_GateCount++; }
    ...
    void ProcessEventQueue() {
      // While processing the current queue, we need a new queue to store
      // additional events that might be generated during current queue events
      // processing. Thanks to the gate mechanism, events put in the new queue
      // will be processed before the remaining events in the current queue (but
      // after processing of the event generating them is completed).
      //
      // For example, MouseDownEvent generates FocusOut, FocusIn, Blur and Focus
      // events. And let's say that FocusIn generates ValueChanged and
      // GeometryChanged events.
      //
      // Without queue swapping, order of event processing would be MouseDown,
      // FocusOut, FocusIn, Blur, Focus, ValueChanged, GeometryChanged. It is
      // not the same as order of event emission.
      //
      // With queue swapping, order is MouseDown, FocusOut, FocusIn,
      // ValueChanged, GeometryChanged, Blur, Focus. This preserve the order of
      // event emission, and each event is completely processed before
    ...
    void ProcessEvent(EventBase evt, IPanel panel) {
      Event e = evt.imguiEvent;
      // Sometimes (in tests only?) we receive Used events. Protect our
      // verification from this case.
      ...
      using(new EventDispatcherGate(this)) {
        ...
        Debug.Assert(imguiEventIsInitiallyUsed || evt.isPropagationStopped ||
                         e == null || e.rawType != EventType.Used,
                     "Event is used but not stopped.");
      }
    }

    void ApplyDispatchingStrategies(EventBase evt, IPanel panel,
                                    bool imguiEventIsInitiallyUsed) {
      foreach (var strategy in m_DispatchingStrategies) {
        if (strategy.CanDispatchEvent(evt)) {
          strategy.DispatchEvent(evt, panel);

          Debug.Assert(
              imguiEventIsInitiallyUsed || evt.isPropagationStopped ||
                  evt.imguiEvent == null ||
                  evt.imguiEvent.rawType != EventType.Used,
