...
public override void OnVersionChanged(VisualElement ve,
	...
	if ((versionChangeType & VersionChangeType.ViewData) !=
...
public override void Update() {
	...
	while (m_LastVersion != m_Version) {
		...
		if (validatePersistentDataCount > kMaxValidatePersistentDataCount) {
			Debug.LogError(
				"UIElements: Too many children recursively added that rely on persistent view data: " +
				visualTree);
			break;
		}
	}
...
private void ValidateViewDataOnSubTree(VisualElement ve,
                                       bool enablePersistence) {
	// Persistence of view data is almost always enabled as long as an element
	// has a valid viewDataKey. The only exception is when an element is in
	// its parent's shadow tree, that is, not a physical child of its logical
	...
	// its logical parent does not have a viewDataKey.
	enablePersistence =
		ve.IsViewDataPersitenceSupportedOnChildren(enablePersistence);
