...
static readonly VisualElementFocusChangeDirection s_Left =
	new VisualElementFocusChangeDirection(FocusChangeDirection.lastValue +
	                                      1);
...
static readonly VisualElementFocusChangeDirection s_Right =
	new VisualElementFocusChangeDirection(FocusChangeDirection.lastValue +
	                                      2);
...
/// <summary>
/// Implementation of a linear focus ring. Elements are sorted according to
/// their focusIndex.
/// </summary>
...
public enum DefaultFocusOrder {
	/// <summary>
	/// Order elements using a depth-first pre-order traversal of the element
	/// tree.
	/// </summary>
	ChildOrder,
	/// <summary>
...
/// Constructor.
/// </summary>
/// <param name="root">The root of the element tree for which we want to
/// build a focus ring.</param> <param name="dfo">Default ordering of the
/// elements in the ring.</param>
public VisualElementFocusRing(
	VisualElement root,
	DefaultFocusOrder dfo = DefaultFocusOrder.ChildOrder) {
...
int FocusRingSort(FocusRingRecord a, FocusRingRecord b) {
	if (a.m_Focusable.tabIndex == 0 && b.m_Focusable.tabIndex == 0) {
		...
		int result = Comparer<int>.Default.Compare(a.m_Focusable.tabIndex,
		                                           b.m_Focusable.tabIndex);
		// but if the focus index result is being equal, we need to fallback
		// with their automatic index
		if (result == 0) {
...
void BuildRingForScopeRecursive(VisualElement ve, ref int scopeIndex,
                                List<FocusRingRecord> scopeList) {
	var veChildCount = ve.hierarchy.childCount;
	for (int i = 0; i < veChildCount; i++) {
		var child = ve.hierarchy[i];

		bool isSlot =
			child.parent != null && child == child.parent.contentContainer;

		if (child.isCompositeRoot || isSlot) {
			var childRecord = new FocusRingRecord {
				m_AutoIndex = scopeIndex++, m_Focusable = child,
				m_IsSlot = isSlot,
			...
			int autoIndex = 0;
			BuildRingForScopeRecursive(child, ref autoIndex,
			                           childRecord.m_ScopeNavigationOrder);
		} else {
			if (child.canGrabFocus && child.tabIndex >= 0) {
				scopeList.Add(new FocusRingRecord {
							m_AutoIndex = scopeIndex++, m_Focusable = child,
							m_IsSlot = false, m_ScopeNavigationOrder = null
						});
			}
...
void SortAndFlattenScopeLists(List<FocusRingRecord> rootScopeList) {
	if (rootScopeList != null) {
		...
		foreach (var record in rootScopeList) {
			if (record.m_Focusable.canGrabFocus &&
			    record.m_Focusable.tabIndex >= 0) {
...
/// right in the focus ring.
/// </summary>
public FocusChangeDirection
GetFocusChangeDirection(Focusable currentFocusable, EventBase e) {
	if (e == null) {
		throw new ArgumentNullException(nameof(e));
	}

	// FUTURE:
	// We could implement an extendable adapter system to convert event to a
	// focus change direction. This would enable new event sources to change
	// the focus.
...
/// </summary>
public Focusable GetNextFocusable(Focusable currentFocusable,
	...
	if (direction == FocusChangeDirection.none ||
		...
		if (direction == VisualElementFocusChangeDirection.right) {
			...
			if (currentFocusable != null && index == 0) {
				// currentFocusable was not found in the ring. Use the element tree
			...
			// FIXME: Element could be unrelated to delegator; should we detect
			// this case and return null? Spec is not very clear on this.
			...
			if (currentFocusable != null && index == -2) {
				// currentFocusable was not found in the ring. Use the element tree
				// to find the previous focusable.
				return GetPreviousFocusableInTree(
					currentFocusable as VisualElement);
			}
...
internal static Focusable
GetNextFocusableInTree(VisualElement currentFocusable) {
...
internal static Focusable
GetPreviousFocusableInTree(VisualElement currentFocusable) {
	if (currentFocusable == null) {
		return null;
	}
