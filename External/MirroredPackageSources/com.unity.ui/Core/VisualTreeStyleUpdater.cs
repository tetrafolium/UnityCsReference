...
namespace UnityEngine.UIElements {
  internal static class StyleCache {
    ...
    // the same set of rules will give the same specified styles, caching the
    // hash of the matching rules before resolving styles allows to skip the
    // resolve part when an existing style data already exists
    private static Dictionary<Int64, ComputedStyle> s_ComputedStyleCache =
        new Dictionary<Int64, ComputedStyle>();
    private static Dictionary<int, StyleVariableContext>
        s_StyleVariableContextCache =
            new Dictionary<int, StyleVariableContext>();
  ...
  internal class VisualTreeStyleUpdater : BaseVisualTreeUpdater {
    ...
    private VisualTreeStyleUpdaterTraversal m_StyleContextHierarchyTraversal =
        new VisualTreeStyleUpdaterTraversal();

    private static readonly string s_Description = "Update Style";
    private static readonly ProfilerMarker s_ProfilerMarker =
        new ProfilerMarker(s_Description);
    public override ProfilerMarker profilerMarker => s_ProfilerMarker;

    public void DirtyStyleSheets() {
      // When a style sheet is re-imported, we must make sure to purge internal
      // caches that are depending on it
      StyleCache.ClearStyleCache();
      visualTree.IncrementVersion(
          VersionChangeType.StyleSheet); // dirty all styles
    }

    public override void OnVersionChanged(VisualElement ve,
                                          VersionChangeType versionChangeType) {
      if ((versionChangeType & VersionChangeType.StyleSheet) !=
          VersionChangeType.StyleSheet)
        return;

      ++m_Version;

      // Applying styles can trigger new changes, store changes in a separate
      // list
    ...
    private void ApplyStyles() {
      ...
      m_StyleContextHierarchyTraversal.PrepareTraversal(
  ...
  internal class VisualTreeStyleUpdaterTraversal : HierarchyTraversal {
    ...
    StyleMatchingContext m_StyleMatchingContext =
        new StyleMatchingContext(OnProcessMatchResult);
    ...
    static void OnProcessMatchResult(VisualElement current,
    ...
    public override void TraverseRecursive(VisualElement element, int depth) {
      ...
      // If the element is fully dirty, we need to erase those flags since the
      // full element and its subtree will be re-styled. If the element is not
      // in the update list, it's a parent of something dirty and therefore it
      // won't be restyled.
      ...
      int originalStyleSheetCount =
          m_StyleMatchingContext.styleSheetStack.Count;
      ...
      // Store the number of custom style before processing rules in case an
      // element stop to have matching custom styles the event still need to be
      // sent and only looking at the matched custom styles won't suffice.
      int originalCustomStyleCount =
          element.computedStyle.customPropertiesCount;
      if (updateElement) {
        m_StyleMatchingContext.currentElement = element;

        StyleSelectorHelper.FindMatches(m_StyleMatchingContext,
                                        m_TempMatchResults);

        ProcessMatchedRules(element, m_TempMatchResults);

        element.inheritedStylesHash =
            element.computedStyle.inheritedData.GetHashCode();
      ...
      // Need to send the custom styles event after the inheritance is resolved
      // because an element may want to read standard styles too
      // (TextInputFieldBase callback depends on it).
      if (updateElement && (originalCustomStyleCount > 0 ||
                            element.computedStyle.customPropertiesCount > 0)) {
      ...
      if (m_StyleMatchingContext.styleSheetStack.Count >
          originalStyleSheetCount) {
        m_StyleMatchingContext.styleSheetStack.RemoveRange(
            originalStyleSheetCount,
            m_StyleMatchingContext.styleSheetStack.Count -
                originalStyleSheetCount);
      }
    ...
    void ProcessMatchedRules(VisualElement element,
                             List<SelectorMatchRecord> matchingSelectors) {
      matchingSelectors.Sort((a, b) => SelectorMatchRecord.Compare(a, b));

      Int64 matchingRulesHash = element.fullTypeName.GetHashCode();
      // Let current DPI contribute to the hash so cache is invalidated when
      // this changes
      matchingRulesHash =
          (matchingRulesHash * 397) ^ currentPixelsPerPoint.GetHashCode();

      int oldVariablesHash =
          m_StyleMatchingContext.variableContext.GetVariableHash();
      int customPropertiesCount = 0;

      foreach (var record in matchingSelectors) {
        customPropertiesCount +=
            record.complexSelector.rule.customPropertiesCount;
      }

      if (customPropertiesCount > 0) {
        // Element defines new variables, add the parents variables at the
        // beginning of the processing context
        m_ProcessVarContext.AddInitialRange(
      ...
      if (StyleCache.TryGetValue(matchingRulesHash, out resolvedStyles)) {
        ...
        foreach (var record in matchingSelectors) {
          m_StylePropertyReader.SetContext(
              record.sheet, record.complexSelector,
              m_StyleMatchingContext.variableContext, dpiScaling);
    ...
    private void ProcessMatchedVariables(StyleSheet sheet, StyleRule rule) {
      foreach (var property in rule.properties) {
        if (property.isCustomProperty) {
          var sv = new StyleVariable(property.name, sheet, property.values);
