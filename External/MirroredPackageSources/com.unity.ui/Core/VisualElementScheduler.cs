...
namespace UnityEngine.UIElements {
  /// <summary>
  /// Represents a scheduled task created with a VisualElement's schedule
  /// interface.
  /// </summary>
  public interface IVisualElementScheduledItem {
    ...
    /// <summary>
    /// Will be true when this item is scheduled. Note that an item's callback
    /// will only be executed when it's VisualElement is attached to a panel.
    /// </summary>
    bool isActive { get; }

    /// <summary>
    /// If not already active, will schedule this item on its VisualElement's
    /// scheduler.
    /// </summary>
    void Resume();
    /// <summary>
    ...
    /// <summary>
    /// Cancels any previously scheduled execution of this item and re-schedules
    /// the item.
    /// </summary>
    /// <param name="delayMs">Minimum time in milliseconds before this item will
    /// be executed.</param>
    ...
    /// </summary>
    /// <param name="delayMs">The minimum number of milliseconds after
    /// activation where this item's action will be executed.</param>
    ...
    /// </summary>
    /// <param name="intervalMs">Minimum amount of time in milliseconds between
    /// each action execution.</param> <returns>This ScheduledItem.</returns>
    IVisualElementScheduledItem Every(long intervalMs);
    /// <summary>
    /// Item will be unscheduled automatically when specified condition is met.
    /// </summary>
    /// <param name="stopCondition">When condition returns true, the item will
    /// be unscheduled.</param> <returns>This ScheduledItem.</returns>
    ...
    /// </summary>
    /// <param name="durationMs">The total duration in milliseconds where this
    /// item will be active.</param> <returns>This ScheduledItem.</returns>
    IVisualElementScheduledItem ForDuration(long durationMs);
  }

  /// <summary>
  /// A scheduler allows you to register actions to be executed at a later
  /// point.
  /// </summary>
  ...
  internal class VisualElementPanelActivator {
    ...
    public VisualElementPanelActivator(
        IVisualElementPanelActivatable activatable) {
    ...
    public void SetActive(bool action) {
      if (isActive != action) {
        ...
        if (isActive) {
          ...
          m_Activatable.element.UnregisterCallback<DetachFromPanelEvent>(
              OnLeave);
          SendDeactivation();
        }
      }
  ...
  public partial class VisualElement : IVisualElementScheduler {
    ...
    /// <summary>
    /// This class is needed in order for its VisualElement to be notified when
    /// the scheduler removes an item
    /// </summary>
    private abstract class BaseVisualElementScheduledItem
      ...
      public bool isActive {
        get { return m_Activator.isActive; }
      }
      ...
      public void Resume() { m_Activator.SetActive(true); }

      public void Pause() { m_Activator.SetActive(false); }
      ...
      public bool CanBeActivated() {
        return element != null && element.elementPanel != null &&
               element.elementPanel.scheduler != null;
      }
    }

    /// <summary>
    /// We invoke updateEvents in subclasses to avoid lambda wrapping
    /// allocations
    /// </summary>
    private abstract class VisualElementScheduledItem<ActionType>
        : BaseVisualElementScheduledItem {
      public ActionType updateEvent;
      public VisualElementScheduledItem(VisualElement handler,
                                        ActionType upEvent)
          : base(handler) {
        updateEvent = upEvent;
      }
      ...
      public static bool Matches(ScheduledItem item, ActionType updateEvent) {
        VisualElementScheduledItem<ActionType> vItem =
            item as VisualElementScheduledItem<ActionType>;

        if (vItem != null) {
          return EqualityComparer<ActionType>.Default.Equals(vItem.updateEvent,
                                                             updateEvent);
        }
        return false;
      }
    ...
    private class TimerStateScheduledItem
        : VisualElementScheduledItem<Action<TimerState>> {
      public TimerStateScheduledItem(VisualElement handler,
                                     Action<TimerState> updateEvent)
    ...
    // we're doing this to avoid allocating a lambda for simple callbacks that
    // don't need TimerState
    ...
    IVisualElementScheduledItem
        IVisualElementScheduler.Execute(Action<TimerState> timerUpdateEvent) {
    ...
    IVisualElementScheduledItem
        IVisualElementScheduler.Execute(Action updateEvent) {
