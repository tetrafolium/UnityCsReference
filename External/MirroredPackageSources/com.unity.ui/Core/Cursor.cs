...
namespace UnityEngine.UIElements {
  /// <summary>
  /// Script interface for <see cref="VisualElement"/> cursor style property
  /// <see cref="IStyle.cursor"/>.
  /// </summary>
  public struct Cursor : IEquatable<Cursor> {
    /// <summary>
    /// The texture to use for the cursor style. To use a texture as a cursor,
    /// import the texture with "Read/Write enabled" in the texture importer (or
    /// using the "Cursor" defaults).
    /// </summary>
    ...
    public bool Equals(Cursor other) {
      return EqualityComparer<Texture2D>.Default.Equals(texture,
                                                        other.texture) &&
    ...
    public override int GetHashCode() {
      ...
      hashCode = hashCode * -1521134295 +
                 EqualityComparer<Vector2>.Default.GetHashCode(hotspot);
  ...
  internal class CursorManager : ICursorManager {
    public void SetCursor(Cursor cursor) {
      if (cursor.texture != null) {
        UnityEngine.Cursor.SetCursor(cursor.texture, cursor.hotspot,
                                     CursorMode.Auto);
      } else {
