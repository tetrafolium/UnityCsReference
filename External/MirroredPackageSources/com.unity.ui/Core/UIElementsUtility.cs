...
static UIEventRegistration() {
	...
	GUIUtility.endContainerGUIFromException += exception =>
	                                           EndContainerGUIFromException(exception);
...
private static bool ProcessEvent(int instanceID, IntPtr nativeEventPtr) {
	...
	foreach (var uiElementsUtility in s_Utilities) {
		if (uiElementsUtility.ProcessEvent(instanceID, nativeEventPtr,
		                                   ref eventHandled)) {
			return eventHandled;
		}
	}
...
RequestRepaintForPanels(Action<ScriptableObject> repaintCallback) {
	foreach (var uiElementsUtility in s_Utilities) {
...
private static Stack<IMGUIContainer> s_ContainerStack =
	new Stack<IMGUIContainer>();
private static Dictionary<int, Panel> s_UIElementsCache =
	new Dictionary<int, Panel>();

private static Event s_EventInstance =
	new Event(); // event instance reused for ProcessEvent()

// When not in editor, this will be all white, so no impact on the overall
// color, except for the multiplication done on the color.
...
private UIElementsUtility() {
	...
	//            ((IUIElementsUtility)this).TakeCapture();
	//            GUIUtility.releaseCapture += () =>
	//            ((IUIElementsUtility)this).ReleaseCapture();
	//            GUIUtility.processEvent += (i, ptr) =>
	//            {
	//                bool handled = false;
	//                ((IUIElementsUtility)this).ProcessEvent(i, ptr, ref
	//                handled); return handled;
	//            };
	//            GUIUtility.cleanupRoots += () =>
	//            ((IUIElementsUtility)this).CleanupRoots();
	//            GUIUtility.endContainerGUIFromException += exception =>
	//            ((IUIElementsUtility)this).EndContainerGUIFromException(exception);
	//
	//            GUIUtility.guiChanged += () =>
	//            ((IUIElementsUtility)this).MakeCurrentIMGUIContainerDirty();
}
...
bool IUIElementsUtility.TakeCapture() {
	if (s_ContainerStack.Count > 0) {
		...
		var currentCapturingElement =
			topmostContainer.panel.GetCapturingElement(
				PointerId.mousePointerId);
		if (currentCapturingElement != null &&
		    currentCapturingElement != topmostContainer) {
...
bool IUIElementsUtility.ProcessEvent(int instanceID, IntPtr nativeEventPtr,
                                     ref bool eventHandled) {
	Panel panel;
	if (nativeEventPtr != IntPtr.Zero &&
	    s_UIElementsCache.TryGetValue(instanceID, out panel)) {
...
void IUIElementsUtility.UpdateSchedulers() {
	// Since updating schedulers jumps into user code, the panels list might
	// change while we're iterating, we make a copy first.
...
void IUIElementsUtility.RequestRepaintForPanels(
	Action<ScriptableObject> repaintCallback) {
...
internal static void BeginContainerGUI(GUILayoutUtility.LayoutCache cache,
                                       Event evt,
	...
	GUIUtility.s_OriginalID =
		container.elementPanel.ownerObject.GetInstanceID();
...
internal static void EndContainerGUI(Event evt, Rect layoutSize) {
	if (Event.current.type == EventType.Layout &&
		...
		GUILayoutUtility.LayoutFromContainer(layoutSize.width,
		                                     layoutSize.height);
	}
	// restore cache
...
// In order for tests to run without an EditorWindow but still be able to
// send events, we sometimes need to force the event type.
// IMGUI::GetEventType() (native) will return the event type as Ignore if
// the proper views haven't yet been initialized. This (falsely) breaks
// tests that rely on the event type. So for tests, we just ensure the event
// type is what we originally set it to when we sent it.
internal static EventBase CreateEvent(Event systemEvent,
                                      EventType eventType) {
	switch (eventType) {
	...
	case EventType.MouseDown:
		// If some buttons are already down, we generate PointerMove/MouseDown
		// events. Otherwise we generate PointerDown/MouseDown events. See W3C
		// pointer events recommendation: https://www.w3.org/TR/pointerevents2
		if (PointerDeviceState.GetPressedButtons(PointerId.mousePointerId) !=
		    0) {
			return PointerMoveEvent.GetPooled(systemEvent);
		} else {
	...
	case EventType.MouseUp:
		// If more buttons are still down, we generate PointerMove/MouseUp
		// events. Otherwise we generate PointerUp/MouseUp events. See W3C
		// pointer events recommendation: https://www.w3.org/TR/pointerevents2
		if (PointerDeviceState.HasAdditionalPressedButtons(
			    PointerId.mousePointerId, systemEvent.button)) {
			return PointerMoveEvent.GetPooled(systemEvent);
		} else {
...
private static readonly ProfilerMarker s_RepaintProfilerMarker =
	new ProfilerMarker(s_RepaintProfilerMarkerName);
private static readonly ProfilerMarker s_EventProfilerMarker =
	new ProfilerMarker(s_EventProfilerMarkerName);

static bool DoDispatch(BaseVisualElementPanel panel) {
	bool usesEvent = false;

	if (s_EventInstance.type == EventType.Repaint) {
		using (s_RepaintProfilerMarker.Auto()) panel.Repaint(s_EventInstance);

		(panel.panelDebug?.debuggerOverlayPanel as Panel)
		?.Repaint(s_EventInstance);
		// TODO get rid of this when we wrap every GUIView inside
		// IMGUIContainers here we pretend to use the repaint event in order to
		// suspend to suspend OnGUI() processing on the native side since we've
		// already run it if we have an IMGUIContainer
		usesEvent = panel.IMGUIContainersCount > 0;
	} else {
		panel.ValidateLayout();

		using (EventBase evt = CreateEvent(s_EventInstance)) {
			bool immediate = s_EventInstance.type == EventType.Used ||
			                 s_EventInstance.type == EventType.Layout ||
			                 s_EventInstance.type == EventType.ExecuteCommand ||
			                 s_EventInstance.type == EventType.ValidateCommand;

			using (s_EventProfilerMarker.Auto())
				panel.SendEvent(evt, immediate ? DispatchMode.Immediate
			      : DispatchMode.Queued);

			// The dispatcher should have finished processing the event,
			// otherwise we cannot return a value for usesEvent.
			// FIXME: this makes GUIPointWindowConversionOnMultipleWindow fails
			// because the event sent in the OnGUI is put in the queue.
...
internal static Panel
FindOrCreateEditorPanel(ScriptableObject ownerObject) {
	Panel panel;
	if (!s_UIElementsCache.TryGetValue(ownerObject.GetInstanceID(),
	                                   out panel)) {
		panel = Panel.CreateEditorPanel(ownerObject);
		RegisterCachedPanel(ownerObject.GetInstanceID(), panel);
	} else {
		Debug.Assert(ContextType.Editor == panel.contextType,
		             "Panel is not an editor panel.");
	}
