...
namespace UnityEngine.UIElements {
  internal class StyleValueCollection {
    ...
    public StyleBackground GetStyleBackground(StylePropertyId id) {
      ...
      if (TryGetStyleValue(id, ref inline)) {
        var texture =
            inline.resource.IsAllocated ? inline.resource.Target as Texture2D
            : null;
    ...
    public StyleFont GetStyleFont(StylePropertyId id) {
      ...
      if (TryGetStyleValue(id, ref inline)) {
        var font =
            inline.resource.IsAllocated ? inline.resource.Target as Font : null;
  ...
  internal partial class InlineStyleAccess : StyleValueCollection {
    ...
    public void ApplyInlineStyles(ComputedStyle sharedStyle) {
      ...
      if (m_InlineRule.sheet != null) {
        ...
        s_StylePropertyReader.SetInlineContext(m_InlineRule.sheet,
                                               m_InlineRule.properties,
                                               m_InlineRule.propertyIds);
    ...
    private bool SetStyleValue<T>(StylePropertyId id, StyleEnum<T> inlineValue,
                                  StyleEnum<T> sharedValue) where T : struct
    , IConvertible {
      var sv = new StyleValue();
    ...
    private bool SetStyleValue(StylePropertyId id, StyleBackground inlineValue,
                               StyleBackground sharedValue) {
      var sv = new StyleValue();
      if (TryGetStyleValue(id, ref sv)) {
        var vectorImage =
            sv.resource.IsAllocated ? sv.resource.Target as VectorImage : null;
        var texture =
            sv.resource.IsAllocated ? sv.resource.Target as Texture2D : null;
    ...
    private bool SetStyleValue(StylePropertyId id, StyleFont inlineValue,
      ...
      sv.id = id;
      sv.keyword = inlineValue.keyword;
      sv.resource = inlineValue.value != null ? GCHandle.Alloc(inlineValue.value)
          : new GCHandle();
      ...
      if (inlineValue.keyword == StyleKeyword.Null) {
        sv.keyword = sharedValue.keyword;
        sv.resource = sharedValue.value !=
                      null ? GCHandle.Alloc(sharedValue.value) : new GCHandle();
      }
