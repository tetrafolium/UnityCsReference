...
namespace UnityEngine.UIElements {
  internal partial class ComputedStyle : ICustomStyle {
    ...
    private ComputedStyle(bool isShared) { this.isShared = isShared; }
    ...
    private bool ApplyGlobalKeyword(StylePropertyReader reader,
                                    ComputedStyle parentStyle) {
      var handle = reader.GetValue(0).handle;
      if (handle.valueType == StyleValueType.Keyword) {
        if ((StyleValueKeyword)
                handle.valueIndex == StyleValueKeyword.Initial) {
          ApplyInitialValue(reader);
          return true;
        } else if ((StyleValueKeyword)
                       handle.valueIndex == StyleValueKeyword.Unset) {
    ...
    public bool TryGetValue(CustomStyleProperty<bool> property,
      ...
      StylePropertyValue customProp;
      if (m_CustomProperties != null &&
          m_CustomProperties.TryGetValue(property.name, out customProp)) {
        value = customProp.sheet.ReadKeyword(customProp.handle) ==
    ...
    public bool TryGetValue(CustomStyleProperty<Color> property,
                            out Color value) {
      StylePropertyValue customProp;
    ...
    public bool TryGetValue(CustomStyleProperty<Texture2D> property,
                            out Texture2D value) {
      StylePropertyValue customProp;
      if (m_CustomProperties != null &&
          m_CustomProperties.TryGetValue(property.name, out customProp)) {
        ...
        if (StylePropertyReader.TryGetImageSourceFromValue(
                customProp, dpiScaling, out source) &&
            source.texture != null) {
    ...
    public bool TryGetValue(CustomStyleProperty<VectorImage> property,
    ...
    private bool TryGetValue(string propertyName, StyleValueType valueType,
                             out StylePropertyValue customProp) {
      customProp = new StylePropertyValue();
      if (m_CustomProperties != null &&
          m_CustomProperties.TryGetValue(propertyName, out customProp)) {
        // CustomProperty only support one value
        var handle = customProp.handle;
        if (handle.valueType != valueType) {
          Debug.LogWarning(string.Format(
              "Trying to read value as {0} while parsed type is {1}", valueType,
              handle.valueType));
          return false;
        }
