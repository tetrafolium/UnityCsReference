...
public static object GetComputedStyleValue(ComputedStyle computedStyle,
                                           string name) {
	StylePropertyId id;
	if (StylePropertyUtil.s_NameToId.TryGetValue(name, out id))
...
public static void SetInlineStyleValue(IStyle style, string name,
                                       object value) {
	StylePropertyId id;
	if (StylePropertyUtil.s_NameToId.TryGetValue(name, out id)) {
...
public static void
FindSpecifiedStyles(ComputedStyle computedStyle,
                    IEnumerable<SelectorMatchRecord> matchRecords,
                    Dictionary<StylePropertyId, int> result) {
	result.Clear();
	...
	foreach (var id in inheritedPropId) {
		...
		var initialValue =
			StyleDebug.GetComputedStyleValue(InitialStyle.Get(), id);
