...
internal class ListViewReorderableDragAndDropController
	: IListViewDragAndDropController {
...
public virtual StartDragArgs SetupDragAndDrop(IEnumerable<object> items) {
	...
	foreach (var unused in items) {
		if (string.IsNullOrEmpty(title)) {
			...
			var label =
				m_ListView.GetRecycledItemFromIndex(index)?.element.Q<Label>();
...
public virtual DragVisualMode HandleDragAndDrop(IListDragAndDropArgs args) {
	...
	return args.dragAndDropData.userData ==
	       m_ListView ? DragVisualMode.Move : DragVisualMode.Rejected;
}
...
public virtual void OnDrop(IListDragAndDropArgs args) {
	...
	var selectedIndices =
		m_ListView.selectedIndices.OrderBy(i => i).ToArray();
	...
	default:
		throw new ArgumentException(
			      $"{args.dragAndDropPosition} is not supported by {nameof(ListViewReorderableDragAndDropController)}.");
	}
...
private void InsertRange(int index) {
	...
	for (var i = 0; i < selectedItems.Length; i++) {
		...
		onItemMoved?.Invoke(new ItemMoveArgs<object> {
					item = item, newIndex = index, previousIndex = selectedIndices[i]
				});
