	...
	public override int GetHashCode() {
		unchecked {
			...
			hashCode = (hashCode * 397) ^
			           (recycledItem != null ? recycledItem.GetHashCode() : 0);
...
private readonly List<VisualElement> m_PickedElements =
...
protected override bool CanStartDrag(Vector3 pointerPosition) {
	...
	if (!targetScrollView.contentContainer.worldBound.Contains(
		    pointerPosition))
		return false;
...
protected override StartDragArgs StartDrag(Vector3 pointerPosition) {
	return dragAndDropController.SetupDragAndDrop(
		targetListView.selectedItems);
}
...
protected override void OnDrop(Vector3 pointerPosition) {
	...
	var args = MakeDragAndDropArgs(dragPosition);
	if (dragAndDropController.HandleDragAndDrop(args) !=
	    DragVisualMode.Rejected)
		dragAndDropController.OnDrop(args);
}

protected void HandleDragAndScroll(Vector2 pointerPosition) {
	var scrollUp = pointerPosition.y <
	               targetScrollView.worldBound.yMin + k_AutoScrollAreaSize;
	var scrollDown = pointerPosition.y >
	                 targetScrollView.worldBound.yMax - k_AutoScrollAreaSize;
	if (scrollUp || scrollDown) {
		targetScrollView.scrollOffset += (scrollUp ? Vector2.down
		                          : Vector2.up) *
		                                 k_PanSpeed;
	}
}
...
protected void ApplyDragAndDropUI(DragPosition dragPosition) {
	...
	case DragAndDropPosition.OverItem:
		dragPosition.recycledItem.element.AddToClassList(
			ListView.itemDragHoverUssClassName);
		break;
	case DragAndDropPosition.BetweenItems:
		if (dragPosition.insertAtIndex == 0)
			...
			PlaceHoverBarAtElement(
				targetListView
				.GetRecycledItemFromIndex(dragPosition.insertAtIndex - 1)
				.element);
		break;
	case DragAndDropPosition.OutsideItems:
		var recycledItem = targetListView.GetRecycledItemFromIndex(
			targetListView.itemsSource.Count - 1);
	...
	default:
		throw new ArgumentOutOfRangeException(
			      nameof(dragPosition.dragAndDropPosition),
			      dragPosition.dragAndDropPosition,
			      $"Unsupported {nameof(dragPosition.dragAndDropPosition)} value");
	}
}

protected bool TryGetDragPosition(Vector2 pointerPosition,
                                  ref DragPosition dragPosition) {
	var recycledItem = GetRecycledItem(pointerPosition);
	if (recycledItem != null) {
		// Below an item
		if (recycledItem.element.worldBound.yMax - pointerPosition.y <
		    k_BetweenElementsAreaSize) {
		...
		// Upon an item
		if (pointerPosition.y - recycledItem.element.worldBound.yMin >
		    k_BetweenElementsAreaSize) {
	...
	if (pointerPosition.y >=
	    targetScrollView.contentContainer.worldBound.yMax)
...
private ListDragAndDropArgs MakeDragAndDropArgs(DragPosition dragPosition) {
	...
	return new ListDragAndDropArgs {
		       target = target, insertAtIndex = dragPosition.insertAtIndex,
...
private void PlaceHoverBarAtElement(VisualElement element) {
	...
	PlaceHoverBarAt(
		Mathf.Min(elementBounds.yMax,
		          contentViewport.localBound.yMax - k_DragHoverBarHeight));
}
...
public static ListView.RecycledItem
GetRecycledItemFromIndex(this ListView listView, int index) {
