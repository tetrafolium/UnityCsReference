...
namespace UnityEngine.UIElements {
  internal struct RuleMatcher {
    ...
    public RuleMatcher(StyleSheet sheet, StyleComplexSelector complexSelector,
                       int styleSheetIndexInStack) {
      this.sheet = sheet;
      this.complexSelector = complexSelector;
    }

    public override string ToString() { return complexSelector.ToString(); }
  }

  /// <summary>
  /// UQuery is a set of extension methods allowing you to select individual or
  /// collection of visualElements inside a complex hierarchy.
  /// </summary>
  public static class UQuery {
    // This scheme saves us 20 bytes instead of saving a Func<object, bool>
    // directly (12 vs 32 bytes)
    internal interface IVisualPredicateWrapper { bool Predicate(object e); }

    internal class IsOfType<T> : IVisualPredicateWrapper where T
        : VisualElement {
      public static IsOfType<T> s_Instance = new IsOfType<T>();

      public bool Predicate(object e) { return e is T; }
    }

    internal class PredicateWrapper<T> : IVisualPredicateWrapper where T
        : VisualElement {
      private Func<T, bool> predicate;
      public PredicateWrapper(Func<T, bool> p) { predicate = p; }
    ...
    internal abstract class UQueryMatcher : HierarchyTraversal {
      ...
      protected virtual bool OnRuleMatchedElement(RuleMatcher matcher,
      ...
      public override void TraverseRecursive(VisualElement element, int depth) {
        ...
        for (int j = 0; j < count; j++) {
          ...
          if (StyleSelectorHelper.MatchRightToLeft(
                  element, matcher.complexSelector,
                  (e, i) => NoProcessResult(e, i))) {
        ...
        // Remove all matchers that we could possibly have added at this level
        // of recursion
        if (m_Matchers.Count > originalCount) {
          m_Matchers.RemoveRange(originalCount,
                                 m_Matchers.Count - originalCount);
        }
      }
    ...
    internal class FirstQueryMatcher : SingleQueryMatcher {
      protected override bool OnRuleMatchedElement(RuleMatcher matcher,
                                                   VisualElement element) {
        if (match == null)
          match = element;
        return true;
  ...
  /// <summary>
  /// Query object containing all the selection rules. Can be saved and rerun
  /// later without re-allocating memory.
  /// </summary>
  public struct UQueryState<T> : IEquatable<UQueryState<T>> where T
      : VisualElement {
    // this makes it non-thread safe. But saves on allocations...
    private static UQuery.FirstQueryMatcher s_First =
        new UQuery.FirstQueryMatcher();
    private static UQuery.LastQueryMatcher s_Last =
        new UQuery.LastQueryMatcher();
    private static UQuery.IndexQueryMatcher s_Index =
        new UQuery.IndexQueryMatcher();
    ...
    /// <summary>
    /// Creates a new QueryState with the same selection rules, applied on
    /// another VisualElement.
    /// </summary>
    /// <param name="element">The element on which to apply the selection
    /// rules.</param> <returns>A new QueryState with the same selection rules,
    /// applied on this element.</returns>
    ...
    /// </summary>
    /// <returns>The first element matching all the criteria, or null if none
    /// was found.</returns>
    public T First() {
    ...
    /// </summary>
    /// <returns>The last element matching all the criteria, or null if none was
    /// found.</returns>
    public T Last() {
    ...
    private class ListQueryMatcher : UQuery.UQueryMatcher {
      ...
      public void Reset() { matches = null; }
    }
    ...
    /// </summary>
    /// <param name="results">Adds all elements satisfying selection rules to
    /// the list.</param>
    ...
    /// </summary>
    /// <returns>A list containing elements satisfying selection
    /// rules.</returns>
    ...
    /// <summary>
    /// Selects the n th element matching all the criteria, or null if not
    /// enough elements were found.
    /// </summary>
    ...
    /// </summary>
    /// <param name="funcCall">The action to be invoked with each matching
    /// element.</param>
    ...
    private class DelegateQueryMatcher<TReturnType> : UQuery.UQueryMatcher {
      ...
      public static DelegateQueryMatcher<TReturnType> s_Instance =
          new DelegateQueryMatcher<TReturnType>();
    ...
    /// Invokes function on all elements matching the query.
    /// </summary>
    /// <param name="result">Each return value will be added to this
    /// list.</param> <param name="funcCall">The function to be invoked with
    /// each matching element.</param>
    ...
    public bool Equals(UQueryState<T> other) {
      return ReferenceEquals(m_Element, other.m_Element) &&
             EqualityComparer<List<RuleMatcher>>.Default.Equals(
                 m_Matchers, other.m_Matchers);
    }
    ...
    public override int GetHashCode() {
      ...
      hashCode = hashCode * -1521134295 +
                 EqualityComparer<VisualElement>.Default.GetHashCode(m_Element);
      hashCode =
          hashCode * -1521134295 +
          EqualityComparer<List<RuleMatcher>>.Default.GetHashCode(m_Matchers);
      return hashCode;
    }

    public static bool operator ==(UQueryState<T> state1,
                                   UQueryState<T> state2) {
  ...
  /// visual element.
  /// </summary>
  public struct UQueryBuilder<T> : IEquatable<UQueryBuilder<T>> where T
      : VisualElement {
    private List<StyleSelector> m_StyleSelectors;
    private List<StyleSelector> styleSelectors {
      get {
        return m_StyleSelectors ??(m_StyleSelectors = new List<StyleSelector>());
      }
    }

    private List<StyleSelectorPart> m_Parts;
    private List<StyleSelectorPart> parts {
      get { return m_Parts ??(m_Parts = new List<StyleSelectorPart>()); }
    }
    ...
    /// </summary>
    /// <param name="visualElement">The root element on which to condfuct the
    /// search query.</param>
    public UQueryBuilder(VisualElement visualElement) : this() {
    ...
    /// <summary>
    /// Selects all elements with the given class. Not to be confused with Type
    /// (see OfType<>()).
    /// </summary>
    ...
    /// <summary>
    /// Selects all elements that are descendants of currently matching
    /// ancestors.
    /// </summary>
    public UQueryBuilder<T2>
    Descendents<T2>(string name = null, params string[] classNames) where T2
        : VisualElement {
    ...
    /// ancestors.
    /// </summary>
    public UQueryBuilder<T2> Descendents<T2>(string name = null,
                                             string classname = null) where T2
        : VisualElement {
      FinishCurrentSelector();
      AddType<T2>();
      AddName(name);
    ...
    /// <summary>
    /// Selects all direct child elements of elements matching the previous
    /// rules.
    /// </summary>
    public UQueryBuilder<T2> Children<T2>(string name = null,
                                          params string[] classes) where T2
        : VisualElement {
      FinishCurrentSelector();
      AddType<T2>();
      AddName(name);
    ...
    /// <summary>
    /// Selects all elements of the specified Type (eg: Label, Button,
    /// ScrollView, etc).
    /// </summary>
    /// <param name="name">If specified, will select elements with this
    /// name.</param> <param name="classes">If specified, will select elements
    /// with the given class (not to be confused with Type).</param>
    /// <returns>QueryBuilder configured with the associated selection
    /// rules.</returns>
    public UQueryBuilder<T2> OfType<T2>(string name = null,
                                        params string[] classes) where T2
        : VisualElement {
      AddType<T2>();
      AddName(name);
    ...
    /// </summary>
    /// <param name="name">If specified, will select elements with this
    /// name.</param> <param name="className">If specified, will select elements
    /// with the given class (not to be confused with Type).</param>
    /// <returns>QueryBuilder configured with the associated selection
    /// rules.</returns>
    public UQueryBuilder<T2> OfType<T2>(string name = null,
    ...
    // Only used to avoid allocations in Q<>() Don't use this unless you know
    // what you're doing
    internal UQueryBuilder<T> SingleBaseType() {
      parts.Add(
          StyleSelectorPart.CreatePredicate(UQuery.IsOfType<T>.s_Instance));
      return this;
    }
    ...
    /// </summary>
    /// <param name="selectorPredicate">Predicate that must return true for
    /// selected elements.</param> <returns>QueryBuilder configured with the
    /// associated selection rules.</returns>
    public UQueryBuilder<T> Where(Func<T, bool> selectorPredicate) {
      // we can't use a static instance as in the QueryState<T>.ForEach below
      // since the query might be long lived
      parts.Add(StyleSelectorPart.CreatePredicate(
          new UQuery.PredicateWrapper<T>(selectorPredicate)));
      return this;
    }
    ...
    private void AddType<T2>() where T2 : VisualElement {
      if (typeof(T2) != typeof(VisualElement))
        parts.Add(
            StyleSelectorPart.CreatePredicate(UQuery.IsOfType<T2>.s_Instance));
    }
    ...
    /// </summary>
    public UQueryBuilder<T> Visible() { return Where(e => e.visible); }

    /// <summary>
    /// Selects all elements that are not visible.
    /// </summary>
    public UQueryBuilder<T> NotVisible() { return Where(e => !e.visible); }
    ...
    private UQueryBuilder<T2>
    AddRelationship<T2>(StyleSelectorRelationship relationship) where T2
        : VisualElement {
      return new UQueryBuilder<T2>(m_Element){
        m_Matchers = m_Matchers,
        m_Parts = m_Parts,
        m_StyleSelectors = m_StyleSelectors,
        m_Relationship = relationship == StyleSelectorRelationship.None ?
    ...
    void AddPseudoStatesRuleIfNecessasy() {
      if (pseudoStatesMask != 0 || negatedPseudoStatesMask != 0) {
        parts.Add(
            new StyleSelectorPart(){type = StyleSelectorType.PseudoClass});
      }
    }
    ...
    private bool CurrentSelectorEmpty() {
      return parts.Count == 0 &&
             m_Relationship == StyleSelectorRelationship.None &&
             pseudoStatesMask == 0 && negatedPseudoStatesMask == 0;
    }
    ...
    public static implicit operator T(UQueryBuilder<T> s) { return s.First(); }

    public static bool operator ==(UQueryBuilder<T> builder1,
    ...
    /// </summary>
    /// <returns>The first element matching all the criteria, or null if none
    /// was found.</returns> <seealso cref="UQueryState{T}.First"/>
    public T First() { return Build().First(); }
    ...
    /// <returns>The last element matching all the criteria, or null if none was
    /// found.</returns>
    public T Last() { return Build().Last(); }
    ...
    /// <returns>A list containing elements satisfying selection
    /// rules.</returns>
    public List<T> ToList() { return Build().ToList(); }
    ...
    /// <param name="results">Adds all elements satisfying selection rules to
    /// the list.</param>
    public void ToList(List<T> results) { Build().ToList(results); }

    /// <summary>
    /// Convenience overload, shorthand for Build().AtIndex().
    /// </summary>
    /// <seealso cref="UQueryState{T}.AtIndex"/>
    public T AtIndex(int index) { return Build().AtIndex(index); }
    ...
    /// Convenience overload, shorthand for Build().ForEach().
    /// </summary>
    /// <param name="funcCall">The function to be invoked with each matching
    /// element.</param>
    public List<T2> ForEach<T2>(Func<T, T2> funcCall) {
    ...
    /// <param name="funcCall">The function to be invoked with each matching
    /// element.</param>
    public void ForEach(Action<T> funcCall) { Build().ForEach(funcCall); }

    public bool Equals(UQueryBuilder<T> other) {
      return EqualityComparer<List<StyleSelector>>.Default.Equals(
                 m_StyleSelectors, other.m_StyleSelectors) &&
             EqualityComparer<List<StyleSelector>>.Default.Equals(
                 styleSelectors, other.styleSelectors) &&
             EqualityComparer<List<StyleSelectorPart>>.Default.Equals(
                 m_Parts, other.m_Parts) &&
             EqualityComparer<List<StyleSelectorPart>>.Default.Equals(
                 parts, other.parts) &&
             ReferenceEquals(m_Element, other.m_Element) &&
             EqualityComparer<List<RuleMatcher>>.Default.Equals(
                 m_Matchers, other.m_Matchers) &&
    ...
    public override int GetHashCode() {
      ...
      hashCode = hashCode * -1521134295 +
                 EqualityComparer<List<StyleSelector>>.Default.GetHashCode(
                     m_StyleSelectors);
      hashCode = hashCode * -1521134295 +
                 EqualityComparer<List<StyleSelector>>.Default.GetHashCode(
                     styleSelectors);
      hashCode = hashCode * -1521134295 +
                 EqualityComparer<List<StyleSelectorPart>>.Default.GetHashCode(
                     m_Parts);
      hashCode =
          hashCode * -1521134295 +
          EqualityComparer<List<StyleSelectorPart>>.Default.GetHashCode(parts);
      hashCode = hashCode * -1521134295 +
                 EqualityComparer<VisualElement>.Default.GetHashCode(m_Element);
      hashCode =
          hashCode * -1521134295 +
  ...
  public static class UQueryExtensions {
    ...
    private static UQueryState<VisualElement> SingleElementTypeAndClassQuery =
        new UQueryBuilder<VisualElement>(null)
            ...
            .Class(String.Empty)
            .Build();
    private static UQueryState<VisualElement>
    ...
    /// </summary>
    /// <param name="e">Root VisualElement on which the selector will be
    /// applied.</param> <param name="name">If specified, will select elements
    /// with this name.</param> <param name="classes">If specified, will select
    /// elements with the given class (not to be confused with Type).</param>
    /// <returns>The first element matching all the criteria, or null if none
    /// was found.</returns>
    public static T Q<T>(this VisualElement e, string name = null,
                         params string[] classes) where T : VisualElement {
    ...
    /// <returns>The first element matching all the criteria, or null if none
    /// was found.</returns>
    public static VisualElement Q(this VisualElement e, string name = null,
    ...
    /// <param name="e">Root VisualElement on which the selector will be
    /// applied.</param> <param name="name">If specified, will select elements
    /// with this name.</param> <param name="className">If specified, will
    /// select elements with the given class (not to be confused with
    /// Type).</param> <returns>The first element matching all the criteria, or
    /// null if none was found.</returns>
    public static T Q<T>(this VisualElement e, string name = null,
                         string className = null) where T : VisualElement {
      ...
      if (name == null) {
        if (className == null) {
          ...
          query.m_Matchers[0].complexSelector.selectors[0].parts[0] =
              StyleSelectorPart.CreatePredicate(UQuery.IsOfType<T>.s_Instance);
        ...
        query.m_Matchers[0].complexSelector.selectors[0].parts[0] =
            StyleSelectorPart.CreatePredicate(UQuery.IsOfType<T>.s_Instance);
        query.m_Matchers[0].complexSelector.selectors[0].parts[1] =
            StyleSelectorPart.CreateClass(className);
        return query.First() as T;
      }
      ...
      if (className == null) {
        ...
        query.m_Matchers[0].complexSelector.selectors[0].parts[0] =
            StyleSelectorPart.CreatePredicate(UQuery.IsOfType<T>.s_Instance);
        query.m_Matchers[0].complexSelector.selectors[0].parts[1] =
            StyleSelectorPart.CreateId(name);
        return query.First() as T;
      }
      ...
      query.m_Matchers[0].complexSelector.selectors[0].parts[1] =
          StyleSelectorPart.CreateId(name);
      query.m_Matchers[0].complexSelector.selectors[0].parts[2] =
          StyleSelectorPart.CreateClass(className);
      return query.First() as T;
    }

    internal static T MandatoryQ<T>(this VisualElement e, string name,
                                    string className = null) where T
        : VisualElement {
    ...
    internal static VisualElement MandatoryQ(this VisualElement e, string name,
                                             string className = null) {
    ...
    Query(this VisualElement e, string name = null, params string[] classes) {
    ...
    /// with this name.</param> <param name="className">If specified, will
    /// select elements with the given class (not to be confused with
    /// Type).</param> <returns>QueryBuilder configured with the associated
    /// selection rules.</returns>
    public static UQueryBuilder<VisualElement>
    Query(this VisualElement e, string name = null, string className = null) {
    ...
    /// <returns>QueryBuilder configured with the associated selection
    /// rules.</returns>
    public static UQueryBuilder<T> Query<T>(this VisualElement e,
                                            string name = null,
                                            params string[] classes) where T
        : VisualElement {
      var queryBuilder =
          new UQueryBuilder<VisualElement>(e).OfType<T>(name, classes);
      return queryBuilder;
    }
    ...
    public static UQueryBuilder<T> Query<T>(this VisualElement e,
        ...
        : VisualElement {
      var queryBuilder =
          new UQueryBuilder<VisualElement>(e).OfType<T>(name, className);
      return queryBuilder;
    }
    ...
    /// </summary>
    /// <param name="e">Root VisualElement on which the selector will be
    /// applied.</param> <returns>QueryBuilder configured with the associated
    /// selection rules.</returns>
