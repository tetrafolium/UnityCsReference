...
namespace UnityEngine.UIElements {
  ...
  internal enum VersionChangeType {
    ...
    BorderRadius = 1 << 7,
    BorderWidth = 1 << 8,
    // changes that may impact the world transform (e.g. laid out position,
    // local transform)
    Transform = 1 << 9,
    // changes to the size of the element after layout has been performed,
    // without taking the local transform into account
    Size = 1 << 10,
  ...
  /// <summary>
  /// Offers a set of values that describe the intended usage patterns of a
  /// specific <see cref="VisualElement"/>.
  /// </summary>
  [Flags]
  public enum UsageHints {
    ...
    None = 0,
    /// <summary>
    /// Marks a <see cref="VisualElement"/> that changes its transformation
    /// often (i.e. position, rotation or scale). When specified, this flag
    /// hints the system to optimize rendering of the <see
    /// cref="VisualElement"/> for recurring transformation changes. The
    /// VisualElement's vertex transformation will be done by the GPU when
    /// possible on the target platform. Please note that the number of
    /// VisualElements to which this hint effectively applies can be limited by
    /// target platform capabilities. For such platforms, it is recommended to
    /// prioritize use of this hint to only the VisualElements with the highest
    /// frequency of transformation changes.
    /// </summary>
    DynamicTransform = 1 << 0,
    /// <summary>
    /// Marks a <see cref="VisualElement"/> that hosts many children with <see
    /// cref="DynamicTransform"/> applied on them. A common use-case of this
    /// hint is a VisualElement that represents a "viewport" within which there
    /// are many <see cref="DynamicTransform"/> VisualElements that can move
    /// individually in addition to the "viewport" element also often changing
    /// its transformation. However, if the contents of the aforementioned
    /// "viewport" element are mostly static (not moving) then it is enough to
    /// use the <see cref="DynamicTransform"/> hint on that element instead of
    /// <see cref="GroupTransform"/>. Internally, an element hinted with <see
    /// cref="GroupTransform"/> will force a separate draw batch with its world
    /// transformation value, but in the same time it will avoid changing the
    /// transforms of all its descendants whenever a transformation change
    /// occurs on the <see cref="GroupTransform"/> element.
    /// </summary>
    GroupTransform = 1 << 1
  }
  ...
  internal enum RenderHints {
    ...
    ClipWithScissors =
        1 << 2 // If clipping is requested on this element, prefer scissoring
  }
  ...
  internal delegate void HierarchyEvent(VisualElement ve,
  ...
  public interface IPanel : IDisposable {
    ...
    /// <summary>
    /// Returns the top element at this position. Will not return elements with
    /// pickingMode set to <see cref="PickingMode.Ignore"/>.
    /// </summary>
    /// <param name="point">World coordinates.</param>
    /// <returns>Top VisualElement at the position. Null if none was
    /// found.</returns>
    VisualElement Pick(Vector2 point);

    /// <summary>
    /// Returns all elements at this position. Will not return elements with
    /// pickingMode set to <see cref="PickingMode.Ignore"/>.
    /// </summary>
    /// <param name="point">World coordinates.</param>
    /// <param name="picked">All Visualelements overlapping this
    /// position.</param> <returns>Top VisualElement at the position. Null if
  ...
  abstract class BaseVisualElementPanel : IPanel {
    ...
    internal abstract void OnVersionChanged(VisualElement ele,
                                            VersionChangeType changeTypeFlag);
    internal abstract void SetUpdater(IVisualTreeUpdater updater,
                                      VisualTreeUpdatePhase phase);
    ...
    internal void SendEvent(EventBase e,
                            DispatchMode dispatchMode = DispatchMode.Queued) {
    ...
    public abstract VisualElement PickAll(Vector2 point,
    ...
    internal
        abstract IVisualTreeUpdater GetUpdater(VisualTreeUpdatePhase phase);

    internal ElementUnderPointer m_TopElementUnderPointers =
        new ElementUnderPointer();
    ...
    internal
        VisualElement RecomputeTopElementUnderPointer(Vector2 pointerPos,
    ...
    internal void ClearCachedElementUnderPointer(EventBase triggerEvent) {
      m_TopElementUnderPointers.SetTemporaryElementUnderPointer(null,
                                                                triggerEvent);
    }

    void SetElementUnderPointer(VisualElement newElementUnderPointer,
                                int pointerId, Vector2 pointerPos) {
      m_TopElementUnderPointers.SetElementUnderPointer(newElementUnderPointer,
                                                       pointerId, pointerPos);
    }
    ...
    internal event Action standardShaderChanged,
        standardWorldSpaceShaderChanged;
    protected void InvokeStandardShaderChanged() {
      if (standardShaderChanged != null)
        standardShaderChanged();
    }
    protected void InvokeStandardWorldSpaceShaderChanged() {
      if (standardWorldSpaceShaderChanged != null)
        standardWorldSpaceShaderChanged();
    }

    internal event HierarchyEvent hierarchyChanged;
    internal void InvokeHierarchyChanged(VisualElement ve,
                                         HierarchyChangeType changeType) {
    ...
    internal void UpdateElementUnderPointers() {
      foreach (var pointerId in PointerId.hoveringPointers) {
        if (PointerDeviceState.GetPanel(pointerId) != this) {
          ...
          // Here it's important to call PickAll instead of Pick to ensure we
  ...
  // Strategy to load assets must be provided in the context of Editor or
  // Runtime
  internal delegate Object LoadResourceFunction(
      string pathName, System.Type type, float dpiScaling);

  // Strategy to fetch real time since startup in the context of Editor or
  // Runtime
  ...
  // Strategy to save persistent data must be provided in the context of Editor
  // or Runtime
...
#pragma warning restore CS0649
    ...
    internal void Focus() { focusController?.SetFocusToLastFocusedElement(); }

    internal void Blur() { focusController?.BlurLastFocusedElement(); }
    ...
    void CreateMarkers() {
      if (!string.IsNullOrEmpty(m_PanelName)) {
        ...
        m_MarkerAnimations =
            new ProfilerMarker($"Panel.Animations.{m_PanelName}");
      } else {
    ...
    internal static Panel CreateEditorPanel(ScriptableObject ownerObject) {
      return new Panel(ownerObject, ContextType.Editor,
                       EventDispatcher.editorDispatcher);
    }

    public Panel(ScriptableObject ownerObject, ContextType contextType,
      ...
      focusController =
          new FocusController(new VisualElementFocusRing(visualTree));
    ...
    // For tests only.
    internal static VisualElement
    PickAllWithoutValidatingLayout(VisualElement root, Vector2 point) {
      return PickAll(root, point);
    }

    private static VisualElement PickAll(VisualElement root, Vector2 point,
    ...
    PerformPick(VisualElement root, Vector2 point,
                List<VisualElement> picked = null) {
      ...
      // Problem here: everytime we pick, we need to do that expensive
      // transformation. The default Contains() compares with rect, while we
      // could cache the rect in world space (transform 2 points, 4 if there is
      // rotation) and be done here we have to transform 1 point at every call.
      // Now since this is a virtual, we can't just start to call it with global
      // pos... we could break client code. EdgeControl and port connectors in
      // GraphView overload this.
      ...
      if (root.enabledInHierarchy && root.visible &&
          root.pickingMode == PickingMode.Position && containsPoint) {
        picked?.Add(root);
    ...
    public override VisualElement PickAll(Vector2 point,
                                          List<VisualElement> picked) {
      ValidateLayout();
    ...
    public override VisualElement Pick(Vector2 point) {
      ValidateLayout();
      var element = m_TopElementUnderPointers.GetTopElementUnderPointer(
          PointerId.mousePointerId, out Vector2 mousePos, out bool isTemporary);
      // The VisualTreeTransformClipUpdater updates the ElementUnderPointer
      // after each validate layout. small enough to be smaller than 1 pixel
    ...
    public override void ValidateLayout() {
      // Reentrancy proofing: ValidateLayout() could be in the code path of
      // updaters. Actual case: TransformClip update phase recomputes elements
      // under mouse, which does a pick, which validates layout. Updaters use
      // version numbers for early exit, but it may happen that an updater
      // invalidates a subsequent updater.
      if (!m_ValidatingLayout) {
        ...
        m_VisualTreeUpdater.UpdateVisualTreePhase(
    ...
    void UpdateForRepaint() {
      // Here we don't want to update animation and bindings which are ticked by
      // the scheduler
    ...
    public override void Repaint(Event e) {
      ...
      using(m_MarkerUpdate.Auto()) { UpdateForRepaint(); }

      panelDebug?.Refresh();
    }

    internal override void
    OnVersionChanged(VisualElement ve, VersionChangeType versionChangeType) {
      ++m_Version;
      m_VisualTreeUpdater.OnVersionChanged(ve, versionChangeType);
      panelDebug?.OnVersionChanged(ve, versionChangeType);
    }

    internal override void SetUpdater(IVisualTreeUpdater updater,
                                      VisualTreeUpdatePhase phase) {
      m_VisualTreeUpdater.SetUpdater(updater, phase);
    }

    internal
        override IVisualTreeUpdater GetUpdater(VisualTreeUpdatePhase phase) {
      return m_VisualTreeUpdater.GetUpdater(phase);
    }
  }

  internal class RuntimePanel : Panel {
    public RuntimePanel(ScriptableObject ownerObject,
    ...
    internal bool drawToCameras {
      get {
        return (GetUpdater(VisualTreeUpdatePhase.Repaint) as UIRRepaintUpdater)
            ?.renderChain?.drawInCameras == true;
      }
      set {
        var renderChain =
            (GetUpdater(VisualTreeUpdatePhase.Repaint) as UIRRepaintUpdater)
            ?.renderChain;
        if (renderChain != null)
    ...
    public override void Repaint(Event e) {
      // if the renderTarget is not set, we simply render on whatever target is
      // currently set
      if (targetTexture == null) {
        ...
        // render on the whole framebuffer, so we force a fullscreen viewport
        // here.
