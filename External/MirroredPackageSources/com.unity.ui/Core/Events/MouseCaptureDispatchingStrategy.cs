...
namespace UnityEngine.UIElements {
  class MouseCaptureDispatchingStrategy : IEventDispatchingStrategy {
    ...
    public bool CanDispatchEvent(EventBase evt) {
      // Send all IMGUI events (for backward compatibility) and MouseEvents with
      // null target (because thats what we want to do in the new system) to the
      // capture, if there is one. Note that events coming from IMGUI have their
      // target set to null.
    ...
    public void DispatchEvent(EventBase evt, IPanel panel) {
      ...
      IEventHandler capturingElement =
      ...
      if (evt.eventTypeId != MouseCaptureOutEvent.TypeId() &&
          captureVE != null && captureVE.panel == null) {
        captureVE.ReleaseMouse();
        return;
      }

      if (panel != null && captureVE != null &&
          captureVE.panel.contextType != panel.contextType) {
        return;
      }
      ...
      if (mouseEvent != null &&
          (evt.target == null || evt.target == capturingElement)) {
      ...
      if ((captureBehavior & EventBehavior.IsCapturable) ==
          EventBehavior.IsCapturable) {
        BaseVisualElementPanel basePanel = panel as BaseVisualElementPanel;

        if (mouseEvent != null && basePanel != null) {
          bool shouldRecomputeTopElementUnderMouse =
              (mouseEvent as IMouseEventInternal)
              ?.recomputeTopElementUnderMouse ?? true;

          if (shouldRecomputeTopElementUnderMouse)
            basePanel.RecomputeTopElementUnderPointer(mouseEvent.mousePosition,
                                                      evt);
        }

        evt.dispatch = true;
        evt.target = capturingElement;
        (capturingElement as CallbackEventHandler)
            ?.HandleEventAtTargetPhase(evt);
        // Do further processing with a target computed the usual way.
        // However, if IsSentExclusivelyToCapturingElement, the only thing
        // remaining to do is ExecuteDefaultAction, which should be done with
        // mouseCapture as the target.
        if ((captureBehavior &
             EventBehavior.IsSentExclusivelyToCapturingElement) !=
            EventBehavior.IsSentExclusivelyToCapturingElement) {
          evt.target = null;
        }
