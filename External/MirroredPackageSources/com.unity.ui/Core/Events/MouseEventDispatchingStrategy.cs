...
namespace UnityEngine.UIElements {
  class MouseEventDispatchingStrategy : IEventDispatchingStrategy {
    public bool CanDispatchEvent(EventBase evt) { return evt is IMouseEvent; }
    ...
    static bool SendEventToRegularTarget(EventBase evt,
    ...
    static bool SendEventToIMGUIContainer(EventBase evt,
                                          BaseVisualElementPanel panel) {
      if (evt.imguiEvent == null)
        return false;

      // Root IMGUI is the container that handles all the GUIView/DockArea logic
      // for EditorWindows.
      ...
      // See case : https://fogbugz.unity3d.com/f/cases/1223087/
        ...
        if (!evt.Skip(rootIMGUI)) {
          bool canAffectFocus =
              evt.target is Focusable f && !f.focusable && f.isIMGUIContainer;
    ...
    static void SetBestTargetForEvent(EventBase evt,
                                      BaseVisualElementPanel panel) {
    ...
    static void UpdateElementUnderMouse(EventBase evt,
                                        BaseVisualElementPanel panel,
                                        out VisualElement elementUnderMouse) {
      bool shouldRecomputeTopElementUnderMouse =
          (evt as IMouseEventInternal) ?.recomputeTopElementUnderMouse ?? true;

      elementUnderMouse = shouldRecomputeTopElementUnderMouse
                            ? panel
                              .RecomputeTopElementUnderPointer(
                                  ((IMouseEvent) evt).mousePosition, evt)
          : panel.GetTopElementUnderPointer(PointerId.mousePointerId);

      // If mouse leaves the window, make sure element under mouse is null.
      // However, if pressed button != 0, we are getting a MouseLeaveWindowEvent
      // as part of of a drag and drop operation, at the very beginning of the
      // drag. Since we are not really exiting the window, we do not want to set
      // the element under mouse to null in this case.
