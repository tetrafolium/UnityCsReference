...
namespace UnityEngine.UIElements {
  /// <summary>
  /// Use this enum to specify during which phases the event handler is
  /// executed.
  /// </summary>
  public enum TrickleDown {
    /// <summary>
    /// The event handler should be executed during the AtTarget and BubbleUp
    /// phases.
    /// </summary>
    NoTrickleDown = 0,
    /// <summary>
  ...
  internal class EventCallbackList {
    ...
    public EventCallbackFunctorBase Find(long eventTypeId, Delegate callback,
      ...
      for (int i = 0; i < m_List.Count; i++) {
        if (m_List [i]
                .IsEquivalentTo(eventTypeId, callback, phase)) {
          return m_List[i];
        }
      }
      return null;
  ...
  internal class EventCallbackRegistry {
    private static readonly EventCallbackListPool s_ListPool =
        new EventCallbackListPool();

    private static EventCallbackList
    GetCallbackList(EventCallbackList initializer = null) {
    ...
    public EventCallbackRegistry() { m_IsInvoking = 0; }
    ...
    bool ShouldRegisterCallback(long eventTypeId, Delegate callback,
                                CallbackPhase phase) {
      if (callback == null) {
        return false;
      }
    ...
    bool UnregisterCallback(long eventTypeId, Delegate callback,
      ...
      var callbackPhase =
          useTrickleDown == TrickleDown.TrickleDown ? CallbackPhase
    ...
    public void RegisterCallback<TEventType>(
        ...
        TrickleDown useTrickleDown = TrickleDown.NoTrickleDown) where TEventType
      ...
      EventCallbackList callbackList = GetCallbackListForReading();
      if (callbackList == null ||
          callbackList.Contains(eventTypeId, callback, callbackPhase) ==
              false) {
        callbackList = GetCallbackListForWriting();
        callbackList.Add(
            new EventCallbackFunctor<TEventType>(callback, callbackPhase));
      }
    }

    public void RegisterCallback<TEventType, TCallbackArgs>(
        EventCallback<TEventType, TCallbackArgs> callback,
      ...
      EventCallbackList callbackList = GetCallbackListForReading();
      if (callbackList != null) {
        var functor = callbackList.Find(eventTypeId, callback, callbackPhase)
                          as EventCallbackFunctor<TEventType, TCallbackArgs>;
        if (functor != null) {
      ...
      callbackList = GetCallbackListForWriting();
      callbackList.Add(new EventCallbackFunctor<TEventType, TCallbackArgs>(
          callback, userArgs, callbackPhase));
    }

    public bool UnregisterCallback<TEventType>(
        EventCallback<TEventType> callback,
        TrickleDown useTrickleDown = TrickleDown.NoTrickleDown) where TEventType
        : EventBase<TEventType>,
          new () {
    ...
    internal bool TryGetUserArgs<TEventType, TCallbackArgs>(
        EventCallback<TEventType, TCallbackArgs> callback,
        TrickleDown useTrickleDown, out TCallbackArgs userArgs) where TEventType
        : EventBase<TEventType>,
          new () {
      ...
      var callbackPhase =
          useTrickleDown == TrickleDown.TrickleDown ? CallbackPhase
                                .TrickleDownAndTarget
          : CallbackPhase.TargetAndBubbleUp;
      var functor = list.Find(eventTypeId, callback, callbackPhase)
                        as EventCallbackFunctor<TEventType, TCallbackArgs>;
    ...
    public void InvokeCallbacks(EventBase evt) {
      ...
      if (m_IsInvoking == 0) {
        // If callbacks were modified during callback invocation, update them
        // now.
        if (m_TemporaryCallbacks != null) {
