namespace UnityEngine.UIElements {
  ...
  /// </summary>
  /// <remarks>
  /// >When an element receives an event, the event propagates from the panel's
  /// root element to the target element.
  ///
  /// In the TrickleDown phase, the event is sent from the panel's root element
  /// to the target element's parent.
  ///
  /// In the AtTarget phase, the event is sent to the target element.
  ///
  /// In the BubbleUp phase, the event is sent from the target element's parent
  /// back to the panel's root element.
  ///
  /// In the last phase, the DefaultAction phase, the event is resent to the
  /// target element.
  /// </remarks>
  public enum PropagationPhase {
    ...
    /// <summary>
    /// The event is sent from the panel's root element to the target element's
    /// parent.
    /// </summary>
    TrickleDown = 1,
    ...
    /// <summary>
    /// The event is sent to the target element, which can then execute its
    /// default actions for the event at the target phase. Event handlers do not
    /// receive the event in this phase. Instead, ExecuteDefaultActionAtTarget
    /// is called on the target element.
    /// </summary>
    DefaultActionAtTarget = 5,

    // After the target has gotten the chance to handle the event, the event
    // walks back up the parent hierarchy back to root.
    /// <summary>
    /// The event is sent from the target element's parent back to the panel's
    /// root element.
    /// </summary>
    BubbleUp = 3,
    ...
    /// <summary>
    /// The event is sent to the target element, which can then execute its
    /// final default actions for the event. Event handlers do not receive the
    /// event in this phase. Instead, ExecuteDefaultAction is called on the
    /// target element.
    /// </summary>
    DefaultAction = 4
  }
  ...
  static class EventDispatchUtilities {
    public static void PropagateEvent(EventBase evt) {
      ...
      if (evt.path == null) {
        ...
        // Call HandleEvent() even if propagation is stopped, for the default
        // actions at target.
    ...
    internal static void PropagateToIMGUIContainer(VisualElement root,
                                                   EventBase evt) {
      // We don't support IMGUIContainers in player
      if (evt.imguiEvent == null ||
          root.elementPanel.contextType == ContextType.Player) {
        return;
      }
      ...
      if (root.isIMGUIContainer) {
        ...
        if (evt.Skip(imContainer)) {
          // IMGUIContainer have no children. We can return without iterating
          // the children list.
          return;
        }

        // Only permit switching the focus to another IMGUIContainer if the
        // event target was not focusable.
