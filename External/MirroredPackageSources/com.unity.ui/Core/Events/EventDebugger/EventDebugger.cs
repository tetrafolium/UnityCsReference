...
namespace UnityEngine.UIElements {
  struct EventDebuggerLogCall : IDisposable {
    ...
    public void Dispose() {
      if (m_Event != null && m_Event.log) {
        ...
        m_Event.eventLogger.LogCall(
            GetCallbackHashCode(), GetCallbackName(), m_Event,
            m_IsPropagationStopped != m_Event.isPropagationStopped,
            m_IsImmediatePropagationStopped !=
                m_Event.isImmediatePropagationStopped,
    ...
    private string GetCallbackName() {
      ...
      if (m_Callback.Target != null) {
        return m_Callback.Target.GetType().FullName + "." +
               m_Callback.Method.Name;
      }

      if (m_Callback.Method.DeclaringType != null) {
        return m_Callback.Method.DeclaringType.FullName + "." +
               m_Callback.Method.Name;
      }
    ...
    private int GetCallbackHashCode() { return m_Callback?.GetHashCode() ?? 0; }
  }
  ...
  struct EventDebuggerLogIMGUICall : IDisposable {
    ...
    public void Dispose() {
      if (m_Event != null && m_Event.log) {
        ...
        IEventHandler capture =
            panel?.GetCapturingElement(PointerId.mousePointerId);
        m_Event.eventLogger.LogIMGUICall(
            m_Event, (long)(Time.realtimeSinceStartup * 1000.0f) - m_Start,
            capture);
      }
    }
  }
  ...
  struct EventDebuggerLogExecuteDefaultAction : IDisposable {
    ...
    public void Dispose() {
      if (m_Event != null && m_Event.log) {
        ...
        IEventHandler capture =
            panel?.GetCapturingElement(PointerId.mousePointerId);
        m_Event.eventLogger.LogExecuteDefaultAction(
            m_Event, m_Event.propagationPhase,
            (long)(Time.realtimeSinceStartup * 1000.0f) - m_Start, capture);
      }
    }
  }
  ...
  class EventDebugger {
    ...
    public void LogCall(int cbHashCode, string cbName, EventBase evt,
                        bool propagationHasStopped,
                        bool immediatePropagationHasStopped,
                        bool defaultHasBeenPrevented, long duration,
                        IEventHandler mouseCapture) {
      AddCallObject(cbHashCode, cbName, evt, propagationHasStopped,
                    immediatePropagationHasStopped, defaultHasBeenPrevented,
                    duration, mouseCapture);
      UpdateModificationCount();
    }

    public void LogIMGUICall(EventBase evt, long duration,
                             IEventHandler mouseCapture) {
      AddIMGUICall(evt, duration, mouseCapture);
      UpdateModificationCount();
    }

    public void LogExecuteDefaultAction(EventBase evt, PropagationPhase phase,
                                        long duration,
                                        IEventHandler mouseCapture) {
    ...
    public static void LogPropagationPaths(EventBase evt,
    ...
    void LogPropagationPathsInternal(EventBase evt, PropagationPaths paths) {
      var pathsCopy =
          paths == null ? new PropagationPaths() : new PropagationPaths(paths);
      AddPropagationPaths(evt, pathsCopy);
      UpdateModificationCount();
    }

    public List<EventDebuggerCallTrace>
    GetCalls(IPanel panel, EventDebuggerEventRecord evt = null) {
      ...
      if ((evt != null) && (list != null)) {
        List<EventDebuggerCallTrace> filteredList =
            new List<EventDebuggerCallTrace>();
    ...
    public List<EventDebuggerDefaultActionTrace>
    GetDefaultActions(IPanel panel, EventDebuggerEventRecord evt = null) {
      ...
      if ((evt != null) && (list != null)) {
        List<EventDebuggerDefaultActionTrace> filteredList =
            new List<EventDebuggerDefaultActionTrace>();
    ...
    GetPropagationPaths(IPanel panel, EventDebuggerEventRecord evt = null) {
      ...
      if ((evt != null) && (list != null)) {
        List<EventDebuggerPathTrace> filteredList =
            new List<EventDebuggerPathTrace>();
    ...
    public List<EventDebuggerTrace>
    GetBeginEndProcessedEvents(IPanel panel,
                               EventDebuggerEventRecord evt = null) {
    ...
    public void ReplayEvents(List<EventDebuggerEventRecord> eventBases) {
      ...
      foreach (var eventBase in eventBases) {
        ...
        if (eventBase.eventTypeId == MouseMoveEvent.TypeId() &&
            eventBase.hasUnderlyingPhysicalEvent) {
          newEvent.type = EventType.MouseMove;
          panel.dispatcher.Dispatch(
              UIElementsUtility.CreateEvent(newEvent, EventType.MouseMove),
              panel, DispatchMode.Default);
        } else if (eventBase.eventTypeId == MouseDownEvent.TypeId() &&
                   eventBase.hasUnderlyingPhysicalEvent) {
          newEvent.type = EventType.MouseDown;
          panel.dispatcher.Dispatch(
              UIElementsUtility.CreateEvent(newEvent, EventType.MouseDown),
              panel, DispatchMode.Default);
        } else if (eventBase.eventTypeId == MouseUpEvent.TypeId() &&
                   eventBase.hasUnderlyingPhysicalEvent) {
          newEvent.type = EventType.MouseUp;
          panel.dispatcher.Dispatch(
              UIElementsUtility.CreateEvent(newEvent, EventType.MouseUp), panel,
              DispatchMode.Default);
        } else if (eventBase.eventTypeId == ContextClickEvent.TypeId() &&
                   eventBase.hasUnderlyingPhysicalEvent) {
          newEvent.type = EventType.ContextClick;
          panel.dispatcher.Dispatch(
              UIElementsUtility.CreateEvent(newEvent, EventType.ContextClick),
              panel, DispatchMode.Default);
        } else if (eventBase.eventTypeId == MouseEnterWindowEvent.TypeId() &&
                   eventBase.hasUnderlyingPhysicalEvent) {
          newEvent.type = EventType.MouseEnterWindow;
          panel.dispatcher.Dispatch(UIElementsUtility.CreateEvent(
                                        newEvent, EventType.MouseEnterWindow),
                                    panel, DispatchMode.Default);
        } else if (eventBase.eventTypeId == MouseLeaveWindowEvent.TypeId() &&
                   eventBase.hasUnderlyingPhysicalEvent) {
          newEvent.type = EventType.MouseLeaveWindow;
          panel.dispatcher.Dispatch(UIElementsUtility.CreateEvent(
                                        newEvent, EventType.MouseLeaveWindow),
                                    panel, DispatchMode.Default);
          ...
          panel.dispatcher.Dispatch(
              UIElementsUtility.CreateEvent(newEvent, EventType.ScrollWheel),
              panel, DispatchMode.Default);
          ...
          panel.dispatcher.Dispatch(
              UIElementsUtility.CreateEvent(newEvent, EventType.KeyDown), panel,
              DispatchMode.Default);
          ...
          panel.dispatcher.Dispatch(UIElementsUtility.CreateEvent(
                                        newEvent, EventType.ValidateCommand),
                                    panel, DispatchMode.Default);
        } else if (eventBase.eventTypeId == ExecuteCommandEvent.TypeId()) {
          newEvent.type = EventType.ExecuteCommand;
          newEvent.commandName = eventBase.commandName;
          panel.dispatcher.Dispatch(
              UIElementsUtility.CreateEvent(newEvent, EventType.ExecuteCommand),
              panel, DispatchMode.Default);
        } else if (eventBase.eventTypeId == IMGUIEvent.TypeId()) {
          Debug.Log("Skipped IMGUI event (" + eventBase.eventBaseName +
                    "): " + eventBase);
          continue;
        } else {
          Debug.Log("Skipped event (" + eventBase.eventBaseName +
                    "): " + eventBase);
          continue;
        }

        Debug.Log("Replayed event (" + eventBase.eventBaseName +
                  "): " + newEvent);
      }
    }

    public Dictionary<string, long>
    ComputeHistogram(List<EventDebuggerEventRecord> eventBases) {
      ...
      foreach (var callObject in list) {
        if (eventBases == null || eventBases.Count == 0 ||
            eventBases.Contains(callObject.eventBase)) {
    ...
    Dictionary<IPanel, List<EventDebuggerDefaultActionTrace>>
        m_EventDefaultActionObjects;
    ...
    public EventDebugger() {
      m_EventCalledObjects =
          new Dictionary<IPanel, List<EventDebuggerCallTrace>>();
      m_EventDefaultActionObjects =
          new Dictionary<IPanel, List<EventDebuggerDefaultActionTrace>>();
      m_EventPathObjects =
          new Dictionary<IPanel, List<EventDebuggerPathTrace>>();
      m_StackOfProcessedEvent =
          new Dictionary<IPanel, Stack<EventDebuggerTrace>>();
      m_EventProcessedEvents =
          new Dictionary<IPanel, List<EventDebuggerTrace>>();
      m_ModificationCount = new Dictionary<IPanel, long>();
      m_Log = true;
    }

    void AddCallObject(int cbHashCode, string cbName, EventBase evt,
                       bool propagationHasStopped,
                       bool immediatePropagationHasStopped,
                       bool defaultHasBeenPrevented, long duration,
                       IEventHandler mouseCapture) {
      if (m_Log) {
        var callObject = new EventDebuggerCallTrace(
            panel, evt, cbHashCode, cbName, propagationHasStopped,
            immediatePropagationHasStopped, defaultHasBeenPrevented, duration,
            mouseCapture);
    ...
    void AddExecuteDefaultAction(EventBase evt, PropagationPhase phase,
                                 long duration, IEventHandler mouseCapture) {
      if (m_Log) {
        var defaultActionObject = new EventDebuggerDefaultActionTrace(
            panel, evt, phase, duration, mouseCapture);
    ...
    void AddIMGUICall(EventBase evt, long duration,
                      IEventHandler mouseCapture) {
      if (m_Log) {
        var callObject =
            new EventDebuggerCallTrace(panel, evt, 0, "OnGUI", false, false,
                                       false, duration, mouseCapture);
        List<EventDebuggerCallTrace> list;
    ...
    void AddEndProcessEvent(EventBase evt, long duration,
                            IEventHandler mouseCapture) {
      ...
      if (!evtHandled) {
        var dbgObject =
            new EventDebuggerTrace(panel, evt, duration, mouseCapture);
        List<EventDebuggerTrace> list;
        if (m_EventProcessedEvents.ContainsKey(panel)) {
          list = m_EventProcessedEvents[panel];
        } else {
