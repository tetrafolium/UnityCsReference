namespace UnityEngine.UIElements {
  class CommandEventDispatchingStrategy : IEventDispatchingStrategy {
    public bool CanDispatchEvent(EventBase evt) { return evt is ICommandEvent; }
        ...
        if (leafFocusElement != null) {
          if (leafFocusElement.isIMGUIContainer) {
            ...
            if (!evt.Skip(imguiContainer) &&
                imguiContainer.SendEventToIMGUI(evt)) {
              evt.StopPropagation();
              evt.PreventDefault();
            }
            ...
            if (!evt.isPropagationStopped && evt.propagateToIMGUI) {
              ...
              EventDispatchUtilities.PropagateToIMGUIContainer(panel.visualTree,
                                                               evt);
            }
          } else {
