...
internal static void SendEnterLeave<TLeaveEvent, TEnterEvent>(
	VisualElement previousTopElementUnderMouse,
	VisualElement currentTopElementUnderMouse, IMouseEvent triggerEvent,
	Vector2 mousePosition) where TLeaveEvent : MouseEventBase<TLeaveEvent>,
new () where TEnterEvent
: MouseEventBase<TEnterEvent>,
new () {
	if (previousTopElementUnderMouse != null &&
	    previousTopElementUnderMouse.panel == null) {
	...
	// We want to find the common ancestor CA of previousTopElementUnderMouse
	// and currentTopElementUnderMouse, send Leave (MouseLeave or DragLeave)
	// events to elements between CA and previousTopElementUnderMouse and send
	// Enter (MouseEnter or DragEnter) events to elements between CA and
	...
	while (prevDepth > currDepth) {
		using (var leaveEvent = MouseEventBase<TLeaveEvent>.GetPooled(
			       triggerEvent, mousePosition, false)) {
			leaveEvent.target = p;
	...
	List<VisualElement> enteringElements =
		VisualElementListPool.Get(currDepth);
	...
	for (var i = enteringElements.Count - 1; i >= 0; i--) {
		using (var enterEvent = MouseEventBase<TEnterEvent>.GetPooled(
			       triggerEvent, mousePosition, false)) {
...
SendMouseOverMouseOut(VisualElement previousTopElementUnderMouse,
                      VisualElement currentTopElementUnderMouse,
                      IMouseEvent triggerEvent, Vector2 mousePosition) {
	// Send MouseOut event for element no longer under the mouse.
	if (previousTopElementUnderMouse != null &&
	    previousTopElementUnderMouse.panel != null) {
		using (var outEvent =
			       MouseOutEvent.GetPooled(triggerEvent, mousePosition, false)) {
...
internal static void SendEnterLeave<TLeaveEvent, TEnterEvent>(
	...
	VisualElement currentTopElementUnderPointer, IPointerEvent triggerEvent,
	Vector2 position, int pointerId) where TLeaveEvent
: PointerEventBase<TLeaveEvent>,
new () where TEnterEvent : PointerEventBase<TEnterEvent>,
new () {
	if (previousTopElementUnderPointer != null &&
	    previousTopElementUnderPointer.panel == null) {
	...
	// We want to find the common ancestor CA of
	// previousTopElementUnderPointer and currentTopElementUnderPointer, send
	// Leave (PointerLeave) events to elements between CA and
	// previousTopElementUnderPointer and send Enter (PointerEnter) events to
	// elements between CA and currentTopElementUnderPointer.
	...
	while (prevDepth > currDepth) {
		using (var leaveEvent = PointerEventBase<TLeaveEvent>.GetPooled(
	...
	for (var i = enteringElements.Count - 1; i >= 0; i--) {
		using (var enterEvent = PointerEventBase<TEnterEvent>.GetPooled(
			       triggerEvent, position, pointerId)) {
			enterEvent.target = enteringElements[i];
			enteringElements [i]
			.SendEvent(enterEvent);
...
internal static void
SendOverOut(VisualElement previousTopElementUnderPointer,
            VisualElement currentTopElementUnderPointer,
            IPointerEvent triggerEvent, Vector2 position, int pointerId) {
	// Send PointerOutEvent for element no longer under the mouse.
	if (previousTopElementUnderPointer != null &&
	    previousTopElementUnderPointer.panel != null) {
		using (var outEvent = PointerOutEvent.GetPooled(triggerEvent, position,
		                                                pointerId)) {
	...
	if (currentTopElementUnderPointer != null) {
		using (var overEvent = PointerOverEvent.GetPooled(triggerEvent, position,
		                                                  pointerId)) {
