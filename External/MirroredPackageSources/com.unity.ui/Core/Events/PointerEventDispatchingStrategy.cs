namespace UnityEngine.UIElements {
  class PointerEventDispatchingStrategy : IEventDispatchingStrategy {
    ...
    static void SetBestTargetForEvent(EventBase evt, IPanel panel) {
      UpdateElementUnderPointer(evt, panel,
    ...
    static void
    UpdateElementUnderPointer(EventBase evt, IPanel panel,
                              out VisualElement elementUnderPointer) {
      ...
      bool shouldRecomputeTopElementUnderPointer =
          (evt as IPointerEventInternal)
          ?.recomputeTopElementUnderPointer ?? true;

      elementUnderPointer =
          shouldRecomputeTopElementUnderPointer
                              ? basePanel?
              .RecomputeTopElementUnderPointer(pointerEvent.position, evt)
