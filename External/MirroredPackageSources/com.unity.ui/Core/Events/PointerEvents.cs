...
namespace UnityEngine.UIElements {
  ...
  public static class PointerType {
    ...
    // A direct manipulation device is a device where the user directly
    // manipulates elements (like a touch screen), without any cursor acting as
    // an intermediate.
  ...
  public static class PointerId {
    ...
    /// </summary>
    /// <remarks>
    /// The pointer ID for a touch event is a number between touchPointerIdBase
    /// and touchPointerIdBase + touchPointerCount - 1.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// The pointer ID for a pen event is a number between penPointerIdBase and
    /// penPointerIdBase + penPointerCount - 1.
    /// </remarks>
    public static readonly int penPointerIdBase =
        touchPointerIdBase + touchPointerCount;
    /// <summary>
  ...
  public interface IPointerEvent {
    ...
    /// </summary>
    /// <remarks>
    /// If the mouse sends the event, this property is set to 0. If a
    /// touchscreen device sends the event, this property is set to the finger
    /// ID, which ranges from 1 to the number of touches the device supports.
    /// </remarks>
    int pointerId { get; }
    /// <summary>
    /// The type of pointer that created this event. This value is taken from
    /// the value defined in `PointerType`.
    /// </summary>
    ...
    /// </summary>
    /// <remarks>
    /// A primary pointer is a pointer that manipulates the mouse cursor. The
    /// mouse pointer is always a primary pointer. For touch events, the first
    /// finger that touches the screen is the primary pointer. Once processed,
    /// pointer events from primary pointers generate compatibility mouse
    /// events.
    /// </remarks>
    bool isPrimary { get; }
    /// <summary>
    /// Integer that indicates which mouse button is pressed: 0 is the left
    /// button, 1 is the right button, 2 is the middle button.
    /// </summary>
    int button { get; }
    /// <summary>
    ...
    /// </summary>
    /// <remarks>
    /// Pressing a mouse button sets a bit; releasing the button clears it. The
    /// left mouse button sets/clears Bit 0. The right mouse button sets/clears
    /// Bit 1. The middle mouse button sets/clears Bit 2. Additional buttons
    /// set/clear other bits.
    /// </remarks>
    ...
    /// <summary>
    /// The difference between the pointer's position during the previous mouse
    /// event and its position during the current mouse event.
    /// </summary>
    Vector3 deltaPosition { get; }
    /// <summary>
    /// The amount of time that has passed since the last recorded change in
    /// pointer values, in seconds.
    /// </summary>
    float deltaTime { get; }
    /// <summary>
    ...
    int clickCount { get; }
    /// <summary>
    /// The amount of pressure currently applied by a touch. If the device does
    /// not report pressure, the value of this property is 1.0f.
    /// </summary>
    float pressure { get; }
    /// <summary>
    ...
    /// </summary>
    /// <remarks>
    /// A value of 0 indicates that the stylus is parallel to the surface. A
    /// value of pi/2 indicates that it is perpendicular to the surface.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// A value of 0 indicates that the stylus is pointed along the x-axis of
    /// the device.
    /// </remarks>
    ...
    float twist { get; }
    /// <summary>
    /// An estimate of the radius of a touch. Add `radiusVariance` to get the
    /// maximum touch radius, subtract it to get the minimum touch radius.
    /// </summary>
    Vector2 radius { get; }
    /// <summary>
    /// Determines the accuracy of the touch radius. Add this value to the
    /// radius to get the maximum touch radius, subtract it to get the minimum
    /// touch radius.
    /// </summary>
    ...
    bool altKey { get; }
    /// <summary>
    /// Returns true if the platform-specific action key is pressed. This key is
    /// Cmd on macOS, and Ctrl on all other platforms.
    /// </summary>
    bool actionKey { get; }
  }
  ...
  /// </remarks>
  public abstract class PointerEventBase<T> : EventBase<T>,
                                              IPointerEvent,
                                              IPointerEventInternal where T
      : PointerEventBase<T>,
        new () {
    /// <summary>
    /// Identifies the pointer that sends the event.
    ...
    /// <summary>
    /// The current target of the event. The current target is the element in
    /// the propagation path for which event handlers are currently being
    /// executed.
    /// </summary>
    ...
    private static bool IsMouse(Event systemEvent) {
      ...
      return t == EventType.MouseMove || t == EventType.MouseDown ||
             t == EventType.MouseUp || t == EventType.MouseDrag ||
             t == EventType.ContextClick || t == EventType.MouseEnterWindow ||
             t == EventType.MouseLeaveWindow;
    }

    /// <summary>
    /// Gets an event from the event pool and initializes it with the given
    /// values. Use this function instead of creating new events. Events
    ...
    public static T GetPooled(Event systemEvent) {
      ...
      if (!(IsMouse(systemEvent) ||
            systemEvent.rawType == EventType.DragUpdated)) {
        Debug.Assert(false, "Unexpected event type: " + systemEvent.rawType +
                                " (" + systemEvent.type + ")");
      }
      ...
      if (systemEvent.rawType == EventType.MouseDown) {
        PointerDeviceState.PressButton(PointerId.mousePointerId,
                                       systemEvent.button);
        e.button = systemEvent.button;
      } else if (systemEvent.rawType == EventType.MouseUp) {
        PointerDeviceState.ReleaseButton(PointerId.mousePointerId,
                                         systemEvent.button);
        e.button = systemEvent.button;
    ...
    /// use `Dispose()` to release them.
    /// </summary>
    /// <param name="touch">A <see cref="Touch"/> structure from the
    /// InputManager.</param> <param name="modifiers">The modifier keys held
    /// down during the event.</param> <returns>An initialized event.</returns>
    public static T GetPooled(Touch touch,
      ...
      e.pressure =
          Mathf.Abs(touch.maximumPossiblePressure) >
          Mathf.Epsilon ? touch.pressure / touch.maximumPossiblePressure : 1f;
      e.tangentialPressure = 0;
      ...
      e.radiusVariance =
          new Vector2(touch.radiusVariance, touch.radiusVariance);
    ...
    internal static T GetPooled(IPointerEvent triggerEvent, Vector2 position,
                                int pointerId) {
    ...
    public static T GetPooled(IPointerEvent triggerEvent) {
      ...
      if (triggerEvent != null) {
        ...
        IPointerEventInternal pointerEventInternal =
            triggerEvent as IPointerEventInternal;
        if (pointerEventInternal != null) {
          ((IPointerEventInternal) e).triggeredByOS =
    ...
    protected internal override void PostDispatch(IPanel panel) {
      ...
      // If ShouldSendCompatibilityMouseEvents == true, mouse event will take
      // care of this.
      if (!panel.ShouldSendCompatibilityMouseEvents(this) &&
          ((IPointerEventInternal) this).triggeredByOS) {
    ...
    protected PointerEventBase() { LocalInit(); }
  }
  ...
  /// </summary>
  /// <remarks>
  /// PointerDownEvent is sent the first time a finger touches the screen or a
  /// mouse button is pressed. Touching the screen with more fingers or pressing
  /// additional buttons triggers PointerMoveEvents.
  /// </remarks>
  public sealed class PointerDownEvent : PointerEventBase<PointerDownEvent> {
    ...
    /// <summary>
    /// Constructor. Avoid creating new event instances. Instead, use
    /// GetPooled() to get an instance from a pool of reusable event instances.
    /// </summary>
    public PointerDownEvent() { LocalInit(); }
  ...
  public sealed class PointerMoveEvent : PointerEventBase<PointerMoveEvent> {
    ...
    /// GetPooled() to get an instance from a pool of reusable event instances.
    /// </summary>
    public PointerMoveEvent() { LocalInit(); }
        ...
        if (imguiEvent != null && imguiEvent.rawType == EventType.MouseDown) {
          using(var evt = MouseDownEvent.GetPooled(this)) {
            ...
            evt.target.SendEvent(evt);
                   ...
                   imguiEvent.rawType == EventType.MouseUp) {
  ...
  /// <summary>
  /// An event sent when a pointer does not change for a set amount of time
  /// determined by the operating system.
  /// </summary>
  public sealed class PointerStationaryEvent
      : PointerEventBase<PointerStationaryEvent> {
    /// <summary>
    /// Resets the event members to their initial values.
    /// </summary>
    protected override void Init() {
    ...
    /// GetPooled() to get an instance from a pool of reusable event instances.
    /// </summary>
    public PointerStationaryEvent() { LocalInit(); }
  }
  ...
  public sealed class PointerUpEvent : PointerEventBase<PointerUpEvent> {
    ...
    /// GetPooled() to get an instance from a pool of reusable event instances.
    /// </summary>
    public PointerUpEvent() { LocalInit(); }
  ...
  public sealed class PointerCancelEvent
      : PointerEventBase<PointerCancelEvent> {
    /// <summary>
    /// Resets the event members to their initial values.
    /// </summary>
    protected override void Init() {
    ...
    /// GetPooled() to get an instance from a pool of reusable event instances.
    /// </summary>
    public PointerCancelEvent() { LocalInit(); }
  ...
  /// same VisualElement. The mouse might move between the two events.
  /// </remarks>
  public sealed class ClickEvent : PointerEventBase<ClickEvent> {
    internal static ClickEvent GetPooled(PointerUpEvent pointerEvent,
                                         int clickCount) {
  ...
  public sealed class PointerEnterEvent : PointerEventBase<PointerEnterEvent> {
    ...
    void LocalInit() { propagation = EventPropagation.TricklesDown; }

    /// <summary>
    /// Constructor. Avoid creating new event instances. Instead, use
    /// GetPooled() to get an instance from a pool of reusable event instances.
    /// </summary>
    public PointerEnterEvent() { LocalInit(); }
  }
  ...
  public sealed class PointerLeaveEvent : PointerEventBase<PointerLeaveEvent> {
    ...
    /// GetPooled() to get an instance from a pool of reusable event instances.
    /// </summary>
    public PointerLeaveEvent() { LocalInit(); }
  }
