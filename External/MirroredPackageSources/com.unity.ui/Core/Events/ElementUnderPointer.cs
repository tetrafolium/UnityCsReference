...
VisualElement[] m_PendingTopElementUnderPointer =
	new VisualElement[PointerId.maxPointers];
VisualElement[] m_TopElementUnderPointer =
	new VisualElement[PointerId.maxPointers];
private IPointerEvent[] m_TriggerPointerEvent =
	new IPointerEvent[PointerId.maxPointers];
private IMouseEvent[] m_TriggerMouseEvent =
	new IMouseEvent[PointerId.maxPointers];
private Vector2[] m_PickingPointerPositions =
	new Vector2[PointerId.maxPointers];

// Some Events need to temporarily set the elementUnderPointer to a specific
// value to leave a predictable state for an expected Event chain. For
// example, MouseLeaveWindowEvent sets elementUnderPointer to null (see
// MouseEventDispatchingStrategy) to ensure it generates a proper
// MouseLeaveWindowEvent | MouseOutEvent event chain. Those temporary values
// should be overwritten by the next call to
// Panel.RecomputeTopElementUnderPointer().
private bool[] m_IsPickingPointerTemporaries =
	new bool[PointerId.maxPointers];

internal VisualElement GetTopElementUnderPointer(int pointerId,
                                                 out Vector2 pickPosition,
...
internal void SetElementUnderPointer(VisualElement newElementUnderPointer,
                                     int pointerId, Vector2 pointerPos) {
	Debug.Assert(pointerId >= 0);

	VisualElement previousTopElementUnderPointer =
		m_TopElementUnderPointer[pointerId];
...
internal void
SetTemporaryElementUnderPointer(VisualElement newElementUnderPointer,
                                EventBase triggerEvent) {
	SetElementUnderPointer(newElementUnderPointer, triggerEvent, temporary
	                       : true);
}
...
void SetElementUnderPointer(VisualElement newElementUnderPointer,
                            EventBase triggerEvent, bool temporary) {
	int pointerId = -1;
	...
	m_PickingPointerPositions [pointerId]
	        = GetEventPointerPosition(triggerEvent);
	...
	m_PendingTopElementUnderPointer[pointerId] = newElementUnderPointer;
	if (m_TriggerPointerEvent[pointerId] == null &&
	    triggerEvent is IPointerEvent) {
		m_TriggerPointerEvent[pointerId] = triggerEvent as IPointerEvent;
	}

	if (m_TriggerMouseEvent[pointerId] == null &&
	    triggerEvent is IMouseEvent) {
	...
	for (var i = 0; i < m_TopElementUnderPointer.Length; i++) {
		...
		if (triggerPointerEvent == null && m_TriggerMouseEvent[i] == null) {
			using (new EventDispatcherGate(dispatcher)) {
				...
				PointerEventsHelper.SendOverOut(previous, current, null, position,
				                                i);
				PointerEventsHelper
				.SendEnterLeave<PointerLeaveEvent, PointerEnterEvent>(
					previous, current, null, position, i);

				m_PickingPointerPositions[i] = position;
				if (i == PointerId.mousePointerId) {
					MouseEventsHelper.SendMouseOverMouseOut(previous, current, null,
					                                        position);
					MouseEventsHelper
					.SendEnterLeave<MouseLeaveEvent, MouseEnterEvent>(
		...
		if (triggerPointerEvent != null) {
			...
			if (baseEvent != null &&
				...
				using (new EventDispatcherGate(dispatcher)) {
					PointerEventsHelper.SendOverOut(previous, current,
					                                triggerPointerEvent, pos3d, i);
					PointerEventsHelper
					.SendEnterLeave<PointerLeaveEvent, PointerEnterEvent>(
		...
		if (triggerMouseEvent != null) {
			...
			if (baseEvent != null) {
				if (baseEvent.eventTypeId == MouseMoveEvent.TypeId() ||
					...
					using (new EventDispatcherGate(dispatcher)) {
						MouseEventsHelper.SendMouseOverMouseOut(
							previous, current, triggerMouseEvent, mousePos);
						MouseEventsHelper
						.SendEnterLeave<MouseLeaveEvent, MouseEnterEvent>(
							previous, current, triggerMouseEvent, mousePos);
					}
				} else if (baseEvent.eventTypeId ==
				           MouseEnterWindowEvent.TypeId() ||
				           baseEvent.eventTypeId ==
				           MouseLeaveWindowEvent.TypeId()) {
					using (new EventDispatcherGate(dispatcher)) {
						PointerEventsHelper.SendOverOut(previous, current, null,
						                                mousePos, i);
						PointerEventsHelper
						.SendEnterLeave<PointerLeaveEvent, PointerEnterEvent>(
					...
					using (new EventDispatcherGate(dispatcher)) {
						...
						.SendEnterLeave<MouseLeaveEvent, MouseEnterEvent>(
							previous, current, triggerMouseEvent, mousePos);
						MouseEventsHelper
						.SendEnterLeave<DragLeaveEvent, DragEnterEvent>(
							previous, current, triggerMouseEvent, mousePos);
					}
				}
			}
