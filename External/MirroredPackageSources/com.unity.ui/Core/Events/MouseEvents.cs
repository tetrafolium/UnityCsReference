namespace UnityEngine.UIElements {
  ...
  public interface IMouseEvent {
    /// <summary>
    /// Flag set holding the pressed modifier keys (Alt, Ctrl, Shift,
    /// Windows/Command).
    /// </summary>
  ...
  /// </summary>
  public abstract class MouseEventBase<T> : EventBase<T>,
                                            IMouseEvent,
                                            IMouseEventInternal where T
      : MouseEventBase<T>,
        new () {
    /// <summary>
    ...
    /// <summary>
    /// The difference of the mouse position between the previous mouse event
    ...
    protected internal override void PreDispatch(IPanel panel) {
      ...
      if (((IMouseEventInternal) this).triggeredByOS) {
        PointerDeviceState.SavePointerPosition(PointerId.mousePointerId,
                                               mousePosition, panel);
      }
    }

    protected internal override void PostDispatch(IPanel panel) {
      EventBase pointerEvent =
          ((IMouseEventInternal) this).sourcePointerEvent as EventBase;
      if (pointerEvent != null) {
        // pointerEvent processing should be done and it should not have
        // returned to the pool.
        Debug.Assert(pointerEvent.processed);

        (panel as BaseVisualElementPanel) ?.CommitElementUnderPointers();

        // We are a compatibility mouse event. Pass our status to the original
        // pointer event.
    ...
    public static T GetPooled(Event systemEvent) {
      ...
      if (systemEvent != null) {
        ...
        e.pressedButtons =
            PointerDeviceState.GetPressedButtons(PointerId.mousePointerId);
    ...
    /// <param name="clickCount">The number of consecutive mouse clicks
    /// received.</param> <param name="delta">The relative movement of the mouse
    /// compared to the mouse position when the last event was received.</param>
    /// <param name="modifiers">The modifier keys held down during the
    /// event.</param> <returns>An initialized event.</returns>
    public static T GetPooled(Vector2 position, int button, int clickCount,
                              Vector2 delta,
                              EventModifiers modifiers = EventModifiers.None) {
      return GetPooled(position, button, clickCount, delta, modifiers, false);
    }

    internal static T GetPooled(Vector2 position, int button, int clickCount,
                                Vector2 delta, EventModifiers modifiers,
                                bool fromOS) {
      T e = GetPooled();
    ...
    internal static T GetPooled(IMouseEvent triggerEvent, Vector2 mousePosition,
      ...
      ((IMouseEventInternal) e).recomputeTopElementUnderMouse =
          recomputeTopElementUnderMouse;
      return e;
    }
    ...
    public static T GetPooled(IMouseEvent triggerEvent) {
      ...
      if (triggerEvent != null) {
        ...
        IMouseEventInternal mouseEventInternal =
            triggerEvent as IMouseEventInternal;
        if (mouseEventInternal != null) {
          ((IMouseEventInternal) e).triggeredByOS =
    ...
    /// use `Dispose()` to release them.
    /// </summary>
    /// <param name="pointerEvent">The pointer event that sent this
    /// event.</param> <returns>An initialized event.</returns>
    protected static T GetPooled(IPointerEvent pointerEvent) {
      ...
      e.imguiEvent = (pointerEvent as EventBase) ?.imguiEvent;
        ...
        IPointerEventInternal pointerEventInternal =
            pointerEvent as IPointerEventInternal;
    ...
    protected MouseEventBase() { LocalInit(); }
  }
  ...
  public class MouseDownEvent : MouseEventBase<MouseDownEvent> {
    ...
    MakeFromPointerEvent(IPointerEvent pointerEvent) {
  ...
  public class WheelEvent : MouseEventBase<WheelEvent> {
    ...
    void LocalInit() { delta = Vector3.zero; }

    /// <summary>
    /// Constructor. Use GetPooled() to get an event from a pool of reusable
    /// events.
    /// </summary>
    public WheelEvent() { LocalInit(); }
  }

  /// <summary>
  /// Event sent when the mouse pointer enters an element or one of its
  /// descendent elements. The event is cancellable, it does not trickle down,
  /// and it does not bubble up.
  /// </summary>
  public class MouseEnterEvent : MouseEventBase<MouseEnterEvent> {
    ...
    void LocalInit() {
      propagation =
          EventPropagation.TricklesDown | EventPropagation.Cancellable;
    }
    ...
    /// </summary>
    public MouseEnterEvent() { LocalInit(); }
  }

  /// <summary>
  /// Event sent when the mouse pointer exits an element and all its descendent
  /// elements. The event is cancellable, it does not trickle down, and it does
  /// not bubble up.
  /// </summary>
  public class MouseLeaveEvent : MouseEventBase<MouseLeaveEvent> {
    ...
    /// GetPooled() to get an instance from a pool of reusable event instances.
    /// </summary>
    public MouseLeaveEvent() { LocalInit(); }
  }

  /// <summary>
  /// Event sent when the mouse pointer enters a window. The event is
  ...
  public class MouseEnterWindowEvent : MouseEventBase<MouseEnterWindowEvent> {
    ...
    void LocalInit() { propagation = EventPropagation.Cancellable; }

    /// <summary>
    /// Constructor. Avoid creating new event instances. Instead, use
    /// GetPooled() to get an instance from a pool of reusable event instances.
    /// </summary>
    public MouseEnterWindowEvent() { LocalInit(); }
  ...
  /// <summary>
  /// Event sent when the mouse pointer exits a window. The event is
  /// cancellable, it does not trickle down, and it does not bubble up.
  /// </summary>
  public class MouseLeaveWindowEvent : MouseEventBase<MouseLeaveWindowEvent> {
    ...
    /// GetPooled() to get an instance from a pool of reusable event instances.
    /// </summary>
    public MouseLeaveWindowEvent() { LocalInit(); }
  ...
  /// <summary>
  /// Event sent when the mouse pointer enters an element. The event trickles
  /// down, it bubbles up, and it is cancellable.
  /// </summary>
  public class MouseOverEvent : MouseEventBase<MouseOverEvent> {}

  /// <summary>
  /// Event sent when the mouse pointer exits an element. The event trickles
  /// down, it bubbles up, and it is cancellable.
  /// </summary>
  ...
  public class ContextualMenuPopulateEvent
      : MouseEventBase<ContextualMenuPopulateEvent> {
    /// <summary>
    ...
    /// use `Dispose()` to release them.
    /// </summary>
    /// <param name="triggerEvent">The event that triggered the display of the
    /// contextual menu.</param> <param name="menu">The menu to
    /// populate.</param> <param name="target">The element that triggered the
    /// display of the contextual menu.</param> <param name="menuManager">The
    /// menu manager that displays the menu.</param> <returns>An initialized
    /// event.</returns>
    public static ContextualMenuPopulateEvent
    GetPooled(EventBase triggerEvent, DropdownMenu menu, IEventHandler target,
              ContextualMenuManager menuManager) {
    ...
    /// Constructor.
    /// </summary>
    public ContextualMenuPopulateEvent() { LocalInit(); }
