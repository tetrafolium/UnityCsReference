...
/// <summary>
/// Instantiates a <see cref="TextElement"/> using the data read from a UXML
/// file.
/// </summary>
...
/// <summary>
/// Enumerator to get the child elements of the <see cref="UxmlTraits"/>
/// of <see cref="TextElement"/>.
/// </summary>
...
/// <summary>
/// Initializer for the <see cref="UxmlTraits"/> for the <see
/// cref="TextElement"/>.
/// </summary>
/// <param name="ve"><see cref="VisualElement"/> to initialize.</param>
/// <param name="bag">Bag of attributes where to get the value
/// from.</param> <param name="cc">Creation Context, not used.</param>
...
/// </summary>
/// <remarks>
/// Text is elided when the element that contains it is not large enough to
/// display the full text, and has the following style property settings.
///
...
///
/// The text Element hides elided text, and displays an ellipsis ('...') to
/// indicate that there is hidden overflow content.
/// </remarks>
...
internal static readonly string k_EllipsisText =
	@"..."; // Some web standards seem to suggest "\u2026" (horizontal
...
internal string ElideText(string drawText, string ellipsisText, float width,
                          TextOverflowPosition textOverflowPosition) {
	// Try full size first
	var size = MeasureTextSize(drawText, 0, MeasureMode.Undefined, 0,
	                           MeasureMode.Undefined);
	if (size.x <= width || string.IsNullOrEmpty(ellipsisText))
		return drawText;

	var minText = drawText.Length > 1 ? ellipsisText : drawText;
	var minSize = MeasureTextSize(minText, 0, MeasureMode.Undefined, 0,
	                              MeasureMode.Undefined);
	if (minSize.x >= width)
		return minText;
	...
	// Don't assume that k_EllipsisText takes as much space as any other
	// string of the same length; we will start by removing one character at a
	// time
	var min = textOverflowPosition == TextOverflowPosition.Start ? 1 : 0;
	var max = (textOverflowPosition == TextOverflowPosition.Start ||
	           textOverflowPosition == TextOverflowPosition.Middle)
	            ? drawTextMax
	            : drawTextMax - 1;
	var mid = (min + max) / 2;
	...
	while (min <= max) {
		if (textOverflowPosition == TextOverflowPosition.Start)
			truncatedText =
				ellipsisText + drawText.Substring(mid, drawTextMax - (mid - 1));
		...
		size = MeasureTextSize(truncatedText, 0, MeasureMode.Undefined, 0,
		                       MeasureMode.Undefined);
		...
		if (textOverflowPosition == TextOverflowPosition.Start) {
			if (size.x > width) {
				if (prevFitMid == mid - 1)
					return ellipsisText +
					       drawText.Substring(prevFitMid,
					                          drawTextMax - (prevFitMid - 1));
				min = mid + 1;
			} else {
				max = mid - 1;
				prevFitMid = mid;
			...
			if (size.x > width) {
				if (prevFitMid == mid - 1)
					if (textOverflowPosition == TextOverflowPosition.End)
						...
						return drawText.Substring(0, prevFitMid - 1) + ellipsisText +
						       drawText.Substring(drawTextMax - (prevFitMid - 1));
				max = mid - 1;
			} else {
				min = mid + 1;
				prevFitMid = mid;
...
private void UpdateVisibleText() {
	...
	if (m_UpdateTextParams ||
	    textParamsHashCode != m_PreviousTextParamsHashCode) {
		m_TextParams = textParams;
		if (m_TextParams.textOverflowMode == TextOverflowMode.Ellipsis)
			m_TextParams.text = ElideText(m_TextParams.text, k_EllipsisText,
			                              m_TextParams.rect.width,
			                              m_TextParams.textOverflowPosition);

		isElided = m_TextParams.textOverflowMode == TextOverflowMode.Ellipsis &&
...
/// <returns>The horizontal and vertical size needed to display the text
/// string.</returns>
public Vector2 MeasureTextSize(string textToMeasure, float width,
                               MeasureMode widthMode, float height,
                               MeasureMode heightMode) {
	return MeasureVisualElementTextSize(this, textToMeasure, width, widthMode,
	                                    height, heightMode, m_TextHandle);
}

internal static Vector2
MeasureVisualElementTextSize(VisualElement ve, string textToMeasure,
	...
	if (elementScaling.x + elementScaling.y <= 0 ||
	    ve.scaledPixelsPerPoint <= 0)
		return Vector2.zero;
	...
	if (widthMode == MeasureMode.Exactly) {
		...
		// Case 1215962: round up as yoga could decide to round down and text
		// would start wrapping
		measuredWidth = textHandle.ComputeTextWidth(textParams, pixelsPerPoint);
		measuredWidth = measuredWidth < pointOffset ? 0
	    : AlignmentUtils.CeilToPixelGrid(measuredWidth, pixelsPerPoint,
		                             pixelOffset);
	...
	if (heightMode == MeasureMode.Exactly) {
		...
		measuredHeight =
			textHandle.ComputeTextHeight(textParams, pixelsPerPoint);
		measuredHeight = measuredHeight < pointOffset ? 0
	    : AlignmentUtils.CeilToPixelGrid(measuredHeight, pixelsPerPoint,
		                             pixelOffset);
...
protected internal override Vector2 DoMeasure(float desiredWidth,
                                              ...
                                              float desiredHeight,
                                              MeasureMode heightMode) {
	return MeasureTextSize(text, desiredWidth, widthMode, desiredHeight,
	                       heightMode);
}

private static MeshGenerationContextUtils.TextParams
GetTextSettings(VisualElement ve, string text) {
	...
	return new MeshGenerationContextUtils.TextParams {
		       ...
		       wordWrapWidth = style.whiteSpace.value ==
		                       WhiteSpace.Normal ? ve.contentRect.width : 0.0f,
		       richText = true,
		       textOverflowMode =
			       MeshGenerationContextUtils.TextParams.GetTextOverflowMode(style),
...
string INotifyValueChanged<string>.value {
	...
	set {
		if (m_Text != value) {
			if (panel != null) {
				using (ChangeEvent<string> evt =
					       ChangeEvent<string>.GetPooled(this.text, value)) {
					evt.target = this;
