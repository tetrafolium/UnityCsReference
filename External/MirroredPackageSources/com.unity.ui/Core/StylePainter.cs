...
namespace UnityEngine.UIElements {
  internal interface IStylePainter {
    MeshWriteData DrawMesh(int vertexCount, int indexCount, Texture texture,
                           Material material,
  ...
  internal struct CursorPositionStylePainterParameters {
    ...
    public static CursorPositionStylePainterParameters
    GetDefault(VisualElement ve, string text) {
    ...
    internal TextNativeSettings GetTextNativeSettings(float scaling) {
      return new TextNativeSettings{
          ...
          scaling = scaling,   style = fontStyle,
          color = Color.white, // N/A
          anchor = anchor,     wordWrap = true,   wordWrapWidth = wordWrapWidth,
          richText = richText};
    }
  }
}
