...
namespace UnityEngine.UIElements {
  ...
  /// </summary>
  /// <remarks>
  /// The focus is used to designate an element that will receive keyboard
  /// events.
  /// </remarks>
  public abstract class Focusable : CallbackEventHandler {
    ...
    /// <summary>
    /// An integer used to sort focusables in the focus ring. Must be greater
      ...
      set {
        if (!((VisualElement) this).isCompositeRoot) {
          throw new InvalidOperationException(
              "delegatesFocus should only be set on composite roots.");
        }
    ...
    internal bool excludeFromFocusRing {
      ...
      set {
        if (!((VisualElement) this).isCompositeRoot) {
          throw new InvalidOperationException(
              "excludeFromFocusRing should only be set on composite roots.");
        }
    ...
    static Focusable GetFirstFocusableChild(VisualElement ve) {
      foreach (var child in ve.hierarchy.Children()) {
        ...
        bool isSlot = child.hierarchy.parent != null &&
                      child == child.hierarchy.parent.contentContainer;
  ...
  /// <summary>
  /// Base class for defining in which direction the focus moves in a focus
  /// ring.
  /// </summary>
  /// <remarks>
  /// Focus ring implementations can move the focus in various direction; they
  /// can derive from this class to formalize the various ways the focus can
  /// change from one element to the other.
  /// </remarks>
  public class FocusChangeDirection {
    /// <summary>
    /// Focus came from an unspecified direction, for example after a mouse
    /// down.
    /// </summary>
    ...
    /// <summary>
    /// The null direction. This is usually used when the focus stays on the
    /// same element.
    /// </summary>
    ...
    protected FocusChangeDirection(int value) { m_Value = value; }
  ...
  public interface IFocusRing {
    /// <summary>
    /// Get the direction of the focus change for the given event. For example,
    /// when the Tab key is pressed, focus should be given to the element to the
    /// right.
    /// </summary>
    FocusChangeDirection GetFocusChangeDirection(Focusable currentFocusable,
                                                 EventBase e);

    /// <summary>
    /// Get the next element in the given direction.
    /// </summary>
    Focusable GetNextFocusable(Focusable currentFocusable,
  ...
  /// </summary>
  /// <remarks>
  /// Each Panel should have an instance of this class. The instance holds the
  /// currently focused VisualElement and is responsible for changing it.
  /// </remarks>
  public class FocusController {
    ...
    internal
      ...
      if (retargetRoot == null) {
        if (m_FocusedElements.Count > 0) {
          return m_FocusedElements[m_FocusedElements.Count - 1]
              .m_FocusedElement;
        }
      } else {
        while (!retargetRoot.isCompositeRoot &&
               retargetRoot.hierarchy.parent != null) {
    ...
    internal void SetFocusToLastFocusedElement() {
      if (m_LastFocusedElement != null &&
          !(m_LastFocusedElement is IMGUIContainer))
    ...
    internal void DoFocusChange(Focusable f) {
      ...
      while (ve != null) {
        if (ve.hierarchy.parent == null || ve.isCompositeRoot) {
          m_FocusedElements.Add(
              new FocusedElement{m_SubTreeRoot = ve, m_FocusedElement = f});
          f = ve;
        }
    ...
    void AboutToReleaseFocus(Focusable focusable, Focusable willGiveFocusTo,
                             FocusChangeDirection direction) {
      using(FocusOutEvent e = FocusOutEvent.GetPooled(
                focusable, willGiveFocusTo, direction, this)) {
        focusable.SendEvent(e);
      }
    }

    void ReleaseFocus(Focusable focusable, Focusable willGiveFocusTo,
                      FocusChangeDirection direction) {
      using(BlurEvent e = BlurEvent.GetPooled(focusable, willGiveFocusTo,
                                              direction, this)) {
        focusable.SendEvent(e);
      }
    }

    void AboutToGrabFocus(Focusable focusable, Focusable willTakeFocusFrom,
                          FocusChangeDirection direction) {
      using(FocusInEvent e = FocusInEvent.GetPooled(
                focusable, willTakeFocusFrom, direction, this)) {
        focusable.SendEvent(e);
      }
    }

    void GrabFocus(Focusable focusable, Focusable willTakeFocusFrom,
                   FocusChangeDirection direction,
                   bool bIsFocusDelegated = false) {
      using(FocusEvent e =
                FocusEvent.GetPooled(focusable, willTakeFocusFrom, direction,
                                     this, bIsFocusDelegated)) {
        focusable.SendEvent(e);
      }
    }
    ...
    void SwitchFocus(Focusable newFocusedElement,
                     FocusChangeDirection direction,
                     bool bIsFocusDelegated = false) {
      ...
      if (newFocusedElement == null || !newFocusedElement.canGrabFocus) {
        ...
        // Retarget event.relatedTarget so it is in the same tree as
        // event.target.
        var retargetedNewFocusedElement = (newFocusedElement as VisualElement)
            ?.RetargetElement(oldFocusedElement as VisualElement);
        var retargetedOldFocusedElement = (oldFocusedElement as VisualElement)
            ?.RetargetElement(newFocusedElement as VisualElement);

        if (oldFocusedElement != null) {
          AboutToReleaseFocus(oldFocusedElement, retargetedNewFocusedElement,
                              direction);
        }

        AboutToGrabFocus(newFocusedElement, retargetedOldFocusedElement,
                         direction);

        if (oldFocusedElement != null) {
          ReleaseFocus(oldFocusedElement, retargetedNewFocusedElement,
                       direction);
        }
    ...
    internal Focusable SwitchFocusOnEvent(EventBase e) {
      FocusChangeDirection direction =
          focusRing.GetFocusChangeDirection(GetLeafFocusedElement(), e);
      if (direction != FocusChangeDirection.none) {
        Focusable f =
            focusRing.GetNextFocusable(GetLeafFocusedElement(), direction);
        SwitchFocus(f, direction);
        // f does not have the focus yet. It will when the series of focus
        // events will have been handled.
        return f;
      }
    ...
    /// <summary>
    /// This property contains the actual keyboard id of the element being
    /// focused in the case of an IMGUIContainer
    /// </summary>
    ...
    internal void SyncIMGUIFocus(int imguiKeyboardControlID,
                                 Focusable imguiContainerHavingKeyboardControl,
                                 bool forceSwitch) {
      imguiKeyboardControl = imguiKeyboardControlID;

      if (forceSwitch || imguiKeyboardControl != 0) {
        SwitchFocus(imguiContainerHavingKeyboardControl,
