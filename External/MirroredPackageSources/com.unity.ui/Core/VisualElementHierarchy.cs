...
namespace UnityEngine.UIElements {
  ...
  public partial class VisualElement {
    ...
    public void Add(VisualElement child) {
      ...
      if (container == null) {
        throw new InvalidOperationException(
            "You can't add directly to this VisualElement. Use hierarchy.Add() if you know what you're doing.");
    ...
    /// </summary>
    public VisualElement ElementAt(int index) { return this[index]; }
    ...
    /// <summary>
    /// Brings this element to the end of its parent children list. The element
    /// will be visually in front of any overlapping sibling elements.
    /// </summary>
    ...
    /// children list. If the element and the sibling position overlap, the
    /// element will be visually behind of its sibling.
    /// </summary>
    ...
    public void PlaceBehind(VisualElement sibling) {
      ...
      if (hierarchy.parent == null ||
          sibling.hierarchy.parent != hierarchy.parent) {
    ...
    /// <summary>
    /// Places this element right after the sibling element in their parent
    /// children list. If the element and the sibling position overlap, the
    /// element will be visually in front of its sibling.
    /// </summary>
    /// <param name="sibling">The sibling element.</param>
    /// <remarks>
    /// The elements must be siblings.
    ...
    /// <summary>
    /// Hierarchy is a struct allowing access to the hierarchy of visual
    /// elements
    /// </summary>
    public struct Hierarchy {
      ...
      internal Hierarchy(VisualElement element) { m_Owner = element; }
      ...
      public void Insert(int index, VisualElement child) {
        ...
        if (ReferenceEquals(m_Owner.m_Children, s_EmptyList)) {
          // TODO: Trigger a release on finalizer or something, this means we'll
          // need to make the pool thread-safe as well
        ...
        int imguiContainerCount = child.imguiContainerDescendantCount +
                                  (child.isIMGUIContainer ? 1 : 0);
      ...
      public void RemoveAt(int index) {
        ...
        m_Owner.elementPanel?.OnVersionChanged(child,
      ...
      public void Clear() {
        if (childCount > 0) {
          // Copy children to a temporary list because removing child elements
          // from the panel may trigger modifications (DetachFromPanelEvent
          // callback) of the same list while we are in the foreach loop.
          ...
          foreach (VisualElement e in elements) {
            ...
            m_Owner.elementPanel?.OnVersionChanged(e,
                                                   VersionChangeType.Hierarchy);
          }
      ...
      private void MoveChildElement(VisualElement child, int currentIndex,
                                    int nextIndex) {
        child.InvokeHierarchyChanged(HierarchyChangeType.Remove);
      ...
      /// </summary>
      public int childCount {
        get { return m_Owner.m_Children.Count; }
      }

      public VisualElement this[int key] {
        get { return m_Owner.m_Children[key]; }
      }
      ...
      /// <returns>The index of the element, or -1 if the element is not
      /// found.</returns>
      public int IndexOf(VisualElement element) {
      ...
      public bool Equals(Hierarchy other) { return other == this; }
    ...
    /// <summary>
    /// Walks up the hierarchy, starting from this element, and returns the
    ...
    /// the first VisualElement of this type
    /// </summary>
    ...
    /// <summary>
    /// Finds the lowest commont ancestor between two VisualElements inside the
    /// VisualTree hierarchy
    /// </summary>
    public VisualElement FindCommonAncestor(VisualElement other) {
      ...
      // Now both are at the same depth, We then walk up the tree we hit the
      // same element
    ...
    internal VisualElement RetargetElement(VisualElement retargetAgainst) {
      ...
      // If retargetAgainst.isCompositeRoot is true, we want to retarget THIS to
      // the tree that holds retargetAgainst, not against the tree rooted by
      // retargetAgainst. In this case we start by setting retargetRoot to
      // retargetAgainst.m_PhysicalParent. However, if
      // retargetAgainst.m_PhysicalParent == null, we are at the top of the main
      // tree, so retargetRoot should be retargetAgainst.
      var retargetRoot = retargetAgainst.m_PhysicalParent ?? retargetAgainst;
      while (retargetRoot.m_PhysicalParent != null &&
             !retargetRoot.isCompositeRoot) {
