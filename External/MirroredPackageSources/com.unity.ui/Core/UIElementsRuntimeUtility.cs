...
namespace UnityEngine.UIElements {
  internal static class UIElementsRuntimeUtility {
    ...
    static UIElementsRuntimeUtility() {
      UIElementsRuntimeUtilityNative.RepaintOverlayPanelsCallback =
          RepaintOverlayPanels;
    }

    public static EventBase CreateEvent(Event systemEvent) {
      Debug.Assert(
          s_RuntimeDispatcher != null,
          "Call UIElementsRuntimeUtility.InitRuntimeEventSystem before sending any event.");
    ...
    public static IPanel
    FindOrCreateRuntimePanel(ScriptableObject ownerObject) {
      Panel panel;
      if (!UIElementsUtility.TryGetPanel(ownerObject.GetInstanceID(),
                                         out panel)) {
        ...
        Debug.Assert(ContextType.Player == panel.contextType,
                     "Panel is not a runtime panel.");
      }
    ...
    private static void RegisterCachedPanelInternal(int instanceID,
                                                    IPanel panel) {
    ...
    private static void RemoveCachedPanelInternal(int instanceID) {
      ...
      // un-register the playerloop callback as the last panel gets
      // un-registered
    ...
    internal static readonly string s_RepaintProfilerMarkerName =
        "UIElementsRuntimeUtility.DoDispatch(Repaint Event)";
    ...
    public static void RepaintOverlayPanels() {
      ...
      foreach (RuntimePanel panel in panelsIteration) {
        if (!panel.drawToCameras && panel.targetTexture == null) {
          using(s_RepaintProfilerMarker.Auto()) panel.Repaint(Event.current);
