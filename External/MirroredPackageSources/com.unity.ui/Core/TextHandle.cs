...
namespace UnityEngine.UIElements {
  internal struct TextHandle {
    ...
    // At the moment, FontAssets are not part of Styles. They're created on
    // runtime and cached
    static Dictionary<Font, FontAsset> fontAssetCache =
        new Dictionary<Font, FontAsset>();
    ...
    static FontAsset GetFontAsset(Font font) {
      ...
      if (fontAssetCache.TryGetValue(font, out fontAsset) && fontAsset != null)
        return fontAsset;
      fontAsset =
          FontAsset.CreateFontAsset(font, 90, 9, GlyphRenderMode.SDFAA, 1024,
    ...
    public Vector2 GetCursorPosition(CursorPositionStylePainterParameters parms,
                                     float scaling) {
      if (useLegacy)
        return TextNative.GetCursorPosition(
            parms.GetTextNativeSettings(scaling), parms.rect,
            parms.cursorIndex);
      return TextCore.TextGenerator.GetCursorPosition(m_TextInfo, parms.rect,
                                                      parms.cursorIndex);
    }

    public float ComputeTextWidth(MeshGenerationContextUtils.TextParams parms,
                                  float scaling) {
      if (useLegacy)
    ...
    public float ComputeTextHeight(MeshGenerationContextUtils.TextParams parms,
                                   float scaling) {
      if (useLegacy)
    ...
    internal TextInfo Update(MeshGenerationContextUtils.TextParams parms,
      ...
      // The screenRect in TextCore is not properly implemented with regards to
      // the offset part, so zero it out for now and we will add it ourselves
      // later
      ...
      TextCore.TextGenerator.GenerateText(m_CurrentGenerationSettings,
                                          m_TextInfo);
    ...
    void UpdatePreferredValues(MeshGenerationContextUtils.TextParams parms) {
      ...
      m_PreferredSize = TextCore.TextGenerator.GetPreferredValues(
          m_CurrentLayoutSettings, m_TextInfo);
      m_PreviousLayoutSettingsHash = paramsHash;
    }

    static void UpdateGenerationSettingsCommon(
        MeshGenerationContextUtils.TextParams painterParams,
      ...
      // in case rect is not properly set (ex: style has not been resolved),
      // make sure its width at least matches wordWrapWidth
      ...
      settings.text =
          string.IsNullOrEmpty(painterParams.text) ? " " : painterParams.text;
      settings.fontSize = painterParams.fontSize > 0
                              ? painterParams.fontSize
                              : painterParams.font.fontSize;
      settings.fontStyle =
          TextGeneratorUtilities.LegacyStyleToNewStyle(painterParams.fontStyle);
      settings.textAlignment =
          TextGeneratorUtilities.LegacyAlignmentToNewAlignment(
              painterParams.anchor);
    ...
    public static float ComputeTextScaling(Matrix4x4 worldMatrix,
                                           float pixelsPerPoint) {
      var axisX =
          new Vector3(worldMatrix.m00, worldMatrix.m10, worldMatrix.m20);
      var axisY =
          new Vector3(worldMatrix.m01, worldMatrix.m11, worldMatrix.m21);
