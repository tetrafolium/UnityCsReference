...
namespace UnityEngine.UIElements {
  ...
  ///         clickable.activators.Add(new ManipulatorActivationFilter { button
  ///         = MouseButton.LeftMouse }); clickable.activators.Add(new
  ///         ManipulatorActivationFilter { button = MouseButton.RightMouse,
  ...
  public struct ManipulatorActivationFilter
      : IEquatable<ManipulatorActivationFilter> {
    /// <summary>
    ...
    /// <summary>
    /// Any modifier keys (ie. ctrl, alt, ...) that are needed to activate the
    /// manipulation.
    /// </summary>
    ...
    public override bool Equals(object obj) {
      return obj is ManipulatorActivationFilter &&
             Equals((ManipulatorActivationFilter) obj);
    }
    ...
    /// <summary>
    /// Checks whether the current mouse event satisfies the activation
    /// requirements.
    /// </summary>
    /// <param name="e">The mouse event.</param>
    /// <returns>True if the event matches the requirements. False
    /// otherwise.</returns>
    public bool Matches(IMouseEvent e) {
      if (e == null) {
        return false;
      }
      // Default clickCount field value is 0 since we're in a struct -- this
      // case is covered if the user did not explicitly set clickCount
    ...
    public static bool operator ==(ManipulatorActivationFilter filter1,
                                   ManipulatorActivationFilter filter2) {
