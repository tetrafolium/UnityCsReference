...
namespace UnityEngine.UIElements {
  internal struct StyleVariable {
    ...
    public StyleVariable(string name, StyleSheet sheet,
  ...
  internal class StyleVariableContext {
    ...
    public void Add(StyleVariable sv) {
      ...
      unchecked {
        m_VariableHash = m_Variables.Count == 0
                             ? sv.GetHashCode()
                             : (m_VariableHash * 397) ^ sv.GetHashCode();
      }
    }
    ...
    public int GetVariableHash() { return m_VariableHash; }
  }

  internal class StyleVariableResolver {
    public enum Result { Valid, Invalid, NotFound }

    // Max resolves is to protect against long variables :
    ...
    public void Init(StyleProperty property, StyleSheet sheet,
                     StyleValueHandle[] handles) {
    ...
    public void AddValue(StyleValueHandle handle) {
      m_ResolvedValues.Add(
          new StylePropertyValue(){sheet = m_Sheet, handle = handle});
    }
    ...
    public Result ResolveVarFunction(ref int index) {
      ...
      if (result != Result.Valid) {
        ...
        if (result == Result.NotFound && argc > 1 &&
            !m_Property.isCustomProperty) {
          var h = m_Handles[++index];
          Debug.Assert(h.valueType == StyleValueType.FunctionSeparator,
                       $"Unexpected value type {h.valueType} in var function");
          if (h.valueType == StyleValueType.FunctionSeparator &&
              index + 1 < m_Handles.Length) {
            ++index;
    ...
    private Result ValidateResolve(StylePropertyValue spv) {
      ...
      // Cannot validate custom property before it's assigned so it's always
      // true in this case
      if (m_Property.isCustomProperty)
        return Result.Valid;
    ...
    private Result ResolveFallback(ref int index) {
      ...
      for (; index < m_Handles.Length && result == Result.Valid; ++index) {
        ...
        if (h.IsVarFunction()) {
          int argc;
          string varName;
          ParseVarFunction(m_Sheet, m_Handles, ref index, out argc,
                           out varName);
          result = ResolveVariable(varName);
          ...
          if (result == Result.NotFound) {
            ...
            if (argc > 1) {
              h = m_Handles[++index];
              Debug.Assert(
                  h.valueType == StyleValueType.FunctionSeparator,
                  $"Unexpected value type {h.valueType} in var function");
              if (h.valueType == StyleValueType.FunctionSeparator &&
                  index + 1 < m_Handles.Length) {
                ++index;
    ...
    private static void ParseVarFunction(StyleSheet sheet,
                                         StyleValueHandle[] handles,
                                         ref int index, out int argCount,
