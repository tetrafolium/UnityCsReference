...
using TableType =
    System.Collections.Generic
        .Dictionary<string, UnityEngine.UIElements.StyleComplexSelector>;
using UnityEngine.UIElements.StyleSheets;

namespace UnityEngine.UIElements {
  /// <summary>
  /// Style sheets are applied to visual elements in order to control the layout
  /// and visual appearance of the user interface.
  /// </summary>
  /// <remarks>
  /// The <see cref="StyleSheet"/> class holds the imported data of USS files in
  /// your project. Once loaded, a style sheet can be attached to a <see
  /// cref="VisualElement"/> object to affect the element itself and its
  /// descendants.
  /// </remarks>
  /// <example>
  /// <code>
  ...
  public class StyleSheet : ScriptableObject {
    [SerializeField] StyleRule[] m_Rules;
    ...
    [SerializeField] StyleComplexSelector[] m_ComplexSelectors;
    ...
    static bool TryCheckAccess<T>(T[] list, StyleValueType type,
                                  StyleValueHandle handle, out T value) {
      bool result = false;
      value = default(T);

      if (handle.valueType == type && handle.valueIndex >= 0 &&
          handle.valueIndex < list.Length) {
        value = list[handle.valueIndex];
        result = true;
      } else {
        Debug.LogErrorFormat(
            "Trying to read value of type {0} while reading a value of type {1}",
    ...
    static T CheckAccess<T>(T[] list, StyleValueType type,
      ...
      if (handle.valueType != type) {
        Debug.LogErrorFormat(
            "Trying to read value of type {0} while reading a value of type {1}",
            type, handle.valueType);
      } else if (list == null || handle.valueIndex < 0 ||
                 handle.valueIndex >= list.Length) {
    ...
    void SetupReferences() {
      ...
      for (int i = 0; i < complexSelectors.Length; i++) {
        ...
        StyleSelector lastSelector =
            complexSel.selectors[complexSel.selectors.Length - 1];
    ...
    internal float ReadFloat(StyleValueHandle handle) {
      ...
      if (handle.valueType == StyleValueType.Dimension) {
        Dimension dimension =
            CheckAccess(dimensions, StyleValueType.Dimension, handle);
    ...
    internal bool TryReadFloat(StyleValueHandle handle, out float value) {
      ...
      bool isDimension = TryCheckAccess(dimensions, StyleValueType.Float,
                                        handle, out dimensionValue);
    ...
    internal bool TryReadDimension(StyleValueHandle handle,
                                   out Dimension value) {
      if (TryCheckAccess(dimensions, StyleValueType.Dimension, handle,
                         out value))
        return true;
      ...
      float floatValue = 0f;
      bool isFloat =
          TryCheckAccess(floats, StyleValueType.Float, handle, out floatValue);
    ...
    internal bool TryReadVariable(StyleValueHandle handle, out string value) {
      return TryCheckAccess(strings, StyleValueType.Variable, handle,
                            out value);
    }
    ...
    internal bool TryReadResourcePath(StyleValueHandle handle,
                                      out string value) {
      return TryCheckAccess(strings, StyleValueType.ResourcePath, handle,
                            out value);
    }
    ...
    internal bool TryReadAssetReference(StyleValueHandle handle,
                                        out Object value) {
      return TryCheckAccess(assets, StyleValueType.AssetReference, handle,
                            out value);
    }
    ...
    internal string ReadFunctionName(StyleValueHandle handle) {
      if (handle.valueType != StyleValueType.Function) {
        Debug.LogErrorFormat(
            $"Trying to read value of type {StyleValueType.Function} while reading a value of type {handle.valueType}");
        return string.Empty;
      }
    ...
    private static bool CustomStartsWith(string originalString,
                                         string pattern) {
      ...
      while (originalPos < originalLength && patternPos < patternLength &&
             originalString[originalPos] == pattern[patternPos]) {
        originalPos++;
        patternPos++;
      }

      return (patternPos == patternLength && originalLength >= patternLength) ||
             (originalPos == originalLength && patternLength >= originalLength);
    }
  }
}
