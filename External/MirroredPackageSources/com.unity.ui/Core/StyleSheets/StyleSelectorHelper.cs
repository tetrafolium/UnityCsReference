...
internal struct MatchResultInfo {
	...
	public readonly PseudoStates
	        triggerPseudoMask; // what pseudo states contributes to matching this
	                           // selector
	public readonly PseudoStates
	        dependencyPseudoMask; // what pseudo states if set, would have given a
	                              // different result

	public MatchResultInfo(bool success, PseudoStates triggerPseudoMask,
...
internal struct SelectorMatchRecord {
	...
	public static int Compare(SelectorMatchRecord a, SelectorMatchRecord b) {
		...
		int res = a.complexSelector.specificity.CompareTo(
			b.complexSelector.specificity);
		...
		if (res == 0) {
			res = a.complexSelector.orderInStyleSheet.CompareTo(
				b.complexSelector.orderInStyleSheet);
		}
...
public static MatchResultInfo MatchesSelector(VisualElement element,
	...
	for (int i = 0; i < count && match; i++) {
		...
		case StyleSelectorType.Predicate:
			UQuery.IVisualPredicateWrapper w =
				parts[i].tempData as UQuery.IVisualPredicateWrapper;
	...
	if (saveMatch && selector.pseudoStateMask != 0) {
		match = (selector.pseudoStateMask & (int) element.pseudoStates) ==
		        selector.pseudoStateMask;

		if (match) {
			// the element matches this selector because it has those flags
			dependencyPseudoMask = selector.pseudoStateMask;
		} else {
			// if the element had those flags defined, it would match this
			// selector
	...
	if (saveMatch && selector.negatedPseudoStateMask != 0) {
		match &=
			(selector.negatedPseudoStateMask & ~(int) element.pseudoStates) ==
			selector.negatedPseudoStateMask;

		if (match) {
			// the element matches this selector because it does not have those
			// flags
			triggerPseudoStateMask |= selector.negatedPseudoStateMask;
		} else {
			// if the element didn't have those flags, it would match this
			// selector
			dependencyPseudoMask |= selector.negatedPseudoStateMask;
		}
	}

	return new MatchResultInfo(match, (PseudoStates) triggerPseudoStateMask,
	                           (PseudoStates) dependencyPseudoMask);
}

public static bool
MatchRightToLeft(VisualElement element,
	...
	while (nextIndex >= 0) {
		...
		MatchResultInfo matchInfo =
			MatchesSelector(current, complexSelector.selectors[nextIndex]);
		...
		if (!matchInfo.success) {
			...
			if (nextIndex < complexSelector.selectors.Length - 1 &&
			    complexSelector.selectors[nextIndex + 1].previousRelationship ==
			    StyleSelectorRelationship.Descendent) {
				current = current.parent;
				continue;
			}

			// otherwise, if there's a previous relationship, it's a 'child' one.
			// backtrack from the saved point and try again ie.  for "#x > .a .b",
			// #x failed, backtrack to .a on the saved element
			if (saved != null) {
				current = saved;
				nextIndex = savedIdx;
				continue;
		...
		// backtracking save
		// for "a > b c": we're considering the b matcher. c's previous
		// relationship is Descendent save the current element parent to try to
		// match b again
		if (nextIndex < complexSelector.selectors.Length - 1 &&
		    complexSelector.selectors[nextIndex + 1].previousRelationship ==
		    StyleSelectorRelationship.Descendent) {
...
static void FastLookup(IDictionary<string, StyleComplexSelector> table,
                       List<SelectorMatchRecord> matchedSelectors,
                       StyleMatchingContext context, string input,
                       ref SelectorMatchRecord record) {
	...
	if (table.TryGetValue(input, out currentComplexSelector)) {
		while (currentComplexSelector != null) {
			if (MatchRightToLeft(context.currentElement, currentComplexSelector,
...
public static void FindMatches(StyleMatchingContext context,
                               List<SelectorMatchRecord> matchedSelectors) {
	Debug.Assert(matchedSelectors.Count == 0);

	Debug.Assert(context.currentElement != null,
	...
	for (int i = 0; i < context.styleSheetStack.Count; i++) {
		...
		FastLookup(styleSheet.orderedTypeSelectors, matchedSelectors, context,
		           element.typeName, ref record);
		FastLookup(styleSheet.orderedTypeSelectors, matchedSelectors, context,
		           "*", ref record);

		if (!string.IsNullOrEmpty(element.name)) {
			FastLookup(styleSheet.orderedNameSelectors, matchedSelectors, context,
			           element.name, ref record);
		}

		foreach (string @class in element.classList) {
			FastLookup(styleSheet.orderedClassSelectors, matchedSelectors,
			           context, @class, ref record);
		}
	}
}
}
