...
namespace UnityEngine.UIElements.StyleSheets {
  internal enum StyleValidationStatus { Ok, Error, Warning }
  ...
  internal struct StyleValidationResult {
    ...
    public string hint;
    public bool success {
      get { return status == StyleValidationStatus.Ok; }
    }
  }
  ...
  internal class StyleValidator {
    ...
    public StyleValidationResult ValidateProperty(string name, string value) {
      var result =
          new StyleValidationResult(){status = StyleValidationStatus.Ok};
      ...
      if (!matchResult.success) {
        ...
        case MatchResultErrorCode.Syntax:
          ...
          if (IsUnitMissing(syntax, value))
            result.hint =
                "Property expects a unit. Did you forget to add px or %?";
          else if (IsUnsupportedColor(syntax))
            result.hint = $"Unsupported color '{value}'.";
          result.message =
              $"Expected ({syntax}) but found '{matchResult.errorValue}'";
          break;
        ...
        case MatchResultErrorCode.ExpectedEndOfValue:
          ...
          result.message =
              $"Expected end of value but found '{matchResult.errorValue}'";
          break;
        default:
          Debug.LogAssertion(
              $"Unexpected error code '{matchResult.errorCode}'");
          break;
        }
      }
    ...
    private bool IsUnitMissing(string propertySyntax, string propertyValue) {
      float val;
      return float.TryParse(propertyValue, out val) &&
             (propertySyntax.Contains("<length>") ||
              propertySyntax.Contains("<percentage>"));
    }
