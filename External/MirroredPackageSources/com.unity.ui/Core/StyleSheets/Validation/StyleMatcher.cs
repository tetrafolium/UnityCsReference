...
namespace UnityEngine.UIElements.StyleSheets {
  ...
  internal struct MatchResult {
    ...
    public bool success {
      get { return errorCode == MatchResultErrorCode.None; }
    }
  }
  ...
  internal abstract class BaseStyleMatcher {
    ...
    public void SaveMark() { m_MarkStack.Push(m_CurrentIndex); }

    public void RestoreMark() { m_CurrentIndex = m_MarkStack.Pop(); }

    public void DropMark() { m_MarkStack.Pop(); }
    ...
    protected bool Match(Expression exp) {
      ...
      if (exp.multiplier.type == ExpressionMultiplierType.None) {
        ...
        Debug.Assert(exp.multiplier.type !=
                         ExpressionMultiplierType.OneOrMoreComma,
                     "'#' multiplier in syntax expression is not supported");
        Debug.Assert(exp.multiplier.type !=
    ...
    private bool MatchGroup(Expression exp) {
      ...
      Debug.Assert(exp.subExpressions.Length == 1,
                   "Group has invalid number of sub expressions");
    ...
    private bool MatchOrOr(Expression exp) {
      // All sub expressions are options but at least one of them must match,
      // and they may match in any order. A sub expression must appear at most
      // one single time
    ...
    private unsafe int MatchMany(Expression exp) {
      ...
      while (i < subExpCount && matchCount + matchVariableCount < subExpCount) {
        ...
        if (result) {
          ...
          // Reset the loop to try the next value on all unmatched sub
          // expressions
          i = 0;
        } else {
          ++i;
  ...
  internal class StyleMatcher : BaseStyleMatcher {
    ...
    private string current =>
        hasCurrent ? m_PropertyParts[m_CurrentIndex] : null;
    ...
    static readonly Regex s_NumberRegex =
    ...
    static readonly Regex s_IntegerRegex =
    ...
    static readonly Regex s_ZeroRegex =
        new Regex(@"^0(?:\.0+)?$",
                  RegexOptions.Compiled); // Zero is accepted without any unit
    static readonly Regex s_LengthRegex =
        new Regex(@"^[+-]?\d+(?:\.\d+)?(?:px)$", RegexOptions.Compiled);
    ...
    static readonly Regex s_PercentRegex =
        new Regex(@"^[+-]?\d+(?:\.\d+)?(?:%)$", RegexOptions.Compiled);
    ...
    static readonly Regex s_HexColorRegex = new Regex(
        @"^#[a-fA-F0-9]{3}(?:[a-fA-F0-9]{3})?$", RegexOptions.Compiled);
    static readonly Regex s_RgbRegex = new Regex(
        @"^rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)$", RegexOptions.Compiled);
    static readonly Regex s_RgbaRegex = new Regex(
    ...
    static readonly Regex s_VarFunctionRegex =
        new Regex(@"^var\(.+\)$", RegexOptions.Compiled);
    static readonly Regex s_ResourceRegex =
        new Regex(@"^resource\((.+)\)$", RegexOptions.Compiled);
    ...
    static readonly Regex s_UrlRegex =
  ...
  internal class StylePropertyValueMatcher : BaseStyleMatcher {
    ...
    private StylePropertyValue current =>
        hasCurrent ? m_Values[m_CurrentIndex] : default(StylePropertyValue);
    ...
    public MatchResult Match(Expression exp, List<StylePropertyValue> values) {
      ...
      if (firstHandle.valueType == StyleValueType.Keyword &&
          (StyleValueKeyword)
                  firstHandle.valueIndex == StyleValueKeyword.Initial) {
        MoveNext();
        match = true;
      } else {
        match = Match(exp);
