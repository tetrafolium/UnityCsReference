...
namespace UnityEngine.UIElements.StyleSheets {
  internal static class StyleSheetCache {
    ...
    static Dictionary<SheetHandleKey, StylePropertyId[]>
        s_RulePropertyIdsCache =
            new Dictionary<SheetHandleKey, StylePropertyId[]>(s_Comparer);

    internal static void ClearCaches() { s_RulePropertyIdsCache.Clear(); }

    internal static StylePropertyId[] GetPropertyIds(StyleSheet sheet,
                                                     int ruleIndex) {
    ...
    static StylePropertyId GetPropertyId(StyleRule rule, int index) {
      ...
      if (!StylePropertyUtil.s_NameToId.TryGetValue(name, out id)) {
        id = property.isCustomProperty ? StylePropertyId.Custom
