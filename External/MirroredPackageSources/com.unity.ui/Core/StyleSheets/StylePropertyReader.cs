...
namespace UnityEngine.UIElements.StyleSheets {
  ...
  internal class StylePropertyReader {
    // Strategy to create default cursor must be provided in the context of
    // Editor or Runtime
    internal delegate int GetCursorIdFunction(StyleSheet sheet,
    ...
    public void SetContext(StyleSheet sheet, StyleComplexSelector selector,
                           StyleVariableContext varContext,
    ...
    public void SetInlineContext(StyleSheet sheet, StyleProperty[] properties,
    ...
    public bool IsKeyword(int index, StyleValueKeyword keyword) {
      ...
      return value.handle.valueType == StyleValueType.Keyword &&
             (StyleValueKeyword) value.handle.valueIndex == keyword;
    }
    ...
    public StyleBackground ReadStyleBackground(int index) {
      ...
      if (value.handle.valueType == StyleValueType.Keyword) {
        if (value.handle.valueIndex != (int) StyleValueKeyword.None) {
          Debug.LogWarning("Invalid keyword for image source " +
                           (StyleValueKeyword) value.handle.valueIndex);
        } else {
          // it's OK, we let none be assigned to the source
        }
      } else if (TryGetImageSourceFromValue(value, dpiScaling, out source) ==
                 false) {
        // Load a stand-in picture to make it easier to identify which image
        // element is missing its picture
        source.texture = Panel.LoadResource(
            "d_console.warnicon", typeof(Texture2D), dpiScaling) as Texture2D;
      }
    ...
    public StyleCursor ReadStyleCursor(int index) {
      ...
      bool isCustom = valueType == StyleValueType.ResourcePath ||
                      valueType == StyleValueType.AssetReference ||
                      valueType == StyleValueType.ScalableImage;
        ...
        if (valueCount < 1) {
          ...
          if (TryGetImageSourceFromValue(value, dpiScaling, out source)) {
            ...
            if (valueCount >= 3) {
              ...
              if (valueX.handle.valueType != StyleValueType.Float ||
                  valueY.handle.valueType != StyleValueType.Float) {
                Debug.LogWarning(
                    "USS 'cursor' property requires two integers for the hot spot value.");
              } else {
      ...
      var cursor = new Cursor(){texture = texture,
          ...
          for (int i = 0; i < sp.values.Length && valid; ++i) {
            ...
            if (handle.IsVarFunction()) {
              ...
              if (result != StyleVariableResolver.Result.Valid) {
                // Resolve failed
                // When this happens, the computed value of the property is
                // either the property’s inherited value or its initial value
                // depending on whether the property is inherited or not. This
                // is the same behavior as the unset keyword so we simply
                // resolve to that value.
                var unsetHandle = new StyleValueHandle(){
                    valueType = StyleValueType.Keyword,
                    valueIndex = (int) StyleValueKeyword.Unset};
                m_Values.Add(new StylePropertyValue(){sheet = m_Sheet,
                                                      handle = unsetHandle});
                ++count;
    ...
    TryGetImageSourceFromValue(StylePropertyValue propertyValue,
                               float dpiScaling, out ImageSource source) {
      source = new ImageSource();

      switch (propertyValue.handle.valueType) {
      case StyleValueType.ResourcePath: {
        string path =
            propertyValue.sheet.ReadResourcePath(propertyValue.handle);
        if (!string.IsNullOrEmpty(path)) {
          // TODO: This will use GUIUtility.pixelsPerPoint as targetDpi, this
          // may not be the best value for the current panel
          source.texture = Panel.LoadResource(path, typeof(Texture2D),
                                              dpiScaling) as Texture2D;
          if (source.texture == null)
            source.vectorImage = Panel.LoadResource(path, typeof(VectorImage),
                                                    dpiScaling) as VectorImage;
        }

        if (source.texture == null && source.vectorImage == null) {
          Debug.LogWarning(
              string.Format("Image not found for path: {0}", path));
          return false;
        }
      } break;
      ...
      default:
        Debug.LogWarning("Invalid value for image texture " +
                         propertyValue.handle.valueType);
        return false;
      }
