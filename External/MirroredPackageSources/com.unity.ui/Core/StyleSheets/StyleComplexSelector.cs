...
namespace UnityEngine.UIElements {
  ...
  internal class StyleComplexSelector {
    [SerializeField] int m_Specificity;

    // This "score" is calculated according to the enclosing complex selector
    // specificity
    ...
    // A complex selector can be considered simple if it's made of only one
    // selector
    public bool isSimple {
      get { return selectors.Length == 1; }
    }

    [SerializeField] StyleSelector[] m_Selectors;
    ...
    [NonSerialized]
        // Points to the possible next selector that indexes to the same lookup
        // table in StyleSheet
    ...
    internal void CachePseudoStateMasks() {
      ...
      if (s_PseudoStates == null) {
        ...
        s_PseudoStates["active"] =
            new PseudoStateData(PseudoStates.Active, false);
        s_PseudoStates["hover"] =
            new PseudoStateData(PseudoStates.Hover, false);
        s_PseudoStates["checked"] =
            new PseudoStateData(PseudoStates.Checked, false);
        s_PseudoStates["selected"] = new PseudoStateData(
            PseudoStates.Checked, false); // for backward-compatibility
        s_PseudoStates["disabled"] =
            new PseudoStateData(PseudoStates.Disabled, false);
        s_PseudoStates["focus"] =
            new PseudoStateData(PseudoStates.Focus, false);
        s_PseudoStates["root"] = new PseudoStateData(PseudoStates.Root, false);

        // A few substates can be negated, meaning them match if the flag is not
        // set
        s_PseudoStates["inactive"] =
            new PseudoStateData(PseudoStates.Active, true);
        s_PseudoStates["enabled"] =
            new PseudoStateData(PseudoStates.Disabled, true);
      }
        ...
        for (int i = 0; i < selector.parts.Length; i++) {
          if (selector.parts[i].type == StyleSelectorType.PseudoClass) {
            ...
            if (s_PseudoStates.TryGetValue(parts[i].value, out data)) {
              ...
              Debug.LogWarningFormat("Unknown pseudo class \"{0}\"",
                                     parts[i].value);
            }
          }
        }
    ...
    public override string ToString() {
      return string.Format(
          "[{0}]",
          string.Join(", ", m_Selectors.Select(x => x.ToString()).ToArray()));
    }
  }
}
