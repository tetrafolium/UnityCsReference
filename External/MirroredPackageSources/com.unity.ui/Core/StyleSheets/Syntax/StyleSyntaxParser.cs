...
private Stack<ExpressionCombinator> m_CombinatorStack =
	new Stack<ExpressionCombinator>();
...
public Expression Parse(string syntax) {
	...
	// A bunch of properties have the same syntax so it's worth caching the
	// result
...
private Expression ParseExpression(StyleSyntaxTokenizer tokenizer) {
	...
	while (!IsExpressionEnd(token)) {
		...
		if (token.type == StyleSyntaxTokenType.String ||
		    token.type == StyleSyntaxTokenType.LessThan) {
		...
		if (nextCombinatorType != ExpressionCombinator.None) {
			if (m_CombinatorStack.Count > 0) {
				...
				// Higher precedence means the previous combinator needs to be
				// created first
				while (previousPrecedence > currentPrecedence &&
				       previousCombinator != ExpressionCombinator.Group) {
					ProcessCombinatorStack();
					previousCombinator = m_CombinatorStack.Count > 0
				       ? m_CombinatorStack.Peek()
	...
	// Step 3 : Rollback the stack of combinators and create associated
	// expressions
...
private void ProcessCombinatorStack() {
	...
	// Small optimization that merge combinator of the same type from the
	// stack into one
	while (m_CombinatorStack.Count > 0 &&
	       combinatorType == m_CombinatorStack.Peek()) {
...
private Expression ParseTerm(StyleSyntaxTokenizer tokenizer) {
	...
	if (token.type == StyleSyntaxTokenType.LessThan) {
		...
		throw new Exception(
			      $"Unexpected token '{token.type}' in expression. Expected term token");
	}
...
private ExpressionCombinator
ParseCombinatorType(StyleSyntaxTokenizer tokenizer) {
	...
	while (!IsExpressionEnd(token) && type == ExpressionCombinator.None) {
		...
		case StyleSyntaxTokenType.Space:
			if (!IsCombinator(next) &&
			    next.type != StyleSyntaxTokenType.CloseBracket) {
		...
		default:
			throw new Exception(
				      $"Unexpected token '{token.type}' in expression. Expected combinator token");
		}
...
private Expression ParseGroup(StyleSyntaxTokenizer tokenizer) {
	...
	if (token.type != StyleSyntaxTokenType.OpenBracket)
		throw new Exception(
			      $"Unexpected token '{token.type}' in group expression. Expected '[' token");
	...
	// Do not eat the space after the ] token because it could be a
	// juxtaposition or multiplier
	tokenizer.MoveNext();
...
private Expression ParseDataType(StyleSyntaxTokenizer tokenizer) {
	...
	if (token.type != StyleSyntaxTokenType.LessThan)
		throw new Exception(
			      $"Unexpected token '{token.type}' in data type expression. Expected '<' token");
	...
	default:
		throw new Exception(
			      $"Unexpected token '{token.type}' in data type expression");
	}
...
private Expression ParseProperty(StyleSyntaxTokenizer tokenizer) {
	...
	if (token.type != StyleSyntaxTokenType.SingleQuote)
		throw new Exception(
			      $"Unexpected token '{token.type}' in property expression. Expected ''' token");

	token = tokenizer.MoveNext();
	if (token.type != StyleSyntaxTokenType.String)
		throw new Exception(
			      $"Unexpected token '{token.type}' in property expression. Expected 'string' token");

	string propertyName = token.text;
	string syntax;
	if (!StylePropertyCache.TryGetSyntax(propertyName, out syntax))
		throw new Exception(
			      $"Unknown property '{propertyName}' <''> expression.");
	...
	if (!m_ParsedExpressionCache.TryGetValue(syntax, out exp)) {
		...
		// Pushing the ExpressionCombinator.Group allow the next call to Parse
...
private void ParseMultiplier(StyleSyntaxTokenizer tokenizer,
	...
	if (IsMultiplier(token)) {
		...
		default:
			throw new Exception(
				      $"Unexpected token '{token.type}' in expression. Expected multiplier token");
		}
...
private void ParseRanges(StyleSyntaxTokenizer tokenizer, out int min,
                         out int max) {
	min = -1;
	max = -1;
	...
	while (token.type != StyleSyntaxTokenType.CloseBrace) {
		...
		default:
			throw new Exception(
				      $"Unexpected token '{token.type}' in expression. Expected ranges token");
		}
