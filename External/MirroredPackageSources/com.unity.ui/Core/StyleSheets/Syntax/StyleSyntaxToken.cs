...
public void Tokenize(string syntax) {
	...
	for (int i = 0; i < syntax.Length; i++) {
		...
		: "EOF";
				...
				m_Tokens.Add(
					new StyleSyntaxToken(StyleSyntaxTokenType.DoubleAmpersand));
				++i;
			}
			break;
		case ',':
		...
		case '!':
			m_Tokens.Add(
				new StyleSyntaxToken(StyleSyntaxTokenType.ExclamationPoint));
			break;
		case '[':
		...
		default:
			if (char.IsNumber(c)) {
				...
				m_Tokens.Add(
					new StyleSyntaxToken(StyleSyntaxTokenType.Number, tokenNumber));
				...
				string tokenText = syntax.Substring(subStrStart, subStrLength);
				m_Tokens.Add(
					new StyleSyntaxToken(StyleSyntaxTokenType.String, tokenText));
			} else {
...
private static bool IsNextLetterOrDash(string s, int index) {
	return index + 1 < s.Length &&
	       (char.IsLetter(s[index + 1]) || s[index + 1] == '-');
}
