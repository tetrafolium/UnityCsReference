...
namespace UnityEngine.UIElements {
  ...
  // notes on precision:
  // the event will be fired no sooner than delayMs, and repeat no sooner than
  // intervalMs this means that depending on system and application state the
  // events could be fired less often or intervals skipped entirely. it is the
  // registrar's responsibility to read the TimerState to determine the actual
  // event timing and make sure things like animation are smooth and time based.
  // a delayMs of 0 and intervalMs of 0 will be interpreted as "as often as
  // possible" this should be used sparingly and the work done should be very
  // small
  /// <summary>
  ...
  public struct TimerState : IEquatable<TimerState> {
    /// <summary>
    /// Start time in milliseconds, or last callback time for repeatable <see
    /// cref="IScheduledItem"/>.
    /// </summary>
    public long start {
      get;
    ...
    /// <summary>
    /// Time difference in milliseconds between <see cref="now"/> and the
    /// previous callback.
    /// </summary>
    public long deltaTime {
      get { return now - start; }
    }
    ...
    public bool Equals(TimerState other) {
      return start == other.start && now == other.now &&
  ...
  internal interface IScheduler {
    ScheduledItem ScheduleOnce(Action<TimerState> timerUpdateEvent,
                               long delayMs);
    ScheduledItem ScheduleUntil(Action<TimerState> timerUpdateEvent,
                                long delayMs, long intervalMs,
                                Func<bool> stopCondition = null);
    ScheduledItem ScheduleForDuration(Action<TimerState> timerUpdateEvent,
                                      long delayMs, long intervalMs,
                                      long durationMs);

    // removes the event.
    // an event that is never stopped will not be stopped until the panel is
    // cleaned-up.
  ...
  internal class TimerEventScheduler : IScheduler {
    ...
    private readonly List<ScheduledItem> m_ScheduleTransactions =
        new List<ScheduledItem>(); // order is important schedules are executed
                                   // in add order
    private readonly HashSet<ScheduledItem> m_UnscheduleTransactions =
        new HashSet<ScheduledItem>(); // order no important. removal from
                                      // m_ScheduledItems has no side effect
    ...
    public void Schedule(ScheduledItem item) {
      ...
      if (scheduledItem == null) {
        throw new NotSupportedException(
            "Scheduled Item type is not supported by this scheduler");
      }
      ...
      if (m_TransactionMode) {
        ...
        } else if (m_ScheduledItems.Contains(scheduledItem) ||
                   m_ScheduleTransactions.Contains(scheduledItem)) {
          throw new ArgumentException(string.Concat(
              "Cannot schedule function ", scheduledItem, " more than once"));
        } else {
    ...
    public ScheduledItem ScheduleOnce(Action<TimerState> timerUpdateEvent,
                                      long delayMs) {
      var scheduleItem =
          new TimerEventSchedulerItem(timerUpdateEvent){delayMs = delayMs};
    ...
    public ScheduledItem ScheduleUntil(Action<TimerState> timerUpdateEvent,
                                       long delayMs, long intervalMs,
                                       Func<bool> stopCondition) {
      var scheduleItem = new TimerEventSchedulerItem(timerUpdateEvent){
          delayMs = delayMs, intervalMs = intervalMs,
    ...
    public ScheduledItem
    ScheduleForDuration(Action<TimerState> timerUpdateEvent, long delayMs,
                        long intervalMs, long durationMs) {
      var scheduleItem = new TimerEventSchedulerItem(timerUpdateEvent){
          delayMs = delayMs, intervalMs = intervalMs,
    ...
    public void Unschedule(ScheduledItem item) {
      ...
      if (sItem != null) {
        if (m_TransactionMode) {
          if (m_UnscheduleTransactions.Contains(sItem)) {
            throw new ArgumentException(
                "Cannot unschedule scheduled function twice" + sItem);
          } else if (m_ScheduleTransactions.Remove(sItem)) {
            // A item has been scheduled then unscheduled in the same
            // transaction. which is valid.
          } else if (m_ScheduledItems.Contains(sItem)) {
            // Only add it to m_UnscheduleTransactions if it is in
            // m_ScheduledItems. if it was successfully removed from
            // m_ScheduleTransaction we are fine.
            m_UnscheduleTransactions.Add(sItem);
          } else {
            throw new ArgumentException(
                "Cannot unschedule unknown scheduled function " + sItem);
          }
        } else {
        ...
        sItem.OnItemUnscheduled(); // Call OnItemUnscheduled immediately after
                                   // successful removal even if we are in
                                   // transaction mode
      }
    }

    bool PrivateUnSchedule(ScheduledItem sItem) {
      return m_ScheduleTransactions.Remove(sItem) ||
             RemovedScheduledItemAt(m_ScheduledItems.IndexOf(sItem));
    }
    ...
    public void UpdateScheduledEvents() {
      try {
        ...
        // TODO: On a GAME Panel game time should be per frame and not change
        // during a frame.
        ...
        for (int i = 0; i < itemsCount; i++) {
          ...
          if (!disableThrottling && currentTime >= maxTime) {
            // We spent too much time on this frame updating items, we break for
            // now, we'll resume next frame
            break;
          }
          ...
          if (currentTime - scheduledItem.delayMs >= scheduledItem.startMs) {
            ...
            if (!m_UnscheduleTransactions.Contains(
                    scheduledItem)) // Don't execute items that have been marked
                                    // for future removal
          ...
          if (unscheduleItem || (scheduledItem.endTimeMs > 0 &&
                                 currentTime > scheduledItem.endTimeMs)) {
            // if the scheduledItem has been unscheduled explicitly in
            // PerformTimerUpdate then it will be in m_UnscheduleTransactions
            // and we shouldn't unschedule it again
