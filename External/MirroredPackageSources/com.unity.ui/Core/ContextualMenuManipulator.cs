...
public ContextualMenuManipulator(
	System.Action<ContextualMenuPopulateEvent> menuBuilder) {
...
protected override void RegisterCallbacksOnTarget() {
	...
	target.RegisterCallback<ContextualMenuPopulateEvent>(
...
protected override void UnregisterCallbacksFromTarget() {
	...
	target.UnregisterCallback<ContextualMenuPopulateEvent>(
		OnContextualMenuEvent);
}
...
void OnMouseUpDownEvent(IMouseEvent evt) {
	if (CanStartManipulation(evt)) {
		if (target.elementPanel != null &&
		    target.elementPanel.contextualMenuManager != null) {
