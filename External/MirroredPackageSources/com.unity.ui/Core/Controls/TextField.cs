...
namespace UnityEngine.UIElements {
  ...
  public class TextField : TextInputBaseField<string> {
    ...
    // This is to save the value of the tabindex of the visual input to achieve
    // the IMGUI behaviour of tabbing on multiline TextField.
    int m_VisualInputTabIndex;

    /// <summary>
    /// Instantiates a <see cref="TextField"/> using the data read from a UXML
    /// file.
    /// </summary>
    ...
    public new class UxmlTraits : TextInputBaseField<string>.UxmlTraits {
      UxmlBoolAttributeDescription m_Multiline =
          new UxmlBoolAttributeDescription{name = "multiline"};

      /// <summary>
      /// Initialize <see cref="TextField"/> properties using values from the
      /// attribute bag.
      /// </summary>
    ...
    /// <summary>
    /// Set this to true to allow multiple lines in the textfield and false if
    /// otherwise.
    /// </summary>
    ...
    /// Creates a new textfield.
    /// </summary>
    /// <param name="maxLength">The maximum number of characters this textfield
    /// can hold. If 0, there is no limit.</param> <param name="multiline">Set
    /// this to true to allow multiple lines in the textfield and false if
    /// otherwise.</param> <param name="isPasswordField">Set this to true to
    /// mask the characters and false if otherwise.</param> <param
    /// name="maskChar">The character used for masking in a password
    /// field.</param>
    public TextField(int maxLength, bool multiline, bool isPasswordField,
                     char maskChar)
    ...
    /// name="maskChar">The character used for masking in a password
    /// field.</param>
    public TextField(string label, int maxLength, bool multiline,
                     bool isPasswordField, char maskChar)
    ...
    internal override void OnViewDataReady() {
      ...
      // Here we must make sure the value is restored on screen from the saved
      // value !
      text = rawValue;
    }
    ...
    protected override void ExecuteDefaultActionAtTarget(EventBase evt) {
      ...
      // On IMGUI, a TextArea "in edit mode" is accepting TAB, doing a
      // Shift+Return will get out of the Edit mode
      //     and a TAB will allow the user to get to the next control...
      // To mimic that behaviour in UIE, when in focused-non-edit-mode, we have
      // to make sure the input is not "tabbable".
      //     So, each time, either the main TextField or the Label is receiving
      //     the focus, we remove the tabIndex on the input, and we put it back
      //     when the BlurEvent is received.
      if (multiline) {
        if ((evt?.eventTypeId == FocusInEvent.TypeId() &&
             evt?.leafTarget == this) ||
            (evt?.eventTypeId == FocusInEvent.TypeId() &&
             evt?.leafTarget == labelElement)) {
          m_VisualInputTabIndex = visualInput.tabIndex;
          visualInput.tabIndex = -1;
        } else if ((evt?.eventTypeId == BlurEvent.TypeId() &&
                    evt?.leafTarget == this) ||
                   (evt?.eventTypeId == BlurEvent.TypeId() &&
                    evt?.leafTarget == labelElement)) {
    ...
    class TextInput : TextInputBase {
      ...
      // Password field (indirectly lossy behaviour when activated via
      // multiline)
      ...
      protected override string StringToValue(string str) { return str; }
      ...
      protected override void ExecuteDefaultActionAtTarget(EventBase evt) {
        ...
        if (evt.eventTypeId == KeyDownEvent.TypeId()) {
          ...
          if (!parentTextField.isDelayed ||
              (!multiline && ((kde?.keyCode == KeyCode.KeypadEnter) ||
          ...
          if (multiline) {
            if (kde?.character == '\t' &&
                kde.modifiers == EventModifiers.None) {
              kde?.StopPropagation();
              kde?.PreventDefault();
            } else if (((kde?.character == 3) &&
                        (kde?.shiftKey == true)) || // KeyCode.KeypadEnter
                       ((kde?.character == '\n') &&
          ...
          if (!parentTextField.isDelayed &&
              (cmdName == EventCommandNames.Paste ||
               cmdName == EventCommandNames.Cut)) {
            parentTextField.value = text;
          }
        }
      }
