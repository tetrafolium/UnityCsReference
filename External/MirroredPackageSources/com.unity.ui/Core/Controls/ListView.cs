...
namespace UnityEngine.UIElements {
  ...
  public enum AlternatingRowBackground {
    ...
    None,
    /// <summary>
    /// Display alternating background colors only for rows that contain
    /// content.
    /// </summary>
    ContentOnly,
    /// <summary>
    /// Display alternating background colors for all rows in the ListView,
  ...
  /// <summary>
  /// A vertically scrollable area that only creates visual elements for visible
  /// items while allowing the binding of many more items. As the user scrolls,
  /// visual elements are recycled and re-bound to new data items.
  /// </summary>
  /// <example>
  /// <code>
  /// <![CDATA[
  ...
  ///     {
  ///         // Create some list of data, here simply numbers in interval [1,
  ///         1000] const int itemCount = 1000; var items = new
  ///         List<string>(itemCount); for (int i = 1; i <= itemCount; i++)
  ...
  ///         // the element with the matching data item (specified as an index
  ...
  ///
  ///         var listView = new ListView(items, itemHeight, makeItem,
  ///         bindItem);
  ///
  ...
  public class ListView : BindableElement, ISerializationCallbackReceiver {
    /// <summary>
    /// Instantiates a <see cref="ListView"/> using the data read from a UXML
    /// file.
    /// </summary>
    ...
    public new class UxmlTraits : BindableElement.UxmlTraits {
      ...
      private readonly UxmlBoolAttributeDescription m_Reorderable =
          new UxmlBoolAttributeDescription{name = "reorderable",
                                           defaultValue = false};
      private readonly UxmlBoolAttributeDescription m_ShowBoundCollectionSize =
          new UxmlBoolAttributeDescription{name = "show-bound-collection-size",
                                           defaultValue = true};
      private readonly UxmlBoolAttributeDescription
          m_HorizontalScrollingEnabled = new UxmlBoolAttributeDescription{
              name = "horizontal-scrolling", defaultValue = false};

      /// <summary>
      /// Returns an empty enumerable, as list views generally do not have
      /// children.
      /// </summary>
      ...
      /// <summary>
      /// Initialize <see cref="ListView"/> properties using values from the
      /// attribute bag.
      /// </summary>
      ...
      public override void Init(VisualElement ve, IUxmlAttributes bag,
        ...
        listView.showAlternatingRowBackgrounds =
            m_ShowAlternatingRowBackgrounds.GetValueFromBag(bag, cc);
        listView.showBoundCollectionSize =
            m_ShowBoundCollectionSize.GetValueFromBag(bag, cc);
        listView.horizontalScrollingEnabled =
            m_HorizontalScrollingEnabled.GetValueFromBag(bag, cc);
      }
    }
    ...
    /// <summary>
    /// Callback for when an item is chosen (double-click). This is different
    /// from just a selection.
    /// </summary>
    [Obsolete("onItemChosen is obsolete, use onItemsChosen instead")]
    public event Action<object> onItemChosen;
    /// <summary>
    /// Callback triggered when the user 'acts on' a selection of one or more
    /// items, for example by double-clicking or pressing Enter.
    /// </summary>
    ...
    /// <summary>
    /// The items data source. This property must be set for the list view to
    /// function.
    /// </summary>
    ...
    /// <summary>
    /// Callback for constructing the VisualElement that will serve as the
    /// template for each recycled and re-bound element in the list. This
    /// property must be set for the list view to function.
    /// </summary>
    ...
    /// <summary>
    /// Computed pixel aligned height for the list elements. This value will
    /// change depending on the current panel's dpi scaling.
    /// </summary>
    ...
    /// <summary>
    /// ListView requires all visual elements to have the same height so that it
    /// can calculate a sensible scroller size. This property must be set for
    /// the list view to function.
    /// </summary>
    ...
    /// <summary>
    /// Gets or sets a value that indicates whether the user can drag list items
    /// to reorder them.
    /// </summary>
        ...
        if (m_Dragger?.dragAndDropController == null) {
          if (value)
            SetDragAndDropController(
                new ListViewReorderableDragAndDropController(this));
    ...
    // Not persisted! Just used for fast lookups of selected indices and object
    // references. This is to avoid also having a mapping from index/object ref
    // to index for the entire items source.
    ...
    /// <summary>
    /// Returns the selected item's index in the items source. If multiple items
    /// are selected, returns the first selected item's index.
    /// </summary>
    ...
    /// <summary>
    /// Returns the selected item from the items source. If multiple items are
    /// selected, returns the first selected item.
    /// </summary>
    public object selectedItem =>
        m_SelectedItems.Count == 0 ? null : m_SelectedItems.First();
    /// <summary>
    ...
    /// <summary>
    /// Controls the selection state. You can set the state to disable
    /// selections, have one selectable item, or have multiple selectable items.
    /// </summary>
    ...
    /// <summary>
    /// Enable this property to display alternating background colors for rows
    /// in the ListView.
    /// </summary>
    ...
    /// <summary>
    /// When you bind a list view to an array, this property controls whether
    /// the list view displays the collection size as the first list item. Set
    /// to true to display the collection size, false to omit it. Default is
    /// true.
    /// </summary>
    ...
    /// <summary>
    /// This flag indicates whether the ListView should show a horizontal scroll
    /// bar when its content does not fit. The default value is False.
    /// </summary>
    public bool horizontalScrollingEnabled {
      ...
      set {
        ...
        m_ScrollView.SetScrollViewMode(
            value ? ScrollViewMode.VerticalAndHorizontal
    ...
    internal static CustomStyleProperty<int> s_ItemHeightProperty =
        new CustomStyleProperty<int>("--unity-item-height");
    ...
    /// <summary>
    /// The USS class name. Enable the showBorder property to apply this class
    /// to the ListView.
    /// </summary>
    public static readonly string borderUssClassName =
        ussClassName + "--with-border";
    /// <summary>
    ...
    public ListView() {
      ...
      m_ScrollView.contentContainer.RegisterCallback<AttachToPanelEvent>(
          OnAttachToPanel);
      m_ScrollView.contentContainer.RegisterCallback<DetachFromPanelEvent>(
          OnDetachFromPanel);
      ...
      m_ScrollView.contentContainer.usageHints &=
          ~UsageHints.GroupTransform; // Scroll views with virtualized content
                                      // shouldn't have the "view transform"
                                      // optimization
    ...
    public ListView(IList itemsSource, int itemHeight,
    ...
    private void OnAttachToPanel(AttachToPanelEvent evt) {
      ...
      if (evt.destinationPanel.contextType == ContextType.Editor) {
        m_ScrollView.contentContainer.RegisterCallback<MouseDownEvent>(
            OnMouseDown);
        ...
        m_ScrollView.contentContainer.RegisterCallback<PointerDownEvent>(
            OnPointerDown);
        m_ScrollView.contentContainer.RegisterCallback<PointerUpEvent>(
            OnPointerUp);
      }
    ...
    private void OnDetachFromPanel(DetachFromPanelEvent evt) {
      ...
      if (evt.originPanel.contextType == ContextType.Editor) {
        m_ScrollView.contentContainer.UnregisterCallback<MouseDownEvent>(
            OnMouseDown);
        m_ScrollView.contentContainer.UnregisterCallback<MouseUpEvent>(
            OnMouseUp);
      } else if (evt.originPanel.contextType == ContextType.Player) {
        m_ScrollView.contentContainer.UnregisterCallback<PointerDownEvent>(
            OnPointerDown);
        m_ScrollView.contentContainer.UnregisterCallback<PointerUpEvent>(
            OnPointerUp);
      }
...
#pragma warning restore 618
      ...
      case KeyCode.PageDown:
        selectedIndex =
            Math.Min(itemsSource.Count - 1,
                     selectedIndex + (int)(m_LastHeight / resolvedItemHeight));
        break;
      case KeyCode.PageUp:
        selectedIndex = Math.Max(
            0, selectedIndex - (int)(m_LastHeight / resolvedItemHeight));
        break;
      case KeyCode.A:
        if (evt.actionKey) {
          SelectAll();
    ...
    /// </summary>
    /// <param name="index">Item index to scroll to. Specify -1 to make the last
    /// item visible.</param>
    public void ScrollToItem(int index) {
      if (!HasValidDataAndBindings())
        throw new InvalidOperationException(
            "Can't scroll without valid source, bind method, or factory method.");
      ...
      if (index == -1) {
        ...
        if (itemsSource.Count < actualCount)
          ...
          m_ScrollView.scrollOffset =
              new Vector2(0, itemsSource.Count * pixelAlignedItemHeight);
      } else if (m_FirstVisibleIndex >= index) {
        m_ScrollView.scrollOffset =
            Vector2.up * (pixelAlignedItemHeight * index);
      } else // index > first
      {
        ...
        var visibleOffset =
            pixelAlignedItemHeight -
            (m_LastHeight - actualCount * pixelAlignedItemHeight);
        var yScrollOffset = pixelAlignedItemHeight * d + visibleOffset;

        m_ScrollView.scrollOffset =
            new Vector2(m_ScrollView.scrollOffset.x, yScrollOffset);
      }
    }
    ...
    private void OnMouseDown(MouseDownEvent evt) {
      ...
      DoSelect(evt.localMousePosition, evt.clickCount, evt.actionKey,
               evt.shiftKey);
    }
    ...
    private void OnPointerUp(PointerUpEvent evt) {
      ...
      if (evt.pointerType != PointerType.mouse) {
        ...
        if (delay < 500 && delta.sqrMagnitude <= 100) {
          DoSelect(evt.localPosition, evt.clickCount, evt.actionKey,
                   evt.shiftKey);
        }
      }
    }

    private void DoSelect(Vector2 localPosition, int clickCount, bool actionKey,
                          bool shiftKey) {
      ...
      case 1:
        ...
        if (selectionType == SelectionType.Multiple && actionKey) {
          ...
          // If drag and drop will be started listview dragger will capture the
          // mouse and ListView will not receive the mouse up event
        } else // single
        {
          m_RangeSelectionOrigin = clickedIndex;
          SetSelection(clickedIndex);
    ...
    private void OnMouseUp(MouseUpEvent evt) {
      ...
      if (selectionType == SelectionType.Multiple && !evt.shiftKey &&
          !evt.actionKey && m_SelectedIndices.Count > 1 &&
          m_SelectedIndices.Contains(clickedIndex)) {
        ProcessSingleClick(clickedIndex);
      }
    }
    ...
    /// <summary>
    /// Sets a collection of selected items without triggering a selection
    /// change callback.
    /// </summary>
    /// <param name="indices">Collection of items to be selected.</param>
    ...
    internal void SetSelectionInternal(IEnumerable<int> indices,
...
#pragma warning restore 618
    ...
    internal void SetDragAndDropController(
        IListViewDragAndDropController dragAndDropController) {
      if (m_Dragger == null)
    ...
    private void OnScroll(float offset) {
      ...
      m_ScrollView.contentContainer.style.paddingTop =
          fistVisibleItem * pixelAlignedItemHeight;
      m_ScrollView.contentContainer.style.height =
          itemsSource.Count * pixelAlignedItemHeight;
        ...
        if (m_Pool.Count > 0) {
          ...
          if (m_FirstVisibleIndex < m_Pool[0].index) // we're scrolling up
          {
            ...
            for (int i = 0; i < count && m_Pool.Count > 0; ++i) {
              ...
              last.element.SendToBack(); // We send the element to the top of
                                         // the list (back in z-order)
            }
          ...
          } else // down
          {
            if (m_FirstVisibleIndex < m_Pool[m_Pool.Count - 1].index) {
              ...
              while (m_FirstVisibleIndex > m_Pool[checkIndex].index) {
                ...
                first.element
                    .BringToFront(); // We send the element to the bottom of the
                                     // list (front in z-order)
              }
    ...
    /// <summary>
    /// Clear, recreate all visible visual elements, and rebind all items. This
    /// should be called whenever the items source changes.
    /// </summary>
    public void Refresh() {
      foreach (var recycledItem in m_Pool)
    ...
    private void ResizeHeight(float height) {
      ...
      var scrollableHeight = Mathf.Max(
          0, contentHeight - m_ScrollView.contentViewport.layout.height);
      m_ScrollView.verticalScroller.highValue = Mathf.Min(
          Mathf.Max(m_ScrollOffset, m_ScrollView.verticalScroller.highValue),
          scrollableHeight);
      m_ScrollView.verticalScroller.value =
          Mathf.Min(m_ScrollOffset, m_ScrollView.verticalScroller.highValue);

      int itemCount =
          Math.Min((int)(height / pixelAlignedItemHeight) + k_ExtraVisibleItems,
                   itemsSource.Count);
    ...
    private void Setup(RecycledItem recycledItem, int newIndex) {
      ...
      if (showAlternatingRowBackgrounds != AlternatingRowBackground.None &&
          newIndex % 2 == 1)
        recycledItem.element.AddToClassList(
            itemAlternativeBackgroundUssClassName);
      else
        recycledItem.element.RemoveFromClassList(
            itemAlternativeBackgroundUssClassName);
      ...
      if (indexInParent == m_ScrollView.contentContainer.childCount) {
        ...
        recycledItem.element.PlaceBehind(
            m_ScrollView.contentContainer[indexInParent]);
      }
    ...
    private void UpdateBackground() {
      var backgroundFillHeight = m_ScrollView.contentViewport.layout.size.y -
                                 m_ScrollView.contentContainer.layout.size.y;
      if (showAlternatingRowBackgrounds != AlternatingRowBackground.All ||
      ...
      var pixelAlignedItemHeight = resolvedItemHeight;
      var itemsCount =
          Mathf.FloorToInt(backgroundFillHeight / pixelAlignedItemHeight) + 1;
      if (itemsCount > m_EmptyRows.childCount) {
        var itemsToAdd = itemsCount - m_EmptyRows.childCount;
        for (var i = 0; i < itemsToAdd; i++) {
          var row = new VisualElement();
          // Inline style is used to prevent a user from changing an item
          // flexShrink property.
      ...
      foreach (var child in m_EmptyRows.hierarchy.Children()) {
        ...
        child.EnableInClassList(itemAlternativeBackgroundUssClassName,
                                index % 2 == 1);
      }
    }
    ...
    private void OnCustomStyleResolved(CustomStyleResolvedEvent e) {
      int height;
      if (!m_ItemHeightIsInline &&
          e.customStyle.TryGetValue(s_ItemHeightProperty, out height)) {
    ...
    void ISerializationCallbackReceiver.OnAfterDeserialize() { Refresh(); }
  }
}
