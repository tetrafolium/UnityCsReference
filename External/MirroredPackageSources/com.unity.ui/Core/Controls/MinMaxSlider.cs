...
namespace UnityEngine.UIElements {
  ...
  public class MinMaxSlider : BaseField<Vector2> {
    /// <summary>
    /// Instantiates a <see cref="MinMaxSlider"/> using the data read from a
    /// UXML file.
    /// </summary>
    ...
    public new class UxmlTraits : BaseField<Vector2>.UxmlTraits {
      UxmlFloatAttributeDescription m_MinValue =
          new UxmlFloatAttributeDescription{name = "min-value",
                                            defaultValue = 0};
      UxmlFloatAttributeDescription m_MaxValue =
          new UxmlFloatAttributeDescription{name = "max-value",
                                            defaultValue = kDefaultHighValue};
      UxmlFloatAttributeDescription m_LowLimit =
          new UxmlFloatAttributeDescription{name = "low-limit",
                                            defaultValue = float.MinValue};
      UxmlFloatAttributeDescription m_HighLimit =
          new UxmlFloatAttributeDescription{name = "high-limit",
                                            defaultValue = float.MaxValue};

      /// <summary>
      /// Initialize <see cref="MinMaxSlider"/> properties using values from the
      /// attribute bag.
      /// </summary>
    ...
    private enum DragState { NoThumb, MinThumb, MiddleThumb, MaxThumb }
    ...
    public float minValue {
      get { return value.x; }
      set { base.value = ClampValues(new Vector2(value, rawValue.y)); }
    }
    ...
    public float maxValue {
      get { return value.y; }
      set { base.value = ClampValues(new Vector2(rawValue.x, value)); }
    }

    // Complete value of the slider position, where X is the minimum, and Y is
    // the maximum in a Vector2
    /// <summary>
    /// This is the value of the slider. This is a <see cref="Vector2"/> where
    /// the x is the lower bound and the y is the higher bound.
    /// </summary>
    ...
    // The complete range that the value could span on, from the minimum to the
    // maximum limit.
    /// <summary>
    ...
    public static readonly string trackerUssClassName =
        ussClassName + "__tracker";
    /// <summary>
    /// USS class name of dragger elements in elements of this type.
    /// </summary>
    public static readonly string draggerUssClassName =
        ussClassName + "__dragger";
    /// <summary>
    /// USS class name of the minimum thumb elements in elements of this type.
    /// </summary>
    public static readonly string minThumbUssClassName =
        ussClassName + "__min-thumb";
    /// <summary>
    /// USS class name of the maximum thumb elements in elements of this type.
    /// </summary>
    public static readonly string maxThumbUssClassName =
        ussClassName + "__max-thumb";

    /// <summary>
    /// Constructor.
    /// </summary>
    public MinMaxSlider() : this(null) {}

    /// <summary>
    /// Constructor.
    /// </summary>
    /// <param name="minValue">The minimum value in the range to be
    /// represented.</param> <param name="maxValue">The maximum value in the
    /// range to be represented.</param> <param name="minLimit">The minimum
    /// value of the slider limit.</param> <param name="maxLimit">The maximum
    /// value of the slider limit.</param>
    public MinMaxSlider(float minValue, float maxValue, float minLimit,
                        float maxLimit)
    ...
    /// value of the slider limit.</param> <param name="maxLimit">The maximum
    /// value of the slider limit.</param>
    public MinMaxSlider(string label, float minValue = 0,
                        float maxValue = kDefaultHighValue,
      ...
      dragElement.RegisterCallback<GeometryChangedEvent>(
          UpdateDragElementPosition);
      visualInput.Add(dragElement);

      // For a better handling of the cursor style, children elements are
      // created so that the style is automatic with the uss.
      ...
      clampedDragger = new ClampedDragger<float>(null, SetSliderValueFromClick,
    ...
    void UpdateDragElementPosition() {
      // UpdateDragElementPosition() might be called at times where we have no
      // panel we must skip the position calculation and wait for a layout pass
      if (panel == null)
        return;
      // This is the main calculation for the location of the thumbs / dragging
      // element
      float offsetForThumbFullWidth = -dragElement.resolvedStyle.marginLeft -
                                      dragElement.resolvedStyle.marginRight;
      var sliceSpan = dragElement.resolvedStyle.unitySliceLeft +
                      dragElement.resolvedStyle.unitySliceRight;
      var newPositionLeft =
          Mathf.Round(SliderLerpUnclamped(
                          dragElement.resolvedStyle.unitySliceLeft,
                          (visualInput.layout.width + offsetForThumbFullWidth) -
                              dragElement.resolvedStyle.unitySliceRight,
                          SliderNormalizeValue(minValue, lowLimit, highLimit)) -
                      dragElement.resolvedStyle.unitySliceLeft);
      var newPositionRight =
          Mathf.Round(SliderLerpUnclamped(
                          dragElement.resolvedStyle.unitySliceLeft,
                          (visualInput.layout.width + offsetForThumbFullWidth) -
                              dragElement.resolvedStyle.unitySliceRight,
      ...
      dragElement.style.width =
          Mathf.Max(sliceSpan, newPositionRight - newPositionLeft);
      dragElement.style.left = newPositionLeft;

      // Calculate the rect for the mouse selection, in the parent coordinate
      // (MinMaxSlider world) ...
      m_DragMinThumbRect =
          new Rect(dragElement.resolvedStyle.left, dragElement.layout.yMin,
                   dragElement.resolvedStyle.unitySliceLeft,
                   dragElement.resolvedStyle.height);
      m_DragMaxThumbRect = new Rect(
          dragElement.resolvedStyle.left +
              (dragElement.resolvedStyle.width -
               dragElement.resolvedStyle.unitySliceRight),
          dragElement.layout.yMin, dragElement.resolvedStyle.unitySliceRight,
          dragElement.resolvedStyle.height);

      // The child elements are positioned based on the parent (drag element)
      // coordinate... Set up the Max Thumb for the horizontal slider...
      dragMaxThumb.style.left = dragElement.resolvedStyle.width -
                                dragElement.resolvedStyle.unitySliceRight;
      dragMaxThumb.style.top = 0f;
      // The child elements are positioned based on the parent (drag element)
      // coordinate... Same location for both Horizontal / Vertical slider
    ...
    internal float SliderNormalizeValue(float currentValue, float lowerValue,
                                        float higherValue) {
    ...
    float ComputeValueFromPosition(float positionToConvert) {
      ...
      interpolant = SliderNormalizeValue(
          positionToConvert, dragElement.resolvedStyle.unitySliceLeft,
          (visualInput.layout.width -
           dragElement.resolvedStyle.unitySliceRight));
    ...
    void SetSliderValueFromDrag() {
      if (clampedDragger.dragDirection !=
          ClampedDragger<float>.DragDirection.Free)
        return;
    ...
    void SetSliderValueFromClick() {
      ...
      } else if (m_DragMaxThumbRect.Contains(
                     clampedDragger.startMousePosition)) {
        m_DragState = DragState.MaxThumb;
      } else if (dragElement.layout.Contains(
                     clampedDragger.startMousePosition)) {
      ...
      if (m_DragState == DragState.NoThumb) {
        // Jump drag element to current mouse position : the new MAX value is
        // simply where the mouse click is done
        m_DragElementStartPos = new Vector2(clampedDragger.startMousePosition.x,
                                            dragElement.resolvedStyle.top);
        ...
        ComputeValueDragStateNoThumb(
            ...
            (visualInput.layout.width -
             dragElement.resolvedStyle.unitySliceRight),
            m_DragElementStartPos.x);
        // For any future dragging after the initial click, this is a Middle
        // Thumb like drag...
        m_DragState = DragState.MiddleThumb;
    ...
    void ComputeValueDragStateNoThumb(float lowLimitPosition,
                                      float highLimitPosition,
      ...
      // The new position is the MAX value... here, we must keep the distance
      // between min and max the same as it was, since this is just a drag...
    ...
    void ComputeValueFromDraggingThumb(float dragElementStartPos,
                                       float dragElementEndPos) {
