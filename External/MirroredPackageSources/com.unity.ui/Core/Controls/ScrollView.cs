...
namespace UnityEngine.UIElements {
  ...
  // If the ScrollView parent is set to flex-direction row the elements height
  // will not stretch. How measure works : Width is restricted, height is not.
  // content-container is set to overflow: scroll
  //
  // Horizontal
  // ---------------------
  // Require elements with a width. If ScrollView is set to flex-grow elements
  // height stretch else they require a height. If the ScrollView parent is set
  // to flex-direction row the elements height will stretch. How measure works :
  // Height is restricted, width is not. content-container is set to overflow:
  // scroll
  //
  // VerticalAndHorizontal
  // ---------------------
  // Require elements with an height, width will stretch.
  // The difference with the Vertical type is that content will not wrap
  // (white-space has no effect). How measure works : Nothing is restricted, the
  // content-container will stop shrinking so that all the content fit and
  // scrollers will appear. To achieve this content-viewport is set to overflow:
  // scroll and flex-direction: row. content-container is set to flex-direction:
  // column, flex-grow: 1 and align-self:flex-start.
  //
  // This type is more tricky, it requires the content-viewport and
  // content-container to have a different flex-direction. "flex-grow:1" is to
  // make elements stretch horizontally. "align-self:flex-start" prevent the
  // content-container from shrinking below the content size vertically.
  // "overflow:scroll" on the content-viewport and content-container is to not
  // restrict measured elements in any direction.
  ...
  public enum ScrollViewMode {
    ...
    /// If <see cref="ScrollView"/> is set to flex-grow or if it's parent is set
    /// to <see cref="FlexDirection.Row"/> elements height stretch else they
    /// require a height.
    /// </remarks>
    Horizontal,
    /// <summary>
    /// Configure <see cref="ScrollView"/> for vertical and horizontal
    /// scrolling.
    /// </summary>
    /// <remarks>
    /// Require elements with an height.
  ...
  public class ScrollView : VisualElement {
    /// <summary>
    /// Instantiates a <see cref="ScrollView"/> using the data read from a UXML
    /// file.
    /// </summary>
    ...
    public new class UxmlTraits : VisualElement.UxmlTraits {
      ...
      UxmlBoolAttributeDescription m_ShowHorizontal =
          new UxmlBoolAttributeDescription{name = "show-horizontal-scroller"};

      UxmlBoolAttributeDescription m_ShowVertical =
          new UxmlBoolAttributeDescription{name = "show-vertical-scroller"};

      UxmlFloatAttributeDescription m_HorizontalPageSize =
          new UxmlFloatAttributeDescription{name = "horizontal-page-size",
                                            defaultValue =
                                                Scroller.kDefaultPageSize};

      UxmlFloatAttributeDescription m_VerticalPageSize =
          new UxmlFloatAttributeDescription{name = "vertical-page-size",
                                            defaultValue =
                                                Scroller.kDefaultPageSize};
      ...
      UxmlFloatAttributeDescription m_ScrollDecelerationRate =
          new UxmlFloatAttributeDescription{
              name = "scroll-deceleration-rate",
              defaultValue = k_DefaultScrollDecelerationRate};

      UxmlFloatAttributeDescription m_Elasticity =
          new UxmlFloatAttributeDescription{name = "elasticity",
                                            defaultValue = k_DefaultElasticity};

      /// <summary>
      /// Initialize <see cref="ScrollView"/> properties using values from the
      /// attribute bag.
      /// </summary>
      ...
      public override void Init(VisualElement ve, IUxmlAttributes bag,
        ...
        scrollView.horizontalPageSize =
            m_HorizontalPageSize.GetValueFromBag(bag, cc);
        scrollView.verticalPageSize =
            m_VerticalPageSize.GetValueFromBag(bag, cc);
        scrollView.scrollDecelerationRate =
            m_ScrollDecelerationRate.GetValueFromBag(bag, cc);
        scrollView.touchScrollBehavior =
            m_TouchScrollBehavior.GetValueFromBag(bag, cc);
    ...
    internal bool needsHorizontal {
      get {
        return showHorizontal ||
               (contentContainer.layout.width - layout.width > 0);
      }
    }
    ...
    internal bool needsVertical {
      get {
        return showVertical ||
               (contentContainer.layout.height - layout.height > 0);
      }
    }
    ...
    /// <summary>
    /// This property is controlling the scrolling speed of the horizontal
    /// scroller.
    /// </summary>
    ...
    /// <summary>
    /// This property is controlling the scrolling speed of the vertical
    /// scroller.
    /// </summary>
    ...
    private float scrollableWidth {
      get {
        return contentContainer.layout.width - contentViewport.layout.width;
      }
    }

    private float scrollableHeight {
      get {
        return contentContainer.layout.height - contentViewport.layout.height;
      }
    }

    // For inertia: how quickly the scrollView stops from moving after
    // PointerUp.
    ...
    /// </summary>
    /// <remarks>
    /// The deceleration rate is the speed reduction per second. A value of 0.5
    /// halves the speed each second. A value of 0 stops the scrolling
    /// immediately.
    /// </remarks>
    ...
    /// <summary>
    /// The amount of elasticity to use when a user tries to scroll past the
    /// boundaries of the scroll view.
    /// </summary>
    /// <remarks>
    /// Elasticity is only used when <see cref="touchScrollBehavior"/> is set to
    /// Elastic.
    /// </remarks>
    ...
    /// <summary>
    /// The behavior to use when a user tries to scroll past the end of the
    /// ScrollView content using a touch interaction.
    /// </summary>
    public enum TouchScrollBehavior {
      ...
      Unrestricted,
      /// <summary>
      /// The content position can overshoot the ScrollView boundaries, but then
      /// "snaps" back within them.
      /// </summary>
      Elastic,
      /// <summary>
    ...
    public void ScrollTo(VisualElement child) {
      ...
      if (!contentContainer.Contains(child))
        throw new ArgumentException(
            "Cannot scroll to a VisualElement that is not a child of the ScrollView content-container.");
    ...
    private float GetXDeltaOffset(VisualElement child) {
      ...
      if ((childBoundaryMin >= viewMin && childBoundaryMax <= viewMax) ||
          float.IsNaN(childBoundaryMin) || float.IsNaN(childBoundaryMax))
        return 0;

      float deltaDistance = GetDeltaDistance(viewMin, viewMax, childBoundaryMin,
                                             childBoundaryMax);
    ...
    private float GetDeltaDistance(float viewMin, float viewMax,
    ...
    public VisualElement contentViewport {
      get; // Represents the visible part of contentContainer
      private set;
    }
    ...
    public static readonly string viewportUssClassName =
        ussClassName + "__content-viewport";
    /// <summary>
    ...
    public ScrollView(ScrollViewMode scrollViewMode) {
      ...
      m_ContentContainer.RegisterCallback<GeometryChangedEvent>(
          OnGeometryChanged);
      ...
      horizontalScroller =
          new Scroller(defaultMinScrollValue, defaultMaxScrollValue,
                       (value) => {
    ...
    private void OnGeometryChanged(GeometryChangedEvent evt) {
      ...
      // Here, we allow the removal of the scrollbar only in the first layout
      // pass. Addition is always allowed.
      if (evt.layoutPass > 0) {
        needsVerticalCached = needsVerticalCached || verticalScroller.visible;
        needsHorizontalCached =
            needsHorizontalCached || horizontalScroller.visible;
      }
    ...
    // Compute the new scroll view offset from a pointer delta, taking
    // elasticity into account. Low and high limits are the values beyond which
    // the scrollview starts to show resistance to scrolling (elasticity). Low
    // and high hard limits are the values beyond which it is infinitely hard to
    // scroll. The mapping between the normalized pointer delta and normalized
    // scroll view offset delta in the elastic zone is: offsetDelta = 1 - 1 /
    // (pointerDelta + 1)
    private static float
    ComputeElasticOffset(float deltaPointer, float initialScrollOffset,
                         float lowLimit, float hardLowLimit, float highLimit,
                         float hardHighLimit) {
      ...
      initialScrollOffset =
          Mathf.Min(initialScrollOffset, hardHighLimit * .95f);
      ...
      if (initialScrollOffset < lowLimit && hardLowLimit < lowLimit) {
        ...
        delta += deltaPointer;
        // Now it is as if the initial offset was at low limit and the pointer
        // delta was delta.
        ...
        // Merge with deltaPointer
        delta += deltaPointer;
        // Now it is as if the initial offset was at high limit and the pointer
        // delta was delta.
    ...
    private void SpringBack() {
      ...
      if (newOffset.x < m_LowBounds.x) {
        newOffset.x = Mathf.SmoothDamp(newOffset.x, m_LowBounds.x,
                                       ref m_SpringBackVelocity.x, elasticity,
                                       Mathf.Infinity, Time.unscaledDeltaTime);
        if (Mathf.Abs(m_SpringBackVelocity.x) < 1) {
          m_SpringBackVelocity.x = 0;
        }
      } else if (newOffset.x > m_HighBounds.x) {
        newOffset.x = Mathf.SmoothDamp(newOffset.x, m_HighBounds.x,
                                       ref m_SpringBackVelocity.x, elasticity,
                                       Mathf.Infinity, Time.unscaledDeltaTime);
        if (Mathf.Abs(m_SpringBackVelocity.x) < 1) {
          m_SpringBackVelocity.x = 0;
      ...
      if (newOffset.y < m_LowBounds.y) {
        newOffset.y = Mathf.SmoothDamp(newOffset.y, m_LowBounds.y,
                                       ref m_SpringBackVelocity.y, elasticity,
                                       Mathf.Infinity, Time.unscaledDeltaTime);
        if (Mathf.Abs(m_SpringBackVelocity.y) < 1) {
          m_SpringBackVelocity.y = 0;
        }
      } else if (newOffset.y > m_HighBounds.y) {
        newOffset.y = Mathf.SmoothDamp(newOffset.y, m_HighBounds.y,
                                       ref m_SpringBackVelocity.y, elasticity,
                                       Mathf.Infinity, Time.unscaledDeltaTime);
        if (Mathf.Abs(m_SpringBackVelocity.y) < 1) {
          m_SpringBackVelocity.y = 0;
      ...
      if (hasInertia && m_Velocity != Vector2.zero) {
        ...
        if (Mathf.Abs(m_Velocity.x) < 1 ||
            touchScrollBehavior == TouchScrollBehavior.Elastic &&
                (scrollOffset.x < m_LowBounds.x ||
                 scrollOffset.x > m_HighBounds.x)) {
          m_Velocity.x = 0;
        }

        if (Mathf.Abs(m_Velocity.y) < 1 ||
            touchScrollBehavior == TouchScrollBehavior.Elastic &&
                (scrollOffset.y < m_LowBounds.y ||
                 scrollOffset.y > m_HighBounds.y)) {
          m_Velocity.y = 0;
        }
    ...
    void OnPointerDown(PointerDownEvent evt) {
      if (evt.pointerType != PointerType.mouse && evt.isPrimary &&
        ...
        m_LowBounds = new Vector2(
            Mathf.Min(horizontalScroller.lowValue,
                      horizontalScroller.highValue),
            Mathf.Min(verticalScroller.lowValue, verticalScroller.highValue));
        m_HighBounds = new Vector2(
            Mathf.Max(horizontalScroller.lowValue,
                      horizontalScroller.highValue),
    ...
    void OnPointerMove(PointerMoveEvent evt) {
      if (evt.pointerId == m_ScrollingPointerId) {
        ...
        if (touchScrollBehavior == TouchScrollBehavior.Clamped) {
          newScrollOffset =
              m_StartPosition - (new Vector2(evt.position.x, evt.position.y) -
          ...
          Vector2 deltaPointer = new Vector2(evt.position.x, evt.position.y) -
                                 m_PointerStartPosition;
          newScrollOffset.x = ComputeElasticOffset(
              deltaPointer.x, m_StartPosition.x, m_LowBounds.x,
              m_LowBounds.x - contentViewport.resolvedStyle.width,
              m_HighBounds.x,
              m_HighBounds.x + contentViewport.resolvedStyle.width);
          newScrollOffset.y = ComputeElasticOffset(
              deltaPointer.y, m_StartPosition.y, m_LowBounds.y,
              m_LowBounds.y - contentViewport.resolvedStyle.height,
              m_HighBounds.y,
              m_HighBounds.y + contentViewport.resolvedStyle.height);
        } else {
          newScrollOffset =
              m_StartPosition - (new Vector2(evt.position.x, evt.position.y) -
                                 m_PointerStartPosition);
      ...
      if (evt.pointerId == m_ScrollingPointerId) {
        ...
        if (touchScrollBehavior == TouchScrollBehavior.Elastic || hasInertia) {
          ...
          if (m_PostPointerUpAnimation == null) {
            m_PostPointerUpAnimation =
                schedule.Execute(PostPointerUpAnimation).Every(30);
          } else {
    ...
    void UpdateScrollers(bool displayHorizontal, bool displayVertical) {
      float horizontalFactor = contentContainer.layout.width >
                               Mathf.Epsilon ? contentViewport.layout.width /
                                   contentContainer.layout.width : 1f;
      float verticalFactor = contentContainer.layout.height >
                             Mathf.Epsilon ? contentViewport.layout.height /
                                 contentContainer.layout.height : 1f;

      horizontalScroller.Adjust(horizontalFactor);
      verticalScroller.Adjust(verticalFactor);

      // Set availability
      horizontalScroller.SetEnabled(
          contentContainer.layout.width - contentViewport.layout.width > 0);
      verticalScroller.SetEnabled(
          contentContainer.layout.height - contentViewport.layout.height > 0);

      // Expand content if scrollbars are hidden
      contentViewport.style.marginRight =
          displayVertical ? verticalScroller.layout.width : 0;
      horizontalScroller.style.right =
          displayVertical ? verticalScroller.layout.width : 0;
      contentViewport.style.marginBottom =
          displayHorizontal ? horizontalScroller.layout.height : 0;
      verticalScroller.style.bottom =
          displayHorizontal ? horizontalScroller.layout.height : 0;
