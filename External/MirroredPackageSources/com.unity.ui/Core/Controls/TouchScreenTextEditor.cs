namespace UnityEngine.UIElements {
  internal class TouchScreenTextEditorEventHandler : TextEditorEventHandler {
    ...
    public TouchScreenTextEditorEventHandler(TextEditorEngine editorEngine,
    ...
    void PollTouchScreenKeyboard() {
      if (TouchScreenKeyboard.isSupported &&
        ...
        if (m_TouchKeyboardPoller == null) {
          m_TouchKeyboardPoller = (textInputField as VisualElement)
              ?.schedule.Execute(DoPollTouchScreenKeyboard).Every(100);
        } else {
    ...
    void DoPollTouchScreenKeyboard() {
      if (TouchScreenKeyboard.isSupported &&
        ...
        if (textInputField.editorEngine.keyboardOnScreen != null) {
          textInputField.UpdateText(textInputField.CullString(
              textInputField.editorEngine.keyboardOnScreen.text));

          if (!textInputField.isDelayed) {
            textInputField.UpdateValueFromText();
          }

          if (textInputField.editorEngine.keyboardOnScreen.status !=
    ...
    public override void ExecuteDefaultActionAtTarget(EventBase evt) {
      ...
      if (!textInputField.isReadOnly && evt.eventTypeId == mouseEventType &&
        ...
        editorEngine.keyboardOnScreen = TouchScreenKeyboard.Open(
            textInputField.text, TouchScreenKeyboardType.Default,
            true, // autocorrection
            editorEngine.multiline, textInputField.isPasswordField);
