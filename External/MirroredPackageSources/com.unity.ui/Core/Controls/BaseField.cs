...
namespace UnityEngine.UIElements {
  ...
  public abstract class BaseField<TValueType>
    ...
    public static readonly string noLabelVariantUssClassName =
        ussClassName + "--no-label";
    /// <summary>
    /// USS class name of labels in elements of this type, when there is a
    /// dragger attached on them.
    /// </summary>
    ...
    [SerializeField] TValueType m_Value;
    ...
    public virtual TValueType value {
      ...
      set {
        if (!EqualityComparer<TValueType>.Default.Equals(m_Value, value)) {
          if (panel != null) {
            using(ChangeEvent<TValueType> evt =
                      ChangeEvent<TValueType>.GetPooled(m_Value, value)) {
              evt.target = this;
    ...
    /// <summary>
    /// This is the <see cref="Label"/> object that appears beside the input for
    /// the field.
    /// </summary>
    ...
    internal override void OnViewDataReady() {
      ...
      if (m_VisualInput != null) {
        ...
        if (!EqualityComparer<TValueType>.Default.Equals(oldValue, m_Value)) {
          using(ChangeEvent<TValueType> evt =
                    ChangeEvent<TValueType>.GetPooled(oldValue, m_Value)) {
            evt.target = this;
  ...
  /// </summary>
  public class BaseFieldTraits<TValueType, TValueUxmlAttributeType>
      : BaseField<TValueType>.UxmlTraits where TValueUxmlAttributeType
      : TypedUxmlAttributeDescription<TValueType>,
      new () {
    TValueUxmlAttributeType m_Value =
        new TValueUxmlAttributeType{name = "value"};
    ...
    /// <param name="cc">The creation context associated with these
    /// traits.</param>
    public override void Init(VisualElement ve, IUxmlAttributes bag,
                              CreationContext cc) {
      base.Init(ve, bag, cc);
      ((INotifyValueChanged<TValueType>) ve)
          .SetValueWithoutNotify(m_Value.GetValueFromBag(bag, cc));
    }
  }
}
