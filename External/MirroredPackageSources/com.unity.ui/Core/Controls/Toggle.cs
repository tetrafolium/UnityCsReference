...
namespace UnityEngine.UIElements {
  ...
  public class Toggle : BaseField<bool> {
    /// <summary>
    /// Instantiates a <see cref="Toggle"/> using the data read from a UXML
    /// file.
    /// </summary>
    ...
    public new class UxmlTraits
      ...
      /// <summary>
      /// Initialize <see cref="Toggle"/> properties using values from the
      /// attribute bag.
      /// </summary>
    ...
    public Toggle(string label) : base(label, null) {
      ...
      var checkMark = new VisualElement(){name = "unity-checkmark",
                                          pickingMode = PickingMode.Ignore};
      checkMark.AddToClassList(checkmarkUssClassName);
      visualInput.Add(checkMark);

      // The picking mode needs to be Position in order to have the Pseudostate
      // Hover applied...
