...
namespace UnityEngine.UIElements {
  ...
  public class Slider : BaseSlider<float> {
    /// <summary>
    /// Instantiates a <see cref="Slider"/> using the data read from a UXML
    /// file.
    /// </summary>
    ...
    public new class UxmlTraits
      ...
      UxmlFloatAttributeDescription m_PageSize =
          new UxmlFloatAttributeDescription{name = "page-size",
                                            defaultValue = kDefaultPageSize};
      ...
      /// <summary>
      /// Initialize <see cref="Slider"/> properties using values from the
      /// attribute bag.
      /// </summary>
    ...
    /// Constructor.
    /// </summary>
    public Slider() : this((string) null, 0, kDefaultHighValue) {}
    ...
    /// Constructor.
    /// </summary>
    public Slider(string label, float start = 0, float end = kDefaultHighValue,
                  SliderDirection direction = SliderDirection.Horizontal,
                  float pageSize = kDefaultPageSize)
    ...
    internal override float SliderLerpUnclamped(float a, float b,
                                                float interpolant) {
      return Mathf.RoundToInt(Mathf.LerpUnclamped(a, b, interpolant) * 100f) /
             100f;
    }

    internal override float SliderNormalizeValue(float currentValue,
                                                 float lowerValue,
                                                 float higherValue) {
