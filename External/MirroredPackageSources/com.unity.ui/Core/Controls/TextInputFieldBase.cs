...
namespace UnityEngine.UIElements {
  ...
  public abstract class TextInputBaseField<TValueType> : BaseField<TValueType> {
    static CustomStyleProperty<Color> s_SelectionColorProperty =
        new CustomStyleProperty<Color>("--unity-selection-color");
    static CustomStyleProperty<Color> s_CursorColorProperty =
    ...
    public new class UxmlTraits
      ...
      UxmlIntAttributeDescription m_MaxLength = new UxmlIntAttributeDescription{
          name = "max-length", obsoleteNames = new[]{"maxLength"},
          defaultValue = kMaxLengthNone};
      UxmlBoolAttributeDescription m_Password =
          new UxmlBoolAttributeDescription{name = "password"};
      UxmlStringAttributeDescription m_MaskCharacter =
          new UxmlStringAttributeDescription{
              name = "mask-character", obsoleteNames = new[]{"maskCharacter"},
              defaultValue = kMaskCharDefault.ToString()};
      UxmlStringAttributeDescription m_Text =
          new UxmlStringAttributeDescription{name = "text"};
      UxmlBoolAttributeDescription m_IsReadOnly =
          new UxmlBoolAttributeDescription{name = "readonly"};

      /// <summary>
      /// Initialize the traits for this field.
      /// </summary>
      /// <param name="ve">VisualElement to which to apply the
      /// attributes.</param> <param name="bag">Bag of attributes where to get
      /// the attributes.</param> <param name="cc">Creation context.</param>
    ...
    /// <summary>
    /// This is the text input visual element which presents the value in the
    /// field.
    /// </summary>
    ...
    public new static readonly string labelUssClassName =
        ussClassName + "__label";
    /// <summary>
    /// USS class name of input elements in elements of this type.
    /// </summary>
    public new static readonly string inputUssClassName =
        ussClassName + "__input";
    ...
    /// <summary>
    /// If set to true, the value property is not updated until either the user
    /// presses Enter or the text field loses focus.
    /// </summary>
    ...
    internal TextEditorEventHandler editorEventHandler =>
        m_TextInputBase.editorEventHandler;
    ...
    /// </summary>
    public void SelectAll() { m_TextInputBase.SelectAll(); }

    internal void SyncTextEngine() { m_TextInputBase.SyncTextEngine(); }

    internal void DrawWithTextSelectionAndCursor(MeshGenerationContext mgc,
                                                 string newText) {
      m_TextInputBase.DrawWithTextSelectionAndCursor(mgc, newText,
                                                     scaledPixelsPerPoint);
    }

    protected TextInputBaseField(int maxLength, char maskChar,
                                 TextInputBase textInputBase)
        : this(null, maxLength, maskChar, textInputBase) {}

    protected TextInputBaseField(string label, int maxLength, char maskChar,
                                 TextInputBase textInputBase)
    ...
    protected override void ExecuteDefaultActionAtTarget(EventBase evt) {
      ...
      if (evt.eventTypeId == KeyDownEvent.TypeId()) {
        ...
        // We must handle the ETX (char 3) or the \n instead of the KeypadEnter
        // or Return because the focus will
        //     have the drawback of having the second event to be handled by the
        //     focused field.
    ...
    protected abstract class TextInputBase : VisualElement, ITextInputField {
      ...
      void RestoreValueAndText() { text = m_OriginalText; }

      /// <summary>
      /// Selects all the text contained in the field.
      /// </summary>
      public void SelectAll() { editorEngine?.SelectAll(); }

      internal void SelectNone() { editorEngine?.SelectNone(); }

      private void UpdateText(string value) {
        if (text != value) {
          // Setting the VisualElement text here cause a repaint since it dirty
          // the layout flag.
      ...
      internal void UpdateValueFromText() {
        ...
        TextInputBaseField<TValueType> parentTextField =
            (TextInputBaseField<TValueType>) parent;
      ...
      internal bool hasFocus {
        get {
          return elementPanel != null &&
                 elementPanel.focusController.GetLeafFocusedElement() == this;
        }
      }
      ...
      internal TextInputBase() {
        ...
        editorEngine =
            new TextEditorEngine(OnDetectFocusChange, OnCursorIndexChange);

        if (touchScreenTextField) {
          editorEventHandler =
              new TouchScreenTextEditorEventHandler(editorEngine, this);
        } else {
          ...
          editorEventHandler =
              new KeyboardTextEditorEventHandler(editorEngine, this);
        }
      ...
      DropdownMenuAction.Status CutCopyActionStatus(DropdownMenuAction a) {
        return (editorEngine.hasSelection && !isPasswordField)
                   ? DropdownMenuAction.Status.Normal
                   : DropdownMenuAction.Status.Disabled;
      }

      DropdownMenuAction.Status PasteActionStatus(DropdownMenuAction a) {
        return (editorEngine.CanPaste() ? DropdownMenuAction.Status.Normal
                                        : DropdownMenuAction.Status.Disabled);
      }

      void ProcessMenuCommand(string command) {
        using(ExecuteCommandEvent evt =
                  ExecuteCommandEvent.GetPooled(command)) {
          evt.target = this;
          SendEvent(evt);
        }
      }
      ...
      private void OnCustomStyleResolved(CustomStyleResolvedEvent e) {
        ...
        if (customStyle.TryGetValue(s_SelectionColorProperty,
                                    out selectionValue))
      ...
      private void OnAttachToPanel(AttachToPanelEvent e) {
        m_TextHandle.useLegacy =
            e.destinationPanel.contextType == ContextType.Editor;
      }
      ...
      internal void OnGenerateVisualContent(MeshGenerationContext mgc) {
        ...
        if (touchScreenTextField) {
          var touchScreenEditor =
              editorEventHandler as TouchScreenTextEditorEventHandler;
          if (touchScreenEditor != null) {
            mgc.Text(MeshGenerationContextUtils.TextParams.MakeStyleBased(
                         this, drawText),
                     m_TextHandle, scaledPixelsPerPoint);
          }
        } else {
          if (!hasFocus) {
            mgc.Text(MeshGenerationContextUtils.TextParams.MakeStyleBased(
      ...
      internal void DrawWithTextSelectionAndCursor(MeshGenerationContext mgc,
        ...
        var playmodeTintColor =
            panel.contextType == ContextType.Editor
                                    ? UIElementsUtility
                                     .editorPlayModeTintColor : Color.white;

        var keyboardTextEditor =
            editorEventHandler as KeyboardTextEditorEventHandler;
        ...
        float textScaling =
            TextHandle.ComputeTextScaling(worldTransform, pixelsPerPoint);

        var textParams =
            MeshGenerationContextUtils.TextParams.MakeStyleBased(this, text);
        textParams.text = " ";
        textParams.wordWrapWidth = 0.0f;
        textParams.wordWrap = false;

        float lineHeight =
            m_TextHandle.ComputeTextHeight(textParams, textScaling);

        float wordWrapWidth = 0.0f;

        // Make sure to take into account the word wrap style...
        if (editorEngine.multiline &&
            (resolvedStyle.whiteSpace == WhiteSpace.Normal)) {
        ...
        int selectionEndIndex =
            string.IsNullOrEmpty(GUIUtility.compositionString)
                ? selectIndex
        ...
        if ((cursorIndex != selectionEndIndex) && !isDragging) {
          int min =
              cursorIndex < selectionEndIndex ? cursorIndex : selectionEndIndex;
          int max =
              cursorIndex > selectionEndIndex ? cursorIndex : selectionEndIndex;

          cursorParams =
              CursorPositionStylePainterParameters.GetDefault(this, text);
          cursorParams.text = editorEngine.text;
          cursorParams.wordWrapWidth = wordWrapWidth;
          cursorParams.cursorIndex = min;

          Vector2 minPos =
              m_TextHandle.GetCursorPosition(cursorParams, textScaling);
          ...
          if (Mathf.Approximately(minPos.y, maxPos.y)) {
            mgc.Rectangle(new MeshGenerationContextUtils.RectangleParams(){
                rect = new Rect(minPos.x, minPos.y, maxPos.x - minPos.x,
                                lineHeight),
                color = selectionColor, playmodeTintColor = playmodeTintColor});
          } else {
            // Draw first line
            mgc.Rectangle(new MeshGenerationContextUtils.RectangleParams(){
                rect = new Rect(minPos.x, minPos.y, contentRect.xMax - minPos.x,
                                lineHeight),
                color = selectionColor, playmodeTintColor = playmodeTintColor});
            ...
            if (inbetweenHeight > 0f) {
              ...
              mgc.Rectangle(new MeshGenerationContextUtils.RectangleParams(){
                  rect = new Rect(contentRect.xMin, minPos.y + lineHeight,
                                  contentRect.width, inbetweenHeight),
            ...
            if (maxPos.x != contentRect.x) {
              mgc.Rectangle(new MeshGenerationContextUtils.RectangleParams(){
                  rect = new Rect(contentRect.xMin, maxPos.y, maxPos.x,
                                  lineHeight),
                  color = selectionColor,
                  playmodeTintColor = playmodeTintColor});
            }
        ...
        if (!string.IsNullOrEmpty(editorEngine.text) &&
            contentRect.width > 0.0f && contentRect.height > 0.0f) {
          textParams =
              MeshGenerationContextUtils.TextParams.MakeStyleBased(this, text);
          textParams.rect = new Rect(contentRect.x - scrollOffset.x,
                                     contentRect.y - scrollOffset.y,
                                     contentRect.width + scrollOffset.x,
                                     contentRect.height + scrollOffset.y);
        ...
        if (!isReadOnly && !isDragging) {
          if (cursorIndex == selectionEndIndex &&
              computedStyle.unityFont.value != null) {
            cursorParams =
                CursorPositionStylePainterParameters.GetDefault(this, text);
            cursorParams.text = editorEngine.text;
            cursorParams.wordWrapWidth = wordWrapWidth;
            ...
            mgc.Rectangle(new MeshGenerationContextUtils.RectangleParams{
                rect = new Rect(cursorPosition.x, cursorPosition.y, 1f,
                                lineHeight),
          ...
          if (editorEngine.altCursorPosition != -1) {
            cursorParams =
                CursorPositionStylePainterParameters.GetDefault(this, text);
            cursorParams.text =
                editorEngine.text.Substring(0, editorEngine.altCursorPosition);
            cursorParams.wordWrapWidth = wordWrapWidth;
            cursorParams.cursorIndex = editorEngine.altCursorPosition;

            Vector2 altCursorPosition =
                m_TextHandle.GetCursorPosition(cursorParams, textScaling);
            altCursorPosition -= scrollOffset;
            mgc.Rectangle(new MeshGenerationContextUtils.RectangleParams{
                rect = new Rect(altCursorPosition.x, altCursorPosition.y, 1f,
                                lineHeight),
                color = drawCursorColor,
                playmodeTintColor = playmodeTintColor});
          }
      ...
      protected internal override Vector2 DoMeasure(float desiredWidth,
        ...
        // If the text is empty, we should make sure it returns at least the
        // height/width of 1 character...
        var textToUse = m_Text;
        if (string.IsNullOrEmpty(textToUse)) {
          textToUse = " ";
        }

        return TextElement.MeasureVisualElementTextSize(
            this, textToUse, desiredWidth, widthMode, desiredHeight, heightMode,
            m_TextHandle);
      }
      ...
      protected override void ExecuteDefaultActionAtTarget(EventBase evt) {
        ...
        if (elementPanel != null &&
            elementPanel.contextualMenuManager != null) {
          elementPanel.contextualMenuManager.DisplayMenuIfEventMatches(evt,
                                                                       this);
        }
      ...
      void ITextInputField.SyncTextEngine() { SyncTextEngine(); }

      bool ITextInputField.AcceptCharacter(char c) {
        return AcceptCharacter(c);
      }

      string ITextInputField.CullString(string s) { return CullString(s); }

      void ITextInputField.UpdateText(string value) { UpdateText(value); }

      TextEditorEngine ITextInputField.editorEngine => editorEngine;

      bool ITextInputField.isDelayed => isDelayed;

      void ITextInputField.UpdateValueFromText() { UpdateValueFromText(); }
      ...
      private static void SyncGUIStyle(TextInputBase textInput,
                                       GUIStyle style) {
        ...
        bool overflowVisible =
            computedStyle.overflow.value == OverflowInternal.Visible;
        style.clipping =
            overflowVisible ? TextClipping.Overflow : TextClipping.Clip;
      ...
      private static void AssignRect(RectOffset rect, int left, int top,
