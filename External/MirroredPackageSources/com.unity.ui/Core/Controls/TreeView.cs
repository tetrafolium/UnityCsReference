  ...
  internal class TreeView : VisualElement {
    ...
    public new class UxmlTraits : VisualElement.UxmlTraits {
      ...
      public override void Init(VisualElement ve, IUxmlAttributes bag,
        ...
        ((TreeView) ve).selectionType =
            m_SelectionType.GetValueFromBag(bag, cc);
        ((TreeView) ve).showAlternatingRowBackgrounds =
    ...
    public TreeView() {
      ...
      RegisterCallback<MouseUpEvent>(OnTreeViewMouseUp,
                                     TrickleDown.TrickleDown);
      RegisterCallback<CustomStyleResolvedEvent>(OnCustomStyleResolved);
    }

    public TreeView(IList<ITreeViewItem> items, int itemHeight,
    ...
    public static IEnumerable<ITreeViewItem>
    GetAllItems(IEnumerable<ITreeViewItem> rootItems) {
      if (rootItems == null)
        yield break;
    ...
    public void SetSelection(int id) { SetSelection(new[]{id}); }
    ...
    internal void SetSelectionInternal(IEnumerable<int> ids,
    ...
    private int GetItemIndex(int id, bool expand = false) {
      ...
      if (item == null)
        throw new ArgumentOutOfRangeException(
            nameof(id), id, $"{nameof(TreeView)}: Item id not found.");
    ...
    public bool IsExpanded(int id) { return m_ExpandedItemIds.Contains(id); }
    ...
    private void ListViewRefresh() { m_ListView.Refresh(); }
    ...
    private VisualElement MakeTreeItem() {
      var itemContainer = new VisualElement(){
          name = s_ItemName, style = {flexDirection = FlexDirection.Row}};
      itemContainer.AddToClassList(s_ItemName);
      itemContainer.RegisterCallback<MouseUpEvent>(OnItemMouseUp);

      var indents =
          new VisualElement(){name = s_ItemIndentsContainerName,
      ...
      var userContentContainer = new VisualElement(){
          name = s_ItemContentContainerName, style = {flexGrow = 1}};
    ...
    private void UnbindTreeItem(VisualElement element, int index) {
      ...
      var item = m_ItemWrappers[index].item;
      var userContentContainer =
          element.Q(s_ItemContentContainerName).ElementAt(0);
    ...
    private int GetItemId(int index) { return m_ItemWrappers[index].id; }
    ...
    private void CollapseItemByIndex(int index) {
      ...
      while (currentIndex < m_ItemWrappers.Count &&
             m_ItemWrappers[currentIndex].depth > currentDepth) {
        recursiveChildCount++;
        currentIndex++;
      }
    ...
    private void ExpandItemByIndex(int index) {
      ...
      CreateWrappers(m_ItemWrappers[index].item.children,
                     m_ItemWrappers[index].depth + 1, ref childWrappers);
    ...
    private void CreateWrappers(IEnumerable<ITreeViewItem> treeViewItems,
                                int depth,
                                ref List<TreeViewItemWrapper> wrappers) {
    ...
    private void OnCustomStyleResolved(CustomStyleResolvedEvent e) {
      ...
      if (!m_ListView.m_ItemHeightIsInline &&
          e.customStyle.TryGetValue(ListView.s_ItemHeightProperty, out height))
