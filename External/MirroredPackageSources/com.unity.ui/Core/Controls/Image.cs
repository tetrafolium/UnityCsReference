...
namespace UnityEngine.UIElements {
  ...
  public class Image : VisualElement {
    /// <summary>
    /// Instantiates an <see cref="Image"/> using the data read from a UXML
    /// file.
    /// </summary>
    ...
    /// <summary>
    /// The texture to display in this image.  You cannot set this and <see
    /// cref="Image.vectorImage"/> at the same time.
    /// </summary>
    ...
    /// <summary>
    /// The <see cref="VectorImage"/> to display in this image.  You cannot set
    /// this and <see cref="Image.image"/> at the same time.
    /// </summary>
    ...
    /// <summary>
    /// The base texture coordinates of the Image relative to the bottom left
    /// corner.
    /// </summary>
    public Rect uv {
      get { return m_UV; }
    ...
    private void OnGenerateVisualContent(MeshGenerationContext mgc) {
      ...
      if (image != null)
        rectParams = MeshGenerationContextUtils.RectangleParams.MakeTextured(
            contentRect, uv, image, scaleMode, panel.contextType);
      else if (vectorImage != null)
        rectParams =
            MeshGenerationContextUtils.RectangleParams.MakeVectorTextured(
                contentRect, uv, vectorImage, scaleMode, panel.contextType);
      rectParams.color = tintColor;
      mgc.Rectangle(rectParams);
    }

    static CustomStyleProperty<Texture2D> s_ImageProperty =
        new CustomStyleProperty<Texture2D>("--unity-image");
    static CustomStyleProperty<VectorImage> s_VectorImageProperty =
        new CustomStyleProperty<VectorImage>("--unity-image");
    static CustomStyleProperty<string> s_ScaleModeProperty =
        new CustomStyleProperty<string>("--unity-image-size");
    static CustomStyleProperty<Color> s_TintColorProperty =
        new CustomStyleProperty<Color>("--unity-image-tint-color");

    private void OnCustomStyleResolved(CustomStyleResolvedEvent e) {
      // We should consider not exposing image as a style at all, since it's
      // intimately tied to uv/sourceRect
      ...
      if (!m_ImageIsInline &&
          customStyle.TryGetValue(s_ImageProperty, out textureValue)) {
        m_Image = textureValue;
        if (m_Image != null)
          m_VectorImage = null;
      }

      if (!m_ImageIsInline && customStyle.TryGetValue(s_VectorImageProperty,
                                                      out vectorImageValue)) {
        m_VectorImage = vectorImageValue;
        if (m_VectorImage != null)
          m_Image = null;
      }

      if (!m_ScaleModeIsInline &&
          customStyle.TryGetValue(s_ScaleModeProperty, out scaleModeValue)) {
        m_ScaleMode = (ScaleMode) StylePropertyUtil.GetEnumIntValue(
            StyleEnumType.ScaleMode, scaleModeValue);
      }

      if (!m_TintColorIsInline &&
          customStyle.TryGetValue(s_TintColorProperty, out tintValue))
