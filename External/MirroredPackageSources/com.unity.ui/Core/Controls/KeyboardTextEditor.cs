...
namespace UnityEngine.UIElements {
  internal class KeyboardTextEditorEventHandler : TextEditorEventHandler {
    ...
    public KeyboardTextEditorEventHandler(TextEditorEngine editorEngine,
    ...
    void OnMouseDown(MouseDownEvent evt) {
      ...
      if (!textInputField.hasFocus) {
        ...
        editorEngine.MoveCursorToPosition_Internal(
            evt.localMousePosition,
            evt.button == (int) MouseButton.LeftMouse && evt.shiftKey);
        ...
        if (evt.clickCount == 2 && textInputField.doubleClickSelectsWord) {
          ...
          m_DragToPosition = false;
                   ...
                   textInputField.tripleClickSelectsLine) {
          editorEngine.SelectCurrentParagraph();
          editorEngine.MouseDragSelectsWholeWords(true);
          editorEngine.DblClickSnap(TextEditor.DblClickSnapping.PARAGRAPHS);
          m_DragToPosition = false;
        } else {
          editorEngine.MoveCursorToPosition_Internal(evt.localMousePosition,
                                                     evt.shiftKey);
    ...
    void OnMouseMove(MouseMoveEvent evt) {
      ...
      // FIXME: presing shift while dragging will change start of selection (alt
      // cursor). Also, adding to selection (with shift click) after a
      // drag-select does not work: it clears the previous selection.
    ...
    void OnKeyDown(KeyDownEvent evt) {
      ...
      if (editorEngine.HandleKeyEvent(evt.imguiEvent,
        ...
        // When the newline character is sent, we have to check if the shift key
        // is down also... In the multiline case, this is like a return on a
        // single line
        ...
        // On windows, key presses also send events with keycode but no
        // character. Eat them up here.
        else if (c == 0) {
    ...
    public void PreDrawCursor(string newText) {
      ...
      if (!string.IsNullOrEmpty(GUIUtility.compositionString)) {
        editorEngine.text = newText.Substring(0, editorEngine.cursorIndex) +
                            GUIUtility.compositionString +
                            newText.Substring(editorEngine.selectIndex);
      ...
      editorEngine.graphicalCursorPos =
          editorEngine.style.GetCursorPixelPosition(
              editorEngine.localPosition, new GUIContent(editorEngine.text),
              cursorPos);
    }

    public void PostDrawCursor() { editorEngine.text = m_PreDrawCursorText; }
  }
}
