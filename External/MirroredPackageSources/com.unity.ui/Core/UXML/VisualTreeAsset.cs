...
namespace UnityEngine.UIElements {
  /// <summary>
  /// An instance of this class holds a tree of `VisualElementAsset`s, created
  /// from a UXML file. Each node in the file corresponds to a
  /// `VisualElementAsset`. You can clone a `VisualTreeAsset` to yield a tree of
  /// `VisualElement`s.
  /// </summary>
  [Serializable]
...
#pragma warning restore 0649
      ...
      get {
        ...
        foreach (var entry in m_Usings) {
          if (entry.asset != null && !sent.Contains(entry.asset)) {
            ...
            var vta = Panel.LoadResource(entry.path, typeof(VisualTreeAsset),
    ...
    /// </summary>
    /// <returns>The root of the tree of VisualElements that was just
    /// cloned.</returns>
    ...
    /// Build a tree of VisualElements from the asset.
    /// </summary>
    /// <param name="bindingPath">The path to the property that you want to bind
    /// to the root of the cloned tree.</param> <returns>The root of the tree of
    /// VisualElements that was just cloned.</returns>
    ...
    /// </summary>
    /// <remarks>
    /// This function will be deprecated. Use <see
    /// cref="VisualElement.Instantiate"/> instead.
    /// </remarks>
    /// <returns>The root of the tree of VisualElements that was just
    /// cloned.</returns>
    public TemplateContainer CloneTree() { return Instantiate(); }
    ...
    /// </summary>
    /// <param name="bindingPath">The path to the property that you want to bind
    /// to the root of the cloned tree.</param> <remarks> This function will be
    /// deprecated. Use <see cref="VisualElement.Instantiate"/> instead.
    /// </remarks>
    /// <returns>The root of the tree of VisualElements that was just
    /// cloned.</returns>
    ...
    /// </summary>
    /// <param name="target">A VisualElement that will act as the root of the
    /// cloned tree.</param>
    ...
    public void CloneTree(VisualElement target, out int firstElementIndex,
                          out int elementAddedCount) {
      if (target == null)
    ...
    CloneTree(VisualElement target,
      ...
      Dictionary<int, List<VisualElementAsset>> idToChildren =
          new Dictionary<int, List<VisualElementAsset>>();
      ...
      for (int i = 0; i < eltcount + tplcount; i++) {
        VisualElementAsset asset =
            i < eltcount ? visualElementAssets[i] : templateAssets[i - eltcount];
      ...
      // Get the first-level elements. These will be instantiated and added to
      // target.
      rootAssets.Clear();
      ...
      foreach (var rootElement in rootAssets) {
        ...
        VisualElement rootVe = CloneSetupRecursively(
            rootElement, idToChildren,
            new CreationContext(slotInsertionPoints, attributeOverrides, this,
                                target));

        if (rootVe != null) {
          // if contentContainer == this, the shadow and the logical hierarchy
          // are identical otherwise, if there is a CC, we want to insert in the
          // shadow
          target.hierarchy.Add(rootVe);
        } else {
          Debug.LogWarning(
              "VisualTreeAsset instantiated an empty UI. Check the syntax of your UXML document.");
        }
      }
    }

    private VisualElement CloneSetupRecursively(
        VisualElementAsset root,
        Dictionary<int, List<VisualElementAsset>> idToChildren,
      ...
      // if the current element had a slot-name attribute, put it in the
      // resulting slot mapping
      string slotName;
      if (context.slotInsertionPoints != null &&
          TryGetSlotInsertionPoint(root.id, out slotName)) {
      ...
      if (idToChildren.TryGetValue(root.id, out children)) {
        ...
        foreach (VisualElementAsset childVea in children) {
          ...
          VisualElement childVe =
              CloneSetupRecursively(childVea, idToChildren, context);
          if (childVe == null)
            continue;

          // if the parent is not a template asset, just add the child to
          // whatever hierarchy we currently have if ve is a scrollView (with
          // contentViewport as contentContainer), this will go to the right
          // place
          ...
          int index = templateAsset.slotUsages == null
                            ? - 1
              : templateAsset.slotUsages.FindIndex(u => u.assetId ==
                                                        childVea.id);
          if (index != -1) {
    ...
    static int CompareForOrder(VisualElementAsset a, VisualElementAsset b) =>
        a.orderInDocument.CompareTo(b.orderInDocument);
    ...
    internal bool AddSlotDefinition(string slotName, int resId) {
      ...
      m_Slots.Add(
          new SlotDefinition{insertionPointId = resId, name = slotName});
      return true;
    }

    internal bool TryGetSlotInsertionPoint(int insertionPointId,
                                           out string slotName) {
      if (m_Slots == null) {
        slotName = null;
        return false;
    ...
    internal VisualTreeAsset ResolveTemplate(string templateName) {
      if (m_Usings == null || m_Usings.Count == 0)
        return null;
      int index = m_Usings.BinarySearch(
          new UsingEntry(templateName, string.Empty), UsingEntry.comparer);
      if (index < 0)
        return null;
      ...
      return Panel.LoadResource(path, typeof(VisualTreeAsset),
                                GUIUtility.pixelsPerPoint) as VisualTreeAsset;
    }

    internal bool TemplateExists(string templateName) {
      if (m_Usings == null || m_Usings.Count == 0)
        return false;
      var index = m_Usings.BinarySearch(
          new UsingEntry(templateName, string.Empty), UsingEntry.comparer);
      return index >= 0;
    }
    ...
    private void InsertUsingEntry(UsingEntry entry) {
      ...
      int i = 0;
      while (i < m_Usings.Count &&
             String.CompareOrdinal(entry.alias, m_Usings[i].alias) > 0)
        i++;

      m_Usings.Insert(i, entry);
    }

    internal static VisualElement Create(VisualElementAsset asset,
                                         CreationContext ctx) {
      List<IUxmlFactory> factoryList;
      if (!VisualElementFactoryRegistry.TryGetValue(asset.fullTypeName,
                                                    out factoryList)) {
        if (asset.fullTypeName.StartsWith(
                "UnityEngine.Experimental.UIElements.") ||
            asset.fullTypeName.StartsWith(
                "UnityEditor.Experimental.UIElements.")) {
          string experimentalTypeName = asset.fullTypeName.Replace(
              ".Experimental.UIElements", ".UIElements");
          if (!VisualElementFactoryRegistry.TryGetValue(experimentalTypeName,
                                                        out factoryList)) {
            Debug.LogErrorFormat(
                "Element '{0}' has no registered factory method.",
                asset.fullTypeName);
            return new Label(
                string.Format("Unknown type: '{0}'", asset.fullTypeName));
          }
        } else if (asset.fullTypeName == UxmlRootElementFactory.k_ElementName) {
          // Support UXML without namespace for backward compatibility.
          VisualElementFactoryRegistry.TryGetValue(
              typeof(UxmlRootElementFactory).Namespace + "." +
                  asset.fullTypeName,
              out factoryList);
        } else {
          Debug.LogErrorFormat(
      ...
      if (factory == null) {
        Debug.LogErrorFormat(
            "Element '{0}' has a no factory that accept the set of XML attributes specified.",
            asset.fullTypeName);
        return new Label(
            string.Format("Type with no factory: '{0}'", asset.fullTypeName));
      }
    ...
    static void AssignClassListFromAssetToElement(VisualElementAsset asset,
    ...
    static void AssignStyleSheetFromAssetToElement(VisualElementAsset asset,
                                                   VisualElement element) {
  ...
  public struct CreationContext : IEquatable<CreationContext> {
    ...
    internal
    CreationContext(Dictionary<string, VisualElement> slotInsertionPoints,
    ...
    public bool Equals(CreationContext other) {
      return EqualityComparer<VisualElement>.Default.Equals(target,
                                                            other.target) &&
             EqualityComparer<VisualTreeAsset>.Default.Equals(
                 visualTreeAsset, other.visualTreeAsset) &&
             EqualityComparer<Dictionary<string, VisualElement>>.Default.Equals(
                 slotInsertionPoints, other.slotInsertionPoints);
    }

    public override int GetHashCode() {
      var hashCode = -2123482148;
      hashCode = hashCode * -1521134295 +
                 EqualityComparer<VisualElement>.Default.GetHashCode(target);
      hashCode = hashCode * -1521134295 +
                 EqualityComparer<VisualTreeAsset>.Default.GetHashCode(
                     visualTreeAsset);
      hashCode =
          hashCode * -1521134295 +
          EqualityComparer<Dictionary<string, VisualElement>>.Default.GetHashCode(
              slotInsertionPoints);
      return hashCode;
    }

    public static bool operator ==(CreationContext context1,
                                   CreationContext context2) {
