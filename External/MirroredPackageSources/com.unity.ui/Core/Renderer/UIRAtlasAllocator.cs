...
namespace UnityEngine.UIElements {
  internal class UIRAtlasAllocator : IDisposable {
    private class Row {
      ...
      /// <summary>
      /// Distance between the left of the row and the horizontal insertion
      /// point.
      /// </summary>
      public int Cursor;

      public static Row Acquire(int offsetX, int offsetY, int width,
                                int height) {
        Row row = s_Pool.Get();
        row.offsetX = offsetX;
        row.offsetY = offsetY;
    ...
    private class AreaNode {
      ...
      public void Release() { s_Pool.Release(this); }
    ...
    static ProfilerMarker s_MarkerTryAllocate =
        new ProfilerMarker("UIRAtlasAllocator.TryAllocate");
...
#region Dispose Pattern
    ...
    public UIRAtlasAllocator(int initialAtlasSize, int maxAtlasSize,
      ...
      maxImageHeight = (initialAtlasSize == maxAtlasSize)
                           ? maxAtlasSize / 2 + m_2SidePadding
                           : maxAtlasSize / 4 + m_2SidePadding;
    ...
    public bool TryAllocate(int width, int height, out RectInt location) {
      using(s_MarkerTryAllocate.Auto()) {
        ...
        // Rows have height of POT+2. The reason for the +2 is that textures are
        // generally POT, but the border adds 2 pixels. Accounting for the
        // border produced better packing because it avoids, for example,
        // fitting a 16x16 pixel (which turns into 18x18) into a row with a
        // height of 32. So the actual heights of the rows are: 3, 4, 6, 10, 18,
        // 34, 66, etc...
        int rowIndex =
            GetLog2OfNextPower(Mathf.Max(height - m_2SidePadding, 1));
        ...
        // Allocate in the row.
        location =
            new RectInt(row.offsetX + row.Cursor, row.offsetY, width, height);
        row.Cursor += width;
        Assert.IsTrue(row.Cursor <= row.width);
        physicalWidth =
            Mathf.NextPowerOfTwo(Mathf.Max(physicalWidth, location.xMax));
        physicalHeight =
            Mathf.NextPowerOfTwo(Mathf.Max(physicalHeight, location.yMax));
    ...
    /// <remarks>
    /// If the provided area is large enough, the lower part of the area is
    /// partitioned to host the row and the lower bound of the area moves
    /// upwards. When the area becomes empty, it is removed from the chain of
    /// unpartitioned areas to avoid spending time on it during future
    /// allocations.
    /// </remarks>
    private bool TryPartitionArea(AreaNode areaNode, int rowIndex,
                                  int rowHeight, int minWidth) {
    ...
    /// </summary>
    /// <remarks>
    /// The areas are built by doubling horizontally and vertically, over and
    /// over. Consider the following example, where the initial atlas size is
    /// 64x64 and the maximum size is 256x256. The resulting 5 areas would be:
    ...
    ///
    /// Because of the exponential nature of this process, we don't need to
    /// worry about very large textures. For example, from 64x64 to 4096x4096,
    /// only 12 iterations are required, leading to 13 areas.
    /// </remarks>
