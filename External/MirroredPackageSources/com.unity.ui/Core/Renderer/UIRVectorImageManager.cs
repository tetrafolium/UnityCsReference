...
namespace UnityEngine.UIElements.UIR {
  class VectorImageRenderInfoPool : LinkedPool<VectorImageRenderInfo> {
    public VectorImageRenderInfoPool()
        : base(() => new VectorImageRenderInfo(),
               vectorImageInfo => vectorImageInfo.Reset()) {}
  }
  ...
  class GradientRemapPool : LinkedPool<GradientRemap> {
    public GradientRemapPool()
        : base(() => new GradientRemap(),
               gradientRemap => gradientRemap.Reset()) {}
  }
  ...
  class VectorImageManager : IDisposable {
    static ProfilerMarker s_MarkerRegister =
        new ProfilerMarker("UIR.VectorImageManager.Register");
    static ProfilerMarker s_MarkerUnregister =
        new ProfilerMarker("UIR.VectorImageManager.Unregister");
    ...
    public Texture2D atlas {
      get { return m_GradientSettingsAtlas?.atlas; }
    }
...
#region Reset Pattern
    ...
    VectorImageRenderInfo Register(VectorImage vi) {
      ...
      if (vi.settings?.Length > 0) {
        // We first attempt to allocate into the gradient settings atlas since
        // it supports deallocation.
        ...
        if (alloc.size > 0) {
          ...
          if (m_AtlasManager.TryGetLocation(vi.atlas, out uvs)) {
            ...
            // Write into the previously allocated gradient settings now that we
            // are sure to use it.
            m_GradientSettingsAtlas.Write(alloc, vi.settings,
                                          renderInfo.firstGradientRemap);
          } else {
            // If the texture atlas didn't fit, keep it as a standalone custom
            // texture, only need to remap the setting indices
            GradientRemap previous = null;
            for (int i = 0; i < gradientCount; ++i) {
              GradientRemap current = m_GradientRemapPool.Get();
              if (i > 0)
          ...
          if (!m_LoggedExhaustedSettingsAtlas) {
            Debug.LogError("Exhausted max gradient settings (" +
                           m_GradientSettingsAtlas.length + ") for atlas: " +
                           m_GradientSettingsAtlas.atlas?.name);
