...
namespace UnityEngine.UIElements.UIR.Implementation {
  ...
  internal static class RenderEvents {
    internal static void ProcessOnClippingChanged(RenderChain renderChain,
                                                  VisualElement ve,
                                                  uint dirtyID,
                                                  ref ChainBuilderStats stats) {
      bool hierarchical = (ve.renderChainData.dirtiedValues &
                           RenderDataDirtyTypes.ClippingHierarchy) != 0;
      if (hierarchical)
        stats.recursiveClipUpdates++;
      else
        stats.nonRecursiveClipUpdates++;
      DepthFirstOnClippingChanged(renderChain, ve.hierarchy.parent, ve, dirtyID,
                                  hierarchical, true, false, false, false,
                                  renderChain.device, ref stats);
    }

    internal static void ProcessOnOpacityChanged(RenderChain renderChain,
                                                 VisualElement ve, uint dirtyID,
                                                 ref ChainBuilderStats stats) {
      stats.recursiveOpacityUpdates++;
      DepthFirstOnOpacityChanged(
          renderChain,
          ve.hierarchy.parent !=
              null ? ve.hierarchy.parent.renderChainData.compositeOpacity : 1.0f,
          ve, dirtyID, ref stats);
    }

    internal static void
    ProcessOnTransformOrSizeChanged(RenderChain renderChain, VisualElement ve,
                                    uint dirtyID, ref ChainBuilderStats stats) {
      stats.recursiveTransformUpdates++;
      DepthFirstOnTransformOrSizeChanged(renderChain, ve.hierarchy.parent, ve,
                                         dirtyID, renderChain.device, false,
                                         false, ref stats);
    }

    internal static void ProcessOnVisualsChanged(RenderChain renderChain,
                                                 VisualElement ve, uint dirtyID,
                                                 ref ChainBuilderStats stats) {
      bool hierarchical = (ve.renderChainData.dirtiedValues &
                           RenderDataDirtyTypes.VisualsHierarchy) != 0;
      if (hierarchical)
        stats.recursiveVisualUpdates++;
      else
        stats.nonRecursiveVisualUpdates++;
      var parent = ve.hierarchy.parent;
      var parentHierarchyHidden =
          parent != null && (parent.renderChainData.isHierarchyHidden ||
                             IsElementHierarchyHidden(parent));
      DepthFirstOnVisualsChanged(renderChain, ve, dirtyID,
                                 parentHierarchyHidden, hierarchical,
                                 ref stats);
    }

    internal static void ProcessRegenText(RenderChain renderChain,
                                          VisualElement ve,
                                          UIRTextUpdatePainter painter,
    ...
    static Matrix4x4 GetTransformIDTransformInfo(VisualElement ve) {
      Debug.Assert(
          RenderChainVEData.AllocatesID(ve.renderChainData.transformID) ||
          (ve.renderHints & (RenderHints.GroupTransform)) != 0);
      Matrix4x4 transform;
      if (ve.renderChainData.groupTransformAncestor != null)
        transform =
            ve.renderChainData.groupTransformAncestor.worldTransform.inverse *
            ve.worldTransform;
      else
        transform = ve.worldTransform;
      transform.m22 = transform.m33 =
          1.0f; // Once world-space mode is introduced, this should become
                // conditional
      return transform;
    }
    ...
    static Vector4 GetClipRectIDClipInfo(VisualElement ve) {
      Debug.Assert(
          RenderChainVEData.AllocatesID(ve.renderChainData.clipRectID));
      if (ve.renderChainData.groupTransformAncestor == null)
      ...
      var transform =
          ve.renderChainData.groupTransformAncestor.worldTransform.inverse;
      var min =
          transform.MultiplyPoint3x4(new Vector3(rect.xMin, rect.yMin, 0));
      var max =
          transform.MultiplyPoint3x4(new Vector3(rect.xMax, rect.yMax, 0));

      return new Vector4(Mathf.Min(min.x, max.x), Mathf.Min(min.y, max.y),
                         Mathf.Max(min.x, max.x), Mathf.Max(min.y, max.y));
    }

    static void GetVerticesTransformInfo(VisualElement ve,
      ...
      if (RenderChainVEData.AllocatesID(ve.renderChainData.transformID) ||
        ...
        transform =
            ve.renderChainData.boneTransformAncestor.worldTransform.inverse *
            ve.worldTransform;
    ...
    internal static uint DepthFirstOnChildAdded(RenderChain renderChain,
                                                VisualElement parent,
                                                VisualElement ve, int index,
                                                bool resetState) {
      ...
      ve.renderChainData.transformID =
          UIRVEShaderInfoAllocator.identityTransform;
      ...
      ve.renderChainData.compositeOpacity =
          float.MaxValue; // Any unreasonable value will do to trip the opacity
                          // composer to work
      ...
      if (parent != null) {
        if ((parent.renderHints & (RenderHints.GroupTransform)) != 0)
          ...
          ve.renderChainData.groupTransformAncestor =
              parent.renderChainData.groupTransformAncestor;
        ve.renderChainData.hierarchyDepth =
            parent.renderChainData.hierarchyDepth + 1;
      } else {
      ...
      if (index > 0) {
        ...
        ve.renderChainData.prev =
            GetLastDeepestChild(parent.hierarchy[index - 1]);
      } else
        ve.renderChainData.prev = parent;
      ve.renderChainData.next =
          ve.renderChainData.prev !=
          null ? ve.renderChainData.prev.renderChainData.next : null;
      ...
      // TransformID
      // Since transform type is controlled by render hints which are locked on
      // the VE by now, we can go ahead and prep transform data now and never
      // check on it again under regular circumstances
      Debug.Assert(
          !RenderChainVEData.AllocatesID(ve.renderChainData.transformID));
      if (NeedsTransformID(ve))
        ve.renderChainData.transformID =
            renderChain.shaderInfoAllocator
                .AllocTransform(); // May fail, that's ok
      else
      ...
      if (!RenderChainVEData.AllocatesID(ve.renderChainData.transformID)) {
        if (parent != null &&
          ...
          if (RenderChainVEData.AllocatesID(parent.renderChainData.transformID))
            ...
            ve.renderChainData.boneTransformAncestor =
                parent.renderChainData.boneTransformAncestor;

          ve.renderChainData.transformID = parent.renderChainData.transformID;
          ve.renderChainData.transformID.ownedState =
              OwnedState.Inherited; // Mark this allocation as not owned by us
                                    // (inherited)
        } else
          ve.renderChainData.transformID =
              UIRVEShaderInfoAllocator.identityTransform;
      } else
        renderChain.shaderInfoAllocator.SetTransformValue(
            ve.renderChainData.transformID, GetTransformIDTransformInfo(ve));

      // Recurse on children
      int childrenCount = ve.hierarchy.childCount;
      uint deepCount = 0;
      for (int i = 0; i < childrenCount; i++)
        deepCount += DepthFirstOnChildAdded(renderChain, ve, ve.hierarchy[i], i,
                                            resetState);
      return 1 + deepCount;
    }

    internal static uint DepthFirstOnChildRemoving(RenderChain renderChain,
                                                   VisualElement ve) {
      // Recurse on children
      int childrenCount = ve.hierarchy.childCount - 1;
      uint deepCount = 0;
      while (childrenCount >= 0)
        deepCount += DepthFirstOnChildRemoving(renderChain,
                                               ve.hierarchy[childrenCount--]);
      ...
      if (ve.renderChainData.isInChain) {
        ...
        if (ve.renderChainData.next != null)
          ve.renderChainData.next.renderChainData.prev =
              ve.renderChainData.prev;
        if (ve.renderChainData.prev != null)
          ve.renderChainData.prev.renderChainData.next =
              ve.renderChainData.next;

        if (RenderChainVEData.AllocatesID(ve.renderChainData.opacityID)) {
          renderChain.shaderInfoAllocator.FreeOpacity(
              ve.renderChainData.opacityID);
          ve.renderChainData.opacityID = UIRVEShaderInfoAllocator.fullOpacity;
        }
        if (RenderChainVEData.AllocatesID(ve.renderChainData.clipRectID)) {
          renderChain.shaderInfoAllocator.FreeClipRect(
              ve.renderChainData.clipRectID);
          ve.renderChainData.clipRectID =
              UIRVEShaderInfoAllocator.infiniteClipRect;
        }
        if (RenderChainVEData.AllocatesID(ve.renderChainData.transformID)) {
          renderChain.shaderInfoAllocator.FreeTransform(
              ve.renderChainData.transformID);
          ve.renderChainData.transformID =
              UIRVEShaderInfoAllocator.identityTransform;
        }
    ...
    static void DepthFirstOnClippingChanged(
        RenderChain renderChain, VisualElement parent, VisualElement ve,
        uint dirtyID, bool hierarchical,
        ...
        UIRenderDevice device, ref ChainBuilderStats stats) {
      bool upToDate = dirtyID == ve.renderChainData.dirtyID;
      if (upToDate && !inheritedClipRectIDChanged &&
          !inheritedStencilClippedChanged)
        return;

      ve.renderChainData.dirtyID =
          dirtyID; // Prevent reprocessing of the same element in the same pass

      if (!isRootOfChange)
        stats.recursiveClipUpdatesExpanded++;

      isPendingHierarchicalRepaint |=
          (ve.renderChainData.dirtiedValues &
           RenderDataDirtyTypes.VisualsHierarchy) != 0;

      // Internal operations (done in this call) to do:
      bool mustUpdateClipRectID =
          hierarchical || isRootOfChange || inheritedClipRectIDChanged;
      bool mustUpdateClippingMethod = hierarchical || isRootOfChange;
      bool mustUpdateStencilClippedFlag =
          hierarchical || isRootOfChange || inheritedStencilClippedChanged;
      ...
      // mustRecurse implies recursing on all children, but doesn't force
      // anything beyond them. hierarchical implies recursing on all descendants
      ...
      ClipMethod newClippingMethod =
          mustUpdateClippingMethod ? DetermineSelfClipMethod(renderChain, ve)
          : oldClippingMethod;
      ...
      if (mustUpdateClipRectID) {
        ...
        if (newClippingMethod == ClipMethod.ShaderDiscard) {
          if (!RenderChainVEData.AllocatesID(ve.renderChainData.clipRectID)) {
            ...
            if (!newClipRectID.IsValid()) {
              newClippingMethod =
                  ClipMethod.Scissor; // Fallback to scissor since we couldn't
                                      // allocate a clipRectID
              // Both shader discard and scisorring work with world-clip
              // rectangles, so no need to inherit any clipRectIDs for such
              // elements, our own scissor rect clips up correctly
          ...
          // Group transforms shouldn't inherit the clipRectID since they have a
          // new frame of reference, they provide a new baseline with the
          // _PixelClipRect instead.
          if ((ve.renderHints & RenderHints.GroupTransform) == 0) {
            newClipRectID =
                ((newClippingMethod != ClipMethod.Scissor) && (parent != null))
                    ? parent.renderChainData.clipRectID
                    : UIRVEShaderInfoAllocator.infiniteClipRect;
        ...
        clipRectIDChanged =
            !ve.renderChainData.clipRectID.Equals(newClipRectID);
        Debug.Assert((ve.renderHints & RenderHints.GroupTransform) == 0 ||
                     !clipRectIDChanged);
      ...
      if (oldClippingMethod != newClippingMethod) {
        ...
        if (oldClippingMethod == ClipMethod.Stencil ||
            newClippingMethod == ClipMethod.Stencil) {
      ...
      if (mustUpdateStencilClippedFlag) {
        ...
        bool newStencilClipped =
            newClippingMethod == ClipMethod.Stencil ||
            (parent != null && parent.renderChainData.isStencilClipped);
      ...
      if ((mustRepaintThis || mustRepaintHierarchy) &&
          !isPendingHierarchicalRepaint) {
      ...
      if (mustRecurse) {
        ...
        for (int i = 0; i < childrenCount; i++)
          DepthFirstOnClippingChanged(
              renderChain, ve, ve.hierarchy[i], dirtyID,
              // Having to recurse doesn't mean that we need to process ALL
              // descendants. For example, the propagation of the transformId
              // may stop if a group or a bone is encountered.
              hierarchical, false, isPendingHierarchicalRepaint,
              clipRectIDChanged, isStencilClippedChanged, device, ref stats);
      }
    }

    static void
    DepthFirstOnOpacityChanged(RenderChain renderChain,
                               float parentCompositeOpacity, VisualElement ve,
                               uint dirtyID, ref ChainBuilderStats stats,
                               bool isDoingFullVertexRegeneration = false) {
      ...
      bool compositeOpacityChanged =
          Mathf.Abs(oldOpacity - newOpacity) > 0.0001f;
      if (compositeOpacityChanged) {
        // Avoid updating cached opacity if it changed too little, because we
        // don't want slow changes to update the cache and never trigger the
        // compositeOpacityChanged condition.
        ve.renderChainData.compositeOpacity = newOpacity;
      }

      bool changedOpacityID = false;
      bool hasDistinctOpacity =
          newOpacity <
          parentCompositeOpacity - 0.0001f; // assume 0 <= opacity <= 1
      if (hasDistinctOpacity) {
        if (ve.renderChainData.opacityID.ownedState == OwnedState.Inherited) {
          changedOpacityID = true;
          ve.renderChainData.opacityID =
              renderChain.shaderInfoAllocator.AllocOpacity();
        }

        if ((changedOpacityID || compositeOpacityChanged) &&
            ve.renderChainData.opacityID.IsValid())
          renderChain.shaderInfoAllocator.SetOpacityValue(
              ve.renderChainData.opacityID, newOpacity);
      } else if (ve.renderChainData.opacityID.ownedState ==
                 OwnedState.Inherited) {
        // Just follow my parent's alloc
        if (ve.hierarchy.parent != null &&
            !ve.renderChainData.opacityID.Equals(
                ve.hierarchy.parent.renderChainData.opacityID)) {
          changedOpacityID = true;
          ve.renderChainData.opacityID =
              ve.hierarchy.parent.renderChainData.opacityID;
        ...
        // I have an owned allocation, but I must match my parent's opacity,
        // just set the opacity rather than free and inherit our parent's
      ...
      } else if (oldOpacity < Mathf.Epsilon &&
                 newOpacity >= Mathf.Epsilon) // became visible
      {
        renderChain.UIEOnVisualsChanged(
            ve, true); // Force a full vertex regeneration, as this element was
                       // considered as hidden from the hierarchy
        isDoingFullVertexRegeneration = true;
      } else if (changedOpacityID && ((ve.renderChainData.dirtiedValues &
                                       RenderDataDirtyTypes.Visuals) == 0)) {
        renderChain.UIEOnVisualsChanged(
            ve, false); // Changed opacity ID, must update vertices.. we don't
                        // do it hierarchical here since our children will go
                        // through this too
      }
      ...
      if (compositeOpacityChanged || changedOpacityID) {
        ...
        int childrenCount = ve.hierarchy.childCount;
        for (int i = 0; i < childrenCount; i++) {
          DepthFirstOnOpacityChanged(renderChain, newOpacity, ve.hierarchy[i],
                                     dirtyID, ref stats,
    ...
    static void DepthFirstOnTransformOrSizeChanged(
        RenderChain renderChain, VisualElement parent, VisualElement ve,
        uint dirtyID, UIRenderDevice device, bool isAncestorOfChangeSkinned,
        bool transformChanged, ref ChainBuilderStats stats) {
      if (dirtyID == ve.renderChainData.dirtyID)
        return;

      stats.recursiveTransformUpdatesExpanded++;

      transformChanged |= (ve.renderChainData.dirtiedValues &
                           RenderDataDirtyTypes.Transform) != 0;

      if (RenderChainVEData.AllocatesID(ve.renderChainData.clipRectID))
        renderChain.shaderInfoAllocator.SetClipRectValue(
            ve.renderChainData.clipRectID, GetClipRectIDClipInfo(ve));
      ...
      if (RenderChainVEData.AllocatesID(ve.renderChainData.transformID)) {
        ...
        // Only the clip info had to be updated, we can skip the other cases
        // which are for transform changes only.
      } else if ((ve.renderHints & RenderHints.GroupTransform) != 0) {
        stats.groupTransformElementsChanged++;
      } else if (isAncestorOfChangeSkinned) {
        // Children of a bone element inherit the transform data change
        // automatically when the root updates that data, no need to do anything
        // for children
        Debug.Assert(RenderChainVEData.InheritsID(
            ve.renderChainData
                .transformID)); // The element MUST have a transformID that has
                                // been inherited from an ancestor
        dirtyHasBeenResolved =
            false; // We just skipped processing, if another later transform
                   // change is queued on this element this pass then we should
                   // still process it
        stats.skipTransformed++;
      } else if ((ve.renderChainData.dirtiedValues &
                  (RenderDataDirtyTypes.Visuals |
                   RenderDataDirtyTypes.VisualsHierarchy)) == 0 &&
                 (ve.renderChainData.data != null)) {
        // If a visual update will happen, then skip work here as the visual
        // update will incorporate the transformed vertices
        if (!ve.renderChainData.disableNudging &&
            NudgeVerticesToNewSpace(ve, device))
          stats.nudgeTransformed++;
        else {
          renderChain.UIEOnVisualsChanged(
              ve, false); // Nudging not allowed, so do a full visual repaint
          stats.visualUpdateTransformed++;
        }
      }

      if (dirtyHasBeenResolved)
        ve.renderChainData.dirtyID = dirtyID; // Prevent reprocessing of the
                                              // same element in the same pass

      // Make sure to pre-evaluate world transform and clip now so we don't do
      // it at render time
      ...
      if ((ve.renderHints & RenderHints.GroupTransform) == 0) {
        ...
        for (int i = 0; i < childrenCount; i++)
          DepthFirstOnTransformOrSizeChanged(
              renderChain, ve, ve.hierarchy[i], dirtyID, device,
              isAncestorOfChangeSkinned, transformChanged, ref stats);
      } else
        renderChain.OnGroupTransformElementChangedTransform(
            ve); // Hack until UIE moves to TMP
    }

    static void DepthFirstOnVisualsChanged(RenderChain renderChain,
                                           VisualElement ve, uint dirtyID,
                                           bool parentHierarchyHidden,
                                           bool hierarchical,
                                           ref ChainBuilderStats stats) {
      if (dirtyID == ve.renderChainData.dirtyID)
        return;
      ...
      ve.renderChainData.isHierarchyHidden =
          parentHierarchyHidden || IsElementHierarchyHidden(ve);
      ...
      Debug.Assert(
          ...
          ve.renderChainData.transformID.Equals(
              ve.hierarchy.parent.renderChainData.transformID) ||
          (ve.renderHints & RenderHints.GroupTransform) != 0);

      UIRStylePainter.ClosingInfo closingInfo =
          PaintElement(renderChain, ve, ref stats);

      if (hierarchical) {
        // Recurse on children
        int childrenCount = ve.hierarchy.childCount;
        for (int i = 0; i < childrenCount; i++)
          DepthFirstOnVisualsChanged(renderChain, ve.hierarchy[i], dirtyID,
                                     ve.renderChainData.isHierarchyHidden, true,
                                     ref stats);
      }

      // By closing the element after its children, we can ensure closing data
      // is allocated at a time that would maintain continuity in the index
      // buffer
    ...
    static bool IsElementHierarchyHidden(VisualElement ve) {
      return ve.resolvedStyle.opacity < Mathf.Epsilon ||
    ...
    static VisualElement GetLastDeepestChild(VisualElement ve) {
      // O(n) of the visual tree depth, usually not too bad.. probably 10-15 in
      // really bad cases
    ...
    static bool IsParentOrAncestorOf(this VisualElement ve,
                                     VisualElement child) {
    ...
    static ClipMethod DetermineSelfClipMethod(RenderChain renderChain,
                                              VisualElement ve) {
      if (!ve.ShouldClip())
        return ClipMethod.NotClipped;

      if (!UIRUtility.IsRoundRect(ve) &&
          !UIRUtility.IsVectorImageBackground(ve)) {
        if ((ve.renderHints &
             (RenderHints.GroupTransform | RenderHints.ClipWithScissors)) != 0)
          return ClipMethod.Scissor;
        return ClipMethod.ShaderDiscard;
      }

      if (ve.hierarchy.parent?.renderChainData.isStencilClipped == true)
        return ClipMethod.ShaderDiscard; // Prevent nested stenciling for now,
                                         // even if inaccurate

      // Stencil clipping is not yet supported in world-space rendering,
      // fallback to a coarse shader discard for now
      return renderChain.drawInCameras ? ClipMethod.ShaderDiscard
          : ClipMethod.Stencil;
    }

    static bool NeedsTransformID(VisualElement ve) {
      return ((ve.renderHints & RenderHints.GroupTransform) == 0) &&
             ((ve.renderHints & RenderHints.BoneTransform) ==
              RenderHints.BoneTransform);
    }

    // Indicates whether the transform id assigned to an element has changed. It
    // does not care who the owner is.
    ...
    internal static UIRStylePainter.ClosingInfo
    PaintElement(RenderChain renderChain, VisualElement ve,
                 ref ChainBuilderStats stats) {
      var isClippingWithStencil =
          ve.renderChainData.clipMethod == ClipMethod.Stencil;
      if ((IsElementSelfHidden(ve) && !isClippingWithStencil) ||
          ve.renderChainData.isHierarchyHidden) {
        if (ve.renderChainData.data != null) {
      ...
      // Retain our command insertion points if possible, to avoid paying the
      // cost of finding them again
      ...
      bool commandsAndClosingCommandsWereConsecutive =
          (ve.renderChainData.firstClosingCommand != null) &&
          (oldCmdNext == ve.renderChainData.firstClosingCommand);
      ...
      if (ve.visible) {
        ...
        // Even though the element hidden, we still have to push the stencil
        // shape in case any children are visible.
      ...
      if (painter.totalVertices > 1 << 16) {
        Debug.LogError(
            $"A {nameof(VisualElement)} must not allocate more than 65536 vertices.");
      ...
      if (entries.Count > 0) {
        ...
        if (painter.totalVertices > 0)
          UpdateOrAllocate(ref data, painter.totalVertices,
                           painter.totalIndices, painter.device, out verts,
                           out indices, out indexOffset, ref stats);
          ...
          if (entry.vertices.Length > 0 && entry.indices.Length > 0) {
            if (!vertexDataComputed) {
              ...
              ve.renderChainData.verticesSpace =
                  transform; // This is the space for the generated vertices
                             // below

              Color32 transformData =
                  renderChain.shaderInfoAllocator.TransformAllocToVertexData(
                      ve.renderChainData.transformID);
              Color32 opacityData =
                  renderChain.shaderInfoAllocator.OpacityAllocToVertexData(
                      ve.renderChainData.opacityID);
            ...
            Color32 clipRectData =
                renderChain.shaderInfoAllocator.ClipRectAllocToVertexData(
                    entry.clipRectID);
            ...
            var targetVerticesSlice =
                verts.Slice(vertsFilled, entry.vertices.Length);
            ...
            if (entry.uvIsDisplacement) {
              if (firstDisplacementUV < 0) {
                ...
                ve.renderChainData.disableNudging =
                    true; // Disjoint displacement UV entries, we can't keep
                          // track of them, so disable nudging optimization
                          // altogether

              CopyTransformVertsPosAndVec(entry.vertices, targetVerticesSlice,
                                          transform, xformClipPages,
                                          idsAddFlags, opacityPage);
            } else
              CopyTransformVertsPos(entry.vertices, targetVerticesSlice,
                                    transform, xformClipPages, idsAddFlags,
                                    opacityPage);

            // Copy indices
            int entryIndexCount = entry.indices.Length;
            int entryIndexOffset = vertsFilled + indexOffset;
            var targetIndicesSlice =
                indices.Slice(indicesFilled, entryIndexCount);
            if (entry.isClipRegisterEntry || !entry.isStencilClipped)
              CopyTriangleIndices(entry.indices, targetIndicesSlice,
                                  entryIndexOffset);
            else
              CopyTriangleIndicesFlipWindingOrder(
                  entry.indices, targetIndicesSlice,
                  entryIndexOffset); // Flip winding order if we're
                                     // stencil-clipped

            if (entry.isClipRegisterEntry)
              painter.LandClipRegisterMesh(
                  targetVerticesSlice, targetIndicesSlice, entryIndexOffset);

            var cmd =
                InjectMeshDrawCommand(renderChain, ve, ref cmdPrev, ref cmdNext,
                                      data, entryIndexCount, indicesFilled,
                                      entry.material, entry.custom, entry.font);
            if (entry.isTextEntry && ve.renderChainData.usesLegacyText) {
              if (ve.renderChainData.textEntries == null)
                ve.renderChainData.textEntries =
                    new List<RenderChainTextEntry>(1);
              ve.renderChainData.textEntries.Add(new RenderChainTextEntry(){
                  command = cmd, firstVertex = vertsFilled,
            ...
            InjectCommandInBetween(renderChain, entry.customCommand,
                                   ref cmdPrev, ref cmdNext);
          } else {
            Debug.Assert(false); // Unable to determine what kind of command to
                                 // generate here
          }
        }
      ...
      if (painter.closingInfo.needsClosing) {
        RenderChainCommand cmdPrev = oldClosingCmdPrev,
                           cmdNext = oldClosingCmdNext;
        if (commandsAndClosingCommandsWereConsecutive) {
        ...
        if (painter.closingInfo.clipperRegisterIndices.Length > 0)
          painter.LandClipUnregisterMeshDrawCommand(
              InjectClosingMeshDrawCommand(
                  renderChain, ve, ref cmdPrev, ref cmdNext, null, 0, 0, null,
                  null, null)); // Placeholder command that will be filled
                                // actually later
        if (painter.closingInfo.popViewMatrix) {
          ...
          cmd.closing = true;
          cmd.owner = ve;
          InjectClosingCommandInBetween(renderChain, cmd, ref cmdPrev,
                                        ref cmdNext);
        }
    ...
    static void ClosePaintElement(VisualElement ve,
                                  UIRStylePainter.ClosingInfo closingInfo,
      ...
      if (closingInfo.clipperRegisterIndices.Length > 0) {
        ...
        // Due to device Update limitations, we cannot share the vertices of the
        // registration mesh. It would be great if we can just point
        // winding-flipped indices towards the same vertices as the registration
        // mesh. For now, we duplicate the registration mesh entirely, wasting a
        // bit of vertex memory
        UpdateOrAllocate(ref ve.renderChainData.closingData,
                         closingInfo.clipperRegisterVertices.Length,
                         closingInfo.clipperRegisterIndices.Length,
                         renderChain.painter.device, out verts, out indices,
                         out indexOffset, ref stats);
        verts.CopyFrom(closingInfo.clipperRegisterVertices);
        CopyTriangleIndicesFlipWindingOrder(
            closingInfo.clipperRegisterIndices, indices,
            indexOffset - closingInfo.clipperRegisterIndexOffset);
        closingInfo.clipUnregisterDrawCommand.mesh =
            ve.renderChainData.closingData;
        closingInfo.clipUnregisterDrawCommand.indexCount = indices.Length;
      }
    }

    static void UpdateOrAllocate(ref MeshHandle data, int vertexCount,
                                 int indexCount, UIRenderDevice device,
                                 out NativeSlice<Vertex> verts,
                                 out NativeSlice<UInt16> indices,
                                 out UInt16 indexOffset,
                                 ref ChainBuilderStats stats) {
      if (data != null) {
        // Try to fit within the existing allocation, optionally we can change
        // the condition to be an exact match of size to guarantee continuity in
        // draw ranges
        if (data.allocVerts.size >= vertexCount &&
            data.allocIndices.size >= indexCount) {
          device.Update(data, (uint) vertexCount, (uint) indexCount, out verts,
                        out indices, out indexOffset);
          stats.updatedMeshAllocations++;
        } else {
          // Won't fit in the existing allocated region, free the current one
          device.Free(data);
          data = device.Allocate((uint) vertexCount, (uint) indexCount,
                                 out verts, out indices, out indexOffset);
          stats.newMeshAllocations++;
        }
      } else {
        data = device.Allocate((uint) vertexCount, (uint) indexCount, out verts,
                               out indices, out indexOffset);
        stats.newMeshAllocations++;
      }
    }

    static void CopyTransformVertsPos(NativeSlice<Vertex> source,
                                      NativeSlice<Vertex> target, Matrix4x4 mat,
    ...
    static void CopyTransformVertsPosAndVec(
        NativeSlice<Vertex> source, NativeSlice<Vertex> target, Matrix4x4 mat,
        Color32 xformClipPages, Color32 idsAddFlags, Color32 opacityPage) {
      int count = source.Length;
    ...
    static void
    CopyTriangleIndicesFlipWindingOrder(NativeSlice<UInt16> source,
                                        NativeSlice<UInt16> target) {
      Debug.Assert(
          source !=
          target); // Not a very robust assert, but readers get the point
    ...
    static void CopyTriangleIndicesFlipWindingOrder(NativeSlice<UInt16> source,
                                                    NativeSlice<UInt16> target,
                                                    int indexOffset) {
      Debug.Assert(
          source !=
    ...
    static void CopyTriangleIndices(NativeSlice<UInt16> source,
                                    NativeSlice<UInt16> target,
                                    int indexOffset) {
      int indexCount = source.Length;
    ...
    static bool NudgeVerticesToNewSpace(VisualElement ve,
                                        UIRenderDevice device) {
      Debug.Assert(!ve.renderChainData.disableNudging);

      Matrix4x4 newTransform;
      GetVerticesTransformInfo(ve, out newTransform);
      Matrix4x4 nudgeTransform =
          newTransform * ve.renderChainData.verticesSpace.inverse;

      // Attempt to reconstruct the absolute transform. If the result diverges
      // from the absolute considerably, then we assume that the vertices have
      // become degenerate beyond restoration. In this case we refuse to nudge,
      // and ask for this element to be fully repainted to regenerate the
      // vertices without error.
      const float kMaxAllowedDeviation = 0.0001f;
      Matrix4x4 reconstructedNewTransform =
          nudgeTransform * ve.renderChainData.verticesSpace;
      float error;
      ...
      ve.renderChainData.verticesSpace =
          newTransform; // This is the new space of the vertices

      int vertCount = (int) ve.renderChainData.data.allocVerts.size;
      NativeSlice<Vertex> oldVerts =
          ve.renderChainData.data.allocPage.vertices.cpuData.Slice(
              (int) ve.renderChainData.data.allocVerts.start, vertCount);
      ...
      for (int i = vertsBeforeUVDisplacement; i < vertsAfterUVDisplacement;
           i++) {
        var v = oldVerts[i];
        v.position = nudgeTransform.MultiplyPoint3x4(v.position);
    ...
    InjectMeshDrawCommand(RenderChain renderChain, VisualElement ve,
                          ref RenderChainCommand cmdPrev,
                          ref RenderChainCommand cmdNext, MeshHandle mesh,
                          int indexCount, int indexOffset, Material material,
                          Texture custom, Texture font) {
      var cmd = renderChain.AllocCommand();
      cmd.type = CommandType.Draw;
      cmd.state =
          new State(){material = material, custom = custom, font = font};
      cmd.mesh = mesh;
    ...
    static RenderChainCommand InjectClosingMeshDrawCommand(
        RenderChain renderChain, VisualElement ve,
        ref RenderChainCommand cmdPrev, ref RenderChainCommand cmdNext,
        MeshHandle mesh, int indexCount, int indexOffset, Material material,
        Texture custom, Texture font) {
      var cmd = renderChain.AllocCommand();
      cmd.type = CommandType.Draw;
      cmd.closing = true;
    ...
    static void FindCommandInsertionPoint(VisualElement ve,
                                          out RenderChainCommand prev,
                                          out RenderChainCommand next) {
      VisualElement prevDrawingElem = ve.renderChainData.prev;

      // This can be potentially O(n) of VE count
      // It is ok to check against lastCommand to mean the presence of
      // closingCommand too, as we require that closing commands only exist if a
      // startup command exists too
      while (prevDrawingElem != null &&
             prevDrawingElem.renderChainData.lastCommand == null)
        prevDrawingElem = prevDrawingElem.renderChainData.prev;

      if (prevDrawingElem != null &&
          prevDrawingElem.renderChainData.lastCommand != null) {
        // A previous drawing element can be:
        // A) A previous sibling (O(1) check time)
        // B) A parent/ancestor (O(n) of tree depth check time - meh)
        // C) A child/grand-child of a previous sibling to an ancestor (lengthy
        // check time, so it is left as the only choice remaining after the
        // first two)
        if (prevDrawingElem.hierarchy.parent == ve.hierarchy.parent) // Case A
          ...
          // Case C, get the last command that isn't owned by us, this is to
          // skip potential closing commands wrapped after the previous drawing
          // element
          var lastCommand =
              prevDrawingElem.renderChainData.lastClosingOrLastCommand;
          for (;;) {
            prev = lastCommand;
            lastCommand = lastCommand.next;
            if (lastCommand == null || (lastCommand.owner == ve) ||
                !lastCommand
                     .closing) // Once again, we assume closing commands cannot
                               // exist without opening commands on the element
              break;
        ...
        while (nextDrawingElem != null &&
               nextDrawingElem.renderChainData.firstCommand == null)
    ...
    static void FindClosingCommandInsertionPoint(VisualElement ve,
                                                 out RenderChainCommand prev,
                                                 out RenderChainCommand next) {
      // Closing commands for a visual element come after the closing commands
      // of the shallowest child If not found, then after the last command of
      // the last deepest child If not found, then after the last command of
      // self

      VisualElement nextDrawingElem = ve.renderChainData.next;

      // Depth first search for the first VE that has a command (i.e. non empty
      // element). This can be potentially O(n) of VE count It is ok to check
      // against lastCommand to mean the presence of closingCommand too, as we
      // require that closing commands only exist if a startup command exists
      // too
      while (nextDrawingElem != null &&
             nextDrawingElem.renderChainData.firstCommand == null)
        nextDrawingElem = nextDrawingElem.renderChainData.next;

      if (nextDrawingElem != null &&
          nextDrawingElem.renderChainData.firstCommand != null) {
        ...
        // C) A next sibling of a parent/ancestor (lengthy check time, so it is
        // left as the only choice remaining after the first two)
        ...
        } else if (ve.IsParentOrAncestorOf(nextDrawingElem)) // Case B
        {
          ...
          for (;;) {
            ...
            if (nextDrawingElem == null ||
                !ve.IsParentOrAncestorOf(nextDrawingElem))
              break;
          }
          next = prev.next;
        } else {
        ...
        prev = ve.renderChainData.lastCommand;
        next = prev.next; // prev should not be null since we don't support
                          // closing commands without opening commands too
      }
    }

    static void InjectCommandInBetween(RenderChain renderChain,
                                       RenderChainCommand cmd,
                                       ref RenderChainCommand prev,
                                       ref RenderChainCommand next) {
      if (prev != null) {
        cmd.prev = prev;
        prev.next = cmd;
      }
    ...
    static void InjectClosingCommandInBetween(RenderChain renderChain,
                                              RenderChainCommand cmd,
                                              ref RenderChainCommand prev,
                                              ref RenderChainCommand next) {
    ...
    static void ResetCommands(RenderChain renderChain, VisualElement ve) {
      if (ve.renderChainData.firstCommand != null)
        renderChain.OnRenderCommandsRemoved(ve.renderChainData.firstCommand,
                                            ve.renderChainData.lastCommand);

      var prev = ve.renderChainData.firstCommand !=
                 null ? ve.renderChainData.firstCommand.prev : null;
      var next = ve.renderChainData.lastCommand !=
                 null ? ve.renderChainData.lastCommand.next : null;
      Debug.Assert(prev == null || prev.owner != ve);
      Debug.Assert(next == null ||
                   next == ve.renderChainData.firstClosingCommand ||
                   next.owner != ve);
      if (prev != null)
        prev.next = next;
      if (next != null)
      ...
      prev = ve.renderChainData.firstClosingCommand !=
             null ? ve.renderChainData.firstClosingCommand.prev : null;
      next = ve.renderChainData.lastClosingCommand !=
             null ? ve.renderChainData.lastClosingCommand.next : null;
      Debug.Assert(prev == null || prev.owner != ve);
      ...
      if (ve.renderChainData.firstClosingCommand != null) {
        renderChain.OnRenderCommandsRemoved(
            ve.renderChainData.firstClosingCommand,
            ve.renderChainData.lastClosingCommand);
      ...
      ve.renderChainData.firstClosingCommand =
          ve.renderChainData.lastClosingCommand = null;
  ...
  internal class UIRStylePainter : IStylePainter, IDisposable {
    ...
    internal struct TempDataAlloc<T> : IDisposable where T : struct {
      int maxPoolElemCount; // Requests larger than this will potentially be
                            // served individually without pooling
      NativeArray<T> pool;
      ...
      internal NativeSlice<T> Alloc(uint count) {
        ...
        var exceeding =
            new NativeArray<T>((int) count, Allocator.Persistent,
      ...
      internal void SessionDone() {
        ...
        if (totalNewSize > pool.Length) {
          if (pool.IsCreated)
            pool.Dispose();
          pool = new NativeArray<T>(totalNewSize, Allocator.Persistent,
                                    NativeArrayOptions.UninitializedMemory);
        }
        takenFromPool = 0;
      }
    ...
    MeshWriteData
    AllocRawVertsIndices(uint vertexCount, uint indexCount,
                         ref MeshBuilder.AllocMeshData allocatorData) {
    ...
    MeshWriteData
    AllocThroughDrawMesh(uint vertexCount, uint indexCount,
                         ref MeshBuilder.AllocMeshData allocatorData) {
      return DrawMesh((int) vertexCount, (int) indexCount,
                      allocatorData.texture, allocatorData.material,
                      allocatorData.flags);
    }
    ...
    public UIRStylePainter(RenderChain renderChain) {
      ...
      m_MeshWriteDataPool =
          new List<MeshWriteData>(meshWriteDataPoolStartingSize);
...
#region Dispose Pattern
    ...
    public void Begin(VisualElement ve) {
      ...
      currentElement.renderChainData.usesLegacyText =
          currentElement.renderChainData.usesAtlas =
              currentElement.renderChainData.disableNudging = false;
      currentElement.renderChainData.displacementUVStart =
          currentElement.renderChainData.displacementUVEnd = 0;
      bool isGroupTransform =
          (currentElement.renderHints & RenderHints.GroupTransform) != 0;
      if (isGroupTransform) {
      ...
      if (currentElement.hierarchy.parent != null) {
        m_StencilClip =
            currentElement.hierarchy.parent.renderChainData.isStencilClipped;
        m_ClipRectID = isGroupTransform ? UIRVEShaderInfoAllocator
                           .infiniteClipRect
            : currentElement.hierarchy.parent.renderChainData.clipRectID;
      } else {
        m_StencilClip = false;
    ...
    public void LandClipRegisterMesh(NativeSlice<Vertex> vertices,
    ...
    public MeshWriteData DrawMesh(int vertexCount, int indexCount,
                                  Texture texture, Material material,
                                  MeshGenerationContext.MeshFlags flags) {
      var mwd = GetPooledMeshWriteData();
      ...
      bool isCustomSVGGradients =
          flags == MeshGenerationContext.MeshFlags.IsCustomSVGGradients;
      if (isSVGGradients || isCustomSVGGradients) {
        m_CurrentEntry.addFlags =
            isSVGGradients ? VertexFlags.IsSVGGradients
            : VertexFlags.IsCustomSVGGradients;
        ...
        if (m_AtlasManager != null &&
            m_AtlasManager.TryGetLocation(texture as Texture2D,
                                          out atlasRect)) {
          m_CurrentEntry.addFlags =
              texture.filterMode ==
              FilterMode.Point ? VertexFlags.IsAtlasTexturedPoint
              : VertexFlags.IsAtlasTexturedBilinear;
          currentElement.renderChainData.usesAtlas = true;
          uvRegion = new Rect(atlasRect.x, atlasRect.y, atlasRect.width,
                              atlasRect.height);
        } else {
    ...
    public void DrawText(MeshGenerationContextUtils.TextParams textParams,
                         TextHandle handle, float pixelsPerPoint) {
    ...
    void DrawTextNative(MeshGenerationContextUtils.TextParams textParams,
                        TextHandle handle, float pixelsPerPoint) {
      float scaling = TextHandle.ComputeTextScaling(
          currentElement.worldTransform, pixelsPerPoint);
      TextNativeSettings textSettings =
          MeshGenerationContextUtils.TextParams.GetTextNativeSettings(
              textParams, scaling);

      using(NativeArray<TextVertex> textVertices =
                TextNative.GetVertices(textSettings)) {
        if (textVertices.Length == 0)
          return;

        Vector2 localOffset =
            TextNative.GetOffset(textSettings, textParams.rect);
        m_CurrentEntry.isTextEntry = true;
        m_CurrentEntry.clipRectID = m_ClipRectID;
        m_CurrentEntry.isStencilClipped = m_StencilClip;
        MeshBuilder.MakeText(textVertices, localOffset,
                             new MeshBuilder.AllocMeshData(){
    ...
    void DrawTextCore(MeshGenerationContextUtils.TextParams textParams,
                      TextHandle handle, float pixelsPerPoint) {
      ...
      for (int i = 0; i < textInfo.materialCount; i++) {
        ...
        m_CurrentEntry.clipRectID = m_ClipRectID;
        m_CurrentEntry.isStencilClipped = m_StencilClip;
        MeshBuilder.MakeText(textInfo.meshInfo[i], textParams.rect.min,
                             new MeshBuilder.AllocMeshData(){
                                 alloc = m_AllocRawVertsIndicesDelegate});
    ...
    public void
    DrawBorder(MeshGenerationContextUtils.BorderParams borderParams) {
      if (currentElement.panel.contextType == ContextType.Editor) {
      ...
      MeshBuilder.MakeBorder(
          borderParams, UIRUtility.k_MeshPosZ,
          new MeshBuilder.AllocMeshData(){
              alloc = m_AllocThroughDrawMeshDelegate,
              material = borderParams.material, texture = null,
              flags = MeshGenerationContext.MeshFlags.UVisDisplacement});
    }
    ...
    public void DrawImmediate(Action callback, bool cullingEnabled) {
      var cmd = m_Owner.AllocCommand();
      cmd.type =
          cullingEnabled ? CommandType.ImmediateCull : CommandType.Immediate;
      cmd.owner = currentElement;
    ...
    public void DrawVisualElementBackground() {
      if (currentElement.layout.width <= Mathf.Epsilon ||
          currentElement.layout.height <= Mathf.Epsilon)
        return;
      ...
      if (style.backgroundColor != Color.clear) {
        ...
        var rectParams = new MeshGenerationContextUtils.RectangleParams{
          ...
          playmodeTintColor =
              ...
              ContextType.Editor ? UIElementsUtility.editorPlayModeTintColor :
                                                        Color.white
        };
        MeshGenerationContextUtils.GetVisualElementRadii(
            currentElement, out rectParams.topLeftRadius,
            out rectParams.bottomLeftRadius, out rectParams.topRightRadius,
      ...
      if (background.texture != null || background.vectorImage != null) {
        ...
        if (background.texture != null) {
          rectParams = MeshGenerationContextUtils.RectangleParams.MakeTextured(
              ...
              new Rect(0, 0, 1, 1), background.texture,
              style.unityBackgroundScaleMode.value,
              currentElement.panel.contextType);
        } else if (background.vectorImage != null) {
          rectParams =
              MeshGenerationContextUtils.RectangleParams.MakeVectorTextured(
                  GUIUtility.AlignRectToDevice(currentElement.rect),
                  new Rect(0, 0, 1, 1), background.vectorImage,
                  style.unityBackgroundScaleMode.value,
                  currentElement.panel.contextType);
        }
    ...
    public void DrawVisualElementBorder() {
      if (currentElement.layout.width >= Mathf.Epsilon &&
          currentElement.layout.height >= Mathf.Epsilon) {
        var style = currentElement.computedStyle;
        if (style.borderLeftColor != Color.clear &&
                style.borderLeftWidth.value > 0.0f ||
            style.borderTopColor != Color.clear &&
                style.borderTopWidth.value > 0.0f ||
            style.borderRightColor != Color.clear &&
                style.borderRightWidth.value > 0.0f ||
            style.borderBottomColor != Color.clear &&
                style.borderBottomWidth.value > 0.0f) {
          ...
          MeshGenerationContextUtils.GetVisualElementRadii(
              currentElement, out borderParams.topLeftRadius,
    ...
    public void ApplyVisualElementClipping() {
      ...
      } else if (currentElement.renderChainData.clipMethod ==
                 ClipMethod.Stencil) {
    ...
    public void
    DrawVectorImage(MeshGenerationContextUtils.RectangleParams rectParams) {
      var vi = rectParams.vectorImage;
      Debug.Assert(vi != null);

      VertexFlags vertexFlags =
          vi.atlas != null ? VertexFlags.IsSVGGradients : VertexFlags.IsSolid;
      int settingIndexOffset = 0;

      // If the vetor image has embedded textures/gradients, register them in
      // our atlases
      if (vi.atlas != null && m_VectorImageManager != null) {
        var gradientRemap = m_VectorImageManager.AddUser(vi);
        vertexFlags =
            gradientRemap.isAtlassed ? VertexFlags.IsSVGGradients
            : VertexFlags.IsCustomSVGGradients;
      ...
      MeshGenerationContext.MeshFlags meshFlags =
          MeshGenerationContext.MeshFlags.None;
      ...
      var meshAlloc = new MeshBuilder.AllocMeshData(){
          alloc = m_AllocThroughDrawMeshDelegate,
          texture = (vertexFlags == VertexFlags.IsCustomSVGGradients) ? vi.atlas
                                                                      : null,
          flags = meshFlags};

      int finalVertexCount;
      int finalIndexCount;
      MeshBuilder.MakeVectorGraphics(rectParams, settingIndexOffset, meshAlloc,
                                     out finalVertexCount, out finalIndexCount);
    ...
    void ValidateMeshWriteData() {
      // Loop through the used MeshWriteData and make sure the number of
      // indices/vertices were properly filled. Otherwise, we may end up with
      // garbage in the buffers which may cause glitches/driver crashes.
      for (int i = 0; i < m_NextMeshWriteDataPoolItem; ++i) {
        ...
        if (mwd.vertexCount > 0 && mwd.currentVertex < mwd.vertexCount) {
          Debug.LogError(
              "Not enough vertices written in generateVisualContent callback " +
              "(asked for " + mwd.vertexCount + " but only wrote " +
              mwd.currentVertex + ")");
        ...
        if (mwd.indexCount > 0 && mwd.currentIndex < mwd.indexCount) {
          Debug.LogError(
              "Not enough indices written in generateVisualContent callback " +
              "(asked for " + mwd.indexCount + " but only wrote " +
              mwd.currentIndex + ")");
    ...
    void GenerateStencilClipEntryForRoundedRectBackground() {
      ...
      MeshGenerationContextUtils.GetVisualElementRadii(
          currentElement, out radTL, out radBL, out radTR, out radBR);
      ...
      var rp = new MeshGenerationContextUtils.RectangleParams(){
        ...
        topLeftRadius =
            Vector2.Max(Vector2.zero, radTL - new Vector2(widthL, widthT)),
        topRightRadius =
            Vector2.Max(Vector2.zero, radTR - new Vector2(widthR, widthT)),
        bottomLeftRadius =
            Vector2.Max(Vector2.zero, radBL - new Vector2(widthL, widthB)),
        bottomRightRadius =
            Vector2.Max(Vector2.zero, radBR - new Vector2(widthR, widthB)),
        playmodeTintColor =
            currentElement.panel.contextType ==
            ContextType.Editor ? UIElementsUtility.editorPlayModeTintColor :
                                                      Color.white
      ...
      if (style.unityOverflowClipBox == OverflowClipBox.ContentBox) {
        ...
        rp.rect.width -=
            style.paddingLeft.value.value + style.paddingRight.value.value;
        rp.rect.height -=
            style.paddingTop.value.value + style.paddingBottom.value.value;
      }
      ...
      MeshBuilder.MakeSolidRect(rp, UIRUtility.k_MaskPosZ,
                                new MeshBuilder.AllocMeshData(){
                                    alloc = m_AllocRawVertsIndicesDelegate});
      if (m_CurrentEntry.vertices.Length > 0 &&
          m_CurrentEntry.indices.Length > 0) {
        m_Entries.Add(m_CurrentEntry);
        totalVertices += m_CurrentEntry.vertices.Length;
        totalIndices += m_CurrentEntry.indices.Length;
        m_StencilClip = true; // Draw operations following this one should be
                              // clipped if not already
  ...
  internal class UIRTextUpdatePainter : IStylePainter, IDisposable {
    ...
    public void Begin(VisualElement ve, UIRenderDevice device) {
      Debug.Assert(ve.renderChainData.usesLegacyText &&
                   ve.renderChainData.textEntries.Count > 0);
      m_CurrentElement = ve;
      m_TextEntryIndex = 0;
      var oldVertexAlloc = ve.renderChainData.data.allocVerts;
      var oldVertexData =
          ve.renderChainData.data.allocPage.vertices.cpuData.Slice(
              (int) oldVertexAlloc.start, (int) oldVertexAlloc.size);
      device.Update(ve.renderChainData.data,
                    ve.renderChainData.data.allocVerts.size,
                    out m_MeshDataVerts);
      RenderChainTextEntry firstTextEntry = ve.renderChainData.textEntries[0];
      if (ve.renderChainData.textEntries.Count > 1 ||
          firstTextEntry.vertexCount != m_MeshDataVerts.Length)
        m_MeshDataVerts.CopyFrom(
            oldVertexData); // Preserve old data because we're not just updating
                            // the text vertices, but the entire mesh
                            // surrounding it though we won't touch but the text
                            // vertices

      // Case 1222517: Background and border are clipped by the parent, which
      // implies that they may have a different clip id when compared to the
      // content, if overflow-clip-box is set to content-box. As a result, we
      // must NOT use the "first vertex" but rather the "first vertex of the
      // first text entry".
      ...
      m_OpacityPagesSettingsIndex =
          oldVertexData[first].opacityPageSVGSettingIndex;
    }

    public void End() {
      Debug.Assert(
          m_TextEntryIndex ==
          m_CurrentElement.renderChainData.textEntries
              .Count); // Or else element repaint logic diverged for some reason
    ...
    public MeshWriteData DrawMesh(int vertexCount, int indexCount,
                                  Texture texture, Material material,
                                  MeshGenerationContext.MeshFlags flags) {
      // Ideally we should allow returning 0 here and the client would handle
      // that properly
      if (m_DudVerts.Length < vertexCount) {
        if (m_DudVerts.IsCreated)
          m_DudVerts.Dispose();
        m_DudVerts =
            new NativeArray<Vertex>(vertexCount, Allocator.Persistent,
                                    NativeArrayOptions.UninitializedMemory);
      }
      if (m_DudIndices.Length < indexCount) {
        if (m_DudIndices.IsCreated)
          m_DudIndices.Dispose();
        m_DudIndices =
            new NativeArray<UInt16>(indexCount, Allocator.Persistent,
                                    NativeArrayOptions.UninitializedMemory);
      }
      return new MeshWriteData(){m_Vertices = m_DudVerts.Slice(0, vertexCount),
    ...
    public void DrawText(MeshGenerationContextUtils.TextParams textParams,
      ...
      float scaling = TextNative.ComputeTextScaling(
          m_CurrentElement.worldTransform, pixelsPerPoint);
      TextNativeSettings textSettings =
          MeshGenerationContextUtils.TextParams.GetTextNativeSettings(
              textParams, scaling);
      ...
      using(NativeArray<TextVertex> textVertices =
                TextNative.GetVertices(textSettings)) {
        var textEntry =
            m_CurrentElement.renderChainData.textEntries[m_TextEntryIndex++];

        Vector2 localOffset =
            TextNative.GetOffset(textSettings, textParams.rect);
        MeshBuilder.UpdateText(textVertices, localOffset,
                               m_CurrentElement.renderChainData.verticesSpace,
                               m_XFormClipPages, m_IDsFlags,
                               m_OpacityPagesSettingsIndex,
                               m_MeshDataVerts.Slice(textEntry.firstVertex,
                                                     textEntry.vertexCount));
