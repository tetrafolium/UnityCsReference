...
namespace UnityEngine.UIElements {
  internal class UIRRepaintUpdater : BaseVisualTreeUpdater {
    ...
    public UIRRepaintUpdater() { panelChanged += OnPanelChanged; }
    ...
    public override void OnVersionChanged(VisualElement ve,
      ...
      bool transformChanged =
          (versionChangeType & VersionChangeType.Transform) != 0;
      bool sizeChanged = (versionChangeType & VersionChangeType.Size) != 0;
      bool overflowChanged =
          (versionChangeType & VersionChangeType.Overflow) != 0;
      bool borderRadiusChanged =
          (versionChangeType & VersionChangeType.BorderRadius) != 0;
      bool borderWidthChanged =
          (versionChangeType & VersionChangeType.BorderWidth) != 0;

      if (transformChanged || sizeChanged || borderWidthChanged)
        renderChain.UIEOnTransformOrSizeChanged(
            ve, transformChanged, sizeChanged || borderWidthChanged);
    ...
    public override void Update() {
      ...
      using(s_MarkerDrawChain.Auto()) {
        ...
        if (clearFlags != PanelClearFlags.None) {
          GL.Clear((clearFlags & PanelClearFlags.Depth) !=
                       0, // Clearing may impact MVP
                   (clearFlags & PanelClearFlags.Color) != 0, Color.clear,
                   UIRUtility.k_ClearZ);
        }
    ...
    static void OnGraphicsResourcesRecreate(bool recreate) {
      ...
      while (it.MoveNext()) {
        var renderChain =
            (it.Current.Value.GetUpdater(VisualTreeUpdatePhase.Repaint)
                 as UIRRepaintUpdater)
            ?.renderChain;
        if (recreate)
    ...
    void OnPanelChanged(BaseVisualElementPanel obj) {
      ...
      if (panel != null) {
        ...
        if (panel.visualTree != null) {
          renderChain.UIEOnChildAdded(
              panel.visualTree.hierarchy.parent, panel.visualTree,
              panel.visualTree.hierarchy.parent == null ? 0
              : panel.visualTree.hierarchy.parent.IndexOf(panel.visualTree));
          renderChain.UIEOnVisualsChanged(panel.visualTree, true);
        }
        panel.standardShaderChanged += OnPanelStandardShaderChanged;
        panel.standardWorldSpaceShaderChanged +=
    ...
    void OnPanelHierarchyChanged(VisualElement ve,
      ...
      case HierarchyChangeType.Add:
        renderChain.UIEOnChildAdded(
            ve.hierarchy.parent, ve,
            ve.hierarchy.parent != null ? ve.hierarchy.parent.IndexOf(ve) : 0);
        break;
    ...
    void OnPanelStandardShaderChanged() {
      ...
      if (shader == null) {
        ...
        Debug.Assert(shader != null,
                     "Failed to load UIElements default shader");
        if (shader != null)
    ...
    void OnPanelStandardWorldSpaceShaderChanged() {
      ...
      if (shader == null) {
        ...
        Debug.Assert(shader != null,
                     "Failed to load UIElements default world-space shader");
        if (shader != null)
          shader.hideFlags |= HideFlags.DontSaveInEditor;
      }
    ...
    void ResetAllElementsDataRecursive(VisualElement ve) {
      ve.renderChainData = new RenderChainVEData(); // Fast reset, no need to go
                                                    // through device freeing
