...
namespace UnityEngine.UIElements.UIR {
  internal struct ChainBuilderStats {
    ...
    public uint recursiveVisualUpdates, recursiveVisualUpdatesExpanded,
        nonRecursiveVisualUpdates;
    public uint dirtyProcessed;
    public uint nudgeTransformed, boneTransformed, skipTransformed,
        visualUpdateTransformed;
  ...
  internal class RenderChain : IDisposable {
    struct DepthOrderedDirtyTracking // Depth then register-time order
    {
      ...
      public uint dirtyID; // A monotonically increasing ID used to avoid double
      ...
      public void RegisterDirty(VisualElement ve,
                                RenderDataDirtyTypes dirtyTypes,
                                int dirtyTypeClassIndex) {
        Debug.Assert(dirtyTypes != 0);
        int depth = ve.renderChainData.hierarchyDepth;
        minDepths[dirtyTypeClassIndex] = depth <
                                         minDepths[dirtyTypeClassIndex] ? depth
            : minDepths[dirtyTypeClassIndex];
        maxDepths[dirtyTypeClassIndex] = depth >
                                         maxDepths[dirtyTypeClassIndex] ? depth
            : maxDepths[dirtyTypeClassIndex];
      ...
      public void ClearDirty(VisualElement ve,
                             RenderDataDirtyTypes dirtyTypesInverse) {
        ...
        if (ve.renderChainData.dirtiedValues == 0) {
          ...
          if (ve.renderChainData.prevDirty != null)
            ve.renderChainData.prevDirty.renderChainData.nextDirty =
                ve.renderChainData.nextDirty;
          if (ve.renderChainData.nextDirty != null)
            ve.renderChainData.nextDirty.renderChainData.prevDirty =
                ve.renderChainData.prevDirty;
          if (tails[ve.renderChainData.hierarchyDepth] == ve) {
            Debug.Assert(ve.renderChainData.nextDirty == null);
            tails[ve.renderChainData.hierarchyDepth] =
                ve.renderChainData.prevDirty;
          }
          if (heads[ve.renderChainData.hierarchyDepth] == ve) {
            Debug.Assert(ve.renderChainData.prevDirty == null);
            heads[ve.renderChainData.hierarchyDepth] =
                ve.renderChainData.nextDirty;
          }
    ...
    struct RenderChainStaticIndexAllocator {
      ...
      public static void FreeIndex(int index) { renderChains[index] = null; }
    ...
    bool m_FontWasReset;
    Dictionary<VisualElement, Vector2> m_LastGroupTransformElementScale =
        new Dictionary<VisualElement, Vector2>();

    internal RenderChainCommand firstCommand {
      get { return m_FirstCommand; }
    }

    // Profiling
    static ProfilerMarker s_MarkerProcess =
        new ProfilerMarker("RenderChain.Process");
    static ProfilerMarker s_MarkerRender =
        new ProfilerMarker("RenderChain.Draw");
    static ProfilerMarker s_MarkerClipProcessing =
        new ProfilerMarker("RenderChain.UpdateClips");
    static ProfilerMarker s_MarkerOpacityProcessing =
        new ProfilerMarker("RenderChain.UpdateOpacity");
    static ProfilerMarker s_MarkerTransformProcessing =
        new ProfilerMarker("RenderChain.UpdateTransforms");
    static ProfilerMarker s_MarkerVisualsProcessing =
        new ProfilerMarker("RenderChain.UpdateVisuals");
    static ProfilerMarker s_MarkerTextRegen =
        new ProfilerMarker("RenderChain.RegenText");

    static RenderChain() {
      UIR.Utility.RegisterIntermediateRenderers +=
    ...
    protected RenderChain(IPanel panel, UIRenderDevice device,
                          UIRAtlasManager atlasManager,
                          VectorImageManager vectorImageManager) {
      Constructor(panel, device, atlasManager, vectorImageManager);
    }

    void Constructor(IPanel panelObj, UIRenderDevice deviceObj,
                     UIRAtlasManager atlasMan,
      ...
      m_DirtyTracker.minDepths =
          new int[(int) RenderDataDirtyTypeClasses.Count];
      m_DirtyTracker.maxDepths =
          new int[(int) RenderDataDirtyTypeClasses.Count];
      m_DirtyTracker.Reset();

      if (m_RenderNodesData.Count < 1)
        m_RenderNodesData.Add(
            new RenderNodeData(){matPropBlock = new MaterialPropertyBlock()});
...
#region Dispose Pattern
    ...
    public void ProcessChanges() {
      ...
      if (shaderInfoAllocator.isReleased)
        RecreateDevice(); // The shader info texture was released, recreate the
                          // device to start fresh
      ...
      // Although shaderInfoAllocator uses an atlas internally, it doesn't need
      // to reset it due to any of the current reasons that the atlas is reset
      // for, thus we don't do it.
      ...
      dirtyFlags = RenderDataDirtyTypes.Clipping |
                   RenderDataDirtyTypes.ClippingHierarchy;
      clearDirty = ~dirtyFlags;
      s_MarkerClipProcessing.Begin();
      for (int depth = m_DirtyTracker.minDepths[dirtyClass];
           depth <= m_DirtyTracker.maxDepths[dirtyClass]; depth++) {
        ...
        while (ve != null) {
          ...
          if ((ve.renderChainData.dirtiedValues & dirtyFlags) != 0) {
            if (ve.renderChainData.isInChain &&
                ve.renderChainData.dirtyID != m_DirtyTracker.dirtyID)
              Implementation.RenderEvents.ProcessOnClippingChanged(
                  this, ve, m_DirtyTracker.dirtyID, ref m_Stats);
      ...
      for (int depth = m_DirtyTracker.minDepths[dirtyClass];
        ...
        while (ve != null) {
          ...
          if ((ve.renderChainData.dirtiedValues & dirtyFlags) != 0) {
            if (ve.renderChainData.isInChain &&
                ve.renderChainData.dirtyID != m_DirtyTracker.dirtyID)
              Implementation.RenderEvents.ProcessOnOpacityChanged(
                  this, ve, m_DirtyTracker.dirtyID, ref m_Stats);
            m_DirtyTracker.ClearDirty(ve, clearDirty);
          }
          ve = veNext;
      ...
      dirtyFlags =
          RenderDataDirtyTypes.Transform | RenderDataDirtyTypes.ClipRectSize;
      clearDirty = ~dirtyFlags;
      ...
      for (int depth = m_DirtyTracker.minDepths[dirtyClass];
        ...
        while (ve != null) {
          ...
          if ((ve.renderChainData.dirtiedValues & dirtyFlags) != 0) {
            if (ve.renderChainData.isInChain &&
                ve.renderChainData.dirtyID != m_DirtyTracker.dirtyID)
              Implementation.RenderEvents.ProcessOnTransformOrSizeChanged(
                  this, ve, m_DirtyTracker.dirtyID, ref m_Stats);
            m_DirtyTracker.ClearDirty(ve, clearDirty);
          }
          ve = veNext;
      ...
      m_BlockDirtyRegistration =
          true; // Processing visuals may call generateVisualContent, which must
                // be restricted to the allowed operations
      m_DirtyTracker.dirtyID++;
      dirtyClass = (int) RenderDataDirtyTypeClasses.Visuals;
      dirtyFlags =
          RenderDataDirtyTypes.Visuals | RenderDataDirtyTypes.VisualsHierarchy;
      clearDirty = ~dirtyFlags;
      ...
      for (int depth = m_DirtyTracker.minDepths[dirtyClass];
        ...
        while (ve != null) {
          ...
          if ((ve.renderChainData.dirtiedValues & dirtyFlags) != 0) {
            if (ve.renderChainData.isInChain &&
                ve.renderChainData.dirtyID != m_DirtyTracker.dirtyID)
              Implementation.RenderEvents.ProcessOnVisualsChanged(
                  this, ve, m_DirtyTracker.dirtyID, ref m_Stats);
            m_DirtyTracker.ClearDirty(ve, clearDirty);
          }
          ve = veNext;
      ...
      if (m_FontWasReset) {
        // We regenerate the text when the font texture was reset since we don't
        // have any guarantees the the glyphs are going to end up at the same
        // spot in the texture. Up to two passes may be necessary with
        // time-slicing turned off to fully update the text.
    ...
    public void Render() {
      ...
      if (m_FirstCommand != null) {
        if (!m_DrawInCameras) {
          ...
          var projection =
              ProjectionUtils.Ortho(viewport.xMin, viewport.xMax, viewport.yMax,
                                    viewport.yMin, -0.001f, 1.001f);
          GL.LoadProjectionMatrix(projection);
          GL.modelview = Matrix4x4.identity;

          // TODO: Reactivate this guard check once InspectorWindow is fixed to
          // stop adding VEs during OnGUI m_BlockDirtyRegistration = true;
          device.EvaluateChain(
              m_FirstCommand, standardMaterial, standardMaterial,
              atlasManager?.atlas, vectorImageManager?.atlas,
              shaderInfoAllocator.atlas,
              (panel as BaseVisualElementPanel).scaledPixelsPerPoint,
              shaderInfoAllocator.transformConstants,
              shaderInfoAllocator.clipRectConstants,
      ...
      if (immediateException != null) {
        ...
        // Wrap the exception, this plays more nicely with the callstack
        // logging.
    ...
    private void ProcessTextRegen(bool timeSliced) {
      ...
      m_DirtyTextStartIndex =
          timeSliced ? m_DirtyTextStartIndex % m_TextElementCount : 0;
      ...
      int maxCount =
          timeSliced ? Math.Min(50, m_DirtyTextRemaining) : m_TextElementCount;
      for (int i = 0; i < maxCount; i++) {
        Implementation.RenderEvents.ProcessRegenText(
            this, dirty, m_TextUpdatePainter, device, ref m_Stats);
        dirty = dirty.renderChainData.nextText;
        m_DirtyTextStartIndex++;
        if (dirty == null) {
          dirty = m_FirstTextElement;
      ...
      if (m_DirtyTextRemaining > 0)
        (panel as BaseVisualElementPanel)
            ?.OnVersionChanged(
                 m_FirstTextElement,
                 VersionChangeType.Transform); // Force a window refresh
...
#region UIElements event handling callbacks
    public void UIEOnChildAdded(VisualElement parent, VisualElement ve,
                                int index) {
      if (m_BlockDirtyRegistration)
        throw new InvalidOperationException(
            "VisualElements cannot be added to an active visual tree during generateVisualContent callback execution nor during visual tree rendering");
      if (parent != null && !parent.renderChainData.isInChain)
        return; // Ignore it until its parent gets ultimately added

      uint addedCount = Implementation.RenderEvents.DepthFirstOnChildAdded(
          this, parent, ve, index, true);
    ...
    public void UIEOnChildrenReordered(VisualElement ve) {
      if (m_BlockDirtyRegistration)
        throw new InvalidOperationException(
            "VisualElements cannot be moved under an active visual tree during generateVisualContent callback execution nor during visual tree rendering");

      int childrenCount = ve.hierarchy.childCount;
      for (int i = 0; i < childrenCount; i++)
        Implementation.RenderEvents.DepthFirstOnChildRemoving(this,
                                                              ve.hierarchy[i]);
      for (int i = 0; i < childrenCount; i++)
        Implementation.RenderEvents.DepthFirstOnChildAdded(
            this, ve, ve.hierarchy[i], i, false);
    ...
    public void UIEOnChildRemoving(VisualElement ve) {
      if (m_BlockDirtyRegistration)
        throw new InvalidOperationException(
            "VisualElements cannot be removed from an active visual tree during generateVisualContent callback execution nor during visual tree rendering");

      m_StatsElementsRemoved +=
          Implementation.RenderEvents.DepthFirstOnChildRemoving(this, ve);
      Debug.Assert(!ve.renderChainData.isInChain);
    }
    ...
    public void UIEOnClippingChanged(VisualElement ve, bool hierarchical) {
      if (ve.renderChainData.isInChain) {
        if (m_BlockDirtyRegistration)
          throw new InvalidOperationException(
              "VisualElements cannot change clipping state under an active visual tree during generateVisualContent callback execution nor during visual tree rendering");

        m_DirtyTracker.RegisterDirty(
            ve,
            RenderDataDirtyTypes.Clipping |
                (hierarchical ? RenderDataDirtyTypes.ClippingHierarchy : 0),
            (int) RenderDataDirtyTypeClasses.Clipping);
      }
    }
    ...
    public void UIEOnOpacityChanged(VisualElement ve) {
      if (ve.renderChainData.isInChain) {
        if (m_BlockDirtyRegistration)
          throw new InvalidOperationException(
              "VisualElements cannot change opacity under an active visual tree during generateVisualContent callback execution nor during visual tree rendering");

        m_DirtyTracker.RegisterDirty(ve, RenderDataDirtyTypes.Opacity,
                                     (int) RenderDataDirtyTypeClasses.Opacity);
      }
    }

    public void UIEOnTransformOrSizeChanged(VisualElement ve,
                                            bool transformChanged,
                                            bool clipRectSizeChanged) {
      if (ve.renderChainData.isInChain) {
        if (m_BlockDirtyRegistration)
          throw new InvalidOperationException(
              "VisualElements cannot change size or transform under an active visual tree during generateVisualContent callback execution nor during visual tree rendering");

        RenderDataDirtyTypes flags =
            (transformChanged ? RenderDataDirtyTypes.Transform
             : RenderDataDirtyTypes.None) |
            (clipRectSizeChanged ? RenderDataDirtyTypes.ClipRectSize
             : RenderDataDirtyTypes.None);
        m_DirtyTracker.RegisterDirty(
            ve, flags, (int) RenderDataDirtyTypeClasses.TransformSize);
      }
    }
      ...
      if (ve.renderChainData.isInChain) {
        ...
        m_DirtyTracker.RegisterDirty(
            ...
            RenderDataDirtyTypes.Visuals |
                (hierarchical ? RenderDataDirtyTypes.VisualsHierarchy : 0),
            (int) RenderDataDirtyTypeClasses.Visuals);
      }
    }
    ...
    internal UIRVEShaderInfoAllocator
        shaderInfoAllocator; // Not a property because this is a struct we want
                             // to mutate
    internal Implementation.UIRStylePainter painter {
      get;
      private set;
    }
    ...
    internal void OnRenderCommandsRemoved(RenderChainCommand firstCommand,
    ...
    internal void RemoveTextElement(VisualElement ve) {
      if (ve.renderChainData.prevText != null)
        ve.renderChainData.prevText.renderChainData.nextText =
            ve.renderChainData.nextText;
      if (ve.renderChainData.nextText != null)
        ve.renderChainData.nextText.renderChainData.prevText =
            ve.renderChainData.prevText;
    ...
    internal void OnGroupTransformElementChangedTransform(VisualElement ve) {
      ...
      if (!m_LastGroupTransformElementScale.TryGetValue(ve, out lastScale) ||
        ...
        m_LastGroupTransformElementScale[ve] =
            new Vector2(ve.worldTransform.m00, ve.worldTransform.m11);
      }
    }
    ...
    internal void BeforeRenderDeviceRelease() {
      ...
      m_RenderDeviceRestoreInfo.root =
          GetFirstElementInPanel(m_FirstCommand?.owner);
    ...
    internal void AfterRenderDeviceRelease() {
      ...
      var atlasManObj =
          m_RenderDeviceRestoreInfo.hasAtlasMan ? new UIRAtlasManager() : null;
      var vectorImageManObj =
          m_RenderDeviceRestoreInfo.hasVectorImageMan ? new VectorImageManager(
              atlasManObj)
          : null;
      ...
      if (root != null) {
        ...
        UIEOnChildAdded(root.parent, root, root.hierarchy.parent == null ? 0
                        : root.hierarchy.parent.IndexOf(panel.visualTree));
      }
    }
    ...
    private unsafe static void OnRenderNodeExecute(IntPtr obj) {
      ...
      Exception immediateException = null;
      rnd.device.EvaluateChain(rnd.firstCommand, rnd.initialMaterial,
                               rnd.standardMaterial, rnd.atlas, rnd.vectorAtlas,
                               rnd.shaderInfoAtlas, rnd.dpiScale,
                               rnd.transformConstants, rnd.clipRectConstants,
    ...
    private static void OnRegisterIntermediateRenderers(Camera camera) {
      ...
      while (panels.MoveNext()) {
        ...
        RenderChain renderChain =
            (p.GetUpdater(VisualTreeUpdatePhase.Repaint) as UIRRepaintUpdater)
            ?.renderChain;
        if (renderChain == null || renderChain.m_StaticIndex < 0 ||
            renderChain.m_FirstCommand == null)
          continue;
        ...
        rndSource.transformConstants =
            renderChain.shaderInfoAllocator.transformConstants;
        rndSource.clipRectConstants =
            renderChain.shaderInfoAllocator.clipRectConstants;

        if (renderChain.m_CustomMaterialCommands == 0) {
          // Trivial case, custom materials not used, so we don't have to chop
          // the chain to multiple intermediate renderers
          rndSource.initialMaterial = standardMaterial;
          rndSource.firstCommand = renderChain.m_FirstCommand;
          OnRegisterIntermediateRendererMat(rtp, renderChain, ref rndSource,
                                            camera, commandOrder++);
          continue;
        }
        ...
        while (command != null) {
          ...
          Material commandMat = command.state.material == null ? standardMaterial
              : command.state.material;
    ...
    OnRegisterIntermediateRendererMat(RuntimePanel rtp, RenderChain renderChain,
                                      ref RenderNodeData rnd, Camera camera,
                                      int sameDistanceSortPriority) {
      ...
      UIR.Utility.RegisterIntermediateRenderer(
          camera, rnd.initialMaterial, rtp.panelToWorld,
          new Bounds(Vector3.zero, new Vector3(float.MaxValue, float.MaxValue,
                                               float.MaxValue)),
          3, 0, false, sameDistanceSortPriority, (ulong) camera.cullingMask,
          (int) UIR.Utility.RendererCallbacks.RendererCallback_Exec,
    ...
    void DrawStats() {
      ...
      GUI.Box(new Rect(20, 40, 200, realDevice ? 380 : 256),
              "UI Toolkit Draw Stats");
      GUI.Label(rc, "Elements added\t: " + m_Stats.elementsAdded);
      rc.y += y_off;
      GUI.Label(rc, "Elements removed\t: " + m_Stats.elementsRemoved);
      rc.y += y_off;
      GUI.Label(rc, "Mesh allocs allocated\t: " + m_Stats.newMeshAllocations);
      rc.y += y_off;
      GUI.Label(rc, "Mesh allocs updated\t: " + m_Stats.updatedMeshAllocations);
      rc.y += y_off;
      GUI.Label(rc, "Clip update roots\t: " + m_Stats.recursiveClipUpdates);
      rc.y += y_off;
      GUI.Label(rc,
                "Clip update total\t: " + m_Stats.recursiveClipUpdatesExpanded);
      rc.y += y_off;
      GUI.Label(rc,
                "Opacity update roots\t: " + m_Stats.recursiveOpacityUpdates);
      rc.y += y_off;
      GUI.Label(rc, "Opacity update total\t: " +
                        m_Stats.recursiveOpacityUpdatesExpanded);
      rc.y += y_off;
      GUI.Label(rc,
                "Xform update roots\t: " + m_Stats.recursiveTransformUpdates);
      rc.y += y_off;
      GUI.Label(rc, "Xform update total\t: " +
                        m_Stats.recursiveTransformUpdatesExpanded);
      rc.y += y_off;
      GUI.Label(rc, "Xformed by bone\t: " + m_Stats.boneTransformed);
      rc.y += y_off;
      GUI.Label(rc, "Xformed by skipping\t: " + m_Stats.skipTransformed);
      rc.y += y_off;
      GUI.Label(rc, "Xformed by nudging\t: " + m_Stats.nudgeTransformed);
      rc.y += y_off;
      GUI.Label(rc, "Xformed by repaint\t: " + m_Stats.visualUpdateTransformed);
      rc.y += y_off;
      GUI.Label(rc, "Visual update roots\t: " + m_Stats.recursiveVisualUpdates);
      rc.y += y_off;
      GUI.Label(rc, "Visual update total\t: " +
                        m_Stats.recursiveVisualUpdatesExpanded);
      rc.y += y_off;
      GUI.Label(rc,
                "Visual update flats\t: " + m_Stats.nonRecursiveVisualUpdates);
      rc.y += y_off;
      GUI.Label(rc, "Dirty processed\t: " + m_Stats.dirtyProcessed);
      rc.y += y_off;
      GUI.Label(rc, "Group-xform updates\t: " +
                        m_Stats.groupTransformElementsChanged);
      rc.y += y_off;
      GUI.Label(rc, "Text regens\t: " + m_Stats.textUpdates);
      rc.y += y_off;
      ...
      GUI.Label(rc, "Frame index\t: " + drawStats.currentFrameIndex);
      rc.y += y_off;
      GUI.Label(rc, "Command count\t: " + drawStats.commandCount);
      rc.y += y_off;
      GUI.Label(rc, "Draw commands\t: " + drawStats.drawCommandCount);
      rc.y += y_off;
      GUI.Label(rc, "Draw ranges\t: " + drawStats.drawRangeCount);
      rc.y += y_off;
      GUI.Label(rc, "Draw range calls\t: " + drawStats.drawRangeCallCount);
      rc.y += y_off;
      GUI.Label(rc, "Material sets\t: " + drawStats.materialSetCount);
      rc.y += y_off;
      GUI.Label(rc, "Immediate draws\t: " + drawStats.immediateDraws);
      rc.y += y_off;
      GUI.Label(rc, "Total triangles\t: " + (drawStats.totalIndices / 3));
      rc.y += y_off;
    }

    static VisualElement GetFirstElementInPanel(VisualElement ve) {
      while (ve != null &&
             ve.renderChainData.prev?.renderChainData.isInChain == true)
  ...
  internal enum RenderDataDirtyTypes {
    ...
    ClippingHierarchy =
        1 << 3,       // Same as above, but applies to all descendants too.
    Visuals = 1 << 4, // The visuals of the VE need to be repainted.
    VisualsHierarchy =
        1 << 5,      // Same as above, but applies to all descendants too.
  ...
  internal struct RenderChainVEData {
    ...
    internal VisualElement prevDirty,
        nextDirty; // Embedded doubly-linked list for dirty updates
    ...
    internal RenderChainCommand firstCommand,
        lastCommand; // Sequential for the same owner
    internal RenderChainCommand firstClosingCommand,
        lastClosingCommand; // Optional, sequential for the same owner, the
                            // presence of closing commands requires starting
                            // commands too, otherwise certain optimizations
                            // will become invalid
    ...
    internal Matrix4x4 verticesSpace; // Transform describing the space which
                                      // the vertices in 'data' are relative to
    ...
    internal RenderChainCommand lastClosingOrLastCommand {
      get { return lastClosingCommand ?? lastCommand; }
    }
    static internal bool AllocatesID(BMPAlloc alloc) {
      return (alloc.ownedState != 0) && alloc.IsValid();
    }
    static internal bool InheritsID(BMPAlloc alloc) {
      return (alloc.ownedState == 0) && alloc.IsValid();
    }
  }
