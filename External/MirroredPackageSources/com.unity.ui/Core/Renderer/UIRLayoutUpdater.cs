...
// :
// - Only call dirty repaint when the layout rect has changed instead of
// "yogaNode.HasNewLayout"
...
public override void OnVersionChanged(VisualElement ve,
	...
	if ((versionChangeType &
	     (VersionChangeType.Layout | VersionChangeType.Hierarchy)) == 0)
		return;

	// Yoga DOES NOT expect the node tree to mutate while layout is being
	// computed. it could lead to crashes (EXC_BAD_ACCESS)
	if ((versionChangeType & VersionChangeType.Hierarchy) != 0 &&
	    panel.duringLayoutPhase) {
		throw new InvalidOperationException(
			      "Hierarchy change detected while computing layout, this is not supported.");
	}
...
public override void Update() {
	...
	while (visualTree.yogaNode.IsDirty) {
		...
		// elements may not be initialized properly and the resulting layout
		// will be invalid.
...
private void UpdateSubTree(VisualElement ve, int currentLayoutPass) {
	Rect yogaLayoutRect =
		new Rect(ve.yogaNode.LayoutX, ve.yogaNode.LayoutY,
		         ve.yogaNode.LayoutWidth, ve.yogaNode.LayoutHeight);
	Rect yogaPaddingRect = new Rect(
		ve.yogaNode.LayoutPaddingLeft, ve.yogaNode.LayoutPaddingTop,
		ve.yogaNode.LayoutWidth -
		(ve.yogaNode.LayoutPaddingLeft + ve.yogaNode.LayoutPaddingRight),
		ve.yogaNode.LayoutHeight -
		(ve.yogaNode.LayoutPaddingTop + ve.yogaNode.LayoutPaddingBottom));
	Rect lastLayoutRect = ve.lastLayout;
	Rect lastPaddingRect = ve.lastPadding;

	VersionChangeType changeType = 0;

	// Changing the layout/padding size should trigger the following version
	// changes:
	...
	// Changing the layout/padding position should trigger the following
	// version change:
	// - Transform: to draw the element and content at the right position
	bool layoutPositionChanged =
		yogaLayoutRect.position != lastLayoutRect.position;
	bool paddingPositionChanged =
		yogaPaddingRect.position != lastPaddingRect.position;
	...
	if (layoutSizeChanged || layoutPositionChanged) {
		using (var evt = GeometryChangedEvent.GetPooled(lastLayoutRect,
		                                                yogaLayoutRect)) {
