...
namespace UnityEngine.UIElements.UIR {
  /// <summary>
  /// This class allows to queue blit commands and apply them up to 8 at a time
  /// later on.
  /// </summary>
  class TextureBlitter : IDisposable {
    ...
    static ProfilerMarker s_CommitSampler =
        new ProfilerMarker("UIR.TextureBlitter.Commit");
...
#region Dispose Pattern
    ...
    public void QueueBlit(Texture src, RectInt srcRect, Vector2Int dstPos,
                          bool addBorder, Color tint) {
      if (disposed) {
    ...
    public void BlitOneNow(RenderTexture dst, Texture src, RectInt srcRect,
                           Vector2Int dstPos, bool addBorder, Color tint) {
      if (disposed) {
        DisposeHelper.NotifyDisposedUsed(this);
        return;
      }
    ...
    public void Reset() { m_PendingBlits.Clear(); }
    ...
    void BeginBlit(RenderTexture dst) {
      ...
      // store viewport as we'll have to restore it once the AtlasManager is
      // done rendering
    ...
    void DoBlit(IList<BlitInfo> blitInfos, int startIndex) {
      int stopIndex =
          Mathf.Min(startIndex + k_TextureSlotCount, blitInfos.Count);

      // Bind and update the material.
      for (int blitIndex = startIndex, slotIndex = 0; blitIndex < stopIndex;
           ++blitIndex, ++slotIndex) {
      ...
      for (int blitIndex = startIndex, slotIndex = 0; blitIndex < stopIndex;
        ...
        float dstRight =
            current.dstPos.x + current.srcRect.width + current.border;
        float dstTop =
            current.dstPos.y + current.srcRect.height + current.border;
        ...
        float srcRight =
            (current.srcRect.xMax + current.border) * srcTexelWidth;
    ...
    void EndBlit() {
      ...
      // restore viewport (which has been implicitly modified as we used a
      // rendertarget)
      GL.Viewport(new Rect(m_Viewport.x, m_Viewport.y, m_Viewport.width,
                           m_Viewport.height));
    }
  }
}
