...
#region Internals
  ...
  // Maintains its available memory range to the best, giving practically a fast
  // allocation time but O(n) free time where n is a factor of fragmentation
  // (number of available blocks) It always coalesces free blocks on every free
  // operation For index buffers, go with immediate coalescing, but for vertex
  // buffers we can be very lazy and don't aim for adjacency or care for
  // internal fragmentation much. Instead we should improve allocation/free time
  // more because vertex buffer content will update often
  class BestFitAllocator {
    ...
    public Alloc Allocate(uint size) {
      // Choose an allocation policy from below, each has
      // performance/vs/fragmentation balance characteristics, but remember to
      // rename the class to reflect the policy chosen All are O(n) where is n
      // is fragmentation (number of available blocks)
      ...
      return new Alloc(){start = block.start, size = block.size,
                         handle = block};
    }
    ...
    public void Free(Alloc alloc) {
      ...
      if (block.end == m_HighWatermark) {
        if (block.prev != null)
          m_HighWatermark = block.prev.allocated ? block.prev.end
              : block.prev
                    .start; // If the previous block is not allocated, then its
                            // start marks the high watermark (which can be 0 or
                            // the end of the previous allocated block)
        else
          m_HighWatermark = 0; // We're the first block and owning the high
                               // watermark and we got freed
      }

      block.allocated = false;

      // Scan availables to find the correct placement for us among the
      // availables list This is the loop that potentially makes this Free
      // operation expensive
    ...
    /// <summary>
    /// Coalesces the provided block with the previous block by expanding the
    /// previous block and discarding the provided block.
    /// </summary>
    /// <remarks>
    /// By keeping the first block, we don't need to change
    /// <c>firstAvailableBlock</c>. We need to keep this in mind if we ever to
    ...
    internal HeapStatistics GatherStatistics() {
      ...
      while (block != null) {
        if (block.allocated) {
          ...
          stats.largestAvailableBlock =
              Math.Max(stats.largestAvailableBlock, block.size);
        }
        stats.blockCount++;
        block = block.next;
      }
      ...
      if (stats.freeSize > 0)
        stats.fragmentation =
            (float)((double)(stats.freeSize - stats.largestAvailableBlock) /
                    (double) stats.freeSize) *
            100.0f;
...
#region Allocation policies
    ...
    class Block : PoolItem {
      ...
      public Block prevAvailable,
          nextAvailable; // Only valid when this block is not allocated
  ...
  // The GPU buffer allocator supports allocations with 2 different types of
  // lifetimes: permanent and temp Permanent allocations grow from bottom to
  // top, while temp grow from top to bottom
  class GPUBufferAllocator {
    ...
    public Alloc Allocate(uint size, bool shortLived) {
      ...
      if (!shortLived)
        ...
        alloc.start =
            m_High.totalSize - alloc.start -
            alloc.size; // Flip the address since we start from the high end
      }
    ...
    public void Free(Alloc alloc) {
      if (!alloc.shortLived)
        ...
        alloc.start =
            m_High.totalSize - alloc.start -
            alloc.size; // Fix the flipped address by flipping it again
        m_High.Free(alloc);
      }
    }

    public bool isEmpty {
      get { return m_Low.highWatermark == 0 && m_High.highWatermark == 0; }
    }

    public HeapStatistics GatherStatistics() {
      HeapStatistics stats = new HeapStatistics();
      stats.subAllocators = new HeapStatistics[]{m_Low.GatherStatistics(),
      ...
      for (int i = 0; i < 2; i++) {
        ...
        stats.totalSize =
            Math.Max(stats.totalSize, stats.subAllocators[i].totalSize);
        stats.allocatedSize += stats.subAllocators[i].allocatedSize;
        stats.largestAvailableBlock =
            Math.Min(stats.largestAvailableBlock,
                     stats.subAllocators[i].largestAvailableBlock);
        stats.availableBlocksCount +=
            stats.subAllocators[i].availableBlocksCount;
        stats.blockCount += stats.subAllocators[i].blockCount;
        stats.highWatermark =
            Math.Max(stats.highWatermark, stats.subAllocators[i].highWatermark);
        stats.fragmentation =
            Math.Max(stats.fragmentation, stats.subAllocators[i].fragmentation);
      }
  ...
  internal unsafe class Page : IDisposable {
    public Page(uint vertexMaxCount, uint indexMaxCount,
                uint maxQueuedFrameCount, bool mockPage) {
      vertexMaxCount = Math.Min(
          vertexMaxCount, 1 << 16); // Because we use UInt16 as the index type
      vertices =
          new DataSet<Vertex>(Utility.GPUBufferType.Vertex, vertexMaxCount,
                              maxQueuedFrameCount, 32, mockPage);
      indices = new DataSet<UInt16>(Utility.GPUBufferType.Index, indexMaxCount,
                                    maxQueuedFrameCount, 32, mockPage);
    }
...
#region Dispose Pattern
    ...
    public bool isEmpty {
      get { return vertices.allocator.isEmpty && indices.allocator.isEmpty; }
    }

    public class DataSet<T> : IDisposable where T : struct {
      public DataSet(Utility.GPUBufferType bufferType, uint totalCount,
                     uint maxQueuedFrameCount, uint updateRangePoolSize,
                     bool mockBuffer) {
        if (!mockBuffer)
          gpuData = new Utility.GPUBuffer<T>((int) totalCount, bufferType);
        cpuData = new NativeArray<T>((int) totalCount, Allocator.Persistent,
        ...
        uint multipliedUpdateRangePoolSize =
            m_UpdateRangePoolSize * maxQueuedFrameCount;
        updateRanges = new NativeArray<GfxUpdateBufferRange>(
            (int) multipliedUpdateRangePoolSize, Allocator.Persistent,
            NativeArrayOptions.UninitializedMemory);
...
#region Dispose Pattern
      ...
      public void RegisterUpdate(uint start, uint size) {
        ...
        int rangeIndex =
            (int)(m_UpdateRangesBatchStart + m_UpdateRangesEnqueued);

        if (m_UpdateRangesEnqueued > 0) {
          // If this update chains with the previous one, just grow the previous
          // one
          ...
          if (lastRange.offsetFromWriteStart + lastRange.size == startBytes) {
            updateRanges[lastIndex] = new GfxUpdateBufferRange(){
                ...
                size = lastRange.size + size * m_ElemStride};
        ...
        var cpuDataSlice = new UIntPtr(
            cpuData.Slice((int) start, (int) size).GetUnsafeReadOnlyPtr());
        updateRanges[rangeIndex] = new GfxUpdateBufferRange(){
            source = cpuDataSlice, offsetFromWriteStart = start * m_ElemStride,
      ...
      public void SendUpdates() {
        ...
        if (m_UpdateRangesSaturated) {
          ...
          updateRanges[(int) m_UpdateRangesBatchStart] =
              new GfxUpdateBufferRange(){
                  source = new UIntPtr(
                      cpuData.Slice((int) m_UpdateRangeMin, (int) updateSize)
        ...
        if (minByte > 0) {
          for (uint i = 0; i < m_UpdateRangesEnqueued; i++) {
            ...
            updateRanges[index] = new GfxUpdateBufferRange(){
                ...
                offsetFromWriteStart =
                    updateRanges[index].offsetFromWriteStart - minByte,
                size = updateRanges[index].size};
          }
        }
        gpuData?.UpdateRanges(updateRanges.Slice((int) m_UpdateRangesBatchStart,
                                                 (int) m_UpdateRangesEnqueued),
                              (int) minByte, (int) maxByte);

        // Reset state for upcoming updates
        m_UpdateRangeMin = uint.MaxValue;
        m_UpdateRangeMax = 0;
        m_UpdateRangesEnqueued = 0;
        m_UpdateRangesBatchStart =
      ...
      public NativeArray<GfxUpdateBufferRange>
          updateRanges; // Powers of two count
