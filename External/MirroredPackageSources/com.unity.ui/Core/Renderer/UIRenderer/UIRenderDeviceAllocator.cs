...
class BestFitAllocator
{
    ...
    public Alloc Allocate(uint size)
    {
        ...
        return new Alloc() {
            start = block.start, size = block.size, handle = block
        };
    }
...
class GPUBufferAllocator
{
    ...
    public bool isEmpty {
        get {
            return m_Low.highWatermark == 0 && m_High.highWatermark == 0;
        }
    }
...
internal unsafe class Page : IDisposable
{
    ...
    public bool isEmpty {
        get {
            return vertices.allocator.isEmpty && indices.allocator.isEmpty;
        }
    }
