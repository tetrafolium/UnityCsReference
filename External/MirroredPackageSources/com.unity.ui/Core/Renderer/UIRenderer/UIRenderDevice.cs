...
namespace UnityEngine.UIElements.UIR {
  [StructLayout(LayoutKind.Sequential)] struct Transform3x4 {
    public Vector4 v0, v1, v2;
  }

  // The values stored here could be updated behind the back of the holder of
  // this object. Hence, never turn this into a struct or else we can't do
  // automatic defragmentation and address ordering optimizations
  internal class MeshHandle : PoolItem {
    internal Alloc allocVerts, allocIndices;
    internal uint triangleCount; // Can be less than the actual indices if only
                                 // a portion of the allocation is used
    internal Page allocPage;
    internal uint allocTime; // Frame this mesh was allocated/updated
    internal uint
        updateAllocID; // If not 0, the alloc here points to a temporary
                       // location managed by an update record with the said ID
  }
  ...
  internal class UIRenderDevice : IDisposable {
    ...
    private const uint k_MaxQueuedFrameCount =
        4; // Support drivers queuing up to 4 frames

    private readonly bool
        m_MockDevice; // Don't access GfxDevice resources nor submit commands of
                      // any sort, used for tests
    ...
    private uint m_NextUpdateID =
        1; // For the current frame only, 0 is not an accepted value here
    ...
    private static LinkedList<DeviceToFree> m_DeviceFreeQueue =
        new LinkedList<DeviceToFree>();              // Not thread safe for now
    ...
    private static bool m_SynchronousFree; // This is set on domain unload or
                                           // app quit, so it is irreversible
    ...
    static readonly int s_1PixelClipInvViewPropID =
        Shader.PropertyToID("_1PixelClipInvView");
    static readonly int s_GradientSettingsTexID =
        Shader.PropertyToID("_GradientSettingsTex");
    static readonly int s_ShaderInfoTexID =
        Shader.PropertyToID("_ShaderInfoTex");
    static readonly int s_ScreenClipRectPropID =
        Shader.PropertyToID("_ScreenClipRect");
    ...
    static ProfilerMarker s_MarkerAdvanceFrame =
        new ProfilerMarker("UIR.AdvanceFrame");
    static ProfilerMarker s_MarkerFence = new ProfilerMarker("UIR.WaitOnFence");
    static ProfilerMarker s_MarkerBeforeDraw =
        new ProfilerMarker("UIR.BeforeDraw");

    static bool ? s_VertexTexturingIsAvailable;
    const string k_VertexTexturingIsAvailableTag =
        "UIE_VertexTexturingIsAvailable";
    ...
    public UIRenderDevice(uint initialVertexCapacity = 0,
                          uint initialIndexCapacity = 0)
        : this(initialVertexCapacity, initialIndexCapacity, false) {}

    protected UIRenderDevice(uint initialVertexCapacity,
                             uint initialIndexCapacity, bool mockDevice) {
      m_MockDevice = mockDevice;
      Debug.Assert(
          !m_SynchronousFree); // Shouldn't create render devices when the app
                               // is quitting or domain-unloading
      ...
      m_NextPageVertexCount =
          Math.Max(initialVertexCapacity,
                   2048); // No less than 4k vertices (doubled from 2k
                          // effectively when the first page is allocated)
      m_LargeMeshVertexCount = m_NextPageVertexCount;
      m_IndexToVertexCountRatio =
          (float) initialIndexCapacity / (float) initialVertexCapacity;
      m_IndexToVertexCountRatio = Mathf.Max(m_IndexToVertexCountRatio, 2);

      m_DeferredFrees =
          new List<List<AllocToFree>>((int) k_MaxQueuedFrameCount);
...
#region Default system resources
    ...
    static internal Texture2D whiteTexel {
      get {
        if (s_WhiteTexel == null) {
          ...
          s_WhiteTexel.filterMode =
              FilterMode.Bilinear; // Make sure it's bilinear so UIRAtlasManager
                                   // accepts it on older HW targets
    ...
    static private Texture2D s_DefaultShaderInfoTexFloat,
        s_DefaultShaderInfoTexARGB8;
    static internal Texture2D defaultShaderInfoTexFloat {
      get {
        if (s_DefaultShaderInfoTexFloat == null) {
          s_DefaultShaderInfoTexFloat =
              new Texture2D(64, 64, TextureFormat.RGBAFloat, false); // No mips
          s_DefaultShaderInfoTexFloat.hideFlags = HideFlags.HideAndDontSave;
          s_DefaultShaderInfoTexFloat.filterMode = FilterMode.Point;
          s_DefaultShaderInfoTexFloat.SetPixel(
              UIRVEShaderInfoAllocator.identityTransformTexel.x,
              UIRVEShaderInfoAllocator.identityTransformTexel.y + 0,
              UIRVEShaderInfoAllocator.identityTransformRow0Value);
          s_DefaultShaderInfoTexFloat.SetPixel(
              UIRVEShaderInfoAllocator.identityTransformTexel.x,
              UIRVEShaderInfoAllocator.identityTransformTexel.y + 1,
              UIRVEShaderInfoAllocator.identityTransformRow1Value);
          s_DefaultShaderInfoTexFloat.SetPixel(
              UIRVEShaderInfoAllocator.identityTransformTexel.x,
              UIRVEShaderInfoAllocator.identityTransformTexel.y + 2,
              UIRVEShaderInfoAllocator.identityTransformRow2Value);
          s_DefaultShaderInfoTexFloat.SetPixel(
              UIRVEShaderInfoAllocator.infiniteClipRectTexel.x,
              UIRVEShaderInfoAllocator.infiniteClipRectTexel.y,
              UIRVEShaderInfoAllocator.infiniteClipRectValue);
          s_DefaultShaderInfoTexFloat.SetPixel(
              UIRVEShaderInfoAllocator.fullOpacityTexel.x,
              UIRVEShaderInfoAllocator.fullOpacityTexel.y,
              UIRVEShaderInfoAllocator.fullOpacityValue);
    ...
    static internal bool vertexTexturingIsAvailable {
      get {
        if (!s_VertexTexturingIsAvailable.HasValue) {
          // Remove this workaround once case 1148851 has been fixed. In the
          // editor, subshaders aren't stripped according to the graphic device
          // capabilities unless the shader is precompiled. Querying tags will
          // always return the tags from the first subshader. The editor shader
          // is precompiled and doesn't suffer this issue, so we can use it as a
          // reference.
          ...
          string tagValue = stockDefaultMaterial.GetTag(
              k_VertexTexturingIsAvailableTag, false);
          UIRUtility.Destroy(stockDefaultMaterial);
          s_VertexTexturingIsAvailable =
              (tagValue == k_VertexTexturingIsAvailableTrue);
        }
    ...
    void InitVertexDeclaration() {
      var vertexDecl = new VertexAttributeDescriptor[]{
          ...
          // TransformID page coordinate (XY), ClipRectID page coordinate (ZW),
          // packed into a Color32
          new VertexAttributeDescriptor(VertexAttribute.TexCoord1,
                                        VertexAttributeFormat.UNorm8, 4),

          // In-page index for (TransformID, ClipRectID, OpacityID), Flags
          // (vertex type), all packed into a Color32
          new VertexAttributeDescriptor(VertexAttribute.TexCoord2,
                                        VertexAttributeFormat.UNorm8, 4),

          // OpacityID page coordinate (XY), SVG SettingIndex (16-bit encoded in
          // ZW), packed into a Color32
...
#region Dispose Pattern
    ...
    public MeshHandle Allocate(uint vertexCount, uint indexCount,
                               out NativeSlice<Vertex> vertexData,
                               out NativeSlice<UInt16> indexData,
                               out UInt16 indexOffset) {
      MeshHandle meshHandle = m_MeshHandles.Get();
      meshHandle.triangleCount = indexCount / 3;
      Allocate(meshHandle, vertexCount, indexCount, out vertexData,
               out indexData, false);
      indexOffset = (UInt16) meshHandle.allocVerts.start;
      return meshHandle;
    }

    public void Update(MeshHandle mesh, uint vertexCount,
                       out NativeSlice<Vertex> vertexData) {
      Debug.Assert(mesh.allocVerts.size >= (uint) vertexCount);
      if (mesh.allocTime == m_FrameIndex) {
        // Update right after allocation and the GPU hasn't used the data yet..
        // update same allocation
        vertexData = mesh.allocPage.vertices.cpuData.Slice(
            (int) mesh.allocVerts.start, (int) vertexCount);
        return;
      }

      uint oldIndexOffset =
          mesh.allocVerts.start; // Cache this before it gets modified in the
                                 // call to Update below
      NativeSlice<UInt16> oldIndexData = new NativeSlice<UInt16>(
          mesh.allocPage.indices.cpuData, (int) mesh.allocIndices.start,
          (int) mesh.allocIndices.size);
      UInt16 indexOffset;
      NativeSlice<UInt16> indexData;
      AllocToUpdate allocToUpdate;
      UpdateAfterGPUUsedData(mesh, vertexCount, mesh.allocIndices.size,
                             out vertexData, out indexData, out indexOffset,
                             out allocToUpdate, false);
    ...
    public void Update(MeshHandle mesh, uint vertexCount, uint indexCount,
                       out NativeSlice<Vertex> vertexData,
                       out NativeSlice<UInt16> indexData,
                       out UInt16 indexOffset) {
      Debug.Assert(mesh.allocVerts.size >= (uint) vertexCount);
      ...
      if (mesh.allocTime == m_FrameIndex) {
        ...
        vertexData = mesh.allocPage.vertices.cpuData.Slice(
            (int) mesh.allocVerts.start, (int) vertexCount);
        indexData = mesh.allocPage.indices.cpuData.Slice(
            (int) mesh.allocIndices.start, (int) indexCount);
        indexOffset = (UInt16) mesh.allocVerts.start;
        return;
      }

      AllocToUpdate allocToUpdate;
      UpdateAfterGPUUsedData(mesh, vertexCount, indexCount, out vertexData,
                             out indexData, out indexOffset, out allocToUpdate,
                             true);
    }

    bool TryAllocFromPage(Page page, uint vertexCount, uint indexCount,
                          ref Alloc va, ref Alloc ia, bool shortLived) {
      ...
      if (va.size != 0) {
        ...
        page.vertices.allocator.Free(
            va); // There is space for the vertices, but not for the indices
        va.size = 0;
      }
      return false;
    }

    void Allocate(MeshHandle meshHandle, uint vertexCount, uint indexCount,
                  out NativeSlice<Vertex> vertexData,
                  out NativeSlice<UInt16> indexData, bool shortLived) {
      ...
      if (vertexCount <= m_LargeMeshVertexCount) {
        ...
        if (m_FirstPage != null) {
          page = m_FirstPage;
          for (;;) {
            if (TryAllocFromPage(page, (uint) vertexCount, (uint) indexCount,
                                 ref va, ref ia, shortLived) ||
                (page.next == null))
              break;
            else
              page = page.next;
        ...
        if (ia.size == 0) {
          ...
          m_NextPageVertexCount =
              Math.Max(m_NextPageVertexCount, vertexCount * 2);
          m_NextPageVertexCount =
              Math.Min(m_NextPageVertexCount,
                       64 * 1024); // Stay below 64k for 16-bit indices
          uint newPageIndexCount =
              (uint)(m_NextPageVertexCount * m_IndexToVertexCountRatio + 0.5f);
          newPageIndexCount =
              Math.Max(newPageIndexCount, (uint)(indexCount * 2));
          Debug.Assert(
              page?.next ==
              null); // page MUST be the last page in the list, but can be null
          page = new Page(m_NextPageVertexCount, newPageIndexCount,
                          k_MaxQueuedFrameCount, m_MockDevice);
          // Link this new page to the head of the list so next allocations have
          // more chance of succeeding rather than scanning through all pages to
          // land in this page
        ...
        // A huge mesh, push it to a page of its own. Put this page at the end
        // so it won't be queried often
        Page lastPage = m_FirstPage;
        while (lastPage != null && lastPage.next != null)
          lastPage = lastPage.next;

        Page dedicatedPage = new Page((uint) vertexCount, (uint) indexCount,
                                      k_MaxQueuedFrameCount, m_MockDevice);
        if (lastPage != null)
          lastPage.next = dedicatedPage;
        else
          m_FirstPage = dedicatedPage;
        page = dedicatedPage;
        va = dedicatedPage.vertices.allocator.Allocate((uint) vertexCount,
                                                       shortLived);
        ia = dedicatedPage.indices.allocator.Allocate((uint) indexCount,
                                                      shortLived);
      }
      ...
      vertexData = new NativeSlice<Vertex>(page.vertices.cpuData,
                                           (int) va.start, (int) vertexCount);
      indexData = new NativeSlice<UInt16>(page.indices.cpuData, (int) ia.start,
                                          (int) indexCount);
    ...
    void UpdateAfterGPUUsedData(MeshHandle mesh, uint vertexCount,
                                uint indexCount,
                                out NativeSlice<Vertex> vertexData,
                                out NativeSlice<UInt16> indexData,
                                out UInt16 indexOffset,
                                out AllocToUpdate allocToUpdate,
                                bool copyBackIndices) {
      allocToUpdate = new AllocToUpdate(){
          id = m_NextUpdateID++, allocTime = m_FrameIndex, meshHandle = mesh,
          copyBackIndices = copyBackIndices};
      Debug.Assert(m_NextUpdateID >
                   0); // Wrapped-around 4 billion in one frame?!

      // Replace the update record that is currently active on the mesh (if
      // present)
      if (mesh.updateAllocID == 0) {
        ...
        int activeUpdateIndex =
            (int)(mesh.updateAllocID -
                  1); // -1 since 1 is the first update id in a frame
        ...
        var queueToFree =
            m_DeferredFrees[(int)(m_FrameIndex % (uint) m_DeferredFrees.Count)];
        queueToFree.Add(new AllocToFree(){
            alloc = mesh.allocVerts, page = mesh.allocPage, vertices = true});
        queueToFree.Add(new AllocToFree(){alloc = mesh.allocIndices,
                                          page = mesh.allocPage,
                                          vertices = false});
      }

      // Try to allocate from the same page, if we fail, we revert to the
      // general case
      if (TryAllocFromPage(mesh.allocPage, (uint) vertexCount,
                           (uint) indexCount, ref mesh.allocVerts,
                           ref mesh.allocIndices, true)) {
        mesh.allocPage.vertices.RegisterUpdate(mesh.allocVerts.start,
                                               mesh.allocVerts.size);
        mesh.allocPage.indices.RegisterUpdate(mesh.allocIndices.start,
                                              mesh.allocIndices.size);
      } else
        Allocate(mesh, vertexCount, indexCount, out vertexData, out indexData,
                 true);
      ...
      m_Updates [(int)(m_FrameIndex % m_Updates.Count)]
          .Add(allocToUpdate);

      vertexData = new NativeSlice<Vertex>(mesh.allocPage.vertices.cpuData,
                                           (int) mesh.allocVerts.start,
                                           (int) vertexCount);
      indexData = new NativeSlice<UInt16>(mesh.allocPage.indices.cpuData,
                                          (int) mesh.allocIndices.start,
                                          (int) indexCount);
      indexOffset = (UInt16) mesh.allocVerts.start;
    }
    ...
    public void Free(MeshHandle mesh) {
      if (mesh.updateAllocID != 0) // Is there an update over this mesh
      {
        ...
        var queueToFree =
            m_DeferredFrees[(int)(m_FrameIndex % (uint) m_DeferredFrees.Count)];
        queueToFree.Add(new AllocToFree(){alloc = oldUpdate.permAllocVerts,
                                          page = oldUpdate.permPage,
                                          vertices = true});
        queueToFree.Add(new AllocToFree(){alloc = oldUpdate.permAllocIndices,
                                          page = oldUpdate.permPage,
                                          vertices = false});
        queueToFree.Add(new AllocToFree(){
            alloc = mesh.allocVerts, page = mesh.allocPage, vertices = true});
        ...
        oldUpdate.allocTime = 0xFFFFFFFF; // Effectively disable the old update
        updates[activeUpdateIndex] = oldUpdate;
      } else if (mesh.allocTime !=
                 m_FrameIndex) // Was it potentially used by the GPU?
      {
        int queueIndex = (int)(m_FrameIndex % (uint) m_DeferredFrees.Count);
        m_DeferredFrees [queueIndex]
            .Add(new AllocToFree(){alloc = mesh.allocVerts,
                                   page = mesh.allocPage, vertices = true});
        m_DeferredFrees [queueIndex]
            .Add(new AllocToFree(){alloc = mesh.allocIndices,
                                   page = mesh.allocPage, vertices = false});
      } else {
        // Freeing in the same frame the allocation happened, totally redundant.
        // As the GPU didn't use this data, we don't need to defer the free
    ...
    static void Set1PixelSizeParameter(DrawParams drawParams,
      ...
      Vector3 v = matVPInv.MultiplyVector(
          new Vector3(_1PixelClipInvView.x, _1PixelClipInvView.y));
    ...
    unsafe static NativeSlice<T> PtrToSlice<T>(void *p, int count) where T
        : struct {
      var slice = NativeSliceUnsafeUtility.ConvertExistingDataToNativeSlice<T>(
          p, UnsafeUtility.SizeOf<T>(), count);
    ...
    public unsafe void EvaluateChain(
        RenderChainCommand head, Material initialMat, Material defaultMat,
        Texture atlas, Texture gradientSettings, Texture shaderInfo,
        float pixelsPerPoint, NativeSlice<Transform3x4> transforms,
        NativeSlice<Vector4> clipRects, MaterialPropertyBlock stateMatProps,
        bool allowMaterialChange, ref Exception immediateException) {
      ...
      if (fullyCreated) {
        ...
        if (gradientSettings != null)
          m_StandardMatProps.SetTexture(s_GradientSettingsTexID,
                                        gradientSettings);
        if (shaderInfo != null)
          m_StandardMatProps.SetTexture(s_ShaderInfoTexID, shaderInfo);
        if (transforms.Length > 0)
          UIR.Utility.SetVectorArray<Transform3x4>(
              m_StandardMatProps, s_TransformsPropID, transforms);
        if (clipRects.Length > 0)
          UIR.Utility.SetVectorArray<Vector4>(m_StandardMatProps,
                                              s_ClipRectsPropID, clipRects);
        Set1PixelSizeParameter(drawParams, m_CommonMatProps);
        m_CommonMatProps.SetVector(s_ScreenClipRectPropID,
                                   drawParams.view.Peek().clipRect);
        Utility.SetPropertyBlock(m_StandardMatProps);
        Utility.SetPropertyBlock(m_CommonMatProps);
      }

      int rangesCount = 1024; // Must be powers of two. TODO: Can be estimated
                              // better from the render chain command count
      ...
      while (head != null) {
        ...
        m_DrawStats.drawCommandCount +=
            (head.type == CommandType.Draw ? 1u : 0u);
        ...
        if (!kickRanges) {
          Material stateMat =
              head.state.material != null ? head.state.material : defaultMat;
          ...
          kickRanges = stateParamsChanges || materialChanges ||
                       (head.mesh.allocPage != curPage);
          if (!kickRanges) // For debugging just disable those lines to get a
                           // draw call per draw command
            stashRange =
                curDrawIndex != (head.mesh.allocIndices.start +
                                 head.indexOffset); // Should we stash at least?
        }
        ...
        if (stashRange) {
          ...
          if (curDrawRange.indexCount > 0) {
            int wrapAroundIndex =
                (rangesStart + rangesReady++) & rangesCountMinus1;
            ranges[wrapAroundIndex] = curDrawRange; // Close the active range

            // TODO: This check only works since ranges are serialized and will
            // break once the ranges are
            //       truly processed in a multi-threaded fashion without copies.
            //       When this happens, a new mechanism will need to be
            //       implemented to handle the "ranges-buffer-full" condition.
            //       For the time being, calling KickRanges will make the whole
            //       buffer available.
            if (rangesReady == rangesCount)
              KickRanges(ranges, ref rangesReady, ref rangesStart, rangesCount,
                         curPage);
          ...
          if (head.type == CommandType.Draw) {
            curDrawRange.firstIndex =
                (int) head.mesh.allocIndices.start + head.indexOffset;
            curDrawRange.indexCount = head.indexCount;
            curDrawRange.vertsReferenced =
                (int)(head.mesh.allocVerts.start + head.mesh.allocVerts.size);
            curDrawRange.minIndexVal = (int) head.mesh.allocVerts.start;
            curDrawIndex = curDrawRange.firstIndex + head.indexCount;
            maxVertexReferenced =
                curDrawRange.vertsReferenced + curDrawRange.minIndexVal;
            m_DrawStats.totalIndices += (uint) head.indexCount;
          }
        } else {
          // We can chain
          if (curDrawRange.indexCount == 0)
            curDrawIndex = curDrawRange.firstIndex =
                (int) head.mesh.allocIndices.start +
                head.indexOffset; // A first draw after a stash
          maxVertexReferenced =
              Math.Max(maxVertexReferenced, (int)(head.mesh.allocVerts.size +
                                                  head.mesh.allocVerts.start));
          curDrawRange.indexCount += head.indexCount;
          curDrawRange.minIndexVal = Math.Min(curDrawRange.minIndexVal,
                                              (int) head.mesh.allocVerts.start);
          curDrawRange.vertsReferenced =
              maxVertexReferenced - curDrawRange.minIndexVal;
        ...
        if (kickRanges) {
          ...
          if (head.type != CommandType.Draw) {
            if (!m_MockDevice)
              head.ExecuteNonDrawMesh(drawParams, pixelsPerPoint,
                                      ref immediateException);
            if (head.type == CommandType.Immediate ||
                head.type == CommandType.ImmediateCull) {
              curState.material = null; // A value that is unique to force
                                        // material reset on next draw command
          ...
          if (materialChanges || stateParamsChanges) {
            if (!m_MockDevice) {
              if (materialChanges) {
                ...
                curState.material.SetPass(
                    0); // No multipass support, should it be even considered?
                ...
                Utility.SetPropertyBlock(stateMatProps);
              else if (m_CommonMatProps != null &&
                       (head.type == CommandType.PushView ||
                        head.type == CommandType.PopView)) {
                Set1PixelSizeParameter(drawParams, m_CommonMatProps);
                m_CommonMatProps.SetVector(s_ScreenClipRectPropID,
                                           drawParams.view.Peek().clipRect);
                Utility.SetPropertyBlock(m_CommonMatProps);
          ...
          } else if (head.type == CommandType.PushView ||
                     head.type == CommandType.PopView) {
      ...
      // Kick any pending ranges, this usually occurs when the draw chain ends
      // with a draw command.
      if (curDrawRange.indexCount > 0) {
      ...
      if (m_Fences != null) {
        ...
        fixed(uint *fence = &m_Fences[(int)(m_FrameIndex % m_Fences.Length)]) {
          for (;;) {
            ...
            if (((int)(newFenceVal - curFenceVal)) <=
                0)   // This is the same test as in
                     // GfxDeviceWorker::WaitOnCPUFence(). Handles wrap around.
              break; // Our newFenceVal is already older than the current one,
                     // so keep the current
            int cmpOldVal = System.Threading.Interlocked.CompareExchange(
                ref * ((int *) fence), (int) newFenceVal, (int) curFenceVal);
            if (cmpOldVal == curFenceVal)
              break; // The exchange succeeded, now newFenceVal is stored
                     // atomically in (*fence)
          }
        }
      }
    ...
    unsafe void KickRanges(DrawBufferRange *ranges, ref int rangesReady,
                           ref int rangesStart, int rangesCount, Page curPage) {
      if (rangesReady > 0) {
        if (rangesStart + rangesReady <= rangesCount) {
          if (!m_MockDevice)
            DrawRanges(
                curPage.indices.gpuData, curPage.vertices.gpuData,
                PtrToSlice<DrawBufferRange>(ranges + rangesStart, rangesReady));
          m_DrawStats.drawRangeCallCount++;
        } else {
          // Less common situation, the numbers straddles the end of the ranges
          // buffer
          int firstRangeCount = rangesCount - rangesStart;
          int secondRangeCount = rangesReady - firstRangeCount;
          if (!m_MockDevice) {
            DrawRanges(curPage.indices.gpuData, curPage.vertices.gpuData,
                       PtrToSlice<DrawBufferRange>(ranges + rangesStart,
                                                   firstRangeCount));
            DrawRanges(curPage.indices.gpuData, curPage.vertices.gpuData,
                       PtrToSlice<DrawBufferRange>(ranges, secondRangeCount));
          }
    ...
    unsafe void DrawRanges<I, T>(Utility.GPUBuffer<I> ib,
                                 Utility.GPUBuffer<T> vb,
                                 NativeSlice<DrawBufferRange> ranges) where T
        : struct where I : struct {
      IntPtr *vStream = stackalloc IntPtr[1];
      vStream[0] = vb.BufferPointer;
      Utility.DrawRanges(ib.BufferPointer, vStream, 1,
                         new IntPtr(ranges.GetUnsafePtr()), ranges.Length,
                         m_VertexDecl);
    }
    ...
    public void AdvanceFrame() {
      ...
      var queueToUpdate =
          m_Updates[(int)(m_FrameIndex % (uint) m_DeferredFrees.Count)];
      foreach (var update in queueToUpdate) {
        if (update.meshHandle.updateAllocID == update.id &&
            update.meshHandle.allocTime == update.allocTime) {
          NativeSlice<Vertex> srcVerts = new NativeSlice<Vertex>(
              update.meshHandle.allocPage.vertices.cpuData,
              (int) update.meshHandle.allocVerts.start,
              (int) update.meshHandle.allocVerts.size);
          NativeSlice<Vertex> destVerts =
              new NativeSlice<Vertex>(update.permPage.vertices.cpuData,
                                      (int) update.permAllocVerts.start,
                                      (int) update.meshHandle.allocVerts.size);
          destVerts.CopyFrom(srcVerts);
          update.permPage.vertices.RegisterUpdate(
              update.permAllocVerts.start, update.meshHandle.allocVerts.size);

          if (update.copyBackIndices) {
            NativeSlice<UInt16> srcIndices = new NativeSlice<UInt16>(
                update.meshHandle.allocPage.indices.cpuData,
                (int) update.meshHandle.allocIndices.start,
                (int) update.meshHandle.allocIndices.size);
            NativeSlice<UInt16> destIndices = new NativeSlice<UInt16>(
                update.permPage.indices.cpuData,
                (int) update.permAllocIndices.start,
                (int) update.meshHandle.allocIndices.size);

            // Carry original indices, but repoint them at the new vertices
            int indexCount = destIndices.Length;
            int indexDifference = (int) update.permAllocVerts.start -
                                  (int) update.meshHandle.allocVerts.start;
            for (int i = 0; i < indexCount; i++)
              destIndices[i] = (UInt16)(srcIndices[i] + indexDifference);

            update.permPage.indices.RegisterUpdate(
                update.permAllocIndices.start,
                update.meshHandle.allocIndices.size);
          }
          queueToFree.Add(new AllocToFree(){
              alloc = update.meshHandle.allocVerts,
              page = update.meshHandle.allocPage, vertices = true});
          queueToFree.Add(new AllocToFree(){
              alloc = update.meshHandle.allocIndices,
              page = update.meshHandle.allocPage, vertices = false});
    ...
    internal static void PrepareForGfxDeviceRecreate() {
      m_ActiveDeviceCount += 1; // Don't let the count reach 0 and unsubscribe
                                // from GfxDeviceRecreate
      if (s_WhiteTexel != null) {
    ...
    internal struct AllocationStatistics {
      public struct PageStatistics {
        internal HeapStatistics vertices, indices;
      }
    ...
    internal AllocationStatistics GatherAllocationStatistics() {
      ...
      page = m_FirstPage;
      while (page != null) {
        stats.pages[pageCount].vertices =
            page.vertices.allocator.GatherStatistics();
        stats.pages[pageCount].indices =
            page.indices.allocator.GatherStatistics();
        pageCount++;
        page = page.next;
      }
      return stats;
...
#region Internals
    private void OnEngineUpdate() { AdvanceFrame(); }
    ...
    private static void OnEngineUpdateGlobal() { ProcessDeviceFreeQueue(); }
