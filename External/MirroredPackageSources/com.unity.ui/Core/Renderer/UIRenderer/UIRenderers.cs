...
namespace UnityEngine.UIElements.UIR {
  internal enum VertexFlags {
    // Vertex Type
    // These values are like enum values, they are mutually exclusive. Only one
    // may be specified on a vertex.
    IsSolid = 0,
    IsText = 1,
  ...
  internal class DrawParams {
    internal static readonly Rect k_UnlimitedRect =
        new Rect(-100000, -100000, 200000, 200000);
    ...
    public void Reset() {
      ...
      view.Push(new ViewTransform{
          ...
          clipRect = UIRUtility.ToVector4(k_FullNormalizedRect)});
      scissor.Clear();
  ...
  internal class RenderChainCommand : PoolItem {
    ...
    internal void ExecuteNonDrawMesh(DrawParams drawParams,
                                     float pixelsPerPoint,
                                     ref Exception immediateException) {
      switch (type) {
      case CommandType.ImmediateCull: {
        RectInt worldRect =
            RectPointsToPixelsAndFlipYAxis(owner.worldBound, pixelsPerPoint);
        if (!worldRect.Overlaps(Utility.GetActiveViewport()))
          break;

        // Element isn't culled, follow through the normal immediate callback
        // procedure
      ...
      case CommandType.Immediate: {
        ...
        bool hasScissor =
            drawParams.scissor.Count >
            1; // We always expect the "unbound" scissor rectangle to exists
        if (hasScissor)
          Utility.DisableScissor(); // Disable scissor since most IMGUI code
                                    // assume it's inactive

        Utility.ProfileImmediateRendererBegin();
        try {
          using(new GUIClip.ParentClipScope(owner.worldTransform,
                                            owner.worldClip)) callback();
        } catch (Exception e) {
        ...
        if (hasScissor)
          Utility.SetScissorRect(RectPointsToPixelsAndFlipYAxis(
              drawParams.scissor.Peek(), pixelsPerPoint));
        break;
      }
      case CommandType.PushView:
        var vt =
            new ViewTransform(){transform = owner.worldTransform,
                                clipRect = RectToClipSpace(owner.worldClip)};
      ...
      case CommandType.PushScissor:
        Rect elemRect =
            CombineScissorRects(owner.worldClip, drawParams.scissor.Peek());
        drawParams.scissor.Push(elemRect);
        Utility.SetScissorRect(
            RectPointsToPixelsAndFlipYAxis(elemRect, pixelsPerPoint));
        break;
      case CommandType.PopScissor:
        ...
        if (prevRect.x == DrawParams.k_UnlimitedRect.x)
          ...
          Utility.SetScissorRect(
              RectPointsToPixelsAndFlipYAxis(prevRect, pixelsPerPoint));
        break;
      }
    }

    static Vector4 RectToClipSpace(Rect rc) {
      // Since the shader compares positions multiplied by the MVP matrix, then
      // we must ensure to use the same MVP matrices the shader uses.. namely,
      // the GPU projection matrix
      Matrix4x4 projection = Utility.GetDeviceProjectionMatrix();
      var minClipSpace = projection.MultiplyPoint(
          new Vector3(rc.xMin, rc.yMin, UIRUtility.k_MeshPosZ));
      var maxClipSpace = projection.MultiplyPoint(
          new Vector3(rc.xMax, rc.yMax, UIRUtility.k_MeshPosZ));
      return new Vector4(Mathf.Min(minClipSpace.x, maxClipSpace.x),
                         Mathf.Min(minClipSpace.y, maxClipSpace.y),
                         Mathf.Max(minClipSpace.x, maxClipSpace.x),
                         Mathf.Max(minClipSpace.y, maxClipSpace.y));
    }
    ...
    static RectInt RectPointsToPixelsAndFlipYAxis(Rect rect,
                                                  float pixelsPerPoint) {
