...
namespace UnityEngine.UIElements.UIR {
  ...
  class GradientSettingsAtlas : IDisposable {
    static ProfilerMarker s_MarkerWrite =
        new ProfilerMarker("UIR.GradientSettingsAtlas.Write");
    static ProfilerMarker s_MarkerCommit =
        new ProfilerMarker("UIR.GradientSettingsAtlas.Commit");
...
#region Dispose Pattern
    ...
    public void Write(Alloc alloc, GradientSettings[] settings,
                      GradientRemap remap) {
      if (disposed) {
        DisposeHelper.NotifyDisposedUsed(this);
        return;
      ...
      if (m_RawAtlas.rgba == null) {
        m_RawAtlas = new RawTexture{rgba = new Color32[m_ElemWidth * m_Length],
                                    width = m_ElemWidth, height = m_Length};
      ...
      for (int i = 0, settingsCount = settings.Length; i < settingsCount; ++i) {
        ...
        if (entry.gradientType == GradientType.Radial) {
          ...
          m_RawAtlas.WriteRawFloat4Packed((float) GradientType.Radial / 255,
                                          (float) entry.addressMode / 255,
                                          focus.x, focus.y, destX++, destY);
        } else if (entry.gradientType == GradientType.Linear) {
          m_RawAtlas.WriteRawFloat4Packed(0.0f, (float) entry.addressMode / 255,
                                          0.0f, 0.0f, destX++, destY);
        }

        Vector2Int pos = new Vector2Int(entry.location.x, entry.location.y);
        var size =
            new Vector2(entry.location.width - 1, entry.location.height - 1);
        if (remap != null) {
          pos = new Vector2Int(remap.location.x, remap.location.y);
          size =
              new Vector2(remap.location.width - 1, remap.location.height - 1);
        }
        m_RawAtlas.WriteRawInt2Packed(pos.x, pos.y, destX++, destY);
        m_RawAtlas.WriteRawInt2Packed((int) size.x, (int) size.y, destX++,
                                      destY);
    ...
    public void Commit() {
      ...
      // TODO: This way of transferring is costly since it is a synchronous
      // operation that flushes the pipeline.
    ...
    void PrepareAtlas() {
      ...
      m_Atlas = new Texture2D(m_ElemWidth, m_Length, TextureFormat.ARGB32, 0,
                              true){
    ...
    struct RawTexture {
      ...
      public void WriteRawFloat4Packed(float f0, float f1, float f2, float f3,
                                       int destX, int destY) {
