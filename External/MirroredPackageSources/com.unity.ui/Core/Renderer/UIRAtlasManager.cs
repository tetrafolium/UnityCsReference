...
namespace UnityEngine.UIElements {
  internal class UIRAtlasManager : IDisposable {
    ...
    // A component interested in registering callbacks to our static creation
    // event, may be initialized after some UIRAtlasManager instances have
    // already been created, We therefore need to provide access to all current
    // instances of UIRAtlasManager
    private static List<UIRAtlasManager> s_Instances =
        new List<UIRAtlasManager>();
    private static ReadOnlyList<UIRAtlasManager> s_InstancesreadOnly =
        new ReadOnlyList<UIRAtlasManager>(s_Instances);
    ...
    static ProfilerMarker s_MarkerReset =
        new ProfilerMarker("UIR.AtlasManager.Reset");

    public int maxImageSize { get; }
    public RenderTextureFormat format { get; }

    /// <summary>
    /// Current atlas texture in use. The texture could change after
    /// <c>UIRAtlasManager.Commit</c> is called.
    /// </summary>
    ...
    public UIRAtlasManager(
        RenderTextureFormat format = RenderTextureFormat.ARGB32,
        FilterMode filterMode = FilterMode.Bilinear, int maxImageSize = 64,
        int initialSize = 64) {
      if (filterMode != FilterMode.Bilinear && filterMode != FilterMode.Point)
        throw new NotSupportedException(
            "The only supported atlas filter modes are point or bilinear");

      this.format = format;
      this.maxImageSize = maxImageSize;
      m_FloatFormat =
          (format ==
           RenderTextureFormat.ARGBFloat); // Identify any other formats to be
                                           // used as float here
...
#region Dispose Pattern
    ...
    private static void LogDisposeError() {
      Debug.LogError(
          "An attempt to use a disposed atlas manager has been detected.");
    }
    ...
    public static void MarkAllForReset() { ++s_GlobalResetVersion; }

    public void MarkForReset() { m_ResetVersion = s_GlobalResetVersion - 1; }
    ...
    /// <remarks>
    /// When textures that have been previously allowed into the atlas manager
    /// change, or if the project color space changes, this method MUST be
    /// called. Textures that had been previously accepted into the atlas may
    ...
    /// <summary>
    /// If the provided texture is already in the atlas, the uvs are returned
    /// immediately. Otherwise, if the texture passes the requirements (format,
    /// etc), it will be virtually added to the atlas and added to the list of
    /// textures to be committed.
    /// </summary>
    public bool TryGetLocation(Texture2D image, out RectInt uvs) {
      ...
      m_Blitter.QueueBlit(image, new RectInt(0, 0, image.width, image.height),
                          new Vector2Int(uvs.x, uvs.y), true, Color.white);
    ...
    public bool AllocateRect(int width, int height, out RectInt uvs) {
      // Attempt to allocate.
      if (!m_Allocator.TryAllocate(width + m_2SidePadding,
                                   height + m_2SidePadding, out uvs))
        return false;
      uvs = new RectInt(uvs.x + m_1SidePadding, uvs.y + m_1SidePadding, width,
                        height);
      return true;
    }

    public void EnqueueBlit(Texture image, int x, int y, bool addBorder,
                            Color tint) {
      m_Blitter.QueueBlit(image, new RectInt(0, 0, image.width, image.height),
                          new Vector2Int(x, y), addBorder, tint);
    }

    /// <summary>
    /// We accept non-HDR formats that have 8 bits or less per component (before
    /// compression).
    /// </summary>
    /// <remarks>
    /// When the texture format is a compressed one, it is possible that we get
    /// additional loss due to the use of the atlas. For example, 1/256 could
    /// end up being uncompressed to 3/512. As long as we are within the shader
    /// with a float, there is no loss. However, since we're storing the value
    /// in a 8 bits per component atlas, 3/512 could end up being stored as
    /// 1/256 or 2/256. This creates potential differences in the results that
    /// may be observable in the following scenarios:
    ///   1) Blending Operations (e.g. blending with 3/512 vs 1/256 or 2/256).
    ///   2) Rendering to high precision (e.g. 16 bits per component) render
    ///   targets.
    ///
    /// That being said, the benefit that we get from allowing these textures
    /// into the atlas seems much larger than the inconvenience caused by the
    /// slight error that we potentially introduce.
    /// </remarks>
    public static bool IsTextureFormatSupported(TextureFormat format) {
      ...
      case TextureFormat
          .BC7: // Source is typically 8 bits per component (BUT COULD BE MORE)
      case TextureFormat
          .BC4: // Source is 1 components per color, 8 bits per component
      case TextureFormat
          .BC5: // Source is 2 components per color, 8 bits per component
...
#pragma warning disable 618
      ...
      // please note that we reuse ASTC_RGB_NxN values for new enums so these
...
#pragma warning restore 618
      ...
      case TextureFormat.RGBA64:         // HDR
        return false;
      default:
        // This exception is required if we want to be able to detect new enum
        // values in test UIRAtlasManagerTests.AllTextureFormatsAreHandled.
        throw new NotImplementedException(
            $"The support of texture format '{format}' is undefined.");
      }
    }
    ...
    private bool IsTextureValid(Texture2D image) {
      ...
      // When in linear color space, the atlas will have sRGB read/write
      // enabled. This means we can't store linear data without potentially
      // causing banding.
      if (!m_FloatFormat && m_ColorSpace == ColorSpace.Linear &&
          image.activeTextureColorSpace != ColorSpace.Gamma)
        return false;
    ...
    public void Commit() {
      ...
      if (m_ForceReblitAll) {
        ...
        foreach (KeyValuePair<Texture2D, RectInt> kvp in m_UVs)
          m_Blitter.QueueBlit(
              kvp.Key, new RectInt(0, 0, kvp.Key.width, kvp.Key.height),
              new Vector2Int(kvp.Value.x, kvp.Value.y), true, Color.white);
      }
    ...
    private void UpdateAtlasTexture() {
      ...
      if (atlas.width != m_Allocator.physicalWidth ||
          atlas.height != m_Allocator.physicalHeight) {
    ...
    private RenderTexture CreateAtlasTexture() {
      ...
      // The RenderTextureReadWrite setting is purposely omitted in order to get
      // the "Default" behavior.
      return new RenderTexture(m_Allocator.physicalWidth,
                               m_Allocator.physicalHeight, 0, format){
