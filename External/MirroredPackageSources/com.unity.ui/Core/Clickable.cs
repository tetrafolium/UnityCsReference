namespace UnityEngine.UIElements {
  ...
  public class Clickable : PointerManipulator {
    ...
    /// <summary>
    /// This property tracks the activation of the manipulator. Set it to true
    /// when the manipulator is activated.
    /// </summary>
    ...
    /// <summary>
    /// Specifies the mouse position saved during the last mouse event on the
    /// target Element.
    /// </summary>
    ...
    // interval is used to determine the time delta between event repetitions.
    // Applies if interval > 0.
    /// <summary>
    /// Constructor.
    /// </summary>
    ...
    private bool IsRepeatable() { return (m_Delay > 0 || m_Interval > 0); }
    ...
    /// <summary>
    /// This method is called when a MouseDownEvent is sent to the target
    /// element.
    /// </summary>
    ...
    /// <summary>
    /// This method is called when a MouseMoveEvent is sent to the target
    /// element.
    /// </summary>
    /// <param name="evt">The event.</param>
    ...
    ProcessDownEvent(EventBase evt, Vector2 localPosition, int pointerId) {
      active = true;
      ...
      if (IsRepeatable()) {
        // Repeatable button clicks are performed on the MouseDown and at timer
        // events
        ...
        if (m_Repeater == null) {
          m_Repeater = target.schedule.Execute(OnTimer)
                           .Every(m_Interval)
                           .StartingIn(m_Delay);
        } else {
    ...
    /// </summary>
    protected virtual void ProcessMoveEvent(EventBase evt,
    ...
    /// </summary>
    protected virtual void ProcessUpEvent(EventBase evt, Vector2 localPosition,
                                          int pointerId) {
      active = false;
