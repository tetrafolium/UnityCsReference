...
namespace UnityEngine.UIElements {
  ...
  public class IMGUIContainer : VisualElement, IDisposable {
    /// <summary>
    /// Instantiates an <see cref="IMGUIContainer"/> using the data read from a
    /// UXML file.
    /// </summary>
    ...
    public new class UxmlTraits : VisualElement.UxmlTraits {
      ...
      /// <summary>
      /// Returns an empty enumerable, as IMGUIContainer cannot have
    ...
    /// </summary>
    /// <remarks>
    /// The function is assigned to onGUIHandler and is similar to <see
    /// cref="MonoBehaviour.OnGUI"/>.
    /// </remarks>
    ...
    // If needed, an IMGUIContainer will allocate native state via this utility
    // object to store control IDs
    ...
    // This is not nice but needed until we properly remove the dependency on
    // GUIView's own ObjectGUIState At least this implementation is not needed
    // for users, only for containers created to wrap each GUIView
    ...
    /// <summary>
    /// When this property is set to true, <see cref="onGUIHandler"/> is not
    /// called when the Element is outside the viewport.
    /// </summary>
    ...
    // We cache the clipping rect and transform during regular painting so that
    // we can reuse them during the DoMeasure call to DoOnGUI(). It's still
    // important to not pass Rect.zero for the clipping rect as this eventually
    // sets the global GUIClip.visibleRect which IMGUI code could be using to
    // influence size. See case 1111923 and 1158089.
    ...
    private float layoutMeasuredWidth {
      get { return Mathf.Ceil(cache.topLevel.maxWidth); }
    }

    private float layoutMeasuredHeight {
      get { return Mathf.Ceil(cache.topLevel.maxHeight); }
    }

    /// <summary>
    /// ContextType of this IMGUIContrainer. Currently only supports
    /// ContextType.Editor.
    /// </summary>
    ...
    // 1) lostFocus : a blur event occurred and we need to make sure the actual
    // keyboard focus from IMGUI is really un-focused
    bool lostFocus = false;
    // 2) receivedFocus : a Focus event occurred and we need to focus the actual
    // IMGUIContainer as being THE element focused.
    ...
    public override bool canGrabFocus =>
        focusOnlyIfHasFocusableControls ? hasFocusableControls &&
    ...
    /// Constructor.
    /// </summary>
    /// <param name="onGUIHandler">The function assigned to <see
    /// cref="onGUIHandler"/>.</param>
    ...
    private void OnGenerateVisualContent(MeshGenerationContext mgc) {
      ...
      // The IStylePainter is kept as an interface rather than a concrete class
      // for now to support tests
    ...
    private void DoOnGUI(Event evt, Matrix4x4 parentTransform,
                         Rect clippingRect, bool isComputingLayout,
                         Rect layoutSize, Action onGUIHandler,
                         bool canAffectFocus = true) {
      // Extra checks are needed here because client code might have changed the
      // IMGUIContainer since we enter HandleIMGUIEvent()
      if (onGUIHandler == null || panel == null) {
        return;
      }

      // Save the GUIClip count to make sanity checks after calling the OnGUI
      // handler
      ...
      // For the IMGUI, we need to update the GUI.color with the actual play
      // mode tint ... In fact, this is taken from
      // EditorGUIUtility.ResetGUIState(). Here, the play mode tint is either
      // white (no tint, or not in play mode) or the right color (if in play
      // mode)
      GUI.color = UIElementsUtility.editorPlayModeTintColor;
      // From now on, Event.current is either evt or a copy of evt.
      // Since Event.current may change while being processed, we do not rely on
      // evt below but use Event.current instead.

      if (Event.current.type != EventType.Layout) {
        if (lostFocus) {
          if (focusController != null) {
            // We dont want to clear the GUIUtility.keyboardControl if another
            // IMGUIContainer just set it in the if (receivedFocus) block below.
            // So we only clear it if own it.
        ...
        if (receivedFocus) {
          if (hasFocusableControls) {
            if (focusChangeDirection != FocusChangeDirection.unspecified &&
                focusChangeDirection != FocusChangeDirection.none) {
              // We got here by tabbing.

              // We assume we are using the VisualElementFocusRing.
              if (focusChangeDirection ==
                  VisualElementFocusChangeDirection.left) {
                GUIUtility.SetKeyboardControlToLastControlId();
              } else if (focusChangeDirection ==
                         VisualElementFocusChangeDirection.right) {
                GUIUtility.SetKeyboardControlToFirstControlId();
              }
            } else if (GUIUtility.keyboardControl == 0 && m_IsFocusDelegated) {
              // Since GUIUtility.keyboardControl == 0, we got focused in some
              // other way than by clicking inside us (for example it could be
              // by clicking in an element that delegates focus to us). Give
              // GUIUtility.keyboardControl to our first control.
              GUIUtility.SetKeyboardControlToFirstControlId();
            }
          }

          if (focusController != null) {
            if (focusController.imguiKeyboardControl !=
        ...
        // We intentionally don't send the NewKeyboardFocus command here since
        // it creates an issue with the AutomatedWindow
        // newKeyboardFocusControlID = GUIUtility.keyboardControl;
      }
      ...
      try {
        ...
        // only for layout events: we always intercept any exceptions to not
        ...
        if (Event.current.type != EventType.Layout && canAffectFocus) {
          ...
          if (focusController != null) {
            if (result < 0) {
              // If CheckForTabEvent returns -1 or -2, we have reach the
              // end/beginning of its control list. We should switch the focus
              // to the next VisualElement.
              Focusable currentFocusedElement =
                  focusController.GetLeafFocusedElement();
              Focusable nextFocusedElement = null;
              using(KeyDownEvent e = KeyDownEvent.GetPooled(
                        '\t', KeyCode.Tab,
                        result == -1 ? EventModifiers.None
              ...
              if (currentFocusedElement == this) {
                if (nextFocusedElement == this) {
                  // We will still have the focus. We should cycle around our
                  // controls.
                  if (result == -2) {
                    GUIUtility.SetKeyboardControlToLastControlId();
                  } else if (result == -1) {
                  ...
                  newKeyboardFocusControlID = GUIUtility.keyboardControl;
                  focusController.imguiKeyboardControl =
                      GUIUtility.keyboardControl;
                } else {
                  // We will lose the focus. Set the focused element ID to 0
                  // until next IMGUIContainer have a chance to set it to its
                  // own control. Doing this will ensure we draw ourselves
                  // without any focused control.
                  GUIUtility.keyboardControl = 0;
                  focusController.imguiKeyboardControl = 0;
                }
              }
            } else if (result > 0) {
              // A positive result indicates that the focused control has
              // changed to one of our elements; result holds the control id.
              focusController.imguiKeyboardControl = GUIUtility.keyboardControl;
              newKeyboardFocusControlID = GUIUtility.keyboardControl;
            } else if (result == 0) {
              // This means the event is not a tab. Synchronize our focus info
              // with IMGUI.

              if (originalEventType == EventType.MouseDown &&
                  !focusOnlyIfHasFocusableControls) {
                focusController.SyncIMGUIFocus(GUIUtility.keyboardControl, this,
                                               true);
              } else if ((currentKeyboardFocus != GUIUtility.keyboardControl) ||
                         (originalEventType == EventType.MouseDown)) {
                focusController.SyncIMGUIFocus(GUIUtility.keyboardControl, this,
                                               false);
              } else if (GUIUtility.keyboardControl !=
                         focusController.imguiKeyboardControl) {
                // Here we want to resynchronize our internal state ...
                newKeyboardFocusControlID = GUIUtility.keyboardControl;

                if (focusController.GetLeafFocusedElement() == this) {
                  // In this case, the focused element is the right one in the
                  // Focus Controller... we are just updating the internal
                  // imguiKeyboardControl
                  focusController.imguiKeyboardControl =
                      GUIUtility.keyboardControl;
                } else {
                  // In this case, the focused element is NOT the right one in
                  // the Focus Controller... we also have to refocus...
                  focusController.SyncIMGUIFocus(GUIUtility.keyboardControl,
                                                 this, false);
                }
              }
            }
      ...
      // See if the container size has changed. This is to make absolutely sure
      // the VisualElement resizes if the IMGUI content resizes.
      if (evt.type == EventType.Layout &&
          (!Mathf.Approximately(previousMeasuredWidth, layoutMeasuredWidth) ||
           !Mathf.Approximately(previousMeasuredHeight,
                                layoutMeasuredHeight))) {
    ...
    private void DoIMGUIRepaint() {
      ...
      HandleIMGUIEvent(elementPanel.repaintData.repaintEvent, m_CachedTransform,
                       m_CachedClippingRect, onGUIHandler, true);
    }

    internal bool SendEventToIMGUI(EventBase evt, bool canAffectFocus = true,
                                   bool verifyBounds = true) {
      if (evt is IPointerEvent) {
        if (evt.imguiEvent != null &&
            evt.imguiEvent.isDirectManipulationDevice) {
          ...
          if (evt is PointerDownEvent) {
            ...
            evt.imguiEvent.type = EventType.TouchUp;
                     ...
                     evt.imguiEvent.rawType == EventType.MouseDrag) {
            sendPointerEvent = true;
          ...
          if (sendPointerEvent) {
            bool result =
                SendEventToIMGUIRaw(evt, canAffectFocus, verifyBounds);
            evt.imguiEvent.type = originalEventType;
            return result;
          }
        }
        // If not touch then we should not handle PointerEvents on IMGUI, we
        // will handle the MouseEvent sent right after
        return false;
      }
    ...
    private bool VerifyBounds(EventBase evt) {
      return IsContainerCapturingTheMouse() || !IsLocalEvent(evt) ||
             IsEventInsideLocalWindow(evt);
    }

    private bool IsContainerCapturingTheMouse() {
      return this == panel?.dispatcher?.pointerState.GetCapturingElement(
                         PointerId.mousePointerId);
    }

    private bool IsLocalEvent(EventBase evt) {
      long evtType = evt.eventTypeId;
      return evtType == MouseDownEvent.TypeId() ||
             evtType == MouseUpEvent.TypeId() ||
             evtType == MouseMoveEvent.TypeId() ||
             evtType == PointerDownEvent.TypeId() ||
             evtType == PointerUpEvent.TypeId() ||
    ...
    private bool IsEventInsideLocalWindow(EventBase evt) {
      ...
      return GUIUtility.HitTest(clippingRect, evt.originalMousePosition,
    ...
    internal bool HandleIMGUIEvent(Event e, Action onGUIHandler,
                                   bool canAffectFocus) {
      GetCurrentTransformAndClip(this, e, out m_CachedTransform,
                                 out m_CachedClippingRect);

      return HandleIMGUIEvent(e, m_CachedTransform, m_CachedClippingRect,
                              onGUIHandler, canAffectFocus);
    }

    private bool HandleIMGUIEvent(Event e, Matrix4x4 worldTransform,
                                  Rect clippingRect, Action onGUIHandler,
                                  bool canAffectFocus) {
      if (e == null || onGUIHandler == null || elementPanel == null ||
          elementPanel.IMGUIEventInterests.WantsEvent(e.rawType) == false) {
        return false;
      }

      EventType originalEventType = e.rawType;
      if (originalEventType != EventType.Layout) {
        if (m_RefreshCachedLayout ||
            elementPanel.IMGUIEventInterests.WantsLayoutPass(e.rawType)) {
          // Only update the layout in-between repaint events.
          e.type = EventType.Layout;
          DoOnGUI(e, worldTransform, clippingRect, false, layout, onGUIHandler,
                  canAffectFocus);
      ...
      if (newKeyboardFocusControlID > 0) {
        ...
        Event focusCommand =
            new Event{type = EventType.ExecuteCommand,
      ...
      if (e.rawType == EventType.Used) {
        ...
        // This can happen if a MouseDown was caught by a different IM element
        // but we ended up here on the MouseUp event because no other element
        // consumed it, including the one that had capture. Example case: start
        // text selection in a text field, but drag mouse all the way into
        // another part of the editor, release the mouse button.  Since the
        // mouse up was sent to another container, we end up here and that is
        // perfectly legal (unfortunately unavoidable for now since no IMGUI
        // control used the event), but hot control might still belong to the IM
        // text field at this point. We can safely release the hot control which
        // will release the capture as the same time.
        GUIUtility.hotControl = 0;
      }

      // If we detect that we were removed while processing this event, hi-jack
      // the event loop to early exit In IMGUI/Editor this is actually possible
      // just by calling EditorWindow.Close() for example
    ...
    protected override void ExecuteDefaultAction(EventBase evt) {
      ...
      //   keyboard, the IMGUIContainer will gain focus via
      //   FocusController.SyncIMGUIFocus.
      // - same thing for tabs: IMGUI should handle them.
      // - we dont want to set the PseudoState.Focus flag on IMGUIContainer.
      //   They are focusable, but only for the purpose of focusing their
      //   children.

      // Here, we set flags that will be acted upon in DoOnGUI(), since we need
      // to change IMGUI state.
      if (evt.eventTypeId == BlurEvent.TypeId()) {
        // A lost focus event is ... a lost focus event.
        // The specific handling of the IMGUI will be done in the DoOnGUI()
        // above...
        lostFocus = true;

        // On lost focus, we need to repaint to remove any focused element blue
        // borders.
        IncrementVersion(VersionChangeType.Repaint);
    ...
    protected internal override Vector2 DoMeasure(float desiredWidth,
      ...
      if (widthMode != MeasureMode.Exactly ||
        ...
        // Make sure the right width/height will be used at the final stage of
        // the calculation
        switch (widthMode) {
        ...
        // When computing layout it's important to not call
        // GetCurrentTransformAndClip because it will remove the dirty flag on
        // the container transform which might set the transform in an invalid
        // state. That's why we have to pass cached transform and clipping state
        // here. It's still important to not pass Rect.zero for the clipping
        // rect as this eventually sets the global GUIClip.visibleRect which
        // IMGUI code could be using to influence size. See case 1111923 and
        // 1158089.
    ...
    private static void GetCurrentTransformAndClip(IMGUIContainer container,
                                                   Event evt,
      ...
      if (evt.rawType == EventType.Repaint && container.elementPanel != null) {
        // during repaint, we must use in case the current transform is not
        // relative to Panel this is to account for the pixel caching feature
        transform = container.elementPanel.repaintData.currentOffset *
