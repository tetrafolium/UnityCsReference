...
namespace UnityEngine.UIElements {
  ...
  public static class PointerCaptureHelper {
    ...
    public static bool HasPointerCapture(this IEventHandler handler,
                                         int pointerId) {
      return GetStateFor(handler) ?.HasPointerCapture(handler, pointerId)
          ?? false;
    }
    ...
    /// </summary>
    /// <param name="handler">The VisualElement that captures the
    /// pointer.</param> <param name="pointerId">The pointer to capture.</param>
    /// <remarks>
    /// When a VisualElement captures a pointer, all pointer events are sent to
    /// the element, regardless of which element is under the pointer.
    /// </remarks>
    public static void CapturePointer(this IEventHandler handler,
                                      int pointerId) {
      GetStateFor(handler) ?.CapturePointer(handler, pointerId);
    }

    /// <summary>
    /// Tests whether an element captured a pointer and, if so, tells the
    /// element to release the pointer.
    /// </summary>
    /// <param name="handler">The element which potentially captured the
    /// pointer.</param> <param name="pointerId">The captured pointer.</param>
    public static void ReleasePointer(this IEventHandler handler,
                                      int pointerId) {
    ...
    public static IEventHandler GetCapturingElement(this IPanel panel,
                                                    int pointerId) {
    ...
    /// </summary>
    /// <param name="panel">The panel that holds the element that captured the
    /// pointer.</param> <param name="pointerId">The captured pointer.</param>
    public static void ReleasePointer(this IPanel panel, int pointerId) {
      panel?.dispatcher?.pointerState.ReleasePointer(pointerId);
    }

    internal static void ActivateCompatibilityMouseEvents(this IPanel panel,
                                                          int pointerId) {
      panel?.dispatcher?.pointerState.ActivateCompatibilityMouseEvents(
          pointerId);
    }

    internal static void PreventCompatibilityMouseEvents(this IPanel panel,
                                                         int pointerId) {
      panel?.dispatcher?.pointerState.PreventCompatibilityMouseEvents(
          pointerId);
    }

    internal static bool ShouldSendCompatibilityMouseEvents(this IPanel panel,
                                                            IPointerEvent evt) {
      return panel?.dispatcher?.pointerState.ShouldSendCompatibilityMouseEvents(
          evt)
          ?? true;
    }

    internal static void ProcessPointerCapture(this IPanel panel,
                                               int pointerId) {
      panel?.dispatcher?.pointerState.ProcessPointerCapture(pointerId);
    }

    internal static void ReleaseEditorMouseCapture() {
      EventDispatcher.editorDispatcher.pointerState.ReleasePointer(
          PointerId.mousePointerId);
      EventDispatcher.editorDispatcher.pointerState.ProcessPointerCapture(
          PointerId.mousePointerId);
    }
  ...
  internal class PointerDispatchState {
    private IEventHandler[] m_PendingPointerCapture =
        new IEventHandler[PointerId.maxPointers];
    private IEventHandler[] m_PointerCapture =
        new IEventHandler[PointerId.maxPointers];
    private bool[] m_ShouldSendCompatibilityMouseEvents =
        new bool[PointerId.maxPointers];

    public PointerDispatchState() { Reset(); }
    ...
    public void ProcessPointerCapture(int pointerId) {
      ...
      if (m_PointerCapture[pointerId] != null) {
        using(var e = PointerCaptureOutEvent.GetPooled(
                  m_PointerCapture[pointerId],
                  m_PendingPointerCapture[pointerId], pointerId)) {
          m_PointerCapture [pointerId]
              .SendEvent(e);
        }

        if (pointerId == PointerId.mousePointerId) {
          using(var e = MouseCaptureOutEvent.GetPooled(
                    m_PointerCapture[pointerId],
                    m_PendingPointerCapture[pointerId], pointerId)) {
            m_PointerCapture [pointerId]
                .SendEvent(e);
      ...
      if (m_PendingPointerCapture[pointerId] != null) {
        using(var e = PointerCaptureEvent.GetPooled(
                  m_PendingPointerCapture[pointerId],
                  m_PointerCapture[pointerId], pointerId)) {
          m_PendingPointerCapture [pointerId]
              .SendEvent(e);
        }

        if (pointerId == PointerId.mousePointerId) {
          using(var e = MouseCaptureEvent.GetPooled(
                    m_PendingPointerCapture[pointerId],
                    m_PointerCapture[pointerId], pointerId)) {
            m_PendingPointerCapture [pointerId]
                .SendEvent(e);
    ...
    public bool ShouldSendCompatibilityMouseEvents(IPointerEvent evt) {
      return evt.isPrimary &&
             m_ShouldSendCompatibilityMouseEvents[evt.pointerId];
    }
  }
}
