...
protected void Recurse(VisualElement element, int depth) {
	...
	while (
		i <
		element.hierarchy.childCount) // here we can't cache childCount as
	                                      // user code could change the hierarchy
	{
		var child = element.hierarchy[i];
		TraverseRecursive(child, depth + 1);

		// if the child has been moved to another parent, which happens when its
		// parent has changed, then do not increment the iterator
