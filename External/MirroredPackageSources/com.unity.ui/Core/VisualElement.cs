...
using PropertyBagValue =
    System.Collections.Generic.KeyValuePair<UnityEngine.PropertyName, object>;

namespace UnityEngine.UIElements {
  // pseudo states are used for common states of a widget
  // they are addressable from CSS via the pseudo state syntax ":selected" for
  // example while css class list can solve the same problem, pseudo states are
  // a fast commonly agreed upon path for common cases.
  [Flags]
  internal enum PseudoStates {
    Active = 1 << 0,   // control is currently pressed in the case of a button
    Hover = 1 << 1,    // mouse is over control, set and removed from dispatcher
                       // automatically
    Checked = 1 << 3,  // usually associated with toggles of some kind to change
                       // visible style
    Disabled = 1 << 5, // control will not respond to user input
    Focus = 1 << 6,    // control has the keyboard focus. This is activated
                       // deactivated by the dispatcher automatically
  ...
  internal static class VisualElementListPool {
    static ObjectPool<List<VisualElement>> pool =
        new ObjectPool<List<VisualElement>>(20);
  ...
  public partial class VisualElement : Focusable, ITransform {
    /// <summary>
    /// Instantiates a <see cref="VisualElement"/> using the data read from a
    /// UXML file.
    /// </summary>
    ...
    public class UxmlTraits : UIElements.UxmlTraits {
      ...
      UxmlStringAttributeDescription m_ViewDataKey =
          new UxmlStringAttributeDescription{name = "view-data-key"};
      protected UxmlEnumAttributeDescription<PickingMode> m_PickingMode =
          new UxmlEnumAttributeDescription<PickingMode>{
              name = "picking-mode", obsoleteNames = new[]{"pickingMode"}};
      UxmlStringAttributeDescription m_Tooltip =
          new UxmlStringAttributeDescription{name = "tooltip"};
      UxmlEnumAttributeDescription<UsageHints> m_UsageHints =
          new UxmlEnumAttributeDescription<UsageHints>{name = "usage-hints"};
      ...
      = new UxmlIntAttributeDescription{
          name = null, obsoleteNames = new[]{"focus-index", "focusIndex"},
          defaultValue = -1};
      UxmlIntAttributeDescription m_TabIndex =
          new UxmlIntAttributeDescription{name = "tabindex", defaultValue = 0};
      /// <summary>
      ...
      = new UxmlBoolAttributeDescription{name = "focusable",
                                         defaultValue = false};

#pragma warning disable 414
      // These variables are used by reflection.
      UxmlStringAttributeDescription m_Class =
          new UxmlStringAttributeDescription{name = "class"};
      UxmlStringAttributeDescription m_ContentContainer =
          new UxmlStringAttributeDescription{name = "content-container",
                                             obsoleteNames =
                                                 new[]{"contentContainer"}};
      UxmlStringAttributeDescription m_Style =
          new UxmlStringAttributeDescription{name = "style"};
#pragma warning restore

      /// <summary>
      /// Returns an enumerable containing
      /// <c>UxmlChildElementDescription(typeof(VisualElement))</c>, since
      /// VisualElements can contain other VisualElements.
      /// </summary>
      ...
      /// <summary>
      /// Initialize <see cref="VisualElement"/> properties using values from
      /// the attribute bag.
      /// </summary>
      ...
      public override void Init(VisualElement ve, IUxmlAttributes bag,
        ...
        // usageHints can't be changed if the element is attached to a panel.
        // Here we ignore usageHints if the VE is attached to a panel already.
        // This happens in the UIBuilder, and the hints don't cause visual or
        // behavioral differences, so it is safe from an editing experience
        // stand point.
        if (ve.panel == null)
        ...
        // We ignore m_ContentContainer, it was processed in
        // UIElementsViewImporter. We ignore m_Style, it was processed in
    ...
    internal static readonly PropertyName userDataPropertyKey =
        new PropertyName("--unity-user-data");
    /// <summary>
    ...
    // Used for view data persistence (ie. scroll position or tree view expanded
    // states)
    private string m_ViewDataKey;
    /// <summary>
    /// Used for view data persistence (ie. tree expanded states, scroll
    /// position, zoom level).
    /// </summary>
    /// <remarks>
    /// This is the key used to save/load the view data from the view data
    /// store. Not setting this key will disable persistence for this <see
    /// cref="VisualElement"/>.
    /// </remarks>
    ...
    // Persistence of view data is almost always enabled as long as an element
    // has a valid viewDataKey. The only exception is when an element is in its
    // parent's shadow tree, that is, not a physical child of its logical
    // parent's contentContainer. In this exception case, persistence is
    // disabled on the element even if the element does have a viewDataKey, if
    // its logical parent does not have a viewDataKey. This check internally
    // controls whether or not view data persistence is enabled as the
    // VisualTreeViewDataUpdater traverses the visual tree.
    ...
    /// <summary>
    /// This property can be used to associate application-specific user data
    /// with this VisualElement.
    /// </summary>
    ...
    public override bool canGrabFocus {
      get {
        ...
        return !focusDisabledOnComposite && visible &&
               (resolvedStyle.display != DisplayStyle.None) &&
               enabledInHierarchy && base.canGrabFocus;
      }
    }
    ...
    /// <summary>
    /// A combination of hint values that specify high-level intended usage
    /// patterns for the <see cref="VisualElement"/>. This property can only be
    /// set when the <see cref="VisualElement"/> is not yet part of a <see
    /// cref="Panel"/>. Once part of a <see cref="Panel"/>, this property
    /// becomes effectively read-only, and attempts to change it will throw an
    /// exception. The specification of proper <see cref="UsageHints"/> drives
    /// the system to make better decisions on how to process or accelerate
    /// certain operations based on the anticipated usage pattern. Note that
    /// those hints do not affect behavioral or visual results, but only affect
    /// the overall performance of the panel and the elements within. Generally
    /// it advised to always consider specifying the proper <see
    /// cref="UsageHints"/>, but keep in mind that some <see cref="UsageHints"/>
    /// may be internally ignored under certain conditions (e.g. due to hardware
    /// limitations on the target platform).
    /// </summary>
    public UsageHints usageHints {
      get {
        return (((m_RenderHints & RenderHints.GroupTransform) != 0)
                    ? UsageHints.GroupTransform
                    : 0) |
               (((m_RenderHints & RenderHints.BoneTransform) != 0)
      ...
      set {
        if (panel != null)
          throw new InvalidOperationException(
              "usageHints cannot be changed once the VisualElement is part of an active visual tree");
    ...
    internal RenderHints renderHints {
      ...
      set {
        if (panel != null)
          throw new InvalidOperationException(
              "renderHints cannot be changed once the VisualElement is part of an active visual tree");
        m_RenderHints = value;
      }
    }
    ...
    Vector3 ITransform.scale {
      ...
      set {
        ...
        IncrementVersion(
            ...
            VersionChangeType.Layout /*This will change how we measure text*/);
      }
    }
    ...
    internal float scaledPixelsPerPoint {
      get {
        return panel == null ? GUIUtility
                            .pixelsPerPoint : (panel as BaseVisualElementPanel)
    ...
    public Rect layout {
      ...
      internal set {
        ...
        // Same position value while type is already manual should not trigger
        // any layout change, return early
        if (isLayoutManual && m_Layout == value)
          return;

        Rect lastLayout = layout;
        VersionChangeType changeType = 0;
        if (!Mathf.Approximately(lastLayout.x, value.x) ||
            !Mathf.Approximately(lastLayout.y, value.y))
          changeType |= VersionChangeType.Transform;
        if (!Mathf.Approximately(lastLayout.width, value.width) ||
            !Mathf.Approximately(lastLayout.height, value.height))
          changeType |= VersionChangeType.Size;

        // set results so we can read straight back in get without waiting for a
        // pass
        m_Layout = value;
        isLayoutManual = true;
    ...
    public Rect contentRect {
      get {
        var spacing = new Spacing(
            resolvedStyle.paddingLeft, resolvedStyle.paddingTop,
            resolvedStyle.paddingRight, resolvedStyle.paddingBottom);
    ...
    protected Rect paddingRect {
      get {
        var spacing = new Spacing(
            resolvedStyle.borderLeftWidth, resolvedStyle.borderTopWidth,
            resolvedStyle.borderRightWidth, resolvedStyle.borderBottomWidth);
    ...
    internal static Rect TransformAlignedRect(Matrix4x4 lhc, Rect rect) {
      ...
      // We assume that the transform performs translation/scaling without
      // rotation.
      return Rect.MinMaxRect(Math.Min(min.x, max.x), Math.Min(min.y, max.y),
                             Math.Max(min.x, max.x), Math.Max(min.y, max.y));
    }

    internal static Vector2 MultiplyMatrix44Point2(Matrix4x4 lhs,
                                                   Vector2 point) {
      Vector2 res;
    ...
    internal void UpdateBoundingBox() {
      if (float.IsNaN(rect.x) || float.IsNaN(rect.y) ||
        ...
        for (int i = 0; i < childCount; i++) {
          ...
          childBB = m_Children [i]
                        .ChangeCoordinatesTo(this, childBB);
    ...
    /// <summary>
    /// AABB after applying the transform to the rect, but before applying the
    /// layout translation.
    /// </summary>
    ...
    internal Rect rect {
      get { return new Rect(0.0f, 0.0f, layout.width, layout.height); }
    }
    ...
    /// -Local Translation
    /// -Layout Translation
    /// -Parent <c>worldTransform</c> (recursive definition - consider identity
    /// when there is no parent)
    /// </summary>
    /// <remarks>
    /// Multiplying the <c>layout</c> rect by this matrix is incorrect because
    /// it already contains the translation.
    /// </remarks>
    ...
    private void UpdateWorldTransform() {
      // If we are during a layout we don't want to remove the dirty transform
      // flag since this could lead to invalid computed transform (see
      // ScopeContentainer.DoMeasure)
      ...
      if (hierarchy.parent != null) {
        m_WorldTransformCache =
            hierarchy.parent.worldTransform * offset * transform.matrix;
      } else {
    ...
    private static readonly Rect s_InfiniteRect =
        new Rect(-10000, -10000, 40000, 40000);
    ...
    private void UpdateWorldClip() {
      if (hierarchy.parent != null) {
        ...
        if (hierarchy.parent !=
            renderChainData
                .groupTransformAncestor) // Accessing render data here?
          m_WorldClipMinusGroup = hierarchy.parent.worldClipMinusGroup;
        else
          m_WorldClipMinusGroup =
              panel?.contextType == ContextType.Player ? s_InfiniteRect
              : GUIClip.topmostRect;
        ...
        m_WorldClipMinusGroup = m_WorldClip =
            (panel != null) ? panel.visualTree.rect : s_InfiniteRect;
      }
    }

    private Rect SubstractBorderPadding(Rect worldRect) {
      // Case 1222517: We must take the scaling into consideration when applying
      // local changes to the world rect.
      ...
      if (computedStyle.unityOverflowClipBox == OverflowClipBox.ContentBox) {
        ...
        worldRect.width -=
            (resolvedStyle.paddingLeft + resolvedStyle.paddingRight) * xScale;
        worldRect.height -=
            (resolvedStyle.paddingTop + resolvedStyle.paddingBottom) * yScale;
      }
    ...
    internal static Rect ComputeAAAlignedBound(Rect position, Matrix4x4 mat) {
      ...
      Vector3 v3 = mat.MultiplyPoint3x4(
          new Vector3(p.x + p.width, p.y + p.height, 0.0f));
    ...
    /// <summary>
    /// Determines if this element can be pick during mouseEvents or <see
    /// cref="IPanel.Pick"/> queries.
    /// </summary>
    ...
    // user-defined style object, if not set, is the same reference as
    // m_SharedStyles
    ...
    // IMGUIContainers are special snowflakes that need custom treatment
    // regarding events. This enables early outs in some dispatching strategies.
    ...
    protected override void ExecuteDefaultAction(EventBase evt) {
      ...
      if (evt.eventTypeId == MouseOverEvent.TypeId() ||
          evt.eventTypeId == MouseOutEvent.TypeId()) {
        // Updating cursor has to happen on MouseOver/Out because exiting a
        // children do not send a mouse enter to the parent.
    ...
    internal void SetPanel(BaseVisualElementPanel p) {
      ...
      // We now gather all Elements in order to dispatch events in an efficient
      // manner
      ...
      try {
        ...
        using(pDispatcherGate) using(panelDispatcherGate) {
    ...
    void ChangePanel(BaseVisualElementPanel p) {
      ...
      IncrementVersion(VersionChangeType.StyleSheet | VersionChangeType.Layout |
                       VersionChangeType.Transform);
    ...
    [Obsolete(
        "SetEnabledFromHierarchy is deprecated and will be removed in a future release. Please use SetEnabled instead.")]
    protected internal bool SetEnabledFromHierarchy(bool state) {
      return SetEnabledFromHierarchyPrivate(state);
    }

    // TODO: rename to SetEnabledFromHierarchy once the protected version has
    // been removed
    ...
    private bool isParentEnabledInHierarchy {
      get {
        return hierarchy.parent == null || hierarchy.parent.enabledInHierarchy;
      }
    }

    // Returns true if 'this' can be enabled relative to the enabled state of
    // its panel
    /// <summary>
    /// Returns true if the <see cref="VisualElement"/> is enabled in its own
    /// hierarchy.
    /// </summary>
    /// <remarks>
    ///  This flag verifies if the element is enabled globally. A parent
    ///  disabling its child VisualElement affects this variable.
    /// </remarks>
    ...
    /// <summary>
    /// Changes the <see cref="VisualElement"/> enabled state. A disabled
    /// VisualElement does not receive most events.
    /// </summary>
    /// <param name="value">New enabled state</param>
    /// <remarks>
    /// The method disables the local flag of the VisualElement and implicitly
    /// disables its children. It does not affect the local enabled flag of each
    /// child.
    /// </remarks>
    ...
    public bool visible {
      get { return resolvedStyle.visibility == Visibility.Visible; }
      set {
        // Note: this could causes an allocation because styles are
        // copy-on-write we might want to remove this setter altogether so
        // everything goes through style.visibility (and then it's documented in
        // a single place)
    ...
    /// <summary>
    /// Called when the <see cref="VisualElement"/> visual contents need to be
    /// (re)generated.
    /// </summary>
    /// <remarks>
    /// When handled, it is possible to generate custom geometry in the content
    /// region of the <see cref="VisualElement"/>.
    ///                     This delegate is called only when the <see
    ///                     cref="VisualElement"/> has been detected to need to
    ///                     regenerate its visual contents. It is not called
    ///                     every frame when refreshing the panel. The content
    ///                     generated is cached and remains intact until a
    ///                     property on the <see cref="VisualElement"/>
    ///                     affecting visuals has changed or <see
    ///                     cref="VisualElement.MarkDirtyRepaint"/> is called.
    ///                     While executing code in a handler to this delegate,
    ///                     refrain from making changes to any property of the
    ///                     <see cref="VisualElement"/>. A correct handler
    ///                     should treat the <see cref="VisualElement"/> as
    ///                     'read-only' and generate the geometry without
    ///                     causing side-effects. Changes done to the <see
    ///                     cref="VisualElement"/> during this event could be
    ///                     missed or lag appearance at best.
    /// </remarks>
    ...
    internal T GetOrCreateViewData<T>(object existing, string key) where T
        : class
    , new () {
      Debug.Assert(
          elementPanel != null,
          "VisualElement.elementPanel is null! Cannot load persistent data.");

      var viewData = elementPanel == null ||
                     elementPanel.getViewDataDictionary == null ? null
          : elementPanel.getViewDataDictionary();

      // If persistency is disable (no data, no key, no key one of the parents),
      // just return the existing data or create a local one if none exists.
      if (viewData == null || string.IsNullOrEmpty(viewDataKey) ||
          enableViewDataPersistence == false) {
        if (existing != null)
    ...
    internal T GetOrCreateViewData<T>(ScriptableObject existing,
                                      string key) where T : ScriptableObject {
      Debug.Assert(
          elementPanel != null,
          "VisualElement.elementPanel is null! Cannot load view data.");
    ...
    internal void OverwriteFromViewData(object obj, string key) {
      ...
      // If persistency is disable (no data, no key, no key one of the parents),
      // just return the existing data or create a local one if none exists.
      if (viewDataPersistentData == null || string.IsNullOrEmpty(viewDataKey) ||
          enableViewDataPersistence == false) {
        return;
      }
    ...
    internal void SaveViewData() {
      if (elementPanel != null && elementPanel.saveViewData != null &&
          !string.IsNullOrEmpty(viewDataKey) && enableViewDataPersistence) {
    ...
    internal bool IsViewDataPersitenceSupportedOnChildren(bool existingState) {
      ...
      // If this element has no key AND this element has a custom
      // contentContainer, turn off view data persistence. This essentially
      // turns off persistence on shadow elements if the parent has no key.
      if (string.IsNullOrEmpty(viewDataKey) && this != contentContainer)
        newState = false;

      // However, once we enter the light tree again, we need to turn
      // persistence back on.
    ...
    /// </summary>
    /// <remarks>
    /// This enum value is passed to <see
    /// cref="UIElements.VisualElement.DoMeasure"/>. This lets UI elements
    /// indicate their natural size during the layout algorithm.
    /// </remarks>
    /// <seealso cref="VisualElement.MeasureTextSize"/>
    public enum MeasureMode {
      /// <summary>
      /// The element should give its preferred width/height without any
      /// constraint.
      /// </summary>
      Undefined = YogaMeasureMode.Undefined,
      /// <summary>
      /// The element should give the width/height that is passed in and derive
      /// the opposite site from this value (for example, calculate text size
      /// from a fixed width).
      /// </summary>
      Exactly = YogaMeasureMode.Exactly,
      /// <summary>
      /// At Most. The element should give its preferred width/height but no
      /// more than the value passed.
      /// </summary>
    ...
    private void AssignMeasureFunction() {
      yogaNode.SetMeasureFunction((node, f, mode, f1, heightMode) =>
                                      Measure(node, f, mode, f1, heightMode));
    }

    private void RemoveMeasureFunction() { yogaNode.SetMeasureFunction(null); }

    protected internal virtual Vector2 DoMeasure(float desiredWidth,
    ...
    internal YogaSize Measure(YogaNode node, float width,
                              YogaMeasureMode widthMode, float height,
                              YogaMeasureMode heightMode) {
      Debug.Assert(node == yogaNode, "YogaNode instance mismatch");
      Vector2 size = DoMeasure(width, (MeasureMode) widthMode, height,
                               (MeasureMode) heightMode);
      float ppp = scaledPixelsPerPoint;
      return MeasureOutput.Make(AlignmentUtils.RoundToPixelGrid(size.x, ppp),
                                AlignmentUtils.RoundToPixelGrid(size.y, ppp));
    }
    ...
    internal void SetSharedStyles(ComputedStyle sharedStyle) {
      ...
      // This is a pre-emptive since we do not know if style changes actually
      // cause a repaint or a layout But those should be the only possible type
      // of changes needed
    ...
    public override string ToString() {
      return GetType().Name + " " + name + " " + layout +
             " world rect: " + worldBound;
    }
    ...
    public IEnumerable<string> GetClasses() { return m_ClassList; }
    ...
    /// <summary>
    /// Toggles between adding and removing the given class name from the class
    /// list.
    /// </summary>
    /// <param name="className">The class name to add or remove from the class
    /// list.</param> <remarks> Checks for the given class name in the element
    /// class list. If the class name is found, it is removed from the class
    /// list. If the class name is not found, the class name is added to the
    /// class list.
    /// </remarks>
    ...
    /// </summary>
    /// <param name="className">The name of the class to enable or
    /// disable.</param> <param name="enable">A boolean flag that adds or
    /// removes the class name from the class list. If true, EnableInClassList
    /// adds the class name to the class list. If false, EnableInClassList
    /// removes the class name from the class list.</param> <remarks> If enable
    /// is true, EnableInClassList adds the class name to the class list. If
    /// enable is false, EnableInClassList removes the class name from the class
    /// list.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// This will ignore the current userData and return the first parent's
    /// non-null userData
    /// </remarks>
    ...
    static void CheckUserKeyArgument(PropertyName key) {
      ...
      if (key == userDataPropertyKey)
        throw new InvalidOperationException(
            $"The {userDataPropertyKey} key is reserved by the system");
    }
    ...
    private void UpdateCursorStyle(long eventType) {
      if (elementPanel != null) {
        if (eventType == MouseOverEvent.TypeId() &&
            elementPanel.GetTopElementUnderPointer(PointerId.mousePointerId) ==
                this) {
  ...
  public static class VisualElementExtensions {
    // transforms a point assumed in Panel space to the referential inside of
    // the element bound (local)
    ...
    // transforms a rect assumed in Panel space to the referential inside of the
    // element bound (local)
    public static Rect WorldToLocal(this VisualElement ele, Rect r) {
      ...
      Vector2 position = VisualElement.MultiplyMatrix44Point2(
          ele.worldTransformInverse, r.position);
      r.position = position;
    ...
    public static Rect LocalToWorld(this VisualElement ele, Rect r) {
      ...
      r.position =
          VisualElement.MultiplyMatrix44Point2(toWorldMatrix, r.position);
      r.size = toWorldMatrix.MultiplyVector(r.size);
      return r;
    }

    // transform point from the local space of one element to to the local space
    // of another
    public static Vector2 ChangeCoordinatesTo(this VisualElement src,
                                              VisualElement dest,
                                              Vector2 point) {
      return dest.WorldToLocal(src.LocalToWorld(point));
    }

    // transform Rect from the local space of one element to to the local space
    // of another
    public static Rect ChangeCoordinatesTo(this VisualElement src,
                                           VisualElement dest, Rect rect) {
    ...
    /// <summary>
    /// The given VisualElement's left and right edges will be aligned with the
    /// corresponding edges of the parent element.
    /// </summary>
    ...
    /// <param name="manipulator">Manipulator to be added to the
    /// VisualElement.</param>
    public static void AddManipulator(this VisualElement ele,
    ...
    /// </summary>
    /// <param name="ele">VisualElement associated to the manipulator.</param>
    /// <param name="manipulator">Manipulator to be removed from the
    /// VisualElement.</param>
    public static void RemoveManipulator(this VisualElement ele,
                                         IManipulator manipulator) {
      if (manipulator != null) {
  ...
  internal static class VisualElementDebugExtensions {
    public static string GetDisplayName(this VisualElement ve,
