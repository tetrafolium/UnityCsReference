...
/// <summary>
/// Instantiates and clones a <see cref="TemplateContainer"/> using the data
/// read from a UXML file.
/// </summary>
...
uxmlNamespace + "." + uxmlName;
}

/// <summary>
/// Defines <see cref="UxmlTraits"/> for the <see
/// cref="TemplateContainer"/>.
/// </summary>
...
UxmlStringAttributeDescription m_Template =
	...
	name = k_TemplateAttributeName,
	use = UxmlAttributeDescription.Use.Required
};

/// <summary>
/// Returns an empty enumerable, as template instance do not have
/// children.
/// </summary>
...
/// <summary>
/// Initialize <see cref="TemplateContainer"/> properties using values
...
public override void Init(VisualElement ve, IUxmlAttributes bag,
	...
	VisualTreeAsset vea =
		cc.visualTreeAsset?.ResolveTemplate(templateContainer.templateId);

	if (vea == null)
		templateContainer.Add(new Label(string.Format(
							"Unknown Template: '{0}'", templateContainer.templateId)));
	else {
		...
		if (bagOverrides != null || contextOverrides != null) {
			// We want to add contextOverrides first here, then bagOverrides, as
			// we want parent instances to always override child instances.
	...
	if (vea == null)
		Debug.LogErrorFormat("Could not resolve template with name '{0}'",
