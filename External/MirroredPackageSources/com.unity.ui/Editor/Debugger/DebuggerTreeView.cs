...
namespace UnityEditor.UIElements.Debugger {
  internal class DebuggerTreeView : VisualElement {
    public bool hierarchyHasChanged {
      get;
      set;
    }

    public IList<ITreeViewItem> treeRootItems {
      get { return m_TreeRootItems; }
      private set {}
    }
    ...
    public void RebuildTree(IPanelDebug panelDebug) {
      ...
      Func<VisualElement> makeItem = () => {
        ...
        element.RegisterCallback<MouseEnterEvent>(
            (e) => { HighlightItemInTargetWindow(e.target as VisualElement); });
      ...
      // Clear selection which would otherwise persist via view data
      // persistence.
    ...
    private TreeViewItem<VisualElement>
    FindElement(IEnumerable<ITreeViewItem> list, VisualElement element) {
      if (list == null)
        return null;
    ...
    public void SelectElement(VisualElement element, string query,
      ...
      var selected = m_TreeView.Q(className
      ...
      var labelContainers =
          content.Query(classes
                        : "unity-debugger-tree-item-label-cont")
              .ToList();
      foreach (var labelContainer in labelContainers) {
        var label = labelContainer.Q<Label>();

        if (label.ClassListContains("unity-debugger-tree-item-type") &&
            searchHighlight != SearchHighlight.Type)
          continue;

        if (label.ClassListContains("unity-debugger-tree-item-name") &&
            searchHighlight != SearchHighlight.Name)
          continue;

        if (label.ClassListContains("unity-debugger-tree-item-classlist") &&
            searchHighlight != SearchHighlight.Class)
          continue;
    ...
    private void AddTreeItemsForElement(IList<ITreeViewItem> items,
                                        VisualElement ve, ref int nextId) {
      if (ve == null)
        return;
