...
namespace UnityEditor.UIElements.Debugger {
  ...
  internal class PanelChoice : IPanelChoice {
    ...
    public PanelChoice(Panel p) { panel = p; }
  ...
  internal class PanelDebugger : IPanelDebugger {
    ...
    public void Initialize(EditorWindow debuggerWindow) {
      ...
      m_PanelSelect = new ToolbarMenu(){name = "panelSelectPopup",
      ...
      if (!string.IsNullOrEmpty(m_LastVisualTreeName))
        m_RestoreSelectionScheduledItem =
            m_Toolbar.schedule.Execute(RestorePanelSelection).Every(500);
    }
    ...
    public void ScheduleWindowToDebug(EditorWindow window) {
      if (window != null) {
        ...
        m_ConnectWindowScheduledItem =
            m_Toolbar.schedule.Execute(TrySelectWindow).Every(500);
      }
    }
    ...
    PopulatePanelChoices(List<IPanelChoice> panelChoices) {
      ...
      while (it.MoveNext()) {
        ...
        GUIView view =
            guiViews.FirstOrDefault(v => v.GetInstanceID() == it.Current.Key);
    ...
    private void RefreshPanelChoices() {
      ...
      foreach (var panelChoice in m_PanelChoices) {
        menu.AppendAction(panelChoice.ToString(), OnSelectPanel,
                          DropdownMenuAction.AlwaysEnabled, panelChoice);
      }
    }

    private void OnSelectPanel(DropdownMenuAction action) {
      if (m_RestoreSelectionScheduledItem != null &&
          m_RestoreSelectionScheduledItem.isActive)
    ...
    public virtual void OnVersionChanged(VisualElement ve,
                                         VersionChangeType changeTypeFlag) {}

    public virtual bool InterceptEvent(EventBase ev) { return false; }
  ...
  internal static class UIElementsDebuggerExtension {
    public static VisualElement GetRootVisualElement(this IPanel panel) {
      ...
      // GUIView only has an IMGUIContainer and HostView the IMGUIContainer +
      // root element
    ...
    public static int FindVisualElementIndex(this IPanel panel,
                                             VisualElement ve) {
      if (panel == null)
        return -1;
    ...
    public static VisualElement FindVisualElementByIndex(this IPanel panel,
                                                         int index) {
    ...
    private static bool RecurseVisualElementIndex(VisualElement root,
                                                  VisualElement ve,
                                                  ref int index) {
      if (root == ve)
        return true;
    ...
    RecurseVisualElementByIndex(VisualElement ve, int index,
