...
namespace UnityEditor.UIElements.Debugger {
  ...
  internal class StylePropertyDebugger : VisualElement {
    static readonly List<StylePropertyInfo> s_StylePropertyInfos =
        new List<StylePropertyInfo>();

    private Dictionary<StylePropertyId, StyleField> m_IdToFieldDictionary =
        new Dictionary<StylePropertyId, StyleField>();
    private Dictionary<StylePropertyId, int> m_PropertySpecificityDictionary =
        new Dictionary<StylePropertyId, int>();
    ...
    public void Refresh() { RefreshFields(); }

    public void SetMatchRecords(VisualElement selectedElement,
                                IEnumerable<SelectorMatchRecord> matchRecords) {
      m_SelectedElement = selectedElement;
      m_PropertySpecificityDictionary.Clear();

      if (selectedElement != null)
        StyleDebug.FindSpecifiedStyles(selectedElement.computedStyle,
                                       matchRecords,
                                       m_PropertySpecificityDictionary);
    ...
    private void FindInlineStyles() {
      ...
      var inlineRulePropIds =
          m_SelectedElement.inlineStyleAccess.inlineRule.propertyIds;
    ...
    private void RefreshFields() {
      ...
      if (customProperties != null && customProperties.Any()) {
        foreach (KeyValuePair<string, StylePropertyValue> customProperty in
                     customProperties) {
          ...
          textField.value =
              propValue.sheet.ReadAsString(propValue.handle).ToLower();
      ...
      foreach (var propertyInfo in s_StylePropertyInfos) {
        ...
        if (!string.IsNullOrEmpty(m_SearchFilter) &&
            styleName.IndexOf(m_SearchFilter,
                              StringComparison.InvariantCultureIgnoreCase) ==
                -1)
          continue;

        var id = propertyInfo.id;
        object val = StyleDebug.GetComputedStyleValue(
            m_SelectedElement.computedStyle, id);
        ...
        if (m_ShowAll || specificity != StyleDebug.UndefinedSpecificity) {
          if (sf != null) {
            ...
            sf = new StyleField(m_SelectedElement, propertyInfo, val,
                                specificity);
    ...
    GetUSSPropertyNameFromComputedStyleName(string computedStyleName) {
  ...
  internal class StyleField : VisualElement {
    ...
    public StyleField(VisualElement selectedElement, StylePropertyInfo propInfo,
                      object value, int specificity) {
    ...
    public void RefreshPropertyValue(object val, int specificity) {
      if (val is StyleFloat) {
        ...
        StyleLengthField field =
            GetOrCreateField<StyleLengthField, StyleLength>();
        if (!IsFocused(field))
        ...
        if (!IsFocused(field))
          field.SetValueWithoutNotify(
              background.vectorImage != null ?(UnityEngine.Object)
                                            background.vectorImage
              : (UnityEngine.Object) background.texture);
        ...
        if (style.value.texture != null) {
          var uiTextureField =
              new ObjectField(m_PropertyName){value = style.value.texture};
          uiTextureField.RegisterValueChangedCallback(e => {
            StyleCursor styleCursor =
                (StyleCursor) StyleDebug.GetComputedStyleValue(
                    m_SelectedElement.computedStyle, m_PropertyInfo.id);
          ...
          var uiHotspotField =
              new Vector2Field("hotspot"){value = style.value.hotspot};
    ...
    private void SetPropertyValue(object newValue) {
      ...
      if (newValue == null) {
        ...
        if (type == typeof(StyleBackground)) {
          val = new StyleBackground(newValue as Texture2D);
                   ...
                   typeof(StyleEnum<Overflow>) &&newValue is OverflowInternal) {
          OverflowInternal newV = (OverflowInternal) newValue;
          Overflow v =
              newV == OverflowInternal.Hidden ? Overflow.Hidden
              : Overflow.Visible;
      ...
      StyleDebug.SetInlineStyleValue(m_SelectedElement.style, m_PropertyInfo.id,
                                     val);
