...
namespace UnityEditor.UIElements.Debugger {
  ...
  internal class OverlayData {
    ...
    public void StartFadeOutAnimation(VisualElement container, int duration) {
      if (m_animation != null) {
        ...
        m_animation = container.experimental.animation
                          .Start(defaultAlpha, 0, duration,
                                 (ve, value) => {
                                   alpha = value;
                                   ve.MarkDirtyRepaint();
                                 })
  ...
  internal abstract class BaseOverlayPainter {
    protected Dictionary<VisualElement, OverlayData> m_OverlayData =
        new Dictionary<VisualElement, OverlayData>();
    ...
    public void ClearOverlay() { m_OverlayData.Clear(); }

    protected abstract void DrawOverlayData(MeshGenerationContext mgc,
                                            OverlayData overlayData);

    protected void DrawRect(MeshGenerationContext mgc, Rect rect, Color color,
                            float alpha) {
      if (mgc == null)
      ...
      var rectParams = MeshGenerationContextUtils.RectangleParams.MakeSolid(
          rect, color, mgc.visualElement.panel.contextType);
      mgc.Rectangle(rectParams);
    }

    protected void DrawBorder(MeshGenerationContext mgc, Rect rect, Color color,
                              float alpha) {
      if (mgc == null)
        throw new NullReferenceException("The MeshGenerationContext is null");
      ...
      var rectParams = MeshGenerationContextUtils.RectangleParams.MakeSolid(
          topRect, color, mgc.visualElement.panel.contextType);
      mgc.Rectangle(rectParams);

      rectParams = MeshGenerationContextUtils.RectangleParams.MakeSolid(
          bottomRect, color, mgc.visualElement.panel.contextType);
      mgc.Rectangle(rectParams);

      rectParams = MeshGenerationContextUtils.RectangleParams.MakeSolid(
          rightRect, color, mgc.visualElement.panel.contextType);
      mgc.Rectangle(rectParams);

      rectParams = MeshGenerationContextUtils.RectangleParams.MakeSolid(
          lefRect, color, mgc.visualElement.panel.contextType);
      mgc.Rectangle(rectParams);
    }
  }
  ...
  internal class HighlightOverlayPainter : BaseOverlayPainter {
    ...
    private static readonly Color kHighlightContentColor =
        new Color(0.1f, 0.6f, 0.9f);
    private static readonly Color kHighlightPaddingColor =
        new Color(0.1f, 0.9f, 0.1f);
    private static readonly Color kHighlightBorderColor =
        new Color(1.0f, 1.0f, 0.4f);
    private static readonly Color kHighlightMarginColor =
    ...
    public void AddOverlay(VisualElement ve,
                           OverlayContent content = OverlayContent.All) {
    ...
    protected override void DrawOverlayData(MeshGenerationContext mgc,
                                            OverlayData od) {
    ...
    private void FillHighlightRects(VisualElement ve) {
      ...
      paddingLeftRect =
          new Rect(contentRect.xMin - paddingLeft, contentRect.yMin,
                   paddingLeft, contentRect.height);

      paddingRightRect = new Rect(contentRect.xMax, contentRect.yMin,
                                  paddingRight, contentRect.height);

      paddingTopRect = new Rect(
          contentRect.xMin - paddingLeft, contentRect.yMin - paddingTop,
          contentRect.width + paddingLeft + paddingRight, paddingTop);

      paddingBottomRect = new Rect(
          contentRect.xMin - paddingLeft, contentRect.yMax,
      ...
      borderLeftRect = new Rect(
          paddingLeftRect.xMin - borderLeft, paddingTopRect.yMin, borderLeft,
          paddingLeftRect.height + paddingBottomRect.height +
              paddingTopRect.height);

      borderRightRect =
          new Rect(paddingRightRect.xMax, paddingTopRect.yMin, borderRight,
                   paddingRightRect.height + paddingBottomRect.height +
                       paddingTopRect.height);

      borderTopRect = new Rect(
          paddingTopRect.xMin - borderLeft, paddingTopRect.yMin - borderTop,
          paddingTopRect.width + borderLeft + borderRight, borderTop);

      borderBottomRect = new Rect(
          paddingBottomRect.xMin - borderLeft, paddingBottomRect.yMax,
      ...
      marginLeftRect = new Rect(
          borderLeftRect.xMin - marginLeft, borderTopRect.yMin, marginLeft,
          borderLeftRect.height + borderBottomRect.height +
              borderTopRect.height);

      marginRightRect =
          new Rect(borderRightRect.xMax, borderTopRect.yMin, marginRight,
                   borderRightRect.height + borderBottomRect.height +
                       borderTopRect.height);

      marginTopRect = new Rect(
          borderTopRect.xMin - marginLeft, borderTopRect.yMin - marginTop,
          borderTopRect.width + marginLeft + marginRight, marginTop);

      marginBottomRect = new Rect(
          borderBottomRect.xMin - marginLeft, borderBottomRect.yMax,
  ...
  internal class RepaintOverlayPainter : BaseOverlayPainter {
    ...
    public void AddOverlay(VisualElement ve, VisualElement debugContainer) {
      ...
      overlayData.StartFadeOutAnimation(debugContainer,
                                        kOverlayFadeOutDuration);
    }
  ...
  internal class LayoutOverlayPainter : BaseOverlayPainter {
    ...
    public override void Draw(MeshGenerationContext mgc) {
      ...
      if (selectedElement != null)
        DrawBorder(mgc, selectedElement.worldBound, kSelectedBoundColor,
                   kDefaultAlpha);
    }
