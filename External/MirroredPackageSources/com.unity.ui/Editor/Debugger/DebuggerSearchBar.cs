...
internal enum SearchHighlight { None, Type, Name, Class }
...
public DebuggerSearchBar(DebuggerTreeView parent) {
	...
	field.RegisterCallback<KeyDownEvent>((e) => {
				...
				if (e.keyCode == KeyCode.F3 || e.keyCode == KeyCode.Return ||
...
private void SelectNext() {
	...
	m_ParentTreeView.SelectElement(m_FoundItems[m_SelectedIndex].item.data,
	                               m_CurrentQuery,
	                               m_FoundItems[m_SelectedIndex].highlight);
	m_CountLabel.text =
...
private void PerformSearch(ChangeEvent<string> evt) {
	...
	foreach (var item in items) {
		...
		if (m_CurrentFilter == DebuggerSearchBarFilter.All &&
		    element.typeName.IndexOf(query,
		...
		if ((m_CurrentFilter & DebuggerSearchBarFilter.Name) != 0 &&
		    element.name.IndexOf(query, StringComparison.OrdinalIgnoreCase) >=
		    0) {
			m_FoundItems.Add(new SearchResultItem(){
		...
		if ((m_CurrentFilter & DebuggerSearchBarFilter.Class) != 0) {
			foreach (var styleClass in element.GetClasses()) {
				if (styleClass.IndexOf(query, StringComparison.OrdinalIgnoreCase) >=
				    0) {
					m_FoundItems.Add(new SearchResultItem(){
	...
	m_ParentTreeView.SelectElement(firstItem.item.data, query,
	                               firstItem.highlight);
}
}
}
