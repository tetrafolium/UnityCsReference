...
namespace UnityEditor.UIElements.Debugger {
  ...
  internal class DebuggerContext {
    ...
    public DebuggerSelection selection { get; }
    = new DebuggerSelection();
  ...
  internal class UIElementsDebugger : EditorWindow {
    ...
    public static readonly string OpenWindowCommand =
        nameof(OpenUIElementsDebugger);
    ...
    private static void DebugWindowShortcut() {
      if (CommandService.Exists(OpenWindowCommand))
        CommandService.Execute(OpenWindowCommand, CommandHint.Shortcut,
    ...
    public void OnDisable() { m_DebuggerImpl.OnDisable(); }

    public void OnFocus() { m_DebuggerImpl.OnFocus(); }
  }

  [Serializable]
  internal class UIElementsDebuggerImpl : PanelDebugger,
                                          IGlobalPanelDebugger {
    const string k_DefaultStyleSheetPath =
        "UIPackageResources/StyleSheets/UIElementsDebugger/UIElementsDebugger.uss";
    const string k_DefaultDarkStyleSheetPath =
        "UIPackageResources/StyleSheets/UIElementsDebugger/UIElementsDebuggerDark.uss";
    const string k_DefaultLightStyleSheetPath =
        "UIPackageResources/StyleSheets/UIElementsDebugger/UIElementsDebuggerLight.uss";
    ...
    public void Initialize(EditorWindow debuggerWindow, VisualElement root,
      ...
      if (EditorGUIUtility.isProSkin)
        colorSheet =
            EditorGUIUtility.Load(k_DefaultDarkStyleSheetPath) as StyleSheet;
      else
        colorSheet =
            EditorGUIUtility.Load(k_DefaultLightStyleSheetPath) as StyleSheet;
      ...
      m_ShowLayoutToggle.RegisterValueChangedCallback(
          (e) => { m_Context.showLayoutBound = e.newValue; });

      m_Toolbar.Add(m_ShowLayoutToggle);

      if (Unsupported.IsDeveloperBuild()) {
        m_RepaintOverlayToggle =
            new ToolbarToggle(){name = "repaintOverlayToggle"};
        m_RepaintOverlayToggle.text = "Repaint Overlay";
        m_RepaintOverlayToggle.RegisterValueChangedCallback(
            (e) => m_Context.showRepaintOverlay = e.newValue);
        ...
        m_UXMLLiveReloadToggle.RegisterValueChangedCallback(
            (e) => m_Context.uxmlLiveReload = e.newValue);
        ...
        m_ShowDrawStatsToggle.RegisterValueChangedCallback(
            (e) => { m_Context.showDrawStats = e.newValue; });
      ...
      m_TreeViewContainer =
          new DebuggerTreeView(m_Context.selection, SelectElement);
    ...
    public void OnFocus() {
      ...
      var globalPanelDebugger =
          DebuggerEventDispatchingStrategy
              .s_GlobalPanelDebug as UIElementsDebuggerImpl;
      if (globalPanelDebugger == null ||
          !globalPanelDebugger.m_Context.pickElement)
        DebuggerEventDispatchingStrategy.s_GlobalPanelDebug = this;
    }
    ...
    public override void Refresh() {
      if (!m_Context.pickElement) {
        ...
        if (selectedElement != m_Context.selectedElement) {
          if (m_Context.selectedElement == null &&
              selectedElement.panel == panelDebug.panel)
    ...
    void OnContextChange() {
      ...
      if (Unsupported.IsDeveloperBuild()) {
        m_RepaintOverlayToggle.SetValueWithoutNotify(
            m_Context.showRepaintOverlay);
        ...
        var updater = (panel as BaseVisualElementPanel)
    ...
    public override void OnVersionChanged(VisualElement ve,
      ...
      if ((changeTypeFlag & VersionChangeType.Repaint) ==
      ...
      if ((changeTypeFlag & VersionChangeType.Hierarchy) ==
          VersionChangeType.Hierarchy)
        m_TreeViewContainer.hierarchyHasChanged = true;

      if (panelDebug?.debuggerOverlayPanel != null)
        panelDebug.debuggerOverlayPanel.visualTree.layout =
            panel.visualTree.layout;
    }
    ...
    protected override void OnSelectPanelDebug(IPanelDebug pdbg) {
      ...
      if (m_Context.panelDebug?.debugContainer != null)
        m_Context.panelDebug.debugContainer.generateVisualContent -=
            OnGenerateVisualContent;

      m_Context.selection.panelDebug = pdbg;

      if (panelDebug?.debugContainer != null)
        panelDebug.debugContainer.generateVisualContent +=
            OnGenerateVisualContent;
    ...
    protected override void OnRestorePanelSelection() {
      var restoredElement =
          panel.FindVisualElementByIndex(m_Context.selectedElementIndex);
    ...
    protected override bool ValidateDebuggerConnection(IPanel panelConnection) {
      ...
      foreach (var dbg in debuggers) {
        ...
        if (uielementsDbg != null && uielementsDbg.panel == p &&
            uielementsDbg.m_Root.panel == panelConnection) {
          // Avoid spamming the console if picking
          if (!m_Context.pickElement)
            Debug.LogWarning(
                "Cross UI Toolkit debugger debugging is not supported");
          return false;
        }
      }
    ...
    public bool InterceptMouseEvent(IPanel panel, IMouseEvent ev) {
      ...
      if (evtType != MouseDownEvent.TypeId() &&
          evtType != MouseOverEvent.TypeId() &&
          evtType != MouseEnterWindowEvent.TypeId())
        return false;
    ...
    public void OnPostMouseEvent(IPanel panel, IMouseEvent ev) {
      var isRightClick =
          (ev as MouseUpEvent) ?.button == (int) MouseButton.RightMouse;
    ...
    private void ShowInspectMenu(VisualElement ve) {
      ...
      menu.AddItem(EditorGUIUtility.TrTextContent("Inspect Element"), false,
                   InspectElement, ve);
      menu.ShowAsContext();
    }
