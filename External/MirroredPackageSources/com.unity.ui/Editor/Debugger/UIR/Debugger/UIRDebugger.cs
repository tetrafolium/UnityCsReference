...
namespace UnityEditor.UIElements.Debugger {
  ...
  internal abstract class UIRDebugger : EditorWindow, IPanelDebugger {
    ...
    public void Disconnect() { SelectVisualTree(null); }

    public void OnVersionChanged(VisualElement ele,
                                 VersionChangeType changeTypeFlag) {}

    public bool InterceptEvent(EventBase ev) { return false; }
    ...
    protected void OnGUIPanelSelectDropDown() {
      var label = m_SelectedPanelDropDownIndex >= 0 &&
                  m_SelectedPanelDropDownIndex <
                      m_Labels
                          .Length ? m_Labels[m_SelectedPanelDropDownIndex]
          : GUIContent.Temp("Select a panel");
      if (GUILayout.Button(label, EditorStyles.popup,
                           GUILayout.Width(kDropDownWidth))) {
        RefreshPanelDropdown();
        Rect rect = EditorGUILayout.GetControlRect(false, kSingleLineHeight,
                                                   EditorStyles.popup);
        var controlID =
            GUIUtility.GetControlID(s_PopupHash, FocusType.Keyboard, rect);
        ...
        EditorUtility.DisplayCustomMenu(
            position, m_Labels,
            m_SelectedPanelDropDownIndex, (data, options, selected) => {
      ...
      autoSelect =
          GUILayout.Toggle(autoSelect, GUIContent.Temp("Auto Select"),
                           EditorStyles.toolbarButton, GUILayout.Width(100));
      bool refresh = false;
      refresh =
          GUILayout.Toggle(refresh, GUIContent.Temp("Refresh"),
                           EditorStyles.toolbarButton, GUILayout.Width(100));
    ...
    private void RefreshVisualTrees() {
      ...
      while (it.MoveNext()) {
        HostView view = guiViews.FirstOrDefault(
            v => v.GetInstanceID() == it.Current.Key) as HostView;
        if (view == null)
          continue;
    ...
    private void AutoSelectTree() {
      ...
      if (m_VisualTrees.Count > 0) {
        ...
        if (m_SelectedVisualTree == null ||
            !m_VisualTrees.Contains(m_SelectedVisualTree)) {
    ...
    public virtual bool InterceptEvents(Event ev) { return false; }
  }
}
