...
namespace UnityEditor.UIElements.Debugger {
  internal class AllocatorDebugger : UIRDebugger {
    ...
    public new void OnEnable() {
      ...
      root.AddStyleSheetPath(
          "UIPackageResources/StyleSheets/UIElementsDebugger/UIRAllocatorDebugger.uss");
    ...
    public override void Refresh() {
      var renderDevice =
          UIRDebugUtility.GetUIRenderDevice(m_SelectedVisualTree.panel);
      Debug.Assert(renderDevice != null,
                   "Allocator debugger fail to retrieve UIRenderDevice");
  ...
  internal class AllocatorStatsDisplay : VisualElement {
    ...
    private void OnGUI() {
      ...
      using(new EditorGUI.DisabledScope(Event.current.type !=
                                        EventType.Repaint)) {
        EditorGUILayout.BeginHorizontal();
        EditorGUILayout.IntField("Fully init", m_Stats.completeInit ? 1 : 0);
        EditorGUILayout.IntField(
            "Pages Count", m_Stats.pages != null ? m_Stats.pages.Length : 0);
    ...
    class HeapStats : VisualElement {
      ...
      private void OnGUIStats() {
        ...
        using(new EditorGUI.DisabledScope(Event.current.type !=
          ...
          EditorGUILayout.IntField("Largest Block",
                                   (int) stats.largestAvailableBlock);
          EditorGUILayout.IntField("Available Block",
                                   (int) stats.availableBlocksCount);
