...
internal enum RepaintMode { Standard, UIR }
...
internal static string DebugName(this VisualElement ve) {
	string t = ve.GetType() == typeof(VisualElement)
	             ? String.Empty
	             : (ve.GetType().Name + " ");
	string n =
		String.IsNullOrEmpty(ve.name) ? String.Empty : ("#" + ve.name + " ");
	string res = t + n +
	             (ve.GetClasses().Any()
	                ? ("." + string.Join(",.", ve.GetClasses().ToArray()))
	                : String.Empty);
	if (res == String.Empty)
