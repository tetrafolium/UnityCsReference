...
namespace UnityEditor.UIElements.Debugger {
  ...
  class EventTypeSelectField : BasePopupField<ulong, StringValuePairs<long>> {
    public new class UxmlFactory
        : UxmlFactory<EventTypeSelectField, UxmlTraits> {}
          ...
          foreach (var type in assembly.GetTypes()) {
            if (typeof(EventBase).IsAssignableFrom(type) &&
              ...
              var methodInfo = type.GetMethod(
                  "TypeId", BindingFlags.Public | BindingFlags.Static |
              ...
              if (methodInfo != null) {
                ...
                m_Choices.Add(
                    new StringValuePairs<long>{s = type.Name, v = typeId});
    ...
    internal override string GetValueToDisplay() { return "Event types"; }
    ...
    enum EventTypeSelection { Mouse, Drag, Keyboard, Command, All }

    internal override void AddMenuItems(GenericMenu menu) {
      menu.AddItem(new GUIContent("Select all"), false,
                   () => SelectAllTypes(true));
      menu.AddItem(new GUIContent("Select all mouse events (basic)"), false,
                   () => SelectAllTypes(true, EventTypeSelection.Mouse));
      menu.AddItem(new GUIContent("Select all drag events"), false,
                   () => SelectAllTypes(true, EventTypeSelection.Drag));
      menu.AddItem(new GUIContent("Select all keyboard events"), false,
                   () => SelectAllTypes(true, EventTypeSelection.Keyboard));
      menu.AddItem(new GUIContent("Select all command events"), false,
                   () => SelectAllTypes(true, EventTypeSelection.Command));
      menu.AddSeparator(String.Empty);
      menu.AddItem(new GUIContent("Unselect all"), false,
                   () => SelectAllTypes(false));
      menu.AddSeparator(String.Empty);
      foreach (var item in m_Choices) {
        menu.AddItem(new GUIContent(item.s), m_State[item.v],
    ...
    void SelectAllTypes(bool state, EventTypeSelection eventTypeSelection =
                                        EventTypeSelection.All) {
