...
namespace UnityEditor.UIElements.Debugger {
  class UIElementsEventsDebugger : EditorWindow {
    [SerializeField] UIElementsEventsDebuggerImpl m_DebuggerImpl;
    ...
    public static void ShowUIElementsEventDebugger() {
      ...
      window.titleContent =
          EditorGUIUtility.TrTextContent("UI Toolkit Event Debugger");
    ...
    void OnDisable() { m_DebuggerImpl.OnDisable(); }
  }
  ...
  class UIElementsEventsDebuggerImpl : PanelDebugger {
    ...
    Dictionary<ulong, long> m_EventTimestampDictionary =
    ...
    void DisplayHistogram(ScrollView scrollView) {
      ...
      if (panel == null) {
        ...
        Dictionary<string, long> histogramValue;
            ...
            m_HistogramTitle.text =
                "Histogram - Element Count : " + histogramValue.Count;
            ...
            foreach (var key in histogramValue.Keys)
              AddHistogramEntry(scrollView, key, histogramValue[key],
                                histogramValue[key] / (float) maxDuration);
      }
    }

    static void AddHistogramEntry(VisualElement root, string name,
                                  long duration, float percent) {
      ...
      var labelNameDuration = new Label(name){name = k_EventsDurationLabelName,
                                              text = durationLength + "ms"};
      var durationGraphLength =
          new VisualElement(){name = k_EventsDurationLengthName};
      ...
      if (panel == null) {
        ...
        foreach (var dbgObject in calls) {
          ...
          entry.style.backgroundColor =
              m_EventTypeFilter.m_Color.ContainsKey(
                  dbgObject.eventBase.eventBaseName)
          ...
          entry.style.top =
              (topPosition > (maxHeight - 5)) ? (maxHeight - 5) : topPosition;
          entry.style.height =
              (maxHeight - topPosition) < 5 ? 5 : (maxHeight - topPosition);
          entry.style.width = 5;
          entry.style.left = lastPixel;
          entry.tooltip = dbgObject.eventBase.eventBaseName + " (" +
                          dbgObject.duration / 1000 + " ms)";
          scrollView.Add(entry);
          lastPixel += 7;
        }

        scrollView.contentContainer.style.width =
            (lastPixel < 100) ? 100 : lastPixel;
    ...
    void DisplayEvents(ScrollView scrollView) {
      ...
      foreach (var eventRegistrationListener in GlobalCallbackRegistry
                   .s_Listeners) {
        VisualElement key = eventRegistrationListener
                                .Key as VisualElement; // VE that sends events
        if (key?.panel == null)
          continue;
        ...
        foreach (var evt in events) {
          ...
          foreach (var evtCallback in evtCallbacks) {
            {
              CodeLine line =
                  new CodeLine(evtCallback.name, evtCallback.fileName,
                               evtCallback.lineNumber, evtCallback.hashCode);
              line.AddToClassList("callback-list-element");
    ...
    public void Initialize(EditorWindow debuggerWindow, VisualElement root) {
      ...
      VisualTreeAsset template = EditorGUIUtility.Load(
          "UIPackageResources/UXML/UIElementsDebugger/UIElementsEventsDebugger.uxml")
                                     as VisualTreeAsset;
      ...
      rootVisualElement.AddStyleSheetPath(
          "UIPackageResources/StyleSheets/UIElementsDebugger/UIElementsEventsDebugger.uss");

      var eventsDebugger = rootVisualElement.MandatoryQ("eventsDebugger");
      eventsDebugger.StretchToParentSize();

      m_EventCallbacksScrollView =
          (ScrollView) rootVisualElement.MandatoryQ("eventCallbacksScrollView");

      m_EventTypeFilter =
          toolbar.MandatoryQ<EventTypeSelectField>("filter-event-type");
      ...
      m_ReplaySelectedEventsButton =
          toolbar.MandatoryQ<Button>("replay-selected-events");
      ...
      m_SelectionCountLabel =
          infoContainer.MandatoryQ<Label>("selection-count");
      var autoScrollToggle = infoContainer.MandatoryQ<Toggle>("autoscroll");
      autoScrollToggle.value = m_AutoScroll;
      autoScrollToggle.RegisterValueChangedCallback(
          (e) => { m_AutoScroll = e.newValue; });

      m_EventPropagationPaths =
          (Label) rootVisualElement.MandatoryQ("eventPropagationPaths");
      ...
      m_HistogramTitle =
          (Label) rootVisualElement.MandatoryQ("eventsHistogramTitle");
      ...
      var eventCallbacksScrollView =
          (ScrollView) rootVisualElement.MandatoryQ("eventCallbacksScrollView");
      eventCallbacksScrollView.StretchToParentSize();

      var eventPropagationPathsScrollView =
          (ScrollView)
              rootVisualElement.MandatoryQ("eventPropagationPathsScrollView");
      eventPropagationPathsScrollView.StretchToParentSize();

      var eventbaseInfoScrollView =
          (ScrollView) rootVisualElement.MandatoryQ("eventbaseInfoScrollView");
      eventbaseInfoScrollView.StretchToParentSize();

      m_EventRegistrationsScrollView =
          (ScrollView)
              rootVisualElement.MandatoryQ("eventsRegistrationsScrollView");
      DisplayEvents(m_EventRegistrationsScrollView);
      m_EventRegistrationsScrollView.StretchToParentSize();

      m_EventsHistogramScrollView = (ScrollView) rootVisualElement.MandatoryQ(
          "eventsHistogramScrollView");
      DisplayHistogram(m_EventsHistogramScrollView);
      m_EventsHistogramScrollView.StretchToParentSize();

      m_EventTimelineScrollView =
          (ScrollView) rootVisualElement.MandatoryQ("eventTimelineScrollView");
      ...
      m_TimelineLegend =
          (Label) rootVisualElement.MandatoryQ("eventTimelineTitleLegend");
    ...
    public override bool InterceptEvent(EventBase evt) {
      ...
      m_EventTimestampDictionary[evt.eventId] =
          (long)(Time.realtimeSinceStartup * 1000.0f);
      IEventHandler capture =
          panel?.GetCapturingElement(PointerId.mousePointerId);
    ...
    void CreateLegendContainer() {
      m_LegendContainer =
          new VisualElement(){name = "eventTimelineLegendContainer"};
      ...
      foreach (var eventName in listOfEventForColor) {
        ...
        var colorCode =
            new VisualElement(){name = "eventTimelineLegendEntryColor"};
        colorCode.style.backgroundColor = m_EventTypeFilter.m_Color[eventName];

        var newEntry =
            new Label(eventName){name = "eventTimelineLegendEntryName"};
    ...
    void ShowLegend(MouseEnterEvent evt) {
      ...
      m_LegendContainer.style.top =
          evt.mousePosition.y - m_LegendContainer.layout.height - 5;
    ...
    void ReplaySelectedEvents() {
      if (m_SelectedEvents == null)
        return;
      m_Debugger.ReplayEvents(
          m_SelectedEvents.Select(x => x.eventBase).ToList());
    }

    void HighlightCodeline(int hashcode) {
      foreach (var codeLine in m_EventRegistrationsScrollView.Children()
                   .OfType<CodeLine>()) {
    ...
    void UpdateEventsLog() {
      ...
      bool allActive =
          activeEventTypes.Count == m_EventTypeFilter.m_State.Count;
      ...
      if (!allInactive) {
        for (var lineIndex = 0; lineIndex < calls.Count; lineIndex++) {
          ...
          if (allActive || activeEventTypes.Contains(eventBase.eventTypeId)) {
            var eventDateTimeStr =
                eventBase.TimestampString() + " #" + eventBase.eventId;
            ...
            string targetName =
                (eventBase.target !=
                 ...
                 : "<null>");
            var line =
                new EventLogLine(lineIndex + 1, "[" + eventDateTimeStr + "]",
                                 handler, targetName, eventBase);
            m_Log.AddLine(line);
          }
        }
      }
    ...
    void OnEventsLogSelectionChanged(object obj) {
      ...
      if (m_SelectedEvents.Any()) {
        ...
        focused =
            line != null ? calls ?[line.lineNumber - 1].focusedElement : null;
      ...
      // Not working :         DisplayTimeline(m_EventTimelineScrollView,
      // m_SelectedVisualTree.panel);
    }

    void ClearEventbaseInfo() { m_EventbaseInfo.text = ""; }

    void UpdateEventbaseInfo(EventDebuggerEventRecord eventBase,
                             IEventHandler focused, IEventHandler capture) {
      ClearEventbaseInfo();
      ...
      m_EventbaseInfo.text +=
          "Focused element: " + EventDebugger.GetObjectDisplayName(focused) +
          "\n";
      m_EventbaseInfo.text +=
          "Capture element: " + EventDebugger.GetObjectDisplayName(capture) +
          "\n";
      ...
      if (eventBase.eventTypeId == MouseMoveEvent.TypeId() ||
        ...
        m_EventbaseInfo.text +=
            "Mouse position: " + eventBase.mousePosition + "\n";
      ...
      if (eventBase.eventTypeId == MouseDownEvent.TypeId() ||
        ...
        m_EventbaseInfo.text +=
            ...
            (eventBase.button == 0
                 ? "Left"
                 : eventBase.button == 1 ? "Middle" : "Right") +
            "\n";
      ...
      if (eventBase.eventTypeId == MouseMoveEvent.TypeId() ||
          ...
          eventBase.eventTypeId == PointerLeaveEvent.TypeId()) {
        m_EventbaseInfo.text +=
            "Pressed buttons: " + eventBase.pressedButtons + "\n";
      }
      ...
      if (eventBase.eventTypeId == KeyDownEvent.TypeId() ||
        ...
        if (char.IsControl(eventBase.character)) {
          m_EventbaseInfo.text +=
              "Character: \\" + (byte)(eventBase.character) + "\n";
        } else {
    ...
    void UpdateEventCallbacks(EventDebuggerEventRecord eventBase) {
      ...
      if (callbacks != null) {
        foreach (EventDebuggerCallTrace callback in callbacks) {
          ...
          Label phaseDurationContainer =
              new Label{name = "phaseDurationContainer"};
      ...
      foreach (EventDebuggerDefaultActionTrace defaultAction in
                   defaultActions) {
        VisualElement container = new VisualElement{name = "line-container"};
    ...
    void ClearEventPropagationPaths() { m_EventPropagationPaths.text = ""; }
    ...
    IList ToList() { return m_Log.lines.ToList(); }
    ...
    public override void OnVersionChanged(VisualElement ve,
    ...
    protected override void OnSelectPanelDebug(IPanelDebug pdbg) {
      ...
      var erTitle =
          rootVisualElement.MandatoryQ<Label>("eventsRegistrationTitle");
      const string prefix = "Registered Event Callbacks";
      erTitle.text = panel != null ? prefix + " in " +
    ...
    public override void Refresh() {
      var eventDebuggerModificationCount =
          m_Debugger.GetModificationCount(panel);
    ...
    void UpdateLogCount() {
      m_LogCountLabel.text =
          m_Log.lines.Count + " event" + (m_Log.lines.Count > 1 ? "s" : "");
    }

    void UpdateSelectionCount() {
      m_SelectionCountLabel.text =
          "(" +
          (m_SelectedEvents != null ? m_SelectedEvents.Count.ToString()
           : "0") +
          " selected)";
    }

    void UpdateReplaySelectedEventsButton() {
      m_ReplaySelectedEventsButton.SetEnabled(m_SelectedEvents != null &&
                                              m_SelectedEvents.Any());
    }
  }
}
