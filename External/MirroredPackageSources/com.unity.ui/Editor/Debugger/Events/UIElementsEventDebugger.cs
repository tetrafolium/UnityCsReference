...
static void AddHistogramEntry(VisualElement root, string name,
	...
	var container = new VisualElement(){
		name = k_EventsContainerName
	};
	var labelName = new Label(name){
		name = k_EventsLabelName
	};
	var durationGraph = new VisualElement(){
		name = k_EventsDurationName
	};
	float durationLength = duration / 1000.0f;
	var labelNameDuration = new Label(name){
		name = k_EventsDurationLabelName,
		text = durationLength + "ms"
	};
	var durationGraphLength =
		new VisualElement(){
		name = k_EventsDurationLengthName
	};
...
void CreateLegendContainer() {
	m_LegendContainer =
		new VisualElement(){
		name = "eventTimelineLegendContainer"
	};
	rootVisualElement.Add(m_LegendContainer);
	m_LegendContainer.visible = false;

	var title = new Label("Legend"){
		name = "eventTimelineLegendTitle"
	};
	...
	foreach (var eventName in listOfEventForColor) {
		var container = new VisualElement(){
			name = "eventTimelineLegendEntry"
		};

		var colorCode =
			new VisualElement(){
			name = "eventTimelineLegendEntryColor"
		};
		colorCode.style.backgroundColor = m_EventTypeFilter.m_Color[eventName];

		var newEntry =
			new Label(eventName){
			name = "eventTimelineLegendEntryName"
		};
