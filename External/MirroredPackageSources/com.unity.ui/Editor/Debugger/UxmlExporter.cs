...
namespace UnityEditor.UIElements.Debugger {
  class UxmlExporter {
    ...
    public static string Dump(VisualElement selectedElement, string templateId,
                              ExportOptions options) {
      Dictionary<XNamespace, string> nsToPrefix =
          new Dictionary<XNamespace, string>(){{UIElementsNamespace, "ui"}};
      ...
      foreach (var it in usings.OrderByDescending(x => x)) {
        template.AddFirst(new XElement("Using", new XAttribute("alias", it),
                                       new XAttribute("path", it)));
      }
      ...
      XmlWriterSettings settings = new XmlWriterSettings{
          ...
          NewLineOnAttributes = (options & ExportOptions.NewLineOnAttributes) ==
                                ExportOptions.NewLineOnAttributes,
    ...
    private static void Recurse(XElement parent,
                                Dictionary<XNamespace, string> nsToPrefix,
                                HashSet<string> usings, VisualElement ve,
                                ExportOptions options) {
      ...
      else if ((options & ExportOptions.AutoNameElements) ==
               ExportOptions.AutoNameElements) {
