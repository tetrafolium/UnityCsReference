...
.APIUpdating     // Funky name because bindings gen is tripping noodles with
...
GetMovedFromAttributeDataForType(Type sourceType, out string assembly,
                                 out string nsp, out string klass) {
	klass = null;
	nsp = null;
	assembly = null;

	var attrs =
		sourceType.GetCustomAttributes(typeof(MovedFromAttribute), false);
...
// Notes:
// -Expected format of obsolete string: comments (UnityUpgradable) ->
// [assembly] namespace.typeName/typeName
// - where:
//     - [Assembly] can occur [0,1] times
//     - namespace can occur [0,n] times
//     - typeName can occur [1,n] times
// - Nested types (class inside a class) with have list of types separated
// by '/' stored in the className argument.
[RequiredByNativeCode]
internal static bool
GetObsoleteTypeRedirection(Type sourceType, out string assemblyName,
                           out string nsp, out string className) {
	var attrs =
		sourceType.GetCustomAttributes(typeof(ObsoleteAttribute), false);
	assemblyName = null;
	nsp = null;
	className = null;
