...
public enum SpritePackingMode { Tight = 0, Rectangle }
...
public enum SpriteSortPoint { Center = 0, Pivot = 1 }
...
public sealed partial class Sprite : Object {
  ...
  internal extern static Sprite
  CreateSpriteWithoutTextureScripting(Rect rect, Vector2 pivot,
                                      float pixelsToUnits, Texture2D texture);

  [FreeFunction("SpritesBindings::CreateSprite")]
  internal extern static Sprite
  CreateSprite(Texture2D texture, Rect rect, Vector2 pivot, float pixelsPerUnit,
  ...
  public extern Vector4 border { get; }
  ...
  // merge conflicts.
  public extern float pixelsPerUnit { [NativeMethod("GetPixelsToUnits")] get; }

  public extern float spriteAtlasTextureScale {
    [NativeMethod("GetSpriteAtlasTextureScale")] get;
  }

  public extern Texture2D associatedAlphaSplitTexture {
    [NativeMethod("GetAlphaTexture")] get;
  }

  public extern Vector2 pivot { [NativeMethod("GetPivotInPixels")] get; }

  public bool packed {
    get { return GetPacked() == 1; }
  }

  public SpritePackingMode packingMode {
    get { return (SpritePackingMode) GetPackingMode(); }
  }

  public SpritePackingRotation packingRotation {
    get { return (SpritePackingRotation) GetPackingRotation(); }
  }

  public Rect textureRect {
    get {
      if (packed && (SpritePackingMode)
                            packingMode != SpritePackingMode.Rectangle)
        return Rect.zero;
  ...
  public extern Vector2[] vertices {
    [FreeFunction("SpriteAccessLegacy::GetSpriteVertices",
                  HasExplicitThis = true)] get;
  }

  public extern UInt16[] triangles {
    [FreeFunction("SpriteAccessLegacy::GetSpriteIndices",
                  HasExplicitThis = true)] get;
  }

  public extern Vector2[] uv {
    [FreeFunction("SpriteAccessLegacy::GetSpriteUVs",
                  HasExplicitThis = true)] get;
  }
  ...
  public int GetPhysicsShapePointCount(int shapeIdx) {
    ...
    if (shapeIdx < 0 || shapeIdx >= physicsShapeCount)
      throw new IndexOutOfRangeException(
          String.Format("Index({0}) is out of bounds(0 - {1})", shapeIdx,
                        physicsShapeCount - 1));
  ...
  private extern static void GetPhysicsShapeImpl(Sprite sprite, int shapeIdx,
    ...
    for (int i = 0; i < physicsShapes.Count; ++i) {
      ...
      if (physicsShape == null) {
        throw new ArgumentNullException(
            String.Format("Physics Shape at {0} is null.", i));
      }
      if (physicsShape.Length < 3) {
        throw new ArgumentException(String.Format(
            "Physics Shape at {0} has less than 3 vertices ({1}).", i,
            physicsShape.Length));
      }
    }
  ...
  private extern static void OverridePhysicsShapeCount(Sprite sprite,
                                                       int physicsShapeCount);

  [FreeFunction("SpritesBindings::OverridePhysicsShape",
                ThrowsException = true)]
  private extern static void
  OverridePhysicsShape(Sprite sprite, Vector2[] physicsShape, int idx);
  ...
  internal static Sprite Create(Rect rect, Vector2 pivot, float pixelsToUnits,
                                Texture2D texture) {
    return CreateSpriteWithoutTextureScripting(rect, pivot, pixelsToUnits,
                                               texture);
  }
  ...
  public static Sprite Create(Texture2D texture, Rect rect, Vector2 pivot,
                              float pixelsPerUnit, uint extrude,
    ...
    if (rect.xMax > texture.width || rect.yMax > texture.height)
      throw new ArgumentException(String.Format(
          "Could not create sprite ({0}, {1}, {2}, {3}) from a {4}x{5} texture.",
          rect.x, rect.y, rect.width, rect.height, texture.width,
          texture.height));

    // fix case 810590
    if (pixelsPerUnit <= 0)
      throw new ArgumentException(
          "pixelsPerUnit must be set to a positive non-zero value.");

    return CreateSprite(texture, rect, pivot, pixelsPerUnit, extrude, meshType,
                        border, generateFallbackPhysicsShape);
  }
