...
static public void Combine(GameObject[] gos, GameObject staticBatchRoot) {
	InternalStaticBatchingUtility.CombineGameObjects(gos, staticBatchRoot,
	                                                 false, null);
}
}
...
const int MaxVerticesInBatch =
	64000; // a little bit less than 64K - just in case
const string CombinedMeshPrefix = "Combined Mesh";

public static void CombineRoot(UnityEngine.GameObject staticBatchRoot,
                               StaticBatcherGOSorter sorter) {
...
static public void Combine(UnityEngine.GameObject staticBatchRoot,
                           ...
                           StaticBatcherGOSorter sorter) {
	GameObject[] gos =
		(GameObject[]) UnityEngine.Object.FindObjectsOfType(typeof(GameObject));
...
public static GameObject[] SortGameObjectsForStaticbatching(
	GameObject[] gos, StaticBatcherGOSorter sorter) {
	gos = gos.OrderBy(x => {
				Renderer aRenderer =
					StaticBatcherGOSorter.GetRenderer(x as GameObject);
				return sorter.GetMaterialId(aRenderer);
			})
	      .ThenBy(y => {
				Renderer aRenderer =
					StaticBatcherGOSorter.GetRenderer(y as GameObject);
				return sorter.GetLightmapIndex(aRenderer);
			})
	      .ThenBy(z => {
				Renderer aRenderer =
					StaticBatcherGOSorter.GetRenderer(z as GameObject);
...
static public void CombineGameObjects(GameObject[] gos,
                                      UnityEngine.GameObject staticBatchRoot,
                                      bool isEditorPostprocessScene,
                                      StaticBatcherGOSorter sorter) {
	...
	List<MeshSubsetCombineUtility.MeshContainer> meshes =
		new List<MeshSubsetCombineUtility.MeshContainer>();

	gos = SortGameObjectsForStaticbatching(
		gos, sorter ?? new StaticBatcherGOSorter());
	...
	foreach (GameObject go in gos) {
		...
		// reject if has no mesh or (mesh not readable and not called from Editor
		// PostprocessScene.cs) Editor is allowed to modify meshes even if they
		// are marked as read-only e.g. Applicatiopn.LoadLevel() called from a
		// script inside the editor player
		if (instanceMesh == null ||
		    (!isEditorPostprocessScene && !instanceMesh.canAccess))
			continue;
		...
		if (materials.Any(m => m != null && m.shader != null &&
		                  m.shader.disableBatching !=
		                  DisableBatchingType.False))
			continue;

		int vertexCount = instanceMesh.vertexCount;
		// Use same tests as MeshCombiner::IsMeshBatchable to stay consistent with
		// C++ code
		if (vertexCount == 0)
			continue;
		...
		MeshSubsetCombineUtility.MeshInstance instance =
			new MeshSubsetCombineUtility.MeshInstance();
		instance.meshInstanceID = instanceMesh.GetInstanceID();
		instance.rendererInstanceID = renderer.GetInstanceID();
		if (meshRenderer != null) {
			if (meshRenderer.additionalVertexStreams != null)
				instance.additionalVertexStreamsMeshInstanceID =
					meshRenderer.additionalVertexStreams.GetInstanceID();
			if (meshRenderer.enlightenVertexStream != null)
				instance.enlightenVertexStreamMeshInstanceID =
					meshRenderer.enlightenVertexStream.GetInstanceID();
		}
		instance.transform =
			staticBatchInverseMatrix * filter.transform.localToWorldMatrix;
		...
		MeshSubsetCombineUtility.MeshContainer mesh =
			new MeshSubsetCombineUtility.MeshContainer();
		mesh.gameObject = go;
		mesh.instance = instance;
		mesh.subMeshInstances =
			new List<MeshSubsetCombineUtility.SubMeshInstance>();

		//;;Debug.Log("New static mesh (" + go.name + ")verts: " +
		//instanceMesh.vertexCount +
		...
		if (materials.Length > instanceMesh.subMeshCount) {
			Debug.LogWarning("Mesh '" + instanceMesh.name +
			                 "' has more materials (" + materials.Length +
			                 ") than subsets (" + instanceMesh.subMeshCount +
			                 ")",
			                 renderer);
			// extra materials don't have a meaning and it screws the rendering as
			// Unity tries to render with those extra materials.
		...
		for (int m = 0;
		     m < System.Math.Min(materials.Length, instanceMesh.subMeshCount);
		     ++m) {
			//;;Debug.Log("   new subset : " + m + ", tris " +
			//instanceMesh.GetTriangles(m).Length);
			MeshSubsetCombineUtility.SubMeshInstance subMeshInstance =
				new MeshSubsetCombineUtility.SubMeshInstance();
...
static private void
MakeBatch(List<MeshSubsetCombineUtility.MeshContainer> meshes,
          Transform staticBatchRootTransform, int batchIndex) {
	if (meshes.Count < 2)
		return;

	List<MeshSubsetCombineUtility.MeshInstance> meshInstances =
		new List<MeshSubsetCombineUtility.MeshInstance>();
	List<MeshSubsetCombineUtility.SubMeshInstance> allSubMeshInstances =
		new List<MeshSubsetCombineUtility.SubMeshInstance>();
	...
	combinedMeshName +=
		" (root: " +
		((staticBatchRootTransform != null) ? staticBatchRootTransform.name
		                            : "scene") +
		")";
	if (batchIndex > 0)
		combinedMeshName += " " + (batchIndex + 1);

	Mesh combinedMesh = StaticBatchingHelper.InternalCombineVertices(
		meshInstances.ToArray(), combinedMeshName);
	StaticBatchingHelper.InternalCombineIndices(allSubMeshInstances.ToArray(),
	                                            combinedMesh);
	int totalSubMeshCount = 0;

	foreach (MeshSubsetCombineUtility.MeshContainer mesh in meshes) {
		// Changing the mesh resets the static batch info, so we have to assign
		// sharedMesh first
		MeshFilter filter =
			(MeshFilter) mesh.gameObject.GetComponent(typeof(MeshFilter));
		...
		// Remove additionalVertexStreams and enlightenVertexStream, all their
		// data has been copied into the combined mesh.
		MeshRenderer meshRenderer = renderer as MeshRenderer;
		if (meshRenderer != null) {
