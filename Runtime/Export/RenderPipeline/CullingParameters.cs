...
namespace UnityEngine.Rendering {
  [UsedByNativeCode]
  [StructLayout(LayoutKind.Sequential)] struct CoreCameraValues
      : IEquatable<CoreCameraValues> {
    int filterMode;
    uint cullingMask;
    int instanceID;
    ...
    public bool Equals(CoreCameraValues other) {
      return filterMode == other.filterMode &&
             ...
             renderImmediateObjects == other.renderImmediateObjects;
    }
    ...
    public static bool operator ==(CoreCameraValues left,
                                   CoreCameraValues right) {
  ...
  [UsedByNativeCode]
  [StructLayout(LayoutKind.Sequential)] unsafe public struct CameraProperties
      : IEquatable<CameraProperties> {
    ...
    // can't make fixed types private, because then the compiler generates
    // different code which BindingsGenerator does not handle yet. We can expose
    // these when we have ref returns
    ...
    // can't make fixed types private, because then the compiler generates
    // different code which BindingsGenerator does not handle yet.
    ...
    public Plane GetShadowCullingPlane(int index) {
      if (index < 0 || index >= k_PlaneCount)
        throw new ArgumentOutOfRangeException(
            $"{nameof(index)} was {index}, but must be at least 0 and less than {k_PlaneCount}");
      unsafe {
    ...
    public bool Equals(CameraProperties other) {
      ...
      return screenRect.Equals(other.screenRect) &&
             viewDir.Equals(other.viewDir) &&
             projectionNear.Equals(other.projectionNear) &&
             projectionFar.Equals(other.projectionFar) &&
             cameraNear.Equals(other.cameraNear) &&
             cameraFar.Equals(other.cameraFar) &&
             cameraAspect.Equals(other.cameraAspect) &&
             cameraToWorld.Equals(other.cameraToWorld) &&
             actualWorldToClip.Equals(other.actualWorldToClip) &&
             cameraClipToWorld.Equals(other.cameraClipToWorld) &&
             cameraWorldToClip.Equals(other.cameraWorldToClip) &&
             implicitProjection.Equals(other.implicitProjection) &&
             stereoWorldToClipLeft.Equals(other.stereoWorldToClipLeft) &&
             stereoWorldToClipRight.Equals(other.stereoWorldToClipRight) &&
             worldToCamera.Equals(other.worldToCamera) && up.Equals(other.up) &&
             right.Equals(other.right) &&
             transformDirection.Equals(other.transformDirection) &&
             cameraEuler.Equals(other.cameraEuler) &&
             velocity.Equals(other.velocity) &&
             farPlaneWorldSpaceLength.Equals(other.farPlaneWorldSpaceLength) &&
             rendererCount == other.rendererCount &&
             baseFarDistance.Equals(other.baseFarDistance) &&
             shadowCullCenter.Equals(other.shadowCullCenter) &&
             layerCullSpherical == other.layerCullSpherical &&
             coreCameraValues.Equals(other.coreCameraValues) &&
             cameraType == other.cameraType &&
             projectionIsOblique == other.projectionIsOblique &&
             isImplicitProjectionMatrix == other.isImplicitProjectionMatrix;
    }
    ...
    public override int GetHashCode() {
      unchecked {
        ...
        fixed(float *distancesPtr = layerCullDistances) {
          for (var i = 0; i < k_NumLayers; i++) {
            hashCode = (hashCode * 397) ^ distancesPtr [i]
                                              .GetHashCode();
          }
        }
    ...
    public static bool operator ==(CameraProperties left,
                                   CameraProperties right) {
      return left.Equals(right);
    }
  ...
  public unsafe struct ScriptableCullingParameters
    ...
    // can't make fixed types private, because then the compiler generates
    // different code which BindinsgGenerator does not handle yet.
    const int k_MaximumCullingPlaneCount = 10;
    public static readonly int maximumCullingPlaneCount =
        k_MaximumCullingPlaneCount;
    internal fixed byte
        m_CullingPlanes[k_MaximumCullingPlaneCount * Plane.size];
    ...
    public int cullingPlaneCount {
      ...
      set {
        if (value < 0 || value > k_MaximumCullingPlaneCount)
          throw new ArgumentOutOfRangeException(
              $"{nameof(value)} was {value}, but must be at least 0 and less than {k_MaximumCullingPlaneCount}");
    ...
    public int maximumPortalCullingJobs {
      ...
      set {
        if (value < k_CullingJobCountLowerLimit ||
            value > k_CullingJobCountUpperLimit)
          throw new ArgumentOutOfRangeException(
              $"{nameof(maximumPortalCullingJobs)} was {maximumPortalCullingJobs}, but must be in range {k_CullingJobCountLowerLimit} to {k_CullingJobCountUpperLimit}");
    ...
    public float GetLayerCullingDistance(int layerIndex) {
      if (layerIndex < 0 || layerIndex >= k_LayerCount)
        throw new ArgumentOutOfRangeException(
            $"{nameof(layerIndex)} was {layerIndex}, but must be at least 0 and less than {k_LayerCount}");
      fixed(float *ptr = m_LayerFarCullDistances) { return ptr[layerIndex]; }
    }
    ...
    public void SetLayerCullingDistance(int layerIndex, float distance) {
      if (layerIndex < 0 || layerIndex >= k_LayerCount)
        throw new ArgumentOutOfRangeException(
            $"{nameof(layerIndex)} was {layerIndex}, but must be at least 0 and less than {k_LayerCount}");
      fixed(float *p = m_LayerFarCullDistances) { p[layerIndex] = distance; }
    }

    public Plane GetCullingPlane(int index) {
      if (index < 0 || index >= cullingPlaneCount)
        throw new ArgumentOutOfRangeException(
            $"{nameof(index)} was {index}, but must be at least 0 and less than {cullingPlaneCount}");
    ...
    public bool Equals(ScriptableCullingParameters other) {
      for (var i = 0; i < k_LayerCount; i++) {
        if (!GetLayerCullingDistance(i).Equals(
                other.GetLayerCullingDistance(i)))
          return false;
      }
      ...
      return m_IsOrthographic == other.m_IsOrthographic &&
             ...
             m_ReflectionProbeSortingCriteria ==
                 other.m_ReflectionProbeSortingCriteria &&
             m_CameraProperties.Equals(other.m_CameraProperties) &&
             m_AccurateOcclusionThreshold.Equals(
                 other.m_AccurateOcclusionThreshold) &&
             m_StereoViewMatrix.Equals(other.m_StereoViewMatrix) &&
             m_StereoProjectionMatrix.Equals(other.m_StereoProjectionMatrix) &&
             m_StereoSeparationDistance.Equals(
                 other.m_StereoSeparationDistance) &&
    ...
    public override bool Equals(object obj) {
      if (ReferenceEquals(null, obj))
        return false;
      return obj is ScriptableCullingParameters &&
             Equals((ScriptableCullingParameters) obj);
    }
    ...
    public override int GetHashCode() {
      unchecked {
        ...
        hashCode =
            (hashCode * 397) ^ m_AccurateOcclusionThreshold.GetHashCode();
    ...
    public static bool operator ==(ScriptableCullingParameters left,
