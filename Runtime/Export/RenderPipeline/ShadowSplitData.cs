...
namespace UnityEngine.Rendering {
  [UsedByNativeCode]
  [StructLayout(LayoutKind.Sequential)] unsafe public struct ShadowSplitData
      ...
      set {
        if (value < 0 || value > k_MaximumCullingPlaneCount)
          throw new ArgumentException(
              $"Value should range from {0} to ShadowSplitData.maximumCullingPlaneCount ({k_MaximumCullingPlaneCount}), but was {value}.");
    ...
    public float shadowCascadeBlendCullingFactor {
      ...
      set {
        if (value < 0f || value > 1f)
          throw new ArgumentException(
              $"Value should range from {0} to {1}, but was {value}.");
    ...
    public Plane GetCullingPlane(int index) {
      if (index < 0 || index >= cullingPlaneCount)
        throw new ArgumentException(
            "index",
            $"Index should be at least {0} and less than cullingPlaneCount ({cullingPlaneCount}), but was {index}.");
    ...
    public static bool operator ==(ShadowSplitData left,
                                   ShadowSplitData right) {
