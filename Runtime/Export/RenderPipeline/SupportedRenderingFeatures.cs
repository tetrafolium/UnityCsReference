...
namespace UnityEngine.Rendering {
  public class SupportedRenderingFeatures {
    ...
    // this is done since the original enum doesn't allow flags due to it
    // starting at 0, not 1
    [Flags]
    ...
    = ReflectionProbeModes.None;

    public LightmapMixedBakeModes defaultMixedLightingModes {
      get;
      set;
    }
    = LightmapMixedBakeModes.None;

    public LightmapMixedBakeModes mixedLightingModes {
      get;
      set;
    }
    = LightmapMixedBakeModes.IndirectOnly | LightmapMixedBakeModes.Subtractive |
      LightmapMixedBakeModes.Shadowmask;

    public LightmapBakeType lightmapBakeTypes {
      get;
      set;
    }
    = LightmapBakeType.Realtime | LightmapBakeType.Mixed |
    ...
    // if this is overridden, please provide a message telling the user where to
    // find it, otherwise it will be an empty string (to prevent confusion)
    ...
    FallbackMixedLightingModeByRef(IntPtr fallbackModePtr) {
      ...
      if ((active.defaultMixedLightingModes != LightmapMixedBakeModes.None) &&
          ((active.mixedLightingModes & active.defaultMixedLightingModes) ==
           active.defaultMixedLightingModes)) {
      ...
      // last restort. make sure Mixed mode is even supported, otherwise the
      // baking pipeline will treat the Mixed lights it as Realtime
      *fallbackMode = MixedLightingMode.IndirectOnly;
    }

    internal static unsafe bool
    IsMixedLightingModeSupported(MixedLightingMode mixedMode) {
      bool isSupported;
    ...
    IsMixedLightingModeSupportedByRef(MixedLightingMode mixedMode,
                                      IntPtr isSupportedPtr) {
      // if Mixed mode hasn't been turned off completely and the Mixed lights
      // will be treated as Realtime
      ...
      // starting at 0, not 1
      *isSupported =
          ((mixedMode == MixedLightingMode.IndirectOnly &&
            ((active.mixedLightingModes &
              LightmapMixedBakeModes.IndirectOnly) ==
             LightmapMixedBakeModes.IndirectOnly)) ||
           (mixedMode == MixedLightingMode.Subtractive &&
            ((active.mixedLightingModes & LightmapMixedBakeModes.Subtractive) ==
             LightmapMixedBakeModes.Subtractive)) ||
           (mixedMode == MixedLightingMode.Shadowmask &&
            ((active.mixedLightingModes & LightmapMixedBakeModes.Shadowmask) ==
             LightmapMixedBakeModes.Shadowmask)));
    }

    internal static unsafe bool
    IsLightmapBakeTypeSupported(LightmapBakeType bakeType) {
      bool isSupported;
    ...
    [RequiredByNativeCode]
    internal static unsafe void
    IsLightmapBakeTypeSupportedByRef(LightmapBakeType bakeType,
                                     IntPtr isSupportedPtr) {
      ...
      if (bakeType == LightmapBakeType.Mixed) {
        ...
        bool isBakedSupported =
            IsLightmapBakeTypeSupported(LightmapBakeType.Baked);

        // we can't support Mixed mode and then not support any of the different
        // modes
        if (!isBakedSupported ||
            active.mixedLightingModes == LightmapMixedBakeModes.None) {
          *isSupported = false;
          return;
        }
      }
      ...
      *isSupported = ((active.lightmapBakeTypes & bakeType) == bakeType);
      ...
      // if we are using realtime GI and Enlighten has been deprecated, don't
      // allow realtime GI
    ...
    [RequiredByNativeCode]
    internal static unsafe void
    IsLightmapsModeSupportedByRef(LightmapsMode mode, IntPtr isSupportedPtr) {
      var isSupported = (bool *) isSupportedPtr;
    ...
    [RequiredByNativeCode]
    internal static unsafe void
    IsLightmapperSupportedByRef(int lightmapper, IntPtr isSupportedPtr) {
      var isSupported = (bool *) isSupportedPtr;

      // 0 = Enlighten
      // if the lightmapper is Enlighten but Enlighten is deprecated, it's not
      // supported
    ...
    [RequiredByNativeCode]
    internal static unsafe void
    IsUIOverlayRenderedBySRP(IntPtr isSupportedPtr) {
      var isSupported = (bool *) isSupportedPtr;
    ...
    [RequiredByNativeCode]
    internal static unsafe void
    FallbackLightmapperByRef(IntPtr lightmapperPtr) {
