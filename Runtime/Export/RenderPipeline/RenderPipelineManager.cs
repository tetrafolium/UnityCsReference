...
public static event Action<ScriptableRenderContext, Camera[]>
beginFrameRendering;
public static event Action<ScriptableRenderContext, Camera>
...
internal static void BeginFrameRendering(ScriptableRenderContext context,
                                         Camera[] cameras) {
	beginFrameRendering?.Invoke(context, cameras);
}

internal static void BeginCameraRendering(ScriptableRenderContext context,
                                          Camera camera) {
	beginCameraRendering?.Invoke(context, camera);
}

internal static void EndFrameRendering(ScriptableRenderContext context,
                                       Camera[] cameras) {
	endFrameRendering?.Invoke(context, cameras);
}

internal static void EndCameraRendering(ScriptableRenderContext context,
                                        Camera camera) {
...
[RequiredByNativeCode]
static void DoRenderLoop_Internal(RenderPipelineAsset pipe, IntPtr loopPtr,
                                  AtomicSafetyHandle safety) {
	PrepareRenderPipeline(pipe);
	if (currentPipeline == null)
		return;

	var loop = new ScriptableRenderContext(loopPtr, safety);
...
static void PrepareRenderPipeline(RenderPipelineAsset pipelineAsset) {
	if (!ReferenceEquals(s_CurrentPipelineAsset, pipelineAsset)) {
		// Required because when switching to a RenderPipeline asset for the
		// first time it will call OnValidate on the new asset before cleaning
		// up the old one. Thus we reset the rebuild in order to cleanup
		// properly.
