...
namespace UnityEngine.Rendering {
  ...
  public struct RenderQueueRange : IEquatable<RenderQueueRange> {
    ...
    public RenderQueueRange(int lowerBound, int upperBound) {
      if (lowerBound < k_MinimumBound || lowerBound > k_MaximumBound)
        throw new ArgumentOutOfRangeException(
            ...
            $"The lower bound must be at least {k_MinimumBound} and at most {k_MaximumBound}.");
      if (upperBound < k_MinimumBound || upperBound > k_MaximumBound)
        throw new ArgumentOutOfRangeException(
            nameof(upperBound), upperBound,
            $"The upper bound must be at least {k_MinimumBound} and at most {k_MaximumBound}.");
      m_LowerBound = lowerBound;
      m_UpperBound = upperBound;
    }

    public static RenderQueueRange all => new RenderQueueRange{
        m_LowerBound = k_MinimumBound, m_UpperBound = k_MaximumBound};

    public static RenderQueueRange opaque => new RenderQueueRange{
        m_LowerBound = k_MinimumBound,
        m_UpperBound = (int) UnityEngine.Rendering.RenderQueue.GeometryLast};

    public static RenderQueueRange transparent => new RenderQueueRange{
        m_LowerBound = (int) UnityEngine.Rendering.RenderQueue.GeometryLast + 1,
        m_UpperBound = k_MaximumBound};
    ...
    public override int GetHashCode() {
      unchecked { return (m_LowerBound * 397) ^ m_UpperBound; }
    }

    public static bool operator ==(RenderQueueRange left,
                                   RenderQueueRange right) {
