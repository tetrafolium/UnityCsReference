...
namespace UnityEngine.Rendering {
  ...
  public struct AttachmentDescriptor : IEquatable<AttachmentDescriptor> {
    ...
    // The Load action to use when this attachment is accessed for the first
    // time in this renderpass
    public RenderBufferLoadAction loadAction {
      get { return m_LoadAction; }
      set { m_LoadAction = value; }
    }

    // Store action to use when the the last subpass that accesses this
    // attachment is done. Note that this isn't set by the constructor, but
    // rather by calling SetStoreTarget/SetResolveTarget
    ...
    public RenderTextureFormat format {
      ...
      set {
        m_Format = GraphicsFormatUtility.GetGraphicsFormat(
            value, RenderTextureReadWrite.Default);
      }
    }
    ...
    // The render texture to resolve the attachment into at the end of the
    // renderpass
    ...
    // If loadAction is set to clear and this is a color surface, clear it to
    // this color
    ...
    // If loadAction is set to clear and this is a depth surface, clear to this
    // value
    ...
    // If loadAction is set to clear and this is a depth+stencil surface, clear
    // to this value
    ...
    // Bind a backing surface for this attachment. If none is set, the
    // attachment is transient / memoryless (where supported) or a temporary
    // surface that's released at the end of the renderpass. If
    // loadExistingContents is true, the current contents of the surface is
    // loaded as the initial pixel values for the attachment, otherwise the
    // initial values are undefined (with the expectation that the renderpass
    // will render to every pixel on the screen) If storeResults is true, the
    // attachment contents at the end of the renderpass are stored to the
    // surface, otherwise the contents of the surface are undefined after the
    // end of the renderpass.
    public void ConfigureTarget(RenderTargetIdentifier target,
                                bool loadExistingContents, bool storeResults) {
      ...
      if (storeResults) {
        if (m_StoreAction == RenderBufferStoreAction.StoreAndResolve ||
    ...
    // If the renderpass has MSAA enabled, AA-resolve this attachment into the
    // given render target.
    ...
    // At the beginning of the renderpass, clear this attachment with the given
    // clear color (or depth/stencil)
    public void ConfigureClear(Color clearColor, float clearDepth = 1.0f,
                               uint clearStencil = 0) {
    ...
    public AttachmentDescriptor(GraphicsFormat format) : this() {
    ...
    public AttachmentDescriptor(RenderTextureFormat format)
        : this(GraphicsFormatUtility.GetGraphicsFormat(
              format, RenderTextureReadWrite.Default)) {}

    public AttachmentDescriptor(RenderTextureFormat format,
                                RenderTargetIdentifier target,
                                bool loadExistingContents = false,
                                bool storeResults = false, bool resolve = false)
        : this(GraphicsFormatUtility.GetGraphicsFormat(
              format, RenderTextureReadWrite.Default)) {}

    public bool Equals(AttachmentDescriptor other) {
      return m_LoadAction == other.m_LoadAction &&
             m_StoreAction == other.m_StoreAction &&
             m_Format == other.m_Format &&
             m_LoadStoreTarget.Equals(other.m_LoadStoreTarget) &&
             m_ResolveTarget.Equals(other.m_ResolveTarget) &&
             m_ClearColor.Equals(other.m_ClearColor) &&
             m_ClearDepth.Equals(other.m_ClearDepth) &&
    ...
    public static bool operator ==(AttachmentDescriptor left,
                                   AttachmentDescriptor right) {
