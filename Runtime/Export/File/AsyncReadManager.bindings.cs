...
public struct AsyncReadManagerRequestMetric
{
    ...
    public double   TimeInQueueMicroseconds     {
        get;    // (if state is InQueue this is set at the point GetMetrics() call was made )
    }
    [NativeName("totalTimeMicroseconds")]
    public double   TotalTimeMicroseconds       {
        get;    // if state is Completed or Failed this is : total time from request to completion (includes latency)
    }
...
public class AsyncReadManagerSummaryMetrics
{
    ...
    public float AverageThroughputMBPerSecond {
        get;    // Takes into account wait time as well as read time, whereas bandwidth is just file read
    }
