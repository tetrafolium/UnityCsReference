...
namespace Unity.IO.LowLevel.Unsafe {
  [StructLayout(LayoutKind.Sequential)] unsafe public struct ReadCommand {
    public void *Buffer;
    public long Offset;
    public long Size;
  }
  ...
  public enum ReadStatus { Complete, InProgress, Failed }
  ...
  public struct ReadHandle : IDisposable {
    ...
    public bool IsValid() { return IsReadHandleValid(this); }

    public void Dispose() {
      if (!IsReadHandleValid(this))
        throw new InvalidOperationException(
            "ReadHandle.Dispose cannot be called twice on the same ReadHandle");
      if (Status == ReadStatus.InProgress)
        throw new InvalidOperationException(
            "ReadHandle.Dispose cannot be called until the read operation completes");
    ...
    public JobHandle JobHandle {
      get {
        if (!IsReadHandleValid(this))
          throw new InvalidOperationException(
              "ReadHandle.JobHandle cannot be called after the ReadHandle has been disposed");
    ...
    public ReadStatus Status {
      get {
        if (!IsReadHandleValid(this))
          throw new InvalidOperationException(
              "ReadHandle.Status cannot be called after the ReadHandle has been disposed");
        return GetReadStatus(this);
      }
    }

    [ThreadAndSerializationSafe()]
    [FreeFunction("AsyncReadManagerManaged::GetReadStatus",
                  IsThreadSafe = true)]
    private extern static ReadStatus GetReadStatus(ReadHandle handle);

    [ThreadAndSerializationSafe()]
    [FreeFunction("AsyncReadManagerManaged::ReleaseReadHandle",
                  IsThreadSafe = true)]
    private extern static void ReleaseReadHandle(ReadHandle handle);

    [ThreadAndSerializationSafe()]
    [FreeFunction("AsyncReadManagerManaged::IsReadHandleValid",
                  IsThreadSafe = true)]
  ...
  [NativeHeader(
      "Runtime/File/AsyncReadManagerManagedApi.h")] unsafe static public class
      AsyncReadManager {
    [ThreadAndSerializationSafe()]
    [FreeFunction("AsyncReadManagerManaged::Read", IsThreadSafe = true)]
    private extern unsafe static ReadHandle
    ReadInternal(string filename, void *cmds, uint cmdCount, string assetName,
                 UInt64 typeID, AssetLoadingSubsystem subsystem);
    public static ReadHandle
    Read(string filename, ReadCommand *readCmds, uint readCmdCount,
         string assetName = "", UInt64 typeID = 0,
         AssetLoadingSubsystem subsystem = AssetLoadingSubsystem.Scripts) {
      return ReadInternal(filename, readCmds, readCmdCount, assetName, typeID,
                          subsystem);
    }
  }
  ...
  public enum FileReadType { Sync = 0, Async = 1 }
  ;
  ...
  public struct AsyncReadManagerRequestMetric {
    ...
    public UInt64 AssetTypeId {
      get; // some are UIDs
    }
    [NativeName("currentBytesRead")]
    public UInt64 CurrentBytesRead {
      get;
    }
    ...
    public Priority PriorityLevel {
      get; // currently only low (0) and high (1)
    }
    ...
    public double RequestTimeMicroseconds {
      get; // Time request was made
    }
    [NativeName("timeInQueueMicroseconds")]
    public double TimeInQueueMicroseconds {
      get; // (if state is InQueue this is set at the point GetMetrics() call
           // was made )
    }
    [NativeName("totalTimeMicroseconds")]
    public double TotalTimeMicroseconds {
      get; // if state is Completed or Failed this is : total time from request
           // to completion (includes latency)
    }
    // if stats is Processing or InQueue this is : total time (at the point
    // GetMetrics() call was made) ioTimeMicroSeconds =
    // totalTimeMicroSeconds-timeInQueueMicroseconds bandwidth = sizeBytes /
    // ioTimeMicroSeconds throughput = sizeBytes / totalTimeMicroSeconds
  }
  ...
  static public class AsyncReadManagerMetrics {
    ...
    [FreeFunction("AreMetricsEnabled_Internal")] extern static public bool
    IsEnabled();

    [FreeFunction("GetAsyncReadManagerMetrics()->ClearMetrics"),
     ThreadSafe] extern static private void ClearMetrics_Internal();

    static public void ClearCompletedMetrics() { ClearMetrics_Internal(); }

    [FreeFunction("GetAsyncReadManagerMetrics()->GetMarshalledMetrics"),
     ThreadSafe] extern static internal
        AsyncReadManagerRequestMetric[] GetMetrics_Internal(bool clear);

    [FreeFunction("GetAsyncReadManagerMetrics()->GetMetrics_NoAlloc"),
     ThreadSafe] extern static internal void
    GetMetrics_NoAlloc_Internal(
        [ NotNull ] List<AsyncReadManagerRequestMetric> metrics, bool clear);

    [FreeFunction(
         "GetAsyncReadManagerMetrics()->GetMarshalledMetrics_Filtered_Managed"),
     ThreadSafe] extern static internal
        AsyncReadManagerRequestMetric[] GetMetrics_Filtered_Internal(
            AsyncReadManagerMetricsFilters filters, bool clear);

    [FreeFunction(
         "GetAsyncReadManagerMetrics()->GetMetrics_NoAlloc_Filtered_Managed"),
     ThreadSafe] extern static internal void
    GetMetrics_NoAlloc_Filtered_Internal(
        [ NotNull ] List<AsyncReadManagerRequestMetric> metrics,
        AsyncReadManagerMetricsFilters filters, bool clear);

    static public AsyncReadManagerRequestMetric[] GetMetrics(
        AsyncReadManagerMetricsFilters filters, Flags flags) {
      bool clear =
          ((flags & Flags.ClearOnRead) == Flags.ClearOnRead) ? true : false;

      return GetMetrics_Filtered_Internal(filters, clear);
    }

    static public void
    GetMetrics(List<AsyncReadManagerRequestMetric> outMetrics,
               AsyncReadManagerMetricsFilters filters, Flags flags) {
      bool clear =
          ((flags & Flags.ClearOnRead) == Flags.ClearOnRead) ? true : false;
    ...
    static public void
    GetMetrics(List<AsyncReadManagerRequestMetric> outMetrics, Flags flags) {
      bool clear =
          ((flags & Flags.ClearOnRead) == Flags.ClearOnRead) ? true : false;
    ...
    [FreeFunction(
        "GetAsyncReadManagerMetrics()->StartCollecting")] extern static public void
    StartCollectingMetrics();

    [FreeFunction(
        "GetAsyncReadManagerMetrics()->StopCollecting")] extern static public void
    StopCollectingMetrics();

    [FreeFunction(
        "GetAsyncReadManagerMetrics()->GetCurrentSummaryMetrics")] extern static internal
        AsyncReadManagerSummaryMetrics GetSummaryMetrics_Internal(bool clear);
    static public AsyncReadManagerSummaryMetrics
    GetCurrentSummaryMetrics(Flags flags) {
      bool clear =
          ((flags & Flags.ClearOnRead) == Flags.ClearOnRead) ? true : false;
    ...
    [FreeFunction(
        "GetAsyncReadManagerMetrics()->GetCurrentSummaryMetricsWithFilters")] extern static internal
        AsyncReadManagerSummaryMetrics GetSummaryMetricsWithFilters_Internal(
            AsyncReadManagerMetricsFilters metricsFilters, bool clear);
    static public AsyncReadManagerSummaryMetrics
    GetCurrentSummaryMetrics(AsyncReadManagerMetricsFilters metricsFilters,
                             Flags flags) {
      bool clear =
          ((flags & Flags.ClearOnRead) == Flags.ClearOnRead) ? true : false;
    ...
    [FreeFunction("GetAsyncReadManagerMetrics()->GetSummaryOfMetrics_Managed"),
     ThreadSafe] extern static internal AsyncReadManagerSummaryMetrics
        GetSummaryOfMetrics_Internal(AsyncReadManagerRequestMetric[] metrics);
    static public AsyncReadManagerSummaryMetrics
    ...
    [FreeFunction(
         "GetAsyncReadManagerMetrics()->GetSummaryOfMetrics_FromContainer_Managed",
         ThrowsException = true),
     ThreadSafe] extern static internal AsyncReadManagerSummaryMetrics
    GetSummaryOfMetrics_FromContainer_Internal(
    ...
    [FreeFunction(
         "GetAsyncReadManagerMetrics()->GetSummaryOfMetricsWithFilters_Managed"),
     ThreadSafe] extern static internal
        AsyncReadManagerSummaryMetrics GetSummaryOfMetricsWithFilters_Internal(
    ...
    [FreeFunction(
         "GetAsyncReadManagerMetrics()->GetSummaryOfMetricsWithFilters_FromContainer_Managed",
         ThrowsException = true),
     ThreadSafe] extern static internal AsyncReadManagerSummaryMetrics
    GetSummaryOfMetricsWithFilters_FromContainer_Internal(
        List<AsyncReadManagerRequestMetric> metrics,
        AsyncReadManagerMetricsFilters metricsFilters);
    static public AsyncReadManagerSummaryMetrics
    GetSummaryOfMetrics(List<AsyncReadManagerRequestMetric> metrics,
                        AsyncReadManagerMetricsFilters metricsFilters) {
      return GetSummaryOfMetricsWithFilters_FromContainer_Internal(
          metrics, metricsFilters);
    }

    [FreeFunction(
         "GetAsyncReadManagerMetrics()->GetTotalSizeNonASRMReadsBytes"),
     ThreadSafe] extern static public UInt64
    GetTotalSizeOfNonASRMReadsBytes(bool emptyAfterRead);
  }
  ...
  public class AsyncReadManagerSummaryMetrics {
    ...
    public float AverageThroughputMBPerSecond {
      get; // Takes into account wait time as well as read time, whereas
           // bandwidth is just file read
    }
    ...
    public UInt64 LongestReadAssetType {
      get;
    }
    [NativeName("longestWaitAssetType")]
    public UInt64 LongestWaitAssetType {
      get;
    }
  ...
  public class AsyncReadManagerMetricsFilters {
    ...
    // Constructors for common use cases, other combinations of filters can be
    // created with Set Filter APIs.
    public AsyncReadManagerMetricsFilters() { ClearFilters(); }
    ...
    public void SetTypeIDFilter(UInt64 _typeID) {
      TypeIDs = new UInt64[]{_typeID};
    }
    public void SetStateFilter(ProcessingState _state) {
      States = new ProcessingState[]{_state};
    }
    public void SetReadTypeFilter(FileReadType _readType) {
      ReadTypes = new FileReadType[]{_readType};
    }
    public void SetPriorityFilter(Priority _priorityLevel) {
      PriorityLevels = new Priority[]{_priorityLevel};
    }
    public void SetSubsystemFilter(AssetLoadingSubsystem _subsystem) {
      Subsystems = new AssetLoadingSubsystem[]{_subsystem};
    }
