...
namespace Unity.Collections {
  ...
  public unsafe struct NativeArray<T> : IDisposable,
                                        IEnumerable<T>,
                                        IEquatable<NativeArray<T>> where T
      : struct {
    ...
    // TODO: Once Burst supports internal/external functions in static
    // initializers, this can become
    //   static readonly int s_staticSafetyId =
    //   AtomicSafetyHandle.NewStaticSafetyId<NativeArray<T>>();
    // and InitStaticSafetyId() can be replaced with a call to
    ...
    public NativeArray(
      ...
      if ((options & NativeArrayOptions.ClearMemory) ==
        ...
        UnsafeUtility.MemClear(m_Buffer,
    ...
    static void Allocate(int length, Allocator allocator,
      ...
      // Make sure we cannot allocate more than int.MaxValue (2,147,483,647
      // bytes) because the underlying UnsafeUtility.Malloc is expecting a int.
      // TODO: change UnsafeUtility.Malloc to accept a UIntPtr length instead to
      // match C++ API
      ...
      array.m_Buffer = UnsafeUtility.Malloc(
          totalSize, UnsafeUtility.AlignOf<T>(), allocator);
    ...
    public void Dispose() {
      if (m_Buffer == null) {
        throw new ObjectDisposedException(
            "The NativeArray is already disposed.");
      }
    ...
    /// <summary>
    /// Safely disposes of this container and deallocates its memory when the
    /// jobs that use it have completed.
    /// </summary>
    /// <remarks>You can call this function dispose of the container immediately
    /// after scheduling the job. Pass the
    /// [JobHandle](https://docs.unity3d.com/ScriptReference/Unity.Jobs.JobHandle.html)
    /// returned by the
    /// [Job.Schedule](https://docs.unity3d.com/ScriptReference/Unity.Jobs.IJobExtensions.Schedule.html)
    /// method using the `jobHandle` parameter so the job scheduler can dispose
    /// the container after all jobs using it have run.</remarks> <param
    /// name="jobHandle">The job handle or handles for any scheduled jobs that
    /// use this container.</param> <returns>A new job handle containing the
    /// prior handles as well as the handle for the job that deletes the
    /// container.</returns>
    public JobHandle Dispose(JobHandle inputDeps) {
      ...
      if (m_AllocatorLabel > Allocator.None) {
        // [DeallocateOnJobCompletion] is not supported, but we want the
        // deallocation to happen in a thread. DisposeSentinel needs to be
        // cleared on main thread. AtomicSafetyHandle can be destroyed after the
        // job was scheduled (Job scheduling will check that no jobs are writing
        // to the container).
        DisposeSentinel.Clear(ref m_DisposeSentinel);

        var jobHandle =
            new NativeArrayDisposeJob{
                Data =
                    new NativeArrayDispose{m_Buffer = m_Buffer,
    ...
    public override int GetHashCode() {
      unchecked { return ((int) m_Buffer * 397) ^ m_Length; }
    }
    ...
    public static void Copy(NativeArray<T> src, NativeArray<T> dst) {
      ...
      if (src.Length != dst.Length)
        throw new ArgumentException(
            "source and destination length must be the same");
    ...
    public static void Copy(NativeArray<T> src, int srcIndex,
                            NativeArray<T> dst, int dstIndex, int length) {
      AtomicSafetyHandle.CheckReadAndThrow(src.m_Safety);
      AtomicSafetyHandle.CheckWriteAndThrow(dst.m_Safety);

      if (length < 0)
        throw new ArgumentOutOfRangeException(
            nameof(length), "length must be equal or greater than zero.");

      if (srcIndex < 0 || srcIndex > src.Length ||
          (srcIndex == src.Length && src.Length > 0))
        throw new ArgumentOutOfRangeException(
            nameof(srcIndex),
            "srcIndex is outside the range of valid indexes for the source NativeArray.");

      if (dstIndex < 0 || dstIndex > dst.Length ||
          (dstIndex == dst.Length && dst.Length > 0))
        throw new ArgumentOutOfRangeException(
            nameof(dstIndex),
            "dstIndex is outside the range of valid indexes for the destination NativeArray.");
    ...
    public static void Copy(T[] src, int srcIndex, NativeArray<T> dst,
      ...
      if (srcIndex < 0 || srcIndex > src.Length ||
        ...
        throw new ArgumentOutOfRangeException(
            nameof(srcIndex),
            "srcIndex is outside the range of valid indexes for the source array.");
      ...
      if (srcIndex + length > src.Length)
        throw new ArgumentException(
            "length is greater than the number of elements from srcIndex to the end of the source array.",
            nameof(length));
      ...
      UnsafeUtility.MemCpy((byte *) dst.m_Buffer +
    ...
    public static void Copy(NativeArray<T> src, int srcIndex, T[] dst,
                            int dstIndex, int length) {
      AtomicSafetyHandle.CheckReadAndThrow(src.m_Safety);
      ...
      if (dstIndex < 0 || dstIndex > dst.Length ||
        ...
        throw new ArgumentOutOfRangeException(
            nameof(dstIndex),
            "dstIndex is outside the range of valid indexes for the destination array.");
      ...
      if (dstIndex + length > dst.Length)
        throw new ArgumentException(
            "length is greater than the number of elements from dstIndex to the end of the destination array.",
            nameof(length));

      var handle = GCHandle.Alloc(dst, GCHandleType.Pinned);
      var addr = handle.AddrOfPinnedObject();

      UnsafeUtility.MemCpy((byte *) addr + dstIndex * UnsafeUtility.SizeOf<T>(),
                           (byte *) src.m_Buffer +
    ...
    [Conditional("ENABLE_UNITY_COLLECTIONS_CHECKS")]
    private void CheckReinterpretLoadRange<U>(int sourceIndex) where U
        : struct {
    ...
    private void CheckReinterpretStoreRange<U>(int destIndex) where U : struct {
      ...
      if (firstByte < 0 || lastByte > byteSize)
        throw new ArgumentOutOfRangeException(
            nameof(destIndex),
            "stored byte range must fall inside container bounds");
    }

    public U ReinterpretLoad<U>(int sourceIndex) where U : struct {
      CheckReinterpretLoadRange<U>(sourceIndex);
      byte *src_ptr = ((byte *) m_Buffer) +
                      ((long) UnsafeUtility.SizeOf<T>()) * sourceIndex;
    ...
    public void ReinterpretStore<U>(int destIndex, U data) where U : struct {
      ...
      byte *dst_ptr =
          ((byte *) m_Buffer) + ((long) UnsafeUtility.SizeOf<T>()) * destIndex;
      UnsafeUtility.WriteArrayElement<U>(dst_ptr, 0, data);
    }

    private NativeArray<U> InternalReinterpret<U>(int length) where U : struct {
      var result = NativeArrayUnsafeUtility.ConvertExistingDataToNativeArray<U>(
          m_Buffer, length, m_AllocatorLabel);
    ...
    // DisposeSentinel is a class so that's not supported in Burst. However, in
    // Burst it's guaranteed that the sentinel is null anyway, so we can just
    // use BurstDiscard on the place that works on it.
    [BurstDiscard] void SetDisposeSentinel<U>(ref NativeArray<U> result) where U
        : struct {
    ...
    public NativeArray<U> Reinterpret<U>(int expectedTypeSize) where U
        : struct {
      long tSize = UnsafeUtility.SizeOf<T>();
      long uSize = UnsafeUtility.SizeOf<U>();
    ...
    public NativeArray<T> GetSubArray(int start, int length) {
      ...
      var result = NativeArrayUnsafeUtility.ConvertExistingDataToNativeArray<T>(
          ((byte *) m_Buffer) + ((long) UnsafeUtility.SizeOf<T>()) * start,
          length, Allocator.Invalid);
    ...
    public unsafe struct ReadOnly {
      ...
      void CheckElementReadAccess(int index) {
        if (index < 0 && index >= m_Length) {
          throw new IndexOutOfRangeException(
              $"Index {index} is out of range (must be between 0 and {m_Length-1}).");
        }
  ...
  // [BurstCompile] - can't use attribute since it's inside
  ...
  internal sealed class NativeArrayDebugView<T> where T : struct {
    ...
    public NativeArrayDebugView(NativeArray<T> array) { m_Array = array; }
...
namespace Unity.Collections.LowLevel.Unsafe {
  public static class NativeArrayUnsafeUtility {
    public static AtomicSafetyHandle
    GetAtomicSafetyHandle<T>(NativeArray<T> array) where T : struct {
      return array.m_Safety;
    }

    public static void
    SetAtomicSafetyHandle<T>(ref NativeArray<T> array,
                             AtomicSafetyHandle safety) where T : struct {
      array.m_Safety = safety;
    }

    /// Internal method used typically by other systems to provide a view on
    /// them. The caller is still the owner of the data.
    public static unsafe NativeArray<T>
    ConvertExistingDataToNativeArray<T>(void *dataPointer, int length,
    ...
    GetUnsafePtr<T>(this NativeArray<T> nativeArray) where T : struct {
      AtomicSafetyHandle.CheckWriteAndThrow(nativeArray.m_Safety);
      return nativeArray.m_Buffer;
    }

    public static unsafe void *
    GetUnsafeReadOnlyPtr<T>(this NativeArray<T> nativeArray) where T : struct {
      AtomicSafetyHandle.CheckReadAndThrow(nativeArray.m_Safety);
      return nativeArray.m_Buffer;
    }

    public static unsafe void *
    GetUnsafeBufferPointerWithoutChecks<T>(NativeArray<T> nativeArray) where T
        : struct {
      return nativeArray.m_Buffer;
    }
  }
