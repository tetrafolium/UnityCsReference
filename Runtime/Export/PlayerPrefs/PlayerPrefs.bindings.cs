...
// This exception is thrown by the [[PlayerPrefs]] class in the Web player if
// the preference file would exceed the allotted storage space when setting a
// value.
...
public class PlayerPrefs {
  [NativeMethod("SetInt")] extern private static bool TrySetInt(string key,
                                                                int value);

  [NativeMethod("SetFloat")] extern private static bool
  TrySetFloat(string key, float value);

  [NativeMethod("SetString")] extern private static bool
  ...
  // NOTE: DisposeSentinel requires access to EditorPrefs but from
  // UnityEngine.dll
  //       (Which cant access UnityEditor.dll)
  //       So we expose the API here. Internal only, users should use the normal
  //       EditorPrefs class

  [StaticAccessor("EditorPrefs", StaticAccessorType.DoubleColon)]
  [NativeMethod("SetInt")] extern internal static void
  EditorPrefsSetInt(string key, int value);

  [StaticAccessor("EditorPrefs", StaticAccessorType.DoubleColon)]
  [NativeMethod("GetInt")] extern internal static int
  EditorPrefsGetInt(string key, int defaultValue);
}
}
