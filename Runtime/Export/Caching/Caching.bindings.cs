...
extern public static bool compressionEnabled {
	get;
	set;
}

extern public static bool ready {
	[NativeName("GetIsReady")] get;
}

// Delete all AssetBundle content that has been cached by the current
// application.
...
[NativeName("ClearCache")] extern internal static bool
ClearCache_Int(int expiration);
...
public static bool ClearCachedVersion(string assetBundleName, Hash128 hash) {
	if (string.IsNullOrEmpty(assetBundleName))
		throw new ArgumentException(
			      "Input AssetBundle name cannot be null or empty.");

	return ClearCachedVersionInternal(assetBundleName, hash);
}

[NativeName("ClearCachedVersion")] extern internal static bool
ClearCachedVersionInternal(string assetBundleName, Hash128 hash);
...
extern internal static bool ClearCachedVersions(string assetBundleName,
                                                Hash128 hash,
                                                bool keepInputVersion);

extern internal static Hash128[] GetCachedVersions(string assetBundleName);

public static void GetCachedVersions(string assetBundleName,
	...
	if (outCachedVersions == null)
		throw new ArgumentNullException(
			      "Input outCachedVersions cannot be null.");
...
public static bool IsVersionCached(string url, Hash128 hash) {
	if (string.IsNullOrEmpty(url))
		throw new ArgumentException(
			      "Input AssetBundle url cannot be null or empty.");
...
[NativeName("IsCached")] extern internal static bool
IsVersionCached(string url, string assetBundleName, Hash128 hash);
...
extern internal static bool MarkAsUsed(string url, string assetBundleName,
                                       Hash128 hash);
...
[NativeConditional(
	 "PLATFORM_IOS || PLATFORM_TVOS")] extern internal static void
SetNoBackupFlag(string url, string assetBundleName, Hash128 hash,
                bool enabled);

[Obsolete(
	 "This function is obsolete and will always return -1. Use IsVersionCached instead.")]
...
public static int spaceUsed {
	get { return (int) Caching.spaceOccupied; }
}

[Obsolete(
	 "This property is only used for the current cache, use Cache.spaceOccupied to get used bytes per cache.")] extern public static long
spaceOccupied {
	[StaticAccessor("GetCachingManager().GetCurrentCache()",
	                StaticAccessorType.Dot)]
	[NativeName("GetCachingDiskSpaceUsed")] get;
}

[Obsolete("Please use use Cache.spaceOccupied to get used bytes per cache.")]
public static int spaceAvailable {
	get { return (int) Caching.spaceFree; }
}

[Obsolete(
	 "This property is only used for the current cache, use Cache.spaceFree to get unused bytes per cache.")] extern public static long
spaceFree {
	[StaticAccessor("GetCachingManager().GetCurrentCache()",
	                StaticAccessorType.Dot)]
	[NativeName("GetCachingDiskSpaceFree")] get;
}

[Obsolete(
	 "This property is only used for the current cache, use Cache.maximumAvailableStorageSpace to access the maximum available storage space per cache.")]
[StaticAccessor("GetCachingManager().GetCurrentCache()",
                StaticAccessorType.Dot)] extern public static long
maximumAvailableDiskSpace {
	[NativeName("GetMaximumDiskSpaceAvailable")] get;
	[NativeName("SetMaximumDiskSpaceAvailable")] set;
}

[Obsolete(
	 "This property is only used for the current cache, use Cache.expirationDelay to access the expiration delay per cache.")]
[StaticAccessor("GetCachingManager().GetCurrentCache()",
...
public static Cache AddCache(string cachePath) {
	...
	if (cachePath.Replace('\\', '/').StartsWith(
		    Application.streamingAssetsPath)) {
		// Set to readonly if the input cache path is under StreamingAssetsFolder.
		isReadonly = true;
	} else {
		if (!Directory.Exists(cachePath))
			throw new ArgumentException("Cache path '" + cachePath +
			                            "' doesn't exist.");

		// Set to readonly if the input cache path is readonly.
		if ((File.GetAttributes(cachePath) & FileAttributes.ReadOnly) ==
	...
	if (GetCacheByPath(cachePath).valid)
		throw new InvalidOperationException("Cache with path '" + cachePath +
		                                    "' has already been added.");

	return AddCache(cachePath, isReadonly);
}

[NativeName("AddCachePath")] extern internal static Cache
AddCache(string cachePath, bool isReadonly);

[StaticAccessor("CachingManagerWrapper", StaticAccessorType.DoubleColon)]
[NativeName("Caching_GetCacheHandleAt")]
[NativeThrows] extern public static Cache GetCacheAt(int cacheIndex);

[StaticAccessor("CachingManagerWrapper", StaticAccessorType.DoubleColon)]
[NativeName("Caching_GetCacheHandleByPath")]
[NativeThrows] extern public static Cache GetCacheByPath(string cachePath);
...
[NativeThrows] extern public static bool RemoveCache(Cache cache);

[StaticAccessor("CachingManagerWrapper", StaticAccessorType.DoubleColon)]
[NativeName("Caching_MoveCacheBeforeByHandle")]
[NativeThrows] extern public static void MoveCacheBefore(Cache src,
                                                         Cache dst);

[StaticAccessor("CachingManagerWrapper", StaticAccessorType.DoubleColon)]
[NativeName("Caching_MoveCacheAfterByHandle")]
[NativeThrows] extern public static void MoveCacheAfter(Cache src, Cache dst);

extern public static int cacheCount {
	get;
}

[StaticAccessor("CachingManagerWrapper",
                StaticAccessorType.DoubleColon)] extern public static Cache
defaultCache {
