...
// Base class to derive custom property attributes from. Use this to create
// custom attributes for script variables.
[System.AttributeUsage(AttributeTargets.Field, Inherited = true,
                       AllowMultiple = false)]
...
public class SpaceAttribute : PropertyAttribute {
  ...
  public SpaceAttribute() {
    ...
    // which reserves 6 pixels, plus the usual 2 pixels caused by the
    // neighboring margin. (Why not 2 pixels for margin both below and above?
    // Because one of those is already accounted for when the space is not
    // there.)
    this.height = 8;
  }
...
public sealed class MultilineAttribute : PropertyAttribute {
  ...
  public MultilineAttribute() { this.lines = 3; }
...
public sealed class ColorUsageAttribute : PropertyAttribute {
  ...
  [Obsolete(
      "Brightness and exposure parameters are no longer used for anything. Use ColorUsageAttribute(bool showAlpha, bool hdr)")]
  public ColorUsageAttribute(bool showAlpha, bool hdr, float minBrightness,
                             float maxBrightness, float minExposureValue,
