...
public static IScriptableRuntimeReflectionSystem system {
	...
	set {
		if (value == null || value.Equals(null)) {
			Debug.LogError(
				"'null' cannot be assigned to ScriptableRuntimeReflectionSystemSettings.system");
			return;
		}
		// We always allow the BuiltinRuntimeReflectionSystem, it is set by
		// Unity on domain reload However, we issue a warning when multiple
		// different IScriptableRuntimeReflectionSystem have been assigned.
		else if (!(system is BuiltinRuntimeReflectionSystem) &&
		         !(value is BuiltinRuntimeReflectionSystem) && system != value)
			Debug.LogWarningFormat(
				"ScriptableRuntimeReflectionSystemSettings.system is assigned more than once. Only a the last instance will be used. (Last instance {0}, New instance {1})",
				system, value);
...
static ScriptableRuntimeReflectionSystemWrapper s_Instance =
	new ScriptableRuntimeReflectionSystemWrapper();

static ScriptableRuntimeReflectionSystemWrapper
Internal_ScriptableRuntimeReflectionSystemSettings_instance {
	[RequiredByNativeCode] get { return s_Instance; }
}

[RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.AfterSceneLoad)]
[StaticAccessor("ScriptableRuntimeReflectionSystem",
