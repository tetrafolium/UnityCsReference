...
struct SnapAxisFilter : IEquatable<SnapAxisFilter> {
  ...
  public static Vector3 operator *(Quaternion rotation, SnapAxisFilter mask) {
    ...
    rotated = new Vector3(Mathf.Abs(rotated.x), Mathf.Abs(rotated.y),
                          Mathf.Abs(rotated.z));

    if (active > 1) {
      return new Vector3(rotated.x > rotated.y || rotated.x > rotated.z ? 1 : 0,
  ...
  public bool Equals(SnapAxisFilter other) { return m_Mask == other.m_Mask; }
  ...
  public override int GetHashCode() { return m_Mask.GetHashCode(); }
}
}
