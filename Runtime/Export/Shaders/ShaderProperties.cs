...
[NativeHeader("Runtime/Graphics/ShaderScriptBindings.h")] partial class Shader {
  [FreeFunction(
      "ShaderScripting::GetPropertyName")] extern private static string
  GetPropertyName([ NotNull ] Shader shader, int propertyIndex);
  [FreeFunction("ShaderScripting::GetPropertyNameId")] extern private static int
  GetPropertyNameId([ NotNull ] Shader shader, int propertyIndex);
  [FreeFunction(
      "ShaderScripting::GetPropertyType")] extern private static ShaderPropertyType
  GetPropertyType([ NotNull ] Shader shader, int propertyIndex);
  [FreeFunction(
      "ShaderScripting::GetPropertyDescription")] extern private static string
  GetPropertyDescription([ NotNull ] Shader shader, int propertyIndex);
  [FreeFunction(
      "ShaderScripting::GetPropertyFlags")] extern private static ShaderPropertyFlags
  GetPropertyFlags([ NotNull ] Shader shader, int propertyIndex);
  [FreeFunction(
      "ShaderScripting::GetPropertyAttributes")] extern private static string
      [] GetPropertyAttributes([ NotNull ] Shader shader, int propertyIndex);
  [FreeFunction(
      "ShaderScripting::GetPropertyDefaultValue")] extern private static Vector4
  GetPropertyDefaultValue([ NotNull ] Shader shader, int propertyIndex);
  [FreeFunction(
      "ShaderScripting::GetPropertyTextureDimension")] extern private static TextureDimension
  GetPropertyTextureDimension([ NotNull ] Shader shader, int propertyIndex);
  [FreeFunction(
      "ShaderScripting::GetPropertyTextureDefaultName")] extern private static string
  GetPropertyTextureDefaultName([ NotNull ] Shader shader, int propertyIndex);
  [FreeFunction("ShaderScripting::FindTextureStack")] extern private static bool
  FindTextureStackImpl([ NotNull ] Shader s, int propertyIdx,
                       out string stackName, out int layerIndex);
  ...
  public bool FindTextureStack(int propertyIndex, out string stackName,
                               out int layerIndex) {
    CheckPropertyIndex(this, propertyIndex);
    var propType = GetPropertyType(propertyIndex);
    if (propType != ShaderPropertyType.Texture)
      throw new ArgumentException("Property type is not Texture.");
    return FindTextureStackImpl(this, propertyIndex, out stackName,
                                out layerIndex);
  }
}
}
