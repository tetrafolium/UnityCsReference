...
// can define their own lights and types, which need not map to the Unity
// types. Having a separate enum for the global illumination backend also
// allows to only expose light types in the baking backends that can then be
// used by specific SRP implementations.
...
public enum LightMode : byte { Realtime, Mixed, Baked, Unknown }
...
public enum AngularFalloffType : byte { LUT, AnalyticAndInnerAngle }

// The linear color struct contains 4 values. 3 values are for specifying
// normalized RGB values, in other words they're between 0 and 1. The fourth
// value is an positive unbounded intensity value. Color information in this
// struct must always be in linear space. When converting a Unity color that's
// either in linear or gamma space, the Convert function will make sure to
// account for all relevant state so that the resulting color is in linear
// space, before being passed down to the baking backends. No further gamma
// handling is performed in the backends.
...
public struct LinearColor {
		...
		set {
			if (value < 0.0f || value > 1.0f)
				throw new System.ArgumentOutOfRangeException(
					      "Red color (" + value + ") must be in range [0;1].");
			m_red = value;
		}
	}
	public float green {
		...
		set {
			if (value < 0.0f || value > 1.0f)
				throw new System.ArgumentOutOfRangeException(
					      "Green color (" + value + ") must be in range [0;1].");
			m_green = value;
		}
	}
	public float blue {
		...
		set {
			if (value < 0.0f || value > 1.0f)
				throw new System.ArgumentOutOfRangeException(
					      "Blue color (" + value + ") must be in range [0;1].");
			m_blue = value;
		}
	}
	public float intensity {
		...
		set {
			if (value < 0.0f)
				throw new System.ArgumentOutOfRangeException("Intensity (" + value +
				                                             ") must be positive.");
			m_intensity = value;
		}
	}
	public static LinearColor Convert(UnityEngine.Color color,
	                                  float intensity) {
		var lc = GraphicsSettings.lightsUseLinearIntensity ?
		         color.linear.RGBMultiplied(intensity)
	  : color.RGBMultiplied(intensity).linear;
	...
	public static LinearColor Black() {
		LinearColor c;
		c.m_red = c.m_green = c.m_blue = c.m_intensity = 0.0f;
		return c;
	}
...
// Each light type known to the baking backends along with its parameters gets
// its own struct. This way it is clear what type supports what parameters.
// Helper functions are provided to convert Unity lights to these types, but a
// render pipeline may choose to map their lights directly to these structs.
// They are then used to initialize the LightDataGI struct, which is the
// actual interop struct.
public struct DirectionalLight {
	...
	[System.Obsolete(
		 "Directional lights support cookies now. In order to position the cookie projection in the world, a position and full orientation are necessary. Use the position and orientation members instead of the direction parameter.",
		 true)]
...
public struct LightDataGI {
	...
	// for area light (interpretation depends on the type, can affect shadow
	// softness) it is width and height for box light it is sizeX and sizeY
	public float shape0;
	public float shape1;
	// types
...
public static LightMode Extract(LightmapBakeType baketype) {
	return baketype == LightmapBakeType.Realtime ? LightMode.Realtime
	  : (baketype == LightmapBakeType.Mixed ? LightMode.Mixed
	     : LightMode.Baked);
}

public static LinearColor ExtractIndirect(Light l) {
	LinearColor result =
		LinearColor.Convert(l.color, l.intensity * l.bounceIntensity);
	return result;
}

public static float ExtractInnerCone(Light l) {
	return 2.0f * Mathf.Atan(Mathf.Tan(l.spotAngle * 0.5f * Mathf.Deg2Rad) *
	                         (64.0f - 18.0f) / 64.0f);

	// TODO(GI) The line below would provide support for innerConeAngle with
	// baked enlighten, however this feature should come as a whole with baked
	// lighting on all backend so it is commented for now. see card :
	// https://favro.com/organization/c564ede4ed3337f7b17986b6/0ae5422e2f78207a0998ab80?card=Uni-68012
...
private static void ApplyColorTemperature(Color cct,
                                          ref LinearColor lightColor) {
...
public static void Extract(Light l, ref DirectionalLight dir) {
	...
	dir.penumbraWidthRadian =
		l.shadows == LightShadows.Soft ? (Mathf.Deg2Rad * l.shadowAngle) : 0.0f;
}
...
public static void Extract(Light l, ref PointLight point) {
	...
	point.range = l.range;
	point.sphereRadius =
		l.shadows == LightShadows.Soft ? l.shadowRadius : 0.0f;
...
/*
 * Builtin Unity does not support the following light types, so no
   extraction utility function will be provided.
 * The following definitions are only here so it doesn't look like they have
   been forgotten.
 *
   public static void Extract(Light l, ref SpotLightBoxShape box)
   {
    Debug.Assert(false, "Builtin Unity does not support the
...
public static void Extract(Light l, out Cookie cookie) {
	...
	cookie.scale = 1.0f;
	cookie.sizes = (l.type == UnityEngine.LightType.Directional && l.cookie)
	                 ? new Vector2(l.cookieSize, l.cookieSize)
...
RequestLightsDelegate(Light[] requests,
...
RequestLights(Light[] lights, System.IntPtr outLightsPtr,
              int outLightsCount, AtomicSafetyHandle safetyHandle) {
	var outLights =
		NativeArrayUnsafeUtility
		.ConvertExistingDataToNativeArray<LightDataGI>(
			(void *) outLightsPtr, outLightsCount, Allocator.None);
	NativeArrayUnsafeUtility.SetAtomicSafetyHandle(ref outLights,
	                                               safetyHandle);
...
private static readonly RequestLightsDelegate s_DefaultDelegate =
	(Light[] requests, NativeArray<LightDataGI> lightsOutput) => {
	...
	for (int i = 0; i < requests.Length; i++) {
		...
		case UnityEngine.LightType.Directional:
			LightmapperUtils.Extract(l, ref dir);
