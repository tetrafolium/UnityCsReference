...
[StaticAccessor("TextureMixerPlayableBindings",
                StaticAccessorType.DoubleColon)]
[RequiredByNativeCode]
public partial struct TextureMixerPlayable
	: IPlayable,
       IEquatable<TextureMixerPlayable> {
		...
		if (handle.IsValid()) {
			if (!handle.IsPlayableOfType<TextureMixerPlayable>())
				throw new InvalidCastException(
					      "Can't set handle: the playable is not an TextureMixerPlayable.");
		}
	...
	CreateTextureMixerPlayableInternal(ref PlayableGraph graph,
