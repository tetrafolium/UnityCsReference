...
namespace UnityEngine.Playables {
  public struct ScriptPlayable<T> : IPlayable,
                                    IEquatable<ScriptPlayable<T>> where T
      : class,
        IPlayableBehaviour,
        new () {
    private PlayableHandle m_Handle;

    static readonly ScriptPlayable<T> m_NullPlayable =
        new ScriptPlayable<T>(PlayableHandle.Null);
    public static ScriptPlayable<T> Null {
      get { return m_NullPlayable; }
    }

    public static ScriptPlayable<T> Create(PlayableGraph graph,
                                           int inputCount = 0) {
      var handle = CreateHandle(graph, null, inputCount);
      return new ScriptPlayable<T>(handle);
    }

    public static ScriptPlayable<T> Create(PlayableGraph graph, T template,
                                           int inputCount = 0) {
      var handle = CreateHandle(graph, template, inputCount);
      return new ScriptPlayable<T>(handle);
    }

    private static PlayableHandle CreateHandle(PlayableGraph graph, T template,
                                               int inputCount) {
      ...
      if (scriptInstance == null) {
        Debug.LogError("Could not create a ScriptPlayable of Type " +
                       typeof(T));
    ...
    CloneScriptInstanceFromEngineObject(UnityEngine.Object source) {
    ...
    public T GetBehaviour() { return m_Handle.GetObject<T>(); }
