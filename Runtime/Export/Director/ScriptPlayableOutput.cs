...
public partial struct ScriptPlayableOutput : IPlayableOutput {
	...
	public static ScriptPlayableOutput Create(PlayableGraph graph,
	                                          string name) {
		...
		if (handle.IsValid()) {
			if (!handle.IsPlayableOutputOfType<ScriptPlayableOutput>())
				throw new InvalidCastException(
					      "Can't set handle: the playable is not a ScriptPlayableOutput.");
		}
	...
	operator PlayableOutput(ScriptPlayableOutput output) {
		return new PlayableOutput(output.GetHandle());
	}

	public static explicit
	operator ScriptPlayableOutput(PlayableOutput output) {
