...
namespace UnityEngine.Experimental.Playables {
  ...
  public struct CameraPlayable : IPlayable,
                                 IEquatable<CameraPlayable> {
      ...
      if (handle.IsValid()) {
        if (!handle.IsPlayableOfType<CameraPlayable>())
          throw new InvalidCastException(
              "Can't set handle: the playable is not an CameraPlayable.");
      }
    ...
    public Camera GetCamera() { return GetCameraInternal(ref m_Handle); }

    public void SetCamera(Camera value) {
      SetCameraInternal(ref m_Handle, value);
    }

    [NativeThrows] extern private static Camera
    GetCameraInternal(ref PlayableHandle hdl);

    [NativeThrows] extern private static void
    SetCameraInternal(ref PlayableHandle hdl, Camera camera);

    [NativeThrows] extern private static bool
    InternalCreateCameraPlayable(ref PlayableGraph graph, Camera camera,
