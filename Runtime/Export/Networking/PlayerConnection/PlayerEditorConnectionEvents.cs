...
public List<MessageTypeSubscribers> messageTypeSubscribers =
	new List<MessageTypeSubscribers>();
...
public Guid MessageTypeId {
	get { return new Guid(m_messageTypeId); }
	set { m_messageTypeId = value.ToString(); }
}
...
public void InvokeMessageIdSubscribers(Guid messageId, byte[] data,
                                       int playerId) {
	IEnumerable<MessageTypeSubscribers> messageSubscribers =
		messageTypeSubscribers.Where(x => x.MessageTypeId == messageId);
...
public UnityEvent<MessageEventArgs> AddAndCreate(Guid messageId) {
	var MessageTypeSubscriber = messageTypeSubscribers.SingleOrDefault(
		x => x.MessageTypeId == messageId);
	if (MessageTypeSubscriber == null) {
		MessageTypeSubscriber = new MessageTypeSubscribers {
			MessageTypeId = messageId, messageCallback = new MessageEvent()
		};
...
public void
UnregisterManagedCallback(Guid messageId,
