...
public unsafe partial struct MeshDataArray : IDisposable {
	...
	internal void ApplyToMeshAndDispose(Mesh mesh, MeshUpdateFlags flags) {
		if (!mesh.canAccess)
			throw new InvalidOperationException(
				      $"Not allowed to access vertex data on mesh '{mesh.name}' (isReadable is false; Read/Write must be enabled in import settings)");
		ApplyToMeshImpl(mesh, m_Ptrs[0], flags);
		Dispose();
	}

	internal void ApplyToMeshesAndDispose(Mesh[] meshes,
	                                      MeshUpdateFlags flags) {
		for (int i = 0; i < m_Length; ++i) {
			Mesh m = meshes[i];
			if (m == null)
				throw new ArgumentNullException(nameof(meshes),
				                                $"Mesh at index {i} is null");
			if (!m.canAccess)
				throw new InvalidOperationException(
					      $"Not allowed to access vertex data on mesh '{m.name}' at array index {i} (isReadable is false; Read/Write must be enabled in import settings)");
		}
	...
	internal MeshDataArray(Mesh mesh) {
		...
		m_Ptrs = (IntPtr *) UnsafeUtility.Malloc(
			totalSize, UnsafeUtility.AlignOf<IntPtr>(), Allocator.Persistent);
	...
	internal MeshDataArray(Mesh[] meshes, int meshesCount) {
		...
		if (meshes.Length < meshesCount)
			throw new InvalidOperationException(
				      $"Meshes array size ({meshes.Length}) is smaller than meshes count ({meshesCount})");
	...
	internal MeshDataArray(int meshesCount) {
		if (meshesCount < 0)
			throw new InvalidOperationException(
				      $"Mesh count can not be negative (was {meshesCount})");
		m_Length = meshesCount;
		var totalSize = UnsafeUtility.SizeOf<IntPtr>() * meshesCount;
		m_Ptrs = (IntPtr *) UnsafeUtility.Malloc(
			totalSize, UnsafeUtility.AlignOf<IntPtr>(), Allocator.Persistent);
...
public partial struct MeshData {
	...
	public void GetVertices(NativeArray<Vector3> outVertices) {
		CopyAttributeInto(outVertices, VertexAttribute.Position,
		                  VertexAttributeFormat.Float32, 3);
	}

	public void GetNormals(NativeArray<Vector3> outNormals) {
		CopyAttributeInto(outNormals, VertexAttribute.Normal,
		                  VertexAttributeFormat.Float32, 3);
	}

	public void GetTangents(NativeArray<Vector4> outTangents) {
		CopyAttributeInto(outTangents, VertexAttribute.Tangent,
		                  VertexAttributeFormat.Float32, 4);
	}

	public void GetColors(NativeArray<Color> outColors) {
		CopyAttributeInto(outColors, VertexAttribute.Color,
		                  VertexAttributeFormat.Float32, 4);
	}
	...
	public void GetUVs(int channel, NativeArray<Vector2> outUVs) {
		...
		CopyAttributeInto(outUVs, GetUVChannel(channel),
	...
	GetVertexData<T>([ DefaultValue("0") ] int stream = 0) where T : struct {
		CheckReadAccess();
		if (stream < 0 || stream >= vertexBufferCount)
			throw new ArgumentOutOfRangeException(
				      $"{nameof(stream)} out of bounds, should be below {vertexBufferCount} but was {stream}");

		ulong vbSize = GetVertexDataSize(m_Ptr, stream);
		ulong stride = (ulong) UnsafeUtility.SizeOf<T>();
		if (vbSize % stride != 0)
			throw new ArgumentException(
				      $"Type passed to {nameof(GetVertexData)} can't capture the vertex buffer. Mesh vertex buffer size is {vbSize} which is not a multiple of type size {stride}");

		var arrSize = vbSize / stride;
		unsafe {
			var array =
				NativeArrayUnsafeUtility.ConvertExistingDataToNativeArray<T>(
					(void *) GetVertexDataPtr(m_Ptr, stream), (int) arrSize,
					Allocator.None);
	...
	unsafe void CopyAttributeInto<T>(NativeArray<T> buffer,
	                                 VertexAttribute channel,
	                                 VertexAttributeFormat format,
	                                 int dim) where T : struct {
		CheckReadAccess();
		if (!HasVertexAttribute(channel))
			throw new InvalidOperationException(
				      $"Mesh data does not have {channel} vertex component");
		if (buffer.Length < vertexCount)
			throw new InvalidOperationException(
				      $"Not enough space in output buffer (need {vertexCount}, has {buffer.Length})");
		CopyAttributeIntoPtr(m_Ptr, channel, format, dim,
		                     (IntPtr) buffer.GetUnsafePtr());
	}

	public void
	SetVertexBufferParams(int vertexCount,
	...
	public void
	GetIndices(NativeArray<ushort> outIndices, int submesh,
	           [ DefaultValue("true") ] bool applyBaseVertex = true) {
		CheckReadAccess();
		if (submesh < 0 || submesh >= subMeshCount)
			throw new IndexOutOfRangeException(
				      $"Specified submesh ({submesh}) is out of range. Must be greater or equal to 0 and less than subMeshCount ({subMeshCount}).");
		int indexCount = GetIndexCount(m_Ptr, submesh);
		if (outIndices.Length < indexCount)
			throw new InvalidOperationException(
				      $"Not enough space in output buffer (need {indexCount}, has {outIndices.Length})");
		unsafe {
			CopyIndicesIntoPtr(m_Ptr, submesh, applyBaseVertex, 2,
			                   (IntPtr) outIndices.GetUnsafePtr());
		}
	}

	public void
	GetIndices(NativeArray<int> outIndices, int submesh,
	           [ DefaultValue("true") ] bool applyBaseVertex = true) {
		CheckReadAccess();
		if (submesh < 0 || submesh >= subMeshCount)
			throw new IndexOutOfRangeException(
		...
		if (outIndices.Length < indexCount)
			throw new InvalidOperationException(
				      $"Not enough space in output buffer (need {indexCount}, has {outIndices.Length})");
		unsafe {
			CopyIndicesIntoPtr(m_Ptr, submesh, applyBaseVertex, 4,
			                   (IntPtr) outIndices.GetUnsafePtr());
		}
	}
	...
	public NativeArray<T> GetIndexData<T>() where T : struct {
		...
		if (ibSize % stride != 0)
			throw new ArgumentException(
				      $"Type passed to {nameof(GetIndexData)} can't capture the index buffer. Mesh index buffer size is {ibSize} which is not a multiple of type size {stride}");

		var arrSize = ibSize / stride;
		unsafe {
			var array =
				NativeArrayUnsafeUtility.ConvertExistingDataToNativeArray<T>(
					(void *) GetIndexDataPtr(m_Ptr), (int) arrSize, Allocator.None);
			NativeArrayUnsafeUtility.SetAtomicSafetyHandle(ref array, m_Safety);
			return array;
		}
	}
