...
public class BeforeRenderOrderAttribute : Attribute {
  ...
  public BeforeRenderOrderAttribute(int order) { this.order = order; }
}
static class BeforeRenderHelper {
  ...
  static int GetUpdateOrder(UnityAction callback) {
    object[] attributes = callback.Method.GetCustomAttributes(
        typeof(BeforeRenderOrderAttribute), true);
    BeforeRenderOrderAttribute updateOrder =
        (attributes != null && attributes.Length > 0)
            ? attributes[0] as BeforeRenderOrderAttribute
            : null;
  ...
  public static void RegisterCallback(UnityAction callback) {
    ...
    lock(s_OrderBlocks) {
      int i = 0;
      for (; i < s_OrderBlocks.Count && (s_OrderBlocks[i].order <= order);
           i++) {
  ...
  public static void UnregisterCallback(UnityAction callback) {
    ...
    lock(s_OrderBlocks) {
      for (int i = 0;
           i < s_OrderBlocks.Count && (s_OrderBlocks[i].order <= order); i++) {
        if (s_OrderBlocks[i].order == order) {
          OrderBlock element = s_OrderBlocks[i];
          element.callback -= callback;
          s_OrderBlocks[i] = element;
