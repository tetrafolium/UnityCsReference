...
extern public Vector3 localReferencePoint {
	get;
	set;
}

// The size of LOD object in local space
extern public float size {
	get;
	set;
}

// The number of LOD levels
extern public int lodCount {
	[NativeMethod("GetLODCount")] get;
}

// The fade mode
extern public LODFadeMode fadeMode {
	get;
	set;
}

// Is cross-fading animated?
extern public bool animateCrossFading {
	get;
	set;
}
...
// Recalculate the bounding region for the LODGroup (Relatively slow, do not
// call often)
[FreeFunction("UpdateLODGroupBoundingBox",
              HasExplicitThis = true)] extern public void RecalculateBounds();

[FreeFunction("GetLODs_Binding",
              HasExplicitThis = true)] extern public LOD[] GetLODs();
...
// Set the LODs for the LOD group. This will remove any existing LODs
// configured on the LODGroup
[FreeFunction("SetLODs_Binding", HasExplicitThis = true)] extern public void
SetLODs(LOD[] lods);

// Force a LOD level on this LOD group
//
// @param index The LOD level to use. Passing index < 0 will return to
// standard LOD processing
[FreeFunction("ForceLODLevel", HasExplicitThis = true)] extern public void
ForceLOD(int index);

[StaticAccessor("GetLODGroupManager()")] extern public static float
crossFadeAnimationDuration {
	get;
	set;
}

extern internal Vector3 worldReferencePoint {
	get;
}
}
}
