...
private static void InvokeOnTextureLoaded_Internal(
	CustomRenderTexture source) => textureLoaded?.Invoke(source);

public static event Action<CustomRenderTexture> textureUnloaded;

[RequiredByNativeCode]
private static void InvokeOnTextureUnloaded_Internal(
	CustomRenderTexture source) => textureUnloaded?.Invoke(source);

[FreeFunction(
	 Name = "CustomRenderTextureManagerScripting::GetAllCustomRenderTextures",
	 HasExplicitThis = false)]
public extern static void GetAllCustomRenderTextures(
	List<CustomRenderTexture> currentCustomRenderTextures);

public static event Action<CustomRenderTexture, int> updateTriggered;

internal static void InvokeTriggerUpdate(CustomRenderTexture crt,
                                         int updateCount) =>
updateTriggered?.Invoke(crt, updateCount);
...
initializeTriggered?.Invoke(crt);
}
}
