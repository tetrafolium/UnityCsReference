...
public enum KeyCode {
  ...
  // We could expose all 10 joysticks here, but I think that a user would rarely
  // want to explicitly specify an eight or higher joystick (and they still can
  // using the string version of Input.KeyDown). Eight joysticks, however, are a
  // common setup (especially on consoles), and we've had a bug report for only
  // exposing three, so I added eight.
}
}
