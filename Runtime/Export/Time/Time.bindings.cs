...
[StaticAccessor("GetTimeManager()", StaticAccessorType.Dot)]
    ...
    public class Time {
  // The time this frame has started (RO). This is the time in seconds since the
  // start of the game.
  ...
  // The time the latest MonoBehaviour::pref::FixedUpdate has started (RO). This
  ...
  // (like MonoBehaviour's MonoBehaviour::pref::FixedUpdate) are performed.
  public static extern float fixedDeltaTime {
    get;
    set;
  }

  // The maximum time a frame can take. Physics and other fixed frame rate
  // updates (like MonoBehaviour's MonoBehaviour::pref::FixedUpdate)
  ...
  // If /captureDeltaTime/ is set to a value larger than 0, time will advance by
  // that increment.
  ...
  // /captureFramerate/ is a convenience (and backwards compatible) accessor for
  // the reciprocal of /captureDeltaTime/ rounded to the nearest integer.
  public static int captureFramerate {
    get {
      return captureDeltaTime == 0.0f
                 ? 0
                 : (int) Mathf.Round(1.0f / captureDeltaTime);
    }
    set { captureDeltaTime = value == 0 ? 0.0f : 1.0f / value; }
  }

  // Returns true if inside a fixed time step callback such as FixedUpdate,
  // otherwise false.
  public static extern bool inFixedTimeStep {
    [NativeName("IsUsingFixedTimeStep")] get;
  }
}
}
