...
public partial struct Pose : IEquatable<Pose> {
  ...
  public override string ToString() {
    return UnityString.Format("({0}, {1})", position.ToString(),
                              rotation.ToString());
  }

  public string ToString(string format) {
    return UnityString.Format("({0}, {1})", position.ToString(format),
                              rotation.ToString(format));
  }

  public Pose GetTransformedBy(Pose lhs) {
    return new Pose{position = lhs.position + (lhs.rotation * position),
                    rotation = lhs.rotation * rotation};
  }

  public Pose GetTransformedBy(Transform lhs) {
    return new Pose{position = lhs.TransformPoint(position),
                    rotation = lhs.rotation * rotation};
  }
  ...
  public static bool operator ==(Pose a, Pose b) { return a.Equals(b); }
