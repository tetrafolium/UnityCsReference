...
// AtomicSafetyHandle is used by the C# job system to provide validation and
// full safety for read / write permissions to access the buffers represented
// by each handle. Each AtomicSafetyHandle represents a single container.
...
public struct AtomicSafetyHandle {
	...
	PrepareUndisposable(ref AtomicSafetyHandle handle);

	// Switches the AtomicSafetyHandle to the secondary version number.
	[ThreadSafe]
	public static extern void
	UseSecondaryVersion(ref AtomicSafetyHandle handle);

	// Switches the AtomicSafetyHandle to the secondary version number.
	[ThreadSafe]
	public static extern void
	SetAllowSecondaryVersionWriting(AtomicSafetyHandle handle,
	                                bool allowWriting);

	[ThreadSafe]
	public static extern void
	SetBumpSecondaryVersionOnScheduleWrite(AtomicSafetyHandle handle,
	                                       bool value);

	[NativeMethod(IsThreadSafe = true, IsFreeFunction = true)]
	public static extern void
	SetAllowReadOrWriteAccess(AtomicSafetyHandle handle,
	                          bool allowReadWriteAccess);

	[NativeMethod(IsThreadSafe = true, IsFreeFunction = true)]
	public static extern bool
	GetAllowReadOrWriteAccess(AtomicSafetyHandle handle);

	// Performs CheckWriteAndThrow and then bumps the secondary version.
	// This allows for example a NativeArray that becomes invalid if the Length
	// of a List is changed to be invalidated, while the NativeList handle
	// itself remains valid.
	[ThreadSafe(ThrowsException = true)]
	[Conditional("ENABLE_UNITY_COLLECTIONS_CHECKS")]
	public static extern void
	CheckWriteAndBumpSecondaryVersion(AtomicSafetyHandle handle);

	// For debugging purposes in unit tests we need to sometimes just sync
	// all jobs against an handle before shutting down.
	[ThreadSafe]
	public static extern EnforceJobResult
	EnforceAllBufferJobsHaveCompleted(AtomicSafetyHandle handle);
	...
	// But sometimes you just need to delete memory right away, for example
	// Mesh.EndWriteVertices().
	[ThreadSafe]
	public static extern EnforceJobResult
	EnforceAllBufferJobsHaveCompletedAndRelease(AtomicSafetyHandle handle);
	...
	// Same as CheckReadAndThrow but the early out has already been performed in
	// the call site for performance reasons.
	[ThreadSafe(ThrowsException = true)]
	[Conditional("ENABLE_UNITY_COLLECTIONS_CHECKS")]
	internal static extern void
	CheckReadAndThrowNoEarlyOut(AtomicSafetyHandle handle);

	// Same as CheckWriteAndThrow but the early out has already been performed
	// in the call site for performance reasons.
	[ThreadSafe(ThrowsException = true)]
	[Conditional("ENABLE_UNITY_COLLECTIONS_CHECKS")]
	internal static extern void
	CheckWriteAndThrowNoEarlyOut(AtomicSafetyHandle handle);

	// Checks if the handle can be deallocated.
	// If not (already destroyed, job currently accessing the data) throws an
	// exception.
	[ThreadSafe(ThrowsException = true)]
	[Conditional("ENABLE_UNITY_COLLECTIONS_CHECKS")]
	public static extern void
	CheckDeallocateAndThrow(AtomicSafetyHandle handle);

	[ThreadSafe(ThrowsException = true)]
	[Conditional("ENABLE_UNITY_COLLECTIONS_CHECKS")]
	public static extern void
	CheckGetSecondaryDataPointerAndThrow(AtomicSafetyHandle handle);

	[ThreadSafe]
	public static unsafe extern int GetReaderArray(AtomicSafetyHandle handle,
	                                               int maxCount, IntPtr output);

	[ThreadSafe]
	public static extern JobHandle GetWriter(AtomicSafetyHandle handle);

	// Checks if the handle can be read from
	// If not (already destroyed, job currently writing to the data) throws an
	// exception.
	[Conditional("ENABLE_UNITY_COLLECTIONS_CHECKS")]
	...
	// If not (already destroyed, job currently reading or writing to the data)
	// throws an exception.
	[Conditional("ENABLE_UNITY_COLLECTIONS_CHECKS")]
	...
	public static unsafe void CheckExistsAndThrow(AtomicSafetyHandle handle) {
		...
		if (handle.version != ((*versionPtr) & ReadWriteDisposeCheck))
			throw new InvalidOperationException(
				      "The NativeArray has been deallocated, it is not allowed to access it");
	}

	[ThreadSafe]
	public static extern string GetReaderName(AtomicSafetyHandle handle,
	                                          int readerIndex);
	...
	[ThreadSafe]
	public static unsafe extern int NewStaticSafetyId(byte *ownerTypeNameBytes,
	                                                  int byteCount);
	...
	SetCustomErrorMessage(int staticSafetyId, AtomicSafetyErrorType errorType,
	                      byte *messageBytes, int byteCount);
	[Conditional("ENABLE_UNITY_COLLECTIONS_CHECKS")]
	public static unsafe void SetStaticSafetyId(ref AtomicSafetyHandle handle,
	                                            int staticSafetyId) {
