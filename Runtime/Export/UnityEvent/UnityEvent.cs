...
namespace UnityEngine.Events {
  ...
  internal class UnityEventTools {
    // Fix for assembly type name containing version / culture. We don't care
    // about this for UI. we need to fix this here, because there is old data in
    // existing projects. Typically, we're looking for .net Assembly Qualified
    // Type Names and stripping everything after '<namespaces>.<typename>,
    // <assemblyname>' Example: System.String, mscorlib, Version=4.0.0.0,
    ...
    internal static string TidyAssemblyTypeName(string assemblyTypeName) {
      ...
      // This way, when a type gets moved to a differnet module, previously
      // serialized UnityEvents still work.
  ...
  class ArgumentCache : ISerializationCallbackReceiver {
    ...
    public Object unityObjectArgument {
      ...
      set {
        ...
        m_ObjectArgumentAssemblyTypeName =
            value != null ? value.GetType().AssemblyQualifiedName : string.Empty;
      }
    }
    ...
    public void OnBeforeSerialize() {
      m_ObjectArgumentAssemblyTypeName = UnityEventTools.TidyAssemblyTypeName(
          m_ObjectArgumentAssemblyTypeName);
    }
    ...
    protected static void ThrowOnInvalidArg<T>(object arg) {
      if (arg != null && !(arg is T))
        throw new ArgumentException(UnityString.Format(
            "Passed argument 'args[0]' is of the wrong type. Type:{0} Expected:{1}",
  ...
  class InvokableCall : BaseInvokableCall {
    ...
    public InvokableCall(object target, MethodInfo theFunction)
      ...
      Delegate += (UnityAction) System.Delegate.CreateDelegate(
          typeof(UnityAction), target, theFunction);
    }
    ...
    public override bool Find(object targetObj, MethodInfo method) {
      // Case 827748: You can't compare Delegate.GetMethodInfo() == method,
      // because sometimes it will not work, that's why we're using Equals
      // instead, because it will compare that actual method inside.
      //              Comment from Microsoft:
      //              Desktop behavior regarding identity has never really been
      //              guaranteed. The desktop aggressively caches and reuses
      //              MethodInfo objects so identity checks often work by
      //              accident. .Net Native doesn’t guarantee identity and
      //              caches a lot less
  ...
  class InvokableCall<T1> : BaseInvokableCall {
    ...
    public InvokableCall(object target, MethodInfo theFunction)
        : base(target, theFunction) {
      Delegate += (UnityAction<T1>) System.Delegate.CreateDelegate(
          typeof(UnityAction<T1>), target, theFunction);
    }
    ...
    public override void Invoke(object[] args) {
      if (args.Length != 1)
        throw new ArgumentException(
            "Passed argument 'args' is invalid size. Expected size is 1");
  ...
  class InvokableCall<T1, T2> : BaseInvokableCall {
    ...
    public InvokableCall(object target, MethodInfo theFunction)
        : base(target, theFunction) {
      Delegate = (UnityAction<T1, T2>) System.Delegate.CreateDelegate(
          typeof(UnityAction<T1, T2>), target, theFunction);
    }
  ...
  class InvokableCall<T1, T2, T3> : BaseInvokableCall {
    ...
    public InvokableCall(object target, MethodInfo theFunction)
        : base(target, theFunction) {
      Delegate = (UnityAction<T1, T2, T3>) System.Delegate.CreateDelegate(
          typeof(UnityAction<T1, T2, T3>), target, theFunction);
    }
  ...
  class InvokableCall<T1, T2, T3, T4> : BaseInvokableCall {
    ...
    public InvokableCall(object target, MethodInfo theFunction)
        : base(target, theFunction) {
      Delegate = (UnityAction<T1, T2, T3, T4>) System.Delegate.CreateDelegate(
          typeof(UnityAction<T1, T2, T3, T4>), target, theFunction);
    }
  ...
  class CachedInvokableCall<T> : InvokableCall<T> {
    ...
    public CachedInvokableCall(Object target, MethodInfo theFunction,
                               T argument)
        : base(target, theFunction) {
      m_Arg1 = argument;
    }
  ...
  class PersistentCall : ISerializationCallbackReceiver {
    // keep the layout of this class in sync with MonoPersistentCall in
    // PersistentCallCollection.cpp
    ...
    public string targetAssemblyTypeName {
      get {
        // Reconstruct TargetAssemblyTypeName from target if it's not present,
        // for ex., when upgrading project
        if (string.IsNullOrEmpty(m_TargetAssemblyTypeName) &&
            m_Target != null) {
          m_TargetAssemblyTypeName = UnityEventTools.TidyAssemblyTypeName(
              m_Target.GetType().AssemblyQualifiedName);
        }
    ...
    public bool IsValid() {
      // We need to use the same logic found in PersistentCallCollection.cpp,
      // IsPersistentCallValid
      return !String.IsNullOrEmpty(targetAssemblyTypeName) &&
             !String.IsNullOrEmpty(methodName);
    }

    public BaseInvokableCall GetRuntimeCall(UnityEventBase theEvent) {
      if (m_CallState == UnityEventCallState.RuntimeOnly &&
      ...
      case PersistentListenerMode.Float:
        return new CachedInvokableCall<float>(targetObject, method,
                                              m_Arguments.floatArgument);
      case PersistentListenerMode.Int:
        return new CachedInvokableCall<int>(targetObject, method,
                                            m_Arguments.intArgument);
      case PersistentListenerMode.String:
        return new CachedInvokableCall<string>(targetObject, method,
                                               m_Arguments.stringArgument);
      case PersistentListenerMode.Bool:
        return new CachedInvokableCall<bool>(targetObject, method,
                                             m_Arguments.boolArgument);
    ...
    private static BaseInvokableCall
    GetObjectCall(Object target, MethodInfo method, ArgumentCache arguments) {
      var type = typeof(Object);
      if (!string.IsNullOrEmpty(arguments.unityObjectArgumentAssemblyTypeName))
        type =
            Type.GetType(arguments.unityObjectArgumentAssemblyTypeName, false)
            ?? typeof(Object);

      var generic = typeof(CachedInvokableCall<>);
      var specific = generic.MakeGenericType(type);
      var ci = specific.GetConstructor(
          new[]{typeof(Object), typeof(MethodInfo), type});

      var castedObject = arguments.unityObjectArgument;
      if (castedObject != null &&
          !type.IsAssignableFrom(castedObject.GetType()))
        castedObject = null;

      // need to pass explicit null here!
      return ci.Invoke(new object[]{target, method, castedObject})
          as BaseInvokableCall;
    }

    public void RegisterPersistentListener(Object ttarget, Type targetType,
                                           string mmethodName) {
      m_Target = ttarget;
      m_TargetAssemblyTypeName = UnityEventTools.TidyAssemblyTypeName(
    ...
    public void OnBeforeSerialize() {
      m_TargetAssemblyTypeName =
          UnityEventTools.TidyAssemblyTypeName(m_TargetAssemblyTypeName);
    }
  ...
  internal class PersistentCallGroup {
    ...
    public PersistentCallGroup() { m_Calls = new List<PersistentCall>(); }

    public int Count {
      get { return m_Calls.Count; }
    }

    public PersistentCall GetListener(int index) { return m_Calls[index]; }

    public IEnumerable<PersistentCall> GetListeners() { return m_Calls; }

    public void AddListener() { m_Calls.Add(new PersistentCall()); }

    public void AddListener(PersistentCall call) { m_Calls.Add(call); }

    public void RemoveListener(int index) { m_Calls.RemoveAt(index); }

    public void Clear() { m_Calls.Clear(); }

    public void RegisterEventPersistentListener(int index, Object targetObj,
                                                Type targetObjType,
                                                string methodName) {
    ...
    public void RegisterVoidPersistentListener(int index, Object targetObj,
                                               Type targetObjType,
                                               string methodName) {
      var listener = GetListener(index);
      listener.RegisterPersistentListener(targetObj, targetObjType, methodName);
    ...
    public void RegisterObjectPersistentListener(int index, Object targetObj,
                                                 Type targetObjType,
                                                 Object argument,
                                                 string methodName) {
      var listener = GetListener(index);
    ...
    public void RegisterIntPersistentListener(int index, Object targetObj,
                                              Type targetObjType, int argument,
                                              string methodName) {
      var listener = GetListener(index);
      listener.RegisterPersistentListener(targetObj, targetObjType, methodName);
    ...
    public void RegisterFloatPersistentListener(int index, Object targetObj,
                                                Type targetObjType,
                                                float argument,
                                                string methodName) {
      var listener = GetListener(index);
    ...
    public void RegisterStringPersistentListener(int index, Object targetObj,
                                                 Type targetObjType,
                                                 string argument,
                                                 string methodName) {
      var listener = GetListener(index);
    ...
    public void RegisterBoolPersistentListener(int index, Object targetObj,
                                               Type targetObjType,
                                               bool argument,
                                               string methodName) {
      var listener = GetListener(index);
    ...
    public void RemoveListeners(Object target, string methodName) {
      ...
      for (int index = 0; index < m_Calls.Count; index++) {
        if (m_Calls[index].target == target &&
            m_Calls[index].methodName == methodName)
          toRemove.Add(m_Calls[index]);
      }
      m_Calls.RemoveAll(toRemove.Contains);
    }

    public void Initialize(InvokableCallList invokableList,
  ...
  class InvokableCallList {
    ...
    public void RemoveListener(object targetObj, MethodInfo method) {
      ...
      for (int index = 0; index < m_RuntimeCalls.Count; index++) {
        if (m_RuntimeCalls [index]
                .Find(targetObj, method))
  ...
  public abstract class UnityEventBase : ISerializationCallbackReceiver {
    ...
    // Dirtying can happen outside of MainThread, but we need to rebuild on the
    // MainThread.
    ...
    protected abstract MethodInfo FindMethod_Impl(string name,
                                                  Type targetObjType);
    internal abstract BaseInvokableCall GetDelegate(object target,
                                                    MethodInfo theFunction);

    internal MethodInfo FindMethod(PersistentCall call) {
      var type = typeof(Object);
      if (!string.IsNullOrEmpty(
              call.arguments.unityObjectArgumentAssemblyTypeName))
        type = Type.GetType(call.arguments.unityObjectArgumentAssemblyTypeName,
                            false)
            ?? typeof(Object);

      var targetType = call.target != null ? call.target.GetType()
          : Type.GetType(call.targetAssemblyTypeName, false);
      return FindMethod(call.methodName, targetType, call.mode, type);
    }

    internal MethodInfo FindMethod(string name, Type listenerType,
      ...
      case PersistentListenerMode.Object:
        return GetValidMethodInfo(listenerType, name,
                                  new[]{argumentType ?? typeof(Object)});
      default:
        return null;
      }
    }

    public int GetPersistentEventCount() { return m_PersistentCalls.Count; }
    ...
    public void SetPersistentListenerState(int index,
    ...
    public static MethodInfo GetValidMethodInfo(object obj, string functionName,
                                                Type[] argumentTypes) {
      return GetValidMethodInfo(obj.GetType(), functionName, argumentTypes);
    }

    public static MethodInfo GetValidMethodInfo(Type objectType,
                                                string functionName,
                                                Type[] argumentTypes) {
      while (objectType != typeof(object) &&objectType != null) {
        ...
        if (method != null) {
          // We need to make sure the Arguments are sane. When using the
          // Type.DefaultBinder like we are above, it is possible to receive a
          // method that takes a System.Object enve though we requested a float,
          // int or bool. This can be an issue when the user changes the
          // signature of a function that he had already set up via inspector.
          // When changing a float parameter to a System.Object the getMethod
          // would still bind to the cahnged version, but the
          // PersistentListenerMode would still be kept as Float.
          // TODO: Should we allow anything else besides Primitive types and
    ...
    protected bool ValidateRegistration(MethodInfo method, object targetObj,
                                        PersistentListenerMode mode) {
    ...
    protected bool ValidateRegistration(MethodInfo method, object targetObj,
      ...
      if (method == null)
        throw new ArgumentNullException(
            "method", UnityString.Format(
                          "Can not register null method on {0} for callback!",
                          targetObj));

      if (method.DeclaringType == null) {
        throw new NullReferenceException(UnityString.Format(
      ...
      if (!method.IsStatic) {
        ...
        if (obj == null || obj.GetInstanceID() == 0) {
          throw new ArgumentException(UnityString.Format(
              ...
              method.Name, targetObj, targetObj == null ? "null"
              : targetObj.GetType().ToString()));
        }
        ...
        if (!method.DeclaringType.IsAssignableFrom(targetType))
          throw new ArgumentException(UnityString.Format(
              ...
              method.Name, method.DeclaringType.Name, obj.GetType().Name));
      } else {
        targetType = method.DeclaringType;
      }

      if (FindMethod(method.Name, targetType, mode, argumentType) == null) {
        Debug.LogWarning(UnityString.Format(
            "Could not register listener {0}.{1} on {2} the method could not be found.",
            targetObj, method, GetType()));
        return false;
      }
      return true;
    }

    internal void AddPersistentListener() { m_PersistentCalls.AddListener(); }

    protected void RegisterPersistentListener(int index, object targetObj,
                                              MethodInfo method) {
    ...
    protected void RegisterPersistentListener(int index, object targetObj,
                                              Type targetObjType,
                                              MethodInfo method) {
      if (!ValidateRegistration(method, targetObj,
                                PersistentListenerMode.EventDefined))
        return;

      m_PersistentCalls.RegisterEventPersistentListener(
          index, targetObj as Object, targetObjType, method.Name);
      DirtyPersistentCalls();
    }
    ...
    internal void RegisterVoidPersistentListener(int index, UnityAction call) {
      ...
      if (!ValidateRegistration(call.Method, call.Target,
                                PersistentListenerMode.Void))
        return;

      m_PersistentCalls.RegisterVoidPersistentListener(
          index, call.Target as Object, call.Method.DeclaringType,
          call.Method.Name);
      DirtyPersistentCalls();
    }

    internal void
    RegisterVoidPersistentListenerWithoutValidation(int index, Object target,
                                                    string methodName) {
      RegisterVoidPersistentListenerWithoutValidation(
          index, target, target.GetType(), methodName);
    }

    internal void RegisterVoidPersistentListenerWithoutValidation(
        int index, Object target, Type targetType, string methodName) {
      m_PersistentCalls.RegisterVoidPersistentListener(index, target,
                                                       targetType, methodName);
      DirtyPersistentCalls();
    }

    internal void AddIntPersistentListener(UnityAction<int> call,
                                           int argument) {
      var count = GetPersistentEventCount();
      AddPersistentListener();
    ...
    internal void RegisterIntPersistentListener(int index,
      ...
      m_PersistentCalls.RegisterIntPersistentListener(
          index, call.Target as Object, call.Method.DeclaringType, argument,
          call.Method.Name);
      DirtyPersistentCalls();
    }

    internal void AddFloatPersistentListener(UnityAction<float> call,
                                             float argument) {
      var count = GetPersistentEventCount();
      AddPersistentListener();
    ...
    internal void RegisterFloatPersistentListener(int index,
      ...
      m_PersistentCalls.RegisterFloatPersistentListener(
          index, call.Target as Object, call.Method.DeclaringType, argument,
          call.Method.Name);
      DirtyPersistentCalls();
    }

    internal void AddBoolPersistentListener(UnityAction<bool> call,
                                            bool argument) {
      var count = GetPersistentEventCount();
      AddPersistentListener();
    ...
    internal void RegisterBoolPersistentListener(int index,
      ...
      m_PersistentCalls.RegisterBoolPersistentListener(
          index, call.Target as Object, call.Method.DeclaringType, argument,
          call.Method.Name);
      DirtyPersistentCalls();
    }

    internal void AddStringPersistentListener(UnityAction<string> call,
                                              string argument) {
      var count = GetPersistentEventCount();
      AddPersistentListener();
    ...
    internal void RegisterStringPersistentListener(int index,
      ...
      m_PersistentCalls.RegisterStringPersistentListener(
          index, call.Target as Object, call.Method.DeclaringType, argument,
          call.Method.Name);
      DirtyPersistentCalls();
    }

    internal void AddObjectPersistentListener<T>(UnityAction<T> call,
                                                 T argument) where T : Object {
      var count = GetPersistentEventCount();
      AddPersistentListener();
      RegisterObjectPersistentListener(count, call, argument);
    }

    internal void RegisterObjectPersistentListener<T>(int index,
                                                      UnityAction<T> call,
                                                      T argument) where T
        : Object {
      if (call == null)
        throw new ArgumentNullException(
            "call", "Registering a Listener requires a non null call");

      if (!ValidateRegistration(call.Method, call.Target,
                                PersistentListenerMode.Object,
                                argument == null ? typeof(Object)
                                : argument.GetType()))
        return;

      m_PersistentCalls.RegisterObjectPersistentListener(
          index, call.Target as Object, call.Method.DeclaringType, argument,
          call.Method.Name);
      DirtyPersistentCalls();
    }
