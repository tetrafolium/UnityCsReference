...
namespace UnityEngine.Windows.Speech {
  ...
  public enum DictationTopicConstraint { WebSearch, Form, Dictation }
  ...
  public struct PhraseRecognizedEventArgs {
    ...
    internal PhraseRecognizedEventArgs(string text, ConfidenceLevel confidence,
  ...
  static partial class PhraseRecognitionSystem {
    ...
    private static void
    PhraseRecognitionSystem_InvokeErrorEvent(SpeechError errorCode) {
      var onError = OnError;
      if (onError != null)
        onError(errorCode);
    }
    ...
    [RequiredByNativeCode]
    private static void PhraseRecognitionSystem_InvokeStatusChangedEvent(
  ...
  public abstract partial class PhraseRecognizer : IDisposable {
    ...
    PhraseRecognizedDelegate(PhraseRecognizedEventArgs args);
    ...
    [RequiredByNativeCode]
    private void InvokePhraseRecognizedEvent(string text,
      ...
      if (onPhraseRecognized != null)
        onPhraseRecognized(new PhraseRecognizedEventArgs(
            text, confidence, semanticMeanings,
            DateTime.FromFileTime(phraseStartFileTime),
            TimeSpan.FromTicks(phraseDurationTicks)));
    }

    [RequiredByNativeCode]
    private static unsafe SemanticMeaning[] MarshalSemanticMeaning(
        IntPtr keys, IntPtr values, IntPtr valueSizes, int valueCount) {
      ...
      for (int i = 0; i < valueCount; i++) {
        ...
        var semanticMeaning =
            new SemanticMeaning{key = new string(((char **) keys)[i]),
                                values = new string[ithValueSize]};

        for (int j = 0; j < ithValueSize; j++)
          semanticMeaning.values[j] =
              new string(((char **) values)[valueIndex + j]);
  ...
  sealed public class KeywordRecognizer : PhraseRecognizer {
    ...
    public KeywordRecognizer(string[] keywords,
      ...
      if (keywords.Length == 0)
        throw new ArgumentException("At least one keyword must be specified.",
                                    "keywords");
      ...
      for (int i = 0; i < keywordsLength; i++) {
        if (keywords[i] == null)
          throw new ArgumentNullException(
              string.Format("Keyword at index {0} is null.", i));
      }
  ...
  sealed public class GrammarRecognizer : PhraseRecognizer {
    ...
    public GrammarRecognizer(string grammarFilePath,
      ...
      m_Recognizer =
          CreateFromGrammarFile(this, grammarFilePath, minimumConfidence);
    }
  }
  ...
  sealed public partial class DictationRecognizer : IDisposable {
    ...
    public delegate void DictationResultDelegate(string text,
                                                 ConfidenceLevel confidence);
    public delegate void
    DictationCompletedDelegate(DictationCompletionCause cause);
    ...
    public SpeechSystemStatus Status {
      get {
        return m_Recognizer !=
               IntPtr.Zero ? GetStatus(m_Recognizer)
    ...
    public DictationRecognizer(ConfidenceLevel minimumConfidence,
    ...
    [RequiredByNativeCode]
    private void
    DictationRecognizer_InvokeHypothesisGeneratedEvent(string keyword) {
    ...
    [RequiredByNativeCode]
    private void DictationRecognizer_InvokeResultGeneratedEvent(
        string keyword, ConfidenceLevel minimumConfidence) {
    ...
    [RequiredByNativeCode]
    private void
    DictationRecognizer_InvokeCompletedEvent(DictationCompletionCause cause) {
    ...
    [RequiredByNativeCode]
    private void DictationRecognizer_InvokeErrorEvent(string error,
                                                      int hresult) {
