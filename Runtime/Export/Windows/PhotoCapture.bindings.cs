...
namespace UnityEngine.Windows.WebCam {
  ...
  [StructLayout(LayoutKind.Sequential)] // needed for IntPtr binding classes
    ...
    static private PhotoCaptureResult
    ...
    OnCaptureResourceCreatedCallback(PhotoCapture captureObject);
    ...
    public delegate void
    OnCapturedToMemoryCallback(PhotoCaptureResult result,
    ...
    public static void
    CreateAsync(OnCaptureResourceCreatedCallback onCreatedCallback) {
      if (onCreatedCallback == null) {
        throw new ArgumentNullException("onCreatedCallback");
      }
    ...
    Instantiate_Internal(bool showHolograms,
                         OnCaptureResourceCreatedCallback onCreatedCallback);

    //-----------------------------------------------------------------
    [RequiredByNativeCode]
    private static void
    InvokeOnCreatedResourceDelegate(OnCaptureResourceCreatedCallback callback,
                                    IntPtr nativePtr) {
    ...
    //-----------------------------------------------------------------
    public void
    StartPhotoModeAsync(CameraParameters setupParams,
    ...
    private extern void StartPhotoMode_Internal(
    ...
    [RequiredByNativeCode]
    private static void
    InvokeOnPhotoModeStartedDelegate(OnPhotoModeStartedCallback callback,
                                     long hResult) {
    ...
    public extern void
    StopPhotoModeAsync(OnPhotoModeStoppedCallback onPhotoModeStoppedCallback);

    //-----------------------------------------------------------------
    [RequiredByNativeCode]
    private static void
    InvokeOnPhotoModeStoppedDelegate(OnPhotoModeStoppedCallback callback,
                                     long hResult) {
      callback(MakeCaptureResult(hResult));
    }
    ...
    //-----------------------------------------------------------------
    public void
    TakePhotoAsync(string filename,
    ...
    private extern void CapturePhotoToDisk_Internal(
        string filename, PhotoCaptureFileOutputFormat fileOutputFormat,
        OnCapturedToDiskCallback onCapturedPhotoToDiskCallback);

    //-----------------------------------------------------------------
    [RequiredByNativeCode]
    private static void
    InvokeOnCapturedPhotoToDiskDelegate(OnCapturedToDiskCallback callback,
                                        long hResult) {
      callback(MakeCaptureResult(hResult));
    }
    ...
    //-----------------------------------------------------------------
    public void
    TakePhotoAsync(OnCapturedToMemoryCallback onCapturedPhotoToMemoryCallback) {
    ...
    private extern void CapturePhotoToMemory_Internal(
        OnCapturedToMemoryCallback onCapturedPhotoToMemoryCallback);

    //-----------------------------------------------------------------
    [RequiredByNativeCode]
    private static void
    InvokeOnCapturedPhotoToMemoryDelegate(OnCapturedToMemoryCallback callback,
                                          long hResult,
...
#pragma warning disable 169
    ...
    public bool TryGetProjectionMatrix(float nearClipPlane, float farClipPlane,
      ...
      if (hasLocationData) {
        ...
        // in the same way that our Camera component does in its
    ...
    public void UploadImageDataToTexture(Texture2D targetTexture) {
      ...
      if (pixelFormat != CapturePixelFormat.BGRA32) {
        throw new ArgumentException(
            "Uploading PhotoCaptureFrame to a texture is only supported with BGRA32 CameraFrameFormat!");
      }
    ...
    private extern void
    UploadImageDataToTexture_Internal(Texture2D targetTexture);
    ...
    internal extern void
        CopyRawImageDataIntoBuffer_Internal([ Out ] byte[] byteArray);

    //-----------------------------------------------------------------
    internal PhotoCaptureFrame(IntPtr nativePtr) {
      m_NativePtr = nativePtr;
      dataLength = GetDataLength(); // Cache DataLength on managed side because
                                    // it will be accessed frequently
