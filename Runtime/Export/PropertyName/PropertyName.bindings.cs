...
class PropertyNameUtils {
  [FreeFunction]
  public extern static PropertyName
  PropertyNameFromString([ Unmarshalled ] string name);
  [FreeFunction]
  ...
  /// Returns 0 if the id is unregistered or it has been mapped to only a single
  /// string. Otherwise returns the number of unique strings mapped to the given
  /// id.
  /// </summary>
  [FreeFunction]
