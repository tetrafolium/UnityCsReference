...
public struct PropertyName : IEquatable<PropertyName> {
  ...
  public bool Equals(PropertyName other) { return this == other; }
  ...
  public override string ToString() {
    ...
    var msg = string.Format("{0}:{1}",
                            PropertyNameUtils.StringFromPropertyName(this), id);
    if (conflictCount > 0) {
      ...
      for (int i = 0; i < conflictCount; i++) {
        ...
        sb.AppendFormat("\"{0}\"",
                        PropertyNameUtils.StringFromPropertyName(
                            new PropertyName(id){conflictIndex = i}));
      }
      msg = sb.ToString();
    }
    return msg;
