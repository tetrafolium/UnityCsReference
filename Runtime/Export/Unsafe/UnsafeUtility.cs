...
private static string GetReasonForTypeNonBlittableImpl(Type t,
                                                       string name) {
	if (!t.IsValueType)
	...
	foreach (FieldInfo f in t.GetFields(BindingFlags.Instance |
		...
		if (!IsBlittableValueType(f.FieldType))
			ret += GetReasonForTypeNonBlittableImpl(
				f.FieldType, String.Format("{0}.{1}", name, f.Name));
	}
	return ret;
}

// while it would make sense to have functions like
// ThrowIfArgumentIsNonBlittable currently we insist on including part of
// call stack into exception message in these cases e.g. "T used in
// NativeArray<T> must be blittable" due to that we will need to pass
// message string to this function
//   but most of the time we will be creating it using string.Format and it
//   will happen on every check instead of "only if we fail check for
//   is-blittable"
// thats why we provide the means to implement this pattern on your code
...
internal static string GetReasonForGenericListNonBlittable<T>() where T
: struct {
	Type t = typeof(T);
	return GetReasonForTypeNonBlittableImpl(t, t.Name);
}
...
internal static string GetReasonForValueTypeNonBlittable<T>() where T
: struct {
	Type t = typeof(T);
	return GetReasonForTypeNonBlittableImpl(t, t.Name);
}
...
public static bool IsValidNativeContainerElementType<T>() {
	...
	if (value == -1) // most common case
		return false;
	if (value == 0)
		IsValidNativeContainerElementTypeCache<T>.value = value =
			IsValidNativeContainerElementType(typeof(T)) ? 1 : -1;
	return value == 1;
}
