...
internal sealed class UnitySynchronizationContext : SynchronizationContext {
  ...
  private readonly List<WorkRequest> m_CurrentFrameWork =
      new List<WorkRequest>(kAwqInitialCapacity);
  ...
  private UnitySynchronizationContext(List<WorkRequest> queue,
                                      int mainThreadID) {
    m_AsyncWorkQueue = queue;
    m_MainThreadID = mainThreadID;
  }

  // Send will process the call synchronously. If the call is processed on the
  // main thread, we'll invoke it directly here. If the call is processed on
  // another thread it will be queued up like POST to be executed on the main
  // thread and it will wait. Once the main thread processes the work we can
  // continue
  ...
  // Post will add the call to a task list to be executed later on the main
  // thread then work will continue asynchronously
  ...
  // CreateCopy returns a new UnitySynchronizationContext object, but the queue
  // is still shared with the original
  ...
  private void Exec() {
    ...
    // When you invoke work, remove it from the list to stop it being triggered
    // again (case 1213602)
  ...
  // SynchronizationContext must be set before any user code is executed. This
  // is done on Initial domain load and domain reload at MonoManager
  // ReloadAssembly
  [RequiredByNativeCode]
  private static void InitializeSynchronizationContext() {
    SynchronizationContext.SetSynchronizationContext(
        new UnitySynchronizationContext(
            System.Threading.Thread.CurrentThread.ManagedThreadId));
  }

  // All requests must be processed on the main thread where the full Unity API
  // is available See ScriptRunDelayedTasks in PlayerLoopCallbacks.h
  [RequiredByNativeCode]
