...
internal enum CreateOptions { None = 0, CreateNativeObject = 1 }

// The text contents of the .txt file as a string. (RO)
public string text {
	get { return DecodeString(bytes); }
}
...
internal static readonly Encoding targetEncoding = Encoding.GetEncoding(
	Encoding.UTF8.CodePage, new EncoderReplacementFallback("�"),
	...
	for (int i = 0; i < encodingLookupLength; i++) {
		...
		if (bytes.Length >= preambleLength) {
			...
			try {
				...
				string text = tempEncoding.GetString(bytes, preambleLength,
