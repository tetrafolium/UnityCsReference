...
public class MonoBehaviour : Behaviour {
  public MonoBehaviour() { ConstructorCheck(this); }

  // Is any invoke pending on this MonoBehaviour?
  public bool IsInvoking() { return Internal_IsInvokingAll(this); }

  public void CancelInvoke() { Internal_CancelInvokeAll(this); }
  ...
  public void InvokeRepeating(string methodName, float time, float repeatRate) {
    if (repeatRate <= 0.00001f && repeatRate != 0.0f)
      throw new UnityException(
          "Invoke repeat rate has to be larger than 0.00001F)");
  ...
  public Coroutine StartCoroutine(string methodName,
    ...
    if (!IsObjectMonoBehaviour(this))
      throw new ArgumentException(
          "Coroutines can only be stopped on a MonoBehaviour");
  ...
  //*undocumented*
  [Obsolete(
      "StartCoroutine_Auto has been deprecated. Use StartCoroutine instead (UnityUpgradable) -> StartCoroutine([mscorlib] System.Collections.IEnumerator)",
      false)]
  ...
  // Logs message to the Unity Console. This function is identical to
  // [[Debug.Log]].
  public static void print(object message) { Debug.Log(message); }

  [NativeMethod(IsThreadSafe = true)] extern static void
  ConstructorCheck([ Writable ] Object self);

  [FreeFunction("CancelInvoke")] extern static void
  Internal_CancelInvokeAll(MonoBehaviour self);

  [FreeFunction("IsInvoking")] extern static bool
  Internal_IsInvokingAll(MonoBehaviour self);

  [FreeFunction] extern static void InvokeDelayed(MonoBehaviour self,
                                                  string methodName, float time,
                                                  float repeatRate);

  [FreeFunction] extern static void CancelInvoke(MonoBehaviour self,
                                                 string methodName);

  [FreeFunction] extern static bool IsInvoking(MonoBehaviour self,
                                               string methodName);

  [FreeFunction] extern static bool IsObjectMonoBehaviour(Object obj);
