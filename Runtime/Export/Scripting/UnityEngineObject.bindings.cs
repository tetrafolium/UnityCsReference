...
public enum HideFlags {
  ...
  // The object will not appear in the hierarchy and will not show up in the
  // project view if it is stored in an asset.
  HideInHierarchy = 1,
...
#pragma warning restore 649
  ...
  public unsafe int GetInstanceID() {
    // Because in the player we dissalow calling GetInstanceID() on a
    // non-mainthread, we're also doing this in the editor, so people notice this
    // problem early. even though technically in the editor, it is a threadsafe
    // operation.
  ...
  public override int GetHashCode() {
    // in the editor, we store the m_InstanceID in the c# objects. It's actually
    // possible to have multiple c# objects pointing to the same c++ object in
    // some edge cases, and in those cases we'd like GetHashCode() and Equals()
    // to treat these objects as equals.
    return m_InstanceID;
  }

  public override bool Equals(object other) {
    Object otherAsObject = other as Object;
    // A UnityEngine.Object can only be equal to another UnityEngine.Object - or
    // null if it has been destroyed. Make sure other is a UnityEngine.Object if
    // "as Object" fails. The explicit "is" check is required since the ==
    // operator in this class treats destroyed objects as equal to null
  ...
  static bool IsNativeObjectAlive(UnityEngine.Object o) {
    ...
    // instanceid. in this case, we want the old c# wrapper to still "work". we
    // only support this behaviour in the editor, even though there are some
    // cases in the player where this could happen too. (when unloading things
    // from assetbundles) supporting this makes all operator== slow though, so we
    // decided to not support it in the player.
    //
    // we have an exception for assets that "are" a c# object, like a
    // MonoBehaviour in a prefab, and a ScriptableObject. in this case, the asset
    // "is" the c# object,  and you cannot actually pretend the old wrapper
    // points to the new c# object. this is why we make an exception in the
    // operator== for this case. If we had a c# wrapper to a persistent
    // monobehaviour, and that one gets destroyed, and placed back with the same
    // instanceID,  we still will say that the old c# object is null.
  ...
  System.IntPtr GetCachedPtr() { return m_CachedPtr; }
  ...
  public static Object Instantiate(Object original, Vector3 position,
    ...
    if (original is ScriptableObject)
      throw new ArgumentException(
          "Cannot instantiate a ScriptableObject with a position and rotation");
  ...
  public static Object Instantiate(Object original, Vector3 position,
    ...
    var obj = Internal_InstantiateSingleWithParent(original, parent, position,
                                                   rotation);
  ...
  public static Object Instantiate(Object original, Transform parent,
    ...
    var obj = Internal_CloneSingleWithParent(original, parent,
  ...
  public extern static void Destroy(Object obj,
                                    [ uei.DefaultValue("0.0F") ] float t);
  ...
  // Destroys the object /obj/ immediately. It is strongly recommended to use
  // Destroy instead.
  [NativeMethod(Name = "Scripting::DestroyObjectFromScriptingImmediate",
                IsFreeFunction = true, ThrowsException = true)]
  public extern static void
  DestroyImmediate(Object obj,
                   [ uei.DefaultValue("false") ] bool allowDestroyingAssets);
  ...
  // We cannot properly deprecate this in C# right now, since the optional
  // parameter creates another method calling this, which creates compiler
  // warnings when deprecated.
  ...
  public static T[] FindObjectsOfType<T>(bool includeInactive) where T
      : Object {
    return Resources.ConvertObjects<T>(
        FindObjectsOfType(typeof(T), includeInactive));
  }
  ...
  [NativeMethod(Name = "Object::GetOffsetOfInstanceIdMember",
                IsFreeFunction = true, IsThreadSafe = true)] extern static int
  GetOffsetOfInstanceIDInCPlusPlusObject();

  [NativeMethod(Name = "CurrentThreadIsMainThread", IsFreeFunction = true,
                IsThreadSafe = true)] extern static bool
  CurrentThreadIsMainThread();

  [FreeFunction("CloneObject")] extern static Object
  Internal_CloneSingle(Object data);

  [FreeFunction("CloneObject")] extern static Object
  Internal_CloneSingleWithParent(Object data, Transform parent,
                                 bool worldPositionStays);

  [FreeFunction("InstantiateObject")] extern static Object
  Internal_InstantiateSingle(Object data, Vector3 pos, Quaternion rot);

  [FreeFunction("InstantiateObject")] extern static Object
  Internal_InstantiateSingleWithParent(Object data, Transform parent,
                                       Vector3 pos, Quaternion rot);

  [FreeFunction("UnityEngineObjectBindings::ToString")] extern static string
  ToString(Object obj);

  [FreeFunction("UnityEngineObjectBindings::GetName")] extern static string
  GetName(Object obj);

  [FreeFunction("UnityEngineObjectBindings::IsPersistent")]
  internal extern static bool IsPersistent(Object obj);

  [FreeFunction("UnityEngineObjectBindings::SetName")] extern static void
  SetName(Object obj, string name);
