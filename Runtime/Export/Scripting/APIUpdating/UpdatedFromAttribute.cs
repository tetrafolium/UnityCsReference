...
internal struct MovedFromAttributeData {
	public void Set(bool autoUpdateAPI, string sourceNamespace = null,
...
//  classes being renamed so that it can
//      read data that was saved by an earlier version.
//
// Notes:
//  - IMPORTANT: the APIUpdater does **NOT** support renaming a klass through
//  this attribute. To do so, a custom
...
// If automatic udpate of scripts is not required (accepted breaking
// change), then this attribute can be set to be ignored by the APIUpdater
// tool (autoUpdateAPI == false), in which case any combination of changes
// to [assembly, namespace, class name] are supported, but *ONLY* by the
// serialization system. Note: any null string is interpreted as "has not
// changed" and it's actual value will be extracted from the decorated type.
public MovedFromAttribute(bool autoUpdateAPI, string sourceNamespace = null,
                          string sourceAssembly = null,
                          string sourceClassName = null) {
