...
public delegate void AdvertisingIdentifierCallback(string advertisingId,
...
// Use this delegate type with RegisterLogCallback to monitor what gets
// logged.
...
public static event LogCallback logMessageReceived {
	...
	remove { s_LogCallbackHandler -= value; }
}
...
public static event LogCallback logMessageReceivedThreaded {
	...
	remove { s_LogCallbackHandlerThreaded -= value; }
}

[RequiredByNativeCode]
private static void CallLogCallback(string logString, string stackTrace,
                                    LogType type, bool invokedOnMainThread) {
...
InvokeOnAdvertisingIdentifierCallback(string advertisingId,
                                      bool trackingEnabled) {
...
private static string ObjectToJSString(object o) {
	if (o == null) {
		...
		string s = o.ToString().Replace(
			"\\", "\\\\"); // escape \ into \\, JS vulnerability.
		s = s.Replace("\"", "\\\"");
		s = s.Replace("\n", "\\n");
		s = s.Replace("\r", "\\r");
		s = s.Replace("\u0000", ""); // String-terminator. JS vulnerability.
		s = s.Replace("\u2028",
		              ""); // Line-terminator via ecma-262-7.3 JS vulnerability.
		s = s.Replace("\u2029", ""); // Same as above
		return '"' + s + '"';
	} else if (o is Int32 || o is Int16 || o is UInt32 || o is UInt16 ||
	           o is Byte) {
		return o.ToString();
	} else if (o is Single) {
...
[Obsolete(
	 "Application.ExternalCall is deprecated. See https://docs.unity3d.com/Manual/webgl-interactingwithbrowserscripting.html for alternatives.")]
public static void ExternalCall(string functionName, params object[] args) {
	Internal_ExternalCall(BuildInvocationForArguments(functionName, args));
}

private static string BuildInvocationForArguments(string functionName,
                                                  params object[] args) {
...
static public void CaptureScreenshot(string filename, int superSize) {
	throw new NotSupportedException(
		      "Application.CaptureScreenshot is obsolete. Use ScreenCapture.CaptureScreenshot instead.");
}
...
[System.Obsolete(
	 "Application.RegisterLogCallbackThreaded is deprecated. Use Application.logMessageReceivedThreaded instead.")]
public static void
RegisterLogCallbackThreaded(Application.LogCallback handler) {
