...
public static void AppendHash(ref Hash128 inHash, ref Hash128 outHash) {
	unsafe {
		fixed(Hash128 *outH = &outHash) fixed(Hash128 *message = &inHash) {
			HashUnsafeUtilities.ComputeHash128(message, (ulong) sizeof(Hash128),
			                                   outH);
		}
	}
}

public static void QuantisedMatrixHash(ref Matrix4x4 value,
                                       ref Hash128 hash) {
	unsafe {
		fixed(Hash128 *h = &hash) {
			...
			HashUnsafeUtilities.ComputeHash128(quantisedMatrix, sizeof(int) * 16,
			                                   h);
		}
	}
}
...
public static unsafe void ComputeHash128<T>(ref T value,
                                            ref Hash128 hash) where T
: struct {
...
public static unsafe void ComputeHash128(void *data, ulong dataSize,
...
public static unsafe void ComputeHash128(void *data, ulong dataSize,
                                         Hash128 *hash) {
	// We don't have ref return in C# 6.0, and we don't want to expose an unsafe
	// pointer to Hash128's internal data. Se we make a copy and then recreate
	// the hash in the end. It won't create GCable memory, but it introduce copy
	// operations.
