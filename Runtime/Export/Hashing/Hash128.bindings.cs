...
public unsafe struct Hash128 : IComparable,
                               IComparable<Hash128>,
                               IEquatable<Hash128> {
  ...
  internal ulong u64_0 {
    get { fixed(uint *ptr0 = &m_u32_0) return *(ulong *) ptr0; }
  }

  internal ulong u64_1 {
    get { fixed(uint *ptr1 = &m_u32_2) return *(ulong *) ptr1; }
  }

  public bool isValid =>
      m_u32_0 != 0 || m_u32_1 != 0 || m_u32_2 != 0 || m_u32_3 != 0;
  ...
  public override string ToString() { return Hash128ToStringImpl(this); }
  ...
  static extern void ComputeFromPtr(IntPtr data, int start, int count,
                                    int elemSize, ref Hash128 hash);
  [FreeFunction("ComputeHash128FromScriptArray", IsThreadSafe = true)]
  static extern void ComputeFromArray(Array data, int start, int count,
                                      int elemSize, ref Hash128 hash);
  ...
  public static Hash128 Compute<T>(NativeArray<T> data) where T : struct {
    var h = new Hash128();
    ComputeFromPtr((IntPtr) data.GetUnsafeReadOnlyPtr(), 0, data.Length,
                   UnsafeUtility.SizeOf<T>(), ref h);
    return h;
  }

  public static Hash128 Compute<T>(NativeArray<T> data, int start,
                                   int count) where T : struct {
    if (start < 0 || count < 0 || start + count > data.Length)
      throw new ArgumentOutOfRangeException(
          $"Bad start/count arguments (start:{start} count:{count})");
    var h = new Hash128();
    ComputeFromPtr((IntPtr) data.GetUnsafeReadOnlyPtr(), start, count,
                   UnsafeUtility.SizeOf<T>(), ref h);
    return h;
  }
  ...
  public static Hash128 Compute<T>(T[] data) where T : struct {
    if (!UnsafeUtility.IsArrayBlittable(data))
      throw new ArgumentException(
          $"Array passed to {nameof(Compute)} must be blittable.\n{UnsafeUtility.GetReasonForArrayNonBlittable(data)}");
    var h = new Hash128();
    ComputeFromArray(data, 0, data.Length, UnsafeUtility.SizeOf<T>(), ref h);
    return h;
  }

  public static Hash128 Compute<T>(T[] data, int start, int count) where T
      : struct {
    if (!UnsafeUtility.IsArrayBlittable(data))
      throw new ArgumentException(
          $"Array passed to {nameof(Compute)} must be blittable.\n{UnsafeUtility.GetReasonForArrayNonBlittable(data)}");
  ...
  public static Hash128 Compute<T>(List<T> data) where T : struct {
    if (!UnsafeUtility.IsGenericListBlittable<T>())
      throw new ArgumentException(
          $"List<{typeof(T)}> passed to {nameof(Compute)} must be blittable.\n{UnsafeUtility.GetReasonForGenericListNonBlittable<T>()}");
    var h = new Hash128();
    ComputeFromArray(NoAllocHelpers.ExtractArrayFromList(data), 0, data.Count,
                     UnsafeUtility.SizeOf<T>(), ref h);
    return h;
  }

  public static Hash128 Compute<T>(List<T> data, int start, int count) where T
      : struct {
    if (!UnsafeUtility.IsGenericListBlittable<T>())
      throw new ArgumentException(
          $"List<{typeof(T)}> passed to {nameof(Compute)} must be blittable.\n{UnsafeUtility.GetReasonForGenericListNonBlittable<T>()}");
    if (start < 0 || count < 0 || start + count > data.Count)
      throw new ArgumentOutOfRangeException(
          $"Bad start/count arguments (start:{start} count:{count})");
    var h = new Hash128();
    ComputeFromArray(NoAllocHelpers.ExtractArrayFromList(data), start, count,
                     UnsafeUtility.SizeOf<T>(), ref h);
    return h;
  }
  ...
  public void Append(string data) { ComputeFromString(data, ref this); }

  public void Append<T>(NativeArray<T> data) where T : struct {
    ComputeFromPtr((IntPtr) data.GetUnsafeReadOnlyPtr(), 0, data.Length,
                   UnsafeUtility.SizeOf<T>(), ref this);
  }

  public void Append<T>(NativeArray<T> data, int start, int count) where T
      : struct {
    if (start < 0 || count < 0 || start + count > data.Length)
      throw new ArgumentOutOfRangeException(
          $"Bad start/count arguments (start:{start} count:{count})");
  ...
  public void Append<T>(T[] data) where T : struct {
    if (!UnsafeUtility.IsArrayBlittable(data))
      throw new ArgumentException(
          $"Array passed to {nameof(Append)} must be blittable.\n{UnsafeUtility.GetReasonForArrayNonBlittable(data)}");
  ...
  public void Append<T>(List<T> data) where T : struct {
    if (!UnsafeUtility.IsGenericListBlittable<T>())
      throw new ArgumentException(
          $"List<{typeof(T)}> passed to {nameof(Append)} must be blittable.\n{UnsafeUtility.GetReasonForGenericListNonBlittable<T>()}");
    ComputeFromArray(NoAllocHelpers.ExtractArrayFromList(data), 0, data.Count,
                     UnsafeUtility.SizeOf<T>(), ref this);
  }
  ...
  public unsafe void Append(int val) { ShortHash4((uint) val); }

  public unsafe void Append(float val) { ShortHash4(*(uint *) & val); }
  ...
  public bool Equals(Hash128 obj) { return this == obj; }

  public override int GetHashCode() {
    return m_u32_0.GetHashCode() ^ m_u32_1.GetHashCode() ^
           m_u32_2.GetHashCode() ^ m_u32_3.GetHashCode();
  }
  ...
  public static bool operator ==(Hash128 hash1, Hash128 hash2) {
    return (hash1.m_u32_0 == hash2.m_u32_0 && hash1.m_u32_1 == hash2.m_u32_1 &&
            hash1.m_u32_2 == hash2.m_u32_2 && hash1.m_u32_3 == hash2.m_u32_3);
  }
  ...
  static unsafe void ShortEnd(ref ulong h0, ref ulong h1, ref ulong h2,
                              ref ulong h3) {
    h3 ^= h2;
    Rot64(ref h2, 15);
    h3 += h2;
