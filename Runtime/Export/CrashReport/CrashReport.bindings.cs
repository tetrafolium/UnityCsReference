...
public sealed class CrashReport {
  ...
  static void PopulateReports() {
    lock(reportsLock) {
      ...
      foreach (var id in ids) {
        ...
        DateTime time =
            new DateTime(1970, 1, 1).AddSeconds(secondsSinceUnixEpoch);
  ...
  public static CrashReport[] reports {
    get {
      PopulateReports();
      lock(reportsLock) { return internalReports.ToArray(); }
    }
  }
  ...
  public void Remove() {
    if (RemoveReport(id)) {
      lock(reportsLock) { internalReports.Remove(this); }
    }
  }

  [FreeFunction(
      Name = "CrashReport_Bindings::GetReports",
      IsThreadSafe = true)] extern private static string[] GetReports();

  [FreeFunction(Name = "CrashReport_Bindings::GetReportData",
                IsThreadSafe = true)] extern private static string
  GetReportData(string id, out double secondsSinceUnixEpoch);

  [FreeFunction(Name = "CrashReport_Bindings::RemoveReport",
                IsThreadSafe = true)] extern private static bool
  RemoveReport(string id);
}
}
