...
public partial struct Vector3 : IEquatable<Vector3>,
  ...
  public static Vector3 MoveTowards(Vector3 current, Vector3 target,
    ...
    // avoid vector ops because current scripting backends are terrible at
    // inlining
  ...
  SmoothDamp(Vector3 current, Vector3 target, ref Vector3 currentVelocity,
             float smoothTime,
             [ uei.DefaultValue("Mathf.Infinity") ] float maxSpeed,
  ...
  public static Vector3 Project(Vector3 vector, Vector3 onNormal) {
    ...
    if (sqrMag < Mathf.Epsilon)
      ...
      return new Vector3(onNormal.x * dot / sqrMag, onNormal.y * dot / sqrMag,
  ...
  // -180 degrees. If you imagine the from and to vectors as lines on a piece of
  // paper, both originating from the same point, then the /axis/ vector would
  // point up out of the paper. The measured angle between the two vectors would
  // be positive in a clockwise direction and negative in an anti-clockwise
  // direction.
  public static float SignedAngle(Vector3 from, Vector3 to, Vector3 axis) {
    ...
    float sign =
        Mathf.Sign(axis.x * cross_x + axis.y * cross_y + axis.z * cross_z);
  ...
  public static Vector3 ClampMagnitude(Vector3 vector, float maxLength) {
    ...
    if (sqrmag > maxLength * maxLength) {
      ...
      // of float precision. without this, the intermediate result can be of
      // higher precision, which changes behavior.
      float normalized_x = vector.x / mag;
      float normalized_y = vector.y / mag;
      float normalized_z = vector.z / mag;
      return new Vector3(normalized_x * maxLength, normalized_y * maxLength,
  ...
  public static Vector3 Min(Vector3 lhs, Vector3 rhs) {
    return new Vector3(Mathf.Min(lhs.x, rhs.x), Mathf.Min(lhs.y, rhs.y),
  ...
  public static Vector3 Max(Vector3 lhs, Vector3 rhs) {
    return new Vector3(Mathf.Max(lhs.x, rhs.x), Mathf.Max(lhs.y, rhs.y),
  ...
  static readonly Vector3 positiveInfinityVector = new Vector3(
      float.PositiveInfinity, float.PositiveInfinity, float.PositiveInfinity);
  static readonly Vector3 negativeInfinityVector = new Vector3(
  ...
  [MethodImpl(MethodImplOptionsEx.AggressiveInlining)]
  public static Vector3
  operator *(Vector3 a, float d) {
    return new Vector3(a.x * d, a.y * d, a.z * d);
  }
  // Multiplies a vector by a number.
  [MethodImpl(MethodImplOptionsEx.AggressiveInlining)]
  public static Vector3
  operator *(float d, Vector3 a) {
    return new Vector3(a.x * d, a.y * d, a.z * d);
  }
  ...
  public string ToString(string format, IFormatProvider formatProvider) {
    ...
    return UnityString.Format(
        "({0}, {1}, {2})", x.ToString(format, formatProvider),
