...
public partial struct Mathf {
  ...
  public static readonly float Epsilon =
      UnityEngineInternal.MathfInternal
          ...
          .FloatMinNormal : UnityEngineInternal.MathfInternal.FloatMinDenormal;
  ...
  // Same as ::ref::Lerp but makes sure the values interpolate correctly when
  ...
  static public float MoveTowardsAngle(float current, float target,
                                       float maxDelta) {
  ...
  public static bool Approximately(float a, float b) {
    ...
    // thus we could use an epsilon of 0.000001f for comparing values close
    // to 1. We multiply this epsilon by the biggest magnitude of a and b.
  ...
  public static float
  SmoothDamp(float current, float target, ref float currentVelocity,
             float smoothTime,
  ...
  public static float SmoothDampAngle(float current, float target,
    ...
    float deltaTime = Time.deltaTime;
    return SmoothDampAngle(current, target, ref currentVelocity, smoothTime,
                           maxSpeed, deltaTime);
  }
  ...
  // Gradually changes an angle given in degrees towards a desired goal angle
  // over time.
  public static float
  SmoothDampAngle(float current, float target, ref float currentVelocity,
                  float smoothTime,
                  [ uei.DefaultValue("Mathf.Infinity") ] float maxSpeed,
                  [ uei.DefaultValue("Time.deltaTime") ] float deltaTime) {
  ...
  internal static bool LineIntersection(Vector2 p1, Vector2 p2, Vector2 p3,
                                        Vector2 p4, ref Vector2 result) {
    float bx = p2.x - p1.x;
    float by = p2.y - p1.y;
    float dx = p4.x - p3.x;
    float dy = p4.y - p3.y;
  ...
  internal static bool LineSegmentIntersection(Vector2 p1, Vector2 p2,
  ...
  static internal long RandomToLong(System.Random r) {
    ...
    r.NextBytes(buffer);
    return (long)(System.BitConverter.ToUInt64(buffer, 0) &
