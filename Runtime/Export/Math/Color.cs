...
public struct Color : IEquatable<Color>,
  ...
  public string ToString(string format, IFormatProvider formatProvider) {
    ...
    return UnityString.Format(
        "RGBA({0}, {1}, {2}, {3})", r.ToString(format, formatProvider),
  ...
  // Subtracts color /b/ from color /a/. Each component is subtracted
  // separately.
  public static Color operator -(Color a, Color b) {
    return new Color(a.r - b.r, a.g - b.g, a.b - b.b, a.a - b.a);
  }
  ...
  // Multiplies color /a/ by the float /b/. Each color component is scaled
  // separately.
  ...
  // Divides color /a/ by the float /b/. Each color component is scaled
  // separately.
  public static Color operator /(Color a, float b) {
    return new Color(a.r / b, a.g / b, a.b / b, a.a / b);
  }
  ...
  // Interpolates between colors /a/ and /b/ by /t/ without clamping the
  // interpolant
  ...
  // Returns new color that has RGB components multiplied, but leaving alpha
  // untouched.
  ...
  public static Color yellow {
    get { return new Color(1F, 235F / 255F, 4F / 255F, 1F); }
  }
  ...
  public static Color gray {
    get { return new Color(.5F, .5F, .5F, 1F); }
  }
  ...
  public Color linear {
    get {
      return new Color(Mathf.GammaToLinearSpace(r), Mathf.GammaToLinearSpace(g),
  ...
  public Color gamma {
    get {
      return new Color(Mathf.LinearToGammaSpace(r), Mathf.LinearToGammaSpace(g),
  ...
  public static void RGBToHSV(Color rgbColor, out float H, out float S,
    ...
    if ((rgbColor.b > rgbColor.g) && (rgbColor.b > rgbColor.r))
      RGBToHSVHelper((float) 4, rgbColor.b, rgbColor.r, rgbColor.g, out H,
                     out S, out V);
    // when green is highest valued
    else if (rgbColor.g > rgbColor.r)
      RGBToHSVHelper((float) 2, rgbColor.g, rgbColor.b, rgbColor.r, out H,
                     out S, out V);
    // when red is highest valued
    else
      RGBToHSVHelper((float) 0, rgbColor.r, rgbColor.g, rgbColor.b, out H,
                     out S, out V);
  }

  static void RGBToHSVHelper(float offset, float dominantcolor, float colorone,
                             float colortwo, out float H, out float S,
                             out float V) {
    V = dominantcolor;
    ...
    if (V != 0) {
      ...
      if (diff != 0) {
        // S = max-min/max
        S = diff / V;
        // H = hue is offset by X, and is the difference between the two
        // smallest colors
        H = offset + ((colorone - colortwo) / diff);
      } else {
        // S = 0 when the difference is zero
        S = 0;
        // H = 4 + (R-G) hue is offset by 4 when blue, and is the difference
        // between the two smallest colors
