...
internal static extern void Internal_Log(LogType level, LogOption options,
...
// Draws a line from the /point/ start to /end/ with color for a duration of
// time and with or without depth testing. If duration is 0 then the line is
// rendered 1 frame.
...
// Draws a line from /start/ to /start/ + /dir/ with color for a duration of
// time and with or without depth testing. If duration is 0 then the line is
// rendered 1 frame.
public static void DrawRay(Vector3 start, Vector3 dir,
                           [ DefaultValue("Color.white") ] Color color,
...
ExtractStackTraceNoAlloc(byte *buffer, int bufferMax, string projectFolder);
...
public static void LogFormat(UnityEngine.Object context, string format,
                             params object[] args) {
	unityLogger.LogFormat(LogType.Log, context, format, args);
}

public static void LogFormat(LogType logType, LogOption logOptions,
                             Object context, string format,
                             params object[] args) {
...
public static void LogErrorFormat(UnityEngine.Object context, string format,
                                  params object[] args) {
...
internal static extern void LogPlayerBuildError(string message, string file,
                                                int line, int column);
...
public static void LogWarningFormat(UnityEngine.Object context, string format,
                                    params object[] args) {
...
public static void Assert(bool condition) {
	if (!condition)
		unityLogger.Log(LogType.Assert, (object) "Assertion failed");
}
...
// Same as Assert (bool, object) but can't deprecate because the script
// updater won't work
[Conditional(Assertions.Assert.UNITY_ASSERTIONS)]
public static void Assert(bool condition, string message) {
	if (!condition)
		unityLogger.Log(LogType.Assert, (object) message);
}
...
// Same as Assert (bool, object, Object) but can't deprecate because the
// script updater won't work
[Conditional(Assertions.Assert.UNITY_ASSERTIONS)]
...
[Conditional(Assertions.Assert.UNITY_ASSERTIONS)]
public static void AssertFormat(bool condition, string format,
                                params object[] args) {
	if (!condition)
...
[Conditional(Assertions.Assert.UNITY_ASSERTIONS)]
public static void AssertFormat(bool condition, Object context, string format,
                                params object[] args) {
	if (!condition)
...
[Conditional(Assertions.Assert.UNITY_ASSERTIONS)]
public static void LogAssertionFormat(Object context, string format,
                                      params object[] args) {
	unityLogger.LogFormat(LogType.Assert, context, format, args);
}
...
GetDiagnosticSwitches(List<DiagnosticSwitch> results);

[NativeThrows]
internal static extern object GetDiagnosticSwitch(string name);

[NativeThrows]
internal static extern void SetDiagnosticSwitch(string name, object value,
                                                bool setPersistent);

[RequiredByNativeCode]
internal static bool CallOverridenDebugHandler(Exception exception,
                                               Object obj) {
...
internal static extern void LogSticky(int identifier, LogType logType,
                                      LogOption logOptions, string message,
                                      Object context = null);
