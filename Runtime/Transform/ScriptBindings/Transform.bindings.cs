...
public partial class Transform : Component,
  ...
  public extern void SetPositionAndRotation(Vector3 position,
                                            Quaternion rotation);
  ...
  Rotate(Vector3 eulers,
         [ UnityEngine.Internal.DefaultValue("Space.Self") ] Space relativeTo) {
  ...
  // Applies a rotation of /zAngle/ degrees around the z axis, /xAngle/ degrees
  // around the x axis, and /yAngle/ degrees around the y axis (in that order).
  public void
  ...
  Rotate(Vector3 axis, float angle,
         [ UnityEngine.Internal.DefaultValue("Space.Self") ] Space relativeTo) {
    if (relativeTo == Space.Self)
      RotateAroundInternal(transform.TransformDirection(axis),
                           angle * Mathf.Deg2Rad);
    else
  ...
  LookAt(Transform target,
         [ UnityEngine.Internal.DefaultValue("Vector3.up") ] Vector3 worldUp) {
    if (target)
  ...
  // opposite of Transform.TransformDirection.
  ...
  // opposite of Transform.TransformVector.
  ...
  // Transforms /position/ from world space to local space. The opposite of
  // Transform.TransformPoint.
  public extern Vector3 InverseTransformPoint(Vector3 position);

  // Transforms the position /x/, /y/, /z/ from world space to local space. The
  ...
  public extern int childCount { [NativeMethod("GetChildrenCount")] get; }
  ...
  internal extern void MoveAfterSibling(Transform transform,
  ...
  private static extern Transform FindRelativeTransformWithPath(
      ...
      [ UnityEngine.Internal.DefaultValue("false") ] bool isActiveOnly);
  ...
  //*undocumented*
  [Obsolete(
      "FindChild has been deprecated. Use Find instead (UnityUpgradable) -> Find([mscorlib] System.String)",
      false)]
  ...
  public IEnumerator GetEnumerator() { return new Transform.Enumerator(this); }
  ...
  //*undocumented* DEPRECATED
  [Obsolete(
      "warning use Transform.childCount instead (UnityUpgradable) -> Transform.childCount",
      false)]
